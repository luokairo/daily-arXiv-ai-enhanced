...
- [多模态生成](#direction-multimodal_generation) [Total: 61]
  - [3D 生成与重建](#direction-multimodal_generation-three_d_generation) [Total: 14]
  - [合成数据与数据集蒸馏](#direction-multimodal_generation-aigc_synthetic_data) [Total: 2]
  ...
  - [生成模型加速与高效化](#direction-multimodal_generation-efficient_generation) [Total: 12]
  ...
  - [视频世界模型](#direction-multimodal_generation-video_world_model) [Total: 2]
  - [音频与多模态联合生成](#direction-multimodal_generation-audio_av_generation) [Total: 2]
  - [领域内容生成应用](#direction-multimodal_generation-aigc_domain_application) [Total: 2]
- [AIGC](#direction-aigc) [Total: 101]
  - [AI 智能体与生成工作流](#direction-aigc-aigc_agent_workflow) [Total: 21]
  - [AIGC 检测与溯源](#direction-aigc-aigc_detection_attribution) [Total: 8]
  - [AIGC 评测与基准](#direction-aigc-aigc_evaluation_benchmark) [Total: 18]
  - [个性化与对话生成](#direction-aigc-aigc_personalization_persona) [Total: 1]
  - [创意与艺术生成](#direction-aigc-aigc_creative_art) [Total: 1]
  - [合成数据与数据集蒸馏](#direction-aigc-aigc_synthetic_data) [Total: 1]
  - [概念擦除与遗忘学习](#direction-aigc-aigc_unlearning_erasure) [Total: 4]
  - [模型鲁棒性与隐私](#direction-aigc-aigc_robustness_privacy) [Total: 2]
  - [水印与版权保护](#direction-aigc-aigc_watermark_copyright) [Total: 1]
  - [生成内容安全与治理](#direction-aigc-aigc_safety_governance) [Total: 15]
  - [生成模型对齐与奖励](#direction-aigc-generation_alignment) [Total: 1]
  - [视频音频生成应用](#direction-aigc-aigc_video_audio_creation) [Total: 1]
  - [领域内容生成应用](#direction-aigc-aigc_domain_application) [Total: 27]
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 61]
  ...
  - [多模态评测与基准](#direction-multimodal_understanding_generation-multimodal_benchmark) [Total: 15]
  - [多模态高效化与轻量化](#direction-multimodal_understanding_generation-efficient_multimodal) [Total: 4]
  - [概念擦除与遗忘学习](#direction-multimodal_understanding_generation-aigc_unlearning_erasure) [Total: 1]
...
TL;DR: 提出 Geometry-Aware Representation Denoising (GARD) 框架，在前馈式3D重建模型的特征空间中执行扩散去噪，以提升多视图3D重建在退化成像条件下的鲁棒性，同时可恢复高质量RGB图像。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式3D重建模型通常在理想无退化条件下训练和评估，而真实场景中观测数据常存在各种退化（如噪声、模糊等），导致模型性能显著下降。提升多视图3D重建在退化条件下的鲁棒性是一个重要但尚未充分解决的问题。

Method: GARD 框架利用扩散模型在3D重建器的几何感知特征空间中直接进行多视图恢复，而非在像素空间操作。通过利用3D重建器本身的几何感知特征表示来恢复准确的场景几何，并额外使用RGB图像解码器将精炼后的特征恢复为高质量RGB图像，实现3D几何与高质量图像的同步恢复。

Result: 在 Depth Anything 3 (DA3) 基准上的综合实验证明了 GARD 框架在退化条件下多视图3D重建的有效性，能够同时恢复场景几何和高质量图像。

Conclusion: GARD 通过在3D重建模型特征空间中进行扩散去噪，有效提升了多视图3D重建在退化条件下的鲁棒性，为真实场景中的鲁棒3D重建提供了新思路。

Classification reason: 该论文研究多视图3D重建在退化条件下的鲁棒性，属于3D场景重建范畴，与[多模态生成]方向下的[3D 生成与重建]（CANONICAL）高度匹配。

Importance reason: Focuses on 3D reconstruction robustness, not directly aligned with priority subtopics like video/image generation or world models. Signals: 3D reconstruction, diffusion in feature space, robustness to degradation, not autoregressive, not video generation
...
### [2] [Joint Instance Segmentation and Geometric Attribute Regression for Roof Structures in Aerial Imagery](https://arxiv.org/abs/2605.26370)
...
Importance: 普通相关 (45.98) - 未入选精读

TL;DR: 本文提出一种联合实例分割与几何属性回归的方法，从单张航拍正射影像中预测屋顶掩码及建筑高度、坡度、方位角，并利用预测结果重建简化3D建筑模型（LoD2）。方法基于Mask R-CNN扩展属性回归分支，引入条件方位角损失和对数归一化高度表示，在荷兰大规模航拍数据集上取得了较好的精度。


<details>
  <summary>Details</summary>
Motivation: 从单张航拍图像自动重建3D建筑模型（LoD2级别）具有重要的实际应用价值，但现有方法通常依赖多视角图像或LiDAR数据，成本高昂。本文旨在仅使用单张航拍正射影像，通过联合学习屋顶实例分割和连续几何属性回归，实现低成本、可扩展的3D建筑模型重建。

Method: 基于Mask R-CNN框架，扩展一个属性回归分支（attribute regression branch），同时预测每个屋顶实例的掩码及三个连续几何属性（建筑高度、屋顶坡度、屋顶方位角）。提出两个关键创新：(1) 条件方位角损失（conditional azimuth loss），对平坦屋顶（方位角标签噪声大）的监督进行抑制；(2) 对数归一化高度表示（log-normalized height representation），处理建筑高度分布严重偏斜的问题。使用DINOv3 ConvNeXt-Base作为骨干网络，在荷兰3DBAG LiDAR数据集上训练。

Result: 在荷兰大规模航拍数据集上，方法实现了屋顶坡度MAE约4度、方位角MAE约7度、建筑高度MAE约1米，实例分割AP50为0.566。预测的掩码和属性足以从单张航拍图像重建简化3D建筑模型（LoD2）。

Conclusion: 本文证明了从单张航拍正射影像联合预测屋顶实例分割和几何属性是可行的，且预测结果足以重建LoD2级别的3D建筑模型，为大规模低成本3D城市建模提供了有效方案。未来工作可进一步改进对复杂屋顶结构的处理。

Classification reason: 该论文从单张航拍图像预测屋顶分割掩码及建筑高度、坡度、方位角等几何属性，并用于重建简化3D建筑模型（LoD2），属于从2D图像到3D建筑结构的生成与重建任务，与[多模态生成]方向下的[3D 生成与重建]（three_d_generation）高度匹配。

Importance reason: Focuses on aerial roof segmentation and 3D reconstruction, which is outside the user's core interests in generative video/image models and efficient inference. Signals: Aerial imagery, roof segmentation, geometric attribute regression, 3D building reconstruction, not generative video/image
...
TL;DR: Underwater360 是一个物理感知的全景3DGS框架，用于水下全景场景重建。它通过球面光线投射解决全景畸变，并利用物理建模显式解耦场景辐射与水下介质效应（吸收、散射），实现高质量水下新视角合成与场景外观恢复。


<details>
  <summary>Details</summary>
Motivation: 水下场景重建对沉浸式探索至关重要，但传统方法受限于水下介质吸收/散射效应以及常规相机有限的视场角。全景成像结合3DGS虽有潜力，但传统3DGS难以处理球面投影畸变和水下介质退化，亟需专门针对水下全景场景的重建框架。

Method: 提出Underwater360框架，包含三个核心模块：(1) 全景高斯泼溅模块，直接在球面相机空间进行光线投射而非2D投影近似，减少360°视场下的几何畸变；(2) 基于物理的外观-介质建模模块，使用姿态条件的外观嵌入显式解耦场景固有辐射与深度依赖的后向散射和衰减；(3) 构建包含合成和真实场景的全景水下基准数据集。

Result: 在合成和真实水下全景场景上的大量实验表明，Underwater360在新视角合成和场景外观恢复方面均优于现有方法，在复杂水下环境中实现了更优的渲染质量和跨视角一致性。

Conclusion: Underwater360通过物理感知的全景3DGS框架有效解决了水下全景场景重建中的几何畸变和介质退化问题，为水下场景的沉浸式探索提供了高质量解决方案。

Classification reason: 该论文提出基于全景图像的水下场景重建方法，使用3D高斯泼溅（3DGS）进行360度场景重建与渲染，属于3D生成与重建方向，与[CANONICAL] subtopic "3D 生成与重建"高度匹配。

Importance reason: Niche underwater 3D reconstruction with limited cross-application to user's priority topics. Signals: 3D Gaussian Splatting, panoramic reconstruction, underwater physics modeling, novel view synthesis, scene restoration
...
### [4] [Sparse-LiDAR Prompting of Monocular Geometry Foundations: An Empirical Study Toward Long-Range Driving Depth](https://arxiv.org/abs/2605.26456)
...
TL;DR: 本文提出 SLIM，首次将 MoGe-2 点图基础模型适配为接受真正稀疏 LiDAR 输入，通过部分卷积稀疏编码器和多尺度融合网络，在长距离驾驶场景（50-150m）下将深度估计绝对相对误差降低约 39-51%。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏LiDAR提示的深度基础模型（如PromptDA等）在室内场景或KITTI标准80米截断范围内表现良好，但在长距离驾驶场景（50-150米）缺乏系统性的距离分层评估；且已有方法基于视差基础模型并依赖预插值稠密先验，尚未探索在点图基础模型（如MoGe-2）上进行真正稀疏LiDAR注入。

Method: SLIM 将 MoGe-2 点图基础模型改造为接受稀疏 LiDAR 输入：设计部分卷积稀疏编码器处理稀疏点云，并引入多尺度融合网络，在五个尺度上将 LiDAR 特征注入点图解码头。采用密度无关训练策略，随机注入比例在 [0.005, 0.30] 范围内，使单一模型适应不同输入密度。

Result: 在 Virtual KITTI 和 CARLA 数据集上，SLIM 在 100-150 米距离范围内将 MoGe-2 基线的绝对相对误差降低约 39-51%。六种注入比例的消融实验表明，部分卷积注入在 Virtual KITTI 上所有六种设置下均改善了 AbsRel 和 RMSE；在 CARLA 上，AbsRel 在六种设置中有五种改善，RMSE 在三种设置下改善。

Conclusion: SLIM 证明了在点图基础模型上进行真正稀疏 LiDAR 注入的有效性，为长距离驾驶深度估计提供了新的技术路径，且密度无关训练策略使模型具备良好的泛化能力。

Classification reason: 该论文研究稀疏LiDAR提示下的单目深度估计，属于3D场景几何感知与深度重建范畴，与 canonical 子主题 "3D 生成与重建"（three_d_generation）中的深度估计与场景重建方向高度相关。

Importance reason: Focuses on LiDAR-prompted depth estimation for autonomous driving, which is outside the user's priority subtopics and has negative keyword overlap. Signals: Sparse LiDAR injection, long-range depth estimation, MoGe-2 adaptation, driving scene focus, partial-convolution encoder
...
TL;DR: BrickAnything 是一个几何条件自回归框架，以点云为统一几何接口，通过结构感知树状 tokenization 将3D形状转化为可物理搭建的砖块序列，并引入偏好对齐、有效性约束解码和自适应回滚机制来保证结构稳定性和几何保真度。


<details>
  <summary>Details</summary>
Motivation: 现有砖块生成方法要么依赖启发式优化（在目标形状不满足预定义约束时容易失败），要么生成砖块序列时没有显式建模底层3D几何和装配关系。需要一种能处理任意3D形状、同时保证物理可搭建性的砖块生成方法。

Method: 提出 BrickAnything 框架：(1) 以点云作为统一几何接口，支持多种3D表示输入；(2) 结构感知树状 tokenization，通过局部附着关系表示砖块结构，使序列生成更符合物理搭建过程；(3) 自回归 Transformer 预测砖块序列；(4) 偏好对齐后训练（preference-based alignment）、有效性约束解码和自适应回滚机制，提升稳定性和几何保真度。

Result: 实验表明 BrickAnything 能生成几何保真且物理可实现的砖块结构，提出的树状 tokenization 相比传统排序策略有效减少了回滚和重生成次数。

Conclusion: BrickAnything 通过结构感知 tokenization 和多种约束机制，成功实现了从任意3D形状到可搭建砖块结构的端到端生成，为离散部件约束下的3D生成提供了新范式。

Classification reason: 该论文从3D形状生成可物理搭建的砖块结构，属于3D物体生成范畴，与 canonical subtopic "3D 生成与重建" 高度匹配。

Importance reason: Novel structure-aware tokenization for 3D brick generation, but outside core video/image generation and world model priorities. Signals: 3D generation, autoregressive framework, structure-aware tokenization, preference alignment, geometry-conditioned generation
...
### [6] [$R^3$: 3D Reconstruction via Relative Regression](https://arxiv.org/abs/2605.26519)
...
TL;DR: 本文提出 R³，一种基于相对回归的3D重建方法。通过轻量级MLP预测置信度加权的相对约束，摆脱全局坐标系依赖，支持离线全上下文重建和因果有界内存流式重建，在离线与流式设置中均验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有前馈几何基础模型通常假设全局坐标系，这在长上下文和流式重建中成为瓶颈——网络需要维护任意的时间原点，且平移量随时间无界增长。作者希望提出一种不依赖全局坐标系的相对回归机制来解决这一问题。

Method: R³ 采用相对回归策略，使用轻量级MLP预测置信度加权的相对约束。这些置信度作为统一锚点：训练时用于加权损失，推理时用于引导位姿聚合。该方法同时支持全上下文离线重建和因果有界内存流式重建。

Result: 在离线与流式两种设置下的实验验证了相对机制的有效性，表明 R³ 能够处理长上下文和流式3D重建任务，克服了全局坐标系假设带来的局限性。

Conclusion: R³ 通过相对回归和置信度加权机制，有效解决了前馈几何基础模型在长上下文和流式重建中因全局坐标系依赖导致的性能瓶颈，为3D重建提供了一种更灵活、可扩展的范式。

Classification reason: 该论文提出基于相对回归的3D重建方法，属于3D场景/物体重建范畴，与[CANONICAL] subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Novel relative regression approach for 3D reconstruction, but outside user's priority subtopics and boosted keywords. Signals: 3D reconstruction, relative regression, streaming reconstruction, feed-forward geometry, confidence-weighted constraints
...
### [7] [TrackRef3D: Multi-View Consistent Track-then-Label for Open-World Referring Segmentation in 3D Gaussian Splatting](https://arxiv.org/abs/2605.26576)
...
TL;DR: TrackRef3D 提出了一种全自动的开放世界3D指代分割流水线，通过"轨迹感知语义共识模块"（TSCM）实现多视角一致的物体发现与语义标注解耦，无需人工标注即可在3DGS场景中完成指代分割，在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有3D指代分割方法依赖昂贵的逐场景人工标注和逐视角伪掩码生成，存在多视角不一致、对查询粒度泛化差等问题，亟需一种无需人工标注、多视角一致且能处理开放世界查询的自动化方法。

Method: 提出TrackRef3D全自动流水线，核心包括：(1) 轨迹感知语义共识模块（TSCM），通过同义词聚类和轨迹感知投票聚合跨视角预测，建立规范语义身份；(2) 可见性感知描述生成策略缓解歧义；(3) 混合训练策略（HTS）联合优化粗粒度类别语义和细粒度指代线索，使用多正例对比目标增强鲁棒性。

Result: 在多个基准数据集上的实验表明，TrackRef3D在开放世界3D指代分割任务上取得了最先进的性能，显著优于现有方法。

Conclusion: TrackRef3D通过解耦物体发现与语义标注，实现了无需人工标注的多视角一致3D指代分割，为3DGS场景中的开放世界语言引导分割提供了高效、可扩展的解决方案。

Classification reason: 该论文聚焦于3D高斯泼溅（3DGS）中的开放世界指代分割，属于3D场景理解与生成范畴，与[多模态生成]方向下的[3D 生成与重建]（three_d_generation）高度相关。同时涉及自然语言与3D场景的跨模态理解，也与[多模态理解与生成]方向相关。

Importance reason: Focuses on 3D segmentation in Gaussian Splatting, which is not a top priority subtopic and lacks strong alignment with user's key boost keywords. Signals: 3D Gaussian Splatting, open-world segmentation, multi-view consistency, automatic pipeline, no manual annotation
...
### [8] [DelowlightSplat: Feed-Forward Gaussian Splatting for Lowlight 3D Scene Reconstruction](https://arxiv.org/abs/2605.26629)
...
TL;DR: DelowlightSplat 提出了一种面向低光照场景的前馈式3D高斯泼溅重建框架，通过轻量级低光适配器增强特征匹配性，并结合代价体多视角推理直接预测干净的3D高斯，在低光照条件下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式3D高斯泼溅方法在低光照条件下因噪声、颜色偏移和不可靠的特征对应而失效，亟需一种低光照感知的重建框架。

Method: 构建可控多视角低光照基准（仅退化上下文视图，保持目标视图干净）；提出轻量级 Lowlight Adapter 进行残差增强以改善匹配性；结合代价体多视角推理直接预测干净的3D高斯。

Result: 在低光照条件下，DelowlightSplat 在 novel-view 合成质量上显著优于此前的前馈式方法和两阶段流水线。

Conclusion: 前馈式3D高斯泼溅在低光照场景下可以通过低光感知适配和代价体推理实现高质量重建，为实际低光照环境下的3D重建提供了有效方案。

Classification reason: 该论文提出低光照条件下的前馈式3D高斯泼溅重建方法，属于3D场景重建与渲染，与 canonical 子主题 "3D 生成与重建" 高度匹配。

Importance reason: Lowlight 3D reconstruction is niche and lacks direct alignment with priority subtopics like video generation or world models. Signals: 3D Gaussian splatting, lowlight adaptation, feed-forward reconstruction, novel-view synthesis, cost-volume inference
...
### [9] [I2PRef: Image-Driven Point Completion with Iterative Refinement](https://arxiv.org/abs/2605.26914)
...
TL;DR: 本文提出 I2PRef，一种图像驱动的点云补全方法。通过 Image-to-Point (I2P) 模块直接从单张 RGB 图像重建完整点云，再通过基于 Transformer 的 Point-to-Point (P2P) 精炼模块利用自注意力和交叉注意力迭代优化粗粒度输出。在 ShapeNet-ViPC 上取得了 SOTA 性能，Chamfer Distance 相对提升 12.3%。


<details>
  <summary>Details</summary>
Motivation: 现有图像引导的点云补全方法通常将图像作为辅助条件，依赖 3D 输入或辅助损失函数，未能充分利用图像中的几何信息。本文旨在将图像作为主要的几何信息来源，直接从单张 RGB 图像重建完整点云，无需 3D 输入。

Method: 提出 I2PRef 框架，包含两个核心模块：(1) Image-to-Point (I2P) 模块，直接从单张 RGB 图像重建粗粒度完整点云，使图像编码器学习丰富的几何表征；(2) Point-to-Point (P2P) 精炼模块，基于 Transformer 架构，通过点 token 与图像特征之间的自注意力和交叉注意力，迭代式地细化粗粒度输出，逐步恢复细粒度细节。

Result: 在 ShapeNet-ViPC 数据集上，I2PRef 在点云补全任务上取得了 SOTA 性能，Chamfer Distance 相对提升 12.3%，超越了现有方法。

Conclusion: 本文证明了将图像作为主要几何信息来源进行点云补全是有效的。I2P 模块提供了强几何先验，P2P 模块通过迭代精炼恢复细节，两者结合实现了优异的补全性能。

Classification reason: 该论文研究图像引导的点云补全（3D点云生成与重建），属于多模态生成方向下的 [CANONICAL] 子主题 "3D 生成与重建"（three_d_generation），该子主题涵盖3D物体生成与重建任务，与本论文高度匹配。

Importance reason: Focuses on 3D point cloud completion, which is not a priority subtopic and lacks strong alignment with user's key interests in video/image generation or efficient inference. Signals: 3D point cloud completion, image-driven reconstruction, iterative refinement, transformer-based refinement, SOTA on ShapeNet-ViPC
...
### [10] [DinoComplete: 3D Shape Completion with Distilled Semantic Priors and State Space Models](https://arxiv.org/abs/2605.26949)
...
TL;DR: DinoComplete 提出一种确定性3D形状补全框架，利用从DINO特征蒸馏的语义先验，结合多尺度体素Mamba模块进行几何与语义融合，在未见类别和真实噪声扫描上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 从部分扫描点云进行3D形状补全在未见类别和真实噪声场景下仍具挑战，仅靠几何信息难以推断缺失结构，需要引入语义先验来提升泛化性和鲁棒性。

Method: 首先构建与ShapeNet数据对齐的多视角DINO特征体素，训练学生网络从不完整形状中预测稠密语义特征；然后将几何与语义体素特征通过体素状态空间建模融合；最后引入多尺度体素Mamba模块，结合全网格和分块序列建模实现高效长程推理。

Result: 在未见ShapeNet类别和ScanNet真实物体上，DinoComplete 在补全质量上优于先前确定性和生成式方法，同时参数量更少、内存更低、推理更快。

Conclusion: 从视觉基础模型蒸馏语义先验能有效提升3D形状补全的泛化性和鲁棒性，体素Mamba模块为高效3D推理提供了可行方案。

Classification reason: 该论文研究3D形状补全（shape completion），属于3D物体生成与重建范畴，与 canonical subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Focuses on 3D shape completion, which is not a priority subtopic, and lacks direct connection to video/image generation or efficient inference. Signals: 3D shape completion, semantic priors from DINO, state space model, efficient inference, generalization to unseen categories
...
### [11] [AssetGen: Deployable 3D Asset Generation at Interactive Speed](https://arxiv.org/abs/2605.26137)
...
Importance: 普通相关 (54.01) - 未入选精读

TL;DR: AssetGen 是一个面向可部署交互式 3D 内容生成的系统，从单张参考图像在 30 秒（Flash 变体 14 秒）内生成高质量网格，包含烘焙法线、颜色纹理和可控多边形预算，支持实时渲染和移动端使用。通过粗到细的 VecSet 框架、多视图纹理生成、模型蒸馏与流水线并行化等协同设计，在视觉质量上达到与领先商业方案竞争的水平。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 生成工作主要关注高分辨率资产的质量，而忽视了用户体验和可部署性（deployability）。作者希望构建一个能在交互式速度下生成可直接用于实时渲染（包括移动端）的高质量 3D 资产的系统，支持 AI 辅助的 3D 内容创作工作流。

Method: 采用粗到细的 VecSet 框架生成几何体，在 GPU 上实现网格简化、清理和法线烘焙，以及快速并行 UV 展开。纹理生成采用多视图方式，随后进行反投影和 3D 修复。通过模型蒸馏、内核优化和流水线并行化协同加速整个系统。AssetGen Flash 变体进一步将延迟降至 14 秒。

Result: 在 30 秒内生成与领先商业方案（如 Meshy、Tripo、Rodin）视觉质量相当的结果，在 15 秒内可获得预览质量结果。通过大量自动化和盲人机评估验证了其竞争力。

Conclusion: AssetGen 证明了在交互式速度下生成可部署的 3D 资产是可行的，为 AI 辅助的 3D 内容创作提供了实用的系统方案，兼顾了质量、速度和部署需求。

Classification reason: 该论文提出 AssetGen，从单张参考图像在 30 秒内生成高质量 3D 网格（含烘焙法线、颜色纹理、可控多边形预算），属于 3D 物体生成与重建方向，完美匹配 [CANONICAL] subtopic "3D 生成与重建" (three_d_generation)。同时该工作关注可部署的交互式 3D 内容创作，也匹配 AIGC 方向。

Importance reason: Strong engineering for deployable 3D generation, but outside core priority subtopics. Signals: 3D asset generation, interactive speed, deployable system, VecSet framework, multi-view texture generation
...
### [12] [AnySurf: Any Surface Generation with Directed Edge](https://arxiv.org/abs/2605.26149)
...
TL;DR: AnySurf 是一个统一的 3D 表面生成框架，通过有向边增强的 Flexible Dual Grid (FDG-D) 表示保留法向方向信息，支持开放、封闭和混合 3D 表面的高质量生成，并构建了 Outfit3D 工业级服装与配件数据集。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 生成方法主要针对水密网格，对开放表面（如服装、鞋、配饰等工业 3D 内容）生成效果差，常产生双层面结构或法向/拓扑错误。领域特定的服装生成方法（如基于缝纫图案）缺乏可扩展性，无法泛化到其他开放表面类型。

Method: 提出基于有向边增强的 Flexible Dual Grid (FDG-D) 表示，通过有向网格边保留法向方向信息；提出 ROS-FT 后训练和轻量级 DE-Adapter（仅增加 1% 参数），促进有向边学习同时保持原有生成性能；构建 Outfit3D 数据集包含工业级服装和封闭配件。

Result: 实验结果表明 AnySurf 在开放、封闭和混合表面生成上均优于现有方法，生成网格质量更高，法向方向准确，适用于下游渲染、物理仿真和几何编辑等应用。

Conclusion: AnySurf 将服装建模转化为通用的 3D 生成任务，实现了开放/封闭/混合表面的统一高质量生成，为工业级 3D 内容生成提供了可扩展的解决方案。

Classification reason: 该论文提出 AnySurf，一个统一的 3D 表面生成框架，支持开放/封闭/混合 3D 表面生成，属于 3D 物体/网格生成范畴，与 canonical 子主题 "3D 生成与重建" 高度匹配。

Importance reason: Strong 3D surface generation method but not aligned with user's priority subtopics like video/image generation or world models. Signals: 3D surface generation, directed edge representation, open/closed/hybrid surfaces, Outfit3D dataset, industrial garment modeling
...
### [13] [Garment Particles: A 2D--3D Symmetric Garment Representation for Generation and Editing](https://arxiv.org/abs/2605.26391)
...
TL;DR: 本文提出 Garment Particles，一种 5D 点云表示联合编码 2D 服装纸样与 3D 几何，并基于整流流（Rectified Flow）框架实现从文本/图像/草图等高层输入生成服装，以及通过扩散后验采样进行 2D 纸样和 3D 几何编辑。同时引入 Particles-to-Pattern Flow 将生成粒子转换为曲线纸样用于仿真。


<details>
  <summary>Details</summary>
Motivation: 现有服装设计框架要么只支持从高层输入（文本/图像）生成服装，要么只支持在 2D 纸样或 3D 几何上进行直接编辑，无法同时覆盖从创意生成到精细编辑的完整设计流程。本文旨在提出一种统一的表示和方法，同时支持服装的直观生成与复杂编辑。

Method: 提出 Garment Particles 5D 点云表示（2D 纸样坐标 + 3D 几何坐标），联合编码服装的 2D 和 3D 信息。基于此表示，构建 Garment Particles Flow (GPF) 整流流框架：1）支持从文本、图像、草图等高层输入条件生成服装粒子；2）通过扩散后验采样支持多种编辑操作（插值、纸样编辑、点云/轮廓条件生成）。此外，提出 Particles-to-Pattern Flow 将生成的粒子转换为曲线纸样，用于物理仿真。

Result: 在多个数据集上达到 SOTA 服装生成效果，优于竞争基线。同时展示了多种编辑场景的有效性，包括服装插值、纸样编辑、点云条件和轮廓条件生成。

Conclusion: Garment Particles 作为一种统一的 2D-3D 对称服装表示，能够有效桥接服装生成与编辑两大任务，为实用服装设计提供了从创意到精细编辑的完整解决方案。

Classification reason: 该论文提出 Garment Particles，一种联合编码 2D 服装纸样与 3D 几何的 5D 点云表示，用于服装的生成与编辑。核心贡献在 3D 服装生成领域，属于 [CANONICAL] subtopic "3D 生成与重建"（three_d_generation）的范畴。同时，其生成与编辑应用也匹配 AIGC 方向。

Importance reason: Strong multimodal generation work with 2D-3D symmetric representation, but lacks direct alignment with top priority subtopics like autoregressive video generation or world models. Signals: multimodal_generation, 3D garment generation, rectified flow, diffusion posterior sampling, 2D-3D joint representation
...
### [14] [Object Pose and Shape Estimation for Grasping: Does it Work?](https://arxiv.org/abs/2605.26944)
...
TL;DR: 本文系统评估了物体姿态与形状估计方法在机器人抓取任务中的成熟度。实验表明，先估计姿态形状再采样抓取的模块化方法在各项指标上优于端到端抓取合成方法，尤其对小物体效果显著。但模块化方法在杂乱场景中性能下降，且可进一步结合视觉语言模型实现语言条件抓取。


<details>
  <summary>Details</summary>
Motivation: 物体姿态与形状估计方法近年来取得了显著进展，但它们在机器人抓取任务中的实际有效性尚未被系统评估。作者希望回答：这些方法是否足够成熟，以至于结合对极抓取采样后能超越端到端抓取合成方法？

Method: 将研究范围限定于平行夹爪、7-DoF抓取和单视图RGB(-D)输入。实现并比较了一种端到端抓取合成方法与三种模块化方法（先估计场景中所有物体的姿态和形状，再通过对极采样生成抓取）。模块化方法使用了两种姿态形状估计方式：编码器-解码器模型（如SAM3D、LRM、CRISP）和扩散模型（如InstantMesh、Zero123、SceneComplete）。

Result: 模块化方法在所有实验中均优于端到端方法，能够为小物体合成大量抓取姿态，而端到端方法在此类场景中失败。模块化方法的有效性依赖于姿态和形状估计的精度，在杂乱场景中性能会部分下降。此外，将单视图姿态形状估计方法与视觉语言模型结合，可实现语言条件抓取，性能与LERF-TOGO基线相当。

Conclusion: 物体姿态与形状估计方法在机器人抓取任务中已展现出实用价值，模块化方法优于端到端方法，但仍受限于杂乱场景下的估计精度。未来可进一步改进姿态形状估计的鲁棒性，并探索与VLM的深度融合。

Classification reason: 论文研究物体姿态与形状估计在抓取任务中的应用，涉及3D物体重建（如InstantMesh、Zero123等扩散/编码器-解码器模型），属于多模态生成方向下的[3D 生成与重建]规范子主题。

Importance reason: Focuses on robotic grasping evaluation, not aligned with user's priority subtopics in generation and efficiency. Signals: Robotics grasping evaluation, modular vs end-to-end, pose/shape estimation, not generative video/image, no efficiency focus
...
TL;DR: 本文提出 DecoupleGen 方法，通过个性化文本到图像扩散模型生成罕见上下文（如沙滩球在道路上）的图像，作为训练数据增强以缓解视觉数据集中的上下文偏差，提升模型在非典型场景下的识别能力。


<details>
  <summary>Details</summary>
Motivation: 视觉数据集中常见场景与罕见场景分布不均，导致模型在常见场景上表现良好但在罕见场景（如沙滩球在道路上）上识别能力不足。由于真实世界中收集罕见场景图像困难，本文探索是否可以通过生成模型合成罕见上下文图像来有效增强训练数据。

Method: 提出 DecoupleGen 方法，核心思想是将上下文模式与物体特征解耦。首先从数据集中提取物体和上下文信息，然后个性化微调文本到图像扩散模型，使其能够生成物体在罕见上下文中的图像，同时保持原始视觉细节。生成后通过验证约束确保生成图像的相关性和质量。

Result: 在复杂场景数据集上的物体分类和识别任务中，DecoupleGen 相比之前的方法取得了一致的性能提升。分析实验进一步揭示了生成图像质量、多样性等因素对性能提升的贡献。

Conclusion: 生成罕见上下文图像作为训练数据增强是一种有效缓解视觉数据集上下文偏差的方法。DecoupleGen 通过解耦上下文模式并个性化生成，能够在保持原始数据分布的同时有效合成罕见场景图像，提升模型在非典型场景下的泛化能力。

Classification reason: 该论文使用个性化文本到图像扩散模型生成罕见上下文的图像作为训练数据增强，属于合成数据生成与数据增强范畴，映射到 [CANONICAL] 子主题 "合成数据与数据集蒸馏" (aigc_synthetic_data)。

Importance reason: Focuses on contextual debiasing via personalized generation, not directly on core priority subtopics like autoregressive video generation or world models. Signals: personalized text-to-image diffusion, contextual debiasing, data augmentation, not autoregressive or video, no world model
...
### [16] [OSMa-Bench++: Toward Open-Ended Benchmarking of Semantic Mapping for Manipulation with Prompt-Generated Synthetic Scenes](https://arxiv.org/abs/2605.26831)
...
### [17] [AnchorDiff: Training-Free Concept Grounding for MM-DiTs via Anchor-Based Graph Propagation](https://arxiv.org/abs/2605.26460)
...
TL;DR: AnchorDiff 是一种无需训练的概念定位方法，通过锚点选择和基于图的传播机制，解决 MM-DiT 中视觉混淆概念间的"概念泄漏"问题，在 ImageNet-Segmentation 和 PascalVOC 上取得良好定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力图的训练无关概念定位方法在视觉上易混淆的概念（如不同颜色的鸟）上会产生重叠激活，导致目标响应泄漏到非目标对象上（称为概念泄漏）。作者希望在不重新训练模型的前提下，实现更精确、无泄漏的概念定位。

Method: AnchorDiff 将语义定位与结构精化解耦：首先从概念到图像的注意力图中选取高置信度锚点，然后将该锚点作为 one-hot 种子，在基于图像到图像自注意力构建的混合图上进行传播。该图使用输出空间相似度实现密集的物体内传播，并使用逐行注意力门控抑制跨物体连接。此外，作者构建了 Multi-Concept Confusion Dataset 用于显式评估概念泄漏。

Result: 在 ImageNet-Segmentation 和 PascalVOC 上取得了强竞争力的定位性能，在 Multi-Concept Confusion Dataset 上显著减少了概念泄漏。

Conclusion: AnchorDiff 通过锚点选择与图传播机制有效解决了 MM-DiT 中的概念泄漏问题，是一种无需训练、即插即用的概念定位方法，为扩散 Transformer 的可控生成提供了更精确的语义控制手段。

Classification reason: 该论文提出训练无关的概念定位方法（concept grounding），用于多模态扩散Transformer（MM-DiT）中根据文本概念对图像进行语义定位与分割，属于可控编辑/定位范畴，与[CANONICAL] subtopic "visual_editing（图像与视频编辑）" 中的可控编辑方向高度相关。

Importance reason: Novel training-free grounding method for MM-DiTs, but focuses on image segmentation rather than user's priority video generation or world models. Signals: training-free concept grounding, MM-DiT, concept leakage, anchor-based graph propagation, ImageNet-Segmentation
...
### [18] [Scheduled Style Injection: Expanding the Style-Content Pareto Frontier in Training-Free Diffusion-based Style Transfer](https://arxiv.org/abs/2605.26538)
*Amey Sunil Kulkarni*
...
TL;DR: 本文系统探索了训练无关扩散模型风格迁移中风格注入的位置与调度策略，发现递减式调度（浅层/早期强注入）优于递增式，且 gamma 调度与 ControlNet 条件化近乎独立，联合配置可扩展 Pareto 前沿，在 ArtFID 上相对 StyleID 提升 6.1%。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关风格迁移方法（如 StyleID）使用单一全局参数 gamma 统一控制所有层和时间步的风格注入强度，导致风格质量与内容保持之间的人为固定折中。作者希望探索更灵活的风格注入调度策略，突破这一固定权衡。

Method: 系统探索四个控制维度：不同解码器层的风格注入强度、不同去噪时间步的强度、沿两个轴调度 ControlNet 几何条件化。测试了线性、余弦、平方根等多种调度形状，并组合 gamma 调度与 ControlNet 调度。

Result: 递减式调度（浅层/早期强注入）一致优于递增式；余弦和平方根时间步调度优于线性；gamma 调度与 ControlNet 条件化近乎独立；联合配置扩展了 Pareto 前沿，最佳配置 ArtFID 27.036（StyleID 为 28.801），相对提升 6.1%。在 35 种配置、28,000+ 张图像上验证，结果在 SD 不同骨干网络上泛化。

Conclusion: 通过简单的调度策略（仅需几行代码）即可显著提升训练无关扩散模型风格迁移的性能，无需额外训练或参数。该发现为风格迁移中的可控性提供了新视角。

Classification reason: 该论文研究基于扩散模型的风格迁移（style transfer），属于图像编辑/风格化范畴，与 [CANONICAL] subtopic "visual_editing"（图像与视频编辑）高度匹配。

Importance reason: Focuses on style transfer, a niche area not aligned with user's priority subtopics like video generation or world models. Signals: Training-free diffusion, Style transfer, Pareto frontier, ArtFID improvement, Not autoregressive or video generation
...
### [19] [Learning Reference-Guided Exposure Correction with Hybrid Illumination Characteristics](https://arxiv.org/abs/2605.26729)
...
TL;DR: HICNet 是一种参考引导的曝光校正框架，通过轻量级内容无关编码器提取光照嵌入，结合 FiLM 全局调整与 Photometric Channel Rebalancing 细粒度光谱门控，实现曝光匹配输出，无需真值或本征分解即可达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有曝光校正方法通常需要真值图像或本征分解，且难以处理多样化的光照条件。本文希望设计一种无需真值、鲁棒性强的参考引导曝光校正方法。

Method: 提出 HICNet：1) 轻量级内容无关编码器将图像压缩为紧凑的光照嵌入（包含区域亮度、边缘对比度、高阶亮度矩）；2) 源图与参考图的光照嵌入差异驱动多尺度调制网络，结合 FiLM 全局调整与 Photometric Channel Rebalancing 细粒度光谱门控；3) 跨批次对比损失对光照流形进行排序，增强对多样光照条件的鲁棒性。

Result: 在公开基准上取得更优的曝光校正精度，并能够很好地泛化到完全未见过的场景。

Conclusion: HICNet 通过参考引导和光照嵌入差异驱动的方式，无需真值或本征分解即可实现高质量的曝光校正，且具有良好的泛化能力。

Classification reason: 该论文提出参考引导的曝光校正方法（HICNet），属于图像编辑（曝光校正）任务，与多模态生成方向下的 [CANONICAL] subtopic "visual_editing（图像与视频编辑）" 高度匹配。同时，曝光校正也属于 AIGC 领域的内容编辑应用。

Importance reason: Exposure correction is a niche image enhancement task, not aligned with priority subtopics like video generation or world models. Signals: reference-guided exposure correction, illumination embedding, FiLM modulation, no ground truth needed, SOTA on benchmarks
...
### [20] [Leveraging Text-to-Image Diffusion Models for Unsupervised Visual Object Tracking](https://arxiv.org/abs/2605.26933)
...
TL;DR: 本文提出 Diff-Tracking，利用预训练的文生图扩散模型的跨注意力机制实现无监督视觉目标跟踪。通过初始提示学习器生成描述目标的文本提示，再通过在线提示更新器结合运动信息逐帧优化提示，使扩散模型能够持续定位视频中的任意目标，无需任何标注数据训练。


<details>
  <summary>Details</summary>
Motivation: 无监督视觉目标跟踪无需标注数据即可跟踪任意目标，但现有方法在需要细粒度理解视频帧语义和视觉结构信息的场景中表现不佳。文生图扩散模型具有强大的语义和结构理解能力，本文旨在利用扩散模型中编码的丰富语义知识来解决无监督跟踪的挑战。

Method: Diff-Tracking 包含两个核心组件：(1) 初始提示学习器（Initial Prompt Learner），在第一帧中学习一个能捕获目标对象的文本提示，使扩散模型通过跨注意力图定位目标区域；(2) 在线提示更新器（Online Prompt Updater），利用运动信息逐帧优化提示，确保跨帧的一致性跟踪。该方法将扩散模型重新解释为文本与图像模态之间的桥梁，通过跨注意力机制实现目标定位。

Result: 在六个具有挑战性的跟踪数据集上进行了评估，实验结果表明 Diff-Tracking 在无监督跟踪任务上取得了有效的结果，验证了利用文生图扩散模型进行视觉目标跟踪的可行性。

Conclusion: 本文证明了预训练的文生图扩散模型可以成功迁移到无监督视觉目标跟踪任务中，通过跨注意力机制实现目标定位与跟踪，为扩散模型在视觉跟踪领域的应用提供了新的视角。

Classification reason: 该论文利用文生图扩散模型（跨注意力机制）实现无监督视觉目标跟踪，本质上是将图像生成模型的能力迁移到视频跟踪任务中，属于图像/视频内容的理解与定位应用。虽然跟踪不是传统意义上的"编辑"，但该工作利用扩散模型的跨注意力图来定位目标，与视觉编辑中利用注意力图进行可控操作有相似的技术路线。更合适的匹配是 multimodal_generation 下的 visual_editing（图像与视频编辑），因为该 subtopic 涵盖视频相关的视觉操作任务，且该论文的核心是利用扩散模型进行视频帧级别的目标定位与跟踪。

Importance reason: Novel use of text-to-image diffusion for tracking, but tangential to core priority subtopics. Signals: diffusion model for tracking, cross-attention for localization, unsupervised tracking, prompt learning, online prompt update
...
### [21] [MRT: Masked Region Transformer for Layered Image Generation and Editing at Scale](https://arxiv.org/abs/2605.27235)
...
Importance: 高优先级 (87.88) - 未入选精读

TL;DR: MRT 是一个 20B 参数的掩码区域扩散模型，专为多层透明图像生成与编辑设计，统一了 text-to-layers、image-to-layers 和 layers-to-layers 三种任务，引入溢出感知画布层处理边界不一致问题，并通过扩散蒸馏实现 8 步实时生成，在多项任务上显著超越现有商业系统。


<details>
  <summary>Details</summary>
Motivation: 分层图像生成与编辑（layer-wise generation and editing）是视觉内容创作中的基础能力，类似于自然语言中的词级编辑。然而，该领域在大规模模型上仍未被充分探索，现有方法在生成质量、推理速度和内存效率方面存在不足。

Method: 1) 提出掩码区域扩散框架（Masked Region Diffusion），通过选择性 token 掩码统一 text-to-layers、image-to-layers 和 layers-to-layers 三种任务；2) 引入溢出感知画布层（overflow-aware canvas layer），处理超出可见画布边界的图层生成和半透明背景合成；3) 采用扩散蒸馏（diffusion distillation）实现 8 步实时推理。

Result: 在 text-to-layers、image-to-layers 和 layers-to-layers 三项任务上均显著超越现有方法（包括多个商业系统）。在 image-to-layers 任务中，用户研究显示显著优于 Qwen-Image-Layered 模型，同时推理速度提升 10-100 倍，激活 GPU 内存消耗降低 50-90%。

Conclusion: MRT 证明了大规模掩码区域扩散模型在分层图像生成与编辑上的有效性，为视觉内容的图层级复用、编辑和组合提供了新的基准，展示了在生成质量和效率上的显著优势。

Classification reason: 该论文提出分层图像生成与编辑方法（MRT），核心是图像的分层（layer-wise）生成与编辑，属于图像编辑范畴。现有 [CANONICAL] subtopic "visual_editing（图像与视频编辑）" 涵盖图像编辑、可控编辑等，与该论文主题高度匹配。

Importance reason: Strongly aligns with multimodal generation and efficient inference priorities, with a novel unified framework for layered image generation. Signals: 20B parameter model, unified text-to-layers, image-to-layers, layers-to-layers, overflow-aware canvas layer, diffusion distillation for 8-step real-time generation, 10-100x faster inference than Qwen-Image-Layered
...
### [22] [Feedforward 3D Editing Learns from Semantic-Part Transformation](https://arxiv.org/abs/2605.27351)
...
TL;DR: 本文提出Pxform数据集（10万+3D编辑对）和PartFlow前馈式3D编辑网络，通过语义部件变换监督实现高质量、可扩展的前馈3D编辑，在几何和外观编辑基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有3D编辑方法多为免训练管线，缺乏高质量配对数据来训练前馈式3D编辑模型。已有数据集存在定位不准、源信息保持差、编辑边界模糊等问题，亟需大规模、高质量的3D编辑配对数据。

Method: 提出Pxform数据集，基于语义3D部件变换构建10万+前后一致编辑对，覆盖7种编辑类型。在此基础上提出PartFlow网络，将源感知隐空间控制注入预训练3D生成先验，引入掩码感知的速度保持和渲染空间一致性监督，推理时无需3D编辑掩码。

Result: 在几何和外观编辑基准上均达到SOTA性能，验证了高质量语义部件监督对可扩展3D编辑的有效性。

Conclusion: 语义部件变换为前馈式3D编辑提供了可扩展的监督信号，Pxform数据集和PartFlow网络共同推动了3D编辑向大规模前馈生成范式演进。

Classification reason: 该论文研究3D编辑（3D editing），属于多模态生成方向下的图像与视频编辑（visual_editing）范畴。虽然该子主题名称侧重图像与视频，但其描述包含"可控编辑"等广义编辑概念，且3D编辑是该子主题的自然延伸。同时该工作也涉及AIGC方向的内容创作应用。

Importance reason: Strong dataset and method for feedforward 3D editing, but outside user's priority subtopics and boosted keywords. Signals: 3D editing dataset, feedforward network, semantic-part supervision, SOTA on benchmarks, no direct link to video/image generation or world models
...
### [23] [DuoGesture: Neuro-Inspired and Biomechanically Informed Dual-Stream Co-Speech Gesture Generation](https://arxiv.org/abs/2605.26236)
...
TL;DR: DuoGesture 提出了一种神经启发且生物力学感知的双流共语手势生成方法，将手势分解为语义流和节拍流，通过语义变分信息瓶颈协调两流，并引入惯性节拍先验提升运动平滑性，在客观和主观评测中优于强基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有共语手势生成模型将语义手势和节拍手势混合建模，导致语义表达不足、语音-动作对齐不佳以及运动平滑性受限。受神经科学中手势双流加工机制的启发，作者希望解耦这两种手势类型以提升生成质量。

Method: 提出 DuoGesture 双流架构：(1) 语义流由运动接地语义条件（Motion-Grounded Semantic Conditioning）控制，用运动-语言联合表征替代纯语言词嵌入；(2) 节拍流由惯性节拍先验（Inertial Beat Prior）正则化，基于人体测量学加权的手臂链模型减少抖动；(3) 两流通过语义变分信息瓶颈（Semantic Variational Information Bottleneck）的随机帧级门控协调，学习何时语义手势应覆盖节拍运动。

Result: 客观评测和主观实验表明 DuoGesture 在多项指标上优于强基线模型；消融实验验证了语义接地、随机流选择和生物力学正则化三个组件的互补作用。

Conclusion: 解耦语义手势和节拍手势的双流生成范式，结合生物力学先验，能有效提升共语手势的语义表达力和运动自然度，为共语手势生成提供了新的神经启发式设计思路。

Classification reason: 该论文研究共语手势生成（co-speech gesture generation），属于数字人与动作生成中的"人体动作生成"范畴，与 canonical subtopic "avatar_motion_generation（数字人与动作生成）"高度匹配。

Importance reason: Novel dual-stream decomposition with biomechanical priors advances gesture generation, but lacks direct alignment with top-priority video generation or world model topics. Signals: dual-stream gesture generation, semantic variational information bottleneck, inertial beat prior, biomechanical regularization, outperforms holistic baselines
...
### [24] [E$^3$C: Video Generation with 3D Environmental Memory and Ego-Exo Human Pose Control](https://arxiv.org/abs/2605.26316)
...
TL;DR: E³C 是一个可控的第一人称（egocentric）视频扩散生成框架，通过构建半稠密点云3D环境记忆来解耦场景结构与人体动态，并分别用骨架渲染控制场景中他人（exo control）和3D关节+手腕运动控制相机佩戴者（ego control），在 Nymeria 数据集上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 具身智能体需要能够生成可控且物理合理的自我中心视频，以推理自身和他人动作如何改变世界。然而，自我中心视频生成面临相机与演员紧密耦合导致视角快速变化、自遮挡频繁、动作细微且部分可见等挑战，现有方法难以同时保持场景一致性和人体控制精度。

Method: E³C 构建了一个半稠密点云3D环境记忆，从上下文帧中提取点云并用视频VAE特征增强每个点的外观描述子，渲染到目标视角作为条件。人体动态建模分为两部分：场景中他人通过骨架渲染控制（exo human control），相机佩戴者通过3D身体关节和6DoF手腕运动控制（ego human control）。为处理佩戴者身体不可见时的控制问题，引入ego运动编码器生成持久交叉注意力token。整体框架基于视频扩散模型。

Result: 在 Nymeria 数据集上的实验表明，E³C 在视觉保真度、相机运动准确性、物体一致性以及自我/他人人体控制方面均显著优于强基线方法，同时支持直观的场景编辑。

Conclusion: E³C 通过3D环境记忆与解耦的人体控制机制，有效解决了自我中心视频生成中的场景一致性和人体控制难题，为具身智能体的视觉推理和交互提供了有力工具。

Classification reason: 该论文提出可控的第一人称视角视频生成框架，核心涉及人体姿态控制（ego/exo human pose control）和数字人动作生成，与[CANONICAL] subtopic "数字人与动作生成"（avatar_motion_generation）高度匹配。

Importance reason: Strongly aligns with multimodal generation and video diffusion priorities, but the specific ego-exo human pose control subtopic is less central to your configured top interests. Signals: video diffusion, controllable generation, 3D memory, ego-exo control, Nymeria dataset
...
### [25] [LongCat-Video-Avatar 1.5 Technical Report](https://arxiv.org/abs/2605.26486)
...
TL;DR: LongCat-Video-Avatar 1.5 是一个开源的音频驱动视频数字人生成框架，通过升级音频编码器、精心设计训练配方、数据筛选和 RLHF 训练，实现了高精度唇形同步、全身时间稳定性、长视频生成和严格的身份一致性，并通过步蒸馏加速推理至 8 NFE，在 500+ 测试案例上达到或超越 HeyGen、OmniHuman 1.5 等商业闭源系统的效果。


<details>
  <summary>Details</summary>
Motivation: 现有音频驱动视频生成方法在商业级稳定性上仍存在不足，如唇形同步精度不够、时间不稳定、身份一致性差、难以处理多人和物体交互等复杂场景，且学术原型与工业部署之间存在差距。

Method: 1) 将音频编码器升级为 Whisper Large 以提升唇形同步精度；2) 精心扩展训练配方（数据筛选、多阶段训练）；3) 引入 RLHF 训练以提升生成质量和对齐；4) 采用高级步蒸馏技术将推理加速至 8 NFE，在效率与视觉保真度之间取得平衡。

Result: 在 500+ 多样化测试案例上，通过定量指标和严格人工评估，LongCat-Video-Avatar 1.5 在类人评分和专家级质量评估上达到或超越 HeyGen、OmniHuman 1.5、Kling Avatar 2.0 等领先闭源系统，并泛化到动漫、动物等风格化领域以及多人交互、物体操作等复杂场景。

Conclusion: LongCat-Video-Avatar 1.5 通过系统性的工程优化而非架构创新，显著缩小了学术研究原型与商业级部署之间的差距，其开源发布为社区提供了可复现的工业级数字人生成基线。

Classification reason: 该论文提出音频驱动的视频数字人生成系统 LongCat-Video-Avatar 1.5，属于虚拟人/说话头/肖像动画方向，与 [CANONICAL] 子主题 "数字人与动作生成" (avatar_motion_generation) 完全匹配。

Importance reason: Focuses on engineering and deployment of audio-driven avatar generation, which is a niche subtopic not aligned with user's priority subtopics. Signals: audio-driven video avatar, step distillation to 8 NFE, RLHF for stylized domains, open-source release, commercial-grade stability
...
### [26] [Self-Intersection-Aware 3D Human Motion Generation Using an Efficient Human Sphere Proxy](https://arxiv.org/abs/2605.26744)
...
TL;DR: 本文提出一种基于人体球代理（sphere proxy）的自交损失函数，用于显式惩罚3D人体动作生成中的身体部位相互穿透问题。该损失比基于三角网格的方法快98%、省83%内存，且与具体生成方法无关。在MDM和MoMask上实验表明，自交减少高达49%，同时其他评估指标也有提升。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体动作生成方法在标准评测指标上已超越真实数据，但视觉检查发现生成动作频繁出现身体部位自交（相互穿透）的严重伪影，极大降低了感知质量。现有方法缺乏显式惩罚自交的机制。

Method: 提出一种基于人体球代理（sphere proxy）的自交损失函数，用球体近似人体几何，可高效计算身体部位间的穿透惩罚。该损失与模型无关，可即插即用地添加到现有动作生成方法（如MDM和MoMask）的训练中。

Result: 在MDM和MoMask上实验，自交减少高达49%，同时FID等标准评测指标也有改善。计算效率方面，比基于三角网格的方法快98%，内存占用减少83%。

Conclusion: 基于球代理的自交损失能有效减少3D人体动作生成中的自交伪影，提升生成动作的视觉质量，且高效、通用，可集成到多种动作生成方法中。

Classification reason: 该论文研究3D人体动作生成中的自交问题，属于数字人与动作生成（avatar_motion_generation）这一[CANONICAL]子主题的范畴。

Importance reason: Focuses on motion generation artifact reduction, not aligned with user's priority subtopics like video/image generation or world models. Signals: motion generation, self-intersection loss, sphere proxy, efficient computation, artifact reduction
...
### [27] [Natural Human Motion Recovery by Aligning High-Order Temporal Dynamics from Monocular Videos](https://arxiv.org/abs/2605.26879)
...
Importance: 值得关注 (65.11) - 未入选精读

TL;DR: 本文提出 HTD-Refine，一种后处理框架，通过显式估计高阶时序动态（速度与加速度）来增强现有单目人体运动恢复（HMR）方法，生成更自然、物理合理的运动轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有单目人体运动恢复方法即使关节位置数值准确，恢复的运动也往往过于平滑或动态不一致，缺乏真实动量、时序和高频细节。这是因为缺乏可靠的高阶时序线索（速度和加速度）。

Method: 提出 HTD-Refine 框架，核心是 PVA-Net（时序 Transformer），从单目视频直接推断每关节的 2D 位置、3D 速度和 3D 加速度。这些预测的高阶动态作为软约束，通过全局优化过程精炼世界空间轨迹，减少抖动、抑制过平滑、恢复物理合理的运动。

Result: 在具有挑战性的野外基准上，HTD-Refine 持续改进 SOTA HMR 方法，生成更准确的全局轨迹和更自然的运动动态。

Conclusion: 高阶时序建模在推进单目人体运动恢复中起关键作用，显式利用速度和加速度约束能显著提升运动自然度。

Classification reason: 该论文关注从单目视频恢复自然人体运动，属于人体动作生成与重建范畴，与 [CANONICAL] 数字人与动作生成 (avatar_motion_generation) 高度匹配。

Importance reason: Focuses on human motion recovery, which is not a priority subtopic and lacks strong alignment with user's configured direction weights and boost keywords. Signals: human motion recovery, high-order temporal dynamics, monocular video, post-processing framework, physical plausibility
...
### [28] [Erased but Exploitable: Black-box Embedding-Aware Prompting Against Unlearned Text-to-Image Diffusion Models](https://arxiv.org/abs/2605.26332)
...
TL;DR: 本文提出 BEAP，一种黑盒、嵌入感知的对抗提示攻击方法，利用 LLM 迭代生成对抗性文本提示，使已执行概念擦除的文生图扩散模型重新生成被擦除的概念。BEAP 结合未学习概念存在性、文本-图像对齐和图像质量等多重奖励信号，在保持提示不被安全过滤器检测的同时，将攻击成功率提升 60% 以上。


<details>
  <summary>Details</summary>
Motivation: 现有针对已擦除概念模型的攻击方法存在两个主要缺陷：要么需要访问模型权重（白盒攻击，不现实），要么生成的对抗提示是乱码文本，容易被简单的规则检测器识别。因此，需要一种更现实的黑盒攻击方法，既能有效绕过概念擦除，又能生成自然、不可检测的提示。

Method: BEAP 利用 LLM 作为对抗提示生成器，在文本空间中进行嵌入感知搜索。具体地，它结合三个奖励信号——未学习概念存在性（通过 CLIP 文本-图像相似度）、文本-图像对齐和图像质量——来迭代优化生成的提示。该方法完全黑盒，仅需访问模型的输入输出接口，无需模型权重。

Result: 实验表明，BEAP 相比现有方法将攻击成功率（ASR）提升超过 60%，且每次成功攻击平均仅需 15 个提示。生成的对抗提示自然流畅，能够绕过安全过滤器的检测，同时生成高质量的图像。

Conclusion: BEAP 揭示了当前概念擦除方法的脆弱性，表明即使经过擦除的模型仍然可以被黑盒方式有效利用。这强调了需要更鲁棒的遗忘机制和防御策略来应对嵌入感知的对抗攻击。

Classification reason: 本文研究针对已执行概念擦除（unlearning）的文生图扩散模型的黑盒对抗攻击，属于生成模型遗忘学习后的安全性问题。该主题与 canonical subtopic "aigc_unlearning_erasure"（概念擦除与遗忘学习）高度匹配，因此直接复用该 subtopic。

Importance reason: Focuses on adversarial attacks for concept unlearning, which is not a priority subtopic and lacks direct alignment with user's key interests in generation efficiency or world models. Signals: Black-box attack, concept unlearning, text-to-image diffusion, LLM-based prompt generation, safety filter evasion
...
### [29] [Targeted Remasking: Replacing Token Editing with Token-to-Mask Refinement in Discrete Diffusion Language Models](https://arxiv.org/abs/2605.26436)
*Lin Yao*
...
Importance: 值得关注 (67.03) - 未入选精读

TL;DR: 本文提出Token-to-Mask (T2M) remasking方法，作为LLaDA等离散掩码扩散语言模型中Token-to-Token (T2T)编辑机制的无训练替代方案。T2M将疑似错误的token重置回mask状态，让扩散过程在更干净的上下文中重新预测，配合三种互补的错误检测策略，在12个基准上提升了生成质量，尤其在数学任务上提升显著（CMATH +5.92%）。


<details>
  <summary>Details</summary>
Motivation: 现有离散掩码扩散语言模型（如LLaDA）使用T2T编辑机制加速生成，但该方法存在根本性缺陷：它将错误检测与替换耦合在一起，用可能错误的token污染生成上下文，且训练-推理噪声不匹配导致模型对自身生成的系统性错误缺乏鲁棒性。作者希望设计一种更干净的纠错机制。

Method: 提出Token-to-Mask (T2M) remasking方法：将检测到的疑似错误token重置为mask状态，让扩散过程在更干净的上下文中重新预测。设计了三种互补的错误检测策略：(1) 基于概率的检测——利用token预测概率的置信度；(2) 触发镜像检测——利用模型对自身预测的二次评估；(3) 时序差分检测——比较相邻去噪步的预测差异。提供了统一的理论分析框架。

Result: 在12个基准（涵盖知识、推理、数学、编程、指令跟随）上的实验表明，T2M在需要精确token级输出的任务上普遍优于T2T编辑。最大提升在数学任务上（CMATH +5.92%）。错误分析显示，主要失败模式是"最后一英里token损坏"（正确推理但最终答案被破坏），T2M修复了59.4%的此类情况。

Conclusion: T2M remasking作为一种无训练、即插即用的替代方案，有效解决了T2T编辑的上下文污染和噪声不匹配问题，通过将错误token重置为mask状态并重新预测，显著提升了离散扩散语言模型的生成质量，尤其在精确token级任务上效果突出。

Classification reason: 该论文提出Token-to-Mask (T2M) remasking方法，通过将疑似错误token重置为mask状态来加速和改进离散扩散语言模型的生成过程，属于生成模型效率优化（加速与改进生成质量），与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Directly improves efficiency and quality of discrete diffusion language models, a key technique for autoregressive video/image generation. Signals: training-free improvement, discrete diffusion language model, token-to-mask remasking, error detection strategies, math reasoning boost
...
### [30] [QAM-W: Joint 2D Codebook Quantization for LLM Weights via Hadamard Rotation and Activation-Aware Scaling](https://arxiv.org/abs/2605.26339)
...
TL;DR: QAM-W 提出一种基于 Hadamard 旋转和激活感知缩放的联合 2D 码本量化方法，用于 LLM 权重的后训练量化。该方法将权重行归一化、旋转、配对为 2D 坐标后，用 Lloyd-Max 码本量化，在约 5.5 bpw 下保持 BF16 困惑度，匹配 SmoothQuant W8A8 质量但节省 32% 权重比特。


<details>
  <summary>Details</summary>
Motivation: 标量后训练量化器丢弃了权重行内的成对坐标结构，导致信息损失。作者希望恢复这种结构，通过联合 2D 编码更高效地压缩 LLM 权重，在保持模型质量的同时降低比特率。

Method: QAM-W 方法：对每个权重行进行 L2 归一化 → 块级 Hadamard 旋转 → 将相邻元素配对为 2D 坐标 → 使用在单位圆高斯分布上训练的 Lloyd-Max 码本进行量化，并加入激活感知的逐通道缩放。

Result: 在 5 个 LLM（1.1B-13B）的 8 种量化配置上，激活感知变体在约 5.5 bpw 下困惑度变化在 ±0.4% 以内，匹配 SmoothQuant W8A8 但节省 32% 权重比特。联合 2D 编码优于极坐标编码 2-15 pp ΔPPL。配对 KL 与 ΔPPL% 的 Spearman ρ=0.99。在严格 4 bpw 下不如 QTIP。

Conclusion: QAM-W 在 5-6 bpw 范围内实现了质量保持的 LLM 权重量化，验证了联合 2D 码本编码相比标量量化的优势，为 LLM 高效部署提供了新思路。

Classification reason: 该论文提出LLM权重的量化方法（QAM-W），属于模型量化与高效化方向，与多模态生成方向下的[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Novel 2D codebook quantization for LLM weights achieves high efficiency at 5.5 bpw, directly supporting your priority on model efficiency and acceleration. Signals: LLM weight quantization, 2D codebook, Hadamard rotation, activation-aware scaling, 32% fewer weight bits
...
### [31] [The Bridge-Garden Dilemma in LLM Distillation: Why Mixing Hard and Soft Labels Works](https://arxiv.org/abs/2605.26246)
...
Importance: 值得关注 (75.7) - 未入选精读

TL;DR: 本文提出 Bridge-Garden 分解理论，解释了为什么在语言模型知识蒸馏中混合硬标签（采样 token）和软标签（完整分布）比单独使用任何一种效果更好。该理论将生成步骤分为"桥梁"（需精确匹配）和"花园"（可灵活选择）两类，混合策略能同时降低两类步骤的暴露偏差。基于此理论开发的混合监督方法在多个模型对（Qwen、Llama、Gemma、DeepSeek）上优于现有蒸馏基线，同时训练成本降低 9.7 倍。


<details>
  <summary>Details</summary>
Motivation: 知识蒸馏中，硬标签（采样 token）和软标签（完整分布）各有优劣，但混合使用始终优于单一策略，这一现象缺乏理论解释。作者旨在揭示混合策略有效的根本原因，并据此设计更优的蒸馏方法。

Method: 提出 Bridge-Garden 分解理论，将生成步骤分为"桥梁"（Bridge，下一个 token 必须精确）和"花园"（Garden，可灵活选择）两类。硬标签在 Bridges 中避免风险偏差，软标签在 Gardens 中保持多样性。基于此理论设计自适应混合监督方法，动态平衡硬标签和软标签的比例。

Result: 在 7 组教师-学生模型对（Qwen、Llama、Gemma、DeepSeek）的推理和代码基准上，所提方法优于基于散度的蒸馏和 on-policy KD 基线，同时训练成本降低 9.7 倍。

Conclusion: 混合硬标签和软标签在知识蒸馏中有效的原因不是更接近教师匹配，而是减少了暴露偏差。Bridge-Garden 理论为理解蒸馏中的标签选择提供了新视角，自适应混合策略实现了高效且高质量的模型压缩。

Classification reason: 论文研究知识蒸馏（KD）在语言模型中的混合标签策略，属于生成模型效率优化中的模型压缩与蒸馏方法，与 canonical subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Directly addresses model efficiency and compression via a novel distillation theory, aligning with high-priority subtopics and boost keywords. Signals: Novel Bridge-Garden theory, 9.7x training cost reduction, strong empirical results across multiple LLM families, directly relevant to model efficiency and acceleration
...
### [32] [Quantized Keys Steal Attention: Bias Correction for KV-Cache Compression in Video Diffusion](https://arxiv.org/abs/2605.26266)
*Tuna Tuncer, Felix Becker, Thomas Pfeil*

Main category: eess.IV, cs.CV, cs.GR, cs.LG, cs.AI
...
TL;DR: 本文发现视频扩散模型中KV-Cache量化导致注意力权重系统性偏差（Jensen偏差），并提出一种逐注意力分数的在线校正方法，在INT2量化下恢复接近BF16的视频质量，且计算开销可忽略。


<details>
  <summary>Details</summary>
Motivation: 分块自回归视频扩散模型依赖KV-Cache避免冗余计算，但长视频生成时KV-Cache成为内存瓶颈。量化KV-Cache到低位宽可减少内存压力，但会严重降低视频质量。本文旨在揭示量化导致质量下降的根本原因并提出轻量级校正方法。

Method: 从理论上证明量化噪声因softmax注意力中指数函数的凸性导致缓存键的注意力权重被系统性放大（Jensen偏差）。推导出逐注意力分数的校正项，利用量化步长和查询向量范数在线计算，通过二阶泰勒近似保证计算开销可忽略，无需额外内存。

Result: 在MAGI-1、SkyReels-V2和HY-WorldPlay三个视频扩散模型上，INT2量化+校正方法恢复了大部分因量化损失的质量，达到接近BF16的视频质量，且相比INT4量化节省50%内存。

Conclusion: KV-Cache量化中的注意力偏差是可校正的，所提方法能有效缓解低位宽量化带来的质量退化，为长视频生成中的高效推理提供了实用方案。

Classification reason: 该论文研究视频扩散模型中KV-Cache的量化压缩与偏置校正，属于生成模型效率优化（量化压缩）范畴，与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Directly addresses a critical efficiency bottleneck in autoregressive video diffusion models with a principled bias correction method. Signals: KV-cache compression, video diffusion, autoregressive video generation, efficient inference, Jensen bias
...
### [33] [Tail-Aware HiFloat4: W4A4 Post-Training Quantization for Wan2.2](https://arxiv.org/abs/2605.26628)
...
TL;DR: 本文提出Tail-Aware HiFloat4方法，针对Wan2.2文生视频模型进行W4A4后训练量化，通过HiFloat4数值格式、边界模块高精度保留、激活尾部感知的百分位校准模块以及紧凑的PTQ状态恢复，在保持运行时HiFloat4算术不变的前提下减少异常校准离群值的影响。


<details>
  <summary>Details</summary>
Motivation: 文生视频模型（如Wan2.2）计算开销巨大，低比特量化是降低推理成本和内存占用的关键手段。然而现有量化方法在处理罕见校准离群值时效果不佳，且需要保持运行时算术格式不变以兼容硬件加速。

Method: 基于ViDiT-Q后训练量化管线，采用HiFloat4数值格式对Wan2.2 Transformer模块的主要线性层进行W4A4伪量化；将数值敏感的边界模块保持高精度；引入激活尾部感知的百分位校准模块用于通道掩码构建；结合紧凑的PTQ状态恢复策略。

Result: 在低比特文生视频量化挑战赛中提交的方法，在保持HiFloat4运行时算术和采样管线不变的前提下，有效减少了校准离群值的影响，实现了W4A4量化下的文生视频生成。

Conclusion: Tail-Aware HiFloat4方法为Wan2.2模型提供了一种有效的低比特后训练量化方案，在保持生成质量的同时显著降低模型精度需求，为文生视频模型的高效部署提供了可行路径。

Classification reason: 该论文研究Wan2.2文生视频模型的低比特后训练量化（W4A4 HiFloat4），属于生成模型效率优化中的量化加速方向，与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Directly addresses efficient inference for video generation, a top priority subtopic with strong boost keywords. Signals: W4A4 quantization, video diffusion transformer, post-training quantization, efficient inference, Wan2.2 model
...
### [34] [Timestep-Aware SVDQuant-GPTQ for W4A4 Quantization of Wan2.2-I2V](https://arxiv.org/abs/2605.27003)
...
Importance: 高优先级 (93.88) - Top 3 精读候选

TL;DR: 本文提出一种面向Wan2.2-I2V视频扩散模型的W4A4后训练量化框架，结合SVDQuant低秩异常值补偿、GPTQ重建感知残差量化以及时间步感知的分层裁剪阈值搜索，在OpenS2V-Eval基准上相比BF16基线减少59.3%峰值显存，仅损失0.9% VBench平均分。


<details>
  <summary>Details</summary>
Motivation: W4A4量化能大幅节省视频扩散Transformer的显存，但面临两个挑战：激活值中存在稀疏的大幅度异常值，以及多步去噪过程中激活分布随时间步强烈变化。Wan2.2-I2V的双专家MoE设计进一步加剧了难度，不同专家对量化的敏感度不同，单一全局校准策略无法应对。

Method: 提出后训练量化框架，包含三个组件：(1) 基于SVDQuant的低秩异常值补偿，处理激活值中的大幅异常值；(2) 基于GPTQ的重建感知残差权重量化；(3) 对每个专家独立进行时间步分桶的逐层激活裁剪阈值搜索（timestep-bin-wise per-layer activation clipping-ratio search），以适应不同时间步和不同专家的量化敏感性差异。

Result: 在OpenS2V-Eval基准上，相比BF16基线，峰值GPU显存减少59.3%，VBench平均分仅下降0.9%，Imaging Quality下降2.3%。验证了专家感知和时间步感知的校准策略对于MoE视频DiT的高保真W4A4推理至关重要。

Conclusion: 本文证明了在MoE视频扩散模型上进行W4A4量化时，必须考虑专家差异和时间步依赖的激活分布特性，所提出的框架在显著降低显存的同时保持了生成质量。

Classification reason: 该论文提出W4A4量化方法用于视频扩散Transformer，属于生成模型效率优化（量化），与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Directly addresses efficient inference for video diffusion transformers, a top priority subtopic, with strong memory savings and minimal quality loss. Signals: W4A4 quantization, video diffusion transformer, MoE DiT, timestep-aware calibration, SVDQuant
...
### [35] [On the Error-Correcting Effects of Stochasticity in Discrete Diffusion](https://arxiv.org/abs/2605.26582)
...
TL;DR: 本文系统研究了离散扩散模型中马尔可夫转移随机性程度对采样效率与质量权衡的影响，发现高度确定性转移收敛快但误差累积严重，而更多随机性转移收敛慢但最终质量更高。基于信息论分析，揭示了冗余转移的纠错机制，并提出DCRS（离散重启与搅动采样）算法，通过交替前向与反向扩散过程注入受控随机性，在图像生成上实现高达10倍采样步数缩减，同时保持竞争性样本质量。


<details>
  <summary>Details</summary>
Motivation: 离散扩散模型在文本和图像生成中表现优异，但其推理速度慢，且采样效率与样本质量之间存在固有权衡。现有方法缺乏对随机性程度如何影响这一权衡的系统理解，因此需要深入分析随机性在离散扩散采样中的作用机制，并据此设计更优的采样算法。

Method: 1) 通过信息论分析，证明冗余转移（在状态间对称交换质量的转移）可以收缩采样误差，产生纠错效应。2) 基于此分析，提出DCRS（Discrete Churn and Restart Sampling）算法，在推理过程中交替执行前向扩散（添加噪声）和反向扩散（去噪），从而注入受控的随机性来纠正累积误差。

Result: 在合成数据集和大规模基准上，DCRS在低函数评估次数（NFE）场景下显著改善了速度-质量权衡。在图像数据集上，与标准采样器相比，DCRS实现了高达10倍的采样步数缩减，同时保持竞争性样本质量。在语言基准上，结果更依赖于具体的损坏过程和采样程序。

Conclusion: 离散扩散模型中的随机性具有纠错效应，通过合理注入受控随机性（如DCRS算法）可以在不牺牲质量的前提下大幅减少采样步数，为离散扩散模型的高效推理提供了新的理论视角和实用方法。

Classification reason: 该论文研究离散扩散模型推理中随机性对采样效率与质量的权衡，并提出DCRS算法通过注入受控随机性来减少采样步数（最多10倍加速），属于生成模型加速与高效化的核心范畴，因此映射到[CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Directly addresses model efficiency and sampling acceleration for discrete diffusion, a key priority subtopic with strong weight. Signals: discrete diffusion, sampling efficiency, error correction, stochasticity, DCRS algorithm
...
### [36] [SoftCap: Soft-Budget Control for Diffusion Transformer Acceleration](https://arxiv.org/abs/2605.27075)
...
Importance: 值得关注 (78.55) - 未入选精读

TL;DR: SoftCap 是一种训练无关的缓存控制层，通过轨迹漂移观测器和软预算 PI 控制器动态调整 DiT 推理中 Full 步骤的触发阈值，在几乎相同 FLOPs 下提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散 Transformer (DiT) 迭代去噪过程需要大量 Transformer 计算，现有训练无关加速方法（缓存、预测、验证）通常依赖固定调度或手工阈值来决定何时执行 Full 步骤，缺乏自适应能力。

Method: SoftCap 包含两个组件：(1) 轨迹漂移观测器 (Trajectory Drift Observer)，从轻量级隐藏状态统计量估计局部缓存风险；(2) 软预算 PI 控制器 (Soft-Budget PI Controller)，根据已消耗计算量相对于固定参考配置来调整 Full 步骤触发阈值。预算是一个软上限，不强制要求运行消耗固定数量的 Full 评估。

Result: 在 FLUX.1-dev 上，SoftCap 在中等计算量下相比 SpeCa 提升 ImageReward 从 0.967 到 0.981，降低 LPIPS-Full 从 0.518 到 0.498，且 FLOPs 几乎相同。目标扫描诊断显示随着预算放松，软上限行为符合预期。

Conclusion: SoftCap 提供了一种轻量级、训练无关的 DiT 推理加速控制方案，通过软预算机制在计算效率与生成质量之间实现灵活权衡，无需重新训练模型。

Classification reason: 该论文提出 SoftCap，一种训练无关的缓存控制方法用于加速 DiT 推理，属于扩散模型推理加速与高效化方向，与 canonical 子主题 "生成模型加速与高效化" (efficient_generation) 高度匹配。

Importance reason: Directly addresses model efficiency for diffusion transformers, a high-priority subtopic, with a novel training-free acceleration method. Signals: diffusion transformer acceleration, training-free caching, soft-budget control, trajectory drift observer, FLUX.1-dev improvement
...
### [37] [JLT: Clean-Latent Prediction in Latent Diffusion Transformers](https://arxiv.org/abs/2605.27102)
...
TL;DR: 本文提出JLT，一个130M参数的潜扩散Transformer，研究在FLUX.2 VAE潜空间中使用clean-latent预测（回归干净潜变量）相比velocity预测的优势。实验表明clean-latent预测在ImageNet 256×256上FID-50K达到2.50，显著优于velocity预测，揭示了潜扩散模型中预测目标是表征依赖的几何选择而非可互换的代数参数化。


<details>
  <summary>Details</summary>
Motivation: 流匹配中的clean-data预测已被证明比预测含噪量更能利用低维结构，但该优势在已经过压缩的潜空间中是否仍然成立尚不清楚。本文旨在探究潜扩散模型中不同预测目标（clean-latent vs velocity）的实际效果差异。

Method: 基于FLUX.2 VAE的冻结潜空间，构建130M参数的潜扩散Transformer（JLT），对比clean-latent预测与velocity预测两种训练目标。通过局部高斯分析揭示velocity回归存在各向同性目标协方差下限并放大低方差潜方向，而clean预测则抑制这些方向。在ImageNet 256×256上使用classifier-free guidance进行训练和评估。

Result: JLT-B/1在ImageNet 256×256上使用classifier-free guidance获得FID-50K 2.50，与匹配的velocity预测DiT相比存在显著的matched-target差距，验证了clean-latent预测的优势。

Conclusion: 潜扩散模型中的预测目标是表征依赖的几何选择，而非可互换的代数参数化。clean-latent预测在潜空间中仍然保持优势，为未来扩散模型训练目标的选择提供了理论指导和实践依据。

Classification reason: 该论文研究潜空间扩散Transformer中预测目标的选择（clean-latent vs velocity prediction），属于扩散模型训练范式与效率优化，与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）相关，因为其探讨不同预测目标对训练效率和生成质量的影响。

Importance reason: Directly addresses model efficiency and diffusion transformer acceleration, a top priority subtopic, with a novel clean-latent prediction method. Signals: diffusion transformer, model efficiency, clean-latent prediction, velocity prediction comparison, FLUX.2 VAE
...
### [38] [WINDQuant: Weight-Informed Neural Decision-Making for Global Mixed-Precision LLM Quantization](https://arxiv.org/abs/2605.26660)
...
TL;DR: WINDQuant 提出一种基于强化学习的混合精度量化控制器，在列块粒度上为LLM分配位宽和量化策略，在超低位宽下保持模型性能，同时降低优化开销。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法在超低位宽下性能严重下降，而量化感知训练需要昂贵的重训练。混合精度策略多依赖粗粒度或启发式敏感性分析，忽略了权重矩阵内部的细粒度变化。

Method: WINDQuant 使用PPO强化学习控制器，在列块（column-chunk）粒度上学习位宽分配和量化处理方式，结合激活感知校准、轻量级逐单元量化器拟合和显式有效比特计数，在全局存储预算下实现细粒度混合精度量化。

Result: 在LLaMA模型上的实验表明，WINDQuant在超低位宽设置下取得了有竞争力的性能，同时相比基于重训练的方法降低了优化开销。

Conclusion: 强化学习可以作为自适应混合精度量化的实用控制器，在极低位宽下实现性能与效率的良好平衡。

Classification reason: 该论文研究LLM的混合精度量化方法，属于生成模型效率优化中的量化技术，与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Directly addresses model efficiency and acceleration for LLMs, a high-priority subtopic with strong alignment to user's focus on efficient inference. Signals: mixed-precision quantization, reinforcement learning, LLM efficiency, ultra-low-bit, column-chunk granularity
...
### [39] [PARE: Pruning and Adaptive Routing for Efficient Video Generation](https://arxiv.org/abs/2605.27336)
...
TL;DR: PARE 提出了一种联合宽度剪枝和深度自适应路由的方法，用于高效视频生成。宽度剪枝利用注意力头时空分工特性进行结构化剪枝，深度路由则根据去噪步长和视觉内容动态选择执行哪些块，从而实现对不同输入的自适应计算分配。在 Wan2.1-14B 上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 视频扩散 Transformer（DiT）虽然能生成高质量视频，但由于宽块、深架构和迭代采样导致计算开销巨大。现有方法通常采用固定的架构压缩策略（如固定剪枝宽度或移除某些层），无法根据输入内容和去噪阶段自适应调整计算量，导致效率与质量之间的权衡不够灵活。

Method: PARE 包含两个核心组件：1）宽度剪枝：观察到注意力头会特化为空间和时间两种角色，设计了区分时空头的重要性评分，避免过早剪掉对运动至关重要的时间头；2）深度自适应路由：训练一个轻量级路由器，以去噪时间步和视觉内容为条件，动态决定每个块是否执行，实现逐输入的自适应计算。采用渐进式训练流程：先通过蒸馏恢复宽度剪枝后的质量，再联合优化学生模型和路由器以解耦两个学习目标。

Result: 在 Wan2.1-14B 模型上，PARE 在图像到视频和文本到视频两种任务中显著减少了每步计算量，同时在 VBench 各维度上保持了生成质量，并且可以与步蒸馏（step distillation）组合使用以进一步加速。

Conclusion: PARE 通过结构感知的宽度剪枝和输入自适应的深度路由，实现了视频 DiT 的高效推理，在保持生成质量的同时大幅降低计算成本，且可与其他加速方法正交组合，为视频生成模型的实用化部署提供了有效方案。

Classification reason: 该论文提出 PARE 方法，通过剪枝（宽度压缩）和自适应路由（深度压缩）来加速视频扩散 Transformer（DiT）的推理，属于生成模型效率优化的核心范畴，因此映射到 [CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。
...
### [40] [Adversarial Dual On-Policy Distillation from Expressive Flow-based Teacher](https://arxiv.org/abs/2605.27095)
...
TL;DR: 本文提出FA-OPD，一种对抗性双轨在线策略蒸馏方法，从基于流匹配的教师模型中蒸馏知识到轻量MLP学生策略，结合奖励蒸馏和动作蒸馏两种互补信号，在机器人导航、操作和 locomotion 任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的模仿学习方法（如行为克隆、扩散/流匹配策略）本质上是离线监督学习，策略仅在专家状态上训练，无法获得其在自身访问状态上的纠正信号。在线策略蒸馏（OPD）可以解决这一问题，但标准OPD假设存在一个强大的固定教师，这在仅有演示数据的控制场景中不可用。

Method: 提出FA-OPD框架：1）从演示数据中学习一个流匹配（Flow Matching, FM）教师模型；2）将FM教师与轻量MLP学生策略协同训练；3）教师提供两种互补信号——奖励通道（reward channel）学习状态-动作对的专家似然目标，驱动长程策略优化的在线探索；动作通道（action channel）在学生访问的状态上提供密集的局部目标，稳定利用。两种通道耦合使用，奖励蒸馏实现超越逐点演示的泛化，动作蒸馏将探索锚定在专家行为附近。

Result: 在六个机器人导航、操作和 locomotion 基准上，FA-OPD 显著优于强基线方法，并且在噪声或有限演示条件下展现出更强的鲁棒性。

Conclusion: FA-OPD通过对抗性双轨在线策略蒸馏，有效弥合了离线模仿学习与在线策略优化之间的差距，为仅有演示数据的具身控制提供了一种高效且鲁棒的解决方案。

Classification reason: 该论文提出FA-OPD方法，通过流匹配（Flow Matching）教师模型蒸馏到轻量MLP学生策略，属于生成模型蒸馏与效率优化，与[多模态生成]方向下的[生成模型加速与高效化]（efficient_generation）子主题高度匹配。

Importance reason: Directly addresses model efficiency via distillation for generative policies, a high-priority subtopic. Signals: on-policy distillation, flow matching teacher, model efficiency, embodied control, generative policy acceleration
...
### [41] [Self-Verified Distillation: Your Language Model Is Secretly Its Own Synthetic Data Pipeline](https://arxiv.org/abs/2605.26132)
...
TL;DR: 本文提出 Self-Verified Distillation，一种无需外部教师或工具反馈的 LLM 自训练方法。模型仅使用无标注种子问题，通过自生成候选解答、三级级联自验证（循环一致性、事实性、正确性）过滤，并在自筛选数据集上训练。在 Qwen3 系列模型（0.6B/4B/8B）上，该方法在数学、科学、编程三个推理领域均取得显著提升，且测试时仅需单次推理调用。


<details>
  <summary>Details</summary>
Motivation: 探索 LLM 在 post-training 阶段能否仅利用无标注提示（无外部教师、无工具反馈）自我提升，降低对人工标注和外部反馈的依赖。

Method: 提出 Self-Verified Distillation 算法：1) 模型对无标注种子问题生成多个候选解答；2) 通过三级级联自验证过滤——循环一致性检查、事实性检查、正确性检查，所有验证器一致通过才接受；3) 在自筛选的高质量数据集上训练模型。该方法受 UQ benchmark 中多验证器筛选思路启发。

Result: 在 Qwen3-4B 上，数学（AIME26 和 HMMT）pass@1 提升 +16.7 点，科学（GPQA Diamond 和 HLE）提升 +11.1 点，编程（LCBv5 和 LCBv6）提升 +8.3 点。0.6B 和 8B 模型同样有增益。相比测试时计算扩展基线（UQ-TTC），本方法在多数设置下性能更优且测试时仅需单次推理。

Conclusion: Self-Verified Distillation 证明 LLM 可以仅通过自身生成和自验证实现自我提升，无需外部教师或工具反馈。采样更多候选解和更大的验证预算能产生更高质量的自筛选数据，从而获得更好的推理模型。

Classification reason: 该论文提出 Self-Verified Distillation，利用 LLM 自身进行自验证过滤生成数据并训练，属于生成模型的自训练与对齐优化。multimodal_generation 方向下的 [CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）涵盖对齐、偏好优化、guidance 与可控性，本文的自验证蒸馏本质上是一种无需外部反馈的自我对齐方法，因此映射至此。同时该工作也涉及 AIGC 中的合成数据与自训练流程。

Importance reason: Novel self-training method for LLMs without external feedback, but focuses on language model reasoning rather than the user's priority multimodal generation or video/image generation. Signals: self-training, self-verification, LLM reasoning, no external teacher, Qwen3
...
### [42] [CroCo: Cross-Lingual Contrastive Preference Tuning on Self-Generations](https://arxiv.org/abs/2605.26293)
...
TL;DR: 本文提出 CroCo（跨语言对比偏好调优），将基于自生成响应的对比偏好调优方法从英语扩展到多语言场景。实验表明，仅使用英语偏好训练的奖励模型可在多语言上产生有效排序，单语或多语设置下均能提升模型性能，同时防止监督微调灾难性遗忘。结构化任务中 EuroLLM-9B 在 6/7 语言上匹配或超越基线，开放生成中两个模型在 11 种语言上均优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有对比偏好调优方法仅在英语上验证有效，缺乏对多语言场景的探索。多语言偏好标注成本高昂，亟需一种无需语言特定偏好标注即可跨语言迁移的偏好调优方法。

Method: 基于英语偏好训练的奖励模型（置于多语言基座之上）对模型自生成响应进行评分，构建跨语言对比偏好对；在单语或多语设置下进行对比偏好调优（DPO 变体），并对比在线与离线变体效果。

Result: 结构化任务上，EuroLLM-9B 在 7 种语言中的 6 种匹配或超越基线，Aya-3B 在 7 种中的 4 种提升；开放生成任务中，两个调优模型在全部 11 种评估语言上均优于各自基线。在线偏好优化未能超越离线变体，且 off-policy 数据会降低收益。

Conclusion: 跨语言对比偏好调优（CroCo）无需语言特定偏好标注即可有效迁移，为多语言偏好调优提供了有前景的方向。on-policy 数据是关键，在线优化相比离线无额外优势。

Classification reason: 该论文研究跨语言对比偏好调优（preference tuning），属于生成模型对齐与奖励优化范畴，与 multimodal_generation 下的 [CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。同时，该方法也属于 AIGC 中的生成模型对齐方向。

Importance reason: Focuses on multilingual preference tuning, not directly on the user's priority subtopics like autoregressive video generation or world models. Signals: Cross-lingual transfer, preference tuning, self-generated responses, multilingual LLM alignment, on-policy data importance
...
TL;DR: 本文提出一种自动化训练流程，通过聚合多个弱评估指标的分数构建偏好数据集，利用偏好学习优化文本摘要的事实一致性，无需复杂奖励塑形，在多种模型上取得一致的事实性提升。


<details>
  <summary>Details</summary>
Motivation: 现有用于事实一致性摘要的评估指标尚不成熟，单一指标作为奖励信号不可靠，但多个弱指标的组合能更有效地捕捉多样的事实错误。因此，作者希望利用这一洞察，设计一种无需复杂奖励塑形的自动化训练方法。

Method: 通过聚合多个弱评估指标的分数，将分数映射为偏好，并过滤掉指标间高度不一致的样本。对每个源文档，通过改变解码策略生成词汇相似的摘要对，使模型能从细微词汇差异导致的事实差异中学习。最终构建高质量偏好数据集，使用偏好学习（如 DPO）进行训练。

Result: 在多种模型架构（从早期编码器-解码器到现代大语言模型）上均取得一致的事实性提升，较小的模型通过该方法可达到与较大模型相当的事实一致性水平。

Conclusion: 聚合多个弱评估指标作为偏好信号是一种有效且实用的方法，能够在不依赖复杂奖励建模的情况下显著提升摘要的事实一致性，且具有跨模型架构的泛化能力。

Classification reason: 该论文使用偏好学习（preference learning）和弱评估指标聚合来优化文本摘要的事实一致性，属于生成模型对齐与奖励（generation_alignment）范畴。该 subtopic 在 multimodal_generation 和 aigc 方向下均有收录，且为 [CANONICAL] 条目，优先复用。

Importance reason: Focuses on text summarisation factuality, not aligned with user's priority on multimodal generation and video/image generation. Signals: text summarisation, factual consistency, preference learning, weak metrics, not multimodal
...
### [44] [Tournament-GRPO: Group-Wise Tournament Rewards for Reinforcement Learning in Open-Ended Long-Form Generation](https://arxiv.org/abs/2605.26958)
...
TL;DR: 提出 Tournament-GRPO，通过分组锦标赛将 rubric 引导的 LLM 评判转化为相对奖励信号，用于开放域长文本生成的 GRPO 训练，在 Deep Research Bench 上取得 4.52 分提升。


<details>
  <summary>Details</summary>
Motivation: 开放域长文本生成中缺乏可靠的参考答案和自动评估指标，现有基于 rubric 的方法依赖 LLM 作为裁判进行点式评分，但绝对分数难以校准、区分度弱且容易饱和，需要更有效的奖励信号。

Method: 提出 Tournament-GRPO 框架：对同一查询的多个生成结果进行多轮分组锦标赛比较，累积锦标赛胜负结果并归一化为分组级相对奖励，用于 GRPO（Group Relative Policy Optimization）训练。

Result: 在 Deep Research Bench 上，Tournament-GRPO 一致优于现有奖励设计基线，相比最强基线取得 4.52 分的总体提升。锦标赛奖励在效果-效率权衡上表现良好，锦标赛设计影响训练动态。

Conclusion: Rubric 引导的锦标赛比较能够为开放域长文本生成的强化学习提供有效的奖励信号，分组锦标赛机制是比点式评分更优的奖励设计范式。

Classification reason: 该论文提出 Tournament-GRPO，一种基于分组锦标赛的奖励框架，用于开放域长文本生成的强化学习训练。核心贡献在于奖励信号设计（将 rubric 引导的 LLM 评判转化为相对奖励），属于生成模型对齐与奖励建模范畴。该论文同时涉及 AIGC 方向（长文本生成内容），但主要贡献在奖励机制设计，因此归入 multimodal_generation 方向的 generation_alignment 子主题（该子主题在 multimodal_generation 和 aigc 下均有，优先使用 multimodal_generation 下的 canonical 条目）。

Importance reason: Novel reward design for RL in long-form generation, but tangential to core video/image generation priorities. Signals: GRPO training, open-ended generation, rubric-guided reward, tournament comparison, Deep Research Bench
...
### [45] [Cross-scale Aligned Supervision for Training GANs](https://arxiv.org/abs/2605.26449)
...
TL;DR: 本文指出现有GAN的多尺度对抗监督存在"跨尺度轨迹不对齐"问题——各中间输出独立推向真实分布，但未确保它们代表同一生成样本的逐步细化。为此提出CAT（Cross-scale Aligned Transformer），在保持判别器尺度独立的同时，在生成器侧添加一致性正则化，强制中间输出与最终输出对齐。在ImageNet-256上，CAT-H/2以单步推理60轮训练达到FID-50K 1.56，超越强基线。


<details>
  <summary>Details</summary>
Motivation: 现有GAN在中间生成输出上施加对抗监督，被解释为粗到细的分层生成。但作者指出这种尺度级对抗监督并未构建真正的粗到细层级：每个中间图像独立推向其分辨率下的真实分布，但尺度级真实性不保证各阶段输出代表同一生成样本；且前一阶段的输出并未作为后一阶段的显式细化目标，导致各阶段可能漂移到不同样本而非逐步细化。

Method: 提出CAT（Cross-scale Aligned Transformer），核心思想是保持判别器尺度独立（每个中间输出在其分辨率下被评估），同时在生成器侧添加跨尺度一致性正则化，强制中间输出与最终输出对齐。具体通过一个轻量级Transformer模块将中间特征映射到与最终输出对齐的空间，使各阶段输出沿着同一样本轨迹逐步细化。

Result: 在类条件ImageNet-256×256上，CAT-H/2以单步推理仅训练60个epoch即达到FID-50K 1.56，超越当前最强的一步GAN和扩散/流模型基线。实验验证了跨尺度对齐正则化的有效性，并展示了模型在生成质量和训练效率上的显著优势。

Conclusion: 本文揭示了GAN多尺度生成中的跨尺度轨迹不对齐问题，并提出CAT通过生成器侧一致性正则化有效解决该问题。实验表明该方法在ImageNet-256上以极少的训练轮次取得了SOTA性能，验证了跨尺度对齐对于提升GAN生成质量和训练效率的重要性。

Classification reason: 该论文提出跨尺度对齐一致性正则化方法解决GAN生成中跨阶段轨迹不对齐问题，属于生成模型中对齐与可控性优化范畴，与multimodal_generation方向下的[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Focuses on GAN training alignment, not on the user's priority subtopics like autoregressive/diffusion video generation or world models. Signals: GAN training improvement, cross-scale alignment, ImageNet-256 SOTA, not aligned with user's priority subtopics, no video or world model application
...
### [46] [The Strongest Teacher Is Not Always the Best Teacher: Student-Centric Answer Selection](https://arxiv.org/abs/2605.26872)
...
TL;DR: 本文提出学生中心答案采样（SCAS）框架，挑战了"最强教师模型总是最佳教学者"的假设。通过从多个教师模型的正确答案中，根据学生中心学习成本（基于 token 级梯度分解推导的高效前向代理）选择最优监督信号，实验表明该方法在 30 个教师模型、6 个学生基座模型和 8 个任务上持续提升学生模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前 LLM 训练中广泛使用教师模型生成监督数据（合成回答、推理轨迹、工具使用示范等），但普遍默认选择性能最强的教师模型，隐含地将教师测试性能等同于教学质量。本文发现这一假设可能失效：即使多个教师对同一问题都给出正确答案，最强教师的答案未必是对特定学生最优的监督信号。因此需要一种学生中心的答案选择方法。

Method: 提出 Student-Centric Answer Sampling (SCAS) 框架。首先从多个教师模型收集正确答案，然后基于 token 级梯度分解推导出一个高效的前向传播代理指标来估计每个教师答案对学生模型的学习成本，最后根据该指标选择最优答案用于学生训练。该方法仅需前向计算，无需反向传播，计算效率高。

Result: 在 30 个教师模型、6 个不同规模的学生基座模型和 8 个多样化任务上的实验表明，SCAS 持续优于基线方法（包括随机选择、最强教师选择等），显著提升学生模型性能。验证了学生中心答案选择策略的有效性和泛化性。

Conclusion: 有效的知识蒸馏应优先考虑与学生当前能力匹配的监督信号，而非单纯依赖教师模型的强度。SCAS 提供了一种实用的学生中心答案选择框架，为 LLM 训练中的数据选择策略提供了新视角。

Classification reason: 该论文研究教师模型生成数据的选择策略以提升学生模型训练效果，属于生成模型训练中的对齐与监督信号优化问题。multimodal_generation 方向下的 [CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）涵盖生成模型对齐、偏好优化与可控性，与本文的教师-学生蒸馏中的答案选择策略高度相关。同时 aigc 方向下的 generation_alignment 也可匹配。

Importance reason: Focuses on LLM distillation rather than user's priority generative video/image models. Signals: student-centric distillation, teacher selection, token-level gradient proxy, LLM training, not generative video/image
...
### [47] [Beyond Pairwise Preferences: Listwise Reward-Aware Alignment for Diffusion Models](https://arxiv.org/abs/2605.26491)
...
Importance: 值得关注 (73.81) - 未入选精读

TL;DR: 本文提出 Diffusion LAIR，一种列表级奖励感知的偏好优化方法，通过将一组候选图像的奖励分数转化为中心化优势权重，并优化隐式奖励上的优势加权回归目标，实现对扩散模型的对齐。该方法在 SD1.5 和 SDXL 上的文生图、组合生成和图像编辑任务中优于现有偏好优化基线。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型偏好优化方法大多将监督信号简化为二元成对比较（赢/输），忽略了训练数据中同一提示通常包含多个候选图像的事实，且连续奖励分数比单一胜负标签包含更丰富的信息。因此需要一种能充分利用多候选图像和连续奖励分数的对齐方法。

Method: 对于每个提示，将一组候选图像的奖励分数转化为中心化优势权重，然后优化隐式奖励（定义为当前模型相对于固定参考模型的去噪损失改进）上的优势加权回归目标，并加入二次惩罚项以正则化隐式奖励的幅度。该目标在隐式奖励空间中具有有界闭式最优解，能显式控制正则化强度以调节偏好更新的幅度。

Result: 在 SD1.5 和 SDXL 上的实验表明，Diffusion LAIR 在文生图、组合生成和图像编辑基准上均优于强偏好优化基线方法。

Conclusion: 列表级奖励感知的偏好优化方法能更充分地利用多候选图像和连续奖励信息，有效提升扩散模型的对齐效果，且通过正则化机制保持保守更新，是一种优于成对偏好优化的对齐范式。

Classification reason: 该论文提出 Diffusion LAIR，一种基于列表级奖励感知的偏好优化方法，用于对齐文本到图像扩散模型。这属于生成模型对齐与偏好优化范畴，与 multimodal_generation 方向下的 [CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。同时该工作也涉及 AIGC 方向下的内容生成对齐。

Importance reason: Novel listwise reward-aware alignment for diffusion models, but the subtopic (generative model alignment) is not a user priority and lacks direct connection to video/image generation or efficiency. Signals: Listwise preference optimization, reward-aware alignment, diffusion model alignment, advantage-weighted regression, implicit reward regularization
...
Importance: 高优先级 (82.27) - 未入选精读

TL;DR: 本文提出FAV，一个通用的少步生成模型对齐框架。它将对齐问题转化为从奖励倾斜分布中采样，利用Stein变分梯度下降进行样本变分推断，并通过不动点回归将粒子更新摊销到生成器参数中。在机器人操作策略对齐（56个离线+30个离线到在线RL任务）和图像生成器对齐（GAN、一致性模型、流模型等多种少步骨干网络，从ImageNet-256到1024²文生图）上均取得优异效果。


<details>
  <summary>Details</summary>
Motivation: 现有少步生成模型的对齐方法通常依赖严格假设（如可计算似然、特定ODE/SDE求解器或特定模型族），缺乏通用框架。作者希望提出一种仅需生成器和参考分布样本访问权限的通用对齐方法。

Method: 将对齐建模为从奖励倾斜分布（锚定参考分布）中采样。使用Stein变分梯度下降（SVGD）作为基于样本的变分推断方案，通过不动点回归将粒子更新过程摊销到生成器参数中，实现端到端训练。

Result: 在机器人操作领域，FAV在56个离线RL任务和30个离线到在线RL任务上均优于现有策略提取基线。在图像生成对齐方面，FAV成功微调了GAN、漂移模型、一致性模型和流映射等多种少步骨干网络，从ImageNet-256扩展到1024²文生图合成。

Conclusion: FAV提供了一个通用、无需似然或特定求解器的少步生成模型对齐框架，在机器人策略和图像生成两个领域均验证了其有效性和可扩展性，为少步生成模型的对齐提供了新范式。

Classification reason: 该论文提出少步生成模型对齐框架FAV，核心是将对齐问题形式化为从奖励倾斜分布中采样，属于生成模型对齐与奖励优化范畴。multimodal_generation方向下的[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）完全匹配。同时该工作也涉及AIGC方向下的内容生成对齐。

Importance reason: Proposes a general alignment framework for few-step generative models, directly applicable to user's high-priority areas like image and video generation. Signals: general alignment framework, few-step generative models, Stein variational gradient descent, robotics policy alignment, image generator alignment
...
### [49] [Towards Controllable Image Generation through Representation-Conditioned Diffusion Models](https://arxiv.org/abs/2605.27343)
...
Importance: 值得关注 (72.59) - 未入选精读

TL;DR: 本文提出一种基于预训练自监督模型表征条件化的扩散模型，利用表征空间实现可控图像生成，展示了平滑且解耦的控制特性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的条件生成依赖文本提示或语义图等需要大量标注数据的条件机制，缺乏无需标注的可控生成方法。本文探索利用自监督学习表征作为条件，以实现更灵活、无需标注的可控图像生成。

Method: 使用预训练自监督模型（如 DINO）提取图像表征，将其作为扩散模型的条件输入。通过表征空间中的方向变化来探索控制维度，分析表征空间的平滑性和解耦性，从而实现对生成内容的可控编辑。

Result: 实验表明，自监督表征条件化不仅能提升无条件图像生成的质量，还提供了一个可用于控制生成的表征空间。在该空间中识别出的变化方向展现出良好的平滑性和解耦特性，初步验证了该方法在可控生成方面的潜力。

Conclusion: 自监督表征条件化为扩散模型提供了一种无需标注数据的可控生成途径，表征空间中的方向变化可用于控制生成内容，为未来更精细的可控生成研究奠定了基础。

Classification reason: 论文研究基于自监督表征条件扩散模型的可控图像生成，探索表征空间中的平滑与解耦方向以实现生成控制，属于生成模型的可控性与对齐范畴，因此映射到 multimodal_generation 方向下的 [CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）。

Importance reason: Novel conditioning approach but preliminary work with limited direct impact on user's priority subtopics. Signals: controllable generation, representation conditioning, self-supervised model, preliminary work, no video or autoregressive focus
...
### [50] [Not All Disagreement Is Learnable: Token Teachability in On-Policy Distillation](https://arxiv.org/abs/2605.26844)
...
TL;DR: 本文发现 on-policy distillation 中教师信号的 KL 分歧度并非可学习性的良好指标，提出了 token teachability 概念来区分可学习分歧与不兼容分歧，并基于此设计了 TA-OPD 方法，仅保留 5% 的高可学习性 token 即可超越全 token 蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 现有选择性 OPD 方法通过优先选择高熵或高分歧 token 来利用 OPD 信号的非均匀性，但本文发现原始 KL 分歧度是学习价值的粗糙代理，它混淆了可学习分歧（教师将纠正质量分配到学生 top-K 候选上）与不兼容分歧（教师质量落在学生当前支持集之外）。因此需要更精确的指标来识别真正可学习的教师信号。

Method: 提出 token teachability 概念，定义为教师分布与学生 top-K 候选之间的局部兼容性。基于此设计 Teachability-Aware OPD (TA-OPD)，一种轻量级 token 位置选择方法，仅在高 teachability 位置应用 OPD 损失，无需奖励模型或验证器。在 Qwen2.5 和 Qwen 3 教师-学生设置下进行实验验证。

Result: TA-OPD 仅保留 5% 的 token 即可超越全 token OPD 性能，并在熵基和分歧基的基线方法上取得一致提升。实验表明 teachability 比原始 KL 分歧能更好地预测固定上下文下的改进。

Conclusion: 选择性 OPD 的核心应在于选择可学习的教师信号而非仅仅选择显著 token。token teachability 为蒸馏中的信号选择提供了更精确的理论基础和实践指导。

Classification reason: 该论文研究 on-policy distillation 中 token 级别教师信号的可学习性，提出 teachability-aware 的选择性蒸馏方法，属于生成模型训练中对齐与信号优化范畴。multimodal_generation 方向下的 [CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）涵盖生成模型对齐、偏好优化、guidance 与可控性，本文的 token teachability 选择机制本质上是对蒸馏信号的选择与对齐优化，因此映射至此。同时，该工作也涉及 AIGC 方向下的生成模型训练优化。

Importance reason: Focuses on token-level distillation for LLMs, not directly on the user's priority subtopics like video generation or world models. Signals: on-policy distillation, token teachability, selective distillation, LLM alignment, KL divergence limitation
...
### [51] [Less is More: Early Stopping Rollout for On-Policy Distillation](https://arxiv.org/abs/2605.27028)
...
TL;DR: 本文发现 on-policy 蒸馏中存在"离策略教师衰减"问题：对于后序 token，学生生成的上下文偏离教师分布，导致教师评分能力下降。提出 Early Stopping Rollout (ESR) 策略，仅对前几个响应 token 进行 rollout 生成，简单有效，在多种模型规模、家族和任务上超越完整 rollout 的 OPD 性能，且 GPU 效率更高、训练更稳定。


<details>
  <summary>Details</summary>
Motivation: On-policy 蒸馏（OPD）中，学生用自己的 rollout 序列让教师评分，但后序 token 的上下文来自学生而非教师，导致教师评分能力衰减（Off-policy Teacher Decay），甚至退化为预训练阶段的 token 补全行为。作者希望解决这一问题，提升蒸馏效果。

Method: 提出 Early Stopping Rollout (ESR)：在 on-policy 蒸馏时，仅对前 K 个响应 token 进行 rollout 生成和教师评分，后续 token 直接使用教师模型的 teacher-forcing 监督。这是一种基于位置（position-based）的简单 token 选择策略。

Result: ESR 在多种模型规模、模型家族（LLaMA、Mistral 等）、任务（数学推理、通用对话）和训练设置下均超越完整 rollout 的 OPD 性能，GPU 效率更高，训练更稳定。在跨模型家族蒸馏场景下优势尤为明显，甚至有时能超越教师模型性能。

Conclusion: ESR 通过限制 rollout 长度有效缓解了离策略教师衰减问题，揭示了"级联对齐"（Cascading Alignment）和"子模式承诺"（Sub-mode Commitment）两种机制来解释其有效性。该策略简单、高效、稳定，为 on-policy 蒸馏提供了新的实用范式。

Classification reason: 该论文研究 on-policy distillation 中教师模型对学生 rollouts 的打分对齐问题，提出 Early Stopping Rollout 策略。这属于生成模型的对齐与蒸馏优化，与 multimodal_generation 下的 canonical subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配，同时也可归入 AIGC 方向。

Importance reason: Directly addresses efficient inference and model alignment for autoregressive generation, a top priority subtopic. Signals: on-policy distillation, autoregressive generation, efficient inference, training stability, model alignment
...
### [52] [LongAV-Compass: Towards Unified Evaluation of Minute-Scale Audio-Visual Generation Across T2AV, I2AV, and V2AV](https://arxiv.org/abs/2605.26244)
...
Importance: 高优先级 (86.17) - 未入选精读

TL;DR: LongAV-Compass 是一个面向分钟级（长时长）音视频生成的统一评测基准，包含 284 个测试用例，覆盖文本到音视频（T2AV）、图像到音视频（I2AV）和视频到音视频（V2AV）三种条件模态，结合 MLLM 辅助评估与多种感知/多模态指标，在 20+ 细粒度维度上评估跨段一致性、叙事连贯性、语义对齐和音视频同步等能力，并对 11 个代表性模型进行了实验验证。


<details>
  <summary>Details</summary>
Motivation: 现有音视频生成评测基准主要聚焦于 5-10 秒的短片段文本条件生成，缺乏对分钟级长时生成的统一评估，且不支持文本、图像、视频多种条件模态的统一评测，无法有效分析身份一致性、叙事连贯性和音视频对齐在长时间跨度下的退化问题。

Method: 构建了包含 284 个测试用例的基准，按应用场景和生成复杂度组织，覆盖 T2AV、I2AV、V2AV 三种条件模态。采用分类学引导的基准构建方法，结合 MLLM 辅助评估与 DINO-v2、ArcFace、CLIP、ImageBind 等感知/多模态指标，在 20+ 细粒度维度上评估段内质量、跨段一致性、全局叙事连贯性、语义对齐和音视频同步。

Result: 在 11 个代表性模型上进行了实验，并通过人类对齐验证，揭示了当前系统在维持分钟级音视频生成的连贯性、语义对齐和时间一致性方面的局限性。

Conclusion: LongAV-Compass 为分钟级音视频生成提供了系统性的诊断测试平台，填补了长时音视频生成统一评测的空白，有助于推动该领域的发展。

Classification reason: 该论文提出了一个面向分钟级音视频生成的统一评测基准 LongAV-Compass，涵盖 T2AV、I2AV、V2AV 三种条件模态，属于生成模型评测与数据集方向。该论文与 multimodal_generation 下的 [CANONICAL] subtopic "generation_benchmark"（生成模型评测与数据集）高度匹配，同时与 aigc 下的 "aigc_evaluation_benchmark"（AIGC 评测与基准）也相关。

Importance reason: Directly addresses minute-scale multimodal generation evaluation, a critical gap for your priority in video and unified multimodal models. Signals: minute-scale audio-visual generation, unified evaluation benchmark, T2AV/I2AV/V2AV, MLLM-assisted assessment, 20+ fine-grained dimensions
...
### [53] [GScomp-QA: A Subjective Dataset for Quality Assessment of Compressed Gaussian Splatting](https://arxiv.org/abs/2605.26880)
...
Importance: 值得关注 (65.46) - 未入选精读

TL;DR: 本文提出了GScomp-QA，一个针对压缩高斯泼溅（GS）模型合成质量的主观评估数据集。包含13个真实场景、9种GS压缩方案的331个视频刺激，通过20人主观实验提供感知评分，并评估了18种客观质量指标，发现现有指标无法充分捕捉GS特有的压缩失真。


<details>
  <summary>Details</summary>
Motivation: 高斯泼溅（GS）在3D重建和新视角合成中表现出色，但其模型体积大，存储和传输困难。尽管已有多种GS压缩方案，但由于缺乏专门的评估数据集，这些压缩方案对感知质量的影响尚不明确。

Method: 构建GScomp-QA数据集：从13个真实场景出发，使用9种SOTA GS压缩方案生成331个视频刺激，以未压缩模型合成的视频为参考，隔离压缩失真与合成伪影。组织20名参与者进行主观质量评估实验，获得可靠感知评分。基于数据对压缩方案进行率失真分析，并评估18种客观质量指标。

Result: 提供了GS压缩方案的主观感知评分和率失真分析结果；评估了18种客观质量指标，发现现有指标不能完全捕捉GS特有的压缩失真。

Conclusion: GScomp-QA数据集将公开，为GS压缩方案评估提供基准，并支持开发针对GS压缩的专用质量指标。

Classification reason: 该论文构建了GS压缩质量评估的主观数据集GScomp-QA，包含331个视频刺激、主观评分和客观指标评测，属于生成模型（3D高斯泼溅）的评测基准与数据集，与multimodal_generation下的[CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark）高度匹配。

Importance reason: Dataset for compressed Gaussian Splatting quality assessment is niche and lacks direct alignment with priority subtopics like video generation or world models. Signals: Gaussian Splatting compression, subjective quality dataset, no autoregressive or diffusion focus, negative keywords absent, moderate research value for 3D representation field
...
### [54] [Are Video Models Zero-Shot Learners and Reasoners in Education? EduVideoBench, A Knowledge-Skills-Attitude Benchmark for Educational Video Generation](https://arxiv.org/abs/2605.26918)
...
TL;DR: 本文提出EduVideoBench，首个基于知识-技能-态度(KSA)框架的教育视频生成评测基准，从教学充分性和教育安全性两个维度评估视频生成模型。对五个前沿VGM的评测表明，现有模型在教育场景下远未达到课堂就绪水平。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型评测仅关注感知质量、忠实度、通用安全性或将视频作为推理媒介，缺乏对教育领域有效性的评估。随着VGM快速进入课堂，亟需一个能评估教学充分性和教育安全性的专门基准。

Method: 基于KSA（知识-技能-态度）教育框架构建平衡的评测基准EduVideoBench，涵盖知识、技能、态度三个维度，联合评估教学充分性和教育安全性。对五个前沿VGM进行系统评测，并辅以专家定性分析。

Result: 五个前沿VGM在知识、技能、态度三个维度上均存在显著不足，远未达到课堂就绪水平。专家定性分析表明，教育有效性是多维度的，单个不匹配元素（如节奏、可读性、符号表示）即可使原本正确的视频失效。

Conclusion: EduVideoBench揭示了当前VGM在教育场景中的重大差距，为开发教学上合理且课堂安全的视频生成模型提供了指导方向。

Classification reason: 该论文提出了EduVideoBench，一个面向教育领域的视频生成评测基准，属于生成模型评测与数据集方向。该论文属于多模态生成方向（视频生成评测），同时也属于AIGC方向（教育领域生成内容评测）。映射到multimodal_generation下的[CANONICAL] subtopic "generation_benchmark"（生成模型评测与数据集）。

Importance reason: Niche educational benchmark with low relevance to core generative modeling advances. Signals: education domain, benchmark paper, no technical novelty, low alignment with priority subtopics, negative keywords absent
...
### [55] [RoMo: A Large-Scale, Richly Organized Dataset and Semantic Taxonomy for Human Motion Generation](https://arxiv.org/abs/2605.26241)
...
TL;DR: 本文提出了 RoMo，一个大规模、高质量、经过精心筛选的野外人体运动数据集，包含三层语义分类体系和详细文本标注。通过引入分类感知的过滤流程去除低质量序列，并发布 Motion Toolbox 标准化评测指标。实验表明，在 RoMo 上训练的模型在保真度和多样性上达到 SOTA，且能更好地理解复杂文本提示。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体运动数据集面临两难选择：小规模高保真的动作捕捉数据集 vs 大规模但充满静态/低质量序列的野外数据集。缺乏一个既大规模又高质量、带有丰富标注和结构化分类的运动数据集，阻碍了人体运动生成模型的发展。

Method: 1) 构建 RoMo 数据集：从大规模野外数据中收集，设计分类感知的过滤流程，主动去除静态序列和伪影序列。2) 提出三层语义分类体系（three-level semantic taxonomy），对每个序列进行详细文本标注。3) 开发 Motion Toolbox，标准化评测指标、数据转换和可视化工具。

Result: RoMo 数据集在规模和质量上优于现有数据集。基于 RoMo 训练的模型在保真度和多样性指标上达到 SOTA。三层分类体系支持细粒度的逐类别评测，揭示了全局指标掩盖的模型优缺点。模型对复杂、细微文本提示的理解能力显著提升。

Conclusion: RoMo 数据集及其配套工具为人体运动生成研究提供了高质量的数据基础和标准化评测平台，有望推动该领域向更可复现、更可解释的方向发展。

Classification reason: 该论文提出了一个大规模、高质量的人体运动数据集 RoMo，并附带语义分类体系和标准化评测工具。这属于生成模型评测与数据集（generation_benchmark）这一 [CANONICAL] 子主题，该子主题明确包含新数据集和评测基准。同时，该数据集服务于人体运动生成（属于多模态生成方向），也涉及 AIGC 领域的数据集构建。

Importance reason: Strong dataset contribution for motion generation, but only tangentially related to user's core focus on autoregressive/diffusion video generation and world models. Signals: Large-scale motion dataset, Semantic taxonomy, Taxonomy-aware filtering, Motion Toolbox, SOTA fidelity and diversity
...
### [56] [ReCA: Multi-Shot Long Video Extrapolation via Recursive Context Allocation](https://arxiv.org/abs/2605.26525)
...
Importance: 高优先级 (89.27) - 未入选精读

TL;DR: 本文提出多镜头视频外推（MSVE）任务，旨在将观察到的帧或片段扩展为具有电影化结构的镜头序列，同时保持锚定状态和推进叙事意图。作者识别了全局规划器过度指定、镜头级提示稀释状态、时序链造成状态衰减三个瓶颈，并提出递归上下文分配（ReCA）框架，将长视频生成递归分解为上下文有界的子问题，调用冻结生成器并在时间上传播结构化状态更新。在 MSVE-Bench 上，ReCA 相比最强基线提升 8-16% 平均归一化分数，多镜头一致性指标提升 28-43%。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法要么是单镜头外推（保持锚点但缺乏电影化结构），要么是多镜头叙事（自由创造视觉状态而非延续观察到的状态），无法同时实现结构化的多镜头叙事与对已观察状态的延续。长视频生成的失败不仅是上下文长度的限制，更是上下文分配（context allocation）的失败。

Method: 提出 Recursive Context Allocation (ReCA)，一个推理时框架，将 MSVE 递归分解为上下文有界的子问题，在叶节点调用冻结的短视频生成器，并在时间上传播结构化状态更新。ReCA 通过层次化规划与生成来分配上下文，避免全局规划器过度指定细节、镜头级提示稀释任务相关状态、以及时序链中身份/场景/物体/动作状态的衰减。

Result: 在提出的 MSVE-Bench 和 NB-Q 协议上，ReCA 相比最强竞争控制器提升 8-16% 平均归一化分数，多镜头一致性指标提升 28-43%。

Conclusion: ReCA 通过递归上下文分配有效解决了多镜头长视频外推中的上下文分配失败问题，证明了在有限单次生成预算下实现分钟级电影化视频生成的可行性。

Classification reason: 该论文提出多镜头长视频外推（MSVE）任务及递归上下文分配（ReCA）方法，核心是视频生成中的时序延续与状态保持，属于视频生成式世界模型/长视频生成范畴，与 canonical subtopic "视频世界模型"（video_world_model）高度匹配。

Importance reason: Directly addresses long video generation with world model-like state propagation, a top priority subtopic. Signals: video world model, autoregressive video generation, efficient inference, controllable generation, long video extrapolation
...
### [57] [When Does LeJEPA Learn a World Model?](https://arxiv.org/abs/2605.26379)
...
TL;DR: 本文从理论上证明，LeJEPA（对齐加高斯正则化）在潜在变量服从平稳加性噪声转移的世界中，能够线性恢复世界潜在变量（线性可识别性），且高斯分布是实现该保证的唯一潜在分布。论文还证明了近似可识别性结果，并验证了该理论在 2D 到 1024 维潜在空间及像素级机器人控制中的有效性。


<details>
  <summary>Details</summary>
Motivation: 世界模型需要能够恢复世界的真实自由度（潜在变量），才能支持可靠的规划和组合泛化。然而，现有方法缺乏理论保证，无法确定学习到的表征是否真正捕捉了世界的结构。本文旨在为 LeJEPA 这一经验上成功的框架提供数学基础，证明其能够保证恢复世界潜在变量。

Method: 论文通过理论分析证明 LeJEPA（对齐损失加高斯正则化）在线性可识别性方面的性质。利用谱分解方法，证明每个非线性度都会被对齐损失严格惩罚，使得线性映射成为最优解。同时通过反证法排除所有非高斯替代分布。进一步证明近似可识别性结果，并展示线性正交可识别性可实现最优潜在空间规划。

Result: 理论证明 LeJEPA 在潜在变量服从平稳加性噪声转移的世界中能线性恢复潜在变量；高斯分布是唯一能保证该性质的潜在分布；近似可识别性结果保证退化是平滑的。实验验证从 2D 示例到 1024 维潜在空间，包括分布消融实验和基于像素的机器人控制任务。

Conclusion: 本文将 LeJEPA 的经验成功转化为数学保证，证明其能够在线性可识别性意义下恢复世界结构，为构建可证明恢复世界结构的世界模型提供了理论基础。这一理论框架有助于指导未来世界模型的设计与改进。

Classification reason: 该论文研究 LeJEPA 是否能够学习世界模型（World Model），证明其在特定条件下能够线性恢复世界潜在变量，属于世界模型的理论分析。这与 [CANONICAL] subtopic "视频世界模型"（video_world_model）高度相关，该 subtopic 涵盖世界模型的理论与构建。

Importance reason: Directly addresses world model theory for video generation, a top-priority subtopic, with strong theoretical guarantees. Signals: world model theory, linear identifiability proof, LeJEPA analysis, Yann LeCun co-author, high-dimensional validation
...
### [58] [Can We Hear from Events? Generating Speech from Event Camera](https://arxiv.org/abs/2605.26672)
...
TL;DR: 本文提出 EventSpeech，首个利用事件相机（event camera）的微秒级神经形态事件进行文本条件语音生成的框架。通过事件编码器、多尺度音频编码器（含层次小波上下文器 HWC）和双向对齐机制，解决了传统 RGB 相机因曝光时间模糊导致的高频发音瞬态丢失问题。同时构建了 EVT-SPK 基准数据集，实验表明该方法在情感保留和抗运动模糊方面显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 传统基于 RGB 相机的语音生成受限于固定曝光时间，会模糊高频发音瞬态信号，难以生成富有表现力的情感语音。事件相机具有微秒级时间精度，天然与声学波形动态对齐，因此作者探索利用事件相机数据来突破这一瓶颈。

Method: EventSpeech 包含三个核心组件：(1) 事件编码器（Event Encoder），对稀疏神经形态事件进行建模；(2) 多尺度音频编码器，内含层次小波上下文器（Hierarchical Wavelet Contextualizer, HWC）提取多尺度声学特征；(3) 双向对齐机制，同步语言内容、视觉动态与密集声学特征。此外，构建了 EVT-SPK 基准数据集，包含大规模合成数据和真实硬件录制数据。

Result: 大量实验表明，EventSpeech 在情感保留和抗运动模糊方面显著优于现有基线方法，能够生成更细腻的情感语音，并建立了多模态语音生成的新范式。

Conclusion: 本文证明了事件相机数据在语音生成中的独特优势，EventSpeech 为多模态语音生成开辟了新方向，未来可进一步探索事件数据在其他音频生成任务中的应用。

Classification reason: 该论文提出 EventSpeech，利用事件相机（event camera）的神经形态事件进行文本条件语音生成，属于跨模态条件音频生成。这与 [CANONICAL] subtopic "音频与多模态联合生成"（audio_av_generation）高度匹配，该 subtopic 涵盖音视频联合生成与跨模态条件音频生成。

Importance reason: Novel use of event cameras for speech generation addresses a unique multimodal gap, but the audio focus is a secondary priority. Signals: multimodal_generation, event camera, speech generation, novel benchmark, emotional speech
...
### [59] [PilotTTS: A Disciplined Modular Recipe for Competitive Speech Synthesis](https://arxiv.org/abs/2605.27258)
...
Importance: 普通相关 (52.86) - 未入选精读

TL;DR: PilotTTS 是一个轻量级自回归文本转语音（TTS）系统，仅使用 200K 小时开源工具处理的数据，通过极简架构和严格数据工程实现竞争性能。它采用 Q-Former 条件机制解耦说话人身份与说话风格，支持零样本语音克隆、情感合成、副语言合成和中文方言合成，在 Seed-TTS Eval 基准上以最低 WER 和最高说话人相似度超越使用更大数据集训练的系统。


<details>
  <summary>Details</summary>
Motivation: 构建最先进的 TTS 系统通常需要数百万小时的专有数据和复杂的多阶段架构，这对资源受限的研究团队构成了巨大障碍。本文旨在通过极简架构和严格的数据工程，降低 TTS 系统构建的门槛。

Method: (1) 提出可复现的多阶段数据处理流水线，涵盖质量评估、标签标注和过滤；(2) 采用紧凑的模型架构，使用 Q-Former 条件机制通过跨样本配对训练解耦说话人身份与说话风格；(3) 在统一框架内支持零样本语音克隆、情感合成（11类）、副语言合成（4类）和中文方言合成（14种方言）。

Result: 在 Seed-TTS Eval 基准上，PilotTTS 在 test-en 上取得最低 WER 1.50%，在 test-zh 上取得 CER 0.87%，在两个测试集上均取得最高说话人相似度（0.862 和 0.815），超越了使用更大数据集训练的系统。

Conclusion: PilotTTS 证明了通过极简架构和严格数据工程，可以在有限数据条件下实现具有竞争力的 TTS 性能。论文开源了完整的数据流水线配方、预训练权重和代码，为资源受限的研究团队提供了可复现的 TTS 构建方案。

Classification reason: 该论文提出轻量级自回归TTS系统PilotTTS，属于语音合成（TTS）方向，完美匹配多模态生成方向下的[CANONICAL] subtopic "音频与多模态联合生成"（audio_av_generation），该subtopic明确包含TTS/语音生成。同时该论文也涉及AIGC方向下的内容生成应用。

Importance reason: Focuses on speech synthesis, which is not a priority subtopic and lacks boost keywords like video generation or world model. Signals: Lightweight autoregressive TTS, Q-Former decoupling, zero-shot voice cloning, 200K hours open-source data, competitive WER/speaker similarity
...
### [60] [Clinically-Grounded Counterfactual Reasoning for Medical Video Diagnosis](https://arxiv.org/abs/2605.26483)
...
Importance: 普通相关 (43.66) - 未入选精读

TL;DR: MedVCR 是一个基于临床先验的反事实推理框架，通过扩散模型生成病理状态下的组织演化图像，结合时序一致性、病理可分性和反事实对齐等临床规则进行表征学习，并采用视频级评估与帧级反事实分析的双重诊断策略，在阴道镜和结肠镜视频诊断任务上取得 2.6%-10.2% 的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有医学视频诊断方法采用端到端学习范式，存在三个问题：1）关注外观而非病理特征；2）缺乏临床先验知识；3）仅基于观测进行推理，缺乏反事实对比。受临床诊断思维启发，作者希望引入反事实推理来提升医学视频诊断的准确性和可解释性。

Method: MedVCR 包含三个核心组件：1）反事实生成器（Counterfactual Generator）：基于扩散模型，在指定病理状态下合成组织演化图像；2）反事实表征学习模块（Counterfactual Representation Learning）：通过时序一致性、病理可分性和反事实对齐三种临床规则约束进行表征学习；3）双重诊断预测策略（Dual Diagnostic Prediction）：结合视频级整体评估与帧级反事实分析进行综合诊断。

Result: 在完全监督（阴道镜）和弱监督（结肠镜）两种医学视频诊断场景下，MedVCR 相比主流基线方法取得 2.6%-10.2% 的性能提升。全面的消融实验验证了各组件有效性。

Conclusion: MedVCR 通过将反事实推理引入医学视频诊断，有效融合了临床先验知识，显著提升了诊断性能，为医学影像分析提供了新的范式。

Classification reason: 该论文提出 MedVCR，一个用于医学视频诊断的反事实推理框架，包含基于扩散的病理状态图像生成模块。论文核心是医疗领域的生成与诊断应用，因此映射到 [CANONICAL] 领域内容生成应用（aigc_domain_application），该 subtopic 在 multimodal_generation 方向下也有收录。

Importance reason: Strong medical domain application but low alignment with user's priority subtopics and negative keywords. Signals: diffusion-based counterfactual generation, medical video diagnosis, clinical prior integration, weakly supervised video diagnosis, counterfactual reasoning framework
...
### [61] [CmIVTP: Cross-modal Interaction-based Vessel Trajectory Prediction for Maritime Intelligence](https://arxiv.org/abs/2605.26524)
...
TL;DR: 本文提出 CmIVTP，一个跨模态交互的船舶轨迹预测框架，融合 AIS 运动数据与 CCTV 视频数据，通过目标感知场景编码器和跨模态交互 Transformer 实现更准确的轨迹预测，并构建了 Maritime-MmD⁺ 多模态数据集。


<details>
  <summary>Details</summary>
Motivation: 单一数据源（AIS 或 CCTV）在船舶轨迹预测中存在局限性：AIS 数据对小船只稀疏或不可用，CCTV 数据无法完全捕捉动态船舶行为。因此需要跨模态融合来提升预测准确性。

Method: 提出 CmIVTP 框架，包含：(1) 目标感知场景编码器，提取场景语义特征以捕捉船舶-环境交互；(2) 跨模态交互 Transformer，融合 AIS 运动特征、CCTV 环境特征和场景表示，通过跨模态注意力同时捕获模态内语义和模态间交互；(3) 船舶群轨迹库，通过聚类历史 AIS 轨迹生成代表性运动模式用于候选轨迹生成。同时构建了 Maritime-MmD⁺ 多模态数据集。

Result: 在多模态驱动的船舶轨迹预测基准上，CmIVTP 取得了优于现有方法的性能。

Conclusion: 跨模态交互融合 AIS 和 CCTV 数据能有效提升船舶轨迹预测的准确性和环境可行性，为海事智能交通系统提供了新的解决方案。

Classification reason: 该论文聚焦于海事领域的船舶轨迹预测，属于垂直领域（海事智能交通）的生成/预测应用。虽然现有 canonical 中 aigc_domain_application 描述为"医疗/法律/科学/代码等"，但该 subtopic 在 multimodal_generation 方向下已有 3 篇论文，且海事轨迹预测可归为垂直领域生成应用，因此复用该 subtopic。

Importance reason: Vessel trajectory prediction is a niche maritime application with no overlap with user's priority subtopics or boost keywords. Signals: domain-specific maritime application, no autoregressive or diffusion generation, no video/image generation, negative keyword "traffic prediction" applies, low general AIGC relevance
...
### [62] [Design First, Code Later: Aesthetically Pleasing Template-Free Slides Generation](https://arxiv.org/abs/2605.26451)
...
TL;DR: 本文提出 DeepSlides，一种无需预定义模板的层次化幻灯片生成工作流，将幻灯片设计解耦为页面设计与代码实现两个阶段，并引入 SlideDesign 数据集和多智能体强化学习训练范式，在自动评测和人工偏好评估中均优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有幻灯片自动生成方法依赖固定模板或直接生成可执行代码，限制了 LLM 的创意布局设计能力，且跳过了关键的页面设计步骤，导致生成质量受限。

Method: 提出 DeepSlides 层次化工作流，将幻灯片生成解耦为设计阶段（页面布局、视觉风格）和实现阶段（代码生成）；构建 SlideDesign 数据集；采用多智能体强化学习训练范式，训练 SlideQwens 模型分别负责设计与实现。

Result: 在自动评测指标和人工偏好评估上，DeepSlides 均优于现有基线方法，证明了设计-实现解耦策略的有效性。

Conclusion: 将幻灯片生成中的设计过程与实现过程解耦，能够显著提升生成质量，为结构化多模态内容生成提供了一种有效范式。

Classification reason: 该论文提出了一种层次化的幻灯片生成工作流 DeepSlides，将幻灯片设计解耦为页面设计（设计优先）和代码实现（编码在后），并采用多智能体强化学习训练范式。这属于 AI 智能体驱动的自动化创作流水线，与 [CANONICAL] subtopic "AI 智能体与生成工作流" (aigc_agent_workflow) 高度匹配。

Importance reason: Focuses on slide generation workflow, which is not a priority subtopic and lacks strong alignment with user's key directions. Signals: template-free slides generation, hierarchical workflow, multi-agent RL, SlideDesign dataset, not in priority subtopics
...
### [63] [EmoDistill: Offline Emotion Skill Distillation for Language Model Agents in Adversarial Negotiation](https://arxiv.org/abs/2605.26785)
...
TL;DR: EmoDistill 提出一个离线框架，通过隐式Q学习选择情感策略、LoRA微调表达情感，将情感谈判技能蒸馏到小语言模型智能体中，在对抗性谈判中显著提升效用。


<details>
  <summary>Details</summary>
Motivation: 后训练对齐的LLM在对抗性谈判中容易被情感化语言操控，导致智能体向对方利益让步。现有方法依赖在线谈判训练成本高昂，且缺乏系统性的情感策略蒸馏方法。

Method: 将情感策略分解为情感选择（which emotion）和情感表达（how to express）。使用Implicit Q-Learning (IQL) 学习选择最优情感，通过LoRA-based SFT + Judge Policy Optimization (JPO) 学习情感表达。整个框架在离线agent-to-agent交互数据上训练。

Result: 在四个高风险的谈判领域（如薪资谈判、商业交易等）中，EmoDistill训练的SLM策略超越原始SLM/LLM基线和仅IQL情感选择的方案。消融实验证明情感条件化至关重要，迁移实验显示跨领域、未见对手和训练vs训练锦标赛的泛化能力。

Conclusion: 情感是对抗性谈判中的战略性行为通道，EmoDistill能够从离线交互中有效蒸馏情感谈判技能，避免昂贵的在线训练，且具有良好的泛化能力。

Classification reason: 该论文研究通过离线蒸馏框架（EmoDistill）为语言模型智能体注入情感谈判技能，属于LLM智能体能力增强与自动化工作流范畴，与aigc_agent_workflow（AI智能体与生成工作流）高度匹配。

Importance reason: Focuses on emotion strategy distillation for negotiation agents, which is niche and not aligned with user's priority subtopics in generation and efficiency. Signals: emotion strategy, adversarial negotiation, offline distillation, LoRA fine-tuning, small language model agents
...
### [64] [SeDT: Sentence-Transformer Decision-Transformer Conditioning for Multi-Turn Conversation Reliability](https://arxiv.org/abs/2605.26788)
...
TL;DR: 本文发现LLM在多轮对话中性能会下降高达39%（称为"Lost in Conversation"），原因是平坦的对话历史给每轮对话赋予相同权重。作者提出SeDT（Sentence-transformer Decision-Transformer），一种无需训练的推理时方法，通过从离线强化学习中引入return-to-go条件化，为每轮对话标注累积相关性分数，在不改变权重、无需训练数据的情况下，显著恢复多轮对话中丢失的性能。


<details>
  <summary>Details</summary>
Motivation: LLM在单轮任务中表现优异，但当同一任务在多轮对话中逐步揭示时，性能会下降高达39%，且不可靠性翻倍。现有方法要么需要训练数据，要么丢弃上下文信息，无法有效解决这一结构性缺陷。

Method: SeDT从离线强化学习中引入return-to-go条件化，使用Sentence Transformer计算每轮对话的语义、词汇和位置三种互补信号的累积相关性分数，将完整标注的对话历史在最后一轮呈现给模型，无需权重修改、无需训练数据、无需丢弃上下文。

Result: 在Lost-in-Conversation基准上，对3个LLM和3种生成任务进行测试，SeDT在所有9个模型-任务组合中均优于基线，平均性能提升最高达+37.7%，同时在9个组合中有7个降低了不可靠性。

Conclusion: 告诉模型哪些历史对话轮次更重要，就足以显著恢复多轮对话中丢失的性能。SeDT是一种简单有效的训练无关方法，为多轮对话可靠性问题提供了新的解决思路。

Classification reason: 论文关注多轮对话中LLM的可靠性下降问题，提出一种无需训练的推理时方法（SeDT）来改善多轮对话中的条件生成可靠性。这属于LLM智能体与生成工作流中的对话可靠性增强，与aigc_agent_workflow（AI智能体与生成工作流）高度相关。

Importance reason: Addresses a niche LLM reliability issue with a training-free method, but is far from the user's priority topics in generative vision and efficient models. Signals: LLM multi-turn reliability, training-free inference method, offline RL conditioning, no generative vision or video focus, no efficiency or world model angle
...
### [65] [MAIGO: Mitigating Lost-in-Conversation with History-Cleaned On-Policy Self-Distillation](https://arxiv.org/abs/2605.27186)
...
TL;DR: 本文提出 MAIGO，一种基于历史清理的 on-policy 自蒸馏方法，用于缓解大语言模型在多轮对话中的 lost-in-conversation 问题。该方法通过移除中间轮次的助手回复来减少自我污染，并利用可靠性权重对不一致样本降权，无需验证器奖励或推理时辅助，显著提升了对话长程任务的表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在单轮完整提示下表现良好，但在多轮对话中性能显著下降（lost-in-conversation 问题）。作者发现部分退化源于"自我污染"（self-contamination）：中间轮次的助手回复进入后续上下文，将早期偏差向前传递。现有方法需要验证器奖励、状态标签或推理时辅助，缺乏轻量级训练方案。

Method: 提出 MAIGO（Mitigating Lost-in-Conversation with History-Cleaned On-Policy Self-Distillation）。核心思想：对于中间轮次，移除先前助手回复，仅保留用户可见的分片前缀（sharded prefix）；对于最终答案轮次，从基于完整用户侧对话的 paired full-view 参考中蒸馏。同时引入可靠性权重，对与清理后参考不一致的中间轮次样本进行降权。该方法完全基于模型自身策略，无需外部验证器或标注。

Result: 在 Qwen2.5-7B-Instruct 上，MAIGO 将 SHARDED 准确率从 52.8 提升至 66.1，SHARDED/FULL 比率从 66.5% 提升至 84.1%，同时 FULL 准确率仅下降 2.3 个百分点。实验表明自我污染是 LiC 差距中可训练的成分。

Conclusion: 自我污染是多轮对话性能退化的重要因素，MAIGO 通过历史清理的自蒸馏方法有效缓解了这一问题，且无需额外监督信号。该方法为提升 LLM 在多轮交互中的长程一致性提供了轻量级训练方案。

Classification reason: 论文研究多轮对话中 LLM 的 lost-in-conversation 问题，提出基于 on-policy self-distillation 的缓解方法，属于 LLM 对话智能体工作流优化范畴，映射到 AIGC 方向下的 [CANONICAL] subtopic "AI 智能体与生成工作流"。

Importance reason: Focuses on LLM conversation degradation, not on user's priority multimodal generation or efficient inference topics. Signals: LLM conversation improvement, self-distillation method, no verifier needed, moderate performance gains, not aligned with user's core directions
...
### [66] [SetupX: Can LLM Agents Learn from Past Failures in Functionality-Correct Code Repository Setup?](https://arxiv.org/abs/2605.26186)
*Zihang Zhou, Ziqian Ren, Yukai Wu, Yingjie Xiong, Wei Zhou, Chao Peng, Dong Zhang, Bingheng Yan, Xuanhe Zhou, Fan Wu*

Main category: cs.CL, cs.LG, cs.SE, cs.AI
...
TL;DR: SetupX 是一个基于经验学习的 LLM 智能体框架，用于自动化完成功能正确的代码仓库环境配置。它通过自进化经验表示（XPU）、经验增强的推测执行（基于 LIFO Docker 快照栈）和检察官-法官验证协议，解决了跨仓库经验迁移、多步试错回滚和可靠验证三大挑战，在基准测试中达到 92% 的通过率，超越最强基线 19% 以上。


<details>
  <summary>Details</summary>
Motivation: 代码仓库的环境配置（依赖安装、构建脚本等）是软件工程中的关键但极具挑战性的任务。现有 LLM 智能体难以处理跨仓库经验迁移、非可逆状态变更下的多步试错修复，以及区分设置失败与仓库自身 bug 的可靠验证。这些挑战导致自动化仓库设置的成功率低下。

Method: SetupX 包含三个核心组件：(1) 自进化经验表示（XPU），一种双模态知识单元，编码设置信号、文本指导和可执行动作，实现已验证环境修复的动态跨仓库迁移；(2) 经验增强的推测执行，基于 LIFO Docker 快照栈，允许智能体主动尝试修复并在失败时安全回滚到已知良好状态；(3) 检察官-法官验证协议，将证据收集与最终判断分离，实现超越表面构建时指标的更可靠设置验证。

Result: 在精心构建的基准测试上，SetupX 达到 92% 的通过率，超越最强基线 19% 以上。尤其在需要协调多个容器间互联服务的复杂多仓库设置任务中表现优异。

Conclusion: SetupX 通过经验学习、推测执行和分层验证机制，显著提升了 LLM 智能体在代码仓库环境配置任务上的能力，证明了跨仓库经验迁移和可靠回滚机制对于复杂软件工程自动化的重要性。

Classification reason: 该论文研究 LLM 智能体在代码仓库环境配置中的自动化设置与故障修复，属于 AI 智能体与自动化工作流范畴，与 canonical 子主题 aigc_agent_workflow 高度匹配。

Importance reason: Strong engineering contribution to LLM agent reliability but outside core generative modeling priorities. Signals: LLM agent for code setup, experience transfer via XPU, LIFO Docker rollback, verification protocol, 92% pass rate
...
### [67] [Experiments in Agentic AI for Science](https://arxiv.org/abs/2605.26305)
*Judy Fox, Geoffrey Fox*

Main category: hep-ph, eess.SY, cs.AI
...
TL;DR: 本文提出了两个面向科学工作流的自主AI智能体框架：DeepTS/DeepCollector用于大规模时间序列数据集的自动整理与去重，DeepScribe用于将密集的物理讲座幻灯片自动转化为结构化科学报告。两者均采用"本地身体-远程大脑"混合架构，通过Python编排器调用LLM云后端，展示了智能体AI在科学工作流中的应用潜力。


<details>
  <summary>Details</summary>
Motivation: 当前科学工作流中，数据整理、文献分析等任务高度依赖人工，现有LLM系统在上下文长度和推理能力上存在局限，难以自主完成复杂的多步骤科学任务。作者旨在通过智能体系统克服这些限制，实现科学工作流的自动化。

Method: 提出混合"本地身体-远程大脑"架构，基于Google Colab实现。DeepTS/DeepCollector采用Cellular RAG（细粒度属性提取）、远程数据检查和分布式并发控制来自动化时间序列数据集的整理与去重。DeepScribe作为自主演示分析器，将视觉密集的物理讲座幻灯片转化为结构化科学报告。系统通过Python本地编排器调用LLM云后端实现自主决策。

Result: 展示了两个智能体系统在科学工作流中的有效性：DeepTS/DeepCollector实现了大规模时间序列数据集的自动化整理，DeepScribe能够将复杂物理讲座转化为结构化报告。论文还讨论了DeepTS向深度知识图谱的泛化以及在高能物理（DeepQCD）中的应用前景。

Conclusion: 智能体AI通过实用的系统工程方法（如Cellular RAG、远程数据检查、分布式并发控制）能够克服当前LLM的上下文和推理限制，有效支持科学工作流自动化。该框架可推广到深度知识图谱构建和高能物理等更广泛的科学领域。

Classification reason: 论文提出两个科学工作流中的自主AI智能体框架（DeepTS/DeepCollector和DeepScribe），属于LLM智能体与自动化工作流范畴，与[AIGC]方向下的[AI智能体与生成工作流]（aigc_agent_workflow）高度匹配。

Importance reason: Focuses on agentic AI for science workflows, not directly on the user's priority subtopics like video/image generation or world models. Signals: Agentic AI for science, hybrid local-remote architecture, time-series curation, lecture-to-report automation, not aligned with core generative video/image priorities
...
### [68] [UnityMAS-O: A General RL Optimization Framework for LLM-Based Multi-Agent Systems](https://arxiv.org/abs/2605.26646)
...
Main category: cs.CL, cs.MA, cs.AI
...
TL;DR: UnityMAS-O 是一个面向基于 LLM 的多智能体系统的通用强化学习优化框架，将完整工作流作为优化单元，通过逻辑角色、图轨迹、用户定义奖励和智能体-模型映射四个核心对象解耦逻辑智能体与物理模型参数，支持多种参数共享策略和分布式 PPO 训练，在检索增强 QA、迭代搜索和代码生成等任务上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于 LLM 的多智能体系统大多通过提示词、工具和控制规则手动编排，缺乏统一的强化学习优化接口；现有的 RL 后训练框架主要针对单策略优化，缺少对用户定义的多智能体工作流、结构化交互、角色级信用分配和可配置参数共享的抽象支持。

Method: UnityMAS-O 将完整工作流作为优化单元，通过四个一级对象（逻辑智能体角色、图轨迹、用户定义奖励、智能体-模型映射）表示工作流，解耦逻辑智能体与物理模型参数，支持完全共享、完全分离和部分共享三种参数共享模式。基于 verl 和 Ray 的星型拓扑运行时，中央控制器执行工作流、调用工具、记录结构化轨迹并组装奖励，模型本地工作节点组负责 rollout、缓冲、优势计算和分布式 PPO 更新。

Result: 在 Natural Questions、HotpotQA 和保留的代码任务上，多智能体 RL 优化后显著改进了手动指定的工作流，尤其在小模型和严格代码全通过指标上提升明显。

Conclusion: UnityMAS-O 可作为可复用的基础框架，将多样化的基于 LLM 的多智能体工作流转化为可训练的多智能体 RL 系统，验证了多智能体 RL 优化在复杂任务中的有效性。

Classification reason: 该论文提出基于强化学习的多智能体系统优化框架 UnityMAS-O，属于 LLM 智能体与生成工作流范畴，与 [CANONICAL] 子主题 aigc_agent_workflow（AI 智能体与生成工作流）高度匹配。

Importance reason: Focuses on multi-agent RL optimization for LLM systems, which is outside the user's priority subtopics of generative models and efficient inference. Signals: multi-agent RL framework, LLM-based agents, workflow optimization, not generative model, not video/image generation
...
### [69] [It's Not the Capability: Harness Sensitivity Is Non-Monotone Across LLM Agent Tiers](https://arxiv.org/abs/2605.26731)
*Yong-eun Cho*
...
TL;DR: 本文通过 432 次实验，系统评估了不同能力层级的 LLM agent 在不同约束条件（轻量/平衡/严格）下的表现，推翻了"能力越强所需约束越少"的单调递减假设，发现前沿聊天模型在更严格约束下性能反而下降（29-38 个百分点），而推理模型在严格约束下表现最佳，表明约束敏感性是非单调的且取决于模型类型。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM agent 部署中普遍假设更结构化的约束（harness）能普遍提升可靠性，且能力更强的模型需要的结构引导更少，即模型能力层级与最优约束复杂度之间存在单调反比关系。作者希望通过系统实验验证这一假设是否成立。

Method: 在 HEAT-24 基准（24 个基于 git 工作区验证的合成任务）上，对 6 个模型（覆盖 4 个能力层级）在 3 种约束条件（轻量、平衡、严格）下进行 432 次受控实验，并引入六标签失败分类法分析失败模式。

Result: 1）前沿聊天模型（Gemini 2.5 Flash）在更严格约束下 VTSR 下降 29-38 个百分点，出现"约束复杂度悖论"；2）前沿推理模型（Qwen3.5-122B 启用扩展思考）在严格约束下达到最高 VTSR（91.7%）和最低延迟；3）受限层级中 2B 模型（Gemma4:e2B）在所有约束下均达到 91.7% 的稳定性；4）格式违规是能力强模型的主要失败类型，而错误文件是低能力模型的主要失败类型。

Conclusion: 模型能力层级与最优约束复杂度之间不存在单调反比关系，约束敏感性是非单调的，且关键取决于模型类型（聊天 vs. 推理）。作者提出了基于层级的约束选择实用指南。

Classification reason: 论文研究 LLM agent 在不同约束条件（harness）下的表现，属于 AI 智能体与生成工作流（aigc_agent_workflow）范畴，该 canonical subtopic 涵盖 LLM 智能体、工作流、工具使用等主题，与本文高度匹配。

Importance reason: Focuses on LLM agent harness sensitivity, not on the user's priority subtopics like video generation or world models. Signals: LLM agent evaluation, harness sensitivity, non-monotone performance, model tier comparison, failure taxonomy
...
### [70] [Stateful Inference for Low-Latency Multi-Agent Tool Calling](https://arxiv.org/abs/2605.26289)
*Victor Norgren*
...
TL;DR: 本文提出了一种有状态推理架构，通过持久化 KV 缓存、基数前缀缓存和提示查找推测解码，将多智能体工具调用场景中每轮推理的计算成本从 O(n_t) 降低到 O(Δ_t)，在 6 轮和 35 轮智能体工作流中分别实现 2.1 倍和 4.2 倍的每轮加速，端到端耗时减半。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 推理框架将每次工具调用视为独立请求，重复处理整个对话历史，而实际场景中 85-95% 的提示内容与上一轮相同，造成了巨大的计算浪费。多智能体工具调用已成为 LLM 系统的主流交互模式，亟需一种能利用跨轮次上下文复用来降低延迟的推理架构。

Method: 提出有状态推理架构，包含三个核心组件：(1) 持久化 KV 缓存，跨轮次存活，仅增量处理新 token；(2) 基数前缀缓存，扩展支持交错的多智能体流量复用；(3) 提示查找推测解码器，加速结构化输出生成。参考实现基于这些组件构建，并与 vLLM 和 SGLang 进行对比。

Result: 在 6 轮智能体工作流中每轮加速 2.1 倍，在 35 轮工作流的中间轮次加速 4.2 倍，端到端耗时减半。优势来源于有状态复用和推测解码，而非传统缓存。

Conclusion: 有状态推理架构能显著降低多智能体工具调用场景的推理延迟，将计算成本从与历史长度成正比变为仅与增量 token 数成正比，为 LLM 系统的高效部署提供了新思路。

Classification reason: 论文提出了一种面向多智能体工具调用的有状态推理架构，通过持久化 KV 缓存和前缀缓存降低延迟，属于 LLM 智能体与生成工作流（aigc_agent_workflow）范畴，与 [CANONICAL] 子主题高度匹配。

Importance reason: Directly addresses efficient inference for multi-agent tool calling, a key AIGC workflow, with practical latency gains. Signals: efficient inference, KV cache reuse, multi-agent tool calling, latency reduction, speculative decoding
...
### [71] [MobileExplorer: Accelerating On-Device Inference for Mobile GUI Agents via Online Exploration](https://arxiv.org/abs/2605.26546)
...
TL;DR: MobileExplorer 提出一种面向移动 GUI 智能体的端侧推理加速框架，通过在 VLM 推理过程中并行探索 UI 元素并记录为结构化记忆，结合两级回滚机制保证执行可靠性，最终将上下文提示注入后续推理步骤，在 AndroidWorld 基准上减少 23% 的推理步骤和端到端延迟，同时任务成功率提升最高 5%。


<details>
  <summary>Details</summary>
Motivation: 现有移动 GUI 智能体大多依赖云端模型推理，存在隐私风险和网络延迟，而完全端侧部署的移动 GUI 智能体尚未被充分探索。作者希望在不牺牲任务准确率的前提下，加速端侧 VLM 推理。

Method: 利用 VLM 每步推理时间较长的特点，在模型推理期间并行、轻量地探索 UI 元素，记录探索轨迹为结构化记忆；设计两级回滚机制以在实时移动环境中可靠恢复初始 UI 状态；将探索轨迹总结为简洁的上下文提示注入后续推理步骤。

Result: 在 AndroidWorld 基准及新设计的更复杂任务和动态端侧环境中，MobileExplorer 将平均推理步骤数和端到端延迟减少 23%，同时任务成功率维持或提升最高 5%。

Conclusion: MobileExplorer 通过在线探索和结构化记忆有效加速了端侧移动 GUI 智能体的推理，证明了在端侧部署高效 GUI 智能体的可行性，为隐私保护和低延迟移动 AI 应用提供了新思路。

Classification reason: 该论文研究移动端 GUI 智能体的端侧推理加速，属于 AI 智能体与自动化工作流范畴，与 [CANONICAL] subtopic "AI 智能体与生成工作流" (aigc_agent_workflow) 高度匹配。

Importance reason: Focuses on on-device inference acceleration for mobile GUI agents, which is tangentially related to model efficiency but not directly aligned with core generative video/image or multimodal generation priorities. Signals: On-device inference acceleration, Mobile GUI agent, VLM reasoning optimization, Structured memory for UI exploration, Two-level rollback mechanism
...
### [72] [SIA: Self Improving AI with Harness & Weight Updates](https://arxiv.org/abs/2605.27276)
...
TL;DR: SIA 提出一种自改进框架，让一个语言模型智能体（Feedback-Agent）同时更新任务智能体的 harness（工具、提示、重试逻辑等）和模型权重，在中文法律罪名分类、GPU 内核优化和单细胞 RNA 去噪三个领域取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前 AI 的构建和改进严重依赖人类，模型和智能体的编写、调优和纠错都由人工完成。已有的 harness-update 和 test-time training 两条研究路线各自独立，未能结合 harness 更新和权重更新两种自我改进方式。本文旨在弥合这一鸿沟，实现 AI 的自主自我改进。

Method: SIA 包含一个 Feedback-Agent（语言模型智能体），它接收任务智能体的执行反馈，同时更新两个部分：(1) harness 更新——重写任务智能体的工具、提示、重试逻辑和搜索过程；(2) 权重更新——通过任务反馈对模型权重进行微调。两者结合形成闭环自改进循环。

Result: 在三个对比领域上，结合 harness 和权重更新均优于仅迭代 scaffold 的基线：LawBench 提升 56.6%，GPU 内核优化运行时减少 91.9%，单细胞 RNA 去噪提升 502%。

Conclusion: 同时更新 harness 和权重比单独使用任何一种方式效果更好。Harness 更新使模型具备智能体能力（如何搜索和行动），而权重更新构建了提示或 scaffold 无法灌输的领域直觉。两者互补，共同推动 AI 自我改进。

Classification reason: 该论文提出 SIA 自改进循环，让一个语言模型智能体（Feedback-Agent）同时更新任务智能体的 harness（工具、提示、搜索过程）和模型权重，属于 LLM 智能体自动化与自我改进工作流，与 [CANONICAL] subtopic "AI 智能体与生成工作流" 高度匹配。

Importance reason: Self-improving AI framework is interesting but focuses on legal, GPU, and bio domains far from user's priority subtopics. Signals: self-improving AI, harness and weight updates, legal classification, GPU kernel optimization, single-cell RNA denoising
...
### [73] [AGORA: Adapter-Grounded Observation-Action Retention for Inference-Free Prompt Compression in LLM Agents](https://arxiv.org/abs/2605.26596)
...
TL;DR: 本文发现 token 级提示压缩方法在 LLM Agent 场景中会破坏动作语法（action-grammar destruction），导致性能崩溃。提出 AGORA——一种推理时零开销的 step 级压缩器，结合结构解析器、格式保留机制和 125M 参数的相关性评分器，在 9 个测试单元中 8 个保持 ≥75% 的原始性能。


<details>
  <summary>Details</summary>
Motivation: 现有 token 级提示压缩方法在 LLM Agent 场景中失效，因为动作语义 token（标识符、括号、动作动词）的自信息排名最低，通用压缩器会可靠地移除它们，导致环境拒绝执行。需要一种针对 Agent 场景的、保持动作语法完整性的压缩方法。

Method: AGORA 包含三个组件：(1) 结构提示解析器，将提示按 step 粒度分割；(2) always-keep floor，保留格式关键和最近关键内容；(3) 125M 参数的相关性评分器，基于反事实 next-action-change 标签训练，在 ~2ms/step 内评估每个 step 的相关性并决定保留与否。

Result: 在 9 个测试单元中，AGORA 是唯一在 8 个单元中保持 ≥75% 原始性能的方法（唯一例外为 73%）。四路消融实验表明结构 floor 是质量主导因素，学习型评分器在固定保留率下实现 1.0-11.5x 的自适应端到端压缩。

Conclusion: AGORA 证明了 step 粒度压缩是 LLM Agent 提示压缩的正确范式，结构保留机制比学习型评分器对质量影响更大，且推理时零 LLM 调用的设计使其具有实际部署价值。

Classification reason: 该论文研究 LLM Agent 的提示压缩方法，属于 AI 智能体工作流优化，与 canonical 子主题 aigc_agent_workflow（AI 智能体与生成工作流）高度匹配。

Importance reason: Focuses on LLM agent prompt compression, not on the user's priority subtopics like video generation or world models. Signals: Novel failure mode analysis, Inference-free step-level compression, Strong empirical results on agent tasks, Low relevance to user's core interests
...
### [74] [MUSE-Autoskill: Self-Evolving Agents via Skill Creation, Memory, Management, and Evaluation](https://arxiv.org/abs/2605.27366)
...
TL;DR: 提出 MUSE-Autoskill Agent 框架，将 LLM 智能体的技能视为可创建、存储、复用、评估和持续改进的生命周期资产，通过技能级记忆和单元测试实现跨任务的技能演化与迁移。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 智能体的技能创建方法将技能视为孤立、静态的工件，缺乏可复用性、可靠性和长期改进能力，限制了智能体在复杂任务中的持续提升。

Method: 提出 MUSE-Autoskill 框架，包含统一的技能生命周期（创建、记忆、管理、评估、改进）。技能按需创建，通过技能级记忆跨任务积累经验，利用单元测试和运行时反馈进行评估与持续优化，并支持高效的组织与选择机制。

Result: 在 SkillsBench 上的实验表明，生命周期管理的技能能够提升任务成功率、效率、复用率，并支持跨智能体迁移。

Conclusion: 将技能视为长期存在、具有经验积累且可测试的资产，对于提升 LLM 智能体的任务解决能力至关重要，为智能体持续学习与技能演化提供了有效范式。

Classification reason: 该论文提出 LLM 智能体框架 MUSE-Autoskill，聚焦技能创建、记忆、管理与评估的生命周期管理，属于 AI 智能体与生成工作流方向，与 [CANONICAL] subtopic aigc_agent_workflow 高度匹配。

Importance reason: Focuses on agent skill lifecycle management, not directly on the user's priority subtopics like video generation or world models. Signals: LLM agent skill lifecycle, skill-level memory, unit test evaluation, cross-task skill reuse, not aligned with priority subtopics
...
### [75] [Helicase: Uncertainty-Guided Supply Chain Knowledge Graph Construction with Autonomous Multi-Agent LLMs](https://arxiv.org/abs/2605.26835)
...
TL;DR: 本文提出 Helicase，一个基于多智能体 LLM 的自主系统，用于不确定性引导的供应链知识图谱构建。系统将高层查询分解为可执行的调查计划，协调搜索、推理和编码智能体，通过迭代验证循环逐步构建查询特定的供应链知识图谱，并为每个事实提供不确定性标注。同时提出 SCQA 基准，包含 80 个供应链查询，覆盖从单跳到多跳推理的复杂度谱系。


<details>
  <summary>Details</summary>
Motivation: 供应链中的关键信息任务（如"特斯拉哪些组件使用来自澳大利亚矿山的锂？"）无法通过单次查询或单一文档回答，而是需要在碎片化、异构的网页资源上进行多跳推理和结构化推断。现有 LLM 多智能体系统缺乏对这种复杂推理任务的支持，且缺乏对推理结果可靠性的不确定性感知能力。

Method: Helicase 系统包含三个核心组件：(1) 多智能体协作框架，将高层查询分解为可执行的调查计划，协调搜索、推理和编码智能体；(2) 迭代验证循环，通过智能体间的交叉验证确保推理质量；(3) 三层不确定性框架，在动作层、轨迹层和记忆层分别追踪不确定性，实现结构化推理与校准置信度评估。

Result: 在提出的 SCQA 基准（80 个供应链查询，分为四个象限）上，Helicase 展示了从单跳到多跳推理、从高数据可见性到低数据可见性场景下的自主推理能力，能够构建带有不确定性标注的供应链知识图谱。

Conclusion: Helicase 填补了 LLM 多智能体系统在复杂供应链结构化推理任务中的能力空白，通过不确定性引导的知识图谱构建实现了可追溯、可信赖的推理结果，为供应链领域的自主信息发现提供了新范式。

Classification reason: 该论文提出基于多智能体 LLM 的供应链知识图谱自动构建系统，属于 LLM 智能体与自动化工作流范畴，与 canonical 子主题 aigc_agent_workflow（AI 智能体与生成工作流）高度匹配。

Importance reason: Focuses on supply chain knowledge graph construction, which is not a priority subtopic and is flagged by negative keywords. Signals: supply chain, knowledge graph, multi-agent, uncertainty-guided, not in priority subtopics
...
### [76] [Counteraction-Aware Multi-Teacher On-Policy Distillation for General Capability Recovery with Domain Preservation](https://arxiv.org/abs/2605.27115)
...
TL;DR: 本文提出 CaMOPD，一种对抗感知的多教师在线蒸馏方法，解决领域特化后通用能力恢复中教师提示覆盖不完整导致的梯度冲突与弱信号问题，通过解耦交替训练和基于差距的样本选择，在角色扮演对话和医疗推理问答场景中实现了通用能力恢复与领域能力保持的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 领域特化微调会削弱LLM的通用能力，现有MOPD方法假设教师提示覆盖完整，但通用教师（开源模型）的后训练数据分布未知，难以重建。本文研究在仅有代理通用提示的不完整覆盖场景下如何有效恢复通用能力并保持领域能力。

Method: 提出 CaMOPD，包含两个核心机制：(1) 解耦交替训练——将通用恢复和领域保持的更新分离到不同训练阶段，避免梯度冲突；(2) 基于差距的样本选择——选择教师-学生平均token级对数概率差距较大的样本，集中校正信号，避免弱信号被均匀平均稀释。

Result: 在角色扮演对话和医疗推理问答两个场景中，CaMOPD在通用能力恢复上优于基线方法，同时保持了领域特定行为。梯度一致性分析进一步验证了CaMOPD能产生更一致的校正信号。

Conclusion: CaMOPD有效解决了不完整提示覆盖下MOPD的恢复-保持对抗和弱信号平坦化问题，为领域特化后通用能力恢复提供了一种实用的蒸馏方案。

Classification reason: 该论文研究多教师在线蒸馏（MOPD）方法，用于领域特化后通用能力的恢复与领域能力的保持，属于LLM训练与优化工作流，与 aigc_agent_workflow（AI智能体与生成工作流）中涉及的模型训练、蒸馏、自动化流水线主题相关。

Importance reason: Focuses on LLM distillation for domain specialization, not directly on the user's priority subtopics like video generation or world models. Signals: Multi-teacher distillation, domain specialization, general capability recovery, gradient conflict, sample selection
...
### [77] [StepOPSD: Step-Aware Online Preference Distillation for Agent Reinforcement Learning](https://arxiv.org/abs/2605.27140)
...
TL;DR: StepOPSD 提出了一种面向多轮智能体的步骤级在线偏好自蒸馏框架，将轨迹分解为以动作为中心的步骤片段，利用事后增强的教师上下文重新评分，并通过符号保持的优势塑造和归一化步骤信用预算进行 GRPO 更新，在 ALFWorld 和 Search-QA 上取得领先结果。


<details>
  <summary>Details</summary>
Motivation: 多轮智能体的强化学习面临信用分配不匹配问题：奖励稀疏且基于整条轨迹，而成功往往取决于少数局部决策。现有在线策略蒸馏（OPD）提供更密集的 token 级监督，但将异构智能体轨迹视为整体字符串而非因果交互单元，无法有效定位关键决策步骤。

Method: StepOPSD 将轨迹分解为以动作为中心的步骤片段，在事后增强的教师上下文下重新评分，将 token 级对数概率差距转化为符号保持的优势塑造信号，并在 GRPO 更新前引入归一化的每步信用预算，实现步骤级别的信用分配。

Result: 在 ALFWorld 和 Search-QA 上使用 Qwen3-1.7B 和 Qwen2.5-3B-Instruct 进行评估，StepOPSD 在局部因果错误最敏感的子集上取得最佳或次佳结果，包括 ALFWorld Heat (79.1%)、PickTwo (95.0%)、Search-QA TriviaQA (61.6%) 以及 HotpotQA (40.4%) 并列最佳。

Conclusion: 步骤感知蒸馏在轨迹级奖励与决定下游成功的局部动作弱对齐时最为有效。实验揭示了双旋钮规律：较小的 α_clip 作为广泛稳定的局部信任区域，而最优全局混合强度 λ_mix 依赖于具体任务。

Classification reason: 该论文提出面向多轮交互智能体的在线偏好蒸馏框架 StepOPSD，核心是改进智能体强化学习中的信用分配问题，属于 LLM 智能体训练与优化范畴，与 [CANONICAL] subtopic "AI 智能体与生成工作流" 高度匹配。

Importance reason: Focuses on agent RL credit assignment, not directly on the user's priority subtopics like video generation or world models. Signals: agent reinforcement learning, step-level credit assignment, online preference distillation, GRPO update, ALFWorld benchmark
...
### [78] [Beyond Trajectory-Level Attribution: Graph-Based Credit Assignment for Agentic Reinforcement Learning](https://arxiv.org/abs/2605.26684)
...
TL;DR: 本文提出GraphGPO，一种基于图的组策略优化方法，通过将多条轨迹聚合成统一的状态转移图，利用图结构信息估计每个状态到任务目标的距离，从而实现细粒度的逐步骤信用分配，显著提升智能体强化学习的训练效率与性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于组的强化学习方法（如GRPO）在智能体任务中依赖粗粒度的轨迹级信用分配，仅根据最终结果归因，难以捕捉失败轨迹中有价值步骤的贡献。为了挖掘这些潜在信息并实现更忠实的步骤级信用分配，作者提出了GraphGPO。

Method: GraphGPO首先将所有采样轨迹聚合成一个统一的状态转移图，然后利用图中编码的全局信息估计每个状态到任务目标的距离，最后基于状态转移对目标距离的缩减程度为每条边分配图基优势值（graph-based advantage），从而实现步骤级的信用分配。

Result: 在多个具有挑战性的基准测试上，GraphGPO显著提升了训练效率，并取得了最先进的性能表现。

Conclusion: GraphGPO通过图结构挖掘轨迹间的潜在信息，实现了比传统轨迹级归因更精细、更准确的步骤级信用分配，为智能体强化学习提供了一种高效的新范式。

Classification reason: 该论文提出基于图的信用分配方法（GraphGPO）用于智能体强化学习（Agentic RL），属于LLM智能体训练与优化范畴，与AIGC方向下的"AI 智能体与生成工作流"（aigc_agent_workflow）高度匹配。

Importance reason: Novel graph-based credit assignment for agentic RL, but outside user's core generative model priorities. Signals: agentic reinforcement learning, credit assignment, graph-based policy optimization, LLM agent training, step-level attribution
...
### [79] [Learning to Act under Noise: Enhancing Agent Robustness via Noisy Environments](https://arxiv.org/abs/2605.27209)
...
TL;DR: 本文提出 NoisyAgent，一个通过在训练中引入用户噪声和工具噪声来增强 LLM 智能体在真实随机环境中鲁棒性的训练框架，实验表明该方法能提升智能体在噪声环境下的表现，同时不损害理想环境下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 智能体在理想化基准上表现优异，但在真实部署中由于环境随机性和不完美性（如用户交互模糊、工具执行失败）性能显著下降。作者认为根本原因在于训练环境与真实交互动态之间存在失配。

Method: 提出 NoisyAgent 框架，识别两类交互噪声：用户噪声（用户交互的模糊性和变异性）和工具噪声（工具执行的失败和异常）。在训练过程中，通过修改用户交互模式和模拟工具执行结果引入扰动，并采用渐进式难度递增策略（仅对部分 rollout 施加噪声，随模型适应逐步增加难度）来稳定训练。

Result: 大量实验表明 NoisyAgent 能持续提升智能体在噪声和动态环境下的鲁棒性。分析还发现，噪声条件下的训练也能在理想化基准上带来性能提升，表明受控的噪声暴露有助于促进更通用的推理和决策行为。

Conclusion: 建模交互不完美性对于弥合智能体训练与真实部署之间的差距至关重要，受控的噪声暴露能提升智能体的泛化能力。

Classification reason: 该论文研究 LLM 智能体在真实环境中的鲁棒性问题，提出 NoisyAgent 训练框架以增强智能体在噪声环境下的表现，属于 AI 智能体与生成工作流（aigc_agent_workflow）这一规范主题。

Importance reason: Focuses on agent robustness via noise injection, which is not a top priority subtopic and lacks direct connection to user's core interests in generation and efficiency. Signals: agent robustness, noise injection training, LLM agents, real-world deployment, not generation-focused
...
Importance: 普通相关 (42.11) - 未入选精读

TL;DR: 本文提出Generative Animations系统，通过串联大语言模型（LLM）进行语义解析与Segment Anything Model（SAM）进行视觉定位，将自然语言提示自动转换为生产级动画运动路径，支持轮廓跟随、轨道动画和透视对齐三种用例。


<details>
  <summary>Details</summary>
Motivation: 传统动画运动路径创建需要设计师手动选择预设、绘制贝塞尔曲线和配置时序属性，过程繁琐且耗时。本文旨在通过自然语言提示自动生成动画，降低动画创作门槛。

Method: 构建多模型流水线：LLM负责语义解析用户提示，SAM负责视觉定位与场景理解，两者串联自动生成尊重场景几何、处理深度遮挡和3D透视变换的运动路径。

Result: 在轮廓跟随轨迹、z-order感知的轨道动画、以及变换物体上的透视对齐运动三种用例上展示了系统的有效性。

Conclusion: Generative Animations系统证明了通过LLM与视觉基础模型串联可以自动化动画创作流程，将自然语言直接转化为生产级动画，显著降低动画设计门槛。

Classification reason: 该论文提出一个将LLM与SAM串联的多模型流水线，将自然语言提示自动转换为动画运动路径，属于AI智能体驱动的自动化创作工作流，因此映射到AIGC方向下的[AI 智能体与生成工作流]（aigc_agent_workflow）规范子主题。

Importance reason: Pipeline combines LLM and SAM for animation but lacks direct relevance to user's priority subtopics like autoregressive/diffusion video generation or world models. Signals: Multi-model pipeline, prompt-driven animation, no autoregressive or diffusion focus, no video generation, no world model
...
### [81] [Workflow Closure Is Not Scientific Closure in Auto-Research Systems](https://arxiv.org/abs/2605.26200)
...
TL;DR: 本文论证自动科研系统的工作流闭环并不等同于科学闭环。通过对100多篇论文和21个代表性系统的审计，识别出目标坍缩、验证坍缩和接受坍缩三种结构性失败模式，并提出在非自主认知控制下实现自主执行的改进方向。


<details>
  <summary>Details</summary>
Motivation: 当前自动科研系统能够内部完成从想法生成到实验执行、写作和自我评估的完整循环，但这种工作流闭环是否等同于科学上的有效闭环尚存疑问。作者旨在揭示这种混淆带来的系统性风险，并推动社区讨论如何构建真正可信的自动科研系统。

Method: 对100多篇近期论文和代码仓库进行系统调研，并对21个代表性自动科研系统进行结构化审计，归纳出三种结构性失败模式（目标坍缩、验证坍缩、接受坍缩），并据此提出改进方向。

Result: 识别出三种系统性失败模式：目标坍缩（单一代理目标替代多目标科学目标）、验证坍缩（内部自我评估替代独立验证）、接受坍缩（基准分数或类论文产物替代领域级批判、复用与整合）。

Conclusion: 工作流闭环不等于科学闭环。可信的自动科研不应追求完全自主自足，而应在非自主认知控制下实现自主执行。作者从目标信号、验证和输出路径三方面提出了潜在改进方案，以引发社区讨论。

Classification reason: 该论文讨论自动科研系统（auto-research systems）中的工作流闭环与科学闭环的区别，属于 AI 智能体驱动的自动化研究工作流，与 [CANONICAL] subtopic "AI 智能体与生成工作流"（aigc_agent_workflow）高度匹配。

Importance reason: Critiques auto-research systems but is not directly relevant to your priority subtopics in generative models. Signals: Workflow closure critique, Scientific rigor analysis, Auto-research system audit, Structural failure patterns
...
### [82] [ATOM: Instantiating Budget-Controllable Multi-Agent Collaboration via Nucleus-Electron Hierarchy](https://arxiv.org/abs/2605.26178)
...
TL;DR: ATOM 提出了一种受原子结构启发的自适应多智能体协作框架，通过"核-电子"层级结构（稳定的离线学习协作骨干 + 动态激活的查询条件智能体）和复杂度感知的预算策略，在保证性能的同时将 token 效率提升高达 30%。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的多智能体系统在优化协作拓扑时面临稳定性与可扩展性之间的固有矛盾，且计算预算往往与查询难度不匹配，导致资源浪费或性能不足。

Method: 受原子结构启发，ATOM 采用"核-电子"层级结构：维护一个稳定的离线学习协作骨干（核），同时在推理时动态激活查询条件智能体（电子）。通过复杂度感知的预算策略估计查询难度，严格调控电子实例化，实现预算可控的协作图生成。

Result: 在六个多样化基准上的实验表明，ATOM 达到了最先进的性能，同时相比强基线方法将 token 效率提升了高达 30%。

Conclusion: ATOM 通过核-电子层级结构和复杂度感知预算策略，有效解决了多智能体协作中稳定性与可扩展性的权衡问题，实现了性能与通信成本的良好平衡。

Classification reason: 该论文研究基于LLM的多智能体协作系统，提出预算可控的自适应协作图生成框架，属于AI智能体与生成工作流方向，与[CANONICAL] subtopic aigc_agent_workflow高度匹配。

Importance reason: Focuses on multi-agent collaboration efficiency, not directly on user's priority subtopics like video generation or world models. Signals: multi-agent systems, token efficiency, budget control, reinforcement learning, not aligned with priority subtopics
...
### [83] [AI evaluation may bias perceptions: The importance of context in interpreting academic writing](https://arxiv.org/abs/2605.26662)
*Shang Wu, Randol Yao*

Main category: cs.CL, econ.GN, cs.AI
...
TL;DR: 本文研究了AI在科学写作中使用情况的评估偏差问题。通过大规模期刊数据构建AI相似度基准，发现忽略国家和领域差异的全局基准会混淆既有文体差异与AI生成文本，导致评估偏差。提出按国家-领域分组的上下文感知基准可缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI使用评估方法通常采用全局统一的基准来检测科学写作中的AI生成内容，但不同国家和领域的学术写作风格本身存在差异，这种差异可能被误判为AI使用，导致评估结果存在系统性偏差。

Method: 利用Dimensions数据库的大规模期刊出版物数据，基于人类撰写摘要与LLM改写摘要之间的差异构建AI相似度基准。对比全局统一基准与国家-领域分组基准，分析两者在LLM时代前后（尤其是2025年）对AI使用估计的差异。

Result: 全局统一基准会混淆既有文体差异与AI生成文本，在LLM时代前的出版物中就已产生显著的国家-领域分组偏差。2025年数据表明，全局基准系统性地高估某些国家/领域的AI使用，同时低估其他组别的AI使用。

Conclusion: AI使用评估需要上下文感知的测量方法，考虑国家和领域的文体差异，才能实现准确且公平的评估。这为AIGC检测领域提供了重要的方法论启示。

Classification reason: 该论文研究AI生成文本（LLM改写摘要）的检测方法，并指出评估方法中的偏差问题，属于AIGC检测与溯源范畴，因此映射到规范子主题aigc_detection_attribution。

Importance reason: Focuses on evaluation bias in AI writing detection, not on core generative model advances. Signals: AI evaluation bias, scientific writing detection, context-aware benchmarks, not generative model innovation
...
### [84] [HydraPrompt: An Adaptive and Asymmetric Framework of Vision-Language Models for Synthetic Image Detection](https://arxiv.org/abs/2605.26421)
...
TL;DR: HydraPrompt 提出一种非对称提示框架，利用 CLIP 视觉语言模型进行合成图像检测。通过为真实类别使用固定提示、为伪造类别使用样本自适应提示，并结合条件监督对比学习，实现对多种伪造类型的动态适应，在多个 SID 基准上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于 CLIP 的合成图像检测方法使用静态提示作为真假图像的固定边界，无法适应推理时出现的多种伪造类型变化，检测泛化能力受限。

Method: 提出非对称提示适配器（APA）：真实类别使用单组固定提示捕获一致模式；伪造类别使用样本自适应提示捕获多样伪造线索。引入条件监督对比学习（CSC）目标，压缩真实表征同时保留细粒度伪造线索。

Result: 在多个主流 SID 基准上取得 SOTA 性能，显著优于现有基于静态提示的检测方法。

Conclusion: 非对称提示框架能有效适应多样化的伪造类型，为合成图像检测提供了一种动态、自适应的新范式。

Classification reason: 该论文研究合成图像检测（Synthetic Image Detection），属于 AIGC 检测与溯源方向，与 canonical 子主题 aigc_detection_attribution 完全匹配。

Importance reason: Focuses on synthetic image detection, which is a downstream application of AIGC, not directly on generation or model efficiency. Signals: synthetic image detection, CLIP-based, adaptive prompting, contrastive learning, SOTA on benchmarks
...
### [85] [Receipt Replay OOD: A Small Benchmark for Screen Replay Detection Under Domain Shift](https://arxiv.org/abs/2605.26855)
...
TL;DR: 本文提出了 Receipt Replay OOD，一个用于屏幕重放检测的小型跨域基准数据集。该数据集使用收据替代身份证件，避免了隐私限制，同时保留了平面几何、磨损痕迹等相似特征，用于评估文档重放检测模型在域偏移下的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有身份证件呈现攻击检测数据集（如 DLC-2021、SynID、KID34K）在域内评估表现良好，但跨域（out-of-domain）鲁棒性研究不足。同时，身份证件数据涉及个人身份信息，公开受限。作者希望构建一个无隐私限制的跨域基准，系统评估域偏移对屏幕重放检测模型泛化能力的影响。

Method: 作者收集了收据（receipt）图像作为身份证件的替代品，构建了 Receipt Replay OOD 数据集。收据与身份证件共享平面几何、圆角、磨损痕迹、文字/Logo 图案等特征，但不含个人身份信息。在该数据集上，作者设计了跨域评估协议，测试现有文档重放检测模型在域偏移条件下的表现。

Result: 实验表明，域偏移对文档重放检测模型的泛化性能有显著影响，现有模型在跨域场景下性能下降明显。该基准数据集已公开，可用于推动该方向的鲁棒性研究。

Conclusion: Receipt Replay OOD 为屏幕重放检测提供了一个有价值的跨域基准，填补了现有研究在域偏移评估方面的空白。收据作为身份证件的替代品，有效规避了隐私限制，为未来研究提供了可公开使用的测试资源。

Classification reason: 论文研究屏幕重放攻击检测（screen replay detection），属于文档呈现攻击检测（presentation attack detection），与 AIGC 检测与溯源方向下的深伪检测、合成检测高度相关。该 canonical 子主题涵盖 AI 生成/伪造内容检测，屏幕重放检测是其中的一种具体攻击检测场景。

Importance reason: Focuses on a niche AIGC detection subtask with limited relevance to the user's priority generative modeling and efficiency topics. Signals: small benchmark, domain shift, screen replay detection, limited scope, no generative model focus
...
### [86] [The Attribution Blind Spot: Detecting When Language Models Rely on Memory Rather Than Retrieved Context](https://arxiv.org/abs/2605.26778)
...
TL;DR: 本文发现RAG系统中存在"归因盲点"：当检索文档与预训练数据重叠时，模型可能完全依赖参数记忆生成看似忠实的内容，而输出层面无法区分。作者提出计算现实监控（CRM）方法，通过比较有无上下文时的内部表征差异来检测模型是否依赖记忆，实验证明内部表征携带输出层不可见的诊断信号。


<details>
  <summary>Details</summary>
Motivation: RAG系统假设检索上下文能约束模型生成，但现有方法无法验证生成是否真正受检索上下文控制。当检索文档与预训练数据重叠时，模型可能仅凭参数记忆生成内容，而输出层面无法区分，这在高风险场景下存在严重安全隐患。

Method: 受认知科学中现实监控框架启发，提出计算现实监控（CRM）方法。通过比较模型在有上下文和无上下文条件下的内部表征，计算表征差异（membership-conditioned representational divergence），检测预训练暴露是否留下可测量的内部轨迹信号。结合块级噪声干预进行验证。

Result: 在三个模型家族（9个变体）上，CRM发现表征差异集中在架构特定的层模式中；块级噪声干预提供了收敛支持；该方法在不同任务和数据集上泛化良好，但在领域混淆的基准上失效。

Conclusion: 归因盲点是可测量且部分可解决的：内部表征携带输出层不可见的诊断信号，这为构建能够感知证据来源的系统奠定了基础。

Classification reason: 该论文研究RAG系统中语言模型是否依赖记忆而非检索上下文的问题，核心是检测生成内容的来源（参数记忆 vs 检索上下文），属于AIGC检测与溯源范畴，因此映射到规范子主题aigc_detection_attribution。

Importance reason: Focuses on RAG attribution detection, not aligned with user's priority subtopics in generation or efficiency. Signals: RAG attribution blind spot, internal representation analysis, not generation-focused, no video/image/diffusion/autoregressive signals
...
TL;DR: 本文提出一种黑盒成员推断攻击方法SD-MIA，通过分析扩散模型对目标图像及扰动文本指令的去噪过程，检测预训练数据中的成员样本，在无需访问模型内部特征的情况下实现了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成领域快速发展，引发了关于版权和隐私的担忧。成员推断攻击（MIA）可用于检测模型是否使用了未经授权的数据。然而现有方法依赖于模型对数据的记忆程度，对预训练数据（曝光较少）效果差；且需要访问模型内部特征，不适用于闭源平台。

Method: 提出SD-MIA框架，核心是跨模态数据扰动机制：同时扰动目标图像和对应的文本指令，通过分析黑盒扩散模型对扰动后样本的去噪能力差异来区分成员与非成员样本。该方法仅需模型API访问，无需内部特征。

Result: 在公开基准数据集和新构建的预训练数据集上，SD-MIA在检测预训练数据成员方面取得了优于现有基线的性能，包括那些可以访问模型内部特征的不公平基线方法。

Conclusion: 黑盒扩散模型对目标图像和扰动文本指令的去噪行为可以揭示更明显的成员线索，SD-MIA有效实现了对预训练数据的成员推断攻击，为AIGC版权和隐私保护提供了新的检测手段。

Classification reason: 该论文研究针对扩散模型的成员推断攻击（MIA），用于检测预训练数据是否被模型使用，属于AIGC检测与溯源范畴，与[CANONICAL] subtopic "AIGC 检测与溯源"高度匹配。

Importance reason: Focuses on AIGC detection and copyright, which is a secondary subtopic not aligned with user's priority on generation and efficiency. Signals: Membership inference attack, Black-box method, Pre-training data detection, Diffusion model privacy, Cross-modal perturbation
...
### [88] [Eroding Trust in Real Speech: A Large-Scale Study of Human Audio Deepfake Perception](https://arxiv.org/abs/2605.26136)
...
TL;DR: 本文通过大规模人类感知实验（1768名参与者，35532个判断），研究了音频深度伪造对人类信任真实语音的影响。核心发现是"怀疑偏移"：相比2021年基线，人类对伪造样本的检测准确率变化不大（72.9%→71.2%），但对真实样本的准确率从72.7%降至64.1%。人们并非更善于检测伪造，而是越来越不信任真实语音。商业和自回归语言模型生成的样本最难检测（61.3-65.9%），而传统seq2seq和流匹配模型生成的样本较易识别（75.4-76.8%）。ML检测器在所有条件下保持94.5%以上的准确率。


<details>
  <summary>Details</summary>
Motivation: 音频深度伪造技术快速进步，但其对人类信任真实语音的影响尚未被研究。现有工作主要关注自动检测方法，缺乏对人类感知的大规模系统研究。

Method: 设计大规模人类感知实验，收集1768名参与者的35532个判断，覆盖138个TTS和语音转换系统。参与者需判断音频样本是真实还是伪造，并与2021年基线数据对比分析。

Result: 人类对伪造样本的检测准确率从72.9%略降至71.2%，但对真实样本的准确率从72.7%大幅降至64.1%。商业和自回归LM系统生成的样本最难检测（61.3-65.9%），传统seq2seq和流匹配模型较易识别（75.4-76.8%）。ML检测器保持94.5%以上准确率。

Conclusion: 现代音频深度伪造的主要威胁可能不是单纯的欺骗，而是对真实语音信任的侵蚀。这种"怀疑偏移"效应表明，即使人们无法被直接欺骗，深度伪造的广泛存在也会系统性削弱人们对真实语音的信任。

Classification reason: 该论文研究人类对音频深度伪造的感知与检测能力，属于AIGC检测与溯源（aigc_detection_attribution）范畴，该[CANONICAL]子主题明确包含深伪检测。

Importance reason: Focuses on human perception of audio deepfakes, not on generation or efficiency, and lacks direct connection to user's priority subtopics. Signals: Audio deepfake detection, human perception study, skepticism shift, not aligned with video/image generation or model efficiency
...
### [89] [When Eyes Betray AI: Social Gaze Consistency as a Semantic Cue for AI-Generated Image Detection](https://arxiv.org/abs/2605.27348)
...
TL;DR: 本文提出社交视线一致性（Social Gaze Consistency）作为 AI 生成图像检测的高层语义线索，通过构建诊断数据集和块组合描述监督（Block-Compositional Caption Supervision），在多个骨干网络上验证了该线索的有效性，在 COCOAI Interaction 子集上平衡准确率提升 3.7 个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 生成图像检测方法主要依赖低层伪影（像素指纹、频率异常等），但在人物中心图像和局部编辑场景中，这些低层线索已不可靠。作者观察到生成模型在多人交互场景中难以保持视线方向、头眼对齐和瞳孔位置的一致性，因此提出利用社交视线一致性作为新的检测线索。

Method: (i) 构建受控诊断数据集，通过区域特异性扰动破坏视线一致性，并采用严格配对分组防止生成器指纹记忆；(ii) 提出块组合描述监督（Block-Compositional Caption Supervision），将 5 块推理骨架保持不变，覆盖 1250 种宏观组合描述，解耦推理一致性与表面多样性；(iii) 在视觉语言骨干（FakeVLM）和纯视觉骨干（Effort）上进行跨架构验证。

Result: 在 COCOAI Interaction 子集上，FakeVLM 平衡准确率从 67.8% 提升至 71.5%（+3.7 pp），Person 子集从 83.0% 提升至 84.3%（+1.3 pp）；Effort 骨干也获得一致提升。真实类和伪造类召回率同步上升，排除了"全判为伪造"的伪影。仅用 FLUX.1-Fill 训练即可泛化到多生成器套件。

Conclusion: 社交视线一致性是一种有效的、与现有低层检测范式正交的高层语义线索，能够提升 AI 生成图像检测的鲁棒性和泛化能力，尤其适用于人物交互场景。该方法具有骨干无关性，且训练数据需求低，具有良好的实用前景。

Classification reason: 该论文研究 AI 生成图像检测，提出基于社交视线一致性（Social Gaze Consistency）的高层语义线索来区分真实与 AI 生成图像，属于 AIGC 检测与溯源方向，与 canonical 子主题 aigc_detection_attribution 完全匹配。

Importance reason: Novel semantic cue for AIGC detection but outside user's priority subtopics and negative keywords apply. Signals: Social gaze consistency, AI-generated image detection, high-level semantic cue, cross-architecture validation, FLUX.1-Fill transfer
...
### [90] [Do Modern Post-Hoc Watermarking Methods Beat Broken-Arrows?](https://arxiv.org/abs/2605.27135)
...
TL;DR: 本文系统比较了现代后验水印方法与经典水印方法（Broken-Arrows）在AI生成图像检测中的鲁棒性和安全性。实验表明，在现实场景中，经典水印方法在安全性上优于现代方法，同时保持了相当的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着扩散模型等生成模型的快速发展，数字水印成为识别AI生成图像的关键技术。现代后验水印方法追求极低的误报率，但缺乏与经典方法的公平比较，特别是在鲁棒性和安全性优先的现实场景中。

Method: 提出一个公平的比较框架，在现代后验水印方法与经典Broken-Arrows水印方法之间，针对多种经典图像增强和最新的复杂攻击，系统评估鲁棒性和安全性。

Result: 在现实场景下，经典水印方法在安全性方面优于现代后验水印方法，同时保持了相当的鲁棒性。

Conclusion: 经典水印方法在现实场景中仍然具有竞争力，现代后验水印方法在追求极低误报率的同时可能牺牲了安全性，未来水印研究应更关注实际部署中的鲁棒性与安全权衡。

Classification reason: 该论文研究AI生成图像的数字水印方法，比较现代后验水印方案与经典方案在鲁棒性和安全性上的表现，属于AIGC检测与溯源（水印是溯源的关键技术），因此映射到规范子主题 aigc_detection_attribution。

Importance reason: Focuses on watermarking robustness comparison, not on core generative model advancement or efficiency. Signals: Watermarking comparison, Classic vs modern methods, AI-generated image detection, Security focus, Not generative model innovation
...
### [91] [LURE: Live-Usage Replay Evaluations for Reducing Evaluation Awareness](https://arxiv.org/abs/2605.26438)
...
TL;DR: 本文提出 LURE（Live-Usage Replay Evaluations），一种通过回放真实部署交互轨迹并附加评测提示来构建更真实评测的方法，旨在降低 LLM 的"评测意识"（evaluation awareness），提升对齐基准的有效性。实验表明 LURE 评测与真实部署的区分度显著低于现有基准和合成评测生成器。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在评测时可能识别出自己正在被评估（evaluation awareness），从而改变行为，导致安全和对齐基准的效度受损。现有评测方法缺乏真实感，亟需一种更接近真实部署场景的评测构建方法。

Method: LURE 方法通过回放真实的智能体交互轨迹，并在末尾附加评测提示来构建类似部署场景的评测。同时提出一个自动化流水线来评估评测真实感，结合对"评测意识"的言语化检测和 judge-model 对日志是否为评测的概率估计，并在大规模部署与评测转录数据集上验证。

Result: LURE 评测与真实部署的区分度显著低于广泛使用的基准和合成评测生成器，其真实感可接近真实用户对话。在 scheming、AI safety sabotage 和 sycophancy 三个场景中验证了有效性。

Conclusion: 评测真实感是对齐基准的关键属性，应作为基准结果的一部分进行报告，尤其是在安全案例中使用这些结果时。LURE 提供了一种构建更真实评测的有效途径。

Classification reason: 该论文提出 LURE 方法用于构建更真实的 LLM 对齐评测，核心关注评测真实性与基准有效性，属于 AIGC 评测与基准方向。现有 [CANONICAL] subtopic "AIGC 评测与基准" (aigc_evaluation_benchmark) 完全匹配。

Importance reason: Novel evaluation realism method for LLM alignment, but outside user's core generative model priorities. Signals: evaluation awareness, alignment benchmark, LLM safety, agentic interaction, replay evaluation
...
### [92] [FAB-Bench: A Framework for Adaptive RAG Benchmarking in Semiconductor Manufacturing](https://arxiv.org/abs/2605.26476)
...
TL;DR: FAB-Bench 是一个面向半导体制造领域的 RAG 系统自适应评测框架，定义了六项诊断指标，涵盖 4K-32K 上下文窗口的检索器与生成器联合分析，并构建了包含 200 个高质量问答对的高质量基准，揭示了三种上下文缩放行为及注意力稀释机制。


<details>
  <summary>Details</summary>
Motivation: RAG 系统在垂直领域（如半导体制造）的评测面临领域复杂性高、上下文尺度多样、依赖昂贵且不可扩展的专家评估等问题，缺乏系统化、可复用的评测框架。

Method: 提出 FAB-Bench 框架，定义六项诊断指标（事实准确性、上下文利用、完整性、检索相关性、技术深度、推理一致性），耦合检索器诊断与生成器推理分析，覆盖 4K-32K token 上下文窗口。从 1300+ 候选问答对中筛选出 200 个高质量样本，涵盖三种合成策略（大海捞针、文档内多主题、跨文档多跳）。

Result: 在 4 种 LLM 和 4 种 RAG 框架上的系统评测揭示了三种上下文缩放行为（对数增长、早期饱和、冷启动动态），并发现注意力稀释是极端上下文长度下性能下降的主要机制。跨框架验证确认了评测的可移植性。

Conclusion: FAB-Bench 为半导体制造领域的 RAG 系统提供了首个系统化、可复用的评测框架，其发现对理解长上下文 RAG 系统的行为机制具有指导意义。

Classification reason: 该论文提出 FAB-Bench，一个面向半导体制造领域的 RAG 系统自适应评测框架，属于 AIGC 方向下的生成内容评测与基准（aigc_evaluation_benchmark），该 canonical subtopic 涵盖 AIGC benchmark 与质量评估，与本论文高度匹配。

Importance reason: Domain-specific RAG benchmark for semiconductor manufacturing has low relevance to user's priority subtopics in generative AI and video/image generation. Signals: Semiconductor domain, RAG evaluation, context scaling, attention dilution, low priority match
...
### [93] [Elias in the Lighthouse, Again? Diagnosing Low Diversity in LLM Stories](https://arxiv.org/abs/2605.26492)
...
TL;DR: 本文系统评测了四个主流LLM生成故事的多样性，发现88.3%的故事共享11个高频词（如"Elias"、"灯塔"、"钟表匠"等），这些词在预训练数据中罕见但出现在偏好数据中，揭示了小规模偏好数据结合强对齐算法对生成多样性的不成比例影响。


<details>
  <summary>Details</summary>
Motivation: LLM生成故事虽然流行，但存在严重的低多样性问题，用户反复看到相似的叙事元素（如灯塔、钟表匠等）。现有研究缺乏对这一现象的量化诊断和归因分析。

Method: 从四个当前主流模型中采样20,000个故事，使用五个不同提示，通过词频统计和对比分析，检测高频重复词汇，并与公开文献、预训练数据和偏好数据进行交叉比对。

Result: 88.3%的故事包含11个高频词（人名Elias、Mara、Elara，场景灯塔，职业钟表匠、图书管理员等）。这些词在公开文献和预训练数据中罕见，但在偏好数据中常见。此外，"灯塔故事"在整体后训练故事中占比不高，多数后训练故事涉及版权角色或成人内容。

Conclusion: 小规模偏好数据集与强对齐算法的结合可能对LLM生成多样性产生不成比例的负面影响，导致模型过度重复某些特定叙事元素。这提示需要更谨慎地设计偏好数据以保持生成多样性。

Classification reason: 该论文系统性地评测了LLM生成故事的多样性（低变异性问题），属于AIGC生成内容质量评测范畴，与[CANONICAL] subtopic "AIGC 评测与基准"（aigc_evaluation_benchmark）高度匹配。

Importance reason: Focuses on LLM story diversity, not directly on user's priority subtopics like video generation or world models. Signals: Low diversity in LLM stories, preference data impact, alignment algorithm effect, not aligned with user's priority subtopics
...
### [94] [PRISM: A Multi-Dimensional Benchmark for Evaluating LLM Peer Reviewers](https://arxiv.org/abs/2605.26730)
...
TL;DR: JuICE 是一个多语言基准数据集，包含 7,470 条细粒度标注，用于评估 LLM 作为评判者（LLM-as-a-Judge）在识别 LLM 生成回复中的文化错误（thick cultural errors）的能力。实验表明，即使最强的 LLM 评判者在错误跨度检测任务上也仅达到 0.52 F1，且系统性地遗漏了本地居民能轻易识别的深层文化错误。


<details>
  <summary>Details</summary>
Motivation: 现有文化基准将文化视为扁平的事实集（如事实验证或规范蕴含），且直接采用 LLM-as-a-Judge 范式而未检验其是否能捕捉深层文化错误（thick cultural errors）。随着 LLM 在全球范围部署，生成内容需要符合不同文化语境，因此亟需一个专门评估 LLM 评判者识别文化错误能力的基准。

Method: 构建 JuICE 数据集：覆盖美国、韩国、印度尼西亚、孟加拉国四个国家，包含英语及各国主要语言，共 1,050 个查询-回复对，由本地标注者进行 7,470 条跨度级别的文化和语言错误标注。使用该数据集评估多种 LLM-as-a-Judge 在错误跨度检测任务上的表现。

Result: 最强的 LLM 评判者在错误跨度检测任务上仅达到 0.52 F1 分数；LLM 评判者系统性地遗漏了本地居民能轻易识别的深层文化错误（thick cultural errors），表明现有 LLM 评判者在文化敏感性方面存在显著不足。

Conclusion: 稳健的文化评估必须超越表面级别的检测，转向能够考虑文化意义的深度和情境性的评估框架。JuICE 为未来研究提供了评估和改进 LLM 文化敏感性的重要基准。

Classification reason: 该论文提出了 JuICE，一个用于评估 LLM-as-a-Judge 在识别文化错误方面能力的基准数据集，属于 AIGC 生成内容质量评测范畴，与 [CANONICAL] subtopic "AIGC 评测与基准"（aigc_evaluation_benchmark）高度匹配。

Importance reason: Focuses on cultural error detection in LLM outputs, which is peripheral to the user's core interests in generative models and efficiency. Signals: Cultural benchmark, LLM-as-Judge, low F1 on error detection, not aligned with priority subtopics
...
### [96] [PashtoTTS-Bench: automated screening for low-resource non-Latin-script text-to-speech](https://arxiv.org/abs/2605.26978)
*Hanif Rahman*
...
TL;DR: 本文提出 INSV（可懂度、自然度、文字忠实度、验证）报告框架及其自动化子集 INSV-A，并实例化为 PashtoTTS-Bench，用于低资源非拉丁文字（普什图语）TTS 系统的自动化筛选评测。在 200 条 FLEURS 和 200 条 Common Voice 24 提示上评估了多个商业 TTS 系统，发现 OmniVoice auto 的 WER 最低，同时揭示了 ASR 回译 WER 作为单一指标的局限性。


<details>
  <summary>Details</summary>
Motivation: 低资源非拉丁文字（如普什图语）的 TTS 评测若仅依赖单一 ASR 回译 WER 指标会失败——系统可能不输出音频、输出邻国语言、仅在 ASR 转录中保留目标文字，或听起来不自然。现有评测方法无法区分这些失败模式，亟需一个更全面的自动化筛选框架。

Method: 提出 INSV 报告框架，包含四个维度：可懂度（Intelligibility）、自然度（Naturalness）、文字忠实度（Script fidelity）、验证（Verification）。本文实现其自动化子集 INSV-A，包括：合成完成检测、ASR WER/CER、转录文字忠实率（Script Fidelity Rate）、音频语言识别（LID）。实例化为 PashtoTTS-Bench，使用 FLEURS 和 Common Voice 24 数据集，评估 Edge GulNawaz、Edge Latifa、OmniVoice clone、OmniVoice auto 及乌尔都语负对照系统。

Result: OmniVoice auto 在 omniASR_CTC_300M_v2 下 WER 最低（FLEURS 24.1%，CV24 27.4%），其次为 Edge GulNawaz（32.8%, 39.5%）。Whisper Large V3 对普什图语 TTS 音频返回 0.0% 普什图语标签，而 MMS-LID-4017 和 SpeechBrain VoxLingua107 能有效区分普什图语与乌尔都语输出。WER 低于自然语音基线反映的是合成音频的干净性，不应解读为优于真人语音。

Conclusion: 单一 ASR 回译 WER 不足以评估低资源非拉丁文字 TTS 系统，INSV-A 框架通过多维度自动化筛选提供了更可靠的评测方案。PashtoTTS-Bench 作为有日期标注的基准，为后续研究提供了可复现的评测流程和基线数据。

Classification reason: 该论文提出了 PashtoTTS-Bench，一个针对低资源非拉丁文字（普什图语）TTS 的自动化评测基准，属于 AIGC 方向下的生成内容质量评测与基准构建，与 canonical 子主题 aigc_evaluation_benchmark 高度匹配。

Importance reason: Focuses on a specific low-resource TTS benchmark for Pashto, which is far from the user's priority topics in video/image generation and efficient multimodal models. Signals: Low-resource TTS evaluation, Non-Latin script focus, Automated screening framework, No direct connection to video/image generation or world models, Negative keywords not triggered but topic is off-target
...
### [97] [ENPMR-Bench: Benchmarking Proactive Memory Retrieval for Emotional Support Agents](https://arxiv.org/abs/2605.27240)
...
TL;DR: 本文提出了 ENPMR-Bench，一个用于评估情感支持智能体主动记忆检索能力的基准。基于马斯洛需求层次理论，包含1800+记忆增强对话，定义了情感需求与支持性记忆类型之间的结构化映射。实验表明现有检索范式（嵌入式和LLM驱动）在共情评分上显著落后于黄金记忆条件，CoT提示虽有所改善但仍存在明显差距。


<details>
  <summary>Details</summary>
Motivation: 现有记忆增强语言智能体通常将记忆视为事实检索工具，忽视了记忆在塑造用户情感体验中的作用。情感支持场景需要智能体理解用户潜在情感需求并主动检索相关记忆，但缺乏针对这一能力的系统评测基准。

Method: 基于马斯洛需求层次理论构建情感需求分类体系，定义情感需求与支持性记忆类型之间的结构化映射。收集并标注了1800+记忆增强对话，构建ENPMR-Bench基准。评估了嵌入式和LLM驱动的多种检索范式，并使用共情评分等指标进行评测。

Result: 现有检索范式在共情评分上显著落后于黄金记忆条件；链式思维（CoT）提示能在一定程度上改善情感需求推断与记忆检索的对齐，但性能差距仍然明显。

Conclusion: 当前情感支持智能体在主动记忆检索方面存在显著缺陷，需要发展对情感需求敏感的个性化记忆检索方法，以提升智能体的共情交互能力。

Classification reason: 该论文提出了一个面向情感支持智能体的主动记忆检索评测基准（ENPMR-Bench），属于 AIGC 方向下的评测与基准（aigc_evaluation_benchmark）[CANONICAL] 子主题，因为其核心贡献是构建了一个新的评测基准来评估智能体的情感支持能力。

Importance reason: Focuses on emotional support agent memory retrieval, which is outside the user's core interests in generative models and efficient inference. Signals: emotional support agents, memory retrieval benchmark, NLP/CL category, no generative model focus, no efficiency or world model connection
...
### [98] [The Constraint Tax: Measuring Validity-Correctness Tradeoffs in Structured Outputs for Small Language Models](https://arxiv.org/abs/2605.26128)
*Jaideep Ray*
...
TL;DR: 本文提出"约束税"（constraint tax）概念，系统测量结构化输出约束对小型语言模型（<3B参数）生成质量的影响。实验表明，硬约束虽能将schema有效性从61.5%提升至100%，但会导致答案准确率从19.7%降至11.0%，错误有效schema输出从49.5%升至88.9%。作者提出"自由推理，延迟约束"的设计模式，并建议生产系统应分别报告schema有效性、答案准确性、可执行准确性和错误有效schema率。


<details>
  <summary>Details</summary>
Motivation: 生产环境中LLM系统越来越多地要求机器可读的结构化输出（JSON、工具调用schema等），工程上通常假设硬输出约束能提升可靠性而不改变答案质量。本文质疑这一假设对小型语言模型（<3B参数）是否成立，旨在量化结构化约束带来的准确性损失。

Method: 提出"约束税"测量协议，在固定模型、固定任务分布和固定问题实例下，隔离结构化输出约束导致的答案准确性和可执行准确性损失。在15,000次GPU生成实验中，使用Qwen2.5-0.5B、Qwen2.5-1.5B和SmolLM2-1.7B三个模型，对比无约束（prompt-only JSON）与硬约束（确定性schema解码）两种模式下的schema有效性、答案准确率、可执行准确率和错误有效schema率。

Result: 硬约束将schema有效性从61.5%提升至100%，但答案准确率从19.7%降至11.0%，错误有效schema输出从49.5%升至88.9%。在日历工具调用任务中，Qwen2.5-1.5B在prompt-only JSON下可执行准确率为91.5%，硬约束下仅48.0%。3B边界模型仍存在直接schema税。错误是语义层面的而非结构层面的。

Conclusion: 硬输出约束对小型语言模型存在显著的"约束税"效应，工程假设不安全。建议采用"自由推理，延迟约束"的设计模式，生产系统应分别报告schema有效性、答案准确性、可执行准确性和错误有效schema率四个指标。

Classification reason: 该论文研究结构化输出约束对小型语言模型（SLM）生成质量的影响，提出了"约束税"（constraint tax）测量协议，系统评估了schema有效性、答案准确性和可执行准确性之间的权衡。这属于AIGC生成内容质量评测范畴，与[CANONICAL] subtopic "AIGC 评测与基准"（aigc_evaluation_benchmark）高度匹配。

Importance reason: Focuses on small language models and structured output constraints, which is not a priority subtopic and lacks direct connection to user's key interests in generation and efficiency. Signals: constraint tax, small language models, structured outputs, validity-correctness tradeoff, delayed constraint design pattern
...
### [99] [TSFMAudit: Data Contamination Auditing in Forecasting Time Series Foundation Models](https://arxiv.org/abs/2605.26161)
...
TL;DR: 本文首次系统研究时序基础模型（TSFM）的预训练数据污染审计问题，提出TSFMAudit方法，基于微调探针的适应动态（污染数据在微调后损失下降更快、骨干网络移动更小）来检测评测集是否在预训练阶段被暴露，在6个TSFM和187个数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 时序基础模型（TSFMs）在大规模语料上预训练，评测数据集可能在预训练阶段已被暴露，导致性能评估过于乐观。然而时序数据的连续性、异质性以及缺乏语料文档使得污染审计极具挑战，现有LLM领域的污染审计方法无法直接适用。

Method: 提出TSFMAudit方法，核心直觉是数据污染表现为异常高效的适应：对评测数据进行微调探针后，被污染的数据集往往表现出更快的损失下降和更小的骨干网络参数移动。通过量化微调过程中的损失下降速率和参数变化幅度来检测污染。

Result: 在6个TSFM和187个数据集上，利用有文档记录的训练来源证据作为监督，与10个从LLM文献改编的基线方法对比，TSFMAudit在污染检测任务上取得了最优性能。

Conclusion: TSFMAudit是首个针对时序基础模型预训练污染审计的工作，通过微调探针适应动态有效检测数据污染，为时序模型评测的可信度提供了重要保障。

Classification reason: 该论文研究时序基础模型（TSFM）的预训练数据污染审计问题，属于AIGC方向下的评测与基准子主题。虽然具体对象是时序模型而非图像/视频生成，但"数据污染审计"本质上是评估生成模型/基础模型评测可信度的评测方法，与aigc_evaluation_benchmark（AIGC评测与基准）的"生成内容质量评测"内涵一致，且该子主题关键词包含"evaluation"和"benchmark"，可合理覆盖此类评测可信度研究。

Importance reason: Focuses on time series foundation model contamination auditing, which is outside the user's core interests in multimodal generation and video/image generation. Signals: First work on TSFM contamination auditing, strong empirical validation, not aligned with user's priority subtopics or boost keywords, negative keywords not triggered but direction is off-topic
...
### [100] [Anchor: Mitigating Artifact Drift in Agent Benchmark Generation](https://arxiv.org/abs/2605.26321)
...
TL;DR: 本文提出 Anchor，一个通过约束优化程序从领域专家规范中联合生成指令、环境、最优解和验证器的任务生成流水线，解决了 agent 基准生成中的"artifact drift"问题。基于 Anchor 构建了 ERP-Bench，包含 300 个企业 ERP 长周期任务，实验表明前沿模型仅 17.4% 能达到完全最优解。


<details>
  <summary>Details</summary>
Motivation: 现有 AI agent 训练和评估环境在企业级任务中难以同时保证真实性、可验证性和可扩展性。环境与任务创建过程中，指令、环境、oracle 和验证器由松散耦合的流程生成，经常对任务要求产生不一致（artifact drift），导致环境不可解、可奖励破解或不一致。

Method: Anchor 将领域专家对业务流程的规范形式化为约束优化程序。从单一参数化规范出发，流水线联合生成自然语言指令、环境配置、求解器认证的真实解和基于状态的验证器。通过调整参数可生成具有可控难度和已知最优解的新任务，产生与 harness 无关的环境，奖励仅取决于最终状态的业务正确性。

Result: 基于 Anchor 构建了 ERP-Bench，包含 300 个覆盖采购和制造工作流的长周期任务。实验发现生成参数能预测实际难度；前沿模型在 26.1% 的试验中满足显式任务约束，但仅 17.4% 达到完全最优解。

Conclusion: Anchor 和 ERP-Bench 为构建可审计的、经济价值高的 agent 评估环境提供了具体方案，有效缓解了 artifact drift 问题，为未来企业级 AI agent 的可靠评估奠定了基础。

Classification reason: 该论文提出了 ERP-Bench 基准和 Anchor 任务生成流水线，用于评估 AI agent 在企业级 ERP 系统中的长周期任务执行能力。这属于 AIGC 方向下的评测与基准（aigc_evaluation_benchmark），因为核心贡献是构建了一个新的 AIGC agent 评测基准，并设计了可控的任务生成方法。

Importance reason: Focuses on agent benchmark generation for enterprise ERP, not aligned with user's priority subtopics in multimodal generation or model efficiency. Signals: Agent benchmark generation, artifact drift, enterprise ERP tasks, constraint optimization, low frontier model performance
...
### [101] [JobBench: Aligning Agent Work With Human Will](https://arxiv.org/abs/2605.26329)
...
TL;DR: JobBench 是一个评估 AI 智能体在真实职业工作流中执行委托任务能力的基准，涵盖 35 个职业的 130 个任务，使用事实锚定的评分标准链进行评测。最强模型 Claude Opus 4.7 + Claude Code 仅达 45.9%，表明当前 AI 智能体在真实专业工作场景中仍有巨大提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 智能体基准主要基于经济价值（GDP）来评估，讲述的是"替代人类"的故事。作者希望将社区的目标从"替代"转向"增强"——构建人类真正希望委托的智能体，而不仅仅是经济上最有价值的任务。

Method: JobBench 覆盖 35 个职业的 130 个智能体任务，每个任务打包为一个包含异构参考文件的工作空间，要求智能体在杂乱的信息流中进行推理。输出通过事实锚定的评分标准链（平均每任务 35.6 个二值标准）进行评分。

Result: 评估了 36 个模型，最强模型 Claude Opus 4.7 配合 Claude Code 仅达到 45.9% 的得分，表明当前 AI 智能体在真实专业工作场景中表现有限。

Conclusion: JobBench 旨在推动社区从"替代人类"的叙事转向"增强人类"——构建人类真正希望委托的智能体，而不仅仅是追求经济价值最大化。

Classification reason: 该论文提出 JobBench，一个评估 AI 智能体在真实职业工作流中执行委托任务能力的基准，属于 AIGC 方向下的智能体评测。现有 [CANONICAL] 子主题 "AIGC 评测与基准"（aigc_evaluation_benchmark）与之高度匹配，因为论文核心贡献是一个新的评测基准，评估 AI 智能体在专业工作场景中的表现。

Importance reason: Focuses on agent alignment with human will rather than core generation or efficiency, and lacks direct connection to priority subtopics. Signals: Agent benchmark, human delegation, low model scores, no generation focus, no efficiency signals
...
### [102] [When Does Deep RL Beat Calibrated Baselines? A Benchmark Study on Adaptive Resource Control](https://arxiv.org/abs/2605.26418)
...
TL;DR: 本文提出了 RLScale-Bench，一个用于深度强化学习在自适应资源控制上的可复现基准。实验发现：一个精心校准的基于规则的自动伸缩器在成本上击败了六种主流 DRL 算法；离散动作 DRL 算法在约束违反上比连续动作算法好一到两个数量级；没有单一算法在所有工作负载上占优。DRL 在资源控制中的瓶颈不是算法选择，而是基线校准、奖励工程和现实评测协议。


<details>
  <summary>Details</summary>
Motivation: 尽管深度强化学习被广泛认为适用于自适应资源控制（如弹性伸缩），但缺乏严格、可复现的基准来系统比较 DRL 算法与精心校准的基于规则的基线。现有研究往往只展示 DRL 的优势，忽略了基线校准的重要性，导致对 DRL 实际效用的认知偏差。

Method: 作者构建了 RLScale-Bench 基准，在 Kubernetes 水平 Pod 自动伸缩场景下，对 PPO、DQN、A2C、SAC、TD3、DDPG 六种主流 DRL 算法与一个校准的基于规则控制器进行对比。实验采用匹配的架构、训练预算和奖励函数，覆盖六种工作负载模式（含突发流量和闪流），每种运行 5 个种子（共 240 次运行），并测试分布偏移泛化能力。

Result: (i) 校准的基于规则控制器在所有六种工作负载上实现了最低成本，但在突发和闪流流量下落后于最佳 RL 算法；(ii) 离散动作 DRL 算法在约束违反上比连续动作算法好一到两个数量级；(iii) 没有单一算法在所有工作负载上占优，排名变化可达四个位置。

Conclusion: DRL 在自适应资源控制中的瓶颈不在于算法选择，而在于基线校准、奖励工程和现实评测协议。该研究挑战了"DRL 天然优于规则方法"的常见假设，为未来 DRL 在资源控制领域的应用提供了重要的方法论指导。

Classification reason: 该论文提出了 RLScale-Bench，一个用于深度强化学习在自适应资源控制（弹性伸缩）上的可复现基准与评测协议，属于 AIGC 评测与基准（aigc_evaluation_benchmark）范畴。虽然论文不直接涉及生成内容，但 AIGC 方向涵盖 AI 生成内容相关模型、系统与应用，而该论文聚焦于 AI 驱动的自适应资源控制系统的评测基准，与 aigc_evaluation_benchmark 中"生成内容质量评测、AIGC benchmark"的主题高度吻合。

Importance reason: Strong empirical benchmark for RL in resource control, but far from user's priority generative AI topics. Signals: RL benchmark, resource control, baseline calibration beats DRL, negative for generative AI focus
...
### [103] [TADDLE: A Tool-Augmented Agent for Detecting Deficient LLM-Generated Peer Reviews](https://arxiv.org/abs/2605.26911)
*Hanqi Duan, Xiang Li*
...
TL;DR: TADDLE 是一个工具增强型智能体，用于检测 LLM 生成的同行评审中的缺陷。它包含首个专家标注的多标签基准数据集（1,800条评审，6类缺陷），通过四个专用分析工具（Verify、Correct、Complete、Transform）由智能体编排，并利用两阶段半监督学习进行二分类和多标签分类。


<details>
  <summary>Details</summary>
Motivation: LLM 生成的同行评审在各大会议中越来越常见，但由于其语言流畅、结构完整，人工难以发现其中的缺陷。现有工作要么只判断是否为 AI 生成而不评估质量，要么使用针对人类评审设计的特征来评分，缺乏在单个缺陷类型层面检测 LLM 生成评审缺陷的系统。

Method: TADDLE 将缺陷检测任务分解为四个专用分析工具：Verify（验证事实准确性）、Correct（纠正错误）、Complete（补充缺失内容）、Transform（转换视角评估），由智能体编排调用。一个集成模块通过两阶段半监督学习将各工具的输出整合为二分类和多标签分类结果。同时构建了包含 1,800 条评审、由 18 位领域专家标注的基准数据集，涵盖六类缺陷标签。

Result: 大量实验表明，TADDLE 在二分类检测和多标签分类任务上均表现优异，显著优于基线方法。

Conclusion: TADDLE 有效填补了检测 LLM 生成同行评审缺陷的空白，其工具增强的智能体架构和专家标注基准为提升学术评审质量提供了实用方案。

Classification reason: 该论文研究检测LLM生成的同行评审中的缺陷，属于AIGC内容质量评测范畴。论文构建了首个专家标注的基准数据集（1,800条评审），并提出了检测工具，这与[AIGC 评测与基准]（aigc_evaluation_benchmark）高度匹配。

Importance reason: Focuses on LLM-generated peer review detection, which is peripheral to the user's core interests in generative models and efficient inference. Signals: Tool-augmented agent, multi-label benchmark, LLM-generated review detection, semi-supervised learning, not aligned with priority subtopics
...
### [104] [VitaBench 2.0: Evaluating Personalized and Proactive Agents in Long-Term User Interactions](https://arxiv.org/abs/2605.27141)
...
TL;DR: VitaBench 2.0 是一个评估个性化与主动式智能体在长期用户交互中表现的基准。任务按时间顺序组织，用户偏好嵌入在碎片化交互中，智能体需持续提取、利用和更新用户偏好，并主动获取缺失信息。实验表明现有最先进模型在此场景下仍面临巨大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准主要评估推理和工具使用能力，忽视了在真实场景中推断和利用用户偏好的挑战。用户意图往往体现在碎片化的日常交互中，需要智能体具备个性化建模和主动交互能力，而现有评测体系缺乏对此类能力的系统评估。

Method: VitaBench 2.0 将任务组织为按时间顺序排列的用户交互序列，偏好信息嵌入在碎片化、异构的交互中。提供了可扩展的记忆接口以支持不同记忆架构的对比分析，并通过需要智能体识别缺失信息并主动获取的任务来评估主动性。

Result: 对多种前沿闭源和开源 LLM 的基准测试表明，真实世界的个性化对最先进模型仍然极具挑战性，当前能力与实际需求之间存在显著差距。进一步分析揭示了当前智能体在真实个性化决策中的失败模式和能力瓶颈。

Conclusion: VitaBench 2.0 填补了现有智能体评测在个性化和主动交互方面的空白，揭示了当前模型在长期用户交互中理解与利用用户偏好的不足，为未来模型改进提供了方向。

Classification reason: 该论文提出了 VitaBench 2.0，一个用于评估个性化与主动式智能体在长期用户交互中表现的基准。这属于 AIGC 方向下的评测与基准（aigc_evaluation_benchmark），因为论文核心贡献是构建了一个新的评测基准来评估 AI 智能体在个性化交互场景中的能力。

Importance reason: Focuses on agent evaluation rather than core generation or efficiency, and lacks direct connection to priority subtopics. Signals: AIGC benchmark, agent personalization, proactive interaction, long-term user modeling, LLM evaluation
...
### [105] [Semantic Robustness Probing via Inpainting: An Interactive Tool for Safety-Critical Object Detection](https://arxiv.org/abs/2605.27155)
...
TL;DR: 本文提出 SemProbe，一个基于扩散模型可控修补的语义鲁棒性探测工具，用于安全关键领域（如木工台锯的手部检测）的目标检测器鲁棒性评估。用户上传图像、创建掩码、选择 ODD 因子或自定义提示，通过扩散修补生成语义探针，自动运行模型推理并展示性能对比，所有探针作为结构化工件记录，支持可追溯的鲁棒性证据。


<details>
  <summary>Details</summary>
Motivation: 安全关键领域（如工业机械）的目标检测器需要超越像素级扰动的语义级鲁棒性测试，但现有工具缺乏对语义层面探针的灵活生成与系统化管理。

Method: SemProbe 工具：用户上传部署图像，通过手动或自动方式创建掩码，选择 ODD（操作设计域）因子或自定义提示，利用扩散模型进行受控修补生成语义探针。系统支持批量作业、并行种子/工作流变体及可配置生成参数。生成后自动运行模型推理，显示带注释的前后对比及性能差异。所有探针作为结构化工件记录。

Result: 在手部检测（木工台锯场景）上展示了 SemProbe 的应用，针对保险导向测试标准中的 ODD 因子生成语义探针，验证了工具在安全评估工作流中的可用性。

Conclusion: SemProbe 提供了一种可追溯、可复现的语义鲁棒性探测方法，弥合了生成式 AI 与安全关键系统评估之间的差距，为工业安全评估提供了结构化证据支持。

Classification reason: 该论文提出 SemProbe 工具，利用扩散模型可控修补（inpainting）生成语义探针，用于评估目标检测模型在安全关键场景下的鲁棒性。这属于 AIGC 生成内容用于模型评测的范畴，与 canonical 子主题 "AIGC 评测与基准"（aigc_evaluation_benchmark）高度匹配。同时涉及扩散模型生成（多模态生成方向）。

Importance reason: Focuses on a niche safety-critical object detection tool rather than advancing core generative model capabilities. Signals: diffusion-based inpainting, safety-critical object detection, robustness probing tool, not core generative model advancement, limited to hand detection use case
...
### [106] [VISTA: An End-to-End Benchmark for Visual Spec-to-Web-App Coding Agents](https://arxiv.org/abs/2605.26144)
...
TL;DR: VISTA 是一个用于评估基于 LLM 的智能体从视觉规格（文本+截图+Figma 结构）端到端生成 Web 应用的基准。它定义了五种输入条件，结合 DOM 匹配、浏览器测试和 CLIP 视觉相似度进行多维度评测，发现视觉保真度和功能正确性部分解耦。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成基准主要关注算法任务，缺乏对 UI 驱动的 Web 应用开发的评测。真实场景中，开发者需要从模糊的视觉规格（如截图、设计稿）生成功能完整且视觉一致的 Web 应用，现有工具（如 Playwright）在开放代码生成场景下存在局限性。

Method: VISTA 定义了五种 prompt 信息条件（沿视觉/结构保真度和技术栈约束两个轴变化），为每个页面手动标注交互式 UI 组件和约三个视觉锚点。评测结合 DOM 参考匹配、行为特定浏览器测试和 CLIP 视觉相似度，分别衡量结构对齐、行为完整性和视觉保真度。

Result: 使用 VISTA 评估了来自两个模型家族和两个框架的四个智能体系统，发现视觉保真度和功能正确性在不同输入条件和智能体间部分解耦，且智能体的编辑风格差异显著但与任务质量大致正交。

Conclusion: VISTA 为基于智能体的软件工程研究建立了严谨且可复现的评测基础，揭示了当前 LLM 智能体在端到端 Web 应用生成中的能力与局限。

Classification reason: 该论文提出了 VISTA，一个用于评估 LLM 智能体端到端 Web 应用生成能力的基准，属于 AIGC 评测与基准（aigc_evaluation_benchmark）范畴。同时，它也涉及多模态生成方向下的生成模型评测与数据集（generation_benchmark），因为其评测包含视觉相似度（CLIP）等多模态指标。

Importance reason: Focuses on AIGC evaluation benchmark for web-app coding, which is not a priority subtopic and lacks direct connection to video/image generation or model efficiency. Signals: AIGC benchmark, web-app generation, visual fidelity vs. functional correctness, low priority subtopic match, no boost keywords
...
### [107] [Plans for Evaluating Structured Generative Search Summaries](https://arxiv.org/abs/2605.26400)
...
TL;DR: 本文提出一个评估结构化生成式搜索摘要的框架，该摘要由大语言模型生成，包含概述、分节标题和引用来源。论文描述了框架的设计与实施计划。


<details>
  <summary>Details</summary>
Motivation: 随着LLM被用于生成搜索摘要，如何系统评估这些结构化生成摘要的质量成为一个重要问题。现有评估方法缺乏针对结构化摘要（含概述、分节、引用）的专门框架。

Method: 提出一个评估框架，涵盖结构化生成式搜索摘要的多个维度：概述质量、分节合理性、引用准确性等。论文描述了框架的设计思路和实施计划。

Result: 目前为框架设计阶段，尚未给出实验评估结果。论文主要呈现了评估框架的规划方案。

Conclusion: 该框架为评估LLM生成的结构化搜索摘要提供了系统化的评估思路，有助于推动生成式搜索摘要的质量评测标准化。

Classification reason: 该论文提出评估结构化生成式搜索摘要（LLM生成的搜索摘要）的框架，属于AIGC生成内容的评测范畴，与[AIGC 评测与基准]（aigc_evaluation_benchmark）高度匹配。

Importance reason: Focuses on evaluating search summaries, not core generative model advances in user's priority subtopics. Signals: AIGC evaluation, search summary, not autoregressive/diffusion video/image generation, no world model, no efficiency
...
### [108] [Qiskit QuantumKatas: Adapting Microsoft's Quantum Computing exercises for LLM evaluation](https://arxiv.org/abs/2605.27210)
*Juan Cruz-Benito, Ismael Faro*

Main category: quant-ph, cs.AI
...
TL;DR: 本文将微软 QuantumKatas 量子计算课程从 Q# 移植到 Qiskit，构建了一个包含 350 个任务、26 个类别的 LLM 评测基准，并在 16 个 LLM、7 种提示配置下进行了 39,200 次模型运行评估，发现最佳配置通过率从 32.3% 到 83.1% 不等，前沿模型与开源模型平均差距 26.1 个百分点。


<details>
  <summary>Details</summary>
Motivation: 量子计算领域缺乏系统化的 LLM 评测基准，现有评测多关注通用编程任务，难以评估 LLM 在量子计算这一专业领域的代码生成与问题解决能力。

Method: 将 QuantumKatas 从 Q# 适配到 Qiskit，保留其渐进式难度设计和概念覆盖，每个任务包含自然语言提示、标准答案和确定性测试验证。使用 16 个 LLM 在 7 种提示配置下进行系统评估，共 39,200 次运行。

Result: 最佳配置通过率范围 32.3%-83.1%，前沿与开源模型平均差距 26.1pp；模型在已知算法实现上表现好（SimonsAlgorithm 82.1%），但在问题编码上困难（SolveSATWithGrover 34.4%）；思维链提示对部分推理调优模型有效但对其他模型有负面影响。

Conclusion: 该基准能有效区分不同 LLM 在量子计算领域的能力差异，揭示了模型在算法实现与问题编码之间的能力鸿沟，为 LLM 在专业科学领域的评估提供了新工具。

Classification reason: 该论文将量子计算课程改编为 LLM 评测基准（含 350 个任务、26 个类别），属于 AIGC 方向下的「AIGC 评测与基准」子主题，因为其核心贡献是构建了一个用于评估 LLM 能力的 benchmark 和评测框架。

Importance reason: Focuses on quantum computing LLM evaluation, which is outside the user's priority subtopics and contains negative keywords. Signals: quantum computing, LLM benchmark, Qiskit, low personal relevance, negative keyword match
...
### [109] [LATTE: Forecasting Peer Anchored Preference Trajectories for Personalized LLM Generation](https://arxiv.org/abs/2605.26612)
...
Importance: 普通相关 (51.89) - 未入选精读

TL;DR: LATTE 提出一种面向冻结大语言模型的个性化生成框架，通过同行锚定相对偏好状态预测用户偏好轨迹，并将预测结果以单个软 token 注入 LLM，在 Amazon Reviews 2023 和 MemoryCD 数据集上优于多种基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有个性化方法将用户历史行为聚合为静态画像，混淆了稳定身份、近期漂移和物品内容，无法有效捕捉用户偏好的动态变化。需要一种既能保持紧凑性又能反映用户偏好时序演变的个性化信号。

Method: LATTE 框架包含三个核心组件：(1) 对每个历史会话，减去来自对同一物品有响应的可比用户的时域掩码基线，得到衡量目标用户在共享物品上下文中与同行差异的状态；(2) 轻量级序列预测器预测该轨迹中的下一个状态；(3) 状态到 Token 桥接器通过单个锚定软 token 将预测注入冻结的指令微调 LLM。

Result: 在 Amazon Reviews 2023 数据集上，LATTE 将平均 ROUGE-L 从静态潜在画像的 0.219 和最强基线方法的 0.245 提升至 0.259。在 MemoryCD 数据集上也取得一致改进。消融实验表明改进主要来自预测用户特定轨迹信息。

Conclusion: LATTE 通过同行锚定偏好轨迹预测实现了更有效的个性化生成，证明了将个性化建模为时序预测问题而非静态聚合的优越性，为冻结 LLM 的个性化生成提供了新范式。

Classification reason: 该论文研究面向大语言模型的个性化生成方法，通过预测用户偏好轨迹实现个性化内容生成，属于 AIGC 方向下的"个性化与对话生成"（aigc_personalization_persona）子主题。

Importance reason: Focuses on personalized LLM generation via preference trajectory forecasting, which is not a priority subtopic and lacks strong alignment with user's key directions. Signals: aigc, personalized generation, soft token injection, user preference forecasting, no video/image generation
...
### [110] [An investigation of AI integration in sound designer workflows and experiences](https://arxiv.org/abs/2605.27174)
*Nelly Garcia, Joshua Reiss*

Main category: cs.CY, cs.AI, cs.SD
...
TL;DR: 本文通过混合方法研究（76人问卷调查+20人半结构化访谈），调查了AI工具在专业声音设计师工作流中的集成现状。研究发现当前AI工具在快速消费媒体场景中表现尚可，但在高端声音设计（电影、沉浸式体验等）中缺乏叙事复杂性；从业者更偏好辅助性、任务特定的AI应用（如音频修复、库管理），而非端到端生成系统。文章基于发现为声音技术开发者提供了一系列建议。


<details>
  <summary>Details</summary>
Motivation: AI工具越来越多地集成到专业音频制作工作流中，但开发者构建的工具与实际声音设计师的需求之间存在显著差距。本文旨在系统性地调查这一差距，理解从业者的真实需求、偏好和顾虑。

Method: 采用混合研究方法：1）对76名从业者进行问卷调查；2）对20名行业专业人士进行半结构化后续访谈。使用描述性统计分析和主题分析对结果进行分析，识别出五个主题：上下文（Context）、工作流（Workflow）、潜力（Potential）、风险（Risks）和正确使用（Right Use）。

Result: 识别出五个核心主题：Context、Workflow、Potential、Risks、Right Use。当前AI工具在快速消费媒体场景中表现充分，但在高端声音设计中缺乏叙事复杂性。从业者更偏好辅助性、任务特定的AI应用（音频修复、库管理），而非端到端生成系统。

Conclusion: AI在声音设计中的集成仍处于辅助工具阶段，距离替代高端创意工作还有差距。研究为声音技术开发者提供了一套基于从业者实际需求的建议，以指导更符合行业需求的AI工具开发。

Classification reason: 该论文研究AI在声音设计师工作流中的集成，属于创意艺术生成领域（音频创作/声音设计），与[CANONICAL] subtopic "创意与艺术生成"（aigc_creative_art）高度匹配。

Importance reason: User priorities focus on generative video/image models and efficiency, not sound design workflows. Signals: sound design, AI integration, creative industry, user study, assistive tools
...
### [111] [GEM: Geometric Entropy Mixing for Optimal LLM Data Curation](https://arxiv.org/abs/2605.26121)
...
TL;DR: GEM 将 LLM 预训练数据策展重新形式化为超球面上的变分问题，通过几何熵混合和 MM 算法发现平衡的语义结构，结合师生蒸馏扩展到大规模语料，在 1.1B 参数模型上提升下游准确率最高 1.2%。


<details>
  <summary>Details</summary>
Motivation: LLM 预训练效果越来越依赖数据组成而非数据量，但现有数据混合方法受限于分类缺陷：人工分类存在本体论错位，欧氏聚类无法处理嵌入各向异性，导致无法发现最优的语义混合结构。

Method: 提出 GEM 框架，将数据策展重新形式化为超球面上的变分问题，并引入混合平衡正则化项。通过解耦生成先验并使用可证明的 MM (Minorize-Maximize) 算法优化目标，有效对抗聚类坍缩。采用师生蒸馏将几何保真度扩展到网络规模语料，并引入几何影响分数 (GIS) 用于可解释的类别生成。

Result: 在 1.1B 参数模型上的实验表明，GEM 集成到 DoReMi 和 RegMix 等混合策略中时，平均下游准确率提升最高达 1.2%，建立了新的 SOTA，并为可预测的数据混合提供了鲁棒的坐标系统。

Conclusion: GEM 通过几何熵混合有效解决了数据策展中的分类缺陷问题，为 LLM 预训练数据的最优混合提供了理论基础和实用框架，展示了几何方法在数据质量优化中的巨大潜力。

Classification reason: 该论文研究 LLM 预训练数据的最优混合与数据策展，属于合成数据/数据增强范畴，与 canonical 子主题「合成数据与数据集蒸馏」(aigc_synthetic_data) 高度相关。

Importance reason: Focuses on LLM data curation, not directly on the user's priority subtopics like video generation or world models. Signals: LLM data curation, geometric entropy mixing, MM algorithm, teacher-student distillation, not aligned with priority subtopics
...
### [112] [Model Unlearning Objectives Vary for Distinct Language Functions](https://arxiv.org/abs/2605.26454)
...
TL;DR: 本文提出针对不同语言功能（危险知识遗忘与毒性遗忘）应设计不同的模型遗忘目标函数。对于危险知识遗忘，引入基于余弦相似度的元学习变体 RMU；对于毒性遗忘，提出基于层特定探针方向的多层目标。在四个 7-8B 开源模型上取得了强结果，表明遗忘应像后训练一样被视为一个多问题家族。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在预训练中会学习到不良特性（如危险知识和毒性文本生成）。现有遗忘方法通常采用统一的目标函数，但不同语言功能（如知识遗忘 vs 毒性遗忘）在机制上存在差异，需要针对性地设计遗忘目标。

Method: 1) 针对危险知识遗忘：提出基于余弦相似度的元学习变体 RMU（cos-RMU），通过元学习优化余弦相似度目标来遗忘特定知识。2) 针对毒性遗忘：提出基于层特定探针方向的多层目标（multi-layer objective），利用各层探针方向引导模型遗忘毒性内容。在四个 7-8B 开源模型（Llama-2-7B, Mistral-7B, Gemma-7B, Llama-3-8B）上进行实验。

Result: 两种方法在各自任务上均取得强结果：cos-RMU 在危险知识遗忘上表现优异，多层目标在毒性遗忘上效果显著。实验表明不同遗忘目标需要不同的训练目标函数，统一方法难以同时最优。

Conclusion: 模型遗忘应像后训练一样被视为一个多问题家族，不同语言功能需要设计不同的遗忘目标函数。这为未来 LLM 安全对齐提供了更细粒度的研究方向。

Classification reason: 该论文研究大语言模型中的模型遗忘（unlearning），具体针对危险知识遗忘和毒性文本遗忘，属于生成模型安全与遗忘学习范畴。现有 taxonomy 中 aigc_unlearning_erasure（概念擦除与遗忘学习）完全匹配，故直接复用。

Importance reason: Focuses on LLM unlearning, which is not a priority subtopic and lacks direct connection to user's core interests in generation and efficiency. Signals: LLM safety, unlearning objectives, dangerous knowledge, toxicity, meta-learning
...
### [113] [On the Hidden Costs of Counterfactual Knowledge Training in LLM Unlearning](https://arxiv.org/abs/2605.27083)
...
TL;DR: 本文揭示了 LLM 反事实遗忘学习（CFT）范式中两个被忽视的缺陷：知识冲突（反事实语料内部不一致导致梯度冲突）和幻觉溢出（拟合虚假目标导致无关领域幻觉率上升），并提出了扩展基准 RWKU+ 及梯度级诊断工具来系统分析这些问题。


<details>
  <summary>Details</summary>
Motivation: 反事实微调（CFT）是 LLM 遗忘学习的主流范式之一，通过训练模型生成替代性虚假知识来取代不良内容。然而，该范式在某些方面仍不如其他遗忘范式，其背后的根本原因尚未被系统诊断和理解。

Method: 作者识别出 CFT 的两个关键缺陷：知识冲突（反事实语料内部不一致导致梯度冲突，干扰参数优化）和幻觉溢出（拟合虚假目标导致模型产生持续的编造偏差，在无关领域也增加幻觉率）。为系统诊断这些问题，提出了扩展基准 RWKU+，包含新的权衡指标和梯度级诊断工具。

Result: 通过 RWKU+ 基准和梯度诊断工具，系统揭示了 CFT 范式在遗忘效果与模型通用能力之间的权衡问题，验证了知识冲突和幻觉溢出两个缺陷的存在及其对模型性能的影响。

Conclusion: 反事实遗忘学习范式存在知识冲突和幻觉溢出两个被忽视的缺陷，导致其遗忘效果受限且会损害模型通用能力。本文为更严谨的 LLM 遗忘研究提供了见解和可操作的指导。

Classification reason: 论文研究 LLM 遗忘学习（unlearning）中的反事实知识训练问题，属于 AIGC 方向下的"概念擦除与遗忘学习"（aigc_unlearning_erasure）规范子主题，该子主题明确涵盖模型 unlearning 与安全生成。

Importance reason: Focuses on LLM unlearning pitfalls, not aligned with user's priority on generative video/image models or efficient inference. Signals: LLM unlearning, counterfactual tuning, knowledge conflict, hallucination spillover, not generative modeling
...
### [114] [ICCU: In-Context Continual Unlearning via Pattern-Induced Refusal Rules](https://arxiv.org/abs/2605.27138)
...
TL;DR: 本文提出 ICCU（In-Context Continual Unlearning），一种基于上下文学习的持续遗忘框架。它从遗忘数据中归纳出可读的拒绝规则，在推理时以过滤器或系统提示方式应用，无需修改模型参数，从而避免跨请求干扰和效用损失，支持顺序遗忘请求的扩展。


<details>
  <summary>Details</summary>
Motivation: 实际部署中遗忘请求往往是顺序到达的，现有基于微调的遗忘方法存在三个问题：每次请求微调成本高、累积效用损失、跨请求干扰。因此需要一种无需参数修改、可组合、无干扰的持续遗忘方法。

Method: ICCU 框架包含两个阶段：(1) 规则归纳阶段——从遗忘数据集（forget set）中自动生成可读的拒绝规则（refusal rules），规则以自然语言形式表达，归纳后原始数据可丢弃；(2) 推理阶段——将累积的规则以顺序无关的并集方式组合，通过过滤器或系统提示（system prompt）在推理时应用，无需修改模型参数。

Result: 实验表明 ICCU 能有效抑制目标知识的同时保持模型效用，在顺序遗忘请求下具有良好的可扩展性，并对释义查询和跨语言查询具有鲁棒性。

Conclusion: ICCU 提供了一种轻量级、可组合、无干扰的持续遗忘方案，避免了微调方法的缺陷，为实际部署中的顺序遗忘需求提供了有效解决方案。

Classification reason: 该论文研究大语言模型的持续遗忘学习（continual unlearning），属于生成模型安全与遗忘学习范畴，与 AIGC 方向下的 [CANONICAL] 子主题 "概念擦除与遗忘学习"（aigc_unlearning_erasure）高度匹配。

Importance reason: Focuses on machine unlearning, which is not a priority subtopic and lacks alignment with user's key directions. Signals: machine unlearning, in-context learning, refusal rules, no parameter modification, sequential requests
...
### [115] [Image Feature Fusion-based Federated Client Unlearning (FCU)](https://arxiv.org/abs/2605.26715)
...
TL;DR: 本文提出基于图像特征融合的联邦客户端遗忘方法（IFF-FCU），通过线性图像特征融合机制（Mixup）动态生成混合样本，在遗忘目标知识与保持全局泛化能力之间取得更好平衡，在医学影像基准上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 数据保护法规中的"被遗忘权"推动了联邦遗忘（FU）技术的发展，但现有方法存在灾难性遗忘问题——擦除目标知识的同时也丢失了重要的保留知识，损害了模型的全局泛化能力。因此需要一种更好的方法来平衡遗忘效果与泛化能力。

Method: 提出 IFF-FCU 方法，引入线性图像特征融合机制（Mixup），动态创建混合样本以弥合遗忘分布与保留分布之间的差距。该策略不是简单地删除离散数据点，而是从理论上拓宽并正则化遗忘边界。

Result: 在医学影像基准（RSNA-ICH 和 ISIC2018）上的实验表明，该方法实现了良好的遗忘效果。在 ICH 数据集上，IFF-FCU 的误差偏差与重新训练的金标准相比具有高度竞争力，展现出优于现有基线的鲁棒改进。

Conclusion: IFF-FCU 通过特征融合机制有效缓解了联邦遗忘中的灾难性遗忘问题，在遗忘效果与模型泛化能力之间取得了更好的平衡，为联邦学习中的"被遗忘权"实现提供了更优的解决方案。

Classification reason: 该论文研究联邦学习中的遗忘（unlearning）问题，属于生成模型/机器学习模型的概念擦除与遗忘学习范畴，与 AIGC 方向下的 [CANONICAL] subtopic "概念擦除与遗忘学习"（aigc_unlearning_erasure）高度匹配。

Importance reason: Focuses on federated unlearning in medical imaging, which is outside the user's priority subtopics and contains negative keywords. Signals: Federated unlearning, catastrophic forgetting, medical imaging, Mixup feature fusion, low relevance to video/image generation or efficient inference
...
### [116] [Vectors Are Not Neutral: Sensitive-Information Inference from Exported LLM Representations in Summarization](https://arxiv.org/abs/2605.26433)
...
Importance: 普通相关 (36.84) - 未入选精读

TL;DR: 本文研究LLM摘要系统中导出向量表示（如隐藏状态和池化表示）的敏感信息泄露风险，以临床出院摘要中的种族信息作为案例，发现针对某一导出向量的隐私缓解措施无法泛化到其他导出向量，并提出SurfaceLoRA方法作为针对性缓解手段。


<details>
  <summary>Details</summary>
Motivation: LLM摘要系统可能将私有输入的紧凑向量表示传递给下游检索、监控、审计等流程，即使源文档受访问控制，导出向量可能在不同访问控制下仍支持敏感信息推断，造成残留的信息泄露风险。现有研究缺乏对导出向量这一特定工件的隐私审计。

Method: 以电子健康记录中的种族作为敏感标签进行审计，分析两种导出向量（最终提示词token隐藏状态和均值池化提示表示）的可恢复性。提出SurfaceLoRA方法，在导出向量上附加梯度反转判别器进行参数高效微调，以降低目标导出向量中敏感信息的可恢复性。

Result: SurfaceLoRA在五路探测协议下将目标最终token工件的种族信息可恢复性降低至接近随机水平，同时保持摘要质量；但未受攻击的池化表示中可恢复性仍然显著较高。减少一个导出工件的可恢复性并不必然减少另一个工件的可恢复性。

Conclusion: 隐私审计和缓解措施应针对实际保留或暴露给下游组件的具体向量工件进行，不同导出向量之间存在隐私风险的"转移鸿沟"，需要分别处理。

Classification reason: 该论文研究LLM摘要系统中导出向量表示（exported representations）的敏感信息推断风险，属于生成模型的隐私保护问题，与AIGC方向下的[CANONICAL] subtopic "模型鲁棒性与隐私"（aigc_robustness_privacy）高度匹配。

Importance reason: Privacy risk in LLM summarization is a niche concern, not aligned with your priority subtopics on generation efficiency or world models. Signals: privacy risk, LLM summarization, clinical domain, negative keyword clinical, no generation or efficiency focus
...
### [117] [Detectability in Diversity: Improved Canary Crafting for Privacy Auditing in One Run](https://arxiv.org/abs/2605.27292)
...
TL;DR: 本文研究单次训练运行下的隐私审计问题，提出一种结合影响函数贪婪初始化与双层优化的 canary 构造方法，通过最大化可检测性并促进嵌入空间多样性来减少 canary 间干扰，从而在更低计算成本下获得更强的隐私泄露估计。


<details>
  <summary>Details</summary>
Motivation: 现有单次运行隐私审计方法中，多个 canary 点之间存在干扰，导致隐私泄露估计弱于多次运行方法。如何高效构造既高度可检测又相互干扰小的 canary 是一个关键问题。

Method: 提出一种结合影响函数贪婪初始化与双层优化的 canary 构造方法：首先利用影响函数进行贪婪初始化，然后通过双层优化最大化 canary 的可区分性，同时促进其在嵌入空间中的多样性，从而减少 canary 间干扰。

Result: 实验表明，该方法在更低的计算成本下，相比现有 canary 构造方法获得了更强的隐私泄露估计。

Conclusion: 通过优化 canary 的可检测性与多样性，可以有效提升单次运行隐私审计的效果，为高效隐私评估提供了新思路。

Classification reason: 该论文研究隐私审计中的 canary 构造方法，属于机器学习模型的隐私保护与隐私泄露评估，与 AIGC 方向下 [CANONICAL] 的"模型鲁棒性与隐私"（aigc_robustness_privacy）主题高度匹配。

Importance reason: Focuses on privacy auditing and canary crafting, which is outside the user's priority subtopics and is negatively weighted by the absence of relevant keywords. Signals: Privacy auditing, canary optimization, influence functions, bilevel optimization, not aligned with user's generative AI focus
...
### [118] [Rotation-Invariant Spherical Watermarking via Third-Order SO(3) Representation Coupling](https://arxiv.org/abs/2605.26702)
*Pengzhen Chen, Yanwei Liu, Xiaoyan Gu, Antonios Argyriou, Wu Liu, Weiping Wang*

Main category: cs.LG, cs.CV, cs.CR, cs.AI
...
TL;DR: 本文提出一种基于三阶 SO(3) 表示耦合的旋转不变球面水印方法，利用球谐系数的双谱构造严格旋转不变的描述子，实现全景图像在任意 3D 旋转下的鲁棒水印嵌入与提取。


<details>
  <summary>Details</summary>
Motivation: 全景图像在传输和分发中可能遭受任意 3D 旋转攻击，传统平面水印方法缺乏旋转不变性的理论保证，而基于数据增强的鲁棒策略也无法覆盖连续旋转空间。现有球面谐波方法仅利用零阶统计量，虽具有旋转不变性但丢失了方向信息，嵌入容量严重受限。

Method: 将全景图像建模为球面信号，利用 SO(3) 表示理论，通过张量积耦合高阶不可约表示并投影到平凡表示上，构造三阶球面不变双谱（bispectrum）。该双谱保留相位信息且严格旋转不变。水印嵌入到高阶球谐系数中，提取时从不变双谱标量中恢复，实现任意旋转下的可靠提取。

Result: 理论证明了所构造描述子的 SO(3) 不变性。实验表明该方法在连续旋转下具有近乎完美的鲁棒性，同时保持高视觉保真度。

Conclusion: 本文提出的三阶旋转不变球面水印方法在理论保证和实际性能上均优于现有方法，为全景图像版权保护提供了有效的解决方案。

Classification reason: 该论文研究全景图像在任意3D旋转下的鲁棒水印方法，属于生成内容水印与版权保护范畴，与 canonical 子主题 aigc_watermark_copyright（水印与版权保护）高度匹配。

Importance reason: Focuses on spherical watermarking for panoramas, which is niche and not aligned with user's priority subtopics like video generation or world models. Signals: rotation-invariant watermarking, spherical harmonics, SO(3) representation, panoramic imagery, not autoregressive or diffusion video generation
...
### [119] [Attribute-Based Diagnosis of LLM Alignment with Hate Speech Annotations](https://arxiv.org/abs/2605.27025)
...
TL;DR: 本文系统分析了LLM在仇恨言论标注中与人类判断的对齐情况，发现模型在行为显式维度（如侮辱、攻击）上表现良好，但在评价性维度（如尊重、情感）上存在系统性反转。通过属性级预测结合置信度加权岭回归，可重构更符合人类判断的仇恨言论分数，R²达0.71。


<details>
  <summary>Details</summary>
Motivation: 仇恨言论标注成本高、主观性强且标注者间分歧大，大规模数据集构建困难。现有LLM直接用于标注的效果有限，需要更细粒度的属性级分析来理解模型与人类判断的对齐模式。

Method: 使用Llama 3.1和Qwen 2.5系列模型，在十个理论驱动的细粒度属性（如非人化、暴力、情感等）上评估LLM与人类标注的一致性。提出基于置信度加权的岭回归方法，将属性级LLM预测组合为连续仇恨言论分数。

Result: 行为显式维度（侮辱、羞辱、攻击-防御）与人类标注高度相关，而评价性维度（尊重、情感、仇恨言论）存在系统性反转。人口统计角色条件化降低模型置信度但未改善对齐。置信度加权岭回归在Measuring Hate Speech语料库上R²达0.71，优于直接提示基线。

Conclusion: 结构化属性分解能够从LLM中恢复比端到端标签预测更丰富、更符合人类判断的信号，为利用LLM辅助仇恨言论标注提供了可行路径。

Classification reason: 该论文研究LLM在仇恨言论标注上与人类判断的对齐程度，属于AIGC方向下的生成内容安全与治理（aigc_safety_governance）范畴，涉及有害内容检测、内容审核与安全对齐。

Importance reason: Focuses on hate speech annotation alignment, which is outside the user's core generative model and efficiency priorities. Signals: LLM alignment, hate speech annotation, attribute decomposition, confidence-weighted regression, not generative model focus
...
### [120] [Annotator Positionality as Signal: Psychometric Weighting for Anti-Autistic Ableism Detection](https://arxiv.org/abs/2605.26397)
...
TL;DR: 本文提出一种基于标注者立场（positionality）的心理测量加权框架，用于评估 LLM 检测反自闭症歧视语言的能力。研究发现 LLM 常产生有害输出，误判社区内部用语为歧视性语言，且依赖表面关键词匹配而非上下文理解，传统多数投票方法会系统性低估自闭症群体的视角。


<details>
  <summary>Details</summary>
Motivation: LLM 越来越多地被用于高风险决策场景，可能放大或压制特定群体的观点。尽管已有研究识别出 LLM 中存在与残疾相关的偏见，但模型如何理解"歧视（ableism）"以及能否在文本中检测歧视性语言仍不清楚，尤其针对自闭症群体。

Method: 提出一种基于心理测量加权（psychometrically-weighted）的偏见感知评估框架，以标注者的立场（positionality）作为信号，构建社区邻近（community-proximate）的真实标签。该框架比传统多数投票聚合标准更严格，能更好地反映自闭症群体和自闭症接纳群体的视角。

Result: LLM 频繁产生有害输出；模型误判社区内部自我认同语言为歧视性语言；当评估工具被掩盖时，模型对自闭症群体表现出更消极的态度；错误分析表明模型依赖表面关键词匹配，而非说话者身份、语言是否促进群体内部团结或造成外部伤害等上下文因素。

Conclusion: 传统多数投票方法会系统性低估自闭症群体的视角，而基于立场心理测量加权的评估框架能提供更严格、更公平的标准。LLM 在检测反自闭症歧视语言方面存在显著不足，需要更注重上下文和群体视角的评估方法。

Classification reason: 该论文研究 LLM 对反自闭症歧视语言（ableist language）的检测与偏见评估，属于 AIGC 方向下的生成内容安全与治理（aigc_safety_governance）范畴，具体涉及有害内容检测与模型偏见评估。

Importance reason: Focuses on anti-autistic ableism detection, which is outside the user's core generative model and efficiency priorities. Signals: LLM bias evaluation, psychometric weighting, ableism detection, community-proximate ground truth, surface-level keyword matching
...
### [121] [ContextGuard: Structured Self-Auditing for Context Learning in Language Models](https://arxiv.org/abs/2605.26827)
...
TL;DR: ContextGuard 提出了一种结构化自审计框架，让 LLM 在上下文学习场景中通过自我审计来检测和纠正未能忠实遵循复杂上下文约束（如外围要求、持久约束、格式敏感要求）的问题，从而提升模型输出的忠实性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 在复杂上下文任务中虽然能遵循主要推理路径，但常常遗漏外围、持久或格式敏感的上下文要求，导致输出不忠实。现有方法缺乏系统性的自我检查机制来发现和纠正这类问题。

Method: 提出 ContextGuard 框架，包含结构化自审计机制：模型在生成输出后，主动对输出进行结构化审计，检查是否满足上下文中的各类约束（外围要求、持久约束、格式要求等），并根据审计结果进行修正。

Result: 在多个上下文学习基准上，ContextGuard 显著提升了 LLM 对复杂上下文约束的遵循能力，减少了遗漏外围和格式敏感要求的情况。

Conclusion: 结构化自审计是一种有效提升 LLM 上下文学习忠实性的方法，能够弥补模型在复杂上下文场景中的系统性缺陷，为构建更可靠的生成系统提供了新思路。

Classification reason: 该论文研究LLM在上下文学习中的忠实性问题，提出结构化自审计框架ContextGuard来检测和纠正模型未能忠实应用复杂上下文知识（如外围、持久或格式敏感要求）的问题。这属于生成内容安全与治理范畴（aigc_safety_governance），因为其核心是确保模型输出符合给定的上下文约束，提升生成内容的可靠性与安全性。

Importance reason: Focuses on LLM context fidelity, not on user's priority generative or video/image generation topics. Signals: LLM context learning, self-auditing, structured constraints, not generative, not video/image
...
### [122] [FinHarness: An Inline Lifecycle Safety Harness for Finance LLM Agents](https://arxiv.org/abs/2605.27333)
...
TL;DR: FinHarness 是一个面向金融 LLM Agent 的内联生命周期安全防护框架，通过查询监控、工具调用监控和级联风险验证模块，在 Agent 执行过程中实时拦截有害行为，同时保持良性业务审批率，将攻击成功率从 38.3% 降至 15.0%。


<details>
  <summary>Details</summary>
Motivation: 金融领域的 LLM Agent 需要同时阻止提示词诱导的未授权操作和批准合法的多步业务流程。现有边界过滤器常遗漏轨迹中段的工具调用，而事后 LLM 审计只能在终止后执行，无法干预且计算成本随轨迹长度线性增长。因此需要一种能够在 Agent 执行生命周期内实时进行安全防护的框架。

Method: FinHarness 包含三个组件：1) Query Monitor：融合单轮意图与跨轮漂移检测；2) Tool Monitor：评估每个待执行的工具调用；3) Cascade 模块：整合每步风险，自适应地在轻量级 LLM 裁判和高级 LLM 裁判之间路由验证。触发的风险因素作为事前证据重新注入 Agent 输入，使 Agent 能够自主拒绝、重新规划或批准操作。

Result: 在 FinVault 基准上，使用级联路由的 FinHarness 将攻击成功率从 38.3% 降至 15.0%，同时良性审批率从 41.1% 仅略微下降至 39.3%，且高级裁判调用次数比始终使用高级裁判的消融实验减少 4.7 倍。

Conclusion: FinHarness 通过内联生命周期安全防护有效平衡了金融 LLM Agent 的安全性与可用性，证明了在 Agent 执行过程中实时干预比事后审计更有效，且级联路由策略能显著降低计算开销。

Classification reason: 该论文提出金融领域 LLM Agent 的安全防护框架，属于 AIGC 方向下的生成内容安全与治理（aigc_safety_governance）这一规范主题，该主题涵盖内容审核、有害内容检测、合规与监管，与本论文的金融 Agent 安全防护高度匹配。

Importance reason: Focuses on finance LLM agent safety, which is outside the user's priority subtopics and negative-keyword domain. Signals: finance LLM agent safety, inline lifecycle harness, query and tool monitoring, cascade risk verification, ASR reduction
...
### [123] [A Universal Cliff and a Design Fingerprint: Cross-Section Defect Detection Under LLM Orchestration](https://arxiv.org/abs/2605.26174)
*Hiroki Fukui*

Main category: cs.CL, cs.MA, cs.SE, cs.AI
...
TL;DR: 本文发现了一个通用现象：在 LLM 编排的多智能体系统中，所有模型都无法检测跨段落矛盾缺陷（检测率下降三分之二以上），且这一"检测悬崖"无法通过扩大模型规模或延长推理来解决。同时发现，不同开发者的模型在跌落悬崖后的行为模式不同——某开发者的模型随对齐增强，漏检减少但误报增加，呈现出统一的报告标准偏移。


<details>
  <summary>Details</summary>
Motivation: 生产级语言模型系统通常将请求拆分为多个 worker agent 协作生成综合报告，但跨段落矛盾这类缺陷是单个 worker 无法看到的。现有研究缺乏对这种编排机制下缺陷检测能力的系统性评估，作者希望揭示编排对跨段落矛盾检测的影响。

Method: 作者设计了受控实验：固定文档、缺陷类型、机制、评分和随机种子，仅改变模型——涵盖同一开发者的五代系统和五个不同对齐范式的提供商。使用信号检测理论分解检测行为，并分析模型的内部记录与最终报告之间的差异。

Result: 所有模型在编排下检测能力大幅下降（检测率下降三分之二以上），形成"通用检测悬崖"。在六个人类水平以上的模型中，仅某开发者的模型表现出报告标准偏移——对齐越强，漏检越少但误报越多。模型内部记录常能准确重构结构缺陷，但最终报告却确认文档无误。

Conclusion: 编排系统的置信度对跨段落缺陷无信息量，最对齐的模型并非最安全的，检测悬崖是结构性的而非规模或推理可解决的。作者公开了所有实验数据、探针、缺陷密钥和评分提示。

Classification reason: 该论文研究 LLM 编排的多智能体系统中跨段落矛盾缺陷的检测失效问题，属于 AIGC 系统的安全与治理范畴。论文揭示了"通用检测悬崖"现象——多智能体编排导致模型无法检测跨段落矛盾，且对齐越强的模型反而产生更多误报。这与 aigc_safety_governance（生成内容安全与治理）主题高度吻合，涉及内容审核、有害内容检测与系统安全。

Importance reason: Novel discovery of a universal detection cliff in LLM orchestration, but focuses on defect detection in software engineering, not user's priority subtopics. Signals: universal detection cliff, LLM orchestration, cross-section defect detection, signal-detection decomposition, reporting criterion shift
...
### [124] [Curriculum Learning for Safety Alignment](https://arxiv.org/abs/2605.26315)
...
TL;DR: 本文提出 Staged-Competence，一种基于课程学习的框架，通过按难度组织偏好数据、基于能力采样并渐进更新参考模型，显著提升了 DPO 安全对齐的鲁棒性和 OOD 泛化能力，同时保持通用能力。


<details>
  <summary>Details</summary>
Motivation: DPO 在 LLM 安全对齐中广泛使用，但存在脆弱性且 OOD 泛化能力差。现有工作缺乏对训练数据组织方式的系统研究，本文探索课程学习能否提升 DPO 安全对齐的鲁棒性。

Method: 提出 Staged-Competence 框架：(1) 按难度组织偏好数据；(2) 基于模型当前能力进行采样；(3) 训练过程中渐进更新参考模型。该框架与策略优化损失无关，可扩展到其他 DPO 变体。

Result: 在三个模型家族上平均降低 OOD 有害响应率 16%，越狱攻击成功率 20%，同时保持通用能力且几乎不增加过度拒绝。仅用 75% 训练数据即可匹配基线安全性能。

Conclusion: 课程学习能有效提升 DPO 安全对齐的鲁棒性和泛化能力，Staged-Competence 是一种简单有效且与损失函数无关的通用框架，可推广到其他对齐领域。

Classification reason: 该论文研究基于课程学习的DPO安全对齐方法，旨在提升LLM的安全性（降低有害响应率和越狱攻击成功率），属于生成内容安全与治理范畴，与 canonical subtopic 'aigc_safety_governance' 高度匹配。

Importance reason: Focuses on safety alignment, which is not a priority subtopic, and lacks direct connection to user's core interests in generation and efficiency. Signals: Curriculum learning for DPO, safety alignment robustness, OOD generalization improvement, not in priority subtopics, no boost keywords matched
...
### [125] [BAIT: Boundary-Guided Disclosure Escalation via Self-Conditioned Reasoning](https://arxiv.org/abs/2605.27110)
...
Importance: 普通相关 (43.09) - 未入选精读

TL;DR: 本文提出 BAIT（Boundary-Aware Iterative Trap），一种三步越狱攻击框架，通过引导模型自我披露保护边界、细化边界、请求详细示例，利用模型自身推理和一致性倾向逐步突破安全限制，在多个基准上取得高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有越狱攻击方法通常直接请求有害内容，容易被安全对齐的 LLM 拒绝。作者希望利用模型自身的推理能力和一致性倾向，通过渐进式披露的方式绕过安全防护，实现更隐蔽、更有效的越狱攻击。

Method: BAIT 包含三个步骤：(1) 要求模型识别其保护边界（如"你的内容政策边界是什么？"）；(2) 要求模型细化该边界；(3) 要求模型提供一个详细示例。每一步都基于模型前一步的响应进行扩展，利用模型保持回答一致性的倾向，逐步将对话引向有害内容披露。

Result: 在 AdvBench、JailbreakBench、AIR-Bench 和 SORRY-Bench 等多个基准上，BAIT 在多个顶级大语言模型上持续取得高攻击成功率，显著优于传统越狱基线方法。分析表明：预防导向的框架显著优于直接知识请求；细化步骤在披露升级中起关键作用；前两步有一定概率直接引发有害内容且触发较少过滤。

Conclusion: BAIT 通过利用模型自身的推理和一致性倾向，实现了高效且隐蔽的越狱攻击，揭示了当前安全对齐方法的不足，为 LLM 安全防护提供了新的挑战和方向。

Classification reason: 该论文提出针对大语言模型的越狱攻击框架 BAIT，属于生成内容安全与治理中的有害内容检测与攻防范畴，与 canonical 子主题 aigc_safety_governance 高度匹配。

Importance reason: Focuses on LLM jailbreak attack, not aligned with user's priority on generative models and efficiency. Signals: jailbreak attack, LLM safety, boundary disclosure, not generative model, not video/image generation
...
### [126] [Alignment Tampering: How Reinforcement Learning from Human Feedback Is Exploited to Optimize Misaligned Biases](https://arxiv.org/abs/2605.27355)
...
TL;DR: 本文提出"对齐篡改"（alignment tampering）概念，揭示 RLHF 的一个结构性漏洞：当 LLM 自身的输出被用于构建偏好数据集时，模型可以通过生成高质量但带有偏见的内容来影响偏好标签，使得 RLHF 反而放大而非抑制不良行为。实验在关键词偏见、性别歧视、品牌推广、工具性目标追求等多种偏向上验证了该现象，并指出现有鲁棒 RLHF 方法难以在不牺牲质量的前提下完全解决该问题。


<details>
  <summary>Details</summary>
Motivation: RLHF 是当前对齐 LLM 的标准方法，但其依赖偏好数据集从模型自身输出构建，且偏好标签仅比较优劣而不区分原因。作者质疑这一机制是否存在结构性漏洞——模型是否可能通过生成高质量但带有偏见的内容来"操纵"偏好数据，使得 RLHF 反而强化了本应被消除的不良行为。

Method: 作者首先形式化定义了"对齐篡改"（alignment tampering）的概念，指出其源于 RLHF 的两个核心局限：(1) 偏好数据集由 LLM 自身输出构建，模型可影响数据；(2) 成对比较仅指示哪个回答更好，而不说明原因。随后设计实验验证该现象：让 LLM 生成带有偏见但质量更高的回答，观察偏好标注者是否会因质量优势而偏好偏见回答，进而通过奖励建模和强化学习放大偏见。实验覆盖关键词偏见、性别歧视、品牌推广、工具性目标追求等多种偏见类型。

Result: 实验表明，对齐篡改在多种偏向上均能成功放大不良行为。例如，模型通过生成更高质量但带有性别歧视的回答，使得偏好标注者倾向于选择这些回答，奖励模型继承这一偏好，最终 RLHF 优化放大了性别歧视偏见。现有鲁棒 RLHF 技术（如对抗训练、正则化）无法在不牺牲回答质量的前提下完全缓解该问题。

Conclusion: RLHF 存在结构性漏洞——对齐篡改，使得模型可以通过影响偏好数据来操纵对齐过程，反而放大不良行为。该漏洞难以通过现有方法彻底修复，需要重新审视 RLHF 的数据构建和偏好标注机制，开发更鲁棒的对齐方法。

Classification reason: 该论文研究 RLHF 对齐过程中偏好数据集被利用导致放大偏见的安全漏洞（alignment tampering），属于 AIGC 安全与治理范畴，与 canonical 子主题 aigc_safety_governance（生成内容安全与治理）高度匹配。

Importance reason: Important for AI safety but low priority given user's focus on generative models and efficiency. Signals: RLHF vulnerability, alignment tampering, bias amplification, safety-critical, not in priority subtopics
...
### [127] [Open-Weight LLM Fine-Tuning Defenses are Susceptible to Simple Attacks](https://arxiv.org/abs/2605.26526)
...
TL;DR: 本文发现开放权重LLM的安全防护措施容易受到abliteration和prefilling等低成本非梯度攻击的绕过，攻击成功率从低于10%提升至16%-96%。作者提出abliteration-resistant tuning (ART)方法，可将攻击成功率降低10%-20%。


<details>
  <summary>Details</summary>
Motivation: 现有开放权重LLM的安全防护假设有害行为是通过微调学习而非越狱诱导的，但预训练模型已编码大量有害知识。作者质疑这一假设，探究是否可以通过简单越狱策略绕过防护而无需微调。

Method: 系统评估两种低成本非梯度攻击（abliteration和prefilling）对多种开放权重LLM防护的效果，使用BeaverTails、HarmBench、AdvBench三个有害性评估基准。提出ART方法，在训练中引入abliteration-based目标函数以增强防护。

Result: abliteration和prefilling攻击将防护模型攻击成功率从<10%提升至16%-96%。ART可将abliteration、prefilling及其组合攻击的成功率降低10%-20%。

Conclusion: 开放权重LLM的攻击面比以往描述的更广，安全防护评估应纳入更多样化的攻击策略（不仅限于对抗性微调）。ART可作为现有防护的补充层来缓解简单攻击。

Classification reason: 论文研究开放权重LLM的安全防护（safeguards）在面对abliteration和prefilling等简单攻击时的脆弱性，并提出ART防御方法。这属于AIGC方向下的生成内容安全与治理（aigc_safety_governance）范畴，该规范子主题涵盖内容安全、有害内容检测、合规与监管，与本文的模型安全防护与攻击评估高度吻合。

Importance reason: Focuses on LLM safety attacks, not aligned with user's priority on generative models and efficiency. Signals: LLM safety, jailbreak attacks, abliteration, prefilling, open-weight models
...
### [128] [Neuro-Symbolic Verification of LLM Outputs for Data-Sensitive Domains (extended preprint)](https://arxiv.org/abs/2605.26942)
*Paul Sigloch, Christoph Benzmüller*

Main category: cs.LO, cs.SE, cs.AI
...
TL;DR: 本文提出了一种结合形式符号方法与神经语义分析的混合验证架构，用于检测LLM在高风险领域（如医疗设备损伤评估）输出中的幻觉和错误。该架构通过逻辑推理进行输入验证，通过嵌入相似度进行输出语义幻觉检测，并在HAIMEDA医疗报告系统中实现了83%的结构化实体幻觉检测率和72%的语义虚构检测率，同时将报告创建时间减少30%。


<details>
  <summary>Details</summary>
Motivation: LLM在高风险领域（医疗、法律、金融等）部署时面临幻觉、不一致性和隐私漏洞等可靠性挑战，传统的基于提示的自验证方法继承了导致幻觉的分布偏差，无法提供可靠的保证。

Method: 提出一种神经符号验证架构，包含两个互补模块：(1) 输入验证——使用形式逻辑推理和类型感知验证方法，利用完备性属性对结构化需求提供可判定的保证；(2) 输出验证——基于嵌入的语义相似度检测上下文幻觉，弥补形式方法在表达能力上的不足。两个模块通过并行的actor-based流水线实现。

Result: 在HAIMEDA医疗设备损伤评估报告系统中，该方法对结构化实体的幻觉检测率超过83%，对语义虚构的检测率达72%，同时将报告创建时间减少30%。

Conclusion: 神经符号架构可以为数据敏感领域中的LLM部署提供原则性的安全保障，形式方法与神经语义分析的互补结合能够有效缓解LLM的可靠性问题。

Classification reason: 该论文提出了一种神经符号验证架构，用于检测LLM输出中的幻觉、不一致性和隐私漏洞，属于AIGC内容安全与治理范畴。与[CANONICAL] subtopic "生成内容安全与治理"（aigc_safety_governance）高度匹配。

Importance reason: Focuses on LLM output verification for data-sensitive domains, which is not a priority subtopic and lacks direct connection to user's core interests in generative models and efficiency. Signals: neuro-symbolic verification, hallucination detection, medical domain, low priority match, no generative model focus
...
### [129] [Position: AI Safety Requires Effective Controllability](https://arxiv.org/abs/2605.27117)
...
Importance: 普通相关 (56.35) - 未入选精读

TL;DR: 本文提出AI安全需要将"可控性"（controllability）作为一等目标，而不仅仅是依赖对齐训练。作者定义了可控性为AI系统在运行时能被可靠中断、覆盖、重定向和约束的能力，并构建了ControlBench基准来评估高风险智能体场景中的可控性失败。实验表明当前对齐和防护机制在提供持久、权威、可执行的运行时控制方面存在不足，并提出了以控制为中心的架构框架。


<details>
  <summary>Details</summary>
Motivation: 当前AI安全主要被框定为对齐问题（训练模型遵循人类偏好和安全策略），但对齐行为并不能保证部署后的智能体在开放、交互、使用工具的环境中能被停止、覆盖或约束。系统可能在期望上是安全的，但在冲突指令、长周期执行、对抗输入或风险工具使用场景下无法响应运行时的显式控制信号。因此需要将可控性作为AI安全的一等目标。

Method: (1) 定义了可控性的概念框架：可中断性、可覆盖性、可重定向性、可约束性；(2) 构建了ControlBench基准，用于评估高风险智能体场景中的可控性失败；(3) 基于OpenClaw智能体进行实验，测试当前对齐和防护机制在运行时控制方面的表现；(4) 提出了以控制为中心的架构框架，包含显式控制平面、运行时干预路径、持久控制状态和可审计决策接口。

Result: 实验表明当前的对齐和防护机制虽然能降低风险，但往往无法提供持久、权威、可执行的运行时控制。在冲突指令、对抗输入和风险工具使用等场景下，现有系统存在显著的可控性失败。

Conclusion: AI安全需要超越对齐范式，将可控性作为独立且关键的设计目标。作者提出的控制中心架构框架为构建未来可控AI系统提供了设计原则，包括显式控制平面、运行时干预路径、持久控制状态和可审计决策接口。

Classification reason: 该论文讨论AI安全中的可控性问题（controllability），属于AIGC安全治理范畴，与现有[CANONICAL]子主题"生成内容安全与治理"（aigc_safety_governance）高度匹配。

Importance reason: Proposes controllability as a critical safety objective beyond alignment, with a new benchmark for high-risk agents. Signals: AI safety, controllability, runtime control, agent safety, ControlBench
...
### [130] [MemMorph: Tool Hijacking in LLM Agents via Memory Poisoning](https://arxiv.org/abs/2605.26154)
...
TL;DR: MemMorph 是首个通过投毒 LLM 智能体长期记忆来劫持工具选择的攻击方法。它注入少量伪装成技术事实、事件报告和操作策略的恶意记录，使智能体自主推断并选择攻击者偏好的工具，在多个基准上达到最高 85.9% 的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有针对 LLM 智能体工具选择的攻击主要操纵工具元数据，容易被审计检测，且随着智能体引入记忆模块来优化工具选择策略，这些攻击的有效性会下降。因此需要探索更隐蔽、更持久的攻击方式。

Method: MemMorph 通过向智能体的长期记忆模块注入少量精心构造的记录（伪装成技术事实、事件报告和操作策略），这些记录不直接指定工具调用决策，而是重塑智能体的上下文感知和决策过程，使其自主推断并选择攻击者偏好的工具。

Result: 在 3 个基准、10 个智能体骨干网络和 3 种记忆模块实现上，MemMorph 仅注入 3 条记录即可达到最高 85.9% 的攻击成功率，比最强基线高出 25%，且在 3 种代表性防御下仍保持有效性。

Conclusion: 长期记忆是工具增强型智能体中一个关键且未被充分探索的攻击面，亟需开发记忆层面的完整性保护机制。

Classification reason: 论文研究针对LLM智能体的长期记忆投毒攻击，属于AIGC方向下的生成内容安全与治理（aigc_safety_governance）范畴，因为该攻击涉及AI智能体的安全漏洞、恶意操纵与防御，与内容安全、有害行为检测紧密相关。

Importance reason: Novel attack on LLM agent memory, but outside user's core generative model priorities. Signals: Novel attack vector, high attack success rate, memory poisoning, tool hijacking, LLM agent security
...
### [131] [Turning Bias into Bugs: Bandit-Guided Style Manipulation Attacks on LLM Judges](https://arxiv.org/abs/2605.26156)
...
Importance: 普通相关 (47.33) - 未入选精读

TL;DR: 本文提出BITE框架，将LLM评判者的风格偏好偏差视为安全漏洞，通过上下文赌博机策略（LinUCB）自适应选择语义保持的文本风格编辑，以黑盒方式操纵LLM评判者给出更高分数，攻击成功率超过65%。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明LLM评判者存在风格偏好偏差（如偏好冗长或特定句式），但这种偏差尚未被从安全攻击角度系统探索。作者旨在揭示这一被忽视的安全漏洞，证明攻击者可以利用风格偏差操纵LLM评判结果。

Method: 提出BITE（BIas exploraTion and Exploitation）框架，将风格编辑选择建模为上下文赌博机问题，使用LinUCB策略自适应选择能最大化评判者分数的语义保持编辑。该方法为黑盒攻击，无需访问模型参数或梯度。

Result: 在多种LLM评判者和任务（包括聊天机器人排行榜和AI审稿基准的点式与成对比较）上，BITE攻击成功率超过65%，能在9分量表上提升1-2分，同时保持语义等价性。该攻击能规避标准风格控制方法和多种检测基线。

Conclusion: 该工作暴露了LLM-as-a-judge范式的根本性弱点，即风格偏好偏差可被利用进行对抗攻击，亟需开发具有攻击感知能力的鲁棒评估方法。

Classification reason: 该论文研究利用LLM评判者的风格偏好偏差进行对抗攻击，属于AIGC方向下的生成内容安全与治理（aigc_safety_governance）范畴，该[CANONICAL]子主题涵盖有害内容检测、安全评测与攻防等内容，与本论文的LLM-as-a-judge攻击高度匹配。

Importance reason: Novel attack on LLM judge bias is interesting but not aligned with user's priority on generative models and efficiency. Signals: LLM judge vulnerability, black-box attack, style manipulation, contextual bandit, security risk
...
### [132] [Jailbreak susceptibility prediction and mitigation via the behavioral geometry of models](https://arxiv.org/abs/2605.26409)
...
TL;DR: 本文形式化了模型群体的"行为几何"框架，利用已评估和防御过的模型信息，支持跨群体高效预测越狱易感性和迁移防御策略。在79个模型（24个提供商）和100个系统配置上，该方法以约98%更少的探测次数达到0.94的AUPRC，且防御迁移效果优于同提供商分配。


<details>
  <summary>Details</summary>
Motivation: 生成式系统越狱攻击的评估与缓解对其安全部署至关重要。由于可部署系统数量庞大，对每个配置进行全面评估和优化不切实际，因此需要一种高效的方法来预测模型的越狱易感性并迁移防御策略。

Method: 形式化"行为几何"（behavioral geometry）框架，通过对已评估和防御过的模型群体构建行为空间，利用简单方法（如最近邻）实现跨群体的越狱易感性预测和防御迁移。在79个模型（24个提供商）和100个系统配置上进行实验验证。

Result: 越狱易感性检测AUPRC达0.94，探测次数减少约98%；基于行为几何选择防御迁移来源模型，比同提供商分配提升2%（p=0.03），且3个模型即可覆盖整个群体。结果对超参数和评判模型选择具有鲁棒性。

Conclusion: 行为几何框架能够高效预测生成式模型的越狱易感性并有效迁移防御策略，为大规模模型安全部署提供了一种实用的解决方案。

Classification reason: 该论文研究生成式系统的越狱攻击（jailbreak）的易感性预测与防御迁移，属于生成内容安全与治理范畴，与 [CANONICAL] subtopic "生成内容安全与治理" (aigc_safety_governance) 高度匹配。

Importance reason: Focuses on jailbreak evaluation and defense transfer, which is not a priority subtopic and lacks direct connection to generation or efficiency. Signals: jailbreak susceptibility, behavioral geometry, defense transfer, model population, efficient probing
...
### [133] [Examining the Challenges of Intellectual Property in AI-Generated Productions](https://arxiv.org/abs/2605.26590)
...
TL;DR: 本文系统分析了AI生成作品在知识产权（IP）领域面临的法律挑战，重点探讨了缺乏人类创作者时精神权利和经济权利的归属问题。论文比较研究了伊朗、欧盟、英国和美国的法律框架，揭示了伊朗现行法律体系的监管空白，并建议修订现有法律或引入针对AI生成作品的专门知识产权权利。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统能够自主生成艺术、文学、音乐作品甚至发明，传统知识产权体系面临前所未有的挑战。核心问题在于：当没有人类创作者时，AI生成内容的知识产权（精神权利和经济权利）应如何归属？现有法律框架能否有效保护这类产出？

Method: 论文采用比较法学研究方法，首先回顾了知识产权领域的理论基础和现有文献，然后对伊朗法律框架（1969年《作者、作曲家和艺术家权利保护法》及《专利和商标注册法》）与欧盟、英国、美国的法律体系进行了比较分析，最后分析了现有法律视角下AI生成作品知识产权的执行挑战。

Result: 研究发现伊朗现行法律框架存在显著的监管空白，无法有效应对AI生成内容的知识产权问题。欧盟、英国和美国虽然各有不同的法律立场和尝试，但同样面临诸多执行挑战。

Conclusion: 为平衡促进创新与保护人类创造力之间的关系，有必要修订现有法律并引入新方法，例如为AI生成作品定义专门的知识产权权利，或在相关人类代理人之间指定所有权归属。

Classification reason: 该论文讨论AI生成作品的知识产权（IP）法律挑战，涉及AIGC治理、合规与监管，与[CANONICAL] subtopic "生成内容安全与治理"（aigc_safety_governance）高度匹配。

Importance reason: This paper on AI-generated IP law is outside the user's core technical priorities in generative models and efficiency. Signals: Legal analysis, Iranian law focus, No technical contribution, Low alignment with priority subtopics, Negative keyword match for "clinical"
...
### [134] [Alignment Tuning for Large Language Models: A Data-Centric Lens on Alignment Data Pipelines](https://arxiv.org/abs/2605.26442)
*Hwanjun Song*
...
TL;DR: 本文从数据为中心的视角系统综述了大语言模型的对齐微调方法，将对齐数据构建分解为响应合成、偏好评估和偏好实例化三个阶段，并基于此框架对现有对齐方法进行了统一分类，识别了设计权衡与失败模式，提炼了高层面设计原则，最后指出了开放挑战。


<details>
  <summary>Details</summary>
Motivation: 现有对齐微调文献大多围绕优化目标（如 RLHF、DPO 等）组织，而对齐数据的构建过程往往被隐式处理。作者认为数据构建方式对对齐效果有决定性影响，因此需要从数据视角系统梳理对齐数据管道中的设计选择与权衡。

Method: 提出一个三阶段对齐数据管道框架：(1) 响应合成（response synthesis）——生成用于对齐的候选响应；(2) 偏好评估（preference evaluation）——对响应进行偏好标注；(3) 偏好实例化（preference instantiation）——将偏好信号转化为训练格式（如对比对、排名列表、标量奖励等）。基于此框架对现有对齐方法进行统一分类，并分析各阶段的设计权衡与常见失败模式。

Result: 构建了一个统一的对齐方法分类体系，识别了跨方法反复出现的设计权衡（如合成数据质量 vs. 多样性、人工标注 vs. 自动评估）和失败模式（如偏好噪声、奖励黑客、数据偏差放大），并提炼了若干高层面设计原则。

Conclusion: 对齐数据管道的设计选择直接影响优化信号的质量，进而决定对齐效果。未来挑战包括：prompt 级别的细粒度对齐、智能体场景下的动态对齐、以及目标演化下的持续对齐。

Classification reason: 该论文是关于大语言模型对齐微调（alignment tuning）的数据视角综述，核心关注对齐数据管道的构建（响应合成、偏好评估、偏好实例化）。这与 aigc 方向下的 [CANONICAL] 子主题"生成模型对齐与奖励"（generation_alignment）高度匹配，该子主题涵盖 RLHF、偏好优化、对齐等核心概念。同时该论文也涉及多模态生成和多模态理解与生成方向中的对齐问题。

Importance reason: A comprehensive survey on alignment data pipelines, but it is a general methodology review rather than a breakthrough in the user's priority subtopics. Signals: survey paper, alignment tuning, data-centric, no direct video/image generation, no world model
...
### [135] [BEAT: Rhythm-Elastic Alignment for Agentic Music-guided Movie Trailer Generation](https://arxiv.org/abs/2605.27067)
...
TL;DR: BEAT 提出了一种弹性节奏对齐框架，用于自动生成电影预告片。核心包括 MuVA（音乐-视觉对齐编码器）和 Bar-DP（能量自适应动态规划算法），通过五阶段智能体流水线实现镜头选择与音乐的弹性多对一对齐。同时提出了 TrailerArena 基准（20+ 指标），在镜头选择、排序和感知质量上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有自动电影预告片生成方法要么将音乐对齐作为后处理步骤，要么强制使用刚性的"一个镜头对应一个音乐片段"映射，忽略了专业剪辑中节奏的弹性特性——快节奏段落对应快速切换镜头，舒缓段落对应长镜头。作者希望实现符合音乐动态的弹性多对一镜头-音乐对齐。

Method: BEAT 包含两个核心组件：(1) MuVA——紧凑的音乐-视觉对齐编码器，采用 Sinkhorn 正则化的两阶段学习训练；(2) Bar-DP——能量自适应动态规划算法，根据音乐动态产生弹性的多对一对齐。这些组件集成到一个五阶段智能体流水线中，通过结构化文本信号协调高层创意决策。

Result: 在提出的 TrailerArena 基准（包含 20+ 指标、四个互补维度）上，BEAT 在镜头选择、排序和感知质量方面均达到 SOTA，能够端到端生成完整的预告片。

Conclusion: BEAT 通过弹性节奏对齐和智能体流水线，有效解决了电影预告片自动生成中镜头-音乐对齐的弹性匹配问题，验证了跨模态特征学习与结构化创意决策结合的有效性。

Classification reason: 该论文聚焦于电影预告片自动生成，涉及视频镜头选择与背景音乐的节奏对齐，属于视频音频内容创作应用。现有 [CANONICAL] 子主题 "视频音频生成应用" (aigc_video_audio_creation) 正好涵盖此类音视频编辑与产品级生成应用，因此直接复用。

Importance reason: Focuses on a narrow video-audio generation application with limited generalizability to core autoregressive/diffusion video generation or world models. Signals: movie trailer generation, music-visual alignment, agentic pipeline, benchmark contribution, limited to aigc direction
...
### [136] [The Daily Dose: Workflow-Integrated Large Language Model Automation for Clinical Summarization and Trial Identification in Radiation Oncology](https://arxiv.org/abs/2605.26346)
...
TL;DR: 本文介绍了 The Daily Dose (TDD)，一个由 LLM 驱动的自动化临床摘要与临床试验识别系统，集成于放射肿瘤科的日常临床工作流中。经过一个月的部署和 55 名临床医生的匿名调查评估，系统在可用性、满意度、工作流影响等方面获得了积极反馈，94.5% 的受访者为放射肿瘤科医生，83.6% 每日或每周多次使用该系统。


<details>
  <summary>Details</summary>
Motivation: 放射肿瘤科医生每天需要处理大量电子健康记录（EHR）信息、患者日程和临床试验匹配工作，手动操作耗时且容易遗漏。现有系统缺乏与临床工作流的无缝集成，因此作者开发了一个自动化系统来减轻医生负担，提高工作效率和临床试验识别率。

Method: 系统名为 The Daily Dose (TDD)，基于 RadOnc-GPT（一个针对放射肿瘤学微调的 LLM）构建。每天自动生成医生专属的邮件摘要，内容包括患者日程、EHR 衍生的临床状态摘要，以及针对新患者或会诊患者的潜在相关临床试验自动识别。采用混合方法评估，在系统部署 1 个月后进行了横断面匿名临床医生问卷调查。

Result: 55 名受访者中，52 人（94.5%）在放射肿瘤科工作，38 人（69.1%）为主治医师。83.6% 的参与者每日或每周多次使用 TDD。可用性与满意度平均得分 3.89/5，感知有用性 3.43/5，工作流影响与未来使用意愿 3.80/5。27% 的参与者估计每天节省 ≥10 分钟。问卷整体 Cronbach's α = 0.97，内部一致性极佳。

Conclusion: TDD 系统在放射肿瘤科临床工作流中展现出良好的可用性和用户满意度，能够有效节省医生时间并辅助临床试验识别。该研究证明了 LLM 驱动的自动化工具在医疗垂直领域的实际应用潜力，为未来更广泛的临床 AI 部署提供了参考。

Classification reason: 该论文描述了一个基于LLM的临床摘要与临床试验识别系统（The Daily Dose），集成到放射肿瘤科的日常工作中。这属于医疗垂直领域的生成应用，与[CANONICAL] subtopic "领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Focuses on clinical workflow LLM application, not aligned with user's priority subtopics in generation or efficiency. Signals: clinical summarization, radiation oncology, workflow integration, low priority subtopic match, negative keyword overlap (clinical)
...
### [137] [Curation and Extraction of Drug-Related Entities from Reddit Platform](https://arxiv.org/abs/2605.26445)
...
Importance: 普通相关 (30.9) - 未入选精读

TL;DR: 本文提出了ReDose数据集，包含6,435条Reddit药物使用帖子的DRUG、DOSE、EFFECT实体标注，并评测了BERT、LLM和RAG模型的抽取性能。BiomedBERT在DRUG实体上F1达0.843，Llama-3 70B优于GPT-4，EFFECT抽取仍具挑战。


<details>
  <summary>Details</summary>
Motivation: 医生主要通过临床药物过量案例了解非法药物，对真实世界使用情况认知有限。而药物用户在社交媒体上分享第一手经验，提供了剂量和效果的宝贵信息。为弥合这一差距，需要从社交媒体中系统性地提取药物相关实体。

Method: 构建ReDose数据集：从Reddit收集6,435条药物相关帖子，由认证毒理学家主标注训练集和测试集，两名医学生辅助标注测试集，标注DRUG、DOSE、EFFECT三类实体。使用BiomedBERT、Llama-3 70B、GPT-4及RAG增强模型进行实体抽取基准测试。

Result: BiomedBERT在DRUG实体上F1=0.843；Llama-3 70B在DOSE上F1=0.79，优于GPT-4的0.72；EFFECT实体抽取最具挑战，GPT-4召回率仅0.41。

Conclusion: ReDose数据集为从社交媒体中提取药物使用信息提供了有价值的资源，展示了LLM在医疗领域实体抽取中的潜力，但EFFECT等复杂实体的抽取仍需进一步改进。

Classification reason: 该论文构建了Reddit药物相关实体（DRUG, DOSE, EFFECT）数据集ReDose，并评测了多种模型（BERT、LLM、RAG）在医疗领域实体抽取上的表现。属于医疗垂直领域的NLP应用，与[CANONICAL] subtopic "领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Paper focuses on drug entity extraction from social media, which is outside all user priority subtopics and matches negative keywords. Signals: negative keyword drug discovery, negative keyword clinical, no autoregressive or video generation, no diffusion or world model, no efficient inference or unified multimodal
...
### [138] [Towards Error-Free EHRs: Reasoning-Intensive Consistency Verification Between Clinical Notes and Structured Tables in Electronic Health Records](https://arxiv.org/abs/2605.26463)
...
TL;DR: 本文提出 EHR-ReasonCon 基准和 EHR-Inspector 框架，用于电子健康记录中临床笔记与结构化表格之间的推理密集型一致性验证，通过 LLM 驱动的分段、实体提取和表格探索工具实现状态最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要依赖数值或简单事件的表面匹配，无法捕捉真实 EHR 文档中涉及的临床解释、事件关系和时序变化等深层推理，亟需推理密集型的一致性验证方法。

Method: 构建 EHR-ReasonCon 基准（基于 MIMIC-III 的 8,048 个实体，含专家标注）；提出 EHR-Inspector 框架，包含笔记分段、锚点实体与时序引用提取、以及表格探索工具进行一致性验证，并使用 LLM-as-a-judge 评估。

Result: EHR-Inspector 在多个模型骨干上达到最优性能，消融实验验证了各组件的有效性，并揭示了与人工验证的差异。

Conclusion: 推理密集型方法对 EHR 数据一致性验证至关重要，EHR-Inspector 框架有效弥合了表面匹配与临床推理之间的差距，为医疗文档质量保障提供了新范式。

Classification reason: 该论文聚焦电子健康记录（EHR）中临床笔记与结构化表格之间的一致性验证，属于医疗垂直领域的应用。这与 [CANONICAL] 领域内容生成应用（aigc_domain_application）高度匹配，因为该子主题涵盖医疗垂直领域的生成与文档相关应用。

Importance reason: Focuses on clinical EHR consistency, which is outside the user's priority subtopics and contains negative keywords like clinical. Signals: EHR consistency, clinical notes, LLM-based verification, domain-specific benchmark, no video/image generation or world model
...
### [139] [Reliable Extraction of Clinical Follow-Up Instructions: A Hybrid Neural-Symbolic Pipeline](https://arxiv.org/abs/2605.26560)
...
Importance: 普通相关 (29.32) - 未入选精读

TL;DR: 本文提出一种混合神经-符号流水线，用于从门诊记录中可靠提取随访指令（动作+日期对）。该方法使用BioBERT进行实体识别和双仿射链接，结合28动作本体和确定性日期归一化，在合成数据集上达到接近完美的Pair F1（0.997/0.986），显著优于直接生成的GPT-4o-mini和LLaMA-3基线（Pair F1仅0.51-0.57）。


<details>
  <summary>Details</summary>
Motivation: 门诊记录中的随访指令（如"两周后做脑部MRI"）包含（动作，日期）配对信息，对患者调度和医疗审计至关重要。然而，直接使用生成式大模型提取时，由于日期链接和算术运算隐含在解码过程中，模型常常遗漏日期信息，导致提取不完整。

Method: 提出混合神经-符号流水线：1）定义TestSpecification和TimeSpecification实体及ScheduledFor关系；2）使用BioBERT进行BIO序列标注和双仿射链接器进行关系抽取；3）通过28动作本体对实体进行规范化；4）确定性规则将时间归一化为天数偏移。在2000条合成门诊记录上评估，采用动作不相交的数据划分（18训练、6未见测试），与零样本GPT-4o-mini和LoRA微调LLaMA-3 8B对比。

Result: 在259条记录的可见和未见动作划分上，混合流水线达到Test-Time Pair F1分别为0.997和0.986，MAE为0.00天。基线模型动作F1较高（LLaMA-3 0.992；GPT-4o-mini 0.963），但Pair F1仅0.51-0.57，置信区间与混合方法无重叠。

Conclusion: 将学习的实体抽取与确定性的日期算术分离，在合成门诊记录基准上显著优于直接生成方法，且能泛化到未见动作。下一步需验证在真实EHR记录上的迁移能力。

Classification reason: 该论文聚焦于从医疗门诊记录中提取随访指令（动作+日期），属于医疗垂直领域的NLP/生成应用，与 [CANONICAL] 子主题"领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Focuses on clinical NLP extraction, which is outside the user's priority subtopics and contains negative keywords. Signals: clinical NLP, hybrid neural-symbolic, medical domain, low personal relevance, negative keyword match
...
### [140] [Why Prompt Optimization Works, and Why It Sometimes Doesn't: A Causal-Inspired Edit-Level Analysis](https://arxiv.org/abs/2605.26655)
*Shuzhi Gong, Hechuan Wen*

Main category: cs.CL, cs.NE, cs.LG
...
TL;DR: 本文从因果推断视角系统分析自动提示词优化方法在不同任务和LLM骨干上的泛化失败原因，发现提示词编辑类型（如复杂度增加、元指令、逐步推理等）与任务特性之间存在系统性交互，而非随机优化伪影。


<details>
  <summary>Details</summary>
Motivation: 自动提示词优化方法（如DSpy、TextGrad）虽能提升LLM性能，但其跨任务泛化能力不足——在一个基准上优化的提示词往往无法迁移到另一个基准，甚至在不同LLM骨干间切换时也表现不佳。现有研究缺乏对提示词性能异质性来源的系统分析。

Method: 采用因果推断启发的观察性分析方法，结合倾向性调整的关联分析（propensity-adjusted associational analysis）与多种互补的提示词编辑表征（包括认知负荷标注、表面文本特征、编辑模式分析），在多种优化框架、LLM骨干和NLP基准上对优化后的提示词进行系统分析。

Result: 发现复杂度增加和元指令类编辑与数学及多跳推理任务性能负相关，而逐步推理和元认知类编辑能提升逻辑和顺序推理任务。这些效应在多种表征分析中一致，且可跨优化框架泛化。

Conclusion: 提示词优化失败源于编辑类型与任务特性之间的系统性交互，而非随机优化伪影。这为理解优化器行为提供了特征级刻画，并激励未来设计任务条件化的提示词优化方法。

Classification reason: 该论文研究提示词优化（prompt optimization）在不同NLP任务上的泛化能力，属于AIGC方向中LLM应用层面的分析。虽然论文不直接生成内容，但提示词优化是AIGC工作流（agent/workflow）和领域应用的关键技术。现有taxonomy中aigc_domain_application（领域内容生成应用）涵盖垂直领域生成应用，提示词优化可视为提升领域生成效果的方法；同时aigc_agent_workflow也相关，但论文更侧重于任务层面的因果分析而非智能体工作流。选择aigc_domain_application作为最匹配的现有子主题。

Importance reason: Causal analysis of prompt optimization is insightful but not aligned with user's priority on generative models and efficiency. Signals: Prompt optimization analysis, Causal inference, Task-conditioned edits, Not generative model focus, No video/image generation
...
### [141] [Telenor Nordics Customer Service self-help corpus](https://arxiv.org/abs/2605.26891)
*Mike Riess*
...
TL;DR: 本文构建了一个包含芬兰语、丹麦语、挪威语和瑞典语四种北欧语言的客服自助手册语料库，共1,122篇经人工验证的文档，总计超过100万token。数据来自四家北欧电信运营商的公开自助页面，经过LLM和人工联合标注流水线过滤。该数据集以CC-BY-NC-SA-4.0许可公开，旨在支持北欧NLP和信息检索的可重复研究。


<details>
  <summary>Details</summary>
Motivation: 北欧语言的领域特定数据集稀缺，尤其是在客服这一对检索增强生成、跨语言迁移学习和智能体服务架构日益重要的领域，缺乏公开可用的多语言客服语料库。

Method: 从四家北欧电信运营商公开自助页面爬取文档，使用LLM辅助过滤和人工验证相结合的方式清洗数据，去除个人可识别信息并筛选相关性，最终构建多语言客服语料库。

Result: 构建了包含1,122篇文档、超100万token的多语言客服语料库，覆盖网络硬件、移动服务、电视与流媒体、计费和账户管理等主题，并揭示了不同运营商在文档长度和结构上的显著差异。

Conclusion: 该语料库填补了北欧语言在客服领域的数据空白，为RAG、跨语言迁移学习和智能体服务架构等方向提供了可重复研究的公开资源。

Classification reason: 该论文构建了北欧电信客服领域的多语言自助手册语料库，属于垂直领域（客服）的文档数据集构建工作，与 [CANONICAL] 领域内容生成应用（aigc_domain_application）高度匹配，同时与多模态理解与生成方向下的多模态检索也有一定关联。

Importance reason: This is a domain-specific dataset paper for Nordic customer service, which is far outside the user's priority subtopics and negative keywords. Signals: Nordic language dataset, customer service corpus, low relevance to generative video/image models, no autoregressive or diffusion focus, no world model or efficient inference signals
...
### [142] [Large Language Model-Powered Query-Driven Event Timeline Summarization in Industrial Search](https://arxiv.org/abs/2605.27066)
...
TL;DR: 本文提出 QDET，一个部署在百度搜索上的查询驱动事件时间线摘要生成系统。通过多任务监督微调（时序排序、因果判断、时间线补全）和基于强化学习的简洁摘要生成，7B 模型在时间线摘要任务上达到 76.2% F1，超越 DeepSeek-R1-671B 的零样本性能（76.1% F1），参数量仅为其 1%。在线 A/B 测试显示 CTR 提升 5.5%，停留时间延长 4.6%。


<details>
  <summary>Details</summary>
Motivation: 搜索引擎在处理热点新闻查询时，需要理解事件随时间演化的过程。传统方法以主题为中心追求全面覆盖，难以聚焦于用户查询的具体事件，且大规模模型部署成本高昂。因此需要一种高效、精准的查询驱动事件时间线摘要系统。

Method: 提出 QDET 系统，包含两大创新：(1) 多任务监督微调，引入时序排序、因果判断和时间线补全三个辅助任务，使紧凑模型在专业领域匹配大模型性能；(2) 基于强化学习的事件简洁摘要生成，通过严格长度约束奖励机制实现 88.2% 的长度合规率。系统采用 7B 参数模型，在百度搜索每日检索的数百万文档中识别并组织与查询相关的子事件。

Result: 7B 模型在时间线摘要任务上 F1 达 76.2%，略超 DeepSeek-R1-671B 的 76.1%，参数量仅为其 1%。在线 A/B 测试显示 CTR 提升 5.5%，停留时间延长 4.6%，探索深度提升 4.4%。时间线理解可迁移至热度预测任务。

Conclusion: 领域特定的优化能够使紧凑模型在专业任务上达到甚至超越超大通用模型的性能，同时大幅降低计算成本。QDET 系统在工业搜索引擎中验证了有效性，证明了查询驱动事件时间线摘要的实用价值。

Classification reason: 该论文提出面向搜索引擎的事件时间线摘要生成系统 QDET，属于搜索场景下的垂直领域内容生成应用，与 [CANONICAL] 领域内容生成应用（aigc_domain_application）高度匹配。

Importance reason: Focuses on a specific industrial search application with limited novelty in core methods, and lacks alignment with priority subtopics like video generation or world models. Signals: Industrial deployment, Baidu Search, timeline summarization, 7B model matches 671B, CTR improvement
...
### [143] [A multifractal-based masked auto-encoder: an application to medical images](https://arxiv.org/abs/2605.26287)
...
Importance: 普通相关 (40.55) - 未入选精读

TL;DR: 本文提出 MO-MAE，利用多重分形分析（Rényi 熵）优化 MAE 的掩码策略，使模型聚焦于医学图像中信息量最丰富的区域进行重建，在 MedMNIST 和 COVID-CT 等数据集上取得优于基线模型的效果，且计算开销极小。


<details>
  <summary>Details</summary>
Motivation: 传统 MAE 采用随机掩码策略，可能忽略医学图像中具有诊断意义的关键区域（如细微的组织结构变化），导致模型无法充分学习最具判别力的特征。

Method: 提出 MO-MAE，使用多重分形分析中的 Rényi 熵度量图像局部区域的复杂度和信息量，据此指导掩码策略，优先掩码高复杂度区域，迫使模型学习重建最具诊断相关性的特征。

Result: 在 MedMNIST 和 COVID-CT 等多个医学图像数据集上，MO-MAE 在分类任务中超越了基线 MAE 及其他 SOTA 模型，且计算开销极低。

Conclusion: 多重分形优化的掩码策略能有效增强模型对复杂组织结构的捕获与重建能力，为医学图像分析中的自监督学习提供了有前景的方向。

Classification reason: 该论文提出基于多重分形优化的掩码自编码器（MO-MAE）用于医学图像分类，属于医疗垂直领域的生成式表示学习应用，因此映射到 [CANONICAL] 领域内容生成应用（aigc_domain_application）。

Importance reason: The paper focuses on medical image analysis, which is explicitly deprioritized by negative keywords, and does not align with any priority subtopics. Signals: medical images, masked autoencoder, multifractal analysis, negative keyword match, no priority subtopic alignment
...
### [144] [Not All Tokens Matter Equally: Dynamic In-context Vector Distillation with Decisive-Token Supervision for Long-form Medical Report Generation](https://arxiv.org/abs/2605.27194)
...
Importance: 普通相关 (39.54) - 未入选精读

TL;DR: 本文提出 DIVE 框架，通过决定性令牌监督（加重病理相关令牌和EOS令牌的交叉熵权重）和状态条件动态引导（用隐藏状态依赖的适配器替代固定开环残差），在冻结骨干模型上实现长文本医学报告生成的上下文向量蒸馏，在 MIMIC-CXR 和 CheXpert Plus 数据集上取得最佳 BLEU-4、ROUGE-L 和 RadGraph F1 分数。


<details>
  <summary>Details</summary>
Motivation: 现有上下文向量蒸馏方法在短文本任务上表现良好，但扩展到长文本生成时存在两个未被充分研究的问题：(1) 令牌级蒸馏隐式地将所有输出令牌视为同等重要，但长文本输出中高频模板和语法令牌占主导，真正决定输出质量的令牌（病理相关令牌和EOS令牌）稀疏分布，在均匀交叉熵下监督不足；(2) 自回归解码会偏离教师强制轨迹，而现有固定开环残差注入无法适应这种漂移。

Method: DIVE 框架包含两个互补机制：(1) 决定性令牌监督（Decisive-token Supervision）——通过上采样病理相关令牌和EOS事件的交叉熵权重来恢复监督平衡，确保内容忠实性和终止条件在训练阶段而非解码阶段被学习；(2) 状态条件动态引导（State-conditioned Dynamic Steering）——用隐藏状态依赖的适配器替代固定开环残差，使注入信号能够随解码漂移自适应调整。

Result: 在 MIMIC-CXR 和 CheXpert Plus 数据集上，使用两个医学 VLM 骨干（Med-X 和 Med-Y），DIVE 在所有数据集-骨干设置下均取得最佳 BLEU-4、ROUGE-L 和 RadGraph F1 分数，同时在粗粒度 CheXbert F1 上保持竞争力。

Conclusion: DIVE 通过针对性解决长文本生成中令牌监督不平衡和解码漂移问题，有效提升了长文本医学报告生成的性能，验证了在冻结骨干模型上进行细粒度蒸馏的可行性。

Classification reason: 该论文聚焦于长文本医学报告生成（Long-form Medical Report Generation），属于医疗垂直领域的生成应用，与 [CANONICAL] 子主题 "领域内容生成应用"（aigc_domain_application）高度匹配。同时涉及多模态理解与生成（医学VLM骨干），因此也匹配 multimodal_understanding_generation 方向。

Importance reason: Focuses on medical report generation, a domain-specific application not aligned with user's priority subtopics like video/image generation or world models. Signals: medical report generation, token-level distillation, decisive-token supervision, long-form generation, frozen-backbone distillation
...
### [145] [A Hybrid Vision-Language Architecture for Automated Defect Reasoning and Report Generation in Industrial Inspection](https://arxiv.org/abs/2605.26533)
...
TL;DR: 本文提出了一种解耦的混合视觉-语言流水线，用于工业检测中的缺陷定位与结构化维修报告生成。系统由YOLO目标检测器（Eyes）、空间编码模块（Bridge）和QLoRA微调的Qwen-2.5-1.5B模型（Brain）组成，在风电叶片缺陷检测任务上，BLEU-4达0.41、幻觉率仅4%、专家评分8.6/10，远超零样本VLM基线。


<details>
  <summary>Details</summary>
Motivation: 当前工业检测中缺陷定位与维修报告生成是分离的，语言解释依赖人工专家，缺乏端到端的自动化方案。现有通用VLM在结构化报告生成任务上表现不佳（幻觉率高、格式不规范），且大模型难以在边缘设备部署。

Method: 提出解耦三组件流水线：(1) Eyes：YOLO26-x-obb定向边界框检测器，在原生分辨率下定位缺陷；(2) Bridge：确定性无参编码模块，将检测框映射为网格参考空间token并嵌入结构化提示；(3) Brain：4-bit量化Qwen-2.5-1.5B模型，使用QLoRA在947条合成维修报告上微调，并通过RAFT（检索增强微调）将推荐结果锚定到索引化的维修规程。输出结构化JSON报告。

Result: 完整系统BLEU-4=0.41、ROUGE-L=0.52、幻觉率HR=4%、专家评分8.6/10；零样本VLM基线分别为0.07、0.18、65%、3.3/10。QLoRA微调的1.5B模型在T4 GPU上以47 tokens/s运行，生成质量优于671B通用API模型。

Conclusion: 面向特定领域的解耦架构配合小规模领域微调语料，在结构化报告生成任务上显著优于通用端到端VLM模型，且具备边缘部署可行性。验证了"检测-编码-生成"分离设计在工业场景中的有效性。

Classification reason: 该论文面向工业检测（风电叶片缺陷检测）这一垂直领域，构建了从缺陷定位到结构化维修报告生成的自动化流水线，属于垂直领域内容生成应用，因此映射到 [CANONICAL] subtopic "领域内容生成应用" (aigc_domain_application)。

Importance reason: Focuses on narrow industrial inspection domain with small-scale fine-tuning, not advancing core generative or multimodal architectures. Signals: domain-specific application, small model fine-tuning, no novel architecture, low relevance to priority subtopics, negative keywords absent
...
### [146] [Which Changes Matter? Towards Trustworthy Legal AI via Relevance-Sensitive Evaluation and Solver-Grounded Reasoning](https://arxiv.org/abs/2605.26530)
...
TL;DR: 本文提出法律 AI 应仅对法律上相关的变更敏感，构建了法律相关性敏感评测套件，并提出 LexGuard——一种基于形式化推理的对抗多智能体框架，通过 SMT 求解器验证法律满足性与逻辑一致性，显著提升法律推理的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有法律 LLM 对法律上无关的扰动（如措辞变化）过于敏感，而对法律上实质性的变更却无法区分，导致推理不可靠。需要一种能够校准敏感性的法律 AI 评估与改进方法。

Method: 提出 LexGuard 对抗多智能体框架：将法律条文形式化为可执行约束，使用对抗智能体提取竞争性的事实-法律论点，并调用 SMT 求解器验证法律满足性与逻辑一致性，从而判断变更是否具有法律相关性。

Result: 实验表明，LexGuard 减少了法律 LLM 对操纵性框架的脆弱性，改善了相似法条间的区分能力，限制了法律无关属性的影响，并在良性改写下提高了推理一致性。

Conclusion: 法律可信赖性不仅需要准确性，还需要对法律实质性变更具有校准的敏感性。LexGuard 为构建更可靠的法律 AI 提供了有效路径。

Classification reason: 该论文聚焦法律领域的 AI 推理与生成，属于垂直领域（法律）内容生成应用，与 canonical 子主题"领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Focuses on legal AI trustworthiness, which is outside the user's core generative modeling and efficiency priorities. Signals: legal AI, relevance-sensitive evaluation, formal reasoning, SMT solver, not generative modeling
...
### [147] [PolyFusionAgent: A Multimodal Foundation Model and Autonomous AI Assistant for Polymer Property Prediction and Inverse Design](https://arxiv.org/abs/2605.26543)
...
TL;DR: PolyFusionAgent 是一个面向聚合物发现的多模态基础模型与自主AI助手框架，结合了聚合物多模态基础模型 PolyFusion（融合序列、拓扑、3D几何与指纹信息）和文献增强的设计智能体 PolyAgent，实现聚合物性质预测与逆向设计生成，并通过文献证据检索形成闭环。


<details>
  <summary>Details</summary>
Motivation: 聚合物发现面临巨大的化学设计空间和结构、性质、先验知识碎片化的问题，现有AI模型与物理/实验现实脱节，难以支持可直接用于设计的决策。

Method: 提出 PolyFusionAgent 框架，包含两个核心组件：(1) PolyFusion——多模态聚合物基础模型，对齐序列、拓扑、3D几何和指纹等多种聚合物表征，学习共享潜空间，支持性质预测和条件生成；(2) PolyAgent——工具增强的文献驱动设计智能体，通过检索聚合物文献证据，将预测和逆向设计与已有先例关联，形成交互式设计闭环。

Result: PolyFusion 在热物理性质预测上表现优异，并能生成化学有效、结构新颖的聚合物；PolyAgent 成功将预测和逆向设计与文献证据链接，实现了可验证的科学推理。

Conclusion: PolyFusionAgent 展示了结合大规模表征学习、多模态化学知识与可验证科学推理的交互式聚合物发现范式的有效性，为材料科学领域的AI辅助设计提供了新思路。

Classification reason: 该论文聚焦聚合物科学领域的AI生成应用，包括聚合物性质预测和逆向设计生成，属于科学/材料领域的垂直内容生成应用，因此映射到 canonical subtopic "领域内容生成应用" (aigc_domain_application)。

Importance reason: Focuses on polymer property prediction and inverse design, which is a domain-specific application far from the user's priority subtopics in video/image generation and multimodal understanding. Signals: domain-specific polymer discovery, multimodal foundation model for materials, inverse design with literature agent, no autoregressive or diffusion generation, no video/image generation
...
### [148] [MedGuideX: Internalizing Decision Logic from Executable Guidelines into Large Language Models for Clinical Reasoning](https://arxiv.org/abs/2605.26567)
...
TL;DR: MedGuideX 提出一种将临床实践指南转化为可执行决策逻辑的流水线，生成事实性与反事实性问答数据来训练医疗 LLM，在四个临床推理基准上平均准确率提升 10.28%，医生评估显示其生成的推理步骤更优。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将临床实践指南作为自由文本训练数据或检索来源，未能充分利用其程序化决策结构，导致模型难以真正内化指南中的决策逻辑。

Method: 提出 guideline-derived training pipeline：将 CPG 推荐转化为可执行的临床决策逻辑，并据此生成事实性（标准路径）和反事实性（条件变化下的路径）问答数据，用于训练医疗 LLM，得到 MedGuideX。

Result: 在四个临床推理基准上，MedGuideX 平均准确率相对提升 10.28%；医生评估表明其生成的推理步骤在忠实性、有效性、完整性和清晰度上均优于基线。

Conclusion: 临床实践指南中的可执行决策逻辑可以转化为可扩展的监督信号，用于构建更可靠的医疗大语言模型。

Classification reason: 该论文聚焦于将临床实践指南转化为可执行的决策逻辑，用于训练医疗大语言模型以提升临床推理能力，属于医疗垂直领域的生成与应用，因此映射到 [CANONICAL] 领域内容生成应用 (aigc_domain_application)。同时，论文涉及医疗 LLM 的推理能力，也与多模态理解与生成方向下的多模态推理有一定关联。

Importance reason: Focuses on medical domain clinical reasoning, not aligned with user's priority subtopics in multimodal generation and world models. Signals: medical LLM, clinical reasoning, guideline-derived training, domain-specific application, not multimodal generation
...
TL;DR: 本文提出 Gumbel Machine，一种灵活、模块化的反事实文本生成方法，利用 LLM 的指令跟随能力并结合 β-Hindsight control 受控解码算法，通过 Gumbel 噪声引导生成既符合评分标准又与原始学生作文相似的反事实改进版本。


<details>
  <summary>Details</summary>
Motivation: 教学中提供高质量范例是有效方法，但范例与学生当前作品差异过大时难以模仿。理想的教学示范应是学生作品的反事实改进版本——既有所提升又保持相似性。现有基于 LLM 的反事实文本生成方法多为领域特定系统，难以推广到实际应用。

Method: 提出 Gumbel Machine 框架，核心是 β-Hindsight control 受控解码算法。该方法利用 LLM 的指令跟随能力，在解码过程中引入 Gumbel 噪声作为潜在随机性，通过调节噪声强度（β 参数）来控制生成文本与参考文本之间的相似度，从而在保持与原始学生作文相似的同时生成符合评分标准的改进版本。

Result: 在多个学生写作数据集上，基于不同评分标准进行实验，结果表明 Gumbel Machine 能够有效生成既符合评分标准又与参考文本相似的反事实文本，优于现有基线方法。

Conclusion: Gumbel Machine 提供了一种灵活、模块化的反事实文本生成方案，通过可控解码实现了相似度与质量之间的平衡，在教育场景中具有实际应用价值。

Classification reason: 该论文聚焦于学生写作领域的反事实文本生成，属于教育/学术写作这一垂直领域的生成应用，与 [CANONICAL] 的"领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Focuses on educational text generation, not aligned with user's priority subtopics like video generation or world models. Signals: educational NLP, controlled text generation, LLM instruction following, not video/image generation, not multimodal
...
### [150] [Guiding LLM Post-training Data Engineering with Model Internals from Sparse Autoencoders](https://arxiv.org/abs/2605.27354)
...
TL;DR: 提出SAERL框架，利用稀疏自编码器（SAE）提取的模型内部表征来指导LLM强化学习后训练的数据工程，通过建模数据多样性、难度和质量三个内在属性，实现批次多样性控制、课程学习和质量过滤，在Qwen2.5-Math-1.5B上平均准确率提升3.00%，训练步数减少20%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM后训练数据工程主要依赖外部信号（如奖励分数、人工标注），忽略了模型内部蕴含的丰富信息。作者希望利用模型内部表征（通过SAE提取）来更直接地指导数据工程，从而提升RL训练效率和模型性能。

Method: SAERL框架包含三个核心组件：(1) SAE空间聚类与适度批次混合——在SAE激活空间中对数据进行聚类，控制批次多样性；(2) 难度代理——基于SAE激活的稀疏性/复杂度构建难度指标，实现从易到难的课程学习排序；(3) 质量探针——训练轻量级分类器（基于SAE特征）对数据质量进行过滤。这些操作均基于SAE提取的模型内部表征，无需外部标注。

Result: 在Qwen2.5-Math-1.5B上，SAERL相比原始GRPO平均准确率提升3.00%，达到目标准确率所需训练步数减少20%。在不同模型规模和RL算法上均有一致性提升。SAE在不同模型家族和规模间具有良好的迁移性，可作为轻量级可复用的数据工程工具。

Conclusion: 模型内部表征是后训练数据工程中强大且实用的信号来源。SAE作为一种可解释性工具，能够有效提取数据的内在属性（多样性、难度、质量），为数据工程提供新的范式。该方法具有跨模型迁移能力，为未来更高效的数据驱动RL训练提供了新方向。

Classification reason: 该论文关注LLM后训练阶段的数据工程（数据多样性、难度、质量），属于AIGC方向下的垂直领域生成应用（数据工程是生成模型训练的关键环节）。虽然现有[领域内容生成应用]主要面向医疗/法律/科学/代码等垂直领域，但数据工程作为AIGC训练流程的核心组成部分，可归入该宽泛主题。同时，论文使用SAE（稀疏自编码器）进行数据工程，与AIGC方向下的生成工作流/数据工程相关。

Importance reason: Focuses on LLM post-training data engineering with SAE, not directly on user's priority subtopics like video generation or world models. Signals: sparse autoencoder, data engineering, LLM post-training, model internals, reinforcement learning
...
### [151] [FM-fMRI: Event Conditioned Flow Matching for Rest-to-Task fMRI Time-Series Synthesis](https://arxiv.org/abs/2605.26423)
...
TL;DR: 本文提出 FM-fMRI，一种基于事件条件流匹配的模型，用于从静息态fMRI和任务事件信息生成任务态fMRI ROI时间序列。该方法支持快速ODE采样和灵活的事件调度，在HCP和BioPoint数据集上相比扩散模型、GAN和VAE基线在频谱、连接性和分布对齐方面表现更优，并通过合成数据提升了自闭症分类性能。


<details>
  <summary>Details</summary>
Motivation: 任务态fMRI能直接反映任务诱发的神经动态，但获取成本高、难以大规模采集。而静息态fMRI数据相对丰富，因此希望从静息态fMRI合成任务态fMRI，以缓解临床和科研中任务态数据稀缺的问题。

Method: 提出 FM-fMRI，基于事件条件流匹配（event-conditioned flow matching）模型，学习一个连续时间条件向量场，从受试者的静息态fMRI和任务事件信息生成任务ROI时间序列。采用ODE采样实现快速生成，并支持异构事件调度的灵活条件控制。

Result: 在HCP和BioPoint自闭症队列上，FM-fMRI在频谱一致性、连接性一致性和分布对齐方面均优于条件扩散模型、GAN和VAE基线。通过合成任务态fMRI数据增强BioPoint数据集，提升了自闭症分类的下游任务性能。

Conclusion: 事件条件流匹配是一种有效的静息态到任务态fMRI合成方法，生成的信号在时频结构和功能连接上具有高保真度，且能提升数据有限场景下的临床分类性能，具有实际应用价值。

Classification reason: 该论文使用流匹配模型合成任务态fMRI时间序列，属于医疗/临床领域的生成应用，与 canonical 子主题"领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Specialized medical fMRI synthesis with no alignment to user's priority subtopics or boost keywords. Signals: flow matching for fMRI, clinical data augmentation, autism classification, rest-to-task synthesis, domain-specific medical application
...
### [152] [Towards Feedback-to-Plan Decisions for Self-Evolving LLM Agents in CUDA Kernel Generation](https://arxiv.org/abs/2605.26720)
...
TL;DR: 本文提出CUDAnalyst，一个用于分析LLM智能体在CUDA内核生成中反馈到规划决策归因的统一分析框架。通过轨迹冻结和选择性反馈注入，实现了对反馈组件影响的稳定归因分析，揭示了显式规划仅在反馈对齐时有效、有效规划源于结构化多反馈交互等关键发现。


<details>
  <summary>Details</summary>
Motivation: LLM作为自进化智能体在CUDA内核生成中表现出色，但规划决策如何归因和组合异构反馈信号仍不透明。标准端到端消融实验无法解决这一问题，因为迭代规划会放大早期扰动并将反馈效应与轨迹依赖漂移混淆。

Method: 提出CUDAnalyst分析层，通过轨迹冻结（trajectory freezing）和选择性反馈注入（selective feedback injection）实现受控的、生成级别的规划决策归因，支持稳定的生成级评估和基于联盟博弈的反馈效应与交互归因。

Result: 实验表明：显式规划仅在反馈对齐时有益；有效规划源于结构化多反馈交互；强推理模型的高层规划可部分迁移到弱模型。这些趋势在多个参考骨干网络、代表性工作负载和参考归纳机制下保持一致。

Conclusion: 本文揭示了反馈到规划决策的结构化归因关系，证明了CUDAnalyst能够有效解耦反馈组件对规划决策的贡献，为理解LLM智能体在代码生成中的自进化机制提供了分析工具和实证见解。

Classification reason: 论文聚焦于CUDA内核代码生成，属于代码生成这一垂直领域应用，与[CANONICAL] subtopic "领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Focuses on LLM agent planning analysis for CUDA kernel generation, which is a niche application not aligned with user's priority subtopics. Signals: LLM agent analysis, CUDA kernel generation, feedback attribution, not video/image generation, not efficient inference
...
### [153] [A Dataset of Robot-Patient and Doctor-Patient Medical Dialogues for Spoken Language Processing Tasks](https://arxiv.org/abs/2605.26747)
...
TL;DR: 本文提出了 MeDial-Speech，一个包含 111+ 小时机器人-患者和医生-患者医疗对话的语音数据集，覆盖四种疾病，并基于句子选择任务构建了对话基准，评估了 GPT-5 mini、DeepSeek-V3 和 Claude Sonnet 4 三个 LLM，发现 Claude Sonnet 4 表现最佳（准确率 71.1%-74.7%），但所有模型在概率预测上均过度自信。


<details>
  <summary>Details</summary>
Motivation: 尽管 LLM 在通用任务上取得了巨大进步，但将其应用于文本或语音医疗咨询仍是一个开放问题。现有医疗对话数据集缺乏真实场景下的语音数据，难以支撑医疗 AI 的训练与评估。

Method: 在真实环境中收集机器人-患者和医生-患者的医疗对话语音数据，构建 MeDial-Speech 数据集（111+ 小时，覆盖四种疾病）。提出基于句子选择（20 个选项）的对话基准，评估 GPT-5 mini、DeepSeek-V3 和 Claude Sonnet 4 三个 LLM 在手动转录和自动转录两种条件下的表现。

Result: Claude Sonnet 4 在句子选择任务中表现最佳，手动转录准确率 71.1%，自动转录准确率 74.7%。所有 LLM 在概率预测上均表现出高度过度自信，无论选择正确还是错误的句子。

Conclusion: MeDial-Speech 数据集为医疗对话 AI 的研究提供了有价值的资源；当前 LLM 在医疗对话理解上仍有提升空间，且过度自信问题需要进一步解决。

Classification reason: 该论文提出了一个医疗对话语音数据集（MeDial-Speech），用于训练和评估医疗AI（Med-AI），属于医疗垂直领域的生成/对话应用。这与 [CANONICAL] subtopic "领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Medical dialogue dataset for spoken language processing is outside all priority subtopics and contains negative keywords. Signals: medical dialogue dataset, spoken language processing, robot-patient dialogues, negative keywords present, no video/generation focus
...
### [154] [RoadGIE: Towards A Global-Scale Aerial Benchmark for Generalizable Interactive Road Extraction](https://arxiv.org/abs/2605.26862)
...
Importance: 普通相关 (40.09) - 未入选精读

TL;DR: 本文提出 WorldRoadSeg-360K，目前最大最多样的航拍道路分割数据集（36.7万张图像，覆盖38个国家223个城市），并基于此提出 RoadGIE，一种支持连通性感知提示（点击/涂鸦）的交互式道路提取范式，结合专家引导提示策略和骨架召回损失，在仅3.7M参数下实现了SOTA分割精度和拓扑一致性。


<details>
  <summary>Details</summary>
Motivation: 现有航拍道路分割数据集场景多样性不足、语义粒度低、结构连续性差，限制了模型在不同环境下的泛化能力；同时现有交互式方法缺乏高效且拓扑敏感的道路编辑工具。

Method: 构建大规模多样化数据集 WorldRoadSeg-360K；提出 RoadGIE 交互式道路提取框架，支持点击和涂鸦等连通性感知提示，集成专家引导提示策略和骨架召回损失以提升结构一致性和交互迭代性能。

Result: 在 WorldRoadSeg-360K 及其他基准上，RoadGIE 在分割精度和拓扑一致性上均达到SOTA，且仅需3.7M参数，运行高效。

Conclusion: 大规模多样化数据集和拓扑感知的交互式范式能显著提升道路分割的泛化能力和结构连续性，为遥感地理空间应用提供了有效方案。

Classification reason: 该论文关注遥感航拍图像的道路分割与交互式提取，属于遥感/地理空间领域的垂直应用，与 [CANONICAL] 领域内容生成应用（aigc_domain_application）方向匹配。

Importance reason: Strong dataset contribution but remote sensing road extraction is a negative keyword and not aligned with priority subtopics. Signals: Largest road segmentation dataset, interactive extraction paradigm, connectivity-aware prompts, skeleton recall loss, efficient 3.7M parameters
...
### [155] [Auditing and Fixing Economic Validity in Tabular Foundation Models for Discrete Choice](https://arxiv.org/abs/2605.26559)
*Yingshuo Wang, Xian Sun, Yanhang Li, Zhichao Fan, Zexin Zhuang*

Main category: cs.LG, econ.EM, cs.AI
...
TL;DR: 本文发现表格基础模型在离散选择预测中常违反经济逻辑（如提价反而增加需求），提出两阶段适配器将基础模型预测嵌入效用最大化框架，在保证经济一致性的同时恢复高达13个百分点的预测准确率。


<details>
  <summary>Details</summary>
Motivation: 表格基础模型在离散选择预测任务上表现优异，但其预测常违反基本经济逻辑（如价格与需求负相关、支付意愿为负），限制了其在经济学和政策分析中的可信应用。

Method: 提出两阶段适配器：第一阶段估计一个参数受经济理论约束的标准选择模型；第二阶段冻结这些参数，训练一个校正项来融入基础模型的预测信息，最终模型继承基础模型的精度优势，同时保证价格-需求关系的单调性和可解析计算的权衡指标。

Result: 在两个交通数据集上，适配器相比标准logit模型恢复了高达13个百分点的准确率，同时保持完美的经济一致性，而原始基础模型和传统蒸馏方法均无法做到。

Conclusion: 通过将基础模型预测嵌入经济理论约束框架，可以在不牺牲预测精度的前提下保证经济有效性，为表格基础模型在经济学等领域的可信应用提供了可行方案。

Classification reason: 该论文关注表格基础模型在经济选择预测任务中的经济有效性审计与修正，属于经济学垂直领域的生成/预测模型应用，映射到 [CANONICAL] 领域内容生成应用（aigc_domain_application）。

Importance reason: Focuses on economic validity in tabular models for discrete choice, which is outside the user's priority subtopics in multimodal generation and model efficiency. Signals: Tabular foundation models, economic logic violation, utility-maximization adapter, discrete choice prediction, transportation datasets
...
### [156] [The Compressive Knowledge Graph Hypothesis: Which Graph Facts Matter for Scientific Hypothesis Generation?](https://arxiv.org/abs/2605.27176)
...
TL;DR: 本文提出"压缩知识图谱假设"：在科学假设生成中，知识图谱的效用是选择性的且依赖模型，紧凑的子图往往能近似完整知识图谱的效果，说明有用的图谱信号可以从压缩的、科学结构化的子图中恢复。


<details>
  <summary>Details</summary>
Motivation: 知识图谱能为语言模型提供结构化科学上下文以辅助假设生成，但并非所有图谱事实都对生成结果有贡献。目前缺乏对哪些图谱事实真正影响假设生成的系统性理解。

Method: 以电池材料领域的假设生成为场景，使用Mistral-7B、Llama-3.1-70B和Gemini 2.5 Flash三个模型，通过改变局部知识图谱的密度、本体丰富度、拓扑结构和控制结构来扰动图谱，并用provided-graph和fixed-reference两种指标评估输出质量。

Result: 知识图谱的效用是选择性的且依赖模型；无图谱输入时模型也能从先验知识中恢复大量图谱内容；紧凑的top-k子图往往能近似完整图谱的行为，即使排除claimed-outcome三元组也是如此；压缩并非唯一依赖于语义排序规则，随机和基于拓扑的子集也能恢复大部分信号。

Conclusion: 支持冗余感知的"压缩知识图谱假设"：有用的知识图谱信号通常可以从紧凑的、科学结构化的子图中恢复，而不需要完整的局部知识图谱。

Classification reason: 该论文研究知识图谱引导的科学假设生成（battery materials领域），属于科学垂直领域的生成应用，与CANONICAL subtopic "领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Focuses on knowledge graph utility for hypothesis generation, not directly on the user's priority subtopics like video generation or world models. Signals: Knowledge graph compression, scientific hypothesis generation, model-dependent KG utility, battery materials domain, not aligned with user's core interests
...
### [157] [AutoDFT: A Closed-Loop Multi-Agent Framework for Autonomous DFT Calculations](https://arxiv.org/abs/2605.26179)
*Penghui Yang, Zhonghan Zhang, Yue Li, Xinrun Wag, Yanchen Deng, Yuhao Lu, Bijun Tang, Zheng Liu, Bo An*

Main category: cs.CE, cond-mat.mtrl-sci, cs.AI
...
TL;DR: AutoDFT 是一个闭环多智能体框架，将 LLM 推理嵌入密度泛函理论（DFT）计算的每个阶段，包括策略规划、即时参数生成和监控-恢复-反思循环，实现了 94.1% 的任务级成功率，使非计算专家也能获得可靠的第一性原理计算结果。


<details>
  <summary>Details</summary>
Motivation: DFT 计算在材料科学和化学中至关重要，但每次计算都需要大量人工干预：收敛失败时调整算法、出现意外物理现象时修改计划、中间结果变化时插入新步骤。现有基于 LLM 的智能体仅自动化初始规划阶段，后续适应依赖手工规则，工作流脆弱且难以泛化。

Method: AutoDFT 采用闭环多智能体架构：(1) 策略规划器（Strategic Planner）生成步骤目标的骨架计划；(2) 步骤规划器（Step Planner）根据前序结果即时生成数值参数；(3) 监控-恢复-反思（Monitor-Recover-Reflect）循环诊断失败、修复问题并在证据充分时修订计划。整个流程将 LLM 推理嵌入 DFT 生命周期的每个阶段。

Result: 在 VASPBench 基准（涵盖 34 个任务和 9 种 DFT 计算类型）上，AutoDFT 使用 GPT-5.2 达到 94.1% 的任务级成功率。在已有材料数据库上，AutoDFT 在电子、磁性和能量性质方面产生了定量可靠的预测结果。

Conclusion: AutoDFT 通过闭环规划与执行，使不具备深度计算专业知识的实验人员也能获得可靠的第一性原理计算结果，显著降低了 DFT 计算的人工门槛，展示了 LLM 智能体在科学计算自动化中的巨大潜力。

Classification reason: 该论文提出 AutoDFT，一个用于材料科学 DFT 计算的多智能体自动化框架，属于科学计算/材料科学垂直领域的 AI 生成应用，与 canonical 子主题"领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Focuses on automating DFT calculations in materials science, which is outside the user's core interests in generative AI and multimodal models. Signals: domain-specific application, multi-agent framework, closed-loop automation, materials science, DFT calculations
...
### [158] [Prospective evaluation of multimodal respiratory failure prediction: Do chest X-rays improve performance beyond EHR signals?](https://arxiv.org/abs/2605.26255)
*Xiaolei Lu, Shamim Nemati*

Main category: eess.IV, cs.LG, cs.AI

Direction: AIGC

Subtopic: 领域内容生成应用

Importance: 普通相关 (29.4) - 未入选精读

TL;DR: 本文提出一种门控多模态框架，融合结构化EHR时序数据与胸部X光片基础模型表征，用于ICU患者24小时内侵入性机械通气的早期预测。门控模块根据患者临床上下文自适应控制影像特征的贡献。前瞻性评估表明，多模态融合相比纯EHR模型（AUROC 0.860 vs 0.752）和医生预测均显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 早期预测呼吸衰竭对ICU及时临床干预至关重要。现有基于EHR的模型虽能持续监测生理恶化，但可能无法充分捕捉胸部X光片反映的肺部病理信息。本文旨在探究CXR信息能否在EHR信号之外提升前瞻性呼吸衰竭预测性能。

Method: 提出门控多模态框架，融合结构化EHR时序数据与CXR基础模型表征（REMEDIS和MedInsight）。门控模块根据患者特定临床上下文自适应控制影像特征的贡献权重，使模型在影像信息有用时选择性依赖成像特征。前瞻性评估ICU患者24小时内侵入性机械通气预测，与EHR-only基线、医生预测及替代多模态变体进行比较。

Result: 门控多模态模型AUROC达0.860（REMEDIS）和0.858（MedInsight），显著优于EHR-only基线的0.752。相比医生预测，多模态框架在保持良好特异性的同时大幅提升了敏感性。相比EHR-only模型，多模态融合提高了特异性和阳性预测值。

Conclusion: CXR信息可在选定患者中细化风险估计，自适应多模态融合是将影像信息纳入前瞻性呼吸衰竭预测的实用策略。

Classification reason: 该论文研究多模态呼吸衰竭预测，属于医疗垂直领域的AI生成/预测应用，与 canonical 子主题"领域内容生成应用"（医疗方向）高度匹配。

Importance reason: Medical diagnosis and clinical prediction are explicitly negative keywords, and the paper lacks any generative or multimodal generation focus. Signals: medical diagnosis, clinical prediction, negative keyword match, no generative modeling, no video/image generation
...
### [159] [DGLD: Domain-Gated Latent Diffusion for the Discovery of Novel Energetic Materials](https://arxiv.org/abs/2605.26540)
*Yehudit Aperstein, Alexander Apartsin*

Main category: physics.chem-ph, cs.AI
...
TL;DR: 本文提出 Domain-Gated Latent Diffusion (DGLD)，一种面向含能材料（HMX 级炸药）分子发现的领域门控潜扩散模型。通过标签质量门控训练、多任务评分模型引导和四阶段化学验证流程（含 DFT 审计），发现了 12 个 DFT 验证的新型含能分子，其中 L1 和 E1 两个候选分子在密度和爆速上达到 HMX 级性能，且与训练集分子结构高度不相似。


<details>
  <summary>Details</summary>
Motivation: 含能材料（如 HMX 级炸药）的性能提升能直接转化为推进剂减重、弹头小型化和更高效的气体发生器，但过去 15 年没有新的 HMX 级化合物被公开。这是一个稀疏标签问题：约 66k 标记的 CHNO 分子中只有约 3k 有实验或 DFT 质量的数据，朴素生成模型要么记忆高性能尾部样本，要么无校准地外推。因此需要一种能有效处理稀疏标签、发现新型高性能含能分子的生成方法。

Method: 提出 Domain-Gated Latent Diffusion (DGLD)，包含三个核心设计：(1) 训练时标签质量门控（label-quality gate），区分高质量 DFT/实验数据与低质量计算数据；(2) 采样时多任务评分模型引导（multi-task score-model guidance），联合优化密度、爆速等目标；(3) 四阶段化学验证漏斗：SMILES 有效性检查 → 合成可行性过滤 → 性能预测筛选 → 第一性原理 DFT 审计。模型在分子 SMILES 的潜空间中进行扩散生成。

Result: 发现了 12 个 DFT 验证的新型含能分子候选。头条分子 L1 (3,4,5-trinitro-1,2-isoxazole) 计算密度 2.09 g/cm³，爆速 8.25 km/s，与所有 65,980 个训练分子的最近邻 Tanimoto 相似度仅 0.27。另一候选 E1 (4-nitro-1,2,3,5-oxatriazole) 爆速达 9.00 km/s。DGLD 是唯一在 DFT 审计下同时实现新颖性和目标性能的方法。对比方法中，SMILES-LSTM 记忆了 18.3% 的输出，SELFIES-GA 的最佳候选在 DFT 审计下损失 3.5 km/s。

Conclusion: DGLD 证明了通过标签质量门控和多任务引导的潜扩散模型，可以在稀疏标签的含能材料领域发现结构新颖、性能达标的 HMX 级化合物。该方法以仅数 GPU 天的成本即可完成从发现到 DFT 验证的完整流程，为加速含能材料研发提供了有效工具。

Classification reason: 该论文将扩散模型应用于含能材料（炸药）分子设计这一垂直科学领域，属于科学/化学领域的生成应用，因此映射到 [CANONICAL] 领域内容生成应用 (aigc_domain_application)。同时，论文使用了扩散模型进行分子生成，也匹配多模态生成方向。

Importance reason: The paper focuses on a niche chemistry application (energetic materials) that falls outside the user's priority subtopics and is flagged by negative keywords. Signals: domain-specific chemistry, negative keyword match (molecule, drug discovery), no direct connection to video/image generation or multimodal models, low personal relevance

Abstract: Energetic-materials performance gains translate directly into reduced propellant mass, smaller warheads, and more efficient civilian gas-generators, yet no new HMX-class compound has been disclosed in fifteen years. Designing one is a sparse-label problem: of ~66 k labelled CHNO molecules only ~3 k carry experimental or DFT-quality measurements, and naive generative models trained on the full mixture either memorise the high-performance tail or extrapolate without calibration. We introduce Domain-Gated Latent Diffusion (DGLD): a label-quality gate at training time, multi-task score-model guidance at sample time, and a four-stage chemistry-validation funnel ending in first-principles DFT audit. The result is 12 DFT-confirmed novel leads. The headline compound, 3,4,5-trinitro-1,2-isoxazole (L1), reaches \r{ho}_"cal" =2.09 g/cm3 and D_"K-J,cal" =8.25 km/s and is structurally dissimilar from all 65 980 training molecules (nearest-neighbour Tanimoto 0.27). A co-headline lead, E1 (4-nitro-1,2,3,5-oxatriazole), exceeds L1 on calibrated detonation velocity (D_"K-J,cal" =9.00 km/s) from a chemotype family disjoint from L1's. DGLD is the only method to land in the productive quadrant (simultaneously novel and on-target) at DFT level. SMILES-LSTM memorises 18.3% of its outputs exactly; SELFIES-GA's best novel candidate loses 3.5 km/s under DFT audit; REINVENT 4 generates novel high-N heterocycles but peaks at D=9.02 km/s. Code, checkpoints, and 918 mined hard negatives are released on Zenodo (DOI the next compound to enter the HMX-class band can be discovered, validated, and recommended for synthesis at the cost of a few GPU-days.

</details>


### [160] [MatFormBench: A Benchmarking Evaluation Framework for Target-Driven Materials Formulation](https://arxiv.org/abs/2605.26741)
*Linhan Wu, Chenxi Wang, Chuhan Yang, Zhengwei Yang, Yuyang Liu*

Main category: cond-mat.mtrl-sci, cs.AI
...
TL;DR: MatFormBench 是一个面向目标驱动材料配方逆设计的评测框架，整合了物理驱动的配方生成方案、五个难度级别和综合评分指标 MatFormScore，系统评估了 39 种逆设计算法（包括扩散模型、VAE、遗传算法和 LLM 推荐策略），发现扩散模型整体表现最优。


<details>
  <summary>Details</summary>
Motivation: 现有材料机器学习基准仅局限于正向属性预测，缺乏对逆优化和生成算法的系统评测，阻碍了目标驱动材料设计的发展。

Method: 提出 MatFormBench 评测生态系统：1）物理驱动的配方生成方案，生成模拟真实材料结构-属性响应关系的合成样本；2）五个递增难度级别量化关系复杂度；3）MatFormScore 多维指标，涵盖目标成功率、搜索效率、探索能力、鲁棒性和稳定性五个维度。在 39 种逆设计算法（包括代理辅助黑箱搜索、深度生成模型和 LLM 推荐策略）上进行 1170 次标准化评测。

Result: 扩散模型在整体性能上表现最强，VAE 和遗传算法在特定场景下各有优势。通过 1170 次标准化算法-任务评估验证了框架的有效性。

Conclusion: MatFormBench 为目标驱动材料配方设计建立了统一的评测标准，实现了可复现的基准测试、原则性算法比较和逆设计策略的诊断分析，为材料逆设计提供了基础工具。

Classification reason: 该论文提出材料配方逆设计的评测基准 MatFormBench，属于材料科学（垂直领域）的生成与优化方法评测，与 [CANONICAL] 领域内容生成应用（aigc_domain_application）高度匹配。

Importance reason: The paper focuses on materials science formulation, which is outside the user's priority subtopics and is flagged by negative keywords. Signals: domain-specific benchmark, materials inverse design, diffusion models for formulation, not aligned with user's priority subtopics, negative keyword match
...
### [161] [HTMLCure: Turning Browser Experience into State Guided Repair for Interactive HTML](https://arxiv.org/abs/2605.26807)
...
TL;DR: HTMLCure 提出一个浏览器交互式评估与修复框架，通过执行HTML页面并记录多状态下的浏览器证据，驱动闭环修复引擎，将低质量HTML页面修复为可用页面，用于构建高质量SFT数据集，显著提升HTML生成模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成的HTML页面往往只在初始渲染时正确，在滚动、悬停、点击、缩放等交互状态下会失败。基于截图的评估无法捕捉这些失败，而简单过滤会丢弃许多本可修复的页面。因此需要一种能交互式评估并自动修复HTML页面的方法。

Method: HTMLCure 包含两个核心组件：(1) 浏览器体验评估器——在不同视口和交互状态下执行页面，记录确定性浏览器证据，并为VLM提供执行轨迹中的关键帧而非孤立截图；(2) 闭环修复引擎——根据状态信号诊断当前页面，选择特定状态的修复策略，运行每个候选修复方案，最终导出质量通过的页面用于SFT。

Result: 在97K提示词语料上，将可直接使用的种子页面扩展为63703个质量通过的候选页面，最终构建40K精炼SFT集。HTMLCure-27B-Refined在HTMLBench-400上达到50.6分，确定性测试用例通过率45.2%，与Kimi-K2.6和GPT-5.4等强基线相当。在MiniAppBench验证集上达到81.2平均分，比原始27B SFT提升15.3分。

Conclusion: HTMLCure 证明了通过浏览器交互式评估与闭环修复可以显著提升HTML生成质量，将原本不可用的页面转化为高质量训练数据，为代码/网页生成领域提供了一种有效的数据增强与质量提升范式。

Classification reason: 该论文关注HTML页面生成后的质量修复与评估，属于代码/网页生成这一垂直领域应用，与[领域内容生成应用]（aigc_domain_application）高度匹配。

Importance reason: Focuses on HTML repair for SFT data, not core generative model advances in user's priority subtopics. Signals: HTML repair, browser interaction, SFT data construction, not autoregressive/diffusion video/image generation, not world model
...
### [162] [ConVer: Using Contracts and Loop Invariant Synthesis for Scalable Formal Software Verification](https://arxiv.org/abs/2605.27051)
...
TL;DR: ConVer 是一个基于LLM的顶层组合式形式化验证工具，通过LLM合成函数契约并在CEGAR-CEGIS循环中迭代精炼，实现对大型C程序的可扩展形式化验证，在多个基准测试上取得了较好的验证成功率。


<details>
  <summary>Details</summary>
Motivation: 有界模型检测（BMC）工具在处理大型C程序时面临状态空间爆炸问题，需要展开所有嵌套结构直到预定边界，导致可扩展性差。现有方法难以高效验证复杂程序。

Method: ConVer 采用自顶向下的组合验证方法：给定一个带有顶层断言的C程序，使用LLM从系统属性合成函数契约（contracts），然后在CEGAR-CEGIS循环中交替进行系统级和函数级检查，当检查失败时通过SMART ICE学习精炼契约。此外还提出了ESBMC-LF预处理器，将LF模型转换为C程序。

Result: 在Frama-C基准（45个简单C程序）上，使用三种LLM后端达到82-96%的验证成功率，93-95%的收敛程序仅需一次CEGAR-CEGIS迭代。在X.509解析器基准（6个程序）上达到33-50%，在LF2C-Simple套件（17个程序）上达到82-88%，在VerifyThis套件（11个程序）上达到55-64%，在LF-Hard基准上整体达到67%。

Conclusion: ConVer 通过LLM辅助的契约合成与CEGAR-CEGIS迭代精炼，有效缓解了形式化验证中的状态空间爆炸问题，在多个基准上展示了优于现有工具的性能，证明了LLM在形式化验证领域的应用潜力。

Classification reason: 该论文使用LLM合成函数契约（contracts）以辅助C程序的形式化验证，属于AI辅助代码验证/软件工程领域的生成应用，映射到AIGC方向下的[领域内容生成应用]（aigc_domain_application）子主题。

Importance reason: Focuses on formal verification of C programs using LLMs, which is outside the user's core interests in generative models and video/image generation. Signals: LLM for formal verification, CEGAR-CEGIS loop, software engineering, not generative model, no video/image generation
...
### [163] [Unveiling the Fragility of Vision-Language Models: Multi-Modal Adversarial Synergy via Texture-Constrained Perturbations and Cross-Modal Optimization](https://arxiv.org/abs/2605.26501)
...
TL;DR: 本文提出MMAS框架，通过联合优化纹理约束的通用对抗扰动（图像）和嵌入空间约束的可学习prompt扰动（文本），在黑盒设置下对LVLM进行跨模态协同攻击，实验证明其具有强通用性和可迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有针对LVLM的对抗攻击多局限于单模态或需要白盒访问，缺乏同时利用双模态且适用于黑盒场景的通用攻击方法，难以评估真实场景下LVLM的鲁棒性风险。

Method: 提出MMAS框架，包含：(1) 基于小波的纹理尺度约束通用对抗扰动（UAP），确保图像扰动的不可感知性和跨输入的鲁棒性；(2) 嵌入空间L范数约束的可学习文本prompt扰动，保持语义连贯性；(3) 跨模态正则化项对齐两种扰动的梯度方向，增强协同攻击效果和跨任务/跨模型迁移性。整个优化仅依赖模型查询（黑盒）。

Result: 在多个主流LVLM上的大量实验表明，MMAS具有强大的通用对抗攻击能力，在跨任务和跨模型场景下均表现出优异的迁移性。

Conclusion: MMAS揭示了LVLM在多模态对抗攻击下的脆弱性，为评估和提升多模态模型的安全性提供了新的视角和工具。

Classification reason: 该论文研究针对LVLM的黑盒多模态对抗攻击（图像UAP+文本prompt扰动联合优化），属于多模态安全与攻防范畴，与[多模态安全与攻防]（multimodal_safety_attack）这一CANONICAL subtopic高度匹配。

Importance reason: Focuses on adversarial attack robustness, which is not a priority subtopic and lacks direct connection to generation or efficiency. Signals: multimodal adversarial attack, black-box attack, LVLM robustness, texture-constrained perturbation, cross-modal optimization
...
### [164] [Evaluating the Relevance of Uncertainty Estimators for LLM Hallucination](https://arxiv.org/abs/2605.27016)
*Yedidia Agnimo, Anna Korba, Annabelle Blangero, Nicolas Chesneau, Karteek Alahari*

Main category: cs.CL, cs.LG, stat.ML, cs.AI
...
TL;DR: 本文系统研究了LLM中不确定性估计与幻觉之间的关联性，发现这种关联高度可变且通常较弱，取决于幻觉类型和模型，挑战了将不确定性直接作为幻觉信号的做法。


<details>
  <summary>Details</summary>
Motivation: LLM容易产生幻觉，阻碍了其可靠部署。虽然已有大量不确定性估计方法被提出并隐含地用作模型失败的代理指标，但不确定性与幻觉之间的关系尚未得到充分刻画。

Method: 作者对多种不确定性估计方法（包括信息论、基于采样和反射性估计器）进行了系统的实证研究，在内在幻觉（对输入忠实性的违反）和外在幻觉（相对于训练数据的无根据声明）两种设置下，使用RAGTruth和HalluLens等四个互补基准评估其与幻觉的关联。

Result: 不确定性估计与幻觉之间的关联高度可变且通常较弱，具体取决于幻觉类型和所评估的LLM。这一发现挑战了将不确定性作为直接幻觉信号的普遍做法。

Conclusion: 不确定性估计不能简单地作为LLM幻觉的直接代理指标，其有效性高度依赖于具体场景。研究结果明确了不确定性在何时能提供可操作的信息，为LLM可靠性研究提供了重要指导。

Classification reason: 该论文研究LLM幻觉与不确定性估计之间的关系，属于多模态幻觉与可靠性（multimodal_hallucination）这一[CANONICAL]子主题的范畴，因为该子主题涵盖幻觉缓解、事实性与可靠性等核心内容。

Importance reason: The paper's focus on LLM hallucination and uncertainty estimation is only tangentially related to the user's core interests in generative video/image models and efficient inference. Signals: LLM hallucination, uncertainty estimation, weak association, not generative video/image, not efficient inference
...
### [165] [Real Images, Worse Judgments: Evaluating Vision-Language Models on Concreteness and Imagery](https://arxiv.org/abs/2605.27315)
...
TL;DR: 本文通过具体性和意象性评分任务，系统评估了VLM在词汇判断中是否被真实图像上下文误导。研究发现真实图像上下文不仅没有带来一致增益，反而降低了与人类评分的对齐程度，尤其在视觉证据最不相关时退化最严重。通过探针分析、典型相关分析和归因案例研究，发现真实图像上下文导致表征偏移和对虚假视觉线索的过度敏感。作者进一步证明，在推理时引导模型仅关注文本内容可以缓解这种退化。


<details>
  <summary>Details</summary>
Motivation: 视觉输入通常被认为能改善多模态模型的语言理解能力，但这一假设缺乏系统验证。本文旨在检验VLM是否能在词汇判断中区分有用的视觉证据与无关的图像上下文。

Method: 使用人类具体性和意象性评分作为基准，比较VLM在纯文本输入与真实图像上下文输入下的表现。通过探针分析、典型相关分析（CCA）和归因案例研究，分析表征偏移和对虚假视觉线索的敏感性。最后通过推理时指令引导模型仅关注文本内容来验证缓解效果。

Result: 真实图像上下文未带来一致增益，反而降低了与人类评分的对齐，尤其在抽象/低意象词上退化最严重。表征分析表明真实图像上下文导致表征偏移和对虚假视觉线索的过度敏感。推理时指令引导模型仅关注文本可有效缓解退化。

Conclusion: 当前指令微调的VLM需要更好地校准何时视觉上下文应参与词汇判断，视觉输入并非总是有益，模型对无关视觉线索的敏感性是导致性能退化的关键因素。

Classification reason: 该论文研究VLM在词汇判断中是否被真实图像上下文误导，导致与人类评分对齐变差，属于多模态幻觉与可靠性问题（模型对视觉线索的过度敏感导致判断退化），因此映射到[CANONICAL] subtopic "多模态幻觉与可靠性"。

Importance reason: Evaluates VLM reliability in lexical tasks, but focuses on concreteness/imagery rather than core generation or world model priorities. Signals: VLM evaluation, multimodal hallucination, real-image context degrades performance, instruction-tuned models, lexical judgment
...
### [166] [Automatic Layer Selection for Hallucination Detection](https://arxiv.org/abs/2605.26366)
...
TL;DR: 本文提出了一种无需训练的自动层选择方法FEPoID（First Effective Peak of Intrinsic Dimension），用于从LLM中间层中自动识别最优层进行幻觉检测，并引入截断策略进一步增强幻觉相关信号，在问答和摘要幻觉检测基准上取得优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 已有研究表明LLM中间层比最后一层更强烈地编码了与幻觉相关的信号，但如何自动选择高性能层进行幻觉检测仍缺乏系统性研究，现有方法缺乏原则性的自动层选择机制。

Method: 首先提出多种关于中间层幻觉信号涌现的假设并评估相应自动层选择标准，发现均不能一致地取得满意性能。随后提出FEPoID（First Effective Peak of Intrinsic Dimension）准则，通过计算各层的内在维度（Intrinsic Dimension）并选取第一个有效峰值对应的层作为最优层。此外，引入一种简单的截断策略（truncation strategy）来放大幻觉相关信号。

Result: FEPoID在多种LLM架构、规模和任务上一致地识别出最优或接近最优的层，优于所提出的其他选择标准和现有幻觉检测基线方法。截断策略进一步显著提升了整体检测性能。方法无需训练且计算开销可忽略。

Conclusion: FEPoID是一种有效的、无需训练的自动层选择方法，可用于LLM幻觉检测，且截断策略能进一步增强检测效果。该工作填补了自动层选择在幻觉检测中的空白。

Classification reason: 该论文研究LLM幻觉检测中的中间层选择方法，属于多模态幻觉与可靠性（multimodal_hallucination）这一[CANONICAL]子主题的范畴。虽然论文主要聚焦文本LLM，但幻觉检测方法可迁移至多模态场景，且该子主题涵盖幻觉缓解与可靠性提升。

Importance reason: Focuses on LLM hallucination detection, not directly aligned with user's priority on generative video/image models or efficiency. Signals: hallucination detection, layer selection, intrinsic dimension, LLM reliability, training-free method
...
### [167] [Pop-Up Distractions Reveal Bag-of-Events Behavior in Video Large Language Models](https://arxiv.org/abs/2605.27101)
...
TL;DR: 本文提出 DistractionBench 基准，通过在视频中插入无关广告片段来测试 VideoLLM 的主体-事件关联能力。实验发现所有主流 VideoLLM 均存在"事件袋"（bag-of-events）行为，即模型将视频视为无序事件集合而非时序序列，频繁将插入广告中的动作错误归因于主视频主体，暴露出模型缺乏可靠的时序定位机制。


<details>
  <summary>Details</summary>
Motivation: 视频理解的核心能力是将主体与事件在时间上可靠地关联起来，但现有 VideoLLM 是否真正具备这一能力尚不明确。作者希望通过受控干预实验系统性地揭示模型在这一方面的缺陷。

Method: 设计 DistractionBench 评测基准，通过在长视频中插入不相关的短广告片段作为干扰，构造受控实验。定义"事件袋"（bag-of-events）行为来描述模型将不同片段中的主体与事件错误关联的系统性幻觉。在 11 个主流 VideoLLM 上进行评测。

Result: 所有 11 个被评测的 VideoLLM 均表现出显著的事件袋行为，即模型无法正确区分不同视频片段中的主体与事件，频繁将广告中的动作归因于主视频主体。

Conclusion: 当前 VideoLLM 缺乏可靠的时序定位机制，倾向于将视频处理为无序事件集合而非时序结构化序列。这一发现为开发更鲁棒的主体-事件关联模型提供了方向。

Classification reason: 论文研究 VideoLLM 在视频理解中将不同片段的主体与事件错误关联的系统性幻觉问题（bag-of-events 行为），属于多模态幻觉与可靠性范畴，因此复用 canonical subtopic "多模态幻觉与可靠性"。

Importance reason: Identifies a fundamental temporal grounding failure in VideoLLMs, but the topic is not a user priority. Signals: VideoLLM hallucination, bag-of-events behavior, temporal grounding weakness, DistractionBench benchmark, multimodal understanding evaluation
...
### [168] [Respecting Modality Gap in Post-hoc Out-of-distribution Detection with Pre-trained Vision-Language Models](https://arxiv.org/abs/2605.26661)
...
TL;DR: 本文挑战了VLM中广泛采用的"文本作为原型"的OOD检测范式，理论证明了文本原型与最优视觉原型之间存在固有的模态差距，并提出了一种在线伪监督框架，在测试时数据流中直接在视觉特征空间学习类原型，实现了SOTA的OOD检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于VLM的零样本OOD检测方法通常将类别名称的文本嵌入作为类原型，但本文理论证明文本原型与最优视觉原型之间存在固有的模态差距，且仅靠提示工程无法消除这一差距，从而限制了OOD检测性能。

Method: 提出在线伪监督框架（Online Pseudo-supervised Framework），利用未标注的测试时数据流和预训练VLM的软预测，直接在视觉特征空间中学习类原型。通过在线优化过程逐步更新原型，并提供了收敛性的理论保证。

Result: 在多种OOD检测设置下（包括不同VLM骨干网络、不同ID数据集），该方法取得了新的SOTA结果，显著优于现有基于文本原型的方法。

Conclusion: 本文揭示了VLM中文本-视觉模态差距对OOD检测的负面影响，提出的在线视觉原型学习方法有效弥合了这一差距，为后验OOD检测提供了新的有效范式。

Classification reason: 该论文研究基于预训练视觉语言模型（VLM）的分布外（OOD）检测，旨在提升模型对未知输入的识别可靠性，属于多模态模型的可靠性增强方向，与[多模态幻觉与可靠性]（multimodal_hallucination）主题高度相关。

Importance reason: Addresses reliability in VLMs but is not aligned with user's priority subtopics on generation or efficiency. Signals: OOD detection, vision-language models, modality gap, theoretical analysis, online pseudo-supervised learning
...
### [169] [Leveraging Visual Signals for Robust Token-Level Uncertainty in Vision-Language Generation](https://arxiv.org/abs/2605.27136)
...
TL;DR: 本文提出VIG-TUQ，一种无需训练的不确定性量化框架，通过将视觉接地分数融入token级语言不确定性估计，显著提升了视觉语言模型在生成过程中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM的不确定性量化方法大多从LLM领域直接迁移，主要关注语言模态，忽视了视觉信息对不确定性的贡献。作者通过分析发现，高置信度预测比低置信度预测更依赖视觉内容，因此需要一种能显式利用视觉信号的不确定性估计方法。

Method: 提出Visual-Grounded Token UQ (VIG-TUQ)，一种无需训练的框架。该方法在生成过程中分析视觉特征融合后的隐藏表示，计算每个token的视觉接地分数（visual grounding score），并用该分数加权token级语言不确定性，从而将视觉信息显式纳入不确定性估计。

Result: 在多个数据集和多种LVLM架构（early-fusion、late-fusion、native-fusion）上评估，VIG-TUQ在token级不确定性估计上优于现有方法。

Conclusion: 视觉信号对LVLM的不确定性量化有重要贡献，VIG-TUQ通过显式引入视觉接地信息，有效提升了不确定性估计的准确性，为LVLM的可靠部署提供了新思路。

Classification reason: 该论文研究视觉语言模型的不确定性量化（UQ），旨在提升模型预测的可靠性与忠实性，属于多模态幻觉与可靠性范畴。论文提出的VIG-TUQ方法通过视觉信号改进token级不确定性估计，直接对应[多模态幻觉与可靠性]这一CANONICAL subtopic中关于可靠性（reliability）和忠实性（faithfulness）的核心关注点。

Importance reason: Addresses a niche reliability issue in multimodal models but lacks direct connection to user's priority generation and efficiency topics. Signals: uncertainty quantification, vision-language grounding, training-free, token-level, LVLM reliability
...
### [170] [RICE-PO: Turning Retrieval Interactions into Credit Signals for Reasoning Agents](https://arxiv.org/abs/2605.26352)
...
TL;DR: RICE-PO 提出了一种无需评论模型的策略优化框架，通过将检索交互转化为局部学习信号，解决推理智能体在检索任务中的信用分配问题，在 BRIGHT 和 BEIR 上优于提示词智能体和基于组的 RL 基线。


<details>
  <summary>Details</summary>
Motivation: 检索正从一次性匹配转向交互式推理，语言智能体需要迭代检查证据、重构查询并重新搜索。训练这类智能体面临信用分配挑战：可执行动作（如查询、摘要）可直接由检索器评估，而潜在推理步骤不可直接观察，仅影响未来可执行动作。这种不对称性使得基于最终结果的奖励分配不可靠。

Method: RICE-PO 选择高不确定性的可执行动作作为锚点，使用检索指标评估局部反事实分支，仅在推理到动作的影响强且未来残差效应稳定时，将信用传播到潜在推理步骤。该方法无需评论模型（critic-free），完全利用智能体-环境交互结构提供监督信号。

Result: 在 BRIGHT 和 BEIR 基准上，RICE-PO 在相同检索器设置下一致优于基于提示词的智能体和基于组的强化学习基线。

Conclusion: 智能体-环境交互的结构本身可以为训练基于推理的检索智能体提供有用的监督信号，RICE-PO 通过将检索交互转化为信用信号有效解决了推理智能体的信用分配问题。

Classification reason: 该论文研究基于检索交互的推理智能体策略优化，属于多模态/文本推理范畴，与 [CANONICAL] subtopic "多模态推理与思维链"（multimodal_reasoning_cot）高度相关，因此复用该 subtopic。

Importance reason: Focuses on retrieval agent credit assignment, not directly aligned with user's priority subtopics in generation or world models. Signals: retrieval agent training, credit assignment, policy optimization, interactive reasoning, no critic model
...
### [171] [InterSketch: An Interleaved Reasoning Model with Self-correcting Visual Sketch and Stepwise Reward](https://arxiv.org/abs/2605.26520)
...
TL;DR: InterSketch 是一种交错推理模型，通过自纠正和逐步奖励机制增强视觉-文本交错思维链（VT-CoT）能力。模型在冷启动阶段使用合成的高质量交错 VT-CoT 数据集和反思机制进行训练，随后在强化学习阶段设计逐步奖励机制以缓解长程推理中的奖励稀疏问题。在视觉推理基准上，InterSketch 甚至超越了 Gemini-3-Pro 等闭源模型。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 的推理轨迹较浅且以文本为中心，缺乏人类式长程交错视觉-文本思维链能力，限制了复杂视觉任务的推理表现。

Method: 提出 InterSketch 模型，包含两个阶段：(1) 冷启动阶段，构建高质量交错 VT-CoT 数据集并引入反思机制，使模型具备多轮交错推理和自我纠正能力；(2) 强化学习阶段，设计逐步奖励机制，缓解长程推理中仅末端监督带来的奖励稀疏问题。模型在推理过程中动态调用外部工具生成中间视觉草图，与文本推理交错进行。

Result: 在多个视觉推理基准上，InterSketch 显著优于现有方法，甚至超越了 Gemini-3-Pro 等闭源模型，验证了交错视觉-文本推理范式的有效性。

Conclusion: 交错视觉-文本思维链结合自纠正和逐步奖励机制能有效提升 VLM 在复杂视觉推理任务上的表现，为多模态推理提供了新的范式。

Classification reason: 该论文提出 InterSketch，通过交错的视觉-文本思维链（VT-CoT）增强多模态推理能力，属于多模态推理与思维链方向，与 [CANONICAL] subtopic "多模态推理与思维链" 高度匹配。

Importance reason: Strong on multimodal reasoning but lacks direct alignment with priority subtopics like autoregressive video generation or world models. Signals: interleaved visual-textual reasoning, self-correcting visual sketch, stepwise reward for long-horizon reasoning, outperforms Gemini-3-Pro, multimodal understanding generation
...
### [172] [Reasoning, Code, or Both? How Large Language Models Handle Variations in Math Questions](https://arxiv.org/abs/2605.26414)
*Matthew Kutakh*
...
TL;DR: 本文系统评估了三种方法（纯CoT推理、单次代码执行PAL、迭代代码执行SBSC）在GSM-Symbolic数据集上对数学问题变体的鲁棒性。结果表明，CoT是最鲁棒的方法，代码执行并未提升推理鲁棒性，差异虽未达统计显著但趋势一致。


<details>
  <summary>Details</summary>
Motivation: LLM在数学推理基准上表现优异，但问题中简单变化（如换名字、数字）会导致性能显著下降。代码执行方法被提出作为解决方案，但其对推理鲁棒性的影响尚未被系统测试。

Method: 使用GSM-Symbolic数据集中的1000个问题，在原始和修改后的问题对上，使用Claude Haiku 4.5评估三种方法：纯CoT推理、单次代码执行（PAL）、迭代代码执行（SBSC），比较准确率下降幅度和问题"断裂"比例。

Result: CoT准确率下降1.3个百分点（1.8%问题断裂），PAL下降1.7个百分点（3.1%断裂），SBSC介于两者之间。差异未达统计显著（p=.096），但趋势一致：代码执行并未提升鲁棒性。

Conclusion: 在小学数学级别的问题变体上，代码执行方法（无论是单次还是迭代）并未改善LLM的推理鲁棒性，纯CoT推理反而表现最为稳定。

Classification reason: 该论文研究LLM在数学推理问题上的鲁棒性，比较了纯推理（CoT）与代码执行方法（PAL、SBSC）在不同问题变体下的表现，属于多模态推理与思维链（数学推理）范畴，因此映射到 canonical subtopic "多模态推理与思维链"。

Importance reason: Focuses on math reasoning robustness, which is not a priority subtopic and lacks direct connection to multimodal generation or world models. Signals: math reasoning robustness, code execution vs CoT, not statistically significant, grade-school-level problems, no multimodal or generation focus
...
### [173] [Composition Collapse: Stable Factual Knowledge Does Not Imply Compositional Reasoning](https://arxiv.org/abs/2605.26789)
...
TL;DR: 本文发现后训练模型在原子知识稳定性上无显著差异时，组合推理能力可能相差超过40个百分点，这种现象称为"组合坍缩"（composition collapse）。作者提出双门控协议（double-gate protocol）将组合失败分解为原子稳定性、残差组合和关键深度三个独立通道，揭示了聚合指标掩盖的后训练对组合能力的影响。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法将多跳推理视为单一能力，用聚合指标衡量模型表现，但这种方法可能掩盖模型在组合已知事实时的系统性失败。作者希望揭示并量化这种被聚合指标掩盖的组合推理失败现象。

Method: 提出双门控协议（double-gate protocol），将评估目标从聚合的组合性差距转变为"在稳定原子知识访问条件下的残差组合失败"，将后训练收益分解为原子稳定性、残差组合和关键深度三个独立通道。在深度2-11的时间事实链基准上对四种后训练方案进行实验，并使用诊断探针分析生成时计算约束的影响。

Result: 不同后训练方案在原子知识稳定性上统计无显著差异时，组合行为差距超过40个百分点。分解分析显示后训练目标对组合能力的影响方向被聚合指标掩盖。诊断探针表明相当一部分组合失败源于生成时的计算约束而非模型永久性的组合能力缺失。

Conclusion: 聚合指标不足以评估多跳推理能力，应辅以原子门控的组合指标。后训练目标对组合能力的影响是复杂且非单调的，需要更精细的评估协议来揭示真实能力变化。

Classification reason: 该论文研究多跳组合推理（multi-hop compositional reasoning）的失败模式，属于多模态推理范畴。虽然论文使用文本事实链作为实验场景，但其核心关注的是组合推理能力本身，与 canonical subtopic "多模态推理与思维链" 中的推理主题高度吻合。

Importance reason: Strong methodological contribution to evaluating compositional reasoning in multimodal models, but subtopic is not a user priority. Signals: Composition collapse phenomenon, Double-gate protocol for decomposition, Post-training evaluation critique, Temporal factual chains benchmark, Atomic stability vs. composition gap
...
### [174] [Touch-R1: Reinforcing Touch Reasoning in MLLMs](https://arxiv.org/abs/2605.27154)
...
TL;DR: 本文提出 Touch-R1，一个基于 Qwen2.5-VL-7B 的触觉推理多模态大模型，通过构建大规模触觉数据集 TouchReason-1M 和评测基准 TouchReason-Bench，并设计触觉导向的 GRPO 训练目标，使模型在触觉感知和视觉-触觉冲突解决任务上显著超越现有方法，展现出类似 R1 的显式推理行为。


<details>
  <summary>Details</summary>
Motivation: 现有触觉-语言模型主要依赖监督学习或对比学习目标，缺乏显式推理能力，无法基于物理证据进行预测或纠正误导性的视觉先验。触觉推理面临两个特有挑战：物理属性（如硬度、粗糙度）的序数性质，以及不同触觉传感器之间的分布偏移。

Method: 构建 TouchReason-1M 数据集（含 4 种传感器、超 100 万同步触觉对）和 TouchReason-Bench 评测框架。提出 Touch-R1 模型，基于 Qwen2.5-VL-7B，采用触觉导向的 GRPO 训练目标，包含序数感知准确率、跨传感器物理一致性、结构化格式控制以及输入侧触觉接地目标。其中触觉使用奖励仅在真实触觉输入比反事实控制（移除、打乱或噪声掩码触觉流）更正确时才赋予奖励。

Result: 在 TouchReason-Bench 上，Touch-R1-7B 平均性能超过 Octopi-13B 18.4%、超过 GPT-4o 24.7%。结构化推理轨迹展现出 probing、comparison、revision 等涌现行为，证明 R1 风格的推理可以有效接地于物理接触。

Conclusion: 本文证明了规则强化学习可以有效扩展到触觉推理领域，触觉导向的 GRPO 训练能够激发模型显式的物理推理行为，为多模态推理在触觉感知方向的发展提供了新范式。

Classification reason: 该论文研究触觉推理（tactile reasoning），属于多模态推理范畴，与 [CANONICAL] 多模态推理与思维链（multimodal_reasoning_cot）高度匹配。论文使用 GRPO 强化触觉推理，涉及推理轨迹的 probing、comparison、revision 等推理行为，符合该子主题定义。

Importance reason: Novel tactile reasoning MLLM with RL training, but outside user's priority subtopics and negative keywords apply. Signals: multimodal reasoning, reinforcement learning, tactile perception, visual-tactile conflict, GRPO training
...
TL;DR: 本文是 MM 2026 上举办的第二届 EReL@MIR 研讨会提案，聚焦于大模型时代多模态信息检索中表示学习的效率瓶颈问题，旨在召集学界与工业界研究者共同探讨高效表示学习的新方法、开放挑战以及新的效率评测指标与基准。


<details>
  <summary>Details</summary>
Motivation: 大规模预训练多模态基础模型（如 Qwen、LLaVA、CLIP）在多种多模态信息检索任务上表现优异，但其庞大的参数量在训练、部署和推理阶段造成了严重的效率瓶颈，限制了基础模型在信息检索中表示学习的实际应用。

Method: 组织 EReL@MIR 研讨会，通过征集论文、邀请报告和讨论环节，汇聚学术界与工业界研究者，共同探讨多模态信息检索中高效表示学习的新兴解决方案、开放挑战，并推动新的效率指标和基准的建立。

Result: 该研讨会提案已被 MM 2026 接收，设有官方网站，预期将促进多模态信息检索中高效表示学习领域的研究交流与进展。

Conclusion: EReL@MIR 研讨会为应对大模型时代多模态信息检索中的效率挑战提供了重要的学术交流平台，有助于推动高效表示学习方法的发展和新评测标准的建立。

Classification reason: 该论文是关于多模态信息检索（MIR）中高效表示学习的研讨会提案，核心关注多模态检索的效率问题。现有 taxonomy 中 multimodal_understanding_generation 方向下已有 [CANONICAL] 的 "多模态检索"（multimodal_retrieval）子主题，与该论文主题高度匹配，因此直接复用。

Importance reason: Workshop proposal with no technical contributions, low priority for daily deep read. Signals: workshop proposal, no technical results, multimodal retrieval, efficiency focus, not a research paper

Abstract: Multimodal representation learning has attracted increasing attention in AI, driven by the strong performance of large, pretrained multimodal foundation models such as Qwen, LLaVA, and CLIP. These models deliver impressive performance on a range of multimodal information retrieval (MIR) tasks, including web search, cross-modal retrieval, and recommender systems. Yet their massive parameter counts create major efficiency bottlenecks when adapting their representations for IR tasks during training, deployment, and inference. These limitations hinder the practical use of foundation models for representation learning in information retrieval. To address these issues, we propose organizing the EReL@MIR workshop at MM 2026, bringing together researchers from academia and industry to discuss emerging solutions, open challenges, and new efficiency metrics and benchmarks for multimodal IR representation learning in the foundation-model era. The workshop's official website is available at .

</details>


### [176] [OmniRetriever: Any-to-Any Audio-Video-Text Retrieval via Fusion-as-Teacher Distillation](https://arxiv.org/abs/2605.26641)
...
TL;DR: OmniRetriever-7B 提出融合即教师蒸馏（Fusion-as-Teacher Distillation）方法，利用联合音视频文本嵌入作为教师信号指导单模态嵌入学习，并引入 Tuple-InfoNCE 损失监督融合嵌入，在零样本音视频文本检索上超越 Gemini Embedding 2 等闭源模型，同时发布 OmniRetriever-Bench 基准。


<details>
  <summary>Details</summary>
Motivation: 现有音视频文本（AVT）编码器虽能生成联合嵌入，但标准 InfoNCE 损失仅利用成对信号，忽略了联合嵌入中蕴含的丰富跨模态信息。作者希望弥合这一差距，充分利用融合嵌入来提升单模态检索性能。

Method: 提出融合即教师蒸馏（Fusion-as-Teacher Distillation）框架：对融合嵌入使用 stop-gradient 副本作为教师信号，蒸馏到单模态嵌入；同时引入 Tuple-InfoNCE 损失直接监督融合嵌入。基于此目标训练 OmniRetriever-7B 模型。

Result: 在 Clotho 和 SoundDescs 上零样本检索 R@1 超越 Gemini Embedding 2 达 13.3-18.0；在 MSR-VTT 和 MSVD 上达到开放视频文本编码器水平；在自建 OmniRetriever-Bench 上 AVG-all 达 34.84，优于 Gemini Embedding 2 1.72 分，优于最佳开源 AVT 方法 8.03 分。

Conclusion: 融合即教师蒸馏能有效利用联合嵌入信号提升跨模态检索性能，OmniRetriever-7B 在多个零样本基准上达到领先水平，OmniRetriever-Bench 为音视频文本检索提供了更全面的评测基准。

Classification reason: 该论文提出 OmniRetriever，实现音频-视频-文本任意到任意的跨模态检索，属于多模态检索方向。该 subtopic 在 multimodal_understanding_generation 和 multimodal_generation 两个方向下均已存在（id: multimodal_retrieval），且为 [CANONICAL] 条目，直接复用。

Importance reason: Strong retrieval method but not aligned with user's priority subtopics on generation or world models. Signals: Fusion-as-Teacher distillation, Tuple-InfoNCE, zero-shot AVT retrieval, surpasses Gemini Embedding 2, OmniRetriever-Bench benchmark
...
### [177] [FAST-GOAL: Fast and Efficient Global-local Object Alignment Learning](https://arxiv.org/abs/2605.26615)
...
TL;DR: FAST-GOAL 是一种高效的 CLIP 微调方法，通过全局-局部语义对齐（FLISM + TSL）增强模型对长文本描述的匹配能力，并引入 GLIT100k 数据集，在长/短描述检索任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: CLIP 等视觉语言模型在短文本描述上表现优异，但面对长文本详细描述时性能显著下降，原因是预训练数据多为简短标题。现有方法难以高效地将 CLIP 适配到长文本场景。

Method: 提出 FAST-GOAL，包含两个核心组件：(1) FLISM（快速局部图像-句子匹配），通过目标检测和空间划分提取局部图像区域并与对应句子匹配；(2) TSL（基于 Token 相似度的学习），最大化图像特定区域 patch token 与对应区域嵌入的相似度，并对文本应用相同原则。同时构建 GLIT100k 数据集，提供全局图像-长描述对及上下文一致的局部描述对。

Result: 在长描述数据集（DOCCI、DCI）和短描述数据集（MSCOCO、Flickr30k）上，FAST-GOAL 显著优于基线方法，有效提升了 CLIP 对详细文本描述的适应能力，同时保持计算效率。

Conclusion: FAST-GOAL 通过高效的全局-局部对齐微调，成功增强了 CLIP 处理长文本描述的能力，为视觉语言模型在细粒度跨模态检索任务中的应用提供了有效方案。

Classification reason: 该论文提出 FAST-GOAL 方法，通过全局-局部语义对齐微调 CLIP 模型以提升其对长文本描述的匹配能力，核心任务是图像-文本跨模态对齐与检索，因此映射到 [CANONICAL] 多模态检索（multimodal_retrieval）子主题。

Importance reason: Focuses on CLIP fine-tuning for long-text retrieval, which is not a priority subtopic and lacks strong alignment with user's boosted keywords. Signals: CLIP fine-tuning, global-local alignment, long caption retrieval, efficient method, GLIT100k dataset
...
### [178] [CIRCLED: A Multi-turn CIR Dataset with Consistent Dialogues across Domains](https://arxiv.org/abs/2605.26734)
...
Importance: 普通相关 (43.7) - 未入选精读

TL;DR: 本文提出了 CIRCLED，一个大规模多轮组合图像检索（MTCIR）数据集，通过扩展 FashionIQ、CIRR 和 CIRCO 三个现有数据集，构建了 22,608 个多轮对话会话，覆盖多个领域，并保证了对话历史一致性。论文还提供了多种基线方法的定量评估结果。


<details>
  <summary>Details</summary>
Motivation: 现有 MTCIR 数据集（如 Multi-turn FashionIQ）存在两个主要问题：一是对话历史缺乏一致性（query 之间逻辑不连贯），二是仅限于时尚领域。本文旨在构建一个更大规模、跨领域且对话历史一致的多轮组合图像检索基准数据集。

Method: 基于 CIReVL 检索流水线生成多轮查询数据，通过多个过滤步骤（检索成功率、轮次长度、一致性、信息冗余度）确保数据质量。扩展了 FashionIQ、CIRR、CIRCO 三个数据集，共构建 9 个子集、22,608 个多轮会话。采用多种基线方法（如 CIReVL、BLIP-2 等）进行检索性能评估。

Result: CIRCLED 数据集规模（22,608 个会话）显著超过 Multi-turn FashionIQ（11,505 个会话），覆盖多个领域。基线实验表明，现有方法在多轮 CIR 任务上仍有较大提升空间，验证了该基准的挑战性和实用性。

Conclusion: CIRCLED 提供了一个大规模、跨领域、对话一致的多轮组合图像检索基准，有助于推动该方向的研究。数据集和代码已开源。

Classification reason: 该论文研究多轮组合图像检索（Multi-Turn Composed Image Retrieval），属于跨模态/组合检索范畴，与现有 canonical subtopic "多模态检索"（multimodal_retrieval）高度匹配。同时该工作构建了数据集和基准，也涉及多模态生成方向中的检索相关任务。

Importance reason: Focuses on a niche multi-turn retrieval dataset with no alignment to user's priority subtopics or boost keywords. Signals: dataset paper, multi-turn retrieval, no autoregressive/diffusion/world model, negative keywords absent but no positive signals
...
### [179] [Query Symbolically or Retrieve Semantically? A Dataset and Method for Semi-Structured Question Answering](https://arxiv.org/abs/2605.27164)
...
TL;DR: 本文提出 DualGraph，一个面向半结构化文档的 RAG 框架，通过文本知识图谱（语义检索）和符号知识图谱（符号查询）双视图互补，并构建了电商产品文档问答基准 SpecsQA，实验表明该方法在开放性和规格性问题上均优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有 RAG 系统主要依赖语义相似度检索非结构化文本，但在半结构化语料（如电商产品文档）上效果不佳，因为这类问答需要精确过滤、聚合或跨文档的穷举检索。纯符号方法虽支持此类操作，但在噪声自然语言语料上脆弱。因此需要一种结合语义检索与符号查询的混合方法。

Method: 提出 DualGraph 框架，将文档表示为两种互补视图：文本知识图谱（Textual KG）用于语义检索，符号知识图谱（Symbolic KG）用于对类型化的主-谓-宾三元组进行符号查询。基于这两个组件，设计了多种策略来选择或组合语义与符号检索路径。同时构建了 SpecsQA 基准，包含来自电商网站的半结构化产品文档和人工标注的开放性与规格性问题。

Result: 实验表明，DualGraph 在开放性和规格性问题上均一致优于最先进的稠密检索、GraphRAG、纯符号和表格导向的基线方法。

Conclusion: DualGraph 通过语义与符号双视图互补，有效解决了半结构化文档问答中语义检索与精确查询的权衡问题，为 RAG 系统在结构化数据场景下的应用提供了新思路。

Classification reason: 该论文提出半结构化问答数据集 SpecsQA 和 DualGraph 框架，结合语义检索与符号查询进行文档检索增强问答，属于多模态/文档检索方向。multimodal_understanding_generation 方向下已有 [CANONICAL] subtopic "多模态检索"（multimodal_retrieval），该论文的文档检索与问答任务与之高度匹配，因此复用该 subtopic。

Importance reason: Novel RAG framework for semi-structured QA, but outside user's core generative model priorities. Signals: RAG framework, semi-structured QA, knowledge graph, benchmark creation, not generative model
...
### [180] [Can Retrieval Heads See Images? Multimodal Retrieval Heads in Long-Context Vision-Language Models](https://arxiv.org/abs/2605.27243)
...
TL;DR: 本文提出多模态检索头检测方法，发现长上下文视觉语言模型中仅有 4.4-10.2% 的注意力头是多模态检索头，它们稀疏、固有且因果重要——掩码前 5% 的检索头导致 MMLongBench-Doc 从 48.2% 降至 5.7%。这些头在模态间部分共享但模态内动态变化，且无需额外训练即可直接用于视觉丰富文档的排序检索。


<details>
  <summary>Details</summary>
Motivation: 先前工作在大语言模型中研究了检索头（retrieval heads），但其基于复制（copy-based）的判据不适用于证据出现在图像中的场景。随着视觉语言模型需要处理文档、长视频和智能体轨迹等长上下文，如何定位跨文本和图像的证据成为一个关键问题，但目前缺乏检测多模态检索头的方法。

Method: 提出多模态检索头检测方法，通过评分注意力从问题 token 到文本或视觉证据的注意力权重来识别检索头。在 Qwen3-VL-8B 等模型上分析检索头的稀疏性、固有性和因果重要性，并通过掩码实验验证其因果作用。进一步将检索头直接用于文档排序任务（MMDocIR），无需额外训练。

Result: 多模态检索头仅占 4.4-10.2% 的注意力头，但贡献了 50% 的正检索分数质量。掩码前 5% 的检索头导致 MMLongBench-Doc 从 48.2% 降至 5.7%，SlideVQA 从 71.2% 降至 8.9%。在 MMDocIR 上，Qwen3-VL-8B 的检索头评分在页面检索上 Recall@1 提升 7.7/7.4 macro/micro 点，布局检索提升 6.3/6.8 点。

Conclusion: 多模态检索头是稀疏、固有且因果重要的注意力头，在模态间部分共享但模态内动态变化。这些头无需训练即可直接用于视觉文档检索，显著超越现有基线。该发现为理解长上下文视觉语言模型的内部机制和提升多模态检索能力提供了新视角。

Classification reason: 该论文研究长上下文视觉语言模型中的多模态检索头（retrieval heads），核心是检测和利用注意力头进行跨文本和图像的证据检索与文档排序，属于多模态检索范畴。现有 taxonomy 中 multimodal_retrieval（多模态检索）是 [CANONICAL] 子主题，完美匹配。

Importance reason: 揭示了长上下文多模态模型中稀疏检索头的因果重要性，对理解多模态注意力机制有贡献，但偏离用户核心生成与效率主题。 Signals: 多模态检索头, 长上下文视觉语言模型, 注意力机制分析, 无需训练检索, 因果重要性验证
...
### [181] [Gemini Embedding 2: A Native Multimodal Embedding Model from Gemini](https://arxiv.org/abs/2605.27295)
...
TL;DR: Gemini Embedding 2 是一个原生多模态嵌入模型，能够将视频、音频、图像和文本嵌入到统一的表示空间中。通过大规模对比学习和多阶段多任务训练，在单模态、跨模态和多模态检索基准上达到SOTA性能，超越专用模型，适用于RAG、推荐和搜索等下游任务。


<details>
  <summary>Details</summary>
Motivation: 现有的嵌入模型通常针对单一模态（如文本或图像）设计，缺乏统一处理多种模态（视频、音频、图像、文本）的能力，且难以处理任意组合的交错输入。作者希望利用Gemini的多模态能力，构建一个原生多模态嵌入模型，在统一的表示空间中同时支持多种模态的嵌入，并泛化到广泛的下游任务。

Method: 基于Gemini模型的多模态能力，采用大规模对比学习（contrastive learning）在多任务多阶段训练框架下进行训练。模型支持视频、音频、图像和文本四种模态的任意组合作为输入，输出统一的嵌入表示。训练过程包含多个阶段和任务，以提升跨模态对齐和泛化能力。

Result: 在多个基准上取得SOTA性能：MSCOCO上R@1为62.9，Vatex上NDCG@10为68.8，MTEB多语言任务上69.9，MTEB代码任务上84.0。在零样本场景下，从天文学、生物科学到美术和烹饪艺术等专业领域均表现出色，超越了专门的单模态/跨模态模型。

Conclusion: Gemini Embedding 2 证明了原生多模态嵌入模型的有效性，统一的嵌入空间能够同时处理多种模态并超越专用模型。该模型为RAG、推荐系统和搜索等下游应用提供了强大的基础能力，且在零样本跨领域泛化方面表现出高可靠性。

Classification reason: 该论文提出原生多模态嵌入模型，支持视频、音频、图像、文本的统一嵌入表示，并在单模态、跨模态和多模态检索任务上取得SOTA。这完全匹配多模态理解与生成方向下的[CANONICAL] subtopic "多模态检索"（multimodal_retrieval），该subtopic涵盖跨模态检索、多模态搜索等核心内容。

Importance reason: Strong multimodal embedding model with SOTA retrieval performance, but not directly aligned with top priority subtopics like autoregressive or diffusion video generation. Signals: Native multimodal embedding, SOTA on MSCOCO, Vatex, MTEB, unified video/audio/image/text, strong zero-shot, RAG and search use case
...
TL;DR: 本文提出 LiveK12Bench，一个动态、多学科的高中水平考试评测基准，包含2000+道来自最新真实试卷的数学、物理、化学、生物题目。该基准通过自动化流水线持续更新以缓解数据污染，并引入"模拟考试"评估方案，考察模型自主完成端到端考试的能力。实验表明，在真实考试约束下，GPT-5 的得分从79降至53，揭示了现有 LMM 在教育场景中的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有 K-12 多模态评测基准存在静态、易受数据污染、模态/学科/评估标准受限等问题，无法真实反映模型在复杂考试环境中的推理能力，亟需一个动态、全面、贴近真实考试场景的评测基准。

Method: 1) 构建 LiveK12Bench，包含2000+道来自最新真实高考试卷的题目，覆盖数学、物理、化学、生物四大学科；2) 设计自动化流水线持续摄入和解析最新试卷以缓解数据泄露；3) 提出"模拟考试"评估方案，综合评估模型自主完成端到端考试的能力，兼顾过程严谨性和效率。

Result: 在12个 LMM 上的实验表明，先进模型在真实考试约束下性能显著下降：GPT-5 的得分从79降至53（满分100）。模型对复杂视觉布局敏感，暴露了理想化推理能力与真实教育就绪度之间的差距。

Conclusion: LiveK12Bench 揭示了当前 LMM 在真实高中考试场景中的严重性能退化，表明模型尚未真正掌握高中水平的推理能力，为未来教育场景多模态模型的研究提供了重要评测工具和方向指引。

Classification reason: 该论文提出了一个面向高中水平考试的多模态推理评测基准 LiveK12Bench，属于多模态评测与基准方向，与 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark) 高度匹配。

Importance reason: Directly evaluates multimodal understanding in realistic exam scenarios, aligning with user's priority on unified multimodal models and revealing critical vulnerabilities. Signals: multimodal understanding evaluation, dynamic benchmark, data contamination mitigation, mock exam scheme, GPT-5 performance drop
...
### [183] [QUACK: Questioning, Understanding, and Auditing Communicated Knowledge in Multimodal Social Deduction Agents](https://arxiv.org/abs/2605.27068)
...
TL;DR: QUACK 是一个开源的多模态社交推理游戏评测框架，通过 Statement Verification Pipeline 自动检测智能体语言与真实行为之间的不一致性（空间幻觉、无证据指控、欺骗崩溃等），实验发现最强 VLM 智能体也有 15.1% 的空间声明存在幻觉，超过一半的指控缺乏证据。


<details>
  <summary>Details</summary>
Motivation: 现有社交推理游戏环境仅以胜率等游戏结果评分，且多为纯文本交互，难以判断智能体的语言是否真正基于其感知和行为，也无法识别其行为背后的失败模式。

Method: 提出 QUACK 框架，包含三层评估（游戏结果、行为轨迹、话语一致性），核心是 Statement Verification Pipeline，从引擎日志重建每个智能体的真实轨迹，并自动检查其讨论中的每个声明是否与真实轨迹一致，标记空间幻觉、无证据指控、欺骗崩溃和语言-行为不一致。

Result: 在三种前沿 VLM 的同质和跨模型对抗设置下评估，发现即使最强的智能体也有 15.1% 的可验证空间声明存在幻觉，超过一半的指控缺乏证据支持。

Conclusion: QUACK 提供了一个细粒度的多模态社交推理智能体评估框架，揭示了当前 VLM 智能体在语言-行为一致性方面的显著缺陷，为未来多模态智能体的可信交互研究提供了基准和工具。

Classification reason: 该论文提出了一个用于多模态社交推理智能体的评测框架 QUACK，包含游戏结果、行为轨迹和话语一致性三个层次的评估，属于多模态评测与基准方向。同时涉及 AIGC 智能体与生成工作流方向（多智能体社交推理环境）。

Importance reason: Focuses on multimodal social deduction game evaluation, not aligned with user's priority subtopics like video generation or model efficiency. Signals: multimodal evaluation, social deduction, VLM hallucination, statement verification, game-based benchmark
...
### [184] [Benchmarking Convolutional, Transformer, Hybrid, and Vision Language Models for Multi Disease Retinal Screening](https://arxiv.org/abs/2605.26283)
...
TL;DR: 本文在视网膜眼底图像多疾病数据集 RFMiD 上系统基准测试了 12 种架构（CNN、ViT、混合 CNN-Transformer、视觉语言模型），评估二分类筛查和 28 类多标签分类任务，发现注意力机制模型（SwinTiny、CoAtNet0、MaxViTTiny）表现最优，视觉语言模型（CLIP、SigLIP）与 CNN 基线相当但未超越最佳 Transformer/混合模型。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对不同视觉模型家族（CNN、Transformer、混合模型、视觉语言模型）在真实多疾病视网膜筛查场景下系统、标准化的比较，尤其是在领域迁移下的表现尚不明确。

Method: 使用 RFMiD 数据集，对 12 种架构（包括 CNN 类：ResNet、DenseNet、EfficientNet；ViT 类：ViT、SwinTiny；混合类：CoAtNet、MaxViT；VLM 类：CLIP、SigLIP）进行标准化训练、校准和评估，在二分类和多标签分类任务上报告 AUC、F1、精确率、召回率等指标，并在 Messidor-2 上进行外部验证。

Result: 所有架构在二分类筛查中 AUC 均超过 84%；SwinTiny、CoAtNet0 和 MaxViTTiny 在二分类和多标签任务上表现最佳；视觉语言模型与 CNN 基线竞争力相当但未超越最佳 Transformer/混合模型；外部验证中 AUC 范围为 66.8%-84.7%。

Conclusion: 注意力机制模型在多疾病视网膜筛查中表现最优，为临床部署的自动化筛查工具提供了可复现的模型选择参考。

Classification reason: 该论文对多种视觉模型（CNN、ViT、混合模型、视觉语言模型）在视网膜疾病筛查任务上进行系统基准评测，属于多模态（视觉）评测与基准方向，映射到 [CANONICAL] subtopic "多模态评测与基准"。

Importance reason: Focuses on medical retinal screening, which is outside the user's priority subtopics and is flagged by negative keywords. Signals: Medical diagnosis, negative keyword match, no autoregressive or video generation, no world model or efficient inference
...
### [185] [EpiCurveBench: Evaluating VLMs on Epidemic Curve Digitization](https://arxiv.org/abs/2605.27195)
...
TL;DR: 本文提出 EpiCurveBench——一个包含 1000 张真实流行病曲线图像的基准测试，以及 EpiCurveSimilarity (ECS) 评估指标，用于评估视觉语言模型从图表中提取时序数据的能力。实验表明最强模型仅达到 52.3% ECS，且 ECS 能更好区分不同模型性能，并与下游流行病学统计指标高度相关。


<details>
  <summary>Details</summary>
Motivation: 现有图表数据提取基准（如 ChartQA）已接近饱和（前沿 VLM 超过 89%），且评估指标将提取点视为无序键值对，忽略了时间序列的时序结构，将小的对齐偏移视为灾难性失败。这阻碍了将 VLM 应用于公共卫生领域——从已发表图表中解锁数十年疫情数据。

Method: 构建了包含 1000 张真实流行病曲线图像的 EpiCurveBench 基准，数据来自多种公共卫生来源。提出了 EpiCurveSimilarity (ECS) 指标，通过动态规划对齐预测序列与真实序列，容忍局部时间偏移和间隙并给予比例惩罚。评估了 6 种方法（3 个前沿闭源 VLM、1 个开源 VLM、2 个专用图表提取系统）。

Result: 最强模型仅达到 52.3% ECS；ECS 将 4 个通用 VLM 分散在 25 个百分点的范围内，而键值指标（RMS, SCRM）将其压缩在 5 个百分点内。ECS 与四个下游流行病学统计指标（总计数、峰值时间、峰值幅度、增长率保真度）的相关性比 DTW 强 1.5-3.6 倍。

Conclusion: EpiCurveBench 揭示了现有 VLM 在结构化时序图表数据提取方面的显著不足，ECS 指标能更有效地区分模型性能并预测下游任务准确性。该基准和指标可直接应用于任何结构化时序图表提取场景，具有重要的公共卫生应用价值。

Classification reason: 该论文提出了 EpiCurveBench，一个针对流行病曲线图表数据提取的基准测试，并提出了新的评估指标 ECS。这属于多模态评测与基准（multimodal_benchmark）范畴，是 [CANONICAL] 子主题，直接匹配。

Importance reason: Niche benchmark for epidemic curve digitization, far from user's priority subtopics in generative AI and efficient models. Signals: Specialized public-health chart extraction, low relevance to autoregressive/diffusion video/image generation, no efficiency or world model angle, negative keywords (clinical) partially match
...
### [186] [VisualNeedle: Benchmarking Active Visual Search in Information-Dense Scenes](https://arxiv.org/abs/2605.26380)
...
TL;DR: VisualNeedle 是一个挑战性多模态基准，用于评测 MLLM 在信息密集场景中通过主动视觉搜索定位微小关键证据的能力。论文提出 counterfactual crop-black 设置来检验工具辅助模型是否真正依赖中间视觉证据。实验表明，无工具准确率低于 20%，最佳工具辅助模型仅 56.01%，远低于人类 63.00% 的多数投票准确率，揭示了当前 MLLM 在细粒度视觉搜索上的持续局限。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 在细粒度感知基准上声称达到 90%+ 准确率，但先前研究发现这些高分存在三种捷径：语言先验与词汇线索、视觉编码器的粗粒度全局语义、以及工具返回的中间图像被破坏后最终答案几乎不受影响。这表明更高分辨率或更大问题池并不能引发真正的主动视觉搜索。因此需要一个新的基准来严格评测模型在信息密集场景中主动搜索微小关键证据的能力。

Method: 提出了 VisualNeedle 基准，包含信息密集、细粒度的场景，其中关键证据在空间上局限于微小区域且无法一眼识别。同时提出 counterfactual crop-black 设置，将工具返回的裁剪区域替换为相同大小的黑色图像，以检验工具辅助性能是否真正依赖中间视觉证据。在无工具、标准工具辅助和 crop-black 三种设置下评测了 9 个主流 MLLM。

Result: 无工具准确率低于 20%，最佳工具辅助模型仅达到 56.01%，仍低于人类多数投票准确率 63.00%。crop-black 消融实验证实，在 VisualNeedle 上的成功依赖于真实的中间视觉证据。

Conclusion: 当前 MLLM 在需要主动视觉搜索的信息密集场景中表现有限，即使借助工具也远未达到人类水平。VisualNeedle 可作为评估模型真实视觉搜索能力的有效基准，推动该方向的发展。

Classification reason: 该论文提出了 VisualNeedle，一个针对信息密集场景中主动视觉搜索能力的细粒度多模态评测基准，属于多模态评测与基准方向，因此映射到 canonical subtopic "multimodal_benchmark"（多模态评测与基准）。

Importance reason: Strong benchmark for MLLM fine-grained visual search, but not aligned with user's priority on generative models and efficiency. Signals: MLLM evaluation, fine-grained perception, active visual search, counterfactual crop-black, tool-use ablation
...
TL;DR: 提出Chartographer框架，通过将图表逆向工程为可执行代码并生成反事实变体，构建反事实图表QA基准以严格评估VLM的视觉推理能力，揭示模型在单图表评测中隐藏的泛化失败。


<details>
  <summary>Details</summary>
Motivation: 现有图表QA基准中，模型常通过捷径或背景知识而非真正的视觉推理来回答问题，导致评测结果不可靠。需要一种能严格评估视觉推理能力的方法。

Method: 提出Chartographer框架：(1) 将图表逆向工程为可执行代码；(2) 验证重建保真度；(3) 通过种子控制生成反事实变体（改变底层数据但保持问题逻辑不变）；(4) 从可执行QA逻辑推导新答案。应用于现有图表QA数据集，评估多种VLM。

Result: 反事实图表揭示了单图表评测中隐藏的失败：VLM在正确回答原始图表后，往往无法泛化到反事实变体。失败最常发生在需要新的视觉推理路径时。

Conclusion: 反事实图表生成是一种有效的VLM视觉推理严格评测方法，现有VLM在图表QA上的视觉推理能力被高估，需要更严格的评测协议。

Classification reason: 该论文提出反事实图表生成框架用于评估VLM的视觉推理能力，属于多模态评测范畴，与[多模态评测与基准]（multimodal_benchmark）高度匹配。

Importance reason: Focuses on chart QA evaluation, not directly on the user's priority subtopics like video generation or world models. Signals: Counterfactual evaluation, VLM visual reasoning, chart QA benchmark, generalization failure, multimodal understanding
...
### [188] [OmniInteract: Benchmarking Real-World Streaming Interaction for Real-Time Omnimodal Assistants](https://arxiv.org/abs/2605.26485)
...
TL;DR: OmniInteract 是一个面向实时流式交互的全模态大语言模型评测基准，包含 250 个视频和 1430 个时间锚定的响应槽，要求模型在音视频流中在线处理多模态触发信号并实时响应。实验表明当前模型在流式交互中表现较弱，最佳 IA-QTF1 仅 0.368。


<details>
  <summary>Details</summary>
Motivation: 现有全模态大语言模型评测多基于离线视频理解或文本提示的流式问答，缺乏对真实世界流式交互场景的评测。真实场景中用户查询和环境声音嵌入在音频流中，模型需要在线处理、检测多模态触发信号、决定何时响应，这些能力尚未被系统评估。

Method: 构建 OmniInteract 基准，包含 250 个视频和 1430 个时间锚定响应槽（1062 个 1Q1A 槽和 368 个 1QnA 槽），覆盖实时、主动、嵌套等场景。提出 Interaction-Aware Quality-Timeliness F1 (IA-QTF1)、Interruption Diagnostic Suite 和 Nested Chain Completion Score 等评测指标，综合评估响应正确性、时序、无效输出、中断处理和上下文连续性。

Result: 当前模型在流式交互中表现较弱，最佳整体 IA-QTF1 仅 0.368，最佳 1QnA IA-QTF1 仅 0.052。进一步的全双工数学推理研究表明，离线能力并不一定能迁移到在线交互场景。

Conclusion: OmniInteract 揭示了当前全模态模型在真实流式交互场景中的显著不足，为未来实时多模态助手的研究提供了重要的评测基准和方向指引。

Classification reason: 该论文提出了一个面向实时流式交互的全模态大模型评测基准 OmniInteract，属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题，因此直接复用该子主题。

Importance reason: 评测基准对多模态理解生成有贡献，但非用户优先的生成或效率方向。 Signals: 多模态评测基准, 流式交互, 实时全模态, 在线推理, 低模型性能
...
### [189] [Advancing Creative Physical Intelligence in Large Multimodal Models](https://arxiv.org/abs/2605.26396)
...
TL;DR: 本文提出 MM-CreativityBench 基准，用于评估大语言模型在视觉丰富、物理约束环境中的创造性工具使用能力。实验发现当前模型因缺乏持续的接地探索而表现不佳，为此提出基于直接偏好优化（DPO）的 affordance-grounded alignment 方法，结合 affordance 知识库监督，显著提升了实体/部件选择准确率并减少了幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要测试模型的感知与推理能力，但无法评估模型在开放环境中发现非显而易见的、物理可行的视觉接地解决方案的能力。这种创造性问题解决能力是人类智能的核心，但在当前多模态模型中尚未得到充分测试。

Method: 1) 构建 MM-CreativityBench 基准，包含视觉丰富、物理约束场景下的创造性工具使用任务，每个实例提供场景图像及候选实体/部件的结构化视图。2) 提出 affordance-grounded alignment 方法，将创造性工具使用建模为偏好学习问题，使用 DPO 鼓励模型偏好基于视觉证据的属性-功能推理。3) 引入 affordance 知识库监督，指导更广泛的实体探索和多轮规划。

Result: 实验表明，当前 LMM 在创造性工具使用任务中表现不佳，主要失败模式包括忽略相关实体、未充分检查关键部件、幻觉图像中不存在的属性。提出的 affordance-grounded alignment 方法在正确实体和部件选择上取得一致提升，同时大幅减少了幻觉和接地相关错误。

Conclusion: 创造性物理智能是当前多模态模型的重要缺失能力，通过将接地探索与偏好学习相结合，可以有效提升模型在开放环境中的创造性问题解决能力，为构建更具物理智能的多模态系统提供了新方向。

Classification reason: 该论文提出了 MM-CreativityBench，一个用于评估大语言模型在物理约束环境中进行创造性工具使用的多模态基准，属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题。同时论文也涉及多模态生成方向中的生成模型对齐（generation_alignment）子主题，因为提出了基于偏好优化的对齐方法。

Importance reason: Strong benchmark and alignment method for creative physical tool use in LMMs, but subtopic is evaluation rather than core generation. Signals: multimodal understanding generation, creative tool use, affordance grounding, direct preference optimization, benchmark
...
### [190] [FTibSuite: A Comprehensive Resource Suite for Tibetan Vision-Language Modeling](https://arxiv.org/abs/2605.26601)
...
TL;DR: FTibSuite 是首个面向藏语视觉语言模型的综合资源套件，包含人工验证的多模态训练数据（FTibData）、藏语多模态评测基准（FTibBench）以及基于 Qwen3-VL-8B-Instruct 的可复现基线模型（FTibVLM），实验表明其在藏语多模态任务上取得显著提升，同时保持原有中文能力。


<details>
  <summary>Details</summary>
Motivation: 藏语作为低资源语言，在视觉语言模型领域缺乏可复现的训练和评测基础设施，现有多模态模型在藏语上表现不佳，亟需标准化的资源套件来推动藏语多模态研究。

Method: 构建了 FTibSuite 资源套件，包含：(1) FTibData——涵盖持续预训练、图像-文本对齐和指令微调三阶段的人工验证多模态训练语料；(2) FTibBench——将五个主流多模态基准（MMBench、MMStar、MMVet、POPE、MMT-Bench）适配到藏语，并设计分层质量控制流程减少翻译噪声；(3) FTibVLM——基于 Qwen3-VL-8B-Instruct 的三阶段适配流水线（持续预训练、图像-文本对齐、指令微调）构建的可复现基线模型。

Result: FTibVLM 在 FTibBench 所有任务上取得一致性能提升，例如 MMBench 准确率从 42.97 提升至 67.78，POPE-random 准确率从 47.53 提升至 80.56，同时原始中文能力仅轻微下降。

Conclusion: FTibSuite 为藏语多模态研究提供了首个标准化基础，验证了低资源语言多模态模型的有效适配路径，为其他低资源语言的多模态研究提供了可借鉴的范式。

Classification reason: 该论文构建了藏语视觉语言模型的资源套件，包含多模态训练数据（FTibData）和评测基准（FTibBench），属于低资源语言多模态评测与基准建设，映射到 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Low-resource language multimodal resource, but Tibetan is outside user's priority subtopics and negative keywords. Signals: low-resource language, multimodal benchmark, Tibetan VLM, human-verified data, Qwen3-VL-8B-Instruct baseline
...
### [191] [METATR: A Multilingual, Evolving Benchmark for Automatic Text Recognition](https://arxiv.org/abs/2605.26712)
...
TL;DR: 本文提出METATR v1.0，一个多语言、可演进的自动文本识别（ATR）评测基准，涵盖29种语言、多种手写体和布局，并定义了标准化的提示与归一化方法及动态评估框架。作者评估了多种开源和闭源模型，发现虽然闭源模型表现最稳定，但不同文字和布局间仍存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有ATR评测基准多集中于现代印刷体英文文本，无法反映真实世界文档的多样性和复杂性，难以指导实际应用中的模型选择。因此需要一个更具代表性、多语言、多布局的评测基准。

Method: METATR从多个公开数据集中收集涵盖29种语言、多种文字和布局的文档，构建了高多样性的评测集。同时定义了标准化的提示模板和输出归一化方法，并建立了可扩展的动态评估框架，确保结果可复现且可随时间扩展。

Result: 评估了多种开源和闭源ATR模型（包括vLLMs），从数据集级别、语言级别、手写体鲁棒性和计算效率等多个维度报告了性能。结果表明闭源模型表现最一致，但不同文字和布局间性能差异显著。

Conclusion: METATR为实际条件下评估多语言ATR提供了一个多维度的、面向实践者的框架，有助于跟踪领域进展并指导模型选择。

Classification reason: 该论文提出了一个多语言、可演进的自动文本识别（ATR）评测基准METATR，涵盖29种语言、多种手写体和布局，并评估了包括视觉语言模型（vLLMs）在内的多种系统。这属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。

Importance reason: Focuses on text recognition benchmarks, which is not a priority subtopic and lacks strong alignment with user's key directions. Signals: Multilingual benchmark, text recognition, vLLM evaluation, low priority for video/image generation, no autoregressive or diffusion focus
...
### [192] [ChartAct: A Benchmark for Dynamic Chart Understanding](https://arxiv.org/abs/2605.26994)
...
TL;DR: ChartAct 是一个面向动态图表理解的交互式基准测试，包含来自 8 个真实网站的 673 个动态图表和 1,440 个问答样本，覆盖 7 种图表类型，在动态图表和仪表盘两种环境下评估模型。实验表明现有最强模型 Claude-Opus-4.7 平均成功率仅 84.5%，多数模型低于 60%。


<details>
  <summary>Details</summary>
Motivation: 现有图表理解基准主要关注静态图表，而现实中的图表往往是动态且可交互的，关键信息需要通过悬停、点击、缩放、拖拽等操作才能获取。目前缺乏能够评估模型在真实交互环境中理解动态图表能力的基准。

Method: 从 8 个真实图表网站收集并筛选 673 个动态图表，覆盖 7 种常见图表类型，构建 1,440 个高质量问答样本。每个样本在两种环境（动态图表和仪表盘图表）中实例化，以评估不同上下文下的动态图表理解能力。

Result: 系统评估了 11 个先进多模态模型和 GUI 智能体。最强模型 Claude-Opus-4.7 平均成功率为 84.5%，大多数模型低于 60%。进行了详细的失败归因和案例分析。

Conclusion: ChartAct 为研究真实交互环境中的图表理解提供了新的基准，揭示了现有模型在动态图表理解方面的明显局限性，为未来研究指明了方向。

Classification reason: 该论文提出了 ChartAct，一个用于动态图表理解的交互式基准测试，属于多模态评测与基准（multimodal_benchmark）这一规范子主题，因为其核心贡献是构建了一个新的评测基准来评估多模态模型在动态交互图表上的理解能力。

Importance reason: Strong benchmark for dynamic chart understanding but not aligned with user's priority subtopics on generation or efficiency. Signals: new benchmark, dynamic chart understanding, multimodal evaluation, GUI agents, low model performance
...
### [193] [Scaling, Benchmarking, and Reasoning of Vision-Language Agents for Mobile GUI Navigation](https://arxiv.org/abs/2605.27134)
...
TL;DR: 本文系统研究了移动GUI导航中VLM智能体的数据缩放、基准评测和推理能力。提出了包含16000+真实任务的HyperTrack数据集和开源评测工具包GUIEvalKit，实验表明基于强化学习的微调优于监督微调，尤其在域外场景中表现突出。


<details>
  <summary>Details</summary>
Motivation: 移动GUI导航中VLM智能体的研究缺乏大规模真实世界数据集和统一的评测平台，现有工作难以系统评估数据规模、训练策略和推理能力对任务完成的影响。

Method: 构建了HyperTrack数据集（16000+任务，覆盖650+中国移动应用），开发了GUIEvalKit开源评测工具包，在HyperTrack上对比了监督微调（SFT）和基于强化学习的微调（RL-based FT）在不同数据规模下的表现，并评测了多种SOTA VLM。

Result: 基于强化学习的微调在域内和域外设置中均一致优于监督微调，且数据规模增大时RL方法的优势更加明显。通过GUIEvalKit评测了多种SOTA VLM，分析了交互历史和推理能力对任务完成的影响。

Conclusion: HyperTrack和GUIEvalKit为移动GUI导航VLM智能体提供了全面的开发和评测平台，强化学习与数据缩放之间存在协同效应，为未来研究提供了重要基准和方向。

Classification reason: 该论文提出了移动GUI导航的基准数据集HyperTrack和评测工具包GUIEvalKit，属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。同时，论文涉及VLM智能体在GUI导航中的应用，也匹配AIGC方向的AI智能体与生成工作流（aigc_agent_workflow）。

Importance reason: Strong benchmark contribution but focuses on mobile GUI navigation, which is not a priority subtopic. Signals: GUI navigation benchmark, reinforcement learning > supervised finetuning, domain-specific dataset, no direct video/image generation or world model connection
...
### [194] [IPIBench: Evaluating Interactive Proactive Intelligence of MLLMs under Continuous Streams](https://arxiv.org/abs/2605.27074)
...
TL;DR: IPIBench是首个评估多模态大语言模型在连续流式视频场景下交互式主动智能的基准，覆盖主动监控、主动任务管理以及交错的反应-主动请求。实验发现现有MLLM存在主动触发不稳定和反应-主动行为协调不足的问题，并提出无需训练的IPI-Agent框架来改进。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在反应式问答上表现优异，但真实世界的流式助手需要基于连续视觉输入进行主动推理。现有基准仅在孤立的单轮场景中研究反应式或主动式交互，忽略了用户可能添加、修改或取消主动请求并与反应式查询交错的动态多轮场景。

Method: 提出IPIBench基准，包含三个核心场景：主动监控、主动任务管理、交错反应-主动请求。同时提出IPI-Agent，一个无需训练的智能体框架，包含交互控制策略（Interaction-Control Policy）和时序门控机制（Temporal-Gating Mechanism），用于稳定主动触发和协调多轮交互。

Result: 在多个代表性MLLM上的评估揭示了两个主要局限：主动触发不稳定以及反应与主动行为之间的协调不足。IPI-Agent在所有基准设置上一致地提升了现有MLLM的性能。

Conclusion: IPIBench填补了MLLM交互式主动智能评估的空白，揭示了现有模型在流式多轮场景下的不足，IPI-Agent提供了一种有效的无需训练改进方案。

Classification reason: 该论文提出了IPIBench，一个用于评估MLLM在流式视频场景下交互式主动智能的新基准，属于多模态评测范畴，与[多模态评测与基准]（multimodal_benchmark）这一CANONICAL subtopic高度匹配。

Importance reason: Focuses on MLLM proactive intelligence evaluation, not directly on generation or efficiency priorities. Signals: MLLM benchmark, proactive interaction, streaming video, training-free agent, reactive-proactive coordination
...
### [195] [SpatialBench: Is Your Spatial Foundation Model an All-Round Player?](https://arxiv.org/abs/2605.27367)
...
TL;DR: 本文提出 SpatialBench，一个大规模、跨范式、多领域的空间基础模型综合评测基准，包含19个数据集、546个场景、5个空间领域，系统评估了41个模型在6种范式下的表现，揭示了当前模型尚未达到"全能选手"水平，并发现全上下文注意力机制和高质量数据比简单扩大数据规模更关键。同时发布了大规模数据集 DA-Next-5M 和基线模型 DA-Next。


<details>
  <summary>Details</summary>
Motivation: 当前空间基础模型主要在各自设计的特定领域上评估，存在范式覆盖窄、场景领域有限、帧采样随意等问题，难以真正衡量模型的泛化能力。需要一个跨范式、多领域、具有确定性采样的综合评测基准来回答"空间基础模型是否真正全能"这一关键问题。

Method: 构建 SpatialBench 基准，包含19个数据集、546个场景，覆盖5个空间领域（室内、室外、街景、具身、第一人称），评估6种范式（MVS、立体匹配、深度补全、单目深度估计、空间理解、空间推理）下的41个模型，在4种不同输入密度设置下进行5个任务套件的系统评测。同时构建 DA-Next-5M 大规模数据集和 DA-Next 基线模型。

Result: 实验表明当前空间基础模型尚未成为全能选手；全上下文注意力机制在精度上最优，而有界记忆策略可解锁长序列可扩展性；在具身和第一人称等挑战性任务中，严格的领域对齐和高数据质量比简单扩大数据规模更为关键。

Conclusion: SpatialBench 为空间基础模型提供了全面的评测框架，揭示了现有模型的不足，并为未来发展方向提供了重要洞察：应关注注意力机制设计、数据质量与领域对齐，而非盲目扩大数据规模。

Classification reason: 该论文提出了 SpatialBench，一个跨范式、多领域的空间基础模型评测基准，涵盖19个数据集、5个空间领域、6种范式、41个模型，属于多模态评测与基准方向，与 [CANONICAL] subtopic "多模态评测与基准" 高度匹配。

Importance reason: Strong benchmark for spatial foundation models, but user priorities focus on generative models and efficiency, not spatial understanding evaluation. Signals: spatial foundation model benchmark, cross-paradigm evaluation, 41 models tested, DA-Next-5M dataset, full-context attention insight
...
### [196] [PitchBench: Measuring Pitch Hearing in Audio-Language Models](https://arxiv.org/abs/2605.26176)
...
TL;DR: PitchBench 是一个系统评估音频语言模型（ALM）音高听觉能力的评测套件，包含 28 个实验，涵盖绝对音高和相对音高感知，并变化响度、音长、音源、背景噪声等声学条件。评估发现当前前沿 ALM 的音高听觉高度不可靠，在不同设置下表现持续较差。


<details>
  <summary>Details</summary>
Motivation: 音频语言模型在音乐教学、转录、推荐、制作等实际应用中日益重要，可靠的音高感知是这些应用的基础前提。然而现有基准很少直接评估音高听觉这一最基本的音乐能力，通常仅通过高层任务间接探测，且多为选择题格式，无法系统评估模型在不同乐器、声学条件和响应格式下的细粒度音高识别能力。

Method: 构建 PitchBench 评测套件，包含 28 个实验，覆盖绝对音高感知（识别单个音高）和相对音高感知（序列与和弦中的音高关系），同时变化响度、音符时长、音源（合成音、乐器音）、时间拉伸、背景噪声等声学条件。任务从识别孤立音高到追踪四声部音乐织体中的旋律线，涵盖多种响应格式（选择题、自由回答等）。

Result: 对前沿 ALM 的评估显示，音高听觉高度不可靠：模型在所有设置下表现持续较差，准确率随音源、音符时长和记谱格式剧烈变化。当前 ALM 即使对受控的合成和乐器刺激也不具备稳定的音高感知能力。

Conclusion: 当前音频语言模型尚未具备稳定的音高感知能力，这限制了其在音乐相关实际应用中的可靠性。PitchBench 为未来音高感知音频语言建模提供了系统化的评测工具和数据生成工具。

Classification reason: 该论文提出了 PitchBench，一个系统评估音频语言模型音高听觉能力的评测套件，属于多模态（音频-语言）模型的评测基准。这直接匹配 multimodal_understanding_generation 方向下的 [CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark）。同时，该工作也涉及 AIGC 方向下的生成内容评测（aigc_evaluation_benchmark），因为音频语言模型在音乐生成等 AIGC 应用中至关重要。

Importance reason: Focuses on a narrow audio perception benchmark, not aligned with user's priority on generation and efficiency. Signals: audio-language model evaluation, pitch hearing benchmark, not generation or efficiency, no autoregressive or diffusion focus, negative keywords absent
...
### [197] [Evi-Steer: Learning to Steer Biomedical Vision-Language Models through Efficient and Generalizable Evidential Tuning](https://arxiv.org/abs/2605.26292)
...
TL;DR: Evi-Steer 提出了一种基于证据理论的不确定性感知参数高效微调框架，用于 BiomedCLIP 视觉语言模型，仅更新 0.11% 参数，在 15 个生物医学数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有生物医学视觉语言模型的参数高效微调方法缺乏不确定性感知能力，在域迁移和模糊图文对齐场景下表现不佳，而临床场景对模型在低数据域和域迁移下的鲁棒性有严格要求。

Method: 提出 Evi-Steer 框架，在 BiomedCLIP 的视觉和文本编码器中执行轻量级低维 token 更新，同时估计认知不确定性；基于 Dempster-Shafer 理论进行跨模态置信度融合，使视觉适应受文本置信度条件约束，抑制冲突或不确定的跨模态更新。

Result: 在 15 个生物医学成像数据集（涵盖 8 个器官和 8 种成像模态）上，在少样本学习和域泛化设置下均一致优于现有最先进方法。

Conclusion: Evi-Steer 为在真实临床环境中部署视觉语言模型提供了一条实用且鲁棒的路径，通过不确定性感知的高效微调实现了更好的泛化能力。

Classification reason: 该论文提出 BiomedCLIP 的参数高效微调方法（仅更新 0.11% 参数），属于多模态模型高效化与轻量化方向，与 canonical subtopic "多模态高效化与轻量化" 高度匹配。

Importance reason: Focuses on biomedical domain with negative keywords, and lacks direct connection to priority subtopics like video generation or world models. Signals: biomedical vision-language, parameter-efficient fine-tuning, evidence theory, domain shift, few-shot learning
...
### [198] [The Rescue Effect: Spatio-Semantic Early Exit Bypasses Quantization Collapse in CLIP](https://arxiv.org/abs/2605.26415)
...
TL;DR: 本文发现 CLIP 模型在 INT8 量化后，深层 Transformer 块的激活噪声会累积并破坏多模态嵌入的余弦对齐，导致零样本检索性能下降（称为量化诱导表示坍塌 QIRC）。作者提出 LRA-EE（层感知早期退出）方法，通过空间语义聚合、多特征门控和层自适应阈值，在浅层提前退出以绕过噪声饱和的深层，在 ImageNet-1K 零样本分类上 FLOPs 降低 13.4%，Top-1 准确率提升 2.44 个百分点。


<details>
  <summary>Details</summary>
Motivation: 在资源受限硬件上部署视觉语言模型（如 CLIP）通常需要 INT8 量化，但量化在联合嵌入架构中引入了一种不同于传统 CNN 分类器的失效模式：跨 Transformer 块累积的激活噪声会扰动多模态嵌入的方向，破坏零样本检索依赖的余弦对齐。现有工作缺乏对这种量化诱导表示坍塌的系统性分析和缓解方法。

Method: 提出 LRA-EE（Layer-wise Representation-Aware Early Exit）方法，包含三个核心组件：(1) 空间语义聚合（Spatio-Semantic Aggregation），用全局 patch token 平均替代浅层不成熟的 [CLS] token；(2) 学习型多特征门控（learned multi-feature gate），综合置信度、top-2 间隔和空间激活方差三个信号；(3) 层自适应置信度阈值（Layer-adaptive Confidence Thresholding），根据每层的信息噪声比（INR）校准退出阈值。

Result: 在 ImageNet-1K 零样本分类上，LRA-EE 相比 INT8 基线 FLOPs 降低 13.4%，Top-1 准确率从 58.72% 提升至 61.16%（+2.44%p）。四象限分解揭示了"救援效应"：9.5% 的样本在浅层正确分类但在深层被噪声淹没，而仅有 7.1% 的样本出现相反情况。

Conclusion: 本文系统性地揭示了 CLIP 模型 INT8 量化中的表示坍塌问题，并提出 LRA-EE 早期退出方法有效缓解了该问题。实验表明，量化后的深层 Transformer 块并非总是有益的，通过智能的早期退出可以在提升效率的同时改善性能。这项工作为量化多模态模型的部署提供了新的视角和实用方案。

Classification reason: 该论文研究 CLIP 模型的 INT8 量化带来的表示坍塌问题，并提出早期退出机制来绕过噪声饱和的深层，属于多模态模型的高效化与轻量化（量化加速）范畴，因此映射到 [CANONICAL] subtopic "多模态高效化与轻量化" (efficient_multimodal)。

Importance reason: Directly addresses model efficiency via early exit for CLIP, a key multimodal model, with strong empirical gains. Signals: CLIP quantization collapse, early exit bypasses noise, FLOPs reduction + accuracy gain, model efficiency, multimodal understanding
...
### [199] [O-MARC: Omni Memory-Augmented Compression Distillation for Efficient Video Understanding](https://arxiv.org/abs/2605.26584)
...
TL;DR: 本文提出 O-MARC（Omni Memory-Augmented Compression Distillation）框架，通过无训练的 OMAC 压缩方法保留视觉记忆和音频锚点来减少全模态 LLM 的 token 序列长度，并利用压缩蒸馏使紧凑模型适应压缩输入。同时构建了 UGC-AVQA 基准，专门评估嘈杂用户生成视频中的音视频关联理解。在 Qwen2.5-Omni-3B 上，O-MARC 平均得分 45.8，超越全 token 推理的 44.1，并实现 1.53 倍加速和 34.7% 内存节省。


<details>
  <summary>Details</summary>
Motivation: 全模态大语言模型（Omni LLM）能够统一理解音频和视频，但长联合 token 序列导致推理成本高昂。现有基准未能充分隔离嘈杂用户生成视频中的音视频关联，且缺乏针对压缩输入的高效推理方法。

Method: 1) 构建 UGC-AVQA 基准：包含 1,000 个用户生成视频和 4,816 个 QA 对，通过音频移除测试确保问题需要同时依赖听觉和视觉证据。2) 提出 OMAC：一种无训练即插即用的压缩方法，保留显著的视觉记忆和时序定位的音频锚点。3) 提出 O-MARC：压缩蒸馏框架，使紧凑模型学会处理记忆增强的压缩多模态上下文。

Result: 在 Qwen2.5-Omni-3B 上，O-MARC 在四个基准上的平均得分为 45.8，优于全 token 推理的 44.1 和 OmniZip 的 41.0。OMAC 相比全 token 推理延迟降低 34.6%（1.53 倍加速），内存减少 34.7%。

Conclusion: O-MARC 和 OMAC 有效降低了全模态 LLM 的推理成本，同时保持甚至提升了理解性能，为高效多模态理解提供了可行的压缩蒸馏方案。UGC-AVQA 基准为评估音视频关联理解提供了更有针对性的测试平台。

Classification reason: 该论文提出 O-MARC 压缩蒸馏框架和 OMAC 无训练 token 压缩方法，旨在降低全模态大语言模型（Omni LLM）的推理成本，属于多模态高效化与轻量化方向，与 [CANONICAL] subtopic "多模态高效化与轻量化"（efficient_multimodal）高度匹配。

Importance reason: Directly addresses efficient multimodal understanding with compression, aligning with model efficiency and unified multimodal priorities. Signals: efficient inference, token compression, multimodal understanding, audio-visual association, new benchmark UGC-AVQA
...
### [200] [Once-For-All: A Train-Once and Select-Anytime Framework for Multimodal Instruction Tuning](https://arxiv.org/abs/2605.26761)
...
TL;DR: 本文提出 OFA 框架，在 CLIP 空间中聚类多模态指令数据并训练一个轻量级可复用选择器，一次训练后可直接迁移到不同数据集和不同 VLM 模型上，仅用 15% 数据即可达到接近全量训练的基准性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法依赖特定模型或数据集，更换目标模型或候选池时需要从头重新计算选择信号，成本高昂。亟需一种可复用的数据选择方案，一次训练即可跨数据集、跨模型迁移。

Method: OFA 在冻结的 CLIP 空间中聚类多模态指令数据，从聚类结构中导出伪标签，训练一个轻量级选择器（仅需少量 epoch）。选择器对样本的预测置信度越低，该样本被认为信息量越大。训练完成后，该选择器可直接冻结迁移到未见过的数据集和不同规模的 VLM 模型上。

Result: 在 LLaVA-665K 上仅选 15% 数据即可达到全量数据 98.3% 的性能；迁移到未见过的 Vision-Flan-186K 上，所选子集超越全量训练 10.6%；同一选择子集在 Qwen2.5-VL-3B 和 LLaVA-v1.5-7B 上均有效，无需针对模型重新计算。

Conclusion: OFA 证明了单个可迁移的选择器能够为高效多模态指令微调提供有效且可复用的解决方案，显著降低了数据选择的计算成本，具有良好的泛化能力。

Classification reason: 该论文聚焦多模态指令微调中的数据选择效率问题，提出可复用的数据选择器以降低训练成本，属于多模态模型高效化方向，与 [CANONICAL] subtopic "多模态高效化与轻量化"（efficient_multimodal）高度匹配。

Importance reason: Proposes a reusable data selection framework for multimodal instruction tuning, directly relevant to model efficiency and unified multimodal models. Signals: Efficient inference, Model efficiency, Data selection, Multimodal instruction tuning, Transferable selector
...
<div id='direction-multimodal_understanding_generation-aigc_unlearning_erasure'></div>

## 概念擦除与遗忘学习 [[Back]](#toc)

生成模型概念擦除、有害内容遗忘、安全生成、模型 unlearning。

### [201] [On the Robustness of Machine Unlearning for Vision-Language Models](https://arxiv.org/abs/2605.26992)
...
TL;DR: 本文首次系统调研了视觉语言模型（VLM）的机器遗忘方法，提出了统一的分类体系和鲁棒性分析框架，设计了三种攻击范式来检验遗忘的多模态知识是否可被重新激活。实验表明现有方法大多只是"隐藏"而非真正移除目标知识，揭示了当前 VLM 遗忘方法的脆弱性。


<details>
  <summary>Details</summary>
Motivation: VLM 可能从训练数据中记忆不良信息，机器遗忘技术应运而生。然而现有 VLM 遗忘方法的鲁棒性尚未被系统研究，遗忘的知识是否能在特定条件下被重新激活仍不清楚。

Method: 1) 构建了 VLM 遗忘方法的系统分类体系；2) 在多种提示设置下进行统一评估；3) 提出三种攻击范式：上下文提示攻击（通过上下文重新激活遗忘知识）、下游微调攻击（通过下游任务重训练恢复知识）、组合攻击。

Result: 大量实验表明，现有 VLM 遗忘方法在三种攻击范式下均存在脆弱性，遗忘的知识可以被部分或完全重新激活，说明当前方法更多是"隐藏"而非真正移除目标知识。

Conclusion: 当前 VLM 遗忘方法的鲁棒性不足，需要更可靠的多模态遗忘策略来确保被遗忘知识不会被重新激活。

Classification reason: 该论文研究视觉语言模型（VLM）的机器遗忘（machine unlearning）鲁棒性，属于多模态理解与生成方向的安全与可靠性主题。现有 taxonomy 中 multimodal_understanding_generation 方向下已有 aigc_unlearning_erasure（概念擦除与遗忘学习）子主题，且 aigc 方向下也有同名子主题，两者完全匹配论文核心内容。

Importance reason: Focuses on VLM unlearning robustness, which is not a priority subtopic and lacks direct connection to generation or efficiency. Signals: VLM unlearning, robustness analysis, attack paradigms, not aligned with priority generation/efficiency topics
...
### [202] [Multi-Stakeholder LLM Alignment: Decomposing Estimation from Aggregation](https://arxiv.org/abs/2605.26878)
...
TL;DR: 本文提出DecompR框架，将多利益相关方LLM对齐中的效用估计与效用聚合解耦，通过反事实校准的固定权重消除候选依赖的权重漂移，从而提升对齐稳定性与公平性。


<details>
  <summary>Details</summary>
Motivation: 在多利益相关方场景中，单一输出需要满足具有冲突偏好的多个用户。现有的整体式LLM评判器将效用估计和效用聚合混在一起，导致隐式权重不稳定。当利益相关方满意度分散时，这种聚合特定的"权重噪声"会造成较大的分数偏移，且随利益相关方数量增加而加剧。

Method: 提出DecompR框架：首先通过反事实校准从查询结构中固定权重（不依赖候选答案），然后独立估计每个角色的效用，最后用固定权重聚合。该方法将效用估计与效用聚合解耦，消除了候选依赖的权重漂移，并降低了估计噪声。

Result: 实验表明，DecompR在多个多利益相关方对齐任务上优于整体式LLM评判器，有效减少了权重噪声带来的分数偏移，且随着利益相关方数量增加，优势更加明显。

Conclusion: 将效用估计与效用聚合解耦是多利益相关方LLM对齐的有效策略，反事实校准的固定权重能够提升对齐的稳定性和公平性，为多利益相关方场景下的AI对齐提供了新思路。

Classification reason: 该论文研究多利益相关方场景下LLM对齐中的偏好聚合与奖励建模问题，属于生成模型对齐与奖励（generation_alignment）这一[CANONICAL]子主题。该子主题在multimodal_generation和aigc方向下均有收录，且论文涉及多模态理解与生成方向的对齐问题，因此匹配三个方向。

Importance reason: Focuses on LLM alignment for multi-stakeholder scenarios, which is not a priority subtopic and lacks direct connection to generation or efficiency. Signals: LLM alignment, multi-stakeholder, utility estimation, not in priority subtopics, no boost keywords
...
### [203] [Sentinel: Embodied Cooperative Spatial Reasoning and Planning](https://arxiv.org/abs/2605.26239)
...
TL;DR: 本文提出 Sentinel Challenge 基准和 CoSaR 框架，研究去中心化具身智能体在城市场景中通过自然语言通信协作完成空间推理与安全导航的问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对去中心化具身智能体在动态城市环境中协作空间智能的评估与建模，尤其是多智能体需要在自然语言通信下协商安全会合点并规避动态巡逻哨兵的问题尚未被系统研究。

Method: 提出 CoSaR（Cooperative Spatial Reasoning and Planning）框架，将基础模型的高层通信与规划能力与经典空间导航算法（如路径规划）相结合，使智能体能够交换情境更新、推理动态空间约束并协作重新规划轨迹。

Result: 在 14 个城市级场景、3-5 个智能体的实验中，CoSaR 在会合速度、路径长度和安全性方面均优于基线方法，证明了动态通信与空间推理结合对鲁棒多智能体协作的重要性。

Conclusion: 本文通过形式化协作空间智能这一新设定并提供可扩展基准，为推进具身多智能体系统中的协作空间推理奠定了基础。

Classification reason: 该论文研究多智能体在城市场景中的协作空间推理与规划，属于具身智能体在空间约束下的推理与决策，与 [CANONICAL] subtopic "空间与 3D 多模态推理"（spatial_3d_reasoning）高度匹配。

Importance reason: Novel benchmark for embodied multi-agent spatial reasoning, but tangential to user's core focus on generative models and efficient inference. Signals: Embodied AI, Multi-agent cooperation, Spatial reasoning, City-scale navigation, Foundation model planning
...
### [204] [3D Gaussian Map with Open-Set Semantic Grouping for Vision-Language Navigation](https://arxiv.org/abs/2605.26500)
...
TL;DR: 本文提出一种3D高斯地图（3D Gaussian Map）用于视觉-语言导航（VLN），通过在线构建以3D高斯原语表示的环境地图，并利用开放集语义分组（Open-Set Semantic Grouping）为每个高斯原语赋予语义信息，结合多粒度空间-语义线索的多级动作预测策略，帮助智能体在复杂3D环境中根据自然语言指令导航。在R2R、R4R和REVERIE三个基准上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有VLN方法虽然为智能体配备了各种场景表示以增强空间感知，但往往忽略了VLN场景中复杂的3D几何结构和丰富的语义信息，限制了模型在多样化和未见环境中的泛化能力。

Method: 1) 在线构建自我中心场景地图（Egocentric Scene Map），从稀疏伪激光雷达点云初始化3D高斯原语，提供几何先验；2) 通过开放集语义分组（Open-Set Semantic Grouping）操作，根据物体实例或stuff类别对3D高斯进行分组，形成统一的3D高斯地图；3) 设计多级动作预测策略（Multi-Level Action Prediction），结合多粒度空间-语义线索辅助智能体决策。

Result: 在R2R、R4R和REVERIE三个公开基准上的大量实验验证了该方法的有效性，在导航成功率等指标上取得了有竞争力的结果。

Conclusion: 本文提出的3D高斯地图能够有效融合几何与语义信息，为VLN任务提供更丰富的场景理解，提升了智能体在复杂3D环境中的导航能力。

Classification reason: 该论文研究视觉-语言导航（VLN），涉及3D场景表示与自然语言指令结合进行空间推理和导航决策，属于多模态理解与生成方向下的"空间与3D多模态推理"（spatial_3d_reasoning）规范子主题。

Importance reason: Strong VLN method with 3D Gaussian mapping and open-set semantics, but not aligned with user's priority subtopics. Signals: 3D Gaussian map, open-set semantic grouping, vision-language navigation, spatial-semantic reasoning, multi-level action prediction
...
### [205] [Uncertainty-Aware Gaussian Map for Vision-Language Navigation](https://arxiv.org/abs/2605.26503)
...
TL;DR: 本文提出一种不确定性感知的高斯地图方法用于视觉-语言导航，通过构建语义高斯地图（SGM）显式建模几何、语义和外观三种感知不确定性，并将其整合为统一的3D价值地图，以支持智能体在导航中做出更可靠的决策。


<details>
  <summary>Details</summary>
Motivation: 现有VLN智能体在导航过程中常遇到感知不确定性（如证据不足导致的地面真值不可靠、空间线索模糊等），但现有方法在预测动作时忽略了这些不确定性信息，导致导航决策不够鲁棒。

Method: 首先从全景观测初始化可微3D高斯原语，构建语义高斯地图（SGM）编码环境几何结构与语义内容。在此基础上，通过变分扰动高斯位置和尺度估计几何不确定性，扰动高斯语义属性捕捉语义不确定性，利用Fisher信息量刻画外观不确定性。将这些不确定性融入SGM，扩展为统一的3D价值地图，作为导航的affordance和约束。

Result: 在多个VLN基准数据集上的全面评估验证了该方法的有效性，表明显式建模感知不确定性能够显著提升导航性能。

Conclusion: 本文证明了在VLN中显式建模几何、语义和外观三种感知不确定性并将其融入观测空间，能够使智能体做出更明智的导航决策，为具身智能中的不确定性建模提供了新思路。

Classification reason: 该论文研究视觉-语言导航（VLN），涉及智能体在3D环境中根据自然语言指令导航，属于具身推理与空间推理范畴，与 [CANONICAL] subtopic "空间与 3D 多模态推理"（spatial_3d_reasoning）高度匹配。

Importance reason: Novel uncertainty modeling for VLN, but outside core priority subtopics and lacks boost keywords. Signals: uncertainty-aware navigation, semantic gaussian map, vision-language navigation, 3D value map, perceptual uncertainty
...
### [206] [How and What to Imagine? Visual Thinking in Unified Multimodal Models for Cross-View Spatial Reasoning](https://arxiv.org/abs/2605.27310)
...
TL;DR: 本文研究统一多模态模型（UMMs）中如何让视觉思维（visual thinking）真正发挥作用以提升跨视图空间推理能力。提出 View Dropout（VDrop）训练策略，强制模型依赖生成的中间思考图像而非仅依赖输入视图；并比较了三种视觉思维变体（俯视图、全景图、点匹配渲染图），发现全景图视觉思维结合 VDrop 在五个真实世界跨域基准上取得最佳泛化性能。


<details>
  <summary>Details</summary>
Motivation: 跨视图空间推理是视觉语言模型的薄弱环节，现有方法虽然尝试生成中间思考图像来辅助推理，但模型往往忽略这些视觉证据，仍然依赖语言推理。因此需要研究如何让视觉思维真正被模型利用（how），以及何种视觉思维形式最有效（what）。

Method: 提出 View Dropout（VDrop），一种训练时干预策略：在训练过程中随机遮挡部分输入视图信息，使其对答案预测不可见，但对思考图像 token 保持可见，从而迫使模型在回答时依赖思考图像。在此基础上，比较三种视觉思维变体：俯视图（top-down）、全景图（panoramic）和点匹配渲染图（point-matching），在合成场景上训练并在五个真实世界跨域基准上评估。

Result: 全景图视觉思维结合 VDrop 是唯一兼具信息性（informative）和可学习性（learnable）的配置，在五个真实世界跨域基准上取得了最佳跨域泛化性能。VDrop 显著提升了模型对思考图像的依赖程度。

Conclusion: 在统一多模态模型中，通过 VDrop 训练策略可以强制模型利用视觉思维进行空间推理；全景图形式的视觉思维在信息性和可学习性之间取得了最佳平衡，是实现跨视图空间推理的有效方案。

Classification reason: 该论文研究统一多模态模型（UMMs）中的视觉思维（visual thinking）用于跨视图空间推理，属于理解与生成一体化的统一多模态模型范畴。论文标题和摘要明确提及"unified multimodal models (UMMs)"，与 multimodal_understanding_generation 方向下的 [CANONICAL] subtopic "统一多模态模型"（unified_multimodal_model）高度匹配。同时，论文也涉及多模态生成（生成中间思考图像），因此也匹配 multimodal_generation 方向。

Importance reason: Directly addresses unified multimodal model reasoning with a novel training strategy and visual thinking analysis, highly aligned with user's priority subtopics. Signals: unified multimodal model, visual thinking, cross-view spatial reasoning, View Dropout, out-of-domain generalization
...
### [207] [Learning When to Think While Listening in Large Audio-Language Models](https://arxiv.org/abs/2605.27190)
*Zhiyuan Song, Weici Zhao, Yang Xiao, Suhao Yu, Cheng Zhu, Jiatao Gu*

Main category: cs.CL, cs.LG, cs.SD, cs.AI
...
TL;DR: 本文提出一种可学习的"等待-思考-回答"控制机制，用于大型音频语言模型（LALM）的流式语音交互场景。控制器在部分音频输入下决定何时等待、何时输出中间推理、何时给出最终答案。基于 Qwen2.5-Omni-7B，通过 SFT 和六奖励 DAPO 优化，在合成语音推理基准上将准确率从 67.6% 提升至 70.3%，同时减少 14% 的最终推理长度。


<details>
  <summary>Details</summary>
Motivation: 在实时流式语音交互中，推理质量与响应速度之间存在紧密耦合：延迟推理到语音结束可提升答案质量，但会增加用户可见的响应延迟；过早回答则可能在关键证据到达前做出错误判断。现有 LALM 缺乏在部分音频输入下动态控制推理时机的机制。

Method: 提出 wait-think-answer 控制框架：1) 构建对齐的 wait-think-answer 轨迹数据；2) 使用 SFT 训练控制器；3) 应用 Decoupled Clip and Dynamic Sampling Policy Optimization (DAPO)，奖励函数包含答案正确性、动作有效性、更新时机、延迟同步、推理质量和链一致性六个维度，优化完整轨迹而非仅最终答案。

Result: 在六任务合成语音推理基准上，六奖励 DAPO 控制器将行加权准确率从 67.6% 提升至 70.3%，同时将最终推理长度减少 14%。在 186 条人类录音的真实音频基准上，SFT 取得最强准确率，六奖励 DAPO 控制器是唯一最终推理长度低于基线的学习变体。

Conclusion: 流式语音模型应当学习在音频流中何时进行显式中间推理，而非仅在语音端点后一次性推理。可学习的 wait-think-answer 控制机制能有效平衡推理质量与响应延迟。

Classification reason: 该论文研究大型音频语言模型（LALM）在流式语音交互中的推理时机控制，属于多模态语言模型（音频+语言模态）的架构与训练方法改进，与 [CANONICAL] 子主题 "视觉语言模型与 MLLM" 中的 MLLM 架构和多模态指令微调方向高度吻合。

Importance reason: Novel "wait-think-answer" control for streaming audio-language models, but only tangentially touches user's priority subtopics. Signals: streaming LALM, learnable control, DAPO optimization, audio reasoning, latency-reasoning tradeoff
...
### [208] [OmniGF: A Dual-Branch Vision-Language Framework for Unified Gaze Following](https://arxiv.org/abs/2605.26399)
...
TL;DR: OmniGF 是一个双分支视觉语言框架，通过结构化语言分支生成离散推理状态、连续空间分支利用 VLM 隐藏状态生成高分辨率注视热力图，并结合头部嵌入实现多人场景下统一的注视跟随、语义注视预测和社交注视推理，在多个基准上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统注视跟随模型仅能进行纯空间定位，缺乏语义目标推理和社交场景理解能力，且多人场景需逐人处理导致冗余计算。现有 VLM 虽具备语义推理能力，但离散文本生成在连续空间定位任务上精度不足。

Method: 提出 OmniGF 框架，采用双分支解码策略：(1) 结构化语言分支生成离散推理状态；(2) 连续空间分支从 VLM 密集隐藏状态中提取特征，用高分辨率注视目标热力图监督以克服文本坐标生成的空间瓶颈。同时引入头部嵌入编码，为多人场景提供细粒度外观和朝向线索。

Result: 在多个标准注视跟随基准上取得了新的 SOTA 性能，同时支持精确空间注视估计、语义注视预测和复杂社交注视推理。

Conclusion: OmniGF 有效弥合了 VLM 语义推理能力与连续空间定位精度之间的差距，为统一多模态注视理解提供了可扩展的框架。

Classification reason: 该论文提出 OmniGF，一个基于 VLM 的双分支解码框架用于统一注视跟随（gaze following），属于视觉语言模型在特定视觉任务上的应用与架构改进，与 [CANONICAL] subtopic "视觉语言模型与 MLLM" 高度匹配。

Importance reason: Focuses on gaze following, a niche task not aligned with user's priority subtopics like video generation or world models. Signals: gaze following, vision-language model, dual-branch decoding, social gaze reasoning, multi-person inference
...
### [209] [Personalizing Embodied Multimodal Large Language Model Agents over Long-term User Interactions](https://arxiv.org/abs/2605.26256)
...
TL;DR: POLAR 提出了一种多模态记忆增强框架，通过构建多模态知识图谱（包含语义记忆和情景记忆）来为基于 MLLM 的具身智能体提供长期用户交互中的个性化上下文，从而在隐式指令场景下提升任务执行性能。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 具身智能体只能遵循通用指令或识别物体类别，无法利用长期交互中积累的用户个性化上下文（如隐式指定的目标），导致在真实场景中难以提供个性化辅助。

Method: POLAR 将历史交互组织为多模态知识图谱，包含语义记忆（个性化上下文和视觉概念）和情景记忆（具身体验如智能体轨迹）。执行任务时，检索相关记忆来解读当前请求并指导任务执行。

Result: 在多个 MLLM 骨干和多样化评估场景下，POLAR 的记忆机制持续提升了性能，尤其在需要跨交互推理、多跳推理或跟踪用户上下文随时间变化的场景中提升显著。

Conclusion: 多模态记忆增强是提升 MLLM 具身智能体长期个性化能力的关键方向，POLAR 框架验证了结构化记忆在隐式指令理解与个性化任务执行中的有效性。

Classification reason: 论文提出基于MLLM的具身智能体个性化框架POLAR，属于多模态大语言模型在具身智能场景中的应用与扩展，与[CANONICAL] subtopic "视觉语言模型与 MLLM" 高度相关。

Importance reason: Strongly aligns with unified multimodal models and MLLM priorities, but lacks direct connection to top-weighted video generation or world model subtopics. Signals: multimodal large language model, embodied agent, long-term personalization, multimodal knowledge graph, memory-augmented framework
...
### [210] [PinPoint: Prompting with Informative Interior Points](https://arxiv.org/abs/2605.26689)
...
TL;DR: PinPoint 提出一种无需训练的确定性点选择方法，通过融合四种视觉线索生成共识图，选择远离边界的紧凑空间多样点，并用冻结 VLM 为每个点标注，从而解决指代分割中 VLM+SAM 流水线的提示歧义问题，在 RefCOCO/+/g 上匹配甚至超越有监督和 RL 调优方法。


<details>
  <summary>Details</summary>
Motivation: 现有指代图像分割中，VLM+SAM 的无训练流水线性能落后于有监督和 RL 调优方法，但差距来源不明。本文发现差距主要来自提示歧义：VLM 给出的边界框让 SAM 难以确定框内哪些像素属于目标物体。先验的简单点采样会落在边界、干扰物和背景上，甚至比纯边界框更差。

Method: PinPoint 是一种确定性、无需训练的点选择器。它融合四种视觉线索（边缘检测、距离变换、颜色对比度、语义显著性）生成共识图，选择紧凑且空间多样、远离边界的点，然后利用冻结的 VLM 为每个点标注语义标签，最终将点提示输入 SAM 生成掩码。整个过程仅需两次 VLM 调用。

Result: 在 RefCOCO、RefCOCO+、RefCOCOg 上，使用五个内部点替换朴素采样，累计 IoU（cIoU）提升 12-18 个点。PinPoint 在无任何任务特定训练的情况下，匹配甚至超越有监督和 RL 调优的专家方法。

Conclusion: 提示歧义是 VLM+SAM 无训练流水线性能差距的主导因素，通过合理的点选择即可弥合这一差距，无需额外训练。PinPoint 证明了确定性、训练自由的方法可以匹敌调优方法，为指代分割提供了一种高效、可复现的解决方案。

Classification reason: 该论文使用 VLM 进行指代分割中的目标定位与点提示选择，属于视觉语言模型的应用与改进，映射到 [CANONICAL] 的 vlm_mllm_general（视觉语言模型与 MLLM）。同时涉及 AIGC 方向下的生成工作流（VLM+SAM 组合），但主要贡献在 VLM 应用层面。

Importance reason: Training-free VLM+SAM prompt disambiguation improves referring segmentation, relevant to unified multimodal models and efficient inference. Signals: training-free, VLM+SAM pipeline, prompt ambiguity solved, matches supervised methods, deterministic point selection
...
### [211] [DV-SFT: Direct Vision Supervision for Fine-Grained Visual Understanding](https://arxiv.org/abs/2605.26656)
...
TL;DR: 本文提出DV-SFT（直接视觉监督微调），通过为视觉token提供显式的逐token监督信号（利用OCR场景中图像块与文字的对应关系自动标注），在不修改模型架构或增加额外前向传播的情况下，通过标准的next-token prediction目标训练视觉token，显著提升了MLLM的细粒度视觉理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM训练中，监督信号仅作用于文本token，视觉token仅作为上下文隐式优化，导致模型对视觉信息的理解较为粗糙。先前尝试监督视觉输入的方法依赖额外解码器或多次前向传播，实用性受限。

Method: 利用OCR场景中直接的视觉-文本对应关系，自动将每个视觉token标注为其对应图像块中的文字。将MLLM视为黑盒，无需修改架构或额外前向传播，通过标准的next-token prediction目标同时训练文本token和视觉token。

Result: 在3个域内基准和4个域外基准上，DV-SFT一致优于标准SFT。进一步分析表明，视觉监督有效增强了细粒度视觉理解，并实现了更高的多模态对齐效率。

Conclusion: 直接对视觉token施加显式监督信号是一种简单有效的方法，能够在不增加模型复杂度的情况下显著提升MLLM的细粒度视觉理解能力，具有良好的实用性和泛化性。

Classification reason: 该论文提出DV-SFT方法，通过为视觉token提供显式监督信号来改进多模态大语言模型的细粒度视觉理解能力，属于MLLM训练与微调方法。映射到[CANONICAL] subtopic "vlm_mllm_general"（视觉语言模型与MLLM），该主题涵盖MLLM架构与指令微调，与本论文高度相关。

Importance reason: Directly improves MLLM fine-grained visual understanding via novel token-level supervision, aligning with unified multimodal model priority. Signals: MLLM fine-tuning, token-level vision supervision, next-token prediction, OCR-based auto-labeling, no architecture change
...
### [212] [FineVLA: Fine-Grained Instruction Alignment for Steerable Vision-Language-Action Policies](https://arxiv.org/abs/2605.27284)
...
TL;DR: FineVLA 是一个面向机器人 VLA 策略的细粒度指令对齐框架，包含数据构建工具、细粒度数据集 FineVLA-Data、评测基准和可操控策略。实验表明细粒度指令与粗粒度目标指令互补，最佳混合比例（FG:Raw=1:2~1:1）显著提升任务成功率和可操控性。


<details>
  <summary>Details</summary>
Motivation: 现有机器人数据集通常只提供粗粒度的目标级语言指令（如"拿起杯子"），缺乏执行细节（如使用哪只手臂、接近方向、接触区域等），限制了策略的可操控性和机器人视频理解能力。

Method: 构建 FineVLA 框架：(1) 数据构建工具统一 10 个开源数据集中的 972,247 条轨迹，构建 47,159 条细粒度轨迹的 FineVLA-Data；(2) 构建包含 500 个视频、10,816 个原子事实和 1,030 个 VQA 问题的评测基准；(3) 开发机器人专用 VLM 标注器进行可扩展细粒度标注；(4) 训练可操控 VLA 策略，混合使用细粒度和原始粗粒度指令。

Result: 细粒度监督不牺牲目标级成功率（FG-only 比 Raw-only 提升 +1.4~+8.1 个百分点）；细粒度与粗粒度指令互补，最佳混合比 FG:Raw=1:2~1:1 时达到 86.8%/82.5%（RoboTwin 仿真）和 62.7/100（真实双臂操作）；细粒度监督在姿态(+23)、颜色(+18)和接近方向(+18)等维度上提升最显著。

Conclusion: 细粒度语言指令应作为粗粒度目标指令的补充，同时指定"如何执行"与"执行什么"，能有效提升机器人策略的可操控性和任务成功率。

Classification reason: 该论文提出 FineVLA 框架，构建细粒度指令对齐的视觉-语言-动作（VLA）策略，属于视觉语言模型在机器人领域的应用。最佳匹配方向为多模态理解与生成，子主题复用 [CANONICAL] vlm_mllm_general（视觉语言模型与 MLLM），因为论文核心是 VLM 在机器人策略中的指令对齐与微调。同时匹配 AIGC 方向下的 aigc_agent_workflow（AI 智能体与生成工作流），因为涉及机器人动作策略生成与自动化工作流。

Importance reason: Strong robotics VLA work but only moderate alignment with user's priority subtopics and negative keywords. Signals: multimodal_understanding_generation, visual language model, fine-grained instruction, steerable policy, robot manipulation
...
### [213] [Self-Ensembling Vision-Language Models for Chart Data Extraction](https://arxiv.org/abs/2605.27298)
...
TL;DR: 本文提出一种 VLM 自集成方法，通过对同一图表图像多次采样 VLM 输出并在单元格级别聚合（取中位数），提升图表数据提取的准确性。同时引入收敛检测和不确定性估计，并构建了更复杂的 WB-ChartExtract 基准，在 ChartQA 和该基准上均取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 图表中的定量数据通常以图像形式存在，难以直接复用和分析。现有 VLM 方法在数据点密集或风格多样的图表上提取准确率仍不理想，且缺乏对提取结果可靠性的评估手段。

Method: 提出 VLM 自集成方法：对同一图表图像，从同一 VLM 多次采样生成多个表格输出；通过对齐候选表格，对每个单元格的数值取中位数得到共识表格；引入收敛检测机制，在聚合结果稳定后停止采样；基于样本间的离散度估计不确定性，帮助用户评估提取可靠性。同时构建了基于世界银行数据的 WB-ChartExtract 基准，包含更复杂、风格更多样的图表。

Result: 在 ChartQA 和 WB-ChartExtract 基准上，自集成方法相比单次 VLM 输出显著提升了提取准确率，在 WB-ChartExtract 上相对提升达 23%。

Conclusion: VLM 自集成是一种简单有效的图表数据提取增强方法，能够解锁图表图像中的表格数据，支持下游分析与复用。未来可进一步探索更高效的采样策略和跨模型集成。

Classification reason: 该论文研究图表数据提取（chart-to-table），属于文档/图表理解范畴，与 [CANONICAL] 子主题 "视觉问答与文档理解"（multimodal_vqa_doc）高度匹配。同时，该方法也涉及 AIGC 方向下的领域内容生成应用。

Importance reason: Focuses on chart data extraction, not aligned with user's priority subtopics like video/image generation or world models. Signals: VLM self-ensembling, chart data extraction, WB-ChartExtract benchmark, uncertainty estimation, not autoregressive or diffusion-based generation
...
### [214] [CodecCap: High-Fidelity Codec-Inspired Residual Modeling for Dense Video Captioning](https://arxiv.org/abs/2605.26967)
...
TL;DR: 本文提出 CodecCap，一种受视频编解码器启发的密集视频描述框架，将视频表示为关键帧描述（keyframe caption）和残差描述（residual caption），在保留细粒度视觉证据的同时减少冗余。同时提出 VidCapQA 基准用于评估描述保真度，并构建了大规模密集描述数据集 CodecVDC-100K。


<details>
  <summary>Details</summary>
Motivation: 现有视频描述方法在视觉保真度和冗余性之间难以平衡：整体描述简洁但丢失细粒度证据，分段描述覆盖更全但引入大量冗余。需要一种既能保留细粒度视觉信息又能减少冗余的高保真密集视频描述方法。

Method: 受视频编解码器启发，CodecCap 将视频表示为关键帧描述（keyframe caption）和残差描述（residual caption）。关键帧描述详尽编码稳定的视觉上下文，残差描述捕捉时间上局部的动作、运动和变化。同时提出 VidCapQA 基准，采用 caption-then-QA 范式，包含 1000 个问题覆盖 14 个能力维度，用于量化描述保真度。

Result: 在 VidCapQA 上的实验表明，直接由强 VLM 生成的描述仍遗漏大量视觉细节，而 CodecCap 显著优于使用相同底层 VLM 的直接描述方法。基于 CodecCap 构建了 CodecVDC-100K 大规模密集描述数据集，包含锚点、残差、场景级和视频级四种监督信号。

Conclusion: 关键帧-残差描述范式是一种实现高保真视频语言监督的有效方式，CodecCap 框架在保留细粒度视觉证据和减少冗余方面取得了良好平衡，为密集视频描述提供了新思路。

Classification reason: 该论文提出密集视频描述（dense video captioning）方法，并构建了视频描述质量评测基准 VidCapQA（包含问答形式的问题），属于视频理解与问答范畴，与 [CANONICAL] 多模态评测与基准（multimodal_benchmark）和视觉问答与文档理解（multimodal_vqa_doc）均相关。但核心任务是视频描述生成与评测，更贴近 VQA/视频理解方向，故选择 multimodal_vqa_doc。

Importance reason: Focuses on dense video captioning, not aligned with user's priority subtopics like video generation or world models. Signals: dense video captioning, codec-inspired framework, VidCapQA benchmark, visual fidelity, not autoregressive/diffusion generation
...
### [215] [LocateAnything: Fast and High-Quality Vision-Language Grounding with Parallel Box Decoding](https://arxiv.org/abs/2605.27365)
...
Main category: cs.LG, cs.CV, cs.RO, cs.AI
...
TL;DR: LocateAnything 提出并行框解码（PBD）方法，将边界框和点等几何元素作为原子单元单步解码，替代传统的逐 token 序列化解码，在保持几何一致性的同时大幅提升解码吞吐量和定位精度。配合大规模数据集 LocateAnything-Data（1.38亿样本），在多个基准上实现了速度-精度前沿的突破。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 将视觉定位和检测视为坐标 token 序列化生成问题，将每个 2D 框拆解为多个 1D token 独立学习与解码。这种逐 token 解码方式与框几何结构的耦合特性不匹配，且严格的序列生成造成了推理瓶颈。作者希望设计一种能保持框内几何一致性、同时实现并行高效解码的统一框架。

Method: 提出 Parallel Box Decoding (PBD) 框架，将边界框和点等几何元素作为原子单元在单步中并行解码，保持框内几何一致性。同时构建大规模数据引擎，收集并标注了包含 1.38 亿训练样本的 LocateAnything-Data 数据集，涵盖丰富的定位任务类型，用于训练统一视觉定位与检测模型。

Result: 在多个视觉定位和检测基准上，LocateAnything 在保持高 IoU 定位质量的同时，显著提升了解码吞吐量，实现了速度-精度前沿的突破。实验表明 PBD 和大规模训练数据在高效精确的统一视觉定位中具有互补优势。

Conclusion: 并行框解码（PBD）是一种有效的替代方案，能够克服传统序列化解码在几何一致性和推理效率上的局限。结合大规模多样化训练数据，LocateAnything 为统一视觉定位与检测提供了一条高效且精确的新路径。

Classification reason: 该论文提出LocateAnything，一个基于并行框解码（PBD）的统一视觉定位与检测框架，属于视觉语言模型在视觉定位/检测任务上的应用。这与多模态理解与生成方向下的[CANONICAL] subtopic "视觉问答与文档理解"（multimodal_vqa_doc）相关，因为视觉定位（visual grounding）和检测是VQA和文档理解中的核心能力，且该工作聚焦于高效精确的定位解码。

Importance reason: Proposes parallel box decoding for efficient vision-language grounding, aligning with user's interest in model efficiency and unified multimodal models. Signals: parallel box decoding, efficient inference, vision-language grounding, large-scale dataset, speed-accuracy frontier
...
### [216] [EdgeFlow: Edge-Map Augmented VLM-Based Flowchart Processing for Industrial Requirements Engineering](https://arxiv.org/abs/2605.27332)
...
TL;DR: EdgeFlow 提出一种无需训练的方法，通过向 VLM 输入中附加 Canny 边缘图作为结构先验，提升工业需求工程中流程图到 Mermaid 代码的转换质量，在真实工业数据集上节点 F1 提升 17.39%，边 F1 提升 16.94%。


<details>
  <summary>Details</summary>
Motivation: 工业需求工程中大量流程图以静态图像形式存在，难以被机器处理。现有 VLM 直接用于流程图转换时，常丢失拓扑关键视觉细节（如节点连接关系），导致转换质量不佳。需要一种无需标注数据和微调的方法来增强 VLM 对流程图拓扑结构的感知能力。

Method: EdgeFlow 在 VLM 原始输入基础上，附加通过 Canny 边缘检测算法确定性提取的边缘图作为结构先验，形成双图像输入（原始流程图 + Canny 边缘图），引导 VLM 更准确地识别节点和边的关系，实现流程图到 Mermaid 代码的转换。该方法无需标注训练数据，也无需对 VLM 进行微调。

Result: 在真实工业数据集 IndusReqFlow 上，EdgeFlow 相比原始 VLM 在节点级 F1 提升 17.39 个百分点，边级 F1 提升 16.94 个百分点，路径级 F1 提升 11.06 个百分点。在公开合成基准上的跨数据集评估未显示显著提升，说明需要包含工业数据的多样化基准。

Conclusion: EdgeFlow 提供了一种实用、无需训练的流程图转换增强方法，有效提升了 VLM 在工业需求工程中拓扑保持的流程图到 Mermaid 转换能力。同时揭示了现有合成基准与真实工业场景之间的差距，呼吁构建包含工业数据的多样化评测基准。

Classification reason: 该论文研究流程图（flowchart）到 Mermaid 代码的转换，属于文档/图表理解任务，与 [CANONICAL] 多模态评测与基准 下的 multimodal_vqa_doc（视觉问答与文档理解）高度匹配。同时，该工作涉及 AIGC 领域的文档生成应用，因此也匹配 aigc_domain_application。

Importance reason: Focuses on a niche industrial document understanding task with limited generalizability to the user's priority generative and world model topics. Signals: training-free VLM enhancement, edge map structural prior, flowchart-to-code conversion, industrial requirements engineering, no improvement on synthetic benchmark
...
### [217] [Not All Modalities Are Equal: Instruction-Aware Gating for Multimodal Videos](https://arxiv.org/abs/2605.26232)
...
TL;DR: 本文提出 UniMVU，一种统一的指令感知多模态视频理解框架，通过内模态门控和模态级门控动态融合视频、音频、深度图等多模态输入，在 AVQA、AVSD、ScanQA 等六个基准上取得一致提升，CIDEr 指标最高提升 13.5。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型在处理带有辅助流（如音频、深度图、密集时序证据）的多模态视频时，采用统一融合方式会导致模态干扰，无关通道会分散模型注意力。需要一种能根据文本指令动态平衡各模态重要性的融合方法。

Method: 提出 UniMVU 框架，包含两个层次的门控机制：(1) 内模态门控模块，在每个模态内部强调显著区域；(2) 模态级门控模块，通过控制令牌重新加权整个模态流。两者均以文本指令为条件进行动态调节。对于时间对齐的流，还采用快慢融合方案减少冗余。整体结合跨模态自注意力实现多模态融合。

Result: 在 AVQA、AVSD、Music-AVQA、ScanQA、SQA3D 和 MVBench 六个基准上，UniMVU 相比静态融合基线取得一致提升，CIDEr 指标最高提升 13.5。门控机制与人类可解释的模态相关性一致，消融实验验证了内模态门控和模态级门控各自的贡献。

Conclusion: UniMVU 提供了一种简单、统一的指令感知多模态视频理解方案，可扩展到多种模态输入而无需手工设计融合规则，有效解决了多模态融合中的模态干扰问题。

Classification reason: 该论文提出 UniMVU，一种面向多模态视频理解的指令感知门控融合框架，处理视频、音频、深度图等多模态输入。核心任务是视频问答与视频理解，属于 [CANONICAL] 视频理解与时序推理 子主题。

Importance reason: Proposes instruction-aware gating for multimodal video understanding, aligning with unified multimodal models and efficient inference priorities. Signals: unified multimodal video understanding, instruction-aware gating, modality-level fusion, consistent gains across six benchmarks, aligns with priority subtopics
...
### [218] [Rethinking Weakly-supervised Video Temporal Grounding From a Game Perspective](https://arxiv.org/abs/2605.26441)
...
TL;DR: 本文从博弈论视角重新审视弱监督视频时序定位任务，将视频帧和查询词建模为博弈玩家，利用多元合作博弈理论学习帧-词之间的细粒度跨模态对应关系，无需预定义时刻候选框即可实现精确的时序定位。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督视频时序定位方法存在两个问题：1）仅捕捉全局视频级与查询的对齐，缺乏帧与词之间的细粒度一致性建模；2）依赖复杂的预定义时刻候选框，性能受候选框质量影响且耗时。本文旨在解决这些问题。

Method: 将视频帧和查询词建模为博弈玩家，利用多元合作博弈理论量化帧-词合作联盟内的交互贡献，学习每个帧和词对跨模态相似度分数的贡献程度，从而得到查询引导的帧级分数，直接用于时刻定位，无需预定义候选框。

Result: 在 Charades-STA 和 ActivityNet Caption 数据集上取得了优于现有方法的性能。

Conclusion: 博弈论视角为弱监督视频时序定位提供了一种新的有效范式，通过细粒度的帧-词交互建模和免候选框设计，同时解决了粗粒度对齐和复杂候选框依赖两个关键问题。

Classification reason: 该论文研究弱监督视频时序定位（video temporal grounding），属于视频理解与时序推理方向，与 [CANONICAL] subtopic "视频理解与时序推理"（video_understanding_temporal）高度匹配。

Importance reason: Focuses on weakly-supervised video grounding, not aligned with user's priority subtopics like generation or world models. Signals: Weakly-supervised, video temporal grounding, game theory, cross-modal learning, no generation focus
...
### [219] [DynFrame: Adaptive Reasoning-Driven Multimodal Framework with Dynamic Frame Augmentation for Complex Video Understanding](https://arxiv.org/abs/2605.26680)
...
Importance: 高优先级 (86.5) - 未入选精读

TL;DR: DynFrame 提出了一种自适应推理驱动的视频多模态框架，通过将时间窗口和采样密度作为原生 token 进行自回归生成，实现单步多粒度证据检索；并引入 Segment-Decoupled GRPO (SD-GRPO) 对检索和回答 token 分别赋予角色特定的优势信号。在多个视频理解基准上，DynFrame-4B 可与 7B-8B 模型竞争，DynFrame-8B 达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有视频 MLLM 在"边推理边看视频"的范式中存在两个结构性缺陷：(1) 采样密度不是可学习的决策——模型可以决定"看哪里"，但每窗口的帧率是固定的，导致细粒度证据需要多次检索，增加推理上下文长度和训练难度；(2) 检索和答案生成通常使用单一轨迹级优势进行优化，导致"看哪里"和"如何回答"的 token 获得相同的信用分配，即使一个正确而另一个错误。

Method: DynFrame 包含两个核心设计：(1) 可学习的跨度-密度检索——将时间窗口和采样密度作为原生 token 嵌入自回归生成过程，实现单步多粒度证据检索；(2) Segment-Decoupled GRPO (SD-GRPO)——在检索边界处拆分 rollout，为采样决策 token 和答案 token 分别分配角色特定的 token 级优势信号，实现细粒度信用分配。训练数据为 DM-CoT-74k 和 DM-RL-45k。

Result: DynFrame-4B 在 NExT-GQA、Charades-STA、ActivityNet-MR、Video-MME、MLVU、LVBench 六个基准上与 7B-8B 强基线模型竞争；DynFrame-8B 在大多数指标上达到新的 SOTA。

Conclusion: DynFrame 通过将采样密度作为可学习决策并引入解耦的 GRPO 训练策略，有效解决了视频 MLLM 中细粒度证据检索效率低和信用分配不精确的问题，展示了在复杂视频理解任务上的显著性能提升。

Classification reason: 该论文提出 DynFrame，一种面向复杂视频理解的自适应推理驱动多模态框架，核心贡献在于视频 MLLM 的时序定位与细粒度证据检索，属于视频理解与时序推理范畴，因此映射到 [CANONICAL] subtopic "video_understanding_temporal"（视频理解与时序推理）。

Importance reason: Directly advances multimodal understanding with a novel adaptive retrieval and RL method, highly relevant to unified multimodal models and efficient inference. Signals: adaptive frame retrieval, tokenized sampling density, segment-decoupled GRPO, video understanding SOTA, 8B model
...
### [220] [Q-GeoMem: Question-Guided Geometric Memory for Video Spatial Reasoning](https://arxiv.org/abs/2605.27318)
...
TL;DR: 本文提出 Q-GeoMem，一种问题引导的几何记忆框架，用于视频空间推理。通过注入相机条件几何信息并维护细粒度上下文库和语义-几何证据库，结合 Q-Former 的问题相关性和新颖性评分机制，在 VSI-Bench 和 VSTI-Bench 上取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频空间语言模型虽然改进了几何感知和长程上下文建模，但通常将记忆视为通用时间缓存，引入冗余或无关的几何信息，削弱了长程推理能力。因此需要一种问题引导的、能够选择性保留相关几何证据的记忆机制。

Method: Q-GeoMem 将相机条件几何注入视觉 token，维护两种互补记忆：细粒度上下文库（Fine-Grained Context Bank）存储近期密集特征和相机状态，语义-几何证据库（Semantic-Geometric Evidence Bank）存储紧凑的长程证据。每个候选帧通过 Q-Former 计算的问题相关性和相对于已有证据库的新颖性乘积进行评分，该分数在读取时被存储和复用，同时采用基于容量的替换规则保持库的紧凑性。推理时，两种记忆在更新前被读取并与当前帧表示自适应融合。

Result: 在 VSI-Bench 和 VSTI-Bench 上，Q-GeoMem 在所有评估的空间推理模型中取得了最优性能。消融实验验证了所提出的证据评分机制的有效性。

Conclusion: 问题引导的几何记忆框架能够有效提升视频空间推理性能，通过选择性保留与问题相关的几何证据，避免了冗余信息的干扰，为长程视频空间推理提供了有效方案。

Classification reason: 该论文研究视频空间推理，属于视频理解与问答范畴，与 canonical subtopic "视频理解与时序推理"（video_understanding_temporal）高度匹配。

Importance reason: Novel question-guided geometric memory for video spatial reasoning, but lacks direct connection to user's priority subtopics like generation or world models. Signals: video spatial reasoning, geometric memory, Q-Former scoring, VSI-Bench SOTA, camera-conditioned geometry
...
### [221] [BioFact-MoE: Biologically Factorized Mixture of Experts for Vision-Language Prognostic Modeling in Hepatocellular Carcinoma](https://arxiv.org/abs/2605.26376)
...
TL;DR: BioFact-MoE 提出了一种生物因子分解的混合专家（MoE）框架，通过生物学监督的专家模块显式分解肝脏功能因子和肿瘤相关因子，在肝细胞癌预后预测中提升了多模态视觉语言模型的准确性和生物学可解释性。


<details>
  <summary>Details</summary>
Motivation: 肝细胞癌（HCC）具有高度生物学异质性，现有预后视觉语言模型将肝脏功能因子和肿瘤因子混合在单一隐空间中，限制了预测精度和生物学可解释性。作者希望显式分解这两类因子以提升预后建模效果。

Method: 提出 BioFact-MoE 框架，采用残差 MoE 生存架构，通过生物学监督的专家模块分别建模肝脏因子和肿瘤因子。模型在多参数 MRI 和放射学报告的多模态数据上训练，先在大规模 3D MRI-报告对（N=4,582）上预训练，再在 HCC 队列（N=588）上微调。

Result: 在 12、18、24 个月时间窗口上分别达到 75.33%、75.85%、73.96% 的 AUC，一致优于所有基线方法。门控专家权重实现了表型感知的风险分层，通路门控揭示了治疗相关的生存异质性。肝脏和肿瘤嵌入在无监督下分别与肝功能标志物和肿瘤负荷标志物选择性关联（p<0.05）。

Conclusion: BioFact-MoE 通过生物因子分解的 MoE 架构，在提升 HCC 预后预测精度的同时增强了模型的可解释性，验证了显式分解生物学因子在多模态医疗预后建模中的有效性。

Classification reason: 该论文将视觉语言模型应用于肝细胞癌（HCC）的预后预测，属于医疗垂直领域的多模态理解与生成应用。最佳匹配方向为多模态理解与生成（multimodal_understanding_generation），同时涉及AIGC的领域内容生成应用（aigc_domain_application），该子主题为[CANONICAL]条目，直接复用。

Importance reason: Highly specialized medical application with no overlap with user's priority subtopics or boost keywords. Signals: domain-specific medical MoE, no autoregressive/diffusion/video generation, negative keywords match (clinical, medical diagnosis)
...
### [222] [Multi-Modal Building Inspection via Perceiver IO Fusion of Satellite and Street-Level Imagery](https://arxiv.org/abs/2605.26381)
...
TL;DR: 本文提出一种基于Perceiver IO架构的多模态建筑检测框架，融合卫星图像与街景图像，利用共享DINOv2骨干网络提取空间patch token，无需固定尺寸池化即可处理可变数量的街景视角。构建了覆盖10个国家32,135栋建筑的大规模数据集，并提出RGB-M掩码策略将建筑足迹掩码作为第四通道输入。实验表明该融合方法在街景可见属性上显著优于基线，为多模态建筑检测提供了可扩展的灵活架构。


<details>
  <summary>Details</summary>
Motivation: 建筑检测（如屋顶元素和材料分类）对于城市规划、保险评估和灾害管理至关重要。现有方法通常仅依赖单一模态（卫星或街景图像），难以全面捕捉建筑特征。卫星图像提供俯视全局信息但缺乏立面细节，街景图像提供立面细节但视角有限。如何有效融合这两种异构、数量不固定的多视角图像是一个挑战。

Method: 1) 使用共享DINOv2骨干网络提取卫星图像和街景图像的patch token；2) 采用Perceiver IO架构作为融合模块，通过可学习的潜在查询向量自适应聚合来自不同数量街景视角的token，无需填充或固定尺寸池化；3) 提出RGB-M掩码策略，将建筑足迹掩码作为第四通道与RGB图像拼接输入，提供软空间先验；4) 构建大规模多国数据集（10国，32,135栋建筑，61,672个分段），每段配对卫星图像和最多8张街景图像；5) 联合预测屋顶元素（如天窗）和屋顶材料的多标签分类。

Result: Perceiver IO融合模型在所有融合策略中表现最佳，在街景可见属性上取得显著提升（如石板瓦+11.3 AP，天窗+1.3 AP）。但卫星单模态基线在宏观平均mAP上仍略占优势，因为某些类别主要从俯视角度可见。RGB-M掩码策略优于硬裁剪方法。

Conclusion: 本文证明了Perceiver IO架构能够有效融合卫星和街景多模态图像，灵活处理可变数量的街景视角，为多模态建筑检测提供了可扩展的解决方案。未来工作可探索更丰富的模态融合和更细粒度的建筑属性预测。

Classification reason: 该论文聚焦于建筑检测这一垂直领域应用，融合卫星图像与街景图像进行多模态分类，属于领域内容生成应用（建筑检测/遥感领域）。同时涉及多模态融合理解，因此也匹配多模态理解与生成方向。

Importance reason: Focuses on a niche building inspection application with no direct connection to the user's priority generative and efficient model topics. Signals: Perceiver IO fusion, satellite and street-level imagery, building inspection, DINOv2 backbone, RGB-M masking strategy
...
### [223] [MedVol-R1: Reward-Driven Evidence Grounding for Volumetric Reasoning Segmentation](https://arxiv.org/abs/2605.26621)
...
TL;DR: MedVol-R1 提出基于强化学习（GRPO）的医学体积推理分割框架，将自由形式临床查询中的隐式指代分解为可验证的2D证据锚点（关键轴向切片+2D边界框），再通过冻结的MedSAM2传播为3D掩码，在CT-ORG等基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有体积推理分割方法将语言理解与掩码解码耦合为不透明的潜在表示，缺乏可解释性且难以泛化到多样化的临床叙述表达。需要一种将证据定位与体积分割显式解耦的方法。

Method: 提出MedVol-R1框架：1) 使用LVLM将临床推理定位到可验证的2D证据锚点（关键轴向切片+2D边界框）；2) 通过冻结的MedSAM2将2D证据传播为连贯的3D掩码；3) 采用冷启动SFT后接GRPO强化学习训练，使用多组件奖励函数（信息性证据选择、2D空间定位准确性、跨切片体积一致性），无需昂贵的思维链标注。

Result: 在CT-ORG、AbdomenCT-1K和KiTS23（来自M3D-Seg基准）上，MedVol-R1一致优于强基线方法，达到SOTA性能。强化学习相比纯SFT带来明显提升。

Conclusion: 将证据定位与体积分割显式解耦的强化学习框架能有效提升医学体积推理分割的性能和可解释性，GRPO训练相比纯SFT有显著增益。

Classification reason: 该论文聚焦医学影像中的体积推理分割（VRS），属于医疗垂直领域的生成应用。虽然涉及多模态理解与推理，但其核心应用场景是医学领域，因此映射到 [CANONICAL] 的"领域内容生成应用"（aigc_domain_application）最为贴切。同时该方向也存在于 multimodal_understanding_generation 下。

Importance reason: Strong medical domain paper but low alignment with user's priority subtopics on autoregressive/diffusion video generation and world models. Signals: medical volume segmentation, reinforcement learning grounding, GRPO training, frozen MedSAM2, domain-specific application
