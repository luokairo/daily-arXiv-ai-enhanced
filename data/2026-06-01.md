...
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 66]
  ...
  - [视觉语言模型与 MLLM](#direction-multimodal_understanding_generation-vlm_mllm_general) [Total: 12]
...
TL;DR: 本文提出语义最小作用原理（SLAP），将视频语言模型中的时序插值问题从概率生成范式转向变分力学范式。通过建立经典力学与语义动力学之间的严格同构，将潜在视频轨迹建模为黎曼流形上的路径，由语义拉格朗日量控制。将插值任务转化为边界值问题，通过离散欧拉-拉格朗日方程求解，自然地保持物体持久性而无需像素级渲染。


<details>
  <summary>Details</summary>
Motivation: 大型视频语言模型（LVLMs）因稀疏帧采样导致严重的"时间间隙"，模型无法感知关键的因果转换。现有方法（如潜扩散或自回归外推）在长时序上难以保持语义一致性，存在物体消失和能量不稳定的问题。

Method: 提出语义最小作用原理（SLAP），建立经典力学与语义动力学的同构关系，将视频潜在轨迹建模为黎曼流形上的路径，由语义拉格朗日量（Semantic Lagrangian）控制。将插值任务形式化为边界值问题（BVP），通过离散欧拉-拉格朗日方程求解，在连续潜空间中自然地保持物体持久性。

Result: 大量实验表明SLAP在长时序语义一致性、物体持久性保持和能量稳定性方面优于现有方法。

Conclusion: SLAP从概率生成转向变分力学，为视频语言模型中的时序建模提供了新的理论范式，有效解决了长时序语义一致性问题。

Classification reason: 该论文提出语义最小作用原理（SLAP），将视频插值建模为黎曼流形上的变分路径问题，通过离散欧拉-拉格朗日方程求解边界值问题，属于连续视觉潜空间中的语义轨迹建模。这与 [CANONICAL] 连续 VLM 与视觉潜空间流（continuous_vlm_latent_flow）高度吻合，同时与多模态理解与生成方向相关。

Importance reason: Directly advances continuous multimodal latent reasoning via variational mechanics, aligning with top-priority continuous language/multimodal models and world model subtopics. Signals: continuous language model, variational mechanics, semantic Lagrangian, object persistence, temporal interpolation
...
TL;DR: 本文研究视觉-语言嵌入模型（如CLIP）中的概念绑定问题——即模型能否正确关联颜色与形状等属性到对应物体。发现CLIP的场景嵌入可加性分解为物体表示，但其绑定函数复杂度高，导致图像与文本编码器无法学习共享的绑定机制，难以泛化到未见概念组合。通过可控Transformer实验证明，足够的数据覆盖可使模型学习低复杂度的乘法交互绑定函数，实现系统性泛化。


<details>
  <summary>Details</summary>
Motivation: 人类能轻松将颜色、形状等概念绑定到对应物体上（概念绑定），但CLIP等视觉-语言嵌入模型在此任务上表现不佳。尽管CLIP在跨模态检索中表现类似"概念袋"模型，但单模态探针却能恢复物体信息。这种矛盾促使作者深入研究CLIP的绑定机制及其局限性。

Method: 首先形式化定义了绑定函数（binding function），将概念嵌入映射到场景嵌入。通过理论分析和实验证明CLIP的场景嵌入可加性分解为物体表示。然后通过可控Transformer模型（从零训练）研究绑定泛化，分析数据覆盖、绑定函数复杂度（线性vs乘法交互）对系统性泛化的影响。

Result: CLIP的场景嵌入可加性分解为物体表示，解释了单模态探针为何能恢复物体信息。但CLIP的绑定函数复杂度高，阻碍了图像与文本编码器学习共享的绑定机制。在可控Transformer实验中，足够的数据覆盖使模型学习到低复杂度的乘法交互绑定函数，实现系统性泛化。

Conclusion: CLIP的概念绑定限制并非根本性缺陷，通过足够的数据覆盖和合适的架构设计，模型可以学习到低复杂度的绑定函数，实现概念绑定的系统性泛化。这为改进多模态嵌入模型的组合泛化能力提供了方向。
...
Importance reason: Directly addresses concept binding in continuous vision-language models, a core subtopic with high weight, and reveals fundamental limitations and a path to systematic generalization. Signals: continuous language model, continuous multimodal model, concept binding, CLIP analysis, systematic generalization
...
### [3] [ReGuLaR: Relation-Grounded Latent Reasoning for Large Vision-Language Models](https://arxiv.org/abs/2605.30587)
...
Importance: 值得关注 (76.39) - 未入选精读

TL;DR: ReGuLaR 提出了一种关系锚定的连续潜空间推理框架，通过训练时的 ReGFormer 模块将潜状态显式锚定到图像中与问题相关的物体及其关系上，在推理时无需该模块即可生成答案，并在多个基准上取得 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有 LVLM 的 CoT 推理使用离散文本作为中间步骤，难以编码连续的视觉证据；而现有的连续潜空间推理方法未能充分将潜状态与视觉证据中的组合性和关系性结构连接起来。

Method: 提出 ReGuLaR 框架，包含：(1) 训练时的 ReGFormer 模块，用于将潜状态聚焦于问题相关的物体和物体间关系；(2) 构建 RGROUNDING-351K 数据集，标注关键物体边界框和物体间关系；(3) 推理时无需 ReGFormer，模型直接在潜空间中推理并生成答案。

Result: 在多个视觉推理基准上，ReGuLaR 一致优于现有方法，取得 SOTA 性能。

Conclusion: 将连续潜空间推理显式锚定到视觉关系结构可以有效提升 LVLM 的推理能力，且训练时的关系引导可以在推理时被移除而不影响性能。

Classification reason: 该论文提出 ReGuLaR，一种将连续潜空间推理与视觉关系结构显式连接的方法，属于连续多模态推理范畴，与 canonical subtopic "连续多模态推理与潜空间思维" 高度匹配。

Importance reason: Directly advances continuous latent reasoning for vision-language models, a top priority subtopic with strong alignment to user's direction weights. Signals: continuous latent reasoning, relation-grounded, state-of-the-art, new dataset, inference-time efficiency
...
### [4] [MergeTok: Unified Continuous and Discrete Visual Tokenization via Token Merging](https://arxiv.org/abs/2605.30904)
...
TL;DR: MergeTok 提出了一种统一的视觉 tokenizer，通过 token merging 技术桥接连续 VAE 和离散 VQ 两种范式，在单一架构中同时实现高保真重建和语义可控的离散表征，在 ImageNet-256 上取得了优于强 VAE 和 VQ 模型的 rFID。


<details>
  <summary>Details</summary>
Motivation: 现有视觉 tokenizer 分为连续 VAE 和离散 VQ 两大流派，各有互补的局限性：VAE 重建质量高但 latent 稠密纠缠、缺乏语义控制；VQ 支持自回归生成但存在梯度稀疏、训练不稳定和码本坍缩问题。作者希望设计一个统一的 tokenizer 架构，同时兼顾两者的优势。

Method: MergeTok 采用编码器-解码器架构，在编码阶段通过 token merging（聚类相似 token）建立结构先验，提供双重监督信号：(1) 在 VAE 分支施加 merged-token 语义对齐，正则化 latent 空间使其解耦且语义感知；(2) 导出分组约束，促进组内多样性和组间排他性，稳定 VQ 训练。最终单一架构同时输出连续和离散两种 token 表征。

Result: 在 ImageNet-256 上，MergeTok 在相同 token 预算下取得了比强 VAE 和 VQ 模型更低的 rFID，重建和生成性能均具有竞争力。生成的 token 表征具有语义组织性，兼容自回归和扩散两种生成范式。

Conclusion: MergeTok 证明了一个统一的 tokenizer 架构可以同时实现连续和离散视觉 tokenization，赋予视觉 tokenizer 鲁棒的语义组织和生成器友好的离散性，为理解与生成统一的视觉表征提供了新思路。

Classification reason: 该论文提出 MergeTok，在编码器-解码器架构中联合优化连续 VAE 和离散 VQ tokenizer，属于连续视觉 tokenizer 与表征自编码器方向，与 [CANONICAL] continuous_visual_tokenizer 完全匹配。

Importance reason: Directly addresses the user's top-priority continuous language/multimodal modeling subtopic with a novel unified tokenizer bridging continuous and discrete paradigms. Signals: continuous visual tokenizer, token merging, unified VAE and VQ, semantic disentanglement, high-fidelity reconstruction
...
TL;DR: ELAN4D 提出了一种以具身为中心的 4D 感知训练框架，通过前向运动学推导机器人关键点（关节、末端执行器）的 3D 位移轨迹作为预测性时空监督信号，以即插即用的辅助分支注入 VLA 策略的动作专家模块，在推理时丢弃该分支，不改变基础策略接口。在 LIBERO、LIBERO-Plus、RoboTwin2.0 及真实世界操控任务上，该方法显著提升了 VLA 策略在分布外扰动（相机、背景、布局变化）下的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 策略大多以反应式方式直接从当前观测回归动作，缺乏对未来动态的显式建模，导致在分布外扰动下泛化能力受限。作者希望通过引入未来机器人关键点轨迹作为预测性时空监督，使策略具备对未来状态的感知能力，从而提升鲁棒性。

Method: ELAN4D 利用前向运动学从本体感知状态推导机器人关键点（关节和末端执行器）的 3D 位移轨迹，无需外部跟踪器或重建。设计了一个轻量级轨迹解码器作为即插即用的辅助分支，通过梯度隔离注入动作专家模块，在保留预训练视觉-语言骨干的同时注入 4D 监督信号。推理时丢弃该解码器，保持基础策略接口不变。

Result: 在 LIBERO、LIBERO-Plus、RoboTwin2.0 和真实世界操控任务上，ELAN4D 一致性地优于强 VLA 基线方法，在分布外扰动（相机、背景、布局变化）下取得了显著提升，整体性能达到最优。

Conclusion: 以具身为中心的 4D 监督信号能够有效增强 VLA 策略对未来动态的建模能力，从而构建更鲁棒和更具泛化性的机器人操控策略。该方法即插即用、计算开销小，具有良好的实用价值。

Classification reason: 该论文提出 ELAN4D，通过未来机器人关键点轨迹作为预测性时空监督来增强 VLA 策略的鲁棒性和泛化能力，核心是面向机器人操控的具身世界建模与动作规划，因此映射到 world_model 方向下的 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Strong embodiment-centric 4D supervision for VLA robustness, but tangential to core continuous language/multimodal and video generation priorities. Signals: 4D supervision, VLA policy, robot keypoint tracks, plug-and-play adaptation, out-of-distribution robustness
...
### [6] [Physics-informed Goal-Conditioned Reinforcement Learning under Hybrid Contact Dynamics](https://arxiv.org/abs/2605.30503)
*Vittorio Giammarino, Anastasios Manganaris, Ahmed H. Qureshi*

Main category: eess.SY, stat.ML, cs.RO
...
TL;DR: 本文提出物理信息目标条件强化学习（Pi-GCRL）在接触丰富操作任务中的扩展。作者发现现有 Pi-GCRL 方法在接触动力学导致的混合动力学、模式依赖可控性和非光滑值函数下性能退化，进而提出接触感知和层次化公式，选择性地应用物理信息归纳偏置，为将 Pi-GCRL 扩展到接触丰富操作提供了原则性步骤。


<details>
  <summary>Details</summary>
Motivation: 现有物理信息目标条件强化学习（Pi-GCRL）方法在导航和无物体目标到达任务中表现良好，但在接触丰富的操作任务中，接触交互导致的混合动力学、模式依赖可控性和非光滑值函数使得现有方法直接应用时性能退化。本文旨在分析这些结构特性带来的挑战，并提出适用于接触丰富操作的改进方法。

Method: 本文首先分析了接触丰富操作中混合动力学对 Pi-GCRL 的影响，然后提出接触感知和层次化的 Pi-GCRL 公式，选择性地将物理信息归纳偏置应用于操作问题的不同部分，以应对接触依赖的动力学特性。

Result: 实验表明，所提出的接触感知和层次化 Pi-GCRL 方法在接触丰富的操作任务中优于直接应用现有 Pi-GCRL 方法的基线，有效缓解了混合动力学带来的性能退化问题。

Conclusion: 本文为将物理信息目标条件强化学习扩展到接触丰富操作任务提供了原则性步骤，证明了在混合接触动力学下选择性应用物理信息归纳偏置的有效性，为未来在更复杂机器人操作任务中的应用奠定了基础。

Classification reason: 该论文研究接触丰富的操作任务中的目标条件强化学习，引入物理信息归纳偏置，属于具身智能中的规划与控制范畴，与 canonical subtopic "具身世界模型与规划控制" 高度匹配。

Importance reason: Focuses on contact-rich manipulation, which is not a priority subtopic and lacks strong alignment with user's key boost keywords. Signals: Physics-informed GCRL, contact-rich manipulation, hybrid dynamics, goal-conditioned RL, robotics
...
### [7] [GSAM: A Generalizable and Safe Robotic Framework for Articulated Object Manipulation](https://arxiv.org/abs/2605.30740)
...
TL;DR: GSAM是一个面向铰接物体操作的通用且安全的机器人框架，通过视觉感知器生成运动学参数、VLM基于常识推理精化感知、LLM生成交互约束函数并应用于轨迹规划，以及运动学感知的规划器验证可达性，在50个铰链任务上相比基线成功率提升36%。


<details>
  <summary>Details</summary>
Motivation: 现有铰接物体操作方法（端到端策略学习、视觉-运动规划、LLM/VLM方法）忽视了铰接物体的多样性和末端执行器与手柄交互的复杂性，导致泛化能力有限且容易发生破坏性碰撞。本文旨在解决这些局限性。

Method: GSAM框架包含四个模块：(1) 基于视觉的感知器生成铰接物体的运动学参数；(2) 微调的VLM精炼器利用COT常识推理修正感知器的原始估计偏差；(3) 交互约束函数生成器整合铰接物体、交互姿态和避障知识，LLM将其功能化并应用于轨迹和姿态规划；(4) 运动学感知的操作规划器验证轨迹和姿态的可达性。

Result: 在5类物体共50个铰链任务和50个随机初始化的末端执行器-手柄配置上，GSAM相比最佳基线将标准差降低3.1%，操作成功率提升36.0%，验证了其物体泛化能力和交互安全性。

Conclusion: GSAM通过结合视觉感知、VLM常识推理和LLM约束生成，有效提升了铰接物体操作的泛化性和安全性，为服务机器人提供了更可靠的解决方案。

Classification reason: 该论文提出面向铰接物体操作的机器人框架GSAM，涉及视觉感知、运动规划、LLM/VLM推理与约束生成，属于具身智能中的机器人操作与规划控制，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on robotic manipulation safety and generalization, not directly on the user's priority subtopics like continuous language models or video generation. Signals: world model, VLM refinement, LLM constraint generation, kinematic-aware planning, articulated object manipulation
...
### [8] [SSR: Scaling Surefooted and Symmetric Humanoid Traversal to the Open World](https://arxiv.org/abs/2605.30770)
...
TL;DR: SSR 是一个端到端的基于第一人称视觉的人形机器人穿越框架，通过想象落脚点引导、等变潜空间对称增强和地形特定多判别器运动先验，实现了在开放世界中安全、稳定、高质量的双足行走，包括楼梯、宽间隙和高平台等极端地形。


<details>
  <summary>Details</summary>
Motivation: 将人形机器人从实验室环境扩展到开放世界面临巨大挑战：机器人需要利用视觉在异构地形上实现安全可靠的落脚点，同时产生协调自然的全身运动。现有方法在动态运动下的视觉引导落脚和全身协调方面仍存在不足。

Method: SSR 提出三个关键组件：(1) 想象落脚点引导（Imagined Foothold Guidance），学习建模即将到来的摆动脚接触并评估其支撑性，在触地前引导摆动脚朝向稳定区域，减少边缘滑移；(2) 等变潜空间对称增强（Equivariant Latent-space Symmetry Augmentation），在高维视觉观测下高效诱导双侧协调行为；(3) 地形特定多判别器运动先验（Terrain-specific Multi-discriminator Motion Priors），在不同场景中鼓励类人行为。

Result: 在多种真实世界地形上的大量实验表明，SSR 实现了安全、稳定、高质量的 locomotion，包括不同结构的楼梯以及宽间隙、高平台等极端挑战，并在开放户外环境中实现了可靠的长时程穿越。

Conclusion: SSR 通过端到端学习视觉引导的落脚点预测、对称性增强和地形自适应运动先验，有效提升了人形机器人在开放世界中的穿越能力，为人形机器人的实际部署提供了可行方案。

Classification reason: 该论文研究基于第一人称视觉的人形机器人全身运动控制与地形穿越，属于具身智能中的机器人规划与控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics world model paper but outside user's core focus on continuous multimodal models and autoregressive generation. Signals: humanoid traversal, world model, equivariant symmetry, foothold guidance, open-world locomotion
...
### [9] [Object-Informed Model Predictive Path Integral Control for Non-Prehensile Robot Manipulation](https://arxiv.org/abs/2605.30778)
...
TL;DR: 本文提出一种对象信息引导的模型预测路径积分（MPPI）控制方法，通过分层规划（先求解简化对象级问题，再以对象轨迹为参考求解联合机器人-对象规划）实现非抓取机器人操作中的长时域规划，在6-DoF xArm6推物体任务中成功率提升40%，控制频率提升26%。


<details>
  <summary>Details</summary>
Motivation: 非抓取机器人操作（如推物体）由于欠驱动和不连续交互特性，长时域规划非常困难。现有MPPI方法在长预测时域下计算负担重，且缺乏对任务高层结构的利用。

Method: 提出分层MPPI框架：1）先求解简化对象级问题（假设物体可直接驱动），得到对象轨迹作为参考；2）在联合机器人-对象规划问题中，以对象轨迹为引导进行MPPI控制，实现高效长时域预测。

Result: 在仿真中任务成功率提升40%，控制频率提升26%；在真实实验中成功率提升20%，计算开销与常规MPPI相当。

Conclusion: 对象信息引导的分层MPPI方法能有效提升非抓取操作中长时域规划的效率与成功率，且不显著增加计算负担。

Classification reason: 该论文提出了一种面向非抓取机器人操作的分层MPPI控制方法，利用对象级规划引导机器人级规划，属于机器人规划与控制范畴，与[具身世界模型与规划控制]方向高度匹配。
...
### [10] [Hide-and-Seek in Trajectories: Discovering Failure Signals for VLA Runtime Monitoring](https://arxiv.org/abs/2605.30834)
...
TL;DR: 本文提出Hide-and-Seek框架，通过轨迹级粗监督学习实现VLA模型的运行时故障检测，无需逐时间步标注，利用轨迹间和轨迹内对比学习定位故障指示性动作，在LIBERO、VLABench和真实机器人平台上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: VLA模型在真实部署中容易发生执行故障，现有故障检测方法要么依赖昂贵的动作重采样或外部模型，要么将轨迹级标签均匀传播到每个时间步，掩盖了局部故障信号。需要一种无需逐时间步标注、能精确定位故障时刻的检测方法。

Method: 提出Hide-and-Seek框架，将VLA故障检测建模为粗监督学习问题。结合轨迹间对比学习（inter-trajectory contrastive）和轨迹内对比学习（intra-trajectory contrastive）目标，从轨迹级监督信号中自动定位故障指示性动作，诱导出具有时序结构的故障信号。在推理时结合保形预测（conformal prediction）实现准确性与及时性的权衡。

Result: 在LIBERO、VLABench和真实机器人平台上，对OpenVLA、π₀和π₀.₅三种代表性VLA策略进行评估，在多任务故障检测中取得SOTA性能，在保形预测框架下实现了良好的准确率-及时性权衡，并泛化到未见任务。

Conclusion: Hide-and-Seek通过粗监督对比学习有效解决了VLA运行时故障检测中缺乏细粒度标注的问题，为具身智能系统的可靠部署提供了一种实用且高效的故障监控方案。

Classification reason: 该论文研究VLA（视觉-语言-动作）模型的运行时故障检测，属于机器人具身智能中的可靠性监控问题，与"具身世界模型与规划控制"这一规范子主题紧密相关。

Importance reason: Strong failure detection for VLA models but only tangentially touches world models and lacks direct connection to your core continuous multimodal or autoregressive generation priorities. Signals: VLA runtime monitoring, trajectory-level contrastive learning, failure detection, embodied AI, conformal prediction
...
### [11] [TARIC: Memory-Augmented Traversability-Aware Outdoor VLN under Interrupted Semantic Cues](https://arxiv.org/abs/2605.31121)
...
TL;DR: 本文提出 TARIC，一种面向户外 VLN 的统一框架，通过可通行性感知的语义方位提取、实时近场可通行性剖面映射以及世界对齐的 3D 线索记忆，在语义线索中断的长时无线索阶段维持目标导向的可执行引导，显著提升了四足和轮式机器人在 600-1000 米路线上的导航成功率。


<details>
  <summary>Details</summary>
Motivation: 户外长距离 VLN 中，语义线索（如目标地标）常因遮挡、稀疏或离开视野而中断，导致机器人陷入回溯、振荡或盲目探索。现有记忆方法在可通行性驱动的绕行场景下失效，因为记忆的线索方向可能不可通行，绕行会延长无线索阶段并使线索逐渐过时。因此，可通行性不仅是局部安全问题，更是维持目标导向引导的稳定性条件。

Method: TARIC 框架包含三个核心模块：(1) 从可见性门控的目标或探索线索中提取语义方位；(2) 利用实时近场可通行性剖面将语义方位转化为可执行航向，提供目标一致且可行的引导（而非仅做拒绝式安全过滤）；(3) 将间歇的 2D 证据提升为世界对齐的 3D 线索记忆，并采用不确定性感知的读出机制，确保在机器人移动过程中引导持续可达且稳定。

Result: 在四足和轮式平台上进行 600-1000 米路线的实验，TARIC 在仿真中比最强基线提升超过 10 个百分点的成功率，在真实世界中成功率达 40%（最强基线为 17.5%），在长时间无线索阶段具有显著更高的鲁棒性。

Conclusion: TARIC 通过将可通行性作为维持目标导向引导的稳定性条件，而非仅作为局部安全约束，有效解决了户外 VLN 中语义线索中断问题。3D 线索记忆与不确定性感知读出机制确保了绕行过程中引导的持续稳定，为户外长距离具身导航提供了有效方案。

Classification reason: 该论文研究户外视觉语言导航（VLN），涉及四足/轮式机器人在长距离开放环境中的导航、路径规划与语义线索中断下的鲁棒控制，属于具身智能与机器人规划控制范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics VLN paper but only tangentially touches world models and lacks direct connection to your priority continuous multimodal or video generation topics. Signals: outdoor VLN, traversability-aware navigation, memory-augmented guidance, semantic cue interruption, quadrupedal/wheeled platforms
...
### [12] [HARP-VLA: Human-Robot Aligned Representation Learning for Vision-Language-Action Model](https://arxiv.org/abs/2605.31234)
...
TL;DR: HARP 提出一种人类-机器人对齐的表征学习框架，通过有限配对数据和大量未配对数据训练对齐的视觉编码器和潜动作模型，实现从人类视频中更有效地预训练 VLA 策略，在 CALVIN 和真实机器人任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 从大规模人类视频中学习可泛化的 VLA 模型具有前景，但跨具身差异（视觉观察和可执行动作）导致人类与机器人视觉表征不一致，使得人类视频难以有效协同训练 VLA 策略。现有潜动作模型虽能缓解动作执行差距，但仍依赖视觉特征，视觉表征的不对齐会导致策略输入不一致和领域依赖的潜动作。

Method: HARP 框架包含：(1) 利用有限配对的人类-机器人演示作为跨具身桥梁，以及大量未配对的人类和机器人视频作为可扩展的动力学监督数据；(2) 训练机器人适配的视觉编码器和潜动作模型，引入操作中心辅助线索和源相对配对判别对齐损失，使机器人表征向人类语义对齐同时保持配对级判别性；(3) 对齐后的视觉编码器和潜动作模型提供统一的视觉和动作表征，用于 VLA 策略学习，其中人类和机器人视频提供视觉-语言到潜动作的监督，轻量级机器人动作头将潜动作转换为可执行命令。

Result: 在 CALVIN ABC→D 任务上达到 4.481 平均长度，在真实世界操作任务上相比最强基线提升 7.1% 的成功率。特征可视化、仿真和真实机器人实验均验证了人类-机器人对齐效果和下游策略性能的提升。

Conclusion: HARP 通过人类-机器人对齐的表征学习有效弥合了跨具身差距，使得从人类视频中预训练 VLA 模型更加高效，为利用大规模人类数据进行机器人策略学习提供了可行方案。

Classification reason: 该论文提出 HARP-VLA，一个面向机器人视觉-语言-动作（VLA）模型的表征学习框架，通过人类-机器人对齐的表征学习实现从人类视频中预训练 VLA 策略。核心关注机器人具身策略学习、动作建模与跨具身泛化，属于具身世界模型与规划控制范畴，因此映射到 canonical subtopic "具身世界模型与规划控制"。

Importance reason: Strong robotics paper but outside user's priority subtopics and negative keywords apply. Signals: VLA pretraining, human-robot alignment, latent action model, CALVIN benchmark, real-world manipulation
...
### [13] [DeMaVLA: A Vision-Language-Action Foundation Model for Generalizable Deformable Manipulation](https://arxiv.org/abs/2605.31286)
...
TL;DR: DeMaVLA 是一个面向通用可变形物体操作的视觉-语言-动作（VLA）基础模型，采用 VLM 骨干网络与流匹配连续动作生成，通过层剪枝提高效率，在约5000小时真实双臂演示上预训练，并通过 DAgger 数据聚合进行后训练，在 RoboTwin 和家庭折叠基准上取得竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 系统通常为不同物体类别训练独立策略，混合多任务训练存在任务干扰和性能下降问题，缺乏能够跨类别、跨场景泛化的可变形物体操作基础模型。

Method: DeMaVLA 采用 VLM 骨干网络（如 Qwen2-VL）结合动作专家模块，使用流匹配（flow matching）进行连续动作生成。动作专家通过每隔一层剪枝 VLM 层并保持层间对齐来构建，降低训练和推理成本。模型先在约5000小时真实双臂演示数据上预训练获取通用操作先验，再通过 DAgger 流程聚合多折叠任务的演示和纠错轨迹进行后训练。

Result: 在 RoboTwin 基准上取得竞争性性能，在自建的家庭折叠基准上展示了强大的真实世界结果，验证了可扩展真实数据、高效动作生成和纠错学习对通用 VLA 策略的价值。

Conclusion: DeMaVLA 证明了通过大规模真实数据预训练、高效动作生成架构和纠错学习，可以构建跨类别、跨场景泛化的可变形物体操作 VLA 基础模型，为家庭机器人操作提供了有效范式。

Classification reason: 该论文提出 DeMaVLA，一个面向可变形物体操作的 VLA 基础模型，使用流匹配生成连续动作，涉及机器人操作、动作规划与控制，属于具身世界模型与规划控制方向。

Importance reason: Strong on world model and efficient action generation via flow matching and layer pruning, but deformable manipulation is a niche subtopic. Signals: world model, flow matching, efficient inference via layer pruning, VLA foundation model, DAgger corrective learning
...
### [14] [Learning Terrain-Aware Whole-Body Control for Perceptive Legged Loco-Manipulation](https://arxiv.org/abs/2605.31343)
...
TL;DR: 本文提出 TA-WBC，一个基于强化学习的地形感知全身控制框架，用于腿式机械臂在复杂地形上的移动操作任务。通过混合外感受编码器提取地形特征、基于足接触平面的末端执行器采样方法以及双策略蒸馏模块，实现了跨地形的稳定全身控制。


<details>
  <summary>Details</summary>
Motivation: 现有腿式机械臂全身控制器主要依赖本体感知（proprioception），缺乏对外部地形拓扑感知（exteroception）的能力，导致在复杂地形上适应性不足。本文旨在将地形感知融入全身控制，提升腿式机械臂在多变环境中的操作能力。

Method: 1) 提出混合外感受编码器（hybrid exteroception encoder）提取地形特征，为机器人主动调整姿态和落脚点提供基础；2) 提出基于足接触平面的末端执行器采样方法，将操作目标与基座波动解耦；3) 引入双策略蒸馏模块（dual-policy distillation），在保持地形适应性的同时整合全身运动，避免灾难性遗忘。

Result: 仿真和真实实验验证了所提控制器的鲁棒性，在可达空间、跟踪误差和意外绊倒次数方面均优于基线方法。

Conclusion: TA-WBC 展示了统一策略在复杂地形上执行腿式机械臂移动操作任务的潜力，为具身机器人在人类环境中的部署提供了有效方案。

Classification reason: 该论文研究腿式机械臂在复杂地形上的全身控制（loco-manipulation），属于具身智能/机器人领域，与 canonical subtopic "具身世界模型与规划控制" 高度匹配。

Importance reason: Focuses on legged locomotion control, not directly aligned with user's priority on continuous multimodal models or video generation. Signals: Legged manipulator, terrain-aware control, whole-body control, RL-based policy, exteroception encoder
...
### [15] [LiftNav: Path Planning via Semantic Lifting in TSDF-Guided Gaussian Splatting](https://arxiv.org/abs/2605.31376)
...
TL;DR: LiftNav 提出一种混合导航框架，融合 TSDF 与 Gaussian Splatting 双地图表示，通过 YOLO 检测、TSDF 3D lifting 和 B-spline 轨迹优化，在未知室内环境中实现安全且具备语义理解的路径规划。


<details>
  <summary>Details</summary>
Motivation: 传统 TSDF 表示支持安全规划但缺乏语义信息，而 Gaussian Splatting 提供丰富外观但几何不精确，难以精确避障。现有方法难以同时兼顾语义理解与碰撞安全。

Method: 基于 GSFusion 的 TSDF+GS 双地图，引入实时 YOLO 目标检测、TSDF 3D lifting 将检测结果提升到 3D 空间，结合 B-spline 轨迹优化与 hinge-loss 碰撞惩罚项，实现无需密集 3D 嵌入的灵活语义导航。

Result: 在 Replica 数据集仿真中，相比 SOTA 辐射场基线方法，LiftNav 实现了 100% 的路径可行性率和更短的轨迹长度。

Conclusion: LiftNav 证明了 TSDF+GS 混合表示结合轻量级语义 lifting 可以有效支持自主机器人在未知室内环境中的安全语义导航，为神经辐射场在机器人导航中的应用提供了实用方案。
...
Importance reason: Focuses on robotics navigation with TSDF and Gaussian Splatting, which is not aligned with user's priority on continuous language/multimodal models or video/image generation. Signals: TSDF+GS hybrid map, YOLO detection, B-spline trajectory, hinge-loss collision penalty, 100% feasibility rate
...
### [16] [EMBGuard: Constructing Hazard-Aware Guardrails for Safe Planning in Embodied Agents](https://arxiv.org/abs/2605.30924)
...
TL;DR: EMBGuard 是首个基于 MLLM 的具身智能体安全护栏，通过解耦物理风险推理与智能体策略，评估（视觉观察，动作）对来识别危险配置并提供自然语言风险解释。同时贡献了 EMBHazard 训练数据集（15.1K 动作条件对）和 EMBGuardTest 基准（329 个真实场景，覆盖 7 类物理风险）。2B/4B 的小模型性能可媲美 GPT-5.1、Gemini-2.5-Pro 等闭源大模型，同时显著降低误报率。
...
Method: 提出 EMBGuard，一个基于 MLLM 的安全护栏框架，将物理风险推理从智能体策略中解耦。它接收（视觉观察，动作）对作为输入，输出危险识别结果和自然语言风险解释。同时构建了 EMBHazard 数据集（15.1K 动作条件对）和 EMBGuardTest 基准（329 个真实场景，7 类物理风险），通过组合变化危险和动作生成多样化的风险/安全场景。

Result: EMBGuard（2B/4B 参数）在 EMBGuardTest 基准上达到与 GPT-5.1、Gemini-2.5-Pro 等闭源大模型竞争的性能，同时显著降低了误报率，有利于实时部署。

Conclusion: EMBGuard 通过解耦风险推理与策略，为具身智能体提供了一种高效、可解释的安全护栏方案，小模型即可达到大模型级别的安全检测能力，为具身智能体的安全部署提供了实用工具。

Classification reason: 该论文关注具身智能体在真实环境中的安全规划，提出基于MLLM的安全护栏来识别物理风险并推理动作条件风险，属于具身智能体规划与安全控制范畴，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Strong safety contribution for embodied agents but outside user's core priority subtopics. Signals: MLLM-based safety guardrail, physical risk reasoning, decoupled from agent policy, small model competitive with large closed-source, EMBHazard dataset
...
### [17] [On-Device Robotic Planning: Eliminating Inference Redundancy for Efficient Decision-Making](https://arxiv.org/abs/2605.31460)
...
TL;DR: 本文提出 REIS 框架，通过利用机器人推理中的时序冗余（连续观测常产生相同动作），结合轻量场景门控、KV-steered affordance 路由和 deliberative reasoning，在 ALFRED 和真实机器人任务上显著降低推理开销，同时保持任务性能。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型和视觉语言模型的推理型机器人策略虽然具备强大的语义规划能力，但推理延迟过高，难以满足实际实时部署需求。作者观察到机器人推理工作负载中存在大量时序冗余——连续观测帧经常产生相同的动作和子目标，因此希望消除这种冗余推理来加速决策。

Method: REIS 框架包含三个核心组件：(1) 轻量场景门控（lightweight scene gating），快速判断当前场景是否需要重新推理；(2) KV-steered affordance 路由，利用 KV 缓存引导 affordance 选择，避免重复计算；(3) deliberative reasoning，仅在必要时进行深度推理。三者协同工作，在保持语义适应性的同时最小化不必要的推理。

Result: 在 ALFRED 基准和真实机器人任务上，REIS 显著抑制了推理开销，同时保持了具有竞争力的任务完成性能。具体地，推理延迟大幅降低，而任务成功率与完整推理基线相当。

Conclusion: REIS 验证了利用时序冗余来加速具身机器人推理的有效性，为在资源受限设备上部署推理型机器人策略提供了可行方案，展示了认知启发的轻量门控机制在机器人决策中的潜力。

Classification reason: 该论文提出面向具身机器人决策的 REIS 框架，结合场景门控、KV 引导的 affordance 路由和 deliberative reasoning 来加速推理，属于具身智能中的规划与控制，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Addresses inference redundancy in embodied world models, aligning with world_model and efficient inference priorities. Signals: world model, efficient inference, robotic planning, temporal redundancy, KV-steered routing
...
### [18] [IDOL: Inverse-Dynamics-Guided Future Prediction for End-to-End Autonomous Driving](https://arxiv.org/abs/2605.31476)
...
TL;DR: IDOL提出逆动力学引导的未来预测框架，在BEV潜空间中通过世界模型预测未来场景状态，再用逆动力学模型解码相邻潜状态间的运动增量，将被动场景预测转化为主动轨迹优化，在NAVSIM基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于世界模型的端到端自动驾驶方法虽然能预测未来场景演化，但预测结果与可执行轨迹生成之间的耦合较弱，未来推理往往只是描述性的，难以直接转化为规划相关的运动决策。

Method: IDOL首先使用BEV世界模型预测多个未来潜场景状态，然后引入逆动力学模型对相邻潜状态进行解码，提取过渡感知的轨迹特征并恢复规划相关的运动增量，利用这些信号优化规划轨迹；最后通过轻量级闭环精修模块，利用优化后的轨迹进行新一轮未来感知推理以提升长时一致性。

Result: 在NAVSIM v1和NAVSIM v2基准上，IDOL在同类方法中取得了最先进的性能，验证了逆动力学引导的未来预测对端到端规划的有效性。

Conclusion: 将逆动力学引入潜空间未来推理能够有效增强世界模型与规划之间的耦合，使未来预测从被动场景预测转变为可执行的规划指导，为端到端自动驾驶提供了一种新的范式。

Classification reason: 该论文提出IDOL框架，在BEV潜空间中利用逆动力学模型将世界模型的未来预测转化为可执行的轨迹规划，属于面向自动驾驶（具身智能/机器人）的规划控制世界模型，与[具身世界模型与规划控制]高度匹配。

Importance reason: Strong world model contribution for autonomous driving but outside user's core priority subtopics. Signals: world model, BEV latent space, inverse dynamics, trajectory optimization, NAVSIM benchmark
...
### [19] [PInVerify: An Offline Embodied Benchmark for Active Instance Verification](https://arxiv.org/abs/2605.30639)
*Yuhang Jiang*
...
TL;DR: 本文提出主动实例验证（AIV）任务，要求具身智能体围绕候选物体主动选择视角，判断其是否匹配细粒度自然语言描述。作者构建了 PInVerify 离线基准（3000 个评估场景、18 个物体类别、6 扇区导航拓扑），并基于开源 MLLM（≤8B）搭建了无需训练和 LoRA 微调的基线系统，包含属性分解、可见性加权多视角追踪器和多种下一最佳视角策略。实验表明，最佳 MLLM 基线超过最佳 embedding 基线 4.9 个百分点，LoRA 微调后达到 85.6%，但主动视角选择策略未带来可靠增益。


<details>
  <summary>Details</summary>
Motivation: 现有具身导航任务中，智能体到达目标物体附近后无法保证找到正确实例，因为细微的属性差异（如"白色花卉"vs"白色条纹"）需要近距离多视角检查。现有工作缺乏对主动细粒度语义验证的系统研究。

Method: 形式化 AIV 为有限时域决策过程；构建 PInVerify 离线基准（多视角捕获 + 6 扇区导航拓扑，含陷阱视图和不可达扇区）；设计无需训练流水线和 LoRA 微调端到端智能体，基于 Qwen3-VL、SenseNova-SI-1.2-InternVL3-8B、CLIP、SigLIP2 等模型，包含属性分解、可见性加权多视角追踪器和三种下一最佳视角策略。

Result: 最佳 MLLM 基线（Qwen3-VL-8B）超过最佳 embedding 基线（SigLIP2）4.9 个百分点；GT-box 消融显示 +3.1 个百分点的检测差距；主动视角选择策略未带来可靠增益；LoRA 微调（SFT+GSPO）达到 85.6%。

Conclusion: PInVerify 为具身 AI 中的主动细粒度语义验证提供了标准化基准，当前最佳 MLLM 基线表现优于 embedding 基线，但主动视角选择策略仍需进一步探索。该基准有望推动具身智能体在细粒度验证方面的研究。

Classification reason: 该论文提出具身智能体在目标附近进行主动实例验证（AIV）的任务，涉及具身智能体在环境中主动选择视角进行细粒度语义验证，属于具身世界模型与规划控制范畴，因此映射到 canonical subtopic "embodied_world_model"。

Importance reason: Embodied world model benchmark with active verification, but lacks strong connection to continuous language/multimodal models or generation. Signals: Embodied AI benchmark, Active instance verification, MLLM baseline, LoRA fine-tuning, No reliable NBV gain
...
### [20] [Learning to Perceive the World Through Control: Empowerment-Based Representation Learning](https://arxiv.org/abs/2605.30656)
...
TL;DR: 本文研究在强化学习中通过 empowerment 目标学习仅保留控制相关特征的表征。作者证明 empowerment 智能体诱导出前向和后向两种互补表征，两者都对控制无关特征具有不变性，从而隐式学习了一个以控制为中心的世界模型。分析表明，通过交互（而非被动数据）学习对获得有用的不变性至关重要。


<details>
  <summary>Details</summary>
Motivation: 在实际强化学习环境中，观测通常远高于控制所需变量的维度。现有表征学习方法往往学习所有可观测信息，而忽略了哪些特征对控制真正重要。本文旨在探索能否学习仅捕获控制相关环境特征的表征。

Method: 通过 empowerment 目标（最大化智能体对环境的影响力）进行理论分析，证明 empowerment 智能体自然诱导出两种表征：前向表征（forward representation）和后向表征（backward representation），分别捕获状态的互补方面，且两者都对控制无关特征具有不变性。

Result: 理论分析表明 empowerment 最大化使智能体隐式学习控制中心的世界模型；前向和后向表征互补且对控制无关特征不变；交互（而非被动数据）对学习这些不变性至关重要。

Conclusion: Empowerment 目标不仅可用于无监督技能学习，还能自然诱导出控制相关的世界模型表征。这强调了通过主动交互学习表征的重要性，与因果学习文献的观点一致。

Classification reason: 该论文通过 empowerment 最大化目标学习控制相关的表征，隐式构建以控制为中心的世界模型，强调通过交互（而非被动数据）学习环境模型，与 embodied_world_model（具身世界模型与规划控制）方向高度吻合。

Importance reason: Directly advances world model learning through control-centric representation, aligning with your high-priority world_model direction and continuous representation interests. Signals: world model, empowerment, control-centric representation, invariance to irrelevant features, interaction-based learning
...
TL;DR: DriveMA 提出一种基于可验证元动作（meta-actions）的驾驶视觉-语言-动作模型框架，通过轨迹标注管道将未来自车运动总结为语言域意图，并利用规则投影进行验证。结合动作中心监督训练和回合级信用分配强化学习，在 Waymo 开放数据集上取得了 SOTA 性能（2B 模型 RFS 8.060，4B 模型 8.079），并在 NAVSIM 上获得有竞争力的闭环规划表现。


<details>
  <summary>Details</summary>
Motivation: 现有 Driving VLA 模型存在语言-动作鸿沟（language-action gap），语言输出与底层轨迹规划之间缺乏直接对齐，导致语言难以有效指导驾驶决策。作者希望通过引入可验证的元动作来弥合这一鸿沟，使高层语言决策与低层轨迹规划能够显式对齐。

Method: DriveMA 框架包含三个关键组件：(1) 元动作定义与标注——将未来自车运动总结为紧凑的语言域意图（如"直行"、"左转"等），通过轨迹接地标注管道从专家轨迹中自动构建；(2) 可验证性——通过基于规则的投影将生成的轨迹反向投影为元动作，实现自动验证；(3) 训练策略——结合动作中心监督训练（action-centric supervised training）和数据高效的回合级信用分配强化学习（turn-level credit assignment RL），通过密集奖励和精确信用分配显式对齐高层决策与低层轨迹规划。

Result: 在 Waymo Open Dataset Vision-based E2E Driving 上，2B 模型达到 Rater Feedback Score (RFS) 8.060，4B 模型进一步提升至 8.079，均超越此前 SOTA。在 NAVSIM 上获得有竞争力的闭环规划性能。

Conclusion: 即使使用简单的元动作接口，只要使其可验证并针对语言-动作对齐进行优化，就能在驾驶规划任务上取得 SOTA 性能。这表明弥合语言-动作鸿沟是提升 Driving VLA 模型效果的关键方向。

Classification reason: 该论文提出 DriveMA，一种驾驶视觉-语言-动作模型（Driving VLA），通过可验证的元动作（meta-actions）将高层语言决策与底层轨迹规划对齐，属于具身智能中的规划控制任务。论文在 Waymo 开放数据集上进行端到端驾驶规划评测，与 embodied_world_model（具身世界模型与规划控制）高度匹配，因此复用该 canonical subtopic。

Importance reason: Strong world model and planning contribution but lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: Driving VLA, meta-actions, language-action alignment, reinforcement learning, Waymo SOTA
...
### [22] [Dreaming Of Others: Latent Teammate Modeling In World Models For Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.31361)
*Tomas Leroy-Stone*
...
TL;DR: 本文提出一种将队友建模为世界模型内部结构化组件的新视角，在 Dreamer 风格的世界模型中引入队友潜变量分解和 Theory-of-Mind 推理头，使智能体能够在潜空间中想象并适应不同协作伙伴，支持零样本和少样本协调。


<details>
  <summary>Details</summary>
Motivation: 在合作多智能体强化学习中，智能体需要与内部策略和意图不可直接观测的伙伴进行协调。现有世界模型（如 Dreamer）在单智能体场景中表现出色，但无法处理队友带来的不确定性，限制了其在 MARL 中的应用。

Method: 提出一种架构，将 Dreamer 风格的 RSSM 潜状态分解为环境组件和队友组件，并学习一个辅助的 Theory-of-Mind (ToM) 推理头，从部分轨迹中推断队友行为的潜嵌入（如角色、意图和预测动作）。这些队友潜变量用于条件化 actor 和 critic，使智能体能够想象并适应多样化的协作伙伴。

Result: 论文为方法提出了基准测试和评估协议，但尚未报告具体的实验数值结果，主要贡献在于概念框架和架构设计。

Conclusion: 本文将世界模型定位为不仅是环境动力学的预测器，更是社会行为的模拟器，为可泛化、与人类兼容的 AI 开辟了新方向，支持零样本和少样本协调。

Classification reason: 该论文将 Dreamer 风格的世界模型扩展到多智能体强化学习场景，将队友建模为世界模型中的结构化可学习组件，属于世界模型在具身智能与多智能体规划控制中的应用，与 [CANONICAL] 具身世界模型与规划控制 方向高度匹配。

Importance reason: Novel social world model for MARL, but tangential to user's focus on continuous multimodal models and video generation. Signals: world model, multi-agent RL, theory-of-mind, latent teammate modeling, Dreamer
...
TL;DR: PatchWorld 提出一种无梯度的框架，通过反例引导的代码修复，将离线轨迹转化为可执行的 Python 世界模型（符号信念状态程序），用于部分可观测环境下的预测与规划。在 AgentGym 的七个环境中，PatchWorld-Simple 在基于代码的规划中达到 76.4% 的宏平均成功率，且世界模型预测模块内部无需调用 LLM。


<details>
  <summary>Details</summary>
Motivation: 现有文本智能体环境通常建模为 POMDP，但智能体无法访问模拟器的潜在状态和转移动力学。作者希望探索能否从离线轨迹中诱导出可执行的代码作为世界模型，用于部分可观测下的预测和规划，从而替代黑盒预测模型。

Method: PatchWorld 是一个无梯度框架，包含两个阶段：(1) 从离线轨迹中诱导出符号信念状态程序（用 Python 实现），该程序维护信念状态并通过动作更新；(2) 通过反例引导的代码修复（counterexample-guided code repair）迭代改进程序。PatchWorld-Simple 是简化版本，使用单步前瞻搜索进行规划，世界模型预测模块内部不调用 LLM。

Result: 在 AgentGym 的七个环境中，PatchWorld-Simple 在基于代码的规划中达到 76.4% 的宏平均成功率，优于对比方法。研究发现人类指定的残差记忆偏置能提高表面观测保真度，但会降低决策效用，揭示了可执行世界模型中观测保真度与动作判别动力学之间的权衡。

Conclusion: PatchWorld 证明了从离线轨迹诱导可执行世界模型的可行性，为部分可观测环境下的规划提供了一种可解释、可检查、可局部修补的替代方案。同时揭示了可执行世界模型中观测保真度与决策效用之间的内在权衡。

Classification reason: 该论文提出 PatchWorld，通过离线轨迹诱导可执行的 Python 世界模型用于部分可观测环境下的预测与规划，属于可交互世界模型/环境动力学建模。映射到 [CANONICAL] subtopic "可交互与物理世界建模"（interactive_physical_world），该 subtopic 涵盖交互式模拟器、环境动力学建模等，与本文的 executable world model 高度吻合。

Importance reason: Directly advances world model research with a novel executable code approach, aligning with top user priority. Signals: world model, executable code, gradient-free, counterexample-guided repair, symbolic belief-state
...
### [24] [Physically Viable World Models: A Case for Query-Conditioned Embodied AI](https://arxiv.org/abs/2605.30542)
...
TL;DR: 本文指出现有观测预测型世界模型虽然能生成视觉上合理的未来帧，但在物理上可能完全错误，因为不同物理系统在视觉上可能相同但在干预下行为不同。作者提出具身AI需要"物理可行的世界模型"——能够识别回答干预查询所需的最简物理抽象，并模块化地组合环境表征、潜状态估计、动作规范、干预动力学和查询级响应组件。通过可控基准实验展示了现有模型的失败案例，并论证了查询条件化的模块化设计原则。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型主要基于观测预测（observation-predictive），即预测未来视觉帧。然而，视觉上相似的场景可能对应完全不同的物理参数（如摩擦系数、质量、弹性），导致在干预（如推动物体）时产生截然不同的结果。这种"物理歧义性"使得现有模型可能推荐不可行的动作、错误预测交互结果或认证不安全行为。作者旨在提出一种新的世界模型设计范式，确保模型在物理上是可行的（physically viable）。

Method: 提出"物理可行的世界模型"框架，核心思想是：世界模型应回答干预查询（intervention query），而非仅仅预测观测。模型由模块化组件构成，包括环境表征、潜状态与参数估计、动作规范、干预动力学和查询级响应。一个自主编排器（orchestrator）根据查询识别相关物理抽象，并动态组合兼容的学习组件和结构化组件。当封闭形式物理不可用或不确定时，转移模型可以是解析的、模拟的、学习的或混合的，但必须保留决定干预结果的结构。该框架还提供了可行性测试：正确的抽象不是最详细的世界模型，而是保留查询相关区分的最简模型。

Result: 通过可控基准实验（固定视觉场景但改变潜物理参数），展示了现有观测预测型世界模型在物理推理上的系统性失败。作者演示了所提出的查询条件化模块化方法在现有模型无法正确回答的查询上能够成功，并概述了编排器如何动态组装和适配物理可行的模型用于规划、控制和验证。

Conclusion: 具身AI需要从根本上重新思考世界模型的设计：从观测预测转向干预查询回答。物理可行的世界模型必须是模块化、可解释、可验证和可审计的。该框架为设计新世界模型提供了原则，也为评估现有模型提供了可行性测试标准。

Classification reason: 该论文批判现有观测预测型世界模型在物理一致性上的根本缺陷，提出世界模型必须能够回答干预查询（intervention queries），并强调物理可解释性、模块化分解与查询条件化建模。这直接对应 [CANONICAL] 子主题"可交互与物理世界建模"（interactive_physical_world），该子主题关注物理一致的世界模型、交互式模拟器与环境动力学建模。

Importance reason: Directly advances world model theory for embodied AI with a query-conditioned, physically viable framework that aligns with your priority on world models and neural simulators. Signals: World model failure analysis, Query-conditioned modular design, Embodied AI physical viability, Intervention dynamics, Orchestrator for abstraction selection
...
### [25] [Light Interaction: Training-Free Inference Acceleration for Interactive Video World Models](https://arxiv.org/abs/2605.31158)
...
TL;DR: Light Interaction 是一个无需训练的推理加速框架，针对交互式视频世界模型，通过自适应上下文管理、去噪缓存加速和3D块稀疏注意力（配合Triton内核），在HY-WorldPlay和Matrix-Game-3.0上实现最高2.59倍加速，同时保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 交互式视频世界模型在实时游戏模拟、虚拟场景导航和具身AI训练中具有重要应用，但长交互轨迹的推理成本极高，主要瓶颈包括：上下文记忆不断增长、注意力二次复杂度以及重复去噪步骤。现有方法缺乏针对交互式场景中轨迹依赖特性的优化。

Method: 提出Light Interaction框架，核心思想是利用交互过程的轨迹依赖性实现自适应计算：1）自适应上下文管理：探索新区域时丢弃检索到的空间记忆，根据局部潜动态调整时间上下文；2）去噪缓存加速：当相机回到熟悉区域时复用早期步骤的模型输出；3）硬件-软件协同设计的3D块稀疏注意力，配合融合Triton内核实现高效计算。

Result: 在HY-WorldPlay和Matrix-Game-3.0两个基准上，Light Interaction实现了最高2.59倍推理加速，同时保持具有竞争力的视觉生成质量，且无需重新训练模型。

Conclusion: Light Interaction证明了通过利用交互式场景中轨迹依赖的自适应计算特性，可以在不牺牲生成质量的前提下显著加速交互式视频世界模型的推理，为实时交互式模拟和具身AI训练提供了实用的加速方案。

Classification reason: 该论文提出交互式视频世界模型的推理加速框架，核心是交互式世界模型（interactive video world models），属于世界模型方向。其关注用户控制的相机运动下的交互式场景模拟，与[可交互与物理世界建模]（interactive_physical_world）这一规范子主题高度匹配。

Importance reason: Directly addresses efficient inference for interactive video world models, a high-priority subtopic with strong boost keyword alignment. Signals: world model, efficient inference, training-free acceleration, interactive video generation, 3D block sparse attention
...
TL;DR: 本文提出 AR Forcing，一种自回归训练策略，将标准扩散损失融入自回归训练循环中，使扩散导航世界模型在训练时暴露于推理状态分布，从而缓解训练与推理之间的分布偏移，提升长时序导航预测的生成一致性与轨迹精度。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的机器人导航世界模型通常使用并行监督训练，但在路径规划时采用自回归推理，导致训练与推理之间存在分布偏移，长时序预测性能不稳定。现有方法需要额外的判别器或分布匹配损失，增加了复杂度。

Method: AR Forcing 将标准扩散损失集成到自回归训练循环中：在每个时间步，模型使用自身预测结果更新上下文，并优化单步噪声预测目标，从而在训练时显式暴露于推理状态分布。该方法不引入额外判别器或分布匹配损失，保留原始扩散框架和采样器。

Result: 在 RECON、SCAND、HuRoN、TartanDrive 等多个域导航数据集上，AR Forcing 相比强基线方法显著提升了长时序导航中生成图像的一致性以及预测轨迹的准确性，增强了模型在复杂已知和未知环境中的鲁棒性。

Conclusion: AR Forcing 是一种简单有效的自回归训练策略，能够缓解扩散世界模型的训练-推理分布偏移问题，提升长时序预测的稳定性和准确性，且易于集成到现有扩散框架中。

Classification reason: 该论文提出 AR Forcing 自回归训练策略，用于扩散导航世界模型的长时序预测，属于动作条件视频生成与未来预测，与 [CANONICAL] 视频世界模型（video_world_model）高度匹配。

Importance reason: Directly addresses world model and autoregressive video generation for robotics, aligning with high-weight priority subtopics and boost keywords. Signals: world model, autoregressive training, diffusion navigation, long-horizon prediction, distribution shift mitigation
...
TL;DR: Robust Dreamer 提出了一种记忆增强的动作控制自回归视频生成框架，通过 Latent Gaussian Memory（在潜空间用高斯原语锚定扩散潜变量并通过高斯泼溅召回）和 Deviation Learning with Dynamic Deviation Archive（通过一步近似模拟推理时潜变量偏移并注入训练）来解决长自回归 rollout 中的视觉退化与 3D 一致性问题，在 ScanNet、DL3DV 和 OmniWorldGame 上取得了 SOTA 长程生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有帧级动作控制的图像到视频生成方法在长自回归 rollout 中面临严重的灾难性漂移，主要源于两个问题：(1) Latent-RGB Cycling 导致的信息损失——生成的潜变量反复解码为 RGB 再重新编码用于未来条件化，VAE 转换累积退化；(2) 训练-推理 gap——训练时使用干净的记忆，推理时记忆被预测噪声污染，模型无法适应这种分布偏移。

Method: (1) Latent Gaussian Memory：将生成过程中继承的扩散潜变量锚定到高斯原语（Gaussian primitives）上，通过潜空间高斯泼溅（latent-space Gaussian splatting）召回，提供密集、几何感知、视角对齐的条件化信息，避免反复 VAE 转换的累积退化。(2) Deviation Learning with Dynamic Deviation Archive：通过一步近似合成 rollout 导致的潜变量偏移，按自回归阶段和去噪时间戳存储，在训练时注入历史记忆，使生成器暴露于真实的 corrupted 记忆状态，学会在推理前进行内部修正。

Result: 在 ScanNet、DL3DV 和 OmniWorldGame 三个数据集上的实验表明，Robust Dreamer 在长程视频生成任务中取得了 SOTA 性能，显著优于现有方法，在视觉保真度和 3D 一致性方面均有明显提升。

Conclusion: Robust Dreamer 通过潜空间高斯记忆和偏差学习机制有效解决了动作控制自回归视频生成中的长程漂移问题，为交互式世界模拟提供了一种鲁棒的框架。其核心贡献在于避免了 Latent-RGB Cycling 的累积退化，并通过训练时模拟推理噪声来弥合训练-推理 gap。

Classification reason: 该论文提出动作控制的图像到视频生成框架用于交互式世界模拟，属于动作条件视频生成与长时序视频建模，与 [CANONICAL] 视频世界模型 (video_world_model) 高度匹配。

Importance reason: Directly advances action-controlled video generation world models with a novel memory mechanism to solve long-horizon drift, a core user priority. Signals: world model, autoregressive video generation, latent Gaussian memory, deviation learning, 3D consistency
...
### [28] [DecMem: Towards Minute-Long Consistent World Generation with Decoupled Memory](https://arxiv.org/abs/2605.31336)
...
Importance: 高优先级 (96.57) - Top 3 精读候选

TL;DR: DecMem 提出了一种解耦记忆架构，通过稀疏全局记忆（Sparse Global Memory）和锚定局部记忆（Anchored Local Memory）解决长时序视频世界模型中的注意力分散和计算效率问题，实现了分钟级可控长视频生成，在时空一致性上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有可控世界模型在长时序推理下难以保持细粒度的时空一致性。显式3D记忆和粗粒度帧级隐式建模存在局限，而朴素的可学习记忆架构在长程外推中面临计算效率低下和注意力分散（attention dispersion）两大根本性问题。

Method: DecMem 包含两个核心组件：1）稀疏全局记忆（Sparse Global Memory），通过稀疏化机制实现高效细粒度的全局历史访问；2）锚定局部记忆（Anchored Local Memory），通过锚定机制实现稳定高质量的外推。两者解耦协作，兼顾全局一致性与局部生成质量。

Result: 大量实验表明，DecMem 在长视频生成任务上显著优于当前最先进方法，能够实现分钟级可控长视频生成，在保真度和一致性上均取得优异表现。

Conclusion: DecMem 通过解耦记忆架构有效解决了长时序视频世界模型中的注意力分散和计算效率问题，为分钟级一致世界生成提供了可行的解决方案，展示了可学习记忆在长视频生成中的巨大潜力。

Classification reason: 该论文提出 DecMem，一种用于分钟级可控长视频生成的解耦记忆架构，核心关注长时序视频世界模型中的时空一致性与外推能力，属于世界模型方向下的视频世界模型（video_world_model）[CANONICAL] 子主题。同时，论文涉及可控长视频生成，也与多模态生成方向相关。

Importance reason: Directly advances world model and long video generation with a novel decoupled memory architecture, highly relevant to user's priority on video world models and autoregressive video generation. Signals: world model, minute-long video generation, decoupled memory, attention dispersion solution, spatio-temporal consistency
...
### [29] [Subspace-Decomposed JEPAs: Disentangling Progression and Content in Latent World Models](https://arxiv.org/abs/2605.31111)
...
TL;DR: 本文提出SD-JEPA，将JEPA潜空间正交分解为低维进度子空间（通过余弦间隔三元组损失塑造）和高维内容子空间（通过SIGReg正则化），证明两种抗坍缩力作用于不相交坐标从而可加性组合。在控制基准上优于LeWM基线，且1D角度进度坐标可作为场景感知的潜空间指南针，在回溯、扰动下表现出语义感知的重新定位能力，在40个held-out立方体片段上定位语义事件的AUROC提升高达+0.18。


<details>
  <summary>Details</summary>
Motivation: 现有JEPA架构学习紧凑的潜世界模型，但潜空间中没有任何单一坐标被指定用于编码任务进度。这导致模型无法区分"发生了什么变化"（进度）和"变化的是什么"（内容），限制了其在规划、控制和异常检测中的可解释性与有效性。

Method: 将JEPA潜空间正交分解为两个子空间：低维进度子空间（通过余弦间隔三元组损失塑造，使同一任务阶段的样本在角度上接近）和高维内容子空间（通过SIGReg目标正则化）。理论上证明两种抗坍缩力作用于不相交坐标，因此可加性组合而非竞争。得到的1D角度进度坐标可作为场景感知的指南针，随任务进度推进、在回溯时回归、在扰动下重新定位到语义合适的任务阶段。

Result: 在控制基准上优于LeWM基线；在Push-T任务上优于最强的非LeWM JEPA基线。子空间消融实验证实分解是关键成分。在40个held-out立方体片段上，|Δθ_t|在定位语义事件方面比标准潜预测误差的AUROC高出+0.18（±1步容差下97.5%的逐片段胜率）。跨四个环境的线性探针显示，仅占潜空间4.2%的8维进度子空间解释了72-95%的任务进度方差。

Conclusion: SD-JEPA通过潜空间解耦有效分离了任务进度与内容表征，提升了世界模型在控制任务上的性能，同时提供了可解释的进度信号。该进度坐标在回溯、扰动和语义事件定位方面表现出色，超越了传统预测误差标量，为世界模型的可解释性和鲁棒性提供了新方向。

Classification reason: 该论文提出SD-JEPA，一种在潜空间中解耦任务进度与内容的联合嵌入预测架构（JEPA），属于世界模型中的潜空间世界建模与未来预测。虽然其更侧重于潜空间表征解耦而非视频生成，但其核心是学习潜世界模型用于控制与规划，与[视频世界模型]子主题中"世界模型、动作条件视频生成、未来预测"高度相关，且LeWM基线本身即为世界模型方法。

Importance reason: Directly advances world model and continuous latent representation research with a novel subspace decomposition that disentangles progression and content, aligning with top user priorities. Signals: world model, continuous latent representation, JEPA, subspace decomposition, progression subspace
...
### [30] [Equivariant Latent Alignment via Flow Matching under Group Symmetries](https://arxiv.org/abs/2605.30705)
...
TL;DR: 本文提出 Residual Latent Flow，一种基于流匹配的框架，用于纠正等变表示学习中潜空间的对齐偏差，提升新视角合成质量。该方法在旋转群 SO(n) 下显著减少了潜空间错位，提高了等变一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的等变表示学习方法在构建潜空间时，往往存在潜空间错位问题——即预期的群作用与实际所需的变换之间存在不一致，导致学到的潜表示无法一致地保持群对称性所施加的等变关系，影响新视角合成的质量。

Method: 提出 Residual Latent Flow，利用流匹配（flow matching）框架对错位的潜表示进行校正。通过学习一个残差流，将不满足等变关系的潜表示映射到正确对齐的潜空间，从而增强潜空间对群对称性的等变一致性。

Result: 在旋转群 SO(n) 下的新视角合成任务中，该方法显著减少了潜空间错位，并提升了新视角合成的视觉质量和一致性。

Conclusion: Residual Latent Flow 能有效纠正等变潜空间中的对齐偏差，为几何感知生成模型提供了一种通用的潜空间校正框架，在保持等变性的同时提升了生成质量。

Classification reason: 论文提出基于流匹配的等变潜空间对齐方法，用于新视角合成（novel view synthesis），属于3D场景生成与重建范畴，因此映射到 canonical subtopic "3D 生成与重建"。同时论文涉及连续潜空间中的 flow matching 对齐，也与连续语言/多模态方向相关。

Importance reason: Focuses on 3D equivariant latent alignment, which is not a priority subtopic and lacks direct connection to user's key interests in continuous language/video/image generation. Signals: Equivariant representation learning, Flow matching for latent correction, Novel view synthesis, Group symmetry (SO(n)), Not aligned with priority subtopics
...
TL;DR: Function2Scene 提出从功能规格（自然语言设计简报）生成3D室内布局的框架，通过解析用户角色和活动需求，从17个功能设计约束准则中推导约束，并利用工具增强的检查-修复循环（结合几何测量、LLM推理和VLM视觉评估）迭代优化布局，在专业室内设计案例中优于现有LLM基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动的3D室内场景合成方法大多基于物体中心提示（"放什么家具"），而非关注空间如何支持用户活动。然而真实室内设计的核心评判标准是空间对 occupants（用户）活动与物理需求的支撑程度。因此需要一种从功能规格出发、以人为中心的室内布局生成方法。

Method: Function2Scene 框架包含三个主要阶段：(1) 解析阶段——从自然语言设计简报中提取 occupant 角色和活动，从包含17个准则（空间、人体工学、活动、环境四类）的 taxonomy 中推导定制化的功能设计约束；(2) 初始布局生成——基于约束生成初始3D场景；(3) 迭代优化——通过工具增强的 check-and-repair 循环，结合几何测量、LLM上下文推理和VLM视觉评估，反复检查约束满足情况并修复布局。

Result: 在30个专业编写的室内设计案例上，Function2Scene 生成的布局在功能需求满足度上优于最近的LLM场景合成基线方法，在94.3%的成对比较中被偏好。

Conclusion: Function2Scene 将文本驱动的室内场景合成从"放置合理物体"重新定义为"设计支持人类使用的空间"，证明了功能规格驱动的布局生成方法的有效性，为以人为中心的室内场景设计提供了新范式。

Classification reason: 该论文研究从功能规格（自然语言描述）生成3D室内场景布局，属于3D场景生成范畴，与[CANONICAL] subtopic "3D 生成与重建"高度匹配。

Importance reason: Novel functional specification approach for 3D layout generation, but outside core priority subtopics. Signals: 3D indoor scene synthesis, functional design constraints, LLM+VLM iterative refinement, tool-augmented check-and-repair, 94.3% preference over baselines
...
TL;DR: 本文发现一个在自然图像和视频上预训练的 Foundation VAE（编码器+解码器冻结）可以作为 CT 重建、增强和生成的统一接口，无需微调即可在异构扫描条件下保持解剖结构、抑制噪声，并支持条件潜扩散模型进行忠实的多疾病 CT 生成。


<details>
  <summary>Details</summary>
Motivation: 训练 CT 专用 VAE 或大量微调通用 VAE 计算成本高昂，且在不同扫描仪、协议和疾病条件下性能退化严重。本文旨在探索是否可以利用一个在自然图像/视频上大规模预训练的 Foundation VAE，以零训练（training-free）的方式作为 CT 三维重建、增强和生成的统一接口。

Method: 使用一个在自然图像和视频上预训练的 Foundation VAE（编码器和解码器均冻结），将 CT 体素压缩到紧凑潜空间再重建，实现去噪与解剖结构保持。在该潜空间上训练条件潜扩散模型（conditional latent diffusion model），实现多疾病 CT 生成。

Result: 在胰腺肿瘤和肺肿瘤分割任务上，基于 Foundation VAE 重建的训练数据使表面精度（NSD）平均提升 3.9%；在生成任务上，条件潜扩散模型在 Foundation VAE 潜空间中实现 FVD 降低 3.9%、CT CLIP 分数提升 36.2%，18 种疾病的生成忠实性 AUC 提升 2.76%。

Conclusion: Foundation VAE 可以作为 CT 重建、增强和生成的可扩展统一接口，无需针对 CT 领域进行训练或微调，展示了自然图像预训练 VAE 在医学影像领域的强大迁移能力。

Classification reason: 该论文关注 CT 三维重建、增强与生成，使用 Foundation VAE 作为统一接口，属于 3D 生成与重建范畴，映射到 canonical subtopic "3D 生成与重建"。

Importance reason: Focuses on medical CT, which is outside your priority subtopics and has negative keywords. Signals: Foundation VAE, CT reconstruction, training-free, conditional latent diffusion, multi-disease generation
...
### [33] [VolFill: Single-View Amodal 3D Scene Reconstruction with Volumetric Flow Matching](https://arxiv.org/abs/2605.31466)
...
TL;DR: VolFill 提出一种基于体积流匹配（Volumetric Flow Matching）的单视图非模态3D场景重建框架。它使用混合3D VAE将稀疏截断有符号距离函数（T-UDF）网格压缩到紧凑潜空间，再通过潜空间扩散Transformer去噪恢复完整场景，并利用几何基础模型提供空间先验。在SCRREAM和NRGB-D数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 从单张RGB图像重建完整3D场景几何极具挑战性，尤其是推断被遮挡的隐藏结构。现有方法受限于逐射线约束或非结构化点云查询，难以恢复完整的场景几何。作者希望提出一种生成式框架，直接预测完整场景的3D结构，而非依赖传统的像素对齐回归。

Method: VolFill 采用混合3D VAE将稀疏T-UDF网格压缩到紧凑潜空间，然后使用潜空间扩散Transformer（DiT）进行去噪生成。生成过程以几何基础模型（如DINOv2）提取的空间先验为条件，通过流匹配（flow matching）训练。最终输出支持直接表面提取和大规模占用查询的结构化表示。

Result: 在SCRREAM和NRGB-D数据集上的实验表明，VolFill在完整场景重建的几何精度和完整性上显著优于现有基线方法，能够有效恢复被遮挡区域的几何结构。

Conclusion: VolFill 提供了一种基于生成式建模的单视图完整3D场景重建新范式，通过体积流匹配和几何先验条件实现了对隐藏结构的鲁棒推理，为整体空间理解奠定了坚实基础。
...
Importance reason: Strong 3D reconstruction method but outside user's priority subtopics and negative keywords. Signals: 3D scene reconstruction, volumetric flow matching, single-view amodal, diffusion transformer, geometry foundation models
...
### [34] [RayDer: Scalable Self-Supervised Novel View Synthesis from Real-World Video](https://arxiv.org/abs/2605.31535)
...
TL;DR: RayDer 是一个统一的、前馈的 Transformer 架构，将相机估计、场景重建和渲染整合到单一骨干网络中，利用真实世界视频进行自监督新视角合成，展现出清晰的数据/计算幂律缩放行为，在零样本设置下达到与有监督方法可比的性能。


<details>
  <summary>Details</summary>
Motivation: 自监督新视角合成（NVS）难以规模化扩展，主要原因在于真实视频训练的脆弱性以及多网络系统设计的缩放行为难以预测。现有方法通常将相机估计、场景重建和渲染拆分为多个独立模块，导致系统复杂且难以随数据量增长而稳定提升。

Method: RayDer 将相机估计、场景重建和渲染统一到一个前馈 Transformer 骨干网络中，将自监督 NVS 转化为一个单模型的缩放问题。引入一个最小动态状态（minimal dynamic state）作为干扰因子，吸收视频中的时变内容（如运动、光照变化），使模型能够在无约束的真实世界视频上稳定训练。模型保持静态场景 NVS 作为目标任务，动态内容仅作为可扩展的监督信号，而非像 4D NVS 那样重建动态场景。

Result: 在多个模型规模和数量级的数据上，RayDer 展现出清晰的数据和计算幂律缩放行为，优于静态场景数据混合。在大量基准测试上，RayDer 的零样本开放集性能与最先进的有监督方法相当。

Conclusion: RayDer 证明了将自监督 NVS 转化为单一模型缩放问题的可行性，通过统一架构和动态状态吸收机制，实现了可预测的缩放行为和强大的零样本泛化能力，为从真实世界视频中学习 3D 场景理解提供了可扩展的范式。

Classification reason: 该论文提出 RayDer，一种自监督新视角合成（NVS）方法，属于 3D 场景重建与渲染范畴，与 canonical subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Strong scaling laws and self-supervised NVS are valuable, but the 3D generation subtopic is not a priority. Signals: Self-supervised novel view synthesis, Unified transformer for camera, reconstruction, rendering, Clean power-law scaling, Zero-shot competitive with supervised, Real-world video training
...
### [35] [Smaller and Faster 3DGS via Post-Training Dictionary Learning](https://arxiv.org/abs/2605.30396)
...
<details>
  <summary>Details</summary>
Motivation: 3DGS 模型在训练后通常占用大量内存，限制了其在低算力设备上的部署。现有压缩方法往往引入额外可训练参数，且压缩后图像质量下降明显。

Method: 提出基于字典学习的后训练压缩框架：将 3DGS 的高斯属性（如颜色、协方差等）投影到字典学习得到的低维子空间中进行压缩，无需修改原始模型架构或重新训练。

Result: 在 13 个基准场景上，对 3DGS、3DGS-MCMC 和 PixelGS 分别实现平均 3.95x、3.10x 和 4.55x 的压缩比，同时渲染速度提升 23.3%、24.3% 和 25.3%，图像质量基本保持不变。

Conclusion: 字典学习是一种简单高效且通用的 3DGS 后训练压缩方案，可即插即用于现有 3DGS 模型，在压缩、速度和保真度之间取得了良好的平衡。

Classification reason: 该论文研究 3D Gaussian Splatting (3DGS) 的压缩方法，属于 3D 场景表示与神经渲染范畴，与 canonical subtopic "3D 生成与重建" 高度匹配。

Importance reason: Focuses on 3DGS compression, which is not a priority subtopic and lacks direct connection to user's core interests in continuous language/multimodal models or video generation. Signals: 3DGS compression, dictionary learning, post-training, rendering speedup, not in priority subtopics
...
TL;DR: DSD-GS 提出一种基于静态-动态分解的高斯泼溅框架，利用前馈编码器和光流模型将场景分为静态和动态部分，仅对动态区域建模，在 Neural 3D 数据集上仅需10分钟训练、超700 FPS渲染速度，且无需 COLMAP 预处理。


<details>
  <summary>Details</summary>
Motivation: 现有动态 3DGS 方法将所有高斯体素视为动态组件，导致计算效率低下；而基于分解的方法虽然有所改进，但重建质量下降且训练时间较长。本文旨在解决这些局限性。

Method: 提出静态-动态分解策略：使用前馈高斯泼溅编码器（Feed-Forward Gaussian Splatting encoder）和光流模型，将场景分解为静态和动态组件，仅对动态区域进行建模，消除静态区域的冗余计算。

Result: 在 Neural 3D 数据集上，训练仅需10分钟，渲染速度超过700 FPS（1352×1014分辨率，单张RTX 5090 GPU），在渲染质量、训练/渲染速度和存储效率上均达到SOTA，且无需COLMAP预处理。

Conclusion: 静态-动态分解策略能有效提升动态场景重建的效率和质量，同时增强可复现性，为动态场景的实时高保真重建提供了可行方案。

Classification reason: 该论文研究动态场景重建与新视角合成，属于动态3D/4D场景生成与重建范畴，与[多模态生成]方向下的[4D 与动态场景生成]（dynamic_4d_generation）子主题高度匹配。

Importance reason: Strong engineering contribution to dynamic 3DGS but outside user's priority subtopics and lacks direct connection to continuous language/multimodal models or autoregressive generation. Signals: Dynamic scene reconstruction, Gaussian Splatting, static-dynamic decomposition, fast training (10 min), high FPS (700+)
...
### [37] [Learning Global Motion with Compact Gaussians for Feed-Forward 4D Reconstruction](https://arxiv.org/abs/2605.31595)
...
TL;DR: C4G 提出了一种前馈式4D重建框架，使用紧凑的、时间条件化的可学习高斯查询token，从单目视频中重建动态场景。每个token聚合全时序上下文特征并解码出3D高斯，其位置由目标时间戳调制，实现全局一致的运动建模。此外引入视频扩散模型增强渲染质量，并支持点跟踪和动态场景理解等下游任务。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式动态场景重建方法逐帧独立预测3D高斯，导致高斯冗余和视角依赖偏差，难以有效学习场景运动。本文旨在设计一种无需逐场景优化、能全局建模运动的前馈式4D重建框架。

Method: 提出C4G框架：1）使用一组紧凑的、时间条件化的可学习高斯查询token，每个token通过交叉注意力聚合全时序上下文特征；2）解码出3D高斯，其位置由目标时间戳调制，实现全局运动建模；3）引入视频扩散模型作为渲染增强模块提升细节质量；4）将特征提升到4D特征场，支持点跟踪和动态场景理解。

Result: 在多个动态场景数据集上，C4G使用显著更少的高斯数量实现了强竞争力的新视角合成性能，且无需相机位姿。在运动建模和大时间间隔鲁棒性方面表现优异，同时支持点跟踪等下游任务。

Conclusion: C4G通过紧凑高斯查询token和全局时序特征聚合，有效解决了前馈式4D重建中的运动建模难题，为动态场景重建提供了一种高效、无需相机位姿的新范式。

Classification reason: 该论文提出前馈式4D动态场景重建框架C4G，使用紧凑高斯查询token进行动态场景的4D重建和新视角合成，属于动态3D/4D场景生成与重建方向，与[CANONICAL] subtopic "4D 与动态场景生成"（dynamic_4d_generation）高度匹配。同时其运动建模能力也与世界模型方向相关。

Importance reason: Novel feed-forward 4D reconstruction with compact Gaussians, but tangential to user's core focus on continuous language/multimodal models and autoregressive generation. Signals: 4D reconstruction, feed-forward, compact Gaussians, video diffusion enhancement, dynamic scene understanding
...
### [38] [Envisioning Beyond the Few: Disentangled Semantics and Primitives for Few-Shot Atypical Layout-to-Image Generation](https://arxiv.org/abs/2605.31266)
...
TL;DR: 本文提出一种解耦语义与基元的表示驱动框架，用于解决少样本非典型布局到图像生成中的表示碎片化问题，通过语义锚定、基元注入和概念引导三个模块在5-shot设定下显著提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有布局到图像生成方法在少样本非典型场景下会产生碎片化和扭曲的生成结果，作者将其归因为"表示碎片化"——即语义身份与视觉细节纠缠导致的粒度不匹配问题。

Method: 提出表示驱动框架，包含三个模块：1) Semantic Anchoring（语义锚定）将类别语义聚合为锚点以保持稳定身份；2) Primitive Imbuing（基元注入）建模可重组的基元以进行鲁棒的局部细节建模；3) Conceptual Steering（概念引导）通过显著性感知目标函数优化，保持前景语义一致性。

Result: 在多个非典型领域的5-shot设定下，该方法在视觉保真度和对齐度上一致优于现有最先进的布局到图像生成方法。

Conclusion: 解耦语义与基元的表示学习能有效缓解少样本布局到图像生成中的表示碎片化问题，为少样本条件下的可控图像生成提供了新思路。

Classification reason: 该论文研究少样本布局到图像生成（few-shot layout-to-image generation），属于条件图像生成中的概念定制与个性化生成范畴，与 CANONICAL 子主题"主体与概念驱动生成"（customization_generation）高度匹配。

Importance reason: Focuses on few-shot layout-to-image generation, which is not a priority subtopic and lacks strong alignment with user's key boost keywords. Signals: Few-shot generation, layout-to-image, disentangled semantics, not autoregressive or diffusion video/image generation, no continuous language model or world model signals
...
TL;DR: PhyDrawGen 是一个神经符号管线，用于从自然语言文本生成物理上准确的示意图。它通过LLM提取场景图，用确定性求解器转换为精确几何图元（PSLG），再通过Qwen-VL的propose-verify循环迭代修正约束违规，在力学、光学和电磁学三大领域的1,449个问题上显著优于GPT-5-image、Gemini 2.5 Flash等模型。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型虽然能产生视觉上合理的物理示意图，但系统性地存在幻觉（如错误力向量）、违反守恒定律和几何约束等问题，无法保证物理准确性。

Method: 提出三阶段神经符号管线：(1) LLM从问题文本中提取类型化场景图；(2) 确定性求解器将场景图转换为平面直线图（PSLG），编码力平衡、光路和场拓扑为精确几何图元；(3) 微调后的Qwen-VL模型执行视觉grounded的propose-verify循环，迭代修正约束违规。

Result: 在包含1,449个问题的基准上（涵盖力学、光学和电磁学），PhyDrawGen显著优于GPT-5-image、Gemini 2.5 Flash和Gemini 3 Pro，在异常物体问题上也展现出鲁棒的物理准确性。

Conclusion: 将神经符号方法与视觉验证循环相结合，可以有效解决物理示意图生成中的幻觉和约束违反问题，为科学教育等需要严格物理准确性的应用提供了可行方案。

Classification reason: 该论文提出从自然语言生成物理示意图的神经符号管线，涉及图像生成与编辑（通过propose-verify循环迭代修正约束违规），属于多模态生成方向。最接近的规范子主题是"图像与视频编辑"（visual_editing），因为其核心是可控的物理约束驱动的视觉生成/编辑过程。同时，论文使用了Qwen-VL进行视觉理解与验证，也涉及多模态理解与生成方向。

Importance reason: Strong neuro-symbolic pipeline for physically accurate diagram generation, but outside user's core focus on continuous multimodal models and autoregressive generation. Signals: neuro-symbolic pipeline, physically grounded generation, propose-verify loop, outperforms GPT-5-image, mechanics/optics/electromagnetism benchmark
...
### [40] [SANA-Streaming: Real-time Streaming Video Editing with Hybrid Diffusion Transformer](https://arxiv.org/abs/2605.30409)
...
TL;DR: SANA-Streaming 是一个系统-算法协同设计的实时流式视频编辑框架，通过混合 Diffusion Transformer 架构、Cycle-Reverse Regularization 训练策略和高效系统协同设计（融合 GDN 核 + 混合精度量化），在单张 RTX 5090 上实现 1280×704 分辨率 24 FPS 的端到端实时视频编辑。
...
Method: (1) 混合 Diffusion Transformer 架构：在部分块中引入 softmax attention 增强局部建模能力，同时保留线性层的效率；(2) Cycle-Reverse Regularization：通过 flow matching 从生成内容预测源帧，强制语义一致性，无需成对长视频训练数据；(3) 高效系统协同设计：融合 GDN 核与针对 NVIDIA Blackwell (RTX 5090) 优化的混合精度量化 (MPQ)，最大化 Tensor Core 利用率。

Result: 在单张 RTX 5090 GPU 上实现 1280×704 分辨率 24 FPS 端到端实时编辑，DiT 核心达 58 FPS。在时序一致性和系统吞吐量上显著优于现有 SOTA 方法。

Conclusion: SANA-Streaming 通过系统-算法协同设计，首次在消费级 GPU 上实现了高分辨率实时流式视频编辑，验证了混合注意力架构、循环反向正则化和硬件感知量化在视频编辑中的有效性。

Classification reason: 该论文提出实时流式视频到视频编辑框架 SANA-Streaming，属于视频编辑方向，与 [CANONICAL] subtopic "visual_editing (图像与视频编辑)" 高度匹配。

Importance reason: Strong system-algorithm co-design for real-time video editing, but subtopic is outside user's priority list and lacks direct connection to continuous language models or world models. Signals: Real-time streaming video editing, Hybrid Diffusion Transformer, Cycle-Reverse Regularization, System-algorithm co-design, RTX 5090 optimization
...
### [41] [DTG-Restore: Training-Free Diffusion Refinement for Generative Video Super-Resolution](https://arxiv.org/abs/2605.30431)
...
TL;DR: DTG-Restore 提出一种无需训练的视频超分辨率框架，通过解耦分类器自由引导中的条件与无条件分支的时间步，利用更干净时间步的无条件分支作为前瞻先验，逐步从结构校正过渡到细节增强，结合即插即用的恢复模块提升生成视频的结构保真度和时间稳定性，并构建了 GenWarp480 基准。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在利用先验进行恢复时，标准分类器自由引导（CFG）中条件分支与无条件分支耦合过强，导致扭曲内容被复制而非修复。作者希望在不重新训练模型的前提下，通过解耦时间步来改善低质视频的结构和细节恢复。

Method: 提出解耦时间引导（Decoupled Time Guidance, DTG），在扩散采样过程中，让无条件分支在更干净的扩散时间步进行评估，从而提供"前瞻先验"以保留几何结构并抑制扭曲内容复制。该时间偏差在采样过程中逐步退火，使模型从结构校正平滑过渡到细节增强。该方法可与任意现成的恢复模块即插即用。

Result: 在 AI 生成和真实世界视频上均取得显著的结构保真度和时间稳定性提升。构建的 GenWarp480 基准包含 4400 个扭曲的 480p 视频，涵盖面部扭曲、身体错位和空间伪影等生成式退化类型，实验表明方法无需训练即可大幅改善恢复质量。

Conclusion: DTG-Restore 通过简单的时间步解耦策略，有效提升了视频超分辨率恢复的质量，无需重新训练模型，具有良好的通用性和即插即用特性。

Classification reason: 该论文提出无训练的视频超分辨率恢复框架，属于视频质量增强与修复任务，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）中的视频编辑/修复方向高度相关。

Importance reason: Training-free video super-resolution is useful but not aligned with user's priority on autoregressive/diffusion video generation or continuous multimodal models. Signals: video super-resolution, training-free, diffusion refinement, classifier-free guidance, GenWarp480 benchmark
...
### [42] [Mitigating Content Shift and Hallucination in GenAI Image Editing via Structural Refinement](https://arxiv.org/abs/2605.30437)
...
TL;DR: 本文提出一种后处理框架，用于解决GenAI图像编辑中生成内容与原始图像之间的空间错位、纹理失真和内容幻觉问题。通过建立粗粒度的空间和光度对应关系，再执行融合阶段，在保留GenAI增强效果的同时抑制幻觉内容，保持像素级结构一致性。


<details>
  <summary>Details</summary>
Motivation: GenAI图像编辑器（如Nano Banana）能生成视觉上令人满意的编辑结果，但其生成性质常引入空间错位、纹理失真和内容幻觉，这些对需要像素级保真度的下游工作流是有害的。现有方法缺乏在保持结构忠实性的同时保留感知增强效果的有效方案。

Method: 提出"结构保持GenAI融合"框架：首先建立输入图像与GenAI增强图像之间的粗粒度空间和光度对应关系，然后执行融合阶段，将期望的增强效果转移过来，同时抑制幻觉内容。该框架作为后处理模块，可应用于黑盒GenAI图像编辑器。

Result: 与光真实感风格迁移和图像融合的代表性方法相比，本文方法在保持像素级结构一致性和输入分辨率的同时，更好地保留了美学质量。

Conclusion: 本文提出的后处理融合框架能有效解决GenAI图像编辑中的内容偏移和幻觉问题，在保持结构忠实性的同时保留感知增强效果，为需要像素级保真度的下游应用提供了可行方案。

Classification reason: 该论文研究GenAI图像编辑中的结构保持融合问题，属于图像编辑范畴，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度匹配。

Importance reason: Addresses image editing hallucination but is a post-processing fusion method, not aligned with priority subtopics like continuous models or video generation. Signals: image editing, hallucination mitigation, structural refinement, post-processing fusion, not autoregressive or diffusion generation
...
TL;DR: 本文系统比较了多种2D Inpainter在3DGS场景物体移除与修复任务中的表现，发现基于重建的Inpainter在3D一致性上优于生成式扩散模型；从头初始化场景比微调现有场景效果更好；并提出了一个包含真实地面真值的新多物体场景数据集。


<details>
  <summary>Details</summary>
Motivation: 3DGS场景中的物体移除与Inpainting面临跨视角3D一致性的挑战，现有方法缺乏对不同2D Inpainter在3D场景中适用性的系统比较，且缺少包含真实地面真值和复杂遮挡场景的360°数据集。

Method: 系统比较了多种2D Inpainter（重建型 vs 生成式扩散模型）在3DGS场景中的表现；对比了两种单步方法：从头初始化场景 vs 微调现有场景；引入了一个包含真实地面真值、多视角遮挡的新多物体场景数据集。

Result: 重建型Inpainter在3D一致性上优于生成式扩散模型；从头初始化场景比微调现有场景产生更高质量的结果；使用生成式2D Inpainter的简单基线表明，在3D场景中先移除物体再进行Inpainting至关重要。

Conclusion: 为3DGS场景的物体移除与Inpainting提供了系统的基准比较，揭示了不同2D Inpainter和不同方法策略的优劣，并贡献了一个新的多物体场景数据集以推动该领域研究。

Classification reason: 论文研究3D高斯泼溅场景中的物体移除与Inpainting方法，属于图像/3D场景编辑范畴，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度匹配。

Importance reason: Focuses on 3D inpainting benchmarks, not on the user's priority subtopics like continuous language models or autoregressive video generation. Signals: 3DGS inpainting, 2D inpainter comparison, multi-object scene dataset, not aligned with priority subtopics, no boost keywords matched
...
### [44] [Guidance for Low-Level Perceptual Editing in Unconditional Diffusion Models](https://arxiv.org/abs/2605.31162)
...
TL;DR: 本文提出一种无需训练的无条件扩散模型图像编辑框架，通过提取退化概念向量并结合瓶颈修补（bottleneck patching）与无分类器引导（CFG），在推理阶段引导采样远离退化流形，实现全局低层感知编辑（如美学增强），解决了现有 h-space patching 方法在全局低层变换上的系统性失败问题。


<details>
  <summary>Details</summary>
Motivation: 无条件扩散模型虽具有强大的生成先验，但如何引导其生成美学增强的输出仍未被充分探索。现有的 h-space patching 方法在全局低层变换（如美学和感知精炼）上系统性失败，因此需要一种新的无需训练的推理时编辑机制。

Method: 提出一个通用的推理时图像编辑框架：(1) 提取退化概念向量（degradation concept vectors）；(2) 结合瓶颈修补（bottleneck patching）与无分类器引导（classifier-free guidance），在采样过程中引导模型远离退化流形，从而生成一致改进的图像，无需任何模型重训练。

Result: 实验表明该方法在无条件扩散模型上成功实现了全局低层感知编辑（如美学增强），相比 h-space patching 基线有显著改进，且无需训练。

Conclusion: 本文证明了通过退化概念向量与瓶颈修补+CFG 的组合，可以在无条件扩散模型上实现有效的训练无关低层感知编辑，为无条件生成模型的可控编辑提供了新范式。

Classification reason: 该论文研究无条件扩散模型中的图像编辑方法，属于多模态生成方向下的图像编辑子主题，与 [CANONICAL] subtopic "visual_editing"（图像与视频编辑）高度匹配。

Importance reason: Focuses on unconditional diffusion model editing, which is not aligned with user's priority on autoregressive video/image generation or continuous multimodal models. Signals: Unconditional diffusion models, training-free editing, low-level perceptual editing, h-space patching, classifier-free guidance
...
### [45] [Dex2HOI: Dexterous Bimanual Two-Object Interaction Generation](https://arxiv.org/abs/2605.30444)
...
TL;DR: Dex2HOI 是一个统一的扩散模型，用于从文本生成单手/双手单物体/双物体的人-物交互动作序列。它采用双流扩散架构，每个物体有独立交互流并通过双向交叉注意力协调，结合运动融合网络、手部相对物体表示和全序列接触感知条件，通过自回归前缀条件窗口采样实现任意长度序列的实时生成，推理速度相比先前方法提升最高540倍。


<details>
  <summary>Details</summary>
Motivation: 现有4D人-物交互（HOI）生成研究主要关注单物体操作，忽略了人类自然行为中双手协调和同时操作多个物体的特性。作者旨在填补这一空白，实现更真实、更具表现力的多物体交互动作生成。

Method: 提出Dex2HOI，核心是双流扩散（Dual-Stream Diffusion）架构：每个物体由独立的交互流处理，流之间通过双向交叉注意力协调。引入运动融合网络（Motion Fusion Network），结合手部相对物体表示（hand-relative object representations）和全序列接触感知条件（contact-aware conditioning）。通过自回归前缀条件窗口采样，实现任意长度序列的实时生成，无需测试时优化。
...
Importance reason: Focuses on dexterous bimanual interaction generation, which is outside the user's priority subtopics and lacks alignment with continuous multimodal or world model directions. Signals: diffusion model for HOI, dual-stream architecture, real-time inference speedup, single/two-object manipulation, no continuous language or world model focus
...
Method: 提出 OmniME 框架，包含三个互补组件：(1) 回顾性特征监督（Retrospective Feature Supervision），在 Transformer 层间施加由粗到细的一致性约束；(2) 运动保持机制（Motion Preservation Mechanism），根据源-目标相似性关注细微变化；(3) 三元组语义对齐（Triplet-based Semantic Alignment），增强文本-运动对应关系。三者形成统一的监督范式来平衡变化与不变。

Result: 在 MotionFix 和 STANCE Adjustment 数据集上的大量实验表明，OmniME 在编辑对齐方面达到了最先进的性能，验证了统一学习框架的有效性。

Conclusion: 本文提出的全监督正负学习框架能够有效平衡运动编辑中的变化与不变性，为文本驱动的人体运动编辑提供了一种新的统一监督范式。

Classification reason: 该论文研究基于文本的人体运动编辑，属于人体动作生成与编辑范畴，与 [CANONICAL] 子主题"数字人与动作生成"（avatar_motion_generation）高度匹配。

Importance reason: Focuses on motion editing, not in priority subtopics, and lacks boost keywords. Signals: motion editing, human motion, diffusion-based, positive-negative learning, text-based editing
...
### [47] [TokTalk: Expressive Real-time Facial Animation from Audio-LLM Tokens](https://arxiv.org/abs/2605.31294)
...
TL;DR: TokTalk 提出一种直接从流式音频-LLM token 实时生成富有表现力的 3D 面部动画的系统，使用基于块的连续条件流匹配模型，并支持轻量级适配到任意基于 token 的音频-LLM。


<details>
  <summary>Details</summary>
Motivation: 现有对话式虚拟人面部表情和对话流程仍显机械，主要因为需要依次经过语音识别、文本生成、文本响应、语音合成和音频驱动面部动画等多个阶段，导致延迟高且表现力不足。作者发现音频-LLM 产生的音频 token 已包含足够信息来重建合理的面部表现，因此希望直接利用这些 token 生成面部动画。

Method: 构建了一个音频 token 到 3D 面部运动的数据集；提出基于块的连续条件流匹配（Chunk-based Conditional Flow Matching）模型；设计轻量级适配策略使模型可无缝连接到任意基于 token 的音频-LLM；通过分块处理实现延迟与面部质量之间的参数化权衡。

Result: 实时性能与先前方法相当，在质量、表现力和 3D 面部控制方面显著优于现有方案（通过感知研究验证）。消融实验展示了延迟与质量之间的权衡。展示了聊天机器人 Avatar、语音驱动用户 Avatar 和动画导演界面等多种应用。

Conclusion: TokTalk 证明了音频-LLM token 可直接用于生成富有表现力的实时 3D 面部动画，为构建更自然、更具表现力的对话式虚拟人提供了有效方案。

Classification reason: 该论文研究从音频-LLM token 实时生成 3D 面部动画（说话头/虚拟人面部动画），属于多模态生成方向下的 [CANONICAL] 数字人与动作生成（avatar_motion_generation）子主题。

Importance reason: Novel application of continuous flow matching to audio-LLM tokens for real-time facial animation, but tangential to user's core priorities on video/image generation and world models. Signals: continuous flow matching, real-time facial animation, audio-LLM tokens, expressive avatar, chunk-based processing
...
### [48] [Controllable Lung Nodule Synthesis via Histogram-Regularized Latent Diffusion Models](https://arxiv.org/abs/2605.30631)
...
TL;DR: 本文提出一种基于直方图正则化的可控潜扩散模型，用于在3D CT体积中合成肺结节。通过引入HU直方图条件与可微直方图正则化项，模型能准确建模不同亚型结节（实性、部分实性、磨玻璃）的强度分布，生成视觉逼真的结节，并有效提升下游分类任务性能。


<details>
  <summary>Details</summary>
Motivation: CT肺癌筛查中，多样化的带标注肺结节数据集稀缺，限制了自动诊断系统的发展。现有扩散生成模型主要优化空间重建损失，关注体素级相似性，但未能充分约束病灶级强度分布，导致生成结节纹理过平滑、不同亚型（实性/部分实性/磨玻璃）的衰减特征区分度不足。

Method: 提出可控潜扩散模型，在潜空间中进行条件生成。条件包括：结节亚型（类别条件）、空间掩码（位置条件）和HU直方图（强度分布条件）。关键创新是引入可微的直方图正则化项，在扩散去噪过程中约束生成结节的体素强度分布与目标直方图一致，而非仅依赖空间损失。模型在3D CT全卷中合成结节。

Result: 在肺部CT数据上的实验表明：该方法在定量指标和视觉图灵测试中均达到强视觉真实性；生成结节用于数据增强时，能提升下游临床任务性能，尤其对欠代表性结节亚型效果显著；对亚型感知的恶性分类也展现出潜在益处。

Conclusion: 直方图正则化能有效约束潜扩散模型生成结节的强度分布，提升不同亚型结节的视觉真实性和亚型一致性，为医学图像数据增强提供了一种可控且有效的生成方案。

Classification reason: 该论文提出可控潜扩散模型用于肺结节合成，属于扩散模型在医学图像生成中的应用。虽然具体任务是医学图像，但核心方法是条件潜扩散模型（latent diffusion model），与[文生图与图像扩散]这一规范子主题的扩散模型生成方法高度契合，因此复用该子主题。

Importance reason: Focuses on medical image synthesis for lung nodules, which is outside the user's priority subtopics and includes negative keywords. Signals: medical diagnosis, lung nodule synthesis, histogram regularization, diffusion model, data augmentation
...
### [49] [OmniMem: Scalable and Adaptive Memory Retrieval for Long Video Generation](https://arxiv.org/abs/2605.30519)
...
TL;DR: OmniMem提出了一种显式的全范围记忆检索框架，通过自适应窗口排除、查询共享KV选择和逐头分散KV访问等机制，在自回归长视频生成中实现高效的稀疏KV检索，显著提升动态度并保持一致性。
...
Method: OmniMem包含三个核心设计：(1) 自适应窗口排除（Adaptive Window Exclusion）——在有足够长程历史时，从候选集中排除局部窗口块，将稀疏预算留给信息量更大的长程检索；(2) 查询共享KV选择（Query-Shared KV Selection）——减少跨查询的多样性，避免冗余选择；(3) 逐头分散KV访问（Per-Head Scattered KV Access）——避免将各头的选择扩展为大的KV缓冲区，允许每个注意力头根据自身选择模式检索非连续的KV块。

Result: 在长视频生成实验中，OmniMem相比强基线将动态度（Dynamic Degree）提升了52.3%，同时保持了强一致性，且内存使用量相当。

Conclusion: OmniMem通过显式全范围稀疏KV检索，有效解决了自回归长视频生成中历史信息访问的局部偏差和联合爆炸问题，在提升视频动态性的同时保持了生成一致性和内存效率。

Classification reason: 该论文聚焦长视频生成中的KV缓存检索优化，属于视频生成方法范畴。虽然论文使用自回归范式，但核心是视频生成中的记忆检索机制，与"文生视频与视频扩散"（text_to_video_diffusion）这一CANONICAL subtopic最为相关，该subtopic涵盖视频生成方法（含自回归视频生成）。

Importance reason: Directly addresses efficient autoregressive video generation with a novel memory retrieval method, aligning with top priority subtopics and boost keywords. Signals: autoregressive video generation, efficient inference, long video generation, sparse KV retrieval, dynamic degree improvement
...
### [50] [CameraNoise: Enabling Faithful Camera Control in Video Diffusion through Geometry-Flow-Guided Noise Warping](https://arxiv.org/abs/2605.30774)
...
TL;DR: CameraNoise 提出一种流到噪声的扭曲方法，将相机运动编码为时序一致的随机表示，通过几何引导重投影流和噪声扭曲算法，在保持扩散高斯先验的同时实现精确的相机控制，显著提升了视频扩散中相机轨迹的忠实度和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在相机姿态控制方面存在不足，直接注入数值相机参数的方法难以弥合抽象坐标与视觉内容之间的鸿沟，导致结构失真。需要一种能将相机运动忠实编码到扩散过程中的方法。

Method: 提出 CameraNoise 框架，包含两个核心组件：(1) 几何引导重投影流 (Geometry-guided Reprojection Flow)，用于计算相机变换下的像素对应关系；(2) 噪声扭曲算法，在保持扩散高斯先验的前提下，根据重投影流对噪声进行扭曲，将相机运动嵌入噪声空间，从而解耦运动与场景外观。

Result: 大量实验表明，该方法在视觉质量和轨迹忠实度上显著优于现有方法，能够生成稳定、高保真的视频，并精确遵循指定的相机轨迹。

Conclusion: CameraNoise 通过将相机控制嵌入噪声空间而非直接注入参数，有效解决了视频扩散中相机控制的几何一致性问题，为视频生成中的精确相机操控提供了新范式。

Classification reason: 该论文提出 CameraNoise，通过几何流引导的噪声扭曲实现视频扩散中的精确相机控制，属于视频扩散模型中的相机控制方法，与 [CANONICAL] 文生视频与视频扩散 子主题高度匹配。

Importance reason: Directly advances controllable video generation with a novel noise warping method, highly relevant to your priority on video generation and world models. Signals: video diffusion, camera control, noise warping, geometry-guided flow, trajectory faithfulness
...
TL;DR: 本文提出SlotMemory，一种面向流式长视频生成的物体中心键值记忆机制。通过将Transformer的KV流形分解为离散的可重用语义槽，实现实体级持久化和提示感知检索，在60秒交互式叙事视频生成中取得SOTA质量评分81.61，动态一致性相对提升22.8%。
...
Method: 提出SlotMemory机制，将Transformer的KV流形分解为离散的可重用语义槽（slot），每个槽作为路由地址索引和存储高保真KV token，实现实体级持久化和跨长时域的提示感知检索。基于Wan2.1-T2V-1.3B骨干进行流式视频扩散生成。

Result: 在60秒交互式叙事视频生成任务上，SlotMemory取得质量评分81.61（SOTA），动态一致性相对提升22.8%，优于现有最强流式基线。
...
Importance reason: Proposes object-centric KV memory for streaming video generation, directly advancing autoregressive video generation and world model persistence. Signals: object-centric memory, streaming long-video generation, entity-level persistence, dynamic consistency improvement, Wan2.1-T2V-1.3B backbone
...
<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的天气临近预报方法依赖确定性压缩来降低高维天气数据的复杂度，限制了解码过程中对不确定性的捕获能力。对于极端天气事件，解码变异性大，需要更好的概率建模。

Method: 提出 FREUD 模型：1) 逐帧编码器（Frame-wise Encoder）对每个雷达帧独立编码，支持连续预报更新；2) 统一视频解码器（United Decoder）保证时序一致性；3) 不确定性保持的第一阶段（uncertainty-preserving first stage）通过集成方式捕获偶然不确定性；4) 在紧凑潜空间中使用 rectified flow transformer 进行生成建模。

Result: 在 SEVIR 基准上达到降水临近预报的 SOTA 性能，并通过模型缩放和测试时缩放进一步提升了性能。

Conclusion: FREUD 证明了基于 rectified flow 的概率生成方法在天气临近预报中的有效性，不确定性保持的编码方式对极端天气事件预测尤其有益，且模型具有良好的可扩展性。

Classification reason: 该论文使用 rectified flow transformer 进行降水临近预报（precipitation nowcasting），本质上是将天气雷达数据作为视频帧进行条件生成，属于视频扩散/流匹配生成方法。虽然应用领域是天气预报，但技术范式（latent-space rectified flow、视频编解码、时空一致性）与文生视频/视频扩散高度一致，因此映射到 canonical subtopic "文生视频与视频扩散"。

Importance reason: While technically solid with rectified flow transformers for video nowcasting, the weather domain is a negative keyword and the paper lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: Rectified flow transformer, probabilistic nowcasting, SEVIR benchmark, frame-wise encoding, uncertainty-preserving first stage
...
### [53] [TunerDiT: Training-free Progressive Steering of Diffusion Transformer for Multi-Event Video Generation](https://arxiv.org/abs/2605.31590)
...
TL;DR: TunerDiT 是一种无需额外训练的视频扩散 Transformer 渐进式引导方法，通过事件分区掩码和跨事件提示融合实现多事件视频生成，在 8 项指标上达到训练无关方法的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成方法在处理包含多个事件的长时间视频时表现不佳，难以在保持视频整体一致性的同时清晰分离不同事件。作者希望探索扩散过程中文本条件如何影响生成，并据此设计一种无需训练即可引导多事件视频生成的方法。

Method: (1) 首先通过分析扩散 DiT 的去噪轨迹，发现存在"转折点"——在去噪早期文本条件主导全局布局，后期则影响细粒度细节。(2) 提出 Event-Partitioned Masking（事件分区掩码），在去噪过程中强制事件边界，同时允许跨事件过渡带。(3) 提出 Cross-Event Prompt Fusion（跨事件提示融合），在后期阶段注入相邻事件的语义信息进行细化。(4) 自建多事件视频生成评测集 Meve。

Result: 在 8 项指标上达到训练无关方法的最优性能，文本对齐度随事件数量增加而提升，表明该方法具有随事件数扩展的潜力。在视频一致性和事件分离之间提供可调节的权衡。

Conclusion: TunerDiT 通过利用扩散过程的固有特性，以无需训练的方式有效解决了多事件视频生成中的事件分离与一致性保持问题，为长视频多事件生成提供了新的思路。

Classification reason: 该论文提出 TunerDiT，一种无需训练的多事件视频生成渐进式引导方法，基于视频扩散 Transformer (DiT)。核心贡献在于视频扩散模型中的多事件文本到视频生成，属于多模态生成方向下的文生视频与视频扩散（text_to_video_diffusion）规范子主题。

Importance reason: Directly advances training-free multi-event video generation with diffusion transformers, aligning with user's high-priority video generation and world model interests. Signals: training-free, diffusion transformer, multi-event video generation, progressive steering, event-partitioned masking
...
TL;DR: LVSA是一种无需训练、模型无关的块稀疏注意力方法，通过结构化窗口模式与旋转全局锚点结合，显著降低长视频扩散Transformer的计算开销（最高3.33x加速），同时避免长距离时序伪影，并能在单GPU上生成长于训练时域的视频。


<details>
  <summary>Details</summary>
Motivation: 长视频扩散推理中，密集自注意力的计算成本随序列长度二次增长，且超出训练时域后模型输出趋于"冻结"重复。现有方法要么需要重新训练成本过高，要么无法同时满足性能和质量的扩展需求。

Method: 提出LVSA（Long Video Sparse Attention），一种块稀疏注意力模式，结合结构化窗口注意力与旋转全局锚点（rotating global anchors），避免固定网格偏差导致的长距离时序伪影。无需训练，即插即用，兼容FlashInfer kernel加速。

Result: 在Wan 2.1 1.3B上6x时域加速3.17x，Wan 2.1 14B上6x时域加速2.98x，HunyuanVideo 1.5上1.5x时域加速3.33x；相比RIFLEx和UltraViCo分别加速2.41x和3.27x；在NPU上也取得2.71x-3.24x加速。同时提出VQeval工具更公平地评估循环视频失败。

Conclusion: LVSA是一种高效、无需训练、模型无关的长视频扩散稀疏注意力方法，在训练时域内质量中性，在扩展时域上质量正向，显著降低计算成本并支持更长视频生成。

Classification reason: 该论文提出LVSA，一种无需训练的块稀疏注意力机制，用于加速长视频扩散模型的推理，属于生成模型效率优化范畴，因此映射到[CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Directly addresses efficient inference for long video diffusion, a high-priority subtopic, with a training-free method that enables longer generation on single GPU. Signals: training-free sparse attention, long video diffusion, 3.33x speedup, single GPU extended horizon, model-agnostic
...
Method: 提出 CoFRe 框架：(1) 固定点掩码生成模型（FP-MGM），用固定点求解器替代部分去噪器层，实现自适应深度且参数更少；(2) 跨步一致性损失（cross-step consistency loss），对齐相邻去噪步骤的隐藏表示；(3) 三态重用（3SR），对未改变、仍掩码和新揭示的 token 分别处理，用前一步解热启动求解器。此外，预训练 MGM 可通过短微调转换为 FP-MGM。

Result: 在 OpenWebText 上，参数量减少 38.8%，训练时间减少 11.5%，显存减少 16.9%，在 96 次 Transformer-block 前向传播预算下生成困惑度从 830.8 降至 101.8。在 ImageNette 上，训练时间减少 48.6%，显存减少 50.7%，所有采样预算下的 FID 均有提升。

Conclusion: CoFRe 提供了一个实用的训练到推理框架，通过固定点求解器实现自适应深度，在减少参数和训练成本的同时，显著提升低预算掩码生成的质量，且支持从预训练 MGM 高效转换。

Classification reason: 该论文提出固定点掩码生成模型（FP-MGM），通过固定点求解器替代部分去噪器实现自适应深度，减少参数量和训练成本，同时提升低预算采样质量。这属于生成模型效率优化（加速、少步采样、蒸馏等），与 canonical subtopic "生成模型加速与高效化" 高度匹配。

Importance reason: Proposes a novel fixed-point solver for masked generative models that significantly improves efficiency and low-budget quality, aligning with user's focus on model efficiency and generation acceleration. Signals: Fixed-point solver for adaptive depth, Cross-step consistency loss, Three-state reuse (3SR) for warm-starting, Reduces parameters, training time, and VRAM, Improves low-budget masked generation quality
...
### [56] [A Unifying View of Variational Generative Wasserstein Flows](https://arxiv.org/abs/2605.31369)
...
TL;DR: 本文提出了生成式Wasserstein流（GWF）的统一理论框架，证明多种现有生成模型（扩散模型、GAN等）可以视为f-散度目标下参数化JKO格式的实例，并将该框架扩展到IPM和MMD，推导出新的JKO生成算法。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型虽然都可以看作最小化分布间散度，但依赖不同的算法和几何原理，缺乏统一的视角。作者希望建立一个基于Wasserstein梯度流的统一理论框架，揭示不同生成模型之间的内在联系。

Method: 将生成建模形式化为Wasserstein梯度流，通过Jordan-Kinderlehrer-Otto (JKO)格式的隐式离散化进行近似。证明多种现有方法（如扩散模型、GAN）是f-散度目标下参数化JKO格式的特例，并建立算法间的等价关系。进一步将框架扩展到积分概率度量（IPM）和平方最大均值差异（MMD），推导新的JKO生成算法。

Result: 建立了生成式Wasserstein流（GWF）的统一理论框架，揭示了多种生成模型之间的等价关系，并提出了基于JKO的新生成算法。实验研究了JKO正则化对不同目标的影响，并分析了参数化Wasserstein流中参数化映射诱导的分布动力学。

Conclusion: GWF框架为理解和设计生成模型提供了统一的理论视角，有助于推动生成模型的理论基础发展和新算法设计。

Classification reason: 该论文提出生成式Wasserstein流（GWF）的统一理论框架，将多种生成模型（扩散模型、GAN等）统一为JKO格式的变分Wasserstein梯度流，核心关注生成模型的训练与优化范式。这与多模态生成方向下的"生成模型加速与高效化"（efficient_generation）子主题相关，因为JKO格式与隐式离散化方法直接关联到生成模型的采样效率和优化加速。同时该框架也涉及f-散度、IPM、MMD等多种目标，属于生成模型基础理论，但最接近的现有子主题是efficient_generation。

Importance reason: Provides a unifying theoretical framework for generative models, but lacks direct application to the user's priority subtopics like continuous language models or video generation. Signals: Unified theory, Wasserstein gradient flows, JKO scheme, f-divergence, IPM
...
### [57] [Latent Geometric Chords for Query-Efficient Decision-Based Adversarial Attacks](https://arxiv.org/abs/2605.31219)
*Ei Hmue Khine, Yao Li, Jiebao Sun, Shengzhu Shi, Zhichang Guo, Boying Wu*

Main category: cs.CV, cs.CR, cs.LG
...
TL;DR: 本文提出 Latent Geometric Chords (LGC)，一种基于决策的黑盒对抗攻击方法。LGC 在压缩语义流形中执行曲率感知的几何搜索，并引入 Residual-based Adversarial Generation (RAG) 机制将语义扰动作为几何和弦叠加到原始图像上，从而绕过低维流形搜索空间限制和重建缺陷，实现高视觉保真度（SSIM>0.99, LPIPS<0.01）和强攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有决策型黑盒对抗攻击方法存在根本性局限：像素级攻击产生不自然的高频伪影，而潜空间方法受限于低维流形的搜索空间和固有的重建缺陷。本文旨在解决这些限制，实现高视觉保真度且查询高效的对抗攻击。

Method: 提出 Latent Geometric Chords (LGC) 方法，核心是在压缩语义流形中执行曲率感知的几何搜索来导航决策边界。引入 Residual-based Adversarial Generation (RAG) 机制，将语义扰动隔离为几何和弦并直接叠加到原始源图像上，从而解决基线重建缺陷并有效加倍允许的搜索空间维度。

Result: LGC 在 5000 次查询下达到 SSIM>0.99、LPIPS<0.01 的视觉保真度，在严格感知约束下保持高攻击成功率，成功攻破对抗训练的鲁棒模型，并展现出跨数据集的可迁移性，显著优于现有基线方法。

Conclusion: LGC 通过几何和弦与残差对抗生成机制，有效解决了决策型黑盒攻击中视觉保真度与搜索空间维度的矛盾，为对抗攻击提供了新的高效范式。

Classification reason: 该论文研究决策型黑盒对抗攻击，属于生成模型的安全与可控性范畴。虽然攻击而非对齐，但"生成模型对齐与奖励"（generation_alignment）是现有[CANONICAL]中最接近的主题，涵盖生成模型的安全、可控性与对抗鲁棒性。

Importance reason: Focuses on adversarial attack methodology, not on generative model alignment or the user's priority subtopics. Signals: adversarial attack, decision-based black-box, geometric search, visual fidelity, not generative model alignment
...
### [58] [Parallel Tempering Initial Sampling in Inference-Time Reward Alignment](https://arxiv.org/abs/2605.30991)
...
TL;DR: 本文提出 PATHS，一种基于并行回火（Parallel Tempering）的初始化采样方法，用于改进推理时奖励对齐中 SMC 方法的粒子初始化，通过多链耦合和 Metropolis 交换机制有效探索复杂奖励景观中的高奖励区域，在布局到图像和数量感知生成任务上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有推理时奖励对齐方法（如 SMC）从标准先验初始化粒子，但高奖励区域在复杂奖励景观中极为稀疏且多模态，导致采样陷入局部模式。即使最近的奖励感知初始化方法也难以克服多模态陷阱问题。

Method: 提出 PATHS，构建一个奖励温度阶梯（reward-tempered ladder），维护多条不同温度下的采样链，通过周期性的 Metropolis 交换操作在链之间交换粒子，使高温链能探索平坦化的奖励景观，低温链能聚焦高奖励区域，从而缓解模式捕获问题。

Result: 在布局到图像生成和数量感知生成任务上，PATHS 在奖励对齐质量上取得一致提升，尤其在复杂提示下表现更优，有效改善了 SMC 方法的初始采样质量。

Conclusion: PATHS 通过并行回火机制显著增强了有限预算下对稀有高奖励区域的探索能力，为推理时奖励对齐提供了一种有效的初始化采样策略。

Classification reason: 该论文研究推理时奖励对齐（inference-time reward alignment），通过并行回火（parallel tempering）改进扩散/流模型的奖励采样，属于生成模型对齐与奖励优化的范畴，因此映射到 [CANONICAL] subtopic "生成模型对齐与奖励" (generation_alignment)。

Importance reason: Focuses on inference-time reward alignment for diffusion models, which is tangentially related to user's core interests in continuous models and generation. Signals: Parallel tempering for SMC initialization, improves reward alignment in layout-to-image generation, addresses mode-trapping in complex reward landscapes, not directly about continuous language/multimodal models or autoregressive generation
...
### [59] [Functional MRI Time Series Generation via Wavelet-Based Image Transform and Spectral Flow Matching for Brain Disorder Identification](https://arxiv.org/abs/2605.30387)
*Hwa Hui Tew, Junn Yong Loo, Fang Yu Leong, Julia K. Lau, Ding Fan, Hernando Ombao, Raphaël C.-W. Phan, Chee Pin Tan, Chee-Ming Ting*

Main category: cs.AI, eess.SP, cs.CV, cs.LG
...
TL;DR: 本文提出 Dual-Spectral Flow Matching (DSFM)，一种基于小波变换和离散余弦变换的双谱流匹配框架，用于生成功能磁共振成像（fMRI）时间序列。该方法通过 DWT 将 BOLD 信号转换为小波分解图捕获多尺度变化，再通过 DCT 投影到余弦频率空间，训练类条件谱流匹配模型生成样本，最后通过逆变换重建生理上合理的 BOLD 信号，有效提升了下游脑网络分类性能。


<details>
  <summary>Details</summary>
Motivation: fMRI 数据采集成本高、样本稀缺，现有生成模型难以复现 BOLD 信号的非平稳性、复杂时空动态和生理变化，亟需一种能生成高保真 fMRI 时间序列的生成方法。

Method: 提出 DSFM 框架：1) 通过离散小波变换（DWT）将 BOLD 信号转换为小波分解图，捕获全局瞬态和多尺度变化；2) 通过离散余弦变换（DCT）投影到余弦频率空间，利用低频主导 BOLD 系数的局部能量压缩；3) 训练类条件谱流匹配（spectral flow matching）模型生成余弦频率表示；4) 通过逆 DCT 和逆 DWT 重建时域 BOLD 信号。双变换方法施加了结构化频率先验，保留了关键生理脑动力学。

Result: 在 fMRI 下游脑网络分类任务上，DSFM 生成的合成数据显著提升了分类性能，验证了生成样本的生理合理性和有效性。

Conclusion: DSFM 通过双谱流匹配有效生成高保真 fMRI 时间序列，为脑疾病识别中的数据稀缺问题提供了可行的生成式解决方案。

Classification reason: 该论文提出 fMRI 时间序列生成方法（DSFM），属于多模态生成方向。虽然 fMRI 生成较为特殊，但核心是生成模型（flow matching）用于数据合成，与"生成模型评测与数据集"（generation_benchmark）中数据增强/合成相关主题匹配。没有更精确的 canonical 子主题，但该子主题可涵盖生成式数据增强与合成任务。

Importance reason: The paper focuses on fMRI time series generation for brain disorder identification, which is outside the user's priority subtopics and contains negative keywords like medical diagnosis. Signals: fMRI generation, brain disorder, spectral flow matching, wavelet transform, medical application
...
### [60] [Crafter: A Multi-Agent Harness for Editable Scientific Figure Generation from Diverse Inputs](https://arxiv.org/abs/2605.30611)
...
TL;DR: 本文提出 Crafter，一个多智能体框架，用于从多种输入条件（文本、草图、图表等）生成可编辑的科学图表，并配套 CraftBench 评测基准和 CraftEditor SVG 转换工具。


<details>
  <summary>Details</summary>
Motivation: 科学图表是论文中传达复杂研究思想的重要手段，但制作出版级质量的图表仍然是论文准备中最耗时的环节之一。现有自动化系统仅针对单一图表类型且仅支持文本输入，无法覆盖研究人员实际使用的多样化图表类型和输入条件，且输出为光栅图像无法局部编辑修改。

Method: 提出 Crafter 多智能体框架，包含多个协作的 LLM 智能体（规划、编码、渲染、审查等），可泛化到多种图表类型和输入条件而无需架构变更；CraftEditor 将光栅输出转换为可编辑 SVG 格式；CraftBench 覆盖三种图表类型和四种输入条件，并带有人工质量标注。

Result: 实验表明 Crafter 在 PaperBanana-Bench 和 CraftBench 上显著优于独立生成器和基线智能体系统；消融实验验证了各组件的独立贡献；CraftEditor 在 SVG 转换质量上超越所有基线。

Conclusion: 多智能体编排框架可以有效解决科学图表生成中多样化的类型和输入条件问题，且可编辑 SVG 输出为局部修正提供了可行方案。

Classification reason: 该论文提出了科学图表生成的多智能体框架 Crafter 和可编辑 SVG 转换系统 CraftEditor，并构建了 CraftBench 评测基准。核心贡献在于生成模型评测（CraftBench）和图像/图表生成（科学图表属于图像生成的一种应用），因此匹配到 multimodal_generation 方向下的 [CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark）作为主要分类，同时也可归入 "图像与视频编辑"（visual_editing）因涉及 SVG 可编辑输出。

Importance reason: Focuses on scientific figure generation, which is not a priority subtopic and lacks alignment with user's key directions like continuous models or video generation. Signals: Multi-agent framework for figure generation, SVG conversion tool, CraftBench benchmark, Not aligned with priority subtopics, No autoregressive or diffusion video/image generation
...
### [61] [GGT-100K: Generative Ground Truth for Generalizable Real-World Image Restoration](https://arxiv.org/abs/2605.31039)
...
TL;DR: 本文提出GGT（生成式真实标注）方法，利用多模态基础模型（如Nano-Banana-2）从真实低质量图像合成高质量目标图像，构建了包含103,707对训练数据的GGT-100K数据集，显著提升了多种图像恢复模型在真实场景中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像恢复面临高质量配对训练数据稀缺的瓶颈。合成数据集难以模拟真实退化，而真实配对数据采集成本高昂，导致现有模型在真实场景中泛化能力有限。

Method: 系统评估了9种多模态基础模型（MFM）在不同场景和退化类型上的表现，发现Nano-Banana-2配合VLM自适应提示能生成感知真实且内容忠实的高质量目标。基于此构建了GGT合成流水线，包含多阶段质量控制（质量评分、内容一致性检查、人工验证），最终构建GGT-100K数据集。

Result: GGT-100K数据集包含103,707对训练图像和500对测试图像，覆盖多种场景和复杂真实退化。实验表明该数据集能一致提升多种图像恢复模型（包括生成式模型）的真实世界泛化能力。

Conclusion: 多模态基础模型可作为面向图像恢复的数据生成实用工具，GGT-100K是扩展真实世界图像恢复模型泛化边界的有用资源。

Classification reason: 该论文构建了GGT-100K数据集（103,707对LQ-HQ图像对），用于真实世界图像恢复训练与评测，属于生成模型评测与数据集（generation_benchmark）范畴。同时论文涉及多模态基础模型（MFM）用于生成高质量目标图像，也与多模态理解与生成方向相关。

Importance reason: Dataset paper for image restoration, not directly aligned with user's priority subtopics like continuous language models or video generation. Signals: generative ground truth, real-world image restoration, multimodal foundation models, dataset construction, Nano-Banana-2
...
TL;DR: 本文提出一种基于共形预测的可靠性评分（reliability score）作为条件生成模型的新评测指标，并设计了 CReL 框架来高效计算该指标。该指标衡量预测集在指定置信水平下的最差性能，弥补了现有单输出评测指标无法捕捉生成不确定性和风险的不足。在合成数据和图像到文本、文本到图像任务上的实验验证了该指标的可解释性和框架的有效性。
...
Method: 基于共形预测（conformal prediction）提出可靠性评分（reliability score），在预设置信水平下衡量预测集的最差性能。提出 CReL 框架，包含两个步骤：(i) 构建具有期望覆盖率的预测集；(ii) 在预测集内优化可靠性评分。针对高维输出空间和非凸性挑战设计了高效计算方法，并提供了覆盖率的理论保证。

Result: 在合成数据和图像到文本、文本到图像任务上的实验表明，CReL 能生成比现有方法更具信息量的预测集，可靠性评分作为新指标具有良好的可解释性，框架在有效性和计算效率上均表现优异。

Conclusion: 本文提出的基于共形预测的可靠性评分和 CReL 框架为条件生成模型提供了一种新的、考虑不确定性的评测方式，弥补了现有评测指标的不足，具有理论保证和良好的实验效果。
...
### [63] [Benchmarking and Enhancing Text-to-Image Models for Generating Visual Representations in Early Arithmetic Education](https://arxiv.org/abs/2605.31212)
...
TL;DR: 本文提出了方程到视觉生成（equation-to-visual generation）任务，构建了E2V-Bench基准，系统评测了现有文生图模型在算术教育中生成教学视觉内容的能力，发现模型在物体计数和关系结构方面存在严重错误，并探索了基准引导的增强策略。


<details>
  <summary>Details</summary>
Motivation: AI系统越来越多地被用于教育内容创作，但现有文生图模型能否生成忠实反映教学概念的视觉内容尚不清楚。作者希望系统性地评估和提升模型在算术教育场景中生成教学视觉表征的能力。

Method: 通过教师访谈和教育材料分析，构建了E2V-Bench基准，涵盖四种教学视觉类型（集合、数轴、十格框、算式图），并设计了自动评测指标（如物体计数准确率、关系结构正确性）。基于该基准评测了多个主流文生图模型，并探索了基准引导的增强策略（如提示优化、微调等）。

Result: 评测发现现有文生图模型在该任务上频繁失败，主要错误类型为物体计数错误和关系结构破坏。基准引导的增强策略在一定程度上提升了模型表现，但仍有较大差距，表明未来模型需要更强的数值和关系理解能力。

Conclusion: 当前文生图模型在算术教育场景中生成教学视觉内容的能力不足，需要更强的数值和关系理解能力。E2V-Bench为评估和推动这一方向提供了有价值的基准。

Classification reason: 该论文提出了方程到视觉生成任务（equation-to-visual generation）并构建了E2V-Bench基准，用于评测文生图模型在算术教育场景中生成教学视觉内容的能力。这属于生成模型评测与数据集（generation_benchmark）范畴，同时涉及多模态理解与生成方向中的视觉语言模型应用。

Importance reason: Focuses on educational text-to-image generation, which is not a priority subtopic and lacks direct connection to user's core interests in continuous multimodal models or video generation. Signals: Educational application, equation-to-visual generation, benchmark for arithmetic visuals, limited to text-to-image, no continuous or autoregressive methods
...
### [64] [AdvScene: Rethinking Adversarial Patch Evaluation Through Scene Robustness](https://arxiv.org/abs/2605.30578)
*Xiaoyong, Yuan, Zhang*
...
TL;DR: 本文提出 AdvScene，一个基于场景重建的对抗补丁鲁棒性评测框架。通过 Adversarial Patch-to-Scene Embedding (APSE) 方法将补丁嵌入到重建的 3D 场景中，在不同视角、距离和场景条件下评估补丁的攻击有效性（即"场景鲁棒性"），揭示了现有图像级和模拟器级评测无法捕捉的显著场景依赖性变化。


<details>
  <summary>Details</summary>
Motivation: 现有对抗补丁评测方法存在两难：真实图像基准虽真实但固定单一，无法覆盖部署后的多变条件；模拟器虽可控但缺乏真实场景的锚定。因此需要一种既能保持真实场景锚定、又能系统控制视角/距离等条件变化的评测框架，以衡量对抗补丁在实际部署中的"场景鲁棒性"。

Method: 提出 AdvScene 框架，核心是 Adversarial Patch-to-Scene Embedding (APSE) 方法。APSE 将对抗补丁从单一锚定视图嵌入到重建的 3D 场景中，形式化为一个带约束的 lifting 问题，满足四个关键性质：保留攻击关键外观、局部性（补丁仅影响目标表面）、目标表面附着、跨视角一致性。通过优化求解，将补丁嵌入到场景的 NeRF 或 3DGS 表示中，从而可以在任意视角和距离下渲染评估。

Result: 使用真实物理数据验证了 AdvScene 的有效性，并对多种现有对抗补丁进行了全面评测。结果表明，对抗补丁的攻击效果在不同视角、距离和场景条件下存在显著的场景依赖性变化，这些变化是现有图像中心或模拟器评测方法无法捕捉的。

Conclusion: AdvScene 为对抗补丁提供了一种更贴近实际部署场景的评测方法，揭示了场景鲁棒性作为对抗补丁实际风险的关键维度。该框架有助于更准确地评估对抗补丁的真实威胁，并为设计更鲁棒的对抗攻击与防御提供指导。

Classification reason: 该论文提出 AdvScene，一个场景级对抗补丁鲁棒性评测框架，属于生成模型/对抗攻击的评测方法。虽然对抗补丁本身属于对抗攻击领域，但论文核心贡献是提出新的评测框架（scene-grounded evaluation），与 canonical subtopic "生成模型评测与数据集"（generation_benchmark）高度匹配，因为该 subtopic 涵盖评测基准、新评测协议和自动评估方法。

Importance reason: Focuses on adversarial patch evaluation in 3D scenes, which is peripheral to the user's core interests in continuous multimodal models and generative video/image. Signals: adversarial patch robustness, scene-grounded evaluation, 3D reconstruction, cross-view consistency, not aligned with priority subtopics
...
### [65] [Towards Streaming Synchronized Spatial Audio Generation via Autoregressive Diffusion Transformer](https://arxiv.org/abs/2605.30940)
...
TL;DR: SwanSphere 提出了一种基于因果自回归扩散 Transformer 的流式框架，用于从全景视频和文本提示实时生成高质量空间音频，并设计了空间视频-音频对比学习（SVAC）和多目标在线直接偏好优化（ODPO）策略，在视频到空间音频和文本到空间音频任务上均取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有空间音频合成技术面临生成质量与推理延迟之间的权衡，且难以从多模态输入中准确捕捉空间信息，缺乏实时流式生成能力。

Method: 1) 提出因果自回归扩散 Transformer 架构，支持流式高质量空间音频生成；2) 设计空间视频-音频对比学习（SVAC）策略对齐视频编码器与声学域；3) 采用多目标在线直接偏好优化（ODPO）增强空间感知能力；4) 开发自动化标注流水线生成详细空间描述以缓解数据稀缺问题。

Result: 实验表明，SwanSphere 在视频到空间音频和文本到空间音频生成任务上均达到最优性能，实现了高质量、低延迟的流式空间音频合成。

Conclusion: SwanSphere 通过因果自回归扩散 Transformer 和对比学习-偏好优化联合训练，有效解决了空间音频生成中质量与延迟的权衡问题，为沉浸式空间音频的实时流式生成提供了统一框架。

Classification reason: 该论文研究从全景视频和文本提示生成空间音频，属于音频与多模态联合生成任务，与 [CANONICAL] subtopic "音频与多模态联合生成" 高度匹配。

Importance reason: Novel streaming autoregressive diffusion transformer for spatial audio aligns with user's priority on autoregressive generation and efficient inference. Signals: autoregressive diffusion transformer, streaming generation, spatial audio, multimodal alignment, efficient inference
...
### [66] [Mental Damage: Caption Poisoning Attacks on Retrieval-Augmented Text-to-Music Generation](https://arxiv.org/abs/2605.30365)
...
TL;DR: 本文提出了一种针对检索增强文生音乐（TTM）系统的字幕投毒攻击方法。攻击者通过向音乐知识库中注入少量精心构造的音乐字幕，使系统检索到恶意字幕，从而偏置提示增强过程，使生成的音乐偏离用户意图。该方法采用双层字幕投毒策略，在保留高层检索锚点的同时注入低层声学描述符，实现对生成结果的定向操控。


<details>
  <summary>Details</summary>
Motivation: 检索增强的文生音乐系统依赖于音乐知识库中字幕的完整性，这种设计引入了安全依赖。作者发现攻击者可以通过投毒数据库来操控生成结果，而无需修改用户提示、检索器或生成器本身，揭示了这类系统存在实际的安全风险。

Method: 提出双层字幕投毒策略（dual-layer caption poisoning strategy）：第一层保留高层检索锚点（如音乐风格、情绪等宏观描述），确保恶意字幕能被检索器命中；第二层注入低层声学描述符（如节奏、音色、乐器等细粒度特征），以引导下游提示增强和音乐生成朝向攻击者选择的目标意图。

Result: 在MusicCaps知识库、CLAP检索器和MusicGen生成管线上，投毒后的生成结果显著更接近攻击者目标，同时与原始用户查询的对齐程度保持可比。实验证明了该攻击方法的有效性。

Conclusion: 检索增强的创意AI系统存在实际完整性风险，攻击者可以通过字幕投毒操控生成结果。该工作揭示了这类系统在安全设计上的薄弱环节，为未来构建更安全的检索增强生成系统提供了警示。

Classification reason: 论文研究检索增强的文生音乐（TTM）系统的投毒攻击，属于音频生成（text-to-music）的安全问题。最佳匹配方向为多模态生成（multimodal_generation），子主题复用[CANONICAL]的"音频与多模态联合生成"（audio_av_generation），因为该子主题涵盖音乐/音频生成。同时，攻击涉及检索增强系统的多模态安全，因此也匹配多模态理解与生成方向下的"多模态安全与攻防"（multimodal_safety_attack）。

Importance reason: Security attack on retrieval-augmented generation is relevant but not aligned with user's priority subtopics. Signals: retrieval-augmented generation, text-to-music, caption poisoning, security attack, creative AI integrity
...
### [67] [ImmersiveTTS: Environment-Aware Text-to-Speech with Multimodal Diffusion Transformer and Domain-Specific Representation Alignment](https://arxiv.org/abs/2605.30965)
...
TL;DR: ImmersiveTTS 提出了一种环境感知的文本到语音模型，利用多模态扩散Transformer和领域特定表征对齐，在环境上下文中生成自然融入的语音，在自然度、可懂度和音频保真度上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导音频生成在音效、语音、音乐等领域取得了进展，但联合生成语音与环境音频仍具挑战，因为两者在声学模式和时序动态上存在固有差异。现有方法难以生成与环境上下文自然融合的语音。

Method: 基于多模态扩散Transformer，将文本对齐的语音潜变量与文本条件的环境上下文通过联合注意力机制融合；引入领域特定表征对齐目标，利用语音和音频编码器的互补自监督表征增强语义一致性。

Result: 在客观指标和人类听力测试中，ImmersiveTTS 在自然度、可懂度和音频保真度上均优于现有方法。

Conclusion: 通过显式建模跨模态交互和领域特定表征对齐，环境感知TTS能够生成与环境无缝融合的自然语音，为沉浸式语音生成提供了有效方案。

Classification reason: 该论文提出环境感知的文本到语音（TTS）模型，结合扩散Transformer实现语音与环境音频的联合生成，属于音频生成与多模态联合生成范畴，与CANONICAL subtopic "音频与多模态联合生成"（audio_av_generation）高度匹配。

Importance reason: While technically solid, the paper's focus on environment-aware TTS is peripheral to the user's core priorities in continuous multimodal models and video generation. Signals: Multimodal diffusion transformer, domain-specific representation alignment, environment-aware TTS, cross-modal joint attention, self-supervised representations
...
### [68] [Chatterbox-Flash: Prior-Calibrated Block Diffusion for Streaming Zero-Shot TTS](https://arxiv.org/abs/2605.30748)
...
TL;DR: Chatterbox-Flash 通过将预训练的自回归 TTS 解码器微调为块扩散解码器，实现零样本 TTS 的流式并行生成。针对离散语音 token 的长尾分布导致并行位置选择偏向高频 token 的问题，提出先验校准评分和早期解码调度两种推理时技术，在不修改架构的情况下提升生成质量。在标准基准上达到与强自回归和非自回归基线相当的高保真合成效果，同时支持流式推理。


<details>
  <summary>Details</summary>
Motivation: 自回归 TTS 模型推理速度慢，非自回归模型难以实现流式生成。现有块扩散解码方法在离散语音 token 上直接迁移效果差，因为长尾 token 分布导致并行位置选择偏向高频 token，需要在不改变架构的前提下解决这一问题。

Method: 将预训练的自回归 TTS 解码器微调为块扩散解码器（block-diffusion decoder），实现块内并行 token 生成、块间流式传输。提出两种推理时技术：(1) 先验校准评分（prior-calibrated scoring），减去块级边际 token 分布以消除高频 token 偏差；(2) 早期解码调度（early-decoding schedule），基于校准置信度自适应终止迭代。

Result: 在标准零样本 TTS 基准上，Chatterbox-Flash 达到与强自回归和非自回归基线相当的高保真合成质量；流式推理的首包延迟与流式自回归系统相当，实时因子显著更低。

Conclusion: 通过先验校准和早期解码调度，块扩散解码可有效应用于离散语音 token 的零样本 TTS 生成，在保持高质量的同时实现高效流式推理，为 TTS 模型的速度-质量权衡提供了新方案。

Classification reason: 该论文提出零样本文本到语音（TTS）模型，属于音频生成方向，与 [CANONICAL] 子主题 "音频与多模态联合生成"（audio_av_generation）完全匹配。

Importance reason: Focuses on TTS streaming efficiency, not aligned with user's priority on continuous multimodal models or video generation. Signals: Block diffusion for TTS, Prior-calibrated scoring, Streaming zero-shot TTS, Inference-time techniques, Not in priority subtopics
...
### [69] [AnchorSteer: Self-Discovered Concept Injection for Structure-Preserving Music Editing](https://arxiv.org/abs/2605.31053)
...
TL;DR: AnchorSteer 提出一种自发现概念注入框架，通过结构锚定与自监督语义解耦，在严格保持节奏和旋律结构的同时实现可控音乐编辑。


<details>
  <summary>Details</summary>
Motivation: 可控音乐编辑面临语义-结构纠缠问题：现有方法要么为了编辑效果破坏结构，要么为了保持结构抑制语义响应。需要一种能同时实现语义变换与结构保持的方法。

Method: AnchorSteer 包含两个核心组件：(1) 自发现概念向量提取——通过自监督重建目标从扩散模型内部表征中提取可解释、无标签的概念向量，实现属性解耦；(2) 结构锚定注入——将概念向量注入扩散隐空间，同时使用结构适配器保持节奏和旋律结构的一致性。提供无条件注入和条件注入两种变体以平衡鲁棒性与语义强度。

Result: 在 ZoME-Bench 基准和主观测试上，AnchorSteer 在语义变换效果和结构保持保真度上均优于纯编辑方法和纯结构保持方法。

Conclusion: AnchorSteer 通过结构锚定与自发现语义注入有效解耦了音乐编辑中的语义-结构纠缠，实现了高保真结构保持下的显著语义变换，为可控音乐编辑提供了新范式。

Classification reason: 该论文研究可控音乐编辑（音频生成/编辑），属于多模态生成方向下的音频生成范畴，与 [CANONICAL] subtopic "音频与多模态联合生成" 高度匹配。

Importance reason: Focuses on music editing, not aligned with user's priority subtopics in continuous multimodal models, video/image generation, or world models. Signals: music editing, structure preservation, diffusion, concept injection, not in priority subtopics
...
### [70] [FBHM: Functional Benchmarking and Steering of VLMs for Hateful Meme Detection](https://arxiv.org/abs/2605.31349)
*Paramananda Bhaskar, Naquee Rizwan, Daksh Jogchand, Saurabh Kumar Pandey, Animesh Mukherjee*

Main category: cs.AI, cs.CL, cs.CV, cs.MM
...
TL;DR: FBHM提出了一个基于功能分类的仇恨模因基准（5000张模因，25种修辞功能×10个目标群体），揭示VLM在标准数据集上表现良好但在FBHM上近乎随机，证明模型依赖数据集特定启发式而非鲁棒多模态推理。提出LSV（可学习转向向量），仅需500个样本即可提升~30 Macro-F1，优于上下文学习和PEFT。


<details>
  <summary>Details</summary>
Motivation: 现有仇恨模因检测基准存在结构性观察偏差，将修辞仇恨机制与目标社区特征混淆，无法对模型漏洞进行因果评估。需要一种系统性的、功能驱动的基准来揭示VLM的真实推理能力缺陷。

Method: 1) 构建FBHM基准：沿修辞功能（25种）和目标社区（10个）两个正交轴系统构造5000张模因。2) 提出LSV（Learnable Steering Vectors）：在少量样本（500个，50张独特基础模因）上学习因果干预向量，通过激活引导修正模型行为。

Result: SOTA VLM在FBHM上性能骤降至接近随机水平。LSV仅用500个样本即可提升约30个Macro-F1点，优于上下文学习和PEFT方法，且不降低源域性能。

Conclusion: 当前VLM在仇恨模因检测中依赖数据集特定启发式而非真正的多模态推理。FBHM为因果评估提供了有效工具，LSV为低数据场景下的模型安全干预提供了高效方案。

Classification reason: 该论文研究VLM在仇恨模因检测中的安全漏洞与鲁棒性，属于多模态安全与攻防范畴。FBHM基准测试系统性地评估VLM面对恶意内容时的脆弱性，LSV方法则是一种防御/干预策略，与[多模态安全与攻防]子主题高度匹配。

Importance reason: Focuses on hateful meme detection and VLM safety, not on the user's priority subtopics like continuous language models or video generation. Signals: VLM safety, hateful meme detection, functional benchmarking, learnable steering vectors, low-data intervention
...
### [71] [Probing Collision Grounding in Vision-Language Models for Safe Human-Robot Collaboration](https://arxiv.org/abs/2605.31196)
*Jun Wang, Xiaohao Xu, Xiaonan Huang*

Main category: cs.AI, cs.CL, cs.CV, cs.RO
...
TL;DR: 本文提出 TouchSafeBench，一个基于 Habitat 3.0 的物理碰撞检测基准，用于评估 VLM 在安全人机协作中的碰撞接地能力。实验表明当前 VLM 在碰撞分类和预警任务上表现不佳（最佳 Macro-F1 < 50%），揭示了视觉流畅性不等于物理可问责性的核心局限。


<details>
  <summary>Details</summary>
Motivation: 安全人机协作需要监控系统不仅具备视觉描述能力，还需判断机器人是否与场景或人发生碰撞或即将碰撞。现有 VLM 缺乏对机器人本体几何、视角、场景布局、人体接近度和时序运动进行联合推理的碰撞接地能力，且缺乏相应的评测基准。

Method: 在 Habitat 3.0 中构建 TouchSafeBench 基准，包含 2,940 个模拟室内共存场景（社交导航和社交重排），提供多视角 RGB-D 观测、轨迹地图、相机元数据和模拟器接触标签。定义两个任务：当前安全状态分类和即将碰撞预警。评估多个前沿/机器人 VLM 及九种视觉表征。

Result: 最佳模型平均 Macro-F1 低于 50%；显式深度信息未能自动转化为碰撞证据；机器人-场景碰撞检测比人-机器人碰撞更难。当前 VLM 在碰撞接地任务上远未达到可靠水平。

Conclusion: 视觉流畅性不等于物理可问责性。可靠的机器人安全监控需要显式绑定视角、机器人形态、度量几何和未来碰撞的表征，而非仅依赖视觉语义理解。

Classification reason: 论文关注人机协作中的碰撞安全检测，属于多模态安全范畴（物理安全），与 canonical subtopic "多模态安全与攻防" 相关；同时涉及机器人场景中的世界建模（碰撞预测），也匹配 world_model 方向。

Importance reason: The paper focuses on VLM safety for robotics, which is not a priority subtopic and lacks direct alignment with the user's focus on continuous multimodal models, autoregressive generation, or world models. Signals: VLM collision grounding benchmark, low Macro-F1 < 50%, Habitat 3.0 simulation, safety for human-robot collaboration, visual fluency vs. physical accountability
...
### [72] [Immuno-VLM: Immunizing Large Vision-Language Models via Generative Semantic Antibodies for Open-World Trustworthiness](https://arxiv.org/abs/2605.30745)
...
TL;DR: Immuno-VLM 受生物免疫负选择机制启发，利用大语言模型的生成推理能力主动生成"语义抗体"（近分布异常文本描述），在潜空间中约束已知类别的决策边界，从而解决VLM在开放世界中因缺乏负知识而将未知异常高置信度误分类为已知类别的可信问题。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在零样本识别中因语义抽象而存在"语义傲慢"问题：在开放世界部署中，模型会将未知异常强制拟合到已知类别并给出高置信度预测，缺乏显式的负知识。传统开集识别方法依赖被动密度估计或像素级异常生成，效率低且效果有限。

Method: 提出 Immuno-VLM 框架，将生物免疫负选择原理适配到高维潜空间。利用LLM的生成推理能力主动"幻觉"出近分布异常样本的文本描述（称为"语义抗体"），包括外观相似物和上下文异常等，从而有效约束已知类别的决策空间。

Result: 在 ImageNet-1K 和四个具有挑战性的 OOD 基准数据集上，Immuno-VLM 取得了新的最优结果（SOTA）。

Conclusion: Immuno-VLM 通过生物启发的免疫负选择机制，结合LLM的生成能力，有效提升了VLM在开放世界中的可信度，为多模态安全与可信部署提供了新思路。

Classification reason: 该论文关注VLM在开放世界部署中的可信安全问题（将未知异常强制分类为已知类别），提出基于免疫负选择的防御框架，属于多模态安全与攻防范畴，因此映射到[CANONICAL] subtopic "多模态安全与攻防"。

Importance reason: Novel bio-inspired OOD method for VLM trustworthiness, but tangential to user's core focus on continuous multimodal models and generation. Signals: Open-world trustworthiness, Immunological negative selection, Semantic antibodies, Hubris of semantics, OOD detection
...
TL;DR: COMPASS 提出了一种基于认知MCTS的过程对齐框架，通过认知树探索合成隐蔽攻击轨迹，并结合内省式步骤级对齐实现搜索智能体的细粒度安全监督，在保持实用性的同时显著提升安全性。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的搜索智能体在多步推理和工具使用中会出现检索诱导的安全退化——有害意图可能分解为看似无害的子查询，最终导致不安全结果。现有对齐方法难以捕获稀疏的安全信号，也无法监督多步交互中的多样化违规行为。

Method: COMPASS 包含两个核心模块：(1) 认知树探索（CTE），利用MCTS高效合成隐蔽攻击轨迹，模拟智能体在多步搜索中的安全漏洞；(2) 内省式步骤级对齐（ISA），对中间动作进行细粒度安全监督，隔离风险行为以实现过程级对齐。

Result: 实验表明，COMPASS 在安全-效用权衡上表现优异，同时所需训练数据大幅减少。

Conclusion: COMPASS 通过认知MCTS引导的过程对齐，有效解决了搜索智能体的安全对齐难题，为多步交互场景下的安全监督提供了新范式。

Classification reason: 该论文研究LLM驱动的搜索智能体的安全对齐问题，涉及对抗攻击（stealthy attack trajectories）与安全防御（process alignment），属于多模态安全与攻防范畴。虽然论文主要聚焦文本搜索智能体，但其安全对齐框架与多模态安全中的越狱攻击、防御方法高度相关，因此复用[CANONICAL] subtopic "多模态安全与攻防"。

Importance reason: Focuses on safety alignment for search agents, which is outside your core priority subtopics of continuous multimodal models and video generation. Signals: LLM safety, process alignment, MCTS, search agents, stealthy attacks
...
### [74] [A Persona-Based Evaluation Framework for Pluralistic Alignment in Generative AI](https://arxiv.org/abs/2605.31021)
*Atahan Karagoz*
...
TL;DR: 本文提出一种基于人格（persona）的生成式AI对齐评估框架，用合成认知画像替代单一聚合基准，实现反映文化、人口和语境多样性的多元评估。实验发现这些模拟评估者在连续推理下会出现状态空间漂移和语义不一致，表明静态对齐约束不足，需要嵌入动态调节机制。


<details>
  <summary>Details</summary>
Motivation: 当前生成式AI的对齐评估主要依赖单一聚合基准，忽略了人类判断在文化、人口和语境上的多样性。作者希望引入一种能够反映多元视角的评估方法，更真实地模拟现实世界中共识的变异性。

Method: 提出状态空间约束仿真框架，用一组合成认知画像（persona）替代单一评估函数，利用生成式架构实例化并维持这些评估人格，实现多元化的、依赖视角的基准测试。通过序列推理和随机提示扰动分析评估者的一致性稳定性。

Result: 现代生成架构能够以较高一致性实例化和维持评估人格，但在序列推理和随机提示扰动下，人格一致性出现系统性退化，表现为状态空间漂移和语义不一致。

Conclusion: 静态对齐约束不足以维持评估行为的长期鲁棒性，需要在生成系统中嵌入动态、可存活性驱动的调节机制。将基于人格的评估视为潜表征流形上的结构化动力系统，为更自适应、更符合人类价值观的评估方法奠定基础。

Classification reason: 该论文提出基于人格（persona）的多视角对齐评估框架，关注生成式AI的对齐（alignment）与安全评测，与[多模态安全与攻防]（multimodal_safety_attack）中涉及的对齐评估、安全评测方向高度相关。虽然论文不限于多模态，但其核心是pluralistic alignment evaluation，属于对齐与安全评测范畴。

Importance reason: Focuses on pluralistic alignment evaluation, not on the user's priority subtopics like continuous language models or video generation. Signals: Persona-based evaluation, pluralistic alignment, state-space drift, synthetic cognitive profiles, generative AI alignment
...
### [75] [Does Visual Information Play a Decisive Role in Vision-Language-Action Model Driving Behavior?](https://arxiv.org/abs/2605.31041)
...
TL;DR: 本文提出一个结构化的多层级视觉扰动框架，用于系统分析VLA（视觉-语言-动作）自动驾驶模型对视觉信息的依赖程度。框架从通道级退化、信息级破坏和结构级修改三个维度设计受控视觉扰动，并在开环轨迹预测和闭环安全评估两种设置下测试模型行为响应。实验发现依赖模式随评估方式不同而变化，且模型在不同抽象层级上的视觉接地不均匀，揭示了当前VLA驾驶模型在视觉-行为依赖方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有VLA驾驶模型在自动驾驶中展现出潜力，但其驾驶行为如何基于视觉信息尚不清楚。当前的评估协议主要关注聚合性能指标，缺乏结构化的诊断方法来量化视觉-行为依赖关系，因此需要系统性的分析框架来理解视觉信息如何塑造驾驶行为。

Method: 提出一个多层级视觉扰动框架，沿三个互补维度组织受控视觉扰动：(1) 通道级退化（如颜色通道扰动）；(2) 信息级破坏（如语义遮挡）；(3) 结构级修改（如空间布局改变）。将该框架应用于VLA驾驶系统，在开环轨迹预测和交互式闭环安全评估两种设置下评估行为响应。

Result: 实验揭示了评估依赖的依赖模式：开环和闭环评估下模型对视觉扰动的敏感度不同；模型在不同抽象层级（低层视觉特征 vs 高层语义信息）上的视觉接地程度不均匀；某些视觉扰动在闭环评估中影响更大，表明交互环境放大了视觉依赖的重要性。

Conclusion: 当前VLA驾驶模型对视觉信息的依赖存在不均匀性和评估方式依赖性，需要更结构化的分析和更原则性的模型设计来理解视觉信息如何塑造行为，从而开发更安全、更鲁棒的自动驾驶系统。

Classification reason: 该论文研究VLA驾驶模型对视觉信息的依赖程度，通过多层级视觉扰动框架系统性地分析视觉-行为依赖关系，属于多模态系统的鲁棒性与安全性分析，与[多模态安全与攻防]（multimodal_safety_attack）中对抗攻击、安全评测方向高度相关。

Importance reason: Provides structured analysis of visual grounding in VLA driving models, but is not aligned with user's priority subtopics. Signals: VLA driving model, visual perturbation framework, open-loop vs closed-loop evaluation, uneven visual grounding
...
### [76] [Vision-Language Models Suppress Female Representations Under Ambiguous Input](https://arxiv.org/abs/2605.31556)
*Arnau Marin-Llobet, Simon Henniger, Mahzarin R. Banaji*

Main category: cs.CL, cs.AI, cs.CY, cs.HC, cs.CV
...
TL;DR: 本文发现，即使经过对齐训练，VLM在模糊性别输入（如背影、全副武装的工人）下仍会系统性输出男性默认，且内部表征与输出存在解耦——模型内部编码了女性关联，但输出时被抑制。作者提出LALS（潜关联倾斜分数）来逐token、逐层测量概念关联，揭示了从中间层到输出层的非对称性别过滤机制。


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法在性别清晰可见时能有效抑制VLM的性别偏见，但实际应用中大量输入是模糊的（如从背后拍摄的人物、全副武装的工人），这些情况下的偏见表现尚不清楚。本文旨在探究模糊输入下VLM的性别表征行为及其内部机制。

Method: 提出LALS（Latent Association Leaning Score），一种零样本度量方法，将视觉token激活投影到模型的文本嵌入空间，逐token、逐层测量概念关联。在15个职业、800+张性别模糊图像上对4个VLM（LLaVA、CLIP等）进行系统性分析，包括输出层分析、逐层分析以及颜色消融实验。

Result: 1）最小提示压力即可暴露职业-性别默认关联，模型在模糊输入下即使对强烈女性刻板印象的职业也坍缩为男性输出；2）内部表征与输出系统性解耦：模型内部编码了女性关联，但输出为男性；3）逐层分析揭示非对称过滤机制——男性信号从中间层到输出层持续放大，女性信号在中间层达到峰值后被抑制；4）颜色消融实验表明服装颜色等文化负载视觉线索会进一步调节内部关联。

Conclusion: VLM的对齐训练在模糊输入下存在盲区，模型内部编码与最终输出之间存在系统性偏差，女性表征在生成阶段被不对称地抑制。这揭示了当前对齐方法的局限性，并为构建更公平的多模态系统提供了新的诊断工具（LALS）和方向。

Classification reason: 该论文研究VLM在模糊输入下对女性表征的抑制，揭示了模型内部编码与输出之间的解耦以及性别偏见的系统性放大，属于多模态模型的安全性与公平性议题，映射到[多模态安全与攻防]（multimodal_safety_attack）规范子主题。

Importance reason: Focuses on gender bias in VLMs, which is outside the user's priority subtopics of continuous models, generation, and efficiency. Signals: VLM gender bias, ambiguous input, LALS metric, internal-external decoupling, asymmetric gender filter
...
### [77] [What Makes LVLMs Hallucinate Less? Unveiling the Architectural Factors Behind Hallucination Robustness](https://arxiv.org/abs/2605.30911)
...
TL;DR: 本文系统探究了LVLM架构设计（语言基础、视觉表征、语义对齐三个维度）对三种幻觉（共现、相似性、不确定性）的影响，提出了CoSimUE基准，发现模型参数规模对减少幻觉效果有限，而更强的视觉编码器、更大的语言基础以及有效的对齐策略分别对不同类型幻觉有显著缓解作用。


<details>
  <summary>Details</summary>
Motivation: LVLM的幻觉问题严重影响其可靠性，但现有研究多关注改进内部组件，缺乏从架构设计层面系统理解幻觉成因的工作。本文旨在揭示架构设计选择如何影响幻觉鲁棒性。

Method: 将架构设计分解为语言基础(LF)、视觉表征(VR)和语义对齐(SA)三个维度，将幻觉分为共现型、相似型和不确定型三类。提出CoSimUE基准，通过受控文本扰动和随机扰动创建细粒度幻觉场景，在7个设计方面进行实验映射设计选择与幻觉行为的关系。

Result: 1) 模型参数规模扩大对减少三类幻觉效果有限；2) 更大、训练更好的语言基础可减少共现幻觉；3) 更强的视觉编码器和更高分辨率缓解相似性错误；4) 有效的对齐策略减轻不确定性幻觉；5) 联合增强视觉保真度和对齐质量可获得最全面的改善。

Conclusion: 本文首次系统地将架构级设计与幻觉鲁棒性联系起来，为开发可靠高效的LVLM提供了实用指导，表明架构设计选择对幻觉缓解至关重要。

Classification reason: 该论文系统研究LVLM幻觉的架构因素，属于多模态幻觉缓解方向，与[CANONICAL] subtopic "多模态幻觉与可靠性" 完全匹配。

Importance reason: Systematic analysis of LVLM hallucination robustness is valuable but not aligned with user's priority on continuous multimodal models and generation. Signals: LVLM hallucination, architectural factors, CoSimUE benchmark, limited scaling effect, visual encoder importance
...
### [78] [BenHalluEval: A Multi-Task Hallucination Evaluation Framework for Large Language Models on Bengali](https://arxiv.org/abs/2605.31483)
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉语作为全球第六大语言，此前没有任何工作系统性地评估大语言模型在该语言上的幻觉问题。现有幻觉评测主要针对英语等资源丰富的语言，缺乏对低资源语言的覆盖。

Method: 构建覆盖四个任务（GQA、代码混合QA、摘要、推理）和12种幻觉类型的12,000个幻觉候选样本，采用双轨评测协议：Track A 在真实实例上测量假阳性率，Track B 在幻觉候选上测量幻觉检测率，并提出 BenHalluScore 双轨校准指标联合惩罚两种失败模式。

Result: 在7个LLM上评测，BenHalluScore 范围为7.72%至55.42%，揭示了模型和任务间幻觉校准的显著差异。思维链提示作为缓解策略仅改变了响应分布，未能持续改善幻觉判别能力。

Conclusion: BenHalluEval 建立了首个孟加拉语专用幻觉基准，揭示了单轨评测和仅提示策略在低资源语言场景中的不足，为低资源语言幻觉研究提供了重要基础。

Classification reason: 该论文构建了孟加拉语大语言模型的幻觉评测框架，属于多模态理解与生成方向下的"多模态幻觉与可靠性"（multimodal_hallucination）规范子主题，因为该子主题涵盖幻觉评测、事实性与可靠性评估。

Importance reason: Focuses on Bengali hallucination evaluation, which is outside the user's priority subtopics and lacks technical novelty in core areas like continuous models or video generation. Signals: Low-resource language benchmark, hallucination evaluation, no overlap with priority subtopics, no boost keywords matched, negative keywords not present but irrelevant
...
TL;DR: 本文提出IC-VCO方法，通过上下文视觉对比优化（将对比图像置于共享多图像上下文中）和视觉对比蒸馏（VCDist）正则化，以及对比样本编辑策略生成硬负样本，有效缓解多模态幻觉问题，在五个基准上取得最佳性能。
...
Method: 提出In-Context Visual Contrastive Optimization (IC-VCO)，将对比图像置于共享多图像上下文中确保目标函数数学严谨性；引入Visual Contrast Distillation (VCDist)作为可靠性门控正则化器，促进多图像对比训练与单图像推理间的一致性；设计对比样本编辑策略，通过精确语义扰动生成硬负样本。

Result: 在五个基准评测上，IC-VCO取得了最佳整体性能，样本编辑策略的有效性也得到了验证。

Conclusion: IC-VCO通过理论严谨的视觉对比优化和硬负样本生成策略，有效缓解了多模态幻觉，为视觉偏好优化提供了更可靠的范式。

Classification reason: 该论文聚焦于缓解多模态大模型的幻觉问题，属于多模态理解与生成方向下的"多模态幻觉与可靠性"（multimodal_hallucination）规范子主题，直接对应。

Importance reason: Addresses multimodal hallucination with a novel contrastive optimization, but falls outside top priority subtopics and lacks direct connection to continuous or generative models. Signals: Multimodal hallucination mitigation, In-context visual contrastive optimization, Visual preference DPO, Hard negative generation, VLM reliability improvement
...
TL;DR: YARD提出了一种无需训练的Y-Architecture Register Decoding框架，通过在中间解码层分支并替换patch级视觉token为register token来构建视觉退化分支，从而在不增加额外完整前向传播的情况下提供有效的对比解码信号，显著缓解LVLM的幻觉问题并降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有训练自由的对比解码方法存在两个问题：完全丢弃视觉token过于极端，会引入语言幻觉；而通过破坏输入图像来控制视觉证据的方式粒度粗糙且需要两次完整前向传播，推理延迟高。作者希望设计一种既保留全局图像语义又缺乏细粒度局部证据的退化分支，同时避免额外的计算开销。

Method: YARD基于一个关键观察：可靠的文本到视觉对齐主要出现在中间解码器层。因此，Y-Architecture共享浅层计算，在中间层进行分支。退化分支将patch级视觉token替换为register token（如[CLS] token），这些token保留全局图像语义但缺乏细粒度局部证据。这种图像感知但局部欠对齐的设计提供了忠实的对比信号，同时Y架构避免了第二次完整前向传播。

Result: 在生成式和判别式幻觉基准上的大量实验表明，YARD在多个LVLM上一致实现了最先进的幻觉缓解效果，同时显著降低了推理延迟。

Conclusion: YARD通过巧妙的Y-Architecture设计，在不增加额外前向传播的前提下，利用register token构建了有效的对比解码信号，为LVLM幻觉缓解提供了一种高效且无需训练的解决方案。

Classification reason: 该论文研究大视觉语言模型（LVLM）中的幻觉缓解问题，提出基于对比解码的Y-Architecture Register Decoding（YARD）方法，属于多模态幻觉缓解方向，与[多模态幻觉与可靠性]（multimodal_hallucination）这一CANONICAL子主题完全匹配。

Importance reason: Novel hallucination mitigation method but outside user's priority subtopics and lacks boost keywords. Signals: training-free contrastive decoding, Y-architecture for efficiency, register tokens for visual degradation, state-of-the-art on hallucination benchmarks, reduces inference latency
...
### [81] [Fighting Numerical Hallucinations via Data-centric Compilation for Online Financial QA](https://arxiv.org/abs/2605.31064)
...
TL;DR: 本文提出数据为中心的推理编译器（DCRC），通过对抗数据构建、多阶段训练和编译-执行推理三阶段框架，系统性地缓解金融QA中LLM的数值推理幻觉问题，并在真实在线金融问答系统中验证了有效性。
...
Method: 提出DCRC框架，包含三个阶段：(1) 对抗数据构建——合成含可控噪声的训练样本以增强鲁棒性；(2) 多阶段训练——训练数据为中心的结构化智能体（DSA），具备显式证据审计和程序合成能力；(3) 编译-执行推理——DSA将用户查询和检索文档转化为可验证、可执行的推理程序。

Result: 在离线基准测试和真实在线金融QA系统中均取得显著效果，有效降低了数值推理幻觉，提升了答案的可靠性和可审计性。

Conclusion: 数据中心的编译范式能够系统性地解决金融QA中的数值推理幻觉问题，通过显式证据审计和可执行程序推理，在保证可靠性的同时提升了可审计性。

Classification reason: 该论文聚焦于金融QA场景中的数值推理幻觉问题，属于多模态/语言模型幻觉缓解与可靠性提升范畴，与[多模态幻觉与可靠性]（multimodal_hallucination）这一CANONICAL子主题高度匹配。

Importance reason: Focuses on financial QA numerical hallucination, which is outside the user's priority subtopics and has negative keyword overlap. Signals: financial QA, numerical hallucination, data-centric compilation, RAG, online deployment
...
### [82] [The Flip Side of RLHF: On-Policy Feedback for Reward Model Self-Supervised Improvement](https://arxiv.org/abs/2605.30888)
...
TL;DR: 本文提出 SAVE 框架，利用价值函数对策略模型在线生成的响应进行评分，将奖励信号转化为自监督信号，通过对比学习更新奖励模型，从而缓解 RLHF 中偏好数据静态性与策略演化之间的不匹配问题。


<details>
  <summary>Details</summary>
Motivation: RLHF 中奖励模型的训练依赖静态偏好数据，而策略模型在训练过程中不断演化，导致 RM 难以准确评估新策略生成的响应。获取多样且可靠的偏好数据成本高昂，亟需一种能利用在线策略反馈来自我改进 RM 的方法。

Method: SAVE 框架利用价值函数（value function）对策略模型在线生成的响应进行评分，以提示特定的价值头作为自适应锚点，将奖励分级的在线响应转化为监督信号。通过计算 RM 优势值并过滤模糊样本，使用对比学习目标更新奖励模型。

Result: 在六个多样化基准上进行了严格实证评估，SAVE 在所有数据集上均取得优于基线的方法，并在三种 RL 算法（GRPO、RLOO、GSPO）和不同策略骨干网络上保持一致的性能提升。

Conclusion: SAVE 框架通过在线策略反馈实现奖励模型的自监督改进，有效解决了静态偏好数据与动态策略之间的不匹配问题，为 RLHF 中 RM 训练提供了一种高效且通用的增强方案。

Classification reason: 该论文研究 RLHF 中奖励模型的自我监督改进，属于多模态/语言模型对齐的核心问题，映射到 [CANONICAL] subtopic "多模态强化学习与对齐"（multimodal_rl_alignment）。

Importance reason: Focuses on RLHF reward model improvement, which is tangential to the user's core interests in continuous multimodal models and video generation. Signals: RLHF, reward model, on-policy feedback, self-supervised, alignment
...
### [83] [Attend to Evidence: Evidence-Anchored Spatial Attention Supervision for Multimodal RLVR](https://arxiv.org/abs/2605.30912)
...
TL;DR: EASE 是一种证据锚定的空间注意力监督方法，通过将标注的视觉证据区域转化为平滑的视觉 token 目标，在 RLVR 训练中引导响应到图像的注意力，从而提升 VLM 在感知、幻觉、视觉数学和多模态推理等任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的 RLVR 方法仅使用最终答案的结果奖励进行优化，无法区分基于视觉证据的回答与依赖语言先验或运气猜测的回答，尤其对于需要视觉定位的问题缺乏细粒度监督。

Method: 将标注的视觉证据区域转换为平滑的视觉 token 目标，在 RL 训练中仅对高奖励轨迹施加响应到图像注意力的引导，利用标注作为特权训练标签，推理时无需额外标注。

Result: 在 Qwen2.5-VL-7B、Qwen3-VL-4B 和 Qwen3-VL-8B 上，EASE 相比 DAPO 在感知、幻觉、视觉数学和多模态推理基准上平均提升 2.5~3.1 分。

Conclusion: EASE 通过引入视觉证据过程监督有效增强了 RLVR 训练，使 VLM 的视觉注意力更好地与标注证据区域对齐，提升了模型在需要视觉定位的任务上的表现。

Classification reason: 该论文提出 EASE 方法，在 RLVR（带可验证奖励的强化学习）框架中引入视觉证据过程监督，属于多模态强化学习与对齐方向，与 canonical subtopic "多模态强化学习与对齐" 高度匹配。

Importance reason: Novel evidence-anchored attention supervision for RLVR improves VLM grounding, but the subtopic is not in your priority list. Signals: RLVR with visual evidence supervision, improves perception, hallucination, math, reasoning, uses privileged training labels, tested on Qwen2.5-VL and Qwen3-VL, no inference overhead
...
TL;DR: SCOPE 是一个无需外部数据的自对弈框架，通过协同进化 Challenger（生成文档基础任务）和 Solver（多轮检索回答）两个策略，并利用冻结的初始模型作为自评判器，在开放任务上显著提升语言模型性能，且能泛化到短问答任务。


<details>
  <summary>Details</summary>
Motivation: 现有自对弈方法依赖规则可检查的答案（如数学题），无法处理开放任务；而开放任务通常需要人工策划的提示或前沿模型作为评判者，成本高且不可扩展。作者希望设计一种完全无外部数据的自对弈框架，使语言模型能在开放任务中自我提升。

Method: SCOPE 框架包含两个协同进化的策略：Challenger 根据文档生成任务，Solver 通过多轮检索回答这些任务。使用冻结的初始模型副本作为自评判器，从源文档编写任务评分标准并据此对 Solver 的回答进行评分。两个策略交替训练，Challenger 被鼓励生成 Solver 当前能力边界附近的任务（保持任务难度适中），Solver 则学习更好地完成任务。

Result: 在 Qwen2.5、Qwen3、OLMo-3 三个 7-8B 模型上，SCOPE 在 8 个开放任务基准上最高提升 +10.4 分，匹配或超越使用约 9K 策划提示训练的 GRPO_data。在 7 个保留的短问答基准上最高提升 +13.8 分，全面超越 GRPO_data。消融实验表明：协同进化 Challenger 对保持任务难度至关重要；检索和合成能力的提升贡献因任务而异；评分标准生成质量是自评判的瓶颈。

Conclusion: SCOPE 证明了无需外部数据的自对弈框架可以有效提升语言模型在开放任务上的能力，并且这种提升能泛化到短问答任务。协同进化的任务生成策略是保持训练有效性的关键，而自评判的质量（尤其是评分标准生成）是当前框架的主要瓶颈，值得进一步改进。

Classification reason: 该论文提出基于自对弈（self-play）的框架 SCOPE，通过协同进化两个策略（Challenger 和 Solver）来训练语言模型，无需外部监督或人工标注数据。这属于多模态/语言模型的自监督强化学习与对齐方法，与 canonical subtopic "多模态强化学习与对齐"（multimodal_rl_alignment）高度相关，因为其核心是使用自对弈和自评判机制进行模型训练与对齐。

Importance reason: Novel self-play framework for open-ended tasks, but tangential to user's priority on continuous multimodal models and video/image generation. Signals: Self-play without external data, Co-evolving policies for open-ended tasks, Uses frozen model as self-judge, Improves open-ended and short-form QA, Not aligned with user's priority subtopics or boost keywords
...
### [85] [Federated Variational Preference Alignment with Gumbel-Softmax Prior for Personalized User Preferences](https://arxiv.org/abs/2605.30873)
...
TL;DR: 本文提出FedVPA-GP框架，在联邦学习场景下通过变分偏好学习与Gumbel-Softmax先验实现个性化用户偏好对齐，解决传统单一奖励模型无法处理冲突偏好的问题，并引入联邦混合先验和正交损失来缓解后验坍塌，在HH-RLHF数据集上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习中的LLM对齐框架通常使用单一的奖励模型，导致相互冲突的用户偏好（如有用性与无害性）被平均化，无法实现个性化。变分偏好学习（VPL）虽能建模多样化偏好，但在去中心化场景下因本地数据稀疏和异构性导致后验坍塌问题严重。

Method: 提出FedVPA-GP框架：1）利用Gumbel-Softmax分布作为离散偏好原型的先验，使每个用户对应一个离散偏好原型；2）引入联邦混合先验（Federated Mixture Prior），让客户端利用聚合的全局分布作为动态先验来稳定变分推断；3）设计正交损失（Orthogonal Loss）显式强制偏好原型在潜空间中分离，增强可区分性。

Result: 在HH-RLHF数据集上的实验表明，FedVPA-GP显著优于单一奖励模型的基线方法，能够成功解耦冲突的用户意图（如helpfulness vs. harmlessness），并支持动态偏好切换。

Conclusion: FedVPA-GP通过变分偏好学习与Gumbel-Softmax先验，在保护隐私的联邦学习框架下有效实现了LLM的个性化偏好对齐，为处理异构用户偏好提供了一种可行的方案。

Classification reason: 该论文研究联邦学习中LLM的对齐问题，提出基于变分偏好学习与Gumbel-Softmax先验的个性化偏好对齐方法，属于多模态/语言模型对齐（RLHF/偏好学习）范畴，因此映射到[多模态强化学习与对齐]（multimodal_rl_alignment）这一规范子主题。

Importance reason: Focuses on federated preference alignment for LLMs, which is outside the user's priority subtopics of continuous multimodal models, video/image generation, and world models. Signals: Federated learning, preference alignment, variational inference, Gumbel-Softmax, HH-RLHF dataset
...
### [86] [Differentially Private Preference Data Synthesis for Large Language Model Alignment](https://arxiv.org/abs/2605.30808)
...
TL;DR: 本文提出DPPrefSyn，一种在差分隐私保护下生成合成偏好数据用于LLM对齐的算法。该方法基于Bradley-Terry偏好模型和成对偏好数据的几何结构，先从私有数据中学习带DP保证的偏好模型，再结合公开提示生成高质量合成偏好数据。实验表明该方法在强DP保证下能达到有竞争力的对齐性能。


<details>
  <summary>Details</summary>
Motivation: LLM的后训练对齐阶段依赖真实人类偏好数据，但这些数据包含敏感的用户提示和人工判断，存在隐私泄露风险。现有方法缺乏在保护隐私的前提下生成可用于对齐的合成偏好数据的系统性方案。

Method: DPPrefSyn基于Bradley-Terry偏好模型和成对偏好数据的几何结构。首先从私有数据中学习带差分隐私保证的偏好模型，利用每簇奖励模型的共享线性结构捕获异构人类偏好，并通过差分隐私主成分分析（DP-PCA）提升学习精度。然后利用学到的模型结合公开提示生成高质量的合成偏好数据。

Result: 大量实验表明，DPPrefSyn在强差分隐私保证下实现了有竞争力的对齐性能，验证了合成偏好数据作为隐私保护对齐的可行替代方案的有效性。

Conclusion: 本文首次提出生成差分隐私合成偏好数据用于LLM对齐，证明了在强隐私保护下仍能实现有效的偏好对齐，为隐私敏感场景下的LLM对齐提供了实用方案。

Classification reason: 该论文研究差分隐私合成偏好数据用于LLM对齐，属于偏好学习与对齐方向。虽然论文聚焦于文本LLM而非多模态，但"多模态强化学习与对齐"（multimodal_rl_alignment）是现有[CANONICAL] subtopic中与偏好对齐最相关的主题，且该subtopic涵盖RLHF、DPO、偏好学习等广义对齐方法，因此复用该subtopic。

Importance reason: Focuses on privacy for LLM alignment, not on the user's priority subtopics like continuous models or video generation. Signals: differential privacy, synthetic preference data, LLM alignment, Bradley-Terry model, DP-PCA
...
### [87] [Diversity Matters: Revisiting Test-Time Compute in Vision-Language Models](https://arxiv.org/abs/2605.30713)
...
TL;DR: 本文系统研究了视觉语言模型（VLM）的测试时计算（TTC）策略，发现特征启发式方法无效，多数投票在单模型场景下收益有限，原因是预测多样性不足。为此提出基于熵的TTC（ETTC），通过预测熵选择最自信的预测，在多模型集成中利用置信度差异优先选择更强模型。理论证明ETTC在温和假设下优于多数投票，实验表明其持续超越投票和最佳单模型，且小模型可协同增强大模型。


<details>
  <summary>Details</summary>
Motivation: 测试时计算（TTC）策略在纯语言大模型中已被证明能有效提升推理能力，但在视觉语言模型（VLM）中的应用和收益尚未被充分探索。本文旨在系统研究TTC在VLM中的效果，并解决现有方法因预测多样性不足而性能受限的问题。

Method: 首先系统评估了特征评分和多数投票两种TTC策略在7个VLM和6个基准上的表现。理论分析表明多数投票在单模型场景下收益有限的原因是输出高度相关、缺乏多样性。针对多模型集成场景，提出基于熵的TTC（ETTC），利用预测熵（predictive entropy）衡量每个模型输出的置信度，选择最自信的预测结果。在单模型情况下ETTC退化为多数投票，但在模型集成中通过置信度差异优先选择更强模型。

Result: 特征启发式方法在VLM中失效；多数投票在单模型场景下仅带来微小提升。ETTC在模型集成中持续超越多数投票和最佳单模型。实验表明小模型可以协同增强大模型，实现标准策略无法达到的集成增益。

Conclusion: 多样性是测试时计算在VLM中有效性的关键因素。基于熵的TTC方法能够有效利用多模型集成中的置信度差异，实现优于传统多数投票的性能。该工作揭示了VLM中TTC策略的局限性，并为多模型集成推理提供了新的有效方案。

Classification reason: 该论文系统研究视觉语言模型（VLM）的测试时计算（TTC）策略，包括特征评分、多数投票以及提出的基于熵的TTC方法，核心关注VLM的推理能力提升。这与[多模态推理与思维链]（multimodal_reasoning_cot）中"推理时计算扩展"高度吻合，因此复用该规范子主题。

Importance reason: Directly addresses test-time compute for VLMs, a key efficiency topic, and proposes a novel entropy-based method with theoretical guarantees. Signals: VLM test-time compute, prediction diversity, entropy-based selection, multi-model ensemble, theoretical proof
...
### [88] [Your Teacher Can't Help You Here: Combating Supervision Fidelity Decay in On-Policy Distillation](https://arxiv.org/abs/2605.30833)
...
TL;DR: 本文发现在线策略蒸馏中存在监督保真度衰减（SFD）问题：随着学生生成前缀变长，教师模型的next-token分布变得不自信且缺乏区分度，导致反向KL蒸馏中的纠正信号减弱。为此提出Lookahead Group Reward（LGR），通过评估学生top-K候选token在下一步诱导的教师置信度来分配组归一化奖励，并设计熵触发树注意力机制保持计算效率。在6个数学和代码基准上，LGR在7B学生模型上平均提升mean@8达2.57分，长序列场景下提升更显著（AIME-26达+4.92分）。


<details>
  <summary>Details</summary>
Motivation: 在线策略蒸馏（On-Policy Distillation, OPD）通过让学生模型在其自身生成的轨迹上接受教师token级反馈来迁移推理能力。然而，作者发现当学生生成前缀变长时，教师对下一个token的预测分布变得不自信且区分度下降，导致反向KL蒸馏中的纠正信号减弱，学生模型在长推理链上出现漂移。现有方法未有效解决这一监督保真度衰减问题。

Method: 提出Lookahead Group Reward（LGR），核心洞察是：下一步的教师置信度反映了未来反向KL监督的区分强度。LGR评估学生top-K候选token在下一步诱导的教师置信度，并分配组归一化奖励。同时设计熵触发树注意力机制（entropy-triggered tree-attention）以保持计算效率，仅在教师分布熵较高时展开树搜索。

Result: 在6个数学和代码基准上，LGR在7B学生模型上平均提升mean@8达2.57分，超越OPD基线。在长序列生成场景下增益更大，AIME-26上在39k token长度时达到+4.92分的提升。

Conclusion: 本文揭示了在线策略蒸馏中的监督保真度衰减问题，并证明Lookahead Group Reward能有效缓解该问题，提升长链推理能力。该方法在数学和代码推理任务上均取得显著改进，且长序列场景下优势更为突出。

Classification reason: 该论文研究在线策略蒸馏中教师监督保真度衰减问题，提出Lookahead Group Reward方法提升长链推理能力，在数学和代码基准上评测。核心关注推理能力蒸馏与长链推理质量，属于多模态推理与思维链（multimodal_reasoning_cot）范畴，该[CANONICAL]子主题涵盖推理时计算扩展与推理能力提升。

Importance reason: Focuses on distillation technique for math/code reasoning, not directly on user's priority subtopics like continuous multimodal models or video generation. Signals: on-policy distillation, supervision fidelity decay, reverse-KL, lookahead group reward, math/code reasoning
...
### [89] [Seeing Isn't Knowing: Do VLMs Know When Not to Answer Spatial Questions (and Why)?](https://arxiv.org/abs/2605.30557)
...
TL;DR: 本文提出SpatialUncertain框架，系统评估VLM在遮挡和视角歧义两种观测挑战下的空间推理能力。发现前沿VLM存在两个一致失败模式：(1) 在视觉证据不完整或误导时过度自信回答（遮挡下准确率约30%，视角歧义下低于10%）；(2) 即使提供额外视角，模型在识别哪些视角能提供可靠证据方面近乎随机。呼吁超越答案正确性，评估模型何时应拒绝回答及如何寻求可靠证据。


<details>
  <summary>Details</summary>
Motivation: 现有空间推理基准通常假设观测是充分可靠的，只关注模型能否给出正确答案，而忽略了模型是否能够识别何时问题无法回答以及需要哪些额外观测。现实世界中遮挡和视角歧义普遍存在，VLM需要具备这种不确定性感知能力才能安全部署。

Method: 构建SpatialUncertain评估框架，设计两类观测挑战：(1) 遮挡——隐藏目标信息；(2) 视角歧义——产生误导性视觉线索。对每种配置设计在干净观测下可回答但在挑战下需拒绝回答的空间问题。进一步评估模型能否识别哪些额外视角能解决视角歧义。

Result: 在多个前沿开源和闭源VLM上测试，遮挡下平均准确率约30%，视角歧义下低于10%。模型在识别可靠额外视角方面表现接近随机。模型普遍存在过度自信回答的倾向。

Conclusion: VLM在空间推理中缺乏对观测不确定性的感知能力，倾向于在证据不足时强行回答。未来研究应超越答案正确性，关注模型何时应拒绝回答以及如何主动寻求可靠证据。

Classification reason: 该论文研究VLM在空间推理任务中何时应拒绝回答（abstention）以及识别可靠视角的能力，属于多模态推理范畴。映射到[CANONICAL] subtopic "多模态推理与思维链"（multimodal_reasoning_cot），因为其核心是视觉空间推理的可靠性评估，而非通用VLM架构或幻觉缓解。
...
### [90] [Seeing Before Agreeing: Aligning Multi-Agent Consensus with Visual Evidence](https://arxiv.org/abs/2605.30698)
...
TL;DR: 本文提出 EAGLE，一个无需训练的多智能体 VLM 协作框架，通过显式暴露每个智能体的视觉定位区域作为证据，让智能体相互验证证据一致性，从而在视觉问答中达成更可靠的共识。在六个 VQA 基准上取得最佳平均性能。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体 VQA 方法主要沿用文本中心协议，关注文本讨论而忽略视觉信息对齐。作者发现仅靠答案层面的一致不足以实现可靠的多智能体 VQA，对齐的视觉证据（智能体依赖的图像区域共享支持）才是可信共识的关键。

Method: 提出 EAGLE（Evidence-Aligned Grounded muLti-agent rEasoning），一个无训练的证据中心框架。它显式暴露每个 VLM 智能体的定位区域作为视觉证据，让智能体相互验证这些证据，并利用证据一致性指导最终决策。

Result: 在六个 VQA 基准上的实验表明，EAGLE 在跨领域任务中取得了最佳平均性能，同时保持轻量、可解释且易于部署。

Conclusion: 对齐视觉证据而非仅对齐答案，是多智能体 VQA 实现可靠共识的关键。EAGLE 框架验证了这一思路的有效性，为多智能体多模态推理提供了新的范式。

Classification reason: 该论文研究多智能体协作视觉问答，核心是多模态推理（多智能体基于视觉证据进行推理与共识），属于 [CANONICAL] 多模态推理与思维链 范畴。

Importance reason: Novel multi-agent VLM consensus framework but lacks direct connection to user's priority subtopics like continuous models or video generation. Signals: multi-agent VLM, visual evidence alignment, training-free, VQA benchmarks, interpretable consensus
...
### [91] [UniScale: Adaptive Unified Inference Scaling via Online Joint Optimization of Model Routing and Test-Time Scaling](https://arxiv.org/abs/2605.30898)
...
TL;DR: UniScale 提出统一推理扩展（UIS）框架，将模型路由和测试时计算扩展（TTS）联合建模为上下文多臂老虎机问题，通过 LinUCB 在线学习推理策略，在动态推理场景中实现更细粒度的质量-成本权衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法将模型路由（在不同规模模型间切换）和测试时计算扩展（TTS，在固定模型内调整推理计算量）作为两个独立维度处理，导致模型路由粒度粗、离散，单模型 TTS 存在容量上限且收益递减，两者分离限制了动态推理环境中的适应性。

Method: 提出统一推理扩展（UIS）框架，将模型路由和 TTS 统一到单一优化空间中。基于此，设计 UniScale 在线框架，将自适应 UIS 建模为上下文多臂老虎机问题，使用 LinUCB 算法学习推理策略，并引入效率感知学习和成本建模以在高维动作空间中实现稳定可扩展的优化。

Result: 实验表明，UniScale 能够有效利用 UIS 空间中模型路由与 TTS 的协同效应，在多样化的动态推理场景中实现更细粒度且持续更优的质量-成本权衡。

Conclusion: 将模型路由与测试时计算扩展统一优化是提升 LLM 推理效率-质量权衡的有效范式，UniScale 通过在线学习框架成功实现了这一目标。

Classification reason: 该论文研究LLM推理时计算扩展（test-time scaling）与模型路由的联合优化，属于推理时计算扩展（inference scaling）范畴，与多模态推理与思维链（multimodal_reasoning_cot）中的"推理时计算扩展"方向高度吻合，因此复用该规范子主题。

Importance reason: Proposes a novel online optimization framework unifying model routing and test-time scaling, directly addressing efficient inference priorities. Signals: efficient inference, model routing, test-time scaling, online learning, quality-cost trade-off
...
### [92] [LARK: Learnability-Grounded Trajectory Selection for Efficient Reasoning Distillation](https://arxiv.org/abs/2605.30651)
...
TL;DR: LARK 提出了一种基于可学习性的推理轨迹选择方法，通过定义可学习因子 ρ 并引入 χ² 正则化选择策略，在保持数据分布覆盖的同时选择学生模型能高效学习的轨迹，在多个基座模型和推理任务上优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有推理蒸馏中的轨迹选择方法多基于启发式规则（如轨迹质量或模型置信度），忽略了轨迹对学生模型是否可学习这一关键因素，导致蒸馏效率低下。

Method: 提出 LARK 方法，核心是可学习因子 ρ 用于刻画学生模型训练损失下降速率；引入可学习性代理（learnability proxy）高效估计 ρ，并设计 χ² 正则化选择策略在可学习性与分布覆盖之间取得平衡，两者均有理论保证。

Result: 在多个基座模型和推理任务上，LARK 一致优于现有数据选择基线方法；诊断分析表明 LARK 分数能预测下游训练效用，且 LARK 选择的轨迹能加速监督微调损失下降。

Conclusion: LARK 验证了可学习性引导的轨迹选择在推理蒸馏中的有效性，为高效推理蒸馏提供了新的理论框架和实践方法。

Classification reason: 该论文研究推理蒸馏中的轨迹选择方法，属于多模态推理（reasoning distillation）范畴，与 canonical subtopic "多模态推理与思维链"（multimodal_reasoning_cot）高度相关。

Importance reason: Focuses on reasoning distillation trajectory selection, not directly on the user's priority subtopics like continuous language models or video generation. Signals: reasoning distillation, trajectory selection, learnability factor, χ²-regularized selection, supervised fine-tuning
...
### [93] [The Sword, Shield, and Achilles' Heel: Characterizing the Linguistic Inductive Bias of Large Language Models for Spatial Reasoning in Navigation Planning](https://arxiv.org/abs/2605.31404)
...
TL;DR: 本文提出一个双干预框架，系统研究LLM在导航规划任务中处理文本化空间表征时的语言归纳偏置。通过表征干预（改变语言格式和压缩程度）和上下文干预（组合/冲突探测），发现：拓扑信息是鲁棒规划的坚实盾牌；语言格式是双刃剑，效果取决于模型大小和任务需求；语义信息是致命弱点——错误语义会系统性破坏规划过程。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的导航系统通常将空间信息（拓扑图、语义栅格地图等）转化为文本描述作为LLM输入，但语言结构的选择（如格式、压缩程度）和上下文特征（拓扑、几何、语义）被视为中性工程决策，缺乏对其如何影响LLM行为的系统研究。

Method: 提出双干预框架：(1) 表征干预——改变语言格式（自然语言 vs 结构化格式）和语言压缩程度（详细 vs 简洁），观察对导航规划的影响；(2) 上下文干预——组合不同上下文特征（拓扑、几何、语义）并设计冲突探测实验，明确LLM处理不同上下文线索时的偏好和弱点。在多种空间推理任务和多个模型规模上进行实验。

Result: 实验揭示一致模式：拓扑信息是鲁棒规划的坚实盾牌和支柱；语言格式是双刃剑，效果取决于模型大小、任务需求和压缩程度；语义信息是致命弱点——不正确的语义线索会系统性破坏规划过程。

Conclusion: 基于LLM的导航系统中，有效的文本化空间表征应保持拓扑完整性、根据模型能力校准表征压缩程度、确保语义正确性，而非简单采用单一表征形式。

Classification reason: 该论文研究LLM在导航规划中的空间推理能力，属于多模态推理范畴。虽然不涉及视觉输入，但空间推理是[多模态推理与思维链]（multimodal_reasoning_cot）的核心主题之一，该子主题涵盖视觉推理、空间推理等。论文分析语言结构对空间推理的影响，与多模态推理中的空间推理方向高度吻合。

Importance reason: Strong linguistic bias analysis for spatial reasoning, but only tangentially touches continuous multimodal representations and lacks direct connection to user's priority subtopics. Signals: Linguistic inductive bias in LLMs, spatial reasoning for navigation, representation intervention, context intervention, topological vs semantic cues
...
### [94] [iVGR: Internalizing Visually Grounded Reasoning for MLLMs with Reinforcement Learning](https://arxiv.org/abs/2605.31096)
...
TL;DR: 本文提出 iVGR 框架，通过强化学习将视觉定位能力内化到 MLLM 的文本推理过程中，使模型在推理时无需显式目标框即可实现精准的视觉定位推理，在细粒度评测上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位 CoT 方法在推理时强制要求生成显式目标框，反而会干扰模型的主要答案预测目标，导致性能下降。作者希望将视觉定位能力内化到文本推理过程中，避免显式定位带来的干扰。

Method: 提出 iVGR 框架，采用双流训练策略：一个文本流与一个高质量视觉定位流通过一致性奖励进行对齐，利用强化学习将定位能力转移到文本推理过程中，使模型在推理时无需显式定位即可准确感知。

Result: 在多个细粒度评测基准上，iVGR 显著优于现有基线方法，同时保持了支持工具辅助推理工作流的灵活性。

Conclusion: 视觉定位能力可以通过强化学习内化到文本推理中，无需在推理时强制显式定位，这为 MLLM 的细粒度感知推理提供了更高效的新范式。

Classification reason: 该论文研究 MLLM 的视觉定位推理与 Chain-of-Thought，属于多模态推理与思维链范畴，与 canonical subtopic "多模态推理与思维链" (multimodal_reasoning_cot) 高度匹配。

Importance reason: Directly advances multimodal reasoning via RL, aligning with user's priority on continuous multimodal models and efficient inference. Signals: multimodal large language model, reinforcement learning, visual grounding, chain-of-thought, fine-grained perception
...
### [95] [Learning Whom to Trust: Market-Feedback Adaptive Retrieval for Frozen LLMs in Event-Driven Financial RAG](https://arxiv.org/abs/2605.31201)
...
TL;DR: 本文提出一种市场反馈自适应的金融RAG方法，通过贝叶斯源记忆模块根据已实现的收益信号动态调整检索来源（新闻 vs SEC文件）的权重，在冻结LLM阅读器的情况下，相比无记忆基线将宏F1从0.438提升至0.471，下游投资组合夏普比率从0.52提升至0.84。


<details>
  <summary>Details</summary>
Motivation: 在金融RAG中，传统方法仅按文本相关性排序检索结果，但金融市场的有用证据来源取决于事件类型、预测时间跨度和市场环境。例如，某些事件下SEC文件比新闻更有信息量，反之亦然。现有方法无法根据市场反馈动态调整检索策略。

Method: 提出Frozen Reader with Source Memory (FRSM) 方法：1) 对每个公司-新闻锚点，检索相关新闻和SEC文件段落，并附加市场背景卡片；2) 使用冻结的LLM阅读器进行多时间跨度残差收益预测；3) 外部贝叶斯源记忆模块根据已实现的残差收益反馈，更新对不同来源（新闻/SEC）的信任权重，从而自适应调整检索层。

Result: 在FinRL-DeepSeek/FNSPID任务的89只纳斯达克股票数据集上，FRSM相比无记忆基线将宏F1从0.438提升至0.471，夏普比率从0.52提升至0.84。有监督LoRA阅读器在静态RAG上表现尚可，但未超过冻结的源记忆阅读器。

Conclusion: 对于金融RAG，学习从何处检索（检索源选择）与学习如何阅读（阅读器能力）同等重要。所提出的模块化源记忆方法提供了一条简单有效的市场反馈自适应路径。

Classification reason: 该论文研究金融事件驱动的RAG系统，核心是检索层自适应优化（通过贝叶斯源记忆更新检索权重），属于多模态/跨模态检索范畴。虽然论文聚焦金融领域，但其方法（检索源选择与自适应更新）是检索增强生成中的通用问题，与 canonical subtopic "多模态检索" 中的文档检索、跨模态检索方向高度相关。

Importance reason: Focuses on financial RAG with market feedback, which is outside your priority subtopics and has negative keywords. Signals: Financial RAG, market-feedback adaptation, Bayesian source memory, event-driven prediction, frozen LLM reader
...
### [96] [Variational Adapter for Cross-modal Similarity Representation](https://arxiv.org/abs/2605.30968)
...
TL;DR: 提出变分适配器 VACSR，将图文匹配中因缺乏细粒度标注导致的二值化相似度压缩问题建模为变分推断，构建跨模态相似度潜空间并通过正则化缓解过拟合，在图像-文本检索、域泛化和 base-to-novel 泛化任务上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 大多数图文匹配和多类图像分类数据集缺乏细粒度跨模态匹配标注，迫使连续的相似度空间被压缩为二分类边界，导致假负样本并严重损害跨模态任务的泛化性能。现有方法仅建模模态内歧义性，忽略了标注本身的缺陷，导致不确定性分配次优。

Method: 提出 Variational Adapter for Cross-modal Similarity Representation (VACSR)，将细粒度语义稀缺下的图文匹配重新表述为变分推断问题。构建跨模态相似度的潜空间，利用正则化技术缓解对二值标注的过拟合，从而恢复连续的相似度表征。

Result: 在图像-文本检索、域泛化和 base-to-novel 泛化三个任务上进行了实验，结果表明 VACSR 有效且具有鲁棒的泛化能力。

Conclusion: VACSR 通过变分推断框架有效解决了图文匹配中因标注粗糙导致的相似度二值化问题，提升了跨模态任务的泛化性能。

Classification reason: 该论文研究图文匹配与跨模态相似度表征，核心任务是图像-文本检索（image-text retrieval），属于多模态检索范畴。现有 [CANONICAL] subtopic "多模态检索"（multimodal_retrieval）完全匹配。

Importance reason: Focuses on cross-modal retrieval with variational inference, not aligned with user's priority on generation or world models. Signals: cross-modal retrieval, variational inference, similarity representation, domain generalization, not generation-focused
...
### [97] [Beyond Classification: Dynamic Adapter Routing for Continual Multimodal Retrieval](https://arxiv.org/abs/2605.31229)
...
TL;DR: 本文提出持续多模态检索（CMR）评测框架，系统评估了现有方法在更具挑战性的检索场景下的表现，发现标准类增量学习方法效果有限。为此提出动态适配器路由（DAR），通过原型路由选择适配器并组合，在多个视觉域上取得优于基线的性能，并展现出良好的分布外泛化能力。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的检索功能至关重要，但持续更新模型以适应检索任务的研究严重不足。现有工作通常将持续检索简化为类增量学习（CIL）问题，未能充分捕捉检索任务的独特动态特性。

Method: 提出动态适配器路由（DAR）方法：基于原型（prototype）路由机制选择适配器，并通过模型集成组合多个适配器的输出，实现持续多模态检索。

Result: 在多个视觉域上，DAR 优于现有基线方法，并在分布外（OOD）评估下展现出强泛化能力。

Conclusion: 持续多模态检索（CMR）具有独特的挑战，标准 CIL 方法难以带来有意义的提升，而 DAR 是一种有效的解决方案，值得进一步研究。

Classification reason: 该论文聚焦于多模态检索（multimodal retrieval）的持续学习问题，属于多模态理解与生成方向下的"多模态检索"（multimodal_retrieval）规范子主题。论文提出持续多模态检索（CMR）评测框架和动态适配器路由（DAR）方法，核心是跨模态检索任务。

Importance reason: Focuses on continual multimodal retrieval, which is not a priority subtopic and lacks strong alignment with user's key boost keywords. Signals: Continual learning, multimodal retrieval, adapter routing, prototype-based, out-of-distribution generalization
...
### [98] [Inference-Free Multimodal Learned Sparse Retrieval for Production-Scale Visual Document Search](https://arxiv.org/abs/2605.30917)
...
TL;DR: 本文提出 V-SPLADE，一种推理时无需神经编码的稀疏检索器，用于大规模视觉文档检索。通过引入标题门控 token 监督（caption-gated token supervision），利用 VLM 生成的标题作为词汇线索激活检索相关词汇维度，解决了视觉稀疏表示难以捕获文档图像中词汇内容的词汇接地问题。在六个基准上 NDCG@5 平均提升 +13.8pp，在 1870 万文档语料上 R@5 提升超过 2 倍。


<details>
  <summary>Details</summary>
Motivation: 现有视觉文档检索方法要么使用 VLM 密集/多向量模型（推理时需神经编码，部署成本高），要么使用 OCR 或标题的 BM25（需耗时的文本提取或生成）。缺乏一种可部署的、能对视觉文档进行词汇索引且推理时无需神经编码的检索系统。

Method: 提出 V-SPLADE，一种推理时无需神经编码的稀疏检索器。核心创新是 caption-gated token supervision（标题门控 token 监督），在训练时利用 VLM 生成的标题作为词汇线索，通过门控机制激活检索相关词汇维度，使视觉稀疏表示能更好地捕获文档图像中的词汇内容。推理时仅需稀疏向量索引和词汇匹配，无需任何神经编码。

Result: 在六个视觉文档检索基准上，V-SPLADE 的 NDCG@5 比同规模密集基线平均提升 +13.8pp，比基于 OCR 或标题的 BM25 基线提升最多 +6.3pp。在 1870 万文档的大规模语料上，R@5 比同规模密集基线提升超过 2 倍，且通过分数融合可进一步提升其他检索器最多 +2.4pp R@5。

Conclusion: V-SPLADE 首次实现了推理时无需神经编码的视觉文档稀疏检索，达到了与密集检索相当甚至更优的效果，填补了可部署的视觉文档检索系统的空白。标题门控 token 监督有效解决了视觉稀疏表示的词汇接地问题。

Classification reason: 该论文研究视觉文档检索，属于跨模态/多模态检索范畴，与 [CANONICAL] subtopic "多模态检索" (multimodal_retrieval) 高度匹配。

Importance reason: Focuses on retrieval efficiency for visual documents, not on the user's priority subtopics like continuous language models or video generation. Signals: sparse retrieval, inference-free, visual document search, lexical grounding, caption-gated token supervision
...
### [99] [TeachObs: A Human-Validated Benchmark for Multimodal Teaching Observation and Model Evaluation](https://arxiv.org/abs/2605.30673)
...
TL;DR: TeachObs 是一个人工验证的多模态课堂教学观察基准，包含30节公开课视频（5158个15秒片段），由7位研究者标注39个二值观察编码（20个视觉+19个非视觉），并提供了片段级和整课级两层人工参考标注。评估5个前沿视觉LLM在三个赛道上的表现，发现没有单一模型在所有赛道中一致最优，添加中间帧会同时增加正确和错误归因，且模型对程序清晰课程的评价高于专家评分。


<details>
  <summary>Details</summary>
Motivation: 课堂教学视频中包含丰富的可观察教学实践，但其教学和视觉信号很少以适合模型评估的形式组织。现有基准缺乏对课堂教学视频的多模态细粒度标注和系统化评估，因此需要构建一个人工验证的基准来评估AI系统在课堂视频分析中的能力边界。

Method: 收集30节来自8个国家的公开课视频，分割为5158个15秒片段。7位研究者对每个片段标注39个二值观察编码（20个视觉编码如手势、板书、指向、视觉材料；19个非视觉编码如指令、监控、提问、反馈、反思）。基于Krippendorff's alpha构建可靠性感知的金标准标签。3位专家对30节课进行整课级评分（教学设计、教学传递、学习者反应、学习材料、课堂收尾）。评估5个视觉LLM在三个赛道：纯文本片段编码、文本+帧片段编码、LLM-as-judge整课评估。

Result: 没有单一模型在所有三个赛道中一致最优；添加中间帧会同时增加每个场景的正确和错误归因；模型对程序清晰课程的评价高于专家评分。TeachObs 支持细粒度标注基准测试和整课评估，揭示了AI系统在课堂视频分析中的能力与局限。

Conclusion: TeachObs 为多模态课堂教学观察提供了人工验证的基准，展示了AI在辅助课堂视频分析方面的潜力，同时表明在多样化科目、课堂形式和标注难度下，专家判断仍然不可或缺。

Classification reason: 该论文提出了 TeachObs，一个针对课堂教学视频的多模态评测基准，包含细粒度标注和整课评估，属于多模态评测与基准方向，因此映射到 canonical subtopic "多模态评测与基准"。

Importance reason: This paper is a domain-specific benchmark for classroom observation, not aligned with the user's priority subtopics on continuous multimodal models, autoregressive generation, or world models. Signals: multimodal benchmark, classroom video, teaching observation, no continuous token or generation focus, low priority subtopic match
...
### [100] [MineExplorer: Evaluating Open-World Exploration of MLLM Agents in Minecraft](https://arxiv.org/abs/2605.30931)
...
TL;DR: 本文提出 MineExplorer 基准，用于评估多模态大语言模型智能体在 Minecraft 开放世界中的探索能力。通过筛选原子任务、构建 ReAct 风格能力框架、设计隐式多跳任务，并采用多智能体合成工作流生成可靠实例，实验表明开放世界探索对现有 MLLM 仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有具身和游戏基准通常将交互压缩为短时任务，或与特定游戏机制纠缠，难以评估 MLLM 在动态开放世界中的持续探索能力。

Method: 首先过滤依赖 Minecraft 特定知识的原子任务以反映通用开放世界推理；围绕 ReAct 风格能力框架组织基准，将原子任务组合为隐式多跳任务；采用多智能体合成工作流联合设计任务图、沙盒场景和基于规则的里程碑评估器。

Result: 人类评估表明多智能体合成工作流比单智能体基线产生更可靠的实例。实验显示开放世界探索仍具挑战性，强模型能处理许多单跳任务，但在需要协调隐藏前提条件的长轨迹任务中性能急剧下降。任务难度与智能体完成度相关，更大模型或思维模式并不一致地带来更好性能。

Conclusion: MineExplorer 基准有效揭示了现有 MLLM 智能体在开放世界探索中的局限性，为未来研究提供了有价值的评测平台。

Classification reason: 该论文提出了 MineExplorer 基准，用于评估 MLLM 智能体在 Minecraft 开放世界中的探索能力，属于多模态评测与基准（multimodal_benchmark）这一规范主题。

Importance reason: Evaluates MLLM agents in open-world exploration but lacks direct connection to user's priority subtopics like continuous models or video generation. Signals: MLLM agent benchmark, Minecraft open-world, multi-agent synthesis, ReAct-style evaluation, implicit multi-hop tasks
...
### [101] [BilliardPhys-Bench: Benchmarking Physical Reasoning and Visual Dynamics of Multimodal LLMs](https://arxiv.org/abs/2605.30900)
...
TL;DR: 本文提出 BilliardPhys-Bench，一个基于合成台球环境的物理推理基准，用于评估多模态大模型在单张图像中预测物体运动与交互的能力。实验发现模型存在"静态偏差"——当物理结果难以推断时倾向于预测无交互，且性能随模拟时间和场景复杂度增加而下降。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型在静态图像识别上表现良好，但在直观物理推理（如预测物体如何运动与交互）方面仍然薄弱。缺乏系统性的基准来评估和诊断这一能力缺陷。

Method: 设计了一个程序化生成的合成台球环境基准，包含摩擦和弹性碰撞的物理模拟。基准测试三个能力：球间碰撞预测、墙壁反弹推理、运动停止后最终位置估计。评估了 GPT、Claude、Gemini、Qwen 系列的最新 MLLM。

Result: 模型性能随模拟时间增加和场景几何复杂度上升而下降。发现一个一致的失败模式——"静态偏差"：当正确物理结果难以推断时，模型倾向于预测无交互。

Conclusion: 当前 MLLM 在视觉动力学推理上存在系统性缺陷，需要更好的物理归纳偏置来改进多模态架构。

Classification reason: 该论文提出了一个用于评估多模态大模型物理推理能力的基准（BilliardPhys-Bench），属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Benchmark reveals static bias in MLLMs but lacks direct connection to user's priority subtopics like continuous models or generation. Signals: physical reasoning benchmark, multimodal LLM evaluation, stasis bias, synthetic billiards, visual dynamics failure
...
### [102] [GraphARC: A Comprehensive Benchmark for Graph-Based Abstract Reasoning](https://arxiv.org/abs/2605.31031)
...
TL;DR: GraphARC 是一个基于图结构的抽象推理基准，将 ARC 的少样本变换学习范式推广到图数据上，涵盖局部、全局和层次化图变换。评估显示当前语言模型在图属性问答上表现尚可，但在完整图变换任务上存在明显的"理解-执行差距"，且随图规模增大性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有抽象推理基准（如 ARC）局限于网格或文本格式，缺乏对图结构数据的系统评测。图结构数据在现实世界中广泛存在（如知识图谱、分子结构、程序代码），需要一个专门针对图抽象推理的基准来评估模型的推理能力。

Method: GraphARC 将 ARC 的少样本变换学习范式推广到图数据上。每个任务包含几个输入-输出图对，模型需推断变换规则并应用到新测试图上。覆盖局部（如节点/边增删改）、全局（如图同构变换）和层次化（如子图合并/拆分）三类图变换。支持在不同图族和图规模下大规模生成实例。

Result: 在多个 SOTA 语言模型上的评估显示：模型能回答图属性相关问题，但在完整图变换任务上表现不佳，存在"理解-执行差距"；在更大图实例上性能显著下降，暴露出扩展性瓶颈。

Conclusion: GraphARC 提供了一个系统评估图抽象推理能力的基准，揭示了当前模型在图变换推理上的局限性。该基准将节点分类、链接预测和图生成整合在同一框架中，为未来图基础模型的评估提供了有前景的测试平台。

Classification reason: 该论文提出了一个基于图结构的抽象推理基准 GraphARC，属于多模态/视觉推理评测范畴。虽然不直接涉及视觉模态，但抽象推理基准（类似 ARC）是多模态理解与生成方向中评测体系的一部分，可归入 [CANONICAL] 多模态评测与基准 子主题。

Importance reason: The paper focuses on graph-based abstract reasoning, which is not aligned with the user's priority subtopics on continuous multimodal models, video/image generation, or world models. Signals: Graph benchmark, Abstract reasoning, Not in priority subtopics, No boost keywords matched, Negative keywords not present but irrelevant
...
### [103] [FAM-Bench: A Multimodal Benchmark for Condition-Aware Food-as-Medicine Reasoning](https://arxiv.org/abs/2605.31410)
...
TL;DR: FAM-Bench 是一个面向"食物即药物"场景的多模态评测基准，包含 2500 个营养专家验证的实例，覆盖 13 种饮食相关健康状况，通过菜肴适宜性评估和比较分析两个任务，测试语言和视觉语言模型在健康约束下的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有食物 AI 基准主要评估菜肴识别、食谱理解、营养估算或一般营养问答，缺乏对"特定健康状况下食物选择是否合适"这一健康感知决策层的评测。作者旨在填补这一空白，构建一个标准化的健康感知推理测试平台。

Method: 构建 FAM-Bench 基准，包含 2500 个营养专家验证的实例，覆盖 13 种饮食相关健康状况。设计两个互补任务：(1) 菜肴级适宜性评估——模型根据菜肴图像和配料列表判断是否适合特定健康状况；(2) 比较性菜肴分析——模型对四个候选菜肴按条件适宜性排序。两个任务都需要整合配料证据、视觉制备线索和临床营养约束。

Result: 论文主要介绍基准构建，未报告具体模型评测结果，但该基准为评估语言和视觉语言模型在 grounded health-aware reasoning 方面的能力提供了标准化测试平台。

Conclusion: FAM-Bench 填补了食物 AI 评测中健康感知决策层的空白，为未来研究提供了标准化的多模态健康推理评测基准。

Classification reason: 该论文提出了一个面向"食物即药物"（Food-as-Medicine）场景的多模态评测基准 FAM-Bench，包含菜肴适宜性评估和比较分析两个任务，属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）子主题。

Importance reason: The benchmark is domain-specific to food-as-medicine, which is not a priority subtopic and lacks alignment with the user's focus on continuous multimodal models, video generation, or world models. Signals: Multimodal benchmark for health reasoning, 2500 expert-verified instances, Two tasks: suitability assessment and comparative analysis, Tests language and vision-language models, No direct connection to continuous tokens, autoregressive generation, or world models
...
### [104] [A Visually Impaired Assistance Benchmark for VLM-as-a-Judge Evaluation](https://arxiv.org/abs/2605.31351)
...
TL;DR: 本文提出 VIABLE，首个面向视障辅助（VIA）场景的 VLM-as-a-Judge 评测基准，包含 30 万+ 判断样本和 12 种失效模式分类。系统评测 7 个 VLM 裁判后发现现有模型在有效性、公正性和稳定性上均不可靠，GPT-5.4 单故障诊断准确率仅 52.6%，自偏好率高达 94.2%。为此提出 VIA-Judge-Agent，通过视觉证据提取和分类引导工作流提升诊断准确率和下游响应质量。


<details>
  <summary>Details</summary>
Motivation: 基于 AI 的视障辅助（VIA）任务依赖人工评估，成本高昂。VLM-as-a-Judge 范式在通用领域有潜力，但在 VIA 场景中是否可信尚不清楚，因此需要系统评测。

Method: 构建 VIABLE 基准（30 万+ 样本，3 个场景，12 种失效模式），提出 Effectiveness-Impartiality-Stability 三维评估框架。系统评测 7 个 VLM 裁判（含 GPT-5.4 等），并提出 VIA-Judge-Agent：模型无关的推理时增强框架，结合视觉证据提取和分类引导工作流。

Result: 现有 VLM 裁判在 VIA 场景中全面不可靠：GPT-5.4 单故障诊断准确率仅 52.6%，自偏好率 94.2%；开源模型存在严重偏见和对抗脆弱性。VIA-Judge-Agent 能有效提升诊断准确率和 BLV 用户偏好的下游响应质量。

Conclusion: VLM-as-a-Judge 在视障辅助任务中尚不可靠，需要专门的评测基准和增强方法。VIA-Judge-Agent 提供了一种有效的改进方案。

Classification reason: 该论文提出了 VIABLE，一个针对 VLM-as-a-Judge 在视障辅助任务上的评测基准，属于多模态评测与基准方向，与 [CANONICAL] subtopic "多模态评测与基准" 高度匹配。
...
### [105] [ERGeoBench:A Comprehensive Benchmark for Embodied Reasoning and Geo-localization in Multimodal Large Language Models](https://arxiv.org/abs/2605.31251)
...
TL;DR: ERGeoBench 是一个面向具身地理定位的多模态大模型诊断基准，包含 2207 个全球街景全景图，在单视图、全景视图和具身视图三种渐进设置下评估模型的感知、空间、常识和地理定位推理能力。实验表明当前 MLLM 能推断高层地理语义，但在细粒度感知、度量定位和跨视图空间一致性上仍存在困难。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 评测缺乏对具身地理定位（embodied geo-localization）这一任务的细粒度评估，而该能力对于具身智能体在真实环境中导航和定位至关重要。

Method: 构建了包含 2207 个全球街景全景图的基准 ERGeoBench，设计了单视图、全景视图和具身视图三种渐进式评测设置（具身视图允许智能体通过偏航、俯仰和缩放主动获取观测），并定义了四种互补能力维度：基础感知、空间意识、常识推理和地理定位推理。

Result: 评测了多个主流闭源和开源 MLLM，发现模型能推断高层地理语义，但在细粒度感知操作、度量定位和跨视图空间一致性上表现不佳；地理定位能力与其他能力维度高度相关，表明准确定位依赖于感知、空间推理和常识推理的整合。

Conclusion: ERGeoBench 为诊断和推进类人具身地理定位提供了统一框架，揭示了当前 MLLM 在该任务上的关键不足，并为未来研究指明了方向。

Classification reason: 该论文提出了 ERGeoBench，一个面向具身地理定位的多模态大模型评测基准，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Benchmark for embodied geo-localization, but outside user's priority subtopics and contains negative keyword "remote sensing". Signals: multimodal understanding generation, benchmark, geo-localization, negative keyword remote sensing, no direct match to priority subtopics
...
### [106] [BlueFin: Benchmarking LLM Agents on Financial Spreadsheets](https://arxiv.org/abs/2605.30907)
*Srivatsa Kundurthy, Clara Na, Colton Moraine, Anoushka Mohta, Case Winter, George Fang, John Ling, Emma Strubell, Zach Kirshner*

Main category: cs.AI, cs.SE, cs.CL, cs.LG
...
TL;DR: BlueFin是一个面向金融电子表格的LLM智能体基准，包含131个复杂任务和3225个细粒度评分标准，由专家验证的LM judge进行评估。前沿LLM在该基准上平均得分低于50%，尤其在动态正确性方面表现薄弱。


<details>
  <summary>Details</summary>
Motivation: 尽管电子表格软件拥有数亿用户（远超专业开发者数量），但LLM在电子表格领域的能力探索相对不足，尤其缺乏反映真实金融职业场景的评测基准。现有基准要么过于简单，要么无法可靠评估复杂任务。

Method: 构建了131个金融电子表格任务（涵盖合成、操作和理解三类），每个任务配有专家验证的细粒度评分标准（共3225条）。设计了一个LM judge评估框架，该judge与专家共识达到高度一致（α=0.826, macro-F1=0.839），能够可靠评估难以程序化验证的复杂任务。

Result: 前沿LLM在BlueFin上平均得分低于50%，模型在动态正确性方面表现出明显弱点。LM judge与专家共识的评估一致性达到α=0.826，macro-F1=0.839。

Conclusion: BlueFin揭示了当前LLM在金融电子表格任务上的显著不足，为未来研究提供了高质量的评测基准和评估框架。电子表格领域仍是一个有待深入探索的重要LLM应用方向。

Classification reason: 该论文提出了一个面向金融电子表格的LLM智能体评测基准BlueFin，属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）子主题。虽然基准聚焦于电子表格（结构化数据）而非传统视觉模态，但其评测框架、LM judge评估、专家验证等核心贡献与多模态评测基准子主题高度吻合。

Importance reason: Focuses on financial spreadsheets, which is outside the user's priority subtopics and includes a negative keyword. Signals: Financial domain, spreadsheet benchmark, LLM agents, low model performance, expert-validated evaluation
...
Importance: 普通相关 (64.47) - 未入选精读

TL;DR: SVI-Bench 是一个大规模多模态评测基准，利用团队体育作为动态微世界，包含约 35K 小时广播视频、15M 标注动作、15K 小时专家解说等数据，设计了从动态场景理解、因果推理、战略模拟到智能体综合的 9 个渐进式任务。评测发现现有最强模型在感知任务上约 73%，但在智能体任务上仅 5%，揭示了当前多模态模型在战略视频智能上的能力断层。


<details>
  <summary>Details</summary>
Motivation: 现有视频基准要么使用真实视频但缺乏因果和战略问题的可验证真值，要么使用合成环境但牺牲了真实多智能体系统的复杂性。团队体育兼具真实多智能体交互的复杂性和规则/结果的可验证性，是理想的动态微世界。作者希望填补从感知到因果推理、模拟和战略规划这一完整能力栈的评测空白。

Method: 构建了一个数据引擎，将原始比赛数据（广播视频、动作标注、专家解说、比赛报告、统计数据）转化为密集交叉引用的语料库。设计了四层九任务的渐进式评测体系：动态场景理解（细粒度动作 QA、多模态检索）、因果推理（反事实推理、因果归因）、战略模拟（假设性模拟、策略预测）、智能体综合（证据收集、自主决策）。

Result: 最强模型（GPT-4o）在感知任务上约 73%，因果推理约 50%，战略模拟约 30%，智能体任务仅 5%。所有模型在需要跨模态证据整合和自主决策的任务上表现极差，存在显著的能力断层。

Conclusion: SVI-Bench 揭示了当前多模态模型在战略视频智能上的严重局限，尤其是因果推理、假设性模拟和自主智能体能力远未达到实用水平。该基准为未来研究提供了明确的方向和挑战性评估平台。

Classification reason: 该论文提出了 SVI-Bench，一个面向战略视频智能（从感知到因果推理、模拟到策略规划）的多层次评测基准，属于多模态评测与基准（multimodal_benchmark）这一规范主题。

Importance reason: Strong benchmark for strategic video intelligence but only tangentially touches your priority subtopics. Signals: new benchmark for strategic video intelligence, large-scale real-world multi-agent data, reveals capability cliff in multimodal models, uses team sports as microworld, no direct connection to continuous language/multimodal models or generation
...
TL;DR: nuReasoning 是一个大规模真实世界自动驾驶推理数据集与基准，包含 20,000 个 20 秒长的视频片段，覆盖多城市多摄像头图像、LiDAR、HD 地图和人工验证的推理标注（空间推理、决策推理、反事实推理）。实验表明，在该数据集上微调 VLM 可显著提升驾驶问答性能，且将推理监督融入 VLA 训练能改善规划性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集和基准主要关注感知、预测或规划，缺乏对长尾场景中推理能力（常识推理、空间关系理解、智能体交互推断、安全决策）的监督和评测。作者旨在填补这一空白，推动自动驾驶系统向可解释、鲁棒的推理驱动方向发展。

Method: 基于 nuScenes/nuPlan 数据，构建 20,000 个 20 秒长尾驾驶片段，包含多摄像头图像、LiDAR、HD 地图和物体标注。人工标注三类推理：空间推理（场景布局与关系）、决策推理（驾驶行为合理性）、反事实推理（替代场景下的假设性推理）。支持推理问答评测和规划评测两种任务。

Result: 在 nuReasoning 上微调 VLM 显著提升了驾驶特定问答的准确性；将推理监督融入 VLA（视觉-语言-动作）训练后，即使推理时关闭文本推理输出，规划性能仍得到改善，表明推理监督对驾驶策略学习有正向迁移作用。

Conclusion: nuReasoning 为长尾自动驾驶场景下的推理评估和规划评估提供了基础性资源，验证了推理监督对提升驾驶系统鲁棒性和可解释性的价值，有望推动推理驱动的自动驾驶系统发展。

Classification reason: 该论文提出了 nuReasoning，一个面向长尾自动驾驶场景的推理数据集与评测基准，包含空间推理、决策推理和反事实推理标注，并支持推理评估与规划评估。这属于多模态评测与基准（multimodal_benchmark）范畴，因此复用该 canonical subtopic。

Importance reason: Focuses on autonomous driving reasoning, not aligned with user's priority on continuous multimodal models or video generation. Signals: Autonomous driving, Long-tail reasoning, VLM fine-tuning, nuScenes lineage, Planning evaluation
...
### [109] [SOCO: Benchmarking Semantic Object Correspondence in Vision Foundation Models](https://arxiv.org/abs/2605.31597)
...
TL;DR: SOCO是一个新的语义对象对应（Semantic Object Correspondence）基准，包含100个类别、超过100万对应标注对，并引入对应类型分类学和关键点语言描述。实验发现视觉基础模型编码了强语义结构但跨类别对应迁移差，LVLM在文本引导定位上强于视觉参考跨图像匹配，且对应性能比ImageNet分类更能预测下游密集任务表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉基础模型的评估缺乏对结构化对象理解（特别是部件级语义对应）的系统性评测，已有协议不一致且部件级监督有限。作者希望建立一个标准化、细粒度的基准来评估模型在跨实例、跨类别、大外观/视角/几何变化下的部件匹配能力。

Method: 构建SOCO基准：定义对应类型分类学（实例级、类别级、功能对应等），在100个类别上标注功能上有意义的关键点，生成超过100万对应对；同时为关键点提供语言描述以支持LVLM评估。在多种视觉基础模型（DINOv2、CLIP、MAE等）和LVLM上进行系统评测。

Result: (i) 视觉基础骨干编码了强语义结构，但跨相关类别对应迁移差，仅部分捕获对象部件位置；(ii) LVLM在文本提示部件定位上强于视觉参考跨图像匹配，揭示语言定位与细粒度视觉对应之间的差距；(iii) 对应性能比ImageNet分类更能预测分割、跟踪、3D姿态估计和3D检测等下游密集任务的表现。

Conclusion: SOCO可作为评估视觉和多模态基础模型结构化部件级表征质量的基准，揭示了当前模型在细粒度语义对应上的不足，并为下游密集任务性能提供了更好的预测指标。

Classification reason: 该论文提出了SOCO基准，用于评估视觉基础模型和视觉语言模型在语义对象对应（semantic object correspondence）上的细粒度部件级理解能力，属于多模态评测与基准方向，因此映射到CANONICAL subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Strong benchmark for vision model evaluation but outside user's priority subtopics on continuous models, generation, and world models. Signals: semantic correspondence benchmark, vision foundation model evaluation, part-level understanding, LVLM text vs visual matching, downstream task prediction
...
### [110] [PEEK: Picking Essential frames via Efficient Knowledge distillation](https://arxiv.org/abs/2605.31029)
...
TL;DR: PEEK 提出一种高效的动态帧采样方法，通过将教师模型的字幕条件帧重要性排序蒸馏到轻量级视觉时序模型中，在极低帧预算下显著提升视频字幕生成的效率与性能，仅增加 5.2% 的计算开销。


<details>
  <summary>Details</summary>
Motivation: 视频语言模型只能处理有限数量的帧，而现有帧采样方法（如均匀采样）对视觉内容不敏感，自适应采样方法虽能选择信息量更大的帧但计算开销过大。因此需要一种既高效又能准确选择关键帧的采样方法。

Method: PEEK 采用知识蒸馏框架：首先使用一个更强的教师模型（基于字幕条件）对视频帧进行相关性排序，生成帧重要性标签；然后训练一个轻量级的时序模型（仅基于视觉特征）来预测帧的重要性分数，从而在推理时无需字幕信息即可快速选择关键帧。该方法在极低帧预算（1-2帧）下尤其有效。

Result: 在 ActivityNet Captions 和 MSR-VTT 上，PEEK 在大多数帧预算配置下优于现有方法，尤其在低帧预算下表现突出。在 ActivityNet Captions 上赢得 16 个配置中的 14 个。效率方面，PEEK 仅增加 5.2% 的字幕生成时间，远低于 CSTA（65.4%）和 MaxInfo（211.9%）。

Conclusion: PEEK 证明了通过知识蒸馏实现高效、准确的动态帧采样是可行的，在极低帧预算下既能保持高精度又能大幅降低计算开销，为视频语言模型的实际部署提供了实用的帧选择方案。

Classification reason: 该论文提出一种高效的动态帧采样方法，通过知识蒸馏将教师模型的帧重要性排序迁移到轻量级时序模型，旨在减少视频语言模型处理的帧数从而提升效率。这属于多模态高效化与轻量化（efficient_multimodal）范畴，具体涉及 token/帧压缩与蒸馏技术。

Importance reason: Efficient frame sampling for video captioning is useful but not central to the user's priority subtopics on continuous multimodal models or autoregressive generation. Signals: Efficient dynamic frame sampling, Knowledge distillation for frame ranking, Low frame budget video captioning, Lightweight temporal model, 5.2% overhead gain
...
### [111] [VisionPulse: Dynamic Visual Sparsity for Efficient Multimodal Reasoning](https://arxiv.org/abs/2605.31457)
...
TL;DR: VisionPulse 提出了一种逐步视觉 token 剪枝框架，在推理过程中动态保留仅 5% 的关键视觉 token，同时将推理轨迹缩短 11.2%，几乎不损失精度，从而显著降低多模态大模型的推理开销。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常在预填充阶段一次性剪枝视觉 token，假设推理过程中所需的视觉证据是静态的。然而本文发现视觉证据具有强烈的步骤依赖性：每个解码步骤仅需少量关键视觉 token，且关键集合随推理过程动态变化。此外，冗余视觉上下文会引导模型关注与查询无关的区域，延长推理轨迹。因此需要一种步骤级别的动态视觉稀疏化方法。

Method: VisionPulse 包含两个核心组件：(1) 轻量级视觉注意力质量（visual attention mass）估计器，利用其与 LMM 有效视觉 token 使用率的强正相关性，动态估计每步的 token 保留预算；(2) 在该预算下仅保留最关键的视觉 token，实现逐步视觉稀疏化。通过强制推理过程中的视觉稀疏性，过滤冗余视觉上下文同时保留相关证据，自然缩短推理轨迹。

Result: 实验表明，VisionPulse 每步仅保留 5% 的视觉 token，推理轨迹缩短 11.2%，同时准确率几乎保持不变。

Conclusion: VisionPulse 验证了视觉证据在推理过程中的步骤依赖性，并提出了一种有效的动态稀疏化框架，在保持精度的同时显著降低推理开销，为多模态大模型的高效部署提供了新思路。

Classification reason: 该论文提出 VisionPulse，一种在推理过程中逐步剪枝视觉 token 的框架，旨在降低多模态大模型推理开销。这属于多模态高效化与轻量化（efficient_multimodal）的典型工作，涉及 token 压缩与稀疏推理，因此复用该规范子主题。

Importance reason: Directly addresses efficient multimodal reasoning via dynamic visual sparsity, aligning with user's high priority on model efficiency and continuous multimodal models. Signals: dynamic visual token pruning, step-wise retention budget, inference efficiency, multimodal reasoning acceleration, visual attention mass
...
### [112] [VLM3: Vision Language Models Are Native 3D Learners](https://arxiv.org/abs/2605.30561)
...
TL;DR: VLM3 提出一种极简方法，通过焦距统一、文本像素参考和数据混合缩放，使标准 VLM 无需架构修改即可在多种 3D 任务上达到专家模型水平，证明了 VLM 是天然的 3D 学习者。


<details>
  <summary>Details</summary>
Motivation: 当前 3D 理解任务严重依赖专家视觉模型和复杂的任务特定设计，而 VLM 虽然在语义理解上表现优异，但在 3D 任务上尚未充分展现能力。作者希望证明 VLM 本身具备 3D 学习能力，只需简单改造即可胜任多种 3D 任务。

Method: 提出 VLM3 方法，核心包含三个关键设计：(1) 焦距统一（focal length unification），将不同焦距的图像归一化到统一视角；(2) 文本像素参考（text-based pixel reference），用文本坐标表示像素位置，避免回归损失；(3) 数据混合与缩放（data mixture and scaling），混合多种 3D 任务数据进行训练。无需修改模型架构、无需大模型、无需复杂数据增强或回归损失。

Result: VLM3 在深度估计上将 VLM 准确率从 0.84 提升至 0.9；在像素对应、相机位姿估计和物体级 3D 理解等任务上达到与专家视觉模型相当的精度，同时保持标准 VLM 架构和基于文本的训练范式。

Conclusion: VLM 是天然的 3D 学习者，通过焦距统一、文本像素参考和数据混合缩放这三个简单设计，即可让标准 VLM 掌握多种 3D 任务，为简单可扩展的 3D 学习开辟了新范式。

Classification reason: 该论文研究如何让标准 VLM 通过简单方法（焦距统一、文本像素参考、数据混合与缩放）掌握多种 3D 任务（深度估计、像素对应、相机位姿估计、物体级 3D 理解），属于多模态理解与生成方向下的空间与 3D 多模态推理（spatial_3d_reasoning）子主题。

Importance reason: Shows standard VLMs can master 3D tasks without architectural changes, aligning with unified multimodal and efficient inference priorities. Signals: unified multimodal, efficient inference, 3D understanding, scalable method, text-based training
...
### [113] [SpatialAct: Probing Spatial Reasoning-to-Action Capabilities of VLM Agents in 3D Scenes](https://arxiv.org/abs/2605.31148)
...
TL;DR: SpatialAct 是一个基于模拟器的基准测试，用于探测 VLM 智能体在 3D 场景中的动作条件空间推理能力。实验揭示了当前 VLM 存在"推理到动作"的差距：在孤立空间推理任务上表现良好，但在多轮交互中难以维持连贯的空间信念并产生可靠动作。


<details>
  <summary>Details</summary>
Motivation: 人类能够轻松感知空间布局、形成认知表征、推理空间关系并将其转化为日常 3D 环境中的动作。虽然 VLM 在观察条件空间感知和推理任务上表现良好，但尚不清楚它们能否构建连贯的空间理解、据此行动并通过多轮反馈优化动作。作者旨在系统性地探测这一能力差距。

Method: 提出 SpatialAct 基准，包含三个层次的任务：(1) 最具挑战性的多轮交互精炼 (Multi-turn Interactive Refinement) 设置；(2) 分解后的单步错误检测与修复 (Single-step Error Detection and Fix)；(3) 五种基础空间能力任务用于诊断模型失败的深层原因。使用模拟器生成 3D 场景和交互轨迹。

Result: 实验表明当前 VLM 在孤立空间推理任务上表现良好，但在多轮交互中难以维持连贯的空间信念并产生可靠动作，性能显著低于人类。存在明显的"推理到动作"差距，即使抽象掉底层控制，VLM 智能体仍缺乏在动作引起的环境变化下进行鲁棒空间状态跟踪的能力。

Conclusion: 当前 VLM 智能体在 3D 场景中的空间推理到动作能力存在显著不足，尤其是在多轮交互反馈场景中。这为未来研究指明了方向：需要提升 VLM 在动态环境中的空间状态跟踪和动作推理能力。

Classification reason: 该论文研究 VLM 智能体在 3D 场景中的空间推理到动作的能力，属于空间与 3D 多模态推理方向，同时涉及具身智能体在环境中的交互，因此也匹配世界模型方向。最佳匹配的 canonical subtopic 是 "空间与 3D 多模态推理" (spatial_3d_reasoning)。

Importance reason: Probes VLM spatial reasoning-to-action gap in 3D, relevant to multimodal understanding but not a top priority subtopic. Signals: VLM spatial reasoning benchmark, reasoning-to-action gap, multi-turn interactive refinement, 3D scene understanding, simulator-grounded evaluation
...
TL;DR: Lumos-Nexus 提出了一种训练高效的视频统一生成框架，采用两阶段设计：训练阶段仅对齐轻量生成器与理解模块，推理阶段通过统一渐进频率桥接（UPFB）在共享潜空间中逐步切换到高容量预训练生成器，实现粗到细的高保真视频生成。同时提出 VR-Bench 基准评估推理驱动的视频生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基于 connector 的视频统一模型在指令引导的视频合成方面表现良好，但将大型高保真生成器集成到统一训练循环中计算成本过高，限制了可实现的视觉质量。因此需要一种训练高效的方案，既能发展强大的推理驱动生成能力，又能显著提升视觉保真度。

Method: Lumos-Nexus 采用两阶段设计：(1) 训练阶段，仅将轻量生成器与理解模块对齐，学习接收推理驱动的语义控制；(2) 推理阶段，提出统一渐进频率桥接（UPFB），在共享潜空间中逐步将生成任务移交给高容量预训练生成器，实现从粗到细的细化，生成高保真视频而不损害推理质量。同时构建 VR-Bench 基准来评估推理驱动的视频生成能力。

Result: 在 VBench 上，Lumos-Nexus 在视觉真实感和时序连贯性方面取得了显著提升；在 VR-Bench 上展示了强大的基于推理的生成性能。

Conclusion: Lumos-Nexus 通过训练高效的统一视频生成框架，成功实现了推理驱动的视频生成与高视觉保真度的兼顾，为视频统一模型的发展提供了新思路。

Classification reason: 该论文提出 Lumos-Nexus，一个基于 connector 的视频统一模型，将理解模块与轻量生成器对齐，再通过渐进频率桥接（UPFB）切换到高保真预训练生成器，实现推理驱动的视频生成。这属于理解与生成一体化的统一多模态模型方向，因此映射到 [CANONICAL] unified_multimodal_model。同时，其视频生成部分也匹配 multimodal_generation 方向下的 text_to_video_diffusion。

Importance reason: Proposes a training-efficient video unified model with frequency bridging in shared latent space, directly advancing unified multimodal models and efficient inference. Signals: unified multimodal model, efficient inference, video generation, continuous latent space, frequency bridging
...
### [115] [Representation Forcing for Bottleneck-Free Unified Multimodal Models](https://arxiv.org/abs/2605.31604)
...
TL;DR: 本文提出 Representation Forcing (RF) 技术，通过让解码器自回归预测视觉表征作为中间 token，再在相同骨干网络中进行像素扩散，从而消除统一多模态模型对独立预训练 VAE 的依赖，实现无瓶颈的端到端理解与生成一体化模型。
...
Method: 提出 Representation Forcing (RF)：在像素扩散之前，强制解码器自回归地预测视觉表征（如 DINOv2 特征）作为中间 token；这些 token 保留在上下文中，在同一骨干网络内引导后续的像素扩散。通过将表征从感知输出转变为生成目标，RF 消除了对外部生成潜空间的需求。

Result: 在图像生成上，像素空间模型搭配 RF 达到了与基于 VAE 的最先进统一模型相当的性能。在图像理解上，像素空间 RF 通常优于其基于 VAE 的变体。RF 同时提升了理解与生成能力。

Conclusion: Representation Forcing 是一种有效的技术，能够消除统一多模态模型对独立 VAE 的依赖，在像素空间中实现无瓶颈的端到端理解与生成，为构建更简洁、更统一的 UMM 迈出了重要一步。

Classification reason: 该论文提出 Representation Forcing (RF) 方法，旨在消除统一多模态模型（UMM）中依赖外部 VAE 的结构瓶颈，实现像素空间中的理解与生成一体化。这直接对应 [CANONICAL] 子主题 "统一多模态模型"（unified_multimodal_model），同时涉及图像生成，因此也匹配 multimodal_generation 方向。

Importance reason: Directly addresses bottleneck-free unified multimodal models with a novel representation forcing technique, aligning with top priority subtopics and boost keywords. Signals: unified multimodal model, bottleneck-free, representation forcing, autoregressive visual tokens, pixel-space diffusion
...
### [116] [A Pilot Study on Curator-Guided Multilingual Art Description for Blind and Low-Vision Audiences with Small Vision-Language Models](https://arxiv.org/abs/2605.31080)
*Iosif Tsangko, Andreas Triantafyllopoulos, George Margetis, Ioana Crihana, Björn W. Schuller*

Main category: cs.CL, cs.AI, cs.MM, cs.HC, cs.CV
...
TL;DR: 该试点研究探索了使用小型 VLM（Qwen2.5-VL-3B）为盲人和低视力观众生成多语言（德语、罗马尼亚语、塞尔维亚语）艺术描述，比较了语言特定 LoRA 适配器与多语言适配器的效果，发现语言特定适配器在可控性和视觉基础描述质量上更稳定。


<details>
  <summary>Details</summary>
Motivation: 盲人和低视力观众在视觉艺术描述方面长期得不到充分服务，尤其是在多语言场景和博物馆环境中，隐私和知识产权限制倾向于使用小型本地部署的 VLM。

Method: 构建了面向 BLV 的平行描述语料库，使用 Qwen2.5-VL-3B-Instruct 作为基础模型，比较语言特定 LoRA 适配器与单一多语言适配器的效果，采用自动词汇/嵌入指标和 LLM-as-Judge 评估协议。

Result: 语言特定适配器在罗马尼亚语和塞尔维亚语上表现出更稳定的可控性和视觉基础描述质量，而多语言适配器在德语上仍具竞争力。

Conclusion: 研究为小型本地部署 VLM 在多语言无障碍艺术描述中的应用提供了初步证据，但需要更大规模的 BLV 用户研究和更广泛的语言覆盖才能得出一般性结论。

Classification reason: 该论文研究使用小型 VLM（Qwen2.5-VL-3B）进行多语言艺术描述生成，属于通用视觉语言模型的应用与微调，与 [CANONICAL] 子主题 "视觉语言模型与 MLLM" 高度匹配。

Importance reason: This paper focuses on accessibility and multilingual art description, which is outside the user's core interests in continuous language models, video/image generation, and world models. Signals: small VLM, multilingual art description, BLV accessibility, LoRA adapters, no continuous token or generation focus
...
### [117] [VLM-GLoc: Vision-Language Model Enhanced Monte Carlo Localization for Robust Semantic Global Localization in Cluttered Quasi-Static Environments](https://arxiv.org/abs/2605.30506)
...
TL;DR: VLM-GLoc 提出了一种利用开放词汇视觉语言模型增强的层次化语义蒙特卡洛定位方法，在几何混淆的准静态环境（如杂货店、实验室）中实现鲁棒的全局定位，相比传统几何方法和专用视觉管线取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统全局定位方法在几何混淆（如平行货架、重复家具）和长尾语义分布的室内环境中表现不佳，而专用视觉管线难以处理瞬态视觉杂乱。作者希望利用 VLM 的开放词汇语义理解能力来提升定位的鲁棒性和泛化性。

Method: 提出 VLM-GLoc 层次化语义蒙特卡洛定位框架：(1) 使用 VLM 作为统一语义观测前端，提取判别性文本特征；(2) 隐式质量过滤模糊/动态物体；(3) 通过永久性推理进行数据增强；(4) 引入逆语义提议机制，通过文本到地图检索播种粒子。

Result: 在两个真实环境（3,500 sq. ft. 杂货店 + 手机平台；3,700 sq. ft. 实验室 + 四足机器人）上，VLM-GLoc 分别达到 70% 和 74% 的全局定位成功率，显著优于传统几何方法和专用视觉基线。

Conclusion: VLM 作为统一语义观测前端能够有效提升机器人在几何混淆、准静态环境中的全局定位性能，验证了开放词汇视觉语言模型在机器人定位任务中的潜力。

Classification reason: 该论文利用 VLM 作为语义观测前端增强蒙特卡洛定位，属于视觉语言模型在机器人定位中的应用，与 [CANONICAL] subtopic "视觉语言模型与 MLLM" 高度匹配。同时涉及机器人定位与规划，也与世界模型方向下的具身世界模型相关。

Importance reason: Focuses on robotics localization with VLMs, not on core continuous multimodal or autoregressive generation priorities. Signals: VLM for MCL, semantic global localization, quasi-static environments, open-vocabulary, hierarchical MCL
...
### [118] [Multi-Turn Multi-Agent Dialogue for Collaborative Reconstruction Improves VLM Performance on Spatial Reasoning, But Only Barely](https://arxiv.org/abs/2605.31387)
...
TL;DR: 本文通过一个协作式结构搭建任务，系统评估了多种VLM在空间推理、视觉解释、语言引导交互和动作生成方面的能力。研究发现，VLM在视觉空间推理上仍存在显著困难，详细的文本描述比视觉输入更有效，分解的图像表示能提升性能。


<details>
  <summary>Details</summary>
Motivation: 机器人在多样化环境中依赖视觉输入理解物体和空间布局，在与人类协作时还需通过语言进行沟通。尽管VLM在视觉解释、问答和指令跟随方面已有进展，但其在需要空间推理的协作对话任务中的能力尚未被充分探索。

Method: 设计了一个协作式结构搭建任务框架，VLM通过多轮多智能体对话，基于视觉和文本输入重建目标结构。评估了多种开源和闭源VLM，比较了不同交互设置、输入模态和图像表示方式对重建成功率的影响。

Result: VLM在视觉空间推理上表现困难；详细的文本表示比视觉输入带来更高的重建成功率；分解的图像表示（如分块图像）能提升性能；但整体上VLM在空间推理上的提升幅度有限。

Conclusion: 当前VLM在视觉空间定位和基于定位的指令生成方面存在明显局限，尚不足以可靠地支持需要精细空间推理的协作机器人任务。

Classification reason: 论文研究VLM在协作对话任务中的空间推理能力，属于通用视觉语言模型能力评估，映射到[CANONICAL] subtopic "vlm_mllm_general"（视觉语言模型与MLLM）。同时涉及机器人协作场景，与world_model方向下的具身世界模型相关。

Importance reason: Focuses on VLM spatial reasoning limitations but lacks direct connection to priority subtopics like continuous models or video generation. Signals: VLM spatial reasoning, collaborative dialogue, visual grounding, limited improvement, not aligned with priority subtopics
...
### [119] [Simple Token-Efficient Vision-Language Model for Case-level Pathology Synoptic Report Generation](https://arxiv.org/abs/2605.30716)
...
Importance: 普通相关 (56.7) - 未入选精读

TL;DR: 本文提出了一种简单、token 高效的视觉语言模型，用于从全切片图像（WSI）生成病例级病理报告。通过使用 5× 放大倍率的 512×512 块将序列长度减少最多 64 倍，结合冻结的病理编码器、轻量 MLP 对齐器和 LLM 解码器，仅需半张 H100 GPU 即可训练，在 ROUGE-L/METEOR/BLEU-4 上取得高分，并在 AI 评估中优于强基线。


<details>
  <summary>Details</summary>
Motivation: 从全切片图像（WSI）生成临床可用的病理报告极具挑战性，因为 WSI 具有千兆像素分辨率、长视觉 token 序列，且一个病例可能包含多个 WSI，涉及异质组织和模糊发现。现有方法计算开销大，难以在有限 GPU 内存下实用。

Method: 采用三组件架构：冻结的病理 patch 编码器、轻量两层 MLP 视觉语言对齐器、大语言模型解码器，并引入显式的 WSI 标记 token 来分隔病例内的不同切片。训练分两阶段监督微调：(1) 对齐器 WSI 字幕生成（使用异质 WSI-文本对）；(2) 病例级监督微调（使用病例-报告对生成结构化报告）。使用 5× 放大倍率的 512×512 patch，将平均序列长度减少最多 64 倍。

Result: 在 ROUGE-L/METEOR/BLEU-4 指标上取得高分，内存和运行时间显著更高效（仅需半张 H100 GPU）。AI 评估中模型一致优于强基线。消融实验刻画了性能-效率权衡，并识别出提升多 WSI 场景鲁棒性的简单选择。

Conclusion: 该工作为高效病理报告生成提供了一个强且可复现的基线，降低了在有限计算资源下进行多 WSI VLM 研究的门槛。
...
Importance reason: Focuses on pathology report generation, which is outside the user's priority subtopics and negative-keyword domain. Signals: pathology report generation, token-efficient VLM, frozen encoder + MLP aligner, low GPU memory training, case-level multi-WSI reasoning
...
TL;DR: 本文提出 SCALE 框架，通过 Selector、Predictor、Judger 三个对抗角色实现 Web Agent 的自主认知探索与自我改进，并引入 SCALE-Hop 图探索策略避免局部陷阱，构建 SCALE-20k 数据集，显著提升了多种 MLLM 在真实网页环境中的表现与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有 Web Agent 依赖手工设计的执行流程或昂贵的专家轨迹，难以适应复杂动态的网页环境，缺乏自主发现自身局限并持续改进的能力。

Method: 提出 SCALE 框架，包含三个对抗角色：Selector 选择待探索的子任务，Predictor 预测执行结果，Judger 评估预测与实际的差异以发现认知盲区。同时提出 SCALE-Hop 图探索策略，将任务分解为子任务图，通过全局规划避免局部探索陷阱。基于探索轨迹自动构建 SCALE-20k 数据集用于后续微调。

Result: 在多个真实网页环境上的实验表明，SCALE 显著提升了多种 MLLM（如 GPT-4V、LLaVA 等）在 Web Agent 任务上的表现和泛化能力，优于现有基线方法。

Conclusion: SCALE 提供了一种可扩展、可泛化的自主 Web Agent 构建方案，通过认知感知的探索与自我改进，有效提升了 MLLM 在复杂动态环境中的适应能力。

Classification reason: 该论文提出基于 MLLM 的自主 Web Agent（SCALE），通过自认知探索提升智能体在复杂网页环境中的适应性和泛化能力。核心是 MLLM 驱动的智能体系统，属于通用视觉语言模型与 MLLM 的应用范畴，因此映射到 canonical subtopic "视觉语言模型与 MLLM"。

Importance reason: Focuses on web agent self-improvement, not on core priority subtopics like continuous multimodal models or video generation. Signals: MLLM web agent, self-improvement, cognitive exploration, graph exploration, SCALE-20k dataset
...
### [121] [DiTTo: Scalable Order-aware All-in-One Image Restoration Agent](https://arxiv.org/abs/2605.30915)
...
TL;DR: DiTTo 提出了一种可扩展的、顺序感知的图像恢复智能体框架，通过 DiTTo 模拟器高效构建最优恢复动作轨迹数据集，并利用顺序感知恢复对齐（ORA）实现即插即用的专家扩展，在多重退化图像恢复任务上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 真实图像往往同时存在多种退化（如模糊、噪声、低光照等），且去除退化的顺序对最终恢复质量有显著影响。现有基于智能体的图像恢复方法需要 O((N^D)^2) 次专家调用构建训练数据，且智能体与固定专家池耦合，无法灵活扩展新的恢复专家。

Method: 提出 DiTTo 框架，包含两个组件：(1) DiTTo 模拟器，结合 ∪S-IR（单步恢复动作模拟）和 AiO-IQA（每动作质量预测），将 ORTD 构建复杂度降至 O(N^D)；(2) DiTTo 智能体，基于 VLM 进行 SFT 训练，再通过顺序感知恢复对齐（ORA）在退化识别、恢复动作排序和输出格式三个独立轴向上对齐，支持即插即用地添加新恢复专家。

Result: 在 MiO-100 评测集（最多五种并发退化）上，DiTTo 智能体在多重退化图像恢复质量上超越了所有现有基于智能体的图像恢复方法。

Conclusion: DiTTo 通过高效的模拟器构建训练数据和顺序感知对齐训练，解决了现有智能体图像恢复方法在效率和可扩展性上的瓶颈，实现了可即插即用的专家扩展，为实际多退化图像恢复提供了更实用的解决方案。

Classification reason: 该论文提出 DiTTo，一个基于视觉语言模型（VLM）的图像恢复智能体框架，使用 VLM 调度多个恢复专家并按序去除退化。这属于多模态理解与生成方向下的通用视觉语言模型（VLM/MLLM）应用，因此映射到 canonical subtopic "视觉语言模型与 MLLM"。

Importance reason: Proposes scalable order-aware agent for multi-degradation image restoration, but lacks direct connection to user's priority subtopics like continuous models or video generation. Signals: agent-based image restoration, order-aware restoration alignment, plug-and-play expert extension, vision-language model scheduling, multi-degradation SOTA
...
### [122] ["Intelegi Româneşte?'' A Recipe for Romanian Vision-Language Models](https://arxiv.org/abs/2605.31401)
...
TL;DR: 本文系统研究了为罗马尼亚语构建视觉语言模型（VLM）的完整流程，包括机器翻译英文VLM训练/评测数据、消融视觉骨干和语言骨干的贡献、以及构建文化原生评测集HoraVQA。罗马尼亚语适配的VLM在各项评测中一致超越同尺寸模型，甚至超过更大尺寸的模型。


<details>
  <summary>Details</summary>
Motivation: 现有VLM主要面向英语，在低资源语言上性能急剧下降，缺乏大规模图像-文本语料库和文化相关的评测基准。本文旨在系统研究如何为低资源语言（罗马尼亚语）构建高性能VLM。

Method: (1) 将英文VLM训练和评测语料通过机器翻译转换为罗马尼亚语，包括文本标注和图像内文字；(2) 消融实验分析视觉骨干（不同规模和预训练）、语言骨干（多语言 vs 罗马尼亚语适配LLM）和OCR风格图像-文本数据的贡献；(3) 构建HoraVQA——基于罗马尼亚日常场景的文化原生评测集。

Result: 罗马尼亚语适配的VLM在各项评测中一致优于同尺寸对照模型，甚至超越更大尺寸的模型。消融实验揭示了视觉骨干规模、语言骨干选择以及OCR数据对低资源语言VLM性能的影响。

Conclusion: 通过机器翻译构建训练数据和适配语言骨干，可以有效为低资源语言构建高性能VLM；文化原生评测集对于准确评估低资源语言VLM至关重要。

Classification reason: 该论文系统研究了为低资源语言（罗马尼亚语）构建视觉语言模型（VLM）的完整流程，包括数据构建、架构选择与评测。这属于通用视觉语言模型（VLM/MLLM）的构建与适配工作，因此映射到 canonical subtopic "vlm_mllm_general"（视觉语言模型与 MLLM）。

Importance reason: Focuses on low-resource language adaptation, not on core continuous multimodal or autoregressive generation priorities. Signals: Low-resource VLM, language-specific adaptation, machine translation pipeline, cultural evaluation, not continuous/autoregressive/world model
...
### [123] [Generating Reports or Repeating Templates? Measuring and Mitigating Template Collapse in 3D CT Report Generation](https://arxiv.org/abs/2605.30984)
...
TL;DR: 本文发现3D医学CT报告生成中的"模板坍塌"现象——模型生成流畅但病理检测能力低、输出多样性差的模板化报告。提出CLarGen框架，将"说什么"（临床检测）与"怎么说"（语言合成）解耦，通过潜查询变换器进行多标签病理检测、病理引导检索和医学语言模型合成报告，显著提升临床准确性（macro-F1从0.189提升至0.487）。


<details>
  <summary>Details</summary>
Motivation: 现有3D医学视觉语言模型在CT报告生成中虽然能生成流畅的放射学风格文本，但存在病理检测能力极低、输出多样性差的问题，倾向于生成忽略罕见但关键发现的通用模板化报告。这种"模板坍塌"源于3D医学影像的独特约束：数据有限、标签严重不平衡、体素编码器信号弱，导致文本生成目标鼓励捷径学习和弱语义对齐的报告生成。

Method: 提出CLarGen解耦框架，包含三个组件：(i) Latent Query Transformer（LQT）用于多标签病理检测，从3D体素特征中检测临床发现；(ii) 病理引导检索模块，根据检测到的病理检索临床匹配的示例报告；(iii) 医学语言模型，将检测到的发现与检索到的上下文结合，合成最终报告。该框架将"检测什么"与"如何表达"分离，避免文本生成目标干扰病理检测。

Result: 在多个SOTA 3D CT报告生成基线上，CLarGen有效缓解了模板坍塌，显著提升临床准确性：macro-F1从0.189提升至0.487，CRG从0.368提升至0.472，同时保持报告流畅性。

Conclusion: 明确的、可测量的临床语义基础是构建抗模板坍塌的3D CT报告生成系统的关键。解耦检测与生成的设计范式能有效克服医学影像报告生成中的数据稀缺和标签不平衡问题。

Classification reason: 该论文研究3D医学CT报告生成的视觉语言模型（VLM），属于多模态理解与生成方向。论文聚焦于VLM在医学影像报告生成中的模板坍塌问题，属于通用视觉语言模型的应用与改进，因此映射到[CANONICAL] subtopic "vlm_mllm_general"（视觉语言模型与MLLM）。

Importance reason: Addresses a critical failure mode in medical VLMs but is outside the user's core focus on continuous multimodal models and video generation. Signals: Template collapse diagnosis, Decoupled detection-synthesis framework, Clinical accuracy improvement, 3D CT report generation, Medical VLM robustness
...
### [124] [Task-Focused Memorization for Multimodal Agents](https://arxiv.org/abs/2605.31075)
...
TL;DR: 本文提出 TaskMem，一种基于强化学习的任务聚焦记忆策略学习框架，使多模态智能体能够从无限的多模态观测流中选择性记忆与任务相关的内容。采用两阶段训练范式：第一阶段学习如何记忆（优化记忆质量），第二阶段学习记忆什么（通过环境任务定义奖励模型微调适配器）。在 VideoMME、EgoLife、EgoTempo 的流式评测基准上，基于 Qwen3-VL-30B-A3B 的 TaskMem 将 VQA 准确率分别提升 6.3%、7.0% 和 5.3%。


<details>
  <summary>Details</summary>
Motivation: 多模态智能体（如具身智能体）持续感知、推理和行动，接收无限的多模态观测流。现有长期记忆研究主要关注记忆模块设计和基本准确性/保真度，但关键挑战在于确定"记忆什么"——智能体需要从信息组合爆炸中有选择地保留与角色相关且对未来任务有价值的内容。本文旨在将记忆生成建模为可学习的记忆化策略，使智能体能够动态调整记忆焦点以适应环境中的真实任务需求。

Method: TaskMem 采用两阶段训练范式：第一阶段（Phase One）通过优化记忆质量（在基本保真度要求下）学习如何记忆；第二阶段（Phase Two）在部署后，智能体通过微调基础 MLLM 上的适配器来学习记忆什么，利用近期环境任务定义奖励模型，引导记忆化策略聚焦于任务相关的内容。为评估该方法，作者将 VideoMME、EgoLife、EgoTempo 重构为流式评测基准，模拟智能体处理流式观测并在线处理任务的真实场景，且问题必须仅通过智能体记忆回答（无法访问原始视频）。

Result: 基于 Qwen3-VL-30B-A3B，TaskMem 在 VideoMME、EgoLife、EgoTempo 三个流式基准上分别将 VQA 准确率提升 6.3%、7.0% 和 5.3%。

Conclusion: 本文证明了将记忆生成建模为可学习的记忆化策略的有效性，通过强化学习框架 TaskMem 使多模态智能体能够动态聚焦于任务相关内容的记忆。两阶段训练范式（先学如何记忆，再学记忆什么）为多模态智能体的长期记忆构建提供了新思路，在多个流式评测基准上取得了显著提升。

Classification reason: 该论文研究多模态智能体的长期记忆策略，基于 MLLM（Qwen3-VL）构建记忆生成与任务聚焦策略，属于多模态理解与生成方向下的通用视觉语言模型与 MLLM 范畴。虽然涉及记忆与强化学习，但核心是 MLLM 的记忆增强与任务适配，因此映射到 [CANONICAL] subtopic "vlm_mllm_general"。

Importance reason: Focuses on memory policy for multimodal agents, not on core continuous/multimodal generation or world model priorities. Signals: Multimodal agent memory, RL-based memorization, streaming VQA benchmarks, not autoregressive generation, not continuous language model
...
TL;DR: FOCUS 提出了一种两阶段训练框架，通过显式优化支持样本边界框与查询图像之间的上下文注意力，并结合 GRPO 强化学习直接最小化定位误差，实现了无需类别监督的类别无关上下文目标定位。7B 模型性能超越 72B 模型，证明了上下文感知定位目标可以超越模型规模扩展。


<details>
  <summary>Details</summary>
Motivation: 现有上下文定位方法依赖显式类别监督，这不仅限制了在无名称或实例特定物体场景下的应用，还引入了类别偏差，使模型倾向于语义先验而非视觉证据。因此，需要一种无需类别监督、真正基于视觉对应关系的上下文定位方法。

Method: 提出两阶段训练框架：第一阶段通过显式优化支持样本边界框与查询图像之间的上下文注意力（in-context attention），建立视觉对应关系；第二阶段使用 GRPO（Group Relative Policy Optimization）强化学习直接最小化定位误差，进一步细化定位结果。整个训练过程无需类别监督。

Result: 7B 参数模型在多个基准上超越了包括 72B 在内的更大模型，消融实验验证了各组件（上下文注意力优化和 GRPO 强化学习）的贡献，证明了上下文感知定位目标的有效性。

Conclusion: 通过显式优化上下文注意力和强化学习直接优化定位误差，可以在无需类别监督的情况下实现鲁棒的实例级上下文定位，且上下文感知定位目标可以超越模型规模扩展带来的收益。

Classification reason: 该论文研究视觉语言模型（VLM）中的上下文定位（in-context localization）能力，属于通用视觉语言模型与MLLM架构的范畴，因此映射到[CANONICAL] subtopic "vlm_mllm_general"（视觉语言模型与MLLM）。

Importance reason: Strong in-context localization method with RL optimization, relevant to multimodal understanding and efficient inference. Signals: In-context localization, GRPO reinforcement learning, category-agnostic, outperforms larger models, multimodal understanding generation
...
### [126] [Detect in Any Scene: An Agentic Framework for Object Detection with Experience-Aware Reasoning](https://arxiv.org/abs/2605.31174)
...
TL;DR: 本文提出 DetAS，一个基于 MLLM 智能体的目标检测框架，将检测建模为动态决策过程，通过自适应图像恢复和多专家检测器组合来应对真实场景中的图像退化与目标分布不均问题，并引入经验积累机制 DetAS-X 实现经验感知推理，在六个基准上平均 F1 提升 28.36%。


<details>
  <summary>Details</summary>
Motivation: 现有目标检测器在真实场景中因图像退化（如低光照、模糊）和目标分布多样而泛化能力不足。传统方法依赖预定义条件且缺乏动态适应性，亟需一种能自适应组合恢复模块和检测器的灵活框架。

Method: DetAS 框架包含两个核心组件：(1) 自适应图像恢复模块，动态判断是否需要以及如何增强图像；(2) 多专家检测模块，集成多个领域专用检测器并通过实例级推理融合预测。DetAS-X 进一步引入自进化经验收集机制，从少量标注数据中积累节点级决策经验，在推理时实现经验感知推理，逐步优化决策策略。

Result: 在六个具有挑战性的基准上，DetAS-X 相比现有 MLLM 检测器平均 F1 提升 28.36%，在 DarkFace 上提升高达 37.01%。

Conclusion: 实验结果表明智能体式检测框架在复杂动态环境中具有显著优势，为自适应目标检测提供了新的范式基础。

Classification reason: 该论文提出 DetAS，利用 MLLM 作为中央智能体进行目标检测的决策与工作流编排，属于通用视觉语言模型与 MLLM 的应用范畴，因此映射到 [CANONICAL] 子主题 "视觉语言模型与 MLLM" (vlm_mllm_general)。

Importance reason: Novel agentic MLLM framework for object detection, but not aligned with user's priority subtopics on continuous models, video/image generation, or world models. Signals: MLLM agent, object detection, image restoration, multi-expert detection, experience-aware reasoning
...
### [127] [Personalize Your Large Vision-language Models With In-context Prompt Tuning](https://arxiv.org/abs/2605.31513)
...
TL;DR: 本文提出 In-Context Prompt Tuning (ICPT)，一种面向大型视觉语言模型（LVLM）的个性化方法。ICPT 使用轻量投影模块从多张参考图像中提取细粒度视觉语义，将其与身份标签映射为连续提示，并自适应确定提示长度。同时引入两种几何正则化方法，解耦关键身份与背景环境、分离不同概念以避免语义混淆。实验表明 ICPT 在多种任务和 LVLM 骨干上达到 SOTA 个性化精度。


<details>
  <summary>Details</summary>
Motivation: 现有 LVLM 个性化方法依赖推理时训练，效率低，且在复杂多图像、多概念场景下难以保持准确性，限制了 LVLM 系统的广泛部署。

Method: 提出 ICPT，包含：(1) 轻量投影模块，从多张参考图像中提取细粒度视觉语义，并与身份标签映射为连续提示；(2) 自适应提示长度机制，根据每个概念的视觉复杂度动态确定提示长度；(3) 两种几何正则化约束——解耦关键身份与瞬态环境状态、分离不同概念以避免语义混淆。

Result: 在多种个性化任务和不同 LVLM 骨干（如 LLaVA）上，ICPT 达到了 SOTA 的个性化精度，显著优于现有方法。

Conclusion: ICPT 通过轻量投影模块和几何正则化，实现了高效、准确的 LVLM 个性化，在复杂多概念场景下表现优异，为 LVLM 的个性化部署提供了有效方案。

Classification reason: 该论文提出 ICPT（In-Context Prompt Tuning），通过轻量投影模块将参考图像中的细粒度视觉语义转化为连续提示，实现 LVLM 的个性化。属于通用视觉语言模型（VLM/MLLM）的个性化微调方法，与 [CANONICAL] subtopic "视觉语言模型与 MLLM" 高度匹配。

Importance reason: Strongly aligns with user's high-weighted continuous multimodal and MLLM subtopics, but focuses on personalization rather than core continuous representation or generation. Signals: multimodal large language model, in-context prompt tuning, visual-language personalization, lightweight projection, geometric regularization
...
### [128] [MechVQA: Benchmarking and Enhancing Multimodal LLMs on Comprehensive Mechanical Drawing Understanding](https://arxiv.org/abs/2605.30794)
...
TL;DR: 本文提出了 MechVQA，首个面向机械工程图纸理解的多模态大语言模型评测数据集，包含3.3k高密度图纸和21K问答对，覆盖识别、推理、判断三个能力层级的10种细粒度任务。基于该数据集，作者进一步开发了 MechVL 模型，通过多阶段训练范式在机械图纸理解上超越最强闭源基线7.57个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 在通用 VQA 任务上表现优异，但在机械工程图纸理解上仍然脆弱——高标注密度、弱领域知识、严格的投影规则和几何约束下的空间关系推理不可靠，导致模型容易遗漏关键线索并给出错误答案。目前缺乏专门针对机械图纸理解的评测基准。

Method: (1) 构建 MechVQA 数据集：通过半自动构建和质量控制流程，包含3.3k高密度图纸和21K问答对，覆盖识别、推理、判断三个能力层级的10种细粒度任务。(2) 开发 MechVL 模型：采用多阶段训练范式（预训练+指令微调），在机械图纸数据上进行领域专业化训练。

Result: MechVL 在 MechVQA 总分上超越最强闭源基线（GPT-4o等）7.57个百分点，显著提升了机械图纸理解能力。

Conclusion: MechVQA 为机械图纸理解提供了首个综合性评测基准，MechVL 展示了领域专业化训练的有效性，为在机械设计和检测场景中部署 MLLM 提供了可复用的基础。

Classification reason: 该论文提出了 MechVQA，一个面向机械工程图纸理解的多模态问答数据集，并构建了 MechVL 模型。这属于视觉问答与文档理解（特别是专业领域文档/图纸理解）范畴，与 [CANONICAL] subtopic "视觉问答与文档理解"（multimodal_vqa_doc）高度匹配。

Importance reason: Specialized domain benchmark for mechanical drawings, but outside user's core focus on continuous multimodal models and video generation. Signals: domain-specific benchmark, multimodal understanding, mechanical engineering, weak on user priority subtopics, no autoregressive or video generation
...
### [129] [Astra: a generalizable report generation foundation model for 3D computed tomography](https://arxiv.org/abs/2605.31437)
...
TL;DR: Astra是一个基于90,678对胸腹部CT-报告数据训练的CT报告生成基础模型，通过报告风格统一和强化学习诊断一致性优化，在六个外部队列上实现了SOTA性能，平均细粒度诊断指标提升44.1%，并在临床工作流中加速胸部报告起草29.6%、提升腹部报告完整性11.3%。


<details>
  <summary>Details</summary>
Motivation: CT报告生成需要放射科医生逐层阅片，耗时且高度依赖专家经验。现有方法缺乏支持多区域报告且对外部真实世界队列鲁棒的可泛化CT报告生成基础模型，跨队列的报告风格和诊断术语不一致导致简单联合训练受限于噪声文本监督。

Method: Astra首先构建了包含90,678对胸腹部CT-报告的大规模数据集CTRgDB，涵盖353,671个异常发现和八个器官系统。通过报告风格统一（harmonizing report style）解决跨队列文本风格不一致问题，再通过强化学习进一步优化诊断一致性，实现风格一致且诊断准确的报告生成。

Result: 在CTRgDB和六个外部队列上，Astra达到SOTA性能，平均细粒度诊断指标提升44.1%（P<0.001）。临床工作流中，Astra辅助使胸部报告起草速度提升29.6%，腹部报告完整性提升11.3%（P<0.001）。此外，Astra作为CT AI开发的基础模型，通过高质量报告合成提升了下游诊断性能和视觉语言预训练效果。

Conclusion: Astra是一个可泛化的CT报告生成基础模型，在多个解剖区域和机构间实现了风格一致且诊断准确的报告生成，可作为广泛可及的临床助手和下一代AI医疗基础设施。

Classification reason: 该论文研究CT影像报告自动生成，属于医学视觉问答与文档理解范畴（将3D CT影像转化为结构化文本报告），与[CANONICAL] subtopic "视觉问答与文档理解"（multimodal_vqa_doc）高度匹配。

Importance reason: Focuses on medical report generation, which is outside the user's priority subtopics and contains negative keywords like clinical. Signals: CT report generation, clinical workflow, medical domain, negative keyword clinical, no overlap with priority subtopics
...
### [130] [Recognizing Co-Speech Gestures in-the-Wild](https://arxiv.org/abs/2605.31589)
...
TL;DR: 本文提出了GRW（Gesture Recognition in the Wild）数据集，包含156,688个手动标注的视频片段，覆盖150个词汇的手势语义标注。基于该数据集训练视频模型，实现三个任务：语义手势分类、手势对应词汇识别、手势时序定位。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型难以捕捉与语音语义相关的伴随手势（co-speech gestures），主要瓶颈在于缺乏精确标注的训练数据。现有数据集规模小、词汇覆盖有限，无法支撑细粒度的语义手势识别研究。

Method: 构建GRW数据集：从大规模视频中收集并手动标注156,688个视频片段，每个片段标注手势对应的具体词汇（共150个词汇类别）及帧级时间边界。基于该数据集训练视频模型，分别用于：(a) 二分类判断手势是否为语义手势；(b) 多分类识别手势对应的词汇；(c) 时序定位手势起止帧。

Result: 建立了三个任务的基准评测结果，展示了GRW数据集在规模、多样性和标注精度上的优势，验证了基于该数据集训练的模型能够有效识别语义伴随手势。

Conclusion: GRW数据集填补了语义伴随手势识别领域大规模精确标注数据的空白，为多模态模型理解人类非语言交流提供了重要资源，有望推动手势理解与视觉-语言对齐研究。

Classification reason: 该论文研究自然场景中伴随语音的手势识别，属于多模态理解范畴。虽然现有[CANONICAL]中没有专门的手势识别子主题，但"视觉问答与文档理解"（multimodal_vqa_doc）是最接近的现有子主题，因为该论文的核心任务是识别手势对应的语义词汇并进行时序定位，本质上是一种细粒度的视觉-语言理解任务，与VQA/文档理解同属多模态理解与问答范畴。

Importance reason: Focuses on gesture recognition, which is outside your priority subtopics and lacks boost keywords. Signals: New dataset for co-speech gesture recognition, addresses a gap in multimodal understanding, no direct link to continuous language models, video generation, or world models
...
### [131] [Towards Effective Long-Video Event Prediction via Multi-Level Event Semantics Mining](https://arxiv.org/abs/2605.31069)
...
Method: VISTA 框架包含三个层级：(1) 字符级视觉提示（character-centric visual prompt）精确提取事件相关视觉细节，增强细节级语义；(2) 知识增强迭代检索策略（knowledge-enhanced iterative retrieval）引导 LLM 逐步构建逻辑连贯的事件链，提升事件级叙事；(3) 类人提议-检索策略（propose-then-retrieve）生成多样化的未来提议并整合多层级线索，产生鲁棒准确的预测。
...
Importance reason: Focuses on long-video event prediction, which is not a priority subtopic and lacks direct alignment with user's key interests in generation, world models, or continuous representations. Signals: long-video event prediction, multi-level semantics mining, character-centric visual prompt, knowledge-enhanced retrieval, propose-then-retrieve strategy
...
### [132] [Polyphony: Diffusion-based Dual-Hand Action Segmentation with Alternating Vision Transformer and Semantic Conditioning](https://arxiv.org/abs/2605.31115)
...
TL;DR: Polyphony 提出了一种三阶段方法用于双手动作分割：交替训练的双手视觉 Transformer 平衡双手梯度贡献、语义特征条件化增强细粒度动作区分、以及扩散分割模型实现双手协调。在 HA-ViD、ATTACH 和 Breakfast 数据集上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 双手动作分割面临双手依赖复杂、视觉不对称、主导手梯度垄断以及细粒度动作语义模糊等挑战，现有方法难以有效处理这些问题。

Method: (1) Alternating Dual-Hand Vision Transformer：交替训练左右手 mini-batch 以平衡梯度贡献，共享时空编码器；(2) Semantic Feature Conditioning：将视觉特征与结构化组合动作描述对齐，增强语义相似动作的区分；(3) Diffusion-Based Segmentation：引入跨手特征融合实现双手协调，并采用自适应损失加权平衡性能。

Result: 在 HA-ViD 和 ATTACH 双手数据集上达到 SOTA，提升最高达 16.8 个点；在单流 Breakfast 数据集上达到 82.5%，超越使用 12 倍更大骨干网络的先前最佳方法。

Conclusion: Polyphony 通过交替训练、语义条件化和扩散分割有效解决了双手动作分割中的关键挑战，且统一单骨干模型优于需要分离双手模型的基线方法。

Classification reason: 该论文研究双手动作分割（dual-hand action segmentation），属于视频理解中的时序密集预测任务，与 [CANONICAL] subtopic "视频理解与时序推理"（video_understanding_temporal）高度匹配。

Importance reason: Focuses on dual-hand action segmentation, which is not aligned with user's priority subtopics on generation, world models, or continuous representations. Signals: video understanding, action segmentation, diffusion-based segmentation, dual-hand interaction, semantic conditioning
...
### [133] [HiERO-StepG @ Ego4D Step Grounding Challenge: hierarchical activity understanding enables zero-shot step grounding](https://arxiv.org/abs/2605.31227)
...
TL;DR: 本文提出 HiERO-StepG，一种零样本步骤定位方法，利用弱监督层次化表征学习（HiERO）从无标注的第一视角视频中自动发现程序性活动的层次结构，通过聚类检测步骤并施加时序单调性约束，在 Ego4D Step Grounding Challenge 上以零样本方式取得第二名（R@1 IoU=0.3 达 56.27%）。


<details>
  <summary>Details</summary>
Motivation: 传统的步骤定位方法需要大量细粒度标注，扩展性差。本文认为程序性活动天然具有层次结构，且这种结构可以从无标注视频中通过共现动作模式自发涌现，因此希望实现无需任务特定标注的零样本步骤定位。

Method: 基于 HiERO 的弱监督表征学习框架，利用细粒度动作级叙述将功能相关的动作在特征空间中拉近；然后通过简单聚类检测步骤边界；进一步引入粗细粒度步骤分配一致性约束、严格时序单调性约束以及后处理去噪来提升定位精度。

Result: 在 Ego4D Step Grounding Challenge 上以零样本方式取得 R@1 (IoU=0.3) 56.27%，排名第二，且无需任何程序特定标注。

Conclusion: 层次化活动理解可以有效支持零样本步骤定位，弱监督表征学习结合简单聚类和时序约束即可达到有竞争力性能，为减少标注依赖提供了可行方向。

Classification reason: 该论文关注第一视角视频中的步骤定位（step grounding），属于视频理解与时序推理范畴，与 canonical subtopic "视频理解与时序推理" 高度匹配。

Importance reason: Focuses on zero-shot step grounding in procedural video, which is not aligned with user's priority on generative and continuous multimodal models. Signals: zero-shot step grounding, hierarchical activity understanding, Ego4D challenge, weak supervision, temporal monotonicity
...
### [134] [EGOSTREAM: A Diagnostic Benchmark for Streaming Episodic Memory in Egocentric Vision](https://arxiv.org/abs/2605.31557)
...
TL;DR: EGOSTREAM 是一个面向第一视角视频流式情节记忆的诊断性评测基准，包含 2,250 个问题，覆盖 7 个认知维度，并引入 Answer Validity Window (AVW) 概念以区分模型遗忘与自然世界状态变化。在统一 Qwen3-VL 骨干上评测多种记忆管理机制，发现不同方法在准确率相近时记忆特征差异显著，所有方法均远低于实时处理速度，最高准确率仅约 45%。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频评测基准缺乏对模型"记住了什么、记住了多久"的诊断能力，无法区分模型遗忘与场景自然变化导致的答案失效，亟需一个细粒度的诊断性评测工具。

Method: 构建 EGOSTREAM 基准：从 Ego4D 等数据集中提取第一视角视频片段，设计 2,250 个问题覆盖细节、空间、时序、事件、社交、因果和前瞻 7 种记忆维度；提出 Answer Validity Window (AVW) 概念定义答案的有效时间窗口，扩展出 8,528 个召回条件评测样本。在统一 Qwen3-VL 框架下集成滑动窗口、attention sink、KV-cache 剪枝/合并/卸载等多种记忆管理机制进行系统评测。

Result: 不同记忆管理机制在 aggregate 准确率相近时表现出截然不同的记忆特征：token 剪枝比 token 合并更好地保留细粒度细节和时序结构，量化卸载可改善超长期召回。所有方法处理速度均低于实时（>1s/帧），最优方法准确率仅约 45%。

Conclusion: EGOSTREAM 揭示了当前流式 MLLM 在情节记忆方面的关键缺陷，为未来改进提供了细粒度的诊断工具和评测框架。
...
### [135] [Linear Scaling Video VLMs for Long Video Understanding](https://arxiv.org/abs/2605.31598)
...
TL;DR: 本文提出 StateKV，一种无需微调或架构修改的推理时方法，通过固定容量的重要性驱动循环状态跨帧传递上下文，将预训练长视频 VLM 的视频预填充复杂度从二次降至线性，在多个长视频基准和模型上接近全自注意力性能，并优于滑动窗口等流式近似方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频 VLM 的编码器依赖时空自注意力，计算和延迟随帧数二次增长，难以扩展到长视频和流式场景。已有的效率方法（如帧/ token 丢弃、粗粒度注意力近似）虽提升可扩展性但损失精度。因此需要一种既能保持精度又能实现线性时间视频预填充的方法。

Method: StateKV 是一种推理时方法，核心思想是将跨帧上下文压缩到一个固定容量的、基于重要性的循环状态（recurrent state）中，用于视频预填充阶段，同时保留第二个完整的逐帧缓存用于解码阶段。该方法无需微调或修改架构，可直接适配预训练的长视频 VLM。

Result: 在三个长视频基准上，对三个模型家族（多种规模）共七个模型进行实验，StateKV 精度接近全自注意力，且一致优于主流的滑动窗口/基于近期性的流式近似方法。同时，StateKV 降低了视频预填充的计算成本（FLOPs），使得在固定计算预算下可以运行更大模型以获得更强精度。

Conclusion: StateKV 提供了一种实用的、无需训练的推理时方法，使预训练长视频 VLM 能够实现线性时间视频预填充，在保持精度的同时显著提升可扩展性，为长视频理解的实际部署迈出了可行的一步。

Classification reason: 该论文聚焦长视频理解中 VLM 的推理效率问题，属于视频理解与时序推理（video_understanding_temporal）这一规范子主题。论文提出的 StateKV 方法针对长视频 VLM 的预填充阶段进行线性时间优化，核心场景是长视频理解，因此映射到该子主题。

Importance reason: Directly addresses efficient inference for long video understanding, a key priority in model efficiency and video world models. Signals: linear scaling video VLM, inference-time efficiency, long video understanding, StateKV, no fine-tuning needed
