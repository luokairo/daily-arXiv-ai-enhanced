...
- [世界模型](#direction-world_model) [Total: 14]
  - [具身世界模型与规划控制](#direction-world_model-embodied_world_model) [Total: 6]
...
### [1] [MUX: Continuous Reasoning via Multiplexed Tokens](https://arxiv.org/abs/2607.18264)
...
Importance: 值得关注 (75.05) - 未入选精读

TL;DR: MUX 提出一种将离散推理步骤无损压缩为连续多路复用 token 的方法，通过在潜空间中用加权线性叠加表示多个子词，实现高带宽、紧凑的连续推理，并在 32 个评估设置中优于现有潜空间推理基线。


<details>
  <summary>Details</summary>
Motivation: 语言模型通过自然语言中间推理步骤解决问题虽然有效，但计算效率低：每个推理步骤只传递一个子词，大量 token 用于表达思想而非执行计算。作者希望突破这一瓶颈，实现高带宽、紧凑的推理。

Method: MUX 将离散推理步骤蒸馏为连续潜空间中的多路复用（multiplexed）token。每个潜 token 训练为表示一段离散推理子词的加权线性叠加（superposition），该叠加通过构造是无损的，且可通过解复用（demultiplexing）完全恢复。作者证明简单的位置相关权重（如几何衰减）可支持无损多路复用，防止潜空间坍塌导致的捷径行为。MUX 还能在需要搜索的问题中实现并行探索。

Result: 在 32 个评估设置、4 种语言模型上，MUX 优于强潜空间推理基线。消融和探针分析表明，学习到的潜 token 编码了忠实且可解释的推理过程。

Conclusion: 无损叠加作为局部学习目标构成了实现强且高效的潜空间连续推理的充分条件。MUX 为高效连续推理提供了简单有效的方法。

Classification reason: 该论文提出 MUX，将离散推理步骤蒸馏为连续潜空间中的多路复用 token，实现高带宽紧凑推理。这属于连续潜空间中的推理方法，与 [CANONICAL] subtopic "连续多模态推理与潜空间思维" 高度匹配。同时论文也涉及语言模型推理，可归入多模态理解与生成方向下的同一 subtopic。

Importance reason: Directly advances continuous latent reasoning with lossless multiplexing, a core priority for your continuous language/multimodal model focus. Signals: continuous latent reasoning, lossless multiplexing, high-bandwidth compact reasoning, outperforms latent baselines, parallel exploration in search
...
### [2] [Beyond Fixed Goal Delivery: Online POMDP Planning for Target Interception in Crowds](https://arxiv.org/abs/2607.18517)
...
TL;DR: 本文研究在拥挤人群中拦截移动目标的机器人规划问题，将问题建模为POMDP并通过在线树搜索求解。对比了顺序路径-速度规划器与联合转向-速度规划器，发现在高密度人群中联合规划器显著优于顺序规划器。


<details>
  <summary>Details</summary>
Motivation: 在拥挤环境中拦截移动目标（如追赶一个人）需要机器人在多个不确定的人类智能体间导航，而人类导航意图不可直接观测，机器人必须推理多种可能的未来交互结果。现有方法缺乏对动作空间结构如何影响搜索树和计算资源分配的深入分析。

Method: 将拦截问题建模为部分可观测马尔可夫决策过程（POMDP），使用在线树搜索在固定计算预算下求解。设计了两种规划器进行对比：(1) 顺序路径-速度规划器：先规划空间路径，再沿路径调节速度；(2) 联合转向-速度规划器：在树搜索中同时分支转向和速度决策。

Result: 在多达200人的仿真环境中，低密度人群下两种方法表现相似；高密度人群下，顺序规划器的安全拦截率比联合规划器低31个百分点，且所需时间多44%。揭示了空间限制的结构性缺陷。

Conclusion: 在拥挤环境中，联合转向-速度规划器比顺序路径-速度规划器更有效，尤其是在高密度人群场景下。动作空间的结构设计对POMDP在线规划的性能有显著影响。

Classification reason: 该论文研究在拥挤人群中拦截移动目标的机器人规划问题，将问题建模为部分可观测马尔可夫决策过程（POMDP），并通过在线树搜索求解。这属于机器人动作规划与控制范畴，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on robotics planning in crowds, which is outside the user's core interests in continuous multimodal models and generative video/world models. Signals: POMDP planning, target interception, crowds, robotics, sequential vs unified planner
...
### [3] [RoboInter1.5: A Holistic Intermediate Representation Suite for Embodied World Modeling and Robotic Manipulation](https://arxiv.org/abs/2607.18709)
...
TL;DR: RoboInter1.5 是一个面向机器人操作和具身世界建模的中间表征套件，包含 23 万+操作片段、密集逐帧标注（10+种中间表征）、VQA 基准、VLA 模型以及基于中间表征的可控世界模型（RoboInter-World），用于统一具身智能中的推理、执行和物理世界模拟。


<details>
  <summary>Details</summary>
Motivation: 现有机器人数据集成本高、具身特异性强、缺乏细粒度标注，难以支持泛化推理、执行和长时序环境动力学模拟。作者希望构建一个统一的中间表征资源，将中间表征作为双向接口，既能规约低层动作空间，又能约束开放世界物理模拟器的潜空间推演。

Method: RoboInter1.5 包含四个核心组件：(1) RoboInter-Data：571 个场景、23 万+操作片段，逐帧标注 subtask、primitive skill、物体/夹爪定位、分割、affordance、抓取姿态、接触点、运动轨迹等 10+种中间表征；(2) RoboInter-VQA：基于上述标注构建时空具身 VQA 任务，用于评测和提升中间表征推理能力；(3) RoboInter-VLA：研究隐式、显式和模块化 plan-then-execute 三种范式下中间表征对动作执行的促进作用；(4) RoboInter-World：利用中间表征作为结构化条件信号，实现可控的未来世界状态预测。

Result: 大量实验表明 RoboInter1.5 提供了统一的时空中间表征框架，中间表征作为双向接口既能规约低层动作空间，又能约束开放世界物理模拟器的潜空间推演，在推理、执行和世界建模任务上均取得良好效果。

Conclusion: RoboInter1.5 验证了密集中间表征在具身世界建模和机器人操作中的核心价值，为未来构建更通用、可泛化的具身智能系统提供了统一的数据、基准和模型基础。

Classification reason: 该论文聚焦机器人操作与具身世界建模，提出包含中间表征（subtask、primitive skill、grasp pose等）的统一数据、基准和模型套件，并利用这些表征进行动作执行和未来世界状态预测，属于具身世界模型与规划控制方向，因此映射到[CANONICAL] subtopic "embodied_world_model"。

Importance reason: Strongly aligns with world model and unified multimodal priorities, with intermediate representations bridging reasoning and physical simulation. Signals: world model, intermediate representation, embodied world modeling, robotic manipulation, VLA
...
Importance: 普通相关 (61.99) - 未入选精读

TL;DR: 本文提出"公司世界模型"(Company World Model)概念，将世界模型框架应用于AI驱动的生物技术公司组织设计，通过45个回顾性决策案例的干实验基准测试，证明以资产到价值的状态表示为核心的原生AI架构优于模仿人类部门结构的组织方式。


<details>
  <summary>Details</summary>
Motivation: 当前AI原生生物技术公司往往简单复制人类生物技术公司的组织架构图到智能体角色中，作者认为这种设计缺乏对AI系统本质优势的利用，需要一种更根本的抽象来重新思考AI组织的运作方式。

Method: 提出Company World Model框架，定义为持久的资产到价值状态表示(asset-to-value state representation)，包含转移模型、显式价值函数、规划和更新机制，覆盖科学、监管、商务、商业、财务和执行约束。构建包含45个回顾性公共信息决策案例的干实验基准，设置严格时间截断、隐藏结果、通用模式、自动评分和盲审对比。比较了四种架构：人类组织模仿、增强版人类组织模仿、AI原生资产中心、AI原生价值转换架构（后者是Company World Model的提示级近似，包含Live Asset Value Record和Deal/Approval/Revenue/Investment Arbiter循环）。

Result: 价值转换架构在外部BD、监管批准与上市、收入纪律定义的成功函数下取得了最高的自动价值转换分数，并被价值特定盲审评委强烈偏好。压力测试显示更强的人类基线仍具竞争力，中性评委未显示稳健的价值转换优势。Codex-only机制消融实验表明Revenue Room、Deal Room和Approval Room在目标目标下承担了有用工作。

Conclusion: 核心发现是目标敏感的：部门结构可能仍是有用的治理视图，但AI原生操作原语应是一个共享的、预测性的资产到价值状态，而非静态的人类组织架构图。研究仅为干实验，未建立真实药物成功、临床效益或收入预测准确性。

Classification reason: 该论文提出"Company World Model"概念，将世界模型思想应用于AI驱动的生物技术公司组织架构设计，包含状态表示、转移模型、价值函数、规划与更新等核心世界模型组件。虽然应用领域是药物研发而非传统机器人/具身智能，但其核心抽象（asset-to-value state representation with transition models, planning, updating）完全符合世界模型的定义框架。映射到[CANONICAL]具身世界模型与规划控制，因为该子主题涵盖最广泛的世界模型应用，且论文中的"规划控制"体现在Deal/Approval/Revenue/Investment Arbiter loops的决策循环中。

Importance reason: The paper applies world models to biotech company design, which is far outside the user's core focus on continuous multimodal models and video/image generation. Signals: world model applied to organizational design, dry-lab benchmark for AI-native biotechs, objective-sensitive results, no real-world drug success validation, tangential to user's priority subtopics
...
TL;DR: DWM 提出一种监督级分解框架，将潜空间世界模型的下一潜态预测显式分解为动作驱动分量和动作不变的世界效应分量，通过辅助世界头、归一化世界对比目标和正交约束实现解耦，在三个标准控制基准的 W 变体上平均提升 13.1% 的 CEM 规划成功率。


<details>
  <summary>Details</summary>
Motivation: 现有动作条件潜世界模型将下一潜态预测视为单一无差别的监督目标，迫使模型用一个整体学习信号吸收所有状态变化来源。然而真实世界的状态变化来自两个异质来源：智能体驱动的动作分量和动作不变的世界效应（如重力、惯性、接触反弹等环境固有动力学）。将两者融合为一个目标会纠缠两种因素，阻碍模型将观测变化归因到其根本原因，并损害所学动力学的可迁移性。

Method: DWM 在潜世界模型的预测器中增加一个辅助世界头（world head），通过归一化世界对比目标（normalized world-contrastive objective）正则化使其对动作不变；同时原始预测头（pred head）通过正交约束与世界头耦合，两者共同将预测的潜态转移显式分解为动作不变分量和互补的动作驱动分量，且不改变底层架构或推理流程。为评估持久世界效应，构造了 PushT-W、Reacher-W 和 TwoRoom-W 三个标准控制基准的 W 变体。

Result: DWM 在平坦基准上匹配强基线，在三个 W 变体上平均提升 13.1% 的 CEM 规划成功率。

Conclusion: DWM 通过显式分解动作驱动和动作不变的世界效应，有效提升了潜世界模型在持久世界效应下的规划性能，验证了分离两种状态变化来源对模型可解释性和迁移性的价值。

Classification reason: 该论文提出分解世界模型（DWM），将潜空间世界模型的预测分解为动作驱动分量和动作不变的世界效应分量，属于世界模型中的具身规划与控制方向，与[具身世界模型与规划控制]（embodied_world_model）高度匹配。

Importance reason: Decomposes latent world model transitions into action-driven and world-effect components, improving planning success by 13.1% on new benchmarks. Signals: world model decomposition, latent transition disentanglement, action-invariant dynamics, CEM planning improvement, new W-benchmarks
...
### [6] [Athena-Brain Technical Report: An Efficient Robot Brain for General Intelligence and Embodied Interactio](https://arxiv.org/abs/2607.18985)
...
TL;DR: Athena-Brain-8B 是一个 8B 参数的紧凑型 LLM，通过多阶段后训练（通用SFT→通用RL→具身专家训练→模型合并）在保持通用语言与推理能力的同时，获得高效的高层具身交互能力，在具身评测中超越同规模模型及部分更大规模前沿模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么侧重通用智能，要么侧重具身能力，难以在单一紧凑模型中同时满足两者需求。随着具身智能体能力增强，需要一种既能保留 LLM 广泛通用智能、又能高效与具身环境交互的端侧大脑模型。

Method: 采用多阶段后训练流水线：(1) 通用监督微调（General SFT）提升基础能力；(2) 通用强化学习（General RL）进一步对齐通用能力；(3) 具身专家训练（Embodied Expert Training）注入高层具身交互能力；(4) 模型合并（Model Merge）融合各阶段优势。最终得到 8B 参数的 Athena-Brain-8B 模型。

Result: 在通用语言和推理基准上与 Qwen3-8B thinking 模型表现相当，但生成响应显著更短。在领域内具身评测中，Athena-Brain-8B 持续优于同规模模型，并在零样本设置下超越多个更大的前沿模型。

Conclusion: 紧凑型语言模型能够有效整合强大的通用智能与具身能力，多阶段后训练策略是实现这一目标的有效途径。

Classification reason: 该论文提出 Athena-Brain-8B，一个面向具身智能的紧凑型 LLM 大脑，通过多阶段后训练（通用SFT、通用RL、具身专家训练、模型合并）在保持通用智能的同时获得高层具身交互能力。这属于世界模型方向下的具身世界模型与规划控制（embodied_world_model）这一[CANONICAL]子主题。

Importance reason: Strongly relevant to world model and embodied interaction, but lacks direct focus on priority subtopics like continuous language models or video generation. Signals: embodied world model, compact LLM for robot brain, multi-stage post-training, general intelligence + embodied, model merge
...
### [7] [Planning as Emergent Behavior in Reinforcement Learning with Relational Hidden States](https://arxiv.org/abs/2607.18589)
*Armin Sommer*
...
TL;DR: 本文发现，在无模型强化学习中，使用关系隐状态（relational hidden states）的神经网络架构能够自发涌现出规划行为。这些隐状态通过学习到的关系恢复环境的转移结构，并在决策时基于学习到的图结构进行规划，从而提升策略性能。对照实验表明，没有这种隐状态绑定的智能体无法涌现规划。论文认为这解释了无模型RL中涌现规划的机制，并推测人脑中可能也存在类似的神经架构先验。


<details>
  <summary>Details</summary>
Motivation: 无模型强化学习中有时会观察到规划行为的涌现，但其背后的条件与机制尚不明确。本文旨在揭示这一现象的决定性因素。

Method: 设计了一种使用关系隐状态的神经网络架构，每个隐状态锚定到一个环境状态，并通过学习到的关系在隐状态间交换消息。在网格世界等环境中训练该智能体，并与一个需要自行发现状态-隐状态映射的对照智能体进行比较。

Result: 关系隐状态智能体成功恢复了环境的转移结构（学习到的关系图与真实转移图一致），并在决策时利用该图进行规划，显著提升了策略性能。对照智能体未能涌现规划行为。

Conclusion: 关系隐状态的架构先验是无模型RL中涌现规划的关键因素。该发现揭示了规划可以从纯奖励最大化目标中通过特定的神经架构设计自发产生，并可能为理解人脑中的规划机制提供启示。

Classification reason: 论文研究强化学习中基于关系隐状态涌现出的规划行为，其核心是智能体在无显式世界模型的情况下，通过隐状态结构恢复环境转移结构并进行规划。这与世界模型方向中的"具身世界模型与规划控制"高度相关，因为该子主题涵盖规划、控制以及隐式/显式世界建模。论文虽不直接涉及机器人，但其机制（隐状态恢复环境转移结构并用于规划）属于世界模型与规划控制的范畴。

Importance reason: Explores emergent planning in model-free RL via relational hidden states, but is tangential to user's focus on continuous multimodal models and video generation. Signals: emergent planning, relational hidden states, model-free RL, world model, neural architecture prior
...
TL;DR: Agentic Real2Sim 是一个基于视觉语言智能体的框架，能够将真实世界的物体-机器人交互记录自动转换为可运行的物理仿真环境（episodic twin），涵盖刚体操作、可变形物体交互和人形运动等多种场景，为机器人策略学习与评估提供真实对齐的仿真环境。


<details>
  <summary>Details</summary>
Motivation: 当前的 Real2Sim（真实到仿真）转换过程高度依赖人工操作，包括视觉基础模型的调参、网格清理、坐标对齐以及不同视觉工具和仿真器之间的脆弱工作流拼接。这导致将真实世界交互转换为可运行物理仿真的过程耗时且难以规模化。作者希望构建一个自动化框架，将真实记录转换为物理一致的仿真环境，以支持下游机器人策略学习与评估。

Method: 提出 Agentic Real2Sim 框架，利用视觉语言智能体（VLM）驱动自动化决策。框架接收真实世界物体-机器人交互视频，通过 VLM 智能体协调多个视觉感知工具（如分割、姿态估计、网格重建等），自动恢复场景几何、物体状态、物理参数，并组装演员、物体、相机、位姿和轨迹，最终生成可运行的物理仿真环境（episodic twin）。框架支持使用开源 VLM 后端，以较低成本达到与前沿模型相当的转换成功率。

Result: 在刚体操作、可变形物体交互和人形运动三种场景上进行了评估，覆盖了通常需要不同 Real2Sim 管线的领域。实验表明，使用开源 VLM 后端（如 LLaVA）可以达到与前沿商业模型（如 GPT-4V）相当的转换成功率，同时成本大幅降低。

Conclusion: Agentic Real2Sim 首次展示了通过 VLM 智能体实现跨多种场景的通用化 Real2Sim 转换，为可扩展的物理世界建模迈出了第一步。生成的真实对齐仿真环境可用于下游机器人策略学习与评估，具有重要的实际应用价值。

Classification reason: 该论文提出 Agentic Real2Sim 框架，将真实世界的物体-机器人交互记录转换为可运行的物理仿真（episodic twin），涉及场景几何恢复、物理参数推断、可交互模拟器构建等，属于世界模型方向下的"可交互与物理世界建模"（interactive_physical_world）这一 CANONICAL 子主题。

Importance reason: Bridges real-world robot interaction to physics simulation via VLM agents, aligning with world model and unified multimodal priorities. Signals: world model, vision-language agent, physics simulation, real-to-sim, robotic interaction
...
### [9] [Generative World Renderer at the Speed of Play](https://arxiv.org/abs/2607.18703)
...
TL;DR: AlayaRenderer-Flash 是一个实时生成式世界渲染器，通过将原始 AlayaRenderer 重构为少步自回归流式模型并引入轻量蒸馏编解码器，将推理速度从 0.56 FPS 提升至 31.54 FPS，实现了与物理引擎联动的可交互生成式世界。


<details>
  <summary>Details</summary>
Motivation: 原始 AlayaRenderer 虽然能从物理引擎导出的结构化世界状态（G-buffer）合成 RGB 帧，保持场景结构且不改变底层世界动力学，但其计算开销过大（0.56 FPS），无法满足实时部署需求。本文旨在将其加速到可交互游玩的速度。

Method: 将原始渲染器重构为少步自回归流式模型（few-step autoregressive streaming model），引入轻量蒸馏编解码器（lightweight distilled codecs）用于高效潜编码和帧重建，保留教师模型的 G-buffer 和文本提示接口，支持对无限长度输入流的连续渲染。

Result: 推理速度从 0.56 FPS 提升至 31.54 FPS，在内容保持、时序一致性、跨窗口稳定性、提示可控性和运行效率方面均保持核心渲染能力，与物理引擎集成后实现了 30 FPS 的完全可游玩生成式世界。

Conclusion: AlayaRenderer-Flash 证明了通过模型架构优化（少步自回归流式+轻量蒸馏编解码器）可以大幅降低生成式世界渲染的计算成本，实现实时交互式世界建模，为可交互生成式世界模拟器提供了可行的技术路径。

Classification reason: 该论文提出 AlayaRenderer-Flash，一个实时生成式前向世界渲染器，将物理引擎导出的结构化世界状态（G-buffer）流实时合成为 RGB 帧，构建了可交互、可游玩的生成式世界。这属于可交互与物理世界建模（interactive_physical_world）的典型工作，同时其生成渲染部分也涉及多模态生成方向。

Importance reason: Directly advances interactive world modeling with real-time generative rendering, aligning with top-priority world model and video generation directions. Signals: world model, real-time generative rendering, autoregressive streaming, interactive play, physics engine integration
...
TL;DR: IGGT4D是一个流式4D实例感知几何Transformer，通过因果时空建模逐步更新相机运动、几何和物体身份的统一表示，实现长序列动态场景的在线4D重建与实例级理解，并构建了大规模4D实例标注数据集InsScene4D-147K。


<details>
  <summary>Details</summary>
Motivation: 现有流式3D重建方法仅关注几何，缺乏时序一致的物体级理解；而语义重建和3D VLM方法依赖外部2D语义线索或松散耦合的几何输入，难以在长动态场景中实现统一的几何-实例联合学习。需要一种能在线处理视频流、同时保持几何与实例一致性的4D场景理解方法。

Method: IGGT4D采用流式Transformer架构，逐帧处理视频序列，通过因果时空建模复用历史上下文，增量更新相机运动、几何和物体身份的统一表示。同时构建InsScene4D-147K大规模数据集，包含真实/合成、静态/动态场景，通过自动几何引导标注管线生成RGB、深度、位姿和时序一致的实例掩码。

Result: 在3D重建、位姿估计、实例空间跟踪和开放词汇分割等任务上，IGGT4D优于现有流式基线方法，并能对长动态序列进行可扩展的在线推理。

Conclusion: IGGT4D证明了流式4D实例感知建模的可行性，通过统一的几何-实例表示实现了动态场景的在线理解与重建，为空间智能体在连续视频流中的场景理解提供了新范式。

Classification reason: 论文提出流式4D实例感知几何Transformer，用于在线4D场景理解与重建，属于动态场景中的世界建模，与[空间3D/4D世界模型]高度匹配。同时涉及4D场景生成与重建，也可归入[4D与动态场景生成]。

Importance reason: Strongly aligns with world model and continuous multimodal priorities via streaming 4D instance-grounded geometry Transformer for dynamic scene understanding. Signals: streaming 4D reconstruction, instance-grounded geometry, causal spatial-temporal modeling, world model, continuous video understanding
...
TL;DR: 本文提出 E2E-CDiff，一个端到端条件扩散框架，用于生成真实且可控的闭环交通场景。该框架以前视视觉观测为条件，联合去噪未来运动状态和可执行的低层控制信号，通过可微引导实现速度、可行驶区域和碰撞行为的控制，在真实性与可控性之间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的交通场景生成方法难以同时兼顾可控性和真实性：要么缺乏细粒度控制，要么以牺牲行为合理性为代价换取可控性。此外，传统两阶段"轨迹-控制器"管线存在规划与控制不匹配的问题。

Method: 提出 E2E-CDiff 端到端条件扩散框架，以前视视觉观测为条件，联合去噪未来运动状态（轨迹）和低层控制信号（油门、刹车、转向）。通过可微引导机制分别调控速度、可行驶区域合规性以及碰撞避免/碰撞寻求行为，实现自然和安全关键场景的统一生成。

Result: 在 Bench2Drive 数据集上，E2E-CDiff 相比强化学习和模仿学习基线实现了更好的可控性-真实性权衡；碰撞引导变体能在多个自动驾驶系统中引发具有挑战性的交互；作为基于学习的自车规划器也表现出竞争力。

Conclusion: 端到端状态-动作扩散生成是平衡交通场景真实性与可控性的有效范式，可微引导机制为安全关键场景生成提供了灵活手段，同时该框架作为自车规划器也展现了良好的通用性。

Classification reason: 该论文提出端到端条件扩散框架用于交通场景生成，联合去噪未来运动状态和低层控制信号，属于动作条件视频生成与未来预测，与 [CANONICAL] 视频世界模型 (video_world_model) 高度匹配。

Importance reason: Focuses on traffic scenario generation for autonomous driving, which is a niche application of world models, not directly aligned with your priority subtopics on continuous multimodal models or video generation. Signals: world model application, autonomous driving, conditional diffusion, controllable generation, not aligned with priority subtopics
...
### [12] [AlayaWorld: Interactive Long-Horizon World Modeling -- Full Technical Report](https://arxiv.org/abs/2607.18367)
...
TL;DR: AlayaWorld 是一个交互式长时序视频世界模型，基于 15B 视频扩散 Transformer，通过自回归生成短潜变量块（chunk）实现 24fps、540p/720p 视频生成。它采用有界视觉上下文（持久 sink 帧、压缩时序历史、几何对齐空间记忆、最近帧条件）来维持时空一致性，并通过损坏历史训练和自滚动预测残差减少长期漂移。同时引入离散自回归蒸馏公式（结合分布匹配蒸馏、self-forcing++ 和一致性蒸馏），将每块采样从约 30 步降至 4 步。在 iWorld-Bench 上取得最佳长时序生成性能。


<details>
  <summary>Details</summary>
Motivation: 传统视频游戏开发依赖劳动密集型的资产制作、动画、物理和编程流程，而视频世界模型旨在从用户输入（文本、图像或视频）即时生成可交互、可探索、持续演化的虚拟世界。实现这一愿景需要交互性、持久时空一致性、稳定长时序生成和高效响应四个紧密耦合的能力，现有方法在这四方面难以兼顾。

Method: AlayaWorld 构建于 15B 参数的视频扩散 Transformer 之上，采用自回归方式逐块（chunk）生成短潜变量视频片段。其有界视觉上下文包含四个组件：持久 sink 帧（persistent sink frame）保持全局场景一致性、压缩时序历史（compressed temporal history）传递长期信息、几何对齐空间记忆（geometry-aligned spatial memory）维护空间结构、最近帧条件（recent-frame conditioning）提供局部连续性。为减少长期漂移，模型使用自身 rollout 收集的损坏历史和预测残差进行训练。此外，提出离散自回归蒸馏公式，融合分布匹配蒸馏（DMD）、self-forcing++ 和一致性蒸馏，将每块推理从约 30 步降至 4 步。

Result: 在 iWorld-Bench 基准上，AlayaWorld 在长时序视频生成任务中取得了最佳性能。通过蒸馏方法，推理效率大幅提升（从 30 步降至 4 步每块），同时保持了生成质量。

Conclusion: AlayaWorld 作为一个全栈、开源、长期项目，为交互式视频世界模型的未来研究提供了可扩展的基础。其有界视觉上下文设计和离散自回归蒸馏框架有效解决了长时序生成中的时空一致性和效率问题。

Classification reason: 该论文提出 AlayaWorld，一个交互式长时序视频世界模型，通过自回归生成短潜变量块实现 24fps 视频生成，属于视频世界模型（video_world_model）这一 [CANONICAL] 子主题的核心范畴。
...
### [13] [Masked Visual Actions for Unified World Modeling](https://arxiv.org/abs/2607.19343)
...
TL;DR: 本文提出 Masked Visual Actions（MVA），一种像素空间的控制接口，通过在视频中部分揭示任意实体的轨迹来表达动作，将预训练视频模型微调为统一的世界模型。仅需15小时微调，单个模型即可实现前向动力学预测、逆动力学建模、策略评估和基于模型的规划。


<details>
  <summary>Details</summary>
Motivation: 视频模型蕴含丰富的视觉世界交互先验，但如何将动作信息以与视觉空间对齐的形式传递给这些模型，同时保持与物理操作的关联，是一个核心挑战。现有方法通常将动作编码为低维向量或文本指令，与视频模型学习的视觉交互先验存在语义鸿沟。

Method: 提出 Masked Visual Actions（MVA），将动作表示为视频中任意实体轨迹的部分遮挡（masked trajectory）。通过揭示机器人运动轨迹，模型作为前向动力学模型预测场景对低层机器人动作的响应；通过揭示目标物体运动轨迹，同一模型可恢复与之一致的机器人行为。使用真实视频和仿真数据中的15小时遮挡示例进行微调。

Result: 单个检查点在多种场景和多种具身形态上实现了强视觉保真度和可控性。在下游操作任务中，模型生成的想象 rollout 与真实执行结果相关，可用于策略评估；通过排序候选未来进行基于模型的规划可提升决策质量；支持从目标物体运动合成机器人动作的逆建模。

Conclusion: Masked Visual Actions 提供了一种简单而有效的像素空间动作接口，使视频模型能够统一地建模前向动力学和逆动力学，为机器人世界建模提供了一种新的范式。

Classification reason: 该论文提出 Masked Visual Actions，通过像素空间的动作表达（部分遮挡的轨迹）将视频模型转化为世界模型，实现前向动力学预测和逆动力学建模。这属于视频世界模型（video_world_model）的典型工作，且该 subtopic 是 [CANONICAL] 条目，因此直接复用。

Importance reason: Directly advances video world models for robotics with a novel pixel-space action interface, aligning with top priority subtopics. Signals: world model, video generation, unified multimodal, efficient fine-tuning, autoregressive video generation
...
### [14] [NaviAIS: A Scenario-Level Vessel Trajectory Prediction Dataset withVectorized Lane Priors and the NaviLane Forecasting Framework](https://arxiv.org/abs/2607.18887)
...
TL;DR: 本文提出NaviAIS，一个标准化的场景级船舶轨迹预测数据集，提供矢量化航道先验、可航行地图等结构化信息。基于该数据集，提出NaviLane框架，采用层次化宏动作编码、轨迹-地图联合编码、离散宏动作码本生成多模态候选轨迹，并通过基于世界模型的风险评估器对候选进行排序。实验表明NaviLane在单模态和多模态设置下均优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有AIS船舶轨迹数据集存在采样率不一致、噪声大、坐标系不统一、缺乏结构化航道表示等问题，限制了可复现的、环境感知的轨迹预测研究。亟需一个标准化、场景级、包含航道先验信息的数据集和相应的预测框架。

Method: 提出NaviAIS数据集，包含统一时间窗口和局部坐标系下的多船历史-未来轨迹，并提供栅格化可航行地图、矢量化航道先验、航道图等结构化表示。基于此提出NaviLane框架：(1) 轨迹-地图联合编码模块统一场景表示；(2) 离散宏动作码本生成粗到细的多模态候选轨迹；(3) 残差精化模块提升局部几何与动力学一致性；(4) 基于世界模型的风险感知评估器，根据交互风险和环境影响对候选排序。

Result: 在NaviAIS数据集上，NaviLane在单模态和多模态轨迹预测设置下均优于代表性基线方法，验证了结构化航道先验、层次化多模态生成和基于世界模型的风险评估的有效性。

Conclusion: NaviAIS数据集填补了船舶轨迹预测领域缺乏结构化航道先验的空白，NaviLane框架证明了融合航道先验、层次化生成与世界模型评估的有效性，为海事自主导航和环境感知轨迹预测提供了新范式。

Classification reason: 该论文提出船舶轨迹预测数据集NaviAIS和预测框架NaviLane，其中NaviLane包含世界模型（world-model-based consequence-aware evaluator）用于评估预测轨迹的风险与可行性，属于世界模型方向中的视频/轨迹未来预测。匹配[世界模型]方向下的[视频世界模型]（video_world_model）子主题，该子主题涵盖未来预测、动作条件生成等。

Importance reason: Focuses on vessel trajectory prediction, which is outside the user's core interests in continuous multimodal models and video generation. Signals: world model for risk evaluation, hierarchical macro-action framework, trajectory-map joint encoding, discrete macro-action codebook, multimodal candidate generation
...
### [15] [ABot-World-0: Infinite Interactive World Rollout on a Single Desktop GPU](https://arxiv.org/abs/2607.19191)
...
TL;DR: ABot-World-0 是一个动作条件视频世界模型，可在单张桌面级 GPU（RTX 5090）上以高达 16 FPS 实时流式生成 720P 视频，支持长时序闭环交互。通过多源数据基础设施（AAA 游戏、模拟引擎、互联网视频）、双向动作条件教师蒸馏、LongForcing 对齐策略以及流式推理栈（轻量 VAE 解码器、高效注意力、低比特 DiT 推理），实现了可控的长时间世界演化与身份一致性保持。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型在长时序闭环交互中面临分布偏移和自回归漂移问题，且通常需要大量计算资源，难以在单桌面级 GPU 上实时运行。作者希望构建一个高效、可控、支持长时间交互的视频世界模型，能够在消费级硬件上实现实时交互式世界模拟。

Method: 1) 构建多源数据基础设施（WorldExplorer），从 AAA 游戏、模拟引擎和互联网视频中收集动作条件视频数据，经过 14 项确定性质量检查、VLM 评估和同步动作/文本标注。2) 提出渐进式蒸馏策略：将双向动作条件教师模型通过 teacher forcing 和 ODE 蒸馏逐步蒸馏为因果学生模型。3) 提出 LongForcing 方法，将学生长自回归 rollout 与扩展视野的教师模型对齐，缓解累积分布偏移。4) 使用原始键盘动作作为统一控制接口，支持场景漫游和第三人称角色交互；引入参考角色记忆保持身份一致性。5) 设计流式推理栈，包括轻量 VAE 解码器、高效注意力、内存感知调度和低比特 DiT 推理。

Result: 在低比特配置下，ABot-World-0 在单张 RTX 5090 上以高达 16 FPS 流式生成 720P 视频，动作到首帧延迟为 1.2 秒，峰值显存约 19GiB。在 WorldRoamBench 和扩展交互 rollout 上展示了有竞争力的可控性和连贯的长时序世界演化。

Conclusion: ABot-World-0 证明了在单桌面级 GPU 上实现实时、长时序、闭环交互的视频世界模型是可行的，为消费级硬件上的交互式世界模拟提供了有效方案。

Classification reason: 该论文提出 ABot-World-0，一个动作条件视频世界模型，支持实时、长时序闭环交互，属于视频世界模型（video_world_model）的典型工作。论文核心贡献包括动作条件视频生成、长时序 rollout、ODE 蒸馏等，与 [CANONICAL] video_world_model 的定义高度吻合。

Importance reason: Directly addresses the user's high-priority world model and video generation subtopics with a practical, real-time interactive system. Signals: world model, video generation, real-time interaction, single GPU deployment, action-conditioned
...
TL;DR: 本文提出一种模型无关的即插即用方法，利用预训练感知模型提取的语义和几何信号来增强单视图3D物体重建，在多个SOTA重建管道上取得一致改进。


<details>
  <summary>Details</summary>
Motivation: 人类视觉中物体感知与重建之间存在紧密联系，但计算机视觉中对此探索不足。作者希望利用学习到的物体感知信号来提升单视图3D重建的质量。

Method: 从预训练感知模型中提取语义和几何信息作为感知信号，以即插即用的方式注入到现有的单视图3D重建方法中，驱动物体从单张图像重建为3D形状。该方法不依赖特定重建架构，可集成到多种重建管道。

Result: 在两个SOTA单视图3D重建管道上的基准数据集实验中，该方法取得了持续且显著的改进，验证了将感知融入生成的有效性。

Conclusion: 学习到的物体感知信号能有效增强单视图3D重建，该方法具有模型无关性和通用性，可广泛适用于现有重建框架。

Classification reason: 论文研究单视图3D物体重建，属于3D生成与重建范畴，与[CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: The paper's focus on 3D reconstruction is not a priority subtopic, and it lacks direct connection to the user's high-weight continuous language/multimodal or video generation interests. Signals: 3D reconstruction, single-view, object perception, plug-and-play, model-agnostic
...
TL;DR: Fluid-SDF 提出一种超轻量、可编辑的隐式形状表示方法，使用可微CSG基元（球体、立方体等）通过平滑最小值函数混合，以不到100个参数实现复杂非凸形状的重建，并支持零样本编辑和抗噪声。


<details>
  <summary>Details</summary>
Motivation: 传统隐式神经表示（INR）如MLP-based SDF存在参数量大、不可编辑、易受噪声影响等问题，难以部署在边缘设备上。本文旨在设计一种超轻量、可编辑且鲁棒的隐式形状表示方法。

Method: 提出 Fluid-SDF 框架，用一组可微的显式几何基元（球体、立方体、圆柱体等）替代MLP，通过平滑最小值函数（smooth minimum）混合这些基元来建模复杂非凸形状。每个基元由少量参数控制（位置、尺寸、旋转等），整体参数严格控制在100个以内。训练时通过可微渲染进行端到端优化。

Result: 在多个2D形状数据集上，Fluid-SDF 以不到100个参数达到了与标准神经INR相当或更优的mIoU；在噪声数据上表现出强大的抗过拟合能力；支持零样本局部和全局形状编辑（如移动、缩放、旋转基元），无需重新训练。

Conclusion: Fluid-SDF 证明了显式可微基元可以替代MLP实现高效、可编辑的隐式形状表示，特别适合移动AI、增强现实和资源受限的嵌入式环境。

Classification reason: 该论文提出一种基于可微CSG基元的隐式形状表示方法，属于3D形状表示与重建范畴，与[CANONICAL] subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Focuses on 3D shape representation with CSG primitives, which is outside the user's priority subtopics of continuous language/video/image generation and world models. Signals: 3D implicit representation, ultra-lightweight (<100 params), zero-shot editing, noise robustness, not aligned with priority subtopics
...
### [18] [Latent Riemannian Flow Matching for Geometry-Grounded 3D Foundation Models](https://arxiv.org/abs/2607.19120)
...
Importance: 高优先级 (89.84) - 未入选精读

TL;DR: 本文提出 Latent Riemannian Flow Matching，在 VGGT 几何基础模型的潜空间（高维超球面乘积流形）上直接进行黎曼流匹配，实现从无位姿图像生成连贯的 3D 场景几何，无需依赖显式的下游表示（如高斯体、网格或视频 VAE 潜变量）。


<details>
  <summary>Details</summary>
Motivation: 几何基础模型（如 VGGT）能从无位姿图像中提供强 3D 先验，但仅能前馈确定性预测，无法生成输入视图未直接支持的几何。而生成式 3D 场景模型需要强几何先验才能从稀疏输入中产生连贯输出。现有方法将生成模型与几何先验结合时，往往依赖显式表示（如 3DGS、NeRF），限制了灵活性。作者希望直接在几何基础模型的潜空间中进行生成，同时保持潜空间的几何结构（超球面乘积流形），从而桥接这两种范式。

Method: 提出 Latent Riemannian Flow Matching (LRFM) 框架：1) 将 VGGT 的多尺度编码器输出的 token 对齐到四个超球面的乘积流形上；2) 在该黎曼流形上定义流匹配过程，确保生成的 token 始终位于解码器所需的合法数据流形上；3) 使用黎曼流匹配的目标函数进行训练，在潜空间中从噪声向数据分布流动；4) 冻结 VGGT 的解码头，仅训练潜空间中的流匹配模块。

Result: 在 RealEstate10K、ScanNet++ 和 ETH3D 数据集上，该方法在逐视角外观和聚合 3D 几何质量方面均优于最近的场景生成基线方法，证明了在几何基础模型潜空间中进行流匹配作为 3D 生成范式的可行性。

Conclusion: 本文展示了在几何基础模型的潜空间中直接进行黎曼流匹配是一种有效的 3D 场景生成范式，能够利用预训练模型的强几何先验，同时保持生成能力，且不依赖特定的显式下游表示。

Classification reason: 该论文在 VGGT 的潜空间中进行流匹配以生成 3D 场景几何，属于 3D 场景生成方法，因此映射到 multimodal_generation 方向下的 [CANONICAL] subtopic "3D 生成与重建"。同时，该方法在连续潜空间中进行流匹配，与 continuous_language_multimodal 方向下的连续潜空间流匹配相关，故也匹配该方向。

Importance reason: Novel latent Riemannian flow matching on geometric foundation models for 3D generation, directly relevant to continuous multimodal representation and flow matching. Signals: flow matching, continuous latent representation, 3D generation, geometric foundation model, Riemannian manifold
...
### [19] [AniGS: Bridging Rendering and Diffusion Prior for 3D Scene Animation](https://arxiv.org/abs/2607.18539)
...
TL;DR: AniGS 提出一种针对大规模 3DGS 重建场景的动画方法，利用预训练视频扩散模型和迭代数据集-模型更新策略，为场景添加自然的植被等环境动态运动，同时保持刚性结构不变，在五个真实户外场景上生成了高质量的新视角视频。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 场景重建（如 3DGS）虽然能生成逼真的新视角渲染，但重建结果通常是静态的，缺乏环境中的细微动态运动（如风吹草动），导致沉浸感不足。已有的 3D 动画技术局限于物体级或小区域，无法处理大规模、杂乱、可导航的场景。

Method: AniGS 使用规范 3DGS 表示场景，并引入时间条件变形场来建模运动。核心创新包括：(1) 利用预训练视频扩散模型作为运动先验；(2) 提出迭代数据集-模型更新策略，逐步扩展视角覆盖范围，并通过渲染-精炼循环反复更新相机固定的训练视频；(3) 引入组合式视频到视频精炼方案，将运动限制在期望区域（如植被），防止静态区域出现伪影。

Result: 在五个真实大规模户外场景上的实验表明，AniGS 能够生成自然的场景级环境动态运动和高品质的新视角视频，显著提升了重建环境的沉浸式观看体验。

Conclusion: AniGS 成功弥合了渲染与扩散先验之间的差距，实现了大规模 3D 场景的动画化，为沉浸式环境体验提供了有效方案。

Classification reason: 该论文对 3DGS 重建场景进行动画化，添加植被等环境动态运动，属于动态 3D 场景生成范畴，与 [CANONICAL] subtopic "4D 与动态场景生成" (dynamic_4d_generation) 高度匹配。

Importance reason: Novel 3D scene animation method using video diffusion priors, but focuses on static-to-dynamic conversion rather than core continuous multimodal or autoregressive generation. Signals: 3DGS animation, video diffusion prior, scene-level dynamics, outdoor scenes, render-and-refine
...
### [20] [DeforM: Reasoning-Guided Physics-Aware Video Generation via Spatial-Temporal Masking](https://arxiv.org/abs/2607.18664)
...
TL;DR: DeforM 是一个推理引导的物理感知图像到视频生成框架，通过 VLM 驱动的物理推理模块定位动态关键区域并生成时空掩码，引导模型关注物理关键区域，从而生成更真实、物理一致的变形动态视频。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型虽然视觉质量高，但难以生成物理感知的视频，尤其是复杂的变形动力学（如物体形变）。模型注意力被无关区域稀释，导致物理动态生成失败。因此需要一种方法引导模型聚焦于物理关键区域。

Method: 提出 DeforM 框架，包含两个核心模块：(1) DeforM-Reason：基于 VLM 的物理推理模块，识别目标物体并生成时空掩码（spatial-temporal masks），定位物理关键区域；(2) 两种引导策略：DeforM-Free（无需训练，用于机制分析）和 DeforM-Injection（基于训练的强大生成器），将掩码信息注入视频生成过程以引导模型关注动态区域。

Result: 实验结果表明，DeforM 在生成变形场景的真实感方面优于基线模型，在视觉质量和物理一致性两个维度均取得更好表现。

Conclusion: 通过 VLM 引导的物理推理和时空掩码机制，可以有效提升视频生成模型对复杂变形动力学的建模能力，为物理感知视频生成提供了新思路。

Classification reason: 该论文提出基于推理引导的图像到视频生成框架，通过VLM物理推理生成时空掩码来引导视频生成中的物理动态区域。这属于可控视频生成/编辑范畴，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度相关，因此复用该分类。

Importance reason: Strongly aligns with user's priority on continuous multimodal models and world models via VLM-guided physical reasoning and spatial-temporal masking for video generation. Signals: VLM-guided physical reasoning, spatial-temporal masking, physics-aware video generation, deformation dynamics, image-to-video generation
...
### [21] [Image Editing Models are Numerical Solvers](https://arxiv.org/abs/2607.18787)
*Ulysse Mizrahi*
...
TL;DR: 本文探索了将预训练的生成式图像编辑模型作为通用数值模拟求解器的可行性。物理输入和求解结果以图像形式呈现，标量参数通过轻量适配器注入。使用相同的架构和训练协议，在多种物理方程（椭圆方程、热方程、Burgers方程、Navier-Stokes、势流、弹性力学、程函方程、相场断裂、熵最优输运等）上进行实验，结果表明图像编辑模型能够表示多种静态和时变物理映射，包括不稳定和激波行为。但该方法也存在数值范围选择、方程约束施加困难等根本性限制，且对混沌系统的长时程模拟失败。


<details>
  <summary>Details</summary>
Motivation: 探索预训练的生成式图像编辑模型是否可以作为数值模拟的通用接口，利用其强大的视觉表征能力来求解多种物理方程，从而为科学计算提供一种新的范式。

Method: 将物理输入和求解结果渲染为图像，标量物理参数（如材料属性、扩散率、载荷参数）通过轻量适配器注入预训练的图像编辑模型。使用相同的架构和训练协议，以传统数值求解器和解析解作为监督信号，训练模型学习从输入图像到输出图像的映射，从而求解多种偏微分方程。

Result: 模型在异质椭圆方程、受迫热方程和Burgers方程、复Ginzburg-Landau动力学、二维Navier-Stokes预测、势流、弹性力学、程函走时、相场断裂和熵最优输运等多种任务上均取得了有效结果，能够表示不稳定和激波行为。但在Kuramoto-Sivashinsky混沌系统上，由于表征误差导致无法进行有意义的长时间模拟。

Conclusion: 预训练的图像编辑模型可以作为多种物理方程的通用数值求解器，但存在根本性限制：图像和潜表征使数值范围选择和方程约束的直接施加变得困难，且对混沌系统的长时程模拟失败。本文是一项能力研究，而非试图超越专用求解器。

Classification reason: 该论文使用预训练的生成式图像编辑模型作为数值求解器，核心是图像编辑模型（image editing model）的迁移应用，因此映射到多模态生成方向下的[CANONICAL] subtopic "图像与视频编辑"（visual_editing）。

Importance reason: Novel capability study but lacks direct alignment with priority subtopics and uses negative keywords. Signals: image editing as numerical solver, physics simulation via images, capability study, not specialized solver, chaotic system failure
...
### [22] [Think Sparse, Predict Dense: Continuous Thought Machines for Image Super-Resolution](https://arxiv.org/abs/2607.18856)
*Zekai Shi*
...
Importance: 普通相关 (52.7) - 未入选精读

TL;DR: 本文提出连续思维机（CTM）用于图像超分辨率，通过引入内部时间维度（思维滴答）和窗口级紧凑到密集的查询机制，在稀疏潜空间中逐步细化密集视觉重建。实验表明，在4个思维滴答的训练下，PSNR从28.10 dB提升至30.28 dB。


<details>
  <summary>Details</summary>
Motivation: 连续思维机（CTM）在语言任务中展现了通过内部时间维度进行逐步推理的能力，但将其扩展到密集视觉预测任务（如图像超分辨率）面临挑战：超分辨率需要在每个输出位置保留空间证据，而非压缩为单一全局表征。本文旨在弥合稀疏潜思维与密集空间重建之间的鸿沟。

Method: 提出DQ-CTM（Dense Query CTM），核心包括：(1) 窗口级CTM：将特征图划分为局部视觉窗口，每个窗口通过共享的思维动力学产生紧凑摘要表征；(2) 紧凑到密集查询机制：通过结构化低秩、参数高效的方式将紧凑思维表征转换为窗口对齐的密集查询，每个位置获得独立查询；(3) 跨思维滴答逐步细化：共享的思维动力学在多个时间步中逐步精化密集表征；(4) ThinkSR实例化：编码特征图经窗口划分→共享细化→恢复原始特征场→解码为高分辨率图像。

Result: 在固定4个思维滴答的训练设置下，PSNR-Y从T=0时的28.1045 dB提升至T=4时的30.2817 dB，PSNR-RGB从26.6271 dB提升至28.7781 dB，平均ℓ1误差从0.034602降至0.023545。所有100张评估图像从T=1到T=4均有改善。

Conclusion: 初步实验验证了稀疏潜思维用于密集空间重建的可行性，表明连续思维架构可有效扩展到密集视觉预测任务，为更广泛的连续思维视觉架构研究提供了基础。

Classification reason: 该论文提出ThinkSR，将连续思维机（CTM）应用于图像超分辨率（super-resolution），属于图像恢复与增强任务，与"图像与视频编辑"（visual_editing）这一规范子主题中的图像增强/恢复方向高度相关。超分辨率是图像编辑/增强的经典子任务。

Importance reason: Proposes continuous thought for dense vision but is preliminary and not aligned with user's priority subtopics. Signals: continuous thought mechanism, image super-resolution, sparse-to-dense reconstruction, preliminary results, not autoregressive or video generation
...
### [23] [Appearance Pointers -- Multimodal Region Control of Diffusion Transformers](https://arxiv.org/abs/2607.19344)
...
TL;DR: 本文提出Appearance Pointers，一种模态无关的区域控制接口，通过区域对应网络和空间聚合机制，使DiT能够根据文本或图像输入在指定空间位置精确控制生成内容的外观，无需重新训练基础模型。


<details>
  <summary>Details</summary>
Motivation: 创意专业人士需要精确的区域级控制（如材质、物体身份、空间布局），但纯文本提示无法可靠实现这种细粒度控制。现有方法通常针对特定模态设计，缺乏统一的、模态无关的区域控制接口。

Method: 提出Appearance Pointers（外观指针），由区域对应网络（Region Correspondence Network）生成，并通过空间聚合机制（Spatial Aggregation）精炼。这些紧凑的token将文本或图像输入与用户指定的掩码对齐，引导DiT在正确的空间位置使用正确的外观线索，支持多个区域描述而不显著增加token负载。

Result: 在多项指标上，单模型达到或超越了模态特定的SOTA方法，实现了精确的区域感知多模态引导。

Conclusion: Appearance Pointers为DiT提供了一种简单、可扩展的模态无关区域控制方法，在不重新训练基础模型的前提下实现了精确的多模态引导图像生成。

Classification reason: 该论文提出了一种基于区域掩码的多模态可控图像生成方法（Appearance Pointers），属于对DiT生成图像的区域级精确控制，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）中的可控编辑方向高度吻合。

Importance reason: Novel region control for DiT aligns with multimodal generation and unified multimodal model priorities. Signals: multimodal region control, diffusion transformer, modality-agnostic interface, no retraining needed, spatial aggregation mechanism
...
### [24] [FlexiAvatar: Unified 3D Gaussian Human Avatars Under Arbitrary Body Visibility](https://arxiv.org/abs/2607.19100)
...
TL;DR: FlexiAvatar 提出一种统一的3D高斯人体数字人框架，仅优化可见身体区域以避免未观测肢体带来的伪影，结合遮挡鲁棒的SMPL-X跟踪、部位级残差精化以及扩散模型补全不可见区域纹理，在全身、半身和仅头部输入场景下均取得更优的重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常从短单目视频中联合优化所有身体区域，导致可见区域因未观测肢体的干扰而质量下降。本文旨在解决这一问题，通过仅优化可见区域来提升重建保真度。

Method: 1) 使用遮挡鲁棒的SMPL-X跟踪获取初始姿态；2) 对可见身体区域进行部位级残差精化以捕捉高频几何与外观细节；3) 对完全不可见区域（如背部）采用扩散模型生成与观测外观一致的纹理；4) 基于3D高斯泼溅（3DGS）进行渲染优化。

Result: 在NeuMan、ZJU-MoCap、WildAvatar（全身）、talk-show（上半身/半身）和INSTA（仅头部）数据集上，FlexiAvatar均取得一致更优的重建质量，平均PSNR提升约3%，同时减少了部分可见场景下的运行时和内存开销。

Conclusion: FlexiAvatar通过仅优化可见区域并利用扩散模型补全不可见区域，有效提升了人体数字人重建的质量和效率，在多种身体可见性场景下均优于现有方法。

Classification reason: 该论文研究从单目视频重建可动画化的3D人体数字人（human avatar），属于多模态生成方向下的数字人与动作生成（avatar_motion_generation）这一[CANONICAL]子主题。

Importance reason: Focuses on 3D avatar reconstruction, which is outside the user's priority subtopics and lacks strong alignment with the configured direction weights. Signals: 3D Gaussian avatars, monocular video reconstruction, diffusion-based texture completion, occlusion-robust tracking, partial body visibility
...
### [25] [Mage-Flow: An Efficient Native-Resolution Foundation Model for Image Generation and Editing](https://arxiv.org/abs/2607.19064)
...
Main category: cs.CV, cs.MM, cs.AI, eess.IV, cs.LG
...
TL;DR: Mage-Flow 是一个紧凑的 4B 参数基础模型，通过 Mage-VAE 轻量级高保真潜空间分词器与 Native-Resolution 多模态扩散 Transformer 的协同设计，实现高效文生图和指令式图像编辑。支持灵活分辨率训练，训练吞吐量提升约 2.5 倍，Turbo 变体可在单张 A100 上 0.59 秒生成 1024² 图像，1.02 秒完成编辑。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视觉生成模型训练、微调和部署成本高昂，且通常依赖重型 VAE（如 SD-VAE）导致 tokenization 开销大。作者希望设计一个紧凑高效的生成栈，在保持生成质量的同时大幅降低计算成本，使高分辨率生成和编辑在交互式场景中实用化。

Method: 提出 Mage-Flow 生成栈，包含两个协同设计的组件：(1) Mage-VAE：采用单步扩散式编码解码与 anchor-latent 正则化，在保持重建质量的同时将 tokenization 成本降低一个数量级以上；(2) Native-Resolution 多模态 Diffusion Transformer：使用 rectified flow matching 训练，支持原生分辨率打包（native-resolution packing）和 CUDA kernel 融合，实现灵活分辨率训练。在此基础上开发了 Base、RL-aligned 和 Turbo 变体，其中 Diffusion-NFT 提升提示跟随和文本渲染质量，few-step 蒸馏结合对抗感知引导实现 4 步 Turbo 推理。

Result: Mage-Flow 在标准文生图和图像编辑基准上取得有竞争力的性能。Turbo 变体在单张 A100 GPU 上，1024² 分辨率下生成图像仅需 0.59 秒，编辑图像仅需 1.02 秒，内存占用小。训练吞吐量相比基线提升约 2.5 倍。

Conclusion: 通过 tokenizer-backbone-system 的协同设计，可以在紧凑的 4B 模型规模下实现强大的高分辨率生成和编辑能力，证明了高效架构设计可以替代单纯扩大模型规模的路线。

Classification reason: 该论文提出 Mage-Flow，一个基于 rectified flow matching 的 Native-Resolution 多模态扩散 Transformer，用于文生图和指令式图像编辑。核心方法属于文生图与图像扩散范畴，因此映射到 [CANONICAL] subtopic "文生图与图像扩散"（text_to_image_diffusion）。同时论文也涉及图像编辑，可关联到 visual_editing，但主要贡献在生成模型架构上。

Importance reason: Strongly aligns with user's priority on efficient continuous multimodal generation via native-resolution diffusion transformer and lightweight tokenizer co-design. Signals: continuous latent tokenizer, native-resolution diffusion transformer, efficient inference, text-to-image generation, instruction-based image editing
...
### [26] [DuSPiT: Dual-Branch Sub-Patch Pixel Diffusion Transformer](https://arxiv.org/abs/2607.18510)
...
TL;DR: DuSPiT 提出了一种双分支子块像素扩散 Transformer，将全局结构推理与局部外观建模分离，通过紧凑基分支和并行高容量像素分支（子块分组）的交叉注意力交互，在像素空间扩散中实现了更丰富的细节和更好的质量-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 现有像素空间扩散模型将每个原始图像块映射为单个 token，迫使一个表征同时处理全局通信和细粒度细节，导致信息丢失。作者希望解决这一矛盾，在像素空间扩散中同时保持高效的全局推理和丰富的局部细节。

Method: DuSPiT 采用双分支架构：(1) 紧凑基分支（base branch）负责高效的全局结构推理；(2) 并行的高容量像素分支（pixel branch）按子块分组组织，保留详细外观信息。两个分支通过交叉注意力机制进行信息交互。

Result: 实验表明，DuSPiT 生成的图像具有更丰富的细节和更强的细粒度结构，在质量-效率权衡上优于先前的像素空间扩散 Transformer。

Conclusion: 通过分离全局推理与局部建模的双分支设计，DuSPiT 有效提升了像素空间扩散模型的生成质量，为未来像素级扩散架构提供了新思路。

Classification reason: 该论文提出了一种新的像素空间扩散Transformer架构（DuSPiT），属于图像扩散模型的基础架构改进，与[CANONICAL] subtopic "文生图与图像扩散"（text_to_image_diffusion）高度匹配。

Importance reason: Novel dual-branch pixel diffusion transformer improves detail and efficiency, aligning with user's interest in diffusion transformers and efficient inference. Signals: pixel-space diffusion, dual-branch architecture, subpatch grouping, quality-efficiency trade-off, diffusion transformer
...
Importance: 值得关注 (74.54) - 未入选精读

TL;DR: 本文提出一个统一框架，将缺失模态的医学图像生成建模为线性逆问题，通过 flow matching 模型的后验采样实现任意缺失模态的生成，并采用多对一采样策略缓解多目标生成中的模态间误差传播。


<details>
  <summary>Details</summary>
Motivation: MRI 有多种模态对比度，提供互补的解剖和病理信息。但由于时间和协议限制，完整的多模态采集往往不可用，导致真实数据集存在模态缺失。现有医学图像翻译方法通常局限于固定的源-目标设置，或需要为每个观察到的源-目标对重新训练，缺乏灵活性。

Method: 提出统一框架，将缺失模态生成建模为联合分布下的线性逆问题，通过 flow matching 模型学习完整模态集的联合先验，在推理时通过引导采样轨迹强制执行与观测模态的测量一致性来重建任意缺失模态。采用多对一采样策略缓解多目标生成中的模态间误差传播。

Result: 在 BraTS 和 IXI 数据集上的实验表明，该方法在大多数缺失模态场景下优于基线方法。在下游肿瘤分割任务中，该方法合成的图像带来了更高的分割性能，表明更好地保留了临床相关结构。

Conclusion: 本文提出的基于后验采样的统一框架能够灵活处理任意缺失模态的医学图像生成，flow matching 模型结合测量一致性引导有效实现了多模态翻译，且生成的图像在临床下游任务中具有实用价值。

Classification reason: 该论文使用 flow matching 模型进行医学图像翻译（缺失模态生成），属于图像生成/扩散方法在医学图像翻译中的应用。虽然具体应用是医学图像，但核心方法是 flow matching（扩散/流匹配生成），与"文生图与图像扩散"这一 CANONICAL 主题下的扩散/流匹配生成方法高度相关，因此复用该 subtopic。

Importance reason: Uses flow matching for missing-modality generation, but in medical imaging, which is a negative keyword area. Signals: flow matching, posterior sampling, medical image translation, missing modality, linear inverse problem
...
### [28] [Text Template Tokens Are Implicit Semantic Registers in Diffusion Transformers](https://arxiv.org/abs/2607.19139)
...
TL;DR: 本文提出一种因果可解释性框架，分析大规模文本到图像DiT的内部计算机制。发现结构模板token（如分隔符、特殊标记）在编码器输出时几乎不含提示语义，但在DiT内部却成为主导的图像到文本注意力汇聚点（attention sink），并因果性地维持物体身份，充当隐式语义寄存器。基于此发现，设计了无需训练的注意力头剪枝规则，可移除20%注意力FLOPs而仅损失1.4个GenEval点。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散Transformer（DiT）虽然取得了显著进展，但其内部去噪过程中的计算机制仍不清晰。理解这些机制有助于揭示模型的工作原理，并为模型压缩、优化提供理论指导。

Method: 提出一种因果可解释性框架，结合注意力分解与跨token跨度、注意力头和层的定向干预。通过分离提示内容token与结构模板token，分析它们在DiT内部的信息流动和因果作用。基于发现设计训练无关的剪枝规则：对提示token注意力最强的头是可去除的。

Result: 结构模板token在编码器输出时几乎不含提示信息，但在DiT内部成为主导的图像到文本注意力汇聚点，因果性地维持物体身份。提示语义首先注入图像潜变量，然后被读回模板token，而非直接从提示token传递。基于此设计的剪枝规则可移除20%注意力FLOPs，仅损失1.4个GenEval点。

Conclusion: 揭示了DiT中编码语义的输入token不一定是在生成过程中维持语义的token，提供了DiT内部机制的因果视角。结构模板token充当隐式语义寄存器，这一发现可用于模型压缩和架构理解。

Classification reason: 该论文研究文本到图像扩散Transformer（DiT）的内部机制，属于文生图与图像扩散这一[CANONICAL]子主题。

Importance reason: Provides causal interpretability of DiTs and a training-free pruning rule, relevant to model efficiency and multimodal generation. Signals: DiT interpretability, attention sink, training-free pruning, 20% FLOPs reduction, implicit semantic registers
...
### [29] [Surprise Forcing: What to Remember, When to Skip in Long Video Generation](https://arxiv.org/abs/2607.18436)
...
TL;DR: Surprise Forcing 提出一种无需训练的框架，通过 Surprise-Gated Memory Bank（基于全局偏差和最近邻新颖性信号管理外部记忆）和 Surprise-Aware Denoising（根据块难度自适应跳过中间去噪步骤），解决流式自回归扩散模型在长视频生成中的上下文遗忘和均匀去噪分配问题，在 VBench 系列基准上提升了长程一致性和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 流式自回归扩散模型虽能实现分钟级视频合成，但其固定上下文窗口和均匀去噪调度在高度非平稳的长视频序列中效率低下：滚动 KV 缓存会遗忘重要的早期视觉信息，而每个生成块无论难度如何都接受相同次数的去噪，导致资源浪费。

Method: 提出 Surprise Forcing 框架，包含两个核心组件：(1) Surprise-Gated Memory Bank——用 value-token 描述符总结被驱逐的帧，通过全局偏差和最近邻新颖性两种互补信号评估其重要性，并在归一化分数空间中通过反馈控制预算调节准入，结合基于优先级的替换和相关性感知路由保持外部记忆紧凑有用；(2) Surprise-Aware Denoising——在第一次去噪后根据最大相邻帧余弦距离估计块难度，使用局部百分位调度器跳过较简单块的中间去噪步骤。

Result: 在 VBench、VBench-Long 和 VBench-2.0 基准上，所提出的资源分配策略在保持实时流式吞吐量的同时，显著提升了长视频生成的长程一致性和视觉质量。

Conclusion: Surprise Forcing 通过将上下文管理和去噪调度视为在线资源分配问题，有效解决了流式长视频生成中的关键瓶颈，为高效长视频合成提供了无需训练的实用方案。

Classification reason: 该论文研究长视频生成中的流式自回归扩散模型，属于视频扩散生成范畴，与 [CANONICAL] 文生视频与视频扩散 主题高度匹配。

Importance reason: Directly addresses long video generation efficiency and consistency, aligning with user's high-priority autoregressive video generation and world model subtopics. Signals: video generation, autoregressive diffusion, long video, training-free, memory management
...
### [30] [Learning Explicit Physical Parameter Control and Benchmarking for Video Generation](https://arxiv.org/abs/2607.18924)
...
TL;DR: 本文提出 PhyParam，一个物理引导的图像到视频扩散模型，通过轻量级物理注意力路由机制在物体级别（力、质量、摩擦、恢复系数）和场景级别（重力）控制物理参数，并构建了包含13万条物理仿真视频的 PhyParam-Dataset 数据集和 PhyParam-Bench 物理一致性评测基准，在保持高视觉保真度的同时提升了视频生成的物理一致性。


<details>
  <summary>Details</summary>
Motivation: 现有图像到视频生成模型虽然能生成看似合理的运动，但这些运动并不受显式物理因果关系的可靠控制，且实例级约束在多物体交互中容易泄露或纠缠。作者认为这归因于两个缺失：大规模细粒度物理参数化数据，以及能够正确将物理属性绑定到实例并强调动态而非外观的模型设计。

Method: 1) 构建 PhyParam-Dataset：包含13万条物理仿真视频，覆盖五种刚体运动类型，带有密集物理参数化（力向量、物体材料属性、环境常数）。2) 提出 PhyParam 模型：基于图像到视频扩散模型，通过轻量级物理注意力路由机制（physical-attention routing）在物体级别条件化力、质量、摩擦、恢复系数，在场景级别条件化重力；并引入语义-结构特征空间监督（semantic-structural feature-space supervision）来改进运动学习。3) 建立 PhyParam-Bench 基准：多层级评测协议，评估时序动态、空间稳定性和语义-物理对齐。

Result: 实验表明 PhyParam 在提升物理一致性的同时保持了高视觉保真度，在物理参数控制方面优于现有方法，能够显式控制刚体物理参数进行图像到视频生成。

Conclusion: 本文通过大规模物理参数化数据集、物理条件扩散模型和物理一致性基准，推进了显式刚体物理参数控制在图像到视频生成中的应用，为未来物理世界模拟奠定了基础。数据集、基准和代码将公开发布。

Classification reason: 该论文提出物理参数可控的图像到视频扩散模型（PhyParam），属于视频生成方法，与 canonical subtopic "文生视频与视频扩散"（text_to_video_diffusion）高度匹配。同时论文涉及物理世界建模和未来预测，也与 world_model 方向下的 "视频世界模型"（video_world_model）相关。

Importance reason: Directly advances controllable video generation with explicit physical parameter control, aligning with user's high-priority video generation and world model interests. Signals: video generation, controllable generation, world model, physical world model, diffusion video generation
...
### [31] [FilmWorld: Agentic Novel-to-Film Generation through Dynamic Cinematic World Modeling](https://arxiv.org/abs/2607.19038)
...
TL;DR: FilmWorld 是一个端到端智能体系统，将小说转化为电影级长视频。它将小说到电影生成建模为动态电影世界建模，分为构建阶段（将抽象文学叙事转化为具体的世界实体）和演化阶段（在情节推进中动态更新实体状态以保持因果一致性）。系统由两组智能体协作完成，并提出了 FilmEval 评测框架。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型仅擅长短片段、单场景的生成，无法处理小说到电影这种需要长时长、多场景、动态实体状态演化的复杂任务。作者希望弥合抽象文学叙事与具体视觉叙事之间的鸿沟。

Method: 将小说到电影生成形式化为动态电影世界建模，分解为构建和演化两个阶段。构建侧智能体执行叙事结构化翻译、世界实体状态建模与视觉锚定、状态驱动的镜头规划；演化侧智能体执行状态锚定的视觉生成、跨镜头动态状态传播、闭环状态验证以保持因果一致性和视觉连贯性。同时提出 FilmEval 评测框架，包含15部代表性小说的难度分级基准和9个客观指标。

Result: 实验表明 FilmWorld 在叙事忠实度和跨场景一致性上显著优于现有最先进的视频生成智能体系统。

Conclusion: FilmWorld 通过动态电影世界建模有效解决了小说到电影的长视频生成挑战，在叙事连贯性和跨场景一致性方面取得了显著提升，为长形式视频生成提供了新的范式。

Classification reason: 该论文提出从小说到电影的长视频生成系统，核心是文生视频任务，属于多模态生成方向。虽然涉及世界建模概念，但本质是视频生成系统，因此映射到 canonical 子主题"文生视频与视频扩散"。同时涉及多模态理解与生成方向中的统一多模态模型概念，但主要贡献在视频生成。

Importance reason: Strong on multimodal generation and world model, but lacks direct focus on continuous language/multimodal models or autoregressive generation. Signals: multimodal generation, world model, video generation, agentic system, long-form video
...
### [32] [Provable diffusion-based posterior sampling for linear inverse problems via DDIM](https://arxiv.org/abs/2607.19333)
...
TL;DR: 本文提出PDDIM算法，通过对标准DDIM更新进行轻量级坐标级修改，显式融入测量模型，实现线性逆问题的扩散后验采样。算法在测量算子的每个奇异方向上根据信噪比自适应切换扩散先验与测量预测，理论上证明收敛到贝叶斯后验，实验在多种图像复原任务上达到最优或接近最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的后验采样方法要么缺乏严格的理论保证，要么计算开销过大。作者希望设计一种既简单高效、又有严格理论保证的后验采样算法，用于求解带噪声的线性逆问题。

Method: 提出PDDIM算法，在DDIM采样框架基础上，对测量算子的每个奇异方向进行坐标级修改：当观测信噪比低于对应扩散信噪比时，遵循学习到的扩散先验；否则切换到基于测量的校准预测器。这种设计将后验采样转化为简单的坐标级DDIM更新。

Result: 在多种图像复原任务（如去噪、超分辨率、修复等）上，PDDIM在大多数评估指标上优于现有扩散后验采样方法，同时保持计算高效性。理论分析证明了算法收敛到贝叶斯后验分布。

Conclusion: PDDIM将带噪线性逆问题的后验采样转化为简单的坐标级DDIM更新，提供了一种高效、易实现且具有严格后验一致性保证的算法，为扩散模型在逆问题中的应用提供了理论基础和实用工具。

Classification reason: 该论文提出基于DDIM的扩散后验采样算法PDDIM，用于线性逆问题求解，属于扩散模型采样方法的高效化改进。虽然论文侧重理论保证（后验一致性），但其核心是DDIM采样器的轻量级修改以实现高效后验采样，与"生成模型加速与高效化"（efficient_generation）这一规范主题高度匹配。

Importance reason: Strong theoretical contribution for diffusion posterior sampling, but only tangentially related to user's priority subtopics. Signals: provable posterior sampling, linear inverse problems, DDIM modification, image restoration, theoretical convergence guarantee
...
### [33] [One Rewrite to Fix Them All? Type-Aware Repair Allocation for Text-to-Image Prompt Optimization](https://arxiv.org/abs/2607.18724)
...
TL;DR: 本文提出 TARA（Type-Aware Repair Allocation），一种无需训练的文本到图像提示词优化框架。它将用户提示中的失败命题按类型（计数、属性、关系、文本）路由到不同的修复算子，并通过语义修复门控机制防止语义退化。在 DSG 和 TIFA 基准上，TARA 在八个生成器-基准组合中均取得最佳语义准确率，同时保持图像质量和运行效率。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成器在遵循复杂提示时经常出错（如计数错误、属性交换、关系歧义、文本不清晰）。已有的提示词优化方法将所有类型的失败统一处理为一种提示扩展，忽略了不同类型失败需要不同修复语言的问题。作者希望提出一种类型感知的、细粒度的提示修复方法。

Method: TARA 框架将语义提示优化形式化为原子修复分配问题：首先诊断提示中每个命题的失败类型，然后将其路由到对应的类型条件修复算子（计数、属性、关系、文本），最后将局部约束编译为可执行的提示。框架包含四个模块：诊断（Diagnosis）、分配（Allocation）、编译（Compilation）和一个语义修复门控（Semantic Repair Gate），后者通过接受或回退机制确保每次只执行一个修复，防止语义退化。

Result: 在 DSG 和 TIFA 两个基准上，使用四个冻结的生成器（SDXL、DALL-E 3、Playground v2.5、PixArt-α）进行实验，TARA 在所有八个基准-生成器组合中均取得最佳语义准确率。相比 VisualPrompter，DSG 上提升 5.6 分，TIFA 上提升 2.6 分。同时保持图像质量，运行速度最快（每提示 16.0 秒 vs 20.0 秒）。

Conclusion: 类型感知的修复分配能有效提升文本到图像生成中提示优化的效果，不同类型的失败需要不同的修复策略。TARA 框架无需训练即可显著提升语义对齐，且不牺牲图像质量和效率。未来工作可探索更细粒度的失败类型分类和更复杂的修复算子组合。

Classification reason: 该论文研究文本到图像生成中的提示词优化，通过类型感知的修复分配来提升生成结果与用户提示的对齐度（语义准确性），属于生成模型对齐与可控性范畴，因此映射到 [CANONICAL] subtopic "生成模型对齐与奖励" (generation_alignment)。

Importance reason: Prompt optimization for text-to-image generation is a niche application, not a core advance in the user's priority subtopics like continuous models or world models. Signals: Prompt optimization, Text-to-image, Type-aware repair, Semantic accuracy, Training-free
...
### [34] [Signed Rectified Flow: Negativity-Controlled Generation](https://arxiv.org/abs/2607.18516)
...
Importance: 普通相关 (53.31) - 未入选精读

TL;DR: 本文提出 Signed Rectified Flow (Signed RF)，将 Rectified Flow 推广到带符号测度，通过引入负分布信息实现生成过程中的排斥约束，从而提升生成质量、减少记忆、抑制不良内容生成。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型（如扩散模型、Rectified Flow）仅从正分布采样，无法自然地融入"不生成什么"的负信息。作者希望建立一种能显式利用负分布信息来约束生成过程的框架，以改善保真度-多样性权衡、缓解记忆效应、抑制有害内容生成。

Method: 将 Rectified Flow 推广到 signed measure π^{sign} = (1+α)π⁺ - απ⁻，其中 π⁺ 是目标分布，π⁻ 是抑制分布。通过 signed continuity equation 和带电粒子类比，推导出负质量形成排斥势垒的机制，并据此设计自适应 guidance 算法，使生成过程在正分布区域集中概率密度，同时排斥负分布区域。

Result: 在 ImageNet 上改善了保真度-多样性权衡；在反记忆实验中降低了最近邻相似度；在 Stable Diffusion 3.5 上有效减少了对抗提示引发的裸体内容生成，同时保持了 CLIP 和美学分数。

Conclusion: Signed RF 为生成模型提供了一种融入负信息与排斥约束的理论框架，在多个应用场景中验证了其有效性，为可控生成与安全对齐开辟了新方向。

Classification reason: 该论文提出 Signed Rectified Flow，通过引入负信息（抑制分布）来改进生成模型的对齐与可控性，属于生成模型中对齐、guidance 与可控性的范畴，因此映射到 canonical 子主题「生成模型对齐与奖励」。

Importance reason: Novel signed measure framework for generative models, but only tangentially touches priority subtopics via flow matching. Signals: Signed rectified flow, negativity-controlled generation, flow matching generalization, fidelity-diversity trade-off, anti-memorization
...
### [35] [Decafs: Disentangled Conditional adversarial Flows](https://arxiv.org/abs/2607.18755)
...
Importance: 值得关注 (68.79) - 未入选精读

TL;DR: 提出Decafs，一种基于Lie群的条件生成框架，通过对抗损失将解耦的潜空间与流模型的潜流空间对齐，实现可解释的条件生成，无需扩展流空间维度，在图像和分子生成任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 流模型在生成建模中表现优异，但其潜空间中的生成因子高度纠缠，导致难以进行可解释的条件生成和可控生成。现有方法需要扩展流空间维度来解耦，但受限于流模型的可逆性要求。

Method: 提出Decafs框架，利用Lie群构造一个解耦的替代潜空间，并通过对抗损失将该解耦空间与流模型的潜流空间对齐，从而实现可解释的条件生成，同时避免扩展流空间维度。

Result: 在条件图像生成任务（MNIST、dSprites）上超越StyleGAN，在分子生成任务（QM9、ZINC、MOSES）上也取得强性能。

Conclusion: Decafs通过Lie群解耦潜空间与对抗对齐流空间，有效实现了可解释的条件生成，为流模型的可控生成提供了新思路。

Classification reason: 论文提出基于Lie群的条件生成器，通过对抗损失解耦潜空间实现可控条件生成，属于生成模型的可控性与对齐方向，映射到[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）。

Importance reason: Focuses on interpretable conditional generation with flows and adversarial loss, but lacks direct connection to user's priority subtopics like continuous models, video generation, or world models. Signals: conditional generation, Lie group, disentangled latent space, adversarial alignment, image and molecule generation
...
TL;DR: Vector-Bench 是一个紧凑且困难的 SVG 代码编辑评测基准，包含 40 个修复任务，每个任务有损坏的 SVG 程序、视觉指令、隐藏目标程序和大量受保护对象。评估了 34 个模型端点，最强模型仅达到 15.0% 的完整规范成功率，表明表面修复进展与忠实于规范的编辑之间存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有基于指令的向量编辑评测通常仅以光栅图像形式判断输出，容易忽略"只修改请求部分、保持其他不变"这一关键能力。作者希望构建一个能精确衡量模型是否真正按照指令进行"外科手术式"编辑的基准。

Method: 构建 40 个 SVG 修复任务，每个任务包含损坏的 SVG 程序、视觉指令、隐藏目标程序、平均 5.05 个标注修复和平均 60.55 个受保护对象。定义确定性二元规范奖励：请求修复使用属性感知感知容差，未请求的渲染或应用相关结构必须语义不变，结果必须是有效 SVG。评估 34 个模型端点（25 个开放权重、5 个廉价控制、4 个前沿闭源端点），共 1360 次请求。

Result: 最强端点仅达到 15.0% 的完整规范成功率，尽管平均修复进度为 43.7%。这表明表面修复进展与忠实于规范的编辑之间存在显著差异。所有提示、输出、评分代码、成本和每个任务的报告均已发布。

Conclusion: Vector-Bench 揭示了当前模型在 SVG 代码编辑任务中"外科手术式"编辑能力的严重不足，为未来向量编辑研究提供了一个精确、可复现的评测工具。

Classification reason: 该论文提出了 Vector-Bench，一个针对 SVG 代码编辑的评测基准，属于生成模型评测与数据集（generation_benchmark）这一 [CANONICAL] 子主题。该子主题在 multimodal_generation 和 multimodal_understanding_generation 两个方向下均有收录，因此匹配两个方向。

Importance reason: Focuses on SVG code editing benchmark, not aligned with user's priority on continuous multimodal models or video/image generation. Signals: SVG editing benchmark, low model performance, not continuous multimodal, not video/image generation, not world model
...
### [37] [Moving Alphabet: A Controlled Study of Training Data for Text-to-Video Generation](https://arxiv.org/abs/2607.18789)
...
TL;DR: 本文提出 Moving Alphabet，一个程序化生成的测试平台，用于系统研究训练数据分布和字幕质量对文生视频模型的影响。实验发现：1) 视频内容和时长的多样性与平衡分布对泛化至关重要；2) 字幕质量显著影响模型性能和训练效率；3) CFG 和高质量微调无法完全弥补预训练数据质量不足的问题。


<details>
  <summary>Details</summary>
Motivation: 文生视频模型在过去五年取得了显著进展，但研究主要集中在模型架构扩展上，训练数据的影响往往被忽视。真实世界的数据整理涉及从原始视频中选取片段和生成字幕，过程复杂且难以控制。作者希望通过可控实验系统性地研究数据分布和字幕质量对文生视频模型的影响，为大规模预训练数据科学提供指导。

Method: 作者设计了一个程序化测试平台 Moving Alphabet，在黑背景上渲染不同字体、颜色、大小和位置的字母，以不同方向和速度移动。通过操纵真实元数据（如字母属性、运动参数）来精确控制数据分布和字幕质量。具体地：1) 通过控制视频内容（字母、颜色、字体等）和时长（1-8秒）的分布来研究数据分布影响；2) 通过向字幕中注入不同级别的噪声（如随机替换字母、颜色、方向等属性）来模拟不同质量的字幕；3) 训练视频扩散模型并在测试集上评估生成质量。

Result: 实验发现：1) 多样且平衡的视频内容和时长分布对模型泛化能力至关重要，不平衡分布会导致模型产生偏见；2) 字幕质量显著影响模型性能和训练效率，低质量字幕不仅降低生成质量，还导致训练收敛变慢；3) 使用 classifier-free guidance (CFG) 和在高质量数据上微调可以部分恢复模型性能，但无法完全弥补预训练数据质量不足带来的损失。

Conclusion: 本文通过可控实验揭示了训练数据分布和字幕质量对文生视频模型的关键影响，强调了预训练数据科学的重要性。研究结果表明，在追求更大模型和更多计算资源的同时，应更加关注训练数据的质量和分布设计。这些见解可为大规模文生视频模型的开发提供指导。

Classification reason: 该论文研究训练数据分布和字幕质量对文生视频模型的影响，并提出了一个可控测试平台 Moving Alphabet 用于系统实验。这属于生成模型的数据集与评测研究，因此映射到 [CANONICAL] subtopic "生成模型评测与数据集" (generation_benchmark)。

Importance reason: Systematic study of training data effects on text-to-video generation, but lacks direct focus on user's priority subtopics like continuous models or world models. Signals: Controlled data distribution study, caption quality impact, limited to simple synthetic data, no autoregressive or diffusion model focus, no world model or continuous representation
...
### [38] [ExpertVerse: A General-Purpose Benchmark for Expert-Level Reasoning in Knowledge-Intensive Visual Synthesis](https://arxiv.org/abs/2607.19341)
...
TL;DR: 本文提出ExpertVerse，一个面向知识密集型视觉合成的专家级推理评测基准，包含9种认知能力×8个专家学科的58个子领域、1,611个专家标注实例及100K规模数据集。基于此训练了KnowThinker VLM推理引擎，并提出Bootstrapped Pareto Policy Optimization (BPPO) 优化方法，揭示了现有模型在知识密集型生成中的严重推理缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有多模态生成模型在显式常识推理、浅层因果理解和直接知识回忆上表现尚可，但在需要深度专业知识的知识密集型视觉合成任务中严重不足。缺乏一个系统性的、能力导向的基准来评估生成模型在专家级推理方面的能力。

Method: 1) 构建ExpertVerse基准：正交分类体系包含9种认知能力和8个专家学科，共58个子领域，1,611个专家标注实例覆盖单图编辑、多图合成和文生图三类任务。2) 开发自动化流程生成ExpertVerse-100K大规模数据集，包含推理轨迹和知识锚定的原理标注。3) 训练KnowThinker VLM推理引擎，使用RL微调，联合生成思考过程和精炼指令。4) 提出BPPO（Bootstrapped Pareto Policy Optimization），包含Bootstrapping Reward Rectification (BRR) 和 Conflict-Aware Pareto Advantage Fusion (CPAF)，解决跨模态信用分配偏差和多目标梯度冲突。

Result: 对多个开源和闭源模型的广泛评测暴露了关键推理缺陷，证明了知识密集型基准对下一代视觉生成的重要性。KnowThinker结合BPPO在知识密集型视觉合成任务上取得了显著提升。

Conclusion: 知识密集型视觉合成需要超越当前模型的推理能力，ExpertVerse基准和KnowThinker模型为该方向提供了评测框架和方法基础，BPPO有效缓解了多奖励优化中的冲突问题。

Classification reason: 该论文提出了ExpertVerse基准，用于评估生成模型在知识密集型视觉合成中的专家级推理能力，包含1,611个专家标注实例和100K规模数据集，属于生成模型评测与数据集范畴，因此映射到multimodal_generation方向下的[CANONICAL] subtopic "generation_benchmark"（生成模型评测与数据集）。同时论文也涉及VLM推理引擎KnowThinker的训练，因此也匹配multimodal_understanding_generation方向。

Importance reason: Introduces a benchmark for knowledge-intensive visual synthesis, which is relevant to multimodal generation but lacks direct focus on the user's priority subtopics like continuous models or autoregressive generation. Signals: knowledge-intensive visual synthesis benchmark, expert-level reasoning evaluation, BPPO optimization method, KnowThinker VLM reasoning engine, reveals reasoning deficits in current models
...
### [39] [Staged Depth-Pruning Distillation of a Flow-Matching Text-to-Speech Teacher: A Compact Hindi Speech Synthesizer](https://arxiv.org/abs/2607.18662)
*Sivateja Trikutam*
...
TL;DR: 本文提出了一种通过深度剪枝蒸馏flow-matching TTS教师模型（IndicF5，337M参数）来构建紧凑印地语语音合成器的实用方案。在仅约17.6小时的数据预算下，通过逐步深度剪枝（22→16→12→8→6层）并配合重新微调，得到的学生模型在249M和190M参数时WER为0.00，最低可降至131M仍保持鲁棒，102M时出现容量悬崖。最终模型可在6GB笔记本GPU上实时运行。


<details>
  <summary>Details</summary>
Motivation: 在数据资源有限（仅17.6小时印地语语音数据）的情况下，从头训练小模型进行TTS合成效果很差。作者希望探索一种实用的蒸馏方案，从大规模flow-matching TTS教师模型中通过深度剪枝蒸馏出紧凑但高质量的学生模型，使其能在消费级GPU上实时运行。

Method: 采用分阶段深度剪枝蒸馏策略：保持教师模型的宽度、文本维度、注意力头数和mel/text I/O不变（非块张量直接复制），仅均匀保留Transformer块的子集。先测试教师模型对深度剪枝的容忍度（-27%块仍接近可用，-50%后崩溃），然后逐步下降（22→16→12→8→6块），每次剪枝后重新微调，每一步用ASR词错误率（WER）作为门控检查。同时记录了两种训练/推理特征和库版本不匹配问题（mel滤波器组和rotary-embedding库版本）及其修复方法。

Result: 学生模型在249M和190M参数时在未见句子上达到WER 0.00，131M参数仍保持鲁棒，102M时出现容量悬崖。190M学生模型在独立50句FLEURS基准上与教师模型和MMS-TTS-hin进行了对比。最终模型可在6GB笔记本GPU上实时运行。

Conclusion: 分阶段深度剪枝蒸馏是一种在数据预算严重受限条件下构建紧凑TTS模型的有效方案。容量悬崖（102M处）主要归因于数据预算不足而非方法本身。该方法可推广到其他低资源语言的TTS模型压缩场景。

Classification reason: 该论文研究基于flow-matching的印地语文本到语音（TTS）合成，属于音频生成方向，与[CANONICAL] subtopic "音频与多模态联合生成"（audio_av_generation）完全匹配。

Importance reason: Focuses on Hindi TTS via depth-pruning distillation, which is outside the user's priority subtopics and lacks the specified boost keywords. Signals: Hindi TTS, flow-matching distillation, depth pruning, low data budget, WER evaluation
...
### [40] [Hazard or Anomaly? Evaluating VLMs for Understanding Dangers and Discrepancies](https://arxiv.org/abs/2607.18325)
...
TL;DR: 本文提出将危险（hazard）与异常（anomaly）明确区分，评估多个SOTA VLM在安全关键场景中的推理能力。实验表明VLM常将异常误判为危险，暴露了依赖上下文不规则性作为危险代理的缺陷。显式区分异常与危险能提供更丰富的安全推理评估。


<details>
  <summary>Details</summary>
Motivation: 现有VLM安全评估通常将危险识别视为二元分类（安全/不安全），无法区分模型是在识别真正的物理危险还是仅对异常场景元素做出反应。需要更细粒度的评估框架来揭示VLM安全推理的真实能力与失败模式。

Method: 引入危险（hazard）与异常（anomaly）的显式区分，在多个数据集上使用多种提示策略评估多个SOTA VLM，分别测试模型识别危险状态和异常状态的能力，并对比二元安全判断与细粒度区分的差异。

Result: VLM经常将异常误判为危险，表现出过度依赖上下文不规则性作为危险代理的倾向。显式分离异常与危险能提供比二元安全判断更丰富的评估信息，并暴露被二元判断掩盖的失败模式。

Conclusion: 当前VLM在安全关键场景中的推理存在系统性偏差，将异常等同于危险。未来需要更精细的安全推理评估框架，并改进VLM对真正物理危险的理解能力。

Classification reason: 该论文评估VLM在安全关键场景中区分危险（hazard）与异常（anomaly）的能力，属于多模态安全评测范畴，与[CANONICAL] subtopic "多模态安全与攻防"（multimodal_safety_attack）高度匹配。

Importance reason: The paper's focus on VLM safety reasoning is tangentially relevant but does not align with the user's priority subtopics on continuous models, generation, or world models. Signals: VLM safety evaluation, hazard vs anomaly distinction, human-robot interaction, safety-critical systems, binary safety judgment limitation
...
TL;DR: SciHazard 是一个面向科学领域安全风险的评测基准，包含 2400 个危害性问题和 600 个过度安全问题的数据集，以及一个分解式危害评分框架 DeHarm-Score，用于系统评估 LLM 在科学 misuse 场景下的安全性。


<details>
  <summary>Details</summary>
Motivation: 现有安全评测基准多使用模板化查询，与实际危害场景脱节，且依赖 LLM-as-a-Judge 范式缺乏领域基础。科学领域 LLM 可能将危险科学知识转化为可操作的 misuse 指导，亟需一个真实场景驱动的、细粒度的科学安全评测框架。

Method: 构建 SciHazard 数据集，包含 12 个学科的 2400 个危害性问题和 600 个过度安全问题，查询均基于受监管实体和已记录的事故场景。提出 DeHarm-Score 分解式评估框架，结合查询危害严重性、拒绝行为和响应级风险三个维度；对未拒绝的响应，进一步分解为可执行性（通过动态检查清单和重要性权重量化）和净新增风险（通过检索增强的声明提取和合成-屏障验证评估）。

Result: 专家验证研究表明，DeHarm-Score 相比最强基线将专家标注一致性提高了 90.17%。对 31 个前沿 LLM 和深度研究智能体的评测显示，深度研究智能体的平均 DeHarm-Score 比标准 LLM 高 32.3%，暴露了自主智能体是当前安全防御的关键盲点。

Conclusion: SciHazard 和 DeHarm-Score 为科学领域 LLM 安全评测提供了更真实、更细粒度的评估工具。自主智能体在科学安全方面存在显著风险，需要更有效的安全防御机制。

Classification reason: 该论文提出 SciHazard 基准，用于评估 LLM 在科学领域的危害性安全风险，属于多模态安全与攻防（multimodal_safety_attack）范畴下的安全评测工作。虽然论文主要关注文本 LLM 而非多模态模型，但安全评测与攻防是该 canonical 子主题的核心内容，且论文的 DeHarm-Score 评估框架可推广至多模态场景，因此合理复用该子主题。

Importance reason: Focuses on scientific safety benchmarks, not on the user's priority subtopics like continuous models or video generation. Signals: Scientific safety benchmark, Decomposed harm scoring, Not aligned with user's priority subtopics, Negative keywords absent but direction weight low
...
TL;DR: 提出一种双对抗微调框架，通过联合优化视觉和语义两个模态的监督信号，增强LVLM在多种下游任务上的对抗鲁棒性，无需任务特定的重新训练。
...
Method: 提出双对抗微调框架，包含两个分支：(1) 视觉监督分支，利用冻结的原始视觉编码器提取的干净图像特征来引导对抗鲁棒性；(2) 语义监督分支，利用图像-标题对齐作为上下文信号，在攻击下保持语义一致性。通过替换原始模型中的CLIP视觉编码器实现跨任务鲁棒性。

Result: 在零样本分类、图像描述和VQA任务上的对抗鲁棒性评估中，该方法优于现有最先进方法。

Conclusion: 双对抗微调框架能有效提升LVLM在多任务场景下的对抗鲁棒性，具有良好的泛化能力。

Classification reason: 该论文研究LVLM的对抗攻击防御方法，属于多模态安全与攻防方向，与[CANONICAL] subtopic "多模态安全与攻防"（multimodal_safety_attack）高度匹配。

Importance reason: Focuses on adversarial robustness for LVLMs, which is not a priority subtopic and lacks direct connection to user's core interests in continuous models, generation, or world models. Signals: Adversarial fine-tuning, LVLM robustness, cross-task defense, visual and semantic supervision, no task-specific retraining
...
### [43] [Attention Without Grounding: Causal Evaluation of Visual Explanations in Medical VLMs](https://arxiv.org/abs/2607.18577)
...
TL;DR: 本文对医学VLM（MedGemma、LLaVA-RAD、Qwen3-VL、CheXagent）在胸部X光片上的注意力/显著性热图进行因果评估，发现所有模型均未同时满足"模型确实使用图像"和"注意力集中在遮挡后影响预测的区域"两个忠实性标准。热图与真实病灶区域重叠度不优于随机基线，而传统CXR分类器通过所有测试，表明失败是VLM热图特有的问题。


<details>
  <summary>Details</summary>
Motivation: 注意力热图被广泛用于解释医学VLM的决策，但其是否真正反映模型使用的图像证据缺乏因果验证。临床应用中不可靠的解释可能误导诊断，因此需要严格评估热图的忠实性。

Method: 使用三个因果评估指标：(1) 与放射科医生标注框的重叠度（PadChest, n=637）；(2) 放射科医生掩码内的注意力质量（CheXlocalize, n=643）；(3) 16x16 patch遮挡映射——记录遮挡哪些区域会改变模型答案。评估了MedGemma-4B三种变体、LLaVA-RAD、Qwen3-VL-8B-Instruct、CheXagent-2-3b，以DenseNet121和ResNet50作为阳性对照。

Result: 没有VLM同时满足两个忠实性标准。MedGemma和Qwen3-VL使用图像，但注意力与patch遮挡重要性呈负相关（rho<0，95% bootstrap CI低于零）。LLaVA-RAD注意力正相关，但模型几乎仅依赖文本（99.1%文本一致率，因果质量接近零）。所有模型注意力与真实解剖区域重叠不优于随机或偏移控制。两个CXR分类器通过所有指标。

Conclusion: 医学VLM的注意力热图视觉上看似合理但实际不忠实，不能可靠解释模型决策。临床解释需要受控的定位指标和因果扰动验证，而非仅依赖视觉检查。

Classification reason: 该论文因果评估医学VLM注意力热图是否忠实反映模型决策依据，核心关注VLM解释的可靠性与忠实性（faithfulness），属于多模态模型可靠性/忠实性范畴，与[CANONICAL]多模态幻觉与可靠性（multimodal_hallucination）高度匹配。

Importance reason: Strong causal evaluation of VLM faithfulness, but focuses on medical domain which is a negative keyword for the user. Signals: Causal faithfulness audit, VLM heatmaps fail, medical domain, negative keyword match, strong methodology
...
### [44] [H$^2$SD: Hybrid Hindsight Self-Distillation](https://arxiv.org/abs/2607.18955)
...
TL;DR: H²SD 是一种混合事后自我蒸馏框架，根据轨迹正确性采用不同的教师策略：成功轨迹用教师信号调节更新幅度，失败轨迹用参考提示进行反向 KL 散度蒸馏，从而在 RLVR 中实现更优的 token 级信用分配与稳定优化。


<details>
  <summary>Details</summary>
Motivation: 现有 RLVR 方法仅对整个轨迹分配标量结果奖励，监督稀疏且缺乏 token 级信用分配。OPD 需要额外教师模型，OPSD 存在信息泄露和不稳定优化问题，RLSD 无法为失败轨迹提供显式修正方向。需要一种既能利用教师信号又避免其缺点的混合方法。

Method: H²SD 框架根据轨迹正确性采用两种不同的教师使用策略：对于成功轨迹，教师接收学生正确回答及改写指令，用教师概率调节更新幅度而不改变奖励决定的方向；对于失败轨迹，教师接收包含关键推理步骤和已验证答案的参考提示，最小化学生到教师的反向 KL 散度。

Result: 在多个具有挑战性的推理基准上，H²SD 持续优于代表性 RLVR、OPSD 和 RLSD 基线，同时保持稳定的优化和良好的生成效率。

Conclusion: H²SD 通过混合事后自我蒸馏框架有效解决了 RLVR 中稀疏监督与 token 级信用分配的权衡问题，为语言模型推理能力的强化学习训练提供了更优方案。

Classification reason: 该论文提出 H²SD，一种混合事后自我蒸馏框架，用于改进大语言模型在可验证奖励强化学习（RLVR）中的推理能力。核心关注点是通过蒸馏和奖励信号进行 token 级信用分配与优化，属于多模态/语言模型强化学习与对齐范畴，因此映射到 [CANONICAL] 多模态强化学习与对齐（multimodal_rl_alignment）子主题。

Importance reason: Focuses on RLVR for LLM reasoning, not on the user's priority subtopics like continuous multimodal models or video generation. Signals: RLVR, self-distillation, token-level credit assignment, reasoning improvement, hybrid teacher strategy
...
TL;DR: RRPO 提出了一种参考相对策略优化方法，通过分层条件 rollout 构建正负锚点集，利用集合对比目标训练度量投影头，生成对比优势分数用于策略优化，无需任务真实验证器即可在可验证推理、开放生成和 SFT 后场景中取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: GRPO 在可验证反馈的强化学习中表现优异，但难以扩展到非可验证场景（如开放生成任务），因为许多任务的成功无法由单一正确性标准衡量。本文旨在将组相对优化推广到更广泛的场景。

Method: RRPO 首先使用分层条件 rollout 构建正负锚点集，然后通过集合对比目标训练一个度量投影头，将候选 rollout 与锚点进行比较得到对齐分数。在策略优化阶段，投影头被冻结，对齐分数在每组 rollout 内中心化后作为对比优势，用于标准组相对优化目标。

Result: 在可验证推理、开放生成和 post-SFT 设置下，RRPO 与基于验证器的优化方法保持竞争力，优于弱监督基线，并在监督微调后提供额外增益。

Conclusion: RRPO 通过参考相对对比优势成功将组相对策略优化推广到非可验证场景，无需任务真实验证器即可实现有效优化，为强化学习从可验证反馈扩展到更广泛任务提供了新思路。

Classification reason: 该论文提出 RRPO（Reference-Relative Policy Optimization），是一种基于参考对比的强化学习策略优化方法，用于从可验证反馈中进行强化学习，属于多模态 RL/对齐方向。论文在 verifiable reasoning、open-ended generation 和 post-SFT 等场景下评估，与 GRPO 密切相关，因此归入 [CANONICAL] 多模态强化学习与对齐。

Importance reason: Novel RL method for multimodal alignment but lacks direct connection to user's priority subtopics like continuous models or video generation. Signals: RL from verifiable feedback, group-relative optimization, contrastive advantages, no ground-truth verifier, open-ended generation
...
### [46] [Contrastive On-Policy Distillation](https://arxiv.org/abs/2607.19046)
...
TL;DR: 提出COPD（对比式在线蒸馏）框架，通过教师模型在学生生成token时对比轻/重推理指令下的log概率差异，构造token级优势信号来指导学生模型学习更简洁高效的推理策略，在9个多模态基准上显著减少推理长度而不损失性能。


<details>
  <summary>Details</summary>
Motivation: 现有在线蒸馏（OPD）方法仅依赖token级分布匹配，缺乏对不同推理模式下token相对兼容性的显式对比信号，无法直接建模推理模式间的偏好，导致学生模型无法学习更简洁高效的推理策略。

Method: 提出COPD框架：对于学生模型生成的每个token，冻结的教师模型在同一学生状态下分别用轻推理和重推理两种对比指令进行评分，计算两者log概率之差作为token级优势信号，用于指导OPD更新。该方法可无缝集成到在线自蒸馏（OPSD）框架中，无需额外教师模型即可实现自对比蒸馏。

Result: 在9个多模态基准（涵盖推理和理解任务）上，COPD显著减少推理长度而不降低模型性能，在不同任务和模型规模上一致提升效率。集成到OPSD后同样有效。

Conclusion: 对比式在线蒸馏通过显式建模推理模式间的偏好，能够有效引导学生模型学习更简洁高效的推理策略，为多模态模型的推理效率优化提供了新思路。

Classification reason: 该论文提出对比式在线蒸馏（COPD）框架，通过教师模型对学生生成token进行偏好对比（轻/重推理指令下的log概率差异）作为优势信号来指导学生模型学习更简洁高效的推理策略。核心是推理过程中的偏好对比学习与对齐，属于多模态强化学习与对齐范畴，因此映射到[CANONICAL] subtopic "多模态强化学习与对齐"。

Importance reason: Focuses on multimodal reasoning efficiency via contrastive distillation, but lacks direct connection to priority subtopics like continuous models or video generation. Signals: multimodal reasoning, token-level advantage, reasoning length reduction, on-policy distillation, contrastive learning
...
### [47] [Cognitive Dual-Process Planning for Autonomous Driving with Structured Scene Knowledge and Verifiable Reasoning-Action Consistency](https://arxiv.org/abs/2607.19194)
...
TL;DR: 本文提出一种认知双过程规划框架，用于自动驾驶的高层规划。通过结构化思维链（S-CoT）表示场景知识，利用自动数据引擎生成标注，并引入轻量级 Arbiter 根据场景复杂度在快速预测与慢速推理之间路由。同时使用基于规则的验证器检查推理与动作的一致性，并通过 GRPO 进行优化。


<details>
  <summary>Details</summary>
Motivation: VLM 在自动驾驶规划中可生成中间推理，但存在三个问题：结构化监督成本高、常规场景中不必要的推理浪费、生成的推理与最终动作可能不一致。本文旨在解决这些问题，实现高效且一致的推理驱动规划。

Method: 提出认知双过程规划框架：(1) 设计结构化思维链（S-CoT）模式，以机器可解析的字段表示场景知识；(2) 自动数据引擎集成感知基础模型、关键路径过滤和专家 VLM，无需人工标注即可生成 S-CoT 监督；(3) 轻量级视觉 Arbiter 从多级视觉编码器特征估计场景复杂度，将输入路由到快速元动作预测或慢速结构化推理；(4) 慢路径中，确定性规则验证器检查 S-CoT 字段与最终元动作的一致性，为 GRPO 提供可验证奖励。

Result: 在 195 场景人工审核中，生成标注的 CoT 准确率达 91.8%，逻辑一致性得分（LCS）98.5%。在 574 个 NAVSIM 测试样本上，规划准确率 80.14%，LCS 97.20%，同时相比全场景慢推理降低延迟 17.39%。在外部长尾子集上的评估进一步揭示了路由和规划性能退化的条件。

Conclusion: 本文展示了如何通过自适应推理和基于规则的验证将显式场景知识用于支持 VLM 的高层规划决策，在保持高规划准确率和推理-动作一致性的同时有效降低推理延迟。

Classification reason: 该论文提出结构化思维链（S-CoT）用于自动驾驶规划，属于多模态推理与思维链（multimodal_reasoning_cot）范畴，该 [CANONICAL] 子主题涵盖多模态 CoT 和视觉推理，与本文高度匹配。

Importance reason: Strong engineering for autonomous driving planning but lacks direct connection to user's priority continuous multimodal models or autoregressive generation. Signals: structured chain-of-thought, dual-process routing, rule-based consistency verification, GRPO optimization, autonomous driving planning
...
### [48] [No Training, Better Flights: Test-Time Scaled VLMs for UAV Navigation](https://arxiv.org/abs/2607.19288)
...
TL;DR: 本文提出一种无需额外训练的测试时缩放方法，通过多候选生成、自校正和多标准评分函数，提升冻结VLM在无人机导航任务中的推理能力，实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视觉语言导航（VLN）方法通常依赖单次推理，在复杂环境中容易产生次优或不安全的轨迹。作者希望在不重新训练模型的前提下，通过测试时计算扩展来提升VLMs的导航推理能力。

Method: 首先提示模型生成多个并行候选导航计划，然后执行自校正步骤让模型重新评估初始计划，最后设计一个多标准评分函数（基于安全性、目标对齐和前向进展）对校正后的候选方案进行评分和选择。

Result: 在无人机VLN任务上取得了SOTA性能，证明无需训练即可通过测试时缩放显著提升导航规划的准确性和安全性。

Conclusion: 测试时缩放是一种简单有效的零训练方法，能够显著提升冻结VLM在复杂导航任务中的推理鲁棒性，为无人机自主导航提供了实用且高效的解决方案。

Classification reason: 该论文研究无人机导航中的视觉语言导航（VLN），通过测试时缩放（test-time scaling）和迭代自校正来增强VLMs的导航推理能力，属于多模态推理范畴，与[多模态推理与思维链]（multimodal_reasoning_cot）高度相关，该子主题涵盖推理时计算扩展（inference scaling）和多模态推理。

Importance reason: Focuses on UAV navigation, which is not a priority subtopic, and lacks direct connection to continuous language models or video generation. Signals: Test-time scaling for VLMs, UAV navigation, no training required, multi-criteria scoring, self-correction
...
### [49] [FSDBN: Foreground-Aware EEG--Visual Alignment via Dynamic Brain Networks](https://arxiv.org/abs/2607.18344)
...
TL;DR: 提出FSDBN框架，通过前景感知的EEG-视觉对齐和动态脑网络建模，实现零样本脑电到图像检索，Top-1准确率69.0%，Top-5准确率92.2%，超越此前SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有EEG视觉解码方法忽略了复杂场景中前景与背景的感知不对称性，导致背景干扰和语义对齐错误；同时EEG信号具有快速时变和非平稳空间模式，难以捕捉与视觉注意力相关的动态脑连接。

Method: 提出FSDBN框架，包含三个核心模块：(1) 语义一致显著性对齐（SCSA），在联合显著性和语义约束下分离前景区域与背景噪声；(2) 语义先验动态门控前景融合（SP-DGFF），自适应调节前景和背景特征的贡献；(3) 自适应时空脑网络建模，将EEG信号建模为动态功能连接网络，捕捉对显著前景的神经响应。

Result: 在零样本脑电到图像检索任务上，FSDBN达到69.0% Top-1准确率和92.2% Top-5准确率，显著优于此前SOTA方法。

Conclusion: FSDBN通过前景感知对齐和动态脑网络建模有效解决了EEG视觉解码中的背景干扰和时变脑连接建模问题，为脑机接口与视觉语义理解提供了新思路。

Classification reason: 该论文研究基于EEG的视觉解码，核心任务是zero-shot brain-to-image retrieval（脑电到图像的零样本检索），属于跨模态检索范畴。因此映射到[多模态理解与生成]方向下的[多模态检索]（CANONICAL）子主题。

Importance reason: The paper focuses on EEG-visual retrieval, which is outside the user's priority subtopics and lacks alignment with key boost keywords. Signals: EEG-based visual decoding, zero-shot retrieval, foreground-background alignment, dynamic brain networks, not in priority subtopics
...
### [50] [CR-Refiner: An Object-Centric Optimal Transport Reranker for Edit-Conditioned 3D Scene Retrieval](https://arxiv.org/abs/2607.19115)
...
TL;DR: CR-Refiner 是一个无需训练的 reranker，用于编辑条件3D场景检索。它通过冻结LLM解析编辑指令为结构化查询，利用非平衡最优传输对候选场景进行对象级匹配，并引入轴条件结构先验和LLM验证器，在多个基检索器上一致提升检索性能。同时发布了3D-CER基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有方法在编辑条件3D场景检索任务上各有不足：2D组合图像检索无法处理3D对象集合；3D基础编码器能编码单个对象但无法组合场景级信息；3D场景定位方法只能定位静态场景中的引用，无法跨库排序修改后的房间。因此需要一种能进行对象级组合匹配的检索方法。

Method: CR-Refiner 包含三个组件：(1) 冻结LLM将自然语言编辑解析为结构化查询实体；(2) 非平衡最优传输（unbalanced optimal transport）求解器，在1×G代价矩阵上对类别、风格、材质和几何进行匹配，允许单实体查询丢弃无关对象的质量；(3) 轴条件结构先验，为几何编辑提供尺寸关键词线索，为空间编辑提供主体-锚点方向线索。最后用LLM验证器对前三候选进行置信度精炼。

Result: 在三个不同基检索器上，CR-Refiner 在所有编辑轴上的困难子集 R@1 和 mAP@10 均有一致提升。同时发布了包含4,963个编辑条件查询、23,381个室内场景的3D-CER基准数据集。

Conclusion: CR-Refiner 通过非平衡最优传输实现了编辑条件3D场景检索中对象级组合匹配的有效性，无需额外训练即可提升多种基检索器的性能。3D-CER数据集为该任务提供了标准化评测基准。

Classification reason: 该论文研究编辑条件3D场景检索（edit-conditioned 3D scene retrieval），属于跨模态组合检索任务（文本编辑+3D场景检索），与多模态检索（multimodal_retrieval）中的组合检索（composed retrieval）方向高度吻合，因此复用该规范子主题。

Importance reason: The paper focuses on 3D scene retrieval, which is not a priority subtopic and lacks alignment with the user's key interests in continuous models, generation, or world models. Signals: 3D scene retrieval, edit-conditioned retrieval, optimal transport reranker, no training required, 3D-CER benchmark
...
### [51] [Computational Humor with Multimodal LLMs: Methods, Datasets, Evaluation, and Challenges](https://arxiv.org/abs/2607.19011)
*Tuo Liang, Zhe Hu, Disheng Liu, Jing Li, Yu Yin*

Main category: cs.AI, cs.CL, cs.MM
...
TL;DR: 本文是一篇关于多模态大语言模型（MLLM）在幽默理解领域的综述，聚焦于 meme、漫画等单图/多图视觉幽默的理解与生成，从识别、推理到生成三个能力层次系统梳理了数据集、评测协议和建模方法，并指出了当前面临的评测捷径、文化覆盖不足、证据推理薄弱和安全问题等挑战。


<details>
  <summary>Details</summary>
Motivation: 多模态幽默（如 meme、漫画）依赖于非字面机制、共享文化知识和交际意图，对 AI 系统极具挑战性。现有综述多关注通用 MLLM 或讽刺检测，缺乏对多模态幽默理解与生成的系统性梳理，因此本文旨在填补这一空白。

Method: 本文采用综述方法，以能力层次（识别→推理→生成）为组织框架，系统回顾了多模态幽默领域的数据集、评测协议和建模方法，追踪了从任务特定融合模型到基于多模态对齐、证据推理和可控生成的大模型范式的演变。

Result: 本文总结了多模态幽默理解的主要进展：数据集从单一 meme 扩展到多图叙事；评测从简单分类准确率发展到多维度评估；模型从早期融合模型演进到 MLLM 驱动的对齐与推理方法。同时指出了四大挑战：捷径式评测、文化叙事覆盖不足、证据推理薄弱、安全与版权问题。

Conclusion: 多模态幽默理解正从任务特定模型向大模型范式转变，但评测可靠性、文化多样性、证据推理和安全问题仍是主要瓶颈，未来需要更鲁棒的评测协议、更丰富的文化覆盖和更强的推理能力。

Classification reason: 该论文是一篇关于多模态幽默理解的综述，系统梳理了数据集、评测协议、建模方法等，核心贡献在于对现有基准和评测方法的总结与分析，因此映射到[CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark）。

Importance reason: This survey on multimodal humor is a niche topic that does not align with the user's priority subtopics on continuous models, video/image generation, or world models. Signals: Multimodal humor survey, MLLM evaluation, Weak evidence grounding, Cultural coverage gap, Shortcut-prone evaluation
...
### [52] [MeetingToM: Evaluating Multimodal LLMs on Theory-of-Mind Reasoning in Multi-Party Meetings](https://arxiv.org/abs/2607.19235)
...
TL;DR: MeetingToM 是一个面向多人群组会议场景的多模态心理理论（ToM）推理基准，包含主体级心理状态预测、二元级听者理解和群体级共识推理三个层次，系统评估了多个 MLLM 在整合非语言线索、推断隐藏态度和区分真实共识与伪共识方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态 ToM 基准主要关注视频问答中显式、外部可验证的信号，对潜在社交状态和群体动态的覆盖有限。多人群组会议中，线索分布在言语和行为中，且存在伪共识（表面同意掩盖私下异议）等复杂社交现象，现有基准难以评估模型在这些场景下的 ToM 推理能力。

Method: 构建 MeetingToM 基准，包含多人群组会议场景的层次化评估体系：(i) 主体级心理状态预测，(ii) 二元级听者理解，(iii) 群体级共识推理。提供统一的评估协议，对多个代表性 MLLM 进行系统分析。

Result: 系统评估揭示出现有 MLLM 在整合非语言线索、推断隐藏态度以及区分真实共识与伪共识方面存在持续性的局限性。

Conclusion: MeetingToM 作为推进多模态模型在会议场景下 ToM 推理能力的测试平台，揭示了当前 MLLM 在复杂社交推理中的关键挑战。

Classification reason: 该论文提出了 MeetingToM，一个评估多模态大模型在多人群组会议中心理理论（ToM）推理能力的基准。这属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题，因为论文核心贡献是一个新的多模态评测基准，用于评估 MLLM 在复杂社交场景中的推理能力。

Importance reason: Focuses on multimodal ToM in meetings, which is outside your priority subtopics and lacks direct connection to continuous models, video generation, or world models. Signals: multimodal_understanding_generation, theory-of-mind benchmark, multi-party meetings, pseudo-consensus, no continuous token or video generation keywords
...
### [53] [Style over Substance: A Shortcut Audit of Emotion-Description Preference Evaluation](https://arxiv.org/abs/2607.18508)
...
TL;DR: 本文对多模态情感理解评测基准 EmoPrefer 进行了系统性捷径审计，发现仅使用描述长度和生成器身份的逻辑回归即可达到与 7B 微调模型相当的性能，揭示了当前评测中存在的风格偏差问题，并提出了改进建议。


<details>
  <summary>Details</summary>
Motivation: 多模态情感理解评测中，偏好评估（如 EmoPrefer）假设模型需要跨模态理解视频才能预测人类偏好。作者怀疑这些评测存在捷径（shortcut），即模型可能不依赖视频内容而仅凭文本风格特征就能做出判断，因此需要系统审计。

Method: 作者设计了内容盲探针（content-blind probes）：(1) 仅使用描述长度和生成器身份的逻辑回归；(2) 分析生成器身份的可恢复性；(3) 长度匹配子集实验；(4) ODIN 诊断方法解耦风格捷径与内容理解。在 EmoPrefer-V2 数据集上进行系统审计。

Result: 逻辑回归（65.8 WAF）与 LoRA 微调的 7B 文本/音视频评判器（66.8 WAF）性能相当；生成器身份可从描述文本中以 99.5% 准确率恢复；人类偏好标签在 66% 的评估对上与生成器胜率先验一致；当人类标签与先验冲突时，训练后的评判器仍遵循风格先验（63-80%）；长度匹配后各模态配置无显著提升；ODIN 诊断中内容头接近随机。

Conclusion: 当前 EmoPrefer 评测分数可以在不验证描述与视频内容匹配的情况下达到，说明评测存在严重的风格捷径问题。建议未来采用源平衡配对、严格长度控制、反刻板切片报告和多标注者共识等改进措施。

Classification reason: 该论文对情感描述偏好评测基准 EmoPrefer 进行了系统性捷径审计，揭示了评测中的风格偏差问题，属于多模态评测与基准方向下的评测协议审计工作，因此映射到 [CANONICAL] subtopic "多模态评测与基准"。

Importance reason: The paper audits a specific emotion-description benchmark, which is not aligned with the user's priority subtopics on continuous models, video/image generation, or world models. Signals: Benchmark shortcut audit, Emotion-description evaluation, Style bias over content, Not in priority subtopics, No boost keywords matched
...
### [54] [MissingBench-Verified: Probing Vision-Language Models' Inability to Detect Missing Object Parts](https://arxiv.org/abs/2607.18673)
...
TL;DR: 本文提出 MissingBench-Verified 基准，系统评估 VLM 在检测图像中缺失物体部件方面的能力。实验发现十个主流 VLM 在该任务上持续且显著失败，即使提供外部工具证据也无法缓解。现有策略（工具辅助验证、自主视觉推理、更长推理时间、微调）均无明显改善，表明当前 VLM 存在根本性局限。


<details>
  <summary>Details</summary>
Motivation: VLM 已知存在幻觉问题（生成不存在的物体），但模型在物体缺失关键部件时能否正确识别尚不清楚。由于真实世界知识偏差和训练数据中缺失部件图像的稀缺性，VLM 可能难以检测到物体部件的缺失，这对工业检测、监控等实际应用至关重要。

Method: 构建 MissingBench-Verified 基准，包含物体缺失关键部件的图像。评估十个主流 VLM（如 GPT-4V、Gemini、LLaVA 等）的检测能力。进一步测试外部工具（裁剪、对比度调整等）辅助验证、自主视觉推理、延长推理时间、在更简单数据集上微调等缓解策略的效果。

Result: 所有测试模型在缺失部件检测任务上均表现出显著且一致的失败率。即使提供明确的外部工具证据，模型仍无法纠正错误判断。现有缓解策略（工具辅助、推理时间延长、微调等）均带来可忽略的改进。

Conclusion: 当前 VLM 在检测缺失物体部件方面存在根本性局限，无法通过提示工程或事后修正策略解决。需要架构层面或训练层面的干预，使模型在面对矛盾证据时能够覆盖内部预期。

Classification reason: 该论文提出了 MissingBench-Verified 基准，专门评估 VLM 在检测缺失物体部件方面的失败模式，属于多模态评测与基准方向。该基准与 [CANONICAL] subtopic "多模态评测与基准" 高度匹配。

Importance reason: Focuses on VLM failure in missing part detection, which is a niche evaluation benchmark not aligned with user's priority on continuous multimodal models or generation. Signals: VLM hallucination, missing object parts, benchmark, failure analysis, tool-assisted verification
...
### [55] [Attributes Should Come from Images, Not Class Names: Distribution-Conditioned Attribute Selection for Vision-Language Models](https://arxiv.org/abs/2607.18695)
...
TL;DR: 本文发现LLM生成的类别描述（如"草莓是红色的"）在零样本分类中缺乏视觉证据，当数据分布偏移时（如素描图）会误导模型。作者提出从目标图像集合中基于CLIP联合嵌入空间选择属性，而非依赖LLM从类别名称生成属性。该方法无需类别名称即可达到23.8%的ImageNet准确率（LLM描述仅15.5%），在分布偏移变体上同样有效，且单样本设置下比CoOp高3个点，训练时间从14小时降至不到1分钟。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用LLM为每个类别生成描述性属性来提示CLIP进行可解释零样本分类，但这些属性是基于类别名称而非实际图像生成的，导致当数据分布发生偏移时（如真实照片→素描），属性描述与图像内容不匹配，严重损害分类性能。

Method: 提出Distribution-Conditioned Attribute Selection (DCAS)方法：首先构建一个大规模属性池（可从LLM生成或外部知识库获取），然后在CLIP的联合嵌入空间中计算每个属性与目标图像集合的匹配分数，为每个类别保留得分最高的属性作为提示。该方法完全不需要类别名称参与属性选择，属性完全由数据驱动。

Result: 在ImageNet上，无类别名称的属性提示达到23.8%（LLM描述仅15.5%）；在ImageNet-Sketch、ImageNet-R、ImageNet-A、ImageNet-V2四个分布偏移变体上同样保持优势；单样本设置下比CoOp高3个点，训练时间从14小时降至不到1分钟。此外，所选属性集可作为数据集的可读摘要，用于描述分布偏移。

Conclusion: 属性应该来自图像而非类别名称——基于数据分布选择属性比依赖LLM从标签生成属性更有效、更鲁棒，且更具可解释性。该方法为可解释零样本分类和数据集分布偏移分析提供了新的思路。

Classification reason: 该论文研究如何为CLIP等视觉语言模型选择更好的属性描述（attribute prompts），以提升零样本分类性能，并提出了基于数据分布的条件属性选择方法。论文的核心贡献在于提出了一种新的评测视角（属性提示的质量评估）和可解释性分析方法（用属性集描述数据集分布偏移），属于多模态模型评测与可解释性分析的范畴，因此映射到[多模态评测与基准]（multimodal_benchmark）这一规范子主题。

Importance reason: Addresses a key flaw in LLM-based attribute generation for zero-shot classification but is not aligned with user's priority subtopics. Signals: zero-shot classification, CLIP, attribute selection, distribution shift, interpretability
...
### [56] [PathAgentBench: Benchmarking Evidence-Seeking Vision-Language Models on Whole-Slide Pathology Image](https://arxiv.org/abs/2607.19261)
...
TL;DR: PathAgentBench是一个针对病理全切片图像（WSI）上证据寻求型视觉语言模型的评测基准，包含1,822张TCGA WSI和17,135条由十位病理学家标注的诊断路径，评估模型在证据解释、证据验证、证据获取和证据整合四个维度的能力。实验表明，现有模型在多尺度推理和跨模态匹配上表现尚可，但在诊断区域定位和自主全切片探索方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有病理学基准大多在预裁剪的图块或预提取的切片特征上评估模型，未能测试模型直接从千兆像素级全切片图像中获取证据的能力。因此需要一个新的基准来系统评估VLM在WSI上的证据寻求能力。

Method: 构建了一个诊断树结构，将不同放大倍数下的嵌套区域与尺度特定的发现和病理诊断路径关联起来。基准包含四个互补能力：图像到文本匹配（证据解释）、文本到图像检索（证据验证）、诊断区域定位（证据获取）和多尺度推理（证据整合）。使用1,822张TCGA WSI和10位病理学家标注的17,135条诊断路径，并额外收集190张乳腺癌WSI用于评估自主全切片探索。

Result: 领先的开源模型在多尺度推理上达到93%以上准确率，跨模态匹配任务上超过50%准确率。但诊断区域定位最具挑战性：最佳文本引导的mIoU低于0.09，甚至不如简单的中心启发式方法。自主探索中，无条件命中率从低放大倍数的0.522下降到中放大倍数的0.185和高放大倍数的0.020。

Conclusion: PathAgentBench揭示了当前VLM在从WSI直接获取证据方面存在显著差距——模型在基于已有证据进行推理时表现良好，但在自主定位和获取诊断相关区域方面能力严重不足。该基准为衡量和改进病理学证据寻求型模型提供了统一框架。

Classification reason: 该论文提出了PathAgentBench，一个用于评估病理全切片图像上证据寻求型视觉语言模型（VLM）的基准，涵盖多尺度推理、跨模态匹配、诊断区域定位等能力。这属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。

Importance reason: The paper is a specialized pathology benchmark with no overlap with the user's priority subtopics on continuous multimodal models, video/image generation, or world models. Signals: Pathology-specific benchmark, No continuous language or multimodal focus, No video or image generation, No world model or efficient inference, Negative keyword "medical diagnosis" applies
...
### [57] [Continual Video-MLLM Adaptation over Evolving Domains](https://arxiv.org/abs/2607.18716)
...
TL;DR: 本文提出 DAER（Distribution-Aware Expert Routing），一种参数高效的持续学习框架，用于视频多模态大模型在演化域上的适应。DAER 通过冻结预训练骨干、维护域隔离的轻量专家模块，并引入域内分布感知路由和域间原型匹配机制，有效缓解跨域干扰和灾难性遗忘。在十个 VidQA 数据集构建的域增量基准上，DAER 在两个强 Video-MLLM 骨干上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实部署中视频数据往往来自持续演化的异构域，现有视频 MLLM 在适应新域时容易遗忘先前学到的能力。现有的持续学习方法通常依赖共享适应空间，导致严重的跨域干扰和灾难性遗忘。因此需要一种能解耦域特定适应与通用多模态知识的方法。

Method: DAER 框架包含以下核心组件：(1) 冻结预训练 Video-MLLM 骨干，维护域隔离的轻量专家模块；(2) 域内分布感知路由机制，使用 MMD（最大均值差异）将输入匹配到专家级原型库；(3) 域间路由机制，在判别性子空间中进行原型匹配以实现无任务标识的域识别；(4) 自适应域合并提升参数可扩展性；(5) 两阶段优化策略稳定持续学习中的专家特化。

Result: 在由十个 VidQA 数据集构建的域增量基准上，DAER 在两个强 Video-MLLM 骨干（如 Video-LLaVA 等）上均一致优于现有持续学习方法，有效缓解了灾难性遗忘并提升了跨域适应能力。

Conclusion: DAER 通过域隔离专家和分布感知路由机制，为视频 MLLM 在持续演化域上的适应提供了一种有效的参数高效方案，在保持预训练知识的同时实现了良好的域特化能力。

Classification reason: 该论文提出 DAER 框架，通过参数高效的专家路由机制实现视频 MLLM 在持续演化域上的适应，核心贡献在于高效微调（PEFT）和持续学习场景下的参数隔离与路由。这与 [CANONICAL] 多模态高效化与轻量化（efficient_multimodal）高度匹配，涉及 PEFT、token 压缩、参数高效适配等主题。

Importance reason: Addresses continual learning for video MLLMs, a practical challenge, but the core method (expert routing) is not a top priority subtopic. Signals: continual learning, video MLLM, domain adaptation, parameter-efficient, expert routing
...
### [58] [PathReportEval: A Systematic Benchmark for Pathology Report Generation](https://arxiv.org/abs/2607.18448)
...
TL;DR: 本文提出 PathReportEval，一个用于病理报告生成的标准化评测基准和框架。该基准在三个数据集上评估四种方法，使用三种病理基础编码器，并引入临床报告质量评分（CRQS）作为基于临床事实的评估指标。实验表明传统NLG指标与临床正确性弱对齐，而CRQS能捕捉临床上有意义的差异。


<details>
  <summary>Details</summary>
Motivation: 病理报告生成领域进展难以衡量，因为现有研究使用异构数据集、模型设置和评估协议，且常用NLG指标（BLEU、ROUGE、METEOR）主要奖励词汇相似性，无法检测临床重要错误（如遗漏诊断、幻觉发现、肿瘤属性不一致）。

Method: 构建标准化基准，统一预处理、特征提取、训练、解码和评估流程；提出CRQS指标，将参考报告和生成报告映射为结构化临床属性，从临床事实覆盖、关键信息召回、幻觉率和临床不一致性四个维度评估；在TCGA、HistAI、REG 2025三个数据集上使用CONCHv1.5、UNI2-h、H-Optimus-1三种编码器评估四种代表性方法。

Result: 传统NLG指标与临床正确性弱对齐，常高估报告质量；CRQS能揭示模型和编码器之间临床有意义的差异，而这些差异是词汇指标无法捕捉的。

Conclusion: PathReportEval基准、公共即插即用框架和CRQS指标为病理报告生成的严格评估建立了可复现的基础，推动该领域向临床可靠的评估标准发展。

Classification reason: 该论文提出了病理报告生成的标准化评测基准（PathReportEval）和临床报告质量评分（CRQS），属于生成模型评测与数据集方向。该论文匹配 multimodal_understanding_generation 方向下的 [CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark），因为其核心贡献是建立评测框架、标准化评估协议和新的临床指标。

Importance reason: Focuses on pathology report generation, a niche medical domain not aligned with user's priority subtopics. Signals: Medical domain, weak alignment with user's continuous multimodal or video generation focus, negative keyword "clinical" present, benchmark for specialized task
...
### [59] [Two-Level Meta-Rubrics for Evaluating Open-Ended Generation: GAMUT, a Benchmark for Factual Completeness](https://arxiv.org/abs/2607.19322)
...
TL;DR: 本文提出了一种双层元评分标准（meta-rubric）框架用于评估开放式生成的事实完整性，并实例化为GAMUT基准。该框架通过结构化元评分标准捕获所需内容的组织和重要性，再机械编译为二元可机器评分的检查清单，由LLM裁判可靠评分。基于真实可穿戴设备图像构建了1,813个问题，覆盖10个领域，评估了14个前沿模型，最佳得分仅58.7%。


<details>
  <summary>Details</summary>
Motivation: 现有长文本生成的事实性评估主要关注精确性（precision），即检测模型生成的陈述是否正确，但忽略了事实完整性（factual completeness）——回答是否包含了所有应该包含的信息。衡量完整性需要枚举完整答案应包含的全部事实，而这些事实往往不是简单的扁平列表，而是涉及开放集合、有序过程和事实间关系，现有方法难以处理。

Method: 提出双层元评分标准框架：(1) 上层为结构化元评分标准（meta-rubric），捕获所需内容的组织结构和重要性；(2) 下层为机械编译生成的扁平化二元评分标准（flat checklist of binary rubrics），由LLM裁判可靠评分。基于真实可穿戴设备图像构建了1,813个问题，每个问题配有专家验证的基于证据的评分标准。

Result: 评估了14个前沿和开源模型，最佳得分仅58.7%（Gemini 3.1 Pro），表明该基准具有挑战性。基准具有高度区分性，且对裁判模型的选择具有鲁棒性。

Conclusion: GAMUT填补了长文本生成事实完整性评估的空白，双层元评分标准框架为开放式生成评估提供了可扩展的范式。该框架是模态无关的，可推广到多模态场景。

Classification reason: 该论文提出了GAMUT基准，用于评估长文本生成的事实完整性（factual completeness），属于生成模型评测范畴。虽然论文主要关注文本生成，但其框架被设计为模态无关的（modality-agnostic），且论文标题提及"Multimodal Factuality"。该论文与多模态理解与生成方向下的[CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark）高度匹配，因此复用该现有分类。

Importance reason: Focuses on evaluation methodology for factual completeness, not on core generative model architectures or priority subtopics. Signals: Evaluation benchmark, Factual completeness, Open-ended generation, LLM judge, Not generative model
...
### [60] [ZeroSplat: Generalized Referring Segmentation in 3D Gaussian Splatting](https://arxiv.org/abs/2607.18801)
...
TL;DR: ZeroSplat 提出了一种无需训练、零特征存储的通用指代3D高斯泼溅分割框架，通过将2D VLM先验经多视图几何约束提升到3D空间，实现任意数量目标（0/1/N）的动态分割，并构建了GR-LERF和GR-ScanNet两个新基准。


<details>
  <summary>Details</summary>
Motivation: 现有指代3DGS方法仅支持单目标查询，无法处理真实指令中目标数量不确定（0/1/N）的模糊性，且存在缺乏3D点级理解、需要逐场景优化存储大量语义特征等瓶颈。

Method: ZeroSplat 提出训练无关、零特征框架：利用2D VLM（如Grounding DINO）生成多视图2D指代分割掩码，通过多视图几何约束（3DGS点云投影一致性）将2D先验提升到3D空间，实现点级语义理解，无需额外特征存储或逐场景优化。

Result: 在GR-LERF和GR-ScanNet基准上，ZeroSplat在通用指代分割和单目标场景下均显著优于现有方法，同时保持极高的效率（无需训练、无需额外特征存储）。

Conclusion: ZeroSplat 有效解决了现有指代3DGS方法在通用场景下的技术瓶颈，为3D场景中的语言引导分割提供了一种高效、灵活的解决方案。

Classification reason: 该论文研究基于3D高斯泼溅的指代分割，涉及3D场景中的语言引导理解与分割，属于空间与3D多模态推理方向，因此映射到 [CANONICAL] subtopic "空间与 3D 多模态推理"。

Importance reason: Novel zero-shot 3D segmentation framework with strong multi-view geometry, but only tangentially touches user's core continuous multimodal and generation priorities. Signals: 3D Gaussian Splatting, referring segmentation, zero-shot, multi-view geometry, VLM priors
...
### [61] [DobicVLM: Aligning Chest X-Ray Report Generation with Clinically-Grounded Programmatic Rewards via Group Relative Policy Optimization](https://arxiv.org/abs/2607.18988)
...
TL;DR: DobicVLM 是一个结合监督微调与 GRPO 强化学习的视觉语言模型，用于胸部X光报告生成。它使用可解释的基于规则的临床奖励组件（结构验证、解剖清单、语义相似度、长度约束）替代神经奖励模型，在私有临床数据集上训练，并在盲审评估中优于 Gemini 2.5 Flash。


<details>
  <summary>Details</summary>
Motivation: 自动化胸部X光报告生成面临结构遵循性、解剖完整性和语义忠实性方面的挑战。现有方法缺乏透明的对齐机制，尤其在资源受限环境中难以部署。

Method: 基于 MedGemma-4B 进行监督微调，然后使用 GRPO（Group Relative Policy Optimization）结合基于规则的临床奖励组件（结构验证、解剖清单、语义相似度、长度约束）进行强化学习对齐，无需神经奖励模型。

Result: 在69个保留病例的盲审专家评估中，DobicVLM 在大多数标准上优于 Gemini 2.5 Flash，印象准确率最高（27.2%），医学术语使用率最高（86.5%），在完整性和转诊建议方面略有不足。

Conclusion: GRPO 结合可解释的基于规则的奖励函数是一种有效的透明对齐方法，适用于资源受限的医学影像报告生成场景。

Classification reason: 该论文提出 DobicVLM，一个用于胸部X光报告生成的视觉语言模型，结合了监督微调与 GRPO 强化学习。这属于通用视觉语言模型（VLM/MLLM）在医学领域的应用，与 [CANONICAL] 子主题 "视觉语言模型与 MLLM" 高度匹配。

Importance reason: Focuses on medical report generation, which is outside the user's priority subtopics and contains negative keywords. Signals: medical domain, chest X-ray, clinical rewards, GRPO, low relevance to continuous models or video generation
...
### [62] [OmniReasoner: Thinking with Long Audio-Video via Native Tool Use](https://arxiv.org/abs/2607.19339)
...
TL;DR: OmniReasoner 是一个面向长音视频推理的全模态 LLM 后训练框架，通过监督微调与强化学习让模型学会在回答前调用 zoom-in 工具对关键时空区域进行高保真检查，并引入 TimeAnchor 机制保证跨采样粒度的时序一致性，以及时序增强数据引擎自动合成训练轨迹。


<details>
  <summary>Details</summary>
Motivation: 长音视频推理中，关键证据往往稀疏、跨模态且分布不均，若对整个视频保持统一高保真输入则计算成本过高。现有全模态 LLM 缺乏在推理过程中动态选择高保真区域的能力。

Method: 提出 OmniReasoner 框架：(1) 全局预览阶段使用低成本的稀疏采样构建全局概览；(2) 模型通过 SFT+RL 学习决定是否及何时调用 zoom-in 工具，对指定时间区间进行高保真视觉和音频检查；(3) 引入 TimeAnchor 机制，将工具的时间参数锚定在原始时间轴上而非特定帧索引，保证跨采样粒度的 round-trip 一致性；(4) 构建时序增强数据引擎，通过视频编辑与合成自动生成工具使用的后训练轨迹，避免昂贵的人工标注。

Result: 在全模态和视频基准测试上，OmniReasoner 在答案准确性和时序定位能力上均取得提升，同时将高保真计算集中在信息丰富的区域，提高了计算效率。

Conclusion: 通过让全模态 LLM 学会在推理过程中主动调用工具进行选择性高保真观察，可以有效解决长音视频推理中稀疏关键证据与计算成本之间的矛盾，为长视频多模态理解提供了一种高效且可训练的范式。

Classification reason: 该论文提出 OmniReasoner，一个面向长音视频推理的全模态 LLM 后训练框架，通过工具调用（zoom-in）实现稀疏高保真计算。这属于通用视觉语言模型/MLLM 架构与多模态指令微调范畴，因此映射到 [CANONICAL] subtopic "视觉语言模型与 MLLM"。

Importance reason: Strongly aligns with user's priority on continuous multimodal models and MLLM reasoning, with novel tool-use and TimeAnchor mechanisms for long-form audio-video. Signals: multimodal_understanding_generation, MLLM reasoning, tool-use post-training, long audio-video, TimeAnchor
...
### [63] [One Model, Many Graphs: Learning over Attributed Graphs across Heterogeneous Modalities with Vision-Language Models](https://arxiv.org/abs/2607.19128)
...
### [64] [Open-Vocabulary Gaze Object Prediction: Benchmark and Method](https://arxiv.org/abs/2607.18827)
...
TL;DR: 本文提出开放词汇注视目标预测（OVGOP）任务，构建了包含86个野外类别的DiSG基准数据集，并提出一个基于文本驱动目标发现和注视引导选择的框架，同时引入梯度信息选择微调（GIST）策略，在开放词汇和封闭词汇设置下均取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有注视目标预测方法局限于封闭词汇范式，在固定标签空间上训练并在场景特定数据集上评测，无法应对真实世界中注视目标的长尾分布和未见类别问题。

Method: 提出一个两阶段框架：1）利用文本驱动的开放词汇目标检测器（如Grounding DINO）定位候选物体；2）通过注视引导选择模块（基于注视热图与候选框的交互）从候选物体中选出注视目标。此外，提出梯度信息选择微调（GIST）策略，选择性更新与类别词汇最相关的参数，以更好地捕捉跨类别语义知识。

Result: 在DiSG基准上，所提方法在开放词汇设置下表现有效，同时在传统封闭词汇设置下也优于现有方法。

Conclusion: 开放词汇注视目标预测是迈向真实场景人机交互理解的重要一步，DiSG基准和所提框架为后续研究提供了基础。

Classification reason: 该论文研究注视目标预测（Gaze Object Prediction），本质上是一个视觉问答/视觉定位任务——从图像中定位并识别人类注视的物体。这与多模态理解中的视觉问答（VQA）和视觉定位任务高度相关，因此归入 canonical 子主题 "视觉问答与文档理解"（multimodal_vqa_doc）。

Importance reason: The paper focuses on gaze object prediction, which is not aligned with the user's priority subtopics like continuous language models, video generation, or world models. Signals: Open-vocabulary gaze prediction, new benchmark DiSG, gradient-informed tuning, closed-vocabulary outperformance, no direct link to user's core interests
...
### [65] [OPD-IAD: From Language Judgment to Industrial Anomaly Detection via On-Policy Self-Distillation](https://arxiv.org/abs/2607.18850)
...
TL;DR: OPD-IAD提出了一种基于策略内自蒸馏的LVLM工业异常检测框架，通过将特权缺陷证据蒸馏到模型自身的策略内判断轨迹上，并结合语言引导的视觉锚定机制，在语言判断指导下生成像素级异常图，实现了语言引导而不主导视觉响应的异常定位。


<details>
  <summary>Details</summary>
Motivation: 现有基于LVLM的工业异常检测方法虽然能提供图像级异常判断和可解释的缺陷推理，但难以从生成的语言判断中产生精确的像素级异常图。语言判断往往主导视觉响应，导致像素级定位精度不足。

Method: 提出OPD-IAD框架，包含两个核心组件：(1) 证据特权密集策略内自蒸馏（Evidence-Privileged Dense On-Policy Self-Distillation），将特权缺陷证据蒸馏到模型自身的策略内判断轨迹上，使最终生成的判断在密集监督下学习；(2) 语言引导的视觉锚定（Language-guided Visual Anchoring），利用判断重前向机制在最终判断条件下重新编码图像和问题作为语义锚点，通过与密集视觉特征对比生成异常图。

Result: 在多个工业异常检测基准上，OPD-IAD在LVLM-based方法中取得了最佳综合性能，在大多数图像级、像素级和QA指标上领先。

Conclusion: 语言判断可以提供紧凑的语义引导，而密集视觉特征仍然是像素级评分的基础，从而让语言引导异常定位而不让语言质量直接决定像素级响应。

Classification reason: 该论文使用大视觉语言模型（LVLM）进行工业异常检测（IAD），核心任务是图像级别的异常判断和像素级别的异常定位，属于视觉问答与理解范畴。虽然应用场景是工业异常检测，但方法本质上是LVLM在VQA任务上的扩展（图像+问题→判断+定位），因此映射到[CANONICAL] subtopic "视觉问答与文档理解"（multimodal_vqa_doc）。

Importance reason: The paper focuses on industrial anomaly detection, which is not a priority subtopic and is not boosted by user keywords. Signals: Industrial anomaly detection, LVLM-based, Self-distillation, Language-guided visual anchoring, Pixel-level localization
...
### [66] [TAP-RAG: Task-Aware Policy Control for Long-Document Multimodal Question Answering](https://arxiv.org/abs/2607.18917)
...
TL;DR: TAP-RAG 提出了一种任务感知策略控制的多模态 RAG 框架，通过 Task-Aware Policy Controller 预测任务先验并生成可执行策略，结合查询引导的流扩散和视觉增强模块，在长文档多模态问答中显著提升了准确率。


<details>
  <summary>Details</summary>
Motivation: 现有长文档多模态 RAG 系统通常采用查询无关的证据使用策略，无法针对不同查询类型（如文本、视觉、布局、全局结构等）自适应地调整证据获取方式，导致检索和推理效果受限。

Method: TAP-RAG 包含三个核心组件：1) Task-Aware Policy Controller (TAPC) 预测任务先验并估计视觉/局部/全局证据信号，生成可执行策略；2) Task-Aware Query-Guided Flow Diffusion (TA-QFD) 在多模态文档图上扩展文本和结构证据；3) Task-Aware Visual Enhancement (TAVE) 在需要视觉或布局证据时选择性检查页面图像。最后通过受保护的合成阶段融合文本、视觉和结构证据，并在证据不足时弃权。

Result: 在 DocBench 和 MMLongBench-Doc 上，TAP-RAG 分别达到 70.2 和 46.7 的准确率，相比匹配的多模态 RAG 基线分别提升 +9.1 和 +4.5 个百分点，取得最佳整体准确率。

Conclusion: 任务感知的策略控制能够有效提升长文档多模态问答中证据获取的针对性和准确性，不同查询类型需要差异化的证据行为，该框架为多模态 RAG 系统提供了更灵活的自适应方案。

Classification reason: 该论文研究长文档多模态问答（Long-Document Multimodal QA），属于视觉问答与文档理解范畴，与 [CANONICAL] subtopic "multimodal_vqa_doc（视觉问答与文档理解）" 高度匹配。

Importance reason: Focuses on RAG for long-document QA, not on the user's priority subtopics like continuous models or video generation. Signals: Multimodal RAG, Task-aware policy, Long-document QA, Visual enhancement, Not autoregressive or continuous
...
### [67] [Context-structured Video Anomaly Detection with Large Vision-Language Models](https://arxiv.org/abs/2607.19077)
...
TL;DR: 本文提出CSI-VAD，一种无需训练的视频异常检测方法，通过将视频分解为环境、物体、时间三种上下文，分别用大视觉语言模型进行上下文特定的异常推理，在UCF-Crime和UBnormal上取得有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 视频异常检测中异常事件多样且罕见，标注成本高。现有基于大视觉语言模型的无训练方法大多对整个视频进行整体推理，可能遗漏上下文特定的异常线索。

Method: 将视频分解为三种上下文（环境、物体、时间），分别构建独立的推理分支，每个分支使用VLM基于该上下文的视觉线索进行异常判断，无需预定义文本提示或数据集特定调优。

Result: 在UCF-Crime和UBnormal数据集上，CSI-VAD一致优于直接整体推理的基线方法，并与现有方法取得有竞争力的性能。

Conclusion: 结构化上下文分解策略能够有效提升无训练视频异常检测的性能，展示了利用VLM进行细粒度上下文推理的潜力。

Classification reason: 该论文使用大视觉语言模型进行无训练的视频异常检测，属于视觉问答与理解范畴。虽然任务具体为异常检测，但核心方法是利用VLM进行视频级视觉推理与判断，与"视觉问答与文档理解"（multimodal_vqa_doc）这一广泛主题相关，该主题涵盖VQA及各类视觉理解任务。

Importance reason: The paper focuses on video anomaly detection, which is not a priority subtopic and lacks alignment with the user's key interests in generative models and continuous representations. Signals: Training-free video anomaly detection, Context decomposition, Large vision-language models, UCF-Crime benchmark, No predefined text prompts
