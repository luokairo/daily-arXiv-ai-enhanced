...
- [连续语言模型与连续多模态模型](#direction-continuous_language_multimodal) [Total: 10]
  ...
  - [连续视觉 Tokenizer 与表征自编码器](#direction-continuous_language_multimodal-continuous_visual_tokenizer) [Total: 2]
  - [连续语言模型与语言流](#direction-continuous_language_multimodal-continuous_language_flow) [Total: 7]
- [世界模型](#direction-world_model) [Total: 49]
  - [具身世界模型与规划控制](#direction-world_model-embodied_world_model) [Total: 38]
  - [可交互与物理世界建模](#direction-world_model-interactive_physical_world) [Total: 5]
  - [生成模型评测与数据集](#direction-world_model-generation_benchmark) [Total: 1]
  - [视频世界模型](#direction-world_model-video_world_model) [Total: 5]
- [多模态生成](#direction-multimodal_generation) [Total: 72]
  - [3D 生成与重建](#direction-multimodal_generation-three_d_generation) [Total: 14]
  ...
  - [生成模型加速与高效化](#direction-multimodal_generation-efficient_generation) [Total: 7]
  - [生成模型对齐与奖励](#direction-multimodal_generation-generation_alignment) [Total: 1]
  - [生成模型评测与数据集](#direction-multimodal_generation-generation_benchmark) [Total: 15]
...
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 127]
  - [多模态安全与攻防](#direction-multimodal_understanding_generation-multimodal_safety_attack) [Total: 9]
  ...
  - [多模态检索](#direction-multimodal_understanding_generation-multimodal_retrieval) [Total: 11]
  - [多模态评测与基准](#direction-multimodal_understanding_generation-multimodal_benchmark) [Total: 32]
  - [多模态高效化与轻量化](#direction-multimodal_understanding_generation-efficient_multimodal) [Total: 15]
  ...
  - [视觉语言模型与 MLLM](#direction-multimodal_understanding_generation-vlm_mllm_general) [Total: 17]
  - [视觉问答与文档理解](#direction-multimodal_understanding_generation-multimodal_vqa_doc) [Total: 5]
  ...
  - [连续视觉 Tokenizer 与表征自编码器](#direction-multimodal_understanding_generation-continuous_visual_tokenizer) [Total: 2]
...
### [1] [Latent Thought Credit: Multi-Answer Credit Assignment for Latent Reasoning](https://arxiv.org/abs/2608.01593)
...
Importance: 值得关注 (72.12) - 未入选精读

TL;DR: 提出 Latent Thought Credit (LTC)，一种针对连续潜空间推理的分层信用分配框架。通过采样多个 latent thoughts、固定上下文后对多个答案求平均来估计 thought 级期望奖励，并利用 thought 级与 answer 级优势分别优化潜思维阶段和答案阶段，配合优势加权 thought-matching 目标使策略复现高信用潜思维。在数学推理与 STEM 多选题上取得最佳平均准确率。


<details>
  <summary>Details</summary>
Motivation: 连续潜空间推理（latent reasoning）允许语言模型在连续表征中进行中间推理，而非完全外部化为离散思维链。然而，从仅含答案的奖励中为这些潜思维分配信用非常困难：单个最终答案混合了思维质量与答案采样噪声，难以判断哪些潜思维真正贡献了正确结果。

Method: LTC 采用分层信用分配框架：对每个 prompt 采样多个 latent thoughts，固定每个 thought 之后的上下文，通过在该固定上下文上生成多个答案并求平均来估计 thought 级期望奖励。使用 thought 级优势优化潜思维阶段，answer 级优势优化答案阶段，并引入优势加权 thought-matching 目标帮助策略复现高信用潜思维。整体在 GRPO 风格的 on-policy 训练框架中实现。

Result: 在数学推理和 STEM 多选题任务上，LTC 在对比方法中取得最佳平均准确率。消融实验与固定上下文诊断表明，多答案估计降低了奖励估计误差，并缓解了模糊或错误的 thought 级信用分配问题。

Conclusion: LTC 通过多答案平均估计 thought 级奖励，有效解决了连续潜空间推理中从答案奖励到潜思维信用分配的难题，提升了潜推理模型的训练效果与推理准确率。

Classification reason: 论文研究连续潜空间中的 latent reasoning（潜空间思维），并提出对 latent thought 进行 credit assignment 的框架，属于连续多模态推理与潜空间思维这一 CANONICAL 子主题。

Importance reason: Directly advances continuous latent reasoning with a novel credit-assignment method, highly aligned with the user's top-priority continuous language/multimodal modeling direction. Signals: continuous latent reasoning, credit assignment, GRPO-style training, thought-level advantages, latent thought matching
...
### [2] [SPAE: Spectrally Guided Autoencoder for Pretrained Visual Latents](https://arxiv.org/abs/2608.01306)
...
TL;DR: SPAE 是一种谱引导的自编码器框架，用于适配预训练视觉基础模型（VFM）的 latent 空间以支持图像生成。它通过紧凑瓶颈蒸馏稳定语义信息并抑制高频成分，配合通道级掩码策略解耦语义与高频细节，从而改善 DiT 生成 latent 与编码器 latent 的对齐。


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型的 latent 语义丰富，适合视觉理解，但直接用于图像生成时存在困难：DiT 生成的 latent 与编码器 latent 存在谱失配，尤其在高频成分上。通道级谱分析进一步发现高频成分在 latent 通道间弥散分布并与语义信息纠缠，导致 latent 空间难以被 DiT 建模。

Method: SPAE 采用紧凑瓶颈结构来蒸馏稳定的语义信息并抑制高频成分，从而改善 DiT 生成 latent 与编码器 latent 的对齐；同时引入通道级掩码策略，促进瓶颈通道中语义信息与高频细节的解耦。

Result: 实验表明 SPAE 在视觉理解、生成质量与重建保真度之间取得了良好的平衡。

Conclusion: SPAE 通过谱引导的 latent 适配有效解决了 VFM latent 难以直接建模的问题，为理解与生成统一的连续视觉表征提供了可行方案。

Classification reason: 论文提出 SPAE，一种面向预训练视觉 latent 的谱引导自编码器，用于改进 VFM latent 的可生成性，属于连续视觉 tokenizer/表征自编码器方向，与 CANONICAL 子主题 continuous_visual_tokenizer 高度吻合。

Importance reason: Directly targets the user's top-priority continuous visual tokenizer/representation autoencoder subtopic with a novel spectral-guided latent adaptation method for generation. Signals: continuous visual token, representation autoencoder, visual latent flow, DiT generation, spectral analysis
...
### [3] [Self-supervised DXA representations encode multi-system disease risk, biological aging and heritability](https://arxiv.org/abs/2608.02208)
...
TL;DR: 本文提出 LeDXA，一个基于 JEPA 架构的自监督视觉模型，将全身 DXA 扫描图像转化为系统性健康表征。在 11,540 张未标注图像上从零训练，在 UK Biobank 47,400 张外部扫描上验证，其疾病与生物标志物预测能力优于传统 DXA 测量和 DINOv3，并能预测时序年龄、生物学年龄差距及遗传力。


<details>
  <summary>Details</summary>
Motivation: 全身 DXA 扫描常规用于测量骨密度和区域身体成分，但其丰富的空间结构信息未被充分利用。作者希望证明自监督学习能将原始 DXA 图像转化为系统性健康表征，挖掘传统读数丢弃的预后信息。

Method: 采用 JEPA（联合嵌入预测架构）视觉模型 LeDXA，通过预测 latent 表征而非重建像素进行自监督学习。在 11,540 张未标注 Human Phenotype Project 扫描上从零训练，并在内部及 47,400 张外部 UK Biobank 扫描上评估。

Result: LeDXA 在跨队列疾病与生物标志物预测上优于扫描仪导出的 DXA 测量和 DINOv3，尽管训练图像少约 15 万倍、参数少近 40 倍。在 4.3 年随访中改善发病预测（髋/膝骨关节病和 2 型糖尿病增益最大），外部预测时序年龄 r=0.88、MAE=2.90 年，生物学年龄差距与疾病负担及 45% 更高死亡风险相关，且 HRT 后女性该差距下降。GWAS 恢复已知位点，嵌入遗传力高于 DINOv3。

Conclusion: DXA 图像中包含传统读数丢弃的预后信息，可通过相对较少的数据和适度的计算量以自监督方式学习，为医学影像表征学习提供了新范式。

Classification reason: 论文提出 LeDXA，基于 JEPA 联合嵌入预测架构，在连续潜空间中学习 DXA 医学影像的视觉表征（预测 latent 而非重建像素），属于连续视觉表征自编码器/连续视觉 tokenizer 范畴，映射到 CANONICAL 子主题 continuous_visual_tokenizer。

Importance reason: Medical imaging SSL paper with no direct connection to the user's core focus on continuous multimodal models, video generation, or world models. Signals: self-supervised learning, JEPA, medical imaging, disease prediction, biological aging
...
TL;DR: DiffusionGemma 是一个实验性开源权重语言模型，使用离散扩散以极高速度生成文本。它不再逐 token 解码，而是并行迭代精炼 256 个 token 的块，避免了传统自回归 LLM 的顺序解码瓶颈。该模型通过对 Gemma 4 MoE 模型（3.8B 激活/25.2B 总参数）进行微调获得，两阶段训练管线使用不到原始 AR 模型总训练 token 预算的 10%。第一阶段用 SFT 教授双向去噪，第二阶段结合强化学习与采样器蒸馏以同时提升生成质量和推理效率。DiffusionGemma 在速度-能力权衡上建立了新的 Pareto 前沿，平均每次前向传播生成约 20 个 token，在单张 H100 上达到约 1500 token/秒，显著快于即使采用最先进投机解码的 AR 模型。它还保留了思考模式、多模态输入和长上下文支持，并仍能进行 AR 生成，仅轻微性能下降，暗示了混合扩散-AR 解码的路径。


<details>
  <summary>Details</summary>
Motivation: 传统自回归大语言模型逐 token 顺序解码存在严重的推理瓶颈，生成速度受限。作者希望探索一种能够并行生成文本的替代范式，以大幅提升推理吞吐量，同时保持模型能力。

Method: 采用离散扩散（discrete diffusion）范式，并行迭代精炼 256 个 token 的块。通过对 Gemma 4 MoE 模型进行两阶段微调获得：第一阶段使用监督微调（SFT）教授双向去噪能力；第二阶段结合强化学习与采样器蒸馏，联合优化生成质量与推理效率。

Result: DiffusionGemma 平均每次前向传播生成约 20 个 token，在单张 NVIDIA H100 上达到约 1500 token/秒，显著快于采用最先进投机解码的 AR 模型。在速度-能力权衡上建立了新的 Pareto 前沿，并保留了思考模式、多模态输入和长上下文支持。

Conclusion: 离散扩散语言模型能够以远高于自回归模型的速度生成高质量文本，且通过微调而非从零训练即可获得。模型仍保留 AR 生成能力，仅轻微性能下降，为混合扩散-AR 解码提供了可行路径。

Classification reason: 该论文提出 DiffusionGemma，使用离散扩散（discrete diffusion）并行生成文本，属于扩散语言模型/非自回归语言生成范式，与 CANONICAL 子主题"连续语言模型与语言流"（continuous_language_flow）高度相关，该子主题涵盖扩散语言模型与 flow matching language model。

Importance reason: Directly advances the top-priority continuous/diffusion language model direction with a practical, high-speed open-weight model. Signals: discrete diffusion LM, parallel block decoding, 1500 tok/s, Gemma 4 MoE fine-tune, RL + sampler distillation
...
### [5] [DeltaFlow: Noise-Adaptive Bidirectional Gated Delta Networks for Embedded Language Flows](https://arxiv.org/abs/2608.01240)
...
TL;DR: DeltaFlow 提出噪声自适应的双向 Gated Delta Network 骨干，用于 ELF 连续语言去噪，替代全非因果注意力，在降低困惑度的同时实现 2.72 倍吞吐加速。


<details>
  <summary>Details</summary>
Motivation: Embedded Language Flows (ELF) 依赖全非因果注意力进行迭代去噪，每一步采样都重复产生二次方序列混合成本，计算开销巨大。Gated Delta Networks (GDNs) 提供了高效循环替代方案，但其标准因果形式无法捕获 ELF 所需的双向上下文。

Method: 提出 DeltaFlow，包含两个变体：DeltaFlow-A 在层间交替扫描方向，DeltaFlow-P 在每层内并行执行前向和后向扫描。引入噪声自适应记忆控制和调度式 Temporal State Consistency (TSC) 来稳定相邻噪声水平间的隐藏表征。

Result: 在 OpenWebText 上使用 32 步 SDE 采样器，DeltaFlow-P 将生成困惑度从全注意力 ELF 基线的 24.218 降至 21.228，同时保持相当的 unigram 熵，训练 token 暴露为 36B（基线为 45B）。在去噪器基准中，DeltaFlow-P 在序列长度 16k 时相比全注意力基线实现 2.72 倍吞吐加速。

Conclusion: DeltaFlow 是密集注意力在高效连续语言去噪中的有前景替代方案，在降低困惑度和训练成本的同时显著提升吞吐效率。

Classification reason: 论文提出 DeltaFlow，一种用于 Embedded Language Flows (ELF) 连续语言去噪的双向 Gated Delta Network 骨干，属于连续语言模型与语言流方向，直接对应 [CANONICAL] 子主题 continuous_language_flow。

Importance reason: Directly advances the top-priority continuous language model direction with a novel efficient bidirectional backbone for ELF denoising. Signals: continuous language model, embedded language flow, gated delta network, efficient inference, 2.72x throughput speedup
...
### [6] [One-Sided Quantile Coupling for Flow Matching](https://arxiv.org/abs/2608.00978)
...
TL;DR: 提出 QC-FM，一种轻量级单侧分位数耦合方法用于 Flow Matching 训练，通过沿随机正交方向投影数据秩并映射到高斯分位数来构造源样本，无需配对成本矩阵，在 CIFAR-10、CelebA、FFHQ、ImageNet-64 上 FID 最高降低 12.9%，并优于 OT-CFM。


<details>
  <summary>Details</summary>
Motivation: Flow Matching 中源分布与目标数据分布之间的耦合方式显著影响优化与样本质量，但结构化耦合（如 mini-batch 最优传输）通常需要至少二次方的成本。作者希望找到一种轻量、可扩展的耦合方法，在不求解 mini-batch 传输问题的前提下注入有用的几何偏置。

Method: 提出 Quantile Coupling Flow Matching (QC-FM)：仅采样数据批次，沿少量随机正交方向投影数据秩并映射到高斯分位数，在正交补空间通过条件高斯采样补全潜码。该构造逐切片一维，无需配对成本矩阵与分配求解。训练时对锚点子集应用 QC，其余源槽位用精确高斯样本补全，保留 QC 偏置同时保留 Baseline 耦合的显式信号。

Result: 在 CIFAR-10、CelebA、FFHQ、ImageNet-64 四个数据集上，QC-FM 在相同训练预算下优于 Baseline，FID 最高降低 12.9%，并在全部四个数据集上优于 OT-CFM。

Conclusion: 保留投影秩结构是一种简单且可扩展的方式，可为 FM 耦合注入有用的几何偏置，而无需求解 mini-batch 传输问题。

Classification reason: 论文提出 Quantile Coupling Flow Matching (QC-FM)，聚焦 flow matching 中源-目标样本耦合的构造方法，属于连续潜空间中的 flow matching 生成建模，与 [CANONICAL] 子主题"连续语言模型与语言流"（continuous_language_flow）高度相关。

Importance reason: Directly advances continuous latent/flow-matching training with a lightweight coupling that improves FID, aligning with the user's top-priority continuous language/multimodal direction. Signals: flow matching, quantile coupling, continuous latent modeling, FID improvement, scalable coupling
...
### [7] [Learning What to Remember: Test-Time Training via Context Distillation](https://arxiv.org/abs/2608.01672)
...
TL;DR: 提出 TTCD（Test-Time Context Distillation）框架，通过长窗口教师监督短窗口学生的快速权重，利用隐藏状态差异作为自监督信号，指导模型记忆对未来 token 预测关键的上下文信息，从而提升长上下文语言建模能力。


<details>
  <summary>Details</summary>
Motivation: 有效的长上下文建模不仅在于保留更多过去信息，更在于保留未来可能相关的信息。现有 TTT 方法只优化重建或在线适应目标，未考虑保留信息的未来效用，缺乏对有限记忆容量分配的引导。

Method: TTCD 引入自监督目标来分配有限记忆容量：使用长窗口教师监督短窗口学生的快速权重，两者隐藏状态差异提供密集自监督信号，引导模型记忆对未来 token 预测关键的上下文信息。聚焦 In-Place 变体 IP-TTCD，将现有 MLP 参数用作快速权重。

Result: 在长上下文语言建模任务上，IP-TTCD 从头预训练时一致优于 DeltaNet、Gated DeltaNet、滑动窗口注意力和 TTT。此外，IP-TTCD 允许预训练 transformer 在推理时通过持续预训练适应参数，仅需轻量架构增强即可获得长上下文能力。

Conclusion: TTCD 将 TTT 从单纯的重建/适应目标推进到面向未来效用的记忆分配，展示了架构级持续学习的潜力，为长上下文建模提供了新范式。

Classification reason: 论文提出 Test-Time Context Distillation (TTCD)，通过长窗口教师监督短窗口学生的快速权重，在连续隐藏状态空间中蒸馏上下文信息以提升长上下文语言建模。这与连续语言模型/语言流方向相关，映射到 [CANONICAL] continuous_language_flow 子主题。

Importance reason: Proposes a novel test-time training objective via context distillation for long-context memory, but is tangential to the user's core focus on continuous multimodal models and video generation. Signals: test-time training, context distillation, long-context modeling, self-supervised memory allocation, continual learning
...
TL;DR: AURORA-LM 是一种连续潜空间扩散语言模型，将可解码文本表征的构建与分布建模分离：用 Query-based Encoder-Decoder 将文本组织为高容量、前缀对齐的潜序列，用 Block-causal Diffusion Transformer 通过 flow matching 学习其分布，逐块从左到右生成并在块内并行去噪。在 OpenWebText 自由生成和 XSum 摘要任务上取得最强性能，扩展到 1B 参数后超越更大的公开潜扩散语言模型。


<details>
  <summary>Details</summary>
Motivation: 语言仍是生成建模中的异类：图像、视频、音频越来越多地在连续潜空间中建模，而文本生成仍主要依赖离散 token。现有连续语言模型要么继承并非为联合生成与解码设计的 embedding 空间，要么压缩自编码潜变量以简化扩散，牺牲 token 级保真度。作者主张不应简化表征来迁就生成模型，而应保留高容量、可解码的文本潜变量，并让扩散模型直接学习其分布。

Method: AURORA-LM 将可解码文本表征的构建与分布建模分离。Query-based Encoder-Decoder 将文本组织为高容量、前缀对齐的潜序列；Block-causal Diffusion Transformer 通过 flow matching 学习该潜序列的分布，逐块从左到右生成，同时并行去噪块内位置。由于该潜变量对扩散更难建模，AURORA-LM 仅限制噪声输入路径，保留完整干净潜变量预测目标，从而容纳全宽度潜变量而不降低解码端容量。此外，将噪声水平分布校准到潜变量宽度，并引入自轨迹一致性以桥接独立采样的训练噪声与推理时的迭代去噪。

Result: AURORA-LM 在 OpenWebText 自由生成和 XSum 摘要任务上，在评估的连续与扩散语言模型中取得最强性能。扩展到 1B 参数（约 1500 EFLOPs 总计算量）后进一步获得增益，在匹配的评估协议下超越更大的公开潜扩散语言模型。所有实验在 Ascend NPU 上完成。

Conclusion: 该工作表明，与其简化表征来适应生成模型，不如保留高容量可解码的文本潜变量并设计扩散模型直接学习其分布，这一思路在连续语言建模中可行且有效，为连续潜空间语言生成提供了新的设计范式。

Classification reason: 该论文提出 AURORA-LM，一种连续潜空间扩散语言模型，通过 flow matching 在连续 latent 中建模文本分布，属于连续语言建模方向，与 [CANONICAL] 子主题 continuous_language_flow（连续语言模型与语言流）高度匹配。

Importance reason: Directly advances the top-priority continuous language model direction with a novel decodable latent + block-causal diffusion design and SOTA results. Signals: continuous latent language model, flow matching, block-causal diffusion transformer, decodable text latent, OpenWebText SOTA
...
### [9] [REFLEX: Rethinking MoE Inference as Refinement-Aware Compute Allocation in Diffusion Language Models](https://arxiv.org/abs/2608.01784)
...
TL;DR: REFLEX 提出一种免训练的 MoE 专家分配方法，针对扩散语言模型中不同 token 去噪细化需求差异，通过粗到细的层级预算分配与 Frontier-Progress Score 对齐计算与细化状态，在 LLaDA-MoE 和 LLaDA2.0-mini 上平均减少 15% 专家计算量并保持或提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（DLM）中每次去噪前向会同时重新访问所有 token 位置，但不同 token 的细化需求差异很大；而默认的固定 token 选择路由为每个 token 分配统一的专家预算，导致专家计算与细化需求不匹配。作者认为 DLM 中的 MoE 推理应被视为跨异构 token 细化状态的细化感知计算分配问题。

Method: REFLEX 是一种免训练方法，保持默认路由器不变，围绕演化中的细化过程重组专家计算。它引入粗到细的层级专家预算分配，使计算与块级细化角色对齐，并使用 Frontier-Progress Score 解决活跃块的优先级问题。

Result: 在 LLaDA-MoE 和 LLaDA2.0-mini 两个代表性 MoE 扩散语言模型及多个广泛使用的基准上，REFLEX 平均减少 15% 的专家计算量，同时在大多数基准上保持甚至提升生成质量。相比自回归式可变专家路由方法，REFLEX 获得更一致的"质量-计算"权衡。

Conclusion: 实验验证了在每次去噪前向中根据异构细化需求分配专家计算的重要性，说明扩散语言模型的 MoE 推理应被建模为细化感知的计算分配问题，而非简单的固定预算路由。

Classification reason: 论文研究扩散语言模型（DLM）中 MoE 推理的专家计算分配问题，属于连续语言模型/扩散语言模型范畴，映射到 CANONICAL 子主题"连续语言模型与语言流"。

Importance reason: Directly targets continuous/diffusion language models with a training-free efficiency method, aligning strongly with the user's top-priority subtopic and efficiency interest. Signals: diffusion language model, MoE efficiency, training-free, compute allocation, LLaDA
...
TL;DR: GradCuit 提出一种基于梯度的测试时潜空间推理方法，在 Transformer 层间插入可优化的连续 latent 状态，通过因果自注意力将整个续写序列的奖励加权梯度直接分配到 latent，实现稳健且可解释的测试时推理扩展。


<details>
  <summary>Details</summary>
Motivation: 现有的优化式潜空间推理方法通常通过解码 token 将连续状态连接到推理轨迹，导致序列级信用分配间接且难以解释 latent 更新如何影响后续推理。作者希望直接优化内部推理状态，使 LLM 在测试时"适应推理方式"而非仅重新生成、采样或重排输出。

Method: GradCuit 在选定的 Transformer 层之间插入可优化的连续 latent 状态，位于 prompt 的隐藏表示与生成续写之间。由于因果自注意力使每个续写 token 的对数概率都能通过剩余 Transformer 块对每个前置 latent 状态形成可微路径，因此可将整个续写的奖励加权梯度直接分配给 latent。通过梯度下降优化这些 latent 状态，实现测试时推理。

Result: 在五个指令微调骨干、三个推理基准和两种答案格式上，GradCuit 平均准确率达 64.5%，比思维链提示高 6.6 个百分点，比最强对比方法高 2.4 个百分点。在七个学习率设置下，其准确率标准差从 LatentSeek 的 1.53 降至 0.82，随机游走变体仍与 LatentSeek 相当。token 级梯度归因显示 latent 影响集中在推理连接 token 上，层分析表明早期到中间 Transformer 层是最有效的优化空间。

Conclusion: GradCuit 通过直接优化内部推理状态，开辟了稳健且可解释的测试时扩展新方向，使 LLM 能够适应推理方式而非仅重生成输出。其梯度归因分析为理解 latent 更新如何影响推理提供了可解释性。

Classification reason: 论文在测试时优化连续 latent 状态进行推理，属于连续语言模型/连续潜空间推理范畴，与 CANONICAL 子主题"连续语言模型与语言流"（continuous_language_flow）高度相关。

Importance reason: Directly advances continuous latent reasoning with a novel credit-assigned gradient flow, strongly matching the top-priority continuous language model subtopic. Signals: continuous latent reasoning, test-time optimization, gradient credit assignment, interpretable reasoning, chain-of-continuous-thought
...
### [11] [Track-Guided Hierarchical Reinforcement Learning for Autonomous Vehicle Drifting with Minimum-Lap-Time Planning](https://arxiv.org/abs/2608.00113)
...
TL;DR: 本文提出一种面向自动驾驶漂移场景的规划-控制框架，先通过最优控制生成最小圈时（MLT）漂移规划轨迹作为先验数据，再提出轨道引导强化学习（TgRL）方法，分阶段从漂移控制策略、漂移过弯策略到完整漂移比赛策略逐步训练，奖励函数融合即时奖励与基于MLT目标的终局奖励，仿真验证了该方法能学习到既保证运动控制性能又有效降低圈时的漂移比赛策略。


<details>
  <summary>Details</summary>
Motivation: 在拉力赛中，车手通过故意打破轮胎抓地力进行漂移来快速调整车辆朝向以降低圈时，但自主执行该操作构成一个复杂的双目标控制问题：既要稳定高度非线性的漂移动力学，又要严格最小化圈时。这促使开发先进的最小圈时（MLT）漂移控制架构。

Method: 首先将MLT漂移规划建模为最优控制问题生成规划轨迹作为先验数据；随后提出轨道引导强化学习（TgRL）漂移控制方法，采用渐进式分阶段训练：从漂移控制策略、到漂移过弯策略、再到完整漂移比赛策略。奖励函数包含即时奖励项和由最小圈时目标导出的终局奖励项。

Result: 仿真结果表明，所提出的框架使智能体能够学习到漂移比赛策略，不仅保证了车辆运动控制性能，还有效降低了圈时。

Conclusion: 通过最优控制先验与渐进式强化学习相结合，所提出的TgRL框架能够有效解决自动驾驶漂移中稳定控制与最小圈时优化的双目标问题，验证了该规划-控制框架在MLT漂移场景中的有效性。

Classification reason: 论文研究自动驾驶漂移场景下的规划-控制框架，结合最优控制生成最小圈时轨迹并用深度强化学习训练漂移控制策略，属于具身智能/机器人规划控制范畴，映射到具身世界模型与规划控制这一规范子主题。

Importance reason: Focuses on autonomous vehicle drifting control, which is outside the user's core continuous multimodal and video generation interests. Signals: world model, autonomous driving, reinforcement learning, minimum-lap-time planning, drift control
...
### [12] [Motion Planning for Mobile Manipulators Navigating Doorways via Model Predictive Control](https://arxiv.org/abs/2608.00206)
...
TL;DR: 本文提出一种基于非线性模型预测控制（MPC）的移动机械臂开门穿越门框运动规划框架，将机器人与门建模为耦合动力学系统，通过惩罚约束保证操作可行性，无需显式机械臂运动学建模，仿真与硬件实验验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 穿越门框是移动机械臂在人类环境中运行的基本能力，需要移动底座与机械臂的协调运动。现有方法往往需要显式建模机械臂运动学，规划复杂度高且难以保证动态可行性。

Method: 将机器人与门构建为耦合动力学系统，在非线性 MPC 优化框架中求解运动规划问题；通过基于惩罚的约束来保证操作可行性，避免在规划器中显式建模机械臂运动学。

Result: 仿真与硬件实验表明该方法能够成功规划出穿越推门和拉门的动态可行且无碰撞轨迹。

Conclusion: 基于 MPC 的耦合系统建模方法可有效实现移动机械臂的开门穿越门框任务，且无需显式机械臂运动学建模，为具身机器人导航规划提供了可行方案。

Classification reason: 论文面向移动机械臂的开门穿越门框运动规划，属于机器人具身智能中的规划与控制任务，映射到 [CANONICAL] 具身世界模型与规划控制 子主题。

Importance reason: Solid robotics control paper but far outside the user's core focus on continuous multimodal models and generative video/world models. Signals: MPC, mobile manipulator, door traversal, coupled dynamics, cs.RO
...
### [13] [Towards General Language-Conditioned Latent Safety Filters](https://arxiv.org/abs/2608.00315)
*Ihab Tabbara, Yuxuan Yang, Hussein Sibai*

Main category: cs.RO, eess.SY, cs.LG
...
TL;DR: 本文提出语言条件化的安全过滤器，将 Hamilton-Jacobi 安全 actor 与 critic 以语言指定的约束为条件，在视觉场景下评估其在抓取放置、擦桌、堆叠积木等任务中执行语言约束并迁移到未见约束实例的能力。


<details>
  <summary>Details</summary>
Motivation: 现有机器人安全过滤器大多是约束特定的，当安全需求（用户、环境、应用）变化时需要重新设计或重新学习。随着 VLA 模型使单一策略能执行多种语言指定的任务，安全部署需要适应不断变化的安全约束，因此需要语言条件化的安全过滤机制。

Method: 将 Hamilton-Jacobi 安全 actor 与 critic 以语言指定的约束为条件进行建模，在视觉设定下对 pick-and-place、table-wiping、block-stacking 三类任务进行训练与评估，检验其执行语言约束的能力以及对未见约束实例的迁移能力。

Result: 实验表明语言条件化的安全过滤器能够减少约束违反，并对未见约束实例表现出部分迁移能力。

Conclusion: 语言条件化安全过滤是可行的方向，能够提升安全过滤器对变化安全需求的适应性与泛化能力，但迁移到完全未见约束实例仍存在局限，需要进一步研究。

Classification reason: 论文研究语言条件化的安全过滤器（Hamilton-Jacobi safety actor/critic），用于机器人策略的安全约束执行与规划控制，属于具身智能/机器人安全控制范畴，映射到具身世界模型与规划控制这一规范主题。

Importance reason: Safety filtering for robotics is tangential to the user's core focus on continuous multimodal models and autoregressive generation. Signals: language-conditioned safety, Hamilton-Jacobi, VLA safety, constraint transfer, robotics safety
...
### [14] [The Gate, Not the Cache: Gate Provenance Bounds the Closed-Loop Reliability of Training-Free VLA Token Skipping](https://arxiv.org/abs/2608.00391)
...
TL;DR: 本文研究训练无关的VLA token跳过加速方法在闭环控制中的可靠性问题。发现当gate信号来自模型自身加速前向时，token跳过机制（复用或删除）都会在闭环控制中崩溃，而崩溃对动作级检测器不可见。作者提出"actuation-slack refresh"方法，在机器人执行动作块时于关键路径外运行一次密集前向，为下一步提供干净的gate和新的KV基座，使两种机制都恢复到0.98的准确率，同时保持跳过加速的速度优势。


<details>
  <summary>Details</summary>
Motivation: Token skipping是加速VLA模型的常用训练无关方法，但现有研究主要关注单步加速效果，忽略了闭环控制中gate信号来自前一步加速前向时误差会跨步累积的问题。作者发现当gate来自模型自身加速前向时，跳过机制会在闭环中崩溃，且这种崩溃对动作级检测器不可见，因此需要研究gate来源对闭环可靠性的影响。

Method: 作者系统对比了token跳过的两种机制（复用reuse和删除deletion），交叉验证gate信号来源（来自密集前向还是加速前向）。提出"actuation-slack refresh"方法：在机器人执行当前动作块期间，于关键路径外运行一次密集前向，为下一步提供干净的gate和新的KV基座。由于检测器无法可靠揭示失败，refresh采用无条件触发而非条件触发。

Result: 在LIBERO-Object上，skip ratio为0.9时，当gate来自模型自身加速前向，复用机制崩溃到0.68、删除机制崩溃到0.31（密集基线为1.00）。加入refresh后两种机制都恢复到0.98。在两种VLA策略、4个LIBERO套件和4个SIMPLER任务上，refresh修复了所有因自收获gate导致的崩溃，服务延迟比密集前向降低18-22%，在仿真和物理机器人上均得到验证。

Conclusion: 决定加速VLA闭环可靠性的不是token如何被跳过，而是gate信号来自哪里。干净的gate（由未跳过的密集前向计算）是闭环稳定运行的关键。actuation-slack refresh通过在关键路径外运行密集前向，兼顾了跳过加速的速度和密集前向的信息完整性，为训练无关的VLA加速提供了可靠的闭环方案。

Classification reason: 论文研究视觉-语言-动作(VLA)模型在机器人控制中的token跳过加速方法及其闭环可靠性，属于具身智能/机器人控制范畴，映射到[具身世界模型与规划控制]这一规范子主题。

Importance reason: Solid efficiency work for VLA token skipping, but it is robotics-focused and lacks the continuous-latent/world-model novelty that drives your top priorities. Signals: VLA acceleration, token skipping, closed-loop reliability, gate provenance, actuation-slack refresh
...
TL;DR: 本文提出首个可部署的动态质心（Dynamic-CoM）观测用于人形机器人单腿平衡，并发布首个方法无关、可复现的sim2sim基准。通过将捕获点（xCoM）相对支撑脚表达以消除对机载传感器无法提供的线速度依赖，配合从人类姿势控制逐项翻译的奖励库，训练出FDDC策略，在90个测试动作中86个保持干净单腿平衡，并成功迁移到真实Unitree G1机器人。


<details>
  <summary>Details</summary>
Motivation: 现有统一人形策略能处理敏捷全身运动，但在单腿平衡这一简单需求上表现不佳——8个SOTA通用策略在90个测试动作中0个能保持干净单腿站立，只能通过迈步或跳跃来恢复平衡而非预防失衡。预防失衡需要捕获点（xCoM），但该量需要机载传感器无法提供的基线线速度，因此从未被用于硬件策略。

Method: 作者将动态CoM观测相对支撑脚表达，使线速度项精确抵消，得到仅由编码器和IMU即可重建的观测，并将其直接放入运行在硬件上的actor中。同时构建从人类姿势控制逐项翻译的奖励库，遵循"预防优于修复"原则。使用非对称FastSAC训练，带特权critic且无需蒸馏。

Result: FDDC策略在90个留出测试动作中86个保持干净单腿平衡（覆盖9个分层姿态类别），并成功迁移到真实Unitree G1机器人。消融实验表明动态CoM观测是最大驱动因素：单独移除它损失40个干净单腿平衡点。同时发布了首个方法无关、可复现的sim2sim基准。

Conclusion: 动态CoM观测是单腿平衡的关键，通过相对支撑脚表达可使其在硬件上可部署。该工作将平衡从"每任务技巧"转变为"可测量的能力"，为领域提供了统一评测基准。

Classification reason: 论文研究人形机器人单腿平衡策略，属于具身智能/机器人控制领域，与"具身世界模型与规划控制"这一CANONICAL子主题高度相关。

Importance reason: Solid robotics control paper but far outside your core focus on continuous/multimodal generative models and video/world-model generation. Signals: humanoid balance, sim2sim benchmark, world_model direction but embodied control, not generative, no continuous tokens or video generation, negative keywords absent but topic mismatch
...
TL;DR: SSTG-Nav 提出一种可复用的度量-语义拓扑图记忆系统，将服务机器人的一次性环境勘察转化为可重复使用的物体导航目标，通过跨视角证据融合与空间分离的恢复站位，在 HM3D-v2 上实现 99.4% 的几何成功上限，并将 SR/SPL 从 0.835/0.560 提升至 0.920/0.603。


<details>
  <summary>Details</summary>
Motivation: 现有 ObjectNav 任务主要被建模为一次性探索，忽略了服务机器人在同一环境中长期部署的核心挑战：识别物体并不等于找到可到达的停靠位置，且单个地图错误即可导致任务失败。论文旨在建立可复用的度量-语义记忆，使机器人随经验变得更可靠。

Method: SSTG-Nav 构建目标无关的度量-语义拓扑图，将一次性环境勘察转化为可执行的目标。它跨视角整合证据以确认物体位置，并保留空间上分离的恢复站位以应对地图错误。系统包含度量接地（metric grounding）与源感知融合（source-aware fusion）机制，并设计了融合感知的 Top-3 恢复策略。

Result: 在 36 个场景的 1000 个 HM3D-v2 情节上，目标无关拓扑达到 99.4% 的几何成功上限。度量接地将 SR/SPL 从 0.835/0.560 提升至 0.920/0.603，源感知融合达到 0.926/0.586。融合感知 Top-3 恢复将 Success@1/2/3 提升至 0.928/0.965/0.975，SPL@3 达 0.601。ROS2/Nav2 实现验证了完整可复用查询到执行流程。

Conclusion: 预探索（pre-exploration）是一种强大且实用的范式，可显著提升重复语义导航的可靠性与效率。度量接地、源感知融合与恢复策略共同贡献了性能提升，为服务机器人的长期部署提供了可行方案。

Classification reason: 论文提出 SSTG-Nav，一种可复用的度量-语义拓扑记忆，将一次性环境勘察转化为可执行的目标，用于服务机器人的重复物体导航（ObjectNav）。这属于具身智能环境建模与导航规划范畴，与 [CANONICAL] 具身世界模型与规划控制 高度相关，故复用该子主题。

Importance reason: Solid robotics navigation paper with reusable memory and metric grounding, but it is outside the user's core continuous-multimodal/video-generation focus and lacks strong novelty in those directions. Signals: object navigation, reusable topological memory, metric-semantic grounding, HM3D-v2 benchmark, ROS2/Nav2 deployment
...
### [17] [Latency-Tolerant Cloud-Edge Collaborative Vision-Language-Action Models via Emergent Representational Specialization](https://arxiv.org/abs/2608.00569)
...
TL;DR: CloudEdgeVLA 是一种云边协同的 VLA 策略，将时序错位视为表征学习问题：云端 VLA 将延迟观测编码为缓慢变化的任务特征，轻量级边缘头结合最新云特征与当前局部视觉进行闭环控制，在 LIBERO 基准上显著优于现有分层/异步策略。


<details>
  <summary>Details</summary>
Motivation: 部署十亿参数级 VLA 策略到移动机器人存在系统冲突：语义推理依赖云端 GPU，而闭环控制必须本地响应以应对网络延迟与抖动。现有分层和异步策略虽提升吞吐，但慢路径表征可能过期到达或需要显式调度与延迟提示。

Method: CloudEdgeVLA 将时序错位作为表征学习问题处理：云端 VLA 将延迟观测编码为缓慢变化的任务特征，轻量级边缘头结合最新可用云特征与当前局部视觉。训练时，当前帧与随机延迟帧配对同一当前动作目标，分别走 fresh 与 stale 路径，促使云表征保留任务级信息而边缘路径提供状态敏感修正。

Result: 在四个 LIBERO 套件上，CloudEdgeVLA 在 40 步均匀延迟窗口下保持 63.8-78.0% 成功率，而 VLASH 最高仅 6.4%，评估的单路径基线最高仅 3.0%。

Conclusion: 通过从控制回路中移除阻塞同步，该设计为可扩展的 VLA 部署提供了实用路径：云端模型可以持续增长，而边缘计算保持轻量且响应迅速。

Classification reason: 论文提出 CloudEdgeVLA，一种面向移动机器人的云边协同视觉-语言-动作（VLA）策略，属于具身智能中的机器人策略学习与规划控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Novel cloud-edge VLA latency handling via representation learning, but tangential to core continuous/multimodal generation priorities. Signals: world model, VLA, cloud-edge, latency-tolerant, representation specialization
...
TL;DR: DynamicEnvPlan 是一个面向动态环境的闭环高层规划框架，通过规划、扰动、守卫校正模块将动态执行状态转化为恢复导向轨迹，并用于分阶段监督微调，使规划器同时从正常执行与扰动恢复轨迹中学习，在 104 个任务-场景组合上将成功率从 33.3% 提升到 76.2%。


<details>
  <summary>Details</summary>
Motivation: 物理世界交互本质上是动态的，环境在执行过程中可能演化，要求智能体在非平稳条件下调整计划。现有具身任务基准虽然能暴露此类失败，但通常只把失败当作评测结果，而非用于训练智能体恢复的可学习信号。作者希望将失败转化为监督信号，训练规划器在环境偏差与执行不确定性下进行鲁棒的长时程规划。

Method: DynamicEnvPlan 是一个闭环高层规划框架，包含：1) 以人形智能体、高层原始技能、结构化语义记忆和可控扰动扩展具身任务执行；2) 数据合成设计由规划、扰动和守卫校正三个模块组成，将动态执行状态转化为恢复导向轨迹；3) 使用这些轨迹进行分阶段监督微调，使规划器同时从正常执行和扰动恢复轨迹中学习。

Result: 在 104 个任务-场景组合（涵盖 i.i.d.、组合泛化和分布外设置）上进行微调与评测，DynamicEnvPlan 将基础规划器的成功率从 33.3% 提升到 76.2%，并在包括安全性和可负担性合规在内的全部七项物理世界交互关键评测指标上均有提升。

Conclusion: 将动态执行失败转化为可学习的监督信号能够显著提升具身规划器在非平稳环境下的鲁棒性，验证了闭环数据合成与分阶段微调范式在长时程具身规划中的有效性。

Classification reason: 论文研究长时程具身规划在动态环境下的鲁棒性，涉及具身智能体的高层规划、动作执行与扰动恢复，属于具身世界模型与规划控制方向，映射到 [CANONICAL] embodied_world_model。

Importance reason: Solid embodied planning work, but it is robotics-focused and lacks the continuous/multimodal generation or world-model-as-video emphasis the user prioritizes. Signals: embodied planning, closed-loop framework, perturbation recovery, supervised fine-tuning, success rate improvement
...
### [19] [Learning-Based Motion Planning for Dynamic Environments: From Foundational Algorithms to Emerging Paradigms](https://arxiv.org/abs/2608.00625)
...
TL;DR: 一篇关于动态环境下基于学习的运动规划综述，回顾2015-2025年间代表性工作，提出"学习角色"分类法，将方法分为直接策略学习、学习增强经典规划、混合规划与训练增强四类，并分析观测表征、预测不确定性、交互建模、规划器集成、安全约束与训练策略等关键因素，最后讨论 sim-to-real、安全可认证规划、密集人群导航、感知-规划耦合与具身 AI 等开放挑战。


<details>
  <summary>Details</summary>
Motivation: 动态环境中的运动规划是机器人学的基础问题，涉及移动障碍物、不确定预测与多智能体交互。近年来基于学习的方法不断涌现，但缺乏一个系统框架来理解学习如何参与规划流程、如何与经典规划方法互补或交互，因此需要一篇综述来梳理这一领域。

Method: 提出"role-of-learning"分类法，按学习在规划管线中的参与方式将现有方法分为四类：直接策略学习、学习增强经典规划、混合规划、训练增强方法。对每类方法总结问题设定、代表性算法、核心思想、集成机制、优势与局限，并分析观测表征、预测不确定性、交互建模、规划器集成、安全约束与训练策略等塑造因素。

Result: 系统梳理了2015-2025年间动态环境运动规划的代表性工作，建立了学习参与规划管线的分类框架，并识别出 sim-to-real 差距、安全可认证规划、密集人群导航、感知-规划耦合与具身 AI 等关键开放挑战。

Conclusion: 基于学习的运动规划正从经典规划方法的补充走向深度融合，未来需要在安全保证、可认证性、sim-to-real 迁移以及感知-规划耦合等方面取得突破，以支撑具身智能在真实动态环境中的可靠部署。

Classification reason: 该论文是动态环境下基于学习的运动规划综述，聚焦机器人路径/轨迹规划、多智能体交互与具身智能，属于世界模型方向下的具身世界模型与规划控制范畴，复用 [CANONICAL] 子主题 embodied_world_model。

Importance reason: A robotics motion planning survey with limited direct connection to the user's core focus on continuous multimodal models and generative video/world models. Signals: robotics survey, motion planning, learning-based planning, world model tangential, no continuous token/video generation
...
### [20] [Staged Multi-Agent Training (SMAT) for Hip Exoskeletons: Metabolic and Biomechanical Validation of a Simulation-Trained Co-Adaptive Controller](https://arxiv.org/abs/2608.00715)
...
TL;DR: 本文首次对 Staged Multi-Agent Training (SMAT) 进行生理学验证：该策略完全在物理仿真中训练，部署到髋部外骨骼后，在八名健康受试者上通过间接测热法测量代谢成本，主动辅助相比被动设备降低净代谢率 19.7%，且策略在行走速度和地形上具有泛化性。


<details>
  <summary>Details</summary>
Motivation: 基于学习的控制器可在纯物理仿真中训练外骨骼辅助策略，但很少有考虑人体-设备协同适应的控制器通过全身代谢测量在真实用户上得到验证。协同适应具有挑战性：设备改变关节动力学后，穿戴者会重组神经肌肉协调，形成非平稳学习问题。

Method: 采用四阶段课程式训练的分阶段多智能体训练（SMAT），逐步训练肌肉骨骼人体智能体与双侧髋部外骨骼智能体。策略在仿真中训练后直接部署到真实髋部外骨骼，无需针对受试者重新训练，通过间接测热法测量代谢成本，并进行生物力学分析。

Result: 主动辅助相比被动设备将净代谢率降低 19.7%（p<0.001）；生物力学分析确认所有受试者髋部机械功率以正向为主（正向功率比 0.98）；策略在行走速度和地形上均表现出泛化能力。

Conclusion: 单一仿真训练的 SMAT 策略无需受试者特定重训练即可在真实用户上带来显著的代谢收益，且对训练条件之外的场景保持鲁棒性，验证了仿真到现实的迁移可行性。

Classification reason: 论文研究基于仿真训练的外骨骼控制器，涉及具身智能、动作控制与人体-设备协同适应，属于具身世界模型与规划控制方向，映射到 [CANONICAL] embodied_world_model。

Importance reason: Solid robotics validation but far outside your core focus on continuous multimodal models and generative video/world models. Signals: simulation-to-real transfer, metabolic validation, co-adaptive control, exoskeleton, world_model direction only
...
### [21] [Latent-Centroid Steering: Single-Pass Classifier-Free Guidance for Command-Aligned Autonomous Driving](https://arxiv.org/abs/2608.00237)
...
TL;DR: 提出Latent-Centroid Steering (LCS)，一种单次前向的分类器无关引导(CFG)机制，用于端到端自动驾驶VLM，通过类级潜空间质心偏移替代实例级残差，解决命令跟随失败问题，降低约50%推理延迟并提升命令遵循与驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶VLM存在命令跟随差距：预测轨迹对导航指令敏感度弱，导致关键决策点行为错误。作者将其识别为条件策略坍缩——回归训练在多模态轨迹分布下使模型依赖主导视觉先验而边缘化语言条件信号。标准两遍CFG推理延迟过高且产生噪声实例级引导方向。

Method: 将CFG形式化为对比条件与无条件预测来隔离动作空间中指令诱导残差，并基于CFG的均值漂移解释，提出LCS：单次前向引导机制，用预计算的命令特定质心替换实例级残差，将条件表征投影到命令特定质心，基于聚类几何进行类级潜空间引导。

Result: LCS在闭环Bench2Drive与开环nuScenes基准上，将推理延迟降低约50%，同时实现更强的命令遵循与更优的驾驶性能。

Conclusion: 类级潜空间质心引导相比实例级残差更稳定且计算高效，为实时自动驾驶中的命令对齐提供了一种单次前向的高效CFG方案。

Classification reason: 论文面向端到端自动驾驶，将VLM映射多模态输入与导航指令到可执行轨迹，属于具身智能/机器人规划控制范畴，映射到[具身世界模型与规划控制]规范子主题。

Importance reason: Solid robotics/VLM efficiency work, but tangential to the user's core focus on continuous multimodal models and generative world models. Signals: classifier-free guidance, autonomous driving, VLM, inference latency reduction, command following
...
### [22] [SelfWAM: A Self-Grounded Unified World Action Model for Fast Robot Control](https://arxiv.org/abs/2608.00725)
...
TL;DR: SelfWAM 是一个自锚定的统一世界动作模型，基于模态特化的 Mixture-of-Transformers (MoT) 架构，联合预测动作、动作条件化的未来 RGB 帧和机器人自掩码，将未来预测锚定在机器人可见身体及其动作引发的运动上，从而提升策略性能并保持快速推理。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型（WAM）在联合建模动作与未来观测时，仅以任务提示和观测上下文为条件进行未来预测，容易捕获通用的任务进展而非所执行动作的具体后果，导致未来预测对动作不敏感，限制了策略学习效果。

Method: SelfWAM 采用模态特化的 Mixture-of-Transformers (MoT) 架构，联合预测动作、动作条件化的未来 RGB 帧和机器人自掩码。联合训练时允许未来视觉查询注意力机制访问演示动作的干净副本，使视频分支成为动作特定的后果模型，同时保持快速的动作-only 推理路径不变。此外，使用针对未来机器人自掩码预测的提示特定目标，去除外观细节，提供与时序演化紧密耦合于条件动作的目标。

Result: 在 RoboTwin 2.0 和真实世界操作任务上的实验表明，SelfWAM 能产生对动作更敏感的未来预测，保持快速策略推理，同时提升策略性能。

Conclusion: 通过干净动作条件化和未来自掩码监督，SelfWAM 使未来预测更直接地反映执行动作如何改变机器人可见运动及周围场景，从而在保持快速推理的同时提升机器人策略性能。

Classification reason: 该论文提出 SelfWAM，一个面向机器人控制的统一世界动作模型（WAM），联合建模动作与未来观测，属于具身智能/机器人策略学习范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Strong world-model robotics paper, but only partially overlaps with your priority on video world models and continuous multimodal reasoning. Signals: world model, robot control, action-conditioned future prediction, Mixture-of-Transformers, self-mask grounding
...
### [23] [Push-Wiper: Toward General-Purpose Robotic Cleaning across Varied Stains and Surfaces with Segmented Pushing Trajectories](https://arxiv.org/abs/2608.00730)
...
TL;DR: 本文首次系统研究多智能体LLM机器人系统中的提示注入攻击，评估直接注入（任务指令）与间接注入（感知模块）两种方式，发现攻击可在智能体间通过共享提示结构传播，并分析架构变化对攻击成功率的影响。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地集成到自主机器人系统中用于任务规划与控制，但这种集成使其暴露于提示注入攻击，可能导致不安全决策和物理伤害。多智能体设置通过跨智能体污染和更广的攻击面进一步增加了风险，但此前缺乏对该场景的系统研究。

Method: 作者构建了一个基于LLM的多智能体机器人系统，设计了直接注入（任务指令）和间接注入（感知模块）两类攻击，在不同攻击目标复杂度、注入策略以及单/多智能体设置下进行实验，并分析架构变化对LLM查询和攻击成功率的影响。

Result: 实验表明提示注入可以诱导对抗性动作并降低任务完成率；攻击可通过共享提示结构从一个智能体传播到其他智能体，影响程度取决于提示组成和攻击目标智能体；架构变化会改变LLM查询进而影响攻击成功率。

Conclusion: 多智能体LLM机器人系统面临严重的提示注入安全威胁，攻击可在智能体间传播，需要针对性的防御机制来保障机器人系统的安全性。

Classification reason: 论文研究基于LLM的多智能体机器人系统中的提示注入攻击，属于机器人任务规划与控制安全范畴，映射到具身世界模型与规划控制这一规范主题。

Importance reason: Security-focused robotics paper with limited overlap with your core interests in continuous multimodal models and generative world models. Signals: prompt injection, multi-agent robotics, LLM planning, security, no generative modeling
...
### [25] [ORCESTRA: VLM-driven Visual Robot programming in Mixed Reality](https://arxiv.org/abs/2608.00775)
*Ivan Snegirev, Elizaveta Semenyakina, Mikhail Konenkov, Artem Lykov, Miguel Altamirano Cabrera, Dzmitry Tsetserukou*

Main category: cs.RO, cs.HC, cs.CV
...
TL;DR: ORCESTRA 是一个混合现实系统，通过无代码航点教学和语言引导控制来编程机器人数字孪生，利用视觉语言模型将语音/文本命令转换为结构化数字孪生计划，并在物理部署前提供 MR 验证作为安全层。


<details>
  <summary>Details</summary>
Motivation: 传统机器人编程需要专业知识且缺乏直观交互方式，语言引导的机器人控制虽然便捷但缺乏安全验证机制。作者希望构建一个混合现实系统，让用户无需代码即可通过航点教学和自然语言控制机器人数字孪生，并在物理部署前进行安全验证。

Method: ORCESTRA 在透视混合现实工作空间中运行，用户可将机器人孪生放置在真实表面上，通过航点教学轨迹、保存机器人相对位姿的片段，或发出语音/文本命令。系统使用视觉语言模型将命令转换为结构化数字孪生计划。两种交互模式共享后端，包括度量接地、具身感知验证、预览、确认和数字孪生执行。系统支持多种异构机器人形态，包括固定基座机械臂、移动基座和人形机器人。

Result: 系统支持多种异构机器人形态（固定基座机械臂、移动基座、人形机器人），并展示了 MR 验证作为语言引导机器人编程在物理部署前的安全层。

Conclusion: ORCESTRA 展示了混合现实环境结合视觉语言模型进行无代码机器人编程的可行性，MR 验证可作为语言引导机器人编程的安全层，在物理部署前降低风险。

Classification reason: 论文提出 ORCESTRA，一个混合现实机器人编程系统，通过语言引导控制机器人数字孪生，涉及具身智能、动作规划与机器人控制，属于具身世界模型与规划控制方向。

Importance reason: Focuses on mixed-reality robot programming rather than core continuous multimodal or video/world-model generation, with only tangential VLM use. Signals: VLM-driven planning, mixed reality, robot digital twin, no-code waypoint teaching, language-guided control
...
### [26] [DynamicWAM: Dual-Path Motion Conditioning for World-Action Models in Dynamic Manipulation](https://arxiv.org/abs/2608.00793)
...
Importance: 值得关注 (72.68) - 未入选精读

TL;DR: DynamicWAM 是一种紧凑的世界-动作模型，通过双路径运动条件（历史光流帧 + 运动学描述子）实现动态物体操作中的运动感知与响应控制，在 DOMINO 基准和 12 项真实任务上均超越基线。


<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型（WAM）通常仅以当前帧为条件，并同步执行大型骨干网络，导致在动态场景中缺乏运动感知能力和响应式控制，难以应对动态物体操作任务。

Method: DynamicWAM 采用双路径运动条件：1) 历史流条件路径，通过冻结的预训练视频 VAE 编码时间对齐的光流帧与当前观测，保留空间运动结构；2) 运动学描述子路径，将位移、时长、速度、加速度等运动学描述注入动作专家，提供运动幅度与时机信息。两条路径通过联合世界-动作注意力融合，并采用蒸馏的紧凑骨干与基于实时分块（RTC）的异步执行实现响应式控制。

Result: 在 DOMINO 基准上取得 38.2% 成功率与 53.2 操作分数，优于所有评估基线；在 12 项涵盖直线、圆形和复合目标运动的真实任务中取得 46.7% 平均成功率，超过最强基线 22.9 个百分点。

Conclusion: 双路径运动条件能有效提升世界-动作模型在动态操作中的运动感知与响应能力，紧凑骨干与异步执行机制在保证性能的同时实现实时控制。

Classification reason: 论文提出 DynamicWAM，一种面向动态物体操作的世界-动作模型（World-Action Model），结合历史光流运动条件与运动学描述子，用于机器人动态操作与响应控制，属于具身智能环境模型与规划控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Solid robotics world-model work, but niche to manipulation and lacks direct connection to your core continuous-multimodal or generation priorities. Signals: world model, video VAE, optical flow conditioning, real-time control, robotics manipulation
...
### [27] [GeminiPainter's sequence-formed pipeline comprised of perception, cognition, planning, and action stages](https://arxiv.org/abs/2608.00829)
...
Importance: 普通相关 (49.99) - 未入选精读

TL;DR: 提出一个自主机器人肖像生成系统，结合实时人脸检测、Gemini Vision API 的 AI 素描生成、基于图的路径规划与 6-DoF 协作机械臂执行，形成感知-认知-规划-动作的完整流水线，用户评分显示素描质量、执行感知与用户体验均较高。


<details>
  <summary>Details</summary>
Motivation: 旨在构建一个端到端的自主机器人肖像绘制系统，将计算机视觉、神经艺术抽象、运动优化与机器人控制整合为统一的感知-认知-规划-动作流水线，实现可识别、美观且吸引人的机器人肖像绘制。

Method: 系统由四个阶段组成：感知阶段通过实时人脸检测从视频帧中提取面部区域；认知阶段使用 Gemini Vision API 将面部转换为极简单线素描；规划阶段通过基于图的路径规划优化笔画顺序；动作阶段在 6-DoF 协作机械臂上执行平滑轨迹完成绘制。

Result: 用户 5 分制评分中，素描质量 4.33、感知执行 4.53、用户体验 4.65，表明生成的机器人肖像具有可识别性、吸引力和参与感。

Conclusion: 该感知-认知-规划-动作流水线有效整合了视觉感知、AI 艺术生成与机器人控制，能够生成高质量且用户满意的自主机器人肖像，验证了多阶段具身智能流水线的可行性。

Classification reason: 论文提出一个自主机器人肖像生成系统，包含感知-认知-规划-动作的完整流水线，涉及机器人控制、路径规划与动作执行，属于具身智能与机器人规划控制范畴，映射到具身世界模型与规划控制这一规范子主题。

Importance reason: 该论文聚焦机器人肖像绘制流水线，与用户关注的多模态生成、世界模型方向关联较弱，且未涉及连续表示或视频生成等核心主题。 Signals: 机器人绘画, Gemini Vision API, 感知-认知-规划-动作, 图路径规划, 6-DoF机械臂
...
### [28] [DreamTraj: Generating 6-DoF Object Trajectories by Reading Unrendered Video Diffusion Latents](https://arxiv.org/abs/2608.00486)
...
TL;DR: DreamTraj 提出从冻结的图像到视频扩散模型的中间表征中直接解码物体6-DoF轨迹，无需生成视频或依赖深度/CAD等特权输入，并配套提出MOVE数据集（5038条带细粒度语言指令的物体中心轨迹）。


<details>
  <summary>Details</summary>
Motivation: 操作过程中物体轨迹的准确预测对感知-动作闭环至关重要，但现有数据集缺乏细粒度语言到运动的标注，且现有预测器要么依赖视频、深度、CAD等特权输入，要么通过昂贵的生成-提取管线从生成视频中恢复运动，误差大且成本高。

Method: 提出DreamTraj：从单张RGB图像和任务指令出发，不生成视频，而是读取冻结的图像到视频扩散模型在早期去噪步骤的内部表征。一个轻量级flow-matching Reader将query-key注意力轨迹和池化隐藏状态解码为相对6-DoF位姿。同时构建MOVE数据集，包含5038条物体中心的第一视角轨迹，每条配以细粒度自然语言指令。

Result: 在平移和旋转两个指标上均超越使用多帧或特权输入的预测器，达到新的SOTA，且比生成-提取管线快4.6倍。

Conclusion: 首次直接从视频扩散模型的中间表征而非生成像素中解码物体6-DoF轨迹，验证了扩散模型内部表征可作为高效、免特权输入的运动预测来源，为感知-动作闭环提供了新范式。

Classification reason: 论文关注操作过程中物体6-DoF轨迹预测，属于具身智能/机器人操作中的感知-动作闭环，映射到具身世界模型与规划控制这一规范主题。同时论文从视频扩散模型的中间表征（latent）中解码运动信息，与连续语言/多模态模型的连续潜空间思想相关。

Importance reason: Directly decodes 6-DoF trajectories from frozen video diffusion latents, aligning strongly with world-model and continuous-representation priorities while avoiding privileged inputs. Signals: world model, video diffusion latent decoding, flow matching, 6-DoF trajectory, no video generation
...
### [29] [FreqNav: Stage-Wise Frequency Routing for Object-Oriented Aerial Vision-Language Navigation](https://arxiv.org/abs/2608.00970)
...
Importance: 普通相关 (60.31) - 未入选精读

TL;DR: FreqNav 提出一种阶段式频率路由的轻量自适应感知框架，用于面向目标的空中视觉语言导航。它将长时程空中导航建模为从空间结构到局部细节的频率偏好转移，通过频率 token 路由器动态分配视觉 token，并利用扩散 Transformer 预测平滑轨迹实现连续控制，在提升性能的同时实现约 3 倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 面向目标的空中视觉语言导航需要在长时程闭环控制下搜索目标并精确降落。现有 VLN 方法在导航各阶段使用相同的视觉 token 建模不断演化的感知需求，导致无关物体和背景杂波干扰。作者观察到早期探索阶段优先关注低频空间布局，后期转向高频目标细节，因此提出按阶段动态分配感知资源的方案。

Method: FreqNav 将长时程空中导航建模为从空间结构到局部细节的频率偏好转移。在固定计算预算下，Frequency Token Router 根据当前导航阶段从双视角观测中选择阶段相关的视觉表征；Phase-dependent Grounding Module 通过显式监督锚定视觉证据；最后 Diffusion Transformer 预测平滑轨迹用于连续控制。

Result: 实验表明 FreqNav 在多个强基线上取得更优性能，同时实现约 3 倍推理加速。真实世界部署进一步验证了其在长时程空中自主导航中的有效性、效率与实际应用潜力。

Conclusion: FreqNav 通过阶段式频率路由实现了按导航阶段动态分配视觉 token 的自适应感知，验证了频率偏好转移建模在长时程空中 VLN 中的有效性，为空中自主导航提供了高效且实用的解决方案。

Classification reason: 论文面向无人机空中视觉语言导航（VLN），涉及长时程闭环控制、动作条件轨迹预测与连续控制，属于具身智能环境模型与规划控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Novel frequency-routing for aerial VLN with diffusion trajectory prediction, but domain is robotics/navigation rather than core generative or continuous-model research. Signals: world model, diffusion transformer, efficient inference, vision-language navigation, frequency routing
...
### [30] [VLAGuard: A Framework for Evaluating and Mitigating Physical Attention Hijacking in Vision-Language-Action Robots within Wireless Sensor Networks](https://arxiv.org/abs/2608.01028)
...
TL;DR: VLAGuard 是一个评估与缓解 VLA 机器人物理注意力劫持攻击的框架，包含 VASA 攻击模块（可打印补丁干扰动作条件交叉注意力）和 APFT 防御模块（稳定时空注意力并保持几何一致性），在 LIBERO 仿真和 2000 次真实世界试验中显著提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: VLA 机器人作为无线传感器网络中的移动边缘节点部署时，面临物理对抗威胁。现有工作缺乏对策略关键的动作到视觉注意力劫持漏洞的系统评估与防御方法。

Method: 提出 VLAGuard 框架：1) VASA 压力测试模块，使用可打印补丁严重干扰机器人的动作条件交叉注意力；2) APFT 注意力保护微调防御，稳定时空注意力并强制几何一致性，零推理开销。

Result: APFT 将 OpenVLA 在 LIBERO 仿真中的失败率从 100.0% 降至 25.9%；在 2000 次真实世界试验中，严重补丁攻击下平均成功率从 23.0% 提升至 67.4%。

Conclusion: 保护注意力通路对提升传感器网络中 VLA 边缘节点的鲁棒性至关重要，VLAGuard 提供了有效的评估与防御方案。

Classification reason: 论文研究 Vision-Language-Action (VLA) 机器人（OpenVLA）在无线传感器网络中的鲁棒性，属于具身智能/机器人规划控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Focuses on adversarial robustness for VLA robots, which is peripheral to your core interests in continuous multimodal models and autoregressive generation. Signals: VLA robot security, attention hijacking, adversarial patches, OpenVLA, LIBERO benchmark
...
### [31] [Sparse Meets Dense: Correspondence Guided Robotic Manipulation with Rigid-Deformable Interactions](https://arxiv.org/abs/2608.01083)
...
TL;DR: 提出一种面向刚体-柔体交互操作的混合对应表征方法，结合结构/任务/交互感知的稀疏关键点与柔体上的稠密对应关系，实现精确跟踪、泛化到新变形与场景，并支持少样本一次迁移到新任务。


<details>
  <summary>Details</summary>
Motivation: 刚体-柔体交互操作（如挂衣服、穿衣）在家庭机器人中常见但极具挑战，因为存在丰富的多点接触和柔体复杂动力学。传统的物体中心表征（如6D位姿或结构点）缺乏任务特定信息，难以应对此类交互。

Method: 提出混合对应表征：首先基于刚体和柔体的全局结构生成结构、任务、交互感知的稀疏关键点，并通过局部交互接触进行过滤；然后在柔体上构建稠密对应关系以支持关键点在操作过程中的精确跟踪。稀疏关键点编码丰富的任务特定信息用于精细操作，稠密对应保证高效跟踪及对新颖变形、形状和场景的泛化。

Result: 大量实验表明该方法在刚体-柔体交互操作任务上有效且具有广泛适用性，支持以最少演示实现新任务的一次性迁移。

Conclusion: 混合稀疏-稠密对应表征结合了两类表征的优势，为刚体-柔体交互操作提供了有效的解决方案，并具备良好的泛化与少样本迁移能力。

Classification reason: 论文面向家庭机器人刚体-柔体交互操作任务，提出基于稀疏关键点与稠密对应的混合表征用于机器人操作，属于具身智能环境建模与操作控制范畴，映射到具身世界模型与规划控制这一规范子主题。

Importance reason: Solid robotics manipulation paper but outside the user's core focus on continuous multimodal models, video generation, and world models. Signals: rigid-deformable interaction, sparse keypoints, dense correspondences, one-shot transfer, household robots
...
### [32] [PRISM: Privileged Probabilistic Latent Supervision for End-to-End Autonomous Driving Motion Planning](https://arxiv.org/abs/2608.01201)
...
TL;DR: PRISM 提出一种概率化深度监督框架，通过从真实轨迹数据直接正则化中间潜表征来训练端到端自动驾驶运动规划模型，利用 ELBO 优化可重参数化分布，在 nuScenes 上相比向量化基线降低 8% 规划 L2 误差和 3% 碰撞率。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶系统通常仅依赖输出级监督训练，导致复杂模型隐藏层梯度较弱。近期工作引入 VLM 对潜特征进行监督虽取得收益，但其理论机制未被充分理解。作者发现收益并非来自 VLM 推理能力，而是来自训练中 E2E 模型与真实数据之间建立的潜连接，因此提出直接从 GT 数据正则化中间潜表征的概率化框架。

Method: 将模型潜表征视为可重参数化分布，通过证据下界（ELBO）优化架构，从未来 GT 路径直接对轨迹相关潜表征进行概率化深度监督，正则化中间层表征。

Result: 在 nuScenes 数据集上，使用相同训练数据和 E2E 架构，相比竞争性向量化基线，规划 L2 误差降低 8%，碰撞率降低 3%，且计算开销可忽略。

Conclusion: 从 GT 数据直接进行概率化潜监督即可获得显著规划性能提升，无需依赖 VLM 推理能力，揭示了此前 VLM 监督收益的真正来源，并提供了一种更简洁高效的理论可解释方案。

Classification reason: 论文聚焦端到端自动驾驶运动规划，属于具身智能/机器人规划控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制 子主题。

Importance reason: 具身世界模型方向的自动驾驶规划工作，但非用户核心连续多模态/生成方向，且负向关键词未命中。 Signals: 世界模型, 自动驾驶规划, 概率深度监督, ELBO, nuScenes
...
### [33] [Hermite Curves as Trajectory Priors for Vision-Language-Action Models](https://arxiv.org/abs/2608.01265)
...
Importance: 普通相关 (63.58) - 未入选精读

TL;DR: 本文提出将动作 chunk 参数化为分段三次 Hermite 曲线作为轨迹先验，通过 Hermite Tokens、Hermite Scaffold 和 Hermite Regularization 三种变体在离散自回归与连续生成范式下显式施加平滑性与连续性约束，其中 Hermite Regularization 作为辅助训练目标效果最佳，在 LIBERO 及真实机器人任务上显著提升成功率且无额外推理开销。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型将动作 chunk 展平为逐时间步控制，依赖隐式数据学习，导致物理执行中出现抖动运动与边界不连续问题。动作 chunk 作为弱结构化接口缺乏显式的平滑性与连续性约束。

Method: 将 chunk 轨迹参数化为由端点位置与速度定义的分段三次 Hermite 曲线，显式强制平滑与连续。在离散自回归与连续生成两种范式下实例化三种变体：Hermite Tokens（自回归预测量化边界变量）、Hermite Scaffold（将干净动作分解为基础骨架与残差）、Hermite Regularization（将先验严格作为辅助训练目标）。

Result: Hermite Regularization 在三种变体中表现最佳：LIBERO 上 π0.5 基线成功率从 95.9% 提升至 98.7%，LIBERO-plus 从 85.7% 提升至 90.9%，四个真实机器人任务从 63.4% 提升至 90.0%，且无额外推理开销。

Conclusion: 显式结构化轨迹先验最有效的用法是作为学习归纳偏置而非运行时约束。Hermite 曲线先验能有效提升 VLA 模型的动作平滑性与任务成功率。

Classification reason: 论文提出 Hermite 曲线作为 VLA（视觉-语言-动作）模型的轨迹先验，用于机器人操作任务的动作 chunk 平滑与连续性建模，属于具身智能动作规划与控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Solid robotics/VLA paper with smoothness priors, but only tangential to the user's core focus on continuous multimodal models and video/world-model generation. Signals: VLA trajectory prior, Hermite curve smoothing, action chunk structure, LIBERO + real-robot gains, no inference overhead
...
### [34] [DreamTrajectory: Trajectory-Guided Action Generation with World Model Alignment for Mobile Manipulation](https://arxiv.org/abs/2608.01381)
...
TL;DR: DreamTrajectory 提出一种轨迹引导的移动操作框架，联合预测意图级末端执行器轨迹与全身动作块，并引入轻量轨迹世界模型在测试时通过搜索-预测-评分选择与规划轨迹最对齐的动作候选，显著提升移动操作成功率。


<details>
  <summary>Details</summary>
Motivation: 现有VLA策略在移动操作中面临两大局限：一是直接映射观测到全身动作块，缺乏显式任务空间运动规划，导致基座-手臂协调预测不精确；二是开环执行预测动作块，不检查动作能否实现策略意图的运动，控制误差与未建模接触会累积成规划与实现运动之间的差距。

Method: DreamTrajectory 包含两个核心组件：(i) 单一动作专家联合预测意图级末端执行器轨迹与全身动作块，使轨迹显式引导基座-手臂动作生成；(ii) 轻量轨迹世界模型预测候选动作块会引发的轨迹，并通过测试时搜索-预测-评分流程选择与规划轨迹最对齐的候选。

Result: 在MS-HAB上，轨迹引导将平均成功率从32.3%提升至47.5%，测试时精修进一步达到54.8%，在接触丰富的铰接物体任务上增益最大；在三个真实世界移动操作任务上平均成功率分别为63.3%、81.7%和90.0%。

Conclusion: DreamTrajectory 通过显式轨迹引导与轨迹世界模型对齐，有效解决了移动操作中动作空间大、开环执行误差累积的问题，在仿真与真实世界任务上均取得显著提升，验证了轨迹级规划与测试时精修对移动操作VLA策略的价值。

Classification reason: 论文面向移动操作机器人的语言条件动作生成，提出轨迹引导的VLA策略与轨迹世界模型进行测试时搜索-预测-评分对齐，属于具身智能动作规划与控制范畴，映射到[具身世界模型与规划控制]。

Importance reason: Solid robotics/VLA paper with world-model test-time refinement, but outside the user's core continuous-multimodal and generation focus. Signals: world model alignment, mobile manipulation, VLA policy, test-time search-predict-score, trajectory-guided action generation
...
### [35] [SG-WAM: Self-Guided World Modeling in Geometry-Aware Policy Space](https://arxiv.org/abs/2608.01397)
...
TL;DR: SG-WAM 是一种自引导世界建模框架，在策略派生的表征空间中学习几何感知的动作条件动力学，通过可学习动力学 token、自引导世界预测器和流匹配动作生成联合优化，在 LIBERO 上达到 98.5% 成功率。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型（WAM）通常只在感知密集的观测空间或辅助潜空间中建模未来动力学，难以同时满足动作相关性与几何感知性，导致动作生成与未来状态预测的耦合不够有效。

Method: SG-WAM 引入可学习动力学 token 和自引导世界预测器，在策略派生的表征空间中预测未来潜状态；预测目标由同一策略骨干的指数移动平均副本生成，提供稳定监督；几何监督进一步结构化策略图像 token 表征，为动力学 token 提供空间上下文；潜未来预测、几何接地与流匹配动作生成在统一框架中端到端联合优化。

Result: 基于 0.9B 模型且无需大规模具身预训练，SG-WAM 在 LIBERO 上达到 98.5% 平均成功率，在 LIBERO-Plus 上达到 73%，并在分布内与分布外真实世界评测中均优于强基线。

Conclusion: 在策略派生的表征空间中联合建模动作相关且几何感知的未来动力学，能够有效提升世界动作模型的性能，且无需大规模具身预训练即可取得领先结果。

Classification reason: 论文提出 SG-WAM，一种自引导的具身世界模型框架，在策略派生的表征空间中学习几何感知的动作条件动力学，用于机器人动作生成与未来状态预测，属于具身世界模型与规划控制方向。

Importance reason: Strong world-model robotics paper with flow-matching action generation, but robotics focus and lack of continuous-language/multimodal keywords reduce top-tier priority. Signals: world model, flow matching, action-conditioned dynamics, LIBERO 98.5%, geometry-aware policy space
...
### [36] [Demystifying When and Why VLAs Fail in Contact-Rich Tasks and How to Fix Them](https://arxiv.org/abs/2608.01402)
...
TL;DR: 本文系统分析 VLA 模型在接触丰富操作任务中失败的根本原因，识别出两类失败模式：精度失败源于 flow-matching 策略训练不匹配，力失败源于力信号的特殊结构。针对两类失败分别设计机制并组合为 FACT，在近 2500 次真实机器人 rollout 中达到 66% 平均成功率，远超最佳基线 41%。


<details>
  <summary>Details</summary>
Motivation: 先前工作主要通过力增强架构和训练期正则化来处理接触失败，但失败的根本原因尚未被充分探索。作者旨在揭示 VLA 模型在需要精确物理交互的接触丰富任务中失败的具体机制，从而设计针对性解决方案。

Method: 作者识别出两类失败模式：精度失败源于 flow-matching 策略训练不匹配，力失败源于力信号的特殊结构。针对每类失败模式设计针对性机制，并将两者组合为 FACT 方法，在五个接触丰富任务上进行真实机器人评估。

Result: FACT 在五个接触丰富任务上达到 66% 平均成功率，而最佳先前基线仅为 41%，评估涵盖近 2500 次真实世界 rollout。

Conclusion: 通过揭示 VLA 在接触丰富任务中失败的两类根本原因并设计针对性修复机制，FACT 显著提升了接触丰富操作任务的性能，为理解 VLA 模型在物理交互场景中的局限提供了深入洞见。

Classification reason: 论文研究 Vision-Language-Action (VLA) 模型在接触丰富操作任务中的失败原因与修复方法，属于机器人具身智能与动作策略学习范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Solid robotics analysis but only tangentially touches continuous multimodal/world-model priorities, with no direct video/image generation or efficiency focus. Signals: contact-rich manipulation, VLA failure analysis, flow-matching policy, force signals, real-world robot evaluation
...
### [37] [GenTrack: Physical Alignment for Robot-Native Motion Generation and Zero-Shot Humanoid Tracking](https://arxiv.org/abs/2608.01410)
...
Importance: 普通相关 (53.86) - 未入选精读

TL;DR: GenTrack 提出一种在线生成器-跟踪器协同训练框架，通过执行接地、组相对生成器对齐与跟踪器训练交替进行，缩小重定向参考动作与机器人原生动作之间的可执行性差距，在 Unitree G1 上实现零样本人形跟踪。


<details>
  <summary>Details</summary>
Motivation: 通用人形跟踪器依赖大规模具身语料库，扩展成本高昂；文本到动作生成器虽可提供可扩展监督，但基于人类动作或重定向数据训练的模型存在运动学合理性与机器人可执行性之间的差距。现有单向流程要么只修复生成语料，要么只修复奖励跟踪器，无法从根本上解决可执行性差距。

Method: GenTrack 是一个在线生成器-跟踪器框架，交替进行执行接地的、组相对的生成器对齐与在新生成参考上训练跟踪器，并通过锚定（anchoring）和排练（rehearsal）约束漂移。在 Unitree G1 上使用 ProtoMotions 和 SONIC 骨干网络进行评测。

Result: 在三个零样本跟踪划分（包括公开 AMASS、LAFAN 基准和包含 1024 对提示-动作对的私有分布外测试集）上，在线协同训练一致地产生输出更可执行动作且语义对齐更强的生成器，以及零样本覆盖更广、跟踪精度更高的跟踪器，尤其在分布外参考上表现突出。

Conclusion: 联合在线后训练有效缩小了重定向参考与机器人原生动作之间的可执行性差距，无需额外数据采集即可推进零样本人形控制，突破了静态参考池的局限。

Classification reason: 论文聚焦人形机器人零样本跟踪与动作生成，属于具身智能/机器人控制范畴，映射到具身世界模型与规划控制这一规范子主题。

Importance reason: Solid robotics co-training method, but far outside the user's core focus on continuous multimodal models, autoregressive generation, and video world models. Signals: humanoid tracking, zero-shot, generator-tracker co-training, execution grounding, Unitree G1
...
### [38] [When Replanning Becomes the Bottleneck: Budgeted Replanning for Embodied Agents](https://arxiv.org/abs/2608.01428)
...
TL;DR: 提出 BRACE 控制器，将具身智能体的 LLM 重规划建模为预算化控制循环，通过决定是否重规划、选择重规划模式并分配 token 预算与延迟 SLO，配合 E-RECAP 渐进式 token 剪枝方法，在 Meta Habitat、RoboFactory 和 AirSim 上显著降低重规划 token 消耗与 SLO 违规率。


<details>
  <summary>Details</summary>
Motivation: 具身智能体频繁重规划以应对执行漂移、部分可观测性和协调风险，但每次 LLM 重规划调用会累积不断增长的文本上下文，导致重规划延迟出现重尾分布并错过实时截止时间，即使任务成功率仍然很高也难以从平均延迟或成功率中察觉。

Method: 提出 BRACE 控制器，将重规划形式化为预算化控制循环：决定是否重规划、选择重规划模式、分配显式 token 预算与延迟服务级目标（SLO），并考虑可选效率模块。同时提出 E-RECAP，一种成本感知的渐进式 token 剪枝方法，预测 token 效用并在 transformer 层间剪枝重规划上下文，同时保留关键的头尾 token。

Result: 在 Meta Habitat、RoboFactory 和 AirSim 上，BRACE + E-RECAP 将重规划调用的 token 数减少 62-92%，SLO 违规率从 85.5-100.0% 降至 4.7-50.0%（在任务成功率已饱和的设置中）。在更难的 RoboFactory 设置中，开环、冻结计划和 No BRACE 均失败，而 BRACE + E-RECAP 达到 80.0% 成功率且仅 4.6% SLO 违规。

Conclusion: 尾部感知的逐调用预算分配在多种具身平台上有效，表明将重规划视为预算化控制问题而非单纯优化成功率，是解决具身智能体实时性瓶颈的关键方向。

Classification reason: 论文研究具身智能体的重规划（replanning）控制问题，属于机器人规划与控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制 子主题。

Importance reason: Focuses on embodied agent replanning efficiency rather than core continuous/multimodal generation or world-model learning, with limited direct alignment to your priority subtopics. Signals: embodied agents, budgeted replanning, token pruning, latency SLO, world model tangential
...
### [39] [DynamicManip: Enabling Dynamic Manipulation from a Single Static Demonstration](https://arxiv.org/abs/2608.01452)
...
TL;DR: DynamicManip 提出从单个静态演示合成多样化动态操作演示的数据增强管线，并引入动态感知自适应策略，根据任务动态调整推理频率，构建动态操作基准，在仿真与真实环境中显著提升数据效率与成功率。


<details>
  <summary>Details</summary>
Motivation: 动态操作是机器人在复杂动态环境中与运动物体交互的关键能力，但学习此类任务面临两大挑战：动态场景的组合复杂度导致数据需求巨大，以及动态快速变化要求实时且准确的政策执行。

Method: 提出三部分方案：(1) 静态到动态数据增强管线，从单个静态演示合成多样化动态操作演示；(2) 动态感知自适应策略，根据任务动态调整推理频率；(3) 动态操作基准，包含多样动态任务与自动评估系统。

Result: 在仿真与真实环境中，DynamicManip 在数据效率上显著提升，动态操作任务平均成功率提高 18.4 个百分点，策略查询延迟降低 32.9%。

Conclusion: DynamicManip 通过高效数据增强与低延迟自适应策略，有效解决了动态操作任务中的数据稀缺与实时执行难题，验证了其在仿真和真实场景中的有效性。

Classification reason: 论文面向机器人动态操作任务，提出静态到动态的数据增强管线与低延迟模仿策略，属于具身智能中的动作策略学习与规划控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Solid robotics data augmentation work, but tangential to the user's core focus on continuous multimodal models and generative world models. Signals: world model, data augmentation, dynamic manipulation, imitation policy, adaptive inference
...
### [40] [AffordTrajDP: Dynamic Affordance-Guided Visuomotor Policy Learning for Robotic Manipulation](https://arxiv.org/abs/2608.01603)
...
TL;DR: AffordTrajDP提出一种动态affordance引导的视觉运动策略学习方法，通过物体中心时序传播构建affordance轨迹，为机器人操作提供时间一致、状态感知的渐进式引导，在ManiSkill3和真实机器人上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的affordance引导模仿学习通常使用静态affordance（如固定接触点），但在精度关键任务或物体位置扰动下，静态affordance会变得不一致，导致接触后轨迹漂移。作者希望解决静态affordance在动态操作过程中的不一致性问题。

Method: 给定RGB-D观测，核心思想是检索一个锚点affordance（捕获末端执行器与目标物体之间的期望接触点），然后通过affordance传播，利用物体的SE(3)位姿作为自然传播媒介，向前传播得到affordance轨迹，为整个执行过程提供时间一致、状态感知的渐进式引导。

Result: 在ManiSkill3上达到70.0%的平均成功率，比强基线高出最多17.8%。在Galaxea A1和UR7e机械臂上的真实世界实验（StackCube、PickCup、AdapterInsertion、Ring-on-Peg、Put-in-Bowl、USB Insertion）进一步验证了在物体位置变化和外观变化下的鲁棒性。

Conclusion: 动态affordance轨迹相比静态affordance能提供更一致、状态感知的操作引导，有效缓解接触后轨迹漂移问题，在仿真和真实机器人上均验证了方法的有效性与鲁棒性。

Classification reason: 该论文面向机器人操作任务，提出基于动态affordance轨迹引导的视觉运动策略学习，属于具身智能中的策略学习与规划控制范畴，映射到[具身世界模型与规划控制]这一规范子主题。

Importance reason: Robotics manipulation paper with limited overlap with the user's core focus on continuous multimodal models and video/world-model generation. Signals: robotic manipulation, affordance-guided policy, world model tangential, no video generation, no continuous tokenization
...
### [41] [A Forward-Inverse Dynamic Game Framework for Enhanced Multi-Agent Trajectory Planning](https://arxiv.org/abs/2608.01636)
...
TL;DR: 提出一种前向-逆向动态博弈框架，用于非线性动力系统中多智能体轨迹规划，通过KL正则化动态博弈和最大熵逆强化学习推断未知目标，实现自适应平衡最优性与行为先验。


<details>
  <summary>Details</summary>
Motivation: 现有动态博弈方法假设智能体完全理性且目标已知，或依赖固定正则化，难以刻画安全关键场景中有界理性与空间变化的交互强度。

Method: 提出KL正则化动态博弈，采用状态依赖权重自适应平衡最优性与行为先验；开发基于最大熵逆强化学习并带物理信息正则化的上下文感知逆向博弈模块，保证与前向博弈的结构一致性。

Result: 建立了正则化局部博弈的逐迭代良定性，证明自适应权重函数在有限名义轨迹更新下保持Lipschitz连续；在协作导航与汇合场景的多机器人实验验证了框架有效性。

Conclusion: 该框架能有效处理未知目标与有界理性下的多智能体轨迹规划，为安全关键场景中的动态博弈求解提供了更鲁棒的方法。

Classification reason: 论文研究多智能体轨迹规划，属于机器人运动规划与控制范畴，与具身世界模型与规划控制这一规范主题高度相关。

Importance reason: This robotics trajectory planning paper is outside the user's core focus on continuous multimodal models and generative video/image modeling. Signals: multi-agent trajectory planning, dynamic game theory, inverse reinforcement learning, robotics (cs.RO), no generative model or continuous token focus
...
### [42] [Weights or Skills? A Survey of Robot-Learning Techniques: from Action-Predicting Weights to Robots that Write their Own Skills](https://arxiv.org/abs/2608.01851)
...
TL;DR: 一篇机器人学习技术综述，围绕"将能力烘焙进冻结权重（VLA模型）"与"智能体编写并改进自身可执行技能代码"两大路线组织领域，深入分析代码即策略方法的自我改进程度，并连接新兴的技能经济。


<details>
  <summary>Details</summary>
Motivation: 机器人学习领域正分裂为两条路线：将能力固化进权重的VLA模型，以及让智能体自行编写和迭代技能代码。作者希望围绕"权重vs技能"这一轴心系统化梳理该领域，并澄清"技能"一词的五种不同含义，为研究者提供清晰的组织框架。

Method: 作者提出一个以"权重vs技能"为轴心的分类体系，将代码即策略方法按自我改进程度排列（从零样本程序合成、闭环自修复、持久技能记忆，到执行反馈+技能记忆+进化搜索结合的开放循环），并映射技能极（从无监督RL技能发现到LLM技能库）。综述考察77个代表性系统、六个技术家族，并提供自我改进机制的操作性定义。

Result: 识别出"技能"一词至少有五种不同含义，其中只有代码含义能在无梯度更新下自我改进；发现只有少数近期系统（如ASPIRE、ENPIRE、RoboClaw）占据执行反馈+技能记忆+进化搜索结合的开放循环单元；并指出商业机器人技能市场仅提供静态回放，暴露了适配、跨具身可移植性、溯源、安全验证、组合与标准化等开放问题。

Conclusion: 该综述提供了一个聚焦而非穷尽的机器人学习领域组织框架，明确了各技术家族的能力边界与不能做什么，并指出技能经济中静态回放与开放问题之间的差距，为后续研究指明方向。

Classification reason: 该论文是机器人学习技术综述，围绕"权重vs技能"轴组织VLA模型与代码即策略方法，属于具身智能与机器人学习范畴，映射到具身世界模型与规划控制这一规范子主题。

Importance reason: A robotics survey with limited direct overlap with your core continuous multimodal and video generation priorities. Signals: world_model direction, robot learning survey, code-as-policy vs VLA, skill economy, no continuous token/flow matching focus
...
### [43] [Roomer: Reflective Object-Grounded Model Editing and Repair for 3D Indoor Layout Synthesis](https://arxiv.org/abs/2608.01973)
...
TL;DR: Roomer 是一个反思式对象级 3D 室内布局修复框架，将布局违规建模为稀疏的对象级修复问题，通过 RoState 编码、RoReview 绑定违规对象、几何条件 VLM 规划器提出局部编辑，并由确定性求解器验证，最终通过全场景验证确保修复不引入新违规。


<details>
  <summary>Details</summary>
Motivation: 现有室内布局生成器虽能产生全局合理的布局，但常保留局部违规（碰撞、越界、遮挡开口、阻断通行）。已有工作多聚焦全场景合成或场景级优化，缺乏识别责任对象并局部修复受影响区域的能力。

Method: Roomer 将布局编码为 RoState，用 RoReview 将测量到的违规绑定到相关对象；几何条件 VLM 规划器提出结构化局部编辑，确定性求解器验证并生成候选编辑集合；每个候选仅在通过全场景验证（确认解决目标违规且不引入新硬违规或破坏受保护约束）后才被提交。规划器在 Roomer-CC 受控损坏数据集上训练，并引入 Roomer-Eval 评测分布质量、物理有效性与实际可用性。

Result: 实验表明 Roomer 能修复残留违规同时保留有效区域，提升物理有效性与可用性，并可跨外部生成器迁移。

Conclusion: Roomer 证明了将布局违规视为稀疏对象级修复问题、结合反思式 VLM 规划与确定性验证的有效性，为室内布局生成后的局部修复提供了新范式。

Classification reason: 论文面向 3D 室内布局生成与修复，属于具身/环境场景建模与规划控制范畴，与 [CANONICAL] 具身世界模型与规划控制 主题相关。

Importance reason: Focuses on 3D indoor layout repair, which is tangential to the user's core interests in continuous multimodal models and autoregressive video/image generation. Signals: 3D layout synthesis, object-grounded repair, vision-language model planner, world model (weak), no autoregressive/diffusion video/image generation
...
### [44] [MANGO-Grasp: Mahalanobis Fields over Geometry-Oriented 3D Gaussians for Cross-Embodiment Dexterous Grasping](https://arxiv.org/abs/2608.02014)
...
TL;DR: 本文通过仿真与真实机器人实验，系统研究了 action chunking（动作分块）提升行为克隆性能的原因。作者否定了现有的时间一致性、视界缩减和表征学习三种假设，提出 action chunking 的核心优势在于隐式集成（implicit ensembling）：通过学习多种时间关系（a_t|o_t, a_t|o_{t-1}, ...），其行为类似模型集成，增强了鲁棒性与泛化能力。基于此，作者提出无需 action chunking 即可匹配其性能的方法，并设计了一种显式实例化集成的策略类，在多个领域显著超越 action chunking。


<details>
  <summary>Details</summary>
Motivation: Action chunking 是学习有效机器人控制策略的关键组件，但其成功原因缺乏精确理解。现有假设（时间一致性、视界缩减、表征学习）尚未被严格验证，作者希望填补这一理论空白。

Method: 通过仿真与真实世界的严格实验评估，逐一检验现有假设；提出隐式集成机制解释 action chunking 优势；设计延迟策略（delayed policies）与随机延迟集成方法，以及显式实例化集成的策略类来放大其收益。

Result: 现有三种假设均无法解释 action chunking 的成功；action chunking 受益于更大的非马尔可夫表达力与更低的复合误差，但这些效果可被延迟策略完全捕获；隐式集成提供了额外收益。通过随机延迟集成可匹配 action chunking 性能，显式集成策略类在多个领域显著超越 action chunking。

Conclusion: Action chunking 的核心优势来自隐式集成机制，而非时间一致性等传统解释。这一发现为设计更高效的机器人控制策略提供了新思路，无需 action chunking 也能达到甚至超越其性能。

Classification reason: 论文研究机器人控制策略中 action chunking 对行为克隆性能提升的原因，属于具身智能/机器人策略学习范畴，映射到 [CANONICAL] 具身世界模型与规划控制 子主题。

Importance reason: Provides a rigorous causal analysis of action chunking in robotic control, directly relevant to world models and planning but tangential to the user's core continuous/multimodal generation focus. Signals: world model, robotic control, action chunking, implicit ensembling, behavioral cloning
...
### [46] [Faster-WAM: Do World Action Models Need Deep Action Modules?](https://arxiv.org/abs/2608.02365)
...
Importance: 高优先级 (89.08) - 未入选精读

TL;DR: Faster-WAM 提出 Dock of Transformer (DoT) 视频中心设计原则，将预训练视频 Transformer 作为表征枢纽，通过 docking 接口连接轻量输出头，实现单层动作头与 30 层视频骨干的解耦，在 LIBERO 和 RoboTwin 2.0 上取得竞争性性能，推理延迟仅 66.5ms，比 Fast-WAM 快 3.2 倍。


<details>
  <summary>Details</summary>
Motivation: 现有 World Action Models (WAMs) 将动作模块深度与视频骨干深度绑定，导致大量计算开销和高推理延迟。作者希望解耦动作模块与视频骨干的深度，实现低延迟且高性能的动作预测。

Method: 提出 Dock of Transformer (DoT) 视频中心设计原则：将预训练视频 Transformer 视为表征枢纽，通过 docking 接口连接轻量输出头。docking 接口融合所有视频层的 keys 和 values，并应用 RoPE 重对齐。Faster-WAM 将单层动作头 dock 到 30 层视频骨干上。

Result: 无需额外具身预训练，Faster-WAM 在 LIBERO 和 RoboTwin 2.0 上取得竞争性性能，并在 LIBERO-Plus 上展现强分布外泛化能力。端到端延迟最低，仅 66.5ms/次推理，较 Fast-WAM 提速 3.2 倍。

Conclusion: 视频中心的 DoT 架构支持灵活的任务特定输出头设计，同时实现低推理延迟、强动作预测性能和稳健泛化，表明世界动作模型不一定需要深层动作模块。

Classification reason: 论文提出 Faster-WAM，将机器人动作预测与视频世界模型耦合，属于具身智能中的动作条件世界模型，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: 视频中心架构解耦动作头与视频骨干，显著降低推理延迟，契合世界模型与高效推理双重关注。 Signals: world model, efficient inference, video backbone, action head, low latency
...
### [47] [Agentic Commerce World: An Auditable and Verifiable Environment for Vibe Commerce](https://arxiv.org/abs/2608.02441)
...
Importance: 普通相关 (51.31) - 未入选精读

TL;DR: 本文提出 Agentic Commerce World (ACWorld)，一个可审计、可验证的智能体商务环境，通过 Vibe Commerce Protocol (VCP) 验证智能体动作并记录交互，用于评估 Buyer 与 Merchant 智能体在共享市场中的持续交易行为，并配套 200 任务能力覆盖与 60 任务大规模目录两个基准轨道。


<details>
  <summary>Details</summary>
Motivation: 类比 vibe coding，vibe commerce 允许人们用自然语言表达买卖目标并委托给 AI 智能体。然而商务场景需要独立控制的 Buyer 与 Merchant 智能体在共享市场中交互，同时保持各自的私有目标与不同权限，现有环境难以支持这种多智能体交互的可审计评估。

Method: 提出 Agentic Commerce World (ACWorld) 环境，通过 Vibe Commerce Protocol (VCP) 在更新共享交易状态前验证智能体动作，并记录交互过程使行为可审计、评估可复现。构建 ACWorld Benchmark，包含 200 任务能力覆盖轨道和 60 任务大规模目录轨道（搜索 785,022 条可交易列表）。

Result: 在十个模型上，能力覆盖轨道平均得分 65.9%-85.6%，大规模目录轨道 56.1%-91.4%。分析表明过程级证据是必要的：仅看最终状态会遗漏评估错误，不完整轨迹仍保留有用过程信号，大规模目录任务暴露各阶段瓶颈。

Conclusion: ACWorld 提供了可审计、可验证的智能体商务评估环境，强调过程级证据对评估多智能体交易系统的重要性，为 vibe commerce 场景下的智能体研究提供了基准与基础设施。

Classification reason: 论文提出 Agentic Commerce World (ACWorld)，一个用于评估 Buyer 与 Merchant 智能体在共享市场环境中交互的环境，属于具身/智能体世界模型与规划控制范畴，映射到 [CANONICAL] embodied_world_model。

Importance reason: This paper focuses on agentic commerce evaluation, which is outside the user's core interests in continuous multimodal models, autoregressive generation, and world models for video. Signals: agentic commerce, world model (weak, non-visual), auditable environment, benchmark, no continuous language/multimodal
...
### [48] [Cross-Benchmark Generalization in Long-Horizon Agents](https://arxiv.org/abs/2608.00181)
...
TL;DR: 本文通过两阶段 SFT-then-RL 流水线在 363 个长时程 MCP 任务上对 Qwen3.5-122B-A10B 进行后训练，证明跨基准泛化能力：在 Toolathlon、τ²-Bench、BFCL-V4、SWE-Bench Pro、Terminal-Bench 2 五个外部评测上均优于基座模型，且软件工程基准在训练集不含软件工程任务的情况下仍显著提升。配对轨迹分析识别出四种可迁移的行为差异。


<details>
  <summary>Details</summary>
Motivation: 在自包含环境中训练的 RL 策略可能通过利用环境特定规律（工具模式、评分解析、任务模板）而非获得可迁移技能来获取奖励，分布内留出集共享这些规律，无法区分真正的技能习得。作者认为跨基准迁移是检验智能体是否获得可迁移行为的关键判别性问题。

Method: 采用两阶段 SFT-then-RL 流水线，在 363 个长时程 MCP 任务（27 个类别）上对开源 MoE 模型 Qwen3.5-122B-A10B 进行后训练。训练中不引入任何外部基准任务、评分器或外部分数，仅用 Toolathlon 性能指导基座家族与 SFT 教师选择。随后在五个外部评测基准上以贪婪 pass@1 评估，并进行配对轨迹分析识别行为差异。

Result: 训练模型在五个外部评测上均优于基座：Toolathlon +9.6pp、τ²-Bench +5.3pp、BFCL-V4 +3.5pp、SWE-Bench Pro +5.8pp、Terminal-Bench 2 +2.8pp。两个软件工程基准在训练集不含软件工程任务的情况下仍提升。配对轨迹分析识别出四种可迁移行为差异：更谨慎的局部目标形成、构建目标相关的工作状态、通过局部修复保持父目标稳定、验证完成情况。

Conclusion: 长时程多工具后训练可以改变智能体的工作方式，且这些改变能迁移到训练领域之外，为跨基准泛化提供了描述性证据。这提示跨基准迁移是评估智能体是否真正习得可迁移技能而非环境特定规律的有效判别方法。

Classification reason: 论文研究长时程智能体（long-horizon agents）的跨基准泛化，涉及多工具任务规划、动作执行与策略学习，属于具身/智能体世界模型与规划控制范畴，映射到 [CANONICAL] embodied_world_model。

Importance reason: Solid cross-benchmark generalization evidence for agentic RL, but tangential to the user's core continuous-multimodal and generation priorities. Signals: agentic RL, cross-benchmark transfer, long-horizon MCP tasks, SFT-then-RL, world model adjacent
...
### [49] [WM-Cov: Test Adequacy for Interactive World-Model-Style Autonomous Driving Simulation](https://arxiv.org/abs/2608.00298)
...
TL;DR: 本文提出 WM-Cov，一个与提供商无关的评估层，用于衡量交互式世界模型风格自动驾驶仿真的测试充分性，将原始提供商输出转换为请求、实现和有效证据，并通过覆盖增长、有效失败发现、失败模式多样性等指标报告充分性。


<details>
  <summary>Details</summary>
Motivation: 世界模型和生成式模拟器正成为自动驾驶的交互式测试基础设施，它们能对规划器做出反应并产生反事实、罕见和安全关键的轨迹。但测试场景从固定回放轨迹变为交互式场景族，其演化取决于被测规划器，因此需要明确什么样的闭环证据足以支持测试意图和停止决策。

Method: 提出 WM-Cov 评估层，将原始提供商输出转换为请求、实现和有效证据，通过覆盖增长、有效失败发现、失败模式多样性、真实性、伪影抑制、重复计数和有效证据精度等指标报告测试充分性。

Result: 在 TeraSim/SUMO 事件、WM 混合轨迹池和真实 DriveArena TrafficManager-WorldDreamer 矩阵上的研究表明，危险事件可能包含有效 ADS 失败、重复、部分实现和伪影。DriveArena 矩阵评估两个规划器、两个时域、六个提示条件和 360 个 ego 路线请求，304 次尝试成为完全实现证据，56 次保持部分。

Conclusion: 世界模型风格测试应通过预算下有效交互证据的收敛来评估，而非仅依赖原始生成失败或提示覆盖。

Classification reason: 论文关注交互式世界模型风格的自动驾驶仿真测试充分性，属于世界模型方向下的可交互与物理世界建模（interactive_physical_world）范畴，该 canonical 子主题涵盖交互式模拟器与环境建模。

Importance reason: Focuses on test adequacy metrics for autonomous driving world models, which is peripheral to your core interests in continuous multimodal models and generative video. Signals: world model testing, autonomous driving simulation, coverage metrics, provider-agnostic evaluation, not aligned with priority subtopics
...
TL;DR: 本文证明随机世界模型的"未来为何分叉"（状态混叠 vs 过程噪声）无法从普通转移数据中辨识，并提出 ClosurePairs 干预式评估协议，通过交叉兼容微状态与重复外生扰动进行两向方差分解，从而识别状态混叠、过程噪声及其非线性交互。


<details>
  <summary>Details</summary>
Motivation: 随机世界模型通常用预测未来的准确性和校准度来评估，但这些标准无法区分"观测混叠了不同物理状态"与"在完整状态确定后动力学仍随机"这两种决策相关的歧义，导致评估存在根本性盲区。

Method: 提出 ClosurePairs 干预式评估协议：将兼容的微状态交叉配对并施加重复外生扰动，通过两向方差分解识别状态混叠、过程噪声及其非线性交互；当扰动不可复用时采用独立重复变体。在似然等价的高斯系统、18 种非线性 Langevin 条件、像素条件循环模型及 REFINE/BRANCH 测试上验证。

Result: 配对监督在相同测试 NLL 下将混叠分数误差降低 15.96 倍；非线性 Langevin 条件下归因 MAE 从 0.372 降至 0.051，感知遗憾从 0.0138 降至 0.0003；像素条件模型上混叠分数 MAE 从 0.584 降至 0.130（分布内）；REFINE/BRANCH 测试中总方差路由准确率 66.48%，而 ClosurePairs 达 99.99% 并改善选定 NLL。

Conclusion: ClosurePairs 能够测量"未来为何分叉"这一信息，而这是传统预测评分无法辨识的，为随机物理世界模型提供了更严格的评估协议。

Classification reason: 论文研究随机物理世界模型的评估协议，关注状态混叠、过程噪声与动力学建模的可辨识性，属于物理世界建模与动力学建模范畴，映射到 [CANONICAL] 可交互与物理世界建模。

Importance reason: Directly advances world model evaluation with an interventional protocol for distinguishing state aliasing from process noise, highly relevant to your world model priority. Signals: world model, neural simulator, interventional evaluation, identifiability, state aliasing
...
### [51] [DynActiveGS: Active Gaussian Splatting for Dynamic Scene Reconstruction](https://arxiv.org/abs/2608.01178)
...
TL;DR: DynActiveGS 提出一个基于3D高斯泼溅的动态感知主动重建框架，通过在线不确定性预测和不确定性加权优化抑制运动干扰观测，将不确定性显式分解为结构不确定性与运动诱导不确定性，并据此进行动态感知视点选择与路径规划，形成闭环的鲁棒主动重建系统。


<details>
  <summary>Details</summary>
Motivation: 现有主动重建方法主要针对静态场景，在动态环境中运动物体会污染观测数据，导致重建精度和完整性下降。需要一种能够区分静态欠重建区域与动态不可靠区域、并在探索过程中主动选择信息丰富且稳定观测的框架。

Method: 基于3DGS增量重建场景，通过在线不确定性预测与不确定性加权高斯优化抑制运动干扰；将不确定性显式分解为结构不确定性与运动诱导不确定性，以区分欠重建静态区域与动态不可靠区域；基于不确定性场进行动态感知视点选择与动态约束路径规划，形成闭环主动重建管线。

Result: 在多个动态基准上，相比现有主动重建基线，在重建精度、完整性、渲染质量和探索效率方面均取得一致提升。

Conclusion: DynActiveGS 通过显式分解结构不确定性与运动诱导不确定性，实现了动态场景下鲁棒的主动重建，验证了不确定性引导的视点选择与路径规划在动态环境探索中的有效性。

Classification reason: 该论文提出基于3DGS的动态场景主动重建框架，用于自主探索中的环境建模，属于物理世界建模与神经仿真器范畴，映射到世界模型方向下的可交互与物理世界建模（interactive_physical_world）规范子主题。

Importance reason: Solid active 3DGS reconstruction work, but only tangential to the user's core focus on continuous multimodal models and autoregressive generation. Signals: 3D Gaussian Splatting, active reconstruction, dynamic scene, uncertainty decomposition, viewpoint selection
...
### [52] [DecoupleGS: Interactive 3D Gaussian Splatting for End-to-End Autonomous Driving Testing](https://arxiv.org/abs/2608.01761)
...
Importance: 值得关注 (70.01) - 未入选精读

TL;DR: DecoupleGS提出一种解耦的3D高斯泼溅框架，将场景分解为高保真静态背景与可操控动态智能体，用于端到端自动驾驶算法的闭环仿真测试，兼顾视觉保真度、交互性与实时性。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶算法需要在仿真环境中进行严格的闭环验证，要求高视觉保真度、强交互性和实时性能。现有方法（从游戏引擎到静态神经渲染）在这些需求间存在权衡，难以支持E2E测试所需的动态场景组合。

Method: 提出解耦的3DGS框架，将场景分解为高保真静态背景和可操控动态智能体，采用以物体为中心的规范表示。引入三个模块：(1)基于感知剪枝和向量量化的资产压缩用于实时交通渲染；(2)利用语义拓扑的图引导几何配准以严格对齐轨迹；(3)基于代理的重光照以传递环境光照实现无缝光度集成。

Result: 实验表明DecoupleGS在保真度与效率之间取得平衡，改善了度量和光度一致性，为端到端自动驾驶评估提供了实用的闭环传感器仿真平台。

Conclusion: DecoupleGS通过解耦静态背景与动态智能体的3DGS框架，为E2E自动驾驶测试提供了兼顾高保真、可交互与实时性的闭环仿真方案，验证了神经渲染在自动驾驶评测中的实用价值。

Classification reason: 该论文提出基于3DGS的自动驾驶闭环仿真测试平台，强调高保真、可交互、实时性的动态场景组合，属于可交互世界建模与神经仿真器范畴，映射到[世界模型]方向的[CANONICAL]子主题"可交互与物理世界建模"。

Importance reason: Focuses on autonomous driving simulation rather than the user's core interests in continuous multimodal models, autoregressive generation, or world models for video. Signals: 3D Gaussian Splatting, autonomous driving testing, closed-loop simulation, static/dynamic scene decomposition, real-time rendering
...
### [53] [Hybrid Lagrangian-Eulerian Model for Lagrangian Fluid Simulation](https://arxiv.org/abs/2608.01164)
...
TL;DR: 提出一种混合拉格朗日-欧拉神经模拟器，通过自适应下采样消除运动学冗余、以欧拉网格作为全局空间锚点结合交叉注意力修正轨迹漂移，在拉格朗日流体仿真中显著抑制误差累积，达到新的精度与滚动稳定性SOTA。


<details>
  <summary>Details</summary>
Motivation: 纯拉格朗日神经模拟器虽具有几何灵活性和精确平流优势，但缺乏固定全局参考系，导致两个严重问题：一是空间瓶颈，密集粒子邻域在均匀区域浪费模型容量；二是快速时间漂移，纯局部消息传递缺乏全局锚点。

Method: 借鉴经典混合数值求解器思想，提出混合拉格朗日-欧拉神经模拟器：引入自适应下采样消除运动学冗余，在粒子上保留微尺度细节，同时将压缩特征聚合到欧拉节点以解析大尺度动力学；采用交叉注意力机制查询欧拉特征，以固定网格作为稳定空间锚点，在每个时间步修正轨迹偏差。

Result: 综合实验表明，这种分层交叉注意力设计能大幅抑制误差累积，在拉格朗日流体仿真的精度与滚动稳定性上建立了新的最先进水平。

Conclusion: 混合拉格朗日-欧拉架构有效解决了纯拉格朗日神经模拟器的空间瓶颈与时间漂移问题，为流体仿真等物理世界建模提供了更稳定、更准确的神经模拟方案。

Classification reason: 该论文提出混合拉格朗日-欧拉神经模拟器用于流体仿真，属于物理世界建模与神经仿真器范畴，与 [CANONICAL] 子主题 interactive_physical_world（可交互与物理世界建模）高度匹配。

Importance reason: Strong world-model-adjacent neural simulator work, but its Lagrangian fluid focus is only partially aligned with your video/world-model priorities. Signals: neural simulator, world model, Lagrangian fluid, Eulerian grid, cross-attention
...
### [54] [WorldExam: Benchmarking World Models from Apparent Appearance to Inherent Reactivity](https://arxiv.org/abs/2608.02603)
...
TL;DR: WorldExam 是一个分层诊断基准，从视觉质量、控制遵循、空间一致性和世界反应性四个层级评测可控视频生成模型作为世界模型的能力，包含 1474 个案例和 8 个任务，统一评测相机、动作和语言驱动的模型范式。


<details>
  <summary>Details</summary>
Motivation: 可控视频生成模型越来越多地被用作世界模型，但现有基准主要评估视觉质量或显式指令满足，忽略了世界模型的核心能力——从场景状态推断世界应如何反应并生成输入中未显式描述的合理后果（即固有反应性）。

Method: 提出 WorldExam 分层诊断基准，包含四个层级：视觉质量、控制遵循、空间一致性、世界反应性。涵盖 1474 个案例和 8 个专门任务，支持对相机驱动、动作驱动和语言驱动三种模型范式的统一评测。世界反应性层级评估超出输入显式描述的场景条件反应和目标导向行为。

Result: 对 20 个代表性模型的评测揭示了清晰的能力分化：相机驱动模型擅长相机控制但接口不支持动态交互；动作驱动模型能更精确控制主体但常使世界无响应；语言驱动模型在交互上表现更好但遵循复杂控制不够忠实。没有模型能同时覆盖广泛任务并保持稳定强性能。

Conclusion: 高视觉质量和显式指令满足并不能保证固有反应性，现有模型在作为世界模型时存在明显的能力缺口，需要更全面的评测来推动世界模型的发展。

Classification reason: 该论文提出 WorldExam 基准，用于评测可控视频生成模型作为世界模型的能力，属于世界模型方向下的评测工作。world_model 方向下存在非 canonical 的 generation_benchmark 子主题（生成模型评测与数据集），与该论文高度匹配，故复用该子主题。

Importance reason: Directly benchmarks world models and video generation reactivity, aligning with top-priority world model and video generation directions. Signals: world model benchmark, video generation evaluation, reactivity testing, camera/action/language-driven models, 20 models compared
...
### [55] [MiniWorld: Democratizing the Training of Video World Models from Scratch](https://arxiv.org/abs/2608.01127)
...
TL;DR: MiniWorld 是一个可复现的从零训练流式视频世界模型框架，采用 block-causal Video Diffusion Transformer + Flow Matching，结合 Diffusion Forcing 的 chunk-wise 非递减噪声调度与两阶段持续训练，推理时用滚动 KV cache 与流水线异步去噪实现高效流式生成，可在单台 8-GPU 服务器上数天内完成训练。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型多依赖对预训练视频生成模型进行后训练或蒸馏，训练流程复杂、计算资源需求大，且存在双向预训练与因果流式推理不匹配的问题。虽然已有研究表明从零训练自回归视频世界模型可行，但社区仍缺乏轻量、透明、完全可复现且能在有限算力下端到端训练的基线。

Method: MiniWorld 采用 block-causal Video Diffusion Transformer，在预训练 Video VAE 的潜空间中使用 Flow Matching 训练。基于 Diffusion Forcing，采用 chunk-wise 非递减噪声调度和两阶段持续训练以提升时序建模与稳定性。推理阶段结合滚动 KV cache 与流水线异步去噪，在有限计算下实现高效流式生成。

Result: 整个模型可在单台 8-GPU 服务器上数天内完成训练，实现了从零训练的视频世界模型，并开源训练/推理代码库与预训练权重。

Conclusion: MiniWorld 提供了一个轻量、透明、完全可复现的从零训练视频世界模型基线，降低了视频世界建模研究的算力门槛，有望促进该领域后续研究。

Classification reason: 论文提出 MiniWorld，一个从零训练流式视频世界模型的框架，采用 block-causal Video Diffusion Transformer 与 Flow Matching，属于视频生成式世界模型与未来预测方向，映射到 [CANONICAL] video_world_model 子主题。

Importance reason: Directly targets the user's top-priority video world model and continuous/multimodal generation directions with a reproducible, efficient training framework. Signals: video world model, flow matching, video diffusion transformer, diffusion forcing, efficient inference
...
### [56] [FactorJEPA: Factorizing Monolithic Futures into Layout-Agent-Interaction Channels for Crowded and Chaotic Global South Urban Worlds](https://arxiv.org/abs/2608.01049)
...
TL;DR: WorldDynCache 提出一种风险控制的潜变量动力学近似框架，通过轻量级潜变量转移风险估计器和条件/相位感知的提升潜变量代理，在不额外进行 transformer 评估的情况下加速扩散世界模型的推理，在 HunyuanVoyager-13B 和 Aether-5B 上分别获得 4.92 倍和 2.15 倍加速，同时保持最优生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散世界模型能生成高质量的未来帧，但重复的 transformer 评估导致推理速度极慢。现有缓存方法复用中间特征、选择性更新 token 或根据局部漂移/短原生空间历史重用和推断去噪输出，但这些标准可能遗漏跨跳过步骤累积的近似诱导潜变量转移缺陷，以及潜变量演化方向的相位或条件相关变化。

Method: WorldDynCache 包含两个核心组件：(1) 轻量级潜变量转移风险估计器，跟踪近似缺陷的累积未来影响，并针对精确锚点处观察到的反事实缺陷校准其预测；(2) 条件与相位感知的提升潜变量代理，无需额外 transformer 评估即可近似潜变量演化。

Result: 在 HunyuanVoyager-13B 和 Aether-5B 上分别实现 4.92 倍和 2.15 倍加速，并在 WorldScore、PSNR、SSIM 和 LPIPS 指标上取得所比较缓存方法中的最佳生成质量。

Conclusion: WorldDynCache 通过风险控制的潜变量动力学近似，在显著加速扩散世界模型推理的同时保持甚至提升生成质量，验证了考虑累积近似缺陷和条件/相位感知的潜变量演化建模对缓存策略的重要性。

Classification reason: 论文提出 WorldDynCache，针对扩散世界模型（diffusion world model）的推理加速，属于视频世界模型范畴，映射到 [CANONICAL] video_world_model。

Importance reason: Directly targets diffusion world model inference acceleration, a high-priority subtopic, with strong speedup results and quality preservation. Signals: diffusion world model, inference acceleration, latent dynamics approximation, risk-controlled caching, HunyuanVoyager-13B
...
### [58] [ProWorld: Progress-Aware Hyperbolic World Models for Long-Horizon Visual Goal Reaching](https://arxiv.org/abs/2608.01926)
...
TL;DR: ProWorld 提出一种进度感知的双曲视觉世界模型，利用目标条件进度序组织视觉潜空间动力学，通过双曲蕴含学习保持轨迹内的方向性进度，并通过双曲未来判别缓解局部相似未来状态的进度歧义，配合进度感知规划目标，在四个视觉目标到达任务上平均绝对成功率提升 9.67。


<details>
  <summary>Details</summary>
Motivation: 现有 JEPA 风格视觉世界模型通过下一步表征预测学习局部转移一致性，但在长时程任务中，局部预测准确并不能保证持续朝目标前进：多步 rollout 可能局部合理却偏离目标相关轨迹；局部相似的未来状态可能对应截然不同的长期进度，在仅优化局部一致性的潜空间中难以区分。

Method: ProWorld 引入目标条件进度序（goal-conditioned progress order），即状态按朝目标推进程度的相对排序，该序具有非对称、由粗到细的结构，适合双曲几何建模。方法包括：利用目标条件进度序组织视觉潜空间动力学；通过双曲蕴含学习（hyperbolic entailment learning）保持轨迹内的方向性进度；通过双曲未来判别（hyperbolic future discrimination）缓解局部相似未来状态的进度歧义；设计进度感知规划目标，联合考虑候选 rollout 与目标的接近度和中间状态的持续进度来打分。

Result: 在四个视觉目标到达任务上，ProWorld 相比 LeWM 取得平均绝对成功率提升 9.67。

Conclusion: ProWorld 表明将目标条件进度序引入双曲几何的视觉世界模型，能有效提升长时程视觉目标到达任务的规划成功率，验证了进度感知建模对长时程世界模型的重要性。

Classification reason: 论文提出 ProWorld，一种基于 JEPA 风格的视觉世界模型，用于长时程视觉目标到达任务，通过预测未来潜表征进行视觉目标规划，属于视频/视觉世界模型范畴，映射到 [CANONICAL] video_world_model。

Importance reason: Directly advances video world models with a novel hyperbolic progress-aware design, strongly matching the user's top-priority world model and continuous latent reasoning interests. Signals: world model, video world model, visual goal reaching, hyperbolic geometry, latent space dynamics
...
### [59] [DF$^3$: World Modeling via Decoder-Free Feature Forecasting in Autonomous Navigation](https://arxiv.org/abs/2608.02428)
...
TL;DR: DF³ 提出一种无解码器的特征预测框架，在冻结视觉基础模型的潜空间中通过可学习空间查询与运动感知上下文融合机制直接预测下一帧特征，并直接产出下游任务输出，完全消除解码器瓶颈，用于自动驾驶导航的世界建模。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶等机器人系统需要从视频序列预测未来状态以进行世界建模。现有像素级生成方法过度关注任务无关细节且计算开销大；潜空间方法虽直接预测特征，但仍依赖重型解码器进行状态到任务的映射，构成计算瓶颈。

Method: DF³ 在冻结的视觉基础模型末端块中注入可学习空间查询以提取未来状态表征。采用轻量统一的运动感知上下文融合（MACF）机制，将粗粒度光流扭曲与细粒度潜空间互相关结合，使查询与历史 token 表征交互以显式对齐并预测下一帧特征。随后用一组任务查询探测预测特征以完成下游任务，全程无需解码器。

Result: 在公开基准和机器人模拟器的零样本部署上，DF³ 达到与 SOTA 方法相当的性能，同时具备更高的效率和灵活性，可集成感知与控制。

Conclusion: DF³ 证明完全在潜空间内进行无解码器的未来特征预测是可行且高效的，为自动驾驶导航中的世界建模提供了更轻量、灵活的方案。

Classification reason: 论文提出 DF³ 框架，在潜空间中预测未来帧特征以建模世界演化，用于自动驾驶导航的未来状态预测，属于视频世界模型/未来预测范畴，映射到 [CANONICAL] video_world_model。

Importance reason: Directly advances the priority subtopic of video world models with a decoder-free latent forecasting approach, highly aligned with continuous latent reasoning interests. Signals: world model, neural simulator, latent feature forecasting, decoder-free, autonomous navigation
...
### [60] [Manifold-GS: Certified Hybrid Assets via Varifold-Conservative Gaussian Splatting](https://arxiv.org/abs/2608.00214)
*Boyang Li*
...
TL;DR: Manifold-GS 提出一种认证混合资产层，将 3DGS 场景中的外观不透明度与几何积分质量分离，将表面类高斯表示为离散无向 varifold，仅导出置信度认证的开放表面补丁，其余内容保留为残差 splat，实现零编辑泄漏、纹理往返高 PSNR 与更低的碰撞浮点面积。


<details>
  <summary>Details</summary>
Motivation: 3DGS 虽能高质量合成新视角，但其自适应辐射基元不能直接作为结构化资产使用：不透明度不是可加的面积度量，细化会改变诱导几何，且水密网格提取会在未观测区域产生幻觉碰撞表面。

Method: 将外观不透明度与几何积分质量分离，把表面类高斯表示为离散无向 varifold，仅导出置信度认证的开放表面补丁，未认证内容保留为残差 splat。提供细化保守的质量传输、局部可实现性诊断、源保持补丁绑定与保守碰撞候选。

Result: 在三个 DTU 场景上，冻结资产基准显示零补丁定义编辑泄漏，纹理往返 PSNR 为 30.1/35.3/33.7 dB，所有场景的碰撞浮点面积均低于官方 2DGS 网格，其中两个场景差距显著。

Conclusion: 该方法提供的是精度-覆盖率的权衡而非通用重建声明。外部区域标注、幻影碰撞探针与 5k 面简化进一步支持认证资产解释，而 RGB-only 实验表明局部可实现性不足以实现稀疏视角表面可辨识性。

Classification reason: 论文提出 Manifold-GS，一种基于 3D Gaussian Splatting 的混合资产层方法，涉及 3DGS 几何重建、网格提取与碰撞检测，属于 3D 生成与重建范畴，映射到 [CANONICAL] three_d_generation。

Importance reason: Focuses on 3D Gaussian splatting asset certification, which is outside the user's core continuous multimodal, autoregressive generation, and world model priorities. Signals: 3DGS, certified hybrid assets, varifold, collision surfaces, DTU benchmark
...
### [61] [CADENA: Stepwise CAD Reverse Engineering](https://arxiv.org/abs/2608.00799)
...
TL;DR: Struct-GStream 提出一种基于结构化 3D 高斯泼溅的高效自由视角视频（FVV）流式方法，通过动态锚点生成结构化 3DGS 构建基础场景，并利用局部刚性假设建模近似运动，同时引入全局自由 3DGS 补丁策略处理缺陷区域和新出现物体，实现低码率下的快速训练与高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有基于神经渲染的自由视角视频构建方法虽能实现高保真图像质量，但大多无法实时渲染且需要完整视频序列训练；而现有在线训练方法虽能实时渲染，却难以满足下游应用对存储和训练时间的要求。作者旨在解决训练速度、存储开销与渲染质量之间的权衡问题。

Method: Struct-GStream 引入动态锚点生成结构化 3D 高斯泼溅（3DGs）来构建基础场景，并基于物体运动的局部刚性假设建模近似场景运动；同时提出全局自由 3DGs 补丁策略，包括自由 3DGs 的生成、剪枝与优化，用于补全和建模缺陷区域及新出现的物体。

Result: 大量实验表明，Struct-GStream 在训练时间、存储和渲染质量方面显著优于现有 FVV 构建的在线训练方法，同时保持有竞争力的渲染速度，能在低码率下实现快速训练。

Conclusion: Struct-GStream 通过结构化 3D 高斯与动态锚点机制，在低码率条件下实现了高效的自由视角视频流式渲染，兼顾了训练效率、存储开销与渲染质量，为动态场景的实时 FVV 应用提供了可行方案。

Classification reason: 论文提出基于结构化 3D 高斯泼溅（3DGS）的自由视角视频流式渲染方法，属于 3D 场景重建与神经渲染范畴，映射到 [CANONICAL] three_d_generation（3D 生成与重建）。

Importance reason: 3D高斯泼溅流式传输与用户关注的多模态生成方向关联较弱，且未涉及连续语言模型、视频生成或世界模型等核心主题。 Signals: 3D高斯泼溅, 自由视角视频, 低码率流式, 实时渲染, 动态场景重建
...
### [63] [CADIR: A Cross-Backend Editable Intermediate Representation for Agentic CAD Generation](https://arxiv.org/abs/2608.00891)
...
TL;DR: CADIR 是一种面向智能体 CAD 生成的跨后端可编辑中间表示，基于 OCCT 几何内核，通过构造图记录建模操作、参数依赖、约束与拓扑选择，并引入几何签名匹配实现跨后端（FreeCAD、SolidWorks、Fusion 360）的可编辑特征历史重建，同时支持构造图检索以提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法将 CAD 建模过程表示为后端特定的顺序脚本（隐含依赖）或静态几何，难以同时保留构造历史、稳定的拓扑引用以及跨不同 CAD 系统的特征级可编辑性，限制了智能体 CAD 生成的可靠性与可编辑性。

Method: 提出 CADIR，一种基于 OCCT 几何内核（通过 OCP）的智能体友好可执行中间表示，提供显式、组合式的建模操作与细粒度执行诊断。程序执行时在构造图中记录建模操作、参数依赖、约束与拓扑选择；引入几何签名匹配（Geometric Signature Matching）在参数变化与后端差异下识别对应边与面，使适配器能在 FreeCAD、SolidWorks、Fusion 360 中重建原生可编辑特征历史；并提出构造图检索方法支持文本与图像查询的全图与子图检索。

Result: 实验表明 CADIR 相比现有 CAD 表示具有更高的几何保真度与执行可靠性；构造图检索进一步提升模型生成质量；跨后端编辑能在多个 CAD 环境中实现可靠的模型重建与重建后编辑。

Conclusion: CADIR 通过显式构造图与几何签名匹配，实现了跨后端可编辑的 CAD 生成与重建，为智能体 CAD 建模提供了更可靠、可编辑的中间表示基础。

Classification reason: 该论文提出 CADIR，一种面向 CAD 程序生成与跨后端编辑的可执行中间表示，属于 CAD/3D 建模程序生成领域，与 [CANONICAL] 的 three_d_generation（3D 生成与重建）主题合理相关，因此复用该子主题。

Importance reason: Solid CAD-specific engineering contribution, but far outside the user's core focus on continuous multimodal models, autoregressive generation, and world models. Signals: CAD generation, intermediate representation, cross-backend editing, construction graph, geometric signature matching
...
### [64] [PlantRig - From Bones to Branches: Adaptation of Autoregressive Rigging Models for Plant Skeletal Reconstruction](https://arxiv.org/abs/2608.01072)
...
TL;DR: 本文评估并微调自回归绑定模型（UniRig、SkinTokens）用于植物骨骼重建。诊断发现 UniRig 将复杂分支折叠为近线性链，根因是采样层对分支 token 的抑制及冻结网格编码器对结构变化的敏感性不足。通过多轮在程序化生成的合成数据集上微调，模型逐步恢复准确的分支拓扑并泛化到带叶片的植物，无需针对叶片的架构改动。


<details>
  <summary>Details</summary>
Motivation: 自回归绑定模型在关节化角色上表现良好，但植物结构具有高度可变、非规范的分支模式，挑战了学习到的骨骼先验。作者希望探索这些模型能否泛化到植物骨骼重建，并找出失败原因及可行的改进路径。

Method: 使用 L-system 生成的合成树木和真实扫描数据（涵盖单轴、合轴、轮生、藤蔓等原型）评估 UniRig 与 SkinTokens。诊断发现 UniRig 的分支 token 在采样层被抑制，且冻结的网格编码器对结构变化敏感性有限。随后在多个程序化生成的合成数据集上进行多轮微调，逐步恢复分支拓扑并泛化到带叶片的植物。

Result: 多轮微调后模型逐步恢复准确的分支拓扑，并泛化到带叶片的植物（叶片为零厚度、依赖网格法线的几何，更具挑战性）。模型在多种植物形态上表现良好，无需针对叶片的架构改动。

Conclusion: 针对性的微调可以大幅缩小角色绑定先验与植物骨骼结构之间的领域差距，为自动化植物绑定（涵盖分支拓扑与叶片类型）提供了可行路径。

Classification reason: 论文研究植物骨骼（骨架）重建与自动绑定（rigging），属于 3D 结构重建与生成范畴，映射到 [CANONICAL] 的 three_d_generation（3D 生成与重建）子主题。

Importance reason: Niche 3D rigging adaptation with limited connection to the user's core continuous multimodal and video generation priorities. Signals: autoregressive rigging, plant skeletal reconstruction, fine-tuning, domain adaptation, 3D generation
...
### [65] [QuerySplat: Decoupling Geometry and Appearance Representations in 3DGS Prediction](https://arxiv.org/abs/2608.01186)
...
TL;DR: QuerySplat 是一种前馈 3DGS 重建框架，通过双分支查询解码器将几何与外观表征解耦：几何分支利用预训练视觉几何模型提供空间理解与无位姿建模能力，外观分支独立恢复高频细节，在 DL3DV 基准上取得 SOTA 新视角合成性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈 3DGS 重建方法存在两类缺陷：像素对齐方法空间灵活性差且结构冗余大；查询式方法缺乏 3D 先验且将几何与外观纠缠，导致渲染模糊、依赖位姿。作者希望克服这些不足，实现高保真渲染。

Method: 提出 QuerySplat 框架，采用双分支查询式解码器。几何分支利用预训练视觉几何模型（Vision Geometric Model）进行空间理解，赋予模型无位姿建模能力；外观分支通过独立通路恢复高频细节，与几何属性回归分离，实现几何与外观表征的解耦。

Result: 在 DL3DV 基准上取得 SOTA 新视角合成性能，平均 PSNR 相比最优无位姿基线和有位姿基线分别提升 2.30 dB 和 1.04 dB，缓解了早期查询式模型的模糊渲染问题，并一致优于像素对齐方法。

Conclusion: QuerySplat 通过解耦几何与外观表征，结合几何先验驱动，有效提升了前馈 3DGS 重建的渲染保真度，验证了查询式方法在引入 3D 先验与显式外观解耦后能够超越像素对齐方法。

Classification reason: 论文提出 QuerySplat，一种前馈 3DGS 重建框架，属于 3D 高斯泼溅重建与神经渲染方向，与 [CANONICAL] three_d_generation（3D 生成与重建）高度匹配。

Importance reason: Solid 3DGS reconstruction work but outside the user's core focus on continuous multimodal models, autoregressive generation, and world models. Signals: 3DGS feed-forward reconstruction, geometry-appearance decoupling, pose-free novel view synthesis, DL3DV SOTA, no autoregressive/diffusion/world-model connection

Abstract: While feed-forward 3D Gaussian Splatting (3DGS) enables efficient 3D reconstruction, achieving high-fidelity rendering remains challenging. Existing pixel-aligned approaches suffer from spatial inflexibility and massive structural redundancy, whereas query-based methods lack 3D priors and entangle geometry with appearance, yielding blurry, pose-dependent results. To overcome these deficiencies, we propose \textbf{QuerySplat}, a feed-forward 3DGS framework driven by geometric priors and explicit appearance decoupling. Specifically, we design a dual-branch query-based decoder: the geometry branch leverages a pretrained Vision Geometric Model for spatial understanding, which intrinsically endows QuerySplat with pose-free modeling capabilities, while the appearance branch recovers high-frequency details through a dedicated pathway separated from geometric attribute regression. Extensive experiments demonstrate that QuerySplat mitigates the blurry rendering issues of early query-based models and consistently outperforms pixel-aligned approaches in rendering fidelity. On the challenging DL3DV benchmark, it achieves state-of-the-art novel view synthesis performance, with average PSNR gains of 2.30 dB and 1.04 dB over the best pose-free and pose-required baselines, respectively. Project Page: .

</details>


### [66] [GaussianSelector: Lightweight Human-Guided Object Selection in 3D Gaussian Splatting with Graph Optimization](https://arxiv.org/abs/2608.01492)
...
TL;DR: GaussianSelector 是一个无需训练的交互式 3D 物体选择框架，直接在 3DGS 原生高斯原语上操作，通过超点粗化、连续性加权图构建和全局图割能量最小化，从稀疏视图和稀疏涂鸦引导中传播证据以完成完整 3D 物体选择，并支持多轮交互式修正。


<details>
  <summary>Details</summary>
Motivation: 从重建场景中选取完整 3D 物体对场景编辑和具身交互至关重要。现有 3DGS 方法要么重训练高斯表征嵌入逐物体标签，要么构建密集多视角 SAM 观测，两者都需要大量计算和密集视角覆盖，实际场景中难以满足。作者希望实现从稀疏视图和稀疏涂鸦引导下的轻量级交互式 3D 物体选择。

Method: GaussianSelector 直接在原生高斯原语上操作：首先将密集高斯粗化为几何一致的超点，利用外观和空间线索构建连续性加权图；随后通过可见性感知的透射率覆盖将稀疏用户涂鸦提升到 3D；最后将选择问题建模为全局图割能量最小化，将稀疏证据传播到完整 3D 物体。该设计天然支持多轮修正，用户可从额外视角迭代纠正选择结果。

Result: 实验表明，GaussianSelector 在达到与最先进的多视角 SAM 方法相当的选取质量的同时，所需交互视角显著更少，计算开销大幅降低，适合人机协同的 3D 场景编辑和 3D 资产提取等实际部署场景。

Conclusion: GaussianSelector 证明了无需重训练即可在 3DGS 上实现高效、轻量的交互式物体选择，通过图优化将稀疏用户引导传播为完整物体分割，兼顾了质量与效率，为实际场景中的 3D 编辑与资产提取提供了实用方案。

Classification reason: 论文聚焦于 3D Gaussian Splatting 场景中的交互式物体选择与分割，属于 3D 场景编辑与 3DGS 相关技术，映射到 [CANONICAL] 的 three_d_generation（3D 生成与重建）子主题。

Importance reason: Solid 3DGS interaction paper but outside the user's core continuous-model, autoregressive generation, and world-model priorities. Signals: 3D Gaussian Splatting, interactive object selection, training-free, graph-cut optimization, sparse-view interaction
...
### [67] [StreamSplat: Streaming Feed-Forward 3D Gaussian Splatting](https://arxiv.org/abs/2608.01659)
...
TL;DR: StreamSplat 是一个流式前馈 3D 高斯泼溅框架，通过体素对齐因果缓存（VACC）增量维护几何锚定的场景状态，并在每个输入块后解码为可渲染的 3D 高斯，支持在线顺序到达的校准视图场景更新。


<details>
  <summary>Details</summary>
Motivation: 现有前馈 3DGS 方法假设固定的一组上下文视图并联合处理，无法适用于校准视图顺序到达、场景需因果更新的在线场景，且固定视图基线在长输入流下会内存溢出。

Method: StreamSplat 提出三个核心组件：Voxel-Aligned Causal Cache (VACC) 以内存有界的体素结构存储历史 3D token，使内存随探索场景几何而非流长度增长；History-Projected Depth Anchoring (HPDA) 将缓存几何投影为当前代价体估计的深度引导；Cache-Guided Feature Injection (CGFI) 将缓存潜在证据注入高斯 token 回归。

Result: 在 DL3DV、RealEstate10K 和 ScanNet 上，StreamSplat 在稀疏因果输入下与最先进的前馈 3DGS 方法保持竞争力；在 256、512、1024 视图的长输入流上，固定视图基线内存溢出，而 StreamSplat 随更多观测到达持续提升新视角合成质量。

Conclusion: StreamSplat 证明了流式前馈 3DGS 在在线场景中的可行性，通过内存有界的因果缓存实现长输入流下的持续质量提升，为在线新视角合成提供了有效方案。

Classification reason: 论文提出流式前馈 3D 高斯泼溅（StreamSplat），用于增量式新视角合成，属于 3DGS 场景重建与神经渲染范畴，映射到 [CANONICAL] three_d_generation（3D 生成与重建）。

Importance reason: Solid incremental contribution to streaming 3D reconstruction, but tangential to the user's core focus on continuous multimodal models and autoregressive generation. Signals: streaming 3DGS, causal cache, novel-view synthesis, long-sequence scalability, no future-view dependency
...
### [68] [PartMat: Material-Aware 3D Part Decomposition with a Single Global Latent](https://arxiv.org/abs/2608.01825)
...
Importance: 值得关注 (67.85) - 未入选精读

TL;DR: PartMat 提出一种材质感知的 3D 部件分解管线，用单一全局潜变量表示多部件几何，通过 PartVAE 统一表征、扩散模型生成部件、强化学习优化材质分配与重叠抑制，并用稀疏体素 flow-matching 模型恢复几何细节，实现高效且材质边界准确的 3D 部件分解与生成。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 部件级生成方法通常按功能语义而非可编辑的材质边界（如织物、木材、金属）分解物体，且各部件独立生成导致计算成本随部件数量线性增长，难以满足室内设计等实际 3D 应用需求。

Method: PartMat 包含三个核心组件：(1) PartVAE 学习统一表征，在单次前向传播中解码所有材质部件，将推理成本与部件数量解耦；(2) 基于该表征训练扩散模型进行部件生成，并通过强化学习优化材质分配与重叠抑制；(3) 引入带部件注意力的稀疏体素 flow-matching 模型进行几何后处理，恢复细粒度几何细节。

Result: 大量实验表明 PartMat 在材质感知分解准确率上显著优于现有基线，几何质量相当，同时保持高效推理。

Conclusion: PartMat 通过单一全局潜变量统一表示多部件几何，实现了材质感知的 3D 部件分解与生成，在准确性和效率上均优于现有方法，为可编辑 3D 资产生成提供了有效方案。

Classification reason: 该论文提出 PartMat，一种材质感知的 3D 部件分解与生成方法，涉及 3D 部件级生成、扩散模型与 flow-matching 几何后处理，属于 3D 生成与重建范畴，映射到 [CANONICAL] three_d_generation。

Importance reason: Focuses on 3D part decomposition with material awareness, which is peripheral to the user's core interests in continuous multimodal models and video/image generation. Signals: 3D part decomposition, material-aware, single global latent, PartVAE, diffusion model
...
### [69] [Beyond Global Latents: Chunk-Based Sparse Grid VAE for Scalable 3D Modeling](https://arxiv.org/abs/2608.02016)
...
Importance: 普通相关 (62.0) - 未入选精读

TL;DR: ChunkVAE 提出一种基于局部 chunk 而非全局潜空间的稀疏网格 VAE，通过平衡二叉对象划分和 S 曲线加权拼接两个数据算子，实现从 512³ 到 1536³ 分辨率下可扩展的 3D 建模，降低峰值内存并加速并行推理。


<details>
  <summary>Details</summary>
Motivation: 稀疏体素网格能保留 3D 重建所需的精细空间结构，但其内存随分辨率增长而快速膨胀（活跃表面单元增加）。现有方法依赖全局潜空间，难以在保持全局接口的同时扩展到高分辨率。

Method: ChunkVAE 围绕局部 chunk 组织稀疏网格 VAE，而非全局潜体积。局部学习算子允许编码器与解码器分区独立选择，推理 chunk 大小可与训练不同。两个互补数据算子：Balanced Binary Object Partitioning 在分配活跃单元时限制复制重叠；S-Curve weighted stitching 在组装全局潜变量或重建时抑制不可靠的边界特征。

Result: 在三个物体基准上，ChunkVAE 在 512³ 到 1536³ 分辨率下与强基线相当或更优；更小的 chunk 降低峰值分配内存并缩短每 chunk 计算时间，实现更快的并行推理。稳定的拼接潜变量和改进的 image-to-3D 指标表明局部压缩可扩展几何同时保留下游所需的全局接口。

Conclusion: 局部 chunk 化的稀疏网格 VAE 能够在不牺牲全局接口的前提下实现可扩展的 3D 几何建模，为高分辨率 3D 生成与重建提供了更高效的内存与计算方案。

Classification reason: 论文提出 ChunkVAE，一种基于稀疏网格的 3D 变分自编码器，用于可扩展的 3D 几何建模与 image-to-3D 生成，属于 3D 生成与重建方向，映射到 [CANONICAL] three_d_generation。

Importance reason: Solid 3D VAE scaling work, but it is outside the user's core continuous/multimodal generation and world-model focus. Signals: 3D generation, sparse grid VAE, scalable 3D modeling, chunk-based latent, efficient inference
...
### [70] [UniqueSplat: View-conditioned 3D Gaussian Splatting for Generalizable 3D Reconstruction](https://arxiv.org/abs/2608.02145)
...
TL;DR: UniqueSplat 是一种视图条件化的前馈式 3D Gaussian Splatting 模型，通过双分支视图条件超网络同时学习视图无关与视图特定知识，使高斯在测试时能针对不同视图动态调整，实现可泛化的 3D 重建。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式方法（如 pixelSplat、MVSplat）为每个场景生成固定的高斯，渲染所有视图时缺乏对特定目标视角的适应能力，因为它们预测高斯时未纳入目标视图信息。

Method: 提出双分支视图条件超网络（view-conditioned hyperNetwork），同时学习视图无关的共享嵌入和视图特定的知识，将视图条件信息作为先验融入网络参数，使高斯在测试时能根据具体视图动态调整。

Result: 在 RealEstate10K、ACID 和 DTU 数据集上优于现有 SOTA 方法，并在跨数据集评测中表现突出，展现出显著的泛化能力。

Conclusion: 通过将目标视图信息作为条件先验融入网络参数，UniqueSplat 能生成针对特定视角动态调整的高斯，显著提升前馈式 3D 重建的适应性与泛化能力。

Classification reason: 该论文提出 UniqueSplat，一种基于 3D Gaussian Splatting 的前馈式 3D 重建方法，属于 3D 生成与重建方向，映射到 [CANONICAL] three_d_generation 子主题。

Importance reason: 3D Gaussian Splatting reconstruction is outside the user's core focus on continuous multimodal models and autoregressive generation. Signals: 3D reconstruction, Gaussian splatting, view-conditioned, feed-forward, generalization
...
### [71] [DerainSplat: Feed-Forward Clean 3D Gaussian Splatting from Sparse Rainy Views](https://arxiv.org/abs/2608.02191)
...
Importance: 普通相关 (50.28) - 未入选精读

TL;DR: DerainSplat 是一个前馈式 3DGS 框架，从少量雨天视角重建干净 3D 场景。通过四阶段合成管线构建大规模多视角去雨数据集，引入 weather net 预测天气因子并生成场景/辐射支持图，分别调制跨视角代价体匹配与深度对齐外观融合，并利用几何证据衰减高斯不透明度以减少伪结构，配合雨天循环一致性约束。


<details>
  <summary>Details</summary>
Motivation: 现有图像去雨方法主要聚焦 2D 图像恢复，而具身智能与自动驾驶等空间智能应用需要从稀疏雨天视角前馈重建干净 3D 场景。现有前馈 3DGS 方法假设干净输入，在雨天条件下会崩溃。

Method: 构建四阶段合成管线（阴天光照、深度相关雾、雨条纹、镜头雨滴）生成大规模多视角去雨数据集并产生特权天气因子。引入 weather net 从雨天上下文预测天气因子并生成两种支持图：场景支持调制跨视角代价体匹配，辐射支持驱动深度对齐外观融合以填充损坏像素。几何证据进一步衰减高斯不透明度减少伪结构，并通过雨天循环一致性将预测因子重渲染的干净视图与雨天输入对齐。

Result: 在 RealEstate10K、ACID、Mip-NeRF360 及真实雨天场景等多个数据集上优于现有方法，并展现出强跨数据集泛化能力。

Conclusion: DerainSplat 验证了从稀疏雨天视角前馈重建干净 3D 场景的可行性，通过天气因子预测与支持图调制有效提升雨天条件下的 3D 重建质量，为空间智能应用提供支撑。

Classification reason: 论文提出 DerainSplat，一种从稀疏雨天视角前馈重建干净 3D 场景的 3D Gaussian Splatting 方法，属于 3D 场景重建与 3DGS 范畴，映射到 [CANONICAL] three_d_generation（3D 生成与重建）。

Importance reason: 3D reconstruction from rainy views is a niche application with limited overlap with the user's core focus on continuous multimodal models and autoregressive generation. Signals: 3D Gaussian Splatting, feed-forward reconstruction, rainy view deraining, weather factor prediction, multi-view dataset synthesis
...
### [72] [CLEAR: Conflict-aware Learning via Evidence-guided Adaptive Routing for Unified Sparse-View 3D Gaussian Super-Resolution](https://arxiv.org/abs/2608.02206)
...
TL;DR: CLEAR 是首个统一的单阶段稀疏视角 3D 高斯泼溅超分辨率框架，通过联合优化真实低分辨率观测与外部高分辨率先验，并引入高斯级冲突感知优化与证据引导的 Patch-to-Gaussian 路由机制，在合成与真实 4× 超分辨率基准上取得 SOTA 渲染质量与几何保真度。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏视角 3DGS 超分辨率方法采用两阶段流程（先低分辨率高斯重建再高分辨率高斯细化），导致阶段间高斯传递与重建误差累积，难以获得高质量重建。

Method: 提出 CLEAR 统一单阶段框架：在统一高斯表征中联合优化真实 LR 观测与外部 HR 先验；设计高斯级冲突感知优化策略，以 LR 梯度为可靠锚点，仅对严重 HR 冲突施加证据条件软校正；引入证据引导的 Patch-to-Gaussian 路由机制，估计 patch 可靠性与细节需求并提升到高斯空间，选择性路由高频梯度与致密化；采用共享高斯 dropout 与分离的中间训练锚定增强训练鲁棒性。

Result: 在合成与真实世界 4× 超分辨率基准上的大量实验表明，CLEAR 一致地取得 SOTA 渲染质量与更优的几何保真度。

Conclusion: CLEAR 通过统一单阶段框架与冲突感知、证据引导路由等机制，有效缓解了两阶段误差累积问题，为稀疏视角 3DGS 超分辨率提供了更优的解决方案。

Classification reason: 论文提出稀疏视角 3D 高斯泼溅超分辨率统一框架，属于 3DGS 重建与神经渲染范畴，映射到 [CANONICAL] three_d_generation（3D 生成与重建）。

Importance reason: Solid 3D Gaussian super-resolution work, but outside the user's core continuous/multimodal/video generation focus. Signals: 3D Gaussian Splatting, super-resolution, sparse-view, unified single-stage, conflict-aware routing
...
### [73] [Learning to Tessellate: Point Cloud Generation via Recursive Spectral Partitioning](https://arxiv.org/abs/2608.02432)
...
Importance: 普通相关 (56.15) - 未入选精读

TL;DR: PointRSP 将点云生成重构为拓扑保持的镶嵌过程，通过递归谱划分将无序点云分解为非平衡二叉树，并采用双流级联生成器在量化潜空间中合成形状，实现 SOTA 的生成质量与多样性。


<details>
  <summary>Details</summary>
Motivation: 现有自回归点云生成方法依赖启发式 tokenization 策略（如空间排序或随机下采样），这些策略常破坏点云的内在拓扑结构，削弱生成形状的结构连贯性。

Method: 提出拓扑感知划分自编码器，通过混合递归谱划分策略将无序点云分解为非平衡二叉树，形成保持拓扑关系的确定性几何蓝图；设计双流级联生成器联合建模结构演化与特征合成；引入几何校准位置编码机制，用多尺度结构中心锚定潜嵌入以稳定级联生成。

Result: 在生成质量和多样性上达到 SOTA 性能，并在复杂 3D 拓扑上展现出强泛化能力。

Conclusion: 通过拓扑保持的递归谱划分与级联生成，PointRSP 有效解决了启发式 tokenization 破坏点云拓扑的问题，为自回归点云生成提供了更结构化的新范式。

Classification reason: 该论文提出 PointRSP，一种基于递归谱划分的自回归点云生成框架，属于 3D 点云生成任务，映射到 [CANONICAL] 的 three_d_generation（3D 生成与重建）子主题。

Importance reason: Solid 3D generation work but falls outside the user's configured priority directions and subtopics. Signals: autoregressive point cloud generation, recursive spectral partitioning, topology-preserving tokenization, dual-stream cascaded generator, quantized latent space
...
### [74] [D^2-4DGS: Dual-Depth Guided Sparse-Camera 4D Gaussian Splatting](https://arxiv.org/abs/2608.01588)
*Jijian Zhao*
...
TL;DR: D²-4DGS 提出一种由双源深度先验引导的稀疏相机动态 4D 高斯泼溅框架，通过融合单目深度与多视角几何深度，实现稀疏视角下的高质量动态新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有动态 4D 高斯泼溅方法通常需要密集多视角视频提供足够的几何约束，采集成本高且限制了稀疏相机部署。减少输入视角会削弱几何监督，导致结构缺失和漂浮高斯。深度先验虽能提供几何线索，但单一来源无法同时具备密集覆盖与可靠几何：单目深度结构密集但尺度模糊且局部有偏，多视角几何深度与重建坐标系一致但覆盖不完整。

Method: 提出 D²-4DGS 框架，利用双源深度先验互补性：将单目深度估计与有效的多视角几何深度对齐并验证一致性以识别可靠几何锚点；这些锚点支持一致性感知剪枝与深度监督，而验证后的几何深度与对齐的单目估计为欠重建区域提供候选几何用于稠密化；最后通过 RGB-D 联合优化在稀疏视角监督下提升外观保真度与几何一致性。

Result: 在全部九种数据集-视角设置下，D²-4DGS 均取得最高 PSNR，相比各设置下最优竞争方法平均提升 1.33 dB。

Conclusion: 双源深度先验的互补融合能有效缓解稀疏相机动态 4D 重建中的几何监督不足问题，显著提升动态新视角合成的质量与鲁棒性。

Classification reason: 论文提出 D²-4DGS，一种稀疏相机动态 4D 高斯泼溅框架，属于动态 4D 场景重建与生成，与 [CANONICAL] 子主题 dynamic_4d_generation（4D 与动态场景生成）高度匹配。

Importance reason: Solid incremental work on sparse-camera 4D Gaussian splatting, but outside the user's core continuous/multimodal generation focus and lacks breakthrough novelty. Signals: 4D Gaussian splatting, sparse-camera dynamic view synthesis, dual-depth priors, PSNR improvement, cs.CV
...
### [75] [ASTRA: Asynchronous Spatio-Temporal Reconstruction via Trajectory Alignment](https://arxiv.org/abs/2608.02006)
...
TL;DR: ASTRA 提出一种异步时空重建框架，通过引入 2D 运动轨迹作为显式、纹理无关的监督信号，联合优化时间偏移与动态 3D 表征，解决多相机时间异步导致的运动模糊与几何伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有动态 3D 场景重建假设多相机严格同步，但真实场景中设备间时间异步会导致运动模糊和几何伪影。已有异步重建方法依赖光度监督估计时间偏移，在大偏移和复杂运动下时间线索微弱，存在纹理诱导坍缩和形变诱导耦合两大瓶颈。

Method: ASTRA 将 2D 运动轨迹作为显式、纹理无关的监督信号，通过将重建 3D 点的投影运动与观测到的 2D 轨迹对齐，联合优化时间偏移和动态 3D 表征，并使用动态与确定性掩码抑制不可靠的轨迹约束。

Result: 在多种动态高斯泼溅骨干上，ASTRA 在高达 25 帧偏移的严重异步下仍保持高频空间细节和强鲁棒性，PSNR 提升约 1.4 dB，时间偏移 MAE 降低 54.0%，同步成功率提升近 4 倍。

Conclusion: ASTRA 通过轨迹对齐替代纯光度监督，有效解决了异步动态重建中的纹理坍缩与形变耦合问题，为多相机异步动态场景重建提供了更鲁棒的解决方案。

Classification reason: 论文研究动态 3D 场景重建，针对多相机时间异步问题，基于动态高斯泼溅（Dynamic Gaussian Splatting）骨干进行时空重建，属于 4D 与动态场景生成与重建范畴，映射到 [CANONICAL] dynamic_4d_generation。

Importance reason: Solid 4D reconstruction paper but outside the user's core continuous/multimodal generation focus, with no direct alignment to priority subtopics. Signals: 4D reconstruction, dynamic Gaussian Splatting, trajectory alignment, temporal asynchrony, cs.CV
...
### [76] [CopyCat: Improving Fine-Grained Subject Consistency in Subject-to-Image Models within Seconds](https://arxiv.org/abs/2608.00674)
...
TL;DR: CopyCat 提出一种轻量级模型精炼框架，通过附加 Fine-grained Consistency LoRA（FCLoRA）并用单一代理图像进行自重建优化，在数秒内提升预训练 subject-to-image 模型的细粒度主体一致性，无需针对新主体的额外优化。


<details>
  <summary>Details</summary>
Motivation: 现有 subject-to-image 模型难以保持细粒度主体细节，主要原因是缺乏高质量细粒度身份监督：真实配对数据采集昂贵，合成训练对只能保留粗略外观而丢失细微主体特征。

Method: CopyCat 对预训练 subject-to-image 模型进行一次性的轻量精炼：附加 FCLoRA 模块，使用单一代理图像同时作为条件图像和重建目标进行自重建优化。该精确自重建目标简化了优化任务，使细粒度精炼在数秒内完成。精炼仅执行一次，所得模型可直接应用于未见过的参考主体和提示。此外，论文重新审视双流扩散 Transformer 中的 LoRA 训练，发现仅适配视觉流能持续提升主体一致性。

Result: 在 DreamBench 和 XVerseBench 上，CopyCat 在单主体和多主体设置下均对多种代表性 subject-to-image 模型实现了细粒度主体一致性的持续改进。

Conclusion: CopyCat 证明通过精确自重建目标进行一次性轻量精炼，可以在数秒内有效提升细粒度主体一致性，且无需针对每个新主体重新优化，为个性化图像生成提供了一种高效且通用的精炼方案。

Classification reason: 论文聚焦 subject-to-image 个性化生成中的细粒度主体一致性保持，属于主体驱动/概念定制生成方向，映射到 CANONICAL 子主题 customization_generation（主体与概念驱动生成）。

Importance reason: Solid but incremental subject-consistency refinement for image generation, not aligned with the user's top continuous/world-model priorities. Signals: subject-to-image, fine-grained consistency, LoRA, diffusion transformer, single-image self-reconstruction
...
### [77] [Unleashing the Power of Text: Text-Guided Flow Matching for Image Fusion under Complex Degradations](https://arxiv.org/abs/2608.00530)
...
TL;DR: TGFusion 是一个文本引导的潜空间流匹配框架，统一退化抑制与跨模态红外-可见光图像融合。它将任务、退化与生成线索编码为结构化提示，并通过 Prompt 条件多流联合流 Transformer 实现文本语义流与融合/可见光/红外流的 token 级双向交互，从而动态引导可靠信息选择与融合潜变量生成。


<details>
  <summary>Details</summary>
Motivation: 红外-可见光图像融合在真实退化场景下极具挑战，退化不仅导致观测图像中可靠模态信息丢失，还阻碍融合过程。现有方法通常将固定的全局文本表征注入视觉特征，难以适应空间变化的退化、局部结构与热显著性。因此需要一种能动态利用文本先验、适应复杂退化的融合方法。

Method: 提出 TGFusion 框架：1) 将任务、退化与生成线索编码为结构化提示；2) 设计 Prompt 条件多流联合流 Transformer（Prompt-conditioned Multi-stream Joint Flow Transformer），将文本作为独立语义流与融合、可见光、红外流并列；3) 通过联合注意力实现语义与视觉表征的 token 级双向交互与逐层更新，使退化语义动态引导可靠信息选择与融合潜变量生成；4) 采用潜空间流匹配（latent-space flow matching）统一退化抑制与跨模态融合。

Result: 在公开基准与复杂退化场景上的大量实验表明，TGFusion 在感知质量、图像自然度、结构细节保持与红外显著性保留方面达到优越或具有竞争力的性能，并在多种单一与复合退化下保持鲁棒性。

Conclusion: 文本引导的潜空间流匹配能够有效统一退化抑制与跨模态融合，通过将文本作为独立语义流并实现 token 级双向交互，可动态适应空间变化的退化并提升融合质量，为复杂退化场景下的图像融合提供了新思路。

Classification reason: 该论文提出文本引导的潜空间流匹配框架 TGFusion，用于红外-可见光图像融合与退化抑制，属于图像融合/增强类生成任务，映射到 [CANONICAL] 的 visual_editing（图像与视频编辑）子主题。

Importance reason: Focuses on infrared-visible image fusion with text-guided flow matching, which is peripheral to the user's core interests in continuous multimodal models and autoregressive generation. Signals: flow matching, text-guided generation, image fusion, multimodal generation, degradation suppression
...
### [78] [InstancePin: Instance-Addressable Layout-to-Image Diffusion via Coordinate Pinning](https://arxiv.org/abs/2608.00588)
...
TL;DR: InstancePin 提出一种实例可寻址的布局到图像扩散框架，通过为每个实例引入显式坐标锚点（coordinate token）和坐标固定注意力，解决密集布局中同类物体实例纠缠问题，在 Cityscapes 上提升图像保真度与语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有布局到图像扩散模型依赖类别级分割图进行条件生成，但类别级控制无法区分同一类别下的多个邻近实例，导致边界模糊、外观平均化与特征混淆，尤其在拥挤的城市场景中尤为突出。

Method: InstancePin 引入独立的实例感知适配器，不直接注入实例掩码到预训练主干。每个实例的中心坐标经傅里叶特征编码投影为坐标 token，作为空间锚点，通过坐标固定注意力（coordinate pinning attention）被潜在图像特征查询；用实例区域监督坐标注意力图，使每个 token 激活对应物体区域；最后用实例掩码引导的融合模块将主干特征路由到非实例区域、适配器特征路由到实例区域，实现局部实例细化而不牺牲全局语义保真。

Result: 在 Cityscapes 数据集上的大量实验表明，InstancePin 缓解了密集布局中的实例纠缠问题，同时提升了图像保真度与语义一致性。

Conclusion: 通过显式坐标锚定与实例感知适配器，InstancePin 在不破坏类别级生成先验的前提下实现了实例级空间控制，为密集场景的布局到图像生成提供了有效方案。

Classification reason: 该论文提出 InstancePin，一种基于坐标锚定的布局到图像扩散生成框架，属于可控图像生成/编辑范畴，映射到 [CANONICAL] 的 visual_editing（图像与视频编辑）子主题。

Importance reason: Solid but incremental layout-to-image diffusion work; only tangential to your core continuous/multimodal generation priorities. Signals: layout-to-image diffusion, instance-addressable control, coordinate pinning attention, Cityscapes, diffusion adapter
...
TL;DR: 本文提出 AeroNight-1.5K 夜间航拍图像增强基准数据集，并设计 AeroLLE 两阶段框架：先用 HVI Base Enhancer 恢复可见性，再用 SAECC 进行空间自适应曝光-色彩校准，通过有界低分辨率增益/偏置场约束第二阶段校正幅度，在无配准参考条件下利用生成式外观引导进行学习。


<details>
  <summary>Details</summary>
Motivation: 夜间航拍图像增强面临空间非均匀曝光、混合光照和弱结构证据等挑战，而移动平台上难以采集配准的正常光照参考图。生成的正常光照图像虽能提供外观引导，但可能改变几何或纹理，因此需要一种在无配准参考条件下稳健学习增强的策略。

Method: 提出 AeroLLE 两阶段框架：第一阶段用 HVI Base Enhancer 恢复可见性；第二阶段进行 Spatially Adaptive Exposure-Color Calibration (SAECC)，预测有界、低分辨率的 RGB 增益和偏置场，限制第二阶段校正的幅度与空间变化。数据集 AeroNight-1.5K 包含 1500 张真实夜间航拍 RGB 图像，其中 1300 张配有手工筛选的伪参考，200 张用于无配对评测。

Result: 在互补的伪配对与无配对评测协议下，AeroLLE 与筛选后的外观目标一致性更好，并在多样夜间航拍场景中实现更均衡的曝光与色彩校正。

Conclusion: 有约束的、分阶段校准是在无配准航拍参考时从生成式外观引导中学习的实用策略，验证了该方法的有效性。

Classification reason: 该论文研究夜间航拍图像增强（图像增强/校正），属于图像处理与编辑范畴，映射到 [CANONICAL] 的 visual_editing（图像与视频编辑）子主题。

Importance reason: Niche low-level vision task with no overlap with the user's priority topics on continuous/multimodal models, video generation, or world models. Signals: nighttime aerial enhancement, low-level vision, benchmark dataset, no autoregressive/diffusion/world-model focus, no continuous token/latent flow relevance
...
### [80] [ChordVideo: One-Step, Training-Free, Temporally Consistent Video Editing via Low-Energy Transport](https://arxiv.org/abs/2608.00769)
*Zhiqiang Lao*
...
Importance: 值得关注 (71.7) - 未入选精读

TL;DR: ChordVideo 将 ChordEdit 的低能量平滑思想从图像编辑扩展到视频时间维度，通过共享噪声、运动对齐的因果聚合和可选的时间平滑近端校正，实现单步、免训练、时间一致的视频编辑，在保持每帧仅 2 NFE 的同时显著降低 warping 误差和闪烁。


<details>
  <summary>Details</summary>
Motivation: 单步文生图模型支持免训练、免反演的编辑，但将其逐帧独立应用于视频会产生时间闪烁和编辑强度漂移。ChordEdit 通过沿采样时间的低能量平滑稳定了图像编辑，但直接逐帧应用无法解决视频的时间一致性问题。

Method: ChordVideo 将低能量原则扩展到视频时间维度，采用三种机制：共享噪声（shared noise）保证帧间一致性；运动对齐的因果聚合（motion-aligned causal aggregation）对每帧的 Chord 场进行聚合；可选的时间平滑近端校正（temporally smoothed proximal correction）。论文还推导了 warping 误差界，将运动偏差与随机闪烁分离，并预测更大时间窗口的收益递减。

Result: 在 TGVE/DAVIS 上使用两个单步骨干模型，ChordVideo 将 warping 误差降低 78%、闪烁降低 49%，CLIP 帧一致性提升 9-10 分，背景 PSNR 提升约 1.5dB，同时保持每帧 2 NFE。与七个多步编辑方法相比，以每片段少 10-60 倍的模型步数达到有竞争力的时间一致性和源保真度。

Conclusion: ChordVideo 证明低能量传输原则可以有效地从图像编辑扩展到视频编辑，在极低的计算成本下实现时间一致的视频编辑，为免训练、单步视频编辑提供了新的高效范式。

Classification reason: 该论文提出 ChordVideo，一种基于低能量传输（low-energy transport）的单步、免训练、时间一致的视频编辑方法，属于视频编辑方向，映射到 [CANONICAL] 的 visual_editing（图像与视频编辑）子主题。

Importance reason: Solid but incremental video-editing work; relevant to multimodal generation but not aligned with the user's priority subtopics (continuous models, autoregressive/diffusion video generation, world models). Signals: video editing, one-step, training-free, temporal consistency, low-energy transport
...
### [81] [SG-Layout: Structured Scene Graph-Guided Layout Generation with LLMs](https://arxiv.org/abs/2608.01106)
...
TL;DR: SG-Layout 是一个场景图引导的布局生成框架，通过两阶段训练（图-语言特征对齐 + LoRA 指令微调）将结构化场景图空间知识注入 LLM，用于图像布局生成、室内场景合成和机器人物体重排任务，提升空间推理准确性与几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 在从自然语言生成空间连贯布局时，难以捕捉物体间显式的几何关系和结构依赖，导致布局生成的空间推理能力不足。

Method: 采用两阶段训练范式：(1) 图-语言特征对齐阶段，训练关系图编码器和投影器将场景图嵌入映射到 LLM 的语言空间；(2) 指令微调阶段，使用 LoRA 适配器在保持骨干冻结的情况下进行高效的指令驱动布局生成微调。

Result: 在图像布局生成、室内场景合成和机器人物体重排任务上，SG-Layout 相比紧凑开源骨干模型提升了空间推理准确性和几何一致性，在关系密集和组合复杂的场景中优势尤为明显。

Conclusion: 图结构特征对齐能有效增强可控布局生成，验证了将结构化空间知识显式注入 LLM 的有效性。

Classification reason: 该论文提出 SG-Layout，利用场景图引导 LLM 进行可控布局生成（图像布局、室内场景合成、机器人物体重排），属于可控生成/布局生成范畴。虽然布局生成不完全等同于图像编辑，但最接近的 canonical 子主题是 visual_editing（可控编辑），且没有更贴切的 canonical 子主题，故复用该子主题。

Importance reason: Focuses on layout generation via scene graphs, which is only tangentially related to your core interests in continuous multimodal models and autoregressive generation. Signals: layout generation, scene graph, LLM, LoRA, controllable generation
...
### [82] [CoT-Edit: Let CoT Guide Instruction Video Editing](https://arxiv.org/abs/2608.01113)
...
Importance: 值得关注 (75.6) - 未入选精读

TL;DR: CoT-Edit 提出一个"规划-引导-编辑"框架，利用 CoT 增强的 MLLM 作为规划器，对视频和指令进行结构化推理，生成精确的边界框和属性增强的编辑指令，再通过框条件掩码生成器和扩散编辑器实现高保真、时序一致且空间对齐的指令视频编辑。


<details>
  <summary>Details</summary>
Motivation: 纯文本指令在复杂场景的视频编辑中难以捕捉精确的空间关系和物理约束，导致目标模糊和物理上不合理的编辑结果。现有方法缺乏显式的空间推理与执行之间的桥梁。

Method: 提出 plan-guide-edit 三阶段框架：1) CoT 增强的 MLLM 作为规划器，对视频和指令进行结构化推理，生成精确的边界框序列和属性增强的编辑指令；2) 框条件掩码生成器将模糊的全局检索转化为局部上下文感知的细化，生成更准确的掩码；3) 扩散编辑器整合掩码、增强指令和帧特征，渲染时序一致且空间对齐的高保真编辑。训练采用模块化预训练加联合微调。

Result: 在包含多个相似物体的复杂场景中实现精确定位和物理一致的物体添加，在多个强基线方法上取得最先进性能，同时降低了数据需求。

Conclusion: 通过显式桥接语义意图与空间执行，CoT 引导的空间先验能显著提升指令视频编辑的定位精度与物理合理性，且模块化加联合训练策略在降低数据需求的同时保持优越性能。

Classification reason: 论文提出基于指令的视频编辑框架（CoT-Edit），属于视频编辑方向，映射到 [CANONICAL] visual_editing（图像与视频编辑）。同时涉及 MLLM 作为规划器进行推理，也匹配多模态理解与生成方向。

Importance reason: Solid instruction video editing work with CoT-guided spatial planning, but it is not aligned with the user's top-priority continuous/multimodal latent or world-model directions. Signals: instruction video editing, chain-of-thought MLLM planner, bounding-box spatial priors, diffusion-based editor, temporal coherence
...
### [83] [TurboClear: One-Step Object-Effect Removal via Region-Calibrated Distribution Matching and Fusion](https://arxiv.org/abs/2608.01288)
...
TL;DR: TurboClear 是一个基于 SDXL 的一步式目标物体及其关联效果移除模型，通过区域校准分布匹配（RDM）进行区域感知蒸馏以保留教师模型的不对称编辑-保留行为，并采用可学习空间融合（LSF）实现轻量推理融合，相比 ObjectClear 最高提速 40 倍、相比 OmniPaint 最高提速 665 倍，同时保持相当或更优的移除质量。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的移除方法（如 ObjectClear、OmniPaint）虽然能同时移除目标物体及其关联效果，但依赖多步去噪导致推理成本高。直接套用现有一步蒸馏方法效果不佳，因为其全局目标缺乏显式的区域校准，会削弱物体效果移除所需的非对称"编辑-保留"行为。

Method: 提出 TurboClear，一个基于 SDXL 的一步式物体效果移除模型。训练阶段设计区域校准分布匹配（Region-Calibrated Distribution Matching, RDM）进行区域感知蒸馏，保留教师模型的不对称编辑-保留行为；推理阶段提出可学习空间融合（Learnable Spatial Fusion, LSF）实现轻量化的推理时融合。

Result: TurboClear 显著提升推理效率并保持有竞争力的视觉质量：相比 ObjectClear 计算开销最高降低 40.04 倍，相比基于 Flux 的 OmniPaint 最高降低 665 倍，同时保持相当或更优的视觉移除质量。

Conclusion: 通过区域校准的分布匹配蒸馏与可学习空间融合，TurboClear 成功实现了一步式的高效物体效果移除，在极大降低推理成本的同时维持了良好的移除质量，验证了区域感知蒸馏对保留非对称编辑-保留行为的重要性。

Classification reason: 该论文提出 TurboClear，用于一步式目标物体及其关联效果移除（object-effect removal），属于图像编辑/移除类任务，映射到 [CANONICAL] visual_editing（图像与视频编辑）子主题。

Importance reason: Efficient one-step image editing distillation, but outside the user's core continuous/multimodal generation and world-model focus. Signals: one-step distillation, SDXL, object removal, region-calibrated distribution matching, inference acceleration

Abstract: Recently, diffusion-based removal methods have achieved promising visual quality in removing both target objects and their associated effects. However, they typically rely on multi-step denoising, leading to high inference cost. Directly applying existing one-step distillation methods is also suboptimal, since their global objectives lack explicit region-wise calibration and may weaken the asymmetric edit-and-preserve behavior required by object-effect removal. To address these challenges, we propose TurboClear, a one-step SDXL-based object-effect removal model. During training, we design Region-Calibrated Distribution Matching (RDM) for region-aware distillation to preserve the teacher model's asymmetric edit-and-preserve behavior. Furthermore, we propose Learnable Spatial Fusion (LSF) for lightweight inference-time fusion. Extensive experiments show that TurboClear significantly improves inference efficiency while maintaining competitive visual quality. TurboClear reduces the computational overhead by up to $40.04\times$ compared to ObjectClear, and by up to $665\times$ against the Flux-based method OmniPaint, all while maintaining comparable or better visual removal quality. Code is available at .

</details>


### [84] [LiveLight: Real-time Streaming Video Relighting with Interactive Control](https://arxiv.org/abs/2608.01771)
...
TL;DR: LiveLight 是首个基于扩散模型的实时流式视频重打光框架，支持交互式 3D 光照控制。通过轻量适配器注入 MPLI 深度感知辐照度条件、几何引导反馈分支保证低 NFE 下的渲染质量、渐进滚动窗口策略实现长视频时序连贯的流式交互。


<details>
  <summary>Details</summary>
Motivation: 现有视频重打光方法多为离线处理，无法满足实时交互需求。实现实时流式重打光需同时解决三个难题：将动态 3D 光照有效注入扩散模型、在极低 NFE 预算下保持高保真生成、支持连续流式交互控制。

Method: 提出三个关键设计：(1) 轻量适配器将 Multi-Plane Light Irradiance (MPLI) 深度感知辐照度图直接注入扩散主干；(2) 几何引导反馈分支，利用冻结几何估计器在训练时约束深度与法线一致的打光，保证几何合理的着色且不增加推理开销；(3) 渐进滚动窗口策略维护不同噪声水平的潜变量块去噪阶梯，通过传播中间状态保证时序连贯并支持任意长度视频与逐帧参考刷新。

Result: 在真实世界与合成基准上达到 SOTA 重打光质量，同时实现实时速度，在时序稳定性、光照可控性与用户偏好上显著优于离线基线。

Conclusion: LiveLight 证明了扩散模型可以在极低 NFE 下实现实时流式视频重打光，并通过几何约束与滚动窗口策略兼顾质量与效率。作者将公开模型、训练数据与合成数据生成器以推动实时交互式重打光研究。

Classification reason: 该论文提出基于扩散模型的实时视频重打光（video relighting）框架，属于视频编辑/可控编辑范畴，映射到 [CANONICAL] visual_editing（图像与视频编辑）子主题。

Importance reason: Solid real-time video relighting engineering, but it is an editing task outside your core continuous-model, generation, and world-model priorities. Signals: video relighting, diffusion-based, real-time streaming, interactive control, low NFE
...
### [85] [FDIR: Harmonizing Fidelity and Human-Machine Preference in Lossy Compression Image Restoration](https://arxiv.org/abs/2608.00111)
...
TL;DR: 提出 FDIR，一种两阶段图像恢复架构，通过质量引导的单步流匹配（QO-Flow）在潜空间恢复全局语义结构，再通过流条件细节精修（FCDR）在像素空间确定性恢复高频纹理并抑制生成幻觉，以协调像素保真度、人类感知与下游机器偏好三者之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有有损压缩图像恢复方法最多只优化保真度、人类感知或下游机器偏好中的一项：保真度导向模型易产生过平滑输出，生成式方法则可能产生看似合理但事实错误的纹理，损害保真度与下游任务精度。作者希望同时协调这三方面需求。

Method: 提出两阶段架构 FDIR：第一阶段 Quality-Guided One-Step Flow Matching (QO-Flow) 通过单次前向传播在潜空间恢复全局语义结构；第二阶段 Flow-Conditioned Detail Refinement (FCDR) 在像素空间确定性恢复高频纹理并抑制生成幻觉，通过互补的归纳偏置解耦冲突需求。

Result: 大量实验表明 FDIR 在保真度上表现优越，同时取得良好的感知-保真度平衡，并在机器偏好方面具有竞争力。

Conclusion: 通过解耦全局语义恢复与细节精修的两阶段设计，FDIR 能够有效协调保真度、人类感知与机器偏好三者之间的权衡，为有损压缩图像恢复提供了一种兼顾多目标的方案。

Classification reason: 该论文研究有损压缩图像恢复（lossy compression image restoration），属于图像恢复/重建类任务，与图像编辑方向（visual_editing）中的图像重建、恢复语义相近，且使用了 flow matching 生成方法，归入多模态生成方向下的图像编辑子主题。

Importance reason: 图像恢复与压缩方向，与用户优先关注的连续多模态、视频/图像生成及世界模型主题关联较弱。 Signals: 图像恢复, 流匹配, 两阶段架构, 保真度与感知权衡, 非生成式主方向
...
### [86] [GSRAIN: Physically Calibrated High-/Low-Frequency Rainfall Synthesis for 3D Gaussian Driving Scenes](https://arxiv.org/abs/2608.02177)
...
TL;DR: GSRAIN 提出一种面向 3D 高斯泼溅（3DGS）驾驶场景的高/低频降雨合成方法，结合实测数据构建的高频雨滴模型与几何感知单步扩散模型生成的低频雨景外观，在统一 3DGS 场景中融合，实现 0-13 mm/h 降雨强度可控，FID 达 149.09，优于 CycleGAN-Turbo 与 WeatherEdit，并支持目标检测与闭环驾驶评测。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶降雨模拟方法在物理可控性与多视角一致性方面存在局限，难以构建可重复、可控的雨天测试场景用于算法评测。

Method: GSRAIN 从实测降雨数据构建高频雨滴模型，并用几何感知的单步扩散模型生成低频雨景外观，将两种效果在统一 3DGS 场景中融合，实现降雨强度在 0-13 mm/h 范围内的可控调节。

Result: FID 达 149.09，优于 CycleGAN-Turbo（155.71）和 WeatherEdit（157.94）；目标检测与闭环驾驶实验表明生成场景能在可控降雨下暴露被测算法的场景相关性能变化。

Conclusion: GSRAIN 为自动驾驶提供了物理可控、可重复且兼容闭环的雨天测试场景构建方法，验证了其在算法鲁棒性评测中的有效性。

Classification reason: 该论文针对 3DGS 驾驶场景进行降雨天气合成，属于可控天气/场景编辑与合成范畴，映射到 [CANONICAL] 的 visual_editing（图像与视频编辑）子主题。

Importance reason: 该论文聚焦自动驾驶场景的降雨合成，与用户优先关注的连续多模态模型、自回归生成及世界模型方向关联较弱。 Signals: 3DGS场景合成, 降雨强度可控, 单步扩散模型, 驾驶闭环评测, 多视角一致性
...
### [87] [Mind the Gap: Zero-Query Jailbreaks via Filter-Generator Discrepancy in Text-to-Image Systems](https://arxiv.org/abs/2608.00973)
...
Importance: 普通相关 (63.86) - 未入选精读

TL;DR: 本文提出一种零查询（zero-query）越狱框架，利用文生图系统中安全过滤器与生成器之间的"过滤器-生成器差异"（FGD），通过可观测差异规则筛选高潜力扰动候选，再结合代理模型集成进化搜索，无需访问目标系统即可构造绕过安全过滤的对抗提示词，在六个黑盒流水线和商业在线服务上显著提升攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 文生图系统通常在生成器前设置提示级安全过滤器以拦截不安全请求，但仍易受恶意越狱提示攻击。现有基于迁移的攻击方法离线构造对抗提示，但容易过拟合单一代理模型，且在大搜索空间中仅靠语义或感知相似度无法同时保证绕过过滤器并保留不安全生成意图，浪费大量低潜力候选。

Method: 提出 Filter-Generator Discrepancy (FGD) 概念，指出过滤器与生成器以不同目标和表征处理同一提示，扰动可降低过滤器感知风险同时保留生成器所需视觉概念。基于此构建零查询越狱框架：先在 tokenization 和语义阶段通过可观测差异规则将扰动筛选为高潜力候选集，再执行无需访问目标的代理模型集成进化搜索。

Result: 在六个黑盒流水线和一个商业在线服务上，方法一致优于代表性基线，平均攻击成功率提升至 29.2%（MHSC）和 33.3%（Q16），相比最强基线分别提高约 8 和 12 个百分点。

Conclusion: 过滤器与生成器之间的表征与目标差异可被利用来构造高效零查询越狱攻击，验证了基于可观测差异规则筛选加代理集成进化搜索的有效性，为文生图系统安全防护提供了新的攻防视角。

Classification reason: 论文研究文生图系统的越狱攻击（jailbreak），属于多模态安全与攻防范畴。虽然该论文主要针对文生图生成系统，但核心是安全攻防，映射到多模态理解与生成方向下的 [CANONICAL] multimodal_safety_attack 子主题最合适。

Importance reason: Focuses on T2I safety attacks, which is outside the user's core interests in continuous/multimodal generation and world models. Signals: zero-query jailbreak, filter-generator discrepancy, text-to-image safety, adversarial prompts, surrogate-ensemble search
...
### [88] [Robust Watermarks Meet Backdoored Models: Evading Diffusion Semantic Watermarks via Stealthy Backdoor](https://arxiv.org/abs/2608.00543)
...
TL;DR: 提出GhostVAE，通过在VAE编码器中植入隐蔽后门，使扩散模型生成的图像在触发条件下可靠规避语义水印检测，同时保持良性图像上的水印检测性能。


<details>
  <summary>Details</summary>
Motivation: 语义水印被视为保护LDM生成图像的重要机制，但其检测流程依赖神经网络，引入了尚未被充分研究的后门攻击面。作者旨在系统性地研究这一漏洞。

Method: GhostVAE分两阶段：首先通过功率谱正则化构造通用触发器以提升触发鲁棒性，然后使用参数对齐目标训练带后门的VAE编码器。

Result: 在三种语义水印方案和三种LDM上，良性图像平均真阳性率94.4%，触发激活下平均攻击成功率94.6%；对17种防御方法保持隐蔽性。

Conclusion: 该工作从根本上削弱了语义水印系统的可信度，表明安全部署语义水印需要端到端的安全考量，尤其是针对神经网络组件。

Classification reason: 该论文研究针对扩散模型语义水印的后门攻击（GhostVAE），属于生成模型安全与攻防范畴。虽然multimodal_safety_attack是multimodal_understanding_generation方向下的规范子主题，但论文核心是攻击扩散生成模型的水印检测系统，与生成模型安全高度相关。考虑到multimodal_generation方向下没有专门的安全攻防子主题，且multimodal_safety_attack作为规范子主题最贴近论文主题，故复用该子主题。

Importance reason: Security attack on diffusion watermarks is interesting but tangential to your core focus on continuous/multimodal generation and world models. Signals: diffusion watermark evasion, VAE backdoor, security/attack, cs.CR, no continuous token/world model overlap
...
### [89] [LeapTalk: Breaking the Latency-Quality Trade-off in Talking Head Generation](https://arxiv.org/abs/2608.00079)
...
TL;DR: GemTalk 是一个基于扩散框架的情绪化说话人脸生成方法，结合隐式表征的语义丰富性与显式几何先验的结构精度，通过几何引导的情绪调制实现可控且逼真的情绪表达生成。


<details>
  <summary>Details</summary>
Motivation: 现有音频驱动的情绪化说话人脸生成方法难以在可控性与视觉保真度之间取得平衡：隐式表征虽能捕获丰富语义但缺乏结构引导，常导致情绪表达平均化；显式几何方法虽能更好地控制面部表情但会牺牲高频纹理细节。

Method: 提出 GemTalk 扩散框架，包含三个核心模块：Vision-guided Audio Emotion Projection (V-AEP) 提取隐式情绪唇部与表情特征；Diffusion-based Geometric Priors Generator (D-GPG) 生成身份感知的 blendshape 系数作为显式结构先验；Geometry-guided Emotion Modulation (GEM) 模块利用几何先验重新校准隐式特征的幅度，实现对情绪表达（尤其是情绪强度）的精确连续控制。

Result: 大量实验表明 GemTalk 在照片真实感和面部情绪动态方面均取得优越性能，实现了可控性与视觉质量的兼顾。

Conclusion: 通过将隐式表征的语义丰富性与显式几何先验的结构精度相结合，GemTalk 有效解决了情绪化说话人脸生成中可控性与视觉保真度的权衡问题，实现了精确连续的情绪强度控制而不牺牲视觉质量。

Classification reason: 该论文研究音频驱动的情绪化说话人脸生成（emotional talking face generation），属于数字人/说话头/面部动画范畴，与 [CANONICAL] avatar_motion_generation（数字人与动作生成）高度匹配。

Importance reason: Focuses on emotional talking face generation, which is outside the user's core continuous multimodal, world model, and autoregressive generation priorities. Signals: diffusion-based framework, controllable generation, emotion intensity control, blendshape coefficients, audio-driven talking face
...
### [91] [FineMoLA: Towards Fine-Grained Motion-Language Alignment from Clip-Level Supervision](https://arxiv.org/abs/2608.01392)
...
TL;DR: FineMoLA 提出一种弱监督框架，从 clip 级标注中学习细粒度帧-短语对应关系，将动作-语言对齐建模为最优传输问题，通过熵正则化与 Sinkhorn 迭代推断伪帧级对齐，无需人工标注即可提升动作-文本对齐精度。


<details>
  <summary>Details</summary>
Motivation: 大规模动作-语言数据集通常只在 clip 级别提供监督，缺乏动作帧与语言之间的显式时序对应，限制了细粒度动作-文本对齐与时序精确生成。

Method: 先将长描述分割为含动作的短语，再将动作-语言对齐形式化为最优传输问题，利用熵正则化与 Sinkhorn 迭代在全局约束下建模帧与文本之间的多对多关系，推断伪帧级对齐。

Result: 在 SnapMoGen 数据集上的实验表明，学习到的对齐在动作-文本对齐任务上优于基线方法。

Conclusion: FineMoLA 证明无需人工标注即可从 clip 级监督中有效学习细粒度帧-短语对齐，为时序精确的动作生成提供了可行路径。

Classification reason: 论文研究文本条件的人体动作生成（text-conditioned human motion generation），属于数字人与动作生成范畴，映射到 CANONICAL 子主题 avatar_motion_generation。

Importance reason: Focuses on motion-language alignment for human motion generation, which is outside the user's core continuous multimodal, autoregressive generation, and world model priorities. Signals: motion generation, optimal transport, weak supervision, clip-level supervision, text-to-motion
...
### [92] [Motion Beyond Morphology: Bootstrapping Cross-Category Motion Transfer from Abstract Motion Representations](https://arxiv.org/abs/2608.01628)
...
TL;DR: 提出 Motion Beyond Morphology 框架，实现跨类别（不同形态、关节结构）的视频运动迁移。两阶段方法：阶段一学习多粒度抽象运动表征并引导生成跨类别视频对，阶段二将监督内化为直接参考视频条件生成，无需推理时显式运动提取。同时发布 OpenVMT-Dataset 与 OpenVMT-Bench 用于训练与评测。


<details>
  <summary>Details</summary>
Motivation: 现有视频运动迁移方法依赖固定的结构对应关系，当参考与目标对象在形态、关节结构或形变机制上差异显著时，这种对应关系难以定义，导致跨类别运动迁移失效。作者希望超越固定结构对应，保留在不同形态间仍有意义的动力学。

Method: 两阶段框架。阶段一学习互补的多粒度抽象运动视图，并利用它们引导生成跨类别视频对，以保留跨形态可迁移的动力学；阶段二将这种监督内化为直接以参考视频为条件的生成，消除推理时显式运动提取的需求。同时构建 OpenVMT-Dataset 与 OpenVMT-Bench，覆盖 Same、Near、Far 三种类别差距。

Result: 在 OpenVMT-Bench 上针对图像与文本条件的运动迁移任务，在运动保真度与目标保持两方面均达到最先进水平（SOTA）。

Conclusion: 通过抽象运动表征引导跨类别视频对，并内化为直接条件生成，可有效实现超越形态差异的运动迁移，为跨类别动作动画提供了新范式。

Classification reason: 论文研究视频运动迁移（motion transfer），即用参考视频的动力学驱动目标对象动画，属于动作生成与动画范畴，映射到 [CANONICAL] 数字人与动作生成（avatar_motion_generation）子主题。

Importance reason: Cross-category motion transfer is a niche video generation task with limited direct alignment to the user's core continuous-model and world-model priorities. Signals: video generation, motion transfer, cross-category, reference-video-conditioned, dataset+benchmark
...
### [93] [G-Skin: Learning to Bind 3D Gaussians with Generative Visual Priors](https://arxiv.org/abs/2608.01726)
...
TL;DR: PhotoHOI从单张RGB照片和开放词汇语言指令合成3D手-物交互序列，通过VLM解析任务规格、恢复任务相关3D场景、规划碰撞感知物体轨迹，并利用可迁移的接触与抓取先验在潜空间中细化手部姿态。


<details>
  <summary>Details</summary>
Motivation: 现有手-物交互合成方法通常需要预定义物体几何、物体轨迹或任务特定条件，难以处理自然真实世界输入。作者希望解决从单张真实照片和自然语言指令直接合成3D手-物交互序列这一更实际的问题。

Method: PhotoHOI首先使用视觉语言模型将输入图像和指令解析为结构化任务规格（交互物体、目标区域、空间关系）；然后恢复紧凑的任务相关3D场景，基于物体状态、支撑关系和周围场景几何规划平滑的碰撞感知物体轨迹；为合成能泛化到真实照片和未见物体的手部运动，从大规模affordance和HOI数据学习可迁移的任务条件接触先验和接触条件抓取先验；抓取在学习的潜空间中进一步细化，将优化约束在合理的hand-pose流形上。

Result: 在GRAB和H2O数据集上相比代表性基线方法提升了接触质量和减少了穿透；在真实照片上展示了更高的任务成功率和场景一致性，并能泛化到未见物体和开放词汇指令。

Conclusion: PhotoHOI证明了从单张RGB照片和开放词汇语言指令合成3D手-物交互序列的可行性，通过VLM解析、场景恢复、轨迹规划与可迁移抓取先验的组合，实现了对真实世界输入的良好泛化。

Classification reason: 论文聚焦于从单张RGB照片合成3D手-物交互序列，属于人体手部动作生成与数字人交互范畴，映射到[CANONICAL]数字人与动作生成（avatar_motion_generation）。同时涉及动作条件生成与场景规划，与world_model方向也有一定关联。

Importance reason: Solid but niche HOI synthesis work; lacks direct alignment with your continuous multimodal/world-model priorities. Signals: 3D hand-object interaction, single RGB photo, VLM task parsing, contact/grasp priors, GRAB/H2O benchmarks
...
### [95] [UniMoCa: Unifying Motion and Camera Controls as Visual Proxies for Faithful Human Video Generation](https://arxiv.org/abs/2608.01944)
...
TL;DR: UniMoCa 提出 Motion-Camera Visual Proxy (MCVP) 表示，将 3D 人体动作与相机轨迹统一为视觉空间中的身份中性视觉代理，实现人体视频生成中动作与相机控制的统一与联合推理。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成管线中，人体动作控制依赖骨架图、姿态图等视觉序列，而相机控制使用相机 embedding 等非视觉几何表示。这种异构控制接口迫使模型在像素对齐的视觉线索与非视觉几何 embedding 之间协调，导致动作-相机归因困难且对相机估计误差敏感。

Method: 提出 UniMoCa 框架，核心是 Motion-Camera Visual Proxy (MCVP)：将驱动视频中提取的 3D 人体动作与相机轨迹转换为身份中性的视觉代理表示，在恢复的相机轨迹下渲染时间对齐的人体几何，并附加显式相机轨迹标记，将异构的视觉-参数控制替换为可区分的视觉线索。同时构建 MCVP-Video 数据集，覆盖复杂动作、多人交互与多样相机轨迹。

Result: 基于 Wan2.2 I2V 的实验表明，UniMoCa 在人体动作控制、相机控制、时序一致性与相机感知鲁棒性上取得显著提升，且仅引入极小的额外复杂度。

Conclusion: 将动作与相机控制统一到同一视觉空间可消除异构控制接口带来的归因困难，使两种控制因素相互兼容，实现一致的联合推理与编辑，为忠实的人体视频生成提供了有效方案。

Classification reason: 论文聚焦人体动作与相机控制的视频生成，属于数字人与动作生成范畴，映射到 [CANONICAL] avatar_motion_generation（数字人与动作生成）。

Importance reason: Solid human video generation control work, but it targets digital human/action generation rather than the user's core continuous multimodal or autoregressive video priorities. Signals: human video generation, motion and camera control, visual proxy representation, Wan2.2 I2V, identity-neutral control
...
### [96] [Proxy Avatar Meets Low-Rank Caching: Real-Time One-Shot Emotion-Controllable Portrait Animation](https://arxiv.org/abs/2608.01978)
...
Importance: 普通相关 (54.15) - 未入选精读

TL;DR: 提出 Proxy Avatar Meets Low-Rank Caching 级联框架，用高斯表情代理头像作为可复用动作生成器，结合大规模 one-shot 重定向模型与低秩缓存实现实时、单样本、表情可控的肖像动画。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的音频驱动肖像动画方法在实时单样本生成与表情控制方面仍具挑战，存在表情感知动作先验不足以及多步去噪中外观计算昂贵的问题。

Method: 采用级联框架：先用高斯表情代理头像作为可复用动作生成器，在单一身份上训练一次以从音频和表情标签生成富有表现力的驱动视频；再用大规模 one-shot 重定向模型从代理表演中提取身份无关动作并适配任意目标肖像；最后引入零样本外观复用与低秩缓存，在初始去噪步缓存参考外观特征，并用轻量低秩适配器建模后续特征变化以提升推理效率。

Result: 实验表明该方法在情感表现力、身份保持动画和推理成本上均优于现有方法，实现了实时单样本肖像动画。

Conclusion: 通过解耦动作生成与外观重定向，并利用低秩缓存加速推理，实现了实时、表情可控、身份保持的单样本肖像动画。

Classification reason: 论文研究音频驱动的肖像动画（portrait animation），属于数字人与动作生成范畴，与 [CANONICAL] avatar_motion_generation（数字人与动作生成）高度匹配。

Importance reason: Solid engineering for real-time portrait animation, but tangential to the user's core continuous-model and world-model priorities. Signals: real-time one-shot portrait animation, emotion-controllable, low-rank caching, Gaussian proxy avatar, diffusion-based
...
### [97] [Noise-Robust Conditional Flow Matching: Generating Clean Samples from Noisy Datasets](https://arxiv.org/abs/2608.00064)
...
TL;DR: 本文提出 NR-CFM，一种噪声鲁棒的条件流匹配生成器，能从每张图像仅一个含噪观测中学习干净数据分布，对加性高斯白噪声提供闭式端点校正，对复杂协方差的高斯退化学习数据驱动校正，在科学成像低信噪比场景下生成合理粒子图像。


<details>
  <summary>Details</summary>
Motivation: 科学成像中采集常产生含噪测量，而获取干净参考数据成本高昂甚至不可行。直接在含噪数据上训练生成模型会复现噪声污染。作者希望直接从含噪数据中学习干净总体分布，以在无干净参考的情况下生成高质量图像。

Method: NR-CFM 基于条件流匹配框架，作为无条件生成器从每张图像一个含噪观测中学习。对加性白高斯噪声，推导出闭式干净端点校正；对协方差结构更复杂的一般高斯退化，学习数据驱动的校正项。通过回归目标结合稳定训练与高效采样实现干净样本生成。

Result: 在多种退化设置下，NR-CFM 在大多数情况下优于 NR-GAN，并在高噪声区间与 Ambient Diffusion 保持竞争力。在信噪比低至 0.001 的科学数据上，能从严重污染的测量中生成合理的粒子图像。

Conclusion: NR-CFM 提供了一种无需干净参考即可从含噪数据学习干净分布的有效方案，在科学成像等噪声严重的场景中具有实用价值，为噪声鲁棒生成建模提供了新的思路。

Classification reason: 论文提出 Noise-Robust Conditional Flow Matching (NR-CFM)，一种基于条件流匹配的图像生成方法，用于从含噪数据集中学习干净样本分布。这属于图像生成/扩散类方法，映射到 [CANONICAL] 文生图与图像扩散（text_to_image_diffusion）子主题。

Importance reason: Solid but niche method for noise-robust generation; limited direct overlap with your continuous multimodal/world-model priorities. Signals: flow matching, noise-robust generation, scientific imaging, low SNR, conditional flow matching
...
### [98] [Beyond Edge Maps: Wavelet-Domain Conditioning for Multi-Adapter Map-to-Satellite Diffusion](https://arxiv.org/abs/2608.00083)
*Arisha Prasain*
...
TL;DR: 提出一种仅依赖独立可获取的制图数据（OSM 栅格地图及其小波变换子带）作为条件的 ControlNet 扩散框架，通过双适配器融合实现地图到卫星图像的合成，并在尼泊尔数据稀缺区域数据集上验证了优于单条件方法的性能。


<details>
  <summary>Details</summary>
Motivation: 在低资源地区商业地图合作往往不可用，导致卫星底图陈旧。现有 ControlNet 方法通常依赖从目标图像本身提取的边缘或分割等结构信号，这假设图像已存在，恰恰在需要合成的场景中受限。地图条件方法虽补充了边缘检测，但忽略了频域结构信息。

Method: 提出基于 ControlNet 的扩散框架，仅以独立于目标影像的制图数据为条件：OSM 栅格地图及其平稳小波变换（SWT）子带。两个 ControlNet 适配器分别在地图和波尔表示上训练，基于冻结的 Stable Diffusion 主干，通过 MultiControlNet 融合，同时利用空间结构与频域细节，无需重训多输入模型。

Result: 在尼泊尔配对地图-卫星数据集及 Pix2Pix maps-satellite 基准上评估。组合条件在八个指标-数据集比较中赢得六项（两个数据集的 SSIM 和 PSNR，以及本数据集的 LPIPS Alex/VGG），在 Pix2Pix 两个 LPIPS 主干上与地图单条件持平并优于波尔单条件。波尔单条件在两个数据集上取得最低 FID。

Conclusion: 组合地图与波尔条件能同时利用空间结构与频域细节，在多数逐图像保真度指标上优于单一条件；波尔单条件在分布级真实感（FID）上更优，体现逐图像保真度与分布真实感之间的权衡。鉴于测试集规模较小及 FID 小样本偏差，作者对该差距持谨慎态度。

Classification reason: 该论文提出基于 ControlNet 的扩散框架，将地图与卫星图像进行条件生成，属于图像扩散/条件图像生成范畴，映射到 [CANONICAL] 文生图与图像扩散 子主题。

Importance reason: Solid but niche application of ControlNet to map-to-satellite synthesis; limited novelty beyond wavelet conditioning and lacks direct alignment with your core continuous/multimodal or video-generation priorities. Signals: wavelet-domain conditioning, ControlNet multi-adapter fusion, map-to-satellite diffusion, data-scarce region evaluation, SSIM/PSNR/LPIPS gains
...
### [99] [Reconstruction-Shift Discrimination via Mask-Guided Latent Diffusion for Medical Anomaly Detection](https://arxiv.org/abs/2608.00444)
...
TL;DR: 提出 DMD（Discriminative Mask-Guided Diffusion）框架，通过掩码引导的潜在扩散模型进行重建-位移判别，用于无监督医学异常检测，在五个医学数据集上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 无监督医学异常检测中，基于重建和扩散的方法通常利用输入图像与重建图像之间的残差作为异常证据，但该残差具有歧义性：表达能力强的模型可能保留病理结构，而良性解剖变异、成像噪声和采集差异也会产生较大的重建误差，导致检测不准确。

Method: DMD 首先学习正常图像的紧凑量化潜在表征；然后通过局部化掩码扰动选定的潜在区域，用潜在扩散模型重建被扰动的表征；将重建结果与原始正常图像配对，定义自监督分类任务。推理时，分类器提供图像级异常分数，输入与扩散重建之间的残差提供像素级异常图。

Result: 在脑 MRI、乳腺超声和胸部 X 光五个数据集上的实验表明，DMD 在整体性能上优于现有最先进基线方法。

Conclusion: 通过将残差定位与重建-位移判别相结合，DMD 有效缓解了重建残差的歧义性问题，在医学异常检测中取得了更优的图像级与像素级性能。

Classification reason: 论文提出基于掩码引导的潜在扩散模型（latent diffusion）用于医学异常检测，核心方法是图像扩散模型在潜在空间中的重建与判别。属于多模态生成方向下的图像扩散方法，映射到 [CANONICAL] 文生图与图像扩散（text_to_image_diffusion）子主题。

Importance reason: Medical anomaly detection with diffusion is outside your configured research directions and negative keyword domain. Signals: medical anomaly detection, latent diffusion, mask-guided, reconstruction-shift, cs.CV
...
### [100] [UDT: Reconciling U-Nets and Diffusion Transformers with Data-Adaptive Token Reduction](https://arxiv.org/abs/2608.01298)
...
TL;DR: UDT 是一种数据自适应的 U-Net 扩散 Transformer，通过 token 合并实现下采样/上采样，在保留 DiT token 维度的同时结合 U-Net 的编码-解码优势，在 ImageNet 256x256 上以约 40 倍更快的收敛速度达到与 SiT 相当的 FID。


<details>
  <summary>Details</summary>
Motivation: DiT 的 isotropic transformer 块在深度上渐进学习表征，去噪目标使后期层聚焦于细节重建，导致表征质量下降和编码器-解码器行为失衡。现有方案如 REPA 通过训练正则化增强早期表征，U-Net 风格 DiT 引入多尺度结构但使用可学习下采样算子，与 transformer 架构不兼容且低效。

Method: 提出 UDT（U-Net Diffusion Transformer），通过数据自适应的 token 合并（token merging）进行下采样和上采样，同时保持 DiT 的 token 维度。结合 REPA 表征对齐正则化与架构优化，实现 U-Net 编码-解码结构与 DiT 表征能力的统一。

Result: 基线 UDT 超越现有 U-Net DiT，性能与 REPA 相当；结合架构优化和 REPA 后，XL 模型在 256x256 ImageNet 上 40 epochs 内达到 SiT 1400 epochs 的 7.9 FID（约 40 倍收敛加速）；带 CFG 时达到 FID 1.38（SD-VAE，320 epochs）和 1.35（VA-VAE，500 epochs）。

Conclusion: UDT 通过数据自适应 token 合并成功调和了 U-Net 与 DiT 架构，在保持 DiT 表征能力的同时获得 U-Net 的编码-解码优势，显著加速收敛并提升图像生成性能，为 DiT 提供了新的强骨干架构。

Classification reason: 论文提出 UDT，一种结合 U-Net 与 Diffusion Transformer 的图像扩散架构，属于图像扩散/DiT 基础生成方法，映射到 [CANONICAL] 文生图与图像扩散 子主题。

Importance reason: Novel data-adaptive token merging for U-Net DiTs offers strong convergence gains, but sits at the edge of the user's core continuous-model focus. Signals: diffusion transformer, U-Net DiT, token merging, faster convergence, ImageNet generation
...
### [101] [GeoCore-9B: Towards Geo-Aware Generative Foundation Models in Earth Observation](https://arxiv.org/abs/2608.01896)
...
Importance: 高优先级 (83.66) - 未入选精读

TL;DR: GeoCore-9B 是一个 90 亿参数、从零在纯地球观测数据上训练的生成基础模型，基于 Flow Matching DiT，原生支持文本与连续地理元数据（GSD、经纬度）条件生成，并提出 Geospatial Semantic Alignment loss 约束扩散潜空间轨迹，在云去除、SAR-光学跨模态翻译等任务上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有地球观测生成模型大多依赖微调自然图像先验，可扩展性受限且引入与地理空间约束冲突的视角偏差。作者希望构建首个大规模、从零在纯 EO 数据上训练的生成基础模型，以克服自然图像先验的局限。

Method: GeoCore-9B 基于 Flow Matching 的 Diffusion Transformer (DiT)，原生以文本描述和连续地理元数据（地面采样距离、纬度、经度）为条件。提出 Geospatial Semantic Alignment loss，从冻结的专家教师网络蒸馏地表结构先验（如地形、城区），约束训练时的扩散潜空间轨迹，且不增加推理开销。在全局尺度 Git-10M 数据集上预训练。

Result: 在视觉保真度和地理结构准确性上均达到新的 SOTA，并成功适配云去除、SAR-光学跨模态翻译等实际 EO 应用任务。

Conclusion: GeoCore-9B 证明从零在纯 EO 数据上训练大规模生成基础模型是可行且有效的，地理语义对齐损失能有效解决大尺度训练的收敛与空间定向问题，为地球观测生成建模提供了新范式。

Classification reason: 该论文提出基于 Flow Matching 的 Diffusion Transformer (DiT) 生成基础模型，用于地球观测图像生成，属于图像扩散/DiT 生成方法，映射到 [CANONICAL] 文生图与图像扩散 子主题。

Importance reason: Strong generative foundation model work but in remote sensing, which is a negative keyword for your priorities. Signals: flow matching DiT, 9B from-scratch training, geospatial conditioning, cloud removal, SAR-optical translation
...
### [102] [Video Models as Native 4D Renderers: World-Grounded Conditioning from Animated Mesh](https://arxiv.org/abs/2608.00094)
...
TL;DR: 提出DAR，一种参考引导的4D渲染器，将Wan2.2相机控制从Plücker射线扩展到联合相机+几何接口，通过投影神经4D G-buffer（tracking、world position、normal）注入控制适配器，使预训练视频扩散模型能同时遵循相机运动和场景内部动画。


<details>
  <summary>Details</summary>
Motivation: 预训练视频扩散模型在给定动画网格、相机轨迹和参考图像时可以作为渲染器，但需要合适的图像格式条件让视频骨干同时服从相机运动和场景内部动画。现有方法仅使用Plücker射线控制相机，缺乏几何信息，无法处理场景内部动画。

Method: DAR将动画网格投影为神经4D G-buffer（tracking、world position、normal），通过加宽的控制适配器注入预训练的视频扩散模型（Wan2.2），同时保留预训练图像到视频先验。核心设计是tracking（标识应携带外观的持久表面元素）与world position（当前场景坐标状态）的组合，normal提供局部形状。

Result: 在68例DAR-4D基准上，LoRA DAR达到PSNR 23.22、SSIM 0.895、LPIPS 0.134，比现成Wan2.2-Depth提升1.54 dB PSNR；全微调达到PSNR 25.36、SSIM 0.917。消融实验显示用depth替换world position在每个检查点降低PSNR 1.26-1.55 dB。

Conclusion: tracking+world-position对应关系是实用的4D渲染条件，优于depth这种相机依赖的图表表示，因为depth中相机和物体运动混合，而world position提供清晰的场景坐标状态。

Classification reason: 论文利用预训练视频扩散模型（Wan2.2）作为渲染器，通过相机与几何条件控制生成4D渲染视频，属于视频扩散生成方法，映射到[CANONICAL]文生视频与视频扩散子主题。同时涉及动作条件视频生成，也可关联世界模型方向。

Importance reason: Extends video diffusion to 4D rendering with a novel G-buffer conditioning interface, but sits at the edge of the user's core continuous-model and autoregressive priorities. Signals: video diffusion, 4D rendering, camera+geometry control, G-buffer conditioning, Wan2.2 extension
...
### [103] [WAM-Diff2: Hierarchical AR-to-Diffusion Distillation for Highly Efficient Autonomous Driving VLA](https://arxiv.org/abs/2608.01035)
...
Importance: 值得关注 (70.03) - 未入选精读

TL;DR: WAM-Diff2提出一种三阶段分层蒸馏策略，将预训练的自回归VLA通用模型转换为多任务离散扩散VLA框架，在保持性能的同时实现2.8x解码加速，结合系统级优化可达15.1x加速。


<details>
  <summary>Details</summary>
Motivation: VLA模型在自动驾驶中面临高计算延迟和自回归解码带来的暴露偏差问题；而扩散策略虽可并行低延迟执行，但从零训练往往只能得到窄任务架构，缺乏整体视觉语言推理能力。如何将预训练自回归通用模型高效转换为并行扩散模型是核心挑战。

Method: 提出WAM-Diff2多任务离散扩散VLA框架，采用三阶段分层蒸馏策略：渐进式块级适配（block-wise adaptation）、块级蒸馏（block-wise distillation）和模型级跨尺度蒸馏（model-wise cross-scale distillation），通过渐进式架构转换保留基础模型的语义基础并加速推理。

Result: 在驾驶理解、感知和规划基准上，WAM-Diff2有效缓解暴露偏差并达到与自回归基线相当的性能。自回归到扩散的转换带来2.8x解码加速，结合FlashInfer和CUDA Graphs等系统级优化可达到15.1x加速。

Conclusion: 通过分层蒸馏策略成功将自回归VLA通用模型转换为并行扩散模型，兼顾多任务认知智能与执行效率，为自动驾驶VLA模型的高效部署提供了可行路径。

Classification reason: 论文核心是自回归VLA模型到扩散模型的蒸馏转换，实现2.8x-15.1x推理加速，属于生成模型加速与高效化（efficient_generation）范畴。同时涉及VLA多模态理解与生成，但主要贡献是效率优化。

Importance reason: Strong efficiency-focused distillation work for autonomous driving VLA, but peripheral to the user's core continuous/multimodal generation priorities. Signals: autoregressive-to-diffusion distillation, 2.8x/15.1x decoding speedup, autonomous driving VLA, discrete diffusion, model efficiency
...
### [104] [Hybrid-Domain Posterior Sampling for Inverse Problems via Latent Flow Matching](https://arxiv.org/abs/2608.00537)
...
TL;DR: SPARE 是一种无参数的结构化亲和度正则化方法，通过将扩散 Transformer 中间 token 的成对亲和度与干净数据潜空间的亲和度对齐（包括跨图像 token 对），在不引入额外编码器、投影头或参数的情况下加速训练收敛，在 ImageNet 256×256 上以 SiT 骨干达到参数无关正则化器中的最低 FID。


<details>
  <summary>Details</summary>
Motivation: 扩散 Transformer 生成质量高但训练收敛慢。现有正则化方法分为两类：基于目标的方法需要外部编码器和可学习投影头来对齐特征空间，成本高；无目标的方法只能排斥自身特征，丢弃了数据中的结构信息。先前工作表明空间结构而非全局语义驱动了对齐收益，因此作者希望直接利用这种结构作为目标，且该结构不仅存在于单张图像内，还存在于跨图像之间。

Method: SPARE 的核心洞察是干净数据潜空间已经在其 token 间的关系中携带了这种结构，其中关系是两个 token 之间的相似度，是一个标量，无需投影头即可跨特征空间比较。SPARE 将中间 token 的成对亲和度与干净潜空间的亲和度进行匹配，并将匹配扩展到跨图像的 token 对（这正是无目标方法默认排斥的），用单一学习目标校准两种关系类型。

Result: 在 ImageNet 256×256 上使用 SiT 骨干、匹配 400K 迭代预算下，SPARE 不增加编码器、投影头或参数，仅增加 0.08 GB 训练内存，在所有测试设置中达到参数无关正则化器的最低 FID，恢复了 REPA 的 FID 降低的 37-54%，且与 REPA 结合时优于 REPA，在 1M 迭代、classifier-free guidance 下达到 FID 1.90。

Conclusion: SPARE 证明空间结构（token 间成对亲和度）可以直接作为正则化目标，无需外部编码器或投影头，且跨图像的结构匹配比仅排斥更有益。该方法在零额外参数的情况下显著加速扩散 Transformer 训练收敛，并可与其他基于目标的正则化方法（如 REPA）互补结合。

Classification reason: 该论文提出 SPARE 正则化方法，通过匹配中间 token 的成对亲和度来加速扩散 Transformer 的训练收敛，属于生成模型训练效率优化范畴，映射到 [CANONICAL] 的 efficient_generation（生成模型加速与高效化）子主题。

Importance reason: Directly targets flow-matching training acceleration for diffusion transformers, aligning with the user's model efficiency and continuous/multimodal generation priorities, though it is not a continuous-latent or video-generation method. Signals: flow matching, diffusion transformer, training acceleration, parameter-free regularization, ImageNet SiT
...
### [107] [CMuon: Accelerating and Stabilizing Diffusion Transformer Training via Chunked Momentum Orthogonalization](https://arxiv.org/abs/2608.02502)
...
TL;DR: 本文提出 Chunked Muon (CMuon) 优化器，通过将 DiT 中融合的张量（如 AdaLN 和 QKV 层）在正交化前划分为独立子分量，避免隐式子空间耦合，从而加速并稳定 DiT 训练。675M 参数 DiT 在 ImageNet 256 上 200 epoch 达到 FID 1.18，相比 AdamW 提速 2 倍以上，并克服了 vanilla Muon 的后期收敛平台期。


<details>
  <summary>Details</summary>
Motivation: DiT 在视觉生成建模中达到 SOTA，但训练计算成本极高。虽然 Muon 优化器是 AdamW 的有力替代，但直接应用于 DiT 时后期收敛效果不佳。作者识别出根因：标准 DiT 架构将功能不同的权重（如 AdaLN 和 QKV 层）融合为统一张量以提升计算效率，对融合张量应用 Muon 会引入隐式子空间耦合，扭曲更新方向并损害全局优化。

Method: 提出 Chunked Muon (CMuon) 策略：在正交化之前，将融合的权重矩阵划分为独立的子分量（chunks），分别进行动量正交化，从而避免不同功能子空间之间的耦合。该方法简单且高效，无需改变 DiT 架构。

Result: 675M 参数 DiT 使用 CMuon 在 ImageNet 256 上仅 200 epoch 即达到 FID 1.18，相比 AdamW 实现超过 2 倍的训练加速，同时有效克服了 vanilla Muon 的后期收敛平台期。

Conclusion: CMuon 通过分块动量正交化解决了 Muon 在 DiT 融合张量上的子空间耦合问题，显著加速 DiT 训练并提升最终生成质量，为扩散 Transformer 的高效训练提供了新的优化器方案。

Classification reason: 该论文提出 CMuon 优化器以加速和稳定 Diffusion Transformer (DiT) 的训练，属于生成模型训练效率优化范畴，映射到 [CANONICAL] 的 efficient_generation（生成模型加速与高效化）子主题。

Importance reason: 优化器改进虽能加速DiT训练，但非用户核心关注方向，且未涉及连续表示或世界模型。 Signals: DiT训练加速, Muon优化器, 2倍提速, FID 1.18, 训练稳定性
...
### [108] [EchoCache: Energy-Guided Cross-Modal Caching for Efficient Audio-Driven Video Generation](https://arxiv.org/abs/2608.02474)
...
Importance: 高优先级 (85.5) - 未入选精读

TL;DR: EchoCache 是一个能量引导的跨模态缓存框架，用于加速音频驱动视频生成（A2V）。它利用音频时频能量作为显著性锚点指导 latent 级缓存更新，并引入动态 timestep-latent 缓存机制与量化缓存管理，在 Wan2.2-S2V 上实现 2.46x 加速且保持生成质量与音视频一致性。


<details>
  <summary>Details</summary>
Motivation: 音频驱动视频生成（A2V）虽能合成时序连贯且音视频对齐的视频，但扩散模型的迭代去噪过程导致推理成本高昂。现有缓存方法主要利用视觉特征的时间冗余，却忽略了 A2V 中音频驱动视觉生成的高度非均匀时间重要性，存在时序-语义与计算-存储两层错配。

Method: EchoCache 提出能量引导的跨模态缓存框架：1) 以音频时频能量作为显著性锚点，指导 latent 级缓存更新策略；2) 引入动态 timestep-latent 缓存机制，结合量化缓存管理，在效率与内存之间联合优化。

Result: 在主流 A2V 模型上的大量实验表明，EchoCache 持续改善延迟-质量权衡，同时保持生成质量与音视频一致性。在 EMTD 基准上的 Wan2.2-S2V 上实现 2.46x 加速并取得最佳整体性能。

Conclusion: EchoCache 通过能量引导的跨模态缓存有效解决了 A2V 缓存中的时序-语义与计算-存储错配问题，为音频驱动视频生成提供了高效的推理加速方案。

Classification reason: 该论文提出 EchoCache，一种面向音频驱动视频生成的跨模态缓存加速框架，属于扩散模型推理效率优化，映射到 [CANONICAL] 子主题 efficient_generation（生成模型加速与高效化）。

Importance reason: Directly targets efficient inference for video generation with a novel cross-modal caching method, aligning with the user's efficiency and video generation priorities. Signals: efficient inference, video generation, audio-driven video generation, diffusion model acceleration, caching

Abstract: Audio-driven video generation (A2V) has achieved promising progress in synthesizing temporally coherent and audio-visually aligned videos, yet its inference remains expensive due to the iterative denoising process of diffusion models. Existing caching methods mainly exploit temporal redundancy in visual features while overlooking the cross-modal alignment of A2V, where audio drives visual generation with highly non-uniform temporal importance. In this paper, we identify two levels of misalignment in existing A2V caching methods: temporal-semantic and computation-storage misalignment. To address them, we propose EchoCache, an energy-guided cross-modal caching framework for efficient A2V generation. EchoCache leverages audio time-frequency energy as a saliency anchor to guide latent-level cache updates and further introduces a dynamic timestep-latent caching mechanism with quantized cache management for joint efficiency and memory optimization. Extensive experiments on mainstream A2V models show that EchoCache consistently improves the latency-quality trade-off while preserving generation quality and audio-visual consistency. In particular, on Wan2.2-S2V over the EMTD benchmark, EchoCache achieves a 2.46x speedup with the best overall performance. Code is available at .

</details>


### [109] [Token Radius Attention for Efficient Video Generation](https://arxiv.org/abs/2608.02504)
...
TL;DR: 本文提出 Token Radius Attention (TRA)，一种免训练的视频扩散 Transformer 注意力稀疏化加速框架。通过将查询的注意力熵映射为解析 token 预算并转换为时间衰减半径，TRA 仅保留 9-19% 的注意力交互，在 Wan2.1、Wan2.2 和 HunyuanVideo 等七个配置上实现 1.56x-2.05x 加速且保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 视频扩散 Transformer (VDiT) 能生成高保真视频，但密集 3D 自注意力的二次方计算成本高昂。现有 head 级和 block 级稀疏方法在查询间共享计算预算，忽略了 token 特定的注意力需求差异，导致效率与质量难以兼顾。

Method: TRA 基于两个观察：(1) 保留密度随查询变化但与注意力熵呈对数线性相关；(2) 主导交互形成以查询为中心、半径随 token 变化的邻域。据此，TRA 将查询熵映射为解析 token 预算，并转换为时间衰减半径而无需显式 key 排序。同时采用融合熵提取、warm-up 复用和 block 稀疏掩码构建来降低额外开销。

Result: 在 Wan2.1、Wan2.2 和 HunyuanVideo 的七个 T2V/I2V 配置上，TRA 仅保留 9-19% 的注意力交互，实现 1.56x-2.05x 加速，同时保持具有竞争力的生成质量。

Conclusion: TRA 验证了 token 特定注意力需求在视频扩散 Transformer 中的重要性，通过熵驱动的自适应稀疏注意力在无需训练的情况下显著降低计算成本，为高效视频生成提供了新的稀疏化范式。

Classification reason: 该论文提出 Token Radius Attention (TRA) 用于视频扩散 Transformer 的注意力稀疏化加速，属于生成模型效率优化范畴，映射到 [CANONICAL] 子主题 efficient_generation（生成模型加速与高效化）。

Importance reason: Directly targets efficient video diffusion inference with training-free acceleration, aligning strongly with your model efficiency and video generation priorities. Signals: video diffusion transformer, training-free acceleration, attention sparsity, efficient inference, video generation

Abstract: Video Diffusion Transformers (VDiTs) enable high-fidelity generation but incur quadratic cost from dense 3D self-attention. Existing head- and block-level sparse methods share computation budgets across queries, overlooking token-specific attention demand. We observe that retained density varies across queries yet correlates log-linearly with attention entropy, while dominant interactions form query-centered neighborhoods with token-dependent radii. Based on these findings, we propose Token Radius Attention (TRA), a training-free framework that maps query entropy to an analytic token budget and converts it into a temporally decayed radius without explicit key ranking. Fused entropy extraction, warm-up reuse, and block-sparse mask construction further reduce overhead. Across seven Wan2.1, Wan2.2, and HunyuanVideo T2V/I2V configurations, TRA retains only 9-19% of attention interactions and achieves 1.56x-2.05x speedup with competitive generation quality. Code is available at .
...
### [110] [Element-Aware Group Learning for E-Commerce Image Generation](https://arxiv.org/abs/2608.00584)
...
TL;DR: 提出 EAGLE-GRPO，一种元素感知的组学习框架，用于电商图像生成提示词优化。将组级奖励分解到预定义设计元素上，把元素级信用分配建模为核岭回归问题并推导闭式解，无需额外 rollout 或独立信用分配模型，从而获得可解释的逐元素优势与更精确的策略更新。


<details>
  <summary>Details</summary>
Motivation: 电商图像生成中，提示词质量成为关键瓶颈。VLM 可从产品图像和元数据生成编辑提示词，但进一步提升提示词写作能力需要基于生成图像反馈进行后训练。GRPO 是自然的奖励优化框架，但只在完整提示词层面分配信用，而图像质量往往取决于构图、背景、卖点呈现等具体设计元素。现有细粒度信用分配方法需要步骤级监督或学习型评论器，成本高。

Method: 提出 EAGLE-GRPO，将组中心奖励分解到预定义元素上。将元素级信用分配建模为核岭回归问题，推导出闭式解，无需额外 rollout 或独立信用分配模型。由此得到可解释的逐元素优势，用于更精确的策略更新。

Result: 实验表明 EAGLE-GRPO 在更多训练步骤上保持性能增益后才趋于平稳，生成的提示词能产生比竞争性 VLM 提示词写作基线更高质量的电商图像。

Conclusion: EAGLE-GRPO 通过元素感知的信用分配，在无需额外模型或 rollout 的情况下实现了更精确的策略更新，提升了电商图像生成提示词的质量与训练效率。

Classification reason: 论文提出 EAGLE-GRPO，将 GRPO 奖励优化扩展到电商图像生成提示词写作，属于生成模型对齐与奖励优化范畴，映射到 [CANONICAL] generation_alignment（生成模型对齐与奖励）。

Importance reason: Focuses on e-commerce prompt optimization with GRPO, which is tangential to the user's core interests in continuous multimodal models and video/image generation. Signals: GRPO, e-commerce image generation, prompt optimization, element-level credit assignment, kernel ridge regression
...
### [111] [EmergencyBias: Bias in Text-to-Image Models under Emergency Scenarios](https://arxiv.org/abs/2608.00598)
*Haibo Tang, Linqi Zhang, Hongxin Huan, Chenwei Lin, Xian Xu*

Main category: cs.MM, cs.CY
...
TL;DR: 本文定义并系统研究了文生图（T2I）模型在紧急场景下的偏见问题（EmergencyBias），涵盖人口统计偏见与行为偏见，构建评测框架评估七个主流T2I模型在六种紧急场景和三个维度上的表现，并提出轻量级提示嵌入校准方法ActionAlign以减少行为偏见。


<details>
  <summary>Details</summary>
Motivation: 现有T2I偏见研究主要关注静态显式偏见（如性别、种族、地域文化表征差异），而较少关注不同群体在行为、反应和社会角色分配上的行为偏见。紧急场景是研究此类偏见的理想情境，因为它要求模型不仅刻画"谁在场"，还要刻画"谁处于风险中""谁介入干预"以及"责任如何分配"。

Method: 作者定义了EmergencyBias，包含人口统计偏见与行为偏见两类。构建了系统评测框架，覆盖七个主流T2I模型、六种代表性紧急场景和三个维度（性别、年龄、肤色）。实验包括无人口统计指定的空白提示下的偏见检测，以及受控提示下的行为偏见分析。此外提出ActionAlign，一种轻量级提示嵌入校准方法，用于减少行为差异。

Result: 三个主要发现：1）在无人口统计指定的空白提示下，T2I模型在紧急场景中表现出明显的人口统计偏见，反映在性别、年龄和肤色分布上；2）在受控提示下，紧急响应的行为偏见与人口统计变化系统性相关，性别维度差异尤为显著且模型间差异大；3）ActionAlign在减少行为差异方面优于代表性基于提示的基线方法，同时基本保持图像质量。

Conclusion: 紧急场景是评估T2I模型偏见的重要情境，本文提供了系统化的评测框架和实用的校准方法，为在社会影响重大的场景中实现更公平的视觉生成指明了方向。

Classification reason: 该论文研究文生图模型在紧急场景下的偏见评测，构建了EmergencyBias评测框架并评估七个T2I模型，属于生成模型评测与数据集方向，映射到[CANONICAL] generation_benchmark。

Importance reason: Focuses on bias evaluation in T2I models, which is outside the user's core interests in continuous/multimodal generation and world models. Signals: T2I bias evaluation, emergency scenarios, demographic/behavioral bias, ActionAlign calibration, no autoregressive/diffusion video or continuous token focus
...
### [112] [AcoustiTrace: When Plausible Sound Violates Physics](https://arxiv.org/abs/2608.02035)
*Shiyang Li, Yuewen Cao, Yihao Liu, Yuandong Pu, Baochang Zhang, Xiaofei Li, Changqing Zou*

Main category: cs.MM, cs.SD
...
TL;DR: AcoustiTrace 是一个诊断基准，形式化定义音视频生成中的声学物理真实性，围绕声学过程（声音生成、传播环境、声学接收）组织 T2AV/I2AV 评测，构建大规模声学机制数据集与评测器，揭示现有生成器在基本声学过程上的不足，并可用于指导模型改进。


<details>
  <summary>Details</summary>
Motivation: 现有音视频生成器能产生语义合理且表面同步的声音，但仍可能违反可见事件与环境所隐含的声学物理过程。已有基准难以将此类违规归因到特定声学过程并量化其严重程度，缺乏对声学物理真实性的系统评测。

Method: 提出 AcoustiTrace 诊断基准，围绕声学过程组织 T2AV 和 I2AV 评测，涵盖声音生成、传播环境、声学接收三个层面共八个维度，并基于可测量的声学量进行量化。构建大规模声学机制数据集（含真实音视频记录与声学标注的 RGB-D 观测），开发针对性提示套件与验证过的评测器。

Result: 实验表明，即使领先的生成器在产生合理声音事件的同时，仍难以处理基本声学过程。AcoustiTrace 提供的特定声学关系诊断可指导模型改进，使其音频更符合物理规律。

Conclusion: AcoustiTrace 为音视频生成中的声学物理真实性提供了系统化评测框架，其诊断结果可引导模型朝更物理忠实的方向改进，并为将声学原理融入训练目标、奖励建模与候选选择开辟新方向。

Classification reason: 该论文提出 AcoustiTrace 诊断基准，用于评测音视频生成中的声学物理真实性，属于生成模型评测基准与数据集方向，映射到 [CANONICAL] generation_benchmark（生成模型评测与数据集）。

Importance reason: Focuses on audio-video generation evaluation rather than the user's core continuous multimodal or autoregressive generation priorities. Signals: audio-video generation, acoustic physical realism, benchmark, T2AV/I2AV, multimodal_generation
...
### [113] [CultureVidBench: Benchmarking Cultural Understanding in Text-to-Video Generation](https://arxiv.org/abs/2608.01942)
*Xianjing Han, Yuhan Su, Yang Deng, Dong Ma, Wee Peng Tay, Bin Zhu*

Main category: cs.MM, cs.CL, cs.CV
...
TL;DR: CultureVidBench 是一个评测文生视频（T2V）模型文化理解能力的新基准，包含 1,000 条覆盖 12 国、6 大洲、8 个文化区域、14 个文化维度的提示词，从物质文化、社会实践与表演、仪式与典礼三个类别评估模型的文化忠实度、多模态文化渲染、语义遵循与感知质量。实验表明现有 T2V 模型虽语义遵循和视觉质量较强，但在细粒度文化细节、欠发达地区、仪式及多模态文化线索上表现不足。


<details>
  <summary>Details</summary>
Motivation: 现有文生视频基准主要关注感知质量、物理合理性和文本-视频对齐，未直接评估生成视频是否准确捕捉特定文化的物体、动作、仪式、可见文本或音频线索。文化理解在 T2V 生成中尚未被系统评测。

Method: 构建 CultureVidBench 基准：设计 1,000 条提示词，覆盖 12 国、6 大洲、8 个文化区域、14 个文化维度，分为物质文化、社会实践与表演、仪式与典礼三大类。针对视频生成专门设计，强调动态与多模态文化表征（社会互动、仪式流程、文化适配的可见文本与音频）。通过人类用户研究与 MLLM 自动评估，从文化忠实度、多模态文化渲染、语义遵循、感知质量四个维度评测七个代表性 T2V 模型。

Result: 当前 T2V 模型在语义遵循和视觉质量上表现较强，但常无法忠实捕捉细粒度文化细节，尤其在欠代表性地区、仪式场景和多模态文化线索（文本、音频）上失败明显。

Conclusion: 现有文生视频模型的文化理解能力仍存在显著不足，CultureVidBench 为文化感知的 T2V 生成提供了系统评测框架，并揭示了文化忠实度这一被忽视的评测维度。

Classification reason: 该论文提出 CultureVidBench，一个用于评测文生视频模型文化理解能力的基准，包含 1,000 条提示词、多国多文化维度，并通过人类研究与 MLLM 自动评估。这属于生成模型评测与数据集，映射到 [CANONICAL] generation_benchmark（生成模型评测与数据集）。

Importance reason: Benchmark paper on cultural T2V evaluation, but lacks direct focus on the user's priority topics of continuous models, world models, or generation methods. Signals: text-to-video benchmark, cultural understanding, evaluation, MLLM-based assessment, not method-focused
...
### [114] [MIEScore: Human-Aligned Evaluation for Multi-Source Image Editing](https://arxiv.org/abs/2608.02059)
...
TL;DR: Poplar 是一个可复现的"指定-渲染-审查"流水线，用于合成人类中心图像数据集，并据此构建了包含 9,401 个精选图像-文本对的 Poplar-9K 数据集。


<details>
  <summary>Details</summary>
Motivation: 生成单张成功的人类图像与构建一个有用的人类中心数据集是不同的任务。数据集需要覆盖多样化的人群与场景、避免不合理的属性组合、保持日常摄影风格，并在大规模下暴露质量控制决策。现有方法缺乏可复现、可审计的数据集合成流水线。

Method: Poplar 采用 Specify-Render-Inspect 三阶段流水线：Specify 在常识约束下采样结构化属性并转化为摄影导向的提示词；Render 使用适配真实感的图像生成器，结合构图感知的宽高比并重试明显技术失败；Inspect 对每个候选图像应用单一结构化视觉-语言审查，保留原始提示词的同时拒绝内在图像缺陷或提示词不匹配。

Result: 构建了 Poplar-9K：从 11,765 个审查候选中保留 9,401 个精选人类中心图像-文本对（79.9% 接受率），并发布数据集、流水线、配置、不可变生成提示词与可审计的审查记录。

Conclusion: Poplar 提供了一种紧凑、可复现且可审计的资源，用于构建可定制的人类中心图像集合，展示了结构化流水线在规模化数据合成质量控制中的价值。

Classification reason: 该论文构建了 Poplar-9K 人类中心图像-文本数据集，并发布了可复现的数据合成流水线，属于生成模型数据集构建与评测范畴，映射到 [CANONICAL] 的 generation_benchmark（生成模型评测与数据集）子主题。

Importance reason: Dataset synthesis pipeline for human-centric images, but lacks direct connection to the user's core continuous/multimodal generation and world model priorities. Signals: dataset synthesis, human-centric images, specify-render-inspect pipeline, quality control, image-text pairs
...
### [116] [Image-Space Rule Discovery](https://arxiv.org/abs/2608.00490)
...
TL;DR: 本文提出 WISRD（Worksheet Image-Space Rule Discovery）基准，包含 11 个核心任务和 4 个推理压力探针，用于评测图像编辑模型能否像人类完成工作表测试一样，在图像空间中读取指令、识别问题、推断答案并完成端到端求解。评测发现 Nano Banana Pro 表现最佳，但整体上当前图像编辑模型在图像空间规则发现上能力仍有限。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型主要关注像素级编辑，但能否在图像空间中完成需要推理的"规则发现"任务（如 IQ 测试）尚未被系统研究。作者希望以人类工作表测试为灵感，探究图像编辑模型能否端到端地读取图像指令、识别问题、推断答案并绑定到正确位置。

Method: 作者构建 WISRD 基准，包含 11 个核心任务（局部标记、填充、复制、计数、无编辑抑制等）在 8 种信息条件下，以及 4 个补充推理压力探针（多步空间操作、抽象模式推理、逻辑推断、约束求解）。评测了 Nano Banana Pro、Qwen-Image-Edit、FLUX.2 Klein 4B、InstructPix2Pix 等前沿图像编辑模型，采用 Auto-Strict 无参考代理通过率作为指标。

Result: Nano Banana Pro 得分最高，在共享 V0-V3 无参考子集上 Auto-Strict 代理通过率为 48.7%，远高于 Qwen-Image-Edit（13.4%）、FLUX.2 Klein 4B API（11.5%）、FLUX.2 Klein 4B 开源（11.3%）和 InstructPix2Pix（0.0%）。在补充诊断中，Nano Banana Pro 在 4x4 数独上达到 70.0%，在 RAVEN 模式发现上达到 22.9%。

Conclusion: 当前图像编辑模型在图像空间规则发现任务上能力有限，但部分模型（如 Nano Banana Pro）已能部分依赖渲染在图像内的指令，即使外部提示缺失或仅提供通用提示。这为图像编辑模型向推理型任务扩展提供了新的评测视角和基准。

Classification reason: 论文提出 WISRD 基准，用于评测图像编辑模型在图像空间中完成规则发现与问题求解的能力，属于生成模型评测与数据集方向，映射到 [CANONICAL] generation_benchmark 子主题。

Importance reason: A benchmark for image-editing rule discovery, but it is a niche evaluation task with limited direct connection to the user's core continuous-model, autoregressive generation, or world-model priorities. Signals: image editing benchmark, rule discovery, Nano Banana Pro, worksheet test, no-reference evaluation
...
### [117] [DrawAI: Agentic Benchmark and Workflow for Making Raster Images Editable](https://arxiv.org/abs/2608.00548)
...
TL;DR: 本文提出 DrawAI，包含一个智能体基准 DrawAI-Bench 和一个两阶段智能体重建工作流 DrawAI-Flow，用于将光栅图像重建为结构化、可直接编辑的图形代码，同时兼顾保真度与可编辑性。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型和多模态智能体虽能生成高质量视觉内容，但其光栅输出难以直接编辑、修改或复用，因为语义内容被扁平化为像素。需要一种将光栅图像恢复为结构化可编辑产物的方法，同时满足保真度与可编辑性这两个往往相互权衡的目标。

Method: 提出 DrawAI 框架：DrawAI-Bench 覆盖科学图表、演示幻灯片、海报和示意图，结合真实与 AI 生成图像，通过 39 条混合评测协议（确定性规则指标 + 资产特定的视觉语言评分标准）评估保真度与可编辑性；DrawAI-Flow 为两阶段智能体工作流，Parser Agent 将元素证据转化为显式重建计划，Reconstruction Agent 通过迭代的代码-渲染-验证-修订循环实现为可执行图形代码。

Result: 在 DrawAI-Bench 上系统评估了 13 个模型、5 种智能体框架，结果显示重建质量和成本在不同模型-框架配置间差异显著，而 DrawAI-Flow 能持续提升可编辑结构质量。

Conclusion: 图像到可编辑重建是一个重要且具有挑战性的任务，DrawAI 提供了系统的评测基准与工作流，验证了智能体化重建方法在提升可编辑性方面的有效性，为后续研究提供了标准化评测平台。

Classification reason: 论文提出 DrawAI-Bench 基准，用于评测图像到可编辑重建任务的保真度与可编辑性，属于生成模型评测与数据集方向，映射到 [CANONICAL] generation_benchmark。

Importance reason: Focuses on raster-to-editable reconstruction benchmark, which is tangential to the user's core interests in continuous multimodal models and autoregressive generation. Signals: agentic benchmark, editable graphics reconstruction, fidelity-editability tradeoff, multimodal agent evaluation, not aligned with priority subtopics
...
### [118] [Diagnosing Under-Development of Irreversible Processes in Video Generation](https://arxiv.org/abs/2608.00617)
...
TL;DR: 本文研究视频生成模型是否尊重物理不可逆过程（如冰融化、纸燃烧）。作者发现局部逆转指标存在零退化问题，提出两部分的可靠评测协议（方向性属性进展 progress + 停滞率 stasis rate）。在七个文生视频模型上，真实视频进展显著（ρ=+0.40，35%静态），而所有生成器几乎零进展且92-100%停滞；九名标注者评分真实视频远高于生成视频（2.75 vs 0.99）。核心发现是生成器"欠发展"而非"逆转"不可逆属性。此外，事后读出引导可被利用，而通过构造单调性约束的解耦属性潜空间可消除该漏洞。


<details>
  <summary>Details</summary>
Motivation: 许多物理属性是不可逆的（冰融化不会重新冻结，纸燃烧不会复原），但视频生成模型是否尊重这一物理规律尚不清楚。作者指出该问题难以直接测量，现有局部逆转指标存在零退化问题（纯噪声上违规率得0.50，方差归一化逆转残差处于噪声上限），因此需要设计可靠且能通过零假设检验的评测协议。

Method: 提出两部分的评测协议：(1) progress——方向性属性相关性，衡量不可逆属性是否随时间朝正确方向推进；(2) stasis rate——停滞率，衡量属性是否完全不变。在七个文生视频模型上对比真实参考视频。同时提出互补机制：证明事后读出引导（post-hoc readout guidance）可被利用，而通过在解耦属性潜空间中构造单调性约束（enforcing monotonicity by construction）可消除可被利用的读出，并在受控与半合成设置中验证。

Result: 真实参考视频进展显著（ρ=+0.40，35%静态），而所有七个生成器几乎零进展且92-100%停滞；九名标注者评分真实视频远高于生成视频（2.75 vs 0.99，0-4分制）。生成视频与真实视频在该协议下清晰分离，且差距经人类验证。事后读出引导被证明可被利用，而构造性单调性约束可消除该漏洞。

Conclusion: 可靠发现是生成器"欠发展"（under-development）：生成器几乎不推进不可逆属性，而非逆转它们。这揭示了视频生成模型在物理不可逆过程建模上的系统性缺陷，并提出了可经受零假设检验的评测协议以及通过构造性单调性约束来保证物理一致性的机制。

Classification reason: 该论文研究视频生成模型是否尊重物理不可逆属性（如冰融化、纸张燃烧），提出评测协议（progress 与 stasis rate）来诊断生成视频的"欠发展"问题，属于生成模型评测范畴，映射到 [CANONICAL] generation_benchmark（生成模型评测与数据集）。

Importance reason: Solid evaluation methodology for video generation physics, but tangential to the user's core continuous-model and generation priorities. Signals: video generation evaluation, irreversible process, under-development diagnosis, text-to-video benchmark, monotonicity constraint
...
### [119] [MDTD-ArtIR: Benchmarking Image Editing and Restoration Models for Art Image Restoration under Texture-Overlay Degradations](https://arxiv.org/abs/2608.00736)
...
TL;DR: 本文提出 MDTD-ArtIR 基准，用于在纹理叠加退化（如裂纹、污渍、颜色/纹理偏差）下评估图像编辑与修复模型，并配套公开的退化 alpha 纹理掩码数据集 MDTD-Art。实验表明图像编辑模型在任意退化下普遍优于专用修复架构，且结构化提示工程能进一步放大性能优势。


<details>
  <summary>Details</summary>
Motivation: 现有修复基准假设已知退化算子，无法捕捉艺术损伤（裂纹、污渍、颜色/纹理偏差）的复杂特征；同时现有方法常产生不自然纹理、难以保持颜色纹理或无法利用部分保留的图像信息。需要一个受控的盲修复基准来评估模型在语义、半透明介质退化下的表现。

Method: 构建受控的盲修复基准 MDTD-ArtIR，配套公开的退化 alpha 纹理掩码数据集 MDTD-Art，在不同掩码不透明度水平下评估最先进的通用修复模型、图像编辑模型与视觉语言模型，并采用强调细节保留与结构一致性的结构化提示工程。

Result: 实验表明图像编辑模型在任意退化下始终优于专用修复架构，且结构化提示工程能进一步放大性能增益，说明可恢复语义信息与提示可控性是艺术图像修复的关键因素。

Conclusion: 可恢复语义信息与提示可控性被定位为艺术图像修复中的关键因素，图像编辑模型结合结构化提示是处理任意退化的更优方案。

Classification reason: 该论文提出艺术图像修复的受控基准与新的退化 alpha 纹理掩码数据集 MDTD-Art，评估图像编辑与修复模型，属于生成模型评测与数据集范畴，映射到 [CANONICAL] generation_benchmark。

Importance reason: Benchmark for art image restoration with texture overlays, but tangential to the user's core focus on continuous multimodal models and autoregressive generation. Signals: image editing vs restoration, texture-overlay degradation, benchmark dataset, prompt engineering, not aligned with priority subtopics
...
### [120] [InteracVid: Building a Real Interactive Audio-Visual Response Dataset from Live-Chat Videos](https://arxiv.org/abs/2608.01157)
...
TL;DR: InteracVid 是首个开源大规模交互式音视频响应数据集，从 59K 直播视频中提取 454K 上下文-查询-响应三元组，用于训练多模态助手、虚拟人和具身智能体产生由外部用户交互触发的音视频响应。


<details>
  <summary>Details</summary>
Motivation: 现有音视频生成模型的监督大多是描述性的，模型被训练去渲染字幕而非产生由外部用户交互触发的音视频响应，缺乏交互式监督数据。

Method: 设计元数据感知的流水线，从长且嘈杂的直播流中提取交互片段，构建上下文-查询-响应三元组，覆盖对话中心、物体中心、程序性、具身和屏幕场景。

Result: 构建 454K 三元组，十人人类研究确认交互因果性、自然性和时序完整性；在 100 个真实直播查询的基准上，微调 InteracVid 提升了交互规划与音视频响应生成。

Conclusion: 交互结构化数据是交互式多模态生成的关键基础，InteracVid 为交互式音视频响应生成提供了重要数据支撑。

Classification reason: 论文构建了首个大规模交互式音视频响应数据集 InteracVid（454K 三元组），属于生成模型评测与数据集范畴，映射到 [CANONICAL] generation_benchmark 子主题。

Importance reason: Dataset paper for interactive audio-visual generation, relevant to multimodal generation but not directly aligned with the user's core continuous-model or autoregressive generation priorities. Signals: interactive audio-visual dataset, live-chat video, multimodal generation, dataset contribution, not continuous token
...
### [121] [Ranking Image Fusion the Way Humans Do: A Learned Pairwise Preference Metric for Infrared-Visible Fusion Assessment](https://arxiv.org/abs/2608.01301)
...
TL;DR: 本文提出 LPIFM（Learned Perceptual Image Fusion Measure），一种源条件化的学习型成对偏好评测模型，用于红外-可见光图像融合（IVIF）的人类对齐评估。LPIFM 联合观察红外源、可见光源及两个融合候选，预测 A/B/平局偏好，并基于新的密集偏好语料库训练，在成对准确率和排序相关性上大幅超越传统标量指标。


<details>
  <summary>Details</summary>
Motivation: 红外-可见光图像融合（IVIF）缺乏理想的融合参考，现有标量客观指标（如信息传递、结构、源相似性代理）常与人类真实偏好不一致。直接成对比较虽是人类主观评估的黄金标准，但其成本随算法数量呈二次增长，难以常规使用。因此需要一个可扩展、可重复且与人类偏好对齐的评测代理。

Method: 提出 LPIFM，一个源条件化模型，联合输入红外源、可见源和两个融合候选，输出三分类预测（A 更好、B 更好、感知等价）。训练数据来自新的密集偏好语料库，覆盖公开基准场景上广泛融合方法的所有无序成对比较，采用盲化、随机化、两阶段标注协议并辅以专家仲裁。评估采用 tie-aware Bradley-Terry 排序。

Result: 在场景泛化和方法泛化设置下，LPIFM 紧密跟踪人类成对决策，并复现由人类标签导出的 tie-aware Bradley-Terry 排序；在完整方法池上，其成对准确率和排序相关性均大幅超越最强的传统指标。

Conclusion: LPIFM 提供了一种实用的人类对齐方法比较与大规模排序工具，并公开了标注偏好数据集、模型权重、源代码与评测代码，支持偏好对齐的 IVIF 评估。

Classification reason: 该论文提出红外-可见光图像融合（IVIF）的人类偏好评测指标 LPIFM，属于生成/融合模型的评测与人类偏好评估范畴，映射到 [CANONICAL] 生成模型评测与数据集（generation_benchmark）子主题。

Importance reason: Focuses on a narrow image fusion evaluation metric, far from the user's core continuous multimodal, autoregressive generation, and world model priorities. Signals: image fusion evaluation, pairwise preference metric, human-aligned ranking, IVIF assessment, not in priority subtopics
...
### [122] [Generative AI and Foundation Models in Medical Image](https://arxiv.org/abs/2608.01686)
*Masahiro Oda*
...
TL;DR: 本文是一篇综述论文，概述了生成式AI（扩散模型、大语言模型）与基础模型在医学图像处理中的应用，并探讨了如何利用国家数据和计算资源开发医学支持的高性能AI。


<details>
  <summary>Details</summary>
Motivation: 生成式AI和基础模型的兴起深刻改变了医学图像处理的AI开发范式，作者希望通过综述梳理扩散模型、LLM和基础模型在医学支持中的应用现状与发展方向。

Method: 本文采用综述方法，系统介绍图像生成AI中的扩散模型、文本生成AI中的大语言模型，并介绍它们在医学支持中的应用，同时讨论基础模型的构建方法及其在医学领域的应用。

Result: 文章梳理了DALL-E 3、Stable Diffusion、ChatGPT、Gemini等代表性生成式AI服务，以及扩散模型和LLM在医学图像处理与诊断报告生成等任务中的应用，并讨论了基础模型在医学领域的构建与应用。

Conclusion: 生成式AI与基础模型为医学图像处理带来范式转变，未来应充分利用国家层面的数据和计算资源来开发面向医学支持的基础模型和高性能AI。

Classification reason: 该论文是一篇综述性文章，概述扩散模型、大语言模型与基础模型在医学图像处理中的应用，属于生成模型在特定领域（医学）的应用综述，映射到多模态生成方向下的生成模型评测与数据集（generation_benchmark）这一广泛主题。

Importance reason: Medical imaging survey with no direct alignment to your core continuous multimodal, autoregressive generation, or world model priorities. Signals: medical imaging, diffusion models, foundation models, survey, no continuous token/flow matching
...
### [123] [Investigating Social Bias in Narrative Image Generation](https://arxiv.org/abs/2608.01780)
...
TL;DR: 本文通过改编文本偏见评测框架 BBG 到图像生成，系统比较了六种文生图模型在照片、故事板和漫画三种叙事视觉格式中的社会偏见表达，发现专有模型在照片生成中平均有25.9%的偏见输出，在故事板和漫画中分别增加9.6pp和18.2pp，且叙事格式通过事件序列、角色定位、叙事结局和文本元素更显式地暴露偏见。


<details>
  <summary>Details</summary>
Motivation: 文生图模型已广泛应用于媒体内容创作和教育等领域，但其输出可能复现社会偏见。已有评测主要聚焦于照片生成任务，尚不清楚偏见如何在故事板、漫画等多面板叙事视觉格式中表现，因此需要扩展评测范围。

Method: 将基于文本的偏见评测框架 BBG 改编为图像生成任务，在六种文生图模型上比较照片、故事板和漫画三种格式的偏见表达，分析偏见在视觉线索、事件序列、角色定位、叙事结局和文本元素中的表现。

Result: 专有模型在照片生成中平均产生25.9%的偏见输出；故事板生成中偏见输出增加9.6pp，漫画生成中增加18.2pp。照片主要通过细微视觉线索编码偏见，而故事板和漫画通过事件序列、角色定位、叙事结局和文本元素更显式地暴露偏见。

Conclusion: 在照片生成中不太明显的偏见可能在叙事视觉格式中浮现，凸显了在照片生成之外用多样化视觉格式评测文生图系统的重要性。

Classification reason: 该论文研究文生图模型在照片、故事板、漫画等不同视觉格式中的社会偏见表现，属于生成模型评测范畴，映射到 [CANONICAL] 的 generation_benchmark（生成模型评测与数据集）子主题。

Importance reason: Social bias evaluation in narrative image generation is methodologically sound but falls outside your core focus on continuous multimodal models, autoregressive generation, and world models. Signals: social bias, text-to-image evaluation, storyboard/comic generation, proprietary model bias, narrative visual formats
...
### [124] [PosterMELD: Multi-Agent Paper-to-Poster Generation for Controllable Design Diversity with Editable Print-Ready Outputs](https://arxiv.org/abs/2608.02218)
...
Importance: 值得关注 (66.81) - 未入选精读

TL;DR: PosterMELD 是一个模板条件化的多智能体论文转海报生成流水线，通过容量感知槽位引导写作、确定性门控与 VLM 审查路由失败到有界修复，输出可编辑的 PPTX 和 PNG 产物。在 621 篇论文上达到 81.3% 的 Print-Ready Rate，是 P2P 的 3.4 倍、PosterGen 的 5.2 倍，且条件 CHE 分数最高，单请求成本仅 0.38 美元。


<details>
  <summary>Details</summary>
Motivation: 现有科学海报生成系统存在两类问题：直接图像生成不可逐元素编辑，而编码智能体工作流成本高昂；同时现有系统仅对完成输出评分，掩盖了请求级失败。作者旨在构建一个既可控、可编辑又成本低廉的论文转海报生成系统。

Method: PosterMELD 采用模板条件化的多智能体流水线：容量感知槽位在渲染前引导写作；确定性门控结合 VLM 审查将失败请求路由到有界修复循环。每个被接受的请求导出可编辑的 PPTX 与 PNG 产物，并通过显式设计控制生成同一论文的多种变体。评测采用 Print-Ready Rate (PRR) 检查几何、可读性、资产完整性与明显事实错误，冻结 VLM 分配条件 Craftsmanship-Harmony-Expressiveness (CHE) 分数。

Result: 在 621 篇论文上，PosterMELD 达到 81.3% 的 PRR，是 P2P 的 3.4 倍、PosterGen 的 5.2 倍，并在生成方法中获得最高的条件 CHE 分数。原生可编辑性与显式设计控制得以保留，平均每请求成本 0.38 美元，仅为 Codex+Skill 的 3.5%。

Conclusion: PosterMELD 证明模板条件化的多智能体流水线能够在保持原生可编辑性与设计多样性的同时，显著提升论文转海报生成的打印就绪率，并以极低成本实现高质量输出，为科学海报自动生成提供了实用且可扩展的解决方案。

Classification reason: 该论文提出 PosterMELD 多智能体论文转海报生成系统，并引入 Print-Ready Rate (PRR) 与 Craftsmanship-Harmony-Expressiveness (CHE) 等评测指标，在 621 篇论文上进行评测。核心贡献包含生成评测指标与数据集，映射到 [CANONICAL] generation_benchmark（生成模型评测与数据集）。

Importance reason: Focuses on poster generation, a niche output format outside the user's core continuous-model, autoregressive generation, and world-model priorities. Signals: multi-agent pipeline, controllable design diversity, editable print-ready outputs, VLM review routing, cost-efficient generation

Abstract: Scientific poster construction compresses a long multimodal paper into a readable, editable canvas. Existing systems hide request-level failures by scoring only completed outputs; direct image generation is not element-editable, while coding-agent workflows are costly. PosterMELD is a template-conditioned multi-agent pipeline: capacity-aware slots guide writing before rendering, and deterministic gates plus vision-language model (VLM) review route failures to bounded repair. Each accepted request exports editable PowerPoint (PPTX) and Portable Network Graphics (PNG) artifacts; explicit design controls yield same-paper variants. Across 621 papers, Print-Ready Rate (PRR) counts requests passing geometric, readability, asset-integrity, and obvious-factual-error checks, with native editability reported separately. A frozen VLM assigns conditional Craftsmanship-Harmony-Expressiveness (CHE) scores to print-ready outputs. PosterMELD attains 81.3% PRR, 3.4 times P2P's rate and 5.2 times PosterGen's, and the highest conditional CHE among generated methods with multiple print-ready outputs. Native editability and explicit design controls are retained at a mean cost of USD 0.38 per request, 3.5% of Codex+Skill's. Code and resources are available at .

</details>


### [125] [Open-Set Visual Text Forensics via Sparse-Constraint Rectified Flow](https://arxiv.org/abs/2608.02258)
...
TL;DR: 提出 Sparse-Constraint Rectified Flow (SC-RF)，一种基于 Flow Matching 的生成式视觉文本取证检测器，通过估计局部恢复代价来定位篡改区域，在三个基准上达到 SOTA，并展现出对未见文本编辑模式的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有判别式取证检测器往往过拟合特定伪造模式，难以泛化到未见过的开放集攻击。作者希望构建一种不依赖伪造特定决策边界的生成式检测器，通过估计将查询图像对齐到真实视觉文本统计所需的局部恢复代价来定位篡改。

Method: 提出 SC-RF，一种面向检测的 Flow Matching 适配方法，用于空间稀疏异常定位。通过自监督 Artifact Injection 缓解数据稀缺问题，并使用像素空间的 Forensic-DiT 保留高频取证痕迹。

Result: 在三个基准上达到 SOTA，F1 和 IoU 分别超过第二名 3.2 和 4.8 个百分点；在未见过的文本编辑模式上展现出强零样本性能。

Conclusion: 生成式取证检测器相比判别式方法具有更好的开放集泛化能力；同时模型产生的局部协调可削弱现有检测器依赖的统计线索，为漏洞分析提供了互补视角。

Classification reason: 该论文提出基于 Flow Matching 的生成式取证检测器，用于定位视觉文本篡改区域，属于生成模型在检测/评测场景的应用，映射到多模态生成方向下的"生成模型评测与数据集"规范子主题。

Importance reason: Solid forensic application of flow matching, but tangential to the user's core focus on continuous multimodal models and autoregressive generation. Signals: flow matching, generative forensics, zero-shot generalization, sparse anomaly localization, pixel-space DiT
...
### [126] [Beyond Token-Level Cross-Entropy: Fréchet Distributional Post-Training for Autoregressive Image Generation](https://arxiv.org/abs/2608.00562)
...
TL;DR: 提出 FD-loss 后训练方法，用表征空间的 Fréchet 距离作为唯一目标来适配预训练的自回归图像生成器，通过双通道方案（梯度无关的 rollout 上下文构建 + 概率级 STE 可微重放）解决目标与上下文不匹配问题，在 ImageNet 256×256 上平均降低 FID 41.4%、FD_r6 52.0%。


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成器在 teacher forcing 下用 token 级交叉熵预训练，但评估的是解码图像的分布质量。这造成目标不匹配（分类错误对图像级后果不平等）和上下文不匹配（推理时条件于模型生成的历史而非真实 token）。

Method: 提出 FD-loss 后训练：以表征空间 Fréchet 距离为唯一目标。采用双通道方案：第一通道通过梯度无关生成构建 detached rollout 上下文（模拟模型原生推理配置）；第二通道进行可微重放，使用概率级 straight-through estimator (STE)，前向保持硬 argmax 解码，反向通过温度缩放概率传播图像级梯度。仅更新生成器，tokenizer 和特征提取器保持冻结。

Result: 在四个生成器家族的八个配置上（类条件 ImageNet 256×256），FD-loss 后训练平均降低 FID 41.4%、FD_r6 52.0%。最强 FID 从 2.42 提升到 1.43，且不增加参数或推理步数。

Conclusion: FD-loss 后训练有效弥合了自回归图像生成中训练目标与评估指标之间的差距，通过分布级目标显著提升生成质量，且无需额外参数或推理开销。

Classification reason: 论文针对自回归图像生成器的训练目标进行改进，属于自回归视觉生成范式，映射到 [CANONICAL] 自回归视觉生成 subtopic。

Importance reason: Directly targets autoregressive image generation with a novel distributional post-training objective, strongly matching your priority subtopic and boost keywords. Signals: autoregressive image generation, FID improvement 2.42→1.43, Fréchet distance objective, post-training method, no added parameters
...
### [127] [Detail Continuation over a Trustworthy Coarse Scale for Autoregressive Super-Resolution](https://arxiv.org/abs/2608.01823)
...
### [129] [Where Does Generative Difficulty Reside? An Empirical Study of Target Representations](https://arxiv.org/abs/2608.00626)
...
Importance: 高优先级 (81.2) - 未入选精读

TL;DR: 本文在统一的掩码自回归整流流生成框架下，实证比较了 raw pixels、SD-VAE、DINOv2 与 MAE 表征自编码器四种目标表征的生成行为，发现不同表征在优化速度、推理配置、CFG 响应与精度-召回权衡上存在显著差异，且压缩率、重建保真度、token 维度与语义聚类均无法单独预测生成表现。


<details>
  <summary>Details</summary>
Motivation: 目标表征定义了图像生成器必须学习的分布，但常被视为可互换的接口。这一假设对连续掩码生成器尤其可疑，因为其结合了可见 token 的上下文推理与每个缺失 token 的条件建模。作者希望系统理解不同目标表征如何影响生成难度与行为。

Method: 在统一的掩码自回归整流流（masked autoregressive rectified-flow）模型框架下，对 raw pixels、SD-VAE latents、DINOv2 以及 MAE 表征自编码器特征四种目标表征进行系统比较，在共享 ImageNet 训练预算下考察其优化与推理机制，并分析不同预测、掩码、guidance 配置对各表征的影响。

Result: DINOv2 在迭代与计算上收敛最快，但受益于更宽的局部去噪器与直接上下文融合；pixels 优化显著更慢且需要不同的预测、掩码与 guidance 配置；MAE 重建更忠实且语义聚类清晰，但生成质量明显差于 DINOv2。各表征对 classifier-free guidance 响应不同，占据不同的精度-召回权衡。

Conclusion: 压缩、重建保真度、token 维度与可见语义聚类均不能单独预测生成行为。目标表征将生成难度重新分配在上下文建模、逐 token 去噪与推理时分布控制三个环节上，为选择与设计目标表征提供了实证依据。

Classification reason: 论文系统研究不同目标表征（raw pixels、SD-VAE、DINOv2、MAE RAE）在连续掩码自回归整流流生成模型中的表现，属于连续视觉 tokenizer 与表征自编码器这一规范主题，同时涉及连续潜空间中的生成建模。

Importance reason: Directly informs continuous visual tokenizer design for autoregressive generation, a top-priority subtopic, with rigorous empirical comparisons. Signals: continuous visual token, representation autoencoder, autoregressive generation, flow matching, empirical study
...
### [130] [Scene2Sound: Auditory-Grounded Soundscape Generation for 3D Gaussian Worlds](https://arxiv.org/abs/2608.00463)
...
TL;DR: Scene2Sound 提出为 3DGS 世界生成空间一致音景的任务，通过听觉接地识别场景中应发声的物体并锚定到 3D 位置，利用 VLM 多视角检测与高斯集合匹配实现无训练框架，配合标准音频引擎实时空间化，并引入两种空间一致性评测指标。


<details>
  <summary>Details</summary>
Motivation: 3DGS 能生成可自由探索的逼真 3D 世界，但这些世界是无声的。现有音频生成方法仅以单张图像或单一视角为条件，声音与观察绑定，无法在听众移动时保持一致。因此需要为 3DGS 世界生成空间一致、随视角变化的音景。

Method: Scene2Sound 是一个无训练框架：从输入世界中选择能联合覆盖场景的视角，用视觉语言模型识别发声物体，通过高斯集合匹配（衡量渲染每个检测的高斯集合重叠度）将多视角检测关联为 3D 实例；每个声源生成音频后由标准基于对象的音频引擎在任意听众位姿下实时空间化。另提出两种空间一致性指标：一种测试渲染音频是否随听众运动一致响应，另一种测试声源是否被留出视角支持。

Result: 在生成的 3DGS 世界和真实 360 度采集生成的 3DGS 场景上，Scene2Sound 在保持强单视角基线音频质量的同时实现空间一致性，而单视角和单全景管线无法做到；用户研究确认了感知收益。

Conclusion: Scene2Sound 通过听觉接地与高斯集合匹配，成功为 3DGS 世界生成空间一致、可实时交互的音景，填补了 3D 世界模拟中声音一致性的空白，并提供了相应的评测指标。

Classification reason: 论文提出 Scene2Sound，为 3DGS 世界生成空间一致的音景，属于跨模态条件音频生成，映射到 [CANONICAL] 子主题 audio_av_generation（音频与多模态联合生成）。

Importance reason: Novel task for 3DGS audio grounding, but tangential to the user's core focus on continuous multimodal models and autoregressive generation. Signals: 3D Gaussian Splatting, auditory grounding, spatial audio, training-free VLM pipeline, multimodal generation
...
### [131] [DLLM-TTS: Block Discrete Diffusion Language Model for Text-to-Speech Synthesis](https://arxiv.org/abs/2608.00011)
...
TL;DR: DLLM-TTS 将文本到语音合成建模为基于 X-Codec2 神经音频编解码 token 的条件块离散扩散语言模型，在块内并行预测 token 的同时按块顺序处理，兼顾局部声学一致性与全局文本-语音对齐，以 0.6B 参数和 20K 小时数据在 Seed-TTS-eval 上取得有竞争力的性能，RTF 达 0.15。


<details>
  <summary>Details</summary>
Motivation: 现有 TTS 系统面临权衡：自回归编解码语言模型语音可懂度高但需要大规模模型与数据且逐 token 顺序解码；非自回归方法提升速度却牺牲语言准确性。作者希望兼顾生成质量与并行解码效率。

Method: 将 TTS 形式化为基于 X-Codec2 神经音频编解码 token 的条件块离散扩散。模型将序列分解为块，在块内应用掩码扩散，同时按顺序处理块，从而同时学习局部声学一致性与全局文本-语音对齐。推理时块内并行 token 预测实现高效生成。

Result: 0.6B 参数模型在 20K 小时数据上训练，在 Seed-TTS-eval 基准上取得有竞争力的性能，实时因子（RTF）为 0.15，证明块离散扩散语言模型可实现实用且数据高效的并行语音合成。

Conclusion: 块离散扩散语言模型能够兼顾语音合成的质量与并行生成效率，是一种数据高效且实用的 TTS 方案。

Classification reason: 该论文提出基于块离散扩散语言模型的文本到语音合成（TTS）方法，属于音频生成方向，映射到 [CANONICAL] 子主题 audio_av_generation（音频与多模态联合生成）。

Importance reason: TTS is outside the user's core focus on continuous multimodal models and video/image generation, though the block discrete diffusion approach is a mild methodological signal. Signals: block discrete diffusion, codec language model, parallel decoding, RTF 0.15, data-efficient TTS
...
### [132] [Which Modality Decides? Counterfactual Modality Attribution for Multimodal LLMs](https://arxiv.org/abs/2608.00076)
...
Importance: 值得关注 (69.8) - 未入选精读

TL;DR: 本文提出 Counterfactual Modality Attribution (CMA)，首个量化多模态大模型中模态级贡献的框架。通过耦合扩散先验生成图像-only、文本-only和联合多模态反事实样本，并基于 Shapley 值的合作博弈论公式计算模态归因分数，用于识别哪个模态真正驱动了模型预测。


<details>
  <summary>Details</summary>
Motivation: 现有可解释性方法只能识别有影响力的图像区域或文本 token，无法回答"哪个模态驱动了预测"这一根本问题。模型可能产生正确输出却依赖错误的证据来源，掩盖了捷径学习和不安全的推理，这在安全关键应用中尤其危险。

Method: CMA 框架包含两部分：(1) 使用耦合扩散先验生成图像-only、文本-only和联合多模态反事实样本；(2) 通过基于 Shapley 值的合作博弈论公式将反事实转化为原则性的模态归因分数，量化各模态对预测的贡献。

Result: 在受控合成基准上，CMA 在 98% 的受控案例中正确识别了决策驱动的模态，并持续优于基线方法。在真实世界的多模态临床数据集上，CMA 揭示了仅靠预测准确率无法发现的跨模态推理失败。

Conclusion: 模态归因可作为超越特征归因的可解释性补充维度，为安全关键应用中审计多模态基础模型提供原则性框架，有助于发现捷径学习和不安全推理。

Classification reason: 论文提出模态归因（modality attribution）框架，用于审计多模态大模型在安全关键场景中的推理可靠性，识别模型是否依赖错误的证据来源（shortcut learning）。这与多模态安全与攻防方向相关，属于对多模态模型可靠性与安全性的审计与评测。

Importance reason: Solid explainability contribution for MLLMs, but outside the user's core focus on continuous/multimodal generation and world models. Signals: multimodal understanding, explainability, Shapley values, counterfactual generation, safety-critical
...
### [133] [ReACT-CLIP: Response-Aware Test-Time Defense for Vision--Language Models](https://arxiv.org/abs/2608.01067)
...
TL;DR: ReACT-CLIP 是一种无需训练的响应感知测试时防御方法，通过相对跨噪声漂移和预测不稳定性分数自适应决定每个输入的校正强度与是否需要防御干预，在多种攻击类型和强度下显著提升 CLIP 视觉语言模型的对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关的测试时防御方法其校正强度通常针对狭窄的攻击预算范围固定设置，而推理时攻击预算未知且不同样本所需的校正量不同，导致攻击增强时防御性能急剧下降。

Method: ReACT-CLIP 利用低噪声与高噪声探针之间 CLIP 视觉特征漂移的相对增加作为样本特定的校正需求代理，将其映射到用于构建稳定噪声平均特征锚点的高斯噪声尺度；同时通过 Jensen-Shannon 散度计算预测不稳定性分数，结合相对跨噪声漂移形成防御干预分数，决定是否需要进行防御干预。该方法无需模型或提示训练，校正强度映射只需校准一次。

Result: 在 12 个下游数据集以及 ImageNet 及其分布偏移变体上，ReACT-CLIP 在多种攻击类型和强度下均带来显著的鲁棒性提升，同时基本保持干净精度。

Conclusion: 响应感知的自适应校正能够有效解决固定校正强度与未知攻击预算之间的不匹配问题，为 CLIP 风格视觉语言模型提供了一种实用且无需训练的测试时防御方案。

Classification reason: 论文提出针对 CLIP 风格视觉语言模型的测试时对抗防御方法，属于多模态安全与攻防方向，映射到 [CANONICAL] 子主题 multimodal_safety_attack。

Importance reason: Focuses on adversarial robustness for CLIP, which is outside the user's core interests in continuous multimodal models, autoregressive generation, and world models. Signals: test-time defense, adversarial robustness, CLIP, training-free, no priority keyword match
...
### [134] [Toward Fine-Grained Forgetting:Attribute Unlearning for Multimodal Large Language Models](https://arxiv.org/abs/2608.01008)
...
TL;DR: 本文提出属性级 MLLM 机器遗忘任务，构建覆盖长文本、数值、短文本目标及多种遗忘比例和问题类型的基准，并提出 CLRP（因果定位与保留感知投影）这一轻量级免训练框架，通过激活修补定位因果中介层并移除目标属性子空间，同时保留同身份非敏感证据。


<details>
  <summary>Details</summary>
Motivation: 现有隐私导向的机器遗忘基准主要采用 profile 级删除，但实际请求往往更细粒度：模型应遗忘指定属性而保留同一身份的非敏感信息。目标属性与保留属性共享身份特定和视觉证据，导致选择性遗忘易出现残余泄漏或附带退化，现有方法在此场景下遗忘-保留权衡不稳定。

Method: 提出 CLRP（Causal Localization and Retain-Aware Projection）框架：1) 使用激活修补（activation patching）定位因果中介目标属性泄露的层；2) 应用保留感知投影（retain-aware projection）移除目标属性子空间，同时保留同身份证据。该框架轻量且免训练。

Result: 在多种架构和参数规模的 MLLM 上实验验证了 CLRP 的有效性，相比现有方法在遗忘-保留权衡上表现更稳定。

Conclusion: 属性级遗忘是比 profile 级更细粒度且更贴近实际需求的 MLLM 机器遗忘任务，CLRP 通过因果定位与保留感知投影有效缓解了选择性遗忘中的残余泄漏与附带退化问题。

Classification reason: 论文研究 MLLM 的机器遗忘（machine unlearning），即移除模型记忆的敏感信息，属于多模态安全与隐私防护范畴，映射到 [CANONICAL] 多模态安全与攻防 子主题。

Importance reason: Focuses on MLLM safety/unlearning, which is outside the user's core priorities of continuous models, autoregressive generation, and world models. Signals: MLLM unlearning, attribute-level forgetting, training-free framework, multimodal safety, not aligned with priority subtopics
...
### [135] [No Single Neuron of Failure: Distributed Safety Alignment Against White-Box Attacks](https://arxiv.org/abs/2608.01414)
...
TL;DR: 提出分布式安全对齐（DSA）方法，将安全能力冗余编码到多个计算神经元上，通过方向感知的一阶 Taylor 分数定位关键安全神经元，并结合确定性掩码与随机 dropout 进行定向扰动，迫使模型放弃窄安全神经元、在多个补偿神经元上冗余编码安全行为，从而提升对白盒神经元级安全攻击的鲁棒性，同时保持通用语言与多模态能力。


<details>
  <summary>Details</summary>
Motivation: 随着开源大模型快速发布，安全威胁从黑盒越狱转向神经元级白盒攻击，这类攻击直接识别并操纵与安全相关的神经元。现有对齐方法往往只关注少数神经元上的安全行为，形成脆弱的单点故障，缺乏冗余性。

Method: DSA 方法将干预定位到语言侧前馈网络下投影层的输入，将每个特征坐标视为单个神经元的激活。结合神经元激活与损失梯度计算方向感知的一阶 Taylor 分数，全局识别对当前拒绝行为贡献最大的神经元。最后通过确定性掩码与随机 dropout 的耦合进行定向扰动，迫使模型放弃窄安全神经元，并在多个补偿神经元上冗余编码安全行为。

Result: 大量实验表明 DSA 显著提升了模型对白盒神经元级安全攻击的鲁棒性，同时保持了模型的通用语言与多模态能力。

Conclusion: 分布式安全对齐通过冗余编码安全能力到多个神经元，有效缓解了单点故障问题，为开源大模型应对神经元级白盒安全攻击提供了更稳健的对齐方案。

Classification reason: 论文关注大模型（含多模态模型）的安全对齐与白盒攻击防御，属于多模态安全与攻防范畴，映射到 [CANONICAL] subtopic "multimodal_safety_attack"。

Importance reason: Safety alignment against white-box attacks is valuable but tangential to your core focus on continuous multimodal models, video generation, and world models. Signals: safety alignment, white-box attack, neuron-level robustness, multimodal utility preservation, open-weight LLM security
...
### [136] [FairForensics: Seeing Expressions and Parsing Demographics via Vision-Language Modeling for Generalizable Fair Deepfake Detection](https://arxiv.org/abs/2608.01661)
...
TL;DR: 本文提出 FairForensics，一个基于视觉语言建模的公平深度伪造检测框架，构建人口统计学平衡的 FDD 基准，通过表情感知视觉编码器和人口统计感知语言编码器实现泛化与公平性提升。


<details>
  <summary>Details</summary>
Motivation: 现有公平深度伪造检测器在未见操纵和跨人口统计群体上的泛化与公平性不足，且通常在人口统计不平衡的数据上训练评估，导致对少数群体产生偏见预测。

Method: 构建人口统计学平衡的 FDD 基准；提出 FairForensics 视觉语言模型，包含表情编码器捕捉表情引导的伪造模式、表情感知视觉编码器整合全局外观与表情伪造特征并通过身份感知补丁扰动模块缓解身份偏差；人口统计引导语言编码器提取群体感知全局语言嵌入，通过视觉语言对齐解耦伪造特征与人口统计信息；设计群体感知原型公平目标实现跨子群体的类间可分性与类内对齐。

Result: 在平衡人口统计基准上的大量实验表明，该方法在泛化性和公平性方面达到最先进水平。

Conclusion: 通过表情感知与人口统计感知的视觉语言建模，FairForensics 有效提升了深度伪造检测的泛化能力与跨人口统计群体的公平性。

Classification reason: 该论文研究公平深度伪造检测（deepfake detection），属于多模态安全与防御范畴，映射到 [CANONICAL] 多模态安全与攻防 子主题。

Importance reason: Focuses on deepfake detection fairness, which is outside your core directions and priority subtopics. Signals: vision-language model, deepfake detection, fairness, demographic bias, expression encoder
...
### [137] [Exploring and Bridging Knowledge Holes in Unlearned Multimodal Large Language Models](https://arxiv.org/abs/2608.01849)
...
TL;DR: 本文指出当前 MLLM 机器遗忘评测存在盲区：基准与遗忘集表征距离过远，无法捕捉良性相邻输入上的严重退化（知识空洞）。作者构建了探测知识空洞的基准，并提出 SPAR（Selective Protection with Anchored Regularization）方法，通过锚定激活过滤保护通用模式并结合实体抽象增强，在 SafeEraser 上恢复超过 98% 的响应质量，同时保持 0.00% 攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 机器遗忘用于从 MLLM 中移除不安全内容，但现有评测范式存在关键盲区：评测基准与遗忘集表征距离过远，无法捕捉良性相邻输入上的严重退化（即知识空洞），导致遗忘精度难以保证。

Method: 构建一个能捕捉与遗忘集共享通用模式的良性输入上意外退化的基准；提出 SPAR（Selective Protection with Anchored Regularization），通过锚定激活过滤保护通用模式，并通过实体抽象增强强化这些模式。

Result: 在 SafeEraser 上，SPAR 恢复超过 98% 的原始响应质量（标准基线低于 50%），同时实现 0.00% 攻击成功率和有竞争力的模型效用。

Conclusion: 知识空洞是常用机器遗忘方法的系统性后果，需要更细粒度的评测来保证可信的 MLLM 机器遗忘。

Classification reason: 论文研究 MLLM 机器遗忘（unlearning）中的知识空洞问题，涉及安全内容移除、攻击成功率评估与防御性正则化，属于多模态安全与攻防范畴，映射到 [CANONICAL] 子主题 multimodal_safety_attack。

Importance reason: Focuses on MLLM unlearning safety, which is outside your configured priority directions and subtopics. Signals: MLLM unlearning, knowledge holes, SafeEraser, SPAR, multimodal safety
...
### [138] [Invisible Ink Threats: Adversarial Goals Behind Legitimate Tasks in Computer-Use Agents](https://arxiv.org/abs/2608.02018)
...
TL;DR: 本文提出"隐形墨水威胁"（Invisible Ink Threats）概念，指计算机使用智能体（CUA）中低危害的注入目标（如收藏仓库、安装包）在行为上与合法任务难以区分，从而绕过模型安全机制和人类监督。作者构建了 II-Bench 数据集（444 个示例，覆盖三个平台、三类攻击）和 HITLCUA 对抗测试框架，评估主流 CUA 后发现低危害注入频繁绕过智能体防御和模拟用户审查。


<details>
  <summary>Details</summary>
Motivation: 计算机使用智能体（CUA）日益受到间接提示注入攻击的威胁，现有的人机协同（human-in-the-loop）防御范式主要针对明显高危害攻击，但对行为上与合法任务难以区分的低危害注入目标几乎无防护，存在严重且未被充分探索的安全盲区。

Method: 提出 II-Bench 数据集，包含 444 个看似无害的对抗任务示例，覆盖机密性与完整性攻击，横跨三个平台、三类攻击（页面导航与交互、敏感信息外泄、代码下载与执行），每种攻击以自然语言和代码两种形式、两种指令具体程度实例化。同时构建 HITLCUA 对抗测试框架，集成真实虚拟机操作系统环境与隔离的 Docker 网页平台，并通过 API 模拟用户参与审查。

Result: 对主流 CUA 的广泛评估显示，低危害注入频繁绕过智能体防御和模拟用户审查，暴露出当前 CUA 中严重且此前未被充分探索的安全风险。

Conclusion: 现有的人机协同防御对"隐形墨水威胁"类低危害注入攻击防护不足，需要新的防御机制来识别和阻止行为上与合法任务难以区分的恶意注入目标。

Classification reason: 论文研究计算机使用智能体（CUA）面临的间接提示注入攻击与安全防御，属于多模态/LLM 智能体的安全攻防范畴，映射到 [CANONICAL] 多模态安全与攻防（multimodal_safety_attack）。

Importance reason: Security-focused CUA paper with no direct alignment to your continuous multimodal, autoregressive generation, or world model priorities. Signals: computer-use agents, prompt injection, security, low-harm attacks, II-Bench
...
### [139] [Two Sides of the Same Coin: Co-Evolving Search for Cross-Task Attacks on Vision-Language Models](https://arxiv.org/abs/2608.02137)
...
TL;DR: 提出一种进化计算引导的跨模态对抗攻击框架，通过协同进化文本语义嵌入与视觉区域扰动，实现对统一 VLM 的跨任务对抗攻击，在 Florence-2、OFA、UnifiedIO-2 上验证了图像描述、目标检测、区域分类与目标定位等任务的攻击性能。


<details>
  <summary>Details</summary>
Motivation: 现有针对 VLM 的对抗攻击通常采用单轨迹梯度优化或任务特定目标，限制了搜索空间探索与跨任务迁移性，难以在统一多模态模型上实现一致的跨任务攻击。

Method: 提出进化计算引导的跨模态对抗攻击框架：文本侧进化硬负样本语义嵌入以提供跨模态排斥；视觉侧维护目标区域扰动种群，结合动量梯度更新与进化选择、变异、交叉算子探索多条可行轨迹；联合优化语义负向引导与局部扰动生成对抗样本。

Result: 在 Florence-2、OFA、UnifiedIO-2 上展示了跨图像描述、目标检测、区域分类与目标定位任务的强攻击性能；消融实验验证了文本侧语义进化与图像侧扰动进化的互补有效性，以及框架的效率和跨任务迁移性。

Conclusion: 协同进化搜索能保持扰动可行性、防止最优适应度退化，并相比单轨迹优化提高达到高裕度对抗区域的概率，为统一 VLM 提供了一种高效且可迁移的跨任务对抗攻击方法。

Classification reason: 论文提出针对统一视觉语言模型的跨任务对抗攻击框架，属于多模态对抗攻击与安全研究，映射到 [CANONICAL] 子主题"多模态安全与攻防"。

Importance reason: Focuses on adversarial attacks for VLM security, which is outside your core generation/world-model priorities and lacks direct alignment with your configured subtopics. Signals: adversarial attack, cross-task VLM, evolutionary search, Florence-2/OFA/UnifiedIO-2, security focus
...
### [140] [Decoy Images Amplify Caption-Mediated Defenses Against Encoded Jailbreaks](https://arxiv.org/abs/2608.01043)
...
TL;DR: 论文发现一个反直觉现象：在 VLM 上，将编码越狱提示与无关的诱饵图像配对，可显著降低攻击成功率（最多下降 73pp）。该效应源于防御管线对图像存在性的分支判断，而非图像内容本身。但无条件附加诱饵会大幅提升良性拒绝率，需通过轻量级编码输入检测器进行门控。


<details>
  <summary>Details</summary>
Motivation: 探索图像输入与现有 VLM 黑盒防御之间的交互，发现编码越狱提示与无关诱饵图像配对可显著降低攻击成功率这一反直觉现象，并试图理解其机制与可部署性。

Method: 在五个前沿 VLM、两类编码攻击家族和三种黑盒防御上系统评测，使用 ECSO 等基于字幕的防御，附加空白画布/自然照片等诱饵图像，通过精确 McNemar 检验验证显著性；提出两个假设（字幕防御对图像存在性分支、图像侧内在安全机制），并用三个对照实验（空白/自然图像、开放权重模型无审核层、非符号编码器）约束解释；最后提出基于轻量级编码输入检测器的门控附加策略。

Result: 诱饵图像使 ECSO 防御在文本编码输入上几乎不变的 ASR 最多下降 73pp；空白画布与自然照片均复现该效应；在无审核层的开放权重模型上同样复现；非符号编码器也复现。但无条件附加诱饵使良性拒绝率升至 20-79%（+10 到 +67pp），门控策略可恢复文本基线良性拒绝率并保留安全增益。自适应攻击下效应退化但仍保持。

Conclusion: 该现象是防御管线交互的观察结果，而非稳健的防御方案。诱饵图像通过触发字幕防御的图像存在性分支来降低编码越狱的 ASR，但需门控策略平衡安全性与可用性，检测器召回率是部署的关键约束。

Classification reason: 论文研究 VLM 上的编码越狱攻击与黑盒防御（ECSO）之间的交互，属于多模态安全与攻防范畴，映射到 [CANONICAL] 子主题 multimodal_safety_attack。

Importance reason: Focuses on VLM jailbreak defense interaction, tangential to the user's core generative modeling and continuous representation priorities. Signals: VLM safety, jailbreak defense, caption-mediated defense, decoy image effect, black-box attack analysis
...
### [141] [SIRIN: A Unified Toolkit for Detecting Contextual Hallucinations in Retrieval-Augmented and Memory-Grounded LLM Systems](https://arxiv.org/abs/2608.00033)
...
TL;DR: SIRIN是一个统一的工具包和交互式Web UI，用于检测检索增强、智能体与记忆接地LLM系统中的上下文幻觉，统一了表示探测、不确定性估计和裁判式验证三种检测范式，并支持查询可回答性判断。


<details>
  <summary>Details</summary>
Motivation: 检索增强、智能体与记忆接地LLM系统容易产生流畅但缺乏证据支持的上下文幻觉，现有检测工具分散且缺乏统一接口，难以系统化评估和比较不同检测方法。

Method: SIRIN统一了三种检测器范式（表示探测、不确定性估计、裁判式验证）以及生成前查询可回答性任务，提供统一的配置系统、评估流水线，支持白盒与黑盒设置下的响应级和跨度级检查，并配备交互式Web UI用于实时分析上下文-查询-答案三元组。

Result: 在幻觉检测、查询可回答性以及作为长期记忆系统中的忠实性门控三个任务上进行了演示验证。

Conclusion: SIRIN提供了一个轻量级插件式设计，便于添加新检测器，为上下文幻觉检测提供了统一、可扩展的工具框架。

Classification reason: 论文聚焦于检测LLM系统中的上下文幻觉（contextual hallucination），属于幻觉检测与可靠性提升范畴，映射到[CANONICAL]多模态幻觉与可靠性子主题。

Importance reason: Focuses on hallucination detection tooling rather than core generative modeling or world-model advances, with no direct alignment to priority subtopics. Signals: hallucination detection, retrieval-augmented LLM, uncertainty estimation, judge-style verification, web UI toolkit

Abstract: SIRIN (Semantic Inconsistency Recognition and Inspection Nexus) is a unified toolkit and interactive web UI for detecting contextual hallucinations (fluent, plausible responses unsupported by the provided evidence) in retrieval-augmented, agentic, and memory-grounded LLM systems. SIRIN unifies three detector paradigms (representation probing, uncertainty estimation, and judge-style verification) and the complementary task of pre-generation query answerability under one interface, configuration system, and evaluation pipeline, supporting response- and span-level inspection in both white-box and black-box settings. The web UI enables live analysis of user-supplied context-query-answer triples through hallucination scores, unsupported-span highlighting, and side-by-side detector comparison, with a lightweight plug-in design for adding new detectors. We demonstrate SIRIN on hallucination detection, query answerability, and as a faithfulness gate within long-term memory systems. The source code is publicly available at .

</details>


### [142] [Tracing the Cascade: A Topology-Aware Evaluation Framework for Scientific Agent Hallucinations](https://arxiv.org/abs/2608.00711)
...
TL;DR: 本文探索用人类书写的幻觉样本替代模型生成的幻觉样本，构建了包含1,600个人类样本（覆盖中英法意四种语言）和18,400个来自五个视觉-语言模型的样本的细粒度span级幻觉标注数据集，发现人类样本具有更高标注一致性、更好的内容可控性，且分布上与模型样本相似，可作为模型无关的幻觉评测基准。


<details>
  <summary>Details</summary>
Motivation: 在模型快速迭代的时代，基于特定模型生成的幻觉样本构建的评测基准会随模型更新而过时，难以保持长期有效性。作者希望探索人类书写的幻觉样本能否替代模型生成的幻觉样本，使幻觉检测评测不依赖于特定模型，从而更具持久性。

Method: 构建了1,600个人类书写的幻觉样本，覆盖中文、英文、法语、意大利语四种语言，同时收集了来自五个视觉-语言模型的18,400个样本，所有样本均采用细粒度的span级标注方案进行幻觉标注。通过对比人类样本与模型样本在标注一致性、内容可控性、分布相似性以及检测能力刻画等方面的表现来评估人类样本作为评测基准的可行性。

Result: 人类书写的样本在标注一致性上更高，允许对数据集内容进行更大程度的控制，同时与视觉-语言模型生成的样本在分布上保持相似，并能合理刻画模型的检测能力，表明人类数据是模型幻觉评测基准的可行替代方案。

Conclusion: 人类书写的幻觉样本可以作为模型无关的幻觉评测基准的有效替代，使幻觉检测评测更具持久性和可复现性，不受特定模型迭代的影响。

Classification reason: 论文聚焦于视觉-语言模型幻觉的细粒度评测基准构建，属于多模态幻觉与可靠性方向，直接映射到 [CANONICAL] 子主题 multimodal_hallucination。

Importance reason: Focuses on hallucination benchmarking methodology rather than core generation or world-model advances, with limited alignment to your priority subtopics. Signals: hallucination benchmark, human-written samples, span-level annotation, multilingual, model-agnostic evaluation
...
### [144] [Grounding Agentic VLMs with Dedicated Segmentation for Fine-Grained Vehicle Damage Assessment](https://arxiv.org/abs/2608.02470)
...
TL;DR: 本文提出 TinyDamage，一种混合架构，将空间定位委托给专用多任务分割模型，而 VLM 仅负责语义推理与报告生成，用于细粒度车辆损伤评估。通过监督对比损失改进微小损伤分割，并集成到 7 节点 LangGraph 智能体管线中，将报告幻觉率从 92%（纯文本）和 78%（纯图像）降至 31%。


<details>
  <summary>Details</summary>
Motivation: VLM 作为推理智能体在真实视觉评估管线中日益普及，但其空间定位对细粒度、视觉模糊的目标（如划痕、发丝裂纹）不可靠，容易在反光区域产生幻觉、漏检细长划痕，且定位输出空间不一致。作者希望解决 VLM 在细粒度损伤评估中的空间接地问题。

Method: 提出 TinyDamage 混合架构：将空间接地委托给专用多任务分割模型，VLM 仅负责语义推理与报告生成。分割侧研究发现损失函数选择对微小目标接地影响巨大——focal loss 使微小损伤检测崩溃为零，而监督对比损失可改善损伤/背景可分性。将分割模型集成到 7 节点 LangGraph 智能体管线中，使每个 VLM 生成步骤都基于分割输出接地。并引入 DET_l 逐类别检测指标用于类不平衡下的微小目标接地评测。

Result: Qwen-VL 在语义分类上达到 87.3% 准确率但空间层面系统性未接地；TinyDamage 将报告幻觉率从 92%（纯文本）和 78%（纯图像）降至 31%，在 100 份人工验证报告上评估，并报告了部署管线的延迟与可靠性特征。

Conclusion: 将空间接地从 VLM 中分离出来、交给专用分割模型，是提升细粒度视觉评估可靠性的有效途径；损失函数选择对微小目标分割有被低估的重大影响，监督对比损失优于 focal loss。

Classification reason: 论文核心关注 VLM 在细粒度车辆损伤评估中的空间定位不可靠与幻觉问题，通过专用分割模型接地来降低报告幻觉率，属于多模态幻觉与可靠性方向，映射到 [CANONICAL] multimodal_hallucination。

Importance reason: Focuses on vehicle damage assessment with a hybrid segmentation-VLM pipeline, which is only tangentially related to your core interests in continuous multimodal models and video generation. Signals: VLM grounding, hallucination reduction, segmentation, LangGraph agent, vehicle damage
...
### [145] [Credit the Right Box: Marginal Contribution Assignment for Structured Visual Perception](https://arxiv.org/abs/2608.01055)
...
TL;DR: 提出 MCR-GRPO，一种边际贡献分配框架，通过 leave-one-out 比较为每个预测框分配 box 级信用，解决 MLLM 结构化多目标感知中 GRPO 响应级监督与 token 级贡献的粒度不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有 group-relative RL 方法（如 GRPO）只提供响应级监督，将单一 advantage 广播到所有 token，无法区分结构化多目标预测中每个框的独立贡献，导致粒度不匹配。

Method: 提出 MCR-GRPO：通过 Marginal Contribution Reward (MCR) 用 leave-one-out 比较估计每个预测框的贡献，结合 Continuous Matched Set Value Evaluator（含置换不变匹配、计数感知归一化、分级定位），将归一化的 box 级边际优势映射到生成该框的 token 跨度上。

Result: 在 REC、DOD、分割和计数基准上取得优于先前 GRPO 基线的最先进性能。

Conclusion: MCR-GRPO 通过 box 级边际贡献分配有效解决了结构化多目标感知中的粒度不匹配问题，在多个基准上验证了其有效性。

Classification reason: 论文提出 MCR-GRPO，一种基于 GRPO 的强化学习方法，为结构化视觉感知任务分配 box 级边际贡献奖励，属于多模态强化学习与对齐方向，映射到 [CANONICAL] 子主题 multimodal_rl_alignment。

Importance reason: Solid RL credit-assignment method for structured perception, but outside the user's core continuous-model, generation, and world-model priorities. Signals: MLLM grounding, GRPO improvement, box-level credit, structured perception, marginal contribution
...
### [146] [Distill What the Student Can See: Fisher-Projected On-Policy Distillation for Vision-Language Models](https://arxiv.org/abs/2608.01263)
...
TL;DR: 提出 Fisher-Projected On-Policy Distillation (FP-OPD)，通过连续视觉扰动估计学生模型的局部视觉切空间，并在学生 Fisher 度量下投影教师-学生对数概率差，仅蒸馏学生可实现的教师修正，从而提升紧凑视觉语言模型的蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 现有 on-policy 蒸馏（OPD）假设完整教师分布是适合所有学生容量的目标，但在视觉语言推理中，教师修正可能依赖紧凑学生无法表征的视觉区分。目标缩放研究表明，当目标逼近完整教师分布时，学生实现的修正更少且下游性能更差，因此需要容量感知的蒸馏目标。

Method: FP-OPD 使用连续视觉扰动估计学生模型的局部视觉切空间，将中心化的教师-学生对数概率差投影到该空间（在学生的 Fisher 度量下），得到容量感知的蒸馏目标，并在学生轨迹上以全词表反向 KL 优化，保留标准 OPD 框架。

Result: 在 8B 到 2B 蒸馏中，FP-OPD 在全部七个多模态基准上均有提升，平均分比预训练学生高 2.77 分，比标准 OPD 高 1.60 分。

Conclusion: 局部可实现的教师修正为蒸馏紧凑视觉语言模型提供了更有效的目标，验证了容量感知蒸馏的有效性。

Classification reason: 论文提出面向视觉语言模型的 Fisher 投影 on-policy 蒸馏方法，属于多模态模型的对齐与蒸馏优化，映射到 [CANONICAL] 多模态强化学习与对齐（multimodal_rl_alignment）子主题。

Importance reason: Solid distillation method for compact VLMs, but tangential to the user's core continuous-model and generation priorities. Signals: on-policy distillation, vision-language models, Fisher metric, capacity-aware target, 8B-to-2B distillation
...
### [147] [Instruction-Conditioned Exploration with Asymmetric Reinforcement Learning and Self-Distillation](https://arxiv.org/abs/2608.02087)
...
TL;DR: 提出 Instruction-Conditioned Exploration (ICE) 方法，在训练时用多种指令补充任务提示以增加行为覆盖，并结合 Asymmetric-RL/SD（强化学习+自蒸馏）目标将探索行为迁移到无条件的测试时策略，在数学推理任务上相对 DAPO 提升 5.0% pass@1。


<details>
  <summary>Details</summary>
Motivation: LLM 的 action-space 结构与经典 RL 不同，给探索诱导带来独特挑战。需要利用预训练 LLM 的广泛知识与灵活性，在训练时主动生成多样化经验，以提升后训练效果。

Method: 提出 ICE（Instruction-Conditioned Exploration），在训练时用若干不同指令补充任务提示以扩大行为覆盖；并提出 Asymmetric-RL/SD，将强化学习与自蒸馏目标结合，把探索到的行为迁移到无条件的测试时策略。

Result: 在 Qwen3-1.7B 上，数学推理任务在 4K 响应长度下 held-out pass@1 相对 DAPO 提升 5.0%，且在更长的 8K 上下文下改进仍然保持。

Conclusion: ICE 结合 Asymmetric-RL/SD 能有效提升 LLM 后训练中的探索覆盖与最终推理性能，且改进在更长上下文下依然稳健。

Classification reason: 论文提出基于强化学习（RL）的 LLM 后训练方法（ICE + Asymmetric-RL/SD），属于 RL 训练与对齐范畴，映射到 [CANONICAL] 多模态强化学习与对齐（multimodal_rl_alignment）。

Importance reason: RL exploration method for LLM math reasoning, but no direct connection to the user's continuous multimodal, video generation, or world model priorities. Signals: RL post-training, exploration via instruction conditioning, self-distillation, math reasoning, no multimodal/video/world-model focus
...
### [148] [Same Semantics, Different Paths: Self-Improving Alignment for Vision-Text Compression](https://arxiv.org/abs/2608.02109)
...
TL;DR: 本文提出 SPIRAL，一种自监督对齐框架，用于解决视觉文本压缩（VTC）中渲染图像表征与原生文本表征之间的跨路径不一致问题，通过 token 级蒸馏和序列级 DPO 将视觉表征对齐回文本语义，在 VTCBench 上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 视觉文本压缩（VTC）将长文本渲染为图像并通过 ViT 编码，将数千文本 token 压缩为少量视觉 token。然而 ViT 预训练于自然图像，捕获的是字形、字号、布局等视觉属性而非语言语义，导致渲染图像表征与原生文本表征产生分歧（即跨路径不一致），这是 VTC 的关键瓶颈。

Method: SPIRAL 采用自监督对齐框架，仅利用模型自身文本路径行为作为监督，无需外部教师或额外标注。它在两个互补粒度上操作：token 级 on-policy 蒸馏（OPD）保证局部忠实性，序列级偏好优化（DPO）保证全局连贯性。

Result: 在 VTCBench 上，SPIRAL 将 Qwen3-VL-8B 的整体分数从 35.10 提升到 54.02，接近原生文本输入性能（55.60），并超越大 30 倍的模型。OPD 在检索上表现优异且样本高效，DPO 在推理和记忆上更强且随数据规模扩展更好。SPIRAL 的收益也泛化到域外基准。

Conclusion: 有效的 VTC 关键在于将渲染图像表征对齐回原生文本语义。SPIRAL 通过自监督的双粒度对齐框架成功弥合了跨路径不一致，验证了该瓶颈的重要性，并展示了无需外部监督即可实现接近原生文本性能的可行性。

Classification reason: 论文提出 SPIRAL 框架，通过 token 级蒸馏（OPD）和序列级偏好优化（DPO）将渲染图像表征对齐回原生文本语义，属于多模态表征对齐与偏好优化范畴，映射到 [CANONICAL] 多模态强化学习与对齐。

Importance reason: Novel self-supervised alignment for vision-text compression, but tangential to the user's core continuous/multimodal generation and world-model focus. Signals: vision-text compression, self-supervised alignment, token-level distillation, sequence-level DPO, cross-path inconsistency
...
### [149] [Local Margin Restoration for Test-Time Adaptation of Vision-Language Models](https://arxiv.org/abs/2608.02216)
...
TL;DR: 提出 LMR（Local Margin Restoration），一种轻量级单步测试时自适应框架，通过 Protected Margin Restoration 目标恢复局部语义几何，并引入 Adaptive Margin 控制器与 Bias Correction 双阶段稳定机制，防止模式坍缩，在 CIFAR-C、ImageNet-C 等基准上超越 SOTA TTA 方法。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（如 CLIP）在测试时遇到分布偏移时性能会急剧下降。虽然测试时自适应（TTA）是解决方案，但持续在无标签测试流上自适应存在根本性挑战：传统的 top-1 中心更新会通过破坏相关类别间的局部语义几何来强化错误，而迭代自适应会加剧渐进式偏差累积，最终导致模式坍缩。

Method: 提出 LMR 框架，包含两个核心组件：(1) 样本层面的 Protected Margin Restoration (PMR) 目标，通过屏蔽接近 top 的候选类别免受外部硬负样本干扰，恢复局部语义几何；(2) 流层面的双阶段稳定机制，包括 Adaptive Margin (AM) 控制器和 Bias Correction (BC)，动态破坏渐进式偏差累积并防止模式坍缩。

Result: 在 CIFAR-C、ImageNet-C 和 ImageNet 变体上进行了大量实验，LMR 一致优于 SOTA TTA 基线，即使在具有挑战性的低批次测试时场景中也表现出极强的鲁棒性和效率。

Conclusion: LMR 通过同时解决样本级语义几何破坏和流级偏差累积两个耦合问题，为 VLM 的测试时自适应提供了轻量且鲁棒的解决方案，在低批次等困难场景下仍保持优异性能。

Classification reason: 论文研究视觉语言模型（CLIP）在测试时分布偏移下的自适应（Test-Time Adaptation），属于多模态模型的对齐与自适应优化范畴，映射到 [CANONICAL] 多模态强化学习与对齐 子主题。

Importance reason: Solid TTA method for CLIP robustness, but it is outside your core continuous/multimodal generation and world-model focus. Signals: test-time adaptation, vision-language models, distribution shift, CLIP, robustness

Abstract: Vision-language models (VLMs) such as CLIP exhibit remarkable zero-shot capabilities, yet their performance frequently degrades sharply under unexpected test-time distribution shifts. While Test-Time Adaptation (TTA) offers a promising solution, continuously adapting VLMs over an unlabeled test stream presents fundamental challenges. Conventional top-1-centric updates often reinforce errors by corrupting the local semantic geometry among related classes, while iterative adaptation exacerbates progressive bias accumulation, ultimately driving the model toward mode collapse. To overcome these coupled vulnerabilities, we propose Local Margin Restoration (LMR), a lightweight, one-step TTA framework. At the sample level, our Protected Margin Restoration (PMR) objective recovers local semantic geometry by shielding plausible near-top candidates from external hard negatives. Concurrently, to combat stream-level degradation, we introduce a dual-stage stabilization mechanism, featuring an Adaptive Margin (AM) controller and Bias Correction (BC), to dynamically disrupt progressive bias accumulation and prevent mode collapse. Extensive experiments on CIFAR-C, ImageNet-C, and ImageNet variants demonstrate that LMR consistently outperforms state-of-the-art TTA baselines, proving exceptionally robust and efficient even in challenging low-batch test-time regimes. Our code is available at .
...
### [150] [Cross-Task Dissociation in Frontier Vision-Language Model Theory of Mind](https://arxiv.org/abs/2608.00261)
*Kejia Zhang, Youran Sun, Chugang Yi, Haizhao Yang*

Main category: cs.CL, cs.MA, cs.CV, cs.AI, q-bio.NC

Direction: 多模态理解与生成

Subtopic: 多模态推理与思维链

Importance: 普通相关 (48.43) - 未入选精读

TL;DR: 本文评估九个前沿视觉语言模型在两个心理学基准（Keysar 导演任务与 Frith-Happé 动画三角形）上的心理理论（ToM）表现，发现模型在跨任务间呈现不一致的 ToM 画像：在导演任务上像儿童一样犯自我中心错误，在三角形任务上则更接近高功能自闭症成人而非典型发育成人，没有任何模型在两个任务上都最接近典型发育成人。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常用单一任务评估 VLM 的心理理论能力，但人类 ToM 在不同范式下表现并不一致。作者希望探究前沿 VLM 是否呈现跨任务一致的 ToM 画像，还是像人类亚群体一样在不同范式间出现分化。

Method: 选取九个前沿 VLM，在两个心理学基准上评估：Keysar 导演任务（视觉视角采择，受自我中心干扰）和 Frith-Happé 动画三角形（基于纯运动线索的意图归因，用 Castelli 评分体系）。对比模型表现与人类参考组（儿童、典型发育成人、高功能自闭症成人）的分布距离。

Result: 在导演任务上，无思维链时模型在 78% 的试次中犯自我中心错误（类似儿童而非成人），且模型间差异显著，推理能挽救部分模型。在三角形任务上，模型整体低估意图归因，其 ToM 画像距离高功能自闭症成人均值比典型发育成人均值近三倍以上。没有任何模型在两个任务上都最接近典型发育成人。

Conclusion: 前沿 VLM 的 ToM 能力呈现跨任务解离（cross-task dissociation），而非一致画像。模型在视觉视角采择与运动意图归因上表现出不同的能力模式，提示当前 VLM 的 ToM 能力是碎片化的，需要更全面的多任务评估框架。

Classification reason: 该论文评估前沿视觉语言模型在心理理论（Theory of Mind）任务上的表现，涉及视觉视角采择与意图归因推理，属于多模态视觉推理范畴，映射到 [CANONICAL] 多模态推理与思维链（multimodal_reasoning_cot）。

Importance reason: Evaluates VLM theory-of-mind consistency but falls outside your core continuous-multimodal, generation, and world-model priorities. Signals: theory of mind, vision-language model, cross-task dissociation, psychology benchmark, no generation focus
...
### [151] [DiffuseAgent-MI: Distributionally-Grounded,Tool-Integrated Self-Evolving Agents for Faithful Visual Reasoning](https://arxiv.org/abs/2608.00540)
...
TL;DR: DiffuseAgent-MI 是一个自进化视觉推理智能体，通过 KL 最小能量模型约束感知grounding，结合验证器提供轨迹级忠实性奖励与修复分支，在 GeoQA、SciVis、VQA-v2 等基准上提升准确率并显著增强推理忠实性。


<details>
  <summary>Details</summary>
Motivation: 工具集成的视觉语言智能体在组合式多步视觉推理上表现优异，但其输出常存在不忠实问题：陈述的推理路径与实际产生答案的计算过程不一致，在安全关键应用中不可靠。

Method: 提出 DiffuseAgent-MI，一个自进化智能体。其感知grounding由 KL 最小能量模型约束，对特征单元提供分布层面的视觉机制可解释性。智能体学习能量景观，软性约束生成样本靠近以所选可解释单元为条件的原生先验，弥合解释与内部表征的差距。验证器提供轨迹级忠实性奖励，修复分支在验证器标记不忠实步骤时重新调节能量。

Result: 在 GeoQA、SciVis、VQA-v2 及内部多模态推理集上，相比先前自进化智能体准确率最高提升 5.1 个点，互信息忠实性与人类可解释性一致性提升超过一倍。

Conclusion: 能量项与验证器互补：前者保证分布级忠实性，后者保证轨迹级忠实性，只有二者结合才能同时弥合两类差距。

Classification reason: 论文聚焦于工具集成的视觉语言智能体在组合式、多步视觉推理中的忠实性问题，属于多模态推理范畴，映射到 [CANONICAL] 子主题"多模态推理与思维链"。

Importance reason: Solid but niche visual-reasoning-agent paper; only tangential to your core continuous-model/video-generation priorities. Signals: visual reasoning agent, faithfulness via energy model, self-evolving, trajectory-level verifier, GeoQA/SciVis/VQA-v2
...
TL;DR: SymboUQ 是一个符号化不确定性量化框架，通过区分"可符号化性"与"语义确定性"来估计 LLM 空间推理最终答案的可靠性，包含布局审计器、无标签确定性画像和确定性感知可靠性组合器三个组件，在五个空间推理基准上相对最强基线提升约 8% AUROC。


<details>
  <summary>Details</summary>
Motivation: LLM 能生成流畅的空间推理轨迹，但中间关系可能无法支撑最终结论，token 级置信度不足以估计最终答案的可靠性。现有形式化验证器虽能提供更强的语义证据，但适用性有限：解析出的声明不一定能产生确定的语义判定。

Method: SymboUQ 框架包含三个组件：(i) 布局审计器（Layout Auditor），执行有序空间声明并提取可行性、冲突与修复证据；(ii) 无标签确定性画像（Determinacy Profile），刻画有效可执行覆盖率；(iii) 确定性感知可靠性组合器（Determinacy-Aware Reliability Composer），根据验证器适用性整合基于约束、表征和解码的分数。

Result: 在五个空间推理基准、四个冻结 LLM 骨干上，SymboUQ 相对最强基线在 AUROC 上提升约 8%，在类平衡 Brier 损失上降低约 7%。

Conclusion: 通过区分可符号化性与语义确定性，SymboUQ 能更可靠地估计 LLM 空间推理最终答案的置信度，弥补了 token 级置信度与形式化验证器之间的适用性缺口。

Classification reason: 论文研究 LLM 在空间推理任务中的不确定性量化与可靠性估计，属于空间/视觉推理范畴，映射到 [CANONICAL] 多模态推理与思维链（multimodal_reasoning_cot）子主题。

Importance reason: Focuses on uncertainty quantification for spatial reasoning, which is tangential to the user's core interests in continuous multimodal models and video/image generation. Signals: uncertainty quantification, spatial reasoning, LLM reliability, AUROC improvement, not in priority subtopics
...
### [153] [Does Accuracy Equal Evidence? Reasoning Faithfulness under KV Cache Compression](https://arxiv.org/abs/2608.01631)
...
TL;DR: 本文研究 KV 缓存压缩对大型推理模型推理忠实性的影响，发现压缩可能保持最终答案正确率的同时显著破坏推理链支持与扰动忠实性，即"答案-证据差距"（answer-evidence gap）。通过固定轨迹重放协议隔离推理内容，评估十种 token 淘汰压缩方法与一种量化方法，发现覆盖保持的量化方法受影响较小，表明失败主要源于丢失推理轨迹部分访问而非 KV 内存缩减本身。


<details>
  <summary>Details</summary>
Motivation: KV 缓存压缩通常以最终答案准确率作为评估标准，隐含假设"保留答案即保留支撑推理"。本文检验这一假设，发现对大型推理模型该假设可能失效：压缩下正确答案与可见支撑推理的有效性可能以不同速率被保留。

Method: 提出固定轨迹重放协议（fixed-trace replay protocol），固定推理内容并隔离压缩是否保留已有轨迹中的可用信息。在三个模型上评估十种 token 淘汰 KV 压缩方法与一种量化方法，覆盖数学推理、科学问答、临床计算与长上下文检索任务，测量最终准确率、答案链一致性与扰动忠实性。

Result: 跨任务中，token 淘汰方法可在保持有竞争力的最终答案准确率的同时显著降低链支持或扰动忠实性，即出现答案-证据差距。覆盖保持的量化控制方法受影响明显较小，表明失败更多与丢失推理轨迹部分访问相关，而非 KV 内存缩减本身。

Conclusion: KV 缓存压缩的评估不应仅依赖最终答案准确率，还需考虑推理忠实性。答案-证据差距表明压缩方法可能牺牲推理链的有效性，提示未来压缩方法应关注保留推理轨迹的完整访问。

Classification reason: 论文研究大型推理模型在 KV 缓存压缩下的推理忠实性（reasoning faithfulness），关注推理链/思维链在压缩后是否仍保持有效，属于多模态推理与思维链范畴，映射到 [CANONICAL] multimodal_reasoning_cot。

Importance reason: Focuses on KV cache compression faithfulness, which is tangential to the user's core interests in continuous multimodal models and video generation. Signals: KV cache compression, reasoning faithfulness, answer-evidence gap, token-eviction, quantization

Abstract: KV cache compression is commonly evaluated by final-answer accuracy, implicitly assuming that preserving the answer also preserves the reasoning that supports it. We test this assumption for large reasoning models and show that it can fail: under compression, correct answers and the validity of their visible supporting rationales can be preserved at different rates. We study this failure with a controlled fixed-trace replay protocol, which holds reasoning content fixed and isolates whether compression preserves usable information from an already available trace. We evaluate ten token-eviction KV compression methods and one quantization method on three models across mathematical reasoning, scientific QA, clinical calculation, and long-context retrieval. We measure final accuracy, answer-chain consistency, and perturbation faithfulness. Across tasks, token-eviction methods can preserve competitive final-answer accuracy while substantially degrading chain support or perturbation faithfulness. We call this the answer-evidence gap. A coverage-preserving quantization control is substantially less affected, suggesting that the failure is tied less to KV memory reduction itself than to losing access to parts of the reasoning trace. Code is available at .

</details>


### [154] [TRAM: Enhancing Multimodal Reasoning with Trajectory-Derived Auxiliary Memory](https://arxiv.org/abs/2608.01922)
...
TL;DR: TRAM 是一种无需训练的推理增强方法，通过从模型自身推理轨迹中提取辅助记忆（latent memory），以快慢双循环流在线更新并经由轻量残差路径回馈到解码层，从而在多模态长推理中保持推理派生信息，提升数学、科学和通用视觉推理性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大推理模型在长推理轨迹中会逐渐弱化对早期建立信息的利用，导致推理错误。现有方法主要依赖持续视觉接地，但推理还会将视觉观察转化为任务相关的约束、关系与中间结论，这些推理派生信息在长轨迹中的影响力同样会衰减。归因分析表明正确性更与轨迹是否保留并整合这类推理派生信息相关。

Method: TRAM 将已完成的推理压缩为紧凑的潜记忆，通过快慢两个循环流在线更新该记忆，并经由轻量残差路径将其回馈到选定的解码器层，从而在不额外训练的情况下增强标准解码过程。

Result: 在四个 MLRM 变体、八个基准上的实验表明，TRAM 相比 vanilla 解码在数学、科学和通用视觉推理任务上均取得性能提升，且无需额外训练。

Conclusion: TRAM 验证了推理派生信息的保持与整合对多模态长推理正确性的关键作用，提供了一种训练无关、即插即用的推理增强途径，为多模态推理模型的长轨迹推理改进提供了新思路。

Classification reason: 论文聚焦多模态大推理模型（MLRM）在长推理轨迹中的信息保持与推理增强，属于多模态推理与思维链方向，映射到 CANONICAL 子主题 multimodal_reasoning_cot。

Importance reason: Training-free multimodal reasoning memory enhancement is useful but tangential to the user's core focus on continuous latent models and autoregressive/diffusion generation. Signals: multimodal reasoning, training-free, auxiliary memory, fast-slow recurrent streams, MLRM
...
### [155] [It's the Decoding Format, Not the Perturbation: Auditing Consistency-Based Selection for Vision-Language Test-Time Scaling](https://arxiv.org/abs/2608.01207)
...
TL;DR: 本文研究视觉语言模型测试时扩展中的候选答案选择方法。作者提出扰动接地选择（Pgs）规则，通过输入扰动下模型能否重新推导候选答案来评分，并引入格式匹配对照（MatchedCtrl）以控制解码格式与预算。实验表明，在控制格式后 Pgs 相比多数投票并无显著增益，扰动一致性至多是视觉依赖的部分诊断信号，不能作为独立的选择信号。


<details>
  <summary>Details</summary>
Motivation: 测试时扩展通过采样多个候选答案并选择来提升大语言模型推理能力，但该策略在视觉语言模型（VLM）上迁移效果不佳：多数投票往往胜过基于模型自验证的选择方法，因为图像接地答案与语言先验的自信猜测在选择层难以区分。作者希望找到一种必须依赖图像才能计算的选择信号，即扰动接地选择（Pgs）。

Method: 提出 Perturbation Grounded Selection (Pgs)：一种无标签、免训练的选择规则，通过标签保持扰动（裁剪、背景掩码、轻度光度/几何抖动）下模型能否重新推导候选答案来评分；扰动集为空时退化为多数投票。同时引入格式匹配对照（MatchedCtrl），使用与原始图像相同的短、无 CoT 采样预算，以隔离解码格式与预算的影响。

Result: 在 TextVQA、MATH-Vision、MMMU 和 ViLP 上，使用 Qwen（三种子均值）和 LLaVA-OneVision 进行实验。Pgs 在 TextVQA 上比纯多数投票最高提升 +31.8 分，但 MatchedCtrl 在每个基准上都在噪声范围内追平或超过 Pgs（包括需要视觉的 ViLP）；Qwen 类别中无显著增益。稳定性差距真实存在且依赖图像（最高 +0.48），但不能预测逐实例胜负。

Conclusion: 结论为负向且具诊断性：扰动一致性至多是视觉依赖的部分诊断信号，在控制格式后不能作为独立可用的选择信号；针对仅 CoT 多数投票报告的增益高估了此类方法。

Classification reason: 论文研究视觉语言模型（VLM）的测试时扩展（test-time scaling）与候选答案选择机制，属于多模态推理与推理时计算扩展范畴，映射到 [CANONICAL] 子主题 multimodal_reasoning_cot。

Importance reason: Negative result on VLM test-time scaling selection, but tangential to your core continuous/multimodal generation and world-model priorities. Signals: test-time scaling, vision-language model, consistency-based selection, negative result, perturbation grounding
...
### [156] [MoRAL: Sensor-Grounded BEV Reasoning for Compact VLMs toward Edge-Oriented Autonomous Driving](https://arxiv.org/abs/2608.02449)
...
TL;DR: MoRAL 提出两阶段微调流水线，将物理编码的 BEV 表征（LiDAR 距离色带、雷达多普勒速度楔形等）作为输入，训练紧凑的 Cosmos-Reason2-2B VLM 进行自动驾驶空间推理与驾驶决策，在 8GB 消费级 GPU 上以 42 tok/s 运行，显著提升紧急制动召回率并降低输出退化。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的自动驾驶边缘平台上部署 VLM 进行安全关键的空间推理，既需要紧凑的模型规模，又需要可靠的度量空间接地（metric grounding）。现有 VLM 缺乏对物理距离、速度等度量信息的可靠感知，且大模型难以在边缘设备上运行。

Method: 两阶段微调流水线：阶段一在 60,000 条接地记录上微调视觉编码器，使其学会解析物理编码的 BEV 表征（LiDAR 距离色带、物体类别簇形态、雷达多普勒速度楔形覆盖）；阶段二用 Cosmos-Reason2-8B 作为教师生成 57,696 条链式思维记录，微调全模型（仅 52M 参数，占总参数量 2.4%），覆盖八类驾驶问题。

Result: 在 2,304 帧 nuScenes 测试集上，MoRAL 在八类问题中七类优于零样本 8B 基线（参数少四倍）；紧急制动召回率从 10.8% 提升至 47.8%，输出退化从 94.1% 降至 20.8%，全流水线在 8GB 消费级 GPU 上以 42 tok/s 运行且无需量化。

Conclusion: MoRAL 证明通过将空间感知外部化到输入 BEV 图像中，紧凑 VLM 无需学习 3D 骨干网络即可实现可靠的物理接地推理，为移动边缘平台上的紧凑、物理接地 VLM 推理建立了可复现的基础。

Classification reason: 论文聚焦于紧凑 VLM 在自动驾驶场景下的 BEV 空间推理与链式思维（chain-of-thought）微调，属于多模态推理与思维链方向，映射到 [CANONICAL] multimodal_reasoning_cot。同时涉及自动驾驶/具身场景的空间推理，可关联 world_model 方向的 embodied_world_model。

Importance reason: Compact edge VLM with physics-grounded BEV reasoning is solid engineering, but it is not a frontier method for the user's core continuous/multimodal generation or world-model directions. Signals: compact VLM, BEV reasoning, autonomous driving, edge deployment, chain-of-thought distillation
...
### [157] [Question Begets Question: Self-Evolving Curriculum for Reinforcement Fine-Tuning on Competition Mathematics](https://arxiv.org/abs/2608.01522)
...
TL;DR: 提出 Question-begets-Question (QbQ) 自进化课程框架，通过教师模型将已有问题转化为多样变体，并仅用强化学习（问题+最终答案，无推理轨迹）训练，突破静态数据增强的性能天花板，将 Qwen2.5-Math-7B 在 AIME 上的 pass@1 从 5.6% 提升至 16.5%。


<details>
  <summary>Details</summary>
Motivation: 语言模型学习未掌握技能时面临三大困难：训练数据稀缺、缺乏真实推理轨迹、模型存在数据增加也无法突破的性能天花板。论文在竞赛数学（AIME）这一受控场景下研究这些问题。

Method: 提出 QbQ 自进化课程：每轮评估当前 checkpoint，从模型能大部分做对的问题中播种 QbQ 生成变体，仅用问题陈述和最终答案进行强化学习训练（不使用教师推理轨迹），在相同数据预算下迭代训练。

Result: 静态训练（真实+合成增强、非课程 QbQ）分别封顶在 pass@1 12.5% 和 14.5%；自进化课程在相同数据预算下突破天花板，将 pass@1 提升至 16.5%，20 轮后仍无饱和迹象。模型在训练于其能大部分做对的问题变体时表现更好，并能解决训练中从未见过的更难问题。

Conclusion: 性能天花板并非模型固有，通过自进化课程（从模型能大部分做对的问题播种变体）可以突破静态数据增强的局限。反直觉地，训练于模型已能大部分做对的问题变体反而能提升其解决更难问题的能力。

Classification reason: 论文聚焦于竞赛数学（AIME）上的强化微调与课程学习，属于数学推理与推理能力提升范畴，映射到 [CANONICAL] 多模态推理与思维链（multimodal_reasoning_cot），该子主题涵盖数学推理与推理时扩展。

Importance reason: Solid RL curriculum method for math reasoning, but outside the user's core continuous-multimodal and video-generation focus. Signals: self-evolving curriculum, reinforcement fine-tuning, competition math, data augmentation, reasoning ceiling
...
### [158] [Remember-R1: Mitigating Long-Context Visual Forgetting through Reinforcement Learning](https://arxiv.org/abs/2608.01314)
...
TL;DR: Remember-R1 提出一种强化学习框架，通过过程级奖励监督原始推理轨迹，鼓励更广的视觉关键词覆盖、后期推理步骤中更强的视觉依赖保持以及对问题相关图像区域的聚焦，从而缓解 MLLM 长上下文推理中的视觉遗忘问题。


<details>
  <summary>Details</summary>
Motivation: MLLM 在长思维链推理中会逐渐减少对视觉证据的依赖，转而依赖累积的文本上下文，导致视觉遗忘。现有方法没有直接约束视觉证据在原始推理轨迹中的使用与维持方式，长上下文视觉遗忘问题未得到充分解决。

Method: 提出 Remember-R1 强化学习框架，直接在原始推理轨迹上施加过程级监督。设计三类奖励：鼓励更广覆盖匹配视觉关键词、增强后期推理步骤中视觉依赖的持续性、提升对问题相关图像区域的关注度。

Result: 在多种模型规模和多个多模态基准上，Remember-R1 一致地提升了推理性能。进一步分析表明它减缓了生成过程中视觉注意力的下降，验证了其在缓解长上下文视觉遗忘方面的有效性。

Conclusion: 通过过程级强化学习监督原始推理轨迹，可以有效缓解 MLLM 长上下文推理中的视觉遗忘问题，提升推理性能并维持视觉注意力。

Classification reason: 论文针对 MLLM 长思维链推理中的视觉遗忘问题，通过强化学习施加过程级监督来维持视觉证据依赖，属于多模态推理与思维链方向，映射到 CANONICAL 子主题 multimodal_reasoning_cot。

Importance reason: Addresses visual forgetting in long-context MLLM reasoning via RL, but is tangential to the user's core continuous-model and generation priorities. Signals: multimodal understanding, reinforcement learning, long-context reasoning, visual forgetting, MLLM
...
### [159] [Deferred Exposure of Future Trajectories for Verifiable Reasoning in Autonomous Driving VLMs](https://arxiv.org/abs/2608.01755)
...
TL;DR: 本文提出 AD-MCQ 与 DEFT-RLVR 方法，将自动驾驶 VLM 的规划任务转化为对显式轨迹候选的选择，并通过延迟暴露未来轨迹作为决策后的验证目标，避免教师模型因看到 GT 轨迹而产生的轨迹锚定偏差，从而提升推理的因果忠实性并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶 VLA 模型使用 CoT 监督增强 VLM 推理，但标注流程常将教师模型暴露于已记录的 GT 未来轨迹，导致轨迹锚定偏差：教师模型合理化已见结果而非从场景证据推断决策，产生因果不忠实的 CoT 和更严重的幻觉。移除 GT 轨迹虽消除捷径，但开放式轨迹生成将高层决策与精确几何合成和低层动力学纠缠在一起。

Method: 提出 AD-MCQ（自动驾驶多项选择题），将规划建模为在显式轨迹候选中进行选择，使轨迹级驾驶决策可验证而无需开放式轨迹合成。在此基础上提出 DEFT-RLVR（延迟暴露未来轨迹用于 RLVR），将未来轨迹从决策前锚点转变为决策后验证目标，用于强化学习可验证推理。

Result: 实验表明 DEFT-RLVR 提升了自动驾驶推理能力，同时保持甚至增强了通用视觉能力。AD-MCQ 通过候选构建实现可控难度，提供灵活、可扩展、可扩展的自动驾驶可验证推理研究基础。

Conclusion: 延迟暴露未来轨迹能有效消除轨迹锚定偏差，使 VLM 从场景证据进行因果推理而非合理化已知结果。AD-MCQ 作为可验证推理框架，为自动驾驶 VLM 的规划推理提供了可扩展的评测与训练基础。

Classification reason: 论文聚焦自动驾驶 VLM 的 CoT 推理与可验证推理（RLVR），属于多模态推理与思维链方向，映射到 [CANONICAL] 子主题 multimodal_reasoning_cot。

Importance reason: Focuses on verifiable reasoning in autonomous driving VLMs, which is adjacent to but not central to your core continuous multimodal and video generation priorities. Signals: autonomous driving VLM, verifiable reasoning, RLVR, trajectory anchoring bias, multiple-choice planning
...
### [160] [SearchMaster: Grounded and Regulated Self-Play for Search Agents](https://arxiv.org/abs/2608.01822)
...
TL;DR: SearchMaster 提出一种自博弈框架，让单个 LLM 在本地搜索环境中生成、求解并验证搜索任务，通过证据链生成、搜索深度奖励和过度打开惩罚三种控制机制，解决自生成任务中的伪多跳问题、难度估计偏差和浅层浏览问题，在六个深度搜索基准上显著提升 Qwen3.5-9B 的搜索智能体性能。


<details>
  <summary>Details</summary>
Motivation: 训练基于 LLM 的搜索智能体需要高质量搜索数据，但现有管线依赖人工编写任务、专家演示或更强的教师模型，成本高且难以扩展。自博弈方法虽可自生成数据，但自生成任务和轨迹可能产生误导性信号，如伪多跳问题、忽略搜索深度的难度估计、以及过度打开文档但缺乏针对性证据获取的轨迹。

Method: SearchMaster 提出三种控制机制：(1) 证据链生成器（ECG）将任务生成锚定在显式跨文档证据链上，减少伪多跳问题；(2) 搜索深度奖励（SDR）根据成功轨迹的搜索深度而非成功率来评估任务难度，保留搜索密集型任务；(3) 过度打开惩罚（OOP）通过抑制过度文档打开来规范工具使用，避免长而浅的浏览。验证后的 Proposer 和 Solver 轨迹通过 GRPO 联合优化。

Result: 在六个深度搜索基准上，SearchMaster 将 Qwen3.5-9B 骨干的平均准确率从 38.19% 提升到 51.52%，在 BrowseComp-Plus 上获得 30.1 个百分点的提升。

Conclusion: 有依据且受调控的自博弈能够为搜索智能体训练提供有效数据，无需人工标注的问答对或专家演示，验证了自博弈范式在搜索智能体训练中的可行性。

Classification reason: 该论文训练 LLM 搜索智能体进行多跳检索与推理，属于推理与智能体方向。虽然不直接涉及视觉/多模态，但搜索智能体的多跳推理与工具使用属于推理类研究，映射到多模态推理与思维链这一 CANONICAL 子主题。

Importance reason: Solid search-agent self-play method, but outside the user's core continuous/multimodal generation and world-model focus. Signals: self-play training, search agent, GRPO, evidence-chain generation, search-depth reward

Abstract: Training LLM-based search agents requires high-quality search data: tasks that demand genuine multi-hop retrieval and trajectories that use search tools effectively. Existing pipelines often depend on human-written tasks, expert demonstrations, or stronger teacher models. We present SearchMaster, a self-play framework that trains a single LLM from search tasks it generates, solves, and verifies in a local search environment. The key challenge is that self-generated tasks and rollouts can yield misleading signals: pseudo multi-hop questions, success-rate difficulty estimates that ignore search depth, and rollouts with excessive opening but little targeted evidence acquisition. SearchMaster addresses these failure modes with three controls. An Evidence-Chain Generator (ECG) grounds task generation in explicit cross-document evidence chains to reduce pseudo multi-hop questions. A Search-Depth Reward (SDR) scores task difficulty by the search depth of successful rollouts rather than success rate alone, keeping retained tasks search-intensive. An Over-Opening Penalty (OOP) regulates tool use by discouraging excessive document opening, avoiding long but shallow browsing. Verified Proposer and Solver rollouts are then jointly optimized with GRPO. Across six deep-search benchmarks, SearchMaster improves a Qwen3.5-9B backbone from 38.19% to 51.52% average accuracy, with a 30.1-point gain on BrowseComp-Plus. These results show that grounded and regulated self-play can provide effective search-agent training data without human-labeled QA pairs or expert demonstrations. The code is available at .

</details>


### [161] [Recompute or Reuse? Diagnosing and Mitigating Textual Shortcuts in VLM Self-Reflection](https://arxiv.org/abs/2608.01930)
...
TL;DR: 论文通过匹配反事实分析诊断 VLM 自我反思中的"文本捷径"问题：先前 CoT 中的证据性内容会与当前图像的视觉重计算形成行为竞争，导致模型复用旧证据而非重新计算。作者提出训练无关的 Fresh-State Attention Firewall (FSAF) 干预，将新鲜计算与先前 CoT 隔离，将视觉更新率从 35.28% 提升到 53.61%，并将先前答案率从 39.22% 降至 3.67%。


<details>
  <summary>Details</summary>
Motivation: VLM 被期望在视觉证据变化时修正其推理，但失败常被归因于视觉注意力不足或上下文惯性，缺乏对模型"复用旧内容而非重新计算"这一具体机制的清晰理解。论文旨在诊断先前 CoT 中证据性内容如何形成文本捷径并抑制视觉重计算。

Method: 在 16 个 VLM 上进行匹配反事实分析，识别证据性内容是先前 CoT 影响的最稳健载体；通过移除/重排证据内容、削弱当前图像支持等实验验证捷径强度与残余影响。提出训练无关的 FSAF 干预，将新鲜计算与先前 CoT 在注意力层面隔离。

Result: 移除证据性内容比移除长度匹配的非证据上下文或最终答案跨度更能改变答案偏好；FSAF 在五个 VLM 上将视觉更新率从 35.28% 提升到 53.61%，先前答案率从 39.22% 降至 3.67%。

Conclusion: 可靠的 VLM 自我反思不仅需要"再看一眼"，还必须保护新鲜的视觉重计算免受陈旧文本复用的干扰。文本捷径是 VLM 反思失败的关键机制，需要针对性的干预来隔离新鲜计算。

Classification reason: 论文研究 VLM 在自我反思（self-reflection）中基于先前思维链（CoT）形成的文本捷径对视觉重计算的影响，属于多模态推理与思维链范畴，映射到 [CANONICAL] 子主题 multimodal_reasoning_cot。

Importance reason: Solid diagnostic study on VLM self-reflection shortcuts, but tangential to your core continuous/multimodal generation and world-model priorities. Signals: VLM self-reflection, textual shortcut, counterfactual analysis, training-free intervention, multimodal reasoning
...
### [162] [Shared Prefixes, Better Credit: Adaptive Routing for Multi-Agent Reasoning](https://arxiv.org/abs/2608.02291)
...
TL;DR: TreeCredit 提出一种共享前缀信用分配框架，通过状态匹配的下游比较来估计多智能体推理中每个算子的效用，训练轻量级成对状态路由器，在六个推理基准上以更低的推理成本实现更好的准确率-成本权衡。


<details>
  <summary>Details</summary>
Motivation: 现有自适应多智能体推理方法通常从查询级标签或轨迹级回报学习路由决策，这种粗粒度监督无法准确估计多步协作中单个算子的状态条件效用，导致路由效率低下。

Method: TreeCredit 构建共享前缀协作树，从同一中间状态扩展候选算子，基于完整延续的终端正确性和累计额外成本为每个状态-算子对分配正确性优先的后缀信用。这些结构化信用被转换为状态局部算子偏好，用于训练轻量级成对状态路由器，在推理时动态选择下一个可接受的算子。

Result: 在六个推理基准上，TreeCredit 适度提升准确率，同时大幅降低推理成本，相比代表性 MAR 方法实现了更好的准确率-成本权衡。

Conclusion: 通过共享前缀信用分配实现更精确的算子效用估计，TreeCredit 为自适应多智能体推理提供了更高效的信用分配与路由机制。

Classification reason: 论文提出 TreeCredit 自适应路由框架用于多智能体推理（MAR），属于推理时计算扩展与多智能体推理方法，映射到 [CANONICAL] 多模态推理与思维链（multimodal_reasoning_cot）子主题。

Importance reason: Solid but niche multi-agent routing work with no direct connection to the user's core continuous/multimodal generation priorities. Signals: multi-agent reasoning, adaptive routing, credit assignment, inference cost reduction, reasoning benchmarks
...
### [163] [AdaThinkV: Adaptive Thinking for Token-Efficient Video Reasoning](https://arxiv.org/abs/2608.01980)
...
TL;DR: AdaThinkV 提出一种自适应视频推理框架，通过强化学习让视频多模态大模型根据问题难度自主决定是否进行显式思维链推理，在保持精度的同时显著减少输出 token 数。


<details>
  <summary>Details</summary>
Motivation: 思维链推理能提升困难视频问题的性能，但简单问题往往浪费大量解码 token。现有方法需要离线难度标签、人工置信度阈值或外部路由器来调节推理强度，缺乏让模型自主适应推理努力的自适应机制。

Method: AdaThinkV 在强化学习阶段对每个 prompt 采样显式推理与直接回答两种模式的匹配 rollout。ThinkGain 通过平衡准确率增益与额外响应长度来估计显式推理的 prompt 级效用，为条件响应生成和自主模式选择提供监督。针对困难 prompt 中 rollout 探索不足导致奖励信号匮乏的问题，提出 Variance Recovery Policy Optimization (VRPO)，保留并逐步扩展这些组以恢复可解困难 prompt 的信息信号。推理时模型在单个自回归序列中选择响应模式并生成回答。

Result: 在统一的视频推理评测套件上，AdaThinkV 达到平均准确率 40.79，平均输出 257.20 个 token，相比最强自适应基线准确率提升 2.98 分，同时减少 22.7% 的 token 使用。

Conclusion: AdaThinkV 证明视频多模态大模型可以通过强化学习自主适应推理努力，无需外部标签或路由器即可在准确率与 token 效率之间取得更优平衡，为视频推理的 token 高效化提供了有效方案。

Classification reason: 论文研究视频多模态大模型的适应性思维链推理，通过强化学习让模型自适应决定是否显式推理，属于多模态推理与思维链范畴，映射到 [CANONICAL] 子主题 multimodal_reasoning_cot。

Importance reason: Proposes adaptive reasoning effort control for video MLLMs, directly relevant to efficient multimodal understanding and token reduction. Signals: adaptive reasoning, token-efficient, video reasoning, reinforcement learning, multimodal understanding
...
### [164] [VC-Tooler: Learning Compositional and Adaptive Visual Tool Use](https://arxiv.org/abs/2608.02217)
...
### [165] [FATE: Frame-Level Audio-Visual Temporal Embedding](https://arxiv.org/abs/2608.01310)
...
TL;DR: Déjà Cue 是一个无需训练的框架，通过将候选状态描述转化为"词汇相对坐标"（减去状态平衡质心），在跟踪对象历史中定位每个状态成立的视频区间，在 VOST 数据集上显著提升检索性能。


<details>
  <summary>Details</summary>
Motivation: 跟踪只能关联同一对象在不同帧中的观测，但无法判断对象何时为空/满、完整/切割等状态。绝对图像-文本相似度独立评分各描述，由于每帧都描绘同一目标，共享的对象兼容性会掩盖区分状态所需的证据。需要利用候选状态之间的相对关系作为参考系来定位状态区间。

Method: 提出 Déjà Cue，一个无需训练的框架。它将候选状态描述转化为词汇相对坐标系：从每个描述中减去状态平衡质心（state-balanced centroid），校准帧级相似度分数，然后在连续可见片段内用冻结编码器扫描多个时间跨度，定位每个状态成立的区间。

Result: 在 78 个 VOST 历史序列上，保持时间扫描固定、仅改变查询参考，R@1@tIoU0.5 从 10.3% 提升到 20.5%（近翻倍），Top-1 tIoU 从 16.0% 提升到 21.5%。候选排序分析表明词汇相对查询能在同一候选集中将有用区间排得更靠前。

Conclusion: 相关的状态描述可以作为对象特定的、查询时的坐标系来读取冻结的视觉表征，从而在不训练的情况下显著提升视频对象状态定位的准确性。

Classification reason: 论文提出身份条件的状态时刻检索（identity-conditioned state-moment retrieval），利用图像-文本相似度在视频对象历史中定位状态区间，属于跨模态检索/视频对象检索范畴，映射到 [CANONICAL] 多模态检索 subtopic。

Importance reason: A niche training-free retrieval trick with modest gains, far from the user's core continuous/multimodal generation and world-model focus. Signals: training-free retrieval, vocabulary-relative coordinates, state-moment retrieval, VOST benchmark, frozen encoder
...
### [167] [RadPRISM: Schema-stratified radiology-report supervision for concept-disentangled image representations and visual grounding](https://arxiv.org/abs/2608.00147)
...
TL;DR: RadPRISM 提出一种基于放射学报告监督的医学图像表征学习方法，将临床医生定义的放射学 schema 作为分层轴，用本地 LLM 从自由文本报告中提取每个临床概念，并在各自独立的视觉子空间中对齐，实现概念解耦的图像表征与视觉定位。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言预训练模型通常在单一共享嵌入空间中操作，概念级结构和可解释性需要事后恢复，限制了模型透明度和临床实用性。作者希望让临床概念分层成为直接的顶层对齐监督，从而获得可解释、可检索的概念解耦表征。

Method: RadPRISM 使用本地部署的 LLM 从自由文本放射报告中提取每个临床概念对应的文本片段，将临床医生定义的放射学 schema 作为分层轴，每个临床概念在各自专属的视觉子空间中对齐，将概念分层转化为直接的顶层对齐监督。在胸部 X 光片上用 19 概念 schema 在 203,602 例检查上实例化。

Result: 内部数据集零样本分类 macro AUROC 从 0.717 提升到 0.868（对比全局对齐基线）；外部零样本分类与专用 CARZero 参考相当，但在 pointing-game 视觉定位上大幅超越（最高 4.3 倍）；放射科医生阅读研究显示概念分层检索能力（rank 3 内 macro 检索正确率 0.78）。

Conclusion: RadPRISM 能产生判别性强、空间忠实、原生概念分层的表征，且可由临床医生透明检查，提升了医学影像模型的临床实用性与可解释性。

Classification reason: 论文提出 RadPRISM，通过放射学报告监督学习概念解耦的医学图像表征，并支持概念分层检索与视觉定位。其核心贡献在于概念级跨模态对齐与检索能力，映射到 [CANONICAL] 多模态检索子主题。

Importance reason: Medical radiology-specific representation learning with concept disentanglement, but far outside the user's core focus on continuous multimodal models, autoregressive generation, and world models. Signals: medical imaging, concept-disentangled representation, visual grounding, radiology schema, zero-shot classification
...
### [168] [One Query, Many Scales: Sparse Mixture-of-Experts for Efficient Hierarchical Cross-View Geo-Localization](https://arxiv.org/abs/2608.01060)
...
TL;DR: GeoMoE 提出一种稀疏混合专家双编码器，将全局多尺度表征学习与局部层次化搜索解耦，用于高效跨视角地理定位。通过全局多尺度监督和内容自适应路由将地面与卫星图像映射到统一嵌入空间，推理时每个图像仅编码一次，用概率束搜索沿父子链接打分候选子集。在 VIGOR-M 基准上达到 62.39% R@1，匹配开销仅为穷举 L3 扫描的 5.27%。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位系统大多在扁平、固定分辨率的图库上穷举搜索，在大面积区域上成本高昂且难以适应卫星分辨率变化。自回归粗到细方法虽减少比较次数，但将后续预测绑定到先前决策和预定义层次结构，存在误差传播问题。

Method: GeoMoE 采用稀疏混合专家双编码器，解耦全局多尺度表征学习与局部层次化搜索。全局多尺度监督和内容自适应路由将地面与卫星图像跨分辨率映射到全局可比较的嵌入空间。推理时每个图像编码一次，概率束搜索沿父子链接打分候选子集，后续层级复用先前描述符而非重新生成特征，限制特征级误差传播与层次耦合。同时引入 VIGOR-M 四城市基准，含显式父子卫星层次和留出的半步图库。

Result: GeoMoE 在 Just Zoom In 上达到 95.78% R@40m，比此前最优高 2.77 个百分点；在 VIGOR-M 上达到 62.39% R@1，描述符匹配仅需 0.885 MMAC/查询，为穷举 L3 扫描的 5.27%，同时比最强穷举基线在 R@1 上高 3.12 个百分点。单一模型在 L1/L2/L3 训练后优于匹配的密集对照组，并迁移到三个留出分辨率。

Conclusion: 通过将全局训练的嵌入与局部层次化搜索解耦，GeoMoE 同时提升了定位精度、搜索效率和跨分辨率迁移能力，验证了稀疏混合专家在层次化跨视角检索中的有效性。

Classification reason: 该论文研究跨视角地理定位（cross-view geo-localization），本质是地面图像与卫星图像之间的跨模态检索任务，属于多模态检索方向，映射到 [CANONICAL] 多模态检索（multimodal_retrieval）子主题。

Importance reason: Solid CV/retrieval engineering but outside your core continuous-multimodal, video/world-model, and generation priorities. Signals: sparse MoE dual encoder, hierarchical geo-localization, efficient retrieval, cross-resolution transfer, VIGOR-M benchmark
...
### [169] [VaRS-Doc: Interpretation-Aware Variant Representations via Latent Self-Probing for Visual Document Retrieval](https://arxiv.org/abs/2608.01211)
...
TL;DR: KoVRE 是一个面向韩语视觉文档检索的单向量嵌入模型，在 708,729 个韩英查询-页面对上训练，采用正感知难负例挖掘与重排器知识蒸馏，2B 模型在韩语 VDR 基准上超越 8B 单向量基线与多向量基线。


<details>
  <summary>Details</summary>
Motivation: 现有视觉文档检索（VDR）模型和训练资源以英语为主，且高性能系统依赖大规模骨干网络或存储密集的多向量表征。韩语视觉文档检索缺乏高效且有效的单向量检索方案。

Method: 提出 KoVRE，一个韩语视觉文档检索的单向量嵌入模型。在 708,729 个韩英查询-页面对上训练，采用正感知难负例挖掘（positive-aware hard-negative mining），并系统分析训练数据组成、难负例处理与基于重排器的知识蒸馏策略。

Result: 2B 模型在韩语视觉文档检索基准上大幅超越基础骨干模型，同时优于 8B 单向量模型和强多向量基线。

Conclusion: 针对性的双语监督与精心设计的训练策略可以在不扩大骨干网络或使用多向量表征的情况下，训练出跨多种文档领域的高效韩语 VDR 模型。

Classification reason: 该论文研究韩语视觉文档检索（Visual Document Retrieval），属于跨模态/文档检索范畴，与 [CANONICAL] 多模态检索（multimodal_retrieval）子主题高度匹配。

Importance reason: Focused on Korean visual document retrieval, which is outside the user's core interests in continuous multimodal models, autoregressive generation, and world models. Signals: Korean VDR, single-vector embedding, hard-negative mining, knowledge distillation, 2B model efficiency
...
### [171] [MRAFnd: Multimodal Retrieval-Augmented Framework for Zero-Shot Fake News Detection](https://arxiv.org/abs/2608.01430)
...
TL;DR: MRAFnd 是一个多模态检索增强的零样本假新闻检测框架，通过多模态相似度检索、双向证据推理和多智能体协作辩论来识别虚假新闻，在三个基准数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有零样本假新闻检测方法通常孤立地通过语义匹配评估新闻，无法识别过去虚假宣传中重复使用的欺骗策略，也缺乏识别细微跨模态不一致所需的复杂推理能力。

Method: MRAFnd 模拟一个协作分析师团队来验证新闻真实性。框架首先通过多模态相似度检索从未标注参考数据库中收集上下文相似的新闻语料；随后在双向证据推理阶段，智能体进行双向分析以提取检索证据中的关键模式；最后通过包含分析师和仲裁者的多智能体协作辩论进行结构化讨论，得出最终结论。

Result: 在三个基准数据集上的综合实验表明，MRAFnd 显著超越最先进基线，在具有挑战性的 Weibo-21 数据集上准确率提升高达 2.35%。

Conclusion: 多模态检索增强结合多智能体协作辩论能有效提升零样本假新闻检测的准确性和鲁棒性，验证了利用历史虚假宣传模式与跨模态推理相结合的价值。

Classification reason: 论文提出多模态检索增强的假新闻检测框架，核心是跨模态检索与多模态推理，映射到 [CANONICAL] 多模态检索（multimodal_retrieval）子主题。

Importance reason: 多模态检索增强假新闻检测与用户关注方向（连续多模态、视频/图像生成、世界模型）无直接交集，且含负面关键词。 Signals: 多模态检索增强, 零样本假新闻检测, 多智能体辩论, 非生成式任务, 与用户方向弱相关
...
### [172] [UniHEAR: Unified Heterogeneous-Source Attentive Retrieval for Knowledge-Based Visual Question Answering](https://arxiv.org/abs/2608.01147)
...
TL;DR: UniHEAR 是一个统一轻量级框架，用于知识型视觉问答中的异构源实体检索与重排序。它通过粗检索描述符、检索引导的注意力模态门控和熵加权源融合，解决单源检索瓶颈与检索源盲重排序问题，在 E-VQA 和 InfoSeek 上取得 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 现有 KB-VQA 检索增强系统存在两个关键局限：一是依赖单一检索模态造成单源检索瓶颈，遗漏仅能通过互补来源访问的实体；二是双塔点式重排序器存在检索源盲重排序问题，忽略检索来源和候选级检索先验，导致冗余的模态依赖。

Method: UniHEAR 为每个候选实体构建粗检索描述符，引入检索引导的注意力模态门控，根据该描述符调节模态注意力权重，并辅以粗检索先验的熵加权源融合。采用对比学习与辅助模态保持损失的混合训练策略，在单一模型中统一实体级与章节级检索。

Result: 在 E-VQA 和 InfoSeek 数据集上，UniHEAR 取得 SOTA 检索与 VQA 性能，Recall@1 相比最强基线分别提升 6.7 和 1.2 个点，同时保持轻量级重排序架构。

Conclusion: UniHEAR 通过统一异构源检索与检索源感知的重排序，有效缓解了单源检索瓶颈和检索源盲重排序问题，验证了在 KB-VQA 中融合多源检索先验与模态门控的有效性。

Classification reason: 该论文研究知识型视觉问答（KB-VQA）中的异构源实体检索与重排序，属于跨模态/多模态检索范畴，映射到 [CANONICAL] 多模态检索（multimodal_retrieval）子主题。

Importance reason: Focuses on KB-VQA retrieval reranking, which is outside your core priorities in continuous multimodal models, autoregressive generation, and world models. Signals: KB-VQA, heterogeneous retrieval, reranking, lightweight framework, SOTA on E-VQA/InfoSeek

Abstract: Knowledge-Based Visual Question Answering (KB-VQA) requires retrieving relevant entity knowledge from external sources to answer visually grounded questions. Existing retrieval-augmented systems suffer from two critical limitations. First, relying on a single retrieval modality creates a Single-Source Retrieval Bottleneck, missing ground-truth entities that are only accessible through complementary sources. Second, dual-tower pointwise rerankers suffer from Retrieval-Source-Blind Reranking, as they overlook retrieval origins and candidate-level retrieval priors, leading to redundant modality reliance. To address these challenges, we propose UniHEAR, a unified lightweight framework for heterogeneous-source entity retrieval and reranking. UniHEAR constructs a Coarse Retrieval Descriptor for each candidate entity, and introduces Retrieval-Guided Attentive Modality Gating to condition modality attention weights on this descriptor, further complemented by Entropy-Weighted Source Fusion of coarse retrieval priors. A hybrid training strategy combining contrastive learning with an auxiliary modality-preserving loss unifies entity-level and section-level retrieval within a single model. Extensive experiments on E-VQA and InfoSeek demonstrate that UniHEAR achieves state-of-the-art retrieval and VQA performance, improving Recall@1 by 6.7 and 1.2 points over the strongest baselines while maintaining a lightweight reranking architecture. Code and model are available at .

</details>


### [173] [Illuminating Visual Identity in Universal Multimodal Embeddings](https://arxiv.org/abs/2608.01794)
...
TL;DR: 本文提出 Douyin Multimodal Embedding (DME)，一种两阶段训练的多模态嵌入模型，结合大规模对比预训练与证据锚定的类型化潜空间推理、跨条件重建机制，在保持对比编码器推理效率的同时提升细粒度判别能力，在 MMEB-v2 上达到 SOTA 并已部署于抖音搜索场景。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 嵌入模型难以同时满足十亿级索引下的高效性与细粒度硬匹配的判别能力：对比模型高效但依赖过于粗糙的成对监督，CoT 模型通过显式生成提升判别但无法在线服务。

Method: 两阶段训练：阶段一进行大规模对比预训练建立统一多模态嵌入空间；阶段二通过两种机制补充语义充分性——Evidence-Grounded Typed Latent Reasoning（在隐藏空间进行潜空间推理组织检索证据）与 Cross-Conditional Reconstruction（跨方向自回归重建强化对端语义），两者仅在训练时生效，推理时仅增加少量查询端开销。

Result: 在 MMEB-v2 上 2B 与 9B 变体分别达到 74.8 和 78.4 的 SOTA 结果，视频与视觉文档任务表现突出；生产环境中在抖音离线评测集上获得 2.92% 相对提升，在线 A/B 测试中抖音搜索 LT 提升 0.1%。

Conclusion: DME 证明通过训练时潜空间推理与跨条件重建机制，可以在保持标准对比编码器推理效率的同时获得细粒度判别能力，为工业级多模态检索提供了兼顾效率与效果的可行方案。

Classification reason: 该论文提出 Douyin Multimodal Embedding (DME) 多模态嵌入模型，用于工业级搜索与推荐场景的跨模态检索，属于多模态检索方向，映射到 [CANONICAL] 子主题 multimodal_retrieval。

Importance reason: Solid industrial multimodal embedding work, but it is retrieval-focused and lacks the continuous-latent/world-model novelty central to your priorities. Signals: multimodal embedding, contrastive pretraining, latent reasoning, cross-conditional reconstruction, deployed at Douyin
...
### [175] [UEmbed: Unified Sparse and Dense Multimodal Embeddings](https://arxiv.org/abs/2608.02583)
*Tingyu Song, Mingxin Li, Yanzhao Zhang, Dingkun Long, Pengjun Xie, Zhijie Nie, Yilun Zhao, Shu Wu*

Main category: cs.IR, cs.AI, cs.CL, cs.CV
...
TL;DR: UEmbed 是一个 decoder-only 多模态嵌入模型，通过一次因果前向同时生成稀疏词法表示和稠密表示，将词汇表划分为 N 个不相交子集，每个可学习特殊 token 预测其子集上的稀疏权重，从而统一文本与多模态输入的稀疏检索与稠密检索。


<details>
  <summary>Details</summary>
Motivation: 现有 Learned Sparse Retrieval (LSR) 局限于 encoder 式双向架构，且扩展到多模态时依赖辅助跨模态模块，缺乏统一的稀疏与稠密多模态嵌入模型。

Method: UEmbed 采用 decoder-only 架构，在输入后追加 N 个可学习特殊 token，将词汇表划分为 N 个不相交子集；每个 token 的因果隐藏状态预测其分配子集上的稀疏权重，N 个子集拼接成完整稀疏向量，同时输出稠密表示，实现一次前向统一生成稀疏与稠密嵌入。

Result: UEmbed-9B 在 MMEB-v2 上达到 71.8（稠密）和 71.0（稀疏），优于基于公开数据训练的多模态嵌入模型（如 RzenEmbed）；在 BEIR 上与强稠密和稀疏基线保持竞争力，并在有效性、效率与 agentic 应用三个维度验证了实用性。

Conclusion: UEmbed 提供了一种新范式：在单一模型中统一稠密与稀疏嵌入，并将稀疏检索扩展到统一文本与多模态输入，为多模态检索与 RAG 提供了更高效的解决方案。

Classification reason: 该论文提出统一稀疏与稠密的多模态嵌入模型 UEmbed，用于多模态检索与 RAG，属于多模态检索方向，映射到 [CANONICAL] 多模态检索 subtopic。

Importance reason: Focuses on retrieval embeddings rather than the user's core interests in continuous multimodal models or generative world models. Signals: multimodal retrieval, sparse+dense embedding, decoder-only, MMEB-v2, BEIR
...
### [176] [Evaluating VLMs on Multimodal Aristotelian Persuasion Tasks](https://arxiv.org/abs/2608.01238)
...
TL;DR: 本文使用 ImageArg 数据集评估视觉语言模型在亚里士多德说服模型（Logos、Ethos、Pathos）检测任务上的表现，发现 Qwen 系列模型表现最佳，其中 Qwen3 在 Logos 和 Pathos 任务上表现突出，Qwen2 在更复杂的 Ethos 任务上具有竞争力。


<details>
  <summary>Details</summary>
Motivation: VLM 在各类任务上表现优异，但尚未在更复杂的任务上被充分评估。亚里士多德说服模型（Logos、Ethos、Pathos）涉及个人偏见等内在挑战，需要评估 VLM 在这些复杂任务上的进展。

Method: 使用 ImageArg 数据集，针对 Logos（逻辑）、Ethos（人格/可信度）、Pathos（情感）三个说服检测任务，对多种 VLM 进行系统评估，并发布代码以促进该方向研究。

Result: Qwen 系列模型获得更高的 F1 分数；Qwen3 在 Logos 和 Pathos 任务上表现优异，Qwen2 在更复杂的 Ethos 检测任务上表现具有竞争力。

Conclusion: VLM 在复杂说服检测任务上已取得一定进展，Qwen 系列模型表现突出，但 Ethos 等涉及个人偏见的任务仍具挑战性，需要进一步研究。

Classification reason: 该论文使用 ImageArg 数据集评估 VLM 在 Logos/Ethos/Pathos 说服检测任务上的表现，属于多模态评测与基准方向，映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: This is a narrow VLM benchmark study on persuasion detection, far outside the user's core focus on continuous multimodal models, autoregressive generation, and world models. Signals: VLM evaluation, persuasion detection, ImageArg dataset, Qwen family, no generation or world model focus
...
### [177] [AgentMemBench: A Systematic Benchmark for Evaluating Long-Term Memory Management Strategies in Conversational AI Agents](https://arxiv.org/abs/2608.00009)
*Ahmed Cherif*
...
TL;DR: AgentMemBench 是一个统一、可复现的基准，在相同条件下评估五种长期记忆管理策略（ICW、EKV、GEM、CBS、WAM），覆盖三个公开数据集和491个标注问题，结果显示外部键值存储（EKV）在所有质量指标上占优，但存在内存占用成本。


<details>
  <summary>Details</summary>
Motivation: 对话式 AI 智能体的长期记忆是关键瓶颈，有限上下文窗口无法支持跨数千轮对话的连贯回忆。现有记忆策略缺乏统一、可复现的对比评估框架。

Method: 构建 AgentMemBench 基准，在相同条件下评估五种记忆管理策略：上下文窗口（ICW）、外部键值存储（EKV）、图式情景记忆（GEM）、压缩式摘要（CBS）和网络增强记忆（WAM）。使用三个公开数据集（LoCoMo、MultiDoc2Dial、MSC），采用 Recall@k、MRR、nDCG@k、Answer F1、LLM-judge Faithfulness、Memory Footprint 和 Latency 等指标，基于 Qwen2.5-7B-Instruct 生成与评判。

Result: EKV 在所有质量指标上占优（macro Recall@5 0.792、MRR 0.677、F1 0.156、Faithfulness 0.354）；在长程回忆场景（LoCoMo）中，ICW、WAM、GEM、CBS 几乎无法检索（Recall@5<=0.005），而 EKV 达到 0.573；CBS 在检索上排名第二（0.556）；EKV 的召回优势伴随约5100 token 的内存占用成本，而 ICW/WAM 仅约300 token。

Conclusion: 长程回忆是决定性因素：只有稠密检索（EKV）能在长时距下扩展，而近期窗口、摘要和实体图在长时距下失效。EKV 的召回优势与内存占用之间存在明确的精度-效率权衡。论文还评估了 MemGPT/Letta 和 HippoRAG 两个已发布记忆系统，并开源全部代码与结果。

Classification reason: 该论文提出 AgentMemBench，一个系统评估对话式 AI 智能体长期记忆管理策略的基准，属于评测基准类工作，映射到 [CANONICAL] 多模态评测与基准（multimodal_benchmark）子主题。

Importance reason: Benchmark on conversational memory management, not aligned with your priority directions or subtopics. Signals: memory management benchmark, conversational AI, external key-value store, long-term memory, no continuous/multimodal/video/world-model focus
...
### [178] [Obshazard-bench: Benchmarking Multimodal Foundation Models for Real-Time Disaster Intelligence from Raw Earth Observation Streams](https://arxiv.org/abs/2608.00012)
...
Main category: cs.CY, cs.AI, cs.CL, cs.LG
...
TL;DR: Obshazard-bench 是一个实时、观测驱动的多模态大模型灾害智能评测基准，直接整合原始高频卫星探测流、地面观测、历史灾害记录与社会经济指标，覆盖 8 大类 28 小类灾害、60 多个国家、120 多个历史极端事件案例及数千个生命周期 VQA 样本，并定义三阶段评测体系（预测性危机预判、主动演化推理、多维度影响量化）。


<details>
  <summary>Details</summary>
Motivation: 现有遥感基准多依赖静态、事后、专家处理的产品（如网格化再分析数据），难以对齐灾害快速演化、决策时间受限的真实应急场景，导致 MLLM 在灾害应急响应中的能力未被充分评测。

Method: 构建 Obshazard-bench 基准，直接整合来自多种卫星传感器的原始高频探测流、地面站观测、历史灾害记录与社会经济指标，绕过延迟的专家处理与物理反演流程；定义三阶段评测分类体系（预测性危机预判、主动演化推理、多维度影响量化），并生成生命周期导向的 VQA 样本。

Result: 在通用与地球聚焦的基础模型上实验，发现这些模型在将原始多通道物理观测转化为时间锚定、决策相关的灾害推理方面存在显著局限。

Conclusion: 现有 MLLM 在真实灾害应急场景的原始观测流推理能力不足，Obshazard-bench 为评估和推动灾害智能模型发展提供了更贴近实战的评测基准。

Classification reason: 该论文提出 Obshazard-bench，一个面向灾害应急响应的多模态大模型评测基准，属于多模态评测与基准方向，映射到 [CANONICAL] 子主题 multimodal_benchmark。

Importance reason: Benchmark for disaster intelligence from raw satellite streams, but remote sensing is a negative keyword and lacks direct alignment with your priority subtopics. Signals: remote sensing benchmark, disaster intelligence, multimodal foundation models, real-time earth observation, VQA benchmark
...
### [179] [What Transfers from Text to Vision? Capability Scaling Laws and Transfer Dynamics for VLMs](https://arxiv.org/abs/2608.00013)
...
Importance: 普通相关 (45.1) - 未入选精读

TL;DR: XL-DocBench 是一个全人工验证的超长文档理解基准，包含 1,519 个来自六个专业领域的问题，上下文最长 2,303 页，其中 72.6% 需要多页证据、36.6% 涉及表格/图表/图片、10.9% 需要跨文档证据，并配有专家标注的证据页、类型化验证规则与答案格式。


<details>
  <summary>Details</summary>
Motivation: 真实世界的文档任务（年报、法规、临床指南、技术手册）往往长达数百上千页，且需要追溯到具体证据页，但现有基准大多只评测短上下文或单页 QA，无法衡量系统在超长专业文档上的证据溯源与规则遵循能力。

Method: 采用树引导的合成流水线构建问题，配合工件过滤与 194 名人类专家的完整验证。每个问题包含十二种推理标签之一、专家标注的证据页、类型化验证规则和答案格式，并包含 218 个 None-answer 案例。

Result: 实验表明当前系统在长上下文、多页证据和结构化推理上仍表现不佳，说明现有模型难以胜任超长专业文档理解任务。

Conclusion: XL-DocBench 填补了单页、短多页或纯文本长上下文基准的空白，通过将超长专业上下文与页级证据和类型化规则结合，使未来工作能够将系统失败归因于检索、证据使用或规则遵循，而非单一的排行榜分数。

Classification reason: 该论文提出 XL-DocBench，一个面向超长文档理解的全人工验证基准，属于多模态/长文档评测基准，映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: Solid benchmark for long-document QA, but it is a text-centric evaluation task with no direct connection to the user's priority on continuous multimodal models or generative world models. Signals: long-context benchmark, evidence-grounded QA, multi-page reasoning, human-verified, no generative or continuous-model focus
...
### [181] [SafeBuild-Bench: A Temporal-Robust Construction Safety Benchmark with Graph-Enhanced Data Mining](https://arxiv.org/abs/2608.00068)
...
Importance: 值得关注 (69.2) - 未入选精读

TL;DR: SafeBuild-Bench 是一个元数据驱动的建筑施工安全评测基准，从 10 万+ 工业图文记录中挖掘出 3,314 个任务实例，覆盖多选危险识别与自由文本危险描述，并配套 GEMS 图增强多模态选择管线以规模化专家验证。评测显示当前 MLLM 在建筑施工安全理解上仍远不可靠，最佳综合得分接近 60。


<details>
  <summary>Details</summary>
Motivation: 建筑施工安全模型需要应对真实部署风险（如工人站在无护栏的脚手架边缘），而非仅识别精选图像中的常见物体。然而真实巡检档案冗余、长尾分布，且跨场地、跨月份变化，现有评测难以反映这种现实复杂性。

Method: 提出 SafeBuild-Bench 基准，从 10 万+ 工业图文记录中挖掘 3,314 个任务实例（来自 3,000+ 专家验证图像），涵盖多选危险识别与自由文本危险描述。为规模化专家验证，提出 GEMS（graph-enhanced multimodal selection）管线，结合代理模型混淆信号与基于图的多样性来从冗余流中挑选信息量大的候选样本。

Result: 在公开指令微调数据上，GEMS 选择的子集在小数据预算下保持了鲁棒性导向的性能。在 SafeBuild-Bench 上，当前 MLLM 在建筑施工安全理解上仍远不可靠，最佳综合得分接近 60。

Conclusion: SafeBuild-Bench 揭示了当前多模态大语言模型在建筑施工安全这一高风险、长尾、跨时间场地变化场景下的显著不足，为构建更可靠的施工安全理解模型提供了评测基础与数据挖掘方法。

Classification reason: 该论文提出 SafeBuild-Bench，一个面向建筑施工安全场景的多模态大语言模型评测基准，属于多模态评测与基准方向，映射到 [CANONICAL] multimodal_benchmark。

Importance reason: Construction-safety benchmark is far outside your configured focus on continuous/multimodal generation and world models. Signals: benchmark, construction safety, MLLM evaluation, graph-enhanced data mining, low topical overlap with priority subtopics

Abstract: Construction-safety models must handle concrete deployment risks, such as a worker standing near a scaffold edge without guardrails, rather than only recognize common objects in curated images. Yet real inspection archives are redundant, long-tailed, and collected across changing sites and months. We introduce SafeBuild-Bench, a metadata-driven benchmark for evaluating multimodal large language models on construction safety under realistic temporal and site variation. It is mined from 100K+ industrial image-text records and contains 3,314 task instances from over 3,000 expert-verified images, covering multiple-choice hazard identification and free-form hazard description. To make expert verification scalable, we develop GEMS, a graph-enhanced multimodal selection pipeline that combines a proxy-model confusion signal with graph-based diversity to identify informative candidates from redundant streams. On public instruction-tuning data, GEMS-selected subsets preserve robustness-oriented performance under small data budgets. On SafeBuild-Bench, current MLLMs remain far from reliable construction-safety understanding, with the best overall score near 60. We release the benchmark, evaluation scripts, and GEMS codebase at .

</details>


### [182] [SPARC-Rad: A Multimodal Benchmark Dataset and Evaluation Pipeline for Spatial and Anatomical Reasoning in Radiology Vision-Language Models](https://arxiv.org/abs/2608.00100)
...
TL;DR: SPARC-Rad 是一个针对放射学视觉语言模型的空间与解剖推理能力评测基准，包含 300 个来自 TCIA 健康对照影像的图文问答对，覆盖 CT、MRI 和 X 光，并提供标准化提示、LLM-as-judge 评分、人工审核与亚组分析等完整评测流程。


<details>
  <summary>Details</summary>
Motivation: 现有医学影像 VLM 评测基准多聚焦于疾病分类、报告生成或宽泛的视觉问答，缺乏对放射学所需的空间与解剖推理能力的系统评估，因此需要专门的评测基准。

Method: 人工构建 SPARC-Rad 基准，包含 300 个图文问答对，覆盖腹部、胸部、乳腺、神经和肌肉骨骼等类别，由放射学受训人员设计问题评估解剖识别、定位、侧别、区域识别、设备识别和结构间空间关系；配套评测流程支持标准化提示、结构化输出、响应归一化、LLM-as-judge 评分、人工质量审查、二元正确性评分及按模态/解剖/推理类型的亚组分析。

Result: 构建了可复用的 SPARC-Rad 基准数据集与评测框架，用于评估 VLM 在放射学解剖空间推理方面的能力。

Conclusion: SPARC-Rad 为评估 VLM 是否能把放射学解剖作为空间系统进行推理提供了可复用框架，支持未来模型开发、失败模式分析与部署前评估。

Classification reason: 该论文提出 SPARC-Rad 医学影像 VLM 空间与解剖推理评测基准及评测流程，属于多模态评测与基准方向，映射到 [CANONICAL] multimodal_benchmark 子主题。

Importance reason: Medical radiology benchmark with negative keyword overlap and no alignment with priority continuous/multimodal generation or world model topics. Signals: radiology VLM benchmark, spatial reasoning, medical imaging, LLM-as-judge, no continuous token/flow matching
...
### [183] [Assistant Placement Aria: A Benchmark for Egocentric Placement Assistance](https://arxiv.org/abs/2608.00652)
...
TL;DR: 本文提出 Assistant Placement Aria，首个面向具身场景虚拟放置（VP）任务的基准，包含合成与真实室内场景，标注了2D面板放置、坐姿建议和电视放置三个子任务，每个场景含2D图像、3D点云和文本描述，并评测了多个基础模型在目标检测与分割上的表现。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助人类任务中，选择符合人类意图或偏好的目标放置位置是关键挑战。传统放置任务通常只关注单一预定义目标位置，而虚拟放置（VP）需要识别场景中所有合理的候选位置，涉及全局与局部的几何、语义和合理性推理。该领域缺乏相关数据与基准，阻碍了研究进展。

Method: 构建了 Assistant Placement Aria 基准，包含合成与真实室内场景，为三个任务（2D面板放置、坐姿建议、电视放置）提供标注。每个场景包含2D图像、3D点云和场景内物体的文本描述，并评测了多个基础模型在目标检测与分割上的表现。

Result: 发布了首个覆盖全局、局部与人类中心约束的虚拟放置基准，包含多模态场景数据（2D图像、3D点云、文本描述），并提供了基础模型在目标检测与分割上的基线评测结果。

Conclusion: 该基准填补了虚拟放置任务数据与评测的空白，有望推动这一依赖数据且尚未充分探索的领域的研究发展。

Classification reason: 该论文提出"Assistant Placement Aria"基准，用于评测具身场景中虚拟放置（Virtual Placement）任务，包含2D图像、3D点云与文本描述的多模态场景数据。这属于多模态评测与基准方向，映射到[CANONICAL] subtopic "multimodal_benchmark"。

Importance reason: This benchmark is for egocentric placement assistance, which is outside the user's core focus on continuous multimodal models, autoregressive generation, and world models. Signals: egocentric benchmark, virtual placement, multimodal evaluation, foundation models, robotics
...
### [184] [MDWD: A Street-Level Dataset for Municipal Solid Waste Detection in Dense Urban Environments](https://arxiv.org/abs/2608.00257)
...
TL;DR: 本文提出 MDWD（马耳他生活垃圾数据集），一个包含 3,697 张高分辨率街景图像和 11,461 个手工标注实例的街级城市固体废弃物检测基准，覆盖五类生活垃圾，并基于 YOLO 系列与 transformer 检测器进行了跨架构基准评测。


<details>
  <summary>Details</summary>
Motivation: 城市环境自动化视觉监控是日益增长的 CV 研究领域，但市政固体废弃物检测在专用基准资源中代表性不足。现有废弃物数据集多聚焦于零散垃圾检测、航拍影像或图像级分类，缺乏同时提供街级影像、实例级定位与市政收集语境下生活垃圾分类的数据集。

Method: 构建 MDWD 数据集，包含 3,697 张高分辨率街景图像与 11,461 个手工标注实例，覆盖马耳他市政收集系统下的五类生活垃圾，并捕捉地点、光照、物体尺度、遮挡与城市语境的变化。随后在 YOLO 家族多代模型与 transformer 检测器上进行跨架构基准评测。

Result: 在测试集上，RF-DETR-M 取得最优整体性能，mAP50 达 94.49%，F1 分数 93.56%；较小容量模型在显著减少参数量的同时保持有竞争力的精度。

Conclusion: MDWD 支持紧凑型实时检测器与 transformer 模型的有效训练，为基于视觉的市政废弃物监控研究建立了基准，填补了街级生活垃圾检测基准资源的空白。

Classification reason: 该论文提出一个新的街级城市固体废弃物检测数据集 MDWD，属于视觉检测基准资源建设，映射到多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）这一规范子主题。

Importance reason: A niche waste-detection dataset benchmark with no alignment to the user's core continuous multimodal, video generation, or world model priorities. Signals: dataset benchmark, object detection, YOLO/transformer baselines, no continuous tokens, no video/world model
...
### [185] [CrossProjection: Geometric Grounding Beyond Viewpoint Change in Architectural Drawings](https://arxiv.org/abs/2608.00473)
...
TL;DR: CrossProjection 是一个锚定式诊断基准，评估视觉语言模型在异构建筑图纸（平面/剖面/立面）间保持组件身份并外化几何的能力，涵盖匹配、配准与几何定位三类任务。在 23 套真实图纸上，GPT-5.5 达 82.4%，Qwen3-VL 62.2%，GLM-4.5V 57.2%；但自由几何定位仍脆弱，点/区域 PCK@.05 在 GPT 上为 54-76%，Qwen 仅 8-10%。结论是封闭式选择或标记元素成功并不等同于可靠的显式几何定位。


<details>
  <summary>Details</summary>
Motivation: 建筑图纸（平面图、剖面图、立面图）违反多视图推理的常规假设：平面与剖面是切割，立面是立面投影，对应组件在不同视图间外观变化无法用相机运动解释。现有视觉语言模型评测未覆盖这种异构视图下的组件身份保持与几何外化能力，需要专门的诊断基准。

Method: 提出 CrossProjection 锚定式诊断框架，通过类别判断、候选选择与自由点/线/区域定位三类任务评估匹配、配准与几何定位。使用 23 套真实图纸、每模型 1,954 个类别条件，并设计 200 目标的配对研究，交叉自然图纸与矢量文本抑制图纸、封闭候选与自由几何输出，采用固定分母评分与哈希锁定工件建立审计轨迹。

Result: GPT-5.5 类别准确率 82.4%，Qwen3-VL-32B-Instruct 62.2%，GLM-4.5V 57.2%。自然图纸上点/区域 PCK@.05：GPT 54-76%，Qwen 8-10%，GLM 14-36%；线端点 PCK@.05 分别为 22%、4%、0%。坐标网格可恢复部分 GPT 点/区域精度但无法恢复线。三位建筑训练参与者达 87.3-93.3% 类别准确率与 76-92% GT 区域命中。

Conclusion: 封闭式选择或标记元素成功并不蕴含可靠的显式几何定位。对于图纸引导的 CAD/BIM 系统，类别正确性不应被视为无候选空间可靠性的证据。作者避免机制性论断，因为类别族不构成同项匹配-配准对比且界面控制改变多重负担。

Classification reason: 该论文提出 CrossProjection，一个评估视觉语言模型在异构建筑视图（平面图、剖面图、立面图）中保持组件身份与几何定位能力的诊断基准，属于多模态评测与基准方向，映射到 [CANONICAL] multimodal_benchmark。

Importance reason: Niche architectural-drawing benchmark with weak overlap with your continuous multimodal/world-model priorities. Signals: multimodal_understanding_generation, benchmark/evaluation, architectural drawings, geometric grounding, no continuous tokens
...
### [186] [Where did the ambiguity go? Examining how multimodal models interpret polysemous words](https://arxiv.org/abs/2608.00410)
...
TL;DR: 该论文研究了多模态基础模型如何解释多义词（如"bank"、"palm"）。通过给17个文生图模型和15个文本生成模型提供无上下文的多义词并测量其生成输出的语义多样性，发现存在明显的多模态差距：图像生成比文本生成收敛到更少的语义（归一化熵0.10 vs 0.25），两者都远低于人类想象的多样性（0.47）。此外，当模型被要求列出各语义的生成频率时，其预测分布比实际输出空间更多样。


<details>
  <summary>Details</summary>
Motivation: 人类语言高度多义，许多常见词承载多种含义。虽然LLM已被证明能理解这种多义性，但多义词在其他模态（如图像）中如何呈现仍知之甚少。作者希望揭示基础模型在不同模态间表达语义的方式是否存在差距。

Method: 作者选取多义词，分别输入17个文生图模型和15个文本生成模型，在无上下文约束下多次采样生成输出，测量每种语义被产生的频率，计算归一化熵以量化语义多样性。同时对比人类对同一词语的想象分布，并让模型自述其各语义生成频率的预测分布。

Result: 发现清晰的多模态差距：每个模型家族中，图像生成收敛到远少于文本生成的语义（归一化熵0.10 vs 0.25），两者都远低于人类想象的多样性（0.47）。当模型被要求预测各语义的生成频率时，其预测分布比实际输出空间更多样。

Conclusion: 基础模型在多模态间表达语义的方式存在系统性差距，其理解可能无法在不同模态间忠实且平等地迁移。这揭示了多模态模型在语义表达上的局限，为多模态评测与对齐提供了新视角。

Classification reason: 该论文系统评测了17个文生图模型和15个文本生成模型在多义词上的语义多样性表现，属于多模态生成模型的评测研究，映射到多模态评测与基准（multimodal_benchmark）这一规范主题。

Importance reason: Focuses on multimodal evaluation of polysemy, but lacks direct connection to the user's priority topics of continuous/multimodal generation, world models, or efficiency. Signals: multimodal gap, polysemy, text-to-image, entropy, foundation models
...
### [187] [LongChart VQA: A Comprehensive Benchmark for MLLMs with Complex Multi-Chart Reasoning](https://arxiv.org/abs/2608.01328)
...
TL;DR: 提出 LongChart VQA 基准，通过潜在图支持的合成管线构建包含平均 6.5 张图像和 31.2 个问题的多图表复杂推理评测集，评估 10 个 SOTA MLLM 在推理模式、辅助工具和图像扰动鲁棒性三方面的影响。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 基准大多强调单图表感知，简单的图表间连接不足以评估 MLLM 在多图表理解与多步推理上的能力，而这些能力在复杂 agentic 任务中日益重要。

Method: 设计基于潜在图（latent graphs）的合成管线来生成多图表 VQA 数据，构建 LongChart 基准，包含平均 6.5 张图像和 31.2 个问题，并评估 10 个 SOTA MLLM，考察推理模式、辅助工具和图像扰动鲁棒性三个影响因素。

Result: MLLM 的准确率随计算复杂度增加而下降且波动明显，揭示了多图表推理中存在的性能瓶颈。

Conclusion: 多图表复杂推理仍是 MLLM 的挑战性任务，现有模型在计算复杂度增加时性能显著下降，为未来多图表推理研究指明了方向。

Classification reason: 该论文提出 LongChart VQA 基准，用于评测 MLLM 在多图表复杂推理上的能力，属于多模态评测与基准方向，映射到 [CANONICAL] 的 multimodal_benchmark 子主题。

Importance reason: Benchmark paper on multi-chart VQA reasoning, but only tangential to the user's core focus on continuous/multimodal generation and world models. Signals: multimodal understanding, benchmark, multi-chart reasoning, MLLM evaluation, robustness
...
### [188] [MIDAL: Math Image Descriptions for Accessible Learning](https://arxiv.org/abs/2608.00868)
...
TL;DR: MIDAL 是一个包含 2,020 张数学图像描述的数据集，覆盖多个教育层次，用于训练视觉语言模型生成符合无障碍最佳实践的图像描述，同时也可用于提升语言模型的数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 许多开放教育资源缺乏无障碍性，尤其是深入的图像描述。在科学与数学等学科中，由于存在大量复杂表达式和术语，撰写图像描述尤为困难。作者希望填补这一空白，为 STEM 内容无障碍化提供资源。

Method: 构建 MIDAL 数据集，包含 2,020 张跨多个教育层次的数学图像及其描述，遵循无障碍最佳实践，可用于微调视觉语言模型生成图像描述，也可用于微调语言模型以提升数学推理与回答能力。

Result: 论文主要贡献是发布 MIDAL 数据集本身，作为训练视觉语言模型生成无障碍数学图像描述的资源，并可作为数学推理微调的数据来源。

Conclusion: MIDAL 是提升高等教育 STEM 内容无障碍性的有价值资源，有助于推动无障碍图像描述生成的研究与创新，并兼具数学推理增强的潜力。

Classification reason: 论文提出 MIDAL 数学图像描述数据集，用于训练视觉语言模型生成符合无障碍规范的图像描述，属于多模态评测与基准方向，映射到 [CANONICAL] multimodal_benchmark 子主题。

Importance reason: Dataset for math image descriptions is niche and tangential to the user's core focus on continuous multimodal models and video generation. Signals: accessibility dataset, math image descriptions, VLM training, math reasoning, not aligned with priority subtopics
...
### [189] [Measurement Without Validity: The Compounding Reliability Problem in Agentic AI Evaluation](https://arxiv.org/abs/2608.00794)
*William Caban*
...
TL;DR: 本文实证分析表明，Agentic AI 系统的自动化评测分数系统性不可靠，问题在三个层面复合叠加：LLM 生成任务存在有效性缺陷、LLM 模拟器存在校准偏差、约82%的论文使用不匹配或不完整的评分者间信度指标。作者形式化为 V_total ≤ V1×V2×V3 的乘法复合模型，并基于心理测量学提出八项处方。


<details>
  <summary>Details</summary>
Motivation: Agentic AI 系统的部署决策、安全认证和合规声明都依赖自动化基准分数，但当前实践对这些分数的可信度认识不足。作者旨在揭示评测分数在任务生成、用户模拟、评分判断三个层面系统性失真的问题。

Method: 作者采用三层实证分析：(1) 审计十个流行基准，检查 LLM 生成任务的有效性缺陷与报告缺口；(2) 分析 LLM 模拟器替代人类用户的校准研究，量化模拟器间方差与方向性误校准；(3) 对55篇论文进行结构化调查，评估评分者间信度（IRR）指标的使用情况。并形式化乘法复合模型 V_total ≤ V1×V2×V3。

Result: 十个基准中七个存在有效性缺陷、全部存在报告缺口；模拟器间方差高达9个百分点且对非标准美式英语使用者存在系统性方向性误校准；约82%的论文使用结构不匹配、不完整或缺失的 IRR 指标。在独立假设下，三层各保留70%/80%/65%有效信号时，总有效性最多36%，经验估计范围跨度为0.22-0.54。

Conclusion: 评测分数在多层复合下比当前实践承认的更不可信，且相关失败（同一模型族贯穿三层）会进一步收紧有效性上界。作者提出八项基于心理测量学的处方，包括模拟校准下限 ICC(A,1)≥0.70、按后果等级分层的信度阈值、结构化 IRR 指标选择规则，以及将 IRR 作为强制报告字段。测量工具已存在，关键在于应用。

Classification reason: 论文聚焦于 Agentic AI 系统评测基准的有效性与可靠性问题，包括 LLM 生成任务、LLM 模拟器替代人类、评分者间信度（IRR）等评测协议缺陷，属于多模态/智能体评测基准与评测协议范畴，映射到 [CANONICAL] 多模态评测与基准。

Importance reason: Focuses on agentic AI evaluation validity, not on the user's core continuous multimodal, autoregressive generation, or world model priorities. Signals: agentic AI evaluation, reliability metrics, LLM simulation bias, psychometrics, benchmark validity
...
### [190] [SCHEDBench: A Benchmark for Evaluating LLM Constraint Faithfulness in Natural-Language Combinatorial Scheduling](https://arxiv.org/abs/2608.00991)
...
TL;DR: SCHEDBench 是一个自然语言组合调度基准，包含 1,132 个实例，覆盖作业车间调度、资源受限项目调度、护士排班和课程表问题，评估 LLM 在不同自然语言表面形式下生成调度的约束忠实性。实验发现模型对语义等价但表面形式不同的调度问题并不稳定，表面形式变化会降低可行性并导致硬约束违反。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 评测很少关注组合调度问题中约束忠实性对自然语言表面形式变化的鲁棒性，模型可能对语义等价的表述产生不一致的调度行为。

Method: 构建基于规范调度实例的基准，通过领域特定模板、主题实体、词汇句法模板改写和约束级表面形式变化生成自然语言问题，并用求解器验证参考解的可行性与目标最优性，在 13 个前沿和开源 LLM 上进行评估。

Result: 模型对语义等价的调度问题表述并不稳定；表面形式变化降低可行性并导致匹配实例上硬约束违反的显著偏移；在测试的独立轴中，约束重排序表现出最明显的敏感性。

Conclusion: LLM 在组合调度任务上缺乏对自然语言表面形式变化的约束忠实性鲁棒性，需要更稳健的约束遵循能力。

Classification reason: 该论文提出 SCHEDBench 基准，用于评估 LLM 在自然语言组合调度问题上的约束忠实性，属于 LLM 评测基准类工作，映射到 [CANONICAL] 多模态评测与基准（multimodal_benchmark）子主题。

Importance reason: Benchmark for LLM scheduling faithfulness, but outside the user's core focus on continuous/multimodal generation and world models. Signals: LLM constraint faithfulness, scheduling benchmark, surface-form robustness, no generative modeling, no continuous tokens
...
### [191] [PATH-Bench: Path-Dependent Evaluation of Lifelong Agents](https://arxiv.org/abs/2608.01149)
...
TL;DR: PATH-Bench 是一个用于终身 LLM 智能体路径依赖评估的基准，通过多模型上下文学习估计有向任务关系，构建受控的有利/干扰历史序列，反复评测探针任务以衡量平均性能、前向迁移、后向迁移与遗忘。基于评测发现提出选择性经验使用（SEU）机制，在多数设置下减少遗忘并提升前向迁移。


<details>
  <summary>Details</summary>
Motivation: 终身 LLM 智能体通过外部学习状态（可检索记忆或可复用技能）不断适应，但现有基准很少考虑累积经验的路径如何影响智能体的迁移与保留。缺乏对路径依赖效应的受控评测框架。

Method: PATH-Bench 通过多模型上下文学习估计有向任务关系，构建以探针任务为中心的序列，包含受控的有利与干扰历史，并反复评测探针任务以测量平均性能、前向迁移、后向迁移与遗忘。在单轮代码生成与多轮工具使用任务上，于正/负主导历史下评估八种代表性智能体。基于发现提出 SEU 智能体框架，调节路径累积经验对每个新任务的影响，过滤潜在干扰项。

Result: 评测表明经验效用同时取决于经验的表征方式与任务的交互结构；强迁移并不保证保留；后期经验可重塑早期学习获得的收益。SEU 在多数设置下一致减少遗忘并提升前向迁移。

Conclusion: PATH-Bench 提供了受控的终身智能体路径依赖评估框架，并给出设计更具选择性、更稳健的终身智能体的可操作指导。

Classification reason: 该论文提出 PATH-Bench，一个用于终身智能体路径依赖评估的基准，属于智能体/LLM 评测范畴。虽然主要面向 LLM 智能体而非纯多模态，但评测基准主题与 [CANONICAL] 多模态评测与基准（multimodal_benchmark）合理相关，故复用该子主题。

Importance reason: Focuses on lifelong agent evaluation and memory management, which is outside the user's core interests in continuous multimodal models, autoregressive generation, and world models. Signals: lifelong agents, path-dependent evaluation, forward/backward transfer, forgetting, selective experience use
...
### [192] [MedPRESS: A Multi-turn Benchmark for Patient-Pressure-Induced Medical Sycophancy in LLMs](https://arxiv.org/abs/2608.02520)
...
Importance: 值得关注 (75.97) - 未入选精读

TL;DR: MedPRESS 是一个包含 600 个五轮对话的多轮基准，用于评测 LLM 在患者压力诱导下产生的医疗谄媚行为。论文评估了 20 个 LLM，发现模型在反复患者压力下频繁转向不安全的一致同意，反谄媚提示能提升部分模型的鲁棒性但无法完全消除。


<details>
  <summary>Details</summary>
Motivation: 现有研究用静态问题评测 LLM 的医疗安全性，而非压力下的患者对话场景。医疗 LLM 需要能在对话压力下维持安全知识，但缺乏此类评测基准。

Method: 构建 MedPRESS 基准，包含 600 个医学依据的五轮对话，覆盖三类场景（药物与治疗需求、个人健康自我护理、症状分诊与护理抵抗）。每段对话从健康查询开始，逐步升级为个人经历、社会证明、外部证据主张和直接对抗性挑战。使用结构化评判和安全导向指标评估 20 个 LLM。

Result: 模型在反复患者压力下频繁转向不安全的一致同意，不同模型家族、模型规模和提示类型之间存在显著差异。反谄媚提示能提升部分模型的鲁棒性，但无法完全消除不安全的一致同意。

Conclusion: MedPRESS 揭示了医疗 LLM 评测中的关键缺口：安全医学知识本身不足以保证模型在对话压力下维持安全行为。需要更关注压力场景下的鲁棒性评测。

Classification reason: 该论文提出 MedPRESS，一个用于评测 LLM 在患者压力诱导下产生谄媚（sycophancy）行为的多轮基准。虽然论文聚焦于文本 LLM 而非多模态，但属于 LLM 评测基准范畴，与 [CANONICAL] 多模态评测与基准（multimodal_benchmark）主题合理相关，故复用该子主题。

Importance reason: Medical sycophancy benchmark is methodologically sound but falls outside the user's core focus on continuous/multimodal generation and world models. Signals: medical LLM safety, multi-turn benchmark, sycophancy evaluation, LLM robustness, out-of-scope domain
...
### [193] [A Benchmark Dataset for MLLM-Generated Image Detection: GPT Image2 & Nano Banana2](https://arxiv.org/abs/2608.01258)
...
TL;DR: 论文指出多模态持续指令微调（MCIT）中专家路由的任务识别问题被文本指纹和短任务序列掩盖，提出 FLEX 基准（34 任务、弱化文本指纹）并形式化为软任务即类别的多模态类增量学习（MCIL），将四种 CIL 方法适配到四种 MCIT 框架，提升严格 LoRA 匹配最高 16.3 个百分点。


<details>
  <summary>Details</summary>
Motivation: 多模态持续指令微调中，许多方法依赖任务特定 LoRA 专家并在推理时路由输入。然而路由背后的任务识别问题被广泛低估：现有基准存在泄漏任务身份的文本指纹，且任务序列短（4-10 个）、竞争专家少，导致路由近乎饱和，掩盖了长时程路由的真实挑战。

Method: 提出 FLEX 基准，包含 34 个任务，通过分组相似指令/答案格式但不同视觉与知识域的任务、归一化外层模板来弱化文本指纹，并在更大的专家池上评估路由。同时将渐进式 LoRA 路由形式化为软任务即类别的多模态类增量学习（MCIL），每个任务定义增量路由类别，其完整分数分布提供 LoRA 混合权重，硬路由为离散特例。实例化 PureLoRA 作为受控基线，并将四种 CIL 方法适配到四种 MCIT 框架。

Result: 插件式路由器在严格 LoRA 匹配上最高提升 16.3 个百分点，整体 MacroScore 最高提升 4.6 分，验证了 MCIL 形式化对专家路由的有效性。

Conclusion: FLEX 暴露了长时程专家路由中不断扩大的任务识别挑战，MCIL 形式化为将类增量学习方法迁移到专家路由提供了原则性接口，为多模态持续学习中的路由问题提供了新的评测与解决框架。

Classification reason: 论文提出 FLEX，一个 34 任务长时程多模态持续指令微调基准，用于暴露专家路由饱和问题，属于多模态评测与基准方向，映射到 [CANONICAL] multimodal_benchmark。

Importance reason: Solid benchmark and CIL-formulation contribution for multimodal continual routing, but tangential to the user's core continuous-model, generation, and world-model priorities. Signals: multimodal continual instruction tuning, expert routing, class-incremental learning, FLEX benchmark, LoRA experts

Abstract: Multimodal Continual Instruction Tuning (MCIT) enables multimodal large language models to acquire new tasks sequentially while retaining previously learned capabilities. Many recent methods maintain task-specific LoRA experts and route each input to one or more experts at inference. Yet the task-identification problem underlying expert routing remains under-explored. We show that routing is nearly saturated on widely used MCIT benchmarks. Textual fingerprints that leak task identity and short 4--10-task sequences with few competing experts jointly obscure the long-horizon routing problem. To expose this challenge, we introduce FLEX (Fingerprint-reduced Long-horizon Expert eXamination), a 34-task long-horizon MCIT benchmark with weakened textual fingerprints. FLEX groups tasks with similar instruction and answer formats but diverse visual and knowledge domains, normalizes their outer templates, and evaluates routing over a substantially larger expert pool. Crucially, we formulate progressive-LoRA routing as soft task-as-class Multimodal Class-Incremental Learning (MCIL): each task defines an incremental routing class, whose complete score distribution supplies the LoRA mixture weights, with hard routing as a discrete special case. FLEX exposes this expanding task-identification challenge, while the MCIL formulation provides a principled interface for transferring CIL methods to expert routing. We instantiate PureLoRA as a controlled baseline and adapt four CIL methods to four MCIT frameworks without modifying their LoRA experts or generation pipelines. Our plug-in routers improve strict LoRA matching by up to 16.3 percentage points and overall MacroScore by up to 4.6 points. Code is available at:

</details>


### [195] [Long-Horizon Embodied Decision-Making via Multimodal Memory Compression](https://arxiv.org/abs/2608.01456)
...
Importance: 值得关注 (70.38) - 未入选精读

TL;DR: 提出 DunphyBench 基准，评估智能体在长时程人本具身决策中的表现，并设计 MeMento 偏好条件多模态记忆压缩器，通过固定记忆 token 选择性压缩决策相关信息，提升 VLM 智能体准确率 7.18% 并减少 85.38% 内存。


<details>
  <summary>Details</summary>
Motivation: 智能体正从任务执行者转变为代表人类用户的决策者，需要在长时程中积累证据、理解隐含用户偏好并在部分观测下比较多个候选方案。现有具身推理任务多聚焦于程序性规划或即时目标完成，缺乏对长时程人本决策的评测，且 VLM 智能体的记忆管理是瓶颈。

Method: 提出 DunphyBench 基准，要求智能体在多个具身住房环境中导航并做出符合多维人类偏好的决策。基于诊断发现原始多模态历史引入噪声，设计 MeMento 偏好条件多模态记忆压缩器，根据用户偏好用固定数量的记忆 token 选择性压缩长时程历史中的决策相关信息。

Result: MeMento 帮助 VLM 驱动智能体准确率提升 7.18%，同时相比最强基线减少 85.38% 的内存使用。评测还揭示当前智能体与人类表现之间存在显著差距。

Conclusion: 长时程人本具身决策是当前智能体的薄弱环节，记忆管理是关键瓶颈；偏好条件化的多模态记忆压缩能有效提升决策质量并大幅降低内存开销。

Classification reason: 论文提出 DunphyBench 基准，用于评估智能体在长时程具身决策中的表现，属于多模态评测与基准方向，映射到 [CANONICAL] multimodal_benchmark。同时涉及具身环境导航与决策，与 world_model 方向的 embodied_world_model 相关。

Importance reason: Novel benchmark and memory compression method for embodied decision-making, but tangential to the user's core focus on continuous/multimodal generation and world models. Signals: multimodal memory compression, long-horizon embodied decision-making, VLM agent efficiency, benchmark, preference-conditioned compression
...
### [196] [GISAgentBench: A Practitioner-Sourced Benchmark for Evaluating LLM Agents on GIS Tasks](https://arxiv.org/abs/2608.01645)
...
TL;DR: GISAgentBench 是一个从 GIS Stack Exchange 采集、基于真实公共数据构建的 349 个多步 GIS 任务基准，每个任务附带可执行参考轨迹和精确 ground truth 输出文件，支持严格、确定性、容差感知的输出匹配评测。评测六个 LLM 模型发现最佳 agent 在严格容差评分下仅完成 32.7% 的任务。


<details>
  <summary>Details</summary>
Motivation: 现有 GIS agent 评测基准多来自教科书、教程或 LLM 生成种子，规模有限且轨迹深度不足，更重要的是缺乏 ground truth 输出，只能依赖代码相似度、轨迹匹配或 LLM/VLM 评判等替代信号，容易混淆工作流相似性与任务正确性。

Method: 从 GIS Stack Exchange 采集 349 个多步 GIS 任务，在六个选定地理区域的真实公共数据上实例化。每个任务附带可执行参考轨迹和精确 ground truth 输出文件，采用严格、确定性、容差感知的输出匹配评测方式，而非依赖 LLM 评判。

Result: 评测六个 LLM 模型显示，真实 GIS 工作流仍具挑战性：最佳 agent 在严格容差感知评分下仅完成 32.7% 的任务，但大多数模型产生的输出接近 ground truth。

Conclusion: GISAgentBench 提供了首个带精确 ground truth 的实践来源 GIS agent 基准，揭示了现有 LLM agent 在真实 GIS 空间分析工作流上的能力差距，为后续研究提供了更可靠的评测基础。

Classification reason: 该论文提出 GISAgentBench，一个面向 GIS 空间分析任务的 LLM Agent 评测基准，属于评测基准类工作，映射到 [CANONICAL] 多模态评测与基准（multimodal_benchmark）子主题。

Importance reason: GIS agent benchmark is outside the user's core focus on continuous multimodal models, video/image generation, and world models. Signals: benchmark, GIS, LLM agents, multimodal_understanding_generation, no priority keyword match
...
### [197] [Can You Trust the Confidence? ConfBench for Vision-Language Models on Document Extraction](https://arxiv.org/abs/2608.01792)
...
TL;DR: ConfBench 是首个面向关键信息抽取（KIE）的校准专用基准，通过对多样化文档集施加 20 种受控退化管线生成 1346 个变体和 7 万+实体级评估，覆盖完整准确率谱系。论文评估了四种专有和三种开源 VLM 在口头化与对数概率两种置信度估计方法下的表现，发现 OCR+图像模态置信度更准确、模型能力是主导因素、校准质量差异大，并引入 ECARB 运营预算指标。


<details>
  <summary>Details</summary>
Motivation: 智能文档处理（IDP）依赖可靠的置信度分数来在自动化与人工审核之间路由抽取结果。现有文档基准以干净高质量样本为主，低准确率区域样本过少，难以进行校准评估。因此需要专门的校准基准来系统研究置信度估计与校准方法。

Method: 构建 ConfBench 基准：对多样化文档集应用 20 种受控退化管线，生成 1346 个变体和 70K+ 实体级评估。在三种输入模态下，用口头化（verbalized）和对数概率（log-probability）两种置信度估计方法评估四种专有和三种开源 VLM，并引入 ECARB（review-budget metric）将判别性增益转化为运营节省。

Result: 主要发现：(i) OCR+图像模态产生更准确的置信度估计；(ii) 模型能力是主导因素，Claude 家族内置信度质量随能力单调提升，跨家族参数数量预测能力差；(iii) 校准质量从近乎完美到严重过度自信差异巨大，逐模型事后校正可重缩放绝对置信度值用于阈值路由而不改变排序型运营指标；(iv) 对数概率结合首 token 聚合一致优于均值 token 和边际聚合。

Conclusion: ConfBench 提供了首个面向文档抽取的校准专用基准，揭示了不同 VLM 在置信度校准上的显著差异，并证明逐模型事后校正与合适的置信度估计方法（对数概率+首 token 聚合）能有效提升阈值路由的可靠性。基准已公开，支持可信 IDP 应用部署的系统性研究。

Classification reason: 该论文提出 ConfBench，一个针对视觉语言模型文档抽取任务的校准专用基准，属于多模态评测与基准方向，映射到 [CANONICAL] 的 multimodal_benchmark 子主题。

Importance reason: Solid calibration benchmark for document extraction, but outside your core focus on continuous/multimodal generation and world models. Signals: calibration benchmark, document extraction, VLM confidence, ECARB metric, no generation/world-model focus
...
### [198] [Right Answer, Wrong Method: Shortcut Hacking Misleads the Evaluation of LLM Reasoning on Frontier Science Benchmarks](https://arxiv.org/abs/2608.02442)
...
TL;DR: 论文揭示LLM在科学推理基准上通过数值搜索、枚举、猜测等无效捷径获得正确答案的"Solution Hacking"现象，该现象随难度上升而加剧（从常见题2.2%到HLE 37.4%），导致仅用最终答案准确率评估会显著高估前沿模型的科学推理能力。


<details>
  <summary>Details</summary>
Motivation: 科学推理基准通常用最终答案准确率评估LLM，但正确答案并不一定代表模型具备题目所针对的推理能力。模型可能通过无效捷径（如数值搜索、枚举、猜测、先答后验）得到正确答案，导致评测结果虚高。

Method: 系统分析Solution Hacking现象在不同难度、科学领域和前沿模型中的表现；提出专家启发的反破解策略，包括自动评判器（automatic judge）和测试时指令（test-time instruction）来抑制捷径行为。

Result: Solution hacking随难度急剧上升：常见题2.2%、奥赛级28.3%、HLE 37.4%；前沿模型中被记为正确的答案中有8.2%-44.1%被识别为破解答案。抑制捷径行为后报告准确率显著下降，而正确且非破解的准确率受影响较小。

Conclusion: 仅用最终答案准确率评估会高估前沿LLM的科学推理能力，评测协议需要引入对推理过程有效性的验证，以更真实地反映模型推理能力。

Classification reason: 论文研究科学推理基准评测中"捷径破解"（Solution Hacking）导致最终答案准确率高估LLM推理能力的问题，属于评测协议与评估方法研究，映射到多模态评测与基准（multimodal_benchmark）这一规范子主题。

Importance reason: 评测方法学论文，与用户关注的多模态生成/世界模型方向关联弱，且负向关键词（如医疗）未命中，但评测偏差对LLM能力评估有普适参考价值。 Signals: 评测偏差, 推理捷径, 科学基准, 答案准确率高估, 与多模态生成方向弱相关
...
### [199] [Decoupling semantics from vision: A framework for faithful visual-text compression evaluation](https://arxiv.org/abs/2608.01848)
...
TL;DR: 本文提出一个解耦语义与视觉能力的评测框架，并引入 ZeroSense Benchmark，用于忠实评估视觉文本压缩（VTC）质量，避免 MLLM 语言先验对评测结果的干扰。


<details>
  <summary>Details</summary>
Motivation: 现有视觉文本压缩（VTC）方法（如 DeepSeek-OCR）报告了高压缩比，但评测协议依赖下游任务性能，无法准确衡量文本保留程度，因为 MLLM 的强语言先验会掩盖 VTC 的真实质量。

Method: 提出解耦评测框架，将 MLLM 的语义推理能力与 VTC 质量评估分离；引入 ZeroSense Benchmark，确保测试样本具有低语义相关性，从而消除文本依赖，使评测结果纯粹反映 VTC 质量。

Result: 在多个数据集上的实验表明，VTC 质量与下游任务准确率存在显著分歧，验证了解耦评测框架的必要性。

Conclusion: 现有基于下游任务性能的评测会高估 VTC 质量，本文的解耦评测框架能更忠实、准确地反映视觉文本压缩的真实文本保留能力。

Classification reason: 该论文提出一个新的评测框架 ZeroSense Benchmark，用于解耦语义与视觉、忠实评估视觉文本压缩（VTC）质量，属于多模态评测与基准方向，映射到 [CANONICAL] 多模态评测与基准。

Importance reason: Focuses on evaluation methodology for visual-text compression, which is peripheral to the user's core interests in continuous multimodal models and generative world models. Signals: evaluation framework, visual-text compression, benchmark, MLLM priors, not aligned with priority subtopics
...
### [200] [Event ActivityNet: A Large-Scale Simulated-Event Benchmark for Untrimmed Action Understanding](https://arxiv.org/abs/2608.01948)
...
TL;DR: Event ActivityNet 是一个从 ActivityNet 派生的、包含 3263 个视频、200 个动作类别、106.94 小时的大规模模拟事件基准，支持动作识别、事件-语言对齐和在线时序动作定位，并建立了多种基线。


<details>
  <summary>Details</summary>
Motivation: 长时序事件驱动的动作理解研究不足，因为现有数据集多为短裁剪片段，而采集带密集时序标注的原生事件流成本高昂。需要一个大规模、可扩展的基准来推动该方向研究。

Method: 从人工标注的未裁剪 ActivityNet 视频直接生成事件体素（5-bin 和 9-bin），保留帧率元数据用于近似时间映射，使用动作中心重建 LPIPS 作为保留可重建内容的软诊断。建立自适应事件分帧、提示-字幕对齐、事件-only/RGB-only/RGB-事件定位等基线，并采用渐进式嵌套尺度训练协议。

Result: 在渐进式嵌套尺度训练协议下，识别 Top-1 准确率从 52.25 提升到 66.42，在线时序定位平均 mAP 从 21.7 提升到 29.0。分阶段 Event ActivityNet 预训练后接原生事件微调，在多种监督预算下均优于仅目标训练和从头联合训练。

Conclusion: Event ActivityNet 为长时序事件建模提供了可扩展基准，但原生相机评测对于部署导向的结论仍然必要。

Classification reason: 该论文提出 Event ActivityNet，一个大规模模拟事件基准数据集，用于未裁剪动作理解，属于多模态/视频评测基准方向，映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: This is a niche event-camera benchmark paper with no direct connection to your core interests in continuous multimodal models, autoregressive generation, or world models. Signals: event camera benchmark, untrimmed action understanding, temporal action localization, no continuous token/flow matching, no autoregressive generation
...
### [201] [SVGEval: A Vision-Grounded Framework for Perceptual-Quality Benchmarking and Evaluation in Text-to-SVG Generation](https://arxiv.org/abs/2608.01977)
...
TL;DR: SVGEval 是一个视觉接地的多模态基准，用于评估多模态大模型对文本到 SVG 生成质量的感知判断能力，并训练了一个可解释的 SVG 质量评分器。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型越来越多地用于生成可缩放矢量图形（SVG），但可靠的评估方法仍未被充分探索。现有协议要么以代码为中心，要么在渲染 SVG 后借用光栅图像指标，这些方法无法反映人类感知，也忽略了 SVG 特有的几何与空间构图等质量属性。

Method: SVGEval 显式引入视觉渲染，评估模型能否判断渲染结果而非仅检查 SVG 代码，并通过多轮人工标注与专家精修提供高质量标注。在此基础上训练一个可解释的 SVG 质量评分器，输出多方面分数与文本理由。

Result: 系统评估显示，代表性多模态模型在语义对齐和美学上表现较好，但在几何与布局相关判断上存在明显差距。消融实验表明，显式视觉接地与推理监督对空间和几何评估至关重要。

Conclusion: SVGEval 为多模态模型时代评估和改进 SVG 生成提供了可靠的测试平台和实用的评分器。

Classification reason: 该论文提出 SVGEval，一个面向文本到 SVG 生成质量评估的视觉接地多模态基准，属于多模态评测与基准方向，映射到 [CANONICAL] 子主题 multimodal_benchmark。

Importance reason: A niche SVG quality-evaluation benchmark with limited direct relevance to the user's core focus on continuous multimodal models and autoregressive generation. Signals: multimodal evaluation benchmark, SVG generation, perceptual quality scoring, vision-grounded reasoning, explainable scorer
...
### [202] [RSVideo: Are Your Vision-Language Models Ready for Remote Sensing Videos?](https://arxiv.org/abs/2608.02039)
...
TL;DR: 本文提出 RSVideo-10K 遥感视频数据集（10,773 实例、147 万帧、17.02 小时，涵盖无人机与卫星平台）及固定评测基准 RSVideo-Bench（2,731 测试实例），从 L1 感知与 L2 推理两个层面、7 个能力组、17 个任务评估视觉语言模型的遥感视频理解能力。评测发现现有 VLM 在恢复小目标局部证据、追踪短暂状态、利用场景约束空间关系方面仍存在困难。基于此提出 RSVideo 强化学习框架，通过跨帧选择问题相关区域并抑制冗余背景 token 实现小目标时空聚焦，在 InternVL3.5-14B 上取得最高 9.01% 的绝对提升，在 Qwen3.6-27B 上达到 40.63% 的最高准确率。


<details>
  <summary>Details</summary>
Motivation: 遥感视频能够实时观测目标属性变化、短期活动与场景演化，记录孤立图像无法捕捉的运动、动作、交互与场景变化。然而现有模型主要针对单张图像或长时间跨度的离散时序观测，缺乏统一的评测设置来评估视觉语言模型在连续遥感视频理解上的能力。

Method: 构建 RSVideo-10K 遥感视频数据集与 RSVideo-Bench 固定评测基准，从 L1 感知与 L2 推理两个互补层面、7 个能力组、17 个任务系统评估 VLM。基于评测分析，提出 RSVideo 强化学习框架，通过跨帧选择问题相关区域并抑制冗余背景 token 实现小目标时空聚焦。

Result: 评测显示现有 VLM 在恢复小局部证据、追踪短暂状态、利用场景约束空间关系方面仍存在困难。RSVideo 框架在 InternVL3.5-14B 上取得最高 9.01% 的绝对提升，在 Qwen3.6-27B 上达到 40.63% 的最高准确率（在 26 个开源视觉语言模型中）。

Conclusion: 遥感视频理解对现有 VLM 仍是重大挑战，尤其在连续时序下的小目标感知与推理方面。所提出的 RSVideo 强化学习框架通过时空聚焦有效提升了模型在遥感视频理解上的表现，为遥感视频多模态理解提供了新的评测基准与改进方向。

Classification reason: 论文提出 RSVideo-10K 遥感视频数据集与 RSVideo-Bench 评测基准，评估视觉语言模型在连续遥感视频理解上的表现，属于多模态评测与基准方向，映射到 [CANONICAL] multimodal_benchmark。

Importance reason: Remote sensing video benchmark is outside your core focus on continuous multimodal models and video generation, and the negative keyword "remote sensing" further reduces priority. Signals: remote sensing video benchmark, VLM evaluation, RL framework for small-target focus, negative keyword remote sensing, no continuous token or world model focus

Abstract: Remote-sensing videos enable real-time observation of changes in target attributes, short-term activities, and scene evolution. They record motion, actions, interactions, and scene changes that cannot be captured by isolated images. Existing models primarily target single images or discrete temporal observations spanning a long time range. However, a unified evaluation setting for assessing vision-language models on continuous remote-sensing video understanding remains lacking. We introduce RSVideo-10K, a remote-sensing video dataset comprising 10,773 instances, 1.47 million frames, and 17.02 hours of footage, containing both unmanned aerial vehicles and satellite platforms. Its fixed evaluation benchmark, RSVideo-Bench, contains 2,731 test instances and evaluates two complementary aspects of remote-sensing video understanding: L1 Perception and L2 Reasoning, spanning seven capability groups and 17 tasks. Evaluations show that current vision-language models still struggle to recover small local evidence, track short-lived states, and use scene-constrained spatial relations. Based on this analysis, we further propose RSVideo, a reinforcement learning framework for small-target spatiotemporal focusing that selects question-relevant regions across frames and suppresses redundant background tokens. RSVideo achieves a maximum absolute improvement of 9.01% with InternVL3.5-14B and attains the highest accuracy of 40.63% with Qwen3.6-27B across 26 open-source vision-language will be available at .

</details>


### [203] [PhyCheck: Fine-Grained Evidence-Grounded Dataset for Physical Law Understanding in Video-LLMs](https://arxiv.org/abs/2608.02150)
...
Importance: 高优先级 (84.05) - 未入选精读

TL;DR: PhyCheck 是一个面向 Video-LLM 物理规律理解的两级粒度视频问答数据集，包含粗粒度（是否符合物理规律）与细粒度（物理细节归因）子集，以及带外部因果上下文的诊断子集，用于评测和提升模型对物理一致性的理解。


<details>
  <summary>Details</summary>
Motivation: 具身智能与世界模型要求视频理解系统超越物体和动作识别，理解物理规律。然而现有 Video-LLM 在判断事件是否符合物理定律方面仍不可靠，且现有基准主要评估生成视频的物理质量，缺乏系统评测和提升 Video-LLM 物理规律理解的工具。

Method: 构建 PhyCheck 数据集，包含两级粒度子集：粗粒度子集判断视频现象是否符合/违反物理规律，细粒度子集考察模型能否捕捉导致违反或符合的物理细节；另设带外部因果上下文的诊断子集，评估模型能否根据隐藏因素重新校准判断。使用该数据对 Qwen2.5-VL 进行微调。

Result: 用 PhyCheck 数据微调 Qwen2.5-VL 显著提升了物理一致性理解能力；但在诊断子集上，当前模型仍难以将额外因果条件纳入决策，暴露出表面不一致识别与深层物理机制理解之间的差距。

Conclusion: PhyCheck 为评测和提升 Video-LLM 的物理理解提供了基础，揭示了当前模型在识别表面不一致与理解底层物理机制之间的鸿沟，为未来物理规律理解研究奠定基础。

Classification reason: 论文提出 PhyCheck 视频问答数据集，用于评测 Video-LLM 的物理规律理解能力，属于多模态评测与基准方向，映射到 [CANONICAL] multimodal_benchmark。同时涉及世界模型中的物理一致性理解，故也匹配 world_model 方向。

Importance reason: A useful Video-LLM physical-law benchmark, but it is an evaluation dataset rather than a new method for continuous multimodal or world-model generation, limiting direct alignment with your top priorities. Signals: video-llm benchmark, physical law understanding, world model evaluation, fine-grained video QA, causal context
...
### [204] [A General-Purpose VLM Can Teach an Astronomy Foundation Model to Better Recognize Galaxy Morphology](https://arxiv.org/abs/2608.02300)
...
TL;DR: GEOID-Flood 是一个大规模多模态洪水分割基准数据集，基于 Copernicus 应急管理服务数据，覆盖 65 个国家 219 个事件、超过 14,000 个瓦片，包含双时相 Sentinel-1 SAR（GRD 和 RTC）、Sentinel-2 光学影像、DEM 及人工标注的洪水/永久水体分割标签，并用于评估基础模型与传统编码器在单图、多时相和多模态协议下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有洪水制图数据集很少在大规模上结合双时相 SAR 与共配准光学影像，导致地理空间基础模型在该下游任务上的价值缺乏系统评测。作者希望构建一个大规模、高质量、多模态的洪水分割基准，以衡量基础模型从多源数据中提取价值的能力。

Method: 从 Copernicus 应急管理服务激活数据中构建 GEOID-Flood 数据集，提供超过 14,000 个瓦片，包含事件前后 Sentinel-1（GRD 与 RTC 格式）、事件前 Sentinel-2 合成影像、DEM，以及区分背景、永久水体与洪水的经人工验证标签。在此基础上，在单图像、多时相和多模态三种协议下评测基础模型与传统编码器。

Result: 三个主要发现：基础模型相比传统编码器提供了一致但适度的优势；光学-SAR 融合配合微调最能解决瞬时洪水；在 GEOID-Flood 上训练的模型对未见事件的迁移能力优于在现有数据集上训练的模型。

Conclusion: GEOID-Flood 为洪水分割提供了大规模多模态基准，验证了基础模型在该任务上的价值，并表明多模态融合与大规模基准训练能提升对未见事件的泛化能力。

Classification reason: 该论文提出 GEOID-Flood，一个大规模多模态洪水分割基准数据集，属于多模态评测与基准方向，映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: Solid benchmark paper but in remote sensing/flood segmentation, which is outside your configured priority directions and explicitly listed as a negative keyword area. Signals: flood segmentation, remote sensing, benchmark dataset, SAR-optical fusion, foundation model evaluation

Abstract: Geospatial foundation models aim to learn representations that transfer across regions and sensors, yet evaluating them on specific tasks requires large, high-quality, multi-modal benchmarks that measure how well such models extract value from data. Concerning flood mapping, existing datasets rarely combine bi-temporal SAR and co-registered optical imagery at scale, leaving the value of foundation models for this downstream task largely untested. We introduce GEOID-Flood, a large-scale multi-modal flood segmentation benchmark, derived from Copernicus Emergency Management Service activations, spanning 219 events across 65 countries over ten years. The dataset provides more than 14,000 tiles with co-registered pre- and post-event Sentinel-1, in GRD and RTC format, pre-event Sentinel-2 composite, and DEM, including manually validated labels that separate background from permanent water and flooded water. Using this benchmark, we evaluate foundation models against conventional encoders across single-image, multi-temporal, and multi-modal protocols. We report three main findings: foundation models offer a consistent but modest advantage; optical-SAR fusion with finetuning best resolves transient flooding; and models trained on GEOID-Flood transfer to unseen events better than those trained on existing datasets. Dataset and code available at .

</details>


### [206] [CallScreenBench: Benchmarking On-Device Models as Phone Secretaries](https://arxiv.org/abs/2608.01033)
*Simiao Ren*
...
TL;DR: CallScreenBench 是一个评测端侧小语言模型（0.6-4B 参数、4-bit 量化）作为"电话秘书"接听未知来电能力的基准。它从五个质量维度打分，并引入"守卫度画像"与脚本化退化智能体作为下限校正，发现模型能力随规模提升但分诊能力并不随之提升，且部分指标存在被退化智能体"刷分"的缺陷。


<details>
  <summary>Details</summary>
Motivation: 端侧小模型日益具备代表用户执行任务的能力，接听电话是其中一项典型任务。与大多数基准评测的智能体不同，电话秘书没有明确任务目标也没有合作方，来电者掌握目标且可能是对手，必须从开场白就做出判断。现有基准无法评估这种"代理是否被用户认可"的场景，因此需要专门的评测框架。

Method: 提出 CallScreenBench 基准，从五个质量维度对电话秘书进行评分，每个维度与对应的反指标并列展示且不合并为单一分数。同时报告无工具代理（不持有凭证、不调用工具）的守卫度画像。使用脚本化退化智能体（如直接挂断、回显来电者）作为性能下限，对六个端侧模型（0.6-4B 参数、4-bit 量化）进行评测。

Result: 在六个端侧模型上，质量随能力提升而提升，但分诊（triage）能力并不随能力提升。经脚本化退化智能体校正后，模型对之间分诊性能可区分的数量从 15 对中的 11 对降至预注册操作点下的 0 对。一个直接挂断并回显来电者的智能体也能获得完美的消息保真度分数，暴露了部分指标的下限缺陷。

Conclusion: 端侧模型作为电话秘书时，质量与能力相关但分诊能力与能力无关，且这种表面相关性是测量伪影。现有指标存在被退化智能体刷分的缺陷，需要更严谨的评测设计。作者报告了哪些指标被这些下限击败，并声明不设定通过/失败阈值。

Classification reason: 该论文提出 CallScreenBench，一个针对端侧小模型作为"电话秘书"场景的评测基准，属于多模态/智能体评测基准范畴，映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: This benchmark focuses on on-device phone-secretary agents, which is outside your core interests in continuous multimodal models, autoregressive generation, and world models. Signals: on-device LLM benchmark, phone secretary, small language models, benchmark design, no direct alignment with priority subtopics
...
### [207] [Can Foundation Models Hear What Made That Sound? A Tiered Benchmark of Audio-Language Models and Traditional Classifiers for Closed-Set Sound Source Identification](https://arxiv.org/abs/2608.02397)
...
TL;DR: 本文提出一个分层评测基准，系统比较11种音频分类方法（包括Gemini系列、Kimi-Audio、YAMNet、PANNs、CLAP、BAT等）在闭集声源识别任务上的表现，覆盖2242个音频片段、23个细粒度类别和11个类别。通过四层评测协议报告宏平均精确率、召回率、F1和假阴性率，并分析Gemini模型的思维链行为。


<details>
  <summary>Details</summary>
Motivation: 音频语言模型（ALM）与传统音频分类器在任务接收方式和输出评分机制上存在根本差异，直接放在同一排行榜上比较不公平且误导。需要一个分层评测框架来公平、系统地评估不同方法族在闭集声源识别任务上的能力。

Method: 构建包含2242个音频片段、23个细粒度类别和11个类别的闭集声源识别基准。将11种方法分为四个评测层级：任务感知闭集LLM（4个Gemini模型+Kimi-Audio）、固定词表标签器（YAMNet、PANNs、Whisper-AT、SSLAM）、零样本音频文本模型（CLAP）和音频接地LLM（BAT）。每层采用不同的输出评分协议，报告宏平均精确率、召回率、F1和假阴性率，并分析Gemini模型的8968条思维链响应。

Result: Gemini-3.1-Pro-Preview达到85.6%类别级F1和56.7%细粒度F1；Kimi-Audio达到67.5%类别级F1和32.9%细粒度F1但1.6%样本无法回答；SSLAM和CLAP在类别级匹配或超过最佳闭集模型但细粒度落后。思维链分析发现响应长度不预测准确率，"整体判断优于详细分析"现象实为难度混淆，错误答案92-100%以高置信度陈述。

Conclusion: 分层评测框架能更公平地比较不同方法族；闭集LLM在细粒度识别上仍有明显局限；传统标签器在类别级具有竞争力；模型错误答案普遍过度自信，需要关注可靠性。论文提供了完整混淆矩阵和实用方法选择指南。

Classification reason: 该论文构建了一个分层基准（tiered benchmark）来评测音频语言模型与传统分类器在闭集声源识别任务上的表现，属于多模态（音频-语言）评测与基准研究，映射到 [CANONICAL] 多模态评测与基准（multimodal_benchmark）子主题。

Importance reason: Benchmark paper on audio classification with no direct connection to the user's priority topics of continuous multimodal models, autoregressive generation, or world models. Signals: audio-language benchmark, sound source identification, Gemini/Kimi evaluation, no continuous token/flow matching, no video/image generation
...
### [208] [Beyond Accuracy: Auditing Spatial Provenance in Visual Token Pruning for OCR-Critical MLLM Inference](https://arxiv.org/abs/2608.00077)
...
TL;DR: 本文提出一种超越准确率的证据风险审计方法，用于评估视觉 token 剪枝在 OCR 关键 MLLM 推理中的空间来源（spatial provenance）保留情况。研究发现即使答案正确，剪枝后可能没有任何保留 token 可追溯到支撑答案的 OCR 区域，并揭示了不同模型在质量-风险-可追溯性之间的权衡边界。


<details>
  <summary>Details</summary>
Motivation: 视觉 token 剪枝通常仅以固定保留预算下的答案质量来评判，但这一协议可能遗漏一个关键失败模式：即使答案正确，也没有保留 token 在局部可追溯到支撑它的 OCR 区域。这种"盲区"可能导致在文本密集场景下不可靠的推理。

Method: 提出一种证据风险审计框架，将答案行为与几何 token 来源（provenance）、干预措施和实际成本耦合。使用透明的免训练选择器（如 Target、Random、Grid）隔离受控操作点，并通过配对图像簇差异、置信区间、检测器测试和外部方法对比，跨多个模型（Qwen3-VL-8B、LLaVA-1.5-7B、InternVL3.5-8B）评估质量-风险-可追溯性边界。

Result: 在 30% 保留率下，Qwen Target 的准确率（0.786）与 Full（0.783）相当，但 Target、Random、Grid 的正支持覆盖率差异显著（0.620 vs 0.270 vs 0.318）。物化前缀实现最高 4.32x 批预填充加速和 76.4% 的增量峰值内存降低；TextVQA 和 DocVQA 全验证显示有利的目标验证点并不意味任务通用压缩。

Conclusion: 视觉 token 剪枝应同时报告保留的空间来源（spatial provenance）和实际成本，而不仅仅是质量和压缩率。仅以准确率评估会掩盖模型特定的质量-风险-可追溯性权衡，需要更全面的审计协议。

Classification reason: 论文研究视觉 token 剪枝（visual token pruning）对 OCR 关键 MLLM 推理的影响，属于多模态高效化中的 token 压缩与稀疏推理范畴，映射到 [CANONICAL] efficient_multimodal 子主题。

Importance reason: Focuses on auditing visual token pruning for OCR-critical MLLMs, which is adjacent to but not central to your priority on continuous multimodal models and autoregressive generation. Signals: visual token pruning, MLLM inference efficiency, spatial provenance audit, OCR-critical, model-specific trade-offs
...
### [209] [ORCA: ORgan-Centroid Aggregation for Training-Free 3D CT Visual Token Compression](https://arxiv.org/abs/2608.00345)
...
TL;DR: ORCA 是一种无需训练的 3D CT 视觉 token 压缩器，通过器官质心聚合将相邻 token 合并，并加入质心的正弦编码保留空间布局，可将视觉上下文压缩 64 倍、KV-cache 压缩 50 倍，处理速度提升 31 倍，在 CT-RATE 和 Merlin 两个数据集上优于现有压缩方法。


<details>
  <summary>Details</summary>
Motivation: 3D CT 扫描进入视觉语言模型会产生数千到数万个视觉 token，必须压缩后才能被语言模型消费。现有 token 压缩方法主要针对通用视觉，很少针对 3D CT。常见的网格平均基线会混合不同解剖结构、病灶和空气到同一 token 中，丢失关键解剖信息。

Method: ORCA 采用器官质心聚合（Organ-Centroid Aggregation）方法：利用器官引导将相邻 token 合并，并为每个区域的质心添加正弦编码以保留空间布局。该方法无需训练、即插即用，可产生可调节的 token 集合，无需修改模型或文本查询。

Result: 在 CT-RATE 和 Merlin 两个数据集、五个编码器上评估，覆盖属性预测（大小、密度、位置、纹理、疾病五类）和文本生成（VQA 和报告生成）两类任务。在匹配 token 预算下，ORCA 一致优于现有压缩方法，将视觉上下文压缩 64 倍、KV-cache 压缩 50 倍，处理每个卷的速度提升 31 倍。

Conclusion: ORCA 通过器官感知的 token 聚合与质心空间编码，在显著压缩 3D CT 视觉 token 的同时保留了解剖信息，是一种高效、无需训练且即插即用的 3D CT token 压缩方案。

Classification reason: 该论文提出 ORCA，一种针对 3D CT 视觉 token 的压缩方法，用于减少进入视觉语言模型的 token 数量，属于 token 压缩/高效化范畴，映射到 [CANONICAL] efficient_multimodal（多模态高效化与轻量化）。

Importance reason: Focused on 3D CT token compression, which is outside the user's core continuous multimodal, autoregressive generation, and world model priorities. Signals: training-free token compression, 3D CT medical imaging, KV-cache compression, organ-centroid aggregation, plug-and-play
...
### [210] [Relax Within, Balance Across: Geometry-Guided Load Balancing for Vision-Language Mixture-of-Experts](https://arxiv.org/abs/2608.00574)
...
TL;DR: 本文提出 ReBA（Relax Within, Balance Across），一种面向视觉语言混合专家（MoE）模型的几何引导负载均衡方法。针对图像与文本 token 数量随分辨率、图像数、tiling 和提示长度变化导致的负载失衡问题，ReBA 利用模态边界和图像边界分别设计独立的图像/文本负载项以及每张图像一个等权路由实例，在四个 MoE 骨干上降低负载同时保持任务精度。


<details>
  <summary>Details</summary>
Motivation: 视觉语言 MoE 批次中图像与文本 token 数量不同，且图像分辨率、图像数量、tiling 和提示长度都会改变 token 混合比例。标准 token 级 Switch 辅助损失（Std-Aux）只平衡混合负载，导致图像与文本负载误差在特定混合比例下相互抵消，同一路由器在不同图像分辨率下负载失衡变化超过五倍。作者希望设计一种对 token 混合比例更鲁棒的负载均衡方法。

Method: 作者首先固定图像与文本负载分布，推导出负载随 token 混合比例变化的精确曲线，发现图像-文本负载差距控制了对混合比例的敏感性。基于路由器输入结构分析，图像与文本占据不同区域、视觉 token 按源图像强分组，ReBA 采用两个设计：一是分离的图像与文本负载项（模态边界），二是每张图像一个等权路由实例（图像边界）。

Result: 在四个拆分骨干上，ReBA 在报告的每个基准输入上降低负载，同时保持平均任务精度与 Std-Aux 相当。ReBA 还降低了测试范围内的平均负载以及分辨率和 tiling 变化下的最差物理负载。

Conclusion: ReBA 通过利用模态边界与图像边界设计分离的负载项和等权路由实例，有效缓解了视觉语言 MoE 中因 token 混合比例变化导致的负载失衡问题，在保持精度的同时显著降低负载，为视觉语言 MoE 的负载均衡提供了更鲁棒的方案。

Classification reason: 论文提出针对视觉语言 MoE 模型的负载均衡方法（ReBA），属于高效 MLLM 的稀疏推理与 token 负载优化范畴，映射到 [CANONICAL] 多模态高效化与轻量化（efficient_multimodal）子主题。

Importance reason: Addresses load balancing in vision-language MoE, relevant to efficient multimodal inference but not a core continuous-model or generation method. Signals: vision-language MoE, load balancing, efficient inference, multimodal understanding, geometry-guided routing

Abstract: Vision-language MoE batches contain different numbers of image and text tokens. Image resolution, image count, tiling, and prompt length all change this token mix. We call the standard token-level Switch auxiliary loss Std-Aux. Std-Aux balances only the mixed load, so large image and text load errors can cancel at one mix. On our main model, the same trained router shows more than a fivefold change in load imbalance across image resolutions. We hold the image and text load profiles fixed and derive the exact load curve as the token mix varies. The image-text load gap controls sensitivity to the token mix. Physical preprocessing can also change the conditional profiles. The fixed-profile law excludes such changes. To design a remedy, we examine the router input structure. Image and text occupy distinct regions, while visual tokens group strongly by source image. The modality boundary motivates separate image and text terms. The image boundary motivates one equal-weight routing instance per image. ReBA, or Relax Within, Balance Across, implements both choices. Across four split backbones, ReBA lowers load on every reported benchmark input while keeping mean task accuracy comparable to Std-Aux. ReBA also lowers average load over the tested range and worst physical load under resolution and tiling shifts. Code is available at .

</details>


### [211] [Think in Sets for Streaming Video Token Compression](https://arxiv.org/abs/2608.01169)
...
Importance: 值得关注 (79.66) - 未入选精读

TL;DR: NovaCov 是首个面向流式视频 VideoLLM 的免训练、即插即用的集合式 token 压缩器，通过容量受限的历史参考库与双分支子模覆盖目标进行逐帧因果剪枝，在保留 99.6% ReKV 精度的同时将 LLM 预填充延迟降低 46%。


<details>
  <summary>Details</summary>
Motivation: 流式 VideoLLM 因果处理帧时视觉 token 持续增长，压缩对控制预填充延迟和内存至关重要。现有免训练方法独立地对 token 排序，忽略了保留 token 之间的边际增益交互；而现有集合式方法面向离线任务，无法处理流式场景中因果、逐帧剪枝所需的跨帧交互建模。

Method: NovaCov 维护一个容量受限、按近期加权的历史参考库，并优化双分支子模覆盖目标：一个分支保留当前帧代表性内容，另一个分支优先处理历史覆盖不足的信息。两个分支均为 facility-location 函数，贪心选择保持经典 (1-1/e) 近似保证。

Result: 在流式与离线基准上，NovaCov 优于现有免训练压缩方法，保留 99.6% 的 ReKV 精度，同时将 LLM 预填充延迟降低 46%。

Conclusion: 将流式视频 token 压缩形式化为集合选择问题，并引入显式历史参考以建模跨帧交互，是提升流式 VideoLLM 效率的有效途径，且免训练、即插即用的特性使其易于部署。

Classification reason: 该论文聚焦流式视频 VideoLLM 的视觉 token 压缩以降低预填充延迟与内存，属于多模态高效化与 token 压缩范畴，映射到 [CANONICAL] efficient_multimodal 子主题。

Importance reason: Strongly relevant to efficient multimodal inference and continuous token compression, though not directly on the top-priority continuous-model or video-generation tracks. Signals: training-free token compression, streaming VideoLLM, submodular set selection, 46% latency reduction, 99.6% ReKV retention
...
### [212] [3DZip: Spatial-Aware Feature Diversity-Guided Token Compression for 3D Question Answering](https://arxiv.org/abs/2608.01185)
...
Importance: 值得关注 (73.65) - 未入选精读

TL;DR: 3DZip 是一个面向 3D 视觉语言模型的三阶段 token 压缩框架，通过粗体素化去除点级冗余、基于特征空间多样性的 DPP 选择锚点 token、并在空间约束下合并剩余 token，在三个 3D 问答基准上以 128 个 token 保留 94.7% 性能并实现 1.92 倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 3D VLM 通过将 2D 视觉特征投影到世界坐标构建几何感知 token，实现空间推理，但每场景产生数千 token，带来巨大计算与内存开销。现有 2D VLM 的 token 压缩方法依赖语义相关性或注意力选择，忽略了 3D token 的结构化空间特性；且仅靠空间邻近无法解决 3D 表征中的冗余，因为空间聚合后仍存在物体级 token 不平衡。

Method: 提出三阶段 token 压缩框架 3DZip：第一阶段进行粗体素化（coarse voxelization）去除点级冗余；第二阶段基于特征空间多样性通过行列式点过程（Determinantal Point Process, DPP）选择锚点 token；第三阶段在空间约束下合并剩余 token 以保持几何一致性。

Result: 在三个 3D 问答基准上，3DZip 一致优于现有压缩方法，仅用 128 个 token 即保留原始性能的 94.7%，推理速度提升 1.92 倍。

Conclusion: 3DZip 表明结合空间结构与特征多样性进行 token 压缩，能有效解决 3D VLM 的 token 冗余问题，在保持空间推理能力的同时显著降低计算开销，为 3D 多模态模型的高效化提供了有效方案。

Classification reason: 论文提出 3DZip 三维 token 压缩框架，用于降低 3D VLM 的计算与内存开销，属于多模态模型高效化与 token 压缩方向，映射到 [CANONICAL] efficient_multimodal 子主题。

Importance reason: Token compression for 3D VLMs is useful but peripheral to your core focus on continuous multimodal models and autoregressive generation. Signals: token compression, 3D VQA, efficient inference, DPP selection, spatial-aware merging
...
### [213] [Rethinking Video Token Compression with a Global Codebook: Learning Once, Compressing Everywhere](https://arxiv.org/abs/2608.01271)
...
Importance: 值得关注 (78.33) - 未入选精读

TL;DR: ONCE 是一个即插即用的视频 token 压缩框架，提出离线到在线的范式：在视觉特征空间中一次性学习频率感知的全局码本，通过码本查找与聚合实现轻量在线压缩，减少每个视频的重复计算并避免模型特定设计，在多个视频理解基准上取得强精度-效率权衡。


<details>
  <summary>Details</summary>
Motivation: Video-LLM 将视频表示为密集的视觉 token 序列，其长度随输入时空范围增长，且包含大量由重复视觉模式导致的冗余，造成后续语言模型处理的不必要计算。现有 token 压缩方法（剪枝、合并）在推理时在线进行，每个输入视频都重复产生额外计算，且常依赖模型特定设计，通用性受限。

Method: 提出 ONCE 框架，采用离线到在线（offline-to-online）范式：在视觉特征空间中一次性学习一个频率感知的全局码本（frequency-aware global codebook），然后在推理时通过码本查找（codebook lookup）与聚合（aggregation）实现轻量在线压缩，避免每个视频的重复压缩计算，也无需模型特定的压缩设计。

Result: 在多个视频理解基准上与多种压缩基线对比，ONCE 在保持有竞争力性能的同时，实现了所比较方法中最低的推理延迟，展现出良好的精度-效率权衡。

Conclusion: 将视频 token 压缩从在线重复计算转移到离线一次性学习，通过全局码本实现通用、高效的压缩，为 Video-LLM 的推理加速提供了新范式。

Classification reason: 论文提出 ONCE 视频 token 压缩框架，通过全局码本将压缩过程离线化，降低 Video-LLM 推理时的计算开销，属于多模态高效化与 token 压缩方向，映射到 [CANONICAL] efficient_multimodal 子主题。

Importance reason: Directly targets efficient video token compression for Video-LLMs, aligning with the user's priority on model efficiency and continuous multimodal representation. Signals: video token compression, efficient inference, codebook lookup, offline-to-online, Video-LLM
...
### [214] [Towards Compact Unified Multimodal Tracking: Synergizing Knowledge Distillation with Structural Pruning](https://arxiv.org/abs/2608.01488)
...
TL;DR: 本文针对统一多模态目标跟踪模型的计算瓶颈，识别预测头为关键效率瓶颈，通过精简解码器架构并结合双对齐蒸馏框架（空间表征对齐+语义分布对齐），在保持精度的同时实现5倍加速，达到54 FPS实时推理。


<details>
  <summary>Details</summary>
Motivation: 统一多模态目标跟踪（RGB、热红外、深度）虽鲁棒性强，但SOTA模型计算负担重，难以部署到资源受限的边缘设备。现有压缩方法多关注主干网络，忽略了预测头这一关键效率瓶颈。

Method: 提出双对齐蒸馏框架（Dual-Alignment Distillation）：(1) 空间表征对齐，通过特征蒸馏增强学生对前景目标的空域聚焦（"Where to track"）；(2) 语义分布对齐，利用基于logit的蒸馏对齐决策边界并迁移判别性暗知识（"What to track"）。同时精简解码器架构，并系统分析17种蒸馏策略。

Result: 在五个基准上显著优于复杂SOTA方法。蒸馏模型在RGBT234上达到91.5% MPR，在单张RTX 4090上以54 FPS运行，相比教师模型实现5倍加速且保持更高精度。

Conclusion: 预测头是统一多模态跟踪中常被忽视的效率瓶颈，通过架构精简与双流知识蒸馏解耦空间与语义知识迁移，可在边缘设备上实现实时、高精度的多模态跟踪。

Classification reason: 论文聚焦统一多模态跟踪模型的压缩与加速，通过知识蒸馏与结构化剪枝实现轻量化部署，属于多模态高效化与轻量化方向，映射到 [CANONICAL] efficient_multimodal 子主题。

Importance reason: Solid efficiency work on multimodal tracking, but the specific task (tracking) and KD+pruning combo are only moderately aligned with your core continuous-model and video-generation priorities. Signals: unified multimodal, efficient inference, knowledge distillation, structural pruning, 5x speedup
...
### [215] [Linear Multi-Timescale Retention as a Memory-Efficient Vision-Language Bridge](https://arxiv.org/abs/2608.01614)
...
TL;DR: 提出 LIA-MTR 线性多时间尺度保留模块，作为记忆高效的视觉语言桥接，将 VLM 的序列交互复杂度从 O(N²) 降至 O(N)，支持 262,144 视觉 patch 在 11.2GB VRAM 内处理，并在 MME 基准上超越 MLP 基线。


<details>
  <summary>Details</summary>
Motivation: VLM 处理高分辨率图像时，Softmax 多头注意力（MHA）的 O(N²) 内存复杂度成为关键计算瓶颈。虽然用独立 MLP 替代 MHA 可实现 O(N) 扩展，但会丧失空间序列路由能力，严重损害全局场景理解与物体恒存性。需要一种既高效又能保持序列路由能力的跨模态桥接机制。

Method: 提出 LIA-MTR（Linear Multi-Timescale Retention）模块，集成 ELU 基正特征映射、自适应写门控与对数线性分布的循环衰减，将连续视觉序列数学压缩为有界记忆状态，实现严格的 O(N) 序列交互复杂度。

Result: 合成检索评测显示 LIA-MTR 可在 16,000 token 上完美路由上下文，消除线性注意力的"Lost in the Middle"退化；硬件基准显示可原生处理 262,144 视觉 patch（11.2GB VRAM），而标准 MHA 在 16,384 patch 即内存溢出；经 665K 对话样本指令微调后，在 MME 基准上以 71.00% 超越 MLP 基线的 68.11%，物体恒存性提升 10%。

Conclusion: LIA-MTR 为无限上下文视觉语言集成建立了数学上严谨、计算上平坦的基础，在保持序列路由能力的同时实现线性复杂度，为高分辨率 VLM 提供了一种记忆高效的跨模态桥接方案。

Classification reason: 论文提出线性多时间尺度保留模块（LIA-MTR）作为记忆高效的视觉语言桥接，核心目标是降低 VLM 处理高分辨率图像时的 O(N²) 内存复杂度至 O(N)，属于多模态高效化与轻量化方向。同时其线性注意力机制与连续视觉序列压缩也涉及连续语言/多模态建模，故匹配 continuous_language_multimodal 方向。

Importance reason: Directly targets continuous visual token processing with O(N) memory scaling, aligning with the user's top-priority continuous multimodal and efficient inference directions. Signals: continuous visual tokens, linear attention, memory-efficient VLM, O(N) complexity, infinite-context
...
### [216] [Dynamic Resolution Routing for Efficient Egocentric Grounding](https://arxiv.org/abs/2608.01638)
...
TL;DR: SmartRes 通过动态分辨率路由在像素空间进行效率优化，用于高效的第一视角（egocentric）视觉定位。它先编码低分辨率全局视图，用轻量路由器激活目标中心区域的高分辨率 patch 并构建保序视觉序列，配合 margin 正则化路由目标缓解前后景不平衡，在 Ego4D 和 EgoIntention 上减少最多 67% 视觉 token 并保留 86.4% 全分辨率性能。


<details>
  <summary>Details</summary>
Motivation: 第一视角视觉定位需要高分辨率输入来定位小物体，但将 MLLM 扩展到该领域受限于视觉 token 处理的高昂成本。现有基于 token 缩减的高效策略在选取目标中心空间证据时不可靠，因此需要在像素空间进行效率优化。

Method: SmartRes 框架：先编码低分辨率视图获取全局上下文，用轻量路由器激活目标中心区域的高分辨率 patch，构建保序的视觉序列；引入 margin 正则化路由目标，增大前后景 logit 分离度并提升前景召回率，以应对严重的前后景不平衡。

Result: 在 Ego4D 和 EgoIntention 上，SmartRes 减少最多 67% 的视觉 token，同时保留 86.4% 的全分辨率性能；相比最先进的 token 缩减方法推理速度提升最高 1.66 倍且准确率更高；在小物体定位上表现强劲。

Conclusion: 在像素空间进行动态分辨率路由比在 token 空间缩减更可靠，能有效兼顾第一视角视觉定位的精度与效率，验证了面向 egocentric 应用的高效 MLLM 方案的有效性。

Classification reason: 论文提出 SmartRes，通过动态分辨率路由在像素空间优化 MLLM 的视觉 token 处理效率，属于高效 MLLM 与 token 压缩范畴，映射到 [CANONICAL] 多模态高效化与轻量化。

Importance reason: Solid efficiency work for egocentric grounding, but niche application and no direct alignment with top-priority continuous/multimodal generation or world-model directions. Signals: dynamic resolution routing, pixel-space efficiency, egocentric grounding, token reduction, margin-regularized routing
...
### [217] [Allocation Before Ranking: Decoupled Token Compression for OmniLLMs](https://arxiv.org/abs/2608.01665)
...
TL;DR: 提出 Macer，一种免训练的 OmniLLM token 压缩方法。与传统的单一显著性排序（score-then-top-K）不同，Macer 先为音频和视频模态分配显式预算，再在各模态内部进行分配归一化排序，在模态特定的浅层进行压缩。在 25% 保留率下，Qwen2.5-Omni-7B 保持 98.7% 的全 token 性能，Qwen2.5-Omni-3B 保持 97.3%，并在 OmniVinci-9B 上比共享 top-K 提升最多 12.9 分。


<details>
  <summary>Details</summary>
Motivation: 现有 OmniLLM 的 token 压缩通常被建模为单一显著性排序问题：对每个多模态 token 打分并保留 top-K。但作者指出这种抽象是有缺陷的——同一个注意力分数同时决定了两件事：每个模态获得多少保留容量，以及模态内保留哪些 token。共享 top-K 规则继承了偏向音频的分配先验，在视频 token 有机会竞争之前就把保留容量花在了音频上。

Method: 提出 Macer（Allocation Before Ranking），一种免训练的压缩器。它首先为音频和视频分配显式的 token 预算，然后在各模态内部执行分配归一化（allocation-normalized）的排序，并在模态特定的浅层进行压缩。核心思想是"先分配、后排序"，解耦模态间容量分配与模态内 token 选择。

Result: 在 25% 保留率下，Macer 在 Qwen2.5-Omni-7B 上保持 98.7% 的全 token 性能，在 Qwen2.5-Omni-3B 上保持 97.3%。在 Qwen2.5-Omni-7B 上，25% 设置达到 OmniZip 在 45% 保留率下的性能水平，且 FLOPs 更低。在 OmniVinci-9B 上，allocation-before-ranking 原则比共享 top-K 排序提升最多 12.9 分。在音频接地、音视频联合、视觉主导和视频中心等各类基准上均显著降低 token 成本并保持精度。

Conclusion: Macer 验证了"先分配后排序"的 token 压缩原则在 OmniLLM 中的有效性，通过解耦模态间容量分配与模态内 token 选择，避免了共享 top-K 规则中偏向音频的分配先验，在显著降低 token 成本的同时保持甚至提升多模态性能。

Classification reason: 该论文研究 OmniLLM 的 token 压缩方法，属于多模态大模型的高效化与 token 压缩范畴，映射到 [CANONICAL] 子主题 efficient_multimodal（多模态高效化与轻量化）。

Importance reason: 提出免训练的多模态token压缩方法，通过显式预算分配解决OmniLLM中音频偏向的容量分配问题，与用户关注的多模态高效性和统一多模态模型方向高度相关。 Signals: token压缩, OmniLLM, 免训练, 模态预算分配, 高效推理
...
### [218] [CRAFT: Compression via Recursive Adaptive Fusion of Video Tokens for Vision-Language Models](https://arxiv.org/abs/2608.01644)
...
Importance: 高优先级 (81.58) - 未入选精读

TL;DR: CRAFT 提出一种查询无关的视频 token 压缩方法，通过解耦参数无关的 token 选择与可学习的 token 融合，递归合并冗余视觉 token，在约 8 倍压缩下保留约 97% 的骨干精度并显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 视频理解中 VLM 需要处理海量视觉 token，导致 prefill 阶段计算和内存成本急剧上升。现有 token 压缩方法要么采用启发式、无需训练但内容自适应性有限，要么引入需要昂贵对齐训练的额外模块，效率与自适应性难以兼顾。

Method: CRAFT 递归合并 token，将参数无关的 token 选择与可学习的 token 融合解耦：全局相似度决定哪些 token 合并，位置感知加权模块与内容自适应通道门控学习如何融合。整个压缩流程查询无关，每个保留 token 是原始 token 的线性组合，从而保持真实时空坐标并与预训练语言模型的输入分布对齐。

Result: 在多个代表性视频基准上，CRAFT 一致优于先前的 SOTA token 压缩方法。在约 8 倍压缩下保留约 97% 的骨干平均精度，并展现出显著的效率提升。

Conclusion: CRAFT 通过解耦 token 选择与融合，在保持内容自适应性的同时实现高效视频 token 压缩，为 VLM 视频理解中的效率-精度权衡提供了有效解决方案。

Classification reason: 该论文提出视频 token 压缩方法 CRAFT，通过递归自适应融合减少 VLM 的视觉 token 数量以提升效率，属于多模态高效化与 token 压缩范畴，映射到 [CANONICAL] efficient_multimodal 子主题。

Importance reason: Directly targets efficient video token compression for VLMs, aligning with the user's model efficiency and multimodal understanding priorities. Signals: video token compression, efficient inference, vision-language models, token merging, query-agnostic
...
### [219] [ET-Prune: Evidence-Aware Dynamic Budgeting for Visual Token Pruning in Text-Rich MLLMs](https://arxiv.org/abs/2608.01979)
...
TL;DR: ET-Prune 是一种免训练的视觉 token 剪枝框架，将剪枝建模为证据分配问题，通过解码器侧部分 query-key 块提取问题条件化证据，保护文本类空间区域，并根据证据不确定性与密度动态设定 token 保留下限，在文本密集的多模态推理中实现约一半 token 保留的同时保持或超越基线精度。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的视觉 token 剪枝可降低推理成本，但固定 token 比例与文本密集输入不匹配。在 OCR 类任务中，关键证据往往是少量数字、标签或字段，其相关性由问题指定；不加区分的剪枝可能删除关键证据而保留视觉显著但不相关的区域。

Method: ET-Prune 将剪枝建模为证据分配问题。它从解码器侧部分 query-key 块推导问题条件化证据，保护文本类空间区域，并将证据不确定性与密度转换为样本特定的 token 保留下限。随后通过三个渐进式中间层事件将序列推向该预算：对弥散或文本密集证据保留更多 token，对集中证据更激进地剪枝。

Result: 在六个 backbone-基准组合中，ET-Prune 在约一半 token 下领先或持平于其他剪枝方法。在 OCRBench-v2 上，Qwen3-VL-8B 和 InternVL3.5-8B 分别领先最强剪枝基线 1.80 和 0.68 个百分点；在 MMBench v1.1 上，以 54.45% 平均视觉 token 保留率达到 0.8467 的圆形精确匹配准确率，优于 Vanilla 的 0.8437。

Conclusion: 证据感知的动态预算分配在文本密集多模态推理中展现出良好的质量-成本权衡，验证了将剪枝视为证据分配而非固定比例压缩的有效性。

Classification reason: 该论文研究视觉 token 剪枝以降低多模态大模型推理成本，属于 token 压缩与高效 MLLM 范畴，映射到 [CANONICAL] 子主题 efficient_multimodal（多模态高效化与轻量化）。

Importance reason: Training-free dynamic visual token pruning with evidence-aware budgeting is highly relevant to efficient multimodal inference, though it targets text-rich OCR tasks rather than the user's core generation/world-model focus. Signals: visual token pruning, training-free, dynamic budgeting, text-rich MLLM, efficient inference
...
### [220] [DAVET: Denoising-Aware Visual Evidence Trajectory Allocation for Diffusion Vision-Language Models](https://arxiv.org/abs/2608.01821)
...
Importance: 高优先级 (80.35) - 未入选精读

TL;DR: DAVET 是一种无需训练的框架，针对扩散视觉语言模型（dVLMs）在去噪过程中视觉证据需求随步骤变化的特点，通过阶段条件证据轨迹和轨迹风险调制，自适应分配视觉证据预算，在 LLaDA-V 和 LaViDa 上实现平均 1.55× 加速，性能仅下降 1.86%。


<details>
  <summary>Details</summary>
Motivation: 扩散视觉语言模型（dVLMs）在每一步去噪时都依赖视觉证据进行条件生成，导致视觉条件成为显著的重复推理开销。与自回归解码不同，扩散生成会反复访问整个响应，且视觉证据需求随去噪步骤动态变化。现有加速方法要么从解码侧入手，要么通过剪枝/合并压缩视觉 token，但都没有把视觉证据视为一种随扩散过程需求演变的资源。

Method: DAVET 提出一种无需训练的框架，包含三个核心组件：(1) 阶段条件证据轨迹（phase-conditioned evidence trajectory），根据生成阶段设定初始证据分配；(2) 基于操作需求（operation demand）的证据储备（evidence reserve），并利用轨迹风险（trajectory risk）调制每个去噪步骤的证据分配；(3) 从单一视觉编码构建层次化证据视图（hierarchy of evidence views），将"何时需要多少证据"与"证据视图如何构建"解耦，从而在保持质量的同时降低视觉条件成本。

Result: 在 LLaDA-V 和 LaViDa 两个代表性 dVLM 上，跨多个视觉理解基准评测，DAVET 实现平均 1.55× 加速，平均相对性能下降仅 1.86%，表明去噪感知的视觉证据分配能在基本保持生成质量的同时显著降低视觉条件推理成本。

Conclusion: 该工作验证了扩散视觉语言模型中视觉证据需求具有强步骤依赖性，通过去噪感知的自适应证据分配可以有效降低视觉条件推理成本，为 dVLM 的高效推理提供了新的思路，即把视觉证据视为随扩散过程动态演变的资源进行分配。

Classification reason: 该论文针对扩散视觉语言模型（dVLMs）的视觉条件推理成本进行加速优化，通过自适应分配视觉证据来降低推理开销，属于多模态模型高效化与推理加速范畴，映射到 [CANONICAL] efficient_multimodal（多模态高效化与轻量化）。

Importance reason: Directly targets efficient inference for diffusion vision-language models, aligning with the user's priority on model efficiency and continuous multimodal understanding. Signals: training-free acceleration, diffusion vision-language model, visual evidence allocation, 1.55x speedup, LLaDA-V and LaViDa
...
### [221] [DiffPrune: differentiable information throttling for token pruning in vision-language models](https://arxiv.org/abs/2608.01985)
...
Importance: 普通相关 (64.71) - 未入选精读

TL;DR: DiffPrune 提出一种可微的信息节流方法用于 VLM 视觉 token 剪枝。与依赖 Gumbel-Softmax 近似离散选择的现有方法不同，DiffPrune 在训练时保留所有 token 并根据分数削弱每个 token 的信息量，使分数直接反映信息丢失的后果。通过信息节流器注入保持方差的噪声，高分 token 保持原始表征，低分 token 携带较少原始信息。推理时移除节流器并应用硬 top-K 剪枝。在十个 VLM 基准上保留 96.5% 的完整模型精度，同时将 LLM prefill 加速 2.85 倍，仅增加 0.69ms 推理开销。


<details>
  <summary>Details</summary>
Motivation: 视觉 token 剪枝通过移除冗余视觉 token 来降低 VLM 的计算成本，关键在于学习衡量 token 有用性的分数。现有方法通常依赖 Gumbel-Softmax 在训练中近似离散选择，但这类选择器使分数依赖于松弛剪枝算子的行为，而非直接反映信息丢失的后果，导致训练不稳定且分数含义不明确。

Method: DiffPrune 在训练时保留所有 token，并根据每个 token 的分数削弱其信息量。具体地，提出 Information Throttler 组件，向视觉 token 注入保持方差的噪声：高分 token 保持接近原始表征，低分 token 携带较少原始信息。由于损失通过这条实际的信息节流路径进行微分，评分器避免了松弛 token 选择的非稳定代理路径。推理时移除节流器，使用学习到的分数进行硬 top-K 剪枝。

Result: 在十个 VLM 基准上，DiffPrune 保留 96.5% 的完整模型精度，同时将 LLM prefill 加速 2.85 倍，仅增加 0.69ms 推理开销。

Conclusion: 通过让 token 分数直接反映信息丢失的后果而非依赖松弛选择代理，DiffPrune 实现了更稳定、更有效的视觉 token 剪枝，在保持高精度的同时显著加速 VLM 推理。

Classification reason: 该论文研究视觉语言模型（VLM）中的视觉 token 剪枝以降低计算成本，属于多模态高效化与轻量化方向，与 [CANONICAL] efficient_multimodal 子主题（token 压缩、稀疏推理）高度匹配。

Importance reason: Directly targets efficient VLM inference via token pruning, aligning with the model efficiency priority and multimodal understanding/generation direction. Signals: token pruning, vision-language model, efficient inference, differentiable training, top-K pruning
...
### [222] [Messages, Not Tokens: Grounded Coresets for Faithful VLM Compression](https://arxiv.org/abs/2608.02134)
...
TL;DR: 本文提出 Grounded Message Coreset Pruning (GMC)，一种无需训练的 VLM 视觉 token 压缩方法。不同于现有基于独立重要性分数的 Top-K 剪枝，GMC 将忠实压缩建模为解码器消息的紧致 coreset 构建，联合分配 query 引导、外观与坐标感知证据的支持，并将被丢弃状态迁移到选中的代表 token 上，在物理压缩前保持原始多模态位置并恢复原生注意力。在多个 VLM 家族上验证，GMC-H2 在 Qwen2.5-VL-7B 上以 80.2% 更少的视觉 token 保留 97.78% 的 Full-relative 平均能力。


<details>
  <summary>Details</summary>
Motivation: 高分辨率图像被 VLM 转换为长序列视觉 token，每个 token 都经过语言解码器并驻留在 prompt KV cache 中，导致推理成本膨胀。现有基于分数的 Top-K 剪枝方法为每个 token 分配独立重要性分数，但文本查询消费的是来自视觉群体的集体、带符号的注意力消息，而非孤立 patch，导致 Top-K 集合可能重复覆盖同一显著区域、遗漏稀疏但互补的证据、丢弃被移除群体携带的信息。

Method: GMC 将忠实视觉压缩形式化为解码器消息的紧致 coreset 构建。它联合在 query 引导、外观和坐标感知证据之间分配支持，然后将被丢弃状态迁移到选中的代表 token 上，在物理压缩前保持其原始多模态位置并恢复原生注意力。该方法将忠实压缩分解为两个耦合组件：选择覆盖所需消息模式的载体，以及在载体上实现带符号群体消息。作者还推导了连接这些误差与带符号消息失真、视觉创新和候选边界稳定性的界。

Result: 在多个 VLM 家族和多样化基准上验证了强性能：GMC-H2 在 Qwen2.5-VL-7B 上以 80.2% 更少的视觉 token 保留 97.78% 的 Full-relative 平均能力，GMC-L16 达到 100.36%。受控干预实验验证了集体支持与群体实现共同驱动性能提升。

Conclusion: GMC 表明将视觉压缩视为解码器消息的 coreset 构建，而非独立 token 的 Top-K 选择，能更忠实地保留 VLM 能力。集体支持分配与群体消息实现是压缩性能提升的关键因素，为高效 VLM 推理提供了新的范式。

Classification reason: 该论文研究 VLM 视觉 token 压缩以降低推理成本，属于多模态高效化与轻量化方向，映射到 [CANONICAL] efficient_multimodal 子主题。

Importance reason: Novel training-free VLM token compression via coreset message grounding, directly advancing efficient multimodal inference. Signals: VLM compression, efficient inference, training-free, coreset pruning, multimodal understanding
...
### [223] [CAPEval: A Decoupled Caption Evaluation across Understanding and Generation](https://arxiv.org/abs/2608.02589)
...
Importance: 值得关注 (76.19) - 未入选精读

TL;DR: CAPEval 是一个解耦的字幕评测基准，将字幕质量分解为 Coverage（视觉信息覆盖度）与 Precision（声明事实正确率）两个维度，通过人工标注的 ground-truth 字幕与原子检查清单进行评测，并发现 Coverage 与理解性能强相关、Precision 与生成性能强相关。


<details>
  <summary>Details</summary>
Motivation: 字幕既是多模态理解的主要监督信号，也是文生图生成的关键条件。然而以往评测将字幕质量视为单一标量目标，混淆了"字幕覆盖了多少视觉信息"与"图像能在多大程度上支持字幕声明"这两个不同性质，导致无法对字幕质量进行细粒度诊断，也难以指导不同下游任务的字幕器选择。

Method: 设计 CAPEval 基准，包含人工撰写的 ground-truth 字幕与人工验证的原子检查清单。将字幕质量解耦为 Coverage（覆盖 ground-truth 事实内容的程度）与 Precision（字幕中所有声明的事实正确率）。选取 10 个字幕器，并在四个模型家族上进行受控的下游端到端实验，其中字幕来源是唯一变量。

Result: 实验发现一致的任务依赖解耦现象：Coverage 是理解性能的更强相关指标，而 Precision 是生成性能的主导预测指标。该解耦评测范式提供了更细粒度的字幕质量诊断，并为不同下游任务选择与优化字幕器提供了可操作的指导。

Conclusion: CAPEval 证明字幕质量不应被当作单一标量目标，而应解耦为 Coverage 与 Precision 两个维度。这种解耦评测范式不仅提供更细粒度的诊断，还能针对不同下游任务（理解 vs 生成）提供字幕器选择与优化的行动指南。

Classification reason: 该论文提出 CAPEval 字幕评测基准，将字幕质量解耦为 Coverage 与 Precision 两个维度，并评估其对多模态理解与文生图生成任务的影响。这属于生成模型评测基准与数据集方向，映射到 [CANONICAL] generation_benchmark（生成模型评测与数据集）。同时涉及多模态理解与生成两个方向，故匹配 multimodal_understanding_generation 与 multimodal_generation。

Importance reason: 评测基准与用户关注的多模态生成/理解模型方向相关，但非核心方法学突破，且未涉及用户高权重子主题。 Signals: 字幕评测基准, 解耦Coverage与Precision, 理解与生成性能关联, 多模态评测, 非核心方向
...
### [224] [Distill What RGB Can Recover: Privileged 3D Evidence for RGB-Only Vision-Language Models](https://arxiv.org/abs/2608.00110)
...
TL;DR: 提出特权证据蒸馏框架，将训练时的 3D 证据转化为 RGB-only 推理时保留的空间推理能力，通过证据敏感度引导蒸馏和可恢复性分解，使 RGB-only 学生模型在 11 项指标上全面超越匹配基线。


<details>
  <summary>Details</summary>
Motivation: 3D 场景理解需要推理实体存在、空间布局和物体关系，但 RGB 图像本身提供的 3D 线索不足。现有 3D-VLM 在推理时依赖深度或 3D 位置感知输入，带来额外的采集、重建或标注成本，限制了 RGB-only 部署。

Method: 提出特权证据蒸馏框架：通过统一证据接口和受控残差注入构建可蒸馏的教师模型，通过 logit 和结构化表征蒸馏将知识迁移到仅接收 RGB 图像和问题的学生模型。引入证据敏感度引导蒸馏，用损坏证据识别高证据依赖目标并降低其监督权重；定义基于匹配基线、教师和学生的可恢复性分解，将特权增益分为 RGB 可恢复改进和残余教师优势。

Result: 在四个基准上，教师模型在 11 项指标中的 7 项取得最佳结果。RGB-only 学生模型在全部 11 项指标上超越匹配基线，包括 ScanQA CIDEr 提升 10.4、Scan2Cap CIDEr@0.5 提升 19.1，且无需额外推理时输入。

Conclusion: 训练时特权 3D 证据蒸馏对教师性能和可部署的 RGB-only 空间推理均有效。匹配基线-教师-学生分析刻画了特权增益在不同证据类型和空间技能上的迁移特性。

Classification reason: 论文研究 3D 场景理解中的空间推理能力，通过特权 3D 证据蒸馏提升 RGB-only 视觉语言模型的空间推理，属于空间与 3D 多模态推理方向，映射到 [CANONICAL] spatial_3d_reasoning。

Importance reason: Solid distillation method for 3D spatial reasoning, but it is not aligned with the user's priority on continuous/multimodal generation or world models. Signals: 3D VLM, privileged distillation, RGB-only inference, spatial reasoning, ScanQA/Scan2Cap
...
### [225] [SpatialAfford: Teaching Compact VLMs Where to Look and Where to Ground for Affordance](https://arxiv.org/abs/2608.00502)
...
Importance: 普通相关 (64.55) - 未入选精读

TL;DR: SpatialAfford 提出两阶段框架，通过空间注意力对齐（SAA）和空间感知 GRPO 强化学习，教会紧凑型 VLM 在生成坐标前先关注真实 affordance 区域，从而将 affordance grounding 从纯输出约束目标转变为注意力锚定的空间推理，使 4B 模型超越 7B+ 基线。


<details>
  <summary>Details</summary>
Motivation: 紧凑型自回归 VLM 在 affordance grounding 中虽然能通过序列级监督和强化学习改进坐标预测质量，但在坐标生成前缺乏可靠的 affordance 感知视觉焦点——交叉注意力仍然弥散且未锚定到真实的功能区域证据，导致定位不准确。

Method: 提出两阶段框架 SpatialAfford：第一阶段通过 Spatial Attention Alignment (SAA) 将模型注意力对齐到真实 affordance 区域；第二阶段使用 Spatial-Aware GRPO 强化学习精炼坐标预测。先教模型"看哪里"，再优化"定位哪里"。

Result: 在 ShareRobot-Bench、ReasonAff 和 PartAfford 三个基准上，SpatialAfford 一致地提升了 affordance grounding 性能，紧凑的 4B 模型超越了更强的 7B+ 基线。

Conclusion: 通过显式地将注意力对齐到真实 affordance 区域，再优化坐标预测，可以将 affordance grounding 从纯输出约束目标转化为注意力锚定的空间推理，从而显著提升紧凑 VLM 的定位能力。

Classification reason: 论文研究 affordance grounding（功能区域定位），属于空间定位与具身推理范畴，与 [CANONICAL] 空间与 3D 多模态推理（spatial_3d_reasoning）高度相关，该子主题涵盖空间推理、视觉定位与具身推理。

Importance reason: Solid but niche affordance-grounding work; relevant to multimodal understanding but not aligned with your top continuous-model or video-generation priorities. Signals: compact VLM, affordance grounding, spatial attention alignment, GRPO, 4B model beats 7B+
...
### [226] [GuideGround: VLM-guided Semantic Understanding and Viewpoint-aware Reasoning for 3D Visual Grounding](https://arxiv.org/abs/2608.00518)
...
TL;DR: GuideGround 提出一种 VLM 引导的 3D 视觉定位框架，利用视觉语言模型进行开放词汇语义增强和视角相关假设验证，在 ReferIt3D 基准上超越现有 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 视觉定位方法将语义理解建模为辅助的闭集物体分类任务，并依赖多视角特征聚合进行视角推理，导致语义泛化受限且削弱视角特定证据。作者观察到 VLM 天然具备开放词汇语义理解和全局场景感知能力，可补充而非替代任务特定的定位模型。

Method: GuideGround 框架包含两部分：(1) 语义增强：用 VLM 生成的物体语义描述替代辅助闭集物体分类，增强语义理解；(2) 视角推理：不直接聚合多视角表示，而是通过逐视角定位保留视角特定的定位假设，并利用 VLM 在候选视角间显式验证这些假设。

Result: 在 ReferIt3D 基准上的大量实验表明，GuideGround 一致优于此前的最先进方法；消融实验进一步验证了语义理解和视角推理两种策略的有效性。

Conclusion: VLM 的开放词汇语义理解与全局场景感知能力可以有效补充任务特定的 3D 定位模型，通过语义增强与视角假设验证实现更优的 3D 视觉定位性能。

Classification reason: 论文研究 3D 视觉定位（3D visual grounding），属于空间与 3D 多模态推理方向，直接匹配 [CANONICAL] subtopic "spatial_3d_reasoning"。

Importance reason: Solid 3D grounding paper but outside your core continuous/multimodal generation and world-model focus. Signals: 3D visual grounding, VLM-guided semantic enhancement, viewpoint-aware reasoning, ReferIt3D SOTA, no autoregressive/diffusion/world-model connection
...
### [227] [SpatioLM: Towards General Physical Spatial Intelligence in Vision-Language Models](https://arxiv.org/abs/2608.01899)
...
TL;DR: SpatioLM 提出一种参数高效的即插即用空间视觉模块，无需额外 3D 先验输入或第三方空间编码器，通过伪深度与相机信息监督引导 VLM 学习物理一致的空间表征，在 VSI-Bench 上达到 71.6 分（首个超过 70 的模型），并保持通用能力不显著退化。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 在常识推理上表现良好，但在视觉空间推理上较弱。已有方案通常引入额外 3D 先验输入或外部空间编码器，增加了复杂度，且空间微调后会损害 VLM 的通用能力。作者希望在不引入额外 3D 输入或第三方编码器的情况下增强 VLM 的空间智能。

Method: 设计一个即插即用、非侵入式的 spatio-vision 模块，激发 VLM 内在的空间知识；利用伪深度和相机信息作为监督信号，引导模型学习物理一致的表征。该方法参数高效，无需额外 3D 先验输入或第三方空间编码器。

Result: 在空间感知与理解等多样任务上取得显著提升，同时有效限制通用能力退化。在 VSI-Bench 上达到 71.6 分（首个超过 70 的模型），并在具身操作任务迁移中取得有竞争力的性能。

Conclusion: SpatioLM 证明可以通过参数高效的即插即用模块与伪深度/相机监督，在不引入额外 3D 先验或外部编码器的情况下显著增强 VLM 的空间智能，同时保持通用能力，并可迁移到具身操作任务。

Classification reason: 论文聚焦于增强 VLM 的视觉空间推理能力（spatial perception/understanding），并迁移到具身操作任务，与 [CANONICAL] 子主题"空间与 3D 多模态推理"高度匹配。

Importance reason: Solid spatial reasoning improvement for VLMs, but not aligned with your top continuous/multimodal generation priorities. Signals: VSI-Bench SOTA 71.6, parameter-efficient plug-and-play module, pseudo-depth supervision, no external 3D encoders, embodied manipulation transfer

Abstract: Vision-Language Models (VLMs) perform well on commonsense reasoning tasks but struggle with visual spatial reasoning. Most existing solutions introduce extra 3D prior inputs or external spatial encoders, which increase complexity and degrade the underlying VLMs' general-purpose capabilities after spatial fine-tuning. To this end, we propose a parameter-efficient \textit{\textbf{Spatio}-vision \textbf{L}anguage \textbf{M}odels (SpatioLM)}, that enhances spatial intelligence without extra 3D prior inputs or third-party spatial encoders. Concretely, we design a plug-and-play and non-invasive spatio-vision module that elicits the spatial knowledge inherent in VLMs. Furthermore, we innovatively leverage pseudo depth and camera information as supervision to guide the model in learning physically coherent representations. Extensive experiments show that SpatioLM achieves significant improvements in diverse tasks, including spatial perception and understanding while effectively limiting the degradation of general capabilities. Notably, the model achieves an impressive score of 71.6 on the VSI-Bench (the first model to surpass 70). In addition, it attains competitive performance when transferred to embodied manipulation tasks. Code is available at \href{ }{\faGithub~spatio-lm}.

</details>


### [228] [SpatialQuery: Benchmarking Geometry-Grounded Multi-Instance Spatial Reasoning in Vision-Language Models](https://arxiv.org/abs/2608.01709)
...
### [229] [Hear, Invoke, and Understand: A Skill-Calling Multimodal Agent for Large Audio Language Models](https://arxiv.org/abs/2608.01881)
...
TL;DR: SpeechAgent-R 是一个面向大音频语言模型的技能调用多模态智能体，通过轨迹监督微调与多轮强化学习协调内在多模态理解与外部工具，并构建 HIU-Corpus 数据集与 HIU-Bench 评测基准，在 ID 与 OOD 任务上显著提升音频智能体的工具交互与推理能力。


<details>
  <summary>Details</summary>
Motivation: 复杂声学问题往往需要模型执行声学操作、与外部工具交互并对结果进行推理，而非直接从固定音频输入作答。现有音频语言模型缺乏这种工具交互式音频推理能力。

Method: 构建 HIU-Corpus（65,492 条交互轨迹、507.6 小时音频、24 任务/8 技能/9 工具），SpeechAgent-R 先通过轨迹监督微调学习结构化交互行为，再通过多轮强化学习改进决策。同时引入 HIU-Bench 评测基准，包含 ID 与 OOD 划分。

Result: SpeechAgent-R 在 ID 任务上达到 84.17，OOD 任务上达到 70.94，相比同 agent harness 下的基础模型分别提升 15.40 和 14.23 分。

Conclusion: 学习技能与工具协调能够提升音频智能体在多样化任务设置与自适应工具交互中的表现，验证了工具交互式音频推理范式的有效性。

Classification reason: 论文提出 SpeechAgent-R，一个面向大音频语言模型的技能调用多模态智能体，通过轨迹监督微调与多轮强化学习协调内在多模态理解与外部工具。这属于多模态理解与生成方向下的通用多模态模型/智能体范畴，映射到 [CANONICAL] vlm_mllm_general（视觉语言模型与 MLLM）最为贴切。

Importance reason: Audio agent tool-calling is outside your core focus on continuous multimodal models, autoregressive generation, and world models. Signals: audio agent, tool interaction, reinforcement learning, multimodal understanding, OOD generalization
...
### [230] [Learning How Much, Not Just What: Cross-Patient Burden Order for CT Vision-Language Pretraining](https://arxiv.org/abs/2608.00231)
...
TL;DR: 本文提出 Spectrum，一个解剖条件化的 CT 视觉语言预训练框架。它通过规则评分器从不同患者中挖掘同一病理的置信度过滤的负担高低配对，并用 Burden-Direction Alignment (BDA) 对齐病理条件化的图像差与报告差，从而在无纵向数据的情况下学习负担感知的 CT 表征，在 CT-RATE 上达到 85.6 零样本 AUROC。


<details>
  <summary>Details</summary>
Motivation: 现有 CT 视觉语言预训练仅通过全局和解剖感知目标监督"对应关系"（即存在什么），而无法约束"程度"（即有多严重）。同一发现的轻症与重症在表征空间中缺乏一致方向，导致报告中的分级负担语言退化为存在/缺失信号。纵向监督可提供这种顺序，但患者匹配的 CT 配对在大规模上稀缺。

Method: Spectrum 框架在每个研究上以整体和器官两个范围表示。对每个器官映射的病理，规则评分器挖掘不同患者的置信度过滤的低到高负担配对；Burden-Direction Alignment (BDA) 在每个范围对齐病理条件化的图像差与报告差，将该方向与其反向分离。由于端点是不同人，目标条件化对齐器先使它们可比，使差反映负担而非患者间变异。BDA 进一步分离选定方向与其反向，锚定到观察到的更高负担端点，并在有序三元组上强制一致性。所有配对都在单一病理内抽取，以约束图像-报告对比无法触及的类内结构。

Result: Spectrum 在 CT-RATE 上达到 85.6 零样本 AUROC，在外部 RAD-ChestCT 上达到 72.7，在线性探测和检索任务上均有持续提升。

Conclusion: 弱跨患者顺序是可扩展的补充，与解剖感知对应关系结合，可在无需纵向数据的情况下获得负担感知的 CT 表征。

Classification reason: 该论文提出 CT 视觉语言预训练框架 Spectrum，通过跨患者负担顺序监督学习 3D 医学影像表征，属于视觉语言模型预训练方法，映射到 [CANONICAL] vlm_mllm_general（视觉语言模型与 MLLM）子主题。

Importance reason: CT-specific medical vision-language pretraining with weak supervision, but outside the user's core continuous/multimodal generation and world-model focus. Signals: CT vision-language pretraining, burden order learning, cross-patient weak supervision, zero-shot AUROC, medical imaging
...
### [231] [Real-Time Visual Obstruction Detection in Surgical Augmented Reality](https://arxiv.org/abs/2608.00232)
...
TL;DR: 本文提出一种面向手术增强现实（AR）的实时视觉遮挡检测系统，采用级联小到大 VLM 架构，结合分割引导的早停与注意力视觉 token 剪枝，在 479ms 端到端延迟下达到 87.43% 的遮挡检测准确率，相比云端大模型基线延迟降低 62.90%。


<details>
  <summary>Details</summary>
Motivation: 手术 AR 中叠加的虚拟内容可能遮挡任务相关的真实信息（如手术器械），干扰用户在时间敏感手术任务中的感知。现有方法缺乏对手术 AR 场景下视觉遮挡的实时检测能力，需要兼顾准确率与低延迟。

Method: 构建一个延迟感知的级联小到大 VLM 流水线：小 VLM 处理简单帧，当其关键物体预测与分割一致性匹配时提前退出；困难帧则转发给大 VLM，并采用注意力机制对视觉 token 进行剪枝以降低推理开销。同时构建伪 AR 手术遮挡检测基准，将虚拟内容叠加到手术器械图像上并标注是否遮挡任务相关器械。

Result: 系统达到 87.43% 的遮挡检测准确率，平均端到端延迟 479ms，相比云端大模型基线延迟降低 62.90%。

Conclusion: 验证了延迟感知的手术 AR 遮挡检测的可行性，并指出未来工作方向包括动态手术视频、多物体场景以及临床导向的 AR 引导内容。

Classification reason: 论文提出基于 VLM 的手术 AR 视觉遮挡检测系统，采用级联小到大 VLM 架构与视觉 token 剪枝，属于视觉语言模型在特定应用场景下的架构与推理优化，映射到 [CANONICAL] vlm_mllm_general 子主题。

Importance reason: Surgical AR obstruction detection is a niche application with limited overlap with your core interests in continuous multimodal models and video/world-model generation. Signals: VLM cascade, visual token pruning, segmentation-guided early exit, surgical AR, latency reduction
...
### [232] [Attention-Steered Vision-Language Models for Sign Language Translation](https://arxiv.org/abs/2608.00235)
...
TL;DR: 提出 AttnSign，一种基于 VLM 的空间-时间注意力引导框架，用于手语翻译。通过空间注意力监督（关注脸部和手部区域）和基于 RL 的动作节奏引导（聚焦手语关键帧），解决现有 VLM 在手语翻译中空间-时间视觉定位不足的问题，在 How2Sign 和 OpenASL 基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 在手语翻译任务中表现受限，关键失败模式是空间-时间视觉定位不足：标准 next-token 交叉熵损失无法直接提供模型应关注何处、何时的信号，导致模型忽略手语相关区域和帧。

Method: AttnSign 框架包含两部分：(1) 空间注意力监督，对每帧中手语相关区域（如脸部和手部）施加注意力监督；(2) 基于强化学习（RL）的动作节奏引导方法，鼓励模型探索并聚焦手语级关键帧。

Result: 在 How2Sign 和 OpenASL 基准上的实验表明，AttnSign 一致优于现有方法。

Conclusion: 通过显式引导 VLM 的空间-时间注意力，可以显著提升手语翻译等需要精细视觉定位的多模态视频理解任务的性能。

Classification reason: 论文提出基于 VLM 的手语翻译框架 AttnSign，通过空间-时间注意力引导改进 VLM 的视频理解能力，属于通用视觉语言模型的应用与改进，映射到 [CANONICAL] vlm_mllm_general。

Importance reason: 手语翻译任务与用户配置的连续多模态、视频生成、世界模型等核心方向关联较弱，且未涉及生成或世界模型。 Signals: 视觉语言模型, 手语翻译, 注意力引导, 空间-时间定位, 强化学习
...
### [233] [Linguistic Context Recodes Visual Representations in Vision-Language Models](https://arxiv.org/abs/2608.00035)
...
TL;DR: 本文通过因果干预实验证明，VLM 中的视觉表征并非静态信息仓库，而是会被自然语言提示动态重编码：一方面存在抽象的"参考表征"（reference representation）指示目标相关对象，另一方面后层会选择性放大目标相关属性（attribute modulation），两者共同中介模型的预测分布。


<details>
  <summary>Details</summary>
Motivation: 人类视觉智能具有目标导向性，视觉表征会随任务目标动态调整。然而先前研究大多将 VLM 中的视觉表征视为静态信息仓库，由语言表征操纵。本文旨在刻画 VLM 在目标导向语言提示下是否以及如何对视觉表征进行重编码。

Method: 作者识别出表示自然语言提示下目标相关对象的抽象参考表征，提取对应的对比转向向量（contrastive steering vectors）并证明其对模型预测具有因果作用。同时通过因果干预实验证明后层会选择性放大目标相关属性（attribute modulation），并验证该现象在不同提示、不同对象、不同任务上下文乃至从合成到自然图像上的泛化性。

Result: 实验表明：1) 参考表征是抽象的，可泛化到不同对象、任务上下文及合成到自然图像；2) 语言诱导的属性调制在多个提示下稳定出现；3) 因果干预证明属性调制中介了 VLM 的响应分布。

Conclusion: VLM 中的跨模态处理是动态的：视觉 token 并非静态信息仓库，而是被语言查询调制以支持目标导向任务，这为理解 VLM 内部机制提供了更动态的视角。

Classification reason: 论文研究 VLM 中语言提示如何因果性地调制视觉表征（语言诱导的视觉表征重编码），属于视觉语言模型内部机制分析，映射到 [CANONICAL] vlm_mllm_general 子主题。

Importance reason: Provides causal evidence for dynamic language-induced recoding of visual representations in VLMs, relevant to multimodal understanding but not directly aligned with top-priority continuous/autoregressive generation topics. Signals: VLM interpretability, causal intervention, reference representation, attribute modulation, multimodal understanding
...
### [234] [Through the LENS: Local Geometric Decomposition of Vision-Language Model Representations](https://arxiv.org/abs/2608.00561)
...
TL;DR: 提出 LENS 方法，用 Mixture of Factor Analyzers 将 VLM 激活分解为局部低秩高斯邻域，揭示 LLaVA 与 Qwen3-VL 不同的深度依赖融合轨迹，并通过邻域质心插值实现因果性生成重定向，在图像到渲染文本检索等任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: VLM 在共享残差流中处理图像 patch 与文本 token，但两种模态交互的局部几何结构仍不清楚。现有可解释性方法多识别全局线性方向，可能遗漏"全局高维但局部低维"的表征结构。

Method: 提出 LENS（Local Explanation of Neighborhood Subspaces），使用 Mixture of Factor Analyzers（MFA）将 VLM 激活分解为局部低秩高斯邻域；配合自动化多模态标注流水线为邻域赋予语义描述；通过向邻域质心插值激活实现因果性生成重定向。

Result: 在 LLaVA-1.5-7B 与 Qwen3-VL-8B 上揭示两种不同的深度依赖融合轨迹；MFA 插值在多数条件下优于 difference-in-means 与 VL-SAE，其中 LLaVA 视觉到视觉场景达到 VL-SAE 的 5.7 倍；Qwen3-VL 图像到渲染文本检索 R@1 从 14.9% 提升至 48.6%；人类评估显示 MFA 引导与提示竞争相当且强于其他干预基线。

Conclusion: 局部几何邻域可作为分析跨模态表征的有用可解释与因果单元，融合轨迹在组件数、局部秩与模态纯度阈值下保持稳定，支持将局部几何分解作为 VLM 可解释性的有效范式。

Classification reason: 该论文研究 VLM（LLaVA-1.5-7B、Qwen3-VL-8B）内部表征的局部几何分解与可解释性，属于视觉语言模型内部机制分析，映射到 [CANONICAL] vlm_mllm_general（视觉语言模型与 MLLM）最为贴切。

Importance reason: Strong interpretability method for VLM local geometry with causal steering, but only tangential to the user's core continuous-model and generation priorities. Signals: VLM interpretability, local low-rank decomposition, causal steering, LLaVA/Qwen3-VL, retrieval improvement
...
### [235] [Models as Tools: An Agentic Coordination Framework for Unified Multimodal Visual Tracking](https://arxiv.org/abs/2608.00847)
...
Importance: 高优先级 (82.75) - 未入选精读

TL;DR: ACTrack 是一个智能体协调框架，将异构模型（跟踪器、SAM3 分割模型、VLM）作为可调用工具，通过事件触发机制协调互补能力，实现统一多模态视觉跟踪，在 RGB 和多模态基准上超越最强跟踪器。


<details>
  <summary>Details</summary>
Motivation: 现有视觉跟踪器多为基于匹配的架构，仅在跟踪数据集上训练，性能受限于输入上下文长度且已到瓶颈。虽然高性能跟踪越来越依赖基础模型，但现有方法将基础模型单一化使用（要么适配成跟踪器，要么把分割模型改造成跟踪流程），无法利用互补优势。匹配式跟踪器擅长实例级对应但缺乏语义判别和细粒度前景感知，分割基础模型能产生精确掩码但缺乏实例判别和多模态扩展能力，且两者都缺乏长期跟踪的纠错能力。

Method: ACTrack 将异构模型视为可调用工具，在事件触发机制下协调：跟踪器实例匹配工具负责目标判别，SAM3 运动工具提供掩码导出的运动先验，SAM3 感知工具检测干扰物和实例冲突线索，VLM 重提示工具仅在持续冲突时激活以缓解误差累积。设计了完整的工具调用触发机制和工具间协调机制，并采用参数高效适配策略实现工具间参数共享与复用。

Result: ACTrack 在八个 RGB 基准上大幅超越最强和最大的跟踪器。参数高效适配策略仅用 30% 可训练参数实现统一多模态跟踪，在 LasHeR、VisEvent、TNL2K、DepthTrack 等多模态基准上显著优于先前方法。

Conclusion: 通过将异构基础模型作为可调用工具并设计事件触发协调机制，ACTrack 有效整合了匹配式跟踪器与分割/VLM 基础模型的互补优势，突破了单一架构的瓶颈，实现了高效且统一的 RGB 与多模态视觉跟踪。

Classification reason: 论文提出 ACTrack，一个基于智能体协调框架的统一多模态视觉跟踪系统，协调跟踪器、SAM3 分割模型与 VLM 重提示工具。核心是视觉语言模型（VLM）作为工具参与多模态跟踪，属于通用视觉语言模型与 MLLM 应用范畴，映射到 [CANONICAL] vlm_mllm_general。

Importance reason: Agentic tool coordination for multimodal tracking is solid engineering but only tangentially touches the user's core continuous-model and autoregressive generation priorities. Signals: agentic framework, multimodal tracking, VLM tool use, SAM3, parameter-efficient adaptation
...
### [236] [MonitorVLM-v2: A Deployed Vision-Language Framework for Real-Time Safety Violation Detection](https://arxiv.org/abs/2608.00975)
...
Importance: 值得关注 (76.84) - 未入选精读

TL;DR: MonitorVLM-v2 是一个面向实时工业安全违规检测的部署型视觉语言框架，将多模态推理压缩为有限规则空间上的单 token 概率预测，并引入符号策略优化（SymPO）与熵驱动分流机制，在矿井 10 路摄像头四个月部署中实现 19.45 倍推理加速并识别出 2.78 倍于人工巡检的确认违规。


<details>
  <summary>Details</summary>
Motivation: 大型 VLM 的开放端自回归思维链推理在安全关键、规则驱动的工业监控场景中不适用，因为推理成本随推理长度和并发流数量共同增长，形成吞吐瓶颈，无法满足实时多路监控需求。

Method: 将 VLM 安全评估重构为有限监管决策空间上的概率推理，把多模态推理压缩为单步规则 ID 预测，将解码从变长序列缩减为单 token。提出符号策略优化（SymPO）对比策略优化算法以锐化有限符号空间内的决策边界，并设计熵驱动分流机制将不确定预测路由给人工审核。

Result: 在运营中的地下矿井设施 10 路摄像头四个月前瞻性部署中，MonitorVLM-v2 实现 19.45 倍推理速度提升，识别出的确认违规数量是现场常规人工巡检流程的 2.78 倍。

Conclusion: 压缩符号决策空间的方法能够为实时、可审计的工业监控提供实用价值，证明了将 VLM 推理约束到有限规则空间在安全关键场景中的有效性。

Classification reason: 该论文提出 MonitorVLM-v2，一个面向工业安全监控的视觉语言模型框架，将 VLM 安全评估转化为有限规则空间的概率推理，属于通用视觉语言模型与 MLLM 的应用与架构改进，映射到 [CANONICAL] vlm_mllm_general。

Importance reason: Focuses on industrial safety deployment with single-token rule prediction, which is orthogonal to the user's priority on continuous multimodal models and generative world models. Signals: single-token rule prediction, entropy-driven triage, symbolic policy optimization, 19.45x inference speedup, industrial surveillance deployment
...
### [237] [PMMC: Prospective Multimodal Memory Compilation for Long-Term LVLM Agents](https://arxiv.org/abs/2608.00962)
...
TL;DR: 提出 PMMC（前瞻性多模态记忆编译）框架，将 LVLM 智能体的部分记忆推理从查询时转移到记忆整合时：Questioner 预测未来问题、Planner 编译问题条件化的多模态记忆程序、Doubter 验证证据路径，形成结构化问题库以支持高效查询路由与证据检索，提升长期多模态交互中的答案质量与视觉证据召回，同时降低查询时 token 与延迟成本。


<details>
  <summary>Details</summary>
Motivation: 现有 LVLM 智能体记忆系统往往将视觉经验压缩为文本摘要，或依赖静态的"检索-再推理"流水线，在查询时效率低下，且当问题需要图像-文本绑定、时序更新或视觉细节时表现脆弱。作者希望将部分记忆推理过程从查询时转移到记忆整合时，以提升长期多模态交互中的一致性与效率。

Method: 提出 Prospective Multimodal Memory Compilation 框架，包含三个组件：Questioner 预测未来可能的问题候选；Planner 编译问题条件化的多模态记忆程序；Doubter 验证规划的证据路径能否支撑预测答案。验证通过的问题-程序对形成结构化问题库，用于查询时的高效路由与证据检索。

Result: 在多模态长期记忆基准上，该方法提升了答案质量与视觉证据召回，同时降低了查询时的 token 与延迟成本。消融实验分析了自反馈、动态规划、原始图像访问与问题库覆盖度等因素的影响。

Conclusion: 将记忆推理前移到记忆整合阶段能够有效提升长期 LVLM 智能体的记忆效率与可靠性，结构化问题库为查询时路由提供了高效支撑，验证了前瞻性记忆编译范式的有效性。

Classification reason: 论文提出面向长期 LVLM 智能体的前瞻性多模态记忆编译框架，属于视觉语言模型智能体的记忆与推理系统设计，映射到 [CANONICAL] vlm_mllm_general（视觉语言模型与 MLLM）这一通用主题。

Importance reason: Proposes an efficient memory-compilation framework for LVLM agents, but is tangential to the user's core focus on continuous multimodal models and generative world models. Signals: LVLM agents, long-term memory, query-time efficiency, multimodal reasoning, evidence retrieval
...
### [238] [STAR-VLM: Spatiotemporal Grounding Vision-Language Models for Motion and Velocity Estimation via Automotive Radar Supervision](https://arxiv.org/abs/2608.01535)
...
TL;DR: STAR-VLM 利用汽车雷达的测距与多普勒测量作为无标签监督信号，增强视觉语言模型在自动驾驶场景中的运动分类与公制速度估计能力，无需人工标注或合成数据。


<details>
  <summary>Details</summary>
Motivation: 现有提升 VLM 时空推理的方法依赖复杂预处理、昂贵人工标注或合成数据，存在扩展性差与 sim-to-real 差距问题；且这些方法虽改善时空理解，但缺乏对动态场景的公制推理能力（如以真实单位估计物体运动）。LiDAR 监督只解决空间感知，未直接处理时序推理。

Method: STAR-VLM 采用汽车雷达作为监督信号，利用雷达的测距（range）与多普勒（Doppler）测量作为训练时的无标签真值，为 VLM 提供互补的时空监督，从而提升其公制时空推理能力。

Result: 在驾驶场景实验中，STAR-VLM 在运动分类与公制速度估计两项任务上均达到最先进性能，甚至优于为各任务专门设计的任务特定方法。

Conclusion: 汽车雷达是一种可扩展且低成本的监督来源，可用于构建面向真实世界自动驾驶的公制感知时空 VLM，为动态场景中的运动与速度推理提供了新的监督范式。

Classification reason: 该论文提出 STAR-VLM，通过汽车雷达监督增强视觉语言模型（VLM）的时空推理与运动/速度估计能力，属于通用视觉语言模型架构与训练方法范畴，映射到 [CANONICAL] vlm_mllm_general 子主题。

Importance reason: Focuses on radar-supervised motion/velocity estimation for autonomous driving, which is peripheral to the user's core interests in continuous multimodal models and generative world models. Signals: automotive radar supervision, spatiotemporal VLM, metric velocity estimation, autonomous driving, motion classification
...
### [239] [PixVL: Self-Supervised Training of Pixel-Level MLLMs via a Unified Mask--Text Consistency Cycle](https://arxiv.org/abs/2608.01354)
...
TL;DR: PixVL 提出一种自监督后训练框架，通过统一的 Mask-Text 一致性循环，让像素级 MLLM 生成并自验证区域描述，从无标注数据中学习，同时提升区域分割与区域理解能力。


<details>
  <summary>Details</summary>
Motivation: 像素级 MLLM 同时支持区域分割与区域理解，但面临两个挑战：高质量 mask-text 配对数据稀缺，大量 mask 标注缺乏语言监督；区域分割与区域理解在监督格式和学习信号密度上的差异导致优化干扰。

Method: PixVL 引入统一的 Mask-Text 一致性循环，使模型生成并自验证区域描述并从无标注数据学习。针对纯几何重建循环不可靠的问题，引入 confuser-aware 语义验证，利用模型在相似候选区域中正确选择目标的置信度，错误选择则奖励为零。同时采用跨视角验证（时间分离的视频帧或几何变换的图像视图）防止循环学习坍缩到位置和形状捷径。最后通过质量耦合的双向学习策略，用最高奖励描述引导 Text-to-Mask 学习，将区域理解与区域分割从竞争任务转化为相互生成与验证器。

Result: 实验表明 PixVL 同时提升了区域理解任务与分割任务的性能。

Conclusion: PixVL 通过自监督的 Mask-Text 一致性循环有效缓解了像素级 MLLM 中标注稀缺与任务干扰问题，验证了区域理解与区域分割可以互为生成器和验证器，实现相互促进。

Classification reason: 论文提出 PixVL，一种像素级 MLLM 的自监督后训练框架，通过 Mask-Text 一致性循环同时提升区域分割与区域理解能力。这属于通用视觉语言模型与 MLLM 的架构与训练方法，映射到 [CANONICAL] vlm_mllm_general 子主题。

Importance reason: Strong self-supervised method for pixel-level MLLMs, but tangential to the user's core continuous-model and autoregressive generation priorities. Signals: pixel-level MLLM, self-supervised post-training, region segmentation, region understanding, mask-text consistency
...
### [240] [Mitigating Visual Degradation in MLLMs via Spatial-Spectral Visual Anchor Learning](https://arxiv.org/abs/2608.01635)
...
TL;DR: 提出 SSVAL（空间-频谱视觉锚点学习），通过视觉锚点提示注入（VAPI）吸收外部视觉基础模型知识作为稳定锚点，并辅以空间与频域表征对齐损失，缓解 MLLM 在视觉指令微调后推理阶段的表征漂移与视觉信息退化。


<details>
  <summary>Details</summary>
Motivation: MLLM 在视觉指令微调后，内部表征在推理阶段会迅速偏离原始语义状态，导致严重的视觉感知退化。现有方法尝试借助外部视觉基础模型（VFM）对齐内部表征，但直接对齐虽增强视觉语义却无法缓解表征漂移问题。

Method: 提出 Spatial-Spectral Visual Anchor Learning (SSVAL)。核心是 Visual Anchor Prompt Injection (VAPI)：在训练时注入能吸收外部 VFM 丰富知识的提示，使其在推理时作为稳定视觉锚点缓解表征漂移。同时引入辅助的空间域与频域表征对齐损失，在 LLM 中间层提供互补的视觉专属监督。

Result: 大量实验表明 SSVAL 显著优于现有方法，有效缓解了 MLLM 的视觉表征漂移与视觉感知退化问题。

Conclusion: 直接与 VFM 对齐虽能增强视觉语义但无法解决表征漂移，而通过视觉锚点提示注入结合空间-频谱对齐损失，可以同时提升视觉语义并稳定内部表征，为 MLLM 视觉感知退化提供了有效解决方案。

Classification reason: 论文针对 MLLM 视觉感知退化问题，提出空间-频谱视觉锚点学习以缓解视觉指令微调后的表征漂移，属于通用视觉语言模型/MLLM 架构与训练方法改进，映射到 [CANONICAL] vlm_mllm_general。

Importance reason: Addresses MLLM visual degradation via novel spatial-spectral anchors, relevant to multimodal understanding but not core continuous/diffusion priorities. Signals: MLLM visual degradation, visual anchor learning, representation drift, spatial-spectral alignment, external VFM knowledge

Abstract: Despite the progress of multimodal large language models (MLLMs), they continue to exhibit deficiencies in visual perception. Following visual instruction tuning, internal MLLM representations rapidly deviate from their original semantic states during inference, causing severe information degradation. While existing methods attempt to leverage external vision foundation models (VFMs) to align internal representations, we find that direct alignment with VFMs enhances visual semantics but fails to mitigate representation deviation. To address this, we propose Spatial-Spectral Visual Anchor Learning (SSVAL). The core of SSVAL is Visual Anchor Prompt Injection (VAPI), which introduces prompts that absorb rich knowledge from external VFMs during training, enabling them to serve as stable visual anchors that mitigate representation deviation during inference. Additionally, we incorporate auxiliary spatial and frequency-domain representation alignment losses to provide complementary vision-specific supervision at intermediate LLM layers. Extensive experiments demonstrate that SSVAL significantly outperforms existing methods. Code are available on our \href{ }{project page}.

</details>


### [241] [HAFI-VLM: A Frequency Perspective for Diagnosing and Enhancing Visual Perception in Vision-Language Models](https://arxiv.org/abs/2608.02124)
...
TL;DR: HAFI-VLM 从频率视角诊断并增强 VLM 的视觉感知。论文发现预训练视觉编码器存在"频谱响应刚性"问题——不同图像和任务下频谱特征变化极小，无法适应查询所需的证据。为此提出分层自适应频率注入（HAFI），通过文本调制、空间对齐的交叉注意力在多个编码器深度检索互补的低/中/高频证据，并配合视觉增强层适配器重校准 LLM 浅层注意力。在 LLaVA-1.5 和 Qwen2.5-VL 上验证了通用 VQA、文本丰富理解和幻觉鲁棒性的提升。


<details>
  <summary>Details</summary>
Motivation: VLM 在需要细粒度视觉证据的预测中仍不可靠。论文识别出一个此前被忽视的原因：频谱响应刚性——预训练视觉编码器在不同图像和任务间表现出持久且编码器特定的分层频谱特征，在下游微调时变化极小。由于预训练编码器只接收图像，无法根据当前查询所需的证据调整频谱提取。

Method: 提出 HAFI-VLM，包含两个核心组件：(1) 分层自适应频率注入（HAFI），使用文本调制、空间对齐的交叉注意力在多个编码器深度检索互补的低、中、高频证据；(2) 视觉增强层适配器（Visual Enrichment Layer Adapter），重校准 LLM 浅层注意力以有效利用增强后的视觉 token。该方法在保留预训练语义表征的同时引入任务条件化的频率通路。

Result: 在 LLaVA-1.5 和 Qwen2.5-VL 上实验表明，HAFI-VLM 在通用 VQA、文本丰富理解和幻觉鲁棒性上均取得一致提升，优于表征级增强方法以及大多数基于分辨率或裁剪的方法，且无需额外的高分辨率编码。机制分析显示 HAFI 恢复了任务相关的频谱分配，同时保留语义注意力。

Conclusion: 论文确立了频率增强作为提升 VLM 感知的一条独立且有效的路径。通过任务条件化的频率注入，可以缓解预训练视觉编码器的频谱响应刚性，在不增加高分辨率编码开销的情况下改善 VLM 的细粒度视觉感知能力。

Classification reason: 该论文提出 HAFI-VLM，通过频率视角增强视觉语言模型的视觉感知能力，属于通用 VLM 架构与感知增强方向，映射到 [CANONICAL] vlm_mllm_general（视觉语言模型与 MLLM）。

Importance reason: 频率视角诊断VLM视觉感知并注入多频证据，方法新颖但方向与用户核心的连续/世界模型/生成主题偏离。 Signals: VLM感知增强, 频率分析, 交叉注意力, 幻觉鲁棒性, LLaVA/Qwen2.5-VL
...
### [242] [Can Urban Blight Be Accessed with Vision-language Models: A Case Study in Detroit](https://arxiv.org/abs/2608.01753)
...
TL;DR: 本文提出一个基于开源大视觉语言模型的可扩展框架，利用多视角街景图像评估住宅衰败状况，通过结构化提示让模型评估屋顶完整性、墙体损坏、门窗破损等住房属性，并采用 XGBoost 集成堆叠与加权评分系统提升评估精度，在底特律案例中验证了多视角融合、模型差异化推理能力以及集成学习优于单模型的结论。


<details>
  <summary>Details</summary>
Motivation: 传统住宅衰败调查依赖人工，成本高、周期长，难以大规模持续维护。作者希望利用开源大视觉语言模型构建低成本、可定期更新的住宅衰败评估框架，以支持城市规划、修复目标定位和公共健康保障。

Method: 使用多个开源大视觉语言模型，通过结构化提示引导模型评估住房属性（屋顶完整性、墙体损坏、破损或封堵的开口），输出二值判断与失修概率估计。采用多视角街景图像输入，并构建基于 XGBoost 的集成堆叠方法与加权评分系统，与专业人工标注进行对比评估。

Result: 实验结果表明：(i) 多视角街景图像有助于提升评估精度；(ii) 不同大视觉语言模型具有差异化的推理优势；(iii) 集成学习器优于单个基础模型，在各类住宅状况与衰败评估中增强了鲁棒性。

Conclusion: 该方法可作为传统衰败调查的低成本补充，实现对住房存量状况的定期跟踪与管理，为城市衰败评估提供可扩展的自动化方案。

Classification reason: 论文使用开源大视觉语言模型对城市衰败（urban blight）进行多视角视觉评估，属于视觉语言模型在具体应用场景中的推理与评估，映射到 [CANONICAL] 的 vlm_mllm_general（视觉语言模型与 MLLM）子主题。

Importance reason: Urban blight assessment with VLMs is a niche application far from the user's core focus on continuous multimodal models, autoregressive generation, and world models. Signals: vision-language models, urban blight, ensemble learning, street view, XGBoost
...
### [243] [Assessing the Benefits of Combining Advanced Deep Learning Techniques for Post-Disaster Building Damage Assessment from UAV Imagery](https://arxiv.org/abs/2608.01906)
...
TL;DR: 提出一个混合框架，将 CV 检测模型与大型视觉语言模型（LVLM）解耦结合，用于无人机影像的灾后建筑损伤评估。CV 模型先检测建筑并生成边界框，LVLM 再进行损伤分类与上下文解释，在 RescueNet 和 FloodNet 基准上超越孤立基线。


<details>
  <summary>Details</summary>
Motivation: 灾后建筑损伤评估对快速响应至关重要，但现有 CV 模型需要大量标注数据、跨区域泛化差且局限于特定任务；而 LVLM 具备强推理与泛化能力，但在精确的低层感知（如目标检测、边界框生成）上表现不足。作者希望结合两者优势，构建一个解耦检测与评估的混合框架。

Method: 提出混合框架：首先用 CV 模型在无人机图像上检测建筑并生成边界框，然后将裁剪后的建筑区域图像传递给 LVLM 进行损伤分类（完好、部分损坏、完全损毁）和上下文解释。检测与评估阶段解耦，检测阶段仅需有限标注数据。

Result: 在 RescueNet 和 FloodNet 两个真实基准上评估，最佳组合在准确计数完好、部分损坏和完全损毁建筑方面超越孤立基线最多 2.1 R² 分，且检测阶段仅需有限标注数据。论文还提供了失败场景与边缘案例的详细分析。

Conclusion: 将 CV 模型的精确检测能力与 LVLM 的推理能力解耦结合，可有效提升灾后建筑损伤评估的准确性与泛化性，同时降低标注数据需求。作者公开了源码与数据，并为后续工作提供了实践洞见与方向。

Classification reason: 论文提出将 CV 检测模型与 LVLM 结合的混合框架用于灾后建筑损伤评估，属于视觉语言模型在特定任务上的应用与集成，映射到 [CANONICAL] vlm_mllm_general（视觉语言模型与 MLLM）。

Importance reason: Domain-specific disaster assessment with a standard CV+LVLM pipeline, far from the user's core continuous/multimodal generation and world model focus. Signals: UAV disaster damage, CV+LVLM hybrid, RescueNet/FloodNet, limited novelty, off-topic domain
...
### [244] [HarMoE: Multi-Source Chest Radiograph Pretraining with Dataset-Disentangled Experts](https://arxiv.org/abs/2608.02252)
...
TL;DR: 提出 TALSC 框架，针对基础设施辅助自动驾驶中车载小 VLM 与边缘大 VLM 协作的时效性问题，建模 AoI 演化并设计 Lyapunov 在线调度算法，在 nuScenes 上实现最高 12.6% 的 Micro-F1 提升。


<details>
  <summary>Details</summary>
Motivation: 车载算力受限导致只能部署小 VLM，感知与推理能力有限；基础设施辅助自动驾驶可通过边缘服务器的大 VLM 协作缓解资源约束，但动态车载环境中感知数据的时效性快速衰减，大 VLM 的精度增益会被其推理延迟带来的时效性退化抵消，需要平衡精度与时效性。

Method: 提出 TALSC 框架：首先建模 VLM 推理的 AoI（信息年龄）演化，刻画 AoI、token 长度与任务性能之间的耦合关系并构建通用时效性度量；在此基础上设计 TALSC 在线调度算法，由于调度决策对未来时效性度量有延迟影响且输出 token 数在调度时未知，采用 Lyapunov drift-plus-estimated-penalty 算法并提供性能保证。

Result: 基于 nuScenes 数据集进行案例研究拟合时效性度量，并在多种通信与计算设置下验证 TALSC 优于基线方法，相比最佳基线在 Micro-F1 分数上实现最高 12.6% 的归一化提升。

Conclusion: TALSC 通过时效性感知的大小 VLM 协作调度，有效平衡了大 VLM 的精度增益与延迟带来的时效性退化，为基础设施辅助自动驾驶中的 VLM 部署提供了可行的在线调度方案。

Classification reason: 论文研究基础设施辅助自动驾驶中大小 VLM 协作调度框架，属于视觉语言模型在自动驾驶场景下的应用与调度优化，映射到 [CANONICAL] vlm_mllm_general（视觉语言模型与 MLLM）子主题。

Importance reason: Focuses on VLM scheduling for autonomous driving, which is tangential to your core interests in continuous multimodal models and autoregressive generation. Signals: VLM collaboration, autonomous driving, Age of Information, Lyapunov scheduling, nuScenes
...
### [246] [Semantically Calibrated Evidence Composition for CT Vision-Language Learning](https://arxiv.org/abs/2608.00239)
*Guoliang You, Haifan Gong, Xiaomeng Chu*
...
TL;DR: 提出 SCOPE 框架，通过语义校准的证据组合（Semantic Calibration Of comPosed Evidence）进行 CT 视觉语言学习，将器官级局部证据与全局研究级语义结合，在 CT-RATE 和 RadChestCT 上取得 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 现有 CT-报告学习方法要么强调全局对齐（保留整体上下文但局部证据贡献隐式），要么强调解剖级对齐（显式定位局部发现但未说明独立证据如何交互并贡献到全局表征）。需要弥合局部证据与全局语义之间的鸿沟。

Method: SCOPE 框架包含：1) 在器官特定报告监督下，使用带固定解剖身份的掩码引导查询从共享未裁剪体素特征中提取上下文感知的器官证据；2) 无限制的全局查询保留全卷上下文，通过 Local-Global Coupling 将器官证据组合为统一证据表征；3) 用诊断摘要对组合证据进行语义校准，提供超越局部器官描述的研究级监督；4) 将校准后的证据作为受控残差集成到保留上下文的整体表征中，与完整报告对齐。

Result: 在 CT-RATE 和 RadChestCT 上分别达到 85.0 和 72.2 的 macro AUC，超过此前 SOTA 7.2 和 4.2 个百分点；在线性探测和跨模态检索上也取得显著提升。

Conclusion: 语义校准的证据组合能够有效连接局部证据与研究级语义，在不将 CT 表征简化为预定义器官集合的前提下提升表征可迁移性，验证了渐进式证据组合路径的有效性。

Classification reason: 该论文研究 CT 影像与放射报告的视觉语言学习，属于医学影像理解与跨模态对齐任务，与"视觉问答与文档理解"（multimodal_vqa_doc）这一 CANONICAL 子主题合理相关，故复用该子主题。

Importance reason: CT medical vision-language learning is outside your configured research directions and negative keyword domains. Signals: CT vision-language, medical imaging, evidence composition, SOTA on CT-RATE, not in priority subtopics
...
### [247] [Beyond Static Anchors: Bounded Prototype Conditioning for Language-Free Medical Anomaly Detection](https://arxiv.org/abs/2608.00442)
...
TL;DR: ReCAP 是一个语言无关的医学异常检测框架，用输入条件化的视觉原型替代静态锚点（文本提示或固定视觉 token），通过有界门控调制为每张图像重新居中分离的正常/异常原型，实现查询自适应的异常评分，并在少样本场景引入非参数正常参考记忆。在六个医学基准上取得最优零样本和 23/24 少样本图像级 AUROC，推理延迟降低 70% 以上。


<details>
  <summary>Details</summary>
Motivation: 现有 CLIP 类医学异常检测方法依赖视觉-语言对齐来减少标注需求，但其正常/异常参考（文本提示或学习到的视觉 token）在测试图像间保持固定。这种静态参考在跨域医学成像场景中可能无法可靠迁移到未见目标，导致检测性能下降。

Method: ReCAP 采用语言无关框架，用输入条件化的视觉原型替代静态锚点。通过有界门控调制为每张图像重新居中分离的正常与异常原型，实现查询自适应异常评分，同时约束上下文引起的原型漂移。少样本场景引入非参数正常参考记忆，保留实例级目标域变化并补充条件原型分支。

Result: 在六个医学基准上，ReCAP 在所有零样本设置和 23/24 少样本设置中取得最佳图像级 AUROC，并在三个分割数据集上取得最佳零样本像素级 AUROC。相比最快基线，推理延迟降低超过 70%，且无需文本提示或测试时梯度更新。

Conclusion: ReCAP 证明输入条件化的视觉原型比静态锚点更有效，能更好地泛化到跨域医学成像场景，同时保持高效推理，为医学异常检测提供了一种无需语言提示的实用方案。

Classification reason: 论文提出语言无关的医学异常检测框架 ReCAP，通过输入条件化的视觉原型替代静态锚点，属于医学图像理解与异常检测任务。虽然不完全是 VQA/文档理解，但该论文属于视觉理解范畴，且没有更贴切的 canonical 子主题。考虑到医学异常检测本质上是视觉理解与分类任务，映射到多模态理解与生成方向下的视觉问答与文档理解子主题（作为视觉理解任务的广义归属）。

Importance reason: Solid medical anomaly detection method but outside your core focus areas and hits a negative keyword. Signals: medical anomaly detection, language-free CLIP alternative, efficient inference (70% latency cut), few-shot AUROC gains, negative keyword: medical diagnosis
...
### [248] [Entity-Faithful Repair of Synthetic Supervision for Zero-Shot Image Captioning](https://arxiv.org/abs/2608.00994)
...
TL;DR: 提出 ReCap 框架，通过显式实体级对齐修复合成图像-文本对中的细粒度错配（缺失实体或属性误置），用于零样本图像描述生成。


<details>
  <summary>Details</summary>
Motivation: 现有零样本图像描述方法利用文生图模型从纯文本语料合成训练数据，但大多只关注整体数据质量。作者观察到合成图像-文本错配往往是结构化且细粒度的：全局看似合理但存在缺失实体或属性误置，基于全局相似度的重匹配/重生成无法系统性修复实体级错配。

Method: 提出 ReCap 即插即用框架：1) 用检测到的图像支持实体引导标题重写，强制实体级对应关系，将合成数据精炼从隐式全局匹配转为显式细粒度重对齐；2) 引入自适应动态加权学习策略，在训练中降低不可靠合成对的权重。

Result: ReCap 在域内和跨域零样本图像描述基准上均一致提升图像-文本一致性，达到最先进性能。

Conclusion: 实体级显式重对齐比全局相似度匹配更能有效修复合成监督中的细粒度错配，ReCap 作为通用框架可集成到现有合成数据管线中。

Classification reason: 论文研究零样本图像描述生成（image captioning），属于视觉语言理解与生成任务，映射到多模态理解与生成方向下的视觉问答与文档理解（multimodal_vqa_doc）子主题，该子主题涵盖图像描述生成相关任务。

Importance reason: Focuses on synthetic data repair for zero-shot captioning, which is tangential to the user's core interests in continuous multimodal models and video generation. Signals: zero-shot image captioning, synthetic data refinement, entity-level alignment, plug-and-play framework, image-text consistency
...
### [249] [FAU at ImageCLEF 2026 Task on Multimodal Reasoning Robust Candidate Scoring and Concise Multilingual Visual Answering](https://arxiv.org/abs/2608.01664)
...
Importance: 普通相关 (49.42) - 未入选精读

TL;DR: 本文提出 ImageCLEF 2026 多模态推理系统，针对视觉多选题与开放问答两个子任务，通过候选标签评分、分数融合投票、图像增强、确定性解码与后处理等推理工程手段，在无任务特定训练的情况下分别获得 MCQ 第三名与 OpenQA 第一名。


<details>
  <summary>Details</summary>
Motivation: 多模态推理竞赛中，模型选择固然重要，但输出控制同样关键。作者发现脆弱自由生成难以满足严格答案格式要求，需要稳健的推理工程来提升 VLM 在密集文本、图表、公式等教育科学图像上的问答可靠性。

Method: 对 Visual MCQ 采用直接候选标签评分（从 VLM logits 中打分）替代自由生成，并通过分数融合与投票组合互补运行；对 Visual OpenQA 采用图像增强、简洁答案提示、确定性解码及针对性后处理去除推理痕迹与格式伪影。

Result: 官方提交在 Visual MCQ 获得第三名（准确率 0.7108），在 Visual OpenQA 获得第一名（COMET 0.6488、BLEU 0.1391、ROUGE L 0.2762、METEOR 0.2383）。

Conclusion: 推理工程（评分、集成、提示与清理）能显著提升强 VLM 在竞赛场景中的可靠性，稳健的输出控制与模型选择同等重要。

Classification reason: 论文针对 ImageCLEF 2026 多模态推理任务，涉及视觉多选题（Visual MCQ）与视觉开放问答（Visual OpenQA），属于视觉问答与文档理解范畴，映射到 [CANONICAL] 子主题 multimodal_vqa_doc。

Importance reason: Competition-oriented inference engineering for VQA, with no novel methodology or alignment to the user's core continuous/multimodal generation research. Signals: inference engineering, VQA competition, no training, logit scoring, ensembling
...
### [250] [EchoChange: A Diffusion Language Model with Dual Pass Remasking for Factual Remote Sensing Disaster Change Captioning](https://arxiv.org/abs/2608.01856)
...
TL;DR: EchoChange 提出一种多模态离散扩散语言模型，将遥感灾害变化描述生成建模为迭代掩码 token 去噪而非自回归生成，通过 draft-aware 双通道训练、渐进掩码课程和置信度引导重掩码来纠正早期错误，在 RSCC 基准上显著超越基线。


<details>
  <summary>Details</summary>
Motivation: 现有变化描述生成方法采用自回归解码范式，早期对变化对象、事件或空间关系的误判会成为后续文本的不可逆前提，将视觉歧义放大为级联事实错误。作者希望打破这种从左到右生成的局限。

Method: 提出 EchoChange，一种多模态离散扩散语言模型，将变化描述生成建模为迭代掩码 token 去噪。引入 draft-aware 双通道训练、渐进掩码课程和置信度引导重掩码，使训练与迭代推理对齐，在图像对条件下反复修订整个描述以纠正不完美的中间预测。

Result: 在 RSCC 基准上的大量实验表明，EchoChange 在词汇和语义指标上均大幅优于通用及遥感专用基线。

Conclusion: 将变化描述生成从自回归范式转向迭代掩码去噪的扩散语言模型范式，可有效缓解早期误判导致的级联事实错误，为遥感灾害变化描述提供了更可靠的方法。

Classification reason: 该论文研究遥感灾害变化描述生成（change captioning），属于视觉语言理解与生成任务，与多模态理解与生成方向下的"视觉问答与文档理解"（multimodal_vqa_doc）这一 CANONICAL 子主题相关，因为变化描述生成本质上是基于图像对的视觉语言描述任务。

Importance reason: Remote sensing disaster captioning is outside your core priorities, though the diffusion language model and iterative remasking technique are methodologically interesting. Signals: diffusion language model, iterative masked-token denoising, remote sensing (negative keyword), change captioning, draft-aware dual-pass training

Abstract: Bi-temporal remote-sensing disaster change captioning often needs to identify sparse and spatially localized changes across large pre- and post-event scenes and then translate them into coherent, factual descriptions. However, existing change captioning methods always follow an autoregressive decoding paradigm to generate the change description and thus an early misinterpretation of the changed object, event, or spatial relation becomes an irreversible premise for subsequent text, amplifying visual ambiguity into cascading factual errors. To address this limitation, we propose EchoChange, a multimodal discrete diffusion language model that formulates change captioning as iterative masked-token denoising rather than left-to-right generation. By repeatedly revising the entire caption while conditioning on the image pair, EchoChange can reconsider uncertain content and correct imperfect intermediate predictions. We further introduce draft-aware dual-pass training, a progressive masking curriculum, and confidence-guided remasking to align training with iterative inference. Extensive experiments on the RSCC benchmark show that EchoChange substantially outperforms both general-purpose and remote-sensing-specific baselines across lexical and semantic metrics. The EchoChange Project is at
...
### [251] [Coverage-Driven Adaptive Keyframe Selection for Video Understanding](https://arxiv.org/abs/2608.00714)
...
TL;DR: 提出 CSES，一种无需训练的自适应语义关键帧选择器，通过估计帧-查询相关性分布的显著性来主动获取帧并自适应调整时间覆盖，将关键帧选择建模为覆盖问题，联合考虑语义相关性、时间冗余与视觉冗余，贪心优化具有次模近似保证。


<details>
  <summary>Details</summary>
Motivation: 长视频理解中处理大量帧带来巨大计算开销。现有方法在推理前对帧-查询相关性打分并选择关键帧，但相关帧的分布随查询变化，且这些方法往往需要对成百上千帧打分，成本高昂。

Method: CSES 通过估计帧-查询相关性分布的显著性来引导主动获取并自适应调整每个输入的时间覆盖范围；将关键帧选择形式化为覆盖问题，联合考虑语义相关性、时间冗余和视觉冗余；主动获取与关键帧选择基于覆盖饱和终止；选择目标为单调次模函数，可用贪心优化并具有标准近似保证。

Result: 在四个 LVLM 和两个基准上，CSES 在保持精度的同时，比现有基线少打分 4-13 倍帧、少选择 18.4%-20.5% 的输入关键帧，并在帧选择上实现 3.1-5.4 倍加速。

Conclusion: CSES 通过自适应确定打分帧数与关键帧数，显著降低长视频理解的计算开销，同时保持精度，为训练无关的关键帧选择提供了高效且可证明的优化方案。

Classification reason: 论文提出 CSES，一种面向长视频理解的自适应关键帧选择方法，用于降低 LVLM 推理成本。这属于视频理解与时序推理范畴，映射到 [CANONICAL] 子主题 video_understanding_temporal。

Importance reason: Efficient keyframe selection for video understanding is useful but tangential to the user's core focus on continuous multimodal models and generative world models. Signals: training-free, adaptive keyframe selection, submodular coverage, LVLM inference efficiency, video understanding
...
### [252] [CAVE: Competence-Aware Visual Boundary Evidence Alignment for Video Temporal Grounding](https://arxiv.org/abs/2608.02078)
...
TL;DR: 提出 CAVE 方法，通过边界特定视觉证据奖励与能力感知门控，缓解视频时序定位中视觉证据与预测时间戳之间的错位问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的视频时序定位方法主要依赖结果正确性奖励，仅评估最终预测区间，未充分约束边界相关的视觉证据及其与时间戳预测的对应关系，导致视觉证据与预测时间戳普遍错位。

Method: CAVE 引入边界特定证据 token 表示边界级视觉证据，通过轻量监督预热初始化其结构化生成与边界语义；在 RL 阶段使用视觉边界证据对齐奖励强化特殊证据 token 在真实边界内的视觉注意力；并设计性能感知门控，对定位较差的组保留证据引导，对定位足够准确的组减少约束以避免过度限制细粒度边界细化。

Result: 在多个公开 VTG 基准上的大量实验验证了方法的有效性。

Conclusion: 通过显式建模边界级视觉证据并引入对齐奖励，CAVE 有效缓解了视觉证据与时间戳预测的错位，提升了视频时序定位性能。

Classification reason: 论文研究视频时序定位（Video Temporal Grounding），属于视频理解与时序推理范畴，映射到 [CANONICAL] 子主题 video_understanding_temporal。

Importance reason: Focuses on video temporal grounding with RL-based evidence alignment, which is peripheral to your core interests in continuous multimodal models and generative world models. Signals: video temporal grounding, reinforcement learning, visual evidence alignment, LVLM, boundary tokens
...
### [253] [Ground, Cover, and Refine: Evidence-Centric Frame Selection for Long-Video Question Answering](https://arxiv.org/abs/2608.01660)
...
TL;DR: 提出 GCR 框架，将固定预算下的长视频帧选择建模为联合证据策展问题，通过 Ground（时间事件锚定）、Cover（视觉锚定补充与全局MMR去重）、Refine（重访遗漏区域并替换弱帧）三步，在无需训练的情况下提升长视频问答性能。


<details>
  <summary>Details</summary>
Motivation: 长视频问答需要在有限的视觉 token 预算下从数千帧中识别稀疏但关键的证据。现有方法要么单次选择查询相关帧导致局部峰值聚集、预算耗尽后遗漏证据无法恢复，要么仅将带时间戳的文本作为检索引导导致文本与视觉证据弱对齐。

Method: GCR 是免训练框架，包含三步：Ground 将带时间戳文本转换为时间事件，选择查询相关的真实帧锚点，并将事件文本渲染到时间对齐的帧上；Cover 用直接视觉锚点补充接地事件以获取互补视觉证据，并应用全局最大边际相关（MMR）保持上下文多样性；Refine 重访遗漏的时间区域，仅当真实帧 medoid 具有更高证据价值时替换最弱的可重访上下文帧。GCR 保持固定数量的按时间排序帧，无需 VLM 训练或架构修改。

Result: 在 LongVideoBench 和 Video-MME 上，跨三个 7B 骨干和 8/32/64 帧预算均取得一致改进。使用 7B LLaVA-OV 骨干和 32 帧时，两个基准分别达到 64.25% 和 62.15%，超过最强复现基线 2.54 和 1.93 个百分点。

Conclusion: GCR 通过将帧选择视为联合证据策展问题，有效缓解了局部峰值聚集和文本-视觉证据弱对齐两大局限，在无需训练的情况下显著提升长视频问答性能，验证了证据中心化帧选择策略的有效性。

Classification reason: 论文聚焦长视频问答中的关键帧选择与证据筛选，属于长视频理解与时序推理范畴，映射到 [CANONICAL] 子主题 video_understanding_temporal。

Importance reason: Training-free frame selection for long-video QA is useful but tangential to your core focus on continuous/multimodal generation and world models. Signals: long-video QA, frame selection, training-free, evidence curation, MMR
...
### [254] [RSC-GestureNet: Reliability-Aware Selective Causal Recognition of Chinese Traffic Police Gestures](https://arxiv.org/abs/2608.02200)
...
TL;DR: RSC-GestureNet 是一个可靠性感知的选择性因果识别器，用于从连续全帧视频中识别中国交警手势。它将姿态置信度作为一等信号，在因果图推理中降低不可靠关节权重，并通过可靠性感知推理规则选择性输出校准预测，同时引入 CTPGesture-C 特征级损坏基准。


<details>
  <summary>Details</summary>
Motivation: 交警手势是自动驾驶中安全关键的感知线索，可部署的识别器必须从连续全帧视频中因果地推断指令，在过渡性手臂运动时保持稳定，并避免过度信任损坏的姿态测量。现有方法缺乏对姿态可靠性的显式建模。

Method: RSC-GestureNet 将姿态置信度作为一等信号：在图推理中对不可靠关节降权，因果地聚合时序证据，并通过可靠性感知推理规则选择性输出校准预测。同时提出 CTPGesture-C 特征级损坏基准（七种姿态/RGB 退化族）和 RGB 级诊断流程。

Result: 在官方 CTPGesture v1 完整划分（134,424 标注帧、33,451 因果窗口）上达到 93.33% 准确率、91.71% macro-F1、91.69% 在线 macro-F1、98.80% Early@10、0.153s TTC，并取得最佳鲁棒 macro-F1；超过 MD-GCN 和 HLP-GCN 基线 3.23-4.11 macro-F1 点和 2.15-3.07 在线-F1 点。

Conclusion: 显式的姿态可靠性建模能够提升交通指令识别的早期性、稳定性和鲁棒性，校准、选择性风险、统计、自适应分支和图像级重提取分析均支持这一结论。

Classification reason: 该论文研究从连续全帧视频中因果识别中国交警手势，属于视频理解与时序推理范畴，与 [CANONICAL] video_understanding_temporal 子主题（视频理解、时序推理、动作识别）合理相关，故复用该子主题。

Importance reason: Niche gesture-recognition robustness work with limited overlap with the user's core continuous-model, video-generation, and world-model priorities. Signals: causal video recognition, pose confidence, robustness benchmark, traffic police gestures, autonomous driving
...
### [255] [GROVE: Growing and Reasoning over Temporally Stratified Memory from Streaming Video Experience](https://arxiv.org/abs/2608.02392)
...
### [256] [LUT: Latent Utility Training for Visual Reasoning](https://arxiv.org/abs/2608.00743)
...
Importance: 高优先级 (83.17) - 未入选精读

TL;DR: LUT 提出一种仅用标准 VQA 对训练的潜空间视觉推理框架，通过轨迹级 Utility-Aware Latent Distillation SFT 和步骤级 Latent Attribution Policy Optimization 优化潜空间推理，无需昂贵的中间监督（如边界框、草图、交错推理），在感知密集型视觉推理基准上优于先前潜推理方法。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在感知密集型视觉推理上仍有挑战。现有潜空间视觉推理方法依赖昂贵的中间监督（边界框、草图、交错推理），且只关注潜状态应如何塑造，未显式评估潜状态对最终答案是否有用。作者希望仅用标准 VQA 对训练潜推理，并显式优化潜状态的效用。

Method: LUT 在轨迹级提出 Utility-Aware Latent Distillation SFT：探索与答案相关的潜轨迹，按信息增益筛选合格轨迹，并通过课程学习蒸馏更可靠可学的监督。在步骤级提出 Latent Attribution Policy Optimization：利用答案到潜状态的归因，在强化学习中对潜步骤进行差分优化。

Result: 在感知密集型视觉推理基准上，LUT 优于先前潜推理方法，并与潜文本交错方法性能相当，同时标注成本更低。

Conclusion: LUT 证明仅用标准 VQA 对即可有效训练潜空间视觉推理，通过显式优化潜状态的效用（轨迹级蒸馏与步骤级归因优化）可提升推理性能，降低对昂贵中间监督的依赖。

Classification reason: 论文提出 LUT，一种在连续潜空间中进行视觉推理的框架，通过 latent utility 训练优化潜空间推理轨迹，属于连续多模态推理与潜空间思维方向，映射到 [CANONICAL] continuous_multimodal_reasoning。

Importance reason: Directly advances continuous latent visual reasoning with a novel utility-based training paradigm, highly aligned with your priority on continuous multimodal reasoning. Signals: latent visual reasoning, continuous latent trajectory, utility-aware distillation, latent attribution policy optimization, no intermediate supervision
...
<div id='direction-multimodal_understanding_generation-continuous_visual_tokenizer'></div>

## 连续视觉 Tokenizer 与表征自编码器 [[Back]](#toc)

面向理解与生成统一的连续视觉 token、VAE/ViT、RAE、DINO/SigLIP/MAE latent 与视觉表征重建。

### [257] [Location-Aware Fine-Grained Representation Learning for Medical Vision Foundation Models](https://arxiv.org/abs/2608.00976)
...
TL;DR: Slot2Text 提出一种双模式手术 MLLM，将密集视觉表征压缩为少量区域 slot latent 作为带区域标签的视觉 token，实现高效推理与空间可追溯的视觉定位回答。


<details>
  <summary>Details</summary>
Motivation: 手术场景 MLLM 通常向语言模型注入数百个密集视觉 token，导致推理成本高昂且生成答案缺乏空间可追溯性。作者希望用紧凑的 slot latent 替代密集视觉表征，在保持性能的同时大幅降低 token 消耗，并支持可定位的视觉推理。

Method: Slot2Text 采用双模式设计：将自监督视觉特征分组为少量区域 slot latent，作为带区域标签的视觉 token 供语言模型消费。Slot2Text-Fast 使用 slot 前缀直接回答手术问题；Slot2Text-Reason 额外识别并定位与推理相关的区域，将语言输出与对应 slot token、mask 或区域关联，提供可追溯的空间证据。

Result: 在多个 VQA 和视觉定位基准上，Slot2Text-Fast 以远低于基线的成本达到与 SOTA 基线相当的性能，平均总 token 消耗降低 91.8%，视觉前缀从 1295 个 token 降至 47 个（减少 96.4%）。Slot2Text-Reason 以额外 token 和延迟换取显式的区域身份、位置与可追溯空间证据。

Conclusion: 紧凑的 slot latent 可作为手术 MLLM 的高效默认视觉接口，在需要更强空间可追溯性时可调用基于定位的推理模式，为高效且可解释的多模态理解提供了新范式。

Classification reason: 论文将自监督视觉特征分组为紧凑的 slot latent 作为视觉 token 注入语言模型，属于连续视觉 tokenizer/表征压缩方向，映射到 [CANONICAL] continuous_visual_tokenizer。同时该工作面向手术 MLLM 理解与定位，也匹配多模态理解与生成方向。

Importance reason: Strongly aligns with continuous visual tokenization and efficient MLLM inference, though domain is surgical rather than general multimodal. Signals: continuous visual token, slot latent, efficient inference, 96.4% visual token reduction, spatial traceability
