...
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 22]
  ...
  - [多模态评测与基准](#direction-multimodal_understanding_generation-multimodal_benchmark) [Total: 4]
...
TL;DR: HRIBench 是一个面向人机协作的交互中心诊断基准，包含13个角色条件任务和650+评估片段，引入同步性、响应性、协议遵循和安全性等可解释指标。评估发现当前基础机器人策略（GR00T、pi0.5、ACT）在协作场景中表现不佳，微调后性能显著提升，真实世界迁移实验将任务成功率从0.10提升至0.43。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 基准主要评估孤立操作技能，缺乏对人机交互结构的建模。真实协作需要共享代理下的意图理解、时序同步、协议遵循和安全交互，现有基准无法诊断这些能力。

Method: 将协作任务建模为结构化场景脚本，显式建模代理角色、时序依赖、协调约束和人类行为分布。定义三种交互角色（Instructor、Collaborator、Intruder），覆盖意图通信、联合协调和人类干预下的鲁棒性。基于 GR00T、pi0.5 和 ACT 在统一协议下评估，并引入同步性、响应性、协议遵循和安全性等可解释指标。

Result: 当前基础机器人策略在协作设置中表现不佳，存在时序协调和意图感知行为的重大局限。在 HRIBench 上微调一致提升协作性能。真实世界迁移实验中，HRIBench 生成的仿真数据将 GR00T N1.5 的物理任务成功率从0.10提升至0.43。

Conclusion: HRIBench 有效揭示了现有机器人策略在人机协作中的不足，为推进交互中心机器人学习提供了有价值的诊断基准和训练数据生成工具。

Classification reason: 该论文提出人机协作基准 HRIBench，评估机器人策略在共享代理下的意图理解、时序同步、协议遵循和安全交互能力，属于具身智能中机器人规划与控制的范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong benchmark for human-robot collaboration but only tangentially touches world model and continuous multimodal priorities. Signals: Human-robot collaboration benchmark, interaction-centric metrics, VLA policy evaluation, real-world transfer improvement, limited direct alignment with continuous language/multimodal models
...
TL;DR: DROPJ 是一种以人为中心的安全智能体训练方法：先学习世界模型，让人类在模拟器中生成轨迹，收集偏好与理由，训练奖励模型，结合世界模型通过 MPC 直接部署。实验表明该方法能降低计算成本、提升部署性能，并通过安全理由显著增强安全性。


<details>
  <summary>Details</summary>
Motivation: 在安全关键环境中，环境动力学未知且缺乏合适的奖励函数，传统强化学习难以应用。需要一种利用人类输入来安全训练和部署智能体策略的方法。

Method: 首先从真实世界轨迹数据集中学习一个世界模型（learned simulator）。人类在该模拟器中交互生成信息丰富的模拟轨迹。从这些轨迹中采样片段对，让人类提供偏好选择及其理由（justification）。基于这些带理由的偏好训练奖励模型，最后结合世界模型使用模型预测控制（MPC）直接部署智能体。

Result: 真实用户实验表明：1）从用户生成信息丰富的模拟轨迹相比其他策略显著降低了训练计算成本；2）在模拟器中训练时，使用偏好反馈相比其他反馈类型大幅提升了部署性能；3）偏好附带的安全理由能显著增强安全性，或优先考虑用户指定的安全方面。

Conclusion: DROPJ 通过世界模型与人类偏好及理由的结合，实现了安全关键环境下的安全策略训练与部署。偏好反馈优于其他反馈类型，而安全理由能进一步提升安全性，为安全强化学习提供了有效的人机协作范式。

Classification reason: 该论文使用世界模型（learned simulator）进行安全策略训练与部署，结合人类偏好和理由（justifications）进行奖励建模，并通过模型预测控制（MPC）部署智能体。这完全符合 [CANONICAL] 具身世界模型与规划控制 的定义，即面向机器人/智能体的动作规划和控制的世界模型。

Importance reason: World model + human preference learning is tangentially relevant but lacks direct connection to your core continuous multimodal or autoregressive generation priorities. Signals: world model, human preferences, safe agent training, model predictive control, safety justifications
...
### [3] [GPUSimBench: Towards Scalable and Reliable GPU-Accelerated Simulators in Embodied AI](https://arxiv.org/abs/2607.13059)
...
TL;DR: 本文提出GPUSimBench基准，系统评估主流GPU加速机器人仿真器（Isaac Lab、Genesis）在可扩展性、物理一致性和计算确定性方面的权衡，揭示了GPU批处理执行中固有的非确定性及其对可重复性的影响。


<details>
  <summary>Details</summary>
Motivation: 数据驱动的具身AI正转向大规模并行仿真范式，GPU加速仿真器作为基础数据基础设施，其并行效率、物理保真度和执行确定性之间的权衡尚未被系统研究，阻碍了可靠机器人学习的发展。

Method: 提出GPUSimBench基准，包含：(1) 物理基础评估——使用受控斜面任务量化仿真动力学与真实世界的分布对齐；(2) 并行可扩展性基准——测量不同环境数量下的吞吐量和内存占用；(3) 非确定性量化——揭示GPU批处理执行中固有的运行间和环境间变异性；(4) 识别仿真器栈中的四种随机性经验区域。

Result: 量化了主流GPU仿真器在物理一致性、可扩展性和确定性方面的性能，揭示了GPU批处理执行引入的显著非确定性，并识别出四种随机性经验区域，表明无约束的扩展会损害可重复性。

Conclusion: GPU加速仿真器在大规模并行训练中至关重要，但其非确定性问题需要明确的约束机制来保证可重复性，为未来具身AI仿真基础设施的设计提供了重要指导。

Classification reason: 该论文研究GPU加速仿真器在具身AI中的可扩展性与可靠性，属于具身智能环境模型与仿真基础设施，与[CANONICAL]具身世界模型与规划控制方向高度匹配。

Importance reason: Focuses on GPU simulator benchmarking for embodied AI, not directly on the user's priority subtopics like continuous language models or video generation. Signals: GPU-accelerated simulators, embodied AI scalability, non-determinism in GPU batch execution, physical consistency evaluation, Isaac Lab and Genesis benchmarking
...
### [4] [Ego-Dynamics-Augmented World Model for Autonomous Driving with Zero-Shot Cross-Chassis Adaptation](https://arxiv.org/abs/2607.13410)
...
TL;DR: DynaDreamer 是一种面向自动驾驶的动力学增强世界模型强化学习方法，通过引入显式的自车动力学先验（物理信息编码器-解码器）来解耦自车运动与场景动态，减少世界模型对自车运动恢复的建模负担，从而提升场景建模保真度和想象轨迹精度，并实现零样本跨底盘迁移。


<details>
  <summary>Details</summary>
Motivation: 现有基于 BEV 表示的驾驶世界模型中，连续帧之间的变换天然纠缠了自车运动与场景动态，导致世界模型需要花费大量容量从扭曲的观测中恢复自车运动，牺牲了场景建模保真度和想象精度。此外，不同底盘参数（如轴距、转向比）的车辆动力学特性不同，现有方法难以零样本迁移到新底盘。

Method: 提出 DynaDreamer，在 Dreamer 框架基础上增加显式的自车动力学先验模块：(1) 物理信息自车动力学编码器-解码器，将自车状态历史提取为紧凑可识别的上下文表示；(2) 因果 Transformer 世界模型，利用该上下文调节先验和后验潜变量；(3) 自车动力学预测器，在想象 rollout 过程中向前传播上下文以保持同步。通过信息论分析证明，该上下文降低了观测转移的预测熵和先验-后验 KL 散度，将世界模型的建模负担限制在场景动态上。

Result: 在城市和高速公路驾驶场景中，DynaDreamer 的任务成功率分别比最强基线提升 28% 和 61%；在迁移到未见过的底盘时，优势提升至 73%。

Conclusion: 显式建模自车动力学先验可以有效解耦自车运动与场景动态，提升世界模型的建模效率和想象精度，同时实现零样本跨底盘迁移，为自动驾驶中世界模型的高效学习和泛化提供了新思路。

Classification reason: 该论文提出 DynaDreamer，一种基于 Dreamer 风格世界模型的强化学习方法用于自动驾驶，属于具身智能中的规划控制范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong world model contribution for autonomous driving with zero-shot adaptation, but outside user's core priority subtopics. Signals: world model, zero-shot cross-chassis adaptation, ego-dynamics prior, autonomous driving, reinforcement learning
...
TL;DR: 提出Anchor-Align方法，通过视觉语言锚定（冻结VLM蒸馏层表征）和语言-动作对齐（将动作目标转为离散运动方向标签联合训练）来缓解VLA策略微调中的表征漂移和语言-动作错位问题，在物理xArm7机器人和多个仿真基准上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA策略通过行为克隆微调预训练VLM时，会逐渐覆盖预训练的表征，导致视觉和语义泛化能力下降。仅靠联合训练网络图像-文本数据无法解决此问题，因为语言和动作损失应用于不同的观测数据，造成语言-动作错位。标准操作基准无法暴露这一问题。

Method: 提出Anchor-Align方法，包含两个目标：(1) 视觉语言锚定（Vision-Language Anchoring）：从冻结的VLM副本中蒸馏逐层表征，防止表征漂移；(2) 语言-动作对齐（Language-Action Alignment）：将每个动作目标转换为离散的运动方向标签，在同一机器人观测上联合训练语言和动作预测。

Result: 在物理xArm7机器人上，两种VLA架构的成功率分别从28%提升至54%、从37%提升至60%。在仿真中，在LIBERO-PRO（OOD扰动）、LIBERO-Plus（感知鲁棒性）和CALVIN（长时程控制）上均取得一致改进。

Conclusion: 保留预训练表征和有效的动作学习并非根本矛盾，Anchor-Align通过表征锚定和语言-动作对齐可以同时提升VLA策略的泛化能力和操作成功率。

Classification reason: 该论文研究机器人VLA策略微调，涉及具身智能中的动作规划与控制，属于世界模型方向下的具身世界模型与规划控制子主题。

Importance reason: Addresses representation drift in VLA finetuning, but focuses on robotics rather than core continuous multimodal or video generation priorities. Signals: VLA finetuning, representation anchoring, language-action alignment, robotics, world model
...
### [6] [Reverse to Advance: Teleoperation-Cost Effective Hard Policy Learning from Reversed Easy Tasks](https://arxiv.org/abs/2607.13455)
...
Importance: 普通相关 (47.27) - 未入选精读

TL;DR: 提出一种遥操作成本高效的框架，通过时间反转易任务轨迹来学习困难操作策略，包含自动数据采集、分层数据精炼和迭代策略学习三个组件，在仿真和真实机器人上提升了困难任务的成功率。


<details>
  <summary>Details</summary>
Motivation: 高质量遥操作数据集收集成本高昂，尤其是困难任务。许多任务存在方向不对称性：正向困难任务难以完成，但反向（放松或破坏环境）却相对容易。利用反向易任务轨迹作为监督信号可降低人工演示成本。

Method: 提出三组件框架：(1) 闭环数据采集管道，交替执行困难/易任务策略以自动重置环境并生成多样化轨迹；(2) 分层数据精炼管道，时间反转易任务轨迹，利用运动学先验和评论家引导的优势过滤器筛选低质量动作；(3) 迭代策略学习方法，结合初始反向易任务演示和过滤后的反向数据进行持续在线学习。

Result: 在两个仿真基准和真实机器人实验中，该方法相比基于反转的基线和强化学习基线，以更高的数据效率和更稳定的训练提升了困难任务成功率，无需大量困难任务遥操作。

Conclusion: 通过自动采集、分层精炼和迭代学习，该方法实现了可扩展、可靠的复杂高精度操作任务训练，有效降低了遥操作成本。

Classification reason: 该论文关注机器人操作策略学习，利用反向易任务数据训练困难任务策略，属于具身智能中的策略学习与规划控制，与 [CANONICAL] 具身世界模型与规划控制 方向高度匹配。

Importance reason: The paper focuses on robotic manipulation and teleoperation cost efficiency, which is outside the user's priority subtopics and negative keywords. Signals: Robotics, Teleoperation, Policy learning, Task reversal, Not in priority subtopics
...
### [7] [Joint On-and-Off Policy Learning for Vision-and-Language Navigation](https://arxiv.org/abs/2607.13461)
...
TL;DR: 本文提出JOP-VLN框架，将离线模仿学习与在线强化学习协同结合，通过三阶段训练流程（模仿学习→DAgger探索→联合在线/离线策略学习）在VLN-CE R2R和RxR基准上取得了新的最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有VLM驱动的VLN方法存在两种独立范式：模仿学习（IL）和强化学习（RL），但缺乏将两者有效整合的方法。作者希望结合离线IL的稳定性和在线RL的探索能力，提升导航智能体的性能。

Method: 提出三阶段训练流程：1）在专家演示上进行IL获得基础导航技能；2）使用DAgger算法生成启发式探索轨迹，再用于IL以提升纠错能力；3）联合在线-离线策略学习框架，包含高熵轨迹采样提升RL训练效率，以及优先纠错的轨迹排序策略。

Result: 在VLN-CE R2R基准上达到69.9%的成功率，在RxR基准上达到68.0%的成功率，在R2R上取得了新的最优结果。

Conclusion: JOP-VLN通过联合离线模仿学习与在线强化学习，有效提升了VLN任务的导航性能，验证了两种范式协同的可行性。

Classification reason: 该论文研究视觉语言导航（VLN），属于具身智能中的导航任务，涉及智能体在物理世界中根据自然语言指令进行动作规划与控制，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Combines imitation and reinforcement learning for VLN, but is not aligned with user's priority on continuous multimodal models or generative world models. Signals: VLN, imitation learning, reinforcement learning, DAgger, embodied agent
...
### [8] [EgoHTR: Egocentric 4D Demonstrations of Human Terrain Traversal](https://arxiv.org/abs/2607.13472)
...
TL;DR: 本文提出了EgoHTR数据集，通过多传感器穿戴设备和便携式3D扫描仪，在复杂非结构化地形中采集了55段场景对齐的4D人体运动序列（超过15万帧），并基于该数据训练感知运动策略，成功部署到Unitree G1人形机器人上。


<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人在非结构化地形中的部署仍然困难，强化学习面临真实世界交互的复杂性，而利用人类先验的方法缺乏场景上下文感知能力。现有数据集无法捕捉挑战性环境中的人-场景序列，限制了运动合成的发展。

Method: 开发了一个开源重建管线，使用第一人称穿戴设备（头戴式+手持式传感器）和便携式3D扫描仪，在多样复杂地形中采集场景对齐的4D人体运动数据。数据集包含55段序列、超过15万帧，并提供了运动捕捉真值用于评估。基于该数据训练感知运动策略（perceptive locomotion policies），在Unitree G1机器人上部署。

Result: 数据集在运动捕捉真值评估中达到了最先进的精度。基于该数据训练的感知运动策略成功在Unitree G1人形机器人上实现了硬件部署，能够穿越非结构化地形。

Conclusion: EgoHTR数据集和重建管线填补了人形机器人学习中人类先验与场景重建之间的空白，为构建上下文感知的、能够在复杂地形中可靠行走的机器人提供了基础。开源管线支持社区扩展数据集。

Classification reason: 该论文构建了面向复杂地形的人体运动数据集，并用于训练具身机器人的感知运动策略（perceptive locomotion policies），最终在Unitree G1人形机器人上部署。这直接属于"具身世界模型与规划控制"（embodied_world_model）这一规范子主题。

Importance reason: Focuses on humanoid locomotion in unstructured terrain, which is tangentially related to world models but lacks direct connection to the user's priority subtopics like continuous language models or video generation. Signals: Egocentric 4D dataset, Humanoid robot deployment, Unstructured terrain, Perceptive locomotion policy, Scene-aligned motion capture
...
### [9] [UESF-Bench: Benchmarking and Probing for Unified Embodied Seeking and Following](https://arxiv.org/abs/2607.13621)
...
TL;DR: 提出 UESF-Bench，一个统一的大规模具身智能体寻找与跟随基准，要求智能体先通过语义引导找到目标人物，再在动态环境中持续跟随。同时提出 SeekFollow-VLA 视觉-语言-动作框架，通过任务驱动的路由机制实现寻找与跟随阶段的隐式推理与转换，在单人和多人环境中均优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有语言引导的人体跟随基准通常假设目标人物在初始时刻可见，忽略了智能体需要先搜索再跟随的现实需求。虽然已有工作开始研究人体搜索，但通常将搜索和跟随作为分离任务，缺乏统一的评测基准。

Method: 提出 UESF-Bench 基准，包含大规模多样化场景。同时提出 SeekFollow-VLA 框架，采用视觉-语言-动作架构，通过任务驱动的路由机制进行潜在阶段推理和寻找/跟随之间的状态转换建模。

Result: SeekFollow-VLA 在单人和多人环境中均显著优于单头（single-head）和双头（dual-head）基线方法，验证了统一框架的有效性。

Conclusion: UESF-Bench 填补了统一具身寻找与跟随评测的空白，SeekFollow-VLA 为这一任务提供了有效的基线方案，未来可进一步扩展到更复杂的动态场景。

Classification reason: 该论文研究具身智能体在动态环境中寻找并跟随语言描述目标的任务，属于具身智能、动作规划与控制的范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Embodied agent benchmark for human seeking/following is not aligned with user's priority on continuous multimodal models or video generation. Signals: Embodied AI benchmark, Human following, Vision-language-action, Task routing, Not in priority subtopics
...
### [10] [Topology-Agnostic Mesh Reconstruction of Deformable Objects from Sparse Touch](https://arxiv.org/abs/2607.13479)
*Everest Yang*
...
TL;DR: 本文提出一种拓扑无关的网格重建方法，仅用少量触觉点（无视觉）即可重建可变形物体（1D绳、2D布、3D软体）的完整网格。使用置换不变的交叉注意力架构，重建误差比非学习方法和高斯过程基线降低约三分之二。进一步利用深度集成不确定性指导主动触觉采样，在稀疏预算下优于随机触摸和主动GP基线。


<details>
  <summary>Details</summary>
Motivation: 在黑暗、遮挡等视觉不可用场景下，触觉是感知可变形物体形状的自然传感器，但触觉点稀疏且局部。现有方法通常针对特定拓扑（如绳、布、软体）分别设计，缺乏统一的拓扑无关估计器。本文旨在解决这一挑战。

Method: 提出一个置换不变的交叉注意力架构，将稀疏触觉点作为输入，直接回归可变形物体的完整网格顶点。使用深度集成（deep ensemble）估计预测不确定性，并基于不确定性指导主动触觉采样策略，选择信息量最大的位置进行下一次触摸。

Result: 在1D绳、2D布、3D软体三种拓扑上，该方法重建误差比非学习几何补全和GP表面基线降低约三分之二；优于简单全局池化集合编码器，且随触觉点增多优势扩大。主动采样策略在稀疏预算下优于随机触摸和GP主动基线，在自遮挡严重时增益更显著。

Conclusion: 本文证明了单一拓扑无关的架构可以从稀疏触觉有效重建可变形物体的完整形状，且不确定性驱动的主动采样能进一步提升重建质量。当视觉可用时，主动触觉采样的收益不大，这验证了该方法的视觉不可用场景定位。

Classification reason: 该论文研究从稀疏触觉估计可变形物体的完整网格形状，涉及机器人操作中的触觉感知与形状重建，属于具身智能中环境感知与建模的范畴，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: The paper focuses on robotic touch-based mesh reconstruction, which is not a priority subtopic and lacks direct connection to the user's core interests in continuous multimodal models, video/image generation, or efficient inference. Signals: sparse touch reconstruction, deformable objects, cross-attention architecture, deep ensemble uncertainty, active touch sampling
...
### [11] [Agile perceptive multi-skill locomotion for quadrupedal robots in the wild](https://arxiv.org/abs/2607.13579)
...
TL;DR: 本文提出APT-RL框架，利用轨迹优化生成大规模2D运动数据集，训练多样化可复用的四足机器人运动技能，并通过Transformer-based强化学习实现单一策略在复杂室内外环境中的多技能自主切换与高速感知运动，峰值速度达6m/s。


<details>
  <summary>Details</summary>
Motivation: 四足机器人在复杂地形（如户外崎岖环境、城市景观）中需要无缝集成多种运动技能、平滑步态切换以及仅依赖机载传感器的高速感知运动，现有方法难以同时满足这些需求。

Method: 提出APT-RL框架：1) 通过轨迹优化与简化动力学生成大规模、特征丰富的2D运动数据集；2) 利用这些数据集训练多样化、可复用的运动技能，作为下游任务的强先验；3) 将技能扩展到3D环境，通过Transformer-based强化学习实现单一策略的多技能自主切换与高速感知运动。

Result: 在真实四足机器人上验证：能够穿越楼梯、障碍物、踏脚石、间隙、倒下的树枝等多种障碍；在动态下落动作中瞬时峰值速度达6m/s；仅依赖机载感知与计算即可在复杂室内外环境中实现敏捷运动。

Conclusion: APT-RL框架通过大规模2D运动数据集预训练多样化技能，结合Transformer强化学习，有效实现了四足机器人在复杂环境中的多技能敏捷运动，展示了单一策略在多种地形上的鲁棒性与通用性。

Classification reason: 该论文提出APT-RL框架，实现四足机器人在复杂地形上的多技能运动、步态自主切换与高速感知运动，属于具身智能中机器人运动规划与控制范畴，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Strong robotics locomotion paper but outside user's core focus on continuous multimodal models and video generation. Signals: quadrupedal robot, multi-skill locomotion, transformer-based RL, trajectory optimization, onboard perception
...
### [12] [From Language to Navigation Goals: A Vision-Language Approach for Semantic Navigation of Mobile Robots Using RGB-D Perception](https://arxiv.org/abs/2607.13624)
...
TL;DR: 本文提出一个基于ROS 2的语言驱动导航框架，使移动机器人能够通过自然语言指令（如"go to the mail box"）理解用户意图，利用RGB-D感知识别目标并估计其位置，最终通过Nav2导航栈自主导航至目标位置。在TurtleBot3 Waffle和Unitree Go2机器人上进行了仿真与真实场景验证。


<details>
  <summary>Details</summary>
Motivation: 自然语言交互为非专业用户提供了与机器人平台通信的直观方式，但将用户请求转化为可执行的导航动作仍是一个挑战，需要整合语言理解、环境感知和自主导航。

Method: 提出模块化ROS 2框架，包含语言理解模块（解析自然语言指令中的目标）、语义感知模块（利用RGB-D数据识别和定位目标）、导航规划模块（生成导航目标并通过Nav2栈执行），各模块通过ROS 2话题和服务通信，支持跨平台移植。

Result: 在仿真和真实场景中，使用TurtleBot3 Waffle和Unitree Go2机器人进行实验，框架成功解释直接命令和上下文请求，生成有意义的自然语言反馈，并导航至目标位置。

Conclusion: 该工作证明了将语义感知与自主导航相结合以实现直观人机交互范式的可行性，且ROS 2实现便于在不同机器人平台上部署。

Classification reason: 该论文提出语言驱动的移动机器人导航框架，将自然语言指令转化为导航动作，涉及环境感知、目标定位与自主导航，属于具身智能中机器人规划与控制的范畴，因此映射到[具身世界模型与规划控制]。

Importance reason: Combines vision-language navigation with mobile robotics but lacks novelty in continuous multimodal or world model directions. Signals: ROS 2 navigation framework, semantic goal detection, RGB-D perception, TurtleBot3 and Unitree Go2 validation, open-source code planned
...
### [13] [A Hybrid Mamba for Audio-Visual Navigation](https://arxiv.org/abs/2607.13110)
*Yi Wang, Yinfeng Yu*

Main category: eess.SP, cs.AI, cs.LG
...
TL;DR: 本文提出Samba，一种基于Mamba的混合架构用于音频-视觉导航任务。通过Mamba状态编码器（M-SE）替代传统GRU进行时序聚合，并构建音频Mamba编码器（AME）改进频谱图全局时频依赖建模。在Matterport3D和Replica数据集上，导航成功率分别提升11.3%和更显著，展现了更强的具身表征能力和更低的计算成本。


<details>
  <summary>Details</summary>
Motivation: 自2020年以来，音频-视觉导航的基础骨干网络（CNN+RNN范式）已超过五年未有本质变化，难以高效表征动态多模态序列。现有方法在捕捉全局时频依赖和长程时序依赖方面存在局限，亟需现代化的架构重构。

Method: 提出Samba架构，包含两个核心组件：(1) Mamba状态编码器（M-SE），利用Mamba的自适应选择机制替代传统GRU进行时序聚合，更高效地建模长程依赖；(2) 音频Mamba编码器（AME），弥补卷积算子在捕捉频谱图全局时频依赖方面的不足，提升音频特征提取能力。

Result: 在Matterport3D数据集上，导航成功率（SR）相比现有最优模型提升11.3%；在场景结构更精细的Replica数据集上性能提升更为显著。模型展现出对未见声源和未见场景的优异泛化能力，且计算成本更低。

Conclusion: Samba通过Mamba架构对传统音频-视觉导航骨干网络进行现代化重构，在更低计算成本下实现了更强的具身表征能力和泛化性能，为音频-视觉导航领域的范式演进提供了高鲁棒性的技术路径。

Classification reason: 该论文研究音频-视觉导航（Audio-Visual Navigation），属于具身智能中的感知与导航任务，与[具身世界模型与规划控制]方向高度匹配。论文使用Mamba状态编码器替代GRU进行时序聚合，改进具身导航中的多模态序列建模能力，属于具身世界模型范畴。

Importance reason: Focuses on audio-visual navigation with Mamba, not aligned with user's priority subtopics like continuous language models or video generation. Signals: Mamba hybrid architecture, audio-visual navigation, state encoder, embodied representation, low computational cost
...
### [14] [Industrial Dexterity Benchmark: A Hardware-Software Benchmarking Platform for Industrial Dexterous Manipulation](https://arxiv.org/abs/2607.14021)
...
TL;DR: 本文提出工业灵巧操作基准（IDB）平台，包含模拟数据中心线缆管理、汽车线束和变速箱装配任务的测试板；同时提出DAG-ROS可扩展模仿学习框架和AG-iDP3多模态扩散策略框架。在数据中心线缆操作任务上，使用多视图RGB图像+R3M编码器的扩散策略达到78%的抓取与插入成功率，显著优于单摄像头RGB基线（36%）。


<details>
  <summary>Details</summary>
Motivation: 灵巧操作仍是工业自动化的关键瓶颈，线缆布线、连接器插入和精密装配等任务仍高度依赖人工。现有机器人方法在鲁棒性、泛化性和部署效率上不足，需要向端到端多模态模仿学习范式转变。

Method: 提出了三个贡献：(1) IDB基准板，模拟数据中心线缆管理、汽车线束和变速箱装配任务；(2) DAG-ROS可扩展模仿学习框架；(3) AG-iDP3多模态扩散策略框架，融合RGB图像、点云、关节位置和腕部力/力矩数据。在数据中心线缆操作板上，使用多视图RGB图像经R3M编码器输入扩散策略进行端到端学习。

Result: 最佳配置（多视图RGB + R3M编码器的扩散策略）在抓取与插入组合任务上达到78%成功率，相比单摄像头RGB扩散策略基线（36%）有显著提升。每个配置仅需约100次遥操作演示。

Conclusion: 正确的学习策略在鲁棒性、泛化性和部署效率上可超越经典视觉与控制机器人方法，证明了向可扩展机器人自动化方向转变的合理性，适用于高可用性工业环境。

Classification reason: 该论文提出面向工业灵巧操作的基准平台和模仿学习框架，涉及机器人操作策略学习（RGB图像、点云、关节位置等多模态融合的扩散策略），属于具身智能/机器人领域的动作规划与控制，因此映射到 world_model 方向下的 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Focuses on industrial dexterous manipulation benchmarks and imitation learning, not directly aligned with user's priority subtopics on continuous language/multimodal models or video generation. Signals: Industrial robotics benchmark, multimodal diffusion policy, cable manipulation, imitation learning, not core to user's continuous language or video generation focus
...
### [15] [Exploratory, Communicative, and Deployable: Vision-Driven Embodied Agents for Open-World Mobile Manipulation](https://arxiv.org/abs/2607.13653)
...
TL;DR: 本文提出 REAL，一个面向开放世界移动操作的具身智能体框架，包含 sim-to-real 一致的环境 API、模拟用户交互、分层训练流程（SFT + RL）以及 REAL-Bench 评测基准。在物理双机械臂机器人上实现了 78.3% 的端到端成功率，展示了零样本迁移到未见家庭场景的能力。


<details>
  <summary>Details</summary>
Motivation: 现有具身智能框架通常依赖特权模拟器状态或假设完整指令，忽略了真实部署中的主动探索、视觉定位和交互式意图消歧等挑战。作者旨在弥合 sim-to-real 差距，构建一个可在开放世界中部署的移动操作智能体。

Method: REAL 框架包含：(1) sim-to-real 一致的环境 API，无需 oracle 感知；(2) 模拟用户实现人在回路交互；(3) 多样化任务组合驱动数据收集；(4) 监督微调 (SFT) 与在线强化学习 (RL) 的分层训练流程；(5) REAL-Bench 评测基准（241 个任务，涵盖主动探索、视觉干扰、铰接操作和交互式消歧）。

Result: 在交互式任务上以 56.9% 的成功率超越领先的商业闭源 VLM；在物理双机械臂机器人上，60 个真实世界 episode 中达到 78.3% 的端到端成功率，展现出对未见家庭场景的鲁棒零样本迁移能力。

Conclusion: REAL 框架通过 sim-to-real 一致的设计和分层训练流程，成功弥合了长时域移动操作中的现实差距，验证了从仿真到真实世界的零样本迁移可行性，为开放世界具身智能体的部署提供了有效方案。

Classification reason: 该论文提出 REAL 框架，面向开放世界移动操作任务，涉及具身智能体在真实环境中的探索、视觉定位、交互式意图消歧与 sim-to-real 迁移，属于具身世界模型与规划控制方向，因此映射到 [CANONICAL] subtopic "embodied_world_model"。

Importance reason: Strong world model and embodied agent work with sim-to-real transfer, but lacks direct focus on priority subtopics like continuous language models or video generation. Signals: world model, embodied agent, mobile manipulation, sim-to-real, reinforcement learning
...
### [16] [DAGR: State-Conditioned Goal Representations via Difference-Aware Goal Cross-Attention](https://arxiv.org/abs/2607.13731)
...
TL;DR: DAGR 提出一种状态条件化的目标表征方法，通过多尺度门控交叉注意力机制，将任意晚期融合编码器的静态目标嵌入细化为状态感知的动态表征，在导航任务上取得改进。


<details>
  <summary>Details</summary>
Motivation: 现有目标条件强化学习中的目标编码器（对比式、度量式、时序距离式、信息论式）均独立于当前状态编码目标，导致策略需要从两个编码器中反向推断哪些部分仍需行动。作者希望让目标表征感知当前状态，从而直接标记目标中仍需执行的部分。

Method: 提出 DAGR（Difference-Aware Goal Representations），包含两个核心组件：(1) 近恒等门控残差（near-identity gated residual），保留基础表征；(2) 差异感知目标交叉注意力（Difference-Aware Goal Cross-Attention），利用逐 token 的状态-目标差异图偏置注意力分数。该方法可应用于任意晚期融合编码器，将静态嵌入细化为状态条件化的动态表征。

Result: 在 OGBench 导航任务上 DAGR 取得改进；消融实验表明增益主要来自门控残差而非差异偏置；在操作和拼图任务上表现持平或低于基线。DAGR 是一种结构化改进，并非通用提升。

Conclusion: DAGR 通过状态条件化的目标表征在导航任务上有效，但并非在所有任务上通用。门控残差是主要贡献组件，差异感知偏置的作用有限。未来工作可探索更有效的状态-目标交互方式。

Classification reason: 该论文研究目标条件强化学习中的目标表征方法，属于具身智能/机器人领域的规划与控制问题，与 [CANONICAL] 具身世界模型与规划控制 方向高度匹配。

Importance reason: Focuses on goal-conditioned RL for navigation, not aligned with priority subtopics like continuous language models or video generation. Signals: state-conditioned goal representations, difference-aware cross-attention, navigation improvement, not universal across tasks, no video or language model connection
...
### [17] [Grounded world models in biological organisms and future embodied AI](https://arxiv.org/abs/2607.13560)
*Giovanni Pezzulo, Davide Nuzzi, Marco D'Alessandro, Riccardo Proietti, Roberto Bottini, Paul Cisek*

Main category: q-bio.NC, cs.AI
...
TL;DR: 本文从神经科学和认知科学出发，阐述了生物有机体中"具身世界模型"的五个神经回路示例（导航、可供性感知、主动感知、稳态控制、自我-世界区分），指出当前具身AI缺乏内在动力学、行动驱动学习、自主探索等关键特征，并讨论了如何将生物原则融入未来具身AI以构建社会共享且对齐人类价值观的世界模型。


<details>
  <summary>Details</summary>
Motivation: 当前生成式与具身AI主要依赖大规模多模态数据的被动预测学习，语言规律成为其他模态信息的支架。然而神经科学与认知科学表明，生物智能的组织方式恰恰相反——通过与环境交互获得的具身世界模型才是语义支架，语言附着其上。本文旨在揭示生物具身世界模型的核心机制，并探讨如何将这些原则应用于未来具身AI。

Method: 本文采用综述与概念分析的方法，通过五个神经科学示例（海马体导航系统、顶叶-前运动皮层的可供性编码、主动感知与探索学习、下丘脑稳态控制与情绪、小脑的前向模型与自我-世界区分）来阐述生物具身世界模型的关键特征，并与当前具身AI方法进行对比分析。

Result: 识别出生物具身世界模型的五个关键特征：（1）内在动力学是学习的基础；（2）行动在将内在动力学与外部世界对齐中起核心作用；（3）自主经验与开放式学习优于被动吸收外部数据；（4）早期预测与控制机制支撑高级认知能力（推理、规划、想象、心智理论、沟通）；（5）社会交互训练可构建社会共享且对齐人类规范的世界模型。

Conclusion: 生物具身世界模型为未来具身AI提供了重要设计原则，包括以内在动力学为基础、以行动驱动学习、强调自主探索与开放式学习，以及通过社会交互构建社会共享的世界模型。这些原则有望推动具身AI从被动数据驱动向主动、交互、具身的方向发展。

Classification reason: 论文从神经科学与认知科学视角讨论具身世界模型（grounded world models），强调通过与环境交互获得语义基础，并探讨如何将生物智能原理应用于未来具身AI。这与[具身世界模型与规划控制]（embodied_world_model）这一CANONICAL子主题高度吻合。

Importance reason: Directly addresses world model grounding and intrinsic dynamics, highly relevant to your priority on world models and continuous multimodal reasoning. Signals: world model, embodied AI, intrinsic dynamics, action-driven learning, autonomous exploration
...
### [18] [Learning Physics-Guided Residual Dynamics for Deformable Object Simulation](https://arxiv.org/abs/2607.13451)
...
TL;DR: 提出 Physics-Guided Residual Dynamics (PGRD)，一种混合仿真框架，将可优化的弹簧质点物理模拟器与学习残差校正的神经网络相结合，用于可变形物体的精确动力学预测，并展示了在模型预测控制和交互式仿真中的应用。


<details>
  <summary>Details</summary>
Motivation: 可变形物体的动力学仿真对机器人操作至关重要，但纯物理方法难以捕捉复杂真实世界动力学，纯学习方法则缺乏物理先验和泛化能力。作者希望结合两者的优势，实现更准确、稳定的可变形物体仿真。

Method: 提出 PGRD 框架，包含：(1) 可优化的弹簧质点模拟器作为物理骨干网络，提供基础动力学预测；(2) 基于滑动窗口 Transformer 的神经网络，学习对物理预测的残差校正；(3) 采用速度公式确保仿真稳定性。在模型预测控制中结合生成的目标图像实现语言条件操作规划，并通过 3D 高斯泼溅实现动作条件视频预测的交互式仿真。

Result: 在多种真实世界可变形物体上，PGRD 的预测精度优于纯物理方法和纯学习方法。在操作规划与交互式仿真两个下游任务中均展示了有效性和实用性。

Conclusion: 物理引导的残差动力学框架能够有效结合物理先验与数据驱动学习的优势，为可变形物体仿真提供更准确的预测，并具备在机器人操作规划与交互式仿真中的实际应用潜力。

Classification reason: 该论文提出物理引导残差动力学（PGRD），结合弹簧质点模拟器与神经网络残差校正，用于可变形物体仿真，属于物理世界建模与交互式模拟器范畴，与[可交互与物理世界建模]（interactive_physical_world）高度匹配。

Importance reason: Strong physics-world-modeling contribution but lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: hybrid simulation, deformable object dynamics, physics-guided residual, world model, model predictive control
...
### [19] [The SIGReg Objective as Variational Free Energy: A Theoretical Active-Inference Account of JEPA World Models](https://arxiv.org/abs/2607.13612)
...
TL;DR: 本文从理论上证明，JEPA世界模型的训练目标（预测损失加加权嵌入正则化）在特定条件下等价于Active Inference的变分自由能。作者将四种非对比正则化器（VICReg、LogDet、PairDist、SIGReg）组织为熵估计器层次结构，证明SIGReg消除了先验校准差距，使目标成为精确的信息瓶颈，并保留了惊奇边界。论文还扩展了多步期望自由能、集成 epistemic 价值和学习策略机制，并指出当前JEPA世界模型缺少的一个AI项：状态认知价值。


<details>
  <summary>Details</summary>
Motivation: JEPA架构是潜空间世界模型的主流设计，但其训练目标通常仅凭经验表现来证明，缺乏规范性的理论原则。本文旨在填补这一空白，从Active Inference的变分自由能角度为JEPA提供理论依据。

Method: 作者将四种非对比正则化器（VICReg、LogDet、PairDist、SIGReg）组织为熵估计器层次结构，通过先验校准差距（prior-miscalibration gap）分析其性质。证明在标准常数噪声编码器模型和SIGReg成功执行（各向同性高斯嵌入）条件下，差距消失，目标成为精确的信息瓶颈。使用Lean 4对代数核心进行了机器验证。

Result: VICReg和LogDet是不安全的上界，PairDist是安全的下界，SIGReg消除了差距。SIGReg下的JEPA目标等价于Active Inference变分自由能，惊奇边界得以保留，潜目标成本成为AI实用价值的精确代理。而VICReg留下不可约的二阶各向异性项。论文还识别出当前JEPA世界模型缺少的状态认知价值项。

Conclusion: SIGReg正则化器使JEPA世界模型在理论上与Active Inference框架一致，为潜空间世界模型的设计提供了规范性原则。预测在性质上有所不同，有待后续实证检验。

Classification reason: 论文从理论角度分析JEPA世界模型的训练目标与Active Inference变分自由能的关系，属于世界模型的理论基础研究，映射到[可交互与物理世界建模]（interactive_physical_world）这一规范子主题，该子主题涵盖物理世界建模与动力学建模的理论工作。

Importance reason: Provides a theoretical foundation for JEPA world models via active inference, directly relevant to your world_model direction and continuous representation interests. Signals: world model theory, active inference, JEPA, variational free energy, SIGReg
...
### [20] [From Pixels to States: Rethinking Interactive World Models as Game Engines](https://arxiv.org/abs/2607.14076)
...
TL;DR: 本文以传统游戏引擎的"动作-状态-观测"循环为组织框架，系统审视了将视频生成模型作为交互式游戏世界引擎的研究进展，从玩家动作控制、游戏状态动力学、状态-观测持久性和实时交互生成四个维度分析现有方法，并发布了《黑神话：悟空》的90小时游戏数据引擎，包含帧对齐的玩家动作、真实游戏状态和视觉观测。


<details>
  <summary>Details</summary>
Motivation: 近期视频生成模型被视为下一代游戏引擎的潜在方案，但实现真正可交互的游戏世界需要满足规则遵循、长程持久性和实时生成等要求。现有工作缺乏对这些关键挑战的系统性梳理，本文旨在提供一个组织框架来审视该领域的研究现状。

Method: 以传统游戏引擎的"动作-状态-观测"循环为分析框架，从四个维度（动作控制、状态动力学、状态-观测持久性、实时交互生成）对现有交互式世界模型方法进行分类和讨论，并构建了《黑神话：悟空》的大规模游戏数据引擎。

Result: 提出了一个系统性的分析框架，将现有方法归类为不同家族并讨论了各自的优势与权衡；发布了包含90+小时游戏数据的数据引擎，提供帧对齐的玩家动作、真实游戏状态和视觉观测及结构化语义标注。

Conclusion: 本文为交互式游戏世界建模提供了清晰的研究全景图，指出了当前方法的局限性和未来方向，并提供了可用于状态感知游戏世界建模的数据资源。

Classification reason: 该论文系统性地审视了将视频生成模型作为交互式游戏世界引擎的方法，围绕玩家动作控制、游戏状态动力学、状态-观测持久性和实时交互生成四个维度展开分析，并提供了游戏状态数据引擎。这属于可交互世界建模的核心范畴，因此映射到 canonical subtopic "可交互与物理世界建模" (interactive_physical_world)。

Importance reason: Directly addresses world model and interactive generation, aligning with top user priorities and boost keywords. Signals: world model, interactive generation, video generation, game engine, state-aware modeling
...
### [21] [Towards Spatial Supersensing in the Wild](https://arxiv.org/abs/2607.13681)
...
TL;DR: 本文提出 VSI-Super-Wild，一个大规模真实世界视频基准，用于评测多模态模型在长时间跨度下的空间超感知能力。基准包含 6,980 个问答对、442 个真实世界视频（最长超4小时），覆盖8类场景。实验发现模型在静态图像理解上表现良好，但在需要持续跟踪世界状态的任务中系统性失败，并归纳出四种失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有空间超感知基准依赖合成长视频（拼接随机短片段）且局限于家庭场景，无法反映真实世界的连续性和多样性。为填补这一空白，作者构建了基于真实世界长视频的大规模评测基准。

Method: 从认知科学出发，系统性地评测世界状态的三个维度：观察者（agent）、物体（objects）和环境（environment/places）。收集442个真实世界视频（8类场景），人工标注6,980个问答对，涵盖长时间跨度（最长4小时+）。对多种多模态模型进行系统评测，并分析性能退化模式。

Result: 模型在静态图像理解任务上表现尚可，但在需要跨时间连贯跟踪世界状态的任务中系统性失败。性能随世界状态复杂度和时间跨度增加而显著下降。归纳出四种失败模式：空间坍缩（spatial collapse）、语义捷径（semantic shortcuts）、更新不足（insufficient update）和实例混淆（instance confusion）。

Conclusion: 当前多模态模型缺乏将物体、观察者和环境绑定为统一空间世界模型的机制，这是实现空间超感知的关键瓶颈。该基准和失败模式分类为未来世界模型研究指明了方向。

Classification reason: 该论文提出了 VSI-Super-Wild 基准，用于评估多模态模型在长时间跨度、真实世界场景中的空间超感知能力（spatial supersensing），属于世界模型评测。world_model 方向下已有 [CANONICAL] generation_benchmark 子主题（生成模型评测与数据集），该子主题明确包含"新数据集"和"评测基准"，且 world_model 方向下已有该子主题的实例（papers: 7），因此直接复用。

Importance reason: Strong benchmark for world model evaluation but lacks direct focus on user's priority subtopics like continuous language models or generation. Signals: world model benchmark, long video spatial reasoning, failure mode taxonomy, real-world diversity, no generation or continuous token focus
...
### [22] [GPOcc++: Unified Sparse Gaussian Occupancy Prediction with Visual Geometry Priors](https://arxiv.org/abs/2607.13481)
...
TL;DR: GPOcc++ 提出了一种统一的稀疏高斯占据预测框架，利用视觉几何先验将表面几何线索转化为占据感知的稀疏高斯表示，支持多视图和时序输入，在室内外场景中实现高效且可泛化的3D场景理解。


<details>
  <summary>Details</summary>
Motivation: 3D占据预测需要从视觉观测中恢复完整的体素表示，但视觉几何先验（如深度、法线）本质上是表面中心的，无法直接推理体素内部和自由空间。现有方法难以在遮挡和未观测区域进行有效建模，且缺乏统一处理多视图与时序信息的框架。

Method: 提出GPOcc/GPOcc++框架：1）将视觉几何先验（深度、法线等）转化为占据感知的稀疏高斯表示，每个高斯原语编码局部占据信息；2）构建统一的稀疏高斯占据表示，支持多视图观测和时序序列的联合建模；3）从室内场景扩展到室外场景，实现统一的占据预测框架。

Result: 在室内（ScanNet等）和室外（nuScenes等）基准上，GPOcc++在多视图和时序设置下均取得一致的领先性能，同时具有较高的效率和良好的泛化能力。

Conclusion: GPOcc++通过稀疏高斯表示有效弥合了视觉几何先验（表面中心）与占据预测（体素内部与自由空间）之间的差距，统一的框架能够同时处理多视图和时序信息，为3D场景理解提供了一种高效且可泛化的解决方案。

Classification reason: 该论文关注3D占据预测（3D occupancy prediction），利用视觉几何先验构建稀疏高斯占据表示，用于室内外场景的3D空间理解，属于世界模型方向中的空间3D/4D世界建模范畴，因此映射到[CANONICAL] subtopic "空间 3D/4D 世界模型"。

Importance reason: Strong in visual geometry priors for 3D occupancy but lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: 3D occupancy prediction, sparse Gaussian representation, visual geometry priors, multi-view and temporal fusion, indoor and outdoor generalization
...
### [23] [M$^\text{4}$World: A Multi-view Multimodal Driving World Model for Interactive Object Manipulation and Minute-long Streaming](https://arxiv.org/abs/2607.14005)
...
TL;DR: M^4World 是一个多视角多模态驾驶世界模型，能够生成环绕视图视频流和同步 LiDAR 扫描，支持交互式物体操控和分钟级稳定长时序生成，通过多阶段训练框架实现仅4步去噪的在线因果生成，并引入 VLM 自动评估管线来评测可控性。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶世界模型在物体级可控性和长时序稳定性方面存在局限，无法支持细粒度的交互式物体操控和分钟级的长时序生成，限制了其在可扩展自动驾驶仿真中的应用。

Method: 提出 M^4World，包含：(1) 灵活的条件接口，支持对单个物体的空间布局和视觉外观进行显式控制；(2) 多阶段训练框架，实现仅4步去噪的在线因果生成，保持长时序滚动中的世界动态一致性；(3) 高效少片段后训练和视觉参考条件生成模型，支持长尾场景定制；(4) 基于 VLM 的自动评判管线，评估场景级条件遵循、视角级物体可控性和跨视角物体一致性。

Result: 实验表明 M^4World 在生成质量、精确可控性和分钟级长时序流稳定性方面均表现优异，在下游长尾数据增强和场景编辑任务中展示了潜力。

Conclusion: M^4World 通过多视角多模态生成、交互式物体操控和稳定长时序生成，为可控、可扩展的自动驾驶仿真提供了有效方案。

Classification reason: 该论文提出多视角多模态驾驶世界模型 M^4World，支持交互式物体操控和分钟级长时序视频流生成，属于视频生成式世界模型，与 [CANONICAL] 视频世界模型 (video_world_model) 高度匹配。同时涉及多视角视频生成，也匹配 multimodal_generation 方向。

Importance reason: Strongly aligns with world model and video generation priorities, with interactive object manipulation and long-horizon stability. Signals: world model, video generation, interactive object manipulation, minute-long streaming, multi-view multimodal
...
### [24] [From Surface Forecasting to Observability Forecasting: A Latent World Model for Cloud-Aware EO Monitoring](https://arxiv.org/abs/2607.13651)
*Mohanad Albughdadi*
...
TL;DR: 本文提出将LeWorldModel（联合嵌入预测架构的世界模型）应用于地球观测中的云感知可观测性预测问题。模型利用多光谱影像和气象协变量，预测下一帧是否可用以及何时可恢复可用视图。在EarthNet2021上的实验表明，该模型在可观测性预测任务上显著优于持久性基线，并在多数场景下优于LightGBM基线。


<details>
  <summary>Details</summary>
Motivation: 地球观测处理链的瓶颈不在于新影像的到达，而在于影像到达时地表是否实际可见（被云遮挡）。现有方法缺乏对"可观测性"的预测能力，无法提前判断何时能获取有效的地表观测数据。

Method: 采用LeWorldModel架构，一种联合嵌入预测（joint-embedding predictive architecture）世界模型。将原始minicubes转换为HDF5序列，包含5个图像通道（蓝、绿、红、近红外、云掩码）和8个气象与日历协变量。模型共18.0M可训练参数，在23,904个训练episode上从头训练。采用锁定协议评估：线性探针仅在训练集上拟合，校准在验证集上确定，然后冻结评估。

Result: 在完整冻结束可观测性基准上，LeWorldModel一致优于持久性基线。下一步可用性预测的平衡准确率为0.769-0.887（持久性为0.493-0.556）。精确首次可用视界预测准确率为0.602-0.806（持久性为0.120-0.369）。与LightGBM相比，在连续晴/云回归和精确恢复时机预测上更优，但在简单二分类任务上LightGBM更强。

Conclusion: LeWorldModel能够有效预测地球观测中的云遮挡可观测性，为卫星任务规划和数据采集提供前瞻性指导。该方法在多数评估场景下优于传统基线，展示了世界模型在环境遥感领域的应用潜力。

Classification reason: 该论文将LeWorldModel（一种联合嵌入预测架构的世界模型）应用于地球观测序列的云感知可观测性预测，属于视频/时序世界模型对未来状态进行预测的范畴，与[视频世界模型]子主题高度匹配。

Importance reason: Applies world model to cloud-aware EO, but domain is remote sensing, which is a negative keyword, and lacks direct connection to priority subtopics like continuous language models or video generation. Signals: world model application, Earth Observation, cloud-aware forecasting, LeWorldModel adaptation, remote sensing domain
...
### [25] [COLMAR: Cooperative View Policy Learning for Multi-Agent Active 3D Reconstruction](https://arxiv.org/abs/2607.13524)
...
TL;DR: COLMAR提出了一种基于共享策略优化的多智能体主动3D重建框架，通过重建感知目标函数和稠密反馈信号，结合3DGS实现高效协作式视角选择与高质量重建。


<details>
  <summary>Details</summary>
Motivation: 多智能体主动3D重建中存在协调效率低下的问题，如冗余观测和空间聚集，导致在有限感知预算下重建质量下降。现有方法缺乏有效的协作策略来优化视角分配。

Method: 将视角分配建模为基于地图中心观测的共享策略优化问题，提出重建感知目标函数（包含重叠感知覆盖、团队级探索和碰撞安全探索），利用增量重建更新提供稠密反馈信号，使用参数共享PPO训练策略，部署时基于融合团队地图进行独立动作选择，无需智能体间消息传递。

Result: 在GLEAM和Replica数据集上，相比启发式和非协作基线，在相同感知预算下重建精度提升高达54%，覆盖度提升49%。

Conclusion: COLMAR通过协作式视角策略学习有效解决了多智能体主动3D重建中的协调问题，验证了重建感知目标函数和稠密反馈机制的有效性，为多机器人协同场景重建提供了新范式。

Classification reason: 论文研究多智能体主动3D重建，使用3DGS进行高保真重建，属于3D生成与重建方向，匹配[CANONICAL] subtopic "3D 生成与重建"。

Importance reason: Focuses on multi-agent 3D reconstruction, which is not a priority subtopic and lacks direct connection to user's core interests in continuous language/multimodal models or video/image generation. Signals: Multi-agent active 3D reconstruction, 3D Gaussian Splatting, PPO policy optimization, Not aligned with priority subtopics, No boost keywords matched
...
TL;DR: DiffGI 提出了一种可微分的几何图像（Geometry Image）框架，用连续 2D TSDF 替代离散二值占用图，结合可微 Marching Squares 算法，实现端到端的 3D 表面表示与优化。基于此训练的 DiffGI-VAE 可将复杂 3D 表面压缩到 32×32 潜空间，并配合基于 Transformer 的潜扩散模型（flow matching）实现条件 3D 生成，在服装和物体数据集上取得高保真重建。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 生成模型多依赖隐式体素表示，强制要求水密拓扑，难以表示薄壳和非流形几何（如服装）。基于几何图像的方法虽以表面为中心，但依赖离散二值占用图，其分辨率相关的边界编码导致阶梯伪影和下采样信息丢失，且表面重建作为不可微的后处理步骤与学习流程脱节。

Method: 1) 用连续 2D TSDF 替代离散二值占用图，在固定网格分辨率下以亚像素精度编码边界位置，消除阶梯伪影。2) 提出基于解析线性插值的可微 Marching Squares 算法，使 3D 表面损失的梯度可反向传播到 2D 潜空间。3) 训练 DiffGI-VAE，结合几何感知法线渲染损失，将复杂 3D 表面压缩到 32×32 超紧凑潜空间。4) 在该潜空间上实例化基于 Transformer 的潜扩散模型，使用 flow matching 目标进行条件 3D 生成。

Result: 在服装和物体数据集上，DiffGI 在重建保真度和边界精度上优于先前的几何图像和体素方法，同时计算资源需求显著更低。

Conclusion: DiffGI 通过可微几何图像框架实现了端到端的 3D 表面表示与生成，有效解决了薄壳几何的表示难题，为高质量 3D 生成提供了新的表面中心范式。

Classification reason: 该论文提出 DiffGI，一种基于可微几何图像的薄壳 3D 生成方法，属于 3D 物体/网格生成范畴，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Strong technical contribution in differentiable 3D surface representation and flow-matching generation, but tangential to user's core focus on continuous language/video models. Signals: Differentiable geometry image, continuous TSDF, flow matching for 3D, thin-shell generation, high-fidelity reconstruction
...
### [27] [TreeSRNF: Square-Root Normal Fields for Generative Modelling of the Geometric and Structural Variability in Tree-like 3D Objects](https://arxiv.org/abs/2607.13456)
*Tahmina Khanam, Hamid Laga, Mohammed Bennamoun, Guanjin Wang, Ferdous Sohel, Farid Boussaid, Anuj Srivastava*

Main category: cs.CG, cs.GR, cs.CV
...
TL;DR: 本文提出TreeSRNF框架，将平方根法向场（SRNF）表示推广到树状3D物体，在黎曼树形空间中对树木的3D几何与分支结构进行统计建模和生成，支持树状3D物体的均值计算、模式分析和新样本合成。


<details>
  <summary>Details</summary>
Motivation: 现有树状3D物体建模方法要么只考虑骨架结构，要么用分支厚度近似3D几何，无法同时精确建模分支的3D几何形状及其连接方式。本文旨在提出一个统一的数学框架，能够同时捕捉树状物体的几何变形和结构变异性。

Method: 将原本用于亏格0曲面的平方根法向场（SRNF）表示推广到树状3D物体；将树状3D物体视为黎曼树形空间中的点，定义新的黎曼度量来度量表面弯曲、拉伸和结构变化；开发了点级和分支级对应关系以及测地路径计算算法；利用这些构建块计算统计摘要（均值和变化模式），并通过从概率分布采样合成新树状物体。

Result: 在真实和合成的植物及树木数据集上，该方法在树状3D物体的统计建模和生成方面显著优于现有方法。

Conclusion: TreeSRNF框架为树状3D物体的几何与结构变异性分析及生成提供了有效的数学工具，在植物学、计算机图形学等领域具有应用潜力。

Classification reason: 该论文提出了一种对树状3D物体（如植物、树木）进行几何与结构变异性生成建模的数学框架，属于3D物体生成范畴，与[CANONICAL] subtopic "3D 生成与重建"高度匹配。

Importance reason: Focuses on 3D tree geometry and branching structure, which is outside the user's priority subtopics and negative keywords. Signals: 3D tree modeling, Square Root Normal Fields, Riemannian shape space, generative modeling, botanical trees
...
### [28] [HIVE-3D: Hierarchical Voxel Enhancement for High-Quality 3D Scene Generation](https://arxiv.org/abs/2607.13468)
...
TL;DR: HIVE-3D 提出一种基于分层体素增强的框架，从单张场景图像生成高质量3D场景。方法包括：粗场景初始化、图像分割与注意力检索对齐2D与3D组件、构建分层组件树、以及体素超分辨率模型，通过粗到细的分层超分辨率逐步提升每个组件的分辨率，最终生成高分辨率高质量3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有单图像3D生成方法在物体级别表现良好，但受限于表示分辨率，难以直接应用于复杂3D场景生成。本文旨在解决从单张图像生成高质量、高分辨率3D场景的挑战。

Method: 1) 从单张场景图像生成粗初始场景；2) 利用图像分割和基于注意力的检索对齐2D图像组件与3D场景组件；3) 将场景关系组织为分层组件树，靠近叶节点的节点表示更细粒度的组件；4) 提出体素超分辨率模型，对目标实例生成精细体素并与粗体素保持强一致性；5) 对每个组件进行粗到细的分层超分辨率处理，逐步提升图像和体素分辨率。

Result: 大量实验表明，该方法显著优于先前方法，达到了最先进的性能，能够生成高分辨率、高质量的3D场景。

Conclusion: HIVE-3D 通过分层体素增强框架有效克服了现有方法在3D场景生成中的分辨率限制，实现了从单张图像到高质量3D场景的生成，为3D场景生成提供了新的有效范式。

Classification reason: 该论文提出基于单张图像的高质量3D场景生成方法，属于3D生成方向，与[CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Novel hierarchical voxel enhancement for 3D scene generation, but lacks direct connection to user's priority subtopics like continuous language models or autoregressive video generation. Signals: 3D scene generation from single image, hierarchical component tree, voxel super-resolution, coarse-to-fine framework, state-of-the-art performance
...
### [29] [ThinkBLOX: 3D Indoor Scene Generation with Progressive Reasoning](https://arxiv.org/abs/2607.13539)
...
TL;DR: ThinkBLOX 提出了一种基于 VLM 的渐进式推理框架，通过逐步推理-行动过程生成和编辑 3D 室内场景，并构建了包含 224K 带 CoT 标注的数据集，结合 SFT 和分层强化学习（Tier-Decoupled GDPO）来优化物理合理性、语义一致性和可交互编辑性。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 驱动的 3D 场景生成多采用一次性规划范式，导致交互编辑时需要全局重优化或完全重建，且容易产生物理或语义上不合理的布局。作者希望实现可逐步推理、可交互编辑的 3D 场景生成。

Method: 提出 ThinkBLOX 框架，将布局生成建模为状态条件的逐步推理-行动过程。构建 ThinkBLOX-Data-200K 数据集（224K 放置对，含多视图上下文、CoT 推理和 JSON 布局）。通过 SFT 微调 VLM 学习推理-行动桥接。提出 Tier-Decoupled GDPO 强化学习方案，将物理有效性、语义合理性和推理-行动一致性等异构奖励分层组织，稳定策略优化。

Result: 在物理合理性、语义对齐和交互可编辑性上显著优于近期一次性生成和迭代基线方法。支持全局/局部生成和场景重排等多种应用。

Conclusion: 渐进式推理框架能有效提升 3D 室内场景生成的质量和可编辑性，分层强化学习方案解决了多目标优化中的奖励冲突问题。

Classification reason: 该论文研究3D室内场景生成，属于多模态生成方向下的[3D生成与重建]规范子主题。同时，论文使用VLM进行逐步推理和布局生成，涉及多模态理解与生成方向，因此也匹配该方向。

Importance reason: Strong 3D generation method with progressive reasoning, but only moderate alignment with user's priority on continuous multimodal models and autoregressive video/image generation. Signals: 3D scene generation, progressive reasoning, VLM-based, chain-of-thought, reinforcement learning
...
### [30] [Nexus: Native Mesh Generation with Diffusion](https://arxiv.org/abs/2607.13563)
...
TL;DR: Nexus 提出了一种基于扩散模型的三角网格生成方法，通过解耦顶点生成（八叉树粗到细扩散）和拓扑生成（Spacetime Interval 连续嵌入扩散），克服了自回归网格序列化方法的误差累积问题，在 Objaverse 和 Toys4K 数据集上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有三角网格生成方法主要依赖网格序列化和自回归过程，存在推理效率低、对误差累积敏感的问题。需要一种能够整体生成高质量三角网格的方法，避免序列建模的固有限制。

Method: Nexus 采用解耦的两阶段扩散方法：(1) 顶点生成：将网格顶点视为按八叉树组织的稀疏体素，使用扩散模型以粗到细的方式生成顶点；(2) 拓扑生成：提出 Spacetime Interval（时空区间），作为 Spacetime Distance 的扩展，将任意边和面拓扑编码为连续的逐顶点嵌入，然后使用扩散模型在生成的顶点上生成这些连续嵌入，从而全局高效地恢复复杂拓扑。

Result: 在 Objaverse 和 Toys4K 数据集以及野外图像上的实验表明，Nexus 优于最先进的自回归和两阶段基线方法，有效规避了序列网格建模的固有限制。来自 3D 从业者的盲测用户研究证实了对其结果的强烈感知偏好。

Conclusion: Nexus 通过解耦顶点和拓扑生成的扩散方法，实现了高质量三角网格的整体生成，克服了自回归方法的误差累积问题，为 3D 网格生成提供了新的有效范式。

Classification reason: 该论文提出 Nexus，一种基于扩散模型的三角网格生成方法，属于 3D 物体/场景生成范畴，与 [CANONICAL] 子主题 "3D 生成与重建" 高度匹配。

Importance reason: Novel diffusion-based mesh generation method, but outside user's priority subtopics and negative for 3D generation. Signals: diffusion mesh generation, decoupled vertex and topology, octree coarse-to-fine, Spacetime Interval embedding, outperforms autoregressive baselines
...
### [31] [UniPhysGen: Unified Physical Grounding for Simulation-Ready 3D Assets](https://arxiv.org/abs/2607.13586)
...
TL;DR: UniPhysGen 提出一个统一框架，自动将原始3D资产转化为具备关节语义和物理属性的仿真就绪资产，并构建了 UniPhys-40K 数据集和 UniPhys-Bench 基准，在物理接地任务上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有3D资产大多缺乏统一的物理语义（关节语义和内在物理属性），无法直接用于具身AI和机器人仿真中的真实交互。现有方法要么独立处理这些语义，要么依赖规范化物体结构，对异构3D资产的鲁棒性有限。

Method: 提出 UniPhys 框架自动将原始3D资产转化为具备统一物理语义的仿真就绪资产，构建 UniPhys-40K 大规模数据集和 UniPhys-Bench 基准。进一步提出 UniPhysGen 统一物理接地模型，联合推理关节语义和内在物理属性，并引入几何鲁棒的关节接地模块以缓解异构部件分解下的几何捷径偏差。

Result: 在关节接地和内在物理属性估计任务上达到 SOTA 性能，生成的资产可直接部署于机器人仿真环境进行真实物理交互。

Conclusion: UniPhysGen 有效解决了异构3D资产的统一物理语义标注问题，为具身AI和机器人仿真提供了可扩展的解决方案，填补了物理接地3D资产自动生成的空白。

Classification reason: 该论文关注将原始3D资产自动转化为具备统一物理语义（关节语义与内在物理属性）的仿真就绪3D资产，属于3D资产生成与重建范畴，与[CANONICAL] subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Focuses on 3D asset physical grounding for simulation, which is not a priority subtopic and lacks direct connection to user's core interests in continuous multimodal models or video/image generation. Signals: 3D asset generation, physical grounding, simulation-ready, articulation semantics, robotic simulation
...
### [32] [TCAM-Diff: Triplane-Aware Cross-Attention Medical Diffusion Model](https://arxiv.org/abs/2607.13812)
...
Importance: 值得关注 (67.79) - 未入选精读

TL;DR: TCAM-Diff 提出了一种基于三平面表示的解码器-自编码器与交叉注意力扩散模型相结合的 3D 医学图像生成方法，在 BrainTumour、Pancreas、Colon 三个不同尺度的数据集上取得了优于现有方法的重建与生成效果。


<details>
  <summary>Details</summary>
Motivation: 高分辨率 3D 医学图像生成面临巨大的内存开销，现有编码器-解码器方法在有限潜空间中难以同时保证重建质量和生成质量。作者希望设计一种内存高效的 3D 生成方法，既能处理高分辨率体数据，又能保持优秀的生成效果。

Method: 提出 TCAM-Diff，包含两个阶段：(1) 使用仅解码器的自编码器（decoder-only autoencoder）从密集体数据中学习三平面（triplane）表示，并利用泛化操作防止过拟合；(2) 采用三平面感知的交叉注意力扩散模型（triplane-aware cross-attention diffusion model）来学习和整合三平面特征。生成的特征可通过预训练解码器模块快速转换为 3D 体数据。

Result: 在 BrainTumour (128³)、Pancreas (256³)、Colon (512³) 三个数据集上，使用 MSE 和 SSIM 评估重建质量，使用 W-GAN critic 评估生成质量。与具有相似潜空间大小的编码器-解码器方法相比，TCAM-Diff 在重建和生成方面均取得了更优的结果。

Conclusion: TCAM-Diff 通过三平面表示和交叉注意力扩散模型，有效降低了 3D 医学图像生成的内存需求，同时保持了高质量的重建与生成性能，为高分辨率 3D 医学数据生成提供了一种高效可行的方案。

Classification reason: 该论文提出 TCAM-Diff，一种基于三平面表示和交叉注意力扩散模型的 3D 医学图像生成方法，属于 3D 场景/数据生成范畴，因此映射到 [CANONICAL] subtopic "3D 生成与重建"。

Importance reason: Focuses on medical image generation, which is a negative keyword area, and does not align with priority subtopics like continuous language models or video generation. Signals: 3D medical image generation, triplane representation, decoder-only autoencoder, cross-attention diffusion, not aligned with user's priority subtopics or boost keywords
...
TL;DR: LPM是一个基于扩散模型的工业级生成式视频修复框架，通过大规模数据工程、增强架构、渐进式训练和时序金字塔推理，实现对任意长度视频的高保真、时序一致修复。已在快手生产环境部署，处理视频占总观看时长的45%，在同等感知质量下可降低20%码率，节省数亿带宽成本。


<details>
  <summary>Details</summary>
Motivation: 用户生成内容（UGC）平台上的视频存在多种复杂退化（模糊、噪声、压缩伪影等），现有视频修复方法难以在工业规模下同时保证高保真度、时序一致性和低成本推理。

Method: LPM采用扩散模型框架，包含：(1) 大规模数据工程，构建覆盖广泛退化的训练数据；(2) 增强的模型架构；(3) 渐进式训练策略；(4) 时序金字塔推理机制，支持任意长度视频的高效修复。

Result: LPM已在快手生产环境部署，处理视频占总观看时长的约45%；在同等感知质量下，相比快手内部编码器可降低20%码率，年节省带宽成本数亿；低服务成本使其可集成到Kling等产品中。

Conclusion: 生成式视频修复可以在工业规模下实现实用、可扩展且成本效益高的部署，为大规模视频处理提供了可行方案。

Classification reason: 该论文提出大规模扩散生成式视频修复框架LPM，属于视频修复/增强任务，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）中的视频编辑、inpainting等方向高度相关，因此复用该分类。

Importance reason: Strong industrial deployment and bandwidth savings, but focuses on video restoration rather than the user's priority subtopics like autoregressive generation or continuous multimodal models. Signals: Industrial-scale deployment, diffusion-based video restoration, temporal-pyramid inference, 45% viewing time coverage, 20% bitrate reduction
...
### [34] [Cyclone: Diffusion Model for Cycle-Consistent Weather Editing from Unpaired Driving Data](https://arxiv.org/abs/2607.13927)
...
TL;DR: Cyclone 是一个基于潜扩散模型的统一天气编辑框架，利用循环一致性约束和图像-文本模型知识，无需配对数据即可实现多种天气条件的编辑（如添加/去除雨、雾、雪等），并支持蒸馏为视频扩散模型以保持时序一致性，在多个下游驾驶感知任务上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在多样化天气条件下的可靠感知仍是一大挑战。现有方法依赖合成数据增强或基于物理的任务特定模型，需要配对训练数据，且难以生成逼真的天气效果或泛化到域外场景。因此，需要一种无需配对数据、能生成真实且结构保持的天气编辑的统一框架。

Method: Cyclone 基于潜扩散模型，引入循环一致性约束（cycle-consistent constraints）和图像-文本模型（如 CLIP）的知识，实现无需配对数据的天气编辑。框架支持双向转换：从正常天气到恶劣天气（如添加雨雾）和从恶劣天气到正常天气（如去雨去雾）。此外，可将图像级模型蒸馏为视频扩散模型，实现时序一致的天气视频编辑。

Result: 实验表明，Cyclone 生成的天气编辑结果比现有基线更真实、结构保持更好，并在多个下游驾驶感知任务（如目标检测、语义分割）上带来一致的性能提升。视频版本也展示了良好的时序一致性。

Conclusion: Cyclone 提供了一种无需配对数据的统一天气编辑方案，兼具逼真性和结构保持能力，能有效提升自动驾驶感知系统在多样化天气下的鲁棒性，且可扩展到时序一致的视频编辑。

Classification reason: 该论文提出基于潜扩散的天气编辑框架 Cyclone，实现不同天气条件下的图像/视频编辑（如添加雨雾、去雨去雾等），属于图像与视频编辑范畴，因此映射到 [CANONICAL] subtopic "visual_editing"（图像与视频编辑）。

Importance reason: Focuses on weather editing for autonomous driving, which is outside the user's priority subtopics and negative keywords. Signals: diffusion model, weather editing, cycle-consistent, unpaired data, autonomous driving
...
### [35] [MultiAnimate: A Unified Framework for Controllable Multi-Character Animation](https://arxiv.org/abs/2607.13415)
...
TL;DR: MultiAnimate 是一个统一框架，支持从多张参考图像同时驱动多个角色在共享环境中进行动画生成，通过身份特定参考网络、身份感知姿态编码器和交互引导模块，保持角色身份一致性和空间关系。


<details>
  <summary>Details</summary>
Motivation: 现有角色动画方法大多局限于单参考图像的单角色动画，无法处理多角色交互场景。本文旨在填补这一空白，实现多角色在共享环境中的并发动画生成。

Method: 提出 MultiAnimate 框架，包含三个核心模块：(1) 身份特定参考网络（Identity-specific Reference Net），从多张参考图像中提取各自的外观特征；(2) 身份感知姿态编码器（Identity-aware Pose Encoder），通过注意力机制将不同姿态序列与对应角色绑定；(3) 交互引导模块（Interaction Guider），利用角色特定的掩码信息增强复杂交互场景的生成质量。

Result: 大量实验和消融分析表明，MultiAnimate 在多角色动画任务上表现优越，尤其在涉及复杂运动序列的场景中效果显著。

Conclusion: MultiAnimate 成功实现了多参考图像驱动的多角色并发动画生成，在保持身份一致性和空间关系方面优于现有方法，为角色动画领域提供了更广泛的应用可能性。

Classification reason: 该论文研究多角色图像动画生成，属于数字人与动作生成（avatar_motion_generation）这一[CANONICAL]子主题，该子主题涵盖人体动作、肖像动画等方向，与本文的多角色动画高度匹配。

Importance reason: Novel multi-character animation framework, but outside user's priority subtopics and negative for video generation. Signals: multi-character animation, identity consistency, spatial relationships, interaction guider, pose encoder
...
### [36] [T3HG-Editor: Text-driven 3D Human Garment Editing with Body Priors Embedded in SMPL-X](https://arxiv.org/abs/2607.13654)
...
TL;DR: T3HG-Editor 提出了一种基于 SMPL-X 人体先验的文本驱动 3D 人体服装编辑方法，通过沿 SMPL-X 法线播种高斯、多视图 token 聚合保持服装一致性、以及 SDF 溢出剪枝，实现了高保真且服装一致的编辑效果。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 高斯编辑方法扩展到人体服装编辑时存在低保真度、扭曲和服装不一致的问题，而 SMPL-X 模型蕴含丰富的人体先验信息，有望解决这些问题。

Method: 三阶段方法：(1) 可编辑高斯获取——沿 SMPL-X 法线播种高斯生成近表面高斯，结合 2D 掩码定位目标高斯；(2) 服装一致性编辑——跨视图聚合对应同一 SMPL-X 顶点的 token 并传播回原视图，无需额外训练；(3) 高斯更新与溢出剪枝——利用 SMPL-X 上的 SDF 构建人体距离场，结合 2D 语义掩码剪枝溢出高斯，防止污染非目标区域。

Result: 在多个不同人体和服装类型上的实验表明，T3HG-Editor 在编辑质量和服装一致性方面均优于现有最先进方法。

Conclusion: 利用 SMPL-X 的几何和关节先验可以有效提升 3D 人体服装编辑的保真度和一致性，为虚拟人编辑提供了新的有效范式。

Classification reason: 该论文研究文本驱动的3D人体服装编辑，属于虚拟人/数字人相关生成任务，与[CANONICAL] subtopic "数字人与动作生成"（avatar_motion_generation）高度匹配。

Importance reason: Focuses on 3D garment editing, which is not a priority subtopic and lacks strong alignment with user's configured direction weights and boost keywords. Signals: 3D human garment editing, SMPL-X body prior, text-driven editing, multi-view token aggregation, SDF overflow pruning
...
### [37] [Music-to-Dance Generation via Atomic Movements](https://arxiv.org/abs/2607.13978)
...
TL;DR: 本文提出一种结构感知的音乐到舞蹈生成框架，将舞蹈建模为原子动作序列（语义可解释的运动事件），通过两阶段生成（原子动作规划 + 过渡感知动作补全）实现结构连贯、节奏对齐且可控的舞蹈生成。


<details>
  <summary>Details</summary>
Motivation: 现有音乐驱动舞蹈生成方法将运动建模为连续信号，忽略了舞蹈的组合结构特性，导致生成的动作结构不连贯且难以控制。受人类编舞过程的启发，作者希望引入原子动作作为舞蹈的基本构建块，实现结构化的舞蹈生成。

Method: 首先对大规模舞蹈数据进行分割并聚类为原子动作组，利用大语言模型对聚类进行语义重标注和精炼，构建可解释且可复用的原子动作词汇表。然后设计两阶段生成框架：1）原子动作规划阶段，模型根据输入音乐预测原子动作的类型、持续时间和时序，形成符号化的舞蹈编排；2）过渡感知补全阶段，基于规划的结构生成平滑且风格一致的运动序列。

Result: 实验表明，该方法在结构连贯性、节奏对齐和感知自然度方面显著优于现有基线方法，同时通过显式的结构表示提供了更好的可解释性和可控编辑能力。

Conclusion: 将舞蹈建模为原子动作序列的结构化方法能够有效提升生成舞蹈的结构连贯性和可控性，为音乐到舞蹈生成提供了一种新的范式。

Classification reason: 该论文研究音乐到舞蹈动作生成，属于人体动作生成范畴，与 [CANONICAL] 子主题 "数字人与动作生成" (avatar_motion_generation) 高度匹配。

Importance reason: Novel atomic movement framework for dance generation, but outside user's priority subtopics and negative keywords not triggered. Signals: music-to-dance generation, atomic movement, structure-aware, two-stage generation, controllable editing
...
### [38] [Text2Sign: A Single-GPU Diffusion Baseline for Text-to-Sign Language Video Generation](https://arxiv.org/abs/2607.13164)
*Ruize Xia*
...
TL;DR: Text2Sign 是一个在单张 L4 GPU 上运行的文本到手语视频扩散基线模型，采用冻结的视觉语言文本编码器与 3D 编解码器结合分解式时空注意力，在 How2Sign 数据集上以极低资源实现了短片段手语视频生成。


<details>
  <summary>Details</summary>
Motivation: 手语是聋哑人群体的主要交流方式，但文本到手语视频生成的研究因视频扩散模型训练和评估成本高昂而进展缓慢。现有方法通常需要大量计算资源，本文旨在探索单 GPU 条件下的低成本基线方案。

Method: Text2Sign 使用冻结的视觉语言文本编码器（SigLIP）提取文本特征，结合 3D encoder-decoder 架构和分解式时空注意力（factorized spatiotemporal attention）来降低全视频注意力的计算成本。对比了卷积与 Transformer 两种骨干网络、冻结预训练与任务特定文本编码器、分解式与全注意力机制。采用 DDIM 8 步采样，guidance scale 为 5.0。

Result: 最佳短运行消融实验验证损失为 0.0648，长运行检查点达到 0.00999。在紧凑评估切片上，SSIM 为 0.2403±0.0238，PSNR 为 15.11±0.42 dB，时序一致性为 1.0000±0.0000。生成 32 帧 64×64 视频耗时 12.60 秒（2.54 fps），峰值推理内存 3.12 GB。消融实验显示冻结文本条件可改善验证损失，但提示词特异性分离有限。

Conclusion: Text2Sign 证明了在单 GPU 上训练文本到手语视频扩散模型的可行性，为低资源手语生成研究提供了基线。但受限于低分辨率、短视频片段且缺乏专家语言学评估，尚不能作为完整的手语生产系统使用。

Classification reason: 该论文提出 Text2Sign，一个文本到手语视频生成的扩散模型，属于文本到视频生成任务。虽然面向手语这一特定领域，但核心方法是视频扩散模型（3D encoder-decoder + factorized spatiotemporal attention），与 canonical subtopic "文生视频与视频扩散" 高度匹配。

Importance reason: Low relevance to priority subtopics like continuous language models or autoregressive video generation, and negative keywords apply. Signals: single-GPU baseline, sign language video, weak prompt sensitivity, low resolution, no expert evaluation
...
### [39] [Efficient Text-to-Audio Generation via Pruning](https://arxiv.org/abs/2607.13330)
...
TL;DR: 本文对基于U-Net的文生音频扩散模型AudioLDM应用滤波器剪枝（filter pruning），在保持生成质量的前提下，最多可减少83%的参数和39%的MACs计算量，并通过轻量微调恢复剪枝导致的特定音效生成能力下降。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在文生音频任务中取得了高质量生成效果，但U-Net去噪骨干网络的计算开销巨大，限制了实际部署。现有工作较少关注音频扩散模型的效率优化，本文旨在通过模型剪枝降低AudioLDM的计算成本。

Method: 对AudioLDM的U-Net卷积块进行参数冗余分析，采用基于范数（norm-based）准则的滤波器剪枝策略，移除重要性较低的滤波器，随后进行轻量微调以恢复剪枝带来的性能损失。

Result: 实验表明，剪枝可减少U-Net最多83%的参数和39%的MACs，同时生成质量与基线持平甚至略有提升。剪枝会影响某些音效（如枪声、警笛、爆炸等安全关键声音以及机械声等）的生成，但轻量微调可基本恢复这些能力。

Conclusion: 模型剪枝是提升文生音频扩散模型推理效率的有效手段，能够在大幅压缩模型的同时保持生成质量。剪枝对特定音效的影响可通过微调恢复，为音频生成模型的实用化部署提供了可行方案。

Classification reason: 该论文对AudioLDM文生音频扩散模型应用模型剪枝（pruning）以提升计算效率，属于生成模型加速与高效化的典型工作，因此映射到[CANONICAL] subtopic "efficient_generation"（生成模型加速与高效化）。

Importance reason: Focuses on pruning for audio generation, which is outside the user's priority subtopics and lacks direct connection to continuous multimodal models or video generation. Signals: AudioLDM pruning, filter pruning for U-Net, 83% parameter reduction, safety-critical sound degradation, not in priority subtopics
...
### [40] [Heavy-Tailed Flow Matching via Random Clocks](https://arxiv.org/abs/2607.13841)
...
TL;DR: 提出 Heavy-Tailed Flow Matching via Random Clocks (HTFM)，通过随机时钟将重尾分布建模为高斯尺度混合，使 flow matching 能够高效生成重尾数据（如长尾图像、金融数据、极端天气），同时保持少步采样优势并提供尾部可控接口。


<details>
  <summary>Details</summary>
Motivation: 标准扩散模型和 flow matching 从高斯噪声出发，对重尾数据（如类别不平衡图像、金融回报、极端天气）的建模能力不足，导致尾部覆盖差、稀有事件生成质量低。需要一种既能保持 flow matching 高效采样优势，又能处理重尾分布的方法。

Method: 将重尾源分布建模为随机时钟条件高斯分布的混合：给定时钟路径，源分布和流是高斯分布；对时钟边缘化后得到高斯尺度混合（覆盖高斯、α-稳定、Student-t 族）。使用截断 logsignature 特征编码路径值时钟，使速度场适应条件空间，计算开销可忽略。

Result: 在 2D 不平衡 α-稳定混合、CIFAR10-LT（长尾分类数据集）和 HRRR 天气场上，HTFM 在模式覆盖、样本质量和尾部统计恢复上优于高斯 flow matching 及其他重尾基线方法，同时保持 flow matching 的低 NFE（少步采样）优势。

Conclusion: 随机时钟公式提供了一种实用的尾部控制接口：仅改变时钟律或尾部参数，同一架构即可在不同分布族间校准生成尾部的"重尾程度"，为处理重尾数据的生成任务提供了灵活高效的框架。

Classification reason: 该论文提出 Heavy-Tailed Flow Matching via Random Clocks (HTFM)，属于 flow matching 生成模型的改进方法，关注重尾数据生成。flow matching 是扩散/生成模型的一种高效范式（少步采样），与 efficient_generation 中的"少步采样"、"加速"方向高度吻合。同时论文也涉及生成模型对齐（tail-control interface），但核心贡献是 flow matching 框架的改进，因此归入 efficient_generation。

Importance reason: Proposes a novel flow matching framework for heavy-tailed data, directly enabling efficient generation of long-tail images and extreme events, aligning with user's priority on efficient inference and multimodal generation. Signals: flow matching, heavy-tailed data, efficient generation, random clocks, tail control
...
### [41] [Kaleido: Algorithm-Hardware Co-Design for Video Diffusion Transformers by Exploiting Latent Space Correlations](https://arxiv.org/abs/2607.13770)
...
TL;DR: Kaleido 提出了一种算法-硬件协同设计方法，通过利用潜空间中的通道级时空相关性来加速视频扩散Transformer（vDiT）。算法层面设计了轻量级通道重用机制跳过冗余计算，硬件层面设计了类似脉动阵列的加速器架构，在三个主流vDiT模型上实现了最高5.9倍加速和16.0倍能效提升。


<details>
  <summary>Details</summary>
Motivation: 视频扩散Transformer（vDiT）生成高质量视频但计算成本极高，主要瓶颈来自长扩散步长和自注意力计算。现有加速方法多继承自大语言模型的稀疏注意力技术，未能充分利用视频数据独特的时空相关性。

Method: 提出通道级时空相关性利用的轻量级通道重用算法，跳过冗余计算并重用部分结果；设计类似脉动阵列的加速器，包含可重构处理单元和轻量级数据调度器，以缓解重用算法带来的不规则稀疏性和数据访问模式问题。

Result: 在三个主流vDiT模型上，Kaleido相比最先进的加速器实现了最高5.9倍加速和16.0倍能效提升，生成质量比现有方法高17dB以上。

Conclusion: 通过算法-硬件协同设计，利用视频数据在潜空间中的通道级时空相关性，可以有效加速vDiT推理，在保持生成质量的同时大幅提升速度和能效。

Classification reason: 该论文提出视频扩散Transformer（vDiT）的算法-硬件协同加速方法，属于生成模型加速与高效化（efficient_generation）这一规范主题。论文核心是加速vDiT推理，与规范主题中的"扩散加速"、"高效采样"等关键词高度吻合。

Importance reason: Directly addresses efficient inference for video diffusion transformers, a key priority, with a novel algorithm-hardware co-design. Signals: video diffusion transformer acceleration, algorithm-hardware co-design, latent space correlation exploitation, high speedup and energy savings, efficient inference priority
...
### [42] [Improving Medical Image Generative Models with Fréchet Distance Loss](https://arxiv.org/abs/2607.13300)
...
Importance: 普通相关 (60.68) - 未入选精读

TL;DR: 本文提出 Fréchet Distance loss（FD-loss）作为扩散生成模型的微调正则化项，通过对齐真实与生成图像在预训练编码器空间中的一阶和二阶特征统计量，改善医学图像中异质性肿瘤的生成质量，使下游分割任务的 DSC 提升超过 5%。


<details>
  <summary>Details</summary>
Motivation: 标准扩散模型的逐像素去噪目标会平滑高方差的不规则肿瘤结构，导致生成的医学图像难以捕捉异质性肿瘤的复杂形态特征，限制了其在分割等下游临床任务中的效用。

Method: 提出 Fréchet Distance loss（FD-loss），在预训练编码器空间中对齐真实图像与生成图像的一阶（均值）和二阶（协方差）特征统计量。将该损失作为正则项对预训练的扩散生成模型进行微调，在多个肝脏和脑肿瘤数据集（CT/MRI）上使用自然图像和医学图像编码器进行实验。

Result: 使用 FD-loss 正则化的合成数据训练下游分割网络，肿瘤 Dice 相似系数（DSC）相比未正则化的合成数据增强提升超过 5%。定性分析表明，FD-loss 有助于更逼真的肿瘤合成并减少分割幻觉。

Conclusion: FD-loss 是一种有效的医学图像生成模型正则化方法，能够提升合成数据的临床可用性，改善下游任务性能。

Classification reason: 该论文提出用 Fréchet Distance loss（FD-loss）微调医学图像扩散生成模型，通过对齐真实与生成图像的特征统计量来改善生成质量。这属于生成模型的对齐/正则化/奖励优化范畴，与 [CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Focuses on medical image generation with a specific FD-loss, which is outside the user's priority subtopics and has negative keyword overlap. Signals: Medical image generation, Fréchet Distance loss, diffusion model finetuning, tumor segmentation, negative keyword "medical diagnosis"
...
### [43] [SARFA: Segment Anything with Radiomic Feature Alignment](https://arxiv.org/abs/2607.13323)
...
TL;DR: SARFA 提出一种基于放射组学特征对齐的 SAM 医学图像分割框架，通过概率提示生成多样化候选掩码，并利用 Fréchet Radiomic Distance (FRD) 和 DPO 进行偏好优化，在 CT 和 MRI 基准上超越现有模糊分割方法。


<details>
  <summary>Details</summary>
Motivation: SAM 在模糊目标（如肿瘤边界）分割时表现不佳，而医学图像中精确的边界勾画至关重要且专家间常存在分歧，现有方法仅依赖像素级重叠度量不足以捕捉临床相关的解剖和纹理特征。

Method: SARFA 通过概率提示生成一组多样化的候选掩码，然后使用基于 Fréchet Radiomic Distance (FRD) 的放射组学特征距离作为训练目标，结合 Direct Preference Optimization (DPO) 对候选掩码进行偏好排序与优化，使分割结果的解剖和纹理特征与临床真实标注对齐。

Result: 在 CT 和 MRI 医学图像分割基准上，SARFA 在模糊分割任务中优于现有的模糊分割方法，验证了放射组学特征对齐和 DPO 风格候选掩码排序作为训练目标的有效性。

Conclusion: 放射组学特征对齐能够有效提升 SAM 在医学图像模糊分割场景下的性能，DPO 作为候选掩码排序的训练目标是可行的，为医学图像分割提供了一种新的对齐范式。

Classification reason: 该论文使用 DPO（Direct Preference Optimization）对 SAM 的医学图像分割候选掩码进行偏好排序与对齐，属于生成模型对齐与奖励优化的范畴，因此映射到 [CANONICAL] subtopic "生成模型对齐与奖励"。

Importance reason: Medical segmentation with radiomics is outside your priority subtopics and uses negative keywords. Signals: medical image segmentation, radiomic feature alignment, SAM adaptation, ambiguous segmentation, CT/MRI benchmarks
...
### [44] [Delving into the Temporal Challenges of Unified Video Protection Against Image-to-Video and Fine-Tuning-based Customization](https://arxiv.org/abs/2607.13336)
...
TL;DR: 本文提出TC-UAP，首个同时防御基于参考（文生视频）和基于微调的视频定制化生成的统一保护方法。通过优化跨视频滑动窗口的通用对抗扰动，并引入时序建模与时序攻击损失，实现时序一致且鲁棒的视频级身份保护。


<details>
  <summary>Details</summary>
Motivation: 现有扩散视频生成模型（文生视频、微调定制）可被用于侵犯个人隐私、身份所有权和知识产权。已有的反定制化工作主要针对图像保护，视频场景下同时防御参考型和微调型定制化尚未被探索，且面临帧级扰动被3D VAE时序压缩破坏、单视频扰动无法保护未见视频、时序不一致扰动易被时序攻击破坏三大挑战。

Method: 提出TC-UAP（Temporally Consistent Universal Adversarial Perturbations）：(1) 在多个视频的滑动窗口上优化身份级多帧通用对抗扰动（UAP），考虑视频VAE时序压缩导致的局部时序依赖；(2) 引入内在时序建模（intrinsic temporal modeling）和外在替代时序攻击损失（extrinsic surrogate temporal-attack loss），使扰动时序一致且对未见时序攻击鲁棒；(3) 单个扰动可保护不同长度的未见视频。

Result: 定量和定性实验表明，TC-UAP在参考型（文生视频）和微调型视频定制化下均达到最强的身份保护效果，且对多种未见时序攻击（如时序编辑、帧插值等）保持鲁棒性，优于现有图像级和视频级基线方法。

Conclusion: 本文首次系统研究了视频定制化生成的统一保护问题，提出的TC-UAP通过时序一致的通用对抗扰动有效防御了参考型和微调型两种视频定制化范式，为视频隐私保护和生成模型安全提供了新思路。

Classification reason: 该论文研究针对视频定制化生成（包括文生视频和微调定制）的对抗性保护方法，属于生成模型的安全与可控性范畴。虽然论文本身是防御/攻击方法，但其核心是防止生成模型被滥用进行身份/版权侵犯，与生成模型对齐（alignment）中关于安全、可控、保护的目标高度一致。因此映射到 [CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）。

Importance reason: Focuses on video protection against customization, which is not a priority subtopic and lacks alignment with user's key directions. Signals: Video protection, adversarial perturbation, temporal consistency, identity protection, not in priority subtopics
...
### [45] [Improving Text-to-Audio Instruction Following via Fine-Grained Feedback from Audio-Aware Large Language Models](https://arxiv.org/abs/2607.13408)
*Chun-Yi Kuan, Siwon Kim, Byeonggeun Kim, Suyoun Kim, Bo-Ru Lu, Qinming Tang, Ankur Gandhe, Hung-yi Lee, Chieh-Chi Kao, Chao Wang*

Main category: cs.AI, cs.CL, cs.LG, cs.SD, eess.AS
...
TL;DR: 本文提出一种指令级框架，利用音频感知大语言模型（ALLM）作为细粒度裁判，验证生成音频中目标事件的存在与时序关系，并构建偏好对进行直接偏好优化（DPO），以提升文生音频模型的多事件时序指令跟随能力。同时引入叙事性基准 S3Bench 用于评估多事件时序指令跟随。


<details>
  <summary>Details</summary>
Motivation: 现有文生音频模型能生成高质量音频，但在涉及多个声音事件和时序顺序的指令跟随方面表现不佳。现有评估和训练信号主要关注全局相似性或感知质量，缺乏对指令级正确性的监督。

Method: 使用音频感知大语言模型（ALLM）作为细粒度裁判，验证生成音频中目标事件的存在和时序关系。通过基准测试和人工验证确认 ALLM 判断的有效性后，利用其反馈构建偏好对，进行直接偏好优化（DPO）训练。同时提出 S3Bench 叙事性基准用于评估多事件时序指令跟随。

Result: 实验表明，该方法在现有基准和 S3Bench 上均提升了事件完整性、时序顺序和联合指令跟随准确率，同时保持了音频质量。

Conclusion: 利用音频感知大语言模型作为细粒度裁判提供指令级反馈，结合偏好优化，能有效改善文生音频模型在多事件时序指令跟随方面的表现，为音频生成对齐提供了新思路。

Classification reason: 该论文使用音频感知大语言模型作为细粒度裁判，构建偏好对进行直接偏好优化（DPO），以提升文生音频模型的指令跟随能力。这属于生成模型对齐与奖励（generation_alignment）范畴，具体涉及偏好优化与奖励建模。

Importance reason: Focuses on text-to-audio instruction following, which is outside the user's priority subtopics and negative keywords. Signals: audio-aware LLM, direct preference optimization, temporal instruction following, S3Bench benchmark, not in priority subtopics
...
### [46] [VGIF-Score: Interpretable and Diagnostic Evaluation of Spatio-Temporal Instruction Following in Video Generation](https://arxiv.org/abs/2607.13527)
...
TL;DR: 本文提出 VGIF-Score，一个自动化、可解释的视频生成指令跟随评测框架，包含基于 ST-DAG 的客观完成分支和基于 AutoRubric 的主观满意度分支，并构建了包含 223 条长复杂指令的 VGIF-Bench 基准，在 14 个视频生成模型上验证了其可靠性和诊断能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型评测协议通常使用短且语义浅的提示词，原子约束有限且时空依赖弱，依赖昂贵的人工评估或手工视觉流水线，缺乏对指令约束成功或失败的诊断性洞察。

Method: VGIF-Score 包含两个互补分支：(1) 客观完成分支，将提示词解析为时空有向无环图 (ST-DAG)，执行依赖感知的 QA 与短路诊断；(2) 主观满意度分支，使用指令条件化的 AutoRubric 评估摄影技法、视觉纯净度、运动平滑度和物理一致性。两者结合产生统一评分。

Result: 在 14 个闭源和开源视频生成模型上，对超过 3000 个生成视频进行评测，VGIF-Score 提供了可靠、可解释且具有诊断价值的指令跟随评估结果。

Conclusion: VGIF-Score 为视频生成指令跟随提供了自动化、可解释的评测方案，能够诊断具体约束的成败，有望推动视频生成模型在复杂指令跟随能力上的提升。

Classification reason: 该论文提出视频生成指令跟随评测框架 VGIF-Score 和基准 VGIF-Bench，属于生成模型评测与数据集方向。该论文同时涉及多模态理解与生成（指令跟随评测），但核心贡献是视频生成评测，因此主方向为多模态生成，匹配方向还包括多模态理解与生成。现有 canonical subtopic "generation_benchmark" 完全匹配。

Importance reason: Focuses on video generation evaluation, not directly on generation methods or world models. Signals: video generation evaluation, instruction following, spatio-temporal constraints, diagnostic framework, benchmark creation
...
TL;DR: Human4K是一个大规模4K多视角全身人体重建数据集，包含超过600万张8视角4K图像，配合Vicon动作捕捉系统提供高精度SMPL-X标注，覆盖11名受试者的复杂全身运动，显著提升了全身3D人体重建的质量，尤其在手部、足部和深度模糊的肢体配置上。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体重建数据集缺乏高分辨率图像、高精度标注和多样化全身运动的组合，导致模型在深度模糊、自遮挡等挑战性场景下产生不稳定的几何结构、不准确的肢体关节和不可靠的预测。为此，作者构建了Human4K数据集以填补这一空白。

Method: 使用8视角4K高分辨率相机系统与专业Vicon动作捕捉系统同步采集数据，覆盖11名受试者的复杂全身运动。设计Motion-Retargeting and Refinement Module (MRRM) 对全身和四肢进行精确对齐，生成高精度SMPL-X标注。数据集包含超过600万张4K图像。

Result: 在标准基准上，使用Human4K训练后全身重建性能一致提升，在手部、足部和深度模糊的肢体配置上取得了特别大的改进。

Conclusion: Human4K通过提供高分辨率、高精度、多样化全身运动的大规模数据集，有效解决了现有数据集的关键不足，为3D全身人体重建研究提供了重要资源。

Classification reason: 该论文提出了Human4K，一个大规模4K多视角全身3D人体重建数据集，属于生成模型评测与数据集范畴，因此映射到[CANONICAL] subtopic "生成模型评测与数据集" (generation_benchmark)。

Importance reason: Dataset paper with limited direct relevance to user's priority on continuous multimodal models and autoregressive generation. Signals: Large-scale 4K multi-view dataset, SMPL-X annotations, improves whole-body reconstruction, hand/foot/depth-ambiguous gains, no direct link to continuous language/multimodal or autoregressive generation
...
### [48] [Concurrent Image Understanding and Generation: Self-Correcting Coupled Markov Jump Processes](https://arxiv.org/abs/2607.13188)
...
### [49] [VideoRAE: Taming Video Foundation Models for Generative Modeling via Representation Autoencoders](https://arxiv.org/abs/2607.14088)
...
TL;DR: VideoRAE 提出一种表征自编码器，利用冻结的视频基础模型（V-JEPA 2, VideoMAEv2）的多尺度层次特征，将其压缩为连续或离散的视频潜变量，用于扩散 Transformer 和自回归生成模型，在 UCF-101 上取得 SOTA 结果且收敛速度比传统 3D-VAE 快约 5 倍。


<details>
  <summary>Details</summary>
Motivation: 传统 3D-VAE 主要针对像素级重建优化，其潜变量捕获的语义和时空结构有限。而视频基础模型（如 V-JEPA 2, VideoMAEv2）具有强大的视频理解能力，但其冻结表征能否转化为紧凑、可重建且生成友好的视频潜变量尚未被探索。

Method: VideoRAE 使用冻结的视频基础编码器提取多尺度层次特征，通过轻量级 1D 自注意力投影器压缩为连续潜变量（用于 DiT）或通过多码本高维量化得到离散 token（用于自回归模型）。解码时采用局部-全局表征对齐目标（与冻结的 VFM 教师对齐），提升语义保持能力，无需 KL 正则化训练。

Result: 在 UCF-101 上，VideoRAE 在连续和离散模式下均取得强重建效果。类到视频生成 gFVD 达到 40（AR）和 93（DiT），收敛速度比竞争自编码器基线快约 5 倍。在 2B 规模文生视频研究中，用 VideoRAE 替换 LTX-VAE 可在相当设置下实现更快收敛。

Conclusion: 冻结的视频基础模型表征可以作为通用且生成友好的视频潜变量，VideoRAE 验证了这一思路的有效性，为视频生成提供了一种替代传统 3D-VAE 的新范式。

Classification reason: 该论文提出 VideoRAE，利用冻结的视频基础模型（V-JEPA 2, VideoMAEv2）的多尺度层次特征，通过表征自编码器（RAE）压缩为连续或离散的生成友好视频潜变量。核心贡献是连续视觉 tokenizer/表征自编码器，与 [CANONICAL] subtopic "连续视觉 Tokenizer 与表征自编码器" 高度匹配。同时涉及视频生成（multimodal_generation）和连续潜空间（continuous_language_multimodal）。

Importance reason: Directly advances continuous visual tokenization and representation autoencoders for video generation, a top priority subtopic. Signals: continuous visual token, representation autoencoder, video generation, frozen VFM, faster convergence
...
### [50] [Bring Music The Horizon: Music-Driven 360$^\circ$ Video Generation](https://arxiv.org/abs/2607.13471)
*Kai Hsu Tsai, Yong Wei Fu, Hung I Yang, Yu-Chih Chen*

Main category: eess.IV, cs.CV, cs.MM, cs.SD, eess.AS
...
TL;DR: 本文提出 Bring Music The Horizon，一个情感感知的音乐驱动360°视频生成流水线。先预测音乐每四小节的 valence-arousal 情感轨迹，通过 EmotiCrafter 和 SEGA 框架生成情感感知的视觉引导，再基于关键帧用图像到视频模型合成连续的360°沉浸式视频。


<details>
  <summary>Details</summary>
Motivation: 现有音乐可视化方法要么严重依赖歌词，要么生成平面非沉浸式视频，难以传达音乐的情感动态并提供沉浸式体验。

Method: 1) 以每四小节为单位预测 valence-arousal 情感轨迹；2) 将情感值通过 EmotiCrafter 转换为情感感知视觉引导向量，并用 SEGA 框架进行细粒度语义控制生成关键帧；3) 使用图像到视频模型将关键帧合成为连续的360°视频。

Result: 在多种音乐风格上展示了生成能力，并与 From-Sound-To-Sight 基线进行了定性比较，生成的360°视频能反映输入音乐的情感进展和时间结构。

Conclusion: 本文证明了情感感知的音乐驱动360°视频生成流水线的有效性，能够生成反映音乐情感动态的沉浸式可视化内容。

Classification reason: 该论文研究音乐驱动的360°视频生成，属于音频到视觉的跨模态生成任务，与[CANONICAL] subtopic "音频与多模态联合生成"（audio_av_generation）高度匹配。

Importance reason: The paper focuses on music-driven 360° video generation, which is not aligned with the user's priority subtopics on continuous language/multimodal models, autoregressive generation, or world models. Signals: music-driven generation, 360° video, emotion-aware pipeline, valence-arousal, image-to-video
...
### [51] [Continuously Evolving Deepfake Detection: An Architecture and Public-Benchmark Evaluation of a Dynamic Detection System](https://arxiv.org/abs/2607.13234)
...
TL;DR: 本文提出 BitMind Forensics (BMF)，一种通过开放对抗竞赛持续更新训练分布的深度伪造检测系统。在19个公开数据集上的评测表明，BMF 在真实世界场景下显著优于静态检测器，AUC 提升达45-50%，并在多个基准上达到或超越最佳商业检测器。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测器在学术基准上表现优异，但在真实世界内容上性能大幅下降（AUC 下降45-50%）。作者认为这一差距是结构性的：静态检测器仅在训练时的一次性快照上训练，而生成技术持续演进，导致检测器快速过时。

Method: 提出 BitMind Forensics (BMF)，通过 Bittensor SN34 开放对抗竞赛持续刷新训练分布。系统包含图像、通用视频和人脸视频三个检查点。采用持续学习范式，利用对抗竞赛中不断涌现的新伪造样本更新模型，使检测器始终紧跟生成前沿。

Result: BMF 在 Sumsub 原始图像上达0.936 AUC，完整四条件测试集上0.872 AUC；在 Deepfake-Eval-2024 上图像0.915、视频0.822，远超最佳开源检测器（0.56和0.63）；在21生成器AI图像面板上达0.991 AUC；时间研究表明后续版本在未见生成器数据上持续提升（图像0.842→0.902，视频0.864→0.936）。

Conclusion: 持续演进的训练范式是应对快速发展的深度伪造生成技术的有效策略。BMF 在多个基准上显著优于静态检测器，验证了开放对抗竞赛作为持续数据源的价值。作者公开了评测工具和模型快照以促进可复现研究。

Classification reason: 该论文研究深度伪造检测（deepfake detection），属于多模态安全与攻防（multimodal_safety_attack）范畴下的防御方向。论文提出持续演进的检测系统，并在多个公开基准上评测，与现有 CANONICAL 子主题"多模态安全与攻防"高度匹配。

Importance reason: Strong deepfake detection benchmark but outside user's priority subtopics and negative keywords. Signals: Deepfake detection, dynamic training, adversarial competition, benchmark evaluation, AUC improvement
...
Main category: cs.CL, cs.MM, cs.AI, cs.CV
...
TL;DR: 本文提出Groc-PO，一种基于多阶段接地上下文偏好优化的框架，通过构建包含物体接地、上下文接地和接地推理三阶段偏好样本的数据集GCPD，对MLLM进行细粒度的偏好对齐，有效缓解幻觉和错误传播，提升模型忠实性。


<details>
  <summary>Details</summary>
Motivation: 标准DPO仅在最终答案层面进行偏好优化，无法对早期接地阶段（如物体定位、上下文整合）提供直接监督，导致接地漂移和上下文不一致引发的错误传播难以抑制。

Method: 提出Groc-PO框架，构建GCPD数据集，将多阶段偏好样本组织为物体接地、上下文接地和接地推理三个阶段，对每个阶段引入显式的偏好监督，实现跨阶段错误传播的抑制。

Result: 在幻觉缓解、忠实推理和整体可靠性方面，Groc-PO相比标准DPO及其他强基线方法取得了更优的性能。

Conclusion: 显式的多阶段接地偏好监督能够有效提升MLLM的忠实性和可靠性，为可信多模态推理提供了有价值的思路。

Classification reason: 该论文聚焦于MLLM的幻觉缓解与忠实推理，属于多模态幻觉与可靠性这一[CANONICAL]子主题。

Importance reason: Addresses MLLM hallucination via grounded preference optimization, but is not in top priority subtopics. Signals: multimodal hallucination, preference optimization, grounded context, error propagation, MLLM reliability
...
### [53] [Demystifying On-Policy Distillation: Roles, Pathologies, and Regulations](https://arxiv.org/abs/2607.13399)
...
Importance: 值得关注 (70.89) - 未入选精读

TL;DR: 本文系统研究了 LLM 后训练中 on-policy distillation (OPD) 的角色、病理与调节方法。OPD 本质上是探索催化剂，通过密集 token 级引导帮助学生模型走向正确推理路径，但不会扩展能力上限。研究发现 OPD 的有效性完全依赖于引导信号的质量，并揭示了两种病理：师生不匹配（Student-Teacher Mismatch）和长度利用（Length Exploitation）。作者提出 advantage clipping 和 log-scale compression 两种轻量信号调节方法，在七个基准上稳定超越 OPD 变体和 RLVR 基线。


<details>
  <summary>Details</summary>
Motivation: On-policy distillation (OPD) 已成为 LLM 后训练的关键范式，但其训练动态尚未被充分理解。现有工作缺乏对 OPD 角色、潜在病理及有效调节方法的系统研究，因此作者希望深入剖析 OPD 的工作原理、失败模式及改进策略。

Method: 1) 通过控制实验明确 OPD 的角色：比较 prompt 多样性与每个问题采样数的影响，验证 OPD 作为探索催化剂而非能力扩展器。2) 识别两种病理：Student-Teacher Mismatch（师生分布差距导致引导信号与任务正确性错位）和 Length Exploitation（聚合 token 级目标产生长度依赖捷径）。3) 提出轻量信号调节方法：advantage clipping（裁剪优势值范围）和 log-scale compression（对数尺度压缩），确保探索由忠实信号引导。

Result: 在七个基准上的实验表明，所提出的信号调节方法有效缓解了长度利用问题，实现了稳定的蒸馏效果，在性能上一致超越 OPD 变体和 RLVR 基线。验证了信号质量（而非教师模型规模）是 OPD 成功探索的关键因素。

Conclusion: OPD 的有效性完全依赖于引导信号的质量，而非教师模型的规模。通过 advantage clipping 和 log-scale compression 等轻量信号调节，可以有效抑制师生不匹配和长度利用等病理现象，实现更稳定有效的蒸馏训练。

Classification reason: 该论文研究 LLM 后训练中的 on-policy distillation (OPD)，探讨其训练动态、病理现象（师生不匹配、长度利用）及信号调节方法（advantage clipping、log-scale compression）。这属于多模态/语言模型的对齐与强化学习范畴，与 canonical subtopic "多模态强化学习与对齐"（multimodal_rl_alignment）高度相关，尽管论文聚焦 LLM 而非多模态，但其方法论（RLVR、reward signal regulation）直接对应 RL/对齐方向。

Importance reason: Strong theoretical analysis of on-policy distillation but lacks direct connection to user's priority subtopics like continuous models or video generation. Signals: on-policy distillation, LLM post-training, exploration catalyst, student-teacher mismatch, length exploitation
...
TL;DR: CANON 是一种无标签自蒸馏方法，通过采样多个推理路径并提取多数答案作为共识锚点，让模型冻结快照以该共识答案为条件生成密集 token 级监督信号，从而提升 LLM 推理准确率，在数学和科学推理基准上 pass@1 提升最高达 12 个点，且计算量仅为无标签强化学习的七分之一。


<details>
  <summary>Details</summary>
Motivation: 现有利用多数投票共识进行无标签训练的方法仅将共识用作过滤、偏好或标量奖励，丢弃了多数答案中包含的丰富信息。作者希望将共识转化为更密集的 token 级监督信号，以更高效地提升模型推理能力。

Method: CANON 对每个无标签 prompt 采样多个推理路径，提取多数答案；然后冻结模型快照，以一条能到达该多数答案的推理路径为条件（consensus-anchored teacher），对模型自身的 rollout 在每个 token 位置进行监督训练。

Result: 在数学和科学推理基准上，CANON 将 pass@1 提升最高 12 个点，比无标签强化学习方法高出 6 个点，计算量仅为其七分之一；在池化无标签数据上训练后，可迁移到未见基准，性能接近使用真实标签的训练方法。模型能解决之前 32 次尝试从未解决的问题，多数投票准确率本身也得到提升。

Conclusion: CANON 证明将共识转化为密集 token 级监督是一种高效的无标签推理能力提升方法，其改进不仅仅是分布锐化，而是真正扩展了模型的问题求解能力。

Classification reason: 该论文提出无标签自蒸馏方法 CANON，利用多数投票共识作为密集 token 级监督信号来提升 LLM 推理能力，属于无标签训练/自监督对齐方法，与多模态强化学习与对齐（multimodal_rl_alignment）中的偏好学习、无标签训练范式高度相关。

Importance reason: Novel self-distillation method for LLM reasoning, but outside user's priority subtopics and direction weights. Signals: label-free self-distillation, consensus anchoring, token-level supervision, LLM reasoning improvement, compute efficient
...
### [55] [SIVA-RL: Sensitivity-Invariance Visual Alignment for Multimodal Reinforcement Learning](https://arxiv.org/abs/2607.13931)
...
TL;DR: SIVA-RL 提出了一种基于样本级结果条件监督的视觉对齐框架，通过 token 对齐的 PatchSwap 构造局部干预，利用冻结审计策略的奖励下降作为软路由权重，分别驱动敏感性和不变性对齐，在 9 个多模态推理基准上显著提升 3B/7B VLM 的 RL 训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有 RLVR 方法仅关注答案级正确性，无法保证 VLM 真正基于视觉证据进行推理。已有的视觉干预方法按干预类型（而非实际效果）分配监督信号，但相同操作在不同样本上会产生异质结果，导致对齐效果不佳。

Method: SIVA-RL 框架包含三个关键设计：(1) 通过 token 对齐、距离约束的 PatchSwap 构造局部化图像干预；(2) 使用冻结的审计策略对每对 clean-intervention 样本评分，根据奖励下降幅度进行软路由；(3) 大下降样本驱动敏感性对齐（关注视觉变化），小下降样本驱动不变性对齐（锚定干净图像），模糊样本降权。该框架兼容 GRPO 和 DAPO 两种 RL 骨干。

Result: 在 9 个多模态推理基准（涵盖数学、逻辑和视觉依赖任务）上，SIVA-RL 在 3B 和 7B 模型上全面超越匹配的 RL 基线。在视觉依赖推理任务上提升 8.79 个百分点，在全部 4 种 GRPO/DAPO 配置上平均相对提升达 14.9%。

Conclusion: SIVA-RL 通过将干预构造与监督分配解耦，实现了更细粒度的视觉对齐，有效提升了多模态 RL 训练中 VLM 的视觉 grounding 能力，且与主流 RL 框架兼容，具有较好的通用性。

Classification reason: 该论文提出 SIVA-RL，一种面向多模态强化学习（RLVR）的视觉对齐框架，通过敏感性与不变性对齐来改进 VLM 的视觉 grounding。核心贡献属于多模态 RL 与对齐范畴，与 [CANONICAL] subtopic "多模态强化学习与对齐"（multimodal_rl_alignment）高度匹配。

Importance reason: Strong multimodal RL alignment method with sample-wise supervision, but not directly on priority subtopics like continuous models or video generation. Signals: Multimodal RL alignment, Token-aligned PatchSwap, Outcome-conditioned supervision, 9 benchmark gains, Compatible with GRPO/DAPO
...
### [56] [Towards Enhancing 3D Spatial Reasoning in Medical Multimodal Large Language Models](https://arxiv.org/abs/2607.13860)
...
TL;DR: 本文提出一种基于切片级数据合成范式的结构化推理数据集构建方法，通过模拟放射科医生的诊断流程，将复杂的3D体素阅读过程分解为细粒度的逐切片观察并合成可解释的CoT，从而在无需昂贵3D预训练的情况下显著提升2D预训练MLLM的3D空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在2D医学图像理解上表现优异，但扩展到3D体素成像面临标注成本高、数据集不透明等问题。当前数据格式（刚性VQA对或非结构化临床报告）无法捕捉显式的临床推理过程，限制了模型对3D体素的理解能力。

Method: 提出一种切片级数据合成范式（slice-wise data synthesis paradigm），模拟放射科医生的真实诊断流程：将3D体素阅读分解为逐切片观察，融合全局临床先验，生成包含顺序空间追踪、多切片空间感知（用于伪影缓解）和鉴别排除等临床原则的可解释CoT推理数据。使用该合成数据对标准2D预训练MLLM进行指令微调。

Result: 在多个3D医学基准上的全面评估表明，该方法相比2D基线有显著性能提升，展现出强大的空间推理能力，与资源密集型的原生3D架构性能相当，有效缩小了性能差距。

Conclusion: 这种数据驱动的策略无需昂贵的3D特定预训练即可实现深度体素理解和高度可解释的临床逻辑推理，为医学MLLM的3D空间推理提供了一种高效可行的路径。

Classification reason: 该论文聚焦于医学3D体素成像中的空间推理，通过构建结构化推理数据集并引入Chain-of-Thought（CoT）来增强MLLM的3D空间推理能力，属于多模态推理与思维链（multimodal_reasoning_cot）范畴。

Importance reason: Focuses on medical domain with negative keyword "medical diagnosis", and lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: medical MLLM, 3D spatial reasoning, data synthesis, chain-of-thought, slice-wise paradigm
...
### [57] [Unleashing Multimodal Large Language Models for Training-free HOI Detection in the Wild](https://arxiv.org/abs/2607.13881)
...
TL;DR: AgentHOI 提出了一种无需训练的多智能体框架，利用 MLLM 的通用多模态推理能力进行开放场景下的人-物交互检测（HOI Detection），通过上下文感知的多轮推理和多面交互定位机制，在无需 HOI 训练数据的情况下超越现有监督和弱监督方法。


<details>
  <summary>Details</summary>
Motivation: 传统 HOI 检测方法受限于预定义交互类别的监督范式，难以泛化到开放世界和组合场景。现有基于 MLLM 的方法仅通过提示策略提取判别性表征，未能充分利用 MLLM 固有的多模态推理能力，导致在模糊和开放交互场景中缺乏上下文推理能力。

Method: 提出 AgentHOI 框架，包含两个关键机制：(1) 上下文感知多轮推理（Context-aware Multi-round Reasoning），通过逐步细化交互假设实现穷举和组合式 HOI 发现；(2) 多面交互定位（Multifaceted Interaction Localization），通过生成融合语义、空间和外观线索的实例级描述来增强定位精度。框架以模块化方式协调互补的视觉基础模型进行开放语义推理和空间定位。

Result: 在多个真实场景数据集上的实验表明，AgentHOI 在无需任何 HOI 训练数据的情况下，性能超越了当前最先进的监督和弱监督方法。

Conclusion: AgentHOI 证明了将通用多模态推理能力直接迁移到 HOI 检测的有效性，为开放世界交互理解提供了一种无需训练的新范式。

Classification reason: 该论文利用MLLM的多模态推理能力进行HOI检测（人-物交互检测），核心贡献在于将通用多模态推理能力迁移到开放场景的交互理解中，属于多模态推理范畴。因此映射到[CANONICAL] subtopic "多模态推理与思维链" (multimodal_reasoning_cot)。

Importance reason: Novel training-free HOI detection via MLLM reasoning, but tangential to user's core focus on continuous models and generation. Signals: training-free, multimodal reasoning, human-object interaction, agentic framework, open-world detection
...
### [58] [Kepler-Encoder-v0.1: Towards a Multimodal Embedding Model for Robots](https://arxiv.org/abs/2607.13522)
...
TL;DR: Kepler-Encoder-v0.1 是一个面向机器人的多模态编码器，将视觉、本体感知和力/力矩融合到共享潜空间，通过 LeJEPA/SIGReg 目标进行掩码跨模态预测自监督训练。推理时仅需视觉输入，但潜空间能恢复相机难以捕捉的力/力矩信息，在 RH20T 数据集上验证了有效性，并可作为无效状态检测器（AUROC 0.90）和通过扩散解码器重建图像。


<details>
  <summary>Details</summary>
Motivation: 机器人需要理解自身状态，但相机只能看到部分信息，力和接触在单帧图像中几乎不可见。原始视觉特征对力的预测 R² 低于 0.10。因此需要一种能融合多模态信息（视觉、本体感知、力/力矩）的编码器，使视觉-only 的潜空间也能包含相机难以捕捉的状态信息。

Method: 提出 Kepler-Encoder-v0.1，使用可学习查询的交叉注意力层将视觉、本体感知和力/力矩融合为共享潜空间。采用 LeJEPA/SIGReg 目标进行掩码跨模态预测自监督训练。训练时使用多模态输入，推理时仅需视觉输入。在 RH20T 数据集上训练，覆盖四种机器人。还训练了一个扩散解码器 PixNerd 从潜空间重建图像。

Result: 在 RH20T 数据集上，视觉-only 潜空间对末端执行器状态和力的恢复显著优于原始 frozen-ViT 特征和计算匹配的视觉-only 控制模型；对电机状态（相机可见）与最强视觉基线统计持平。潜空间几何结构跟踪状态变化。跨模态预测误差可作为训练无关的无效状态检测器（AUROC 0.90 对超出范围状态，0.69 对场景交换状态）。扩散解码器可从潜空间重建相机帧。

Conclusion: Kepler-Encoder-v0.1 验证了通过多模态融合训练，视觉-only 潜空间可以编码相机难以直接观测的机器人状态信息（特别是力/力矩），且单个与具身无关的编码器可覆盖多种机器人。单时间步情况已得到验证，下一步是原生速率的时间融合。

Classification reason: 该论文提出一个机器人多模态编码器，将视觉、本体感知和力/力矩融合到共享潜空间中，通过掩码跨模态预测进行自监督训练，最终得到视觉-only的embedding。核心是多模态表征学习与跨模态检索/对齐，属于多模态检索方向下的跨模态表征学习。

Importance reason: Focuses on robot-specific multimodal embedding, not aligned with user's priority subtopics like continuous language models or video generation. Signals: robot multimodal encoder, masked cross-modal prediction, force/torque recovery, vision-only latent, invalid-state monitor
...
### [59] [Fine-grained CLIP fine-tuning with self-annotated region alignment](https://arxiv.org/abs/2607.13661)
...
TL;DR: 提出SFF-CLIP，一种仅使用图像-文本对进行CLIP微调的方法，通过运行时区域-短语对齐机制提升细粒度密集特征表示能力，同时保持全局视觉-语义一致性，无需额外区域标注。


<details>
  <summary>Details</summary>
Motivation: CLIP在预训练时关注全局图像-文本匹配，导致细粒度密集特征表示能力不足。现有微调方法需要额外区域标注，限制了语义多样性且处理成本高，同时往往牺牲CLIP原有的全局视觉表示能力。

Method: 设计运行时区域-短语对齐方案：从输入句子中提取概念短语，利用文本特定的热力图将短语与对应的基于区域的视觉特征进行对齐。仅使用图像-文本对作为输入，无需额外区域标注。

Result: 在细粒度密集特征表示任务上取得显著性能提升，同时在图像级任务上保持了原始CLIP的性能。

Conclusion: SFF-CLIP有效提升了CLIP的细粒度表示能力，同时不牺牲全局视觉-语义一致性，为CLIP微调提供了一种无需额外标注的高效方案。

Classification reason: 该论文通过自标注区域-短语对齐来提升CLIP的细粒度密集特征表示能力，本质上是改进视觉-语言跨模态对齐与检索能力，属于多模态检索方向下的细粒度对齐增强工作。映射到[多模态检索]这一规范子主题。

Importance reason: Improves CLIP fine-grained features without extra annotations, but is not aligned with user's priority subtopics like continuous models or video generation. Signals: CLIP fine-tuning, region-phrase alignment, no extra annotations, fine-grained dense features, global consistency maintained
...
### [60] [Accuracy Without Grounding: Diagnosing Visual Dependency Dissociation in Video LLM Benchmarks](https://arxiv.org/abs/2607.13305)
*Jae Joong Lee*
...
TL;DR: 本文提出 Visual Dependency Gap (VDG) 指标，通过对比视频 LLM 在原始视频与黑屏条件下的正确率差异，系统性地诊断基准测试是否真正衡量了视觉依赖能力。在 20 个模型上的实验表明，准确率与视觉依赖是可分离的，时序顺序对视觉贡献几乎为零，而帧多样性提供了大部分视觉收益。


<details>
  <summary>Details</summary>
Motivation: 当前视频 LLM 基准测试的准确率常被当作视觉理解的证据，但模型可能依赖语言先验或记忆而非真正的视觉理解。作者希望诊断基准测试是否真正衡量了视觉依赖能力，揭示准确率与视觉理解之间的脱节。

Method: 提出 Visual Dependency Gap (VDG) 指标，即每个问题在原始视频与黑屏条件下正确率的差异。使用配对 McNemar 检验评估统计显著性。设计诊断阶梯：黑屏 → 单帧 → 打乱帧 → 原始视频，逐步剥离视觉信息源。还进行了 FPS 消融实验和 H.264 编码实验，分析帧率影响和答案翻转现象。

Result: 在 MVBench 上，模型在原始视频上差异显著 (p=0.0003)，但在黑屏上无显著差异 (p=0.53)。属性感知任务高度依赖视觉，而时序推理接近纯语言基线。帧多样性提供了大部分视觉收益，时序顺序贡献近乎为零。FPS 从 0.5 到 24 的消融排除了稀疏采样的影响。H.264 实验显示聚合准确率稳定但隐藏了双向答案翻转。API 模型的 VDG 值在 0.025 到 0.315 之间。

Conclusion: 准确率与视觉依赖是可分离的，VDG 应作为视频基准测试的标准审计工具，以衡量模型是否真正具备视觉依赖能力。当前许多视频 LLM 基准可能高估了模型的视觉理解，尤其是时序推理任务。

Classification reason: 该论文提出 Visual Dependency Gap (VDG) 方法，系统性地审计视频 LLM 基准测试是否真正衡量了视觉理解能力，属于多模态评测方法论，因此映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: Introduces a diagnostic metric for visual dependency in video LLMs, but focuses on evaluation rather than the user's priority subtopics of generation or continuous models. Signals: Visual Dependency Gap, video LLM benchmark audit, accuracy vs. visual grounding, temporal order negligible, frame diversity key
...
### [61] [GSM-Plus-BN: A Perturbation-Based Benchmark for Bangla Mathematical Reasoning in Large Language Models](https://arxiv.org/abs/2607.13248)
...
TL;DR: 本文提出了GSM-Plus-BN，一个基于GSM-Plus的孟加拉语数学推理扰动评测基准，包含9000个样本（1000种子题+8000扰动变体），评估了6个开源LLM在标准提示和思维链提示下的表现。实验发现GPT-OSS-20B在标准提示下种子题准确率达96.08%，但所有模型在孟加拉语扰动文本上的表现均显著低于英文基准，凸显了低资源语言数学推理的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理评测主要聚焦英语等资源丰富的语言，缺乏对孟加拉语等低资源语言的系统性评估，尤其是缺乏扰动式评测数据集来检验模型是否真正理解数学推理而非依赖模式匹配。

Method: 从英文GSM-Plus基准出发，通过人工翻译和验证构建孟加拉语版本GSM-Plus-BN，包含1000个种子问题和8种扰动类型的8000个变体。在6个开源LLM上使用标准提示和思维链提示两种设置进行评测。

Result: GPT-OSS-20B在标准提示下种子题准确率最高（96.08%）；Llama-3.3-70B和GPT-OSS-120B在扰动变体上鲁棒性更强；CoT提示显著提升了大多数模型的推理能力；但所有模型在孟加拉语上的表现与英文基准相比仍有明显差距。

Conclusion: GSM-Plus-BN为孟加拉语数学推理研究提供了首个扰动式评测基准和基线结果，揭示了当前LLM在低资源语言数学推理方面的不足，为未来研究指明了方向。

Classification reason: 该论文提出了孟加拉语数学推理基准GSM-Plus-BN，属于多模态推理评测范畴（数学推理是语言模型推理能力的重要评测维度），因此映射到[多模态评测与基准]这一规范子主题。

Importance reason: Focuses on Bengali math reasoning, which is outside the user's priority subtopics and negative keywords. Signals: Low-resource language benchmark, Perturbation-based evaluation, No direct connection to continuous models, video/image generation, or world models
...
### [62] [Evaluation Ability Does Not Imply Optimization Utility: LLM-as-a-Judge Signals in Closed-Loop Table Recognition](https://arxiv.org/abs/2607.13347)
*Donghwan Kim*
...
TL;DR: 本文研究LLM-as-a-judge在表格识别闭环再生中的优化效用。实验表明，LLM裁判信号在TEDS评估下表现弱（分数平局多、排名不可复现），仅靠裁判分数无法有效选择候选结果；结构保持指令可减少严重损失但无性能提升。结论：评估能力不等于优化效用，闭环优化需要能确定性检测结构变化的验证信号。


<details>
  <summary>Details</summary>
Motivation: LLM-as-a-judge被广泛用于闭环再生中的反馈和选择信号，但其在优化场景中的有效性尚未得到充分验证。作者希望系统性地检验LLM裁判信号在表格识别这一可控测试场景中的实际效用。

Method: 使用FinTabNet和OmniDocBench两个数据集，以确定性TEDS作为评估指标，构建闭环再生实验。分析LLM裁判信号的分数分布、排名可复现性、选择策略效果，并引入结构保持指令作为对照，通过2x2析因设计分析裁判反馈与结构约束的交互作用。

Result: 1) LLM裁判信号在两个数据集上均表现弱：分数频繁平局、排名不可复现，唯一优于随机的选择策略依赖于最早迭代平局规则，优势不能归因于裁判分数本身。2) 结构保持指令显著降低了FinTabNet上的严重损失率，在OmniDocBench上方向一致。3) 结构保持约束减少了严重损失尾部但未带来性能提升，且保留裁判反馈时保护效果不稳定。

Conclusion: 评估能力不等于优化效用。迭代优化至少需要一个能确定性检测结构变化的验证信号，而非仅依赖LLM裁判分数。该结果不否定LLM作为评估器的价值，但警示其在闭环优化中的直接应用需要更谨慎的设计。

Classification reason: 该论文研究LLM-as-a-judge在表格识别闭环优化中的信号有效性，属于多模态评测方法（评测信号质量、评测协议分析），映射到[多模态评测与基准]这一规范主题。

Importance reason: Focuses on LLM-as-judge in table recognition, not aligned with user's priority on continuous multimodal models or video/image generation. Signals: LLM-as-judge, table recognition, closed-loop regeneration, evaluation vs optimization, TEDS
...
### [63] [DevicesWorld: Benchmarking Cross-Device Agents in Heterogeneous Environments](https://arxiv.org/abs/2607.13465)
...
Main category: cs.AI, cs.MA, cs.SE, cs.HC, cs.CL
...
TL;DR: DevicesWorld 是一个大规模跨设备智能体协作基准，包含 6,140 个任务，覆盖手机、桌面和 IoT 三类设备环境，用于评测 LLM 智能体在异构设备间完成端到端任务的能力。实验表明现有最强系统成功率仅 12.5%，揭示了跨设备信息获取、界面操作和设备混淆等关键挑战。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 智能体评测大多聚焦于单一设备环境（如仅手机、仅桌面或仅智能家居），但真实用户需求往往涉及多设备协作（如手机获取信息、桌面处理、另一设备输出结果）。缺乏一个可执行的、可复现的跨设备协作评测基准来系统评估智能体在异构设备间获取和整合信息的能力。

Method: 构建 DevicesWorld 基准，包含 6,140 个任务，集成三类设备环境（移动端、桌面端、IoT）到统一的跨设备交互与评测框架。每个任务包含自然语言用户目标、参与设备与初始状态、可执行动作、基于规则的验证器和清理流程。采用多阶段构建与质量控制流水线确保任务贴近真实用户需求，并通过设备状态和生成文件自动验证最终结果。

Result: 在固定评测集上评估了五种前沿 LLM 智能体系统，最佳成功率仅 12.5%。失败案例中约 28.7% 满足至少一个评分条件但仍未完成全部任务。轨迹分析显示智能体在信息获取或界面操作时卡住、混淆源设备和输出设备、或在所有条件满足前提前终止。

Conclusion: DevicesWorld 将跨设备协作操作转化为一个可执行、可复现且具有诊断价值的评测问题，为研究可靠的跨设备智能体提供了重要基准。现有方法表现不佳表明跨设备协作仍是 LLM 智能体的重大挑战，需要进一步研究。

Classification reason: 该论文提出了 DevicesWorld，一个跨设备（手机、桌面、IoT）智能体协作的基准评测框架，属于多模态智能体评测范畴。虽然涉及执行环境而非纯视觉理解，但作为 LLM-based agent 在异构设备环境中的可执行评测基准，最匹配 [CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark），该 subtopic 涵盖多模态评测、新基准与评测协议。

Importance reason: Strong benchmark for cross-device agents but outside user's core focus on continuous multimodal models and generation. Signals: cross-device agent benchmark, low success rate 12.5%, heterogeneous environments, not in priority subtopics, no boost keywords matched
...
### [64] [Attention-Free and Lightweight Token Reduction for Efficient Vision-Language Models](https://arxiv.org/abs/2607.13500)
...
TL;DR: 本文提出一种无注意力、轻量级的 token 缩减框架，作为 VLM 的即插即用模块，通过基于熵的重要性估计和变换诱导的一致性信号实现 token 选择，在保持性能的同时显著加速推理。


<details>
  <summary>Details</summary>
Motivation: VLM 在处理大量视觉 token 时计算开销巨大，难以部署在资源受限的边缘设备上。现有 token 缩减方法要么依赖注意力图（与现代加速框架不兼容），要么依赖计算密集的成对相似度比较，可扩展性差。

Method: 提出无注意力、轻量级的 token 缩减框架：1）基于信息论视角，使用熵准则量化 token 信息量，保留表达力强且退化少的 token；2）引入变换诱导的一致性信号，使相似 token 产生相似信号，通过排序和步长选择实现多样化 token 覆盖。

Result: 在多个 VLM 基准上，该方法在激进压缩下仍能保持有竞争力的性能，实现了良好的精度-效率权衡。

Conclusion: 本文提出的无注意力 token 缩减方法有效解决了现有方法依赖注意力图或高计算开销的问题，为 VLM 在边缘设备上的高效部署提供了实用方案。

Classification reason: 该论文提出一种无注意力、轻量级的视觉 token 缩减框架，用于加速 VLM 推理，属于多模态高效化与轻量化（efficient_multimodal）这一 [CANONICAL] 子主题。

Importance reason: Directly addresses efficient inference for VLMs, a priority subtopic, with a novel attention-free token reduction method. Signals: efficient inference, token reduction, vision-language model, attention-free, lightweight
...
TL;DR: GeoAnchor 提出了一种交错文本-潜空间推理框架，将3D空间信息分解为位置、方向和几何三种互补的连续潜变量，通过协作训练实现从局部感知到全局理解的3D空间推理，在复杂3D推理任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM依赖符号文本token进行3D空间推理，但文本token缺乏表示连续几何信息的能力；而仅使用单一类型的连续潜变量又无法适应多样化空间任务，导致复杂几何场景下的对齐失败。

Method: GeoAnchor 将3D空间信息分解为三种互补的连续潜变量：位置潜变量（position latents）用于物体定位、方向潜变量（direction latents）用于关系朝向、几何潜变量（geometry latents）用于场景结构。这些组件在结构化空间中重组以构建局部证据并捕获全局上下文，实现动态可解释推理。同时引入协作训练策略，引导模型从局部空间感知逐步过渡到全面3D理解。

Result: 在多种复杂3D推理任务上的实验表明，GeoAnchor 超越了现有最先进方法，验证了其有效性和泛化能力。

Conclusion: 通过将3D空间信息分解为多种互补的连续潜变量并进行协作训练，可以有效提升MLLM在复杂3D空间理解任务上的表现，为连续潜空间推理在空间理解中的应用提供了有效范式。

Classification reason: 该论文研究从2D图像进行3D空间关系理解与推理，属于多模态理解与生成方向下的[空间与3D多模态推理]（CANONICAL）。同时，论文使用连续latent表示（position/geometry/direction latents）进行推理，也与连续语言模型方向下的[连续多模态推理与潜空间思维]相关。

Importance reason: Directly advances continuous latent reasoning for 3D spatial understanding, a key multimodal reasoning direction aligned with your priority subtopics. Signals: continuous latent decomposition, interleaved text-latent reasoning, 3D spatial understanding, collaborative training strategy, state-of-the-art on complex 3D reasoning
...
### [66] [Boogu-Image-0.1: Boosting Open-Source Unified Multimodal Understanding and Generation](https://arxiv.org/abs/2607.13125)
...
TL;DR: Boogu-Image-0.1 是一个开源统一多模态理解与生成模型家族，包含 Base、Turbo、Edit、Edit-Turbo 四个变体，在文生图、快速推理、指令编辑和双语文本渲染方面表现优异。通过提升模型理解能力、数据质量和训练流程，并结合推理时扩展，在有限算力下达到接近闭源系统的性能，仅使用 2.08 亿张图像，理论训练成本约 40 万美元。


<details>
  <summary>Details</summary>
Motivation: 闭源多模态系统（如 Nano-Banana-Pro 和 GPT-Image-2）通过系统级集成实现强大性能，但其内部实践未公开。作者希望证明在有限算力预算下，通过针对性的模型理解改进、数据质量提升和训练流程优化，结合推理时扩展，可以显著提升生成和编辑性能，推动开源统一多模态理解与生成生态的发展。

Method: Boogu-Image-0.1 基于统一多模态理解与生成架构，包含 Base（标准文生图）、Turbo（快速推理）、Edit（指令编辑）、Edit-Turbo（快速编辑）四个变体。核心改进包括：1）提升模型理解能力；2）优化数据质量与训练流程；3）引入 agentic inference-time scaling（推理时扩展）。模型仅使用 2.0862 亿张独特图像进行训练。

Result: 在标准基准评测中，Boogu-Image-0.1 持续匹配或超越其他开源模型，并达到接近领先闭源系统的性能。理论训练成本仅约 40 万美元。代码、权重和配方以 Apache 2.0 协议开源。

Conclusion: 通过针对性的模型理解、数据质量和训练流程改进，结合推理时扩展，可以在有限算力下实现接近闭源系统的统一多模态理解与生成性能。该工作为开源社区提供了有价值的实践经验和可复现的资源。

Classification reason: 该论文提出 Boogu-Image-0.1，一个统一多模态理解与生成模型家族，支持文生图、快速推理、指令编辑和双语文本渲染，属于"统一多模态模型"（unified_multimodal_model）这一现有子主题。该子主题在 multimodal_understanding_generation 和 multimodal_generation 两个方向下均有收录，论文同时匹配这两个方向。

Importance reason: Directly advances open-source unified multimodal understanding and generation with practical training recipes and strong performance under limited compute. Signals: unified multimodal model, open-source release, text-to-image generation, inference-time scaling, bilingual text rendering
...
### [67] [FM$^2$: Unified Federated Foundation Models for Heterogeneous Multimodal Medical Imaging](https://arxiv.org/abs/2607.13386)
...
TL;DR: 本文提出 FM²，一个统一的联邦基础模型框架，用于解决异构多模态医学影像中的成像模态异质性挑战。框架支持重叠模态（共享模态但标签分布异构）和非重叠模态（各客户端模态完全不相交）两种场景，通过双 MoE 模块（类级 MoE 和域级 MoE）结合异构模态对齐正则化器（HMA）实现个性化与跨模态共享表征，并引入字幕增强学习（CEL）利用 GPT-4o 生成的本地字幕作为文本语义桥梁实现跨客户端表征迁移。实验在 MIMH 基准和真实医学 VQA 数据集上取得 SOTA 结果。


<details>
  <summary>Details</summary>
Motivation: 现有联邦基础模型要么微调自然图像模型导致医学域迁移效果差，要么在单一模态内从头训练缺乏统一多任务的灵活性。本文识别了"成像模态异质性"这一未被充分探索的挑战，旨在解决联邦学习中客户端间模态重叠/非重叠异构场景下的多模态医学影像基础模型训练问题。

Method: FM² 框架包含：(1) 从头训练的核心骨干以保持医学域保真度，可选集成生物医学预训练编码器进行视觉语言对齐；(2) 每个客户端配备双 MoE 模块——类级 MoE 用于个性化类别知识，域级 MoE 用于跨模态共享表征；(3) 异构模态对齐正则化器（HMA）显式对齐模态特定专家参数，具有 O(1/√T) 收敛和泛化保证；(4) 字幕增强学习（CEL），利用本地 GPT-4o 生成的字幕作为文本语义桥梁实现跨客户端表征迁移；(5) 可扩展到联邦医学 VQA。

Result: 在 MIMH 基准（分类和 CEL）和真实医学 VQA 数据集上的实验表明，FM² 在所有三个任务上一致优于 SOTA 联邦基线方法，并展现出强大的跨模态泛化能力。

Conclusion: FM² 有效解决了联邦学习中多模态医学影像的模态异质性挑战，通过双 MoE 和 HMA 正则化实现了个性化与共享表征的平衡，CEL 策略进一步实现了非重叠模态间的知识迁移，为隐私保护下的多模态医学基础模型训练提供了统一框架。

Classification reason: 该论文提出 FM²，一个面向异构多模态医学影像的联邦基础模型框架，涉及视觉语言对齐（Caption-Enhanced Learning）和医学 VQA，属于多模态理解与生成方向下的通用视觉语言模型与 MLLM 范畴，因此映射到 canonical subtopic "vlm_mllm_general"。

Importance reason: Focuses on medical imaging and federated learning, which are not in your priority subtopics and are explicitly negative-keyworded. Signals: Federated learning, Medical imaging, Multimodal heterogeneity, Not in priority subtopics, Negative keywords present
...
### [68] [GHR-VLM: Making Zero-Shot Transit Video Analytics Realizable with Grounded Hybrid Reasoning](https://arxiv.org/abs/2607.13569)
...
TL;DR: GHR-VLM 提出了一种边缘-云混合推理框架，利用轻量级边缘监控模块检测车门状态并分割乘客片段，后端 VLM 通过粗到细的时空证据精炼来识别上车乘客并分类支付行为，在 486 分钟真实公交监控视频上验证了零样本乘客级支付分析的有效性。


<details>
  <summary>Details</summary>
Motivation: 公交视频理解能提供传统乘客计数器无法捕获的细粒度数据，但监督视频模型需要任务特定标注，而直接对长视频应用 VLM 既不可靠又成本高昂。作者希望结合两者的互补优势，实现零样本的公交视频分析。

Method: 提出 GHR-VLM 框架，采用边缘-云协同设计：边缘端轻量级监控器持续跟踪车门状态并分割乘客片段；云端 VLM 通过两阶段粗到细的时空证据精炼，先识别上车乘客，再分类支付行为。仅在接地乘客片段和联系单上调用 VLM，减少云端推理成本。

Result: 在 486 分钟真实公交监控视频上验证了框架在乘客级支付分析中的潜力，展示了接地边缘-云推理的有效性，同时也揭示了退化视频条件带来的挑战。

Conclusion: GHR-VLM 通过显式视觉接地将长监控流转换为紧凑的乘客中心时空证据，提升了 VLM 推理能力，为零样本公交视频分析提供了一种可行的边缘-云混合方案，但视频质量退化仍是主要挑战。

Classification reason: 该论文提出 GHR-VLM，一种基于视觉语言模型（VLM）的零样本公交视频分析框架，属于通用视觉语言模型在特定场景（公交监控视频）中的应用。虽然涉及视频理解，但核心是 VLM 的推理框架设计，因此映射到 [CANONICAL] 的 vlm_mllm_general（视觉语言模型与 MLLM）。

Importance reason: Focuses on transit video analytics with VLM, not on core priority subtopics like continuous language models or autoregressive generation. Signals: zero-shot video analytics, edge-cloud hybrid reasoning, VLM for transit, passenger payment classification, real-world surveillance evaluation
...
### [69] [OvisOCR2 Technical Report](https://arxiv.org/abs/2607.13639)
...
TL;DR: OvisOCR2 是一个 0.8B 参数的端到端文档解析模型，能够将文档页面图像解析为自然阅读顺序的 Markdown 格式（含文本、公式、表格和视觉区域）。通过数据引擎（真实标注+合成数据）、SFT、4B 分支上的多组件奖励 RL、on-policy 蒸馏到 0.8B 模型以及模型融合等训练策略，在 OmniDocBench v1.6 上以 96.58 总分达到 SOTA，首次让端到端模型登顶该榜单。


<details>
  <summary>Details</summary>
Motivation: 现有文档解析方法多采用流水线（pipeline）方式（如先检测再识别），流程复杂且错误累积。端到端模型在文档解析任务上尚未达到流水线方法的性能。作者希望证明端到端模型在文档解析任务上也能取得 SOTA 性能。

Method: 1) 数据引擎：结合过滤的真实文档标注与合成数据（从同一 HTML 源生成渲染图像和 Markdown 目标）；2) 训练流程：先进行 SFT，然后在 4B 分支上使用多组件奖励设计进行强化学习，再通过 on-policy 蒸馏将知识迁移到 0.8B 模型，最后进行模型融合。

Result: 在 OmniDocBench v1.6 上取得 96.58 总分（SOTA），在 PureDocBench 上取得 75.06 Avg3 分数（最高），在内部长尾/挑战性场景基准上也取得最佳整体性能。

Conclusion: OvisOCR2 证明了端到端文档解析模型能够超越传统流水线方法，达到 SOTA 性能，展示了端到端范式在文档解析领域的巨大潜力。

Classification reason: 该论文提出端到端文档解析模型 OvisOCR2，将文档页面图像解析为 Markdown 格式，属于文档理解与 OCR-free 文档智能范畴，与 [CANONICAL] 子主题 "视觉问答与文档理解" (multimodal_vqa_doc) 高度匹配。

Importance reason: Focuses on a narrow document parsing task with a small 0.8B model, lacking direct connection to the user's priority subtopics like continuous multimodal models or video generation. Signals: End-to-end document parsing, SOTA on OmniDocBench, 0.8B small model, RL + distillation training, No continuous or video focus
...
### [70] [EgoProceVQA: A Novel Egocentric Procedural Understanding Task with Self-Skill-Exploration Agent](https://arxiv.org/abs/2607.13792)
...
TL;DR: 本文提出EgoProceVQA，一个面向第一视角视频的程序性理解VQA任务，系统评估MLLM在关键步骤级别的程序推理能力。同时构建了EgoProceGen数据生成平台和包含3600个问题的基准，并提出EgoProceAgent——一种无需真实标注即可自我探索技能策略的智能体框架，在多个任务上达到开源模型最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有第一视角视频理解评测很少关注程序性理解，且缺乏关键步骤级别的复杂推理能力评估，而这对于构建可穿戴设备上的程序性AI助手至关重要。

Method: 1) 定义EgoProceVQA任务，包含六种关键步骤中心的问题类型；2) 开发EgoProceGen数据生成平台，高效构建不同问题类型的QA数据；3) 构建包含3600个问题、4种常见程序场景和31个日常程序任务的基准；4) 提出EgoProceAgent智能体框架，设计通用工具库和标准化子技能库，使智能体无需真实标注即可自我探索技能组合与选择策略。

Result: 现有MLLM和智能体在EgoProceVQA上仍有较大提升空间；EgoProceAgent在多个任务上达到开源模型最优性能。

Conclusion: 本文的基准、数据生成平台和智能体框架为第一视角程序性理解VQA任务建立了统一基础，揭示了当前模型在该任务上的不足，并提供了有效的解决方案。

Classification reason: 该论文提出了一个面向第一视角视频的程序性理解VQA任务（EgoProceVQA），属于视觉问答（VQA）范畴，与[multimodal_vqa_doc]（视觉问答与文档理解）这一CANONICAL subtopic高度匹配。

Importance reason: Focuses on egocentric procedural VQA, not aligned with user's priority on continuous multimodal models or video generation. Signals: Egocentric video, procedural understanding, VQA benchmark, agent framework, self-skill-exploration
...
TL;DR: 提出OCP-CT框架，通过器官条件化的模式token对齐实现CT影像与放射报告的细粒度视觉语言预训练，在零样本异常诊断任务上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有CT视觉语言预训练方法通常采用全局扫描-报告对比，虽然可扩展但模糊了不同器官的异质性证据；而直接的器官级对齐又过于粗糙，因为同一器官可能呈现多种不同的放射学表现。因此需要更细粒度的对齐单元——器官条件化的放射学模式。

Method: OCP-CT框架包含三个核心组件：(1) 稳定的全局CT-报告对比分支；(2) 器官模式接口：稀疏MoE根据潜在放射学模式路由图像和文本token，可学习slot将路由后的token查询为连续模式token；(3) 配对token对比：利用报告导出的临床相似性构建结构化软目标，对齐图像-文本模式token。

Result: 在CT-RATE和RAD-ChestCT基准上，零样本异常诊断的平均AUROC分别达到84.5%和69.9%，相比此前最强结果分别提升6.7和0.8个百分点。

Conclusion: 器官条件化的模式token对齐能够有效捕捉CT影像中细粒度的放射学模式，显著提升视觉语言预训练在医学影像理解任务上的性能。

Classification reason: 该论文关注CT影像与放射报告的视觉语言预训练，属于医学影像理解与视觉问答范畴。虽然具体领域为CT医学影像，但核心任务是视觉语言预训练（VLP）与零样本异常诊断，可归入多模态理解与生成方向下的"视觉问答与文档理解"（multimodal_vqa_doc）子主题，该子主题涵盖VQA及文档/医学影像理解。

Importance reason: Strong CT-specific VLP method but outside user's priority subtopics and negative keywords. Signals: CT vision-language pretraining, organ-conditioned pattern tokens, sparse MoE routing, zero-shot abnormality diagnosis, SOTA on CT-RATE and RAD-ChestCT
...
### [72] [FOLIO: Focused Semantic Memory for Streaming Video Understanding](https://arxiv.org/abs/2607.13298)
...
TL;DR: FOLIO 提出了一种无需训练的聚焦语义记忆系统，用于在线流式视频理解。它通过动态焦点状态在段级别更新记忆，结合短期视觉缓冲区和长期语义记忆（围绕实体组织并链接视觉证据缓存），在查询时通过轻量级混合检索实现高效信息获取。在 OVO-Bench 和 StreamingBench 上达到 SOTA，同时大幅降低流式记忆维护成本。


<details>
  <summary>Details</summary>
Motivation: 在线流式视频理解中，视频帧持续到达且查询随时可能发生，系统需要不断压缩和保留已观测视频前缀的信息。核心挑战在于决定保留哪些信息以及如何组织维护的历史：随着流增长，记忆成本增加且保留了大量冗余视觉细节，而后续查询往往依赖于特定实体、动作及其时序变化。现有方法缺乏高效的聚焦记忆管理机制。

Method: FOLIO 是一个无需训练的聚焦语义记忆系统。它按段级别更新记忆，由动态焦点状态引导，结合短期视觉缓冲区与长期语义记忆（围绕观测实体组织，并链接到视觉证据缓存）。查询时，通过轻量级混合检索结合结构化记忆上的直接匹配与语义查询扩展来获取相关信息。

Result: 在 OVO-Bench 上使用 Qwen3-VL-8B 达到 82.0/69.1 的 Perception/Backward 准确率，在 StreamingBench 上达到 74.5 总体准确率，均为 SOTA。同时显著降低了流式记忆维护成本，仅对聚焦实体保留详细记录，对周围上下文进行紧凑存储。

Conclusion: FOLIO 通过聚焦语义记忆系统有效解决了在线流式视频理解中的信息压缩与检索难题，在性能与效率之间取得了良好平衡，为长视频流场景下的视频理解提供了实用方案。

Classification reason: 该论文关注在线流式视频理解中的语义记忆管理，属于视频理解方向，与 [CANONICAL] subtopic "视频理解与时序推理"（video_understanding_temporal）高度匹配。

Importance reason: Focuses on streaming video memory management, not on the user's priority subtopics like continuous language models or autoregressive generation. Signals: streaming video understanding, training-free memory system, entity-centric semantic memory, OVO-Bench SOTA, no generative or continuous modeling focus
