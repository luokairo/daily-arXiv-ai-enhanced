...
- [世界模型](#direction-world_model) [Total: 75]
  - [具身世界模型与规划控制](#direction-world_model-embodied_world_model) [Total: 59]
  - [可交互与物理世界建模](#direction-world_model-interactive_physical_world) [Total: 4]
  - [空间 3D/4D 世界模型](#direction-world_model-spatial_3d_4d_world_model) [Total: 4]
  - [视频世界模型](#direction-world_model-video_world_model) [Total: 8]
- [多模态生成](#direction-multimodal_generation) [Total: 92]
  - [3D 生成与重建](#direction-multimodal_generation-three_d_generation) [Total: 8]
  ...
  - [数字人与动作生成](#direction-multimodal_generation-avatar_motion_generation) [Total: 14]
  - [文生图与图像扩散](#direction-multimodal_generation-text_to_image_diffusion) [Total: 8]
  - [文生视频与视频扩散](#direction-multimodal_generation-text_to_video_diffusion) [Total: 6]
  - [生成模型加速与高效化](#direction-multimodal_generation-efficient_generation) [Total: 11]
  - [生成模型对齐与奖励](#direction-multimodal_generation-generation_alignment) [Total: 13]
...
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 100]
  ...
  - [多模态强化学习与对齐](#direction-multimodal_understanding_generation-multimodal_rl_alignment) [Total: 9]
  - [多模态推理与思维链](#direction-multimodal_understanding_generation-multimodal_reasoning_cot) [Total: 12]
  - [多模态检索](#direction-multimodal_understanding_generation-multimodal_retrieval) [Total: 3]
  - [多模态评测与基准](#direction-multimodal_understanding_generation-multimodal_benchmark) [Total: 31]
  - [多模态高效化与轻量化](#direction-multimodal_understanding_generation-efficient_multimodal) [Total: 2]
  - [生成模型评测与数据集](#direction-multimodal_understanding_generation-generation_benchmark) [Total: 5]
  ...
  - [视觉问答与文档理解](#direction-multimodal_understanding_generation-multimodal_vqa_doc) [Total: 11]
...
### [1] [FlowDec: Temporal Conditional Flow Decorruptor for Robust Continuous Vision-Language Navigation](https://arxiv.org/abs/2606.22424)
...
TL;DR: FlowDec 是一种面向连续环境视觉语言导航（VLN-CE）的时序条件流去损框架，通过混合时序条件策略和动作质心引导滤波，在保持低延迟的同时显著提升 LM 基导航代理在真实视觉损坏下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有 VLN-CE 方法在大模型驱动下取得了进展，但在真实世界中视觉损坏（如模糊、噪声、光照变化等）会严重降低导航性能，而这一问题尚未被充分探索。作者旨在设计一种高效的图像恢复框架，专门针对 LM 基 VLN-CE 场景，提升其在不可预测真实条件下的鲁棒性。

Method: FlowDec 包含两个核心设计：(1) 混合时序条件策略（Hybrid Temporal Conditioning），将历史帧的视觉上下文与当前损坏帧对齐，引导生成流路径；(2) 动作质心引导滤波（Action-Centroid Guided Filtering），根据代理的动作信息动态评估并融合恢复输出，确保时序一致性和导航相关性。整体基于 flow matching 框架进行连续潜空间中的图像去损。

Result: 在多个 VLN-CE 基准上，FlowDec 在导航准确率和生成延迟两方面均优于现有最先进的去损方法，证明了其在真实世界视觉损坏下的有效性和高效性。

Conclusion: FlowDec 为鲁棒的具身导航提供了一种高效、可扩展的范式，展示了连续潜空间流匹配方法在视觉语言导航中的潜力，为未来在不可预测真实环境中的具身智能研究奠定了基础。

Classification reason: 该论文提出 FlowDec，一种基于 flow matching 的连续图像恢复框架，用于连续环境视觉语言导航（VLN-CE），涉及连续视觉 token 恢复、flow matching 与多模态理解。与 canonical subtopic "连续 VLM 与视觉潜空间流"（continuous_vlm_latent_flow）高度匹配。同时与 world_model 方向下的具身导航相关。

Importance reason: Directly advances continuous vision-language navigation with a novel temporal flow decorruption method, aligning with top-priority continuous multimodal and world model directions. Signals: continuous vision-language navigation, temporal conditional flow, robust to visual corruptions, low-latency generation, action-centroid guided filtering
...
### [2] [B[FM]$^2$: Brain Foundation Model via Flow Matching with SplitUNet](https://arxiv.org/abs/2606.20812)
...
TL;DR: B[FM]² 提出了一种基于连续时间 flow matching 的脑电图基础模型，直接在原始多通道 EEG 信号上预训练，无需分块、tokenization 或掩码。针对时间-电极不对称性设计了 SplitUNet 速度网络，在仅使用约 307 小时数据（比现有模型少 30 倍）的情况下，在 9 个下游任务中的 7 个上达到 SOTA，且生成的合成 EEG 让神经科医生无法区分。


<details>
  <summary>Details</summary>
Motivation: 现有 EEG 基础模型通常将连续多通道 EEG 波形离散化为 patch 或 codebook token，并用掩码自监督训练 Transformer。这种离散化破坏了连续的脑节律，掩盖了细粒度的时序动态。作者希望设计一种与 EEG 数据固有连续特性一致的预训练方法。

Method: 提出 B[FM]² 框架，核心是使用连续时间 flow matching 直接在原始 EEG 信号上预训练，避免任何离散化。针对多通道 EEG 的时间-电极不对称性（时间维度密集采样且高度自相关，电极维度通道数少但空间位置不同），设计了 SplitUNet 作为速度网络，将每个块分解为独立的 1D 时间卷积和 1D 电极卷积，仅沿时间维度下采样，在整个层级中保留电极拓扑结构。

Result: 在 9 个标准下游 EEG 分类任务中的 7 个上达到新 SOTA；预训练仅使用 36,895 个片段（约 307 小时），比现有 EEG 基础模型少 1-2 个数量级（约 30 倍）；生成的合成 EEG 经两位认证神经科医生盲测无法与真实脑电区分（Cohen's κ = -0.096）。

Conclusion: 连续时间 flow matching 是 EEG 基础模型的有效预训练范式，避免了离散化带来的信息损失；SplitUNet 有效解决了多通道 EEG 的时间-电极不对称性；该方法在数据效率上显著优于现有离散化方法，且具备生成高质量合成 EEG 的能力。

Classification reason: 该论文使用连续时间 flow matching 直接在原始 EEG 信号上预训练，属于连续潜空间建模（连续语言模型与语言流方向）。虽然 EEG 不是自然语言，但论文的核心方法（连续时间 flow matching、无离散化 tokenization）与连续语言模型范式高度一致，且该方向涵盖扩散语言模型与 flow matching language model，因此映射到 canonical 子主题"连续语言模型与语言流"。

Importance reason: Strong methodological contribution (flow matching on raw EEG) but outside user's core multimodal and generation priorities. Signals: flow matching on raw signals, SplitUNet for time-electrode asymmetry, 30x less data than prior EEG models, synthetic EEG indistinguishable from real, not in user's priority subtopics

Abstract: EEG foundation models can learn generalizable representations from large-scale EEG corpora to enable single-backbone transfer across diverse clinical and brain-computer interface tasks. Existing models typically discretize the continuous multi-channel EEG waveform into patches or codebook tokens and train a transformer with masked self-supervision. Recognizing that this discretization fragments continuous brain rhythms and obscures fine-grained temporal dynamics, we present B[FM]$^2$(Brain Foundation Model via Flow Matching), whose inductive bias aligns with the data by pretraining directly on the raw signal using continuous-time flow matching without patches, tokenization, or masking. However, multi-channel EEG signals pose an architectural challenge for flow matching: time is densely sampled and highly autocorrelated (thousands of timepoints), while the electrode axis is short (tens of channels) at distinct scalp positions. To address this time-electrode asymmetry, we introduce SplitUNet, a velocity network that factorizes each block into separate 1D temporal and 1D electrode convolutions and downsamples only along time, preserving electrode topology throughout the hierarchy. B[FM]$^2$ sets a new state of the art on 7 of 9 standard downstream EEG classification tasks, using a pretraining budget of only 36,895 segments ($\approx$ 307h), 1-2 orders of magnitude ($\approx$ 30x) less than required by existing EEG foundation models. Further, it generates synthetic EEGs that two board-certified neurologists cannot distinguish from brain data (Cohen's $\kappa =$ -0.096).

</details>


### [3] [When to Plan, When to Polish: Noise Level as a Granularity Axis for Diffusion Language Models](https://arxiv.org/abs/2606.21802)
...
TL;DR: 本文提出噪声依赖粒度控制（NDGC），一种单层扩散语言模型方法，利用噪声水平作为粒度线索，在高噪声阶段使用连贯 token 组支持早期语义承诺，低噪声阶段回归 token 级精炼，实现无需显式规划器或分层架构的从粗到细去噪。


<details>
  <summary>Details</summary>
Motivation: 标准 token 级扩散语言模型在整个去噪过程中保持 token 粒度训练和推理，高噪声时产生分散的局部片段而非连贯证据，难以形成早期粗略结构，而这正是规划敏感生成所需要的。分层规划方法虽能分离规划与措辞，但需要额外的规划器、块潜变量或两阶段设计。

Method: 提出 Noise Dependent Granularity Control (NDGC)，一种单层扩散方法，将噪声水平作为粒度线索。高噪声步骤使用连贯 token 组（coherent token groups）支持早期意义承诺，低噪声步骤回归 token 级精炼，实现从粗到细的去噪过程，无需显式规划器或分层架构。

Result: 在控制实验、消融研究和 WritingPrompts 数据集上，NDGC 展示了更早的骨架形成、更好的有序恢复和更健康的输出。

Conclusion: 噪声水平可以作为扩散语言模型中粒度的有效控制轴，NDGC 通过简单的单层设计实现了类似规划的分层从粗到细生成效果，为扩散语言模型的生成质量提升提供了新思路。

Classification reason: 该论文研究扩散语言模型中的噪声依赖粒度控制，属于连续语言建模范畴，与 canonical 子主题"连续语言模型与语言流"（continuous_language_flow）高度匹配。

Importance reason: Directly advances continuous language model diffusion with noise-level granularity control, aligning with top-priority subtopic and boosting autoregressive-like planning. Signals: continuous language model, diffusion language model, noise-dependent granularity, coarse-to-fine denoising, planning without explicit planner
...
### [4] [Test-Time Training with Next-Token Prediction](https://arxiv.org/abs/2606.21803)
...
TL;DR: 本文提出 TTT-NTP，一种即插即用的测试时训练方法，利用预训练语言模型自身的 next-token prediction 信号（下一个位置的上下文隐状态）作为快速权重更新的监督目标，无需修改模型架构即可提升长上下文任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时训练（TTT）方法要么需要重新设计模型架构，要么使用与自监督信号无关的局部值代理进行快速权重更新。作者希望利用语言模型预训练中已有的 next-token prediction 信号来直接监督测试时的快速权重适应，使 TTT 与模型自身的训练目标一致。

Method: TTT-NTP 是一种即插即用的快速权重适应方法。在测试时，对于每个输入 token，模型将其上下文隐状态通过一个点线性投影作为值目标，该目标与下一个位置的上下文隐状态对齐，从而遵循与 next-token prediction 相同的因果计算范式。快速权重通过在线梯度下降更新，无需修改预训练模型的主干架构。

Result: 在 RULER Full-13 基准上（平均 4k/8k/16k/32k 上下文长度），TTT-NTP 在 Llama-3.1-8B（+3.9）、Mistral-7B-v0.3（+3.0）、Qwen3-4B（+4.1）和 Qwen3-0.6B（+2.9）上均一致提升。在 LongBench-v2 长文档 QA 上，Llama-3.1-8B 提升 +5.6，Mistral-7B-v0.3 提升 +3.7，同时保持常识和知识性能。

Conclusion: TTT-NTP 证明了利用 next-token prediction 这一自监督信号进行测试时训练的有效性，是一种无需修改模型架构即可提升预训练 LLM 长上下文能力的通用方法，在多个模型家族和规模上均表现出一致的改进。

Classification reason: 该论文提出 TTT-NTP，利用语言模型自身的 next-token prediction 信号进行 test-time training（快速权重适应），核心思想是在连续隐空间中通过 next contextual hidden state 监督局部更新，属于连续语言模型中的快速权重适应方法，与 continuous_language_flow 中连续 latent 空间语言建模的主题高度相关。

Importance reason: Directly improves long-context LLMs via test-time training with next-token prediction, aligning with user's focus on efficient inference and continuous language models. Signals: test-time training, next-token prediction, long-context improvement, drop-in adaptation, Llama-3.1-8B
...
### [5] [Scheduling Thoughts: Learning the Order of Thought in Diffusion Language Models](https://arxiv.org/abs/2606.23567)
...
TL;DR: 本文提出 Self-Aware Scheduling (SAS)，通过策略优化学习掩码扩散语言模型的最优解码顺序（即"思维顺序"），在 Sudoku 和数学推理任务上显著超越启发式调度方法。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型的解码顺序（unmasking order）对生成质量影响很大，但现有方法通常采用启发式规则（如随机顺序、从左到右等），缺乏理论指导。作者希望将顺序选择形式化为一个可优化的策略问题。

Method: 推导了顺序解码失配的 KL 散度上界，该上界与模型的路径对数似然相关，从而将顺序选择转化为策略优化问题。基于此提出 SAS，使用 GRPO 学习轻量级顺序策略，可无缝应用于 any-order 和 semi-autoregressive 解码。

Result: 在 Sudoku 任务上，1B MDM 的准确率从 82.0%（最佳启发式调度）提升至 91.8%，结合第二阶段微调达到 97.5%。在数学推理任务上，LLaDA-8B 在 GSM8K 上 pass@1 从 64% 提升至 76%，在 MBPP 上从 39.5% 提升至 41%。

Conclusion: SAS 证明了通过学习解码顺序可以显著提升扩散语言模型的生成质量，为扩散语言模型的解码策略提供了理论驱动的优化框架。

Classification reason: 该论文研究掩码扩散语言模型（Masked Diffusion Language Model）中的解码顺序学习，属于连续潜空间中的语言建模方法，与 [CANONICAL] 连续语言模型与语言流 高度匹配。

Importance reason: Directly advances continuous language model decoding with a principled order-learning method, highly aligned with your top-weighted subtopic. Signals: continuous language model, diffusion language model, order of thought, policy optimization, decoding order
...
### [6] [Flow Annealing Posterior Sampling for Function-Space Regression and Inverse Problems](https://arxiv.org/abs/2606.22346)
...
TL;DR: 本文提出 FAPS，首个统一随机过程回归与 PDE 逆问题的函数空间后验采样框架，基于预训练的 flow-matching prior，通过似然引导的后验推理和 Langevin 校正，在稀疏噪声观测下实现准确的不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 随机过程的回归和 PDE 逆问题长期缺乏统一的函数空间后验采样框架，现有方法要么需要显式先验密度评估，要么无法灵活处理稀疏噪声观测和变分辨率查询。

Method: FAPS 利用预训练的函数空间 flow-matching prior，通过似然引导进行后验推理，支持变分辨率查询且无需显式先验密度评估。其 Langevin 校正采用低秩协方差预条件子，利用跨分辨率的函数空间主导相关性。

Result: 在高斯和非高斯随机过程回归基准以及多种 PDE 逆问题上，FAPS 生成一致的后验样本并具有准确的不确定性量化，显著优于现有函数回归基线，在 PDE 噪声逆问题上达到或优于基于扩散的后验采样器，同时降低了测试时采样成本。

Conclusion: FAPS 为函数空间回归和逆问题提供了一种高效、灵活且可扩展的后验采样框架，展示了 flow matching prior 在科学计算中的巨大潜力。

Classification reason: 该论文提出 Flow Annealing Posterior Sampling (FAPS)，基于预训练的 function-space flow-matching prior 进行后验采样，属于连续潜空间中的 flow matching 方法，与 [CANONICAL] 连续语言模型与语言流 中的 flow matching 主题高度相关。

Importance reason: Proposes a novel flow-matching posterior sampling framework for function-space regression, aligning with continuous language model and flow matching priorities. Signals: flow matching posterior sampling, function-space regression, PDE inverse problems, uncertainty quantification, Langevin correction
...
### [7] [Latent Goal Prediction from Language for Model-Based Planning](https://arxiv.org/abs/2606.20627)
...
TL;DR: LAGO 提出一种从语言指令预测潜在子目标序列的框架，结合世界模型在统一潜空间中进行动作条件 rollout 和基于模型的规划，通过在线更新子目标和软最小轨迹代价实现长程精确规划。


<details>
  <summary>Details</summary>
Motivation: 基于世界模型的规划面临两个瓶颈：预测误差累积导致长程规划失效，以及难以定义可优化的目标。视觉目标提供精确局部梯度但缺乏远距离引导，语言灵活但受限于跨模态对齐噪声或依赖不适合高采样规划的大生成模型。

Method: LAGO 在统一潜空间中同时预测语言指令对应的中间子目标状态序列和动作条件 rollout。它将指令动态分解为显式预测的局部可解潜在子目标，在规划过程中在线更新这些子目标，并使用软最小轨迹代价（soft minimum trajectory cost）进行规划，使智能体能够跟随连贯的潜在轨迹完成长程任务。

Result: 在多个环境和规划时域上的评估表明，LAGO 避免了先前方法的性能急剧退化，实现了鲁棒且精确的长程规划，弥合了视觉目标的精确性与文本引导的灵活性之间的差距。

Conclusion: LAGO 通过语言引导的潜在子目标预测与在线更新，有效解决了基于世界模型的长程规划中的累积误差和目标定义难题，为语言引导的模型规划控制提供了新范式。

Classification reason: 该论文提出 LAGO 框架，利用语言指令预测潜在子目标，并结合世界模型进行基于模型的规划与控制。这直接属于世界模型方向下的"具身世界模型与规划控制"（embodied_world_model）这一规范子主题，因为其核心是使用世界模型进行长程规划，且涉及动作条件 rollout 和子目标分解。

Importance reason: Strong world model planning method but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: latent subgoal prediction, world model planning, language-guided control, long-horizon planning, soft minimum trajectory cost
...
### [8] [Expected Free Energy-based Planning as Variational Inference](https://arxiv.org/abs/2606.20658)
...
TL;DR: 本文证明基于期望自由能(EFE)的规划可以形式化为在具有认知先验的生成模型上的变分自由能最小化，将规划统一到与感知和学习相同的推理过程中，并在三个具身环境中验证了该方法的信息寻求行为和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于EFE的规划方法通常采用专门的优化过程，难以扩展或分析。作者希望将EFE规划统一到变分自由能最小化的框架下，使其与自由能原理在理论上保持一致，从而将规划、感知和学习纳入同一推理过程。

Method: 作者将EFE规划重新表述为在具有认知先验的生成模型上的变分自由能最小化。通过精心选择先验，变分自由能最小化可分解为期望计划成本(EFE)加上复杂度项。该方法支持基于策略的推理（优于基于计划的推理），并通过时间因子分解实现可扩展性。

Result: 在确定性T-maze、随机Reactivity Maze和部分可观测MiniGrid DoorKey-8x8三个环境中验证：1) 认知先验能诱导信息寻求行为；2) 变分公式下的基于策略的推理在随机转换下优于基于计划的方法；3) 时间因子分解使方法可扩展到现有表格型主动推理方法无法处理的复杂环境。

Conclusion: EFE规划可以自然地形式化为变分自由能最小化，这加强了与自由能原理的理论一致性，并为规划提供了更灵活、可扩展的推理框架，有望统一感知、学习和规划过程。

Classification reason: 该论文研究基于期望自由能(EFE)的规划方法，将其形式化为变分自由能最小化，属于主动推理框架下的规划与控制。这与[具身世界模型与规划控制]方向高度吻合，该方向涵盖面向机器人、具身智能、动作规划和控制的世界模型。论文在T-maze、Reactivity Maze和MiniGrid等具身环境中验证，进一步确认了该分类。

Importance reason: The paper's theoretical unification of planning and inference is valuable for world models, but its focus on tabular active inference and small-scale environments limits direct relevance to the user's priority on scalable continuous multimodal and video generation models. Signals: Active inference, Expected Free Energy, Variational Free Energy, planning as inference, information-seeking behavior
...
### [9] [JPPD: Joint Prediction_Planning Diffusion with Differentiable Safety Guidance for Dynamic Obstacle Avoidance in Intelligent Transportation Systems](https://arxiv.org/abs/2606.20686)
...
TL;DR: 提出 JPPD 联合预测-规划扩散框架，将参与者预测与机器人规划统一为单条件轨迹生成问题，使用因果 Transformer 跨轨迹注意力联合采样，并引入可微安全势引导与条件流匹配，在共享空间动态避障中提升尾部安全性与运行效率。


<details>
  <summary>Details</summary>
Motivation: 现有系统将轨迹预测与运动规划分解为两个独立模块，导致单向信息流：预测影响规划，但规划无法影响预测。这种分离方式在共享空间动态交互场景中难以保证安全性与协同性。

Method: 提出联合预测-规划扩散框架（JPPD），将参与者轨迹预测与机器人规划视为单一条件轨迹生成问题，使用因果 Transformer 跨轨迹注意力从联合分布中采样所有轨迹。引入可微安全势引导（differentiable safety potential guidance），用随时间变化的占用概率势的梯度直接引导联合采样器；采用条件流匹配减少推理步数同时保持多模态轨迹多样性。

Result: 在场景仿真、自然行人回放、Isaac Sim 验证以及 ROS/Orin 部署实验中，联合采样相比分离的预测-规划基线在尾部安全性（near misses、blockage time、induced deviation、hard-braking）和运行时效率上均有提升。

Conclusion: 联合预测-规划扩散框架能有效改善共享空间动态避障中的安全性与效率，可微安全势引导替代了启发式排斥后处理，条件流匹配在保持多模态性的同时加速推理。

Classification reason: 该论文提出联合预测-规划扩散框架，将参与者轨迹预测与机器人运动规划统一为条件轨迹生成问题，面向共享空间中的动态障碍物避让。这属于具身智能中的规划控制与动态环境建模，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on robotics planning diffusion, not directly on the user's priority subtopics like continuous language models or video generation. Signals: Joint prediction-planning diffusion, differentiable safety guidance, dynamic obstacle avoidance, conditional flow matching, world model for transportation
...
### [10] [SafeDojo: Safe Reinforcement Learning for VLA via Interactive World Model](https://arxiv.org/abs/2606.20698)
...
TL;DR: SafeDojo 提出了首个基于世界模型的安全强化学习框架，用于视觉-语言-动作（VLA）策略。它通过交互式视频世界模型生成动作条件未来预测，结合 ResNet 成功分类器和轻量级安全头分别评估任务进度和轨迹安全成本，并利用基于 Lagrangian 约束的 GRPO 目标平衡任务成功与安全约束。在 SafeLIBERO 基准和真实 Franka 机器人上取得了最优的安全成功率。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的具身智能需要安全控制，但现有安全强化学习方法要么需要昂贵的真实世界探索，要么依赖手工设计的安全函数，难以扩展到部署在开放世界物理环境中的视觉-语言-动作（VLA）模型。

Method: SafeDojo 是一个基于模型的安全强化学习框架。它构建了一个交互式视频世界模型，生成动作条件的未来帧预测。从预测帧中，一个 ResNet 成功分类器估计每步任务进度，一个轻量级安全头从潜在上下文和动作块中预测每步安全成本。任务奖励和安全成本信号通过 Lagrangian 约束的 GRPO 目标进行平衡优化。

Result: 在 SafeLIBERO 基准上，SafeDojo 在任务成功率、安全成功率和执行效率方面均优于推理时安全、无模型 RL 和基于模型 RL 的基线方法，在 Level I 上比最强基线高出 8.25 个百分点。真实 Franka 机器人部署在五个任务上取得了最佳平均任务成功率和安全成功率。

Conclusion: 基于世界模型的安全强化学习是实现安全具身智能的一种可扩展且可泛化的路径。SafeDojo 证明了通过世界模型想象进行安全学习可以有效替代真实世界探索和手工安全函数。

Classification reason: 该论文提出 SafeDojo，一个基于交互式视频世界模型的安全强化学习框架，用于视觉-语言-动作（VLA）策略的安全控制。核心是使用世界模型进行动作条件未来预测，并结合安全成本约束进行策略优化，属于具身智能中的世界模型与规划控制方向，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Strongly aligns with world model and VLA priorities, but safety focus is secondary to core continuous multimodal and generation interests. Signals: World model-based safe RL, VLA policy, Interactive video world model, Lagrangian constrained GRPO, Real-world robot deployment
...
### [11] [Real-World Deployment of Massively Parallel Sampling-Based MPC for Contact-Rich Manipulation](https://arxiv.org/abs/2606.20712)
...
TL;DR: 本文提出了一个基于JAX大规模并行化和MuJoCo MJX高保真仿真的采样型模型预测控制（SMPC）框架，并在Franka Research 3机器人上通过完整的real-to-sim-to-real流程部署到Push-T操作任务中。MTP变体结合结构化全局采样在需要模式切换的任务中优于CEM、MPPI和PS等基线。同时评估了在线域随机化在MPC采样预算内的效果，揭示了接触敏感性和计算预算紧张等关键挑战。


<details>
  <summary>Details</summary>
Motivation: 采样型MPC在接触丰富的机器人操作中具有潜力，但大多数先前工作依赖简化动力学或仅停留在仿真中，缺乏在真实机器人上的部署验证。本文旨在弥合仿真与真实之间的差距，探索大规模并行化SMPC在真实接触操作任务中的可行性。

Method: 利用JAX进行大规模并行化计算，结合高保真MuJoCo MJX仿真器，构建采样型MPC框架。提出MTP（Multi-modal Trajectory Prior）变体，采用结构化全局采样策略。通过real-to-sim-to-real流程在Franka Research 3机器人上部署Push-T操作任务，并在线评估域随机化对MPC的影响。

Result: MTP变体在需要模式切换的任务中，在仿真和真实硬件上均优于CEM、MPPI和PS等单峰基线。接触初始化参数的域随机化能产生可解释的自适应信号，而全局物理参数在典型重规划频率下提供的反馈过弱，难以可靠利用。

Conclusion: 采样型MPC在接触丰富的真实机器人操作中是可行的，但面临接触敏感性、严格的计算预算以及在线获取有效域随机化信号的困难等关键挑战，为未来研究指明了方向。

Classification reason: 该论文提出基于采样的模型预测控制（SMPC）框架，利用JAX大规模并行化和MuJoCo MJX高保真仿真，在Franka机器人上执行接触丰富的Push-T操作任务，涉及机器人规划、控制与物理世界建模，属于具身世界模型与规划控制方向。

Importance reason: Strong in world_model direction and robotics, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: contact-rich manipulation, sampling-based MPC, real-to-sim-to-real, JAX parallelization, MuJoCo MJX
...
### [12] [SignVLA: Real-Time Sign Language-Guided Robotic Manipulation via Attention LSTM and Vision-Language-Action Models](https://arxiv.org/abs/2606.20857)
...
TL;DR: SignVLA 提出一个实时手语引导的机器人操作框架，通过注意力增强 LSTM 网络从视频中识别手语手势并转化为语义指令，再传递给下游 VLA 策略执行操作任务，为听障用户提供无障碍人机交互接口。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型主要依赖语音或文本输入，无法服务于聋哑、听障及语言障碍用户。作者希望构建一个基于手语输入的 VLA 框架，提升多模态具身智能的可及性。

Method: SignVLA 采用模块化设计：(1) 从视频流中提取手部关键点特征；(2) 使用注意力增强 LSTM 网络捕捉时序手势动态，进行字母级和命令级手语识别；(3) 引入时序稳定模块提升实时交互中的预测一致性；(4) 将识别出的指令序列传递给下游 VLA 策略，执行手语条件化的机器人操作任务。

Result: 实验表明 SignVLA 实现了稳定的实时手语识别，并成功执行了由手语输入驱动的机器人操作任务。

Conclusion: 轻量级时序手语识别可以作为多模态具身智能的有效且实用的可及性层，为听障用户提供更自然的人机交互方式。

Classification reason: 该论文提出 SignVLA，将手语识别与 VLA（Vision-Language-Action）模型结合，实现手语引导的机器人操作。核心是具身智能中的动作规划与控制，属于 [CANONICAL] 具身世界模型与规划控制 范畴。同时涉及多模态理解（手语视觉识别+语言指令），因此也匹配 multimodal_understanding_generation 方向。

Importance reason: Focuses on sign language accessibility for robotics, not on core continuous multimodal models or video generation priorities. Signals: sign language, robotic manipulation, LSTM, VLA, accessibility

Abstract: Vision-Language-Action (VLA) models enable robots to execute manipulation tasks from natural-language instructions grounded in visual observations. However, existing VLA interfaces primarily rely on speech or text input, limiting accessibility for deaf, hard-of-hearing, and speech-impaired users. We present SignVLA, a real-time sign-language-guided VLA framework for accessible human-robot interaction. The system introduces a modular sign-to-text interface that converts visual sign gestures into semantic instructions compatible with downstream VLA policies. Given video streams, SignVLA extracts hand landmark features and employs an attention-enhanced Long Short-Term Memory (LSTM) network to capture temporal gesture dynamics for alphabet- and command-level sign recognition. A temporal stabilization module further improves prediction consistency in real-time interaction generated instruction sequence is then passed to a downstream VLA policy for sign-conditioned robotic manipulation. Experimental results demonstrate stable real-time sign recognition and successful execution of manipulation tasks driven by sign-language inputs. Our findings suggest that lightweight temporal sign recognition can serve as an effective and practical accessibility layer for multimodal embodied intelligence.
...
TL;DR: UNSEEN 是一个面向未知环境的统一不确定性感知导航框架，仅使用前视摄像头，以6Hz频率估计稀疏地图和机器人位姿及其不确定性，并在滚动时域中联合优化任务进度与估计精度，在仿真和真实实验中实现了比SOTA更低的定位误差和100%的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉导航方法多采用松散耦合的模块化流水线，对感知质量或环境结构有强假设，且常依赖多模态传感器增加系统复杂度。纯视觉导航在运动模糊、低纹理和光照变化下性能严重下降，因为它们忽略了指令运动与感知之间的紧密耦合。感知感知方法虽部分解决了该问题，但通常优化单个模块，未能一致地在整个导航栈中传播不确定性。

Method: UNSEEN 提出统一的不确定性与感知感知导航框架，显式耦合定位、建图和规划。系统仅使用前视摄像头，以6Hz频率估计稀疏地图和机器人位姿及其不确定性，并在滚动时域中规划轨迹，联合优化任务进度与估计精度。框架包含UNSEEN-SLAM（改进的稀疏SLAM）和UNSEEN-Plan（不确定性感知规划器）。

Result: 在仿真和真实未知环境实验中，UNSEEN-SLAM将绝对平移误差降低9.8%，UNSEEN-Plan将估计精度提升高达45%，任务成功率达到100%。

Conclusion: UNSEEN 通过显式耦合定位、建图和规划中的不确定性传播，显著提升了未知环境下纯视觉导航的鲁棒性和成功率，为资源受限平台提供了一种轻量级且有效的解决方案。

Classification reason: 该论文提出面向未知环境的视觉导航框架UNSEEN，将定位、建图与规划紧密耦合，利用稀疏地图和位姿不确定性进行重规划，属于机器人具身导航与规划控制范畴，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Strong robotics navigation paper but outside user's core focus on continuous multimodal models and generative video/image generation. Signals: uncertainty-aware navigation, sparse estimation, perception-aware planning, real-world validation, 100% task success rate
...
### [14] [World Action Models: A Survey](https://arxiv.org/abs/2606.20781)
...
TL;DR: 本文对World Action Models (WAMs)进行了系统综述，澄清了世界模型、视频生成模型、动作条件视频世界模型、视觉-语言-动作策略与WAM之间的边界，并从"生成内容"和"方法解剖"两个互补视角组织现有工作，讨论了可交互性、因果性、持久性、物理合理性和泛化性等关键维度，以及数据、评测和开放挑战。


<details>
  <summary>Details</summary>
Motivation: 近年来，世界动作模型（WAMs）快速发展，但边界模糊——大型视频生成模型被重新用于预测，同时也有基于语言或视觉语言骨干的非视频生成方法。这种快速扩张模糊了世界模型、视频生成模型、动作条件视频世界模型、VLA策略和WAM之间的界限，亟需一个统一的框架来厘清这些概念并系统组织现有工作。

Method: 本文采用两个互补视角组织文献：(1) 生成内容视角——按方法需要生成的内容分类，包括渲染的未来（rendered futures）、潜空间未来（latent futures）和无视频生成的动作推理；(2) 方法解剖视角——按预测基质（predictive substrate）、骨干网络、动作耦合方式和部署模式进行分解。基于此框架，统一讨论了可交互性、因果性、持久性、物理合理性和泛化性等关键属性，以及数据、评测和开放挑战。

Result: 揭示了一致的设计模式：WAMs并非简单的"带动作头的视频生成器"，而是预测-动作方法，其设计选择在表征丰富度与计算、内存、延迟和动作标签成本之间进行权衡。领域正朝着"生成更少的未来内容，同时保留控制所需信息"的方向发展。

Conclusion: WAMs领域正在从生成完整未来视频转向更高效、更聚焦的预测-动作范式，在保持控制能力的同时降低计算开销。该综述为领域提供了统一的术语和分类框架，有助于指导未来研究方向。

Classification reason: 该论文是关于World Action Models (WAMs)的综述，聚焦于具身预测-动作模型，涵盖视频生成式世界模型、语言/视觉语言骨干的动作推理等，与[具身世界模型与规划控制]高度匹配。

Importance reason: Comprehensive survey clarifying world model boundaries and design trade-offs, directly aligning with user's high-priority world model and continuous multimodal subtopics. Signals: world model survey, action-grounded video world models, predictive-action models, design trade-offs, interactability

Abstract: World Action Models (WAMs) are embodied predictive-action models that make a forecast of the future available to action. Recent WAMs repurpose large video generation models, and a parallel line relies on language or vision-language backbones without a video-generation core. This rapid expansion has blurred the boundary among broad world models, video generation models, action-grounded video world models, Vision-Language-Action policies, and WAMs. This survey gives the field a common account. It first clarifies these boundaries, then organizes existing works through two complementary views. The first view asks what each method is required to generate, spanning rendered futures, latent futures, and video-generation-free action reasoning. The second view decomposes each method by predictive substrate, backbone, action coupling, and deployment regime. This anatomy supports a unified discussion of interactability, causality, persistence, physical plausibility, and generalization, followed by data, evaluation, and open challenges. Across these axes, a consistent design pattern emerges: WAMs are not simply video generators with action heads, but predictive-action methods whose design choices trade representational richness against compute, memory, latency, and action-label cost. The field is moving toward methods that generate less of the future while preserving what control requires. The survey homepage is available at .

</details>


### [15] [Geometric Entropy: When Trajectory Diversity Helps and Hurts in Imitation Learning](https://arxiv.org/abs/2606.20871)
...
Importance: 普通相关 (58.21) - 未入选精读

TL;DR: 本文提出几何熵（H_G）这一任务无关的度量，用于量化演示轨迹的形状多样性。在多种模仿学习架构、模拟和真实机器人接触式操作任务中，发现成功率与几何熵之间存在倒U型关系：低多样性时增加多样性提升鲁棒性，但高多样性导致策略模糊性从而降低性能。最优熵随数据量增加、任务变易或先验增强而降低，对预训练VLA模型呈单调递减趋势。H_G可用于演示数据集的快速预训练审计。


<details>
  <summary>Details</summary>
Motivation: 模仿学习中演示数据的多样性对策略学习有显著影响，但缺乏一个任务无关的度量来量化轨迹形状多样性，且不清楚多样性何时有益、何时有害。

Method: 提出几何熵（Geometric Entropy, H_G），通过对齐目标帧归一化外在变化（如目标姿态、工作空间尺度），量化轨迹的内在形状多样性。在多种IL架构（BC、IBC、Diffusion Policy）和接触式操作任务上进行系统实验，分析H_G与成功率的关系。

Result: 发现成功率与H_G呈一致的倒U型关系；最优熵随数据量增加、任务变易或先验增强而降低；预训练VLA模型（如π0）呈现单调递减趋势。H_G可有效预测演示数据集的可学习性。

Conclusion: 几何熵是评估演示数据集质量的有效工具，为校准演示数据提供了简单指导原则，有助于将数据调整到可学习区间。

Classification reason: 该论文研究模仿学习中轨迹多样性对性能的影响，涉及机器人操作任务（接触式操作），属于具身智能/机器人学习范畴，与"具身世界模型与规划控制"方向高度相关。

Importance reason: Introduces a novel task-agnostic metric for imitation learning, but focuses on robotics manipulation, not the user's priority subtopics. Signals: Geometric entropy, trajectory diversity, imitation learning, inverted-U relationship, robotics manipulation
...
### [16] [Vesta: A Generalist Embodied Reasoning Model](https://arxiv.org/abs/2606.20905)
...
TL;DR: Vesta是一个统一的具身通用基础模型，将定位、空间推理、导航和长程规划整合到单一模型中，在多个基准上平均超越专用SOTA模型20%以上，在真实机器人任务中成功率提升35%以上，证明了通用模型可以替代专用模型组合。


<details>
  <summary>Details</summary>
Motivation: 机器人在开放世界环境中需要同时具备定位、空间推理、导航和长程规划等多种能力。现有方法通常为每个任务训练专用模型，但多模型堆叠计算开销大且容易产生级联错误。作者希望构建一个统一的具身通用模型来替代多模型组合方案。

Method: Vesta采用单一基础模型架构，结合大规模精心策划的训练语料库（用于诱导空间接地能力）和简单的多模态记忆机制（支持长程推理）。模型统一处理定位、空间推理、导航和长程规划等任务，无需为不同任务切换不同模型。

Result: 在多个多样化基准上，Vesta平均超越单个SOTA基线20%以上，超越每类最佳基线的集成模型10%以上。在需要记忆和推理的真实机器人任务中，Vesta将任务成功率提升35%以上。

Conclusion: Vesta证明了单一通用模型在具身智能领域是可行、可扩展且优于专用模型组合的替代方案。通用模型可以匹配甚至超越专用模型的表现，同时避免了多模型堆叠的计算开销和级联错误问题。

Classification reason: 该论文提出Vesta，一个统一的具身通用模型，整合了定位、空间推理、导航和长程规划等能力，属于机器人/具身智能领域的世界模型。主要匹配world_model方向下的[CANONICAL]具身世界模型与规划控制（embodied_world_model）。同时，Vesta作为统一多模态模型也涉及理解与生成，因此也匹配multimodal_understanding_generation方向下的unified_multimodal_model（统一多模态模型）。

Importance reason: Strong world model and unified multimodal reasoning, but lacks direct focus on continuous language/multimodal models or video generation. Signals: embodied generalist, world model, unified multimodal, spatial reasoning, long-horizon planning
...
### [17] [ReFPO: Reflow Regularization for Flow Matching Policy Gradients](https://arxiv.org/abs/2606.21086)
...
TL;DR: ReFPO 提出了一种基于 Reflow 正则化的流匹配策略梯度在线强化学习方法，通过显式几何正则化（仅需一行代码修改）稳定 PPO 训练、减少 CFM 代理比尖峰，并实现高质量一步推理，在 GridWorld、MuJoCo 和 Humanoid 控制任务上提升了平均性能和离散化鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配策略梯度（FPO）方法在训练中存在 CFM 代理比尖峰、PPO 训练不稳定等问题，且多步推理效率低。作者希望利用 Reflow 正则化来稳定训练并实现高效的一步推理。

Method: ReFPO 在 FPO 基础上引入显式的 Reflow 几何正则化项，该正则化可以仅通过一行代码修改实现，无需额外计算开销或辅助蒸馏阶段。该方法将 FPO 的梯度更新解释为隐式的优势加权 Reflow 过程，通过优势引导的更新与路径矫正协同作用，稳定训练并提升一步推理质量。

Result: 在 GridWorld、MuJoCo Playground 和高维 Humanoid Control 任务上，ReFPO 相比基线方法在平均性能和离散化鲁棒性上均有提升，一步推理质量接近或超过多步推理。

Conclusion: ReFPO 提供了一种简单、可扩展且稳定的生成式策略在线 RL 方法，适用于复杂物理仿真中的机器人控制任务。

Classification reason: 该论文提出基于流匹配策略梯度（FPO）的在线强化学习方法，用于物理仿真中的机器人控制任务（GridWorld、MuJoCo、Humanoid），属于具身智能中的策略学习与规划控制，因此映射到[CANONICAL]具身世界模型与规划控制。

Importance reason: Focuses on robot control and RL, not aligned with user's priority on continuous multimodal models, video/image generation, or efficient inference. Signals: robotics, flow matching, reinforcement learning, policy gradients, reflow regularization
...
### [18] [Social World Model for Lifelong Social Intelligence](https://arxiv.org/abs/2606.21315)
*Yu Luo*
...
TL;DR: 本文提出 Social World Model，将社交交互分解为五个维度构建闭环学习框架，使语言智能体通过交互经验持续积累社交能力。在 ASCENT-Bench 上，Qwen2.5-7B 经交互训练后五项核心指标均超越基线，在完成率上匹配 Gemini 3 Flash，通过率超越之，且实现零遗忘。


<details>
  <summary>Details</summary>
Motivation: 当前社交智能研究主要关注静态能力评估，而非能力的持续塑造与积累。现有方法存在两个痛点：社交交互轨迹缺乏统一结构化表示以形成可迭代的学习信号；能力提升与保持通常被孤立研究，难以评估持续演化。因此需要一种可持续学习范式。

Method: 将社交交互分解为场景设置、观察、心理状态、动作和对话五个维度，构建闭环学习框架。智能体收集交互经验，将其转化为偏好信号用于模型更新，再将更新后的策略重新部署进行持续学习。同时提供可复用的数据合成机制和终身学习基准。

Result: 在 ASCENT-Bench 上，交互训练的 Qwen2.5-7B 在五项核心指标上均超越基线。在完成率上匹配闭源 Gemini 3 Flash，通过率超越之，且在三个难度级别上实现零遗忘。

Conclusion: 本文提供了一种可训练、可验证、可保持的端到端路径，证明小型开源模型可以通过持续交互可持续地获得具有竞争力的社交协调能力，将社交能力从"评估对象"转变为"可持续训练对象"。

Classification reason: 该论文提出 Social World Model，将社交交互分解为场景、观察、心理状态、动作和对话五个维度，构建闭环学习框架，使智能体通过交互经验持续学习社交能力。这属于世界模型在社交智能领域的应用，与"具身世界模型与规划控制"（embodied_world_model）方向高度相关，因为该方向涵盖面向智能体动作规划、策略学习和持续交互的世界模型。虽然论文聚焦社交而非物理具身，但其核心范式（交互→偏好信号→策略更新→持续学习）完全符合世界模型驱动的持续学习框架，因此复用该 canonical 子主题。

Importance reason: Proposes a social world model for lifelong learning, but is not aligned with user's priority subtopics on continuous multimodal models or video generation. Signals: world model, lifelong learning, social intelligence, language agents, zero forgetting
...
### [19] [PoLAR: Factorizing Extent and Mode in Latent Actions for Robot Policy Learning](https://arxiv.org/abs/2606.21139)
...
TL;DR: PoLAR 提出了一种具有径向结构的潜动作表示方法，通过将潜动作空间分解为半径（编码转移幅度）和方向（编码转移模式），并在双曲空间中实例化，提升了机器人策略学习的下游性能。


<details>
  <summary>Details</summary>
Motivation: 现有潜动作预训练方法将观察对之间的转移编码为单一无结构表示，混淆了转移幅度（extent）和转移模式（mode），导致下游策略学习效果受限。作者希望解耦这两个因素，提升潜动作表示的表达能力和可迁移性。

Method: PoLAR 在潜动作空间中引入径向-方向结构：半径编码转移幅度，方向编码转移模式。利用时间偏移作为转移幅度的弱代理信号，鼓励时间间隔大的观察对对应更大的半径。该结构在双曲空间中实例化，利用其随半径扩张的体积特性容纳更多样化的转移模式。训练时通过对比学习和半径约束联合优化。

Result: 在任务内和大规模预训练设置下，PoLAR 在仿真和真实机器人实验中均优于现有潜动作基线方法和强预训练 VLA 方法，验证了潜动作空间几何结构对策略学习的重要性。

Conclusion: 潜动作空间的几何结构是视觉预训练迁移到下游机器人策略学习的重要设计选择，PoLAR 的径向分解策略有效提升了表示质量和策略性能。

Classification reason: 该论文提出 PoLAR，一种在潜动作空间中引入径向结构以解耦转移幅度和转移模式的表示学习方法，用于机器人策略学习。这属于具身智能中世界模型的潜动作预训练与策略学习，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics paper but only tangentially touches world models, lacking direct connection to user's priority subtopics like continuous language models or video generation. Signals: latent action representation, radial structure in hyperbolic space, robot policy learning, world model adjacent, no direct video or language generation
...
### [20] [OmniV2X: A Generative Foundation Planner for Efficient End-to-End Cooperative Driving](https://arxiv.org/abs/2606.21165)
...
TL;DR: OmniV2X 是一个面向车联网（V2X）协同驾驶的生成式基础规划模型，通过序列建模和交叉注意力机制隐式学习多模态多智能体观测，以轨迹生成损失进行端到端监督训练。该模型在DAIR-V2X-Seq数据集上以不到10%的微调数据和不到1%的通信带宽达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有V2X协同驾驶方法存在三个主要问题：密集3D感知计算成本高、协同场景数据稀缺导致泛化性差、多模态融合为共享表征时难以兼容标准化消息协议。作者希望设计一个高效、轻量且兼容标准协议的生成式基础规划模型来解决这些问题。

Method: OmniV2X 将多模态多智能体观测建模为独立上下文序列，通过交叉注意力注入机制让生成式序列规划器隐式学习利用多模态输入。采用端到端监督训练，以下游轨迹生成损失作为优化目标。作为基础模型，先在单智能体规划数据集上预训练，再通过轻量级、兼容标准协议的V2X token 高效适配到协同环境。

Result: 在DAIR-V2X-Seq数据集上，OmniV2X 超越现有端到端协同驾驶基线方法，达到SOTA性能。仅需不到10%的微调V2X数据集和不到1%的通信带宽即可实现优异效果，并在真实约束下展现出计算效率和鲁棒性。

Conclusion: OmniV2X 验证了生成式基础模型在V2X协同驾驶中的有效性，通过序列建模和隐式学习避免了传统密集感知的高成本，同时以轻量级token实现高效协同适配，为自动驾驶协同规划提供了新范式。

Classification reason: 该论文提出面向V2X协同驾驶的生成式基础规划器，核心是端到端轨迹生成与多智能体协同规划，属于具身智能中的规划控制任务，因此映射到[具身世界模型与规划控制]。

Importance reason: Strong in world model direction but focuses on autonomous driving, not the user's priority subtopics like continuous language models or video generation. Signals: V2X cooperative driving, generative foundation planner, trajectory generation loss, cross-attention injection, DAIR-V2X-Seq dataset
...
### [21] [FOCA: Future-Oriented Conditioning for Data-Efficient Vision-Language-Action Adaptation](https://arxiv.org/abs/2606.20867)
...
TL;DR: FOCA提出一种未来导向条件框架，通过显式预测任务相关的未来交互嵌入和隐式对齐未来目标观测，实现数据高效的VLA模型少样本适应，在LIBERO、RoboCasa和真实机器人上取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在少样本场景下性能急剧下降，缺乏数据高效的适应策略。作者希望通过引入未来导向的推理机制，使VLA模型在少量演示下仍能有效学习长程任务。

Method: FOCA结合两种未来条件：1) 显式预测任务相关的未来交互嵌入（future interaction embeddings）；2) 隐式对齐未来目标观测（future goal observations）。该方法支持利用视频世界模型生成的合成视频进行动作无关联合训练，可解释为学习一种未来条件的类价值函数表示。

Result: 在LIBERO上20个演示达到95.7%成功率，在RoboCasa上提升7-12%，在真实机器人上实现高达26%的绝对性能提升。

Conclusion: FOCA通过未来导向条件框架有效解决了VLA模型在少样本场景下的适应瓶颈，验证了结合视频世界模型进行动作无关预训练的可行性，为数据高效的机器人学习提供了新范式。

Classification reason: 该论文研究VLA模型在少样本模仿学习中的适应性问题，提出未来导向条件框架FOCA，结合视频世界模型进行动作无关联合训练，属于具身智能中机器人动作规划与控制范畴，因此映射到[具身世界模型与规划控制]。

Importance reason: FOCA's future-conditioned VLA adaptation with world model co-training directly advances the user's priority on world models and efficient multimodal adaptation. Signals: world model, few-shot adaptation, vision-language-action, future-conditioned representation, video world model co-training
...
### [22] [Horizon Adaptive Offline Policy Learning via Value Stitching](https://arxiv.org/abs/2606.21136)
...
TL;DR: 本文提出VAST（Horizon Adaptive Offline Policy Learning via Value Stitching），通过递归的、自适应地平线的值函数组合替代固定地平线备份，结合未来状态和地平线长度条件的辅助值函数与拼接策略，实现变长回报的准确估计与组合，在OGBench的50个任务上超越固定步长、扩展地平线和生成式值函数离线RL基线。


<details>
  <summary>Details</summary>
Motivation: 传统TD学习存在随地平线累积的值估计偏差，而n步TD备份和Q-chunking等扩展地平线方法采用固定地平线建模，难以灵活捕捉长时域多阶段任务中复杂的值函数结构。

Method: VAST将固定地平线备份替换为递归的、自适应地平线的值组合。核心包括：(1) 一个未来状态和地平线长度条件的辅助值函数，通过直接数据监督学习；(2) 一个拼接策略，最优选择奖励最大化的地平线长度和未来子目标，实现地平线自适应值拼接。

Result: 在OGBench的50个任务上，VAST优于固定步长方法、扩展地平线方法和生成式值函数离线RL基线，在高复杂度长时域决策任务中表现尤为突出。

Conclusion: 自适应地平线的值函数组合能够提供更准确且可贪婪利用的值监督信号，有效提升离线策略学习在长时域复杂任务上的性能。

Classification reason: 该论文提出VAST方法，通过自适应地平线值函数拼接进行离线策略学习，面向长时域复杂任务决策，属于机器人/具身智能中的规划与控制范畴，因此映射到[具身世界模型与规划控制]这一规范子主题。

Importance reason: Strong offline RL method for long-horizon tasks, but outside user's core multimodal and generation priorities. Signals: offline reinforcement learning, long-horizon value estimation, horizon-adaptive composition, OGBench benchmark, robotics and control
...
### [23] [Inverting the Bellman Equation: From $Q$-Values to World Models](https://arxiv.org/abs/2606.21173)
...
TL;DR: 本文证明基于价值的强化学习智能体在足够丰富的奖励函数（如目标条件RL）上训练时，隐式编码了准确的环境动力学模型。作者提出P-learning算法，从Q值、策略和奖励中逆向解码世界模型，并在Reacher、MountainCar等环境中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统上，基于模型和无模型的强化学习被视为两个独立范式。本文试图挑战这一二分法，证明无模型的价值函数方法实际上可以隐式编码世界模型，从而弥合两者之间的鸿沟。

Method: 提出P-learning算法，作为Q-learning的逆模拟。通过从智能体的Q值、策略和奖励函数中采样，逆向解码环境转移核P。给出了在有限/连续状态空间、随机/确定性MDP中，智能体编码真实转移核的充分条件。

Result: 在Reacher、MountainCar和随机FourRooms环境中，即使假设条件被违反，仅用少量奖励函数训练的智能体也能编码准确的动力学。在Reacher中，基于隐式世界模型训练的策略在速度目标的OOD泛化上表现准最优，尽管训练时仅使用位置目标。

Conclusion: 价值函数隐式编码了环境模型，P-learning可以提取该模型。这揭示了基于模型、无模型和目标条件RL之间的深层联系，并表明智能体具有隐藏的泛化能力。

Classification reason: 该论文提出从Q值中逆向解码环境动力学（世界模型），属于世界模型方向。虽然不直接涉及具身机器人，但其方法适用于任何MDP环境建模，与具身世界模型/规划控制方向紧密相关，且[CANONICAL] embodied_world_model 是世界模型方向下最广泛的子主题，可覆盖该工作。

Importance reason: Bridges model-free and model-based RL with theoretical proof, but focuses on RL world models rather than generative video world models. Signals: world model from Q-values, P-learning algorithm, goal-conditioned RL, implicit dynamics encoding, out-of-distribution generalization
...
### [24] [Robot Self-Improvement via Human-Video Dynamics Models](https://arxiv.org/abs/2606.21406)
...
TL;DR: 本文提出将视觉观测通过ASCII艺术渲染为文本输入，使纯文本LLM能够充当VLA（视觉-语言-动作）控制器，在2D操作任务中实现指令跟随与动作规划，无需多模态骨干网络。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA控制器通常依赖多模态视觉-语言模型，需要大量数据和计算资源。作者希望探索一种轻量级、可解释的替代方案，利用纯文本LLM和ASCII渲染作为视觉到文本的模态桥梁，降低VLA控制器的门槛。

Method: 将视觉观测（图像）通过ASCII艺术渲染为文本表示，输入到纯文本LLM中；LLM输出约束化的可执行动作序列。使用基于规划的专家演示和DAgger迭代改进进行微调，在2D操作基准上比较多种LLM和VLM模型。

Result: 在2D操作基准的仿真和物理机器人实验中，ASCII渲染的LLM控制器能够识别任务相关实体并规划可行的动作序列，性能可与基于VLM的VLA控制器相媲美。

Conclusion: ASCII渲染可以作为从图像到文本的轻量级、可解释的模态桥梁，补充传统的VLA管线，为使用纯文本骨干进行VLA研究开辟了新方向。

Classification reason: 该论文将文本LLM通过ASCII渲染视觉观测转化为VLA控制器，在2D操作任务中实现动作规划与控制，属于具身智能/机器人领域的动作条件控制，因此映射到[具身世界模型与规划控制]（CANONICAL）。同时，该方法涉及将视觉信息转化为文本供LLM理解，也部分关联到多模态理解方向。

Importance reason: Novel ASCII-as-vision interface for LLM-based VLA control, but limited to 2D manipulation and not aligned with user's priority subtopics. Signals: ASCII rendering for VLA, text-only LLM controller, lightweight modality bridge, 2D manipulation benchmark, DAgger fine-tuning
...
TL;DR: 本文提出 w²VLA，一种模块化的视觉-语言-动作模型，通过解耦声明性知识（概念与实体语义）和程序性知识（操作方式），实现对新物体的零样本技能迁移，克服了现有 VLA 模型在空间、语义和任务变化下的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型将声明性知识（物体概念与语义）和程序性知识（如何操作）耦合在模型参数中，导致对未见物体的零样本技能迁移能力差，数据收集成本高。作者希望解耦这两种知识，实现更鲁棒的行为克隆。

Method: 提出 w²VLA，采用模块化信息流设计：不将所有多模态 token 直接输入到大型黑箱动作专家中，而是以组合式、可解释的方式，用视觉、空间和技能信息调制机器人状态序列，从而解耦声明性与程序性知识表征。

Result: 实验表明，w²VLA 在分布内操作任务上达到 SOTA 性能，同时在面对不同且未见过的物体时，展现出前所未有的零样本技能迁移能力，显著优于现有 VLA 模型。

Conclusion: 模块化解耦声明性与程序性知识是提升 VLA 模型泛化能力的关键方向，w²VLA 验证了该思路在零样本技能迁移上的有效性，为通用机器人智能体的部署提供了新范式。

Classification reason: 该论文提出 w²VLA，一种面向机器人操作的行为克隆模型，属于具身智能中的视觉-语言-动作（VLA）模型，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Decouples declarative and procedural knowledge for zero-shot skill transfer in VLA, but is in robotics, not the user's priority subtopics. Signals: zero-shot skill transfer, modular VLA, world model, robotics, not in priority subtopics
...
### [27] [UniviewVLA: A Unified Multiview Vision-Language-Action Model with World Modeling](https://arxiv.org/abs/2606.21501)
...
TL;DR: UniviewVLA 是一个统一多视图视觉-语言-动作模型，通过世界模型生成多视图未来场景来揭示遮挡信息并建模场景演化，从而提升机器人操作中的动作预测能力。它提出了运动信息令牌压缩（将每视图从625压缩到16个token，延迟从6-7秒降至0.2-0.3秒）和无训练的动作熵视图选择机制，在标准无遮挡基准上达到95.8%（LIBERO）和4.60（CALVIN），在定制遮挡任务上将成功率从40.0%提升至73.3%。


<details>
  <summary>Details</summary>
Motivation: 机器人操作中的遮挡任务一直是瓶颈。现有方案要么增加物理相机（需要训练-推理相机一致），要么依赖显式3D重建（计算成本高），且都未能捕捉遮挡信息和未来场景演化。因此需要一种能从标准双相机观测中推断多视图场景演化以辅助动作预测的方法。

Method: UniviewVLA 包含三个核心组件：(1) 世界模型模块，从标准双相机观测生成多视图未来场景，揭示遮挡信息并建模场景演化；(2) 运动信息令牌压缩（Motion-Informative Token Compression），将每个生成视图从625个token压缩至16个，大幅降低延迟；(3) 无训练的动作熵视图选择（Action-Entropy View Selection），动态识别不同推理阶段对动作预测最有信息量的视图。

Result: 在标准无遮挡基准上，LIBERO 达到95.8%，CALVIN ABCD→D 达到4.60。在定制遮挡任务上，成功率从40.0%提升至73.3%；真实机器人平均成功率提升33.4个百分点。每视图推理延迟从6-7秒降至0.2-0.3秒。

Conclusion: UniviewVLA 通过世界模型生成多视图未来场景，有效解决了机器人操作中的遮挡问题，无需额外硬件或显式3D重建，同时在标准无遮挡基准上保持优异性能，展示了世界模型在具身智能中的巨大潜力。

Classification reason: 该论文提出 UniviewVLA，一个统一多视图视觉-语言-动作模型，利用世界模型生成多视图未来场景来辅助机器人操作中的遮挡任务和动作预测。这直接属于世界模型方向下的具身世界模型与规划控制（embodied_world_model）这一规范子主题。

Importance reason: Strongly aligns with world model and continuous multimodal priorities, with efficient token compression and action-driven view selection. Signals: world model for occlusion reasoning, motion-informative token compression (625→16 tokens), training-free action-entropy view selection, strong occlusion benchmark gains (40%→73.3%), real-robot success rate +33.4 points
...
### [28] [VQActFlow: Vector-Quantized Action Mode Steering for Multi-Task Robot Manipulation](https://arxiv.org/abs/2606.21600)
...
TL;DR: VQActFlow 提出一种基于向量量化动作 token 与变分流匹配的多任务机器人操作策略，通过离散码本分离动作模式，并利用推理时引导（语言条件无分类器引导 + 码本批评器）来提升模式选择的准确性与可行性，在 LIBERO、Unitree G1 人形机器人和 ALOHA 双臂平台上均优于连续和离散基线。


<details>
  <summary>Details</summary>
Motivation: 多任务机器人操作策略需要从多模态演示分布中学习，传统单一网络难以正确选择动作模式，导致执行错误任务或不可行动作。现有方法缺乏对动作模式的显式偏好控制。

Method: 将连续动作 chunk 通过向量量化 token 化为离散码本，利用变分流匹配（Variational Flow Matching）生成动作码序列。引入推理时引导机制：语言条件无分类器引导（CFG）将策略导向指令对应的动作模式，以及一个学习的码本批评器（codebook critic）提供可行性信号来辅助模式选择。

Result: 在 LIBERO 仿真基准、Unitree G1 人形机器人全身抓取放置任务和 ALOHA 双臂接触丰富任务上，VQActFlow 在成功率和任务完成质量上均优于连续策略和离散基线方法。

Conclusion: 离散动作 token 化结合流匹配生成与推理时引导，能有效分离多任务操作中的动作模式，提升策略在多模态演示分布下的选择准确性和执行可行性。

Classification reason: 该论文提出 VQActFlow，一种面向多任务机器人操作策略的方法，将连续动作 token 化为离散码本并通过变分流匹配生成动作序列，涉及具身智能中的动作规划与控制。这与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Applies vector-quantized action tokens and flow matching to robot manipulation, which is tangentially related to continuous language/multimodal models but not directly advancing those core priorities. Signals: Robotics manipulation, vector-quantized action tokens, variational flow matching, multi-task policy, inference-time guidance
...
### [29] [Nous: A Predictive World Model for Long-Term Agent Memory](https://arxiv.org/abs/2606.22030)
*Pranav Singh*
...
TL;DR: Nous 是一种基于预测世界模型的智能体长期记忆架构，将知识存储为实体-属性对的概率分布（维度），通过信息论惊喜度评分和贝叶斯后验更新来维护记忆，遗忘自然通过熵衰减实现。在 LoCoMo 基准上使用 GPT-4o-mini 作为骨干，F1 分数在单跳、多跳、时序和开放域任务上分别达到 63.50、55.32、58.57 和 62.50，无需外部向量数据库或图引擎。


<details>
  <summary>Details</summary>
Motivation: 现有智能体记忆系统（如向量数据库、知识图谱）将知识作为静态事实存储，缺乏对不确定性的建模和自然的遗忘机制。作者认为知识本质上是预测而非存储，因此需要一种能够表达信念不确定性、支持自然遗忘和身份消歧的记忆架构。

Method: Nous 维护一个预测世界模型，由一组分类概率分布（称为"维度"）组成，每个维度对应一个实体-属性对。每个新观测通过信息论惊喜度 S = -log2 P(obs | D) 评分，然后通过闭式贝叶斯后验更新分布。存储的主要工件是 delta（先验到后验信念的偏移记录），而非事实本身。遗忘通过向均匀分布的熵衰减自然发生，身份消歧通过实体维度集之间的互信息处理。

Result: 在 LoCoMo 长期对话记忆基准的 10 段对话（1,540 个问题）上，使用 GPT-4o-mini 作为骨干，Nous 在单跳任务上 F1 为 63.50，多跳 55.32，时序 58.57，开放域 62.50。与 A-MEM 和 BeliefMem 的对比显示在多数类别上有显著提升，但作者坦诚讨论了评估流程中的可重复性问题。

Conclusion: Nous 证明了基于预测世界模型的记忆架构可以有效替代传统的向量数据库或知识图谱方法，在长期对话记忆任务上取得有竞争力的结果。其核心优势在于自然的遗忘机制、不确定性建模和无需外部存储引擎的简洁设计。

Classification reason: 论文提出一种基于预测世界模型的智能体记忆架构，将知识建模为实体-属性对的概率分布（世界模型），通过信息论惊喜度评分和贝叶斯更新来维护长期记忆。这与世界模型方向中的"具身世界模型与规划控制"（embodied_world_model）相关，因为该论文的核心是构建一个预测性世界模型用于智能体记忆，属于世界模型在智能体/记忆场景中的应用。虽然更偏向记忆而非传统机器人规划，但世界模型作为核心概念与 canonical 主题匹配。

Importance reason: Novel world-model memory approach but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: world model for agent memory, Bayesian surprise for forgetting, no external vector DB, LoCoMo benchmark gains, reproducibility issues noted
...
### [30] [Energy-based Compositional Diffusion Planning](https://arxiv.org/abs/2606.21646)
...
Importance: 值得关注 (75.76) - 未入选精读

TL;DR: 本文提出 Energy-based Compositional Diffuser (ECD)，将组合扩散规划中的全局轨迹建模为局部桥势能之和的最小化问题，通过能量视角定义保守校正场和边界反应项，并引入基于马尔可夫的分数近似实现高效推理，在 OGBench 任务上达到 SOTA 成功率且推理速度接近启发式拼接方法。


<details>
  <summary>Details</summary>
Motivation: 现有组合扩散规划器依赖启发式拼接局部预测来解决长程机器人任务，但拼接更新通常产生非保守场，数学上不对应任何有效的全局轨迹对数密度函数，缺乏理论保证。

Method: 提出 ECD 框架，将全局轨迹定义为局部桥势能之和的最小化器，引入保守校正场和边界反应项；通过马尔可夫分数近似将反应项计算转化为单次块三对角求解，保持时间复杂度与规划步长线性关系。

Result: 在 OGBench 的多种拼接任务上取得 SOTA 成功率，同时推理速度接近启发式拼接方法。

Conclusion: ECD 从能量视角为组合扩散规划提供了理论严谨的框架，在保持高效推理的同时显著提升了任务成功率，验证了能量基方法在长程机器人规划中的有效性。

Classification reason: 该论文提出基于能量的组合扩散规划器（ECD），用于解决长程机器人任务的组合规划问题，属于机器人动作规划与控制范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics planning method but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: world model, diffusion planning, energy-based model, robotic task stitching, efficient inference

Abstract: Compositional diffusion planners aim to solve long-horizon robotic tasks using short training trajectories. Yet, current approaches often rely on the heuristic stitching of local predictions. We show that the resulting stitched update is generally a non-conservative field} that does not mathematically correspond to any valid global trajectory log-density function. We propose Energy-based Compositional Diffuser (ECD), a framework that formulates the global trajectory as the minimizer of the sum of local bridge potentials. This energy-based perspective defines a conservative correction field and contains a boundary reaction term that heuristic stitching omits. To enable efficient inference, we further introduce a Markov-based score approximation that computes the reaction term via a single block-tridiagonal solve, maintaining time complexity linear in the planning horizon. Empirically, ECD achieves state-of-the-art success rates on a range of OGBench stitching tasks, while nearly matching the inference speed of heuristic stitching methods. Code is available at .

</details>


### [31] [Imitation from Heterogeneous Demonstrations using Grounded Latent-Action World Models](https://arxiv.org/abs/2606.21672)
...
TL;DR: 本文提出GLAM，一种基于接地潜动作空间的世界模型，用于从异构演示数据（不同动作空间、甚至无动作标签）中进行模仿学习。GLAM通过生成模型对共享潜动作空间进行对齐，使产生相同环境效果的动作获得相同表征，从而支持跨数据源的动作迁移。在5个操作任务上，GLAM策略相比基线方法平均提升48%的成功率。


<details>
  <summary>Details</summary>
Motivation: 模仿学习受限于演示数据的规模和质量，而现实中存在大量异构数据源（不同动作空间、缺少动作标签），但现有方法依赖启发式的手工对齐技术。作者认为动作表征应基于预测进行接地——产生相同环境效果的动作应共享相同表征，无论其来源如何。

Method: 提出GLAM（Grounded Latent-Action World Model），包含一对生成模型（前向动力学模型和逆动力学模型），通过共享潜动作空间跨数据源对齐。该潜动作空间通过一致地预测未来观测来接地。训练后，使用行为克隆（BC）策略将观测映射到潜动作，再解码为机器人动作，实现从异构数据中学习。

Result: 在5个模拟和真实世界的操作任务上，GLAM对齐的策略显著优于BC基线和先前的潜动作方法，在数据稀缺场景下平均任务成功率提升48%。

Conclusion: GLAM成功学习了对齐的潜动作空间，促进了跨数据源（有/无动作标签）的动作迁移，为从异构演示数据中进行模仿学习提供了一种有效范式。

Classification reason: 该论文提出GLAM（grounded latent-action world model），利用共享潜动作空间的世界模型从异构演示数据中学习模仿策略，并在机器人操作任务中验证。这直接属于世界模型方向下的"具身世界模型与规划控制"（embodied_world_model）这一规范子主题。

Importance reason: Proposes grounded latent-action world models for imitation from heterogeneous demonstrations, relevant to world model and efficient learning but not directly aligned with user's priority subtopics. Signals: world model, latent action space, imitation learning, heterogeneous demonstrations, robotics manipulation

Abstract: Imitation learning has emerged as a powerful paradigm for learning visuomotor policies, but its generalisation and stability are limited by the scale and quality of demonstration data needed. A promising direction is to leverage more abundant but heterogeneous data sources, which differ in action space and often lack action labels altogether. Existing co-training approaches that combine heterogeneous data sources rely on heuristic and hand-engineered alignment techniques. In contrast, we argue that action representations should be grounded in prediction: actions that produce the same effect on the environment should share the same representation, regardless of their sources. To this end, we instantiate this principle by using a grounded latent-action world model (GLAM), a pair of generative models with a shared latent action space across data sources that is grounded by predicting future observations consistently across sources. This latent action space is used to train downstream behavioural cloning (BC) policies which map observations to latent actions and decode them back to robot actions, providing a paradigm for learning from heterogeneous data. Empirically, we demonstrate that GLAM successfully learns an aligned latent action space that facilitates action transfer across data sources with and without action labels. Across five manipulation tasks in simulation and in the real world, GLAM-aligned policies significantly outperform BC baselines and prior latent-action methods, achieving an average of +48% improvement in task success rate with the same data-scarce setting. Videos and code are available at .

</details>


### [32] [THREAD: Trajectory Planning for Hybrid Rigid-Soft Manipulators with Environment-Aware Diffusion](https://arxiv.org/abs/2606.21792)
...
TL;DR: THREAD 是首个基于扩散的混合刚柔机械臂轨迹规划器，通过物理启发的损失函数联合优化刚性和柔性段的曲率、平滑度和碰撞约束，在仿真中达到92.4%任务成功率且碰撞减少5倍，并实现了跨实体真实世界迁移。


<details>
  <summary>Details</summary>
Motivation: 在狭窄受限环境（如穿过小孔径）中进行操作对传统刚性机器人极具挑战。混合刚柔机械臂虽有潜力，但面临两个规划难题：自由空间中可行的骨架形状在环境接触下变得不可行，且独立规划刚性和柔性段忽略了它们的运动学耦合。现有方法缺乏能够联合考虑这些因素的轨迹规划器。

Method: 提出 THREAD，一个基于扩散模型的轨迹规划器。它学习一个生成式先验，以局部环境几何为条件，生成物理上可实现的主干轨迹。采用物理启发的损失函数，联合编码曲率、平滑度和碰撞约束，同时作用于刚性和柔性两个段。在仿真环境中训练，并通过少量在线更新实现跨实体真实世界迁移。

Result: 在仿真中达到92.4%的任务成功率，碰撞次数比最强基线减少5倍。成功实现跨实体真实世界迁移，仅需最少的在线更新，能够穿过直径仅为柔性段直径1.3倍的孔径。

Conclusion: THREAD 证明了扩散模型作为混合刚柔机械臂轨迹规划器的有效性，能够联合考虑刚柔耦合和环境约束，在狭窄空间操作中显著优于基线，并具备良好的跨实体迁移能力。

Classification reason: 该论文提出基于扩散的轨迹规划器THREAD，用于混合刚柔机械臂在受限环境中的操作规划，属于机器人具身智能中的规划与控制，与[具身世界模型与规划控制]高度匹配。

Importance reason: Strong diffusion-based planning for hybrid robots, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: Diffusion-based trajectory planner, Hybrid rigid-soft manipulators, Physics-inspired loss functions, Cross-embodiment real-world transfer, 92.4% task success with 5x fewer collisions
...
### [33] [VLA-FAIL: Efficient Task Failure Detection for Finetuned Vision-Language-Action Models](https://arxiv.org/abs/2606.21386)
...
TL;DR: VLA-FAIL 提出了一种轻量级、无需故障数据的运行时故障检测框架，结合最后一层马氏距离（LLMD）和动作块一致性（ACC）两种互补检测器，在多种机器人操作任务上实现了高效、早期的故障检测，并引入 AUCPDT 指标以联合评估检测精度与延迟。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作模型（VLA）在机器人操作中表现优异，但在分布外场景下可能行为不可预测。现有故障检测方法要么计算开销大（需要大量动作采样），要么依赖特定架构假设（不适用于VLA），要么需要故障数据。因此需要一种轻量、通用、无需故障数据的运行时故障检测方法，以确保VLA在真实世界中的安全部署。

Method: 提出 VLA-FAIL 框架，包含两个互补的故障检测器：(1) 最后一层马氏距离（LLMD），通过测量VLA最后一层特征中每个token相对于训练数据的马氏距离来检测分布外状态；(2) 动作块一致性（ACC），利用回退水平控制带来的时间重叠，当连续动作块不一致时检测故障。两者结合实现最小开销的故障检测。同时提出 AUCPDT 指标，联合评估精度、召回率和检测时间。

Result: 通过大量真实世界和仿真实验，LLMD 和 ACC 能够捕获互补的故障模式，两者组合在多种任务上实现了可靠且早期的故障检测，性能经常显著优于计算成本更高的基线方法。

Conclusion: VLA-FAIL 提供了一种轻量、通用、无需故障数据的故障检测方案，有效提升了VLA模型在真实世界部署中的安全性。LLMD 和 ACC 的互补性使得组合检测器能够在早期准确识别多种故障模式。

Classification reason: 该论文关注视觉-语言-动作模型（VLA）在机器人操作任务中的运行时故障检测，属于具身智能/机器人领域的安全与可靠性问题。虽然论文侧重故障检测而非世界模型本身，但VLA模型是具身世界模型与规划控制的重要组件，且论文涉及动作条件预测与状态分布检测，与"具身世界模型与规划控制"方向紧密相关，因此复用该[CANONICAL]子主题。

Importance reason: Focuses on failure detection for VLAs, which is tangentially related to world models but lacks direct connection to your priority subtopics like continuous language models or video generation. Signals: VLA failure detection, runtime safety, lightweight detector, no failure data needed, AUCPDT metric
...
### [34] [Self-Evolving Cognitive Framework via Causal World Modeling for Embodied Scientific Intelligence](https://arxiv.org/abs/2606.22449)
...
TL;DR: 本文提出一种自演化认知框架，将具身智能从预测性世界模型推向因果世界建模驱动的认知系统，通过因果发现、干预驱动推理和反事实推理实现内部因果表征的持续构建与精炼，并重新将具身交互解释为因果假设生成与知识获取的认识论过程。


<details>
  <summary>Details</summary>
Motivation: 现有具身世界模型主要优化预测目标，在分布偏移和未见情境下泛化能力有限，无法系统性地推理假设性干预。作者认为具身智能应超越预测性世界模型，转向能够通过与环境的交互持续构建和精炼内部因果表征的自演化认知系统。

Method: 提出一个三组件框架：(1) 因果世界建模——通过因果发现构建内部因果图；(2) 干预驱动因果推理——利用干预反馈和反事实推理进行假设检验；(3) 持续认知精炼——不断修正和扩展因果世界模型。将具身交互重新解释为认识论过程：因果假设生成→干预驱动实验→持续知识获取。

Result: 本文为概念性和理论性工作，提出了完整的框架定义、组件设计和评估范式（干预驱动的因果-认识论基准），未提供实验验证。

Conclusion: 该工作为从预测性智能向认识论智能的转变提供了概念和理论基础，认为智能通过与环境的持续交互不断构建、修正和精炼因果世界模型而涌现。建议采用干预驱动的因果-认识论基准范式来评估自演化具身科学智能。

Classification reason: 该论文提出面向具身科学智能的自演化认知框架，核心是因果世界建模、干预驱动因果推理与持续认知精炼，属于具身智能中的世界模型研究。与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Directly advances world model research with causal reasoning, a top-weighted direction, and aligns with continuous multimodal reasoning via causal representation. Signals: world model, causal reasoning, embodied intelligence, self-evolving cognitive framework, epistemic intelligence
...
### [35] [SCOPE: Evolving Symbolic World for Planning in Open-Ended Environments](https://arxiv.org/abs/2606.22488)
...
TL;DR: SCOPE提出一种自适应的符号规划框架，通过符号执行模拟器（SESim）和自适应符号记忆（SASMem）两个模块，在开放环境中不断修正动作计划并演化符号世界表示，显著提升了长程规划的成功率和跨任务适应性。


<details>
  <summary>Details</summary>
Motivation: 现有VLM与经典规划器结合的方法在开放环境中，从感知获得的符号表示往往不完整，导致规划性能次优。需要一种能够自适应地完善符号世界表示并修正规划的方法。

Method: SCOPE包含两个协同模块：(1) 符号执行模拟器（SESim），对动作计划进行符号验证和实际执行，利用反馈修正计划并演化符号世界；(2) 自适应符号记忆（SASMem），将反馈蒸馏为演化的符号知识，增强长程规划和对符号世界的建模能力。

Result: 在开放环境中的实验表明，SCOPE显著提升了符号世界的完整性、环境扰动下的计划成功率，以及跨任务的基础能力和适应性。

Conclusion: SCOPE通过自适应的符号世界演化机制，有效解决了开放环境中符号表示不完整的问题，为具身智能中的长程规划提供了一种可行的自适应框架。

Classification reason: 该论文研究具身智能中的长程规划，使用VLM与经典规划器结合，通过符号执行模拟器和自适应符号记忆来完善符号世界表示，属于具身世界模型与规划控制范畴，与[CANONICAL]具身世界模型与规划控制高度匹配。

Importance reason: Strong world model planning but lacks direct connection to continuous multimodal or autoregressive generation priorities. Signals: world model, symbolic planning, open-ended environments, VLM integration, long-horizon tasks
...
### [36] [Imagine to Ensure Safety in Hierarchical Reinforcement Learning](https://arxiv.org/abs/2606.22509)
...
TL;DR: 本文提出一种分层强化学习方法，结合可学习世界模型与高层/低层双策略，在长时域任务中实现安全探索。高层策略生成安全子目标，低层策略利用世界模型进行想象推演以减少不安全行为，在长时域导航和操作任务上显著优于现有安全RL基线。


<details>
  <summary>Details</summary>
Motivation: 强化学习中的安全探索问题在长时域任务中尤为突出，现有安全方法因累积估计误差和受限探索能力而面临根本性局限。需要一种能够在长时间跨度下同时保证任务成功率和安全约束的方法。

Method: 提出分层安全强化学习框架，包含：(1) 可学习的世界模型，用于模拟环境动力学；(2) 高层策略，生成中间子目标以引导探索偏向安全区域；(3) 低层策略，利用世界模型进行想象推演（imagined rollouts）来减少到达子目标过程中的不安全行为。两层策略共同在分层级别上促进安全性。

Result: 在长时域导航和操作任务（高维动作空间）上评估，该方法在成功率和约束满足方面显著优于现有Safe RL基线，能够一致地满足预设的安全预算，而先前方法无法有效解决这些复杂的长时域场景。

Conclusion: 结合可学习世界模型与分层策略是解决长时域安全探索问题的有效途径，高层子目标引导与低层想象推演的双重安全机制能够克服传统方法的累积误差和探索限制。

Classification reason: 该论文提出结合可学习世界模型与双层策略（高层策略生成子目标、低层策略利用世界模型想象展开）来保障安全探索，属于面向机器人/具身智能中规划与控制的世界模型方法，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong safe RL method with world model, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: hierarchical RL, world model for safety, long-horizon tasks, safe exploration, high-level subgoal planning
...
### [37] [Physics-Informed Eikonal Caging for Whole-Arm Manipulation Planning](https://arxiv.org/abs/2606.22143)
*Yan Zhang, Yiming Li, Yifei Dong, Florian T. Pokorny, Sylvain Calinon*

Main category: cs.RO, cs.LG, cs.GT
...
TL;DR: 本文提出一种物理信息Eikonal Caging方法，将caging重新表述为最小时间逃逸问题，通过PINN近似逃逸时间场，并将其嵌入全身操作规划中，提升对接触模型误差的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 全身操作规划中，机器人肢体与物体的复杂接触动力学难以精确建模，传统方法对接触模型误差敏感。需要一种不依赖精确接触模型的规划原则来提升鲁棒性。

Method: 将caging重新定义为物体逃离机器人包围几何的最小时间逃逸问题，推导出逃逸时间场满足eikonal方程，使用物理信息神经网络（PINN）近似该场，得到光滑可微的表示，并将其作为目标函数嵌入全身操作规划中。

Result: 在仿真和真实实验中，该方法相比基线方法在抵抗扰动和接触模型失配方面表现出更好的鲁棒性，支持使用简化接触模型（如准动态近似和简化物体几何）进行规划。

Conclusion: 几何包围（caging）可以作为全身操作规划的一种实用鲁棒性原语，通过PINN求解eikonal方程得到的逃逸时间场能够有效提升操作对接触模型误差的鲁棒性。

Classification reason: 该论文研究基于caging的全身操作规划，使用物理信息神经网络求解eikonal方程来度量机器人对物体的包围质量，并将其嵌入操作规划中以提升鲁棒性。这属于面向机器人操作规划与控制的世界模型方法，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Strong robotics manipulation planning paper but only tangentially touches world models and lacks direct connection to user's priority subtopics. Signals: physics-informed neural network, eikonal caging, whole-arm manipulation, robustness to contact model mismatch, continuous escape-time field
...
### [38] [Backpropagating Through Simulation: Analytic Policy Gradients for Sample and Learning Efficient Differentiable Continuous Control](https://arxiv.org/abs/2606.21525)
*Yueci Deng*
...
TL;DR: 本文提出 Analytic Policy Gradients (APG)，利用可微环境动力学通过模拟反向传播计算精确策略梯度，替代传统无模型强化学习（如PPO）的高方差采样梯度估计。在四个连续控制任务上，APG在样本效率和梯度效率上均显著优于PPO，并提出了分段反向传播方案以缓解长时域任务中的梯度退化问题。


<details>
  <summary>Details</summary>
Motivation: 传统无模型强化学习算法（如PPO）将环境视为黑箱，需要数百万次交互采样来估计策略梯度，方差高、样本效率低。当环境动力学可微时，回报函数成为策略参数的可微函数，可以利用精确梯度计算来大幅提升样本效率。

Method: 提出 Analytic Policy Gradients (APG)，利用可微环境动力学，通过模拟轨迹反向传播计算精确策略梯度。设计了分段反向传播方案（segmented backpropagation），支持MC和critic-based两种bootstrap模式以缓解长时域梯度退化。在四个连续控制任务（1D点质量目标到达、2D点质量避障导航、2D刚体T-block推、7-DOF Franka FR3末端执行器到达）上与PPO进行对比，保持相同的模型架构、观测归一化和优化器设置。

Result: APG在样本效率（环境步数）和梯度效率（梯度步数）上均显著优于PPO。分段反向传播有效缓解了长时域任务中的梯度退化问题。消融实验验证了分段长度和bootstrap策略的影响。

Conclusion: 当环境动力学可微时，Analytic Policy Gradients 是一种比PPO更高效、更稳定的连续控制策略优化方法，为可微模拟器中的强化学习提供了新的范式。

Classification reason: 该论文提出 Analytic Policy Gradients (APG)，利用可微环境动力学通过模拟反向传播计算精确策略梯度，用于连续控制任务（包括7-DOF机械臂末端执行器任务）。这属于具身智能中利用可微模拟器进行策略学习与控制的范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on differentiable simulation for control, not on the user's priority subtopics like continuous language models or video generation. Signals: Differentiable simulation, Analytic policy gradients, Continuous control, Sample efficiency, Not aligned with user's priority subtopics
...
### [39] [FlowDPG: Deterministic Policy Gradient on Flow Matching Policies for Real-World Manipulation](https://arxiv.org/abs/2606.22303)
...
TL;DR: FlowDPG 提出一种专为流匹配策略设计的 DDPG 风格强化学习方法，通过将评论家梯度蒸馏到速度场中，避免了传统方法中沿 ODE 轨迹反向传播时间（BPTT）的计算瓶颈，并在真实世界的双机械臂 AirPods 装配任务上达到 92% 的成功率。


<details>
  <summary>Details</summary>
Motivation: 将策略梯度方法应用于流匹配策略时，需要沿多步 ODE 轨迹进行时间反向传播（BPTT），这在计算上极其昂贵且数值不稳定，严重限制了流匹配策略在真实世界机器人操作任务中的强化学习应用。

Method: FlowDPG 将 DDPG 风格的确定性策略梯度思想引入流匹配策略：在训练时将评论家（critic）梯度蒸馏到速度场中，从而完全绕过 BPTT。具体地，FlowDPG 结合了两个互补的向量——演示驱动的速度（保持动作可行性）和评论家驱动的修正（引导动作朝向更高价值），并通过三个显式近似建立了 FlowDPG 更新方向与原始确定性策略梯度之间的形式化联系。

Result: 在真实世界的长时域、多阶段、双机械臂 AirPods 装配任务上，FlowDPG 实现了 92% 的端到端成功率，显著优于包括价值条件化、辅助模块自适应和伴随方法评论家梯度在内的多种近期强化学习方法。

Conclusion: FlowDPG 提供了一种无需 BPTT 的稳定策略改进框架，使流匹配策略能够有效利用强化学习进行优化，为真实世界机器人操作任务中的流匹配策略学习开辟了新途径。

Classification reason: 该论文研究基于流匹配策略的机器人操作强化学习方法，在真实世界双机械臂装配任务上验证，属于具身智能与机器人规划控制范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics RL method but outside user's priority subtopics and negative keywords. Signals: flow matching policy, DDPG-style RL, BPTT-free distillation, real-world dual-arm assembly, world model connection weak

Abstract: Real-world reinforcement learning for robotic manipulation remains challenging, and this difficulty is amplified for flow matching policies: applying policy gradient methods to these policies is fundamentally limited by the need to backpropagate through time(BPTT) along the multi-step ODE that maps noise to actions, which is computationally prohibitive and numerically fragile. We propose FlowDPG, a DDPG-style method specifically designed for flow matching policies that distills the critic gradient into the velocity field at training time, bypassing BPTT entirely. Intuitively, FlowDPG combines two complementary vectors: the demonstration-driven velocity that keeps the action feasible, and the critic-driven correction that steers it toward higher value. Our contributions are threefold: (1) a BPTT-free distillation framework that enables stable DDPG-style policy improvement on flow matching policies, (2) a formal connection between the FlowDPG update direction and vanilla Deterministic Policy Gradient via three explicit approximations, and (3) real-world validation on a long-horizon, multi-stage, dual-arm AirPods assembly task, where FlowDPG attains a 92% end-to-end success rate, substantially outperforming recent RL methods spanning value-conditioning, auxiliary-module adaptation, and adjoint-based critic-gradient approaches. Videos and more results are provided on the project page .

</details>


### [40] [EmbodiedUS-FS: Fast Slow Intelligence for Ultrasound Robotics](https://arxiv.org/abs/2606.22319)
...
Importance: 普通相关 (48.21) - 未入选精读

TL;DR: 提出EmbodiedUS-FS，一个快慢分层具身超声机器人系统。Slow Brain负责意图解析与阶段式任务规划，Fast Brain融合多模态反馈进行局部动作优化，并集成安全护盾与层级升级策略，在动态临床环境中提升任务成功率并减少安全违规。


<details>
  <summary>Details</summary>
Motivation: 真实临床环境中的机器人超声扫描需要同时具备高层临床工作流推理和低层闭环执行能力，而医生的自然语言指令常包含隐式解剖目标、流程逻辑、图像质量要求和安全约束，执行过程还受患者运动、接触变化和目标漂移等动态因素影响。现有方法难以同时处理这些高层规划和低层执行挑战。

Method: 提出快慢分层架构：Slow Brain通过API和手册语料库进行知识增强的意图解析和阶段式任务规划，构建任务图并执行结构化计划验证；Fast Brain融合超声图像、机器人位姿/力状态和患者运动等多模态反馈，细化局部动作并执行图像质量引导的恢复行为；Safety Shield与层级升级策略约束危险动作，在持续失败或安全边界违规时触发重规划或人工确认。

Result: 在规划评估、动态扰动下的闭环执行以及安全机制验证实验中，所提出的分层设计在提高任务成功率的同时减少了安全违规次数。

Conclusion: 快慢分层具身架构能有效结合高层临床推理与低层闭环控制，提升超声机器人在动态临床环境中的安全性和任务完成能力。

Classification reason: 该论文提出面向超声机器人的快慢分层具身系统，包含高层任务规划（Slow Brain）和低层闭环执行（Fast Brain），属于机器人具身智能中的规划与控制，与[具身世界模型与规划控制]高度匹配。

Importance reason: The paper focuses on robotic ultrasound, which is outside the user's core priority subtopics and negative keywords, and lacks direct connection to continuous language models, video generation, or world models as defined. Signals: Robotic ultrasound, Fast-slow hierarchical system, Safety shield, Multimodal feedback, Task planning
...
### [41] [Benchmarking Robot Memory Under Interference](https://arxiv.org/abs/2606.22338)
*Soumil Rathi*
...
TL;DR: 本文提出 RoboMME-Interference 基准，用于评测机器人在跨会话干扰下的长上下文记忆能力。实验发现现有记忆增强的 VLA 模型在无干扰时表现良好，但随着无关会话累积，记忆性能持续显著下降，揭示了当前系统在长上下文记忆和抗干扰方面的严重不足。


<details>
  <summary>Details</summary>
Motivation: 现实部署的机器人需要在多个会话和任务中积累经验，并跨多个会话回忆信息。然而现有机器人记忆基准大多基于单轮或短上下文，无法评估长上下文和干扰下的记忆能力。因此需要一个新的基准来系统衡量这一问题。

Method: 基于 RoboMME 构建跨会话基准 RoboMME-Interference。对每个查询片段，构造包含相关先验演示和可控数量无关会话的会话历史，将其作为记忆输入给 VLA 模型，并测量准确率。直接使用 RoboMME 已发布的记忆增强 π₀.₅ 变体进行评测。

Result: 感知记忆变体在无干扰历史下能提升成功率，但随着无关会话数量增加，性能持续且稳定地下降，表明现有系统在长上下文记忆和抗干扰能力上存在严重缺陷。

Conclusion: 长上下文记忆和抗干扰能力对机器人实际部署至关重要，而当前系统在此类能力上表现不佳，亟需进一步研究。

Classification reason: 该论文研究机器人在多会话场景下的长上下文记忆能力，属于具身智能中机器人记忆系统评测，与[具身世界模型与规划控制]方向高度相关。

Importance reason: Focuses on robot memory under interference, which is peripheral to the user's core interests in continuous multimodal models and autoregressive generation. Signals: robot memory benchmark, cross-session interference, VLA memory decay, long-context evaluation, robotics
...
### [42] [Semi-Supervised Vision-Language-Action Model](https://arxiv.org/abs/2606.21493)
*Hongyang He, Jiuming Liu, Victor Sanchez*

Main category: cs.CV, cs.ET
...
TL;DR: 提出SemiVLA，一种自蒸馏教师-学生框架，通过可靠性控制器和瓶颈投影对齐更新，在仅有少量标注轨迹的情况下利用伪动作进行半监督VLA模型适应，在LIBERO和CALVIN上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: VLA模型适应新环境依赖大量带动作标签的演示数据，标注成本高昂。实际场景中大量轨迹仅有视觉-语言观测而无动作标签，如何利用这些无标签数据降低标注依赖是核心问题。

Method: 提出SemiVLA半监督框架：1) 教师-学生自蒸馏架构，教师生成伪动作指导学生训练；2) VLA专用可靠性控制器，评估视觉-语言对齐、动作可行性和时序过渡一致性，筛选高质量伪动作；3) Bottleneck-Projected Alignment Update (BPAU)，通过瓶颈投影更新教师参数，避免噪声反馈污染。以OpenVLA为骨干，结合多种PEFT策略。

Result: 在LIBERO和CALVIN上，10%标注轨迹条件下，SemiVLA+Selective LoRA在LIBERO上平均成功率89.0%，比有监督LoRA高8.0个百分点，且不增加推理成本。

Conclusion: SemiVLA有效利用无动作标签的视觉-语言轨迹数据，显著降低VLA模型对动作标注的依赖，为机器人具身模型在有限监督下的适应提供了可行方案。

Classification reason: 该论文研究机器人VLA模型在有限动作标签下的半监督适应方法，核心是具身智能中的动作预测与规划控制，与[具身世界模型与规划控制]高度匹配。

Importance reason: Focuses on semi-supervised VLA adaptation for robotics, which is not a priority subtopic and lacks direct connection to continuous multimodal models or video generation. Signals: Semi-supervised VLA, self-distillation, reliability controller, bottleneck-projected alignment, OpenVLA backbone
...
### [43] [Active Inference as the Test-Time Scaling Law for Physical AI Agents](https://arxiv.org/abs/2606.22813)
...
TL;DR: 本文提出了一种基于主动推理的测试时扩展律（Test-Time Scaling Law），使物理AI智能体能够在测试时利用世界模型进行推理，从而泛化到训练分布外的未见场景。该方法通过软贝叶斯推理动态更新策略，最小化自由能，并在自动驾驶任务中优于Q学习和贝叶斯强化学习，推理效率提升36%以上。


<details>
  <summary>Details</summary>
Motivation: 现有扩展律（如Scaling Law）受限于模型大小和训练数据量，无法应对物理AI智能体在真实世界中遇到的非平稳、分布外场景。需要一种能够利用测试时计算资源进行推理和泛化的新范式。

Method: 基于主动推理的第一性原理，将智能体的策略更新建模为软贝叶斯推理过程：在测试时，智能体通过最小化预期预测误差（作为似然函数）来更新关于策略的信念，得到后验策略。采用变分推理求解自由能下界，并将测试时解决的新实例同时回传到策略和世界模型，实现超越训练的学习。

Result: 在自动驾驶任务中，该方法优于模型无关的Q学习和基于模型的贝叶斯强化学习，在未见场景中实现了鲁棒的泛化，同时推理效率提升超过36%。

Conclusion: 主动推理为物理AI智能体提供了一种自然的测试时扩展机制，其扩展规模不再受限于模型参数和训练数据，而是与智能体在真实世界中的连续经验成正比，为具身智能的泛化提供了新路径。

Classification reason: 论文提出基于主动推理（Active Inference）的测试时扩展律，使物理AI智能体利用世界模型在测试时进行推理以泛化到未见场景。核心是具身智能体在非平稳环境中的规划、推理与控制，与[具身世界模型与规划控制]高度匹配。

Importance reason: Proposes a test-time scaling law for physical AI agents using active inference, directly relevant to world models and efficient inference. Signals: world model, test-time scaling, active inference, physical AI, autonomous driving
...
### [44] [Scalable Multi-Task Data Generation via Reinforcement Learning for Language-Conditioned Bimanual Dexterous Manipulation](https://arxiv.org/abs/2606.22471)
...
TL;DR: 提出一种基于强化学习的可扩展多任务数据生成流水线，通过通用奖励设计、域随机化和语言条件任务标注，合成高质量双臂灵巧操作数据集，训练语言条件多任务策略，显著提升跨任务泛化能力。


<details>
  <summary>Details</summary>
Motivation: 训练通用双臂灵巧操作策略缺乏大规模高质量数据集，人工遥操作数据存在形态不匹配、物理交互缺失、任务多样性有限等问题，而现有RL方法受限于手工设计的任务特定奖励函数。

Method: 提出系统化的RL数据生成流水线，包含：(1) 通用化奖励设计，可跨任务复用；(2) 有效的域随机化策略增强数据多样性；(3) 语言条件任务标注机制。在仿真环境中合成多样化高质量数据集，用于训练语言条件多任务策略。

Result: 在三个代表性双臂灵巧操作任务上，生成的数据显著提升了策略的泛化能力，验证了流水线的有效性。

Conclusion: 基于RL的合成数据生成是解决灵巧操作数据瓶颈的有效途径，通用奖励设计和域随机化可大幅提升数据多样性和策略泛化性，为训练通用机器人策略提供了可扩展方案。

Classification reason: 该论文研究基于强化学习的双臂灵巧操作数据生成，用于训练语言条件多任务策略，属于机器人具身智能与规划控制范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics manipulation data generation method, but outside user's core focus on continuous multimodal models and video generation. Signals: RL-based data generation, bimanual dexterous manipulation, language-conditioned policy, domain randomization, simulation-to-real transfer
...
### [45] [ARP: Enhancing Quantized Skill Abstractions via Visual Alignment and Iterative Refinement for Robotic Manipulation](https://arxiv.org/abs/2606.22480)
...
TL;DR: 提出 Aligned Refinement Policy (ARP)，一种基于离散量化的技能模仿学习框架，通过视觉-动作对比对齐和轻量级迭代残差头（IRH）两步精炼，提升长程机器人操作任务的视觉语义基础和执行精度，在 LIBERO、Meta-World 和真实人形机器人平台上取得 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于离散量化的技能模仿学习方法主要编码动作轨迹为潜在技能，缺乏视觉-语义对齐，难以利用视觉观测进行技能选择；同时离散 tokenization 在连续动作生成中引入精度误差，影响精细操作执行。

Method: ARP 包含两个核心组件：(1) 视觉-动作对齐目标（Visual-Action Alignment），通过对比学习将视觉嵌入与预量化的动作表征在共享潜空间中对齐，同时保持状态无关的技能解码器；(2) 轻量级迭代残差头（Iterative Residual Head, IRH），执行两步精炼以恢复细粒度控制，补偿离散化精度损失。

Result: 在 LIBERO 和 Meta-World 基准上达到 SOTA 性能；在 Kuavo 4 Pro 人形机器人平台上的真实机器人实验中，在两个具有挑战性的操作任务上持续优于多个基线方法。

Conclusion: 视觉-动作对齐和迭代精炼能有效提升离散技能抽象框架的语义基础和执行精度，为长程机器人操作提供更鲁棒的策略学习方案。

Classification reason: 该论文提出面向机器人操作（Robotic Manipulation）的离散技能抽象框架，通过视觉-动作对齐和迭代精炼提升长程操作策略性能，属于具身智能中机器人操作与技能学习方向，与[具身世界模型与规划控制]高度匹配。

Importance reason: Robotics manipulation skill learning is outside the user's core focus on continuous multimodal models and video generation. Signals: Robotics manipulation, discrete skill quantization, visual-action alignment, iterative refinement, not in priority subtopics
...
### [46] [Temporal Logic Guidance for Action-Only Diffusion Policies with World Models](https://arxiv.org/abs/2606.22729)
...
TL;DR: 本文提出一种利用世界模型对纯动作扩散策略进行时序逻辑引导的方法，通过STL鲁棒性梯度的注入，在不重新训练的情况下提升约束满足度，在Robomimic搬运任务中将约束违反率从80%降至4%，同时保持100%任务成功率。


<details>
  <summary>Details</summary>
Motivation: 扩散策略能够生成多模态机器人行为，但在推理时缺乏对行为模式的选择能力，难以满足人类对机器人行为的约束要求。现有方法需要联合生成未来动作和状态，增加了复杂度和运行时间。

Method: 提出一种针对纯动作扩散策略的引导方法，使用一个单独学习的可微世界模型来评估信号时序逻辑（STL）鲁棒性，将STL鲁棒性梯度注入扩散过程，从而在不重新训练的情况下引导行为满足约束。

Result: 在Robomimic的Can Transport任务上，该方法在保持100%任务成功率的同时，将约束违反率从基线方法的80%以上降低到4%。

Conclusion: 所提方法有效提升了纯动作扩散策略的约束满足能力，且无需重新训练模型。未来工作可进一步扩展鲁棒性和更复杂约束的处理。

Classification reason: 该论文提出一种结合世界模型和信号时序逻辑（STL）引导的扩散策略方法，用于机器人动作生成与约束满足。核心是使用世界模型对动作扩散策略进行可微引导，属于机器人规划控制与世界模型结合的工作，因此映射到[具身世界模型与规划控制]（embodied_world_model）。

Importance reason: Focuses on robotics world model guidance for diffusion policies, which is tangentially related to your priority on world models but lacks direct connection to your core continuous multimodal or video generation interests. Signals: World model for diffusion policy guidance, STL robustness gradient injection, Action-only diffusion policy, Robomimic task success with constraint reduction, Not aligned with priority subtopics or boost keywords
...
### [47] [UniFS: Unified Fast-to-Slow Hierarchical Architecture for Vision-Language-Action Models](https://arxiv.org/abs/2606.22794)
...
TL;DR: AdaReP提出了一种基于扰动动态遗憾分析的自适应重规划容忍度调整方法，在不修改世界模型或规划器的情况下，通过在线估计局部动力学敏感度和当前偏差来动态决定何时需要重规划，在图像空间、潜空间和真实机器人操作任务中实现了80%以上的规划查询减少，同时保持任务性能。


<details>
  <summary>Details</summary>
Motivation: 神经世界模型结合MPC需要在每个环境步进行重规划以限制累积预测误差，但计算开销巨大。缓存计划复用可减少开销，但其有效性受预测失配传播的影响。现有方法缺乏对重规划频率与任务性能之间权衡的理论分析和自适应机制。

Method: 首先通过扰动动态遗憾分析框架理论分析了缓存计划复用中惩罚随重用容忍度、累积失配和局部动力学敏感度增长的关系。基于此提出AdaReP，一个无需训练的包装器，在线估计当前缓存轨迹与真实观测的偏差以及局部动力学敏感度，自适应调整重规划容忍度阈值，当偏差超过容忍度时触发重规划。

Result: 在图像空间规划、潜空间控制和真实机器人操作任务上，AdaReP显著减少了规划器端计算量，同时保持可比的任务性能。在50次试验的真实机器人操作研究中，实现了超过80%的规划查询减少。

Conclusion: AdaReP通过理论驱动的自适应重规划策略，有效平衡了神经世界模型MPC中的计算效率与任务性能，且无需修改底层模型或规划器，具有较好的通用性和实用性。

Classification reason: 该论文提出AdaReP，一种无需训练的包装器，用于自适应调整神经世界模型+MPC中的重规划容忍度，以减少计算开销。论文在真实机器人操作任务上进行了验证，属于面向机器人规划控制的世界模型方法，与[具身世界模型与规划控制]这一规范子主题高度匹配。

Importance reason: Directly advances world model efficiency for embodied planning, aligning with high-priority world_model and efficient inference directions. Signals: world model, efficient inference, model predictive control, adaptive replanning, real-world robot validation
...
### [49] [Foresight: Failure Detection for Long-Horizon Robotic Manipulation with Action-Conditioned World Model Latents](https://arxiv.org/abs/2606.23085)
...
TL;DR: Foresight 是一个利用动作条件世界模型的潜表征进行长时程机器人操作故障检测的框架，仅需最终任务级成功/失败标签即可训练，并通过函数共形预测自适应校准检测阈值，在仿真和真实机器人上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 长时程机器人任务中的故障检测尚未被充分探索，故障发生往往模糊不清且缺乏密集的时间标注，现有方法难以在仅有最终任务标签的情况下有效检测故障。

Method: 利用动作条件世界模型提取潜表征，仅使用最终任务级成功/失败标签训练故障检测器；采用函数共形预测（FCP）自适应校准检测阈值，提供统一的跨策略故障检测框架。

Result: 在 LIBERO-Long、ManiSkill-Long、BEHAVIOR-1K 仿真环境以及真实机器人（ReactorX-200 和 Franka 机械臂）上，Foresight 优于现有故障检测方法，表明动作条件世界模型潜表征为长时程操作中的可靠故障监控提供了可扩展的表示。

Conclusion: 动作条件世界模型的潜表征能够有效支持长时程机器人操作的故障检测，且该方法具有跨策略通用性和实际部署可行性。

Classification reason: 该论文提出 Foresight，利用动作条件世界模型的潜表征进行长时程机器人操作的故障检测，属于面向机器人具身智能的世界模型方法，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Strong world model application for failure detection, but lacks direct focus on user's priority subtopics like continuous language models or video generation. Signals: world model for failure detection, long-horizon manipulation, action-conditioned latents, conformal prediction calibration, real robot validation
...
### [50] [Assistron: Bayesian Shared Autonomy with Off-the-shelf Vision-Language-Action Models](https://arxiv.org/abs/2606.23147)
...
TL;DR: Assistron 是一种基于 VLA 模型的贝叶斯共享自主框架，通过 VLA 自主执行宏观运动并在接触密集的关键失败点引入人类干预，无需微调 VLA，显著提升任务成功率并降低人类认知与体力负担。


<details>
  <summary>Details</summary>
Motivation: 现有辅助操作系统要么完全依赖人类遥操作（认知与体力负担重），要么完全依赖自主 VLA 模型（在接触密集任务中容易失败）。作者希望结合两者优势，在减少人类负担的同时保证任务成功率，且不微调 VLA 以避免灾难性遗忘。

Method: Assistron 包含三个核心组件：(1) VLA 驱动的自主宏观运动执行，根据用户语言指令生成抓取轨迹；(2) 相位感知交互检测机制，识别 VLA 在接触密集阶段可能失败的时刻；(3) 人类干预接口，通过 flow matching guidance 调整 VLA 的动作生成。整体采用贝叶斯共享自主框架，无需微调 VLA 模型。

Result: 在多任务场景恢复基准上，Assistron 相比纯自主基线显著提升了任务成功率，同时相比传统遥操作大幅降低了人类认知与体力工作量。

Conclusion: Assistron 提供了一种可扩展、平滑且省力的辅助操作范式，证明了在无需微调 VLA 的前提下，通过贝叶斯共享自主与关键点人类干预可以有效结合自主与人类控制的优势。

Classification reason: 该论文提出 Assistron，一种利用 VLA 模型进行共享自主控制的辅助操作框架，涉及机器人动作规划、人机协作与失败恢复，属于具身智能中的世界模型与规划控制范畴，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Focuses on robotics shared autonomy with VLA models, not directly aligned with user's priority on continuous multimodal models or video/image generation. Signals: Bayesian shared autonomy, VLA models, contact-rich manipulation, flow matching guidance, no fine-tuning needed
...
### [51] [Asymmetric physics enables efficient learning in quadrupedal robot swarms](https://arxiv.org/abs/2606.23153)
...
TL;DR: 本文提出非对称物理（Asymmetric Physics）方法，在训练时结合高保真非可微模拟器与可微代理模型，使多达512个四足机器人通过端到端强化学习学会基于视觉的分散式群体导航策略，并在真实环境中零样本迁移到6个实体机器人。


<details>
  <summary>Details</summary>
Motivation: 动物群体能在复杂环境中通过局部协调高效导航，但机器人群体在物理世界中复制这一能力仍面临挑战。端到端学习在具身群体中难以扩展，因为视觉感知、密集的机器人间交互和接触丰富的运动需要同时学习，标准采样式强化学习效率低下。

Method: 提出非对称物理（Asymmetric Physics）训练框架：在共享环境中，使用高保真非可微物理模拟器生成真实的运动和接触动力学，同时使用可微代理模型为导航和运动策略提供梯度。通过这种分离，实现了多达512个四足机器人的端到端协同导航策略学习。部署时每个机器人仅依靠单个前向深度摄像头，无需显式通信、集中规划或全局地图。

Result: 策略在森林、桥梁、围栏、狭窄通道和迷宫等多种环境中泛化良好，并零样本迁移到6个实体四足机器人的5个真实场景。群体展现出预测性避让、右侧让行、瓶颈前暂停和沿墙行走等涌现行为。

Conclusion: 非对称物理方法能够高效训练可扩展的分散式控制策略，使四足机器人群体在复杂环境中实现类似动物群体的协调导航行为，为具身群体智能的端到端学习提供了有效路径。

Classification reason: 该论文研究四足机器人群体在复杂环境中的分散式控制策略学习，属于具身智能与机器人规划控制范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics contribution but outside user's core focus on continuous multimodal models and video generation. Signals: Asymmetric physics for efficient swarm learning, 512-robot simulation, zero-shot real-world transfer, decentralized vision-based navigation, world model for quadruped locomotion
...
### [52] [Causal Reward World Models: Zero-shot Reward Design for Automated Skill Generation](https://arxiv.org/abs/2606.23280)
...
TL;DR: 本文提出因果奖励世界模型（CRWM），通过离线多任务交互数据预训练，显式建模候选奖励组件与目标任务物理变量之间的因果拓扑关系，使LLM在推理时能零样本生成可执行的奖励函数，无需迭代反馈调优，在复杂连续控制任务上匹配或超越SOTA，并显著降低新技能获取的延迟。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的自动奖励设计方法本质上是相关性驱动的，依赖迭代环境反馈来修正奖励假设，推理效率低且容易引入语义合理但因果虚假的奖励组件，导致优化失效。本文旨在解决这一问题，提出一种白盒化的因果奖励设计范式。

Method: 提出Causal Reward World Model (CRWM)，采用粗到细的预训练策略：首先通过多任务交互数据离线学习候选奖励组件与目标物理变量之间的因果拓扑结构（粗粒度结构先验），然后通过联合优化模块（显式机制解耦+置信感知软融合）利用微观轨迹数据细化结构先验，构建鲁棒可解释的因果骨架。推理时，LLM将CRWM作为任务无关的因果先验来约束奖励生成，实现零样本奖励函数设计。

Result: 在复杂连续控制基准上，CRWM无需反馈驱动的奖励细化即可生成可执行奖励函数，匹配或超越SOTA性能，显著降低新机器人技能获取的设计延迟，并在未见任务和不同机器人形态上展现出强泛化能力。

Conclusion: CRWM为自动奖励设计开辟了新的白盒范式，通过因果建模替代相关性驱动的奖励搜索，实现了高效、可解释且泛化性强的零样本奖励生成，为机器人技能自动化获取提供了新思路。

Classification reason: 该论文提出Causal Reward World Model (CRWM)，通过离线预训练构建因果奖励结构，用于机器人技能生成的零样本奖励设计，属于机器人具身智能中的世界模型与规划控制范畴，与[具身世界模型与规划控制]高度匹配。

Importance reason: Focuses on robotics reward design, not aligned with user's priority on continuous multimodal models or video generation. Signals: world model, causal reasoning, zero-shot reward design, offline pre-training, continuous control
...
### [53] [When Robots Rate Their Own Interactions: Engagement Validity and the Strangeness Failure](https://arxiv.org/abs/2606.23339)
...
Importance: 普通相关 (53.85) - 未入选精读

TL;DR: 本文提出"反向评估"方法，让LLM驱动的机器人从自身视角完成人机交互问卷，发现机器人在参与度维度上与人类评分有中高度一致性，但在舒适/陌生感维度上系统性反转，表明当前LLM机器人缺乏评估内部情感状态的能力。


<details>
  <summary>Details</summary>
Motivation: 传统人机交互评估完全依赖人类填写的问卷，忽略了机器人的视角。本文希望探索LLM驱动的机器人能否从自身视角完成标准化评估问卷，并检验其与人类评分的一致性，从而建立LLM作为交互评估者的边界条件。

Method: 研究1：使用5个LLM（Claude Sonnet 4.5等）对HRI-CUES数据集中25个交互的1522个评估完成HRI-CUES、Godspeed和RoSAS问卷，计算与人类评分的相关性。研究2：在Nao机器人上部署Claude Sonnet 4.5，在4次真实交互中进行实时逐轮评估，验证模式的可迁移性。

Result: LLM在参与度维度（满意度r=0.65，享受度r=0.72）上达到中高度一致性，重测信度优秀（ICC≥0.82），但在舒适/陌生感维度上系统性反转（r=-0.44到-0.67），将参与度与舒适感混淆。该"陌生感失败"模式在5个模型、合成控制和实体部署中均持续存在。

Conclusion: 当前基于LLM的机器人缺乏访问内部情感状态的能力，无法评估陌生感等需要情感感知的构念。反向评估需要补充生理信号、注视、近距空间等模态才能超越行为代理。该研究为LLM作为HRI评估者建立了重要的边界条件。

Classification reason: 该论文研究LLM驱动的机器人对自身交互进行评价（inverted evaluation），属于人机交互（HRI）中机器人自我评估的范畴，与具身智能、机器人评估和控制密切相关，因此映射到[具身世界模型与规划控制]这一规范主题。

Importance reason: Focuses on HRI evaluation methodology with LLMs, not on core continuous multimodal or video generation priorities. Signals: HRI evaluation, LLM-driven robots, engagement validity, strangeness failure, inverted evaluation
...
### [54] [Flowing With Purpose: Latent Action Guided Flow Matching Policies For Robotic Manipulation](https://arxiv.org/abs/2606.23420)
...
TL;DR: 本文提出LAFM（Latent Action Guided Flow Matching），通过潜动作模型学习离散运动基元，为flow matching策略提供自适应的结构化先验分布，替代传统全局各向同性高斯分布，显著提升机器人操作任务的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有flow matching策略在机器人操作行为克隆中采用全局固定的各向同性高斯源分布，但机器人动作空间具有强碎片化和异方差结构，这种无信息初始化迫使模型学习高度纠缠的向量场，限制了训练效率和策略性能。

Method: LAFM框架包含两个核心组件：(1) 潜动作模型，将当前观测映射到离散运动基元，学习动作的先验分布；(2) 基于潜动作引导的flow matching，根据选择的基元使用对应的自适应先验分布初始化去噪过程，使传输轨迹更短、纠缠更少。

Result: 在真实机器人部署中任务成功率提升23.4%，在LIBERO-90基准上提升10.4%，超越大规模预训练的视觉-语言-动作模型，同时使用更小的网络架构。

Conclusion: LAFM通过潜动作引导的自适应先验分布有效解决了flow matching策略的结构性不匹配问题，为机器人操作行为克隆提供了更高效、更强大的新范式。

Classification reason: 该论文提出LAFM框架，将潜动作模型与flow matching结合用于机器人操作策略学习，属于具身智能中的动作规划与控制，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Strong robotics manipulation paper but outside user's priority subtopics and negative keywords. Signals: flow matching, latent action model, robotic manipulation, behavior cloning, heteroscedastic action spaces
...
### [55] [SkyJEPA: Learning Long-Horizon World Models for Zero-Shot Sim-to-Real Control of Quadrotors](https://arxiv.org/abs/2606.23444)
...
TL;DR: 本文提出SkyJEPA，一种基于JEPA（联合嵌入预测架构）的潜空间动力学模型，用于四旋翼无人机的长时域预测与实时控制。结合物理启发的prober将潜变量映射为可解释状态，并配合采样最优控制实现零样本sim-to-real迁移，在开环和闭环实验中展示了准确的预测与强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有神经网络动力学模型在长时域预测中因自回归展开导致误差累积，而JEPA类方法在机器人导航中主要用于运动学级规划，缺乏在高频控制场景中的研究。本文旨在将JEPA引入四旋翼无人机的实时控制，实现长时域准确预测与零样本sim-to-real迁移。

Method: 提出SkyJEPA模型，包含：(1) 潜空间动力学模型（JEPA风格），在隐空间中预测未来状态；(2) 物理启发的prober，将冻结的潜变量映射为可解释的物理状态；(3) 结合采样最优控制（如MPC）利用预测能力进行实时控制；(4) 自动化数据集生成管道，减少对昂贵且危险的真实数据采集的依赖。

Result: 在开环和室外闭环实验中，SkyJEPA展示了准确的长时间预测能力、鲁棒的零样本sim-to-real迁移，以及在多种不同操作条件下的强泛化性能。

Conclusion: SkyJEPA证明了JEPA风格潜空间动力学模型在高频四旋翼控制中的有效性，为具身智能系统中长时域预测与sim-to-real迁移提供了新范式。

Classification reason: 该论文提出用于四旋翼无人机实时控制的JEPA风格世界模型，结合潜空间动力学与采样最优控制实现零样本sim-to-real迁移，属于机器人具身世界模型与规划控制范畴，因此映射到[CANONICAL]具身世界模型与规划控制。

Importance reason: Strong world model contribution with JEPA and zero-shot sim-to-real, but focused on quadrotor control rather than user's priority subtopics. Signals: JEPA latent dynamics, zero-shot sim-to-real, physics-inspired prober, long-horizon prediction, real-time control
...
### [56] [HoloAgent-0: A Unified Embodied Agent Framework with 3D Spatial Memory](https://arxiv.org/abs/2606.23565)
...
TL;DR: HoloAgent-0 是一个统一的具身智能体框架，通过 Embodied AgentOS、3D 空间记忆和具身技能三层耦合架构，将语言指令转化为可执行技能图，实现真实机器人的闭环执行、资源调度和重规划，并在空间记忆、长程导航、跨机器人协作等任务上进行了评估。


<details>
  <summary>Details</summary>
Motivation: 现有具身 AI 系统在操作、空间理解、导航和人形控制等方面取得了进展，但这些能力往往是专门的模块或松散耦合的决策循环，缺乏统一的框架将 LLM agent 的闭环执行循环扩展到物理机器人。物理执行具有连续性、依赖具体本体、不确定性和安全约束，使得扩展困难。

Method: 提出 HoloAgent-0 框架，包含三个耦合层：(1) Embodied AgentOS——将语言指令转换为可执行技能图，调度机器人资源，监控执行并根据运行时反馈触发澄清或重规划；(2) 3D 空间记忆——用于物理世界接地；(3) 具身技能——用于机器人动作执行。系统在真实硬件上部署。

Result: 在真实硬件上评估了空间记忆、长程导航和闭环执行能力，涵盖运动生成、物体搜索、跨机器人协调和移动操作等任务。

Conclusion: HoloAgent-0 展示了将 LLM agent 的闭环执行循环扩展到物理机器人的可行性，通过三层耦合架构实现了统一的具身智能体框架，为真实世界机器人部署提供了有效方案。

Classification reason: 该论文提出 HoloAgent-0，一个面向真实机器人部署的统一具身智能体框架，包含 3D 空间记忆、闭环执行和技能调度，属于世界模型方向下的具身世界模型与规划控制（embodied_world_model）这一规范主题。

Importance reason: While the paper presents a unified embodied agent framework with 3D spatial memory, it focuses on robotics and physical world grounding, which is not a priority subtopic and lacks direct connection to the user's core interests in continuous language/multimodal models and generative video/image generation. Signals: Embodied agent framework, 3D spatial memory, Real-world robot deployment, Long-horizon navigation, Cross-robot coordination
...
### [57] [RECALL: Recovery Experience Collection for Active Lifelong Learning in Vision-Language-Action Models](https://arxiv.org/abs/2606.23617)
...
TL;DR: 本文提出RECALL框架，通过不确定性引导的主动数据收集实现VLA模型的持续学习，相比被动模仿学习能更高效地微调，同时发现仅用主动收集的恢复数据会导致灾难性遗忘，并评估了回放混合与弹性权重巩固等持续学习技术的效果。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型通过被动模仿学习微调，需要机器人先失败才能触发数据收集，且无法区分哪些状态需要监督，浪费了演示者在策略已表现良好部分的精力。本文旨在提出一种主动、持续的VLA学习范式来解决这些问题。

Method: 提出RECALL（Recovery Experience Collection for Active Lifelong Learning）框架，采用不确定性引导的主动数据收集策略，在策略不确定性高的状态下收集恢复演示数据；同时结合回放数据混合（replay-based data mixing）和弹性权重巩固（elastic weight consolidation）等持续学习技术，在适应新数据与保留已有知识之间取得平衡。

Result: 实验表明，不确定性引导的主动数据收集比被动收集的演示数据能实现更高效的微调；但仅用主动收集的恢复数据微调会导致灾难性遗忘；回放混合和弹性权重巩固等方法可以在可塑性与记忆保留之间取得折中。

Conclusion: 本文建立了自回归VLA模型主动持续学习的实证研究，证明不确定性引导的恢复演示可以提高适应效率，同时也揭示了将目标性新数据融入大规模机器人策略时面临的开放挑战。

Classification reason: 该论文研究VLA（视觉-语言-动作）模型的主动持续学习，涉及机器人策略学习与不确定性引导的数据收集，属于具身智能中世界模型与规划控制范畴，因此映射到[CANONICAL]具身世界模型与规划控制。

Importance reason: Focuses on active continual learning for VLA models, which is tangentially related to world models but lacks direct connection to the user's priority subtopics like continuous language models or video generation. Signals: Active continual learning, VLA model fine-tuning, uncertainty-guided data collection, catastrophic forgetting, replay-based mixing
...
### [58] [dVLA-RL: Reinforcement Learning over Denoising Trajectories for Discrete Diffusion Vision-Language-Action Models](https://arxiv.org/abs/2606.23623)
...
TL;DR: 本文提出 dVLA-RL，将强化学习引入离散扩散视觉-语言-动作模型（dVLA），通过将去噪过程建模为马尔可夫决策过程（MDP），将学习目标从边际动作概率转移到生成路径的联合概率，并引入统一步调度策略，在 LIBERO 上达到 99.7% 的成功率，在 RoboTwin 2.0 上相比 SFT 基线提升 30.6%。


<details>
  <summary>Details</summary>
Motivation: 现有的离散扩散 VLA 模型（dVLA）仅通过监督微调（SFT）训练，尚未探索强化学习（RL）进一步优化策略的潜力。然而，dVLA 生成最终动作的边际概率难以计算，导致无法直接应用标准 RL 方法。

Method: 将 dVLA 的去噪过程建模为马尔可夫决策过程（MDP），将学习目标从边际动作概率转移到采样生成路径的联合概率（即各步转移概率的乘积）。基于这一轨迹级目标，提出统一步调度策略，为不同复杂度的任务自适应分配去噪步数，以平衡成功率和计算效率。

Result: 在 LIBERO 基准上达到 99.7% 的成功率；在 RoboTwin 2.0 上相比 SFT 基线提升 30.6%，与强世界-动作模型基线竞争。

Conclusion: 通过将 RL 引入离散扩散 VLA 模型，有效提升了机器人操作策略的性能，验证了轨迹级概率建模和自适应步调度的有效性。

Classification reason: 该论文研究面向机器人操作的 Vision-Language-Action (VLA) 模型，使用离散扩散进行策略学习，属于具身智能中的动作规划与控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Novel RL training for discrete diffusion VLA models aligns with world model and continuous token priorities. Signals: discrete diffusion VLA, RL over denoising trajectories, 99.7% LIBERO success, 30.6% improvement over SFT, unified step scheduling
...
### [59] [LIBERO-Safety: A Comprehensive Benchmark for Physical and Semantic Safety in Vision-Language-Action Models](https://arxiv.org/abs/2606.23686)
...
TL;DR: LIBERO-Safety 是一个面向视觉-语言-动作（VLA）模型的物理与语义安全综合基准。它通过参数化安全场景生成和关键帧驱动数据流水线，构建了包含19,664条无碰撞演示的大规模数据集，并对8个VLA模型和2个具身基础模型进行了系统评估，揭示了泛化-安全之间的张力。


<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人操作中展现出强大能力，但其在严格约束下的操作安全性尚未得到充分验证。现有基准缺乏对安全关键场景的系统覆盖，且人工遥操作数据采集存在可扩展性瓶颈。

Method: 提出参数化安全基准，程序化生成安全关键场景并引入全面随机性；开发关键帧驱动的数据生成流水线，克服人工遥操作的可扩展性瓶颈；构建大规模无碰撞演示数据集（19,664条），包含广泛的域随机化；对8个VLA模型和2个具身基础模型进行跨范式系统评估。

Result: 揭示了泛化-安全之间的关键张力：高多样性训练有助于生成更安全的轨迹，但任务成功率仍受限于次优轨迹合成和语义对齐问题。提供了可扩展的流水线、鲁棒的数据集和深入的失败模式分析。

Conclusion: LIBERO-Safety 为开发安全可靠的VLA模型奠定了重要基础，指出了当前VLA模型在安全约束下的关键瓶颈，并为未来研究提供了标准化评测平台。

Classification reason: 该论文研究VLA（视觉-语言-动作）模型在机器人操作中的物理与语义安全性，属于具身智能/机器人领域的安全评测与基准构建，与[具身世界模型与规划控制]方向高度相关，因此复用该规范子主题。

Importance reason: Strong benchmark for VLA safety but only tangentially touches user's priority on continuous multimodal models and world models. Signals: VLA safety benchmark, physical and semantic safety, generalization-safety tension, keypose-driven data pipeline, collision-free dataset
...
### [60] [Flow6D: Discrete-to-Continuous Flow Matching for Efficient and Accurate Category-Level 6D Pose Estimation](https://arxiv.org/abs/2606.23293)
...
TL;DR: Flow6D 提出一种离散到连续的层次化流匹配框架，用于高效且准确的类别级 6D 位姿估计。该方法先通过离散流匹配将旋转/平移参数离散化为 bins 以锁定潜空间，再通过连续流匹配回归局部残差以精化位姿，在合成和真实数据集上达到 SOTA 性能并支持 70 FPS 实时推理。


<details>
  <summary>Details</summary>
Motivation: 现有类别级 6D 位姿估计方法直接在无限高维连续空间中回归，面临两大挑战：一是噪声和局部最优导致精度有限，二是在无限空间中搜索效率低下，难以实现实时性能。

Method: Flow6D 采用两阶段层次化流匹配策略：(1) 离散潜空间定位阶段——将旋转和平移参数离散化为 bins，用离散流匹配模型将潜空间锁定在真实位姿附近，降低搜索复杂度；(2) 连续位姿回归阶段——在潜空间中采样，用连续流匹配模型预测局部位姿残差以优化估计。该方法还自然扩展到铰接物体。

Result: 在合成和真实数据集上，Flow6D 超越现有 SOTA 方法，并以 70 FPS 实现实时推理。

Conclusion: Flow6D 通过离散到连续的层次化流匹配策略，有效解决了类别级 6D 位姿估计中精度与效率的权衡问题，为机器人操作等具身智能应用提供了实用的实时解决方案。

Classification reason: 该论文提出 Flow6D，一种面向类别级 6D 位姿估计的分层流匹配框架，用于机器人操作等具身智能任务。这与 [CANONICAL] 具身世界模型与规划控制 高度相关，因为 6D 位姿估计是机器人抓取和操作的核心感知模块。

Importance reason: Strong robotics pose estimation method but only tangentially touches world models and lacks direct connection to priority subtopics. Signals: category-level 6D pose estimation, discrete-to-continuous flow matching, real-time 70 FPS, articulated object extension, SOTA on synthetic and real datasets

Abstract: 6D pose estimation is a key task in computer vision and embodied AI, widely used in robotic manipulation, augmented reality, etc. Existing methods directly regress in a high-dimensional continuous space, facing two key challenges in category-level pose estimation: limited accuracy due to noise and local optima, and inefficient search over an infinite space that hinders real-time performance. This paper proposes Flow6D, a hierarchical flow matching framework with a two-stage discrete latent space localization-continuous pose regression strategy. Rotation and translation parameters are first discretized into bins, with a discrete flow matching model locking the latent space around the true pose to reduce search complexity. Then, by sampling in the latent space, a continuous flow matching model predicts local pose residuals to optimize the estimate and regress to an accurate pose. The framework also naturally extends to articulated objects, outperforming state-of-the-art methods on synthetic and real datasets with real-time inference at 70 FPS. Project website: .

</details>


### [61] [A Watermark for Vision-Language-Action and World Action Models](https://arxiv.org/abs/2606.23574)
*Yule Liu, Shuai Liu, Jiaheng Wei, Xinlei He*

Main category: cs.RO, cs.CR
...
TL;DR: 本文提出一种针对视觉-语言-动作（VLA）模型和世界动作模型（WAM）的水印方法——keyed latent-provenance verification。该方法通过在模型生成前将高斯噪声种子替换为带密钥的种子来嵌入指纹，使水印动作与普通动作统计上不可区分。验证阶段通过梯度最大后验估计从机器人执行的动作通道中恢复种子，检测密钥存在性以判定模型归属。实验表明该方法在保持任务性能的同时，对输出端移除攻击和权重级编辑具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: VLA模型和WAM模型作为通用机器人控制的核心模型，训练需要专有数据和大量算力，部署为黑盒服务后模型本身成为有价值的知识产权。现有水印方法难以直接应用于机器人策略模型，需要一种不影响任务性能且对攻击鲁棒的模型所有权保护方案。

Method: 提出keyed latent-provenance verification方法：注入阶段，将模型生成时采样的高斯噪声种子替换为密钥派生种子（与普通噪声同分布），使水印动作与正常动作统计不可区分；验证阶段，从机器人执行的动作通道（可能部分且后处理过）中，通过梯度最大后验估计恢复种子，检测密钥存在性，聚合多个rollout的分数判定模型归属。

Result: 在两个机器人套件上的两个代表性模型上进行了实验，覆盖指纹检测、多密钥识别、多种攻击鲁棒性测试。结果表明指纹可被可靠检测，对任务性能影响极小，且在输出端移除攻击和权重级编辑下仍保持可检测性。

Conclusion: 本文提出的水印方法能有效保护VLA和WAM模型的版权，在保持任务性能的同时具备良好的隐蔽性和鲁棒性，为机器人基础模型的IP保护提供了可行方案。

Classification reason: 该论文关注VLA（视觉-语言-动作）模型和世界动作模型（WAM）的知识产权保护与水印方法，这些模型直接用于通用机器人控制，属于具身智能/机器人领域。虽然论文核心是安全/水印技术而非世界模型本身，但其保护的对象（VLA和WAM）正是具身世界模型与规划控制方向的核心模型，因此映射到[CANONICAL]具身世界模型与规划控制。

Importance reason: Novel watermarking for VLA/WAM models, but peripheral to core priority subtopics. Signals: world model watermarking, VLA model IP protection, keyed latent-provenance, gradient MAP seed recovery, robotics policy fingerprinting
...
### [62] [PolicyTrim: Boosting Intrinsic Policy Efficiency of Vision-Language-Action Models](https://arxiv.org/abs/2606.22540)
...
TL;DR: PolicyTrim是一个基于强化学习的后训练框架，通过动态探索策略延长VLA模型可靠动作块长度，并设计冗余感知奖励减少物理执行步骤，实现高达5.83倍的端到端部署加速而不牺牲任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在实际部署中受限于执行效率，已有工作主要关注计算效率（减少单步推理延迟），但忽略了内在的策略效率——即预测动作块的有效执行长度和完成任务所需的总物理步数。当前VLA策略存在规划不可靠性（动作块尾部预测退化严重）和动作冗余（产生不必要的物理步骤）的问题。

Method: 提出PolicyTrim框架，包含两个核心组件：(1) 可靠块扩展——采用动态探索策略，显式奖励成功完成更长可执行长度的动作块，逐步将可信预测推向经验极限；(2) 步数效率优化——设计冗余感知奖励函数，直接奖励用更少步骤成功完成任务，同时惩罚不可复现的捷径行为，消除冗余物理动作。

Result: 在三个基准和三个VLA模型上的实验表明，PolicyTrim将动作块利用率提升3倍，物理执行步骤减少51.4%，端到端部署速度提升最高达5.83倍，且不牺牲任务成功率。

Conclusion: PolicyTrim有效解决了VLA模型的策略效率问题，通过强化学习后训练同时提升了动作块可靠长度和步数效率，为VLA模型的实际部署提供了实用加速方案。

Classification reason: 该论文研究VLA（视觉-语言-动作）模型的策略效率，通过强化学习后训练框架提升动作块的有效执行长度并减少冗余物理步骤，直接面向机器人操作中的动作规划与控制，属于具身世界模型与规划控制范畴。

Importance reason: Addresses VLA model efficiency via RL-based action chunk extension, but focuses on robotics rather than core continuous language/multimodal or video generation priorities. Signals: VLA model efficiency, RL-based post-training, action chunk extension, step reduction, robotics deployment
...
### [63] [Attacking the Trusted Imagination: Oracle-Level Integrity Attacks on Imagine-then-Act World Models](https://arxiv.org/abs/2606.22966)
...
TL;DR: 本文发现 imagine-then-act 世界模型中的"可信想象"（latent trajectory）是暴露的攻击面，攻击者通过观测扰动可以轻易破坏想象轨迹（非目标攻击），但精确控制想象轨迹（目标攻击）较为困难。实验表明非目标攻击效果显著（约随机扰动的60倍），而自适应攻击者只能放弃破坏来逃避检测。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 策略依赖世界模型先想象未来轨迹再基于此做决策，但该"想象"环节的安全性未被充分研究。作者识别出下游组件（安全门、MPC规划器、验证器）直接消费想象轨迹，其鲁棒性不等同于策略本身的鲁棒性，存在不对称的攻击面。

Method: 采用基于能力的威胁模型，在观测上施加 L∞ 有界扰动。通过可微的观测到想象映射，使用投影梯度下降（PGD）进行攻击。非目标攻击将想象轨迹推离自然未来流形；目标攻击则需精确到达指定流形上的目标。同时提出一种无参数的去噪检测器利用 off-manifold 特性进行防御。

Result: 在 RynnVLA-002、LingBot-VA 和 LaDi-WM 三个模型上，非目标攻击效果约为随机扰动的60倍，检测 AUC 达 1.0。目标攻击效果有限。自适应攻击者只能放弃破坏来逃避检测。原生想象驱动的 MPC 在 epsilon=0.01 时成功率从 0.70 降至 0.05（Fisher p < 10^-4），首次出现针对特定对手的任务失败。

Conclusion: 世界模型的想象轨迹是脆弱且易被攻击的环节，非目标攻击极易实现且难以防御。下游组件（如 MPC 规划器）对想象轨迹的依赖会引入新的安全风险。需要重新审视 imagine-then-act 范式的安全设计。

Classification reason: 论文研究 imagine-then-act 世界模型（WAM）的安全攻击，涉及视觉-语言-动作（VLA）策略、世界模型想象轨迹、MPC规划器与安全门等具身智能规划控制组件，属于具身世界模型与规划控制方向。

Importance reason: Focuses on adversarial robustness of world models, not on core generative or continuous modeling priorities. Signals: world model security, adversarial attack, imagine-then-act, latent trajectory, robustness asymmetry
...
### [64] [Superhuman AI for Generals.io Using Self-Play Reinforcement Learning](https://arxiv.org/abs/2606.23348)
...
TL;DR: 本文提出了一个在实时策略游戏Generals.io中达到超人类水平的AI智能体。通过自博弈强化学习，在4天内在4块NVIDIA H200 GPU上训练完成，在5000+人类玩家的1v1排行榜上排名第一，并以199-70的总战绩击败两位顶级人类玩家。关键创新是一个JAX原生的高速模拟器，单GPU可达数千万帧/秒，相比之前模拟器加速约10000倍，消除了数据瓶颈。


<details>
  <summary>Details</summary>
Motivation: 实时策略游戏（RTS）需要长期规划和短期战术的平衡，且存在强不完全信息，对AI构成重大挑战。现有方法受限于模拟器速度瓶颈，难以在合理时间内通过自博弈学习到超人类水平。作者希望证明，一旦移除数据瓶颈，简单的策略梯度方法配合高速模拟器即可达到超人类表现。

Method: 1) 构建JAX原生模拟器，单GPU可达数千万帧/秒，相比之前C++模拟器加速约10000倍。2) 使用Vision Transformer作为策略网络，端到端训练。3) 自博弈训练，策略梯度循环，稀疏的胜负奖励信号。4) 采用top-advantage样本过滤（仅保留优势值最高的样本）和策略参数的指数移动平均（EMA）来稳定训练。

Result: 1) 在5000+人类玩家的1v1排行榜上排名第一，领先第二名与第二名到第25名的差距相当。2) 与两位顶级人类玩家进行269场天梯比赛，以199-70的总战绩获胜。3) 仅需4天在4块H200 GPU上训练完成。4) 消融实验表明，top-advantage过滤和EMA策略参数对性能至关重要，而复杂的奖励塑形、对手采样策略等并非必要。

Conclusion: 本文证明，在实时策略游戏中，一旦通过高速模拟器消除数据瓶颈，简单的自博弈策略梯度方法即可达到超人类水平。关键发现是：高速模拟器是核心使能技术，top-advantage过滤和EMA策略参数是有效技巧，而复杂的奖励设计等并非必要。这为游戏AI和更广泛的强化学习研究提供了重要启示。

Classification reason: 该论文使用自博弈强化学习训练了一个实时策略游戏（Generals.io）的超人类AI智能体，核心是构建了一个JAX原生的高速模拟器（世界模型）用于策略学习。这属于具身/游戏世界模型与规划控制的范畴，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Strong engineering contribution with a 10,000x simulator speedup, but the game domain and RL focus are outside the user's core priority subtopics. Signals: Self-play RL, JAX-native simulator, real-time strategy game, superhuman AI, policy-gradient loop

Abstract: We present a superhuman AI agent for , a real-time strategy game that requires both long-horizon planning and short-term tactics under strong imperfect information. Trained for four days on 4x NVIDIA H200 GPUs, our agent reaches #1 on the public 1v1 leaderboard of over 5,000 human players, leading the second-ranked player by the same margin that separates second place from 25th, and beats the two top-ranked humans head-to-head with a combined 199-70 record across 269 ladder matches. A key enabler is a JAX-native simulator that reaches tens of millions of frames per second on a single GPU, roughly a 10,000x speedup over the prior simulator. On top of this, we train a vision transformer policy end-to-end by self-play with a policy-gradient loop and sparse win/loss reward, using top-advantage sample filtering and an exponential moving average of the policy parameters. Taken together, our findings highlight what matters, and what does not, once a fast simulator removes the data bottleneck.

</details>


### [65] [Adversarial observations in probabilistic State-Space Models for robust Reinforcement Learning](https://arxiv.org/abs/2606.20880)
*M. Santos-Pascual, D. Ríos Insua*

Main category: stat.ML, cs.LG, stat.ME
...
TL;DR: 本文分析了线性概率状态空间模型中对抗性观测攻击对潜状态推断和策略决策的影响，在似然约束下确保扰动一致性，为构建鲁棒强化学习系统提供了理论路径，尤其适用于机器人等安全关键领域。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测或对抗性观测环境下，强化学习系统需要准确推断环境潜状态及其不确定性。现有工作缺乏对对抗性观测如何影响潜状态和策略决策的系统性理论分析，本文旨在填补这一空白。

Method: 基于线性概率状态空间模型（SSM），分析攻击者在似然约束下修改观测数据以保持统计一致性的对抗性攻击，研究此类观测偏移如何传播到潜状态估计并最终影响基于策略的决策。

Result: 揭示了对抗性观测在概率状态空间模型中的传播机制，展示了似然约束下的观测扰动如何影响潜状态推断和策略决策，为鲁棒强化学习提供了理论分析框架。

Conclusion: 该工作为构建更鲁棒的强化学习系统提供了理论路径，对机器人等安全关键领域在传感器噪声、部分故障和对抗条件下的可靠运行具有重要意义。

Classification reason: 该论文研究在部分可观测或对抗性观测下，概率状态空间模型中的鲁棒强化学习，直接涉及机器人等安全关键领域的决策与控制，属于具身世界模型与规划控制的范畴，因此复用 [CANONICAL] subtopic "具身世界模型与规划控制"。

Importance reason: Focuses on adversarial robustness in state-space models for RL, which is tangential to your priority on continuous multimodal models and video generation. Signals: adversarial observations, probabilistic state-space models, robust reinforcement learning, safety-critical robotics, likelihood-constrained attacks
...
### [66] [VegSim: A Geospatial World Model for Scenario-Conditioned Vegetation Simulation](https://arxiv.org/abs/2606.21961)
...
TL;DR: IOI提出一种混合交互世界模型，将解析运动学先验与学习到的物理动力学解耦，通过多视角正交投影的运动学引导注入视频生成器，实现精确的动作对齐和物理合理的视觉反馈，在RoboTwin基准上达到SOTA，并支持零样本OOD泛化和策略评估。


<details>
  <summary>Details</summary>
Motivation: 纯数据驱动的交互世界模型缺乏显式结构约束，难以保证精确的动作控制对齐和物理合理的视觉反馈，容易出现时空漂移问题。

Method: IOI将确定性运动（运动学）与随机物理交互（动力学）解耦：首先从动作序列计算正向运动学得到精确运动轨迹，渲染为前、侧、顶三视角正交投影；然后通过多视角运动学聚合与注入模块将这些几何线索融合并注入视频生成器，使生成以确定性轨迹为条件，生成器只需建模随机物理交互。

Result: 在RoboTwin基准上，IOI在运动学保真度、分布外泛化和策略评估方面达到SOTA；零样本泛化到未见OOD任务；作为策略评估器，其成功率与真实物理模拟器高度一致；在真实平台上，基于IOI合成数据训练的策略与基于遥操作演示训练的策略性能相当。

Conclusion: IOI通过解耦运动学与物理动力学，有效解决了数据驱动交互世界模型的精确性和物理合理性难题，为具身策略学习提供了实用且可靠的模拟环境。

Classification reason: 该论文提出IOI混合交互世界模型，结合解析运动学先验与学习到的物理动力学，用于模拟动作条件交互环境。这属于可交互与物理世界建模（interactive_physical_world）这一[CANONICAL]子主题的核心范畴。

Importance reason: Decouples kinematics from physics in world models, directly advancing interactive world model and video generation for embodied AI. Signals: world model, interactive world model, video generation, kinematics-physics decoupling, RoboTwin benchmark
...
### [68] [MeGAS: Thermomechanical Dynamic Gaussian Splatting for Thermophysical Scene Editing](https://arxiv.org/abs/2606.23455)
...
TL;DR: MeGAS将热力学相变动力学（熔化、凝固等）融入3D高斯泼溅框架，提出热机械动态高斯泼溅表示，结合热对流-扩散求解器与MPM动力学，实现物理一致的热物理现象渲染与场景编辑。


<details>
  <summary>Details</summary>
Motivation: 现有物理驱动的神经渲染方法仅关注机械动力学（如弹性、塑性），忽略了温度这一基础但不可见的物理因素。温度驱动的相变（熔化、凝固等）在真实世界中广泛存在，现有方法无法建模此类热机械过程。

Method: 1) 提出热机械动态高斯泼溅表示，为3DGS增加温度属性；2) 设计热对流-扩散求解器，结合MPM（物质点法）动力学，模拟包含相变的热机械过程；3) 提出拓扑自适应高斯渲染策略，缓解极端变形下的开裂和漂浮伪影。

Result: 实验表明MeGAS能生成物理一致的热机械行为（如熔化、凝固），同时保持高保真照片级渲染质量，在多个热物理场景上优于现有方法。

Conclusion: MeGAS成功将热力学相变引入神经渲染框架，推动了物理集成世界模型的发展，为热物理场景编辑与动态渲染提供了新范式。

Classification reason: 该论文将热力学相变动力学与3DGS神经渲染结合，构建物理一致的热机械世界模型，属于世界模型方向下的"可交互与物理世界建模"（interactive_physical_world）规范子主题。同时，论文涉及3DGS渲染与动态场景生成，也匹配多模态生成方向下的"4D与动态场景生成"（dynamic_4d_generation）。

Importance reason: Integrates thermomechanical phase-change dynamics into 3DGS, advancing physics-integrated world models with high relevance to world_model and continuous multimodal representation. Signals: world_model, physics-integrated rendering, thermomechanical dynamics, 3D Gaussian Splatting, phase transitions
...
### [69] [Scene-Level Heterogeneous Physics Simulation with 3D Gaussian Splats](https://arxiv.org/abs/2606.21753)
...
TL;DR: 本文提出一个框架，通过表示抽象层将3DGS资产、虚拟网格和流体统一为物理粒子集，首次实现3DGS资产在场景级、异构、多求解器物理仿真中的参与，支持可变形3DGS与复杂静态环境的双向交互。


<details>
  <summary>Details</summary>
Motivation: 3DGS在逼真渲染方面取得了SOTA，但其表示形式与生产级物理引擎不兼容，导致这些资产无法参与物理交互。现有物理-for-3DGS方法仅支持孤立环境中的物体级物理，无法处理复杂静态碰撞几何或异构资产。

Method: 提出表示抽象框架（Representation Abstraction Framework），将3DGS、虚拟网格、流体等异构资产统一转换为物理粒子集；该粒子集与从场景捕获中提取的静态碰撞边界一起输入求解器无关的物理内核；物理结果再映射回各资产的视觉表示，驱动其重建。

Result: 展示了可变形3DGS资产与标准CG资产（流体、网格）及大规模捕获静态环境之间的复杂双向交互，实现了先前无法实现的真实耦合物理现象。

Conclusion: 该框架首次弥合了3DGS与物理仿真之间的鸿沟，使3DGS资产能够参与场景级异构多求解器物理仿真，为神经渲染资产在物理交互中的应用开辟了新方向。

Classification reason: 该论文提出将3DGS资产与物理模拟桥接的框架，实现场景级异构物理仿真（包括可变形3DGS、流体、网格与静态环境的双向交互），属于物理世界建模与交互式模拟器范畴，与[CANONICAL] subtopic "可交互与物理世界建模"（interactive_physical_world）高度匹配。

Importance reason: Bridges 3DGS with scene-level physics simulation, enabling interactive world models with heterogeneous assets. Signals: world model, 3D Gaussian Splatting, physical simulation, heterogeneous assets, scene-level interaction
...
### [70] [Spectral GS-SLAM: Observability-Aware, Degeneracy-Robust Tracking for Real-Time 3D Gaussian Splatting SLAM](https://arxiv.org/abs/2606.21258)
...
TL;DR: Spectral GS-SLAM 提出一种结合ICP与特征匹配的鲁棒跟踪框架，通过自适应补偿退化场景中的欠约束方向，并引入高斯感知平面性加权机制，在无纹理/无结构环境中实现实时（40.14 FPS）且稳定的3DGS-SLAM跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS-SLAM系统在无纹理环境中特征匹配易失败，在无结构或几何退化场景中ICP跟踪因病态优化而失效。本文旨在解决这些退化场景下的跟踪不稳定问题。

Method: 提出Spectral GS-SLAM框架，将ICP与互补的特征约束集成，通过自适应补偿退化场景中的欠约束方向来缓解数值不稳定性；引入高斯感知平面性加权机制，利用3D高斯协方差结构表征场景几何并指导信息融合。

Result: 在TUM RGB-D序列上实现实时性能（40.14 FPS），在无结构和无纹理环境中保持一致的跟踪，在退化场景中保持轨迹完整性，在非退化条件下保持竞争性性能。

Conclusion: Spectral GS-SLAM通过自适应补偿和平面性加权机制有效提升了3DGS-SLAM在退化场景中的鲁棒性，实现了实时且稳定的跟踪。

Classification reason: 该论文提出基于3D高斯泼溅的SLAM系统，属于3D场景建图与跟踪，与[空间3D/4D世界模型]（spatial_3d_4d_world_model）高度匹配，该子主题涵盖3D场景、神经场与空间世界建模。

Importance reason: Focuses on SLAM robustness in degenerate scenes, which is niche and not aligned with user's priority on continuous multimodal models or video/image generation. Signals: 3DGS-SLAM, real-time tracking, degeneracy-robust, ICP+feature matching, Gaussian-aware planarity weighting
...
TL;DR: ϕ-Scene 提出了一种物理驱动的单图到3D场景重建方法，将场景重建视为拓扑驱动的物理组装过程：先推断物体间的支撑关系并排序，再通过SDF优化消除穿透，最后用刚体仿真将物体稳定放置到物理合理的接触状态，显著提升了重建场景的物理有效性。


<details>
  <summary>Details</summary>
Motivation: 现有单图3D场景重建方法主要关注视觉和几何预测，输出结果常存在物体漂浮、相互穿透、接触不稳定等物理不合理问题，限制了其在仿真、机器人和交互环境中的实际应用。作者希望将场景重建从纯视觉几何预测提升为物理一致的系统建模。

Method: ϕ-Scene 采用拓扑驱动的物理组装流程：1) 从单张图像中检测物体并预测初始位姿；2) 推断物体间的支撑关系（如桌子支撑杯子），构建拓扑排序；3) 按拓扑顺序逐个处理物体，先通过SDF优化消除与已固定支撑物体的穿透；4) 使用刚体仿真在重力等物理约束下将物体稳定放置到接触状态。整个过程确保每个物体都建立在已稳定的支撑上下文之上。

Result: 在3D-Front数据集上，ϕ-Scene 在域外方法中取得最佳综合性能，与域内方法相比也极具竞争力。人类评估和VLM评估显示其在视觉质量、参考对齐和物理合理性方面获得强烈偏好。专门的物理合理性指标（静态接触质量和动态稳定性）表明，ϕ-Scene 大幅减少了穿透伪影，后仿真漂移显著降低，重建场景更加稳定和物理可信。

Conclusion: ϕ-Scene 验证了将物理约束（支撑关系、穿透消除、刚体仿真）显式融入3D场景重建的有效性，为生成物理可用的3D场景提供了新范式，对仿真、机器人和交互式环境等下游应用具有重要价值。

Classification reason: 该论文从单张图像重建具有物理一致性的3D场景，关注场景的物理稳定性（支撑关系、穿透消除、接触稳定性），属于世界模型方向下的空间3D世界建模。同时涉及3D场景生成，也匹配多模态生成下的3D生成与重建。最佳匹配的规范子主题为 world_model 下的 [CANONICAL] spatial_3d_4d_world_model（空间3D/4D世界模型）。

Importance reason: Strong physical grounding for 3D scene reconstruction, but only indirectly related to your top continuous multimodal and video generation priorities. Signals: physically grounded 3D reconstruction, single-image to scene, topology-driven assembly, SDF penetration resolution, rigid-body simulation
...
### [72] [Humanoid-OmniOcc: Stereo-Based Full-View Occupancy Dataset for Embodied AI](https://arxiv.org/abs/2606.22971)
...
TL;DR: 本文提出Humanoid-OmniOcc，一个面向人形机器人的大规模全景立体占用预测数据集，包含15个模拟室内场景和5个真实环境，共155K+样本。同时提出Humanoid Surround Stereo-guided Occupancy模型，利用深度先验实现准确的2D到3D提升，在模拟和真实场景中均优于单目基线。


<details>
  <summary>Details</summary>
Motivation: 现有占用预测数据集主要面向自动驾驶，存在前视相机、远场几何和静态道路先验等车辆中心偏差，不适用于人形机器人的感知需求。需要专门针对人形机器人的全景立体占用数据集。

Method: 采用Real2Sim2Real闭环范式：1）真实传感器参数驱动物理精确仿真；2）仿真生成大规模标注训练数据；3）仿真训练的模型直接在真实场景评估。提出Humanoid Surround Stereo-guided Occupancy模型，利用立体深度先验进行2D到3D提升。

Result: 在未见过的模拟测试场景和真实环境中，所提方法均优于单目基线方法，验证了Real2Sim2Real设计的有效性。

Conclusion: Humanoid-OmniOcc填补了面向人形机器人的占用预测数据集空白，Real2Sim2Real范式有效弥合了仿真到真实的差距，为具身智能的空间感知提供了重要基础。

Classification reason: 该论文提出面向人形机器人的全景立体占用预测数据集与模型，属于3D场景空间占用建模，与[空间 3D/4D 世界模型]（spatial_3d_4d_world_model）高度匹配。

Importance reason: Strong dataset contribution for embodied AI but only tangentially touches user's priority on continuous multimodal models and video generation. Signals: world model dataset, stereo occupancy, humanoid robot, Real2Sim2Real, not continuous language/multimodal

Abstract: Occupancy prediction at voxel-level granularity is essential for safe robotic navigation and interaction in complex environments. Existing occupancy datasets, however, are predominantly designed for autonomous driving with vehicle-centric biases -- forward-facing cameras, far-field geometry, and static road priors -- limiting their applicability to embodied humanoid perception. We present Humanoid-OmniOcc, a large-scale panoramic stereo-based occupancy dataset tailored for humanoid robots. The dataset encompasses 15 diverse simulated indoor scenes and 5 real-world environments, yielding over 155K samples with broad scene and style diversity. Importantly, the dataset is designed around a Real2Sim2Real closed-loop paradigm: real sensor specifications drive physically accurate simulation, simulation produces large-scale annotated training data, and models trained in simulation are directly evaluated on real-world captures -- enabling iterative refinement of the sim-to-real pipeline. We further propose \textbf{H}umanoid \textbf{S}urround \textbf{S}tereo-guided \textbf{Occ}upancy model (Humanoid-OmniOcc) that exploits robust depth priors for accurate 2D-to-3D lifting. Extensive experiments show that Humanoid-OmniOcc consistently outperforms monocular baselines and generalizes well to both unseen simulated test scenes and real-world environments, validating the effectiveness of the Real2Sim2Real design. Code and data will be available upon acceptance at .

</details>


### [73] [Feed-forward Motion In-betweening for Any 4D](https://arxiv.org/abs/2606.22131)
...
TL;DR: 本文提出一种前馈式4D网格关键帧插值框架，利用帧级网格VAE将每帧编码为拓扑无关的潜变量token，并引入关键帧条件化的整流流模型（MMDiT骨干）在稀疏关键帧之间合成中间帧，实现任意4D网格的高效可控插值。


<details>
  <summary>Details</summary>
Motivation: 现有4D生成方法（如text-to-4D）依赖视频扩散先验的蒸馏或测试时优化，推理速度极慢；而前馈生成器虽快但时空可控性差，且短时生成在长序列中误差累积严重。本文旨在实现快速、可控、长时一致的4D网格插值。

Method: 提出帧级网格VAE，将每帧网格编码为拓扑无关的潜变量token（以参考网格锚定）；构建关键帧条件化的整流流模型（Rectified Flow），采用MMDiT骨干网络，以稀疏关键帧为条件生成非关键帧的潜变量，再通过VAE解码器重建网格。

Result: 在DyMesh16和DyMesh32基准上，该方法在插值质量和可控性上均优于现有方法，能够高效生成长时程4D动态序列。

Conclusion: 本文证明了前馈式关键帧条件化流模型在4D网格插值中的有效性，为快速、可控的4D动态生成提供了新范式，有望推动动画、游戏等领域的4D世界建模应用。

Classification reason: 该论文研究4D动态网格（随时间演化的3D几何）的关键帧插值生成，属于动态场景/4D世界建模范畴，与[空间3D/4D世界模型]（spatial_3d_4d_world_model）高度匹配。同时，其feed-forward生成范式也与多模态生成方向相关。

Importance reason: Directly advances world model and 4D dynamics with a feed-forward, controllable in-betweening method using flow matching and latent tokens. Signals: world model, 4D mesh interpolation, feed-forward, keyframe conditioning, rectified flow
...
### [74] [MemoryVAM: Integrating Memory into Video Action Model for Robot Manipulation](https://arxiv.org/abs/2606.20679)
...
TL;DR: MemoryVAM 为视频世界模型策略引入情景记忆机制，通过 Recap-Cue 模块将历史帧的 CLIP 嵌入压缩为记忆 token 并注入视频骨干和动作解码器，使策略能够感知任务进度并基于完整历史进行动作决策。在 LIBERO-Mem 上成功率从 5% 提升至 42.5%，在真实机器人上达到 75-80% 的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的视频世界模型策略仅依赖短时间窗口的观测进行动作预测，但在长程操作任务中，正确的动作往往依赖于更早的历史事件（非马尔可夫性），导致策略在长程任务中表现不佳。作者希望引入情景记忆机制，使策略能够感知任务进度并利用完整历史信息进行决策。

Method: 提出 MemoryVAM，核心是 Recap-Cue (RC) 模块，包含两个子模块：(1) Recap Compressor：基于 Perceiver 架构，将每帧 CLIP 嵌入压缩为紧凑的记忆 token；(2) Cue Gate：轻量级门控模块，根据记忆 token 和语言指令估计任务完成进度。记忆 token 通过交叉注意力注入视频骨干（UNet 或 DiT）和动作解码器。训练使用视频预测损失、delta 重建辅助损失和 episode 边界监督，无需逐帧进度标签。

Result: 在 LIBERO-Mem 基准上，MemoryVAM 将平均成功率从 5% 提升至 42.5%。在真实机器人实验中，计数任务成功率达 78.3%，空间回忆任务达 80.0%，顺序跟踪任务达 75.0%。该方法兼容 UNet 和 DiT 两种骨干网络。

Conclusion: MemoryVAM 通过情景记忆机制有效解决了视频世界模型策略在长程操作任务中的非马尔可夫性问题，显著提升了长程任务的成功率，并在真实机器人上验证了其有效性。该方法为将记忆机制融入视频世界模型策略提供了通用框架。

Classification reason: 该论文提出 MemoryVAM，为视频世界模型策略（video-world-model policy）引入情景记忆机制，以解决长程操作任务中的非马尔可夫性问题。论文核心是动作条件视频预测与未来观测预测，属于视频世界模型范畴，因此映射到 [CANONICAL] subtopic "视频世界模型" (video_world_model)。

Importance reason: Integrates episodic memory into video world models for robot manipulation, but focuses on robotics rather than core video generation or continuous multimodal modeling. Signals: video world model, episodic memory, robot manipulation, LIBERO-Mem benchmark, real robot validation

Abstract: Video-world-model policies learn action-relevant representations by predicting future observations. However, they condition on only a short observation window, which renders long-horizon manipulation non-Markovian when the correct action depends on earlier events that are no longer visible. We present MemoryVAM, an episodic memory mechanism for video-world-model policies. We employ a Recap-Cue (RC) module, in which a Perceiver-based Recap Compressor maps per-frame CLIP embeddings into compact memory tokens, and a lightweight Cue Gate estimates task completion from memory and language. These tokens are injected into both the video backbone and the action decoder, aligning policy imagination with episode progress and conditioning actions on history. Our model trains the memory module with video prediction, a delta-reconstruction auxiliary loss, and episode-boundary supervision, requiring no per-frame progress labels. The same mechanism applies to UNet and Diffusion Transformer (DiT) backbones by changing only the cross-attention injection interface. On LIBERO-Mem, our model improves average success from 5% to 42.5%. On real robots, it achieves 78.3% success on counting tasks, 80.0% on spatial recall, and 75.0% on sequential tracking. Project page:

</details>


### [75] [Reference-Free Assessment of Physical Consistency in World Model-based Video Generation](https://arxiv.org/abs/2606.22363)
*Yun Oh, Sukmin Yun*
...
TL;DR: 本文提出了一种无参考的物理一致性评估方法，结合相对和绝对两种方式评估世界模型生成视频的物理保真度。利用DROID-SLAM和SEA-RAFT量化物理不一致性，无需人类投票或真实参考。经相对一致性筛选的视频可将任务成功率提升8%以上，绝对评估还能实现时空定位，可视化物理伪影出现的位置和时间。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型生成的视频在物理一致性上存在缺陷，导致仿真环境无法准确复现VLA模型在真实世界中的任务成功率。而现有的评估方法要么需要昂贵的人工投票（Elo），要么需要不可获取的真实参考（FVD），缺乏高效、无参考的物理一致性评估手段。

Method: 提出无参考的物理一致性评估方法，结合相对评估（relative assessment）和绝对评估（absolute assessment）。利用DROID-SLAM进行视觉里程计估计，SEA-RAFT进行光流估计，通过分析视频中物体运动的物理合理性来量化不一致性。相对评估用于筛选物理一致的视频，绝对评估用于时空定位物理伪影。

Result: 经相对一致性评估筛选后的视频，在WorldGym/WorldEval等仿真环境中，VLA模型的任务成功率提升了超过8%，有效缩小了仿真与真实之间的差距。绝对评估能够可视化物理伪影发生的具体时间和空间位置。

Conclusion: 本文提出的无参考物理一致性评估方法能够有效量化世界模型生成视频的物理保真度，无需人工标注或真实参考，为世界模型视频生成的质量控制提供了实用工具，有助于缩小仿真到真实的差距。

Classification reason: 论文提出视频生成中物理一致性的无参考评估方法，核心关注世界模型生成视频的物理保真度评估，属于世界模型方向下的视频世界模型子主题（[CANONICAL] video_world_model）。同时涉及视频生成质量评估，也可归入多模态生成下的生成模型评测与数据集（generation_benchmark）。

Importance reason: Directly addresses world model video generation evaluation, a high-priority subtopic, with a novel reference-free method that improves task success rates. Signals: world model evaluation, physical consistency, reference-free assessment, spatio-temporal localization, simulation-to-reality gap
...
### [76] [BadDreamer: Transferable Backdoor Attacks against Video World Models for Autonomous Driving](https://arxiv.org/abs/2606.21172)
...
TL;DR: BadDreamer 提出了一种针对自动驾驶视频世界模型的可迁移时空后门攻击方法。通过在少量训练序列中植入"触发-擦除"模式（黄色外卖骑手在上下文帧中出现但在未来帧中被擦除），使模型学会在触发出现时生成骑手消失、道路畅通的虚假未来。这种被污染的未来感知表征会迁移到下游动作模块，导致不安全的路点预测。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的视频世界模型用于预测未来场景演化，其学习到的未来动态表征直接影响下游动作规划。然而，这类模型的训练时安全风险尚未被充分探索。作者旨在揭示视频世界模型在表征层面的安全漏洞，即通过污染模型学习到的转移动力学来间接操纵下游决策。

Method: BadDreamer 构建"触发-擦除"序列：在观察帧中保留黄色外卖骑手（物理触发），但在未来帧中将其擦除。在少量此类序列上微调视频世界模型后，模型学会隐藏的条件关联——当触发出现时，生成骑手消失、道路畅通的幻觉未来。该攻击不直接修改标签或动作监督，而是通过污染未来感知表征间接影响下游动作模块。

Result: 在开源感知-动作流水线上实例化了该攻击，展示了被污染的未来感知表征可以迁移到下游动作模块，诱导出不安全的非避让路点预测。

Conclusion: 揭示了自动驾驶视频世界模型在表征层面的安全风险，表明仅关注生成质量不足以确保模型安全，需要引入后门感知的验证机制。

Classification reason: 该论文针对自动驾驶中的视频世界模型提出后门攻击，属于视频世界模型的安全风险研究，与 [CANONICAL] subtopic "视频世界模型" 高度匹配。

Importance reason: Directly targets video world models for autonomous driving, a high-priority subtopic, with a novel backdoor attack that reveals critical security risks in learned future dynamics. Signals: video world model security, autonomous driving safety, transferable backdoor attack, spatio-temporal trigger, representation-level vulnerability
...
TL;DR: Wh0 提出利用生成式视频世界模型作为可扩展的 egocentric 人手操作数据源，生成 50k 集数据集 WM-H，通过手部运动重建和视觉编辑将其转化为机器人可训练监督信号，结合少量真实机器人数据，显著提升预训练 VLA 模型在灵巧操作任务上的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有灵巧操作数据来源存在规模与场景/具身对齐之间的权衡：遥操作数据对齐好但成本高，仿真可扩展但存在 sim-to-real 差距，真实 egocentric 视频可扩展但与机器人部署不对齐。需要一种既能大规模生成又能保持场景和具身对齐的数据来源。

Method: Wh0 框架包含两个阶段：(1) 使用生成式视频世界模型（基于语言、物体和场景条件）生成 WM-H 数据集——50k 集 egocentric 人手-物体交互视频；(2) 通过手部运动重建和视觉编辑将生成视频转化为机器人可训练的监督信号。将 WM-H 与少量真实机器人数据联合训练，使预训练 VLA 模型适应灵巧操作部署。

Result: 在 18 个真实世界灵巧操作任务上，相比仅在机器人数据上后训练的模型，Wh0 将未见任务的零样本成功率从 8.3% 提升至 38.9%。消融实验表明，可扩展生成和场景/具身对齐是性能提升的关键驱动因素。

Conclusion: Wh0 证明了生成式视频世界模型可以作为可扩展且可控的 egocentric 人手操作数据源，有效弥合了数据规模与机器人部署对齐之间的鸿沟，为灵巧操作提供了新的数据扩展范式。

Classification reason: 该论文使用生成式视频世界模型生成人手操作视频数据，属于视频世界模型的应用（视频生成式世界模型、动作条件视频生成），因此映射到 world_model 方向下的 [CANONICAL] subtopic "视频世界模型" (video_world_model)。同时，论文涉及视频生成与视觉编辑，也匹配 multimodal_generation 方向。

Importance reason: Directly advances world model and video generation for scalable robot data, aligning with top priority subtopics. Signals: world model for data generation, egocentric hand manipulation, scalable 50k dataset, improves VLA zero-shot, video world model as data source
...
### [78] [Beyond the Next Step: Variable-Length Latent World Models for Long-Horizon Planning](https://arxiv.org/abs/2606.21775)
...
TL;DR: 本文提出变长潜世界模型（VLWM），通过直接学习变长动作序列条件下的未来潜状态预测，替代传统单步递归预测，有效缓解长时序规划中的误差累积问题，在长程控制任务上平均提升13%。


<details>
  <summary>Details</summary>
Motivation: 现有JEPA风格的潜空间世界模型仅训练单步预测，长时序规划时需递归展开，导致误差累积且训练目标与下游规划任务不匹配。

Method: 提出VLWM框架，学习变长动作序列到未来潜状态的直接映射；设计课程训练策略，逐步扩展动作预测长度；在测试时设计适配变长预测能力的规划方法。

Result: 在长时序控制任务上，VLWM相比SOTA方法LeWM平均提升13%，在需要长程规划的任务上提升尤为显著。

Conclusion: VLWM为改进潜空间世界模型的长时序预测与规划提供了一种简单有效的范式。

Classification reason: 该论文提出变长潜空间世界模型（VLWM），用于长时序规划中的未来状态预测，属于世界模型方向。其核心是动作条件化的潜空间动力学预测，与 [CANONICAL] 视频世界模型（video_world_model）高度匹配，该子主题涵盖动作条件视频生成、未来预测与长时序建模。

Importance reason: Directly advances world model planning with variable-length latent prediction, a key priority for your long-horizon control and neural simulator interests. Signals: world model, latent dynamics, long-horizon planning, variable-length prediction, error accumulation mitigation
...
### [79] [Compression and Retrieval: Implicit Memory Retrieval for Video World Models](https://arxiv.org/abs/2606.23105)
...
TL;DR: 本文提出 Compression and Retrieval (CaR)，一种基于注意力驱动的隐式记忆检索机制，用于解决视频世界模型在复杂相机轨迹下的长时记忆一致性问题。通过位置编码注入视角信息实现灵活记忆检索，并引入轻量级上下文压缩网络高效处理长上下文。同时构建了 SceneFly 大规模合成数据集用于训练和评估。实验表明该方法在多个基准上达到 SOTA，并展现出对开放域场景的强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型在复杂相机轨迹下难以维持一致的长时记忆，传统方法依赖计算昂贵的上下文扩展或刚性的启发式检索机制，缺乏对不同轨迹和环境的泛化能力。

Method: 提出 CaR 框架：(1) 通过位置编码注入视角信息，利用注意力计算实现灵活的记忆检索；(2) 引入轻量级上下文压缩网络，以最小计算开销高效处理长上下文；(3) 构建 SceneFly 大规模合成数据集，包含逼真的相机轨迹和帧级标注。

Result: 在多个已建立基准上取得 SOTA 结果，并展现出对开放域场景的强泛化能力。

Conclusion: CaR 通过注意力驱动的隐式记忆检索机制有效解决了视频世界模型的长时记忆一致性问题，为复杂相机轨迹下的交互环境模拟提供了可行方案。

Classification reason: 该论文提出 CaR 方法，通过隐式记忆检索机制增强视频世界模型的长时记忆一致性，属于视频世界模型方向，与 [CANONICAL] video_world_model 子主题高度匹配。

Importance reason: Directly advances video world models with implicit memory retrieval, a top priority subtopic, and uses flow-matching-compatible attention mechanisms. Signals: video world model, implicit memory retrieval, long-term consistency, camera trajectory, SceneFly dataset
...
### [80] [P-JEPA: Procedural Video Representation Learning via Joint Embedding Predictive Architecture](https://arxiv.org/abs/2606.23256)
...
TL;DR: P-JEPA 提出一种与骨干网络无关的方法，通过将程序性视频映射到密集帧对齐的动作空间并预测池化掩码潜向量，实现对超过30分钟长视频的有效表征学习，在 EgoExo4D 细粒度动作分类上达到 SOTA，参数量比 LLM 方法少一个数量级且可实时运行。


<details>
  <summary>Details</summary>
Motivation: 程序性视频包含具有长程依赖关系的动作（如开关炉灶视觉相似但语义不同），现有视频基础模型因自注意力的二次复杂度无法支持长视频理解。需要一种能处理30分钟以上长视频、捕捉长程动作依赖的表征学习方法。

Method: 提出 Procedural Joint Embedding Predictive Architecture (P-JEPA)，将长视频表征学习问题简化为密集帧对齐动作空间上的预测任务。通过池化掩码潜向量预测，骨干网络无关，可兼容 VJEPA2.1、TSM、I3D 等特征提取器。

Result: 在 EgoExo4D、EgoProceL、Assembly101 三个数据集上评估，P-JEPA 在线性可分性、流式推理和时间动作分割上持续提升，在 EgoExo4D 细粒度动作分类上达到 SOTA，参数量比 LLM 方法少一个数量级，且可实时运行。

Conclusion: P-JEPA 通过动作空间预测范式有效解决了程序性视频的长程依赖建模问题，实现了高效的长视频表征学习，为具身智能辅助系统提供了实用的视频理解基础。

Classification reason: 该论文关注程序性视频表征学习，通过联合嵌入预测架构（JEPA）学习长视频中的动作依赖关系，用于活动理解、时空定位和预测控制，属于视频世界模型范畴。同时涉及视频理解与时序推理，因此也匹配 multimodal_understanding_generation 下的 video_understanding_temporal。主要方向选择 world_model 下的 [CANONICAL] video_world_model。

Importance reason: Strongly aligns with world model and video representation learning priorities, with efficient long-video handling and SOTA results. Signals: world model, procedural video, joint embedding predictive architecture, long-range dependencies, real-time inference
...
### [81] [Rethinking Object-Centric Representations for Video Dynamics Modeling](https://arxiv.org/abs/2606.23436)
...
TL;DR: 本文提出STAITUS框架，通过将每个slot显式解耦为外观和几何姿态（位置/尺度）两部分，解决了基于slot的视频动态建模中外观-姿态纠缠导致的物体身份不一致问题。同时引入自适应门控机制动态调整活跃slot数量以缓解过分割。在合成与真实基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于slot的视频动态建模方法强制slot嵌入在时间上保持一致以维持物体身份，但外观与姿态的纠缠使得一致性目标与物体运动/视角变化冲突，导致slot锁定静态背景、前景物体碎片化或身份频繁交换。本文旨在解决这一矛盾。

Method: 提出STAITUS框架：1) 将每个slot显式解耦为外观表示和几何姿态（位置/尺度）两部分；2) 利用解耦实现帧内空间分离，仅在appearance空间施加时间对齐约束；3) 引入自适应门控机制，根据场景复杂度动态调整活跃slot数量，缓解过分割。

Result: 在合成和真实世界基准上，STAITUS在分割质量和跟踪稳定性方面大幅超越现有最先进方法，在运动、遮挡、物体进出场景等挑战性条件下表现出更清晰的掩码和更持久的身份保持。

Conclusion: 外观与几何姿态的解耦是提升基于slot的视频动态建模中物体身份一致性的有效途径，自适应门控机制进一步提升了模型对场景复杂度的适应能力。

Classification reason: 该论文提出STAITUS框架，通过解耦外观与几何姿态的slot表示，实现无监督视频物体跟踪与动态场景分解。核心任务是视频动态建模（video dynamics modeling），属于视频世界模型范畴，与[视频世界模型]子主题高度匹配。

Importance reason: Directly advances world model and video dynamics modeling with a novel disentangled slot representation, aligning with top user priorities. Signals: world model, video dynamics, object-centric representation, disentangled appearance-pose, adaptive gating
...
### [82] [From Uncertainty to Stability and Fidelity: Guiding Sparse-View 3D Gaussian Splatting with Fisher Information](https://arxiv.org/abs/2606.20842)
...
TL;DR: 本文提出一种基于Fisher Information引导的稀疏视角3DGS方法，通过主动选择信息量最大的支撑视图进行立体增强，并利用Fisher Information自适应调整高斯点的Dropout概率，从而减少随机性带来的不稳定，显著提升稀疏视角下的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 稀疏视角下3DGS容易过拟合，现有方法（如随机视角增强、随机Dropout）缺乏原则性指导，引入的随机性导致不稳定和渲染质量下降。需要一种量化引导机制来提升稀疏视角3DGS的稳定性和保真度。

Method: 提出两个核心组件：(1) 基于Fisher Information的立体增强——利用Fisher Information主动选择信息量最大的支撑视图，结合深度先验生成可靠的伪真值，减少增强中的随机性；(2) 不确定性感知正则化——用Fisher Information量化每个3D高斯的不确定性，自适应调整其被移除的概率，使Dropout正则化更稳定有效。

Result: 在多个稀疏视角新视角合成基准上达到SOTA性能，有效缓解过拟合，显著提升渲染保真度和优化稳定性。

Conclusion: Fisher Information能够有效指导稀疏视角3DGS中的先验利用和正则化策略，减少随机性带来的不稳定性，为稀疏视角3DGS提供了一种原则性的优化框架。

Classification reason: 该论文研究稀疏视角下的3D高斯泼溅（3DGS）新视角合成，属于3D场景生成与重建范畴，与[CANONICAL] subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Focuses on 3DGS sparse-view rendering, not directly aligned with user's priority subtopics like continuous language models or video generation. Signals: 3D Gaussian Splatting, sparse-view, Fisher Information, novel view synthesis, rendering fidelity
...
### [83] [ACEsplat: Accelerated 3D Gaussian Scene Regression via RGB and Poses Only](https://arxiv.org/abs/2606.22091)
...
TL;DR: ACEsplat 提出了一种仅需 RGB 图像和相机位姿、无需外部几何先验（如 SfM 点云或深度图）的快速 3DGS 场景重建框架。通过两阶段流水线——自监督场景坐标回归（SCR）构建内部几何先验，再融合 SCR 特征进行轻量级高斯初始化与优化——在 15-25 分钟内完成单场景重建，在 Wayspots 和 Cambridge Landmarks 等数据集上取得了有竞争力的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 实际机器人、AR 等场景部署中，3DGS 通常依赖 SfM 点云或深度估计等外部几何初始化，这些过程缓慢且脆弱，限制了现场快速部署。作者希望设计一种仅依赖 RGB 图像和相机位姿、无需外部 3D 先验的快速场景重建方案。

Method: ACEsplat 采用两阶段流水线：(1) 自监督场景坐标回归（SCR）模块，在 4-5 分钟内从 RGB 图像和位姿中构建内部几何先验；(2) 轻量级高斯初始化头融合 SCR 特征和坐标先验，随后进行逐场景 3DGS 优化。整个过程无需 SfM 或深度监督。

Result: 在 Wayspots 数据集上使用实时 SLAM 位姿达到 29.11 dB PSNR，在 Cambridge Landmarks 上使用 SfM 精化位姿达到 33.20 dB PSNR。在 RealEstate10K 稀疏视角新视图合成（2 视角设置）中取得有竞争力的图像保真度。单 GPU 上 15-25 分钟完成场景 SCR 映射和 3DGS 重建。

Conclusion: ACEsplat 证明了仅凭 RGB 图像和相机位姿即可实现快速、高质量的 3DGS 场景重建，无需外部几何先验，为机器人和混合现实应用中的快速场景部署提供了实用解决方案。

Classification reason: 该论文提出 ACEsplat，一种仅依赖 RGB 图像和相机位姿的快速 3DGS 场景重建框架，属于 3D 场景重建与渲染方向，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Focuses on 3DGS acceleration for robotics, not on user's priority subtopics like continuous language models or autoregressive video/image generation. Signals: 3D Gaussian Splatting, scene reconstruction acceleration, RGB+pose only, robotics/AR application, no external geometry priors
...
### [84] [ACE-GS: Acing the Trade-off with Accurate, Compact and Efficient 3D Gaussian Splatting](https://arxiv.org/abs/2606.21244)
*Jijian Zhao*
...
TL;DR: ACE-GS 提出了一种渐进式优化框架，通过动量一致性引导的稠密化策略、统计敏感性驱动的稀疏化机制以及跨维度残差频率补偿方案，在 3D Gaussian Splatting 中实现了渲染质量、压缩率和训练速度三者之间的最佳平衡，仅需 3-5 分钟即可收敛，PSNR 提升最高达 0.89 dB，训练加速最高达 3.7 倍。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) 虽然实现了出色的实时渲染，但其计算和存储开销巨大，限制了广泛部署。现有的加速范式往往通过激进剪枝来快速收敛，导致高频细节严重丢失。如何在保持高质量渲染的同时实现超快重建速度，是一个尚未解决的根本性问题。

Method: ACE-GS 包含三个核心组件：(1) 动量一致性引导的稠密化策略（Momentum Consistency-guided Densification），严格约束原始生长到真实的几何流形上，避免计算浪费并加速收敛；(2) 统计敏感性驱动的稀疏化机制（Statistical Sensitivity-driven Sparsification），精确剪枝冗余原始体，压缩存储占用；(3) 跨维度残差频率补偿方案（Cross-dimensional Residual Frequency Compensation），显式地将高频误差能量注入原始体属性中，恢复锐利的几何细节。

Result: 在保持高度紧凑的场景表示的同时，ACE-GS 相比快速框架 Speedy-Splat 实现了最高 3.7 倍的训练加速。仅需 3-5 分钟即可收敛，取得了最高的结构相似性（SSIM），PSNR 相比原始 3DGS 提升最高达 0.89 dB，为超快、高保真的新视角合成建立了新基准。

Conclusion: ACE-GS 通过精确的原始体管理策略，成功打破了 3DGS 中渲染质量与重建速度之间的权衡，证明了精细的原始体管理是实现高质量、高压缩与高效率三者平衡的关键。该方法为 3D 场景表示的实用化部署提供了有效方案。

Classification reason: 该论文提出 ACE-GS，一种针对 3D Gaussian Splatting 的渐进式优化框架，旨在实现高质量、高压缩率与超快重建速度的平衡。论文核心贡献在于 3DGS 的加速、压缩与细节保持，属于 3D 场景表示与渲染的经典方向，因此映射到 [CANONICAL] 的 "3D 生成与重建" 子主题。

Importance reason: Focuses on 3D Gaussian Splatting optimization, which is outside the user's priority subtopics and direction weights. Signals: 3D Gaussian Splatting, efficient rendering, compression, training acceleration, novel view synthesis
...
### [85] [DreamUV: Unwrap Artist-like UV by End-to-End Flow Matching](https://arxiv.org/abs/2606.22445)
...
TL;DR: DreamUV 提出了一种端到端的 Flow Matching 框架，将 UV 展开建模为生成式流匹配问题，学习从噪声到艺术家风格 UV 布局的传输过程，生成具有更直边界和更紧轴对齐岛屿的 UV 布局。


<details>
  <summary>Details</summary>
Motivation: 传统 UV 展开方法优化手工设计的能量函数，难以捕捉专业艺术家偏好的结构模式（如笔直接缝、轴对齐岛屿、灵活的内部变形），这些模式难以显式公式化。现有方法生成的 UV 布局与生产级标准之间存在差距。

Method: DreamUV 将 UV 展开建模为网格条件化的 Flow Matching 生成问题，学习从噪声分布到艺术家风格 UV 布局的传输过程。采用边界感知训练策略优先处理接缝几何，并引入 Model-in-the-Loop Finetuning (MITL) 方案显式考虑采样过程中的离散化误差，稳定异构监督下的传输动力学。

Result: 在大规模专业 UV 布局数据集上的实验表明，DreamUV 生成的边界显著更直、轴对齐岛屿更紧凑，优于经典方法和学习方法，同时保持有竞争力的失真指标。专业艺术家的用户研究进一步验证了其布局符合实际生产需求。

Conclusion: DreamUV 证明了将 UV 展开作为生成式 Flow Matching 问题的有效性，能够学习到艺术家风格的 UV 布局模式，弥合了几何失真目标与专业审美偏好之间的差距，为 3D 内容创建中的 UV 参数化提供了新的范式。

Classification reason: 该论文提出 DreamUV，一种基于 Flow Matching 的端到端 UV 展开框架，属于 3D 内容创建中的 UV 参数化问题，与 [CANONICAL] 3D 生成与重建（three_d_generation）方向高度相关。

Importance reason: Novel flow matching for UV unwrapping, but outside user's priority subtopics and negative keywords not triggered. Signals: flow matching, 3D generation, UV parameterization, artist-like layout, end-to-end learning
...
### [86] [Arbor: Explicit Geometric Conditioning for Controllable 3D Asset Generation](https://arxiv.org/abs/2606.23514)
...
TL;DR: Arbor 是一种面向可控 3D 资产生成的训练附件，通过引入约束网格（hull/avoidance/touch 区域）作为原生 3D 控制接口，将约束网格编码为 token 并路由到冻结去噪器的对应潜空间区域，在不牺牲生成质量和多样性的前提下显著提升空间约束遵从度。


<details>
  <summary>Details</summary>
Motivation: 现有文本/图像条件 3D 生成模型缺乏对物体空间位置和形状的直接控制能力，而创作过程中空间意图（如椅子应适配坐姿空间、道具应留出运动间隙等）往往是预先已知的。文本和图像提示难以精确表达这类空间约束，亟需一种显式的 3D 控制接口。

Method: Arbor 在冻结的文本条件潜空间 3D 生成器（denoiser）上附加一个可训练模块。它将约束网格（hull 区域、avoidance 区域、touch 区域）转换为 token，并通过路由机制将不同空间位置的约束信号注入到去噪器中对应的潜空间区域，使每个潜区域只接收与其空间位置相关的约束信息，无需额外的遵从损失函数。

Result: 在自动评测和艺术家策划的控制基准上，Arbor 在 hull、avoidance、touch 三类约束下均显著提升了约束遵从度，同时保持了生成质量和多样性。用户偏好研究也验证了其有效性。

Conclusion: Arbor 证明了通过约束网格作为显式 3D 控制接口，可以在不牺牲生成质量和多样性的前提下有效提升 3D 资产生成的空间可控性，为 3D 内容创作提供了更直观、精确的控制方式。

Classification reason: 该论文提出 Arbor，一种通过约束网格（constraint meshes）对文本条件 3D 生成进行空间可控生成的方法，属于 3D 物体生成与可控生成范畴，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Strong 3D generation control method but outside user's priority subtopics and negative for direct reading. Signals: 3D asset generation, explicit geometric control, constraint mesh interface, frozen denoiser routing, user study validation
...
### [87] [Mesh2GS: White-Box 3DGS Construction via Plenoptic Sampling](https://arxiv.org/abs/2606.21898)
...
TL;DR: Mesh2GS 提出一种基于全光采样理论的白色盒 3DGS 构建框架，直接从网格几何生成 3D 高斯分布，实现奈奎斯特级别的全局光照渲染，并引入反照率-明暗分解与神经光照增强模块处理非朗伯效应。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注从多视角图像重建 3DGS 或从 3DGS 重建网格，而将网格转换为 3DGS 的问题较少被研究。启发式地将 3D 高斯绑定到网格的策略缺乏理论保证，无法实现高质量全局光照渲染。

Method: 1) 基于全光采样理论推导采样视图的最小采样率与 3D 高斯的分布策略；2) 提出反照率-明暗分解的 3DGS 更新过程以高效捕获全局光照；3) 引入神经光照增强模块处理非朗伯效应（如镜面高光）。

Result: 在多个基准上超越现有方法，能够实现实时共享渲染，并有效捕获非朗伯效应和镜面高光。

Conclusion: Mesh2GS 提供了一种从网格到 3DGS 的理论驱动转换方法，在渲染质量和效率上均优于启发式基线，为 3DGS 与网格表示的桥接提供了新思路。

Classification reason: 该论文提出从网格（Mesh）到 3DGS 的转换框架，属于 3D 场景表示与渲染方法，与 [CANONICAL] 的 "3D 生成与重建" 子主题高度匹配。

Importance reason: Novel white-box 3DGS construction from mesh with theoretical sampling, but outside user's priority subtopics and negative for 3D reconstruction. Signals: 3D Gaussian Splatting, mesh-to-3DGS, plenoptic sampling, global illumination, non-Lambertian effects
...
### [88] [VolHuMe: a High-Resolution Large Scale Dataset of Volumetric Human Meshes](https://arxiv.org/abs/2606.23062)
...
TL;DR: VolHuMe 是一个高质量4D人体扫描数据集，包含104名受试者，使用64个RGB和32个深度相机采集，提供SMPL-X、高分辨率网格、多视图RGB/深度图像、服装分割、手部和面部几何等丰富标注，并在多个3D/4D人体重建方法上进行了基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有4D人体数据集在几何精度和纹理分辨率上存在局限，缺乏细粒度身体部位细节的高质量标注。作者旨在构建一个高分辨率、多模态标注的4D人体数据集，以推动3D/4D人体重建研究并暴露当前评测基准的不足。

Method: 使用64个RGB和32个深度相机构建体积捕捉工作室，对104名受试者进行近距离高分辨率扫描。数据集提供SMPL-X参数模型、高分辨率网格、绑定网格、点云、多视图RGB/深度图像、服装分割掩码以及精细的手部和面部几何信息。

Result: 在多个SOTA 3D/4D人体重建方法上进行了基准测试，展示了数据集的高质量，同时揭示了现有评测基准在细粒度几何和纹理评估方面的局限性。

Conclusion: VolHuMe 为4D人体重建提供了高质量、多模态的基准数据集，有助于推动该领域的发展，并为未来更精细的人体建模研究提供了重要资源。

Classification reason: 该论文提出了一个高分辨率4D人体扫描数据集 VolHuMe，用于3D/4D人体重建任务的评测。这属于3D生成与重建方向下的数据集/评测基准工作，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: High-quality 4D human dataset but outside user's priority subtopics and negative keywords not triggered. Signals: 4D human scans, high-resolution meshes, SMPL-X, garment segmentation, benchmark for 3D/4D reconstruction
...
### [89] [MeshFlow: Mesh Generation with Equivariant Flow Matching](https://arxiv.org/abs/2606.23489)
...
TL;DR: MeshFlow 提出了一种基于等变最优传输流匹配的三角网格直接生成方法，利用置换等变性处理网格的面和顶点对称性，在保持与自回归方法相当质量的同时实现约 18 倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 网格是常见的 3D 场景表示，但直接生成网格面临对称性挑战（面的置换不变性、顶点在面内的置换不变性）。现有方法通常将网格序列化为长自回归序列，效率低下且破坏了这些对称性。本文旨在设计一种直接生成网格且尊重其固有对称性的高效方法。

Method: 将网格表示为 triangle soup（无序三角面集合），采用等变最优传输流匹配模型来建模速度场。对 Diffusion Transformer 架构进行简单而有效的修改，使其在保持所需等变性的同时可扩展地建模速度场。引入基于最优传输的训练目标，通过消除违反对称性的监督信号来改善收敛。

Result: MeshFlow 在网格生成质量上与最先进的自回归网格生成器相当，同时推理速度提升约 18 倍。

Conclusion: 通过等变流匹配直接生成三角网格是可行且高效的，尊重网格固有对称性的设计既能保证生成质量又能大幅加速推理，为 3D 网格生成提供了新的高效范式。

Classification reason: 该论文提出 MeshFlow，直接生成三角网格（triangle mesh），属于 3D 物体/场景生成范畴。现有 [CANONICAL] subtopic "3D 生成与重建" 完全覆盖该主题，因此直接复用。

Importance reason: Strong 3D generation method with flow matching and equivariance, but outside user's priority subtopics. Signals: 3D mesh generation, flow matching, equivariant optimal transport, 18x inference speedup, Diffusion Transformer modification
...
### [90] [Multi4D: High-Fidelity Dynamic Gaussian Splatting via Multi-Level Competitive Allocation](https://arxiv.org/abs/2606.22197)
...
TL;DR: Multi4D 提出了一种多级竞争分配框架用于高保真动态高斯泼溅，通过静态结构、持久动态几何和瞬态外观三个层次自适应分配建模能力，在保持运动一致性的同时捕捉精细动态细节，实现了SOTA渲染质量和实时性能，并支持高效的4D语义分割。


<details>
  <summary>Details</summary>
Motivation: 现有动态3D高斯泼溅方法在运动一致性与视觉保真度之间存在根本性矛盾：基于形变的方法保持时序对应但过度平滑高频动态，而4D基元方法能捕捉精细细节却导致时序过参数化、破坏物体同一性并带来巨大存储开销。需要一种既能保持长期运动一致性又能捕捉精细动态细节的统一框架。

Method: Multi4D 将建模能力分配到三个结构化层次：静态结构基元、持久动态几何基元和瞬态外观基元。通过共享光栅化和残差驱动的优化，这些层次动态竞争以解释光度误差，无需预分配即可实现自适应特化。显式追踪紧凑的持久高斯基元，后续可嵌入语义特征。

Result: 在多个动态场景基准上达到SOTA渲染质量，实现实时性能，且使用的动态基元数量显著更少。在4D语义分割任务上达到SOTA精度，同时实现一个数量级的加速。

Conclusion: 多级竞争分配策略有效解决了动态高斯泼溅中运动一致性与视觉保真度的矛盾，为高保真动态场景建模提供了高效且可扩展的解决方案，同时自然支持下游语义分割任务。

Classification reason: 该论文研究动态3D高斯泼溅（4D Gaussian Splatting），属于多模态生成方向下的4D与动态场景生成（dynamic_4d_generation）这一[CANONICAL]子主题，完全匹配。

Importance reason: Strong 4D Gaussian splatting paper but outside user's priority subtopics and lacks direct connection to continuous language/multimodal models or autoregressive generation. Signals: Dynamic Gaussian splatting, Multi-level competitive allocation, High-fidelity rendering, Real-time performance, 4D semantic segmentation

Abstract: Dynamic 3D Gaussian splatting faces a fundamental tension between motion consistency and visual fidelity. Deformation-based approaches preserve temporal correspondence but suffer from motion over-factorization, oversmoothing high-frequency dynamics. In contrast, 4D-primitive methods capture fine visual details yet incur temporal overparameterization, breaking object identity and leading to severe storage overhead. To resolve this, we introduce Multi4D, a framework for high-fidelity dynamic Gaussian Splatting based on multi-level competitive allocation. Instead of a monolithic representation, we distribute modeling capacity across three structured levels: static structure, persistent dynamic geometry, and transient appearance primitives. Through shared rasterization and residual-driven optimization, these levels dynamically compete to explain photometric error, enabling adaptive specialization without pre-assigned decomposition. This allocation preserves long-term motion consistency while capturing fine dynamic detail, achieving state-of-the-art rendering quality and real-time performance with significantly fewer dynamic primitives. Furthermore, because our representation explicitly tracks compact persistent Gaussians over time, semantic features can be embedded afterward, enabling Multi4D to achieve state-of-the-art 4D segmentation accuracy with an order-of-magnitude speedup. Project page:

</details>


### [91] [DrivingVoxels: Compositional Sparse Voxel Rasterization for Dynamic Driving Scene Reconstruction](https://arxiv.org/abs/2606.23031)
...
TL;DR: DrivingVoxels 提出了一种基于组合稀疏体素光栅化的动态驾驶场景重建框架，利用多个独立八叉树分别表示静态背景和每个动态刚体对象，通过 LiDAR 引导的结构初始化实现高效优化，在 PandaSet 上达到与 3DGS 方法相当或更优的重建质量且训练时间更短。


<details>
  <summary>Details</summary>
Motivation: 现有动态驾驶场景重建方法中，稀疏体素方法主要针对静态场景设计，而基于 3DGS 的动态方法虽然保真度高但训练耗时且在大场景中内存增长不可控。因此需要一种既能高效处理动态场景又能控制内存开销的渲染框架。

Method: DrivingVoxels 采用组合式稀疏体素渲染框架：每个刚体动态对象用一个局部坐标系下的独立八叉树表示，静态背景用另一个八叉树表示，所有八叉树在单次渲染中联合光栅化。该方法采用完全显式、无神经网络的表示，并利用 LiDAR 点云引导的结构初始化高效捕捉场景几何结构。

Result: 在 PandaSet 基准上，DrivingVoxels 在新视角合成和重建任务中，感知指标与 3DGS 方法相当，结构指标更优，同时由于 LiDAR 先验引导的高效优化流程，训练时间显著缩短。

Conclusion: DrivingVoxels 证明了组合稀疏体素光栅化在动态驾驶场景重建中的有效性，在保持高质量重建的同时大幅提升了训练效率，为动态城市场景的实时渲染提供了新的方向。

Classification reason: 该论文关注动态驾驶场景的重建与渲染，属于动态3D场景生成与重建范畴，与 [CANONICAL] subtopic "4D 与动态场景生成" (dynamic_4d_generation) 高度匹配。

Importance reason: Focuses on driving scene reconstruction with sparse voxels, not aligned with user's priority subtopics like continuous language models or autoregressive generation. Signals: dynamic scene reconstruction, sparse voxel rasterization, LiDAR-guided initialization, not in priority subtopics, no boost keywords matched
...
### [92] [Temporally Aware Densification for Dynamic 3D Gaussian Splatting](https://arxiv.org/abs/2606.23212)
...
TL;DR: 本文提出 Visibility-Aware Densification (VAD) 框架，通过时序可见性感知、自适应阈值调整和时序偏移变形，改进了动态3D高斯泼溅中的致密化策略，显著提升了动态区域的视觉质量，并可作为即插即用模块推广到多种动态3DGS方法。


<details>
  <summary>Details</summary>
Motivation: 现有动态3DGS方法沿用了静态场景的致密化策略，忽略了时序行为，导致短寿命高斯体因监督稀疏而无法有效致密化，动态区域出现重建不足和模糊问题。

Method: 提出 VAD 框架，包含三个核心组件：(1) Visibility-Aware Densification (VAD) —— 将时序可见性融入致密化过程，根据高斯体的实际时序存在进行细化；(2) Temporally-Adaptive Thresholding (TAT) —— 根据高斯体的时序寿命自适应调整致密化阈值，促进静态与动态区域的平衡细化；(3) Temporal Offset Warping (TOW) —— 增强时序中心附近的变形能力，延长高动态高斯体的寿命，促进更有效的致密化。

Result: 在三个动态多视角基准数据集上，该方法在动态区域视觉质量上显著优于现有方法，且 VAD 模块作为即插即用组件能一致提升多种动态3DGS方法的动态重建效果。

Conclusion: 时序感知的致密化策略对动态场景重建至关重要，所提方法有效解决了动态3DGS中因静态致密化策略导致的动态区域重建不足问题，具有良好的泛化性。

Classification reason: 该论文研究动态3D高斯泼溅（Dynamic 3DGS）中的时序感知致密化方法，属于动态3D场景重建与生成范畴，与[CANONICAL] subtopic "4D 与动态场景生成"（dynamic_4d_generation）高度匹配。

Importance reason: Focuses on dynamic 3DGS densification, which is not a priority subtopic and lacks direct connection to user's core interests in continuous language/multimodal models or autoregressive generation. Signals: Dynamic 3D Gaussian Splatting, Visibility-Aware Densification, Temporal motion, Not in priority subtopics, No boost keywords matched
...
### [93] [Ocean4D: Generative Underwater 4D Reconstruction via Medium-Aware Video Diffusion](https://arxiv.org/abs/2606.23298)
...
TL;DR: Ocean4D 提出了一种基于介质感知视频扩散的水下4D重建生成框架，通过4D几何一致条件构建和隐式介质感知去噪模块，从单目视频生成跨视角一致的动态水下场景重建结果。


<details>
  <summary>Details</summary>
Motivation: 现有4D重建方法大多假设空气环境，未显式建模水下吸收和背向散射效应；同时近静态假设对动态粒子与干扰物敏感，导致几何不稳定和跨视角不一致。亟需一种能处理水下复杂光传输和动态变化的重建方法。

Method: Ocean4D 包含两个互补组件：1) 4D-GCC 构建4D几何一致条件，提升跨帧覆盖；2) Medium-Aware Block 在潜扩散过程中进行隐式介质感知去噪，稳定水下外观。输入单目视频和目标相机参数，生成沿目标轨迹的视频并保持全局结构和跨视角一致性。

Result: 在动态和静态水下基准上均达到最先进性能，有效处理了水下吸收、散射和动态干扰物问题，生成几何稳定、跨视角一致的4D重建结果。

Conclusion: Ocean4D 通过将介质感知机制引入视频扩散模型，成功实现了水下动态场景的4D重建，为复杂水下环境中的生成式重建提供了有效范式。

Classification reason: 该论文提出水下4D重建生成框架，属于动态场景生成与重建范畴，与 [CANONICAL] subtopic "4D 与动态场景生成" (dynamic_4d_generation) 高度匹配。

Importance reason: Niche underwater 4D reconstruction with medium-aware diffusion, but lacks direct alignment with priority subtopics like continuous language models or autoregressive generation. Signals: Underwater 4D reconstruction, Video diffusion, Medium-aware denoising, Generative framework, Cross-view consistency
...
### [94] [Lift4D: Harmonizing Single-View 3D Estimation for 4D Reconstruction In-the-Wild](https://arxiv.org/abs/2606.23688)
...
TL;DR: Lift4D 提出一种测试时优化框架，从单目视频中重建动态非刚体4D场景。它先通过因果潜在条件使单视图3D重建模型生成时序一致的逐帧预测，作为可变形3D高斯泼溅的初始化；再通过遮挡感知优化，结合视图条件扩散先验，忠实恢复可见表面细节并补全未观测区域，在复杂野外场景上显著优于先前方法。


<details>
  <summary>Details</summary>
Motivation: 现有4D重建方法要么直接从视觉输入预测4D表示（受限于4D训练数据稀缺），要么先初始化3D表示再基于视频变形优化（仅利用先验进行初始重建，后续仅依赖视频监督）。两者在存在大形变和遮挡的复杂野外场景中表现不佳。Lift4D旨在同时克服这两个局限。

Method: 1) 因果潜在条件：对单视图3D重建模型进行时序适配，通过因果潜在条件化生成时序一致的逐帧预测，提供可变形3D高斯泼溅的连贯初始化。2) 遮挡感知优化：通过遮挡感知的优化过程"雕刻"该表示以匹配输入视频，忠实恢复可见表面细节，同时使用视图条件扩散先验补全未观测区域。

Result: 在具有严重遮挡和非刚体运动的挑战性野外序列上，Lift4D显著优于先前的4D重建方法，能够更好地恢复可见表面细节并合理补全被遮挡区域。

Conclusion: Lift4D通过结合时序一致的初始化与遮挡感知的扩散先验优化，有效解决了单目视频中动态非刚体4D重建的难题，特别是在复杂野外场景下取得了明显改进。

Classification reason: 该论文从单目视频重建动态非刚体4D场景，使用可变形3D高斯泼溅表示，属于4D动态场景生成与重建方向，与[CANONICAL] subtopic "4D 与动态场景生成"（dynamic_4d_generation）高度匹配。

Importance reason: Novel test-time optimization for 4D reconstruction from monocular video, but not aligned with user's priority subtopics. Signals: 4D reconstruction, single-view video, deformable 3D Gaussian Splatting, occlusion-aware optimization, diffusion prior
...
### [95] [MythraGen: Two-Stage Retrieval Augmented Art Generation Framework](https://arxiv.org/abs/2606.22924)
...
Importance: 普通相关 (64.81) - 未入选精读

TL;DR: MythraGen 提出一种两阶段检索增强生成框架，先从大规模艺术数据集中检索与文本提示最相似的图像，再用这些图像通过 LoRA 微调 Stable Diffusion，生成符合用户输入的艺术风格图像。


<details>
  <summary>Details</summary>
Motivation: 现有文生图模型在生成高质量、上下文准确且忠实匹配文本描述的艺术图像方面仍存在挑战，尤其是需要特定艺术家风格的艺术生成任务。

Method: 两阶段框架：(1) 检索阶段：从 WikiArt 数据集中提取特征，检索与查询提示最相似的图像；(2) 生成阶段：用检索到的图像通过 LoRA 微调 Stable Diffusion，生成符合用户输入的艺术作品。

Result: 在 WikiArt 数据集上的实验和用户研究表明，该方法能生成与用户输入高度匹配的艺术作品，显著优于现有方案。

Conclusion: 检索增强结合 LoRA 微调是一种简单高效的艺术图像生成方法，能够有效提升生成图像与文本描述之间的一致性及艺术风格保真度。

Classification reason: 该论文通过检索外部艺术数据库中的相似图像，结合 LoRA 微调 Stable Diffusion 实现特定艺术风格的文本到图像生成，属于参考图引导的个性化/风格化生成，与 [CANONICAL] subtopic "主体与概念驱动生成" (customization_generation) 高度匹配。

Importance reason: The paper focuses on retrieval-augmented art generation, which is not aligned with the user's priority subtopics like continuous language models, video generation, or world models. Signals: retrieval augmented generation, LoRA fine-tuning, artistic image generation, Stable Diffusion, not aligned with priority subtopics
...
### [96] [TeleStyle V2: Beyond Content-Preserving Style Transfer with Self-Distillation and Distribution-Matching-Distillation](https://arxiv.org/abs/2606.20709)
...
TL;DR: TeleStyle V2 提出自蒸馏数据合成策略和分布匹配蒸馏（DMD）方法，将风格迁移模型扩展到支持四种内容-风格组合（真实/艺术交叉），并保持通用图像编辑能力，性能与商业模型 Gemini-3-Pro 相当。


<details>
  <summary>Details</summary>
Motivation: TeleStyle V1 仅支持真实内容+艺术风格，无法处理艺术内容+真实风格等组合；且 SFT 微调后通用图像编辑能力退化，内容一致性下降。

Method: 1) 自蒸馏数据合成：利用 TeleStyle V1 生成伪三元组（内容、风格、输出）来训练 V2；2) 分布匹配蒸馏（DMD）：保留基础模型的通用文本引导图像编辑能力，缓解 SFT 导致的内容一致性退化；3) 引入 prompt enhancer 解决内容/风格参考顺序混淆问题；4) 使用 Qwen2.5-VL-7B 作为 VLM 编码器自动生成内容提示和风格提示。

Result: TeleStyle V2 支持 RnR、RnS、SnR、SnS 四种风格迁移模式；TeleStyleV2-QIE-2509-DMD 在通用图像编辑任务上与 Qwen-Image-Edit-2509-DMD 性能相当；风格迁移效果与商业模型 Gemini-3-Pro-Image-Preview 可比。

Conclusion: 自蒸馏策略有效扩展了风格迁移的适用范围，DMD 能保持通用编辑能力，提示增强器解决了顺序混淆问题，整体方法在风格迁移和通用编辑之间取得了良好平衡。

Classification reason: 该论文研究内容保持的风格迁移（content-preserving style transfer），属于图像编辑（图像风格化）范畴，与 [CANONICAL] subtopic "visual_editing（图像与视频编辑）" 高度匹配。

Importance reason: Style transfer is a niche subtask not aligned with user's priority subtopics like continuous models or video generation. Signals: Style transfer, Self-distillation, Distribution matching distillation, General image editing, Not in priority subtopics
...
### [97] [ChronoLock: Protecting Videos from Unauthorized Text-to-Video Personalization](https://arxiv.org/abs/2606.21146)
...
TL;DR: ChronoLock 是首个针对文生视频个性化微调（T2V personalization）的主动保护框架，通过在视频上添加精心设计的对抗性扰动，破坏时序去噪轨迹中的运动学习过程，从而阻止他人利用公开视频进行未经授权的 T2V 个性化训练。


<details>
  <summary>Details</summary>
Motivation: 随着文生视频扩散模型和个性化微调技术的发展，用户上传到网络的视频可能被收集并用于未经授权的 T2V 个性化训练，模仿视频中的主体、风格或运动模式，造成数据滥用风险。现有保护性扰动主要针对图像识别或文生图个性化，仅破坏静态外观线索，无法有效防御视频个性化中对时序去噪动力学的利用。

Method: ChronoLock 通过优化时序去噪轨迹上的有界扰动来破坏运动学习过程。具体包括：(1) 破坏块内时序适应（intra-chunk temporal adaptation），使用结合拟合误差、帧间去噪关系和相邻帧变化的扩散目标函数；(2) 放大块间边界不匹配（inter-chunk boundary mismatch），削弱长程运动连续性；(3) 采用变换采样更新（transformation-sampled updates）增强对常见预处理的鲁棒性。

Result: 在 UCF Sports 和 HMDB51 数据集上，使用流行的 T2V 骨干网络和个性化方案进行实验，ChronoLock 在自动评估指标和人工评估下均有效降低了运动模仿的质量。

Conclusion: ChronoLock 是首个针对 T2V 个性化微调的视频保护框架，通过直接破坏时序去噪轨迹中的运动学习过程，有效防御了未经授权的视频个性化生成，为视频数据版权保护提供了新思路。

Classification reason: 该论文提出针对文生视频个性化微调的保护性扰动方法，属于对视频内容的保护性编辑/扰动，与图像与视频编辑（visual_editing）这一[CANONICAL]子主题相关，因为其核心是在视频上添加不可见扰动以阻止未经授权的个性化生成，属于视频内容保护性编辑范畴。

Importance reason: Focuses on video protection rather than core generation or world model advances, with no alignment to priority subtopics. Signals: video protection, adversarial perturbation, T2V personalization, temporal denoising, motion disruption

Abstract: Text-to-video (T2V) diffusion models have made it increasingly easy to synthesize realistic and temporally coherent videos, while recent personalization techniques allow such models to imitate a specific subject, style, or motion pattern from only a few reference clips. This capability creates a new data-misuse risk: videos shared online can be collected and used for unauthorized T2V fine-tuning. Existing protective perturbations are mainly designed for image recognition or text-to-image personalization, and therefore focus on corrupting static appearance cues rather than the temporal denoising dynamics that make video personalization possible. To address this gap, we introduce ChronoLock, the first proactive protection framework that makes released videos difficult to exploit for unauthorized T2V personalization. ChronoLock targets the motion-learning process directly by optimizing bounded perturbations over temporal denoising trajectories. It first disrupts intra-chunk temporal adaptation with a diffusion objective that combines fitting error, frame-relative denoising relations, and adjacent-frame variation, and then enlarges inter-chunk boundary mismatch to weaken long-range motion continuity. Transformation-sampled updates further improve robustness to common preprocessing on UCF Sports and HMDB51 with popular T2V backbones and personalization scheme show that ChronoLock effectively reduces motion imitation under automatic metrics and human evaluation.

</details>


### [98] [VT-DUDA: Visual Token Conditioning for Diffusion-guided Unsupervised Domain Adaptation](https://arxiv.org/abs/2606.21700)
...
TL;DR: VT-DUDA 提出一种视觉token条件框架，利用源图像生成实例级别的视觉token序列，与文本嵌入拼接作为扩散模型的混合条件，从而更精确地指导目标域风格图像合成，提升无监督域适应性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的UDA方法仅依赖类别文本提示或域适配模块控制外观，缺乏实例级别的细粒度条件，导致目标域风格合成不够精确，影响下游分类性能。

Method: 将源图像映射为紧凑的视觉token序列，与对应文本嵌入沿交叉注意力上下文维度拼接，形成混合条件输入到潜在扩散模型中；同时使用目标域适配器分支进行风格迁移，支持推理时通过token选择和强度调节控制条件信号。

Result: 在Office-31、Office-Home和VisDA-2017三个基准上，VT-DUDA的平均目标域准确率超过了强判别式方法和现有扩散UDA基线。

Conclusion: 在基于生成的UDA中，更强的条件接口（视觉token）能够提升合成目标域风格数据对下游任务的有用性。

Classification reason: 该论文使用扩散模型进行无监督域适应（UDA），通过视觉token条件控制生成目标域风格的图像，属于图像风格迁移/编辑范畴，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）中的可控编辑、风格化方向高度相关。

Importance reason: Focuses on domain adaptation with diffusion, not on core priority subtopics like continuous language models or autoregressive generation. Signals: diffusion-based UDA, visual token conditioning, instance-level synthesis, not autoregressive, not continuous language model
...
### [99] [IDAG-Edit: Multi-Object Video Editing via Instance-Decoupled Attention and Guidance](https://arxiv.org/abs/2606.22042)
...
TL;DR: IDAG-Edit 是一个无需训练的多目标视频编辑框架，通过布局引导的注意力调制和实例级掩码实现细粒度、时序一致的多目标视频编辑。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频编辑方法在多目标场景下存在注意力泄漏、身份漂移和时序不稳定等问题，难以实现精确的物体级控制。

Method: 提出 IDAG-Edit 框架，包含两个核心模块：(1) Layout-guided Attention Modulation，用于促进多目标协同编辑；(2) Instance-level Masks，用于保持每个物体的个体身份并强制注意力集中在各自物体区域内，实现细粒度的物体级编辑。整个框架无需额外训练。

Result: 大量定性和定量评估表明，该方法在时序稳定性和多目标可控性上优于现有的最先进视频编辑方法。

Conclusion: IDAG-Edit 通过实例解耦的注意力机制和引导策略，有效解决了多目标视频编辑中的注意力泄漏和身份漂移问题，实现了无需训练的高质量编辑。

Classification reason: 该论文提出多目标视频编辑框架 IDAG-Edit，属于视频编辑任务，与 [CANONICAL] subtopic "visual_editing (图像与视频编辑)" 高度匹配。

Importance reason: Video editing is relevant but not a top priority subtopic, and the training-free approach is useful but lacks direct alignment with your core interests. Signals: training-free video editing, multi-object control, attention modulation, temporal consistency, diffusion-based editing
...
### [100] [Encoder-Decoder Manifold Alignment for Idempotent Generation](https://arxiv.org/abs/2606.22304)
...
TL;DR: 本文指出现有幂等生成模型因编码器与解码器学习到的数据流形几何不匹配而无法实现精确不动点，提出一种新的训练框架通过对齐编码器与解码器的流形表示来弥合这一差距，从而获得稳定的幂等映射。实验表明该方法在图像生成和图像编辑任务上显著降低了幂等误差，并提升了身份保持与信息稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有幂等生成模型在重复应用时无法达到精确不动点，导致不稳定和漂移。作者认为根本原因是编码器和解码器学习到的数据流形存在几何不匹配，阻碍了模型学习真正的幂等映射。

Method: 提出一种新的训练框架，通过显式对齐编码器和解码器学习到的流形几何，迫使两者学习同一底层数据流形的一致表示，从而鼓励稳定的投影和幂等映射。

Result: 在图像生成和图像编辑任务上，该方法相比现有方法实现了显著更低的幂等误差，并在重复应用下持续生成相同输出。同时，幂等性的提升改善了身份保持和信息稳定性，使生成式编辑模型更加真实和可控。

Conclusion: 编码器-解码器流形对齐是解决幂等生成模型不稳定问题的有效途径，该方法在图像生成和编辑任务上均表现出优越性能，为可控生成编辑提供了更可靠的框架。

Classification reason: 该论文提出编码器-解码器流形对齐框架以实现幂等生成，并在图像生成和图像编辑任务上验证效果。图像编辑是论文的核心应用场景之一，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度匹配。

Importance reason: Addresses a fundamental geometric mismatch in idempotent generation, but the image editing focus and lack of direct connection to priority subtopics limit personal relevance. Signals: idempotent generation, manifold alignment, encoder-decoder mismatch, image generation, image editing
...
### [101] [Physically-guided Image Generation for Multi-Projection Mapping](https://arxiv.org/abs/2606.22477)
...
TL;DR: 本文提出 ConPhyG，一个物理引导的可控生成框架，用于多投影映射。它定义了合作范式（生成语义与物理属性协调）和对抗范式（辐射补偿消除表面干扰），通过注入像素级色域、深度和边缘先验到扩散过程，支持用户动态切换约束以平衡艺术自由与物理可行性，并在真实四投影仪系统上验证了优越性。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽能生成定制化内容，但直接用于投影映射（PM）时存在理想化2D生成与真实物理约束（如投影仪色域、物体几何形状、表面纹理干扰）之间的严重不匹配，现有方法缺乏统一的物理引导生成框架。

Method: 提出 ConPhyG 框架，包含两种范式：(1) 合作模式——将多维物理先验（逐像素色域、深度、边缘）注入扩散过程的去噪步骤中，引导生成内容与物理属性对齐；(2) 对抗模式——释放生成潜力，对有界数值优化进行多投影仪辐射补偿。支持用户动态切换约束，并扩展到360度多视角一致投影映射的序列生成策略。

Result: 在真实四投影仪系统上的定量和定性评估表明，ConPhyG 在几何对齐、色域利用和语义保真度方面显著优于现有方法。

Conclusion: 本文证明了将物理约束显式注入扩散生成过程可以有效弥合理想化2D生成与真实投影映射之间的鸿沟，为投影映射中的内容创作提供了灵活可控的物理引导生成框架。

Classification reason: 该论文研究基于物理约束引导的图像生成用于多投影映射（Projection Mapping），核心是将扩散模型生成的图像与物理约束（色域、深度、边缘）对齐，属于可控图像生成与编辑范畴。映射到 [CANONICAL] subtopic "visual_editing"（图像与视频编辑），因为其本质是在生成过程中注入物理先验进行可控编辑/调整，使生成内容适配真实3D物体的投影映射。

Importance reason: Focuses on projection mapping, not aligned with user's priority subtopics like continuous language models or video generation. Signals: projection mapping, diffusion-based generation, physical constraints, multi-projector system, not autoregressive or video generation
...
### [102] [Trajectory Forcing: Structure-First Generation with Controllable Semantic Trajectories](https://arxiv.org/abs/2606.22527)
...
Importance: 高优先级 (92.28) - 未入选精读

TL;DR: 本文提出 Trajectory Forcing (TF)，一种以轨迹为中心的图像生成框架，将生成过程显式组织为从全局布局到细节的语义阶段序列，每个阶段产生可解码、可编辑的中间潜状态，支持局部编辑和结构一致性控制。


<details>
  <summary>Details</summary>
Motivation: 现有扩散和流匹配生成模型以端点为控制中心，用户只能指定条件并等待最终输出，中间生成过程不可见、不可交互。尽管已有方法开始利用生成顺序和过程分解来提升样本质量，但中间状态仍被视为内部计算而非可交互对象。本文旨在让生成路径本身变得显式、语义化且可编辑。

Method: TF 将图像合成组织为从全局布局到物体、部件再到细节的语义阶段序列。通过聚类预训练视觉表征（如 DINOv2）构建粗到细的教师层级，并在每个层级训练一个层级条件的一步流匹配模型。每个阶段产生可解码的潜状态，可在下一阶段开始前进行检查、评估和局部编辑。此外，引入轨迹感知指标来衡量结构一致性和局部可控性。

Result: 实验表明，TF 在保持竞争性样本质量的同时，能够暴露连贯的中间状态，并支持跨语义层次的局部编辑。轨迹感知指标验证了其结构一致性和局部可控性优于基线方法。

Conclusion: TF 将生成模型的关注点从最终图像转向生成路径本身，为可控、轨迹感知的图像合成开辟了新途径。通过显式化中间语义阶段并支持交互式编辑，TF 在可控生成方面迈出了重要一步。

Classification reason: 该论文提出 Trajectory Forcing 框架，将图像生成过程分解为从全局布局到细节的语义阶段，支持中间潜状态的可视化、评估和局部编辑。核心贡献在于生成路径的可控编辑能力，属于可控图像编辑范畴，因此映射到 [CANONICAL] subtopic "图像与视频编辑" (visual_editing)。

Importance reason: Proposes a trajectory-centric generation framework with explicit semantic stages and local editability, aligning with user's high-priority continuous multimodal and flow matching interests. Signals: flow matching, continuous latent state editing, structure-first generation, trajectory-aware metrics, DINOv2 clustering
...
### [103] [RS-Gen: A Multi-Stage Agentic Framework for Reasoning and Search-Augmented Image Generation](https://arxiv.org/abs/2606.23221)
...
TL;DR: RS-Gen 是一个即插即用、无需训练的多阶段智能体框架，通过"提问-求解"闭环机制增强图像生成与编辑模型的推理能力和外部知识获取能力，在 WISE Verified 和 RISEBench 基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成与编辑模型在处理模糊意图、逻辑推理和分布外知识时表现不佳，缺乏深度推理能力和实时外部信息获取能力。虽然统一理解-生成模型试图弥合这一差距，但仍受限于参数规模和静态知识。

Method: RS-Gen 采用多阶段智能体框架，创新性地引入"提问-求解"闭环机制：首先识别逻辑问题和知识缺口，然后自主规划行动（如搜索外部知识）来弥补信息不足，最后执行深度逻辑推理以增强图像生成/编辑结果。该框架是即插即用、无需训练的，可适配现有基础模型。

Result: 在 WISE Verified 基准上，RS-Gen 使 Qwen-Image 获得 0.313 的绝对性能提升；在 RISEBench 基准上，使 Qwen-Image-Edit-2511 获得 19.70 的绝对提升，均达到开源模型的 SOTA 水平。

Conclusion: RS-Gen 通过智能体范式有效扩展了基础图像生成与编辑模型的能力边界，证明了推理增强和外部知识检索在图像生成任务中的巨大潜力。

Classification reason: 该论文提出一个多阶段智能体框架用于推理和搜索增强的图像生成与编辑，核心任务涉及图像生成和编辑，因此映射到多模态生成方向下的[CANONICAL] subtopic "图像与视频编辑"（visual_editing）。同时该框架也涉及理解与生成结合，因此也匹配多模态理解与生成方向。

Importance reason: Focuses on agentic image generation, not on the user's priority subtopics like continuous models or autoregressive video generation. Signals: Agentic framework, Training-free, Image generation & editing, Reasoning & search augmentation, Not aligned with priority subtopics
...
### [104] [SteerVTE: Seamless Video Text Editing with Style and Glyph Control](https://arxiv.org/abs/2606.23254)
...
Importance: 值得关注 (77.76) - 未入选精读

TL;DR: SteerVTE 是一个统一的视频文本编辑框架，基于冻结的视频扩散 Transformer，通过轻量级文本上下文适配器（包含风格编码器和双粒度字形编码器）实现精确的文本修改，并提出了字形感知空间-焦点损失和三阶段渐进训练策略，在文本准确性、风格一致性和时序连贯性上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视频文本编辑任务尚未被充分探索，现有方法在跨帧准确性、时序连贯性和风格保真度方面存在挑战，尤其是文本区域小、需要笔画级精度的局部编辑任务。

Method: 基于冻结的视频扩散 Transformer（DiT），附加轻量级文本上下文适配器，包含风格编码器（捕捉原始文本视觉属性）和双粒度字形编码器（行级和字符级编码目标文本）；提出字形感知空间-焦点损失和三阶段渐进训练（图像→视频）；构建 SteerVTE-1M 数据集（100万三元组）。

Result: 在文本准确性、风格一致性和时序连贯性上大幅优于现有视频编辑基线方法。

Conclusion: SteerVTE 通过风格和字形控制实现了精确、连贯的视频文本编辑，为视频文本编辑任务提供了有效的统一框架。

Classification reason: 该论文研究视频文本编辑（Video Text Editing），属于视频编辑的子任务，与 [CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度匹配。

Importance reason: Video text editing is a niche application, not aligned with user's priority subtopics like continuous language models or world models. Signals: Video text editing, Style and glyph control, Frozen video diffusion transformer, Lightweight adapter, Three-stage training
...
### [105] [BoxCtrl: 3D-Aware Visual Prompting for Geometric Image Editing](https://arxiv.org/abs/2606.23270)
...
TL;DR: BoxCtrl 提出了一种3D感知的视觉提示框架，通过在2D图像上投影带有不同RGB颜色的3D边界框作为视觉提示，结合SFT和RL两阶段训练，实现了精确的几何图像编辑（平移、旋转、缩放和组合编辑）。


<details>
  <summary>Details</summary>
Motivation: 现有的指令式图像编辑模型在几何编辑（如3D空间中的平移、缩放、旋转）方面精度不足，文本或粗粒度2D引导难以准确传达几何意图，缺乏精确且一致的几何图像编辑方法。

Method: 提出BoxCtrl框架：1) 设计RGB 3D边界框投影到2D图像上作为视觉提示，三个正交面用不同颜色编码位置、大小和方向，解耦几何控制与外观控制；2) 两阶段训练：先在大规模合成数据集上进行监督微调（SFT），再使用未配对真实数据进行在线强化学习（RL），奖励函数评估几何精度和视觉保真度。

Result: 在平移、旋转、缩放和组合编辑任务上均达到最先进性能，显著提升了几何编辑的精确度，同时保持了照片级真实感。

Conclusion: BoxCtrl通过精心设计的3D感知视觉提示和SFT-RL两阶段训练范式，有效解决了精确几何图像编辑的挑战，为可控图像编辑提供了新的思路。

Classification reason: 该论文提出BoxCtrl，一种3D感知的视觉提示框架，用于几何图像编辑（平移、旋转、缩放等），属于图像编辑范畴，因此映射到[CANONICAL] subtopic "visual_editing"（图像与视频编辑）。

Importance reason: Novel 3D-aware visual prompting for geometric editing, but lacks direct connection to user's priority subtopics like continuous models or autoregressive generation. Signals: 3D-aware visual prompting, geometric image editing, SFT-RL training, synthetic-to-real domain gap, not in priority subtopics
...
### [106] [Vera: A Layered Diffusion Model for Content-Preserving Video Editing](https://arxiv.org/abs/2606.23610)
...
TL;DR: Vera 提出分层扩散框架用于内容保持的视频编辑，通过生成编辑层和 alpha 遮罩与源视频合成，采用 Mixture-of-Transformers (MoT) 架构分离编辑与内容保持，在 486K 帧分层数据上训练，在内容保持方面优于现有开源模型。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在编辑时往往重新生成所有像素，导致本应保持不变的元素（如角色或背景场景）被意外改变。核心挑战在于如何实现内容保持的视频编辑。

Method: Vera 提出分层扩散框架：不重新生成整个视频，而是生成一个编辑层（edit layer）和对应的 alpha 遮罩，与源视频进行合成。将文本到视频的 DiT 扩展为 Mixture-of-Transformers (MoT) 架构，每层有独立的 DiT，通过联合自注意力（joint self-attention）进行交互。同时构建了高质量的分层数据集，包含精确的 alpha 遮罩、多样场景和动态效果。

Result: 在定量基准和人类偏好研究中，Vera 在内容保持方面优于领先的开源视频编辑模型，同时在编辑质量上保持竞争力，仅使用 486K 帧分层训练数据。

Conclusion: Vera 通过分层生成和合成策略有效解决了视频编辑中的内容保持问题，分离了创意编辑与内容保持，为视频编辑提供了一种新的范式。

Classification reason: 该论文提出 Vera，一种分层扩散框架用于内容保持的视频编辑，属于视频编辑方向，与 [CANONICAL] subtopic "visual_editing (图像与视频编辑)" 高度匹配。

Importance reason: Novel layered diffusion framework for video editing, but not aligned with user's priority subtopics like autoregressive generation or world models. Signals: Video editing, content preservation, layered diffusion, Mixture-of-Transformers, alpha matte compositing
...
### [107] [Controllable Texture Tiling with Transformed RoPE-Enhanced Diffusion Models](https://arxiv.org/abs/2606.22945)
...
TL;DR: 本文提出一种基于扩散变换器（DiT）的可控纹理平铺框架，通过坐标变换旋转位置编码（Coordinate-Transformed RoPE）实现精确的平铺参数控制，并利用分离注意力掩码（Disjoint Attention Mask）防止语义泄漏，在保持参考纹理结构保真度的同时与场景光照几何自然融合。


<details>
  <summary>Details</summary>
Motivation: 现有材质迁移和参考引导的修复方法无法满足纹理平铺任务中精确重复参考图案（按频率、方向、尺度等参数）的需求，且生成方法在保持纹理结构保真度和场景光照几何一致性方面存在不足。

Method: 基于扩散变换器（DiT），提出两个关键技术：(1) 坐标变换旋转位置编码（Coordinate-Transformed RoPE），将2D仿射变换直接应用于目标潜变量与图像条件之间的相对位置嵌入，实现无需显式像素扭曲的精确平铺控制；(2) 分离注意力掩码（Disjoint Attention Mask），屏蔽参考特征中的语义泄漏，在保持结构完整性的同时将合成纹理与场景原始光照几何无缝融合。

Result: 大量实验表明，该方法在控制精度和纹理保真度方面均优于现有基线方法。

Conclusion: 本文提出的框架有效解耦了空间操作与内容生成，实现了高保真、可控的纹理平铺，为图像编辑中的纹理合成任务提供了新的解决方案。

Classification reason: 该论文提出可控纹理平铺（texture tiling）框架，将用户指定的参考纹理按照频率、方向、尺度等参数精确重复到场景图像中，属于图像编辑中的可控纹理合成与表面外观编辑任务，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度匹配。

Importance reason: Novel technical contributions in diffusion transformers for texture tiling, but the subtopic is not in user's priority list and lacks direct alignment with key directions. Signals: Diffusion Transformer, Controllable generation, RoPE enhancement, Texture tiling, Image editing
...
### [108] [XmoPipe: A Pipeline for Large-Scale In-the-Wild Human Motion Dataset Construction](https://arxiv.org/abs/2606.20731)
...
TL;DR: 提出XmoPipe，一个从网络视频中大规模构建野外人体运动数据集的流水线，支持3D身体/面部运动提取与文本描述生成，训练的运动模型性能可与传统动捕数据集相媲美。


<details>
  <summary>Details</summary>
Motivation: 传统基于标记的运动捕捉数据成本高、规模有限且多样性不足，而现有野外运动数据集构建方法缺乏可扩展性和灵活性。需要一种能从海量在线视频中自动提取高质量人体运动数据的流水线。

Method: XmoPipe流水线包含三个主要阶段：(1) 视频检索——根据关键词从网络视频中检索相关视频；(2) 运动提取——使用单目运动捕捉方法提取3D身体和面部运动；(3) 文本描述生成——利用视频语言理解模型生成高层级文本描述。系统支持灵活配置，可针对不同运动类型、多人交互或表情行为进行定向采集。

Result: 使用该流水线构建的数据集训练运动重建和运动生成模型，性能与传统动捕数据集训练的模型相当，并展现出强大的跨数据集泛化能力。

Conclusion: XmoPipe提供了一种可扩展、低成本的野外人体运动数据集构建方案，能够有效弥补传统动捕数据的不足，为大规模运动模型训练提供了可行路径。

Classification reason: 该论文提出大规模野外人体运动数据集构建流程，核心是人体动作提取与生成，属于[数字人与动作生成]（avatar_motion_generation）这一规范主题。

Importance reason: Pipeline for large-scale motion data is useful but not aligned with your priority subtopics on continuous models, video generation, or world models. Signals: Human motion dataset construction, In-the-wild video pipeline, 3D body and facial motion extraction, Text description generation, Motion model training comparable to mocap
...
### [109] [Odoriko: A Shape-Aware Multimodal Diffusion Framework for Human Motion](https://arxiv.org/abs/2606.21135)
...
TL;DR: Odoriko 是首个统一的多模态人体动作生成框架，能够根据主体的生物形态信息（如性别、体型）生成形态一致的动作，覆盖文本到动作、音乐到舞蹈、视频到动作三种条件，并在缺乏形态信息时联合估计形态与动作。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态动作生成框架将所有主体视为形态等效，忽略了性别、体型等生物形态因素对运动特征的影响，导致生成的动作缺乏个体特异性。

Method: 提出 Odoriko 框架，基于扩散模型，将主体生物形态信息（如性别、体型参数）作为条件输入，与文本/音乐/视频等多模态条件一起引导动作生成。当形态信息缺失时，模型可同时恢复形态参数并生成动作，实现估计与生成的统一。

Result: 在 text-to-motion、music-to-dance、video-to-motion 多个基准上，Odoriko 匹配或超越了此前专门化模型的标准指标，同时实现了现有统一框架无法支持的形态一致性动作生成。

Conclusion: 将生物形态信息纳入多模态动作生成是可行且有效的，能够在不牺牲生成质量的前提下实现更具个体特异性的动作合成，为未来个性化动作生成提供了新方向。

Classification reason: 该论文研究多模态（文本、音乐、视频）条件的人体动作生成，属于[CANONICAL] subtopic "数字人与动作生成"（avatar_motion_generation）的范畴，该主题涵盖人体动作生成、虚拟人动画等方向。

Importance reason: Novel unified multimodal motion generation with shape awareness, but outside user's priority subtopics and lacks core boost keywords. Signals: multimodal generation, human motion, shape-aware, text-to-motion, music-to-dance
...
### [110] [Context-Aware Autoregressive Diffusion for Gloss-Wise Sign Language Production](https://arxiv.org/abs/2606.21234)
...
Importance: 普通相关 (47.34) - 未入选精读

TL;DR: 本文提出 GARD（Context-aware Gloss-wise AutoRegressive Diffusion model），一种基于词素（gloss）级别的自回归扩散框架用于手语生成。通过语义和运动上下文条件建模协同发音，并引入词素间过渡引导（Inter-Gloss Transition Guidance）和全局运动协调器（Global Motion Harmonizer）来确保动作自然连续。在 Phoenix-T 和 CSL-Daily 数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有手语生成方法通常端到端生成整个序列，随着句子变长容易出现时间漂移和手部运动模糊，且无法精确控制单个词素（gloss）的动作。本文旨在通过词素级别的逐次生成方式，结合上下文条件建模，生成更自然、准确的手语序列。

Method: 提出 GARD 框架：1）词素级自回归扩散模型，每个词素生成时以语义上下文（linguistic context）和运动上下文（kinematic context）为条件，建模协同发音（coarticulation）；2）Inter-Gloss Transition Guidance：基于梯度的引导机制，在词素边界处进行运动学对齐，确保姿态连续性；3）Global Motion Harmonizer：根据过渡引导调整后的边界姿态，对整个词素运动序列进行全局精炼。

Result: 在 Phoenix-T 和 CSL-Daily 数据集上，GARD 在语言准确性和运动相似性指标上均优于现有 SLP 方法，有效缓解了长句生成中的时间漂移和运动模糊问题。

Conclusion: 词素级别的自回归扩散生成结合上下文条件建模和过渡引导，能够有效提升手语生成的自然度和准确性，为长序列人体动作生成提供了新的思路。

Classification reason: 该论文研究手语生成（Sign Language Production），属于人体动作生成范畴，与现有 [CANONICAL] 子主题"数字人与动作生成"（avatar_motion_generation）高度匹配，该子主题涵盖虚拟人、人体动作、面部重演等，手语生成是人体动作生成的一个具体应用。

Importance reason: Focuses on sign language production, which is outside the user's priority subtopics and lacks boost keywords. Signals: gloss-wise autoregressive diffusion, sign language production, coarticulation modeling, inter-gloss transition guidance, global motion harmonizer
...
### [111] [Text Dictates, Music Decorates: Energy-based Attention for Editable Dance Motion Generation](https://arxiv.org/abs/2606.22726)
...
TL;DR: 本文提出 STREAM，一种基于能量注意力机制的解耦扩散 Transformer，用于可编辑的舞蹈动作生成。通过将文本语义（AdaLN）与音乐节奏（BEAM）严格分离，解决了多模态条件生成中的模态坍缩问题，并引入 Motorica++ 数据集和 EDS 评估指标，实现了语义可控且节奏对齐的舞蹈动作生成。


<details>
  <summary>Details</summary>
Motivation: 现有舞蹈动作生成模型通常将文本和音乐作为混合条件输入，导致密集的声学节奏信号淹没稀疏的文本语义，造成模态坍缩，用户无法通过文本精确控制舞蹈动作的语义内容。亟需一种解耦的条件控制机制，使文本主导动作的语义结构，音乐提供节奏装饰。

Method: 提出 STREAM（Structural-Temporal Rhythmic Energy-based Attention for Motion），一种模态解耦的扩散 Transformer。文本语义通过 AdaLN（自适应层归一化）控制全局运动学结构；音乐节奏通过 Bimodal Energy-Based Attention Module (BEAM) 将文本特征路由到音乐节拍上而不覆盖语义。同时构建 Motorica++ 数据集，增加领域特定舞蹈词汇和帧级语义标注，并提出 Exchange Evaluation Protocol 和 Editable Dance Score (EDS) 量化零样本可编辑性。

Result: 实验表明，STREAM 在运动-音乐对齐和编舞语义保持方面达到 SOTA，同时具备良好的零样本可编辑性，用户可通过修改文本提示灵活控制舞蹈动作的语义内容。

Conclusion: STREAM 通过解耦文本语义和音乐节奏的条件控制，有效解决了多模态条件舞蹈生成中的模态坍缩问题，使 AI 从被动的动作合成器转变为可控的、可协作的编舞伙伴。

Classification reason: 该论文研究文本和音乐条件驱动的舞蹈动作生成，属于人体动作生成范畴，与[CANONICAL] subtopic "数字人与动作生成"（avatar_motion_generation）高度匹配。

Importance reason: Dance motion generation is outside your priority subtopics and lacks boost keywords like video generation or world model. Signals: Modality-decoupled diffusion transformer, energy-based attention for music-text conflict, new dance dataset and evaluation metric, not in priority subtopics, no boost keywords matched

Abstract: Choreographic motion generation poses unique challenges for AI, demanding precise semantic control over complex, temporally structured, and expressive full-body dynamics. While existing models can synthesize motion from music, they remain largely black boxes. Conversely, attempting to condition generation on both text and music frequently leads to modality collapse, where dense acoustic rhythms overwhelm sparse semantic text prompts, destroying user controllability. To resolve this spatial-temporal conflict, we propose STREAM (Structural-Temporal Rhythmic Energy-based Attention for Motion), a modality-decoupled diffusion transformer. STREAM strictly separates conditioning pathways: global text semantics dictate the kinematic structure via Adaptive Layer Normalization (AdaLN), while a novel Bimodal Energy-Based Attention Module (BEAM) routes these features to the musical beat without overwriting the semantics. We further introduce Motorica++, a newly curated dataset enriched with domain-specific dance vocabulary and frame-level semantic annotations from existing Motorica dataset. Additionally, to rigorously quantify zero-shot editability, we propose the Exchange Evaluation Protocol and Editable Dance Score (EDS). Through extensive experiments, STREAM achieves state-of-the-art alignment between motion and music while perfectly preserving choreographic semantics, positioning AI not merely as a reactive synthesizer, but as a controllable, collaborative partner for artistic direction. The source code and datasets are available at .

</details>


### [112] [The Impact of VAE Design on Latent Pose Representations for Diffusion-based Sign Language Production](https://arxiv.org/abs/2606.22959)
...
TL;DR: 本文系统研究了变分自编码器（VAE）的设计选择（架构与训练目标）对潜空间中手语姿态编码结构的影响，并分析了这些潜空间属性如何影响下游潜扩散模型在文本到手语生成任务上的性能。实验表明，生成性能的差异有时更能由潜空间属性而非VAE重建精度来解释。


<details>
  <summary>Details</summary>
Motivation: 现有手语生成（SLP）中的潜扩散方法通常仅用重建质量（几何指标）评估VAE编码阶段，但忽略了潜空间结构对下游生成模型训练和性能的影响。作者希望揭示VAE设计如何影响潜空间属性，以及这些属性与下游生成性能之间的关系。

Method: 在Phoenix14T数据集上，设计多种VAE变体（不同架构和训练目标）对手语姿态序列进行编码，分析潜空间结构（如各向同性、聚类性、维度利用率等），然后训练潜扩散模型进行文本到姿态生成，通过反向翻译BLEU分数评估生成性能，并关联潜空间属性与生成质量。

Result: 不同VAE设计导致潜空间结构显著不同；生成性能（BLEU分数）的差异有时与潜空间属性（如各向同性程度）的相关性高于与VAE重建精度的相关性；某些潜空间属性对下游生成质量有更好的预测能力。

Conclusion: VAE设计选择对潜空间结构有重要影响，进而影响下游潜扩散模型的生成性能。评估VAE时应超越单纯的重建指标，关注潜空间属性，这有助于更好地设计和选择用于手语生成的潜编码器。

Classification reason: 该论文研究手语生成（Sign Language Production），属于人体动作生成范畴，与[CANONICAL] subtopic "数字人与动作生成"（avatar_motion_generation）高度匹配。手语是一种特定的人体姿态序列生成任务，符合该子主题的定义。

Importance reason: Focuses on sign language production, which is not in your priority subtopics and lacks boost keywords. Signals: VAE design for latent pose, diffusion-based sign language, latent space analysis, not in priority subtopics, no boost keywords matched
...
### [113] [TEXEDO : Test Time Scaling for Controller-aware Language-conditioned Humanoid Motion Generation](https://arxiv.org/abs/2606.22998)
...
Importance: 值得关注 (70.34) - 未入选精读

TL;DR: 本文提出 Identity-Action Decoupling (IaD) 框架，通过身份解耦损失和文本对齐损失，将身份特征与动作特征分离，在不进行主体特定微调的情况下，实现身份保持的文本到视频肖像生成，生成具有丰富表情和场景变化的视频。


<details>
  <summary>Details</summary>
Motivation: 现有身份保持文本到视频生成方法（如 ID-Animator、ConsisID）仅在推理时注入身份特征，忽略了面部嵌入中包含的身份无关信息（如表情、姿态），导致生成的面部动作单调或不准确，难以精确跟随文本提示。本文旨在解决这一问题，实现身份一致且动作可控的视频肖像生成。

Method: 提出 Identity-Action Decoupling (IaD) 框架，包含两个核心损失函数：(1) Identity Decoupling Loss，用于将身份特征与动作特征解耦，去除面部嵌入中与身份无关的信息；(2) Text Alignment Loss，用于增强生成视频与文本描述的对齐。该方法无需针对特定主体进行微调，在推理时即可生成视频。

Result: IaD 生成的视频在跨时间身份一致性上表现优异，同时能够生成丰富、可控的表情和场景变化，与输入文本描述高度匹配。实验表明该方法优于现有基线方法。

Conclusion: 本文提出的 IaD 框架通过身份-动作解耦有效解决了现有 IPT2V 方法中面部动作单调、不跟随文本提示的问题，在不进行主体特定微调的前提下实现了身份保持与动作可控的平衡，为视频肖像生成提供了新的思路。

Classification reason: 该论文研究身份保持的文本到视频肖像生成（IPT2V），涉及参考图像驱动的面部动画和视频生成，属于数字人/肖像动画/面部重演范畴，因此映射到 [CANONICAL] subtopic "avatar_motion_generation（数字人与动作生成）"。

Importance reason: Focuses on identity-preserving video generation but lacks direct connection to user's priority subtopics like continuous language models or world models. Signals: Identity-Action Decoupling, identity-preserving text-to-video, no subject-specific fine-tuning, facial dynamics control, not autoregressive or diffusion transformer
...
### [115] [Generative Relightable Avatars](https://arxiv.org/abs/2606.22718)
...
TL;DR: 提出 Generative Relightable Avatars (GRA)，一种结合物理可重光照与生成式细化的全身数字人方法，利用扩散模型进行纹理细化和视频到视频的时序一致增强，实现可重光照、自由视角的高质量数字人渲染。


<details>
  <summary>Details</summary>
Motivation: 现有可重光照数字人方法通常采用确定性回归方式建模外观细节，但细粒度外观建模本质上是一个"一对多"问题（同一光照条件下有多种合理的外观细节），确定性方法难以捕捉这种多样性。因此需要一种生成式方法来解决。

Method: GRA 采用混合方法：(1) 基于跟踪的动画网格，在 UV 空间优化材质参数，在目标 HDR 环境图下渲染粗粒度的可重光照外观；(2) 用前馈模型细化纹理，捕捉姿态相关的纹理动态和超出简化反射假设的光照效果；(3) 微调视频到视频扩散模型，将物理渲染结果转化为时序连贯的高细节视频，并采用错误回收策略生成长视频。

Result: 实验表明，GRA 在感知质量上优于先前的可重光照数字人基线方法，能够生成更逼真、细节更丰富的全身数字人渲染结果。

Conclusion: 将物理可重光照与生成式细化相结合是数字人渲染的有效范式，生成式方法能够更好地处理外观细节的多样性，同时保持对光照和视角的 3D 控制。

Classification reason: 该论文提出可重光照的数字人全身渲染方法，属于虚拟人/数字人领域，与 [CANONICAL] 子主题 "数字人与动作生成" (avatar_motion_generation) 高度匹配。

Importance reason: Combines physics-based relighting with generative diffusion refinement for avatars, but is person-specific and not aligned with user's priority subtopics. Signals: generative relightable avatars, diffusion-based texture refinement, video-to-video diffusion, physics-grounded relighting, person-specific method

Abstract: We present Generative Relightable Avatars (GRA), a person-specific method for photorealistic free-view rendering and environment-map relighting of full-body humans. We postulate that modeling fine-grained appearance details is inherently a one-to-many problem that can benefit from a generative formulation. In contrast to fully regressive relightable avatar methods, GRA follows a hybrid approach that combines controllable, physics-grounded relighting with probabilistic refinement. Starting from a tracked animated mesh, we optimize material parameters in UV-space and render a coarse relit appearance under a target HDR environment map. Next, we refine the textures with a feed-forward model to capture pose-dependent texture dynamics and illumination effects beyond simplified reflectance assumptions. Finally, a fine-tuned video-to-video diffusion model transforms the physically grounded renderings into temporally coherent, high-detail videos while preserving 3D control, with an error-recycling strategy for generating long videos. Experimental evaluations demonstrate our method's improved perceptual quality over prior relightable avatar baselines. Project Page:

</details>


### [116] [Policy-as-Data: Learning Generalizable HOI Diffusion Models from Simulated Physics](https://arxiv.org/abs/2606.22806)
...
TL;DR: 本文提出 Policy-as-Data 框架，利用物理模拟器中的强化学习策略生成任务导向的人-物交互数据，训练可泛化的 HOI 扩散模型，实现对未见物体和长时程交互的生成。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的 HOI 生成方法依赖昂贵的动捕数据，难以扩展且功能多样性有限，导致模型无法泛化到未见物体且缺乏物理一致性。

Method: 提出 Policy-as-Data 框架：(1) 在物理模拟器中用强化学习训练策略，生成任务导向的 HOI 数据；(2) 设计粗到细的重定向流程，将模拟器中的简化模型映射到标准参数化人体模型；(3) 在增强数据集上训练扩散生成模型。

Result: 实验表明该方法在未见物体泛化、长时程生成、动态多样性和物理合理性方面均优于现有方法。

Conclusion: 利用物理模拟器合成数据可以有效缓解 HOI 生成中的数据瓶颈，提升模型的泛化能力和物理一致性。

Classification reason: 该论文研究人体-物体交互（HOI）生成，属于人体动作生成范畴，与 [CANONICAL] 数字人与动作生成（avatar_motion_generation）高度相关。

Importance reason: Focuses on HOI generation via physics simulation, which is not a priority subtopic and lacks direct alignment with user's key interests in continuous multimodal models or video generation. Signals: Human-Object Interaction generation, physics simulation data augmentation, diffusion model for HOI, generalization to unseen objects, long-horizon generation
...
### [117] [Integrating Facial Generation into Full-Duplex Spoken Dialogue Systems](https://arxiv.org/abs/2606.21970)
*Jingjing Jiang, Atsumoto Ohashi, Ryuichiro Higashinaka*

Main category: cs.CL, cs.HC, cs.CV, eess.AS
...
TL;DR: Moshi-Face 是首个全双工对话模型，在语音对话基础上联合处理用户音频和面部输入，同时生成语音和面部动作（3D 头部网格），通过 VQ-VAE 将面部编码为离散 token 并用 Face Transformer 非自回归生成，实现低延迟的音画同步。


<details>
  <summary>Details</summary>
Motivation: 现有全双工语音对话模型（如 Moshi）仅支持音频模态，缺乏人类交流中至关重要的面部表情信息，限制了交互的自然性和丰富性。

Method: 1) 构建 VQ-VAE 作为面部编解码器，将面部视频提取的 3D 头部网格编码为紧凑的离散 face token，并可重建回 3D 网格。2) 在 Moshi 基础上扩展 Face Transformer 模块，以非自回归方式生成 face token，实现音频 token 与 face token 的实时同步生成。

Result: Moshi-Face 在低延迟下实现了音画对齐，同时保持了原始纯音频模型的对话质量。

Conclusion: 将面部生成集成到全双工语音对话系统中是可行的，能够在不牺牲对话质量的前提下实现实时音画同步，为更自然的人机交互提供了新方向。

Classification reason: 该论文提出 Moshi-Face，在对话系统中联合生成语音与面部动作（3D 头部网格），属于数字人/说话头/面部动画生成范畴，与 [CANONICAL] subtopic "avatar_motion_generation（数字人与动作生成）" 高度匹配。

Importance reason: Focuses on facial generation for spoken dialogue, which is peripheral to your core priorities in continuous multimodal models and video generation. Signals: Full-duplex dialogue, Facial generation, VQ-VAE face codec, Non-autoregressive generation, Low-latency audiovisual alignment
...
### [118] [InteractiveAvatar: Real-Time Streaming Video Generation for Consistent and Intent-Aware Avatars](https://arxiv.org/abs/2606.22905)
...
TL;DR: InteractiveAvatar 是一个实时无限流式视频生成框架，通过自回归蒸馏实现长时数字人视频生成，提出长短视觉记忆（LSVM）保持视觉一致性，以及推理-反应模块（RRM）实现意图感知的交互。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的音频驱动数字人生成方法在实时流式场景中难以维持视觉时序一致性，且无法显式感知用户意图，限制了复杂交互场景下的应用。

Method: 采用自回归蒸馏实现实时无限流式生成；提出长短视觉记忆（LSVM）机制，将历史视觉信息压缩为紧凑token以保持短程连贯性和长程一致性；提出推理-反应模块（RRM），包含状态循环（State-Cycling）策略和缓存切换（Cache-Switching）机制，使数字人的语音和动作与用户意图对齐。

Result: 在多种场景下的实验表明，该方法在长时生成的视觉一致性上达到SOTA，并能实现实时复杂用户-数字人交互。

Conclusion: InteractiveAvatar 通过视觉记忆机制和意图感知模块，有效解决了实时流式数字人视频生成中的视觉一致性和交互意图对齐问题，为数字人交互应用提供了可行方案。

Classification reason: 该论文提出音频驱动的实时流式数字人视频生成框架，涉及说话头、面部动画、虚拟人交互，属于[数字人与动作生成]（avatar_motion_generation）这一CANONICAL子主题的范畴。

Importance reason: Focuses on avatar video generation, which is not a priority subtopic, and lacks direct connection to continuous language/multimodal models or world models. Signals: real-time streaming video generation, autoregressive distillation, long-short visual memory, intent-aware interaction, digital human avatar
...
### [119] [MotionMAR: Multi-scale Auto-Regressive Human Motion Reconstruction from Sparse Observations](https://arxiv.org/abs/2606.23000)
...
TL;DR: MotionMAR 提出了一种多尺度自回归框架，从稀疏观测数据中由粗到细地重建人体运动：先估计全局轨迹，再逐步细化时间细节，在 AMASS 数据集上达到 SOTA 精度。


<details>
  <summary>Details</summary>
Motivation: 从稀疏传感器或部分观测数据中重建完整人体运动是一个具有挑战性的问题。现有方法难以同时捕捉运动的全局结构（如整体轨迹）和局部高频细节（如肢体摆动），且缺乏对运动层次化时序结构的显式建模。

Method: MotionMAR 包含四个组件：(1) TMT VQ-VAE：将运动数据编码为多时间分辨率的离散 token，分离语义运动与微小抖动；(2) Motion Autoregressive Network (MAN)：在潜空间中跨尺度自回归预测运动，先预测粗粒度索引建立全局结构，再生成细粒度索引恢复细节；(3) Scale-Aware Control (SAC)：融合稀疏跟踪数据，确保生成结果与观测对齐；(4) Motion Refinement Network (MRN)：平滑相邻姿态并消除量化伪影。

Result: 在 AMASS 数据集上的实验表明，MotionMAR 在运动重建精度上达到 SOTA，能够从稀疏观测中可靠地恢复具有层次结构的人体运动。

Conclusion: MotionMAR 通过多尺度自回归建模有效利用了人体运动的层次化时序结构，为从稀疏观测中重建高质量人体运动提供了一种结构感知的可靠方案。

Classification reason: 该论文研究从稀疏观测数据中重建人体运动，属于人体动作生成与重建范畴，与 [CANONICAL] 数字人与动作生成 (avatar_motion_generation) 高度匹配。

Importance reason: Applies multi-scale autoregressive framework to human motion reconstruction, but the subtopic is outside user's priority areas. Signals: multi-scale autoregressive, human motion reconstruction, sparse observations, coarse-to-fine, AMASS dataset
...
### [120] [IMAGIN-4D: Image-Guided Controllable Interaction Generation](https://arxiv.org/abs/2606.23675)
...
TL;DR: PIAvatar提出了一种基于变形梯度解耦的物理交互式3D人体虚拟化身框架，通过将运动速度与变形梯度解耦，结合骨骼框架和物质点法（MPM），实现了化身与化身、化身与环境之间的物理一致交互仿真。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体虚拟化身虽然视觉保真度高，但缺乏物理交互能力，在化身之间或化身与环境交互时只能表现出有限的物理变形，导致交互行为受限。本文旨在解决这一问题，实现具有物理感知的交互式虚拟化身。

Method: 核心思想是将运动学速度与变形梯度解耦。当外力作用于化身时，运动速度会产生应力，阻碍化身达到期望姿态。通过解耦，化身能更好地响应外力同时保持姿态控制。此外，在化身内部集成骨骼框架，支持在非刚性物理交互过程中以闭合形式估计姿态和实时跟踪。整个方法基于传统的物质点法（MPM）框架实现，确保物理一致性。

Result: 在人与物体交互、人与人交互等多种交互场景下评估了方法的性能，展示了在不同交互设置下的物理一致行为。

Conclusion: PIAvatar通过变形梯度解耦和骨骼框架集成，在MPM框架下实现了物理一致的化身-化身和化身-环境交互，为具有物理交互能力的3D虚拟化身提供了有效方案。

Classification reason: 该论文研究3D人体虚拟化身（avatar）的物理交互仿真，属于数字人与动作生成（avatar_motion_generation）这一[CANONICAL]子主题的范畴。论文关注虚拟化身之间的物理交互以及化身与环境的交互，涉及非刚性变形人体仿真，与数字人/虚拟人方向高度吻合。

Importance reason: Focuses on physically interactive avatars, which is outside the user's priority subtopics and negative keywords. Signals: 3D avatar, physical interaction, deformation gradient, material point method, human-object interaction
...
### [122] [UNITY: Attention Flow Networks for Adaptive Conditioning in Diffusion](https://arxiv.org/abs/2606.20971)
...
Importance: 值得关注 (79.13) - 未入选精读

TL;DR: UNITY 提出了一种两阶段训练范式（通用阶段+专门化阶段），通过 Morphable Attention Flow (MAF) 网络和 Morph Wrapper 模块，在扩散图像生成中实现高效、可扩展的多模态复合条件适配，在保持内存效率的同时达到 SOTA 图像保真度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的条件适配方法通常为每种条件模态（如文本、深度图、边缘图等）单独训练适配器，缺乏跨模态共享语义的利用，且在多条件组合场景下计算开销大。作者希望设计一种统一、高效、可扩展的复合条件适配方法。

Method: 提出两阶段训练范式：(1) 通用阶段（Universal Stage），用一半训练步数学习所有条件模态的跨模态共享表征；(2) 专门化阶段（Specialization Stage），用剩余训练预算细化各模态特定特征。核心模块为 Morphable Attention Flow (MAF) 网络和 Morph Wrapper，通过可学习的流场（flow fields）和注意力融合实现通道感知与空间自适应的特征对齐，支持单条件和复合条件设置，且计算复杂度恒定。

Result: 在多个数据集上的实验表明，UNITY 在图像保真度上达到 SOTA，同时显著降低了推理延迟和内存消耗，在复合条件设置下优于现有方法。

Conclusion: UNITY 通过两阶段训练和 MAF 网络实现了高效、可扩展的扩散模型复合条件适配，验证了跨模态共享语义学习的有效性，为多条件图像生成提供了新的高效范式。

Classification reason: 该论文提出 UNITY，一种用于扩散模型图像生成的多模态条件适配方法，属于文生图与图像扩散方向。虽然涉及多条件适配，但核心是扩散图像生成，因此映射到 [CANONICAL] subtopic "文生图与图像扩散"。

Importance reason: Novel two-stage adapter for diffusion conditioning, but lacks direct connection to priority subtopics like continuous models or video generation. Signals: diffusion image generation, composite conditioning, attention flow, memory efficient, two-stage training

Abstract: We introduce UNITY, a Universal-to-Specialized adapter for efficient and scalable composite conditioning in diffusion based image generation. Unlike prior methods that train separate adapters for each conditioning modality, UNITY jointly learns shared semantics across multiple conditioning types and subsequently specializes without modifying the underlying architecture. The proposed two stage training paradigm consists of a Universal Stage that captures cross modal representations across all conditioning modalities using half of the total training steps, followed by a Specialization Stage that refines modality specific features using the remaining training budget. At the core of UNITY are the Morphable Attention Flow (MAF) Network and Morph Wrapper modules, which enable channel aware and spatially adaptive feature alignment through learnable flow fields and attention based fusion. This constant complexity formulation supports flexible operation under both single and composite conditioning settings while significantly reducing inference latency and memory consumption. Extensive experiments across multiple datasets demonstrate that UNITY achieves state of the art image fidelity while maintaining superior memory efficiency. Code:

</details>


### [123] [A Test-time Actor-Critic Approach to News Images Generation](https://arxiv.org/abs/2606.21304)
...
TL;DR: 本文提出 ACIG（Actor-Critic Image Generation），一种测试时、模型无关的 Actor-Critic 图像生成方法，用于根据新闻标题生成相关图像。该方法通过迭代生成提示、生成图像、评估结果并优化提示的反馈循环，在 MediaEval NewsImages 2026 挑战赛中取得最佳成绩。


<details>
  <summary>Details</summary>
Motivation: 新闻标题配图是新闻发布中的重要环节，但人工配图成本高、效率低。现有图像生成方法在新闻场景下生成的图像与标题相关性不足，需要一种能够自动评估并迭代优化生成结果的方法。

Method: 受强化学习中 Actor-Critic 范式的启发，ACIG 包含四个步骤：(1) 提示生成（Actor）——根据新闻标题生成图像描述提示；(2) 图像生成——使用现有文生图模型根据提示生成图像；(3) 评估（Critic）——评估生成图像与新闻标题的相关性；(4) 反馈循环——若评估不达标，则根据反馈优化提示并重新生成，直至满足要求。

Result: 在 MediaEval NewsImages 2026 挑战赛中，ACIG 在排行榜上取得了最佳结果，验证了其测试时迭代优化策略的有效性。

Conclusion: ACIG 是一种模型无关的测试时优化方法，通过 Actor-Critic 反馈循环显著提升了新闻标题到图像生成的相关性，展示了在测试阶段引入评估与迭代机制的价值。

Classification reason: 该论文关注新闻标题到图像的生成任务，属于文本到图像生成范畴，映射到 [CANONICAL] subtopic "文生图与图像扩散"。

Importance reason: Focuses on a narrow news image generation task with a test-time actor-critic method, lacking novelty in core continuous or autoregressive generation. Signals: Test-time optimization, Model-agnostic, News domain, Actor-critic for image generation, No continuous or autoregressive focus
...
### [124] [Adversarial Domain Prompt Tuning and Generation for Single Domain Generalization](https://arxiv.org/abs/2606.21736)
...
TL;DR: 本文提出渐进式对抗提示微调（PAPT）框架，首次利用预训练文生图扩散模型为单域泛化生成多样化的域外训练数据，通过对抗学习学习域不变类别提示和域特定风格提示，在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 单域泛化（SDG）旨在仅用一个域训练数据实现跨多个未见域的泛化。现有方法通过数据增强或图像生成生成域外数据，但手动设计文本提示描述所有可能域不切实际，且某些域特征过于抽象难以用文字描述。因此需要自动学习提示来引导扩散模型生成多样域风格图像。

Method: 提出PAPT框架，包含两组可学习的软提示：域不变类别提示（捕获类别共享特征）和域特定风格提示（建模域风格）。通过对抗学习机制，类别提示与风格提示相互竞争，使扩散模型在保持类别关键特征的同时生成不同域风格的图像。采用渐进式训练策略逐步增强生成多样性。

Result: 在多个单域泛化基准（如PACS、VLCS、OfficeHome等）上，PAPT生成的训练数据显著提升了分类器在未见域上的泛化性能，超越了现有SDG方法。消融实验验证了对抗提示学习和渐进式训练的有效性。

Conclusion: 本文证明了利用预训练文生图扩散模型进行数据增强是单域泛化的有效途径，对抗提示微调能够自动学习域风格提示而无需人工设计，为SDG提供了新的范式。

Classification reason: 该论文利用预训练文生图扩散模型生成训练数据以增强单域泛化，核心方法涉及文本到图像生成与扩散模型，因此映射到 [CANONICAL] 文生图与图像扩散 子主题。

Importance reason: Uses diffusion for domain generalization, but not directly aligned with priority subtopics like continuous models or video generation. Signals: Single domain generalization, adversarial prompt tuning, diffusion model for OOD data, not autoregressive, not continuous language model
...
### [125] [Multimodal Image Colorization: Quantifying the Impact of Text-Conditioned Guidance on Grayscale-to-Color Translation](https://arxiv.org/abs/2606.20722)
*Colten Reissmann, Hugo Garrido-Lestache Belinchon*

Main category: cs.LG, cs.CL, cs.CV, cs.GR
...
TL;DR: 本文量化了文本条件引导对灰度图彩色化任务的影响，通过控制变量实验比较 U-Net 和 Stable Diffusion 1.5 在有/无 CLIP 文本条件下的性能，发现文本条件在 PSNR、SSIM、LPIPS 和色彩丰富度上均带来一致且可量化的提升。


<details>
  <summary>Details</summary>
Motivation: 灰度图像在历史照片修复、医学影像和艺术媒体中广泛存在，但自动上色是一个病态问题（同一灰度输入对应多种合理彩色化结果）。现有研究缺乏对文本条件引导在彩色化任务中量化效果的系统分析。

Method: 采用控制变量实验设计，比较两种架构（U-Net 和 Stable Diffusion 1.5），每种架构分别在有无 CLIP 文本条件下进行灰度图彩色化，保持其他变量不变。使用 PSNR、SSIM、LPIPS 和色彩丰富度（colorfulness）作为评估指标。

Result: U-Net 中文本条件使 PSNR 提升 5.6%、SSIM 提升 1.2%、色彩丰富度提升 36.6%、LPIPS 降低 7.6%；Stable Diffusion 中文本条件使 PSNR 提升 5.8%、SSIM 提升 1.5%、色彩丰富度提升 0.6%、LPIPS 降低 11.3%。文本条件在两种架构上均带来一致改进。

Conclusion: 文本条件引导能够为灰度图彩色化任务提供一致且可量化的质量提升，且这种提升在不同架构规模上均成立。该研究为文本条件在图像恢复任务中的应用提供了实证基础。

Classification reason: 该论文研究文本条件引导的灰度图彩色化（图像到图像翻译），使用 Stable Diffusion 1.5 和 U-Net 架构，属于图像生成范畴。虽然任务特定（colorization），但核心是文本条件图像生成，与 [CANONICAL] 文生图与图像扩散 主题相关。

Importance reason: Image colorization with text conditioning is a narrow application, not aligned with the user's priority on continuous multimodal models, video generation, or world models. Signals: Text-conditioned colorization, U-Net vs Stable Diffusion, CLIP guidance, PSNR/SSIM/LPIPS metrics, Grayscale-to-color translation
...
### [126] [SeFi-Image: A Text-to-Image Foundation Model with Semantic-First Diffusion](https://arxiv.org/abs/2606.22568)
*SeFi-Team*
...
Importance: 高优先级 (87.44) - 未入选精读

TL;DR: SeFi-Image 提出语义优先扩散（Semantic-First Diffusion）范式，在潜空间扩散中先对语义信息进行建模再生成细节，以大幅降低训练计算量。在 1B/2B/5B 三个规模上验证，5B 模型仅用 125K A800 GPU 小时（约为 Z-Image 的 10-20%）即可达到与 Qwen-Image、Z-Image 相当或更优的性能，并提供了 DMD2 蒸馏的少步 Turbo 变体。


<details>
  <summary>Details</summary>
Motivation: 训练图像生成基础模型需要大量计算资源，现有方法虽尝试利用语义引导加速训练，但仅在 ImageNet 等简单数据集、低分辨率和小规模模型上验证。本文旨在探索语义优先扩散在大规模文生图任务中的有效性，以大幅降低训练成本。

Method: 提出语义优先扩散（Semantic-First Diffusion）范式，在潜空间扩散过程中先对高层语义信息进行建模，再逐步生成细节。基于该范式构建 SeFi-Image 模型，在 1B、2B、5B 三个参数量级上进行训练，并采用 DMD2 蒸馏得到少步 Turbo 变体以适应不同硬件和延迟需求。

Result: 5B 模型仅用 125K A800 GPU 小时训练（约为 Z-Image 的 10-20%），在 GenEval、DPG、LongTextBench、OneIG、CVTG-2K 等多个基准上达到与 Qwen-Image 和 Z-Image 相当或更优的性能。代码和权重已开源。

Conclusion: 语义优先扩散范式能够显著降低文生图基础模型的训练计算量，同时保持甚至超越现有大模型的生成质量，为高效训练图像生成模型提供了有价值的思路和可直接部署的模型选项。

Classification reason: 该论文提出 SeFi-Image，一个基于语义优先扩散（semantic-first diffusion）的文生图基础模型，属于文本到图像生成与图像扩散模型范畴，因此映射到 [CANONICAL] subtopic "文生图与图像扩散" (text_to_image_diffusion)。

Importance reason: Semantic-first diffusion significantly reduces training compute for text-to-image, aligning with user's priority on model efficiency and continuous multimodal generation. Signals: semantic-first diffusion, training efficiency, text-to-image foundation model, 5B model with 125K GPU hours, DMD2 distillation
...
### [127] [Modular Diffusion Models for Structured Visual Recognition](https://arxiv.org/abs/2606.22702)
...
TL;DR: 本文提出模块化扩散模型（MDMs），将扩散过程分解为任务特定的独立模块（如目标类别、空间位置、关系），每个模块独立学习并在推理时无缝集成，用于目标检测、实例分割和场景图生成等结构化视觉识别任务，能够捕捉输出空间中的多模态不确定性。


<details>
  <summary>Details</summary>
Motivation: 传统结构化视觉识别方法（目标检测、分割、场景图生成）通常产生确定性的点估计输出，无法捕捉复杂视觉场景中固有的不确定性（如遮挡物体的尺寸模糊、分割边界的局部歧义等）以及训练数据的噪声和稀疏性带来的多模态性。

Method: 提出模块化扩散模型（MDMs），将扩散过程分解为多个任务特定的独立模块，每个模块专注于结构化信息空间的不同方面（如目标类别、空间位置、物体间关系）。各模块可独立训练，推理时无需额外训练即可无缝集成。模块化设计使扩散过程能够处理异构输出空间（如连续的边界框和离散的类别标签）。

Result: 在目标检测、实例分割和场景图生成三个结构化任务上的实验验证了MDMs的有效性，能够生成多样化的合理输出，捕捉预测的不确定性。

Conclusion: 模块化扩散模型为结构化视觉识别提供了一种简单而有效的框架，通过分解扩散过程实现了对异构输出空间的建模和预测不确定性的捕捉，展示了扩散模型超越传统生成任务的应用潜力。

Classification reason: 该论文提出模块化扩散模型（MDMs）用于结构化视觉识别任务（目标检测、实例分割、场景图生成），属于扩散模型在视觉生成/识别中的应用。虽然任务偏识别而非生成，但核心方法是扩散模型，与[文生图与图像扩散]这一扩散模型大类最相关。

Importance reason: Modular diffusion for structured recognition is interesting but not aligned with your priority on continuous language/video generation or world models. Signals: modular diffusion, structured visual recognition, object detection, instance segmentation, scene graph generation
...
### [128] [Text-to-Image Generative AI for Modeling and Simulation: Methods, Opportunities, and Applications](https://arxiv.org/abs/2606.20991)
*Philippe J. Giabbanelli*

Main category: cs.AI, cs.ET
...
TL;DR: 本文是一篇面向建模与仿真(M&S)领域的文生图教程，系统介绍了文本到图像生成技术如何支持概念模型沟通、仿真结果可视化、教育材料生成以及多尺度仿真中的异构模型接口等任务，提供了概念指导与实践工作流。


<details>
  <summary>Details</summary>
Motivation: 建模与仿真(M&S)领域已广泛使用大语言模型进行文档、编码和解释，但图像生成的潜力尚未被充分探索。本文旨在向M&S社区介绍文生图技术，填补这一空白。

Method: 本文以教程形式展开，结合概念指导与实践工作流，解释现代图像生成器的工作原理、如何将提示词和仿真输出转化为视觉场景，以及如何将这些工具集成到可复现的本地管线中。重点放在可迁移的原则而非特定工具上。

Result: 本文提供了文生图在M&S中多个应用场景的系统化框架，包括概念模型可视化、仿真结果可视化、教育材料生成和多尺度仿真中的异构模型接口。

Conclusion: 文生图技术能够有效支持建模与仿真中的多项任务，M&S从业者应掌握评估、采纳和调整这些工具的能力，以将其融入仿真工作流。

Classification reason: 该论文是一篇面向建模与仿真(M&S)领域的文生图教程，系统介绍文生图技术如何支持概念模型可视化、仿真结果可视化、教育材料生成等任务。虽然侧重应用教程而非新方法，但核心主题是文本到图像生成，与[CANONICAL] subtopic "文生图与图像扩散"（text_to_image_diffusion）高度相关，故复用该分类。

Importance reason: Tutorial on text-to-image for modeling and simulation lacks focus on user's priority subtopics like continuous models or autoregressive generation. Signals: tutorial, modeling and simulation, text-to-image, conceptual guidance, not user's core direction
...
### [129] [Delta-Diffusion: Modeling Longitudinal Brain Amyloid-PET Trajectories via Conditional Poisson Diffusion Bridge](https://arxiv.org/abs/2606.22216)
*Yongheng Sun, Minhui Yu, Mengqi Wu, Maureen Kohi, Mingxia Liu*

Main category: eess.IV, cs.CV, cs.AI
...
TL;DR: Delta-Diffusion提出了一种基于条件Poisson扩散桥（PDB）的纵向脑淀粉样蛋白PET图像合成框架，将生成任务建模为以基线PET为锚点的条件分布转移，利用DiT架构和物理启发的泊松扰动来捕捉病理进展中的细微变化，在两个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 纵向脑PET成像是量化β-淀粉样蛋白时空积累的金标准，但高昂的运营成本和累积辐射风险限制了其临床广泛应用。现有深度生成模型在纵向图像合成中存在身份漂移（identity drift）和倾向于复制基线信号而非建模时间过渡的偏差，难以捕捉细微的病理进展。

Method: 提出Delta-Diffusion框架，将纵向PET合成重新定义为条件Poisson扩散桥（PDB）过程。与从高斯噪声出发的标准扩散模型不同，PDB以受试者的基线PET为数学锚点，将生成任务转化为淀粉样蛋白轨迹的条件分布转移。引入物理启发的泊松扰动到DiT架构中，使用自适应尺度-偏移调制（adaptive scale-shift modulation）精确校准合成结果与临床时间间隔和结构MRI上下文。设计体积感兴趣区域平衡目标（volume-of-interest balanced objective）来强调稀疏的高风险淀粉样蛋白积累区域。

Result: 在两个包含542名受试者的数据集上验证，Delta-Diffusion在捕捉纵向淀粉样蛋白沉积变化方面优于现有最先进方法，能够更准确地建模疾病进展过程中的细微病理变化。

Conclusion: Delta-Diffusion为追踪疾病进展提供了一个鲁棒的计算框架，通过条件扩散桥和物理启发的噪声建模有效解决了纵向医学图像合成中的身份漂移和基线复制偏差问题。

Classification reason: 该论文提出Delta-Diffusion，使用条件Poisson扩散桥和DiT架构进行纵向脑PET图像合成，属于图像扩散模型在医学图像生成中的应用。虽然应用领域特殊，但核心方法是扩散模型与DiT，与[CANONICAL]的"文生图与图像扩散"主题在方法论上高度匹配。

Importance reason: Focuses on medical PET imaging, which is outside your priority subtopics and has negative keyword overlap. Signals: medical imaging, longitudinal synthesis, Poisson diffusion bridge, not in priority subtopics, negative keyword medical diagnosis
...
### [130] [TriMotion: Modality-Agnostic Camera Control for Video Generation](https://arxiv.org/abs/2606.20774)
...
TL;DR: TriMotion 提出了一种模态无关的相机控制视频生成框架，通过将视频、姿态和文本三种模态的相机轨迹映射到共享的运动嵌入空间，实现跨模态的相机运动控制。论文还构建了 Motion Triplet 数据集并提供潜空间运动一致性约束，支持高质量视频生成及运动组合与跨模态插值等应用。


<details>
  <summary>Details</summary>
Motivation: 现有相机控制视频生成方法通常只支持单一模态输入（如显式姿态轨迹或参考视频），无法灵活处理异构用户输入。作者希望构建一个统一的、模态无关的框架，让用户可以通过视频、姿态序列或文本描述任意一种方式指定相机运动，从而提升生成系统的可用性和灵活性。

Method: 1) 构建 Motion Triplet Dataset：基于 Multi-Cam Video Dataset，利用相机外参生成几何驱动的运动文本描述，形成视频-姿态-文本三元组监督数据。2) 设计模态无关的运动编码器，将三种模态的相机轨迹映射到共享的运动嵌入空间。3) 引入潜空间运动一致性目标（latent motion consistency objective），直接在潜空间约束生成视频遵循目标相机轨迹，避免像素级解码开销。4) 基于扩散模型进行相机可控视频生成。

Result: 实验表明 TriMotion 在三种模态输入下均能生成高质量视频，准确跟随目标相机轨迹。共享运动嵌入空间还支持顺序运动组合和跨模态运动插值等灵活应用，验证了框架的有效性和泛化能力。

Conclusion: TriMotion 成功实现了模态无关的相机控制视频生成，通过共享运动嵌入空间统一了视频、姿态和文本三种输入方式，为视频生成中的相机控制提供了更灵活、更通用的解决方案。

Classification reason: 该论文研究相机可控视频生成，属于视频生成范畴，与 [CANONICAL] 文生视频与视频扩散 子主题高度相关。虽然论文强调多模态输入（视频、姿态、文本）控制相机轨迹，但其核心是视频生成中的运动/相机控制，属于视频扩散生成方法的扩展。

Importance reason: Strongly aligns with user's high-priority continuous multimodal and video generation directions, with a novel modality-agnostic camera control approach. Signals: Modality-agnostic camera control, shared motion embedding space, video generation, multimodal generation, continuous multimodal representation
...
### [131] [GroundShot: Visually Consistent Multi-Shot Long Video Generation via Entity-Grounded Shot Scheduling](https://arxiv.org/abs/2606.20799)
...
TL;DR: GroundShot 提出了一种无需训练、模型无关的智能体框架，通过实体级视觉记忆和镜头调度策略，在多镜头长视频生成中保持角色、物体和场景的视觉一致性，并配套发布了实体级一致性评测基准 GroundBench。


<details>
  <summary>Details</summary>
Motivation: 多镜头长视频生成中，随着镜头数量增加，跨镜头重复出现的实体（角色、物体、场景）会出现视觉漂移，导致一致性下降。现有方法缺乏对实体级一致性的显式建模和调度策略。

Method: GroundShot 构建实体级视觉记忆，通过以下步骤实现：1) 根据镜头作为实体参考的预期有用性调度生成顺序；2) 从已生成的视频中提取实体；3) 验证实体可靠性后再加入记忆库；4) 在生成每个镜头前从记忆中检索合适的实体参考。整个框架无需训练，可即插即用于现有视频生成模型。

Result: 在 GroundBench 基准上，GroundShot 相比现有方法在多镜头一致性上取得显著提升，且无需额外训练或模型修改。

Conclusion: GroundShot 通过实体锚定的镜头调度和视觉记忆机制，有效缓解了多镜头视频生成中的一致性漂移问题，为长视频生成提供了一种轻量级、模型无关的解决方案。

Classification reason: 该论文研究多镜头长视频生成中的视觉一致性，属于视频生成范畴。现有 canonical subtopic "文生视频与视频扩散"（text_to_video_diffusion）涵盖视频生成方法，虽然该论文侧重多镜头调度而非基础生成模型，但视频生成主题高度匹配，故复用该 subtopic。

Importance reason: Directly addresses video generation consistency, a key challenge in autoregressive and diffusion video generation, with a training-free agentic framework. Signals: video generation, multi-shot consistency, training-free, entity-grounded, agentic framework
...
### [132] [Go-with-the-Track: Video Compositing and Motion Control with Point Tracking](https://arxiv.org/abs/2606.20891)
...
TL;DR: Go-with-the-Track 是一个基于点轨迹条件（point-track）的视频扩散模型，通过联合多参考图像和参考锚定点轨迹实现精确的视频合成与运动控制。它引入空间感知的点轨迹嵌入，通过轻量适配器注入视频扩散 Transformer，并采用混合训练策略，在单一模型中同时实现运动控制和参考图像合成。


<details>
  <summary>Details</summary>
Motivation: 电影制作需要精确的运动控制和参考图像合成能力，但现有方法将两者分开处理：点轨迹条件图像到视频模型将内容插入限制在第一帧，而参考到视频模型缺乏对参考内容在帧间集成的细粒度时空控制。因此需要一种统一的方法同时实现精确合成和运动控制。

Method: 提出 Go-with-the-Track 模型，核心创新包括：(1) 扩展传统点轨迹为参考锚定点轨迹，建立生成帧与参考图像之间的显式对应关系；(2) 空间感知点轨迹嵌入，使用坐标级 MLP 加时序池化编码完整点轨迹坐标序列，嵌入相似度与空间邻近度直接相关；(3) 通过轻量适配器将点轨迹嵌入注入视频扩散 Transformer，解决像素到 patch 分辨率不匹配问题；(4) 混合训练策略，联合动态、静态和合成场景视频数据集训练以增强运动可控性。

Result: 实验表明 Go-with-the-Track 在单一模型中实现了优越的运动和参考控制，并支持新能力：多参考条件视频生成与点轨迹驱动合成，以及静态和动态场景的相机控制。

Conclusion: Go-with-the-Track 成功统一了视频合成与运动控制，通过参考锚定点轨迹和空间感知嵌入实现了精确的帧间参考内容集成和运动控制，为电影制作等应用提供了更灵活的视频生成工具。

Classification reason: 该论文提出基于点轨迹条件（point-track）的视频扩散模型，实现视频合成与运动控制，属于视频扩散生成方法，与 [CANONICAL] 文生视频与视频扩散 子主题高度匹配。

Importance reason: Strongly aligns with user's priority on multimodal generation and video diffusion, with novel point-track conditioning for motion control. Signals: video diffusion, point-track conditioning, motion control, reference compositing, multimodal generation
...
### [133] [Towards Error-Free Long Video Generation](https://arxiv.org/abs/2606.22370)
...
TL;DR: 本文提出一个面向无限长度、无错误累积的长视频生成框架，通过因果注意力跨片段建模、KV缓存机制和截断整流流（T-RFlow）技术，在短视频数据上微调扩散模型作为视频扩展模型，实现分钟级高质量、身份一致的单镜头长视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法在生成长视频时面临错误累积、属性漂移和长视频数据稀缺三大挑战，难以生成高质量、身份一致的长视频。

Method: 首先在短视频数据上微调扩散模型作为视频扩展模型，以自回归方式生成时序连贯的片段。借鉴LLM的成功经验，在片段间采用因果注意力（片段内双向注意力、片段间单向注意力）进一步在长视频数据上微调，保持长期上下文信息。推理时使用KV缓存机制维持恒定内存，并引入截断整流流（T-RFlow）技术抑制错误累积。

Result: 实验表明该方法能生成高质量、动态且身份一致的分钟级长视频，建立了新的基准。

Conclusion: 本文提出的框架有效解决了长视频生成中的错误累积和属性漂移问题，为现实且连贯的分钟级视频合成建立了新标杆。

Classification reason: 该论文研究长视频生成，使用扩散模型进行视频扩展与自回归生成，属于视频扩散/文生视频范畴，与 [CANONICAL] 文生视频与视频扩散 (text_to_video_diffusion) 高度匹配。

Importance reason: Directly addresses long video generation with error-free autoregressive extension, highly relevant to user's priority on autoregressive video generation and world models. Signals: autoregressive video generation, long video generation, error accumulation mitigation, causal attention across clips, KV caching
...
### [134] [Gen2Balance: Generative Balancing for Long-Tailed Video Action Recognition](https://arxiv.org/abs/2606.22416)
...
TL;DR: Gen2Balance 提出利用文生视频扩散模型为长尾视频动作识别任务生成平衡的训练数据，通过动作描述引导的文本提示生成稀缺类别的视频片段，结合两阶段训练策略缓解域偏移，在 UCF-LT、K100-LT 和 RareAct 上显著超越现有长尾学习方法。


<details>
  <summary>Details</summary>
Motivation: 视频动作识别中的长尾分布问题严重，尾部类别样本稀缺导致模型性能严重下降。现有长尾学习方法（如重采样、重加权、知识迁移）在视频领域效果有限，且尾部类别数据难以收集。作者希望利用生成模型主动合成稀缺类别的视频数据，从根本上解决数据不平衡问题。

Method: Gen2Balance 包含三个关键组件：(1) 基于动作描述（action profile）和训练样本生成多样化文本提示，输入文生视频扩散模型（如 VideoFusion/ModelScope）合成尾部类别的视频片段；(2) 将合成视频与真实视频混合构建平衡训练集；(3) 两阶段训练策略：第一阶段在平衡的合成+真实数据上训练，第二阶段仅在真实数据上微调以缓解域偏移。

Result: 在 UCF-LT 上比最强基线提升 5.1%，在 K100-LT 上提升 7.0%，在 RareAct 稀有动作（如 cut keyboard）上提升 31.9%。部分平衡策略（仅添加 27% 计算量的合成数据）即可获得 79% 的性能增益，展示了良好的可扩展性。

Conclusion: Gen2Balance 证明了文生视频生成模型可以有效缓解视频动作识别中的长尾问题，通过生成稀缺类别的训练数据显著提升尾部类别识别精度。两阶段训练策略有效缓解了合成数据与真实数据之间的域偏移。该方法具有实际可扩展性，为生成模型辅助长尾学习提供了新范式。

Classification reason: 该论文使用文生视频扩散模型（text-to-video generative model）生成训练数据以解决长尾视频动作识别问题，核心涉及视频生成技术，因此映射到多模态生成方向下的[CANONICAL]文生视频与视频扩散（text_to_video_diffusion）子主题。

Importance reason: Uses video diffusion for data augmentation, but the core task (long-tail action recognition) is not a priority subtopic and lacks direct connection to continuous multimodal models or world models. Signals: Video diffusion for data augmentation, Long-tail action recognition, Two-stage training for domain shift, Partial balancing reduces compute cost, Strong gains on RareAct
...
### [135] [OrthoMotion:Disentangling Camera and Subject Motion via Geometry Semantics Orthogonal Attention](https://arxiv.org/abs/2606.22835)
*Zijie Meng*
...
Importance: 高优先级 (87.08) - 未入选精读

TL;DR: OrthoMotion 通过几何语义正交注意力机制，在注意力算子层面将相机运动（RoPE相位旋转）与主体运动（门控值注入）解耦，利用轻量解耦正则化迫使两个响应子空间正交，首次从构造上保证解耦而非期望其涌现，在保持保真度的同时将串扰降低2.4倍以上。


<details>
  <summary>Details</summary>
Motivation: 可控视频生成需要独立控制相机和主体运动，但2D条件（如光流）将两者纠缠在一起——相机和物体引起的光流共享相同的逆深度缩放，无法仅从图像证据中分离。现有方法无法从表示层面解决这一非可识别逆问题。

Method: 将解耦问题重新定义为算子设计问题：在注意力算子层面，将相机运动路由到几何通道（RoPE相位的保范旋转），将主体运动路由到语义通道（交叉注意力中的门控值注入）。这两个子算子在代数上互补（旋转 vs 平移），通过轻量解耦正则化迫使它们的响应子空间正交，从而消除干扰。

Result: 在相机和主体精度上达到SOTA，同时最小化串扰。提出新的Cross-Talk Error (CTE)指标，将串扰降低2.4倍以上，且不损失保真度，可泛化到不同骨干网络。

Conclusion: OrthoMotion 首次从构造上保证相机与主体运动的解耦，而非依赖模型涌现，为可控视频生成提供了更可靠的控制范式。

Classification reason: 该论文提出可控视频生成中相机运动与主体运动的解耦方法，属于视频生成领域，映射到 [CANONICAL] 文生视频与视频扩散（text_to_video_diffusion）子主题。

Importance reason: Directly addresses controllable video generation via a novel orthogonal attention mechanism, aligning with high-priority video generation and world model subtopics. Signals: controllable video generation, camera-subject disentanglement, orthogonal attention, cross-talk reduction, RoPE phase rotation
...
### [136] [ScalePredictor: Instance-aware Scale Learning for Accurate Quantization of Vision Transformers](https://arxiv.org/abs/2606.21947)
...
Importance: 普通相关 (57.88) - 未入选精读

TL;DR: 本文提出ScalePredictor，一种面向视觉Transformer的动态量化框架，通过浅层激活分布范围预测深层最优量化尺度，利用泰勒多项式投影模块高效生成所有量化尺度，在ImageNet上取得优于现有PTQ方法的精度-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化（PTQ）方法采用静态量化范式，对所有实例统一应用相同的量化尺度，但自然图像多样性导致不同样本的激活分布差异显著，静态方法本质上是次优的。

Method: 首先发现浅层激活的分布范围与深层最优量化尺度之间存在隐藏相关性；基于此设计尺度学习机制：通过高效的范围提取方法获取浅层鲁棒的分布统计量，再输入到泰勒驱动的多项式尺度投影模块，一次性生成所有量化尺度。

Result: 在ImageNet上的大量实验表明，ScalePredictor在精度-效率权衡上持续优于现有PTQ方法，计算开销极小，无需昂贵的即时校准。

Conclusion: ScalePredictor通过实例感知的动态量化尺度学习，有效解决了静态量化在多样化图像上的次优问题，为ViT的高效部署提供了实用方案。

Classification reason: 该论文研究视觉Transformer的后训练量化（PTQ）方法，属于生成模型/视觉模型的高效化与加速方向，与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Proposes dynamic quantization for ViTs, improving efficiency, but is not directly aligned with top-priority continuous multimodal or video generation topics. Signals: dynamic quantization, vision transformer, post-training quantization, efficient inference, instance-aware scale learning
...
### [137] [CoDMD: Copula-aware Distribution Matching Distillation for Fast Video Generation](https://arxiv.org/abs/2606.21982)
...
TL;DR: CoDMD 提出了一种基于 Copula 感知的分布匹配蒸馏方法，通过引入轻量级的关系正则化器，在视频扩散模型的少步蒸馏中保持帧间和样本间的结构一致性，解决了标准 DMD 在有限 NFE 下布局不稳定、过饱和和运动断裂的问题，在 Wan-2.1-T2V 模型上实现 25 倍加速并取得领先的 VBench 分数。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型在实际部署中面临推理速度慢的问题，少步蒸馏是加速的关键技术。然而，现有的分布匹配蒸馏（DMD）方法在极少数步（如4步）下性能严重退化，表现为布局不稳定、过饱和和运动断裂。作者分析发现，标准 DMD 的逐坐标梯度只关注样本内分布匹配，缺乏对批次元素间和时序帧间关系结构的显式约束，导致在少步场景下容易陷入局部最优。

Method: CoDMD 提出一种轻量级的关系正则化器，核心思想是重用冻结的教师模型和在线假样本模型已经计算出的分数估计，构建样本间和帧间的成对关系矩阵，然后通过一个额外的分布目标来匹配这些关系矩阵。该方法不需要额外的网络、数据集或采样轨迹，仅作为 DMD 目标的补充正则化项，引导蒸馏过程保持样本间和帧间的 copula 结构。

Result: 在 Wan-2.1-T2V 模型系列（1.3B 和 14B 规模）上，CoDMD 将 50 步教师模型蒸馏为 4 步学生模型，实现约 25 倍加速。VBench 分数分别达到 84.46 和 84.87，优于基于轨迹的方法（rCM: 82.81 & 84.05）和基于分布的方法（DMD: 83.38 & 83.81）。

Conclusion: CoDMD 通过引入 copula 感知的关系正则化，有效解决了 DMD 在视频生成少步蒸馏中的退化问题，在保持高质量的同时实现了显著的加速效果，为视频扩散模型的高效部署提供了有效方案。

Classification reason: 该论文提出 CoDMD，一种面向视频扩散模型的少步蒸馏方法，旨在加速视频生成（从50步蒸馏到4步，加速约25倍），属于生成模型加速与高效化的典型工作，因此映射到 [CANONICAL] subtopic "efficient_generation"（生成模型加速与高效化）。

Importance reason: Directly addresses efficient video generation acceleration, a high-weight priority subtopic, with a novel copula-aware distillation method. Signals: video generation acceleration, distribution matching distillation, copula-aware regularization, 25x speedup, Wan-2.1-T2V model
...
### [138] [Curvature-Adaptive Consistency Flow Matching: Autonomous Trajectory Optimization via Reinforcement Learning](https://arxiv.org/abs/2606.22394)
...
TL;DR: 本文提出曲率自适应一致性流匹配（CACFM），利用轻量级强化学习智能体动态探索概率流 ODE 轨迹，自动构建关注关键区域的效率导向课程，结合流分布匹配蒸馏（DMD）目标，在 FLUX 和 SDXL 等大规模模型上实现少步生成的新 SOTA，有效缓解结构畸形并保留高频细节。


<details>
  <summary>Details</summary>
Motivation: 现有一致性蒸馏方法使用静态采样策略（如 Logit-Normal 先验），无法适应训练过程中不断变化的学习需求。作者发现一致性蒸馏的优化瓶颈主要存在于边界阶段（初始化或最终细化）而非中间步骤，呈现 U 形难度分布，因此需要一种动态、自适应的蒸馏策略。

Method: 提出 Curvature-Adaptive Consistency Flow Matching (CACFM)：1) 将蒸馏建模为动态决策过程，使用轻量级 RL 智能体主动探测概率流 ODE 轨迹，自动构建关注关键区域的效率导向课程；2) 提出 Flow Distribution Matching Distillation (DMD) 目标函数，用于分布匹配蒸馏；3) 无需人工调度即可自适应地优先处理困难区域。

Result: 在 FLUX 和 SDXL 等大规模模型上取得新的 SOTA 结果，在极端少步（few-step）生成场景下有效缓解结构畸形并保留高频细节，实现了前所未有的视觉保真度。

Conclusion: CACFM 通过强化学习驱动的自适应课程学习策略，有效解决了一致性蒸馏中静态采样无法适应动态学习需求的问题，为扩散模型的高效少步生成提供了新的范式。

Classification reason: 该论文提出 Curvature-Adaptive Consistency Flow Matching (CACFM)，通过强化学习驱动的蒸馏策略加速扩散模型推理，属于生成模型加速与高效化（efficient_generation）这一 [CANONICAL] 子主题。

Importance reason: Proposes a novel RL-driven curriculum for consistency distillation that achieves SOTA few-step generation on FLUX/SDXL, directly advancing efficient inference and generative model acceleration. Signals: consistency distillation, flow matching, reinforcement learning, few-step generation, FLUX
...
### [139] [One-Step Flow Matching for Generative Modeling of Path-Dependent Physical Fields](https://arxiv.org/abs/2606.22752)
...
TL;DR: 本文提出一种基于Transformer的flow matching模型，在VAE潜空间中通过设计非高斯源分布实现单步生成路径依赖的塑性应力场，将物理模拟建模为视频合成任务，在消费级GPU上实现约两个数量级的加速。


<details>
  <summary>Details</summary>
Motivation: 复杂几何结构中的路径依赖本构模型（如塑性）的物理模拟计算成本极高，现有生成模型（如DDPM）需要数百步采样且难以处理路径依赖的应力场，亟需高效且能处理时序依赖的生成方法。

Method: 提出基于Transformer backbone的flow matching模型，在VAE潜空间中将塑性场模拟建模为视频合成任务（直接生成所有时间步的应力场）；设计非高斯源分布以减少条件传输路径交叉，实现无需蒸馏的单步生成；引入token级加载嵌入和两个辅助网络增强路径依赖模拟性能。

Result: 在有限训练数据下，模型能准确生成高分辨率路径依赖应力场；相比有限元分析，在CPU上加速6-7倍，在消费级GPU上加速约两个数量级。

Conclusion: 本文证明了flow matching结合非高斯源分布设计可以在无需蒸馏的情况下实现单步高质量生成，为路径依赖物理场的快速模拟提供了有效方案，展示了生成模型在计算力学中的巨大潜力。

Classification reason: 该论文提出基于flow matching的单步生成模型用于物理场模拟，核心贡献在于实现单步采样（无需蒸馏），属于生成模型加速与高效化方向，因此映射到[CANONICAL] subtopic "生成模型加速与高效化"。

Importance reason: Focuses on physical simulation acceleration, not directly on user's priority subtopics like continuous language models or video generation. Signals: One-step flow matching, VAE latent space, path-dependent physical fields, transformer backbone, non-Gaussian source distribution
...
### [140] [NullFlow: One-Step Generative Reconstruction](https://arxiv.org/abs/2606.22696)
...
TL;DR: NullFlow 提出一种一步式生成图像重建框架，通过将生成流约束在测量一致子空间中，学习流的平均速度，实现单次网络评估即可完成采样，在图像修复任务上匹配 SOTA 扩散求解器性能，同时将推理成本从数百次网络评估降至一次。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型和流匹配的逆问题求解器需要多次网络评估（通常数百步）才能完成采样，推理效率极低。虽然已有一些少步采样方法，但它们通常需要额外的数据保真校正步骤，增加了复杂性。作者希望设计一种真正的一步式生成重建方法，无需任何额外的数据保真校正。

Method: NullFlow 的核心思想是将生成流约束在测量一致子空间中，使得流永远不会离开该子空间，从而无需单独的数据保真校正。通过学习该约束流的平均速度，模型可以在单次网络评估中完成采样，避免了传统流匹配的逐步积分。作者证明了该约束流的平均速度对应的训练目标的全局最小化器是一个一步后验采样器。

Result: 在图像修复（inpainting）任务上，NullFlow 匹配了最先进的扩散求解器的重建质量，同时将推理从数百次网络评估减少到仅一次。

Conclusion: NullFlow 提供了一种原则性的一步式生成重建框架，通过测量一致子空间约束和平均速度学习，在保持生成质量的同时大幅提升推理效率，为高效生成式逆问题求解开辟了新路径。

Classification reason: 该论文提出NullFlow，一种一步式生成图像重建框架，将生成流约束在测量一致子空间中，通过单次网络评估完成采样，避免了传统flow matching的逐步积分。这属于生成模型加速与高效化（少步采样/一步采样）方向，与[CANONICAL] subtopic "生成模型加速与高效化"高度匹配。

Importance reason: One-step generative reconstruction via constrained flow matching directly advances efficient inference, a high-priority subtopic. Signals: one-step sampling, flow matching acceleration, measurement-consistent subspace, inference cost reduction, image inpainting SOTA
...
### [141] [ScalingAttention: Discovering Intrinsic Sparse Attention Topology for Video Diffusion Transformers](https://arxiv.org/abs/2606.23019)
...
TL;DR: ScalingAttention 提出一种无需训练的稀疏注意力框架，通过发现权重编码的内在稀疏拓扑（WEST）和保真度感知的敏感度调节（FAST），结合硬件对齐的块稀疏内核，在 Wan2.1 视频扩散 Transformer 上实现最高 1.90 倍端到端加速，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 视频扩散 Transformer（DiT）依赖 3D 全注意力机制，计算复杂度呈二次增长。现有稀疏方法中，动态剪枝运行时开销大且内存碎片化，静态启发式方法无法捕捉细粒度依赖。因此需要一种无需训练、高效且保真的稀疏注意力加速方案。

Method: 提出 ScalingAttention 框架，包含两个核心组件：(1) WEST（Weight-Encoded Sparse Topology），利用注意力头的高质量区域收敛到稳定、与提示无关的内在稀疏拓扑这一归纳偏置，离线提取鲁棒的块稀疏先验掩码，消除运行时搜索；(2) FAST（Fidelity-Aware Sensitivity Tuning），根据扩散保真度需求自适应调节每个注意力头的稀疏度。同时设计了硬件对齐的按位块稀疏内核以实现实际加速。

Result: 在 Wan2.1 视频扩散模型上，ScalingAttention 实现最高 1.90 倍端到端加速，在保持优越保真度的同时建立了新的帕累托前沿，优于现有基线方法。

Conclusion: ScalingAttention 通过发现注意力头中权重编码的内在稀疏拓扑，实现了无需训练、高效且保真的视频扩散 Transformer 加速，为视频生成模型的效率优化提供了新思路。

Classification reason: 该论文提出视频扩散Transformer的稀疏注意力加速方法，属于生成模型效率优化，映射到[CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Directly addresses efficient inference for video diffusion transformers, a high-priority subtopic with strong boost keywords. Signals: Training-free sparse attention, 1.90X speedup on Wan2.1, Hardware-aligned block-sparse kernel, Intrinsic sparse topology discovery, Fidelity-aware sparsity tuning
...
### [142] [HyperQuant: A Rate-Distortion-Optimal Quantization Pipeline for Large Language and Diffusion Models](https://arxiv.org/abs/2606.23406)
...
TL;DR: HyperQuant 是一个统一的训练后量化管线，结合随机化Hadamard变换、最优格点量化（E8/D4/A2/Z）、无损位剥离与Rice编码、以及KV缓存偏差校正，在3-5比特权重量化和低至1.7比特KV量化上超越现有方法，并在19B参数LTX-2视频DiT模型上实现近无损压缩。


<details>
  <summary>Details</summary>
Motivation: 大语言模型和扩散Transformer的部署受限于巨大的模型权重和KV缓存内存开销，现有量化方案在极低比特率下精度损失严重，缺乏统一的、率失真最优的量化框架。

Method: HyperQuant 管线包含四个组件：(i) 逐块随机化Hadamard变换（RHT），使权重和激活的逐坐标分布近似高斯；(ii) 量化到低维最优格点（E8、D4、A2或Z）；(iii) 无损位剥离与近熵最优的变长Rice编码；(iv) KV缓存的偏差校正方法，保持内积无偏以保留注意力语义。同时集成了8-bit和4-bit Tensor-Core MMA路径（fp8-e4m3、int8、nvfp4、mxfp4）。

Result: 在多个LLM上，HyperQuant在3-5比特权重量化下全面优于HIGGS方案；KV量化低至1.7 bps时优于TurboQuant和OCTOPUS。在19B参数LTX-2视频DiT模型上量化后无可见逐帧伪影。在H100上4 bps时，线性权重压缩约3.9倍，KV缓存压缩约3.79倍，达到近无损质量。

Conclusion: HyperQuant 通过组合Hadamard变换、格点量化、熵编码和偏差校正，实现了统一的率失真最优量化管线，在LLM和扩散Transformer上均取得优异效果，为高效部署提供了实用方案。

Classification reason: 该论文提出统一的量化管线（HyperQuant），用于大语言模型和扩散Transformer的权重与KV缓存量化，属于生成模型效率优化（量化压缩）方向，因此映射到[CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Strong quantization pipeline for diffusion and LLM models, but only tangentially touches priority subtopics like efficient inference without direct focus on continuous models or video generation. Signals: 4-bit weight & KV cache quantization, LTX-2 video DiT compression, lattice quantization + Hadamard transform, near-lossless 3.9x compression, no direct video generation or world model contribution

Abstract: We present HyperQuant (Hadamard, optimallY Packing, Entropy Rice-coding), a unified post-training quantization pipeline for the weights and the KV cache of large language and diffusion transformers. Across a suite of self-contained experiments (Table 1), HyperQuant outperforms the recent HIGGS scheme at every operating point from 3 to 5 bits per scalar (bps) on weights, and beats both TurboQuant and OCTOPUS on KV quantization down to 1.7 bps. Beyond the LLM setting, HyperQuant quantizes the 19B-parameter LTX-2 DiT video model with no observable per-frame artifacts. End-to-end on an H100 at 4 bps, HyperQuant compresses the linear weights ~3.9x and the KV cache ~3.79x at near-lossless quality. HyperQuant combines four known ideas into a single construction: (i) a per-tile Randomized Hadamard Transform that makes the per-coordinate distribution of weights and activations approximately Gaussian; (ii) quantization to a low-dimensional optimal lattice (E8, D4, A2, or Z); (iii) lossless bit-stripping and near-entropy-optimal variable-length Rice coding of the lattice indices; and (iv) bias-correction methods for the KV cache that keep the reconstruction unbiased under inner products, preserving attention semantics. We further integrate the pipeline with 8-bit and 4-bit Tensor-Core MMA paths (fp8-e4m3, int8, nvfp4, mxfp4), and find that int8 beats fp8 on the post-RHT lattice output. Project page:

</details>


### [143] [GRINQH: Graded Input-based Quantization Hierarchy for Efficient LLM Generation](https://arxiv.org/abs/2606.23419)
...
Importance: 普通相关 (59.16) - 未入选精读

TL;DR: GRINQH提出了一种基于输入激活幅度的分级量化框架，通过将权重通道动态分配到不同精度级别，在LLM自回归解码阶段实现灵活的平均位宽，从而加速生成并建立新的质量-速度帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: LLM自回归解码主要受限于GPU内存带宽，尤其在边缘计算场景中。现有量化方法将推理过程视为均匀过程，未能区分计算密集的预填充阶段和内存密集的解码阶段之间的不对称性，因此需要一种能针对性加速解码阶段的量化方法。

Method: GRINQH是一种仅权重的训练后量化框架，利用激活幅度作为计算重要性的代理指标，动态地将权重通道分配到不同精度级别（如2/3/4位），在解码时实现灵活的平均位宽。同时结合稀疏化，并设计了分层嵌套内存布局的多精度存储自定义GPU内核来验证理论加速。

Result: 在Llama3和Qwen3模型上，GRINQH在3位和4位设置下优于现有固定精度和混合精度基线，甚至实现了有效的2位生成。通过自定义GPU内核实验验证了理论加速效果，建立了LLM生成质量与推理速度之间新的帕累托前沿。

Conclusion: GRINQH通过统一量化与稀疏化，利用激活幅度指导分级精度分配，有效加速了LLM解码阶段，为边缘部署提供了新的效率-质量权衡方案。

Classification reason: 该论文提出GRINQH，一种基于激活幅度的分级量化框架，用于加速LLM自回归解码。这属于生成模型（LLM文本生成）的效率优化，与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配，该子主题涵盖量化、加速等效率优化方法。

Importance reason: Strong quantization method for LLM efficiency, but only tangentially touches priority subtopics like continuous models or video generation. Signals: LLM quantization, efficient inference, weight-only PTQ, dynamic precision, Pareto frontier
...
### [144] [Keep The Essentials: Efficient Reference Conditioned Generation via Token Dropping](https://arxiv.org/abs/2606.23682)
...
Importance: 高优先级 (80.26) - 未入选精读

TL;DR: 本文提出Sparse Context方法，通过丢弃参考图像中的冗余token来加速参考条件扩散模型。训练时采用随机token丢弃增强鲁棒性，推理时使用任务感知的token选择策略保留重要区域。在单参考生成中实现2倍加速，多参考生成中实现4倍加速，且不损失视觉质量。


<details>
  <summary>Details</summary>
Motivation: 参考条件扩散模型在运行时计算成本高昂，且随参考图像数量增加而严重扩展。虽然扩散模型效率已被广泛研究，但参考条件生成场景中的效率问题尚未被充分探索，尤其是参考图像被表示为密集token网格存在大量冗余。

Method: 提出Sparse Context方法：(1) 训练阶段采用随机token丢弃策略，在不同丢弃比例下微调模型，使其对部分参考表征具有鲁棒性；(2) 推理阶段使用任务感知的token选择策略（如基于注意力分数或空间位置），优先保留参考图像中最具信息量的区域，自适应调整token预算。

Result: 在空间对齐编辑和主体驱动生成任务上，单参考生成实现2倍加速，多参考生成实现4倍加速，且不牺牲视觉质量。

Conclusion: 通过token丢弃构建稀疏参考表征是一种有效的加速参考条件扩散模型的方法，训练时的随机丢弃策略使模型与特定token选择规则解耦，推理时的任务感知选择进一步提升了效率与质量的平衡。

Classification reason: 该论文提出通过token丢弃来加速参考条件扩散模型的推理，属于生成模型效率优化，与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Directly addresses model efficiency for controllable generation, a priority subtopic, with practical speedups. Signals: Token dropping for reference-conditioned diffusion, 2-4x inference speedup, Task-aware token selection, Robustness via random dropping training, Relevant to efficient inference and controllable generation
...
### [145] [FlowCodec: One-Step Flow Prior for Generative Image Compression](https://arxiv.org/abs/2606.21030)
...
Importance: 高优先级 (87.48) - 未入选精读

TL;DR: FlowCodec 提出一种解耦的框架，将预训练的大规模文生图扩散先验（如 Qwen-image-2512 和 FLUX.1-dev）以即插即用的方式集成到超低比特率图像压缩中。该方法将流程分解为潜压缩和潜传输两个阶段，无需额外条件信号或辅助网络，仅需极少量可训练参数（<0.54%）即可支持多码率，在 <0.05 bpp 下实现高质量压缩。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的图像压缩方法虽然利用强大的生成先验在超低比特率下取得了出色的感知质量，但通常需要精心设计的条件机制或辅助分支，且随着模型规模增长，重新训练成本高昂。作者希望探索能否通过更简单、更具可扩展性的设计，将更强的生成先验集成到压缩中。

Method: FlowCodec 将压缩流程解耦为两个阶段：(1) 潜压缩阶段，将干净的潜变量映射到受比特率约束的噪声潜变量；(2) 潜传输阶段，利用预训练的生成先验（Qwen-image 或 FLUX.1-dev）在单步中从噪声潜变量恢复干净潜变量。该方法不需要额外的条件信号或辅助网络，通过轻量级适配即可灵活支持多码率，可训练参数不到生成骨干网络的 0.54%。

Result: 在 <0.05 bpp 的超低比特率下，FlowCodec 保持了高视觉质量。Qwen-image 变体在 LPIPS 和 DISTS 指标上显著优于现有方法，两个变体均实现了比现有单步扩散方法更高的 PSNR 和更快的编码速度，FLUX 变体还保持了有竞争力的解码速度。

Conclusion: FlowCodec 验证了将预训练的大规模文生图先验以解耦、即插即用的方式集成到图像压缩中的可行性，为生成式图像压缩提供了一种更简单、可扩展的设计范式。

Classification reason: 该论文提出FlowCodec，利用预训练的文生图扩散先验（Qwen-image, FLUX.1-dev）实现超低比特率图像压缩，核心贡献在于单步（one-step）潜空间传输，属于生成模型在压缩任务中的高效应用。匹配[CANONICAL] subtopic "efficient_generation"（生成模型加速与高效化），因为其核心是单步采样/高效推理。

Importance reason: Integrates pretrained text-to-image diffusion priors into compression with minimal training, aligning with efficient inference and generative model acceleration. Signals: generative image compression, diffusion prior, one-step flow, ultra-low bitrate, efficient adaptation
...
### [146] [ZeroGVC: Zero-Shot Generative Video Compression with Autoregressive Diffusion Priors](https://arxiv.org/abs/2606.22371)
...
Importance: 值得关注 (69.29) - 未入选精读

TL;DR: ZeroGVC 提出了一种零样本生成式视频压缩框架，利用预训练的自回归扩散先验（扩散码本模型）实现超低码率下的高质量视频重建。通过码本引导的潜压缩和少步一致性采样，无需额外训练即可实现高效视频压缩。


<details>
  <summary>Details</summary>
Motivation: 现有生成式视频压缩方法通常需要额外训练来使生成模型适应紧凑表征的重建，缺乏零样本能力。作者希望利用预训练扩散先验，在不进行额外训练的情况下实现超低码率下的感知高质量视频压缩。

Method: ZeroGVC 将每个 GOP 的第一帧用图像编解码器编码，后续 P 帧通过码本引导的自回归潜压缩（Codebook-Guided Autoregressive Latent Compression）表示。该方法利用扩散码本模型在少步一致性采样中的有效性，选择可复现码本噪声向量的紧凑组合，引导潜去噪轨迹朝向目标 P 帧，解码器仅需少量去噪步即可复现相同轨迹。此外，设计了可选的 bidirectional reference mode，利用下一个 I 帧上下文缓解误差传播而不增加码率开销。

Result: 在标准视频压缩基准上的大量实验表明，ZeroGVC 在超低码率下无需任何额外训练即可实现优越的感知重建质量。

Conclusion: ZeroGVC 证明了预训练自回归扩散先验可以零样本地用于高效视频压缩，为生成式视频压缩提供了一种无需训练的新范式，在超低码率场景下具有显著优势。

Classification reason: 该论文提出零样本生成式视频压缩框架，利用预训练自回归扩散先验实现超低码率下的高质量重建，核心贡献在于通过码本引导的潜压缩和少步一致性采样实现高效生成。这与 [CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配，因为论文重点在于扩散模型的少步采样加速和压缩效率优化。

Importance reason: Novel zero-shot video compression using autoregressive diffusion priors, but tangential to user's core focus on continuous multimodal models and generation. Signals: zero-shot generative video compression, autoregressive diffusion priors, codebook-guided latent compression, few-step consistency sampling, ultra-low bitrate reconstruction
...
### [147] [Balancing Performance and Diversity in GRPO Autoregressive Text-to-Image Post-Training](https://arxiv.org/abs/2606.21498)
...
TL;DR: 本文系统研究了GRPO风格自回归文生图后训练中对齐策略中参考策略散度的选择问题，在统一的f-散度框架下分析了前向KL、反向KL和JS散度对token级更新的影响，发现JS正则化在优化性能和生成多样性之间取得了最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 自回归文生图模型虽发展迅速，但生成图像与人类偏好对齐仍具挑战。现有GRPO风格在线强化学习方法通常将参考策略散度视为固定超参数，忽略了不同散度选择对策略优化的直接影响。

Method: 在统一的f-散度框架下系统分析前向KL、反向KL和JS散度在GRPO风格自回归T2I对齐中的作用，推导不同散度对token级更新的重塑方式，并在LlamaGen和Janus-7B上进行实验验证。

Result: JS散度在大多数评测指标上取得了最强或极具竞争力的优化性能，同时保持了良好的生成多样性，实现了性能与多样性的最佳权衡。

Conclusion: 在GRPO风格自回归文生图后训练中，JS正则化通过缓解对参考策略的均匀偏差同时抑制过大偏离，实现了优化性能与生成多样性的有效平衡，是散度选择的较优方案。

Classification reason: 该论文研究自回归文生图模型的后训练对齐问题，使用GRPO风格的在线强化学习与f-散度正则化来对齐人类偏好，属于生成模型对齐与奖励（generation_alignment）这一[CANONICAL]子主题。同时论文使用Janus-7B（一个统一多模态模型）进行实验，也与多模态理解与生成方向相关。

Importance reason: Directly addresses autoregressive image generation alignment, a high-priority subtopic, with a novel f-divergence analysis for GRPO. Signals: autoregressive image generation, GRPO alignment, f-divergence analysis, generation diversity, LlamaGen

Abstract: Autoregressive text-to-image (T2I) generation has recently advanced rapidly, yet aligning generated images with human preferences remains challenging. GRPO-style online reinforcement learning provides an effective framework; however, existing methods typically treat reference-policy divergence as fixed, despite its direct impact on policy optimization. We study this overlooked factor within a unified f-divergence framework, encompassing forward KL, reverse KL, and JS divergence, for GRPO-style autoregressive T2I alignment. Our systematic theoretical analysis reveals that different divergences reshape token-level updates in distinct ways. In particular, under the sampled-token shaping form used, JS regularization achieves a favorable trade-off by mitigating uniform bias relative to the reference policy while still discouraging large deviations. Extensive experiments on LlamaGen and Janus-7B show that JS divergence achieves the strongest or highly competitive optimization performance on most evaluation metrics while maintaining favorable generation diversity. The code is available at .

</details>


### [148] [ELDiff: When Evidential Learning Meets Text-to-Image Diffusion](https://arxiv.org/abs/2606.20924)
...
TL;DR: 本文提出一种基于强化学习的放射学报告生成框架，通过引入临床奖励和组相对训练策略，实现对报告临床精确率与召回率的可控权衡，在MIMIC-CXR数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有放射学报告生成方法主要优化自然语言生成指标（如流畅性），缺乏对临床重要因素（如精确率和召回率）的控制，生成的报告虽流畅但难以满足不同临床需求。

Method: 提出强化学习框架，包含一个控制参数在推理时显式调节临床精确率与召回率的权衡；引入临床奖励（clinical reward）提升临床有效性；采用组相对训练策略（group-relative training）对组内奖励进行归一化，降低奖励方差并提升训练稳定性。

Result: 在MIMIC-CXR数据集上，该方法在自然语言生成指标和临床有效性指标上均一致优于现有最先进方法，并能可靠地控制临床精确率与召回率的权衡。

Conclusion: 通过强化学习与临床奖励结合，可以在保持语言流畅性的同时，灵活控制放射学报告生成的临床精确率与召回率，满足不同临床场景的需求。

Classification reason: 该论文使用强化学习框架和临床奖励（clinical reward）来优化放射学报告生成，属于生成模型中对齐与奖励建模的范畴，与 [CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Focuses on clinical radiology report generation with RL reward control, which is outside the user's core interests in continuous multimodal models and video/image generation. Signals: Radiology report generation, Clinical reward learning, Precision-recall control, MIMIC-CXR dataset, Not in priority subtopics
...
### [150] [Causal Variational Deep Embedding: A Family of Interventional Generators for Confounded Images](https://arxiv.org/abs/2606.21806)
...
TL;DR: 本文提出CauVaDE（因果变分深度嵌入），基于规范增广SCM将未观测混杂因子坍缩为离散潜聚类变量，构建混合VAE生成器族，通过熵正则化参数γ遍历可行干预分布区域，在图像数据上生成多样化的因果干预样本并改善FID。


<details>
  <summary>Details</summary>
Motivation: 深度生成模型会继承训练数据中的虚假关联（如混杂偏差），导致用户无法在采样时独立控制属性。现有因果生成方法依赖过强的结构假设来唯一确定干预分布，但在图像领域这些假设通常不成立。本文旨在构建一个能生成多样化干预分布族的方法，而不强加不合理的结构假设。

Method: 提出CauVaDE，基于规范增广SCM（因果图），将未观测混杂因子无损地坍缩为有界离散潜聚类变量，连续变化吸收为独立噪声。证明该规范类在观测和干预Wasserstein距离下稠密。实例化为混合VAE，聚类变量作为规范混杂因子，通过熵正则化参数γ控制聚类后验，生成一族候选因果效应，拟合观测数据的同时遍历可行干预区域。

Result: 在图像数据基准上，CauVaDE能产生多样化的干预样本，相比未混杂参考基线改善了FID指标，验证了该方法能有效生成覆盖可行区域的因果干预分布。

Conclusion: CauVaDE提供了一种无需强结构假设即可生成多样化因果干预分布的方法，通过规范增广SCM和混合VAE框架，在图像生成中实现了可控的因果干预采样，为生成模型的因果可控性提供了新思路。

Classification reason: 该论文提出CauVaDE，通过因果变分自编码器解决生成模型中的混杂偏差问题，生成可控的干预分布样本。这属于生成模型的可控性与因果对齐，映射到[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment），该主题涵盖生成模型的可控性、guidance与对齐。

Importance reason: Causal generative modeling for confounded images is not aligned with user's priority on continuous language/multimodal models or video generation. Signals: Causal generative model, confounded images, mixture VAE, entropy regularization, FID improvement
...
### [151] [TailorMind: Towards Preference-Aligned Multimodal Content Generation](https://arxiv.org/abs/2606.23643)
...
TL;DR: 本文提出马尔可夫链从人类反馈（MCHF）方法，利用成对偏好直接定义模型输出上的转移核，通过马尔可夫链迭代实现生成模型对齐。理论证明MCHF几何快速收敛到平稳分布，并与NLHF、RLHF在理论上统一。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF将成对偏好简化为标量奖励，丢失了偏好的非传递结构；NLHF通过KL正则化极小极大优化保留成对效用但计算复杂。本文希望提出一种更简洁、直接利用成对偏好且具有理论保证的对齐方法。

Method: 定义成对效用函数U(x,y)和参考分布μ_ref，构造马尔可夫核P(x,dy)∝exp(U(x,y))μ_ref(dy)，以μ_ref为初始分布迭代运行马尔可夫链作为对齐过程。通过半范数||U||_⊕刻画偏好的非传递结构，并证明几何收敛速率。进一步通过扰动分析统一RLHF、NLHF和MCHF三种对齐范式。

Result: 理论证明MCHF几何快速收敛到平稳分布，收敛速率由偏好非传递结构半范数控制。NLHF的镜像下降算法具有类似的结构自适应收敛保证。当非传递性较小时，MCHF和NLHF在RLHF解附近一阶一致，且第一步均恢复基于列和奖励的RLHF解。

Conclusion: MCHF提供了一种简洁、理论上优雅的生成模型对齐方法，统一了基于奖励、博弈论和马尔可夫链三种对齐视角，为偏好对齐提供了新的理论基础和算法框架。

Classification reason: 该论文提出MCHF方法，从成对人类偏好中对生成模型进行对齐，属于生成模型对齐与偏好优化范畴，与[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Strong theoretical contribution to alignment methods but lacks direct connection to user's priority subtopics like continuous models or video generation. Signals: Markov chain alignment, preference-based generation, theoretical convergence, RLHF unification, non-transitive utility
...
### [153] [PG-MAP: Joint MAP Optimization for Inference-Time Alignment of Diffusion and Flow-Matching Models](https://arxiv.org/abs/2606.22958)
...
TL;DR: PG-MAP 是一种无需训练的推理时对齐框架，将文本到图像模型的对齐形式化为轨迹级别的 Gibbs-MAP/近端能量优化，联合优化条件 c 和潜状态 z_t，兼容扩散模型和 flow-matching 模型，在 SD1.5、SDXL 和 SD3.5-medium 上显著提升 PickScore 和 Aesthetic 等对齐指标。


<details>
  <summary>Details</summary>
Motivation: 现有推理时对齐方法通常只沿单一控制轴（如 CFG、注意力编辑或基于奖励的潜扰动）进行，无法建模条件与潜变量之间的联合依赖关系，且难以跨生成传输（扩散 vs. flow-matching）迁移。作者希望提出一种统一的、无需训练的框架，能够联合优化条件和潜变量，同时兼容扩散和 flow-matching 模型。

Method: PG-MAP 将推理时对齐形式化为轨迹级别的 Gibbs-MAP / 近端能量优化问题，通过前向一致性耦合（forward-consistency coupling）联合优化条件 c 和潜状态 z_t，并可选择性地由冻结的偏好奖励模型引导。针对扩散模型和 flow-matching 模型分别设计了传输特定的适配版本。对于 flow-matching 模型（如 SD3.5-medium），该方法退化为仅潜变量变体。

Result: 在扩散模型（SD1.5、SDXL）上，PG-MAP 持续提升 PickScore 和 Aesthetic 等对齐指标，并能与调优后的 CFG 有效结合达到最强性能。在 flow-matching 模型（SD3.5-medium）上，潜变量变体实现了 91.9% PickScore 和 75.7% HPS 胜率，消融实验排除了噪声相关伪影。人工评估进一步确认了相对于强基线（包括调优 CFG 和计算匹配的通用引导）的持续偏好。Oracle 路由分析表明条件和潜优化的相对重要性取决于提示类型。

Conclusion: PG-MAP 提供了一种统一的、无需训练的推理时对齐框架，通过联合优化条件和潜变量，在扩散和 flow-matching 模型上均能有效提升文本-图像对齐质量。不同提示类型对条件和潜优化的依赖程度不同，提示了按提示类型选择优化策略的进一步提升空间。

Classification reason: 该论文提出 PG-MAP，一种无需训练的推理时对齐框架，通过联合优化条件与潜变量来提升扩散模型和 flow-matching 模型的文本-图像对齐质量（如 PickScore、Aesthetic），属于生成模型对齐与奖励优化方向，因此映射到 canonical subtopic "生成模型对齐与奖励" (generation_alignment)。

Importance reason: Proposes a novel training-free inference-time alignment method for diffusion and flow-matching models, but lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: Inference-time alignment, Diffusion and flow-matching, Training-free, Joint optimization, Text-to-image alignment
...
### [154] [FedOT: Ownership Verification and Leakage Tracing via Watermarks for Federated LDMs](https://arxiv.org/abs/2606.22875)
...
TL;DR: VESFlow 是一种无需训练的安全生成方法，针对 flow matching 文生图模型在极少采样步数下通过编辑速度场来移除不安全概念，同时保持良性提示的生成质量。


<details>
  <summary>Details</summary>
Motivation: Flow matching 文生图模型在少步采样下难以应用传统的迭代轨迹修正或 CLIP 嵌入干预等安全方法，亟需一种适配少步 flow matching 的安全生成方案。

Method: 利用 flow matching 模型学习边际速度场的特性，直接通过安全条件后验编辑速度场（VESFlow），引导轨迹朝向安全输出；进一步引入风险评分过滤，对良性提示跳过编辑以降低计算开销；VESFlow+ 在编辑速度场朝向安全方向的同时，还将其推离不安全方向。

Result: 在 4 步 MeanFlow 模型上，VESFlow+ 将 Ring-A-Bell 的攻击成功率降至 6.3%，MMA-Diffusion 降至 6.8%，同时保持良性提示的生成保真度。

Conclusion: VESFlow 系列方法有效解决了 flow matching 少步采样下的安全生成问题，为新一代文生图模型的安全对齐提供了可行的训练-free 方案。

Classification reason: 该论文提出针对 flow matching 文生图模型的安全生成方法（VESFlow），通过编辑速度场移除不安全概念，属于生成模型的安全对齐与可控性方向，与 [CANONICAL] subtopic "生成模型对齐与奖励" 高度匹配。

Importance reason: Addresses safety in few-step flow matching generation, but the safety alignment subtopic is not a user priority. Signals: flow matching safety, training-free velocity editing, few-step generation, concept removal, benign prompt preservation
...
### [156] [DiT-Reward: Generative Representations for Text-to-Image Reward Modeling](https://arxiv.org/abs/2606.23626)
...
TL;DR: 本文提出DiT-Reward，将预训练的文本到图像扩散Transformer（DiT）转换为奖励模型，通过处理近干净的图像潜变量并聚合跨层的文本条件图像表征，在偏好预测基准上超越HPSv3，并可用于优化扩散模型生成质量。


<details>
  <summary>Details</summary>
Motivation: 图像生成模型的快速发展使得自动评估生成图像质量成为关键需求。现有奖励模型通常需要从头训练或依赖特定架构，本文探索能否利用生成模型本身学到的表征来支持生成图像的评估，从而更高效地构建奖励模型。

Method: DiT-Reward基于预训练的文本到图像DiT，在去噪过程的近干净潜变量阶段提取图像表征，跨Transformer层聚合文本条件图像特征，通过轻量级学习头进行偏好预测。生成骨干网络可冻结或微调，支持直接潜变量评分以加速推理。

Result: 在相同训练数据混合下，DiT-Reward在HPDv2上达85.6%、HPDv3上达77.6%，全面超越HPSv3。冻结骨干网络时轻量头仍能提取有效偏好预测。中间到后期层的表征最强，跨阶段组合表征有增益。模型容量越大性能越好。用于优化Stable Diffusion 3.5 Large时，DiT-Reward优于HPSv3，尤其在真实感方面。直接潜变量评分比HPSv3快1.65倍。

Conclusion: 预训练的生成式DiT提供了可迁移的表征用于奖励建模和策略优化，表明生成模型学到的表征可以自然地支持生成质量评估，为构建更高效的奖励模型提供了新思路。

Classification reason: 该论文将预训练的文本到图像扩散Transformer（DiT）转换为奖励模型，用于评估生成图像的质量，属于生成模型的对齐与奖励建模方向。这与[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Directly leverages diffusion transformer representations for reward modeling, aligning with user's focus on generative model alignment and efficient inference. Signals: Diffusion Transformer reward model, generative representation transfer, preference prediction improvement, inference speedup 1.65x, Stable Diffusion 3.5 optimization
...
### [157] [Semantic Browsing: Controllable Diversity for Image Generation](https://arxiv.org/abs/2606.23679)
*Sara Dorfman, Maya Vishnevsky, Omer Dahary, Or Patashnik, Daniel Cohen-Or*

Main category: cs.LG, cs.CV, cs.GR, cs.AI
...
TL;DR: 本文提出Semantic Browsing方法，通过利用VLM在文本层面而非像素层面引入结构化变化，实现对图像生成多样性的语义级控制，使用户能够在有意义的、可解释的变体轴上进行导航式创意探索。


<details>
  <summary>Details</summary>
Motivation: 现有文生图模型虽然视觉保真度和提示遵循性优秀，但生成样本多样性不足，且现有多样性方法产生的变化是随机的、无意义的，而非用户可理解的设计选择。作者希望实现一种受控的多样性，让用户能在结构化的图像画廊中沿语义轴进行系统化浏览。

Method: 利用现代文生图模型训练于详细描述文本这一事实，将语义决策与像素生成解耦。使用VLM在完整场景上下文中操作，通过agentic workflow显式地根据原始提示生成结构化的文本变体（如改变物体颜色、材质、风格、布局等），然后将这些文本变体输入文生图模型生成对应图像，形成可导航的设计空间。

Result: 实验表明该方法能生成多样且可导航的设计空间，每个变体对应一个具体的、用户可理解的语义决策。与基线方法相比，在保持视觉质量的同时实现了更有意义的多样性，用户可以通过语义轴（如颜色、材质、风格）系统性地探索生成空间。

Conclusion: 本文证明了通过在文本层面而非像素层面引入结构化变化，可以实现对图像生成多样性的语义级控制。这种范式转变使得用户能够进行有意义的创意探索，为可控图像生成提供了新的方向。

Classification reason: 该论文研究文本到图像生成中的可控多样性（controlled diversity），通过VLM和agentic workflow在文本层面诱导结构化变化，实现用户可导航的语义浏览。这属于生成模型的可控性与对齐范畴，与[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）中的guidance与可控性高度吻合。

Importance reason: Controllable diversity via VLM-driven text-level variation is interesting but not aligned with your priority subtopics on continuous models, video generation, or world models. Signals: Controllable image generation, VLM-based diversity, Semantic browsing, Not autoregressive or video, No continuous token or flow matching
...
### [158] [Improving Text-to-Music Generation with Human Preference Rewards](https://arxiv.org/abs/2606.21670)
*Yonghyun Kim, Junwon Lee, Haiwen Xia, Yinghao Ma, Chris Donahue*

Main category: cs.LG, cs.AI, cs.SD
...
TL;DR: 本文提出在文生音乐任务中引入人类偏好奖励（TuneJury）作为训练时条件信号和样本选择标准，结合五项工程决策（奖励条件化、架构搜索、专家迭代、偏好微调、推理后处理），在120M参数的FluxAudio-S骨干上显著提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有文生音乐模型主要依赖FAD-CLAP和CLAP score等自动指标，但这些指标与人类听觉偏好存在差距。作者希望通过引入学习到的人类偏好奖励来弥合这一差距，提升生成音乐的主观质量。

Method: 基于120M参数的FluxAudio-S骨干，提出五项工程决策：(i) 训练时奖励条件化，同时作为推理时CFG轴；(ii) 搜索五种分数条件化架构，训练和推理使用不同变体；(iii) 对前10%样本进行专家迭代；(iv) 使用CRPO进行短偏好微调以对齐音频-文本；(v) 推理后处理包括联合CFG、源分离和响度归一化。

Result: 在100个Song Describer提示上的逐阶段消融实验表明：训练时奖励条件化是有效的条件轴，专家迭代是主要贡献者，偏好微调仅带来噪声级增益，推理时分数标量在链末端已饱和。

Conclusion: 人类偏好奖励可以有效提升文生音乐质量，其中专家迭代和奖励条件化是最关键的改进手段，而偏好微调贡献有限。该工作为ATTM Grand Challenge效率赛道提供了有效的基线方案。

Classification reason: 该论文使用人类偏好奖励（TuneJury）对文生音乐模型进行偏好优化和训练时条件控制，属于生成模型对齐与奖励建模方向，与 canonical subtopic "生成模型对齐与奖励" 高度匹配。

Importance reason: Focuses on text-to-music generation with human preference rewards, which is outside the user's priority subtopics and negative keywords. Signals: human preference reward, text-to-music generation, reward conditioning, expert iteration, preference tuning
...
### [159] [Variance-Tilted Diffusion Models for Diverse Sampling](https://arxiv.org/abs/2606.22239)
...
TL;DR: 本文提出方差倾斜扩散模型（Variance-Tilted Diffusion Models），通过 Doob h-变换推导出带有排斥项和曲率项的交互粒子采样器，使得扩散模型采样出的样本集合具有更大的经验方差，从而实现多样化采样。


<details>
  <summary>Details</summary>
Motivation: 扩散模型通常独立采样，但许多下游任务（如分子设计、药物发现）需要获得多样化的候选集合。现有方法多采用启发式的排斥漂移，缺乏明确的概率目标。本文旨在为多样化采样提供一种有理论保证的、基于显式概率目标的交互粒子采样方法。

Method: 定义方差加权批分布（variance-weighted batch distribution），该分布偏好经过指定线性特征映射后经验方差较大的样本集合。将该目标作为 Doob h-变换的势函数，推导出独立扩散动力学的修正项。修正项包含两部分：一个排斥项（repel posterior denoised means）和一个曲率项（move particles to region of higher feature variance），从而得到具有透明概率目标的交互粒子采样器。

Result: 在合成数据和实际任务上的实验表明，该方法能够有效提升采样集合的多样性，同时保持样本质量。与启发式排斥方法相比，本文方法具有明确的概率解释和更稳定的性能。

Conclusion: 本文为扩散模型的多样化采样提供了一种理论严谨的框架，通过 Doob h-变换将方差加权目标转化为可计算的交互粒子动力学。该方法在保持样本质量的同时显著提升多样性，为需要多样化候选集的下游应用提供了有效工具。

Classification reason: 该论文提出方差倾斜扩散模型（Variance-Tilted Diffusion Models），通过 Doob h-变换引入交互项实现多样化采样，属于扩散模型的可控生成与采样优化，与 [CANONICAL] 生成模型对齐与奖励（generation_alignment）中的 guidance 与可控性方向高度相关。

Importance reason: Proposes a principled method for diverse sampling in diffusion models, but is not directly aligned with the user's priority subtopics. Signals: Diffusion model sampling, Diverse generation, Interacting particle sampler, Doob h-transform, Not in priority subtopics
...
### [160] [BanglaFake: Constructing and Evaluating a Specialized Bengali Deepfake Audio Dataset](https://arxiv.org/abs/2505.10885)
*Istiaq Ahmed Fahad, Kamruzzaman Asif, Sifat Sikder*

Main category: eess.AS, cs.MM, cs.AI, cs.SD

Direction: 多模态生成

Subtopic: 生成模型评测与数据集

Importance: 普通相关 (42.74) - 未入选精读

TL;DR: 本文构建了BanglaFake——一个孟加拉语深度伪造音频数据集，包含12,260条真实语音和13,260条由SOTA TTS模型生成的合成语音。通过30名母语者的MOS评分（自然度3.40，可懂度4.01）和MFCC的t-SNE可视化进行评测，为低资源语言的深度伪造检测提供了重要资源。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语等低资源语言的深度伪造音频检测面临数据集匮乏和声学特征细微的挑战，现有研究主要集中于英语等资源丰富的语言。

Method: 使用SOTA文本转语音（TTS）模型生成高质量合成语音，构建包含12,260条真实和13,260条伪造音频的数据集；通过30名母语者进行MOS评分（自然度和可懂度），并利用MFCC的t-SNE可视化分析真假语音的区分难度。

Result: MOS评分显示合成语音的自然度为3.40，可懂度为4.01；t-SNE可视化揭示了真实与伪造语音在声学特征上的区分挑战。

Conclusion: BanglaFake数据集为孟加拉语深度伪造音频检测研究提供了关键资源，有助于推动低资源语言在音频安全领域的研究进展。

Classification reason: 该论文构建了一个孟加拉语深度伪造音频数据集（BanglaFake），包含真实与合成语音，并通过MOS评分和t-SNE可视化进行评测。这属于生成模型（音频生成）的评测与数据集构建，与[CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark）高度匹配。

Importance reason: Focuses on Bengali deepfake audio dataset, far from user's priority on continuous multimodal models and video generation. Signals: Low-resource language dataset, Deepfake audio detection, TTS evaluation, Not in priority subtopics, No boost keywords matched
...
### [161] [GEOPHYS: The Geometry of Physical Plausibility](https://arxiv.org/abs/2606.20707)
...
TL;DR: 本文提出 GEOPHYS，一种利用冻结图像编码器每帧嵌入的五种几何属性来检测视频物理不合理性的方法。该方法在 LikePhys 和 IntPhys2 上分别达到 98.3% 和 93.3% 的检测准确率，远超 V-JEPA 2、GPT-4o 等模型。作为视频生成的 best-of-N 验证器，GEOPHYS 将 MAGI-1 24B 在 PhysicsIQ 上的准确率从 50.01% 提升至 64.50%，同时计算和内存成本远低于 V-JEPA 2。


<details>
  <summary>Details</summary>
Motivation: 人类能在毫秒级识别物理不合理事件，但现有机器学习方法要么依赖外部多模态大语言模型作为评判器（速度慢、成本高），要么需要对训练过程进行特殊修改。作者希望找到一种高效、无需额外训练的方法来评估视频的物理合理性。

Method: GEOPHYS 利用冻结图像编码器（如 DINOv2）提取视频每帧的嵌入，然后计算五种几何属性：帧间距离、帧间角度、帧间距离方差、帧间角度方差、以及嵌入空间中的曲率。这些几何属性聚合后作为物理合理性的指示信号。作者通过分析这些信号与人类 EEG 响应的相关性验证其有效性，并在物理不合理视频检测和视频生成验证两个任务上评估性能。

Result: 在 LikePhys 上达到 98.3%，在 IntPhys2 上达到 93.3% 的物理违规检测准确率，而 V-JEPA 2、GPT-4o、Gemini 及 12 个现代视频扩散模型接近随机水平。作为 best-of-N 验证器，GEOPHYS 将 MAGI-1 24B 在 PhysicsIQ 上的准确率从 50.01% 提升至 64.50%，且 wall-clock 时间降低 1.5 倍，内存降低 4.65 倍。

Conclusion: 视频的物理合理性可以通过利用图像编码器提取的时间特征的几何属性来高效评估，无需额外训练或大型模型。GEOPHYS 提供了一种轻量级、可解释且与模型无关的物理合理性评估方法，对视频生成和世界模型的质量控制具有重要价值。

Classification reason: 该论文提出 GEOPHYS 方法，利用图像编码器嵌入的几何属性来检测视频中的物理不合理性，并作为视频生成模型的物理合理性验证器（best-of-N verifier）。这属于生成模型评测与对齐范畴，与 [CANONICAL] 的 generation_benchmark（生成模型评测与数据集）高度匹配，因为论文核心是提出一种新的物理合理性评估信号和评测方法。同时，该方法也可作为视频世界模型的物理一致性验证工具，因此也匹配 world_model 方向。

Importance reason: Novel geometric method for video physical plausibility detection, but only indirectly supports priority subtopics like video generation and world models. Signals: physics violation detection, video generation verifier, efficient inference, world model evaluation, geometric embedding properties
...
### [162] [One Image is All You Need: Agentic One-Shot Image Generation via Text-Based World Models for Long-Tail Spatial Perception](https://arxiv.org/abs/2606.20764)
...
TL;DR: 提出WMGen-v1框架，利用单张参考图像通过LVLM构建结构化场景表示，LLM进行物理合理的场景扩展，扩散模型生成长尾空间感知训练数据，在自动驾驶和海事监控等任务上验证了合成数据可接近真实数据性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界时空数据存在严重的长尾分布，安全关键场景数据稀缺导致检测模型性能下降。现有生成方法（扩散模型、GAN）缺乏显式空间约束，生成的场景存在空间和物理不一致性。

Method: WMGen-v1采用智能体式文本世界模型框架：(1) LVLM从单张参考图像构建结构化场景表示；(2) LLM在物理合理性和常识约束下进行引导式场景扩展；(3) 扩散模型基于结构化语义表示生成多样且物理一致的长尾训练数据。

Result: 在内部工业数据集、ROADWork和LaRS基准上，WMGen-v1优于基线方法。仅使用WMGen-v1合成数据训练的检测器在聚合数据集级指标上接近真实数据性能。

Conclusion: WMGen-v1通过文本世界模型实现了从单张图像到多样化长尾数据的生成，有效缓解了长尾数据稀缺问题，为空间感知任务提供了一种可扩展的合成数据生成方案。

Classification reason: 该论文提出WMGen-v1框架，利用单张参考图像通过LVLM+LLM构建结构化场景表示，再通过扩散模型生成长尾空间感知训练数据。核心贡献是面向长尾场景的合成数据生成方法，属于生成模型的数据集/数据增强范畴，因此映射到[multimodal_generation]方向下的[generation_benchmark]（生成模型评测与数据集）子主题。同时该方法涉及LVLM/LLM进行场景理解与推理，也匹配[multimodal_understanding_generation]方向。

Importance reason: Novel agentic world model for synthetic data generation but focuses on long-tail spatial perception, not core continuous multimodal or autoregressive generation. Signals: world model for data generation, long-tail spatial perception, LVLM+LLM+diffusion pipeline, not autoregressive or continuous token, negative keywords absent
...
### [163] [Object-Centric Dataset Resources for Constrained-Data Image Generation and Augmentation](https://arxiv.org/abs/2606.21113)
...
TL;DR: 本文发布了三个面向目标中心（object-centric）的低数据量图像生成与增强数据集资源：Cityscapes-Pedestrian（行人密集场景）、TrafficSigns（交通标志）和 COCO PottedPlant（盆栽植物），提供标准化256×256裁剪、边界框标注、清单脚本及重建文档，支持在有限标注样本条件下进行目标中心图像生成和数据增强的评测。


<details>
  <summary>Details</summary>
Motivation: 现有图像数据集通常面向分类、检测或场景理解，缺乏专门针对目标中心（object-centric）图像生成与增强的标准化数据集资源，尤其是在类别样本有限（constrained-data）的场景下（如智能城市行人分析、交通标志检测、特定领域目标检测），难以公平比较不同生成与增强方法。

Method: 从 Cityscapes、TrafficSigns 和 COCO 三个来源中提取目标中心裁剪（256×256），标准化边界框标注，构建三个子集：Cityscapes-Pedestrian（含隐私模糊和遮挡的行人场景）、TrafficSigns（高对比度交通标志）、COCO PottedPlant（上下文多样的盆栽场景）。提供直接数据、清单脚本、标注表、校验和及重建文档。

Result: 发布了 3,009 个 TrafficSigns 样本、2,156 个 Cityscapes-Pedestrian 清单记录和 7,679 个 COCO PottedPlant 清单记录，支持标签和划分检查、子集创建、从上游数据重建以及目标中心图像生成或合成数据增强方法的评测。

Conclusion: 该数据集资源填补了目标中心低数据量图像生成领域标准化评测数据的空白，为行人分析、交通标志检测和特定领域目标检测等场景下的生成与增强方法提供了可复现的共享评测基准。

Classification reason: 该论文发布了三个面向目标中心图像生成的数据集资源（Cityscapes-Pedestrian, TrafficSigns, COCO PottedPlant），属于生成模型评测与数据集范畴，与 [CANONICAL] subtopic "生成模型评测与数据集" 高度匹配。

Importance reason: This is a dataset resource paper for object-centric generation, but it lacks novelty in methods or models and does not align with the user's priority subtopics on continuous models, autoregressive generation, or world models. Signals: dataset resource, object-centric generation, low-data setting, no new model or method, weak alignment with priority subtopics

Abstract: Object-centric image generation is important in settings with few labeled examples, including pedestrian analysis in smart-city scenes, traffic-sign inspection, and domain-specific object detection. Synthetic images are most useful for training and evaluation when datasets preserve object structure, bounding boxes, visual diversity, and realistic context. Existing image datasets usually target classification, detection, or scene understanding rather than controlled object-centric generation and augmentation with limited class-specific data. We present a shareable collection of three object-centric dataset resources: Cityscapes-Pedestrian, TrafficSigns, and COCO PottedPlant. The collection standardizes 256-by-256 object-centric crops and bounding-box annotations across three regimes: dense pedestrian scenes with privacy blur and occlusion, cleaner high-contrast traffic signs, and context-diverse potted-plant scenes. The release contains 3,009 TrafficSigns samples, 2,156 Cityscapes-Pedestrian manifest records, and 7,679 COCO PottedPlant manifest records. The larger COCO-derived manifest preserves contextual and multi-instance diversity, while equal-size subsets can be drawn with a fixed random seed for controlled comparisons. The release provides direct TrafficSigns data where redistribution is permitted, together with scripts, manifests, box-level annotation tables, checksums, and reconstruction documentation for the Cityscapes- and COCO-derived subsets. It is available through the Latzi/object-centric-low-data-datasets GitHub repository and Zenodo DOI . The collection supports label and split inspection, subset creation, reconstruction from upstream data, and evaluation of object-centric image generation or synthetic-data augmentation methods on shared records.

</details>


### [164] [Koshur Pixel: a large-scale synthetic ocr dataset for kashmiri](https://arxiv.org/abs/2606.23144)
...
TL;DR: 本文提出了Koshur Pixel，首个大规模克什米尔语合成OCR数据集，包含613,078个图像-文本对，覆盖多种字体和文本粒度，采用25种以上数据增强策略模拟真实文档退化，为低资源语言的OCR系统训练提供了基础资源。


<details>
  <summary>Details</summary>
Motivation: 克什米尔语（Kashmiri）是一种低资源语言，使用复杂的Perso-Arabic Nastaliq文字，存在上下文字形变形、密集连字和正字法变体等挑战，缺乏标注训练数据严重制约了OCR技术的发展。

Method: 基于KS-PRET-5M语料库，使用SynthOCR-Gen框架生成613,078个图像-文本对，涵盖单词到整页文档多种文本粒度，并应用25种以上数据增强策略模拟真实文档退化（如噪声、模糊、倾斜等）。

Result: 构建了首个大规模克什米尔语合成OCR数据集，提供了可扩展且成本低廉的替代人工标注方案。

Conclusion: Koshur Pixel为克什米尔语OCR系统训练、文本遗产数字化以及低资源语言技术发展奠定了重要基础。

Classification reason: 该论文构建了首个大规模克什米尔语合成OCR数据集（Koshur Pixel），属于多模态生成方向中的数据集构建工作，与[CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark）高度匹配。

Importance reason: Low-resource OCR dataset for Kashmiri is far from the user's priority subtopics and negative keywords. Signals: low-resource language, synthetic OCR dataset, no autoregressive or video generation, no continuous language model, no world model
...
### [165] [Unmasking LAION-5B: Age, Gender, Race, and Emotion Biases in Large-Scale Image Datasets](https://arxiv.org/abs/2606.23204)
...
TL;DR: 本文对LAION-5B数据集（LAION-2B-en和LAION-2B-multi）进行了全面的人口统计偏差分析，使用FairFace、DeepFace和Emo-AffectNet模型检测人脸并分析年龄、性别、种族和情绪偏差。研究发现年轻成年人（20-39岁）、白人和男性被显著过度代表，而少数种族群体和中老年女性被持续低估，同时存在人口统计属性与情绪之间的刻板关联（如"愤怒"主要关联男性，"快乐"关联女性）。这些偏差模式在两个数据集组件和两个人口统计模型中一致，表明偏差深深嵌入这一广泛使用的训练数据集中。


<details>
  <summary>Details</summary>
Motivation: 大规模图像-文本数据集（如LAION-5B）是现代AI系统的基础，但其庞大规模和未经筛选的特性引发了关于人口统计和刻板印象偏差的严重担忧。现有研究缺乏对LAION-5B中年龄、性别、种族和情绪偏差的系统性、大规模分析，特别是交叉性偏差（intersectional biases）的研究。

Method: 使用三个最先进的面部分析模型（FairFace、DeepFace和Emo-AffectNet）检测LAION-2B-en和LAION-2B-multi中的人脸，提取年龄、性别、种族和情绪属性。通过统计分析和可视化，比较各人口统计组在数据集中的分布与真实世界人口分布，分析代表性偏差、刻板印象偏差和交叉性偏差。

Result: 年轻成年人（20-39岁）、白人和男性被显著过度代表；少数种族群体和中老年女性被持续低估；存在刻板情绪关联（"愤怒"→男性，"快乐"→女性）；这些偏差在两个数据集组件和两个人口统计模型中一致；交叉性分析揭示了更细微的偏差模式。

Conclusion: LAION-5B中存在深刻且系统性的年龄、性别、种族和情绪偏差，这些偏差可能通过训练数据影响下游生成模型的输出和行为。研究强调了在大规模数据集构建和AI模型训练中需要更加关注数据多样性和公平性。

Classification reason: 该论文对LAION-5B数据集进行了全面的人口统计偏差分析（年龄、性别、种族、情绪），属于对大规模训练数据集的系统性评测与分析。这与[多模态生成]方向下的[生成模型评测与数据集]（generation_benchmark）高度吻合，因为该子主题涵盖数据集分析与评测基准。同时该数据集广泛用于多模态生成模型训练，也与[多模态理解与生成]方向相关。

Importance reason: Dataset bias analysis is important but not aligned with user's focus on model architectures and generation methods. Signals: LAION-5B bias analysis, demographic imbalance, stereotypical associations, large-scale dataset auditing, generative model training data
...
### [166] [GeoFidelity-Bench: Evaluating Segment-Level Geographic Fidelity in Text-to-Image Street-View Generation](https://arxiv.org/abs/2606.23669)
...
TL;DR: GeoFidelity-Bench 是一个评估文生图模型在街景生成中路段级地理忠实度的基准，包含 7,117 张图像覆盖 25 个城市 109 个路段。实验发现添加街道和社区名称可将 top-1 检索准确率提升 5.5 个百分点，但目标路段与同城最近路段的相似度差距接近零，表明模型擅长生成城市级或社区级合理的街景，而非精确的路段级忠实生成。


<details>
  <summary>Details</summary>
Motivation: 现有文生图模型能生成视觉上合理的城市街景，但无法判断这些输出是否真正对应了用户指定的具体路段，还是仅仅利用了城市先验知识。缺乏一个系统性的基准来评估路段级的地理忠实度。

Method: 构建 GeoFidelity-Bench 基准，包含 7,117 张 Mapillary 图像，覆盖 25 个城市 109 个 OpenStreetMap 路段。设计参考面板排序任务：对每个生成面板，将目标参考面板与同城最近路段、同城其他路段、其他城市路段的面板进行排序，以局部区分能力作为主要测试指标。评估 6 个开源文生图模型在仅城市、城市+街道+社区、GPS 增强三种提示条件下的表现。

Result: 添加街道和社区名称后，top-1 检索准确率比仅城市提示提升 5.5 个百分点（95% CI: 3.4-7.7）。但目标路段与同城最近路段的相似度差距接近零。使用错误街道/社区名称的提示仍能获得部分增益。附加 GPS 坐标作为普通文本没有统计上显著的额外收益。真实图像查询能成功恢复路段身份。

Conclusion: GeoFidelity-Bench 揭示了当前文生图模型在街景生成中存在城市/社区级合理性与精确路段级忠实性之间的持续差距。模型擅长生成"看起来像该城市"的街景，但无法精确复现特定路段的独特视觉特征。

Classification reason: 该论文提出了 GeoFidelity-Bench，一个用于评估文生图模型在街景生成中地理忠实度的新基准，属于生成模型评测与数据集方向，因此映射到 [CANONICAL] subtopic "生成模型评测与数据集" (generation_benchmark)。

Importance reason: Focuses on geographic fidelity evaluation for street-view generation, which is a niche subtopic not aligned with user's priority on continuous multimodal models or video generation. Signals: Geographic fidelity benchmark, street-view generation, text-to-image evaluation, limited personal relevance, no direct connection to continuous language or video generation
...
### [167] [The Watermark Shortcut: How Provenance Marking Sabotages Audio Deepfake Detection](https://arxiv.org/abs/2606.23335)
...
TL;DR: 本文发现音频深度伪造检测中，当合成语音被加水印而真实语音没有时，检测器会学习"水印=>伪造"的虚假捷径，导致泛化下降、水印可被剥离绕过、真实语音被误判为伪造三重问题。实验表明该问题可通过在两类数据上同时加水印来修复，并发布了WASP数据集。


<details>
  <summary>Details</summary>
Motivation: 水印技术被广泛用于标记合成语音以增强可追溯性，但本文发现水印的存在反而会破坏深度伪造检测器的性能，因为检测器会利用水印作为虚假特征。作者旨在系统性地揭示这一未被充分认识的安全隐患。

Method: 通过白盒实验（在受控条件下训练带水印检测器）和黑盒实验（测试商业API），系统分析了水印导致的三种耦合失效模式：泛化退化、剥离绕过、标记误判。提出通过在真实和合成语音上同时加水印来去相关化，从而消除捷径。

Result: 白盒实验中，水印训练导致标记误判的等错误率从16%升至75%。黑盒实验中，对真实语音加水印可使其被商业API误判为伪造。通过在两类数据上同时加水印重新训练，检测器恢复正常性能。

Conclusion: 水印作为虚假特征会严重破坏音频深度伪造检测器的可靠性，但该问题可以通过在训练时对正负样本同时加水印来缓解。研究呼吁在部署水印与检测系统时需谨慎考虑这种交互效应。

Classification reason: 该论文研究音频深度伪造检测中水印导致的虚假关联问题，属于生成模型（音频合成）的安全性与评测范畴。虽然涉及音频生成，但核心贡献是揭示水印作为伪影对检测器的影响，并发布WASP数据集，因此映射到[CANONICAL]的generation_benchmark（生成模型评测与数据集）最为合适。

Importance reason: Focuses on audio deepfake detection watermark shortcut, not aligned with user's priority on continuous multimodal models, video/image generation, or world models. Signals: Audio deepfake detection, Watermark shortcut, Generalization degradation, Strip-to-evade, Mark-to-frame
...
Main category: cs.CL, cs.CV, cs.AI, cs.MM
...
TL;DR: 提出 Gazer，一种无需训练的自回归视觉模型语义纠错框架，通过多模态大模型反馈在生成过程中诊断并修正语义错误，提升组合图像与视频生成的语义对齐质量。


<details>
  <summary>Details</summary>
Motivation: 自回归视觉模型（AVM）在按尺度预测的生成过程中，不同粒度的离散尺度使得语义错误难以识别和修正，现有训练无关方法忽略了中间生成状态，导致语义错误累积到最终输出中。

Method: Gazer 框架包含两个协作阶段：反思诊断阶段（Reflective Diagnosis）从中间生成状态诊断语义错误；语义修正阶段（Semantic Correction）回退并修正生成轨迹，使其重新对齐目标提示。通过将多模态大模型反馈集成到 AVM 采样循环中实现无需训练的语义纠错。

Result: 在组合图像和视频基准测试上，Gazer 在多个 AVM 上提升了语义对齐和组合准确性，且无需额外训练。

Conclusion: Gazer 证明了利用多模态大模型反馈在生成过程中进行语义纠错的有效性，为提升自回归视觉模型生成质量提供了一种轻量级、无需训练的解决方案。

Classification reason: 论文研究自回归视觉模型（AVM）的语义纠错，属于自回归视觉生成范式，与 canonical 子主题「自回归视觉生成」完全匹配。

Importance reason: Directly addresses autoregressive visual model semantic errors with a training-free correction method, highly relevant to your priority on autoregressive image/video generation and efficient inference. Signals: autoregressive visual models, training-free correction, multimodal large language model feedback, compositional image and video generation, semantic alignment improvement
...
### [169] [EmoInstruct-TTS: Dual-Path Instruction-Guided Emotional Speech Synthesis](https://arxiv.org/abs/2606.20650)
*Minghui Wu, Ganjun Liu, Zikun Fang, Ting Meng, Hongchuan Wu, Bingao Xu, Yonglong Cai, Jiasheng Chen, Jun Du*

Main category: cs.CL, eess.AS, cs.AI, cs.SD
...
TL;DR: EmoInstruct-TTS 提出了一种双路径指令引导的情感语音合成框架，通过 Emotion2embed 覆盖48种情感状态（含细粒度类别与强度），并设计 ICE-Flow 从自由文本指令生成声学情感嵌入，集成到 LLM 合成管线中实现显式情感控制。


<details>
  <summary>Details</summary>
Motivation: 现有指令可控语音合成方法通常依赖粗粒度的情感标签，缺乏对细粒度情感类别和强度的显式建模，难以从自然语言指令中精确控制情感表达。

Method: 提出 Emotion2embed 监督式语义-声学情感嵌入，覆盖48种情感状态（含细粒度类别与强度）；设计 Instruction-Conditioned Emotion Flow Model (ICE-Flow) 从自由文本指令生成声学情感嵌入；将生成的嵌入集成到 LLM 合成管线中，在保持语义规划的同时实现显式情感控制。

Result: 实验表明，EmoInstruct-TTS 在情感可控性和语音自然度上均优于强基线方法。

Conclusion: 通过双路径指令引导框架和细粒度情感嵌入，实现了更精确、更自然的情感语音合成，验证了从自由文本指令到声学情感表征的可行路径。

Classification reason: 该论文研究情感语音合成（Emotional TTS），属于音频生成方向，与现有 canonical subtopic "音频与多模态联合生成"（audio_av_generation）高度匹配。

Importance reason: Focuses on emotional speech synthesis, which is not a priority subtopic and lacks strong overlap with user's key boost keywords. Signals: Emotional speech synthesis, instruction-guided control, fine-grained emotion embedding, dual-path framework, LLM-based synthesis pipeline
...
### [170] [UnityShots: Memory-Driven Multi-Shot Audio-Video Generation with Boundary-Aware Gating](https://arxiv.org/abs/2606.21661)
...
Importance: 高优先级 (87.22) - 未入选精读

TL;DR: UnityShots 提出一种基于记忆驱动的多镜头音视频生成系统，通过长短期记忆槽（LTM/STM）和边界条件门控机制保持跨镜头的主体外观、场景和说话人身份一致性，并引入离散切镜类型先验作为推理时控制。在 I2V、T2V、R2V 三种模式下均优于开源基线，在跨镜头一致性指标上媲美最强闭源系统。


<details>
  <summary>Details</summary>
Motivation: 现有多镜头视频生成方法要么受限于固定长度序列无法扩展，要么依赖线性增长的记忆库，或使用 LLM 编排预训练生成器但缺乏多镜头感知的骨干网络。需要一种能高效保持跨镜头一致性的音视频生成方法。

Method: 基于 LTX-2.3 骨干网络，视频流维护两个固定大小的记忆槽：长期记忆（LTM）锚定开场镜头，短期记忆（STM）保存前一镜头的尾部，通过边界条件门控融合视觉切镜概率和节拍检测信号来更新。音频流在每个镜头注入参考说话人 token 以保持音色。离散切镜类型先验通过 AdaLN 学习，作为推理时控制过渡强度的旋钮。

Result: 在 I2V、T2V、R2V 三种条件下，UnityShots 在所有跨镜头一致性指标上领先开源基线，在多镜头轴上与最强闭源系统持平。同时发布了包含 200 个多文化多镜头序列的基准数据集，覆盖 6 个种族区域和 10 种以上语言。

Conclusion: UnityShots 通过固定大小的记忆槽和边界条件门控机制，有效解决了多镜头音视频生成中的跨镜头一致性问题，无需线性增长的记忆库，且提供了推理时可控制的切镜强度。

Classification reason: 该论文提出多镜头音视频生成系统，涉及音频（说话人音色保持）与视频（跨镜头一致性）的联合生成，属于[CANONICAL] subtopic "音频与多模态联合生成"（audio_av_generation）的范畴。

Importance reason: Strongly aligns with user's priority on multimodal generation and continuous representation, with a novel memory-driven multi-shot audio-video generation system. Signals: multimodal generation, memory-driven generation, audio-video generation, cross-shot coherence, boundary-aware gating
...
### [171] [Bagpiper-TTS: Natural Language Guided Universal Speech Synthesis](https://arxiv.org/abs/2606.22811)
...
TL;DR: Bagpiper-TTS 是一个通用语音合成系统，通过自然语言提示推理用户意图，生成包含转录和元数据的丰富文本描述（caption），再据此合成目标语音。系统支持多说话人、意图到语音、角色扮演合成、歌声合成等多种任务，在 Seed-TTS-Eval 上达到 1.7% WER，性能与专用模型相当。


<details>
  <summary>Details</summary>
Motivation: 传统 TTS 系统依赖固定的输入格式和预定义元数据槽位，无法灵活满足用户多样化的自然语言需求。作者希望构建一个能理解自然语言提示、自动推理用户意图并生成高质量语音的通用系统。

Method: Bagpiper-TTS 采用两阶段流水线：1) 自然语言提示首先通过一个推理模块（基于 LLM）分析用户意图，生成一个丰富的文本描述（caption），包含转录文本和细粒度元数据（如说话人身份、情感、语速、风格等）；2) 该 caption 作为引导条件输入语音合成模块，生成目标语音。系统统一了多种 TTS 相关任务（多说话人、意图到语音、角色扮演、歌声合成等）在一个框架下。

Result: 在 Seed-TTS-Eval 基准上达到 1.7% 的 WER；在 LLM-as-a-judge 和人类主观评测中，多个应用场景下性能与专用模型相当。

Conclusion: Bagpiper-TTS 证明了通过自然语言提示驱动的通用语音合成框架可以统一多种 TTS 任务，在保持高质量的同时提供极大的灵活性，为下一代 TTS 系统提供了新范式。

Classification reason: 该论文提出 Bagpiper-TTS，是一个基于自然语言提示的通用语音合成系统，属于 TTS/语音生成方向，映射到 [CANONICAL] 音频与多模态联合生成 (audio_av_generation) 子主题。

Importance reason: Novel universal TTS with natural language guidance, but outside user's core priority subtopics and negative keywords apply. Signals: TTS, natural language guidance, universal speech synthesis, multi-task, Seed-TTS-Eval
...
### [172] [Synthesizing the Lombard Effect: Multi-Level Control of Speech Clarity and Vocal Effort in TTS](https://arxiv.org/abs/2606.23176)
...
TL;DR: 本文提出一种基于flow-matching的TTS模型，通过发声努力和发音伪标签实现连续且解耦的语音清晰度与发声努力控制，并支持词级强调，成功模拟了人类在嘈杂环境中的Lombard效应，提升了合成语音的可懂度。


<details>
  <summary>Details</summary>
Motivation: 人类在嘈杂环境或与听障人士交流时会自然地提高音量和清晰度（Lombard效应），而现有TTS系统缺乏对这种自适应语音清晰度控制的模拟能力，因此需要一种能够连续、解耦地控制发声努力和发音清晰度的语音合成方法。

Method: 采用flow-matching框架训练TTS模型，使用发声努力（vocal effort）和发音（articulation）伪标签作为条件控制信号，实现连续且解耦的语音清晰度调节；同时引入词级强调机制，允许对特定词语进行清晰度增强。

Result: 实验表明，该模型能有效改善与清晰度相关的声学特征；在噪声环境下的语音可懂度测试中，成功复现了人类清晰语音在嘈杂条件下的可懂度提升效果。

Conclusion: 基于flow-matching的TTS模型能够有效模拟Lombard效应，实现多级、解耦的语音清晰度与发声努力控制，为自适应语音合成和助听场景提供了可行方案。

Classification reason: 该论文提出基于flow-matching的TTS模型，实现Lombard效应（嘈杂环境下语音清晰度与发声努力的多级控制），属于语音生成/TTS方向，映射到[CANONICAL] subtopic "音频与多模态联合生成"。

Importance reason: Flow-matching TTS with disentangled control is solid but outside core priority subtopics and lacks boost keywords. Signals: flow-matching TTS, Lombard effect, vocal effort control, articulation control, speech intelligibility
...
### [173] [LambdaMark: Semantic Audio Watermarking for Robustness and Radioactivity](https://arxiv.org/abs/2606.21365)
...
TL;DR: LambdaMark 是一种基于语义音频潜空间的多比特水印方案，通过将水印信息嵌入到音频的语义表示中，实现鲁棒性和放射性（radioactivity）——即水印能够从微调后的下游生成模型输出中被检测到。实验表明该方法在常见失真和对抗移除攻击下均具有近乎完美的鲁棒性，且是唯一在所有评估的移除攻击下仍保持鲁棒的水印方案。


<details>
  <summary>Details</summary>
Motivation: 生成式音频技术的进步使得语音克隆变得极其容易，导致语音欺诈、冒充等未经授权的使用。现有音频水印方法通常将信号嵌入到波形或频谱图等低级表示中，容易受到信号级操作的影响，且难以迁移到下游模型。需要一种更鲁棒且具有放射性（即水印能被下游微调模型继承）的水印方案。

Method: LambdaMark 包含轻量级水印编码器和解码器。编码器将多比特水印信息注入到音频的语义潜空间表示中，解码器检测水印存在并恢复嵌入的比特信息。使用自定义多组件损失函数训练，以保持水印音频的保真度、提高比特级恢复率，并增强对常见失真和对抗移除攻击的鲁棒性。

Result: LambdaMark 在常见失真下实现了近乎完美的鲁棒性，是唯一在所有评估的移除攻击下仍保持鲁棒的水印方案。此外，LambdaMark 表现出通用且鲁棒的放射性，即使在微调模型的生成输出中，对失真和对抗移除攻击仍然保持鲁棒。

Conclusion: LambdaMark 是首个通用的放射性水印方案，通过将水印嵌入语义音频潜空间而非低级表示，实现了更强的鲁棒性和放射性，为防御语音生成模型滥用提供了有效手段。

Classification reason: 该论文提出语义级音频水印方法LambdaMark，属于音频安全与生成模型保护方向。虽然论文主要关注水印而非生成，但其核心应用场景是防御语音生成模型的滥用（voice cloning），且水印嵌入在语义音频潜空间中，与音频生成模型紧密相关。映射到[CANONICAL] subtopic "音频与多模态联合生成"（audio_av_generation），该方向涵盖音频生成及相关技术，水印作为生成模型安全防御机制可归入此范畴。

Importance reason: Novel semantic audio watermarking with strong robustness, but outside user's core priority subtopics. Signals: Semantic audio watermarking, Robustness and radioactivity, Multi-bit watermark in latent space, Downstream model finetuning defense, First generic radioactive watermarking scheme
...
### [174] [MIRAGE: Stealthy Visual Prompt Injection for Vulnerability Detection in Web Agents](https://arxiv.org/abs/2606.20717)
*Xuelong Dai, Jianyu Ma, Boyang Ma, Biwei Yan, Yijun Yang, Yue Zhang*

Main category: cs.CR, cs.CV, cs.AI
...
TL;DR: MIRAGE提出了一种针对MLLM-based web agent的视觉间接提示注入框架，利用扩散模型生成感知上无害的对抗性图像，在受限的广告位等空间内实现隐蔽的下一动作劫持攻击。


<details>
  <summary>Details</summary>
Motivation: 现有的针对MLLM web agent的对抗评估常依赖宽松的威胁模型和视觉上明显的攻击痕迹，缺乏在真实受限场景（如第三方广告位）下隐蔽攻击的研究。作者希望探索在可信平台中仅作为无特权第三方时，能否通过视觉提示注入实现有效的漏洞检测。

Method: 提出MIRAGE框架，结合扩散模型生成感知上良性的对抗图像，并引入曲率感知对抗扩散引导（curvature-aware adversarial diffusion guidance）与稀疏暗像素残差扰动（sparse dark-pixel residual perturbations）的鲁棒优化技术，在攻击者可控的受限空间内实现隐蔽攻击。

Result: 在SeeAct和OpenClaw两个主流MLLM web agent框架上的实验表明，MIRAGE在受限设置下实现了高效、逼真且隐蔽的下一动作劫持攻击。

Conclusion: MIRAGE揭示了MLLM-based web agent在视觉层面存在新的安全漏洞，即使攻击者仅控制一个语义合法、空间受限的区域（如广告位），也能通过视觉间接提示注入实现隐蔽攻击，为web agent的安全性研究提供了新的视角。

Classification reason: 该论文研究针对MLLM-based web agent的视觉间接提示注入攻击（视觉对抗攻击），属于多模态安全与攻防范畴，与[CANONICAL] subtopic "多模态安全与攻防"（multimodal_safety_attack）高度匹配。

Importance reason: Focuses on adversarial security for web agents, which is outside the user's priority subtopics and negative keywords. Signals: multimodal understanding generation, adversarial attack, web agent security, diffusion model, visual prompt injection
...
### [175] [PROTON: Prototype-Based Test-Time Online OOD Detection for Medical VLMs](https://arxiv.org/abs/2606.20913)
...
TL;DR: PROTON提出一种轻量级后处理模块，通过在线维护原型库并自适应融合原型距离与MCM评分，在无需修改模型或训练数据的情况下，显著提升医学VLM在协变量偏移、语义偏移和远域OOD上的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学VLM的OOD检测方法在不同偏移类型上表现不均衡，例如MCM在远域OOD上表现良好但在协变量偏移上近乎随机。作者发现协变量偏移样本在softmax空间与ID样本不可区分，但在VLM嵌入空间中占据不同区域，这一未被利用的信号启发了PROTON的设计。

Method: PROTON是一个轻量级后处理模块，包含：(1) 在线原型库，从高置信度测试预测中维护原型；(2) 自适应融合机制，通过流级方差统计将原型距离与MCM评分动态融合，无需模型修改、训练数据或提示工程。

Result: 在FLAIR+FIVES眼科基准上，PROTON在协变量偏移上提升MCM +23.9 AUROC，语义偏移+8.8，远域OOD+8.1，成为首个无需分层提示或标注数据即可同时提升三类偏移的零样本方法。

Conclusion: PROTON有效利用了VLM嵌入空间中未被挖掘的结构信息，通过在线原型库和自适应融合实现了对多种偏移类型的鲁棒OOD检测，为医学VLM的安全部署提供了实用方案。

Classification reason: 该论文关注医学VLM部署时的OOD检测问题，属于多模态模型的安全性与可靠性范畴。虽然OOD检测不完全等同于攻防，但它是多模态安全（检测异常/分布外输入）的核心子问题，且现有[多模态安全与攻防]分类涵盖防御与安全评测，因此映射至此。

Importance reason: Focuses on medical VLM OOD detection, which is outside the user's priority subtopics and contains negative keywords. Signals: Medical VLM, OOD detection, Prototype-based, Covariate shift, Negative keywords: medical diagnosis

Abstract: Medical vision-language models (VLMs) enable zero-shot clinical image classification, yet reliably detecting out-of-distribution (OOD) inputs at deployment remains an open problem. No static scoring method works across all shift types: Maximum Concept Matching (MCM) on FLAIR achieves 76.4% AUROC for far-OOD but only 42.4% for covariate shifts such as ultra-wide-field fundus images, effectively random. We trace this to a structural mismatch: covariate-shifted inputs are indistinguishable from in-distribution samples in softmax space, yet occupy distinct regions in the VLM embedding space. To exploit this untapped signal, we propose PROTON (PROtotype-based Test-time ONline OOD detection), a lightweight post-hoc module that maintains an online prototype bank from high-confidence test predictions and adaptively fuses prototype distance with MCM scoring via stream-level variance statistics, requiring no model modification, training data, or prompt engineering. On the ophthalmology benchmark FLAIR + FIVES, PROTON improves MCM by +23.9 AUROC on covariate shift, +8.8 on semantic shift, and +8.1 on far-OOD, making it the only zero-shot method to improve all three without hierarchical prompts or labeled data. Code is available at , and the project page is available at .

</details>


### [176] [Extraction and Analysis of Multimodal Concepts in Vision Language Models through Sparse Autoencoders](https://arxiv.org/abs/2606.21197)
...
TL;DR: 本文研究了LLM在对抗性prefill攻击下能否可靠地自我识别其输出被攻击的问题。实验表明，10个开源指令微调LLM均无法可靠识别被攻击的输出，平均有27.3%的概率声称被prefill的输出是自主意图。内省信号主要来自安全与拒绝相关推理，且对探测方式敏感。LoRA微调（SFT、GRPO、DPO）虽能扩大意图探测差距，但无法迁移到篡改探测，且多数模型上反而提高了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 已有研究表明LLM在良性任务上具备内省能力，但在安全场景下，模型能否可靠识别自身输出是否受到对抗性prefill攻击尚不清楚。理解这一能力对于构建可信赖的AI系统至关重要。

Method: 在10个开源指令微调LLM（3B-70B）和4个安全基准上，系统评估模型识别自身被对抗性prefill攻击输出的能力。通过权重正交化分析拒绝方向的作用，设计不同探测方式（意图探测vs篡改探测），并测试三种LoRA微调方法（SFT、GRPO、DPO）对8个模型（3B-27B）的影响。

Result: 无模型能可靠识别被攻击输出，平均27.3%声称prefill输出为自主意图。内省信号主要来自安全/拒绝推理。权重正交化消除拒绝方向后，prefill与自然输出的声称率差距几乎消失。探测方式显著影响结果。LoRA微调在所有8B-27B模型上扩大了意图探测差距，但无法迁移到篡改探测，且多数模型上攻击成功率反而上升。

Conclusion: LLM在安全场景下的自我报告可靠性存在严重风险。内省信号机制复杂，依赖于安全推理和探测方式，且现有微调干预效果有限甚至适得其反。这提示需要更谨慎地对待LLM的自我报告，并开发更可靠的防御机制。

Classification reason: 该论文研究LLM在对抗性prefill攻击下自我报告可靠性的问题，属于多模态安全与攻防（multimodal_safety_attack）范畴下的对抗攻击与安全评测方向。虽然论文主要关注文本LLM，但其研究的安全攻击范式（adversarial prefill）与多模态安全攻防中的越狱、对抗攻击主题高度一致，且该[CANONICAL] subtopic涵盖多模态安全场景，是现有最匹配的分类。

Importance reason: The paper's focus on LLM safety introspection is outside the user's core priority subtopics and negative keywords apply. Signals: LLM introspection, adversarial prefill, safety alignment, LoRA fine-tuning, refusal direction
...
### [178] [NegAS: Negative Label Guided Attention and Scoring for Out-of-Distribution Object Detection with Vision-Language Models](https://arxiv.org/abs/2606.22537)
...
TL;DR: 本文提出NegAS框架，利用LLM生成的负标签引导注意力机制和基于sigmoid的OOD评分函数，首次将VLM目标检测器（如YOLO-World、Grounding DINO）应用于OOD检测任务，显著降低了FPR95。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测主要针对单模态检测器或VLM分类器，VLM目标检测器在OOD场景下的潜力尚未被探索。VLM检测器存在两个挑战：文本引导注意力仅增强ID标签前景而忽略潜在OOD背景区域；sigmoid多标签输出与softmax OOD评分不兼容。

Method: 提出NegAS框架，包含两个模块：(1) NegA（负标签引导注意力模块），利用LLM生成视觉相似但语义不同的负标签，引导注意力关注潜在OOD背景区域；(2) NegS（sigmoid OOD评分函数），同时利用ID标签和负标签，对ID实例产生强响应、对OOD实例产生抑制响应。

Result: 在COCO数据集上FPR95降低11.4%，在OpenImages数据集上降低25.5%，同时保持ID检测精度。该方法可泛化到YOLO-World和Grounding DINO等多种VLM检测器。

Conclusion: NegAS有效解决了VLM目标检测器在OOD场景下的两大挑战，显著提升了OOD检测性能，且具有良好的泛化性，为VLM检测器的安全部署提供了新思路。

Classification reason: 该论文研究基于VLM的OOD目标检测，属于多模态安全与鲁棒性范畴，与[多模态安全与攻防]（multimodal_safety_attack）方向高度相关，该方向涵盖对抗攻击、防御、安全评测等主题，OOD检测是安全防御的重要组成部分。

Importance reason: Focuses on OOD detection for VLM detectors, which is outside the user's priority subtopics and lacks direct connection to continuous models, generation, or world models. Signals: OOD detection, VLM object detector, negative label guidance, sigmoid scoring, safety-critical application
...
### [179] [SingGuard: A Policy-Adaptive Multimodal LLM Guardrail with Dynamic Reasoning](https://arxiv.org/abs/2606.22873)
*SingGuard Team*
...
TL;DR: T-VSS 是一种轻量级的测试时对抗防御方法，通过在视觉特征空间中构建样本特定的低秩子空间并学习共享特征校正，直接修复被攻击的视觉表征，从而提升 VLM 的对抗鲁棒性，同时保持干净准确率和计算效率。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLM）在零样本识别上表现优异，但对对抗扰动高度脆弱。现有测试时自适应方法（如 prompt 优化或输入空间优化）通过间接且昂贵的路径改进预测，未能直接修复被破坏的视觉表征本身。因此需要一种直接在视觉特征空间中进行高效测试时自适应的方法。

Method: T-VSS 首先从被攻击图像的多视角特征残差中构建样本特定的低秩子空间，然后在该子空间内通过可靠性加权熵最小化学习共享的特征校正。通过将自适应约束在紧凑的视觉几何结构中，T-VSS 将被攻击特征引导至更稳定和更具判别性的预测方向，避免全空间噪声更新。

Result: 在细粒度分类、ImageNet 和 ImageNet-OOD 基准上，T-VSS 显著提升了对抗鲁棒性，同时保持了有竞争力的干净准确率，并且比之前的测试时自适应方法更高效。

Conclusion: T-VSS 通过在视觉特征空间中直接进行低秩子空间引导的测试时自适应，为 VLM 对抗防御提供了一种轻量、高效且有效的解决方案，验证了在紧凑视觉几何中约束自适应优于间接的全空间优化路径。

Classification reason: 该论文研究视觉语言模型（VLM）的对抗鲁棒性，提出测试时视觉子空间引导（T-VSS）方法，在视觉特征空间中直接进行对抗防御。这属于多模态安全与攻防（对抗攻击与防御）范畴，因此映射到 [CANONICAL] subtopic "多模态安全与攻防"。

Importance reason: Focuses on adversarial defense for VLM, not aligned with user's priority on generative models and continuous representations. Signals: Test-time adaptation, adversarial robustness, vision-language model, visual subspace steering, low-rank correction
...
### [181] [Finetuning with Scientific Data Increases Hallucinations: A Multi-domain Factuality Evaluation of LLMs](https://arxiv.org/abs/2606.21359)
...
TL;DR: 本文提出SciFactCheck基准，包含2500个提示词覆盖5个科学领域，评估18个LLM的科学事实性幻觉。研究发现：科学微调模型在所有幻觉类型和领域上事实可靠性均下降，且模型内部置信度降低但语言表达更自信。人类实验表明现有事实核查工具与专家判断一致性有限。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多用于科学交流，但其幻觉倾向在高风险科学场景中构成重大风险。现有幻觉评估局限于生物医学领域、将幻觉视为二元任务，且未研究科学微调LLM家族。本文旨在填补这些空白。

Method: 构建SciFactCheck基准（2500个提示，5个科学领域），设计模块化评估框架针对三种事实性幻觉类型（不可验证性、过度声称、归因）。采用最小配对设计，比较18个科学微调模型与其通用基座模型。

Result: 1）科学微调模型在所有幻觉类型和科学领域上事实可靠性均下降；2）微调模型内部置信度更低但语言表达更自信；3）现有事实核查工具与专家判断一致性有限，科学可核查声明的定义在人类标注者间也存在争议。

Conclusion: 研究结果从根本上挑战了当前领域特定微调对事实性的改善效果，呼吁开发改进的科学内容验证基础设施。

Classification reason: 该论文研究LLM在科学领域的幻觉问题（事实性、可靠性），属于多模态理解与生成方向下的"多模态幻觉与可靠性"（multimodal_hallucination）子主题。虽然论文主要关注纯文本LLM而非多模态，但该子主题涵盖幻觉、事实性、可靠性等核心概念，且该子主题是[CANONICAL]条目，优先复用。

Importance reason: Challenges domain-specific fine-tuning for factuality, but focuses on scientific text LLMs, not the user's priority multimodal generation or world models. Signals: Scientific fine-tuning degrades factuality, multi-domain hallucination benchmark, human fact-checking tools limited, not aligned with user's continuous multimodal or video generation focus
...
### [182] [MedHal-Loc: Are "Explainable-by-Architecture" Medical Hallucination Detectors Faithful Localizers? A Localization Benchmark](https://arxiv.org/abs/2606.21517)
...
TL;DR: MedHal-Loc 是一个用于评估临床文本幻觉检测器定位忠实性的基准。研究发现，基于知识图谱三元组的管线虽然检测性能尚可，但定位能力仅比随机好3.3个百分点（不显著），瓶颈在于实体抽取覆盖率仅约59%。而NLI、一致性检测和专用跨度检测器FAVA的定位效果显著更好。检测能力不等于定位忠实性，架构的可解释性需要验证而非假设。


<details>
  <summary>Details</summary>
Motivation: 现有临床文本幻觉检测系统声称具有可解释性（能定位错误跨度），但这一能力通常被假设而非实际测量。作者希望系统性地评估不同范式检测器的定位忠实性，揭示检测与定位之间的差距。

Method: 构建 MedHal-Loc 基准，包含受控子集（300条PubMedQA衍生语句，注入4类可定位错误）和自然子集（真实幻觉）。评估四种范式：NLI-per-clause、consistency-per-sentence、KG三元组管线、专用跨度检测器FAVA，使用新提出的定位忠实性指标。

Result: KG三元组管线定位仅比随机好3.3pp（不显著），实体抽取覆盖率约59%；NLI-per-clause、consistency-per-sentence和FAVA均显著优于随机。自然子集中真实幻觉以结论翻转为主，难以定位跨度。

Conclusion: 检测能力不能保证定位忠实性；架构层面的可解释性（如KG三元组分解）必须经过实证验证，不能仅凭设计假设。未来需要更关注定位忠实性的评测与改进。

Classification reason: 该论文研究临床文本中的幻觉检测与定位忠实性，属于多模态/文本幻觉与可靠性范畴，与 [CANONICAL] 多模态幻觉与可靠性 子主题高度匹配。

Importance reason: Focuses on medical hallucination localization, which is outside the user's priority subtopics and negative-keyword domain. Signals: Medical hallucination detection, localization faithfulness benchmark, KG-triple pipeline underperforms, NLI/consistency/FAVA better, entity extraction bottleneck
...
### [183] [Pre-Generation Hallucination Detection in Large Language Models via Soft-Target Attention Probing](https://arxiv.org/abs/2606.21917)
...
TL;DR: 本文提出一种生成前幻觉检测方法，通过软目标监督（基于随机采样输出的经验错误率）和注意力探针（attention probing）来估计模型对每个提示的出错概率，从而在解码前识别幻觉风险。实验表明该方法在短答案任务上优于线性探针，软目标监督持续提升检测质量。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法通常需要在生成完整输出后才能判断，成本高昂。本文希望在生成之前就能检测到幻觉风险，从而支持提前弃权、检索增强或路由决策，避免不必要的解码开销。

Method: 将生成前幻觉检测形式化为风险估计问题，提出软目标监督：基于模型随机采样输出的经验答案错误率作为无偏最小方差估计器。同时将注意力探针（attention probing）适配到生成前设置，使检测器能够选择性聚合与幻觉相关的提示表征。

Result: 在三个问答基准和五个模型上，注意力探针在短答案任务上优于线性探针；用软目标监督替代二元标签进一步持续提升了检测质量。

Conclusion: 生成前幻觉检测是可行的，软目标监督和注意力探针的组合能有效估计模型的逐提示错误概率，为提前干预幻觉提供了实用方案。

Classification reason: 该论文研究LLM生成前的幻觉检测问题，属于多模态幻觉与可靠性（multimodal_hallucination）这一[CANONICAL]子主题的范畴。虽然论文主要针对文本LLM，但幻觉检测方法可直接迁移至多模态场景，且该子主题涵盖幻觉缓解与可靠性提升的通用方法。

Importance reason: Focuses on hallucination detection in LLMs, which is not a priority subtopic and lacks direct connection to user's key interests in continuous models, video/image generation, or world models. Signals: hallucination detection, attention probing, soft-target supervision, pre-generation risk estimation, not aligned with priority subtopics
...
### [184] [MotionHalluc: Diagnosing Kinematic Hallucinations in Fine-Grained Motion Reasoning](https://arxiv.org/abs/2606.23061)
...
TL;DR: 本文提出 MotionHalluc 基准，系统评估大多模态模型在跨视频比较中的运动幻觉（方向幻觉、属性幻觉、时间幻觉），并给出 PPV 基线方法，通过注入显式运动测量值平均提升 10.6% 性能。


<details>
  <summary>Details</summary>
Motivation: 现有大多模态模型在跨视频比较中生成运动指令时，常出现运动幻觉（motion hallucination），即生成的反馈无法反映真实运动差异。缺乏专门针对此类幻觉的系统性评测基准。

Method: 构建 MotionHalluc 基准（1540 个细粒度问题，553 个视频对），沿方向幻觉、属性幻觉、时间幻觉三个维度评测。提出 Perceive-Parse-Verify (PPV) 训练无关基线，将候选指令转为可执行测量查询，在推理时注入运动学测量值。

Result: 多个 SOTA 多模态模型在 MotionHalluc 上表现出高幻觉率；PPV 方法通过注入显式运动测量值，平均提升 10.6% 性能。

Conclusion: 运动推理中引入显式定量测量是减少跨视频比较幻觉的关键因素。

Classification reason: 该论文研究跨视频比较中运动指令生成的运动幻觉问题，属于多模态幻觉（hallucination）范畴，与 [CANONICAL] subtopic "多模态幻觉与可靠性" 高度匹配。

Importance reason: Focuses on diagnosing motion hallucinations in video comparison, which is relevant to multimodal understanding but not aligned with user's priority subtopics like continuous models or generation. Signals: motion hallucination benchmark, cross-video comparison, kinematic measurement injection, PPV baseline, multimodal hallucination diagnosis
...
### [185] [Faithful Grounded Visual Reasoning via Learned Proxy-Tokens](https://arxiv.org/abs/2606.23354)
...
TL;DR: 本文提出 Composer，一种通过可学习代理 token（proxy-tokens）实现忠实视觉定位的多模态大语言模型。代理 token 作为离散符号指针显式索引图像潜空间，替代传统文本坐标机制，弥合语义-空间鸿沟，在保持答案准确率的同时将视觉定位准确率提升 9.0 个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 在视觉问答中缺乏可解释性，Grounded Visual Reasoning (GVR) 方法虽引入文本坐标形式的视觉定位信息，但缺乏与视觉特征的可学习语义连接，导致模型产生与图像证据不对应的幻觉坐标（语义-空间鸿沟）。

Method: 提出 Composer 模型，核心创新是 learned proxy-tokens——一组可学习的离散符号指针，每个 token 显式索引图像潜空间中的特定区域，使模型能够将视觉区域作为可寻址、可语义操作的集合进行推理。同时构建 ComposerGCoT 数据集用于全面评估推理一致性与定位准确性。

Result: Composer 在最终答案准确率上与基于坐标的基线持平，同时视觉定位准确率提升 9.0 个百分点，证明离散代理 token 比传统文本坐标更有效地捕获空间语义。

Conclusion: 具有可学习语义连接的视觉定位机制是构建可信赖 MLLM 的有前景方向，代理 token 范式为忠实可解释的视觉推理提供了新思路。

Classification reason: 论文提出 Composer 模型，通过可学习的 proxy-token 机制提升视觉定位的忠实性与可解释性，减少幻觉（hallucination），属于多模态幻觉与可靠性方向。该论文的核心贡献是缓解视觉定位中的语义-空间鸿沟（hallucination），因此映射到 canonical 的 multimodal_hallucination 子主题。

Importance reason: Novel proxy-token mechanism improves visual grounding faithfulness, but the subtopic is not a priority and lacks direct connection to continuous multimodal or generation. Signals: visual grounding, proxy-tokens, MLLM interpretability, semantic-spatial gap, ComposerGCoT dataset
...
### [186] [MAGNIFIED: RL Fine-tuning of Multimodal Large Language Models for Motion Planning](https://arxiv.org/abs/2606.20641)
...
TL;DR: 本文提出 MAGNIFIED，一种基于强化学习微调（RLFT）的方法，用于对齐多模态大语言模型（MLLM）的自动驾驶规划目标。通过将预测 token 序列映射为车辆轨迹并从规划奖励中学习，MAGNIFIED 在 Waymo Open Motion Dataset 上相比 SFT 基线实现了超过 10.5% 的重叠率降低和 38.9% 的离路率降低。


<details>
  <summary>Details</summary>
Motivation: MLLM 在语义理解和常识推理方面表现出色，但传统的 next-token 文本预测预训练和 SFT 目标无法很好地满足自动驾驶规划需求——它仅鼓励逐 token 的文本模仿，忽略了多步后果以及与关键规划考量（如给其他道路参与者留出空间）的对齐。

Method: 提出 MAGNIFIED 方法，采用强化学习微调（RLFT）框架。将 MLLM 预测的 token 序列映射为对应的车辆轨迹，从规划奖励（如合规性、舒适性、效率）中学习，优化真实规划目标而非仅关注 token 预测精度。在 Waymo Open Motion Dataset 上使用栅格化鸟瞰图和 token 化轨迹作为输入，以规划导向的 X-Y 坐标序列作为输出。

Result: 在 Waymo Open Motion Dataset 上，RLFT 相比 SFT 基线实现了超过 10.5% 的重叠率降低和 38.9% 的离路率降低，显著提升了规划性能。

Conclusion: RLFT 能够有效对齐 MLLM 的自动驾驶规划目标，使其超越简单的模仿学习，生成更合规、舒适和高效的驾驶规划轨迹。

Classification reason: 该论文使用强化学习微调（RLFT）对齐多模态大语言模型（MLLM）以实现自动驾驶规划目标，属于多模态强化学习与对齐方向。同时涉及自动驾驶规划，与具身世界模型方向也有一定关联。

Importance reason: Applies RL fine-tuning to MLLMs for motion planning, but focuses on autonomous driving, which is not a priority subtopic. Signals: RL fine-tuning for MLLMs, token-level planning rewards, Waymo Open Motion Dataset, planning alignment beyond imitation, not in priority subtopics
...
### [187] [Translating Inference-Time Control to Radiology Vision-Language Models: Activation Steering for Pneumonia Classification on Chest X-rays](https://arxiv.org/abs/2606.20852)
...
TL;DR: 本文评估了推理时激活引导方法CAA在胸部X光片肺炎分类任务中对医学VLM的诊断性能提升效果。在三个冻结权重的VLM上测试发现，该方法能显著改变预测分数分布，但仅在NV-Reason-CXR-3B上观察到有意义的性能提升（F1从0.7692提升至0.8727），表明激活引导可作为轻量级医学VLM行为适配方法。


<details>
  <summary>Details</summary>
Motivation: 推理时工程方法可以在不微调的情况下改变模型行为，但其在医学VLM中提升诊断性能的效用尚不明确。本文旨在评估CAA方法能否在不更新模型权重的情况下改善胸部X光片VLM的肺炎分类性能。

Method: 使用CAA方法，构建三种引导向量：答案偏置控制、肺炎文本对比向量、图像条件对比向量。在三个冻结的胸部X光VLM（MedGemma-4B-IT、NV-Reason-CXR-3B、CheXOne-3B）上，基于二元提示下Yes/No token的logits进行分类。使用200张确定性开发集进行层和尺度选择（100张）及阈值校准（100张），在Kermany肺炎测试集上评估ROC-AUC、PR-AUC、F1等指标。

Result: 固定阈值F1改进常见但不一致。NV-Reason-CXR-3B效果最佳：校准F1从零样本的0.7692提升至肺炎文本引导的0.8619和图像条件引导的0.8727。CheXOne-3B的肺炎文本引导使校准F1从0.8528提升至0.8666，但置信区间跨越零。MedGemma-4B-IT未观察到一致改善。

Conclusion: CAA能显著改变预测分数分布和操作特性而无需微调。在三个VLM中有一个观察到有意义的性能提升，表明激活引导可作为轻量级医学VLM行为适配方法，但效果因模型而异。

Classification reason: 该论文研究推理时激活引导（Contrastive Activation Addition）来调整医学VLM的肺炎分类行为，属于无需微调的对齐/行为控制方法，与多模态对齐（multimodal_rl_alignment）方向高度相关。

Importance reason: Focuses on medical VLM steering for pneumonia, which is outside the user's priority subtopics and negative-keyword domains. Signals: medical VLM, activation steering, pneumonia classification, chest X-ray, inference-time control
...
### [188] [When Does a Video-Language Model Stop Watching? Reward Strength Controls the Formation and Reversal of Visual Shortcuts in Multimodal RLVR](https://arxiv.org/abs/2606.22043)
*Zekun Xu*
...
Importance: 值得关注 (66.4) - 未入选精读

TL;DR: 本文系统研究了多模态 RLVR 训练中视觉捷径（模型忽略视频输入而依赖语言先验）的形成与反转动力学。通过控制 grounding penalty 强度 λ，发现捷径在训练中突然出现、随 λ 增加单调抑制、且存在滞后不对称性——形成后更难去除。早期干预可阻止捷径形成，而后期干预效果显著下降。


<details>
  <summary>Details</summary>
Motivation: 多模态 RLVR 中，模型可能停止关注视觉输入而利用语言先验（视觉捷径），但该现象的形成机制、是否可逆以及何时干预有效尚不清楚。

Method: 将 grounding penalty 强度 λ 作为控制变量，在训练时间轴上刻画视觉捷径的形成-反转动力学。在留出的分布外诊断集上分析捷径的 onset、dose-response 和干预窗口。

Result: (i) 捷径在窄窗口内突然出现且跨随机种子鲁棒；(ii) λ 增加单调抑制捷径，中等 λ 下先形成后反转，呈现滞后不对称性；(iii) 在 onset 前施加惩罚可阻止捷径形成，而 consolidation 后相同惩罚效果显著下降。

Conclusion: 视觉捷径崩溃不是二元缺陷，而是一个可控、时间依赖且不对称的过程，对多模态 RLVR 中何时以及如何施加正则化有直接启示。

Classification reason: 该论文研究多模态 RLVR（带可验证奖励的强化学习）中视觉捷径的形成与反转动力学，属于多模态强化学习与对齐方向，因此映射到 canonical 子主题"多模态强化学习与对齐"。

Importance reason: Provides fundamental dynamics of visual shortcut formation in multimodal RLVR, directly relevant to understanding failure modes in continuous multimodal models and MLLM alignment. Signals: visual shortcut dynamics, multimodal RLVR, grounding penalty control, hysteresis in shortcut reversal, critical intervention window
...
TL;DR: READ 是首个将强化学习框架应用于音频描述（AD）生成的工作，通过序列级优化和多种奖励函数（参考匹配、长度、格式、连贯性奖励）显著提升了 AD 生成的准确性和连贯性，在多个基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有音频描述生成方法要么依赖提示现成的多模态模型（常与 AD 风格不匹配），要么部分优化基于 next-token prediction 的训练系统（未充分探索模型能力且偏向通用表达）。作者希望引入强化学习框架，通过序列级优化来提升 AD 生成的准确性和连贯性。

Method: READ 将 AD 生成建模为序列级优化问题，设计了四种奖励函数：参考匹配奖励（reference-matching）、长度奖励（length）、格式奖励（format）以及上下文感知的连贯性奖励（coherence reward），通过强化学习训练生成模型，使其输出更准确、连贯且符合 AD 风格。

Result: 在 MAD-Eval、CMD-AD 和 TV-AD 三个基准上，READ 在多种评测指标上显著优于先前方法，证明了 RL 范式在 AD 生成中的有效性。

Conclusion: 强化学习是音频描述生成的一种有前景的新范式，能够有效提升生成结果的准确性和叙事连贯性。代码、模型和基准结果将公开。

Classification reason: 该论文提出 READ，首个将强化学习（RL）框架应用于音频描述（AD）生成的训练方法，通过序列级优化和多种奖励函数（参考匹配、长度、格式、连贯性奖励）提升生成质量。这属于多模态生成任务中的 RL 对齐方法，因此映射到 [CANONICAL] 多模态强化学习与对齐（multimodal_rl_alignment）子主题。

Importance reason: Applies RL to audio description, but the topic is outside your priority subtopics and lacks boost keywords. Signals: First RL for AD generation, sequence-level optimization, coherence reward, multimodal understanding generation, no continuous language model or video generation focus
...
### [190] [EvoRubrics: Dynamic Rubrics as Rewards via Adversarial Co-Evolution for LLM Reinforcement Learning](https://arxiv.org/abs/2606.23038)
...
TL;DR: EvoRubrics 提出一种对抗性共同进化强化学习框架，其中策略 LLM 和评分标准生成器在每一步训练中通过对抗交互共同提升，使评分标准随策略进化而动态调整，避免固定奖励的饱和与欺骗问题，在多个基准上优于静态和动态评分标准基线。


<details>
  <summary>Details</summary>
Motivation: 基于评分标准的奖励为开放任务中的强化学习提供了可解释且细粒度的优化信号，但预构建的评分标准在训练过程中保持静态，随着模型能力提升，固定标准逐渐失去判别力，导致奖励饱和甚至被 hack。现有动态评分标准方法依赖外部前沿模型或标准答案，且更新粒度粗糙。

Method: 提出 EvoRubrics 框架，包含一个策略 LLM 和一个评分标准生成器（Rubric Generator），两者在每一步训练中通过对抗性交互共同进化：策略 LLM 在评分标准生成器的指导下改进，评分标准生成器则根据策略的进化动态调整标准以保持判别力和信息量，实现实时跟踪策略的评估，并自然形成自动课程学习。

Result: EvoRubrics 在多个基准上一致优于静态和动态评分标准基线。学习到的评分标准生成器可作为可迁移的奖励模型泛化到新任务。即使完全自监督的变体（无任何外部监督）也能取得有意义的提升，表明生成与评估之间的共同进化本身就能提供足够丰富的学习信号。

Conclusion: EvoRubrics 通过对抗性共同进化实现了评分标准与策略的动态适配，有效解决了固定奖励的饱和问题，且评分标准生成器具有良好的迁移能力。自监督变体的成功表明生成与评估的共同进化是一种强大的自学习范式。

Classification reason: 该论文提出 EvoRubrics，一种基于对抗性共同进化的 LLM 强化学习框架，通过动态评分标准作为奖励信号。虽然论文主要聚焦于 LLM 的 RL 训练，但其核心方法（评分标准作为奖励、对抗性共同进化、奖励模型迁移）与多模态强化学习与对齐方向高度相关，属于该方向在纯文本领域的延伸。该论文的方法论（动态奖励、对抗性共同进化）可自然迁移到多模态 RL 对齐场景。

Importance reason: Novel co-evolutionary RL framework for LLM alignment, but tangential to user's core focus on continuous multimodal models and video generation. Signals: adversarial co-evolution, dynamic rubric rewards, LLM reinforcement learning, self-supervised alignment, reward hacking mitigation
...
### [191] [Concept Alignment Contrast and Long-Short Prompt Memory for Test-Time Adaptation of SAM3 in Medical Image Segmentation](https://arxiv.org/abs/2606.22963)
...
TL;DR: 本文提出 CM-TTA 框架，通过概念对齐对比（CAC）指标和长短期提示记忆（LSPM）模块，实现 SAM3 在医学图像分割上的测试时自适应，无需标注即可提升分割性能。


<details>
  <summary>Details</summary>
Motivation: SAM3 在自然图像上表现优异，但在医学图像上因域差距性能下降。现有视觉-语言 TTA 方法主要依赖图像级不确定性最小化，无法反映区域级语义正确性，且缺乏持续单遍自适应中的稳定性机制。

Method: 提出 CM-TTA 框架，包含三个核心模块：(1) 概念对齐对比（CAC）指标，利用文本-视觉语义一致性评估预测质量，选择最佳增强视图作为伪监督；(2) 长短期提示记忆（LSPM），短期记忆动态融合近期提示实现敏捷局部自适应，长期记忆维护稳定全局提示生成增强伪标签；(3) 密集监督提示更新（DSPU），用增强伪标签作为密集监督优化提示嵌入。

Result: 在前列腺和皮肤病变分割任务上的大量实验表明，CM-TTA 显著优于现有的 SAM3 测试时自适应方法。

Conclusion: CM-TTA 通过概念对齐对比和长短期记忆机制，有效解决了医学图像分割中 SAM3 的域差距问题，为无标注测试时自适应提供了新思路。

Classification reason: 该论文提出测试时自适应（TTA）方法，通过概念对齐对比（CAC）和长短期提示记忆（LSPM）来对齐视觉-语言语义，属于多模态对齐范畴，因此映射到 [CANONICAL] 多模态强化学习与对齐（multimodal_rl_alignment）。

Importance reason: Focuses on medical image segmentation, which is a negative keyword, and lacks connection to priority subtopics like continuous models or video generation. Signals: Medical imaging, Test-time adaptation, SAM3, Concept alignment, Prompt memory
...
### [192] [Black-Box Continual Learning for Vision-Language Models](https://arxiv.org/abs/2606.22999)
...
TL;DR: 本文提出Black-CL基准和BETA方法，解决VLM在黑盒场景下的持续学习问题。BETA仅通过优化文本原型（0.05M参数），利用语义投影累积、潜分布回放和测试时原型自适应三个组件，在10个数据集上达到与白盒方法相当甚至更优的性能。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法大多依赖白盒范式（可访问模型权重和梯度），但实际部署中VLM常以云托管黑盒形式提供服务，无法获取梯度或修改架构。因此需要一种更真实、更实用的黑盒持续学习方案。

Method: 提出BETA方法，包含三个核心组件：1) 语义投影累积（SPA）——通过低秩投影增量学习新知识；2) 潜分布回放（LDR）——用高斯分布建模并回放旧任务的嵌入分布以缓解灾难性遗忘；3) 测试时原型自适应（TTPA）——在推理时动态调整分类边界。整个方法仅需访问模型输出嵌入/logits，无需梯度或架构修改。

Result: 在10个数据集和多种骨干网络上，BETA仅用0.05M可训练参数（比竞争方法少180-3000倍）即显著超越现有黑盒方法，性能与白盒持续学习方法相当甚至更优。

Conclusion: 黑盒持续学习是VLM实际部署中的重要问题，BETA通过纯文本原型优化有效解决了这一挑战，为持续学习从学术界向实际系统过渡提供了基础框架。

Classification reason: 该论文研究视觉语言模型（VLM）的持续学习（continual learning），属于多模态模型的对齐与适应问题。虽然持续学习不完全等同于RL/对齐，但该论文关注的是VLM在动态环境中的持续适应与遗忘缓解，与多模态对齐/适应方向高度相关。现有taxonomy中无专门的"持续学习"子主题，而"多模态强化学习与对齐"（multimodal_rl_alignment）是最接近的[CANONICAL]子主题，涵盖多模态模型的适应与优化。

Importance reason: Addresses continual learning for VLMs but is not aligned with user's priority on generative models and continuous representations. Signals: Black-box continual learning, VLM adaptation, parameter-efficient, task-agnostic inference, prototype optimization
...
### [193] [Attention-Spectrum Regularization for Replay-Free Continual Multimodal LLMs](https://arxiv.org/abs/2606.23063)
...
### [195] [CogniRoute: Learning to Route Social Evidence in Omni-Modal Models](https://arxiv.org/abs/2606.20970)
...
TL;DR: CogniRoute 提出了一种基于认知模式引导的混合专家框架，用于全模态社交视频问答。它通过训练阶段的认知模式分解跨模态关系、推理需求和时间范围，结合路由感知强化学习联合优化 token 生成和专家分配。在构建的 OmniSocialBench 基准上，CogniRoute 比最强闭源基线高出 15.33 个百分点，比最强开源全模态基线高出 26.77 个百分点。


<details>
  <summary>Details</summary>
Motivation: 全模态模型虽然能同时处理视频、音频和文本，但统一的多模态访问并不保证模型使用了正确的证据。在社交视频问答中，答案可能取决于手势、语调、时序线索或言语与视觉表达之间的不匹配，现有模型缺乏对这种复杂跨模态推理的结构化引导。

Method: CogniRoute 包含三个核心组件：(1) 训练阶段的认知模式（cognitive schema），将每个样本按跨模态关系、推理需求和时间范围进行分解；(2) 模式引导的混合专家（MoE）架构，将全局路由签名与认知模式结构对齐；(3) 路由感知强化学习，联合优化 token 生成和专家分配，奖励函数包含答案正确性、模态一致性推理和认知时序定位。同时构建了 OmniSocialBench 数据集，包含 118K 结构化训练样本、推理轨迹、模式标签和时序证据跨度。

Result: CogniRoute 在 OmniSocialBench 上达到 59.38% 的平均准确率，比最强闭源基线（GPT-4o 等）高出 15.33 个百分点，比最强开源全模态基线高出 26.77 个百分点。在需要音视频协调、冲突解决和时序社交推理的问题上提升最大。

Conclusion: CogniRoute 通过认知模式引导的路由机制和路由感知强化学习，有效提升了全模态模型在社交视频问答中的推理能力，特别是在需要跨模态协调和时序推理的复杂场景中表现显著优于现有方法。

Classification reason: 该论文聚焦于社交视频问答中的多模态推理，提出基于MoE的框架进行跨模态关系推理、时序推理和冲突解决，属于多模态推理范畴。虽然涉及视频理解，但核心贡献在于推理机制（schema-guided routing + route-aware RL），因此映射到[多模态推理与思维链]（CANONICAL）最为贴切。

Importance reason: Novel routing framework for omni-modal social reasoning, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: Mixture-of-Experts, social video QA, cognitive schema, reinforcement learning for routing, OmniSocialBench benchmark
...
### [196] [Denoising Iterative Self-Correction: Structured Verification Loops for Reliable LLM Reasoning](https://arxiv.org/abs/2606.21724)
...
TL;DR: 本文提出DISC（Denoising Iterative Self-Correction），一种将验证问题输出视为噪声测量、通过多次verify-judge-correct迭代逐步减少推理错误的方法。二值判断门控防止破坏已正确回答，在BIG-Bench Mistake、HotpotQA、GPQA Diamond上优于Chain-of-Verification和Self-Refine，并发现跨模型角色分配可缓解自我确认偏差。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在多步推理中常产生流畅但错误的输出，而简单的自我纠错方法可能破坏原本正确的答案。现有方法缺乏对纠错过程的精确控制，难以在修复错误的同时不引入新的退化。

Method: 提出DISC框架，将验证问题的输出视为关于解决方案何处可能被破坏的噪声测量，通过多次verify-judge-correct迭代逐步减少错误。引入二值判断门控（binary judgment gate）来控制纠错精度，只有当判断门认为需要修正时才允许重写，从而保护已正确的答案。同时探索跨模型角色分配（cross-model role allocation），将验证和判断任务分配给与生成器不同的模型以缓解自我确认偏差。

Result: 在BIG-Bench Mistake、HotpotQA、GPQA Diamond三个基准和四个模型上，DISC在精度-召回率权衡上显著优于Chain-of-Verification和Self-Refine。在BIG-Bench Mistake（Sonnet 4.5）上达到81.6%准确率，每次退化的改进次数比Chain-of-Verification多13倍、比Self-Refine多5倍。在GPQA Diamond上发现能力下限现象：当模型能力不足时，判断器能识别证据矛盾但无法转化为修正。

Conclusion: DISC通过结构化验证循环有效提升了LLM推理的可靠性，验证了将纠错视为迭代去噪过程的有效性。跨模型角色分配可缓解自我确认偏差，为未来推理时计算扩展提供了新方向。

Classification reason: 该论文关注LLM多步推理中的自我纠错机制，属于推理时计算扩展（test-time scaling）范畴，与多模态推理与思维链（multimodal_reasoning_cot）中的推理时计算扩展方向高度吻合。虽然论文主要实验在纯文本推理上，但其方法论（迭代验证-判断-纠错）可直接迁移到多模态推理场景，且该[CANONICAL] subtopic涵盖推理时计算扩展。

Importance reason: Focuses on LLM reasoning correction, not on the user's priority continuous multimodal or video generation topics. Signals: LLM reasoning, self-correction, verification loops, denoising, BIG-Bench Mistake
...
### [197] [The Unreasonable Effectiveness of VLMs for Zero-shot Procedural Mistake Detection](https://arxiv.org/abs/2606.21579)
...
TL;DR: 本文提出ZeProM框架，利用单个预训练VLM以零样本方式联合解决程序性错误检测和时序动作分割，在EgoPER和CaptainCook4D基准上接近甚至超越全监督方法，展示了统一VLM方法在程序性错误检测中的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有程序性错误检测方法依赖多阶段流水线（时序分割+错误检测+可解释性），需要任务特定的监督训练数据，限制了通用性。本文旨在实现零样本程序性错误检测，摆脱对标注数据的依赖。

Method: 提出ZeProM（Zero-shot Procedural Mistake detection）框架，使用单个预训练VLM（Video-Language Model）统一完成时序动作分割和错误检测。通过精心设计的提示策略，让VLM直接对视频进行推理，无需任何任务特定训练。

Result: 在EgoPER和CaptainCook4D两个基准上，ZeProM在零样本设置下接近甚至超越全监督方法。在EgoPER的5个任务上，平均EDA提升4.4点，F1@.5提升2.0点。

Conclusion: 预训练VLM在零样本程序性错误检测中展现出惊人的有效性，统一方法有望替代复杂的多阶段流水线，推动该领域向更通用的解决方案发展。

Classification reason: 该论文使用预训练VLM进行零样本程序性错误检测，核心是利用VLM的推理能力完成时序动作分割和错误检测任务，属于多模态推理范畴，因此映射到[CANONICAL] subtopic "多模态推理与思维链"。

Importance reason: Strong zero-shot VLM reasoning for procedural tasks, but outside user's priority subtopics and negative keywords. Signals: zero-shot VLM, procedural mistake detection, temporal action segmentation, EgoPER benchmark, CaptainCook4D benchmark
...
### [198] [Finding the Evidence: Discovering Decision-Supporting Tokens for On-Policy Reasoning Distillation](https://arxiv.org/abs/2606.22830)
...
TL;DR: 本文发现推理链中存在两种知识：决策（分支点）和证据（支撑决策的中间步骤）。当前方法仅捕获决策，忽略了证据中的实质性知识。作者提出DEAR方法，通过学生熵识别决策，利用隐状态余弦相似度发现支撑证据，并结合师生差异优先覆盖最大知识差距。在数学和代码基准上，DEAR相比标准OPD提升最高+2.5pp（竞赛数学）和+5.7pp（代码生成）。


<details>
  <summary>Details</summary>
Motivation: 现有on-policy推理蒸馏方法通过密集token级监督传递推理能力，但传递信号的本质尚不明确。作者发现推理链包含决策（分支点）和证据（支撑决策的中间步骤）两种知识，当前方法仅捕获决策，而学生自信但错误位置隐藏的证据知识未被传递，导致蒸馏效果受限。

Method: 提出DEAR（Decision-Evidence Aware Reasoning Distillation）方法：1）通过学生熵识别决策token；2）利用隐状态余弦相似度（与决策锚点）发现支撑证据；3）结合师生差异（teacher-student divergence）优先覆盖最大知识差距。该方法在密集token级监督中同时捕获决策和证据知识。

Result: 在三种师生配置下，DEAR在数学和代码基准上一致优于标准OPD：竞赛数学提升+2.5pp，代码生成提升+5.7pp。

Conclusion: 推理链中的证据知识对蒸馏至关重要，DEAR通过发现并传递决策支撑证据显著提升了推理蒸馏效果，揭示了on-policy蒸馏中可传递信号的本质。

Classification reason: 该论文研究推理蒸馏（reasoning distillation），通过决策-证据感知方法提升数学和代码推理能力，属于多模态推理与思维链（multimodal_reasoning_cot）范畴。虽然论文主要聚焦数学和代码推理（非视觉模态），但该[CANONICAL] subtopic涵盖数学/代码/科学推理，且论文的on-policy reasoning distillation方法可推广至多模态场景，因此映射至此。

Importance reason: Focuses on reasoning distillation for math/code, not on the user's priority continuous multimodal or video generation topics. Signals: reasoning distillation, decision vs evidence tokens, on-policy distillation, math and code benchmarks, student entropy and hidden-state cosine similarity
...
### [199] [T-MOR: Learning Motion-Aware Skeleton Representations for Human Action Recognition](https://arxiv.org/abs/2606.21607)
...
TL;DR: T-MOR 提出一种基于骨架序列的可迁移动作表示学习方法，通过多模态对比学习对齐骨架运动与视频、文本表征，仅用轻量骨架输入即可实现高效的人体动作识别，并在多个基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如 CLIP）主要依赖图像或视频的外观级监督，缺乏对人体运动的显式建模，难以捕捉动作中时序结构和物理驱动的身体运动，限制了细粒度人体动作识别的性能。

Method: T-MOR 采用多模态对比学习框架，在训练阶段将骨架运动序列与视频视觉表征、文本语义表征进行对齐，推理时仅需轻量骨架输入。同时构建了包含超过100万条视频-骨架-文本三元组的大规模数据集 PoseCap-1M 以支持预训练。

Result: 在 Toyota Smarthome、Penn Action、UAV-Human、TSU、Charades 等多个动作识别基准上，T-MOR 在动作分类和帧级时序检测任务中均取得一致性能提升，并在少样本和零样本设置下展现出强泛化能力。

Conclusion: T-MOR 验证了以运动为中心的表征学习对于可迁移动作理解的有效性，表明结合骨架运动与多模态监督能够学习到更具泛化能力的动作表示。

Classification reason: 该论文使用多模态对比学习（视频、骨架、文本）进行人体动作识别，属于多模态推理与理解范畴。虽然涉及动作识别，但核心是多模态对齐与推理，因此映射到 [CANONICAL] 多模态推理与思维链（multimodal_reasoning_cot）子主题。

Importance reason: Focuses on skeleton-based action recognition, which is not a priority subtopic and lacks alignment with key boost keywords. Signals: Skeleton-based action recognition, Multi-modal contrastive learning, PoseCap-1M dataset, Few-shot/zero-shot generalization, Lightweight inference
...
### [200] [Look Light, Think Heavy: What Multimodal Chain-of-Thought Reasoning Can and Cannot Do](https://arxiv.org/abs/2606.22565)
...
TL;DR: 本文系统评估了多模态 Chain-of-Thought (CoT) 推理在12个感知与推理任务上的表现，发现：(1) CoT 并非万能，在感知任务中可能带来副作用，而在数学、科学、多图像推理等任务中有效；(2) 现有开源多模态推理模型相比基础模型提升有限，可能因过度侧重数学推理；(3) 视觉推理是当前多模态 CoT 的关键瓶颈，模型呈现"轻视觉、重语言"的模式——语言反思在推理过程中有起伏，而视觉反思持续衰减。


<details>
  <summary>Details</summary>
Motivation: CoT 在纯文本 LLM 中已被广泛验证有效，但在多模态任务中的效果尚不明确。本文旨在系统回答：多模态 CoT 推理能做什么、不能做什么，以及为什么存在局限。

Method: 在12个多模态任务（涵盖感知类和推理类）上，使用14个非推理模型和8个推理模型进行系统评估，分析 CoT 在不同任务类型上的表现差异，并深入探究视觉反思与语言反思在推理过程中的动态变化。

Result: (1) CoT 在感知任务（如视觉定位、目标计数）中可能降低性能，在推理任务（数学、科学、多图像推理）中有效；(2) 开源多模态推理模型相比基础模型提升有限，过度侧重数学推理；(3) 视觉反思在推理过程中持续衰减，而语言反思有起伏，模型呈现"Look Light, Think Heavy"模式。

Conclusion: 多模态 CoT 在处理语言反思方面相对有效，但缺乏在整个推理过程中维持深度视觉内省的能力。视觉推理仍是当前多模态 CoT 的关键瓶颈，需要未来研究加以突破。

Classification reason: 该论文系统研究多模态 Chain-of-Thought 推理的能力与局限，属于多模态推理与思维链方向，直接匹配 [CANONICAL] subtopic "multimodal_reasoning_cot"。

Importance reason: Provides critical analysis of multimodal CoT limitations, directly informing the user's priority on multimodal understanding and reasoning. Signals: multimodal chain-of-thought, visual reasoning bottleneck, look light think heavy, perception vs reasoning tasks, open-source reasoning model evaluation
...
### [201] [Does the Same Token Mean the Same State? MoE Routing as Signal for Reasoning Control](https://arxiv.org/abs/2606.22798)
...
TL;DR: 本文发现稀疏 MoE 语言模型中，相同 token id 在不同上下文下由不同专家产生，路由状态蕴含任务上下文和推理努力模式。基于此提出 RAD（Routing Agreement Decoding），利用锚点 token 附近的 MoE 路由状态进行多 rollout 选择，无需解析答案字符串，在数学、GPQA、代码等任务上达到与 Majority Voting 相当的效果，并适用于代码生成等字符串投票不适用的场景。


<details>
  <summary>Details</summary>
Motivation: 在稀疏 MoE 语言模型中，相同 token id 是否意味着相同的路由状态和专家激活？作者发现并非如此——即使 token id 相同，产生它的专家仍能区分任务上下文、轨迹历史和推理努力模式。这一发现启发了利用路由状态进行测试时推理控制的方法。

Method: 提出 RAD（Routing Agreement Decoding）：在固定锚点（如边界锚点、分隔符锚点）处，将每个 rollout 的 MoE 路由状态表示为锚点窗口内的路由向量，然后通过加权 Jaccard K-NN 寻找最密集的路由盆地中心作为最终选择。无需解析、归一化、执行或投票答案字符串。

Result: 在 10 种稀疏 MoE 配置（gpt-oss, Qwen3-MoE）和 6 个数据集（数学、GPQA、代码）上，RAD 与 Majority Voting 表现相当（RAD 73.9, RAD+DC 74.2 vs Majority 73.6）。在代码生成中，RAD 可直接给出 pass@1 结果，而字符串投票不适用。在 SWE-bench Verified 上，基于路由密度的 patch 选择优于随机基线。

Conclusion: MoE 路由状态蕴含丰富的推理结构信息，可用于测试时推理控制。RAD 提供了一种不依赖答案字符串的多 rollout 选择方法，在字符串投票不适用的场景（如代码生成、patch 选择）中具有独特价值。但 RAD 并非验证器，密集的错误盆地仍可能胜出。

Classification reason: 该论文研究 MoE 路由信号与推理控制，涉及数学推理、代码推理和推理时计算扩展（multi-rollout selection），与 [CANONICAL] 多模态推理与思维链 中的推理时计算扩展、数学/代码推理高度相关。

Importance reason: Novel use of MoE routing states for test-time reasoning control, but tangential to user's priority on continuous multimodal models and generation. Signals: MoE routing as reasoning signal, test-time control without answer parsing, applicable to code generation, not a verifier, limited to sparse MoE models
...
### [202] [VeriEvol: Scaling Multimodal Mathematical Reasoning via Verifiable Evol-Instruct](https://arxiv.org/abs/2606.23543)
*Haoling Li, Kai Zheng, Jie Wu, Can Xu, Qingfeng Sun, Han Hu, Yujiu Yang*

Main category: cs.CL, cs.CV, cs.LG, cs.AI
...
TL;DR: VeriEvol 提出了一种可验证的进化指令数据构建框架，通过类型感知的进化模块提升题目难度，并通过 HTV-Agent 验证器确保答案可靠性，从而为多模态数学推理的强化学习提供可扩展的高质量训练数据。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态数学推理数据扩展方法在扩大数据量时无法保证答案标签的可靠性，且依赖标注者或假设答案正确。本文旨在将数据扩展视为一个可验证的构造问题，在策略更新前解耦题目难度提升和答案可靠性验证两个维度。

Method: 提出 VeriEvol 框架，包含两个可扩展组件：(1) 类型感知进化模块，将低难度图像-问题种子改写为更难的、基于图像的题目；(2) HTV-Agent 验证器，通过多源反证法（假设检验证伪）来验证答案的正确性，只有无法被反驳的答案才被接受。

Result: 在五个视觉数学基准上，将进化后的 SFT 数据从 10K 扩展到 250K 样本，平均准确率从 35.42 提升至 54.73；在固定 backbone、SFT 初始化和 GRPO 配方的情况下，VeriEvol 在未进化的 RL 基线上累计提升 +3.88，其中进化提示贡献 +1.82，HTV-Agent 验证器贡献 +2.06。

Conclusion: VeriEvol 通过解耦难度提升与答案验证，实现了可扩展、可审计的多模态数学推理数据构建，能够直接集成到现有的 GRPO 风格强化学习流程中，显著提升推理性能。

Classification reason: 该论文聚焦于多模态数学推理的数据扩展与验证，属于多模态推理范畴，与 [CANONICAL] subtopic "多模态推理与思维链" (multimodal_reasoning_cot) 高度匹配。

Importance reason: Strongly aligns with multimodal reasoning and verifiable data scaling, but lacks direct connection to continuous language/multimodal models or world models. Signals: multimodal mathematical reasoning, verifiable data construction, type-aware evolution, HTV-Agent verifier, GRPO-style RL
...
### [203] [Improving Reasoning in Vision-Language Models via Perception Verified Self-Training](https://arxiv.org/abs/2606.22158)
...
TL;DR: 本文提出感知验证自训练框架（Perception Verified Self-Training），通过CoT模板（描述-推理-结论）解耦感知与推理，利用无监督方法PerceptEval评估描述质量，结合答案正确性将数据分为易/中/难三档，并设计两阶段课程学习策略，提升VLM的视觉推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLM推理方法依赖人工标注或专有模型的CoT推理链，成本高且难以扩展；自训练方法虽可缓解，但存在视觉幻觉（描述不存在的内容）和语言捷径（依赖文本先验而非视觉依据）问题，因为仅通过答案正确性过滤推理链，未验证视觉感知的正确性。

Method: 提出感知验证自训练框架：(1) 设计CoT模板（描述-推理-结论），将感知与推理解耦；(2) 提出PerceptEval无监督方法，基于描述与图像中视觉/文本元素的对齐程度评估描述质量；(3) 结合描述验证与答案正确性将数据分为三档：易（描述和结论都正确）、中（描述正确但结论错误）、难（描述错误）；(4) 两阶段课程学习：阶段1用易样本训练，阶段2加入中样本，通过描述引导的推理增强过程重新生成推理链，仅保留结论正确的样本。

Result: 在多个VLM推理基准上，该方法显著提升了模型推理性能，有效缓解了视觉幻觉和语言捷径问题，验证了感知验证与课程学习策略的有效性。

Conclusion: 通过显式验证视觉感知的正确性并设计课程学习策略，可以有效提升VLM的视觉推理能力，为低成本、可扩展的VLM推理增强提供了新思路。

Classification reason: 该论文提出基于感知验证的自训练框架来改进VLM的推理能力，核心是Chain-of-Thought推理与视觉感知验证，属于多模态推理与思维链（multimodal_reasoning_cot）这一[CANONICAL]子主题。

Importance reason: Proposes a novel self-training framework with perception verification to reduce hallucinations in VLM reasoning, aligning with multimodal understanding and efficient inference priorities. Signals: self-training for VLM reasoning, perception verification via PerceptEval, curriculum learning with CoT, addresses visual hallucination, no ground-truth captions needed
...
### [204] [Gold Points Sniper: Self-guided Visual Reasoning in VLM for Fine-grained Action Understanding](https://arxiv.org/abs/2606.22409)
...
TL;DR: 本文提出一个三探针评测方案，系统评估强制思维链（forced CoT）在视频问答中的可靠性。在Qwen2.5-VL/Video-MME上的实验表明：CoT链确实高度依赖视频内容（视频交换后链内容大幅变化），但强制CoT并未提升多项选择题准确率，在7B模型上甚至出现统计显著的性能下降。


<details>
  <summary>Details</summary>
Motivation: 强制思维链（forced CoT）被广泛认为能提升视觉语言模型在视频问答中的可靠性，但这一假设缺乏系统性的评测验证。作者希望设计一套简洁的评测方案来检验该假设是否成立。

Method: 提出三探针评测方案：(1) 配对准确率比较：在直接回答、CoT、先答后链、无视频四种条件下对比；(2) 反事实视频交换诊断：交换输入视频后检查CoT链的重叠度和最终答案变化；(3) 四阶视觉退化阶梯。每个探针使用严格和宽松两种正则评分器，并进行多重假设检验校正。在Qwen2.5-VL（7B和72B）上对Video-MME子集进行实验。

Result: CoT链高度视频条件化：交换视频后链重叠度极低，大部分最终答案翻转，与"模板链"零假设相反。但强制CoT并未提升MCQ准确率，7B模型上甚至出现统计显著的性能下降（在事后主评分器选择下）。

Conclusion: 强制CoT在视频问答中的效果并非如普遍假设的那样可靠——虽然CoT链确实依赖视频内容，但强制CoT并未带来准确率提升，甚至可能损害小模型的性能。作者强调结论不必然泛化到其他模型/数据集，并开源了原始响应和复现脚本。

Classification reason: 该论文研究强制思维链（forced CoT）在视频问答中的可靠性，属于多模态推理与思维链（multimodal_reasoning_cot）这一[CANONICAL]子主题的范畴。

Importance reason: Focuses on CoT reliability in video QA, not on the user's priority subtopics like continuous models or generation. Signals: Video-MME evaluation, forced CoT reliability, Qwen2.5-VL, MCQ accuracy drop, counterfactual video-swap
...
### [206] [AIR: Adaptive Interleaved Reasoning with Code in MLLMs](https://arxiv.org/abs/2606.23678)
...
### [207] [MindAlign: Decoding Inner Speech from fMRI Signals via Multimodal Embedding Alignment under Limited Data](https://arxiv.org/abs/2606.20696)
...
TL;DR: MindAlign提出一种两阶段脑信号到文本解码框架：第一阶段将fMRI信号映射到共享多模态语义空间提取语义草图，第二阶段结合视觉上下文提示冻结的多模态语言模型生成自由文本。在静默图像描述fMRI数据上优于基线，且语义到语言投影可跨被试泛化。


<details>
  <summary>Details</summary>
Motivation: 从非侵入性脑信号（fMRI）解码内心语言（inner speech）是极具挑战的任务，主要面临缺乏显式语言输出、训练数据有限、被试间差异大等问题。现有方法依赖任务特定的解码器微调，可扩展性差且难以适应新被试。

Method: 提出MindAlign解耦两阶段框架：第一阶段学习被试特定的神经-语义对齐（neural-semantic alignment），将fMRI活动映射到共享多模态语义空间（使用CLIP空间），提取内心生成句子的潜在语义草图；第二阶段将该语义草图与视觉上下文结合，提示冻结的多模态语言模型进行自由文本生成。两阶段解耦使得语言模型无需微调。

Result: 在静默图像描述fMRI数据集上，MindAlign一致优于仅使用fMRI的基线和随机基线。学习到的语义到语言投影可以跨被试泛化，当与特定被试的神经对齐配对时，仍能实现有效的解码。

Conclusion: 神经信号调制的语义内容超越了图像驱动的先验知识，支持一种可扩展、模块化的脑信号到文本解码方向。两阶段解耦框架为脑机接口中的开放文本生成提供了新范式。

Classification reason: 该论文将fMRI脑信号与多模态语义空间对齐，实现从脑信号到文本的跨模态解码，核心是跨模态嵌入对齐（neural-semantic alignment），属于多模态检索/跨模态对齐范畴。映射到[多模态检索]（multimodal_retrieval）这一CANONICAL subtopic，因为其本质是将fMRI信号作为查询，在共享语义空间中检索/解码对应的文本内容。

Importance reason: This paper focuses on fMRI-to-text decoding, which is outside the user's core priority subtopics and lacks direct connection to continuous multimodal models or video/image generation. Signals: fMRI decoding, brain-to-text, multimodal alignment, limited data, cross-subject generalization
...
### [208] [MAPS: Multi-Anchor Projection Similarity for Joint Vision-Language Geo-Localization](https://arxiv.org/abs/2606.22543)
...
TL;DR: 本文提出MAPS（多锚点投影相似度）框架，将视觉-语言联合地理定位（VLGL）形式化为多锚点几何对齐问题，通过将目标特征投影到由视觉和文本查询特征构建的锚点平面上计算相似度，并引入基于MAPS的对比损失进行训练，在VLGL任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有地理定位模型主要基于点对点对齐（如余弦相似度），无法有效处理联合图像-文本查询中视觉和文本线索共同定义语义子空间的需求，缺乏几何一致性的检索度量。

Method: 提出MAPS（Multi-Anchor Projection Similarity）度量，利用视觉和文本查询特征在高维空间中构建锚点平面，通过目标特征在该平面上的投影长度衡量相似度；同时引入基于MAPS的对比损失，驱动目标特征向对应的锚点平面靠近，实现联合查询与目标之间的几何一致性对齐。

Result: 在VLGL任务上取得SOTA性能，优于基于余弦相似度的基线方法，验证了多锚点几何对齐在联合视觉-语言检索中的有效性。

Conclusion: MAPS框架通过几何对齐的方式有效建模了联合图像-文本查询与目标之间的语义关系，为视觉-语言地理定位提供了一种更具判别力的检索范式。

Classification reason: 该论文研究视觉-语言联合地理定位（VLGL），属于跨模态检索任务，与[多模态检索]（multimodal_retrieval）这一CANONICAL子主题高度匹配。

Importance reason: The paper focuses on geo-localization, which is not a priority subtopic and lacks alignment with the user's key interests in continuous models, video/image generation, or world models. Signals: Multimodal retrieval, Geo-localization, Contrastive learning, Not in priority subtopics, No boost keywords matched
...
### [209] [LightSTAR: Efficient Visual Document Retrieval via Lightweight Selection with Vision-Adaptive Refinement](https://arxiv.org/abs/2606.23539)
...
TL;DR: LightSTAR 提出一种高效的视觉文档检索框架，将检索分解为无 LLM 的视觉选择阶段（利用关键词锚定快速筛选候选页）和视觉自适应语义精炼阶段（对候选页进行细粒度语义匹配），在保持 SOTA 检索精度的同时大幅降低端到端延迟。


<details>
  <summary>Details</summary>
Motivation: 现有基于 MLLM 的视觉文档检索方法虽然精度高，但需要对每一页进行密集的 MLLM 编码，计算成本极高。作者观察到用户查询通常包含关键词锚定，这些词应直接出现在相关页面的可见文本中，因此可以利用这一线索快速缩小候选范围，实现精度与效率的更好权衡。

Method: LightSTAR 包含两个阶段：(1) LLM-free Visual Selection：利用内容锚定的查询编码聚焦信息性词汇，使用无 LLM 的视觉嵌入生成高召回率的候选集；(2) Vision-adaptive Semantic Refinement：仅对候选页进行细粒度语义匹配，通过自适应区域特征融合结合文本与布局线索，并使用困难感知对比目标进行优化。

Result: 实验表明 LightSTAR 在多个视觉文档检索基准上达到 SOTA 检索精度，同时端到端延迟降低数倍，有效解决了精度-效率的权衡问题。

Conclusion: LightSTAR 通过将视觉文档检索分解为粗筛和精炼两阶段，在保持高精度的同时显著提升效率，为实际部署提供了实用的解决方案。

Classification reason: 该论文研究视觉文档检索（visual document retrieval），属于多模态检索范畴，与 [CANONICAL] subtopic "多模态检索"（multimodal_retrieval）高度匹配。

Importance reason: Focuses on efficient visual document retrieval, which is not a priority subtopic and lacks direct connection to user's core interests in continuous multimodal models or generation. Signals: Efficient retrieval, No LLM stage, Visual document focus, Accuracy-efficiency trade-off, Not aligned with priority subtopics

Abstract: Visual document retrieval requires rapidly locating relevant pages from large multi-modal corpora in response to user queries. While recent methods powered by Multi-modal Large Language Models (MLLMs) show competitive accuracy, they suffer from prohibitive computational costs by applying intensive MLLM encoding to every single page. Meanwhile, we observe that user queries are typically keyword-anchored, containing semantically rich words that are expected to appear directly in the visible text of relevant pages, offering an efficient cue for quickly narrowing down candidate pages. Building on this insight, we propose LightSTAR, an efficient framework that decomposes visual document retrieval into: 1) LLM-free Visual Selection, which utilizes content-grounded query encoding to focus on informative words and employs LLM-free visual embeddings to produce a high-recall candidate set; and 2) Vision-adaptive Semantic Refinement, which further performs fine-grained semantic matching exclusively on these top candidates via adaptive region-wise feature fusion to effectively combine textual and layout cues, optimized through a hardness-aware contrastive objective. Experimental results demonstrate that LightSTAR achieves state-of-the-art retrieval accuracy while reducing end-to-end latency by several-fold, offering a highly practical solution to the accuracy-efficiency trade-off in visual document retrieval. Code is available at .
...
### [210] [From Knowing to Acting: Benchmarking Self-Awareness Capability of LLM Agents](https://arxiv.org/abs/2606.20661)
...
TL;DR: 本文提出KAPRO框架和KAware数据集，用于系统评估LLM智能体的自我意识能力——即判断问题是否需要外部工具还是仅靠内部参数化知识解决的能力。实验表明自我意识能力与任务成功率强相关，开源模型和指令跟随模型更容易过度使用工具，而专有模型和推理模型表现出更可靠的认知门控。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体评测基准主要关注执行成功率，忽略了智能体是否具备自我意识能力——即判断一个问题是否需要外部资源（工具/知识库）还是可以仅靠内部参数化知识解决。这种认知-行为对齐的缺失导致智能体可能过度依赖工具或错误地依赖内部知识。

Method: 提出了KAPRO（Knowing-Acting Quadrant PRObe）框架，通过解耦智能体的元认知判断（Knowing）和自发执行（Acting）来评估认知-行为对齐。构建了KAware数据集，将任务严格划分为外部（需要工具）、内部（仅需参数化知识）和混合（两者结合）三个子空间，系统性地探测智能体的认知边界。

Result: 实验表明：1）自我意识能力与任务成功率强相关；2）在内部能力场景中自我意识能力急剧下降；3）开源模型和指令跟随模型由于浅层模式匹配表现出更强的工具过度使用倾向；4）专有模型和推理模型展现出更可靠的认知门控能力。

Conclusion: 自我意识能力是LLM智能体可靠性的关键维度，现有模型在这方面存在显著不足，尤其是开源模型容易过度依赖工具。该工作为智能体认知-行为对齐提供了系统化的评测框架，有助于推动更可靠的自主智能体系统的发展。

Classification reason: 该论文提出了一个评估LLM智能体自我意识能力（区分何时需要外部工具vs内部知识）的基准框架KAPRO和数据集KAware。这属于对LLM智能体认知-行为对齐的评测，与多模态评测与基准（multimodal_benchmark）方向高度匹配，尽管论文主要关注文本LLM而非多模态，但评测框架本身可扩展至多模态场景，且该[CANONICAL] subtopic是最佳匹配。

Importance reason: Focuses on agent self-awareness benchmarking, not on the user's priority subtopics like continuous multimodal models or video generation. Signals: LLM agent self-awareness, tool overuse, cognitive gating, KAPRO framework, KAware dataset
...
### [211] [Jury Duty: Calibration and Orientation Failures in MLLM-as-a-Judge Under Cultural Ambiguity](https://arxiv.org/abs/2606.20676)
...
TL;DR: 本文提出 VOIR DIRE 基准，包含626个跨文化（美国 vs 中国）配对的图像-提示样本，用于评估 MLLM 作为评判者时的文化偏差。研究发现两种失败模式：校准失败（评分尺度压缩）和取向失败（默认偏向某一文化规范）。建议对每个参考人群分别报告对齐度，并将跨文化分歧视为评判者属性。


<details>
  <summary>Details</summary>
Motivation: MLLM-as-a-Judge 通常通过与人类标注的一致性来验证，但当人类标注者来自不同文化背景时，一致性指标无法定义。现有研究忽视了文化异质性对 MLLM 评判的影响，缺乏系统性的跨文化评测基准。

Method: 构建 VOIR DIRE 基准，包含626个文化配对的图像-提示样本，涵盖美国和中国大陆在食物、时尚和建筑三个领域的文化语境。招募两个文化背景的标注者群体，验证了组内可靠性高（a=0.86/0.74）但组间评价分歧大（Q1 r=-0.12）。在6个 MLLM 上分析偏差，将其分解为校准失败（评分尺度压缩）和取向失败（默认偏向某一文化规范）。

Result: 在争议性样本上，评分地板效应机械地验证了更宽容的中国解读；人物提示（persona prompting）部分恢复了校准，但取向偏差仍然存在；参考池的上下文示例加深了取向偏差并推高了高分端而非恢复低分端的使用；模型来源增加了约0.10 MAE 的微小附加偏差。

Conclusion: 建议对每个参考人群分别报告对齐度，并将跨文化分歧视为评判者本身的属性而非单纯的数据问题。MLLM-as-a-Judge 的验证需要超越单一文化标注的一致性指标。

Classification reason: 该论文提出了 VOIR DIRE，一个跨文化多模态评测基准（626个文化配对的图像-提示样本），用于评估 MLLM-as-a-Judge 在文化模糊性下的校准与取向失败。这属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "multimodal_benchmark"（多模态评测与基准）。

Importance reason: Focuses on cultural bias in MLLM evaluation, not on core priority subtopics like continuous models or generation. Signals: MLLM-as-Judge, cultural bias, calibration failure, orientation failure, cross-cultural benchmark
...
### [212] [Beyond 'One Language, One Script': Quantifying Orthographic Bias in Multilingual VLMs with PuMVR](https://arxiv.org/abs/2606.20770)
...
TL;DR: 本文揭示了当前多语言VLM存在"一种语言对应一种文字"的错误假设，提出了PuMVR基准（包含375个跨三种旁遮普语文字的文化图像推理任务），评估10个SOTA VLM后发现显著的"文字差距"——模型在一种文字上能解决视觉谜题，在另一种文字上却失败，准确率差异达16%，脚本一致性率低至24.8%。视觉输入虽提升绝对性能但不缩小相对偏差，CoT推理路径也因文字不同而分化。提出SCR作为文字无关评估的核心指标。


<details>
  <summary>Details</summary>
Motivation: 当前多语言VLM评估假设一种语言对应单一文字系统，忽略了旁遮普语、塞尔维亚语等使用多种文字的语言的数十亿用户，导致模型能力因文字偏差而被割裂。现有基准缺乏对文字相关偏差的系统量化。

Method: 构建PuMVR基准：375个文化相关的图像推理任务，覆盖旁遮普语的三种活跃文字（Gurmukhi、Shahmukhi、Roman）。评估10个SOTA VLM，提出Script Consistency Rate (SCR)作为文字无关评估的核心指标，分析跨文字推理迁移性和CoT路径差异。

Result: 发现显著的Script Gap：模型在一种文字上解决视觉谜题而在另一种文字上失败，准确率差异达16%，SCR低至24.8%。视觉输入提升绝对性能但不缩小相对偏差，推理模式跨文字迁移性有限，CoT路径因文字不同而分化。

Conclusion: 当前多语言VLM存在严重的文字偏差，需要将文字无关评估纳入多语言VLM评测标准。PuMVR和SCR为构建更公平的多语言AI提供了框架。

Classification reason: 该论文提出了PuMVR基准，用于量化多脚本语言在多模态VLM中的正交偏差，属于多模态评测与基准方向，与[多模态评测与基准]（multimodal_benchmark）高度匹配。

Importance reason: Orthographic bias is a novel finding but the paper's focus on Punjabi script evaluation is far from your core priorities in continuous multimodal models and video generation. Signals: Orthographic bias in multilingual VLMs, Script Gap up to 16% accuracy delta, Script Consistency Rate (SCR) metric, Visual input doesn't close script bias, Limited cross-script reasoning transfer
...
### [213] [Evaluation of Medical Vision Language Models HuluMed and MedGemma, and general purpose chatbots Gemma 3, ChatGPT Plus, and Claude Pro on real previously unseen wound images](https://arxiv.org/abs/2606.20723)
...
Importance: 普通相关 (37.46) - 未入选精读

TL;DR: 本文评估了HuluMed、MedGemma、Gemma 3、ChatGPT Plus和Claude Pro等通用与医学专用VLM在真实伤口图像上的临床分析能力。在240个临床决策中，ChatGPT表现最佳（72.50%），Claude次之（62.08%），开源医学专用模型HuluMed最高仅40%。结果表明通用前沿多模态系统在伤口分析上显著优于医学专用模型。


<details>
  <summary>Details</summary>
Motivation: 慢性伤口评估是临床难题，需要准确解读伤口形态、组织成分、血管特征和感染风险。VLM的进步为自动化多模态伤口分析提供了可能，但现有模型在真实临床场景中的表现尚缺乏系统评估。

Method: 构建包含20例临床多样伤口的扩展数据集（涵盖血管、手术、缺血、静脉、淋巴水肿和截肢相关病因），设计12个结构化临床问题框架（涵盖伤口分类、感染风险、血管干预建议、清创紧迫性、伤口治疗选择和高级管理规划），由临床医生对6个VLM的输出进行评分。

Result: ChatGPT 174/240（72.50%），Claude 149/240（62.08%），HuluMed 96/240（40.00%），Gemma 3 81/240（33.75%），MedGemma 4B 62/240（25.83%），MedGemma 27B 42/240（17.50%）。通用模型显著优于医学专用模型。

Conclusion: 通用前沿多模态系统在伤口分析方面表现优于医学专用替代方案，突显了广泛多模态推理能力与领域特定医学知识结合的重要性。当前VLM在高级伤口管理推理、程序规划和自主临床可靠性方面仍存在显著局限。

Classification reason: 该论文评估多个VLM在临床伤口分析任务上的表现，属于多模态模型在特定领域（医学）的评测工作，可归入[多模态评测与基准]这一规范主题。

Importance reason: Paper focuses on medical VLM evaluation, which is outside user's priority subtopics and contains negative keywords. Signals: medical VLM evaluation, wound analysis, negative keywords present, no overlap with priority subtopics, low direction weight match
...
### [214] [How Well Can Your Video Model Remember? Measuring Memory-Budget Trade-offs in Long Video Understanding](https://arxiv.org/abs/2606.20726)
*Yixian Tian*
...
TL;DR: 本文提出了一个紧凑的实证模型，量化长视频理解中回答准确率随帧预算 B 和时间距离 D 的退化规律。基于约 155,000 个二元预测，拟合出加权最小二乘模型，发现 logit-准确率与 log-预算呈线性关系，且距离相关指数 α(D) 随距离对数线性衰减。该模型揭示了不同模型和采样策略在长距离记忆上的巨大差异，并可作为流式视频模型的诊断指标。


<details>
  <summary>Details</summary>
Motivation: 长视频理解模型在有限帧预算下运行，但现有研究缺乏一个统一的框架来预测准确率如何随预算减少和时间距离增加而退化，无法为预算分配提供理论指导。

Method: 收集约 155,000 个二元预测（覆盖 10 个模型和 3 种采样策略），拟合加权最小二乘模型，建立 logit-准确率与 log-预算的线性关系，其中距离相关指数 α(D) 随距离对数线性衰减。

Result: 模型在细胞级加权 R² = 0.05-0.75 范围内有效。在 D=1000s 时，STREAMINGVLM 的 α(1000)=1.26，预算增加 10 倍带来 +29 百分点准确率提升；而 Qwen3-VL 基础模型 α(1000)=0.17，仅提升 +4 百分点。不同采样策略表现出模型相关的权衡。

Conclusion: α(D) 可作为流式视频模型的有用诊断指标，支持原则性的预算分配，并揭示了长距离下的模型排名反转现象。

Classification reason: 该论文研究长视频理解中帧预算与时间距离对回答准确率的影响，提出了一个量化退化规律的实证模型和评测框架，属于多模态视频理解评测方法，因此映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: Provides a useful empirical scaling law for long video memory, but focuses on evaluation rather than the user's priority subtopics of generation or continuous models. Signals: long video understanding, memory-budget trade-off, empirical scaling law, streaming video models, diagnostic metric
...
### [215] [REKEY: Metadata-Grounded Visual-Key Regeneration for Contamination-Resilient VQA Evaluation](https://arxiv.org/abs/2606.20736)
...
TL;DR: ReKey 是一种动态 VQA 评测协议，通过在评测时随机再生图像中的局部视觉关键区域（visual key），生成新的实例、新的答案和受控的视觉搜索难度，从而防止基准泄露导致分数虚高。实验表明，在 V*Bench 上，原始题目得分比再生变体高出 9.5-18.8 个百分点，揭示了现有评测中严重的记忆效应。


<details>
  <summary>Details</summary>
Motivation: 静态 VQA 基准一旦泄露到训练数据中，模型得分反映的是记忆而非真实视觉能力，而重建高质量基准需要大量人工标注且很快又会泄露。因此需要一种可持续更新的评测协议。

Method: ReKey 协议包含三个步骤：(1) 人工标注编辑槽（edit slots），即图像中可被替换的局部区域及其对应的问答对；(2) 在评测时，从候选池中随机采样新的视觉关键区域替换原区域，生成新实例；(3) 自动生成新的答案和构造验证标签，并控制视觉搜索难度。

Result: 在 V*Bench 上对 8 个前沿 VLM 进行评测，原始题目得分比 ReKey 再生变体高 9.5-18.8 个百分点，表明原始基准存在严重的记忆效应。ReKey 能有效区分模型真实视觉能力与记忆能力。

Conclusion: ReKey 通过可再生视觉关键区域的方式，为 VQA 评测提供了一种抗泄露的动态基准协议，能够随模型和数据演化持续保持评测有效性，是静态基准的有力替代方案。

Classification reason: 该论文提出 ReKey，一种通过随机再生视觉关键区域来防止 VQA 评测基准泄露的协议，属于多模态评测方法创新，因此映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: Addresses benchmark contamination in VQA, but is not aligned with user's priority on generative models and continuous representations. Signals: VQA evaluation, benchmark contamination, visual key regeneration, V*Bench, not generative model
...
### [216] [Is Our Benchmark Enough? An Analysis of Continual Learning for MLLMs](https://arxiv.org/abs/2606.20961)
...
TL;DR: 本文分析 MLLM 持续学习基准 MLLM-CL 的结构性缺陷，发现其任务在表征空间中高度可分且固定任务顺序导致结论对单一课程敏感，因此基准主要奖励孤立学习而非真正的持续迁移。作者提出简单无训练的原型路由方法 RePRo 可匹配 MR-LoRA 的 MLLM 路由性能，并指出共享专家对持续学习无益，最后给出新基准设计建议。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 持续学习方法（如 MR-LoRA）高度依赖 MLLM 路由器的假设，但该假设是否真正必要、基准设计是否合理尚未被充分审视。作者希望重新审视 MLLM-CL 基准，揭示其结构性局限，并推动更合理的持续学习评测设计。

Method: 提出 RePRo（Replay-free Prototypical Routing），使用冻结的预训练特征和任务原型进行路由，无需训练和重放。通过实验分析 MLLM-CL 基准中任务的可分离性和任务顺序敏感性，揭示基准的结构性缺陷。

Result: RePRo 以远低于 MR-LoRA 的计算成本匹配其路由性能；共享专家在 MLLM-CL 上并未带来持续学习增益；MLLM-CL 的任务在表征空间中高度可分，且固定任务顺序使结论对单一课程敏感。

Conclusion: MLLM-CL 基准存在结构性局限，主要奖励孤立学习而非持续迁移。未来基准应设计重叠的任务流形、多种任务顺序、细粒度领域漂移，以及奖励前向迁移和保留的评估协议。

Classification reason: 该论文分析现有 MLLM 持续学习基准（MLLM-CL）的结构性缺陷并提出新基准设计建议，属于多模态评测与基准方向，因此映射到 canonical 的 multimodal_benchmark 子主题。

Importance reason: Critiques a popular MLLM continual learning benchmark but focuses on routing and expert analysis, not on the user's priority subtopics. Signals: MLLM continual learning benchmark analysis, training-free prototype routing, shared expert ineffectiveness, benchmark structural limitations, not aligned with user's priority subtopics
...
### [217] [Trip+: Benchmarking Agents in Personalized Interactive Travel Planning](https://arxiv.org/abs/2606.21169)
...
TL;DR: Trip+ 是一个面向个性化交互式旅行规划的智能体评测基准，通过 LLM 模拟器评估智能体在动态交互中生成分钟级行程的能力，发现现有模型倾向于生成技术上可行但忽略用户偏好的疲劳行程。


<details>
  <summary>Details</summary>
Motivation: 现有旅行规划评测往往孤立地评估可行性、个性化或交互性，缺乏一个能够全面衡量智能体在动态交互中根据用户画像进行个性化规划的综合基准。

Method: 构建 Trip+ 基准，包含旅行者画像和动态交互场景，要求智能体生成并修订分钟级行程；使用基于 LLM 的模拟器评估端到端旅行体验，包括疲劳度等主观指标；场景涵盖从简单请求到复杂环境驱动的重新规划。

Result: 评估了 18 个语言模型，发现模型在体验质量上存在一致差距——倾向于生成技术上可行但令人疲惫的行程，与用户画像偏好严重偏离。

Conclusion: Trip+ 揭示了现有语言模型在个性化交互式旅行规划中的不足，为未来研究提供了新的评测方向。

Classification reason: 该论文提出了 Trip+，一个面向个性化交互式旅行规划的智能体评测基准，属于多模态/语言智能体的评测与基准范畴。虽然不直接涉及视觉模态，但评测的是语言模型在复杂交互规划中的能力，与 multimodal_benchmark（多模态评测与基准）这一 [CANONICAL] 子主题高度匹配，因为该子主题涵盖评测基准、评测协议和自动评估。

Importance reason: Focuses on travel planning agents, not on core continuous multimodal or video generation priorities. Signals: Travel planning benchmark, LLM-based simulator, fatigue evaluation, not aligned with priority subtopics, no boost keywords matched
...
### [218] [CheXpercept: A Benchmark for Evaluating Expert-Level Lesion Perception in Chest X-rays](https://arxiv.org/abs/2606.21020)
...
TL;DR: CheXpercept 是一个面向胸部 X 光片（CXR）的专家级病灶感知评测基准，包含粗粒度检测、细粒度轮廓评估与修正、语义级属性提取三个层级，覆盖 7 种临床关键病灶，共 10,400 个 QA 对。对 14 个通用和医学 VLM 的评测表明，模型仅在粗粒度任务上表现尚可，在深层视觉任务上性能急剧下降，且医学 VLM 相比通用 VLM 几乎没有优势。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 在胸部 X 光分析上的评测主要局限于疾病存在性分类，缺乏视觉定位验证，无法评估模型是否具备临床可靠的专家级病灶感知能力。

Method: 构建 CheXpercept 基准：设计模拟放射科医生认知工作流的顺序多层级感知任务（粗粒度检测→细粒度轮廓评估与修正→语义级属性提取）；采用半自动生成管线配合六位医学专家审核构建高质量数据集；对 14 个通用和医学 VLM 进行系统评测。

Result: 模型仅在粗粒度检测层级表现尚可，在细粒度轮廓评估和语义级属性提取任务上性能急剧下降。医学 VLM 相比通用 VLM 几乎没有感知优势，揭示了当前领域自适应的系统性缺陷。

Conclusion: CheXpercept 揭示了现有 VLM（包括医学专用 VLM）在专家级病灶感知方面的严重不足，为未来医学影像 VLM 的感知能力提升提供了重要的评测基准和方向指引。

Classification reason: 该论文提出了 CheXpercept，一个用于评估 VLM 在胸部 X 光片上病灶感知能力的多层级基准，属于多模态评测与基准方向。该论文与 [CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark）高度匹配。

Importance reason: Strong benchmark for medical VLM perception but outside user's core focus on continuous models, generation, and world models. Signals: Medical VLM benchmark, Expert-level lesion perception, Coarse-to-fine evaluation, Medical VLMs show no advantage over general VLMs, Not in user's priority subtopics
...
### [219] [MammoExpert: Benchmarking Chain-of-Thought Reasoning in Mammography Diagnosis](https://arxiv.org/abs/2606.21119)
...
TL;DR: MammoExpert 是首个带有 Chain-of-Thought 推理标注的乳腺钼靶影像数据集，包含 2,379 张图像、67 种 WHO 组织病理学亚型，由 9 位资深放射科医生标注了 42 项影像特征，覆盖三个诊断阶段（原始观察、事实评估、诊断综合）。实验表明，结合该数据集和 CoT 推理训练可显著提升乳腺病变分类的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有公开乳腺钼靶数据集在规模和标注丰富度上有限，尤其缺乏病理亚型覆盖和结构化诊断推理标注，限制了 AI 在可解释乳腺诊断中的发展。

Method: 构建 MammoExpert 数据集，包含 2,379 张钼靶图像、67 种 WHO 分类亚型，每例标注 42 项影像特征，并设计了三阶段 Chain-of-Thought 推理标注（原始观察→事实评估→诊断综合）。在乳腺病变分类任务上，结合公开数据集 CBIS-DDSM 和 MammoExpert 进行训练，并让模型学习 CoT 推理。

Result: 结合 CBIS-DDSM 与 MammoExpert 使分类准确率提升 7.1%，进一步训练 CoT 推理在 MammoExpert 测试集上再提升 4%。在 INBreast 和 Vindr 数据集上，完整方法分别带来 6.9% 和 6.7% 的准确率提升。

Conclusion: MammoExpert 可作为可解释乳腺病变诊断的基准，通过显式 CoT 推理提升诊断准确性和可解释性。

Classification reason: 该论文提出了 MammoExpert，一个带有 Chain-of-Thought 推理标注的乳腺钼靶影像数据集和评测基准，属于多模态（视觉-语言）医学影像领域的评测基准工作，因此映射到 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Medical domain paper with negative keyword overlap and no alignment with priority subtopics. Signals: Medical diagnosis, Chain-of-Thought reasoning, Mammography, Negative keyword "clinical", No priority subtopic match
...
### [220] [CulMind: Benchmarking Multimodal Understanding and Reasoning in Chinese Cultural Heritage](https://arxiv.org/abs/2606.21618)
...
TL;DR: 本文提出 MedLayXPlain，首个大规模医学平实语言生成（MLLG）多模态基准与评估框架，包含 122,789 个区域级样本（覆盖 8 种影像模态），通过 HOVER 流程构建专家-平实语言配对描述，并引入 MedLayEval 轻量评估器。对 33 个 VLM 的评测揭示了医学 VLM 在专家描述上表现优异但平实语言退化严重，通用 VLM 语言可及性高但临床精度不足的系统性专家-平实鸿沟。


<details>
  <summary>Details</summary>
Motivation: 21世纪治愈法案要求患者即时获取诊断影像结果，但现有 Med-VLM 在生成患者可理解的平实语言描述方面缺乏系统评估，专家-平实鸿沟（Expert-Lay Gap）尚未被量化研究。

Method: 构建 MedLayXPlain-122K 数据集（12个公开源数据集、8种影像模态、122,789个样本），通过 HOVER（分层本体验证精炼）三阶段流程生成平实描述：患者中心词汇映射、LLM约束重写、跨模型视觉验证。提出 MedLayEval（3B参数），从27B验证器蒸馏而来，基于5个临床属性评分专家-平实对齐度。

Result: 对33个VLM的评测显示：医学VLM在专家描述上表现强但平实语言显著退化；通用VLM语言更可及但缺乏临床精度；两者均无法充分满足面向患者的沟通需求。

Conclusion: 当前医学VLM和通用VLM均存在专家-平实鸿沟，无法有效服务于患者沟通。MedLayXPlain为弥合这一鸿沟提供了评测基础，呼吁未来研究关注面向患者的医学语言生成。

Classification reason: 该论文提出了医学视觉语言模型（Med-VLM）在患者可理解性方面的首个大规模多模态评测基准 MedLayXPlain-122K 和评估框架 MedLayEval，属于多模态评测与基准方向，因此映射到 canonical 子主题"多模态评测与基准"。

Importance reason: Strong benchmark for medical VLM evaluation but outside user's priority subtopics and negative keywords. Signals: Medical VLM benchmark, Expert-lay gap, Patient communication, Multimodal evaluation, Low personal direction match
...
### [222] [SkillAudit: From Fixed-Suite Benchmarking to Skill-Centered Assessment](https://arxiv.org/abs/2606.22613)
...
TL;DR: SkillAudit 是一个端到端的 Agent 技能评估框架，能够自动从任意技能包生成多维评测报告（效用、效率/成本、安全性），通过基线对比原则和两阶段安全检测（静态语义分析+动态运行时验证）对真实技能市场中的技能进行审计，发现超过7%的技能存在风险。


<details>
  <summary>Details</summary>
Motivation: 现有 Agent 技能评估方法仍依赖固定任务套件，无法区分技能的边际贡献与骨干模型能力，且当任务超出技能预期范围时评估失效。随着技能市场快速扩张，亟需一种以技能本身为中心的自动化评估框架。

Method: SkillAudit 以任意 Agent 技能包为输入，自动从技能包中构建能力对齐的评估任务，在隔离沙箱环境中执行并收集证据，使用 LLM 判断进行自动化检查。采用基线对比原则衡量效用和效率/成本，引入两阶段检测范式（静态语义分析+动态运行时验证）评估安全风险。

Result: 对覆盖23个职业类别的真实技能包进行扫描后，发现超过7%的技能存在风险状态。

Conclusion: SkillAudit 提供了一种以技能为中心的自动化评估框架，弥补了固定套件评估范式的不足，能够有效识别高风险技能，为技能市场的健康发展提供保障。

Classification reason: 该论文提出 SkillAudit，一个面向 LLM Agent 技能的自动化评估框架，属于多模态理解与生成方向下的评测范畴。虽然论文聚焦于 Agent 技能评估而非传统多模态评测，但其核心贡献是构建评测基准与自动化评估方法，与 [CANONICAL] subtopic "多模态评测与基准" 的评测协议、自动评估等主题高度吻合，因此复用该 subtopic。

Importance reason: Focuses on agent skill evaluation rather than core multimodal generation or world model advances. Signals: Agent skill assessment, safety auditing, fixed-suite limitation, LLM-based judging, sandbox testing
...
### [223] [LEViL: Label-Efficient Video Learning via Zero-Shot Distillation over VLM-Generated Pseudo-Label Spaces](https://arxiv.org/abs/2606.21358)
*Aslı Çelik*
...
TL;DR: 提出LEViL框架，通过VLM生成伪标签空间进行零样本蒸馏，实现无标注视频预训练+目标标签集感知微调，在UCF101和HMDB51上超越现有半监督视频动作识别方法。


<details>
  <summary>Details</summary>
Motivation: 有监督视频预训练需要大规模标注源数据集，且源域与目标域的相似性影响迁移效果。构建不同目标域的标注预训练数据集成本高、难以扩展。因此需要一种标签高效的视频学习方法。

Method: 预训练阶段：VLM生成未标注视频的文本描述，构建可解释的语义伪标签空间；冻结的视频语言模型在该空间上产生零样本软目标分布，学生视频编码器学习语义丰富的表征。下游微调阶段：目标标签集感知微调结合有监督学习与零样本蒸馏，保留VLM语义引导的同时适配目标任务。

Result: 在UCF101和HMDB51上，LEViL在所有评估的有限标注设置下均优于对比的半监督视频动作识别方法。无标注预训练阶段学到的表征在完整数据微调中也能提供有效初始化。

Conclusion: LEViL框架通过VLM生成的伪标签空间实现零样本蒸馏，有效解决了视频预训练对大规模标注数据的依赖问题，在标签稀缺场景下表现出色。

Classification reason: 论文提出了一种标签高效的视频学习框架，利用VLM生成伪标签空间进行零样本蒸馏，并在UCF101和HMDB51上评测半监督视频动作识别性能。核心贡献是评测框架和方法，属于多模态评测与基准（multimodal_benchmark）范畴。

Importance reason: Focuses on semi-supervised video action recognition with VLM distillation, not aligned with user's priority on generative models or continuous representations. Signals: video action recognition, semi-supervised learning, VLM distillation, pseudo-label space, UCF101/HMDB51
...
### [224] [MIRCaps: A Large-Scale Mixed-Domain Dataset with Image-Level and Region-Level Captions for Fine-Grained Vision-Language Learning](https://arxiv.org/abs/2606.21419)
...
TL;DR: 本文提出了MIRCaps，一个大规模混合域图像-标题数据集，包含141,364张图像、约98万图像级标题、174万区域级标题和139万边界框标注。每个图像平均有7个图像级标题和每个标注框7个区域级标题，覆盖目标类别、大小、颜色、动作、状态和环境上下文。实验表明轻量级VLM（SmolVLM-256M-Instruct、BLIP、BLIP2、Qwen2.5-VL 3B-Instruct）可在该数据集上有效微调，提升图像描述和目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有混合域图像-标题数据集在通用和CCTV视频监控场景下仍十分有限，缺乏同时包含图像级和区域级细粒度标题的大规模数据集，限制了VLM学习细粒度视觉属性的能力。

Method: 构建MIRCaps数据集：收集141,364张图像（混合通用域和CCTV监控域），为每张图像生成平均7个图像级标题（描述场景不同方面），为每个标注边界框生成7个区域级标题（描述目标类别、大小、颜色、动作、状态和环境上下文），总计981,947个图像级标题、1,742,264个区域级标题和1,391,779个边界框标注。使用该数据集对轻量级VLM进行微调。

Result: 轻量级VLM（SmolVLM-256M-Instruct、BLIP、BLIP2、Qwen2.5-VL 3B-Instruct）在MIRCaps数据集上微调后，在图像描述和目标检测两个下游任务上取得有效提升，证明了数据集的质量和实用性。

Conclusion: MIRCaps数据集填补了混合域细粒度图像-标题数据的空白，其图像级和区域级互补标题设计有助于VLM学习细粒度视觉属性，可有效支持轻量级VLM的微调训练。

Classification reason: 该论文提出了一个大规模混合域图像-标题数据集（MIRCaps），包含图像级和区域级标题，用于细粒度视觉语言学习。这属于多模态评测与基准（multimodal_benchmark）范畴，因为论文的核心贡献是构建了一个新的多模态数据集/基准，用于训练和评估VLM在下游任务（图像描述、目标检测）上的表现。

Importance reason: Dataset focuses on fine-grained vision-language learning but lacks direct connection to user's priority subtopics like continuous models or video generation. Signals: large-scale mixed-domain dataset, fine-grained captions, lightweight VLM fine-tuning, no continuous or autoregressive focus, no video or world model signals

Abstract: Despite recent progress in Vision-Language Models (VLMs), mixed-domain image-caption datasets for both general-purpose and CCTV-based video surveillance systems remain limited. To address this gap, we introduce a large-scale multimodal dataset comprising 141,364 images, 981,947 image-level captions, 1,742,264 region-level captions, and 1,391,779 bounding box annotations. Each image is associated with an average of seven image-level captions describing different aspects of the overall scene, as well as seven region-level captions for each annotated bounding box. These complementary caption types are designed to help VLMs learn fine-grained visual attributes, including object categories, estimated sizes, colors, actions, states, and surrounding environmental context. We demonstrate the effectiveness of the dataset on two important downstream tasks: image captioning and object detection. Experimental results show that lightweight VLMs, including SmolVLM-256M-Instruct, BLIP, BLIP2, and Qwen2.5-VL 3B-Instruct, can be effectively fine-tuned using our dataset. Our dataset and code are publicly available at .

</details>


### [225] [BabelJudge: Measuring LLM-as-a-Judge Reliability Across Languages and Agent Trajectories](https://arxiv.org/abs/2606.22329)
*Shreyas KC*
...
TL;DR: 本文提出 Agent-as-a-Router 框架，将LLM路由建模为上下文-行动-反馈循环，通过部署中积累的执行经验来弥补信息不足，并构建了 CodeRouterBench 评测基准，实验表明该方法在分布内和分布外任务上均取得最低累积遗憾。


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由方法将路由视为静态的一次性分类问题，存在信息不足的性能瓶颈。作者发现仅向路由器中添加任务维度的性能统计信息就能带来15.3%的相对提升，说明信息缺口是主要限制因素，因此需要一种能动态积累执行经验的路由框架。

Method: 提出 Agent-as-a-Router 框架，将路由形式化为 C-A-F（上下文→行动→反馈→上下文）循环。实例化为 ACRouter，包含编排器（Orchestrator）、验证器（Verifier）和记忆模块（Memory），在部署过程中不断积累执行经验来缩小信息缺口。同时构建 CodeRouterBench 评测环境，包含约10K个任务实例和8个前沿LLM的验证分数，支持基于遗憾的路由比较。

Result: ACRouter 在分布内任务上取得最低累积遗憾，并能泛化到分布外的智能体编程任务，证明该路由框架能主动缩小信息缺口。

Conclusion: 将LLM路由从静态分类问题重新定义为动态经验积累的循环过程，能有效提升路由性能，为多模型协作场景提供了新范式。

Classification reason: 该论文提出 CodeRouterBench（包含约10K个任务实例的评测基准）用于评估LLM路由策略，属于多模态/语言模型评测与基准方向，因此映射到 canonical subtopic "多模态评测与基准"。

Importance reason: Focuses on LLM routing for coding tasks, not on the user's priority subtopics like continuous multimodal models or video generation. Signals: LLM routing, coding tasks, information deficit, cumulative regret, CodeRouterBench

Abstract: Real-world users typically have access to multiple Large Language Models (LLMs) from different providers, and these LLMs often excel at distinct domains, yet none dominate all. Consequently, routing each task to the most suitable model becomes critical for both performance and cost. Existing routers treat this as a static, one-off classification problem. However, we identify the performance bottleneck for these routers as information deficit: simply augmenting a vanilla LLM router with performance statistics at the task-dimension level yields a 15.3% relative gain, surpassing a heuristic router built on the same dimension-level priors. Motivated by this finding, we propose Agent-as-a-Router, a framework that formalizes routing as a C-A-F loop (Context->Action->Feedback->Context). It closes the information gap by accumulating execution-grounded experience during deployment. We instantiate this framework as ACRouter, composed of an Orchestrator, a Verifier, a Memory module, and introduce CodeRouterBench, an evaluation environment comprising ~10K task instances with verified scores from 8 frontier LLMs, enabling regret-based router comparison on streaming tasks. Experiments show that ACRouter achieves the lowest cumulative regret on in-distribution tasks and generalizes to out-of-distribution agentic-programming tasks, demonstrating that our routing framework actively closes the information gap. Codes and benchmarks are released at .

</details>


### [227] [Chehre: An Emoji-Prompted Video Dataset for Perceptually Diverse Facial Expression Recognition](https://arxiv.org/abs/2606.21657)
...
TL;DR: Chehre 是一个由 emoji 提示驱动的面部表情视频数据集，包含 2111 个高质量视频，由 203 名表演者录制、902 名标注者验证。数据集使用合成人脸保护隐私，并定义了主导表情识别和分布表情识别两个基准任务。评测显示当前视觉语言模型在该任务上表现有限，最佳模型 Top-1 准确率仅 32.5%。


<details>
  <summary>Details</summary>
Motivation: 现有面部表情识别数据集多关注静态图像、基本情绪类别或单一确定性标注，缺乏对动态面部表情和人际感知多样性的建模。作者希望构建一个覆盖更广泛表情（40 种 emoji 表情）、包含动态视频、并捕捉标注者间感知差异的数据集。

Method: 收集 203 名参与者根据 40 个面部 emoji 提示录制的视频，将面部运动迁移到合成人脸以保护隐私。由 902 名标注者使用 emoji 和标签进行标注。定义两个任务：主导表情识别（预测多数人选择的标签）和分布表情识别（预测标注分布）。使用随机采样和 persona prompting 方法让 VLM 生成多个预测。

Result: 最佳模型（GPT-4o）在主导表情识别上 Top-1 准确率仅 32.5%，分布表情识别的 Spread Ratio 远低于人类参考值。表明当前 VLM 在细粒度、多样化的面部表情识别上仍有显著不足。

Conclusion: Chehre 数据集为动态、多样化、分布式的面部表情识别提供了新的评测基准，揭示了现有 VLM 在感知多样性方面的局限性，为未来研究提供了方向。

Classification reason: 该论文提出了一个面向面部表情识别的视频数据集 Chehre，并定义了 dominant expression recognition 和 distributional expression recognition 两个基准任务，评测了多个视觉语言模型。这属于多模态评测与基准（multimodal_benchmark）范畴，该 subtopic 是 [CANONICAL] 条目，直接复用。

Importance reason: Focuses on facial expression recognition dataset, not aligned with user's priority on continuous multimodal models or video generation. Signals: Emoji-prompted video dataset, facial expression recognition, vision-language model benchmark, low model accuracy, privacy-preserving synthetic faces
...
### [228] [Zero-Shot Vision-Language Models for Classroom Engagement Recognition: A Benchmark Study of Prompt Sensitivity and Cross-Dataset Generalization](https://arxiv.org/abs/2606.21861)
...
TL;DR: 本文系统评测了CLIP、BLIP-VQA、GPT-4o、LLaVA-1.5-7B和Qwen2.5VL-7B五种VLM在课堂参与度识别任务上的零样本表现，揭示了三个主要失败模式：个体学生识别近乎随机、严重的类别坍缩（85-100%预测集中在单一类别）、以及极端提示敏感性（相同图像上准确率波动高达32个百分点）。场景级分类表现较好，CLIP和GPT-4o在行为锚定提示下kappa约0.60。


<details>
  <summary>Details</summary>
Motivation: 自动化课堂参与度识别对可扩展学习分析具有重要意义，但现代VLM在零样本条件下对此任务的适用性尚未被系统探索。

Method: 在DAiSEE（个体学生视频，300个测试片段）和SCB（场景级图像，1168张）两个数据集上，使用三种提示变体（最小化、基于评分标准的、思维链）对五种VLM进行零样本评测，分析其性能、类别分布和提示敏感性。

Result: 个体学生识别上Cohen's kappa不超过0.10（近乎随机）；模型将85-100%预测分配给单一参与度等级（类别坍缩）；相同图像上提示措辞变化导致准确率波动高达32个百分点；场景级分类上CLIP和GPT-4o在行为锚定提示下kappa约0.60；GPT-4o的安全过滤器拒绝了98%涉及个体学生面部的思维链请求。

Conclusion: 零样本VLM目前不适合个体学生层面的课堂参与度识别，但场景级分类更具可行性。研究为VLM在教育观察系统中的应用提供了校准基线和关键设计考量。

Classification reason: 该论文系统性地评测了多种VLM在课堂参与度识别任务上的零样本表现，属于多模态模型在特定任务上的评测基准研究，与[CANONICAL] subtopic "多模态评测与基准" 高度匹配。

Importance reason: Focuses on classroom engagement recognition, which is far from the user's priority subtopics like continuous language models, video generation, or world models. Signals: zero-shot VLM benchmark, prompt sensitivity, class collapse, educational domain, no continuous token or generation focus
...
### [229] [BLUEX v2: Benchmarking LLMs on Open-Ended Questions from Brazilian University Entrance Exams](https://arxiv.org/abs/2606.22723)
...
TL;DR: BLUEX v2是一个面向巴西大学入学考试第二阶段开放性问题的评测基准，包含395道题（919个子题），55.7%含图像，覆盖多学科，使用LLM-as-a-judge评估了21个模型，发现数学推理和图像理解是最困难的能力维度。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评测在葡萄牙语上关注不足，尤其是需要深度推理和生成能力的开放性问题。BLUEX仅覆盖了巴西大学入学考试第一阶段的多选题，未涉及更具挑战性的第二阶段自由作答题目。

Method: 从UNICAMP(Comvest)和USP(Fuvest)两所巴西顶尖大学2022-2025年的第二阶段入学考试中收集395道题（919个子题），55.7%含图像。每道题标注学科、官方参考答案、LLM生成的评分标准和六种认知能力标签。采用LLM-as-a-judge协议评估21个SOTA LLM。

Result: 模型得分在4.18-9.10之间（0-10分制），差距达4.92分。数学推理和图像理解是最困难的能力维度。数据集、评估代码和模型输出已公开。

Conclusion: BLUEX v2填补了葡萄牙语开放性问题评测的空白，揭示了当前LLM在数学推理和图像理解方面的不足，为多语言、多模态评测提供了新基准。

Classification reason: 该论文提出了BLUEX v2基准，用于评测LLM在巴西大学入学考试开放性问题上的表现，包含图像理解（55.7%的问题含图像）和多学科推理。这属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题，因为论文核心贡献是构建了一个新的多模态评测基准。

Importance reason: Focuses on Portuguese exam benchmarking, not aligned with user's priority on continuous multimodal models or video/image generation. Signals: Portuguese benchmark, open-ended exam questions, LLM-as-a-judge, math reasoning, image understanding

Abstract: Although Large Language Models (LLMs) excel in many tasks, their assessment in Portuguese has received less attention, particularly for open-ended, discursive tasks that demand deeper reasoning and generation capabilities. While the original BLUEX benchmark addressed the scarcity of Portuguese evaluation datasets through multiple-choice questions from Brazilian university entrance exams, it did not cover the more challenging second-phase examinations, which require free-form written responses. In this work, we introduce BLUEX v2, a benchmark derived from the second-phase entrance exams of Brazil's two leading universities: UNICAMP (Comvest) and USP (Fuvest), spanning exam years 2022-2025. Our dataset comprises 395 questions unfolding into 919 graded subquestions, with 55.7% of questions containing associated images. Each question is annotated with subject area, official reference answers, LLM-generated rubric criteria, and six cognitive capability tags. We evaluate 21 state-of-the-art LLMs using an LLM-as-a-judge protocol. Results reveal a 4.92-point performance spread across models (4.18-9.10 on a 0-10 scale), with Mathematical Reasoning and Image Understanding emerging as the hardest capability dimensions. The dataset, evaluation code, and model outputs are publicly available at .

</details>


### [230] [DynamicMem: A Long-Horizon Memory Benchmark in Real-World Settings](https://arxiv.org/abs/2606.22877)
...
TL;DR: DynamicMem 是一个面向 LLM agent 长期记忆能力的合成基准，构建了15个月、跨16个应用的用户活动轨迹（平均220万token），用于评测系统在异构用户画像（属性、习惯、偏好）随时间演变时的记忆与推理能力。实验发现：记忆检索是主要瓶颈（93%失败源于检索而非模型），且现有系统无法同时保持正确事实和更新变化事实。


<details>
  <summary>Details</summary>
Motivation: 现有记忆评测基准仅通过短时、简化的交互评估 LLM agent 的记忆能力，忽略了真实世界中用户画像的异构性（属性、习惯、偏好以不同速度演变）、外部上下文驱动变化（季节、生活事件）以及证据分散于多个应用中的隐式推理需求。

Method: 构建合成基准 DynamicMem，为每个用户生成15个月、跨16个应用（电商、健身、社交等）的活动轨迹，平均220万token、1772个事件。用户画像包含属性、习惯、偏好三类，随时间演变且不显式给出，需从分散信号中推断。在5个季度检查点评估5种代表性记忆系统。

Result: (i) 画像重建性能随历史长度增长而下降，但服务任务准确率保持平稳；(ii) 现有系统无法同时保持正确事实和更新变化事实，错误集中在偏好和命名指代上；(iii) 超过93%的失败源于记忆检索而非生成模型本身。

Conclusion: 长期记忆的核心瓶颈在于记忆检索机制而非模型推理能力，未来改进应聚焦于记忆系统的设计。DynamicMem 为评估和推动 LLM agent 长期记忆能力提供了更真实的基准。

Classification reason: 该论文提出了 DynamicMem，一个评估 LLM agent 长期记忆能力的合成基准，属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）子主题。虽然论文主要关注文本记忆，但其评测框架涉及多应用、多模态场景下的用户画像记忆，与多模态评测基准高度相关。

Importance reason: Focuses on LLM agent memory benchmarks, not on the user's priority subtopics like continuous language models or video generation. Signals: Long-horizon memory benchmark, LLM agent evaluation, memory retrieval bottleneck, not aligned with priority subtopics, no boost keywords matched
...
### [231] [StatABench: Dataset and Framework for Evaluating Statistical Analysis Capabilities of LLMs](https://arxiv.org/abs/2606.22977)
...
TL;DR: 本文提出 StatABench，一个系统评估大语言模型统计分析能力的基准，包含 Stat-Closed（404道题，18个统计主题，多种题型）和 Stat-Open（30个开放式建模任务）两部分。实验表明，即使 GPT-5.1 在 Stat-Closed 上也仅达 68.6%，最佳开源模型为 60.6%，最佳 agent 框架在 Stat-Open 上平均得分 61.86，揭示了当前 LLM 与可靠统计分析之间的显著差距。


<details>
  <summary>Details</summary>
Motivation: 统计分析是一个广泛且复杂的领域，需要领域知识和工具熟练度。现有评估 LLM 统计分析能力的基准在范围和格式上都非常有限，缺乏系统全面的评测框架。

Method: 构建了 StatABench 基准，包含两个互补部分：(1) Stat-Closed：404道题，覆盖18个统计主题，包含选择题、填空题、决策题和实际应用题四种格式；(2) Stat-Open：30个来自专业竞赛的复杂开放式建模任务。使用 LangChain MCP 框架和多个数据科学 agent 评估 LLM，并通过 LLM-as-Judge 协议评估 Stat-Open 的解决方案。

Result: GPT-5.1 在 Stat-Closed 上仅达 68.6%，最佳开源模型为 60.6%。在 Stat-Open 上，最佳 agent 框架平均得分 61.86。揭示了当前 LLM 在工具接地推理、方法论决策和端到端统计建模方面仍存在显著挑战。

Conclusion: 当前 LLM 与可靠统计分析能力之间仍存在显著差距，特别是在工具接地推理、方法论决策和端到端统计建模方面。StatABench 为系统评估和追踪 LLM 统计分析能力的进展提供了有价值的基准。

Classification reason: 该论文提出了 StatABench，一个评估 LLM 统计分析能力的基准，包含封闭式问答和开放式建模任务。这属于多模态理解与生成方向下的"多模态评测与基准"（multimodal_benchmark）子主题，因为该子主题涵盖各类评测基准，且论文评估的是 LLM 在统计分析这一复杂领域的综合能力。

Importance reason: This paper focuses on statistical analysis evaluation, which is outside your configured priority subtopics and lacks alignment with your direction weights. Signals: LLM evaluation benchmark, statistical analysis, GPT-5.1 performance, open-ended modeling tasks, tool-grounded reasoning
...
### [232] [PIVOTSBench: Evaluating Fine-Grained Interpersonal Relationship Reasoning in Multimodal Large Language Models](https://arxiv.org/abs/2606.23092)
...
TL;DR: PIVOTSBench 是首个评估多模态大模型在细粒度人际关系推理能力的基准，基于 Social-IQ 2.0 和 YouTube 数据构建，包含辅助任务评估模型识别关键视觉线索的能力。实验表明现有 MLLM 在此任务上表现有限，视觉模态和社交角色信息对推理有显著影响。


<details>
  <summary>Details</summary>
Motivation: 人类天生具备理解细粒度人际关系的能力，这是日常社交互动的核心。然而，尽管这种推理本质上是多模态的，现有 MLLM 在此方面尚未被充分探索。作者旨在填补这一空白，系统评估 MLLM 在人际关系维度上的推理能力。

Method: 基于 Social-IQ 2.0 和 YouTube 数据构建 PIVOTS 基准，包含双向人际关系维度预测任务（基于心理学研究）以及辅助任务（识别关键视觉线索）。评估了闭源和开源 MLLM，并通过消融实验分析视觉模态和对话中显式社交角色信息的影响，以及联合预测和成对预测设置的效果。

Result: 现有 MLLM 在细粒度人际关系推理上表现有限；视觉模态和显式社交角色信息对推理有显著影响；联合预测和成对预测设置有助于提升 MLLM 在双向 PIVOTS 维度上的评分能力。

Conclusion: PIVOTSBench 揭示了当前 MLLM 在细粒度人际关系推理方面的不足，为未来多模态社交智能研究提供了新的评测基准和分析方向。

Classification reason: 该论文提出了 PIVOTSBench，这是首个评估多模态大模型在细粒度人际关系推理能力的基准，属于多模态评测与基准方向。该论文与 [CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark）高度匹配。

Importance reason: Focuses on a niche multimodal benchmark for interpersonal reasoning, not aligned with user's priority subtopics like continuous models or generation. Signals: Multimodal benchmark, interpersonal reasoning, Social-IQ 2.0, MLLM evaluation, visual cues
...
### [233] [Bayesian Adaptation Gym: A Benchmark for the Bayesian Low-Rank Adaptation of Multi-Modal Language Models](https://arxiv.org/abs/2606.22188)
...
TL;DR: 本文提出 Bayesian Adaptation Gym (BAG)，一个用于多模态语言模型贝叶斯低秩适配的标准化评测基准，包含多模态数据集、任务套件和经典基线实现，用于评估校准、分布偏移鲁棒性和主动学习下的不确定性决策。


<details>
  <summary>Details</summary>
Motivation: 大型多模态语言模型在高风险场景中部署时，良好的不确定性校准至关重要。传统贝叶斯方法对所有权重进行后验近似，对现代大模型不可行。贝叶斯低秩适配虽被提出，但缺乏标准化基准来评估这些方法的效果和适用场景。

Method: 构建 BAG 基准，提供经典贝叶斯基线和最新适配方法的参考实现，设计多模态数据集和任务套件，用于评估校准性能、分布偏移下的鲁棒性以及主动学习中的不确定性决策能力，并在不同模型规模、数据集和任务上进行广泛实验。

Result: 通过大量实验揭示了当前贝叶斯适配方法在不同场景下的成功与失败，为贝叶斯低秩适配方法提供了系统性的比较分析。

Conclusion: BAG 填补了贝叶斯低秩适配方法缺乏标准化评测基准的空白，为后续研究提供了开源平台和参考基线。

Classification reason: 该论文提出了一个用于多模态语言模型贝叶斯低秩适配的标准化评测基准（BAG），包含多模态数据集、任务套件和基线实现，属于多模态评测与基准方向。因此映射到 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Focuses on Bayesian adaptation benchmark for MLLMs, which is tangentially related to unified multimodal models but lacks direct connection to priority subtopics like continuous models or generation. Signals: Bayesian low-rank adaptation benchmark, multi-modal language model calibration, distribution shift robustness, active learning, not aligned with priority subtopics

Abstract: Large multi-modal language models are increasingly deployed in high-stakes domains, making well-calibrated uncertainty essential. Traditional Bayesian methods approximate posteriors over all model weights, which becomes intractable for modern large models. For this reason, recent work instead considers Bayesian low-rank adaptation to enable tractable posterior approximation. Due to a lack of a standardized benchmark to evaluate these approaches, it remains unclear where these methods provide meaningful benefits. To fill this gap, we introduce Bayesian Adaptation Gym (BAG), a benchmark for the Bayesian adaptation of multi-modal language models. BAG provides reference implementations of classic Bayesian baselines and state-of-the-art adaptation methods, along with a multi-modal dataset and task suite designed to probe calibration, robustness under distribution shift, and decision-making under uncertainty via active learning. Using BAG, we conduct and report extensive experiments across model sizes, datasets, and tasks to highlight the successes and failures of current Bayesian adaptation approaches. To enable further research, BAG is fully open source: .

</details>


### [234] [SAGE: An Expert-Annotated South Asian GI Endoscopy Dataset for Multimodal Learning and Hallucination Analysis](https://arxiv.org/abs/2606.22144)
...
TL;DR: SAGE是一个由专家标注的南亚胃肠内窥镜多模态数据集，包含1300张图像、描述文本（含幻觉标签）、18个标签和14726个问答对。实验表明，现有模型在该数据集上性能显著下降，分类模型平均下降58%，LMM在解剖标志检测和异常检测上的GREEN分数分别下降0.308和0.410，揭示了地理种群偏移对AI辅助诊断的严重影响。


<details>
  <summary>Details</summary>
Motivation: 南亚地区胃肠癌负担日益加重，但缺乏公开的本地内窥镜数据集，导致现有AI诊断模型可能存在种群偏差。作者旨在填补这一地理多样性空白，构建首个南亚胃肠内窥镜数据集，以评估和推动地理包容性的AI辅助诊断工具开发。

Method: 收集1300张南亚地区胃肠内窥镜图像，由专家标注18个标签、图像描述（含幻觉标签）和14726个QA对。基于该数据集，对多类分类模型（如ResNet、EfficientNet等）和当代LMM（如LLaVA、GPT-4V等）进行跨种群偏移的基准测试，使用GREEN评分等指标评估性能。

Result: 多类分类模型在南亚数据集上平均性能下降58%。LMM在解剖标志检测上GREEN分数下降0.308，异常检测下降0.410，揭示了显著的地理种群偏移问题。

Conclusion: 现有GI诊断AI模型存在严重的地理种群偏差，南亚地区缺乏代表性数据。SAGE数据集为开发地理包容性的AI辅助诊断工具提供了重要资源，并呼吁社区关注多模态模型在低资源地区的公平性和泛化能力。

Classification reason: 该论文提出了一个面向南亚胃肠内窥镜图像的多模态数据集（SAGE），包含图像描述、多标签分类和VQA任务，并对现有模型进行了跨种群偏移的基准评测。这属于多模态评测与基准（multimodal_benchmark）范畴，是[CANONICAL]子主题。

Importance reason: Focuses on a specialized medical domain dataset with no direct connection to the user's priority subtopics in continuous multimodal models, video/image generation, or world models. Signals: Medical domain dataset, Geographic population bias, Hallucination analysis, No priority subtopic alignment, Negative keyword "clinical" present
...
### [235] [MMGist: A Comprehensive Multimodal Benchmark for 2027](https://arxiv.org/abs/2606.22437)
...
TL;DR: MMGist是一个经过三阶段筛选（文本消融过滤、跨模型饱和过滤、异常检测过滤）构建的高质量多模态评测基准，包含7,262个样本，覆盖7个能力维度。实验表明，MMGist在减少69%评测样本的同时，保持了高保真度的模型排名（Spearman ρ=0.98），并将跨模型区分度提升78%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言基准存在三大问题：1）许多样本不依赖视觉线索，无法有效衡量多模态理解；2）许多样本已接近性能饱和，区分度不足；3）少量异常样本影响评测可靠性。因此需要构建一个更高质量、更具区分度的多模态评测基准。

Method: 提出三阶段筛选流水线：1）文本消融过滤——去除仅凭文本即可回答的样本；2）跨模型饱和过滤——去除所有模型均能正确或均无法回答的样本；3）异常检测过滤——去除标注错误或歧义样本。最终从23,250个原始样本中筛选出7,262个高质量样本，覆盖7个能力维度。

Result: 在27个主流LVLM上评测，MMGist与原始池的模型排名Spearman ρ=0.98，评测样本减少69%，跨模型区分度提升78%。发现视觉逻辑（Visual Logic）仍是当前LVLM的系统性短板，知识密集型维度（如Expert Knowledge）是区分闭源与开源模型的重要因素。

Conclusion: 高质量评测应优先关注视觉依赖性、区分度和可靠性，而非单纯追求基准规模。MMGist为多模态模型评测提供了一个更高效、更可靠的基准方案。

Classification reason: 该论文提出MMGist，一个经过精心筛选的多模态评测基准，属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题，直接匹配。

Importance reason: Strong benchmark curation methodology but low alignment with user's priority on continuous multimodal models and generation. Signals: Multimodal benchmark curation, 69% item reduction with high fidelity, Visual Logic weakness identified, Not aligned with user's continuous/generation focus
...
### [236] [CVSBench: A Comprehensive Benchmark for Cross-view Spatial Reasoning and Dreaming](https://arxiv.org/abs/2606.22476)
...
TL;DR: 本文提出CVSBench，一个大规模跨视角空间推理基准，通过卫星-街景图像对评估VLM的跨视角空间推理能力。基准包含3,297个图像组、9,468个目标级标注和40,679个QA对，支持跨视角VQA、跨视角定位和视角识别任务。实验表明现有VLM在视角剧烈变化下难以保持目标级和布局一致性，而引入3D场景想象管线可显著提升跨视角推理性能。


<details>
  <summary>Details</summary>
Motivation: 人类可以轻松地在不同视角间进行场景推理，但VLM是否具备类似的跨视角空间能力尚不清楚。卫星-街景对具有复杂上下文和极端视角变化，是理想的测试场景。现有基准缺乏对跨视角空间推理的系统评估，因此作者构建了CVSBench来填补这一空白。

Method: 构建了CVSBench基准数据集，包含3,297个跨视角图像组（卫星-街景对），9,468个目标级标注和40,679个QA对，支持跨视角VQA、跨视角定位和视角识别三类任务。在此基础上，评估了多种VLM，并探索了两类改进方法：(1) 空间接地推理（语言推理）；(2) 通过3D场景想象管线引入视觉空间想象，将卫星图像转换为3D场景再渲染为街景视角。

Result: 现有VLM在跨视角空间推理任务上表现不佳，尤其在目标级和布局一致性方面。仅依赖语言推理的改进效果有限，而引入3D场景想象管线（显式视觉空间表征）能显著提升跨视角推理性能，表明显式视觉-空间表征对鲁棒空间认知至关重要。

Conclusion: CVSBench揭示了当前VLM在跨视角空间推理方面的显著不足，证明了显式视觉-空间表征（如3D场景想象）对于实现类人空间认知的必要性。该基准为未来研究提供了系统评估平台。

Classification reason: 该论文提出了CVSBench，一个用于跨视角空间推理的大规模基准数据集，包含跨视角VQA、跨视角定位和视角识别等任务，属于多模态评测与基准方向。该论文与[多模态评测与基准]（multimodal_benchmark）这一CANONICAL subtopic高度匹配。

Importance reason: Strong benchmark for cross-view spatial reasoning but not aligned with user's priority subtopics on continuous models or generation. Signals: cross-view spatial reasoning benchmark, satellite-street VLM evaluation, 3D scene imagination pipeline, multimodal understanding, not in priority subtopics

Abstract: Humans can effortlessly reason about scenes across different viewpoints, yet it remains unclear whether Vision-Language Models (VLMs) possess similar cross-view spatial abilities. Satellite-street scene pairs, with their complex contexts and extreme viewpoint variations, provide an ideal testbed. Motivated by this, we introduce CVSBench, a large-scale benchmark for evaluating cross-view spatial reasoning through satellite-street pairs. This benchmark supports multiple tasks, including cross-view VQA, cross-view grounding, and viewpoint identification. CVSBench comprises 3,297 cross-view image groups with 9,468 object-level annotations and 40,679 question-answer (QA) pairs, enabling systematic and controlled evaluation of cross-view spatial reasoning. Extensive evaluations reveal that advanced VLMs struggle to maintain object-level and layout consistency under drastic viewpoint changes. To bridge this gap towards human-like spatial cognition, we investigate two categories of approaches: spatially grounded reasoning and the incorporation of cognitive map inputs. Our findings demonstrate that language-only reasoning yields marginal improvements, while incorporating visual spatial imagination via a 3D scene imagination pipeline substantially improves cross-view reasoning. These results highlight the necessity of explicit visual-spatial representations for robust spatial cognition in VLMs. Our data and code are released at .

</details>


### [237] [Benchmarking Vision-Language Models for Microscopic Plant Image Understanding](https://arxiv.org/abs/2606.22497)
...
TL;DR: 本文提出了 PlantMicro，一个专门用于评估视觉语言模型在植物显微图像理解能力的综合基准，包含 5000+ 图像和 9000+ VQA 对。实验表明当前 VLM 在细粒度识别和生物学推理上表现不佳，GPT-5 在病原分类任务上仅 34.93% 准确率，揭示了显微级植物图像理解方面的显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 基准主要关注宏观植物图像，而显微级别的植物图像理解领域尚未被充分探索。植物显微成像对于研究细胞和亚细胞水平的植物生物学和病理学至关重要，因此需要专门的基准来评估和推动 VLM 在该领域的能力。

Method: 构建 PlantMicro 基准，收集超过 5000 张来自不同宿主、生物领域和成像模态的显微图像，设计互补任务（如病原分类、细胞结构识别等），构建超过 9000 个 VQA 对，系统评估当前 VLM 在显微植物图像理解方面的能力。

Result: 当前 VLM 在 PlantMicro 上表现不佳，GPT-5 在病原分类任务上仅 34.93% 准确率，仅略高于随机猜测基线，表明现有 VLM 在细粒度识别和生物学推理方面存在显著不足。

Conclusion: PlantMicro 为推进 VLM 实现可靠且全面的显微级植物图像理解提供了标准化基础，揭示了当前模型在该领域的能力差距，为未来研究指明了方向。

Classification reason: 该论文提出了 PlantMicro，一个用于评估 VLM 在植物显微图像理解能力的基准，包含 5000+ 图像和 9000+ VQA 对。这属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题，直接匹配。

Importance reason: Focuses on a narrow plant microscopy benchmark, not aligned with user's priority on continuous multimodal models or video generation. Signals: Plant microscopy benchmark, VLM evaluation, fine-grained recognition gap, low GPT-5 accuracy, not in priority subtopics
...
### [238] [MapReason-OSM: Can Vision-Language Models Make Graph-Verifiable Mobility Decisions from Street Maps ?](https://arxiv.org/abs/2606.22597)
...
TL;DR: 本文提出 MapReason-OSM，一个基于 OpenStreetMap 的基准和评测框架，用于评估视觉语言模型在街道地图上做出图可验证的移动决策（如路径规划、设施选址等）的能力。包含 6000 个实例、12 种任务，从有效性、合法性、最优性、约束满足和跨缩放一致性五个维度进行评测。实验表明，VLM 能简单读图和路径规划，但在图代价推理和跨缩放一致性上表现很差。


<details>
  <summary>Details</summary>
Motivation: 现有的地图评测基准大多依赖自由文本或多选题答案，无法对底层道路图进行可验证的评估。而实际物流、配送、无障碍导航等场景中，模型输出的决策（如路线、停车点）必须严格遵循道路网络约束。因此需要一个能够对 VLM 的地图决策进行图可验证评估的基准。

Method: 作者从 OpenStreetMap 渲染固定风格的地图面板，覆盖美国十个城市中心区，使用两种对齐缩放尺度，叠加一致的标记语法。每张面板配以隐藏的街道图和精确的 oracle，构建了 6000 个实例（两个缩放尺度共 12000 张面板），涵盖 12 种路由、设施选址和视觉消歧任务。模型输出结构化决策，通过图回退（snap back）进行有效性、合法性、最优性、约束满足和跨缩放一致性评分。

Result: 在 7 个 VLM 上的实验表明：模型能简单读图和路径规划，但在图代价推理任务（如单设施选址）上表现极差，即使是前沿推理模型也接近随机水平；模型经常出现跨缩放不一致的问题。

Conclusion: 当前 VLM 在地图阅读和简单路径规划方面有一定能力，但在需要图代价推理和跨缩放一致性的复杂移动决策任务上存在显著不足。作者开源了基准、评测框架和确定性生成器，为未来研究提供了标准化的评估工具。

Classification reason: 该论文提出了 MapReason-OSM，一个用于评估 VLM 在地图场景下进行图可验证移动决策能力的基准和评测框架，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准"。

Importance reason: Provides a rigorous graph-verifiable benchmark for VLM map reasoning, but the subtopic is a low-priority evaluation benchmark, not a core direction. Signals: VLM map reasoning benchmark, graph-verifiable evaluation, cross-zoom consistency, poor graph-cost reasoning, low priority subtopic
...
### [239] [Each Judge Its Own Yardstick: Discovering Per-VLM Taxonomies for Physical Video Evaluation](https://arxiv.org/abs/2606.22918)
*Yu Cao, Ziquan Liu, Zhensong Zhang, Jiankang Deng, Shaogang Gong, Jifei Song*

Main category: cs.CV, cs.GT

Direction: 多模态理解与生成

Subtopic: 多模态评测与基准

Importance: 高优先级 (85.63) - 未入选精读

TL;DR: JudgeFit 提出了一种为每个 VLM 自动发现其自身物理视频评估维度的迭代优化方法，通过初始聚类构建 taxonomy、诊断校准、LLM 修复的循环流程，为 16 个 VLM 定制个性化评测体系，平均相对提升约 32%，并揭示了模型家族特有的盲区。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用统一的全局评测标准来评估 VLM 对视频物理一致性的判断能力，但不同 VLM 在训练数据和架构上差异巨大，编码物理现象的内部表征各不相同。统一的评测轴无法反映每个 VLM 实际能感知到的物理维度，导致评估不准确、盲区被掩盖。

Method: JudgeFit 包含三个步骤：(1) 初始 taxonomy 构建——在少量视频上提示目标 VLM 列举物理错误，对描述进行聚类得到初始维度；(2) 诊断校准——将 VLM 各维度评分与人类物理常识评分对齐，诊断不可靠或冗余的维度；(3) LLM 修复——用 LLM 根据诊断结果修复 taxonomy，迭代直至收敛。最终为每个 VLM 生成个性化的评测 taxonomy。

Result: 在 16 个 VLM（覆盖 8 个模型家族）上的实验表明，JudgeFit 的个性化 taxonomy 在所有 VLM 上均优于全局统一评测基线，平均相对提升约 32%。此外，per-VLM 评测画像揭示了模型家族特有的盲区和可靠性模式，这是整体排名无法反映的。

Conclusion: JudgeFit 证明了为每个 VLM 定制评测 taxonomy 的有效性，能够更准确地评估 VLM 在物理视频一致性判断上的能力，并暴露模型特定的盲区。该方法为视频生成和世界模型的自动化评估提供了更精细、更可靠的工具。

Classification reason: 该论文提出 JudgeFit，一种为每个 VLM 发现其自身评测维度的迭代优化方法，用于物理一致性视频评估。核心贡献是构建了一个新的多模态评测基准（per-VLM taxonomy），属于多模态评测与基准（multimodal_benchmark）这一规范主题。同时涉及视频生成评测，也可匹配 multimodal_generation 下的 generation_benchmark。

Importance reason: Proposes per-VLM evaluation taxonomies for physical video understanding, relevant to multimodal understanding but not a core priority subtopic. Signals: VLM evaluation, physical video understanding, per-model taxonomy, iterative refinement, diagnostic calibration
...
### [240] [Data Selection Through Iterative Self-Filtering for Vision-Language Settings](https://arxiv.org/abs/2606.23611)
...
TL;DR: 本文提出一种自举式数据筛选方法（Self-Filtering），通过迭代训练CLIP模型并基于模型自身预测动态选择高质量、多样化的训练数据，无需额外数据或预训练模型即可提升视觉语言模型的下游性能。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言数据集通常包含大量噪声，人工清洗成本过高。现有数据筛选方法依赖启发式规则、参考数据集或预训练模型，这些方法要么不够灵活，要么需要额外资源。本文旨在提出一种无需外部监督的自举式数据筛选方法。

Method: 提出Self-Filtering方法：在每次迭代中，使用当前CLIP模型对训练数据进行评分，选择高置信度（高概率）样本作为"干净"数据，同时保留一部分低置信度但多样性的样本以维持分布覆盖。通过迭代训练和重新选择数据混合，逐步提升数据质量和模型性能。

Result: 在多个视觉语言下游任务上，使用Self-Filtering筛选数据训练的CLIP模型相比使用原始噪声数据训练的模型取得了显著的性能提升，且优于基于启发式规则和参考数据集的筛选方法。

Conclusion: Self-Filtering是一种有效且无需外部资源的自举式数据筛选方法，能够自动提升视觉语言训练数据的质量，从而改善下游任务性能。该方法为大规模多模态数据清洗提供了新的思路。

Classification reason: 该论文提出了一种自举式的数据筛选方法（Self-Filtering），用于提升视觉语言模型（CLIP）的训练数据质量，并通过下游任务评测验证效果。虽然论文核心是数据筛选方法，但其应用场景和评测对象是视觉语言模型，且涉及数据质量对模型性能的影响评估，与多模态评测与基准（multimodal_benchmark）方向相关——该方向涵盖评测协议、数据质量评估等。同时论文也涉及多模态对齐（CLIP训练），但更偏向数据筛选与评测验证，故归入multimodal_benchmark。

Importance reason: Data selection method for vision-language models, but not aligned with priority subtopics like continuous models or generation. Signals: Self-filtering data selection, CLIP training, no extra data needed, improves downstream performance, vision-language setting
...
### [241] [FlowTrain: Flow-Based Decoupled Training for Industrial-Grade Vision-Language Models](https://arxiv.org/abs/2606.23087)
...
TL;DR: FlowTrain 提出了一种基于数据流的解耦训练框架，通过统一内存池实现视觉编码器与语言骨干的异步执行，并引入异构并行分配器和动态打包调度器，将 VLM 训练效率提升至 50%+ MFU 和 1.7x 吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 工业级 VLM 分布式训练效率远低于单模态 LLM，现有方案（单体式统一并行或分离式批同步流水线）仍不够高效，需要进一步解耦异构模块的训练过程。

Method: FlowTrain 将 VLM 训练重构为生产者-消费者数据流，通过统一内存池协调；引入异构并行分配器，通过求解吞吐匹配问题为各模块分配特定并行策略；设计动态打包调度器，根据 LLM 端实际计算代价在运行时构建平衡的微批次。

Result: 在真实工作负载上，FlowTrain 实现了超过 50% 的 MFU 和最高 1.7x 的吞吐量提升，缩小了与纯 LLM 训练的效率差距。

Conclusion: FlowTrain 通过解耦 VLM 异构模块的训练执行，显著提升了工业级分布式训练效率，为大规模多模态模型的高效训练提供了新范式。

Classification reason: 该论文关注 VLM 分布式训练效率优化，属于多模态模型高效化训练范畴，与 [CANONICAL] 多模态高效化与轻量化（efficient_multimodal）方向匹配。

Importance reason: Directly addresses efficient VLM training, a key bottleneck for unified multimodal models, with strong practical gains. Signals: VLM training efficiency, decoupled training framework, heterogeneous parallelism, dynamic packing, 1.7x throughput improvement
...
### [242] [Kamera: Unified Position-Invariant Multimodal KV Cache for Training-Free Reuse](https://arxiv.org/abs/2606.23581)
...
TL;DR: Kamera提出了一种无需训练的位置无关多模态KV缓存复用方法，通过低秩条件补丁修复跨块绑定损失，支持重排序、滑动窗口存活和召回三种窗口操作，在MM-NIAH和文档QA等基准上恢复完整任务精度，同时大幅降低KV存储开销。


<details>
  <summary>Details</summary>
Motivation: 多模态agent在滑动窗口和迭代推理中反复重新编码相同的视频帧、UI截图等视觉内容，现有前缀缓存仅在固定位置支持复用，导致大量重复计算。作者发现朴素KV复用会丢失跨块条件绑定（cross-chunk conditioning），导致多跳推理准确率减半，因此需要一种位置无关且能保留跨块绑定的高效缓存复用方案。

Method: Kamera的核心是：(1) 精确的RoPE重旋转（re-rotation）到任意目标位置，支持MLA、GQA和MHA三种注意力架构；(2) 一个训练免费的低秩条件补丁（low-rank conditioning patch），存储在每个位置无关的块旁，用于恢复跨块绑定信息。基于此实现三种窗口操作：重排序（一个补丁服务缓存集的所有排序）、滑动窗口存活（通过旋转而非重编码迁移块）、召回（被驱逐的块通过补丁恢复而无需重编码）。

Result: 在MM-NIAH跨块绑定基准上，rank-m补丁恢复完整任务精度；在两页文档QA上同样有效；在SGLang内核中跨6个骨干网络重建重填充KV达到bf16舍入精度。视觉和视频流中的条件信号最强，使得该方法在多模态agent中效果最为显著。

Conclusion: Kamera证明了多模态agent中重复视觉编码是可以避免的，通过位置无关的KV缓存复用加低秩条件补丁，可以在不训练的情况下显著减少计算开销，同时保持多跳推理精度。该方法对多模态agent的高效推理具有重要实用价值。

Classification reason: 该论文提出了一种无需训练的多模态KV缓存复用方法，通过低秩条件补丁实现位置无关的KV缓存重用，显著减少多模态agent在重复编码上的计算开销。这属于多模态模型的高效推理优化，与[多模态高效化与轻量化]（efficient_multimodal）高度匹配。

Importance reason: Directly addresses multimodal KV cache reuse inefficiency with a training-free patch, aligning with user's priority on model efficiency and unified multimodal models. Signals: Training-free KV cache reuse, Position-invariant multimodal cache, Low-rank conditioning patch, Cross-chunk binding recovery, Reduces recompute for video/UI agents
...
### [243] [FirstPass: Grounding AI Scientific Judgment in Multi-Round Editorial Outcomes](https://arxiv.org/abs/2606.20769)
...
TL;DR: FirstPass是一个基于Nature Communications多轮同行评审对话的数据集和微调模型，通过LoRA微调Qwen2.5-7B实现评审生成、评审更新和修订周期预测三项任务。关键发现是响应-only损失掩码是必要条件，使编辑结果预测准确率达到80.5%，显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有AI同行评审系统存在三个缺陷：仅训练于计算机科学和机器学习领域、忽略验证科学的迭代对话、以及基于风格模仿而非真实编辑判断进行评估。作者旨在构建一个跨学科、基于真实多轮编辑对话的AI评审系统。

Method: 从Nature Communications收集3,668个完整的多轮同行评审对话（覆盖生物学、化学、神经科学、物理学和地球科学五个领域），利用2022年11月实施的强制性透明同行评审政策，并通过自动化审计验证100%内容完整性。使用LoRA微调Qwen2.5-7B-Instruct，在三个任务上训练：评审生成、评审更新和修订周期预测。关键技术创新是响应-only损失掩码（response-only loss masking）。

Result: 在编辑结果预测（标准vs扩展修订周期）上达到80.5%准确率和78.2% F1-macro，比Gemini-3.1-flash-lite-preview零样本高出10.4个百分点，所有结果具有统计显著性（McNemar p < 0.001）。生成的评审平均1,187词，远优于基线，ROUGE-L达0.154。

Conclusion: FirstPass证明了响应-only损失掩码对于AI同行评审任务的关键性，并展示了跨学科泛化能力。该模型可作为预提交环节的预期性科学合著者，模拟专家评审并预测修订周期结果。

Classification reason: 该论文提出了FirstPass数据集（多轮同行评审对话）和微调模型，核心贡献在于构建了一个新的评测基准与数据集来评估AI的同行评审能力。这与[multimodal_understanding_generation]方向下的[generation_benchmark]（生成模型评测与数据集）高度匹配，因为论文主要贡献是数据集构建和评测协议。

Importance reason: Focuses on peer review dataset and editorial prediction, not on the user's priority subtopics like continuous language models or video generation. Signals: peer review dataset, editorial outcome prediction, response-only loss masking, cross-domain evaluation, not aligned with priority subtopics
...
### [244] [DataClaw0: Agentic Tailoring Multimodal Data from Raw Streams](https://arxiv.org/abs/2606.21337)
...
TL;DR: 本文提出 DataClaw0，一种基于智能体数据精炼（Agentic Data Tailoring）范式的多模态数据处理模型。通过两阶段流水线生成大规模训练数据，结合 SFT 和 GRPO 训练 9B 模型，并构建首个数据精炼评测基准 DataClaw0-val。实验表明其生成的高信息密度数据能有效提升下游任务（视频生成、VQA、GUI导航）在有限数据下的模型适应能力。


<details>
  <summary>Details</summary>
Motivation: 原始多模态数据流存在高熵、高噪声问题，传统被动标注范式依赖启发式规则或通用 VLM，成本高、单调且无法挖掘数据中的深层过程逻辑。需要一种可学习的、主动的数据精炼能力来替代被动标注。

Method: 提出 Agentic Data Tailoring 范式，将数据处理提升为可学习能力。设计两阶段流水线：基于确定性事实锚点（Factual Anchors）的生成式语义合成，构建覆盖五个核心物理与数字领域的大规模数据集。使用 DataClaw0-9B 模型，结合 SFT 和 GRPO 进行训练，实现与复杂精炼意图的对齐。同时构建首个数据精炼评测基准 DataClaw0-val。

Result: 在视频生成、真实世界 VQA 和 GUI 导航三个下游任务上验证，DataClaw0 生成的高信息密度精炼数据能在有限训练数据条件下有效促进模型对新任务的适应。

Conclusion: 将数据精炼提升为可学习能力是可行的，Agentic Data Tailoring 范式能有效降低数据熵、提升下游任务性能，为多模态数据处理提供了新方向。

Classification reason: 该论文提出了 DataClaw0-val，据我们所知是第一个专门用于数据精炼（data refinement）的评测基准，并构建了大规模多模态数据集用于训练数据精炼能力。这属于生成模型评测与数据集（generation_benchmark）范畴，该 canonical subtopic 涵盖新数据集和评测基准，因此复用该分类。

Importance reason: Novel agentic data tailoring paradigm but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: Agentic data tailoring, Multimodal data refinement, SFT+GRPO training, DataClaw0-val benchmark, Downstream task improvement

Abstract: Massive unstructured multimodal streams suffer from high "data entropy," impeding both efficient human knowledge acquisition and high-quality AI post-training. Existing passive annotation paradigms, heavily reliant on heuristic rules or general VLMs, are costly, monotonous, and fail to unlock the deep procedural logic embedded in raw data. We elevate data processing to a learnable capability, proposing a paradigm shift towards Agentic Data Tailoring, which actively refining and structuring data to align with diverse user and downstream intents. To overcome the data scarcity bottleneck in training such high-order capabilities, we design a two-stage pipeline grounding generative semantic synthesis in deterministic Factual Anchors, yielding a large-scale dataset spanning five core physical and digital domains. Building upon this, $\text{DataClaw}_0$-9B model synergizes Supervised Fine-Tuning (SFT) with Group Relative Policy Optimization (GRPO), achieving robust alignment with complex refinement and tailoring intents. To systematically quantify this capability, we construct $\text{DataClaw}_0$-val, the first benchmark dedicated to data refinement. Crucially, we adopt downstream post-training as the ultimate validation touchstone. Evaluations on video generation, real-world VQA, and GUI navigation confirm that $\text{DataClaw}_0$ delivers high-information-density tailored data, facilitating efficient model adaptation to new tasks under limited training data regimes. Project page:

</details>


### [245] [CapRiCorn-1K: A Comprehensive Benchmark for Video Captioning and Subject Referential Consistency Across Temporal Scales](https://arxiv.org/abs/2606.21949)
...
TL;DR: 本文提出 CapRiCorn-1K，一个用于评估视频字幕生成质量和主体指代一致性的综合基准，覆盖多种时长和场景，支持音视频和纯视觉两种设置。实验表明现有模型在长视频上字幕质量和指代一致性均下降，且评测指标与下游理解/生成任务性能高度相关。


<details>
  <summary>Details</summary>
Motivation: 现有视频字幕评测基准缺乏对长时序下主体指代一致性的客观全面评估，难以推动视频字幕模型的发展。

Method: 构建 CapRiCorn-1K 基准，包含多种时长和场景的视频数据，设计评估字幕质量和主体指代一致性的指标，支持音视频和纯视觉两种评测设置。

Result: 现有模型在生成准确全面字幕和保持主体指代一致性方面普遍表现不佳；随着视频时长增加，字幕质量和指代一致性均下降；所提评测指标与下游理解/生成任务性能高度相关。

Conclusion: CapRiCorn-1K 能有效评估视频字幕生成与主体指代一致性，揭示了现有模型的不足，并为未来研究提供了有价值的评测工具。

Classification reason: 该论文提出了视频字幕生成与主体指代一致性的评测基准 CapRiCorn-1K，属于多模态理解与生成方向的评测工作。同时，该基准也评估生成字幕对下游生成任务的影响，因此也匹配多模态生成方向。最佳匹配的现有 subtopic 是 [CANONICAL] 的 generation_benchmark（生成模型评测与数据集），以及 multimodal_benchmark（多模态评测与基准）。考虑到论文核心是视频字幕评测基准，且同时涉及生成任务评估，选择 generation_benchmark 作为主 subtopic。

Importance reason: Benchmark for video captioning consistency, but not directly aligned with user's priority on generative models or continuous representations. Signals: video captioning benchmark, subject referential consistency, temporal scales, audiovisual evaluation, correlation with downstream tasks

Abstract: Accurate and comprehensive video captions with consistent subject references are critical for downstream understanding and generation tasks. However, few existing benchmarks can objectively and comprehensively evaluate these properties across diverse durations and scenarios, thereby hindering the advancement of video captioning models. To bridge this gap, we propose CapRiCorn-1K, a comprehensive benchmark designed to evaluate both video captioning quality and subject referential consistency across long temporal horizons and diverse video domains. To accommodate varied evaluation needs, our benchmark supports both audiovisual and visual-only settings. Extensive experiments on CapRiCorn-1K reveal that current models generally struggle to generate accurate and comprehensive captions while maintaining consistent subject references. Moreover, as video duration increases, both the overall caption quality and subject referential consistency decline. Notably, our evaluation metrics exhibit strong correlations with the performance of downstream understanding and generation tasks conditioned on the generated captions, further validating their effectiveness. The project is available at .

</details>


### [246] [Litmus: Zero-Label, Code-Driven Metric Specification for Evaluating AI Systems](https://arxiv.org/abs/2606.23403)
...
TL;DR: Litmus 是一个零标注、代码驱动的评估指标自动设计系统，通过分析源代码和定向询问来识别AI流水线需要评估的内容和原因，自动构建合理的指标组合。在三个真实AI流水线上，Litmus 在覆盖范围、冗余度和有效性方面均优于 AutoMetrics 和 DynamicRubric 基线。


<details>
  <summary>Details</summary>
Motivation: 随着智能体LLM系统从原型走向部署，评估变得越来越重要且困难。现有评估方法往往隐含评估目标，指标选择难以证明、解释或验证。需要一种系统化的方法来自动识别需要评估什么以及为什么评估，从而构建合理的指标组合。

Method: Litmus 通过分析源代码和定向询问来提取评估意图，将评估目标转化为约束条件，为流水线的每个阶段构建合理的指标组合。它不需要任何标注数据，完全基于代码驱动。

Result: 在金融账户分组、科学QA和固有风险评估三个真实AI流水线上，Litmus 实现了最广或并列最广的关注覆盖，覆盖更多流水线阶段，产生近乎零冗余的指标组合，并且在所有三个流水线上与逐行质量标签的有效性排名中位列第一——在科学QA上显著领先（Spearman ρ=0.72，所有基线均低于0.47）。

Conclusion: 研究支持从自动指标实现向自动指标规范的转变：在询问计算哪个指标之前，评估系统应该先问需要测量什么以及为什么。

Classification reason: 该论文提出Litmus系统，通过代码驱动的方式自动设计AI系统的评估指标，属于评估方法论。虽然论文不限于生成模型，但其核心是自动指标规范与评测，与"生成模型评测与数据集"（generation_benchmark）这一[CANONICAL]主题高度相关，且该主题已涵盖评测基准、自动评测指标等广义评估内容。

Importance reason: Focuses on evaluation metric design for AI pipelines, not on core generative model or multimodal architecture advances. Signals: zero-label metric specification, code-driven evaluation, not generative model, not multimodal, not autoregressive
...
### [247] [T-IMPACT: A Severity-Aware Benchmark for Contextual Image-Text Manipulation](https://arxiv.org/abs/2606.22339)
...
TL;DR: T-IMPACT 是一个面向新闻风格图像-文本对篡改的严重性感知基准，包含 98,786 个样本，覆盖原始、仅图像篡改、仅文本篡改和联合篡改四种类型，提供校准的连续严重性分数和粗粒度低/中/高标签。实验表明现有模型能恢复部分真实性信号，但严重性预测仍远弱于人类判断。


<details>
  <summary>Details</summary>
Motivation: 现有多模态篡改数据集主要关注真实性二分类、上下文不匹配或篡改类型，缺乏对编辑操作如何改变帖子语义解释程度的量化度量。作者希望推动多模态篡改研究从二元真伪分类走向分级上下文影响评估。

Method: 构建了一个自动化流水线：从新闻图像-文本对中提取语义锚点并进行空间定位，执行局部图像编辑和受限字幕重写，利用有限的人类评分校准上下文影响分数。数据集包含 98,786 个样本，覆盖原始、仅图像篡改、仅文本篡改和联合篡改四种类型，提供校准的连续严重性分数和粗粒度低/中/高标签。

Result: 现有模型在真实性检测上能恢复部分信号，但严重性预测任务显著更难，与人类判断的对齐度较弱。这表明当前模型缺乏对多模态篡改上下文影响的细粒度理解能力。

Conclusion: T-IMPACT 为研究超越二元真伪分类的多模态篡改检测提供了初始基准，揭示了严重性预测这一更具挑战性的研究方向，呼吁社区关注分级上下文影响评估。

Classification reason: 该论文提出了一个面向多模态篡改（图像-文本联合编辑）的严重性感知评测基准 T-IMPACT，属于多模态生成内容检测与评测范畴。它既匹配 multimodal_understanding_generation 下的 [CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark），也匹配 multimodal_generation 下的同名 subtopic。该基准关注编辑操作的上下文影响严重性，而非传统二分类真伪检测，是典型的生成模型评测工作。

Importance reason: The paper focuses on a benchmark for multimodal misinformation severity, which is not a priority subtopic and lacks direct connection to the user's core interests in continuous models, generation, or world models. Signals: Multimodal misinformation benchmark, Severity-aware dataset, Not aligned with priority subtopics, No autoregressive or diffusion generation focus, Weak personal relevance due to negative keywords absence
...
### [248] [OmniSpace: Efficient Geometry Awareness for Autonomous Vehicles MLLMs](https://arxiv.org/abs/2606.22617)
...
TL;DR: OmniSpace 是一种即插即用的范式，通过相机位姿注入、多视图极线注意力和3D几何蒸馏，在不依赖辅助3D模型的情况下，提升自动驾驶 MLLM 的几何感知与空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有几何感知 MLLM 通常依赖推理时的辅助3D模型，导致流水线复杂且存在级联失败风险。作者发现当前 MLLM 的瓶颈在于弱跨视图对应关系和深度估计不足，因此希望仅从2D观测中实现高效的几何感知空间推理。

Method: OmniSpace 包含三个核心模块：(1) Camera Pose Injector，将相机位姿信息注入 MLLM；(2) Multi-view Epipolar Attention，利用极线几何增强跨视图对应关系；(3) 3D Geometric Distillation 目标，通过蒸馏将3D几何知识迁移到模型中。整体为即插即用范式，无需推理时调用外部3D模型。

Result: 在 nuScenes、Bench2Drive（规划）、nuInstruct（风险检测）、Omnidrive（语言）和 DriveBench（泛化）等多个自动驾驶基准上，OmniSpace 超越了现有方法。

Conclusion: OmniSpace 证明了通过精心设计的几何感知模块，可以在纯2D观测下有效提升 MLLM 的空间推理能力，为自动驾驶场景下的几何感知 MLLM 提供了一种简洁高效的解决方案。

Classification reason: 该论文聚焦于自动驾驶场景下 MLLM 的几何感知与空间推理能力，通过相机位姿注入、多视图极线注意力与3D几何蒸馏提升空间智能，属于多模态理解与生成方向下的「空间与 3D 多模态推理」[CANONICAL] 子主题。

Importance reason: Focuses on autonomous driving MLLM spatial reasoning, which is not a priority subtopic and lacks direct connection to continuous language/multimodal models or video generation. Signals: Autonomous driving MLLM, geometry-aware spatial reasoning, camera pose injection, multi-view epipolar attention, 3D geometric distillation
...
### [249] [SATURN: Symbolic Spatial Reasoning for Multi-Perspective Grounding](https://arxiv.org/abs/2606.22694)
*Danial Kamali, Tanawan Premsri, Shreya Rajpal, Amir Zadeh, Chuan Li, Parisa Kordjamshidi*

Main category: cs.CV, cs.SC
...
TL;DR: SATURN 是一个神经符号框架，通过重建近似 3D 场景、推导软性视角感知空间谓词，并使用无训练的 Python 符号执行器进行组合推理，实现了视角感知的组合空间推理。同时提出了 3D FORCE 诊断基准来评估推理深度和视角复杂度。在 3D FORCE 和真实世界 MindCube 基准上，SATURN 显著优于 VLM 基线。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 在需要组合关系推理（尤其是涉及参考框架的空间关系）时表现不可靠。现有的神经符号方法虽然使推理更显式，但通常依赖脆弱的几何过程和硬决策，无法很好地处理感知噪声。

Method: SATURN 包含三个步骤：(1) 从单张 RGB 图像重建近似 3D 场景；(2) 推导软性（soft）视角感知空间谓词，保留不确定性；(3) 使用无训练的 Pythonic 符号执行器进行多跳组合推理，将感知与推理分离。同时构建了 3D FORCE 诊断基准，控制推理深度、视角和视角组合。

Result: 在 3D FORCE 基准上，VLM 和空间训练模型随深度和视角复杂度增加性能急剧下降，而 SATURN 保持稳定并优于强基线。在真实世界 MindCube 基准上，SATURN 达到 78.57% 总体准确率，超过最强基线 14 个百分点。

Conclusion: 神经符号方法在视角感知组合空间推理任务上显著优于纯神经 VLM，尤其是在推理深度和视角复杂度增加时。将感知与推理分离并保留不确定性传递是提升空间推理可靠性的有效途径。

Classification reason: 该论文提出 SATURN，一个面向视角感知的组合空间推理的神经符号框架，涉及 3D 场景重建、空间谓词推导和指代表达定位。这属于多模态理解与生成方向下的"空间与 3D 多模态推理"（spatial_3d_reasoning）规范子主题，该子主题涵盖空间推理、3D 视觉定位和几何推理。

Importance reason: Strong neuro-symbolic spatial reasoning but outside user's priority subtopics and lacks boost keywords. Signals: neuro-symbolic framework, perspective-aware spatial reasoning, 3D scene reconstruction, symbolic executor, 3D FORCE benchmark
...
### [250] [Dense Reward for Multi-View 3D Reasoning with Global Maps and Local Views](https://arxiv.org/abs/2606.23557)
...
TL;DR: 本文提出DR-MV3D，一种基于地图锚定的密集奖励学习框架，用于多视图3D视觉问答。该方法将MV3D-VQA分解为全局地图构建、视角轨迹规划和局部定位回答三个步骤，并引入全局一致性奖励和局部轨迹奖励来提供过程级密集监督，通过GRPO优化整个流程。实验表明该方法在多个基准上优于强基线。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型在多视图3D VQA任务中通常仅使用答案级别的稀疏监督，导致跨视图推理不一致和视角选择脆弱。缺乏对中间推理步骤（如全局地图构建、视角轨迹规划）的显式监督是性能瓶颈。

Method: 将MV3D-VQA分解为三个可监督的中间步骤：(1) 全局地图构建（allocentric global map），(2) 问题条件化的视角轨迹规划，(3) 基于自我中心的定位回答。引入两种密集奖励：全局一致性奖励（利用冻结的3D视觉基础模型VGGT+SAM3生成几何一致的伪标签来对齐预测地图）和局部轨迹奖励（监督有序视角选择）。使用轨迹级策略优化（GRPO）端到端优化整个流程。

Result: 在MindCube、VSI-Bench和BLINK (MV)三个基准上，DR-MV3D一致优于强多图像基线方法，验证了过程级密集监督对多视图3D推理的有效性。

Conclusion: 过程级的密集奖励监督能够有效提升多视图3D VQA的推理质量，地图锚定的分解式学习框架为多步空间推理提供了可行的训练范式。

Classification reason: 该论文研究多视图3D视觉问答（MV3D-VQA），涉及3D场景推理、空间推理和多步空间推理过程监督，属于多模态理解与生成方向下的"空间与3D多模态推理"（spatial_3d_reasoning）这一[CANONICAL]子主题。

Importance reason: The paper focuses on 3D VQA with dense rewards, which is not a priority subtopic and lacks direct alignment with the user's key interests in continuous models, video/image generation, or world models. Signals: Multi-view 3D VQA, Dense reward learning, GRPO optimization, Global map construction, View-trajectory planning
...
### [251] [BioMatrix: Towards a Comprehensive Biological Foundation Model Spanning the Modality Matrix of Sequences, Structures, and Language](https://arxiv.org/abs/2606.22138)
...
Main category: q-bio.BM, cs.CL, cs.LG, cs.AI
...
TL;DR: BioMatrix 是首个原生整合分子序列、分子结构、蛋白质序列、蛋白质结构和自然语言五种模态的生物基础模型，采用统一离散 token 化方案和单一 decoder-only 架构，在 80 个下游任务中 77 个达到 SOTA 或可比性能。


<details>
  <summary>Details</summary>
Motivation: 现有生物基础模型要么在单一实体类型上融合多模态，要么覆盖多种实体类型但缺乏显式结构建模或依赖适配器设计（模型不能原生生成其可读取的模态）。BioMatrix 旨在弥合这一差距，实现真正的原生多模态统一。

Method: 基于 Qwen3 语言模型（1.7B 和 4B），将分子序列（SMILES/SELFIES）、分子结构、蛋白质序列、蛋白质结构和自然语言映射到共享离散 token 空间，采用统一 token 化方案，所有模态在单一 next-token prediction 目标下统一消费和生成，无需外部编码器、投影适配器或模态特定输出头。在 3044 亿 token 上持续预训练，涵盖通用和领域文本、分子和蛋白质的序列与结构视图、跨模态语料。

Result: 在涵盖 6 大类 80 个下游任务的综合评测中，BioMatrix 在 77 个任务上达到 SOTA 或可比性能，证明单一原生多模态通用模型可以在广泛的生物任务上匹配或超越专门方法。

Conclusion: BioMatrix 证明了在生物领域构建原生多模态统一基础模型的可行性，为跨序列、结构和语言的全模态理解与生成提供了有效范式。

Classification reason: 该论文提出了 BioMatrix，一个将分子序列、分子结构、蛋白质序列、蛋白质结构和自然语言统一到共享离散 token 空间的原生多模态基础模型，采用单一 decoder-only 架构和 next-token prediction 目标，无需外部编码器或适配器。这属于"统一多模态模型"（unified_multimodal_model）范畴，该 subtopic 在 multimodal_understanding_generation 方向下已有定义，且为 canonical 条目。

Importance reason: This paper focuses on biological foundation models, which are outside your configured priority subtopics and are explicitly penalized by your negative keywords. Signals: Biological domain, Unified multimodal for molecules/proteins, Strong benchmark results, Not in priority subtopics, Negative keyword match (molecule, protein)
...
### [252] [SPAR: Semantic-Pixel Self-Alignment and Adaptive Routing for Unified Multimodal Models](https://arxiv.org/abs/2606.23041)
...
TL;DR: SPAR 提出了一种语义-像素自对齐与自适应路由的统一多模态框架，通过非对称双流统一分词器（语义流+像素流）弥合语义感知与像素级重建之间的差距，并利用自对齐生成范式将分词器作为扩散模型的内部对齐教师，同时引入动态 token 路由实现灵活的多模态交互。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 在视觉理解上表现出色，但在视觉生成方面受限于语义感知与像素级重建之间的特征差异。现有方法通常依赖外部教师模型来弥合这一差距，导致系统复杂且效率低下。因此，需要一种无需外部依赖的统一多模态框架，同时具备理解与生成能力。

Method: 提出 SPAR 框架，包含三个核心组件：(1) 非对称双流统一分词器（Asymmetric Dual-Stream Unified Tokenizer），其中轻量级语义流锚定判别性特征，Transformer 增强的像素流恢复细粒度视觉细节，两者映射到统一的紧凑潜空间；(2) 自对齐生成范式（Self-Aligned Generation Paradigm），将优化后的分词器作为扩散模型的内部对齐教师，无需外部依赖；(3) 动态 token 路由（Dynamic Token Routing），使每个 token 能根据其语义需求自适应地聚合多层 MLLM 特征。

Result: 大量实验表明，SPAR 在统一架构中达到了最先进水平，在生成和重建质量上表现优异，同时保持了基础的视觉理解能力。

Conclusion: SPAR 通过语义-像素自对齐和自适应路由机制，成功弥合了语义感知与像素重建之间的鸿沟，为统一多模态理解与生成提供了一种无需外部教师的高效框架。

Classification reason: 该论文提出 SPAR 框架，旨在统一多模态理解与生成，属于"统一多模态模型"（unified_multimodal_model）这一现有 subtopic。该 subtopic 在 multimodal_understanding_generation 和 multimodal_generation 两个方向下均有定义，论文同时匹配这两个方向。

Importance reason: Directly addresses unified multimodal models with a novel self-alignment and routing method, highly relevant to your priority subtopics and boost keywords. Signals: unified multimodal model, self-aligned generation, dynamic token routing, asymmetric dual-stream tokenizer, state-of-the-art on unified architectures
...
### [253] [VTOS: Learning to Orchestrate Vision Tools by Co-Searching Solutions and Observers](https://arxiv.org/abs/2606.20728)
...
TL;DR: VTOS 提出一种联合搜索解决方案与观察者（observer）的框架，通过视觉编程自适应编排 Grounding DINO、SAM 等视觉基础工具，并在密集目标计数和零样本植物病害分割任务上超越静态工具流水线和现有视觉编程基线。


<details>
  <summary>Details</summary>
Motivation: 现有视觉编程方法生成固定的解决方案流水线，在密集目标、遮挡、小目标和域迁移等挑战性场景下表现脆弱。需要一种能够自适应诊断失败模式并动态调整工具编排的策略。

Method: VTOS 框架联合搜索两类程序：解决方案程序（组合视觉工具如 Grounding DINO、SAM、NMS、slice-and-detect 等）和观察者程序（诊断候选方案、识别失败模式并生成可操作反馈）。观察结果累积到共享的 VisionThoughts 知识库中，指导后续搜索。

Result: 在 LVIS-Count 密集目标计数和 PlantSeg-OOD 零样本植物病害分割两个任务上，VTOS 均优于静态工具流水线和现有 agentic 视觉编程基线方法。

Conclusion: 联合搜索解决方案与观察者是一种有效的策略，能够使视觉工具自适应地适应具有挑战性的计算机视觉任务。

Classification reason: 该论文提出 VTOS 框架，通过视觉编程（visual programming）组合 Grounding DINO、SAM 等视觉基础工具来解决视觉任务，属于视觉语言模型/智能体（VLM agent）在工具编排方面的应用，与 canonical 子主题 vlm_mllm_general（视觉语言模型与 MLLM）相关。

Importance reason: Proposes adaptive vision tool orchestration but focuses on static tool pipelines, not the user's priority of continuous multimodal models or autoregressive generation. Signals: visual programming, tool orchestration, dense object counting, zero-shot segmentation, not continuous language/multimodal
...
### [254] [FleetAgent: Teleoperation Assistant for Autonomous Fleets via Vectorized V2N Messages](https://arxiv.org/abs/2606.21222)
...
TL;DR: FleetAgent 是一个基于多模态大语言模型的云端远程操作助手，通过紧凑的结构化 V2N 消息（而非原始传感器数据）为自动驾驶车队提供场景描述、规划评估和干预优先级评分，显著降低上行链路负载和 KV-cache 内存消耗。


<details>
  <summary>Details</summary>
Motivation: 大规模自动驾驶车队依赖远程操作处理罕见故障，但传输大量车辆的原始传感器数据成本高昂，且远程操作员同时只能监控有限数量的车辆。现有方法缺乏高效、可解释的远程监控方案。

Method: 提出 FleetAgent 系统，包含：(1) VecFormer，一个向量到嵌入的接口模块，通过可微分的 Top-K 上下文选择将结构化 V2N 消息转换为 MLLM 兼容的 token 嵌入，控制上下文长度和 KV-cache 增长；(2) 基于 Qwen2.5-VL-7B 的 MLLM，生成结构化自然语言响应（叙述、解释、评估）和干预紧急度评分；(3) VecEval 数据集，基于 nuScenes 构建，包含人工和合成的不完美规划及人工验证的语言标注。

Result: 与原始图像相比，上行链路负载降低最多 625 倍；与原始文本描述相比，KV-cache 内存减少 16.54 倍。在 VecEval 上，FleetAgent 相比使用语言描述的 Qwen2.5-VL-7B，Lingo-Judge 评分提升 16.8%，干预失败率降低 19.9%。

Conclusion: FleetAgent 通过紧凑的结构化 V2N 消息实现了高效、可解释的自动驾驶车队远程监控，验证了 MLLM 在自动驾驶远程操作场景中的可行性和优势。

Classification reason: 该论文提出 FleetAgent，一个云端的 MLLM 助手，用于自动驾驶车队远程监控。它使用结构化 V2N 消息（地图元素、检测对象、规划路径）作为输入，生成自然语言响应和干预评分。这属于通用 MLLM 在特定应用场景（自动驾驶远程操作）中的应用，与 [CANONICAL] 的 "视觉语言模型与 MLLM" 子主题匹配。

Importance reason: Focuses on teleoperation for autonomous fleets, which is outside your core priority subtopics of continuous multimodal models, video/image generation, and world models. Signals: MLLM for autonomous driving, V2N structured messages, efficient cloud inference, VecFormer vector-to-embedding, VecEval dataset
...
### [255] [NAC: Neural Action Codec for Vision-Language-Action Models](https://arxiv.org/abs/2606.21372)
...
TL;DR: 本文提出 Neural Action Codec (NAC)，借鉴神经音频编解码器架构（多尺度 RVQGAN），将机器人短动作轨迹视为多通道 1D 信号进行压缩与离散 tokenization，为 VLA 模型提供高效、保真的动作 token 表示，在 LIBERO-10、RoboMimic 及真实操作任务中优于现有动作 tokenizer。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型依赖离散动作 tokenizer 来桥接连续机器人控制与自回归序列建模，但已有 tokenizer 在压缩率、延迟和下游性能之间存在权衡，缺乏高效且保真的动作 tokenization 方案。

Method: 借鉴神经音频编解码器（如 EnCodec）的架构，采用多尺度 RVQGAN（残差向量量化生成对抗网络）对短机器人动作轨迹进行编码；将动作轨迹视为多通道 1D 信号，使用时域损失和非梅尔谱重建损失替代音频专用的梅尔谱目标，配合 Vocos 风格解码器（ISTFT 头）和对抗判别器恢复平滑轨迹，并通过偏移码本（offset codebooks）生成紧凑有序的 token 空间。

Result: 在 LIBERO-10、RoboMimic 和真实操作任务上，NAC 在同等或更优压缩率下，重建误差和任务成功率均优于 binning、FAST 及基于 VQ 的 prior tokenizer。

Conclusion: 改造后的神经音频编解码器为 VLA 模型的动作 tokenization 提供了强大且实用的基础架构，验证了跨模态编解码器迁移的有效性。

Classification reason: 该论文提出 Neural Action Codec (NAC)，将机器人动作轨迹编码为离散 token 以用于 VLA 模型，属于视觉-语言-动作模型（VLA）的动作 tokenization 方法，与视觉语言模型（VLM/MLLM）架构紧密相关，因此归入 canonical 子主题 "视觉语言模型与 MLLM"。

Importance reason: Strongly relevant to continuous multimodal representation and VLA tokenization, but not directly on priority subtopics like continuous language models or video generation. Signals: neural audio codec for action tokenization, multi-scale RVQGAN, continuous action representation, VLA model frontend, high-fidelity trajectory compression
...
### [256] [GTA-Net: Cooperative Game Theory for Vision-Language Alignment in Chest X-Ray Report Generation](https://arxiv.org/abs/2606.21915)
...
TL;DR: GTA-Net 将胸部 X 光报告生成建模为合作博弈论对齐问题，通过 BinaryGameAligner（图像区域-文本 token 的 Shapley 重要性加权）和 Disease-Aware Ternary Aligner（图像-报告-疾病概念三元交互）实现显式的跨模态对齐，在 CheXpertPlus 和 IU-XRay 上取得 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在医学报告生成中依赖隐式注意力机制，无法保证显式的区域-词对应关系和疾病级别的一致性，导致临床可靠性不足。

Method: 提出 GTA-Net 框架：(1) BinaryGameAligner 使用基于相似度的支付矩阵和 Shapley 重要性加权建模图像区域与文本 token 的交互；(2) Disease-Aware Ternary Aligner 捕获图像、报告和结构化疾病概念之间的三元联合交互；(3) 采用 Swin Transformer 视觉编码器 + LoRA 适配的大语言模型，以统一目标函数联合训练生成与对齐。

Result: 在 CheXpertPlus 和 IU-XRay 数据集上，GTA-Net 在标准生成指标（BLEU、ROUGE、METEOR、CIDEr）上达到 SOTA，并展现出更好的临床一致性。

Conclusion: 显式的博弈论对齐方法能有效提升医学视觉语言生成中跨模态对应关系的准确性和临床可靠性，为医疗多模态生成提供了新思路。

Classification reason: 该论文提出 GTA-Net，一个用于胸部 X 光报告生成的视觉语言对齐框架，属于视觉语言模型（VLM）在医学领域的应用。它涉及图像-文本跨模态对齐与 LLM 结合，因此映射到 [CANONICAL] subtopic "视觉语言模型与 MLLM" (vlm_mllm_general)。同时，论文也涉及报告生成（多模态生成），因此匹配 multimodal_generation 方向。

Importance reason: Focuses on medical report generation, which is outside your priority subtopics and has negative keyword overlap. Signals: Medical domain, Cooperative game theory alignment, Chest X-ray report generation, Not in priority subtopics, Negative keyword "clinical" present
...
TL;DR: VideoAgent 是一个统一的智能体框架，通过自动镜头创建、跨模态检索和多智能体编排（30+专业编辑智能体），实现长视频理解与专业级视频编辑，在 VideoEdit 基准上达到 87-95% 的编排成功率，API 成本降低 60%，人工评估仅比人类创作视频低 4%。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑系统局限于短片段处理和特定领域任务，无法处理多样化的视频理解与编辑操作，且缺乏长视频理解能力以支持连贯叙事创作。

Method: VideoAgent 包含两大创新：(1) 自动视频镜头创建——通过镜头规划智能体实现连贯叙事，结合跨模态检索对齐视觉内容；(2) 多智能体编排框架——集成 30+ 专业编辑智能体，通过意图解析过滤相关工具，利用文本-梯度图优化组装复杂编辑流水线。

Result: 在提出的 VideoEdit 基准和公开数据集上，VideoAgent 优于现有多模态 LLM 和智能体系统，编排成功率达 87-95%，API 成本降低 60%。人工评估显示其生成内容接近人类水平，评分仅比人类创作视频低 4%。

Conclusion: VideoAgent 证明了基于多智能体编排的统一框架能够有效解决长视频理解与编辑的挑战，在专业性和效率上均取得显著优势。

Classification reason: 该论文提出 VideoAgent，一个面向视频理解与编辑的统一智能体框架，属于多模态理解与生成方向。其核心是多模态 LLM 驱动的视频理解与编辑智能体系统，与 [CANONICAL] subtopic "视觉语言模型与 MLLM"（vlm_mllm_general）高度相关。同时涉及视频编辑操作，也可匹配 multimodal_generation 下的 visual_editing，但主要贡献在于多模态智能体框架而非编辑方法本身。

Importance reason: Strong engineering for video editing but lacks novelty in core continuous/multimodal modeling or world model directions. Signals: unified video understanding and editing, multi-agent orchestration, long-video handling, 30+ specialized editing agents, VideoEdit benchmark
...
### [258] [From Driving Videos to Simulatable Scenarios](https://arxiv.org/abs/2606.21993)
...
TL;DR: 本文提出 D-V2S 框架，利用 VLM 从驾驶视频中提取道路布局和交通交互的自然语言描述，再通过 LLM 将这些描述转化为可执行的仿真驾驶场景，实现从真实视频到可仿真场景的自动生成。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆需要在可控、可重复、可扩展的仿真环境中测试各种驾驶场景（包括罕见事件），但手动创建仿真场景成本高、效率低。因此，需要一种能从真实驾驶视频自动生成可仿真场景的方法。

Method: D-V2S 包含两个阶段：(1) Driving Record Analyzer (DRA) 使用 VLM 和设计的提示词从输入视频生成自然语言描述，捕捉道路布局和动态交通交互；(2) Scenario Generator (SG) 使用 LLM 和条件上下文将这些描述转化为可执行的仿真场景。

Result: 仿真实验表明 D-V2S 生成的场景中 90% 的相关语义元素与视频一致；SG 相比其他 SOTA 方法获得了 75% 的偏好率；消融实验验证了 VLM 选择对 DRA 的重要性。

Conclusion: D-V2S 能够有效将真实驾驶视频自动转化为可仿真场景，为自动驾驶安全评估提供了高效、可扩展的场景生成方案。

Classification reason: 该论文使用 VLM 从驾驶视频生成自然语言描述，再通过 LLM 转化为可仿真场景，核心是利用 VLM 进行视觉理解与场景描述生成，属于通用视觉语言模型的应用，因此映射到 [CANONICAL] subtopic "vlm_mllm_general"。

Importance reason: Uses VLM for driving scenario generation, but is not aligned with priority subtopics like continuous models or autoregressive generation. Signals: VLM for driving, scenario generation, simulation, not priority subtopics, no continuous/autoregressive focus
...
### [259] [Video2Code: Generating Interactive Webpages from UI Videos via Action-Aware Revisit](https://arxiv.org/abs/2606.20711)
...
TL;DR: Video2Code 提出一种动作感知的视频到代码方法，通过先定位视频中动作关键区域再以高时间分辨率重访这些区域，从UI交互视频中恢复可执行的状态转换并生成HTML/CSS/JavaScript代码，显著提升了多步交互场景下的功能正确性。


<details>
  <summary>Details</summary>
Motivation: 现有视频视觉语言模型在处理UI视频到代码生成任务时，通常采用稀疏采样或压缩时序表示，容易遗漏短时动作边界，破坏状态-动作-状态转换的连续性，导致生成代码无法正确实现网页交互行为。作者将此问题定义为"状态转换未对齐"。

Method: Video2Code 采用两阶段策略：首先对视频进行粗粒度理解，定位动作关键区域；然后调用时序裁剪工具，以更高时间分辨率重新访问这些区域，获取细粒度动作信息；最后基于对齐后的视频-代码监督信号生成HTML/CSS/JavaScript代码。

Result: 实验表明，Video2Code 显著增强了开源模型在UI视频到代码生成任务上的性能，在视觉和功能评估指标上均优于直接视频观察的方法，尤其在密集多步交互场景中功能正确性提升明显。

Conclusion: Video2Code 通过动作感知的时序重访策略有效解决了UI视频到代码生成中的状态转换未对齐问题，验证了在视频理解中优先关注动作关键区域而非均匀分配视觉预算的有效性。

Classification reason: 该论文从UI视频生成交互式网页代码，属于视觉理解与代码生成任务，与多模态理解中的文档/UI理解密切相关。现有[multimodal_vqa_doc]涵盖VQA、文档理解等，UI视频到代码生成可视为文档/UI理解的延伸，因此复用该规范子主题。

Importance reason: Focuses on UI video-to-code generation, which is not a priority subtopic and lacks strong alignment with user's configured direction weights and boost keywords. Signals: Action-aware video understanding, State-transition recovery, UI video-to-code, Functional correctness improvement, Dense multi-step interaction
...
### [260] [Scaling Diverse Language Generation for 3D Visual Grounding](https://arxiv.org/abs/2606.20946)
...
TL;DR: 本文提出ViGiL3D++，一种可扩展的场景无关方法，通过结合场景图约束采样与LLM语言生成，生成多样化的3D视觉定位查询，以解决现有3DVG数据缺乏多样性的问题，提升模型在多个3DVG基准上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位（3DVG）模型受限于缺乏多样化的自然语言描述数据，难以泛化到简单语言模式之外。已有的数据增强方法在约束类型和语言多样性上不足，而纯描述方法无法精确对比物体，这对于视觉定位至关重要。

Method: 提出ViGiL3D++，一种场景无关的3DVG查询生成方法。它首先从场景图中采样多种约束组合（如空间关系、属性对比等），然后利用LLM将这些结构化约束转化为多样化的自然语言查询，从而生成大规模、高多样性的训练数据。

Result: 生成的查询在约束类型和语言多样性上显著优于现有数据集。使用ViGiL3D++数据训练的模型在多个3DVG基准上取得了性能提升，同时揭示了当前VLM在3D视觉定位中的局限性。

Conclusion: ViGiL3D++能够有效扩展3DVG数据的多样性和规模，提升模型泛化能力，但也暴露出现有VLM在精细3D空间理解上的不足，为未来研究指明了方向。

Classification reason: 该论文研究3D视觉定位（3DVG），属于视觉问答与定位范畴。3DVG是VQA在3D场景中的特化任务，与现有[CANONICAL] subtopic "视觉问答与文档理解"（multimodal_vqa_doc）中的VQA方向高度相关，因此复用该分类。

Importance reason: Focuses on 3D visual grounding data diversity, not aligned with user's priority on continuous multimodal models or video/image generation. Signals: 3D visual grounding, data diversity, scene graph, LLM generation, not continuous language model
...
### [261] [An approach with Visual and Tabular Mamba to multimodal medical data using Mixed Fusion](https://arxiv.org/abs/2606.20738)
...
TL;DR: 本文提出了一种基于Mamba架构的混合多模态融合方法（Mixed Fusion），用于癌症分类。该方法使用两个Mamba变体分别处理视觉和表格数据，通过SHAP增强可解释性。在PAD-UFES-20和NDB-UFES两个医学数据集上进行了实验，在NDB-UFES上表现优于Transformer方法，并在召回率指标上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 多模态医学数据（如图像与临床表格数据）的融合分类对于癌症诊断至关重要，但现有Transformer-based方法计算开销大且可解释性不足。作者希望探索基于状态空间模型（Mamba）的轻量级替代方案，同时提升模型的可解释性。

Method: 提出Mixed Fusion架构，包含两个Mamba变体：Visual Mamba处理病变/组织图像并生成类别概率，Tabular Mamba将这些概率与临床/社会人口学表格数据结合进行最终分类。采用SHAP方法对融合后的决策过程进行可解释性分析。

Result: 在PAD-UFES-20（皮肤病变）数据集上，平衡准确率略低于Transformer方法；在NDB-UFES（口腔癌）数据集上表现优于Transformer方法。在召回率指标上有显著提升，表明模型对阳性样本的识别能力更强。

Conclusion: Mamba-based模型是多模态医学数据分类的有效替代方案，尤其在需要高灵敏度（召回率）的场景中具有优势。Mixed Fusion架构结合SHAP方法提供了良好的可解释性，为临床决策支持提供了可行方案。

Classification reason: 该论文涉及多模态医学数据分类（皮肤病变和口腔癌），使用视觉和表格数据的混合融合架构，属于多模态理解中的视觉问答与文档理解（医学图像分类+临床表格数据）范畴。虽然具体任务是医学分类，但核心是多模态融合与理解，与现有 canonical subtopic "多模态评测与基准" 或 "视觉问答与文档理解" 相关，其中 "视觉问答与文档理解" 更贴近多模态医学数据分类任务。

Importance reason: The paper focuses on medical diagnosis with tabular data, which is a negative keyword, and does not align with the user's priority subtopics like continuous language models or video generation. Signals: medical diagnosis, tabular data, Mamba architecture, cancer classification, SHAP interpretability
...
### [262] [Robusto-2: Benchmarking Humans & VLMs for Autonomous Driving in Lima & New York City](https://arxiv.org/abs/2606.20980)
...
TL;DR: 本文提出Robusto-2基准，通过VQA范式比较人类驾驶员（利马与纽约）与VLM在利马和纽约的自动驾驶行车记录仪视频上的表现，涵盖事实、评分、反事实和推理四类问题。研究发现人类与VLM的回答存在差异，但地理因素对回答的影响不显著。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶汽车国际化并采用VLM作为认知骨干，这些系统在新地理环境中的泛化能力，尤其是分布外边缘场景下的表现，是一个亟待研究的问题。

Method: 收集利马和纽约的行车记录仪视频，构建VQA数据集，包含事实、评分、反事实和推理四类问题。对比利马人类驾驶员、纽约人类驾驶员以及多个VLM的回答，进行全因子分析。

Result: 人类与VLM的回答存在差异，差异程度受问题类型调节；不同地区的人类驾驶员回答相似；未发现地理因素对回答有显著影响，可能由于场景的高度分布外特性。

Conclusion: 当前VLM在自动驾驶OOD场景中的表现与人类存在差距，地理因素影响不显著，表明需要进一步提升VLM在复杂驾驶环境中的泛化能力。

Classification reason: 该论文在VQA范式下对自动驾驶场景进行视觉问答评测，属于多模态理解与生成方向下的"视觉问答与文档理解"（multimodal_vqa_doc）这一[CANONICAL]子主题。

Importance reason: Focuses on VLM benchmarking for autonomous driving in specific cities, which is not aligned with user's priority subtopics on continuous models, video generation, or world models. Signals: VQA benchmark, autonomous driving, human vs VLM comparison, geography effect, out-of-distribution

Abstract: As Self-Driving Cars continue to expand internationally and use multi-modal systems such as VLMs as a cognitive backbone for their Action models; how well will these systems generalize in new settings, in particular out-of-distribution (OOD) edge-case scenarios in new geographies? In this paper, we study this open question by providing a full factorial analysis with human drivers of Lima, human drivers from New York City, and VLMs and showing them dashcam footage collected from Lima and New York City -- prompting them with a variety of questions under a Visual Question Answering (VQA) paradigm. In particular, we pick these two cities as they are highly challenging driving locations where no Self-Driving Car company currently operates in, and ask questions that span 4 categories: Factual, Ratings, Counterfactual and Reasoning. We find that Humans and VLMs diverge in their responses -- though this is modulated by the type of questions asked, and that Humans answer similarly independent of where they are from (Lima/NYC). To our surprise, we did not find a strong difference in terms of answers (Humans or VLMs) that was modulated by geography, likely due to their high out-of-distribution nature. Our dataset is available at:

</details>


### [263] [A DVDrive Approach for doScenes Instructed Driving Challenge](https://arxiv.org/abs/2606.21623)
...
Importance: 普通相关 (45.17) - 未入选精读

TL;DR: 本文提出基于OmniDrive的指令条件轨迹预测方法，通过引入DVPE风格的分视图感知模块改进多视图视觉定位，在doScenes Instructed Driving Challenge中实现6秒未来轨迹预测。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中，传统的轨迹预测仅依赖视觉场景和历史运动信息，缺乏对自然语言驾驶指令的理解能力。本文旨在解决指令条件下的轨迹预测问题，使模型能够根据语言指令（如"前方路口右转"）预测未来轨迹。

Method: 基于OmniDrive视觉-语言-动作驾驶智能体，在指令标注的nuScenes场景上训练，预测12个未来路点表示的6秒轨迹。在感知头中引入DVPE风格的分视图感知模块，将查询特征和图像token分组到局部视图空间，进行可见性感知的交叉注意力，减少跨视图干扰，增强语言指令与局部驾驶相关视觉证据的对齐。

Result: 在doScenes Instructed Driving Challenge上提交的结果表明，该方法能够有效根据自然语言指令生成合理的未来轨迹，分视图感知模块改善了多视图视觉定位能力。

Conclusion: 本文验证了将视觉-语言-动作模型应用于指令条件轨迹预测的可行性，DVPE风格的分视图感知模块有助于减少跨视图干扰，提升语言指令与视觉场景的对齐效果。

Classification reason: 该论文研究指令条件下的轨迹预测，属于自动驾驶中的视觉问答与推理任务（以自然语言指令为输入，结合视觉场景输出轨迹），与多模态VQA/文档理解方向中的视觉推理任务高度相关。虽然具体应用是自动驾驶，但核心是视觉-语言-动作的联合推理，映射到[多模态VQA与文档理解]下的视觉问答与推理范畴。

Importance reason: Incremental adaptation of OmniDrive for a specific driving challenge, lacking novelty in core methods. Signals: Autonomous driving trajectory prediction, OmniDrive adaptation, DVPE divided-view perception, doScenes challenge, Low relevance to priority subtopics

Abstract: Instruction-conditioned trajectory prediction is an emerging problem in autonomous driving, where a model predicts the future ego trajectory not only from visual scene context and historical motion, but also from a natural-language maneuver instruction. This paper presents our submission to the doScenes Instructed Driving Challenge, built upon OmniDrive, a vision-language-action driving agent with 3D perception, reasoning, and planning capabilities. We adapt OmniDrive to the doScenes setting by training it on instruction-annotated nuScenes scenes and generating a 6-second ego trajectory represented by 12 future waypoints. To improve multi-view visual grounding, we further introduce a DVPE-style divided-view perception module into the OmniDrive perception head. Instead of attending globally to all camera features, the proposed module groups query features and image tokens into divided local view spaces and performs visibility-aware cross-attention within each view. This design reduces irrelevant cross-view interference and helps the model better align language instructions with local driving-relevant visual evidence. The code is publicly available at: .

</details>


### [264] [RAPID: A Reproducible Multi-Agent Pipeline for Interpretable Disaster Damage Assessment from Satellite and Street-View Imagery](https://arxiv.org/abs/2606.21819)
...
TL;DR: RAPID 是一个基于多智能体协作的可复现管道，利用卫星图像和街景图像进行零样本灾害损伤评估，无需任务特定微调即可实现多灾害类型分类（准确率0.92）和跨视角损伤严重性预测。


<details>
  <summary>Details</summary>
Motivation: 现有灾害损伤评估方法依赖监督学习和任务特定微调，难以泛化到域偏移、长尾分布和异构地理空间数据场景，且缺乏跨模态（卫星+街景）信息整合与推理能力。

Method: RAPID 构建了一个多智能体管道，协调多个专门智能体执行跨视角理解、图像恢复、结构化损伤识别和地理推理，利用预训练视觉语言模型在零样本条件下联合利用遥感与地面视角的互补信息，生成细粒度、可解释的评估报告。

Result: 在飓风、洪水、野火和地震等多灾害场景下，RAPID 实现多灾害类型分类准确率0.92，跨视角损伤严重性预测最高达0.627。

Conclusion: RAPID 展示了多智能体多模态管道在灾害智能中的潜力，可作为自主灾害智能的基础框架，支持早期应急响应。

Classification reason: 该论文使用多模态大模型（VLM）对卫星图像和街景图像进行灾害损伤评估，属于视觉问答与多模态理解范畴。虽然涉及地理空间数据，但核心任务是跨视角图像理解与结构化损伤识别，与[多模态评测与基准]或[视觉语言模型与MLLM]也有一定关联，但最贴切的现有规范子主题是"视觉问答与文档理解"（multimodal_vqa_doc），因为论文本质上是基于多模态输入的问答式评估与报告生成。

Importance reason: Focuses on disaster damage assessment with satellite/street-view imagery, not aligned with user's priority subtopics like continuous language models or video generation. Signals: Multi-agent pipeline, zero-shot damage assessment, cross-view understanding, satellite and street-view imagery, disaster damage classification
...
### [265] [Look Before You Zoom: Adaptive Routing for the Resolution-Context Trade-off in Visual RAG](https://arxiv.org/abs/2606.21968)
...
TL;DR: ViRGo 是一个轻量级框架，通过自适应路由解决视觉 RAG 中分辨率与上下文之间的权衡问题，根据目标尺度动态选择全局感知、patch 检索或 attention 检索，在多个 VQA 基准上提升了准确率与效率的平衡。


<details>
  <summary>Details</summary>
Motivation: VLM 在处理小目标查询时表现不佳，现有训练无关方法通过动态检索和放大局部区域来缓解，但盲目应用检索会忽略分辨率-上下文的权衡：patch 检索恢复小目标细节但破坏大目标全局上下文，attention 检索保留大目标但对小细节不可靠，而全局感知在无需检索时最快。因此需要一种自适应路由机制来根据目标尺度选择最优策略。

Method: ViRGo 利用 VLM 内在的定位头（localization heads）在首次前向传播中估计目标尺度，并结合语义 token 置信度，自适应地在三种策略间路由：全局感知（global perception）、基于 patch 的检索（patch-based retrieval）和基于 attention 的检索（attention-based retrieval），仅需极少的额外计算开销。

Result: 在多个 VQA 基准和不同目标尺寸分组上的实验表明，ViRGo 在小目标上匹配 patch 检索的精度，在大目标上利用 attention 检索，并在无需放大时路由到全局基线以减少推理时间，整体上改善了准确率与效率的权衡。

Conclusion: ViRGo 通过自适应路由有效解决了视觉 RAG 中分辨率与上下文的权衡问题，为 VLM 在小目标场景下的推理提供了一种轻量级、训练无关的优化方案。

Classification reason: 该论文提出 ViRGo 框架，通过自适应路由选择全局感知、基于 patch 的检索或基于 attention 的检索来提升 VLM 在小目标 VQA 上的表现，核心任务属于视觉问答（VQA），因此映射到 [CANONICAL] 的 multimodal_vqa_doc 子主题。

Importance reason: Addresses resolution-context trade-off in visual RAG but is not aligned with user's priority subtopics on continuous models or generation. Signals: visual RAG, adaptive routing, resolution-context trade-off, VQA benchmarks, training-free
...
### [266] [Beyond Time Series: Spatial Reasoning for Epidemic Forecasting via Multimodal Learning](https://arxiv.org/abs/2606.22171)
...
TL;DR: 本文提出M-SPICE，一个结构感知的多模态时空流行病预测框架，通过注意力融合机制对时序疾病动态与空间上下文进行联合推理，在COVID-19、流感和ILI预测任务上持续超越SOTA基线。


<details>
  <summary>Details</summary>
Motivation: 传统流行病预测模型将行政区域视为原子单元，忽略了区域内空间结构对疾病传播的影响。现有方法无法有效融合分辨率不一致的区域级时序数据与空间定位辅助信号，因此需要一种结构感知的多模态时空预测方法。

Method: 提出M-SPICE框架，采用基于注意力的多模态融合机制，对时序疾病动态与空间上下文进行联合推理。空间信号通过注意力机制选择性调节不同预测时域上的时序表示，实现跨分辨率、跨结构的多模态数据融合。

Result: 在COVID-19、流感和ILI真实预测任务上，M-SPICE持续超越最先进的多元时间序列、多模态和流行病预测基线，同时保持强概率预测性能。可解释性分析揭示了空间信号被利用的时机、位置和方式。

Conclusion: 空间结构信息对流行病预测至关重要，M-SPICE通过多模态注意力融合有效利用了空间上下文，显著提升了预测性能，并揭示了纯时序区域聚合模型最容易失败的情景。

Classification reason: 该论文提出多模态时空流行病预测框架M-SPICE，融合区域级时序数据与空间辅助信号进行注意力融合推理。虽然主题是流行病预测，但其核心是多模态融合与时空推理，与多模态理解与生成方向下的"视觉问答与文档理解"（multimodal_vqa_doc）中的多模态推理任务有合理关联。该论文涉及多模态数据融合与结构化推理，可归入该现有子主题。

Importance reason: Paper focuses on epidemic forecasting with spatial-temporal multimodal learning, which is outside the user's core interests in continuous language/multimodal models, video/image generation, and world models. Signals: epidemic forecasting, spatial-temporal reasoning, multimodal fusion, time series, not autoregressive generation
...
### [267] [4DVLT: Dynamic Scene Understanding with Worldline-Centered Vision-Language Tracking](https://arxiv.org/abs/2606.22631)
...
TL;DR: 本文提出4DVLT，一个以世界线为中心的4D动态场景理解任务，以及Instruct-4D基准（129.4K问答对、851场景、9种推理查询类型），并设计了4DTrack方法，通过目标中心4D状态图、度量引导路由、双向解码和运动学校准实现指令条件跟踪。4DTrack-Qwen3.5-9B在Instruct-4D上达到62.68 TGA_Top1，超过最佳基线19.62个点。


<details>
  <summary>Details</summary>
Motivation: 现有4D动态场景理解方法存在局限：大语言模型能推理丰富视觉证据但难以保持度量拓扑结构，视觉语言跟踪则局限于碎片化的2D/3D输出和局部连续性。缺乏一个将语言指令与持久世界线（绑定身份、度量3D运动和多视角2D投影）相结合的统一范式。

Method: 提出4DTrack方法，将指令条件跟踪建模为图条件世界线推理。核心组件包括：(1) 目标中心4D状态图，构建跨时间/视角的物体状态关联；(2) 度量引导路由，利用3D度量信息引导推理路径；(3) 双向解码，同时利用过去和未来信息；(4) 运动学校准，保证轨迹的物理一致性。基于Qwen3.5-9B作为基础模型。

Result: 在Instruct-4D基准上，4DTrack-Qwen3.5-9B达到62.68 TGA_Top1，超过最佳适配的VLT基线19.62个点。实验表明世界线中心建模同时提升了目标定位和恢复的世界线质量。

Conclusion: 世界线中心的建模方法能有效提升4D动态场景中的指令条件跟踪性能，在目标定位和轨迹质量上均显著优于现有方法。Instruct-4D基准为4D场景理解提供了更全面的评测平台。

Classification reason: 该论文提出了一个面向4D动态场景理解的指令条件跟踪任务（Instruct-4D基准），包含129.4K问答对和9种推理查询类型，本质上属于视觉问答（VQA）范畴，因此映射到[CANONICAL] subtopic "视觉问答与文档理解"（multimodal_vqa_doc）。

Importance reason: Novel 4D tracking task but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: 4D dynamic scene understanding, worldline-centered tracking, Instruct-4D benchmark, 9 reasoning query types, 4DTrack method

Abstract: 4D dynamic scene understanding requires grounding language to a persistent worldline that binds identity, metric 3D motion, and synchronized multi-view 2D projections. Existing paradigms capture only part of this structure: large multimodal models reason over rich visual evidence but rarely preserve metric topology, while vision-language tracking remains tied to fragmented 2D or 3D outputs and local continuation. We therefore introduce \textbf{4DVLT}, a worldline-centered task for instruction-conditioned 4D dynamic scene understanding in fully observed multi-view video, and \textbf{Instruct-4D}, a benchmark with 129.4K question-answer pairs, 64.7K target entities, 851 scenes, and 9 reasoning-oriented query types. To address this setting, we present \textbf{4DTrack}, which casts instruction-conditioned tracking as graph-conditioned worldline inference through an object-centric 4D state graph, metric-guided routing, bidirectional decoding, and kinematic calibration. On Instruct-4D, 4DTrack-Qwen3.5-9B reaches 62.68 $\mathrm{TGA}_{\mathrm{Top1}}$ and surpasses the best adapted VLT baseline by 19.62 points. These results show that worldline-centered modeling improves both target grounding and recovered worldline quality. The project page is available at .

</details>


### [268] [Evo-RAD: Navigating Rare Retinal Disease Diagnosis via Self-Evolving Agentic Retrieval](https://arxiv.org/abs/2606.22955)
...
TL;DR: Evo-RAD 提出一种自进化智能体检索框架，将罕见视网膜疾病诊断中的证据获取建模为马尔可夫决策过程，通过 GRPO 优化图基智能体动态增删参考样本，显著提升罕见病诊断性能。


<details>
  <summary>Details</summary>
Motivation: 大规模预训练基础模型在通用医学筛查中表现优异，但在罕见疾病上因数据稀缺而性能不佳。传统检索增强诊断方法存在 hubness 问题，即检索到视觉相似但语义错误的常见疾病样本，导致误诊。因此需要一种动态、自适应的证据获取机制。

Method: 将检索过程建模为马尔可夫决策过程（MDP），设计图基智能体（graph-based agent）观察参考集状态，执行三种动作：DELETE（移除不一致证据）、INSERT（获取病理一致样本）、TERMINATE（终止进化）。采用组相对策略优化（GRPO）结合同质性感知奖励（homogeneity-aware reward）训练智能体，最大化支持参考集的诊断同质性。

Result: 在视网膜疾病基准上，Evo-RAD 在罕见病诊断上比视网膜基础模型提升 +21.04%，比检索增强方法和参数高效微调方法提升 +3.56%。

Conclusion: Evo-RAD 通过将检索转化为动态决策过程，有效缓解了 hubness 问题，显著提升了罕见视网膜疾病的诊断准确率，验证了自进化智能体框架在医学少样本诊断场景中的潜力。

Classification reason: 该论文聚焦于罕见视网膜疾病的诊断，属于医学视觉问答/诊断任务，与多模态理解方向下的"视觉问答与文档理解"（multimodal_vqa_doc）子主题高度相关，因此复用该规范子主题。

Importance reason: Focuses on rare retinal disease diagnosis with agentic retrieval, which is outside the user's priority subtopics and contains negative keywords. Signals: medical diagnosis, rare disease, retrieval-augmented, GRPO, retinal foundation model
...
### [269] [Brain-Adapter: A Dual-Stream Vision-Language MIL Framework for Comprehensive 3D CT Diagnosis of Acute Intracranial Pathologies](https://arxiv.org/abs/2606.23494)
...
Importance: 普通相关 (40.0) - 未入选精读

TL;DR: 本文提出 Brain-Adapter，一种双流多实例学习框架，利用预训练的 2D 生物医学视觉语言模型和原始诊断报告，实现 3D 脑部 CT 扫描的自动多标签分类，无需密集标注。


<details>
  <summary>Details</summary>
Motivation: 3D 脑部 CT 的自动诊断对重症监护至关重要，但现有方法严重依赖人工标注且语义理解有限。2D 基础 VLM 虽有泛化能力，但如何将其表征能力有效迁移到 3D 体数据仍是开放问题。

Method: 提出 Brain-Adapter 双流 MIL 框架：(1) 文本条件注意力（TCA）机制，以原始诊断句子作为语义查询，动态对齐视觉线索与疾病概念；(2) 并行视觉 MIL 流，利用 LLM 提取的结构化标签监督全局扫描特征；(3) 一致性约束确保双流表征协同；(4) 推理时的不确定性感知精炼（UAR）模块动态校准和融合双流预测。

Result: 大量实验表明，该方法显著优于现有的 3D 模型和标准 MIL 方法，消除了对密集标注的依赖。

Conclusion: Brain-Adapter 为 3D 急性颅内病理分析提供了一种高度可扩展且临床可行的解决方案，证明了利用 2D VLM 和文本报告进行 3D 医学诊断的有效性。

Classification reason: 该论文提出 Brain-Adapter 框架，利用预训练的 2D 生物医学 VLM 和原始诊断报告进行 3D CT 扫描级多标签分类，属于医学视觉问答与诊断理解任务，与 [CANONICAL] subtopic "多模态评测与基准" 不匹配，但与 "视觉问答与文档理解"（multimodal_vqa_doc）中的医学 VQA/诊断理解方向高度相关，因此复用该 subtopic。

Importance reason: This paper focuses on medical diagnosis, which is a negative keyword, and does not align with the user's priority subtopics like continuous language models or video generation. Signals: medical diagnosis, negative keyword, 3D CT, MIL framework, no continuous language model
...
### [270] [Fine-grained Human Motion Understanding with Language Models](https://arxiv.org/abs/2606.20888)
...
TL;DR: 本文提出MotionLLM，一种基于LLM的细粒度人体运动理解模型，将运动表示为带显式时间戳的骨骼姿态序列，通过姿态级和运动级多样监督训练，在多个基准上达到SOTA，且仅用2D骨骼输入即可超越此前3D方法。


<details>
  <summary>Details</summary>
Motivation: 现有运动理解方法多依赖3D动作捕捉数据，且缺乏对运动时序细节（如顺序、时长、节奏）的细粒度推理能力。作者希望构建一个能同时处理2D/3D骨骼运动、具备时序推理能力的统一模型。

Method: 将人体运动表示为带显式时间戳的骨骼姿态序列，每个姿态编码关节位置并与时间戳token对齐；构建包含姿态描述、姿态问答、运动描述、运动问答的多样训练混合数据；采用统一姿态编码器支持2D和3D骨骼输入，并可融合视频上下文信息。

Result: 在BABEL-QA、HuMMan-QA、CompMo、NTU-RGB+D、QEVD-Coach等多个基准上达到SOTA性能；消融实验表明姿态级和运动级多样监督是主要增益来源，分阶段训练有额外收益；仅用2D骨骼输入即可超越此前基于3D的方法。

Conclusion: 显式时序编码与多样化的姿态/运动级监督对细粒度人体运动理解非常有效，且2D骨骼输入在实际应用中更具可行性，为运动理解提供了更实用的范式。

Classification reason: 该论文研究基于LLM的细粒度人体运动理解，涉及2D/3D骨骼姿态序列的时序推理、运动描述与问答，属于视频理解与时序推理（video_understanding_temporal）范畴，该方向涵盖视频LLM、时序推理、运动理解等主题，与论文高度匹配。

Importance reason: Strong fine-grained motion understanding but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: explicit temporal encoding, pose-level supervision, 2D skeleton surpasses 3D, motion-language reasoning, BABEL-QA SOTA
...
### [271] [HPP: Hierarchical Programmatic Probing for Long Video Understanding by Decoupling Perception and Reasoning](https://arxiv.org/abs/2606.21734)
...
TL;DR: 本文提出 HPP（Hierarchical Programmatic Probing）框架，通过将长视频理解重构为对分层分割视频的迭代程序化探测，解耦语义感知与高阶时序推理。编码能力的 LLM 在交互式编码环境中规划并执行多步策略，按需调用 VLM 进行局部感知，在 LongVideoBench、EgoSchema、VideoMME、MLVU 等基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 将视频帧编码为视觉 token，在单次前向传播中隐式完成感知与多步规划，但 LLM 的潜表征能力有限，难以发现和执行多步推理策略，导致长视频理解存在瓶颈。

Method: 提出 HPP 框架，包含三个核心组件：(1) 信息密度感知的分层视频分割；(2) 延迟交互语义检索；(3) 结构化探测函数实现从粗到细的时间定位。编码能力的 LLM 在交互式 Python 环境中迭代执行探测程序，按需调用 VLM 进行局部感知，将感知与推理解耦。

Result: 在 LongVideoBench 上验证了感知与推理解耦的有效性，并在 EgoSchema、VideoMME、MLVU 等多个长视频基准上取得显著性能提升。

Conclusion: 通过将感知与推理解耦为分层程序化探测，HPP 有效克服了 VLM 在长视频理解中的推理瓶颈，为长视频时序推理提供了一种新的范式。

Classification reason: 该论文聚焦长视频理解，提出分层程序化探测（HPP）框架，将感知与推理解耦以提升长视频时序推理能力，属于视频理解与时序推理方向，因此复用该规范子主题。

Importance reason: Decouples perception and reasoning for long video understanding, but lacks direct connection to priority subtopics like continuous models or generation. Signals: long video understanding, hierarchical programmatic probing, decoupling perception and reasoning, VLM + LLM coding, temporal reasoning
...
### [272] [CoVStream: Edge-Cloud Collaboration for Understanding of Long Video Streams](https://arxiv.org/abs/2606.22804)
...
TL;DR: CoVStream 提出首个边缘-云协作的长视频流理解框架，边缘设备将视频流蒸馏为紧凑视觉特征和语义描述传输到云端，云端构建实体图和全局视觉上下文，仅在用户查询时激活大模型推理。在多个基准上带宽降低87.6%，同时保持99.2%的云端基线准确率。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解方法采用"采样-编码-推理"范式，但忽略了视频流通常由计算受限的边缘设备产生这一实际部署场景。直接云端卸载带来巨大带宽开销，而纯边缘处理受限于硬件能力，两者均不可行。因此需要一种边缘-云协作方案来平衡带宽与推理能力。

Method: CoVStream 框架包含边缘端和云端两个模块。边缘端将原始视频流蒸馏为紧凑的视觉特征（如 CLIP 特征）和语义描述（caption），仅传输这些轻量数据到云端。云端接收后构建实体图（entity graph）和全局视觉上下文，当用户查询到达时，才激活云端的大模型进行深度推理。通过这种"按需推理"机制，避免了持续传输原始视频流的高带宽消耗。

Result: 在 VideoMME-Long、LVBench 和 RTV-Bench 三个基准上评估，CoVStream 相比纯云端方案降低 87.6% 的带宽使用，同时在 LVBench 上保留了 99.2% 的云端基线准确率，实现了带宽与性能的良好平衡。

Conclusion: CoVStream 验证了边缘-云协作框架在长视频流理解中的有效性，通过边缘端特征蒸馏和云端按需推理，大幅降低带宽开销的同时保持了高推理精度，为实际部署场景下的长视频理解提供了可行方案。

Classification reason: 该论文关注长视频流理解，提出边缘-云协作框架处理连续视频流，属于视频理解范畴。映射到 [CANONICAL] subtopic "视频理解与时序推理"（video_understanding_temporal），该主题涵盖视频LLM、长视频理解等方向。

Importance reason: Focuses on edge-cloud video streaming efficiency, not on the user's priority subtopics like continuous language models or autoregressive generation. Signals: Edge-cloud collaboration, Long video stream understanding, Bandwidth reduction, Entity graph, Not aligned with priority subtopics
...
### [273] [VideoLatent: Video-Language Learning via Latent Self-Forcing](https://arxiv.org/abs/2606.22870)
...
TL;DR: VideoLatent 提出了一种基于潜空间自强制（latent self-forcing）训练范式的视频理解与推理 MLLM，通过潜对齐和潜多样性两个目标在连续潜空间中进行视觉推理，仅需标准视频-问答三元组即可训练，在14个基准上超越现有方法，且训练/推理开销分别降低约6倍/68倍。


<details>
  <summary>Details</summary>
Motivation: 现有基于思维链（CoT）的多模态大模型需要大量人工 CoT 标注，训练和推理开销巨大。而现有的视觉潜空间推理方法主要针对图像任务，且依赖额外的监督信号（如 CoT 轨迹、辅助图像或细粒度标注），难以扩展到视频任务。因此，需要一种高效、可扩展的视频潜空间推理方法。

Method: 提出 VideoLatent，包含一个潜注入模块（latent injection module），并设计了一种新的潜自强制（latent self-forcing）训练范式，包含两个目标：(1) 潜对齐（latent alignment）—— 使模型生成的潜表示与后续推理所需的潜表示对齐；(2) 潜多样性（latent diversity）—— 鼓励潜表示的多样性以避免坍塌。该方法仅需标准视频-问答三元组即可训练，无需额外标注。

Result: 在14个视频理解与推理基准上，VideoLatent 一致优于现有标准 MLLM 和潜空间 MLLM。与 Video-R1 相比，训练开销降低约6倍，推理开销降低约68倍。此外，该方法在不同 MLLM 骨干网络和不同模型规模上均表现出良好的泛化能力。

Conclusion: VideoLatent 通过潜自强制训练范式，成功将连续潜空间推理从图像任务扩展到视频任务，在显著提升计算效率的同时保持了优异的视频理解与推理性能，为高效视频 MLLM 提供了新方向。

Classification reason: 该论文提出 VideoLatent，在连续潜空间中进行视觉推理（visual latent reasoning），属于多模态模型在连续 latent 中进行 reasoning 的范畴，与 [CANONICAL] subtopic "连续多模态推理与潜空间思维" 高度匹配。同时该方法也涉及连续视觉语言模型，因此也匹配 continuous_language_multimodal 方向。

Importance reason: Proposes a highly efficient latent self-forcing paradigm for video-language MLLMs, directly advancing continuous multimodal reasoning with massive compute savings. Signals: continuous multimodal reasoning, latent self-forcing, video-language MLLM, 68x inference reduction, 14 benchmark SOTA
