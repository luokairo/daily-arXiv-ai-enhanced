...
- [多模态生成](#direction-multimodal_generation) [Total: 20]
...
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 61]
  ...
  - [数字人与动作生成](#direction-multimodal_understanding_generation-avatar_motion_generation) [Total: 1]
...
### [1] [Multimodal Spatiotemporal Atmospheric Data Assimilation with Latent Flow-matching](https://arxiv.org/abs/2608.05103)
*Dibyajyoti Chakraborty, Romit Maulik*

Main category: math-ph, physics.flu-dyn, cs.LG, physics.ao-ph
...
TL;DR: 本文提出一种基于 latent video flow-matching 的大气数据同化统一方法，利用 ERA5 再分析数据训练先验生成连续轨迹，并通过后验采样同化真实观测数据，实现滤波、平滑与稀疏观测直接生成全状态集合预报。


<details>
  <summary>Details</summary>
Motivation: 传统数据同化（DA）依赖贝叶斯推断更新数值预报模型状态，计算复杂且难以统一处理多种同化任务。作者希望提出一种根本不同的统一方法，利用生成式潜空间流模型实现大气数据同化。

Method: 使用 latent video flow-matching 从 ERA5 再分析数据（8天窗口、69个变量）训练先验，生成时序一致的连续轨迹；通过后验采样同化 NOAA 无线电探空仪和综合地表数据库等真实观测；由于先验生成连续轨迹，信息可在观测帧与非观测帧间自然传播，仅通过改变观测帧即可完成滤波与平滑等不同 DA 任务。

Result: 该方法可直接从稀疏观测生成全状态集合预报，性能与最先进的观测到预报模型相当。

Conclusion: 潜空间流匹配为大气数据同化提供了一种统一、灵活且高效的生成式框架，能够自然处理多种同化任务并达到与先进方法竞争的性能。

Classification reason: 论文使用 latent video flow-matching 对大气多模态时空数据进行连续潜空间建模与数据同化，属于连续潜空间流（flow matching）方法，映射到 [CANONICAL] 子主题 continuous_vlm_latent_flow（连续 VLM 与视觉潜空间流）。

Importance reason: Directly applies latent flow-matching to continuous multimodal trajectory generation, strongly matching the top-priority continuous VLM/visual latent flow subtopic despite the atmospheric domain. Signals: latent video flow-matching, continuous trajectory prior, posterior sampling for data assimilation, sparse observation to full-state ensemble forecast, ERA5 reanalysis
...
TL;DR: DyLaR 提出动态潜空间推理方法，先通过感知潜变量定位问题相关的视觉证据，再自适应决定是否在潜空间中追加推理潜变量，从而在视频问答中大幅减少 token 生成量并提升准确率。


<details>
  <summary>Details</summary>
Motivation: 现有视频问答方法通常依赖冗长的文本思维链（CoT）来推理，但许多问题在定位到相关对象、动作或帧后即可直接回答。长文本推理不仅低效，还引入了大量不必要的 token 开销。

Method: DyLaR 首先将问题 grounding 到一段感知潜变量（continuous hidden states 编码查询相关的视觉证据），然后自适应决定是否追加推理潜变量（在潜空间中对该证据进行推理）再作答。训练上通过将感知潜变量 grounding 到已验证的视觉证据、将已验证的推理链蒸馏到推理潜变量，并配合强化学习进一步优化"何时推理"的决策。

Result: 在九个视频基准和四个多模态语言模型骨干上，DyLaR 相比同骨干基线提升平均准确率，同时每个查询仅生成不到 20 个 token。在 Qwen3-VL-4B 上，平均准确率从 54.0 提升到 58.2，响应长度从 1220.7 降至 18.5 token。消融实验表明感知潜变量 grounding、推理潜变量的 rationale 监督以及自适应路由均带来提升。

Conclusion: DyLaR 表明"先感知后推理"的动态潜空间推理策略能显著提升视频问答的效率与准确性，验证了在连续潜空间中进行感知定位与自适应推理的有效性。

Classification reason: 论文提出 Dynamic Latent Reasoning (DyLaR)，在连续潜空间中先进行感知定位（perception latents）再自适应决定是否进行推理（reasoning latents），属于连续多模态推理与潜空间思维，映射到 [CANONICAL] continuous_multimodal_reasoning。同时涉及视频理解与问答，也匹配 multimodal_understanding_generation 方向。

Importance reason: Directly advances continuous latent reasoning for video understanding, a top-priority subtopic, with strong efficiency gains and broad benchmark validation. Signals: continuous latent reasoning, dynamic adaptive reasoning, video QA, token efficiency, RL-optimized reasoning routing
...
### [3] [SJEPA: Learning Elegant Latent Dynamics with Hybrid Symbolic-Neural Predictors](https://arxiv.org/abs/2608.04060)
*Yongchao Huang*
...
TL;DR: SJEPA 是一种无重建的 JEPA 框架，通过混合符号-神经预测器学习具有简洁符号描述的潜动力学，在连续潜空间中联合学习表征与动力学，并形式化诱导动力学复杂度与表征坍缩捷径。


<details>
  <summary>Details</summary>
Motivation: 联合嵌入预测架构（JEPA）的转移模型通常是不可解释的神经映射，缺乏对潜动力学结构的理解。作者希望学习既能保持预测能力又具有简洁符号描述的潜表征与动力学。

Method: SJEPA 采用混合转移模型，将符号定律与正则化的神经修正相结合。核心原则是学习"最简单且充分"的动力学：表征约束保持信息丰富且不坍缩的预测坐标，算子压缩偏好低复杂度的符号-神经转移。框架支持交替的表征-方程学习和固定表征上的符号动力学拟合，并通过诱导动力学复杂度形式化该原则。

Result: 在受控摆实验中，联合学习相比事后拟合发现了显著更简单的符号动力学，且长时程 rollout 误差和散度更低；无约束的一步诊断实现了预测的坍缩捷径。在语法误设定下，修正正则化保留了可表征的符号机制并将神经组件导向残差动力学。

Conclusion: SJEPA 揭示了预测保真度、表征质量、符号简洁性与符号-神经分配之间的可控权衡，为可解释的连续潜动力学学习提供了新框架。

Classification reason: 论文提出 SJEPA，一种在连续潜空间中学习预测表征与动力学（latent dynamics）的 JEPA 框架，属于连续潜空间建模与连续表征学习范畴，与连续语言模型/连续潜空间方向高度相关，映射到 CANONICAL 子主题"连续语言模型与语言流"。

Importance reason: Directly advances continuous latent dynamics with symbolic-neural hybrid predictors, aligning with the top-priority continuous language/multimodal modeling direction. Signals: continuous latent dynamics, JEPA, symbolic-neural hybrid, representation collapse, flow matching adjacent
...
TL;DR: 本文从联合视角重新审视白盒网络的自监督学习，利用 LeJEPA 自监督范式优化稀疏率缩减目标中的扩展项 R(Z)，并通过 ADMM 推导出仅含注意力层的白盒 Transformer（去除 ISTA 结构与 MLP 层）。在 CIFAR-10/100 上取得与原始 CRATE 相当的分类精度，同时参数减少约 31%。


<details>
  <summary>Details</summary>
Motivation: 现有白盒网络的自监督学习研究通常将优化算法推导的白盒网络与自监督学习范式解耦。作者希望从联合视角统一两者，利用 LeJEPA 自监督范式来优化白盒 Transformer 的目标函数，从而简化网络结构。

Method: 观察到 LeJEPA 自监督框架假设各向同性高斯分布为最优嵌入分布，这与稀疏率缩减目标中的扩展项 R(Z) 概念等价。使用 LeJEPA 范式优化 R(Z)，并通过 ADMM 交替方向乘子法推导剩余项 R^c(Z|U_[K])+λ||Z||_0，得到仅含注意力层的白盒 Transformer，去除原始设计中的 ISTA 结构与 MLP 层。

Result: 在 Base 规模下，注意力仅白盒 Transformer 在 CIFAR-10 上达到 88.88% 分类精度，CIFAR-100 上达到 63.54%，与原始 CRATE（89.18%/63.56%）相当，参数减少约 31%。此外，在标准 ViT 中，用 ReLU 激活替换所有 MLP 块并在知识蒸馏下可去除约 66% 参数而保持竞争性精度。

Conclusion: 注意力仅白盒 Transformer 在保持竞争性能的同时显著减少参数，验证了白盒网络与自监督学习联合设计的有效性，并进一步暗示标准 ViT 架构中 MLP 模块可能存在冗余。

Classification reason: 论文基于 LeJEPA 自监督范式与稀疏率缩减目标，推导出仅含注意力层的白盒 Transformer，属于连续 latent 空间中的表征学习与建模，与连续语言模型/连续潜空间建模方向相关，映射到 CANONICAL 子主题 continuous_language_flow。

Importance reason: 白盒Transformer与LeJEPA结合虽新颖，但实验仅限CIFAR小规模分类，与用户主攻的连续多模态/视频生成方向关联较弱。 Signals: 白盒Transformer, LeJEPA自监督, 注意力-only架构, 参数减少31%, CIFAR-10/100
...
TL;DR: SCOPE 是一个面向未知 3D 环境的视场感知路径规划框架，通过安全体积认证（safety-volume certification）确保机器人运动路径在有限视场传感器下被完全观测并验证为自由空间后才执行，实现条件完备规划。


<details>
  <summary>Details</summary>
Motivation: 带有限视场传感器的机器人安全导航要求运动路径的完整机器人膨胀体积在执行前被观测并验证为自由。现有方法难以在未知体素地图中保证这一点，需要一种将观测义务与执行解耦的规划框架。

Method: SCOPE 构建认证图（certified graph），其顶点对应安全体积完全已知为自由的位姿。框架将乐观目标导向引导与认证执行解耦：将乐观路径上第一个未认证点转化为显式观测义务，通过目标中心视点搜索解决，并在有用视点尚未认证可达时递归清除中间义务。配备认证预览机制和观测感知轨迹优化后端。

Result: 在三个未知 3D 环境的 60 个随机任务中，SCOPE 全部到达目标且几乎不进入未认证膨胀空间；预览机制使平均任务时间降低 27%；两个代表性场景的真实机器人演示验证了完整系统。

Conclusion: SCOPE 通过安全体积认证与观测规划解耦，在理想单调感知和穷举有限域图搜索条件下可证明条件完备规划，为有限视场传感器下的未知环境安全导航提供了有效框架。

Classification reason: 论文提出 SCOPE 路径规划框架，面向未知 3D 环境中带有限视场传感器的机器人安全导航，属于具身智能中的动作规划与控制，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Solid robotics planning contribution but far outside the user's core focus on continuous multimodal models and generative video/world models. Signals: safety-volume certification, conditional complete planning, field-of-view-aware path planning, unknown 3D environments, real-robot validation
...
### [6] [Mind-VLA: Instruction-Aware Spatial Representation Alignment for Vision-Language-Action Models](https://arxiv.org/abs/2608.04633)
...
TL;DR: Mind-VLA 提出一种指令感知的空间表征对齐方法，通过提取语言指令指定的目标物体的三视图 VAE 和 VGGT 特征，将 VLA 模型的潜表征与之对齐，实现指令感知的 3D 理解，在 LIBERO、CALVIN 及真实机器人目标遮挡任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 方法通过将表征与 3D 场景几何对齐来提升泛化能力，但这类对齐是指令无关的：它们均匀地对齐整个场景，忽略了语言指令指定的具体目标物体的 3D 几何。这导致在细粒度操作和目标遮挡任务上失败，因为这些任务的成功取决于对目标物体的精确 3D 理解而非整个场景。

Method: Mind-VLA 首先通过语言指令获取目标物体，然后准备目标物体的三视图（tri-view），并提取对应的 VAE 和 VGGT 特征。最后将 VLA 模型的潜表征与这些特征对齐，以实现指令感知的 3D 理解。

Result: Mind-VLA 在 LIBERO 上达到 93.9%，在 CALVIN 上达到 4.47，使用紧凑的 345M 参数骨干网络。在真实机器人目标遮挡任务上达到 54% 的平均成功率，比最佳指令无关方法高出 32 个百分点。

Conclusion: 指令感知的空间表征对齐能够显著提升 VLA 模型在细粒度操作和目标遮挡场景下的表现，验证了将语言指令指定的目标物体 3D 几何纳入表征对齐的重要性。

Classification reason: 该论文提出 Mind-VLA，一种面向 Vision-Language-Action (VLA) 模型的指令感知空间表征对齐方法，用于机器人操作任务，属于具身智能与动作规划控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Instruction-aware 3D alignment for VLA is solid robotics work, but it is tangential to the user's core focus on continuous multimodal models and autoregressive generation. Signals: VLA, instruction-aware alignment, VGGT, tri-view VAE, real-robot occlusion
...
### [7] [Suppression Sticks, Locality Is Fragile: A Closed-Loop Target-and-Control Audit of Task-Vector Negation in VLA Policies](https://arxiv.org/abs/2608.04692)
...
TL;DR: 本文对多任务 VLA 策略中的任务向量减法进行闭环目标-控制审计，发现减法在 LIBERO-Goal 技能上产生三种不同行为模式（目标-控制分离、抵抗、全局崩溃），且目标抑制编辑会损害无关控制技能，表明任务向量减法是一种快速但脆弱的干预手段。


<details>
  <summary>Details</summary>
Motivation: 任务向量算术提供了一种闭式修改模型的方法，但其在闭环机器人控制中的行为局部性尚不清楚。作者希望系统审计任务向量减法在 VLA 策略中的行为，评估其作为模型编辑手段的可靠性与局部性。

Method: 作者对 LIBERO-Goal 的十个技能进行逐技能任务向量减法，区分三种行为模式（目标-控制分离、抵抗、全局崩溃）；在保留初始状态上测试目标抑制成功率与控制保留率；对比连续回归、离散 token 和 flow-matching 三种动作头；使用匹配范数控制、多向量组合、保留感知梯度基线等方法进行对照分析，并进行单技能重学习探针。

Result: 五个可抑制目标保持 0% 成功率，但平均基线归一化控制保留率仅 52%，每个目标抑制编辑至少损害一个无关控制。Goal 面板在三种动作头上均显示分离，而 Spatial 无干净分离、Object 和 Long-horizon 面板出现控制崩溃。任务向量余弦无法解释这些差异，匹配范数控制发现局部符号不对称性。

Conclusion: 任务向量减法是一种快速但脆弱的模型编辑干预，其行为局部性在闭环具身控制中不可靠。作者强调在评估具身模型编辑的局部性时需要闭环目标-控制评估，并指出减法更接近行为掩蔽而非认证遗忘。

Classification reason: 论文研究 VLA（视觉-语言-动作）策略中的任务向量减法审计，属于具身智能/机器人策略编辑范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Focuses on task-vector editing in VLA policies, which is only tangentially related to your core continuous multimodal and video generation priorities. Signals: embodied world model, task-vector negation, closed-loop audit, VLA policy editing, behavioral locality
...
### [8] [Explicit Language Memory for Long-Horizon Planning in Vision-Language-Action Models](https://arxiv.org/abs/2608.04765)
...
TL;DR: 提出一种带显式语言记忆模块的分层长时程 VLA 架构，将离散时序观测转化为带时序逻辑的文本记忆序列，高层 VLM 负责语义推理与记忆更新，低层 VLA 执行连续控制，提升长时程任务成功率与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型在长时程任务中面临四大挑战：稀疏专家演示限制跨任务组合泛化、非马尔可夫性导致仅依赖当前观测难以保持时序一致性、闭环纠错能力有限导致误差累积、端到端动作微调削弱 VLM 高层语义表征。

Method: 提出分层长时程 VLA 架构，包含显式语言记忆模块。系统解耦为高层 VLM 与低层 VLA：高层 VLM 通过 VQA 训练范式进行语义推理，递归地以先前记忆为上下文锚点更新语言记忆与子任务指令；低层 VLA 基于子任务指令与视觉观测执行精确连续控制。

Result: 在多个仿真环境及真实机器人平台的 sim-to-real 实验中，显式语言记忆显著提升了 VLA 模型在复杂长时程任务上的成功率与鲁棒性，并提供可解释的决策语义描述。

Conclusion: 显式语言记忆模块通过将时序观测转化为带时序逻辑的文本记忆，有效解决长时程任务中的时序一致性与误差累积问题，同时保持决策过程的可解释性。

Classification reason: 论文提出面向长时程任务的 VLA 分层架构，通过显式语言记忆模块实现语义推理与连续控制，属于机器人具身智能中的规划与控制，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: 具身世界模型与长时程规划方向有相关性，但未涉及连续语言/多模态核心优先子主题，且机器人控制非用户重点。 Signals: world_model, VLA, long-horizon planning, language memory, hierarchical architecture
...
### [9] [DreamWAM: Beyond RGB Future Prediction for World Action Models](https://arxiv.org/abs/2608.04996)
...
TL;DR: 提出一种集成架构，将自适应经验回放、在线不确定性估计与安全 actor-critic 最优控制统一起来，在二维机器人导航任务上验证了耦合估计、安全过滤与回放的有效性。


<details>
  <summary>Details</summary>
Motivation: 安全 actor-critic 控制通常将障碍过滤、不确定性估计和经验回放视为独立模块，忽略了它们各自都会改变用于学习和控制的数据分布，缺乏统一的集成视角。

Method: 构建集成架构：不确定性估计更新控制障碍函数所用的障碍几何；过滤干预与估计残差决定回放优先级；critic 从实际执行动作而非名义动作学习。在带损坏障碍测量的二维机器人导航任务上，比较六种组件匹配配置。

Result: 在极端压力测试（乘数6.0）中，集成配置零接触并在全部五个评估种子到达目标，平均代价7.63±0.44，障碍信念RMSE为3.52±0.55cm；不确定性消融也零接触但仅四/五种子到达目标。另给出有限训练界与鲁棒障碍条件。

Conclusion: 结果支持在该基准上耦合估计、安全过滤与回放的有效性；更广泛的安全性与收敛性结论仍需进一步研究。

Classification reason: 论文面向机器人导航的安全 actor-critic 最优控制，涉及控制障碍函数、不确定性估计与经验回放，属于具身智能/机器人规划控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: This paper focuses on safe actor-critic control with experience replay, which is outside your core interests in continuous multimodal models, autoregressive generation, and world models for video. Signals: safe actor-critic, control barrier function, experience replay, uncertainty estimation, robot navigation
...
### [12] [Learning When to Stop: Prefix-Optimal Dynamic Diffusion Policies for Continuous Control](https://arxiv.org/abs/2608.05084)
...
TL;DR: 提出 Prefix-Optimal Generative Policies (POGP) 框架，在扩散策略的每个中间去噪步骤学习前缀价值函数，通过 Bellman 式递归实现自适应提前停止，在 MuJoCo 环境中将去噪迭代次数减少约 2.7 倍且保持近满任务性能。


<details>
  <summary>Details</summary>
Motivation: 扩散策略在连续控制中表现强大，但其迭代去噪过程带来显著计算瓶颈。现有方法无法根据每个动作的难度自适应调整去噪步数，同时保持任务性能。

Method: 提出 POGP 框架，在去噪链的每个中间步骤学习前缀价值函数，通过 Bellman 式递归定义。该价值函数有两个作用：作为辅助训练目标促使中间输出成为高质量动作，以及在测试时提供停止规则，当额外去噪步骤不太可能带来有意义改进时终止去噪。

Result: 在四个 MuJoCo 环境和 12 个基线的对比中，POGP 将所需去噪迭代次数减少约 2.7 倍，同时保持近满任务性能；相比最先进的动态扩散基线，前缀训练还使最终任务性能提升约 3.5%。

Conclusion: 监督中间去噪步骤不仅有助于自适应提前停止，还能作为辅助目标提升学习策略的质量，为扩散策略的高效推理提供了新思路。

Classification reason: 论文提出用于连续控制任务的扩散策略（diffusion policy），通过前缀价值函数实现自适应提前停止去噪，属于机器人/具身智能中的策略学习与控制，映射到具身世界模型与规划控制这一规范子主题。

Importance reason: Focuses on diffusion policy efficiency for robot control, which is only tangentially related to your core interests in continuous multimodal models and video/world-model generation. Signals: diffusion policy, adaptive early stopping, prefix value function, MuJoCo, inference efficiency
...
### [13] [CofactVLA: Deconfounding Vision-Language-Action Models via Counterfactual Intervention](https://arxiv.org/abs/2608.04396)
...
TL;DR: CofactVLA 提出一种因果干预框架，通过语言掩码反事实分支、动作级正交投影引导与特征级反事实协方差缩减，消除 VLA 模型中的视觉混淆偏差，在仿真与真实机器人实验中取得显著提升。


<details>
  <summary>Details</summary>
Motivation: VLA 模型在机器人操作中常出现"视觉主导"现象：由于密集视觉流与稀疏语言指令之间的模态不平衡，策略会过度拟合突出的视觉混淆因素（如显著物体或熟悉布局），绕过原始语言指令，导致因果混淆。

Method: 将动作生成形式化为双路径去混淆图（DDG），提出 CofactVLA 因果干预框架。在单次前向传播中动态构建语言掩码反事实分支，通过两个协同机制消除视觉混淆：动作级正交投影引导（OPG）在连续 flow matching 中将事实速度场几何投影远离反事实视觉偏差；特征级反事实协方差缩减（CCR）通过惩罚协方差差的正特征空间来去混淆潜在表征。

Result: 在多个仿真基准上达到新的 SOTA；真实机器人实验中，在分布外场景下取得 52.3% 的绝对成功率提升，验证了方法的因果有效性。

Conclusion: CofactVLA 通过因果干预有效缓解了 VLA 模型的视觉主导偏差，使语言指令真正成为动作生成的因果驱动因素，显著提升了模型的泛化能力。

Classification reason: 论文提出 CofactVLA，针对视觉-语言-动作（VLA）模型在机器人操作中的视觉主导偏差进行因果干预，属于具身智能与机器人动作生成/规划控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Strong causal deconfounding method for VLA world models, but robotics focus is peripheral to core continuous multimodal and generation priorities. Signals: world model, VLA, counterfactual intervention, flow matching, causal deconfounding
...
### [14] [Faster-WAM: Efficient Inference-Time Future Conditioning for Robust World Action Models](https://arxiv.org/abs/2608.04404)
...
TL;DR: Faster-WAM 提出一种高效的推理时未来条件化世界动作模型，通过 SparseMoT 稀疏视频-动作交互和 Interval KV-Fusion 多深度未来表征聚合，在保持未来感知鲁棒性的同时大幅降低计算开销，在 LIBERO-Plus 等基准上取得更优的性能-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型（WAM）存在根本性两难：Joint-WAM 在推理时保留未来感知表征但计算成本高昂，而高效替代方案在推理时移除未来建模，可能丧失时序推理带来的鲁棒性收益。作者希望证明推理时未来条件化对分布偏移下的泛化至关重要，并设计一种既保留未来表征又避免昂贵视频-动作交互的高效方案。

Method: Faster-WAM 引入稀疏未来条件化框架：未来表征只计算一次并在整个动作去噪过程中选择性复用。具体包含两个组件：SparseMoT 用稀疏视频-动作交互替代逐层融合，仅在紧凑的网络阶段子集上进行交互；Interval KV-Fusion 在不增加注意力复杂度的前提下聚合多深度未来表征。

Result: 在分布外 LIBERO-Plus 基准上，Faster-WAM 相比 Fast-WAM 将成功率从 49.14% 提升到 73.57%，同时比 Joint-WAM 快 2.21 倍；在 LIBERO 和 RoboTwin 2.0 上达到 SOTA，并在真实世界操作中展现出强鲁棒性。

Conclusion: 推理时未来条件化对 WAM 在分布偏移下的泛化至关重要，Faster-WAM 通过稀疏未来条件化框架在保持未来感知鲁棒性的同时显著提升效率，实现了更优的性能-效率权衡，验证了未来表征在机器人操作世界模型中的关键作用。

Classification reason: 论文提出 Faster-WAM，一种面向机器人操作的高效世界动作模型（World Action Model），通过稀疏未来条件化提升分布偏移下的泛化能力，属于具身智能/机器人操作中的世界模型与动作规划方向，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Strong efficiency-focused world action model with inference-time future conditioning, directly relevant to world model and efficient inference priorities. Signals: world model, efficient inference, robot manipulation, future conditioning, sparse interaction
...
### [15] [MobileWAM: Bridging World Action Models to Mobile Manipulation with Chain-of-Foresight](https://arxiv.org/abs/2608.04657)
...
TL;DR: MobileWAM 将基于视频扩散的世界动作模型（WAM）从桌面操作扩展到移动操作，采用 mixture-of-transformers 架构融合视频扩散 Transformer 与轻量动作专家，并通过 Chain-of-Foresight 中间表征逐级预测未来潜空间块来稠密化监督，在 ManiSkill-HAB 与真实 ARX Lift2 机器人上超越 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型（WAM）局限于桌面操作，而移动操作需要同时进行移动与全身操作，且面临场景级动态。当前移动操作策略仍依赖对动态不敏感的视觉编码器与手工设计的协调机制，缺乏利用互联网规模运动先验的能力。

Method: 提出 MobileWAM 架构：将预训练的视频扩散 Transformer 与轻量动作专家通过逐层联合注意力融合，把互联网规模运动先验转化为全身控制。动作专家的每个前馈层采用三专家混合（共享、移动、操作专家），由动作 token 中的运动意图软路由。提出 Chain-of-Foresight（CoF）：中间表征顺序预测未来潜空间块链，每步以前一步为条件，配合解耦的视频-动作去噪方案稠密化监督。推理时仅保留当前帧编码器，丢弃预测链与视频生成。

Result: 在 ManiSkill-HAB 基准上超越现有移动操作策略 SOTA，并在真实 ARX Lift2 移动操作机器人上微调，在多种任务上展现出强泛化能力。

Conclusion: MobileWAM 成功将世界动作模型从桌面操作扩展到移动操作场景，通过混合专家架构与 Chain-of-Foresight 稠密监督，实现了移动与操作的全身协调控制，验证了视频生成先验在移动操作中的有效性。

Classification reason: 论文提出 MobileWAM，将基于视频生成的世界动作模型（WAM）扩展到移动操作机器人，涉及动作条件生成、全身控制与具身规划，属于具身世界模型与规划控制方向，映射到 [CANONICAL] embodied_world_model。

Importance reason: Bridges video diffusion world models to mobile manipulation with a novel Chain-of-Foresight supervision, directly advancing world-model-driven control. Signals: world model, video diffusion, mobile manipulation, mixture-of-transformers, chain-of-foresight
...
Main category: cs.AI, cs.MA, eess.SY
...
TL;DR: 本文提出一个物理约束的基准来评估 LLM 规划智能体在信息物理系统中的规划架构有效性，通过智能电网需求响应系统验证了规划架构对结果有实质影响、执行保真度需要超越模式一致性、以及可行性预筛可显著降低遗憾值。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 规划智能体评估主要关注任务是否成功或计划是否被遵循，但在战略信息物理系统中，更关键的问题是规划架构在自主参与者响应和物理约束下是否仍然合适。需要一个受控、物理约束的基准来系统评估不同规划架构。

Method: 构建一个物理约束的基准，围绕规划诱导的控制轨迹展开。在包含40个异构产消者和独立模拟径向馈线的智能电网需求响应系统中实现预定义、顺序、分层和搜索四种执行器。LLM 被限制为类型化策略声明和短操作员消息，调度构建、产消者动力学和潮流保持为显式代码。使用配对强制模式反事实、公共随机响应抽取和事件级截止可行性协议。

Result: 规划架构显著改变结果：强制搜索在全部五个基线种子中为 oracle。执行保真度需要超越模式一致性：目标替换使一致性保持1.0但电压缺口增加2.68倍。144场景576回合库中四种架构中有三种有可行 oracle。预指定压力保持岭平均遗憾90.7，无超过固定顺序的可检测价值；应用已知截止可行性预筛后遗憾降至29.0，比固定顺序改善61.1%。全可行消融未超过固定搜索，将剩余挑战定位在可行内质量选择。五模型扩展区分了压力条件、状态盲和不变声明者。

Conclusion: 规划架构选择对信息物理系统中的 LLM 智能体结果有实质性影响，执行保真度评估需要超越简单的模式一致性，可行性预筛能显著降低遗憾值，而剩余挑战在于可行方案内的质量选择。实时可行性应被概率化处理。

Classification reason: 论文研究 LLM 智能体在信息物理系统（智能电网需求响应）中的规划策略评估，涉及规划架构、执行器与物理过程交互，属于具身/物理环境中的规划控制，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Focuses on LLM planning in cyber-physical systems, which is tangential to the user's core interests in continuous multimodal models and video/world-model generation. Signals: LLM agent planning, cyber-physical systems, smart grid, planning architecture evaluation, world model (weak link)
...
### [17] [Deltoris: Enabling Real-time VLA Inference in Embodied AI via Bit-level Sparsity and Speculative Inference](https://arxiv.org/abs/2608.04428)
...
TL;DR: Deltoris 是一个面向扩散式 VLA 模型的算法-硬件协同设计框架，通过时序感知的比特级稀疏算法与投机推理技术，实现具身智能边缘设备上的实时推理，相比移动 GPU 最高提速 34.2 倍。


<details>
  <summary>Details</summary>
Motivation: 扩散式 VLA 模型在具身智能中具有优越的动作质量与泛化能力，但计算密集且需在 50-200Hz 的高控制频率下运行，对边缘设备的延迟和能耗提出严格约束，亟需高效推理方案。

Method: 提出 Deltoris 算法-硬件协同设计框架：(1) 利用连续输入的时间相似性，提出时序感知比特稀疏算法，仅计算连续输入间的差异，消除冗余比特级运算；(2) 提出投机推理技术，将数据加载分摊到多个控制步，缓解额外片外流量；(3) 协同设计专用加速器，采用定制的一维 systolic 比特串行 PE 阵列，消除 PE 工作负载不均衡。

Result: 相比移动 GPU 最高提速 34.2 倍，相比先前加速器提速 6.1 倍，同时保持相当的精度。

Conclusion: Deltoris 通过算法与硬件协同设计，有效解决了扩散式 VLA 模型在边缘设备上的实时推理瓶颈，为具身智能的高频控制提供了高效可行的推理方案。

Classification reason: 论文聚焦具身智能中的 VLA（视觉-语言-动作）模型高效推理，属于具身智能与动作控制范畴，映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Algorithm-hardware co-design for diffusion VLA inference is valuable but tangential to the user's core continuous/multimodal generation focus. Signals: diffusion VLA, bit-level sparsity, speculative inference, edge acceleration, embodied AI
...
### [18] [PhysMind: From Video to Executable Worlds for Training-Free Physical Reasoning](https://arxiv.org/abs/2608.04575)
...
Importance: 值得关注 (69.85) - 未入选精读

TL;DR: PhysMind 是一个免训练智能体框架，从视频中构建可复用的可执行物理世界，通过物体分割、网格重建和 6D 位姿跟踪恢复动态场景，并拟合连续时间动力学与潜在物理参数，从而无需展开时间步模拟器即可回答物理推理与反事实问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLM）难以可靠地理解视频中的物体运动、交互与干预响应，在物理推理和反事实预测上表现不佳。作者希望构建一个无需训练、可复用的可执行世界模型来提升物理推理能力。

Method: PhysMind 采用免训练智能体框架：首先通过物体分割、网格重建和 6D 位姿跟踪恢复时序一致的动态场景；然后拟合解析的连续时间动力学模型和潜在物理参数，无需展开时间步模拟器；给定问题时，检查、延续或编辑该世界，并从生成的轨迹和交互中得出答案。

Result: 相比同一 VLM 的直接思维链（CoT）推理，PhysMind 在 CLEVRER 上准确率提升 38.23 分，在 Physion++ 上提升 8.08 分；在反事实问题上，超过最强 VLM 基线 GPT-5.5 达 19.25 分。

Conclusion: PhysMind 表明通过构建可执行的物理世界模型，可以显著提升 VLM 的物理推理能力，尤其在反事实推理上大幅超越直接推理的 VLM 基线，验证了"从视频到可执行世界"这一范式的有效性。

Classification reason: 论文构建可执行的物理世界模型，从视频恢复动态场景并拟合连续时间动力学与物理参数，用于物理推理与反事实问答，属于可交互与物理世界建模（interactive_physical_world）这一 CANONICAL 子主题。

Importance reason: Bridges video understanding and world models via executable physical worlds, directly matching the top-priority world_model and continuous multimodal reasoning directions. Signals: world model, video understanding, physical reasoning, training-free agent, 6D pose tracking
...
### [19] [TIDE: A Physically Diverse 3D Turbulence Benchmark Dataset for Advancing Scientific Machine Learning](https://arxiv.org/abs/2608.04222)
*Yilong Dai, Yiming Sun, Yiheng Chen, Shengyu Chen, Peyman Givi, Xiaowei Jia, Runlong Yu*

Main category: physics.flu-dyn, cs.LG
...
TL;DR: TIDE 是一个 256^3 的 3D 不可压缩湍流 DNS 语料库与基准，包含 15 种配置、独立系综、压力场与方程级验证，提供五个任务、标准化基线、受控泛化划分及物理保真度指标，用于评测科学机器学习模型在 3D 湍流动力学上的表现。


<details>
  <summary>Details</summary>
Motivation: 湍流是物理动力学机器学习的核心测试平台，但其现有研究多停留在 2D，而 3D 湍流物理本质不同且模拟成本高昂；现有 3D 资源通常每个配置仅提供一个实现，难以区分模型是学到了动力学还是拟合了单一流动的统计特征。

Method: 构建 TIDE 基准：256^3 DNS 语料库，覆盖 8 个受控轴上的 15 种配置，提供独立系综、压力场与方程级验证；设计五个任务、标准化学习基线、受控泛化划分，并引入物理保真度指标（除逐点误差外）来评测模型。

Result: 在主要预测配置上，当前学习模型仅勉强优于持久性预测，且误差约为给定真实方程的光谱求解器的两倍；更低的逐点误差可能伴随小尺度动力学严重失真，说明精度并不保证物理保真度；泛化实验显示多数状态转移反映训练覆盖不足，而 forced-to-decay 迁移暴露了缺失的条件变量。

Conclusion: TIDE 使精度、保真度与条件化这三方面的差距成为可量化的开放问题，为科学机器学习在 3D 湍流动力学上的进展提供了标准化评测平台。

Classification reason: 论文提出 3D 湍流 DNS 基准数据集与评测任务，用于推动物理动力学上的科学机器学习，属于物理世界建模与神经仿真器范畴，映射到 CANONICAL 子主题 interactive_physical_world（可交互与物理世界建模）。

Importance reason: Strong benchmark for physical world modeling but focuses on turbulence physics rather than the user's core multimodal generation and continuous representation priorities. Signals: world model, neural simulator, physical fidelity, 3D turbulence benchmark, generalization splits
...
### [20] [Helping Music Co-Creation Agents 'Listen' Well: Hierarchical Self-Supervised World Models for Understanding and Generation](https://arxiv.org/abs/2608.04378)
*Scott H. Hawley*

Main category: cs.SD, cs.LG, eess.AS
...
TL;DR: 本文提出一个用于符号音乐的层次化自监督世界模型：2.55M 参数的 Swin V2 编码器在 MIDI 钢琴卷帘图像上以 JEPA 式目标（音高/时间平移等变、掩码嵌入预测、分布正则化）训练，无需标签。冻结嵌入探测显示音乐属性解码层级与其时间尺度对应；条件 flow-matching 模型作为解码器，在像素空间流动，实现高保真重建与图形化提示的掩码修复，支持 CPU 上 2.8s 的实时交互建议。


<details>
  <summary>Details</summary>
Motivation: 协作式音乐智能体需要既能支持理解又能支持生成的内部表征，同时要足够灵活以保留人类主导权。现有方法往往依赖标签或音乐理论词汇，缺乏无监督的、层次化的世界模型来同时支撑理解与生成。

Method: 采用层次化自监督世界模型：Swin V2 编码器在 MIDI 钢琴卷帘图像上训练，使用 JEPA 式目标（音高/时间平移等变、掩码嵌入预测、分布正则化），无标签无音乐理论词汇。遵循 Representation AutoEncoder 范式，用条件 flow-matching 模型替代训练解码器，在像素空间从 PCA 降维条件流动；通过层级条件 dropout 实现可控变化与图形化提示的掩码修复。

Result: 冻结嵌入探测显示：乐句边界在最粗层级可读，音符密度与和声细节在最细层级可读；时序与乐句结构从自监督目标中涌现。小和弦监督头将联合和弦恢复从 .18 提升到 .54，从未监督的调性检测从 .16 提升到 .70。flow-matching 解码器在像素 F1 达到 0.996，CPU 上 2.8s、Apple MPS 上 0.6s 生成建议。

Conclusion: 层次化自监督世界模型能够同时支撑符号音乐的理解与生成，其表征层级与音乐时间尺度自然对应，且无需标签即可涌现时序与乐句结构。结合 LLM 大脑，该能力构成协作式音乐创作智能体的核心，服务于而非取代人类主导权。

Classification reason: 论文提出用于符号音乐的层次化自监督世界模型，采用 JEPA 式目标与 flow-matching 生成，属于世界模型方向。其核心是可交互的生成式世界模型（支持图形化提示与掩码修复），映射到 CANONICAL 的 interactive_physical_world（可交互与物理世界建模）。同时其 JEPA 自监督表征与 flow-matching 生成范式与 continuous_language_multimodal 方向的连续潜空间建模相关。

Importance reason: Applies world-model and flow-matching ideas to symbolic music, but the domain is off-priority and the technical novelty is moderate. Signals: world model, flow matching, self-supervised, symbolic music, JEPA
...
### [21] [MarsCast: Transfer Learning of AI Weather Foundation Models to Planetary Atmospheres](https://arxiv.org/abs/2608.05054)
*M.L. Carroll, J. Li, S.D. Guzewich, G.Villanueva, J.A. Caraballo-Vega, M.J. Frost*

Main category: cs.AI, cs.CV, cs.LG, astro-ph.EP
...
TL;DR: MarsCast 将地球天气预测基础模型 GraphCast 迁移到火星大气，利用火星气候数据库（MCD）进行零样本与微调预测，微调后模型在10个训练周期内即可捕捉火星昼夜循环与季节/垂直温度结构，实现长达10天的火星天气预测。


<details>
  <summary>Details</summary>
Motivation: 地球天气基础模型（如 GraphCast）在陆地预测上表现优异，但其对非地球行星大气（如火星）的适用性尚未被探索。火星天气预测对任务规划、沙尘暴风险缓解和未来人类探索至关重要，因此需要验证地球训练模型能否迁移到行星大气建模。

Method: 将 GraphCast 图神经网络天气预测模型适配到火星，使用火星气候数据库（MCD）提供的全球大气场（含垂直高度层，类似地球气压层）。评估零样本与微调两种模式，微调时使用 MCD 变量和大气层顶太阳辐射强迫，同时保持湿度恒定，预测火星温度和风场。

Result: 零样本预测能大致刻画当前条件但无法复现昼夜变化，且快速衰减到气候平均态。微调后模型在10个训练周期内即开始捕捉昼夜循环，可预测长达10天的季节与垂直温度结构。预测质量随训练样本量提升，并对季节初始化敏感。

Conclusion: 地球训练的 AI 天气模型可以被适配来模拟火星大气动力学，为快速行星天气预测提供路径，可支持任务运行、沙尘暴风险缓解和未来人类探索。

Classification reason: 论文将地球天气基础模型 GraphCast 迁移到火星大气动力学建模，属于物理世界建模/环境动力学建模范畴，映射到 CANONICAL 子主题 interactive_physical_world（可交互与物理世界建模）。

Importance reason: Novel cross-planet transfer learning for weather foundation models, but tangential to the user's core focus on continuous multimodal models and generative video/image systems. Signals: world model, transfer learning, GraphCast, planetary weather, zero-shot
...
TL;DR: 本文提出 CoCo（Counterfactual Consistency）框架，通过多步反事实一致性与动作-空间反事实一致性两种约束，减少动作条件世界模型对统计捷径（visual inertia 和重复运动模式）的依赖，从而增强动作可控性。同时提出 ARC 和 DE 两个评估指标及 Mini-SSMB 反事实评测基准。


<details>
  <summary>Details</summary>
Motivation: 动作条件世界模型常因未来帧可由视觉惯性和重复运动模式高度预测，而利用统计偏差走捷径，导致模型不真正依赖动作来驱动动态变化：不同动作产生相似未来，零动作下运动仍持续。作者认为仅注入动作特征不够，需要强制动作与观测在反事实变化下的一致性。

Method: 提出 CoCo 框架，包含两类互补约束：(1) 多步反事实一致性——约束 reference、inverse-action 与 zero-action 三种 rollout 的一致性；(2) 动作-空间反事实一致性——在镜像场景与变换动作下强制一致预测。另提出 ARC（Action Response Consistency）与 DE（Drift Energy）评估动作可控性，并构建 Mini-SSMB 同状态多动作反事实评测基准。

Result: 在 Mini-SSMB 上，完整模型 ARC_inv 达 0.412、ARC_ref 达 0.483，DE 相对基线降低 17.07%；在 VP2 视觉规划上取得 SOTA 平均成功率 73.1%；在 BAIR 和 RoboNet 上验证了视频预测质量保持且方法可跨模型设置迁移。

Conclusion: 通过反事实一致性约束可有效抑制动作条件世界模型对统计捷径的依赖，显著提升动作可控性，同时不损害视频预测质量，并能在不同模型设置间迁移。

Classification reason: 论文研究动作条件世界模型（action-conditioned world model），关注如何克服统计偏差以增强动作可控性，属于视频世界模型/动作条件视频生成范畴，映射到 [CANONICAL] video_world_model。

Importance reason: Directly targets action-controllable video world models with counterfactual consistency, a strong fit for the world model priority. Signals: world model, video generation, action controllability, counterfactual consistency, neural simulator
...
TL;DR: muSync-GS 是一个物理同步的驾驶视频合成框架，将降水衍生的路面条件、道路高程剖面与校准的车辆动力学模型耦合，在恶劣天气和道路几何危险场景下生成物理一致的驾驶视频及同步物理标注。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要大量高质量驾驶数据，但恶劣天气、低摩擦制动、不平整路面等安全关键场景难以大规模采集且成本高风险大。现有视频生成和 3D Gaussian 编辑方法只修改外观或几何，未与轮胎-路面交互和车辆动力学耦合，导致编辑后的视频轨迹与修改后的路况不一致。

Method: muSync-GS 提出物理同步框架：降水衍生的路面条件同时控制路面外观和轮胎摩擦系数；共享的道路高程剖面同时驱动可见路面几何编辑和车轴激励；校准的车辆模型预测速度、滑移率、法向载荷和俯仰角，用于构建 ego 相机轨迹和同步物理标注。

Result: 在 12 个 CarSim 测试用例上，速度 RMSE 为 0.0273 m/s，俯仰角 RMSE 为 0.0590 度，滑移率 RMSE 为 0.0101，每轮法向载荷 RMSE 为 26.61 N。重建场景实验进一步验证了在未见控制输入下准确复现车辆响应并与可控场景编辑和 ego 相机运动同步。

Conclusion: muSync-GS 能准确复现车辆在未见控制下的动力学响应，并将其与可控场景编辑和 ego 相机运动同步，为自动驾驶世界模型和生成式数据合成提供了物理一致的解决方案。

Classification reason: 论文提出 muSync-GS，将物理同步的车辆动力学与驾驶视频合成结合，用于自动驾驶世界模型数据生成，属于视频世界模型方向。同时涉及 3D Gaussian 场景编辑与视频生成，也匹配多模态生成方向。

Importance reason: Solid physics-synchronized driving video synthesis, but it is a niche application of world models rather than a core advance in continuous multimodal or autoregressive generation. Signals: world model, video generation, physics-synchronized, driving video synthesis, 3D Gaussian editing
...
### [24] [WorldCycle: Self-Verifiable Reinforcement Learning for Long-Horizon Video World Models](https://arxiv.org/abs/2608.04964)
...
Importance: 高优先级 (95.43) - Top 1 精读候选

TL;DR: WorldCycle 提出一种自验证强化学习框架，利用可逆动作循环为长时序视频世界模型提供无标注的长期正确性监督，通过空间闭合奖励与时间一致性奖励将动作学习为一致的状态算子，显著降低状态漂移并提升复合动作准确性。


<details>
  <summary>Details</summary>
Motivation: 交互式视频世界模型在长时序规划中至关重要，但存在累积误差问题。后训练方法（如 RL）虽可改善模型，却面临验证瓶颈：对任意动作序列，不存在真实未来状态来衡量长期漂移，导致无法有效监督长时序正确性。

Method: 核心洞察是可逆动作循环使验证成为可能：由动作及其逆动作组成的序列必须解析地回到初始状态，从而提供无标注的长时序正确性监督。WorldCycle 从普通动作序列构造闭合动作循环及其重复执行，优化两个互补奖励：空间闭合奖励（强制前向与反向镜像段之间的对称性）和时间一致性奖励（对齐重复循环执行间的状态）。

Result: WorldCycle 将状态返回漂移降低最多 44%，复合动作准确性相对基础模型提升近 4 倍。同时发布 CycleBench 诊断基准，用于评估复杂动作结构下的状态返回能力。

Conclusion: 通过自验证的强化学习，WorldCycle 使视频世界模型将动作学习为一致的状态算子而非记忆化的时序模式，并能泛化到基础模型处理不佳的分布外复合动作循环，为物理一致的世界模型奠定基础。

Classification reason: 论文提出 WorldCycle，通过自验证强化学习改进长时序交互式视频世界模型，属于视频世界模型的训练与优化，映射到 [CANONICAL] video_world_model 子主题。

Importance reason: Directly targets long-horizon video world model drift via self-verifiable RL, a core bottleneck for the user's world model priority. Signals: world model, video world model, reinforcement learning, long-horizon, state drift
...
### [25] [HelloWorld: Enabling Socially Interactive Characters in Video World Models](https://arxiv.org/abs/2608.05070)
...
TL;DR: HelloWorld 是一个支持用户与世界内角色进行社交交互的视频世界模型。用户按下按钮即可提示屏幕中的角色面向镜头回应（转头、挥手、点头或说简短问候）。论文提出自蒸馏流水线，在模型自身合成的数据上微调视频生成模型，使模型学习相机姿态条件而不降低交互质量；推理时引入免训练模块，通过调制 DiT 的交叉注意力掩码，将交互相关文本提示仅作用于按下窗口内的帧，实现角色响应的时间定位。同时构建 HelloWorldBench 基准（400 样本，含三个社交交互指标和三个常规指标）进行评测。


<details>
  <summary>Details</summary>
Motivation: 尽管视频世界模型近期取得了显著进展，但用户与世界内角色之间的社交交互仍不受支持。现有视频世界模型无法让用户与画面中的角色进行自然互动，如让角色面向镜头回应、挥手或说话。

Method: 提出自蒸馏流水线，在模型自身合成的数据上微调视频生成模型，每个合成片段同时包含社交交互和相机运动，使模型学习相机姿态条件而不降低交互质量。推理时引入免训练模块，通过调制 DiT 的交叉注意力掩码，使交互相关文本提示仅作用于按下窗口内的帧，实现角色响应的时间定位。

Result: 实验表明 HelloWorld 在交互质量上超越多种基线，同时保持最先进的画面美学质量和相机姿态跟随能力。构建的 HelloWorldBench 基准包含 400 个样本，涵盖三个社交交互指标和三个常规指标。

Conclusion: HelloWorld 成功实现了视频世界模型中的社交交互能力，通过自蒸馏和免训练注意力掩码调制，在保持生成质量的同时支持用户与角色间的自然互动，为视频世界模型增添了交互式社交维度。

Classification reason: 论文提出 HelloWorld，一个支持用户与世界中角色进行社交交互的视频世界模型，属于视频生成式世界模型范畴，映射到 [CANONICAL] video_world_model 子主题。

Importance reason: Novel interactive video world model with self-distillation and training-free temporal control, directly advancing the user's top-priority video world model and continuous multimodal directions. Signals: video world model, social interaction, self-distillation, camera-pose conditioning, training-free cross-attention modulation
...
### [26] [VoxStruct3D: Structure-Leading Flow Matching for Voxel-Space 3D MRI Synthesis](https://arxiv.org/abs/2608.04557)
...
TL;DR: VoxStruct3D 是一个体素空间的 flow matching 框架，用于高保真 3D MRI 合成。它通过 Volumetric Voxel Generator 直接建模全分辨率 MRI 体积，并引入 Structure-First, Image-Follows (SFIF) 策略，利用冻结的 3D 医学编码器和 StructVAE 提取结构 token 引导图像生成，在病理和健康 T1 脑 MRI 数据集上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 高保真 3D MRI 合成需要全局解剖一致性和细粒度体素级细节。虽然潜在扩散使体积生成可行，但其图像自编码器引入重建瓶颈，限制了最终体积中可恢复的精细细节。

Method: 提出体素空间 flow matching 框架 VoxStruct3D，使用 clean-data 预测目标直接建模全分辨率 MRI 体积。其 Volumetric Voxel Generator (VVG) 结合分解式 3D patch embedding、重叠上采样、时间调制残差细化与跳跃融合，使相邻 token 联合重建共享体素区域并抑制 patch 边界伪影。同时引入 Structure-First, Image-Follows (SFIF) 策略：冻结的预训练 3D 医学编码器和 StructVAE 提取紧凑结构 token 保留主导解剖结构，结构引导调度使结构轨迹领先于图像轨迹；Patch-Aligned RoPE 对齐不等 token 网格，非对称注意力实现结构到图像的单向引导。

Result: 在病理和健康 T1 加权脑 MRI 数据集上，VoxStruct3D 在特征分布对齐、样本多样性和感知质量方面取得最强综合性能，生成解剖一致且视觉真实的体积。

Conclusion: VoxStruct3D 通过体素空间 flow matching 结合结构引导策略，有效克服了潜在扩散的重建瓶颈，实现了高保真、解剖一致的 3D MRI 合成。

Classification reason: 论文提出 VoxStruct3D，一种体素空间 3D MRI 合成框架，属于 3D 体积数据生成任务，映射到 [CANONICAL] 的 three_d_generation（3D 生成与重建）子主题。

Importance reason: Medical MRI synthesis with a structure-leading flow matching design, but outside the user's core continuous/multimodal generation and world-model focus. Signals: flow matching, 3D generation, medical imaging, structure-guided generation, voxel-space synthesis
...
### [27] [Towards Valid B-Rep Generation: Training-Free Wireframe Anomaly Detection and Repair](https://arxiv.org/abs/2608.04955)
...
TL;DR: 提出 WDR（Wireframe Detection and Repair），一个无需训练的框架，在多阶段 B-Rep 生成的中间线框阶段进行几何与拓扑异常检测和修复，通过 VLM 粗筛与几何/拓扑检测器预测下游无效风险，并利用能量引导重采样与扩散引导实现测试时修复，提升最终 CAD 模型的有效性。


<details>
  <summary>Details</summary>
Motivation: 多阶段 B-Rep 生成依赖中间线框合成 CAD 模型，但线框中的几何与拓扑风险（自交、边塌缩、顶点断开）会传播导致最终 B-Rep 无效。重训大型生成模型来缓解这些失败在计算上不可行，因此需要一种无需训练的干预方案。

Method: 提出 WDR 框架，包含两个核心模块：(1) Geometric-Topology Anomaly Detector (GTAD)，结合并行 VLM 粗筛与几何/拓扑检测器预测下游无效风险并路由生成到专用分支；(2) Energy-Guided Geometric-Topology Repair (EGGTR)，通过检测器触发的引导重生成，在几何与拓扑分支中执行能量引导重采样和无需训练的扩散引导，可无缝集成到自回归与扩散管线中。

Result: 大量实验表明，WDR 在保持合成 CAD 模型多样性与分布质量的同时，持续提升 kernel-checked 有效性（validity）。

Conclusion: WDR 通过在线框阶段进行训练无关的异常检测与修复，为提升 B-Rep 生成有效性提供了一种高效、可扩展的解决方案，无需重训大型生成模型。

Classification reason: 论文关注 B-Rep（边界表示）CAD 模型生成中的线框异常检测与修复，属于 3D/CAD 几何生成范畴，映射到 [CANONICAL] three_d_generation（3D 生成与重建）子主题。

Importance reason: Focuses on CAD/B-Rep generation with training-free repair, which is outside the user's core continuous multimodal, video/image generation, and world model priorities. Signals: training-free, anomaly detection, diffusion guidance, autoregressive pipeline, CAD generation
...
### [28] [Free-Lunch Augmentation by Revisiting Diffusion-Based Data Generation for Cross-Domain Few-Shot Object Detection](https://arxiv.org/abs/2608.04394)
...
TL;DR: 本文提出 SITN（Selective Inpainting with Tailored Noise）方法，通过扩散模型合成数据来增强跨域少样本目标检测（CDFSOD）。作者将域差距分解为视觉差距与语义差距：视觉差距通过添加减弱噪声缓解，语义差距通过背景 inpainting 弥合。在 6 个 CDFSOD 数据集和 4 个跨域少样本分割数据集上取得 SOTA。


<details>
  <summary>Details</summary>
Motivation: 跨域少样本目标检测（CDFSOD）面临显著域差距与数据稀缺的双重挑战。作者重新审视数据增强这一自然但未被充分探索的路径，尝试直接用扩散模型合成数据补充有限训练样本，但发现由于大域差距，现有扩散方法效果不佳甚至低于原始图像。

Method: 将域差距分解为视觉差距与语义差距分别分析。视觉差距方面，发现扩散模型在专家域上无法区分噪声与有用信息，通过添加减弱噪声缓解；语义差距方面，发现背景语义的域间差距远小于前景语义，通过背景 inpainting 弥合。据此提出 SITN，包含 Generation Module（添加定制噪声）与 Selection Module（动态选择 inpainting 区域），针对不同域差距动态采取不同合成策略。

Result: 在 6 个 CDFSOD 数据集和 4 个跨域少样本分割（CDFSS）数据集上验证了方法的有效性，能够合成有帮助的数据，取得新的 SOTA 性能。

Conclusion: 通过将域差距分解为视觉与语义两个维度并分别设计缓解策略，扩散模型可以有效地为跨域少样本检测合成有益的训练数据，验证了数据增强在 CDFSOD 中的潜力。

Classification reason: 论文利用扩散模型进行数据合成与背景 inpainting（背景修复）来增强跨域少样本目标检测训练数据，核心方法涉及图像编辑/Inpainting 技术，映射到 [CANONICAL] visual_editing（图像与视频编辑）子主题。

Importance reason: Focused on few-shot object detection with diffusion-based augmentation, which is outside the user's core continuous/multimodal generation and world-model priorities. Signals: diffusion data augmentation, cross-domain few-shot detection, domain gap decomposition, background inpainting, tailored noise
...
### [29] [When Diffusion Models Forget Who You Are: Identity Preservation in Face Inpainting under Large Occlusions](https://arxiv.org/abs/2608.04820)
...
TL;DR: ReSem-Face 是一个级联扩散框架，通过显式的身份条件语义先验实现多参考人脸修复，在严重遮挡下保持身份保真度并兼容文本驱动的编辑。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在人脸修复中已取得良好视觉质量，但在显著遮挡和冲突文本引导下保持身份保真度仍是重大挑战。现有方法在像素缺失时缺乏足够的语义约束，导致身份重建不稳定。

Method: 提出 ReSem-Face 级联扩散框架：从多个参考图像中蒸馏代表性身份特征以重建缺失语义区域，形成显式的身份条件语义先验；通过多流条件架构将该先验注入扩散过程，在像素缺失时提供强语义约束，稳定身份重建并兼容提示驱动的编辑。

Result: 在 CelebAHQ-IDI-5 和 VGGFace2 数据集上，ReSem-Face 在严重语义掩码下实现更可靠的身份保持补全，并相比代表性基线提升了文本控制编辑质量。

Conclusion: 显式的身份条件语义先验结合多流条件扩散架构能有效解决人脸修复中的身份保真问题，同时保持与文本驱动编辑的兼容性。

Classification reason: 论文研究人脸图像修复（face inpainting）中的身份保持问题，属于图像编辑/修复范畴，映射到 [CANONICAL] visual_editing（图像与视频编辑）子主题。

Importance reason: Specialized face inpainting paper with limited overlap with the user's core focus on continuous multimodal models and autoregressive generation. Signals: face inpainting, diffusion framework, identity preservation, occlusion handling, text-driven editing
...
### [30] [DAC-Pose: Dual-Agent Collaborative Framework for Pose-Guided Human Generation](https://arxiv.org/abs/2608.04622)
...
TL;DR: DAC-Pose 提出一种双智能体协作框架用于姿态引导的人体生成，通过 Prior Semantic Reasoning (PSR) 智能体进行语义推理以推断不可见区域属性，以及 Discrepancy-Aware Visual Encoding (DAVE) 智能体量化视角引起的空间错位并反馈给生成过程，在 DeepFashion 和 Market-1501 上验证了在剧烈视角变化下保持纹理对齐与身份一致性的优势。


<details>
  <summary>Details</summary>
Motivation: 传统姿态引导人体生成方法在剧烈视角变化下会产生严重视觉伪影，因为它们缺乏逻辑推断不可见区域和建模复杂空间形变的认知能力。作者希望引入 AI 智能体范式，使系统具备语义推理能力而非被动的像素级映射。

Method: DAC-Pose 将单视角人体生成重构为协作式双智能体系统：PSR（Prior Semantic Reasoning）智能体作为认知引擎，利用协作推理推断不可见区域的细粒度属性；DAVE（Discrepancy-Aware Visual Encoding）智能体作为视觉感知智能体，量化并编码视角引起的空间错位，持续将稳健的空间约束反馈到生成过程中，形成语义推理与视觉感知之间的自主反馈回路。

Result: 在 DeepFashion 和 Market-1501 基准上的大量实验验证了该智能体驱动范式的优越性，尤其在剧烈视角变化下在纹理对齐和身份一致性方面表现突出。

Conclusion: DAC-Pose 证明了将生成任务重构为多智能体协作系统能够有效提升姿态引导人体生成在极端视角变化下的保真度，为生成式图像合成中的语义推理范式提供了新思路。

Classification reason: 该论文研究姿态引导的人体生成（pose-guided human generation），属于人体动作/姿态条件下的图像生成任务，与 [CANONICAL] 数字人与动作生成（avatar_motion_generation）高度相关，直接复用该子主题。

Importance reason: Pose-guided human generation with agent-based reasoning is a niche application, weakly aligned with the user's core focus on continuous multimodal models, autoregressive generation, and world models. Signals: agent-driven generation, pose-guided human generation, semantic reasoning, spatial misalignment, identity consistency
...
### [31] [Coupled Continuous-Discrete Generation for Scene Text Image Super-Resolution](https://arxiv.org/abs/2608.04525)
...
TL;DR: DualTSR 将场景文本图像超分辨率（STISR）建模为耦合的连续-离散生成问题：条件 flow matching 恢复连续图像 latent，吸收态离散扩散重建文本 token，两者共享多模态 transformer 骨干并在生成过程中交互，无需外部 OCR 先验。在 CTR-TSR 和 RealCE 上取得最佳 FID/LPIPS/ACC/NED，相比 DiffTSR 在 X4 下 ACC 提升 12.78 个百分点，参数量从 1.23B 降至 203M，延迟从 13.3s 降至 132ms。


<details>
  <summary>Details</summary>
Motivation: 现有 STISR 系统通常依赖外部生成的先验或分离的图像与文本模型，导致误差传播和多阶段推理成本高。作者希望构建一个统一框架，在生成过程中让图像与文本状态相互交互，无需外部 OCR 先验。

Method: DualTSR 将 STISR 形式化为耦合的连续-离散生成：条件 flow matching 恢复连续图像 latent，吸收态离散扩散重建文本 token。两者共享一个多模态 transformer 骨干，使演化中的图像与文本状态在生成全程交互，推理时无需外部 OCR 先验。

Result: 在 CTR-TSR 上取得最佳 FID、LPIPS、ACC、NED（X2 与 X4）；在对齐的 RealCE 子集上取得最佳 FID、ACC、NED 且 LPIPS 有竞争力。相比 DiffTSR 在 X4 下 ACC 提升 12.78 个百分点，参数量从 1.23B 降至 203M，端到端延迟从 13.3s 降至 132ms。

Conclusion: DualTSR 通过耦合连续-离散生成，在准确性和效率上均优于现有 STISR 方法，验证了统一生成框架在场景文本图像超分辨率中的有效性。

Classification reason: 论文提出 DualTSR，用条件 flow matching 恢复连续图像 latent 并结合离散扩散重建文本 token，实现场景文本图像超分辨率。核心是图像生成/重建任务，属于多模态生成方向；同时涉及连续 latent 与 flow matching，与连续语言多模态方向相关。映射到 [CANONICAL] 文生图与图像扩散（图像扩散/生成方法）作为主方向。

Importance reason: Solid method for scene text SR, but niche application with limited direct impact on your core continuous multimodal/world-model priorities. Signals: coupled continuous-discrete generation, flow matching + discrete diffusion, efficient 203M params, 100x latency reduction, no external OCR prior
...
### [32] [NuclearDiffusion: Text-to-Image Foundation Models for Learning Nuclear Energy Concepts](https://arxiv.org/abs/2608.04030)
*Mohammed I. Radaideh, Jeremy Moon, Andre Gala-Garza, Emma Son, Yug Shah, Majdi I. Radaideh*

Main category: cs.LG, cs.CY, cs.CV, cs.AI, cs.GR
...
TL;DR: 本文首次系统研究核能领域文生图扩散模型的领域适配，构建1000张带标注的核能图像数据集，微调SDXL、SD-v3.5-Medium和Flux.1三个开源扩散模型，并与GPT-Image-2、Gemini、Midjourney等商业系统对比，发现微调效果高度依赖底层生成架构。


<details>
  <summary>Details</summary>
Motivation: 通用文生图基础模型缺乏核工程等专业领域的领域知识，常生成物理不正确或概念不一致的图像。本文旨在探索通过领域微调使扩散模型生成专业领域可信图像。

Method: 构建1000张带标注的核能图像数据集（覆盖反应堆、燃料循环、辐射等概念），对SDXL、SD-v3.5-Medium和Flux.1三个开源扩散模型进行微调，使用图像相似度指标和专家定性评估与零样本模型对比，并与三个商业系统进行横向比较。

Result: 微调显著提升SDXL的保真度，对SD-v3.5-Medium提升有限，对Flux.1无显著改善；GPT-Image-2和Gemini对宽泛核概念生成可信，但在专业工程提示上失败，而微调后的开源模型在专业提示上更准确。

Conclusion: 领域特定微调是构建专业领域可信生成式AI工具的有效途径，但适配效果强烈依赖底层生成架构而非模型规模，为专业领域文生图应用提供了实践路径。

Classification reason: 论文研究核能领域文生图扩散模型的领域适配微调，属于文本到图像生成方向，映射到 [CANONICAL] 的 text_to_image_diffusion 子主题。

Importance reason: Domain-specific fine-tuning study for nuclear text-to-image, but outside the user's core continuous/multimodal generation and world-model focus. Signals: text-to-image, diffusion fine-tuning, domain adaptation, SDXL, Flux.1
...
### [33] [CSGen: A Multi-Domain Curvilinear Structure Generation Model via Hierarchical Multimodal Diffusion](https://arxiv.org/abs/2608.04655)
...
TL;DR: CSGen 是一个分层多模态扩散模型，用于生成与多种控制条件精确对齐的高保真曲线结构图像，通过多域数据集、分层渐进控制策略和稀疏感知损失重加权机制提升结构精度与视觉真实感。


<details>
  <summary>Details</summary>
Motivation: 曲线结构分析是多媒体中的重要基础任务，但可控生成具有精确曲线结构对象的图像仍是开放挑战。现有方法难以在多种控制条件下同时保证结构拓扑完整性与视觉真实感。

Method: CSGen 基于三个关键创新：1) 构建包含 5 个域、7 种标注类型、超过 24K 样本的多域多模态数据集训练统一生成模型；2) 提出分层渐进控制策略，通过分阶段信号注入将拓扑线索与视觉上下文解耦，缓解语义漂移并保证稀疏结构的拓扑完整性；3) 设计稀疏感知损失重加权机制，应对曲线结构的极端稀疏性，增强优化过程中对细薄脆弱结构的关注。

Result: 大量实验表明 CSGen 生成的图像具有优越的结构精度和视觉真实感，显著提升下游分割性能，并在多样提示下保持鲁棒性。

Conclusion: CSGen 被验证为一种可扩展、以数据为中心的范式，适用于多种多媒体应用中复杂曲线结构的分析。

Classification reason: 论文提出 CSGen，一个基于分层多模态扩散的曲线结构图像生成模型，属于图像扩散生成方法，映射到 [CANONICAL] 文生图与图像扩散 子主题。

Importance reason: Focuses on curvilinear structure generation for multimedia analysis, which is outside the user's core interests in continuous multimodal models, autoregressive/diffusion video generation, and world models. Signals: multimodal diffusion, controllable generation, sparse structure generation, downstream segmentation, multi-domain dataset
...
### [34] [UniWorld-View: Large-Baseline View Synthesis via Video Diffusion Models](https://arxiv.org/abs/2608.04701)
...
TL;DR: UniWorld-View 提出一个统一框架，将显式 3D 几何引导与视频扩散模型结合，实现从单目输入进行可控的大基线新视角合成，通过遮挡感知点云渲染提供几何先验，支持极端相机运动下的高保真视图生成及多视角视频输出。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上大量随意拍摄的单目视频和图像为沉浸式内容创作提供了宝贵素材，但从稀疏观测生成新视角可极大提升用户体验。然而在输入覆盖极度有限时，基于重建的方法（如 NeRF、3DGS）严重退化且无法显式处理遮挡，生成式方法则因几何引导不准确而难以处理大基线视图合成。

Method: UniWorld-View 将显式 3D 引导与生成式扩散建模集成到统一框架中。几何引导通过遮挡感知点云渲染策略获得，该策略解决可见性歧义并为扩散合成提供准确先验；将该渲染策略与强大的视频扩散骨干结合，实现精确相机控制和几何一致的视图生成。

Result: 在 WorldScore 基准和零样本 NVS 基准上的实验表明，UniWorld-View 在可控性、几何一致性和视觉保真度方面均表现有效，即使在极端相机运动和大基线变化下也能生成高保真新视图，并可提供多视角视频用于下游动态 3DGS 重建。

Conclusion: UniWorld-View 通过将显式 3D 几何引导与视频扩散模型相结合，有效解决了大基线新视角合成中稀疏输入和遮挡处理难题，为从单目输入生成几何一致、可控的高保真视图提供了统一解决方案。

Classification reason: 论文利用视频扩散模型进行大基线新视角合成，属于视频扩散生成方法，映射到 [CANONICAL] 文生视频与视频扩散 子主题。同时涉及视频世界模型/未来预测，但主要贡献是生成式视图合成，故主方向为多模态生成。

Importance reason: 将视频扩散模型与显式3D几何先验结合用于大基线新视角合成，与视频生成世界模型方向高度相关，但非核心连续多模态主线。 Signals: video diffusion, world model, novel view synthesis, controllable generation, geometric guidance
...
### [35] [ContextMaster: Interactive Multi-Shot Video Creation via Fixed-Budget Sparse Context Routing](https://arxiv.org/abs/2608.04956)
...
TL;DR: ContextMaster 提出交互式多镜头视频创作（IMVC）框架，通过角色感知上下文表示、固定预算稀疏上下文路由和 ConstraintSink 机制，让一个统一模型在多个镜头间保持共享历史，同时控制去噪步骤的上下文读取成本，并采用两阶段特权上下文蒸馏实现少步推理。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型虽然支持生成、参考条件与编辑，但通常将这些操作作为固定输入的独立操作。实际创作是多镜头交互过程，需要模型在多个镜头间保持共享历史，同时不能让上下文读取成本随历史增长而膨胀。

Method: ContextMaster 采用角色感知上下文表示统一三种操作（文本生成、参考跟随、编辑），结合可复用干净上下文状态与固定预算稀疏上下文路由，并使用 ConstraintSink 保持任务约束可见。针对稀疏上下文访问与少步推理的双重挑战，提出两阶段特权上下文蒸馏框架：先通过一致性蒸馏从密集教师转移全上下文行为，再用分布匹配精炼部署 rollout。

Result: 在三种基本任务上相比专用基线提升了任务完成度与跨镜头一致性；用户研究验证了灵活组合工作流；模型在单 GPU 上达到 16 FPS。

Conclusion: ContextMaster 证明统一模型可以在固定预算稀疏上下文路由下实现交互式多镜头视频创作，兼顾任务约束保持、跨镜头一致性与高效推理。

Classification reason: 该论文提出 ContextMaster，一个统一的视频生成模型，支持文本生成、参考条件生成与编辑等多种操作，属于视频扩散生成范畴，映射到 [CANONICAL] 文生视频与视频扩散 子主题。

Importance reason: Solid engineering for multi-shot video creation, but the core sparse-context routing and distillation ideas are incremental for the user's priority on continuous latent/world-model video generation. Signals: video generation, autoregressive, controllable generation, efficient inference, video diffusion
...
### [36] [Poly-OPD: Heterogeneous Multi-Teacher On-Policy Distillation for Capability-Selectable Flow Models](https://arxiv.org/abs/2608.04349)
...
TL;DR: Poly-OPD 是一个异构多教师在线策略蒸馏框架，通过像素桥接、DINOv2 空间监督、梯度兼容性诊断与差距感知课程，将多个互补的开放文生图模型能力整合进一个紧凑的 flow-matching 学生模型，实现能力可切换的单一模型。


<details>
  <summary>Details</summary>
Motivation: 开放文生图模型往往具有互补优势（如一个偏好对齐美学更强、另一个组合指令遵循更忠实），但由于自编码器与噪声调度的差异，难以跨模型迁移这些优势。作者希望将多个异构教师模型的互补能力整合进一个紧凑的学生模型。

Method: Poly-OPD 通过像素桥接进行在线策略蒸馏：学生生成的图像由所选教师的编码器重新编码，并在教师噪声调度下按幅度匹配的噪声水平进行精炼；目标进一步在冻结的 DINOv2 空间中与学生匹配，实现跨不兼容潜空间的监督。为保留互补能力并避免跨教师干扰，使用梯度兼容性诊断组织适配器：注意力 LoRA 模块跨教师共享，前馈适配器保持教师特定。蒸馏期间采用差距感知课程，将更多训练分配给学生仍落后于教师的组合类别。

Result: 将 FLUX.1-dev 和 Z-Image 蒸馏到 2.5B SD3.5-Medium 学生模型，GenEval 从 67.3 提升到 73.3，超越两个更大的教师模型；DrawBench HPSv3 从 9.34 提升到 11.35，在可切换模型中整合了两种优势。

Conclusion: Poly-OPD 证明通过像素桥接与 DINOv2 空间监督可以克服异构教师之间不兼容的潜空间，将互补能力整合进紧凑学生模型，同时通过梯度兼容性诊断与差距感知课程避免跨教师干扰并高效利用训练资源。

Classification reason: 论文提出 Poly-OPD，通过多教师蒸馏将异构教师模型的能力整合到单一紧凑的 flow-matching 学生模型中，属于生成模型的蒸馏与高效化方向，映射到 [CANONICAL] efficient_generation（生成模型加速与高效化）。

Importance reason: 通过异构多教师蒸馏将多个文生图模型能力整合进单一紧凑flow-matching学生模型，与用户关注的多模态生成与模型高效性高度相关。 Signals: flow matching, 多教师蒸馏, 文生图, 模型压缩, 能力可切换
...
### [37] [Rethinking Pixel Mean Flows via Interval Denoiser](https://arxiv.org/abs/2608.04818)
...
TL;DR: 本文提出 Interval Denoiser，一个从 flow matching ODE 严格推导的潜空间无关生成框架，为中间轨迹状态建立精确解析映射，使网络可直接在像素上回归，并通过残差裁剪与时间采样课程实现少步生成，在 ImageNet 256x256 上达到 1-NFE FID 4.55、2-NFE FID 3.98。


<details>
  <summary>Details</summary>
Motivation: 现代扩散与流模型正逐步转向少步、潜空间无关（latent-free）生成，以绕过多步采样的计算开销和外部自编码器的重建瓶颈。然而现有方法缺乏严格的理论框架，且经验性的代数替换会导致梯度评估偏差。

Method: 从 flow matching ODE 出发推导 Interval Denoiser，为中间轨迹状态建立精确解析映射，证明预测位于低维流形上使像素级回归可解；通过避免经验代数替换正确隔离纯时间导数，保证精确一阶优化；并引入残差裁剪与时间采样课程以支持长区间训练。

Result: 在 ImageNet 256x256 上从零训练，无需感知损失，1-NFE 达到 FID 4.55，2-NFE 达到 FID 3.98。

Conclusion: Interval Denoiser 提供了潜空间无关少步生成的严格理论框架，通过精确解析映射与训练策略改进，在无需外部自编码器和感知损失的情况下实现了具有竞争力的少步生成性能。

Classification reason: 论文提出 Interval Denoiser，一种面向少步（few-step）潜空间无关（latent-free）图像生成的扩散/流匹配框架，核心目标是减少采样步数、提升生成效率，属于生成模型加速与高效化方向，映射到 [CANONICAL] efficient_generation。

Importance reason: Directly advances continuous latent-free flow matching with few-step generation, aligning with the user's top-priority continuous language/multimodal and efficient inference directions. Signals: flow matching, few-step generation, latent-free, continuous visual token, efficient inference
...
### [38] [STEP-OPD: Rethinking Output Targets and Internal Dynamics in On-Policy Distillation for Diffusion Models](https://arxiv.org/abs/2608.04887)
...
TL;DR: STEP-OPD 是一种面向扩散模型的 on-policy 蒸馏框架，通过将学习目标扩展到教师模型之外（利用任务教师与共享基座模型的 velocity 差作为额外学习方向），并对学生内部表征演化施加显式约束，从而提升多任务图像生成模型蒸馏的效果。


<details>
  <summary>Details</summary>
Motivation: 现有 on-policy 蒸馏（OPD）方法主要让学生匹配教师的输出 velocity，使教师成为优化目标的上限，且仅靠输出级监督无法约束学生逐层表征演化的过程，削弱了跨层渐进发展的能力迁移。

Method: STEP-OPD 将任务专用教师与共享基座模型之间的 velocity 差作为进一步学习的方向，并将该差值的缩放版本加到教师 velocity 上，实现超越教师的学习。同时，对齐学生与教师表征变化的方向和幅度，使学生学习网络块间表征的渐进变换方式。

Result: 在组合对齐、文本渲染和人类偏好等实验上，STEP-OPD 一致优于标准 OPD 方法。其中 DiffusionOPD 的 GenEval 分数从 0.927 提升到 0.961，同时 OCR 和所有偏好指标均有提升。统一学生模型在三个能力组上均超越对应的单任务教师。

Conclusion: 输出外推（output extrapolation）能够实现超越教师的学习，而表征变化对齐为学生内部变换提供了互补的引导，验证了同时扩展学习目标与约束内部动力学对蒸馏的有效性。

Classification reason: 论文提出 STEP-OPD，一种面向扩散模型的 on-policy 蒸馏（OPD）框架，用于将多个任务专用图像生成模型蒸馏为单一学生模型。蒸馏属于生成模型效率优化范畴，与 [CANONICAL] 子主题"生成模型加速与高效化"（efficient_generation）高度相关。

Importance reason: Solid distillation method for diffusion image generation, but tangential to your core focus on continuous multimodal models and video/world-model generation. Signals: diffusion distillation, on-policy distillation, image generation acceleration, model efficiency, beyond-teacher learning
...
### [39] [Simile Understanding in Text-to-Image Models: An Evaluation Framework](https://arxiv.org/abs/2608.04750)
*Luecheng Wang, Shintaro Ozaki, Hidetaka Kamigaito, Katsuhiko Hayashi, Jingun Kwon, Manabu Okumura, Taro Watanabe*

Main category: cs.CV, cs.CL, cs.MM
...
TL;DR: 本文提出一个可扩展的文生图模型明喻理解评测框架，包含受控明喻数据集、基于YOLO检测的自动定位指标，以及利用Diffusion Lens分析文本编码器中隐喻载体如何涌现的方法。实验发现多种架构的文生图模型存在一致的"字面化"失败模式，即把隐喻载体误当作对象本身。


<details>
  <summary>Details</summary>
Motivation: 文生图模型虽然能从明喻提示词生成视觉上令人信服的输出，但即使是前沿模型也经常误解隐喻载体并将其与对象混淆。这种系统性失败揭示了文生图模型在比喻语言与对象级视觉定位之间存在鸿沟，需要系统的评测框架来量化这一问题。

Method: 提出三部分评测框架：(1) 受控明喻数据集，隐喻载体来自预定义的可检测对象类别并结合多种模板；(2) 基于YOLO检测的自动定位指标，用于评估隐喻载体是否正确生成；(3) 使用Diffusion Lens进行文本编码器层分析，追踪隐喻载体在生成过程中的涌现机制。

Result: 在多种架构的文生图模型上实验，发现一致的"字面化"失败模式，即模型倾向于将隐喻载体当作字面对象生成，而非理解其比喻含义。Diffusion Lens分析揭示了隐喻载体在文本编码器各层中的涌现过程。

Conclusion: 文生图模型在明喻理解上存在系统性缺陷，本文提出的评测框架可扩展地量化这一差距，并讨论了改善明喻定位的潜在缓解策略。

Classification reason: 论文提出一个用于评估文生图模型明喻理解能力的评测框架，包含受控数据集、基于YOLO的自动定位指标和Diffusion Lens文本编码器分析，属于生成模型评测与数据集范畴，映射到[CANONICAL] generation_benchmark。

Importance reason: Evaluation framework for figurative language in T2I models, but tangential to the user's core focus on continuous/multimodal generation and world models. Signals: text-to-image evaluation, simile understanding, diffusion lens, YOLO grounding, literalization failure
...
### [40] [InvFlowFD: Reference-Free and Background-Set-Free Perceptual Music Quality Metric with Flow Matching Inversion](https://arxiv.org/abs/2608.04142)
*Alon Ziv, Harel Pogoda, Yossi Adi*

Main category: cs.AI, cs.SD, cs.LG
...
TL;DR: InvFlowFD 是一种基于 Flow Matching 反演的无参考、无背景集音乐感知质量评估指标，仅用预训练 Flow Matching 主干即可检测人工失真并准确排序音乐生成模型，与人类感知高度相关。


<details>
  <summary>Details</summary>
Motivation: 现有无参考音乐质量评估方法虽无需成对的噪声-干净数据，但仍依赖背景集来计算干净音频样本的聚合统计量，这限制了其灵活性和适用性。作者希望消除这一依赖，实现完全无参考、无背景集的质量评估。

Method: 利用预训练的 Flow Matching 主干，通过简单的 Euler 积分执行无条件 Flow Matching 反演，将一组反演样本与先验分布进行比较，从而检测人工失真并评估生成模型质量。该方法无需任何参考音频或背景集。

Result: 定量实验和人类研究均表明，InvFlowFD 与人类对声音失真的感知以及生成模型质量高度相关，且比现有指标更灵活、限制更少。

Conclusion: 无条件 Flow Matching 反演足以作为无参考、无背景集的音乐感知质量评估基础，为生成音频质量评估提供了一种更通用、更少约束的新范式。

Classification reason: 该论文提出一种无参考、无背景集的音乐感知质量评估指标 InvFlowFD，属于生成模型评测与自动评估指标范畴，映射到 [CANONICAL] 生成模型评测与数据集（generation_benchmark）子主题。

Importance reason: Novel metric for music quality evaluation, but outside the user's core focus on continuous multimodal models and generative video/image generation. Signals: flow matching inversion, music quality metric, reference-free evaluation, generative model ranking, human perception correlation
...
### [41] [OmniEdit-Bench: A Comprehensive Benchmark for Instruction-based Video Editing](https://arxiv.org/abs/2608.05049)
...
TL;DR: OmniEdit-Bench 是一个面向指令式视频编辑（IVE）的综合性评测基准，将编辑任务分解为空间、时序、音频和参考式编辑等多个视频特有维度，区分显式与隐式指令，并提出从准确性、保持性、真实性和一致性四个维度评估的框架，引入准确性感知惩罚机制防止视觉上合理但错误的编辑获得高分。


<details>
  <summary>Details</summary>
Motivation: 现有指令式视频编辑评测基准存在两大局限：一是任务覆盖有限，继承了图像编辑的框架而忽略了视频特有的维度；二是评测指标不充分，无法衡量指令忠实度，导致错误的编辑因原始视频的强视觉先验而获得高分。

Method: 构建结构化基准，将编辑任务分解为空间、时序、音频和参考式编辑等视频特有维度，区分显式与隐式指令并引入推理式场景；提出四维评估框架（准确性、保持性、真实性和一致性），结合人类判断与先进视觉语言模型，并引入准确性感知惩罚机制，使其他分数以准确性为条件，防止视觉上合理但错误的编辑获得虚高评价。

Result: 对代表性开源和商业模型的大量实验表明，当前 IVE 模型仍远未达到令人满意的水平。

Conclusion: OmniEdit-Bench 为指令式视频编辑提供了全面可靠的测试平台，并为未来研究方向提供了洞察。

Classification reason: 该论文提出 OmniEdit-Bench，一个面向指令式视频编辑的综合性评测基准，属于生成模型评测与数据集方向，映射到 [CANONICAL] generation_benchmark（生成模型评测与数据集）。

Importance reason: Benchmark paper for video editing, but lacks direct focus on generation methods, world models, or continuous representations prioritized by the user. Signals: video editing benchmark, instruction fidelity, evaluation framework, accuracy-aware penalty, video-specific dimensions
...
### [42] [AudioScape-TTA: A Structured Soundscape Benchmark for Fine-Grained Text-to-Audio Evaluation](https://arxiv.org/abs/2608.04479)
...
TL;DR: AudioScape-TTA 是一个面向细粒度文生音频评测的结构化基准，包含 2,258 个音频-文本对和 25,707 个二元 QA 评分项，通过模态感知语义结构和事件密度/结构复杂度标注，提出基于 rubric 的音频接地评测框架，验证事件实现、声学属性和语音内容，实验表明现有 TTA 模型在细粒度属性控制、语音内容保持和组合声景生成上存在持续局限。


<details>
  <summary>Details</summary>
Motivation: 现有文生音频（TTA）评测主要依赖全局相似度指标，无法揭示细粒度的语义失败，难以判断生成音频是否忠实满足复杂文本指令。

Method: 构建 AudioScape-TTA 基准，通过模态感知语义结构表示真实声景，用事件密度和结构复杂度刻画生成复杂度；提出基于 rubric 的音频接地评测框架，通过细粒度语义标准验证事件实现、声学属性和语音内容。

Result: 在 13 个代表性开源 TTA 模型上实验，揭示细粒度属性控制、语音内容保持和组合声景生成方面的持续局限；人工验证表明 rubric 评测比传统全局相似度指标与人类语义判断对齐更强。

Conclusion: AudioScape-TTA 提供了可扩展、可解释的 TTA 细粒度评测方案，能更准确地反映生成音频对复杂文本指令的忠实程度，为 TTA 系统改进提供诊断性洞察。

Classification reason: 该论文提出 AudioScape-TTA，一个用于细粒度文生音频（TTA）评测的结构化基准，属于生成模型评测与数据集方向，映射到 [CANONICAL] 的 generation_benchmark 子主题。

Importance reason: Benchmark/evaluation paper for text-to-audio, tangential to the user's core focus on continuous multimodal models and autoregressive generation. Signals: text-to-audio benchmark, fine-grained evaluation, rubric-based scoring, soundscape generation, no autoregressive or continuous token focus
...
### [43] [OmniVR: Joint Video-Audio Conditional Generation for Restoring Degraded Historical Films](https://arxiv.org/abs/2608.04224)
...
TL;DR: OmniVR 是首个联合音频-视频生成式修复模型，基于 22B 参数音视频生成骨干，将低质量视频与音频编码为潜在条件，在统一多模态 DiT 中联合去噪恢复视觉结构、时序运动与声学细节，并配套提出 OmniVRBench 基准。


<details>
  <summary>Details</summary>
Motivation: 历史影片同时存在视觉与音频退化（模糊、噪声、闪烁、嘶声、削波、丢失），但现有方法各自独立修复单一模态，导致质量差距与跨模态不一致。需要一个联合的音视频生成式修复框架。

Method: 基于 22B 参数音视频生成骨干，将修复建模为统一多模态 DiT 中的条件生成：低质量视频与音频编码为潜在条件，结合固定修复提示词联合去噪。三个关键设计：(1) 联合音视频退化管线模拟真实老片特征；(2) 架构保持的 T2AV 到 AV2AV 迁移与提示退火以保留生成先验；(3) 首帧 I2V 锚定、损失重加权与波形监督用于长视频外推与音频保真。

Result: 在 OmniVRBench（200 个真实历史片段）上，OmniVR 在全部六项视觉指标上超越所有先前方法，取得最佳音频质量，并产生自然的彩色化效果，是首个同时处理视觉、音频与彩色化三方面的方法。

Conclusion: 联合音视频生成式修复能有效解决历史影片的跨模态退化问题，统一多模态 DiT 范式可同时恢复视觉与音频并保持跨模态一致性，为老片修复提供了新方向。

Classification reason: 论文提出 OmniVR，基于 22B 音频-视频生成骨干，将历史影片修复建模为统一多模态 DiT 中的条件生成，联合去噪恢复视觉与音频。这属于音视频联合生成/跨模态条件生成，映射到 [CANONICAL] audio_av_generation（音频与多模态联合生成）。同时涉及统一多模态生成，但主方向为多模态生成。

Importance reason: Strong technical contribution in joint audio-video generative restoration, but the restoration focus and lack of direct alignment with top-priority continuous/multimodal reasoning or world-model subtopics temper its daily-read relevance. Signals: joint audio-video generation, unified multimodal DiT, 22B backbone, AV2AV adaptation, OmniVRBench benchmark
...
### [44] [Visual Representation Matters: Exploiting Temporal Differences in Video-to-Audio Generation](https://arxiv.org/abs/2608.04902)
*Zehua Chen, Junyou Wang, Yuxuan Jiang, Zhenying Fang, Yusheng Dai, Jianfei Chen, Ziwei Liu, Jun Zhu*

Main category: cs.CV, cs.LG, cs.SD
...
TL;DR: TD-V2A 提出利用时序差异（temporal differences）作为视频到音频生成的关键视觉表征，通过层级连续学习策略和退火时序差异引导方法，在扩散训练与采样过程中逐步学习并利用时序差异信息，显著提升端到端 V2A 生成质量。


<details>
  <summary>Details</summary>
Motivation: 视频到音频生成（V2A）相比图像到音频生成（I2A）引入了连续帧，这些帧提供了音频合成所需的关键时序线索。然而现有基于条件扩散的 V2A 方法通常需要额外的音视频监督、声学结构预测或大模型推理来增强视觉条件，这需要额外网络或强归纳偏置。作者希望找到一种无需额外网络、仅通过最小架构修改就能有效利用时序信息的方法。

Method: 作者提出 TD-V2A 框架：1）在帧级和特征级两个层面研究时序差异（TD），确定 TD 补充视觉表征最有效的表征层级；2）基于研究发现，设计层级连续学习策略（hierarchically continual learning strategy）在扩散训练过程中逐步学习 TD 信息；3）提出退火时序差异引导方法（annealed temporal differences guidance）在采样过程中利用 TD 信息。

Result: 在基准数据集上的大量实验表明，有效利用时序差异能显著提升端到端 V2A 生成质量，甚至优于专用的 V2A 表征（如对比音视频预训练 CAVP）。

Conclusion: 时序差异是区分 V2A 与 I2A 的关键表征，通过层级连续学习和退火引导策略，可以在最小架构修改下有效利用时序差异信息，提升视频到音频生成质量。

Classification reason: 该论文研究视频到音频生成（V2A），属于跨模态条件音频生成，与 [CANONICAL] 子主题 audio_av_generation（音频与多模态联合生成）高度匹配。

Importance reason: Solid V2A method using temporal differences, but tangential to your core continuous/multimodal generation focus. Signals: video-to-audio generation, temporal differences, diffusion-based, visual representation, end-to-end generation
...
### [45] [Objects as Audio-Visual Modal Sound Fields](https://arxiv.org/abs/2608.05145)
...
TL;DR: 提出AV-MSF，一种物体级音频-视觉模态声场表示，基于3D高斯泼溅与密集3D视觉特征，用紧凑的物理模态参数表示冲击声场，实现从多视角图像和少量冲击声录音的鲁棒少样本重建，在冲击声渲染上超越物理仿真与数据驱动基线，并支持接触定位与物体声音编辑。


<details>
  <summary>Details</summary>
Motivation: 现代3D重建擅长建模物体几何与外观，但忽略了物理交互产生的丰富声学线索。物体冲击声蕴含材质、刚度与结构属性，可补充视觉信息。现有冲击声建模要么依赖昂贵的物理仿真，要么需要大规模数据才能以纯数据驱动方式泛化。

Method: AV-MSF将3D高斯泼溅与密集3D视觉特征集成，提供强几何感知先验，并使用紧凑、物理上有意义的模态参数表示冲击声场，实现鲁棒的少样本重建。

Result: 在两个真实世界数据集上，AV-MSF在冲击声渲染上达到最先进水平，超越基于物理仿真和数据驱动的基线，并展示了接触定位与物体声音编辑等下游应用。

Conclusion: AV-MSF证明结合几何感知先验与物理模态参数的物体级声场表示，能够以少量样本实现高质量冲击声渲染，并支持多种下游应用，为音频-视觉联合建模提供了新方向。

Classification reason: 论文提出基于3D高斯泼溅与视觉特征的物体级冲击声场表示，实现从多视角图像和少量冲击声录音重建可渲染的音频-视觉模态声场，属于跨模态条件音频生成，映射到[CANONICAL]音频与多模态联合生成（audio_av_generation）。

Importance reason: Novel object-level audio-visual sound field with 3DGS, but falls outside the user's core continuous/multimodal generation and world model priorities. Signals: 3D Gaussian Splatting, few-shot impact sound rendering, contact localization, object sound editing, audio-visual joint modeling
...
### [46] [Eliciting Intrinsic Hallucinations in LLMs via Semantically Equivalent Adversarial Attacks](https://arxiv.org/abs/2608.04286)
...
TL;DR: 提出一种通过语义等价的对抗扰动来压力测试 LLM 内在幻觉鲁棒性的框架，在多种攻击设置下证明最先进模型对保义扰动高度脆弱，上下文忠实性最多下降 50%。


<details>
  <summary>Details</summary>
Motivation: 尽管 RAG 等外部知识增强方法被广泛用于提升 LLM 的事实准确性并减少幻觉，但这些系统仍易受内在幻觉影响——模型生成与检索证据不符的虚假信息。现有研究缺乏系统评估模型对此类现象鲁棒性的方法。

Method: 提出一个评估框架，通过对抗优化方法生成自然、语义等价的用户查询变体来压力测试模型。框架强制执行严格的语义等价约束和内在幻觉目标，并应用于白盒、灰盒和黑盒三种对抗攻击设置。

Result: 在 5 个开源和 5 个闭源生成模型、3 个数据集上评估，证明即使最先进的模型也对保义扰动高度敏感，上下文忠实性显著下降（GPT-5-mini 下降高达 50%）。

Conclusion: 对上下文证据的忠实使用即使在最先进的 LLM 中仍然脆弱，这激励了能够独立于表面查询形式实现稳健接地（grounding）的架构和训练目标。

Classification reason: 论文研究通过语义等价对抗攻击来诱发 LLM 的内在幻觉，属于对抗攻击与安全评测范畴，映射到 [CANONICAL] 多模态安全与攻防子主题。

Importance reason: Solid adversarial robustness work but outside the user's core focus on continuous/multimodal generation and world models. Signals: LLM hallucination, adversarial attack, semantic equivalence, RAG robustness, GPT-5-mini
...
TL;DR: ColorFD 提出一种基于多纯色补丁的黑盒物理对抗攻击方法，用于遥感目标检测。通过差分进化联合优化补丁位置与颜色参数，结合目标级适应度选择、关键区域定位与共性特征提取等引导策略，在 YOLOv3u、YOLOv5u 和 Faster R-CNN 上优于现有黑盒补丁方法，并可与强白盒基线竞争，物理实验验证了数字到真实成像域的迁移性。


<details>
  <summary>Details</summary>
Motivation: 遥感目标检测器对对抗扰动存在脆弱性，但现有研究多集中于数字或白盒设置，黑盒物理攻击研究不足。这类攻击常受限于物理可行性不足以及高维搜索空间中优化效率低下等问题。

Method: 提出 ColorFD 黑盒物理攻击方法，使用多个纯色补丁，通过差分进化（DE）联合优化补丁位置与颜色参数。设计目标级适应度与选择机制评估每个目标的攻击状态并保留目标特定改进；采用两种引导策略约束补丁搜索空间：关键区域定位通过有限差分颜色探测识别敏感区域，共性特征提取提供类别级空间先验并避免重复定位。

Result: 在 YOLOv3u、YOLOv5u 和 Faster R-CNN 上，ColorFD 在所有被测检测器上均优于测试的黑盒补丁方法，并与强白盒基线保持竞争力。物理世界实验表明优化后的纯色补丁可从数字域迁移到真实成像条件。

Conclusion: ColorFD 展示了黑盒物理对抗攻击在遥感目标检测中的有效性，通过有限差分引导与差分进化优化实现了高效且物理可行的攻击，且方法不局限于飞机类别，具有更广泛的适用性。

Classification reason: 论文研究遥感目标检测的黑盒物理对抗攻击，属于多模态/视觉模型安全与攻防范畴，映射到 [CANONICAL] 多模态安全与攻防 子主题。

Importance reason: Remote sensing adversarial attack is outside your configured research directions and is explicitly penalized by your negative keywords. Signals: remote sensing, black-box adversarial attack, object detection, differential evolution, physical attack
...
### [48] [A Model Merging Approach for Continual MLLM Unlearning](https://arxiv.org/abs/2608.04548)
...
TL;DR: 提出 MCU（Merging for Continual Unlearning），一种面向持续 MLLM 遗忘的模型合并方法。通过 leave-one-out 合并分析揭示一次性遗忘适配器之间存在跨任务依赖，MCU 将适配器投影到共享表征空间，保留主导方向、抑制过度集中坐标并重构跨任务依赖，以缓解干扰并增强迁移性，在 ICU-Bench 和 MLLMU-Bench 上取得更优遗忘效果同时保持保留知识与多模态通用能力。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 遗忘方法大多面向一次性请求，无法有效处理持续遗忘场景。反复应用一次性遗忘操作会导致累积效用退化、遗忘反弹和保留知识漂移等问题。

Method: MCU 在收到每个新的遗忘请求时，将多个一次性遗忘适配器动态合并为统一适配器。通过 leave-one-out 合并分析发现适配器间存在强跨任务依赖，MCU 将这些适配器投影到共享表征空间，保留其主导方向、抑制过度集中的坐标，并重构跨任务依赖以缓解干扰同时增强跨任务遗忘迁移性。

Result: 在 ICU-Bench 和 MLLMU-Bench 基准上，MCU 实现了更优的遗忘效果，同时保持了保留知识与通用多模态效用。

Conclusion: 持续 MLLM 遗忘可通过适配器合并策略有效实现，跨任务依赖的双重效应（迁移性与干扰）可通过共享表征空间中的方向保留与坐标抑制来平衡，为持续遗忘场景提供了可行方案。

Classification reason: 论文研究 MLLM 遗忘（unlearning），即从多模态大语言模型中移除隐私/敏感/专有信息，属于多模态安全与可靠性范畴，映射到 [CANONICAL] 多模态安全与攻防 子主题。

Importance reason: Focuses on MLLM unlearning safety, which is outside the user's core generation/world-model priorities and lacks direct alignment with continuous multimodal or video generation directions. Signals: continual unlearning, model merging, MLLM safety, cross-task dependencies, ICU-Bench

Abstract: Multimodal large language model (MLLM) unlearning methods have been proposed to remove private, sensitive, or proprietary information from well-trained models. However, most existing MLLM unlearning methods are designed for one-shot requests and fail to adequately address continual scenarios, as repeatedly applying one-shot operations leads to cumulative utility degradation, unlearning rebound, and retention drift. We introduce Merging for Continual Unlearning (MCU), an approach that dynamically merges multiple one-shot unlearning adapters into a unified adapter upon receiving each new unlearning a leave-one-out merging analysis, we reveal that these unlearning adapters exhibit strong cross-task dependencies. Such dependencies have two contrasting effects: they can facilitate cross-task unlearning transferability, but they can also introduce severe interference that degrades unlearning effectiveness and compromises retained knowledge. To address this challenge, MCU projects the adapters into a shared representation space, preserves their dominant directions, suppresses over-concentrated coordinates, and reconfigures cross-task dependencies to mitigate interference while enhancing transferability. Experiments on ICU-Bench and MLLMU-Bench demonstrate that MCU achieves superior unlearning effectiveness while preserving both retained knowledge and general multimodal utility.

</details>


### [49] [Unleashing the Potential of Vision-Language Models for Generalizable AI-Generated Image Detection](https://arxiv.org/abs/2608.04935)
...
TL;DR: 本文发现视觉语言模型的 Perception Encoder (PE) 在冻结特征空间中比 DINOv3 具有更强的局部来源语义组织能力，但线性探测无法利用这一结构。作者提出语义原型校准 (SPC) 方法，构建类别原型并利用监督数据校准，得到 PE-SPC 检测器，在跨生成器、后处理和野外基准上超越 DINOv3 基线，达到新的 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明在冻结视觉基础模型表征上使用简单线性探测即可达到 SOTA 的 AI 生成图像检测性能，DINOv3 成为主流基线。但作者发现视觉语言模型的 Perception Encoder 具有更强的来源语义组织能力，只是语义无关的线性探测无法充分利用这一结构，导致 PE-Linear 性能落后于 DINOv3-Linear。

Method: 提出语义原型校准 (Semantic Prototype Calibration, SPC) 方法：从取证语义信息构建类别原型，并用监督数据对原型进行校准。将 SPC 应用于 Perception Encoder，得到 PE-SPC 检测器。

Result: PE-SPC 在跨生成器、后处理和野外基准上均超越之前的 DINOv3 基线，取得新的 SOTA 结果，展现出更强的泛化能力。

Conclusion: 视觉语言模型的感知编码器在 AI 生成图像检测中具有比 DINOv3 更大的潜力，其语言对齐表征保留了高层来源语义信息；通过语义原型校准可以充分挖掘这一结构优势，实现更强的泛化检测性能。

Classification reason: 论文研究 AI 生成图像检测（AIGI detection），属于多模态安全与攻防范畴，映射到 [CANONICAL] 多模态安全与攻防 子主题。

Importance reason: Solid but niche AIGI detection paper; limited overlap with your core continuous/multimodal generation and world-model priorities. Signals: AIGI detection, vision-language model, semantic prototype calibration, DINOv3 baseline, cross-generator generalization
...
### [50] [Adversarial Attacks for Good: A Survey of Proactive Protection across the Visual Content Lifecycle](https://arxiv.org/abs/2608.04314)
...
TL;DR: 本文系统综述了"对抗攻击用于善"这一范式，即数据所有者、创作者、平台或审计者主动施加扰动或结构化信号，以破坏未经授权的自动化处理或支持事后问责。论文覆盖视觉资产生命周期五个研究社区：隐私过滤、不可学习样本、生成式防护、对抗CAPTCHA与溯源机制，并沿可迁移性、适应性与部署就绪度三个轴评估其可比性。


<details>
  <summary>Details</summary>
Motivation: 视觉内容一旦进入AI流水线，其所有者往往对内容如何被使用失去技术控制。法律与监管手段可应对滥用，但许多技术干预必须在内容发布或访问时提前应用。本文旨在系统梳理围绕这一干预点发展起来的"对抗攻击用于善"保护范式，并整合五个独立发展的研究社区。

Method: 本文采用综述方法，将视觉资产生命周期中的主动保护方法划分为五个研究社区：共享时的隐私过滤（对抗识别）、不可学习样本（对抗未授权训练）、生成式防护（对抗恶意编辑或模仿）、对抗CAPTCHA（访问控制）、以及溯源机制（传播后归因）。作者沿可迁移性、适应性与部署就绪度三个共享轴对五类方法进行比较评估，并整合跨阶段的对抗措施与开放问题。

Result: 综述发现，大多数保护方法仍主要针对静态或弱自适应对手进行验证，超出受控基准的证据仍然稀缺。五类方法在独立场所开发、成功标准不兼容，但许多方法利用了人类感知、语义解释与机器推理之间的持续差距，表明该范式在视觉流水线向多模态模型与自主智能体演进时仍具相关性。

Conclusion: 作者认为"对抗攻击用于善"范式在视觉内容生命周期中具有重要价值，但当前保护方法在鲁棒性、可组合性与可部署性方面仍面临挑战。文章通过整合跨阶段对策并指出开放问题，为构建稳健、可组合、可部署的所有者侧保护提供了方向。

Classification reason: 论文系统综述了对抗攻击在视觉内容生命周期中的防御性应用（隐私过滤、不可学习样本、生成式防护、对抗CAPTCHA、溯源机制），属于多模态安全与攻防范畴，映射到 [CANONICAL] 多模态安全与攻防 子主题。

Importance reason: Survey on adversarial protection is tangential to your core focus on continuous/multimodal generation and world models. Signals: adversarial attacks for good, visual content lifecycle, privacy filters, unlearnable examples, generative safeguards
...
### [51] [When Memory Lies: An Empirical Study of Spatial Memory Staleness in VLM Agents](https://arxiv.org/abs/2608.04574)
...
TL;DR: 本文实证研究记忆增强型 VLM 智能体在环境变化后空间记忆过期时的行为。通过动态 FrozenLake 测试平台，结合过期检测任务与下游导航任务，在三个闭源和三个开源 VLM 上测试。发现：文本可解性不代表视觉接地能力（视觉 F1 从 0.887 到 0.067 不等）；盲目信任过期记忆比无记忆更危险（GPT-4o 下死亡次数翻倍）；读取时审计过滤能降低文本模式安全成本，但视觉审计不可靠时过滤无一致收益。


<details>
  <summary>Details</summary>
Motivation: 记忆增强型 VLM 智能体依赖持久空间知识行动，但环境变化会使这些知识悄然过期。作者希望探究当智能体必须调和自信的记忆主张与矛盾的观察时会发生什么，以及当前模型能否在冲突演变为安全相关错误之前发现它。

Method: 使用动态 FrozenLake 测试平台，将过期检测任务与下游导航任务配对。在三个闭源模型和三个开源 VLM 上，以文本和图像两种输入形式进行测试（1800 次检测运行，12000 次文本模式导航回合，四个 LLM 导航器，共享 50 个种子）。对比信任原始记忆、无记忆、以及带读取时审计过滤等不同策略。

Result: 三个主要发现：1) 文本可解性不蕴含视觉接地能力，模型在相同网格上视觉 F1 从 0.887 到 0.067 不等，最弱的模型仍做出流畅自信但忽略图像的决策；2) 无审计地消费过期记忆是安全负担，GPT-4o 下信任原始记忆的智能体死亡次数是无记忆智能体的两倍以上；3) 审计有帮助但未弥合差距，透明读取时过滤在文本模式大幅降低安全成本，但即使 oracle 过期标签在当前网格大小下也无显著额外收益，视觉审计不可靠时过滤无一致收益。

Conclusion: 空间记忆过期是记忆增强型智能体的安全失效模式。可靠的视觉接地以及在记忆-观察冲突下的动作选择是记忆增强型智能体的核心开放挑战。

Classification reason: 论文研究 VLM 智能体在空间记忆过期（stale memory）与当前观察冲突时的可靠性问题，属于多模态模型的可靠性/忠实性研究，映射到 [CANONICAL] 多模态幻觉与可靠性（multimodal_hallucination）子主题。

Importance reason: Solid empirical study on VLM agent memory reliability, but tangential to your core focus on continuous/multimodal generation and world models. Signals: VLM agent reliability, spatial memory staleness, safety failure mode, visual grounding gap, memory audit filtering
...
### [52] [ReGround: Restoring Visual Grounding in Multi-Step Reasoning through Self-Diagnosis and Visual Re-Examination](https://arxiv.org/abs/2608.04385)
...
TL;DR: ReGround 提出两阶段框架，通过自诊断与选择性视觉重检，恢复 VLM 在多步推理中丢失的视觉接地能力，无需架构修改或外部工具。


<details>
  <summary>Details</summary>
Motivation: VLM 在多步推理中随着推理链增长，后续步骤越来越依赖语言先验而非图像证据，导致视觉接地能力退化。作者识别出这一退化在注意力熵上的基准级特征，并发现有效的视觉重检需要图像重注入与针对性自诊断两个互补要素。

Method: ReGround 采用两阶段框架：第一阶段通过能力引导（capability bootstrapping），用同模型家族的更强变体在数据构建阶段提供诊断脚手架；第二阶段让策略模型在推理时自主诊断接地失败并选择性重检视觉证据。无需架构修改或外部工具。

Result: 在八个基准、两个 VLM 主干上取得一致提升，尤其在视觉密集的多步推理任务上效果显著，且推理开销相对工具增强基线仅适度增加。

Conclusion: 准确的自我诊断是视觉重检能否带来收益的关键因素；ReGround 通过自诊断与选择性重检有效恢复 VLM 的视觉接地能力，且策略模型在推理时能自主诊断并保留大部分辅助增益。

Classification reason: 论文关注 VLM 在多步推理中丢失视觉接地（visual grounding）的问题，通过自诊断与视觉重检来恢复对图像证据的依赖，属于视觉幻觉/可靠性/忠实性提升范畴，映射到 [CANONICAL] 多模态幻觉与可靠性。

Importance reason: Addresses VLM visual grounding degradation in multi-step reasoning, but is tangential to the user's core focus on continuous/multimodal generation and world models. Signals: VLM reasoning, visual grounding, self-diagnosis, attention entropy, no architecture change
...
### [53] [CARGO-VL: Counterfactual Arbitration with Risk-Constrained Group Optimization for Vision-Language Models](https://arxiv.org/abs/2608.04509)
...
TL;DR: CARGO-VL 提出一种组相对优化框架，将视觉语言模型的反事实证据状态（对齐、仅图像正确、仅文本正确、均错误）作为一组匹配变体联合优化，通过条件正确性耦合、转换奖励（答案不变性、源等变性、答案到弃权切换）以及原始-对偶风险控制器，提升模型在证据冲突下的可靠仲裁能力，并贡献了 XMC 四条件冲突训练资源。


<details>
  <summary>Details</summary>
Motivation: 视觉语言系统结合图像与检索文本时，这些来源可能相互矛盾或共同无法支撑答案。现有后训练目标对实例独立打分，无法在反事实证据变化下强制执行一致行为，导致模型难以识别可信来源并在证据不足时弃权。

Method: 提出 CARGO-VL 组相对框架：将覆盖 A/V/T/N 四种证据状态的匹配变体作为一个 bundle 联合优化；目标函数耦合条件正确性与转换奖励（答案不变性、源等变性、答案到弃权切换）；采用原始-对偶控制器在不安全答案与过度弃权之间取得平衡。同时贡献 XMC 四条件冲突训练资源。

Result: 在多个随机种子下，CARGO-VL 相比逐点基线在冲突处理、不支持答案规避和模态平衡上均有提升；消融实验表明关系转换信号与自适应风险控制具有互补收益，并在 CMC-Bench 和 Modality-Bias 上验证了迁移能力。

Conclusion: 反事实一致性可作为可靠多模态证据仲裁的实用优化目标，CARGO-VL 的组相对优化与自适应风险控制为提升视觉语言模型在证据冲突场景下的可靠性提供了有效方案。

Classification reason: 论文提出 CARGO-VL，一种面向视觉语言模型的组相对优化框架，通过反事实证据状态（A/V/T/N）的匹配变体优化、转换奖励与原始-对偶风险控制来提升多模态证据仲裁的可靠性。这属于多模态模型的对齐与偏好优化范畴，映射到 [CANONICAL] 多模态强化学习与对齐（multimodal_rl_alignment）。

Importance reason: Focuses on multimodal alignment/arbitration rather than the user's priority of continuous/multimodal generation or world models. Signals: multimodal understanding, alignment, counterfactual, risk control, not generation-focused
...
### [54] [Privileged, but Biased: How PI-Conditioned Teachers Break Self-Distillation](https://arxiv.org/abs/2608.04794)
...
TL;DR: 本文系统研究自蒸馏（SD）作为独立目标（无奖励项）时是否真正有效。作者复现了 SDPO 在简单场景下的收益，但在困难任务（问答、数学、代码、多轮智能体工具使用）中发现 per-token loss 持续下降而验证准确率不升反降。通过 PI Bias Score 量化，揭示教师因看到单一参考解而产生 PI 偏差，导致学生目标对正确性近乎失明，惩罚探索性 token，最终产生更平坦、更不果断且推理能力无提升的学生模型。


<details>
  <summary>Details</summary>
Motivation: 自蒸馏被提出作为可验证奖励强化学习的计算高效替代方案，但已有收益几乎全部来自狭窄、低难度的设定。作者希望回答一个基本问题：作为唯一目标（无奖励项），自蒸馏是否真的教会了模型任何东西。

Method: 作者复现 SDPO 在简单场景的收益，然后在困难任务（问答、数学、代码、多轮智能体工具使用）上应用相同设置，覆盖多种推理模式、模型规模和 PI 形式，同时测试 SDPO 与 OPSD 两种配方。提出 PI Bias Score 量化教师因看到单一参考解而产生的偏差，并追踪从 loss 到最终模型的因果链。

Result: 在困难任务上，per-token loss 持续下降但验证准确率不提升且通常退化。PI 偏差使教师目标偏向单一参考轨迹而非一般正确性；学生目标对 rollout 是否正确近乎失明，loss 主要落在低信息 token（停用词、标点、不确定性标记）上，正确 rollout 中探索性 token 承受最高散度，惩罚了推理所需的犹豫。

Conclusion: 作为独立目标，自蒸馏优化的是一个与任务成功解耦的信号，无法教会模型推理。PI 条件教师引入的偏差是根本原因，这为自蒸馏方法在困难任务上的应用提出了重要警示。

Classification reason: 论文研究自蒸馏（self-distillation）作为 RL 的替代方案，分析 PI 条件教师如何破坏自蒸馏，属于对齐与偏好学习范畴，映射到 [CANONICAL] 多模态强化学习与对齐。

Importance reason: Solid negative-result analysis of self-distillation, but tangential to the user's core focus on continuous/multimodal generation and world models. Signals: self-distillation failure analysis, PI bias quantification, negative result on hard tasks, no generation/world-model focus, no continuous/multimodal token emphasis
...
### [55] [GeoReward: Mitigating Contextual Variable Overestimation in Vision-Language Models for Cross-Market Preference Prediction](https://arxiv.org/abs/2608.04504)
...
TL;DR: 论文诊断了 VLM 中"上下文变量高估"(CVE)问题——模型过度依赖主导视觉文本线索而低估稀疏但关键的上下文变量（如地域市场信息）。作者构建了跨市场广告创意数据集，并提出 GeoReward 奖励模型，集成市场感知检索增强、上下文引导视觉调制和选择性敏感度损失三种机制，用于预测不同地理市场的广告图像偏好，并引导 VLM 的 RL 微调生成市场感知的广告背景设计。


<details>
  <summary>Details</summary>
Motivation: VLM 在跨市场广告偏好预测等真实应用中存在系统性偏差：模型倾向于高估主导视觉文本线索（如产品属性、密集图像块），而低估稀疏但决策关键的上下文变量（如地域市场信息），导致对不同国家的产品图像偏好预测崩溃为一致输出。这种 Contextual Variable Overestimation (CVE) 问题尚未被系统诊断和解决。

Method: 首先收集真实广告创意及其多国点击率表现的新多模态数据集。然后提出 GeoReward 奖励模型，集成三种机制：(1) Market-Aware Retrieval Augmentation 市场感知检索增强；(2) Context-Guided Visual Modulation 上下文引导视觉调制；(3) Selective Sensitivity Loss 选择性敏感度损失。最后利用 GeoReward 引导 VLM 的 RL 微调，为文生图模型生成市场感知的广告背景设计。

Result: 实验验证了该框架能缓解 CVE 问题，并在跨市场广告偏好预测任务上优于现有基线方法。

Conclusion: 该工作不仅诊断了 VLM 对主导感知特征的系统性偏差，还提供了针对稀疏上下文变量主导决策场景的解决方案，展示了奖励模型引导 RL 微调在生成市场感知广告创意方面的有效性。

Classification reason: 论文提出 GeoReward 奖励模型用于跨市场广告图像偏好预测，并用于引导 VLM 的 RL 微调，属于多模态偏好学习与对齐方向，映射到 [CANONICAL] 多模态强化学习与对齐。

Importance reason: Focuses on advertising preference prediction and RL alignment, which is tangential to your core interests in continuous multimodal models and video/image generation. Signals: VLM bias diagnosis, reward model, cross-market preference, RL fine-tuning, advertising creatives
...
### [56] [Agentic Reinforcement Learning with Observation-Calibrated Self-Distillation](https://arxiv.org/abs/2608.04788)
...
TL;DR: 提出 Observation-Calibrated Self-Distillation (OCSD)，通过对比 Full 与 Observation-Ablated 两种结构匹配的 replay 视图，消除 replay scaffold 带来的分数偏移，得到观测残差用于调制 token 级 GRPO 更新，在 ALFWorld、WebShop、Search-QA 上超越强基线。


<details>
  <summary>Details</summary>
Motivation: LLM 智能体通常用稀疏轨迹级奖励进行强化学习训练，对单个 token 应如何更新缺乏细粒度指导。On-Policy Self-Distillation (OPSD) 通过特权 replay 视图重打分生成 token 以获得密集 token 级监督，但其支持信号混杂了特权信息与 replay scaffold 引起的分数偏移，尤其在以未来环境观测作为特权信息时，重建扩展 scaffold 本身会扰动 token 分数，难以将支持信号归因于该信息。

Method: 提出 Observation-Calibrated Self-Distillation (OCSD)：构造两个结构匹配的 replay 视图 Full 与 Observation-Ablated，二者仅在是否包含真实未来观测上不同，通过对比得到观测残差，从而折扣掉 replay scaffold 共享的分数变化。该残差用于在高不确定性步骤调制 token 级 GRPO 更新，同时保留轨迹级更新方向。

Result: 在 ALFWorld、WebShop 和 Search-QA 三个任务上，跨三种 Qwen3 模型规模，OCSD 一致优于强基线。诊断分析进一步表明校准后的残差与局部环境反馈对齐更好。

Conclusion: OCSD 通过观测校准的自蒸馏有效解决了 OPSD 中特权信息与 replay scaffold 分数偏移混杂的问题，为 LLM 智能体的 token 级强化学习训练提供了更精确的监督信号。

Classification reason: 论文提出基于强化学习（GRPO）的智能体训练方法，通过观测校准的自蒸馏改进 token 级奖励信号，属于多模态/LLM 智能体的强化学习与对齐方向，映射到 [CANONICAL] 多模态强化学习与对齐。

Importance reason: Solid RL-for-agents method paper, but it is outside your core continuous/multimodal generation and world-model focus. Signals: token-level GRPO, self-distillation, agent RL, ALFWorld/WebShop, no multimodal generation

Abstract: Large language model agents are commonly trained through reinforcement learning with sparse trajectory-level rewards, which offer limited guidance on how strongly individual tokens should be updated. On-Policy Self-Distillation (OPSD) addresses this by re-scoring generated tokens under a privileged replay view to obtain dense, token-level supervision. However, we identify a confounding issue: the resulting support may reflect both the privileged information contained in the replay view and score shifts induced by the replay scaffold, making it difficult to attribute the support specifically to that information. This issue is especially pronounced when future environment observations serve as privileged information, since replaying them requires reconstructing an extended scaffold that itself perturbs token scores. To resolve this confounding, we propose Observation-Calibrated Self-Distillation (OCSD), which contrasts two structurally matched replay views, Full and Observation-Ablated, differing only in whether the actual future observation is present, to derive an observation residual that discounts score changes shared by the replay scaffold. OCSD then applies this residual to modulate token-level GRPO updates at high-uncertainty steps, while preserving the trajectory-level update direction. Experiments on ALFWorld, WebShop, and Search-QA across three Qwen3 model scales show that OCSD consistently outperforms strong baselines. Diagnostic analyses further confirm that the calibrated residual aligns better with local environment feedback. Our code is publicly available at .

</details>


### [57] [Teaching MLLMs to Say No: Generalized Referring Expression Comprehension via Refusal Calibrated GRPO](https://arxiv.org/abs/2608.04698)
...
TL;DR: 本文提出 Refusal-Calibrated GRPO (RC-GRPO)，一种校准的强化学习策略，用于增强 MLLM 在广义指代表达理解（GREC）任务中的拒绝能力，同时保持正样本的定位精度，在三个基准上取得最优效果。


<details>
  <summary>Details</summary>
Motivation: MLLM 在定位已存在物体方面表现出色，但由于训练时缺乏负样本，往往无法拒绝不存在的物体，产生幻觉边界框。现有的 SFT 和 RL 后训练方法虽能增强拒绝行为，但通常会降低正样本的定位精度，损害模型核心能力。

Method: 提出 RC-GRPO：在 rollout 中强制负样本输出 "None" 以获得有效的优势估计，并对正样本施加惩罚以防止过度拒绝，实现精度与可靠性的平衡。此外引入第二阶段推理强化，巩固因果理解与可解释性。

Result: 在三个 GREC 基准上，RC-GRPO 在保持强拒绝能力的同时取得了更优的定位精度。

Conclusion: 通过校准的强化学习策略，可以在增强 MLLM 拒绝能力的同时不牺牲正样本定位精度，实现精度与可靠性的平衡，并提升模型的因果理解与可解释性。

Classification reason: 论文提出 RC-GRPO，一种基于 GRPO 的强化学习策略来增强 MLLM 的拒绝能力（refusal）并保持定位精度，属于多模态强化学习与对齐方向，映射到 [CANONICAL] 子主题 multimodal_rl_alignment。

Importance reason: Solid but narrow MLLM alignment work on refusal calibration; relevant to multimodal understanding but not to your core continuous-model, video/world-model, or efficiency priorities. Signals: MLLM, GRPO, refusal calibration, referring expression comprehension, hallucination mitigation
...
### [58] [OPD-V: Visual On-Policy Self-Distillation with Modality Balance](https://arxiv.org/abs/2608.05131)
...
TL;DR: OPD-V 是一种面向 MLLM 的视觉 On-Policy 自蒸馏方法，利用正负教师（Zoom-In 与 Mask 图像）构造模态平衡信任区域，选择 on-policy token 进行自蒸馏，缓解模态不平衡问题并提升视觉推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有 On-Policy Self-Distillation (OPSD) 方法从多样输入源提取特权信息来引导自蒸馏，但忽略了 MLLM 推理中固有的模态不平衡问题——当文本信息主导生成时，模型无法充分整合多模态输入，导致精心设计的特权信息未被充分利用，限制了 OPSD 的效果。

Method: 构造 Positive Teacher（Zoom-In 图像）与 Negative Teacher（Mask 图像），二者呈现不同程度的模态不平衡。通过观察其推理正确性与 token logits 变化，发现模态平衡本身可作为特权信息。据此提出 OPD-V：用 Positive Modality-Balance Logits Margins 定义 Modality-Balance Trust Region，选择用于自蒸馏的 on-policy token。

Result: 在 6 个基准、4 个 MLLM 骨干和 5 种后训练方法上，OPD-V 一致地提升了推理性能，同时降低了训练成本。

Conclusion: 模态平衡可以作为 MLLM 自蒸馏的有效特权信息，OPD-V 通过正负教师构造的信任区域选择 on-policy token，在提升推理性能的同时降低训练开销，验证了模态平衡在 OPSD 中的关键作用。

Classification reason: 论文提出 OPD-V，一种基于 On-Policy Self-Distillation 的 MLLM 后训练方法，通过正负教师构造模态平衡信任区域来引导自蒸馏，属于多模态模型的对齐与后训练优化，映射到 [CANONICAL] 多模态强化学习与对齐 子主题。

Importance reason: Focuses on MLLM post-training self-distillation for modality balance, which is adjacent to but not central to your continuous multimodal and video generation priorities. Signals: MLLM post-training, on-policy self-distillation, modality imbalance, visual reasoning, efficient training
...
### [59] [When Prompts Become Pixels: Prompt-Region Grounding for Multimodal Reasoning](https://arxiv.org/abs/2608.04726)
...
TL;DR: 提出 Visualized Task Semantics (VTS) 干预方法，将问题文本移入图像中，发现六个 MLLM 在四个基准上准确率平均下降 17.8 点，暴露了超出 OCR 的语义通道差距；并提出 prompt-region grounding 方法，将问题区域与类型化语义对齐并从掩码视图恢复干净表征，在匹配训练成本下将 VTS 准确率从 58.0 提升到 66.3。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型越来越多地需要推理截图和文档，其中任务本身可能以像素形式写在图像中。然而现有基准通常将问题放在文本中，无法确定模型是否在不同通道上同样有效地使用指令。作者希望揭示并解决模型在图像中读取任务文本并作为推理指令使用的语义通道差距。

Method: 提出 Visualized Task Semantics (VTS) 受控干预方法，将问题移入图像而保持源问题和答案不变。在此基础上提出 prompt-region grounding 方法，其核心设计是将问题区域与类型化语义对齐，并从掩码视图恢复其干净表征，训练时无需 OCR 或区域元数据。

Result: 在六个 MLLM 和四个基准上，所有 24 个模型-任务对的准确率均下降，平均下降 17.8 点。模型常能正确转录视觉问题却未能使用它，暴露了超出 OCR 的语义通道差距。prompt-region grounding 在匹配训练成本下将四基准 VTS 准确率从 58.0 提升到 66.3，同时保持原始界面的准确率。

Conclusion: 读取承载任务的文本并将其作为推理指令进行 grounding 是两种不同的能力。模型在图像中读取问题文本的能力与将其作为推理指令使用的能力存在显著差距，prompt-region grounding 方法能有效缩小这一差距。

Classification reason: 论文研究多模态大模型在图像中读取任务文本并将其作为推理指令进行grounding的能力，属于多模态推理范畴，映射到[CANONICAL]多模态推理与思维链子主题。

Importance reason: Interesting multimodal reasoning robustness study, but tangential to the user's core focus on continuous representations and generative world models. Signals: multimodal reasoning, MLLM robustness, prompt-region grounding, semantic channel gap, no OCR needed
...
### [60] [Q-CueGraph: Query-Conditioned Visual Evidence Graphs for Multimodal Reasoning](https://arxiv.org/abs/2608.04452)
...
TL;DR: Q-CueGraph 将问题与图像表示映射为预算化的坐标级观察，为冻结的 MLLM 读者提供任务条件化的"在哪里看"策略。文本密集图像复用 OCR/布局图，自然图像搜索实例化查询条件化的视觉节点，可选效用精炼从训练答案正确性学习可用裁剪，无需区域框监督。在 V*Bench 上以 19% 图像面积预算达到 0.833 准确率（全图推理为 0.696），在 InfographicVQA 上以约一半图像面积达到全图 ANLS 的 92%。


<details>
  <summary>Details</summary>
Motivation: 高分辨率像素和裁剪/缩放工具赋予 MLLM 检查图像的能力，但缺乏可靠的任务条件化策略来决定"在哪里检查"。现有方法无法根据问题自动定位需要关注的图像区域，导致推理效率低且准确率受限。

Method: Q-CueGraph 将问题与图像表示映射为预算化的坐标级观察。文本密集图像使用可复用的 OCR/布局图；自然图像搜索实例化查询条件化的视觉节点，统一在相同的选择、组合和预算接口下。可选效用精炼模块从训练答案正确性学习哪些候选裁剪对冻结读者可用，无需区域框监督。

Result: 使用冻结的 Qwen2.5-VL-7B 读者，Q-CueGraph 在 V*Bench 上以 19% 图像面积预算达到 0.833 准确率（全图推理为 0.696），在 InfographicVQA 上以约一半图像面积达到全图 ANLS 的 92%。跨六个基准，显式观察在证据可定位、问题能区分位置、分辨率限制全图阅读时最有价值。

Conclusion: 显式的查询条件化观察策略能显著提升 MLLM 在局部证据场景下的推理准确率与效率，且无需区域框监督即可学习。该方法在证据可定位、问题具有判别性、分辨率受限的场景中收益最大。

Classification reason: 论文提出 Q-CueGraph，通过查询条件化的视觉证据图引导 MLLM 在图像中定位关键区域进行推理，属于视觉推理与多模态推理范畴，映射到 [CANONICAL] 多模态推理与思维链 (multimodal_reasoning_cot)。

Importance reason: Introduces a query-conditioned visual evidence graph that improves multimodal reasoning efficiency and accuracy, aligning with the user's focus on multimodal understanding and efficient inference. Signals: multimodal reasoning, visual evidence graph, efficient inference, frozen MLLM, V*Bench improvement
...
### [61] [Toward Skill-Native LLMs: Skill Entropy for Benchmarking and Training Long-Horizon Reasoning](https://arxiv.org/abs/2608.05139)
...
TL;DR: 本文提出 Skill Entropy 度量模型在长程推理中切换不同技能（如数学推导后规划日程）的难度，构建 Skill²-Bench 跨技能长程推理基准（558 个技能、9 个领域），并提出 Skill-Entropy RL 训练框架，将技能熵作为奖励信号，显著提升 Qwen3 系列模型在跨技能长程推理上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常只评测单一技能，缺乏对模型在长程推理链中切换不同技能能力的度量。跨技能长程任务（cross-skill long-horizon tasks）要求模型在推理链中切换不同推理技能并依赖前序输出，需要一个原则性的方式来衡量这种技能切换能力。

Method: 提出 Skill Entropy 度量技能切换难度；构建 Skill²-Bench 基准，覆盖 558 个技能、9 个可验证与开放领域，每个任务分配技能熵分数并分为三个难度等级。提出 Skill-Entropy RL 训练框架，模型在每一步不仅预测答案还预测所用技能，奖励结合步骤级正确性与技能熵奖励（衡量模型预测技能序列与金标准技能序列的对齐程度）。

Result: 在 Skill²-Bench 上评测 8 个前沿模型和 4 个开源模型，发现技能切换差距：高熵任务上准确率下降。Skill-Entropy RL 在 Qwen3-4B-Instruct 上将 Skill²-Bench 分数从 34.4% 提升到 68.4%，在 Qwen3-1.7B 上从 14.6% 提升到 40.1%，优于竞争基线。该管线也可应用于现成训练数据（如 OpenR1-Math），表明技能熵是可复用的训练信号。

Conclusion: 技能熵是衡量长程推理中技能切换能力的有效度量，既能作为评测尺度也能作为训练信号。Skill-Entropy RL 通过让模型显式预测技能序列并对其对齐进行奖励，显著提升跨技能长程推理能力，且具有跨数据集的通用性。

Classification reason: 论文聚焦长程推理中技能切换的评测与训练，属于推理与思维链方向，映射到多模态推理与思维链（multimodal_reasoning_cot）这一规范主题。

Importance reason: Focuses on LLM skill-switching and RL training, tangential to the user's core continuous multimodal and video generation priorities. Signals: long-horizon reasoning, skill entropy, RL training, benchmark, LLM reasoning
...
### [62] [ABSeeker: Training Long-Horizon Search Agents via Answer-Backtracked Credit Assignment](https://arxiv.org/abs/2608.05102)
...
TL;DR: 提出 Answer-Backtracked Credit Assignment (ABC) 框架，通过从答案回溯恢复线索并将稀疏轨迹级结果转化为稠密步骤级奖励，用于训练长时序搜索智能体 ABSeeker，在 BrowseComp 上显著超越同规模模型。


<details>
  <summary>Details</summary>
Motivation: 长时序搜索智能体需要执行多步顺序动作来搜索、检索、验证和整合证据，但现有 SFT 和 RL 训练方法将所有步骤一视同仁，无法区分有用动作与错误或冗余动作，导致训练效率低下。

Method: 提出 ABC 框架：1) Answer-Backtracked Clue Recovery 从答案回溯恢复解题所需中间线索；2) Clue-Anchored Step Scoring 依据线索评估每个搜索步骤，将稀疏二元结果监督转化为稠密步骤级奖励；3) 基于奖励构建 ABC-SFT（重加权每轮损失）和 ABC-GRPO（以步骤级分数作为 GRPO 奖励）。基于 Qwen3.5-4B 用 8.5k 样本训练 ABSeeker。

Result: ABSeeker 在 BrowseComp 上达到 37.3%，在 BrowseComp-ZH 上达到 39.1%；加入上下文管理后分别提升至 55.3% 和 52.9%，显著优于同规模（4B）智能体，甚至匹配约 30B 的更大模型。

Conclusion: 答案回溯的步骤级信用分配能有效训练长时序搜索智能体，通过区分有用与错误/冗余步骤显著提升搜索推理性能，验证了稠密步骤级监督相对稀疏轨迹级监督的优势。

Classification reason: 论文关注长时序搜索智能体的训练，通过细粒度信用分配提升多步推理与搜索能力，属于推理与推理时计算扩展范畴，映射到多模态推理与思维链（multimodal_reasoning_cot）这一规范主题。

Importance reason: Solid RL credit-assignment method for search agents, but outside the user's core continuous/multimodal generation and world-model focus. Signals: long-horizon search agents, credit assignment, step-level rewards, BrowseComp, RL training
...
### [63] [SPOT: Sparse Probing and Outcome Calibration for On-Policy Distillation](https://arxiv.org/abs/2608.04419)
...
TL;DR: SPOT 是一种面向 on-policy distillation 的稀疏探测与结果校准方法，通过 acquisition-exploration-exploitation 流程，在有限探测预算下选择关键位置并基于验证器评分构建 KL 正则化的蒸馏目标，提升学生模型推理性能。


<details>
  <summary>Details</summary>
Motivation: 标准 on-policy distillation 使用 reverse-KL 训练，可能对其它合理续写分配不足的概率；教师熵无法区分不确定性是集中在少数候选还是分散在长尾，也无法判断学生是否已能表示这些候选；局部教师概率不能预测下游成功。因此需要解决"在哪里探测"和"蒸馏什么"两个耦合问题。

Method: SPOT 采用 acquisition-exploration-exploitation 三阶段流程。acquisition 阶段用位置级分数（结合归一化教师熵、top-k 候选集捕获的概率质量、学生-教师不匹配度）分配有限探测预算；exploration 阶段通过验证器评分的学生续写评估教师提出的候选；exploitation 阶段利用这些结果构建闭式、KL 正则化的目标，偏向下游结果更好的候选同时锚定教师分布。

Result: 在多个学生模型和推理基准上的大量实验表明，SPOT 能有效提升推理性能，并在解质量与覆盖率之间取得平衡。

Conclusion: SPOT 通过稀疏探测与结果校准，解决了 on-policy distillation 中探测位置选择与蒸馏目标构建两个耦合问题，在提升推理性能的同时兼顾解质量与覆盖率。

Classification reason: 论文提出 SPOT 方法，通过 on-policy distillation 提升学生模型的推理性能，属于推理能力提升与推理时优化范畴，映射到 [CANONICAL] 多模态推理与思维链子主题。

Importance reason: Focuses on distillation methodology for reasoning, not on the user's priority continuous/multimodal generation or world-model directions. Signals: on-policy distillation, KL-regularized target, reasoning benchmarks, sparse probing, verifier scoring
...
### [64] [Trace, Verify, and Correct: A Training-Free Framework for Spatial Reasoning in Multimodal LLMs](https://arxiv.org/abs/2608.04759)
...
TL;DR: 提出一个无需训练的模块化框架，通过构建空间证据图（SEG）和空间证据可靠性评估（SERA）来验证并修正 MLLM 空间推理链中的不忠实中间判断，在 15 个模型-数据集设置上平均准确率达 68.94%，比基线高 8.55 个百分点。


<details>
  <summary>Details</summary>
Motivation: MLLM 的空间推理可能产生与输入图像不一致的中间判断，导致错误沿推理链传播并影响最终答案。现有方法主要通过训练或额外空间信息改进空间推理，未考虑推理过程本身是否忠实于模型输入。研究表明不忠实的推理链会显著降低最终答案准确率。

Method: 提出模块化且无需训练的框架：1) 构建 Spatial Evidence Graph (SEG)，将 CoT 推理中提取的原子空间证据与视觉实体、空间关系、来源步骤和视觉证据关联；2) 使用 Spatial Evidence Reliability Assessment (SERA) 基于物体存在性、定位和几何测量评估视觉证据可靠性；3) 识别最早被可靠视觉证据反驳的空间证据单元，引导原始 MLLM 修正后续推理和最终答案。

Result: 在 15 个模型-数据集设置上平均准确率达 68.94%，比对比基线平均高出 8.55 个百分点。

Conclusion: 通过验证和修正推理链中的不忠实中间判断，可以显著提升 MLLM 空间推理的最终答案准确率，且无需额外训练。

Classification reason: 论文聚焦 MLLM 的空间推理，通过构建 Spatial Evidence Graph 验证和修正 Chain-of-Thought 推理链，属于多模态推理与思维链范畴，映射到 [CANONICAL] 子主题 multimodal_reasoning_cot。

Importance reason: Solid training-free spatial reasoning correction framework, but only tangential to your core continuous/multimodal generation and world-model priorities. Signals: training-free, spatial reasoning, chain-of-thought verification, multimodal LLM, reasoning correction
...
### [65] [Predict, Then Retrieve: Cross-Instance Future-State Retrieval from Video Prefixes](https://arxiv.org/abs/2608.04426)
...
TL;DR: 提出 Predictive State Retrieval (PSR) 任务：模型观察短视频前缀和关于物体未来状态的时序问题，从其他视频或图像中检索描述该状态的实例。构建了基于四个数据集的基准，并提出 LFTR 轻量检索器，通过冻结编码器预测问题与时间跨度条件化的未来潜变量，在语义与视觉空间中匹配。天花板分解显示真正的瓶颈在于预测而非感知。


<details>
  <summary>Details</summary>
Motivation: 现有任务如动作预测（预测标签）、时刻检索（定位已观察事件）和视频生成（合成像素）都无法解决"从视频前缀预测物体未来状态并跨实例检索"这一组合问题。作者希望将预测与跨实例检索结合，并揭示该任务中真正的学习瓶颈。

Method: 构建 PSR 基准（基于四个数据集，含分级人工验证的 ground truth、难度分层与 oracle 上限）。提出 LFTR：使用冻结编码器的轻量检索器，预测问题与时间跨度条件化的未来潜变量，并在互补的语义与视觉空间中匹配。通过天花板分解分析预测器与 oracle 的差距。

Result: 天花板分解显示：真实未来状态一旦被指定就高度可检索，而所有预测器（包括可访问前缀帧的大型多模态语言模型）都远低于 oracle，说明预测而非感知是核心学习挑战。LFTR 以显著更低的推理成本缩小了这一差距；消融实验表明其增益来自跨空间融合与难负样本训练，而非潜变量 rollout。

Conclusion: PSR 任务揭示了未来状态预测是核心瓶颈，LFTR 提供了低成本的有效方案。作者开源了基准、代码与评测脚本，为未来研究提供基础。

Classification reason: 论文提出 Predictive State Retrieval (PSR) 任务，核心是跨实例的未来状态检索，属于跨模态/跨实例检索范畴，映射到 [CANONICAL] 多模态检索 (multimodal_retrieval) 子主题。

Importance reason: Novel cross-instance future-state retrieval task with useful bottleneck analysis, but tangential to the user's core focus on continuous multimodal models and autoregressive generation. Signals: future-state retrieval, cross-instance retrieval, video prefix, frozen encoders, lightweight retriever
...
TL;DR: SurgNarrator 是一个面向手术视频理解的新型生成式检索框架，通过构建手术专属词汇检索空间、适配 Qwen3-VL-Embedding 学习时序感知的判别性临床表征，并采用分层过程感知检索策略，在十二个基准上零样本取得一致提升，同时将输出延迟降低两个数量级以上。


<details>
  <summary>Details</summary>
Motivation: 现有手术视频理解方法存在权衡：自回归视频语言模型支持全面推理但延迟高，不适合临床实时场景；对比模型延迟低但复杂场景理解能力弱。生成式检索虽在通用视频理解中有所探索，但直接迁移到手术领域面临近似的视觉外观可能对应语义不同事件、术语高度手术专属等挑战。

Method: SurgNarrator 包含三部分：1) 从手术字幕中构建精心策划的手术中心词汇表，定义临床有意义的检索空间；2) 适配预训练的 Qwen3-VL-Embedding-8B，使用时序感知对比目标学习判别性临床表征；3) 推理时采用分层、过程感知的检索策略，先缩小到相关手术类型再检索，实现快速有效响应。

Result: 在十二个基准的零样本设置下，SurgNarrator 相比最先进基线取得一致性能提升，同时与生成式基线相比将输出阶段延迟降低两个数量级以上。

Conclusion: 生成式检索框架能够有效平衡手术视频理解的准确性与实时性需求，通过手术专属词汇空间和分层检索策略，在保持高性能的同时大幅降低延迟，为临床实时手术视频理解提供了可行方案。

Classification reason: 论文提出 SurgNarrator，一种面向手术视频理解的生成式检索框架，核心是构建手术词汇检索空间并学习判别性临床表征，属于跨模态/视频检索方向，映射到 [CANONICAL] 多模态检索 subtopic。

Importance reason: Focuses on surgical video retrieval with negative domain keywords and no alignment with priority subtopics. Signals: generative retrieval, surgical video, zero-shot, latency reduction, contrastive learning
...
### [67] [Teaching Nemotron Greek: Mining a Corpus, Adapting Retrieval, and Grounding Generation for Modern Greek across Specialist Domains](https://arxiv.org/abs/2608.05138)
...
TL;DR: 本文对 NVIDIA Nemotron 检索栈进行现代希腊语端到端适配，涵盖语料挖掘、合成监督、检索模型训练、重排序器适配、阅读器微调，并引入首个大规模希腊语 RAG 基准 HERA。BM25 基线在专业希腊语语料上优于多个现成多语言稠密检索模型；微调后 Nemotron 1B embedder 的 nDCG@10 从 0.362 提升至 0.835；LoRA 微调 Nemotron 30B-A3B MoE 阅读器将答案正确率从 29.4% 提升至 66.9%，并显著改善忠实性与引用质量。


<details>
  <summary>Details</summary>
Motivation: 现代希腊语在 NVIDIA Nemotron 检索模型及主流多语言检索基准中缺失，但法律、能源、金融和医疗等专业领域对希腊语 RAG 系统有重要需求，因此需要端到端的希腊语检索与生成适配。

Method: 提出完整的 Nemotron 检索栈希腊语适配流程：语料挖掘、合成监督数据生成、检索模型微调（65,773 个希腊语检索对）、交叉编码器重排序器适配，以及 Nemotron 30B-A3B MoE 阅读器的 LoRA 微调，并构建 HERA 基准用于评测。

Result: 微调后 Nemotron 1B embedder 的 nDCG@10 从 0.362 提升至 0.835，显著优于未适配版本；重排序器在各专业领域一致提升；LoRA 微调阅读器将答案正确率从 29.4% 提升至 66.9%，忠实性与引用质量也显著改善。

Conclusion: 学习到的语言能力可迁移至通用领域希腊语，但相对 BM25 的优势仍依赖领域；作者发布了适配模型与 HERA 基准以支持希腊语 RAG 系统的后续研究。

Classification reason: 论文聚焦于检索增强生成（RAG）系统的端到端适配，包括检索模型训练、重排序器适配与生成器微调，属于多模态/跨语言检索与检索增强生成范畴，映射到 [CANONICAL] 多模态检索子主题。

Importance reason: Focuses on Greek-language RAG adaptation, which is outside the user's core interests in continuous multimodal models and video/image generation. Signals: Greek RAG benchmark, retrieval adaptation, LoRA reader fine-tuning, domain-specific retrieval, no continuous/multimodal generation focus
...
### [68] [Multimodal Alignment Through Joint Kernel Entropic Gromov--Wasserstein Optimal Transport](https://arxiv.org/abs/2608.04234)
*Yixuan Florence Wu, Yilun Zhu, Naichen Shi*

Main category: math.ST, cs.LG, stat.ML
...
TL;DR: 提出 JK-EGW 框架，通过联合核熵 Gromov-Wasserstein 最优传输将多模态数据对齐到共享潜空间，在数据稀缺场景下提升多模态检索性能，并建立了 n^{-1/2} 的样本复杂度界。


<details>
  <summary>Details</summary>
Motivation: 在强预训练单模态编码器可用但跨模态配对数据稀缺的场景下，如何将多模态数据对齐到共享表征空间是一个关键问题。现有对齐方法依赖原始特征空间距离，难以捕捉细粒度相似性关系。

Method: 提出联合核熵 Gromov-Wasserstein 最优传输（JK-EGW）框架，通过最小化二次最优传输目标将多模态映射到公共潜空间。利用模态内与模态间的细粒度相似性构建全局亲和核，而非依赖原始特征距离。理论上建立 n^{-1/2} 的参数样本复杂度率；算法上通过低秩核近似与变分提升推导可扩展的交替求解过程，利用现有 EOT 求解器。

Result: 在数据稀缺场景下对预训练编码器嵌入进行事后对齐，相比现有对齐基线，在跨模态检索任务上取得更优性能。

Conclusion: JK-EGW 提供了一种结构保持的多模态对齐方法，能显式控制潜嵌入的几何与分布，在数据稀缺条件下有效提升多模态检索性能，并具备理论保证。

Classification reason: 论文研究多模态数据对齐到共享表征空间以提升跨模态检索性能，属于多模态检索方向，映射到 [CANONICAL] 多模态检索 subtopic。

Importance reason: Solid theory but niche optimal-transport alignment method, far from the user's core continuous/multimodal generation and world-model focus. Signals: optimal transport, multimodal alignment, sample complexity, data-scarce retrieval, no generation
...
### [69] [CoCo-IR: Contextual Composed Image Retrieval](https://arxiv.org/abs/2608.05149)
...
TL;DR: 本文提出 Contextual Composed Image Retrieval (CoCo-IR) 任务，允许用户通过多轮交互逐步细化图像检索结果。作者基于大型多模态模型构建上下文感知推理器，生成跨轮次演化的可变换图像嵌入（TIE），并设计全自动数据引擎生成高质量上下文检索数据与难负样本。在 CIRCO 单轮基准上达到 39.4 mAP@5，在 4 轮对话的 CoCo-IR 基准上达到 44.1 R@1，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于指令的图像检索系统仅支持单轮交互，无法捕捉真实世界复杂视觉搜索的迭代性。用户往往需要多轮逐步细化检索条件，而现有方法无法利用交互历史来持续改进检索结果。

Method: 提出基于大型多模态模型（LMM）的上下文感知推理器，该模型解读完整交互历史并生成随轮次演化的可变换图像嵌入（Transformable Image Embeddings, TIE）。同时开发全自动、可扩展的数据引擎，利用 LMM 生成高质量上下文检索数据，并通过模型引导的验证挖掘具有挑战性的难负样本。

Result: 在单轮基准 CIRCO 上达到 39.4 mAP@5 的新 SOTA；在新建的 CoCo-IR 基准上，4 轮对话达到 44.1 R@1，大幅超越现有方法（28.2 4-turn R@1），后者无法处理多轮上下文。

Conclusion: CoCo-IR 任务与模型证明了多轮上下文交互对复杂视觉检索的重要性，基于 LMM 的上下文推理器结合可演化嵌入能有效处理迭代式检索，为组合图像检索开辟了多轮交互的新方向。

Classification reason: 论文提出上下文组合图像检索（CoCo-IR）任务，属于组合检索（composed image retrieval）范畴，与多模态检索方向高度契合，复用 [CANONICAL] 多模态检索 subtopic。

Importance reason: Solid but niche retrieval paper with limited overlap with your core continuous-model, generation, and world-model priorities. Signals: composed image retrieval, multi-turn interaction, LMM-based reasoning, new benchmark, strong single-turn CIRCO results
...
Main category: cs.IR, cs.CV, cs.MM
...
TL;DR: CLIP-CC-Bench 是一个面向长段落视频描述的视频语言模型评测套件，基于 5 小时电影内容切分为 90 秒片段并配专家撰写的段落级参考，采用五个 LLM 嵌入模型集成与粗细粒度语义匹配两种方法，评估了 17 个 SOTA 视频语言模型并报告 Borda 聚合排名与平均得分。


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型评测大多聚焦于短视频片段和单句指标，缺乏对长段落级视频描述生成能力的系统评估，无法判断当前模型能否生成准确的长篇描述。

Method: 构建 CLIP-CC-Bench 评测套件：从 5 小时电影内容中切分 90 秒片段，配专家撰写的段落级参考描述；采用五个 SOTA LLM 嵌入模型集成以降低单模型偏差；应用粗粒度与细粒度两种语义匹配方法比较模型生成描述与参考；通过 Borda 聚合排名和平均得分评估 17 个视频语言模型，并用 inter-judge agreement 与 bootstrap 排名稳定性量化协议内部可靠性。

Result: 评估了 17 个 SOTA 视频语言模型，报告了 Borda 聚合排名与平均得分，并量化了评测协议的内部可靠性（inter-judge agreement 与 bootstrap 排名稳定性）。

Conclusion: CLIP-CC-Bench 为长段落视频描述提供了实用的评测框架，填补了现有短片段与仅问答基准的空白，并发布标准化评测脚本、模型输出与聚合工具以支持可复现性。

Classification reason: 该论文提出 CLIP-CC-Bench，一个用于长段落视频描述评测的基准套件，评估 17 个视频语言模型，属于多模态评测与基准方向，映射到 [CANONICAL] 子主题 multimodal_benchmark。

Importance reason: Benchmarking paper for long-form video description, but it is a generic evaluation suite with no direct connection to the user's priority topics on continuous models, autoregressive generation, or world models. Signals: video-language benchmark, paragraph-level description, 17 models evaluated, LLM embedding ensemble, no autoregressive/world-model focus
...
### [71] [Hallucinations on the Board: Tool-Augmented Evaluation of LLM Chess Commentary](https://arxiv.org/abs/2608.04240)
...
TL;DR: 本文提出 ACT-Eval，一个工具增强的 LLM 国际象棋解说评测框架，将解说分解为原子声明并通过引擎工具与专家标注的黄金参考来评估事实正确性、概念覆盖与走棋质量判断，并发布包含 325 个局面-走棋对与五类错误分类法的基准。


<details>
  <summary>Details</summary>
Motivation: 超级国际象棋引擎能提供专家级评估，但缺乏自然语言解释；LLM 可弥补这一差距，却常因领域知识有限而产生幻觉，且现有基于参考或 LLM-as-judge 的评测框架无法可靠检测这些错误。

Method: 提出 ACT-Eval 框架，将国际象棋解说分解为原子声明，路由到引擎支持的工具和专家标注的黄金参考，评估事实正确性、概念覆盖和走棋质量判断；发布 325 个局面-走棋对基准，含 125 个专家验证的黄金原子与五类错误分类法。

Result: GPT-5.4 无工具时错误子声明占 22.0%，较小开源模型超过 40%；工具增强显著提升事实正确性与走棋质量评估，但所有模型对专家战略战术想法的覆盖仍有限；ACT-Eval 的事实判断落在人类间一致性范围内，覆盖分数与人类战略完整性评估强相关。

Conclusion: 事实幻觉在国际象棋解说中仍然普遍，工具增强能有效改善事实正确性与走棋质量判断，但专家级战略战术覆盖仍是所有模型的短板；ACT-Eval 提供了一种可靠且与人类判断一致的评测方案。

Classification reason: 论文提出 ACT-Eval 评测框架，用于评估 LLM 在国际象棋解说中的事实正确性、概念覆盖与走棋质量判断，属于 LLM 能力评测与基准构建，映射到 [CANONICAL] 多模态评测与基准（multimodal_benchmark）。

Importance reason: Focuses on chess commentary evaluation, which is outside the user's core interests in continuous multimodal models, video/image generation, and world models. Signals: tool-augmented evaluation, LLM hallucination benchmark, chess domain, factual correctness, coverage scoring
...
### [72] [RESPClinBench: Benchmarking Multimodal Clinical Decision-Making and Longitudinal Disease Management in Respiratory Specialty Care](https://arxiv.org/abs/2608.04514)
...
TL;DR: RESPClinBench 是一个面向呼吸专科临床决策与纵向疾病管理的多模态评测基准，包含 AECOPD-PIM（427 例 COPD 开放问答）与 PNBIM（196 例结合胸部 CT 与结构化临床信息的多模态肺结节病例）两个子集，评估七个大语言模型在 623 例病例上的临床决策能力，最终平均得分 68.58，Qwen3.6-27B 综合排名第一。


<details>
  <summary>Details</summary>
Motivation: 呼吸专科诊疗需要多模态解读、纵向风险评估、指南一致干预与全程管理，而现有医学基准多为应试导向，无法真实反映临床决策能力。作者旨在构建一个基于真实临床场景的基准，以评估大语言模型在呼吸专科临床决策中的表现。

Method: RESPClinBench 病例改编自脱敏呼吸临床数据，由三位主治医师修订病例、参考答案与原子临床行动点，一位高级呼吸专科医师进行交叉审查与最终裁定。AECOPD-PIM 含 427 例 COPD 开放问答，PNBIM 含 196 例多模态肺结节病例（CT+结构化临床信息）。七个模型通过标准化 API 推理生成 4361 条响应，自动化框架以原子行动召回率与基于 rubric 的 LLM-as-a-Judge 评估的算术平均作为最终得分。

Result: 623 例病例平均最终得分 68.58。Qwen3.6-27B 综合第一（71.22），Qwen3.5-397B-A17B 在 PNBIM 领先（72.48），Qwen3.6-27B 在 AECOPD-PIM 领先（71.11）。PNBIM 中影像幻觉与严重医疗风险分别占 31.85% 与 8.16%；AECOPD-PIM 中用药安全风险与严重医疗风险分别占 26.93% 与 1.44%。

Conclusion: RESPClinBench 识别出模型在多模态肺结节评估与纵向 COPD 管理中的任务特定局限。结合显式临床行动覆盖、整体评估与独立安全标志，为模型选择与前瞻性验证提供了临床依据。

Classification reason: 该论文提出 RESPClinBench，一个面向呼吸专科临床决策的多模态评测基准，包含多模态肺结节病例（CT+结构化临床信息）与纵向 COPD 管理评测，属于多模态评测与基准方向，映射到 [CANONICAL] 多模态评测与基准。

Importance reason: Medical benchmark with negative keywords (clinical, medical diagnosis) and no overlap with priority subtopics. Signals: medical benchmark, respiratory clinical, multimodal evaluation, negative keyword clinical, no continuous/video/world-model focus
...
### [73] [Easy to Complete, Hard to Choose: Investigating LLM Performance on the ProverbIT Benchmark](https://arxiv.org/abs/2608.04670)
...
TL;DR: 本文提出 ProverbIT，一个包含 100 道多项选择题的意大利谚语补全基准，评估 13 个前沿 LLM（含推理模型）在谚语补全、带正确答案多项选择、无正确答案多项选择三个任务上的表现，发现模型在补全任务表现良好但在无正确答案多项选择中性能大幅下降，并通过 CoT 分析揭示模型依赖记忆模式而非深层语义理解。


<details>
  <summary>Details</summary>
Motivation: LLM 在众多 NLP 任务上表现优异，但对文化嵌入的语言表达（如谚语）的理解仍存在显著差距。现有评测缺乏针对谚语补全与语义理解的系统基准，作者希望揭示模型在文化语言表达上的真实能力与局限。

Method: 构建 ProverbIT 意大利谚语基准（100 道多项选择题），设计三个任务：谚语补全、带正确答案多项选择、无正确答案多项选择。评估 13 个前沿模型（含 LRM 与传统 LLM），并对两个 LRM 进行详细的 Chain-of-Thought 分析以探究推理过程。

Result: 几乎所有模型在谚语补全任务上表现良好，但在无正确答案的多项选择格式下性能大幅下降，即使最先进的推理模型也出现显著退化。CoT 分析显示模型强烈偏向选择字面同义词，且常在推理中提及正确谚语结尾却未能识别其不在给定选项中。

Conclusion: 当前 LLM 在谚语理解上严重依赖记忆化模式而非深层语义理解，对文化根基的比喻性语言理解能力存在重要局限，凸显了模型在比喻性语言推理方面的不足。

Classification reason: 该论文提出意大利谚语补全基准 ProverbIT，评估 LLM 在谚语补全与多项选择任务上的表现，属于语言模型评测基准类工作，映射到 [CANONICAL] 多模态评测与基准（multimodal_benchmark）子主题。

Importance reason: This paper is a niche LLM benchmark on Italian proverbs with no connection to the user's priority topics on continuous/multimodal models, video/image generation, or world models. Signals: LLM benchmark, Italian proverbs, multiple-choice evaluation, chain-of-thought analysis, memorization vs semantic understanding
...
### [74] [BrainBench: Benchmarking Large Language Models for Comprehensive EEG Understanding](https://arxiv.org/abs/2608.04156)
...
TL;DR: BrainBench 是一个统一的、指令条件化的 EEG 综合理解评测基准，包含四个子集（基础分析、睡眠评估、神经认知评估、生理整合），覆盖 17 个数据集、多类任务和大量真实数据实例，通过数值、分类、集合、序列、语义和伪影六种验证方式评估 LLM 的 EEG 理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有 EEG 分析评测主要针对孤立的解码任务或系统特定演示，未能充分量化 LLM 在连接自然语言指令、信号处理、定量证据与科学解释的综合 EEG 理解能力。

Method: 构建 BrainBench 基准，包含四个子集和 17 个数据集，要求系统根据指令和 EEG 记录（可选生理信号）进行分析并生成科学报告及伪影。采用两种执行范式：自主代码执行（CodeAct）和结构化智能体分析（BrainAgent），通过数值、分类、集合、序列、语义和伪影六种验证方式评估。

Result: 在超过 10 万次执行中评估了多个代表性 LLM，结果显示不同模型、子集、难度级别和执行范式之间表现差异显著，表明 EEG 能力取决于模型及其操作化方式。

Conclusion: BrainBench 为推进基于 LLM 的 EEG 综合理解提供了可复现的测试平台，代码和基准将公开发布，评测结果持续更新。

Classification reason: 该论文提出 BrainBench，一个面向 EEG 综合理解的统一评测基准，属于多模态（脑电信号+语言指令）评测基准，映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: EEG benchmark is outside your core focus on continuous multimodal models, video generation, and world models. Signals: EEG benchmark, multimodal understanding, LLM evaluation, not in priority subtopics, no boost keywords matched
...
### [75] [GEB-Bench: Abstract Structures Told in Many Voices](https://arxiv.org/abs/2608.04111)
*Tong Zhang, Zhiyuan Shi, Yun Peng, Tao Xie*

Main category: cs.CV, cs.CL, cs.LO
...
TL;DR: GEB-Bench 是一个以抽象结构母题（自指、奇异环、莫比乌斯扭转等）为单元的多模态评测基准，每个母题通过自然场景、民间故事、数学定理和程序骨架等多种"声音"呈现，评估模型在识别结构及跨声音映射上的能力。评测 12 个开源与闭源模型后发现：抽象失败是有规律的——模型在单一声音内识别结构远好于跨声音迁移，所有模型都付出这一"税"，且错误与设计的正式几何强相关、不同厂商的前沿模型收敛到相同错误答案。


<details>
  <summary>Details</summary>
Motivation: 现有多模态评测基准多聚焦于表面内容理解（物体识别、问答等），缺乏对抽象结构推理与跨模态结构映射能力的系统评测。作者希望探究模型能否在不同模态/表达形式之间识别并迁移抽象结构（如自指、奇异环），从而揭示模型抽象能力的本质局限。

Method: 构建 GEB-Bench 基准，以抽象结构母题为单元，每个母题通过自然场景构图、民间故事（含可机械检查的形式装置）、数学定理和程序骨架等多种"声音"呈现，表面参数被声明为干扰变量不计分。母题、声音及其间结构变化构成小型跨模态范畴，评测任务即该范畴的问题。对 12 个开源与闭源模型进行系统评测，分析识别与跨声音映射的差距、错误与正式几何/感知几何的对齐关系，以及模型间错误收敛模式。

Result: 发现抽象失败是有规律的：模型在单一声音内识别结构的能力显著强于跨声音映射能力，所有模型都付出这一"映射税"；错误与设计的正式几何对齐程度高于与感知几何的对齐；不同厂商的前沿模型在相同错误答案上收敛；表面复杂度对所有读取结构的模型都造成负担，容量带来的是余量而非免疫。

Conclusion: GEB-Bench 揭示了多模态模型在抽象结构识别与跨声音映射之间存在系统性差距，且该差距具有规律性（正式几何驱动、跨厂商收敛）。这为理解模型抽象能力的边界提供了新评测范式，基准完全生成式并随流水线发布。

Classification reason: 该论文提出 GEB-Bench，一个以抽象结构母题（自指、奇异环、莫比乌斯扭转）为核心、跨自然场景/民间故事/数学定理/程序骨架多种"声音"的多模态评测基准，评估模型在跨模态结构映射与抽象推理上的能力。这属于多模态评测与基准方向，映射到 [CANONICAL] 的 multimodal_benchmark 子主题。

Importance reason: 该基准聚焦多模态抽象结构识别与跨模态映射，与用户连续多模态理解方向高度相关，且揭示模型抽象失败规律。 Signals: 多模态评测基准, 抽象结构识别, 跨声音映射, 连续多模态理解, 模型抽象失败规律
...
### [76] [SIGNPOST-Bench: Benchmarking Text-Vision Conflict Resolution in Multimodal Large Language Models](https://arxiv.org/abs/2608.04244)
...
TL;DR: SIGNPOST-Bench 是一个受控反事实基准，通过将源图像变换为 Original、Blank、Similar、Random、Adversarial 五种变体，评测 MLLM 在文本与视觉证据冲突时的消解能力。包含 5,111 个反事实组、25,555 张图像变体，评估 20 个 MLLM，发现对抗变体使地理定位中位误差从 282km 升至 1,347km（4.8 倍），且干净输入定位性能不能完全预测对冲突文本的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 基准很少揭示模型在视觉与文本证据冲突时如何仲裁取舍，缺乏受控的评测框架来度量这种冲突消解能力。

Method: 构建受控反事实基准 SIGNPOST-Bench：将每张源图像变换为 Original、Blank、Similar、Random、Adversarial 五种变体，通过合成局部场景文本干预保持非文本内容不变，从而配对测量定位性能变化及向冲突文本注入地理目标的定向偏移。

Result: 评估 20 个 MLLM（来自 7 个提供商）。对抗变体使中位定位误差从 282km 升至 1,347km（4.8 倍）；可地理编码的对抗样本中 6.5-20.1% 的预测距注入目标小于 50km；所有模型在 Blank 到 Adversarial 的目标距离上均呈现正的平均配对缩减。

Conclusion: 视觉地理定位可作为场景文本仲裁的连续诊断指标，SIGNPOST-Bench 为评测 MLLM 如何消解冲突多模态证据提供了受控框架；兼容、无关与冲突文本替换对预测产生不同影响，且干净输入定位性能不能完全预测对冲突文本的鲁棒性。

Classification reason: 该论文提出 SIGNPOST-Bench，一个用于评测 MLLM 文本-视觉冲突消解能力的反事实基准，属于多模态评测与基准方向，映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: Solid benchmark for MLLM conflict resolution, but outside the user's core focus on continuous/multimodal generation and world models. Signals: benchmark, MLLM robustness, text-vision conflict, geolocation, counterfactual evaluation
...
### [77] [IslamicTurathBench: A Multi-Task, Multi-Discipline Benchmark for Evaluating Large Language Models on the Islamic Scholarly Tradition (turath)](https://arxiv.org/abs/2608.04703)
...
TL;DR: 本文提出 IslamicTurathBench (ISTB)，一个由领域专家构建的多任务、多学科基准数据集，用于评估大语言模型在古典伊斯兰学术传统（turath）上的能力。数据集包含 3,465 个问答条目，覆盖 35 部权威著作、12 个世纪、7 个伊斯兰研究领域，并沿学术需求（初级/中级/高级）和任务格式（选择题、段落理解、开放问答）两个维度组织，提供人类参考面板评分和十个系统的零样本基线。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地被用于问答、教育和研究，包括依赖专门源传统的宗教与文化领域。然而在伊斯兰研究中，权威学术传统（turath）中的关键概念、方法和辩论缺乏高质量标注资源，导致模型在该领域的评估缺乏可靠基准。

Method: 由领域专家开发和审阅，构建包含 3,465 个问答条目的数据集，来源为 35 部公认著作，覆盖 12 个世纪、7 个伊斯兰研究领域。数据集沿两个轴组织：学术需求（初级/中级/高级）和任务格式（选择题、段落理解、开放问答），并提供人类参考面板评分和十个系统的零样本基线。

Result: 数据集包含 3,465 个问答条目，来自 35 部权威著作，覆盖 12 个世纪和 7 个伊斯兰研究领域，并提供了人类参考面板的聚合评分以及十个系统的零样本基线结果。

Conclusion: ISTB 支持在历史分层的学术领域中，跨源著作、学科、学术需求水平和问题格式对语言模型行为进行可复现的评估，填补了伊斯兰学术传统领域高质量评测资源的空白。

Classification reason: 该论文提出一个用于评估大语言模型在伊斯兰学术传统领域能力的多任务、多学科基准数据集（IslamicTurathBench），属于评测基准类工作，映射到 [CANONICAL] 多模态评测与基准（multimodal_benchmark）子主题。

Importance reason: This benchmark is domain-specific to Islamic scholarship and lacks direct connection to the user's core interests in continuous multimodal models, autoregressive generation, or world models. Signals: benchmark, LLM evaluation, Islamic studies, low topical overlap, no generation or world-model focus
...
TL;DR: OmniRouting 是首个大规模 PCB 布线推理基准，包含 1681 个工业级原理图耦合 PCB 设计，涵盖几何布线推理、设计规则感知推理、电气功能推理和工具增强智能体布线四个任务，揭示当前 LMM 在路径规划、设计规则遵循和电气功能保持方面的显著局限。


<details>
  <summary>Details</summary>
Motivation: 尽管 LLM 在约束感知导航、迷宫推理和图推理方面取得显著进展，但其在严格几何、拓扑和电气约束下进行复杂布线推理的能力尚未被探索，而布线是电子设计自动化中最具挑战性的关键环节之一。

Method: 构建包含 1681 个工业级原理图耦合 PCB 设计的大规模基准，包含板几何、人工工程师的可布线元件布局、封装、焊盘位置、网表、叠层信息和布线约束。设计四个任务：几何布线推理、设计规则感知布线推理、电气功能推理和工具增强智能体布线。

Result: 实验揭示当前 LMM 在 PCB 布线中存在显著局限：路径规划能力弱、对设计规则约束遵循度差、电气功能保持不一致。基准数据和评估代码将开源。

Conclusion: OmniRouting 填补了 LLM 在复杂约束布线推理评测方面的空白，为未来研究提供了标准化的评测平台，并指出了当前模型在空间推理与约束遵循方面的关键短板。

Classification reason: 该论文提出 OmniRouting，一个用于评估 LLM/LMM 在 PCB 布线约束感知空间推理能力的大规模基准，属于多模态评测与基准方向，映射到 [CANONICAL] multimodal_benchmark 子主题。

Importance reason: A domain-specific EDA benchmark with limited crossover to the user's core focus on continuous multimodal models, video/image generation, and world models. Signals: PCB routing benchmark, LMM spatial reasoning, design-rule constraints, tool-augmented agentic routing, no continuous token or video/image generation focus
...
### [79] [The First EgoCross Challenge at EgoVis 2026: Cross-Domain Egocentric Video Question Answering](https://arxiv.org/abs/2608.04589)
...
TL;DR: EgoCross 是首个跨域第一视角视频问答基准，在 CVPR 2026 EgoVis 研讨会上举办挑战赛，评估多模态大模型在手术、工业装配、极限运动、动物视角四个目标域上的泛化能力，共收到超1500份提交。


<details>
  <summary>Details</summary>
Motivation: 现有第一视角视频问答基准多聚焦于日常场景，难以评估多模态大模型在专业或非日常领域（如手术、工业装配、极限运动、动物视角）的泛化能力，因此需要构建跨域基准来推动该方向研究。

Method: 构建 EgoCross 跨域第一视角视频问答基准，每个测试样本包含一段第一视角视频、一个问题与四个候选答案。设置 Source-Limited Track（限制使用官方基线模型与小型支持集）和 Open-Source Track（允许更广泛模型与训练数据选择，但禁止手工构建目标域训练数据）两个官方 Codabench 赛道。

Result: 挑战赛共收到来自130多名参与者的超过1500份提交，其中19支队伍参加 Open-Source Track，38支队伍参加 Source-Limited Track。报告公布了官方排行榜结果并总结了两个赛道的获胜方案。

Conclusion: EgoCross 为跨域第一视角视频理解提供了公开的基准资源、基线实现与获胜队伍代码，可作为推进跨域第一视角视频问答研究的技术参考。

Classification reason: 该论文介绍 EgoCross 跨域第一视角视频问答基准与挑战赛，属于多模态评测与基准方向，映射到 [CANONICAL] 的 multimodal_benchmark 子主题。

Importance reason: A benchmark/challenge report on egocentric video QA, but it is a competition summary rather than a novel method, and it falls outside your core priority subtopics. Signals: egocentric video QA, cross-domain benchmark, challenge report, multimodal LLM evaluation, no method novelty
...
### [80] [Evaluating the Diagnostic Robustness of Vision-Language Models Under Visual and Textual Perturbations](https://arxiv.org/abs/2608.04885)
...
TL;DR: 本文利用组织病理学验证的脑 MRI 数据集，系统评估四类 VLM 在保持临床证据不变的扰动（切片重排、标签位置交换）下的诊断鲁棒性，发现模型在序列反转下高达 48.9% 的预测翻转、标签重排下 67.8% 的不一致诊断，以及移除病灶切片后 76.1% 的过度诊断，表明高准确率掩盖了临床可靠性缺陷。


<details>
  <summary>Details</summary>
Motivation: 标准准确率指标往往掩盖 VLM 在敏感领域（如临床诊断）中的可靠性失败，需要系统评估模型在证据保持扰动下的稳定性。

Method: 使用组织病理学验证的脑 MRI 数据集，对四类 VLM 家族施加两类证据保持扰动：重排解剖切片顺序、交换目标标签位置，并设计阴性对照测试（移除专家标注的病灶切片），评估模型预测一致性。

Result: 模型在简单序列反转下预测翻转高达 48.9%；标签重排在视觉输入不变时引发高达 67.8% 的不一致诊断；阴性对照中模型在移除病灶切片后仍产生高达 76.1% 的分类诊断。

Conclusion: 高准确率会高估临床可靠性，掩盖模型对序列呈现和文本框架的敏感性，凸显了在安全关键临床应用中部署 VLM 时引入基于稳定性的评测指标的必要性。

Classification reason: 该论文系统评估 VLM 在视觉与文本扰动下的诊断鲁棒性，属于多模态模型评测范畴，映射到 [CANONICAL] 多模态评测与基准（multimodal_benchmark）。

Importance reason: Focuses on clinical MRI diagnostic robustness, which falls outside your core interests in continuous multimodal models, autoregressive generation, and world models. Signals: medical diagnosis, negative keyword, multimodal understanding_generation, robustness evaluation, no generation or world model focus
...
### [81] [Towards a satellite image manipulation and deepfake localization benchmark dataset](https://arxiv.org/abs/2608.04840)
...
TL;DR: 本文提出一个卫星图像篡改检测与定位的初步基准数据集，包含60张图像（30张篡改、30张真实），覆盖复制粘贴拼接和扩散模型修复三种篡改类型，每张图像配有ground-truth掩码和采集元数据，支持像素级定位评测与元数据研究。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的进步，卫星图像深度伪造日益严重，但遥感社区缺乏高质量、细粒度的篡改数据集用于训练和评估检测与取证算法。现有数据集要么缺少用于定位评估的ground-truth掩码，要么由GAN或扩散模型生成的整图构成，无法衡量定位性能。

Method: 构建一个原型基准数据集，包含60张图像：30张通过三种篡改类型（复制粘贴拼接、扩散模型修复等）精心篡改，30张为真实图像。每张图像附带ground-truth掩码和采集元数据，支持像素级定位指标、图像元数据研究以及篡改检测性能与图像采集参数关系的分析。

Result: 发布了包含60张图像的原型数据集，支持像素级篡改定位评测、元数据研究以及检测性能与采集参数关联分析，为遥感图像取证和地理空间深度伪造检测提供初步基准。

Conclusion: 该原型数据集填补了遥感领域篡改检测与定位基准的空白，为后续更大规模、更细粒度的卫星图像取证数据集构建奠定基础，支持图像取证与地理空间深度伪造检测的进一步研究。

Classification reason: 论文构建卫星图像篡改检测与定位的基准数据集，属于多模态/视觉评测基准范畴，映射到 [CANONICAL] 多模态评测与基准 (multimodal_benchmark)。

Importance reason: Satellite deepfake localization is a niche forensic benchmark with no overlap with the user's core continuous multimodal, autoregressive generation, or world model priorities. Signals: satellite imagery, deepfake localization, benchmark dataset, diffusion inpainting, copy-paste splicing
...
### [82] [RUTA: Principled Visual Token Allocation via Rate-Utility Optimization](https://arxiv.org/abs/2608.04132)
...
TL;DR: RUTA 是一种基于率-效用优化的视觉 token 分配方法，在进入 LLM 前联合学习保留哪些 token 以及为每个图像-查询对分配多少 token，在 LLaVA-NeXT-7B 和 Qwen3-VL-8B 上仅用 2.0% 和 4.2% 的视觉 token 即保留 88.2% 和 94.4% 的任务性能。


<details>
  <summary>Details</summary>
Motivation: 高分辨率图像和长视频为视觉语言模型提供丰富上下文，但产生的长视觉 token 序列使 LLM 侧计算和内存开销巨大。现有视觉 token 缩减器通常以固定比例操作，或使用方法特定的学习阈值/重要性预测器来跨输入自适应 token 数量，缺乏原则性的联合优化框架。

Method: RUTA 在进入 LLM 之前进行 token 缩减，联合学习保留哪些 token 以及为每个图像-查询对分配多少 token。它构建查询条件化的候选 token，并为每个候选预测保留概率。训练时这些概率参数化独立的 Bernoulli 门控，其总和提供可微的训练时 token 数量估计。保留的 token 作为锚点，根据语义亲和性和空间邻近性聚合非保留 token 的信息。RUTA 使用带惩罚的率-效用目标进行优化，平衡下游任务损失与期望 token 使用量。

Result: 在五个基准上平均，相对于各骨干网络的完整 token 基线，RUTA 在 LLaVA-NeXT-7B 上仅使用 2.0% 的视觉 token 即保留 88.2% 的任务性能，在 Qwen3-VL-8B 上仅使用 4.2% 的视觉 token 即保留 94.4% 的任务性能。

Conclusion: RUTA 通过率-效用优化的原则性框架，实现了极低视觉 token 使用量下对任务性能的高保真保留，为视觉语言模型的高效推理提供了有效方案。

Classification reason: 该论文提出 RUTA 视觉 token 分配方法，通过率-效用优化在进入 LLM 前压缩视觉 token 数量，属于多模态高效化与 token 压缩方向，映射到 [CANONICAL] efficient_multimodal 子主题。

Importance reason: Strongly aligns with efficient multimodal inference and continuous token reduction priorities, though not directly on continuous latent modeling. Signals: visual token allocation, rate-utility optimization, efficient inference, LLaVA-NeXT, Qwen3-VL
...
### [83] [Beyond Global Routing Aggregation: Phase-Aware Expert Merging for MoE Vision-Language Models](https://arxiv.org/abs/2608.04454)
...
TL;DR: 提出 RoleMerge，一种基于阶段感知（phase-aware）的 MoE-VLM 专家合并方法。通过构建每个专家的路由角色画像（RRP）并依据专家-阶段信息损失合并兼容专家，保留答案解码专家区分，在多个模型和基准上以更少的专家保留更多性能。


<details>
  <summary>Details</summary>
Motivation: MoE-VLM 通过稀疏专家激活提升容量，但部署需存储全部专家池。训练无关的专家合并可减轻负担，但现有基于路由统计的全局聚合方法忽略了 MoE-VLM 推理的阶段结构（图像上下文、问题、答案 token 数量与路由分布不同），导致图像上下文 token 过多而掩盖阶段条件化的专家角色，使服务于不同阶段的专家被误判为可互换，损害性能。

Method: 提出 RoleMerge：从阶段归一化的路由统计中构建每个专家的路由角色画像（RRP），刻画其相对阶段偏好；以专家-阶段信息损失为引导，合并画像兼容的专家及其对应路由条目，同时保留答案解码专家的区分。

Result: 在三个模型和多个基准上，RoleMerge 在相同专家保留率下比现有专家合并方法保留更多完整模型性能，六任务宏平均性能相对提升最高达 9.6%。

Conclusion: 验证了阶段条件化的专家角色比全局路由聚合更适合作为 MoE-VLM 专家合并的依据，为高效部署 MoE-VLM 提供了更有效的训练无关合并方案。

Classification reason: 论文研究 MoE-VLM 的专家合并（expert merging）以降低部署存储开销，属于多模态模型高效化与轻量化范畴，映射到 [CANONICAL] 子主题 efficient_multimodal。

Importance reason: Solid efficiency work on MoE-VLM expert merging, but it is a niche optimization method rather than a core advance in the user's priority directions. Signals: MoE-VLM, expert merging, training-free, phase-aware routing, model efficiency
...
### [84] [Not All Redundant Tokens Are Alike: Analyzing Visual Token Pruning through Token Roles](https://arxiv.org/abs/2608.04483)
...
TL;DR: 本文通过 EmbedLens 识别的 token 角色（token roles）来分析视觉 token 剪枝方法，发现不同剪枝方法存在不同的 token 角色偏好，但这些偏好与下游性能并不直接相关；通过保留非存活（non-alive）token 的剪枝变体，发现弱语义对齐的 token 在剪枝下仍可能影响模型行为。


<details>
  <summary>Details</summary>
Motivation: VLM 将图像处理为大量视觉 token，造成推理计算瓶颈。现有视觉 token 剪枝方法通过移除看似冗余的 token 来加速推理，但这些剪枝决策与视觉 token 的功能角色之间的关系尚不明确，缺乏系统性的分析。

Method: 利用 EmbedLens 识别视觉 token 的角色（如 alive/non-alive 等），分析代表性剪枝方法在不同 token 角色上的剪枝偏好；随后改进 token 角色分配流程，并设计保留特定角色 token 的剪枝变体（role-protected pruning）进行对照实验。

Result: 代表性剪枝方法表现出不同的 token 角色偏好，但这些偏好与下游性能无直接相关性；保留非存活 token 的剪枝变体有时能维持甚至提升性能，说明弱直接语义对齐的 token 在剪枝下仍可能影响模型行为。

Conclusion: 视觉 token 剪枝决策与 token 功能角色之间存在复杂关系，不能仅依据 token 的语义对齐强度来判断其可剪枝性；保留某些弱语义 token 可能对模型行为有益，为设计更有效的 token 剪枝策略提供了新视角。

Classification reason: 论文研究视觉 token 剪枝（visual token pruning）对 VLM 推理效率的影响，属于多模态高效化与轻量化方向，映射到 [CANONICAL] efficient_multimodal 子主题。

Importance reason: Analyzes visual token pruning via token roles, relevant to efficient multimodal inference but not directly aligned with top continuous-model or generation priorities. Signals: visual token pruning, token roles, EmbedLens, VLM efficiency, pruning analysis
...
### [85] [DIVE: Dynamic Iterative Visual Evidence Construction for Efficient Vision-Language Models](https://arxiv.org/abs/2608.04496)
...
TL;DR: DIVE 提出一种无需训练的视觉 token 剪枝框架，将 token 剪枝重构为动态证据构建过程，通过"选择-更新-重评估"循环保留互补且与提示相关的视觉证据，在减少 88.9% 视觉 token 的情况下仍保持 98.2% 的平均性能。


<details>
  <summary>Details</summary>
Motivation: 视觉输入在 VLM 中被编码为远长于文本的 token 序列，成为推理效率的主要瓶颈。现有方法通常单次打分并剪枝低分 token，但单次打分不足以反映 token 的提示相关有用性，因为该有用性取决于已保留的证据。

Method: DIVE 将视觉 token 剪枝重构为动态证据构建：反复选择残差条件得分最高的剩余 token，更新视觉与提示残差以折扣已解释的证据，并重新评估剩余 token。该"选择-更新-重评估"过程构建出一个互补且提示相关的证据保留集，全程无需训练。

Result: 在八个图像理解基准上，DIVE 在不同 token 预算下均保持性能。在减少 88.9% 视觉 token 时，仍保留未压缩模型平均性能的 98.2%。

Conclusion: DIVE 证明动态迭代的证据构建比单次剪枝更有效，通过残差条件得分与证据折扣机制能够保留互补且提示相关的视觉证据，在显著压缩视觉 token 的同时保持模型性能。

Classification reason: 论文聚焦于通过动态迭代视觉证据构建来压缩视觉 token、提升 VLM 推理效率，属于多模态高效化与 token 压缩方向，映射到 [CANONICAL] efficient_multimodal 子主题。

Importance reason: Training-free visual token pruning with dynamic evidence construction directly addresses efficient inference for multimodal LLMs, a high-priority direction. Signals: efficient inference, visual token pruning, training-free, vision-language models, token reduction
...
### [86] [HiSC: Hierarchical Spatial Clustering Token Compression for Efficient 3D Scene Understanding](https://arxiv.org/abs/2608.04610)
...
TL;DR: HiSC 是一个无需训练的层次化空间聚类 token 压缩框架，用于 3D 视觉语言模型（3D VLM）。它通过空间图合并（SGraM）在 LLM 推理前合并跨视角冗余 token，并在推理中通过空间聚类剪枝（SCluP）进行簇内与簇间层次压缩，在保持对象完整性和细节的同时实现超过 90% 的 token 缩减。


<details>
  <summary>Details</summary>
Motivation: 3D 视觉语言模型（3D VLM）在多视角场景中进行空间推理时，由于重复观测和大量无信息区域导致 token 冗余严重，计算成本高昂。现有的视觉 token 压缩方法主要针对 2D VLM，无法捕捉 3D 场景的结构化特性，导致空间覆盖不完整和细粒度细节丢失。

Method: HiSC 将 token 压缩从 token 级选择提升到簇级处理，利用联合几何与语义线索将 token 组织为空间锚定的簇。具体包含两个策略：(1) 空间图合并（SGraM）：将跨视角冗余建模为空间连通性，在 LLM 推理前合并物理一致的相似冗余 token；(2) 空间聚类剪枝（SCluP）：在 LLM 推理内进行簇间与簇内层次压缩，保留对象实例完整性并维持重要区域的细粒度细节。

Result: 在多个 3D 推理基准上的实验表明，HiSC 在高效视觉 token 剪枝比例下表现有效，可实现超过 90% 的 token 缩减且性能下降极小。

Conclusion: HiSC 通过层次化空间聚类实现了 3D VLM 的高效 token 压缩，在保持空间覆盖和细粒度细节的同时大幅降低计算成本，为 3D 场景理解中的 token 冗余问题提供了有效解决方案。

Classification reason: 论文提出 HiSC 框架，通过层次化空间聚类对 3D VLM 的视觉 token 进行压缩以降低计算成本，属于多模态模型 token 压缩与高效化方向，映射到 [CANONICAL] efficient_multimodal（多模态高效化与轻量化）。

Importance reason: Efficient token compression for 3D VLMs is relevant to model efficiency, but the 3D-specific focus and lack of continuous/multimodal generation novelty limit its priority. Signals: training-free token compression, 3D VLM efficiency, >90% token reduction, spatial clustering, no continuous token or generation focus
...
TL;DR: SlotNarrative 提出一种基于 slot 的视觉接口，将视频组织为持久对象叙事，用紧凑的对象状态 token 表示，仅需 144 个视觉 token 位置即可驱动冻结的 Video-LLM，在精度与 token 数之间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: Video-LLM 在开放视频理解上取得进展，但其视觉接口 token 密集且缺乏显式结构来关联跨时间的重复对象证据。现有方法通常在建立时序对应前压缩帧级特征，效率与结构均有限。

Method: SlotNarrative 首先将视觉特征分组为对象级 slot，再通过轻量、无参数的记忆模块整合多种互补匹配线索，将重复观测关联到 clip 级对象条目。每个保留条目序列化为两类 token：身份 token（总结持久对象外观）与状态 token（编码段级外观、几何、可见性与轨迹信息）。最终仅分配 144 个视觉 token 位置给冻结的 Video-LLM，与采样帧数无关。

Result: 在多个数据集上，SlotNarrative 相比先前的紧凑 Video-LLM 接口在精度与视觉 token 数之间取得更优的权衡，验证了持久对象叙事作为紧凑、结构化、时序组织的视觉接口的有效性。

Conclusion: 持久对象叙事可作为 Video-LLM 的一种紧凑、结构化且时序组织的视觉接口，在显著降低 token 开销的同时保持理解精度。

Classification reason: 论文提出 SlotNarrative，通过 slot 化视觉特征与对象状态 token 将视频压缩为仅 144 个视觉 token 位置，显著降低 Video-LLM 的视觉 token 开销，属于多模态高效化与 token 压缩方向，映射到 [CANONICAL] efficient_multimodal。

Importance reason: Token-efficient video interface with persistent object narratives aligns with user's efficiency and multimodal understanding priorities, though not directly on continuous latent modeling. Signals: token-efficient video-LLM, slot-based visual interface, persistent object narratives, 144 visual tokens, frozen Video-LLM
...
<div id='direction-multimodal_understanding_generation-avatar_motion_generation'></div>

## 数字人与动作生成 [[Back]](#toc)

虚拟人、说话头、人体动作、肖像动画、面部重演。

### [88] [EmpaAva: An Open-source Agentic 3D-Avatar Empathetic Live Chatbot](https://arxiv.org/abs/2608.04709)
...
TL;DR: EmpaAva 是首个开源、智能体化的3D数字人共情聊天机器人，通过视频通话式界面让用户与3D数字人实时对话，数字人从语音和视觉中感知情感，并以情感语音、唇形同步面部动作和照片级3D高斯渲染回复。核心是一个LLM协调的三智能体架构（感知、共情回复规划、具身渲染闭环），配合回复规划层将每条回复编译为可执行的多模态计划。


<details>
  <summary>Details</summary>
Motivation: 将共情回复生成（ERG）从纯文本交互扩展到实时的面对面交互，填补开源3D数字人共情聊天机器人的空白，使数字人能够感知用户情感并以多模态方式（语音、表情、渲染）进行共情回应。

Method: 采用LLM协调的Tri-Agent架构：感知智能体从语音和可选视觉中读取情感，共情回复规划智能体生成情感回复，具身渲染智能体负责3D高斯渲染与唇形同步面部动作，三者形成闭环。回复规划层将每条回复编译为可执行的多模态计划，统一语音、表情和渲染于同一共情意图。

Result: 在自动和人工评测中，EmpaAva在情感理解、回复质量和音视频一致性上均超越纯文本、2D说话头和多模态数字人基线。

Conclusion: EmpaAva证明了基于开源模块构建可控、可检查的3D数字人共情聊天机器人的可行性，并开源了系统与在线演示。

Classification reason: 论文提出一个3D数字人共情聊天机器人，涉及3D虚拟人、说话头、面部动画、语音驱动表情与3D高斯渲染，属于数字人与动作生成（avatar_motion_generation）范畴。同时涉及多模态理解与生成（情感感知、多模态回复规划），故匹配多模态理解与生成方向。

Importance reason: 3D数字人共情聊天机器人，与用户重点关注的连续多模态/世界模型方向关联较弱，且非生成类核心任务。 Signals: 3D avatar, empathetic chatbot, tri-agent architecture, multimodal plan, open-source
...
### [89] [SmartMage: Dynamic Modality Orchestration for 3D Scene Understanding](https://arxiv.org/abs/2608.05137)
...
TL;DR: SmartMage 是一个统一的多模态大语言模型，通过语义引导的模态自适应路由（SMART）和模态感知门控专家（MAGE）动态编排视觉与几何等异构模态，实现语义感知的 3D 场景理解，在五个 3D 场景理解基准上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 在 3D 场景理解中通常依赖固定的模态组合，忽略了不同查询对模态的相关性差异。这种刚性设计会引入无关模态的语义噪声，同时未能充分利用信息量更大的模态，导致计算浪费和推理能力被稀释。

Method: SmartMage 包含两个核心模块：(1) Semantic-guided Modality Adaptive RouTng (SMART) 模块，利用语义先验、文本-模态对齐和模态质量来选择任务相关的模态；(2) Modality-Aware Gating Expert (MAGE) 模块，利用模态先验引导专家激活，促进多模态推理中的自适应专业化。此外还构建了诊断基准 ScanFacet，将任务划分为细粒度语义类别以分析各语义类型偏好的模态组合。

Result: SmartMage 在五个 3D 场景理解基准上达到 SOTA 性能，并在 RGB-only 视频理解基准上取得有竞争力的结果。诊断基准 ScanFacet 中观察到的模态-语义模式进一步验证了方法的有效性。

Conclusion: 动态模态编排能够根据查询语义自适应选择相关模态，有效减少语义噪声并提升 3D 场景理解性能，为多模态推理中的模态选择提供了新的思路。

Classification reason: 论文提出 SmartMage，一个面向 3D 场景理解的多模态大语言模型，动态编排视觉与几何等异构模态。这属于空间与 3D 多模态推理方向，映射到 [CANONICAL] spatial_3d_reasoning。同时与具身智能/世界模型相关，但核心是 3D 场景理解与多模态推理。

Importance reason: Solid 3D MLLM routing work, but it is outside your core continuous-model/video-generation focus and lacks the transformative novelty needed for a top daily pick. Signals: 3D scene understanding, modality routing, gating experts, MLLM, SOTA on 3D benchmarks
...
### [90] [Towards Physics of Multimodal Pretraining: Knowledge Flow, Modality Synergy, Early Unification, and Recipes](https://arxiv.org/abs/2608.05000)
...
TL;DR: 本文通过受控实验系统探索多模态统一预训练的"物理机制"，揭示知识流、模态协同与竞争、早期统一以及高效训练配方四大洞察，并在 13.5B MoE 模型上以 2T token 规模验证。


<details>
  <summary>Details</summary>
Motivation: 尽管原生统一多模态预训练正成为趋势，但模态在统一训练中如何交互的设计空间与底层机制仍未被充分探索，缺乏原则性的理解来指导规模化。

Method: 在合成与大规模真实数据集上进行受控实验，解耦语言、视觉理解与视觉生成之间的知识流，分析数据复杂度对模态协同/竞争的影响，比较共享注意力与归一化等架构选择，并推导高效预训练配方，最终在 13.5B MoE 模型上以 2T token 验证。

Result: 四大发现：(i) 知识流呈现跨模态不对称影响模式；(ii) 数据复杂度决定模态协同或竞争，共享注意力与归一化加模态特定 FFN 促进协同，且跨不同视觉 tokenizer 泛化；(iii) 早期统一优于晚期对齐或顺序训练，并揭示"视觉懒惰"现象；(iv) 仅用 5% 计算预算即可获得强生成性能的高效配方。

Conclusion: 该研究为理解和扩展多模态预训练提供了原则性基础，明确了架构选择、训练顺序与数据复杂度对模态交互的影响，并给出可复现的高效训练配方。

Classification reason: 论文系统研究原生统一多模态预训练（理解与生成一体化）的机制与配方，属于统一多模态模型方向，映射到 CANONICAL 子主题 unified_multimodal_model。

Importance reason: Directly addresses unified multimodal pretraining mechanisms and efficient recipes, highly aligned with your priority on unified multimodal models and model efficiency. Signals: unified multimodal, mllm, efficient inference, controllable generation, multimodal understanding_generation
...
### [91] [ToolArtist: Tool-Using Unified Multimodal Models for Agentic Image Generation](https://arxiv.org/abs/2608.04436)
...
TL;DR: ToolArtist 通过对统一多模态模型（UMM）进行后训练，构建完全智能体化的图像生成模型。它在一个统一策略中动态编排推理、外部工具调用与原生图像生成，采用 SFT 收集教师智能体轨迹并转换为 UMM 兼容格式，再通过提出的 Reason-Act-Draw GRPO（RAD-GRPO）强化学习算法，用互补的意图与质量奖励联合优化模型。


<details>
  <summary>Details</summary>
Motivation: 现有文生图模型在需要复杂语义理解、多步推理和外部世界知识整合的开放世界任务上能力有限。已有工作虽引入智能体能力，但要么预设固定工作流，要么只将部分生成过程置于智能体控制之下，导致推理、工具调用与图像生成无法由单一策略协调。

Method: ToolArtist 采用两阶段后训练：(1) SFT 阶段，用配备搜索工具和图像生成工具的教师智能体收集轨迹，转换为 UMM 兼容格式，隐藏图像生成工具但保留生成的图像；(2) RL 阶段，开发面向 UMM 的智能体强化学习基础设施，提出 Reason-Act-Draw GRPO（RAD-GRPO），使用互补的意图奖励与质量奖励联合优化模型。

Result: 实验表明，将整个开放世界图像生成过程置于智能体策略之下，一致优于固定流水线或仅部分智能体控制的方案。

Conclusion: 完全智能体化的图像生成策略优于固定流水线或部分智能体控制的方法，并公开了训练数据与完整的后训练基础设施。

Classification reason: 论文提出 ToolArtist，通过对统一多模态模型（UMM）进行后训练，使其具备智能体式图像生成能力，将推理、工具调用与原生图像生成统一到一个策略中。这属于理解与生成一体化的统一多模态模型范畴，映射到 [CANONICAL] unified_multimodal_model 子主题。

Importance reason: Directly advances unified multimodal models with agentic tool use and RL post-training, matching a top-priority subtopic. Signals: unified multimodal model, agentic image generation, tool use, RL post-training, GRPO
...
### [92] [Breaking the Curse ofMultilinguality inMany-to-Many Speech-to-Text Translation via a Resource-AwareMixture of Speech Encoders](https://arxiv.org/abs/2608.04586)
...
TL;DR: 提出 MSRT 框架，通过资源感知的混合语音编码器（MoSE）解决多语言语音到文本翻译中的"多语言诅咒"问题，使用显式语言路由器将话语分配给专家编码器，并引入五阶段课程学习策略，仅需每语言 10 小时配对数据即可在 45 种语言上实现 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在语音到文本翻译（S2TT）中取得显著成功，但单一共享语音编码器处理多语言输入时面临"多语言诅咒"：不同资源水平的语言竞争有限的表征容量，导致高资源语言性能强而低资源语言性能大幅下降。

Method: 提出 MSRT 框架，核心是资源感知的混合语音编码器（MoSE）：显式语言路由器将每条话语分配给合适的专家编码器；冻结的专家保留高资源语言能力，可训练的专家专门适配中低资源语言。同时引入五阶段课程学习策略，显著降低数据依赖，仅需每语言 10 小时配对 S2TT 数据即可有效对齐。

Result: 在 45 种语言上系统评估全部 45×44 个翻译方向。4B 参数模型达到 SOTA 性能，超越规模更大的基线模型。MoSE 同时提升高、中、低资源语言性能，其中低资源语音提升最大，在不牺牲高资源性能的前提下打破多语言诅咒。

Conclusion: 资源感知的混合语音编码器能有效缓解多语言语音翻译中的表征竞争问题，通过专家分工与课程学习实现数据高效的多语言 S2TT，为多语言语音翻译研究提供了新思路。

Classification reason: 论文提出基于多模态大语言模型（MLLM）的语音到文本翻译框架 MSRT，属于通用 MLLM 架构与多模态指令微调范畴，映射到 [CANONICAL] vlm_mllm_general 子主题。

Importance reason: Solid multilingual S2TT engineering with curriculum learning, but outside the user's core continuous-model, video-generation, and world-model focus. Signals: mixture of experts, speech-to-text translation, multilingual, curriculum learning, MLLM
...
### [93] [Radar4D-VLM: Proposal-Grounded Temporal 4D Radar Reasoning Across Frozen Language Models](https://arxiv.org/abs/2608.04130)
...
TL;DR: Radar4D-VLM 是一个仅依赖 4D 雷达点云（无需相机或 LiDAR）的时序视觉语言模型，从连续十帧雷达扫描中提取几何锚定的目标提案，组织为 object/scene/kinematic 三级 token 层级，通过参数高效投影器映射到冻结语言骨干，并联合建模目标计数、空间分布、运动状态、碰撞风险、语义类别与径向速度。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶视觉语言模型主要依赖相机和 LiDAR，4D 雷达作为独立感知模态在很大程度上未被探索，尽管它在恶劣能见度下具有鲁棒性并能直接测量径向速度。作者希望建立仅基于 4D 雷达的时序视觉语言推理基础。

Method: Radar4D-VLM 提取几何锚定的目标提案，将雷达证据组织为 object、scene、kinematic 三级 token 的紧凑层级；用参数高效投影器将这些 token 映射到冻结语言骨干；可审计预测头联合建模目标计数、空间分布、运动状态、碰撞风险、语义类别和径向速度。在统一冻结骨干接口下结合提案锚定的时序目标 token 化、全局场景上下文与显式运动学 token。

Result: 在序列隔离的 K-Radar 开发验证集上，Top-64 提案召回率达 98.13%（4m），超过固定网格和均匀随机对照 6.40 和 22.83 个百分点。在八个冻结 Qwen、Phi、Mistral、Llama、Gemma 骨干的 24 组匹配运行中，雷达 token 接口在所有五个语言模型家族上兼容；对齐/置换/无语言对照显示传感器依赖但无稳定的直接头增益。

Conclusion: Radar4D-VLM 建立了仅雷达多模态场景与运动推理的可复现基础，同时将接口兼容性与语言监督的收益分离——接口在所有语言模型家族上通用，但语言监督并未带来稳定的直接头增益。

Classification reason: 该论文提出 Radar4D-VLM，一个基于 4D 雷达点云的时序视觉语言模型，将雷达证据组织为 object/scene/kinematic token 并映射到冻结语言骨干，属于通用视觉语言模型架构设计，映射到 [CANONICAL] vlm_mllm_general。

Importance reason: Radar-only VLM for driving is a niche application with limited overlap with your core continuous multimodal/world-model focus. Signals: radar-only VLM, frozen LLM adaptation, temporal 4D point cloud, driving perception, proposal-grounded tokens
...
### [94] [TriCLE: Tri-Modal Vision-Language Reasoning for Edge-Deployed Fine-Grained Clustering](https://arxiv.org/abs/2608.04175)
...
TL;DR: TriCLE 是一个面向边缘部署的飞机分类三模态视觉语言系统，从单张 RGB 图像生成 FLIR 热像与伪 LiDAR 深度投影，融合到紧凑的 Qwen3-VL 骨干中，并通过 GSPO 策略对齐实现 88.33% 验证准确率，量化后可在 8GB 边缘设备上运行。


<details>
  <summary>Details</summary>
Motivation: 边缘平台进行空中观测时需在有限内存、有限算力和间歇性连接下解释飞机图像，而标准 RGB 识别模型和通用 VLM 在缺乏校准热像与 LiDAR 数据时难以胜任，需要一种面向边缘约束的飞机分类系统。

Method: TriCLE 从单张 RGB 飞机图像生成结构保持的 FLIR 风格热像视图和伪 LiDAR 深度投影，将三个对齐视图与任务指令融合到紧凑的 Qwen3-VL 骨干中。模型对齐到基于推进方式、机身家族、尺寸、设计年代和配置的专家飞机分类体系。评估了监督微调、旋转保持 SFT 以及 GRPO、GSPO、DAPO 三种策略对齐方法，并采用 4-bit 量化与注意力内存优化以适配边缘部署。

Result: 序列级 GSPO 取得最强验证性能，验证准确率 88.33%、加权 F1 0.91；在留出测试集上准确率 78.00%、加权 F1 0.793，同时保持 94.00% 的可解析输出格式。4-bit 量化后 4B 模型适配 8GB 部署目标，每个三模态三元组处理耗时 1.48 秒。

Conclusion: TriCLE 作为可解释、边缘可行的飞机分组实用原型，验证了三模态视觉语言方法在边缘约束下的有效性，同时强调需要在真实对齐的热像与 LiDAR 传感器流上进一步验证。

Classification reason: 论文提出 TriCLE，一个基于 Qwen3-VL 骨干的三模态视觉语言系统，用于边缘部署下的飞机分类分组，属于通用视觉语言模型与 MLLM 架构及指令微调范畴，映射到 [CANONICAL] vlm_mllm_general。

Importance reason: Niche edge-deployed aircraft classification with synthetic thermal/LiDAR, far from the user's core continuous multimodal and video generation focus. Signals: edge deployment, tri-modal fusion, Qwen3-VL, GSPO policy alignment, aircraft taxonomy
...
### [95] [EndoVLM: An Endoscopy Vision-Language Pre-training Model via Anatomy-Guided Sparsity and Progressive Alignment](https://arxiv.org/abs/2608.04472)
...
TL;DR: EndoVLM 是一个面向内窥镜影像分析的视觉语言预训练基础模型，利用超过 34.8 万对内窥镜检查报告与图像集合进行预训练，通过解剖引导稀疏池化、渐进语义感知对齐和语义集中掩码自编码器，实现从全局患者级匹配到细粒度局部对齐的跨模态学习，在多项下游任务中超越现有基础模型并具备零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有内窥镜基础模型主要依赖单模态图像或视频的自监督学习，忽略了临床报告中丰富的语义知识；同时，结构化解剖描述与高冗余、未整理的视觉流之间存在根本性的模态鸿沟，难以有效利用这些临床记录。

Method: EndoVLM 包含三个核心组件：(1) 解剖引导稀疏池化（Anatomy-Guided Sparse Pooling），以文本描述为查询驱动稀疏注意力，从冗余图像集中聚合语义显著帧为解剖特异的视觉表征；(2) 渐进语义感知对齐（Progressive Semantic-Aware Alignment），通过结构化软目标建模临床分类体系（解剖与病理状态），弥合从全局患者级匹配到细粒度局部对齐的鸿沟；(3) 语义集中掩码自编码器（Semantic-Concentrated Masked Autoencoder），仅应用于语义丰富帧，整合低层视觉精度与高层语义表征。

Result: 在多种下游任务上的大量实验表明，EndoVLM 优于现有基础模型，并与任务特定方法具有竞争力；同时展现出稳健的零样本泛化能力，凸显其在更广泛临床应用中的潜力。

Conclusion: EndoVLM 通过解剖引导的稀疏池化与渐进对齐策略，有效利用临床报告中的语义知识来预训练内窥镜视觉语言基础模型，为医学影像基础模型的发展提供了新思路，并展现出良好的零样本泛化潜力。

Classification reason: 该论文提出 EndoVLM，一个面向内窥镜图像分析的视觉语言预训练基础模型，属于通用视觉语言模型（VLM/MLLM）范畴，映射到 [CANONICAL] vlm_mllm_general 子主题。

Importance reason: Domain-specific medical VLM with limited transfer to the user's core focus on continuous multimodal models and autoregressive generation. Signals: medical VLM, endoscopy, vision-language pretraining, sparse pooling, progressive alignment
...
### [96] [FocusMem: Factorizing Content, Readout, and Trust in Latent GUI Memory](https://arxiv.org/abs/2608.04530)
...
TL;DR: FocusMem 提出一种分解式潜空间 GUI 记忆接口，将内容保留、状态条件读取与信任门控分离，在冻结 GUI 策略下训练，提升五个 GUI 智能体基准上的表现。


<details>
  <summary>Details</summary>
Motivation: GUI 智能体需要同时记住先前任务的经验与当前交互的未完成进度。现有潜空间记忆方法将每条轨迹映射为固定记忆块并主要依赖 next-action 监督训练，导致压缩时丢失细节、同一记忆块需服务不同决策阶段、无关检索轨迹误导智能体等问题。

Method: FocusMem 在紧凑潜记忆接口中分离三类职责：角色感知的内容基（role-aware content basis）让情景记忆保留可复用经验、工作记忆保留任务进度；状态条件读取（state-conditioned readout）为同一存储证据生成决策特定视图；轻量信任门（trust gate）抑制与当前步骤无关的记忆块。所有组件在 GUI 策略冻结的情况下训练。

Result: 在五个 GUI 智能体基准上，FocusMem 一致优于完全匹配的仅动作固定记忆基线与先前潜记忆适配方法。分析表明语义与功能监督保留互补信息，状态条件读取在轨迹上下文增长时更鲁棒，信任门减少了注入无关情景证据造成的损害。

Conclusion: 有效的潜记忆不仅取决于压缩过去交互，还取决于保留什么、暴露什么以及允许什么。FocusMem 通过分解内容、读取与信任三个维度，显著提升了 GUI 智能体的潜空间记忆能力。

Classification reason: 论文提出 FocusMem，为 GUI 智能体设计潜空间记忆接口，将多模态轨迹压缩为连续 token 并用于决策。这属于视觉语言模型/多模态智能体架构范畴，映射到 [CANONICAL] vlm_mllm_general（视觉语言模型与 MLLM）最为贴切。

Importance reason: Solid latent memory work for GUI agents, but tangential to your core focus on continuous multimodal models and generative world models. Signals: latent memory, GUI agents, continuous tokens, frozen policy, trust gate
...
### [97] [On the Effectiveness of Adaptation Strategies for VLM-Based Federated Learning in Remote Sensing](https://arxiv.org/abs/2608.04791)
...
TL;DR: 本文首次系统比较了遥感图像分类联邦学习中 VLM 的四种适配策略（全微调、编码器微调、提示学习、LoRA），从非 IID 泛化能力、通信开销和本地计算复杂度三个维度进行分析，并在 BigEarthNet-S2、EuroSAT、RESISC45 和 ImageNet 上验证，最终给出不同约束下适配策略选择的指导准则。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在遥感领域因隐私、法规和带宽限制而尤为重要，但非 IID 数据异构会阻碍收敛和泛化。VLM 具有可迁移表征可缓解分布偏移，但其大参数量在联邦设置中会带来通信和计算负担，因此需要研究如何选择合适的 VLM 适配策略以平衡泛化能力与资源约束。

Method: 作者对遥感图像分类联邦学习中的四种 VLM 适配策略进行系统比较：全微调、编码器特定微调、提示学习（prompt learning）和低秩适配（LoRA）。从三个标准评估：非 IID 数据下的泛化能力、通信开销、本地计算复杂度。在 BigEarthNet-S2、EuroSAT、RESISC45 和 ImageNet 四个数据集上进行实验。

Result: 实验揭示了任务特化、跨域泛化与效率之间的不同权衡。不同适配策略在非 IID 数据下的泛化表现、通信开销和本地计算复杂度上各有优劣，没有单一策略在所有维度上最优。

Conclusion: 基于实验结果，作者提出了在不同运行约束下为遥感图像分类联邦学习选择合适 VLM 适配策略的指导准则，帮助研究者和实践者在泛化能力与通信/计算约束之间做出权衡。

Classification reason: 论文研究 VLM 在联邦学习中的适配策略（全微调、编码器微调、提示学习、LoRA），属于视觉语言模型的应用与适配研究，映射到 [CANONICAL] vlm_mllm_general 子主题。

Importance reason: Remote sensing federated learning with VLM adaptation is outside your core focus on continuous multimodal models and video/world-model generation. Signals: remote sensing, federated learning, VLM adaptation, non-IID generalization, communication overhead
...
### [98] [UG-UMRE: Uncertainty-Guided Modality Augmentation and Distributional Calibration for Unified Multimodal Relation Extraction](https://arxiv.org/abs/2608.04949)
*Bo Kong, Liruiz Jia, Yi Liang, Chao Liu, Dongfang Han, Tianwei Yan, Yuan Liu, Shengquan Liu*

Main category: cs.CV, cs.IT, cs.CL, cs.MM
...
TL;DR: 提出UG-UMRE网络，通过不确定性引导的单模态增强（UDUA）和联合偶然不确定性对齐（JAUA）模块，解决统一多模态关系抽取中的噪声传播和模态分布异质性问题，在UMRE、MORE和MNRE三个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态关系抽取（UMRE）方法存在两个关键问题：一是忽略了固有的偶然不确定性导致噪声传播；二是不同模态分布之间的深层异质性阻碍了跨模态对齐。

Method: 提出UG-UMRE网络，包含两个核心模块：(1) 不确定性驱动的单模态增强模块（UDUA），基于变分信息瓶颈将特征建模为高斯分布，结合不确定性感知的自监督对比学习机制过滤噪声并保持语义一致性；(2) 联合偶然不确定性对齐模块（JAUA），作为全局语义预校准机制，利用概率分布一致性构建共享潜空间，通过同步跨模态统计属性消除分布差距。

Result: 在UMRE、MORE和MNRE三个基准数据集上达到最先进性能，进一步分析验证了UDUA和JAUA模块的可插拔性和有效性。

Conclusion: 不确定性引导的模态增强与分布校准方法能有效缓解UMRE中的噪声传播和模态异质性问题，且所提模块具有可插拔性，可推广到其他多模态任务。

Classification reason: 该论文研究统一多模态关系抽取（UMRE），涉及文本实体与视觉对象之间的跨模态关系识别，属于多模态理解与推理任务，映射到多模态理解与生成方向下的"视觉问答与文档理解"（multimodal_vqa_doc）子主题，因为关系抽取与VQA/文档理解同属多模态语义理解范畴。

Importance reason: Focuses on multimodal relation extraction with uncertainty modeling, which is outside your core priorities of continuous multimodal models, autoregressive generation, and world models. Signals: multimodal relation extraction, uncertainty-guided augmentation, distributional calibration, contrastive learning, SOTA on UMRE/MORE/MNRE
...
### [99] [Representing Visual Evidence for Item Difficulty Prediction: Visual Textualization and Image-Native Modeling](https://arxiv.org/abs/2608.04554)
...
TL;DR: 本文研究题目难度预测中视觉证据的表示方式，比较纯文本、视觉文本化（将视觉证据转成语言）与图像原生建模（保留原始图像）三种接口，使用 Eedi 数学题目和 LLM/VLM 进行难度回归，发现两种视觉接口均达到最低点估计误差，且图像原生建模是文本化的有力替代方案。


<details>
  <summary>Details</summary>
Motivation: 在缺乏足够学生作答数据时，从题目内容预测难度可为新题提供初始估计。现有方法通常将题目题干和选项表示为文本，当数学题含视觉组件时，常见做法是先文本化视觉证据再用文本预测器。作者质疑这一默认流程，探究视觉证据应如何表示才能更好地预测题目难度。

Method: 使用 Eedi 题目（难度由学生作答校准），对比三种表示：纯题干文本、视觉文本化（用 Open-VLM 将视觉证据转成语言）、图像原生建模（保留原始图像）。分别训练 LLM 和 VLM 直接进行难度回归，并评估测试时干预与计算工作流差异。

Result: 两种视觉接口均达到最低点估计误差，但领先系统无法可靠排序。Open-VLM 文本化对所有 LLM 产生更低的 RMSE 点估计，而更广泛的适配对所有图像原生 VLM 更优。测试时干预显示对完整题目图像有依赖，但无法隔离额外视觉组件。两种视觉接口在题目级错误上部分互补，计算工作流差异显著。

Conclusion: 文本化不应被视为唯一实用的视觉证据接口：图像原生建模是竞争性替代方案，其有效性取决于 VLM 的适配方式。两种接口的错误互补性提示未来可探索融合策略。

Classification reason: 论文研究如何表示视觉证据以预测题目难度，涉及数学题目中的视觉组件（图表等）理解与 VLM 建模，属于视觉问答与文档理解范畴，映射到 [CANONICAL] 子主题 multimodal_vqa_doc。

Importance reason: 该论文聚焦教育测评中的题目难度预测，与用户关注的多模态生成、世界模型和连续表示方向关联较弱。 Signals: 视觉文本化与图像原生建模对比, 题目难度回归, VLM/LLM 回归, 教育测评应用, 非生成式任务
...
### [100] [Monte Carlo Tree Search for Table-to-Multimodal Report Generation](https://arxiv.org/abs/2608.04071)
...
TL;DR: 提出 MCTS-Report，一种基于蒙特卡洛树搜索（MCTS）驱动的框架，将表格到多模态报告生成建模为结构化搜索空间上的渐进式构建过程，通过原子动作分解、LLM 推理轨迹存储、多维奖励函数与多样性惩罚，联合优化事实准确性、图表质量与叙事连贯性，并构建 MMRBench 基准验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有表格到多模态报告生成方法采用固定线性流水线和孤立子任务处理，难以联合优化事实准确性、视觉质量与叙事连贯性，缺乏对报告生成过程的全局搜索与优化。

Method: 将报告生成分解为章节规划、可视化任务识别、图表生成、洞察组织与叙事润色等原子动作，每个动作由 LLM 基于当前报告状态动态推理执行；在 MCTS 中存储逐步推理轨迹以实现上下文连贯构建；设计多维奖励函数联合评估数值事实一致性（通过 SQL）、图表质量、图表-文本对齐与结构完整性，并加入多样性惩罚与前置条件检查来剪枝无效动作。

Result: 在 MMRBench 基准上，MCTS-Report 在结构完整性、数值准确性、图表-文本对齐与洞察新颖性上显著优于强基线，总体得分达到 77.9。

Conclusion: MCTS 驱动的渐进式搜索框架能有效提升表格到多模态报告生成的联合质量，为数据智能中的专业报告自动化生成提供了新范式。

Classification reason: 论文关注从结构化表格数据自动生成包含文本分析与可视化图表的专业多模态报告，属于表格/图表理解与文档智能范畴，映射到 [CANONICAL] 子主题"视觉问答与文档理解"。

Importance reason: 表格到多模态报告生成与用户关注的核心方向（连续多模态、视频/图像生成、世界模型）关联较弱，且未涉及优先子主题关键词。 Signals: MCTS-driven generation, table-to-report, multimodal report, LLM reasoning, MMRBench benchmark
...
### [101] [Thinking with Anchors: Grounded and Efficient Document Reasoning](https://arxiv.org/abs/2608.04424)
...
TL;DR: 本文提出 ADOPD 2026，一个面向推理的文档理解基准扩展，将文本块、视觉实体、语义标签、边界框与多边形掩码统一为共享的视觉锚词汇表，支持区域级语义标注、统一视觉语言 grounding 与锚定推理，并引入 DocCount 密集计数基准揭示现有模型在文档语义理解上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有文档理解基准大多聚焦于定位页面元素，而真实文档智能需要模型联合推理区域语义、空间关系与视觉结构。作者希望将页面分解从单纯定位任务推进到空间锚定的文档理解。

Method: 在 ADOPD 2024 数据集基础上扩展，加入人工清洗的标题、语义标签和基于文档区域生成的思维链（CoT）轨迹。将文本块、视觉实体、语义标签、边界框和多边形掩码统一为共享的视觉锚词汇表，支持三种能力：区域级语义标注、统一视觉语言 grounding（生成文本区域与视觉实体及其坐标/多边形轮廓）、以及基于锚的推理。同时构建 DocCount 密集计数基准。

Result: 当前最先进的模型在 DocCount 密集计数任务上仍表现不佳，揭示了标准布局基准常掩盖的长尾语义失败，凸显了 Thinking-with-Anchors 流程在文档语义理解中的必要性。

Conclusion: ADOPD 2026 通过将页面分解与可验证的视觉锚推理相连接，提供了一个将文档理解从定位推进到锚定文档智能的任务框架。

Classification reason: 论文聚焦文档理解基准 ADOPD 2026，将页面分解转化为空间锚定的文档理解，涉及区域语义标注、视觉语言 grounding 与文档推理，属于文档理解与视觉问答方向，映射到 [CANONICAL] 子主题 multimodal_vqa_doc。

Importance reason: Document layout grounding benchmark is only tangentially related to your core focus on continuous multimodal models and video generation. Signals: document understanding benchmark, visual anchor grounding, region-level semantic tagging, dense counting, not aligned with priority subtopics
...
### [102] [EgoAfford: Task-Oriented Affordance Grounding via Egocentric Referring Segmentation](https://arxiv.org/abs/2608.04533)
...
TL;DR: EgoAfford 是一个面向第一视角桌面任务的功能区域定位基准，包含约 15.5k 人工验证图像与 EgoAfford-Real 真实数据；配套提出 EgoLens，一个 3B 多模态大语言模型，配备角色特定掩码解码器，用于联合完成下一步规划推理与动作角色条件的功能区域分割。


<details>
  <summary>Details</summary>
Motivation: 现有部分级功能区域定位（affordance grounding）仅支持基本动作，难以扩展到复杂多步任务。作者希望将对象语义角色、任务状态对齐的视觉观察与多步规划三者联系起来，构建一个能同时进行规划推理与功能区域分割的基准与模型。

Method: 构建 EgoAfford 基准：约 15.5k 人工验证图像来自 2000 个生成的多步场景，组织为语义对齐、任务完整的图像序列；另含 EgoAfford-Real 102 张人工拍摄图像覆盖 26 个任务。提出 EgoLens，一个 3B 多模态大语言模型，配备角色特定掩码解码器，用于下一步动作的直接对象、工具和目的地三个组件的功能区域分割。

Result: 对近期参照分割 MLLM、商业 VLM-SAM2 流水线及 EgoLens 的评估显示，下一步推理与动作角色条件部分定位存在互补性挑战；EgoLens 在生成图像与人工拍摄图像上均建立了强参考性能。

Conclusion: EgoAfford 与 EgoLens 为多步桌面任务中感知与规划的联合研究提供了基础，揭示了任务导向功能区域定位中规划推理与角色条件分割的互补挑战。

Classification reason: 论文提出 EgoAfford 基准与 EgoLens 多模态大模型，用于第一视角参照分割与任务导向的功能区域定位，属于视觉问答/理解类任务，映射到 [CANONICAL] 多模态视觉问答与文档理解子主题。

Importance reason: This paper focuses on egocentric affordance grounding and referring segmentation, which is tangential to your core interests in continuous multimodal models, autoregressive generation, and world models. Signals: egocentric affordance, referring segmentation, multimodal LLM, task planning, tabletop tasks
...
### [103] [Consistency-Driven Co-Evolution for Self-Supervised Cross-Representation Learning](https://arxiv.org/abs/2608.04926)
...
TL;DR: CoCoEvolve 提出一种自监督的跨表征一致性学习方法，通过定义图表、表格与代码之间的一对一对应关系，利用表征间一致性作为优化信号，无需额外标注即可提升跨表征理解能力，并配套提供覆盖六类跨表征任务的评测套件。


<details>
  <summary>Details</summary>
Motivation: 图表图像、表格数据和可视化代码在多个领域日益重要，但跨表征理解面临三大挑战：表征间关系本质是一对多、监督信号模糊且昂贵、模型优化缺乏既方向自适应又表征可泛化的原则性信号。

Method: CoCoEvolve 将跨表征映射从一对多问题转化为显式的一对一对应关系，利用表征间一致性作为优化目标，无需额外标注。训练阶段（CoCoEvolve@Train）在图表-表格-代码循环中进行协同演化；推理阶段（CoCoEvolve@Test）应用相同的一致性目标进行测试时协同优化；并构建 CoCoEvolve@Eval 评测套件覆盖全部六类跨表征任务。

Result: 在四个基准上，CoCoEvolve 在训练时和测试时两种设置下均提升了跨表征理解性能。

Conclusion: 通过将跨表征映射定义为显式一对一对应并利用一致性作为自监督信号，CoCoEvolve 有效缓解了监督模糊与标注成本问题，为图表、表格与代码的跨表征理解提供了可泛化的优化框架。

Classification reason: 论文关注图表图像、表格数据与可视化代码之间的跨表征理解与一致性学习，属于文档/图表理解范畴，映射到 [CANONICAL] 子主题"视觉问答与文档理解"。

Importance reason: Focuses on chart/table/code cross-representation learning, which is outside the user's core continuous multimodal, video generation, and world model priorities. Signals: self-supervised cross-representation learning, chart-table-code co-evolution, test-time optimization, evaluation suite, no direct overlap with priority subtopics
...
### [104] [Visual Anchoring in Diffusion: Multimodal Zero-Shot Skeleton Action Recognition](https://arxiv.org/abs/2608.04623)
...
TL;DR: 提出 TDSM-MM，一种基于"分类即生成"范式的多模态零样本骨架动作识别方法。通过文本条件去噪 Transformer 预测骨架特征上的噪声来给每个类别打分，并引入非扩散的 RGB 条件 token 作为稳定视觉锚点，避免传统多模态方法中独立评分分支的融合权重选择问题。


<details>
  <summary>Details</summary>
Motivation: 零样本骨架动作识别（ZSAR）在未见动作共享相似骨架关节动态但物体或场景上下文不同时存在歧义。RGB 提供了缺失线索，但现有多模态方法通常维护独立的骨架和 RGB 评分分支并融合输出，固定融合权重无法捕捉类别对相关的模态可靠性，自适应规则又缺乏目标侧反馈来决定哪个分支应主导。

Method: 采用 classify-by-generation 范式，每个类别通过文本条件去噪器预测添加到骨架特征上的噪声的准确度来打分。该公式将逐步损坏的骨架与固定条件分离，使 RGB 和文本联合条件化单一类别评分函数而非产生独立分数。具体实现为 Multimodal Triplet Diffusion for Skeleton-Text Matching (TDSM-MM)，在文本条件去噪 Transformer 上增加非扩散的 RGB 条件 token 作为骨架数据重建期间的稳定视觉锚点。

Result: 在 NTU-60/120 四个划分中的三个上取得最佳归纳准确率，并在 NTU-120 96/24 上超越转导式 SOTA（71.3% vs 69.1%），且无需测试时自适应。

Conclusion: 扩散式方法可成为零样本学习的有前景方向，通过分类即生成范式规避了多模态融合权重选择问题，RGB 视觉锚点有效提升了骨架动作识别的零样本性能。

Classification reason: 论文研究零样本骨架动作识别（ZSAR），通过多模态（RGB+文本）条件扩散模型进行骨架-文本匹配分类。这属于多模态理解与生成方向下的视觉问答/识别类任务，映射到 [CANONICAL] multimodal_vqa_doc（视觉问答与文档理解）作为最接近的现有子主题。

Importance reason: Diffusion-based zero-shot skeleton action recognition is a niche application with limited overlap with the user's core focus on continuous multimodal models and video/world-model generation. Signals: diffusion-based classification, zero-shot action recognition, multimodal fusion, skeleton-RGB-text, classify-by-generation
...
### [105] [REZE: Recognition-Based Zero-Shot Extraction for Video Temporal Grounding](https://arxiv.org/abs/2608.04480)
...
TL;DR: REZE 提出一种无需训练的零样本视频时序定位方法，将视频切分为短片段，让 VLM 输出片段级置信度分数，再用确定性算法将分数曲线转换为任务输出，在 QVHighlights 上刷新了训练无关方法的 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有零样本视频时序定位方法直接让 VLM 生成起止时间戳，结果高度依赖模型设计与训练，不同 VLM 之间定位精度差异大。作者希望提出一种简单、训练无关且跨模型稳健的方法。

Method: REZE 将视频切分为短片段，对每个片段询问 VLM 关于查询的片段级置信度分数，然后在模型外部用确定性算法将分数曲线聚合为任务所需输出，支持单/多区间时刻检索与高亮检测等多种任务输出。

Result: 在 QVHighlights 上，REZE 将训练无关时刻检索 mAP 从 38.23 提升到 40.32，高亮检测达到 44.18 mAP 和 73.41 HIT@1，均为训练无关方法中的新 SOTA；HIT@1 甚至超过所有全监督 SOTA。在七个骨干模型上验证，Charades-STA 和 QVHighlights 上均优于直接时间戳生成。

Conclusion: 将时序聚合放在模型外部进行，使 REZE 能适配不同任务输出并跨模型稳健迁移；早期模型配合 REZE 可接近同家族更新模型的原生性能，说明该方法具有通用性与可移植性。

Classification reason: 论文研究视频时序定位（video temporal grounding），属于视频理解与时序推理范畴，映射到 [CANONICAL] 子主题 video_understanding_temporal。

Importance reason: 该论文聚焦视频时序定位的零样本方法，与用户优先关注的连续多模态模型、视频生成及世界模型方向关联较弱。 Signals: 零样本视频时序定位, 训练无关方法, VLM片段级置信度, QVHighlights SOTA, 视频理解而非生成
...
### [106] [MetaVideoAgent: Automated Video-Agent Evolution for Long-Form Video Understanding](https://arxiv.org/abs/2608.04587)
...
TL;DR: MetaVideoAgent 是一个自动演化视频智能体的框架，通过从稀疏采样帧中分析信息密度与证据需求来指导初始设计，将局部失败压缩为独立可执行的最小验证任务，构建证据锚定的 Gold Paths 并审计学生轨迹，将重复失败归因到负责模块，实现模块级更新。在 VA-EvoBench 八个视频分布上，四次演化迭代将宏平均准确率从 38.44% 提升到 51.47%，超越最强固定设计视频智能体 6.39 个百分点。


<details>
  <summary>Details</summary>
Motivation: 长视频理解需要定位稀疏且与问题相关的证据，但真实世界视频在模态信息密度、内容结构和证据模式上差异巨大，固定设计的视频智能体要么冗余处理要么因不匹配而失败。将自动智能体演化从文本扩展到视频面临全视频执行验证成本高、失败跨阶段传播、代码级更新难以可靠实现等挑战。

Method: MetaVideoAgent 包含四个核心组件：(1) 从稀疏采样帧和相关查询中刻画信息密度与证据需求以指导初始设计；(2) 将局部失败压缩为独立可执行的最小验证任务以降低验证成本；(3) 构建证据锚定的 Gold Paths，审计学生轨迹，聚合跨样本的重复失败并归因到负责模块；(4) 采用模块化智能体表征，将每次更新约束到主要负责模块及其必要依赖上。同时引入 VA-EvoBench 基准，覆盖八个视频分布并划分演化集与留出集。

Result: 在 VA-EvoBench 上，每个分布进行四次演化迭代后，MetaVideoAgent 提升了所有初始智能体，宏平均准确率从 38.44% 提升到 51.47%，平均演化成本为每分布 3.54M tokens。演化后的智能体比最强固定设计视频智能体高出 6.39 个百分点，同时使用最少的 tokens 和每问题视频帧数。

Conclusion: MetaVideoAgent 证明了将自动智能体演化扩展到视频领域的可行性，通过证据锚定的失败归因和模块化更新机制，能够针对不同视频分布自适应地演化出更高效、更准确的视频理解智能体，为长视频理解提供了新的自动化范式。

Classification reason: 论文聚焦长视频理解，通过自动演化视频智能体来提升长视频问答能力，属于视频理解与时序推理范畴，映射到 [CANONICAL] video_understanding_temporal 子主题。

Importance reason: Focuses on video-agent evolution for long-form understanding, but lacks direct connection to the user's priority topics of continuous multimodal models, autoregressive/diffusion video generation, or world models. Signals: video agent evolution, long-form video understanding, automated design, VA-EvoBench, modular updates
