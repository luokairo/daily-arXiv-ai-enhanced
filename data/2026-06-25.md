...
- [多模态生成](#direction-multimodal_generation) [Total: 1]
  - [3D 生成与重建](#direction-multimodal_generation-three_d_generation) [Total: 1]
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 4]
  - [多模态评测与基准](#direction-multimodal_understanding_generation-multimodal_benchmark) [Total: 1]
...
### [1] [Memory Retrieval in Visuomotor Policies for Long-Horizon Robot Control](https://arxiv.org/abs/2606.25136)
...
TL;DR: HALO 是一种面向长时域机器人控制的视觉运动策略，通过注意力机制从长达8分钟的历史经验中检索任务相关信息，并利用 VLM 蒸馏和稀疏注意力分别解决伪相关和记忆误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 通用机器人在部分可观测环境（如家庭）中需要记忆机制来支持自主性，但现有模仿学习方法存在两个关键挑战：(1) 策略可能学习到过去信息与预测动作之间的伪相关；(2) 闭环控制中预测误差累积导致模型漂移和级联失败。

Method: HALO 包含两个核心组件：(1) VLM 先验蒸馏——从演示轨迹生成与记忆相关的问答对，联合训练视频问答目标，引导策略检索任务相关信息，抑制伪相关；(2) 稀疏注意力机制——限制检索仅关注历史中最相关的部分，减少闭环控制中累积误差的影响。

Result: 在长时域机器人控制任务上，HALO 能够从长达8分钟的历史经验中可靠检索任务相关信息，相比基线方法显著提升了长时域任务的成功率，有效缓解了伪相关和记忆误差累积问题。

Conclusion: HALO 通过 VLM 蒸馏引导的注意力记忆检索和稀疏注意力机制，有效解决了长时域机器人控制中的伪相关和记忆误差累积两大挑战，为通用机器人在复杂环境中的自主操作提供了可行的记忆方案。

Classification reason: 该论文提出 HALO，一种基于注意力记忆检索的视觉运动策略，用于长时域机器人控制。这属于具身智能/机器人领域的世界模型与规划控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong world model and memory retrieval for long-horizon robot control, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: world model, long-horizon robot control, memory retrieval, VLM distillation, sparse attention
...
### [2] [Learning Perceptive Platform Adaptive Locomotion Controllers for Quadrupedal Robots](https://arxiv.org/abs/2606.25179)
...
TL;DR: 本文研究如何将感知信息整合到形态感知的强化学习架构中，以训练跨四足机器人形态的通用运动控制器。基于MorAL框架，训练了盲基线、仅评论家感知（MorAL+）和完全感知演员-评论家（PPAL）三种策略，在仿真和ANYmal硬件上评估，发现仅评论家感知策略在鲁棒性和跟踪一致性上优于盲基线，且比完全感知策略对感知噪声更稳定。


<details>
  <summary>Details</summary>
Motivation: 现有四足机器人运动控制器要么针对单一机器人训练，要么使用忽略实时感知的盲策略，导致跨形态泛化能力差。完全感知策略对噪声敏感，而盲控制器缺乏地形感知能力。因此需要研究如何将感知信息有效整合到形态感知的强化学习架构中，以实现可部署的跨形态四足机器人运动控制。

Method: 基于MorAL框架，训练三种变体策略：(1) 盲基线——不使用感知信息；(2) MorAL+——仅在评论家（critic）中使用感知信息；(3) PPAL（Perceptive Policy Actor-critic Locomotion）——在演员和评论家中均使用感知信息。使用自适应地形课程（adaptive terrain curricula）在多个参考四足机器人形态上训练，并在仿真（平坦和崎岖地形）及ANYmal硬件上评估。

Result: 仿真和硬件实验表明：仅评论家感知（MorAL+）相比盲基线提升了鲁棒性和跟踪一致性；完全感知策略（PPAL）在感知噪声下稳定性较差；感知信息的放置位置和课程设计是跨形态运动控制可扩展性的关键因素。

Conclusion: 感知信息在强化学习架构中的放置位置对四足机器人运动控制器的性能有显著影响。仅评论家感知策略在鲁棒性和稳定性之间取得了良好平衡，为跨形态可部署运动控制提供了实用方案。未来工作可进一步探索感知噪声鲁棒性和更复杂的感知融合方式。

Classification reason: 该论文研究四足机器人的感知自适应运动控制器，涉及具身智能中的运动规划与控制，属于世界模型方向下的具身世界模型与规划控制子主题。

Importance reason: Focuses on quadruped locomotion control, not aligned with user's priority on continuous multimodal models or video generation. Signals: Quadruped locomotion, morphology-aware control, perception integration, reinforcement learning, ANYmal hardware
...
### [3] [RAVEN: Long-Horizon Reasoning & Navigation with a Visuo-Spatio-Temporal Memory](https://arxiv.org/abs/2606.25206)
...
TL;DR: RAVEN 是一个面向长期机器人部署的智能记忆系统，通过将视觉嵌入与空间位姿和时间信息存储在向量数据库中，实现高效的语义、空间和时间检索，在长时域视频问答和自然语言导航任务上超越基于字幕的记忆系统，并以10倍更低的检索成本匹配前沿VLM性能。


<details>
  <summary>Details</summary>
Motivation: 长期机器人部署需要紧凑且可扩展的记忆系统，能够保留细粒度视觉语义、在空间和时间上定位观测，并支持高效存储与检索。现有基于图像到文本字幕的记忆方法存在信息丢失问题，而直接存储原始图像则效率低下。

Method: RAVEN 将视觉嵌入（由预训练视觉编码器提取）与位姿（pose）和时间戳一起存储在向量数据库中，并在空间地图上对检索进行空间约束。系统通过空间-时间-语义联合检索来回答自然语言查询，并支持基于自然语言目标的长期导航任务。

Result: 在多个模拟和真实世界视频问答基准上，RAVEN 一致超越基于字幕的记忆系统，并在长时域任务上以10倍更低的检索成本匹配前沿VLM的性能。在Unitree Go1机器人上的真实环境长期导航部署也取得了成功。

Conclusion: RAVEN 证明了直接基于视觉嵌入的记忆系统在长期机器人部署中的有效性，避免了字幕方法的信息损失，同时实现了高效的空间-时间-语义联合检索，为具身智能体的长期记忆与导航提供了实用方案。

Classification reason: 该论文提出RAVEN，一个用于长期机器人问答与导航的智能记忆系统，将视觉嵌入与位姿和时间信息存储在向量数据库中，并在空间地图上进行检索以回答查询和导航到目标。这属于具身智能中的机器人导航与记忆系统，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Strong world model and embodied memory work, but lacks direct connection to continuous language/multimodal models or autoregressive generation. Signals: world model, long-horizon navigation, visuo-spatial memory, efficient retrieval, embodied AI
...
### [4] [RigPI: Dynamic Parameter Identification of Rigid Body via VLM-Seeded Differentiable Simulation](https://arxiv.org/abs/2606.25212)
...
TL;DR: RigPI提出一个系统框架，利用VLM提供语义先验初始化，结合可微物理仿真器进行梯度优化，实现机器人操作中刚体（含旋转关节和滑动关节）动力学参数（惯性、摩擦等）的准确辨识，并在真实机器人上验证了参数预测的有效性。


<details>
  <summary>Details</summary>
Motivation: 机器人操作中物体的物理参数（惯性、摩擦等）准确辨识是实现高级操作和数字孪生的基础，但现有方法受限于传感噪声、建模误差和缺乏先验知识，难以从真实交互中获取物理一致的参数。

Method: RigPI将VLM（视觉语言模型）用于参数初始化和约束搜索空间，结合力-力矩传感器测量和运动观测，在可微物理仿真器中进行梯度优化的两阶段参数精炼策略，以缓解噪声影响并避免物理不合理解。

Result: 在真实世界实验中，对具有旋转关节和滑动关节的物体实现了准确且稳定的参数估计，并成功在真实机器人上复现了操作轨迹，验证了参数感知的预测有效性。

Conclusion: RigPI证明了VLM先验与可微物理仿真相结合的框架在真实机器人系统辨识任务中的有效性和鲁棒性，为机器人操作中的物理参数辨识提供了系统化解决方案。

Classification reason: 该论文研究机器人操作中刚体动力学参数辨识，利用VLM提供先验初始化，结合可微物理仿真器进行参数优化，属于机器人/具身智能中的系统辨识与物理参数估计，与[具身世界模型与规划控制]方向高度相关。

Importance reason: Strong world model and VLM-seeded differentiable simulation for robotics, but outside core priority subtopics. Signals: world model, VLM initialization, differentiable simulation, rigid body dynamics, real-world validation
...
### [5] [Decoupling Semantics and Geometric Grounding: Spatial Visual Prompts for Language-Conditioned Imitation Learning](https://arxiv.org/abs/2606.25360)
...
TL;DR: 本文提出SVP-IL，一种解耦的VLA架构，通过视觉语言基础模型将语言指令解析为零样本几何掩码（空间视觉提示），并将其注入连续动作生成器，实现语义推理与空间控制的解耦，在少量演示数据下显著提升语言条件模仿学习的成功率。


<details>
  <summary>Details</summary>
Motivation: 端到端VLA模型将语义推理与空间控制耦合在一起，导致在数据受限的模仿学习中难以精确区分目标物体，存在严重的对齐瓶颈。

Method: SVP-IL采用解耦架构：1）利用视觉语言基础模型将语言指令解析为零样本几何掩码（Spatial Visual Prompts, SVP）；2）通过轻量级直接特征级融合机制将SVP注入连续动作生成器，提供显式且无污染的空间梯度引导。

Result: 在50-100条演示数据下，SVP-IL将高歧义语言条件任务的平均成功率从24.0%提升至39.5%，在标准基准上达到67.8%。真实机器人实验进一步验证了其在非结构化物理环境中的鲁棒性和数据效率。

Conclusion: 解耦语义推理与空间控制是提升数据受限条件下语言条件模仿学习性能的有效策略，SVP-IL通过显式空间视觉提示实现了显著的性能提升。

Classification reason: 该论文提出SVP-IL，一种解耦的视觉-语言-动作（VLA）架构，用于机器人操作中的语言条件模仿学习。论文核心关注具身智能中的动作生成与空间控制，属于世界模型方向下的具身世界模型与规划控制（embodied_world_model）这一[CANONICAL]子主题。

Importance reason: Focuses on robotic imitation learning, not directly aligned with your priority subtopics like continuous language models or video generation. Signals: Decoupled VLA architecture, spatial visual prompts, data-efficient imitation learning, zero-shot geometric grounding, low-data regime improvement
...
### [6] [Reliability-Asymmetric Spacecraft Autonomy: Co-Designing a Capable Learned GNC Stack with a Verified, Adaptation-Aware Runtime Shield](https://arxiv.org/abs/2606.25366)
*Alireza Shojaei*
...
TL;DR: 本文提出 AMPLE-GNC，一个面向深空探测的三层制导导航控制栈，结合小基础模型指挥官（自然语言→PDDL+）、故障自适应控制器（Rapid Motor Adaptation 在线推断执行器故障）和运行时盾牌（LTL不变式+Kind 2验证）。在6-DOF Basilisk测试平台上，指挥官通过语法约束解码实现84%可执行动作；故障自适应控制器恢复97.8%的符号故障和94.4%的连续增益故障；运行时盾牌通过分裂共形恢复截止证书实现94.5%的自主率同时保证安全。


<details>
  <summary>Details</summary>
Motivation: 深空探测任务需要同时具备高能力和可认证性的星上自主系统。基于规则的自主系统可认证但脆弱，而基于学习的自主系统能力强但难以验证。现有方法无法同时满足能力与可认证性需求。

Method: 提出 AMPLE-GNC 三层架构：(1) 能力路径：微调360M预训练模型+语法约束解码生成PDDL+规划，经约束筛选验证器过滤；(2) 故障自适应控制器：Rapid Motor Adaptation 在线推断潜在执行器故障并自适应调整控制；(3) 运行时盾牌：9个线性时序逻辑不变式，由Kind 2模型检查器机器验证预测器正确性，采用分裂共形预测生成恢复截止证书，自适应地决定何时触发安全保持模式。

Result: 指挥官：语法约束解码实现硬输出有效性保证，84%规划器可执行动作；去泄露测试中新颖措辞泛化38%精确匹配、51%动作匹配，经措辞多样性再微调提升至48%。故障自适应控制器：恢复97.8%执行器符号故障和94.4%连续增益故障（训练随机化范围内），故障无关PD和端到端RL均为0%，最强经典自适应基线仅55%；超出分布范围时分裂共形重训练达57-67%。运行时盾牌：自适应感知的接合策略使控制器保持94.5%自主率同时捕获非恢复情况。

Conclusion: 本文展示了可靠性不对称的航天器自主系统设计：通过能力路径与运行时盾牌的协同设计，可以在保持高自主能力的同时实现可认证的安全性。关键发现包括：语法约束解码有效分离语法有效性与语义准确性；随机化广度而非数据量驱动故障适应泛化；自适应感知的盾牌接合策略能平衡安全与自主性。

Classification reason: 该论文提出 AMPLE-GNC，一个面向深空探测的三层制导、导航与控制（GNC）栈，包含基础模型指挥官（自然语言到PDDL+规划）、故障自适应控制器（Rapid Motor Adaptation）和运行时盾牌（LTL不变式+Kind 2验证）。核心关注具身智能体（航天器）在不确定环境中的自主规划、故障适应与安全控制，属于具身世界模型与规划控制范畴，因此映射到 canonical 子主题 "具身世界模型与规划控制"。

Importance reason: Strong in world model and verification but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: spacecraft autonomy, runtime shield, fault-adaptive control, LTL verification, small foundation model
...
### [7] [MAPL: Multi-Objective Preference Learning for Robot Locomotion](https://arxiv.org/abs/2606.25398)
...
TL;DR: MAPL 提出一种多目标偏好学习框架，利用 LLM 生成沿语义维度的轨迹偏好，训练多头偏好评分模型以替代人工设计的奖励函数，在四足机器人 locomotion 任务上达到与专家奖励相当的性能。


<details>
  <summary>Details</summary>
Motivation: 机器人 locomotion 中奖励函数设计是强化学习的瓶颈，现有基于 LLM 的偏好学习方法仅提供单一整体判断，难以捕捉高质量运动所需的多个竞争性目标。

Method: MAPL 使用 LLM 沿多个语义有意义的准则（如步态效率、稳定性等）独立比较轨迹对，生成目标级偏好；训练多头偏好评分模型，每个头对应一个目标；将多头输出聚合为标量奖励用于策略优化。

Result: 在四个四足机器人 locomotion 环境中，仅使用 LLM 生成的偏好训练的策略性能达到或超过专家设计奖励的策略，消除了任务特定的奖励工程。

Conclusion: MAPL 证明了利用 LLM 进行多目标偏好学习可以有效替代手工奖励设计，为机器人 locomotion 提供了一种无需领域专家知识的高效奖励学习范式。

Classification reason: 该论文研究四足机器人 locomotion 的偏好学习奖励设计，属于机器人具身智能中的策略学习与奖励建模，与 [CANONICAL] 具身世界模型与规划控制 方向高度匹配。

Importance reason: Uses LLM for reward design in robotics but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: LLM-based preference learning, multi-objective reward design, quadruped locomotion, no continuous token or video generation focus, weak alignment with priority subtopics
...
### [8] [HEART: Coordination of Heterogeneous Expert Agents for Physically Grounded Robotic Task Planning](https://arxiv.org/abs/2606.25404)
...
TL;DR: 提出HEART框架，通过异构多LLM专家代理协作，将复杂指令分解为原子推理任务并分配给角色专业化代理，在token预算约束下进行物理可行的机器人任务规划，在家庭场景基准上优于单LLM和基于规则的规划器。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的规划器直接将文本翻译为动作序列，缺乏对物理可行性、可达性和逻辑顺序的结构化推理，导致生成无效或不完整的规划。需要一种能够处理物理和空间约束的鲁棒规划方法。

Method: 提出HEART框架，采用异构多LLM架构：将指令分解为原子推理任务，分配给角色专业化的专家代理（如能力检查、可达性分析、约束验证等），在token预算约束下进行角色导向推理，然后通过约束驱动的计划合成生成物理可执行的动作序列。

Result: 在多个家庭场景基准测试中，HEART相比单LLM规划器和基于规则的规划器持续提升了规划成功率，证明了异构LLM协作在资源约束下实现鲁棒且可扩展的机器人任务规划的有效性。

Conclusion: 异构多LLM协作框架能够有效提升机器人任务规划的物理可行性和鲁棒性，为在真实世界计算和通信约束下部署可扩展的机器人规划系统提供了可行方案。

Classification reason: 该论文提出基于异构多LLM协作的机器人任务规划框架，关注物理可行性、可达性等约束条件下的规划与控制，属于具身智能/机器人领域的任务规划，与[具身世界模型与规划控制]这一规范子主题高度匹配。

Importance reason: Focuses on robotic task planning with LLMs, not directly on the user's priority subtopics like continuous language models or video generation. Signals: Heterogeneous multi-LLM, robotic task planning, token budget, household benchmarks, physically grounded
...
### [9] [SAGE-Nav: Leveraging LLM Planning and Alignment Fusion for Hierarchical Scene Graph-Guided Navigation](https://arxiv.org/abs/2606.25497)
...
TL;DR: SAGE-Nav提出一种分层导航框架，利用LLM进行全局语义规划，结合层次化场景图编码器（HSGE）和目标感知对齐融合网络（GAFN），在i-THOR和RoboTHOR环境中实现SOTA的物体目标导航性能与零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有单块式方法在长程推理和陌生环境泛化方面表现不佳，亟需一种能够解耦全局语义规划与高频反应控制的分层导航框架。

Method: 提出SAGE-Nav分层框架：(1) LLM作为全局规划器，将抽象指令分解为语义路标序列；(2) 层次化场景图编码器(HSGE)利用关系图卷积生成结构感知嵌入；(3) 目标感知对齐融合网络(GAFN)通过自适应门控机制动态融合实时感知与结构先验，为底层策略提供鲁棒的视觉-拓扑对齐。

Result: 在i-THOR和RoboTHOR环境中达到SOTA性能，在导航效率和零样本泛化方面取得显著提升，同时保持物理机器人部署所需的低控制延迟。

Conclusion: SAGE-Nav通过LLM规划与场景图对齐融合的有效结合，验证了分层架构在物体目标导航任务中的优越性，为具身智能体的长程导航提供了可行方案。

Classification reason: 该论文提出面向具身导航的分层框架，结合LLM全局规划与场景图引导的低层控制，属于具身智能中的导航与规划控制任务，与[具身世界模型与规划控制]高度匹配。

Importance reason: Strong in embodied world models and LLM planning but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: LLM planning for navigation, hierarchical scene graph, zero-shot generalization, object-goal navigation, alignment fusion network
...
### [10] [ASSCG: Just-Right Gating over Chattering for Fast-Slow LLM Planning in Autonomous Driving](https://arxiv.org/abs/2606.25509)
...
TL;DR: 提出ASSCG自适应门控机制，通过RWKV骨干网络和GRPO强化学习训练，在自动驾驶快慢双系统规划中智能决策何时调用慢速LLM，在nuPlan和NAVSIM上分别提升性能并显著降低延迟。


<details>
  <summary>Details</summary>
Motivation: LLM能提升自动驾驶规划能力，但在线查询成本高；现有快慢规划器依赖手工设计的触发规则，要么过度调用慢系统，要么在错误时机调用，导致效率与性能难以平衡。

Method: 将慢系统调用建模为资源感知的序列决策问题，提出ASSCG（自适应慢系统控制门），在帧级别做出Query/Cache/Drop三种决策。使用RWKV骨干网络进行高效长时序门控，先通过监督微调训练，再使用GRPO风格的计算感知强化微调优化。

Result: 在nuPlan Hard20闭环评测中，ASSCG将AsyncDriver得分提升至67.28（+2.28），端到端推理延迟降低60%；在NAVSIM上，基于RecogDrive的双系统达到91.4 PDMS（+0.6），平均速度提升25%。

Conclusion: ASSCG通过数据驱动的自适应门控替代手工规则，在保持甚至提升自动驾驶规划性能的同时大幅降低计算开销，为LLM在自动驾驶中的高效部署提供了可行方案。

Classification reason: 论文研究自动驾驶规划中LLM的快速-慢速推理门控机制，属于具身智能中的规划控制问题，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on autonomous driving planning with LLM gating, which is peripheral to your core interests in continuous multimodal models and video generation. Signals: Autonomous driving planning, Fast-slow LLM systems, GRPO reinforcement learning, nuPlan benchmark, Latency reduction
...
### [11] [One Body, Two Minds: Variable Autonomy Approach for a Co-embodied Robotic Hand](https://arxiv.org/abs/2606.25575)
...
TL;DR: 本文提出了一种"一个身体，两个心智"的共具身可变自主性机器人手系统。人类和机器人共享同一物理身体，在不同任务阶段以不同自主级别协作：机器人通过示教学习的扩散策略自主抓取已知物体，人类通过头部手势控制工具操作，人类始终保留否决权。44人用户研究表明任务完成时间提升23.3%，成功率最高达93.6%。


<details>
  <summary>Details</summary>
Motivation: 辅助机器人系统面临根本性权衡：完全自主的系统剥夺用户自主权，而完全由用户控制的系统需要持续的认知努力。现有的共享自主方法虽然混合了人类和机器人指令，但大多部署在分离的物理身体中。作者希望实现人类与机器人共享同一物理身体的协作范式。

Method: 提出共具身（co-embodiment）与可变自主性（variable autonomy）框架：人类和机器人共享一个可穿戴机器人手。采用基于示教学习的视觉运动扩散策略（visuomotor diffusion policy）实现自主抓取；抓取完成后系统通知用户，用户通过免手持的头部手势控制工具操作（如钻头、喷雾瓶等）。用户始终可通过释放手势否决机器人动作。

Result: 44名参与者完成5项双手任务：任务完成时间在试验间提升23.3%（p<0.001, Cohen's d=0.94），最佳策略变体达到93.6%任务成功率，用户接受度评分高（总体印象5.70/7，日常使用意愿5.52/7）。

Conclusion: 共具身与可变自主性是一种可行的辅助机器人方法，实现了"一个身体，两个心智"的人机协作范式，在保持物理耦合的同时允许从完全人类控制到完全自主动作的可变自主级别。

Classification reason: 该论文提出了一种共具身可变自主性机器人手，结合了示教学习的视觉运动扩散策略用于自主抓取，以及人类通过头部手势控制工具操作。这属于具身智能、机器人控制与人类-机器人协作的范畴，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on robotic hand co-embodiment and variable autonomy, not aligned with user's priority on continuous multimodal models or video/image generation. Signals: Robotics, shared autonomy, diffusion policy, user study, not multimodal generation or world model for video
...
### [12] [WOLF-VLA: Whole-Body Humanoid Optimal Locomotion Framework for Vision-Language-Action Learning](https://arxiv.org/abs/2606.25591)
...
TL;DR: WOLF-VLA 提出一个统一框架，将全身最优控制运动合成与大规模多模态数据集结合，训练视觉-语言-动作（VLA）模型直接从自然语言指令生成人形机器人运动策略，在六个运动任务族上展示了强推理能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人操作任务中展现了强大的泛化能力，但在全身、接触丰富的人形机器人运动领域尚未被充分探索，主要受限于数据稀缺、缺乏动态一致的演示以及难以在基于学习的流程中编码最优性和安全性。

Method: 构建了一个包含六类运动任务族的动态可行人形机器人轨迹数据集，每类任务通过环境变化、物体颜色、位置和视觉干扰物参数化。使用收集的关节轨迹、自我中心视觉观察和自然语言指令训练VLA模型，生成运动策略。

Result: 训练得到的策略在多个任务和环境设置中展现出强推理能力、对初始条件变化的鲁棒性以及有竞争力的性能。消融实验证明了每个模态对模型性能的影响。

Conclusion: WOLF-VLA 建立了第一个可复现的动态一致的人形机器人全身运动VLA控制基准，为指令驱动的运动策略的可扩展迁移研究提供了基础。数据集、模型和仿真套件将开源发布。

Classification reason: 该论文研究人形机器人全身运动控制，将最优控制轨迹合成与VLA模型结合，从自然语言指令生成运动策略，属于具身智能与机器人控制范畴，因此映射到[CANONICAL] subtopic "具身世界模型与规划控制"。

Importance reason: Focuses on humanoid locomotion VLA, which is outside the user's core priority subtopics of continuous language/multimodal models and autoregressive generation. Signals: Humanoid locomotion VLA, Whole-body optimal control, Multi-modal dataset, Not in priority subtopics, No continuous token or autoregressive generation
...
### [13] [Event-Adaptive Motion Planning with Distilled Vision-Language Model in Safety-Critical Situations](https://arxiv.org/abs/2606.25629)
...
TL;DR: 提出EAMP框架，通过可配置语义事件触发器（PC-SET）选择性激活VLM干预，结合蒸馏的SemNav-VLM进行策略级决策，以及语义模型预测控制（SMPC）将高层推理转化为底层控制优化，在安全关键物流场景中实现高效、实时的机器人导航。


<details>
  <summary>Details</summary>
Motivation: 安全关键场景中机器人导航面临动态智能体不可预测行为导致的碰撞风险。虽然VLM具备常识推理能力，但频繁调用会引入严重计算延迟，破坏物理执行的稳定性。现有方法难以兼顾高层语义推理与实时底层控制。

Method: 提出EAMP框架，包含三个核心模块：(1) 可配置语义事件触发器（PC-SET），通过持续监控短时序片段中的行为异常，选择性激活语义干预；(2) 事件触发的蒸馏SemNav-VLM，通过物理验证的语义蒸馏微调，将检测到的异常映射为离散策略级决策；(3) 语义模型预测控制（SMPC），将策略转化为优化目标和几何参考的动态重构。

Result: 在安全关键物流场景中的实验表明，EAMP有效对齐高层推理与底层控制，相比现有基线显著提升了动态安全裕度，同时保持了实时效率。

Conclusion: EAMP通过事件触发机制和蒸馏VLM，在安全关键导航中实现了高层语义推理与底层实时控制的平衡，为VLM在机器人领域的实际部署提供了有效方案。

Classification reason: 该论文研究安全关键场景下基于VLM的机器人导航与运动规划，属于具身智能中的规划控制问题，与[CANONICAL]具身世界模型与规划控制高度匹配。

Importance reason: Applies VLM to robotics but focuses on motion planning, not core continuous multimodal or world model generation. Signals: VLM for robot navigation, safety-critical motion planning, event-triggered VLM intervention, distilled VLM for efficiency, semantic model predictive control
...
TL;DR: DeformGen 提出一种基于动力学的数据增强框架，通过局部物理扰动+前向模拟生成拓扑多样且物理合理的可变形物体状态，并利用变形场扭曲将操作轨迹迁移到新状态，从而提升可变形物体操作策略学习的性能。


<details>
  <summary>Details</summary>
Motivation: 现有演示数据增强方法在可变形物体操作中存在两个根本挑战：(1) 可变形物体状态空间高维且受物理约束，低维位姿扰动无法生成有效的新状态；(2) 轨迹迁移非等变性，变形后物质点不再刚性运动。因此需要一种能生成拓扑多样且物理合理的可变形状态及其对应操作轨迹的增强方法。

Method: DeformGen 包含两个核心模块：(1) 状态增强——对可变形物体施加局部物理扰动（如局部力、重力方向变化），通过前向动力学模拟得到拓扑一致且物理合理的变形状态；(2) 轨迹增强——利用变形场（deformation field warping）将源操作轨迹从原始状态映射到新变形状态，将每个粒子的位移提升为连续空间函数，使末端执行器轨迹与变形几何保持一致。

Result: 在高保真可变形物体操作基准上，DeformGen 相比仅使用原始演示数据训练以及使用刚性风格增强基线，在策略学习上取得了普遍提升。

Conclusion: DeformGen 通过动力学驱动的状态增强和变形场轨迹迁移，有效解决了可变形物体操作中数据增强的拓扑多样性和轨迹非等变性问题，为可变形操作策略学习提供了一种高效的数据扩充方案。

Classification reason: 该论文关注可变形物体操作策略学习中的数据增强，核心是利用动力学前向模拟生成物理合理的变形状态，并通过变形场扭曲将操作轨迹迁移到新状态上。这属于具身智能中面向机器人操作的世界模型与策略学习范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on deformable manipulation policy learning, which is not a priority subtopic and lacks direct connection to user's core interests in continuous multimodal models or video generation. Signals: Deformable manipulation, Data augmentation, Policy learning, Dynamics-based, Robotics
...
TL;DR: 提出 Action ControlNet (ACNet)，一种轻量级延迟感知适配器，通过将已执行的动作后缀作为残差条件输入到冻结的动作头中，解决 VLA 模型异步执行时的动作不连续和抖动问题，无需重新训练完整策略。


<details>
  <summary>Details</summary>
Motivation: VLA 模型在机器人操作中展现出强大潜力，但其推理延迟阻碍了稳定的高频控制。异步执行虽能缓解延迟问题，但下一动作块仍基于过时观测预测，导致拼接不连续、动作抖动及接触式操作失败。现有方案需要完整策略重训练或特定架构运行时逻辑，缺乏轻量通用解决方案。

Method: 提出 Action ControlNet (ACNet)，一种轻量级延迟感知适配器。ACNet 保持预训练骨干网络不变，仅引入少量可训练参数，将已执行的动作后缀作为残差条件输入到大部分冻结的动作头中。该方法兼容扩散和流匹配等生成式动作头，通过条件化已执行动作来平滑异步轨迹拼接。

Result: 在 Kinetix、Meta-World MT50 和真实 SO-ARM101 平台上，ACNet 在推理延迟下提升了鲁棒性，生成了比直接拼接更平滑的异步轨迹，同时比完整的延迟条件重训练更轻量。

Conclusion: ACNet 提供了一种轻量、通用且无需重训练完整策略的解决方案，有效缓解了 VLA 模型异步执行中的动作不连续问题，为高频机器人控制提供了实用方案。

Classification reason: 该论文提出 Action ControlNet，一种轻量级延迟感知适配器，用于视觉-语言-动作（VLA）模型在机器人操作中的异步控制，解决推理延迟导致的动作不连续问题。这属于具身智能中的机器人规划与控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on robotic control latency, not core priority subtopics like continuous language models or video generation. Signals: Robotics, VLA model, Asynchronous control, Lightweight adapter, Not in priority subtopics
...
TL;DR: FORCE 是一个三阶段 VLA 强化学习微调框架，通过值校准预热和自蒸馏过滤，在不依赖人工干预的情况下显著提升机器人操作任务的成功率和训练效率。


<details>
  <summary>Details</summary>
Motivation: VLA 模型受限于次优数据的模仿天花板，RL 微调虽能突破但样本效率极低，主要源于 Q 函数不稳定导致的灾难性遗忘和低质量探索数据导致的低效策略更新，且常需昂贵的人工干预。

Method: FORCE 包含三个阶段：(1) 值校准预热阶段，利用在线 rollout 缓解 Q 函数的分布偏移；(2) 在线阶段，用校准后的 Q 函数过滤策略自身的动作提案和专家数据，仅保留高价值动作用于策略更新；(3) 自蒸馏阶段进一步稳定训练。

Result: 在多种仿真和真实机器人任务上，FORCE 实现了 79% 的绝对成功率提升，比先前 RL 方法高出 10%，训练速度加快 32.5%，且无需人工干预即可避免常见的成功率下降问题。

Conclusion: FORCE 通过值校准预热和自蒸馏有效解决了 VLA 模型 RL 微调中的样本效率问题，为部署自主机器人智能体迈出了重要一步。

Classification reason: 该论文提出 FORCE 框架，通过强化学习微调 VLA（视觉-语言-动作）模型以提升机器人操作性能，属于具身智能中动作规划与控制的范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong RL fine-tuning method for VLA models, but only tangentially touches priority subtopics like world models and efficient inference. Signals: VLA reinforcement fine-tuning, value-calibrated warm-up, self-distillation, sample efficiency, robotic manipulation
...
TL;DR: G2DP提出一种基于扩散模型的自动驾驶规划器，通过构建可微的时空代价体积（融合未来占据概率与路线进度图）在去噪过程中注入密集梯度引导，实现安全且高效的轨迹生成，在nuPlan等基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散规划器在密集交互交通中能有效建模多样驾驶行为，但其随机性导致在闭环执行中需要显式引导来确保安全性和路线遵循。现有引导方法依赖稀疏的实体几何查询或事后精炼，情境感知能力有限，在交互场景中表现脆弱。

Method: G2DP构建一个可微的时空代价体积，融合概率性未来占据分布与路线进度图，将其形式化为连续安全能量泛函，在扩散去噪循环中直接注入密集梯度，主动引导轨迹生成朝向无碰撞且进度最优的区域。

Result: 在nuPlan上达到SOTA，反应分数超过最强模仿学习基线+7.2分；零样本迁移到interPlan和DeepScenario基准仍保持最高分，在interPlan上碰撞避免比无引导方法提升+10.15。

Conclusion: 时空代价网格可作为扩散规划中鲁棒引导的有效表征，G2DP通过密集约束注入显著提升了扩散规划器在交互场景中的安全性和性能。

Classification reason: 该论文提出基于扩散模型的自动驾驶运动规划器G2DP，利用时空代价网格在去噪过程中引导轨迹生成，属于具身智能中的规划控制任务，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on autonomous driving planning, not aligned with user's priority subtopics on continuous language/multimodal models or video generation. Signals: diffusion planner, spatio-temporal cost volume, autonomous driving, nuPlan benchmark, world model application
...
Importance: 高优先级 (92.93) - Top 1 精读候选

TL;DR: ICWM提出了一种上下文世界建模框架，让机器人策略通过短时自生成任务无关交互序列隐式推断系统配置（如相机视角、机器人形态），无需参数更新即可适应新环境，在仿真和真实机器人上显著优于标准VLA基线。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型通常仅以当前观测和语言指令为条件，忽略了底层系统配置（如相机视角、机器人形态）这一变量，导致在新环境中需要大量数据微调才能泛化。作者希望将系统辨识视为上下文适应问题，使模型能自主推断系统变量并适应新配置。

Method: ICWM框架包含两个阶段：1) 任务无关的交互阶段——机器人执行一组短时自生成的动作序列并记录观测历史；2) 任务执行阶段——将交互历史作为上下文输入VLA模型，使模型隐式捕获当前系统的世界动力学。该方法利用上下文窗口理解系统如何运作（而非传统ICL中理解要执行什么任务），无需参数更新即可适应新配置。

Result: 在仿真和真实机器人平台上，ICWM在新相机视角等未见配置下显著优于标准VLA基线方法，展示了强大的零样本泛化能力。

Conclusion: ICWM通过将系统辨识转化为上下文学习问题，有效提升了VLA模型对未知环境配置的泛化能力，为机器人策略的快速适应提供了一种无需微调的有效范式。

Classification reason: 该论文提出ICWM框架，使机器人策略通过短历史交互隐式推断系统变量并适应新配置（如相机视角变化），属于面向机器人控制的世界模型方法，与[具身世界模型与规划控制]高度匹配。

Importance reason: Novel in-context world modeling for robotic control aligns with world model and efficient inference priorities, bypassing parameter updates for system adaptation. Signals: world model, in-context adaptation, robotic control, system identification, zero-shot generalization
...
### [19] [RoboAtlas: Contextual Active SLAM](https://arxiv.org/abs/2606.26046)
...
TL;DR: RoboAtlas 是一个上下文主动 SLAM 框架，通过可扩展的 3D 语义地图系统 OpenRoboVox 自适应平衡几何探索与语义推理，结合前沿探索、全局语义地图推理和基于 VLM 的自我中心推理，在大型真实环境中实现 100% 任务成功率，并在 GOAT-Bench 上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有主动 SLAM 方法主要关注几何探索，缺乏语义理解能力；而基于 VLM 的方法受限于局部视野，难以进行全局语义推理。作者希望将几何探索与语义推理有机结合，使机器人能够在场景理解逐步提升的过程中从探索平滑过渡到语义引导的导航。

Method: 提出 RoboAtlas 框架，包含三个核心组件：(1) OpenRoboVox，一个可扩展的 3D 语义地图系统；(2) 前沿探索模块用于几何探索；(3) 全局语义地图推理与自我中心 VLM 推理模块。通过上下文多臂赌博机（contextual multi-armed bandit）自适应地在探索与语义引导导航之间切换，随着场景理解提升逐渐偏向语义导航。

Result: 在超过 1800 m² 的大型真实环境中（约 30k 语义实例）实现 100% 任务成功率。在 GOAT-Bench "Val Unseen" 基准上达到 SOTA，使用 GPT-4o 时成功率为 90.6%，比最强基线提升 17.8 个百分点；使用更小的 Qwen2.5-VL-7B 模型仍达 88.8% 成功率，超越所有使用 GPT-4o 的基线。

Conclusion: 将基础模型与大尺度 3D 语义地图相结合，能够实现鲁棒且高效的上下文主动 SLAM。语义地图框架带来的信息增益比单纯替换底层基础模型更为关键，这为机器人导航中融合几何与语义推理提供了有效范式。

Classification reason: 该论文提出 RoboAtlas，一个面向机器人的上下文主动 SLAM 框架，结合几何探索与语义推理，在真实机器人（Unitree Go2）上实现导航与规划。这属于具身智能中的环境建模、规划与控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong SLAM and world model integration, but lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: Active SLAM, 3D semantic mapping, VLM-based reasoning, real-world robot deployment, GOAT-Bench SOTA
...
### [20] [Learning Action Priors for Cross-embodiment Robot Manipulation](https://arxiv.org/abs/2606.26095)
...
TL;DR: 本文提出一种两阶段训练框架，在VLA模型的动作模块中预训练跨具身时序运动先验，再通过解码器复用和潜蒸馏将其迁移到VLA联合训练中，显著提升跨具身机器人操作的收敛速度、成功率和数据效率。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型将VLM骨干与动作模块联合优化，动作模块几乎从零学习物理运动，缺乏显式运动先验，导致早期优化需同时发现时序动作动力学和跨模态对齐，在跨具身场景下尤为困难。

Method: 提出两阶段训练框架：Stage 1 使用轻量级 flow-matching 编码器-解码器动作模块，仅从无条件的动作轨迹中学习时序运动结构，不处理视觉或语言 token；Stage 2 通过解码器复用和早期潜蒸馏将学到的先验迁移到VLA训练中，同时允许端到端策略精调。训练好的编码器还可作为紧凑的历史压缩器，将状态-动作历史汇总为单个时序上下文 token。

Result: 在13个跨具身任务（模拟和真实平台）上，相比无动作先验的VLA训练，该方法实现更快的收敛、更高的成功率，在数据稀缺的真实世界任务上表现尤为突出。扩大Stage 1的动作数据规模可得到更泛化的动作先验，直接提升下游VLA性能。

Conclusion: 通过两阶段训练为VLA模型的动作模块注入跨具身运动先验，能有效解决动作模块从零学习的问题，提升跨具身机器人操作的学习效率和泛化能力。

Classification reason: 该论文提出跨具身机器人操作的动作先验学习框架，属于具身智能中的动作规划与控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Proposes action prior pretraining for VLA models, aligning with world model and efficient inference priorities. Signals: cross-embodiment robot manipulation, flow-matching action module, latent distillation, data efficiency, world model connection
...
### [21] [Conformal Orbit-Valid Trust Horizons for Equivariant World Models](https://arxiv.org/abs/2606.24946)
*Hongbo Wang*
...
TL;DR: 本文研究了具有群对称性的潜世界模型的信任视界认证问题。通过单步潜残差和有限时间扩展估计形成原始视界曲线，并用分裂共形乘性因子进行校准。理论证明精确等变性可将校准后的信任视界曲线沿群轨道传输，使滚动误差和信任视界在轨道上恒定。实验表明等变模型在2D和3D环境中均能实现单扇区安全且非平凡的轨道有效认证，而非等变基线需要多扇区校准或存在违规。


<details>
  <summary>Details</summary>
Motivation: 学习到的世界模型只有在滚动误差可控的视界范围内才有用。现有方法缺乏对信任视界的系统认证，特别是当环境具有对称性时，如何利用对称性来保证模型在不同状态下的可靠性是一个重要问题。

Method: 提出一种基于分裂共形预测的信任视界认证框架。首先从单步潜残差和有限时间扩展估计构建原始视界曲线，然后用分裂共形乘性因子校准。理论证明当环境动力学、编码器、预测器、动作变换和潜度量满足等变/不变条件时，滚动误差和信任视界在群轨道上恒定。在2D对称基板和3D偏航审计环境中验证了等变模型、普通模型和增强模型的轨道有效性。

Result: 在可复现审计集上共形因子γ_α=1.0，原始证书已足够保守。50次稳定审计中零反保守违规，违规率精确二项式95%上界为5.8%。等变模型轨道传输残差中位数1.1%，最大4.1%。证书非平凡（中位认证/测量视界比0.67）。2D基板上所有模型均单扇区有效；3D偏航审计中仅等变模型获得单扇区安全且非平凡的轨道有效证书。

Conclusion: 等变世界模型可以利用群对称性实现单扇区校准即可获得轨道有效的信任视界证书，而非等变模型需要多扇区校准或面临违规风险。该证书是一种保守的分布性审计而非全局可达性保证，证书引导的子目标间距在当前的3D CEM-MPC行为层中尚未得到确认。

Classification reason: 该论文研究具有已知群对称性的潜世界模型的信任视界（trust horizon）认证问题，属于世界模型的可信度与安全性分析，与具身世界模型中的规划控制密切相关。论文使用等变模型（equivariant model）在2D和3D环境中进行实验，涉及动作条件预测与规划（CEM-MPC），因此映射到[具身世界模型与规划控制]。

Importance reason: Strong theoretical contribution to world model certification with equivariance, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: world model certification, equivariance for trust horizons, conformal prediction, orbit-valid certificates, 2D/3D environment validation
...
### [22] [Reflective VLA: In-Context Action Consequences Make VLAs Generalize](https://arxiv.org/abs/2606.25215)
...
TL;DR: Reflective VLA 通过在上下文中引入观察-动作-后果三元组，使 VLA 模型能够感知动作的实际效果，从而在具身控制中实现更好的跨环境泛化，在 LIBERO-Plus 和 LIBERO-Plus-Hard 上分别提升 5.4 和 4.2 个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型是反应式的，仅根据当前观测预测动作，无法捕捉部署环境中特有的相机-机器人几何关系、标定误差或系统执行偏差，导致过拟合训练环境、泛化能力差。

Method: 提出 Reflective VLA，将每个决策建立在观察-动作-后果三元组上下文之上，记录机器人观测、执行动作以及场景变化。架构上通过 VLM 的共享注意力机制处理所有观测模态，动作专家基于历史三元组和当前观测进行推理，采用块因果掩码实现并行多帧训练并支持 KV 缓存实时推理。

Result: 在 LIBERO 和 SimplerEnv-Bridge 上保持强分布内性能；在 LIBERO-Plus 和 LIBERO-Plus-Hard 分布偏移场景下，相比反应式基线分别提升 5.4 和 4.2 个百分点。消融实验证明动作后果信息而非单纯增加上下文长度是泛化关键。

Conclusion: Reflective VLA 通过引入动作后果上下文有效提升了 VLA 模型在分布偏移下的泛化能力，验证了在具身控制中建模动作-效果映射的重要性，为构建更鲁棒的机器人策略提供了新思路。

Classification reason: 该论文提出 Reflective VLA，通过观察-动作-后果三元组上下文来改进 VLA 模型的泛化能力，核心是具身机器人控制中的动作条件建模与分布外泛化，属于具身世界模型与规划控制方向。

Importance reason: Strongly aligns with world model and efficient inference priorities via in-context action consequences for embodied generalization. Signals: world model, in-context learning, action consequences, embodied generalization, block-causal mask
...
### [23] [ROAD-VLA: Robust Online Adaptation via Self-Distillation for Vision-Language-Action Models](https://arxiv.org/abs/2606.25800)
...
Importance: 高优先级 (88.36) - Top 3 精读候选

TL;DR: ROAD-VLA提出了一种基于优势引导自蒸馏的VLA模型在线自适应框架，通过在动作空间构建近端教师模型，将稀疏奖励转化为密集的token级监督信号，在7个机器人操作环境中优于PPO。


<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人操作任务中的在线自适应面临挑战：稀疏奖励对高维自回归动作策略的监督信号弱，而传统的基于文本特权教师的自蒸馏方法存在符号指导与底层机器人动作之间的模态鸿沟，效果不佳。

Method: 提出ROAD-VLA框架，核心是优势引导的自蒸馏：1) 在动作空间中通过扰动动作token logits并加入校准的优势估计来构建近端教师模型；2) 将稀疏奖励转化为密集的token级监督信号；3) 推导了在校准优势和精确教师匹配下的策略改进下界。

Result: 在7个机器人操作环境（含分布内和分布外偏移场景）中，ROAD-VLA在几乎所有设置下均优于PPO，展现出鲁棒的在线VLA自适应能力。

Conclusion: ROAD-VLA通过动作空间中的优势引导自蒸馏有效解决了VLA模型在线自适应中的稀疏奖励和模态鸿沟问题，为机器人操作任务的策略优化提供了新的有效范式。

Classification reason: 该论文提出ROAD-VLA，一种面向VLA（视觉-语言-动作）模型的在线自适应框架，通过自蒸馏和优势估计实现机器人操作任务的策略优化。这属于具身智能中机器人动作规划与控制的范畴，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Strong contribution to VLA online adaptation with self-distillation, but only tangentially touches priority subtopics like world models or continuous representations. Signals: VLA online adaptation, advantage-guided self-distillation, token-level supervision, robotic manipulation, PPO outperformance
...
### [24] [When Do Conservation Laws Survive Learned Representations? Certified Horizons for Latent World Models](https://arxiv.org/abs/2606.24945)
*Hongbo Wang*
...
Importance: 高优先级 (92.58) - Top 2 精读候选

TL;DR: 本文研究物理世界模型中守恒律在潜空间表征学习后是否仍可被认证。作者提出"解码物理不变量"作为认证对象，并推导出"壳-地平线"认证框架，将预算分解为表征、读出和潜动力学缺陷三部分。实验表明硬规范辛结构在已知相坐标下表现最佳但无法跨学习图表，而受控Lipschitz对齐的软不变量在潜表征设置中有效，像素级认证需在读出稳定子管上恢复。


<details>
  <summary>Details</summary>
Motivation: 物理世界模型常通过潜空间表征学习来建模复杂物理系统，但学习到的潜表征是否还能保持物理守恒律（如能量守恒）是一个关键但未被充分研究的问题。现有方法要么学习潜哈密顿量（可能偏离真实能量），要么学习标量见证函数，但都无法保证解码后的物理不变量在 rollout 过程中始终守恒。本文旨在建立一套可认证的框架，从可测量的模型缺陷出发，预先给出 rollout 步数的保证边界。

Method: 提出"解码物理不变量"作为认证核心对象，即对潜状态解码后评估已知物理不变量。推导"壳-地平线"认证框架（shell-horizon certificates），将认证预算分解为表征缺陷、读出缺陷和潜动力学缺陷三部分。引入单调对齐桥（monotone alignment bridge），使软学习见证函数能为解码不变量提供可认证的地平线。在状态观测、学习提升观测和像素观测三种设置下，对保守系统进行实验验证。

Result: 硬规范辛结构在已知相坐标下产生最长地平线，但无法跨学习图表；受控Lipschitz对齐的软不变量在潜表征设置中有效存活；像素级认证需在读出稳定子管上恢复；开普勒问题暴露了几何边界。实验表明守恒认证可以在潜表征学习中存活，但并非所有几何先验都能同等存活。

Conclusion: 核心对象不是潜哈密顿量，而是解码物理不变量——其对表征学习的鲁棒性可以被度量、认证和证伪。这为物理世界模型的可信度提供了理论保证，也为设计更可靠的物理表征学习方法指明了方向。

Classification reason: 论文研究物理世界模型中守恒律在潜空间表征学习后的可认证性，属于物理世界建模与动力学模型范畴，与 [CANONICAL] 可交互与物理世界建模 高度匹配。

Importance reason: Directly addresses certified conservation in latent world models, aligning with top-priority world model and continuous representation subtopics. Signals: world model certification, latent representation conservation, shell-horizon framework, decoded physical invariant, geometric priors comparison
...
### [25] [AISPO: Enhancing Depth Reliability for Robotic Manipulation of Non-Lambertian Objects via Affine-Invariant Shape Prior](https://arxiv.org/abs/2606.25503)
...
TL;DR: AISPO 是一个面向机器人操作的深度补全框架，通过多尺度 RGB-D 特征融合与仿射不变形状先验，提升透明/高反光等非朗伯体物体的深度估计可靠性，显著改善抓取成功率。


<details>
  <summary>Details</summary>
Motivation: 非朗伯体（透明、高反光）物体的深度测量常被噪声污染或缺失，导致机器人运动规划失败、抓取位姿无效。现有方法多关注平均深度精度，缺乏对深度图物理合理性与结构完整性的保障。

Method: 提出 AISPO 框架，包含：(1) 多尺度 RGB-D 特征融合模块，结合彩色图像与原始深度信息；(2) 仿射不变形状先验（Affine-Invariant Shape Prior），强制预测深度图保持几何一致性与结构完整性，缓解灾难性深度失败。

Result: 在多个基准数据集上取得竞争性能，对未见物体和新场景泛化能力强。真实机器人抓取实验表明，深度可靠性提升显著提高了操作成功率，尤其对透明物体效果突出。

Conclusion: AISPO 通过引入仿射不变形状先验有效增强了非朗伯体物体的深度补全质量，为机器人操作提供了更可靠的深度感知，在挑战性场景下优于现有方法。

Classification reason: 该论文提出深度补全框架 AISPO，利用多尺度 RGB-D 融合与仿射不变形状先验提升非朗伯体（透明/高反光）物体的深度可靠性，服务于机器人抓取。核心是深度估计与几何一致性增强，属于 3D 感知与重建范畴，映射到 [CANONICAL] subtopic "3D 生成与重建"（three_d_generation）。

Importance reason: Focuses on robotic depth completion for non-Lambertian objects, which is outside the user's core priority subtopics of continuous models, video/image generation, and world models. Signals: Robotics manipulation, Depth completion, Non-Lambertian objects, Affine-invariant shape prior, Not aligned with priority subtopics
...
### [26] [From Sounds to Scenes: A Benchmark for Evaluating Context-Aware Auditory Scene Understanding in Large Audio Language Models](https://arxiv.org/abs/2606.25391)
...
TL;DR: 本文提出CASU基准，用于评估大型音频语言模型在真实听觉场景中整合语音、声音事件和背景环境等多层声学信息进行上下文感知场景理解的能力，包含四个任务：上下文问答、实体提取、说话者角色推断和反事实推理。


<details>
  <summary>Details</summary>
Motivation: 现有音频语言模型评测主要孤立评估语音、声音、音乐等单个声学层，忽略了真实世界中多个声源同时出现时的复杂上下文关系。缺乏一个能够评估模型在真实听觉场景中进行上下文感知场景理解（CASU）能力的基准。

Method: 提出一个可扩展的流水线，通过组合真实场景声音与合成语音构建时间精确的半合成音频流。基于此数据设计了四个评测任务：上下文问答、场景实体提取、说话者角色推断和反事实推理（通过操纵场景元素）。

Result: 在多个LALM上的实验表明，有效的听觉场景理解需要整合所有声学层的信息，而非仅依赖语音或声音单一层，验证了CASU对于推进复杂音频理解的必要性。

Conclusion: CASU基准揭示了现有LALM在真实听觉场景理解方面的不足，强调了多声学层整合的重要性，为未来音频语言模型在复杂场景理解能力上的提升提供了评测基础。

Classification reason: 该论文提出了CASU基准，用于评估大型音频语言模型（LALMs）在上下文感知的听觉场景理解能力，属于多模态（音频）评测与基准方向。该论文与[多模态评测与基准]（multimodal_benchmark）这一CANONICAL subtopic高度匹配。

Importance reason: Focuses on audio scene understanding benchmark, not aligned with user's priority on continuous multimodal models or video/image generation. Signals: Audio LLM benchmark, context-aware scene understanding, not in priority subtopics, no boost keywords matched, negative keywords not present
...
### [27] [Toward Low-Latency Vision-Language Models with Doubly-Correct Predictions in Egocentric Visual Understanding](https://arxiv.org/abs/2606.25160)
...
TL;DR: 本文研究面向第一人称视觉理解的 VLM 低延迟推理，提出一种基于 rationale 的剪枝策略，在保持模型高效的同时确保预测既准确又有证据支撑（doubly-correct），在 egocentric 视频数据集上取得了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 人机协作任务中，VLM 需要在板载设备上实现低延迟推理，同时保证安全交互——预测不仅要准确，还要有正确的证据定位（doubly-correct）。现有剪枝方法往往牺牲预测准确性来保留证据定位，缺乏兼顾两者的策略。

Method: 提出 rationale-informed pruning 策略，在剪枝过程中引入证据定位（rationale）信息，使模型在压缩后仍能保持预测与证据的一致性，实现 doubly-correct predictions。

Result: 在 egocentric 视频数据集上的实验表明，所提方法在预测准确率和 doubly-correct 指标上均优于现有剪枝方法。

Conclusion: 本文证明了在 VLM 剪枝中显式对齐证据与决策的重要性，为高效且可靠的 VLM 在人机交互与具身智能中的应用提供了新思路。

Classification reason: 论文研究 VLM 的权重剪枝（pruning）以实现低延迟推理，属于多模态模型高效化与轻量化方向，与 [CANONICAL] subtopic "多模态高效化与轻量化"（efficient_multimodal）高度匹配。

Importance reason: Focuses on VLM pruning for egocentric video, which is tangentially related to efficient inference but not directly on your priority subtopics like continuous models or autoregressive generation. Signals: VLM pruning, low-latency inference, egocentric video, doubly-correct predictions, rationale-informed pruning
...
### [28] [fARfetch: Enabling Collocated AR-HRC in Large Visually Diverse Environments with VLM-Driven AR Content Adaptation](https://arxiv.org/abs/2606.25162)
*Christian Fronk, Hanting Ye, David Hunt, Miroslav Pajic, Maria Gorlatova*

Main category: cs.HC, cs.CV, cs.RO
...
TL;DR: fARfetch 是一个面向大规模视觉多样环境的 AR 人机协作系统，利用 VLM 驱动的 AR 内容自适应（颜色、大小、方向）来保持虚拟内容的可读性，结合共享语义地图和世界缩微图表示，在 30.5m 户外真实场景中显著提升了任务完成时间和降低了用户工作负荷。


<details>
  <summary>Details</summary>
Motivation: 增强现实（AR）可以改善人机协作，但在大型、视觉多样的户外环境中，远距离和超出视线范围时，AR 内容的可读性（legibility）面临挑战，现有系统缺乏对内容的自适应调整能力。

Method: fARfetch 系统包含三个核心组件：(1) 共享语义环境地图——AR 头显与机器人共同构建语义地图，可视化检测到的地标以支持地标引导的 go-to 指令；(2) 上下文感知的世界缩微图（world-in-miniature）表示，用于精细路径编辑；(3) VLM 驱动的 AR 视图管理，联合调整虚拟内容的颜色、大小和方向以保持可读性。系统基于 Meta Quest 3 和 Unitree Go2 四足机器人实现。

Result: 在 30.5m 户外真实场景的 13 人用户研究中，fARfetch 相比非 AR 基线任务完成时间提升 66%，心理需求降低 43%，时间需求降低 34%，挫败感降低 66%。可读性调查表明系统有效维持了户外大场景中虚拟内容的可读性。

Conclusion: fARfetch 证明了 VLM 驱动的 AR 内容自适应能够有效解决大规模视觉多样环境中的 AR 可读性问题，显著提升人机协作效率，为户外 AR-HRC 系统提供了可行的设计范式。

Classification reason: 该论文使用 VLM 驱动 AR 内容自适应（颜色、大小、方向），属于视觉语言模型在 AR-HRC 场景中的应用，与 [CANONICAL] subtopic "视觉语言模型与 MLLM" 相关。

Importance reason: Focuses on AR-HCI and VLM-driven content adaptation, not on core continuous multimodal models or autoregressive generation. Signals: AR-HRC system, VLM-driven content adaptation, outdoor AR legibility, user study, not core priority subtopics
...
### [29] [OracleAnalyser: Analysing Implicit Semantics of Oracle Bone Scripts through MLLMs with Post-training](https://arxiv.org/abs/2606.25906)
...
Importance: 值得关注 (65.49) - 未入选精读

TL;DR: OracleAnalyser 是一个基于后训练技术的甲骨文分析推理框架，通过多阶段微调 Qwen2.5-VL-3B-Instruct 并引入 Stable Focal Preference Optimization (SFPO) 算法，在仅 3B 参数下超越更大规模模型，同时发布了甲骨文推理数据集、偏好数据集和评测基准。


<details>
  <summary>Details</summary>
Motivation: 现有甲骨文研究方法主要局限于识别任务，忽略了同样关键的语义分析环节，缺乏针对甲骨文分析的推理框架和评测基准。

Method: 基于 Qwen2.5-VL-3B-Instruct 进行多阶段后训练（post-training），包括监督微调和偏好优化；提出 Stable Focal Preference Optimization (SFPO) 算法，针对甲骨文数据集特点进行定制；构建了甲骨文推理数据集和偏好数据集，以及新的评测基准。

Result: OracleAnalyser 在 3B 参数规模下取得了优异的分析性能，超越了参数量更大的模型。

Conclusion: 后训练技术和定制的偏好优化算法能有效提升 MLLM 在甲骨文分析这一专业视觉文档理解任务上的能力，小模型也能超越大模型。

Classification reason: 该论文研究甲骨文（Oracle Bone Scripts）的语义分析，属于视觉文档理解与推理任务，与 [CANONICAL] 多模态评测与基准（multimodal_benchmark）也有一定关联，但核心是视觉问答与文档理解（multimodal_vqa_doc），因为甲骨文是一种古代文字文档，论文构建了推理数据集和评测基准，并基于 MLLM 进行视觉推理分析。

Importance reason: Focuses on a niche historical document analysis task with no overlap with user's priority subtopics or boost keywords. Signals: Oracle bone script analysis, MLLM post-training, preference optimization, no autoregressive generation, no continuous language model
