...
### [1] [ChannelTok: Efficient Flexible-Length Vision Tokenization](https://arxiv.org/abs/2606.04461)
...
TL;DR: ChannelTok 提出了一种轻量、高效的通道级灵活长度视觉 tokenizer，通过将每个潜空间通道视为视觉 token，结合 CNN-Transformer 混合骨干和随机尾部丢弃训练，实现了在多种 token 预算下的一致高质量，并在解码速度（8.6×）和参数量（2.1×）上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有灵活长度视觉 tokenizer 虽然质量高，但依赖参数庞大的骨干网络和缓慢的多步生成式解码器，计算成本极高。作者希望设计一种简单、轻量、快速的 tokenizer，同时保持灵活长度和高质量。

Method: 提出通道级（channel-wise）视觉 tokenization：将每个潜空间通道视为一个视觉 token，使用参数高效的 CNN-Transformer 混合骨干网络。训练时采用随机尾部丢弃（stochastic tail-dropping）策略，迫使通道按语义重要性自动排序。推理时只需保留前 k 个通道即可实现灵活压缩，并支持变长自回归图像生成。

Result: 在 ImageNet 上，ChannelTok 在多种 token 预算下保持一致的感知质量，rFID 达到 2.92，解码速度比次优方法快 8.6 倍，参数量仅 159M（小 2.1 倍），建立了新的质量-效率前沿。

Conclusion: 通道级 tokenization 是一种强大且实用的视觉表征范式，在保持高质量的同时大幅提升效率，为高效视觉表征和自回归生成提供了新方向。

Classification reason: 该论文提出了一种通道级（channel-wise）的连续视觉 tokenizer，通过将每个潜空间通道视为一个视觉 token，实现灵活长度的视觉表征。这与 [CANONICAL] subtopic "连续视觉 Tokenizer 与表征自编码器" 高度吻合，属于连续视觉 token 与表征自编码方向。
...
### [2] [Supportive Token Revealing for Fast Diffusion Language Model Decoding](https://arxiv.org/abs/2606.04236)
...
TL;DR: AXON 是一种无需训练的模块，通过选择"锚点"（confident masked tokens）来辅助不确定位置的去噪，从而改善扩散语言模型并行解码的质量-延迟权衡。


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型在并行解码时面临质量-延迟权衡：激进解码可能过早提交相互依赖的 token，保守解码则需要大量去噪步。现有方法仅关注哪些 token 安全可揭示，但未考虑剩余不确定 token 可能因依赖被掩码 token 而形成解码瓶颈。

Method: 提出 AXON，一个即插即用的训练无关模块。它监控剩余不确定掩码 token 的状态，当需要额外上下文时介入，将选择标准从"哪些 token 最安全可揭示"转变为"哪些置信揭示最能支持后续去噪"。AXON 利用注意力、不确定性和置信度信号，选择不确定位置所关注的置信掩码 token 作为"锚点"。

Result: 在推理和代码生成基准上，AXON 在多个扩散语言模型上改善了现有并行解码策略的质量-延迟权衡，通常能在保持或提升准确率的同时减少函数评估次数。

Conclusion: AXON 作为一种轻量级、无需训练的辅助模块，能有效提升扩散语言模型并行解码的效率与质量，为扩散语言模型的加速推理提供了新思路。

Classification reason: 该论文研究扩散语言模型的并行解码加速方法，属于连续语言模型（离散扩散语言模型在连续潜空间中进行去噪生成）范畴，与 canonical 子主题"连续语言模型与语言流"高度匹配。

Importance reason: Directly addresses continuous language model decoding efficiency, a top priority subtopic, with a novel training-free method. Signals: diffusion language model, parallel decoding, quality-latency trade-off, training-free module, anchor tokens
...
### [3] [Global Sketch-Based Watermarking for Diffusion Language Models](https://arxiv.org/abs/2606.04486)
*Daniel Zhao*

Main category: cs.CL, stat.ML, cs.CR, cs.LG
...
TL;DR: 本文提出一种针对掩码扩散语言模型的全局草图水印方法，利用扩散模型在生成过程中可全局控制序列统计量的特性，设计了一种与上下文无关、顺序无关的水印方案，并分析了其失真性、可靠性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型水印方法主要针对自回归模型，依赖局部上下文扰动下一个 token 分布。扩散语言模型在生成时同时对多个未确定位置进行联合采样，使得全局序列统计量在生成过程中可被追踪，现有局部水印方案无法充分利用这一特性。因此，作者希望设计一种适用于扩散语言模型的全局水印方法。

Method: 提出一种基于全局草图（sketch）表示的水印方法。在掩码扩散语言模型的生成过程中，控制一个向量值草图（vector-valued sketch）作为文本的全局统计表示。水印规则不依赖于局部上下文，而是基于全局草图进行控制，使得检测与生成时的局部上下文解耦，实现顺序无关的统计量。

Result: 分析了所提方法的失真性（distortion）、可靠性（soundness）和鲁棒性（robustness）性质，给出了理论分析结果。

Conclusion: 全局草图水印方法适用于掩码扩散语言模型，相比传统上下文相关水印，具有顺序无关、不表现为简单 token 偏置等优势，为扩散语言模型的水印技术提供了新思路。

Classification reason: 该论文提出面向掩码扩散语言模型（masked diffusion language model）的全局草图水印方法，属于连续语言模型（扩散语言模型）的安全与鲁棒性研究。论文核心涉及扩散语言模型的生成过程，与 [CANONICAL] 子主题"连续语言模型与语言流"高度相关，因此复用该子主题。

Importance reason: Novel watermarking for diffusion language models aligns with continuous language model priority, but lacks direct video/image generation focus. Signals: diffusion language model watermarking, global sketch control, order-agnostic detection, robustness analysis, continuous language model application
...
### [4] [STaR-Quant: State-Time Consistent Post-Training Quantization for Diffusion Large Language Models](https://arxiv.org/abs/2606.04945)
...
TL;DR: STaR-Quant 提出了一种面向扩散大语言模型（DLLM）的状态-时间一致后训练量化框架，通过状态引导激活变换（SGAT）和时序注意力补偿（TAC）分别解决掩码/非掩码 token 激活分布差异和跨步量化误差累积问题，在低比特量化下显著优于现有 PTQ 方法，并实现 1.69 倍加速和 3.14 倍内存节省。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（DLLM）虽在文本生成上展现出潜力，但其模型规模大且需要迭代去噪，导致巨大的内存和计算开销。现有后训练量化（PTQ）方法未考虑 DLLM 特有的两个挑战：1）掩码与非掩码 token 在同一去噪步中激活分布差异大（状态依赖）；2）量化误差在迭代解码过程中跨步累积（时序累积）。因此需要专门针对 DLLM 的量化方法。

Method: 提出 STaR-Quant 框架，包含两个核心组件：1）状态引导激活变换（SGAT）：将掩码和非掩码 token 分配到不同的激活变换空间，同时使用统一的静态权重侧变换，解决状态依赖的激活分布差异；2）时序注意力补偿（TAC）：通过轻量级块对角仿射映射校正量化后的注意力表示，缓解跨步误差累积。

Result: 在代表性 DLLM 上的实验表明，STaR-Quant 在低比特权重量化上持续优于强 PTQ 基线，同时相比 FP16 部署实现最高 1.69 倍加速和 3.14 倍内存节省。

Conclusion: STaR-Quant 有效解决了 DLLM 量化中的状态依赖激活差异和时序误差累积两大挑战，为扩散语言模型的高效部署提供了可行的量化方案。

Classification reason: 该论文研究扩散大语言模型（DLLM）的后训练量化，DLLM 属于连续语言模型范畴（通过迭代掩码去噪生成文本），因此映射到 canonical 子主题"连续语言模型与语言流"。

Importance reason: Directly addresses efficient inference for continuous language models via quantization, a key priority subtopic. Signals: continuous language model, efficient inference, diffusion LLM quantization, state-time consistency, 1.69x speedup
...
### [5] [CLAW: Learning Continuous Latent Action World Models via Adversarial Latent Regularization](https://arxiv.org/abs/2606.04130)
...
TL;DR: CLAW 是一个完全端到端自监督框架，通过对抗性潜正则化和扩散视频生成，从无动作标签的视频中联合学习连续潜动作表示与世界模型，支持模仿学习和目标导向规划。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型通常依赖动作标签或标注，限制了其在无动作数据场景下的应用。本文旨在从纯视觉观察中学习有意义的潜动作表示和世界模型，无需任何动作标注。

Method: CLAW 包含潜动作模型（Latent Action Model）和世界模型，通过对抗性潜正则化（adversarial latent regularization）约束潜动作空间的结构化，利用扩散模型进行视频生成来建模环境动力学。潜动作模型从观测序列中推断潜动作，世界模型基于当前观测和潜动作预测未来状态，两者联合训练。

Result: 在多种任务和机器人平台上，CLAW 生成的潜动作表示具有语义意义，支持有效的动作迁移，在模仿学习和目标导向规划任务上优于现有方法。

Conclusion: CLAW 证明了从无动作视频中学习连续潜动作世界模型的可行性，为无监督的机器人学习和规划提供了新范式。

Classification reason: 该论文提出 CLAW 框架，从无动作标签的视频中联合学习连续潜动作表示与世界模型，支持模仿学习和目标导向规划，属于面向机器人/具身智能的动作规划和控制的世界模型，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Directly advances continuous latent action world models with diffusion video generation, aligning with top-priority continuous multimodal and world model directions. Signals: continuous latent action, world model, diffusion video generation, adversarial latent regularization, imitation learning from observation
...
### [6] [CoPark: Learning Reactive Parking via Self-Play](https://arxiv.org/abs/2606.04149)
...
TL;DR: CoPark 提出一种基于自博弈强化学习和残差策略架构的反应式自主泊车方法。通过预计算离线规划提供固定动作先验，残差头学习反应式修正，并利用合作伙伴威胁调制的通道非对称释放机制在纵向和横向通道间分配控制权，实现了高精度泊车与安全交互的平衡。在多个停车场零样本测试中达到约70-85%的成功率和仅3-6%的碰撞率，并涌现出倒车让行、中途让行、窄道通行和排队等交互行为。


<details>
  <summary>Details</summary>
Motivation: 自主泊车中，高精度几何目标达成与安全交互之间存在冲突：精确性要求遵循固定几何规划，而交互需要即时偏离规划以避让其他车辆。现有方法（经典规划、模仿学习、大规模RL）难以同时兼顾这两个目标。

Method: 提出 CoPark，基于残差策略架构的多智能体自博弈RL方法。预计算离线规划提供固定动作先验，残差头学习反应式修正。关键设计是合作伙伴威胁调制的通道非对称释放机制：连续威胁信号将纵向通道控制权交给残差头以实现让行，横向通道保持锚定在预计算参考上以保持亚米级车位对齐。闭环精炼层修正动作网格离散化带来的残余终端误差。

Result: 在6个停车场训练，在Dragon Lake Parking (DLP) 和 DeepScenario Open 3D (DSC3D) 基准上零样本测试，CoPark 达到约70-85%的成功率，仅3-6%的碰撞率，显著优于经典规划、模仿学习和大规模RL基线。涌现出倒车让行、中途让行、窄道通行和排队等交互行为。

Conclusion: CoPark 通过残差策略与通道非对称控制机制有效解决了高精度泊车与安全交互的冲突，展示了自博弈RL在复杂多智能体交互场景中的潜力，为反应式自主泊车提供了新的有效范式。

Classification reason: 该论文研究基于自博弈强化学习的自主泊车策略，涉及多智能体交互、动作规划与安全控制，属于具身智能中机器人规划与控制的范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics contribution but outside user's core priority subtopics and negative keywords. Signals: Self-play RL, reactive parking, residual policy, emergent interaction, zero-shot transfer
...
### [7] [Multi-Agent Next-Best-View Optimization for Risk-Averse Planning](https://arxiv.org/abs/2606.04158)
...
TL;DR: 提出一种分布式、风险感知的多机器人Next-Best-View框架，每个机器人维护私有局部3DGS地图，通过Consensus ADMM在通信图上联合优化期望信息增益，同时使用AV@R建模碰撞风险，在Gibson环境中以极低通信开销接近集中式方法的建图质量和轨迹安全性。


<details>
  <summary>Details</summary>
Motivation: 集中式多机器人NBV方法依赖共享原始传感器数据或大量通信开销，可扩展性差。现有分布式方法缺乏对安全性和信息增益的联合优化，且未考虑轨迹碰撞风险。

Method: 每个机器人维护私有局部3DGS地图；团队联合最大化沿规划轨迹掩码区域内的期望信息增益（EIG）；分布式目标通过Consensus ADMM（C-ADMM）在通信图上求解，仅交换候选视点、轨迹描述符和标量EIG贡献；使用局部3DGS地图上的Average Value-at-Risk（AV@R）建模轨迹碰撞风险，用于调整掩码半径和路径评分。

Result: 在Gibson环境中多个团队规模下，分布式方法在映射质量和轨迹安全性上接近集中式基线，同时将通信量降低数个数量级。

Conclusion: 所提出的分布式风险感知NBV框架在保持高建图质量和安全性的同时，显著降低了通信开销，为大规模多机器人探索提供了可扩展的解决方案。

Classification reason: 该论文研究多机器人分布式Next-Best-View规划，使用3DGS地图进行风险感知路径规划，属于机器人具身智能中的规划与控制，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on multi-robot planning and 3DGS mapping, not on the user's priority subtopics like continuous language models or video generation. Signals: Distributed NBV, Consensus ADMM, 3D Gaussian Splatting, Risk-averse planning, AV@R
...
### [8] [Affordance2Action: Task-Conditioned Scene-level Affordance Grounding for Real-Time Manipulation](https://arxiv.org/abs/2606.04172)
...
TL;DR: 本文提出 Affordance2Action (A2A)，一个面向场景级、任务条件化的部件可操作区域定位的基准与学习框架。核心贡献包括 A2A-Bench 操作基准（覆盖单区域和多区域指令对应）、A2A-AffordGen 自动化标注管线，以及基于此的实时可操作区域定位与操作策略。实验表明现有分割、VLM 定位和可操作区域蒸馏方法在该任务上存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有可操作区域数据集和基准与任务条件化操作场景不匹配：它们通常聚焦于抓取或物体级可操作性，依赖合成场景，或假设单指令-区域对应关系。然而真实场景中，同一物体在不同任务下可能提供不同交互功能，且一个任务可能对应多个有效功能区域。因此需要面向场景级、任务条件化的部件可操作区域定位基准。

Method: 构建 A2A-Bench 操作基准，包含日常场景中单区域和多区域指令对应。开发 A2A-AffordGen 自动化标注管线，结合语言模型过滤、交互式部件分割、实例级掩码细化、任务推理指令生成和人工验证。基于该基准，训练实时可操作区域定位模型，并用于下游操作策略。

Result: 实验表明 A2A 暴露了通用分割、VLM 定位和可操作区域蒸馏基线在该任务上的显著差距，同时提升了任务级定位精度，并为下游操作提供了有用的空间先验。

Conclusion: A2A 填补了任务条件化场景级可操作区域定位的空白，提供了高质量基准和自动化标注工具，有望推动具身操作中感知与规划的紧密结合。

Classification reason: 该论文聚焦于任务条件化的场景级可操作区域（affordance）定位，服务于实时机器人操作，属于具身智能中的感知-规划-控制流程，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on robotics affordance grounding, not directly on the user's priority subtopics like continuous language models or video generation. Signals: Robotics manipulation, affordance grounding, task-conditioned, real-time, benchmark
...
TL;DR: 本文提出了 DLO-Lab，一个面向可变形线性物体（如绳索、电缆、橡皮筋）操作的可微仿真器与基准平台。该仿真器支持多种材料属性建模（可/不可拉伸、弹性、弯曲塑性等），并设计了代表性任务基准和专用 DLO 操作智能体，用于评估各种策略学习算法，并进行了 sim-to-real 迁移实验。


<details>
  <summary>Details</summary>
Motivation: 机器人操作可变形线性物体（DLO）在现实应用中非常重要，但现有工作多局限于狭窄的任务特定问题，依赖真实世界演示或手工启发式方法，难以扩展到多样化的材料和任务。同时，现有仿真环境对 DLO 材料行为的支持有限，缺乏通用化的学习与评估平台。

Method: 1) 设计了一个可微仿真器，支持多种材料属性建模（可/不可拉伸、弹性、弯曲塑性、与其他物体的复杂交互）；2) 基于该仿真器构建了一套代表性任务基准，涵盖 DLO 操作中的拓扑复杂性和抓取敏感性挑战；3) 提出了一种专用 DLO 智能体，通过策略性抓取点选择和长程任务分解来最大化控制能力；4) 在该平台上评估了多种策略学习算法，并进行了 sim-to-real 迁移实验。

Result: 实验表明，该平台能够有效支持多种策略学习算法的训练与评估，sim-to-real 迁移实验验证了仿真到真实世界的可迁移性，证明了该平台在推动 DLO 操作研究方面的潜力。

Conclusion: DLO-Lab 提供了一个可微、可扩展的仿真与基准平台，能够支持多样化的 DLO 材料建模和操作任务学习，为可变形线性物体操作的研究提供了标准化评估框架，并展示了 sim-to-real 迁移的可行性。

Classification reason: 该论文关注机器人对可变形线性物体（DLO）的操作，提出了一个可微仿真器用于学习和评估操作技能，并设计了专用智能体进行抓取点规划和长程任务分解。这属于机器人操作与规划控制范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on robotic manipulation of deformable objects, which is outside the user's core interests in continuous multimodal models and video generation. Signals: Deformable linear object manipulation, Differentiable physics simulator, Benchmark for DLO tasks, Sim-to-real transfer, Robotics and control focus
...
TL;DR: PerceptTwin 提出全自动流水线，将机器人感知得到的语义场景表示转化为交互式仿真环境，结合3D资产生成、affordance预测和常识条件检查，并引入LLM裁判验证规划正确性与人类偏好对齐，实验表明可显著提升LLM规划器的成功率并增强安全性。


<details>
  <summary>Details</summary>
Motivation: 传统上为机器人规划创建仿真环境耗时费力，难以针对每个部署环境定制。本文旨在利用机器人已有的感知输出自动构建交互式仿真，从而低成本地验证和优化规划方案。

Method: PerceptTwin 流水线包含：(1) 从机器人感知栈获取开放词汇语义场景表示；(2) 结合3D资产生成构建交互式仿真环境；(3) 预测物体affordance并检查常识条件；(4) 引入LLM裁判验证规划正确性与人类偏好对齐。规划器可迭代利用仿真反馈优化规划。

Result: 在测试任务套件中，PerceptTwin 使 GPT5、GPT5Mini、GPT5Nano 规划器的平均成功率提升约39%；对于因技能前提条件未满足而失败的规划，人类规划验证准确率提升最高达18%。

Conclusion: 从机器人感知自动构建语义场景仿真是一种有前景的范式，可为更安全、更可靠的机器人规划提供基础，同时展示了开放词汇场景仿真在规划验证与对齐中的潜力。

Classification reason: 该论文构建从机器人感知（语义场景表示）自动生成交互式仿真环境的流水线，用于LLM规划器的验证与优化，属于面向机器人规划控制的世界模型，与[具身世界模型与规划控制]高度匹配。

Importance reason: Strong world model and LLM planning verification, but lacks direct focus on continuous multimodal or autoregressive generation. Signals: world model, LLM planning verification, semantic scene reconstruction, interactive simulation, human preference alignment
...
### [11] [Instant-Fold: In-Context Imitation Learning for Deformable Object Manipulation](https://arxiv.org/abs/2606.04269)
...
TL;DR: Instant-Fold 提出了一种基于上下文模仿学习的可变形物体操作框架，通过时序对比预训练学习变形感知视觉表征，并利用流匹配变换器策略从单个人类演示中推断多种操作模式，无需梯度更新即可零样本迁移到真实世界。


<details>
  <summary>Details</summary>
Motivation: 可变形物体操作（如折叠衣物）因高维、部分可观测的状态、长时序拓扑变化以及多种有效操作模式而极具挑战性。现有方法通常需要大量数据或针对特定任务设计，缺乏从单次演示中泛化到多种操作模式的能力。

Method: 首先通过时序对比预训练学习变形感知的视觉表征，然后训练一个流匹配（flow-matching）变换器策略，该策略以单个人类演示为条件，预测动作以执行相应的操作模式。整个策略在仿真中训练，无需梯度更新即可在推理时适应新演示。

Result: 在仿真环境中，Instant-Fold 在多种折叠模式上实现了高成功率，并展现出对空间执行方式和操作顺序变化的泛化能力。在零样本迁移到真实世界设置中，无需额外数据收集或微调即可成功执行折叠任务。

Conclusion: Instant-Fold 证明了通过上下文模仿学习，结合变形感知视觉表征和流匹配策略，可以从单次演示中有效泛化到多种可变形物体操作模式，并实现仿真到真实的零样本迁移，为可变形物体操作提供了一种高效且通用的解决方案。

Classification reason: 该论文提出面向可变形物体操作的即时模仿学习框架，使用流匹配变换器策略从单个人类演示中推断并执行操作模式，属于机器人具身智能中的世界模型与规划控制方向，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Focuses on robotic manipulation, not directly aligned with your priority subtopics on continuous multimodal models or video generation. Signals: Deformable object manipulation, In-context imitation learning, Flow-matching transformer, Zero-shot sim-to-real transfer, Temporal contrastive pretraining
...
### [12] [MAD: Mapping-Aware World Models for Agile Quadrotor Flight](https://arxiv.org/abs/2606.04534)
...
TL;DR: 本文提出 MAD (Mapping-Aware Dreamer)，一种几何感知的世界模型，用于视觉四旋翼飞行。MAD 学习循环潜动力学，重建以机器人为中心的占据网格和可见性网格图，而非原始图像重建。该模型在 DiffAero 中训练，支持基于想象（MAD-Dreamer）和特征提取器（PPO/SHAC）的策略学习。在视觉导航和竞速任务中，MAD 相比纯视觉基线实现了更高的成功率、更快的飞行速度和更好的跨任务迁移，并在真实四旋翼上以 5.05 m/s 的速度完成室外森林飞行。


<details>
  <summary>Details</summary>
Motivation: 在杂乱场景中进行敏捷四旋翼飞行，仅靠深度图像到控制指令的反应式映射是不够的——飞行器需要记忆已观测区域、推断附近占据空间，并在部分可见性和严格延迟约束下行动。现有世界模型通常以原始图像重建为主要自监督目标，缺乏对几何结构和可见性历史的显式编码，难以直接服务于碰撞避免。

Method: 提出 Mapping-Aware Dreamer (MAD)，一种几何感知的世界模型。核心设计包括：(1) 学习循环潜动力学，重建以机器人为中心的占据网格图和可见性网格图，结合本体感知状态（速度、角速度等），迫使潜状态编码局部几何、可见性历史和自运动信息；(2) 在 DiffAero 仿真器中训练，使用 GPU 并行地图构建模块提供高吞吐量的占据和可见性监督；(3) 支持三种策略学习模式：基于想象的 MAD-Dreamer、以及基于 PPO 和 SHAC 的特征提取器变体。

Result: 在视觉导航和竞速任务中，MAD 基智能体相比纯视觉基线实现了更高的成功率、更快的飞行速度和更好的跨任务迁移。模型能产生可解释的地图预测和准确的自运动估计。在仿真中达到 9.66 m/s，在真实森林实验中以 Intel RealSense D435i 实现 5.05 m/s 的安全室内外飞行。

Conclusion: 映射感知的世界模型在模块化空中导航和端到端学习之间提供了一种实用的中间方案。通过显式建模占据和可见性几何信息，世界模型能够更好地支持敏捷飞行中的碰撞避免和策略学习，且具备良好的可解释性和跨任务迁移能力。

Classification reason: 该论文提出 Mapping-Aware Dreamer (MAD)，一种面向视觉四旋翼飞行的几何感知世界模型，用于在杂乱场景中的敏捷飞行、碰撞规避和策略学习。这直接属于世界模型方向下的具身世界模型与规划控制（embodied_world_model）这一规范子主题。

Importance reason: Strong world model paper for robotics but outside user's core focus on continuous multimodal models and video generation. Signals: world model, quadrotor flight, occupancy grid, mapping-aware, real-world deployment
...
### [13] [MineXplore: An Open-Source Reinforcement Learning Exploration Benchmark for GNSS-Denied Underground Environment](https://arxiv.org/abs/2606.04569)
...
TL;DR: MineXplore 是一个基于真实矿井几何数据的开源 MuJoCo 导航仿真基准，支持 GPU 加速的强化学习策略训练，在 104,423 平方米的隧道网络中验证了 PPO 策略的可复现学习效果。


<details>
  <summary>Details</summary>
Motivation: 地下矿井环境存在 GPS 缺失、光照退化、拓扑复杂等极端条件，现有开源仿真基准缺乏基于真实矿井几何数据且兼容 GPU 加速学习管线的环境。

Method: 基于 Leung et al. 2017 智利地下铜矿数据集，通过六阶段 contour-to-MJCF 管线重建隧道网络，包含八边形墙截面、LiDAR 锯齿状墙几何、三种地形摩擦区、全局 5 度倾斜和周期性点光源。使用单智能体 PPO 算法通过 RLlib 在 5 个随机种子上训练验证。

Result: 几何保真度 IoU 达 0.9538，表面纹理相似度 79.4%；PPO 基线最佳滚动覆盖率达 88.89%（5 个种子中 3 个达到 90% 覆盖目标）。

Conclusion: MineXplore 提供了首个基于真实矿井几何数据的开源导航仿真基准，支持稳定可复现的策略学习，为地下环境自主导航研究提供了标准化测试平台。

Classification reason: 该论文构建了一个面向地下矿井环境的机器人导航仿真基准，支持强化学习策略训练，属于具身智能中机器人规划与控制的范畴，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Focuses on underground mining navigation, which is outside the user's priority subtopics and negative keywords. Signals: Open-source benchmark, GNSS-denied navigation, Reinforcement learning, MuJoCo simulation, Low personal relevance
...
### [14] [VISTA: Vision-Grounded and Physics-Validated Adaptation of UMI data for VLA Training](https://arxiv.org/abs/2606.04708)
...
TL;DR: VISTA提出一个三组件框架，通过UMI-VQA数据集对齐VLM到鱼眼视觉域、物理验证管线过滤不可行轨迹、两阶段协同训练，解决UMI数据训练VLA模型时的视觉分布偏移和物理不可行动作两大问题。


<details>
  <summary>Details</summary>
Motivation: UMI数据采集方式可扩展但存在两个关键不匹配：腕装鱼眼视图的严重畸变与局部视角导致预训练VLM分布外；人类采集轨迹常违反运动学限制、发生碰撞或超出控制器带宽，导致VLA策略学到物理不可行的动作。

Method: VISTA包含三个协同组件：(1) UMI-VQA，首个面向腕装鱼眼观测的大规模VQA数据集，通过辅助视觉-语言监督对齐VLM表征到畸变视觉域；(2) 物理验证管线，对轨迹进行数据完整性预检查，并对每条有效轨迹评分（轨迹连续性、自碰撞风险、执行保真度）；(3) 两阶段协同训练，在UMI-VQA上学习视觉-语言基础，在验证后的轨迹上学习动作预测。

Result: 实验表明，UMI-VQA一致提升下游策略性能，物理验证分数与部署成功率强相关。在多种仿真和真实操作任务上，VISTA显著优于π0.5、LingBot-VLA、Wall-X等强基线。

Conclusion: VISTA通过视觉对齐和物理验证有效弥合了UMI数据与VLA训练之间的双重鸿沟，为利用大规模非专业采集数据训练具身策略提供了可行方案。

Classification reason: 该论文聚焦于利用UMI数据训练VLA（视觉-语言-动作）模型，涉及机器人操作策略学习、物理验证与动作规划，属于具身智能与机器人世界模型范畴，因此映射到[具身世界模型与规划控制]（CANONICAL）。同时，论文使用VLM进行视觉-语言对齐，也匹配多模态理解与生成方向。

Importance reason: Strong robotics VLA paper but outside user's core continuous multimodal and video generation priorities. Signals: VLA training, fisheye domain adaptation, physical validation, UMI data, world model for manipulation
...
### [15] [COP-Q: Safety-First Reinforcement Learning for Robot Control via Cholesky-Ordered Projection](https://arxiv.org/abs/2606.04749)
...
TL;DR: COP-Q 提出了一种安全优先的强化学习方法，通过 Cholesky 分解将奖励与安全 Q 值之间的协方差信息纳入联合 Q 值估计，构建广义置信界，在保持安全保守性的同时减少奖励目标的过度保守，提升了样本效率。在 Brax 和 Safety-Gymnasium 上的机器人实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有离线安全强化学习方法通常独立处理奖励和安全 Q 值的不确定性，忽略了二者之间的相关性，导致对奖励目标的过度保守估计，降低了样本效率。本文旨在通过引入目标间协方差来缓解这一问题。

Method: 提出 Cholesky-Ordered Projection Q-learning (COP-Q)，将奖励和安全 Q 值建模为向量值 Q 函数，利用 Cholesky 分解对协方差矩阵进行分解，按目标优先级（安全优先）顺序投影，构建广义置信界，同时用于 TD 目标计算和 actor 优化。

Result: 在 Brax 机器人运动控制和 Safety-Gymnasium 安全导航任务上，COP-Q 在硬安全和软安全设置下均实现了强安全性能，同时相比基线方法具有竞争性或更优的样本效率。

Conclusion: COP-Q 通过引入目标间协方差信息，在安全优先的框架下自适应地减少奖励目标的过度保守性，是一种轻量级且兼容现有深度 Q 学习框架的安全强化学习方法。

Classification reason: 该论文提出COP-Q，一种面向机器人安全强化学习的Cholesky-Ordered Projection Q-learning方法，在机器人运动控制和安全导航场景中验证，属于具身智能中机器人规划与控制的范畴，因此映射到[具身世界模型与规划控制]这一规范子主题。

Importance reason: Focuses on safe RL for robotics, which is not a priority subtopic and lacks direct connection to user's core interests in continuous multimodal models and video generation. Signals: Safe RL, Cholesky decomposition, robot control, sample efficiency, safety constraints
...
### [16] [M3imic: Learning a Versatile Whole-Body Controller for Multimodal Motion Mimicking](https://arxiv.org/abs/2606.04829)
...
TL;DR: M3imic提出了一种通用人形机器人全身控制框架，通过多模态编码器将关节角度、人体姿态和末端执行器位姿等异构运动参考映射到共享潜空间，利用大规模强化学习训练单一策略，无需针对不同模态重新训练即可实现sim-to-real迁移，在Unitree G1机器人上达到98.42%的峰值成功率。


<details>
  <summary>Details</summary>
Motivation: 不同的人形机器人下游任务依赖不同的运动参考模态（如 locomotion 依赖关节轨迹，manipulation 依赖末端执行器轨迹），现有方法往往忽略这些异构运动参考之间的表征不匹配问题，需要针对每种模态单独训练策略，缺乏通用性。

Method: 提出Multi-Modal Mimic (M3imic)框架，包含：(1) 模态特定编码器，将机器人关节角度、人体姿态轨迹和末端执行器位姿三种异构运动参考映射到共享潜空间；(2) 在仿真器中通过大规模强化学习训练单一全身控制策略；(3) 实现sim-to-real迁移，无需针对不同模态重新训练。

Result: 在Unitree G1机器人仿真实验中，策略在未见过的测试数据集上达到98.42%的峰值成功率，展示了出色的泛化能力。在真实机器人上也成功实现了多模态运动模仿的sim-to-real迁移。

Conclusion: M3imic通过多模态统一表征和单一策略训练，有效解决了人形机器人不同任务间运动参考模态不匹配的问题，为构建通用全身控制器提供了可行方案，在仿真和真实实验中均验证了其有效性和泛化能力。

Classification reason: 该论文提出人形机器人全身控制框架M3imic，通过多模态运动参考（关节角度、人体姿态、末端执行器位姿）在共享潜空间统一建模，并利用大规模强化学习训练单一策略实现sim-to-real迁移。这属于面向机器人具身智能的动作规划与控制，与[具身世界模型与规划控制]（embodied_world_model）高度匹配。

Importance reason: Strong robotics control paper but outside user's priority subtopics and negative keywords. Signals: humanoid whole-body control, multimodal motion mimicking, sim-to-real transfer, reinforcement learning, Unitree G1 robot
...
### [17] [D$^3$-MoE:Dual Disentangled Diffusion Mixture-of-Experts for Style-Controllable End-to-End Autonomous Driving](https://arxiv.org/abs/2606.04884)
...
TL;DR: D³-MoE 提出了一种双解耦扩散混合专家框架，在行为轴上将轨迹生成与选择解耦（风格条件扩散生成多风格候选轨迹），在物理轴上将纵向与横向运动解耦（自监督路由激活对应专家），实现了风格可控的端到端自动驾驶规划，在 NAVSIM 基准上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统端到端自动驾驶框架在高方差人类演示数据上训练时，会陷入"风格平均化"困境，产生同质化、风格不可控甚至运动学不安全的策略。现有方法缺乏对驾驶风格（如激进/保守）的显式建模与控制能力。

Method: 提出 D³-MoE 框架，包含两个解耦轴：(1) 行为轴：将轨迹生成与选择解耦，使用风格条件扩散过程并行生成多风格候选轨迹，再由下游模块根据用户偏好或评分选择最优轨迹；(2) 物理轴：将纵向与横向运动解耦，通过自监督正交运动学信号训练两个路由器，推理时激活对应专家（DiT 架构，配备风格条件 AdaLN 和非对称横向融合交叉注意力），独立预测各自物理状态后重组为统一轨迹。还提出 Best-of-Three 集成策略扩展多模态解空间。

Result: 在 NAVSIM 基准上，默认设置达到 88.2 PDMS 和 84.3 EPDMS，Best-of-Three 集成策略提升至 91.3 PDMS 和 87.5 EPDMS，均达到 SOTA。定量和定性分析验证了规划质量和风格可控性的优势。

Conclusion: D³-MoE 通过双解耦设计有效解决了端到端自动驾驶中的风格平均化问题，实现了风格可控的高质量轨迹规划，为自动驾驶中的个性化驾驶策略提供了新思路。

Classification reason: 该论文提出端到端自动驾驶框架，使用扩散模型进行轨迹规划，属于具身智能中的机器人规划与控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong in world model and diffusion transformer for autonomous driving, but outside user's core priority subtopics. Signals: world model, diffusion transformer, style-controllable generation, end-to-end autonomous driving, mixture-of-experts
...
### [18] [WAM-Nav: Asymmetric Latent World-Action Modeling for Unified Visual Navigation](https://arxiv.org/abs/2606.04907)
...
Importance: 高优先级 (87.81) - 未入选精读

TL;DR: WAM-Nav 提出了一种非对称潜世界-动作模型，通过共享 Diffusion Transformer 联合扩散生成长期动作与短期视觉预测，实现统一视觉导航（Image-Goal、Point-Goal、No-Goal），在仿真和真实场景中均取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉导航方法中，直接映射观测到动作的反应式策略缺乏前瞻性推理，难以主动避障；而模块化方法将场景预测与策略学习分离，导致误差累积和推理效率低下。本文旨在联合学习动作生成与视觉预测，实现鲁棒且具有前瞻性的导航决策。

Method: WAM-Nav 使用共享的 Diffusion Transformer 进行非对称联合扩散，同时生成长时域动作和短时域视觉预测，减少多步自回归 rollout 带来的推理延迟和视觉误差累积。引入双流上下文条件机制，融合 episode 级自运动历史与序列视觉观测；统一目标对齐模块支持 Image-Goal、Point-Goal 和 No-Goal 三种导航模式。

Result: 在 ClutterScenes 和 InternScenes 基准上，Image-Goal 和 Point-Goal 导航成功率分别提升 15.7% 和 3.3%。真实场景零样本 sim-to-real 迁移在多种室内外环境中达到平均 85% 的任务成功率。

Conclusion: WAM-Nav 通过非对称潜世界-动作联合建模，有效结合了视觉预测的前瞻性与动作生成的效率，在多种导航任务和真实部署中展现了强大的泛化能力，为具身导航提供了一种统一且高效的范式。

Classification reason: 该论文提出 WAM-Nav，一个用于具身视觉导航的潜世界-动作模型（Latent World-Action Model），联合学习动作生成与潜视觉预测，属于面向机器人导航与规划控制的具身世界模型，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Proposes asymmetric latent world-action diffusion for unified visual navigation, aligning with world model and efficient inference priorities. Signals: world model, diffusion transformer, latent action-visual foresight, unified navigation, zero-shot sim-to-real
...
### [19] [Potential-Guided Flow Matching for Vision-Language-Action Policy Improvement](https://arxiv.org/abs/2606.04968)
...
TL;DR: ForesightFlow 提出一种解耦优势加权流匹配方法，使 VLA 策略在生成动作 chunk 的同时学习成功势能轨迹，实现无需外部 critic 的 best-of-K 推理，在仿真和真实机器人任务上提升成功率并降低训练计算量。


<details>
  <summary>Details</summary>
Motivation: 大规模 VLA 策略作为条件生成模型训练时，部署产生的混合质量经验（成功、部分完成、可恢复错误、失败）难以被标准模仿学习有效利用。完整行为克隆会模仿失败，过滤式 BC 丢弃有用子轨迹，离线强化学习则需要额外的大规模 critic 模型。

Method: 提出 ForesightFlow，一种自引导的 flow-matching 策略。核心创新包括：(1) 解耦优势加权流匹配——对动作速度施加指数化优势权重，对势能速度进行均匀训练，避免值幻觉；(2) 一步边界估计器——通过单次 stop-gradient 前向传播计算优势，无需迭代采样；(3) 自引导采样——同一 flow 同时生成候选动作并评分，实现 best-of-K 推理。

Result: 在 BEHAVIOR-1K 的 5 个仿真任务和 5 个真实世界双臂机器人任务上，ForesightFlow 优于模仿学习基线，在仿真成功率上与最强分离 critic 基线持平，在真实任务上提升成功率，并减少 38% 的训练计算量。消融实验表明解耦机制防止值幻觉，一步估计器保持候选排序忠实性，自引导采样改善长程执行效果。

Conclusion: ForesightFlow 通过解耦优势加权流匹配和自引导采样，有效提升了 VLA 策略在混合质量经验下的性能，无需外部 critic 即可实现策略改进，为机器人学习中的策略优化提供了高效且实用的新范式。

Classification reason: 该论文提出 ForesightFlow，一种自引导的 flow-matching 策略，用于视觉-语言-动作（VLA）策略改进，通过生成成功势能轨迹实现动作 chunk 的评分与选择，无需外部 critic。论文在 BEHAVIOR-1K 仿真任务和真实双臂机器人任务上验证，属于面向机器人动作规划与控制的世界模型方法，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Strong world model and flow matching method, but focused on robotics rather than core continuous language/multimodal or video generation priorities. Signals: flow matching for VLA, decoupled advantage weighting, best-of-K inference, world model for action, robotics policy improvement
...
### [20] [Generalization of World Models under Environmental Variability for Vision-based Quadrotor Navigation](https://arxiv.org/abs/2606.05015)
...
TL;DR: 本文系统研究了基于 DreamerV3 的世界模型在视觉四旋翼导航任务中面对环境变化的鲁棒性，通过跨环境验证和真实世界部署发现：SSL 预训练阶段的鲁棒性是 sim-to-real 迁移的强预测指标，而离散潜变量大小和训练序列长度是影响世界模型质量的关键因素。


<details>
  <summary>Details</summary>
Motivation: 世界模型在样本高效的机器人学习中展现出巨大潜力，但其对环境变化的鲁棒性尚未被系统研究。理解世界模型在环境变化下的泛化能力对于实际部署至关重要。

Method: 以视觉四旋翼导航为测试平台，训练基于 DreamerV3 的世界模型，在不同环境随机性水平下进行训练，并通过跨环境验证（涵盖 SSL 预训练和 RL 微调）评估模型。最后在真实四旋翼平台上部署所有世界模型及导航策略，包括一个仅接收 2.5 秒真实传感器输入后完全依靠想象导航 12 米的开环实验。

Result: SSL 预训练阶段的鲁棒性是 sim-to-real 迁移的强预测指标：所有在跨环境 SSL 验证中表现良好的模型均成功部署到真实世界（通过窄至 0.67m 的间隙），而在仿真策略评估中占优的模型在真实平台上失败。离散潜变量大小和训练序列长度是影响世界模型质量的主导因素。

Conclusion: 世界模型在 SSL 预训练阶段的跨环境鲁棒性比仿真策略性能更能预测真实世界部署的成功率，这为世界模型的评估和设计提供了重要指导。

Classification reason: 该论文研究基于 DreamerV3 的世界模型在视觉四旋翼导航中的泛化能力，涉及机器人具身控制、规划与策略学习，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on quadrotor navigation world models, not directly on user's priority subtopics like continuous language models or video generation. Signals: World model robustness, DreamerV3, SSL pretraining predictor, sim-to-real transfer, discrete latent size
...
### [21] [X4Val: Learning Neural Surrogates for Variance-Reduced Policy Evaluation](https://arxiv.org/abs/2606.05159)
...
TL;DR: X4Val 提出一种通用框架，通过将真实域与辅助域（仿真、历史日志等）样本嵌入共享表征空间，学习可迁移的真实世界指标预测器，并将其作为控制变量引入估计器，在无配对样本条件下实现方差缩减。在自动驾驶和机器人操作任务上最高实现 38.4% 的方差缩减。


<details>
  <summary>Details</summary>
Motivation: 真实世界机器人测试数据昂贵且有限，而仿真、历史策略日志等异构辅助数据虽然丰富但无法直接代表真实世界性能。现有方法难以在无配对样本的情况下利用这些异构数据实现高置信度的性能估计，因此需要一种能有效利用多域异构数据进行方差缩减的评估框架。

Method: X4Val 框架包含两个核心组件：(1) 一个共享表征空间嵌入器，将真实域和辅助域样本映射到同一表征空间；(2) 一个可迁移的真实世界指标预测器，学习从共享表征到真实世界性能的映射。该预测器作为控制变量（control variate）融入估计器，在无配对样本条件下实现方差缩减。论文还提供了理论分析证明其方差缩减性质。

Result: 在自动驾驶和真实机器人操作任务上，X4Val 相比强基线方法最高实现 38.4% 的方差缩减，并在多个实验设置中表现出一致的改进效果。

Conclusion: X4Val 证明了无配对的异构数据可以被有效利用来显著提升机器人系统验证的样本效率，为机器人策略评估提供了一种实用且理论完备的方差缩减框架。

Classification reason: 该论文关注机器人策略评估中的方差缩减方法，利用异构数据（仿真、历史策略日志等）来提升真实世界性能估计的样本效率，属于具身智能/机器人领域的策略评估与验证，与[具身世界模型与规划控制]方向紧密相关。

Importance reason: Focuses on robotic evaluation variance reduction, not on the user's priority subtopics like continuous language models or video generation. Signals: world model, variance reduction, control variates, autonomous driving, robot manipulation
...
### [22] [Dive into the Scene: Breaking the Perceptual Bottleneck in Vision-Language Decision Making via Focus Plan Generation](https://arxiv.org/abs/2606.04046)
*Boyuan Xiao, Bohong Chen, Yumeng Li, Ji Feng, Yao-Xiang Ding, Kun Zhou*

Main category: cs.RO, cs.LG, cs.CV, cs.CL, cs.AI
...
TL;DR: SceneDiver 提出一种粗到细的焦点规划方法，通过场景图构建和逐步子问题分解，缓解 VLM/VLA 在具身决策任务中的视觉幻觉，提升长程规划与反应控制能力。


<details>
  <summary>Details</summary>
Motivation: VLM 和 VLA 在具身决策任务中受限于相同的感知瓶颈：模型无法区分任务相关物体与干扰物，导致视觉幻觉。简单的一步注意力聚焦效果不佳，因为有效聚焦需要深度场景理解。

Method: 提出 SceneDiver，一种粗到细的焦点规划方法：先构建整体场景图建立初步理解，然后通过识别-理解-分析的迭代循环逐步将任务分解为更简单的子问题。同时设计轻量级适配器，将 VLM 的聚焦能力蒸馏到 VLA 中，实现反应控制。

Result: 在标准具身 AI 基准上，SceneDiver 显著减少了 VLM 和 VLA 的视觉幻觉，同时在需要快速执行的任务中保持了计算效率。

Conclusion: 粗到细的焦点规划能有效打破具身视觉-语言决策中的感知瓶颈，且通过蒸馏可将规划能力迁移到反应式控制模型中，兼顾性能与效率。

Classification reason: 论文聚焦具身视觉-语言决策任务（机器人操作与导航），提出粗到细的焦点规划方法缓解视觉幻觉，属于具身智能中的规划与控制，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Addresses visual hallucination in VLM/VLA for embodied decision making, relevant to world model and efficient inference priorities. Signals: world model, embodied decision making, visual hallucination, scene graph, coarse-to-fine focus
...
### [23] [Dual Advantage Fields](https://arxiv.org/abs/2606.04188)
...
TL;DR: 提出 Dual Advantage Fields (DAF)，一种将双线性对偶值模型转化为局部优势信号的策略提取方法，通过动作-效应模型预测特征位移与目标方向的对齐程度来评分动作，在 OGBench 的 locomotion、manipulation 和 puzzle 任务上取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 离线目标条件强化学习需要同时具备长程可达性估计和局部动作比较能力。现有的对偶目标表示方法能提供捕获全局目标可达性的值场，但无法直接指定在给定状态下应优先选择哪个动作。因此需要一种方法将对偶值模型转化为可指导局部动作选择的优势信号。

Method: DAF 利用双线性对偶参数化，其中目标嵌入是值场关于状态表示的梯度。学习一个动作-效应模型来预测动作引起的折扣特征位移，然后通过该位移与目标方向的对齐程度来评分动作。在可实现情况下，该分数等于目标条件 Bellman 优势，从而提供标准的局部策略改进保证。

Result: 在 OGBench 的 locomotion、manipulation 和 puzzle 任务上，DAF 在聚合 RLiable 指标上取得改进，并且在局部正确动作与直接朝向最终目标移动不同的场景中表现强劲。

Conclusion: DAF 有效地将对偶值模型转化为局部优势信号，在需要精细局部动作选择的场景中优于直接朝向目标移动的策略，为离线目标条件强化学习提供了一种新的策略提取范式。

Classification reason: 该论文提出 Dual Advantage Fields (DAF)，一种面向离线目标条件强化学习的策略提取方法，通过双线性对偶值模型生成局部优势信号，用于机器人动作规划与控制。这属于具身智能中世界模型驱动的规划控制范畴，与 canonical subtopic "具身世界模型与规划控制" 高度匹配。

Importance reason: Focuses on offline RL for robotics, not directly on the user's priority subtopics of continuous multimodal models or video generation. Signals: world model, offline RL, goal-conditioned, robotics, OGBench
...
TL;DR: 3DThinkVLA提出一种3D思维引导的协同训练框架，通过潜空间中的3D几何感知模块、在线3D推理蒸馏模块和空间增强动作集成模块，使VLA模型在动作预测时隐式进行3D空间推理，无需3D传感器、外部模型或显式文本生成，在LIBERO等基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型缺乏3D空间推理能力，通常依赖外部3D传感器或显式CoT文本生成，导致推理效率低、部署成本高。作者希望在不改变VLM主干架构、不引入额外传感器或显式文本生成的前提下，让VLA模型在动作预测中隐式地进行3D空间推理。

Method: 提出3D-thinking-guided co-training框架，包含三个紧密耦合的组件：(1) 潜空间3D几何感知模块，将中间视觉特征与3D基础模型对齐以获取几何先验；(2) 在线3D推理蒸馏模块，通过共享推理锚点token将3D VLM的显式推理蒸馏到VLA模型中；(3) 空间增强动作集成模块，将几何与推理特征联合注入动作查询token。部署时仅保留轻量适配器，丢弃3D基础模型和教师分支。

Result: 在LIBERO、LIBERO-PLUS、SimplerEnv和真实机器人操作任务上达到SOTA性能，且无需3D传感器、外部模型或显式文本生成，同时防止了预训练VLM的灾难性遗忘。

Conclusion: 通过将3D几何感知和空间推理解耦并在不同特征层级注入，可以在不增加部署负担的情况下显著提升VLA模型的3D空间推理能力，为具身智能中的高效3D推理提供了新范式。

Classification reason: 该论文提出3DThinkVLA框架，使VLA模型在动作预测中隐式进行3D空间推理，核心是面向机器人操作的具身智能与动作规划，与[具身世界模型与规划控制]高度匹配。同时涉及多模态理解与生成方向中的VLM/MLLM架构。

Importance reason: Strongly aligns with world model and continuous multimodal reasoning priorities via latent 3D priors and implicit spatial reasoning. Signals: world model, latent 3D reasoning, VLA co-training, implicit spatial priors, no 3D sensors at deployment
...
TL;DR: MIRAGE 是一个面向移动端 GUI 智能体的框架，它将显式思维链推理压缩为连续隐空间表示，并引入生成式世界模型目标，使智能体在行动前预测未来界面状态。在 AndroidWorld 和 AndroidControl 上，MIRAGE 以 3-5 倍更少的解码 token 达到了与显式 CoT 微调相当或更优的性能。


<details>
  <summary>Details</summary>
Motivation: 现有移动端 GUI 智能体通常依赖显式的长文本思维链进行推理，这导致交互速度慢、监督成本高且部署复杂。作者希望将显式推理压缩为紧凑的隐状态表示，使智能体能够内部推理而无需解码冗长的推理链，同时引入世界模型目标让智能体学会预测未来界面状态。

Method: MIRAGE 框架包含两个核心组件：(1) 从可见的文本推理轨迹中学习连续隐空间推理表示，将显式推理压缩为紧凑的隐藏状态；(2) 生成式世界模型目标：将隐推理向量与未来截图对齐，鼓励智能体在行动前预测即将到来的界面状态。推理时，MIRAGE 在连续隐空间中进行推理，大幅减少 token 生成量。

Result: 在 AndroidWorld 上，MIRAGE 在 4B 消融实验中以 3-5 倍更低的解码 token 预算匹配了显式 CoT 监督微调的性能，并将指令微调基线提升了 10.2 个点；在 AndroidControl 上，MIRAGE 在提升动作定位的同时生成了少于 75% 的 token。

Conclusion: MIRAGE 证明了将显式推理压缩为连续隐空间表示并结合生成式世界模型目标，可以在移动端 GUI 智能体任务中显著提升效率（减少 token 生成）的同时保持或提升性能，为高效部署移动端智能体提供了新思路。

Classification reason: 该论文提出 MIRAGE 框架，将显式推理压缩为连续隐空间表示，并引入生成式世界模型目标（将隐推理向量与未来截图对齐），使智能体在行动前预测界面状态变化。这属于面向移动端 GUI 智能体的具身世界模型与规划控制，因此映射到 [CANONICAL] 具身世界模型与规划控制。同时，其连续隐空间推理也涉及连续语言/多模态模型方向。

Importance reason: Directly advances continuous latent reasoning and world models for GUI agents, aligning with top-priority continuous language/multimodal models and world model subtopics. Signals: continuous latent reasoning, generative world model, mobile GUI agent, 3-5x fewer tokens, matches explicit CoT
...
TL;DR: DiffAero 是一个轻量级、GPU 加速且完全可微分的四旋翼无人机仿真框架，支持环境级和智能体级并行，集成多种动力学模型与传感器，通过全 GPU 并行化消除 CPU-GPU 传输瓶颈，可在消费级硬件上数小时内学习鲁棒飞行策略。


<details>
  <summary>Details</summary>
Motivation: 现有四旋翼仿真器在可微分性、GPU 并行化和训练效率方面存在不足，难以支持高效的控制策略学习，尤其是可微分与混合学习算法的探索。

Method: DiffAero 采用全 GPU 并行化设计，同时并行物理仿真与渲染，消除 CPU-GPU 数据传输瓶颈；支持环境级与智能体级双重并行；集成多种动力学模型和可定制传感器（IMU、深度相机、LiDAR）；提供统一的 GPU 原生训练接口，支持可微分与混合学习算法。

Result: 在多种飞行任务上的基准测试和真实世界飞行实验表明，DiffAero 相比现有仿真器实现了数量级的吞吐量提升，结合混合学习算法可在消费级硬件上数小时内学习鲁棒的飞行策略。

Conclusion: DiffAero 是一个高效、可微分的四旋翼仿真平台，为可微分与混合学习算法在机器人策略学习中的应用提供了有力支撑，有望加速无人机自主飞行策略的研究与部署。

Classification reason: 该论文提出四旋翼无人机控制策略学习的可微分仿真框架，属于机器人具身智能中的策略学习与规划控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on quadrotor simulation, not directly on the user's priority subtopics like continuous language models or video generation. Signals: GPU-accelerated differentiable simulation, quadrotor policy learning, world model application, not aligned with priority subtopics, no boost keywords matched
...
### [27] [OSCAR: Omni-Embodiment Skeleton-Conditioned World Action Model for Robotics](https://arxiv.org/abs/2606.04463)
*Zhuoyuan Wu, Jun Gao*
...
TL;DR: OSCAR 是一个动作条件视频世界模型，通过大规模标准化数据管道和 2D 运动学骨架渲染作为统一条件表征，在单张 GH200 GPU 上微调 Cosmos-Predict2.5-2B 模型，实现了跨机器人形态的泛化，并可用于虚拟策略评估。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型在机器人策略评估中面临三大挑战：训练数据场景多样性有限、动作跟随不精确、跨形态泛化能力差。作者希望构建一个精确的动作条件视频世界模型，能够泛化到不同机器人形态，并支持虚拟策略评估。

Method: 1) 构建大规模标准化数据管道，对多种机器人数据集和第一人称人类数据集进行整理、过滤和去重，得到统一的联合训练数据集。2) 采用 2D 运动学骨架渲染作为统一条件表征，泛化到不同机器人臂甚至人手。3) 在 Cosmos-Predict2.5-2B 模型上进行微调，使用单张 GH200 GPU。

Result: 在动作跟随、外观质量和运动一致性方面显著优于现有基线方法（这些方法要么模型更大，要么需要更多 GPU）。在 RoboArena 上的实验表明，OSCAR 中的虚拟策略评估与真实世界评估之间存在显著相关性。

Conclusion: OSCAR 展示了视频世界模型在机器人策略虚拟评估中的潜力，为未来完全在虚拟生成世界中进行机器人策略评估铺平了道路。

Classification reason: 该论文提出动作条件视频世界模型 OSCAR，面向机器人策略评估，属于视频世界模型方向。匹配 [CANONICAL] subtopic "视频世界模型" (video_world_model)。

Importance reason: Directly advances world model and video generation for robotics, aligning with top priority subtopics and boost keywords. Signals: world model, video world model, action-conditioned video generation, cross-embodiment generalization, efficient single-GPU finetuning
...
### [28] [Dream.exe: Can Video Generation Models Dream Executable Robot Manipulation?](https://arxiv.org/abs/2606.04811)
...
TL;DR: Dream.exe 提出一个视频生成模型评估框架，通过将生成的操作视频转化为机器人轨迹并在物理模拟器中执行，以执行成功率衡量模型对物理世界的理解。在101个操作任务上评估了8个模型，发现视觉质量与可执行性之间存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型评估仅依赖视觉质量指标，无法衡量模型是否真正理解物理世界。作者提出用机器人操作执行作为物理世界理解的"试金石"，通过生成视频能否转化为可执行机器人动作来评估模型对物理规律的掌握程度。

Method: 提出 Dream.exe 评估框架，包含三步流水线：(1) 给定场景图像和任务描述，视频生成模型生成操作视频；(2) 将生成视频中的运动转化为机器人轨迹；(3) 在物理模拟器中执行轨迹并评估成功率。覆盖101个手工筛选的操作任务，分三个物理复杂度等级，从视觉质量、轨迹保真度和执行成功率三个维度评估。

Result: 评估了8个模型（包括闭源、开源和机器人专用模型），发现部分模型确实能取得可测量的执行成功率，表明互联网数据训练的生成先验已编码了有意义的物理知识。但视觉质量与可执行性之间相关性很弱，说明标准视觉评估无法捕捉模型对物理世界的理解能力。

Conclusion: 视频生成模型从互联网数据中学到了一定的物理世界知识，但视觉质量不能反映物理可执行性，需要新的评估范式来度量生成模型对物理世界的理解。Dream.exe 提供了一个可行的评估框架。

Classification reason: 该论文提出用机器人操作执行成功率来评估视频生成模型对物理世界的建模能力，属于视频世界模型（video_world_model）的评测与应用方向。同时涉及文生视频模型（text_to_video_diffusion）作为被评估对象。

Importance reason: Directly evaluates video generation models as executable world models for robot manipulation, aligning with top-priority world model and video generation subtopics. Signals: world model evaluation, video-to-execution pipeline, physical grounding, visual quality vs executability gap, robot manipulation benchmark
...
TL;DR: 本文提供了一份3D视觉的"食谱"式综述，以数据为中心构建了涵盖几何表征、数据集、学习范式和应用的统一分类体系，系统分析了点云、网格、体素、3D高斯等表征及其获取流程，并探讨了2D监督3D学习、隐式神经表征和4D世界建模等前沿方向。


<details>
  <summary>Details</summary>
Motivation: 3D视觉领域在数据表征、学习范式和建模策略上日益多样化，但不同表征和基准之间仍然碎片化，缺乏统一的视角来理解效率、保真度和可扩展性之间的权衡。

Method: 采用数据驱动的分类法，将几何表征（点云、网格、体素、3D高斯）、数据集、学习框架和应用连接在一个统一的概念图中。首先分析3D数据的主要结构表征及其获取流程，然后考察数据集设计、基准构建和监督范式如何推动最新进展，涵盖2D监督3D学习、隐式神经表征和4D世界建模。

Result: 提供了一个整合的3D视觉分类体系，阐明了表征、学习范式和下游任务（重建、生成、视频建模）之间的关系，并总结了在效率与保真度之间取得平衡以及向多模态几何基础模型发展的趋势。

Conclusion: 该综述为3D视觉领域提供了系统性的整合视角，有助于研究者理解不同技术路线之间的联系，并为未来在效率-保真度权衡和多模态几何基础模型方向的研究提供了指导。

Classification reason: 该论文是3D视觉领域的综述，涵盖点云、网格、体素、3D高斯等几何表征，以及3D重建、生成和4D世界建模。核心主题属于3D生成与重建，同时涉及世界模型中的4D建模，因此匹配到 canonical subtopic "3D 生成与重建"。

Importance reason: A broad 3D vision survey with limited direct connection to your priority subtopics on continuous language/multimodal models and autoregressive generation. Signals: 3D vision survey, data-centric taxonomy, 2D-supervised 3D learning, implicit neural representations, 4D world modeling
...
### [30] [MeshFlow: Efficient Artistic Mesh Generation via MeshVAE and Flow-based Diffusion Transformer](https://arxiv.org/abs/2606.04621)
...
TL;DR: MeshFlow 提出了一种基于 VAE 和 Rectified Flow Transformer 的高效 3D 网格生成方法，将连续顶点位置和离散拓扑结构编码到紧凑的连续潜空间中，通过并行生成所有顶点和边，相比自回归方法实现 18 倍加速且精度更优。


<details>
  <summary>Details</summary>
Motivation: 现有自回归网格生成方法推理成本随网格大小呈二次增长，且需要对顶点坐标进行离散化引入量化误差。作者希望解决这些可扩展性和精度问题。

Method: 提出 MeshVAE，使用对比损失监督，将连续顶点位置和离散连接关系编码到紧凑的连续潜空间；然后基于 Rectified Flow Transformer 构建 3D 生成器，并行生成所有网格顶点和边。

Result: MeshFlow 比最快的自回归生成器快 18 倍，同时在标准网格生成指标上达到优异的精度。

Conclusion: MeshFlow 通过连续潜空间编码和并行流匹配生成，有效克服了自回归方法的可扩展性瓶颈和量化误差问题，实现了高效且高质量的 3D 网格生成。

Classification reason: 该论文提出 MeshFlow，一种基于 VAE 和 Rectified Flow Transformer 的 3D 网格生成方法，属于 3D 物体/场景生成范畴，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: While technically solid for 3D generation, it lacks direct connection to the user's priority subtopics like continuous language models or video generation. Signals: 3D mesh generation, VAE latent space, Rectified Flow transformer, efficient parallel generation, 18x speedup over AR
...
### [31] [MeshWeaver: Sparse-Voxel-Guided Surface Weaving for Autoregressive Mesh Generation](https://arxiv.org/abs/2606.04688)
...
TL;DR: MeshWeaver 提出了一种自回归网格生成框架，将网格生成视为表面编织过程，通过多层级稀疏体素编码器注入几何上下文，实现了高压缩比（18%）和高质量网格生成（最高16K面片）。


<details>
  <summary>Details</summary>
Motivation: 现有自回归网格生成方法存在两个根本局限：(i) token化效率低，导致长序列无法扩展到高面片网格；(ii) 缺乏几何感知引导，生成仅依赖全局形状嵌入而非局部表面线索。

Method: MeshWeaver 将网格生成视为表面编织过程，直接预测下一个顶点而非独立坐标。核心是多层级稀疏体素编码器，以三种互补方式注入几何上下文：(1) 提供体素特征作为顶点表示；(2) 通过交叉注意力引导 token 预测；(3) 作为结构支架约束生成围绕输入表面。层级化设计支持单解码步中的粗到细顶点预测。

Result: MeshWeaver 实现了18%的压缩比（SOTA），可生成多达16K面片的网格，几何保真度显著优于先前方法。

Conclusion: 通过稀疏体素引导的自回归表面编织方法，有效解决了现有自回归网格生成中 token 效率低和缺乏几何感知的问题，为高质量网格生成提供了新范式。

Classification reason: 该论文提出自回归网格（mesh）生成框架，属于3D物体生成范畴，与[CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Novel autoregressive mesh generation with sparse-voxel guidance, but outside user's priority subtopics. Signals: autoregressive mesh generation, high compression ratio, sparse-voxel encoder, surface weaving, 3D generation
...
### [32] [Recent Advances and Trends in Learning-based 3D Representations](https://arxiv.org/abs/2606.04871)
...
TL;DR: 本文系统综述了基于学习的3D表示方法，从离散显式格式（网格、点云、体素网格）到连续隐式场（神经辐射场、3D高斯泼溅等），分析了各类表示的公式、变体、优缺点及关键应用，并指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 3D表示的选择是现代计算机视觉与图形学管线中的基础设计决策，直接影响3D重建、新视角合成、形状分析、识别与生成等任务的效率和质量。随着神经和基元表示（如3DGS）的出现，传统表示面临新的挑战与机遇，亟需对各类3D表示进行系统梳理。

Method: 论文采用综述方法，按家族分类梳理3D表示：离散显式表示（网格、点云、体素网格）、连续隐式表示（神经辐射场NeRF、符号距离函数SDF、占用场）、基元泼溅表示（3D高斯泼溅3DGS及其变体）。对每类表示介绍其通用公式、变体、优缺点和关键应用。

Result: 论文系统总结了各类3D表示的特性对比，指出隐式表示（特别是神经隐式场和3DGS）正成为主流范式，改变了传统3D/4D工作流。同时识别了开放挑战：可编辑性、动态场景建模、大规模场景扩展、表示紧凑性与质量权衡等。

Conclusion: 3D表示正经历从离散显式向连续隐式的范式转变，NeRF和3DGS等隐式表示在质量、可微性和灵活性上具有显著优势，但仍面临编辑、动态建模和规模化等挑战，未来方向包括混合表示、4D表示和更高效的表示学习。

Classification reason: 该论文系统综述了基于学习的3D表示方法（包括NeRF、3DGS、隐式场等），属于3D生成与重建领域的核心基础主题，与[CANONICAL] subtopic "3D 生成与重建"高度匹配。

Importance reason: Comprehensive survey of 3D representations, but only tangentially touches on user's priority subtopics like continuous multimodal models or video generation. Signals: 3D representation survey, implicit fields, neural rendering, 3D Gaussian Splatting, paradigm shift
...
### [33] [SymTRELLIS: Symmetry-Enforced Voxel Latents for 3D Generation](https://arxiv.org/abs/2606.04108)
*Guangda Ji, Qimin Chen, Qinchan Li, Mingrui Zhao, Kai Wang, Hao Zhang*

Main category: cs.LG, cs.AI, cs.CV, cs.GR
...
TL;DR: SymTRELLIS 提出一种无需重新训练 VAE 或流模型的对称性增强方法，通过在体素潜空间中学习空间变换的线性近似算子，并在 ODE 采样时对预测速度进行对称化平均，从而在 TRELLIS.2 的流匹配 3D 生成中强制执行任意有限点群对称性（旋转、反射、多面体群），显著降低对称性误差。


<details>
  <summary>Details</summary>
Motivation: 单视图 3D 生成模型虽视觉质量高，但常产生对称性缺陷（即使很细微），导致生成的模型在物理上不可用。现有方法无法满足结构或功能上的对称性要求，因此需要一种能在不重新训练基础模型的前提下强制执行对称性的方法。

Method: 核心思想是将空间变换在潜空间中的作用近似为体素潜空间上的线性算子，通过一个轻量级的空间变换潜映射器（spatial-transform latent mapper）在通用非对称 3D 数据上训练得到。生成时，在每个 ODE 步骤中对所有对称等价变换下的预测流速度进行平均（velocity symmetrization），从而强制执行对称性。对称性规格可从初始 TRELLIS.2 生成结果自动估计或由用户指定。

Result: 在包含 266 个严格对称物体（涵盖 2 到 20 折旋转及多面体对称群）的基准上，SymTRELLIS 相比 TRELLIS.2、Hunyuan3D-2.1 和 TripoSG 大幅降低了所有对称性误差指标，同时保持了与基础模型相当的重建精度。

Conclusion: SymTRELLIS 证明了在流匹配 3D 生成中通过潜空间速度对称化可以有效强制执行任意有限点群对称性，无需重新训练基础生成模型，为结构约束下的 3D 生成提供了实用且高效的解决方案。

Classification reason: 该论文提出 SymTRELLIS，在 TRELLIS.2 流匹配 3D 生成框架中通过对称性约束增强生成质量，属于 3D 物体/场景生成方法，因此映射到 canonical 子主题 "3D 生成与重建"。

Importance reason: Novel symmetry enforcement for 3D generation, but outside user's priority subtopics and negative for 3D generation. Signals: 3D generation, symmetry enforcement, flow matching, TRELLIS, no retraining
...
### [34] [4D Reconstruction from Sparse Dynamic Cameras](https://arxiv.org/abs/2606.04593)
...
TL;DR: 本文提出一种从稀疏动态相机（多台独立移动相机）进行4D重建的方法，通过3D轨迹初始化、深度排序正则化和时空多样批次采样策略，解决多视角间时空不一致问题，并发布了包含5个序列的LetCamsGo真实世界数据集。


<details>
  <summary>Details</summary>
Motivation: 单目动态相机4D重建受深度模糊限制，而密集固定相机阵列成本高昂。稀疏动态相机（少量独立移动相机）在保持低成本的同时引入多视角约束，但现有方法无法解决跨视角和跨时间的复杂时空不一致问题，且缺乏标准化评测基准。

Method: 提出一个4D重建框架：(1) 3D轨迹初始化方法，通过跨相机特征匹配与单相机内点跟踪相结合确保时空一致性；(2) 噪声鲁棒的深度排序正则化损失；(3) 时空多样批次采样策略以增强优化稳定性和跨视角泛化能力。同时构建了LetCamsGo数据集（5个序列，4种环境，3台移动相机+1台固定相机）。

Result: 在LetCamsGo数据集上的综合评测表明，所提框架在动态区域的4D重建质量上优于基线方法。

Conclusion: 本文验证了稀疏动态相机设置用于低成本野外4D重建的可行性，提出的方法有效解决了多视角时空不一致问题，为实际视频制作场景（体育、音乐会、电视节目）中的4D重建提供了实用范式。

Classification reason: 该论文研究从稀疏动态相机进行动态3D（4D）重建，属于动态场景重建与生成范畴，与 [CANONICAL] subtopic "4D 与动态场景生成"（dynamic_4d_generation）高度匹配。

Importance reason: Focuses on 4D reconstruction from sparse dynamic cameras, which is not a priority subtopic and lacks strong alignment with user's configured direction weights and boost keywords. Signals: 4D reconstruction, sparse dynamic cameras, spatiotemporal consistency, LetCamsGo dataset, not in priority subtopics
...
### [35] [Controllable Dynamic 3D Shape Generation via 3D Trajectories and Text](https://arxiv.org/abs/2606.05162)
...
Importance: 值得关注 (73.84) - 未入选精读

TL;DR: T2Mo 是一个前馈式可控动态3D形状生成框架，以3D轨迹和文本为条件，通过形状锚定的轨迹嵌入处理任意配置的轨迹输入，生成空间上遵循轨迹、语义上反映文本的动态3D形状。


<details>
  <summary>Details</summary>
Motivation: 仅用文本描述动态3D形状的运动存在语言固有的歧义性，难以精确指定物体的运动路径和空间行为。现有方法要么仅依赖文本，要么采用级联式视频生成+视频到动态网格的流程，缺乏直接、可控的3D运动生成方式。

Method: T2Mo 采用前馈式架构，以3D轨迹集合和文本描述作为条件输入。提出形状锚定的轨迹嵌入（shape-grounded trajectory embedding），将任意配置（密集/稀疏/不均匀分布）的轨迹点映射为覆盖整个物体的形状感知token集，从而鲁棒地处理轨迹输入。结合文本语义与轨迹空间引导，生成动态3D形状。

Result: 与基于文本的基线方法和级联式视频基线方法相比，T2Mo 在定量和定性评估以及用户研究中均表现出更优的运动忠实度、表达力和运动质量，能更准确地遵循给定的轨迹和文本提示。

Conclusion: T2Mo 通过结合3D轨迹的空间可控性和文本的语义丰富性，实现了更精确、更可控的动态3D形状生成，为动态3D内容创作提供了新的有效范式。

Classification reason: 该论文研究基于3D轨迹和文本条件的可控动态3D形状生成，属于动态3D/4D场景生成范畴，与[CANONICAL] subtopic "4D 与动态场景生成" (dynamic_4d_generation) 高度匹配。

Importance reason: Novel controllable dynamic 3D generation with trajectory+text conditioning, but tangential to user's core focus on continuous language/multimodal models and autoregressive generation. Signals: dynamic 3D shape generation, controllable motion via 3D trajectories, feed-forward framework, shape-grounded trajectory embedding, text+trajectory conditioning
...
### [36] [Crafting Your Evolving Dreams: Concept-Incremental Versatile Customization](https://arxiv.org/abs/2606.04797)
...
TL;DR: 本文提出 Continually Customizable Diffusion Model (CCDM)，解决扩散模型在增量学习新个性化概念时的灾难性遗忘和概念忽视问题。通过属性解耦 LoRA (AD-LoRA) 模块、相关性引导的 AD-LoRA 聚合策略以及可控区域上下文合成策略，实现了概念增量式的持续定制生成。


<details>
  <summary>Details</summary>
Motivation: 现有定制扩散模型（CDM）假设用户收集的个性化概念集合是静态的，无法随时间增量增长。当顺序学习新概念时，模型会严重遗忘先前学过的概念（灾难性遗忘）或忽视已有概念（概念忽视）。

Method: 1) 属性解耦 LoRA (AD-LoRA) 模块：为每个任务解耦并保留概念特定属性；2) 相关性引导的 AD-LoRA 聚合策略：利用任务间有益相关性增强持续学习；3) 可控区域上下文合成策略：在用户指定条件下进行多概念组合合成，保证区域语义独立性和边界平滑过渡。

Result: 实验表明 CCDM 在概念增量定制任务上显著优于基线方法，有效缓解了灾难性遗忘和概念忽视问题。

Conclusion: 本文提出的 CCDM 框架首次将持续学习引入个性化概念定制扩散模型，通过属性解耦和相关性聚合策略实现了概念增量式定制生成，为动态增长的个性化概念集合提供了可行的解决方案。

Classification reason: 该论文研究个性化概念定制扩散模型（Custom Diffusion Models），提出持续学习框架实现概念增量定制，属于主体与概念驱动生成（customization_generation）这一规范主题。

Importance reason: Addresses concept-incremental customization in diffusion models, but lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: concept-incremental customization, diffusion model, catastrophic forgetting, AD-LoRA, multi-concept synthesis
...
### [37] [Reflection Separation from a Single Image via Joint Latent Diffusion](https://arxiv.org/abs/2606.04107)
...
TL;DR: 本文提出一种基于联合潜扩散模型的单图像反射分离方法，通过统一扩散模型同时生成透射层和反射层，引入跨层自注意力机制进行特征解耦，并采用不相交采样策略和潜空间优化步骤，在强光/弱反射等极端条件下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 单图像反射分离在强光或弱反射等极端条件下极具挑战性，现有方法因信息不足难以同时恢复两个图层。作者希望利用扩散模型的生成先验来鲁棒地分离反射和透射层。

Method: 提出一个联合潜扩散模型，同时生成透射层和反射层。核心设计包括：(1) 跨层自注意力机制（cross-layer self-attention），让两个图层在扩散过程中相互交互以更好地解耦特征；(2) 不相交采样策略（disjoint sampling），在扩散迭代中交替减少层间干扰；(3) 潜空间优化步骤，利用学习到的组合函数在复杂真实场景中进一步优化结果。

Result: 在多个真实世界基准数据集上，该方法在定量指标（PSNR、SSIM等）和定性视觉效果上均超越了当前最先进的反射分离方法，尤其在强光和弱反射场景下优势明显。

Conclusion: 本文证明了扩散模型结合精心设计的解耦机制（跨层注意力、不相交采样、潜空间优化）能够有效解决单图像反射分离这一病态问题，为图像恢复任务提供了新的范式。

Classification reason: 该论文研究单图像反射分离（reflection separation），属于图像编辑/恢复任务，与 canonical subtopic "图像与视频编辑"（visual_editing）高度相关。

Importance reason: This paper focuses on a narrow image editing task (reflection separation) with no direct connection to the user's priority subtopics like continuous language models, video generation, or world models. Signals: Single-image reflection separation, Joint latent diffusion, Cross-layer self-attention, Disjoint sampling, Weak reflection handling
...
TL;DR: MetaPoint 提出将连续2D坐标编码为单个特殊token，无需修改模型架构即可实现像素级空间控制，支持物体位置、边界框的精确定位，并可作为空间原语组合用于智能体驱动的生成与交互式编辑。


<details>
  <summary>Details</summary>
Motivation: 现有生成视觉模型难以实现精确的空间控制，因为模型能处理文本描述但无法直接映射数值坐标到2D图像画布上。作者希望弥合这一鸿沟，提供一种轻量级、无需架构修改的精确空间控制方法。

Method: MetaPoint 将连续2D坐标表示为单个特殊token，利用模型固有的位置编码机制将该token解释为画布上的虚拟点。仅需一个token控制物体位置，两个token控制边界框，无需架构修改或注意力掩码。MetaPoint token 可组合作为空间原语，由规划智能体将高层用户请求分解为结构化原语序列供生成器执行。

Result: MetaPoint 实现了像素级精确的空间控制，支持物体位置定位和边界框约束生成，并可用于构建组合式生成智能体和交互式编辑系统。

Conclusion: MetaPoint 提供了一种简单、精确且可扩展的空间控制基础组件，解锁了更强大的组合式生成智能体能力，并支持直观的交互式编辑系统。

Classification reason: 该论文提出MetaPoint，通过将连续2D坐标表示为特殊token来实现精确空间控制，支持像素级位置控制、边界框控制，并可用于交互式编辑系统。这与[CANONICAL] subtopic "visual_editing"（图像与视频编辑）中的可控编辑方向高度吻合。同时该方法也涉及多模态理解与生成中的视觉生成可控性。

Importance reason: Introduces a novel continuous spatial token for precise control, aligning with continuous multimodal representation priorities. Signals: continuous coordinate token, spatial control, agentic generation, no architecture change, compositional primitives
...
### [39] [GeM-NR: Geometry-Aware Multi-View Editing for Nonrigid Scene Changes](https://arxiv.org/abs/2606.05142)
...
TL;DR: GeM-NR 是一种无需训练的、快速灵活的多视图一致图像编辑方法，专门处理非刚性（几何和外观大幅变化）的场景编辑。通过深度图对齐、视角投影和条件细化三个阶段，在保持多视图一致性的同时支持大幅几何变形编辑，在多种编辑任务上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有多视图编辑方法大多局限于刚性变换或仅外观编辑，无法处理大幅改变场景几何结构的非刚性编辑（如物体变形、姿态变化等）。尽管已有一些方法针对特定编辑任务（如物体移除/添加），但通用的非刚性多视图一致编辑仍然是一个开放挑战。

Method: GeM-NR 包含三个阶段：(i) 深度图估计与对齐——通过最大化编辑后场景与未编辑场景的 3D 点云对齐来优化深度图；(ii) 将编辑后的锚点图像投影到查询视角；(iii) 以未编辑的查询图像为条件对投影结果进行细化。该方法基于预训练编辑器（如 FLUX、Qwen、BrushNet）编辑锚点图像，然后通过上述流程将编辑传播到其他视图，无需额外训练。

Result: 在多种编辑任务（包括大幅几何和外观变化）上，GeM-NR 在编辑质量、几何一致性和光度一致性方面均达到 SOTA。该方法支持从两视图扩展到多视图，并能生成编辑后场景的 3D 表示。

Conclusion: GeM-NR 有效解决了非刚性多视图一致编辑这一难题，通过训练自由的流水线实现了大幅几何变形场景下的高质量多视图编辑，为 3D 内容生成和定制提供了新的可能性。

Classification reason: 该论文提出多视图一致的非刚性场景编辑方法，属于多视图图像编辑任务，与 [CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度匹配。

Importance reason: Focuses on multi-view editing for nonrigid scenes, which is not a priority subtopic and lacks direct alignment with user's key directions like continuous language models or video generation. Signals: Training-free multi-view editing, Nonrigid scene geometry changes, Depth map alignment for consistency, Conditioning-based refinement, SOTA on geometric and photometric consistency
...
### [40] [DiverAge: Reliable Pluralistic Face Aging with Cross-Age Identity Relation Guidance](https://arxiv.org/abs/2606.04881)
...
TL;DR: DiverAge 提出一种基于扩散自编码的分层式多模态人脸老化框架，通过跨年龄身份关系引导（CARR）在保持外观多样性的同时提升老化序列的序级可靠性。


<details>
  <summary>Details</summary>
Motivation: 人脸老化本质上是一对多的生成问题（同一人在目标年龄可能有多种外观），现有确定性方法缺乏随机多样性，而多样性方法又未能显式约束完整老化序列的身份演化，导致序列级可靠性不足。

Method: 基于扩散自编码的分层框架：1）通过随机扩散解码和年龄条件语义调制保持外观级多样性；2）提出跨年龄身份关系调节器（CARR），利用从真实同身份跨年龄对估计的跨年龄身份相似度（CIS）先验，在推理时通过单侧采样引导联合去噪多个目标年龄组，无需修改训练目标或引入额外可训练参数。

Result: 实验表明 DiverAge 在提升序列级序级可靠性的同时，保持了身份保持、年龄准确度、图像质量和外观级多样性。

Conclusion: DiverAge 通过推理时的跨年龄身份关系引导，在不增加训练复杂度的情况下有效平衡了人脸老化的多样性与可靠性，为可靠的多模态人脸老化生成提供了新思路。

Classification reason: 该论文研究人脸老化生成（face aging），属于数字人/肖像生成范畴，与 [CANONICAL] 子主题 "数字人与动作生成"（avatar_motion_generation）中的人脸/肖像相关生成任务高度匹配。

Importance reason: Face aging is a niche subtask not aligned with user's priority subtopics like continuous multimodal models or video generation. Signals: face aging, diffusion autoencoding, identity preservation, cross-age guidance, not in priority subtopics
...
TL;DR: 本文提出 Visual Prompt Engineering (VPE)，在统一内部架构中先自回归生成视觉语义 token（如 SigLIP 2）作为"视觉提示"捕获语义布局，再基于该提示生成完整图像，从而弥合文本到像素的建模鸿沟。VPE 可无缝集成到 Transfusion、BAGEL 等内部框架中，在类别条件生成、文生图和图像编辑任务上加速收敛、提升质量上限，并在编辑保真度上显著优于外部两阶段方案（PSNR: 26.76 vs. 19.92）。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成方法面临文本到像素的建模鸿沟：外部两阶段方案（如 X-Omni）使用独立自回归模型生成语义 token 再送入扩散解码器，但解码器无法同时访问原始输入和语义计划，导致信息瓶颈和细节丢失；内部统一架构（如 Transfusion、BAGEL）虽能跨模态交互，但缺乏中间语义引导，仍面临困难的文本到像素建模。作者希望在不牺牲内部架构优势的前提下，引入中间视觉语义引导来降低建模难度。

Method: VPE 的核心思想是在统一内部模型中引入"视觉提示"作为中间语义表示。具体流程：(1) 模型先自回归生成视觉语义 token（如 SigLIP 2 特征），这些 token 作为"视觉提示"捕获图像的语义布局；(2) 然后基于该视觉提示生成完整的图像 token。VPE 可无缝集成到 Transfusion、BAGEL、Show-o2 等内部架构中，无需外部独立模块。实验在类别条件生成、文生图和图像编辑三种任务上验证，覆盖多种 token 类型和模型架构。

Result: VPE 在多个任务上取得显著提升：加速模型收敛、提高生成质量上限；在图像编辑任务中，VPE 的编辑保真度（PSNR: 26.76）远超同参数规模的外部两阶段方案（PSNR: 19.92），同时保持有竞争力的编辑响应能力。实验表明视觉提示作为中间语义引导能有效弥合文本到像素的建模鸿沟。

Conclusion: VPE 通过引入可内部集成的视觉语义提示，有效解决了图像生成中文本到像素的建模困难，在保持内部架构优势的同时提升了生成质量和编辑保真度。该方法为统一多模态生成模型的设计提供了新的思路，即通过中间视觉语义表示来桥接文本与像素之间的语义鸿沟。

Classification reason: 该论文提出 Visual Prompt Engineering (VPE)，在图像生成（包括文生图、类别条件生成、图像编辑）中先自回归生成视觉语义 token 作为中间视觉提示，再生成完整图像。核心贡献在图像生成方法上，因此映射到 [CANONICAL] 文生图与图像扩散 (text_to_image_diffusion)。同时该方法涉及理解与生成一体化思想，也匹配多模态理解与生成方向。

Importance reason: Directly advances continuous multimodal representation and autoregressive generation with a novel visual prompt engineering method. Signals: continuous visual token, autoregressive generation, visual semantic token, internal architecture, image editing
...
### [42] [Echo-Infinity: Learning Evolving Memory for Real-Time Infinite Video Generation](https://arxiv.org/abs/2606.04527)
...
TL;DR: Echo-Infinity 提出了一种基于可学习演化记忆的自回归框架，用于实时无限视频生成。它通过可学习的 Memory Query 替代手工设计的 KV-cache 管理策略，结合门控机制和注意力更新，实现对任意长度历史信息的动态过滤与压缩，计算量恒定。同时引入 Unified Relative RoPE Recipe，解除有限 RoPE 约束并消除训练-测试外推差距。在长/短视频生成上达到 SOTA，首次展示了 24 小时（>130 万帧）实时 rollout。


<details>
  <summary>Details</summary>
Motivation: 现有无限视频生成方法依赖预定义的 KV-cache 调度、固定比例启发式压缩或推理时 RoPE 适配，这些手工设计因缓存窗口有限且忽略自回归生成噪声，不可避免地丢失历史信息并放大累积误差。受人类记忆巩固机制启发，作者希望用可学习的演化记忆替代手工记忆管理，实现任意压缩比、计算量恒定的长视频生成。

Method: 1) 可学习演化记忆（Evolving Memory）：设计可学习的 Memory Query，当过去帧从局部窗口被驱逐时，通过注意力机制和门控机制更新这些查询，形成端到端优化的演化记忆，支持任意压缩比且计算量与视频长度无关。2) Unified Relative RoPE Recipe：将 sink 帧锚定在 id 0，最新帧 id 最多增长到 DiT 预训练的最大时间 RoPE id，从而解除有限 RoPE 约束，消除训练-测试 RoPE 外推差距。3) 整体框架为自回归 AR 架构，结合视频扩散 Transformer (DiT) 进行逐帧生成。

Result: 在长视频和短视频生成任务上均达到 SOTA 性能。首次展示了 24 小时（超过 130 万帧）的实时 rollout，证明了通向无限视频生成的实用路径。

Conclusion: Echo-Infinity 通过可学习演化记忆和 Unified Relative RoPE Recipe，有效解决了无限视频生成中的历史信息丢失和累积误差问题，实现了计算量恒定的任意长度视频生成，为实时无限视频生成提供了可行方案。

Classification reason: 该论文提出 Echo-Infinity，一个基于自回归框架和视频扩散 Transformer (DiT) 的无限长视频生成方法，核心贡献在于长视频生成中的记忆管理和 RoPE 扩展。这属于多模态生成方向下的文生视频与视频扩散（text_to_video_diffusion）范畴，该 canonical subtopic 涵盖视频扩散模型和视频 DiT 方法。

Importance reason: Directly advances continuous autoregressive video generation with a learnable memory mechanism, aligning with top priority subtopics and boost keywords. Signals: continuous language model, autoregressive video generation, video generation, world model, efficient inference
...
TL;DR: 本文提出 PILA（Physics-Informed Latent Alignment），一种向预训练视频生成模型的冻结 flow-matching 动力学中注入物理结构化潜空间引导的框架。通过锚定场估计、混合专家路由和物理属性库构建，PILA 在不破坏预训练视觉先验的前提下提升生成视频的物理合理性，在 VBench-2.0、VideoPhy-2 和 PhyGenBench 上取得 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视频生成模型在语义一致性和视觉质量上表现优异，但其基于像素级拟合的动力学无法自然符合真实世界的运动与交互规律，导致生成的视频在物理合理性上存在持续缺陷。作者旨在解决这一关键问题，即如何在不破坏预训练模型视觉先验的前提下，注入物理知识以提升生成视频的物理可信度。

Method: PILA 包含三个核心模块：(1) 锚定场估计（Anchored Field Estimation），将冻结生成器的潜变量映射到由场代理槽组织的可操作物理属性库，利用可观测运动作为运动学锚点来构建难以直接观测的代理量；(2) 混合专家设计（Mixture-of-Experts），通过标签先验掩码专家路由选择类别特定的算子专家，并用从物理关系中抽象出的操作残差进行正则化；(3) 物理属性融合与校正，将精炼后的代理量融合回物理属性库并解码为对 flow-matching 向量场的校正，从而注入物理感知引导。采用分阶段适配器训练，在 Wan 2.1-1.3B 上训练后直接迁移到 Wan 2.2-14B。

Result: 在 VBench-2.0、VideoPhy-2 和 PhyGenBench 三个基准上，PILA 在视觉质量和物理合理性两方面均取得了 SOTA 结果。适配器可直接从 Wan 2.1-1.3B 迁移到 Wan 2.2-14B，展示了良好的可扩展性。

Conclusion: PILA 通过物理信息潜空间对齐，有效提升了预训练视频生成模型的物理合理性，同时保持了其视觉先验。混合专家设计和锚定场估计为处理真实世界动态的异质性提供了可行方案，且适配器的可迁移性表明该方法具有良好的泛化能力。

Classification reason: 该论文提出 PILA 框架，在预训练视频生成模型（Wan 2.1/2.2）的 flow-matching 动力学中注入物理信息引导，属于视频生成方法改进。最匹配的 canonical 子主题是 "文生视频与视频扩散"（text_to_video_diffusion），因为论文核心是视频生成模型的物理合理性增强，且 backbone 为视频扩散/flow-matching 模型。同时论文涉及物理世界建模，也可匹配 world_model 方向下的 "可交互与物理世界建模"。

Importance reason: Directly addresses physical plausibility in video generation via a novel physics-informed latent alignment framework, highly relevant to your priority on video generation world models and diffusion video generation. Signals: physics-informed video generation, mixture-of-experts latent alignment, flow-matching dynamics, physical plausibility, state-of-the-art on VBench-2.0, VideoPhy-2, PhyGenBench
...
### [44] [Optimal Transport Flow Matching by Design](https://arxiv.org/abs/2606.04092)
...
Importance: 高优先级 (93.94) - 未入选精读

TL;DR: 本文提出将先验分布视为设计选择而非固定输入，通过选择数据低频投影作为先验，实现与数据的最优传输恒等耦合，使流匹配轨迹更直（曲率降低2倍以上），从而支持少步/单步高质量生成，无需修改流模型本身。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配模型依赖最优传输（OT）耦合来获得直线轨迹以实现快速生成，但高维空间中计算OT耦合困难，现有方法存在偏差或高开销。作者希望在不求解OT问题的情况下，通过重新设计先验分布来自然获得OT最优耦合。

Method: 将先验分布视为设计选择，选择自然图像的低频投影（如DCT低通滤波）作为先验。该先验与数据之间存在恒等映射，经验上满足OT最优性。推理时用轻量模型（如小扩散模型）从噪声生成低频先验，再通过流匹配模型从低频先验生成完整图像。还可将先验与高斯噪声插值以进一步提升质量。

Result: 在多个基准上，该方法相比现有流匹配方法将轨迹曲率降低2倍以上，在少步生成场景下取得更优的生成质量，且与潜空间模型、无分类器引导和单步生成框架自然兼容。

Conclusion: 通过将先验作为设计选择而非固定输入，可以绕过高维OT求解的困难，获得更直的流匹配轨迹，实现高效生成。该方法简单有效，无需修改流模型架构，具有良好的通用性。

Classification reason: 该论文提出通过最优传输（OT）耦合设计先验分布，使流匹配（flow matching）轨迹更直，实现少步甚至单步生成，属于生成模型加速与高效化的核心范畴，因此映射到 [CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Directly advances continuous multimodal generation via flow matching with optimal transport, aligning with top priority subtopics. Signals: flow matching, optimal transport, continuous language model, efficient inference, generative model acceleration
...
### [45] [Efficient and Training-Free Single-Image Diffusion Models](https://arxiv.org/abs/2606.04299)
...
TL;DR: 本文提出一种无需训练的单图像扩散模型，通过构建多尺度patch数据集并利用闭式最优去噪器替代神经网络训练，实现高效的单图像生成，支持无条件生成、文本引导风格化、图像对称化等应用，可在1秒内完成百万像素级生成。


<details>
  <summary>Details</summary>
Motivation: 现有的单图像扩散模型需要针对每张图像从头训练一个神经网络，计算成本高昂（数小时优化）。本文旨在消除训练需求，实现高效、无需训练的单图像生成。

Method: 将单张图像在不同尺度上提取的patch集合作为数据集，利用patch维度低、数据集有限的特点，推导出闭式最优去噪器（closed-form denoiser）来计算噪声patch的score function，从而无需神经网络训练。将该patch去噪器集成到扩散模型中，并支持潜空间扩散和多种加速技术。

Result: 在生成质量和多样性上达到SOTA，超越需要训练的单图像扩散模型；支持百万像素级图像在1秒内生成，十亿像素级在数分钟内生成。

Conclusion: 本文证明了无需神经网络的单图像扩散模型是可行的，且效率远超训练式方法，同时与经典patch-based图像复原技术建立了联系，为高效图像生成开辟了新路径。

Classification reason: 该论文提出一种无需训练的单图像扩散模型，利用多尺度patch数据集和闭式最优去噪器替代神经网络训练，大幅提升生成效率（毫秒级/秒级生成）。核心贡献在于生成模型的训练-free加速与高效化，因此映射到[CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Proposes a training-free single-image diffusion method with strong efficiency, but lacks direct connection to priority subtopics like continuous models or video generation. Signals: training-free diffusion, single-image generation, efficient inference, patch-based denoising, closed-form denoiser
...
### [46] [DSA: Dynamic Step Allocation for Fast Autoregressive Video Generation](https://arxiv.org/abs/2606.04432)
...
TL;DR: DSA提出一种置信度引导的自适应计算框架，为自回归视频扩散模型动态分配每帧的去噪步数，简单帧提前终止、复杂帧额外细化，在H100上实现22.63 FPS的实时视频生成，同时保持与双向扩散模型相当的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归视频扩散模型虽然通过蒸馏降低了延迟，但仍为每一帧使用固定的去噪步数，导致在可预测的简单帧上浪费计算资源，而在复杂帧上细化不足。作者希望实现一种自适应计算分配方法，在保证生成质量的同时最大化推理效率。

Method: DSA在蒸馏后的自回归视频扩散模型上附加一个轻量级置信度头（confidence head），与生成器在分布匹配蒸馏（DMD）目标下联合训练，用于估计每帧去噪的可靠性。推理时，根据置信度信号动态调整每帧的扩散步数：置信度高的简单帧提前终止，置信度低的复杂帧获得更多细化步数。该方法无需额外视频数据、无需启发式规则，且对架构改动极小。

Result: 在H100 GPU上，DSA实现了22.63 FPS的实时自回归视频生成，延迟低于1秒。在VBench评测中，DSA在保持竞争性或更优的视觉质量的同时，显著优于现有的自回归和双向视频扩散模型。

Conclusion: 置信度引导的自适应采样为交互式视频生成提供了一条有效且实用的路径，能够在实时推理速度下保持高质量的视频生成效果。

Classification reason: 该论文提出DSA动态步数分配框架，通过置信度引导的自适应采样加速自回归视频扩散模型，属于生成模型加速与高效化（efficient_generation）这一[CANONICAL]子主题。
...
TL;DR: 本文提出 CAPR（Cached-Amortized Path Refinement），一种针对扩散语言模型（dLLM）的强化学习算法。CAPR 通过缓存去噪轨迹状态、压缩路径状态并训练块级价值头，将稀疏的最终奖励重新分配到各个去噪块上，从而在不进行完整树展开的情况下获得类似树搜索的细粒度监督信号。在 Sudoku、Countdown、GSM8K 和 Math500 等任务上，CAPR 在 256/512 token 预算下取得了 dLLM RL 微调的最新最优结果，计算成本仅为树展开方法的 0.6 倍。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散语言模型（dLLM）强化学习方法对去噪轨迹中的丰富信号利用不足：扁平展开（flat rollout）计算廉价但仅分配单一奖励给整个轨迹；树展开（tree rollout）提供细粒度监督但计算开销大。作者希望利用去噪轨迹本身提供类似树展开的细粒度监督，而无需承担树展开的计算成本。

Method: CAPR 算法包含三个关键设计：(1) 将去噪轨迹压缩为紧凑的路径状态（path state）；(2) 利用缓存的轨迹状态生成廉价的兄弟延续（sibling continuation）；(3) 训练块级价值头（block-level value head）用于局部块级监督。在块级去噪调度下，CAPR 记录路径状态和块进度特征，然后将最终结果奖励根据每个块中揭示的 token 重新分配到各个块上，从而将稀疏奖励转化为块级 PPO 权重。

Result: 在 4×4 Sudoku、Countdown、GSM8K 和 Math500 任务上，使用 LLaDA 密集和 MoE 骨干网络，CAPR 在 256 和 512 token 预算下取得了 dLLM RL 微调的最新最优结果。在 Sudoku 上，CAPR 以不到三分之一的每步计算量匹配了最强的树结构基线。CAPR 的 rollout 生成成本约为扁平展开的 0.75 倍、树展开的 0.6 倍。

Conclusion: CAPR 证明了去噪轨迹本身可以有效地提供类似树展开的细粒度监督信号，而无需承担树展开的计算开销。该方法在多个推理任务上取得了 SOTA 结果，为扩散语言模型的高效强化学习对齐提供了新思路。

Classification reason: 该论文提出 CAPR 算法，使用强化学习（PPO）对扩散语言模型（dLLM）进行对齐与奖励优化，属于生成模型对齐与奖励范畴，因此映射到 [CANONICAL] subtopic "生成模型对齐与奖励" (generation_alignment)。

Importance reason: Directly advances continuous language model RL alignment with efficient trajectory-aware supervision, matching top priority subtopic. Signals: continuous language model, diffusion language model, reinforcement learning, efficient inference, block-level value head
...
TL;DR: 本文提出 CDPM-Align，一种多尺度引导对齐的条件扩散预训练方法，用于小样本解剖标志点检测。通过生成式预训练学习鲁棒表征，在仅10或25张标注图像的少标注场景下，显著提升了下游标志点检测的准确性和不确定性估计，推动了安全高效的临床部署。


<details>
  <summary>Details</summary>
Motivation: 解剖标志点检测是医学图像分析的基础任务，现有方法虽能达到亚毫米级精度，但仅靠精度不足以满足临床部署对可靠性和鲁棒性的要求。表征学习在该场景下的影响尚未被充分探索，尤其是在小样本和少标注条件下。

Method: 提出 CDPM-Align，一种多尺度引导对齐的条件扩散预训练框架。在预训练阶段，利用三个小规模异质数据集进行条件生成式预训练，学习鲁棒的多尺度视觉表征。在下游任务中，仅使用10或25张标注图像进行小样本解剖标志点检测，通过生成预训练获得的表征提升检测精度和不确定性估计。

Result: 在三个异质小规模基准数据集上的实验表明，生成式预训练使模型学习到鲁棒的表征，在小样本和少标注场景下，下游标志点检测的准确性和不确定性估计均得到显著提升。

Conclusion: CDPM-Align 验证了条件扩散生成预训练在小样本解剖标志点检测中的有效性，为在标注资源受限的临床场景下实现安全高效的部署提供了可行方案。

Classification reason: 该论文提出 CDPM-Align，一种多尺度引导对齐的条件扩散预训练方法，用于小样本解剖标志点检测。核心是扩散生成预训练（conditional diffusion pre-training）与引导对齐（guidance-aligned），属于生成模型中的对齐/引导/可控性方向，因此映射到 [CANONICAL] subtopic "生成模型对齐与奖励" (generation_alignment)。

Importance reason: Focuses on medical landmark detection, which is outside the user's priority subtopics and has negative keywords. Signals: Few-shot landmark detection, diffusion pretraining, medical imaging, low annotation regime, uncertainty estimation
...
### [49] [MaskForge: Structure-Aware Adaptive Attacks for Jailbreaking Diffusion Large Language Models](https://arxiv.org/abs/2606.04027)
...
Importance: 普通相关 (50.49) - 未入选精读

TL;DR: MaskForge 是一种针对扩散大语言模型（dLLM）的黑盒自适应越狱攻击方法。它将红队测试建模为在结构模式库上的优化搜索，通过UCB bandit选择与目标兼容的填充模式，并在库失效时启用评分器引导的备用策略，成功模式被蒸馏回库中实现经验积累。在5个公开dLLM和3个基准上平均攻击成功率达79.3%，比最强基线提升17.6%。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（dLLM）通过双向上下文迭代去噪部分掩码序列生成文本，其安全面与自回归LLM截然不同。掩码token是原生输入，token由置信度而非位置决定，因此有害内容可通过填充（infilling）在受监控前缀之外诱导。现有越狱方法要么忽略了dLLM的原生填充能力，要么依赖低多样性的掩码模板，缺乏结构自适应性和攻击经验积累。

Method: MaskForge 将dLLM红队测试建模为在结构模式库上的优化搜索。核心组件包括：(1) 模式抽象——将成功攻击尝试抽象为可复用的结构模式；(2) UCB bandit选择器——根据目标选择最兼容的模式；(3) 评分器引导的备用策略——当现有库失败时启用；(4) 经验蒸馏——成功尝试被蒸馏回模式库，实现跨目标的经验积累。整个攻击过程是完全黑盒的。

Result: 在5个公开dLLM和3个基准上，MaskForge平均攻击成功率达79.3%，比最强dLLM基线相对提升17.6%。成熟的模式库可直接迁移到AdvBench，无需任何更新即达到88.2%的攻击成功率，相对最强基线提升67%。

Conclusion: MaskForge 证明了针对dLLM的结构自适应越狱攻击的有效性，揭示了现有dLLM安全防护的脆弱性。其模式库的可迁移性表明dLLM存在系统性的安全漏洞，需要开发专门针对dLLM独特安全面的防御机制。

Classification reason: 该论文研究扩散大语言模型（dLLM）的红队攻击（jailbreak），属于生成模型的安全对齐与攻防范畴。虽然论文聚焦于攻击而非对齐方法本身，但"生成模型对齐与奖励"（generation_alignment）是现有[CANONICAL]中涵盖生成模型安全、偏好优化、可控性的最相关主题，且该主题下已有19篇论文，包含安全对齐相关内容。

Importance reason: Focuses on jailbreaking diffusion LLMs, which is outside the user's priority subtopics and direction weights. Signals: Jailbreak attack, diffusion LLM safety, black-box adaptive method, structure-aware patterns, UCB bandit optimization
...
### [50] [Activation Steering of Video Generation Models via Reduced-Order Linear Optimal Control](https://arxiv.org/abs/2606.04775)
*Jihoon Hong, Alice Chan, Qiyue Dai, Julian Skifstad, Glen Chou*

Main category: eess.SY, math.OC, cs.LG, cs.CV, cs.AI
...
TL;DR: 本文提出 LA-LQR，一种基于降阶线性最优控制的激活干预框架，用于对文生视频模型进行最小侵入式的安全对齐。该方法将 T2V 推理建模为动态系统，通过对比提示对提取低维子空间、估计局部线性动力学并求解 LQR 问题，生成逐层逐时间步的干预信号，在减少不安全生成的同时保持视觉质量和提示忠实度。


<details>
  <summary>Details</summary>
Motivation: 现有文生视频模型可能生成有害内容，而传统的微调或提示过滤方法成本高或效果有限。已有的激活干预方法在 T2V 场景中较为粗糙，通常采用非前瞻性的固定干预，容易导致过度干预和内容质量下降。因此需要一种更精细、最小侵入式的视频生成对齐方法。

Method: 将 T2V 推理过程建模为动态系统，利用对比提示对（安全/不安全）将高维激活投影到低维任务相关子空间，在该潜空间中估计局部线性动力学，然后求解线性二次型调节器（LQR）问题，得到时间步和层特定的最优反馈干预信号，从而将激活引导至期望的特征设定点。

Result: 在概念引导和视频安全基准上，LA-LQR 相比基线方法减少了不安全内容的生成，同时保持了提示忠实度和视觉质量。论文还提供了理论界，证明潜空间设定点跟踪与原始激活空间特征控制之间的关系，并实验验证了降阶潜动力学的保真度。

Conclusion: LA-LQR 提供了一种理论驱动的、最小侵入式的视频生成模型对齐方法，通过最优控制框架实现了比现有粗粒度干预更有效的安全引导，为视频生成模型的安全可控性提供了新思路。

Classification reason: 该论文提出 LA-LQR 框架，通过激活干预（activation steering）对文生视频模型进行安全对齐，减少有害内容生成，属于生成模型对齐与可控性方向，与 [CANONICAL] subtopic "生成模型对齐与奖励" 高度匹配。

Importance reason: Novel activation steering for video generation using optimal control, but lacks direct connection to user's priority subtopics like continuous models or world models. Signals: activation steering, video generation safety, linear optimal control, reduced-order dynamics, minimal invasive intervention
...
### [51] [Evaluating Reasoning Fidelity in Visual Text Generation](https://arxiv.org/abs/2606.04479)
...
TL;DR: 本文系统评估了当前文生图模型在视觉文本生成中的推理忠实度，发现即使渲染文本视觉清晰，模型仍频繁产生语义错误、逻辑不一致和中间步骤错误，与纯文本模型的强推理能力形成鲜明对比，揭示了视觉文本生成与程序化推理之间的显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有文生图模型能够生成清晰可读的文本，但尚不清楚这些系统在需要通过渲染文本表达复杂推理过程时，是否真正保留了推理能力，还是仅仅模仿了表面模式。作者希望探究视觉文本生成中的推理忠实度问题。

Method: 设计了包含长文本渲染、事实知识探测、上下文理解和多步推理四个维度的评测框架，系统评估当前主流文生图模型在视觉文本生成中的推理表现，并与纯文本模型进行对比分析。

Result: 当前文生图模型在视觉文本生成中频繁产生语义错误、逻辑不一致和中间步骤错误，即使渲染文本视觉清晰。这些失败与纯文本模型在相同任务上的强推理表现形成鲜明对比。

Conclusion: 视觉文本生成与程序化推理之间存在显著差距，需要更可靠的视觉文本推理方法。

Classification reason: 该论文系统评估了文生图模型在视觉文本生成中的推理忠实度，包括长文本渲染、事实知识、上下文理解和多步推理等维度，属于生成模型的评测工作，因此映射到 [CANONICAL] subtopic "生成模型评测与数据集" (generation_benchmark)。同时论文也涉及多模态理解与生成方向，故匹配 multimodal_understanding_generation。

Importance reason: Evaluates reasoning fidelity in visual text generation, a niche subtopic not aligned with user's priority on continuous multimodal models or video/image generation. Signals: visual text generation, reasoning fidelity, T2I model evaluation, semantic errors, not in priority subtopics
...
### [52] [Impostor: An Agent-Curated Benchmark for Realistic AIGC Manipulation Localization](https://arxiv.org/abs/2606.04545)
...
TL;DR: 本文提出了Impostor，一个包含10万张AI编辑图像的操作定位数据集，由CraftAgent智能体框架自动生成，覆盖7种AIGC模型和3种操作类型。同时提出PANet网络，利用局部相位建模和语义-取证一致性学习来定位语义合理但取证异常的操作区域。实验表明Impostor对现有LVLM和专用IMDL方法构成显著挑战，PANet在多个基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有图像操作检测与定位（IMDL）基准在视觉真实感、操作多样性和生成器覆盖方面存在局限，难以反映近期生成式图像编辑技术的发展趋势，亟需一个更全面、更真实的基准数据集。

Method: 提出CraftAgent闭环智能体框架，包含场景感知、编辑规划、操作执行、质量验证和迭代反思五个模块，自动生成多样化且视觉真实的编辑图像。同时提出PANet（PhaseAware-Net），引入局部相位建模和语义-取证一致性学习，结合语义特征与频域取证特征来定位操作区域。

Result: Impostor数据集包含100K图像，覆盖7种AIGC模型和3种操作类型。现有LVLM和专用IMDL方法在该数据集上表现不佳，而PANet在Impostor和多个公开基准上均取得最优性能。

Conclusion: Impostor为AIGC图像操作定位提供了更全面的评测基准，PANet通过融合语义与取证信息有效提升了操作区域定位能力，为应对日益逼真的AI编辑图像检测挑战提供了新思路。

Classification reason: 该论文提出了一个AI编辑图像操作定位数据集Impostor（100K图像），属于生成模型评测基准与数据集方向，与[多模态生成]方向下的[生成模型评测与数据集]（generation_benchmark）规范主题高度匹配。同时该数据集也用于评测大视觉语言模型（LVLM），因此也匹配[多模态理解与生成]方向。

Importance reason: Focuses on AIGC manipulation detection, not directly on the user's priority subtopics like continuous language models or video generation. Signals: AIGC manipulation localization, dataset benchmark, agent-curated, phase-aware network, not aligned with user's core interests
...
### [53] [UniCanvas: A Diffusion-base Unified Model for Text-in-Image Joint Generation](https://arxiv.org/abs/2606.04264)
...
TL;DR: UniCanvas 首次尝试利用扩散模型在统一像素画布上实现文本与图像的联合生成（text-in-image generation），将语言表示为视觉图案嵌入图像中，实现无缝的多模态生成，性能优于之前的统一模型。


<details>
  <summary>Details</summary>
Motivation: 现有的统一视觉语言模型（如自回归 VLM）虽然能跨模态推理，但无法生成高质量图像；而扩散模型能生成逼真图像却难以生成连贯的文本。目前缺乏一个统一的模型能够无缝处理视觉和文本的联合生成。

Method: UniCanvas 基于扩散模型，将文本视为视觉图案直接"绘制"在共享的像素画布上。模型不生成离散文本 token，而是在其固有的多模态嵌入空间中学习将语言表示为图像内部的视觉模式，从而在图像合成过程中自然地"画出"文本，实现文本与图像的联合生成。

Result: 实验表明，UniCanvas 在文本-图像联合生成任务上优于之前的统一模型，证明了扩散模型作为统一多模态生成范式的可行性。

Conclusion: UniCanvas 验证了扩散模型通过 text-in-image 生成实现统一多模态生成的潜力，为未来构建更强大的统一多模态模型提供了新方向。

Classification reason: 该论文提出 UniCanvas，利用扩散模型在统一像素画布上实现文本与图像的联合生成（text-in-image generation），属于理解与生成一体化的统一多模态模型方向。该论文同时涉及多模态生成（扩散模型生成图像与文本）和多模态理解与生成（统一模型），因此匹配 multimodal_generation 和 multimodal_understanding_generation 两个方向。在 subtopic 层面，论文的核心贡献是统一扩散模型进行多模态内容生成，与 [CANONICAL] 的 "统一多模态模型"（unified_multimodal_model）高度吻合，故复用该 subtopic。

Importance reason: Directly advances unified multimodal generation via diffusion models, aligning with top priority subtopics and boost keywords. Signals: First diffusion-based text-in-image generation, Unified multimodal generation on pixel canvas, Strong performance over prior unified models, High relevance to continuous multimodal and unified MLLM priorities
...
### [54] [Self-Distilled Policy Gradient](https://arxiv.org/abs/2606.04036)
...
TL;DR: 本文提出 SDPG（Self-Distilled Policy Gradient），一种自蒸馏策略梯度框架，结合 group-relative verifier 优势、归一化标准差、精确全词汇 on-policy 自蒸馏以及参考策略 KL 正则化，在稀疏奖励强化学习中提升稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 在稀疏奖励的强化学习（如 RLHF）中，奖励信号稀疏且延迟，导致训练不稳定。On-policy 自蒸馏可以利用语言模型自身的特权上下文提供密集监督信号，但现有方法尚未充分探索如何有效结合自蒸馏与策略梯度。

Method: SDPG 框架包含三个关键组件：(1) Group-relative verifier advantages，利用验证器对同一 prompt 的多个生成进行相对评分；(2) 归一化标准差，稳定优势估计；(3) 精确全词汇 on-policy 自蒸馏，作为学生到教师的逆向 KL 散度辅助损失，提供密集监督；(4) 参考策略 KL 正则化，防止策略偏离过远。

Result: 实验表明 SDPG 在稳定性和性能上均优于 RLVR（强化学习与验证器奖励）和纯自蒸馏基线方法。

Conclusion: SDPG 通过将自蒸馏形式化为全词汇逆向 KL 散度损失，并与策略梯度框架有机结合，为稀疏奖励 RL 提供了一种稳定且有效的训练范式。

Classification reason: 该论文提出 SDPG（自蒸馏策略梯度）框架，结合 group-relative verifier advantages 与 on-policy self-distillation 来改进语言模型的强化学习训练。这属于多模态/语言模型的 RL 对齐方法，与 canonical 子主题「多模态强化学习与对齐」(multimodal_rl_alignment) 高度匹配。

Importance reason: Focuses on RL for language models, not on the user's priority subtopics like continuous multimodal models or video generation. Signals: RL for LLM, self-distillation, sparse reward, policy gradient, not in priority subtopics

Abstract: On-policy self-distillation, where a language model conditions on privileged context to supervise its own generations, is a promising source of dense supervision for sparse-reward reinforcement learning. Actually, it can be instantiated as an auxiliary full-vocabulary student-to-teacher reverse Kullback-Leibler divergence loss. We therefore propose SDPG, a self-distilled policy-gradient framework that combines group-relative verifier advantages with normalized standard deviation, exact full-vocabulary on-policy self-distillation, as well as reference-policy KL regularization. Empirically, SDPG improves stability and performance over RLVR and self-distillation baselines. The code is available at .

</details>


### [55] [Sparse Mixture-of-Experts Reward Models Learn Interpretable and Specialized Experts for Personalized Preference Modeling](https://arxiv.org/abs/2606.04284)
...
TL;DR: 本文提出稀疏混合专家（MoE）奖励模型，通过稀疏路由和专家多样性训练，从二元偏好数据中学习可解释、专业化的偏好组件，提升个性化偏好建模能力。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF方法假设通用奖励函数，忽略了人类偏好的多样性和异质性。虽然已有工作尝试从二元数据中学习多个偏好组件，但这些组件往往缺乏连贯性和可解释性，难以有效实现个性化。

Method: 提出稀疏MoE奖励模型，在二元偏好数据训练中引入稀疏路由和专家多样性正则化，使不同专家自动学习不同的偏好维度（如有用性、安全性、风格等），并通过专家权重的后适应调整实现个性化。

Result: 在受控实验和真实场景中，稀疏MoE学习到可解释的路由模式和专业化专家，提升了测试时个性化性能，专家权重的后适应变化为分析模型如何适应个性化偏好提供了定性视角。

Conclusion: 稀疏MoE奖励模型能够从二元偏好数据中自动学习可解释、解耦的偏好组件，有效提升个性化偏好建模能力，为理解偏好多样性和实现细粒度对齐提供了新思路。

Classification reason: 该论文研究基于稀疏MoE的奖励模型用于个性化偏好建模，属于RLHF/偏好学习范畴，与多模态强化学习与对齐（multimodal_rl_alignment）方向高度相关。虽然论文主要聚焦于文本LLM，但其方法可直接推广到多模态场景，且该[CANONICAL] subtopic涵盖RLHF与偏好建模。

Importance reason: Focuses on reward model interpretability for RLHF, which is tangential to the user's core interests in continuous multimodal models and video generation. Signals: Sparse MoE for preference modeling, Interpretable expert specialization, Personalized reward adaptation, Not directly related to priority subtopics, No boost keywords matched
...
### [56] [Answer Self-Consistency with Margin-Triggered Question Re-Arbitration for the CVPR 2026 VidLLMs Challenge](https://arxiv.org/abs/2606.04323)
...
Importance: 值得关注 (72.61) - 未入选精读

TL;DR: 本文提出 ASC-MQRA，一种无需训练的自一致性测试时推理框架，用于 CVPR 2026 VidLLMs Challenge Track 2 视频关系推理任务。核心方法通过多次随机视频问答并聚合答案选择来提升推理准确性，并进一步对低置信度样本进行条件性重新仲裁。最终提交版本使用 ASC 方法，在测试集上达到 81.16% 的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 视频关系推理任务要求模型推断视频中不总是显式可见的关系，单次推理往往不够可靠。作者希望通过多次推理的答案自一致性来提升推理的鲁棒性和准确性，并利用投票分布中的不确定性信号进一步优化低置信度样本。

Method: 提出 ASC-MQRA 框架：(1) ASC（Answer Self-Consistency）：对同一视频进行多次随机视频问答推理，收集答案分布，通过答案级自一致性投票聚合得到最终答案；(2) MQRA（Margin-Triggered Question Re-Arbitration）：对第一轮投票中 margin（最高票与次高票差距）较低的样本，缩小候选答案集合并重新观看视频进行二次推理。最终提交版本仅使用 ASC 方法。

Result: 在验证集上，ASC 达到 72.73% 平均准确率和 78.34% 类别宏平均准确率；MQRA 进一步提升了验证集性能。在测试集上，ASC 达到 81.16% 平均准确率和 80.91% 类别宏平均准确率，但 MQRA 在测试集上略有下降，表明重新仲裁对触发子集的规模和类别分布敏感。

Conclusion: 答案自一致性（ASC）是一种简单有效的测试时推理增强方法，能显著提升视频关系推理的准确性。低 margin 的投票分布可以作为有用的不确定性信号，但重新仲裁策略的效果依赖于触发样本的分布特性，需要进一步研究以提升其泛化能力。

Classification reason: 该论文提出 ASC-MQRA 框架，通过多次随机视频问答和答案级自一致性来提升视频关系推理能力，属于多模态推理（视觉推理）范畴，与 canonical subtopic "多模态推理与思维链" 高度匹配。

Importance reason: A competition report on test-time self-consistency for video reasoning, but lacks novelty and does not align with the user's priority subtopics. Signals: Test-time reasoning, Self-consistency, Video relation reasoning, No training required, Low novelty competition solution
...
### [57] [BiNSGPS: Geometry Problem Solving via Bidirectional Neuro-Symbolic Interaction](https://arxiv.org/abs/2606.04648)
...
TL;DR: BiNSGPS 提出了一种双向神经符号交互框架，通过 MLLM 顾问与符号求解器之间的动态反馈循环，实现几何问题的鲁棒求解，克服了单向流水线中早期错误传播的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有几何问题求解方法中，纯符号方法适应性有限，纯神经方法容易产生幻觉，而神经符号混合方法多为单向流水线（神经输出→符号求解器），缺乏反馈机制，导致早期错误无法纠正，系统脆弱。

Method: 提出 BiNSGPS 框架，建立 MLLM Adviser 与 Symbolic Solver 之间的双向神经符号交互（BiNS）。MLLM Adviser 主动接收符号求解器的反馈，动态修正不一致的形式化表示或提出辅助假设，从而解决符号冲突并促进复杂推理。

Result: 在几何问题求解基准上验证了 BiNSGPS 的有效性，双向交互机制显著提升了求解准确率，尤其是在需要多步推理和错误纠正的复杂问题上。

Conclusion: 双向神经符号交互是提升几何问题求解鲁棒性的有效范式，MLLM 与符号求解器之间的动态反馈循环能够有效缓解早期错误传播问题，为神经符号融合提供了新方向。

Classification reason: 该论文研究几何问题求解，属于多模态数学/科学推理范畴，与 [CANONICAL] 多模态推理与思维链 (multimodal_reasoning_cot) 高度匹配。

Importance reason: Focuses on geometry problem solving with neuro-symbolic interaction, which is not a priority subtopic and lacks direct alignment with user's key directions like continuous multimodal models or video generation. Signals: Neuro-symbolic interaction, MLLM advisor, Symbolic solver, Geometry reasoning, Bidirectional feedback
...
TL;DR: Food-R1 是一个面向食品分析的多任务视觉语言模型，通过 CoT 冷启动指令微调 + GRPO 强化微调，在卡路里推理等食品相关任务上超越强基线。


<details>
  <summary>Details</summary>
Motivation: 现有食品分析 VLM 主要依赖监督微调（SFT），推理和泛化能力有限；同时缺乏高质量的大规模营养标注数据。因此需要一种结合 CoT 推理和强化学习的方法来提升食品分析模型的性能。

Method: 构建了 CalorieBench-80K 基准数据集，包含卡路里标签和饮食建议标注，并首次引入 CoT 标注用于卡路里推理。提出 Food-R1 模型，采用多任务学习范式，先进行 CoT 冷启动指令微调，再使用 GRPO（Group Relative Policy Optimization）进行强化微调（RFT）以提升推理能力。

Result: 在 CalorieBench-80K 和代表性基准上，Food-R1 在食品相关任务上持续优于强基线模型。

Conclusion: CoT 冷启动 + GRPO 强化微调能有效提升食品分析 VLM 的推理和泛化能力，为食品领域多模态推理提供了新范式。

Classification reason: 该论文提出 Food-R1，一个面向食品分析的多任务视觉语言模型，使用 CoT 冷启动指令微调 + GRPO 强化微调来提升推理能力。这属于多模态推理与思维链（multimodal_reasoning_cot）范畴，且论文明确使用了 Chain-of-Thought 标注和 GRPO 强化推理，与 canonical 子主题高度匹配。

Importance reason: Food-specific VLM with limited generalizability; no direct alignment with priority subtopics. Signals: food vision-language model, multi-task learning, CoT reasoning, GRPO reinforcement fine-tuning, calorie estimation benchmark
...
### [59] [Overview of the EReL@MIR 2025 Multimodal Document Retrieval Challenge (Track 1)](https://arxiv.org/abs/2606.04240)
*Jingbiao Mei*
...
TL;DR: 本文介绍了 EReL@MIR 2025 多模态文档检索挑战赛（Track 1），该赛事包含两个互补任务：长文档内的闭集页面检索（MMDocIR）和基于图像或图文查询的开放域维基百科段落检索（M2KR）。共有22支队伍参赛，前三名均基于 Qwen2-VL 系列的多模态 LLM 嵌入器，而非 CLIP 风格编码器。无训练的多路融合+重排序系统仅以0.1分差距落后于微调集成系统。


<details>
  <summary>Details</summary>
Motivation: 大多数现有检索器在处理图文混合文档时忽略了视觉通道，而多模态文档检索对于多模态检索增强生成至关重要。该挑战赛旨在推动构建统一的检索系统，同时处理闭集文档页面检索和开放域多模态检索两个互补场景。

Method: 挑战赛设计了两个任务：MMDocIR（长文档内基于文本查询的页面检索）和 M2KR（基于图像或图文查询的开放域段落检索）。使用宏平均 Recall@{1,3,5} 作为排名指标。前三名系统均基于 Qwen2-VL 系列解码器多模态 LLM 嵌入器，区别在于：第一名使用微调集成，第二名使用无训练多路融合+强视觉语言重排序器，第三名使用零样本后期交互。

Result: 挑战赛吸引了455名参赛者和586次提交，来自22支队伍。无训练的多路融合+重排序系统仅以0.1分差距落后于微调集成系统，展示了无需微调即可达到接近最优性能的可能性。

Conclusion: 多模态文档检索挑战赛表明，基于多模态 LLM 的嵌入器（如 Qwen2-VL）在图文混合文档检索中优于传统 CLIP 风格编码器，且无训练方法也能取得极具竞争力的结果，为未来多模态检索研究提供了重要参考。

Classification reason: 该论文是关于多模态文档检索挑战赛的概述，涉及图文混合文档的检索任务，属于多模态检索范畴。现有 [CANONICAL] subtopic "multimodal_retrieval（多模态检索）" 完全匹配。

Importance reason: Focuses on multimodal document retrieval challenge, not on user's priority subtopics like continuous language models or video generation. Signals: Multimodal retrieval challenge, Qwen2-VL embedders, not user's priority subtopics, no video or continuous model focus, negative keywords absent but not relevant
...
### [60] [MM-BizRAG: Rethinking Multimodal Retrieval-Augmented Generation for General Purpose Enterprise Q&A](https://arxiv.org/abs/2606.04231)
...
TL;DR: MM-BizRAG 提出了一种面向企业级多模态文档的检索增强生成方法，通过文档结构感知的拆分、方向感知的解析流水线、LLM驱动的工件转换与推理时多模态组装，在不需微调的情况下显著优于现有视觉中心基线，并引入高效评测指标 FastRAGEval。


<details>
  <summary>Details</summary>
Motivation: 现有 MM-RAG 方法倾向于最小化解析，依赖页面级图像进行检索和生成，忽略了企业文档中丰富的结构化信息（如报告、幻灯片）。本文旨在显式提取和表示文档结构，以提升复杂企业文档上的问答性能。

Method: MM-BizRAG 包含四个核心组件：(1) 文档结构感知拆分器，根据文档方向（垂直/水平）动态路由到不同解析流水线；(2) 布局感知解析（垂直文档）与整体页面级表示（水平文档）；(3) LLM驱动的工件转换流水线，通过占位符对齐保持自然阅读顺序；(4) 推理时多模态组装，解耦检索表示与生成上下文，无需微调。

Result: 在大型企业数据集和两个公开基准（SlideVQA、FinRAGBench-V）上，MM-BizRAG 比 SOTA 视觉中心基线高出最多 32 个百分点，尤其在报告式布局上表现突出。同时提出的 FastRAGEval 指标将 RAGChecker 成本减半且与人类对齐更强。

Conclusion: 显式建模文档结构而非依赖隐式捕获，能显著提升多模态 RAG 在企业文档问答中的效果。MM-BizRAG 无需微调即可实现强性能，FastRAGEval 提供了更高效的生成召回评测方案。

Classification reason: 该论文提出 MM-BizRAG，聚焦企业级多模态文档的检索增强生成（RAG），涉及文档结构感知的解析、检索与生成。这与 [CANONICAL] 多模态检索（multimodal_retrieval）高度匹配，因为核心贡献在于多模态文档检索与生成中的结构化表示与检索策略。

Importance reason: Focuses on enterprise multimodal RAG with structured document parsing, not on the user's priority subtopics like continuous language models or video generation. Signals: Enterprise Q&A, Multimodal RAG, Document structure-aware parsing, No finetuning, FastRAGEval metric
...
### [61] [VT-3DAD: Cross-Category 3D Anomaly Detection via Visual-Text Normal Space Alignment](https://arxiv.org/abs/2606.04369)
...
TL;DR: VT-3DAD提出了一种无需训练的跨类别3D异常检测框架，通过冻结CLIP视觉编码器提取多视图深度图特征，并结合文本编码器构建语义正常锚点，融合视觉偏差与语义偏差进行异常评分，在ShapeNetPart上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于训练的3D异常检测方法需要逐类别优化，而基于多视图CLIP视觉特征的无训练方法仅依赖视觉相似性，容易被几何相似的类别混淆。因此需要一种跨类别、无需训练且能利用语义信息的3D异常检测方法。

Method: VT-3DAD包含两个分支：(1) 视觉分支：将少量正常参考点云和测试点云渲染为多视图深度图，用冻结CLIP视觉编码器提取视图级特征，计算参考-测试偏差；(2) 文本分支：设计深度感知和3D感知的文本提示，用冻结CLIP文本编码器编码构建文本正常锚点，提供语义正常性约束。最终融合视觉偏差和语义偏差得到异常分数。

Result: 在ShapeNetPart数据集上，VT-3DAD在one-shot设置下将平均AUC-ROC从92.49%提升至94.80%，同时将平均标准差从5.64降至3.41，达到SOTA性能。

Conclusion: VT-3DAD通过视觉-文本正常空间对齐，有效解决了跨类别3D异常检测中几何相似类别混淆的问题，验证了语义先验在3D异常检测中的重要性。

Classification reason: 该论文使用CLIP视觉-文本对齐进行3D异常检测，核心是视觉-文本特征空间的对齐与检索（正常样本vs异常样本的偏差度量），属于多模态检索范畴。映射到[CANONICAL] subtopic "多模态检索"。

Importance reason: Focuses on 3D anomaly detection, not aligned with user's priority subtopics like continuous language models or video generation. Signals: 3D anomaly detection, cross-category, training-free, CLIP visual-text alignment, ShapeNetPart benchmark
...
### [62] [Fine-grained Fragment Retrieval in Multi-modal Long-form Dialogues](https://arxiv.org/abs/2606.04591)
...
TL;DR: 本文提出细粒度片段检索（FFR）任务，旨在从多模态长对话中检索与特定主题相关的多轮次、多图像语义片段。针对单对话场景提出基于强化学习的生成式检索模型 F2RVLM，针对语料库场景提出两阶段系统 FFRS（片段索引+粗召回+细粒度推理）。构建了目前最长的多模态对话检索数据集 MLDR 和微信真实测试集，实验验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 多模态对话平台普及后，用户常需要从长对话中检索与特定主题相关的连贯对话片段（而非孤立语句），现有检索方法缺乏对多轮次、多图像片段级语义连贯性的建模能力。

Method: 提出两个设置：(1) 单对话内 FFR：采用生成式检索模型 F2RVLM，使用强化学习训练，设计多目标奖励和难度感知课程采样以增强片段连贯性。(2) 对话语料库 FFR：两阶段系统 FFRS，离线将对话分解为最小语义片段并由片段嵌入模型（FEM）编码存入向量库，在线先粗召回 Top-K 候选，再由 F2RVLM 细粒度推理定位最相关子内容。同时构建了 MLDR 数据集和微信真实测试集。

Result: 在 MLDR 和微信真实测试集上，F2RVLM 和 FFRS 在单对话和语料库级 FFR 任务中均取得一致优越性能。

Conclusion: 本文提出的细粒度片段检索任务和方法有效解决了多模态长对话中的片段级检索问题，强化学习训练和两阶段检索架构均被证明有效。

Classification reason: 该论文研究多模态长对话中的细粒度片段检索，属于多模态检索任务。现有 [CANONICAL] subtopic "multimodal_retrieval（多模态检索）" 完全匹配，因此直接复用。

Importance reason: Focuses on multi-modal retrieval in dialogues, not on the user's priority subtopics like continuous models, video/image generation, or world models. Signals: multi-modal retrieval, long-form dialogues, reinforcement learning, fragment retrieval, not generative
...
### [63] [When Seeing Is Not Believing -- A Benchmark for Search-Grounded Video Misinformation Detection](https://arxiv.org/abs/2606.04098)
...
TL;DR: 本文提出 EVID-Bench，一个面向搜索增强型视频虚假信息检测的基准，包含222个视频、9种操作类型（AI生成、单源编辑、多源编辑），所有样本均无法通过纯视觉检测识别。评估9个前沿多模态模型，最佳系统仅达61.43%点级准确率和43.24%视频级准确率，AI生成操作尤其困难。


<details>
  <summary>Details</summary>
Motivation: 现有视频虚假信息检测方法主要依赖视觉线索（如伪影、不一致性），但许多虚假信息在语义和证据层面操作（如选择性剪辑、时间重排、跨源拼接、AI生成内容），这些操作无法仅从输入视频本身可靠验证，因为缺失、重排、替换或重新语境化的证据在视频之外。因此需要搜索增强的跨视频比较方法。

Method: 构建 EVID-Bench 基准，包含222个视频，覆盖9种操作类型（3类：AI生成、单源编辑、多源编辑）。所有样本经人工验证确保前沿模型无法通过纯视觉检测识别。采用检索增强验证（RAV）基线，让多模态模型搜索开放网络获取相关视频，通过跨视频比较识别虚假信息。

Result: 评估9个前沿多模态模型，最佳系统点级准确率61.43%，视频级准确率43.24%。AI生成类操作最具挑战性。错误分析显示模型存在三大问题：关注无关锚点、将合成内容误归因于编辑拼接、在完全解释操作前过早终止搜索。

Conclusion: EVID-Bench 揭示了当前多模态模型在搜索增强型视频虚假信息检测上的严重不足，尤其是AI生成内容的检测。该基准为未来研究提供了有价值的评估平台，表明需要更有效的检索增强验证策略和更鲁棒的多模态推理能力。

Classification reason: 该论文提出了 EVID-Bench，一个面向搜索增强型视频虚假信息检测的新基准，属于多模态评测与基准方向。该基准涵盖9种操作类型、222个视频，并评估了多个前沿多模态模型的表现，完全符合 [CANONICAL] subtopic "多模态评测与基准" 的定义。

Importance reason: Strong benchmark for video misinformation detection but outside user's core focus on generative models and continuous representations. Signals: video misinformation detection, search-grounded benchmark, multimodal evaluation, AI-generated video detection, cross-video verification
...
### [64] [GroupToM-Bench: Benchmarking Group Theory of Mind and Nonlinear Social Emergence in MLLMs](https://arxiv.org/abs/2606.04184)
...
TL;DR: 本文提出 GroupToM-Bench，首个面向群体级心理理论（Group Theory of Mind）的多模态评测基准，通过七级认知审计框架评估 MLLM 从个体信念/欲望/意图到群体张力、结构约束及宏观结果预测的因果推理能力。实验表明当前 MLLM 与人类基线存在显著差距，尤其在处理社会结构和非线性集体动力学方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 真正的通用智能不仅需要物理世界模型，还需要社会世界模型——即推断个体心理状态如何相互作用并结晶为群体层面结果的能力。现有 MLLM 在个体级心理理论（ToM）推理上虽有进展，但在群体级 ToM 上完全失败，因为集体行为从社会张力、从众动力学和结构约束中非线性涌现，无法通过简单加总个体意图来恢复。

Method: 构建 GroupToM-Bench 多模态基准，围绕微观层 BDI 状态（信念、欲望、意图）、中观层群体张力与结构约束、宏观层结果预测与机制归因的因果链设计，开发七级认知审计框架（7-level cognitive audit framework）来系统评估模型在群体级 ToM 各层面的表现。

Result: 实验揭示当前 MLLM 与人类基线之间存在显著差距，模型在处理社会结构和非线性集体动力学方面存在根本性失败。

Conclusion: 当前 MLLM 在群体级心理理论推理上远未达到人类水平，需要发展能够建模社会结构和非线性集体涌现的新能力，这对构建真正通用智能至关重要。

Classification reason: 该论文提出了 GroupToM-Bench，一个用于评估多模态大语言模型群体心理理论（Group Theory of Mind）能力的多模态评测基准，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Novel benchmark for social cognition in MLLMs, but far from user's priority on continuous multimodal models and video generation. Signals: Group Theory of Mind, MLLM social reasoning benchmark, nonlinear collective dynamics, cognitive audit framework, human baseline gap
...
### [65] [FindIt: A Format-Informed Visual Detection Benchmark for Generalist Multimodal LLMs](https://arxiv.org/abs/2606.04282)
...
TL;DR: 本文提出 FindIt，首个系统评估通用多模态大模型（MLLM）在可提示定位能力上的标准化基准，涵盖目标检测、指代表达检测、实例级检测和视频检测四类任务，并统一了输入输出格式与评估协议。实验表明现有MLLM在定位任务上表现有限，且对输出格式约束高度敏感。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评测主要集中在自由形式的视觉语言任务（如VQA、描述、摘要），但实际应用中MLLM越来越多地被用于结构化计算机视觉任务（如目标检测等定位任务）。目前缺乏一个系统化、标准化的基准来评估MLLM在这类可提示定位能力上的表现。

Method: 构建了一个统一的评测框架，涵盖四类核心定位任务：目标检测、指代表达检测、实例级检测和视频检测。标准化了输入格式，强制输出可解析的边界框，并定义了透明的评估协议。在该基准上评估了多种开源和闭源MLLM，并分析了模型对输出格式规范的遵循能力。

Result: 现有MLLM在定位任务上表现有限，且对输出格式约束高度敏感，即使微小的格式变化也会导致性能显著下降。不同模型在不同任务上各有优劣，揭示了当前系统在结构化定位场景中的能力边界。

Conclusion: FindIt基准系统揭示了当前MLLM在定位任务上的优势与不足，强调了输出格式规范对模型性能的重要影响，为改进多模态模型设计与评估提供了重要方向。

Classification reason: 该论文提出了首个专门评估通用MLLM在结构化定位任务（目标检测、指代表达检测、实例级检测、视频检测）上的标准化基准，属于多模态评测与基准方向，与[CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark）高度匹配。

Importance reason: Benchmark for MLLM localization is useful but not aligned with user's priority on continuous multimodal models and generation. Signals: MLLM evaluation benchmark, localization tasks, format sensitivity, not generative, not continuous model
...
### [66] [MemoryDocDataSet: A Benchmark for Joint Conversational Memory and Long Document Reasoning](https://arxiv.org/abs/2606.04442)
...
TL;DR: MemoryDocDataSet 是一个合成基准数据集，包含 50 个微世界和 1000 个 QA 对，用于评估 AI 系统同时处理多轮对话记忆和长文档阅读理解的能力。其中 75.1% 的问题需要先导航对话历史找到相关文档，再从中提取答案。最佳基线（RAG-Both）仅达到 0.358 总体 F1 和 0.342 Hybrid F1，揭示了联合检索的显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准要么评估对话记忆，要么评估长文档理解，但没有一个能同时评估这两种能力。随着 AI 系统需要处理越来越复杂的任务，同时具备多轮对话记忆导航和长文档深度阅读理解的能力变得至关重要。

Method: 构建了 50 个微世界，每个包含 3-5 个人物、一个时间事件图、3-5 篇真实长文档（来自 Caselaw Access Project，每篇 2-5 万 token）、基于这些文档的多轮对话，以及 20 个 QA 对（覆盖 5 类推理）。核心创新是 Hybrid 标签问题，要求系统先导航对话历史确定相关文档，再从文档中提取答案。使用 LLM-as-judge 进行提示敏感性自一致性分析。

Result: 最佳基线 RAG-Both 在总体 F1 上为 0.358，Hybrid 问题上为 0.342。仅文档检索的 RAG-Doc 在 Hybrid 上降至 0.267，而在纯文档问题上为 0.453，显示出明显的联合检索差距。提示敏感性自一致性分析的中位 Cohen's κ = 0.634。

Conclusion: 现有方法在联合对话记忆与长文档推理任务上表现不佳，存在显著的联合检索差距，这激励了未来研究设计统一对话记忆与长文档导航的架构。数据集和代码已开源。

Classification reason: 该论文提出了一个结合对话记忆与长文档推理的新基准 MemoryDocDataSet，属于多模态/语言模型评测基准，与 [CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark）高度匹配。

Importance reason: Focuses on a niche synthetic benchmark for conversational memory and long document reasoning, not aligned with user's priority subtopics. Signals: synthetic benchmark, conversational memory, long document reasoning, hybrid retrieval gap, legal domain documents
...
### [67] [VAMPS: Visual-Assisted Mathematical Problem Solving Benchmark](https://arxiv.org/abs/2606.04244)
...
Main category: cs.LG, cs.AI, cs.CL, cs.CV
...
TL;DR: VAMPS 是一个面向图辅助数学问题求解的多模态双语评测基准，包含 1,168 道选择题，用于评估多模态大模型在借助可视化工具进行推理时的表现。实验发现，直接解析求解反而优于借助工具生成图形再推理的方式。


<details>
  <summary>Details</summary>
Motivation: 现有多模态基准主要评估模型对固定视觉输入的推理能力，但现实工程和科学工作流中，模型常常需要先通过工具（如绘图）外部化问题，再基于工具输出进行推理。这种"工具辅助推理"的能力尚未被充分研究。

Method: 构建了 VAMPS 基准，包含 1,168 道双语（波斯语/英语）选择题，来源于伊朗大学入学考试的代数和微积分题目，并经过人工审核的 LLM 生成变体扩充。题目设计使得绘图（揭示交点、极值、渐近线等）成为自然的解题策略。评测了多种多模态大模型在直接解析求解与工具辅助可视化求解两种设置下的表现。

Result: 在多种模型上，直接解析求解的表现一致优于借助工具生成图形再推理的方式，即使对于绘图是自然策略的题目也是如此。

Conclusion: 当前多模态大模型在需要借助外部工具生成可视化并基于可视化进行推理的任务上存在明显不足，这为未来模型在工具辅助推理能力上的改进提供了方向。

Classification reason: 该论文提出了 VAMPS，一个面向数学问题求解的多模态评测基准，属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题。基准包含图辅助数学题，用于评估模型在借助可视化工具进行推理时的表现，完全符合该子主题的定义。

Importance reason: This benchmark focuses on tool-aided visual reasoning for math, which is not a priority subtopic and lacks direct connection to the user's core interests in continuous models, generation, or world models. Signals: Multimodal benchmark, Math problem solving, Tool-aided reasoning, Bilingual, Not aligned with priority subtopics
...
### [68] [VCIFBench: Evaluating Complex Instruction Following for Video Understanding](https://arxiv.org/abs/2606.04588)
...
TL;DR: VCIFBench 是一个用于评估多模态大模型在视频理解中遵循复杂指令能力的基准，包含 306 条可满足测试指令、540 对 DPO 偏好数据集和 30 项冲突诊断子集，实验表明联合约束满足仍具挑战性，DPO 训练可提升指令跟随性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准大多依赖简单提示，缺乏对模型能否满足显式输出约束（如内容、格式、风格、结构要求）的评估，因此需要构建一个专门评估复杂指令跟随能力的基准。

Method: 从基准适配和直接视频锚定两种方式构建约束丰富的指令，覆盖内容、格式、风格和结构四类要求；采用混合验证流水线评估模型输出；构建了 306 条可满足测试指令、540 对 DPO 偏好数据集和 30 项冲突诊断子集。

Result: 在 10 个 MLLM 上的实验表明，联合约束满足仍然具有挑战性；在 VCIFBench 数据上进行 DPO 训练可以提升指令跟随性能。

Conclusion: VCIFBench 为视频理解中的复杂指令跟随提供了系统化的评测框架，揭示了现有 MLLM 在联合约束满足方面的不足，并展示了 DPO 训练作为改进手段的有效性。

Classification reason: 该论文提出了 VCIFBench，一个用于评估视频理解中复杂指令跟随能力的新基准，属于多模态评测与基准方向。该论文与 [CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark）高度匹配。

Importance reason: Focuses on video instruction following benchmark, not directly on priority subtopics like continuous models or generation. Signals: Video understanding benchmark, Complex instruction following, DPO training, Not aligned with priority subtopics, No boost keywords matched
...
### [69] [A Systematic Evaluation of Positional Bias in Multi-Video Summarization with MLLMs](https://arxiv.org/abs/2606.04596)
*Huangchen Xu, Yuan Wu, Yi Chang*
...
TL;DR: 本文系统评估了多模态大语言模型在多视频摘要任务中的位置偏差问题，构建了包含ActivityNet和新闻视频的基准，使用Coverage、DPB和MEG三种指标评估9个开源和闭源MLLM，发现位置效应具有领域和模型依赖性，增加视觉或生成预算不能均匀消除偏差。


<details>
  <summary>Details</summary>
Motivation: MLLM在多视频输入场景下的可靠性尚未被充分理解，特别是输入顺序/位置对每个视频摘要质量的影响（位置偏差）缺乏系统研究。

Method: 构建了来自ActivityNet和新闻视频的基准，涵盖Cooking、Domestic、Leisure和News四个领域，包含2视频和4视频输入设置。使用Coverage、Directional Positional Bias (DPB)和Middle-Edge Gap (MEG)三种互补指标，评估9个开源和闭源MLLM的位置效应，并分析提示级别的缓解方法。

Result: 位置效应具有领域和模型依赖性；有向位置偏差可能很小但中间位置表现不佳；增加视觉或生成预算不能均匀消除偏差。

Conclusion: 多视频摘要对输入协议和位置仍然敏感，需要更鲁棒的顺序不变多模态系统。

Classification reason: 该论文系统评估了MLLM在多视频摘要任务中的位置偏差（positional bias），构建了新的评测基准并提出了DPB、MEG等评测指标，属于多模态评测与基准方向，因此映射到[CANONICAL] subtopic 'multimodal_benchmark'。

Importance reason: Focuses on positional bias evaluation in multi-video summarization, which is not a priority subtopic and lacks direct connection to user's high-weight directions like continuous language models or video generation. Signals: MLLM evaluation, positional bias, multi-video summarization, domain-dependent effects, prompt mitigation
...
### [70] [Beyond Symmetric Alignment: Spectral Diagnostics of Modality Imbalance in Vision-Language Models in the Medical Domain](https://arxiv.org/abs/2606.04613)
...
TL;DR: 本文提出Spectral Alignment Score (SAS)，一种非对称的模态对齐度量，通过将两种模态投影到锚定模态的主特征基上并计算特征值加权的每特征模态相关性，量化模态信息不平衡。实验表明医学图像比其配对报告保留更丰富的结构信息，SAS在医学领域与检索性能的零标签相关性最强，可作为临床部署的诊断工具。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型(VLM)在医学图像-文本数据上表现不佳，但缺乏有效的诊断工具。现有的表示对齐度量都是对称的，将两种模态合并为一个分数，无法揭示哪种模态驱动了跨模态性能退化。

Method: 提出Spectral Alignment Score (SAS)，一种非对称度量：将两种模态投影到锚定模态的主特征基上，计算特征值加权的每特征模态相关性，得到方向性分数，其差值量化模态信息不平衡。将SAS嵌入评测框架，在自然与医学图像-文本数据集上评估15个VLM，结合6种对齐度量和双向检索。

Result: 医学图像比其配对的临床报告保留更丰富的结构信息，这种方向性不对称性对所有竞争度量都是不可见的。SAS在医学领域与检索性能的零标签相关性最强。

Conclusion: SAS作为一种非对称诊断工具，能够有效揭示VLM在多模态对齐中的模态不平衡问题，特别适用于医学领域，为临床部署提供了实用的诊断手段。

Classification reason: 该论文提出了一种新的非对称对齐度量SAS，用于诊断视觉语言模型在多模态表示对齐中的模态不平衡问题，并在自然与医学图像-文本数据上进行了系统性评测。这属于多模态评测与基准（multimodal_benchmark）范畴，因为核心贡献是新的评测指标和诊断工具。

Importance reason: Focuses on medical domain VLM diagnostics, which is outside your priority subtopics and has negative keyword overlap. Signals: Novel asymmetric alignment metric, Strong correlation with retrieval, Medical domain focus, Not aligned with user's priority subtopics
...
### [71] [Benchmarking Living-Screen-Native GUI Agents on Short-Video Platforms](https://arxiv.org/abs/2606.04701)
...
TL;DR: 本文提出 Living-Screen-Native GUI Agent 概念，并构建 LivingScreen 基准，用于评测 GUI Agent 在短视频平台这类动态屏幕环境中的表现。基准包含忠实浏览器环境、三层任务套件以及联合评估准确率与信息效率的指标。实验发现现有模型均未达到人类水平，主要失败模式为过度观察或观察不足，表明观察控制是未来 GUI Agent 缺失的关键能力。


<details>
  <summary>Details</summary>
Motivation: 现有 GUI Agent 假设屏幕在两次动作之间是静态的，但短视频等真实应用的内容持续播放，用户需要决定看什么、看多久。现有基准无法评测这种动态屏幕场景，因此需要新的评测框架。

Method: 形式化定义 Living-Screen-Native GUI Agent 任务；构建 LivingScreen 基准，包含基于浏览器的忠实环境、三层任务套件（基础操作、信息获取、综合决策），以及联合评估准确率与信息效率的指标。

Result: 评测多种前沿模型，发现均未达到人类成本-准确率性能；主要失败模式为过度观察（看太多不必要内容）和观察不足（错过关键信息），表明观察控制是缺失的关键能力。

Conclusion: 动态屏幕环境对 GUI Agent 提出新挑战，观察控制是未来需要重点发展的能力维度。LivingScreen 为这一方向提供了标准化评测平台。

Classification reason: 该论文提出了 LivingScreen，首个面向短视频平台的 GUI Agent 评测基准，属于多模态评测与基准（multimodal_benchmark）范畴。虽然 GUI Agent 本身涉及多模态理解，但论文核心贡献是构建评测环境、任务套件和评估指标，因此归入该 canonical 子主题。

Importance reason: Novel benchmark for dynamic GUI agents, but tangential to user's core focus on continuous multimodal models and generation. Signals: GUI agent benchmark, short-video platform, dynamic screen, observation control, multimodal understanding
...
### [72] [NextMotionQA: Benchmarking and Judging Human Motion Understanding with Vision-Language Models](https://arxiv.org/abs/2606.04773)
...
TL;DR: NextMotionQA 是一个面向 VLM 人体运动理解能力的综合评测基准，包含多项选择问答、视频描述和细粒度纠错三个任务，覆盖三个语义轴和三个难度层级。对12个代表性 VLM 的评估揭示了传统单任务评测无法发现的能力缺口，并验证了 VLM 作为运动质量评判者的有效性边界。


<details>
  <summary>Details</summary>
Motivation: 现有的人体运动理解评测基准存在语义粒度粗糙、难度无区分、标注质量有限以及答案歧义等问题，无法诊断当前模型在哪些方面真正失败。因此需要构建一个更全面、更细粒度的基准来系统评估 VLM 的人体运动理解能力。

Method: 利用 VLM 进行半自动化标注，结合专家验证，构建了 NextMotionQA 基准。包含三个互补任务：多项选择问答、视频描述和细粒度纠错。每个任务沿三个核心语义轴（动作、交互、场景）系统组织，并划分为三个任务复杂度层级。对12个代表性 VLM 进行全面评估，并进一步测试 VLM 作为文本到运动生成质量评判者的能力。

Result: 12个代表性 VLM 的评估揭示了传统单任务评测无法发现的关键能力缺口和弱点。在 VLM 作为评判者的实验中，VLM 在粗粒度标准上与专家评分高度一致（Cohen's κ=0.70），但在细粒度、部件级别的评判上表现崩溃（κ=0.10）。

Conclusion: NextMotionQA 能够有效诊断 VLM 在人体运动理解中的细粒度能力缺陷。VLM 作为运动质量评判者在粗粒度标准上有效，但在细粒度部件级评判上存在明显局限，需要进一步改进。

Classification reason: 该论文提出了 NextMotionQA，一个用于评估 VLM 人体运动理解能力的综合基准，包含多项选择问答、视频描述和细粒度纠错三个任务，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic 'multimodal_benchmark'。

Importance reason: Focuses on human motion understanding benchmark, not directly on user's priority subtopics like continuous language models or video generation. Signals: Human motion understanding benchmark, VLM evaluation, fine-grained error correction, limited personal alignment with priority subtopics
...
### [73] [Agent Planning Benchmark: A Diagnostic Framework for Planning Capabilities in LLM Agents](https://arxiv.org/abs/2606.04874)
...
TL;DR: 本文提出 Agent Planning Benchmark (APB)，一个专门诊断 LLM Agent 规划能力的评测基准，包含 4,209 个多模态案例、22 个领域和 5 种设置。在 12 个 MLLM 上的实验揭示了系统性的规划缺陷，并验证了 APB 引导的优化能提升下游执行指标。


<details>
  <summary>Details</summary>
Motivation: 现有 Agent 评测通常只报告端到端成功率，难以区分失败源于规划还是执行阶段。缺乏专门诊断规划能力的评测基准。

Method: 构建 APB 基准，包含 4,209 个多模态案例，覆盖 22 个领域和 5 种设置：整体规划、反馈条件逐步规划、无关工具鲁棒性、损坏工具鲁棒性、不可解任务鲁棒性。在 12 个 MLLM 上进行系统评测，并在 ToolSandbox 和 τ²-bench 上验证 APB 引导的优化效果。

Result: APB 揭示了 MLLM 在长程规划、工具噪声鲁棒性、校准拒绝和推理时优化方面的系统性弱点。APB 引导的优化在 3 个代表性模型上持续提升了规划正确性、规划等级和下游执行指标。

Conclusion: APB 可作为执行基准的上游诊断补充，帮助识别和改善 LLM Agent 的规划能力缺陷。

Classification reason: 该论文提出了 Agent Planning Benchmark (APB)，一个面向 LLM Agent 规划能力的诊断性评测基准，包含 4,209 个多模态案例，覆盖 22 个领域和 5 种设置。这属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题。

Importance reason: Focuses on agent planning benchmarks rather than core continuous multimodal or generation priorities. Signals: Diagnostic benchmark for LLM agent planning, Multimodal cases but not continuous token or flow matching, No direct alignment with priority subtopics like video generation or world models, Negative keywords not triggered but lacks boost keyword overlap
...
TL;DR: NoRA 是一个第一人称视频基准，用于评估 VLM 在视觉场景中生成合理动作并给出可检查理由的能力，包含 1420 个视频片段，通过动作对齐、事实依据和支持绑定三个维度计算综合合理性分数。评测发现当前 VLM 能恢复合理动作和场景事实，但难以构建完整的合理动作空间并将动作绑定到正确的局部支持上。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么仅在文本中评估规范性判断，要么将问题简化为从固定候选动作中选择。实际场景中智能体需要从零开始识别合理动作，并基于可见事实提供可检查的理由。现有基准无法衡量这种能力。

Method: 构建 NoRA 基准，包含 1420 个第一人称视频片段（HumanGold-190 和 LLMSilver-1230 两个子集），要求模型生成候选下一步动作并通过事实-理由-动作支持图进行论证。通过动作对齐、事实依据和支持绑定三个维度聚合为单一合理性分数。在 12 个多模态系统上采用直接、深思和结构化三种提示策略进行评测。

Result: 当前 VLM 能恢复合理的动作和相关场景事实，但难以构建完整的合理动作空间，且无法将选定动作绑定到正确的局部视觉支持上。NoRA 使这一差距变得可测量。

Conclusion: NoRA 将评估问题从"模型能否选择动作"转变为"模型能否为正确的可见理由证明一个合适动作"，揭示了当前 VLM 在规范性动作推理中的关键不足。

Classification reason: 该论文提出了 NoRA，一个面向第一人称视频的规范性动作推理评测基准，属于多模态评测与基准方向，与 [CANONICAL] subtopic "多模态评测与基准" 高度匹配。

Importance reason: Focuses on normative action reasoning benchmark, not aligned with user's priority on continuous multimodal models or generation. Signals: first-person video benchmark, normative action reasoning, VLM evaluation, grounded reasonableness, not generation or continuous model
...
TL;DR: 本文提出了一个面向主动式程序性辅助的大规模可穿戴第一人称数据集 EgoProactive 和统一基准 Pro²Bench，包含明确的"偏离计划"(OOP)标注和恢复步骤，并提出了解耦的规划器-交互器架构用于实时步骤指导，在多个数据集上超越 GPT、Claude、Gemini 等强基线。


<details>
  <summary>Details</summary>
Motivation: 现有程序性辅助系统缺乏大规模跨域基准来反映真实场景中用户偏离预期步骤序列的常见情况，限制了主动式多模态助手的发展。

Method: (1) 发布 EgoProactive 数据集，包含 OOP 标注和恢复步骤；(2) 将五个已有基准统一为 Pro²Bench；(3) 提出解耦的规划器-交互器架构，专门处理程序状态、视觉线索和恢复注入；(4) 引入跨模型家族的后训练方案。

Result: 训练的 Llama-4 系统在所有六个数据集上显著优于 Claude Opus 4.6、Gemini 3.1 Pro、GPT 5.2 和 Qwen3 VL 235B 等基线；Oracle-plan 实验表明在控制计划质量时，训练的 duplex 模型在 OOP 恢复上取得大幅提升。

Conclusion: 本文通过大规模数据集、统一基准和专用架构推动了主动式程序性辅助的研究，证明了在偏离计划场景下提供高质量实时指导的可行性。

Classification reason: 该论文提出了 EgoProactive 数据集和 Pro²Bench 基准，用于主动式程序性辅助（proactive procedural assistance）的多模态评测，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Strong benchmark and architecture for proactive procedural assistance, but far from user's priority subtopics on continuous models, video generation, or world models. Signals: EgoProactive dataset, Pro²Bench benchmark, decoupled planner-interaction architecture, Out-of-Plan recovery, wearable egocentric video
...
### [76] [M$^3$Eval: Multi-Modal Memory Evaluation through Cognitively-Grounded Video Tasks](https://arxiv.org/abs/2606.05008)
...
TL;DR: M³Eval 是首个系统评估多模态模型记忆能力的综合基准，基于认知心理学设计任务，从记忆保持、信息忠实性和抗干扰性三个维度进行评测。实验发现模型在并行视频流处理、干扰模式、时空记忆源定位和符号记忆方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有视频数据集和基准主要关注感知与推理能力，缺乏对多模态模型记忆能力的系统评估，包括模型记住了什么、信息保存的忠实度以及记忆在干扰下的鲁棒性。

Method: 基于认知心理学理论设计评测框架，构建了针对不同记忆维度（保持、忠实性、抗干扰性）的精心设计的视频任务，对多种代表性多模态模型进行实验。

Result: 模型在处理并行视频流时难以保持分离的表征；干扰模式与人类记忆存在显著差异；空间域的记忆源定位比时间域更可靠；符号记忆能力有限。

Conclusion: 记忆是多模态模型中被忽视但关键的能力，M³Eval 为未来研究提供了有价值的资源，其发现为设计更有效的多模态模型记忆机制提供了启示。

Classification reason: 该论文提出了M³Eval，一个基于认知心理学设计的视频多模态记忆评测框架与基准，属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。

Importance reason: Provides a novel cognitive-psychology-grounded benchmark for multi-modal memory, but is not aligned with user's priority subtopics on generation or continuous models. Signals: Novel memory benchmark, cognitive grounding, reveals model weaknesses in parallel streams and temporal grounding, not in priority subtopics, no boost keywords matched
...
TL;DR: MorphoQuant 是一种模态感知的 PTQ 量化框架，通过分布感知偏置补偿(DABC)和形态导向量化函数优化(MDQFO)解决全模态大语言模型在4-bit量化时跨模态分布异质性和异常值问题，在Qwen2.5-Omni上实现W4A4量化且性能超越W4A16基线。


<details>
  <summary>Details</summary>
Motivation: 现有PTQ方法在全模态大语言模型(OLLM)的4-bit量化中表现不佳，主要因为不同模态（文本、图像、音频等）的分布异质性极大，异常值模式差异显著，导致量化精度严重下降。

Method: 提出MorphoQuant框架，包含两个核心组件：(1) 分布感知偏置补偿(DABC)，选择性地将长尾异常值吸收到通道级偏置中，保护异常值幅度的同时保持密集内点的高精度离散化；(2) 形态导向量化函数优化(MDQFO)，联合优化量化网格与偏置掩码，实现跨模态的细粒度对齐。

Result: 在Qwen2.5-Omni上，W4A4量化模型在ScienceQA上达到76.63%，显著优于现有SOTA W4A4方法，甚至超越W4A16基线，在MMMU、Video-MME等多个基准上均表现优异。

Conclusion: MorphoQuant有效解决了全模态大语言模型量化中的跨模态分布异质性问题，实现了精度与效率的优异权衡，为OLLM的高效部署提供了可行方案。

Classification reason: 该论文提出面向全模态大语言模型(OLLM)的量化方法MorphoQuant，属于多模态模型高效化与轻量化方向，与[CANONICAL] subtopic "多模态高效化与轻量化"（efficient_multimodal）高度匹配。

Importance reason: Directly addresses efficient inference for unified multimodal models, a priority subtopic, with a novel PTQ method achieving strong results on a recent omni-modal LLM. Signals: Modality-aware quantization, 4-bit PTQ for omni-modal LLMs, Distribution-Aware Bias Compensation, Outlier mitigation for multimodal distributions, Strong results on Qwen2.5-Omni
...
### [78] [Video2LoRA: Parametric Video Internalization for Vision-Language Models](https://arxiv.org/abs/2606.04351)
...
TL;DR: Video2LoRA 提出一种参数化视频内化方法：通过感知器超网络从冻结 VLM 编码视频时产生的中间表示中直接预测 LoRA 适配器权重，将视频信息压缩到 LoRA 参数中，使得推理时无需任何视觉 token 即可回答查询，在保持性能的同时将视觉 token 负载降低最多 1500 倍，查询 TTFT 降低 6-80 倍。


<details>
  <summary>Details</summary>
Motivation: VLM 处理视频时每帧占用数百个 token，推理成本随帧数和查询次数线性增长，现有方法难以高效处理长视频和重复查询。

Method: 提出 Video2LoRA 框架：使用感知器超网络（perceiver hypernetwork）读取冻结 VLM 逐层编码视频时产生的中间表示，在单次前向传播中直接生成 LoRA 适配器权重（无需梯度更新）。训练时在视频摘要和字幕任务上优化，使冻结 VLM 仅凭 LoRA 适配器即可回答查询，推理时上下文不含任何视觉 token。

Result: 在 SmolVLM2 500M 和 2.2B 上，Video2LoRA 在所有 5 个字幕基准和 7/8 个视频问答基准上统计上不劣于直接视频上下文推理；支持高达 1024 帧和 1024px 分辨率（直接推理在此范围常退化）；视觉 token 负载减少最多 1500 倍，TTFT 降低 6-80 倍；独立生成的非重叠视频段 LoRA 可在秩空间中组合。

Conclusion: Video2LoRA 通过参数化视频内化实现了高效的视频推理，在极大降低计算开销的同时保持性能，并展示了 LoRA 组合用于长视频内化的潜力。

Classification reason: 该论文提出 Video2LoRA，通过感知器超网络从视频编码中间表示直接生成 LoRA 适配器，将视频信息参数化到 LoRA 权重中，从而在推理时消除视觉 token 开销，显著降低计算成本。这属于多模态高效化与轻量化（efficient_multimodal）范畴，具体涉及 token 压缩、高效推理和参数高效微调。

Importance reason: Directly addresses efficient multimodal inference and continuous representation compression, aligning with top priority subtopics and boost keywords. Signals: LoRA weight prediction from video, zero visual tokens at query time, 1500x token reduction, 6-80x TTFT reduction, stable up to 1024 frames
...
### [79] [From Symbolic to Geometric: Enabling Spatial Reasoning in Large Language Models](https://arxiv.org/abs/2606.04381)
...
TL;DR: 本文提出 Spatial Language Model (SLM)，首个将位置信息作为一等模态的多模态 LLM，能够在推理过程中进行几何空间推理而非仅依赖符号模式匹配。通过构建空间指令数据集和 SpatialEval 基准，实验证明 SLM 在属性、距离、拓扑和相对位置等空间推理任务上显著优于现有基于符号推理的 LLM 方法。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 虽然表面上表现出空间推理能力，但本质上是通过对空间语言的模式匹配进行符号推理，而非真正的几何空间推理。由于 LLM 基于离散 token 操作，缺乏对连续空间表示、显式几何计算和结构化空间算子的原生支持，因此需要一种能够直接操作连续空间表示的多模态模型。

Method: 提出 Spatial Language Model (SLM)，将位置信息作为独立模态融入多模态 LLM 推理过程。构建 Spatial Instruction Dataset 对齐空间表示、原子几何操作和自然语言指令。设计 SpatialEval 基准评估属性、距离、拓扑和相对位置四类空间推理任务。模型直接操作学习到的空间表示而非空间关系的文本描述。

Result: SLM 在 SpatialEval 基准上显著优于现有基于提示工程或文本抽象的 LLM 符号推理方法，在属性、距离、拓扑和相对位置任务上均取得更好表现，证明了集成几何空间表示对鲁棒空间推理的有效性。

Conclusion: 将位置信息作为一等模态并直接操作连续空间表示，能够使 LLM 具备真正的几何空间推理能力，超越传统的符号推理范式。SLM 为空间推理提供了新的方向，相关数据集、基准和模型均已开源。

Classification reason: 该论文提出 Spatial Language Model (SLM)，将位置信息作为一等模态，在 LLM 推理过程中实现几何空间推理。这属于多模态理解与生成方向下的"空间与 3D 多模态推理"（spatial_3d_reasoning）[CANONICAL] 范畴。同时，论文涉及连续空间表示（continuous spatial representations），与连续语言模型方向也有一定关联。

Importance reason: Introduces geometric spatial reasoning as a first-class modality in LLMs, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: spatial reasoning, multimodal LLM, geometric representation, spatial instruction dataset, SpatialEval benchmark
...
TL;DR: 本文提出了UniCAD，一个面向多模态CAD学习的统一基准，涵盖点云到CAD重建、文本/图像到CAD生成和CAD问答等任务；同时提出了UniCAD-MLLM，一个统一的多模态大语言模型，能够端到端地处理文本、图像、草图和点云输入，在单一框架内完成上述异构任务，在UniCAD和Fusion360基准上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: CAD研究通常将各个任务孤立地研究，缺乏统一的多模态多任务基准，阻碍了多模态多任务学习在CAD领域的发展。

Method: 构建了UniCAD基准数据集，包含点云到CAD重建、文本/图像到CAD生成、CAD问答等任务的多模态数据。提出了UniCAD-MLLM，一个统一的多模态大语言模型，能够同时处理文本、图像、草图和点云输入，在单一框架内端到端地执行多种CAD任务。

Result: 在UniCAD和Fusion360基准上，UniCAD-MLLM在所有任务上均取得了最先进的性能，超越了现有的任务特定模型和多任务基线。

Conclusion: UniCAD填补了CAD领域缺乏统一多模态多任务基准的空白，UniCAD-MLLM证明了统一模型在CAD多任务学习中的有效性，将开源数据集、代码和预训练模型以促进未来研究。

Classification reason: 该论文提出了一个统一的多模态CAD基准（UniCAD）和一个统一的多模态大语言模型（UniCAD-MLLM），能够端到端地处理文本、图像、草图、点云等多种输入模态，并同时完成点云到CAD重建、文本/图像到CAD生成、CAD问答等多项任务。这属于理解与生成一体化的统一多模态模型，因此映射到[multimodal_understanding_generation]方向下的[unified_multimodal_model]（统一多模态模型）子主题。

Importance reason: Strong benchmark and unified MLLM for CAD, but CAD is a niche domain not aligned with user's priority subtopics. Signals: Unified multimodal benchmark, CAD-specific domain, MLLM for heterogeneous tasks, SOTA on Fusion360, No direct match to priority subtopics
...
### [81] [Geometry-Preserving Unsupervised Alignment for Heterogeneous Foundation Models](https://arxiv.org/abs/2606.04385)
...
TL;DR: 本文提出 GPUA（Geometry-Preserving Unsupervised Alignment），一种无监督框架，通过正交映射将视觉基础模型（VFM）的特征空间对齐到视觉语言模型（VLM）的语义空间，无需标签或模型参数更新，保留几何结构，提升跨模型兼容性，在零样本识别和分割任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 视觉基础模型存在两大范式：VLM（如 CLIP）具有语言语义对齐能力但视觉表征粗糙，VFM（如 DINOv2）学习判别性感知几何但缺乏语义基础。两者互补但难以兼容，现有方法需要标签或微调，成本高且不通用。作者希望无监督、无需参数更新地将 VFM 特征对齐到 VLM 语义空间，实现两者优势互补。

Method: 受跨语言对齐启发，GPUA 将 VFM 特征视为一种"视觉语言"，学习一个正交映射矩阵将 VFM 空间转换到 VLM 语义空间。该方法仅需特征级访问预训练模型，无需标签或更新模型参数，通过保持几何结构的正交变换实现无监督对齐，且任务无关。

Result: 在多个基准上，GPUA 显著提升了跨模型兼容性，在零样本识别和分割等下游任务中取得强增益，且计算开销极低。

Conclusion: GPUA 有效弥合了 VFM 与 VLM 之间的模态鸿沟，实现了两种范式优势的无监督融合，为异构基础模型的对齐提供了一种轻量、通用且高效的解决方案。

Classification reason: 该论文提出 GPUA 框架，将视觉基础模型（VFM）的特征空间对齐到视觉语言模型（VLM）的语义空间，属于视觉语言模型的特征对齐与兼容性增强工作，与 [CANONICAL] 的 vlm_mllm_general（视觉语言模型与 MLLM）主题高度相关。

Importance reason: Focuses on aligning VFM and VLM features, which is not a core priority subtopic and lacks direct connection to continuous multimodal models or video generation. Signals: Unsupervised alignment, geometry-preserving, zero-shot recognition, cross-model compatibility, no model update
...
TL;DR: 本文提出 Audio Interaction Model（音频交互模型），将离线大音频语言模型与流式音频模型统一为在线实时交互模型。通过 SoundFlow 框架（流式数据构建、理解感知训练、异步低延迟推理）和 StreamAudio-2M 数据集，实现了实时 ASR、流式音频指令跟随和主动干预等能力，在 8 个基准上保持主流音频任务性能的同时解锁了离线模型无法实现的功能。


<details>
  <summary>Details</summary>
Motivation: 现有大音频语言模型（LALM）均为离线模型，无法实时交互；而流式音频模型各自只处理单一任务（如流式 ASR 或语音对话）。亟需一个统一的在线 LALM，通过始终开启的"感知-决策-响应"循环，实时监听声音、环境和指令并即时反应。

Method: 提出 Audio-Interaction 统一流式模型，包含：(1) SoundFlow 框架，端到端实现感知-决策-响应循环，包括流式原生数据构建、理解感知训练和异步低延迟推理；(2) StreamAudio-2M 数据集，包含 260 万条流式数据，覆盖 7 种基础能力和 28 个子任务；(3) Proactive-Sound-Bench 用于评估主动音频干预能力。模型在保持离线任务性能的同时，新增实时 ASR、流式音频指令跟随和主动帮助等在线能力。

Result: 在 8 个基准上，Audio-Interaction 在主流音频任务上保持有竞争力的性能，同时解锁了离线 LALM 无法实现的能力，包括实时 ASR、流式音频指令跟随和主动干预。

Conclusion: 本文首次将离线 LALM 与流式音频模型统一为在线交互模型，证明了通过流式原生数据构建和理解感知训练，可以在不牺牲离线任务性能的前提下实现实时音频交互，为下一代音频交互系统奠定了基础。

Classification reason: 该论文提出统一的流式音频语言模型（Audio-Interaction），属于多模态大语言模型（MLLM）在音频模态的扩展，与 [CANONICAL] 子主题 "视觉语言模型与 MLLM"（vlm_mllm_general）高度匹配，因为该子主题涵盖通用 MLLM 架构与多模态指令微调，音频语言模型是其自然延伸。

Importance reason: Unifies offline and streaming audio models into an online interactive system, advancing multimodal understanding and generation. Signals: unified multimodal model, streaming audio interaction, real-time ASR, proactive audio intervention, SoundFlow framework
...
### [83] [Stateful Visual Encoders for Vision-Language Models](https://arxiv.org/abs/2606.04433)
...
TL;DR: 本文提出 Stateful Visual Encoder（状态化视觉编码器），通过在 VLM 的视觉编码器中引入跨图像的状态记忆机制，使每个图像的视觉表征能够感知先前的视觉上下文，从而提升多图像、多轮交互场景下的视觉比较与变化检测能力。


<details>
  <summary>Details</summary>
Motivation: 现有开源 VLM 的视觉编码器是无状态的——每张图像独立编码，无法访问先前的视觉上下文。在多图像、多轮 agentic 场景中，任务关键的小变化（如医学影像中的细微病变）可能在进入语言模型前就被衰减或丢失，导致模型无法有效进行跨图像比较。

Method: 提出 Stateful Visual Encoder，在视觉编码器中引入状态机制：将先前图像的视觉特征作为条件输入到当前图像的编码过程中，使每个视觉表征都包含历史视觉上下文信息。该方法通过监督微调（SFT）集成到现有 VLM 中，兼容不同分辨率、语言模型大小和 VLM 骨干网络。

Result: 在跨图像空间聚合、多目标视觉差异比较和视觉轨迹行为克隆等受控任务上取得一致改进。在真实场景（纵向放射学影像、细粒度图像比较、遥感）中，状态化编码器持续提升通用 VLM 基线性能，并在部分领域达到或超越专用模型。

Conclusion: 状态化视觉编码器是一种简单有效的 VLM 架构改进，通过赋予视觉编码器跨图像记忆能力，显著提升多图像场景下的视觉比较与变化检测性能，且具有良好的通用性和可扩展性。

Classification reason: 该论文提出 Stateful Visual Encoder，通过引入状态机制使视觉编码器能够感知历史视觉上下文，属于视觉语言模型（VLM）架构改进。这与 [CANONICAL] 子主题 vlm_mllm_general（视觉语言模型与 MLLM）高度匹配。

Importance reason: Novel stateful visual encoder improves multi-image VLM reasoning but is not directly aligned with top priority continuous/multimodal latent modeling or generation subtopics. Signals: stateful visual encoder, cross-image memory, VLM improvement, multi-image reasoning, not continuous token or generation focus
...
### [84] [Hyper-ICL: Attention Calibration with Hyperbolic Anchor Distillation for Multimodal In-Context Learning](https://arxiv.org/abs/2606.04434)
...
TL;DR: Hyper-ICL 提出一种轻量级训练框架，通过低秩 logit 级适配器校准注意力分布、查询自适应调制机制和双曲锚点蒸馏损失，实现无需演示样本的多模态上下文学习，在多个基准上提升准确率和稳定性。


<details>
  <summary>Details</summary>
Motivation: 多模态上下文学习（ICL）虽灵活，但推理延迟高且对演示样本的格式、顺序和内容敏感，稳定性差。现有方法依赖推理时提供演示样本，效率低下。

Method: 1) 参数高效的低秩 logit 级适配器，校准注意力分布以匹配演示诱导的注意力重分布；2) 查询自适应调制机制，根据当前查询在 token 级别自适应控制各层各头的干预强度；3) 层间双曲锚点蒸馏损失，通过 Lorentz 测地距离对齐学生中间特征与演示条件教师特征，重建演示-查询关系。

Result: 在 VQAv2、OK-VQA、COCO Caption 等六个多模态基准上，Hyper-ICL 一致优于 vanilla ICL 和现有 SOTA 方法，在准确率和稳定性上均有提升。

Conclusion: Hyper-ICL 通过轻量级训练框架有效消除了推理时对演示样本的依赖，同时提升了多模态 ICL 的性能和稳定性，为高效多模态上下文学习提供了新思路。

Classification reason: 该论文研究多模态上下文学习（Multimodal ICL），属于多模态大语言模型（MLLM）的推理范式改进，与 [CANONICAL] 的 vlm_mllm_general（视觉语言模型与 MLLM）高度匹配。

Importance reason: Focuses on multimodal ICL efficiency but lacks direct connection to priority continuous language/video generation models. Signals: Multimodal ICL, attention calibration, hyperbolic distillation, low-rank adapter, demonstration-free
...
### [85] [Beyond Prompt-Based Planning: MCP-Native Graph Planning-based Biomedical Agent System](https://arxiv.org/abs/2606.04494)
...
TL;DR: BioManus 是一个基于 MCP 原生架构的生物医学智能体系统，通过将异构生物信息学工具编译为标准 MCP 服务器并构建类型化异构图，实现图结构化的操作级工作流规划，显著提升了执行准确率和上下文效率。


<details>
  <summary>Details</summary>
Motivation: 当前生物医学智能体面临两大瓶颈：生物信息学工具接口和执行环境高度异构，且智能体规划仍依赖扁平化的提示词检索工具描述。随着工具生态增长，工具覆盖与上下文大小之间的耦合导致工具混淆、规划不稳定和执行效率低下。

Method: BioManus 首先引入 BioinfoMCP Compiler，将异构生物信息学软件编译为标准 MCP 服务器，构建可执行的 MCP 生态系统。然后将该生态系统组织为类型化异构图（MCP 图），包含工具、操作、数据类型和工作流阶段等节点。推理时，检索紧凑的任务特定子图，合成操作级工作流骨架，将规划复杂度与原始工具库存大小解耦。

Result: 在 BioAgentBench 和 LAB-Bench 基准上，BioManus 在执行准确率、工作流有效性和上下文效率方面均优于先进的生物医学智能体基线。理论分析表明，在高召回检索下实现了 Θ(N/(h·m̄)) 的上下文压缩比。

Conclusion: 可扩展的生物医学推理需要结构化的可执行能力图，而非越来越大的提示级工具检索。这项工作标志着从基于提示的规划向图结构规划范式的转变。

Classification reason: 该论文提出 BioManus，一个基于 MCP（Model Context Protocol）的生物医学智能体系统，属于多模态理解与生成方向下的通用 MLLM/智能体架构。虽然论文聚焦生物医学领域，但其核心贡献在于图结构化的工具规划范式，属于 VLM/MLLM 智能体系统的范畴，因此映射到 [CANONICAL] subtopic "vlm_mllm_general"。

Importance reason: Focuses on biomedical agent tool orchestration, not on the user's priority subtopics like continuous language models or video generation. Signals: MCP-native architecture, graph-based planning, biomedical domain, context compression, tool heterogeneity
...
TL;DR: MAOAM 是一个基于 VLM 的统一图像选择框架，支持通过文本或点击交互进行物体级和材质级的分割，并利用多任务训练和辅助 VQA 任务提升材质理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于 VLM 的图像选择方法仅支持物体级选择且交互模态单一（仅文本或仅点击），无法满足材质级选择（如选择所有木质表面）的需求，限制了交互式图像编辑的实用性。

Method: MAOAM 采用 VLM 加分割头的架构：VLM 编码用户意图（物体或材质）及视觉语义，分割头将输出 token 解码为像素级掩码。提出可扩展的数据生成流程，收集真实与合成图像的材质掩码，并用 VLM 生成富含视觉语义的材质描述。采用多任务训练，同时优化点击和文本选择，并引入辅助 VQA 任务增强材质理解。

Result: 实验表明 MAOAM 在多种物体、材质和交互场景下均能生成准确且一致的分割结果。尽管仅用单模态提示训练，模型在推理时结合文本和点击交互展现出涌现性的性能提升。

Conclusion: MAOAM 统一了物体和材质级选择，支持文本和点击两种交互模态，为交互式图像编辑提供了更灵活、更鲁棒的解决方案。

Classification reason: 该论文提出 MAOAM，利用 VLM 进行图像中物体和材质的交互式选择（分割），属于通用视觉语言模型的应用，因此映射到 [CANONICAL] subtopic "vlm_mllm_general"（视觉语言模型与 MLLM）。

Importance reason: Focuses on VLM-based selection for objects and materials, but lacks direct connection to user's priority subtopics like continuous models or autoregressive generation. Signals: VLM-based segmentation, material selection, multi-task training, text and click interaction, lacks autoregressive or continuous model focus
...
### [87] [Geometry-Aware Distillation for Prompt Tuning Biomedical Vision-Language Models](https://arxiv.org/abs/2606.04922)
...
TL;DR: 提出 Omni-Geometry Knowledge Distillation (OGKD) 框架，通过将类别关系结构注入教师模型，生成保留类别间几何关系的方向性目标，并设计全局几何感知蒸馏 (GAD) 和标签引导几何蒸馏 (LGD) 两种损失，在 11 个医学数据集上显著提升 VLM 提示调优性能。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 提示调优方法仅优化真实类别，将所有其他类别视为同等错误，忽略了医学影像中具有临床意义的类别间关系，导致在标注有限的情况下决策边界不稳定。

Method: 提出 OGKD 框架：(1) 将类别关系结构（如层次结构、相似性）注入教师模型，生成保留真实标签同时尊重类别间几何关系的方向性目标；(2) 全局几何感知蒸馏 (GAD) 作用于全局图像 token；(3) 标签引导几何蒸馏 (LGD) 将相同几何结构应用于注意力 patch token，提升细粒度对齐。

Result: 在 11 个医学数据集上的 base-to-novel 和 few-shot 评估中，OGKD 平均绝对准确率提升 1.7%-2.8%，优于所有现有 SOTA VLM 适配方法，且对未见类别具有良好泛化能力。

Conclusion: OGKD 通过注入类别几何关系知识，有效提升了 VLM 在医学影像少样本场景下的适应性能与可靠性，为临床敏感数据场景下的 VLM 微调提供了新范式。

Classification reason: 该论文研究医学视觉语言模型的提示调优方法，属于通用 VLM 适配与微调范畴，与 [CANONICAL] subtopic "vlm_mllm_general（视觉语言模型与 MLLM）" 高度匹配。

Importance reason: Focuses on medical VLM prompt tuning, which is outside your priority subtopics and has negative keyword overlap. Signals: Medical imaging, Prompt tuning, Knowledge distillation, Class-relation geometry, Limited supervision
...
TL;DR: BabyCL 是一个持续多模态学习框架，以单次时间顺序处理 SAYCam 儿童自我中心视频数据集，结合流式视觉表征学习与图像-文本对比目标，在接近儿童真实体验的训练条件下学习词-指代物映射，显著优于流式学习基线并缩小了与离线训练的差距。


<details>
  <summary>Details</summary>
Motivation: 儿童从连续的、时间结构化的自我中心经验流中学习词义，而现有神经网络方法通常对数据进行数百轮随机打乱训练，与儿童实际学习方式不符。作者希望构建一个更贴近儿童真实体验的持续学习框架，验证在接近真实条件下能否学习到有意义的词-指代物映射。

Method: BabyCL 框架包含三个关键组件：(1) 多阶段时间分割，将连续视频流分割为时间窗口；(2) 双回放缓冲区，分别管理视觉和多模态历史；(3) 共享骨干网络上的三个对比损失联合训练（视觉对比、多模态对比等）。整个框架以单次时间顺序处理 SAYCam 数据集，不进行数据打乱。

Result: 在 SAYCam Labeled-S 4AFC 基准上，BabyCL 在匹配优化预算下显著优于流式学习基线，大幅缩小了与离线训练上限的差距。消融实验表明，在线时间分割窗口长度和回放缓冲区驱逐策略对性能增益具有鲁棒性。

Conclusion: 有意义的词-指代物映射可以在更接近儿童实际体验的训练条件下涌现，表明持续多模态学习范式是研究儿童语言习得的有效计算模型。

Classification reason: 该论文提出 BabyCL，一个从儿童自我中心视频流中持续学习视觉-语言映射的框架，属于多模态理解与生成方向下的视觉语言模型（VLM）范畴。虽然强调持续学习，但核心是视觉-语言对比学习与多模态表征，与 vlm_mllm_general 主题高度匹配。

Importance reason: Strong continual multimodal learning framework but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: continual multimodal learning, egocentric video, streaming visual representation, contrastive learning, replay buffer
...
### [89] [BreastGPT: A Multimodal Large Language Model for the Full Spectrum of Breast Cancer Clinical Routine](https://arxiv.org/abs/2606.04911)
...
TL;DR: 本文提出 BreastGPT，一个面向乳腺癌临床全流程（筛查、诊断、治疗规划）的统一多模态大语言模型。作者首先构建了 BreastStage 指令语料库（186万指令对，涵盖5种影像模态、17个子数据集、136个任务模板）及其评测基准 BreastStage-Bench。BreastGPT 采用双分支视觉编码器和概念保持的 token 压缩技术，弥合标准放射影像与千兆像素病理图像之间的尺度差异，在基准上达到75.66%的封闭式准确率和89.92%的开放式评分，超越通用和医学专用 MLLM。


<details>
  <summary>Details</summary>
Motivation: 现有医学 MLLM 受限于数据稀缺和模型通用性不足，通常仅在孤立模态或狭窄任务族上评测，无法支持临床工作流级别的多模态推理。乳腺癌临床管理涉及筛查、诊断、治疗规划等多个阶段，每个阶段需要不同的影像模态、任务目标和推理模式，亟需一个工作流对齐的数据集和统一模型。

Method: 1) 构建 BreastStage 语料库：从5种影像模态（如乳腺X线、超声、MRI、病理等）的17个子数据集中，基于136个任务模板生成186万指令对，并划分出评测基准 BreastStage-Bench。2) 提出 BreastGPT：采用双分支视觉编码器（分别处理放射影像和病理图像），结合概念保持的 token 压缩技术，弥合标准分辨率与千兆像素病理图像之间的尺度差异，统一输入到 LLM 进行多模态推理。

Result: 在 BreastStage-Bench 上，BreastGPT 在封闭式任务上达到75.66%准确率，开放式任务上达到89.92%评分，在临床各阶段和任务格式上均超越通用 MLLM（如 GPT-4o）和医学专用 MLLM。

Conclusion: 工作流对齐的数据和跨尺度视觉建模对于构建临床落地的医学 MLLM 至关重要。BreastGPT 证明了统一多模态模型在乳腺癌全流程临床推理中的潜力，所有数据、代码和模型均已开源。

Classification reason: 该论文构建了乳腺癌临床全流程的多模态指令数据集 BreastStage 和评测基准 BreastStage-Bench，并提出了统一 MLLM BreastGPT，涵盖筛查、诊断、治疗规划各阶段的视觉问答与医学影像理解任务。这与 [CANONICAL] subtopic "multimodal_vqa_doc"（视觉问答与文档理解）高度匹配，因为论文核心是医学领域的 VQA 和多模态理解，且涉及多种影像模态的问答推理。

Importance reason: Focuses on a narrow medical domain (breast cancer) with no overlap with user's priority subtopics or boost keywords. Signals: Medical MLLM, breast cancer clinical workflow, dual-branch visual encoder, token compression, BreastStage dataset
