...
- [多模态生成](#direction-multimodal_generation) [Total: 65]
...
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 62]
  ...
  - [多模态幻觉与可靠性](#direction-multimodal_understanding_generation-multimodal_hallucination) [Total: 7]
...
Motivation: 现有的等变表示学习方法在构建潜空间时，往往存在潜空间错位（latent misalignment）问题——即理论上预期的群作用与实际需要的变换之间存在不一致，导致学到的潜表示无法一致地保持群对称性所施加的等变关系，进而影响新视角合成的质量。

Method: 提出 Residual Latent Flow，一个基于流匹配（flow matching）的框架，通过学习残差流来修正错位的潜表示，使其更好地满足底层等变关系。该方法在旋转群 SO(n) 下进行建模，通过流匹配对潜空间中的变换进行校正。

Result: 实验表明，该方法显著减少了潜空间错位，并在旋转群 SO(n) 下的新视角合成任务中提升了合成质量和几何一致性。

Conclusion: Residual Latent Flow 通过流匹配有效解决了等变潜空间中的对齐问题，为几何感知的生成模型提供了更鲁棒的潜表示，提升了新视角合成的保真度和一致性。

Classification reason: 该论文提出基于流匹配的等变潜空间对齐方法，用于改进新视角合成（novel view synthesis）的质量和几何一致性。新视角合成是3D生成与重建的核心任务之一，因此映射到 canonical subtopic "3D 生成与重建"。

Importance reason: Focuses on 3D equivariant latent alignment for novel view synthesis, not directly on the user's priority subtopics like continuous VLM or autoregressive video generation. Signals: Equivariant representation learning, Flow matching for latent correction, Novel view synthesis, Group symmetries SO(n), Latent misalignment problem
...
TL;DR: Function2Scene 提出从功能规格（自然语言设计简报）生成3D室内布局的框架，通过解析用户角色和活动，从17条功能设计约束分类中推导约束，并利用工具增强的检查-修复循环（结合几何测量、LLM推理和VLM视觉评估）迭代优化布局，在专业室内设计案例上优于现有LLM基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动的3D室内场景合成方法大多基于物体中心提示（"放什么家具"），而非关注空间如何被使用。然而在真实室内设计中，布局的好坏取决于其对使用者活动与物理需求的支持程度。作者希望将室内场景生成从"放置合理的物体"转向"设计支持人类使用的空间"。

Method: Function2Scene 包含三个核心模块：(1) 功能解析器，从自然语言设计简报中解析 occupant 角色和活动；(2) 功能约束推导器，从包含17条空间、人体工学、活动和环境标准的分类学中推导定制化功能设计约束；(3) 布局生成与优化模块，通过工具增强的检查-修复循环（check-and-repair loop），结合几何测量、LLM上下文推理和VLM视觉评估，迭代评估和优化布局。

Result: 在30个专业编写的室内设计案例上，Function2Scene 生成的布局在满足功能需求方面优于最近的LLM-based场景合成基线，在94.3%的成对比较中被人类评估者偏好。

Conclusion: Function2Scene 成功将文本驱动的室内场景合成从"放置合理物体"重新定义为"设计支持人类使用的空间"，通过功能规格驱动的约束和迭代优化，显著提升了布局的功能合理性。

Classification reason: 该论文研究从功能规格（自然语言设计简报）生成3D室内场景布局，属于3D场景生成范畴，与[CANONICAL] subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Strongly aligns with multimodal generation and VLM-based iterative refinement, a key priority for your daily deep read. Signals: multimodal generation, VLM visual assessment, LLM reasoning, 3D layout generation, functional design constraints
...
TL;DR: 本文发现，在自然图像和视频上大规模预训练的 Foundation VAE，其编码器和解码器冻结后，可作为 CT 重建、增强和生成的统一接口。该方法在保持解剖结构的同时抑制采集噪声，训练分割模型时表面精度提升 3.9% NSD；在潜空间中训练条件潜扩散模型，FVD 降低 3.9%，CT CLIP 分数提升 36.2%，18 种疾病的生成忠实度 AUC 提升 2.76%。


<details>
  <summary>Details</summary>
Motivation: 训练 CT 专用的 VAE 需要大量计算和工程成本，且在不同扫描仪、协议和疾病下性能容易退化。作者希望利用已有的自然图像/视频预训练 VAE，无需微调即可作为 CT 三维数据的统一表示接口，实现零训练成本的 CT 重建、增强与生成。

Method: 作者观察到自然图像/视频预训练的 Foundation VAE（如 Stable Diffusion 的 VAE）可以直接处理 CT 三维体素数据。将 CT 切片堆叠为三维张量输入冻结的 VAE 编码器得到紧凑潜变量，再通过冻结的解码器重建。在潜空间中训练条件潜扩散模型（LDM）进行 CT 生成。整个过程 VAE 参数完全冻结，仅训练扩散模型。

Result: Foundation VAE 重建的 CT 体积在保持解剖结构的同时有效抑制噪声；基于重建数据训练的分割模型在胰腺肿瘤和肺肿瘤上表面精度（NSD）平均提升 3.9%；在潜空间中训练的 LDM 相比基线 FVD 降低 3.9%，CT CLIP 分数提升 36.2%，18 种多疾病生成的 AUC 提升 2.76%。

Conclusion: 自然图像/视频预训练的 Foundation VAE 可以作为 CT 三维数据的通用表示接口，实现无需训练的 CT 重建、增强与生成，为医学影像的表示复用和忠实生成提供了可扩展的实用方案。

Classification reason: 该论文研究基于 Foundation VAE 的 CT 三维体素重建、增强与生成，属于 3D 医学影像生成与重建范畴。现有 canonical subtopic "3D 生成与重建"（three_d_generation）涵盖 3D 物体/场景生成与重建，CT 体积重建是 3D 重建的典型应用，因此映射至此。

Importance reason: Focuses on medical CT reconstruction, which is outside the user's priority subtopics and has negative keyword "medical diagnosis". Signals: Foundation VAE for CT, frozen encoder/decoder, reconstruction augmentation generation, surface accuracy improvement, conditional latent diffusion
...
TL;DR: 本文系统比较了多种单步修复方法在多物体 3DGS 场景中的物体移除与修复效果，发现基于重建的 2D 修复器在 3D 一致性上优于生成式扩散模型，从头初始化场景比微调现有场景效果更好，并提出了一个新的多物体场景数据集用于评估。


<details>
  <summary>Details</summary>
Motivation: 3DGS 场景中的物体移除和修复面临跨视角 3D 一致性的挑战，现有 360° 数据集缺乏真实地面真值和遮挡场景，亟需系统性的基准测试来评估不同 2D 修复器在 3D 领域的适用性。

Method: 比较多种 2D 修复器（重建型 vs 生成式扩散模型）在 3DGS 场景中的表现，集成到不同的单步方法中（从头初始化场景 vs 微调现有场景），并引入一个新的多物体场景数据集（含真实地面真值和多视角遮挡）。

Result: 重建型修复器在 3D 一致性上优于生成式扩散模型；从头初始化场景比微调现有场景质量更高；在 3D 修复前先进行物体移除至关重要。

Conclusion: 为 3DGS 场景修复提供了系统的基准评估，揭示了不同 2D 修复器和修复策略的优劣，为后续 3D 场景编辑研究提供了参考。

Classification reason: 论文研究 3D Gaussian Splatting 场景中的物体移除与修复（inpainting），属于 3D 场景生成与重建范畴，与 canonical subtopic "3D 生成与重建" 高度匹配。

Importance reason: Focuses on 3DGS inpainting benchmarks, not on the user's priority subtopics like continuous VLM, autoregressive video/image generation, or world models. Signals: 3D Gaussian Splatting, object removal, inpainting benchmark, reconstruction-based vs. generative, multi-object scene dataset
...
TL;DR: 本文提出 SHELLS，一种高效的前馈式多视图3D头部重建框架。通过分层采样策略将特征提取与网格分辨率解耦，利用DINOv2+LoRA提取多视图特征，先预测稀疏粗网格，再构建分层表面感知采样壳进行精细重建。相比体积基线方法，GPU内存减少88%（2.4GB vs 20GB），注册误差降低21%-29%，推理速度提升3.5倍，且仅用合成数据训练即可泛化到真实场景。


<details>
  <summary>Details</summary>
Motivation: 现有3D头部重建方法通常通过局部特征体独立优化顶点，导致内存消耗与网格分辨率强耦合，难以扩展到高密度拓扑（>10k顶点），且易产生表面噪声。同时，现有方法依赖昂贵的预注册多视图数据集。因此需要一种高效、可扩展且能泛化到真实场景的3D头部重建方法。

Method: SHELLS 采用分层采样策略：1）使用DINOv2骨干网络+LoRA适配提取多视图特征；2）投影采样稀疏全局特征云，预测中间粗网格作为先验；3）基于粗网格构建分层的表面感知采样壳（layered surface-aware sampling shells），作为精细重建的离散搜索空间；4）在采样壳上进行局部特征采样，预测最终高密度网格。整个过程为前馈式，无需测试时优化。

Result: 在18k顶点网格上，SHELLS相比体积基线方法：GPU内存减少88%（2.4GB vs 20GB），中位注册误差降低21%-29%，推理速度提升3.5倍（0.08s vs 0.29s）。模型仅用合成数据训练，即可有效泛化到真实世界拍摄数据。

Conclusion: SHELLS通过粗到细的分层表面采样策略，成功解耦了特征提取与网格分辨率，实现了高效、可扩展的多视图3D头部重建。该方法在显著降低计算开销的同时提升了重建精度，且摆脱了对昂贵预注册真实数据集的依赖，为3D头部重建的实际应用提供了可行方案。

Classification reason: 该论文提出从多视图图像进行3D头部重建的feed-forward框架，属于3D物体/场景生成与重建范畴，与[CANONICAL] subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Efficient 3D head reconstruction with strong synthetic-to-real generalization, but outside user's priority subtopics and lacks autoregressive/diffusion video generation focus. Signals: Efficient feed-forward 3D reconstruction, 88% GPU memory reduction, synthetic-to-real generalization, DINOv2+LoRA feature extraction, coarse-to-fine layered sampling
...
Importance: 高优先级 (89.33) - 未入选精读

TL;DR: VolFill 提出一种基于体积流匹配的生成式框架，从单张 RGB 图像重建完整 3D 场景几何。该方法使用混合 3D VAE 将稀疏截断有符号距离函数网格压缩为紧凑潜空间，再通过潜扩散 Transformer 去噪恢复完整场景，并利用几何基础模型提供空间先验。在 SCRREAM 和 NRGB-D 数据集上显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 从单张 RGB 图像重建完整 3D 场景几何极具挑战性，尤其是推断被遮挡的隐藏结构。现有方法受限于逐射线约束或非结构化点云查询，难以恢复完整的场景几何。

Method: 提出 VolFill 生成式框架：1）混合 3D VAE 将稀疏截断有符号距离函数（T-UDF）网格压缩为紧凑潜空间；2）潜扩散 Transformer 在潜空间中去噪以恢复完整场景；3）以几何基础模型（geometry foundation models）提取的空间先验作为条件，引导生成过程；4）输出结构化表示，支持直接表面提取和大规模占用查询。

Result: 在 SCRREAM 和 NRGB-D 数据集上的大量实验表明，VolFill 在完整 3D 场景重建任务上显著优于现有基线方法。

Conclusion: VolFill 为单视图完整 3D 场景重建提供了一种鲁棒的生成式解决方案，为整体空间理解奠定了坚实基础。

Classification reason: 该论文提出 VolFill，从单张 RGB 图像重建完整 3D 场景几何，使用 3D VAE 和扩散 Transformer 生成完整场景的 3D 结构。这属于 3D 场景生成与重建范畴，与 canonical subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Strong 3D reconstruction method using flow matching and VAE latent space, but not directly aligned with top priority subtopics. Signals: 3D scene reconstruction, volumetric flow matching, hybrid 3D VAE, latent diffusion transformer, geometry foundation model
...
TL;DR: RayDer 是一个统一的、前馈式 Transformer 架构，将相机估计、场景重建和渲染整合到单一骨干网络中，将自监督新视角合成转化为一个良态的单模型缩放问题。通过引入最小动态状态吸收时变内容，实现在无约束真实世界视频上的稳定训练，并展现出清晰的数据与计算幂律缩放行为，零样本性能可媲美有监督方法。


<details>
  <summary>Details</summary>
Motivation: 自监督新视角合成（NVS）虽然可以利用大量视频数据，但训练过程对真实视频的鲁棒性差，且多网络系统设计的缩放行为难以预测。现有方法缺乏一个统一的可扩展框架。

Method: 提出 RayDer，一个统一的前馈 Transformer，将相机估计、场景重建和渲染合并为单一骨干网络。引入最小动态状态（minimal dynamic state）作为干扰因子，吸收时变内容（如运动、光照变化），使模型能在无约束真实视频上稳定训练。动态内容仅作为可扩展的监督信号，不参与动态场景重建，保持静态场景 NVS 作为目标任务。

Result: 在多个模型规模和数量级的数据上，RayDer 展现出清晰的数据与计算幂律缩放行为，优于静态场景数据混合。在大量基准测试上，零样本开放集性能与有监督 SOTA 方法相当。

Conclusion: RayDer 证明了将自监督 NVS 转化为单一模型缩放问题的可行性，通过统一架构和最小动态状态设计，实现了可扩展、稳定的训练，并取得了与有监督方法竞争的性能。

Classification reason: 该论文研究从真实世界视频中进行自监督新视角合成（Novel View Synthesis），属于 3D 场景重建与渲染的核心问题，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Strong scaling laws and unified architecture for NVS, but tangential to user's core focus on autoregressive/diffusion video generation and continuous VLM. Signals: self-supervised novel view synthesis, unified transformer, power-law scaling, zero-shot NVS, real-world video training
...
### [8] [Smaller and Faster 3DGS via Post-Training Dictionary Learning](https://arxiv.org/abs/2605.30396)
...
TL;DR: 本文提出首个基于字典学习的 3DGS 后训练压缩框架，无需重新训练即可压缩任意 3DGS 模型，在 13 个基准场景上平均压缩 3.95x 并提升渲染速度约 24%，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 3DGS 模型在实时渲染方面表现出色，但训练后的模型内存占用大，难以部署在资源受限设备上。现有压缩技术往往引入额外可训练参数，且压缩后图像质量下降明显。

Method: 提出基于字典学习的后训练压缩框架，对已训练好的 3DGS 模型进行压缩，无需重新训练或修改模型架构。通过字典学习对高斯属性进行编码，实现高效表示。

Result: 在 13 个基准场景上，对 3DGS、3DGS-MCMC 和 PixelGS 分别实现平均 3.95x、3.10x 和 4.55x 的压缩比，同时渲染速度分别提升 23.3%、24.3% 和 25.3%，图像质量保持良好。

Conclusion: 字典学习方法为 3DGS 提供了一种简单高效、即插即用的后训练压缩方案，在压缩率、渲染速度和图像质量之间取得了良好平衡，有助于 3DGS 在轻量级设备上的部署。

Classification reason: 该论文研究 3D Gaussian Splatting 模型的压缩方法，属于 3D 场景表示与渲染的范畴，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Focuses on 3DGS compression, not directly aligned with user's priority subtopics like continuous VLM or autoregressive video generation. Signals: 3DGS compression, dictionary learning, post-training, rendering speedup, no re-training needed
...
### [9] [DSD-GS: Dynamic-Static Decomposition of Gaussian Splatting for Efficient and High-Fidelity Dynamic Scene Reconstruction](https://arxiv.org/abs/2605.30863)
...
TL;DR: DSD-GS 提出一种基于动态-静态分解的高斯泼溅框架，利用前馈高斯泼溅编码器和光流模型将场景分为静态和动态组件，仅对动态部分建模，从而在动态场景重建中实现高效训练（10分钟）、高渲染质量（>700 FPS）和存储效率，无需 COLMAP 预处理。


<details>
  <summary>Details</summary>
Motivation: 现有动态 3DGS 方法将所有高斯体素视为动态组件，导致计算效率低下；而基于分解的方法虽有所改进，但仍存在重建质量下降和训练时间过长的问题。本文旨在通过高效的静态-动态分解策略，在保证高保真重建质量的同时大幅提升训练和渲染效率。

Method: 提出 DSD-GS 框架，核心是静态-动态分解策略：使用前馈高斯泼溅编码器（Feed-Forward Gaussian Splatting Encoder）和光流模型（Optical Flow Model）将场景划分为静态区域和动态区域，仅对动态高斯体素进行建模和优化，消除静态区域的冗余计算。该方法无需 COLMAP 预处理，支持确定性初始化。

Result: 在 Neural 3D 数据集上，训练仅需 10 分钟，渲染速度超过 700 FPS（1352×1014 分辨率，单张 RTX 5090 GPU），在渲染质量、训练/渲染速度和存储效率方面均达到 SOTA，优于现有基线方法。

Conclusion: DSD-GS 通过高效的静态-动态分解策略，在动态场景重建中实现了效率与质量的平衡，为虚拟现实、机器人、数字孪生等应用提供了实用的解决方案。

Classification reason: 该论文提出基于动态-静态分解的高斯泼溅框架用于动态场景重建，属于动态3D/4D场景生成与重建方向，与 canonical subtopic "4D 与动态场景生成"（dynamic_4d_generation）高度匹配。

Importance reason: Strong engineering contribution to dynamic scene reconstruction but lacks direct alignment with priority subtopics like continuous VLM or autoregressive generation. Signals: Dynamic-static decomposition, Gaussian Splatting, 10-minute training, >700 FPS rendering, no COLMAP needed
...
### [10] [Learning Global Motion with Compact Gaussians for Feed-Forward 4D Reconstruction](https://arxiv.org/abs/2605.31595)
...
TL;DR: C4G 提出一种前馈式 4D 重建框架，使用紧凑的可学习高斯查询令牌（Gaussian query tokens）聚合全时序上下文特征，解码出时间条件化的 3D 高斯，实现无需逐场景优化的全局一致运动建模，并引入视频扩散模型增强渲染质量，支持新视角合成、点跟踪和动态场景理解。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式动态场景重建方法逐帧像素级预测 3D 高斯，导致高斯重复和视角依赖偏差，难以有效学习场景运动。本文旨在设计一种无需逐场景优化、能全局建模运动的前馈式 4D 重建方法。

Method: 提出 C4F 框架：1）使用一组紧凑的、时间戳条件化的可学习高斯查询令牌，每个令牌聚合全时序上下文中的对应特征；2）解码出位置由目标时间戳调制的 3D 高斯，实现全局一致的运动建模；3）引入基于视频扩散模型的渲染增强模块以捕捉细粒度细节；4）将特征提升扩展到 4D 特征场，支持点跟踪和动态场景理解。

Result: 在多个动态场景数据集上，C4G 使用显著更少的高斯数量实现了强的新视角合成性能，无需相机位姿，展现出更强的运动建模能力和对大时间间隔的鲁棒性。

Conclusion: C4G 通过紧凑高斯查询令牌和全局时序特征聚合，有效解决了前馈式 4D 重建中的运动建模难题，为动态场景重建提供了一种高效、无需相机位姿的新范式。

Classification reason: 该论文提出前馈式4D重建框架C4G，从单目视频重建动态3D场景（4D），属于多模态生成方向下的[4D 与动态场景生成]（dynamic_4d_generation）规范子主题。

Importance reason: Novel feed-forward 4D reconstruction with compact Gaussians and video diffusion enhancement, but tangential to core priority subtopics. Signals: 4D reconstruction, feed-forward, compact Gaussians, video diffusion, dynamic scene
...
<details>
  <summary>Details</summary>
Motivation: 真实世界图像复原受限于高质量配对训练数据的稀缺。合成数据集无法模拟真实退化，而真实配对数据采集成本高昂，导致现有模型在真实场景中泛化能力有限。
...
Result: GGT-100K 在多种图像复原模型上持续提升了真实世界泛化能力，尤其对生成式 IR 模型的微调效果显著。实验表明 MFM 可作为面向复原的数据生成实用工具。

Conclusion: 多模态基础模型能够作为实用的数据生成工具，GGT-100K 数据集是扩展真实世界图像复原模型泛化边界的有用资源。

Classification reason: 该论文利用多模态基础模型（MFM）从真实低质量图像生成高质量目标，构建合成配对数据集 GGT-100K 用于图像复原训练，属于合成训练数据生成范畴，与 [CANONICAL] 子主题"合成数据与数据集蒸馏"高度匹配。

Importance reason: Focuses on image restoration data generation, not directly aligned with user's priority subtopics like continuous VLM or autoregressive video generation. Signals: Image restoration, Generative Ground Truth, Multimodal foundation models, Dataset construction, Real-world generalization
...
TL;DR: SANA-Streaming 是一个系统-算法协同设计的实时流式视频编辑框架，通过混合扩散 Transformer 架构、Cycle-Reverse Regularization 训练策略和高效系统协同设计（融合 GDN 内核 + 混合精度量化），在单张 RTX 5090 GPU 上实现了 1280×704 分辨率、24 FPS 端到端的实时视频编辑，显著优于现有方法。
...
Method: (1) 混合扩散 Transformer 架构：在部分块中引入 softmax 注意力以增强局部建模能力，同时保留线性层的效率；(2) Cycle-Reverse Regularization：通过流匹配从生成内容预测源帧，强制语义一致性，无需成对长视频数据；(3) 高效系统协同设计：融合 GDN 内核与针对 NVIDIA Blackwell (RTX 5090) 优化的混合精度量化 (MPQ)，最大化 Tensor Core 利用率。

Result: 在单张 RTX 5090 GPU 上实现 1280×704 分辨率、24 FPS 端到端实时编辑，DiT 核心达到 58 FPS。在时序一致性和系统吞吐量上显著优于现有 SOTA 方法。

Conclusion: SANA-Streaming 通过系统-算法协同设计，首次在消费级 GPU 上实现了高分辨率实时流式视频编辑，为交互式视频编辑应用提供了可行的解决方案。

Classification reason: 该论文提出实时流式视频到视频编辑框架，属于视频编辑方向，映射到 multimodal_generation 下的 [CANONICAL] subtopic "visual_editing"（图像与视频编辑）。同时涉及 AIGC 方向下的视频音频生成应用。

Importance reason: Strong alignment with user's priority on efficient inference and video generation, with a novel hybrid diffusion transformer and system co-design for real-time editing. Signals: real-time video editing, hybrid diffusion transformer, efficient system co-design, flow matching regularization, consumer GPU deployment
...
TL;DR: 本文提出DTG-Restore，一种无需训练的视频超分辨率框架，通过解耦分类器自由引导中的条件/无条件分支时序信号（Decoupled Time Guidance, DTG），利用更干净的扩散时间步提供前瞻先验，在保持几何结构的同时抑制扭曲内容复制，逐步从结构校正过渡到细节增强。配合即插即用的现成恢复模块，在AI生成和真实视频上均取得显著的结构保真度和时序稳定性提升。同时构建了GenWarp480基准（4400个480p扭曲视频）用于评估生成式退化。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在生成保真度上取得了显著进展，但利用这些先验进行视频恢复时，标准分类器自由引导（CFG）中条件分支与无条件分支的强耦合限制了恢复效果，导致扭曲内容被复制而非修复。作者希望在不重新训练模型的前提下，利用扩散先验增强低分辨率/扭曲视频的质量。

Method: 提出Decoupled Time Guidance (DTG)：在采样过程中，无条件分支在更干净的扩散时间步进行评估，提供"前瞻先验"以保持几何结构并抑制扭曲内容复制。该时序偏差在采样过程中逐步退火（annealed），使模型从结构校正自然过渡到细节精炼。DTG可与任意现成恢复模块即插即用组合。同时构建GenWarp480基准，包含4400个由多种文生视频模型生成的480p扭曲视频，涵盖扭曲人脸、身体错位和空间伪影等典型生成退化。

Result: 大量实验表明，该方法在不进行任何模型训练的情况下，在结构保真度和时序稳定性上取得了显著提升。在AI生成视频和真实世界视频上均有效，能够修复扭曲人脸、身体错位和空间伪影等生成式退化。

Conclusion: DTG-Restore是一种有效的无需训练的视频超分辨率框架，通过解耦CFG时序信号实现了结构校正与细节增强的渐进式过渡，为利用扩散先验进行视频恢复提供了新思路。GenWarp480基准为评估生成式退化下的鲁棒性提供了专用测试平台。

Classification reason: 该论文提出一种无需训练的视频超分辨率（VSR）框架，属于视频质量增强与修复任务，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）中的视频编辑/修复方向高度匹配。同时涉及AIGC领域的生成内容质量提升。

Importance reason: Training-free video super-resolution with decoupled guidance is solid but peripheral to your core focus on autoregressive/diffusion video generation and continuous VLM. Signals: video super-resolution, training-free diffusion refinement, decoupled time guidance, GenWarp480 benchmark, generative video restoration
...
TL;DR: 本文提出一种后处理融合框架，用于缓解GenAI图像编辑中的空间错位、纹理失真和内容幻觉问题。该方法通过建立粗粒度的空间与光度对应关系，将原始输入图像与GenAI增强结果融合，在保留感知增强效果的同时确保结构忠实性，无需修改黑盒GenAI模型本身。


<details>
  <summary>Details</summary>
Motivation: GenAI图像编辑器（如Nano Banana）能通过文本提示生成视觉上令人满意的编辑结果，但其生成性质常引入空间错位、纹理失真和内容幻觉，这些缺陷对需要像素级保真度的下游工作流（如摄影后期、医学成像）是有害的。现有方法缺乏针对"结构保持的GenAI融合"这一问题的专门解决方案。

Method: 提出一个后处理融合框架，包含两个阶段：(1) 粗粒度空间与光度对应建立阶段——在原始输入图像与GenAI增强输出之间建立空间和光度对应关系；(2) 融合阶段——将期望的增强效果从GenAI输出转移到原始图像，同时抑制幻觉内容。该方法以黑盒方式工作，无需访问或修改GenAI模型内部。

Result: 与真实感风格迁移和图像融合的代表性方法相比，该方法在保持像素级结构一致性和原始分辨率的同时，更好地保留了美学质量。实验证明了该方法在缓解内容偏移和幻觉方面的有效性。

Conclusion: 本文提出的结构保持GenAI融合框架有效解决了GenAI图像编辑中的内容偏移和幻觉问题，在不牺牲感知质量的前提下保证了结构忠实性，为黑盒GenAI图像编辑的后处理提供了一种实用方案。

Classification reason: 该论文研究GenAI图像编辑中的内容偏移与幻觉缓解问题，提出结构保持的后处理融合框架，属于图像编辑范畴。multimodal_generation方向下的[CANONICAL] subtopic "visual_editing"（图像与视频编辑）完美匹配。同时该工作也涉及AIGC方向下的生成内容质量与编辑应用。

Importance reason: Focuses on image editing hallucination mitigation, which is not a priority subtopic and lacks direct connection to user's core interests in autoregressive generation, world models, or continuous VLM. Signals: Image editing, hallucination mitigation, structural refinement, post-processing fusion, no priority subtopic match
...
TL;DR: 本文发现无条件扩散模型中主流的 h-space patching 编辑方法在全局低层感知编辑（如美学增强）上系统性失效，提出了一种无需训练的推理时编辑框架，通过提取退化概念向量并结合瓶颈 patching 与无分类器引导，引导采样远离退化流形，从而持续生成质量提升的图像。


<details>
  <summary>Details</summary>
Motivation: 无条件扩散模型虽具备强大的生成先验，但如何引导其生成美学上更优的输出仍未被充分探索。现有 h-space patching 方法在全局低层变换（如美学和感知优化）上系统性失效，亟需一种无需重新训练即可实现低层感知编辑的通用框架。

Method: 提出一种推理时编辑框架：1) 提取退化概念向量（degradation concept vectors）表征低层退化特征；2) 结合瓶颈 patching（bottleneck patching）与无分类器引导（classifier-free guidance），在采样过程中引导模型远离退化流形，从而生成质量提升的图像。整个过程无需模型重新训练。

Result: 实验表明，该方法在无条件扩散模型上能够持续生成质量改善的图像，有效克服了 h-space patching 在全局低层感知编辑上的局限性，无需额外训练即可实现美学和感知层面的编辑效果。

Conclusion: 本文提出的推理时编辑框架为无条件扩散模型中的低层感知编辑提供了一种有效且无需训练的解决方案，填补了该领域的研究空白，为后续美学引导的生成编辑研究奠定了基础。

Classification reason: 该论文研究无条件扩散模型中的图像编辑方法，提出一种无需训练的推理时编辑框架，属于图像编辑范畴。因此映射到 multimodal_generation 方向下的 [CANONICAL] subtopic "visual_editing"（图像与视频编辑）。

Importance reason: Focuses on low-level perceptual editing in unconditional diffusion models, which is not a priority subtopic and lacks direct connection to user's key interests in autoregressive generation, continuous VLMs, or world models. Signals: Unconditional diffusion models, h-space patching failure, low-level perceptual editing, inference-time editing, no training required
...
TL;DR: 本文提出一种解耦语义与基元的表示驱动框架，用于解决少样本非典型布局到图像生成中的表示碎片化问题，通过语义锚定、基元注入和概念引导三个模块实现鲁棒的少样本适应，在5-shot设置下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有布局到图像生成方法在少样本非典型场景下会产生碎片化和扭曲的生成结果，作者将其归因为"表示碎片化"问题——语义身份与视觉细节纠缠导致的粒度不匹配。

Method: 提出三模块框架：(1) 语义锚定（Semantic Anchoring）将类别语义聚合为锚点以保持稳定身份；(2) 基元注入（Primitive Imbuing）建模可重组的基元以实现鲁棒的局部细节建模；(3) 概念引导（Conceptual Steering）通过显著性感知目标约束优化过程，保持前景语义一致性。

Result: 在5-shot设置下，在多个非典型领域上相比现有最先进L2I方法在视觉保真度和对齐度上均取得一致改进。

Conclusion: 解耦语义与基元的表示学习能有效解决少样本布局到图像生成中的表示碎片化问题，为少样本可控生成提供了新思路。

Classification reason: 该论文研究布局到图像生成（Layout-to-Image），属于可控图像生成/编辑范畴，与多模态生成方向下的 [CANONICAL] subtopic "visual_editing（图像与视频编辑）" 高度相关，因为布局到图像生成本质上是一种基于空间布局的可控图像编辑/生成任务。

Importance reason: Focuses on few-shot layout-to-image generation, which is not a priority subtopic and lacks strong overlap with user's key boost keywords. Signals: Few-shot generation, Layout-to-image, Disentangled representation, Not autoregressive or diffusion video/image generation, No continuous VLM or world model keywords
...
### [17] [Text-guided Feature Disentanglement for Cross-modal Gait Recognition](https://arxiv.org/abs/2605.30784)
...
TL;DR: 本文提出TCFDNet，利用文本引导的特征解耦网络解决LiDAR-相机跨模态步态识别（LCCGR）问题。通过大语言模型构建步态模态文本字典（GMTD），使用CLIP多粒度编码器对齐视觉-文本特征，并通过文本引导特征解耦（TFD）模块和特征稳定性增强（FSE）模块提取模态共享特征，在SUSTech1K和FreeGait数据集上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现实场景中步态识别常涉及异构传感模态（如LiDAR和RGB相机），2D视频与3D点云序列之间存在巨大的模态鸿沟，现有方法难以有效提取跨模态共享的步态特征。

Method: 1) 构建Gait Modality Text Dictionary (GMTD)，利用LLM生成跨模态、跨视角的丰富语义描述；2) CLIP-based Multi-grained Feature Encoder对齐视觉与文本特征到统一视觉-语言空间；3) Text-guided Feature Disentanglement (TFD)模块通过topk文本描述重建模态特定特征，并利用残差分解和正交约束提取模态共享特征；4) Feature Stability Enhancement (FSE)模块建模空间和通道相关性增强特征鲁棒性；5) 引入跨模态patch交换策略提升泛化能力。

Result: 在SUSTech1K和FreeGait数据集上，TCFDNet在所有评估协议下均达到新的SOTA结果，消融实验验证了各模块的有效性。

Conclusion: 文本引导的特征解耦策略能有效弥合LiDAR与RGB之间的模态鸿沟，为跨模态步态识别提供了一种新的有效范式。

Classification reason: 该论文研究跨模态步态识别（LiDAR-相机），核心是利用CLIP文本引导进行视觉-文本特征对齐与解耦，属于跨模态检索/匹配任务。multimodal_generation方向下的multimodal_retrieval（多模态检索）子主题与之最匹配，且为[CANONICAL]子主题。

Importance reason: Focuses on gait recognition, not aligned with user's priority subtopics like continuous VLM, video generation, or world models. Signals: cross-modal gait recognition, text-guided feature disentanglement, LiDAR-camera, CLIP-based alignment, not autoregressive or diffusion video generation
...
TL;DR: Dex2HOI 提出了一种统一的扩散模型，用于从文本生成单手/双手单物体/双物体的人-物交互动作序列，采用双流扩散架构和运动融合网络，实现了实时推理速度（比先前方法快540倍）并在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有4D人-物交互生成研究主要关注单物体操作，忽略了人类自然行为中双手协调和同时操作多个物体的特性。作者旨在填补这一空白，实现更真实、更具表现力的多物体交互动作生成。

Method: 提出Dex2HOI，核心是双流扩散（Dual-Stream Diffusion）架构：每个物体由独立的交互流处理，通过双向交叉注意力协调。引入运动融合网络（Motion Fusion Network），结合新颖的手部相对物体表示和全序列接触感知条件。通过自回归前缀条件窗口采样，生成任意长度序列，无需测试时优化。

Result: 在单物体和双物体基准上均达到SOTA定量结果；推理速度相比先前方法提升高达540倍，支持实时生成。
...
Importance reason: Focuses on human-object interaction generation, not aligned with user's priority subtopics like continuous VLM or autoregressive video/image generation. Signals: diffusion model, human-object interaction, motion generation, real-time inference, dual-stream architecture
...
TL;DR: MultiAct 是一种无需重新训练的推理时方法，通过自适应放大复合文本提示中未被充分关注的成分的交叉注意力分数，解决现有文生动作模型在处理多动作复合描述时语义坍缩的问题，提升语义覆盖率和动作真实感。


<details>
  <summary>Details</summary>
Motivation: 现有文生动作模型在处理描述多个同时发生动作的复合文本提示时，往往只优先生成一个主导动作而忽略其他成分，导致生成的动作不完整或语义模糊。作者希望在不重新训练或修改模型架构的前提下，解决这一语义坍缩问题。

Method: MultiAct 是一种推理时框架，直接作用于预训练的文生动作生成器。核心思路是自适应地放大与未被充分关注的提示成分相关的交叉注意力分数。由于有效的调制依赖于提示特定的选择（如哪些 token 和哪些层需要增强），作者引入了一个轻量级的辅助决策方案，自动确定最有效的注意力增强参数化配置。

Result: 大量定量和定性评估表明，MultiAct 在复合提示上持续优于现有基线方法，在提升语义覆盖率的同时保持了动作的真实感。

Conclusion: MultiAct 通过推理时的注意力引导，有效解决了复合文本到动作生成中的语义坍缩问题，为无需重新训练的生成模型可控性增强提供了新思路。

Classification reason: 该论文研究文本到人体动作生成（text-to-motion），属于多模态生成方向下的数字人与动作生成（avatar_motion_generation）这一规范子主题，该子主题明确包含人体动作生成。

Importance reason: Focuses on text-to-motion generation, which is not a priority subtopic and lacks strong overlap with user's configured direction weights. Signals: inference-time method, no retraining needed, addresses semantic collapse in composite prompts, improves semantic coverage, motion realism preserved
...
TL;DR: 本文提出 OmniME，一个全监督正负学习框架，用于基于文本的人体动作编辑。通过回顾性特征监督、动作保持机制和三元组语义对齐三个互补组件，在保持未编辑部分不变的同时精确编辑目标区域，在 MotionFix 和 STANCE Adjustment 数据集上达到 SOTA。
...
Method: 提出 Omni-Supervised Positive-Negative Learning 框架 (OmniME)，包含三个组件：(1) 回顾性特征监督——在 transformer 层间施加由粗到细的一致性约束；(2) 动作保持机制——根据源-目标相似性关注细微变化；(3) 三元组语义对齐——增强文本-动作对应关系。三者形成统一的监督范式来平衡变化与不变性。

Result: 在 MotionFix 和 STANCE Adjustment 数据集上，OmniME 在编辑对齐方面达到了最先进的性能，验证了统一学习框架的有效性。

Conclusion: 全监督正负学习框架能够有效平衡动作编辑中的变化与不变性，为文本驱动的人体动作编辑提供了新的有效范式。

Classification reason: 该论文研究基于文本的人体动作编辑，属于人体动作生成与编辑范畴，与 [CANONICAL] 数字人与动作生成 (avatar_motion_generation) 高度匹配。

Importance reason: Focuses on human motion editing, which is not a priority subtopic and lacks boost keywords. Signals: motion editing, diffusion-based, positive-negative learning, text-based editing, SOTA on MotionFix
...
TL;DR: Polyphony 提出了一种三阶段方法用于双手动作分割，包括交替训练的双手视觉Transformer、语义特征条件化以及扩散模型分割，在多个数据集上取得了SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 双手动作分割面临手部间复杂依赖、视觉不对称、主导手梯度垄断以及细粒度动作语义模糊等挑战，现有方法难以有效处理这些问题。

Method: Polyphony 包含三个阶段：(1) 交替双手视觉Transformer（Alternating Dual-Hand ViT），在左右手小批量间交替训练以平衡梯度贡献；(2) 语义特征条件化（Semantic Feature Conditioning），将视觉特征与结构化的组合动作描述对齐以增强语义判别；(3) 扩散模型分割（Diffusion-Based Segmentation），结合跨手特征融合和自适应损失加权。

Result: 在HA-ViD和ATTACH两个双手数据集上取得SOTA，提升高达16.8个点；在单流Breakfast数据集上达到82.5%，超越使用12倍大骨干网络的先前最佳方法。

Conclusion: Polyphony 通过交替训练、语义条件化和扩散分割有效解决了双手动作分割中的关键挑战，且统一单骨干模型优于需要独立手模型的基线方法。

Classification reason: 该论文研究双手动作分割（dual-hand action segmentation），属于人体动作理解与生成范畴，与[CANONICAL] subtopic "数字人与动作生成"（avatar_motion_generation）中的人体动作分析高度相关。

Importance reason: Focuses on dual-hand action segmentation, which is outside the user's priority subtopics and lacks boost keywords. Signals: action segmentation, dual-hand, diffusion-based, vision transformer, semantic conditioning
...
TL;DR: DriveMA 提出了一种基于可验证元动作（meta-actions）的驾驶视觉-语言-动作模型框架，通过轨迹标注管道将专家轨迹转化为语言域意图，并利用规则投影进行验证。结合动作中心监督训练和回合级信用分配强化学习，实现了高层决策与低层轨迹规划的显式对齐，在 Waymo Open Dataset 上取得了 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶 VLA 模型存在语言-动作鸿沟（language-action gap），语言输出与底层轨迹规划之间缺乏显式对齐，限制了端到端规划性能的提升。

Method: 提出可验证元动作（verifiable meta-actions）作为语言域与动作域之间的桥梁。元动作通过轨迹标注管道从专家轨迹中自动构建，并通过规则投影对生成轨迹进行验证。框架包含动作中心监督训练和回合级信用分配强化学习，利用密集奖励实现高层决策与低层轨迹的精确对齐。

Result: 在 Waymo Open Dataset 视觉端到端驾驶任务上，2B 模型达到 8.060 Rater Feedback Score，4B 模型进一步提升至 8.079，均达到 SOTA；在 NAVSIM 上获得有竞争力的闭环规划性能。

Conclusion: 即使使用简单的元动作接口，只要使其可验证并针对语言-动作对齐进行优化，就能在自动驾驶规划任务上取得 SOTA 性能，验证了元动作作为语言-动作桥梁的有效性。
...
Importance reason: Strong on driving VLA alignment but outside user's priority subtopics and negative keywords for driving. Signals: Driving VLA, meta-actions, language-action alignment, Waymo SOTA, reinforcement learning
...
TL;DR: TokTalk 提出一种直接从 Audio-LLM 的音频 token 实时生成 3D 面部表情动画的系统，使用基于分块的连续条件流匹配模型，无需经过传统的语音识别-文本生成-语音合成-面部动画多阶段流水线，在保持低延迟的同时显著提升了面部动画的表达质量和可控性。


<details>
  <summary>Details</summary>
Motivation: 现有对话式虚拟人面部动画通常依赖多阶段流水线（语音识别→文本生成→语音合成→音频驱动面部动画），导致表情僵硬、对话流程不自然。作者发现 Audio-LLM 产生的音频 token 已包含足够信息来重建合理的面部表现，因此希望直接利用这些 token 实现更自然、更具表现力的实时面部动画。

Method: TokTalk 构建了一个音频 token 到 3D 面部运动的专用数据集，采用基于分块的条件流匹配（Chunk-based Conditional Flow Matching）模型进行训练。通过轻量级适配策略，训练好的模型可以无缝连接到任意基于 token 的 Audio-LLM，计算开销极小。分块处理机制允许在延迟和面部质量之间进行参数化权衡。

Result: 消融实验验证了分块处理在延迟-质量权衡上的有效性。与现有方案相比，TokTalk 在延迟方面相当，而在 3D 面部表现的质量、表达力和可控性方面通过感知研究显著优于先前方法。展示了聊天机器人 Avatar、语音驱动用户 Avatar 和动画导演界面等多种应用场景。

Conclusion: TokTalk 证明了 Audio-LLM 的音频 token 可以直接驱动高质量、富有表现力的实时 3D 面部动画，跳过多阶段流水线，为对话式虚拟人提供了一种更高效、更自然的解决方案。

Classification reason: 该论文研究从音频-LLM token 实时生成 3D 面部动画，属于虚拟人/说话头/面部动画生成，与 [CANONICAL] 子主题 "数字人与动作生成" (avatar_motion_generation) 高度匹配。

Importance reason: Novel application of flow matching to audio-LLM tokens for real-time facial animation, but tangential to user's core focus on continuous VLM, autoregressive generation, and world models. Signals: flow matching, real-time facial animation, audio-LLM tokens, chunk-based processing, expressive 3D avatar
...
Main category: cs.MA, cs.AI, cs.LG
...
TL;DR: 本文提出将队友作为可学习组件纳入Dreamer风格世界模型的方法，通过分解隐状态为环境和队友两部分，并引入心智理论（ToM）头来推断队友的潜在表征（如角色、意图、预测动作），从而支持多智能体强化学习中的零样本和少样本协作。


<details>
  <summary>Details</summary>
Motivation: 在合作式多智能体强化学习中，智能体需要与内部策略和意图不可直接观测的队友协调。现有世界模型（如Dreamer）在单智能体场景中表现出色，但无法处理队友带来的不确定性，限制了其在多智能体场景中的应用。

Method: 提出一种架构，将Dreamer风格的循环状态空间模型（RSSM）的隐状态分解为环境组件和队友组件，并学习一个辅助的心智理论（ToM）头，从部分轨迹中推断队友行为的隐嵌入（如角色、意图和预测动作）。这些队友隐变量用于条件化actor和critic，使智能体能够想象并适应不同的协作伙伴。

Result: 论文提出了方法框架、基准测试和评估协议，但未提供具体的实验数值结果，属于方法提案/立场论文。

Conclusion: 该工作将世界模型定位为不仅是环境动态的预测器，更是社会行为的模拟器，为可泛化、与人类兼容的AI开辟了新方向。

Classification reason: 该论文研究多智能体强化学习中基于世界模型的队友建模，涉及智能体对队友行为（角色、意图、动作）的隐空间推断与模拟。虽然核心是多智能体强化学习，但其方法基于Dreamer风格的世界模型（RSSM），且涉及对智能体/队友行为的建模与生成式模拟，与"数字人与动作生成"（avatar_motion_generation）中关于智能体行为建模、虚拟人动作生成的主题有合理关联。同时该方向下的世界模型与动作生成主题最为接近。

Importance reason: Focuses on multi-agent RL teammate modeling, not on the user's priority subtopics like continuous VLM, autoregressive video generation, or world models for video. Signals: Multi-agent RL, world model, teammate modeling, theory of mind, not video generation
...
TL;DR: CameraNoise 提出一种流到噪声的扭曲方法，将相机运动编码为时间一致的随机表示，通过几何引导的重投影流和噪声扭曲算法，在保持扩散高斯先验的同时实现精确的相机姿态控制，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散方法通过直接注入数值相机参数来控制相机姿态，但抽象坐标与视觉内容之间存在鸿沟，导致结构失真。需要一种能将相机运动忠实编码到扩散过程中、同时保持几何一致性的方法。

Method: 提出 CameraNoise 框架，包含两个核心组件：(1) 几何引导重投影流（Geometry-guided Reprojection Flow），根据相机参数计算帧间像素对应关系；(2) 噪声扭曲算法（Noise Warping），将相机运动编码到噪声空间中，在保持扩散高斯先验的同时实现一致的噪声传播。该方法将相机姿态直接嵌入噪声空间，解耦运动与场景外观。

Result: 大量实验表明，CameraNoise 在视觉质量和轨迹忠实度上均显著优于先前方法，能够生成稳定、高保真的视频，精确遵循指定的相机运动轨迹。

Conclusion: CameraNoise 通过将相机控制嵌入噪声空间而非直接注入参数，有效弥合了抽象坐标与视觉内容之间的鸿沟，为视频扩散中的精确相机控制提供了一种新颖且有效的范式。

Classification reason: 该论文提出 CameraNoise，通过几何流引导的噪声扭曲方法实现视频扩散中的精确相机姿态控制，属于视频扩散模型中的可控生成方法。最佳匹配方向为多模态生成下的 [CANONICAL] 文生视频与视频扩散（text_to_video_diffusion），同时与 AIGC 方向下的视频音频生成应用（aigc_video_audio_creation）也相关。

Importance reason: Strongly aligns with user's priority on video generation and diffusion models, with a novel noise warping method for camera control. Signals: video diffusion, camera control, noise warping, geometry-guided flow, diffusion transformer
...
### [26] [SlotMemory: Object-Centric KV Memory for Streaming Long-Video Generation](https://arxiv.org/abs/2605.31033)
...
TL;DR: SlotMemory 提出了一种以物体为中心的键值记忆机制，将 Transformer 的 KV 流形分解为离散的可复用语义槽，用于流式长视频生成中的实体级持久化和提示感知检索，在 60 秒交互叙事生成任务上取得了 SOTA 质量分数和动态一致性提升。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频生成模型依赖时间中心记忆（原始帧、片段或未聚类 token），当物体离开画面或提示词切换时，常出现身份漂移和语义不一致问题。作者认为需要从"何时发生"转向"什么被表示"的记忆抽象。

Method: 提出 SlotMemory，将 Transformer 的 KV 流形分解为离散的可复用语义槽（slots），作为路由地址来索引和存储高保真 KV token，实现跨长时域的实体级持久化和提示感知检索。基于 Wan2.1-T2V-1.3B 骨干进行流式视频生成。

Result: 在 60 秒交互叙事生成任务上，SlotMemory 达到 81.61 的质量分数，动态一致性相比最强基线提升 22.8%。

Conclusion: 结构化语义表示（而非原始时间容量）是实现持久长视频合成的关键原语。SlotMemory 通过物体级记忆有效缓解了身份漂移和语义不一致问题。

Classification reason: 该论文提出面向流式长视频生成的物体级KV记忆机制SlotMemory，基于视频扩散模型（Wan2.1-T2V-1.3B骨干），属于文本到视频生成与视频扩散方向，因此映射到[CANONICAL] subtopic "文生视频与视频扩散"。

Importance reason: Directly advances continuous VLM and visual latent flow with object-centric KV memory for streaming video generation, matching top priority subtopics. Signals: object-centric KV memory, streaming long-video generation, entity-level persistence, prompt-aware retrieval, 22.8% dynamic consistency improvement
...
### [27] [TunerDiT: Training-free Progressive Steering of Diffusion Transformer for Multi-Event Video Generation](https://arxiv.org/abs/2605.31590)
...
TL;DR: TunerDiT 是一种无需额外训练的视频扩散 Transformer 渐进式引导方法，通过事件分区掩码和跨事件提示融合，实现多事件视频生成，并在 8 项指标上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有文生视频模型在生成长时间、包含多个事件的视频时表现不佳，难以在保持视频整体一致性的同时清晰区分不同事件。作者希望利用扩散过程的内在特性，在不引入额外训练的前提下解决多事件视频生成问题。

Method: 首先通过探针实验发现 DiT 去噪轨迹中存在关键转折点——条件文本对生成的影响从全局布局逐渐过渡到细粒度细节。基于此，提出 TunerDiT，包含两个引导手柄：(1) 事件分区掩码（Event-Partitioned Masking），强制事件边界同时允许跨事件过渡带；(2) 跨事件提示融合（Cross-Event Prompt Fusion），在后期细化阶段注入相邻事件语义。此外还贡献了多事件生成评测套件 Meve。

Result: 在 8 项指标上达到训练无关方法中的最优性能，文本对齐度随事件数量增加而提升，表明该方法具有随事件数扩展的潜力，并可在视频一致性与事件分离之间进行可调权衡。

Conclusion: TunerDiT 通过分析扩散过程的内在特性，提出了一种简单有效的无需训练的多事件视频生成引导方法，在多个指标上取得领先结果，为长视频多事件生成提供了新的思路。

Classification reason: 该论文提出 TunerDiT，一种无需训练的多事件视频生成渐进式引导方法，基于视频扩散 Transformer（DiT）的逆向过程分析。核心属于文生视频与视频扩散方向，因此映射到 multimodal_generation 下的 [CANONICAL] subtopic "文生视频与视频扩散"（text_to_video_diffusion）。同时论文包含自建评测套件 Meve，也涉及 AIGC 评测，因此匹配 aigc 方向。

Importance reason: Directly advances controllable video generation with a training-free method, highly relevant to your priority on diffusion video generation and efficient inference. Signals: training-free, diffusion transformer, multi-event video generation, progressive steering, state-of-the-art metrics
...
### [28] [Unlearning in Diffusion Models: A Unified Framework with KL Divergence and Likelihood Constraints](https://arxiv.org/abs/2605.30825)
*Shervin Khalafi, Alejandro Ribeiro, Dongsheng Ding*

Main category: cs.LG, math.OC, stat.ML, cs.AI
...
TL;DR: 本文提出一个基于约束优化的扩散模型遗忘学习统一框架，通过KL散度和似然约束将遗忘学习形式化为最小化与预训练模型偏差的同时满足与遗忘分布的分离约束，推导了强对偶性并给出了最优解和原始-对偶算法，在概念遗忘和数据遗忘任务上取得了更优的保留-遗忘权衡。


<details>
  <summary>Details</summary>
Motivation: 扩散模型中的遗忘学习需要在移除不需要的数据或概念（如有害内容、版权图像）的同时尽可能保留预训练模型的生成能力，这两个目标本质上是冲突的。现有方法缺乏统一的理论框架，难以在保留与遗忘之间取得良好平衡。

Method: 提出一个约束优化框架，将遗忘学习建模为在满足与遗忘分布分离约束的条件下最小化与预训练模型的偏差。具体设计了三个约束优化问题：基于反向KL散度、前向KL散度和似然约束。前两者统一了现有概念遗忘和数据遗忘方法，后者提供了新的遗忘形式。尽管KL约束非凸，但证明了三个问题的强对偶性，从而显式刻画了最优解作为遗忘目标，并开发了相应的原始-对偶算法。

Result: 实验表明，基于KL约束的方法在概念遗忘和数据遗忘任务上相比基于权重的基线方法实现了更优的保留-遗忘权衡；基于似然的方法在匹配遗忘效果的同时更好地保留了保留概念。

Conclusion: 本文为扩散模型遗忘学习提供了一个统一的理论框架，通过约束优化视角统一了多种遗忘范式，并证明了强对偶性使得最优解可解析刻画。该框架为生成模型的安全生成和概念擦除提供了理论基础和实用算法。
...
Importance reason: Focuses on diffusion model unlearning, which is outside the user's priority subtopics and lacks boost keywords. Signals: diffusion model unlearning, constrained optimization, KL divergence, concept/data forgetting, strong duality
...
### [29] [OmniMem: Scalable and Adaptive Memory Retrieval for Long Video Generation](https://arxiv.org/abs/2605.30519)
...
Importance: 高优先级 (90.63) - 未入选精读

TL;DR: OmniMem提出了一种显式全范围记忆检索框架，通过自适应窗口排除和查询共享KV选择等机制，在自回归长视频生成中实现高效的稀疏KV检索，显著提升动态度指标并保持强一致性，同时维持可比的内存使用。


<details>
  <summary>Details</summary>
Motivation: 自回归视频生成在生成长视频时需要反复访问不断增长的KV缓存，现有方法通过截断或压缩为隐式记忆来降低开销，但会丢失对查询相关历史细节的显式访问能力。本文旨在解决这一矛盾，实现既高效又能保留显式历史信息访问的长视频生成。

Method: 提出OmniMem框架，包含三个关键设计：(1) 自适应窗口排除（Adaptive Window Exclusion）——当有足够长程历史可用时，从候选集中排除局部窗口块，将稀疏预算留给信息量更大的长程检索；(2) 查询共享KV选择（Query-Shared KV Selection）——减少跨查询的多样性；(3) 逐头分散KV访问（Per-Head Scattered KV Access）——避免将逐头选择扩展为大的KV缓冲区，允许每个注意力头根据自身选择模式检索非连续的KV块。

Result: 在长视频生成实验中，OmniMem相比强基线将动态度（Dynamic Degree）提升了52.3%，同时保持了强一致性，且内存使用与基线相当。

Conclusion: OmniMem通过显式全范围稀疏KV检索有效解决了自回归长视频生成中历史信息访问与效率之间的权衡，为长视频生成提供了一种可扩展且自适应的记忆检索方案。

Classification reason: 该论文聚焦于自回归视频生成中历史KV缓存的高效检索问题，通过稀疏KV检索和自适应窗口排除等策略降低长视频生成中的内存开销，属于生成模型效率优化范畴，因此映射到[CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Directly addresses efficient autoregressive long-video generation via sparse KV retrieval, aligning with top-priority subtopics and boost keywords. Signals: autoregressive video generation, efficient inference, sparse KV retrieval, long video generation, adaptive window exclusion
...
### [30] [LVSA: Training-Free Sparse Attention for Long Video Diffusion](https://arxiv.org/abs/2605.31057)
...
TL;DR: LVSA是一种无需训练、模型无关的块稀疏注意力方法，通过结构化窗口模式与旋转全局锚点结合，在长视频扩散模型中实现高达3.33倍的计算加速，同时保持或提升生成质量，并能在单GPU上生成训练时2倍长度的视频。


<details>
  <summary>Details</summary>
Motivation: 长视频扩散推理中，密集自注意力的计算成本随序列长度二次增长，且超出训练长度时模型会生成近乎静态的重复视频。现有方法要么需要重新训练成本过高，要么无法同时满足性能和质量的扩展需求。

Method: LVSA结合结构化窗口注意力模式与旋转全局锚点，形成块稀疏注意力模式，消除固定网格偏差导致的长程时序伪影。配合FlashInfer内核实现高效计算，无需重新训练即可应用于不同视频扩散Transformer模型。

Result: 在Wan 2.1 1.3B上6倍长度下加速3.17倍，Wan 2.1 14B上6倍长度下加速2.98倍，HunyuanVideo 1.5上1.5倍长度下加速3.33倍。相比RIFLEx加速2.41倍，相比UltraViCo加速3.27倍。在NPU上也实现2.71-3.24倍加速。同时提出VQeval工具公平评估循环视频伪影。

Conclusion: LVSA是一种高效、无需训练的长视频扩散稀疏注意力方法，在训练长度下质量中性，在扩展长度下质量正向，为长视频生成提供了实用的加速方案。

Classification reason: 该论文提出LVSA，一种无需训练的块稀疏注意力机制，用于加速长视频扩散模型推理，显著降低计算开销。这属于生成模型加速与高效化（efficient_generation）这一[CANONICAL]子主题。

Importance reason: Directly addresses efficient inference for long video diffusion, a high-priority subtopic, with a training-free sparse attention method that achieves significant speedups and enables longer generation. Signals: training-free sparse attention, long video diffusion, 3.33x compute reduction, enables 2x horizon generation, model-agnostic
...
TL;DR: 本文提出了一种方差缩减的零阶 Langevin 采样方法，用于黑盒非对数凹分布采样，并首次建立了非渐近收敛保证。进一步提出 ZO-APMC 算法，将预训练得分生成模型作为先验用于黑盒反问题后验采样，在理论和实验上均验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 在高维非对数凹分布采样中，梯度信息不可获取或计算代价高昂的黑盒场景下，现有零阶 Langevin 方法存在方差大、缺乏非渐近收敛保证等问题，限制了其在反问题等实际应用中的使用。

Method: 提出一种方差缩减的零阶梯度估计器，显著降低了经典批零阶估计器的方差，消除了批大小对维度的不利依赖。基于此设计了 ZO-Langevin 采样算法，并进一步提出 ZO-APMC 算法，将预训练得分生成模型作为先验，用于黑盒线性/非线性反问题的后验采样。

Result: 首次建立了零阶非对数凹采样的非渐近收敛保证（ε-相对 Fisher 信息距离），在 Poincaré 不等式假设下还给出了平方总变差距离的收敛界。合成实验和实际线性/非线性反问题实验均验证了方法的有效性。

Conclusion: 本文提出的方差缩减零阶 Langevin 采样方法有效解决了黑盒非对数凹采样的高方差和缺乏理论保证的问题，为基于生成先验的黑盒反问题提供了首个具有非渐近收敛保证的后验采样算法。

Classification reason: 该论文提出了一种方差缩减的零阶 Langevin 采样方法（ZO-Langevin），用于黑盒非对数凹分布采样，并应用于基于预训练得分生成先验的反问题后验采样（ZO-APMC）。核心贡献在于采样效率的方差缩减与收敛保证，属于生成模型采样加速与高效化的范畴，因此映射到 [CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Proposes variance-reduced zeroth-order Langevin sampling with first non-asymptotic guarantees for non-log-concave sampling, relevant to efficient inference in generative models. Signals: variance reduction, zeroth-order Langevin, non-log-concave sampling, black-box inverse problems, score-based generative priors
...
### [32] [QVGGT: Post-Training Quantized Visual Geometry Grounded Transformer](https://arxiv.org/abs/2605.31124)
...
TL;DR: QVGGT提出了一种针对VGGT（1.2B参数视觉几何Transformer）的后训练量化框架，通过选择性混合精度策略、token滤波与相机信息补偿、以及任务感知尺度搜索，实现了W4A16近无损量化，在保持所有3D预测头精度的同时带来3~4.9倍内存压缩和最高2.8倍实际加速。


<details>
  <summary>Details</summary>
Motivation: VGGT能够从单张图像前馈预测相机参数、深度图和点云，但其1.2B参数量严重限制了在无人机、移动AR等资源受限平台上的部署。现有量化方法未考虑VGGT中不同Transformer块对量化的异质性敏感度，以及高方差token（相机token、register token）带来的量化误差放大问题。

Method: (1) 逐块量化敏感度分析 + 选择性混合精度策略，为最脆弱的Transformer块分配更高精度；(2) Token滤波与相机信息补偿，去除高方差异常token并用PCA导出的全局补偿token恢复几何信息；(3) 任务感知尺度搜索，通过层重建损失、多头监督和跨头几何一致性（相机位姿、深度图、点图）联合评估候选量化尺度。

Result: 在多个几何感知基准上，QVGGT实现近无损W4A16量化，所有3D预测头精度几乎无损，内存减少3~4.9倍，实际硬件加速最高2.8倍（相比FP32）。

Conclusion: QVGGT使得高保真3D感知在边缘设备上变得可行，为前馈式3D重建模型在真实受限环境中的实际部署提供了有效方案。

Classification reason: 该论文研究VGGT（视觉几何接地Transformer）的后训练量化压缩，属于生成模型/3D感知模型的效率优化，与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Strong model efficiency work with practical edge deployment focus, but not directly on top-priority continuous VLM or autoregressive generation. Signals: post-training quantization, visual geometry transformer, mixed-precision, token filtering, task-aware scale search
...
### [33] [Are Full Rollouts Necessary for On-Policy Distillation?](https://arxiv.org/abs/2605.31490)
...
<details>
  <summary>Details</summary>
Motivation: 标准 on-policy distillation (OPD) 在训练时需要生成完整的 rollout 轨迹，计算开销大，且早期训练阶段学生模型在长 rollout 末尾位置可能接收到不可靠的教师反馈。作者希望识别并解决 rollout 长度这一关键瓶颈，提升 OPD 的训练效率。

Method: 提出两种 horizon 控制策略：(1) Progressive OPD (POPD) — 训练过程中逐步扩展 rollout 长度；(2) Truncated OPD (TOPD) — 永久性地在可靠的截断 rollout 上进行蒸馏。两种方法均基于 OPD 不需要完整轨迹或最终答案奖励即可提供学习信号的观察。

Result: 在数学推理任务上，POPD 将 OPD 训练效率提升最高3倍；TOPD 仅使用10%的 rollout 长度即可匹配完整 OPD 的性能，显著减少了 wall-clock 时间和内存消耗。

Conclusion: 控制 rollout 长度是提升 on-policy distillation 效率的简单而有效的途径，为长程推理任务的后训练提供了更实用的方案。
...
### [34] [Fixed-Point Masked Generative Modeling](https://arxiv.org/abs/2605.31215)
...
TL;DR: 本文提出 Fixed-Point Masked Generative Models (FP-MGMs)，通过固定点求解器共享注意力层实现自适应深度，配合跨步一致性损失和三状态复用（3SR）策略，在减少参数和训练成本的同时提升掩码生成模型在低采样预算下的生成质量。


<details>
  <summary>Details</summary>
Motivation: 掩码生成模型（MGM）虽能并行解码且跨模态表现优异，但每一步都需要全序列双向 Transformer，导致训练成本高、低采样预算下生成质量差。现有方法通过更好的采样器或固定深度去噪器提升效率，但每个精炼步骤仍分配固定计算量，缺乏自适应深度机制。

Method: 提出 CoFRe 框架：(1) 用固定点求解器替代部分去噪器层，共享注意力层实现自适应深度；(2) 跨步一致性损失（cross-step consistency loss）对齐相邻去噪步的隐表示；(3) 三状态复用（3SR）策略，对未改变、仍掩码和新揭示的 token 分别处理，用前一步解热启动求解器。此外，预训练 MGM 可通过短时微调转换为 FP-MGM。

Result: 在 OpenWebText 上，参数减少 38.8%，训练时间减少 11.5%，VRAM 减少 16.9%，在 96 次 Transformer 前向传播预算下生成困惑度从 830.8 降至 101.8。在 ImageNette 上，训练时间减少 48.6%，VRAM 减少 50.7%，所有采样预算下 FID 均有提升。

Conclusion: CoFRe 提供了一种实用的训练到推理框架，能够以更低的训练成本和更强的低预算生成质量实现高效的掩码生成，且兼容预训练模型转换。

Classification reason: 该论文提出固定点掩码生成模型（FP-MGM），通过固定点求解器替代部分去噪器，实现自适应深度、减少参数和训练成本，属于生成模型效率优化范畴，因此映射到 [CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Directly addresses model efficiency and generation acceleration with a novel fixed-point approach, highly relevant to your priority on efficient inference and autoregressive generation. Signals: model efficiency, adaptive depth, masked generative models, fixed-point solver, training cost reduction
...
### [35] [Toward Identifiable Sparse Autoencoders](https://arxiv.org/abs/2605.31245)
...
TL;DR: 本文从理论和实践上证明稀疏自编码器（SAE）存在高度不稳定性，不同训练运行会产生不同的概念字典和稀疏编码。通过最小化架构和训练过程的改动，提出可识别SAE（iSAE），在降低重建误差的同时显著提升稳定性，并理论上将其与传统字典学习方法建立联系。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器（SAE）被广泛用于解释和操控神经网络内部表示，但经验表明不同训练运行得到的SAE高度不稳定，产生不一致的概念字典和稀疏编码。这种不稳定性严重影响了SAE作为可解释性工具的可信度和实用性，亟需理论分析和实践改进。

Method: 首先从理论上刻画了阻碍真实SAE稳定性的模型属性，然后通过最小化架构和训练过程的改动来解决这些问题。具体提出了两种版本的可识别SAE（iSAE），是标准TopK SAE的变体。通过将SAE与传统字典学习方法建立理论联系，证明实际学习的字典满足近似受限等距条件（Restricted Isometry Property），从而使稀疏编码接近可识别。

Result: iSAE相比标准TopK SAE在降低重建误差的同时显著提升了稳定性。理论分析表明，实际学习的字典满足近似受限等距条件，保证了稀疏编码的近似可识别性。

Conclusion: 本文揭示了SAE不稳定性的根源，并提出了有效的改进方案iSAE，为SAE在神经网络可解释性中的可靠应用奠定了基础。理论连接传统字典学习也为后续研究提供了新的视角。

Classification reason: 该论文研究稀疏自编码器（SAE）的稳定性与可识别性问题，提出可识别SAE（iSAE）以提升字典学习的稳定性和重建质量。SAE是解释和操控神经网络表示的重要工具，与生成模型中的表示解耦、可解释性及高效表示学习密切相关。映射到多模态生成方向下的[CANONICAL]子主题"生成模型加速与高效化"，因为SAE常用于生成模型（如扩散模型）的表示分析和效率优化，且论文核心关注模型架构与训练过程的稳定性改进，属于高效表示学习范畴。

Importance reason: Focuses on SAE stability for interpretability, not directly on user's priority subtopics like continuous VLM or video generation. Signals: SAE instability theory, iSAE improves stability and reconstruction, connects SAE to dictionary learning, minimal architecture changes, cs.LG direction
...
TL;DR: 本文形式化定义了标签排序（label ranking）中的校准概念，建立了包含完整排序、子排序和top-k排序的校准层次体系，证明了各层次间的蕴含关系，并在RLHF奖励模型上验证了校准与基准准确率之间的强相关性但不完全一致，表明校准捕捉了超越top-1准确率的质量维度。


<details>
  <summary>Details</summary>
Motivation: 校准（预测概率与真实结果频率的对齐）在分类和回归中已被广泛研究，但在概率标签排序（预测标签排序上的分布）中尚未被正式定义。现有方法将排序视为类别会忽略其结构信息，且无法捕捉成对和top-k预测等重要模态。同时，RLHF奖励模型的校准质量尚未被系统研究。

Method: 形式化定义了标签排序的校准概念，构建了包含完整排序校准、子排序校准和top-k排序校准的层次化框架，并证明了各层次间的逻辑蕴含关系。在多个标签排序数据集上评估了流行模型的校准性能，并将该框架应用于分析RLHF奖励模型的校准特性。

Result: 证明了完整排序校准蕴含子排序和top-k校准，但反之不成立；子排序校准与top-k校准之间不可比较。实验发现流行的标签排序模型通常校准较差，子排序与top-k指标之间存在显著差异。在RLHF奖励模型上，校准与基准准确率强相关但不完全一致。

Conclusion: 校准捕捉了超越top-1准确率的质量维度，为奖励模型评估提供了新的视角。未来工作应关注校准不良的下游影响及校准纠正方法。

Classification reason: 该论文研究标签排序（label ranking）中的校准问题，并将其应用于RLHF奖励模型的校准分析。这与多模态生成和多模态理解与生成方向下的"生成模型对齐与奖励"（generation_alignment）子主题高度相关，因为RLHF奖励模型是生成模型对齐的核心组件，论文直接讨论了奖励模型的校准质量与基准准确率的关系。

Importance reason: Focuses on label ranking calibration, not aligned with user's priority subtopics in multimodal generation or AIGC. Signals: Label ranking calibration, RLHF reward models, Not multimodal generation, Not autoregressive video/image generation, Not continuous VLM
...
TL;DR: 本文形式化定义了"成对参考对齐"（pairwise reference alignment）作为模型级别的序数可观测量，给出了基于模型评分函数与参考偏好分布的对齐统计量、有限样本估计量及浓度界，并在 Qwen2.5 模型和 RewardBench 上进行了初步实证验证。


<details>
  <summary>Details</summary>
Motivation: 现有工作广泛使用成对偏好数据进行模型排名、奖励建模和偏好优化，但缺乏对"模型与参考偏好排序一致程度"这一基本测量问题的形式化定义和统计基础。本文旨在填补这一概念空白，为偏好对齐提供严格的统计框架。

Method: 定义成对参考对齐为模型评分函数诱导的排序与参考偏好排序一致的概率。引入中心化序参量统计量和基于间隔的扩展，给出独立采样假设下的有限样本估计量和浓度界。在 Qwen2.5 系列模型和 RewardBench 数据集上进行实证分析。

Result: 提出的对齐统计量随模型规模和指令微调而增加，且在不同参考偏好子集上的变化符合理论预测。实证结果验证了该统计量作为模型级别序数可观测量的有效性。

Conclusion: 本文为成对参考对齐提供了概念性和统计性的形式化框架，澄清了参考偏好分布的作用，并将通用序数可观测量与具体的评分选择（如归一化对数概率或基于能量的分数）区分开来。该框架可作为偏好对齐研究的基础测量工具。

Classification reason: 该论文研究成对偏好数据下的模型对齐度量（pairwise reference alignment），核心是定义模型评分与参考偏好排序一致性的统计量，属于生成模型对齐与偏好优化的基础理论问题。该主题与 multimodal_generation 和 aigc 方向下的 [CANONICAL] subtopic "generation_alignment（生成模型对齐与奖励）" 高度匹配，因此复用该 subtopic。

Importance reason: Formalizes alignment measurement but lacks direct connection to user's priority subtopics like continuous VLM or video generation. Signals: alignment observable, model-level ordinal statistic, concentration bounds, Qwen2.5 validation, no direct video/image generation focus
...
TL;DR: 本文发现在线策略蒸馏（On-Policy Distillation）中存在监督保真度衰减（SFD）问题：随着学生生成前缀变长，教师模型的下一token分布变得不自信且区分度下降，导致反向KL蒸馏中的纠正信号减弱。为此提出Lookahead Group Reward（LGR），通过评估学生候选token在后续步骤中诱导的教师置信度来分配组归一化奖励，并设计熵触发的树注意力机制保持计算效率。在6个数学和代码基准上，7B学生模型mean@8提升2.57分，长序列场景（AIME-26，39k tokens）提升达4.92分。


<details>
  <summary>Details</summary>
Motivation: 在线策略蒸馏中，学生模型在自身生成轨迹上训练，教师提供token级反馈。但研究发现随着学生生成序列变长，教师对后续token的预测置信度下降、分布区分度减弱，导致反向KL散度蒸馏中的纠正信号失效，学生模型在长推理链上发生漂移。现有方法未有效解决这一监督信号衰减问题。

Method: 提出Lookahead Group Reward（LGR）：基于"下一步教师置信度反映未来反向KL监督的区分强度"这一洞察，对学生模型top-K候选token，计算每个token在后续步骤中诱导的教师置信度作为奖励，并进行组归一化。同时设计熵触发的树注意力机制（entropy-triggered tree-attention），仅在教师置信度熵较高时展开树搜索，以保持计算效率。

Result: 在6个数学和代码基准（GSM8K、MATH、AIME-24、AIME-26、HumanEval、MBPP）上，7B学生模型mean@8平均提升2.57分；在长序列场景AIME-26（39k tokens）上提升达4.92分。消融实验验证了LGR各组件的有效性。

Conclusion: 本文首次识别并形式化了在线策略蒸馏中的监督保真度衰减问题，提出的Lookahead Group Reward有效缓解了长序列下的学生漂移，在数学推理和代码生成任务上取得显著提升。该方法为提升蒸馏在长推理链场景下的效果提供了新思路。

Classification reason: 该论文研究在线策略蒸馏中教师监督信号衰减问题，提出基于教师置信度的组奖励机制（Lookahead Group Reward），属于生成模型的对齐与奖励优化范畴，与 multimodal_generation 方向下的 canonical subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Focuses on LLM distillation for math/code, not on the user's priority subtopics like continuous VLM or video generation. Signals: on-policy distillation, supervision fidelity decay, lookahead group reward, math and code benchmarks, not multimodal generation
...
TL;DR: 本文提出SAVE框架，利用价值函数作为自适应锚点，将在线策略生成的响应转化为监督信号，通过对比学习目标更新奖励模型，解决了RLHF中奖励模型因策略漂移而退化的问题。在六个基准上验证了有效性，并兼容多种RL算法。


<details>
  <summary>Details</summary>
Motivation: 构建强奖励模型（RM）用于语言模型对齐时，获取多样且可靠的偏好数据成本高昂且困难。随着策略模型不断演化超出静态RM训练范围，RM性能会严重退化。现有方法依赖人工标注或评判模型，难以持续提供高质量反馈。

Method: 提出SAVE（Self-supervised reward model improvement via Value-Anchored On-policy feedback）框架。核心思想是利用价值函数（value function）作为提示特定的自适应锚点，对在线策略生成的响应进行评分并转化为监督信号。具体包括：1）计算RM优势值（advantage）；2）过滤模糊样本；3）通过对比学习目标更新RM。

Result: 在六个多样化基准上进行了严格实证评估，SAVE在所有数据集上均取得优于基线方法的结果，并且在三种RL算法（GRPO、RLOO、GSPO）和不同策略骨干网络上均保持一致的性能提升。

Conclusion: SAVE框架通过在线策略反馈实现了奖励模型的自我监督改进，有效缓解了策略漂移导致的RM退化问题，为RLHF中奖励模型的持续优化提供了一种高效、无需额外人工标注的解决方案。

Classification reason: 该论文提出SAVE框架，通过价值锚定的在线策略反馈来改进奖励模型（RM）训练，核心是RLHF中的奖励建模与偏好优化，与[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Focuses on RLHF reward model improvement, not on the user's priority subtopics like continuous VLM, video generation, or unified multimodal models. Signals: RLHF, reward model, on-policy feedback, self-supervised, language model alignment
...
TL;DR: 本文提出 FedVPA-GP，一种联邦变分偏好对齐框架，通过 Gumbel-Softmax 先验和联邦混合先验解决联邦学习中因数据稀缺和异构性导致的后验坍塌问题，实现个性化用户偏好解耦，在 HH-RLHF 数据集上显著优于单模型基线。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习对齐框架通常使用单一奖励模型，不可避免地平均化用户间冲突的偏好（如有用性与无害性）。变分偏好学习虽能实现个性化，但在去中心化场景中面临后验坍塌问题——本地数据稀缺和异构性导致变分推理不稳定。因此需要一种既能保护隐私又能有效解耦多样化偏好的联邦对齐方法。

Method: 提出 FedVPA-GP 框架：1) 引入联邦混合先验（Federated Mixture Prior），使客户端能够利用聚合的总体分布作为动态先验，稳定变分推理；2) 采用 Gumbel-Softmax 先验实现离散偏好原型的可微采样；3) 引入正交损失（Orthogonal Loss），显式强制偏好原型在潜空间中分离，增强偏好解耦能力。

Result: 在 HH-RLHF 数据集上的实验表明，FedVPA-GP 显著优于单模型基线方法，成功解耦了冲突的用户意图，并支持动态偏好切换。

Conclusion: FedVPA-GP 为联邦学习中的个性化偏好对齐提供了一种有效的隐私保护方案，通过联邦混合先验和正交损失解决了后验坍塌问题，实现了对多样化用户偏好的解耦与动态适应。

Classification reason: 该论文提出联邦变分偏好对齐框架 FedVPA-GP，核心是解决联邦学习中 LLM 对齐时用户偏好冲突与个性化问题，属于生成模型对齐与偏好优化方向。该方向在 multimodal_generation 和 aigc 下均有 canonical subtopic "generation_alignment"（生成模型对齐与奖励），且 aigc 下也有同名 subtopic，因此复用该 subtopic。

Importance reason: Focuses on federated preference alignment for LLMs, not on the user's priority subtopics like continuous VLM, autoregressive video generation, or world models. Signals: Federated learning, preference alignment, Gumbel-Softmax prior, posterior collapse, HH-RLHF dataset
...
TL;DR: 提出 PATHS 方法，利用并行回火（parallel tempering）耦合多条采样链，通过奖励温度阶梯和 Metropolis 交换机制改进扩散/流模型在推理时的奖励对齐初始采样，缓解标准 SMC 方法因初始粒子采样不足而陷入局部模式的问题，在布局到图像和数量感知生成任务上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有推理时奖励对齐方法（如 SMC）从标准先验初始化粒子，但复杂奖励景观中高奖励区域极其稀疏且多模态，导致标准方法难以有效探索稀有高奖励区域，容易陷入局部模式。

Method: 提出 PATHS（Parallel Tempering for High-complexity reward Sampling），维护一组奖励温度阶梯链（从高温到低温），通过周期性 Metropolis 交换操作在不同温度链之间交换粒子状态，使高温链能探索平坦奖励景观，低温链聚焦高奖励区域，从而缓解模式陷阱问题。

Result: 在布局到图像生成和数量感知生成任务上，PATHS 在多个奖励指标上一致优于标准 SMC 和现有奖励感知初始采样方法，尤其在复杂提示上对齐质量提升显著。

Conclusion: 并行回火初始采样能有效提升推理时奖励对齐在复杂奖励景观下的探索能力和对齐质量，为扩散/流模型的推理时可控生成提供了新的有效初始化范式。

Classification reason: 该论文研究推理时奖励对齐（inference-time reward alignment），通过并行回火（parallel tempering）改进扩散/流模型的初始采样以对齐用户指定奖励。这属于生成模型对齐与奖励优化的核心范畴，与 canonical 子主题 generation_alignment（生成模型对齐与奖励）高度匹配。

Importance reason: Focuses on inference-time reward alignment for diffusion/flow models, which is tangentially related to your priority on continuous VLM and flow matching but lacks direct application to visual latent flow or autoregressive generation. Signals: Parallel tempering for reward sampling, addresses mode-trapping in SMC, layout-to-image and quantity-aware generation tasks, no direct link to continuous VLM, autoregressive video/image generation, or world models
...
### [42] [A Unifying View of Variational Generative Wasserstein Flows](https://arxiv.org/abs/2605.31369)
...
TL;DR: 本文提出了生成式Wasserstein流（GWF）的统一理论框架，将多种现有生成模型（扩散模型、GAN等）视为f-散度目标下参数化JKO格式的实例，并建立了若干近期算法之间的等价关系。该框架还扩展到积分概率度量（IPM）和最大均值差异（MMD），推导出新的JKO生成算法，并分析了参数化Wasserstein流的动力学性质。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型虽然都可以看作最小化分布间散度，但依赖不同的算法和几何原理，缺乏统一的视角。作者希望建立一个基于Wasserstein梯度流的统一理论框架，揭示不同生成模型之间的内在联系，并推导新的生成算法。

Method: 论文以Wasserstein梯度流为理论基础，通过Jordan-Kinderlehrer-Otto（JKO）格式的隐式离散化来近似连续时间优化。将多种生成模型统一为f-散度目标下的参数化JKO方案，并扩展到IPM和平方MMD，推导出新的JKO生成算法。同时分析了参数化Wasserstein流中动力学受限于参数化映射诱导分布的情况。

Result: 建立了多种生成模型（扩散模型、GAN等）在Wasserstein梯度流框架下的统一视角，证明了若干近期算法之间的等价关系，推导了基于IPM和MMD的新JKO生成算法，并通过实验研究了JKO正则化对不同目标函数的影响。

Conclusion: 生成式Wasserstein流提供了一个统一的生成模型理论框架，能够解释和连接多种现有方法，并为设计新的生成算法提供了理论指导。该框架的扩展性表明其有望进一步统一更多生成范式。

Classification reason: 该论文提出生成式Wasserstein流（GWF）的统一理论框架，将多种生成模型（扩散模型、GAN等）统一为f-散度下的参数化JKO格式，属于生成模型的理论基础与对齐优化范畴，与multimodal_generation方向下的[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Provides a unifying theoretical framework for generative models but lacks direct application to the user's priority subtopics like continuous VLM or video generation. Signals: Unifying theory, Wasserstein flows, JKO scheme, f-divergence, generative model equivalence
...
### [43] [Trust-Region Behavior Blending for On-Policy Distillation](https://arxiv.org/abs/2605.31159)
...
TL;DR: 提出 Trust-Region Behavior Blending (TRB)，一种用于在线策略蒸馏的预热方法，通过在学生策略的KL信任域内用最接近教师的行为策略替换早期 rollout 策略，并将KL预算退火至零，在数学推理蒸馏任务上取得最优平均性能。


<details>
  <summary>Details</summary>
Motivation: 在线策略蒸馏（OPD）虽然解决了离线蒸馏中前缀不匹配的问题，但早期学生 rollout 质量仍然较差，导致教师监督作用在低质量前缀上效果不佳。需要一种预热方法来改善早期训练阶段的学生策略质量。

Method: 提出 Trust-Region Behavior Blending (TRB)，在训练初期将学生 rollout 策略替换为在KL信任域内最接近教师的行为策略，同时保持每个前缀上的反向KL OPD损失不变。KL预算随训练过程退火至零，使训练在预热后回归纯学生 rollout。

Result: 在两个数学推理蒸馏设置上，TRB 在所有对比方法中取得了最强的平均性能，有效提升了在线策略蒸馏的效果。

Conclusion: TRB 作为一种简单有效的预热方法，通过信任域行为策略混合显著改善了在线策略蒸馏的早期训练质量，在数学推理任务上验证了其有效性。

Classification reason: 该论文研究在线策略蒸馏（on-policy distillation）中通过KL信任域进行行为策略混合以改进数学推理蒸馏效果，属于生成模型的对齐与优化方法，与 canonical subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Focuses on distillation for math reasoning, not on the user's priority subtopics like continuous VLM, video generation, or world models. Signals: on-policy distillation, trust-region, KL divergence, math reasoning, warmup method
...
TL;DR: 本文提出 Count Anything，一个文本引导的跨域通用目标计数模型，并构建了包含约 22 万张图像、619 个类别、1500 万个实例的跨域大规模目标计数数据集 CLOC。模型采用双粒度实例枚举策略，结合区域级稀疏计数器和像素级密集计数器，通过互补计数融合实现多域泛化，在开放世界计数任务上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有目标计数模型通常针对特定领域（如人群、车辆、细胞、农作物、遥感等）设计，难以跨类别、视觉域、目标尺度和密度分布泛化。缺乏统一的跨域计数基准和通用计数模型。

Method: 提出 Count Anything 模型，采用双粒度实例枚举架构：(1) 区域级稀疏计数器（Region-level Sparse Counter）为大尺度稀疏目标提供目标级锚点；(2) 像素级密集计数器（Pixel-level Dense Counter）通过密集点预测处理小尺度、拥挤和弱边界目标。采用点级监督策略统一异构标注，并通过无参数互补计数融合（Complementary Count Fusion）结合两个计数器的输出。

Result: 在 CLOC 数据集及跨域评测中，Count Anything 在多个领域（通用场景、遥感、组织病理、细胞显微、农业、微生物）均取得优异计数精度，显著优于现有开放世界计数方法，展示了强大的多域泛化能力。

Conclusion: 本文证明了通过双粒度实例枚举和跨域数据统一训练，可以构建一个通用的文本引导目标计数模型，有效克服现有计数模型领域碎片化的问题，为开放世界目标计数提供了新的范式。

Classification reason: 该论文构建了跨域大规模目标计数数据集 CLOC 并提出通用计数模型 Count Anything，属于生成模型评测与数据集方向（generation_benchmark），因为其核心贡献之一是统一多领域数据源构建新基准数据集，同时涉及跨域泛化评测。此外，该工作也属于 AIGC 领域内容生成应用方向（aigc_domain_application），因为其面向农业、医学、遥感等多领域计数应用。

Importance reason: Solid counting benchmark and model, but the topic is not a priority subtopic and lacks autoregressive/diffusion generation or continuous VLM focus. Signals: New counting dataset and model, domain-specific counting, no generation focus, no autoregressive or diffusion method, negative keywords present (remote sensing, medical)
...
TL;DR: GraphARC是一个基于图结构的抽象推理基准，将ARC的少样本变换学习范式推广到图数据上，涵盖局部、全局和层次化图变换。评估显示现有语言模型在完整图变换任务上表现有限，存在理解-执行鸿沟和规模扩展障碍。


<details>
  <summary>Details</summary>
Motivation: 现有抽象推理基准（如ARC）局限于网格或文本格式，缺乏对图结构数据的系统评测。图数据在现实世界中广泛存在，需要专门的基准来评估模型的图抽象推理能力。

Method: 提出GraphARC基准，将ARC的少样本变换学习范式推广到图数据。每个任务从少量输入-输出图对中推断变换规则并应用于新测试图，覆盖局部、全局和层次化图变换。支持跨不同图族和大小的规模化生成。

Result: 评估了多种先进语言模型，发现模型能回答图属性问题但难以完成完整图变换任务，存在理解-执行鸿沟。在更大图实例上性能显著下降，暴露出规模扩展障碍。

Conclusion: GraphARC为图基础模型提供了有前景的测试平台，结合了节点分类、链接预测和图生成等任务于统一框架中，揭示了当前模型在图抽象推理方面的局限性。

Classification reason: 该论文提出了GraphARC，一个基于图结构的抽象推理基准，属于生成模型评测与数据集（generation_benchmark）范畴。它扩展了ARC的少样本变换学习范式，涵盖图变换规则推理与图生成，与多模态生成方向下的生成模型评测与数据集这一[CANONICAL]子主题高度匹配。

Importance reason: Graph-based abstract reasoning benchmark is not aligned with user's priority on multimodal generation and continuous VLM. Signals: GraphARC benchmark, abstract reasoning on graphs, limited relevance to user's core topics, no direct connection to video/image generation or continuous VLM
...
### [46] [Benchmarking and Enhancing Text-to-Image Models for Generating Visual Representations in Early Arithmetic Education](https://arxiv.org/abs/2605.31212)
...
TL;DR: 本文提出equation-to-visual生成任务，构建E2V-Bench基准评测文生图模型在早期算术教育中生成教学视觉内容的能力，发现现有模型在物体计数和关系结构上频繁出错，并探索了基于基准的增强策略。


<details>
  <summary>Details</summary>
Motivation: AI系统越来越多地用于教育内容创作，但现有文生图模型能否生成忠实反映教学概念的视觉内容尚不清楚。作者希望通过构建专门的教育视觉生成基准来系统评估和提升模型能力。

Method: 通过教师访谈和教材分析，构建了E2V-Bench基准，涵盖四种教学视觉类型（集合、数轴、十格框、算式图），并设计了自动评估指标（物体计数正确性、关系结构正确性等）。基于基准发现的问题，探索了多种增强策略（如提示工程、视觉引导、微调等）。

Result: 现有文生图模型在E2V-Bench上表现不佳，主要错误类型为物体计数错误和关系结构破坏。基准引导的增强策略能提升代表性模型的表现，但仍有较大改进空间。

Conclusion: 当前文生图模型在算术教育视觉生成任务中缺乏数值和关系结构的精确性，未来模型需要更强的数值和关系理解能力。E2V-Bench为评估和推动这一方向提供了基础。

Classification reason: 该论文构建了E2V-Bench基准，用于评测文生图模型在算术教育场景中生成教学视觉内容的能力，属于生成模型评测与数据集方向，与multimodal_generation下的[CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark）高度匹配。

Importance reason: Focuses on educational content generation, not on the user's priority subtopics like continuous VLM or autoregressive video generation. Signals: Educational application, benchmark for arithmetic visuals, not aligned with priority subtopics, no autoregressive or continuous VLM focus, negative keywords absent
...
### [47] [Conformal Reliability: A New Evaluation Metric for Conditional Generation](https://arxiv.org/abs/2605.30807)
...
Importance: 值得关注 (69.39) - 未入选精读

TL;DR: 本文提出基于共形预测的可靠性分数（reliability score）作为条件生成模型的新评测指标，并设计了 CReL 框架来高效计算该指标，在图像到文本和文本到图像任务上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有条件生成模型的评测指标通常只评估单个输出，无法捕捉生成结果的变异性或潜在风险，缺乏考虑模型固有不确定性的可靠性度量。

Method: 基于共形预测（conformal prediction）定义可靠性分数，衡量在预设置信水平下预测集内的最差性能。提出 CReL 框架，包含两个步骤：(i) 构建具有期望覆盖率的预测集；(ii) 在预测集内优化可靠性分数。提供了覆盖率的理论保证。

Result: 在合成数据和图像到文本、文本到图像任务上的实验表明，该方法能生成比现有方法更具信息量的预测集，可靠性分数具有可解释性，计算框架有效且有效。

Conclusion: CReL 为条件生成模型提供了一种新的可靠性评估范式，弥补了现有指标在不确定性量化方面的不足，具有理论保证和实际可操作性。

Classification reason: 该论文提出基于共形预测的生成模型可靠性评估指标，属于生成模型评测范畴。multimodal_generation 方向下的 [CANONICAL] subtopic "generation_benchmark"（生成模型评测与数据集）完美匹配，因为该 subtopic 明确包含"自动评测指标"和"评估"。同时该工作也涉及 AIGC 评测。

Importance reason: Proposes a new evaluation metric for conditional generation, but is not directly aligned with user's priority subtopics on autoregressive/diffusion generation or continuous VLM. Signals: New evaluation metric, conformal prediction, conditional generation, image-to-text, text-to-image
...
### [48] [3DAE: Binaural Quality Assessment for Audio Novel View Synthesis with Spatial Maps and Benchmark](https://arxiv.org/abs/2605.30469)
*Jialu Xu, Yifan Zhou*
...
TL;DR: 本文提出3DAE，一个全参考诊断框架，利用时频音频误差图（幅度、ILD、IPD、时间对齐、响度、高频失败）对双耳音频新视角合成模型进行可解释的质量评估，并构建了模型无关的基准3DAE Bench。实验揭示了ViGAS模型在不同数据集上的主导失败模式（Replay-NVAS上时间错位，SoundSpaces上ILD不匹配）。


<details>
  <summary>Details</summary>
Motivation: 现有3D音频和新视角声学合成模型通常使用全局指标评估，这些全局指标往往掩盖了双耳预测在何处以及为何失败，缺乏可解释的诊断工具。

Method: 提出3DAE Map，包含6种时频音频误差图（幅度、ILD、IPD、时间对齐、响度、高频失败），用于可视化诊断；在此基础上构建模型无关的基准3DAE Bench，接受任意真实-预测双耳音频对并报告预测质量。

Result: 在ViGAS模型上对Replay-NVAS和SoundSpaces两个数据集进行实验，发现不同数据集上主导失败模式不同：Replay-NVAS上主要是时间错位，SoundSpaces上主要是ILD不匹配。

Conclusion: 该框架提供了可解释的失败模式总结和直观的视觉映射，有助于音频新视角合成模型的开发和优化。

Classification reason: 该论文提出了一个用于双耳音频新视角合成质量评估的诊断框架和基准（3DAE Bench），属于生成模型评测范畴。多模态生成方向下的 [CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark）完美匹配该工作。

Importance reason: Focuses on binaural audio evaluation, not aligned with user's priority on visual generation and multimodal understanding. Signals: Audio novel view synthesis, Binaural quality assessment, Not visual generation, Not multimodal language model, No autoregressive or diffusion focus
...
### [49] [MergeTok: Unified Continuous and Discrete Visual Tokenization via Token Merging](https://arxiv.org/abs/2605.30904)
...
TL;DR: MergeTok 提出一种统一连续（VAE）和离散（VQ）视觉分词器的架构，通过 token 合并技术作为语义桥梁，在编码器-解码器框架内联合优化两种分词器，实现高保真重建与语义可控性的兼顾。


<details>
  <summary>Details</summary>
Motivation: 现有视觉分词器分为连续 VAE 和离散 VQ 两大流派，各有互补性局限：VAE 重建质量高但潜空间稠密纠缠、缺乏语义可控性；VQ 模型支持自回归生成但存在梯度稀疏、训练不稳定和码本坍塌问题。目前缺乏一种统一架构同时兼顾两者的优势。

Method: MergeTok 采用编码器-解码器架构，在编码过程中通过 token 合并（token merging）技术将相似 token 聚类，建立结构先验。该先验提供双重监督信号：(1) 在 VAE 分支施加合并 token 的语义对齐约束，正则化潜空间朝向解耦、语义感知的表征；(2) 导出分组约束，促进组内多样性和组间排他性，稳定 VQ 训练。最终单个架构同时支持自回归和扩散生成器。

Result: 在 ImageNet-256 上，MergeTok 在相同 token 预算下取得了比强 VAE 和 VQ 模型更低的 rFID，同时生成语义组织的 token 表征，兼容自回归和扩散两种生成范式。

Conclusion: MergeTok 证明单一架构可以赋予视觉分词器鲁棒的语义组织能力和生成器友好的离散性，为统一连续与离散视觉分词提供了有效方案。

Classification reason: 该论文提出 MergeTok，统一了连续（VAE）和离散（VQ）视觉分词器，属于统一多模态表征/分词方向。multimodal_generation 方向下已有 id=unified_multimodal_model 的现有 subtopic（统一多模态模型），描述为"理解与生成一体化、统一 token 表征"，与本文主题高度吻合，因此直接复用。

Importance reason: Directly addresses continuous VLM and unified multimodal tokenization, a top priority subtopic with high weight. Signals: continuous visual token, unified multimodal, token merging, VAE and VQ joint optimization, autoregressive and diffusion compatible
...
### [50] [Lumos-Nexus: Efficient Frequency Bridging with Homogeneous Latent Space for Video Unified Models](https://arxiv.org/abs/2605.31603)
...
TL;DR: Lumos-Nexus 提出了一种训练高效的视频统一生成框架，采用两阶段设计：训练阶段仅对齐轻量生成器与理解模块以学习推理驱动的语义控制；推理阶段通过统一渐进式频率桥接（UPFB）在共享潜空间中逐步过渡到高容量预训练生成器，实现粗到细的细化，生成高保真视频。同时提出 VR-Bench 基准评估推理驱动视频生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基于连接器的视频统一模型在指令引导的视频合成方面表现良好，但将大型高保真生成器集成到统一训练循环中计算成本过高，限制了可实现的视觉质量。因此需要一种训练高效的框架，既能发展强大的推理驱动生成能力，又能显著提升视觉保真度。

Method: Lumos-Nexus 采用两阶段设计：(1) 训练阶段，仅将轻量生成器与理解模块对齐，学习接收推理驱动的语义控制；(2) 推理阶段，提出统一渐进式频率桥接（UPFB），在共享潜空间中逐步将生成任务过渡到高容量预训练生成器，实现从粗到细的细化，生成高保真视频而不损害推理质量。同时构建了 VR-Bench 基准来评估推理驱动视频生成能力。

Result: 在 VBench 上，Lumos-Nexus 在视觉真实感和时序一致性上取得了显著提升；在 VR-Bench 上展示了强大的基于推理的生成性能。

Conclusion: Lumos-Nexus 通过训练高效的连接器设计和渐进式频率桥接策略，成功实现了推理驱动的高保真视频生成，为视频统一模型的发展提供了有效方案。

Classification reason: 该论文提出 Lumos-Nexus，一个连接器式的视频统一模型，将理解模块与轻量生成器对齐，并通过渐进式频率桥接（UPFB）在共享潜空间中利用高保真预训练生成器实现推理驱动的视频生成。这属于理解与生成一体化的统一多模态模型方向，因此映射到 multimodal_generation 和 multimodal_understanding_generation 下的现有 subtopic "统一多模态模型"（unified_multimodal_model）。

Importance reason: Directly addresses unified multimodal models and efficient inference, with strong alignment to priority subtopics and boost keywords. Signals: unified multimodal model, efficient inference, video generation, reasoning-driven generation, frequency bridging
...
### [51] [SLAP: The Semantic Least Action Principle for Variational Video-Language Modeling](https://arxiv.org/abs/2605.30750)
...
TL;DR: 本文提出SLAP（语义最小作用量原理），将视频语言模型中的帧间插值问题从概率生成范式转向变分力学范式。通过建立经典力学与语义动力学之间的严格同构，将潜在视频轨迹建模为黎曼流形上的路径，由语义拉格朗日量支配。将插值任务形式化为边界值问题，通过离散欧拉-拉格朗日方程求解，无需像素级渲染即可自然保持物体持久性。


<details>
  <summary>Details</summary>
Motivation: 大型视频语言模型（LVLMs）因稀疏帧采样的计算需求而产生根本性的"时间间隙"，导致模型对关键因果转换失明。现有基于生成式幻觉（如潜在扩散）或自回归外推的方法在长时程上难以保持语义一致性，存在物体消失和能量不稳定性问题。

Method: 提出语义最小作用量原理（SLAP），建立经典力学与语义动力学的同构关系，将潜在视频轨迹建模为黎曼流形上的路径，由语义拉格朗日量支配。将帧插值任务形式化为边界值问题（BVP），通过离散欧拉-拉格朗日方程求解，实现语义一致的视频轨迹生成。

Result: 大量实验表明SLAP在保持语义一致性、物体持久性和能量稳定性方面优于现有方法。

Conclusion: SLAP通过将视频插值从概率生成转向变分力学，为视频语言模型中的时间间隙问题提供了新的解决范式，无需像素级渲染即可自然保持物体持久性。

Classification reason: 该论文提出SLAP（语义最小作用量原理），将视频帧插值建模为黎曼流形上的变分力学问题，通过离散欧拉-拉格朗日方程求解边界值问题，实现语义一致的视频轨迹生成。这属于视频世界模型/视频生成式模拟的范畴，与[multimodal_generation]下的[视频世界模型]（video_world_model）高度匹配。

Importance reason: Directly advances continuous VLM and visual latent flow via a novel variational mechanics framework, aligning with top-priority subtopics. Signals: continuous vision-language model, visual latent flow, variational mechanics, object persistence, semantic Lagrangian
...
### [52] [Robust Dreamer: Deviation-Aware Latent Gaussian Memory for Action-Controlled AR Video Generation](https://arxiv.org/abs/2605.30855)
...
Importance: 高优先级 (96.17) - Top 3 精读候选

TL;DR: Robust Dreamer 提出了一种记忆增强的动作控制 AR 视频生成框架，通过 Latent Gaussian Memory（潜空间高斯记忆）避免 Latent-RGB 循环导致的信息损失，并通过 Deviation Learning（偏差学习）让模型在训练时暴露于推理时可能出现的记忆损坏状态，从而显著提升长程自回归视频生成的视觉保真度和 3D 一致性。


<details>
  <summary>Details</summary>
Motivation: 现有动作控制的图像到视频生成方法在长程自回归生成中面临严重的累积漂移问题，主要源于两个障碍：(1) Latent-RGB 循环导致的信息损失——生成的潜变量反复解码为 RGB 再重新编码用于后续条件化，造成信息退化；(2) 训练-推理差距——训练时使用干净的记忆，而推理时记忆被预测误差污染，模型无法处理这种不匹配。

Method: 提出 Robust Dreamer 框架，包含两个核心组件：(1) Latent Gaussian Memory——将生成过程中继承的扩散潜变量锚定到高斯原语上，通过潜空间高斯泼溅（latent-space Gaussian splatting）召回，提供密集、几何感知、视角对齐的条件信息，避免 VAE 反复转换的累积退化；(2) Deviation Learning with Dynamic Deviation Archive——通过一步近似合成自回归 rollout 导致的潜变量偏差，按自回归阶段和去噪时间戳存储，在训练时注入历史记忆，使生成器暴露于真实的损坏记忆状态并学会内部修正。

Result: 在 ScanNet、DL3DV 和 OmniWorldGame 三个数据集上的实验表明，Robust Dreamer 在长程视频生成任务上达到了最先进的性能，显著优于现有方法，在视觉保真度和 3D 一致性方面均有明显提升。

Conclusion: Robust Dreamer 通过潜空间高斯记忆和偏差学习有效解决了动作控制 AR 视频生成中的长程漂移问题，为交互式世界模拟提供了一种鲁棒的生成范式。该方法验证了在潜空间维护几何感知记忆以及通过合成偏差进行对抗式训练的有效性。

Classification reason: 该论文提出动作控制的图像到视频生成框架用于交互式世界模拟，属于视频世界模型（video_world_model）这一[CANONICAL]子主题。同时涉及AIGC方向下的视频生成应用。

Importance reason: Directly advances action-controlled AR video generation with a novel memory mechanism that solves the critical Latent-RGB cycling drift problem, highly relevant to your top-weighted continuous VLM and video generation priorities. Signals: Latent Gaussian Memory, Deviation Learning, action-controlled video generation, autoregressive video generation, world model
...
### [53] [Physically Viable World Models: A Case for Query-Conditioned Embodied AI](https://arxiv.org/abs/2605.30542)
...
Importance: 高优先级 (89.6) - 未入选精读

TL;DR: 本文指出现有观测预测型世界模型在具身AI中会产生视觉合理但物理错误的预测，提出物理可行的世界模型应基于干预查询（intervention query）构建最简物理抽象，由模块化组件和自主编排器组成，并展示了该方法在现有系统失败案例上的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型仅预测未来观测，可能产生视觉合理但物理错误的推演，导致推荐不可行动作、错误预测交互结果或认证不安全行为。这种失败是结构性的——不同的物理系统可能看起来相同但在干预下表现不同。因此需要一种物理可行的世界模型设计原则。

Method: 提出查询条件化（query-conditioned）的物理可行世界模型框架：模型由环境表示、潜在状态与参数估计、动作规范、干预动力学和查询级响应等模块化组件构成；一个自主编排器（orchestrator）根据查询识别相关抽象并组合兼容的学习和结构化组件；当封闭形式物理不可用时，过渡模型可以是解析、模拟、学习或混合的，但必须保留决定干预结果的结构。

Result: 在控制实验中，固定可见场景而改变潜在物理参数，现有观测预测模型无法正确回答干预查询；而所提出的物理可行世界模型框架能够正确回答这些查询，并展示了编排器如何动态组装和适应物理可行模型用于规划、控制和验证。

Conclusion: 具身AI需要的是能够回答干预查询的物理可行世界模型，而非最详细的世界模型。正确的抽象是保留与查询相关区分的最简模型。该分解使模型可解释、组件可验证、输出可审计，为设计新世界模型和测试现有模型提供了原则。

Classification reason: 该论文讨论具身AI的世界模型，强调物理可行性（physically viable world models），属于视频世界模型（video_world_model）这一[CANONICAL]子主题的范畴，该子主题涵盖世界模型、可交互模拟器、物理世界建模等方向。

Importance reason: Directly addresses a critical failure mode in world models for embodied AI, aligning with the user's high-priority subtopic of video generation world models. Signals: world model, intervention query, physical viability, embodied AI, modular components
...
### [54] [Light Interaction: Training-Free Inference Acceleration for Interactive Video World Models](https://arxiv.org/abs/2605.31158)
...
TL;DR: 本文提出 Light Interaction，一种无需训练的交互式视频世界模型推理加速框架，通过自适应上下文管理、去噪缓存加速和硬件-软件协同设计的 3D 块稀疏注意力，实现高达 2.59 倍加速，同时保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 交互式视频世界模型在实时游戏模拟、虚拟场景导航和具身 AI 训练中具有重要应用，但长交互轨迹的推理成本极高，主要瓶颈包括上下文记忆增长、二次注意力复杂度和重复去噪步骤。现有方法缺乏针对交互场景的推理加速方案。

Method: Light Interaction 基于交互自然产生的轨迹依赖自适应计算这一关键洞察，结合三种技术：1) 自适应上下文管理——探索新区域时丢弃检索到的空间记忆，根据局部潜动态调整时间上下文；2) 去噪缓存加速——当相机回到熟悉区域时复用早期步骤的模型输出；3) 硬件-软件协同设计的 3D 块稀疏注意力，使用融合 Triton 内核实现高效计算。

Result: 在 HY-WorldPlay 和 Matrix-Game-3.0 两个基准上，Light Interaction 实现了最高 2.59 倍的速度提升，且无需模型重新训练，同时保持了有竞争力的视觉质量。

Conclusion: 本文证明了通过利用交互轨迹的自适应特性，可以在不牺牲生成质量的前提下显著加速交互式视频世界模型的推理，为实时交互式视频生成应用提供了实用的加速方案。

Classification reason: 该论文研究交互式视频世界模型的推理加速，属于视频生成式世界模型范畴，与 [CANONICAL] subtopic "视频世界模型" (video_world_model) 高度匹配。

Importance reason: Directly addresses efficient inference for interactive video world models, a high-priority subtopic, with a training-free acceleration framework. Signals: video world model, efficient inference, training-free acceleration, interactive video generation, 3D block sparse attention
...
### [55] [Probabilistic Precipitation Nowcasting with Rectified Flow Transformers](https://arxiv.org/abs/2605.31204)
...
<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的天气临近预报方法依赖确定性压缩来降低高维天气数据的复杂度，限制了解码过程中对不确定性的捕获能力，尤其在极端天气事件中表现不足。

Method: 提出 FREUD 模型，包含逐帧编码器（Frame-wise Encoder）和统一解码器（United Decoder），基于整流流变换器（Rectified Flow Transformer）实现时空天气数据的高效压缩。逐帧编码支持连续预报更新，统一视频解码器保证时序一致性，不确定性保持的第一阶段通过集成方式捕获偶然不确定性。

Result: 在 SEVIR 基准上达到降水临近预报的 SOTA 性能，并通过模型缩放和测试时缩放进一步提升了性能。

Conclusion: 不确定性保持的压缩方法结合整流流变换器能有效提升天气临近预报的准确性，尤其对极端天气事件的预测具有优势。

Classification reason: 该论文使用整流流变换器进行降水临近预报，属于视频生成式世界模型（视频世界模型）范畴，与 canonical subtopic "video_world_model" 高度匹配。

Importance reason: Strong probabilistic nowcasting with rectified flow transformers, but weather domain is not a priority subtopic. Signals: rectified flow transformers, probabilistic nowcasting, video diffusion, uncertainty-preserving encoding, SEVIR SOTA
...
### [56] [PatchWorld: Gradient-Free Optimization of Executable World Models](https://arxiv.org/abs/2605.30880)
...
TL;DR: PatchWorld 提出一种无梯度的框架，通过反例引导的代码修复，将离线轨迹转化为可执行的 Python 世界模型（符号信念状态程序），在 AgentGym 环境中实现无需 LLM 调用的高效规划，并揭示了观测保真度与决策效用之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有文本智能体环境通常建模为 POMDP，其潜在状态和转移动态对智能体是隐藏的。现有方法多使用黑盒模型预测下一观测，缺乏可解释性和可调试性。作者希望诱导出可执行的代码作为世界模型，实现可检查、可重放、可局部修补的符号信念状态程序。

Method: PatchWorld 框架包含三个模块：(1) 世界模型生成器，利用 LLM 从离线轨迹生成初始 Python 世界模型；(2) 反例引导的代码修复器，通过执行模型与真实轨迹对比发现错误并迭代修复；(3) 基于世界模型的规划器，使用生成的世界模型进行一步前瞻（one-step lookahead）规划。整个过程无需梯度优化，世界模型预测模块内部不调用 LLM。

Result: 在 7 个 AgentGym 环境中，PatchWorld-Simple 在基于代码的规划评分中达到最高，宏平均成功率为 76.4%（一步前瞻）。同时发现，引入人类指定的残差记忆偏差可提高表面观测保真度，但会削弱动作判别动态，揭示了可执行世界模型中观测保真度与决策效用之间的权衡。

Conclusion: PatchWorld 证明了通过反例引导的代码修复可以从离线轨迹中诱导出可执行的符号世界模型，在无需 LLM 推理的情况下实现有效规划。该工作揭示了可执行世界模型中观测保真度与决策效用之间的内在权衡，为构建可解释、可调试的智能体世界模型提供了新方向。

Classification reason: 该论文提出通过离线轨迹诱导可执行的Python世界模型，用于部分可观测环境下的预测与规划，属于世界模型（world model）范畴。虽然不直接涉及视频生成，但"视频世界模型"（video_world_model）是multimodal_generation方向下最相关的[CANONICAL]子主题，涵盖生成式世界模型、可交互模拟器等概念，与本论文的"可执行世界模型"精神一致。

Importance reason: Directly addresses world model and neural simulator subtopics with a novel gradient-free executable approach, highly relevant to user's priority on video generation world models. Signals: world model, neural simulator, executable code, gradient-free, counterexample-guided repair
...
### [57] [DecMem: Towards Minute-Long Consistent World Generation with Decoupled Memory](https://arxiv.org/abs/2605.31336)
...
TL;DR: 本文提出 DecMem，一种解耦记忆架构，通过稀疏全局记忆（Sparse Global Memory）和锚定局部记忆（Anchored Local Memory）实现分钟级长时程一致的可控世界生成与视频生成，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有可控世界模型在长时程推理下难以保持细粒度的时空一致性，显式3D记忆和粗粒度帧级隐式建模均存在局限，朴素可学习记忆架构在长程外推中存在计算效率低和注意力分散两大问题。

Method: 提出 DecMem 解耦记忆架构：1）稀疏全局记忆（Sparse Global Memory）实现高效细粒度全局历史访问；2）锚定局部记忆（Anchored Local Memory）提供稳定高质量的外推。通过系统分析注意力分散现象，设计解耦结构分别处理全局一致性和局部生成质量。

Result: 实验表明 DecMem 在长时程一致视频生成任务上显著超越当前最先进方法，实现了分钟级可控长视频生成，具有高保真度和一致性。

Conclusion: DecMem 通过解耦记忆架构有效解决了长时程世界生成中的一致性问题，为分钟级可控视频生成提供了可行方案，展示了精确高效长时记忆在视频世界模型中的关键作用。

Classification reason: 该论文提出 DecMem，一种解耦记忆架构用于长时程一致的世界生成与可控长视频生成，属于视频生成式世界模型范畴，与 [CANONICAL] 视频世界模型 (video_world_model) 高度匹配。

Importance reason: Directly advances minute-long consistent world generation with a novel decoupled memory architecture, highly relevant to video world models and autoregressive video generation. Signals: video world model, long-horizon consistency, decoupled memory, sparse global memory, anchored local memory
...
### [58] [Subspace-Decomposed JEPAs: Disentangling Progression and Content in Latent World Models](https://arxiv.org/abs/2605.31111)
...
TL;DR: 本文提出SD-JEPA，将JEPA潜空间分解为正交的进度子空间（低维，通过余弦间隔三元组损失塑造）和内容子空间（高维，通过SIGReg正则化），证明两个抗坍缩力作用于不相交坐标从而可加性组合。在控制基准上优于LeWM基线，且1D角度进度坐标可作为场景感知的罗盘，在回溯时倒退、在扰动时重新定位到语义合适的任务阶段，在40个隐藏立方体片段上定位语义事件的AUROC提升高达+0.18。


<details>
  <summary>Details</summary>
Motivation: 现有JEPA架构学习紧凑的潜世界模型，但潜空间中没有任何单一坐标被指定编码任务进度，导致模型难以区分"进展到什么阶段"和"场景内容是什么"，限制了世界模型在规划与决策中的效用。

Method: 将JEPA潜空间分解为两个正交子空间：低维进度子空间（通过余弦间隔三元组损失塑造，使角度坐标随任务进度单调变化）和高维内容子空间（通过SIGReg正则化保持多样性）。理论上证明两个抗坍缩力作用于不相交坐标，可加性组合而非竞争。在多个控制基准上评估，并设计了子空间消融实验验证分解的有效性。

Result: SD-JEPA在大多数控制基准上以相同计算量优于LeWM基线，在Push-T上优于最强的非LeWM JEPA基线。1D角度进度坐标可作为场景感知罗盘：随任务进度前进、回溯时倒退、扰动后重新定位到语义合适的任务阶段。在40个隐藏立方体片段上，|Δθ_t|在定位语义事件方面比标准潜预测误差AUROC提升高达+0.18（±1步容忍度下每片段胜率97.5%）。跨4个环境的线性探针显示8维进度子空间（占潜空间4.2%）解释了72-95%的任务进度方差。

Conclusion: SD-JEPA通过解耦进度与内容表征，不仅提升了世界模型的规划性能，还赋予了潜空间可解释的语义进度感知能力，能够区分"意外时刻"与"意外含义"，这是预测误差标量无法做到的。

Classification reason: 该论文提出SD-JEPA，在潜空间中解耦任务进度与内容表征，用于学习潜世界模型（latent world model）并进行规划控制。这属于视频世界模型（video_world_model）范畴，该[CANONICAL]子主题涵盖世界模型、可交互模拟器、物理世界建模等方向。

Importance reason: Directly advances continuous VLM and world model priorities with a novel subspace decomposition for latent progression. Signals: world model, continuous latent representation, progression subspace, cosine-margin triplet loss, scene-aware compass
...
### [59] [Towards Streaming Synchronized Spatial Audio Generation via Autoregressive Diffusion Transformer](https://arxiv.org/abs/2605.30940)
*Ke Lei, Yu Zhang, Changhao Pan, Xueyi Pu, Wenxiang Guo, Ruiqi Li, Zhou Zhao*

Main category: eess.AS, cs.MM, cs.SD
...
TL;DR: SwanSphere 是一个统一的流式框架，通过因果自回归扩散 Transformer 架构，从全景视频和文本提示实时生成高保真空间音频。它引入空间视频-音频对比学习（SVAC）和多目标在线直接偏好优化（ODPO），并开发了自动标注流水线以缓解空间音频数据稀缺问题。
...
Method: 1) 提出因果自回归扩散 Transformer 架构，支持流式高质量空间音频生成；2) 设计空间视频-音频对比学习（SVAC）策略对齐视频编码器与声学域，并采用多目标在线直接偏好优化（ODPO）增强空间感知；3) 开发自动化标注流水线生成详细的空间描述，缓解空间音频数据集稀缺问题。

Result: 在视频到空间音频和文本到空间音频生成任务上均取得了优于现有方法的性能。

Conclusion: SwanSphere 实现了流式、高质量的空间音频生成，有效解决了延迟与质量之间的权衡问题，并为空间音频数据稀缺提供了可行的解决方案。

Classification reason: 该论文提出 SwanSphere，一个从全景视频和文本提示生成流式空间音频的统一框架，属于音视频联合生成与跨模态条件音频生成。该方向与 multimodal_generation 下的 [CANONICAL] subtopic "audio_av_generation（音频与多模态联合生成）" 高度匹配，因此直接复用该 subtopic。

Importance reason: Novel streaming autoregressive diffusion transformer for spatial audio, but outside core priority subtopics. Signals: autoregressive, diffusion transformer, multimodal generation, spatial audio, streaming
...
### [60] [Chatterbox-Flash: Prior-Calibrated Block Diffusion for Streaming Zero-Shot TTS](https://arxiv.org/abs/2605.30748)
...
TL;DR: Chatterbox-Flash 是一种零样本 TTS 模型，通过对预训练自回归 TTS 解码器进行微调，将其转换为块扩散解码器，实现块内并行 token 生成同时保持逐块流式推理。针对离散语音 token 的长尾分布导致并行位置选择偏向高频 token 的问题，提出了先验校准评分和早期解码调度两种推理时技术，在标准基准上达到与强基线相当的高保真合成质量，同时支持流式推理。


<details>
  <summary>Details</summary>
Motivation: 现有零样本 TTS 模型主要分为自回归（AR）和非自回归（NAR）两类。AR 模型质量高但推理速度慢；NAR 模型速度快但通常不支持流式推理。块扩散解码（block-diffusion decoding）在图像生成中已展现出并行生成与流式推理的潜力，但直接迁移到离散语音 token 上会导致质量下降，因为语音 token 的长尾分布使得并行位置选择偏向高频 token。本文旨在解决这一迁移问题，实现兼具高质量、流式推理和低延迟的零样本 TTS。

Method: 1) 模型架构：基于预训练的 AR TTS 解码器（Chatterbox），通过微调将其转换为块扩散解码器，每个块内并行生成多个 token，块间保持自回归流式顺序。2) 先验校准评分（Prior-Calibrated Scoring）：在扩散迭代的 logit 预测中减去块级边际 token 分布，消除长尾分布带来的偏差，使低概率 token 也能被公平选择。3) 早期解码调度（Early-Decoding Schedule）：基于校准后的置信度自适应终止迭代，减少不必要的计算步骤，加速推理。

Result: 在标准零样本 TTS 基准上，Chatterbox-Flash 达到了与强 AR 和 NAR 基线相当的高保真合成质量。在流式推理方面，首包时间（time-to-first-packet）与流式 AR 系统相当，实时因子（real-time factor）显著更低，实现了更高效的推理。

Conclusion: 本文证明了通过先验校准和早期解码调度，可以将块扩散解码成功应用于离散语音 token 的零样本 TTS 生成，在保持高质量的同时实现流式并行推理。该方法无需修改模型架构，仅通过推理时技术即可解决长尾分布带来的质量退化问题，为零样本 TTS 的实用化部署提供了有效方案。

Classification reason: 该论文提出零样本文本到语音（TTS）生成模型，属于音频生成方向，与多模态生成方向下的 [CANONICAL] subtopic "音频与多模态联合生成"（audio_av_generation）高度匹配。同时该工作也属于AIGC范畴。

Importance reason: Novel block-diffusion for streaming TTS, but outside user's priority subtopics and negative keywords apply. Signals: zero-shot TTS, block diffusion, streaming inference, prior-calibrated scoring, early-decoding schedule
...
### [61] [ImmersiveTTS: Environment-Aware Text-to-Speech with Multimodal Diffusion Transformer and Domain-Specific Representation Alignment](https://arxiv.org/abs/2605.30965)
...
TL;DR: ImmersiveTTS 是一种环境感知的文本到语音模型，基于多模态扩散 Transformer，通过联合注意力机制融合文本对齐的语音潜变量与环境上下文，并引入领域特定的表征对齐目标，实现与环境音频自然融合的语音生成。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导音频生成在音效、语音、音乐等领域取得了进展，但联合生成与环境音频无缝融合的语音仍然困难，主要由于语音与环境音频在声学模式和时序动态上存在固有差异。

Method: 基于多模态扩散 Transformer 架构，通过联合注意力机制融合文本对齐的语音潜变量与文本条件的环境上下文；引入领域特定的表征对齐目标，利用语音和音频编码器的互补自监督表征来增强语义一致性。

Result: 在客观指标和人类听力测试中，ImmersiveTTS 在自然度、可懂度和音频保真度方面均优于现有方法。

Conclusion: ImmersiveTTS 通过显式建模跨模态交互和领域特定的表征对齐，有效解决了环境感知 TTS 中语音与环境音频的融合难题，实现了更自然、更高质量的生成效果。

Classification reason: 该论文提出环境感知的文本到语音（TTS）模型，融合语音与环境音频的跨模态生成，属于多模态生成方向下的 [CANONICAL] 子主题"音频与多模态联合生成"（audio_av_generation），该子主题涵盖 TTS、语音合成、音视频联合生成等。

Importance reason: Focuses on environment-aware TTS, which is outside the user's priority subtopics and lacks strong overlap with boost keywords. Signals: multimodal diffusion transformer, domain-specific representation alignment, environment-aware TTS, cross-modal speech-audio generation, limited relevance to visual or autoregressive generation
...
### [62] [Controllable Lung Nodule Synthesis via Histogram-Regularized Latent Diffusion Models](https://arxiv.org/abs/2605.30631)
...
Importance: 普通相关 (35.26) - 未入选精读

TL;DR: 本文提出一种可控的潜在扩散模型，通过直方图正则化约束体素强度分布，在3D CT体积中合成肺部结节，提升不同亚型（实性、部分实性、磨玻璃）结节的视觉真实感和亚型一致性，并验证了其在数据增强和下游分类任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 基于CT的肺癌筛查中，自动化诊断系统受限于多样化、带标注的肺结节数据集的稀缺性。现有条件扩散模型主要优化空间重建损失，鼓励体素级相似性，但未能充分约束病灶级别的强度分布，导致生成结节纹理过于平滑，无法准确表征不同亚型（实性、部分实性、磨玻璃）的衰减特性。

Method: 提出一种可控潜在扩散模型，结合亚型、空间掩码和HU直方图条件，并引入可微的特征空间直方图正则化项，在生成过程中约束体素强度分布，而非仅依赖空间损失。该直方图正则化项通过可微操作将预测与目标直方图对齐，使生成结节更符合真实CT值分布。

Result: 在肺部CT数据上的实验表明，该方法在定量指标和视觉图灵测试中均达到强视觉真实感。用于数据增强时，生成的结节提升了下游临床任务性能，尤其对欠代表性结节亚型效果显著，并对亚型感知的恶性分类有潜在益处。

Conclusion: 直方图正则化能有效约束生成结节的强度分布，提升医学图像合成的亚型一致性和视觉真实性，为医学影像数据增强提供了一种可行方案。

Classification reason: 该论文聚焦于肺部结节合成这一医疗垂直领域生成应用，属于"领域内容生成应用"（aigc_domain_application）范畴，该 subtopic 在 multimodal_generation 和 aigc 方向下均有收录，且为 CANONICAL 条目，因此直接复用。

Importance reason: Focuses on medical CT nodule synthesis, which is outside the user's priority subtopics and includes negative keywords. Signals: Medical domain, negative keyword "clinical", no overlap with priority subtopics like video generation or continuous VLM, uses diffusion but not for video/world model
...
### [63] [PhyDrawGen: Physically Grounded Diagram Generation from Natural Language](https://arxiv.org/abs/2605.30512)
...
TL;DR: PhyDrawGen 是一个神经符号流水线，通过 LLM 提取场景图、确定性求解器转换为精确几何图元、Qwen-VL 视觉验证循环三步，从自然语言问题生成符合物理定律的示意图，在力学、光学和电磁学 1449 个问题上显著优于 GPT-5-image 等模型。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型生成的物理示意图虽然视觉上合理，但系统性地出现力向量幻觉、忽略守恒定律、违反几何约束等问题，无法满足物理学科对严格物理定律遵循的要求。

Method: PhyDrawGen 采用三阶段神经符号流水线：(1) LLM 从问题文本中提取带类型的场景图；(2) 确定性求解器将场景图转换为平面直线图(PSLG)，编码力平衡、光路和场拓扑为精确几何图元；(3) 微调后的 Qwen-VL 模型执行视觉化的"提议-验证"循环，迭代修正任何约束违反。

Result: 在涵盖力学、光学和电磁学的 1449 个问题基准上，PhyDrawGen 在物理准确性上显著优于 GPT-5-image、Gemini 2.5 Flash 和 Gemini 3 Pro，即使在处理非常规物体问题时也表现出鲁棒的物理准确性。

Conclusion: 将神经语义理解与确定性物理约束求解相结合的神经符号方法，能够有效解决生成模型在物理示意图生成中的幻觉和约束违反问题，为科学领域的内容生成提供了可行范式。

Classification reason: 该论文提出从自然语言生成物理示意图的神经符号流水线，属于科学/物理领域的垂直内容生成应用，与现有 canonical 子主题"领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: 该论文聚焦于物理示意图生成，属于领域应用，与用户优先的连续VLM、自回归视频/图像生成等核心方向关联较弱。 Signals: 神经符号流水线, 物理约束满足, 视觉验证循环, 领域特定应用, 非核心生成范式
...
TL;DR: 本文提出 Dual-Spectral Flow Matching (DSFM)，一种基于小波变换和离散余弦变换的双谱流匹配框架，用于生成生理上可信的 fMRI BOLD 时间序列数据，并通过下游脑网络分类任务验证了生成数据的有效性。


<details>
  <summary>Details</summary>
Motivation: fMRI 数据采集成本高、样本有限，限制了数据驱动脑分析模型的发展。现有生成模型难以复现 fMRI BOLD 信号的非平稳性、复杂时空动态和生理变化，亟需一种能生成高保真 fMRI 时间序列的方法。

Method: 提出 DSFM 框架：首先通过离散小波变换 (DWT) 将 BOLD 信号转换为小波分解图以捕获多尺度瞬态变化，再通过离散余弦变换 (DCT) 将信号投影到频域空间以利用低频系数的能量压缩特性；然后训练一个谱流匹配 (spectral flow matching) 模型生成类别条件的余弦频率表示；最后通过逆 DCT 和逆 DWT 重建时域 BOLD 信号。

Result: 在 UK Biobank 数据集上，DSFM 生成的 fMRI 数据在保留生理动态特性方面优于现有方法，且在下游脑网络分类任务中提升了分类性能。

Conclusion: DSFM 通过双变换结构施加结构化频率先验，能够生成生理上可信的 fMRI 时间序列，为脑疾病识别等下游任务提供了有效的数据增强手段。

Classification reason: 该论文聚焦于fMRI脑功能成像数据的生成，属于医疗/科学领域的生成应用，与现有非规范化的 subtopic "aigc_domain_application（领域内容生成应用）" 高度匹配。该 subtopic 在 multimodal_generation 和 aigc 方向下均有收录，且描述明确包含"医疗/科学等垂直领域生成应用"。

Importance reason: The paper focuses on fMRI time series generation for medical brain disorder identification, which is outside the user's priority subtopics and is explicitly penalized by negative keywords. Signals: fMRI generation, medical application, spectral flow matching, wavelet transform, brain disorder classification
...
### [65] [What Gets Unmasked First? Trajectory Analysis of Diffusion Models for Graph-to-Text Generation](https://arxiv.org/abs/2605.31564)
...
TL;DR: 本文首次系统研究掩码扩散语言模型（MDLM）在图到文本生成中的解码轨迹，发现MDLM优先生成实体词、再生成关系词和功能词、最后生成结构词。SFT会破坏这一策略，导致输出长度固定和幻觉。作者提出λ缩放结构解码（训练无关的推理时修改）和Graph-LLaDA模型，在LAGRANGE数据集上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 图到文本生成（如知识图谱描述）是重要的结构化生成任务，但现有研究主要基于自回归LLM。掩码扩散语言模型（MDLM）作为一种新兴的非自回归生成范式，其解码轨迹特性尚未被研究，且SFT对MDLM的影响也未被探索。

Method: 1) 系统分析MDLM解码轨迹，发现实体优先、关系词次之、结构词最后的自然解码顺序；2) 发现SFT会破坏该策略，使结构词过早解码，固定输出长度；3) 提出λ缩放结构解码（λ-scaled structural decoding），在推理时降低结构词置信度以恢复自然解码顺序；4) 提出Graph-LLaDA，将图Transformer编码器集成到LLaDA解码过程中以显式编码图结构信息。

Result: λ缩放结构解码在BLEU-4上提升+9.4；Graph-LLaDA在LAGRANGE跨数据集评估中表现优异；发现以往基线过拟合数据集特定模式，而LLM和MDLM方法泛化性更好。

Conclusion: MDLM在图到文本生成中展现出与自回归模型不同的解码策略，SFT会破坏这一自然策略，而训练无关的推理时干预可以有效恢复。将图结构编码器与MDLM结合能进一步提升生成质量。

Classification reason: 该论文研究图到文本生成（graph-to-text generation），属于结构化数据到文本的生成任务，可归入"领域内容生成应用"（aigc_domain_application）这一现有子主题。该子主题涵盖医疗/法律/科学/代码等垂直领域生成应用，图到文本生成（如知识图谱描述）属于结构化数据到文本的生成范畴。

Importance reason: Focuses on graph-to-text generation with MDLMs, which is not a priority subtopic and lacks strong alignment with user's key boost keywords. Signals: graph-to-text generation, masked diffusion language model, trajectory analysis, SFT failure mode, lambda-scaled structural decoding
...
### [66] [Rationalize: Shared Semantic Reasoning for Human-AI Alignment](https://arxiv.org/abs/2605.30632)
*Aritra Dasgupta, Naga Datha Saikiran Battula, Avina Nakarmi, Sohom Sen, Subhodeep Ghosh, Xun Song*

Main category: cs.HC, cs.LG, cs.AI
...
TL;DR: Rationalize 是一个角色对框架，通过将人机交互建模为四种互补角色对（探索者-引导者、调查者-信息提供者、教师-学生、法官-辩护人），在共享推理空间中显式化目的、问题、假设、证据、推理和含义，实现人类与 AI 模型在语义推理层面的对齐，而不仅仅是输出层面的对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的人机对齐方法主要关注输出层面的对齐（如 RLHF），缺乏对推理过程和意图层面的对齐。人类分析师与 AI 模型在数据驱动的意义构建过程中需要更深层次的语义推理对齐，以建立真正的协作关系。

Method: 提出 Rationalize 角色对框架，定义四种互补角色对（Explorer-Guide, Investigator-Informant, Teacher-Student, Judge-Advocate），在共享推理空间中显式化六个推理要素（目的、问题、假设、证据、推理、含义），并关联到双向人机对齐框架（AI 对齐人类 vs 人类对齐 AI），为对齐设计与评估提供元素级和角色特定的方法。

Result: 论文提出了一个概念性框架和协作研究议程，尚未提供实验验证或定量结果。框架将角色对映射到双向对齐的不同维度，并勾勒了元素级和角色特定的对齐评估方法。

Conclusion: Rationalize 框架为人类与 AI 在语义推理层面的对齐提供了系统化的概念基础，通过角色对和共享推理空间的设计，有望推动更深层次的人机协作对齐研究。

Classification reason: 该论文提出 Rationalize 框架，将人机交互建模为互补角色对（Explorer-Guide, Investigator-Informant, Teacher-Student, Judge-Advocate），在共享推理空间中实现人类与 AI（如 LLM）的对齐。这属于 AI 智能体与生成工作流（aigc_agent_workflow）范畴，涉及 LLM 智能体协作、人机对齐与交互工作流设计。同时与多模态理解与生成方向下的对齐主题相关。

Importance reason: Focuses on human-AI interaction alignment in data sensemaking, not on core generative model or multimodal generation priorities. Signals: Human-AI alignment, shared reasoning, role-pair framework, not generative model, not multimodal
...
### [67] [Extending the UXR Point of View Pyramid: A Generative AI-Augmented Methodology for Human-Centred AI Systems](https://arxiv.org/abs/2605.31143)
...
TL;DR: 本文针对英国金融服务中AI驱动的债务管理技术，扩展了用户体验研究（UXR）的PoV金字塔框架，提出了一种生成式AI增强的方法论，包括AI增强的PoV金字塔、结构化提示架构和AI驱动的Playbook Card系统，将生成式AI作为认知支持工具嵌入UXR工作流，同时保持可追溯性和伦理监督。


<details>
  <summary>Details</summary>
Motivation: 英国日益增长的家庭债务和生活成本压力使得AI驱动的金融科技在信贷评估、还款结构和债务支持服务中扮演越来越重要的角色。然而，现有UXR PoV框架并非为AI中介的金融系统设计，无法充分处理可解释性、公平性和问责制等关键问题，因此需要一种AI增强的方法论来改进高风险的金融AI环境中的UXR实践。

Method: 本文形式化地提出了三个核心组件：(1) AI增强的UXR PoV金字塔，扩展了传统框架以适配AI金融系统；(2) 结构化提示架构，用于综合研究证据和生成假设；(3) AI驱动的Playbook Card系统，将生成式AI嵌入UXR工作流，同时保持可追溯性和伦理监督。生成式AI被定位为认知支持机制而非分析权威，需经过人工验证和监管意识审查。

Result: 本文提出了一个完整的AI增强UXR方法论框架，并将其应用于债务管理技术场景（包括可负担性评估、还款规划和财务压力预测系统），展示了生成式AI如何在不牺牲可追溯性和伦理监督的前提下增强UXR工作流。

Conclusion: 该工作推进了高风险金融AI环境中的UXR方法论发展，为负责任的AI驱动UXR实践做出了贡献，并为CHI社区提供了可操作的框架。生成式AI作为认知支持工具而非分析权威的定位，确保了人类验证和监管合规在AI辅助决策中的核心地位。

Classification reason: 该论文将生成式AI嵌入到用户体验研究（UXR）工作流中，提出AI增强的UXR PoV金字塔和Playbook Card系统，属于AI智能体与生成工作流（aigc_agent_workflow）范畴。

Importance reason: Focuses on UX research methodology for financial AI, not on core technical advances in multimodal generation or autoregressive models. Signals: Human-centred AI, UX methodology, financial debt management, generative AI as cognitive support, no technical model contribution
...
### [68] [Crafter: A Multi-Agent Harness for Editable Scientific Figure Generation from Diverse Inputs](https://arxiv.org/abs/2605.30611)
...
TL;DR: Crafter 是一个多智能体框架，用于从多种输入条件（文本、图表、代码等）生成可编辑的科学图表，并配套 CraftEditor 将栅格输出转换为可编辑 SVG，以及 CraftBench 评测基准。


<details>
  <summary>Details</summary>
Motivation: 现有科学图表生成系统仅支持单一图表类型和纯文本输入，且输出为不可局部编辑的栅格图像，无法满足科研人员多样化的图表类型和输入条件需求。

Method: 提出 Crafter 多智能体框架，通过多个 LLM 智能体协作完成图表生成，无需修改架构即可泛化到不同图表类型和输入条件；CraftEditor 将栅格输出转换为可编辑 SVG 格式；CraftBench 覆盖三种图表类型和四种输入条件，并带有人工质量标注。

Result: Crafter 在 PaperBanana-Bench 和 CraftBench 上显著优于独立生成模型和智能体基线；CraftEditor 在 SVG 转换质量上超越所有基线方法。

Conclusion: 多智能体框架能有效解决科学图表生成中结构化布局的局部错误问题，且无需更强的生成模型，为科学图表自动化生成提供了实用方案。

Classification reason: 该论文提出基于多智能体的科学图表生成系统 Crafter，属于 AI 智能体驱动的自动化创作流水线，与 [CANONICAL] 子主题 "AI 智能体与生成工作流" 高度匹配。

Importance reason: Focuses on scientific figure generation via multi-agent harness, not on core multimodal generation or video/image autoregressive models. Signals: Multi-agent framework, scientific figure generation, editable SVG output, benchmark creation, not aligned with priority subtopics
...
### [69] [DiTTo: Scalable Order-aware All-in-One Image Restoration Agent](https://arxiv.org/abs/2605.30915)
...
TL;DR: DiTTo 提出了一种可扩展的、顺序感知的全能图像恢复智能体框架，包含 DiTTo 模拟器（高效生成最优恢复动作轨迹数据集）和 DiTTo Agent（通过顺序感知恢复对齐训练），实现了对新恢复专家的即插即用扩展，在 MiO-100 多退化恢复任务上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 真实图像往往同时存在多种退化（如模糊、噪声、低光照等），且去除退化的顺序对最终恢复质量影响显著。现有基于智能体的图像恢复方法需要 O((N^D)^2) 次专家调用构建最优恢复轨迹数据集，且训练好的智能体无法扩展新的恢复专家，效率和可扩展性严重受限。

Method: 提出 DiTTo 框架，包含两个核心组件：(1) DiTTo 模拟器，结合 ∪S-IR（单步恢复动作模拟）和 AiO-IQA（每步质量预测），将 ORTD 构建复杂度降至 O(N^D)；(2) DiTTo Agent，通过 SFT 在模拟器生成的 ORTD 上训练，再通过顺序感知恢复对齐（ORA）沿退化识别、恢复动作排序和输出格式三个独立轴进行对齐训练，支持新恢复专家的即插即用扩展。

Result: 在 MiO-100 评估集（最多五种并发退化）上，DiTTo Agent 在多项多退化图像恢复指标上超越了所有先前的基于智能体的图像恢复方法，验证了其高效性和可扩展性。

Conclusion: DiTTo 通过模拟器驱动的训练数据生成和顺序感知对齐训练，有效解决了基于智能体的图像恢复方法在效率和可扩展性上的瓶颈，为构建可扩展的多退化图像恢复系统提供了新范式。

Classification reason: 该论文提出基于视觉语言模型的图像恢复智能体（DiTTo Agent），通过调度多个预训练恢复专家模块实现多退化图像恢复，属于LLM智能体与自动化工作流范畴，与aigc_agent_workflow（AI智能体与生成工作流）高度匹配。

Importance reason: Novel agent-based image restoration framework with scalable plug-and-play extension, but outside user's priority subtopics. Signals: Agent-based image restoration, Order-aware restoration alignment, Scalable plug-and-play extension, MiO-100 SOTA, Vision-language model scheduling
...
### [70] [Exploring Autonomous Agentic Data Engineering for Model Specialization](https://arxiv.org/abs/2605.30407)
...
Main category: cs.LG, cs.IR, cs.CL, cs.AI
...
TL;DR: 本文提出"自主智能体数据工程"任务，让 LLM 作为自主数据工程师，通过规划、生成和迭代优化训练数据来驱动模型专业化。实验表明，GPT-5.2 构建的训练课程使学生模型性能提升 57.29%，完全通过迭代的智能体驱动数据适配实现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用任务上表现优异，但缺乏高质量领域特定数据时难以适应专业领域。现有基于 LLM 的数据策展方法主要依赖人工设计的工作流，尚未探索 LLM 能否自主执行端到端的数据工程流水线来实现模型专业化。

Method: 形式化定义了"自主智能体数据工程"任务，将数据视为可优化组件，让 LLM 智能体自主规划、生成并迭代优化训练数据，以模型微调后的性能提升作为反馈信号指导数据改进。采用多轮迭代框架，智能体根据验证集性能自动调整数据生成策略。

Result: GPT-5.2 作为自主数据工程师，通过迭代的智能体驱动数据适配，使学生模型性能提升 57.29%。实验在多个专业领域验证了方法的有效性，展示了自主数据工程作为可衡量能力的潜力。

Conclusion: 本文揭示了自主 LLM 数据工程师在模型专业化中的巨大潜力，同时指出了当前存在的瓶颈，为智能体驱动的模型专业化开辟了新路径，并将自主数据工程确立为一种可衡量的能力。

Classification reason: 该论文研究 LLM 作为自主数据工程师，通过端到端数据策展流水线实现模型专业化，属于 LLM 智能体驱动的自动化数据工程工作流，与 [CANONICAL] 子主题 "AI 智能体与生成工作流" 高度匹配。

Importance reason: Focuses on agentic data engineering for model specialization, which is not a priority subtopic and lacks direct connection to user's core interests in multimodal generation and continuous representations. Signals: Agent-driven data curation, Model specialization via LLM, Iterative data optimization, Not multimodal generation, No video/image generation focus
...
### [71] [Harness Updating Is Not Harness Benefit: Disentangling Evolution Capabilities in Self-Evolving LLM Agents](https://arxiv.org/abs/2605.30621)
...
TL;DR: 本文系统分析了 LLM 智能体在自演化过程中的两种能力：harness-updating（产生有用 harness 更新的能力）和 harness-benefit（从更新中受益的能力）。研究发现：harness-updating 能力在不同能力层级的模型间差异不大，而 harness-benefit 能力呈非单调关系——中等能力模型受益最大，弱模型和强模型受益较少。弱模型受益低的原因包括无法激活相关 harness 工件或无法忠实遵循指令。


<details>
  <summary>Details</summary>
Motivation: LLM 智能体越来越多地部署为围绕可编辑外部 harness（提示、技能、记忆、工具）构建的系统，这些 harness 通过执行经验进行自演化。然而，模型的基础任务解决能力与其 harness 自演化能力之间的关系尚不明确：哪些模型能产生有用的 harness 更新？哪些模型能真正从中受益？本文旨在厘清这两种能力与模型基础能力之间的关系。

Method: 本文将 harness 自演化分解为两种能力：harness-updating（从执行证据中产生有用持久 harness 更新的能力）和 harness-benefit（在任务求解中从更新后的 harness 中受益的能力）。通过设计实验，对不同能力层级（弱、中、强）的模型进行系统比较，分析两种能力与模型基础能力的关系，并诊断弱模型受益低的失败模式。

Result: 1) Harness-updating 能力在不同能力层级模型间差异不大，Qwen3.5-9B 的更新收益与 Claude Opus 4.6 相当。2) Harness-benefit 能力呈非单调关系：弱模型受益很小，中等模型受益最大，强模型受益反而低于中等模型。3) 弱模型受益低的原因有两种失败模式：无法激活相关 harness 工件，或激活后无法忠实遵循。

Conclusion: 研究表明，在自演化智能体系统中，应将能力预算投入到任务求解智能体本身而非演化器；同时应针对 harness 调用和长程指令遵循进行智能体训练。这为 LLM 智能体的自演化设计提供了重要指导。

Classification reason: 该论文研究 LLM 智能体的自演化能力，分析智能体在提示、技能、记忆和工具等外部 harness 上的更新能力与收益能力，属于 AI 智能体与生成工作流范畴，与 [CANONICAL] subtopic aigc_agent_workflow 高度匹配。

Importance reason: Focuses on LLM agent self-evolution, not on the user's priority subtopics like continuous VLM, video generation, or multimodal models. Signals: LLM agent evolution, harness-updating vs harness-benefit, non-monotonic benefit, weak model failure modes, not multimodal or generative
...
### [72] [Planner-Centric Reinforcement Learning for Deep Research with Structure-Aware Reward](https://arxiv.org/abs/2605.30824)
...
TL;DR: 提出 DecomposeR，一种以规划者为中心的深度研究框架，将研究计划表示为类型化有向无环图（DAG），通过两阶段强化学习（规划器 RL + 回答器 RL）分别优化计划分解与分支执行，在长文本基准上相比强基线提升 5.1-8.0 分。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究训练范式要么依赖短格式可验证 QA 作为代理任务，要么优化整体长轨迹，导致规划与执行难以解耦，且对规划过程的信用分配薄弱。需要一种显式、结构化、可奖励的研究规划方法。

Method: 将研究计划表示为类型化有向无环图（DAG），使规划显式化、结构化且可奖励。训练 Qwen3-8B 模型分两阶段：规划器 RL 先学习图结构和查询分解以改进研究规划，回答器 RL 再学习分支级执行和最终综合，将奖励分配给显式的规划 token 和结构化组件而非扁平轨迹。

Result: DecomposeR-8B 在多个长文本基准上相比强开源基线提升 5.1-8.0 分，验证了结构化规划奖励对深度研究任务的有效性。

Conclusion: 通过将规划显式建模为 DAG 并分阶段优化，DecomposeR 实现了更细粒度的规划优化，减少了端到端训练的模糊性，为深度研究任务提供了一种有效的 planner-centric 训练范式。

Classification reason: 该论文提出 DecomposeR，一个面向深度研究的 planner-centric 框架，将研究计划建模为有向无环图（DAG），并通过两阶段强化学习优化规划与执行。这属于 LLM 智能体与自动化工作流范畴，与 [CANONICAL] subtopic "AI 智能体与生成工作流" 高度匹配。

Importance reason: Strong RL-based planning framework for deep research, but misaligned with user's focus on multimodal generation and continuous representations. Signals: RL for planning, DAG-structured research, long-form QA, weak multimodal connection, no visual token or generation focus
...
### [73] [UniScale: Adaptive Unified Inference Scaling via Online Joint Optimization of Model Routing and Test-Time Scaling](https://arxiv.org/abs/2605.30898)
...
TL;DR: UniScale 提出统一推理扩展（UIS）框架，将模型路由与测试时扩展联合建模为上下文多臂老虎机问题，通过 LinUCB 在线学习最优推理策略，在动态推理场景中实现更细粒度的质量-成本权衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法将模型路由（在不同规模模型间切换）和测试时扩展（TTS，在固定模型内调整推理计算量）作为两个独立维度处理，导致模型路由只能提供粗粒度的离散性能变化，而单模型TTS存在容量上限和边际收益递减问题，两者分离限制了动态推理环境中的适应性。

Method: 提出统一推理扩展（UIS）框架，将模型路由和TTS统一到同一优化空间中。在此基础上设计UniScale在线框架，将自适应UIS建模为上下文多臂老虎机问题，使用LinUCB算法学习推理策略，并引入效率感知学习和成本建模，确保在高维动作空间上的稳定可扩展优化。

Result: 实验表明，UniScale能够有效利用UIS空间中的协同效应，在多样化的动态推理场景中实现更细粒度的、持续更优的质量-成本权衡。

Conclusion: 统一模型路由与测试时扩展的联合优化能够突破各自独立优化的局限性，在线学习框架UniScale为LLM推理部署提供了一种自适应、细粒度的质量-成本平衡方案。

Classification reason: 该论文提出UniScale框架，将模型路由（在不同规模LLM间切换）与测试时扩展（TTS）统一为在线联合优化问题，属于LLM推理工作流的自动化与智能调度，与AIGC方向下的"AI智能体与生成工作流"（aigc_agent_workflow）高度匹配。

Importance reason: Focuses on LLM inference optimization, not directly on user's priority subtopics like continuous VLM or video generation. Signals: Model routing, test-time scaling, online learning, quality-cost trade-off, LLM inference
...
### [74] [Counterfactual Graph for Multi-Agent LLM Calibration](https://arxiv.org/abs/2605.30653)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统中，智能体之间的通信会导致相关性故障和虚假共识，使得简单的投票一致性无法可靠反映答案质量。现有方法忽略了通信拓扑对共识可靠性的影响。

Method: 提出CAGE-CAL框架，对每个查询构建一个观测到的通信后智能体图，并与一个匹配的反事实无通信图进行对比，捕获成对故障相关性和组级依赖关系，估计观测与无通信依赖之间的反事实偏移，并据此校准置信度。

Result: 在五个基准上，CAGE-CAL提升了可靠性判别能力，ECE指标具有竞争力，且校准后的置信度进一步改进了拓扑选择，优于最佳固定拓扑策略。

Conclusion: 多智能体LLM系统的通信拓扑会影响共识的可靠性，通过反事实图校准可以有效缓解虚假共识问题，提升系统输出的可信度。

Classification reason: 该论文研究多智能体LLM系统的校准与可靠性问题，属于LLM智能体工作流范畴，与[AIGC]方向下的[AI智能体与生成工作流]（aigc_agent_workflow）高度匹配。
...
### [75] [ElasticMem: Latent Memory as a Learnable Resource for LLM Agents](https://arxiv.org/abs/2605.30690)
...
TL;DR: ElasticMem 提出一种可学习的弹性潜记忆框架，通过离线记忆库、自适应检索、可变潜预算分配和策略优化，使 LLM 智能体能够更高效地利用长期记忆，在记忆密集型 QA 和具身智能体控制任务上显著提升性能并降低 token 开销。


<details>
  <summary>Details</summary>
Motivation: 现有记忆增强方法将记忆视为固定资源：文本空间方法将检索到的记忆拼接到上下文中，导致大量 token 开销且对噪声敏感；潜空间方法虽减少文本成本，但仍依赖刚性检索或固定容量接口。这造成了查询相关的记忆效用与固定记忆分配之间的不匹配。

Method: ElasticMem 构建离线潜记忆库（含检索键和内容缓存），从推理器的隐藏状态自适应检索记忆，通过学习到的策略为每个检索到的记忆分配可变的潜预算，并将选中的潜状态作为软记忆 token 注入生成过程。整个记忆使用过程通过 GRPO（Group Relative Policy Optimization）在下游任务奖励下进行优化。

Result: 在 MemorySuite 的记忆密集型 QA 和 ALFWorld 具身智能体控制任务上，基于 Qwen2.5-3B-Instruct 和 Qwen2.5-7B-Instruct 骨干，ElasticMem 的加权平均 QA 准确率分别提升 26.2% 和 24.6%，ALFWorld 成功率分别提升 66.3% 和 27.2%，同时实现了最低的 ALFWorld token 成本。

Conclusion: 自适应检索和弹性预算分配帮助 ElasticMem 优先利用有用证据和可迁移计划，超越了刚性余弦相似度方法。该工作展示了将记忆作为可学习弹性资源的有效性，为 LLM 智能体的长期记忆管理提供了新范式。

Classification reason: 该论文提出 ElasticMem，一种为 LLM 智能体设计的可学习弹性记忆框架，核心是增强智能体的长期记忆与推理能力。这与 [CANONICAL] 子主题 "AI 智能体与生成工作流"（aigc_agent_workflow）高度匹配，该子主题涵盖 LLM 智能体、工作流、工具使用等方向。

Importance reason: Novel latent memory framework for LLM agents, but outside user's priority subtopics and direction weights. Signals: LLM agent memory, latent memory bank, adaptive retrieval, elastic budget allocation, token efficiency
...
### [76] [ExpGraph: Model-Agnostic Experience Learning with Graph-Structured Memory for LLM Agents](https://arxiv.org/abs/2605.30712)
...
Importance: 值得关注 (68.1) - 未入选精读

TL;DR: ExpGraph 是一个模型无关的经验学习框架，通过自演化的经验图结构存储历史轨迹中的可复用技能和失败教训，结合图扩散检索和效用感知排序，使冻结的 LLM 执行器无需参数更新即可通过外部经验重用提升性能。在问答、数学推理、代码生成和多步智能体环境（ALFWorld、AppWorld）中，ExpGraph 相比最强基线提升 4.7%-21.4%，同时减少交互步骤 12.7%-21.6%。


<details>
  <summary>Details</summary>
Motivation: LLM 智能体在推理、工具使用和多步交互中表现出色，但通常从零开始解决任务，无法复用先前成功的策略或失败教训。微调虽然可以改善经验复用，但当更强或更合适的执行器出现时不够灵活。因此需要一种模型无关的经验学习框架，使冻结且可替换的 LLM 执行器能够通过外部经验重用提升性能。

Method: ExpGraph 将历史轨迹总结为可复用技能和失败教训，作为节点组织在自演化的经验图中。通过图扩散和效用感知排序检索有用经验。训练一个轻量级检索副驾驶（retrieval copilot），使用强化学习（基于有无检索经验时执行器性能对比的反馈）进行训练。图结构从下游任务结果在线更新。

Result: 在 ExpSuite 上评估，覆盖问答、数学推理、代码生成以及 ALFWorld 和 AppWorld 多步智能体环境。ExpGraph 在静态任务上分别提升 12.2%（小执行器）和 4.7%（大执行器），在智能体环境中提升 21.4% 和 12.7%，同时平均减少交互步骤 12.7% 和 21.6%。消融实验表明图结构经验、效用感知排序和自适应检索共同实现了跨任务和执行器模型的有效经验复用。

Conclusion: ExpGraph 通过模型无关的经验图框架有效解决了 LLM 智能体经验复用问题，无需更新执行器参数即可提升性能，且在不同任务和执行器上具有泛化能力。图结构经验、效用感知排序和自适应检索是成功的关键组件。

Classification reason: 该论文提出 ExpGraph，一种模型无关的经验学习框架，使 LLM 智能体能够通过外部经验图重用成功策略和失败教训，属于 LLM 智能体与生成工作流方向，因此映射到 AIGC 下的 [CANONICAL] 子主题 "AI 智能体与生成工作流"。

Importance reason: Focuses on LLM agent memory and experience reuse, which is outside the user's core priorities in multimodal generation and autoregressive video/image generation. Signals: LLM agents, experience graph, model-agnostic, retrieval copilot, ALFWorld
...
### [77] [Skill is Not One-Size-Fits-All: Model-Aware Skill Alignment for LLM Agents](https://arxiv.org/abs/2605.30723)
...
TL;DR: 本文提出MASA（Model-Aware Skill Alignment）框架，针对不同LLM骨干网络自适应调整技能库（过程性指令），通过分层技能进化流水线和轻量级模型条件技能重写器，在不修改智能体权重的情况下实现技能与模型能力的对齐，在多个交互环境和骨干网络上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体的技能库通常被视为与模型无关的，同一套技能在不同规模和能力的骨干网络上被复用。然而实验发现，技能的有效性高度依赖于模型：对一个骨干有益的技能可能对另一个有害。因此需要一种模型感知的技能对齐方法。

Method: MASA包含两个阶段：(1) 分层技能进化流水线，利用爬山算法和UCB驱动的树搜索，结合环境反馈和模型能力画像，迭代重写通用技能和任务特定技能；(2) 轻量级模型条件技能重写器，在进化轨迹上训练，能够单次前向传播复现技能适配过程。

Result: 在三个交互环境和四个骨干网络上，MASA一致取得最佳整体性能，相比最强基线提升高达25.8个点。训练好的重写器能泛化到未见过的任务和环境，无需额外搜索，以极低的推理成本持续超越更大的教师LLM。

Conclusion: 技能并非"一刀切"，模型感知的技能对齐对LLM智能体至关重要。MASA框架通过分层进化和轻量级重写器，有效实现了技能与模型能力的自适应对齐，具有良好的泛化能力和效率优势。
...
Importance reason: Focuses on LLM agent skill alignment, not directly on the user's priority subtopics like multimodal generation or video generation. Signals: LLM agents, skill alignment, model-aware, hierarchical evolution, interactive tasks
...
### [78] [COLLEAGUE.SKILL: Automated AI Skill Generation via Expert Knowledge Distillation](https://arxiv.org/abs/2605.31264)
...
TL;DR: 本文提出了COLLEAGUE.SKILL系统，一个从专家知识痕迹自动蒸馏生成可版本化、可检查、可修正、可部署的AI技能包的端到端工作流。技能包包含能力轨道（实践、思维模型、决策启发）和边界行为轨道（沟通风格、交互规则、修正历史），支持自然语言反馈更新、回滚、跨代理主机安装和受控分发。系统开源，已有约18.5k GitHub星标，215个技能由165位贡献者创建。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体的记忆和角色系统只能捕获零散的用户证据，而技能框架虽然提供了可移植的封装格式，但缺乏将异构痕迹蒸馏为可检查、可修正、可被智能体使用的技能的端到端工作流。构建具有人类专业知识、判断力和交互风格的个性化智能体仍然困难。

Method: 提出trace-to-skill蒸馏系统，将目标人物或角色的材料（异构痕迹）自动生成版本化的技能包。技能包包含两个协调轨道：能力轨道（实践、思维模型、决策启发）和边界行为轨道（沟通风格、交互规则、修正历史）。系统实现了工件契约定义、生成工作流、修正生命周期、部署接口和领域预设，支持自然语言反馈更新、回滚、跨代理主机安装和受控分发。

Result: 开源仓库获得约18.5k GitHub星标；技能画廊列出215个技能，来自165位贡献者，累计超过10万星标。系统展示了如何将人物扎根的技能表示为可移植、可修正的包，而非不透明的提示或隐藏的记忆。

Conclusion: 本文证明了人物扎根的AI技能可以通过自动蒸馏工作流被表示为可移植、可检查、可修正的包，为构建具有人类专业知识和交互风格的LLM智能体提供了系统化的解决方案。

Classification reason: 该论文提出了一套从专家知识痕迹自动蒸馏为可检查、可修正、可部署的AI技能包的端到端工作流，属于LLM智能体技能生成与自动化工作流范畴，与[AIGC]方向下的[AI智能体与生成工作流]规范子主题高度匹配。

Importance reason: Focuses on AI agent skill generation, not on the user's priority subtopics like continuous VLM, video generation, or multimodal models. Signals: AI agent skill generation, expert knowledge distillation, not multimodal generation, not video/image generation, not efficient inference
...
### [79] [Learning to Adapt: Self-Improving Web Agent via Cognitive-Aware Exploration](https://arxiv.org/abs/2605.31365)
...
TL;DR: 本文提出 SCALE 框架，通过 Selector、Predictor、Judger 三个对抗性角色和图探索策略 SCALE-Hop，使 MLLM 驱动的 Web 智能体能够自主发现自身局限、探索环境并自我改进，无需人工专家轨迹或手工流水线。


<details>
  <summary>Details</summary>
Motivation: 现有 Web 智能体依赖手工设计的执行流水线或昂贵的专家轨迹，难以适应复杂动态环境，缺乏自主探索和自我改进的能力。

Method: 提出 SCALE 框架，包含三个对抗性角色：Selector（选择待探索的子任务）、Predictor（预测执行结果）、Judger（评估预测与实际结果的差异以发现局限）；提出 SCALE-Hop 图探索策略实现全局规划并避免局部探索陷阱；构建 SCALE-20k 数据集（来自 19 个真实网站的多样化任务与结构化演示）。

Result: 实验表明该方法显著提升了多种 MLLM 在多个 Web 环境中的性能和泛化能力，证明了框架的可扩展性和通用性。

Conclusion: SCALE 提供了一种可扩展且通用的解决方案，用于构建真正自主和自适应的 Web 智能体，通过认知感知探索实现自我改进。

Classification reason: 该论文提出 SCALE 框架，通过三个对抗性角色（Selector, Predictor, Judger）和图探索策略 SCALE-Hop 实现 Web 智能体的自主探索与自我改进，属于 LLM 智能体与自动化工作流范畴，因此映射到 [CANONICAL] aigc_agent_workflow。

Importance reason: Focuses on web agent self-improvement, not on user's priority subtopics like continuous VLM, video/image generation, or model efficiency. Signals: web agent, self-improving, cognitive-aware exploration, MLLM, graph exploration
...
### [80] [Eywa: Provenance-Grounded Long-Term Memory for AI Agents](https://arxiv.org/abs/2605.30771)
*Resham Joshi*
...
TL;DR: Eywa 是一种基于溯源（provenance）的 AI 智能体长期记忆架构，通过"先证据后信念"的设计，将不可变源证据、规范事实、检索上下文与回答策略分离，实现可审计、可诊断的记忆系统。在 LoCoMo、LongMemEval-S 和 BEAM 基准上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 智能体记忆系统将源证据、提取事实、检索上下文和回答策略混入一个不透明的提示路径中，导致错误难以诊断——错误答案可能来自缺失证据、不支持的提取、过时状态、检索损失或回答模型行为。需要一种可审计、可诊断、可追溯的长期记忆架构。

Method: Eywa 采用"证据先于信念"（evidence before belief）的设计原则：1）先存储不可变的源证据，再从中推导规范事实；2）通过类型化信号和源支持验证提取的记忆；3）通过确定性多路径读取路径检索有界记忆上下文，检索过程中零 LLM 调用；4）检索到的上下文与回答指令分离返回，允许同一记忆基座在不同模型（前沿/预算/本地）上评估。
...
Conclusion: Eywa 证明了基于溯源的记忆架构能够显著提升 AI 智能体的长期记忆可靠性、可审计性和可诊断性，为构建跨会话持久化智能体提供了系统化的记忆管理方案。

Classification reason: 该论文提出 Eywa，一种面向 AI 智能体的、基于溯源（provenance）的长期记忆架构，核心关注智能体的记忆检索、审计、更新与擦除能力，属于 LLM 智能体与生成工作流范畴，因此映射到 [CANONICAL] 的 aigc_agent_workflow（AI 智能体与生成工作流）。

Importance reason: Focuses on AI agent memory provenance, not on the user's priority subtopics like continuous VLM, autoregressive video generation, or world models. Signals: AI agent memory, provenance-grounded, long-term memory, evidence before belief, LoCoMo benchmark
...
### [81] [AutoSci: A Memory-Centric Agentic System for the Full Scientific Research Lifecycle](https://arxiv.org/abs/2605.31468)
...
TL;DR: AutoSci 是一个以记忆为中心的智能体系统，旨在自动化科学研究的完整生命周期。它包含四个核心模块：SciMem（结构化科研记忆）、SciFlow（五阶段科研流程执行）、SciDAG（DAG 形状的多智能体算子与模板）和 SciEvolve（基于反馈的自我进化机制），使系统能够在多个研究项目中执行、记忆并持续改进。


<details>
  <summary>Details</summary>
Motivation: 传统科学研究高度依赖人力，研究人员需要在漫长的项目周期中协调文献、想法、实验、手稿和审稿回复。现有 LLM 科学智能体系统要么部分满足、要么完全不满足全生命周期支持、结构化持久记忆和流程自我改进这三个关键需求，因此需要一个统一的自动化科研系统。

Method: AutoSci 由四个模块组成：(1) SciMem 提供模式驱动的科研记忆，分为长期知识记忆和活跃研究记忆；(2) SciFlow 通过一个控制状态、上下文、验证、反馈和编排的 harness 执行五阶段科研生命周期（文献理解→头脑风暴→实验→写作→回复）；(3) SciDAG 使用 DAG 形状的多智能体算子和可复用的阶段特定模板增强困难技能；(4) SciEvolve 将来自用户、实验、审稿和外部环境的反馈信号转化为对 SciMem 组织、SciFlow 技能和 SciDAG 模板的版本化更新。

Result: 论文展示了 AutoSci 的模块化架构设计，并提供了开源代码仓库。系统能够支持从文献调研到审稿回复的完整科研流程，具备跨项目的持久记忆能力和基于反馈的自我进化能力。

Conclusion: AutoSci 通过记忆中心化的智能体系统设计，实现了对完整科研生命周期的自动化支持，其记忆分离、流程编排、多智能体协作和自进化机制为构建持久化的科研 AI 系统提供了有效框架。

Classification reason: 该论文提出 AutoSci，一个基于 LLM 的科研智能体系统，覆盖文献理解、实验、写作、审稿回复等完整科研工作流，属于 AI 智能体与自动化工作流方向，与 [CANONICAL] subtopic aigc_agent_workflow 高度匹配。

Importance reason: Focuses on LLM-based scientific agent workflow, not on core multimodal generation or autoregressive video/image generation priorities. Signals: AI agent for scientific research, memory-centric system, full research lifecycle automation, no direct connection to user's priority subtopics
...
TL;DR: 本文提出 SPIRE，一个基于多智能体框架的人文学科研究引擎，通过将人文学科的核心学术操作（来源发现、证据标注、比较、溯源检查、引用绑定、论证综合等）建模为协作智能体角色，结合多尺度细读机制（段落、上下文图社区、跨上下文语义聚类），在古典中文和希腊-罗马拉丁文学基准上实现了比 Naive LLM、Text RAG 和 GraphRAG 更可靠的原始来源证据检索和更高质量的论证生成。


<details>
  <summary>Details</summary>
Motivation: 现有基于 LLM 的研究智能体主要面向科学和工程领域，优化于可执行实验、代码和定量信号，而人文学科研究需要基于原始来源的阐释性、证据支撑的论证推理，强调忠实引用、可验证溯源和细读。现有研究智能体缺乏这种证据支撑的阐释性推理能力。

Method: SPIRE 框架基于学术原始操作理论，将人文学科常见操作建模为七个协作智能体角色：来源发现、证据标注、比较、溯源检查、采样、引用绑定和论证综合。同时构建多尺度细读基础：段落级、上下文图社区级和跨上下文语义聚类级，支持从细粒度到全局的证据检索与综合。

Result: 在同行评审论文基准（古典中文和希腊-罗马拉丁文学）上，SPIRE 在原始来源证据检索可靠性上优于 Naive LLM、Text RAG 和 GraphRAG，并在答案准确性、深度、覆盖范围和证据质量方面获得更高的盲审评分。消融实验表明学术操作智能体和细读检索机制对证据支撑论文生成均有贡献。

Conclusion: SPIRE 框架有效弥合了 LLM 研究智能体在人文学科领域的应用鸿沟，通过将学术原始操作建模为多智能体协作并结合多尺度细读机制，实现了更可靠的证据支撑学术写作。该工作为将 AI 研究能力扩展到人文学科提供了可行路径。

Classification reason: 该论文提出 SPIRE 多智能体框架，将人文学科研究操作（来源发现、证据标注、比较、溯源检查等）建模为协作智能体角色，属于 LLM 智能体与生成工作流方向，与 canonical 子主题 aigc_agent_workflow 高度匹配。

Importance reason: This paper focuses on humanities research agents, which is outside the user's core interests in multimodal generation, video/image generation, and model efficiency. Signals: humanities scholarship, multi-agent framework, evidence-grounded reasoning, classical Chinese and Greco-Roman Latin, not multimodal generation or video/image generation
...
Main category: cs.CR, cs.CL, cs.IR, cs.AI
...
TL;DR: 本文提出了一种面向受监管网络安全运营的组织级 LLM 智能体运行时架构，通过类型化的安全上下文、运行时核心、逻辑子智能体、受控工具适配层、结构化发现、分层人机交互门控和仅追加审计等组件，实现检索、工具调用、内存、发现、报告和审计的全流程组织范围管控，并保持模型无关和本地可部署性。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 智能体系统在孤立的网络安全任务上表现良好，但缺乏一个可审计的平台架构来支持受监管的安全运营中心（SOC）和合规工作流，其中单个分析师的操作可能绑定整个组织，且运行时需要与现有 SIEM/XDR 堆栈集成作为上下文和告警驱动触发的主要来源。

Method: 提出组织级 LLM 智能体运行时架构，核心组件包括：(1) 类型化安全上下文，在每个入口点创建并在所有组件边界强制执行；(2) 共享运行时核心与逻辑子智能体；(3) 受控工具适配层，在统一策略和审计下暴露 SIEM/XDR 查询、增强和响应原语；(4) 结构化发现与证据引用；(5) 分层人机交互门控；(6) 仅追加审计。MCP、扩展遥测、数字孪生、图检索和联邦知识共享作为可选扩展路径。

Result: 论文提出了一个可实现的切片作为架构的可测试性表面，并提出了一个可证伪的评估计划，包含架构就绪性、安全策略执行、证据可追溯性、输出质量和运营可观测性等维度的指标级通过标准。

Conclusion: 该架构为受监管的网络安全运营提供了首个组织级 LLM 智能体运行时设计，填补了现有 LLM 智能体系统在可审计平台架构方面的空白，为金融网络安全等受监管领域的实际部署奠定了基础。

Classification reason: 该论文提出了一种面向受监管网络安全运营的组织级 LLM 智能体运行时架构，涉及智能体工作流、工具调用、审计与 HITL 门控，与 [CANONICAL] subtopic "AI 智能体与生成工作流" 高度匹配。

Importance reason: Focuses on regulated cybersecurity operations, which is outside the user's priority subtopics and contains negative keywords. Signals: Cybersecurity operations, regulated workflows, LLM agent runtime, SOC compliance, model-agnostic architecture
...
TL;DR: SCOPE是一个无需外部数据的自我对弈框架，通过协同进化Challenger（生成文档基础任务）和Solver（多轮检索回答）两个策略，并利用冻结的初始模型作为自评判器，在开放任务上提升LLM性能。在8个基准上最高提升+10.4分，在7个保留的短问答基准上最高提升+13.8分。


<details>
  <summary>Details</summary>
Motivation: 现有自我对弈方法需要规则可验证的答案（如数学、代码），无法处理开放任务（如问答、推理）。开放任务依赖人工策划的提示词或前沿模型作为评判器，限制了自我对弈的通用性和可扩展性。

Method: SCOPE框架包含三个组件：(1) Challenger策略——从文档中生成开放任务；(2) Solver策略——通过多轮检索回答任务；(3) Self-Judge——冻结的初始模型副本，根据源文档编写任务特定评分标准并评分。Challenger和Solver通过自我对弈协同进化，Challenger不断生成接近Solver能力边界的任务以推动进步。

Result: 在Qwen2.5、Qwen3、OLMo-3三个7-8B模型上，SCOPE在8个开放任务基准上最高提升+10.4分，匹配或超越使用约9K策划提示词的GRPO_data。在7个保留的短问答基准上最高提升+13.8分，全面超越GRPO_data。消融实验表明协同进化Challenger至关重要，检索和合成能力的贡献因任务而异，评分标准生成质量是自评判的瓶颈。

Conclusion: SCOPE证明了无需外部监督即可通过自我对弈提升开放任务性能，且能泛化到短问答任务。协同进化策略和基于文档的评分标准生成是核心创新点，但评分标准质量仍是主要瓶颈，未来可探索更强大的自评判机制。
...
Importance reason: Focuses on LLM self-play for open-ended tasks, not directly on the user's priority subtopics like continuous VLM, video generation, or multimodal models. Signals: Self-play framework, Co-evolving policies, Open-ended tasks, No external data, Improves LLM performance
...
TL;DR: 本文提出Sheaf-ADMM，一种可微优化框架，将输入分解为重叠局部视图，每个智能体通过神经编码器参数化的凸子问题处理，利用ADMM和细胞层指定智能体间约束进行协调。通过展开优化反向传播联合训练整个多智能体系统。在迷宫寻路、图像分类和数独任务上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有消息传递架构（如MPNN）在多智能体协调中缺乏可解释性和可干预性，且难以处理异构的全局一致性约束。作者希望提出一种结构化的可微优化框架，使智能体能够通过显式的约束机制进行协调，同时保持端到端可训练性。

Method: 将输入分解为重叠局部视图，每个视图由独立智能体处理，智能体求解由神经编码器参数化的凸子问题。智能体间通过ADMM进行协调，约束由细胞层（cellular sheaf）指定，定义相邻解必须在哪些方面达成一致。通过展开ADMM迭代并反向传播，联合训练所有组件。

Result: 在迷宫寻路、MNIST图像分类和数独任务上验证：1）局部视图分解的智能体通过协调能产生正确的全局输出；2）在MNIST上相比标准CNN对分布偏移具有更好的鲁棒性；3）在数独上相比参数匹配的MPNN基线显著提高了求解率；4）ADMM结构暴露了原始、共识和对偶状态变量，可直接分析和干预协调动态。

Conclusion: Sheaf-ADMM提供了一种结构化的多智能体协调框架，兼具可微训练性、可解释性和可干预性，在多个任务上优于传统消息传递方法，为多智能体系统的设计提供了新范式。

Classification reason: 该论文提出了一种基于可微优化框架的多智能体协调方法，使用ADMM和细胞层（cellular sheaf）实现智能体间的约束协调。这属于AI智能体与生成工作流（aigc_agent_workflow）范畴，因为其核心是多智能体系统的协调与自动化工作流，且该方向已有116篇论文积累，是最佳匹配。

Importance reason: The paper focuses on multi-agent coordination via ADMM, which is not aligned with the user's priority subtopics in multimodal generation, video/image generation, or efficient inference. Signals: Multi-agent coordination, ADMM optimization, cellular sheaf, not multimodal generation, not autoregressive video/image
...
TL;DR: 本文研究了多智能体LLM架构在预测市场结果裁决中的应用，比较了独立聚合与协商共识两种策略，发现置信度加权投票的独立聚合达到83.43%准确率，优于单模型基线，而协商共识因错误传播反而降低性能。提出了混合AI-人类裁决系统的路由准则。


<details>
  <summary>Details</summary>
Motivation: 预测市场依赖可靠的结果裁决机制，现有方案在快速自动化与准确人工仲裁之间存在权衡。单LLM裁决器虽有一定准确性，但缺乏自我纠错机制。本文探索多智能体LLM架构能否提升裁决准确性。

Method: 在1,189个已解决的预测市场问题上，比较独立聚合（置信度加权投票）与协商共识两种多智能体策略，以GPT-5 Nano、DeepSeek V3和Llama-3.3-70B为基座模型，所有智能体共享Exa证据层，检索按发表日期过滤以隔离推理与检索质量。

Result: 独立聚合置信度加权投票达到83.43%准确率，比最佳单模型高1.01个百分点；协商共识准确率降至约76%，低于所有单模型基线。模型间错误相关性（0.529-0.689）限制了集成增益。混合方案中，仅自动裁决一致高置信度问题可在47%数据上达到97.87%准确率。

Conclusion: 多智能体独立聚合可适度提升预测市场裁决准确性，但协商共识因错误传播而失效。模型间错误相关性是集成方法的天花板。建议采用混合AI-人类裁决系统，由智能体间分歧触发人工审查。

Classification reason: 该论文研究多智能体LLM架构用于预测市场结果裁决，属于LLM智能体与自动化工作流范畴，与AIGC方向下的[AI智能体与生成工作流]规范子主题高度匹配。

Importance reason: Focuses on multi-agent LLM oracles for prediction markets, which is outside the user's core interests in multimodal generation and autoregressive video/image generation. Signals: Multi-agent LLM oracle, prediction market resolution, independent aggregation vs. deliberative consensus, confidence-weighted voting, hybrid AI-human routing
...
TL;DR: 本文提出idSCD方法，利用模型学习到的语义相关性结构（SCD）进行数据集级别的成员推断，通过白盒指纹识别训练数据集的归属，在多个任务上优于现有黑盒和白盒基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据集成员推断方法依赖置信度分数、损失值、生成样本等行为或分布证据，忽略了数据集在模型训练过程中留下的独特语义相关性痕迹。作者希望利用数据集特有的虚假相关性（spurious correlations）作为指纹来识别训练数据集归属。

Method: 提出语义相关性描述符（SCD），捕获模型学习到的语义相关性结构。通过白盒方式提取模型的SCD，并与目标数据集的独立SCD进行比较，设计基于SCD的成员分数来判断目标数据集是否属于模型的训练混合数据。在留一数据集诊断中验证SCD能完美分离匹配与非匹配数据集对。

Result: 在自然语言推理、情感分类和医学文本分类三个实验设置中，基于SCD的分类器在平均性能和标准差上均优于RMIA、Attack-P、LiRA等黑盒基线以及SIF白盒基线，最大相对增益在ROC-AUC上超过60%。

Conclusion: 数据集可以通过模型内部学习的语义相关性结构进行追溯，idSCD方法在数据集成员推断任务上展现出显著优势，尤其当数据集组具有独特的语义特征时效果最佳。

Classification reason: 该论文研究通过语义相关性描述符（SCD）识别训练数据集归属，属于AIGC检测与溯源方向（aigc_detection_attribution），该子主题涵盖生成模型溯源与归因，与本论文的dataset membership inference任务高度吻合。

Importance reason: Focuses on dataset membership inference, not on the user's priority generative or multimodal topics. Signals: Dataset membership inference, Semantic correlation descriptors, White-box fingerprinting, AIGC detection, Not generative modeling
...
TL;DR: 本文提出BIAS-ID框架，用于系统分析和量化AI生成图像检测器中的变换偏差（transformation biases），揭示现有检测器在真实场景下因训练数据偏差而依赖虚假相关性而非真实取证痕迹的问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测器在受控设置下表现良好，但在真实数据上性能骤降。潜在原因是训练数据中存在细微偏差，导致检测器依赖虚假相关性而非真正的取证痕迹。目前缺乏评估检测器偏差程度的标准化协议。

Method: 首先从理论上讨论检测器偏差的含义及其与鲁棒性不足的区别；然后提出BIAS-ID框架，通过系统性地分析检测器在不同图像变换下的行为来量化和识别变换偏差；最后在六个检测器和两个数据集上进行验证评估。

Result: 实验表明多个最先进的AI生成图像检测方法受到严重偏差影响，验证了BIAS-ID框架在揭示检测器偏差方面的有效性。

Conclusion: 偏差感知评估对于开发可靠的AI生成图像检测器至关重要，BIAS-ID为标准化偏差分析提供了可行的框架。

Classification reason: 论文研究AI生成图像检测器的变换偏差分析框架，属于AIGC检测方向，与[CANONICAL] subtopic "AIGC 检测与溯源"（aigc_detection_attribution）高度匹配。

Importance reason: Focuses on AI image detector bias analysis, not aligned with user's priority on generation models and multimodal understanding. Signals: AIGC detection, transformation bias, forensic artifacts, detector robustness, spurious correlations
...
TL;DR: 本文提出利用语言训练的正则化能力来提升深度伪造检测器的泛化性和可解释性。采用双编码器架构（冻结的专家检测器 + LoRA微调的MLLM编码器），通过两阶段训练：先进行二值对齐缓解过拟合，再通过强化学习阶段鼓励模型先描述推理再分类，从而优先利用高层级可描述特征。实验在多个基准数据集上显著超越现有方法。
...
Method: 采用双编码器架构：一个冻结的专家检测器（如LSDA）和一个LoRA微调的MLLM编码器（如LLaVA）。两阶段训练：第一阶段进行二值对齐训练，让MLLM学习组合特征以缓解过拟合；第二阶段使用强化学习（基于GRPO），鼓励模型在分类前先生成描述性推理（"先解释再分类"），通过奖励机制优先利用高层级鲁棒特征。

Result: 在多个跨数据集基准测试上，该方法显著优于现有最先进方法，同时生成可解释的描述性推理。即使推理时省略推理链，模型仍能保持性能提升，表明语言训练确实起到了正则化作用。

Conclusion: 语言训练的正则化能力可以有效提升深度伪造检测器的泛化性和可解释性。通过强化学习鼓励"先解释再分类"的行为，模型学会了优先利用高层级可描述特征，从而在跨数据集场景下取得更好性能。该方法为深度伪造检测提供了一种新的联合优化范式。

Classification reason: 该论文研究深度伪造检测（deepfake detection），属于 AIGC 检测与溯源这一 [CANONICAL] 子主题的核心范畴。同时涉及 MLLM 用于检测，因此也匹配多模态理解与生成方向。

Importance reason: Focuses on deepfake detection, not aligned with user's priority subtopics in generation or multimodal understanding. Signals: deepfake detection, language regularization, MLLM, reinforcement learning, generalization
...
TL;DR: 本文提出一种基于跨相机双合成参考的 CDP 认证框架，通过注册相机捕获和深度学习翻译器，联合数字模板与注册图像生成高质量参考，提升印刷防伪图案在不同移动相机下的认证鲁棒性。


<details>
  <summary>Details</summary>
Motivation: Copy Detection Patterns (CDP) 用于物理物体防伪，但实际中打印机随机性和相机畸变导致数字模板与捕获图像难以直接比对，限制了认证鲁棒性。现有方法仅考虑相机域合成参考，忽略了打印变异性。

Method: 提出基于注册的跨相机双合成参考框架：先用注册相机捕获每个印刷 CDP，再通过深度学习翻译器联合利用数字模板和注册捕获图像，为验证相机生成高质量参考图像。从信息论角度证明双参考比仅基于模板的参考包含更多信息。

Result: 在异构移动相机上的实验表明，该方法提升了认证性能，对基于机器学习的复制攻击具有鲁棒性，并能从小尺寸 CDP 区域和低端设备上实现可靠验证。

Conclusion: 跨相机双合成参考框架有效解决了打印变异性与相机畸变问题，提升了 CDP 认证的实用性和安全性，尤其适用于资源受限的移动设备场景。

Classification reason: 该论文研究 Copy Detection Patterns (CDP) 的认证方法，属于生成内容（印刷防伪图案）的检测与真伪鉴别，与 AIGC 检测与溯源（aigc_detection_attribution）方向高度吻合，因此复用该规范子主题。

Importance reason: Focuses on AIGC detection and physical authentication, which is not a priority subtopic and lacks alignment with user's key interests in generation and multimodal models. Signals: AIGC detection, cross-camera authentication, copy detection patterns, low personal relevance to generation or multimodal reasoning
...
Main category: cs.HC, cs.CL, cs.AI
...
TL;DR: 本文提出Social AI Design Code框架和EUDAIMONIA基准，用于评估LLM在社交交互中是否鼓励有害亲密、依赖或过度参与等不良动态。对22个最新LLM的评估表明，即使最强模型也有27-31%的违规率，且扩展推理无法缓解该问题。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地被用作社交伴侣，提供陪伴、情感倾诉和人际建议，但现有能力导向或传统安全评估无法捕捉社交交互中的不良动态（如鼓励有害亲密、依赖或过度参与），需要专门的评估框架和基准。

Method: 提出Social AI Design Code框架，定义社交交互中与用户福祉对齐的设计要求。基于WildChat数据集，通过弱到强过滤、多模型重标注和受控改写构建EUDAIMONIA基准，包含969个用户输入和3,147个设计要求违规检查项。

Result: 评估22个最新LLM，最强模型Claude-Opus-4.7和GPT-5.5分别违规30.7%和27.2%的检查项。扩展推理（extended thinking）不能降低违规率，表明这些失败是持久的社会对齐问题，而非可通过测试时推理解决的缺陷。

Conclusion: 当前LLM在社交交互中存在显著的社会对齐问题，即使最强模型也有较高违规率，且扩展推理无法缓解。需要专门针对社交交互福祉的设计和评估方法。

Classification reason: 该论文提出了一个评估LLM在社交交互中不良动态的基准（EUDAIMONIA），属于AIGC方向下的生成内容质量评测与安全评估范畴，与[CANONICAL] subtopic "AIGC 评测与基准"（aigc_evaluation_benchmark）高度匹配。

Importance reason: Focuses on social interaction safety evaluation, not aligned with user's priority subtopics in multimodal generation or efficient inference. Signals: LLM social safety, benchmark, not multimodal, not generation, not efficiency
...
Main category: cs.HC, cs.CL, cs.CY, cs.AI
...
TL;DR: 基于大规模匿名聊天机器人交互数据，实证分析了全球早期用户使用生成式AI的差异，发现低收入国家更多用于教育，高收入国家更多用于休闲，且语言因素显著影响使用模式。


<details>
  <summary>Details</summary>
Motivation: 虽然生成式AI已被全球广泛使用，但不同国家用户的使用方式差异尚缺乏大规模实证研究。理解这些差异对于评估AI是否扩大或缩小数字鸿沟至关重要。

Method: 使用一个广泛可用的免费AI聊天机器人的大规模匿名化交互数据集，按国家收入水平和语言对用户使用模式进行统计分析，考察教育、休闲等不同使用领域的分布差异及其与GDP、语言的关系。

Result: 教育是大多数国家（尤其是低收入国家）最常见的AI使用领域，且与GDP呈强负相关；休闲相关使用与GDP正相关；英语交互在非英语主导语言国家中被过度使用，表明现有模型的语言覆盖存在不足。

Conclusion: 生成式AI的使用模式因国家收入和语言而异，改善多语言性能可能是决定该技术能否缩小数字鸿沟或实现跨越式发展的关键因素。

Classification reason: 该论文研究全球不同国家用户对生成式AI聊天机器人的使用模式差异，属于AIGC应用层面的实证分析与评测，与[AIGC 评测与基准]子主题相关（关注AIGC使用行为、跨文化差异与数字鸿沟评估）。

Importance reason: This paper is a social science analysis of AIGC usage patterns, not a technical advance in the user's priority subtopics. Signals: AIGC usage analysis, cross-country study, language bias, education vs leisure, no technical contribution to generation models
...
Result: 该协议能够系统评估 ChatGPT 在生物医学关联生成中的表现，通过 RAG 增强的跨模型多数投票机制有效识别幻觉，并提供了语义验证的用例。

Conclusion: 该协议为评估 LLM 在生物医学知识生成中的可靠性提供了可复用的框架，RAG 增强的跨模型验证方法有助于提升生成内容的可信度并暴露幻觉。

Classification reason: 该论文提出评估 ChatGPT 生成生物医学关联能力的协议，属于 AIGC 生成内容的质量评测范畴，与 [CANONICAL] 子主题 "AIGC 评测与基准" 高度匹配。

Importance reason: Focuses on biomedical association evaluation, which is outside all priority subtopics and uses negative keywords. Signals: biomedical, RAG, ChatGPT evaluation, ontology validation, hallucination detection
...
### [94] [Auditing LLM Benchmarks with Item Response Theory](https://arxiv.org/abs/2605.30504)
...
TL;DR: 本文提出基于项目反应理论（IRT）的指标，用于检测LLM基准测试中的错误标签。在7个偏好和多选题基准测试中，使用114个模型的响应，该方法在前200个示例中达到95%的精确率，优于有监督分类器。研究发现错误标签源于机械标注启发式、上游数据集继承的错误以及本质模糊的题目。模型拟合还揭示了奖励模型在风格偏好而非事实知识上的专业化，并发现一个前沿奖励模型与检测到的错误标签有78%的一致性（其他模型仅38%），暗示基准污染或过度优化。


<details>
  <summary>Details</summary>
Motivation: LLM基准测试的标签在发布时被冻结，错误会无声地传播到下游基准中。现有方法缺乏系统性地发现和追溯这些标签错误的手段，导致基准评估的可信度受损。

Method: 提出基于项目反应理论（IRT）的统计指标，利用114个模型在7个偏好和多选题基准上的响应分布，自动识别可能错误标注的样本。通过模型拟合分析奖励模型的能力分布，并追溯错误标签的来源。

Result: 该方法在前200个示例中达到95%的精确率，优于有监督分类器。发现错误标签来自机械标注启发式、上游数据集继承的错误以及本质模糊的题目。一个前沿奖励模型与检测到的错误标签有78%的一致性（其他模型仅38%），暗示基准污染或过度优化。

Conclusion: IRT方法能有效审计LLM基准中的标签错误，揭示基准污染和奖励模型过度优化问题，为提升基准可信度提供了可扩展的统计工具。

Classification reason: 该论文使用项目反应理论（IRT）审计LLM基准测试中的标签错误，属于对生成模型评测基准的质量评估与审计，与[AIGC 评测与基准]（aigc_evaluation_benchmark）高度匹配。

Importance reason: Focuses on benchmark auditing methodology, not on the user's priority generative modeling or multimodal topics. Signals: Item Response Theory for mislabel detection, benchmark contamination detection, reward model specialization analysis, not aligned with user's priority subtopics
...
### [95] [EHRBench: An Automated and Reliable EHR-based Benchmark for Clinical Decision Making with LLMs](https://arxiv.org/abs/2605.30637)
...
TL;DR: EHRBench 是一个基于真实电子健康记录（EHR）自动构建的临床决策基准，通过 EHR-LLM-知识库交互流水线生成近百万个 QA 项，覆盖诊断、治疗和预后三类任务，系统评估了 30 多个 LLM 的临床决策能力。


<details>
  <summary>Details</summary>
Motivation: 现有临床决策基准缺乏规模与质量兼顾的自动化构建方法，且大多未基于真实患者 EHR 数据，难以评估 LLM 在实际临床场景中所需的生物医学知识和临床推理能力。

Method: 提出 EHR-LLM-KB 交互流水线：先用专用 LLM 自动将 EHR 轨迹转换为结构化模板，再确定性实例化为 QA 项；同时利用知识库进行系统性验证和丰富，过滤幻觉或模糊关系，提升可靠性。最终构建了约 96 万个 QA 项，覆盖诊断、治疗和预后三类推理型临床决策任务。

Result: 在 30 多个代表性 LLM 上进行了基准测试，展示了不同设置下一致的能力趋势，验证了 EHRBench 的可靠性，并揭示了 LLM 在临床可靠性方面的可改进差距。

Conclusion: EHRBench 提供了一个自动化、可靠且大规模的临床决策评测基准，能够有效评估 LLM 在真实临床决策任务上的表现，为构建临床可靠的 LLM 系统提供了重要参考。

Classification reason: 该论文构建了基于电子健康记录（EHR）的临床决策基准 EHRBench，用于评估 LLM 在诊断、治疗和预后等临床决策任务上的表现，属于 AIGC 方向下的评测与基准（aigc_evaluation_benchmark）子主题。

Importance reason: Focuses on clinical LLM benchmark, which is outside user's core interests in multimodal generation and efficient models. Signals: EHR-based benchmark, clinical decision making, LLM evaluation, automated pipeline, low personal direction match
...
TL;DR: 本文提出有界行为不可区分性（(ε,q,t,A)-behavioral indistinguishability）框架，用于评估黑盒LLM蒸馏的效果。通过在Qwen和Llama系列模型上的实验表明，LoRA蒸馏虽然提升了语义相似度，但对抗性评估仍能发现学生与教师之间的行为差异，说明语义保真度不足以衡量蒸馏质量，需要结合有界、对抗性和类别感知的评估方法。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒LLM蒸馏评估仅关注输出匹配（语义相似性或任务一致性），但输出相似并不等同于行为不可区分。需要一种更严格的评估框架来量化蒸馏模型是否真正模仿了教师模型的行为。

Method: 形式化定义了(ε,q,t,A)-行为不可区分性，其中ε为区分优势上界，q为查询次数上界，t为计算资源上界，A为对抗器类别。在Qwen和Llama教师-学生对上，使用5000提示的行为探测套件进行实验，比较基础学生与LoRA蒸馏学生，通过对抗性判别器、成对教师识别、查询预算实验等方法评估行为不可区分性。

Result: LoRA将Qwen的语义相似度从0.788提升至0.862，Llama从0.814提升至0.874。但对抗性评估显示行为差异仍然存在：学习判别器保持非零优势，风格/格式、鲁棒性和领域技术提示中存在伪影。跨族Llama评判器下，Qwen区分优势从0.158降至0.081。查询预算实验表明，分歧引导采样并不始终优于分层随机采样。

Conclusion: 语义保真度是有用但不足的蒸馏评估指标。黑盒LLM蒸馏需要结合有界、对抗性和类别感知的评估方法，行为不可区分性应成为蒸馏质量评估的重要维度。

Classification reason: 该论文研究黑盒LLM蒸馏的行为不可区分性评估，提出有界行为不可区分性形式化框架，属于AIGC评测范畴（评估蒸馏模型是否真正模仿了教师模型的行为），因此映射到[AIGC评测与基准]这一规范子主题。

Importance reason: Focuses on LLM distillation evaluation, not on the user's priority subtopics like continuous VLM, video generation, or world models. Signals: LLM distillation evaluation, behavioral indistinguishability, adversarial evaluation, not aligned with user's priority subtopics, no boost keywords matched
...
### [97] [The Architecture of Errors: From Universal Impossibility to Patch-Local LLM Reliability](https://arxiv.org/abs/2605.30628)
...
Importance: 值得关注 (70.39) - 未入选精读

TL;DR: 本文提出通用LLM可靠性不是有限库问题：在无界域中，新的可区分失败模式可以无限出现，因此有限干预字典无法保证有界残差错误。但在操作有界的局部补丁（如法律审查、医疗RAG、代码修复等）中，失败模式稀疏且重复，可靠性转化为局部目录发现与干预覆盖问题。论文通过两个命题和一个推论形式化这一转变，证明在局部补丁中干预预算随序列长度呈多对数增长，并在目录饱和后趋于常数。


<details>
  <summary>Details</summary>
Motivation: 现有LLM可靠性方法试图通过有限干预字典覆盖所有可能的失败模式，但理论上无法保证在无界域中覆盖所有新出现的失败模式。作者希望从理论层面阐明这一局限，并指出在实际部署场景中，由于操作域有界，可靠性问题可以转化为更易处理的局部目录覆盖问题。

Method: 通过形式化理论分析，提出两个命题和一个推论：命题1（最坏情况负结果）——无界域中有限干预字典无法覆盖每个可区分的失败模式；推论1（逆发现蕴含）——对数上界的模式发现无法容纳线性更多的尾模式而不需要指数级更多的观测硬失败事件；命题2（正局部结果）——在log活跃模式暴露和头部重覆盖下，每次硬决策的干预预算随序列长度呈多对数增长，并在目录饱和后变为域常数。

Result: 理论证明了通用LLM可靠性在无界域中的不可能性，以及在操作有界补丁中可靠性问题的可解性。框架将长上下文困难重新定位而非消解：当硬决策数量随任务长度增长时，可靠性仍然困难；贡献在于识别了轴上的干预而非使这些区域变得容易。

Conclusion: LLM可靠性应被视为局部目录发现与干预覆盖问题，而非全局有限库问题。在实际部署中，通过识别和覆盖操作补丁内的重复失败模式，可以以可控的干预预算实现可靠性的显著提升。该框架为构建可靠LLM系统提供了理论基础。

Classification reason: 该论文从理论层面论证了LLM可靠性问题的本质，提出在操作有界补丁（如法律审查、医疗RAG等）内，失败模式是稀疏且可重复的，从而将可靠性转化为局部目录发现与干预覆盖问题。这属于对AIGC系统（尤其是LLM生成内容）的可靠性评测与理论分析，与AIGC评测与基准方向高度相关。

Importance reason: The paper's theoretical framework on LLM reliability patches is not aligned with the user's priority on generative models and multimodal systems. Signals: LLM reliability theory, patch-local failure analysis, no generative model focus, no multimodal content, no video/image generation
...
TL;DR: 本文提出一种基于人格（persona）的多元对齐评估框架，用合成认知画像替代单一评估函数，实现反映文化、人口和语境差异的多元评测。同时发现这些模拟评估者在连续推理下会出现状态空间漂移和语义不一致，论证了需要动态、适应性调控机制来维持连贯的认知模拟。


<details>
  <summary>Details</summary>
Motivation: 当前生成式AI的对齐评估范式依赖单一的基准测试框架，将人类判断的多元性简化为聚合统计基线，忽视了文化、人口统计和语境差异。作者希望引入一种能反映人类观点多样性的评估方法。

Method: 提出状态空间约束的仿真评估框架，用结构化流形上的合成认知画像（persona）替代单一评估函数。利用现代生成架构实例化并维持这些评估性人格，实现多元、视角依赖的基准测试。同时通过序列推理和随机提示扰动分析这些模拟评估者的稳定性。

Result: 现代生成架构能以较高一致性实例化和维持评估性人格；但在序列推理和随机提示扰动下，人格连贯性出现系统性退化，表现为状态空间漂移和语义不一致。

Conclusion: 静态对齐约束不足以维持长期稳健的评估行为，需要嵌入动态、可行性驱动的调控机制来保持连贯的认知模拟。将基于人格的评估建模为潜在表征流形上的结构化动力系统，为更自适应、更符合人类对齐的AI评估方法奠定基础。

Classification reason: 该论文提出基于人格（persona）的多元对齐评估框架，核心贡献在于生成式AI的评测方法论，属于AIGC评测与基准方向。论文讨论的是评估框架而非具体生成模型，因此归入aigc方向的[AIGC 评测与基准]（aigc_evaluation_benchmark）最为贴切。同时论文涉及对齐（alignment）概念，与generation_alignment也有交叉，但评测是主线。

Importance reason: Focuses on AI evaluation and alignment, not on core generative model architectures or efficiency. Signals: AIGC evaluation, persona-based alignment, not autoregressive or diffusion generation, no video/image generation, no model efficiency
...
### [99] [LLM-FACETS: A Privacy-Preserving Framework for Evaluating LLM Transparency and Accountability](https://arxiv.org/abs/2605.31167)
...
TL;DR: LLM-FACETS 是一个面向非技术从业者的开源 LLM 审计框架，提供浏览器界面和插件架构，支持确定性指标本地运行和 LLM-judge 指标可控调用，通过 token 级 log-probability、多裁判共识和 RAG Triad 指标实现透明度与可复现性评估。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 审计工具需要编程技能和复杂环境配置，且云平台会将评估数据发送到外部服务，导致领域专家和合规官员难以参与 AI 监管。需要一种对非技术用户友好、隐私保护且可复现的 LLM 评估框架。

Method: 提出 LLM-FACETS 框架，包含浏览器可访问界面和插件架构，围绕三类从业者画像（技术专家、领域专家、合规官员）设计，对应 EU AI Act 和 NIST AI Risk Management Framework 的利益相关者分类。确定性指标（BLEU、ROUGE、BERTScore）在自托管服务器本地运行，LLM-judge 指标显式调用外部 API 且用户保留凭证控制权。通过 token 级 log-probability 可视化、多裁判共识和 RAG Triad 指标（Faithfulness、Answer Relevance、Context Relevance）实现透明度。

Result: 通过 18 个指标实现与规范参考库的交叉验证，验证了框架的有效性。

Conclusion: LLM-FACETS 为 LLM 审计提供了可访问、隐私保护且可复现的解决方案，将 AI 问责制与系统构建团队解耦，支持跨检查多种指标以确保评估的可靠性。

Classification reason: 该论文提出 LLM-FACETS 框架，用于评估 LLM 输出的事实性、校准性和可复现性，属于 AIGC 生成内容质量评测范畴，与 [CANONICAL] subtopic "AIGC 评测与基准" 高度匹配。

Importance reason: Focuses on LLM auditing and transparency, which is not a priority subtopic and lacks alignment with user's core interests in generation and multimodal models. Signals: LLM auditing, privacy-preserving, non-technical practitioners, EU AI Act, RAG Triad
...
### [100] [Industrializing Prediction-Powered Inference: The GLIDE Library for Reliable GenAI and Agentic Systems Evaluation](https://arxiv.org/abs/2605.31278)
...
TL;DR: 本文提出 GLIDE，一个开源的 Python 库，统一了多种预测驱动推断（PPI）估计器和采样器，用于对生成式 AI 和智能体系统进行无偏且具有有效置信区间的可靠评估，显著减少人工标注成本。


<details>
  <summary>Details</summary>
Motivation: 智能体系统的可靠评估需要在昂贵的人工标注和有偏的 LLM-as-judge 代理之间取得平衡。现有 PPI 方法分散在不同论文中且实现不完整，缺乏统一易用的工具库。

Method: GLIDE 库统一了 PPI++、Stratified PPI、Predict-Then-Debias 及其分层变体、Active Statistical Inference 等先进 PPI 估计器，以及均匀、分层、主动、成本最优等采样器，采用 scipy 风格 API 专门针对均值估计。库包含可复现的蒙特卡洛验证套件和基于经验的方法选择决策树。

Result: 通过智能体评估案例研究，GLIDE 在同等精度下实现了显著的人工标注节省，验证了 PPI 方法在生成式 AI 评估中的有效性。

Conclusion: GLIDE 为生成式 AI 和智能体系统的可靠评估提供了统一、易用的工具，有助于推动评估方法从有偏代理和昂贵人工标注向高效、无偏的 PPI 方法转变。

Classification reason: 该论文提出 GLIDE 库，统一了预测驱动推断（PPI）方法用于可靠评估生成式 AI 和智能体系统，核心关注评估方法的可靠性与效率，属于 AIGC 评测与基准方向，因此映射到 canonical 子主题 aigc_evaluation_benchmark。

Importance reason: Focuses on evaluation methodology for GenAI/agents, not on core generation or multimodal model advances. Signals: PPI library, evaluation reliability, annotation cost reduction, not core generation, no multimodal focus
...
### [101] [Counterfactual Evaluation Reveals Hidden Capability Profiles in Clinical LLMs and Agents](https://arxiv.org/abs/2605.30590)
*Matt Turk*
...
TL;DR: 本文提出因果敏感性评分（CSS），一种预注册的干预式评测指标，通过沿五个临床维度（生物标志物翻转、既往治疗失败、生物标志物移除、手术状态变化、分期扰动）突变肿瘤病例输入，评估临床LLM和智能体是否根据临床信号变化正确更新其推荐。实验表明，CSS与传统的覆盖式指标（CMS）产生几乎相反的模型排名，并揭示了所有前沿模型在手术状态干预上的普遍安全盲点。


<details>
  <summary>Details</summary>
Motivation: 现有临床AI评测主要依赖覆盖式指标（如加权召回率），这些指标无法捕捉模型对输入变化的响应能力。两个模型可能在覆盖式指标上得分相近，但对患者输入变化的反应截然不同——一个更新推荐，另一个输出不变。因此需要一种能评估模型因果响应能力的评测方法。

Method: 提出因果敏感性评分（CSS），通过沿五个临床维度（生物标志物翻转、既往治疗失败、生物标志物移除、手术状态变化、分期扰动）对224个肿瘤病例进行干预式突变，使用{0, 0.5, 1.0}三级评分评估模型是否在预注册的正确方向上更新推荐。与覆盖式指标共识匹配评分（CMS）进行对比，在六个前沿模型上评估，并扩展到ReAct式工具使用智能体场景。

Result: 六个模型的CSS与CMS排名几乎完全相反：所有模型排名均发生变化，CMS最差模型成为CSS最佳模型，一个CMS中上模型在CSS上排名垫底。所有前沿模型在手术状态干预上表现极差（CSS最高仅17.2%），这一安全盲点未被CMS发现。工具使用提升了五个模型的CSS（+2.5至+20.3个百分点），但最低CSS模型即使检索相同图表仍无法更新推荐，暴露出结构性响应缺陷。

Conclusion: 干预式预注册指标（如CSS）能够补充覆盖式评测在临床AI智能体评估中的不足，捕捉覆盖指标遗漏的响应能力，并为未来的智能体强化学习系统提供候选密集奖励信号。

Classification reason: 该论文提出了一种新的因果敏感性评分（CSS）来评估临床LLM和智能体的响应能力，属于AIGC评测与基准范畴。CSS是一种干预式评测指标，用于评估模型在临床输入变化时是否更新其推荐，这与AIGC评测与基准（aigc_evaluation_benchmark）高度匹配。同时，论文涉及临床LLM和智能体，也与多模态理解与生成方向相关。

Importance reason: Focuses on clinical LLM evaluation, which is outside the user's core interests in multimodal generation and autoregressive models. Signals: Clinical LLM evaluation, counterfactual metric, safety blind spot, not multimodal generation, not autoregressive
...
### [102] [CodeGolf Bench: A Multi-Language Benchmark for Evaluating Concise Code Generation Capabilities of Large Language Models](https://arxiv.org/abs/2605.30394)
*Vedant Padwal*
...
TL;DR: 本文提出了 CodeGolf Bench，一个基于代码高尔夫（code golf）的多语言基准，用于评估 LLM 生成简洁高效代码的能力，覆盖 60 种编程语言。实验表明推理模型（如 o1）在简洁代码生成上显著优于非推理模型，尤其在 C++ 等语法严格的语言中差距更大。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成基准多关注功能正确性，缺乏对代码简洁性和效率的评估，且语言覆盖有限。代码高尔夫作为一种追求最短代码的竞赛形式，为衡量 LLM 生成高效简洁代码的能力提供了独特视角。

Method: 基于 Code Golf Stack Exchange 平台构建基准，支持 60 种编程语言，提供实时人类表现基线。评估了 9 个 LLM（包括推理模型如 o1、o3-mini 和非推理模型如 GPT-4o、Claude 3.5 Sonnet 等）在 Python 和 C++ 任务上的表现，使用百分位数排名作为指标。

Result: 推理模型在简洁代码生成上显著优于非推理模型，最佳平均百分位达 70.97%。在 C++ 任务上差距尤为明显，非推理模型在效率优化方面表现较差。CodeGolf Bench 提供了动态更新的框架，可跟踪 LLM 相对于人类表现的进展。

Conclusion: CodeGolf Bench 为评估 LLM 的代码简洁生成能力提供了有效基准，揭示了推理模型在该任务上的显著优势，并为未来代码生成研究提供了新的评测维度。

Classification reason: 该论文提出了 CodeGolf Bench，一个评估 LLM 在 60 种编程语言中生成简洁代码能力的基准，属于 AIGC 领域中的代码生成评测，与 [CANONICAL] 子主题 "AIGC 评测与基准" 高度匹配。

Importance reason: Focuses on code golf benchmark for LLMs, not aligned with user's priority on multimodal generation, video, or efficient models. Signals: code generation benchmark, multi-language, reasoning models, not multimodal, not video/image generation
...
### [103] [Beyond Static Dialogues: Benchmarking Realistic, Heterogeneous, and Evolving Long-Term Memory](https://arxiv.org/abs/2605.31086)
...
TL;DR: 本文提出 RHELM，一个面向 LLM 长期记忆的现实、异构、动态演化的评测基准。通过精心设计的用户画像和 LOOP 模块，构建了具有长期语义一致性和时间演化的多场景对话，并整合了与用户事件轨迹同步的异构外部数据源。基准涵盖 7 类查询和 27 种关键记忆特征，实验表明现有全上下文模型、RAG 方法和记忆框架在复杂真实场景中仍存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 记忆评测基准存在两个主要缺陷：对话缺乏长期语义一致性，用户画像扁平静态；且真实场景中用户与助手的交互涉及文档、邮件等异构数据流，现有评测未能覆盖这些复杂场景。因此需要更真实、更具挑战性的长期记忆评测基准。

Method: 提出 RHELM 基准，核心包含：(1) 精心设计的用户画像，确保长期一致性；(2) LOOP（pLan-rOllout-evOlve-Prune）模块，用于生成具有时间演化和长期连贯性的多场景对话；(3) 将对话与用户时间事件轨迹同步的异构外部数据源（如文档、邮件）深度融合；(4) 构建涵盖 7 类查询类型、映射到 27 种关键记忆特征的问答对。

Result: 在全上下文模型、RAG 方法和代表性记忆框架上的综合实验表明，现有方法在复杂真实场景中仍存在关键弱点，特别是在多源聚合推理和真实世界上下文推理方面表现不佳。

Conclusion: RHELM 揭示了当前 LLM 长期记忆方法在真实复杂场景中的不足，为未来研究提供了更具挑战性和现实意义的评测基准，尤其强调了多源信息聚合和上下文推理能力的重要性。
...
Importance reason: Focuses on LLM long-term memory benchmarks, not directly on the user's priority subtopics like continuous VLM or video generation. Signals: LLM memory benchmark, long-term dialogue, heterogeneous data, not multimodal generation, not autoregressive video/image
...
### [104] [KnowledgeGain: Evaluating and Optimizing Science News Generation for Reader Learning](https://arxiv.org/abs/2605.31099)
...
TL;DR: 本文提出 KnowledgeGain，一个衡量科学新闻生成文本对读者知识获取效果的评估指标，并通过人类实验和 LLM 模拟器验证其有效性，用于优化科学新闻生成。


<details>
  <summary>Details</summary>
Motivation: 现有生成文本评估指标（如语义相似度、事实一致性）无法衡量读者从科学新闻中实际学到了多少知识，缺乏面向知识获取的生成质量评估方法。

Method: 提出 KnowledgeGain 指标，通过对比读者阅读前后的知识水平差异来评估科学新闻质量。首先进行受控人类实验验证指标有效性，然后基于实验数据校准一个仅用提示的 LLM 读者模拟器，用于对候选文章进行排序和筛选。

Result: 人类实验表明 KnowledgeGain 能有效捕捉读者阅读不同类型科学媒体后的知识差异。使用 LLM 模拟器筛选的文章在第二项人类实验中显著提升了读者阅读后的准确率和归一化 KnowledgeGain，优于强生成基线。

Conclusion: KnowledgeGain 是一种有效的面向知识获取的科学新闻生成评估指标，结合 LLM 模拟器可优化生成质量，向实现 Bloom 认知分类学中知识与理解目标迈进一步。

Classification reason: 该论文提出 KnowledgeGain 指标，用于评估科学新闻生成内容对读者知识获取的效果，属于生成内容质量评测范畴，与 [CANONICAL] 子主题 aigc_evaluation_benchmark（AIGC 评测与基准）高度匹配。

Importance reason: Focuses on science news evaluation for reader learning, not on the user's priority subtopics like continuous VLM, autoregressive video generation, or world models. Signals: AIGC evaluation metric, science news generation, reader knowledge gain, LLM simulator, not aligned with priority subtopics
...
### [105] [TSM-Bench: Detecting LLM-Generated Text in Real-World Wikipedia Editing Practices](https://arxiv.org/abs/2605.31113)
...
TL;DR: 本文提出 TSM-Bench，一个多语言、多生成器、多任务的基准，用于评估 AI 生成文本检测器在真实维基百科编辑任务（如摘要、改写等）上的表现。实验表明，现有 SOTA 检测器在任务特定 MGT 上的准确率比通用生成文本下降 10-40%，且存在泛化不对称性：在任务特定数据上微调可泛化到通用数据，反之则不行。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 生成文本检测基准主要关注通用文本生成任务（如"写一篇关于机器学习的文章"），但实际用户生成内容平台（如维基百科）的编辑者通常使用 LLM 执行特定写作任务（如摘要、改写），这些任务特定 MGT 因受约束的任务设定和上下文条件而更接近人类写作，现有检测器在此场景下的表现尚不清楚。

Method: 构建 TSM-Bench 基准，涵盖 5 种维基百科编辑任务（摘要、改写、事实核查、风格转换、翻译），使用 4 种 LLM（GPT-4o、Claude、Gemini、Llama），覆盖 3 种语言（英语、德语、中文），包含 10,000+ 样本。在基准上评估 10 种 SOTA 检测器（包括统计方法、微调模型、零样本 LLM 检测器），分析任务特定 vs 通用 MGT 的检测性能差异及泛化特性。

Result: 1) 任务特定 MGT 检测准确率比通用 MGT 下降 10-40%；2) 存在泛化不对称性：在任务特定数据上微调可泛化到通用数据（甚至跨领域），但在通用数据上微调无法泛化到任务特定数据；3) 在通用数据上微调的模型过拟合到机器生成的表面伪影；4) 大多数检测器在真实维基百科编辑场景中仍不可靠。

Conclusion: 现有 AI 生成文本检测器在真实世界的维基百科编辑场景中表现不佳，TSM-Bench 为开发更鲁棒的检测方法提供了关键基础。未来检测器需要针对任务特定 MGT 进行专门设计和评估。

Classification reason: 该论文提出了 TSM-Bench，一个用于评估 AI 生成文本检测器在真实维基百科编辑场景中表现的多语言、多生成器、多任务基准。这属于 AIGC 检测与评测的交叉领域，但核心贡献是评测基准，因此映射到 [CANONICAL] 的 aigc_evaluation_benchmark（AIGC 评测与基准）。

Importance reason: Focuses on AIGC detection in Wikipedia, which is outside the user's priority subtopics and has negative keywords like "clinical" not directly applicable. Signals: AIGC detection benchmark, task-specific MGT, generalization asymmetry, low personal relevance to user's multimodal generation focus
...
### [106] [Preference-Aware Rubric Learning for Personalized Evaluation](https://arxiv.org/abs/2605.31545)
...
TL;DR: 本文提出PARL框架，将个性化评估建模为学习问题，通过从用户历史交互中学习偏好感知评估准则（rubric），并引入自验证机制和判别式强化学习目标，实现对个性化文本生成的高保真评估。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法（自动指标、LLM-as-a-judge）无法捕捉用户长期交互历史中蕴含的主观、个性化的偏好，缺乏对个性化对齐的有效评估手段。

Method: 提出"个性化评估即学习"范式，将评估从静态判断转化为学习问题。PARL框架包含两个核心模块：(1) 准则归纳（Rubric Induction），从原始用户历史中学习偏好感知的评估准则；(2) 自验证机制（Self-Validation），确保准则与用户偏好一致。同时引入判别式强化学习目标，对比用户撰写的回复与个性化模型输出，学习精确的用户特定决策边界。

Result: 在真实个性化文本生成任务上，PARL能够一致地归纳出高保真评估准则，可靠地识别与用户对齐的回复，并在不同用户和任务间具有良好的泛化能力，同时捕捉稳定的风格偏好和细粒度评估模式。

Conclusion: 将个性化评估建模为学习问题是一种有效范式，PARL通过偏好感知准则学习能够更好地捕捉用户个性化偏好，为个性化对齐评估提供了新思路。

Classification reason: 该论文研究个性化评估方法，属于AIGC评测范畴，与[AIGC 评测与基准]（aigc_evaluation_benchmark）高度匹配。同时涉及个性化对齐评估，也与多模态理解与生成方向相关。

Importance reason: Focuses on personalized evaluation for text generation, which is outside the user's core interests in multimodal generation and video/image generation. Signals: AIGC evaluation, personalized alignment, rubric learning, text generation, no multimodal or video/image focus
...
### [107] [BlueFin: Benchmarking LLM Agents on Financial Spreadsheets](https://arxiv.org/abs/2605.30907)
*Srivatsa Kundurthy, Clara Na, Colton Moraine, Anoushka Mohta, Case Winter, George Fang, John Ling, Emma Strubell, Zach Kirshner*

Main category: cs.CL, cs.AI, cs.SE, cs.LG
...
TL;DR: BlueFin 是一个面向金融电子表格领域的 LLM 智能体评测基准，包含 131 个复杂任务和 3,225 个细粒度评分标准，由专家人工标注验证 LM 评判器，前沿模型平均得分不足 50%。


<details>
  <summary>Details</summary>
Motivation: 电子表格软件拥有数亿用户，远超专业开发者数量，但 LLM 在电子表格领域的能力探索严重不足，尤其是面向金融专业场景的复杂任务评测资源匮乏。

Method: 构建了 131 个金融电子表格任务（含合成、操作和理解三类），每个任务配有细粒度评分标准（共 3,225 条），由专家人工标注验证 LM 评判器的评估可靠性，并开源了智能体评估框架。

Result: LM 评判器与专家共识达到高度一致（α=0.826，macro-F1=0.839）；前沿 LLM 平均得分低于 50%，在动态正确性方面表现尤为薄弱。

Conclusion: 现有前沿 LLM 在金融电子表格任务上表现不佳，BlueFin 为这一重要但被忽视的领域提供了高质量评测基准，揭示了模型在动态正确性等方面的显著不足。

Classification reason: 该论文提出了一个针对金融领域电子表格任务的 LLM 智能体评测基准 BlueFin，属于 AIGC 方向下的评测与基准（aigc_evaluation_benchmark）[CANONICAL] 子主题，因为其核心贡献是构建了包含 131 个复杂任务的评测数据集和评估框架。

Importance reason: Focuses on financial spreadsheet agents, which is outside the user's priority subtopics and includes negative keyword "financial". Signals: Financial domain, LLM agent benchmark, spreadsheet tasks, low model performance, expert-validated rubric
...
### [108] [Personalized to Persuade: The Effects of Contextualization and Warmth on Trust and Reliance in Conversational AI](https://arxiv.org/abs/2605.31275)
...
Importance: 普通相关 (34.94) - 未入选精读

TL;DR: 本文通过2×2被试间实验（N=380）研究对话式AI的个性化（contextualization）与温暖感（warmth）如何影响用户对AI助手的信任和依赖。结果表明：个性化单独使用会降低AI的说服力，但与温暖感结合后通过交叉交互恢复说服力；用户对AI的依赖在所有条件下均存在且不受对话设计影响；信任是说服和依赖的强预测因子，但个性化与温暖感并非通过信任起作用；AI素养高的用户对AI信任更低，但反而更易被说服和依赖。


<details>
  <summary>Details</summary>
Motivation: 个性化（contextualization）已被识别为政治和营销中的说服策略，但在日常任务中（用户常缺乏先验知识）其说服效果尚不明确。本文旨在探究个性化与温暖感如何共同影响用户对AI助手的信任和依赖。

Method: 采用2×2被试间实验设计（N=380），操纵两个因素：contextualization（有/无）和warmth（温暖/中性），让AI助手针对专家建议提出反对论点，测量用户的信任、说服效果和依赖行为。

Result: 1) 个性化单独降低AI说服力，但与温暖感结合后通过交叉交互恢复说服力；2) 用户对AI的依赖在所有条件下均存在且不受对话设计影响；3) 信任是说服和依赖的强预测因子，但个性化与温暖感不通过信任起作用；4) AI素养高的用户信任更低，但更易被说服和依赖。

Conclusion: 用户倾向于依赖AI代理而非人类专家判断，但界面级别的对话设计选择对行为的影响有限。个性化与温暖感的设计组合可以调节AI的说服效果，但信任机制并非其中介变量。

Classification reason: 该论文研究对话式AI中个性化（contextualization）与温暖感（warmth）对用户信任和依赖的影响，属于AIGC方向下的个性化与对话生成（aigc_personalization_persona）这一规范主题，因为其核心关注个性化助手对话设计及其对用户行为的影响。

Importance reason: This paper focuses on human-AI trust and persuasion in conversational AI, which is outside the user's core technical priorities in multimodal generation and autoregressive models. Signals: Human-computer interaction, Persuasion and trust, Not technical generation, No multimodal or autoregressive focus, Low alignment with priority subtopics
...
### [109] [Latent Space Disentanglement via Activation Steering for Interpretable Attribute Control in Symbolic Music Generation](https://arxiv.org/abs/2605.31295)
*Ioannis Prokopiou, Pantelis Vikatos, Maximos Kaliakatsos-Papakostas, Theodoros Giannakopoulos, Themos Stafylakis*

Main category: cs.LG, cs.IR, cs.AI, cs.SD
...
TL;DR: 本文提出一种基于激活引导（activation steering）的框架，利用 Difference-in-Means 方法和 Gram-Schmidt 正交化，在 Multitrack Music Transformer 的残差流中解耦音高和时长等属性的潜在方向，实现无需重训练的、可解释的符号音乐生成属性控制。


<details>
  <summary>Details</summary>
Motivation: Transformer 在符号音乐生成中取得了显著进展，但缺乏细粒度、可解释的属性控制能力。现有方法通常需要重训练或引入额外模块，且多属性控制时存在特征纠缠问题。本文旨在通过机械可解释性方法实现无需重训练的确定性属性调制。

Method: 采用 Difference-in-Means (DiffMean) 方法从残差流中提取音高和时长的潜在方向，验证线性表示假设。针对多属性引导中的特征纠缠，提出 Dual Steering 框架，利用 Gram-Schmidt 正交化对属性方向进行几何解耦，实现独立控制。

Result: 实验表明，引导强度与属性偏移之间具有高相关性，验证了线性表示假设。与朴素向量加法相比，Gram-Schmidt 正交化解耦方法显著减少了概念干扰和信号退化，在强自回归条件下仍能实现独立的确定性控制。

Conclusion: 本文证明了通过激活引导可以在符号音乐生成中实现可解释的属性控制，Gram-Schmidt 正交化有效解决了多属性引导中的特征纠缠问题，为生成模型的可控性和可解释性提供了新思路。

Classification reason: 该论文研究符号音乐生成中的可解释属性控制，属于创意与艺术生成范畴，与 [CANONICAL] aigc_creative_art（创意与艺术生成）高度匹配。

Importance reason: Focuses on symbolic music generation, which is outside the user's priority subtopics and negative-keyword-filtered domains. Signals: activation steering, interpretable control, symbolic music, transformer, disentanglement
...
### [110] [SteerFace: Debiasing Synthetic Face Generation via Adaptive Residue Perturbation](https://arxiv.org/abs/2605.30894)
...
Method: 提出SteerFace训练框架，在扩散模型训练过程中对身份嵌入施加自适应扰动：将身份嵌入向嵌入超球面上的随机正交方向推动，扰动强度通过可学习参数自适应调整，兼顾样本级偏好和整体统计特性。该扰动作为身份保持的正则化项，惩罚生成器对非身份成分的依赖。

Result: 实验表明SteerFace有效缓解了视觉倾向性，在多个下游人脸识别基准上优于先前方法，且在不同训练数据集和生成流水线上均具有良好的泛化能力。

Conclusion: SteerFace通过简单高效的自适应残差扰动策略，有效改善了合成人脸数据的分布质量，缩小了合成-真实性能差距，为合成数据在人脸识别训练中的实用化提供了新思路。

Classification reason: 该论文研究使用合成人脸数据训练人脸识别模型，核心是改进合成数据的质量以缩小与真实数据的差距，属于合成训练数据生成范畴。aigc_synthetic_data（合成数据与数据集蒸馏）是[CANONICAL]子主题，完美匹配。同时该论文也涉及多模态生成（扩散模型生成人脸），因此也匹配multimodal_generation方向。

Importance reason: Focuses on face recognition synthetic data debiasing, not aligned with user's priority subtopics in multimodal generation, video/image generation, or model efficiency. Signals: Face recognition, synthetic data debiasing, identity embedding perturbation, not in priority subtopics, no boost keywords matched
...
TL;DR: 本文研究非英语临床编码检索中句子嵌入模型性能退化的问题，提出使用大语言模型（Gemini）生成多语言合成数据来微调双编码器+交叉编码器检索器，在西班牙语、加泰罗尼亚语、法语、葡萄牙语上显著提升召回率，同时保持英语性能可接受。


<details>
  <summary>Details</summary>
Motivation: 句子嵌入模型在英语语料上表现优异，但在非英语临床检索（尤其是 ICD-10-CM 编码检索）中召回率严重下降，且聚合基准测试掩盖了这种退化。现有方法依赖英语生物医学预训练，缺乏多语言临床检索的解决方案。

Method: 提出两阶段检索器：双编码器（bi-encoder）进行初筛，交叉编码器（cross-encoder）进行重排序。使用 Gemini 生成覆盖英语、西班牙语、加泰罗尼亚语、意大利语、葡萄牙语和法语的合成数据，微调西班牙语生物医学编码器（PlanTL-GOB-ES/bsc-bio-ehr-es）。

Result: 双编码器在 MRR（0.876 vs 0.866）上匹配 BioBERT-ST，在 R@3（0.650 vs 0.626）和 R@5（0.804 vs 0.790）上超越。加入交叉编码器后，R@5 提升至 0.822，在四种非英语语言上均占优（西班牙语+0.017、加泰罗尼亚语+0.033、法语+0.018、葡萄牙语+0.037），仅英语略有下降。葡萄牙语 R@5 达 0.829，远超 BioBERT-ST 的 0.714。

Conclusion: 大语言模型生成的合成数据可以有效弥补非英语临床编码检索的性能差距，提供了一种无需英语生物医学预训练即可构建领域特定医疗检索器的开放方案，且性能提升在临床可接受范围内。

Classification reason: 该论文使用大语言模型（Gemini）生成合成数据来训练临床编码检索器，属于利用生成模型构建合成训练数据的典型工作，因此映射到 AIGC 方向下的 [CANONICAL] 子主题"合成数据与数据集蒸馏"。

Importance reason: Focuses on clinical coding retrieval in non-English languages, which is outside the user's priority subtopics and includes negative keywords like clinical. Signals: Non-English clinical retrieval, synthetic data from LLM, bi-encoder + cross-encoder, negative keyword "clinical", no overlap with priority subtopics
...
### [112] [Not All Synthetic Data Is Yours to Learn From](https://arxiv.org/abs/2605.31126)
...
TL;DR: 本文提出"潜在能力浮现假说"：弱自训练（无提示、无教师、无验证器的自生成文本微调）可以放大预训练模型中已有的能力，但前提是合成数据与学生模型之间存在"兼容性"关系。实验表明合成数据的效用是关系性的而非内在的，自生成数据最有效，同源迁移优于异源，跨族迁移最弱；常见的固有代理指标（语义相似度、token似然）无法预测数据效用；该机制还能在保持能力的同时将逐字记忆降低95%以上，实现能力与记忆的解耦。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型能否仅通过自身生成的纯文本（无提示、无教师、无验证器、无奖励模型）进行自我改进，以及这种改进的机制和条件是什么。

Method: 在无提示无条件自训练（prompt-free unconditional self-training）的最小化设定下，对基础语言模型仅使用从BOS token生成的文本进行微调，无任何任务说明或外部监督。通过控制数据来源（自生成、同族不同源、跨族）进行对比实验，分析合成数据效用与源-学生关系、固有代理指标的关系，并在Pythia模型上测试能力与逐字记忆的解耦效果。

Result: 1) 合成数据效用是关系性的：自生成数据最有效，同源迁移优于异源，跨族迁移最弱；2) 常见固有代理指标（语义相似度、平均token似然）无法预测数据效用；3) 在Pythia实验中，能力保持或提升的同时，逐字精确匹配提取下降超过95%，无需遗忘集、隐私目标或定向遗忘。

Conclusion: 无提示自训练通过放大学生模型已有的知识来工作，而非从数据中导入新结构；该机制揭示了一个无需显式遗忘目标即可分离能力与逐字记忆的范式，为理解合成数据在自训练中的作用提供了新视角。

Classification reason: 该论文研究语言模型在无提示、无监督条件下使用自身生成的合成文本进行自训练的效果，核心关注合成数据的效用（兼容性）及其与能力提升/记忆解耦的关系，属于合成数据用于模型训练的范畴，因此映射到 [CANONICAL] 子主题 aigc_synthetic_data（合成数据与数据集蒸馏）。同时论文也涉及生成模型（语言模型自生成文本），因此也匹配 multimodal_generation 方向。

Importance reason: Explores synthetic data utility and capability-memorization decoupling, but focuses on text-only LLMs, not the user's priority multimodal generation or video/image domains. Signals: Weak self-training amplifies latent capabilities, Synthetic data utility is relational, not intrinsic, Capability and verbatim memorization decouple without unlearning, Negative keywords not triggered, No direct connection to continuous VLM, video generation, or world models
...
### [113] [Differentially Private Preference Data Synthesis for Large Language Model Alignment](https://arxiv.org/abs/2605.30808)
...
TL;DR: 本文提出 DPPrefSyn，一种在差分隐私保证下合成偏好数据以用于大语言模型对齐的算法。它基于 Bradley-Terry 偏好模型和聚类共享线性结构，结合 DP-PCA 学习私有偏好模型并生成高质量合成偏好数据，在强隐私保证下实现了有竞争力的对齐性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的后训练对齐阶段依赖真实人类偏好数据，但这些数据往往包含敏感的用户提示和人工判断，存在隐私泄露风险。现有工作缺乏在保护隐私的前提下生成可用于对齐的合成偏好数据的方法。

Method: DPPrefSyn 基于 Bradley-Terry 偏好模型框架，首先从私有数据中学习具有形式化差分隐私保证的偏好模型，利用聚类共享线性结构捕获异构偏好，并通过 DP-PCA 提高学习精度；然后利用学习到的模型结合公共提示合成高质量的偏好数据。

Result: 大量实验表明，DPPrefSyn 在强差分隐私保证下实现了有竞争力的对齐性能，验证了合成偏好数据作为隐私保护对齐的可行替代方案的有效性。

Conclusion: 本文首次提出生成差分隐私合成偏好数据用于 LLM 对齐的方法，展示了合成数据在隐私保护对齐中的潜力，为广泛的应用场景提供了实用替代方案。

Classification reason: 该论文提出差分隐私合成偏好数据生成方法，属于合成训练数据生成范畴，与 AIGC 方向下的 [CANONICAL] 子主题"合成数据与数据集蒸馏"高度匹配。同时涉及 LLM 对齐（generation_alignment）和隐私保护，但核心贡献是合成数据生成。

Importance reason: Novel DP synthetic preference data for LLM alignment, but outside user's core multimodal generation and efficiency focus. Signals: first DP synthetic preference data for LLM alignment, Bradley-Terry model + DP-PCA, strong privacy guarantees, competitive alignment performance, not multimodal or generation-focused
...
### [114] [MAAT: Multi-phase Adapter-Aware Targeted Unlearning](https://arxiv.org/abs/2605.30514)
...
TL;DR: 本文指出现有机器遗忘评测中"为什么"类因果知识占比极低（<1.3%），导致方法在因果知识上失效却无法被检测。为此提出平衡的5W评测基准5WBENCH（5000样本，每类1000例），并发现现有方法无法同时实现因果知识的高遗忘与高保留。进一步提出MAAT框架，通过多阶段LoRA适配器操作（梯度投影上升、SVD秩剪枝、任务向量否定、KL-隐状态保留修复），首次在因果知识上达到遗忘-保留Pareto前沿的新操作点。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘评测中"Why"类（因果/关系知识）问题占比极低（CounterFact中<0.06%，TOFU中<1.3%），导致方法在因果知识上失败时仍能获得高分，且这种失败无法被检测。需要平衡的评测基准和能处理因果知识遗忘的方法。

Method: 1) 构建5WBENCH平衡基准：5000样本，5W类别各1000例；2) 提出MAAT三阶段框架：阶段一（梯度投影上升）在LoRA权重上沿遗忘梯度方向上升；阶段二（SVD秩剪枝+任务向量否定）对LoRA权重进行SVD分解剪枝低秩分量并应用任务向量否定；阶段三（混合KL-隐状态保留修复）通过KL散度和隐状态对齐修复保留知识。

Result: 在5WBENCH上，MAAT是首个同时在Why类因果知识上实现高遗忘率和高保留率的方法，在遗忘-保留Pareto前沿上达到了新的操作点。实验表明Why类知识的困难源于多跳推理链（44% vs 其他≤2%）和长答案跨度（平均40.1 token）导致的梯度稀释。

Conclusion: 机器遗忘评测需要平衡的5W覆盖，因果知识的遗忘比事实知识更具挑战性。MAAT通过多阶段LoRA适配器操作首次实现了因果知识的高质量遗忘，为未来机器遗忘研究提供了新的基准和方法框架。
...
Importance reason: Focuses on machine unlearning evaluation and causal knowledge, not aligned with user's priority on multimodal generation and video/image generation. Signals: machine unlearning, causal knowledge, benchmark, LoRA adapter, Pareto frontier
...
### [115] [AMNESIA: A Large Scale Medical Unlearning Benchmark Suite with Disease-Informed Analysis](https://arxiv.org/abs/2605.30599)
...
TL;DR: AMNESIA 是首个大规模、开源的医疗领域机器遗忘基准测试，包含 70,560 个问答对，覆盖 11 种疾病类别，用于评估遗忘方法在患者级别和疾病级别上的效果，并揭示了遗忘个体患者会损害同疾病其他患者知识的现象。


<details>
  <summary>Details</summary>
Motivation: 医学知识不断更新，需要从已训练的医疗LLM中选择性地遗忘特定数据，但现有遗忘基准依赖合成或小规模通用数据，缺乏临床遗忘的系统性评估。

Method: 构建了包含 8,820 份患者笔记、70,560 个问答对的大规模医疗遗忘基准 AMNESIA，涵盖事实性问题和推理问题，在随机患者和疾病级别上评估四种主流遗忘方法，并引入检测医学术语泄露的新指标。

Result: 遗忘个体患者会侵蚀同疾病其他患者的知识；现有遗忘方法在疾病级别遗忘上表现不足；新指标有效检测了医学术语的泄露。

Conclusion: 需要开发能够更好区分个体患者与共享临床知识的遗忘方法，AMNESIA 为医疗遗忘研究提供了标准化评估平台。

Classification reason: 该论文聚焦于医疗LLM的机器遗忘（machine unlearning），属于生成模型/LLM的概念擦除与遗忘学习范畴，与aigc方向下的[CANONICAL] subtopic "概念擦除与遗忘学习"（aigc_unlearning_erasure）高度匹配。

Importance reason: The paper focuses on medical unlearning, which is outside your configured priority subtopics and is explicitly penalized by negative keywords like "medical diagnosis" and "clinical". Signals: Medical unlearning benchmark, Disease-level knowledge erosion, Low alignment with user's multimodal generation and autoregressive video/image generation priorities
...
### [116] [Divergence Decoding: Inference-Time Unlearning via Auxiliary Models](https://arxiv.org/abs/2605.31293)
...
TL;DR: 本文提出 Divergence Decoding (DD)，一种推理时遗忘方法，通过小型辅助模型在推理时调整 LLM 的 logits，使其远离需要遗忘的特定数据。该方法在遗忘基准上显著优于 SOTA 基线，且可蒸馏回基座模型，并泛化到图像生成领域。


<details>
  <summary>Details</summary>
Motivation: LLM 经常记忆敏感训练数据，带来隐私和版权风险。现有的遗忘方法要么导致严重的效用损失，要么对复杂查询无效。因此需要一种高效、低成本且不损害模型能力的遗忘机制。

Method: 提出 Divergence Decoding (DD)，使用小型辅助模型（如小规模预训练或微调模型）在推理时对 LLM 的 logits 进行引导，使其分布偏离需要遗忘的数据。该方法仅需标准预训练和微调设置即可训练辅助模型，且引导后的分布可以蒸馏回基座模型。

Result: 在多种模型规模和训练数据集规模上，DD 在遗忘基准上显著优于 SOTA 基线方法。该方法也展示了在图像生成领域的泛化能力，表明其适用于任意概率模型。

Conclusion: Divergence Decoding 是一种有效且低成本的推理时遗忘方法，可解决 LLM 的记忆隐私问题，且具有跨模态泛化潜力。

Classification reason: 该论文提出 Divergence Decoding 方法，通过辅助模型在推理时引导 LLM 的 logits 远离特定数据，实现模型遗忘（unlearning），属于生成模型的概念擦除与遗忘学习方向。该方向在 AIGC 下有 [CANONICAL] 子主题 aigc_unlearning_erasure，且论文也探索了在图像生成领域的泛化性，因此也匹配 multimodal_generation 方向。

Importance reason: Focuses on LLM unlearning, which is outside the user's priority subtopics and negative-keyword-filtered. Signals: Inference-time unlearning, auxiliary model steering, logit adjustment, generalizes to images, not in priority subtopics
...
### [117] [De-attribute to Forget for LLM Unlearning](https://arxiv.org/abs/2605.30919)
...
TL;DR: 本文提出DareU，首个基于数据归因奖励的LLM遗忘学习框架，通过强化学习降低模型生成响应与遗忘数据的归因分数（即"去归因"），实现有效遗忘的同时避免过度遗忘和模型效用下降。


<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法多依赖优化预测损失（如最大化遗忘集损失），容易导致过度遗忘和模型效用下降。本文旨在通过新的优化视角——将遗忘目标转化为归因分数归零——来解决这些问题。

Method: 提出DareU框架，将LLM遗忘的优化目标重新定义为归因分数归零。具体地，使用强化学习更新LLM，降低模型生成响应与遗忘数据所有者之间的归因分数（即去归因）。使用LLM分类器作为归因的高效近似。

Result: 实验表明，DareU在遗忘质量和模型效用之间取得了良好平衡，优于现有基线方法。

Conclusion: 基于数据归因奖励的遗忘学习是一种有效且实用的LLM遗忘范式，能够缓解过度遗忘问题，为LLM安全治理提供了新思路。

Classification reason: 该论文研究LLM遗忘学习（unlearning），属于生成模型安全与遗忘范畴。aigc_unlearning_erasure（概念擦除与遗忘学习）是[CANONICAL]子主题，完美匹配。同时该主题也出现在multimodal_generation和multimodal_understanding_generation方向下，因此匹配多个方向。

Importance reason: Focuses on LLM unlearning via data attribution, which is outside the user's priority subtopics and direction weights. Signals: LLM unlearning, data attribution, reinforcement learning, forget quality, model utility
...
### [118] [Forgetting Has Neighbors: Localized Collateral Forgetting in Machine Unlearning](https://arxiv.org/abs/2605.31317)
...
TL;DR: 本文研究了机器遗忘中的局部附带遗忘现象：在梯度上升和随机标签等遗忘方法中，遗忘效果在示例层面高度不均匀，与遗忘集几何邻近的样本受到更大的附带影响。作者揭示了该现象源于遗忘时使用的替代目标与重训练局部预测结构不一致，并提出局部教师蒸馏（Local Teacher Distillation）策略，通过在遗忘集邻域上训练小教师模型生成软标签来缓解该问题。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘评估通常使用聚合指标（如准确率、遗忘分数），这些指标可能掩盖局部层面的失败模式。作者希望揭示遗忘方法在示例级别的非均匀影响，并理解其背后的机制，从而改进遗忘质量。

Method: 1）在示例级别比较遗忘模型与重训练模型的预测差异，发现梯度上升和随机标签方法中差异随与遗忘集的几何邻近度增大而增大（局部附带遗忘）。2）分析机制：遗忘时使用的替代目标（如随机标签）与重训练诱导的局部预测结构不一致，这种不一致通过共享表征传播到邻近样本。3）提出局部教师蒸馏（Local Teacher Distillation）：在遗忘集邻域样本上训练一个小型教师模型，用其软标签替代随机目标进行遗忘。

Result: 在 CIFAR-100 部分类别删除任务上，局部教师蒸馏使遗忘模型在遗忘集附近样本上的预测显著更接近重训练模型，同时保持了有竞争力的聚合遗忘指标。

Conclusion: 机器遗忘存在局部附带遗忘问题，其根源在于替代目标与局部预测结构的不一致。局部教师蒸馏是一种简单有效的缓解策略，为设计更精细的遗忘方法提供了新方向。
...
Importance reason: Focuses on machine unlearning, which is not a priority subtopic and lacks alignment with user's key directions. Signals: machine unlearning, localized collateral forgetting, gradient ascent, random labeling, local teacher distillation
...
### [119] [Linear Ensembles Wash Away Watermarks: On the Fragility of Distributional Perturbations in LLMs](https://arxiv.org/abs/2605.30501)
...
TL;DR: 本文揭示了LLM文本水印的一个根本性漏洞：当用户访问多个模型时，通过线性平均多个模型的输出概率分布即可有效消除水印信号。作者提出WASH方法，解决了异构模型间的词汇表不对齐和分词差异问题，实验表明平均3-5个模型即可将检测z-score从5-300降至2以下，同时提升生成质量27.5%且速度提升6倍。


<details>
  <summary>Details</summary>
Motivation: 现有LLM文本水印方法通过在输出分布中嵌入统计信号来实现AI生成文本的检测与归因。然而，现实中用户可访问多个模型提供商的服务，水印扰动在不同提供商之间通常是独立的。作者希望探究这种多模型访问场景下水印是否仍然有效，并揭示其根本性脆弱性。

Method: 作者提出WASH（Watermark Attenuation via Statistical Hybridisation）方法。核心思想是：水印对输出分布施加了扰动，而多个独立水印模型的输出分布平均后，扰动会相互抵消，恢复出接近无水印的分布。WASH解决了两个实际挑战：(1) 词汇表不对齐——通过取并集构建统一词汇表；(2) 分词差异——通过将不同分词器的token映射到统一词汇表。最终通过平均多个模型的logits来生成文本。

Result: 在6种水印方案和3种LLM上的实验表明：平均3个模型即可将检测z-score从5-300降至2以下（低于检测阈值4），TPR@5%FPR降至50%以下；同时生成质量提升27.5%，长序列生成速度比最优基线快6倍。

Conclusion: 线性平均多个模型的输出分布可以有效消除水印信号，揭示了当前LLM水印方案在多模型访问场景下的根本性脆弱性。鲁棒的AI文本检测要么需要接受这一漏洞，要么需要模型提供商之间前所未有的协调。

Classification reason: 该论文研究LLM文本水印的脆弱性，属于AIGC方向下的水印与版权保护（aigc_watermark_copyright）这一[CANONICAL]子主题，直接对应水印的鲁棒性与安全性分析。

Importance reason: This paper reveals a fundamental vulnerability in LLM watermarking, but it is outside the user's priority subtopics and direction weights. Signals: LLM watermark vulnerability, linear ensemble attack, WASH method, distributional perturbation, model averaging
...
### [120] [NumLeak: Public Numeric Benchmarks as Latent Labels in Foundation Models](https://arxiv.org/abs/2605.30393)
*Anany Kotawala*
...
TL;DR: 本文提出 NumLeak 框架，系统性地揭示了大语言模型在公开数值基准（如金融回报率、失业率、通胀率等）上的记忆泄露问题。通过 API 边界探测和白盒验证，发现顶级模型能以极高精度回忆 Fama-French 因子、美国经济指标等公开数据，且闭源 API 的黑盒探测低估了记忆程度。论文还提出一行系统提示即可阻断 99.8% 的非自适应单轮后缀攻击，且对概念和历史查询几乎无性能损失。


<details>
  <summary>Details</summary>
Motivation: 公开数值基准数据常出现在预训练语料中，导致模型在评估时可能只是记忆召回而非真正泛化。现有研究多关注文本记忆，对数值型基准的记忆泄露缺乏系统性测量，且闭源 API 的黑盒探测可能低估记忆程度。作者希望量化这一风险并提出防御方案。

Method: 提出 NumLeak 测量框架，包含两部分：(1) API 边界探测——对闭源前沿模型（如 GPT-4、Claude）进行数值回忆测试，使用种子池化 Pearson r 和基点误差等指标；(2) 白盒验证——在开源因果 LM 上复现剂量-反应关系，利用 logprob 排名检测记忆。还设计了"拒绝-回忆不对称性"检验和残差化分析来区分记忆与真实推理。

Result: 顶级模型对 Fama-French 市场超额回报的 3-种子池化 Pearson r 达 0.97-0.99，五个兄弟因子误差在 25 基点内；对美国失业率、CPI 通胀、NOAA 温度也有类似高保真度。在最新发布的保留集上，解析率降至 21-57%，但已回答月份的 r 仍约 0.99。白盒实验复现了剂量-反应关系，logprob 排名能检测到开放生成遗漏的记忆。一行系统提示防御可阻断 99.8% 的非自适应单轮后缀攻击。

Conclusion: 大模型对公开数值基准存在严重的记忆泄露问题，闭源 API 的黑盒探测会低估这一现象。简单的系统提示防御可以有效缓解数值记忆泄露，且对模型正常能力影响极小。研究呼吁对模型记忆进行更系统的评估和治理。

Classification reason: 该论文研究大模型在公开数值基准上的记忆泄露问题（NumLeak），属于生成模型的安全与治理范畴。映射到 AIGC 方向下的 [CANONICAL] 子主题"生成内容安全与治理"（aigc_safety_governance），因为论文核心关注模型记忆导致的安全隐患（数据泄露、评估污染）及防御措施（系统提示防御），与内容安全、合规监管高度相关。

Importance reason: Focuses on LLM memorization of public numeric benchmarks, which is outside the user's priority subtopics of multimodal generation, video/image generation, and model efficiency. Signals: LLM memorization, benchmark contamination, security, not multimodal, not generative
...
### [121] [COMPASS: Cognitive MCTS-Guided Process Alignment for Safe Search Agents](https://arxiv.org/abs/2605.30838)
...
TL;DR: COMPASS 提出了一种基于认知 MCTS 引导的过程对齐框架，用于增强 LLM 搜索智能体的安全性，通过认知树探索合成攻击轨迹和自省式逐步对齐实现细粒度过程监督，在保持通用能力的同时显著提升安全性。


<details>
  <summary>Details</summary>
Motivation: LLM 驱动的搜索智能体具备多步推理和工具使用能力，但这也引入了检索诱导的安全退化问题——有害意图可能分解为看似无害的子查询，最终导致不安全结果。现有对齐方法难以捕获稀疏的安全信号，也无法对多步交互中的各种违规行为进行有效监督。

Method: COMPASS 框架包含两个核心模块：(1) 认知树探索（CTE），高效合成隐蔽攻击轨迹，模拟有害意图分解为看似无害子查询的过程；(2) 自省式逐步对齐（ISA），隔离风险中间动作，对每一步进行细粒度过程监督，实现安全对齐。

Result: 实验结果表明，COMPASS 在安全性与通用能力之间取得了良好的权衡，同时所需的训练数据量大幅减少。

Conclusion: COMPASS 通过认知 MCTS 引导的过程对齐，有效解决了 LLM 搜索智能体的检索诱导安全退化问题，为安全对齐提供了一种数据高效的解决方案。

Classification reason: 该论文关注 LLM 搜索智能体在多步推理中的安全对齐问题，属于 AIGC 方向下的生成内容安全与治理（aigc_safety_governance）范畴，同时涉及多步交互中的安全对齐，也与多模态理解与生成方向下的多模态安全与攻防（multimodal_safety_attack）相关。

Importance reason: Focuses on LLM agent safety alignment, not on user's priority subtopics like multimodal generation or autoregressive video. Signals: LLM search agent safety, process alignment, MCTS-guided, not multimodal, not generation
...
### [122] [MosaicLeaks:Privacy Risks in Querying-in-the-Open for Deep Research Agents](https://arxiv.org/abs/2605.30727)
...
TL;DR: 本文提出MosaicLeaks基准，用于评估深度研究智能体在结合私有文档与外部检索时产生的隐私泄露风险（马赛克效应）。实验发现多种模型普遍存在隐私泄露，零样本提示缓解效果有限，任务导向的强化学习反而加剧泄露。作者提出PA-DR框架，结合任务奖励与隐私分类器进行密集信用分配，在Qwen3-4B-Instruct上将准确率从48.7%提升至58.7%，并将答案泄露从34.0%降至9.9%。


<details>
  <summary>Details</summary>
Motivation: 深度研究智能体越来越多地结合私有本地文档与外部工具（如网络检索），这带来了隐私风险：智能体的外部查询可能泄露本地上下文中的敏感信息。这种风险被"马赛克效应"放大——单个查询看似无害，但聚合起来可能暴露大量信息。目前缺乏系统评估此类隐私泄露的基准和缓解方法。

Method: 1) 构建MosaicLeaks基准：包含1,001个多跳深度研究任务，将私有企业文档与公共网络语料链接，迫使智能体做出依赖本地信息的外部查询。2) 设计三层次泄露评估：研究意图、具体问题答案、可验证声明。3) 提出PA-DR（Privacy-Aware Deep Research）框架：结合任务成功的情境奖励与学习到的隐私分类器，对每个查询和马赛克级泄露进行密集信用分配，通过强化学习训练模型。

Result: 多种模型在三个层次上普遍存在隐私泄露；零样本隐私提示能减少但无法消除泄露；仅以任务性能为目标的强化学习反而加剧泄露。PA-DR训练Qwen3-4B-Instruct后，准确率从48.7%提升至58.7%，答案泄露从34.0%降至9.9%，完整信息泄露从34.0%降至9.9%。

Conclusion: 深度研究智能体的隐私泄露是一个严重且尚未充分解决的问题。PA-DR框架通过将隐私约束纳入强化学习训练，在保持任务性能的同时显著降低隐私泄露，为构建更安全的AI智能体提供了有效方向。

Classification reason: 该论文研究深度研究智能体在查询外部工具时泄露私有信息的隐私风险，属于AIGC安全与治理范畴。论文提出了MosaicLeaks基准和PA-DR框架来检测和缓解隐私泄露，与[CANONICAL] subtopic "生成内容安全与治理"（aigc_safety_governance）高度匹配。

Importance reason: Privacy risk in deep research agents is a timely topic, but it falls outside the user's priority subtopics and is flagged by negative keywords. Signals: privacy leakage, deep research agents, mosaic effect, RL safety alignment, benchmark
...
TL;DR: 本文对六种大语言模型（Claude、GPT、Gemini、DeepSeek、Syn-Pro、HyperCLOVA X）在英语、韩语、中文和日语四种语言中的性别刻板印象进行了跨语言审计。通过HEXACO-100人格量表锚定跨文化人类基线，发现LLM的性别刻板印象波动范围约为人类跨国差异的2.5倍，且跨语言效应会叠加。作者提出了四模式框架（一致、抑制、重组、放大）来刻画模型行为，并指出翻译不仅改变刻板印象强度，还会改变与之关联的属性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM偏见研究通常仅检测模型是否存在偏见，但缺乏与人类基线（尤其是目标语言所在文化的人类基线）的对比，且跨语言场景下的偏见漂移尚未被系统研究。

Method: 采用HEXACO-100人格量表作为评估工具，将模型输出锚定到覆盖48个国家的跨文化人类数据集上。对6个LLM在4种语言下进行系统性审计，引入四模式框架（concordance, suppression, reorganization, amplification）对24个（模型×语言）单元进行分类，并进行项目级别的细粒度分析。

Result: LLM的性别刻板印象波动范围约为人类跨国差异的2.5倍；跨语言效应会叠加，例如一个英语中心模型在韩语提示下达到当地基线5倍的偏见水平；翻译不仅改变刻板印象强度，还会改变与之关联的具体人格特质。

Conclusion: 没有单一的偏见消除流程能够均匀地解决跨语言边界的偏见问题，不同语言和文化背景下需要针对性的去偏策略。

Classification reason: 该论文审计了多个LLM中的性别刻板印象偏见，属于AIGC治理与公平性范畴，与[CANONICAL] subtopic "生成内容安全与治理"（aigc_safety_governance）高度匹配。

Importance reason: Focuses on LLM gender bias cross-lingual audit, not aligned with user's priority on multimodal generation, video/image generation, or model efficiency. Signals: LLM gender bias, cross-lingual audit, HEXACO-100 anchoring, four-pattern framework, not multimodal generation
...
TL;DR: 本文提出一种针对检索增强文生音乐（TTM）系统的字幕投毒攻击方法。攻击者通过向音乐知识库注入少量精心构造的音乐字幕，使系统检索到恶意字幕，从而偏置提示增强过程，使生成结果偏离用户意图。该方法采用双层字幕投毒策略，保留高层检索锚点的同时注入低层声学描述符，实现对生成内容的定向操控。实验表明，在MusicCaps数据库、CLAP检索器和MusicGen生成流水线上，投毒后的生成结果显著偏向攻击者目标，同时仍与原始用户查询保持一定对齐。


<details>
  <summary>Details</summary>
Motivation: 检索增强文生音乐系统依赖于音乐知识库中字幕的完整性，但现有研究未充分探索这种依赖关系带来的安全风险。作者旨在揭示攻击者可以通过污染数据库中的少量字幕来操控生成结果，从而暴露检索增强型创意AI系统的实际完整性风险。

Method: 提出双层字幕投毒策略（dual-layer caption poisoning）：第一层保留高层检索锚点（如音乐风格、流派等关键语义信息），确保恶意字幕能被检索器召回；第二层注入低层声学描述符（如节奏、音色、乐器等），以偏置提示增强和下游音乐生成朝向攻击者选择的目标意图。攻击者无需修改用户提示、检索器或生成器。

Result: 在MusicCaps知识库、CLAP检索器和MusicGen生成流水线上，投毒后的生成结果在声学特征上显著接近攻击者目标，同时与原始用户查询的对齐程度仍保持在可比水平，验证了攻击的有效性和隐蔽性。

Conclusion: 检索增强文生音乐系统存在实际的安全隐患，少量精心构造的恶意字幕即可有效偏置生成结果。该工作揭示了检索增强型创意AI系统在数据完整性方面的脆弱性，呼吁社区关注此类安全风险并开发相应的防御机制。

Classification reason: 该论文研究检索增强文生音乐系统中的字幕投毒攻击，属于AIGC安全与治理范畴。aigc_safety_governance（生成内容安全与治理）是[CANONICAL]子主题，涵盖内容安全、有害内容检测等，与本论文的攻击与安全风险分析高度匹配。

Importance reason: Focuses on security of retrieval-augmented music generation, which is outside the user's core interests in multimodal generation and autoregressive video/image generation. Signals: Novel attack on TTM, dual-layer caption poisoning, security risk for creative AI, not aligned with user's priority subtopics
...
Main category: cs.CR, cs.CV, cs.LG
...
TL;DR: 本文提出 Latent Geometric Chords (LGC)，一种基于决策的黑盒对抗攻击方法。LGC 在压缩语义流形上执行曲率感知的几何搜索，并引入 Residual-based Adversarial Generation (RAG) 机制将语义扰动作为几何弦叠加到原始图像上，解决了像素级攻击产生高频伪影和潜空间方法搜索空间受限的问题。实验表明 LGC 在极低扰动（SSIM>0.99, LPIPS<0.01）下实现高攻击成功率，并具有跨数据集迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有决策型黑盒对抗攻击存在两大局限：像素级攻击引入不自然的高频伪影，潜空间方法受限于低维流形的搜索空间和重建缺陷。需要一种既能保持高视觉保真度又能有效搜索对抗样本的方法。

Method: 提出 Latent Geometric Chords (LGC) 方法，核心包括：(1) 在压缩语义流形上执行曲率感知的几何搜索来导航决策边界；(2) Residual-based Adversarial Generation (RAG) 机制，将语义扰动作为几何弦分离出来并直接叠加到原始图像上，解决重建缺陷并有效加倍可搜索空间维度。

Result: LGC 在 5000 次查询下达到 SSIM>0.99、LPIPS<0.01 的视觉保真度，同时保持高攻击成功率，能够成功攻破对抗训练过的鲁棒模型，并展现出跨数据集迁移能力，显著优于现有基线方法。

Conclusion: LGC 通过几何感知的语义流形搜索和残差对抗生成机制，有效解决了决策型黑盒攻击中视觉质量与攻击效果之间的权衡问题，为对抗攻击领域提供了新的技术路径。
...
Importance reason: Focuses on adversarial attack methodology rather than generative model advancement or multimodal understanding. Signals: adversarial attack, black-box attack, geometric search, visual fidelity, query efficiency
...
TL;DR: 本文对Wu等人(2025)在AxBench基准上的结论提出部分反驳，证明稀疏自编码器（SAE）在LLM模型引导任务上可以取得接近LoRA的性能，关键在于使用监督式特征选择与标注流程。同时发现高稀疏性对基于可解释性的引导并非必要。


<details>
  <summary>Details</summary>
Motivation: 先前研究（Wu et al., 2025）在AxBench基准上发现SAE在模型引导任务上表现不如简单基线方法，导致SAE在可解释性与可控生成方面的潜力受到质疑。本文旨在重新评估SAE的引导能力，证明其实际性能被低估。

Method: 提出一个监督式特征选择与标注流程：首先利用SAE提取LLM内部特征，然后通过可解释性组件筛选与目标标签因果相关的特征，最后使用这些特征进行模型输出引导。在AxBench基准上评估引导性能，并与LoRA及简单基线方法对比。

Result: SAE在AxBench上的引导性能接近LoRA水平，显著优于简单基线。基于可解释性选出的特征具有令人惊讶的因果性。高稀疏性（低l0）对基于可解释性的引导并非关键因素，这与Wang et al. (2025)的发现相反。

Conclusion: SAE在LLM引导任务上具有被低估的潜力，通过合适的特征选择与标注方法可以达到接近LoRA的性能。这为SAE在模型安全、可控生成等领域的应用提供了新的支持。

Classification reason: 该论文研究使用稀疏自编码器（SAE）对LLM进行模型引导（steering），属于生成内容安全与治理范畴下的模型行为控制与安全生成方向。现有aigc_safety_governance（生成内容安全与治理）子主题涵盖内容审核、有害内容检测、安全生成等，与本文的模型输出引导与安全控制高度相关。

Importance reason: Focuses on LLM steering with SAEs, not aligned with user's priority on multimodal generation and video/image generation. Signals: Sparse autoencoders, LLM steering, AxBench benchmark, supervised feature selection, interpretability
...
TL;DR: 本文提出 AdvScene，一个基于场景重建的对抗性补丁鲁棒性评估框架。通过将补丁嵌入到重建的真实3D场景中，系统性地评估补丁在不同视角、距离和场景条件下的攻击有效性（即"场景鲁棒性"），并引入 APSE（对抗性补丁到场景嵌入）方法解决跨视角外观一致性问题。实验表明现有评估方法无法捕捉的场景依赖性变化。


<details>
  <summary>Details</summary>
Motivation: 现有对抗性补丁评估方法存在局限：真实图像基准固定不变，无法覆盖多种场景条件；模拟器虽可控但缺乏真实场景的锚定。对抗性补丁的实际风险取决于其在部署后不同视角、距离和场景条件下的持续有效性（即场景鲁棒性），但现有评估未能有效衡量这一属性。

Method: 提出 AdvScene 框架，将评估重新定义为操作测量：给定一个固定部署的补丁，刻画其"操作包络"（operational envelope）——攻击在哪些视角和距离下成功。核心挑战是补丁通常只在单个锚定视角定义，而评估需要跨视角保持外观一致性。为此提出 APSE（Adversarial Patch-to-Scene Embedding），通过约束提升（constrained lifting）解决跨视角歧义，同时保留攻击关键外观、强制局部性、目标表面附着和跨视角一致性。

Result: 使用真实世界物理数据验证 AdvScene，并对现有对抗性补丁进行全面评估。结果表明，攻击有效性存在显著的场景依赖性变化，这些变化无法被现有的基于图像或基于模拟器的评估方法捕捉。

Conclusion: AdvScene 提供了一种更贴近真实部署场景的对抗性补丁评估方法，揭示了场景鲁棒性作为衡量对抗性补丁实际风险的关键维度，为未来对抗性攻击与防御研究提供了更可靠的评估基准。

Classification reason: 该论文研究对抗性补丁（adversarial patches）在真实场景下的鲁棒性评估框架，属于AI安全与对抗攻击评估范畴。AIGC方向下的[CANONICAL] subtopic "生成内容安全与治理"（aigc_safety_governance）涵盖内容安全、有害内容检测等，对抗性补丁评估是AI安全治理的重要组成部分，因此归入该方向。

Importance reason: Focuses on adversarial patch evaluation in 3D scenes, which is not aligned with user's priority subtopics on generative models and efficient inference. Signals: adversarial patch robustness, scene-grounded evaluation, cross-view consistency, not generative model, not user priority
...
TL;DR: EvoDefense 提出一种经验引导的共进化黑盒防御范式，利用守卫LLM检测恶意查询，并通过攻击-防御持续进化循环，在不重训练的情况下泛化到未见攻击类型和目标模型，在HarmBench等基准上显著降低攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒防御方法通常依赖预定义的过滤启发式规则，难以泛化到未见过的攻击类型和目标模型架构，亟需一种能自适应进化的通用防御范式。

Method: EvoDefense 包含三个核心组件：一个守卫LLM用于检测恶意查询；一个经验记忆模块用于积累防御知识；以及一个攻击-防御共进化循环，其中攻击生成器与守卫模型通过经验引导的优化迭代改进攻击策略和防御策略。
...
Conclusion: EvoDefense通过共进化范式实现了对未见攻击和未见目标模型的泛化防御能力，同时保持了良好的通用能力，为LLM黑盒防御提供了新的自适应解决方案。

Classification reason: 该论文提出针对LLM黑盒攻击的防御方法，属于生成内容安全与治理范畴，与AIGC方向下的[CANONICAL]子主题"生成内容安全与治理"（aigc_safety_governance）高度匹配。

Importance reason: Focuses on LLM security defense, not aligned with user's priority subtopics in multimodal generation, video/image generation, or model efficiency. Signals: LLM security, black-box defense, co-evolution, guard LLM, not multimodal generation
...
TL;DR: 本文提出 SCOUT 框架，将提示注入防御重新定义为检测器分配问题：根据每个检测器在相似历史输入上的表现，动态决定对当前请求运行哪些检测器以及是否升级到 LLM 裁判，从而在安全性和实用性之间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有提示注入检测器各有优劣，没有一个检测器在所有攻击类型上都可靠，但现有系统仍采用固定单检测器流水线，导致每个请求都会落入某个检测器的盲区。作者希望将防御从固定流水线转变为动态分配，根据每个样本自适应选择最合适的检测器组合。

Method: SCOUT 框架通过预测每个检测器在给定样本上的可靠性和延迟（基于该样本与历史输入的相似性），动态决定运行哪些检测器以及是否升级到 LLM 裁判。系统暴露一个安全-效用阈值供操作员调节。同时构建了 SCOUT-450 基准，包含结构复杂、面向智能体的提示注入样本。

Result: 在 SCOUT-450 上，安全导向的操作点相比始终使用 GPT-4o 裁判，攻击成功率降低 46%，总延迟降低 40%，仅牺牲 5.1 个百分点的良性效用。在 BIPIA、IPI 和 IHEval 三个外部基准上也改进了安全-效用前沿。

Conclusion: 动态检测器分配是一种有效的提示注入防御范式，SCOUT 通过预判每个检测器的可靠性实现了更好的安全-效用权衡，且提供了可调节的安全阈值，具有良好的实用性和迁移性。

Classification reason: 该论文研究提示注入攻击的防御策略，属于生成内容安全与治理范畴，与 canonical 子主题 aigc_safety_governance（生成内容安全与治理）高度匹配。

Importance reason: Focuses on prompt-injection defense, which is outside the user's priority subtopics and negative keywords. Signals: Prompt-injection defense, Detector allocation, Safety-utility trade-off, Benchmark SCOUT-450, Not aligned with user's core interests
...
TL;DR: 本文提出可配置安全奖励模型（CSRM），通过配置目标数据增强和联合优化，实现对细粒度安全配置的敏感性和泛化能力，在 CoSApien 和 DynaBench 基准上达到 SOTA，并显著改善下游安全对齐中 helpfulness-safety 的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有指令微调 LLM 和安全分类器难以泛化到新的安全配置，缺乏可配置的奖励模型来适应异构且快速变化的安全需求。

Method: 提出 CSRM，联合优化校准的安全合规性和奖励建模；采用配置目标数据增强，强制指令遵循同时保留相对严重性结构。

Result: 在 CoSApien 上达到 94.6% F1，DynaBench 上达到 75.8% F1，无需额外人工标注；下游安全对齐中显著改善 helpfulness-safety 权衡。

Conclusion: CSRM 实现了对未见安全配置的良好泛化，为 LLM 安全对齐提供了一种可配置、高效的奖励建模方案。

Classification reason: 该论文提出可配置的安全奖励模型（CSRM），用于大语言模型的安全对齐，核心贡献在于奖励建模与安全对齐。该主题与 [CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配，该 subtopic 在 aigc、multimodal_generation、multimodal_understanding_generation 三个方向下均有收录，因此选择该已有 subtopic。

Importance reason: Focuses on safety alignment reward models, not directly on the user's priority subtopics like continuous VLM, video generation, or world models. Signals: Safety alignment, reward model, helpfulness-safety tradeoff, configurable safety, LLM alignment
...
TL;DR: 本文比较了程序化生成的合成音效与真实录音在真人实拍和动画两种视觉场景中的可信度感知。实验表明，合成音效在剧情片和科幻片中效果良好（如激光、撞击、空气、火箭等音效模型），但在卡通动画中表现日常动作时可信度不足。研究还识别了需要优化的音效模型和音频特征，并收集了音频专业人士的反馈。


<details>
  <summary>Details</summary>
Motivation: 影视和游戏中的音效制作通常需要重复录制、分层处理或使用音效库，耗时且重复性高。程序化音频（数字拟音）可以快速生成样本，但合成样本相比真实样本的可信度（believability）尚不明确，需要系统评估。

Method: 使用在线程序化音频引擎生成合成音效样本，将其分别与真人实拍和动画两种视觉内容结合，通过主观听感实验比较合成音效与真实录音在不同场景类型（剧情片、科幻片、卡通动画）中的可信度感知，并收集音频专业人士的定性反馈。

Result: 程序化音频在剧情片和科幻片场景中效果良好且被认为可信，尤其是激光、撞击、空气和火箭等音效模型；但在卡通动画中表现日常动作时，合成音效的可信度不足。研究还识别了需要优化的具体音效模型和音频特征。

Conclusion: 程序化音频在特定场景类型（剧情、科幻）和特定音效模型（激光、撞击等）中具有较高的可信度和实用性，但在卡通动画的日常动作音效方面仍需改进。研究为程序化音频在媒体制作中的实际应用提供了有价值的指导。

Classification reason: 该论文研究程序化音频（数字拟音）在影视和动画中的合成音效与真实录音的对比感知，属于AIGC方向下的音视频内容创作应用。现有[CANONICAL] subtopic "视频音频生成应用"（aigc_video_audio_creation）涵盖视频后期、音视频编辑、内容创作产品级生成应用，与本论文主题高度匹配。

Importance reason: Focuses on sound effects believability, not on the user's priority subtopics like continuous VLM, autoregressive video generation, or world models. Signals: Sound effects, procedural audio, comparative analysis, not core to user's multimodal generation or video generation priorities
...
TL;DR: AnchorSteer 提出一种自发现概念注入框架，通过结构锚定与语义引导解耦，实现保留节奏和旋律结构的可控音乐编辑。


<details>
  <summary>Details</summary>
Motivation: 可控音乐编辑面临语义-结构纠缠问题：编辑方法常破坏音乐结构，而结构适配器会抑制语义响应。现有方法难以同时实现语义变换和结构保持。

Method: 提出 AnchorSteer 框架，包含两个核心组件：(1) 自发现语义引导——通过自监督重建目标探测内部表征，提取可解释、无标签的概念向量；(2) 结构锚定——使用结构适配器在编辑过程中保持节奏和旋律一致性。提供无条件注入和有条件注入两种变体以平衡鲁棒性和语义强度。

Result: 在 ZoME-Bench 和主观测试上，AnchorSteer 优于纯引导和纯锚定基线，实现了显著的语义变换同时保持高保真结构。

Conclusion: AnchorSteer 通过解耦语义引导与结构保持，有效解决了音乐编辑中的语义-结构纠缠问题，为可控音乐编辑提供了新的框架思路。
...
Importance reason: Music editing is outside user's priority subtopics and negative keywords, and lacks direct connection to multimodal generation or video/image generation. Signals: music editing, diffusion-based, structure preservation, concept injection, self-supervised
...
TL;DR: 本文提出了一种生成式AI增强的用户体验研究方法论，基于UXR PoV Playbook，结合DBT、SDT和COM-B心理学框架，利用生成式AI作为协同分析工具，为ADHD成人设计神经包容性情绪调节数字干预系统。最终产出一套10张理论驱动的UXR Play Cards，将心理学机制转化为可操作的设计指南。


<details>
  <summary>Details</summary>
Motivation: 现有数字情绪调节干预系统缺乏理论整合、对神经多样性的包容不足，且缺少结构化的用户体验研究方法论来桥接心理学洞察与设计实践。

Method: 提出四阶段生成式AI增强的UXR流程：(1) AI支持的假设生成；(2) 基础规划；(3) 通过Building Blocks进行洞察生成；(4) 构建利益相关者特定的PoV叙事。利用生成式AI作为协同分析工具辅助综合、假设形成和设计表达，并产出10张理论驱动的UXR Play Cards。

Result: 产出一套10张理论驱动的UXR Play Cards，将DBT、SDT和COM-B等心理学机制与实证发现转化为可操作的设计指南，用于指导神经包容性情绪调节数字干预系统的设计。

Conclusion: 本文提供了一个可复现、具有偏差意识的框架，将生成式AI整合到UXR实践中，推进了以人为中心和神经包容性的数字心理健康设计方法。

Classification reason: 该论文将生成式AI应用于数字心理健康领域（ADHD情绪调节干预设计），属于垂直领域生成应用，与 canonical 子主题"领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: This paper focuses on neuroinclusive UX for emotion regulation, which is outside the user's core technical priorities in multimodal generation and efficient models. Signals: generative AI as co-analytic tool, UXR methodology, ADHD emotion regulation, DBT/SDT/COM-B frameworks, UXR Play Cards
...
### [134] [On-Device Generative AI for GDPR-Compliant Visual Monitoring: Natural Language Alerts from Local Object Detection](https://arxiv.org/abs/2605.30544)
...
TL;DR: 本文提出了一种符合GDPR数据最小化原则的隐私保护视觉监控系统，所有推理均在边缘设备（Raspberry Pi 5 + Hailo-8L加速器）上完成。YOLOv5n-seg进行实时目标检测后立即丢弃原始图像数据，状态触发引擎将最小化JSON事件传递给本地运行的Phi-3 Mini（3.8B参数，Q4_0量化），生成1-2句自然语言警报。整个过程中无图像数据离开设备，仅文本警报被传输。


<details>
  <summary>Details</summary>
Motivation: 基于云端的视觉监控系统将原始图像数据暴露给外部服务，与GDPR第5(1)(c)条的数据最小化原则存在根本性冲突。现有方案无法在保护隐私的同时提供可读的自然语言警报输出。

Method: 设计了一个隐私保护流水线：(1) YOLOv5n-seg模型编译到Hailo-8L AI加速器，在Raspberry Pi 5上实现实时目标检测；(2) 推理后立即丢弃原始像素缓冲区；(3) 状态触发引擎将最小化JSON事件负载转发给本地托管的Phi-3 Mini（3.8B参数，Q4_0量化）；(4) LLM生成1-2句自然语言警报供操作员阅读。整个过程中无图像数据跨越网络边界。

Result: 在目标硬件上测量了推理延迟和资源利用率，并展示了代表性生成的警报文本。结果表明，在单板计算机上结合专用神经网络加速器与本地大语言模型不仅是可行的，而且能产生实际可部署、可读的监控输出。

Conclusion: 该概念验证系统证明，通过将全部推理限制在边缘设备上，可以在不牺牲功能性的前提下实现GDPR合规的视觉监控。边缘AI加速器与本地LLM的组合为隐私保护监控提供了实用且可部署的解决方案。

Classification reason: 该论文构建了一个边缘设备上的隐私保护视觉监控系统，使用YOLOv5n进行本地目标检测，并通过本地LLM（Phi-3 Mini）生成自然语言警报。这属于垂直领域（监控/安全）的生成应用，与aigc_domain_application（领域内容生成应用）高度匹配。

Importance reason: Focuses on GDPR compliance and edge deployment of small models, not on advancing core generative AI or multimodal understanding. Signals: Edge deployment, GDPR compliance, small model (Phi-3 Mini), no image data transmitted, YOLOv5n-seg + Phi-3
...
### [135] [Translation Analytics for Freelancers II: Benchmarking Local LLMs for Confidential Translation Workflows](https://arxiv.org/abs/2605.31452)
*Yuri Balashov, Rex VanHorn, Mingxi Xu, Austin Downes*

Main category: cs.HC, cs.CL
...
TL;DR: 本文为自由译者和中小型语言服务商开发了一套评估本地可运行LLM翻译质量的方法，扩展了多语种语料库（RFMC），在四个语言方向上对多个本地LLM进行基准测试，发现精心选择的本地LLM在保密翻译场景中具有可行性，性能可匹敌甚至超越本地NMT系统和前沿LLM，但仍落后于顶级商业NMT。


<details>
  <summary>Details</summary>
Motivation: 在保密性敏感的翻译场景中，云翻译引擎和商业LLM因隐私限制无法使用，自由译者和中小型语言服务商缺乏评估本地可运行LLM翻译质量的实用方法。

Method: 扩展Reeve Foundation Trilingual Corpus为多语种语料库（RFMC），新增德语和简体中文参考翻译；通过Ollama在四个语言方向上对多个本地LLM进行基准测试，使用一致的单一提示调用，无微调或领域适配；与商业NMT（DeepL、Baidu）、前沿LLM（GPT-5.2）和专业级本地NMT系统（OPUS-CAT、NeuralDesktop、Promt）对比；使用MATEO进行自动评估。

Result: 本地LLM在不同语言方向和模型规模上表现差异显著；最佳本地LLM可匹敌或超越本地NMT系统和前沿LLM，但仍落后于顶级商业NMT。

Conclusion: 精心选择的本地LLM翻译对于隐私受限的专业翻译场景具有可行性，为未来模型扩展和多语言能力研究提供了参考。

Classification reason: 该论文研究面向保密翻译场景的本地LLM翻译基准测试，属于垂直领域（翻译/语言服务）的生成应用，与现有规范子主题"领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Focuses on translation analytics for freelancers, which is outside the user's priority subtopics and contains negative keywords like clinical. Signals: Low priority subtopic match, negative keyword "clinical" in abstract, no overlap with boost keywords, domain-specific application not in user's focus
...
### [136] [ImmigrationQA: A Source-Grounded Dataset and Small-Model Adaptation for U.S. Immigration Law](https://arxiv.org/abs/2605.30589)
*Nazarii Shportun*
...
TL;DR: 本文构建了 ImmigrationQA 数据集（17,058 对问答，覆盖 13 个移民法子领域），基于 11 个官方和社区来源，使用 Claude Sonnet 生成结构化 QA 对，并对 Llama 3.2 3B 模型进行 LoRA 微调。微调模型在 101 例分层抽样评估中平均得分 1.08/3.0（完全正确率 16.8%），相比 Llama 3 8B 基线的 0.85/3.0（4%）提升 27%，但弱于 Claude Sonnet 零样本的 1.52/3.0（25%）。全部计算成本约 $29，所有资源已开源。


<details>
  <summary>Details</summary>
Motivation: 美国移民法涉及数千页官方政策、联邦法规和程序指南，频繁变更且对缺乏法律代理的申请人影响重大。现有通用 QA 系统难以覆盖这一高度专业化的领域，亟需一个来源可追溯、领域专精的法律问答数据集和适配模型。

Method: 从 11 个主要和次要来源（USCIS 政策手册、8 CFR、BIA 先例判决、社区 Q&A 等）收集语料，生成 10,056 份验证过的规范文档和 18,308 个文本块。使用 Claude Sonnet 4.6 通过五种模式特定提示从文本块生成结构化 QA 对，剔除 22 对来源跨度不足的样本，最终得到 17,058 对问答。使用 LoRA 对 Llama 3.2 3B Instruct 模型进行参数高效微调，在 993 对的保留集上评估，采用 LLM-as-judge 对 101 例分层样本评分。

Result: 微调模型平均得分 1.08/3.0（完全正确率 16.8%），相比 Llama 3 8B 基线的 0.85/3.0（4%）提升 27%；Claude Sonnet 零样本基线为 1.52/3.0（25%）。微调模型在程序性子领域（旅行证件、身份调整、非移民签证）表现集中提升，但在复杂法律推理和时效性统计方面仍较弱。全部计算成本约 $29。

Conclusion: 本文证明了使用参数高效微调（LoRA）将小型开源模型适配到专业法律领域的可行性和成本效益，但模型能力仍显著弱于大型闭源模型，尤其在复杂法律推理方面。数据集、模型、代码和提示模板已全部开源，但系统不能替代法律咨询，且不反映语料爬取日期后的法规变化。

Classification reason: 该论文构建了美国移民法律领域的问答数据集并微调模型，属于法律垂直领域的生成应用，与 [CANONICAL] 子主题 "领域内容生成应用" (aigc_domain_application) 高度匹配。

Importance reason: This paper is a domain-specific legal QA dataset and small-model fine-tuning study with no connection to the user's priority topics in multimodal generation, autoregressive video/image generation, or world models. Signals: domain-specific legal QA, small-model fine-tuning, no multimodal or generation focus, no priority subtopic keywords, negative keywords not present but topic irrelevant
...
### [137] [Cross-Modal Clinical Knowledge Integration for Mammography Report Generation](https://arxiv.org/abs/2605.31093)
...
TL;DR: 本文提出 MammoRG，一种模拟放射科医生临床报告流程的乳腺钼靶报告生成框架。它采用两阶段训练：第一阶段通过分类监督学习四视图钼靶影像的临床先验知识；第二阶段引入术语感知的监督微调，将乳腺影像专用临床术语建模为原子语义单元。同时开发 MammoRGTool 工具从自由文本报告中提取结构化临床信息。实验在多个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 乳腺钼靶筛查产生大量影像，给放射科医生带来沉重负担。现有自动报告生成方法主要关注直接的视觉到文本映射，忽略了放射科医生在实际临床工作中遵循的结构化临床推理过程，导致生成的报告临床一致性不足。

Method: 提出 MammoRG 框架，包含两阶段训练：(1) 第一阶段通过分类监督学习从患者四视图钼靶影像中整合临床相关先验知识；(2) 第二阶段采用术语感知的监督微调策略，将乳腺影像专用临床术语（如 BI-RADS 描述词）建模为原子语义单元，提升报告临床一致性。同时开发 MammoRGTool 报告解析工具，从自由文本中提取结构化临床信息用于评估。

Result: 在内部数据集、两个外部数据集和 VinDr-Mammo 数据集上，MammoRG 在多项临床疗效指标上持续优于现有方法。在诊断相关的 BI-RADS F1 指标上，分别超越第二名模型 2.73%、2.04%、1.90% 和 3.27%。

Conclusion: MammoRG 通过显式模拟临床报告工作流程并融入 BI-RADS 指南先验知识，有效提升了乳腺钼靶报告生成的临床一致性和诊断准确性，为医学影像报告生成提供了更贴近临床实践的新范式。

Classification reason: 该论文聚焦乳腺钼靶影像报告生成，属于医疗垂直领域的生成应用，与 [CANONICAL] 子主题 "领域内容生成应用" (aigc_domain_application) 高度匹配。

Importance reason: Focuses on medical report generation, which is outside the user's priority subtopics and has negative keywords. Signals: domain-specific medical application, no autoregressive or video generation, no continuous VLM or flow matching, no world model or efficient inference
...
TL;DR: 本文提出基于自然语言推理（NLI）的语义验证框架，用于筛选临床LLM中保持语义不变的提示改写，并引入三个指标（MVS、ΔC、WCI）量化模型对语义等价改写的敏感性。在16个开源通用和医学LLM上的实验表明，领域专业化并不一致地提升或降低鲁棒性，结果高度依赖具体模型。


<details>
  <summary>Details</summary>
Motivation: LLM在临床应用中，对语义等价的输入改写（如同义句、句法变化）表现出高度敏感性，这在安全关键的医疗场景中构成风险。现有基于嵌入相似度的度量无法有效捕捉否定、时间性、严重程度等语义关键差异，因此需要更可靠的语义验证框架来评估模型稳定性。

Method: 提出基于NLI的语义验证框架，通过NLI过滤保持语义的提示改写，再用LLM-as-a-judge和临床专家审核进一步精炼。引入三个指标：MVS（语义保持改写敏感性）、ΔC（置信度变化）、WCI（最差情况不稳定性）。在DiagnosisQA和MedQA数据集上评估16个开源通用和医学LLM。

Result: 领域专用（DS）模型与通用（GP）模型之间的鲁棒性差异是混合且高度模型依赖的：领域专业化并不一致地提升或降低对语义保持改写的鲁棒性；部分DS模型属于最鲁棒的，而强GP基线也保持竞争力。

Conclusion: 临床LLM对语义等价改写的鲁棒性不能简单归因于领域专业化，需要针对具体模型进行评估。提出的语义验证框架和敏感性指标为临床LLM的可靠性评估提供了实用工具。

Classification reason: 该论文研究临床LLM在语义等价改写下的稳定性与鲁棒性，属于医疗垂直领域的大语言模型应用评估，与AIGC方向下的[领域内容生成应用]（aigc_domain_application）高度匹配。

Importance reason: Focuses on clinical LLM robustness, which is outside your priority subtopics and has negative keyword overlap. Signals: clinical LLM, semantic stability, NLI verification, domain specialization, healthcare safety
...
### [139] [Astra: a generalizable report generation foundation model for 3D computed tomography](https://arxiv.org/abs/2605.31437)
...
TL;DR: Astra是一个基于90,678对胸腹部CT-报告数据训练的可泛化CT报告生成基础模型，通过报告风格统一和强化学习诊断一致性优化，在多个外部队列上实现了SOTA性能，并在临床工作流中显著提升了报告撰写效率。


<details>
  <summary>Details</summary>
Motivation: CT报告撰写耗时且高度依赖专家经验，现有自动报告生成方法缺乏支持多区域报告且跨外部真实队列鲁棒的可泛化基础模型。不同队列间报告风格和诊断术语的内在不一致性使得简单联合训练容易受到噪声文本监督的影响，限制了模型泛化能力。

Method: Astra首先构建了包含90,678对胸腹部CT-报告的大规模数据集CTRgDB（涵盖353,671个异常发现、8个器官系统）。通过报告风格统一（harmonization）处理跨队列风格差异，再使用强化学习进一步优化诊断一致性。模型基于视觉-语言架构，支持多区域报告生成。

Result: 在CTRgDB和6个外部队列上，Astra在细粒度诊断指标上平均提升44.1%（P<0.001）。在真实临床工作流中，Astra辅助使胸部报告起草速度提升29.6%，腹部报告完整性提升11.3%（P<0.001）。此外，Astra作为CT AI开发的基础模型，通过高质量报告合成提升了下游诊断性能和视觉-语言预训练。

Conclusion: Astra是一个可泛化的CT报告生成基础模型，在跨机构、跨解剖区域的报告生成中实现了风格一致且诊断准确的性能，可作为广泛可及的临床辅助工具和下一代AI医疗的基础设施。

Classification reason: 该论文聚焦于CT影像报告生成这一医疗垂直领域应用，属于AIGC方向下的"领域内容生成应用"（aigc_domain_application）子主题，该子主题明确包含医疗生成应用。同时论文涉及多模态（3D CT图像到文本报告）生成，也匹配多模态生成方向。

Importance reason: Focuses on medical report generation, which is outside the user's priority subtopics and has negative keyword overlap. Signals: Medical domain, CT report generation, clinical workflow, not aligned with user's multimodal generation or video/image generation priorities
...
### [140] [Beyond Accuracy: Evaluating Efficiency, Robustness and Explainability in Deep Learning for Malaria Diagnosis](https://arxiv.org/abs/2605.30734)
...
TL;DR: 本文在 NLM-Malaria 数据集上系统评估了四种深度学习模型在疟疾诊断中的预测性能、鲁棒性和可解释性，发现轻量级模型在性能上与重型模型相当，但所有 XAI 方法在图像损坏下均不可靠，为资源受限环境下的临床部署提供了重要参考。


<details>
  <summary>Details</summary>
Motivation: 疟疾是撒哈拉以南非洲的主要致死原因，诊断基础设施匮乏。深度学习有望实现自动化疟疾筛查，但计算成本和决策不透明性阻碍了临床采用。本文旨在系统评估不同模型在性能、鲁棒性和可解释性方面的权衡，为实际部署提供指导。

Method: 在 NLM-Malaria 数据集上对四种深度学习模型（涵盖轻量级到重型架构）进行基准测试，联合评估预测性能、鲁棒性（三种图像损坏类型）和事后可解释性（CAM 类方法和细粒度归因方法），并使用 Friedman 检验进行统计显著性分析。

Result: 轻量级高效模型与重型模型在预测性能上相当，Friedman 检验无显著差异。CAM 方法能一致定位诊断相关区域，而细粒度归因方法在重型骨干网络上产生较不聚焦的解释。模型置信度在损坏下比准确率下降更快，但所有 XAI 方法在临床可行的噪声水平下均不可靠。

Conclusion: 支持在资源受限环境中部署轻量级架构用于疟疾诊断，同时强调事后解释的脆弱性是负责任临床部署的重要考量因素。

Classification reason: 该论文研究深度学习模型在疟疾诊断（医疗领域）中的应用，属于垂直领域内容生成应用，映射到现有 canonical 子主题 aigc_domain_application（领域内容生成应用）。

Importance reason: Focuses on medical diagnosis with negative keywords, not aligned with user's priority subtopics. Signals: Medical diagnosis, negative keywords, no autoregressive/video/image generation, no continuous VLM, no world model
...
### [141] [Learning Whom to Trust: Market-Feedback Adaptive Retrieval for Frozen LLMs in Event-Driven Financial RAG](https://arxiv.org/abs/2605.31201)
...
<details>
  <summary>Details</summary>
Motivation: 金融RAG系统通常仅按文本相关性排序检索结果，但金融市场中不同事件类型、预测周期和市场环境下，有用的信息来源（如新闻 vs SEC文件）各不相同。现有方法忽略了这种动态性，需要一种能根据市场反馈自适应调整检索策略的方法。

Method: 提出Frozen Reader with Source Memory框架：保持LLM冻结，通过外部贝叶斯源记忆模块（Bayesian Source Memory）根据已实现的残差收益反馈更新每个检索来源（新闻、SEC文件、市场上下文）的权重。系统为每个公司-新闻锚点检索相关金融新闻和SEC文件段落，附加市场上下文卡片，预测多时间跨度的残差收益信号。

Result: 在FinRL-DeepSeek/FNSPID任务的89只纳斯达克股票数据集上，Frozen Reader with Source Memory将留出集macro-F1从0.438提升至0.471，下游投资组合夏普比率从0.52提升至0.84。监督LoRA微调阅读器对静态RAG有适度提升，但未超过冻结源记忆阅读器。

Conclusion: 对于金融RAG，学习从何处检索（学习信任谁）与学习如何阅读同等重要。所提出的源记忆方法提供了一种简单、模块化的市场反馈自适应方案，无需更新LLM参数即可显著提升性能。

Classification reason: 该论文研究金融领域的检索增强生成（RAG）系统，属于垂直领域内容生成应用，与 [CANONICAL] 子主题"领域内容生成应用"（aigc_domain_application）高度匹配。
...
### [142] [Wind Turbine Maintenance Log Labelling Framework: LLM-Driven Data Correction and Enrichment via Semantic Extraction of Reliability Intelligence](https://arxiv.org/abs/2605.31281)
...
Importance: 普通相关 (34.86) - 未入选精读

TL;DR: 本文提出一种基于LLM的框架，用于自动标准化和结构化风力涡轮机维护日志中的非结构化自然语言描述，校正层级系统编码错误，提取维护动作和故障模式的实证分类体系，将大规模定性现场观测转化为定量可靠性指标。


<details>
  <summary>Details</summary>
Motivation: 风力涡轮机维护日志中的故障事件描述通常为非结构化自然语言，难以用于定量可靠性分析。现有手动故障模式与影响分析(FMEA)主观性强且成本高，亟需一种可扩展、自动化的方法将大量定性现场观测转化为定量可靠性指标。

Method: 提出一个模型无关的LLM驱动流水线：(1) 基于自由文本描述自动校正层级系统编码；(2) 提取维护动作和故障模式的实证分类体系；(3) 利用基于系统的日志批次构建故障模式、可观察症状、主导机制和候选原因的经验字典。在280台涡轮机9年间的16,316条维护日志上验证。

Result: 自动化流水线成功结构化超过70%的数据集，解决了普遍存在的分类错误问题（如隔离先前未分类的桨距系统故障、恢复缺失的系统编码），并通过应用实证分类体系丰富了记录。

Conclusion: 该方法为将大规模定性现场观测转化为定量可靠性指标提供了高度可扩展、低成本的蓝图，为可再生能源领域的集成根因分析、改进FMEA和高级预测性维护奠定了基础。

Classification reason: 该论文利用LLM对风力涡轮机维护日志进行结构化标注、数据校正与语义增强，属于垂直工业领域的AI生成内容应用，与现有[CANONICAL]子主题"领域内容生成应用"高度匹配。

Importance reason: This paper applies LLMs to wind turbine maintenance, which is outside the user's core interests in multimodal generation, autoregressive video/image generation, and world models. Signals: domain-specific application, no multimodal generation, no autoregressive generation, no world model, no continuous VLM
...
TL;DR: 本文首次将LLM生成的目标端释义增强和LLM-as-a-Judge评估应用于手语翻译（SLT）。使用GPT-4o生成参考句子的受控释义变体，在Signformer风格的姿态Transformer上采用两阶段训练（预训练+微调）。在PHOENIX14T上BLEU-4从9.56提升至10.33，但在GSL和LSA-T数据集上揭示了方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 手语翻译受限于配对的手语视频/文本语料库规模小且目标词汇存在长尾分布，数据稀缺是主要瓶颈。

Method: 使用GPT-4o生成参考句子的受控释义变体（保持手语输入不变），构建增强语料库；采用Signformer风格的姿态Transformer，两阶段训练：先在增强语料上预训练，再在原始参考句子上微调；使用LLM-as-a-Judge进行语义评估。

Result: 在PHOENIX14T上BLEU-4从9.56提升至10.33；GSL基线已接近饱和，LSA-T极度稀疏，两者均未获得显著提升；语义评估显示忠实度提升被词汇重叠指标低估。

Conclusion: LLM生成的目标端释义增强对中等词汇多样性的手语翻译数据集有效，但对近饱和或极度稀疏场景效果有限；语义评估揭示了传统BLEU指标的不足。

Classification reason: 该论文研究手语翻译（SLT），属于垂直领域内容生成应用。手语翻译是语言翻译的一个特殊子领域，使用LLM进行目标端释义增强来提升翻译质量，符合"领域内容生成应用"（aigc_domain_application）的定义。该子主题在aigc和multimodal_generation方向下均有收录。

Importance reason: Focuses on sign language translation, which is outside all priority subtopics and has no overlap with boost keywords. Signals: Sign language translation, LLM paraphrase augmentation, LLM-as-a-Judge, low BLEU gains, domain-specific application
...
TL;DR: 本文提出一个面向多语言（英语、印地语、旁遮普语）骨科临床叙述文本的可靠性导向分类框架，通过领域自适应编码器 IndicBERT-HPA 结合语言感知适配头，并引入确定性选择性验证层（置信度门控、证据一致性检查、语言风险筛查），在自然临床分布下达到 0.8792 Macro-F1，选择性验证在 72.3% 覆盖率下达到 84.4% 准确率。


<details>
  <summary>Details</summary>
Motivation: 多语言骨科临床决策支持在低资源医疗场景中面临专业术语、混合脚本、不完整证据、标签不平衡和语言依赖的文档模式等挑战，现有方法缺乏可靠性保障。

Method: 对比了任务对齐的多语言Transformer编码器、DistilBERT基线、零样本指令微调LLM和领域自适应编码器IndicBERT-HPA。IndicBERT-HPA在IndicBERT基础上增加语言感知的骨科适配头。进一步实现了一个确定性选择性验证层，结合置信度门控、证据一致性检查和语言风险筛查，支持显式弃权（deferral）。

Result: 在自然临床分布下，IndicBERT-HPA达到平均Macro-F1 0.8792、Macro-AUROC 0.894、AUPRC 0.902。零样本LLM在封闭集分类上远不如任务适配编码器。选择性验证层在5000条随机保留子集上达到84.4%选择性准确率和0.76选择性Macro-F1（72.3%覆盖率），优于全接受基线（71.5%准确率，0.65 Macro-F1）。

Conclusion: 任务适配的领域编码器在多语言骨科分类中显著优于零样本LLM，结合选择性验证的弃权机制能有效提升临床决策支持的可靠性，为低资源多语言医疗场景提供了实用方案。

Classification reason: 该论文聚焦于多语言骨科临床决策支持，属于医疗垂直领域的AI应用，与现有 canonical 子主题"领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Focuses on multilingual clinical NLP for orthopedics, far outside the user's core interests in multimodal generation, autoregressive video/image generation, and world models. Signals: Multilingual clinical NLP, Orthopedic decision support, Selective verification, Low-resource healthcare, No multimodal or generative focus
...
### [145] [GSAM: A Generalizable and Safe Robotic Framework for Articulated Object Manipulation](https://arxiv.org/abs/2605.30740)
...
TL;DR: GSAM是一个面向铰接物体操作的通用且安全的机器人框架，通过视觉感知器生成运动学参数、VLM基于常识推理进行精炼、LLM将交互约束函数化并用于轨迹规划，在50个铰链任务上相比最优基线将操作成功率提升36.0%。


<details>
  <summary>Details</summary>
Motivation: 现有铰接物体操作方法往往忽视物体多样性和末端执行器与手柄之间交互的复杂性，导致泛化能力有限且容易发生破坏性碰撞。

Method: 提出GSAM框架，包含三个核心模块：(1) 基于视觉的感知器生成运动学参数；(2) 微调VLM结合链式思维常识推理对感知结果进行精炼；(3) 交互约束函数生成器整合铰接物体、交互姿态和避障知识，LLM将其函数化并用于轨迹与姿态规划，最后由运动学感知的操作规划器验证可达性。

Result: 在5类物体共50个铰链任务和50个随机初始化的末端执行器-手柄配置上，GSAM相比最优基线将标准差降低3.1%，操作成功率提升36.0%。

Conclusion: GSAM通过结合视觉感知、VLM常识推理和LLM约束规划，有效提升了铰接物体操作的泛化能力和安全性，在实际场景中表现出优越性能。

Classification reason: 该论文提出面向铰接物体操作的机器人框架GSAM，属于机器人操作这一垂直领域应用。虽然论文涉及VLM/LLM推理，但其核心是机器人操作任务，映射到AIGC方向下的"领域内容生成应用"（aigc_domain_application）最为合适，该子主题涵盖垂直领域生成应用。

Importance reason: Focuses on robotic manipulation, not aligned with user's priority subtopics in multimodal generation, video/image generation, or model efficiency. Signals: Robotics, articulated object manipulation, VLM/LLM for robotics, safety in manipulation, not in priority subtopics
...
### [146] [Improving Small Language Models for Code Generation with Reinforcement Learning from Verification Feedback](https://arxiv.org/abs/2605.30478)
...
TL;DR: 本文系统研究了使用可验证奖励的强化学习（RLVR）提升小语言模型（Qwen3-0.6B 和 Llama3.2-1B）的 Python 代码生成能力。通过对比单元测试奖励、静态分析（Ruff linter）奖励以及组合奖励，结合 GRPO 和 GSPO 优化方法，发现组合奖励在 MBPP 测试集上将 pass@1 提升最多 13 个百分点，但纯静态分析奖励会导致模型偏向生成更短代码而牺牲功能正确性。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在代码生成任务上性能有限，而强化学习（尤其是可验证奖励 RLVR）已被证明能提升大模型的代码生成能力，但在小模型上的效果尚缺乏系统研究。本文旨在填补这一空白，探索不同奖励设计和优化方法对小模型代码生成的影响。

Method: 使用 Qwen3-0.6B 和 Llama3.2-1B 两个小模型，在 MBPP 基准上进行 LoRA 微调。设计了三种奖励方案：纯单元测试奖励、纯静态分析（Ruff linter）惩罚奖励、以及两者组合奖励。采用 GRPO（Group-based Policy Optimization）和 GSPO（Group-based Supervised Policy Optimization）两种优化方法，通过 pass@1、生成长度、Ruff 严重度分布和执行错误类型等多维度指标评估效果。

Result: 组合奖励配置下，MBPP 测试集 pass@1 提升最多 13 个百分点。纯静态分析奖励会导致模型生成更短的代码以减少 lint 错误，但功能正确性并未可靠提升。组合奖励能缓解这种退化，在正确性和代码风格之间取得更稳定的平衡。

Conclusion: RLVR 对小模型代码生成的有效性高度依赖于奖励设计和优化粒度；仅靠 pass@1 不足以全面评估模型行为变化，需要结合生成长度、lint 错误分布和执行错误类型等诊断指标来识别失败模式。

Classification reason: 该论文研究使用强化学习（RLVR）提升小语言模型的代码生成能力，属于代码生成这一垂直领域应用。现有 taxonomy 中 aigc_domain_application（领域内容生成应用）明确包含代码生成，且该 subtopic 在 aigc 和 multimodal_generation 两个方向下均有定义，是最佳匹配。

Importance reason: Focuses on code generation for small models, which is not a priority subtopic and lacks alignment with user's multimodal generation and video/image generation interests. Signals: code generation, small language models, reinforcement learning, verification feedback, not multimodal
...
### [147] [A Padding Method for Enhanced Encoding of Inorganic Structures with Varying Chemical Compositions](https://arxiv.org/abs/2605.30743)
...
TL;DR: 本文提出一种利用晶体对称性（Wyckoff 位置长度感知填充）增强无机材料编码的填充方法，结合生成模型与机器学习势模型，构建端到端系统以生成稳定且新颖的无机材料结构，在质子导体数据上重建精度提升5.3%，在perov-5数据集上生成的新颖稳定材料比基线多63.5%。


<details>
  <summary>Details</summary>
Motivation: 无机材料的结构多样性和化学组成复杂性使得传统生成模型在准确性和效率上存在局限，亟需创新的AI驱动方法来提升无机材料生成的质量和稳定性。

Method: 提出一种基于晶体对称性（Wyckoff 位置）的填充编码技术，将对称性感知的填充策略集成到编码器架构中，获得更鲁棒的无机材料表示；并构建端到端系统，将生成模型与机器学习势模型（用于稳定性分析）相结合，实现从初始数据到验证输出的全流程自动化。

Result: 在质子导体数据上重建精度提升5.3%；在perov-5数据集上生成的新颖稳定无机材料数量比基线模型多63.5%。

Conclusion: 利用晶体对称性信息增强编码能够显著提升无机材料生成模型的精度和稳定性，为自动化材料发现与设计提供了有效的新范式。

Classification reason: 该论文关注无机材料结构的生成式建模与编码，属于科学/材料领域的垂直生成应用，与 [CANONICAL] 子主题 "领域内容生成应用" (aigc_domain_application) 高度匹配。

Importance reason: Focuses on inorganic material generation, which is outside the user's priority subtopics and is flagged by negative keywords. Signals: Domain-specific material science, No overlap with priority subtopics, Negative keyword match (molecule, drug discovery)
...
### [148] [Knowledge Boundary Probing and Demand-Guided Intervention for LLM-Based Power System Code Generation](https://arxiv.org/abs/2605.31478)
*Hui Wu, Xiaoyang Wang, Zhong Fan*

Main category: cs.CL, cs.SE, eess.SY
...
TL;DR: 本文提出 PowerCodeBench 基准和知识边界感知干预方法，用于提升 LLM 在电力系统代码生成中的可靠性。通过探测模型对 pandapower API 的知识边界，结合查询端 API 需求估计与主动文档注入、路由纠正策略，在 2000 个任务上使 7B 以上开源模型和商业 API 提升 32-56 个准确率点，无需微调或云端推理即可实现可靠的本地部署。


<details>
  <summary>Details</summary>
Motivation: 电力系统分析需要本地部署 LLM 以保证机密性、合规性和可复现性，但开源模型在生成电力系统仿真代码时存在 API 知识边界错误（如幻觉函数名、参数误用、结果表处理错误），亟需一种无需微调即可提升本地部署模型可靠性的方法。

Method: 1) 构建 PowerCodeBench：将自然语言操作员查询与 pandapower 代码及数值真值配对的执行验证基准；2) 设计 L0-L3 文档驱动探测流程，测量每个模型的 API 知识分布；3) 提出边界感知干预方法，结合查询端 API 需求估计、主动文档注入和路由纠正策略，在推理时提升代码生成准确性。

Result: 在 2000 个任务上评估 10 个开源模型（1.5B-480B）和 4 个商业 API。干预使所有 ≥7B 的开源模型和所有商业 API 提升 32-56 个准确率点；70B-120B 开源模型达到商业中端 API 水平；Llama-3.1-405B 和 Qwen3-Coder-480B 表现最佳；目标提示仅用 41% 的 token 成本即可保持全上下文准确率上限。

Conclusion: 本文证明了电力系统代码生成中的主要错误源于结构化 API 知识边界而非纯推理能力，提出的边界感知干预方法为本地部署 LLM 辅助电网分析工作流提供了一条无需微调或云端推理的可靠路径，兼顾准确性和部署成本。

Classification reason: 该论文聚焦电力系统领域的代码生成，属于垂直领域（能源/电力）的生成应用，与 [CANONICAL] 子主题"领域内容生成应用"高度匹配。

Importance reason: Focuses on domain-specific code generation for power systems, which is outside the user's priority subtopics and contains negative keywords. Signals: Domain-specific code generation, power systems, API knowledge boundary, no fine-tuning, on-premise deployment
...
### [149] [Fighting Numerical Hallucinations via Data-centric Compilation for Online Financial QA](https://arxiv.org/abs/2605.31064)
...
Importance: 普通相关 (36.59) - 未入选精读

TL;DR: 本文提出数据为中心的推理编译器 DCRC，通过对抗数据构建、多阶段训练和编译执行推理三阶段，解决金融 QA 中 RAG 系统的数值推理幻觉问题，在离线基准和在线系统中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 在金融问答中易出现数值推理幻觉，RAG 虽被广泛采用但引入噪声敏感、计算脆弱和审计危机三大挑战，而现有模型中心方法（单独优化检索器或生成器）难以系统性解决这些问题。

Method: 提出 DCRC 框架，包含三个阶段：(1) 对抗数据构建——合成带可控噪声的训练样本以增强鲁棒性；(2) 多阶段训练——培养数据中心结构化智能体（DSA），使其具备显式证据审计和程序合成能力；(3) 编译执行推理——DSA 将用户查询和检索文档转化为可验证、可执行的推理程序。

Result: 在离线金融 QA 基准上进行了广泛实验，并在真实在线金融问答系统中部署验证，结果表明 DCRC 有效缓解了数值推理幻觉问题。

Conclusion: 数据中心的编译执行范式能够有效解决金融 QA 中的数值推理幻觉问题，通过显式证据审计和可执行程序推理确保了数值推理的忠实性。

Classification reason: 该论文聚焦金融领域问答中的数值幻觉问题，属于垂直领域（金融）的生成应用，与 [CANONICAL] 的"领域内容生成应用"（aigc_domain_application）高度匹配。

Importance reason: Focuses on financial QA numerical hallucination, which is outside your priority subtopics and has negative keyword "financial". Signals: data-centric compilation, numerical reasoning hallucination, RAG for FinQA, adversarial data construction, multi-stage training
...
### [150] [GUI-C$^2$: Coarse-to-Fine GUI Grounding via Difficulty-Aware Reinforcement Learning](https://arxiv.org/abs/2605.30884)
...
TL;DR: 本文提出 GUI-C²，一种基于难度感知强化学习的粗到细 GUI 视觉定位方法。在数据层面，提出 GUI-D 数据挖掘与难度评分流水线，筛选高训练价值样本并分配难度权重；在策略层面，提出区域门控的粗到细精炼机制，通过模型内部不确定性信号逐步缩小视野，并辅以改进感知的阶段奖励来确保每次精炼都有效提升定位精度。实验表明该方法达到了 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的 GUI 视觉定位方法存在两个层面的局限：数据层面，所有训练样本被同等对待，忽略了不同难度样本对基线模型的训练价值差异，导致训练效率低下甚至崩溃；策略层面，现有框架难以平衡裁剪大区域（获取充分上下文）与小区域（减少冗余）之间的权衡，且复杂决策对小参数模型不友好、推理时间长。

Method: 数据层面提出 GUI-D 流水线，通过适当测试识别有价值的训练样本并分配难度分数以指导训练权重。策略层面提出 GUI-C²，采用区域门控的粗到细精炼机制：利用模型内部不确定性信号逐步缩小视野，自适应地为大目标保留上下文、为小目标放大精度；同时引入改进感知的阶段奖励，确保每次精炼都真正提升定位效果。此外简化了决策过程以减少额外推理时间。

Result: 大量实验表明，该方法在 GUI 视觉定位任务上达到了 SOTA 性能。

Conclusion: 本文通过难度感知的数据筛选和粗到细的精炼策略，有效解决了 GUI 视觉定位中训练效率低下和上下文-冗余权衡的问题，为工具增强型 GUI 智能体提供了更高效、更精确的定位方案。

Classification reason: 该论文研究 GUI 智能体中的视觉定位（GUI grounding），属于多模态理解与生成方向下的 AI 智能体与生成工作流（aigc_agent_workflow）范畴。论文提出的粗到细定位机制和强化学习训练策略属于智能体工作流优化，与现有 canonical subtopic 高度匹配。

Importance reason: Focuses on GUI grounding for agents, not directly on the user's priority subtopics like continuous VLM, video generation, or unified multimodal models. Signals: GUI grounding, reinforcement learning, coarse-to-fine refinement, difficulty-aware training, agentic systems
...
### [151] [Task-Focused Memorization for Multimodal Agents](https://arxiv.org/abs/2605.31075)
...
TL;DR: 本文提出 TaskMem，一种基于强化学习的任务聚焦记忆策略学习框架，使多模态智能体（如具身智能体）能够从无限的多模态观测流中动态选择与未来任务相关的信息进行记忆。采用两阶段训练范式：第一阶段学习如何记忆（优化记忆质量），第二阶段在部署后通过环境任务定义奖励模型，指导记忆策略聚焦任务相关内容。在 VideoMME、EgoLife、EgoTempo 的流式基准上，基于 Qwen3-VL-30B-A3B 的 TaskMem 将 VQA 准确率分别提升 6.3%、7.0% 和 5.3%。


<details>
  <summary>Details</summary>
Motivation: 多模态智能体（如具身智能体）持续感知、推理和行动，接收无限的多模态观测流。从这种信息组合爆炸中，智能体必须有选择地保留与其角色相关且对未来任务有价值的内容。现有方法主要关注记忆模块设计或基本准确性/保真度，但缺乏对"记忆什么"这一核心问题的学习机制。

Method: 将记忆生成建模为可学习的记忆策略（memorization policy），提出 TaskMem 框架。采用两阶段训练：Phase One 在基本保真度要求下优化记忆质量，学习如何记忆；Phase Two 在部署后，通过环境中的实际任务定义奖励模型，使用强化学习微调基座 MLLM 上的适配器（adapter），使记忆策略聚焦任务相关内容。同时将 VideoMME、EgoLife、EgoTempo 重构为流式基准，模拟智能体处理流式观测和在线任务的真实场景。

Result: 基于 Qwen3-VL-30B-A3B，TaskMem 在重构的 VideoMME、EgoLife、EgoTempo 流式基准上，VQA 准确率分别提升 6.3%、7.0% 和 5.3%。

Conclusion: TaskMem 通过将记忆生成建模为可学习的策略，并利用强化学习根据实际任务动态调整记忆焦点，有效提升了多模态智能体在流式环境中的长期记忆能力和任务表现。两阶段训练范式（先学如何记忆，再学记什么）为智能体记忆系统设计提供了新思路。

Classification reason: 该论文研究多模态智能体的长期记忆生成策略，属于多模态智能体（multimodal agent）工作流范畴。在 multimodal_understanding_generation 方向下，aigc_agent_workflow（AI 智能体与生成工作流）是唯一匹配的 [CANONICAL] 子主题，涵盖 LLM 智能体、工具使用、自动化流水线等。同时该论文也涉及 AIGC 方向下的智能体工作流。

Importance reason: Strong memory framework for multimodal agents but lacks direct connection to priority subtopics like continuous VLM or autoregressive generation. Signals: multimodal agent memory, reinforcement learning memorization, streaming VQA benchmarks, Qwen3-VL-30B-A3B, task-focused policy
...
### [152] [Detect in Any Scene: An Agentic Framework for Object Detection with Experience-Aware Reasoning](https://arxiv.org/abs/2605.31174)
...
TL;DR: 本文提出 DetAS/DetAS-X，一个基于 MLLM 智能体的目标检测框架，通过自适应图像恢复和多专家检测器动态编排，结合经验积累机制实现经验感知推理，在六个挑战性基准上平均 F1 提升 28.36%。


<details>
  <summary>Details</summary>
Motivation: 现实场景中的目标检测面临图像退化多样、目标分布异质等问题，现有检测器泛化能力不足。传统方法依赖预定义条件和静态流水线，缺乏对动态环境的适应性。

Method: 提出 DetAS 框架，以 MLLM 为核心智能体，包含两个关键组件：(1) 自适应图像恢复——动态判断是否需要以及如何增强图像；(2) 多专家检测——集成多个领域专用检测器并通过实例级推理融合预测。进一步提出 DetAS-X，引入自进化经验收获机制，从少量标注数据中积累节点级决策经验，实现推理时的经验感知推理，逐步优化决策策略。

Result: 在六个挑战性基准上，DetAS-X 显著优于现有基于 MLLM 的检测器，平均 F1 提升 28.36%，在 DarkFace 上提升高达 37.01%。

Conclusion: 本文验证了智能体式检测框架在复杂动态环境中的有效性，为将目标检测建模为动态决策过程提供了新范式，奠定了智能体检测的坚实基础。

Classification reason: 该论文提出 DetAS，一个基于 MLLM 作为中央智能体的目标检测框架，通过动态组合恢复模块和专用检测器来构建检测工作流，属于 LLM 智能体与自动化工作流范畴。虽然该论文聚焦于目标检测而非内容生成，但其核心思想——利用 MLLM 智能体动态编排工具和工作流——与 aigc_agent_workflow（AI 智能体与生成工作流）高度契合，且该 subtopic 在 multimodal_understanding_generation 方向下已有收录。

Importance reason: Agentic detection framework with MLLM reasoning is interesting but not aligned with user's priority subtopics on continuous VLM, autoregressive generation, or world models. Signals: MLLM agent for detection, experience-aware reasoning, adaptive image restoration, multi-expert detection, F1 improvement on benchmarks
...
TL;DR: DeMaVLA 是一个面向可变形物体（如衣物）折叠操作的 VLA 基础模型，采用 VLM 骨干 + 流匹配动作生成，通过约 5000 小时真实双机械臂演示预训练和 DAgger 校正数据后训练，实现了跨类别、跨场景的通用折叠策略。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 系统通常为不同物体类别训练独立策略，混合多任务训练存在任务干扰和性能下降问题。可变形物体（如衣物）折叠是家庭机器人面临的代表性挑战，需要能够跨类别、几何形状、材料和场景泛化的通用操作策略。

Method: DeMaVLA 采用 VLM 骨干网络（视觉-语言模型）结合动作专家（action expert）的架构，使用流匹配（flow matching）生成连续动作。动作专家通过每隔一层剪枝 VLM 的 transformer 层构建，同时保持层间对齐以降低训练和推理成本。模型先在约 5000 小时的真实双机械臂演示数据上预训练获取通用操作先验，再通过人类参与的 DAgger 数据聚合流程，在混合折叠数据上进行后训练，包含自收集的演示和真实机器人失败后的校正轨迹。

Result: DeMaVLA 在 RoboTwin 基准上取得了有竞争力的性能，并在自建的家庭折叠基准上展示了强大的真实世界表现，验证了可扩展的真实世界数据、高效动作生成和校正学习对通用 VLA 策略的价值。

Conclusion: DeMaVLA 证明了通过大规模真实世界演示预训练、高效的动作生成架构和 DAgger 校正学习，可以训练出跨类别、跨场景泛化的可变形物体操作 VLA 基础模型，为家庭机器人通用操作策略提供了有效范式。

Classification reason: 该论文提出 DeMaVLA，一个面向可变形物体操作的 VLA（视觉-语言-动作）基础模型，属于机器人操作智能体范畴。论文涉及 VLM 骨干、动作生成（flow matching）、数据聚合（DAgger）等工作流，与 [CANONICAL] subtopic "AI 智能体与生成工作流"（aigc_agent_workflow）高度匹配。该 subtopic 在 multimodal_understanding_generation 方向下已有记录，因此直接复用。

Importance reason: Focuses on robotic manipulation, not core multimodal generation or autoregressive video/image generation. Signals: VLA foundation model, deformable manipulation, flow matching action generation, real-world dual-arm data, DAgger corrective training
...
### [154] [Hide-and-Seek in Trajectories: Discovering Failure Signals for VLA Runtime Monitoring](https://arxiv.org/abs/2605.30834)
...
TL;DR: 本文提出Hide-and-Seek框架，将VLA模型的执行故障检测建模为粗监督学习问题，通过轨迹间和轨迹内对比学习，仅利用轨迹级标签即可定位故障指示性动作，无需逐帧标注。在LIBERO、VLABench和真实机器人平台上，基于OpenVLA、π₀、π₀.₅三种策略均取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: VLA模型在真实部署中容易发生执行故障，现有故障检测方法要么依赖昂贵的动作重采样或外部模型，要么将轨迹级标签均匀传播到每个时间步，掩盖了局部故障信号。需要一种无需逐帧标注、能精确定位故障时刻的检测方法。

Method: 提出Hide-and-Seek框架，将VLA故障检测建模为粗监督学习问题。设计两种对比学习目标：(1) 轨迹间对比——区分正常轨迹与故障轨迹；(2) 轨迹内对比——在故障轨迹内部区分故障时刻与正常时刻。通过联合优化，模型仅从轨迹级标签就能学习到时间结构化的故障信号，定位故障指示性动作。

Result: 在LIBERO、VLABench和真实机器人平台上，基于OpenVLA、π₀、π₀.₅三种VLA策略进行评测，Hide-and-Seek在多任务故障检测中达到SOTA性能，在保形预测框架下实现了准确率与时效性的实用权衡，并良好泛化到未见任务。

Conclusion: Hide-and-Seek通过粗监督对比学习有效解决了VLA执行故障检测中缺乏逐帧标注的问题，为机器人系统的安全可靠部署提供了一种实用且高效的故障监测方案。

Classification reason: 该论文研究VLA（视觉-语言-动作）模型在执行过程中的故障检测，属于多模态系统在真实部署中的安全性与可靠性问题。这与[多模态安全与攻防]（multimodal_safety_attack）这一CANONICAL子主题高度相关，该子主题涵盖多模态系统的安全、防御、可靠性评测等方向。
...
### [155] [Vision-Language Models Suppress Female Representations Under Ambiguous Input](https://arxiv.org/abs/2605.31556)
*Arnau Marin-Llobet, Simon Henniger, Mahzarin R. Banaji*

Main category: cs.CV, cs.HC, cs.CY, cs.CL, cs.AI

Direction: 多模态理解与生成

Subtopic: 多模态幻觉与可靠性

Importance: 普通相关 (53.75) - 未入选精读

TL;DR: 本文发现VLM在模糊性别输入（如背影、全副武装的工人）下，即使内部编码了女性关联，输出仍系统性偏向男性。作者提出LALS指标，通过将视觉token激活投影到文本嵌入空间来逐层测量概念关联，揭示了从中间层到输出层的"不对称滤波"——男性信号被放大而女性信号被抑制。


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法在性别清晰可见时能有效消除VLM的人口统计偏见，但对实践中常见的模糊输入（如从背后看到的工人）研究甚少。作者希望探究模糊输入下VLM的输出是否反映其内部真实编码，以及是否存在系统性的性别表征偏差。

Method: 提出LALS（Latent Association Leaning Score），一种零样本度量方法，将视觉token的激活值投影到模型文本嵌入空间，逐层逐token测量概念关联强度。在15种职业、800+张性别模糊图像上，对4种VLM（CLIP、LLaVA等）进行内部表征与输出行为的对比分析，并进行逐层分析和颜色消融实验。

Result: 内部表征与输出存在系统性解耦：模型内部编码了女性关联，但输出却为男性。逐层分析发现不对称滤波——男性信号从中间层到输出层持续放大，而女性信号在中间层达到峰值后在被抑制。颜色消融表明服装颜色等文化负载视觉线索会进一步调节内部关联。

Conclusion: VLM在模糊输入下存在系统性的性别表征偏差，输出层对女性信息的抑制是一个结构性问题，而非简单的内部编码缺失。这揭示了当前对齐方法的局限性，并为构建更公平的多模态系统提供了新的诊断工具（LALS）。

Classification reason: 该论文研究VLM在模糊输入下对女性表征的抑制现象，属于模型输出与内部表征的可靠性/忠实性问题，与[多模态幻觉与可靠性]（multimodal_hallucination）主题高度吻合。

Importance reason: Focuses on gender bias in VLMs, not aligned with your priority subtopics on generation, efficiency, or world models. Signals: VLM bias, ambiguous input, internal representation decoupling, LALS metric, social impact
...
### [156] [Immuno-VLM: Immunizing Large Vision-Language Models via Generative Semantic Antibodies for Open-World Trustworthiness](https://arxiv.org/abs/2605.30745)
...
TL;DR: Immuno-VLM 受生物免疫负选择机制启发，利用大语言模型的生成推理能力主动生成"语义抗体"（近分布异常文本描述），为大视觉语言模型在开放世界中的零样本识别提供可靠性和可信度保障，在 ImageNet-1K 和多个 OOD 基准上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在零样本识别中因语义抽象而存在"语义傲慢"问题——将未知异常强行归入已知类别并给出高置信度预测，缺乏显式的负知识。现有开放集识别方法依赖被动密度估计或像素级异常生成，效率低且泛化差。因此需要一种主动、高效的机制来界定决策边界，提升开放世界部署的可信度。

Method: Immuno-VLM 借鉴生物免疫负选择机制，利用大语言模型的生成推理能力主动"幻觉"出"语义抗体"——即近分布异常样本的文本描述（如相似物、上下文异常等），这些抗体在潜在空间中有效界定已知类别的决策边界，从而帮助 VLM 识别未知异常。

Result: 在 ImageNet-1K 和四个具有挑战性的开放集识别（OOD）基准上，Immuno-VLM 取得了新的 SOTA 结果，显著优于现有开放集识别方法。

Conclusion: Immuno-VLM 通过生物启发的免疫负选择机制，将 LLM 的生成推理能力用于主动构建语义抗体，有效解决了 VLM 在开放世界中的可信度悖论，为提升多模态模型的可靠性和安全性提供了新范式。

Classification reason: 该论文关注大视觉语言模型在开放世界部署中的可靠性问题，提出通过生成"语义抗体"来缓解模型对未知异常样本的误判（即语义幻觉），属于多模态幻觉与可靠性范畴，因此映射到 canonical subtopic "多模态幻觉与可靠性"。

Importance reason: Novel bio-inspired OOD detection for VLMs but outside user's priority subtopics and lacks autoregressive/diffusion generation focus. Signals: Open-world trustworthiness, Immunological negative selection, Semantic antibodies, Zero-shot OOD detection, VLM reliability
...
### [157] [What Makes LVLMs Hallucinate Less? Unveiling the Architectural Factors Behind Hallucination Robustness](https://arxiv.org/abs/2605.30911)
...
TL;DR: 本文系统探索了LVLM架构设计（语言基础、视觉表示、语义对齐三个维度）对三种幻觉（共现、相似性、不确定性）鲁棒性的影响，提出了细粒度幻觉基准CoSimUE，揭示了模型参数规模对幻觉缓解效果有限、联合增强视觉保真度与对齐质量效果最佳等关键发现。


<details>
  <summary>Details</summary>
Motivation: 幻觉是LVLM可靠性的关键挑战，现有工作多关注改进内部组件，但缺乏从架构设计层面系统理解幻觉成因的研究。本文旨在回答"什么样的架构设计能让LVLM产生更少的幻觉"这一根本问题。

Method: 将架构设计分解为语言基础(LF)、视觉表示(VR)和语义对齐(SA)三个维度，将幻觉分为共现、相似性和不确定性三类。提出CoSimUE基准，通过受控文本扰动和随机扰动创建细粒度幻觉场景，在7个设计方面进行实验映射设计选择与幻觉行为之间的关系。

Result: 1) 模型参数规模扩展对减少三类幻觉效果有限；2) 更大更好的语言基础可减少共现幻觉；3) 更强的视觉编码器和更高分辨率可缓解相似性错误；4) 有效的对齐策略可减少不确定性幻觉；5) 联合增强视觉保真度与对齐质量可获得最全面的改进。

Conclusion: 本文首次系统探索了架构级设计与幻觉鲁棒性之间的联系，为开发可靠高效的LVLM提供了实用指导，表明单纯扩大参数规模并非缓解幻觉的有效途径，而应注重视觉表示与语义对齐的协同优化。

Classification reason: 该论文研究LVLM幻觉问题，探讨架构设计对幻觉鲁棒性的影响，属于多模态幻觉与可靠性这一[CANONICAL]子主题。

Importance reason: Systematic architectural analysis of LVLM hallucination is valuable but not aligned with user's priority on generation and continuous VLM. Signals: LVLM hallucination robustness, architectural factor analysis, CoSimUE benchmark, limited scaling impact, cross-dimensional design insights
...
### [158] [Human-Alignment, Calibration, and Activation Patterns in Large Language Model Uncertainty](https://arxiv.org/abs/2605.30675)
...
TL;DR: 本文研究大语言模型的不确定性与人类不确定性的相似程度，提出"不确定性对齐"（uncertainty alignment）概念，分析模型在行为层面和内部激活模式中是否同时具备对齐与校准能力，并探究指令微调对这两方面的影响。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性量化研究主要关注校准（模型对自身判断的置信度是否准确），但很少探讨模型的不确定性是否与人类的不确定性相似。理解这种相似性对于构建更可信、更符合人类直觉的AI系统至关重要。

Method: 在多项选择题和开放式事实回忆数据集上，通过分析LLM的显式行为（如输出置信度）和内部激活模式（如隐藏状态），检测人类相似的不确定性信号（不确定性对齐），并同时评估校准质量，研究指令微调对两者的影响。

Result: 发现LLM在行为层面和内部激活模式中均存在与人类不确定性相似的信号，且模型可以同时表现出对齐和校准；指令微调对不确定性对齐和校准均有显著影响。

Conclusion: 不确定性对齐是评估LLM可信度的重要维度，与校准互补；未来研究应同时关注对齐与校准，以构建更可靠的语言模型。

Classification reason: 该论文研究大语言模型的不确定性量化（uncertainty quantification），核心关注模型校准（calibration）与人类不确定性对齐（human-alignment），旨在识别和对抗幻觉（hallucination）。这与多模态理解与生成方向下的 [CANONICAL] 子主题"多模态幻觉与可靠性"（multimodal_hallucination）高度相关，因为该子主题涵盖幻觉缓解、事实性与可靠性提升。

Importance reason: Focuses on uncertainty alignment and calibration in LLMs, which is not a priority subtopic and lacks direct connection to multimodal generation or autoregressive video/image generation. Signals: Uncertainty alignment, calibration, LLM behavioral analysis, instruction fine-tuning effects, human-similar uncertainty signals
...
### [159] [Learning from Fine-Grained Visual Discrepancies: Mitigating Multimodal Hallucinations via In-Context Visual Contrastive Optimization](https://arxiv.org/abs/2605.31312)
...
Importance reason: Addresses multimodal hallucination in VLMs with a novel contrastive optimization method, but is not directly aligned with top-priority subtopics like continuous VLM or autoregressive generation. Signals: Multimodal hallucination mitigation, Visual preference optimization, In-context contrastive learning, Hard negative generation, VLM reliability improvement
...
### [160] [YARD: Y-Architecture Register Decoding for Efficient Hallucination Mitigation in Large Vision-Language Models](https://arxiv.org/abs/2605.31429)
...
TL;DR: YARD提出了一种无需训练的Y-Architecture Register Decoding框架，通过共享浅层计算并在中间解码层分支，用register tokens替换patch-level视觉token来构建视觉退化分支，从而高效缓解LVLM的幻觉问题，同时显著降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有的无训练对比解码方法存在两个问题：完全丢弃视觉token过于极端，会引入语言幻觉；而破坏输入图像的方式对视觉证据的控制粗糙，且需要两次完整前向传播导致高推理延迟。因此需要一种更高效、更精确的幻觉缓解方法。

Method: YARD基于一个关键观察：可靠的文本到视觉定位主要出现在中间解码层。因此，Y-Architecture在浅层共享计算，在中间关键层进行分支。对于退化分支，用register tokens（保留全局图像语义但缺乏细粒度局部证据）替换patch-level视觉token，提供忠实对比信号且避免极端模态不匹配。同时Y-Architecture避免了第二次完整前向传播，大幅降低延迟。

Result: 在生成式和判别式幻觉基准测试上，YARD在多个LVLM上一致达到最先进的幻觉缓解效果，同时显著降低了推理延迟。

Conclusion: YARD通过Y-Architecture和register token的设计，在无需训练的条件下有效缓解了LVLM的幻觉问题，兼顾了效果和效率，为对比解码方法提供了新的思路。

Classification reason: 该论文研究大型视觉语言模型（LVLM）中的幻觉缓解问题，提出Y-Architecture Register Decoding（YARD）框架，属于多模态幻觉缓解方向。这与[多模态幻觉与可靠性]（multimodal_hallucination）这一CANONICAL subtopic完全匹配。

Importance reason: Novel training-free hallucination mitigation for LVLMs, but only tangentially touches continuous VLM or multimodal latent reasoning. Signals: Training-free contrastive decoding, Y-architecture reduces latency, Register tokens for visual degradation, State-of-the-art on hallucination benchmarks, Applies to multimodal_understanding_generation
...
### [161] [BenHalluEval: A Multi-Task Hallucination Evaluation Framework for Large Language Models on Bengali](https://arxiv.org/abs/2605.31483)
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉语是世界第六大语言，但此前没有系统性地评估大语言模型在该语言上的幻觉问题。现有幻觉评测主要针对英语等资源丰富的语言，低资源语言的幻觉行为尚不明确。
...
Result: BenHalluScore 在不同模型和任务上从7.72%到55.42%不等，揭示了模型在幻觉校准上的显著差异。思维链提示作为缓解策略会改变响应分布，但未能持续改善幻觉判别能力。

Conclusion: BenHalluEval 建立了首个孟加拉语幻觉专用基准，揭示了单轨评测和仅使用提示策略在低资源语言环境中的不足，为低资源语言幻觉研究提供了基础。
...
Importance reason: Focuses on Bengali hallucination evaluation, which is outside the user's priority subtopics and negative-keyword-filtered directions. Signals: Low-resource language benchmark, Hallucination evaluation, No overlap with priority subtopics, No boost keywords matched, Negative keywords not triggered but irrelevant
...
### [162] [Attend to Evidence: Evidence-Anchored Spatial Attention Supervision for Multimodal RLVR](https://arxiv.org/abs/2605.30912)
...
TL;DR: 本文提出 EASE（Evidence-Anchored Spatial Attention），一种为多模态 RLVR 引入视觉证据过程监督的方法。EASE 将标注的证据区域转换为平滑的视觉 token 目标，在 RL 训练中引导响应到图像的注意力分布，仅在高奖励轨迹上施加约束。在 Qwen2.5-VL-7B、Qwen3-VL-4B 和 Qwen3-VL-8B 上，EASE 在感知、幻觉、视觉数学和多模态推理基准上相比 DAPO 平均提升 2.5-3.1 分。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态 RLVR 仅使用最终答案的奖励信号进行优化，无法区分基于正确视觉证据的回答与依赖语言先验或运气猜测的回答。对于需要视觉定位的问题，这种仅关注结果的奖励无法引导模型关注正确的图像区域，导致模型可能通过捷径学习而非真正的视觉理解来获得高分。

Method: EASE 方法包含三个关键步骤：(1) 将人工标注的证据区域（bounding box）转换为平滑的视觉 token 注意力目标分布；(2) 在 RL 训练中，仅对高奖励轨迹施加响应到图像的注意力分布约束，使其与标注证据区域对齐；(3) 标注仅在训练阶段作为特权标签使用，推理时仅需原始图像和问题。该方法基于 DAPO 框架实现，通过额外的注意力监督信号引导模型关注正确的视觉证据。

Result: 在 Qwen2.5-VL-7B、Qwen3-VL-4B 和 Qwen3-VL-8B 三个模型上，EASE 相比 DAPO 基线在感知、幻觉、视觉数学和多模态推理等基准上平均提升 2.5-3.1 分。诊断实验和消融研究表明，EASE 能更好地将视觉注意力与标注的证据区域对齐。

Conclusion: EASE 通过引入视觉证据过程监督，有效弥补了多模态 RLVR 中仅依赖结果奖励的不足，使模型在需要视觉定位的任务上表现更优。该方法简单有效，且推理时无需额外标注，具有良好的实用价值。

Classification reason: 该论文提出 EASE 方法，在视觉语言模型的多模态 RLVR（带可验证奖励的强化学习）中引入视觉证据过程监督，属于多模态强化学习与对齐方向，因此映射到 [CANONICAL] subtopic "多模态强化学习与对齐" (multimodal_rl_alignment)。

Importance reason: Focuses on RLVR for multimodal understanding, but lacks direct connection to priority subtopics like continuous VLM or autoregressive generation. Signals: RLVR with visual evidence supervision, attention alignment for VLMs, improves perception and hallucination benchmarks, uses privileged training labels, no inference overhead
...
Main category: cs.CV, cs.LG, cs.MM
...
TL;DR: 本文系统研究了视觉语言模型（VLM）中的测试时计算（TTC）策略，发现特征启发式方法失效、多数投票在单模型场景下增益有限，原因是输出缺乏多样性。为此提出基于预测熵的ETTC方法，在多模型集成中利用置信度差异优先选择更强模型，理论上证明其优于多数投票，实验表明小模型可协同增强大模型。


<details>
  <summary>Details</summary>
Motivation: 测试时计算（TTC）策略在LLM中已被证明能有效提升推理能力，但在VLM中的应用和收益尚未被充分探索。本文旨在系统研究TTC在VLM中的表现，并解决现有方法在缺乏预测多样性时效果有限的问题。

Method: 在7个VLM和6个基准上系统分析特征评分和多数投票方法；理论上证明多数投票在输出高度相关时收益有限的根本原因；提出基于预测熵的ETTC方法，在模型集成中根据置信度选择最自信的预测，单模型场景下退化为多数投票。

Result: ETTC在多个基准上一致优于多数投票和最佳单模型；小模型可以协同增强大模型，实现标准策略无法达到的集成增益；理论证明ETTC在温和假设下优于多数投票。

Conclusion: 多样性是VLM测试时计算策略有效性的关键因素；基于熵的ETTC方法能有效利用多模型集成中的置信度差异，释放集成增益潜力，为VLM推理时计算扩展提供了新思路。

Classification reason: 该论文系统研究视觉语言模型（VLM）的测试时计算（TTC）策略，分析特征评分和多数投票等方法，并提出基于熵的ETTC方法。核心关注推理时计算扩展（inference scaling），属于多模态推理与思维链（multimodal_reasoning_cot）这一规范主题。

Importance reason: Strong theoretical and empirical analysis of test-time compute for VLMs, but only tangentially touches priority subtopics like continuous VLM or video generation. Signals: VLM test-time compute, diversity analysis, entropy-based ensemble, multi-model synergy, no direct priority subtopic match
...
### [164] [ReGuLaR: Relation-Grounded Latent Reasoning for Large Vision-Language Models](https://arxiv.org/abs/2605.30587)
...
TL;DR: ReGuLaR 提出一种关系锚定的潜空间推理框架，通过训练阶段的 ReGFormer 模块将潜状态显式锚定到图像中与问题相关的物体及其关系上，从而增强 LVLM 的视觉推理能力，并在多个基准上取得 SOTA 结果。


<details>
  <summary>Details</summary>
Motivation: 现有 LVLM 的 CoT 推理使用离散文本作为中间推理步骤，难以充分编码连续的视觉证据。虽然已有工作将推理引入连续潜空间，但这些方法未能充分利用视觉证据中的组合性和关系结构，导致潜推理与视觉证据之间的连接不足。

Method: 提出 ReGuLaR 框架，包含：(1) 训练阶段的 ReGFormer 模块，专注于对问题相关的物体及其间关系进行潜空间推理锚定；(2) 推理阶段无需 ReGFormer，模型直接推理并生成答案；(3) 构建 RGROUNDING-351K 数据集，包含真实世界图像中关键物体边界框和物体间关系标注，用于训练。

Result: 在多个多样化基准上的实验表明，ReGuLaR 一致性地优于现有方法，取得了 SOTA 性能。

Conclusion: 将潜空间推理显式锚定到视觉关系结构上能有效提升 LVLM 的推理能力，为多模态推理提供了新的思路。

Classification reason: 该论文提出 ReGuLaR，一种基于关系锚定的潜空间推理框架，用于提升大视觉语言模型（LVLM）的推理能力，属于多模态推理与思维链（CoT）方向。论文核心是改进视觉推理中连续潜空间与视觉关系结构的连接，与 [CANONICAL] subtopic "多模态推理与思维链" 高度匹配。

Importance reason: Strongly aligns with continuous VLM and multimodal latent reasoning priority, proposing a novel relation-grounded latent reasoning framework. Signals: continuous latent reasoning, relation grounding, LVLM reasoning, state-of-the-art on multiple benchmarks, new dataset RGROUNDING-351K
...
Importance: 高优先级 (89.67) - 未入选精读

TL;DR: 本文提出 iVGR，一种基于强化学习的框架，将视觉定位能力内化到 MLLM 的文本推理过程中，使得模型在推理时无需显式输出目标框即可实现精准定位，从而避免显式视觉接地对答案预测的干扰，在细粒度评测上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉接地思维链（visually grounded CoT）在推理时强制要求模型输出显式目标框，反而比纯文本 CoT 性能更差。作者假设视觉定位能力可以被内化到文本推理过程中，显式接地反而干扰了模型的主要目标（答案预测）。

Method: 提出 iVGR 框架，采用双流训练策略：一个文本流（textual stream）与一个高质量视觉接地流（visually grounded stream）通过一致性奖励（consistency reward）进行对齐，利用强化学习将定位能力转移到文本推理过程中，使模型在推理时无需显式接地即可准确感知位置。

Result: 在多个细粒度评测基准上，iVGR 显著优于现有基线方法，同时保持支持工具辅助推理工作流的灵活性。

Conclusion: 视觉定位能力可以通过强化学习内化到文本推理过程中，无需显式接地即可实现精准的视觉感知，为 MLLM 的细粒度推理提供了新范式。

Classification reason: 该论文研究 MLLM 的视觉定位推理与思维链（CoT），属于多模态推理与思维链范畴，与 [CANONICAL] subtopic "多模态推理与思维链"（multimodal_reasoning_cot）高度匹配。

Importance reason: Directly advances multimodal understanding and reasoning for MLLMs, aligning with high-priority continuous VLM and unified multimodal topics. Signals: multimodal large language model, reinforcement learning, visual grounding, chain-of-thought, fine-grained perception
...
### [166] [LARK: Learnability-Grounded Trajectory Selection for Efficient Reasoning Distillation](https://arxiv.org/abs/2605.30651)
...
TL;DR: LARK 提出了一种基于可学习性的推理轨迹选择方法，通过估计学生模型的学习速率因子 ρ 并结合 χ² 正则化策略，在保证数据分布覆盖的同时选择学生可高效学习的轨迹，在多个基座模型和推理任务上优于现有基线。
...
Method: 提出 LARK 方法，核心是定义一个可学习性因子 ρ 来刻画学生模型训练损失下降的速率；引入可学习性代理（learnability proxy）高效估计 ρ，并设计 χ² 正则化选择策略在可学习性与分布覆盖之间取得平衡，同时提供理论保证。

Result: 在多个基座模型和推理任务上，LARK 一致优于现有数据选择基线；诊断分析表明 LARK 分数能有效预测下游训练效用，且 LARK 选择的轨迹能诱导更快的监督微调损失下降。

Conclusion: LARK 通过可学习性驱动的轨迹选择有效提升了推理蒸馏的效率与泛化能力，为数据选择提供了新的理论视角和实用方法。

Classification reason: 该论文研究推理蒸馏中的轨迹选择方法，属于多模态推理（reasoning distillation）范畴，与 canonical 子主题 "多模态推理与思维链"（multimodal_reasoning_cot）高度相关。

Importance reason: Focuses on reasoning distillation trajectory selection, not directly on the user's priority subtopics like continuous VLM or video generation. Signals: reasoning distillation, trajectory selection, learnability estimation, χ² regularization, student model training
...
### [167] [The Sword, Shield, and Achilles' Heel: Characterizing the Linguistic Inductive Bias of Large Language Models for Spatial Reasoning in Navigation Planning](https://arxiv.org/abs/2605.31404)
...
TL;DR: 本文提出一个双干预框架，系统评估LLM在导航规划任务中的语言归纳偏置。通过表示干预（改变语言格式与压缩程度）和上下文干预（组合与冲突探测不同上下文特征），发现拓扑信息是鲁棒规划的坚实支柱，语言格式是双刃剑，语义信息是致命弱点——错误语义线索会系统性破坏规划过程。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的导航系统通常将文本化空间表示（如拓扑图、语义栅格图）视为中性工程选择，忽略了语言结构本身对LLM行为的影响。本文旨在填补这一空白，系统揭示语言归纳偏置如何影响LLM的空间推理与导航规划能力。

Method: 提出双干预框架：(1) 表示干预——改变空间表示的语言格式（如自然语言描述 vs 结构化格式）和语言压缩程度；(2) 上下文干预——组合不同上下文特征（拓扑、几何、语义）并进行冲突探测，分析LLM在处理不同上下文线索时的偏好与弱点。在多种空间推理任务和多个模型规模上进行实验。

Result: 实验揭示一致模式：拓扑信息是鲁棒规划的坚实支柱（盾牌）；语言格式是双刃剑，效果取决于模型大小、任务需求和压缩程度；语义信息是致命弱点（阿喀琉斯之踵），错误语义线索会系统性破坏规划过程。

Conclusion: 基于LLM的导航系统中，有效的文本化空间表示应保持拓扑完整性、根据模型容量校准表示压缩程度、确保语义正确性，而非简单采用单一表示格式。

Classification reason: 该论文研究LLM在导航规划中的空间推理能力，分析语言归纳偏置对空间推理的影响，属于多模态推理（空间推理）范畴，与[多模态推理与思维链]（multimodal_reasoning_cot）高度匹配。

Importance reason: Focuses on linguistic bias in LLM navigation planning, not on the user's priority subtopics like continuous VLM, video generation, or unified multimodal models. Signals: LLM spatial reasoning, navigation planning, linguistic inductive bias, not aligned with priority subtopics, no video/image generation
...
TL;DR: LongTraceRL 提出利用搜索智能体轨迹构建高混淆度的长上下文训练数据，并设计基于实体链的细粒度过程奖励（rubric reward），通过强化学习提升LLM的长上下文推理能力，在多个基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文推理方法受限于低混淆度的干扰项和稀疏的最终答案奖励信号，无法有效监督中间推理步骤，导致模型难以在大量干扰信息中定位和整合关键信息。

Method: 通过知识图谱随机游走生成多跳问题，利用搜索智能体轨迹构建分层干扰项（已读未引用/未读文档）；提出基于推理链上实体的细粒度过程奖励（rubric reward），仅对答案正确的响应施加该奖励以区分推理质量并防止奖励黑客。

Result: 在三种推理LLM（4B-30B）和五个长上下文基准上，LongTraceRL 持续优于强基线方法，并促进了基于证据的全面推理。

Conclusion: LongTraceRL 通过高质量训练数据构建和细粒度过程奖励设计，有效提升了LLM的长上下文推理能力，验证了搜索智能体轨迹和实体级过程监督在推理强化学习中的价值。

Classification reason: 该论文聚焦于长上下文推理，通过搜索智能体轨迹构建多跳问题与分层干扰项，并设计细粒度过程奖励（rubric reward）来提升LLM的推理质量。这属于多模态推理与思维链（multimodal_reasoning_cot）范畴下的长上下文推理与推理奖励设计，因此复用该规范子主题。

Importance reason: Focuses on long-context reasoning via RL and search trajectories, not directly on user's priority subtopics like continuous VLM or video generation. Signals: Long-context reasoning, RL with verifiable rewards, search agent trajectories, rubric reward, entity-level process supervision
...
Importance: 值得关注 (66.51) - 未入选精读

TL;DR: 本文提出变分适配器 VACSR，将图像-文本匹配中因细粒度语义标注稀缺导致的二值分类问题重新建模为变分推断问题，通过构建跨模态相似性的潜空间并引入正则化技术缓解对二值标注的过拟合，在图像-文本检索、域泛化和基类到新类泛化任务上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在跨模态相似性度量中，由于缺乏细粒度的跨模态匹配标注，被迫将连续的相似性空间压缩为二值分类边界，导致假负样本问题并严重损害跨模态任务的泛化性能。先前工作尝试通过建模模态内歧义性来缓解，但忽略了标注本身的固有缺陷，导致次优的不确定性分配。

Method: 提出变分适配器 VACSR，将具有细粒度语义稀缺性的图像-文本匹配重新表述为变分推断问题。具体地，构建跨模态相似性的潜空间，利用变分推断和正则化技术来缓解对二值标注的过拟合，从而更准确地建模连续相似性分布。

Result: 在图像-文本检索、域泛化和基类到新类泛化三个任务上进行了实验，结果表明所提方法有效且具有鲁棒的泛化能力。

Conclusion: VACSR 通过变分推断框架有效解决了跨模态相似性表示中因标注稀缺导致的二值化问题，提升了跨模态任务的泛化性能，为细粒度语义稀缺场景下的跨模态匹配提供了新思路。

Classification reason: 该论文提出变分适配器（VACSR）用于改进图像-文本匹配中的跨模态相似性表示，核心任务是跨模态检索（image-text retrieval），属于多模态检索范畴。在 multimodal_understanding_generation 方向下有 [CANONICAL] 的 multimodal_retrieval 子主题，且 multimodal_generation 方向下也有同名子主题，因此复用该子主题。

Importance reason: Focuses on cross-modal retrieval with binary annotation issues, not aligned with priority subtopics like continuous VLM or video generation. Signals: Cross-modal retrieval, Variational inference, Binary annotation problem, Domain generalization, Not autoregressive or diffusion-based
...
TL;DR: 本文提出FOCUS框架，通过两阶段训练（显式优化上下文注意力 + GRPO强化学习）实现无类别监督的上下文目标定位，7B模型性能超越72B模型。


<details>
  <summary>Details</summary>
Motivation: 现有上下文定位方法依赖显式类别监督，导致类别偏差（偏向语义先验而非视觉证据），且无法处理未命名或实例特定的对象。亟需一种类别无关、视觉驱动的上下文定位方法。

Method: 提出两阶段训练框架：(1) 第一阶段通过视觉支持约束显式优化支持框与查询图像之间的上下文注意力，无需类别监督；(2) 第二阶段使用GRPO（组相对策略优化）强化学习直接最小化定位误差，强制模型关注视觉对应关系而非语义先验。

Result: 7B参数模型在多个定位基准上超越72B参数模型，消融实验验证了各组件的贡献，证明了上下文感知定位目标可以超越单纯规模扩展的效果。

Conclusion: 通过显式优化上下文注意力和强化学习直接优化定位误差，可以有效实现类别无关的鲁棒实例级上下文定位，且小模型可通过训练目标设计超越大模型。

Classification reason: 该论文研究基于支持示例的上下文目标定位（in-context localization），本质上是视觉检索与定位任务，与多模态检索（multimodal_retrieval）方向高度相关。论文使用VLM进行查询图像中目标对象的定位，属于跨模态/视觉检索范畴。

Importance reason: Strong in-context localization method but lacks direct connection to priority subtopics like continuous VLM or autoregressive generation. Signals: In-context localization, category-agnostic, GRPO reinforcement learning, 7B outperforms 72B, visual correspondence over semantic priors
...
TL;DR: 本文提出持续多模态检索（CMR）的新评测框架，发现传统类增量学习方法在此场景下效果有限，并提出动态适配器路由（DAR）方法，通过原型路由选择适配器并组合，在跨域持续检索中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的检索功能需要持续更新以适应新领域，但现有工作多从类增量学习视角处理持续检索，未能充分捕捉检索任务的独特动态。缺乏针对持续多模态检索的专门评测框架和方法。

Method: 提出 Dynamic Adapter Routing (DAR) 方法：基于原型路由（prototype-based routing）选择适配器（adapters），并通过模型集成组合多个适配器，实现持续多模态检索中的知识保留与迁移。

Result: 在提出的 CMR 评测框架下，DAR 方法优于现有基线方法，并在分布外（OOD）评估中展现出强泛化能力。

Conclusion: 持续多模态检索具有独特的挑战，传统 CIL 方法难以直接适用；DAR 方法通过动态适配器路由有效应对这些挑战，为该方向提供了新的研究思路。

Classification reason: 论文聚焦于持续多模态检索（continual multimodal retrieval），属于多模态检索方向。该方向在 multimodal_understanding_generation 下有 [CANONICAL] 的 multimodal_retrieval 子主题，同时在 multimodal_generation 下也有同名子主题，因此直接复用该规范子主题。

Importance reason: Focuses on continual multimodal retrieval, which is not a priority subtopic and lacks direct connection to user's key interests in generation, world models, or continuous VLM. Signals: Continual multimodal retrieval, dynamic adapter routing, prototype-based routing, out-of-distribution generalization, not aligned with priority subtopics
...
### [172] [Inference-Free Multimodal Learned Sparse Retrieval for Production-Scale Visual Document Search](https://arxiv.org/abs/2605.30917)
...
TL;DR: 本文提出 V-SPLADE，一种推理无关的多模态学习型稀疏检索器，用于大规模视觉文档检索。通过引入标题门控 token 监督（caption-gated token supervision），利用 VLM 生成的标题作为词汇线索激活检索相关维度，解决了视觉稀疏表示难以捕获文档图像中词汇内容的词汇接地问题。在六个视觉文档检索基准上，V-SPLADE 的 NDCG@5 比同规模稠密基线提升 +13.8pp，比 OCR/标题 BM25 基线提升 +6.3pp；在 1870 万文档语料上 R@5 翻倍于稠密基线。


<details>
  <summary>Details</summary>
Motivation: 现有视觉文档检索方法存在两难：VLM 稠密/多向量模型检索质量高但需要推理时神经编码，部署成本高；OCR/标题 BM25 避免查询编码但需要耗时的文本提取或生成。缺乏一种可部署的、能对视觉文档进行词汇索引且推理时无需神经编码的检索系统。

Method: 提出 V-SPLADE，基于 SPLADE 架构扩展为多模态稀疏检索。核心创新是 caption-gated token supervision（标题门控 token 监督）：在训练时利用 VLM 生成的文档标题作为词汇线索，通过门控机制激活与检索相关的词汇维度，使视觉稀疏表示能更好地捕获文档图像中的词汇内容。该方法推理时无需任何神经编码，仅需稀疏索引和词汇匹配。

Result: 在六个视觉文档检索基准上，V-SPLADE 的 NDCG@5 比同规模稠密基线提升 +13.8pp，比 OCR/标题 BM25 基线提升 +6.3pp。在 1870 万文档的大规模语料上，R@5 比同规模稠密基线翻倍，且通过分数融合可进一步提升其他检索器 +2.4pp R@5。

Conclusion: V-SPLADE 填补了推理无关多模态学习型稀疏检索的空白，证明了在视觉文档检索中，通过训练时引入词汇监督信号，稀疏检索可以达到甚至超越稠密检索的效果，同时保持推理时的高效性和可部署性。

Classification reason: 该论文研究面向视觉文档的推理无关（inference-free）学习型稀疏检索方法，属于跨模态/多模态文档检索范畴。multimodal_understanding_generation 方向下有 [CANONICAL] subtopic "多模态检索"（multimodal_retrieval），且 multimodal_generation 方向下也有同名 subtopic，完美匹配。

Importance reason: Focuses on inference-free sparse retrieval for visual documents, which is not a priority subtopic and lacks autoregressive generation or continuous VLM signals. Signals: multimodal retrieval, sparse retrieval, inference-free, visual document search, caption-gated token supervision
...
### [173] [FBHM: Functional Benchmarking and Steering of VLMs for Hateful Meme Detection](https://arxiv.org/abs/2605.31349)
*Paramananda Bhaskar, Naquee Rizwan, Daksh Jogchand, Saurabh Kumar Pandey, Animesh Mukherjee*

Main category: cs.CV, cs.CL, cs.AI, cs.MM
...
TL;DR: 本文提出FBHM基准，沿修辞功能与目标社区两个正交轴系统构建5000个有害模因，评测发现SOTA VLM在该基准上性能骤降至接近随机水平，表明模型依赖数据集特定启发式而非鲁棒的多模态推理。作者进一步提出LSV（可学习转向向量），仅用500个样本即可提升约30个Macro-F1点，且不损害源域性能。


<details>
  <summary>Details</summary>
Motivation: 现有有害模因检测基准存在结构性观察偏差，将修辞仇恨机制与目标社区特征混淆，无法对模型脆弱性进行因果评估。需要构建一个解耦的、功能导向的基准来真正评测VLM的鲁棒多模态推理能力。

Method: 1) 构建FBHM基准：沿25种修辞功能×10个目标社区两个正交轴，系统生成5000个有害模因。2) 提出LSV（可学习转向向量）：在超低数据 regime（500个样本，50个独特基础模因）下，通过因果干预目标学习转向向量，引导VLM关注真正的有害信号而非虚假关联。

Result: SOTA VLM在FBHM上性能骤降至接近随机水平（如BLIP-2从标准基准的~70%降至~30%），证明存在严重泛化差距。LSV仅用500个样本即可提升约30个Macro-F1点，优于上下文学习和PEFT，且不损害源域性能。

Conclusion: 现有VLM在有害模因检测中依赖数据集特定启发式而非鲁棒的多模态推理。FBHM为因果评估提供了有效工具，LSV为高效缓解泛化差距提供了可行方案。

Classification reason: 该论文提出了FBHM基准，系统性地构建了基于功能的有害模因检测基准，并对多种VLM进行了评测，揭示了泛化差距。这属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。同时，有害模因检测也涉及AIGC安全治理方向。

Importance reason: Focuses on hateful meme detection benchmarking, which is not aligned with user's priority on generative models and continuous VLM. Signals: multimodal understanding, hateful meme detection, benchmark, steering vectors, low-data intervention
...
### [174] [Seeing Isn't Knowing: Do VLMs Know When Not to Answer Spatial Questions (and Why)?](https://arxiv.org/abs/2605.30557)
...
TL;DR: 本文提出 SpatialUncertain 评测框架，系统评估 VLM 在空间推理中面对遮挡和视角歧义时的表现，发现模型普遍存在过度自信回答的问题，在遮挡和视角歧义下准确率分别低于 30% 和 10%，且难以识别哪些额外视角能提供可靠证据。


<details>
  <summary>Details</summary>
Motivation: 现有空间推理基准通常假设观测信息充分可靠，仅关注模型能否给出正确答案，而忽略了模型是否能够识别问题不可回答的情况以及需要哪些额外观测信息。本文旨在填补这一空白。

Method: 构建 SpatialUncertain 受控评测框架，引入两类观测挑战：(1) 遮挡——隐藏目标信息；(2) 视角歧义——产生误导性视觉线索。针对每种配置设计空间问题，在干净观测下可回答，但在挑战条件下应拒绝回答。同时评估模型能否识别哪些额外视角可解决视角歧义。

Result: 在多个前沿开源和闭源 VLM 上的实验显示两个一致的失败模式：(1) 模型倾向于过度自信回答，遮挡下平均准确率约 30%，视角歧义下低于 10%；(2) 即使提供额外视角，部分模型在识别可靠视角上接近随机水平。

Conclusion: 研究呼吁超越单纯追求答案正确性，转向评估模型是否知道何时应拒绝回答以及如何寻求可靠证据。

Classification reason: 该论文构建了 SpatialUncertain 评测框架，系统评估 VLM 在空间推理中面对遮挡和视角歧义时是否知道何时应拒绝回答，属于多模态评测与基准方向，与 canonical 子主题 multimodal_benchmark 高度匹配。

Importance reason: Focuses on VLM spatial reasoning limitations, not on the user's priority subtopics like continuous VLM or autoregressive generation. Signals: VLM spatial reasoning, occlusion and perspective ambiguity, overconfident answering, abstention evaluation, not aligned with user's priority subtopics
...
### [175] [PInVerify: An Offline Embodied Benchmark for Active Instance Verification](https://arxiv.org/abs/2605.30639)
*Yuhang Jiang*
...
Importance: 值得关注 (73.01) - 未入选精读

TL;DR: 本文提出 Active Instance Verification (AIV) 任务，要求具身智能体通过主动选择视角对候选物体进行细粒度语义验证，并构建了 PInVerify 离线基准（3000 个场景、18 个类别），基于开源 MLLM 构建基线，最佳模型（LoRA 微调）达到 85.6% 准确率。


<details>
  <summary>Details</summary>
Motivation: 现有具身导航任务中，智能体到达目标位置后无法保证找到正确的实例，因为细粒度属性差异（如"白色花卉"vs"白色条纹"）需要近距离多视角检查。目前缺乏一个标准化的基准来评估智能体在主动视角选择下的细粒度实例验证能力。

Method: 形式化 AIV 为有限时域决策过程，构建 PInVerify 基准：3000 个评估场景、18 个物体类别、6 扇区导航拓扑（含陷阱视角和不可达扇区）。基于开源 MLLM（Qwen3-VL 4B/8B、SenseNova-SI-1.2-InternVL3-8B 等）构建无训练流水线和 LoRA 微调端到端智能体，包含属性分解、可见性加权多视角追踪器和三种 next-best-view 策略。

Result: 最佳 MLLM 基线超过最佳嵌入基线 4.9 个百分点；GT-box 消融显示 +3.1 个百分点的检测差距；在测试的 NBV 策略中未观察到可靠的主动视角选择增益；LoRA 微调智能体（SFT+GSPO）达到 85.6% 准确率。

Conclusion: PInVerify 为具身 AI 中的主动细粒度语义验证提供了标准化基准，实验表明 MLLM 在该任务上优于嵌入模型，但主动视角选择策略尚未带来可靠提升，为未来研究指明了方向。

Classification reason: 该论文提出了 PInVerify，一个面向具身智能中主动实例验证（AIV）任务的离线评测基准，包含 3000 个评估场景和多视角数据。这属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）这一规范主题，因为其核心贡献是构建了一个新的多模态具身评测基准。

Importance reason: Focuses on embodied AI benchmark for instance verification, which is peripheral to the user's core interests in generative models and continuous representations. Signals: Embodied AI benchmark, Active Instance Verification, MLLM baseline, Offline evaluation, Not generative model
...
### [176] [BilliardPhys-Bench: Benchmarking Physical Reasoning and Visual Dynamics of Multimodal LLMs](https://arxiv.org/abs/2605.30900)
...
TL;DR: 本文提出 BilliardPhys-Bench，一个基于合成台球环境的物理推理基准，用于评估多模态大模型从单张图像预测物体运动与交互的能力。该基准通过程序化引擎生成随机场景，测试碰撞预测、墙壁反弹和最终位置估计三项能力。评估了 GPT、Claude、Gemini、Qwen 等系列模型，发现随着模拟时间和场景复杂度增加，性能显著下降，并观察到模型存在"静止偏差"——当正确物理结果难以推断时，模型倾向于预测无交互。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在静态图像识别上表现良好，但在直觉物理推理方面仍然薄弱，从单张图像预测物体如何运动与交互对现有系统来说仍然困难。作者旨在系统性地评估和揭示 MLLM 在视觉动力学推理上的不足。

Method: 构建 BilliardPhys-Bench 基准，使用程序化引擎生成带有摩擦和弹性碰撞的随机台球场景。基准测试三项能力：(1) 预测球与球碰撞，(2) 推理墙壁反弹，(3) 估计运动停止后球的最终位置。评估了 GPT、Claude、Gemini、Qwen 系列的最新 MLLM。

Result: 随着模拟时间增加和场景几何复杂度提升，模型性能显著下降。发现一个一致的失败模式——"静止偏差"（stasis bias）：当正确物理结果更难推断时，模型倾向于预测无交互发生。

Conclusion: 这些发现揭示了当前 MLLM 在视觉动力学推理上的关键薄弱环节，表明多模态架构需要更好的物理归纳偏置来提升物理推理能力。

Classification reason: 该论文提出了 BilliardPhys-Bench，一个用于评估多模态大模型物理推理能力的基准，属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题。

Importance reason: Focuses on physical reasoning benchmark for MLLMs, not directly on user's priority subtopics like continuous VLM or autoregressive generation. Signals: Physical reasoning benchmark, MLLM evaluation, stasis bias, synthetic billiards, not aligned with user's priority subtopics
...
### [177] [Does Visual Information Play a Decisive Role in Vision-Language-Action Model Driving Behavior?](https://arxiv.org/abs/2605.31041)
...
TL;DR: 本文提出一个结构化多层级视觉扰动框架，系统分析VLA（视觉-语言-动作）自动驾驶模型对视觉信息的依赖程度，发现模型在不同抽象层级上的视觉依赖不均衡，且开环与闭环评估结果存在差异。


<details>
  <summary>Details</summary>
Motivation: 现有VLA驾驶模型评估主要关注聚合性能指标，缺乏结构化的诊断工具来量化模型行为对视觉信息的依赖程度，导致对模型如何利用视觉信息进行决策的理解不足。

Method: 提出一个多层级视觉扰动框架，沿三个互补维度组织受控视觉扰动：通道级退化、信息级破坏和结构级修改。在开环轨迹预测和交互式闭环安全评估两种设置下，系统性地扰动视觉输入并观察VLA驾驶模型的行为响应变化。

Result: 实验揭示了评估依赖的依赖模式：开环和闭环评估下模型对视觉扰动的敏感度不同；模型在不同抽象层级上的视觉信息利用不均衡，某些层级对视觉扰动的鲁棒性更强。

Conclusion: 当前VLA驾驶模型对视觉信息的利用存在不均匀的视觉接地现象，需要更结构化的分析和更原则性的模型设计来理解视觉信息如何塑造驾驶行为，从而开发更安全、更鲁棒的系统。

Classification reason: 该论文提出了一种结构化多层级视觉扰动框架，用于系统分析VLA驾驶模型对视觉信息的依赖程度，本质上是一种针对VLA模型行为的新型评测协议。这与[多模态评测与基准]（multimodal_benchmark）主题高度吻合，该主题涵盖多模态评测、新基准和评测协议。

Importance reason: Systematic analysis of VLA visual grounding is valuable but focuses on autonomous driving, not the user's priority subtopics. Signals: VLA model analysis, visual perturbation framework, open-loop vs closed-loop evaluation, autonomous driving focus, not aligned with priority subtopics
...
### [178] [TeachObs: A Human-Validated Benchmark for Multimodal Teaching Observation and Model Evaluation](https://arxiv.org/abs/2605.30673)
...
TL;DR: TeachObs 是一个经人工验证的多模态课堂教学观察基准，包含来自8个国家的30节公开课视频，划分为5158个15秒场景，由7位研究者标注39个二元观察编码（20个视觉编码+19个非视觉编码），并提供了课程级专家评分。基于该基准评估了5个前沿视觉 LLM，发现没有单一模型在所有三个评测轨道上一致最优，且添加视频帧会同时增加正确和错误归因。


<details>
  <summary>Details</summary>
Motivation: 课堂教学视频中包含丰富的可观察教学行为，但其教学和视觉信号很少以适合模型评估的形式组织。现有基准缺乏对课堂教学视频中细粒度教学行为的系统标注和多层次评估，亟需一个经人工验证的基准来评估多模态模型在教学观察中的能力。

Method: 收集30节来自8个国家的公开课视频，分割为5158个15秒场景。7位研究者对每个场景标注39个二元观察编码（20个视觉编码如手势、板书、指向、视觉材料，19个非视觉编码如指令、监控、提问、反馈、反思）。基于 Krippendorff's alpha 构建可靠性感知的金标准片段标签。3位专家对30节课进行课程级评分和定性评估。使用5个视觉 LLM 在三个评测轨道（纯文本片段编码、文本+帧片段编码、LLM-as-judge 课程级评估）上进行评估。

Result: 没有单一模型在所有三个评测轨道上一致最优；添加视频帧会同时增加正确和错误归因；模型评估倾向于高估程序清晰的课程（相对于专家评分）。

Conclusion: TeachObs 支持细粒度标注基准测试和课程级评估，展示了 AI 系统在哪些方面可以辅助课堂视频分析，以及在哪些方面仍需专家判断。该基准覆盖了不同学科、课堂形式和标注难度，为多模态教学观察提供了标准化的评估平台。

Classification reason: 该论文提出了一个针对课堂教学视频的多模态评测基准 TeachObs，包含细粒度标注和课程级评估，属于多模态评测与基准（multimodal_benchmark）这一规范主题。同时该基准涉及 AIGC 模型（LLM）的评估，也匹配 AIGC 评测与基准（aigc_evaluation_benchmark）。

Importance reason: This paper is a domain-specific benchmark for classroom observation, which is far from the user's priority subtopics in multimodal generation and continuous VLM. Signals: Multimodal benchmark, classroom video, teaching observation, no direct link to generation or continuous VLM, negative keywords absent but topic mismatch
...
### [179] [FAM-Bench: A Multimodal Benchmark for Condition-Aware Food-as-Medicine Reasoning](https://arxiv.org/abs/2605.31410)
...
TL;DR: FAM-Bench 是一个多模态食物即药物（Food-as-Medicine）基准，包含2500个营养专家验证的实例，覆盖13种饮食相关健康状况，通过菜肴适宜性评估和比较分析两个任务，评测语言和视觉语言模型在健康约束下的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有食物AI基准主要评估菜肴识别、食谱理解、营养估计或一般营养问答，缺乏对模型能否根据具体健康状况判断食物选择是否合适的健康感知决策层的评测。

Method: 构建了包含2500个营养专家验证实例的多模态基准，涵盖13种饮食相关健康状况。设计两个互补任务：1）菜肴级适宜性评估——模型根据图像和配料列表判断菜肴是否适合某种健康状况；2）比较菜肴分析——模型对四个候选菜肴按条件适宜性排序。两个任务都需要整合配料证据、视觉烹饪线索和临床营养约束。

Result: 该基准为语言和视觉语言模型在健康感知推理方面提供了标准化测试平台，揭示了现有模型在食物即药物推理上的不足。

Conclusion: FAM-Bench填补了食物AI评测中健康感知决策层的空白，为未来开发更具健康意识的食物推理模型提供了标准化测试基准。

Classification reason: 该论文提出了FAM-Bench，一个面向"食物即药物"场景的多模态评测基准，包含两个任务（菜肴适宜性评估和比较分析），属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）这一规范主题。

Importance reason: Niche food-as-medicine benchmark with limited overlap with user's priority subtopics on continuous VLM, video/image generation, or world models. Signals: Multimodal benchmark, food-as-medicine, health-aware reasoning, no autoregressive or diffusion generation, no continuous VLM or latent flow
...
TL;DR: 本文提出TouchSafeBench，一个基于Habitat 3.0物理仿真的基准，用于评估视觉语言模型（VLM）在安全人机协作中的"碰撞接地"能力——即判断机器人是否与场景或人发生碰撞或即将碰撞。实验表明，当前前沿VLM在该任务上表现不佳（最佳Macro-F1低于50%），视觉流畅性不等于物理可靠性。


<details>
  <summary>Details</summary>
Motivation: 安全人机协作需要监控系统不仅具备视觉描述能力，还需判断机器人是否与场景/人发生碰撞或即将碰撞。现有VLM缺乏这种"碰撞接地"能力的系统评估，因此需要专门的基准来揭示这一局限性。

Method: 在Habitat 3.0中构建包含2,940个室内共现场景的TouchSafeBench基准，涵盖社交导航和社交重排两类任务，提供多视角RGB-D观测、俯视轨迹图、相机元数据和仿真接触标签。定义两个任务：当前安全状态分类和即将碰撞预警。评估三个前沿/机器人VLM和九种视觉表征。

Result: 最佳平均Macro-F1低于50%；显式深度信息并未自动转化为机器人碰撞证据；机器人与场景的接触检测比人机接触更难。当前VLM在碰撞接地任务上远未达到可靠水平。

Conclusion: 视觉流畅性不等于物理可靠性。可靠的机器人安全监控需要显式绑定视角、机器人形态、度量几何和未来碰撞的表征，而非仅依赖视觉语言模型的表面理解能力。

Classification reason: 该论文提出了TouchSafeBench，一个用于评估VLM碰撞接地能力的物理仿真基准，属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。同时涉及机器人安全监控，也匹配AIGC方向下的生成内容安全与治理（aigc_safety_governance）。

Importance reason: Focuses on safety evaluation for human-robot collaboration, which is peripheral to the user's core interests in generative models and efficient inference. Signals: robotics safety benchmark, VLM collision grounding, low model performance, not generative, no autoregressive or diffusion focus
...
### [181] [ERGeoBench:A Comprehensive Benchmark for Embodied Reasoning and Geo-localization in Multimodal Large Language Models](https://arxiv.org/abs/2605.31251)
...
TL;DR: ERGeoBench是一个面向具身地理定位的多模态大模型诊断基准，包含2207个全球街景全景图，在单视图、全景视图和具身视图三种渐进设置下评估模型的感知、空间、常识和地理定位推理能力。实验表明现有MLLM能推断高层地理语义，但在细粒度感知、度量定位和跨视图空间一致性上仍存在困难。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在具身智能方面展现出潜力，但具身地理定位（embodied geo-localization）由于缺乏细粒度评测而未被充分探索。现有基准未能系统评估模型在主动获取观测（通过偏航、俯仰、缩放变化）条件下的地理定位能力。

Method: 构建ERGeoBench基准，包含2207个全球分布的街景全景图，设计三种渐进评测设置（单视图、全景视图、具身视图），测量四种互补能力（基础感知、空间意识、常识推理、地理定位推理），并对主流闭源和开源MLLM进行系统评估。

Result: 现有MLLM能推断高层地理语义（如国家、城市），但在细粒度感知操作、度量定位和跨视图空间一致性上表现不佳。地理定位能力与其他能力维度强相关，表明准确定位依赖于感知、空间推理和常识推理的整合。

Conclusion: ERGeoBench为诊断和推进类人具身地理定位提供了统一框架，揭示了当前MLLM在该任务上的关键瓶颈，为未来研究指明了方向。

Classification reason: 该论文提出了ERGeoBench，一个用于具身地理定位的多模态大模型诊断基准，属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。

Importance reason: The benchmark is well-constructed but focuses on geo-localization, which is not a priority subtopic and is flagged by negative keywords. Signals: Embodied geo-localization benchmark, MLLM diagnostic evaluation, fine-grained perception gap, not aligned with user's priority subtopics
...
### [182] [MineExplorer: Evaluating Open-World Exploration of MLLM Agents in Minecraft](https://arxiv.org/abs/2605.30931)
...
TL;DR: 本文提出 MineExplorer，一个用于评估多模态大语言模型（MLLM）智能体在 Minecraft 开放世界中探索能力的基准。通过筛选依赖 Minecraft 特定知识的原子任务，并组织为 ReAct 风格的多跳隐式任务，结合多智能体合成工作流自动构建任务图、沙盒场景和里程碑评估器。实验表明开放世界探索对当前 MLLM 智能体仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有具身和游戏基准通常将交互压缩为短视任务，或与领域特定的游戏机制纠缠不清，难以评估 MLLM 在动态开放世界中的持续探索能力。

Method: 1) 筛选依赖 Minecraft 特定知识的原子任务以反映通用开放世界推理；2) 围绕 ReAct 风格能力框架组织基准，将原子任务组合为隐式多跳任务；3) 使用多智能体合成工作流联合设计任务图、沙盒场景和基于规则的里程碑评估器。

Result: 人类评估表明多智能体合成工作流比单智能体基线产生更可靠的实例。实验显示开放世界探索仍具挑战性：强模型能处理许多单跳任务，但在需要协调隐藏先决条件的长轨迹任务中性能急剧下降。任务难度与智能体完成度相关，更大模型或思考模式并不一致地带来更好性能。

Conclusion: MineExplorer 基准有效揭示了当前 MLLM 智能体在开放世界探索中的局限性，为未来研究提供了有价值的评测平台和方向指引。
...
Importance reason: Evaluates MLLM agents in Minecraft but focuses on general exploration rather than the user's priority subtopics like continuous VLM or autoregressive generation. Signals: MLLM agent benchmark, open-world exploration, Minecraft, multi-hop tasks, not aligned with priority subtopics
...
### [183] [On the Robustness of Multilingual Text Embedding Rankings Across Learning Tasks, Languages, and Benchmark Datasets](https://arxiv.org/abs/2605.31142)
...
TL;DR: 本文对多语言文本嵌入模型在MTEB基准上的排名鲁棒性进行元研究，引入数据集组成鲁棒性和排名方案鲁棒性两个指标，分析五种语言、九类任务上的模型表现稳定性，发现大模型通常是鲁棒的顶尖模型，但在检索等任务上并非一致。


<details>
  <summary>Details</summary>
Motivation: 大规模多语言文本嵌入模型在研究和工业中广泛应用，但MTEB等基准测试中模型排名的优越性结论往往依赖于隐式的数据集组成选择和性能聚合方法，其鲁棒性尚未被充分理解。

Method: 采用多准则决策分析（MCDM）排名方案，引入数据集组成鲁棒性（对数据集组成变化的敏感性）和排名方案鲁棒性（对聚合方法变化的敏感性）两个指标，对英语、法语、德语、印地语、西班牙语五种语言在九类任务上进行系统敏感性分析，并额外发布约230种语言的结果。

Result: 任务特定分析表明，基于大规模LLM的模型通常是鲁棒的顶尖模型，但在检索任务上并非一致；任务无关分析显示，只有少数模型在所有任务、排名方案和数据子样本上保持稳定强表现。

Conclusion: 基准测试结论对数据集组成和排名方案的选择敏感，需要更系统的鲁棒性分析来确保模型评估的可靠性。

Classification reason: 该论文研究多语言文本嵌入模型在MTEB基准上的鲁棒性，分析不同任务、语言和数据集组合下排名稳定性的元评测问题，属于多模态评测与基准（multimodal_benchmark）范畴。

Importance reason: Focuses on multilingual embedding robustness, not aligned with user's priority on generative models and continuous VLM. Signals: Multilingual text embedding, MTEB benchmark, ranking robustness, meta-study, not generative
...
TL;DR: SVI-Bench 是一个大规模视频智能评测基准，利用团队体育作为动态微世界，包含约 35K 小时广播视频、15M 标注动作等数据，构建了从动态场景理解、因果推理、战略模拟到智能体合成的四层九任务评测体系。实验发现当前最强模型在感知任务上表现尚可（~73%），但在因果和战略推理任务上急剧下降，智能体任务仅达 5% 准确率，揭示了视频智能领域的能力断层。


<details>
  <summary>Details</summary>
Motivation: 现有视频基准要么使用真实世界视频但缺乏因果和战略问题的可验证真值，要么使用合成环境但牺牲了真实多智能体系统的复杂性。作者希望构建一个兼具真实世界复杂性和可验证性的基准，系统评估视频智能从感知到因果推理、模拟再到策略规划的完整能力栈。

Method: 利用团队体育（篮球、足球、冰球）作为动态微世界，通过数据引擎将原始比赛数据转化为密集交叉引用的语料库，包含广播视频、动作标注、专家解说、比赛报告和统计数据。设计了四层九任务的渐进式评测体系：动态场景理解（细粒度动作QA、事件定位）、因果推理（反事实QA、因果归因）、战略模拟（假设模拟、策略预测）、智能体合成（自主证据收集与整合）。

Result: 评估了多种强多模态和智能体基线模型，发现能力断层：感知任务上最佳模型达约73%准确率，但因果推理任务降至约40%，战略模拟任务约25%，智能体合成任务最强模型仅5%。模型在需要跨时间整合证据、反事实推理和自主决策的任务上表现极差。

Conclusion: SVI-Bench 揭示了当前视频智能模型在感知之外的高级认知能力上的严重不足，为未来研究提供了明确的方向——需要发展能够进行因果推理、战略模拟和自主决策的视频智能系统。

Classification reason: 该论文提出了 SVI-Bench，一个面向战略视频智能（从感知到因果推理、模拟再到策略规划）的大规模多模态评测基准，属于多模态评测与基准（multimodal_benchmark）这一规范主题。同时涉及视频理解与生成方向，因此也匹配 multimodal_generation 方向下的 generation_benchmark。

Importance reason: Strong benchmark for video intelligence but lacks direct alignment with user's priority subtopics like continuous VLM or autoregressive generation. Signals: Video intelligence benchmark, Multi-agent microworld, Causal reasoning gap, Agentic synthesis task, Strong perceptual but weak strategic models
...
TL;DR: EGOSTREAM是一个用于第一视角视频流式情景记忆评测的诊断基准，包含2,250个问题覆盖7个认知维度，并引入Answer Validity Window（AVW）概念来区分模型遗忘与自然世界状态变化。在统一Qwen3-VL骨干上评估多种记忆管理机制，发现所有方法均远低于实时处理速度，最高准确率仅约45%，揭示了当前架构的严重不足。


<details>
  <summary>Details</summary>
Motivation: 当前流式视频基准缺乏诊断模型记忆内容和记忆时长的工具，无法区分模型真正的遗忘与场景自然变化导致的答案失效，亟需一个细粒度的诊断基准来评估第一视角视频中的情景记忆能力。

Method: 构建EGOSTREAM基准，包含2,250个精心设计的问题，覆盖细节、空间、时间、事件、社会、因果和前瞻记忆7个认知维度。引入Answer Validity Window（AVW）概念，将问题扩展为8,528个回忆条件评估，支持从即时到超长期的受控测试。在统一Qwen3-VL骨干框架下比较多种记忆管理机制（滑动窗口、注意力汇、KV缓存剪枝、合并和卸载）。

Result: 不同记忆管理机制在相似总体准确率下表现出截然不同的记忆特征：token剪枝在保留细粒度细节和时间结构上显著优于token合并；量化卸载改善了超长期回忆。所有方法均远低于实时处理速度（>1秒/帧），最佳方法准确率仅约45%。

Conclusion: EGOSTREAM揭示了当前流式多模态大模型在情景记忆方面的严重不足，为未来改进提供了诊断测试平台。现有架构在记忆保持和处理效率上均存在关键差距，需要新的设计来弥合。

Classification reason: 该论文提出了EGOSTREAM，一个用于第一视角视频流式情景记忆评测的诊断基准，属于多模态评测与基准（multimodal_benchmark）这一规范主题。论文构建了2,250个问题，覆盖7个认知维度，并引入Answer Validity Window（AVW）概念，属于典型的多模态评测基准工作。

Importance reason: Strong diagnostic benchmark for egocentric video memory, but low alignment with user's priority on generative models and continuous VLM. Signals: streaming episodic memory, egocentric vision benchmark, Answer Validity Window, memory management mechanisms, Qwen3-VL backbone
...
TL;DR: nuReasoning 是一个面向长尾自动驾驶场景的大规模推理数据集与基准，包含 20,000 个 20 秒长的真实驾驶片段，提供空间推理、决策推理和反事实推理的人工验证标注。实验表明，在该数据集上微调 VLM 能显著提升驾驶问答性能，且将推理监督融入 VLA 训练可改善规划性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集和基准主要关注感知、预测或规划，对长尾场景中的推理能力（常识推理、空间关系理解、智能体交互推断、安全决策）缺乏充分的监督和评测。

Method: 基于 nuScenes/nuPlan 数据，构建包含 20,000 个 20 秒长驾驶片段的数据集，提供多相机图像、LiDAR、高清地图、目标标注以及人工验证的推理标注（空间推理、决策推理、反事实推理）。支持推理评测和规划评测，并设计了 VLM 微调和 VLA 训练的实验范式。

Result: 在 nuReasoning 上微调 VLM 显著提升了驾驶特定问答能力；将推理监督融入 VLA 训练后，即使在推理时禁用文本推理输出，规划性能仍得到改善。

Conclusion: nuReasoning 为评估和改进长尾场景中鲁棒、可解释、推理驱动的自动驾驶系统提供了基础性数据集和基准。

Classification reason: 该论文提出了 nuReasoning，一个面向自动驾驶长尾场景推理的大规模真实世界数据集与基准，包含空间推理、决策推理和反事实推理标注，属于多模态评测与基准范畴，与 [CANONICAL] subtopic "多模态评测与基准" 高度匹配。

Importance reason: Focuses on autonomous driving reasoning, not on the user's priority subtopics like continuous VLM or video generation. Signals: Long-tail autonomous driving, reasoning dataset, VLM fine-tuning, planning improvement, not aligned with user's core interests
...
### [187] [Recognizing Co-Speech Gestures in-the-Wild](https://arxiv.org/abs/2605.31589)
...
TL;DR: 本文提出了 GRW（Gesture Recognition in the Wild）数据集，这是首个大规模、细粒度标注的共语手势识别基准，包含 156,688 个视频片段，覆盖 150 个词汇的物理动作、空间描述和抽象概念。基于该数据集，论文训练视频模型完成三项任务：手势语义分类、手势对应词汇识别和手势时间定位。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型难以捕捉与语音语义相关的共语手势（co-speech gestures），主要瓶颈在于缺乏精确标注的训练数据。现有数据集规模小、类别有限，无法支撑细粒度的语义手势识别研究。

Method: 构建 GRW 数据集：从大规模视频语料中收集并人工标注 156,688 个视频片段，每个片段标注手势对应的具体词汇（150 词分类体系）及帧级时间边界。基于该数据集，训练视频模型完成三个任务：(a) 手势是否为语义手势的二分类；(b) 识别手势对应的词汇；(c) 时间定位手势起止帧。

Result: 建立了三个任务的基准评测结果，展示了当前视频模型在共语手势识别上的性能水平，并验证了 GRW 数据集对训练手势识别模型的有效性。

Conclusion: GRW 数据集填补了共语手势识别领域大规模精细标注数据的空白，为多模态理解中的人体动作与语言对齐研究提供了重要资源，有望推动更细粒度的多模态语义理解发展。

Classification reason: 该论文提出了首个大规模共语手势识别基准数据集 GRW，包含 156,688 个精细标注的视频片段，覆盖 150 个词汇的手势分类与时间定位任务，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准"。

Importance reason: Focuses on gesture recognition benchmark, not aligned with priority subtopics like continuous VLM, video generation, or world models. Signals: New gesture dataset, multimodal understanding, not autoregressive or diffusion, no visual latent flow, no world model
...
### [188] [SOCO: Benchmarking Semantic Object Correspondence in Vision Foundation Models](https://arxiv.org/abs/2605.31597)
...
TL;DR: SOCO是一个新的语义对象对应（Semantic Object Correspondence）基准，包含100个类别、超过100万对应标注对，并引入对应类型分类法。实验发现视觉基础模型编码了强语义结构但跨类别对应迁移差，LVLMs在文本引导定位上强于跨图像视觉匹配，且对应性能比ImageNet分类更能预测密集下游任务表现。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉基础模型评估缺乏对结构化对象理解（特别是部件级语义对应）的一致评测协议，且缺乏大规模、细粒度的部件级标注基准。

Method: 构建SOCO基准：定义对应类型分类法（实例级、类别级、跨类别等），在100个类别上提供一致的功能性关键点标注，超过100万对应对，并为关键点提供语言描述以支持LVLMs评估。

Result: (i) 视觉基础骨干编码了强语义结构但跨类别对应迁移差；(ii) LVLMs在文本引导部件定位上强于跨图像视觉匹配；(iii) 对应性能比ImageNet分类更能预测分割、跟踪、3D姿态估计和3D检测等密集下游任务。

Conclusion: SOCO可作为评估视觉和多模态基础模型中结构化、部件级表示质量的基准，揭示了当前模型在细粒度视觉对应上的关键差距。

Classification reason: 该论文提出了SOCO基准，用于评估视觉基础模型和视觉语言模型在语义对象对应（semantic object correspondence）上的细粒度部件级理解能力，属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。

Importance reason: Focuses on vision benchmark for object correspondence, not directly on user's priority subtopics like continuous VLM or autoregressive generation. Signals: Semantic correspondence benchmark, vision foundation models, LVLM evaluation, part-level understanding, dense downstream task prediction
...
### [189] [A Visually Impaired Assistance Benchmark for VLM-as-a-Judge Evaluation](https://arxiv.org/abs/2605.31351)
...
TL;DR: 本文提出 VIABLE，首个面向视障辅助（VIA）场景的 VLM-as-a-Judge 评测基准，包含 30 万+ 判断样本和 Effectiveness-Impartiality-Stability 评估框架。实验发现现有 VLM 评判者普遍不可靠，最强模型 GPT-5.4 单故障诊断准确率仅 52.6%，自偏好率高达 94.2%。作者提出 VIA-Judge-Agent，通过视觉证据提取和分类引导工作流提升诊断准确率和下游响应质量。


<details>
  <summary>Details</summary>
Motivation: AI 辅助视障（VIA）任务依赖人工评估，成本高昂。VLM-as-a-Judge 范式在通用领域有潜力，但在 VIA 场景中是否可信尚不清楚。本文旨在系统评估 VLM 评判者在 VIA 任务中的可靠性。

Method: 构建 VIABLE 基准，包含 300K+ 判断样本，覆盖三个场景；提出 Effectiveness-Impartiality-Stability 评估框架和 12 种故障分类体系。基于该基准系统评估 7 个不同规模的 VLM 评判者。提出 VIA-Judge-Agent，一种模型无关的推理时增强框架，通过视觉证据提取和分类引导工作流提升评判质量。

Result: 现有 VLM 评判者在所有评估维度上均不可靠：最强模型 GPT-5.4 单故障诊断准确率仅 52.6%，自偏好率 94.2%；开源模型存在严重偏见和对抗脆弱性。VIA-Judge-Agent 在诊断准确率和下游 VIA 响应偏好上均取得正向提升。

Conclusion: VLM-as-a-Judge 在 VIA 场景中尚不可靠，需要专门的增强方法。VIA-Judge-Agent 展示了通过推理时增强提升评判可靠性的可行路径，为 VIA 领域的自动评估提供了新方向。

Classification reason: 该论文提出了 VIABLE，首个面向视障辅助（VIA）场景的 VLM-as-a-Judge 评测基准，属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题。同时涉及 AIGC 评测（aigc_evaluation_benchmark）方向，因为其核心是评估生成式 VLM 作为评判者的可靠性。

Importance reason: Focuses on VLM-as-a-Judge for visually impaired assistance, which is a niche evaluation benchmark not aligned with your priority subtopics like continuous VLM, video generation, or world models. Signals: VLM-as-a-Judge benchmark, Visually impaired assistance, Evaluation framework, Low diagnostic accuracy, Self-preference bias
...
### [190] [TRINE: A Token-Aware, Runtime-Adaptive FPGA Inference Engine for Multimodal AI](https://arxiv.org/abs/2603.22867)
...
TL;DR: TRINE是一个面向多模态AI的FPGA推理加速引擎与编译器，通过统一计算模式、运行时模式切换、token剪枝和依赖感知的层卸载，在单比特流中实现端到端多模态推理，相比RTX 4090降低22.57倍延迟，功耗仅20-21W。


<details>
  <summary>Details</summary>
Motivation: 多模态模型（ViT、CNN、GNN、Transformer NLP）在嵌入式平台上运行时，计算和内存模式差异大，且硬实时目标要求严格，现有硬件方案难以高效支持端到端多模态推理。

Method: TRINE将各层统一为DDMM/SDDMM/SpMM计算模式，映射到可模式切换的引擎上，在共享PE阵列中运行时切换权重/输出驻留脉动阵列、1xCS SIMD和可路由加法树（RADT）。采用宽度匹配的两阶段top-k单元实现流式token剪枝，依赖感知的层卸载（DALO）在可重构处理单元间重叠独立内核以维持利用率。

Result: 在Alveo U50和ZCU104上评估，TRINE相比RTX 4090延迟降低22.57倍，相比Jetson Orin Nano降低6.86倍，功耗仅20-21W；token剪枝在ViT-heavy流水线上带来最高7.8倍加速；DALO贡献最高79%吞吐提升；int8量化下精度损失<2.5%。

Conclusion: TRINE通过统一的硬件架构和编译器设计，在单比特流中高效支持视觉、语言和图三种模态的端到端推理，实现了领先的延迟和能效，为多模态AI在嵌入式平台上的部署提供了可行方案。

Classification reason: 该论文提出面向多模态AI的FPGA推理加速引擎，核心贡献在于通过硬件加速、量化、token剪枝等技术提升多模态模型推理效率，属于多模态高效化与轻量化（efficient_multimodal）这一[CANONICAL]子主题。同时与多模态生成方向下的efficient_generation也有一定关联。

Importance reason: Strong FPGA acceleration for multimodal inference with token pruning and runtime adaptation, but focuses on hardware efficiency rather than core multimodal understanding or generation. Signals: FPGA inference engine, multimodal AI acceleration, token pruning, runtime mode switching, dependency-aware layer offloading
...
### [191] [VisionPulse: Dynamic Visual Sparsity for Efficient Multimodal Reasoning](https://arxiv.org/abs/2605.31457)
...
TL;DR: VisionPulse 提出了一种在 LMM 推理过程中按步骤动态剪枝视觉 token 的方法。通过轻量级视觉注意力质量估计每步的保留预算，仅保留最关键的 5% 视觉 token，同时将推理轨迹缩短 11.2%，且几乎不损失精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常在预填充阶段一次性剪枝视觉 token，假设推理过程中所需的视觉证据是静态不变的。然而本文实验发现，视觉证据具有强烈的步骤依赖性：每个解码步骤仅需少量关键视觉 token，且关键集合随推理过程动态变化。此外，冗余的视觉上下文会引导模型关注与查询无关的区域，导致推理轨迹变长。因此需要一种按步骤动态剪枝视觉 token 的方法。

Method: VisionPulse 包含两个核心组件：(1) 轻量级视觉注意力质量（visual attention mass），用于估计每步的 token 保留预算，该质量与 LMM 有效视觉 token 使用率呈强正相关；(2) 在该预算下仅保留最关键的视觉 token。通过推理过程中强制视觉稀疏性，过滤冗余视觉上下文同时保留相关证据，从而自然缩短推理轨迹。

Result: 实验表明，VisionPulse 每步仅保留 5% 的视觉 token，推理轨迹缩短 11.2%，同时准确率几乎保持不变。在多个多模态推理基准上验证了方法的有效性。

Conclusion: VisionPulse 验证了视觉证据在 LMM 推理中的步骤依赖性，并提出了一种轻量级、动态的视觉 token 剪枝框架，在不牺牲性能的前提下显著提升推理效率，为多模态模型的部署提供了实用方案。

Classification reason: 该论文提出 VisionPulse，一种在推理过程中动态剪枝视觉 token 的方法，旨在提升多模态大模型（LMM）的推理效率。这属于多模态高效化与轻量化（efficient_multimodal）这一 [CANONICAL] 子主题，该子主题涵盖 token 压缩、稀疏推理等效率优化方法。同时论文也涉及多模态推理，但核心贡献是效率优化，因此归入该子主题。

Importance reason: Directly addresses efficient multimodal reasoning via dynamic visual sparsity, aligning with user's high priority on model efficiency and continuous VLM reasoning. Signals: dynamic visual token pruning, step-wise reasoning efficiency, multimodal LMM inference acceleration, strong relevance to efficient inference and multimodal latent reasoning, no negative keywords
...
### [192] [Linear Scaling Video VLMs for Long Video Understanding](https://arxiv.org/abs/2605.31598)
...
TL;DR: StateKV 是一种推理时方法，通过固定容量的重要性循环状态携带跨帧上下文，将长视频 VLM 的视频预填充从二次复杂度降至线性，在多个长视频基准上接近全自注意力性能，且无需微调或架构改动。
...
Method: StateKV 设计了一个固定容量的、基于重要性的循环状态来跨帧传递上下文（视频预填充阶段线性复杂度），同时保留第二个完整的逐帧缓存用于解码阶段。该方法无需微调或架构改动，可直接适配预训练的长视频 VLM。

Result: 在三个长视频基准、七个模型（涵盖三个模型家族和多个规模）上，StateKV 性能接近全自注意力，并一致优于滑动窗口/基于近因的流式近似方法。同时降低了视频预填充的 FLOPs，在固定计算预算下可使用更大模型获得更强精度。

Conclusion: StateKV 提供了一种实用的、无需训练的推理时方法，有效解决了长视频 VLM 的可扩展性问题，为长视频理解走向实际应用迈出了可行的一步。

Classification reason: 该论文提出 StateKV 方法，通过固定容量的循环状态将长视频 VLM 的预填充复杂度从二次降至线性，属于多模态模型的高效推理优化，与 [CANONICAL] 多模态高效化与轻量化（efficient_multimodal）高度匹配。

Importance reason: Directly addresses efficient inference for long video understanding, a key priority in model efficiency and multimodal understanding. Signals: linear scaling video prefill, state-based cross-frame context, no fine-tuning needed, strong benchmark results, aligns with efficient inference priority
...
### [193] [VLM3: Vision Language Models Are Native 3D Learners](https://arxiv.org/abs/2605.30561)
...
TL;DR: VLM3 提出了一种极简方法，通过焦距统一、文本像素引用和数据混合缩放，使标准VLM无需架构修改即可掌握深度估计、像素对应、相机位姿估计和物体级3D理解等多种3D任务，在深度估计上达到0.9的准确度，匹配专家视觉模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D理解严重依赖专家视觉模型和复杂的任务特定设计，而VLM虽然在语义理解上表现出色，但在3D任务上能力有限。作者希望证明VLM本身就是天然的3D学习者，只需简单设计即可掌握多种3D任务。

Method: VLM3 的核心方法包含三个关键设计：(1) 焦距统一（focal length unification），将不同焦距的图像统一到标准视角；(2) 基于文本的像素参考（text-based pixel reference），用文本坐标表示像素位置；(3) 数据混合与缩放（data mixture and scaling），混合多种3D任务数据进行训练。无需改变模型架构、使用大模型、复杂数据增强或回归损失函数。

Result: VLM3 在深度估计任务上将准确率从0.84提升至0.9，同时在像素对应、相机位姿估计和物体级3D理解等任务上达到与专家视觉模型相当的精度，且保持了标准VLM架构和纯文本训练范式。

Conclusion: VLM3 证明了标准VLM通过简单设计即可成为天然的3D学习者，为简单可扩展的3D学习开辟了新范式，挑战了"3D任务需要复杂专家模型"的传统认知。

Classification reason: 该论文研究如何让标准VLM掌握3D深度估计、像素对应、相机位姿估计等3D任务，属于多模态理解与生成方向下的"空间与3D多模态推理"（spatial_3d_reasoning）这一[CANONICAL]子主题。同时论文涉及3D生成式理解能力，也匹配多模态生成方向。

Importance reason: Shows standard VLMs can master 3D tasks without architectural changes, aligning with your priority on unified multimodal models and efficient inference. Signals: Unified multimodal model, 3D understanding without architecture change, Focal length unification, Text-based pixel reference, Data mixture scaling
...
### [194] [SpatialAct: Probing Spatial Reasoning-to-Action Capabilities of VLM Agents in 3D Scenes](https://arxiv.org/abs/2605.31148)
...
TL;DR: 本文提出 SpatialAct，一个基于模拟器的基准，用于探测 VLM 智能体在 3D 场景中的动作条件空间推理能力。实验揭示了当前 VLM 在孤立空间推理任务上表现良好，但在多轮交互中难以维持连贯的空间信念并产生可靠动作，存在明显的推理到动作的鸿沟。


<details>
  <summary>Details</summary>
Motivation: 人类能够轻松感知空间布局、形成认知表征、推理空间关系并将推理转化为行动。尽管 VLM 在观察条件下的空间感知和推理任务上表现良好，但它们是否能够构建连贯的空间理解、据此行动并通过多轮反馈优化动作，仍不清楚。本文旨在系统性地研究这一问题。

Method: 提出了 SpatialAct 基准，包含三个层次的任务设置：(1) 最具挑战性的多轮交互式精炼（Multi-turn Interactive Refinement）；(2) 其分解版本——单步错误检测与修复（Single-step Error Detection and Fix）；(3) 五个基础空间能力任务（如空间关系推理、物体定位等），用于诊断模型失败的潜在原因。所有任务均在 3D 模拟器环境中进行。

Result: 实验揭示了明显的推理到动作鸿沟：当前 VLM 在孤立的空间推理任务上表现良好，但在多轮反馈中难以维持连贯的空间信念并产生可靠动作，性能显著低于人类。这表明即使抽象掉了底层控制，当前 VLM 智能体在动作引起的环境变化下仍缺乏鲁棒的空间状态跟踪能力。

Conclusion: 当前 VLM 智能体在 3D 场景中的空间推理到动作能力存在显著不足，尤其是在多轮交互场景中。这为未来研究指明了方向——需要提升 VLM 在动态环境中的空间状态跟踪和动作推理能力。

Classification reason: 该论文研究 VLM 智能体在 3D 场景中的空间推理到动作的能力，并提出了一个基于模拟器的基准 SpatialAct。这属于多模态理解与生成方向下的"空间与 3D 多模态推理"（spatial_3d_reasoning）规范主题，该主题涵盖空间推理、3D 视觉定位、具身推理等，与本论文高度匹配。

Importance reason: Probes VLM spatial reasoning-to-action gap in 3D scenes, relevant to multimodal understanding but not a top priority subtopic. Signals: VLM spatial reasoning, 3D scene benchmark, reasoning-to-action gap, multi-turn interaction, simulator-grounded evaluation
...
### [195] [Multi-Turn Multi-Agent Dialogue for Collaborative Reconstruction Improves VLM Performance on Spatial Reasoning, But Only Barely](https://arxiv.org/abs/2605.31387)
...
TL;DR: 本文通过一个协作式结构重建任务，研究VLM在多轮多智能体对话中的空间推理能力。实验发现VLM在视觉空间推理上表现有限，详细文本描述比视觉输入更有效，分解图像表示略有帮助，但整体性能仍不理想。


<details>
  <summary>Details</summary>
Motivation: 机器人在多样化环境中依赖视觉输入理解物体和空间布局，并需要与人类通过语言协作。现有VLM在视觉问答和指令跟随方面有进展，但在需要空间推理的协作对话任务中的能力尚未被充分探索。

Method: 设计了一个协作式结构重建任务框架：VLM通过多轮对话（多智能体交互）从视觉和文本输入中重建目标结构。评估了开源和闭源VLM在不同交互设置、输入模态和图像表示下的表现。

Result: VLM在视觉表示的空间推理上仍然困难；详细文本描述比视觉输入带来更高的重建成功率；分解的图像表示（如分块图像）能提升性能，但提升幅度有限。

Conclusion: 当前VLM在协作式空间推理任务中存在显著局限，视觉空间接地和接地指令生成能力不足，需要进一步改进才能用于真实机器人协作场景。

Classification reason: 该论文研究VLM在协作式结构重建任务中的空间推理能力，属于多模态空间推理范畴，与[空间与 3D 多模态推理]（spatial_3d_reasoning）这一CANONICAL子主题高度匹配。

Importance reason: Focuses on spatial reasoning in VLMs but shows limited performance and lacks direct connection to priority subtopics like continuous VLM or video generation. Signals: spatial reasoning, VLM dialogue, limited performance, not aligned with priority subtopics, negative keywords absent
...
### [196] [VLM-GLoc: Vision-Language Model Enhanced Monte Carlo Localization for Robust Semantic Global Localization in Cluttered Quasi-Static Environments](https://arxiv.org/abs/2605.30506)
...
TL;DR: VLM-GLoc 提出一种结合视觉语言模型（VLM）与蒙特卡洛定位（MCL）的分层语义全局定位方法，在几何混淆、准静态环境（如杂货店、实验室）中通过VLM提取判别性语义特征、隐式质量过滤和永久性推理增强数据增强，实现70%-74%的全局定位成功率，显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 在杂货店、办公室等几何结构重复、语义分布长尾的准静态环境中，传统基于几何特征或专用视觉管线的全局定位方法难以应对语义模糊和瞬态视觉杂乱，亟需一种统一、鲁棒的语义定位方案。

Method: 提出VLM-GLoc框架，将开放词汇VLM作为统一的语义观测前端，用于：(1) 提取高判别性的丰富文本特征；(2) 隐式过滤模糊或动态物体的质量；(3) 通过永久性推理进行针对性数据增强。引入逆语义提议机制，通过文本到地图检索来播种粒子，实现分层语义蒙特卡洛定位。

Result: 在3,500平方英尺的杂货店（手机平台）和3,700平方英尺的实验室空间（四足机器人平台）上分别达到70%和74%的全局定位成功率，大幅超越传统纯几何方法和领域专用基线。

Conclusion: VLM-GLoc验证了VLM作为统一语义前端在机器人全局定位中的有效性，为在几何混淆、语义丰富的准静态环境中实现鲁棒定位提供了新范式。

Classification reason: 该论文利用VLM增强机器人全局定位，属于具身推理与空间定位范畴，与[空间与 3D 多模态推理]（spatial_3d_reasoning）高度匹配。

Importance reason: Focuses on robotics localization, not aligned with user's priority subtopics in multimodal generation or video/image generation. Signals: VLM for robotics, semantic global localization, Monte Carlo localization, quasi-static environments, not autoregressive or diffusion generation
...
### [197] [Representation Forcing for Bottleneck-Free Unified Multimodal Models](https://arxiv.org/abs/2605.31604)
...
TL;DR: 本文提出 Representation Forcing (RF) 技术，通过让解码器在像素生成前自回归预测视觉表征作为中间 token，使统一多模态模型无需外部 VAE 即可在像素空间直接生成高质量图像，同时提升理解与生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型（UMM）依赖冻结的、单独预训练的 VAE 进行图像生成，这引入了结构瓶颈，限制了模型的端到端能力。直接移除 VAE 会导致生成质量下降，因为模型需要同时学习高层结构和低层像素细节。作者希望消除这一瓶颈，实现真正的无瓶颈统一多模态模型。

Method: 提出 Representation Forcing (RF) 技术：在解码阶段，模型先自回归地预测视觉表征（如 DINOv2 特征）作为中间 token，这些 token 保留在上下文中，然后作为条件引导同一骨干网络内的像素扩散过程。通过将表征从感知输出转变为生成目标，RF 消除了对外部生成潜空间（如 VAE）的需求。

Result: 在图像生成任务上，基于像素空间的 RF 模型达到了与基于 VAE 的最先进统一模型相当的性能。在图像理解任务上，像素空间的 RF 模型通常优于其基于 VAE 的变体。实验表明 RF 同时提升了理解和生成能力。

Conclusion: Representation Forcing 是一种有效的技术，能够消除统一多模态模型中的 VAE 结构瓶颈，实现端到端的无瓶颈统一多模态模型，同时提升理解和生成性能，为未来更简洁的统一多模态架构提供了可行方向。

Classification reason: 该论文提出 Representation Forcing 方法，旨在消除统一多模态模型中外部 VAE 的结构瓶颈，实现端到端的理解与生成一体化。这与 multimodal_understanding_generation 方向下的 [CANONICAL] 子主题 "统一多模态模型"（unified_multimodal_model）高度匹配，同时与 multimodal_generation 方向下的同名子主题也相关。

Importance reason: Directly advances bottleneck-free unified multimodal models with a novel representation forcing technique that boosts both understanding and generation. Signals: Unified multimodal model, bottleneck-free, representation forcing, pixel-space generation, autoregressive visual tokens
...
### [198] [How can embedding models bind concepts?](https://arxiv.org/abs/2605.31503)
...
Importance: 值得关注 (74.84) - 未入选精读

TL;DR: 本文研究视觉语言嵌入模型（如 CLIP）在多物体场景中的概念绑定能力。发现 CLIP 的嵌入可加性分解为物体表征，但其绑定函数复杂度高，导致图像与文本编码器无法学习共享的绑定机制以泛化到未见概念组合。通过从零训练的受控 Transformer 实验证明，足够的数据覆盖可使模型学习低复杂度的乘法交互绑定函数，实现系统性泛化。


<details>
  <summary>Details</summary>
Motivation: 人类能轻松将颜色与形状正确绑定（如"红色圆形"），但 CLIP 等视觉语言模型在跨模态检索中表现出"概念袋"行为，无法正确绑定概念。理解这一缺陷的根本原因并探索解决方案，对于提升多模态模型的组合泛化能力至关重要。

Method: 首先通过理论分析证明 CLIP 的场景嵌入可加性分解为物体表征。然后通过受控实验，在合成数据集上从零训练 Transformer 模型，研究数据覆盖度对绑定泛化的影响，并分析绑定函数的复杂度（线性 vs. 乘法交互）。

Result: CLIP 的嵌入可加性分解解释了为何单模态探针能恢复物体信息，但其绑定函数复杂度高，阻碍了跨模态共享绑定机制的学习。在受控实验中，当训练数据覆盖足够多的概念组合时，模型能学习低复杂度的乘法交互绑定函数，实现系统性泛化。

Conclusion: CLIP 的概念绑定缺陷并非根本性限制，通过充足的数据覆盖和合适的模型设计，视觉语言模型可以学习到具有系统性泛化能力的绑定机制。这为改进多模态模型的组合理解能力提供了方向。

Classification reason: 论文研究 CLIP 等视觉语言嵌入模型的概念绑定（concept binding）能力，分析其表征分解与泛化机制，属于视觉语言模型的基础理解与表征分析，映射到 [CANONICAL] 子主题"视觉语言模型与 MLLM"。

Importance reason: Explores concept binding in CLIP, but lacks direct connection to user's priority subtopics like continuous VLM or autoregressive generation. Signals: concept binding, CLIP embedding analysis, systematic generalization, transformer experiments, no direct video/image generation or continuous VLM focus
...
### [199] [Personalize Your Large Vision-language Models With In-context Prompt Tuning](https://arxiv.org/abs/2605.31513)
...
Importance: 高优先级 (90.01) - 未入选精读

TL;DR: 本文提出 In-Context Prompt Tuning (ICPT)，一种轻量级投影模块方法，通过从多张参考图像中提取细粒度视觉语义并转化为连续提示，结合两种几何正则化（解耦身份与环境、分离概念），实现 LVLM 的高效个性化定制，在多种任务和骨干网络上达到 SOTA。
...
Method: 提出 ICPT，包含：(1) 轻量投影模块，从多张参考图像中提取细粒度视觉语义，将特征与身份-标签映射转化为连续提示；(2) 自适应提示长度机制，根据每个概念的视觉复杂度动态确定提示长度；(3) 两种几何正则化——解耦关键身份与瞬态环境状态、分离不同概念以避免语义混淆。

Result: 在多种个性化任务和不同 LVLM 骨干网络上，ICPT 在个性化准确率上达到 SOTA 性能，显著优于现有方法。

Conclusion: ICPT 通过轻量级上下文提示调优，实现了高效、准确的 LVLM 个性化，在复杂多图像多概念场景下表现优异，为 LVLM 的个性化部署提供了有效方案。

Classification reason: 该论文提出 ICPT（In-Context Prompt Tuning）方法，通过轻量投影模块实现大型视觉语言模型（LVLM）的个性化定制，属于通用视觉语言模型与 MLLM 架构改进方向，因此映射到 canonical 子主题 vlm_mllm_general。

Importance reason: Strongly aligns with user's priority on multimodal understanding/generation and MLLM personalization, with efficient in-context tuning and geometric regularization. Signals: multimodal_understanding_generation, MLLM personalization, in-context prompt tuning, lightweight projection, geometric regularization
...
### [200] ["Intelegi Româneşte?'' A Recipe for Romanian Vision-Language Models](https://arxiv.org/abs/2605.31401)
...
TL;DR: 本文系统研究了为罗马尼亚语构建视觉语言模型（VLM）的完整流程，包括将英文训练和评测语料通过机器翻译适配为罗马尼亚语、消融不同视觉骨干和语言骨干的贡献、以及构建文化原生评测集 HoraVQA。实验表明，罗马尼亚语适配的 VLM 在各项评测中一致超越同尺寸模型，甚至超过更大尺寸的模型。
...
Method: (1) 将英文 VLM 训练和评测语料通过机器翻译适配为罗马尼亚语，包括文本标注和图像内文本的翻译，保持视觉基础的同时调整文本内容；(2) 消融实验：对比不同规模/预训练的视觉骨干、多语言 vs 罗马尼亚语适配的 LLM 骨干、以及 OCR 风格图像-文本数据的影响；(3) 构建 HoraVQA——基于罗马尼亚日常场景的文化原生评测集。

Result: 罗马尼亚语适配的 VLM 在所有评测基准上一致优于同尺寸的对照模型，甚至超过更大尺寸的模型。

Conclusion: 通过系统性的数据翻译适配和模型消融，可以有效为低资源语言构建高性能 VLM，文化原生评测集对于准确评估模型能力至关重要。

Classification reason: 该论文系统研究了为低资源语言（罗马尼亚语）构建视觉语言模型（VLM）的完整流程，包括数据构建、架构消融实验和文化评测集，属于通用视觉语言模型构建与多模态指令微调范畴，与 [CANONICAL] 子主题 vlm_mllm_general（视觉语言模型与 MLLM）高度匹配。

Importance reason: Focuses on low-resource language adaptation, not on the user's priority subtopics like continuous VLM, autoregressive generation, or world models. Signals: Low-resource VLM, Romanian language, machine translation pipeline, cultural VQA, not aligned with user's priority subtopics
...
### [201] [Seeing Before Agreeing: Aligning Multi-Agent Consensus with Visual Evidence](https://arxiv.org/abs/2605.30698)
...
TL;DR: 本文提出EAGLE，一个无需训练的证据对齐多智能体VQA框架。核心思想是：多智能体VQA中仅靠答案层面的一致不够可靠，需要对齐各智能体所依赖的视觉证据（图像区域）。EAGLE显式暴露每个VLM智能体的视觉定位区域作为证据，通过智能体间相互验证证据一致性来指导最终决策。在6个VQA基准上取得最佳平均性能，且轻量、可解释、易于部署。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在VQA任务中仍存在幻觉和盲点。多智能体协作在文本QA中已取得成功，但在多模态领域探索不足。现有方法主要采用文本中心协议，关注文本讨论而忽略视觉信息对齐。作者发现仅靠答案层面的一致不足以确保可靠的多智能体VQA，对齐的视觉证据（各智能体依赖的图像区域）才是可信共识的关键。

Method: 提出EAGLE（Evidence-Aligned Grounded muLti-agent rEasoning）框架，包含三个步骤：(1) 证据暴露：每个VLM智能体在回答时同时输出其依赖的图像定位区域（视觉证据）；(2) 证据互验：智能体之间相互检查对方的视觉证据是否合理，进行交叉验证；(3) 证据一致性决策：基于各智能体视觉证据的一致性程度来引导最终答案选择。整个框架无需额外训练，直接使用现成VLM。

Result: 在6个VQA基准（涵盖通用VQA、科学QA、图表QA等不同领域）上进行实验，EAGLE取得了最佳平均性能，超越了单智能体基线和现有文本中心的多智能体方法。同时保持了轻量级、可解释性和易于部署的特点。

Conclusion: 本文揭示了多智能体VQA中视觉证据对齐的重要性，并证明通过显式暴露和交叉验证各智能体的视觉定位区域，可以显著提升多智能体VQA的可靠性和性能。EAGLE框架无需训练、轻量可解释，为多智能体多模态推理提供了新的范式。

Classification reason: 该论文研究多智能体协作下的视觉问答（VQA），属于多模态理解与生成方向下的视觉问答与文档理解（multimodal_vqa_doc）这一规范子主题。论文核心是提出基于视觉证据对齐的多智能体VQA框架，直接对应VQA领域。

Importance reason: Novel multi-agent VQA alignment method but outside core priority subtopics and lacks direct connection to continuous VLM or autoregressive generation. Signals: Multi-agent VQA, Visual evidence alignment, Training-free framework, Interpretable reasoning, VLM grounding
...
### [202] [Annotations Are Not All You Need: A Cross-modal Knowledge Transfer Network for Unsupervised Temporal Sentence Grounding](https://arxiv.org/abs/2605.30742)
...
TL;DR: 本文提出一种跨模态知识迁移网络（CKTNet），用于无监督时序句子定位（TSG）。该方法利用来自图像-名词和视频-动词任务的跨模态对齐知识（实体感知外观知识和事件感知动作知识），通过知识迁移使模型无需任何视频-查询配对标注或片段边界标注即可完成时序定位。在 ActivityNet Captions 和 Charades-STA 数据集上，该方法优于现有无监督方法，甚至可与有监督方法竞争。


<details>
  <summary>Details</summary>
Motivation: 现有时序句子定位方法严重依赖大量昂贵的视频-查询配对标注，在实际应用中收集这些标注需要大量人力。因此，本文旨在探索一种更实用但更具挑战性的无监督设置，无需任何配对标注即可完成时序定位。

Method: 提出跨模态知识迁移网络（CKTNet），包含三个模块：1）从图像-名词任务中提取实体感知的物体引导外观知识，并适配到每个视频帧；2）从视频-动词任务中提取事件感知的动作表示，并通过复制-粘贴方法将其细化为更复杂的实际场景；3）通过调制和迁移外观与动作知识，使模型能够直接利用这些通用知识关联视频和查询，无需训练即可准确检索相关片段。

Result: 在 ActivityNet Captions 和 Charades-STA 两个数据集上的大量实验表明，该方法优于现有无监督方法，甚至可与有监督方法竞争。

Conclusion: 本文证明了通过迁移其他跨模态任务（图像-名词、视频-动词）中易于获取的对齐知识，可以有效解决无监督时序句子定位这一挑战性任务，为减少对昂贵标注的依赖提供了新思路。

Classification reason: 该论文研究无监督时序句子定位（Temporal Sentence Grounding），属于视频-文本跨模态理解任务，与视觉问答/文档理解方向下的时序定位任务高度相关。现有 [CANONICAL] 子主题 "视觉问答与文档理解" 包含 VQA 和文档理解，而 TSG 是视频问答与定位的交叉任务，可归入该子主题。

Importance reason: Unsupervised temporal grounding is not a priority subtopic and lacks direct connection to user's focus on generation, continuous VLMs, or world models. Signals: Unsupervised temporal sentence grounding, cross-modal knowledge transfer, no generation or autoregressive focus, no continuous VLM or flow matching, negative keywords absent but topic off-target
...
### [203] [MechVQA: Benchmarking and Enhancing Multimodal LLMs on Comprehensive Mechanical Drawing Understanding](https://arxiv.org/abs/2605.30794)
...
TL;DR: 本文提出了 MechVQA，首个面向机械工程图纸理解的多模态大模型评测数据集，包含 3.3k 高密度图纸和 21K QA 对，覆盖识别、推理、判断三个能力层级共 10 种细粒度任务。基于该数据集，作者通过多阶段训练范式构建了 MechVL 模型，在总得分上超越最强闭源基线 7.57 个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 在通用 VQA 任务上表现优异，但在机械工程图纸理解上仍然脆弱，主要挑战包括：高标注密度、弱领域知识、以及严格的投影规则和几何约束下的空间关系推理不可靠。目前缺乏专门针对机械图纸理解的评测基准。

Method: (1) 构建 MechVQA 数据集：通过半自动构建和质量控制流水线，收集 3.3k 高密度机械图纸，生成 21K QA 对，涵盖识别、推理、判断三个能力层级共 10 种细粒度任务。(2) 开发 MechVL 模型：采用多阶段训练范式，在通用 MLLM 基础上进行领域微调，建立领域专用基线模型。

Result: MechVL 在 MechVQA 总得分上超越最强闭源基线 7.57 个百分点，显著提升了机械图纸理解能力。

Conclusion: MechVQA 为机械工程图纸理解提供了首个综合性评测基准，MechVL 展示了多阶段领域微调的有效性，为 MLLM 在机械设计与检测场景中的部署提供了可复用的基础。

Classification reason: 该论文提出了机械工程图纸理解数据集 MechVQA 和模型 MechVL，属于视觉问答与文档理解（特别是专业领域文档理解）范畴，与 [CANONICAL] 子主题 "视觉问答与文档理解"（multimodal_vqa_doc）高度匹配。

Importance reason: Specialized domain benchmark for mechanical drawings, but outside user's priority subtopics and negative for general multimodal understanding. Signals: domain-specific benchmark, mechanical engineering, MLLM evaluation, semi-automated pipeline, multi-stage training
...
### [204] [DisasterLex: An Expert Concept-to-Schema Knowledge Graph for Geospatial Reasoning in Disaster Analytics](https://arxiv.org/abs/2605.30538)
...
TL;DR: DisasterLex 是一个知识图谱中介的 Text-to-SQL 框架，通过专家知识图谱（EKG）将用户自然语言查询与灾害分析数据库（36张地理空间表）桥接，在75个查询测试集上以1.4x-2.75x的优势超越四种SOTA基线。


<details>
  <summary>Details</summary>
Motivation: 现有 Text-to-SQL 方法在灾害分析领域表现不佳，因为该领域的查询涉及异构地理空间模式且需要因果推理，而通用方法难以处理这种复杂领域。

Method: 提出 DisasterLex 框架，核心是插入一个专家知识图谱（EKG），包含107个概念、117条因果边和52个概念-模式链接。框架分四个阶段运行：识别查询实体、路由到操作域、在因果边上规划、生成SQL，每一步限制传递给模型的模式范围。

Result: 在7种基础模型（涵盖闭源和开源）上，DisasterLex 以1.4x-2.75x的优势超越 LightRAG、HippoRAG 2、ReFoRCE、CHESS 四种基线，绝对分数为1.65-3.56（满分5.0）。错误分析表明基线失败集中在路由和多表SQL组合上。

Conclusion: 知识图谱中介的 Text-to-SQL 方法能有效提升灾害分析领域的查询性能，尤其解决了路由和多表组合这两个关键难点。

Classification reason: 该论文研究面向灾害分析领域的文本到SQL（Text-to-SQL）方法，涉及自然语言查询结构化表格数据（地理空间表格），属于文档/表格理解范畴，与多模态理解与生成方向下的[CANONICAL] subtopic "视觉问答与文档理解"（multimodal_vqa_doc）高度相关。

Importance reason: Focuses on disaster analytics Text-to-SQL, not aligned with user's priority subtopics in multimodal generation or efficient models. Signals: Knowledge graph for Text-to-SQL, Geospatial disaster domain, Not multimodal generation, No autoregressive or diffusion video/image, No continuous VLM or world model
...
### [205] [WristCompass: Kinematic Coupling as a Learnable Visual Concept for Ego-Camera Orientation](https://arxiv.org/abs/2605.30671)
...
TL;DR: WristCompass 提出利用手腕运动与相机朝向之间的运动学耦合关系（kinematic coupling dynamics），从第一视角操作视频中恢复自我相机朝向。该方法仅需 200K 参数的 GRU 模型，在桌面操作数据上训练后能零样本迁移到 Epic Kitchens 烹饪视频，达到 14.3° 中位测地线误差，接近 1B 参数场景重建模型 VGGT 的性能。


<details>
  <summary>Details</summary>
Motivation: 从第一视角操作视频中分离手部运动与相机运动是模仿学习的关键步骤。传统方法依赖场景几何推断相机朝向，但当手部遮挡大部分画面时，场景几何信息缺失，导致大模型（如 1B 参数的 VGGT）表现甚至不如常数预测器。因此需要寻找替代的视觉线索来恢复相机朝向。

Method: WristCompass 识别出"运动学耦合动力学"（kinematic coupling dynamics）这一视觉概念——即手腕运动与相机朝向之间由手臂-肩膀-头部运动链施加的结构化物理关系。具体方法：(1) 提取 4D 腕间特征（双手腕位置/朝向）而非 126D 全手关键点；(2) 使用 GRU 在短时间窗口内建模时序依赖，而非逐帧预测；(3) 仅需桌面操作视频训练，利用解剖学而非场景外观的物理不变性实现零样本迁移。

Result: 在 TACO 基准上，WristCompass 以 200K GRU 参数达到 14.3° 中位测地线误差，零样本迁移到 Epic Kitchens 烹饪视频。相比之下，1B 参数的 VGGT 场景重建模型在相同设置下表现更差。4D 腕间特征优于 126D 全手关键点，时序建模（GRU）优于逐帧方法。

Conclusion: WristCompass 证明了运动学耦合动力学是一种有效的视觉概念，可以在场景几何信息缺失时替代性地恢复相机朝向。该方法紧凑、时序依赖且物理可迁移，为第一视角模仿学习中的手-相机运动解耦提供了新思路。

Classification reason: 该论文关注从第一视角（egocentric）操作视频中恢复相机朝向，涉及手部遮挡下的时序推理与运动学耦合建模，属于第一视角视频理解与时序推理范畴，与 [CANONICAL] subtopic "视频理解与时序推理"（video_understanding_temporal）高度匹配。

Importance reason: Novel use of kinematic coupling for ego-camera orientation, but tangential to user's core generative and multimodal reasoning priorities. Signals: zero-shot transfer, compact 200K GRU, outperforms 1B scene model, robotics/egocentric focus, no generative or continuous VLM connection
...
### [206] [PEEK: Picking Essential frames via Efficient Knowledge distillation](https://arxiv.org/abs/2605.31029)
...
TL;DR: PEEK 提出一种高效的动态帧采样方法，通过将教师模型的字幕条件帧相关性排名蒸馏到轻量级时序模型中，仅基于视觉内容即可选择信息量最大的帧用于视频字幕生成。在 ActivityNet Captions 和 MSR-VTT 上，PEEK 在低帧预算下优于现有方法，且计算开销极低（仅增加 5.2% 的字幕生成时间）。


<details>
  <summary>Details</summary>
Motivation: 视频语言模型只能处理有限数量的帧，而现有帧选择方法（如均匀采样）忽略视觉内容，自适应采样方法虽更优但计算成本高。因此需要一种既高效又能准确选择信息帧的方法。

Method: PEEK 将字幕条件帧相关性排名从强教师模型（如基于 CLIP 的模型）蒸馏到一个轻量级时序模型（仅基于视觉特征），实现无需字幕即可高效预测帧重要性的动态帧采样。

Result: 在 ActivityNet Captions 上，PEEK 在 16 种配置中赢得 14 种；在 MSR-VTT 上，低帧预算下零样本迁移表现最佳。计算开销仅增加 5.2%，远低于 CSTA（65.4%）和 MaxInfo（211.9%）。

Conclusion: PEEK 在低帧预算下实现了更优的帧选择准确性和更高的效率，为视频字幕生成提供了一种实用的动态帧采样方案。

Classification reason: 该论文研究视频帧选择（关键帧采样）以提升视频字幕生成的效率与质量，属于视频理解与多模态模型的高效处理范畴。现有 [CANONICAL] 子主题 "视频理解与时序推理"（video_understanding_temporal）涵盖视频理解、视频问答、时序定位等，与本文的视频帧选择与视频字幕生成任务高度匹配。

Importance reason: Focuses on efficient frame sampling for video captioning, which is not a top priority subtopic and lacks direct connection to user's high-weight areas like continuous VLM or autoregressive generation. Signals: Efficient frame sampling, Knowledge distillation, Video captioning, Low computational overhead, Adaptive sampling
...
### [207] [Towards Effective Long-Video Event Prediction via Multi-Level Event Semantics Mining](https://arxiv.org/abs/2605.31069)
...
TL;DR: 本文提出 VISTA，一个多层级事件语义挖掘框架，用于长视频事件预测。通过角色中心视觉提示提取细节语义、知识增强迭代检索构建事件链、以及类人"先提议后检索"策略生成未来预测，在真实数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有长视频语言模型（LVLM）在长视频问答和摘要上表现良好，但难以泛化到事件预测任务，因为它们既不能精确提取事件相关细节，也无法对事件发展进行细粒度分析。长视频事件预测因涉及多模态上下文和复杂叙事而未被充分探索。

Method: VISTA 框架包含三个层级：(1) 细节级语义——角色中心视觉提示，精确提取事件相关视觉细节；(2) 事件级语义——知识增强迭代检索策略，引导 LLM 逐步构建逻辑连贯的事件链；(3) 预测级语义——类人"先提议后检索"策略，生成多样化未来提议并整合多层级线索，产生鲁棒准确的预测。

Result: 在真实世界长视频数据集上的实验验证了 VISTA 在长视频事件预测任务上的有效性，相比基线方法取得了显著提升。

Conclusion: VISTA 通过多层级事件语义挖掘有效解决了长视频事件预测中细节提取和事件链构建的挑战，为长视频理解与未来事件预测提供了新的框架思路。

Classification reason: 该论文研究长视频事件预测，属于长视频理解与时序推理范畴，与 [CANONICAL] subtopic "视频理解与时序推理" (video_understanding_temporal) 高度匹配。

Importance reason: Focuses on long-video event prediction, not on the user's priority subtopics like continuous VLM, autoregressive generation, or world models. Signals: Long-video event prediction, Multi-level semantics, Character-centric visual prompt, Knowledge-enhanced retrieval, Propose-then-retrieve strategy
...
### [208] [HiERO-StepG @ Ego4D Step Grounding Challenge: hierarchical activity understanding enables zero-shot step grounding](https://arxiv.org/abs/2605.31227)
...
TL;DR: 本文提出 HiERO-StepG，一种零样本的步骤定位方法，利用弱监督层次化表示学习（HiERO）从非结构化视频中自动发现程序性活动的层次结构，无需任务特定微调或程序级标注，在 Ego4D Step Grounding 挑战赛中取得第二名。


<details>
  <summary>Details</summary>
Motivation: 程序性活动（如烹饪、维修）天然具有层次结构（步骤和子步骤），但传统步骤定位方法依赖大量标注且泛化性差。作者希望利用视频中动作共现的重复模式，让层次结构自然涌现，实现零样本步骤定位。

Method: 基于 HiERO 的弱监督表示学习框架，利用细粒度动作级叙述将功能相关的动作在特征空间中映射到相近位置，然后通过简单聚类检测步骤。HiERO-StepG 在此基础上增加粗细粒度步骤分配一致性约束、严格的时间单调性约束，以及后处理去噪。

Result: 在 Ego4D Step Grounding 挑战赛中以 R@1 (IoU=0.3) 56.27% 的成绩排名第二，且完全零样本，无需程序特定标注。

Conclusion: 层次化活动理解可以有效实现零样本步骤定位，弱监督方法在无需大量标注的情况下即可达到有竞争力的性能，为程序性视频理解提供了新思路。

Classification reason: 该论文聚焦于 Ego4D 视频中的步骤定位（step grounding），属于视频理解与时序推理任务，与 [CANONICAL] subtopic "视频理解与时序推理" 高度匹配。

Importance reason: Focuses on zero-shot step grounding for procedural video understanding, which is not a priority subtopic and lacks direct connection to user's key interests in generation, continuous VLM, or world models. Signals: Zero-shot step grounding, Weakly-supervised hierarchical representation, Ego4D challenge, No generation or autoregressive focus, No continuous VLM or flow matching
...
### [209] [A Pilot Study on Curator-Guided Multilingual Art Description for Blind and Low-Vision Audiences with Small Vision-Language Models](https://arxiv.org/abs/2605.31080)
*Iosif Tsangko, Andreas Triantafyllopoulos, George Margetis, Ioana Crihana, Björn W. Schuller*

Main category: cs.CV, cs.MM, cs.HC, cs.CL, cs.AI

Direction: 多模态理解与生成

Subtopic: 领域内容生成应用

Importance: 普通相关 (43.95) - 未入选精读

TL;DR: 本文探索使用小型视觉语言模型（Qwen2.5-VL-3B-Instruct）为盲人和低视力观众生成多语言（德语、罗马尼亚语、塞尔维亚语）艺术描述。通过构建平行描述语料库，比较语言特定 LoRA 适配器与多语言适配器的效果，发现语言特定适配器在可控性和视觉基础描述质量上更稳定。


<details>
  <summary>Details</summary>
Motivation: 盲人和低视力观众在视觉艺术描述方面长期得不到充分服务，尤其是在多语言场景和博物馆环境中，隐私和知识产权限制倾向于使用小型本地部署的视觉语言模型。现有研究缺乏针对小型 VLM 在多语言艺术描述场景下的系统评估。

Method: 使用 Qwen2.5-VL-3B-Instruct 作为基础模型，构建面向 BLV 观众的平行多语言艺术描述语料库（包含图像和元数据）。对比两种微调策略：语言特定的 LoRA 适配器 vs. 单一多语言 LoRA 适配器，在固定骨干网络和训练预算下进行实验。评估采用自动词汇和嵌入指标，以及针对罗马尼亚语 BLV 小规模用户研究校准的 LLM-as-Judge 协议。

Result: 在实验设置下，语言特定适配器在罗马尼亚语和塞尔维亚语上展现出更稳定的可控性和视觉基础描述质量，而多语言适配器在德语上仍具有竞争力。

Conclusion: 研究为小型本地部署 VLM 在多语言艺术描述场景中的部署提供了初步证据，同时强调在得出通用结论之前，需要更大规模的 BLV 用户研究和更广泛的语言覆盖。

Classification reason: 该论文研究策展人引导的多语言艺术描述生成，面向盲人和低视力观众，属于垂直领域（博物馆/艺术）的内容生成应用。该主题与现有 subtopic "领域内容生成应用"（aigc_domain_application）高度匹配，该 subtopic 在 multimodal_understanding_generation 和 aigc 方向下均有收录。

Importance reason: This paper focuses on a niche accessibility application with small VLMs, not aligning with the user's priority on advanced multimodal generation and reasoning. Signals: small VLM, multilingual art description, BLV accessibility, LoRA adapters, low relevance to core topics
...
### [210] [Simple Token-Efficient Vision-Language Model for Case-level Pathology Synoptic Report Generation](https://arxiv.org/abs/2605.30716)
...
TL;DR: 本文提出了一种简洁的 token 高效视觉语言模型，用于从全切片图像生成病例级病理学概要报告。通过使用 5× 放大倍率的 512×512 斑块将序列长度减少最多 64 倍，结合两阶段监督训练（对齐器 WSI 描述 + 病例级报告微调），仅需半张 H100 GPU 即可训练，在 ROUGE-L/METEOR/BLEU-4 上取得高分数，并在 AI 评估中优于强基线。


<details>
  <summary>Details</summary>
Motivation: 病理学全切片图像（WSI）具有千兆像素分辨率、长视觉 token 序列以及病例级推理的复杂性（单个病例可能包含多个 WSI，组织异质且发现模糊），现有方法在有限 GPU 内存下难以实用。本文旨在降低多 WSI 视觉语言模型研究的门槛，实现高效且临床可用的病理报告生成。

Method: 采用三组件架构：冻结的病理斑块编码器、轻量两层 MLP 视觉语言对齐器、大语言模型解码器，并引入显式 WSI 标记 token 分隔病例内不同切片。训练分两阶段：(1) 对齐器 WSI 描述预训练（使用异质 WSI-文本对）；(2) 病例级监督微调（病例-报告对）。使用 5× 放大倍率下 512×512 斑块表示每张切片，大幅缩短序列长度，结合高效训练技术实现仅需半张 H100 GPU 的实用训练。

Result: 在 ROUGE-L/METEOR/BLEU-4 指标上取得高分，内存和运行时间显著优于强基线。AI 评估中模型持续被偏好。消融实验揭示了性能-效率权衡，并确定了在多 WSI 设置下提升鲁棒性的简单选择。

Conclusion: 本文提供了一个强有力且可复现的高效病理报告生成基线，显著降低了有限计算资源下多 WSI VLM 研究的门槛，为医疗领域文档生成应用提供了实用方案。

Classification reason: 该论文聚焦于病理学全切片图像的结构化报告生成，属于医疗垂直领域的生成应用，与现有 subtopic "领域内容生成应用"（aigc_domain_application）高度匹配。该 subtopic 在 multimodal_understanding_generation 和 aigc 方向下均有存在，且为 CANONICAL 条目。

Importance reason: The paper focuses on a narrow medical domain (pathology report generation) with no overlap with the user's priority subtopics like continuous VLM, autoregressive video/image generation, or world models. Signals: domain-specific pathology, token efficiency via low magnification, constrained GPU training, no autoregressive or continuous VLM methods, no video/image generation
...
### [211] [Generating Reports or Repeating Templates? Measuring and Mitigating Template Collapse in 3D CT Report Generation](https://arxiv.org/abs/2605.30984)
...
Importance: 普通相关 (49.89) - 未入选精读

TL;DR: 本文发现3D CT报告生成中的"模板坍缩"问题——模型倾向于生成流畅但缺乏病理检测多样性的模板化报告，并提出了解耦框架CLarGen，将临床检测与语言生成分离，通过潜在查询变换器、病理引导检索和医学语言模型来缓解模板坍缩，显著提升临床准确性。


<details>
  <summary>Details</summary>
Motivation: 现有3D医学视觉语言模型虽然能生成流畅的放射学风格文本，但病理检测能力低、输出多样性差，会坍缩为忽略罕见但关键发现的通用模板。这种"模板坍缩"问题源于3D医学影像的独特约束（数据有限、标签严重不平衡、体素编码器信号弱），需要被系统诊断和缓解。

Method: 提出CLarGen解耦框架，将"说什么"（临床检测）与"怎么说"（语言合成）分离。包含三个组件：(i) 潜在查询变换器（Latent Query Transformer）进行多标签病理检测；(ii) 病理引导检索（pathology-guided retrieval）获取临床匹配的示例报告；(iii) 医学语言模型根据检测结果和检索上下文合成最终报告。

Result: 在多个SOTA 3D CT报告生成基线上，CLarGen有效缓解了模板坍缩，显著提升临床准确性（macro-F1从0.189提升至0.487；CRG从0.368提升至0.472），同时保持报告流畅性。

Conclusion: 显式、可测量的临床基础对于抗模板坍缩的3D CT报告生成至关重要。解耦临床检测与语言生成的框架是解决医学报告生成中模板化问题的有效方向。

Classification reason: 该论文研究3D CT影像报告生成，属于医疗垂直领域的生成应用，与现有 canonical subtopic "aigc_domain_application"（领域内容生成应用）高度匹配。同时涉及多模态理解与生成（3D医学VLM），因此主方向为 multimodal_understanding_generation，也匹配 aigc。

Importance reason: Addresses a specific medical report generation failure mode, but is far from the user's priority subtopics in multimodal generation and autoregressive video/image generation. Signals: Template collapse in 3D CT report generation, Decoupled detection-generation framework, Clinical accuracy improvement, Medical VLM failure analysis, Low relevance to user's core interests
