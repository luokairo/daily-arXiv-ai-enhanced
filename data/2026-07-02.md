...
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 50]
...
Importance: 高优先级 (95.55) - Top 3 精读候选

TL;DR: 本文提出非对称互变分学习（AMVL）框架，通过双向KL散度校准解决连续潜空间多模态推理中的训练-推理不匹配问题，在BLINK基准上平均提升+10.83分。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM受限于语言空间瓶颈，将复杂视觉推理强制压缩为离散token，丢失感知细节。连续潜空间推理虽具潜力，但存在严重的训练-推理不匹配：训练时后验分布可访问答案信息导致"答案泄露"，而推理时先验分布无法获取该信息，标准变分训练迫使先验模仿有信息泄露的后验，导致性能下降。

Method: 提出非对称互变分学习（AMVL）框架：前向KL散度训练目标无关的先验分布匹配后验分布；反向KL散度同时正则化后验分布，防止其坍缩到推理不兼容的区域，缓解"答案泄露"。将AMVL实例化到潜空间集成的MLLM中，实现连续潜空间推理。

Result: 在复杂多模态推理基准BLINK上平均提升+10.83分，在单个推理任务上最高提升+32.00分，一致优于强离散推理和潜空间推理基线。分析实验证实了潜空间稳定性的提升。

Conclusion: AMVL通过双向KL散度校准有效解决了连续潜空间推理中的训练-推理不匹配问题，为连续多模态推理提供了更稳定的训练范式，理论分析证明了该目标能减少先验污染。

Classification reason: 该论文提出AMVL框架，在连续潜空间中进行多模态推理，解决训练-推理不匹配问题，属于连续多模态推理与潜空间思维（continuous_multimodal_reasoning）这一[CANONICAL]子主题。

Importance reason: Directly addresses continuous multimodal latent reasoning with a novel theoretical solution to train-inference mismatch, a top priority subtopic. Signals: continuous multimodal reasoning, latent space, variational learning, train-inference mismatch, BLINK benchmark
...
### [2] [Low Perplexity is Repetition: A One-Dimensional Self-Conditioning Attractor in Continuous Diffusion LMs](https://arxiv.org/abs/2607.00588)
...
TL;DR: 本文发现连续扩散语言模型（如ELF）的低困惑度（Gen-PPL）指标存在缺陷：模型通过大量重复文本来降低困惑度，而Gen-PPL奖励重复而非惩罚它。作者将重复现象追溯到自条件反馈循环中一个单一方向上的收缩吸引子，并提出ACE方法——在每一步减去该方向，有效消除重复，同时保持生成质量，且在不同规模模型和采样器间可迁移。


<details>
  <summary>Details</summary>
Motivation: 连续扩散语言模型（如ELF）报告了创纪录的低生成困惑度（Gen-PPL），但作者发现这些模型生成的文本重复率远高于人类文本，而Gen-PPL指标实际上奖励重复而非惩罚它，导致低分数虚高了生成质量。需要揭示这一缺陷的根源并提出修复方法。

Method: 作者首先通过实验证明ELF模型存在严重重复问题，且Gen-PPL指标有偏。然后通过理论分析，将重复现象追溯到自条件反馈循环中一个单一方向上的收缩吸引子。基于此，提出ACE（Attractor-Contrast-Escape）方法：在每一步自条件反馈中减去该单一方向（无需标签），从而打破吸引子。该方向在105M模型上估计一次，即可迁移到342M和652M模型及不同采样器。

Result: 去除重复后，ELF-B的Gen-PPL从19.5上升到27.7；最小的模型因重复最多反而获得最佳Gen-PPL。ACE方法将重复率降低到接近人类水平，同时保持生成质量竞争力，且在不同规模模型间迁移几乎不变。在生成干净文本所需的计算量上，ACE比基线方法便宜1.5-5倍。

Conclusion: 连续扩散语言模型存在由自条件反馈中一维吸引子导致的重复生成问题，Gen-PPL指标有严重缺陷。ACE方法通过减去该单一方向有效消除重复，且高效可迁移。这项工作揭示了连续语言模型评估中的关键陷阱，并为改进提供了实用方案。

Classification reason: 该论文研究连续扩散语言模型（ELF）中的重复生成问题，属于连续语言模型范畴，与 [CANONICAL] 子主题"连续语言模型与语言流"高度匹配。

Importance reason: Directly addresses a critical flaw in continuous diffusion LMs (ELF) with a simple, transferable fix, highly relevant to your top-weighted continuous language model subtopic. Signals: continuous diffusion LM, Gen-PPL flaw, repetition attractor, one-dimensional fix, ACE method
...
### [3] [Self-conditioned Flow Map Language Models via Fixed-point Flows](https://arxiv.org/abs/2607.00714)
...
TL;DR: 本文提出固定点流（Fixed-point Flows），一种二维自条件流语言模型框架，将自条件机制建模为固定点迭代，并通过固定点蒸馏和流映射蒸馏压缩为少步生成器 FMLM*，在 OpenWebText 上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 自条件机制在连续流语言模型中效果显著但理论理解不足，且如何在少步流映射生成器中有效利用自条件尚不明确。

Method: 将自条件流语言模型重新解释为固定点迭代，提出固定点流——二维流过程，第一维为流过程，第二维为固定点迭代。通过固定点蒸馏压缩迭代步数，流映射蒸馏压缩流步数，得到 FMLM*。

Result: FMLM* 在 OpenWebText 上的一步和少步生成中优于现有自条件模型和少步模型。

Conclusion: 固定点流为自条件流语言模型提供了统一的理论视角，并实现了高效的少步生成。

Classification reason: 该论文提出基于固定点流的连续流语言模型（FMLM*），属于连续语言模型与语言流方向，与 [CANONICAL] subtopic "连续语言模型与语言流" 高度匹配。

Importance reason: Directly advances continuous language model flow matching with a novel fixed-point self-conditioning framework, achieving SOTA few-step generation. Signals: continuous language model, flow matching, self-conditioning, fixed-point iteration, few-step generation
...
### [4] [When to Personalize Household Object Search: A Rigidity-Gated Hybrid Policy](https://arxiv.org/abs/2607.00022)
...
TL;DR: 本文提出 PerSim，一种刚性门控混合策略，用于服务机器人家庭物体搜索。通过人类校准的仿真管道生成居民条件化的物体放置动态，结合大五人格向量预测房间级先验和室内共现线索，仅在放置行为可变时启用个性化，在统一人类研究（N=200）和数字孪生实验中验证了搜索成本的降低。


<details>
  <summary>Details</summary>
Motivation: 服务机器人搜索家庭物体依赖空间先验，但物体位置随居民个性特征变化。收集纵向、个性特定的家庭轨迹成本高且难以规模化。因此需要研究何时个性化有帮助，并提出可扩展的个性化搜索策略。

Method: 提出 PerSim 刚性门控混合策略，结合个性条件先验与群体频率基线，仅在放置行为可变时启用个性化。采用人类校准的仿真管道生成物体放置转换数据，训练预测器注入连续大五人格向量输出房间级先验和室内共现线索。通过统一人类研究（N=200）进行双层验证，并在家庭数字孪生中评估端到端搜索成本。

Result: 合成转换被判断为行为合理（平均3.85/5, p<1e-6）；盲测中个性化在低刚性物体上更受青睐（p=0.005）；在连续五维特质空间上的插值优于最近离散配置匹配（p=0.035）；数字孪生中 PerSim 降低了期望搜索成本。

Conclusion: PerSim 通过刚性门控机制有效判断何时个性化，结合仿真管道和连续人格向量实现了可扩展的个性化家庭物体搜索，在端到端搜索成本上取得了显著改进。

Classification reason: 该论文研究服务机器人在家庭环境中基于居民个性特征（Big Five）进行物体搜索的个性化策略，属于具身智能中的规划与控制问题，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on household robot search personalization, not core to user's priority on continuous multimodal models or video generation. Signals: robotics, household object search, personalization, simulation pipeline, Big Five personality
...
### [5] [Joint Discovery of Object and Action Symbols through Effect Prediction for Robotic Manipulation Planning](https://arxiv.org/abs/2607.00031)
...
TL;DR: 本文提出一种联合发现物体类别和操作原语的模型，通过二元瓶颈层预测多模态交互效果（物体运动、接触、力反馈），并基于发现的离散符号表示进行机器人操作规划，在桌面重定位和堆叠任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 自主机器人需要将连续的高维传感器运动交互抽象为离散的物体和动作表示，以便进行复杂操作规划。现有方法要么基于视觉外观分类物体（无法区分外观相似但行为不同的物体），要么基于交互效果分类但局限于预定义动作。本文旨在联合发现高层操作原语和物体类别，克服上述局限。

Method: 提出一个包含二元瓶颈层的神经网络模型，从随机交互数据中预测多模态结果（物体运动、接触、力反馈），从而联合发现离散的操作原语（动作符号）和物体类别（物体符号）。基于发现的二元表示，利用离散规划方法，通过预测效果轨迹中的中间步骤实现部分动作执行，以进行精确的低层控制。对于新物体，通过比较少量交互效果与学习到的物体符号预测效果，实现基于行为而非视觉相似性的少样本泛化。

Result: 在桌面重定位和堆叠任务上的实验表明，该效果驱动的规划方法在已知和新物体上的规划精度均优于最先进方法和基于视觉的替代方法。

Conclusion: 本文证明了通过效果预测联合发现物体和动作符号的有效性，该方法能够实现基于行为相似性的少样本泛化，为机器人操作规划提供了一种更鲁棒的抽象表示学习框架。

Classification reason: 该论文研究机器人操作规划中的物体与动作符号发现，通过效果预测进行离散规划，属于具身智能中的世界模型与规划控制范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on robotic manipulation planning with discrete symbols, not directly aligned with user's priority on continuous language/multimodal models or video generation. Signals: Robotic manipulation planning, Joint object-action discovery, Effect prediction, Discrete symbol grounding, Not continuous representation
...
### [6] [Learning Dexterous Manipulation Using Contact Wrench Guidance From Human Demonstration](https://arxiv.org/abs/2607.00033)
...
TL;DR: CHORD 是一个利用人类示教中的接触力/力矩引导（contact wrench guidance）来训练灵巧操作策略的框架，结合强化学习在大规模仿真基准（4739个双手操作任务）上达到82.12%平均成功率，并成功迁移到真实世界。


<details>
  <summary>Details</summary>
Motivation: 灵巧机器人操作可以从丰富的人类示教中受益，但将人类示教直接迁移到机器人策略仍然非常困难，尤其是在接触丰富的长时程操作任务中。

Method: 提出 CHORD 框架，核心思想是物体中心的接触力/力矩空间引导：将人类和机器人运动表示为它们能在物体上施加的力和力矩，通过诱导的瞬时运动来衡量相似性。利用这种引导使强化学习在接触丰富的灵巧操作中更具可扩展性。同时构建了一个包含4739个双手灵巧操作任务的大规模仿真基准，数据来自动捕数据集和室内重建视频。

Result: 在1831个基准任务上评估，CHORD 平均成功率达82.12%；从仅手部示教和第三人称示教泛化到全身操作，成功率达90.77%；学到的策略在开环和闭环设置下均可迁移到真实世界。

Conclusion: CHORD 通过接触力/力矩空间引导有效弥合了人类示教与机器人策略之间的鸿沟，在灵巧操作任务上展现出强大的可扩展性和真实世界迁移能力。

Classification reason: 该论文研究灵巧手操作（dexterous manipulation），利用人类示教通过接触力/力矩空间引导强化学习策略，属于机器人具身操作与策略学习范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on dexterous manipulation with contact wrench guidance, which is outside the user's core priority subtopics of continuous multimodal models and video/image generation. Signals: dexterous manipulation, contact wrench guidance, human demonstration, reinforcement learning, bimanual tasks
...
### [7] [3D Point World Models: Point Completion Enables More Accurate Dynamics Learning](https://arxiv.org/abs/2607.00148)
...
TL;DR: 本文提出3D Point World Models (3DPWM)，一种完全在3D空间中运行的与任务无关的世界模型。它先补全部分点云，再在补全后的3D场景中学习动作条件动力学，实现长时程可靠 rollout（100-300+步），支持开环和闭环规划，并成功实现仿真到真实的迁移。


<details>
  <summary>Details</summary>
Motivation: 现有的基于视频的动力学模型缺乏显式3D空间结构，长期 rollout 存在几何不一致和累积误差；而基于部分点云的3D动力学模型对遮挡和预测漂移敏感。因此需要一种在完整3D几何上建模动力学的方法。

Method: 3DPWM 包含两个核心模块：(1) 点云补全模块，将部分观测点云补全为完整3D场景；(2) 基于补全几何的动作条件动力学模型，在3D空间中预测下一时刻点云状态。模型完全在3D空间操作，支持长时程 rollout 和基于模型的规划。

Result: 在多种机器人形态和桌面操作基准上，3DPWM 实现了显著更可靠的长时程 rollout（100-300+步），支持开环和闭环规划，并成功实现仿真到真实的迁移。

Conclusion: 通过点云补全获得完整3D几何信息，能够显著提升动力学模型的长时程预测可靠性和规划性能，为机器人操作提供了一种有效的3D世界模型范式。

Classification reason: 该论文提出3D点云世界模型，通过点云补全实现动作条件动力学建模，用于机器人规划与控制，属于具身世界模型与规划控制方向。

Importance reason: Strong world model contribution with 3D point completion for long-horizon dynamics, but lacks direct alignment with priority subtopics like continuous language models or video generation. Signals: 3D world model, point cloud completion, long-horizon rollout, sim-to-real transfer, task-agnostic planning
...
### [8] [ASPIRE: Agentic /Skills Discovery for Robotics](https://arxiv.org/abs/2607.00272)
...
Main category: cs.MA, cs.RO, cs.AI
...
TL;DR: ASPIRE 是一个面向机器人的自主技能发现与持续学习系统，通过闭环执行引擎、可扩展技能库和进化搜索三个组件，让机器人自主编写、调试和积累可复用的控制程序，在多个基准上显著超越先前方法，并展示了仿真到真实的技能迁移能力。


<details>
  <summary>Details</summary>
Motivation: 传统机器人编程需要协调多模态感知、处理物理接触动力学、应对多样化配置和执行失败，过程复杂且耗时。现有方法缺乏自主发现可跨任务、跨场景、跨本体复用的技能的能力。

Method: ASPIRE 包含三个核心组件：(1) 闭环机器人执行引擎，提供细粒度多模态轨迹，支持自主故障诊断、修复合成与验证；(2) 持续扩展的技能库，将验证后的修复提炼为可复用、可迁移的知识；(3) 进化搜索，生成多样化的任务序列和控制程序，探索超越单轨迹优化的策略。

Result: 在 LIBERO-Pro 扰动场景下提升 77%，Robosuite 双手交接任务提升 72%，BEHAVIOR-1K 长时家务任务提升 32%。在 LIBERO-Pro Long 零样本泛化中达到 31% 成功率（先前方法仅 4%）。仿真发现的技能可迁移到真实机器人，显著减少编程工作量。

Conclusion: ASPIRE 证明了通过持续探索和技能积累，机器人可以自主发现可跨任务、跨场景、跨本体复用的技能，显著提升任务成功率和泛化能力，为机器人自主编程和技能学习提供了有效范式。

Classification reason: 该论文提出 ASPIRE 系统，通过代码即策略范式让机器人自主编写和优化控制程序，并持续积累可复用的技能库。这属于具身智能中机器人自主编程、技能发现与持续学习的范畴，与 [CANONICAL] 具身世界模型与规划控制 方向高度匹配。

Importance reason: Strong world model and skill discovery for robotics, but lacks direct focus on user's priority subtopics like continuous language models or video generation. Signals: world model, skill discovery, code-as-policy, sim-to-real transfer, continual learning
...
### [9] [Learning Gait-Aware Quadruped Locomotion with Temporal Logic Specifications](https://arxiv.org/abs/2607.00442)
...
TL;DR: 本文提出一种基于信号时序逻辑（STL）的奖励塑造框架，用于四足机器人步态学习。通过参数化STL模板定义三种步态（walking-trot, trot, bound）的约束条件，计算STL鲁棒性作为稠密奖励信号，结合PPO算法在Barkour四足机器人上实现更稳定的训练和更精确的速度跟踪。


<details>
  <summary>Details</summary>
Motivation: 传统四足机器人强化学习依赖手工设计的马尔可夫奖励函数，缺乏可解释性且无法显式控制步态行为。作者希望引入一种形式化、可解释的规范方法来指定步态行为，从而提升策略学习的可控性和稳定性。

Method: 提出基于信号时序逻辑（STL）的奖励塑造框架：1）为三种速度模式（walking-trot, trot, bound）定义参数化STL模板，包含安全边界、步态同步约束、指令跟踪和驱动边界；2）从参考轨迹中校准模板参数；3）使用STL鲁棒性的平滑近似计算稠密连续奖励；4）将STL塑造的奖励与PPO结合，在MuJoCo XLA中并行化训练Barkour四足机器人，并采用域随机化增强鲁棒性。

Result: 与手工设计奖励的基线相比，STL塑造的奖励实现了更紧密的速度跟踪和更稳定的训练过程。并行化训练加速了收敛，域随机化提升了策略的鲁棒性。

Conclusion: 基于STL的奖励塑造为四足机器人步态学习提供了一种可解释、可参数化的替代方案，能够显式编码步态行为约束，提升训练稳定性和控制精度，未来可推广到更复杂的机器人控制任务。

Classification reason: 该论文研究四足机器人步态控制，使用信号时序逻辑（STL）规范来塑造强化学习奖励，属于具身智能中机器人运动规划与控制范畴，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on quadruped locomotion with temporal logic, which is outside the user's core interests in continuous multimodal models and video generation. Signals: robotics, quadruped locomotion, temporal logic, reinforcement learning, not in priority subtopics
...
### [10] [From Technical Metrics to User Perception: A User Study of a Multimodal Human-Robot Interaction System for Object Detection and Grasping](https://arxiv.org/abs/2607.00530)
...
TL;DR: 本文通过用户实验验证了多模态人机交互系统中技术性能提升（端到端任务成功率从75%提升至90%）能否被用户感知。24名受试者的实验表明，70.83%的用户偏好改进后的系统，且在感知速度、可靠性和整体能力三个维度上改进系统均获得显著更高的评分。


<details>
  <summary>Details</summary>
Motivation: 技术指标（如任务成功率）的提升是否真正能被用户感知到，是HRI系统评估中的一个关键问题。本文旨在验证15个百分点的端到端任务成功率提升是否足以在用户交互体验中产生可测量的差异。

Method: 基线系统使用Whisper（语音识别）、Florence-2（开放词汇目标检测）、LLaMA 3.1（动作提取）和区间Type-2模糊逻辑控制器（运动执行）。改进系统将感知和语言模块替换为Grounding DINO + SAM和Qwen 3.5 9B。采用24名受试者的组内设计用户研究，在桌面物体抓取任务上比较两个系统，使用7点Likert量表评估感知速度、可靠性和整体能力。

Result: 17/24（70.83%）受试者偏好改进系统（精确二项检验，p=0.043，h=0.43）；改进系统在感知速度、可靠性和整体能力三个维度上均显著高于基线（Holm校正后p<0.001），效应量大到非常大。

Conclusion: 技术性能的改进确实能被用户在直接交互中感知到，这强调了在评估机器人操作流水线时，除了基准测试外还应补充以用户为中心的实证证据。

Classification reason: 该论文研究人机交互系统中的物体检测与抓取任务，涉及机器人操作、感知与动作执行，属于具身智能与机器人规划控制范畴，因此映射到 world_model 方向下的 [CANONICAL] 具身世界模型与规划控制 子主题。

Importance reason: User study on HRI perception is tangentially related to world models but lacks direct technical novelty in continuous multimodal or video generation. Signals: user study, human-robot interaction, object detection, grasping, multimodal system
...
### [11] [Domain Arithmetic: One-Shot VLA Adaptation under Environmental Shifts](https://arxiv.org/abs/2607.00666)
...
TL;DR: 提出DART方法，通过权重向量算术（子空间对齐+领域特定信息注入）实现VLA模型在环境变化下的单样本高效适应，仅需一条目标域演示即可完成迁移。


<details>
  <summary>Details</summary>
Motivation: VLA模型在环境变化（如相机位姿改变、机器人本体更换）下性能严重下降，而传统适应方法需要大量目标域演示数据，收集成本高昂。因此需要一种高效、低数据需求的VLA适应方法。

Method: 提出Domain ARiThmetic (DART)，基于类比推理的权重向量算术方法。核心思想是：通过源域与目标域的单条演示数据，计算领域特定权重向量，再通过子空间对齐过滤噪声分量，最后将干净的领域信息注入到预训练VLA模型权重中，实现单样本适应。

Result: 在仿真和真实机器人实验中，DART在单样本场景下显著优于现有VLA适应方法，在视觉变化（相机位姿）和本体变化（不同机器人）的多种迁移任务中均取得最佳性能。

Conclusion: DART通过权重向量算术和子空间对齐，以极低的演示数据需求实现了VLA模型在环境变化下的高效适应，为机器人策略模型的实用化部署提供了可行方案。

Classification reason: 该论文研究VLA模型在环境变化（相机位姿、机器人本体变化）下的适应方法，属于具身智能中机器人策略模型的环境泛化问题，与[具身世界模型与规划控制]方向高度相关。

Importance reason: Focuses on VLA adaptation for robotics, which is outside the user's core priority subtopics of continuous multimodal models and video/image generation. Signals: VLA adaptation, one-shot learning, weight arithmetic, environmental shift, robotics
...
### [12] [From World Models to World Action Models: A Concise Tutorial for Robotics](https://arxiv.org/abs/2607.00836)
...
TL;DR: 本文以设计空间视角系统梳理了世界模型在具身智能与机器人中的应用，将世界模型定义为动作条件预测模型，并进一步提出"世界动作模型"（World Action Model）概念，总结了四种将预测未来与机器人动作执行相连接的范式。


<details>
  <summary>Details</summary>
Motivation: 世界模型在不同社区中的定义和范围模糊不清，缺乏统一的概念框架来指导其在机器人控制中的实际应用。

Method: 提出设计空间视角，将世界模型分为观测空间和状态空间两类，分析其视觉保真度、空间结构、物理可解释性和控制可用性等权衡；进而引入世界动作模型，总结四种范式：先想象后执行、视频特征条件动作预测、联合视频-动作建模、辅助视频预测策略学习。

Result: 提供了一个结构化的世界（动作）模型分类体系，清晰界定了不同方法的适用场景与权衡。

Conclusion: 世界动作模型为连接预测未来与机器人动作执行提供了统一框架，有助于指导具身智能中预测与控制的系统化设计。

Classification reason: 该论文是一篇关于世界模型在机器人/具身智能中应用的教程，系统梳理了动作条件预测模型、world action model 以及四种具身控制范式，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Directly addresses world models for robotics, a high-weight direction, with a structured taxonomy that aligns with your priority on world models and neural simulators. Signals: World model tutorial, world action model, embodied intelligence, action-conditioned prediction, four paradigms
...
### [13] [Human-Centric Transferable Tactile Pre-Training for Dexterous Robotic Manipulation](https://arxiv.org/abs/2607.01067)
...
TL;DR: 本文提出 H-Tac，一个包含 160 小时人类视频、300+ 任务、135k 片段的大规模触觉-动作数据集，并基于此提出可迁移触觉预训练（TTP）框架，通过统一触觉与动作空间实现从人类数据到机器人的知识迁移，在仿真和真实机器人上展示了优越的灵巧操作性能。


<details>
  <summary>Details</summary>
Motivation: 触觉感知对于灵巧操作至关重要，但现有触觉数据集规模小、接触覆盖窄，且视觉-语言-动作（VLA）模型受限于缺乏动力学建模的后训练，限制了下游任务性能上限。

Method: 构建大规模人类触觉-动作数据集 H-Tac（160 小时人类视频，300+ 任务，135k 片段）；提出可迁移触觉预训练（TTP）框架，在预训练和后训练阶段使用统一的触觉与动作空间以保持人类到机器人迁移的先验知识；利用触觉专家模型进行未来触觉预测，显式建模接触动力学与物理交互。

Result: 在仿真和真实机器人上的大量实验表明，该方法实现了优越的性能，展现出鲁棒的泛化能力和精细操作能力。

Conclusion: TTP 通过人类到机器人的迁移为可扩展的触觉预训练铺平了道路，证明了大规模人类触觉数据结合统一表征可以有效提升灵巧机器人操作能力。

Classification reason: 该论文研究基于触觉的灵巧机器人操作，涉及触觉预训练、人类到机器人的迁移、接触动力学建模与精细操作控制，属于具身智能与机器人规划控制范畴，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Focuses on tactile pre-training for robotics, which is outside the user's core interests in continuous language/multimodal models and video/image generation. Signals: Tactile pre-training, Human-to-robot transfer, Dexterous manipulation, Large-scale tactile dataset, World model for contact dynamics
...
TL;DR: MuSix 是一个面向具身智能体的多尺度世界模型混合框架，通过尺度感知的路由机制和尺度依赖的遗忘率，解决了 MoE 在动态环境中缺乏尺度显式控制和统一更新策略的问题，在 EmbodiedBench 和 HAZARD 上取得了 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 具身智能体在真实世界中需要多尺度推理和知识适应，但现有 MoE 方法缺乏显式的尺度概念，无法在特定尺度上进行针对性更新，且统一的更新策略无法适应不同尺度知识过时速度的差异。

Method: 提出 MuSix 框架，包含两阶段路由机制：元路由器基于体验距离（受解释水平理论启发的情境新颖性度量）将权重映射到连续尺度空间，然后每个尺度的基础路由器选择该尺度内的世界模型。在适应方面，采用尺度依赖的遗忘率使低尺度知识快速更新而高尺度抽象知识持久保留，并通过门控跨尺度传输保持层级间的一致性。

Result: 在 EmbodiedBench 和 HAZARD 基准上的实验表明，MuSix 在多尺度推理和动态适应任务上优于现有 SOTA 基线方法。

Conclusion: MuSix 通过尺度感知的世界模型混合与演化，有效解决了具身智能体在动态环境中的多尺度推理和知识适应问题，验证了基于解释水平理论的尺度选择机制和尺度依赖遗忘率的有效性。

Classification reason: 论文提出 MuSix 框架，面向具身智能体在动态环境中的多尺度世界模型混合与演化，属于具身世界模型与规划控制方向，因此映射到 [CANONICAL] subtopic "具身世界模型与规划控制"。

Importance reason: Directly advances world model and efficient inference for embodied agents, aligning with top user priorities. Signals: world model, mixture of experts, multi-scale reasoning, dynamic adaptation, embodied agents
...
### [15] [FastBridge: Closing the Model-Based Realization Gap in Safety Filters on 3D Gaussian Splatting for Fast Quadrotor Flight](https://arxiv.org/abs/2607.01200)
...
TL;DR: 本文提出FastBridge，一种基于3D高斯泼溅(3DGS)的非线性、执行器感知的安全滤波器，用于四旋翼飞行器快速飞行中的避障。通过引入高相对度的碰撞锥指数CBF和备份CBF，在完整四旋翼动力学约束下保证QP可行性，相比现有方法减少47%轨迹抖动并提速2.25倍。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS安全滤波器使用简化模型（单/双积分器），忽略执行器限制并假设加速度瞬时实现，导致实际飞行中存在模型实现差距。本文旨在弥合这一差距，实现更安全、更平滑的快速四旋翼飞行。

Method: 基于3DGS的碰撞锥屏障，引入非线性执行器感知安全滤波器，通过完整四旋翼动力学约束实现。推导了高相对度碰撞锥指数CBF（ECCBF）和备份CBF（BCBF），利用前向模拟备份策略在输入约束下保持QP可行性。

Result: 相比SOTA 3DGS安全滤波器，轨迹抖动减少47%，运行速度快2.25倍。在仿真和真实硬件上验证了在杂乱感知环境中的实时导航能力。

Conclusion: FastBridge通过引入完整动力学约束的CBF设计，有效弥合了3DGS安全滤波器的模型实现差距，在保证安全性的同时显著提升了轨迹平滑度和计算效率，适用于快速四旋翼飞行。

Classification reason: 该论文研究基于3DGS的无人机安全滤波器，涉及四旋翼飞行器的动力学建模、碰撞避免与实时导航控制，属于机器人具身智能与规划控制范畴，因此映射到[具身世界模型与规划控制]。

Importance reason: Strong robotics safety filter work but outside user's core priority subtopics and negative keywords apply. Signals: 3D Gaussian Splatting, safety filter, quadrotor, collision cone CBF, actuator-aware
...
### [16] [FurnitureVLA: Learning Long-Horizon Bimanual Furniture Assembly with Vision-Language-Action Model](https://arxiv.org/abs/2607.01212)
...
TL;DR: FurnitureVLA首次系统研究真实尺度双臂机器人家具组装，提出进度增强的VLA模型，联合预测动作和连续进度信号，实现自动子任务切换，在仿真中将成功率从48%提升至80%，并在真实Kinova Gen3平台上验证。


<details>
  <summary>Details</summary>
Motivation: 现有机器人家具组装工作多局限于玩具级场景或单臂操作，缺乏对真实尺度双臂组装任务的系统性研究。长时序、多子任务、高精度的家具组装对机器人感知与控制提出了极大挑战。

Method: 提出FurnitureVLA框架：1）开发可扩展的仿真管线用于专家数据生成与评估；2）构建VR遥操作系统实现单操作员双臂控制以收集真实演示数据；3）提出进度增强的VLA模型，在语义子任务上微调，联合预测动作和连续进度信号，实现自动子任务转换并减少推理时的累积误差；4）系统研究感知与控制设计因素对精度的影响。

Result: 在三种家具类型上，仿真成功率从基线48%提升至80%，设计因素研究额外带来21%的提升。在真实Kinova Gen3平台上验证，最难任务仅下降16%的性能。

Conclusion: FurnitureVLA证明了进度增强的VLA模型在真实尺度双臂家具组装中的有效性，为长时序具身操作任务提供了可行的范式。

Classification reason: 该论文研究双臂机器人家具组装任务，使用VLA模型进行长时序动作规划与控制，属于具身智能/机器人领域的世界模型与规划控制，与[CANONICAL]具身世界模型与规划控制高度匹配。

Importance reason: Strong world model and VLA contribution for long-horizon bimanual assembly, but lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: world model, VLA, bimanual manipulation, long-horizon assembly, progress signal
...
### [17] [ABot-M0.5: Unified Mobility-and-Manipulation World Action Model](https://arxiv.org/abs/2607.00678)
...
TL;DR: ABot-M0.5 是一种面向移动操作机器人的统一世界动作模型，通过在时间粒度、动作空间和训练-测试一致性三个层面进行对齐设计（引入中间潜动作、双级MoT架构、dream-forcing训练策略），在长程任务成功率和精细控制精度上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有VLA策略通常是反应式的，缺乏显式世界建模；而已有的World Action Model（WAM）在移动操作任务中存在三个关键问题：操作在粗粒度视频块上、导航与操作动作纠缠、逆动力学训练与自回归推理条件不匹配，导致细粒度接触动力学丢失、动作分布冲突和长程误差累积。

Method: 提出ABot-M0.5，核心设计包括：(1) 时间粒度对齐——引入中间潜动作（intermediate latent actions）捕获局部视觉状态转移，作为视频潜空间与具体控制之间的桥梁；(2) 动作空间对齐——设计双级Mixture-of-Transformers架构，解耦模态表示与异构动作子空间（如基座移动与机械臂操作）；(3) 训练-测试一致性对齐——提出dream-forcing训练策略，在模型预测的视频上渐进训练逆动力学，提升自回归预测时的鲁棒性。

Result: 在具有挑战性的移动操作和精细操作基准上，ABot-M0.5在长程任务成功率和精细控制精度上均达到SOTA性能。

Conclusion: 实验结果表明，粒度对齐、动作解耦和推理一致的世界动作建模对于移动操作任务至关重要，ABot-M0.5验证了这三个层面对齐设计的有效性。

Classification reason: 该论文提出ABot-M0.5，一种面向移动操作（mobile manipulation）的统一世界动作模型（World Action Model），属于具身智能中机器人动作规划与控制的范畴，与[具身世界模型与规划控制]这一规范子主题高度匹配。

Importance reason: Strong world model paper with novel alignment techniques, but focuses on robotics rather than user's priority subtopics. Signals: world model, mobile manipulation, action alignment, dream-forcing training, Mixture-of-Transformers
...
### [18] [Learning When to Listen: Gated Affect Fusion for Human Motion Prediction](https://arxiv.org/abs/2607.00296)
*Jingni Huang*
...
TL;DR: 本文系统研究了在无约束真实世界视频中，利用面部情感特征辅助人体运动预测的有效性及时间局限性。提出门控情感变换器（GAT）动态调节跨模态信息流，实验表明朴素早期融合会降低预测精度，而门控机制能稳定融合；情感特征仅在短中期窗口（~30帧）内提供有限预测线索，长期轨迹仍由运动学连续性主导。


<details>
  <summary>Details</summary>
Motivation: 真实世界视频中的人体运动预测面临未来行为模糊性和多模态观测噪声的挑战。面部情感可能提供互补的行为线索，但其在实际运动预测框架中的效用和机制边界尚不明确。本文旨在系统研究情感条件化运动预测的有效性及其时间局限性。

Method: 构建了结合MediaPipe身体姿态轨迹与HSEmotion面部情感表征的多模态流水线，提出门控情感变换器（GAT），通过可学习的门控机制动态调节情感信息的跨模态融合。在严格的主体内协议下进行多时间跨度评估，并设计了随机/打乱情感输入的反事实实验验证门控效果。

Result: 朴素早期跨模态拼接一致地降低预测精度；GAT门控机制通过自适应控制情感流稳定了跨模态融合；门控能有效抑制无意义跨模态噪声同时对合理情感信号保持响应；面部情感特征仅在短中期窗口（~30帧）内提供有限预测线索，长期轨迹由运动学连续性主导。

Conclusion: 面部情感应被视为互补的行为线索而非未来运动的主导驱动因素，为无约束人体运动预测中的选择性多模态融合提供了实践指导。

Classification reason: 该论文研究在真实世界视频中基于多模态（姿态+面部情感）进行人体运动预测，属于动作条件预测与具身/环境建模范畴，与具身世界模型中的动作预测和规划控制方向高度相关，因此复用 [CANONICAL] 具身世界模型与规划控制 子主题。

Importance reason: Focuses on affect fusion for motion prediction, which is peripheral to the user's core interests in continuous multimodal models and video generation. Signals: Gated affect fusion, human motion prediction, in-the-wild video, temporal limitations of affect cues, MediaPipe + HSEmotion pipeline
...
### [19] [Towards Metric-Agnostic Trajectory Forecasting](https://arxiv.org/abs/2607.01133)
...
TL;DR: 本文提出一种度量无关的轨迹预测范式：使用度量无关的概率目标训练模型，将度量优化作为下游任务应用于预测分布。具体提出 TraDiE 策略（将预测分布映射为 K 条轨迹和置信度）和 DONUT-NLL 模型，在 Waymo 运动预测基准上取得 SOTA。


<details>
  <summary>Details</summary>
Motivation: 当前 SOTA 轨迹预测模型针对不同基准度量定制训练目标，但这些度量鼓励相互冲突的行为，导致模型难以同时优化所有指标。作者希望改变这一范式，将度量优化与概率建模解耦。

Method: 提出 TraDiE (Trajectory Distribution Evaluation) 策略——一组度量特定的策略函数，将预测分布映射为轨迹预测度量所需的 K 条轨迹及置信度。引入 DONUT-NLL 模型，将 SOTA 模型 DONUT 的训练目标改为直接优化预测分布的负对数似然（NLL），再通过 TraDiE 策略进行下游度量优化。

Result: DONUT-NLL 结合 TraDiE 策略在 Waymo 运动预测基准的所有度量上均达到 SOTA 结果。

Conclusion: 度量无关的概率训练范式是有效的，将度量优化作为下游任务可以避免不同度量间的冲突，实现更优的全局性能。

Classification reason: 该论文研究自动驾驶场景中的轨迹预测，属于具身智能中的运动规划与控制范畴，与 canonical subtopic "具身世界模型与规划控制" 高度匹配。

Importance reason: Focuses on trajectory forecasting for autonomous driving, which is not a priority subtopic and lacks direct connection to user's core interests in continuous multimodal models or video generation. Signals: Metric-agnostic training, TraDiE policy, DONUT-NLL model, Waymo benchmark SOTA, Autonomous driving focus
...
### [20] [Valdi: Value Diffusion World Models](https://arxiv.org/abs/2607.00917)
...
TL;DR: Valdi 提出了一种结合潜在扩散动力学模型与端到端在线训练的 MPC 框架，在 CarRacing 环境中使用单步扩散即可达到与确定性 MLP 基线相当的性能，并揭示了预测多模态性与控制性能之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 世界模型可用于模型预测控制（MPC），但需要同时满足在线使用的低延迟要求和表达不确定未来的能力。扩散模型天然适合建模不确定动力学，但其迭代推理过程难以用于低延迟的潜在规划。现有方法难以兼顾这两点。

Method: Valdi 将潜在扩散模型作为世界模型的动力学预测模块，并与端到端在线训练结合用于 MPC。在训练和推理时仅使用单步扩散，以平衡预测多模态性与推理速度。在 CarRacing 环境中进行初步实验验证。

Result: 在 CarRacing 环境中，Valdi 使用单步扩散在训练和推理时均能达到与确定性 MLP 基线相当的性能。实验揭示了预测多模态性与控制性能之间存在权衡关系。

Conclusion: Valdi 初步验证了将扩散世界模型与在线 MPC 结合的有效性，单步扩散即可匹配确定性基线，但多模态性与控制性能的权衡仍需进一步研究。

Classification reason: 该论文提出 Valdi（Value Diffusion World Models），将潜在扩散动力学模型与端到端在线训练结合用于模型预测控制（MPC），属于世界模型方向中面向规划与控制的工作，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Combines world models with diffusion for MPC, relevant to world model and efficient inference priorities. Signals: world model, diffusion model, model predictive control, latent planning, online training
...
### [21] [SD-RouteFusion: Ego-Trajectory Prediction with SD-Map Route Conditioning](https://arxiv.org/abs/2607.01139)
...
TL;DR: SD-RouteFusion提出一种端到端的自车轨迹预测方法，融合前视摄像头、车辆运动学与SD地图导航路线，在48万驾驶场景的大规模数据集上实现16.9%的ADE降低，无需HD地图基础设施即可实现路线感知的轨迹预测。


<details>
  <summary>Details</summary>
Motivation: 现有自车轨迹预测方法大多依赖高精地图（HD map），但HD地图成本高、维护难、可扩展性差。本文旨在利用更易获取的标准定义地图（SD map）中的导航路线信息，实现可大规模部署的路线感知轨迹预测。

Method: 提出SD-RouteFusion融合框架：1）将SD地图导航路线编码为语义先验；2）设计双假设（dual-hypothesis）结构，分别生成路线跟随和自由驾驶两种轨迹假设；3）使用门控分类器（gated classifier）在两种假设间动态选择，以应对路线损坏和视觉不确定性；4）融合前视摄像头图像、车辆运动学数据和SD路线信息进行端到端预测。

Result: 在覆盖10个欧洲国家和美国的48万驾驶场景数据集上，SD路线条件带来10.5%的ADE提升，完整融合策略在8秒预测范围内实现16.9%的ADE降低。同时发布了SD路线生成工具包，支持在任意包含自车位姿和未来轨迹的数据集上进行SD路线条件轨迹预测。

Conclusion: SD-RouteFusion证明了SD地图导航路线作为长程语义先验的有效性，为无需HD地图基础设施的路线感知轨迹预测提供了实用且可扩展的解决方案，推动了自车轨迹预测的实际部署。

Classification reason: 该论文研究基于SD地图导航路线的自车轨迹预测，属于具身智能中的规划控制任务，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on trajectory prediction with SD maps, which is peripheral to the user's core interests in continuous multimodal models and autoregressive generation. Signals: world model, trajectory prediction, SD map, autonomous driving, ego-trajectory
...
### [22] [Path Planning in Physically Viable World Models](https://arxiv.org/abs/2607.00673)
...
TL;DR: 本文提出一种物理可行的世界模型，将物理仿真与3D高斯泼溅场景重建相结合，使机器人能够在部署前评估未来地形变化对导航路径可行性的影响，无需重新采集传感器数据或重建地图。


<details>
  <summary>Details</summary>
Motivation: 机器人在非结构化户外环境中部署时，通常使用部署前采集的场景重建地图进行规划。由于无法在每次任务前重新测绘大面积或偏远区域，机器人必须依赖可能过时的地图，而环境的物理变化（如洪水、塌方）可能使原本可行的路径变得不安全或不可达。现有方法无法在无需重新采集数据的情况下评估未来环境变化对路径的影响。

Method: 系统首先使用3D高斯泼溅（3DGS）重建场景，然后通过基于物理的仿真（如模拟不同严重程度的洪水）生成同一环境的物理修改版本，无需重新采集传感器数据或重建地图。在此基础上，实现了一个地形感知规划器，能够考虑世界模型模拟的物理事件、障碍物和地形变形，使机器人或操作员在提交路径前评估路径是否仍然可行。

Result: 在德克萨斯州中部的真实户外场地进行实验，模拟了多个严重程度的洪水场景。结果表明，物理可行的世界模型能够暴露在原始重建环境中规划时无法发现的长时域路径失败和重新路由行为，使机器人能够在部署前评估未来地形变化对路径可行性的影响。

Conclusion: 物理可行的世界模型通过将物理仿真与3D场景重建结合，有效提升了机器人在动态变化环境中的路径规划鲁棒性，使机器人能够在条件变化前评估路径可行性，特别是在撤退或恢复可能变得不可能的受限环境中具有重要价值。

Classification reason: 该论文提出一种物理可行的世界模型（physically viable world model），通过物理仿真增强3D高斯泼溅场景，用于评估机器人导航路径在环境变化后的可行性。这属于世界模型方向下的可交互与物理世界建模（interactive_physical_world）范畴，该[CANONICAL]子主题涵盖物理一致的世界模型、交互式模拟器与环境动力学建模，与本文高度匹配。

Importance reason: Directly advances world model and neural simulator priorities with a novel physically viable world model for robot navigation. Signals: world model, neural simulator, 3D Gaussian splatting, physics-based simulation, terrain-aware planning
...
### [23] [AGI Maze as a Benchmark Framework for World-Modeling Agents](https://arxiv.org/abs/2607.00627)
*Alexey Potapov*
...
TL;DR: 本文提出AGI Maze，一个轻量级网格迷宫基准框架，用于评测LLM智能体在部分可观测、有状态环境中的世界建模能力。初步实验表明，普通LLM在推理时无法内部表示迷宫，即使允许使用消息历史作为工作记忆，LLM智能体在步数预算内仍无法可靠解决小规模迷宫。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在文本推理任务上表现优异，但在部分可观测、需要记忆和结构化假设的复杂环境中表现不佳。缺乏一个轻量级、无需高维感官输入的基准来系统评测智能体的世界建模能力。

Method: 提出AGI Maze框架，提供一系列基于网格的迷宫任务，包含简洁API和多种难度级别。智能体需要学习并使用世界状态表征，而非仅从给定观测中推断局部规则。框架支持部分可观测环境，要求智能体维护内部世界模型。作者还引入了一个基线智能体，允许其利用消息历史作为工作记忆来构建观测描述。

Result: 多个普通LLM在简单迷宫上无法在推理时内部表示迷宫结构。即使允许使用消息历史作为工作记忆，LLM智能体在充足的步数预算内仍无法可靠解决小规模迷宫，而人类可以轻松完成。

Conclusion: AGI Maze是一个有效的世界建模能力评测框架，揭示了当前LLM在需要持久、可操作的外部世界表征的任务上的根本局限性，为未来世界模型研究提供了有价值的基准。

Classification reason: 该论文提出了AGI Maze，一个用于评测世界建模智能体的轻量级基准框架，属于世界模型方向的评测基准。world_model方向下已有[CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark），虽然该subtopic原属multimodal_generation，但world_model方向下也有同名的generation_benchmark（papers: 5），且该论文核心贡献是评测框架而非具体世界模型方法，因此复用该subtopic最为合适。

Importance reason: Proposes a benchmark for world-modeling in LLMs, but is a lightweight grid maze framework with limited direct relevance to the user's priority subtopics on continuous multimodal models and video generation. Signals: world model benchmark, LLM agent evaluation, partial observability, grid maze, fails to represent state
...
### [24] [Structured 4D Latent Predictive Model for Robot Planning](https://arxiv.org/abs/2607.01166)
...
TL;DR: 本文提出一种结构化4D潜空间预测模型（Structured 4D Latent Predictive Model），在3D结构化潜空间中预测场景演化，结合文本指令和观测，解码为多种3D格式，并通过逆动力学模块将预测的未来场景转化为机器人可执行动作，在复杂操作任务上优于基于2D视频的规划方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频预测模型通常基于2D视频序列，缺乏3D几何理解，难以实现精确的空间推理和物理一致性。本文旨在引入3D结构化的潜空间预测，提升机器人规划的3D一致性和多视角连贯性。

Method: 提出结构化4D潜空间预测模型，将场景的3D结构编码为结构化潜表示，以观测和文本指令为条件预测场景演化；预测的未来场景通过目标条件逆动力学模块解码为可执行动作。该表示可解码为多种3D格式（如点云、体素等），支持多视角一致的场景理解。

Result: 实验表明，该方法生成的未来场景具有强视觉质量，在3D一致性和多视角连贯性上显著优于基于视频的规划器；在复杂操作任务上表现优越，对新颖视觉条件具有鲁棒泛化能力，并在真实机器人平台上验证了有效性。

Conclusion: 结构化4D潜空间预测模型通过引入3D几何先验到视频预测中，有效提升了机器人规划的3D一致性和任务性能，为具身智能中的长期规划提供了新的范式。

Classification reason: 该论文提出结构化4D潜空间预测模型，在3D结构化潜空间中预测场景演化，用于机器人规划，属于世界模型方向下的空间3D/4D世界模型（spatial_3d_4d_world_model）这一规范子主题。

Importance reason: Strongly aligns with world model and 3D structured latent prediction, a key priority subtopic with high weight. Signals: 4D latent predictive model, world model for robot planning, 3D structured latent space, inverse dynamics for action, strong 3D consistency
...
### [25] [Cross4D-JEPA: Dense Cross-modal Correspondence Distillation for 4D Point Cloud Representation Learning](https://arxiv.org/abs/2607.00514)
...
TL;DR: Cross4D-JEPA 提出一种教师-学生框架，通过密集跨模态对应蒸馏将冻结的2D基础模型（DINOv2/V-JEPA）的特征迁移到4D点云编码器，在四个基准上超越同类方法。


<details>
  <summary>Details</summary>
Motivation: 动态4D点云标注成本高昂，现有自监督方法几乎全是模态内预训练，少数跨模态方法仅使用单全局embedding，丢弃了2D基础模型丰富的逐patch语义信息。

Method: 采用教师-学生架构，教师为冻结的2D基础模型（DINOv2或V-JEPA），学生为4D点云编码器。通过密集跨模态对应将每个3D点映射到其投影对应的教师patch特征，在潜空间中逐点匹配，无需掩码、负样本或解码器。

Result: 在MSR-Action3D、DeformingThings4D、NTU-RGB+D 60和HOI4D四个基准上，一致优于模态内和全局跨模态基线；与更重的已发表4D方法竞争力相当；密集表征具有跨域迁移能力、标签效率高，且13倍小的编码器可匹配重权池化骨干。

Conclusion: 密集跨模态对应蒸馏是提升4D点云表征学习效果的有效途径，性能提升主要来自对应的细粒度而非教师模态本身。

Classification reason: 论文研究动态4D点云的自监督表征学习，属于动态3D/4D场景理解与建模，与 world_model 方向下的 [CANONICAL] 子主题 "空间 3D/4D 世界模型" 高度匹配。

Importance reason: Focuses on 4D point cloud representation, which is peripheral to your core interests in continuous language/multimodal models and video generation. Signals: 4D point cloud, self-supervised learning, cross-modal distillation, DINOv2/V-JEPA teacher, dense correspondence
...
### [26] [DeWorldSG: Depth-Aware 3D Semantic Scene Graph Generation via World-Model Priors](https://arxiv.org/abs/2607.00889)
...
TL;DR: DeWorldSG 提出一种深度感知的3D语义场景图生成框架，通过深度引导滤波估计实例级3D高斯分布作为概率节点，并利用世界模型（V-JEPA 2）的上下文先验聚合时空证据来补全缺失的关系，在3DSSG和ReplicaSSG数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D语义场景图生成方法因不稳定的3D物体表示和逐帧推理导致的关系稀疏性，难以构建时空鲁棒的场景图。

Method: 通过深度引导滤波估计实例级几何3D高斯分布，将每个物体表示为概率3D节点而非单点投影；跨物体对聚合时空证据，并利用世界模型V-JEPA 2的上下文先验来细化关系预测。

Result: 在3DSSG和ReplicaSSG数据集上，triplet召回率提升77.4%，predicate召回率提升23.2%，达到SOTA性能，且生成时空一致的场景结构。

Conclusion: DeWorldSG通过概率3D节点表示和世界模型先验有效解决了3D场景图生成中的不稳定性和关系稀疏性问题，适用于机器人操作和AR应用。

Classification reason: 该论文利用世界模型（V-JEPA 2）作为先验知识来生成3D语义场景图，属于世界模型方向中空间3D/4D世界建模的范畴，因此映射到[CANONICAL] subtopic "空间 3D/4D 世界模型"。

Importance reason: Strong world-model priors for 3D scene graph generation, but lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: World model priors, Depth-aware 3D scene graph, V-JEPA 2, Instance-level 3D Gaussians, SOTA on 3DSSG/ReplicaSSG
...
### [27] [RoboWorld: Fast and Reliable Neural Simulators for Generalist Robot Policy Evaluation](https://arxiv.org/abs/2607.01060)
...
TL;DR: RoboWorld 提出了一种结合快速自回归视频世界模型与任务进度感知 VLM 评分的自动化评估流水线，用于机器人策略评估。通过 Step Forcing 机制减少自回归 rollout 中的训练-测试不匹配，实现了与真实机器人评估高度一致的结果（Pearson r=0.989）。


<details>
  <summary>Details</summary>
Motivation: 真实世界机器人策略评估受限于物理约束和工程负担，视频世界模型虽可作为可扩展替代方案，但世界模型误差导致生成 rollout 不可靠，且推理速度慢限制了大规模吞吐量。本文旨在解决这些问题，实现快速且可靠的机器人策略评估。

Method: 提出 RoboWorld 评估流水线，包含两个核心组件：(1) 快速自回归视频世界模型，用于生成动作条件视频 rollout；(2) 任务进度感知 VLM 评分器，评估 rollout 中任务完成情况。提出 Step Forcing 机制，结合锚定上下文（anchored context）和单步自前馈上下文（one-step self-forwarded context），在保持动作-观测动态的同时减少训练-测试分布不匹配。

Result: 在多个任务和环境上，RoboWorld 与真实机器人评估高度对齐，Pearson r=0.989，Spearman ρ=0.970。相比基线方法，在评估可靠性和推理速度上均有显著提升。

Conclusion: RoboWorld 证明了视频世界模型可以成为可靠且高效的机器人策略评估工具，Step Forcing 机制有效缓解了自回归 rollout 中的累积误差问题，为大规模机器人策略评估提供了可行方案。

Classification reason: 该论文提出 RoboWorld，一个基于自回归视频世界模型进行机器人策略评估的流水线，属于视频世界模型方向，因此映射到 [CANONICAL] subtopic "视频世界模型" (video_world_model)。

Importance reason: Directly advances world model and video generation for robot policy evaluation, aligning with top priority subtopics. Signals: world model, video generation, autoregressive video world model, Step Forcing mechanism, task-progress-aware VLM scoring
...
TL;DR: 本文研究将预训练的基础视频世界模型（Cosmos3-Nano）通过LoRA适配到零售场景，比较了外视角（exocentric）和内视角（egocentric）训练数据的适配效果。实验发现，仅使用外视角数据训练的模型在6/7指标上达到或超过联合训练，且在外视角数据上添加内视角数据反而会降低性能。


<details>
  <summary>Details</summary>
Motivation: 基础视频扩散模型在互联网通用视频上预训练，与真实部署场景（如零售）存在领域差距。需要研究如何高效适配这些世界模型到特定领域，并探索不同视角（外视角vs内视角）的训练数据对适配效果的影响。

Method: 构建了RetailSMV数据集，包含从5家超市采集的32,105个带字幕的零售视频片段，同步采集了员工视角（内视角）和监控视角（外视角）的视频。使用Cosmos3-Nano作为基础视频世界模型，训练了三种LoRA配置：仅内视角、仅外视角、联合训练，在相同超参数下进行比较。使用7种互补指标在200个保留测试片段上进行严格配对统计评估。

Result: 仅外视角适配在6/7指标的点估计上达到或超过联合训练，在LPIPS、PSNR和DreamSim上显著更好。对称配对比较表明，在外视角数据上添加内视角数据会降低性能，而在内视角数据上添加外视角数据则有帮助。适配差距在最短展开时间窗口最大，表明近视野预测是适配最受益的区域。

Conclusion: 在零售场景中，外视角（监控视角）数据比内视角（员工视角）数据对视频世界模型的适配更有效，且外视角数据已足够达到甚至超过联合训练的效果。这为零售场景中视频世界模型的高效部署提供了指导。

Classification reason: 该论文研究将预训练的基础视频扩散模型（作为世界模拟器）通过LoRA适配到零售场景，属于视频世界模型在特定领域的适配与评测。同时涉及视频扩散模型的参数高效微调，与多模态生成方向下的生成模型加速与高效化（efficient_generation）也有一定关联。

Importance reason: Focuses on retail domain adaptation of video world models, which is a niche application not aligned with user's priority subtopics. Signals: Retail-specific dataset, LoRA adaptation, exocentric vs egocentric comparison, not autoregressive or continuous language model, no flow matching or multimodal latent reasoning
...
TL;DR: CORGI 是一个从单张野外图像进行一致性感知的3D狗重建框架，无需3D监督。它通过规范驱动轨道生成（CDOG）、一致性感知可变形3DGS（CA-3DGS）和自监督变形条件生成修复（DCGR）三个模块，生成几何精确、视觉一致且可动画化的3D资产。


<details>
  <summary>Details</summary>
Motivation: 从单张野外图像重建高保真3D高度关节动物（如狗）仍极具挑战性，现有方法存在生成不一致性和缺乏多视角观测的问题。

Method: 提出CORGI框架，包含三个核心模块：(1) Canonical-Driven Orbital Generation (CDOG) 策略，利用规范LoRA和轨道LoRA将任意输入姿态归一化并合成可靠的360度视频观测；(2) Consistency-aware Deformable 3DGS (CA-3DGS) 模块，基于D-SMAL先验，通过神经变形场显式建模每视角生成误差以学习精确的顶点级位移；(3) Deformation-Conditioned Generative Repair (DCGR) 模块，自监督地消除结构畸变并恢复高频细节。

Result: 实验表明CORGI在多种狗品种上达到最先进性能，生成几何精确、视觉一致且可动画化的3D资产。

Conclusion: CORGI成功实现了从单张野外图像到高保真3D狗模型的重建，无需3D监督，且生成的3D资产可直接用于下游应用。

Classification reason: 该论文从单张野外图像重建高保真3D狗模型，涉及3DGS、神经变形场等3D重建技术，属于多模态生成方向下的[3D 生成与重建]规范子主题。

Importance reason: Focuses on 3D dog reconstruction, which is not aligned with user's priority subtopics on continuous language models, video/image generation, or world models. Signals: 3D reconstruction, single image, dog, consistency-aware, no 3D supervision
...
### [30] [EO-VGGT: Orbital Ray-Conditioned 3D Foundation Models for Satellite Multi-View Reconstruction](https://arxiv.org/abs/2607.00417)
...
TL;DR: EO-VGGT 是一个面向卫星多视角重建的3D基础模型框架，通过显式物理几何（轨道推扫几何）适配冻结的透视假设模型，提出GCCS视图选择策略、Sensor-Ray Encoder参数化视线几何、以及Ray-Pointing-Aware Adapter将几何token注入Transformer骨干，实现鲁棒的卫星3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈3D基础模型基于透视投影假设设计，无法直接适配卫星遥感中轨道推扫几何的成像机制，且卫星多视角数据存在显著的视角异质性，导致直接迁移效果差。因此需要一种能显式融入轨道物理几何的适配框架。

Method: 1) GCCS（Geometry-Correlation Constrained Selection）：通过平衡几何多样性和辐射一致性，从多视角中剪枝次优观测，优化输入序列。2) SRE（Sensor-Ray Encoder）：基于有理函数模型（RFM）参数化像素级推扫视线，将其编码为高维空间几何token。3) RPAA（Ray-Pointing-Aware Adapter）：采用门控残差块将几何token注入冻结的Transformer骨干，实现物理几何与预训练模型的融合。

Result: 在卫星多视角重建任务上，EO-VGGT显著优于现有方法，能够从稀疏多视角卫星图像中生成高质量的数字表面模型（DSM），验证了显式物理几何与优化视图选择对前馈卫星3D重建的必要性。

Conclusion: 将显式物理几何与优化视图选择相结合，是使冻结的透视驱动3D基础模型适应卫星遥感重建的关键，为卫星多视角3D重建提供了有效的前馈框架。

Classification reason: 该论文关注卫星多视角图像的三维重建（DSM重建），属于3D场景重建范畴，与[3D 生成与重建]（three_d_generation）这一规范子主题高度匹配。

Importance reason: Focuses on satellite remote sensing 3D reconstruction, which is outside your priority subtopics and has negative keyword overlap. Signals: Satellite multi-view reconstruction, orbital pushbroom geometry, frozen 3D foundation model adaptation, view selection strategy, remote sensing application
...
### [31] [Robust 3D Alignment of Generative Reconstructions via Partial Monocular Observations](https://arxiv.org/abs/2607.00498)
...
TL;DR: 本文提出一种无需训练、可解释的几何对齐框架，通过 Sim(3) 相似变换将生成式3D先验与部分单目观测对齐，引入显式尺度因子、粗到细对齐策略和幻觉过滤操作，并构建了 GenPMOAlign 基准，显著优于传统和基于学习的基线方法。


<details>
  <summary>Details</summary>
Motivation: 生成式3D重建与部分单目观测之间的对齐是一个关键但未被充分研究的问题。由于单目输入稀疏、有噪声，而生成先验存在尺度模糊和几何幻觉，且两者缺乏初始重叠，传统配准方法无法有效处理这一任务。

Method: 提出基于 Sim(3) 相似变换的无训练几何对齐框架：(1) 引入显式尺度因子解决度量模糊；(2) 粗到细对齐策略，使用几何感知描述子进行鲁棒初始化，再通过解耦闭式求解器进行精调；(3) 幻觉过滤操作抑制由生成先验中幻觉几何导致的离群点。

Result: 在提出的 GenPMOAlign--Where2Place 基准上，该方法实现了稳定准确的配准，显著优于经典几何配准流程和基于学习的 SOTA 基线方法。

Conclusion: 本文提出的无训练几何对齐框架有效解决了生成式3D重建与部分单目观测对齐的挑战，为生成式3D先验在实际场景中的应用提供了可行的几何对齐方案。

Classification reason: 该论文研究将生成式3D重建与部分单目观测进行对齐的问题，属于3D生成与重建中的几何对齐任务，因此映射到 canonical 子主题 "3D 生成与重建"。

Importance reason: Strong geometric alignment work but lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: 3D alignment, generative reconstruction, monocular observation, training-free, hallucination filtering
...
### [32] [Restore3D: Breathing Life into Broken Objects with Shape and Texture Restoration](https://arxiv.org/abs/2607.00522)
...
TL;DR: Restore3D 是一个同时恢复破损 3D 物体形状和纹理的框架，利用多视角图像和 Mask Self-Perceiver 模块实现高质量、语义一致的多视角修复，并通过粗到精策略重建带纹理的 3D 网格。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 修复方法主要关注几何补全，忽略了纹理修复，且难以处理复杂多样的物体。文化遗产保护、遮挡物体重建和艺术设计等领域迫切需要同时恢复形状和纹理的完整 3D 修复方法。

Method: Restore3D 包含三个关键组件：(1) 自动化数据生成流水线，从大规模 3D 数据集合成配对的不完整-完整样本；(2) 多视角修复模型，核心是 Mask Self-Perceiver 模块和 Depth-Aware Mask Rectifier，通过学习修正的掩码引导图像集成与增强，保留观测到的形状纹理模式并提升生成区域分辨率；(3) 粗到精重建策略，从精炼的多视角图像恢复带纹理的高分辨率 3D 网格。

Result: 在合成和真实破损物体基准上的实验表明，Restore3D 在多视角修复质量和带纹理网格重建方面优于代表性的修复、补全和重建基线方法。

Conclusion: Restore3D 通过同时修复形状和纹理，为破损 3D 物体恢复提供了一种有效框架，在文化遗产保护等应用中具有潜力。

Classification reason: 该论文提出 Restore3D 框架，从多视角图像同时恢复破损 3D 物体的形状和纹理，属于 3D 物体重建与生成范畴，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Focuses on 3D shape and texture restoration, which is not a priority subtopic and lacks strong alignment with user's configured direction weights. Signals: 3D restoration, texture restoration, multi-view inpainting, Mask Self-Perceiver, coarse-to-fine reconstruction
...
### [33] [Efficient Compression of Structured and Unstructured Volumes via Learned 3D Gaussian Representation](https://arxiv.org/abs/2607.01164)
...
TL;DR: 本文提出一种基于3D高斯基元的显式体数据压缩模型，通过将3D高斯集合重新解释为标量场的显式表示，结合加权聚合采样策略和CUDA加速管线，在结构化与非结构化体数据上实现了优于隐式神经表示（INR）的压缩质量和训练速度，并天然编码几何信息，无需额外网格存储。


<details>
  <summary>Details</summary>
Motivation: 现有隐式神经表示（INR）在压缩非结构化体数据时无法编码几何信息，需要额外存储部分网格用于采样，限制了压缩率。同时，3D高斯泼溅在新视角合成中表现出色，但其在体数据压缩中的应用尚未被探索。

Method: 将3D高斯基元集合重新解释为标量场的显式表示，通过空间位置处与相交高斯的加权聚合重建标量值。设计了优化的CUDA加速采样管线、鼓励精确域编码的损失函数，以及基于采样误差的致密化策略。

Result: 在结构化体数据上，该方法达到与INR相当的重建质量，但训练速度显著提升；在非结构化体数据上，所有指标均明显优于现有INR方法，且天然编码几何信息，无需网格存储。

Conclusion: 基于3D高斯的显式模型为体数据压缩提供了一种高效的新范式，尤其适用于非结构化体数据，在压缩率、重建质量和训练效率之间取得了更好的平衡。

Classification reason: 该论文使用3D高斯基元（3D Gaussian primitives）对体数据进行压缩与重建，属于3D高斯泼溅（3DGS）相关方法，与[3D 生成与重建]子主题高度匹配。

Importance reason: Novel compression method using 3D Gaussians, but tangential to user's focus on continuous language/video generation and world models. Signals: 3D Gaussian representation, volume data compression, explicit model, CUDA acceleration, outperforms INR on unstructured data
...
### [34] [Pano2World: End-to-End 3D Generation via Unified Multi-View Sequences](https://arxiv.org/abs/2607.00832)
...
TL;DR: Pano2World 提出一种端到端方法，从单张室内全景图直接生成可自由探索的3D高斯场景。该方法先重建粗粒度3D高斯代理并渲染引导全景图，再通过全景扩散模型联合去噪所有目标视角（使用视角感知注意力路由），最后通过潜特征适配器将去噪隐特征直接解码为最终3D高斯场景，避免了VAE解码的信息损失。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么采用逐视角补全的迭代方式导致误差累积和繁琐的多步流程，要么依赖视频生成模型的时序一致性先验但受限于连续轨迹约束而无法同时覆盖多个方向。需要一种能从单张全景图直接生成一致、可探索3D场景的端到端方法。

Method: 1) 从输入全景图重建粗粒度3D高斯代理，并在自适应采样的邻近视角渲染几何对齐的引导全景图；2) 全景扩散模型通过视角感知注意力路由（View-Aware Attention Routing）联合去噪所有目标视图，每个目标视图同时接收来自对应引导全景图的几何约束和来自源全景图的全局语义约束；3) 引入潜特征适配器（Latent Feature Adapter），将联合去噪过程中形成的多视角隐特征直接蒸馏为场景潜码，再解码为最终3D高斯场景，避免VAE解码的信息损失。

Result: 在多位置全景新视角合成基准上，Pano2World 显著优于现有方法。

Conclusion: Pano2World 实现了从单张全景图到可探索3D高斯场景的端到端生成，通过视角感知注意力路由和潜特征适配器有效保证了跨视角一致性和信息完整性，为全景图驱动的3D场景探索提供了新范式。

Classification reason: 该论文从单张全景图直接生成可探索的3D高斯场景（3DGS），属于3D场景生成与重建范畴，与[CANONICAL] subtopic "3D 生成与重建"高度匹配。

Importance reason: Novel end-to-end 3D generation from panorama, but outside user's priority subtopics and negative keywords apply. Signals: 3D generation, panorama-to-scene, diffusion model, view-aware attention, latent feature adapter
...
### [35] [Improving Sparse-View 3DGS Generalization via Flat Minima Optimization](https://arxiv.org/abs/2607.00885)
...
TL;DR: 本文提出一种基于平坦最小值优化的轻量训练框架，通过各向异性感知的高斯扰动和周期性重初始化策略，改善稀疏视角下3DGS的过拟合问题，提升对新视角的泛化能力，无需修改模型架构。


<details>
  <summary>Details</summary>
Motivation: 3DGS在稀疏输入视角监督下容易过拟合到观测图像，对未见视角泛化能力差。现有方法缺乏从优化角度系统解决该问题的方案。

Method: 将平坦最小值优化思想适配到3DGS：1) 根据每个高斯的各向异性和训练进度施加受控的高斯参数扰动，在保持细节的同时提升鲁棒性；2) 引入周期性重初始化，在短窗口内将非位置参数临时恢复至初始状态以稳定优化过程。

Result: 在LLFF和Mip-NeRF360数据集上，稀疏视角设置下定量指标和感知质量均有提升，重建结果更清晰、更稳定，对新视角泛化更好。

Conclusion: 平坦最小值优化能有效缓解稀疏视角下3DGS的过拟合问题，所提方法轻量且即插即用，为3DGS泛化提供了新的优化视角。

Classification reason: 该论文研究稀疏视角下的3D高斯泼溅(3DGS)泛化问题，属于3D场景重建与神经渲染范畴，与[CANONICAL] subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Focuses on 3DGS generalization, which is not a priority subtopic and lacks direct connection to user's core interests in continuous language/multimodal models or video/image generation. Signals: 3D Gaussian Splatting, sparse-view generalization, flat minima optimization, no architectural changes, LLFF and Mip-NeRF360 datasets
...
### [36] [Relation-Centric Open-Vocabulary 3D Gaussian Segmentation](https://arxiv.org/abs/2607.01140)
...
TL;DR: PairGS 提出了一种基于高斯间关系建模的开放词汇 3D 高斯分割框架，通过构建关系图实现高效的多粒度查询分割，无需昂贵的逐场景优化，在基准上达到 SOTA，快速变体比优化方法快 50 倍。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇 3D 高斯分割方法要么将语言知识嵌入单个高斯（导致噪声分割），要么优化逐高斯实例特征（需要昂贵的逐场景优化）。作者希望设计一种既高效又准确的分割方法。

Method: PairGS 将高斯分割重新定义为高斯间成对关系建模。利用 3D 高斯表示提供的丰富信号（如视角贡献权重、多视图掩码证据）构建关系图。首先使用低维描述子提出稀疏边候选，仅对这些候选计算精确成对亲和度，然后构建层次聚类树以支持多粒度查询。

Result: 在开放词汇 3D 高斯分割基准上达到 SOTA 结果，快速变体比基于优化的实例特征方法快 50 倍。

Conclusion: PairGS 通过关系图建模有效解决了开放词汇 3D 高斯分割中的噪声和效率问题，验证了利用高斯表示固有信号进行关系推理的可行性。

Classification reason: 该论文研究开放词汇 3D 高斯泼溅分割，属于 3DGS 场景理解与分割任务，与 [CANONICAL] subtopic "3D 生成与重建" 中的 3DGS 方向紧密相关。

Importance reason: Strong 3D segmentation work but outside user's priority subtopics and negative for graph neural network. Signals: 3D Gaussian segmentation, open-vocabulary, relation graph, 50x faster, no per-scene optimization
...
### [37] [Ink3D: Sculpting 3D Assets with Extremely Complex Textures via Video Generative Models](https://arxiv.org/abs/2607.01222)
...
TL;DR: Ink3D 提出一种利用大规模视频生成模型为 3D 资产合成极其复杂纹理的框架。它先用现成 3D 模型重建白模几何，再用条件视频生成模型 OrbitPainter 生成密集轨道扫描视频，最后通过 TextureOptimizer 神经烘焙模块将多视角观测融合为一致纹理，实现比以往方法更丰富、更保真的纹理生成。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 生成模型能合成高质量几何，但由于缺乏大规模带有丰富表面外观的 3D 训练数据，难以从参考图像复现复杂纹理。而视觉生成模型在数量级更大的数据集上训练，擅长建模复杂视觉模式。作者希望弥合这一差距，利用视频生成模型的大规模先验来增强 3D 纹理生成。

Method: Ink3D 包含三个步骤：(1) 使用现成 3D 生成模型重建白模几何；(2) 提出 OrbitPainter，一个条件视频生成模型，生成密集轨道扫描视频以捕捉物体各视角外观；(3) 提出 TextureOptimizer，一个神经烘焙模块，融合密集多视角观测并缓解视频生成带来的几何不一致性，最终生成一致纹理。

Result: Ink3D 能够生成比以往方法显著更丰富、更保真的纹理，在复杂纹理合成方面取得了优异效果，展示了利用视频生成先验提升 3D 纹理质量的可行性。

Conclusion: 通过解耦几何与纹理合成，并利用大规模预训练视频先验，Ink3D 实现了远超先前方法的纹理生成质量，为 3D 资产生成中纹理复杂度的瓶颈问题提供了有效解决方案。

Classification reason: 该论文提出 Ink3D 框架，利用视频生成模型为 3D 资产生成复杂纹理，核心任务是 3D 纹理生成与几何重建，属于 3D 生成与重建范畴，因此映射到 canonical 子主题 three_d_generation。

Importance reason: Uses video generative models for 3D texture synthesis, but the subtopic is outside your priority areas. Signals: 3D texture generation, video generative model, neural baking, decoupled geometry and texture, complex texture synthesis
...
### [38] [Progressive Pose-Guided 4D Animal Reconstruction from Monocular Video](https://arxiv.org/abs/2607.00157)
...
TL;DR: 本文提出一种渐进式测试时优化框架，基于3D高斯泼溅从单目视频进行高保真4D动物重建。通过对称感知时序编码和部件条件形变机制，解耦关节姿态与非刚性形变，在多种动物物种上实现了优于现有方法的几何精度、时序一致性和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 现有4D动物重建方法要么依赖严格的类别先验（限制泛化性），要么使用无约束生成模型（牺牲输入保真度）。本文旨在弥合这一差距，实现无需类别先验的高保真4D动物重建。

Method: 提出渐进式测试时优化框架，基于3D Gaussian Splatting。核心包括：(1) 对称感知时序编码，利用双边线索并吸收相机估计漂移；(2) 部件条件形变机制，由可学习部件锚点和可学习蒙皮场引导，解耦关节姿态与非刚性形变。

Result: 在多种动物物种上实验，该方法在几何精度、时序一致性和视觉保真度方面均优于现有基线方法，即使在先验严重不匹配的情况下也能鲁棒泛化。

Conclusion: 粗粒度形状先验结合渐进式姿态-形变解耦策略足以实现跨物种的鲁棒4D动物重建，为无模板4D动态场景重建提供了有效范式。

Classification reason: 该论文从单目视频重建4D动物，使用3D Gaussian Splatting，属于动态4D场景生成与重建，与[CANONICAL] subtopic "4D 与动态场景生成" (dynamic_4d_generation) 高度匹配。

Importance reason: Focuses on 4D animal reconstruction, which is outside the user's priority subtopics and lacks alignment with key boost keywords. Signals: 4D reconstruction, monocular video, 3D Gaussian Splatting, animal-specific, test-time optimization
...
### [39] [World from Motion: Generative Dynamic Gaussian Reconstruction from Monocular Video](https://arxiv.org/abs/2607.01202)
...
TL;DR: 提出 World from Motion，一种从单目视频生成可自由渲染的动态3D高斯表征的方法。通过视频模型条件化于密集像素对齐的渲染结果，纠正重建伪影并填补缺失区域，在4D重建上达到SOTA，并泛化到野外视频。


<details>
  <summary>Details</summary>
Motivation: 从单目视频重建动态3D场景是极具挑战的任务，现有方法受限于输入视角稀疏，导致渲染伪影和缺失区域。作者希望利用视频生成模型的先验知识来补全和修正这些不完整/有缺陷的重建结果。

Method: 1) 构建对齐的多视角视频对和动态3DGS数据集，模拟单目重建的典型伪影；2) 训练视频模型，以初始重建的密集像素对齐渲染（外观、几何、3D运动）为条件，沿输入和目标相机轨迹生成修正后的渲染结果；3) 测试时将模型生成的新观测区域和运动蒸馏回单个一致的动态3DGS表征。

Result: 在多个基准上达到4D重建SOTA，能处理大视角变化和动态运动的野外视频，显著提升新视角合成质量和3D运动估计精度。

Conclusion: 将视频生成先验与动态3DGS重建相结合是一种有效的范式，能够从单目视频中生成高质量、可自由渲染的动态3D场景，为4D内容创建提供了新思路。

Classification reason: 该论文从单目视频生成可自由渲染的动态3D高斯表征（4D重建），属于多模态生成方向下的[4D 与动态场景生成]（CANONICAL）子主题，同时与[空间3D/4D世界模型]也有一定关联。

Importance reason: Strong 4D reconstruction from monocular video with video model conditioning, but not directly aligned with top priority continuous language/multimodal models. Signals: dynamic 3DGS, monocular video to 4D, video model conditioning, novel view synthesis, SOTA 4D reconstruction
...
### [40] [Decoupled Guidance: Disentangling Subject and Context Pathways in Text-to-Image Personalization](https://arxiv.org/abs/2607.00766)
...
TL;DR: 本文提出 Decoupled Guidance (DeGu)，一种即插即用的框架，通过将主体身份和场景上下文路由到两个独立的引导流中，并引入空间混合机制动态融合，解决了文本到图像个性化生成中保真度与可编辑性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像个性化方法将主体身份（保真度）和场景上下文（可编辑性）通过同一条件路径编码，导致两者竞争注意力图资源，产生"条件纠缠"现象，引发保真度-可编辑性权衡。作者希望解耦这两个目标以同时提升性能。

Method: 提出 Decoupled Guidance (DeGu)，包含两个独立引导流分别处理主体身份和场景上下文，并设计空间混合机制动态融合这两个流，确保每个流在其语义相关区域内操作而不相互干扰。DeGu 可作为即插即用模块应用于现有个性化方法，无需修改底层骨干模型。

Result: 在多种个性化方法和骨干模型（包括 flow-matching Diffusion Transformers）上，DeGu 一致提升了整体个性化性能，并实现了推理时对保真度-可编辑性平衡的可控调节。

Conclusion: DeGu 通过解耦条件路径有效缓解了条件纠缠问题，打破了保真度与可编辑性之间的固有权衡，为个性化生成提供了一种灵活、通用的改进方案。

Classification reason: 该论文研究文本到图像个性化生成（text-to-image personalization），属于主体驱动/概念定制生成方向，与 [CANONICAL] subtopic "主体与概念驱动生成 (customization_generation)" 完全匹配。

Importance reason: Directly addresses controllable generation and subject-driven generation, a priority subtopic, with a novel decoupling method applicable to flow-matching DiTs. Signals: controllable generation, subject-driven generation, flow-matching DiTs, plug-and-play, fidelity-editability trade-off
...
TL;DR: MindAU 提出了一种基于EEG信号控制面部动作单元（AU）编辑的统一框架，通过双流流形对齐将EEG特征与AU级文本语义和视觉位移轨迹对齐到Qwen2.5-VL多模态空间，结合EEG感知的多模态旋转位置编码、地标引导参考掩码和AU感知区域监督，实现高保真、身份保持的面部编辑。同时构建了E-CAFE基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有脑解码研究主要关注从神经信号重建外部视觉内容，但利用EEG信号指导面部表情编辑尚未被探索。该任务需要从噪声EEG信号中识别与面部动作相关的模式，并将其映射到局部化、身份保持的表情编辑中，具有独特挑战。

Method: MindAU包含三个核心模块：(1) 通过时间掩码重建和AU分类监督学习噪声鲁棒且AU判别性的EEG表示；(2) 双流流形对齐（Dual-Stream Manifold Alignment），将EEG特征与AU级文本语义和身份约简的视觉位移轨迹对齐到Qwen2.5-VL多模态空间；(3) 将EEG感知的多模态旋转位置嵌入、地标引导参考掩码和AU感知区域监督集成到多模态扩散编辑器中，实现高保真身份保持编辑。

Result: 大量实验证明了MindAU的有效性，在E-CAFE基准上实现了基于EEG信号的精细面部AU编辑，表明其作为面向面部神经肌肉障碍患者的辅助表达技术的潜力。

Conclusion: MindAU首次探索了EEG条件的面部动作单元编辑任务，通过双流流形对齐有效弥合了EEG信号与面部编辑之间的模态鸿沟，为未来辅助表达技术提供了新方向。

Classification reason: 该论文研究基于EEG信号的面部动作单元（AU）编辑，属于图像编辑中的可控面部编辑任务，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度匹配。同时涉及多模态理解与生成方向中的视觉语言模型（Qwen2.5-VL）作为多模态对齐空间。

Importance reason: Novel EEG-to-facial-AU editing but outside user's core priority subtopics and negative keywords. Signals: EEG-conditioned editing, dual-stream manifold alignment, multimodal diffusion, facial action unit, E-CAFE benchmark
...
### [42] [SPECSIA: Stylization Dataset for Novel-View Enhancement in Drawing-based 3D Animation](https://arxiv.org/abs/2607.00525)
...
TL;DR: 本文提出SPECSIA-15K数据集和DraViE轻量级模块，用于从单张2D绘画生成3D动画时的新视角伪影去除与风格保持，无需逐样本微调即可提升新视角保真度和时序一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于2D绘画的3D动画管线在生成新视角时，由于逐样本2D细化优化容易过拟合到观察视角，无法修正投影导致的伪影（如纹理扭曲、几何变形），需要一种能去除新视角伪影同时保持角色风格和运动合理性的方法。

Method: 构建SPECSIA-15K配对风格化数据集（14,980对伪影-修正图像对，来自1,498个3DBiCar角色）；提出DraViE（Drawing-based View Enhancement）轻量级即插即用模块，利用数据级先验训练，去除新视角伪影同时保持风格与运动合理性。

Result: 实验表明，DraViE在新视角保真度和时序一致性上持续优于逐样本微调方法，且每个角色的适配成本更低。

Conclusion: 通过数据驱动的先验学习，可以高效解决2D绘画3D动画中的新视角伪影问题，为轻量级、可泛化的动画增强提供了有效方案。

Classification reason: 该论文关注从2D绘画生成3D动画中的新视角增强与风格化，核心是去除新视角伪影并保持风格一致性，属于图像/视频编辑与增强范畴，映射到[CANONICAL] subtopic "图像与视频编辑" (visual_editing)。

Importance reason: Focuses on a narrow 2D-to-3D animation stylization dataset and artifact removal module, with limited connection to the user's priority subtopics like continuous language models or video generation. Signals: dataset contribution, lightweight module, novel-view artifact removal, drawing-based 3D animation, no direct link to priority subtopics
...
### [43] [GMO-E$^2$DIT: Grounded Multi-Operation Editing for E-Commerce Images](https://arxiv.org/abs/2607.00920)
...
TL;DR: GMO-E²DIT 是一个面向电商图像的智能编辑框架，通过VLM代理将模糊指令分解为区域级编辑计划，结合掩码条件图像编辑器与反思循环机制，实现多步、局部、可审计的图像编辑操作，在指令准确性和编辑保真度上达到与闭源模型竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 现有单次编辑模型将意图解析、空间定位和图像合成耦合为单一步骤，在处理电商场景中多操作、局部化、可审计的编辑需求时，常出现部分操作执行失败的问题，这在商业场景中不可接受。

Method: 提出GMO-E²DIT框架，包含：(1) VLM代理将模糊指令解析为区域级编辑议程，解耦认知推理与生成渲染；(2) 掩码条件图像编辑器通过操作感知掩码和参考图执行子程序；(3) 反思驱动循环检查中间结果、重试失败操作、恢复错误，确保可靠执行；(4) 统一数据流水线提供规划、执行和反思的对齐监督，以及EComEditBench评测基准。

Result: 在EComEditBench上，GMO-E²DIT在指令准确性和编辑保真度上优于现有基线方法，与GPT-4o等强闭源模型性能相当。

Conclusion: 将认知推理与生成渲染解耦的代理式编辑框架能有效处理电商图像的多操作编辑任务，反思循环机制显著提升了多步编辑的可靠性和鲁棒性。

Classification reason: 该论文提出电商图像的多操作编辑框架，属于图像编辑（visual_editing）这一[CANONICAL]子主题。同时，框架中使用了VLM进行规划与推理，也涉及多模态理解与生成方向。

Importance reason: Focuses on e-commerce image editing, which is not a priority subtopic and lacks direct alignment with user's core interests in continuous models, video generation, or world models. Signals: VLM agent for multi-step editing, mask-conditioned image editor, reflection loop for error recovery, e-commerce domain benchmark, decouples reasoning from generation
...
### [44] [GaussianEmoTalker: Real-Time Emotional Talking Head Synthesis with Audio-Driven and Blendshape-Based 3D Gaussian Splatting](https://arxiv.org/abs/2607.00959)
...
TL;DR: GaussianEmoTalker 提出基于 3D Gaussian Splatting 的实时情绪化说话头合成框架，将情绪动画建模为中性的情绪残差变形问题，通过 GaussianBlendshapes 构建中性说话空间，并利用空间-音频-情绪注意力模块预测高斯属性偏移，实现可控情绪强度、准确唇形同步和实时渲染。


<details>
  <summary>Details</summary>
Motivation: 现有音频驱动的说话头合成在唇形同步和视觉质量上取得了进展，但生成具有可控情绪强度的表情化虚拟人仍面临挑战，尤其是在实时性约束下。

Method: 首先构建身份特定的中性说话空间（GaussianBlendshapes），提供高保真高斯属性和音素同步的中性运动；然后预测情绪条件残差变形，融合网格位移线索、音频特征、情绪类别和强度编码；通过空间-音频-情绪注意力模块估计高斯属性偏移，实现富有表现力和时序稳定的渲染。

Result: 大量实验表明，GaussianEmoTalker 在视频质量、唇形同步精度、可控情绪表达和实时渲染方面均优于近期情绪化说话头方法。

Conclusion: 提出了一种基于 3DGS 的实时情绪化说话头合成框架，通过中性到情绪的残差变形策略和跨模态注意力机制，有效平衡了渲染质量、唇形同步、情绪可控性和实时性。

Classification reason: 该论文研究音频驱动的情绪化说话头合成（talking head synthesis），属于数字人与动作生成（avatar_motion_generation）这一[CANONICAL]子主题的典型范畴。

Importance reason: Focuses on emotional talking head synthesis with 3D Gaussian Splatting, which is not a priority subtopic and lacks strong alignment with user's key directions. Signals: 3D Gaussian Splatting, emotional talking head, real-time rendering, audio-driven, blendshape-based
...
### [45] [UltraFlux: Data-Model Co-Design for High-quality Native 4K Text-to-Image Generation across Diverse Aspect Ratios](https://arxiv.org/abs/2511.18050)
...
TL;DR: UltraFlux 提出数据-模型协同设计方法，基于 Flux/DiT 架构实现原生 4K 分辨率、多宽高比的文生图。通过 Resonance 2D RoPE 位置编码、非对抗 VAE 后训练、SNR-Aware Huber Wavelet 损失函数和分阶段美学课程学习，在 4K 评测上显著优于开源基线，并匹配或超越 Seedream 4.0。


<details>
  <summary>Details</summary>
Motivation: 现有扩散 Transformer（DiT）在 1K 分辨率下表现良好，但扩展到原生 4K 多宽高比时，位置编码、VAE 压缩和优化三者耦合失效，单独解决任一因素效果有限。需要数据-模型协同设计来突破这一瓶颈。

Method: (1) 构建 MultiAspect-4K-1M 数据集：100 万张 4K 图像，覆盖多种宽高比、双语字幕、VLM/IQA 元数据；(2) Resonance 2D RoPE + YaRN：实现训练窗口感知、频率感知和宽高比感知的 4K 位置编码；(3) 非对抗 VAE 后训练：提升 4K 重建保真度；(4) SNR-Aware Huber Wavelet 损失：跨时间步和频率带重新平衡梯度；(5) 分阶段美学课程学习：在高噪声步集中施加高美学监督。

Result: 在 Aesthetic-Eval at 4096 基准和多宽高比 4K 设置下，UltraFlux 在保真度、美学质量和图文对齐指标上持续优于强开源基线；配合 LLM 提示优化器可匹配或超越商业模型 Seedream 4.0。

Conclusion: 数据-模型协同设计是突破高分辨率文生图瓶颈的有效路径，UltraFlux 证明了原生 4K DiT 训练的可行性与优越性，为后续高分辨率生成研究提供了系统性的解决方案。

Classification reason: 该论文提出 UltraFlux，基于 Flux/DiT 架构实现原生 4K 文生图，属于文本到图像生成与扩散模型方向，因此映射到 [CANONICAL] subtopic "文生图与图像扩散" (text_to_image_diffusion)。

Importance reason: Strong data-model co-design for native 4K text-to-image generation with novel positional encoding and loss functions, directly relevant to image generation and diffusion transformer priorities. Signals: native 4K text-to-image, Flux/DiT architecture, Resonance 2D RoPE, non-adversarial VAE post-training, SNR-Aware Huber Wavelet loss
...
### [46] [Vertigo Vertigo: Reconstructing a Cinematic Ideal through its Predictive AI Double](https://arxiv.org/abs/2607.00047)
*Adam Cole, Mick Grierson*

Main category: cs.GR, cs.MM, cs.CV
...
TL;DR: 本文使用仅占原片2.78%的关键帧，通过大规模视频扩散模型逐场景重建了希区柯克的《迷魂记》（1958），生成完整的AI重构版本。评估显示73.1%的帧可被识别为原片的合理再现，仅3.6%完全失败，表明经典电影规范已深度压缩在生成模型的潜在先验中。


<details>
  <summary>Details</summary>
Motivation: 探索生成式AI系统是否内化了经典好莱坞电影的叙事与视觉规范，以及用AI重构经典电影作为媒介理论探针的可能性。

Method: 从《迷魂记》原始影片中提取稀疏关键帧（仅2.78%的帧），然后使用大规模视频扩散模型进行首尾帧插值（first-last frame interpolation），预测中间缺失序列，逐场景重建整部电影。

Result: 73.1%的帧被识别为原片的合理再现，仅3.6%完全失败。计算分析与媒介理论家（Lev Manovich, Shane Denson, Kevin L. Ferguson）的批判性反馈均表明重构具有显著的结构保真度。

Conclusion: 经典电影规范已深度压缩在生成模型的潜在先验中；生成式媒体并非对电影范式的颠覆，而是对其欲望与虚假真实性逻辑的加速，从经典好莱坞延伸到重塑当代感知的预测性媒体环境。

Classification reason: 该论文使用大规模视频扩散模型进行关键帧插值（first-last frame interpolation），属于视频扩散生成方法，与 [CANONICAL] 文生视频与视频扩散（text_to_video_diffusion）子主题匹配。

Importance reason: Focuses on cinematic reconstruction and media theory, not on core technical advances in video generation or continuous multimodal models. Signals: Video diffusion for film reconstruction, Low keyframe interpolation, Media theory analysis, Not a new model or method, No autoregressive or continuous token approach
...
### [47] [ECoSim: Data Efficient Fine-Tuning for Controllable Traffic Simulation](https://arxiv.org/abs/2607.00545)
...
Importance: 普通相关 (46.3) - 未入选精读

TL;DR: 本文提出 ECoSim，一种轻量级控制适配框架，通过在预训练的扩散/自回归交通模型上添加 identity-initialized FiLM 层，仅需不到 1% 的配对数据即可实现草图、潜行为码和文本等多模态可控交通仿真，支持反事实场景生成和长尾合成。


<details>
  <summary>Details</summary>
Motivation: 现有可控交通仿真方法通常需要重新训练大型生成模型并依赖大量标注数据，成本高昂且不灵活。本文旨在以极少的标注数据高效地为预训练交通模型添加新的控制模态。

Method: 提出轻量级控制适配框架，在预训练扩散/自回归交通模型的中间特征上插入 identity-initialized FiLM（特征线性调制）层，通过条件注入实现草图、潜行为码和文本三种控制模态的适配，同时保持基础模型的生成先验。

Result: 在 Waymo Open Sim Agents Challenge 上，仅用不到 1% 的配对控制数据即可实现强可控性，并保持稳定的闭环驾驶真实性和安全性，支持反事实场景生成和长尾合成。

Conclusion: ECoSim 通过轻量级 FiLM 适配，以极低数据成本为预训练交通模型添加多模态可控性，为可控交通仿真和自动驾驶测试提供了高效实用的新方案。

Classification reason: 论文关注可控交通仿真（traffic simulation），属于视频/场景生成范畴，使用扩散和自回归模型进行多模态可控生成。这与"文生视频与视频扩散"（text_to_video_diffusion）主题下的动作条件视频生成、可控视频生成高度相关，且该 canonical 主题可覆盖交通场景视频生成。

Importance reason: Focuses on traffic simulation, which is outside the user's priority subtopics and has negative keyword overlap. Signals: traffic simulation, controllable generation, FiLM layers, diffusion/autoregressive models, autonomous driving
...
### [48] [TrajLoc: Trajectory-Attention Localization for Multi-Object Motion Control](https://arxiv.org/abs/2607.00861)
...
TL;DR: TrajLoc 提出了一种基于轨迹注意力定位的多物体运动控制方法，通过为每个物体 token 在交叉注意力层中替换为高斯热力图来实现严格的逐物体空间约束，在 CogVideoX 5B 和 WaN 2.1 14B 两个骨干网络上均显著提升了视觉保真度和轨迹跟随精度。


<details>
  <summary>Details</summary>
Motivation: 现有图像到视频生成方法在处理多物体运动控制时，将多个轨迹纠缠在共享的密集条件信号中，导致物体数量增多、轨迹交叉或遮挡时难以保持物体级别的对应关系。需要一种能够独立隔离每个物体实例的运动控制方法。

Method: TrajLoc 在交叉注意力层中，将每个物体 token 的交叉注意力权重替换为以该物体目标位置为中心的高斯热力图，实现严格的逐物体空间约束。通过学习的嵌入向量携带轨迹和深度信息，并编码第一帧外观作为物体 token 来保持身份一致性。

Result: 在六个数据集上（最多 20 个同时控制物体及分布外真实场景），TrajLoc 在 CogVideoX 5B 和 WaN 2.1 14B 两个骨干网络上相比最强基线，PSNR 平均提升 +4.3 dB，轨迹终点误差降低 51%。

Conclusion: TrajLoc 通过解耦的逐物体空间约束有效解决了多物体运动控制中的身份保持和轨迹跟随难题，方法简洁且可迁移至不同架构的视频生成模型，为复杂场景下的可控视频生成提供了新范式。

Classification reason: 该论文研究图像到视频生成中的多物体运动控制，属于视频生成范畴。现有 [CANONICAL] 子主题 "文生视频与视频扩散" 涵盖视频扩散模型与视频生成方法，虽然论文侧重 I2V 而非 T2V，但视频扩散生成的核心主题一致，故复用该子主题。

Importance reason: Focuses on multi-object trajectory control in video generation, which is a niche application not aligned with the user's priority subtopics like continuous language models or world models. Signals: Multi-object motion control, trajectory attention, Gaussian heatmap, CogVideoX 5B, WaN 2.1 14B
...
### [49] [Vitality-Aware Compression for Efficient Image-to-Shape Diffusion Transformers](https://arxiv.org/abs/2607.00382)
...
TL;DR: 本文提出首个针对图像到3D形状生成DiT模型的压缩框架，通过活力引导的结构化剪枝、自适应量化和定向微调，在保持几何保真度的前提下实现高达66%的模型尺寸缩减。


<details>
  <summary>Details</summary>
Motivation: 现有的3D形状生成DiT模型虽然效果显著，但计算开销巨大，难以在资源受限场景部署。已有的扩散模型压缩方法主要针对图像/视频领域，难以直接迁移到3D生成；而现有的3D效率工作主要关注推理加速而非骨干网络压缩。

Method: 提出活力感知压缩框架（Vitality-Aware Compression），包含三个步骤：(1) 结构化剪枝——基于观察发现3D DiT各层对几何合成的重要性非均匀，定义"活力"指标衡量每层重要性并剪枝低活力层；(2) 自适应量化——对不同层采用不同位宽量化；(3) 定向微调——对压缩后模型进行针对性微调恢复性能。

Result: 在多个SOTA图像到3D模型上实现高达66%的模型尺寸缩减，同时生成质量与全尺寸模型相当，验证了该框架作为即插即用解决方案的潜力。

Conclusion: 本文证明了3D DiT层对几何合成的重要性存在非均匀性，并基于此提出了首个有效的图像到3D DiT压缩框架，为资源受限场景下的3D形状生成提供了可行方案。

Classification reason: 该论文提出针对图像到3D形状生成DiT模型的压缩方法（结构化剪枝+自适应量化+微调），属于生成模型效率优化范畴，与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度匹配。

Importance reason: Novel compression for 3D DiTs, but 3D generation is not a top priority subtopic. Signals: DiT compression, structured pruning, 3D shape generation, model efficiency, plug-and-play framework
...
### [50] [Post-Training Pruning for Diffusion Transformers](https://arxiv.org/abs/2607.00927)
...
TL;DR: 本文提出 DiT-Pruning，一种针对 Diffusion Transformers 的后训练剪枝方法。通过设计基于能量的新显著性度量（平衡权重与激活贡献）和聚类感知的剪枝粒度，在 50% 稀疏度下仅损失 0.001 CLIP 分数，显著优于现有剪枝方法。


<details>
  <summary>Details</summary>
Motivation: DiT 在图像生成中性能优异但计算开销巨大。现有后训练剪枝方法主要针对 LLM 设计，由于 DiT 独特的架构设计、参数分布和权重幅度远大于 LLM，直接套用会导致严重性能下降。因此需要专门为 DiT 设计剪枝方法。

Method: 提出 DiT-Pruning 方法：(1) 从能量角度设计新的显著性度量，平衡权重和激活的贡献，更有效识别重要元素；(2) 观察到二维权重空间中的聚类模式，采用聚类感知的剪枝粒度实现有效的稀疏分配。

Result: 在多种 DiT 模型上验证，FLUX.1-dev 在 512x512 分辨率 MJHQ 数据集上，50% 稀疏度下仅损失 0.001 CLIP 分数，大幅优于近期剪枝方法。

Conclusion: DiT-Pruning 通过定制化的显著性准则和剪枝粒度，有效解决了 DiT 后训练剪枝中的性能退化问题，在高稀疏度下仍能保持图像质量。

Classification reason: 该论文研究 Diffusion Transformers (DiTs) 的后训练剪枝方法，属于生成模型效率优化（加速、稀疏化、剪枝），与 [CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation) 高度匹配。

Importance reason: Directly addresses model efficiency for diffusion transformers, a key priority, with strong results and a novel method. Signals: post-training pruning, diffusion transformer, model efficiency, high sparsity, CLIP score preservation
...
### [51] [SNAP-FM: Sparse Nonlinear Accelerated Projection for Physics-Constrained Generative Modeling](https://arxiv.org/abs/2607.00095)
...
Main category: cs.CE, cs.LG, cs.AI
...
TL;DR: SNAP-FM 利用 GPU 稀疏非线性优化加速物理约束生成模型（如 Flow Matching）的约束投影步骤，通过利用块稀疏 Jacobian 和 KKT 系统结构，在保持约束满足的同时显著加速非线性约束投影。


<details>
  <summary>Details</summary>
Motivation: 生成模型作为物理模拟的替代方案缺乏对守恒律、边界条件等物理约束的保证。约束采样可以在推理时强制执行这些约束，但投影、校正等步骤计算成本高，尤其是非线性约束在标准 ML 框架（密集张量代数、稀疏求解器组合性差）下效率低下。

Method: 利用样本批处理和局部 PDE 耦合在投影子问题中自然产生的块稀疏 Jacobian 和 KKT 系统结构，通过 GPU 稀疏非线性优化求解器高效求解稀疏非线性规划，加速 Physics-Constrained Flow Matching (PCFM) 中的约束投影步骤。

Result: 在线性、非线性、一维和二维 PDE 基准上，SNAP-FM 显著加速了非线性约束投影，同时保持了约束满足精度。

Conclusion: GPU 稀疏非线性优化是科学机器学习中约束生成采样的实用基础，能够有效解决物理约束生成模型的计算瓶颈。

Classification reason: 论文提出SNAP-FM方法，通过稀疏非线性优化加速物理约束下的生成模型采样（Flow Matching），属于生成模型在推理时施加约束（可控性/对齐）的范畴，因此映射到[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）。

Importance reason: Focuses on physics-constrained generative modeling with sparse GPU optimization, which is outside the user's core priority subtopics. Signals: physics-constrained generative modeling, sparse GPU optimization, flow matching acceleration, nonlinear constraint projection, scientific machine learning
...
### [52] [PAPA: Online Personalized Active Preference Alignment](https://arxiv.org/abs/2607.00486)
...
Importance: 普通相关 (46.56) - 未入选精读

TL;DR: 本文提出PAPA（个性化在线主动偏好对齐）方法，无需参数化奖励模型，通过在线用户反馈直接优化扩散模型，实现反馈高效的偏好对齐。基于变分推断框架，PAPA能有效利用实时交互反馈来微调扩散模型，使其生成符合用户偏好的内容。此外，作者还提出加速变体EPAPA，降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 在个性化推荐等应用中，目标不是建模整个数据分布，而是建模能最大化用户偏好的特定区域。传统方法需要大规模偏好数据来训练参数化奖励模型，这在实践中往往不可行。因此，需要一种无需显式奖励模型、能通过在线交互反馈高效对齐用户偏好的方法。

Method: PAPA基于变分推断框架，将扩散模型的偏好对齐建模为在线优化问题，直接利用实时用户反馈（而非参数化奖励模型）来更新扩散模型。具体地，通过将偏好对齐视为隐变量模型中的变分推断，推导出可直接优化扩散模型的训练目标。EPAPA则基于理论分析，提出更高效的计算策略，加速微调过程。

Result: 在多种类别条件生成和细粒度对齐任务上的大量实验和消融研究表明，PAPA在反馈效率和对齐质量上均优于基线方法。EPAPA在保持对齐效果的同时显著降低了计算预算，加速了微调过程。

Conclusion: PAPA提供了一种无需参数化奖励模型的在线偏好对齐框架，通过变分推断直接利用用户反馈优化扩散模型，在反馈效率上具有优势。EPAPA进一步提升了实际部署的可行性。该方法为个性化生成模型的在线对齐提供了新思路。

Classification reason: 该论文提出PAPA方法，通过在线用户反馈直接优化扩散模型以对齐用户偏好，属于生成模型对齐与偏好优化方向，与[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Focuses on diffusion model alignment via online feedback, which is not a top priority subtopic and lacks direct connection to continuous multimodal or video generation. Signals: diffusion model alignment, online user feedback, no reward model, personalized generation, variational inference
...
### [53] [Flow-Map GRPO: Reinforcement Learning for Few-Step Flow-Map Generators via Anchored Stochastic Composition](https://arxiv.org/abs/2607.00535)
...
TL;DR: 本文提出 Flow-Map GRPO，一种针对确定性少步流映射生成器（如一致性模型、MeanFlow）的在线强化学习后训练框架。其核心组件 ASFMC（锚定随机流映射组合）通过锚点条件重采样引入随机性，同时保持原始确定性流映射的边缘概率路径。在基于 FLUX 的少步文生图模型上实验表明，该方法能有效提升奖励、感知和任务级指标。


<details>
  <summary>Details</summary>
Motivation: 确定性少步流映射生成器（如一致性模型、MeanFlow）虽然能加速采样，但由于其确定性本质，难以应用需要随机轨迹和似然比的强化学习后训练方法。现有的 SDE 随机化技术是为基于速度的采样器设计的，不适用于长程流映射。因此，需要一种专门针对确定性少步流映射生成器的 RL 对齐框架。

Method: 提出 Flow-Map GRPO 框架，核心是 Anchored Stochastic Flow Map Composition (ASFMC) 机制。ASFMC 通过锚点条件重采样引入随机性，在保持原始确定性流映射边缘概率路径的同时生成随机轨迹。在此基础上，推导了单时间和双时间流映射参数化下的 GRPO 目标函数，用于在线 RL 后训练对齐。

Result: 在基于 FLUX 的少步文生图模型（MeanFlow 和 sCM）上实验，Flow-Map GRPO 在奖励指标、感知质量和任务级评估指标上均优于预训练的确定性流映射基线模型，证明了确定性少步流映射生成器可以通过 RL 后训练有效对齐。

Conclusion: 本文证明了确定性少步流映射生成器可以通过 Flow-Map GRPO 框架进行有效的 RL 后训练对齐，无需修改原始模型参数化或重新训练为原生随机模型。该方法为加速生成模型的对齐提供了新途径。

Classification reason: 该论文提出 Flow-Map GRPO，一种针对确定性少步流映射生成器（如一致性模型、MeanFlow）的在线 RL 后训练对齐框架，通过锚定随机流映射组合（ASFMC）实现随机化并应用 GRPO 目标优化。核心贡献是将强化学习后训练对齐方法扩展到少步流映射生成器，属于生成模型对齐与奖励优化方向，因此映射到 [CANONICAL] subtopic "生成模型对齐与奖励" (generation_alignment)。

Importance reason: Directly advances RL alignment for few-step flow-map generators, a key technique for efficient continuous multimodal generation. Signals: continuous language model, flow matching, efficient inference, multimodal generation, reinforcement learning
...
### [54] [The Illusion of High Utility in Safety Alignment of Text-to-Image Diffusion Models](https://arxiv.org/abs/2607.00402)
...
TL;DR: 本文揭示文生图扩散模型的安全对齐方法存在"高效用幻觉"——粗粒度指标（FID、CLIPScore）看似保持效用，但细粒度语义评估（TIFA）显示物体计数、属性、关系等结构化语义大幅下降。作者诊断出文本编码器嵌入空间的"语义坍缩"现象，并提出结构感知几何正则化（SAGE）来保持嵌入分布与相似性结构，在维持安全性的同时恢复结构化效用（TIFA +5.0%）。


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法在粗粒度指标上表现良好，但缺乏对细粒度语义正确性的评估，可能造成"高效用"的假象。作者希望揭示这一幻觉，并找到安全对齐导致语义 fidelity 下降的根本原因，进而提出能同时保证安全性与语义正确性的对齐方法。

Method: 1) 使用 TIFA（基于 VQA 的细粒度语义评估）揭示安全对齐模型在物体计数、属性、关系等结构化语义上的显著下降。2) 分析文本编码器 prompt 嵌入空间，发现安全对齐导致嵌入分布收缩（spread 减小）和 inter-prompt 相似性结构扭曲，称为"语义坍缩"。3) 提出 StructureAware Geometric Regularization (SAGE)，在安全对齐微调时加入两项正则化：保持嵌入 spread 的几何正则化 + 保持 inter-prompt 相似性结构的结构正则化。

Result: SAGE 在 TIFA 细粒度语义评估上比此前最优方法提升 5.0%，同时保持强安全性能和可比的粗粒度指标（FID、CLIPScore）。实验验证了语义坍缩与结构化效用损失之间的强相关性，并证明 SAGE 能有效缓解语义坍缩。

Conclusion: 安全对齐方法存在"高效用幻觉"，粗粒度指标不足以评估对齐对生成质量的影响。文本编码器嵌入空间的语义坍缩是结构化效用损失的关键原因。SAGE 通过保持嵌入几何结构，实现了安全性与细粒度语义 fidelity 的双赢。

Classification reason: 该论文研究文生图扩散模型的安全对齐（safety alignment），属于生成模型对齐与奖励（generation_alignment）这一规范主题。论文提出新的对齐方法 SAGE，旨在平衡安全性与生成效用，与 canonical 主题「生成模型对齐与奖励」高度匹配。

Importance reason: Reveals a critical semantic collapse in safety alignment for diffusion models, but is not directly on the user's priority subtopics. Signals: Safety alignment illusion, Structured utility loss, Semantic collapse diagnosis, SAGE regularization, TIFA evaluation
...
### [55] [Training-Free Debiasing of Diffusion Models via CLIP-Guided Denoising Optimization](https://arxiv.org/abs/2607.00817)
...
TL;DR: 本文提出TES（Text Embedding Steering），一种无需训练的推理时框架，通过在扩散过程中直接优化条件文本嵌入来缓解文生图模型中的人口统计偏见（性别/种族），采用两阶段策略（早期全局对齐+迭代去噪时CLIP反馈优化），在不修改模型参数的情况下实现公平性控制。


<details>
  <summary>Details</summary>
Motivation: 现有文生图扩散模型在生成时存在严重的人口统计偏见（如性别和种族刻板印象），而现有去偏方法要么需要昂贵的重训练，要么在推理时干预会损害图像质量和语义对齐。因此需要一种无需训练、不牺牲质量的可控去偏方法。

Method: 提出TES框架，包含两阶段策略：(1) 早期阶段进行全局对齐，将条件文本嵌入向目标属性方向调整；(2) 迭代去噪阶段利用CLIP模型提供视觉反馈，在每一步去噪中优化文本嵌入以引导生成结果朝向期望的属性分布，同时保持语义一致性。整个过程无需修改模型参数。

Result: 在Stable Diffusion上的大量实验表明，TES在公平性指标上优于现有训练无关的基线方法，同时保持了具有竞争力的图像质量和语义对齐效果。

Conclusion: 推理时文本嵌入优化是一种实用且可扩展的公平性生成解决方案，能够在无需重训练的情况下有效缓解扩散模型中的偏见问题。

Classification reason: 该论文提出TES方法，通过CLIP引导的推理时文本嵌入优化来缓解扩散模型中的性别和种族偏见，属于生成模型的对齐与公平性控制，与[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Addresses bias in diffusion models but is tangential to user's core focus on continuous multimodal models and autoregressive generation. Signals: Training-free debiasing, CLIP-guided optimization, Inference-time fairness, Not in priority subtopics, No autoregressive or continuous token focus
...
### [56] [EquiSteer: Cross-Attention Steering Towards a Fairer Text-Guided Image Generation](https://arxiv.org/abs/2607.01147)
...
Importance: 值得关注 (68.39) - 未入选精读

TL;DR: EquiSteer 是一种无需训练的推理时方法，通过操控交叉注意力激活来减少文生图扩散模型中的性别偏见，在多个模型上平均将性别差距降低高达 87%，且对图像质量和文本对齐影响极小。


<details>
  <summary>Details</summary>
Motivation: 文生图扩散模型在训练数据中继承了人口统计偏见，例如"护士"偏向女性、"CEO"偏向男性。现有去偏方法需要重训练、批次级控制或特定提示调优，缺乏可扩展性。作者希望提出一种无需训练、逐样本工作的去偏方法。

Method: EquiSteer 是一种训练无关的推理时方法。首先，对每个目标属性（如性别），从对比提示对（如"a photo of a nurse" vs "a photo of a nurse, female"）预计算交叉注意力（CA）的引导向量。生成时，一个提示感知的门控机制区分属性特定提示（如"a female nurse"）和中性提示（如"a nurse"）：对中性提示，清除 CA 激活中已有的属性信号并注入目标属性信号；对属性特定提示则保持原样。

Result: 在 SD-1.5、SD-2.1、SDXL 和 SANA 四个模型上，EquiSteer 将平均性别差距降低高达 87%，同时保持图像质量和文本-图像对齐几乎不变。

Conclusion: EquiSteer 提供了一种高效、可扩展且无需训练的推理时去偏方案，能显著减少文生图模型中的性别偏见，且不牺牲生成质量。

Classification reason: 该论文提出 EquiSteer，一种无需训练的推理时交叉注意力引导方法，用于缓解文生图扩散模型中的性别偏见。这属于生成模型的对齐与可控性（公平性去偏）问题，与 [CANONICAL] subtopic "生成模型对齐与奖励" 高度匹配。

Importance reason: Focuses on fairness in image generation, which is not a priority subtopic and lacks direct connection to continuous multimodal models or world models. Signals: training-free debiasing, cross-attention steering, gender bias reduction, text-to-image diffusion, inference-time method
...
Importance: 普通相关 (63.54) - 未入选精读

TL;DR: 本文提出一个多智能体LLM框架，自动对化学反应进行分类并生成可验证的反应规则，将标准分类体系从68类扩展到14,073类，在66万+专利反应上实现97.7%的分类准确率，构建了一个可自我扩展的"活"反应数据库。


<details>
  <summary>Details</summary>
Motivation: 计算机辅助合成规划依赖大量反应规则库，但化学领域呈长尾分布，手动编码规则不可行，现有工具依赖固定规则集无法适应新化学。因此需要一种自动化、可扩展的反应规则生成与分类方法。

Method: 提出一个多智能体LLM框架，包含多个大语言模型智能体协同工作：对665,901个美国专利反应进行分类，并自动编写反应规则。每个规则在验证循环中生成，通过测试规则在语料库上的表现来确保正确性。使用轻量级指纹分类器进行最终分类。

Result: 将标准分类体系从68类扩展到14,073类，无需人工干预。轻量级指纹分类器对未见反应分类准确率达97.7%，与领先的商业分类器性能相当，但分辨率更细，且能按需扩展到训练分布之外的化学领域。

Conclusion: 本文展示了将生成式模型转化为可靠、可自我扩展的符号系统的通用路径，构建了一个"活"的反应数据库，为计算机辅助合成规划提供了可动态扩展的分类基础。

Classification reason: 该论文构建了一个自动化反应规则分类与生成管线，在665,901个专利反应上扩展标准分类体系从68类到14,073类，并评测了分类准确率（97.7%）。核心贡献是生成式模型驱动的符号系统扩展与评测，属于生成模型评测与数据集（generation_benchmark）范畴。

Importance reason: Focuses on chemistry reaction classification, far outside user's priority subtopics in multimodal generation and world models. Signals: LLM multi-agent framework, reaction rule generation, chemistry domain, low personal relevance, no overlap with priority keywords
...
TL;DR: MolSafeEval 是一个专门评估 AI 生成分子安全风险的基准，通过构建分子安全知识图谱并结合大语言模型推理，系统检测生成分子的毒性、反应性等安全隐患，覆盖无条件生成、性质优化、靶点蛋白设计和文本生成四类任务。


<details>
  <summary>Details</summary>
Motivation: 现有分子生成基准主要关注任务复杂度、分子新颖性和性质对齐，忽视了生成分子可能存在的毒性、反应性等安全风险，缺乏系统性的安全评估方法。

Method: 构建了一个结构化的分子安全知识图谱，整合毒理学数据库和危险规则等多种安全知识；基于该知识图谱，利用大语言模型进行推理，系统检测和解释生成化合物中的不安全特征；将分子生成模型分为无条件生成、性质优化、靶点蛋白设计和文本生成四类，并为每类提供标准化数据集和安全评估协议。

Result: 系统揭示了当前分子生成方法的安全漏洞，为分子模型评测提供了新的安全视角。

Conclusion: MolSafeEval 为 AI 生成分子的安全性评估提供了系统化基准，有助于推动更安全、更可信的分子设计。

Classification reason: 该论文提出了 MolSafeEval，一个专门用于评估 AI 生成分子安全风险的评测基准，属于生成模型评测范畴。现有 [CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark）完美匹配，该 subtopic 涵盖生成模型评测基准、新数据集等，MolSafeEval 正是分子生成领域的安全评测基准。

Importance reason: The paper focuses on molecular safety, which is outside the user's priority subtopics and is a negative keyword area. Signals: molecular generation safety, benchmark, knowledge graph, LLM reasoning, out-of-scope for user
...
### [59] [AV-SyncBench: Decoupled Benchmarking of Temporal and Semantic Audio-Visual Synchronization](https://arxiv.org/abs/2607.00726)
...
TL;DR: 本文提出AV-SyncBench，首个将时序和语义音视频同步评估完全解耦的基准。包含3,269个视频和38,390个样本，覆盖语音、音乐、声音三大类10个场景5个挑战任务，评估了5个代表性模型在音视频对齐和下游任务中的特征质量。


<details>
  <summary>Details</summary>
Motivation: 现有音视频特征提取模型的评估协议存在维度偏差，要么只关注语义匹配，要么只关注时序偏移检测，且数据构建耦合在一起，无法独立评估时序一致性和语义一致性。

Method: 从野外视频中构建数据集，经过自动过滤和人工验证确保屏幕内声源。将评估任务解耦为时序同步（偏移检测）和语义同步（匹配/不匹配检测）两个独立维度，覆盖语音、音乐、声音三大类共10个场景和5个挑战任务。

Result: 构建了3,269个视频和38,390个样本的基准，评估了5个代表性模型，量化了它们在音视频对齐和下游任务中的特征质量。

Conclusion: AV-SyncBench为音视频同步评估提供了首个解耦的基准，能够独立评估时序和语义一致性，有助于推动多模态理解与生成中音视频特征提取的发展。

Classification reason: 该论文提出了AV-SyncBench，一个专门用于音视频同步评测的基准数据集，包含时序和语义两个维度的解耦评估。这属于生成模型评测与数据集（generation_benchmark）范畴，该[CANONICAL] subtopic明确涵盖评测基准和新数据集。同时该工作也涉及多模态理解与生成方向，因此匹配两个方向。

Importance reason: Focuses on audio-visual synchronization evaluation, which is peripheral to your core interests in continuous multimodal models and video generation. Signals: Audio-visual benchmark, temporal vs semantic decoupling, not directly advancing generation or world models, limited overlap with priority subtopics
...
### [60] [Dataset Biases and Shortcut Learning in Motion-Based AI-Generated Video Detection](https://arxiv.org/abs/2607.00948)
...
TL;DR: 本文系统评估了四种基于运动的AI生成视频检测器，发现它们存在显著的预处理和采样偏差，且高度依赖数据集中"AI视频运动较少"这一偏置。当在无此偏置的数据集上评估时，所有检测器性能降至随机水平。相比之下，基于频率的检测器在所有数据集上保持强性能，表明频率方法可能更具泛化性。


<details>
  <summary>Details</summary>
Motivation: AI生成视频质量快速提升，人眼难以区分真假，现有基于运动的检测器声称性能优异，但其鲁棒性和泛化能力尚未被充分质疑。作者希望揭示这些检测器背后的偏差和捷径学习问题。

Method: 对四种SOTA基于运动的AI视频检测器进行系统性鲁棒性评估：1) 识别预处理和采样偏差；2) 分析运动偏置（AI视频帧间运动更少）；3) 在无运动偏置的数据集上测试；4) 进行数据集重平衡和空间增强扰动实验；5) 与基于频率的检测器对比。

Result: 1) 检测器性能很大程度来自预处理/采样偏差；2) 所有检测器高度依赖"AI视频运动更少"的数据集偏置；3) 在无此偏置的数据集上性能降至随机水平；4) 重平衡和空间增强导致严重性能下降；5) 基于频率的检测器在所有数据集上保持强性能。

Conclusion: 基于运动的AI生成视频检测器存在严重的捷径学习问题，其报告性能不可靠。频率域方法可能更具泛化性。呼吁构建更具代表性的无偏数据集和更鲁棒的评估协议。

Classification reason: 该论文研究AI生成视频检测器的鲁棒性和评估偏差，属于生成模型评测范畴，映射到[CANONICAL] subtopic "生成模型评测与数据集" (generation_benchmark)。

Importance reason: Focuses on detection bias rather than generation methods, which is peripheral to your priority on continuous multimodal models and video generation. Signals: motion-based detection, dataset bias, frequency-based detector, AI-generated video detection, robustness evaluation
...
### [61] [Towards Memory-Efficient Autoregressive Video Generation via Instance-Specific Parametric Absorption](https://arxiv.org/abs/2607.00712)
...
TL;DR: 提出ISPA框架，通过将部分注意力层从全局注意力转换为局部注意力，并将历史上下文"吸收"到模型权重中，实现自回归视频生成中KV缓存的高效压缩（最多减少50%），同时保持近无损的视觉质量。


<details>
  <summary>Details</summary>
Motivation: 自回归流式模型在长视频生成中面临KV缓存线性增长导致的内存过载和推理吞吐下降问题。现有方法通过丢弃冗余KV token来压缩缓存，但会破坏长程依赖，导致时序闪烁和身份丢失。

Method: 提出ISPA框架，核心思想是将KV缓存压缩从"丢弃"转变为"蒸馏"：将部分注意力层从全局注意力（F-Layer）转换为局部注意力（L-Layer），通过将历史上下文"吸收"到模型权重中。具体地，在短暂的预热阶段监控全局与局部注意力的输出差异，在转换点求解一个闭式最小二乘问题，计算实例特定的权重调制来补偿缺失的历史信息。

Result: 在1.3B到14B不同规模的架构上，ISPA可以移除高达50%的KV缓存，同时实现近无损的视觉质量，有效避免了时序闪烁和身份丢失问题。

Conclusion: ISPA提供了一种超越外部token级缓存管理的参数化记忆整合新视角，为流式生成模型的高效推理开辟了新方向。

Classification reason: 论文研究自回归视频生成中的KV缓存压缩问题，属于自回归视觉生成范式下的效率优化工作，因此映射到[CANONICAL] subtopic "自回归视觉生成"。

Importance reason: Directly addresses memory efficiency in autoregressive video generation, a top priority subtopic, with a novel parametric absorption method. Signals: autoregressive video generation, efficient inference, KV cache compression, memory-efficient, instance-specific weight modulation
...
### [62] [MEPA: Multi-Scale Representation Alignment for Visual Autoregressive Modeling with Mixture of Experts](https://arxiv.org/abs/2607.00371)
...
TL;DR: MEPA 针对 VAR 多尺度自回归生成中不同尺度表征学习冲突和早期语义误差传播的问题，提出尺度感知的 token 路由 MoE 架构实现解耦表征学习，并利用外部自监督特征通过残差特征聚合增强早期尺度语义建模，在 ImageNet 256×256 上以更少训练轮次和参数取得更优 FID。


<details>
  <summary>Details</summary>
Motivation: VAR 的粗到细多尺度自回归范式在图像生成中表现出色，但存在两个关键缺陷：(1) 低尺度捕获全局语义、高尺度关注细粒度细节，共享架构导致优化冲突；(2) 因果自回归过程中早期尺度的不准确语义会传播并严重降低最终输出质量。现有方法未能有效解决这些问题。

Method: (1) 提出尺度感知的 token 路由 MoE 架构（Scale-Aware Token-Routed MoE），为不同尺度分配不同的专家子网络，实现尺度自适应的解耦表征学习；(2) 引入外部自监督特征（如 DINOv2）增强早期尺度的语义建模，并设计残差特征聚合方案（Residual Feature Aggregation），将外部特征以残差形式注入 VAR 的 token 序列，避免与原生特征冲突。

Result: 在 ImageNet 256×256 上，MEPA 相比密集基线（VAR-d30）在仅需一半默认训练轮次和更小参数量预算下取得更优 FID，且训练成本仅略有增加。随着训练轮次增加，性能差距进一步扩大。

Conclusion: MEPA 通过 MoE 解耦多尺度表征和残差特征聚合增强早期语义，有效解决了 VAR 中的优化冲突和误差传播问题，显著提升了训练效率和生成质量，验证了在自回归视觉生成中引入尺度感知架构和外部语义特征的有效性。

Classification reason: 该论文提出 MEPA，在 VAR（Visual AutoRegressive modeling）框架下引入 MoE 架构和多尺度表征对齐，属于自回归视觉生成范式。因此映射到 multimodal_generation 方向下的 [CANONICAL] 子主题 "自回归视觉生成"（autoregressive_visual_generation）。

Importance reason: Directly advances autoregressive image generation with a novel MoE architecture that addresses multi-scale representation conflicts, aligning with top priority subtopics. Signals: autoregressive image generation, mixture of experts, multi-scale representation, visual autoregressive modeling, efficient training
...
### [63] [Enhancing Flow Matching with A Unified Guidance Framework for Efficient and Robust Speech Synthesis](https://arxiv.org/abs/2607.00363)
...
TL;DR: 本文提出一种统一引导框架，通过数据引导（异构增强）和模型引导（轨迹修正+内在引导目标）两种互补策略，加速 Flow Matching 语音合成近3倍，同时提升说话人相似度并缓解音色泄露问题。


<details>
  <summary>Details</summary>
Motivation: Flow Matching 在语音生成中表现出色，但存在推理延迟高和音色泄露（timbre leakage）两大瓶颈，亟需提升生成效率和鲁棒性。

Method: 提出统一引导框架：(1) 数据引导（Data-guidance）：通过异构数据增强迫使模型解耦语言内容与声学残差；(2) 模型引导（Model-guidance）：结合轨迹修正与内在引导目标，将条件知识蒸馏到网络权重中，拉直推理轨迹路径，从而消除无分类器引导（CFG）的开销。

Result: 实验表明，该方法在保持或提升语音质量的前提下，推理速度提升近3倍，说话人相似度优于现有最先进基线。

Conclusion: 统一引导框架能有效解决 Flow Matching 语音合成中的效率与鲁棒性问题，为高效、高质量的语音生成提供了可行方案。

Classification reason: 该论文研究语音合成中的 Flow Matching 方法，属于音频生成范畴，映射到 [CANONICAL] 子主题 "音频与多模态联合生成" (audio_av_generation)。

Importance reason: Flow matching guidance for speech synthesis is tangentially relevant to continuous language models but not a priority subtopic. Signals: Flow matching acceleration, speech synthesis, guidance framework, trajectory rectification, timbre leakage mitigation
...
### [64] [A Geometric Perspective on Composable Emotion Steering in Text-to-Speech Models](https://arxiv.org/abs/2607.00946)
...
TL;DR: 本文首次比较了语音语言模型（SLM）和条件流匹配（CFM）模块作为激活引导位点用于混合情感语音合成。研究发现SLM具有干净的低维情感子空间和良好的说话人-情感解耦，而CFM因说话人-情感纠缠导致跨说话人泛化差。联合引导虽能增强情感强度，但会降低比例控制和语音质量。


<details>
  <summary>Details</summary>
Motivation: 先前工作探索了混合TTS系统中的情感控制，但这些模块的几何特性及其对可引导性的影响尚不明确。本文旨在从几何视角理解不同模块的情感表征结构，为多站点激活引导提供实用指导。

Method: 使用线性探针（linear probing）和局部本征维度（LID）表征情感表示，在SLM和CFM模块上分别进行单站点和联合站点激活引导，实现混合情感语音合成。

Result: SLM提供干净的低维情感特定子空间且说话人-情感解耦良好；CFM因说话人-情感纠缠导致跨说话人泛化差；联合引导增强情感强度但降低比例控制和分布内数据上的语音质量。

Conclusion: SLM和CFM作为激活引导位点各有优劣，联合引导需权衡情感强度与语音质量，表征几何特性对可控语音生成至关重要。

Classification reason: 该论文研究文本到语音（TTS）中的情感控制与混合情感合成，属于音频生成范畴，映射到 [CANONICAL] 音频与多模态联合生成（audio_av_generation）子主题。

Importance reason: Focuses on emotion steering in TTS, which is outside the user's priority subtopics and lacks key boost keywords. Signals: Speech language model, conditional flow-matching, emotion steering, hybrid TTS, representation geometry
...
### [65] [Steal the Patch Size: Adversarially Manipulate Vision-Language Models](https://arxiv.org/abs/2607.00174)
...
TL;DR: 本文提出一种黑盒模型窃取攻击，利用ViT分块机制产生的任务级侧信道，通过合成网格图像与隐藏patch网格对齐时精度周期性下降的现象，推断VLM的视觉patch size和预处理配置（动态/固定分辨率、resize分辨率）。在开源Qwen-VL和闭源GPT、Claude上成功恢复tokenizer参数，并展示了泄露信息可用于预处理感知的迁移攻击和模型定向对抗操纵。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLM）的视觉tokenizer配置（如patch size、预处理管线）通常被视为非公开的模型细节。作者发现这些配置可以通过一种任务级侧信道泄露，且泄露的信息可被攻击者利用来实施更有效的对抗攻击。因此，揭示这一漏洞并展示其安全影响是本文的核心动机。

Method: 核心方法基于ViT分块（patchification）的特性：当输入一个合成网格图像，网格线与patch边界对齐时，边界线索在tokenization过程中被消除，导致模型精度周期性下降。通过扫描网格单元尺寸并测量精度坍塌点，推断patch size；通过引入padding和一致性检查测试，进一步区分动态分辨率与固定分辨率预处理，并恢复目标resize分辨率。攻击完全在黑盒条件下进行，仅需模型输出（如分类logits或文本响应）。

Result: 在开源Qwen-VL系列（Qwen-VL-Chat、Qwen2-VL-7B/72B）以及闭源模型（GPT-4o、GPT-4V、Claude 3.5 Sonnet）上成功恢复了patch size和预处理配置。例如，Qwen-VL-Chat的patch size=14、动态分辨率预处理等参数被准确推断。基于泄露信息，作者构建了预处理感知的迁移攻击，相比基线攻击成功率显著提升。

Conclusion: 本文揭示了VLM视觉tokenizer配置可通过任务级侧信道泄露，且该泄露具有实际安全风险——攻击者可利用这些信息实施更有效的对抗攻击。研究呼吁模型部署者关注此类侧信道漏洞，并考虑对tokenizer参数进行保护或混淆。

Classification reason: 该论文提出一种针对视觉语言模型（VLM）的黑盒模型窃取攻击，通过ViT分块产生的侧信道泄露来恢复视觉tokenizer配置（patch size、预处理管线），并利用泄露信息进行对抗性操纵。这属于多模态安全与攻防（对抗攻击、模型窃取）范畴，因此映射到[CANONICAL] subtopic "多模态安全与攻防"。

Importance reason: Novel side-channel attack on VLM tokenizer, but falls outside user's priority subtopics and is flagged by negative keywords. Signals: Black-box model stealing, ViT patch size inference, Side-channel attack, VLM security, Not in priority subtopics
...
### [66] [HARC: Coupling Harmfulness and Refusal Directions for Robust Safety Alignment](https://arxiv.org/abs/2607.00572)
...
Importance: 普通相关 (52.58) - 未入选精读

TL;DR: 本文发现对齐LLM在残差流中编码了有害性（harmfulness）和拒绝（refusal）两个可分离方向，越狱攻击通过抑制其中一个方向实现。作者提出HARC方法，将这两个方向在提示端和响应端进行耦合微调，在不损害通用能力的前提下显著提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 理解对齐LLM内部如何表征安全信息对于诊断对齐漏洞至关重要，现有工作虽发现有害性和拒绝方向的存在，但缺乏对越狱机制的系统性分析以及基于这些方向的鲁棒对齐方法。

Method: HARC（Harmfulness-And-Refusal Coupling）是一种微调方法，将有害性和拒绝两个方向在提示端和响应端进行配对耦合干预，仅作用于有害性-拒绝子空间，不影响残差流的其余部分。

Result: 在五个模型家族和两种规模上，HARC在鲁棒性-能力-可用性权衡上优于六种基线方法（涵盖训练时和推理时的安全方法），且不增加过度拒绝或降低通用能力。

Conclusion: 有害性和拒绝方向在提示端和响应端跨模型家族和规模可迁移，HARC通过耦合这两个方向实现了鲁棒的安全对齐，为设计更安全的LLM提供了新思路。

Classification reason: 该论文研究LLM安全对齐中的有害性方向与拒绝方向耦合方法，属于多模态安全与攻防（multimodal_safety_attack）范畴下的安全对齐与防御方向，与现有[CANONICAL]子主题高度匹配。

Importance reason: Focuses on LLM safety alignment, which is outside your configured priority subtopics and negative keywords. Signals: safety alignment, jailbreak analysis, harmfulness-refusal subspace, robust fine-tuning, cross-model transfer
...
### [67] [Adversarial Pragmatics for AI Safety Evaluation: A Benchmark for Instruction Conflict, Embedded Commands, and Policy Ambiguity](https://arxiv.org/abs/2607.01153)
*Brett Reynolds*
...
TL;DR: 本文提出"对抗性语用学"（adversarial pragmatics）作为评估语言模型安全性的基准与标注协议，系统处理指令冲突、嵌入命令、引用、范围歧义、指示语、间接言语行为及多轮智能体对话等语言现象，并构建了18项种子基准、54行本地试点、专家评估协议及评估者有效性、诊断歧义性、分类漂移等指标，将语言学判断方法转化为安全评估的实用工具。


<details>
  <summary>Details</summary>
Motivation: 现有安全评估基准通常将复杂的语言行为差异压缩为通过/失败标签，掩盖了失败源于能力限制、策略歧义、指令冲突、脚手架失败还是评估者判断不稳定等问题，缺乏对自然语言中微妙语用现象的细粒度标注与评估方法。

Method: 提出对抗性语用学框架，包括：语言学控制的分类体系、18项种子基准（含验证者强制元数据）、54行本地试点、区分任务成功/策略合规/安全风险/拒绝结果/评估者信心的专家评估协议，以及评估者有效性、诊断歧义性和分类漂移等指标。

Result: 构建了种子基准和试点数据集，通过专家评估协议验证了框架的有效性，展示了如何将语言学判断方法论转化为安全评估、LLM评判器、金标准构建、提示注入测试和安全文档的实用工具。

Conclusion: 对抗性语用学为语言模型安全评估提供了更细粒度、语言学严谨的方法论框架，有助于更准确地诊断安全失败的根本原因，提升安全评估的可靠性和可解释性。

Classification reason: 该论文关注语言模型的安全评估，涉及指令冲突、嵌入命令、策略歧义等对抗性语用学问题，属于多模态安全与攻防（multimodal_safety_attack）范畴下的安全评测与基准构建，因此复用该规范子主题。

Importance reason: Focuses on adversarial safety evaluation for language models, which is outside your configured priority subtopics and negative-keyword-filtered areas. Signals: safety evaluation, adversarial pragmatics, instruction conflict, benchmark, not in priority subtopics
...
### [68] [Surfacing Variations to Calibrate Perceived Reliability of MLLM-generated Image Descriptions](https://arxiv.org/abs/2507.15692)
*Meng Chen, Akhil Iyer, Amy Pavel*

Main category: cs.CV, cs.HC, cs.CL
...
TL;DR: 本文探索通过系统性地呈现MLLM多次响应之间的差异（variations），帮助盲人和低视力用户检测不可靠的图像描述信息。作者设计了三种差异呈现风格的原型系统，并通过15名盲人用户的实验证明，该方法能将用户识别不可靠信息的能力提升4.9倍，14/15的用户更偏好差异呈现方式。


<details>
  <summary>Details</summary>
Motivation: MLLM在图像描述中常产生错误，盲人和低视力用户无法通过视觉检查来发现这些错误，导致在用药识别、着装选择等场景中存在安全和社交风险。现有的交叉验证和求助明眼人等方法耗时且不实用，亟需一种无需视觉检查即可帮助用户判断MLLM输出可靠性的方法。

Method: 作者首先定义了MLLM图像描述中差异的设计空间（design space），包括差异的诱发方式（如温度采样、提示变化）和呈现方式。基于此设计空间，构建了一个原型系统，实现了三种差异呈现风格：列表式（List）、对比式（Compare）和摘要式（Summary）。通过15名盲人和低视力用户的用户研究，评估不同呈现风格对用户识别不可靠信息能力的影响。

Result: 呈现差异相比单一描述，用户识别不可靠信息的能力提升了4.9倍；用户对MLLM响应的感知可靠性显著降低；14/15的参与者更偏好差异呈现方式而非单一描述；所有参与者均表示有兴趣在实际场景中使用该系统。

Conclusion: 系统性地呈现MLLM多次响应间的差异是一种有效且实用的方法，能够帮助盲人和低视力用户更好地判断图像描述的可信度，提升MLLM在无障碍场景中的安全性和可用性。

Classification reason: 该论文关注MLLM生成图像描述中的可靠性问题，通过呈现多个响应间的差异来帮助盲人和低视力用户检测不可靠信息，属于多模态安全与可靠性范畴。虽然论文侧重辅助技术而非传统攻防，但其核心是提升MLLM输出的可靠性和安全性，与[多模态安全与攻防]（multimodal_safety_attack）中"安全评测"、"可靠性"等关键词高度吻合，因此复用该规范子主题。

Importance reason: The paper focuses on MLLM reliability for accessibility, which is not a priority subtopic and lacks alignment with the user's strong emphasis on continuous models, generation, and world models. Signals: MLLM reliability, blind/low vision users, variation presentation, user study, accessibility
...
### [69] [Synergistic Perception-Reasoning Governance: Grounding Medical MLLMs with Verifiable Anatomical Evidence](https://arxiv.org/abs/2607.00060)
...
TL;DR: 本文提出一种无需额外训练的证据注入框架，通过ROI引导的视觉感知调制和解剖坐标到语义token的文本锚定，系统性地缓解医学MLLM的推理时幻觉，在多个医学基准上提升闭式准确率约6%，减少开式幻觉约35%。


<details>
  <summary>Details</summary>
Motivation: 医学MLLM在临床VQA和报告生成中表现出潜力，但推理时幻觉问题严重削弱了可信度——模型可能生成与影像证据矛盾的流畅结论。现有缓解方法依赖额外训练、外部检索或多阶段事后验证，成本高、泛化差。

Method: 提出训练无关的双侧证据注入框架：(1) 利用MedSAM获取ROI先验，通过ROI引导的激活调制重新校准视觉感知轨迹；(2) 将解剖坐标映射为离散语义token作为可验证外部记忆，锚定文本推理轨迹；(3) 引入任务感知动态路由器，根据任务语义选择模态特定干预，平衡感知基础与语言流畅性。

Result: 在LLaVA-1.5-7B、LLaVA-Med-1.5-7B、Qwen3-VL-8B/32B、InternVL-3.5-8B/38B等模型上，对2个任务5个数据集进行系统评估，闭式准确率提升约6%，开式幻觉减少约35%，消融实验和可视化进一步验证了框架有效性。

Conclusion: 本文提出的训练无关证据注入框架能有效缓解医学MLLM的幻觉问题，且具有良好的跨模型泛化能力，为医学多模态系统的可信部署提供了实用方案。

Classification reason: 该论文聚焦于医学MLLM的幻觉缓解，提出训练无关的证据注入框架来减少推理时幻觉，属于多模态幻觉与可靠性这一[CANONICAL]子主题。

Importance reason: Focuses on medical MLLM hallucination mitigation, which is outside your priority subtopics and negative-keyword domain. Signals: medical MLLM, training-free evidence injection, ROI-guided modulation, anatomical anchoring, hallucination reduction
...
### [70] [Information-Regularized Attention for Visual-Centric Reasoning](https://arxiv.org/abs/2607.00434)
...
TL;DR: 本文提出信息正则化注意力（IRA），一种随机注意力机制，通过显式调控中间Transformer层中视觉信息的注入量来缓解VLM的物体幻觉、视觉接地弱和灾难性遗忘问题。IRA将视觉表征的不确定性转化为局部噪声，产生更平滑的曲率轨迹并抑制注意力下沉，提升了VLM的可靠性。


<details>
  <summary>Details</summary>
Motivation: VLM在标准next-token预测目标下，视觉嵌入被被动优化，容易注入冗余或虚假信号，导致物体幻觉、视觉接地弱和全参数指令微调后的灾难性遗忘。现有方法缺乏对视觉表征学习的显式控制。

Method: 提出信息正则化注意力（IRA），一种随机注意力机制，通过局部重参数化将视觉表征的不确定性转化为独立于数据点的局部噪声，显式调控注入中间Transformer层隐藏状态的视觉信息量。

Result: IRA产生更平滑的曲率轨迹，抑制了所有层的注意力下沉现象，在缓解幻觉和提升视觉接地方面优于基线模型。

Conclusion: 随机注意力不仅是正则化手段，更是生成式架构中表征学习的关键贡献者，为构建更可靠的VLM提供了新方向。

Classification reason: 该论文针对VLM中的物体幻觉、视觉接地弱、灾难性遗忘等问题，提出信息正则化注意力机制来显式控制视觉信息注入，提升VLM的可靠性和稳定性。这与[多模态幻觉与可靠性]（multimodal_hallucination）这一CANONICAL子主题高度匹配，该子主题关注视觉幻觉缓解、事实性、可靠性、忠实性提升。

Importance reason: Directly addresses VLM reliability via a novel stochastic attention mechanism, aligning with user's focus on multimodal understanding and continuous representations. Signals: VLM hallucination mitigation, stochastic attention regularization, visual grounding improvement, attention sink suppression, representation learning in generative architectures
...
### [71] [VLM-AR3L: Vision-Language Models for Absolute and Relative Rewards in Reinforcement Learning](https://arxiv.org/abs/2607.00483)
...
TL;DR: VLM-AR3L 提出利用视觉语言模型（VLM）为强化学习生成绝对奖励（单状态评分）和相对奖励（相邻状态比较），在 Minecraft 等复杂环境中优于先前基于 VLM 的奖励学习方法。


<details>
  <summary>Details</summary>
Motivation: 在开放环境强化学习中，设计有效的奖励函数是一大挑战，尤其是当任务目标抽象且难以量化时。现有方法依赖人工设计奖励或 VLM 直接评分，但存在不稳定或缺乏比较监督的问题。

Method: VLM-AR3L 框架利用 VLM 根据自然语言任务目标对智能体的视觉观察进行理解，生成偏好标签来训练两个奖励模型：绝对奖励模型（对单个状态预测标量评分）和相对奖励模型（比较连续观察推断任务进展）。两者结合实现状态评估的稳定性与比较监督的鲁棒性。

Result: 在经典控制、操作任务和开放世界具身任务（特别是 Minecraft）上的实验表明，VLM-AR3L 在各项基准上一致优于先前的 VLM 奖励学习方法。

Conclusion: VLM-AR3L 通过结合绝对与相对奖励，有效提升了 VLM 在强化学习奖励设计中的表现，为开放环境下的奖励学习提供了新范式。

Classification reason: 该论文使用 VLM 为强化学习提供绝对和相对奖励，属于多模态强化学习与对齐方向，与 canonical subtopic "多模态强化学习与对齐"（multimodal_rl_alignment）高度匹配。

Importance reason: Focuses on RL reward design with VLMs, not aligned with user's priority on continuous multimodal models or video/image generation. Signals: VLM for RL rewards, absolute and relative reward, Minecraft benchmark, not generative or continuous token, no flow matching or autoregressive generation
...
### [72] [Linguistic Relative Policy Optimization for Video Anomaly Reasoning](https://arxiv.org/abs/2607.00654)
...
TL;DR: 提出 Linguistic Relative Policy Optimization (LRPO)，一种无需参数更新的视频异常推理方法，通过从多条推理轨迹中蒸馏语义优势作为语言化异常经验先验，并注入上下文引导模型输出分布，在 XD-Violence、UCF-Crime 和 UBnormal 上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大模型的视频异常检测方法仍依赖大规模标注或专家设计的先验知识，限制了模型以尽可能少的人工干预获取异常知识的能力。本文旨在设计一种无需参数更新、仅通过上下文注入经验先验的方法来提升视频异常推理能力。

Method: 提出 LRPO 框架，包含两个互补的经验表示：通用经验（跨场景可迁移的异常偏好）和场景经验（上下文相关的异常规则）。通过从多条推理轨迹中蒸馏组相对语义优势（group-relative semantic advantages）作为语言化异常经验先验，并引入异常对齐奖励（anomaly alignment reward）来引导轨迹优化，使其匹配人类风险偏好并强化时序推理。最终将经验先验注入上下文来引导模型输出分布，无需任何参数更新。

Result: 在 XD-Violence、UCF-Crime 和 UBnormal 三个基准上，LRPO 在无微调（tuning-free）设置下显著优于现有 SOTA 方法。

Conclusion: LRPO 通过语言化经验先验和上下文注入的方式，在不更新模型参数的情况下有效提升了视频异常推理性能，验证了语义优势蒸馏与偏好对齐在视频异常检测中的有效性。

Classification reason: 该论文提出 LRPO（Linguistic Relative Policy Optimization），利用多模态大模型进行视频异常推理，通过从多条推理轨迹中蒸馏语义优势作为语言化异常经验先验，并注入上下文来引导模型输出分布，无需参数更新。这属于多模态强化学习与对齐（偏好优化、奖励建模）方向，因此映射到 canonical subtopic "多模态强化学习与对齐"。

Importance reason: Focuses on video anomaly reasoning with RL-based tuning, which is outside your priority subtopics and lacks direct connection to continuous models, generation, or world models. Signals: Video anomaly detection, RL-based optimization, Tuning-free adaptation, Multimodal LLM, No parameter update
...
### [73] [Know When to Stop: Segment-Level Credit Assignment for Reducing Overthinking](https://arxiv.org/abs/2607.00482)
...
TL;DR: 本文提出 DASH（Drift Aware advantage SHaping）方法，通过将推理轨迹中的中间答案承诺与真实标签对比，自动识别每个推理段落是否有助于正确性，从而为段落分配信用分数，减少推理语言模型中的过度思考行为。在 AIME25 等竞赛级数学基准上，DASH 在减少无效自我反思的同时取得了更高准确率。


<details>
  <summary>Details</summary>
Motivation: 推理语言模型经常出现过度思考（overthinking）现象，产生大量无用的推理 token（如犹豫、放弃、自我矛盾），消耗计算资源却不改善答案质量。现有方法难以在细粒度上判断哪些推理步骤是有益的，而人工标注步骤级标签成本过高。因此需要一种自动化的方法，在不依赖额外监督的情况下识别并抑制无效推理。

Method: DASH 利用推理轨迹中的中间答案承诺（intermediate answer commitments）作为廉价代理信号：将每个最终答案候选与真实标签对比，判断后续推理段落是否朝向正确性。基于此，DASH 为每个推理段落分配段级别的优势信用分数（segment-level advantage），通过 GRPO 风格的强化学习优化模型，奖励朝向正确性的推理段落，惩罚偏离正确性的段落。

Result: 在 AIME25 上 DASH 达到 50.8% 准确率，优于 GRPO 的 45.4%；在 AIME24 和 AMC23 上也取得一致提升。DASH 有效减少了过度思考行为（如犹豫、自我矛盾），实现了更高效的自我纠正。

Conclusion: DASH 通过段级别的信用分配，在不依赖人工标注的情况下有效缓解了推理语言模型的过度思考问题，提升了推理效率和准确性。该方法为推理时计算扩展提供了更精细的优化方向。

Classification reason: 该论文研究推理语言模型中的过度思考（overthinking）问题，提出基于段落的信用分配方法 DASH 来减少无效推理。虽然论文聚焦于纯文本数学推理（AIME 基准），但其核心问题——推理过程中的 token 效率、自我反思的收益评估、推理时计算扩展——与多模态推理与思维链（multimodal_reasoning_cot）方向高度相关，该方向涵盖数学/代码/科学推理及推理时计算扩展。论文的方法论（段级别信用分配）可推广到多模态推理场景。

Importance reason: Addresses overthinking in reasoning models, a key efficiency issue, but is not directly aligned with the user's priority subtopics on continuous models or generation. Signals: overthinking reduction, segment-level credit assignment, reasoning efficiency, math reasoning, self-reflection analysis
...
### [74] [PixelEyes: Decoupling Perception and Reasoning for Pinpoint Visual Evidence Seeking](https://arxiv.org/abs/2607.00115)
...
TL;DR: PixelEyes 提出将多轮视觉推理中的推理与感知解耦：推理器决定"找什么"，专用感知工具（指分割模型）回答"在哪里"。引入掩码引导的视觉搜索和语义区域广度优先搜索（BFS），消除冗余定位循环。构建 PixelEyes-6K 数据集注入该逻辑，并提出 Pinpoint-Bench 零提示视觉搜索基准用于细粒度失败分析。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 在多轮视觉推理中反复定位失败，导致冗长冗余的轨迹。根本原因是推理与感知在同一模型中纠缠——模型同时进行推理和定位，不准确的定位会触发额外的推理轮次，使轨迹膨胀。

Method: 1) 掩码引导视觉搜索：调用指代分割模型提供掩码级精确定位，避免推理器补偿不精确的定位。2) 语义区域广度优先搜索（BFS）：将探索组织为语义区域上的 BFS，消除反复裁剪错误子区域导致的冗余循环。3) 构建 PixelEyes-6K 数据集，从现有数据重新合成专家轨迹，显式嵌入掩码引导搜索和 BFS 逻辑。4) 提出 Pinpoint-Bench 零提示视觉搜索基准，提供实例级掩码和边界框，分离定位失败与推理失败。

Result: 在 Pinpoint-Bench 上，最新的 SOTA MLLM 和视觉推理智能体表现仍有较大提升空间，验证了该基准的难度和质量。PixelEyes 通过解耦策略显著提升了多轮视觉定位的准确性和效率。

Conclusion: 将推理与感知解耦是解决多轮视觉推理中定位失败和轨迹冗余问题的有效方案。掩码引导搜索和语义区域 BFS 可消除冗余循环，Pinpoint-Bench 为细粒度分析视觉搜索失败模式提供了有价值的工具。

Classification reason: 该论文提出多轮视觉推理智能体 PixelEyes，将推理与感知解耦以改进视觉定位，属于多模态推理（视觉推理）范畴，与 [CANONICAL] 多模态推理与思维链 高度匹配。

Importance reason: Decouples perception from reasoning in MLLMs, but focuses on visual search rather than continuous multimodal models or generation. Signals: decoupled reasoning and perception, mask-guided visual search, semantic-region BFS, Pinpoint-Bench benchmark, failure analysis for MLLMs
...
### [75] [Decompose, Compare, and Decide: Multimodal LLMs are Implicit Few-Shot Learners](https://arxiv.org/abs/2607.00125)
...
TL;DR: 本文提出 DeCoDe，一种无需额外训练即可将现成 MLLM 转化为强大少样本分类器的方法。通过将少样本分类分解为成对图像比较的二元决策，利用 MLLM 对肯定回答的 logit 作为相似度分数，在 12 个数据集上显著超越现有少样本方法。


<details>
  <summary>Details</summary>
Motivation: MLLM 在图像分析方面表现出色，但将其能力迁移到少样本图像分类仍具挑战。现有少样本方法通常需要额外训练或微调，而 MLLM 的零样本能力尚未被有效利用于少样本分类任务。

Method: DeCoDe 将少样本分类分解为一组成对比较：对于查询图像和每个候选类别的支持图像，MLLM 被提示判断两者是否属于同一类别，使用肯定回答的 logit 作为相似度分数，将查询图像分配给得分最高的类别。此外，提供数据域等高层信息可进一步提升性能。

Result: 在 12 个数据集（6 个已有 + 6 个新构建）上的实验表明，DeCoDe 显著优于当前最先进的少样本方法，在标准和新型领域均表现优异。

Conclusion: 简单的分解技术可以将现成 MLLM 转化为强大的少样本学习器，无需任何训练即可在少样本分类任务上取得 SOTA 结果，展示了 MLLM 作为隐式少样本学习者的潜力。

Classification reason: 该论文利用 MLLM 进行少样本图像分类，通过将分类任务分解为成对比较并利用 logit 作为相似度分数，本质上是一种多模态推理方法。这与 [CANONICAL] 多模态推理与思维链 (multimodal_reasoning_cot) 高度相关，因为论文的核心是让 MLLM 进行推理决策（比较两张图是否属于同一类别），属于视觉推理范畴。

Importance reason: Novel MLLM few-shot method but outside user's priority subtopics and lacks continuous/autoregressive focus. Signals: MLLM few-shot classification, pairwise image comparison, logit-based similarity, no training required, 12 dataset evaluation
...
### [76] [Perceive-to-Reason: Decoupling Perception and Reasoning for Fine-Grained Visual Reasoning](https://arxiv.org/abs/2607.01191)
...
TL;DR: 本文提出 Perceive-to-Reason (P2R) 框架，将细粒度视觉推理显式解耦为感知（定位证据区域）和推理（基于裁剪区域回答问题）两个阶段，并引入 PRA-GRPO 角色感知强化学习策略交替优化感知与推理能力。在 Qwen3-VL 上实验表明，该方法在 V-Star、HR-Bench 等细粒度推理基准上显著超越基线，且收益可泛化到更广泛的多模态推理任务。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 在处理高分辨率图像中的细微视觉线索时表现不佳，虽然已有重复裁剪或测试时视觉搜索等方法引入局部证据，但这些方法没有显式区分感知与推理两个认知阶段，导致两者相互干扰，限制了细粒度推理能力。

Method: 提出 P2R 框架：(1) 两阶段解耦——Perceiver 阶段定位问题相关的视觉证据区域，Reasoner 阶段基于标注图像和裁剪区域回答问题；(2) PRA-GRPO 强化学习策略——交替进行感知聚焦和推理聚焦的 GRPO 更新，仅使用最终答案作为监督信号，实现角色感知的优化。

Result: 基于 Qwen3-VL-Instruct-2B/4B/8B，P2R 持续提升性能。P2R-4B 在 V-Star 上达 93.2%，HR-Bench-4K 上达 81.9%，HR-Bench-8K 上达 80.5%，显著优于对应基线。此外，P2R 的收益可泛化到更广泛的多模态推理任务。

Conclusion: 显式解耦感知与推理为细粒度视觉推理提供了一种有效框架，PRA-GRPO 策略能够在不依赖额外标注的情况下分别优化两个阶段的能力，该方法在多个基准上取得显著提升并具有良好的泛化性。

Classification reason: 该论文聚焦细粒度视觉推理，提出将感知与推理解耦的两阶段框架，属于多模态推理范畴。现有 [CANONICAL] 子主题 "多模态推理与思维链" (multimodal_reasoning_cot) 涵盖视觉推理、多模态 CoT 等，与该论文主题高度匹配。

Importance reason: Decouples perception and reasoning for fine-grained visual reasoning, aligning with multimodal understanding and reasoning priorities. Signals: fine-grained visual reasoning, perception-reasoning decoupling, PRA-GRPO reinforcement learning, Qwen3-VL backbone, high-resolution benchmarks
...
### [77] [HyFL-CLIP: Hyperbolic Fine-Tuning of CLIP for Robust Long-Context Understanding](https://arxiv.org/abs/2607.00428)
...
TL;DR: HyFL-CLIP 提出双曲空间微调框架，通过跨流形相似度蒸馏将 CLIP 的欧几里得图文对齐迁移到双曲空间，利用双曲几何的层次蕴含关系建模长文本与短文本、图像之间的部分-整体关系，显著提升长上下文跨模态检索的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: CLIP 在长文本描述（>77 tokens）下表现不佳，因为其绝对位置编码和短标题预训练导致无法处理长上下文中的句子重排、摘要或部分省略。现有方法通过扩展位置编码来延长长度，但在文本扰动下图文对齐质量严重下降。作者认为根本原因在于欧几里得对比学习目标强制一对一的严格匹配，缺乏建模全局上下文与其组成元素之间层次关系的机制。

Method: 提出 HyFL-CLIP，一种双曲空间微调框架：(1) 通过跨流形相似度蒸馏（cross-manifold similarity distillation）将欧几里得 CLIP 学到的图文对齐迁移到双曲空间；(2) 利用双曲几何的蕴含关系建模层次语义，通过 Einstein 中点聚合（Einstein midpoint aggregation）连接摘要级 token 特征、长文本描述、短文本组件和图像，捕获部分-整体关系；(3) 在双曲空间中训练，保持 CLIP 骨干网络不变，仅微调少量参数。

Result: 在长上下文跨模态检索、带文本扰动的跨模态检索、模态内检索和短文本跨模态检索等多个基准上，HyFL-CLIP 均取得更鲁棒的长上下文理解。在文本扰动下的长文本跨模态检索中，相比最佳先前方法提升高达 19.5%。此外，该方法可无缝集成到其他模型框架中（如应用于 SDXL）。

Conclusion: HyFL-CLIP 通过双曲空间微调有效解决了 CLIP 在长上下文理解中的局限性，验证了双曲几何在建模图文层次关系中的优势，且具有良好的可迁移性。

Classification reason: 该论文主要研究长文本条件下的跨模态检索（图像-文本对齐），属于多模态检索范畴，因此映射到 canonical 子主题 "多模态检索" (multimodal_retrieval)。

Importance reason: Focuses on long-context CLIP fine-tuning, not directly on the user's priority subtopics like continuous models or video generation. Signals: Hyperbolic fine-tuning, Long-context retrieval, CLIP adaptation, Cross-modal retrieval, Not in priority subtopics
...
### [78] [VideoSearch-R1: Iterative Video Retrieval and Reasoning via Soft Query Refinement](https://arxiv.org/abs/2607.00446)
...
TL;DR: VideoSearch-R1 是一个基于智能体框架的迭代视频检索与时序定位系统，通过 Soft Query Refinement (SQR) 在连续潜空间中优化搜索查询，并利用 GRPO 进行训练，在 VCMR 任务上取得了 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频检索方法将检索视为预处理步骤，初始检索失败后无法修正，导致后续细粒度时序推理失败；而现有的智能体框架通常假设查询相关视频已给定，只关注视频内推理。因此需要一种能够迭代检索、在检索失败时自动优化查询的框架。

Method: 提出 VideoSearch-R1 智能体框架，包含 Soft Query Refinement (SQR) 模块，在连续潜空间中优化搜索查询 token（而非在离散文本空间中重写查询），并使用 Group Relative Policy Optimization (GRPO) 基于检索和下游任务的奖励信号进行训练，实现多轮交互式视频检索与时序定位。

Result: 在 Video Corpus Moment Retrieval (VCMR) 的三个数据集上取得了 SOTA 性能。分析表明 SQR 能有效优化原始查询，且生成的 token 数量显著少于显式文本级查询优化方法。

Conclusion: VideoSearch-R1 通过连续潜空间查询优化与强化学习训练，有效解决了视频检索中初始检索失败无法修正的问题，为大规模视频语料库的检索与细粒度推理提供了新的范式。

Classification reason: 该论文研究视频语料库检索与时序定位，核心是视频检索（跨模态检索）与查询优化，属于多模态检索范畴。论文提出的 Soft Query Refinement 在连续潜空间中优化检索查询，但整体任务仍是视频检索与定位，因此映射到 [CANONICAL] subtopic "多模态检索" (multimodal_retrieval)。

Importance reason: Strong retrieval and reasoning framework but not aligned with user's priority subtopics on continuous models or generation. Signals: iterative video retrieval, soft query refinement, GRPO training, VCMR SOTA, agentic framework
...
TL;DR: ReVisIT 是一个免训练的多模态上下文学习框架，通过检索图像-标签对作为"视觉思维"单元，结合结构化类定义、每查询多模态检索和交替注入示例，有效弥合了开放集与封闭集之间的性能差距。在多个基准上显著提升了不同规模 VLM 的性能。


<details>
  <summary>Details</summary>
Motivation: 现有"用图像思考"的方法要么需要生成新图像（依赖外部工具或训练），要么需要合成代码，成本高昂。作者希望探索第四条路径：通过检索已有的标注示例图像来进行推理，无需训练、无需生成、无需外部工具，弥合开放集与封闭集之间的性能差距。

Method: ReVisIT 框架包含三个核心组件：(1) 结构化类定义（Structured Class Definitions），为每个类别提供语义描述；(2) 每查询多模态检索（Per-Query Multimodal Retrieval），根据查询检索最相关的图像-标签对作为示例；(3) 交替用户/助手注入（Alternating User/Assistant Injection），将检索到的示例以交替方式注入到上下文中，然后进行联合多属性解码。框架可以优雅地降级到任务允许的任何组件组合。

Result: 在 VL-ICL Bench Fast Open MiniImageNet 上，Qwen3-VL-30B-A3B 使用 ReVisIT 在 4-shot 设置下达到 98.5%，与 72B LLaVA-OneVision SOTA (98.7%) 统计上无显著差异，参数量仅为其约 1/2.4。仅 turns 层就为 GPT-4.1 在自由形式概念归纳（Bongard-OpenWorld）上提升 26.1 个点。完整框架在三个骨干网络上对 MAAC-Bench 实现 4-6 个点的宏平均提升。组件分析表明检索+交替注入是通用杠杆，83% 的检索增益来自检索质量而非示例的存在。

Conclusion: ReVisIT 证明了通过检索而非生成来实现动态视觉推理的可行性，是一种免训练、高效的多模态上下文学习范式。框架的组件可灵活组合，且检索质量是性能提升的关键因素。同时发布了 MAAC-Bench 基准及基于标注的 LLM 验证协议。

Classification reason: 该论文提出 ReVisIT，一种基于检索的免训练多模态上下文学习框架，核心是通过检索图像-标签对作为视觉思维单元来辅助推理。这属于多模态检索与推理的交叉，与 canonical 子主题「多模态检索」(multimodal_retrieval) 高度匹配，因为论文的核心创新在于利用多模态检索获取 exemplar 图像来增强 VLM 的上下文学习能力。

Importance reason: Training-free multimodal in-context learning is interesting but not aligned with your priority on continuous models or generation. Signals: training-free, multimodal in-context learning, open-vs-closed gap, retrieval-based reasoning, visual thought
...
### [80] [MoVA: Learning Asymmetric Dual Projections for Modular Long Video-Text Alignment](https://arxiv.org/abs/2607.00858)
...
TL;DR: MoVA 提出了一种非对称双投影框架，通过理论分析建立视频-文本灵活对齐的条件，并设计文本侧自适应选择帧感知子空间、视频侧解耦文本相关视觉概念，实现长视频与长文本的高效对齐，在多个视频-文本对齐任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频-文本对比预训练模型（如 CLIP 的扩展）存在两个关键问题：时间错位（文本描述仅与特定时间窗口相关）和语义不对称（帧级视觉细节与标题级概念之间存在稀疏、双向、非等价的关联）。短标题导致歧义，长标题则导致静态对象与时序演化的纠缠。现有方法无法有效处理这些挑战。

Method: MoVA 首先从理论上建立了视频与文本表示在时间维度和不同粒度下灵活对齐的条件。基于此，提出模块化长视频-文本对齐框架，学习两组非对称投影：文本侧投影自适应选择与帧相关的标题子空间；视频侧投影解耦与文本相关的视觉概念。该框架在保持全局跨模态语义的同时，解耦演化的帧特定概念，并能自然扩展到长标题和长视频。

Result: 在多个视频-文本对齐任务上的实验表明，MoVA 在性能上优于现有方法，验证了非对称双投影框架在处理长视频-文本对齐中的有效性。

Conclusion: MoVA 通过理论分析和非对称双投影设计，有效解决了视频-文本对齐中的时间错位和语义不对称问题，为长视频与长文本的灵活对齐提供了新范式。

Classification reason: 该论文研究长视频与文本的对齐（video-text alignment），属于跨模态检索/对齐范畴，与现有 canonical subtopic "多模态检索"（multimodal_retrieval）高度匹配。

Importance reason: Strong theoretical contribution to video-text alignment but falls outside user's priority subtopics and negative keywords. Signals: video-text alignment, asymmetric dual projections, long video understanding, contrastive pre-training, temporal misalignment
...
### [81] [Attribute-Prompted Kernel Hashing for Unsupervised Data-Efficient Cross-Modal Retrieval](https://arxiv.org/abs/2607.00379)
...
TL;DR: 本文提出 Attribute-Prompted Kernel Hashing (APKH)，一种数据高效的跨模态哈希检索方法。通过引入上下文优化属性核映射（CAKM）和核平滑对比对齐（KSCA），利用视觉语言基础模型的广义属性先验，在数据受限场景下构建紧凑的模态对齐汉明空间，有效缓解过拟合问题，在从已知到未知类别的跨模态检索任务中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有无监督跨模态哈希方法严重依赖大规模图像-文本对，但在隐私和专业化约束下，对齐数据稀缺且收集成本高。此外，现有方法在有限训练数据上容易过拟合，对未见类别的泛化能力差。因此需要一种数据高效的跨模态哈希方法。

Method: APKH 包含两个核心模块：(1) 上下文优化属性核映射（CAKM），通过超球面径向基函数（RBF）核映射实现跨模态对齐，利用提示学习优化动态属性核以捕获模态不变语义；(2) 核平滑对比对齐（KSCA），将有限的配对数据建模为连续核分布，替代传统点对点对比学习，显式平滑模态间隙以缓解对稀疏成对相关性的过拟合。

Result: 在多个跨模态检索基准上，APKH 在数据受限场景下从已知类别到未知类别的检索任务中，显著优于现有最先进的无监督哈希方法。

Conclusion: APKH 通过利用视觉语言基础模型的属性先验和核平滑策略，有效解决了数据稀缺场景下跨模态哈希的过拟合和泛化问题，为数据高效的跨模态检索提供了新思路。

Classification reason: 该论文研究跨模态检索（cross-modal retrieval），属于多模态检索范畴。现有 [CANONICAL] subtopic "多模态检索"（multimodal_retrieval）完全匹配该论文主题。

Importance reason: Focuses on cross-modal retrieval hashing, which is not a priority subtopic and lacks direct connection to user's core interests in generation, world models, or continuous representations. Signals: Unsupervised cross-modal hashing, data-efficient retrieval, attribute kernel mapping, seen-to-unseen generalization, not aligned with user's priority subtopics
...
### [82] [Identifying and Resolving Pitfalls of Knowledge-Based VQA Benchmarks: Auditing, Repairing, and Augmenting](https://arxiv.org/abs/2607.00159)
*Qian Ma, S M Rayeed, Charles V. Stewart, Qiong Wu, Yao Ma*

Main category: cs.MM, cs.IR, cs.CV, cs.CL
...
TL;DR: 本文系统审计了现有KB-VQA基准中的三大系统性缺陷：答案不可从知识库推导、问题约束不足、视觉场景过于简单（单实体），导致准确率成为误导性指标。作者提出审计-修复协议和多实体增强协议，重新评估后模型排名和推理能力评估结果发生显著变化。


<details>
  <summary>Details</summary>
Motivation: 现有KB-VQA基准广泛使用答案准确率作为评估指标，隐含假设答案可从知识库推导、问题充分约束、视觉场景需要知识消歧。但这些假设在实际基准中系统性被违反，导致准确率成为误导性指标，高估了VLM的知识推理能力。

Method: (1) 审计协议：系统检查KB-VQA基准中答案是否可从关联知识库推导、问题是否充分约束、视觉场景是否需要知识消歧；(2) 修复协议：对违反假设的样本进行修复（补充缺失答案、修正矛盾答案、澄清模糊问题）；(3) 多实体增强协议：引入视觉歧义性（多实体场景）以挑战初始检索和知识推理。

Result: 审计发现大量样本存在答案缺失/矛盾、问题模糊、视觉场景过于简单（单实体）等问题。在修复和增强后的设置下重新评估，模型排名和性能趋势发生显著变化，表明原始基准高估了VLM的推理能力。

Conclusion: 现有KB-VQA基准存在系统性缺陷，使准确率成为不可靠的评估指标。需要重新思考评估协议，设计更注重可验证推理而非简单匹配的交互感知型KB-VQA基准。

Classification reason: 该论文系统性地审计了KB-VQA基准中的缺陷（答案缺失/矛盾、问题模糊、视觉场景过于简单），提出了审计-修复协议和多实体增强协议，属于多模态评测与基准方向。映射到[CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark）。

Importance reason: Focuses on benchmark auditing for KB-VQA, not on the user's priority subtopics like continuous models or video generation. Signals: KB-VQA benchmark flaws, audit-and-repair protocol, multi-entity augmentation, evaluation metric critique, not aligned with user's priority subtopics
...
### [83] [EmbodimentSemantic: A Spatial Scene-Graph Dataset and Benchmark for Vision-Language Models on Embodied Manipulation Trajectories](https://arxiv.org/abs/2607.00020)
...
TL;DR: 本文提出 EmbodimentSemantic，一个空间场景图数据集和基准，用于评估视觉语言模型（VLM）在具身操作轨迹中的空间关系接地能力。数据集包含真实机器人操作数据和基于 LIBERO 模拟器的 6 万帧标注数据，支持物体绑定、关系预测和空间一致性评估。实验表明当前 VLM 在精确深度感知和视角依赖的空间结构理解上仍存在困难。


<details>
  <summary>Details</summary>
Motivation: 当前 VLA（视觉-语言-动作）系统在空间接地方面存在关键局限——模型能识别物体和遵循语言指令，但缺乏对物体空间排列（如支撑、包含、遮挡、深度关系）的显式表征。现有基准无法系统评估 VLM 在具身操作场景中的空间关系理解能力。

Method: 构建 EmbodimentSemantic 数据集，将场景表示为有向物体-关系-物体三元组，使用固定关系集描述空间关系。包含真实 SO101 机器人臂操作数据和基于 LIBERO 模拟器的 6 万帧数据（含 12 万+场景图），通过 MuJoCo 几何、世界坐标、相机投影和可见性约束自动推导真实关系。评估多个开源和商业 VLM 在物体绑定、关系预测和空间一致性上的表现，并测试将场景图注入 VLA 策略提示的下游控制效果。

Result: 当前 VLM 能预测合理的关系，但在精确的深度感知和视角依赖的空间结构上表现不佳。场景图注入 VLA 策略提示可提升下游控制性能。

Conclusion: EmbodimentSemantic 为诊断 VLM 在具身操作中的空间接地能力提供了统一框架，揭示了现有模型在深度感知和视角依赖空间理解方面的不足，并为 VLA 系统改进提供了评测基础。

Classification reason: 该论文提出了 EmbodimentSemantic，一个用于评估 VLM 在具身操作轨迹中空间关系接地能力的场景图数据集和基准。这属于多模态评测与基准（multimodal_benchmark）范畴，因为核心贡献是构建新基准和数据集来评测 VLM 的空间接地能力。

Importance reason: Focuses on spatial grounding benchmark for embodied manipulation, which is not a priority subtopic and lacks direct connection to continuous language/multimodal models or video generation. Signals: spatial scene-graph dataset, VLM spatial grounding benchmark, embodied manipulation, LIBERO simulator, low direct alignment with user's priority subtopics
...
### [84] [MSQA: A Natively Sourced Multilingual and Multicultural SimpleQA Benchmark](https://arxiv.org/abs/2607.00724)
...
TL;DR: MSQA是一个原生采集的多语言多文化简单问答基准，包含11个语系、5个文化维度和3个难度等级的1064个问题。评估18个LLM后发现显著的文化退化现象和"地域效应"——文化能力更依赖预训练数据暴露而非推理能力。常见的推理时干预（校准、重复采样、检索增强）无法有效解决文化对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准多通过翻译构建多语言评测集，导致模型可以利用英语中心的知识迁移来"假装"理解其他文化。作者提出"文化对齐幻觉"概念，即模型能说用户的语言并不意味着理解该语言背后的文化。需要原生采集而非翻译的多文化评测基准来真实评估模型的文化理解能力。

Method: 从11个语系（含中文、日语、阿拉伯语、印地语等）原生采集1064个问题，覆盖5个文化维度（历史、地理、社会、艺术、日常生活）和3个难度等级。问题设计为简单问答（SimpleQA风格），答案可客观验证。评估18个LLM（含GPT-4o、Claude、Gemini、DeepSeek等），分析文化退化程度、地域效应及推理时干预效果。

Result: 发现显著的文化退化现象：模型在英语文化问题上表现最好，非英语文化问题表现大幅下降。存在"地域效应"：文化能力与预训练数据暴露程度正相关，而非与通用推理能力相关。推理时干预（校准、重复采样、RAG）效果有限：模型对不熟悉文化问题过度自信，重复采样不稳定，RAG仅对长尾事实有部分帮助。

Conclusion: 文化对齐不能从多语言能力推断得出，需要比推理时校准、采样或检索更深层次的干预。多语言流畅性不等于文化理解，模型需要更根本的训练数据层面或训练方法层面的改进来实现真正的文化对齐。

Classification reason: 该论文提出了MSQA多语言多文化问答评测基准，属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）子主题。虽然论文主要关注LLM而非多模态VLM，但评测基准主题与multimodal_benchmark高度吻合，且该子主题涵盖评测协议与基准构建。

Importance reason: Strong cultural evaluation benchmark but outside user's core technical priorities. Signals: Multilingual cultural benchmark, Illusion of Cultural Alignment, Locality Effect, Inference-time remedies ineffective, Not in priority subtopics
...
### [85] [Beyond Document Grounding: Span-Level Hallucination Detection over Code, Tool Output, and Documents](https://arxiv.org/abs/2607.00895)
...
TL;DR: 本文提出了一个统一的span级幻觉检测基准，覆盖代码、工具输出、结构化文档和自然语言RAG数据。通过从正确回答中注入局部幻觉并精确标注字符位置构建数据集。微调的Qwen3.5-2B检测器在统一测试集上达到0.689 span-F1，在代码agent源上大幅超越现有方法（0.60 vs LettuceDetect-large的0.17），同时在自然语言基准上保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG幻觉检测评测主要针对自然语言文档证据，但实际应用中的grounded生成系统越来越多地依赖结构化输入（源代码、工具输出、Markdown文档、表格等），缺乏统一的评测基准来评估这些场景下的幻觉检测能力。

Method: 构建统一基准：从grounded正确回答出发，注入局部幻觉并精确标注字符位置，覆盖代码、工具输出、结构化文档和自然语言RAG数据集。使用Qwen3.5-2B模型进行微调作为检测器，在span级别进行幻觉检测。

Result: 微调后的Qwen3.5-2B在统一测试集上达到0.689 span-F1，在代码agent源上达到0.60，大幅超越LettuceDetect-large（0.17）和最强零样本LLM评判（≤0.22）。在自然语言基准上，RAGTruth example-F1为81.8，English PsiloQA IoU为0.724。

Conclusion: 本文证明了在代码、工具输出等结构化输入上进行span级幻觉检测的可行性和重要性，所提出的统一基准和检测器为RAG系统的可靠性评估提供了更全面的工具。

Classification reason: 该论文提出了一个统一的幻觉检测基准，覆盖代码、工具输出、结构化文档和自然语言RAG数据，属于多模态/多源评测与基准构建，映射到[多模态评测与基准]（multimodal_benchmark）这一规范主题。

Importance reason: Focuses on hallucination detection in code/tool/document RAG, which is outside your priority subtopics and lacks direct connection to continuous multimodal models or video/image generation. Signals: Span-level hallucination detection, Code and tool output grounding, Qwen3.5-2B fine-tuned detector, Outperforms LettuceDetect on code-agent, No priority subtopic alignment
...
### [86] [EgoSafetyBench: A Diagnostic Egocentric Video Benchmark for Evaluating Embodied VLMs as Runtime Safety Guards](https://arxiv.org/abs/2607.00218)
...
TL;DR: EgoSafetyBench 是一个面向具身 VLM 运行时安全守卫的自我中心视频诊断基准，包含 1200 个半秒粒度标注的机器人视角场景，通过情境追踪和视觉通道追踪两个维度评估 VLM 对危险场景的识别能力，发现现有 VLM 在上下文危险识别和误导性文本干扰下表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有安全基准仅做二元安全分类，无法区分真正危险与表面可疑但安全的日常活动。具身智能体需要运行时安全守卫，但缺乏细粒度、诊断性的评测基准来评估 VLM 在流式视频中识别危险时刻的能力。

Method: 构建 EgoSafetyBench 基准，包含 1200 个自我中心视频场景，以半秒粒度标注。情境追踪（800 场景）覆盖四类场景：常规安全、安全但可疑、明显危险、上下文危险。视觉通道追踪（400 场景）测试场景内文本（标志、贴纸、标签）对安全判断的影响，每个误导性标志配有真实版本。采用对比阶梯设计：近乎相同的场景仅在一个可见决定线索上不同，确保正确判断依赖于该线索而非整体场景类型。评估了 10 个开源和闭源 VLM。

Result: VLM 能可靠识别包含危险的视频，但经常错过具体的危险时刻，尤其是上下文危险。误导性场景内文本会降低所有测试守卫的性能：脆弱模型错过多达三分之一的危险，而鲁棒模型对安全内容过度干预。对照实验表明，表面上的安全鲁棒性往往反映的是不加区分的报警而非真正的物理推理。

Conclusion: EgoSafetyBench 揭示了现有 VLM 作为运行时安全守卫的局限性，特别是在上下文危险识别和对抗误导性文本方面。该基准为开发更可靠的具身安全守卫提供了诊断性评估工具。

Classification reason: 该论文提出了 EgoSafetyBench，一个用于评估具身 VLM 作为运行时安全守卫的自我中心视频基准，属于多模态评测与基准方向。该基准包含 1200 个机器人视角场景，细粒度标注，并设计了对比性测试来诊断 VLM 的安全判断能力。这完全匹配 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Focuses on embodied VLM safety evaluation, not directly on the user's priority subtopics like continuous models or video generation. Signals: Egocentric video benchmark, runtime safety guard, contextual hazard detection, misleading in-scene text, contrastive ladder evaluation
...
### [87] [DroneIQA-VLE: Multi-Task Drone Image Quality Assessment via Vision-Language Ensemble](https://arxiv.org/abs/2607.00416)
...
TL;DR: 本文提出 DroneIQA-VLE，一种用于低空无人机图像质量评估的多任务视觉语言集成框架，通过融合 SigLIP2 视觉编码器回归头与 LoRA 微调的 Qwen3.5-9B MLLM 两条流水线，在 ICME 2026 Drone-IQA 挑战赛中取得第二名。


<details>
  <summary>Details</summary>
Motivation: 无人机图像质量评估（IQA）在低空航拍场景中具有重要应用价值，但现有方法缺乏针对目标感知的多维度质量评估能力。ICME 2026 Drone-IQA 挑战赛提出了全局、目标和背景三个维度的质量评分任务，需要设计有效的多任务评估框架。

Method: 提出双流水线集成框架：(1) SigLIP2 视觉编码器 + 多任务回归头，分别预测全局、目标和背景质量分数；(2) LoRA 微调的 Qwen3.5-9B 多模态大语言模型进行质量分数回归。最终将两条流水线的全局质量预测结果取算术平均作为最终输出。

Result: 在 ICME 2026 Drone-IQA Grand Challenge 中取得第二名，验证了方法的有效性。

Conclusion: 视觉编码器回归头与 MLLM 回归的集成策略能够有效提升无人机图像质量评估的性能，多任务学习框架有助于捕捉不同维度的质量特征。

Classification reason: 该论文针对无人机图像质量评估（IQA）这一特定任务，提出了多任务视觉语言集成方法，属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）范畴，因为其核心是参与一项挑战赛（Drone-IQA Grand Challenge），涉及图像质量评估的评测任务。

Importance reason: Niche drone image quality assessment task with no overlap with user's priority subtopics or boost keywords. Signals: Drone image quality, vision-language ensemble, challenge solution, no continuous language model, no video generation
...
### [88] [MindEdit-Bench: Benchmarking Object-Level Counterfactual Spatial Reasoning in VLMs from In-the-Wild Photos](https://arxiv.org/abs/2607.00491)
...
TL;DR: MindEdit-Bench 是一个基于真实室内场景照片构建的 VLM 空间反事实推理基准，包含6个任务（其中2个为物体级反事实推理），覆盖120个私有场景、1003个问题。15个VLM的平均准确率仅8%-31%，而人类多数投票准确率为81%-97%，人机差距达53个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 基准主要测试观测性空间推理（描述已可见的关系），而缺乏对物体级反事实空间推理（如假设移动或旋转物体后的场景变化）的评测。作者希望填补这一空白，系统评估 VLM 在反事实空间推理上的能力。

Method: 从智能手机拍摄的三张室内场景照片出发，通过自动野外3D场景图提取管道构建基准。设计了6个空间推理任务：前4个测试感知和视角变换（基于观测结构），后2个（L4空间编辑、L5跨视角可见性编辑）测试物体级反事实推理。每个问题提供8-24个结构化选项，支持答案级别的错误诊断。共120个私有室内场景、1003个人工验证问题。

Result: 15个VLM在6个任务上的平均准确率仅8%-31%，人类多数投票准确率为81%-97%。人机差距最小39个百分点，平均53个百分点。结构化答案空间揭示了非均匀的失败模式，包括相机深度轴推理较弱以及在困难可见性编辑案例上的回退行为。

Conclusion: 当前VLM在物体级反事实空间推理上表现远逊于人类，存在显著的能力差距。该基准为未来VLM在空间推理能力上的改进提供了有价值的评测工具和诊断分析。

Classification reason: 该论文提出了 MindEdit-Bench，一个针对 VLM 空间反事实推理能力的新评测基准，属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题。

Importance reason: Strong benchmark for VLM spatial reasoning but lacks direct connection to user's priority subtopics like continuous models or generation. Signals: VLM spatial reasoning benchmark, counterfactual reasoning, large human-AI gap, private dataset, structured answer choices
...
### [89] [EgoGapBench: Benchmarking Egocentric Action Selection in Multi-Agent Scenes](https://arxiv.org/abs/2607.00547)
...
TL;DR: EgoGapBench 是一个诊断性基准，用于在存在其他智能体的场景中评估自我中心动作选择（EAS）能力。人类在该基准上表现可靠，而开源和闭源 MLLM 均表现较差，且系统性地选择其他可见智能体的动作。微调现有自我中心数据无法弥合差距，甚至可能有害。


<details>
  <summary>Details</summary>
Motivation: 现有自我中心基准主要从第一人称视角数据构建，难以单独评估自我中心视角理解能力。理解第一人称视角输入和采取自我中心视角是两种可分离的能力，尤其是在第一人称身体线索缺失或存在其他智能体时。作者希望隔离自我中心视角理解，专门评估模型在多智能体场景中从自身视角选择适当动作的能力。

Method: 作者构建了 EgoGapBench 基准，包含多智能体自我中心场景，要求模型从智能体自身视角选择正确动作。通过对比人类表现与多种 MLLM（包括开源和闭源模型）的表现，分析模型在自我中心动作选择上的缺陷。还进行了微调实验，包括在现有自我中心数据上微调以及在 EgoGapBench 训练数据上微调。

Result: 人类在 EgoGapBench 上表现可靠，而所有 MLLM 表现显著较差，且系统性地倾向于选择其他可见智能体的动作。在现有自我中心数据上微调无法缩小差距甚至有害，而在 EgoGapBench 训练数据上微调能提升准确率但仍未达到人类水平。

Conclusion: 自我中心动作选择（EAS）难以仅从第一人称视角数据中习得，MLLM 不仅应评估场景理解能力，还应评估自我中心动作选择能力。现有自我中心数据不足以培养 EAS 能力，需要专门的数据和训练策略。

Classification reason: 该论文提出了一个用于评估多智能体场景下自我中心动作选择能力的新基准 EgoGapBench，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Focuses on egocentric action selection benchmark, not directly on user's priority subtopics like continuous language models or video generation. Signals: Benchmark for egocentric action selection, MLLMs fail systematically, fine-tuning on egocentric data insufficient, human performance gap, multi-agent scenes
...
### [90] [LongVQUBench: Benchmarking Long-Term Video Quality Understanding of Vision-Language Models](https://arxiv.org/abs/2607.01086)
...
TL;DR: LongVQUBench 是一个针对大视觉语言模型（LVLM）长期视频质量理解能力的综合评测基准，包含1200+视频和1500个问题，设计了三个递进难度级别（局部事件质量理解、跨事件质量推理、全局质量理解）以及针式失真问答范式，实验表明现有LVLM在长视频和深度推理场景下性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有视频质量评测基准主要关注短视频片段和孤立失真，忽略了长时间内容中的时序连续性、累积退化及推理复杂性，缺乏对LVLM长期视频质量理解能力的系统性评估。

Method: 构建了包含1200+多样化视频（电影、纪录片、监控、第一人称、动画等）和1500个选择题/开放题的基准；设计了三个递进评估级别：局部事件质量理解（LQU）、跨事件质量推理（CQR）、全局质量理解（GQU）；并在所有级别中嵌入针式失真问答（NDQA）范式，通过稀疏插入时空伪影来探测细粒度检测与推理能力。

Result: 在14个最先进LVLM上的实验表明，随着视频长度和推理深度的增加，模型性能显著下降，揭示了它们在长程时序整合和感知归因方面的能力有限。

Conclusion: LongVQUBench 可作为系统化、层次化、可解释地评估LVLM长期视频质量理解能力的基础性工作，为未来改进模型的长视频感知与推理能力提供方向。

Classification reason: 该论文提出了 LongVQUBench，一个用于评估视觉语言模型长期视频质量理解能力的新基准，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准"。

Importance reason: Benchmark for long-term video quality understanding, but not aligned with user's priority subtopics like continuous models or generation. Signals: video quality benchmark, long-term understanding, LVLM evaluation, needle distortion QA, temporal reasoning
...
### [91] [MoHallBench: A Benchmark for Motion Hallucination in Video Large Language Models](https://arxiv.org/abs/2607.01117)
...
TL;DR: MoHallBench是一个专门用于诊断视频大语言模型（VideoLLM）中运动幻觉的基准，包含11,306个视频片段和40,493个问答对，覆盖二选一、多选和生成式设置，系统评估共现先验、序列推理和相似性混淆三类幻觉来源。实验表明当前模型在动作识别与抗幻觉能力之间存在明显解耦，序列推理幻觉最为严重。


<details>
  <summary>Details</summary>
Motivation: 现有VideoLLM幻觉评测基准主要关注物体幻觉或粗粒度动作感知，忽略了视频特有的运动幻觉问题——即模型推断出视频中不存在的运动。作者旨在系统性地定义、评测和诊断VideoLLM中的运动幻觉现象。

Method: 构建MoHallBench基准，包含11,306个视频片段和40,493个问答对，覆盖三类运动幻觉来源：共现先验（co-occurrence priors）、序列推理（sequential inference）和相似性混淆（similarity confusion）。引入双向提问协议（bi-directional questioning protocol）和偏差感知指标（bias-aware metrics）以减少二选一评估中的肯定偏差。在10个开源VideoLLM上进行评测。

Result: 10个开源VideoLLM的实验表明：动作识别与抗幻觉能力之间存在明显解耦；序列推理幻觉最为严重，模型倾向于从部分运动线索过度推断预期结果；更强的先验知识和更细粒度的相似性会显著放大幻觉。

Conclusion: MoHallBench为VideoLLM中的运动幻觉提供了系统性的评测框架，揭示了当前模型在运动幻觉方面的严重缺陷，特别是序列推理幻觉问题。该基准有望促进未来对运动幻觉的评测与缓解研究。

Classification reason: 该论文提出了MoHallBench，一个专门用于诊断VideoLLM中运动幻觉的评测基准，属于多模态评测与基准方向。该基准系统评估了共现先验、序列推理和相似性混淆三类幻觉来源，并引入了双向提问协议和偏差感知指标。这与[多模态评测与基准]（multimodal_benchmark）这一CANONICAL subtopic高度匹配。

Importance reason: Focuses on video hallucination benchmark, not directly on priority subtopics like continuous models or generation. Signals: VideoLLM hallucination benchmark, motion hallucination diagnosis, sequential inference challenge, bias-aware evaluation protocol
...
### [92] [MemSyco-Bench: Benchmarking Sycophancy in Agent Memory](https://arxiv.org/abs/2607.01071)
...
TL;DR: MemSyco-Bench是一个用于评估LLM Agent系统中记忆引发谄媚（sycophancy）问题的综合基准，涵盖五个任务来测试Agent是否能在记忆与事实冲突时正确决策。


<details>
  <summary>Details</summary>
Motivation: 现有记忆基准主要评估记忆是否正确存储、检索或更新，但忽略了检索到的记忆如何影响下游推理和决策。记忆可能引发谄媚问题，导致Agent过度迎合用户而牺牲事实准确性或客观推理。

Method: 提出MemSyco-Bench基准，包含五个任务：拒绝记忆作为事实证据、尊重记忆适用范围、解决记忆与客观证据冲突、跟踪记忆更新、以及使用有效记忆进行个性化。通过设计特定场景评估Agent在记忆影响下的决策行为。

Result: 论文构建了基准数据集和评测协议，但摘要未给出具体实验结果。

Conclusion: MemSyco-Bench填补了Agent记忆评测中关于谄媚问题的空白，为构建更可靠、更真实的长期协作Agent提供了评测工具。

Classification reason: 该论文提出了一个评测LLM Agent记忆引发谄媚（sycophancy）问题的基准，属于多模态/语言模型评测范畴，与[多模态评测与基准]（multimodal_benchmark）方向高度匹配。

Importance reason: Focuses on agent memory sycophancy, not aligned with user's priority on continuous multimodal models, video/image generation, or world models. Signals: agent memory benchmark, sycophancy evaluation, not in priority subtopics, no boost keywords matched, negative keywords not present
...
### [93] [DroneFINE: Domain-Aware Parameter-Efficient Fine-Tuning of Vision-Language Detectors for Drone Images](https://arxiv.org/abs/2607.00338)
...
TL;DR: DroneFINE 提出了一种面向无人机图像的视觉语言检测器领域感知参数高效微调方法，包含 HyperAdapter（数据依赖、前景感知的多路径适配）和 SemanticGate（文本条件背景抑制）两个模块，在 VisDrone 和 UAVDT 上超越现有 PEFT 方法，性能接近全量微调且参数量大幅减少。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 检测器在无人机航拍场景中面临严重的领域差距——VLM 预训练数据以自然场景、前景主导为主，而无人机图像具有鸟瞰视角、背景主导、小目标等特点。现有的 PEFT 方法由于静态结构约束，无法有效弥合这一领域差距。

Method: 提出 DroneFINE 框架，包含两个互补模块：(1) HyperAdapter——数据依赖、前景感知的多路径适配机制，克服 PEFT 的静态结构约束；(2) SemanticGate——文本条件背景抑制算法，利用背景词汇主动引导模型抑制无关区域响应。

Result: 在 VisDrone 和 UAVDT 数据集上，DroneFINE 显著优于现有 PEFT 方法，性能接近全量微调，同时大幅减少可训练参数量。

Conclusion: 面向特定领域（如无人机航拍）的 VLM 适配需要领域感知的 PEFT 设计，HyperAdapter 和 SemanticGate 的组合有效弥合了领域差距，为 VLM 在开放动态环境中的高效部署提供了新思路。

Classification reason: 该论文研究面向无人机图像的视觉语言检测器参数高效微调（PEFT），属于多模态模型高效化与轻量化方向，与 [CANONICAL] subtopic "多模态高效化与轻量化"（efficient_multimodal）高度匹配。

Importance reason: Focuses on domain-specific PEFT for drone detection, not directly on the user's priority subtopics like continuous models or video generation. Signals: Parameter-efficient fine-tuning, Vision-language detectors, Drone image domain adaptation, HyperAdapter, SemanticGate
...
### [94] [MVDGC: Joint 3D and 2D Multi-view Pedestrian Detection via Dual Geometric Constraints](https://arxiv.org/abs/2607.00273)
...
TL;DR: MVDGC 提出一种统一框架，通过3D圆柱查询联合优化BEV平面行人定位与图像视图2D边界框检测，利用双几何约束消除投影畸变，提升密集场景下的多视角行人检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有MVPD方法将图像特征投影到BEV空间时存在严重畸变，导致空间结构破坏和特征模糊，尤其在密集区域定位不准；同时BEV定位与2D检测之间的强几何关系未被充分利用。

Method: 提出MVDGC框架，使用稀疏3D圆柱查询（将每个行人建模为垂直圆柱体），通过相机投影在BEV平面和图像视图间建立双几何约束，直接从完整图像视图特征中提取2D特征，避免投影畸变，将BEV定位与图像检测统一为3D圆柱位置与形状优化任务。

Result: 在多个多视角行人检测基准上取得SOTA性能，有效缓解了密集场景下的定位模糊问题，验证了双几何约束的有效性。

Conclusion: MVDGC通过3D圆柱查询统一了BEV与图像视图的定位任务，利用双几何约束显著提升了多视角行人检测的精度和鲁棒性，为多视角3D感知提供了新范式。

Classification reason: 该论文研究多视角行人检测中的3D空间定位与2D检测联合优化，属于空间推理与3D视觉定位范畴，与[空间与 3D 多模态推理]（spatial_3d_reasoning）高度匹配。

Importance reason: Focuses on multi-view pedestrian detection with geometric constraints, not aligned with user's priority on continuous multimodal models or video/image generation. Signals: Multi-view pedestrian detection, 3D cylindrical queries, BEV and 2D joint optimization, dense scene occlusion reasoning, not generative or autoregressive
...
### [95] [NoPA: Non-Parametric Online 3D Scene Graph Generation](https://arxiv.org/abs/2607.00529)
...
TL;DR: NoPA 提出一种非参数化在线 3D 场景图生成方法，用每个物体的非参数分布（固定粒子集）替代单高斯近似，保留 3D 几何细节的同时保持实时推理速度，并设计了基于最大均值差异（MMD）的鲁棒合并策略与关系传播机制。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 场景图生成方法因环境建图计算量大且需生成中间点云表示，无法实时运行。近期工作虽用轻量级高斯分布近似每个物体实现实时推理，但单高斯近似丢失了大量 3D 几何细节，且近似误差导致在线推理中物体候选合并不准确。

Method: NoPA 将每个物体表示为独立的非参数分布（固定粒子集），保留 3D 几何信息。提出基于核密度估计的最大均值差异（MMD）进行鲁棒的物体候选合并，同时保持固定粒子集以控制计算复杂度。此外，通过高亲和力物体间的关系传播来纠正因误分类导致的关系损失。

Result: 实验表明 NoPA 在 3D 场景图生成任务上显著优于现有方法，同时不牺牲实时推理速度。

Conclusion: 非参数化分布表示能够有效保留 3D 几何细节，结合 MMD 合并策略与关系传播机制，可在实时推理条件下实现更准确的在线 3D 场景图生成。

Classification reason: 该论文提出非参数化在线3D场景图生成方法，属于3D场景理解与空间推理范畴，与[空间与3D多模态推理]（spatial_3d_reasoning）高度匹配。

Importance reason: Focuses on 3D scene graph generation, which is not a priority subtopic and lacks strong overlap with user's configured direction weights. Signals: 3D scene graph, non-parametric distribution, online inference, real-time, MMD merging
...
### [96] [GaussianFusion: Unified 3D Gaussian Representation for Multi-Modal Fusion Perception](https://arxiv.org/abs/2607.00746)
...
TL;DR: GaussianFusion 提出了一种基于3D高斯表示的多模态融合感知通用框架，替代传统BEV离散网格范式，在连续3D高斯空间中统一多模态特征，支持3D目标检测和语义占用等多种感知任务，在nuScenes上超越BEVFusion和GaussFormer。


<details>
  <summary>Details</summary>
Motivation: 传统BEV表示采用离散网格，导致细节丢失严重，限制了特征对齐和跨模态信息交互。作者希望打破BEV范式，提出一种更自然的连续3D表示来统一多模态融合感知。

Method: 提出基于3D高斯表示的多模态融合框架GaussianFusion。设计前向投影的多模态高斯初始化模块，将多模态特征投影到3D高斯空间；共享跨模态高斯编码器通过注意力机制迭代更新高斯属性（位置、协方差、颜色、透明度等），实现多模态特征在连续3D空间中的统一表示。

Result: 在nuScenes数据集上，3D目标检测任务比BEVFusion提升2.6 NDS；3D语义占用任务比GaussFormer提升1.55 mIoU，同时仅使用30%的高斯数，推理速度提升450%。

Conclusion: GaussianFusion验证了3D高斯表示作为多模态融合感知统一框架的有效性，在保持细节信息的同时实现了任务无关的通用感知能力，为替代传统BEV范式提供了新方向。

Classification reason: 该论文提出基于3D高斯表示的多模态融合感知框架，用于3D目标检测和语义占用预测等3D感知任务，属于空间与3D多模态推理方向，因此复用[CANONICAL] subtopic "空间与 3D 多模态推理"。

Importance reason: Proposes a continuous 3D Gaussian representation for multi-modal fusion, aligning with continuous multimodal representation priority but focused on autonomous driving perception rather than language or generation. Signals: continuous 3D Gaussian representation, multi-modal fusion perception, task-agnostic framework, outperforms BEVFusion, 450% speedup
...
### [97] [OmniView-Space: Reinforcing Spatial Reasoning via Multi-Perspective Spatial Mapping](https://arxiv.org/abs/2607.00881)
...
TL;DR: OmniView-Space 提出一种多视角空间映射框架，通过查询对齐的视觉认知图和文本空间图、工具引导的自我中心推理策略以及认知图蒸馏，显著提升多模态大模型在单图和多图空间推理基准上的性能，达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在空间推理中难以动态地将证据重新锚定到查询所需的特定参考系（如相机中心、物体中心或方向中心），导致多步推理时空间一致性不足。

Method: 提出OmniView-Space框架，包含三个核心组件：(1) 多视角空间映射（MPSM），将重建的几何结构重新锚定到查询对齐的视觉认知图和文本空间图；(2) 工具引导的自我中心推理，训练模型主动选择查询所需的自我锚点并请求对应的MPSM证据；(3) 认知图蒸馏，利用MPSM生成的轨迹和自我帧奖励训练模型使用自生成的认知图进行推理。

Result: 在单图和多图空间推理基准上达到SOTA性能，蒸馏后的模型在减少对外部几何管线依赖的同时保持高性能。

Conclusion: OmniView-Space通过多视角空间映射和认知图蒸馏有效增强了MLLM的空间推理能力，为构建更鲁棒的空间智能系统提供了新思路。

Classification reason: 该论文聚焦于多模态大模型的空间推理能力增强，通过多视角空间映射和认知图蒸馏提升空间一致性，属于多模态理解与生成方向下的空间与3D多模态推理（spatial_3d_reasoning）这一[CANONICAL]子主题。

Importance reason: Strong spatial reasoning method for MLLMs, but not aligned with user's priority subtopics on continuous models or generation. Signals: spatial reasoning, multimodal large language model, multi-perspective mapping, cognitive map distillation, state-of-the-art on benchmarks
...
### [98] [Rosetta: Composable Native Multimodal Pretraining](https://arxiv.org/abs/2607.00293)
...
TL;DR: Rosetta 提出了一种可组合的原生多模态预训练框架，通过动量锚定正交投影（MAOP）解决多模态训练中的梯度冲突与灾难性遗忘问题，在保持已有语言和视觉理解能力的同时，实现图像生成能力的扩展与跨模态协同。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型在扩展新模态时面临严重的梯度冲突和灾难性遗忘问题，标准 MoE 和 MoT 架构均无法有效解决，严重阻碍了多模态模型的可扩展性。

Method: Rosetta 采用模块化范式：全局共享专家保留核心基础知识，即插即用专家处理模态特定能力。提出动量锚定正交投影（MAOP），利用优化器的动量状态作为隐式语义锚点，选择性中和新模态的冲突梯度分量，同时保留协同更新。

Result: 实验表明，Rosetta 在保持已有语言和视觉理解能力的同时，实现了优越的图像生成性能，并展现出跨模态协同效应，而标准 MoE 和 MoT 架构则遭受严重的灾难性遗忘。

Conclusion: Rosetta 为可组合的统一多模态基础模型提供了一种有效的预训练框架，能够实现非破坏性的模态扩展，为构建真正的通用多模态智能铺平了道路。

Classification reason: 该论文提出 Rosetta 框架，旨在实现理解与生成一体化的原生多模态预训练，属于"统一多模态模型"这一现有 subtopic。该 subtopic 在 multimodal_understanding_generation 方向下为 [CANONICAL] 条目，且 multimodal_generation 方向下也有同名非 canonical 条目，两者均匹配。

Importance reason: Directly addresses unified multimodal model expansion without catastrophic forgetting, aligning with high-priority subtopics and boost keywords. Signals: unified multimodal model, continuous multimodal representation, gradient conflict resolution, catastrophic forgetting mitigation, composable modular architecture
...
### [99] [Wake up for Touch! Mask-isolated Tactile Alignment Learning in MLLMs](https://arxiv.org/abs/2607.00302)
*Yoonhyung Park, Minji Kim, Sungwon Moon, Jiyoung Lee*

Main category: cs.RO, cs.MM, cs.CV
...
TL;DR: Splash 提出了一种掩码隔离的触觉对齐学习框架，通过量化预训练参数的重要性，将参数空间划分为休眠子空间和关键子空间，冻结关键子空间以保护视觉知识，仅更新休眠子空间来内化触觉对齐，从而在不破坏原有视觉语言推理能力的前提下为紧凑型 MLLM 添加触觉感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有为 MLLM 添加触觉感知的努力面临零和权衡：紧凑模型的有限参数预算迫使在新感官模态获取与原有视觉语言推理能力保持之间做出选择，导致灾难性遗忘。

Method: Splash 首先量化每个预训练参数的重要性，将参数空间划分为休眠子空间（dormant subspace）和关键子空间（critical subspace）。训练时冻结关键子空间作为稳定锚点以保护通用视觉知识，仅更新休眠子空间来学习触觉对齐，实现选择性、非破坏性的模态扩展。

Result: 在 SSVTP、TVL 和 TacQuad 等视觉-触觉基准上达到最先进性能，同时保持了原有的通用视觉语言能力，且不增加 LLM 部分的推理开销。

Conclusion: Splash 通过参数空间隔离策略有效解决了多模态扩展中的灾难性遗忘问题，为紧凑型 MLLM 添加新感官模态提供了一种高效、无损的范式。

Classification reason: 该论文研究如何为多模态大语言模型（MLLM）引入触觉感知能力，属于 MLLM 的模态扩展与架构改进，与 [CANONICAL] 子主题 "视觉语言模型与 MLLM" 高度相关。

Importance reason: Novel tactile alignment method for MLLMs but outside user's priority subtopics and negative keywords. Signals: tactile reasoning, multimodal LLM, catastrophic forgetting, parameter partitioning, visuo-tactile benchmarks
...
### [100] [What's Hidden Matters: Identifying Planning-Critical Occluded Agents using Vision-Language Models](https://arxiv.org/abs/2607.00283)
...
TL;DR: 本文提出一种利用 VLM 识别自动驾驶中规划关键性遮挡代理的框架，通过规划KL散度（PKL）度量遮挡代理对自车规划的影响，并基于此生成标注数据微调 VLM，使小模型在零样本大模型上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中规划关键性代理可能被遮挡，现有方法要么对所有遮挡采取统一保守策略导致过度防御，要么推断遮挡空间但不评估对规划的影响。本文旨在弥合感知与规划之间的关键差距，使 VLM 能够识别并推理对自车轨迹最关键的遮挡代理。

Method: 提出 Planning KL-divergence (PKL) 信息论度量，系统识别和排序遮挡代理对自车规划的影响；利用 PKL 排序指导专家 VLM (GPT-5) 生成结构化标注；在 nuScenes 数据集上构建高影响场景基准；对多种通用和领域适配 VLM 进行微调。

Result: 微调后的小模型显著超越其更大的零样本对应模型；PKL 引导的数据选择策略相比随机采样提升约 30% 性能；所有微调模型均获得大幅性能提升。

Conclusion: 本文首次提出系统性方法训练 VLM 关注规划关键性遮挡，实现了更语义化、更高效的自动驾驶风险评估，验证了规划感知的数据选择策略对 VLM 微调的有效性。

Classification reason: 该论文使用 VLM（视觉语言模型）进行自动驾驶场景中遮挡代理的识别与推理，属于通用视觉语言模型的应用与微调，映射到 [CANONICAL] 子主题 vlm_mllm_general（视觉语言模型与 MLLM）。

Importance reason: Focuses on autonomous driving occlusion planning, not core to user's priority subtopics like continuous language models or video generation. Signals: VLM for autonomous driving, planning-critical occlusion, PKL metric, nuScenes benchmark, fine-tuning improves zero-shot
...
### [101] [Seed2.0 Model Card: Towards Intelligence Frontier for Real-World Complexity](https://arxiv.org/abs/2607.00248)
*Bytedance Seed*
...
TL;DR: Seed2.0 是字节跳动 Seed 团队发布的多模态大模型系列，针对长尾知识与复杂指令跟随两大挑战进行优化，在推理、视觉理解、搜索等能力上达到领先水平，并通过真实场景用例展示了处理复杂现实任务的能力。


<details>
  <summary>Details</summary>
Motivation: 现有大模型在处理复杂现实任务时仍面临长尾知识不足和复杂指令跟随能力有限的挑战，难以满足用户真实需求。Seed2.0 旨在通过构建可靠的评测体系和针对性优化，提升模型在复杂、长程任务上的可靠性。

Method: 首先识别用户真实需求，基于这些需求选择和抽象基准，构建可靠的前瞻性评测系统。在此评测系统指导下，针对长尾知识和复杂指令跟随两大挑战进行优化，同时提升推理、视觉理解和搜索能力。

Result: Seed2.0 在推理、视觉理解和搜索能力上达到世界领先水平，通过大量真实场景用例展示了处理初始复杂现实任务的能力，为数亿用户提供价值。

Conclusion: Seed2.0 在解决复杂现实任务方面迈出了有意义的一步，通过系统性的评测驱动优化，在多个关键能力上取得突破，展示了向通用人工智能前沿迈进的可能性。

Classification reason: 该论文介绍 Seed2.0 系列模型，涵盖推理、视觉理解、搜索等能力，属于通用多模态大语言模型（MLLM）范畴，因此映射到 [CANONICAL] 的 vlm_mllm_general（视觉语言模型与 MLLM）。

Importance reason: Directly addresses unified multimodal models and complex instruction following, aligning with high-priority subtopics and boost keywords. Signals: unified multimodal model, multimodal large language model, complex instruction following, long-tail knowledge, real-world tasks
...
### [102] [Where Am I? Semantic Map Grounding via Vision-Language Models for Multi-Modal Localization](https://arxiv.org/abs/2607.01079)
...
TL;DR: 本文提出将 GPS 缺失的室内机器人定位重构为语义推理任务，通过 LoRA 微调 Qwen2.5-VL-7B 并附加回归头，融合前视相机图像、LiDAR 点云和语义栅格地图直接预测连续位姿，在 12 万样本数据集上达到 96.75% 完整位姿准确率，并展现出对未见物体类别和不完整地图的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统几何定位方法在 GPS 缺失的室内环境中鲁棒性有限，而人类可以通过物体级线索和标注地图进行定位。作者希望探索视觉语言模型能否利用语义推理能力，融合多模态输入（图像、LiDAR、语义地图）来推断机器人位姿。

Method: 使用 Qwen2.5-VL-7B 作为骨干，通过 LoRA 进行参数高效微调，并在模型末端附加轻量级回归头，直接从最终隐藏状态预测连续位姿坐标 (x, y, theta)，绕过文本生成。训练采用复合位置-方向损失函数和课程学习策略。在自定义 Gazebo 数据集（120,112 样本，527 场景）上训练和评测。

Result: 在分布内测试集上达到 98.23% 位置准确率、98.00% 方向准确率、96.75% 完整位姿准确率，平均位置误差 0.11m，平均方向误差 5.7°，单样本推理时间 0.62s。在 7 个未见物体类别上位置准确率仅下降 7.2 个百分点（90.99%）。不完整地图下微调可恢复至 93.72% 位置准确率。消融实验表明 LiDAR 在相机语义缺失时提供可靠后备信号。

Conclusion: VLM 能够有效利用语义推理进行多模态机器人定位，对未见物体类别和不完整地图具有良好泛化性，且 LiDAR 与相机语义信号互补，在遮挡或稀疏布局下提供可靠后备。

Classification reason: 该论文微调 Qwen2.5-VL-7B（视觉语言模型）并附加回归头进行机器人定位，属于通用 VLM 的应用与微调，映射到 [CANONICAL] subtopic "视觉语言模型与 MLLM"。

Importance reason: Applies VLM to indoor localization but is a niche robotics application, not advancing core continuous multimodal or generation priorities. Signals: VLM for robot localization, LoRA fine-tuning, semantic map grounding, multi-modal fusion, indoor GPS-denied
...
### [103] [AdaBoosting Text Prompts for Vision-Language Models](https://arxiv.org/abs/2607.00684)
...
TL;DR: 本文提出 Text Prompt Boosting (TPB)，一种受 AdaBoost 启发的框架，通过将每个基于文本提示的分类器视为弱学习器，顺序聚合为强集成模型，显式关注难分类样本，从而在少量标注样本下提升 VLM 分类准确率，并支持跨模型迁移。


<details>
  <summary>Details</summary>
Motivation: 预训练 VLM 的分类准确率高度依赖文本提示词质量。现有少样本文本提示方法在构建提示词时未显式关注误分类样本，导致随着样本数增加性能提升有限。本文旨在充分利用少样本监督信号，通过聚焦难例来提升提示词质量。

Method: 提出 TPB 框架，受 AdaBoost 启发，将每个文本提示词对应的分类器视为弱学习器，通过迭代方式顺序构建提示词集合：每轮根据前一轮的误分类样本加权，生成新的文本提示词，最终将所有提示词分类器集成为强分类器。该方法在文本空间中保留任务固有的、与模型无关的线索，支持跨模型迁移。

Result: 在 11 个分类基准上，TPB 在源模型上提升了准确率，并且当迁移到更大、更强的 VLM 时，能够保持少样本带来的性能增益，而现有方法难以维持这种提升。

Conclusion: TPB 通过 AdaBoost 风格的提示词集成，有效利用少样本监督信号聚焦难例，显著提升了 VLM 分类性能，并展现出良好的跨模型迁移能力。

Classification reason: 该论文研究的是面向视觉语言模型（VLM）的文本提示词提升方法，属于通用视觉语言模型范畴，因此映射到 [CANONICAL] 的 vlm_mllm_general（视觉语言模型与 MLLM）子主题。

Importance reason: Focuses on text prompt boosting for VLM classification, not on the user's priority subtopics like continuous models or video generation. Signals: AdaBoost-inspired, few-shot VLM classification, cross-model transfer, not generative, not continuous token or video
...
TL;DR: StochasT 提出随机轮次深度方法，在视觉指令微调中随机将同一图像的语言任务分组为不同大小的轮次簇，以弥合多轮训练与单轮评测之间的差距，提升 LVLM 在单轮和多轮场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉指令微调（VIT）通常将同一图像的多个语言任务打包成多轮对话进行训练，但评测基准却在孤立的单轮场景下评估模型。这种训练与评测的不匹配导致模型在多轮训练中产生视觉注意力衰减和上下文过拟合，难以在单轮测试中发挥最佳性能。

Method: 提出 StochasT（Stochastic Turn Depth），在训练时随机将同一图像的语言任务聚合成不同大小的轮次簇（turn depth），同时保持任务的原始顺序。该方法借鉴了 Dropout 和 ResNet 的随机深度思想，但并非丢弃任何数据，而是通过随机分组最大化训练数据的利用率。此外，还引入基于平衡拉丁方（Balanced Latin Square）的评测机制，用于衡量 LVLM 在不同上下文依赖下的鲁棒性。

Result: 大量实验表明，StochasT 有效提升了 LVLM 在单轮和多轮场景下的能力，使模型在两种场景下都获得均衡且强大的性能。

Conclusion: StochasT 通过随机轮次深度策略弥合了视觉指令微调中训练与评测之间的差距，是一种简单有效且不损失训练数据利用率的方法，为 LVLM 的指令微调提供了新的视角。

Classification reason: 该论文研究视觉指令微调（VIT）中多轮训练与单轮评测不匹配的问题，提出随机轮次深度方法 StochasT 来提升 LVLM 的鲁棒性。这属于通用视觉语言模型训练与微调方法，与 [CANONICAL] 子主题 "视觉语言模型与 MLLM" 高度匹配。

Importance reason: Addresses a training-evaluation gap in LVLMs but is not aligned with top-priority continuous multimodal or video generation topics. Signals: visual instruction tuning, multi-turn vs single-turn gap, stochastic depth for LVLMs, robustness evaluation, not in priority subtopics
...
### [105] [LeVLJEPA: End-to-End Vision-Language Pretraining Without Negatives](https://arxiv.org/abs/2607.00784)
...
TL;DR: LeVLJEPA 是首个完全非对比式的端到端视觉语言预训练方法，通过跨模态预测、stop-gradient 目标和逐模态分布正则化进行学习，无需负样本、温度参数、动量编码器或师生调度。实验表明，其编码器作为冻结的 VLM 骨干网络时，在 GQA、VQAv2、POPE 等任务上优于对比式基线，并在语义分割上表现更优。


<details>
  <summary>Details</summary>
Motivation: 视觉语言预训练长期被对比学习范式主导（如 CLIP），而纯视觉自监督学习已广泛采用非对比方法（如 VICReg、SimSiam）。同时，视觉语言编码器的角色已从零样本分类器转变为 VLM 和密集预测系统的冻结视觉骨干，需要输出完整的 patch token 网格而非单个池化嵌入。现有对比方法依赖负样本和温度参数，且生成的全局特征对密集语义任务支持不足。

Method: 提出 LeVLJEPA（LeViT-JEPA），一种非对比式端到端视觉语言预训练方法。核心思想是跨模态预测：给定图像 patch 和文本 token，模型通过 stop-gradient 目标进行预测学习，并引入逐模态分布正则化（per-modality distributional regularization）。该方法无需负样本、温度参数、动量编码器或师生调度，且在大规模训练中保持稳定。

Result: 作为冻结的 VLM 骨干网络，LeVLJEPA 在 GQA、VQAv2 和 POPE 上，在两种不同语言模型下均优于所有评估的对比式编码器；在语义分割任务上超越对比基线；在全局线性探测等任务上与对比方法持平。

Conclusion: 非对比式预训练能够有效产生密集的语义视觉特征，为视觉语言模型的视觉骨干预训练提供了一种新的有效范式，挑战了对比学习在该领域的统治地位。

Classification reason: 该论文提出了一种全新的非对比式端到端视觉语言预训练方法 LeVLJEPA，属于视觉语言模型（VLM）的预训练方法创新，与 [CANONICAL] 子主题 "视觉语言模型与 MLLM" 高度匹配。

Importance reason: Novel non-contrastive VLM pretraining aligns with user's priority on unified multimodal models and efficient representation learning. Signals: First fully non-contrastive VLM pretraining, Strong dense semantic features for frozen VLM backbone, Outperforms contrastive baselines on GQA, VQAv2, POPE, No negatives, temperature, or momentum encoder, Relevant to continuous multimodal representation and MLLM subtopics
...
### [106] [GenAU: Language-Grounded Industrial Anomaly Understanding with Vision-Language Models](https://arxiv.org/abs/2607.01049)
...
TL;DR: GenAU 是一个统一的视觉语言框架，通过引入两个分割 token ([SEG_defect] 和 [SEG_normal]) 将图像级异常检测、像素级分割、多类型异常检测和缺陷分析整合到单个指令跟随模型中，在跨数据集基准上取得了有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有工业异常检测方法要么只能检测和定位异常但缺乏语言级缺陷理解（如 CLIP 方法），要么能描述缺陷但无法生成像素级分割掩码（如指令微调的 VLM）。作者希望构建一个统一的通用模型，同时完成检测、分割、多类型异常检测和缺陷分析四项任务。

Method: GenAU 在视觉语言模型中引入两个特殊分割 token [SEG_defect] 和 [SEG_normal]，其隐藏状态作为语言引导的查询，对多尺度视觉特征进行交叉注意力以生成像素级定位图。图像级分数通过融合分割图与解码器的文本正常/异常决策得到。语言解码器生成结构化的缺陷感知响应。模型使用联合语言建模和分割目标进行训练。

Result: 在 VisA 和 Real-IAD 数据集上，GenAU 在零样本图像级检测中达到 CLIP 方法中最强性能；分割性能接近但未超越专门的 CLIP 基线。模型以可量化的检测和分割性能代价，实现了零样本多类型检测和语言引导的缺陷分析。

Conclusion: GenAU 证明了将工业异常检测的多个子任务（检测、分割、分类、分析）统一到一个 VLM 框架中的可行性，为工业视觉检测提供了更全面的解决方案，尽管在纯分割性能上略逊于专用方法。

Classification reason: 该论文提出 GenAU，一个面向工业异常检测与理解的通用视觉语言模型，通过指令微调统一了检测、分割和缺陷分析任务。这属于通用视觉语言模型（VLM/MLLM）的范畴，与 canonical subtopic "vlm_mllm_general" 高度匹配。

Importance reason: Focuses on industrial anomaly detection, not aligned with user's priority subtopics like continuous language models or video generation. Signals: Industrial anomaly detection, Vision-language model, Segmentation tokens, Cross-dataset benchmark, Limited personal relevance
...
### [107] [DART-VLN: Test-Time Memory Decay and Anti-Loop Regularization for Discrete Vision-Language Navigation](https://arxiv.org/abs/2607.01043)
...
TL;DR: DART-VLN 提出了一种无需训练的测试时控制框架，通过记忆衰减和反循环正则化来改善离散视觉语言导航代理在部分可观测环境中的可靠性和效率。


<details>
  <summary>Details</summary>
Motivation: 基于记忆的离散视觉语言导航（VLN）代理在部分可观测环境下运行时，即使使用强大的冻结骨干网络，在测试时仍存在两个常见故障模式：记忆读取时陈旧历史证据的干扰，以及动作选择时低效的局部回溯。现有方法需要重新训练或引入额外参数，缺乏轻量级的测试时解决方案。

Method: 提出 DART-VLN 框架，包含两个核心组件：(1) 测试时记忆衰减（Test-Time Memory Decay），一种读取侧的记忆重加权规则，在不修改存储内容的情况下抑制陈旧和冗余证据；(2) 反循环正则化（Anti-Loop Regularization），一种轻量级的下一跳惩罚机制，阻止动作选择中的立即反向移动。该框架不引入任何可学习参数，保持骨干网络不变。

Result: 在 R2R 和 REVERIE 基准上的实验表明：仅使用记忆衰减即可获得稳定的读取侧增益；记忆衰减+反循环正则化实现了最佳的质量-效率权衡，产生更短的轨迹、更低的运行时间和更好的导航性能。行为分析证实反循环正则化减少了局部回溯并提高了路径效率。

Conclusion: 适度的测试时控制可以在无需重新训练的情况下，使基于记忆的离散 VLN 代理更加可靠和高效。记忆衰减和反循环正则化作为轻量级插件，可有效提升冻结骨干网络的导航性能。

Classification reason: 该论文研究离散视觉语言导航（VLN），属于视觉语言理解与具身推理的交叉领域。VLN 本质上是一种视觉问答与空间推理任务，与 canonical 子主题"视觉问答与文档理解"（multimodal_vqa_doc）相关，因为 VLN 涉及视觉场景理解与指令跟随。同时该论文也涉及空间推理，但 multimodal_vqa_doc 作为更广泛覆盖的 canonical 子主题更为合适。

Importance reason: Focuses on discrete VLN navigation, which is not a priority subtopic and lacks alignment with user's key boost keywords. Signals: Test-time control, memory decay, anti-loop regularization, no new parameters, R2R/REVERIE
...
### [108] [Personalized Object Identification and Localization via In-Context Inference with Vision-Language Models](https://arxiv.org/abs/2607.00357)
...
TL;DR: 本文提出个性化目标识别与定位（POIL）任务，要求VLM在定位目标对象的同时拒绝不包含该对象的查询图像。作者构建了POIL数据集，并提出IPLoc-ID算法，通过自回归框架中的自提问机制连接候选框预测与目标验证两步，有效抑制误检。


<details>
  <summary>Details</summary>
Motivation: 现有个性化目标定位（POL）方法假设查询图像始终包含目标对象，这严重限制了其在真实场景中的应用。实际场景中大量查询图像并不包含目标对象，需要模型具备拒绝能力。

Method: 提出IPLoc-ID算法，包含两步：1）预测候选边界框；2）判断候选框是否对应参考对象实例。引入自提问（self-posed query）机制，将两步连接在单一自回归生成框架中，使VLM能先预测框再自我验证。

Result: 在构建的POIL数据集上，IPLoc-ID在负样本查询图像上大幅抑制了误检，同时保持了与IPLoc相当的正样本定位性能。

Conclusion: IPLoc-ID有效解决了实际场景中的实例级POIL任务，弥补了传统目标检测、少样本目标检测和纯定位方法无法充分处理的空白。

Classification reason: 该论文研究个性化目标定位（POL/POIL），属于视觉问答与细粒度视觉理解范畴。虽然任务更接近目标检测，但方法基于VLM的上下文推理（in-context inference），与多模态视觉问答/理解高度相关。复用[CANONICAL] subtopic "multimodal_vqa_doc"（视觉问答与文档理解），因其涵盖VLM驱动的视觉理解任务。

Importance reason: Focuses on a narrow VLM in-context localization task with limited connection to user's priority subtopics like continuous models or video generation. Signals: VLM in-context inference, object localization, false-positive rejection, self-posed query, limited to instance-level detection
...
### [109] [GEAR-Seg: A Grounded Explainable Agent for Reasoning Segmentation and Data Engine](https://arxiv.org/abs/2607.00544)
...
TL;DR: GEAR-Seg 提出一种显式解耦的推理分割智能体，将视觉像素转化为属性文本，通过 LLM 进行显式逻辑推理，实现零样本推理分割，并可作为数据引擎自动构建大规模数据集 GEAR-131K。


<details>
  <summary>Details</summary>
Motivation: 现有端到端推理分割模型将感知与推理纠缠在不可解释的黑箱中，缺乏可解释性和可扩展性。作者希望将推理分割过程显式解耦为类无关分割、语义描述和 LLM 推理三个独立步骤，使推理过程透明可追踪。

Method: GEAR-Seg 采用三阶段解耦架构：(1) 类无关分割模块（SAM）生成候选掩码；(2) 语义描述模块为每个掩码生成丰富的属性文本描述；(3) LLM 基于文本描述进行显式推理，选择与查询匹配的目标。整个流程将视觉推理转化为显式的文本逻辑链。同时，该框架可作为数据引擎自动生成推理分割训练数据，构建了 GEAR-131K 数据集。

Result: 在多个推理分割和细粒度指代分割基准上取得具有竞争力的零样本性能。利用 GEAR-131K 数据集进行蒸馏实验，轻量级模型在仅使用自动生成数据监督下，性能接近使用昂贵人工标注数据的上界。

Conclusion: GEAR-Seg 通过显式解耦感知与推理，为推理分割提供了可解释、可扩展的新范式，同时其数据引擎能力可大幅降低高质量训练数据的获取成本。

Classification reason: 该论文提出 GEAR-Seg，一个用于推理分割（reasoning segmentation）的显式解耦智能体，将视觉像素转化为属性丰富的文本，通过 LLM 进行显式推理链。这属于视觉问答与推理分割范畴，与 [CANONICAL] subtopic "多模态评测与基准"（构建了 GEAR-131K 数据集）和 "视觉问答与文档理解"（推理分割本质上是细粒度 VQA + 分割）均相关。选择 multimodal_vqa_doc 作为主分类，因为论文核心任务是推理分割（reasoning segmentation），属于 VQA 的延伸。

Importance reason: Decoupled agent for reasoning segmentation is solid but not aligned with user's priority on continuous multimodal models or video generation. Signals: reasoning segmentation, zero-shot inference, data engine, LLM-based agent, interpretable logic chain
...
TL;DR: 本文提出HEE（Hierarchical Entity Exploration），一种无需训练、模型无关的框架，通过层次化实体探索将高分辨率图像理解转化为动态、查询引导的探索过程。HEE采用双评分机制评估区域信息充分性，在信息不足时对最有希望的区域进行目标检测提取细粒度实体，聚类为子区域并构建多级语义层次进行深入探索，同时引入置信度引导的回溯机制处理失败路径。在Visual Probe和HR-Bench基准上，HEE在多个MLLM上优于ZoomEye和RAP等训练无关方法，并在MME-RealWorld上展现了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 高分辨率图像感知对MLLM仍是关键挑战，现有方法要么需要额外训练来学习关注位置，要么启发式地将图像划分为固定区域，在复杂高分辨率场景中泛化能力不足。本文旨在设计一种无需训练、能自适应探索图像细粒度信息的框架。

Method: HEE框架包含三个核心模块：(1) 双评分机制——评估每个区域是否包含足够证据回答问题；(2) 层次化实体探索——在最有希望的区域应用目标检测提取细粒度实体，聚类为语义子区域并构建多级层次结构；(3) 置信度引导的回溯机制——当深层区域无法给出置信答案时，回溯到替代路径。整个过程无需训练，可即插即用于不同MLLM。

Result: 在Visual Probe和HR-Bench两个复杂高分辨率基准上，HEE在Qwen2.5-VL和LLaVA-OneVision等多个MLLM上均优于ZoomEye和RAP等训练无关方法，在准确率和效率上均取得更好表现。在MME-RealWorld基准上也展现了良好的泛化能力。

Conclusion: HEE通过将静态图像理解转化为动态查询引导的实体探索，有效提升了MLLM在高分辨率图像上的细粒度感知能力，且无需额外训练，具有良好的通用性和泛化能力。

Classification reason: 该论文提出HEE框架，通过层次化实体探索提升MLLM在高分辨率图像上的视觉感知能力，核心任务是对高分辨率图像进行细粒度视觉问答。这与[CANONICAL] subtopic "视觉问答与文档理解"（multimodal_vqa_doc）高度匹配，因为论文聚焦于HR图像上的VQA任务，且方法无需训练、模型无关。

Importance reason: Novel training-free hierarchical exploration for HR visual perception, but not aligned with user's priority subtopics. Signals: training-free, hierarchical entity exploration, high-resolution perception, MLLM, query-guided
...
### [111] [Learning to Watch: Active Video Anomaly Understanding via Interleaved Policy Optimization](https://arxiv.org/abs/2607.00622)
...
TL;DR: 本文提出 Anom-π，一个将视频异常理解重构为主动序贯决策过程的闭环框架。通过交错策略统一内部认知推理与外部证据获取，利用时序原子操作（局部回溯、时间扩展、细粒度采样）赋予模型感知主动性，并设计交互式直接偏好优化（iDPO）在视频级弱监督下实现轨迹级策略对齐。仅2B参数即在复杂场景中显著超越大规模VAU模型。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常理解方法采用被动范式，静态采样无法区分语义上不同的事件（观测混叠问题）。受人类视频回放行为启发，作者希望将视频理解重构为主动序贯决策过程，使模型能够主动获取证据来消除歧义。

Method: 提出 Anom-π 闭环框架，包含：(1) 交错策略，统一内部认知推理与外部证据获取；(2) 时序原子操作（局部回溯、时间扩展、细粒度采样）；(3) 交互式直接偏好优化（iDPO），在视频级弱监督下实现轨迹级策略对齐；(4) 主动证据查询（AEI）效用函数，平衡任务成功、信息获取与交互成本。

Result: 仅2B参数即达到高度竞争性性能，在复杂场景中显著超越大规模VAU模型。

Conclusion: 主动序贯决策范式能有效解决视频异常理解中的观测混叠问题，弱监督下的轨迹级策略对齐方法可行且高效。

Classification reason: 该论文关注视频异常理解（Video Anomaly Understanding），属于视频理解范畴，与现有 canonical subtopic "视频理解与时序推理"（video_understanding_temporal）高度匹配。论文将视频理解重构为主动序贯决策过程，涉及时序推理和视频级理解。

Importance reason: Novel active sequential decision-making framework for video anomaly understanding, but outside user's priority subtopics. Signals: Active sequential decision-making, Interleaved policy optimization, Video anomaly understanding, Weak supervision, 2B parameter model
...
### [112] [DART: Difficulty-Adaptive Routing for Zero-Shot Video Temporal Grounding](https://arxiv.org/abs/2607.00672)
...
TL;DR: DART 提出了一种难度自适应路由的零样本视频时序定位方法，通过 DPP 选择关键帧并评估查询难度，简单查询走快速路径直接预测，复杂查询走慢速路径使用时序标记提示进行结构化推理，在 Charades-STA 和 ActivityNet Captions 上取得了 SOTA 零样本性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本视频时序定位方法依赖帧-查询特征匹配，对简单事件有效，但难以处理需要理解时序顺序和因果结构的复杂多阶段查询。作者将这种差距称为"推理鸿沟"（reasoning gap），并希望弥合这一差距。

Method: DART 包含两个核心组件：(1) 查询条件 DPP（Determinantal Point Process），用于选择多样且与查询相关的关键帧作为时序证据，同时通过谱熵提供难度指示；(2) 双路径路由机制：简单查询走 Fast 路径直接预测，复杂查询走 Slow 路径，使用 Temporal Markup Prompting 将定位分解为全局事件分析、逐帧时序角色标注和边界提取三个步骤。

Result: 在 Charades-STA 和 ActivityNet Captions 上，DART 在 ID 和多个 OOD 设置下均达到零样本 SOTA，mIoU 最高提升 3.5 个点，同时使用的帧数减少 7 倍以上。

Conclusion: DART 通过难度自适应路由和结构化推理有效弥合了零样本视频时序定位中的推理鸿沟，在性能和效率上均优于现有方法，验证了根据查询复杂度动态分配计算资源的有效性。

Classification reason: 该论文研究零样本视频时序定位（Video Temporal Grounding），属于视频理解与时序推理方向，与 [CANONICAL] subtopic "视频理解与时序推理"（video_understanding_temporal）高度匹配。

Importance reason: Focuses on zero-shot video grounding, which is not a priority subtopic and lacks direct connection to continuous language models or generation. Signals: zero-shot video temporal grounding, difficulty-adaptive routing, DPP keyframe selection, temporal markup prompting, Charades-STA
...
TL;DR: EFlow 是一个面向长视频推理的证据优先框架，基于 Qwen3-VL，通过分离时序定位与逻辑推理（CoT for Temporal Grounding 和 CoT for Reasoning），并引入置信度感知的反思机制，在证据不足时重新审视全视频，结合 SFT、RL 和 RFT 训练，在五个视频理解基准上持续提升长视频推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强的视频框架通常将时序定位与答案推理交织在单一轨迹中，导致早期语义假设（过早语义承诺）偏置证据定位，从而检索到不完整证据并进一步强化错误推理。作者旨在解决这一失败模式。

Method: EFlow 将时序定位与逻辑推理显式分离为两个 CoT 阶段：先进行时序定位 CoT 检索相关证据，再进行推理 CoT 回答问题。同时引入置信度感知的反思机制，当检索到的证据可能不充分时重新评估全视频。构建专用轨迹数据集，通过监督微调、强化学习和强化微调（RFT）训练模型。

Result: 在五个视频理解基准上的大量实验表明，EFlow 一致性地提升了长视频推理性能。

Conclusion: 证据优先的分离式推理范式能有效缓解过早语义承诺问题，置信度感知的反思机制进一步增强了长视频推理的鲁棒性。

Classification reason: 该论文聚焦长视频推理，提出将时序定位与逻辑推理分离的证据优先框架，属于视频理解与时序推理范畴，与 [CANONICAL] subtopic "视频理解与时序推理" 高度匹配。

Importance reason: Strong evidence-first reasoning framework for long-video understanding, but not directly aligned with user's priority subtopics like continuous models or generation. Signals: long-video reasoning, evidence-first framework, confidence-aware reflection, Qwen3-VL, SFT+RL+RFT training
...
### [114] [QCA: Query- and Content-Aware Keyframe Selection for Long Video Understanding](https://arxiv.org/abs/2607.00983)
...
TL;DR: 提出 QCA（Query- and Content-Aware）关键帧选择框架，通过查询相关性和内容偏差联合建模，动态分配帧预算并从长视频中选取紧凑且信息丰富的帧子集，无需额外训练即可集成到现有 Video-LLM 中，在多个长视频理解基准上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 长视频理解中，密集帧序列存在严重的时序冗余，处理所有帧既语义低效又计算昂贵，而实际只有少量帧与给定查询相关。现有方法缺乏对查询相关性和内容多样性的联合考虑。

Method: QCA 框架包含两个阶段：1）将视频划分为时序片段，通过联合建模查询相关性和内容偏差估计每个片段的信息贡献，动态分配关键帧预算；2）在每个片段内，锚定最查询相关的帧，并迭代添加帧以最大化多样性同时保持高语义相关性。方法无需额外训练。

Result: 在多个长视频理解基准上达到 SOTA，例如 LongVideoBench 上使用 128 帧达到 67.8%，而 GPT-4o 使用 256 帧仅达 66.7%。

Conclusion: QCA 是一种高效、无需训练的关键帧选择方法，能显著提升 Video-LLM 在长视频理解任务上的性能与效率，具有良好的泛化能力。

Classification reason: 该论文研究长视频理解中的关键帧选择方法，旨在提升 Video-LLM 在长视频上的理解效率与性能，属于多模态理解与生成方向下的视频理解与时序推理（video_understanding_temporal）这一规范子主题。

Importance reason: Focuses on long video understanding keyframe selection, not directly on the user's priority subtopics like continuous models or generation. Signals: Long video understanding, Keyframe selection, Query-aware, Content-aware, No training required
