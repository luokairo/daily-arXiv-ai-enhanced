...
- [世界模型](#direction-world_model) [Total: 42]
  - [具身世界模型与规划控制](#direction-world_model-embodied_world_model) [Total: 36]
...
- [多模态生成](#direction-multimodal_generation) [Total: 47]
  ...
  - [自回归视觉生成](#direction-multimodal_generation-autoregressive_visual_generation) [Total: 4]
...
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 64]
  ...
  - [多模态推理与思维链](#direction-multimodal_understanding_generation-multimodal_reasoning_cot) [Total: 9]
...
### [1] [Closed-Loop Control with Rule-Aligned Small Language Models and Multi-Agent Self-Correction](https://arxiv.org/abs/2607.09713)
*Yuchen Wang, Javal Vyas, Tong Liu, Mehmet Mercangoz*

Main category: cs.AI, cs.MA, cs.RO
...
TL;DR: 本文提出一种基于小语言模型(SLM)的闭环控制框架，使用Qwen2.5-1.5B模型经GRPO对齐，结合动作智能体、数字孪生验证层和重新提示智能体，在热控制仿真中实现91.5%的动作对齐准确率和3.84秒平均推理延迟，验证了SLM+验证器架构在边缘自主控制中的可行性。


<details>
  <summary>Details</summary>
Motivation: 工业自主运行的关键在于能够从自然语言需求规格中创建和重新配置控制策略，但大型云端模型推理延迟高、计算开销大、数据敏感，不适合边缘闭环控制场景。因此需要探索紧凑的小语言模型能否被重新训练用于控制推理，并嵌入验证器引导的校正循环中。

Method: 使用Qwen2.5-1.5B小语言模型，通过GRPO(Group Relative Policy Optimization)进行对齐训练。框架包含三个组件：(i) 动作智能体(action agent)生成候选控制动作；(ii) 符号/数字孪生验证层检查动作有效性；(iii) 重新提示智能体(reprompting agent)迭代修正输出直至生成有效动作。在随机热控制仿真中进行30组实验，每组500步。

Result: 框架在30组实验中平均动作对齐准确率达91.5%（范围86.3%-100%），平均推理延迟3.84秒。在符号重映射条件下，保持95%的在范围率(in-range rate)，表明尽管token级一致性有所下降，物理调节仍然稳健。

Conclusion: SLM+验证器架构是实现边缘可重构自主控制的一条实用路径，在保持较低推理延迟的同时实现了高动作对齐准确率，为工业边缘部署提供了可行方案。

Classification reason: 该论文研究基于小语言模型(SLM)的闭环控制策略生成，结合数字孪生验证器进行动作校正，属于具身智能/机器人控制中的世界模型与规划控制方向，与[具身世界模型与规划控制]规范子主题高度匹配。

Importance reason: Focuses on SLM control alignment, not on the user's priority continuous multimodal or video generation topics. Signals: SLM control, GRPO alignment, digital twin validation, multi-agent self-correction, edge deployment
...
### [2] [A Control Theory of Predictability in Latent World Models](https://arxiv.org/abs/2607.10362)
...
TL;DR: 本文从控制理论角度分析了潜空间世界模型中预测误差与控制性能之间的脱耦问题，证明数据平均的预测误差不能约束规划器性能，并提出了基于规划器可达状态分布的保真度评分作为更好的模型选择指标。


<details>
  <summary>Details</summary>
Motivation: 当前潜空间世界模型训练和选择都依赖预测误差（单步或多步 rollout loss），认为更低的预测误差意味着更好的控制性能。但规划器在规划时查询的是候选动作所到达的状态，这些状态往往偏离训练数据流形，因此数据平均的预测误差无法有效反映控制性能。作者希望从理论上揭示这一结构性缺陷，并提出更好的模型选择准则。

Method: 将目标重新定义为预测计划成本与真实计划成本之间的差异，证明规划器次优性受该差异的两倍上界约束，而数据平均预测误差既不约束也不跟踪该差异。在线性控制假设下，该差异分解为两项：流形上的残差（由潜转移算子的非正态性决定）和流形外的发散（由动作将状态带离流形导致）。通过合成算子验证定价公式，并在潜模型预测控制（MPC）实验中验证了单步验证误差与控制成功率几乎不相关，而基于规划器可达状态分布的保真度评分则能有效跟踪控制性能。

Result: 理论推导表明规划器次优性由预测与真实计划成本之差的上界决定，而非数据平均预测误差。合成算子实验验证了定价公式的正确性。潜MPC实验显示：不同种子下，单步验证误差与控制成功率几乎不相关（r≈0），而规划器可达保真度评分与控制成功率高度相关。

Conclusion: 数据平均的预测误差不能作为世界模型控制性能的可靠指标，规划器可达状态分布上的保真度评分是更好的模型选择准则。这一发现对当前世界模型的训练和评估实践具有重要指导意义，提示需要重新设计面向控制的模型训练目标。

Classification reason: 该论文研究潜空间世界模型中的预测误差与控制性能之间的关系，核心关注 planner 使用世界模型进行动作规划时的可控性与预测误差的脱耦问题，属于世界模型方向中具身规划与控制的理论分析，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Directly addresses a critical flaw in latent world model evaluation, highly relevant to your world model and continuous representation priorities. Signals: World model theory, Control theory analysis, Planner-reachable distribution, Off-manifold divergence, Model selection metric
...
TL;DR: ABot-AgentOS 是一个通用机器人 Agent 操作系统，位于底层控制器之上，提供场景条件规划、上下文隔离技能执行、多阶段验证、多模态记忆和边云协作。同时提出 EmbodiedWorldBench 可执行基准测试和 Universal Multi-modal Graph Memory 持久化记忆机制，通过故障驱动的自我进化循环实现持续改进。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM/VLA 系统提升了机器人感知和动作预测能力，但长时域具身智能体仍缺乏一个通用的运行时层来支持推理、记忆、工具使用、验证和跨本体执行。现有系统在持久化记忆、可审计交互历史和持续自我改进方面存在不足。

Method: 提出 ABot-AgentOS 架构，包含：(1) 场景条件规划器；(2) 上下文隔离技能执行；(3) 多阶段验证机制；(4) Universal Multi-modal Graph Memory（将对话、视觉观察、空间上下文、时序关系和任务轨迹编码为类型化节点和边）；(5) 边云协作架构；(6) 故障驱动的自我进化循环（将诊断出的记忆故障转化为门控运行时进化资产，仅推广到后续评估分割）。同时构建 EmbodiedWorldBench 可执行基准测试。

Result: 在 EmbodiedWorldBench 子集上，ABot-AgentOS 在任务成功率和目标完成度上均优于单控制器基线。在记忆基准测试上，ABot-AgentOS Static 在 LoCoMo 上达 87.5，OpenEQA EM-EQA 上达 59.9，Mem-Gallery 上达 88.6，NExT-QA 上达 76.5 Acc@All；自我进化进一步将 LoCoMo 提升至 88.7，OpenEQA 提升至 60.4，Mem-Gallery 提升至 89.0。

Conclusion: 通用 Agent OS 层能够改善长时域具身执行，同时提供持久化、可审计的记忆以支持持续交互。ABot-AgentOS 展示了在具身智能系统中集成记忆、规划、验证和自我进化机制的有效性。

Classification reason: 论文提出ABot-AgentOS，一个通用机器人Agent操作系统，包含场景条件规划、多模态记忆、跨本体执行等能力，属于具身智能/机器人领域的世界模型与规划控制。同时其多模态记忆与评测基准也与多模态理解与生成方向相关。主要映射到[CANONICAL]具身世界模型与规划控制。

Importance reason: Strong world model and memory architecture for embodied agents, but lacks direct focus on user's top priority continuous language/multimodal models. Signals: world model, lifelong multi-modal memory, embodied agent OS, failure-driven self-evolution, EmbodiedWorldBench benchmark
...
TL;DR: ABot-N1 是一个面向通用视觉语言导航的基础模型，通过慢-快双架构将高层认知推理与低层运动控制解耦：慢速视觉语言推理器执行显式思维链推理并生成像素级目标锚点，快速动作专家利用文本线索和像素引导以原生控制频率生成连续航点。该方法在室内外大规模导航任务上取得 SOTA，POI 到达率提升 35.0% 至 77.3%，室内外场景成功率分别达 95.4%/92.9%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航方法通常采用端到端黑盒策略直接将观测映射到动作，存在坐标漂移、长尾语义处理能力差、缺乏可解释性等问题，难以同时实现通用性、鲁棒性和透明性。

Method: 提出 ABot-N1，采用慢-快双架构：(1) 慢速视觉语言推理器（Slow Vision-Language Reasoner）执行显式 Chain-of-Thought 推理，输出像素级目标锚点（pixel goal）作为通用接口；(2) 快速动作专家（Fast Action Expert）结合文本线索和像素引导，以原生控制频率生成连续航点。通过像素级锚点桥接高层意图与低层控制，实现可解释、鲁棒且泛化的导航。

Result: 在城市场景 POI 导航中到达率提升 35.0%（达 77.3%），复杂室内外场景成功率分别达 95.4%/92.9%，在物体到达、人物跟随和指令跟随任务中均保持优越鲁棒性，建立了新的 SOTA。

Conclusion: ABot-N1 通过认知-控制解耦的慢-快架构，有效解决了视觉语言导航中的坐标漂移、长尾语义和可解释性问题，为通用视觉语言导航基础模型迈出了重要一步。

Classification reason: 该论文提出视觉语言导航基础模型 ABot-N1，采用慢-快架构将认知推理与运动控制解耦，面向具身导航任务（点目标、物体目标、POI、指令跟随、人物跟随），属于具身智能中的规划控制与导航，因此映射到 world_model 方向下的 [CANONICAL] 具身世界模型与规划控制。同时其视觉语言推理部分也涉及多模态理解与生成方向。

Importance reason: Strongly aligns with world model and embodied navigation priorities, with a novel slow-fast architecture and SOTA results. Signals: world model, visual language navigation, slow-fast architecture, pixel goal anchor, SOTA indoor/outdoor navigation
...
Importance: 普通相关 (64.41) - 未入选精读

TL;DR: 本文提出Traj-VLN，通过自回归轨迹生成让VLM直接在2D像素空间中学习导航交互，无需深度或3D几何信息。模型根据语言指令和历史观测，逐像素预测轨迹坐标，在VLN-CE任务上以有限计算资源达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 现有VLN-CE方法通常依赖深度或3D几何信息来建模空间交互，但VLM主要在RGB图像上预训练，对深度信息不熟悉。作者希望探索一种替代方案：让VLM直接在2D像素空间中学习导航交互，避免引入VLM不熟悉的3D几何信息。

Method: 将VLN任务分解为一系列子任务，每个子任务对应一个3D空间交互过程。给定语言指令和历史观测，模型通过自回归方式依次预测一系列像素坐标，在当前观测的底部中心绘制一条轨迹。使用像素空间轨迹监督替代离散动作学习，微调VLM以学习导航交互。

Result: 实验证明像素空间轨迹监督显著优于离散动作学习，旗舰模型在有限计算资源和训练数据下达到了SOTA级别的性能。

Conclusion: 像素空间轨迹生成是一种有效的VLN-CE方法，避免了引入VLM不熟悉的3D几何信息，同时利用VLM强大的先验知识实现了跨场景泛化。

Classification reason: 该论文研究具身智能体在连续环境中的视觉语言导航（VLN-CE），通过自回归轨迹生成实现像素空间中的交互与导航规划，属于具身世界模型与规划控制方向。同时涉及VLM微调用于导航任务，也与多模态理解与生成方向相关。

Importance reason: Applies autoregressive trajectory generation in pixel space for embodied navigation, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: Autoregressive trajectory generation, Pixel-space interaction, VLN-CE, Embodied world model, No depth/3D needed
...
### [6] [Maximizing Human Efficiency in Large-Scale Robot Post-Training via VLAC-Cut Guided Pipeline](https://arxiv.org/abs/2607.09776)
...
TL;DR: 本文提出了一种最大化人类效率的VLA模型后训练流水线，通过角色分工（远程操作员+地面监控员）和VLAC-Cut自动轨迹筛选工具，在四个真实操作任务上实现了80%-95%的成功率，并将任务吞吐量提升1.7-4.2倍。


<details>
  <summary>Details</summary>
Motivation: VLA模型在下游任务中需要多轮后训练迭代，但单轮数据无法解决所有问题。现有方法依赖大量人工干预，人力成本高、效率低。本文旨在最大化单位人力和时间投入下的策略改进与任务吞吐量。

Method: 提出人机协作的后训练流水线：(1) 角色分工：远程操作员负责高价值远程干预和恢复演示，地面监控员负责多台机器人监控、接管触发和物理重置；(2) VLAC-Cut自动轨迹筛选工具：将自主机器人轨迹分割为进展、空闲、失败诱导和恢复四类，保留有用片段，过滤有害或无信息片段；(3) 将筛选后的轨迹数据与人在环数据结合用于下一轮后训练。

Result: 在四个真实操作任务上，经过迭代后训练，最终策略达到80%-95%的成功率，任务吞吐量相比基础模型提升1.7-4.2倍。在相同人工干预预算下，VLAC-Cut引导的轨迹复用优于仅使用人在环数据的训练方法。

Conclusion: 通过角色分工和自动轨迹筛选，可以显著提升VLA模型后训练中的人类效率，使少量操作员能够监督多台机器人，实现更高效的策略迭代和部署。

Classification reason: 该论文研究VLA（视觉语言动作）模型在下游任务中的后训练流程优化，涉及机器人操作策略改进、人类监督效率提升以及自动轨迹数据筛选（VLAC-Cut），属于具身智能中机器人策略学习与规划控制的范畴，因此映射到[具身世界模型与规划控制]这一规范子主题。

Importance reason: Focuses on robot post-training efficiency, not on user's priority subtopics like continuous language models or video generation. Signals: robot post-training, VLA model, human efficiency, trajectory curation, manipulation tasks
...
### [7] [A Comprehensive Survey and Systematic Real-World Evaluation of Embodied Vision-and-Language Navigation](https://arxiv.org/abs/2607.09792)
...
TL;DR: 本文对具身视觉-语言导航（VLN）进行了全面综述，沿动作范式（层次化 vs 整体化）和模型范式（判别式 vs 生成式）两个维度对现有方法进行分类分析，并在真实机器人平台上进行了系统评估。实验表明，仿真与真实部署之间存在显著性能差距（如单模态RGB方法仿真成功率61%降至真实22%），揭示了当前VLN方法在感知、决策和控制方面的关键挑战。


<details>
  <summary>Details</summary>
Motivation: 现有导航方法依赖高度结构化的模型和强先验假设，在开放不确定的真实环境中鲁棒性不足。VLN通过数据驱动方式融合自然语言理解与视觉感知，有望提升机器人导航的鲁棒性，但缺乏系统的方法论分类和真实世界验证。

Method: 沿两个正交维度组织现有VLN方法：（1）动作范式——层次化框架（高层规划+低层控制）与整体化框架（端到端策略）；（2）模型范式——判别式方法（如基于BERT的跨模态匹配）与生成式方法（如基于LLM的推理）。并在物理机器人平台上对代表性系统配置进行真实世界评估。

Result: 在10个真实场景中，单模态RGB整体化方法仿真成功率61%降至真实22%；层次化框架真实成功率为51%，鲁棒性更强。揭示了仿真到真实的显著性能差距。

Conclusion: VLN在真实部署中面临感知、决策和控制方面的重大挑战，层次化框架比整体化方法更具鲁棒性。未来需弥合仿真与真实之间的差距，提升模型在开放环境中的泛化能力。

Classification reason: 该论文是关于具身视觉-语言导航（VLN）的综述，涉及机器人导航、自然语言理解与视觉感知的融合，属于具身智能与规划控制范畴，与[具身世界模型与规划控制]这一规范子主题高度匹配。

Importance reason: Comprehensive survey with real-world validation, but focuses on embodied VLN which is peripheral to your core continuous multimodal and generation priorities. Signals: comprehensive survey, real-world evaluation, simulation-to-reality gap, hierarchical vs monolithic, discriminative vs generative
...
### [8] [TS-Mask VLA: 2D Temporal-Spatial Masking for Vision-Language-Action Model with Effective Bridging](https://arxiv.org/abs/2607.09818)
...
Importance: 值得关注 (65.15) - 未入选精读

TL;DR: TS-Mask VLA 提出了一种面向机器人操作的 VLA 框架，通过离散扩散动作专家（Discrete Diffusion Action Expert）结合桥接注意力（Bridge Attention）条件注入，以及时序-空间 2D 掩码策略，显式建模动作序列的时空结构，在 LIBERO 和 CALVIN 等基准上以 0.5B 参数取得了领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有自回归 token 式 VLA 模型将动作生成简化为 next-token 预测，缺乏对动作序列时空结构的显式建模，也未能有效解耦视觉-语言表征与动作生成，导致在长程和复杂场景中性能受限。

Method: TS-Mask VLA 包含两个核心设计：(1) 离散扩散动作专家（Discrete Diffusion Action Expert），配备桥接注意力（Bridge Attention）条件注入模块，实现 VLM 的多层条件注入，生成更准确稳定的动作；(2) 时序-空间 2D 掩码策略，对离散动作 token 在时间维度和维度间进行掩码，增强模型对跨时间依赖和维度间耦合的理解。

Result: 在 LIBERO 基准上以 0.5B 参数达到 95.7% 平均成功率，超越更大模型；在 CALVIN 上取得最佳平均序列长度 4.19，长程任务表现优异。

Conclusion: TS-Mask VLA 通过离散扩散动作生成和 2D 时空掩码策略，有效提升了 VLA 模型在长程复杂操作任务中的性能，验证了显式建模动作时空结构的有效性。

Classification reason: 该论文提出 TS-Mask VLA，一个面向机器人操作的视觉-语言-动作（VLA）框架，涉及具身智能中的动作生成与规划控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics VLA work with diffusion action modeling, but only moderate alignment with user's priority subtopics. Signals: world_model, discrete diffusion action, bridge attention, temporal-spatial masking, robot manipulation
...
### [9] [More Structure, Not More Capacity: Object-Centric Representations for Visuomotor Imitation Learning](https://arxiv.org/abs/2607.09825)
...
TL;DR: 本文研究在机器人模仿学习中，使用物体中心化槽表征（SPOT）替代全局场景嵌入或密集补丁网格，在ManiSkill3 PickCube-v1任务上，冻结编码器条件下将成功率从32.6%提升至55.0%，并引入运动学失败分类法分析失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作策略依赖预训练视觉模型，但无论是全局场景嵌入还是密集补丁网格，都会混合任务相关与无关特征，导致策略学习效率低下。本文探索物体中心化槽表征能否通过结构化特征分组来提升模仿学习性能。

Method: 使用冻结的SPOT编码器（DINO ViT-B/16 + Slot Attention）提取物体中心化槽表征，与密集DINO全局特征和密集补丁网格基线对比。在ManiSkill3 PickCube-v1和StackCube-v1任务上，保持策略、目标token、渲染和校准固定，仅更换编码器。同时引入显式2D空间目标和原生分辨率渲染，并设计自动化运动学失败分类法（Near-Miss vs No-Grasp）。

Result: SPOT表征在PickCube-v1上达到55.0±2.9%成功率，比DINO全局特征基线（32.6±1.5%）高出22.4%。密集补丁网格（16倍token数）性能不优于全局特征。加入2D空间目标和原生分辨率渲染后，完整系统达到68.7±4.2%，接近3D oracle上限（71.7±4.1%）。失败分类显示空间接地减少了Near-Miss失败，但对No-Grasp无改善。该分类法可迁移至StackCube-v1，指出遮挡为主要瓶颈。

Conclusion: 物体中心化表征在机器人模仿学习中比全局特征和密集网格更有效，结构化的特征分组是关键，而非增加模型容量或token数量。空间接地有助于缓解空间精度类失败，但遮挡问题仍需进一步解决。

Classification reason: 该论文研究基于物体中心化表征（object-centric slot representations）的机器人模仿学习策略，属于具身智能中的视觉运动策略学习，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Object-centric representations for robotics are relevant to world models but lack direct connection to the user's priority subtopics like continuous language models or video generation. Signals: object-centric slot representation, visuomotor imitation learning, frozen encoder comparison, kinematic failure taxonomy, robotics manipulation
...
### [10] [Diffusion for Long-Horizon Multi-Robot Path Planning in Human-Shared Environments](https://arxiv.org/abs/2607.09911)
...
TL;DR: 提出 MRRD 框架，结合滚动时域、并行扩散推理、冲突搜索和紧急度条件控制，实现实时、长时域的多机器人路径规划，在拥挤人机共享环境中支持 15 个机器人协同导航。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型生成路径的方法受限于固定时长和高计算延迟，难以适应不同目标距离和实时部署需求，且缺乏对多机器人协作和社交感知的联合建模。

Method: MRRD 框架包含四个核心组件：(1) 滚动时域方案，适应人类运动预测的有限视野；(2) 并行化扩散推理，实现可扩展的类人路径生成；(3) 基于冲突搜索的机制，解决机器人间碰撞；(4) 紧急度时序条件生成变速度路径，以及差异化引导项最大化社交感知与机器人间协调。

Result: 在拥挤环境中，MRRD 成功扩展到 15 个机器人实时运行，在安全性和任务成功率上显著优于现有基线方法。

Conclusion: MRRD 证明了扩散模型结合滚动时域和冲突解决机制可以有效实现大规模多机器人实时路径规划，为人机共享环境下的机器人导航提供了可行方案。

Classification reason: 该论文研究多机器人路径规划，属于机器人规划与控制范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics planning but low alignment with user's priority on continuous multimodal models and autoregressive generation. Signals: multi-robot path planning, diffusion model, human-shared environments, real-time navigation, conflict-based search
...
### [11] [Task Planning for Mobile Manipulation in Retail Stores using Foundation Models with Iterative Re-planning](https://arxiv.org/abs/2607.09962)
...
TL;DR: 本文提出了一种基于大语言模型和视觉语言模型的任务规划方法，用于零售超市场景中的移动操作补货任务，在PyBullet仿真环境中验证了端到端的拾取-放置系统，并采用基于反馈的迭代重规划策略进行错误纠正。


<details>
  <summary>Details</summary>
Motivation: 零售、仓储和物流行业的自动化需求日益增长，但此前的工作多集中在结构化环境中的后台操作（如打包和分拣）。零售商店货架等变化性大、以人为中心的环境对自动化提出了更高挑战。本文旨在利用基础模型（LLM/VLM）的推理能力，解决零售场景中的补货任务规划问题。

Method: 提出一个端到端的任务规划系统，使用LLM和VLM进行高层任务分解与规划，结合用户驱动的提示词和基于反馈的迭代重规划机制进行错误纠正。系统部署在定制的全向移动操作平台上，在PyBullet仿真环境中验证拾取-放置任务。

Result: 在PyBullet仿真环境中成功验证了端到端系统的有效性，展示了基于LLM/VLM的任务规划与迭代重规划在零售补货场景中的可行性。

Conclusion: 本文证明了利用基础模型（LLM/VLM）结合迭代重规划可以有效地解决零售商店等非结构化环境中的移动操作任务规划问题，为零售自动化提供了新的思路。

Classification reason: 该论文使用LLM和VLM进行零售场景中的移动操作任务规划，包含迭代重规划的错误纠正机制，属于具身智能中的任务规划与控制，因此映射到[CANONICAL]具身世界模型与规划控制。

Importance reason: Focuses on retail restocking with LLM/VLM planning, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: LLM task planning, mobile manipulation, retail restocking, iterative re-planning, PyBullet simulation
...
### [12] [TAC-LOCO: Unified Whole-Body Control for Quadrupedal TACtile-Informed LOCO-Manipulation](https://arxiv.org/abs/2607.10132)
...
TL;DR: TAC-LOCO 提出一种触觉增强的统一强化学习框架，将触觉阵列观测编码为紧凑潜表征，与本体感知联合用于四足机器人的腿、臂、夹爪全身协调控制，在零样本部署中实现动态触觉感知的移动操作，抓取力降低47%，物体掉落率低于1%。


<details>
  <summary>Details</summary>
Motivation: 现有四足机器人动态移动操作中缺乏触觉反馈，通常采用紧抓策略而非根据交互力调节抓取力，导致在不确定外力下物体易滑落或抓取力过大。触觉感知在动态全身控制中的作用尚未被充分探索。

Method: 提出 TAC-LOCO 框架：1) 将触觉阵列观测通过编码器压缩为紧凑潜表征；2) 将触觉潜表征与本体感知联合作为策略输入；3) 设计抓取稳定性奖励，使策略同时跟踪身体速度和末端执行器轨迹、调节抓取力、防止物体滑落；4) 在仿真中训练后零样本部署到真实 Unitree Go2 机器人。

Result: 在真实机器人上实现动态触觉感知的移动操作，抓取力降低47%，物体掉落率低于1%，在渐变负载变化和突然释放事件下均能保持稳定。

Conclusion: 触觉反馈在动态全身控制中具有显著价值，能够有效平衡抓取力与操作稳定性，为四足机器人动态移动操作提供了新的解决方案。

Classification reason: 该论文提出触觉增强的统一强化学习框架用于四足机器人全身控制（腿、臂、夹爪协调），属于具身智能中的机器人控制与规划，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on robotics control with tactile sensing, not on the user's priority subtopics like continuous language models or video generation. Signals: Robotics control, Tactile sensing, Quadrupedal locomotion, Not in priority subtopics, No boost keywords matched
...
### [13] [ActiveFly-Bench: Aligning Embodied Question Answering with Vision-Language-Action for Aerial Embodied Perception](https://arxiv.org/abs/2607.10180)
...
TL;DR: ActiveFly-Bench 是首个连接虚拟推理与物理交互的无人机具身感知基准，包含三个层次化任务（空中具身问答、观察行为规划、细粒度语言控制），并开发了闭环无人机智能体 ActiveFly，实验表明当前 VLM/VLA 模型在行为规划与视角调整方面仍存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有无人机感知基准多聚焦于被动感知任务（如目标检测、跟踪），缺乏对主动感知中高层任务理解、行为规划与底层控制之间闭环连接的评估。亟需一个能够系统评估无人机在真实与仿真环境中主动感知能力的基准。

Method: 将主动感知分解为三个层次化任务：Air-EQA（空中具身问答）、OBP（观察行为规划）、FLUC（细粒度语言引导无人机控制）。从真实与仿真户外环境收集数据构建数据集。开发 ActiveFly 闭环智能体，集成视觉语言推理与细粒度控制，并在物理无人机平台上部署验证。

Result: 代表性 VLM 和 VLA 模型在 ActiveFly-Bench 上的表现表明，当前无人机智能体在行为规划、视角调整和鲁棒任务完成方面仍存在显著困难，该基准可作为空中具身智能的新测试平台。

Conclusion: ActiveFly-Bench 有效填补了无人机主动感知评估的空白，揭示了现有模型在行为规划与闭环控制方面的不足，为未来空中具身智能研究提供了标准化评测平台。

Classification reason: 该论文提出无人机具身感知基准与闭环智能体，涉及具身问答、行为规划与底层控制，属于具身智能与机器人规划控制范畴，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Focuses on UAV embodied perception, which is outside the user's priority subtopics and lacks strong alignment with continuous language/multimodal models or video generation. Signals: Embodied QA, UAV benchmark, vision-language-action, world model (tangential), no continuous token or autoregressive generation
...
### [14] [Source-Lifted Flow Matching for Intervenable Multimodal Imitation](https://arxiv.org/abs/2607.10206)
...
TL;DR: 本文提出 Source-Lifted Flow Matching (SL-FM)，一种可干预的流匹配模仿学习策略。通过正交源提升机制，将源端点的随机性转化为可操作的干预变量，使用户能在不改变速度场的情况下选择不同的行为模式，解决了流匹配策略中随机性被动、无法直接选择有效延续行为的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的流匹配策略虽然能建模复杂的多模态动作分布，但其随机性是被动的——重复采样可能产生多样行为，但用户无法从同一状态直接选择有效的延续行为。本文旨在将这种被动随机性转化为可操作的干预变量，使用户能够主动选择不同的行为模式。

Method: 提出 Source-Lifted Flow Matching (SL-FM)，核心是正交源提升（Orthogonal Source Lifting）机制。该方法将可干预的源端点提升到辅助正交坐标空间中，而目标动作保持在原始动作子空间，从而避免路径交叉歧义。同时学习状态相关的源混合分布，并使用责任下限（responsibility floor）为每个干预句柄提供弱监督，防止死模式。

Result: 在交叉流诊断和机器人控制基准实验上，SL-FM 将被动源随机性转化为可操作的干预变量。它消除了交叉引起的复合轨迹，在匹配前缀干预中 91.1% 成功改变未来路径，并在多个基准设置中取得了强自由部署性能。

Conclusion: 源几何结构提供了可操作的多模态控制，无需根据所选模式对速度场进行条件化。SL-FM 在保持标准流匹配公式的同时，实现了对多模态行为的选择性干预，为模仿学习中的可控策略学习提供了新思路。

Classification reason: 该论文提出 Source-Lifted Flow Matching (SL-FM)，一种用于模仿学习的可干预流匹配策略，核心是机器人动作策略学习与多模态动作分布建模，属于具身智能中的策略学习与规划控制，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Novel flow-matching intervention method for imitation learning, but only tangentially touches user's priority on continuous language/multimodal models. Signals: flow matching, imitation learning, multimodal action distributions, source intervention, orthogonal source lifting
...
### [15] [PrismAD: Decoupled Planning via Semantic Mixture-of-Planners for End-to-End Autonomous Driving](https://arxiv.org/abs/2607.10336)
...
TL;DR: PrismAD 提出一种基于语义混合专家（Semantic Mixture-of-Planners）的解耦端到端自动驾驶规划框架，将场景 token 划分为交互、几何和意图三组，分别由独立专家建模，并通过语义感知路由器自适应聚合，在 nuScenes 和 NeuroNCAP 上取得有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有端到端规划器将异构场景 token 聚合到耦合的表征空间中，迫使单个规划分支同时建模智能体交互、道路几何和驾驶意图，削弱了各因素的特化推理能力，且难以区分不同规划线索的贡献。

Method: 将场景 token 划分为交互、几何和意图三组，分别分配给三个结构相同但参数独立的规划专家（Expert），每个专家学习特化的运动规划表征；设计语义感知路由器（Semantics-aware Router），为运动预测和自车规划分别生成独立的聚合权重；引入稀疏 top-K 激活与噪声门控（Noisy Gating）提升路由鲁棒性并减少不必要的专家计算。

Result: 在 nuScenes 开放循环数据集和 NeuroNCAP 闭环基准上，PrismAD 展现出有竞争力的性能，优于或持平现有端到端规划方法。

Conclusion: 解耦的语义混合专家架构能够有效提升端到端自动驾驶规划中各因素的特化推理能力，且稀疏激活机制在保持性能的同时降低了计算开销。

Classification reason: 该论文提出端到端自动驾驶规划框架，属于机器人/具身智能中的规划控制任务，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: This paper focuses on autonomous driving planning, which is outside the user's priority subtopics and lacks the specified boost keywords. Signals: Autonomous driving, Mixture-of-Experts, Decoupled planning, nuScenes, End-to-end
...
### [16] [VINE: Taming Generative Control Policies for Reinforcement Learning](https://arxiv.org/abs/2607.10369)
...
TL;DR: VINE提出了一种面向强化学习的流匹配策略采样方法，通过在每个去噪步骤重建新的插值状态来稳定端到端值梯度优化，在OGBench离线RL基准和真实机器人操作任务上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 流匹配策略在机器人学习中表现出色，但将其与值梯度强化学习结合时存在训练不稳定的问题。先前工作认为不稳定性源于迭代生成过程，因此牺牲了迭代生成、高表达能力或值梯度优化中的某一方面。本文发现不稳定性实际上源于原始的行为克隆采样策略，而非迭代生成本身。

Method: 提出VINE（RL-oriented sampling method），核心思想是在每个去噪步骤中不沿单一流轨迹前进，而是重建一个新的插值状态，从而创建稳定的可微路径用于值梯度传播，同时保持与原始流匹配去噪过程的兼容性。该方法支持端到端反向传播通过所有10个去噪步骤。

Result: 在OGBench离线RL基准测试中，VINE一致优于现有SOTA强化学习方法；在真实机器人操作任务上也取得了优异表现。实验证明VINE能够实现稳定的策略改进，同时保留了流匹配的表达能力和迭代生成特性。

Conclusion: VINE证明了流匹配策略的迭代生成本身并非值梯度RL不稳定的根源，通过设计RL导向的采样策略可以同时实现端到端值梯度优化和流匹配的高表达能力，为机器人学习中的策略优化提供了新思路。

Classification reason: 该论文提出VINE方法，用于流匹配策略（flow-matching policies）的强化学习优化，核心是机器人动作生成与控制策略，属于具身智能/机器人领域的策略学习与规划控制，因此映射到[具身世界模型与规划控制]（embodied_world_model）这一规范子主题。

Importance reason: Strong RL+flow-matching contribution but outside user's core priority subtopics. Signals: flow-matching policy, reinforcement learning, value-gradient optimization, robotic manipulation, offline RL benchmark
...
### [17] [SUREFlow: State-space Uncertainty-aware REsidual Flow Matching for Robust Robot Manipulation](https://arxiv.org/abs/2607.10504)
...
TL;DR: SUREFlow 提出一种基于状态空间模型（Mamba）的不确定性感知残差流匹配框架，用于鲁棒的机器人操作。该方法联合预测动作速度和输入依赖的残差不确定性，在 LIBERO 基准上达到 92.5% 的成功率，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的生成式视觉-语言-动作策略在噪声、部分可观测性和随机初始条件下表现不稳定，长时间 rollout 中速度误差累积导致执行可靠性下降。扩散和流策略通常假设同方差残差，缺乏显式的不确定性建模，限制了迭代 rollout 的鲁棒性。

Method: SUREFlow 基于 Mamba 骨干网络构建状态空间不确定性感知残差流匹配框架。该方法联合预测动作速度和输入依赖的残差不确定性，在无需环境反馈的情况下选择性修正不可靠的动作维度，同时保持计算效率。

Result: 在 LIBERO 基准上达到 92.5% 的平均成功率，比基于 Mamba 的 MaIL 高出 34.2%。在 LIBERO-PRO 上仅用 179M 参数达到约 49% 的成功率，性能可与 3-7B 参数的大规模 VLA 模型相媲美。

Conclusion: SUREFlow 通过显式建模动作残差不确定性，有效提升了机器人操作策略在噪声和部分可观测条件下的鲁棒性，为生成式策略在具身智能中的可靠部署提供了新思路。

Classification reason: 该论文提出基于流匹配的机器人操作策略，属于具身智能中的动作规划与控制，与[CANONICAL]具身世界模型与规划控制高度匹配。

Importance reason: Focuses on robot manipulation with Mamba and flow matching, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: flow matching for robotics, Mamba backbone, uncertainty-aware action generation, LIBERO benchmark, 92.5% success rate

Abstract: Generative vision-language-action policies have advanced robot manipulation, but they often exhibit instability under noise, partial observability, and stochastic initial conditions. During extended rollouts, small velocity errors accumulate, degrading execution reliability. Existing diffusion and flow-based policies typically assume homoscedastic residuals and lack explicit uncertainty modeling within action generation, limiting robustness during iterative rollout. We propose SUREFlow, a state-space uncertainty-aware residual flow matching framework built on a Mamba backbone. The method jointly predicts action velocities and input-dependent residual uncertainty, enabling selective refinement of unreliable action dimensions without environment feedback while preserving computational efficiency. On LIBERO, SUREFlow achieves 92.5% average success rate (SR), outperforming the Mamba-based MaIL by 34.2%. On LIBERO-PRO, it attains around 49% SR using only 179M parameters, achieving performance comparable to large VLAs with 3-7B parameters. SUREFlow source code is available on:

</details>


### [18] [BucketKD: A Safety-Aware Bucket-Based Knowledge Distillation Framework for End-to-End Motion Planning](https://arxiv.org/abs/2607.10565)
...
Importance: 值得关注 (71.59) - 未入选精读

TL;DR: 提出 BucketKD，一种基于桶（bucket）的安全感知知识蒸馏框架，用于端到端自动驾驶运动规划。通过将环境变量离散化为自适应桶来丰富语义，并设计基于碰撞时间（TTC）的安全感知路点注意力机制，使学生模型在压缩的同时保留安全关键行为。在 CARLA/Bench2Drive 上显著优于 SOTA。


<details>
  <summary>Details</summary>
Motivation: 端到端运动规划模型参数量大，难以部署在资源受限平台。现有知识蒸馏方法使用简化的规划状态表示，丢失了丰富的场景语义，且缺乏对安全关键行为的显式建模。

Method: 1) 桶式知识蒸馏（Bucket-based KD）：将关键环境变量（如障碍物距离、速度等）离散化为自适应桶，捕获更丰富的场景语义。2) 安全感知路点注意力机制：结合障碍物接近度和相对运动，通过 TTC（碰撞时间）公式评估每个路点的风险等级，使学生模型在蒸馏过程中更好地保留安全关键行为。

Result: 在 CARLA 模拟器中使用 Bench2Drive 数据集进行实验，BuckeKD 在规划准确性和安全性方面显著优于 SOTA 方法，同时保持强压缩比。

Conclusion: BuckeKD 通过桶式离散化和安全感知注意力机制，有效解决了端到端运动规划模型压缩中的语义丰富性和安全性保留问题，为自动驾驶中的轻量化安全规划提供了可行方案。

Classification reason: 该论文研究端到端运动规划（motion planning）在自动驾驶中的应用，属于具身智能/机器人规划控制范畴，与 canonical 子主题"具身世界模型与规划控制"高度匹配。

Importance reason: Focuses on autonomous driving motion planning, which is not a priority subtopic and lacks direct connection to user's core interests in continuous multimodal models or video generation. Signals: Knowledge distillation for motion planning, Safety-aware via TTC, CARLA/Bench2Drive evaluation, Not in priority subtopics, No boost keywords matched
...
### [19] [Dual-Process Atomic Skill Learning: Decoupling Semantic Reasoning and Real-Time Control](https://arxiv.org/abs/2607.10625)
...
TL;DR: DASL 提出一种异步分层模仿学习框架，通过慢频策略（VQ 离散技能推理）和快频策略（潜扩散+决策 Transformer 生成动作）的解耦，缓解了联合训练中技能码本干扰问题，在仿真和真实实验中显著提升了多步组合任务的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 语言条件模仿学习在泛化到多步组合任务时面临挑战，现有分层方法在联合训练中常因高层技能推理与低层动作生成紧密耦合而导致训练不稳定和码本坍塌问题。

Method: DASL 受双过程认知理论启发，采用异步分层架构：Slow-Frequency Policy 通过向量量化（VQ）预测可解释的离散技能；High-Frequency Policy 利用潜扩散模型和 Decision Transformer 以这些潜技能为条件生成精确动作。两个模块异步协调，扩散模型用于结构化潜空间，缓解码本干扰。

Result: 在仿真基准和真实机器人实验中，DASL 显著优于现有 SOTA 基线，在技能获取和对未见指令的组合泛化方面表现突出。

Conclusion: 解耦高层语义推理与低层运动控制的异步分层框架能有效提升语言条件模仿学习的组合泛化能力，为机器人技能学习提供了新的范式。

Classification reason: 该论文提出面向机器人语言条件模仿学习的异步分层框架，将高层语义推理与低层运动控制解耦，属于具身智能中的动作规划与控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Decouples semantic reasoning and real-time control for robot learning, but is in robotics (cs.RO) and lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: Hierarchical imitation learning, Dual-process theory, VQ discrete skill reasoning, Latent diffusion + Decision Transformer, Compositional generalization

Abstract: Language-conditioned Imitation Learning (IL) is essential for enabling robots to perform complex tasks following natural language instructions. However, generalizing to multi-step compositional tasks remains a significant challenge. While hierarchical approaches attempt to address this by decomposing tasks into atomic skills, existing methods often suffer from training instability and codebook collapse due to the tight coupling between high-level skill reasoning and low-level action generation in joint training paradigms. Inspired by the Dual-Process Theory of cognition, we propose Dual-Process Atomic Skill Learning (DASL), a novel asynchronous hierarchical imitation learning framework that decouples slow semantic reasoning from fast, real-time motion control. DASL comprises a Slow-Frequency Policy that predicts interpretable, discrete skills via Vector Quantization, and a High-Frequency Policy that leverages a latent diffusion model and a Decision Transformer to generate precise actions conditioned on these latent skills. By asynchronously coordinating these modules and utilizing diffusion to structure the latent space, our framework mitigates the skill codebook interference problem common in joint training paradigms. Evaluations across simulation benchmarks and experiment demonstrate that DASL significantly outperforms state-of-the-art baselines, excelling in skill acquisition and compositional generalization to unseen instructions. GitHub page:

</details>


### [20] [World Models as Adversaries: Multi-Agent Self-Play Fine-Tuning for Robust Motion Planning](https://arxiv.org/abs/2607.10630)
...
TL;DR: 本文提出 AWM（对抗世界建模），一种基于多智能体自博弈微调的框架，将世界模型转化为角色条件化的对抗者，通过约束极小-极大博弈实现自动驾驶运动规划的鲁棒性训练，在 nuPlan 和 InterPlan 基准上取得了竞争性的闭环性能。


<details>
  <summary>Details</summary>
Motivation: 密集交通中的鲁棒运动规划需要自动驾驶车辆能够应对罕见且安全关键的交互场景，而这些场景在自然驾驶数据中代表性不足。现有对抗训练方法依赖外部场景生成器、启发式扰动或大量仿真回滚，难以与现代自回归规划器集成。

Method: 将对抗鲁棒规划器学习建模为约束极小-极大博弈，提出 AWM 框架。内层极小化中，将规划器的预测世界模型转换为角色条件化的对抗者，通过反事实信用分配学习稀疏的场景自适应攻击联盟；外层极大化中，自车规划器利用尾风险加权和参考锚定信任区域优化对冻结 AWM 的遗憾感知鲁棒最优响应，在保持正常驾驶行为的同时改善困难场景恢复。

Result: 在 nuPlan 和 InterPlan 基准上的实验表明，该方法能生成可迁移的对抗交互，并在正常和高度交互的长尾场景中均取得具有竞争力的闭环性能。

Conclusion: AWM 提供了一种理论上可解释的多智能体自博弈微调框架，有效提升了自动驾驶规划器在长尾安全关键场景中的鲁棒性，且无需外部场景生成器或大量仿真回滚。

Classification reason: 该论文提出 Adversarial World Modeling (AWM)，将世界模型作为对抗者用于自动驾驶运动规划的鲁棒性训练，属于具身智能中机器人规划与控制的范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Directly advances world models for robust planning via adversarial self-play, aligning with high-priority world_model direction and video generation world model subtopic. Signals: world model as adversary, multi-agent self-play fine-tuning, constrained min-max game, robust motion planning, nuPlan benchmark
...
### [21] [D-SafeMPC: Diffusion-Driven Safe Model Predictive Control with Discrete-Time Control Barrier Functions](https://arxiv.org/abs/2607.10842)
...
TL;DR: D-SafeMPC提出一种扩散驱动的安全模型预测控制方法，通过控制障碍函数(CBF)和控制李雅普诺夫函数(CLF)引导扩散逆过程，并结合迭代投影方案，使MPC在每个去噪步骤中细化轨迹，从而生成安全、可行且目标导向的机器人运动规划。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在机器人规划中缺乏对安全性和动力学约束的固有保证，生成的轨迹常不可行或不安全。现有的混合方法（扩散+MPC）因扩散模型提供的轨迹初始化质量差，导致MPC无法收敛到安全可行解。

Method: 提出D-SafeMPC框架：(1) 在扩散逆过程中引入CBF和CLF作为引导信号，将采样导向安全且目标导向的区域；(2) 采用迭代投影方案，在每个去噪步骤中由MPC细化轨迹，为后续步骤提供可靠的MPC热启动；(3) 在Franka机械臂上进行了四个仿真场景（一个静态障碍物、三个动态障碍物）和一个sim-to-real实物实验验证。

Result: 在Franka机械臂的仿真和实物实验中，D-SafeMPC在安全性、任务成功率和规划效率上均优于现有基线方法。

Conclusion: 通过将CBF/CLF引导融入扩散逆过程并与MPC迭代协作，D-SafeMPC有效解决了扩散模型在机器人安全规划中的约束问题，为安全关键型机器人应用提供了可行方案。

Classification reason: 该论文提出D-SafeMPC方法，将扩散模型与模型预测控制(MPC)结合，利用控制障碍函数(CBF)和控制李雅普诺夫函数(CLF)引导扩散逆过程，实现机器人安全规划与控制。这属于具身智能中的机器人规划与控制，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Applies diffusion models to robotics control, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: diffusion model, model predictive control, control barrier functions, robotics planning, safety constraints
...
### [22] [A Single Diffusion-Policy Controller for Multi-Task Block Pushing with Zero-Shot Sim-to-Real Transfer](https://arxiv.org/abs/2607.10892)
...
TL;DR: 本文提出一种基于强化学习从零训练扩散策略的框架，用于多任务机器人推块操作。通过重加权证据下界作为策略损失函数，结合反向课程生成和目标中心化表征，在稀疏奖励仿真环境中学习多任务策略，并实现零样本仿真到真实世界的迁移。


<details>
  <summary>Details</summary>
Motivation: 现有扩散策略主要依赖行为克隆（BC）从专家演示中学习，但获取高质量演示成本高昂且难以覆盖多样任务。本文希望探索无需演示、仅通过强化学习从零训练扩散策略的可行性，以实现多任务机器人操作策略的自主学习与仿真到真实世界的迁移。

Method: 提出一个框架，核心是使用重加权证据下界（reweighted ELBO）作为策略损失函数，该损失可无缝嵌入RL算法中作为策略学习模块。为应对无演示下的探索挑战，引入反向课程生成（reverse curriculum generation）和目标中心化表征（objective-centric representations）。结合扩散策略的表达能力，在稀疏奖励仿真环境中训练多任务推块策略。

Result: 在仿真环境中成功训练出多任务推块策略，并在真实世界中零样本迁移成功，能够适应不同的目标位置、块形状、块重量和表面摩擦系数等环境变化。

Conclusion: 本文证明了扩散策略可以通过强化学习从零训练，无需专家演示即可学习多任务机器人操作策略，并且具备零样本仿真到真实世界迁移的能力，为机器人策略学习提供了一种新的范式。

Classification reason: 该论文使用扩散策略（diffusion policy）结合强化学习训练多任务机器人推块操作策略，并实现零样本仿真到真实世界的迁移，属于机器人具身智能中的动作规划与控制，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Focuses on robot control and sim-to-real transfer, which is peripheral to the user's core interests in continuous multimodal models and video generation. Signals: diffusion policy, reinforcement learning, multi-task manipulation, zero-shot sim-to-real, robotics
...
### [23] [Think When It Matters: Conditional VLM Reasoning for Social Navigation with RL Policies](https://arxiv.org/abs/2607.10991)
...
TL;DR: 本文提出HUMA混合架构，将反应式RL导航策略与条件触发的VLM语义推理相结合，在社交机器人导航中动态平衡计算效率与深度语义理解，在Social-MP3D和Social-HM3D基准上分别提升20%和3%的任务成功率，并显著减少个人空间侵犯和碰撞。


<details>
  <summary>Details</summary>
Motivation: 现有社交机器人导航方法中，RL策略推理快但缺乏灵活语义推理能力，难以泛化到复杂社交场景；而VLM虽能提供深度语义理解，但计算成本高、推理慢，难以实时部署。作者希望结合两者优势，实现高效且语义智能的社交导航。

Method: 提出HUMA混合架构：使用反应式RL策略处理低密度常规导航任务；当人类进入机器人近距敏感区域时，条件触发一个后训练的高层VLM进行语义推理，并以此调节RL策略的行为。在Social-MP3D和Social-HM3D基准上评估，并在Mirokaï移动机器人上进行了真实世界部署验证。

Result: 在Social-MP3D和Social-HM3D基准上，任务成功率分别提升20%和3%，个人空间侵犯和人类碰撞显著减少。消融实验验证了各架构组件的有效性，真实世界部署进一步证明了方法的实用性。

Conclusion: HUMA混合架构有效结合了RL的高效推理与VLM的深度语义理解，在社交导航中实现了性能与效率的平衡，为机器人实时社交导航提供了一种实用的解决方案。

Classification reason: 论文提出HUMA混合架构，将RL导航策略与VLM语义推理结合用于社交机器人导航，属于具身智能中的机器人规划与控制，因此映射到[具身世界模型与规划控制]（CANONICAL）。同时涉及VLM用于社交场景理解，也匹配多模态理解与生成方向。

Importance reason: Combines RL and VLM for efficient social navigation, but is not directly aligned with your priority subtopics on continuous models or generation. Signals: social navigation, hybrid RL-VLM, conditional reasoning, real-world deployment, world model application
...
### [24] [Affordance-Based Manipulation Planning with Text Goals and Sim-to-Real Generalisation via Real-to-Sim Image Conversion](https://arxiv.org/abs/2607.11004)
...
TL;DR: 本文提出一个基于 affordance 识别和动作效果预测的操作规划系统，通过视觉形式的未来推理和文本目标匹配进行候选规划评估，并引入图像转换模块实现从真实世界图像到统一视觉外观的转换，以促进 sim-to-real 泛化。系统在仿真和真实硬件上均展示了操作规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有操作规划系统在物体被遮挡或初始描述符无法识别未来状态中的物体时难以生成有效动作计划，且 sim-to-real 迁移面临物体形状和外观多样性的挑战。

Method: 系统包含三个核心模块：(1) affordance 识别与动作效果预测模块，通过视觉形式推理可能的未来状态；(2) 多模态目标匹配模块，将预测结果与运行时设置的文本目标进行匹配评估候选规划；(3) 图像转换模块，将真实世界图像中形状和外观各异的物体转换为一致的视觉外观，便于在物理机器人上实现操作规划。

Result: 在仿真和真实硬件上对一系列具有挑战性的任务进行了评估，展示了系统的操作规划能力，并对各模块进行了独立性能测试。

Conclusion: 该系统通过 affordance 识别、视觉未来推理和文本目标匹配实现了有效的操作规划，图像转换模块有助于 sim-to-real 泛化，为机器人操作规划提供了一种可行的方案。

Classification reason: 该论文提出基于 affordance 识别和动作效果预测的机器人操作规划系统，通过视觉形式的未来推理和文本目标匹配进行规划，并包含 sim-to-real 泛化模块。这属于面向机器人操作规划与控制的世界模型，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on affordance-based manipulation planning with sim-to-real transfer, which is peripheral to the user's core interests in continuous multimodal models and autoregressive generation. Signals: Affordance-based planning, Sim-to-real via image conversion, Text goal matching, Robotic manipulation, Not aligned with priority subtopics
...
### [25] [Whole-Body Semantic-to-Actuation Grounding of Elephant-Inspired Soft-Trunk Motion via Lightweight Flow Matching](https://arxiv.org/abs/2607.11018)
...
TL;DR: 提出一种基于轻量级流匹配的全身语义到动作映射框架，将多模态大语言模型的响应转化为象鼻软体机器人的可行肌腱驱动轨迹，在物理HRI实验中显著提升用户满意度。


<details>
  <summary>Details</summary>
Motivation: 软体象鼻机器人在人机交互中需要全身表达，但端效应器运动无法描述整体形态，而直接全身控制维度高且难以保证可行性，现有方法缺乏从语义到全身动作的有效映射。

Method: 将MLLM响应转化为有界、形态对齐的意图-强度元组，用Catmull-Rom样条参数化肌腱驱动轨迹，通过整流流生成器（rectified-flow generator）采样可行全身运动。

Result: 在保留测试中，正确率从25.0%（密集回归基线）提升至77.2%；相比去噪扩散基线，正确率从71.9%提升至77.2%，推理时间从7.86ms降至4.87ms；100人物理HRI实验显示，增加软体躯干运动通道后，总体满意度从46%提升至82%。

Conclusion: 轻量级流匹配能有效实现软体象鼻机器人的全身语义到动作映射，在正确性、效率和多样性上均优于基线方法，显著增强人机交互体验。

Classification reason: 论文提出面向象鼻软体机器人的全身语义到动作映射框架，使用轻量级流匹配生成可行全身运动，属于机器人具身智能中的动作规划与控制，与[具身世界模型与规划控制]高度匹配。

Importance reason: Focuses on soft robotics and HRI, not on core priority subtopics like continuous language models or video generation. Signals: flow matching for motion generation, soft robotics HRI, lightweight inference, multimodal LLM grounding, world model application
...
### [26] [Learning to Navigate Efficiently with Only 0.58M Trainable Parameters](https://arxiv.org/abs/2607.11029)
...
TL;DR: 本文提出一种分解式视觉导航模型，将几何、占据、坐标变换等已知结构解析计算，仅训练0.58M参数（共22.7M）和44k帧数据（不到1 GPU小时），在6060个点目标导航episode上接近SOTA性能，碰撞率最低，推理速度50Hz，且可迁移至无目标探索任务。


<details>
  <summary>Details</summary>
Motivation: 当前视觉导航方法严重依赖大规模数据和海量参数（数亿参数、数十亿帧），本文质疑单个任务族是否真的需要如此大的规模，并探索用已知结构（投影几何、占据网格、坐标变换）替代部分学习负担，从而大幅降低可训练参数和数据需求。

Method: 将导航模型分解为三个小型可学习模块：(1) 出口预测器（egress predictor），将episode目标锚定到当前视图中的局部子目标；(2) 导航预测器（navigation predictor），估计目标条件化的轨迹后验分布；(3) 端点锚定残差扩散生成器（endpoint-pinned residual diffusion generator），从该后验中采样轨迹形状。几何、占据、坐标变换等操作以解析方式计算，作为模块间的接口。仅0.58M参数可训练，其余22.12M参数固定。

Result: 在6060个点目标导航episode和60个环境上，以233倍更少的可训练参数接近SOTA性能；在所有评估方法中碰撞率最低；推理速度达50Hz；仅需44k帧数据、不到1 GPU小时训练；可迁移至无目标探索任务（仅重训练123k参数的出口头）；在传感器损坏时具有透明且可解析修正的失败模式。

Conclusion: 通过将已知结构解析计算与小型可学习模块相结合，可以在极低参数和数据量下实现接近SOTA的视觉导航性能，挑战了"更大规模必然更好"的假设，并为高效、可解释的具身导航提供了新范式。

Classification reason: 该论文提出一种分解式视觉导航模型，将投影几何、占据网格、坐标变换等已知结构进行解析计算，仅训练三个小型学习模块（出口预测器、导航预测器、端点锚定残差扩散生成器），在极少量参数（0.58M可训练参数）和极低数据量（44k帧）下接近SOTA导航性能。这属于具身智能中的视觉导航与规划控制问题，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on efficient navigation with minimal parameters, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: Efficient navigation, 0.58M trainable parameters, 50Hz inference, decomposed model, low collision rate
...
### [27] [VIA: Visual Interface Agent for Robot Control](https://arxiv.org/abs/2607.11119)
...
TL;DR: VIA 是一个将机器人操控重新定义为智能体任务的框架：通过浏览器 3D 界面，通用基础模型驱动的智能体通过截图、发出指令、观察结果并调整来操控机械臂，无需机器人数据微调，在桌面操作任务上零样本达到 96.7% 的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有将基础模型转化为视觉-语言-动作（VLA）模型的方法受限于微调数据和计算资源，导致模型规模远小于前沿基础模型，限制了其通用能力。作者希望探索是否可以直接利用基础模型已有的通过视觉界面操作软件的能力来控制机器人。

Method: VIA 将机器人控制重构为智能体任务：使用现成的基础模型（如 Claude Code、Codex）作为智能体，通过浏览器中的 3D 界面（基于 Three.js 构建）操控机器人。智能体通过截图感知环境，使用一组通用工具（如移动、抓取、旋转等命令）进行操作，观察结果并闭环调整，无需任何机器人专用微调或特权状态信息。

Result: 在 LIBERO-Goal 的三个任务上达到 96.7% 的成功率，在长时域彩虹组装任务上达到 100% 的成功率。性能随基础模型规模增强而提升。与现有 VLA 方法相比，VIA 在零样本设置下表现出色，并具备闭环错误恢复和重新规划能力。

Conclusion: 前沿智能体已经具备可直接迁移到机器人控制的技能，只需提供合适的接口。这表明"你的编程或计算机使用智能体，在某种意义上，也是一个秘密的机器人控制智能体"。

Classification reason: 该论文提出 VIA 框架，利用通用基础模型通过浏览器 3D 界面进行机器人操控，属于具身智能中的机器人控制与规划，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Novel agentic interface for robot control but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: robot control via browser interface, zero-shot manipulation, foundation model agent, no robot-specific fine-tuning, closed-loop error recovery
...
### [28] [Pix2Act: Image-Space Manipulation Policies with Equivariant Augmentation](https://arxiv.org/abs/2607.11167)
...
TL;DR: Pix2Act 提出将 3D 操作策略表示为相机平面中的 2D 关键点轨迹，通过三角化无损恢复末端执行器位姿，并利用等变数据增强提升泛化能力，在模拟和真实操作任务中均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 将操作动作表示为相机平面中的 2D 轨迹是一种紧凑且可解释的方式，但面临轨迹出框和精度有限等挑战。Pix2Act 旨在通过生成连续图像空间关键点轨迹并利用三角化恢复位姿来解决这些问题，同时利用等变增强提升泛化性。

Method: Pix2Act 将高维 3D 控制重新表述为更简单的 2D 预测问题：在每个相机平面中生成连续的图像空间关键点轨迹，并通过三角化无损恢复末端执行器位姿。该方法将观测和动作对齐到同一坐标空间，使得等变变换可以联合旋转相机图像及其对应的图像空间动作。网络架构设计支持融合多视角信息，同时保持各视角旋转的等变性。

Result: 在多种模拟和真实世界操作任务中，Pix2Act 在性能上优于现有最先进的基线方法，并且在相机扰动下保持鲁棒性。等变增强有效扩大了数据分布的支持范围，学习了跨变换的不变动作结构。

Conclusion: Pix2Act 通过图像空间策略表示与等变增强，有效解决了 3D 操作策略学习中的泛化与精度问题，为机器人模仿学习提供了一种简洁而有效的范式。

Classification reason: 该论文提出Pix2Act，一种基于图像空间关键点轨迹的模仿学习方法，用于机器人操作策略学习，属于具身智能中的动作规划与控制，与[具身世界模型与规划控制]高度匹配。

Importance reason: Strong robotics manipulation paper but outside user's priority subtopics and negative keywords. Signals: 2D keypoint trajectory policy, equivariant augmentation, camera-plane action representation, triangulation for 3D pose, robust to camera perturbations
...
### [29] [Towards Predictive, Aligned, and Scalable Robot Learning](https://arxiv.org/abs/2607.11270)
...
TL;DR: Lumo-2 是一个潜空间世界-动作模型，通过多阶段模态预对齐策略将动作表征与潜世界动力学、视觉和语言逐步对齐，在潜空间中实现基于物理世界动力学的预测推理，在长时程和灵巧操作等复杂机器人任务上优于 VLA 和 WAM 基线。


<details>
  <summary>Details</summary>
Motivation: 现有基于重建的动作分词化目标会导致表征偏向低层信号保真度，使得重建质量与下游控制性能之间出现错配。作者假设动作生成质量由潜空间的几何结构决定，因此需要一种结构化对齐策略来提升潜空间质量。

Method: 提出 Lumo-2，包含：(1) 潜世界-动作模型，在潜空间中推理世界动力学并生成动作；(2) 多阶段模态预对齐策略，逐步将动作表征与潜世界动力学、视觉和语言对齐，强制跨模态一致性并诱导结构化潜空间；(3) 系统研究潜世界建模与模态对齐在缩放定律和分布外泛化中的作用。

Result: Lumo-2 在需要时序推理、物理理解或高控制复杂度的真实世界任务（包括长时程和灵巧操作）上，一致优于强 VLA 和 WAM 基线。

Conclusion: 结构化的多模态对齐与预测推理是推进具身智能的基本原理，潜空间几何结构对动作生成质量至关重要。

Classification reason: 论文提出 Lumo-2，一个潜空间世界-动作模型（latent world-action model），用于机器人学习中的预测推理与动作生成，核心是具身智能中的世界建模与规划控制，因此映射到 [CANONICAL] 具身世界模型与规划控制。同时涉及多模态对齐（视觉、语言、动作），也匹配多模态理解与生成方向。

Importance reason: Strongly aligns with world model and continuous latent representation priorities, with a novel multi-stage alignment strategy for robot learning. Signals: world model, latent space reasoning, multimodal alignment, continuous representation, robot learning
...
### [30] [A Glimpse into Long-term Physical Coexistence with Intelligent Robots](https://arxiv.org/abs/2607.11377)
...
TL;DR: 本文提出PHILIA，一个基于机器人网关抽象的多机器人智能体系统，通过解耦高层语义推理与低层机器人执行，支持多种用户界面、机器人本体和策略后端的即插即用集成，在Astribot S1机器人上验证了长时程、灵巧的家居服务任务。


<details>
  <summary>Details</summary>
Motivation: 长期物理共存需要机器人系统不仅具备强大的策略能力，还需支持多样化的用户界面、长时程记忆、跨机器人协调以及将人类意图转化为安全物理执行的能力，现有系统缺乏统一的架构设计。

Method: 提出PHILIA架构，核心是机器人网关抽象层，将OpenClaw的交互和工具生态与机器人本地运行时、感知、导航、语音和策略通过统一能力接口暴露，解耦低频高层语义推理与高频低层机器人执行。

Result: 在Astribot S1机器人上验证了架构有效性，展示了从简单整理到长时程灵巧服务任务（如打包背包、提起垃圾袋）的代表性用例，实现了基于智能体记忆和场景理解的机器人动作落地。

Conclusion: PHILIA通过机器人网关抽象实现了用户界面、机器人本体和策略后端的组合式集成，为长期人机物理共存提供了可扩展的系统架构，人机交互中上下文理解和人在回路确认对有效辅助至关重要。

Classification reason: 该论文提出PHILIA多机器人智能体系统，关注机器人与人类长期物理共存，涉及具身智能、机器人规划控制、人机交互等核心主题，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on robot system integration rather than core world model or continuous multimodal advances. Signals: multi-robot agent, robot gateway abstraction, long-horizon memory, human-robot interaction, not world model or continuous language model
...
TL;DR: 本文提出机器人中心点图（robot-centric pointmaps），将3D场景点坐标编码到机器人坐标系中，以解决VLA模型中相机视角与机器人动作坐标系不匹配的问题，在RoboCasa和真实机器人实验中显著提升了跨视角泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在相机坐标系中观察场景，但动作定义在机器人坐标系中。固定视角下这种不匹配问题不大，但当大规模数据集聚合来自不同相机设置的演示时，策略需要跨视角泛化，视角不匹配成为瓶颈。

Method: 提出robot-centric pointmaps，即像素存储场景点在机器人坐标系中3D坐标的图像。这种表示在提供机器人坐标系3D几何信息的同时，保留了预训练2D VLA所需的H×W密集网格结构，只需最小架构改动即可集成到现有VLA模型中。

Result: 在RoboCasa数据集上，pointmaps同时提升了pi0.5和SmolVLA的性能，优于代表性的相机视角和3D感知基线。在真实机器人实验中，当相机被移动到训练中未见过的位置时，pointmaps相比纯RGB策略的优势更加明显。

Conclusion: 机器人中心点图是一种简单有效的表示方法，能够弥合VLA模型中观测坐标系与动作坐标系之间的鸿沟，显著提升策略的跨视角泛化能力，且易于集成到现有VLA架构中。

Classification reason: 该论文提出机器人中心点图（robot-centric pointmaps），将3D场景坐标映射到机器人坐标系，用于视觉-语言-动作（VLA）模型，提升机器人策略的跨视角泛化能力。这属于具身智能中机器人动作规划与控制的核心问题，因此映射到[CANONICAL]具身世界模型与规划控制。

Importance reason: Strong robotics paper but outside user's priority subtopics and lacks direct connection to continuous multimodal models or video generation. Signals: Robot-centric pointmaps, VLA frame mismatch, cross-view generalization, 3D scene encoding, minimal architecture change
...
### [32] [SKooP: Symmetric Koopman Predictions for Faster and More Generalizable Legged Robot Locomotion with Reinforcement Learning](https://arxiv.org/abs/2607.11624)
...
TL;DR: SKooP结合Koopman算子理论与形态对称性，通过自编码器学习系统动力学的Koopman模型作为评论家的特权观测，并在演员-评论家网络中引入群等变性，显著加速四足机器人双足运动策略的强化学习收敛并提升奖励，同时策略可迁移至不同仿真环境。


<details>
  <summary>Details</summary>
Motivation: 强化学习算法在机器人应用中样本效率低下，现有方法多局限于低维基准系统，难以处理高维机器人的复杂非线性动力学。本文旨在通过融合物理先验（对称性与Koopman动力学模型）来提升策略学习的效率与泛化能力。

Method: 提出SKooP框架：1) 利用自编码器学习系统动力学的Koopman模型（将非线性动力学映射到线性潜空间）；2) 将Koopman预测作为评论家的特权观测，提供更平滑、信息更丰富的特征；3) 在演员、评论家、编码器和解码器网络中引入群对称性，构建高度等变的策略网络。

Result: 在四足机器人上多个具有挑战性的双足运动任务中，SKooP一致地减少了收敛时间并提高了学习奖励。学习到的策略可迁移至不同的仿真环境。深入分析了Koopman模型与对称策略各自对性能的影响。

Conclusion: SKooP通过结合Koopman动力学预测与形态对称性，有效提升了强化学习在复杂机器人运动任务中的样本效率与泛化能力，为高维机器人系统的策略学习提供了一种有效的先验融合范式。

Classification reason: 该论文使用Koopman模型学习机器人系统动力学，结合强化学习提升四足机器人双足运动策略的学习效率与迁移能力，属于具身智能中机器人规划与控制的范畴，因此映射到[具身世界模型与规划控制]。

Importance reason: While combining Koopman theory with symmetry for robot locomotion is novel, it is far from the user's priority subtopics in continuous multimodal models and video generation. Signals: Koopman operator, morphological symmetry, legged locomotion, reinforcement learning, sample efficiency
...
### [33] [DA-Nav: Direction-Aware City-Scale Vision-Language Navigation](https://arxiv.org/abs/2607.11638)
...
TL;DR: DA-Nav 提出了一种面向城市级室外视觉语言导航的框架，利用商业导航工具（如 Google Maps）的方向指令，将导航重新定义为在自我中心2D图像平面上的离散空间定位问题，并通过 Chain-of-Thought 推理实现轨迹恢复。在 CARLA 仿真中达到 56.16% 的成功率，且无需微调即可迁移到四足机器人和人形机器人上实现公里级闭环导航。


<details>
  <summary>Details</summary>
Motivation: 现有城市级室外导航严重依赖稠密地图或昂贵的导航监督，缺乏利用商业导航工具（如 Google Maps）中广泛可用的方向指令的方法。同时，长程导航中的误差累积问题亟待解决。

Method: DA-Nav 将导航重新定义为在自我中心2D图像平面上的离散空间定位问题。它采用 Chain-of-Thought (CoT) 推理过程，包括偏差评估、动作预测和目标网格选择，以实现轨迹恢复。同时构建了 ReDA 数据集，提供方向感知指令和恢复轨迹以增强空间定位能力。

Result: 在 CARLA 仿真中，DA-Nav 在未见过的城市环境中达到 56.16% 的成功率，优于现有最先进方法，并展现出更强的轨迹恢复能力。无需微调即可直接迁移到四足机器人和人形机器人上，在真实复杂环境中实现公里级闭环室外导航。

Conclusion: DA-Nav 通过利用商业导航工具的方向指令和 CoT 推理，有效解决了城市级视觉语言导航中的稠密地图依赖和误差累积问题，展示了从仿真到真实机器人的良好泛化能力。

Classification reason: 该论文研究城市级视觉语言导航（VLN），涉及机器人具身导航、动作规划与闭环控制，属于世界模型方向下的具身智能与规划控制范畴，因此映射到[CANONICAL] subtopic "具身世界模型与规划控制"。

Importance reason: Focuses on outdoor navigation with commercial tools, not on core continuous multimodal or video generation priorities. Signals: City-scale navigation, Direction-aware VLM, Chain-of-Thought recovery, Real-world robot transfer, No fine-tuning needed
...
### [34] [Xiaomi-Robotics-U0: Unified Embodied Synthesis with World Foundation Model](https://arxiv.org/abs/2607.11643)
...
TL;DR: 小米机器人团队提出Xiaomi-Robotics-U0，一个380亿参数的多模态自回归世界基础模型，统一了文生图、图像编辑、具身场景生成、具身迁移和具身视频生成五项任务，在保持预训练泛化能力的同时适配具身场景，在具身场景生成、迁移和视频生成上达到SOTA，并将pi_0.5在真实操作任务上的OOD成功率从36.9%提升至63.2%。


<details>
  <summary>Details</summary>
Motivation: 现有的基础图像/视频生成模型虽然泛化能力强，但直接应用于具身场景时受限于多视图一致性、几何一致性和机器人具身约束。现有方法通常用有限的机器人数据适配基础模型，往往牺牲大规模预训练获得的视觉知识。因此需要一种统一框架，既能保留基础模型的泛化能力，又能适配具身场景的特殊需求。

Method: 提出Xiaomi-Robotics-U0，一个380亿参数的多模态自回归模型。它将具身生成视为基础图像/视频生成的扩展，统一优化五项任务：文生图、图像编辑、具身场景生成、具身迁移和具身视频生成。模型采用多模态自回归架构，在预训练的世界基础模型基础上进行联合训练，保持泛化能力的同时适配具身场景。支持多机器人具身的高质量多视图场景生成，并引入结构化可控的具身迁移实现细粒度编辑，同时保持多视图一致性和交互动态。

Result: 在单步和序列生成任务上达到SOTA；在具身场景生成和迁移的人类评估中超越GPT-Image-2.0；在World Arena具身视频生成排行榜上排名第一；将pi_0.5在挑战性真实操作任务上的OOD成功率从36.9%提升至63.2%。

Conclusion: 基础世界模型既可以作为具身世界模型，也可以作为具身智能的可扩展数据引擎。统一框架能够有效保留预训练视觉知识并适配具身场景，为具身智能提供高质量的生成能力和数据支持。

Classification reason: 该论文提出一个380亿参数的多模态自回归模型，用于统一具身合成（场景生成、迁移、视频生成），核心是作为具身世界模型服务于机器人操作任务，与[CANONICAL]具身世界模型与规划控制高度匹配。
...
### [35] [From World Action Models to Embodied Brains: A Roadmap for Open-World Physical Intelligence](https://arxiv.org/abs/2607.11689)
...
TL;DR: 本文综述了世界动作模型（WAM）的演进，分析了模型角色与表征、目标与标准化、系统组合三个关键差距，并提出了以"具身大脑"为核心的协同演进路线图，通过物理约束、共享契约和闭环后训练构建模块化的物理智能栈。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型、动作模型和视觉-语言-动作策略的研究碎片化严重，模型使用不兼容的动作空间和预测目标，数据集和任务遵循不同约定，运行时系统缺乏可复用接口，亟需统一框架来推动物理智能的发展。

Method: 论文通过系统综述分析现有WAM的局限性，提出"具身大脑"作为长期模型目标——整合多模态上下文、比较候选干预、发出状态转换或能力请求而非直接执行器命令。同时提出物理约束（工具、控制器、验证、日志）、共享契约（对齐异构模型/数据/任务/具身）和闭环后训练（将验证交互转化为可复用经验）三大组件，构成模块化物理智能栈。

Result: 识别出三个关键差距：模型角色与表征差距、目标与标准化差距、系统组合差距；提出了具身大脑的概念框架和物理智能栈的模块化架构设计。

Conclusion: WAM为具身大脑的预测功能提供了有前景的原型，通过物理约束、共享契约和闭环后训练的组合，可以构建自适应、自改进的具身智能体，为开放世界物理智能提供了一条清晰的协同演进路线。

Classification reason: 该论文综述了从世界动作模型（WAM）到具身大脑的演进路线，核心关注具身智能体在物理世界中的推理、规划与控制，与[CANONICAL]具身世界模型与规划控制方向高度匹配。

Importance reason: Strongly aligns with world model and embodied AI priorities, offering a structured roadmap for physical intelligence. Signals: world model roadmap, embodied brain concept, physical intelligence stack, closed-loop post-training, shared contracts
...
### [36] [AutoPath: Learning Transferable Goal-Conditioned Stochastic Path Prior for Safe Navigation Without Human Demonstrations](https://arxiv.org/abs/2607.11739)
...
TL;DR: 本文提出 AutoPath，一种可迁移的目标条件随机路径先验学习方法，用于无人类演示的机器人安全导航。通过目标对齐规范状态表示和极坐标动作流形参数化局部路径，结合多目标分布 rollout 的风险敏感效用整形，实现密集静态和动态行人环境中的高效安全导航，并支持跨平台迁移。


<details>
  <summary>Details</summary>
Motivation: 在杂乱动态环境中实时导航需要无碰撞且动力学可行的运动，但可行导航行为本质上是多模态的（障碍物周围存在多条路径）。现有方法依赖人类演示或特定机器人运动约束，缺乏可迁移的通用路径先验。

Method: 1) 将导航建模为学习可迁移的目标条件随机路径先验，建模目标对齐的几何一致局部路径分布；2) 提出目标对齐规范状态表示，消除平面旋转模糊性并归一化局部几何；3) 开发结构化先验学习框架，用几何感知极坐标动作流形参数化局部路径，结合多目标分布 rollout 的风险敏感效用整形进行安全规划。

Result: 在密集静态环境和动态行人场景中，AutoPath 实现了持续高成功率与有竞争力的效率，且单个在差分驱动机器人上学习的路径先验可直接迁移到四足机器人平台，无需重新训练。

Conclusion: AutoPath 通过可迁移的随机路径先验学习，有效解决了多模态导航路径建模和跨平台迁移问题，为无人类演示的安全导航提供了新范式。

Classification reason: 该论文研究机器人导航中的路径先验学习与安全规划，属于具身智能中的运动规划与控制问题，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics navigation paper but outside user's core focus on continuous multimodal models and video generation. Signals: world model, goal-conditioned path prior, safe navigation, cross-platform transfer, no human demonstrations
...
### [37] [A Dynamic Scene Interaction Reasoning Framework for Scene-level Lane-Change Intention and Trajectory Prediction of Multiple Interacting Vehicles](https://arxiv.org/abs/2607.09740)
...
Importance: 普通相关 (50.6) - 未入选精读

TL;DR: 提出DSiGAT动态场景图注意力框架，将交通场景建模为时变交互图，通过图注意力消息传递捕捉车辆间依赖关系，实现多车辆换道意图与轨迹的联合预测，在NGSIM和highD数据集上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有换道预测方法多聚焦于单目标车辆，而多智能体预测方法仅输出未来位置，缺乏对每辆车具体机动（如换道意图）的显式预测。需要一种场景级框架，同时预测所有相关车辆的换道意图和未来轨迹。

Method: 将交通场景表示为时变交互图，车辆为节点，空间和运动学关系为显式边特征。通过时序图注意力消息传递捕捉车辆间动态依赖和换道前线索，意图引导解码器将预测的机动与对应未来轨迹关联，并引入场景级一致性损失以促进多车未来轨迹的兼容性。

Result: 在NGSIM I-80和US-101上意图预测准确率分别达90.12%和90.97%，轨迹RMSE相比最强基线降低最多52.94%，碰撞率和联合位移误差也更低。

Conclusion: DSiGAT通过场景级图注意力建模有效提升了多车辆换道意图和轨迹预测的准确性与一致性，验证了场景聚焦公式的有效性。

Classification reason: 该论文提出动态场景图注意力框架，对交通场景中多车辆进行换道意图预测和轨迹预测，属于对动态交通环境（物理世界）的交互建模与未来预测，与[可交互与物理世界建模]（interactive_physical_world）高度匹配。

Importance reason: Focuses on traffic scene graph prediction, which is not aligned with user's priority on continuous multimodal models or video generation. Signals: traffic scene graph, lane-change prediction, multi-agent trajectory, NGSIM/highD datasets, graph attention
...
### [38] [Stateful Worlds, Stateless Elasticity: Exact-State Serving for Interactive World Models](https://arxiv.org/abs/2607.10389)
*Jin Li, Jiawei Chen*
...
TL;DR: 本文提出 WorldMove，一种用于交互式世界模型的精确状态迁移系统，能够在 18.8ms 内完成同节点状态迁移（比 save/load 快 101 倍），在 100Gb 网络上达到 92.1-94.8 Gb/s 的校验吞吐，并保证迁移后状态比特级一致。同时提出拥塞感知的准入控制机制，解决接收端校验导致的协议级拥塞问题。


<details>
  <summary>Details</summary>
Motivation: 交互式世界模型（如视频生成式世界模型）在 GPU 上运行时，其注意力缓存（multi-gigabyte attention cache）几乎每步生成都会被重写，无法在交互时间内重新计算或近似。这导致活跃会话被"钉"在特定 GPU 上，无法弹性调度。现有 save/load 方案太慢，无法满足交互式时间要求。因此需要一种能在交互时间内精确迁移世界模型状态的方法。

Method: WorldMove 系统包含三个核心组件：(1) 状态迁移器——在保证目标端与源端比特级一致的前提下，18.8ms 内完成同节点缓存迁移，利用 RDMA 在 100Gb 网络上达到 92.1-94.8 Gb/s 的校验吞吐；(2) 可接纳性条件——判断迁移是否能在读取时间窗内完成，考虑带宽、状态大小和脏数据速率；(3) 拥塞感知准入控制——解决接收端校验在扇入场景下导致的协议级拥塞和 incast 问题，通过无损 GPU 编解码器拓宽准入窗口。

Result: WorldMove 在 18.8ms 内完成同节点状态迁移，比 save/load 快 101 倍；在 100Gb 网络上达到 92.1-94.8 Gb/s 的校验吞吐；在跨两个云提供商的 48 次迁移中全部实现比特级一致；拥塞感知准入控制在 1.4 倍负载下零丢包，超载时优雅拒绝。

Conclusion: 本文证明了交互式世界模型可以实现精确状态的弹性迁移，关键在于将传输和校验作为联合调度问题来处理。WorldMove 为交互式世界模型的大规模部署提供了基础设施层面的支持，使得世界模型可以在集群中弹性调度而不中断交互体验。

Classification reason: 该论文聚焦于交互式世界模型（interactive world model）的弹性服务与状态迁移问题，属于世界模型方向中可交互与物理世界建模（interactive_physical_world）这一规范子主题。论文讨论的是如何在不中断交互的情况下迁移世界模型的运行状态（attention cache），核心场景是交互式世界模型的持续服务。

Importance reason: Directly addresses exact-state migration for interactive world models, a critical infrastructure need for your high-priority world model and efficient inference subtopics. Signals: world model serving, state migration, interactive world model, efficient inference, GPU memory management
...
### [39] [GeoGS-SLAM: Online Monocular Reconstruction Using Gaussian Splatting with Geometric Priors](https://arxiv.org/abs/2607.11184)
...
TL;DR: GeoGS-SLAM 是一个基于3D高斯泼溅的在线单目稠密重建系统，结合预训练视觉几何模型提供的几何先验，通过粗到细的联合优化策略同时优化相机位姿和场景地图，并引入闭环检测保证全局一致性，在室内外基准上实现了优于现有方法的渲染质量和跟踪精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于3DGS的SLAM方法通常需要外部深度传感器提供几何信息，而利用几何先验的SLAM系统在优化过程中往往丢弃原始RGB信息，导致重建质量下降。本文旨在结合3DGS地图表示与学习到的几何先验，实现无需深度传感器的在线单目稠密重建。

Method: 1) 使用前馈视觉几何模型预测相机和场景先验；2) 从RGB输入和几何先验中直接采样高斯原语来扩展高斯场景地图；3) 通过粗到细策略联合优化相机位姿和场景地图，最小化光度损失和几何损失；4) 引入在线闭环检测和位姿图优化保证全局一致性。

Result: 在室内外多个基准数据集上的实验表明，GeoGS-SLAM在渲染质量和跟踪精度上均优于现有最先进方法，同时保持在线实时性能。

Conclusion: 本文证明了结合3DGS地图表示与学习几何先验可以有效实现高质量的单目在线稠密重建，无需外部深度传感器，为SLAM系统提供了一种新的有效范式。

Classification reason: 该论文基于3D高斯泼溅(3DGS)进行在线单目稠密重建与SLAM，属于3D场景空间建模与重建，与[空间 3D/4D 世界模型]这一规范子主题高度匹配。

Importance reason: Strong SLAM contribution but only tangentially touches priority continuous multimodal and video generation topics. Signals: 3D Gaussian Splatting SLAM, geometric priors from pretrained models, online monocular reconstruction, loop closure for global consistency, real-time performance

Abstract: SLAM methods based on 3D Gaussian Splatting (3DGS) have demonstrated impressive tracking and mapping performance, but typically require additional geometric information from external depth sensors. Meanwhile, recent SLAM systems that leverage geometric priors from pre-trained feed-forward models enable real-time dense reconstruction, yet often discard original RGB information during optimization, thus degrading overall reconstruction quality. We present GeoGS-SLAM, an online monocular dense reconstruction system that combines the 3DGS-based map representation with learned geometric priors. Given uncalibrated RGB input, we first employ a feed-forward visual geometry model to predict camera and scene priors. The Gaussian scene map is then expanded by directly sampling Gaussian primitives from both RGB input and geometric priors. Camera poses and the scene map are jointly optimized through a coarse-to-fine strategy that minimizes both photometric and geometric losses. To ensure global consistency, we further incorporate online loop closure detection and pose graph optimization. Extensive experiments across indoor and outdoor benchmarks demonstrate that GeoGS-SLAM achieves superior rendering quality and tracking accuracy compared to state-of-the-art methods while maintaining online real-time performance. Project page: .
...
### [40] [When Does Depth Survive Composition? Compute--Quality Regimes in Latent World Models](https://arxiv.org/abs/2607.10203)
...
TL;DR: 本文通过预注册实验，系统研究了自适应计算世界模型中深度（depth）在自回归 rollout 中是否有效。在9个DeepMind Control任务上发现了三种计算-质量区间：深度有益（6/9）、浅层反超（2/9）和平坦（1/9）。反直觉的"反转"现象是由训练方式造成的，且与任务的可路由性形成"catch-22"困境。深度效果部分可预测，且影响CEM规划器的性能。


<details>
  <summary>Details</summary>
Motivation: 自适应计算世界模型（如early-exit或mixture-of-depths）假设深度能带来更好的预测，且可以自适应路由。但在自回归rollout中，深度带来的单步精度提升能否在组合传播中存活下来？这一基本假设尚未被系统验证。

Method: 设计预注册实验，定义浅层惩罚指标ρ = err(最浅层exit rollout) / err(全深度rollout)。在9个DeepMind Control任务上，分别在单步(K=1)和多步(K=4)训练下进行测试，每任务3个种子。通过消融实验（仅在第一rollout步监督early exit）分析反转现象的成因，并使用Spearman相关分析预测因素。

Result: 发现三种区间：6/9任务深度有益（ρ最高4.7×），2/9任务浅层反超（ρ低至0.85×），1/9平坦。反转现象（cheetah任务）由训练方式造成——消融实验消除反转（ρ: 0.87→1.18）。观测/动作维度和单步模型误差与ρ的Spearman相关系数约0.75。在CEM规划器中，ρ的符号预测深度是否有利于规划。

Conclusion: 深度在自回归世界模型中的有效性并非理所当然，存在计算-质量区间。反转现象与可路由性之间存在"catch-22"困境：使exit可路由的监督方式恰恰训练它们超越全深度模型。任务区间部分可先验预测，且影响下游规划性能。研究结果对自适应计算世界模型的设计具有重要指导意义。

Classification reason: 论文研究潜空间世界模型中深度（depth）对自回归 rollout 预测质量的影响，属于世界模型中的视频/时序预测建模，与 [CANONICAL] 视频世界模型（video_world_model）高度匹配。

Importance reason: Directly investigates depth composition in latent world models, a core issue for autoregressive video generation and world models, with surprising inversion regimes. Signals: world model depth composition, compute-quality regimes, routability catch-22, DeepMind Control tasks, CEM planning impact
...
### [41] [Is Energy Guidance All You Need? Training-Free Norm Injection for Driving World Models](https://arxiv.org/abs/2607.10781)
...
TL;DR: 本文提出一种无需重新训练的能量引导方法，通过在采样时注入可微能量函数来驱动驾驶世界模型遵循交通规范，实验基于Open-Sora 2.0骨干网络验证了该方法可使模型在反事实目标处刹车，但发现生成视频尚未完全跟随引导轨迹，指出跨流耦合是实现端到端可控的关键。


<details>
  <summary>Details</summary>
Motivation: 驾驶世界模型基于大规模视频扩散骨干生成逼真场景，但难以控制其行为——强制执行交通规范通常需要重新训练骨干网络或依赖手工构建的布局。本文旨在探索是否可以在不进行特定知识重训练的情况下，仅通过采样时引导实现世界模型的可控性。

Method: 基于修正流（rectified-flow）驾驶世界模型，该模型联合生成未来视频和规划的自我轨迹。在采样时，通过可微能量函数编码驾驶规范（如刹车），将能量引导注入采样过程，从而引导轨迹生成，无需对扩散骨干进行特定知识的重新训练。骨干网络采用Open-Sora 2.0的MM-DiT架构。

Result: 实验表明，该方法能够成功引导世界模型在反事实目标处刹车。然而，生成视频并未完全跟随被引导的轨迹，通过骨干网络的联合自注意力机制发现，跨流耦合（cross-stream coupling）是实现端到端可控rollouts的关键要求。

Conclusion: 能量引导可以在无需重训练的情况下驱动驾驶世界模型遵循交通规范，但当前生成视频与引导轨迹之间仍存在差距，跨流耦合是未来实现完全可控的关键方向。

Classification reason: 该论文基于视频扩散骨干构建驾驶世界模型，通过能量引导在采样时控制轨迹生成，属于动作条件视频生成与未来预测，映射到 canonical 子主题"视频世界模型"。

Importance reason: Directly addresses world model controllability via training-free energy guidance, aligning with top-priority video world model and continuous multimodal reasoning subtopics. Signals: world model, training-free control, energy guidance, driving world model, diffusion backbone
...
### [42] [Cycle-World: Mitigating Error Accumulation in Long-term Video World Models via Reverse-Prediction Cycle Consistency](https://arxiv.org/abs/2607.11836)
...
TL;DR: Cycle-World 提出一种基于反向预测循环一致性的框架，通过训练阶段的反向预测模型和推理阶段的梯度引导校正，有效缓解自回归扩散模型在长视频生成中的误差累积问题，在 60 秒长视频生成上达到 SOTA 质量与时序一致性。


<details>
  <summary>Details</summary>
Motivation: 自回归扩散模型在长时序视频生成中，由于逐帧预测的误差会随时间累积，导致生成漂移、结构崩塌和视觉退化。现有方法缺乏对误差累积的有效约束机制。

Method: Cycle-World 包含训练和推理两个阶段。训练阶段：集成一个高效的反向预测模型，对前向生成器施加隐式因果约束，迫使生成可逆序列以贴合自然视频流形。推理阶段：利用冻结的反向模型作为运行时校正器，通过基于梯度的循环引导（cycle guidance）迭代优化生成的潜表征，主动抑制累积误差。

Result: 在 VBench 基准上的大量实验表明，Cycle-World 的双阶段协同机制显著缓解了误差漂移，在 60 秒长视频合成中实现了整体生成质量和长时序时间一致性的 SOTA 性能。

Conclusion: Cycle-World 通过循环一致性约束有效解决了长视频生成中的误差累积问题，为稳定、时序一致的长视频世界模型提供了可行的框架。

Classification reason: 该论文提出 Cycle-World 框架，通过反向预测循环一致性来缓解自回归扩散模型在长视频生成中的误差累积问题，属于视频世界模型中的长时序视频建模与未来预测方向，因此映射到 [CANONICAL] subtopic "视频世界模型" (video_world_model)。同时论文涉及长视频生成方法，也匹配 multimodal_generation 方向下的 text_to_video_diffusion 子主题。

Importance reason: Directly addresses error accumulation in long-term video world models, a core challenge for your high-priority world model and video generation directions. Signals: world model, video generation, error accumulation, cycle consistency, autoregressive diffusion
...
### [43] [Incremental Online Scene Reconstruction by 3D Gaussian Triangulation](https://arxiv.org/abs/2607.10690)
...
TL;DR: 本文提出一种增量式在线场景重建框架，通过直接三角化密集几何高斯表示来重建和更新高保真显式网格，无需离线转换为隐式场。该方法支持高质量渲染和增量表面重建，并采用基于平面的拉拽约束将3D高斯原语动态对齐到局部表面，同时通过动态冻结历史区域减少长序列处理中的内存和计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法大多需要将优化后的高斯离线转换为中间隐式场才能提取显式网格，这阻碍了与下游任务的无缝集成。因此，本文旨在实现一种无需离线转换的增量式在线场景重建方法。

Method: 提出一种在线框架，通过直接三角化密集几何高斯表示来增量式重建和更新高保真显式网格。设计了一种直接网格化算法，从高斯集合中高效提取和更新网格。采用基于平面的拉拽约束，动态将3D高斯原语对齐到近似的局部表面。通过动态冻结完全优化的历史区域，减少长序列处理中的内存和计算开销。

Result: 在公开数据集上的实验表明，该方法在渲染质量和重建精度上均优于传统基于高斯的方法。

Conclusion: 本文提出的增量式在线场景重建框架有效解决了现有3DGS方法需要离线转换的问题，实现了高质量渲染与增量表面重建的统一，为实际应用中的实时场景重建提供了可行方案。

Classification reason: 该论文提出基于3D高斯泼溅的增量式在线场景重建方法，直接通过三角化密集几何高斯表示提取显式网格，属于3D场景重建与3DGS相关研究，映射到[CANONICAL] subtopic "3D 生成与重建"。

Importance reason: Focuses on 3D reconstruction and meshing, not directly aligned with user's priority subtopics like continuous language models or video generation. Signals: 3D Gaussian Splatting, incremental reconstruction, explicit mesh extraction, plane-based pulling constraint, online framework
...
### [44] [MAC-Splat: Multi-Attribute Consistency for High-Fidelity Sparse-View Reconstruction](https://arxiv.org/abs/2607.10792)
...
TL;DR: MAC-Splat 提出了一种基于多属性一致性（MAC）损失的训练框架，利用 MASt3R 几何骨干网络和 DINOv3 编码器获取语义感知的2D对应点作为几何锚点，在3D空间中联合正则化匹配高斯原语的位置、形状和外观属性，从而在稀疏视角条件下实现高保真3D场景重建。在 ScanNet++ 上相比 Splatt3R 提升超过 4.5 dB PSNR。


<details>
  <summary>Details</summary>
Motivation: 现有可泛化3DGS方法在稀疏视角设置下常出现几何伪影，因为仅依赖2D光度损失无法解决深度和对应关系的歧义性。需要一种直接的3D一致性监督来缓解这一问题。

Method: 基于 MASt3R 几何骨干网络和冻结的 DINOv3 编码器获取语义感知的2D对应点作为几何锚点，定义多属性一致性（MAC）损失，在公共世界坐标系中对匹配高斯原语的位置、形状和外观属性进行联合正则化，并对协方差矩阵进行几何感知处理以增强对异常值的鲁棒性。

Result: 在 ScanNet++ 数据集上，MAC-Splat 在多种重叠率设置下均优于强基线方法，平均 PSNR 相比 Splatt3R 提升超过 4.5 dB，LPIPS 降低，且在相机位姿间隔增大时仍能保持性能。

Conclusion: 直接的、多属性的3D一致性目标结合高质量对应点，是解决稀疏视角重建这一病态问题的有效方法。

Classification reason: 该论文研究稀疏视角下的3D场景重建，基于3D Gaussian Splatting (3DGS) 框架，属于3D生成与重建范畴，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Strong 3D reconstruction method but not aligned with user's priority subtopics on continuous language/video models or world models. Signals: 3D Gaussian Splatting, sparse-view reconstruction, multi-attribute consistency, geometric anchors, ScanNet++ benchmark
...
### [45] [DP-Splat: Bayesian Nonparametric Complexity Control for Gaussian Splatting](https://arxiv.org/abs/2607.10912)
*Aqi Dong*
...
TL;DR: DP-Splat 将 3D Gaussian Splatting 中的密度控制替换为截断的 stick-breaking Dirichlet Process 先验，使高斯成分数量自适应场景复杂度，同时保持闭形式坐标上升更新，在匹配预算下以 5.9-7.6x 更少的成分达到或超过 VBGS 的预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有 3D Gaussian Splatting 方法依赖启发式密度控制或用户设定的固定成分数量 K，缺乏理论驱动的复杂度自适应机制。VBGS 虽引入变分贝叶斯推断，但 K 仍固定。本文旨在用贝叶斯非参数方法让模型自动决定所需高斯成分数量。

Method: 将有限对称 Dirichlet 先验替换为截断 stick-breaking Dirichlet Process 先验（以及稀疏过拟合有限 Dirichlet 作为替代），使占用成分数量自适应数据。所有更新保持闭形式坐标上升步骤；提出自然梯度随机变体使每步计算成本与点数无关。给出了单调性保证、截断误差界修正，并分析了拟合成分数量的估计含义。

Result: 在合成数据上，有效复杂度 K̂ 自适应场景复杂度，恢复真实 K 在 ±1 以内。在 3D 场景上，DP-Splat 以 5.9-7.6x 更少的成分匹配或超过 VBGS 的留出颜色预测。后验预测颜色方差在校准良好的合成数据上表现良好。发现变分坐标上升下 DP 先验抵抗过分割，而稀疏有限混合饱和其截断。

Conclusion: DP-Splat 成功将贝叶斯非参数方法引入 3DGS，实现了理论驱动的复杂度自适应，显著减少了冗余高斯成分，同时保持或提升了重建质量。该工作为 3D 场景表示中的模型选择提供了新的理论视角。

Classification reason: 该论文提出 DP-Splat，在 3D Gaussian Splatting 中引入贝叶斯非参数先验（Dirichlet Process）以自适应控制高斯成分数量，属于 3DGS 场景表示与重建方法，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Novel Bayesian nonparametric control for 3DGS, but outside user's priority subtopics and negative for 3D generation. Signals: 3D Gaussian Splatting, Bayesian nonparametric, Dirichlet process, variational inference, complexity control
...
### [46] [AsySplat: Efficient Asymmetric 3D Gaussian Splatting for Long-Sequence Scene Modeling](https://arxiv.org/abs/2607.10995)
...
TL;DR: AsySplat 提出一种非对称3D高斯泼溅架构，将几何与外观建模解耦：几何分支处理粗粒度token进行多视图重建，外观分支用更少参数处理细粒度token捕捉细节。在32视图960P输入上，匹配优化方法的同时实现近800倍加速，且以更少参数超越现有可泛化模型。


<details>
  <summary>Details</summary>
Motivation: 现有可泛化3D高斯泼溅模型在长序列新视角合成中存在大量冗余计算。作者观察到：(i) 高质量NVS并不严格要求高精度几何；(ii) 外观学习通常比几何恢复更容易。基于此，提出非对称架构以减少计算冗余。

Method: 提出非对称架构解耦几何与外观建模。几何分支使用粗粒度token和大部分参数进行多视图重建；外观分支使用细粒度token和显著更少的参数捕捉细节。两分支通过双向连接交互，实现任务间的相互指导。

Result: 在32视图960P输入上，匹配优化方法质量的同时实现近800倍加速；以更少参数和更低的训练/推理开销超越现有可泛化模型的零样本性能，整体效率显著提升。

Conclusion: 任务感知的非对称设计有效减少了计算冗余，更合理地分配计算资源，提升了参数效率，使更小模型也能达到强性能，为长序列场景建模提供了高效新范式。

Classification reason: 该论文提出非对称3D高斯泼溅架构用于长序列场景建模与新视角合成，属于3D场景生成与重建方向，映射到[CANONICAL] subtopic "3D 生成与重建"。

Importance reason: Focuses on 3D reconstruction efficiency, not directly on the user's priority subtopics like continuous language models or video generation. Signals: 3D Gaussian Splatting, asymmetric architecture, long-sequence scene modeling, efficiency improvement, not in priority subtopics
...
### [47] [WiFi-JEPA: Self-supervised Learning for WiFi-CSI 3D Human Pose Estimation](https://arxiv.org/abs/2607.11064)
...
TL;DR: WiFi-JEPA 提出了一种自监督学习框架，通过预测掩码潜嵌入而非重建原始CSI信号，从WiFi信道状态信息中学习3D人体姿态估计。该方法设计了CSI特定的token化和链路掩码策略，并利用光线追踪仿真生成无标注预训练数据，在Person-in-WiFi-3D数据集上取得了SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有基于WiFi-CSI的人体姿态估计方法在环境变化下泛化能力差，且依赖昂贵的相机标注流程，限制了数据规模。因此需要一种无需标注的自监督方法，学习对环境和硬件变化鲁棒的CSI表征。

Method: 提出WiFi-JEPA框架：(1) CSI特定的token化与链路掩码策略，在信道(C)、时间(T)、链路(L)三维张量上操作，掩码整个Tx-Rx天线链路迫使模型从其他链路预测被掩码链路的潜嵌入；(2) 基于光线追踪的CSI仿真管线，从随机几何基元生成多样化无标注CSI数据用于预训练；(3) 采用JEPA范式，在潜空间预测掩码嵌入而非重建原始信号，避免学习硬件特定伪影。

Result: 在Person-in-WiFi-3D数据集上，WiFi-JEPA在单人和多人3D姿态估计任务中均超越所有先前WiFi-CSI基线方法。仿真CSI与真实CSI互补，提供有效的预训练信号。对比实验表明四种视觉SSL目标（如MAE、SimCLR等）在CSI任务上性能甚至低于从头训练，而WiFi-JEPA持续提升下游姿态估计性能。

Conclusion: WiFi-JEPA证明了自监督潜空间预测范式在WiFi-CSI人体姿态估计中的有效性，通过CSI特定的掩码策略和仿真数据预训练，实现了对环境变化鲁棒的3D姿态估计，为隐私保护的人体感知提供了新思路。

Classification reason: 该论文研究从WiFi-CSI信号进行3D人体姿态估计，属于3D人体姿态重建任务，与[3D 生成与重建]（three_d_generation）方向相关。虽然方法侧重自监督学习而非传统生成，但目标是从非视觉信号重建3D人体姿态，属于3D重建范畴。

Importance reason: WiFi-based pose estimation is niche and not aligned with user's priority on continuous multimodal models or video generation. Signals: self-supervised learning, WiFi-CSI, 3D human pose estimation, environment shift robustness, ray-tracing simulation
...
### [48] [SalientGS: Unified SfM-to-3DGS with Importance-Guided MCMC Gaussian Allocation](https://arxiv.org/abs/2607.11285)
...
TL;DR: SalientGS 提出了一种从无序图像直接到 3DGS 的端到端管线，通过重要性引导的 MCMC 高斯分配机制，利用多视图残差聚合生成欠拟合与冗余信号，将高斯容量从已拟合区域重新分配到欠拟合区域，在 15 分钟内实现高质量 3D 重建，无需传统 SfM 预处理。


<details>
  <summary>Details</summary>
Motivation: 传统 3D 场景重建依赖昂贵的 SfM 预处理（如 COLMAP），且 SfM 与后续 3DGS 优化之间存在冻结的位姿接口，限制了端到端效率。现有 3DGS 方法在高斯分配上缺乏自适应性，导致容量浪费在已拟合区域而欠拟合区域得不到充分覆盖。

Method: 提出 SalientGS 统一管线，核心是重要性引导的 MCMC 高斯分配：1) 聚合多视图残差生成每个高斯的欠拟合与冗余信号；2) 基于这些信号构建平滑的重要性加权采样分布，偏置高斯的生成（birth）和重定位（relocation）到欠拟合区域；3) 从已拟合区域重新分配容量，同时保持随机梯度 Langevin 动力学（SGLD）不变。

Result: 在 15 分钟内完成端到端重建，达到最先进的感知质量（LPIPS 等指标）。提供了逐场景定性比较和逐图像 LPIPS 分析，包括失败案例。

Conclusion: SalientGS 通过重要性引导的 MCMC 高斯分配，实现了无需 SfM 预处理的端到端 3DGS 重建，在效率和感知质量上均取得优异表现，为 3D 场景重建提供了一种更简洁高效的范式。

Classification reason: 该论文提出从无序图像直接重建3D高斯泼溅（3DGS）的端到端管线，属于3D场景重建与3DGS方法，与[多模态生成]方向下的[3D 生成与重建]（CANONICAL）高度匹配。

Importance reason: Novel SfM-free 3DGS pipeline with MCMC allocation, but outside user's priority subtopics and lacks boost keywords. Signals: 3D reconstruction, Gaussian Splatting, MCMC allocation, no SfM, 15-minute pipeline
...
### [49] [HierCAD: Hierarchical Text-to-CAD Design via Structure Alignment and Parameter Grounding](https://arxiv.org/abs/2607.11339)
...
TL;DR: HierCAD 提出了一种层次化文本到CAD设计框架，通过将CAD构建树分解为对象级和部件级推理轨迹，并引入结构对齐与参数接地学习策略，在CAD序列生成和重建模型评估上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本到CAD方法在复杂设计中难以保持结构一致性，且几何参数预测不够精确。

Method: 将CAD生成重构为渐进推理过程，分解为对象级程序推理和部件级拓扑推理轨迹；提出统一的结构对齐与参数接地（SAPG）学习策略，包括拓扑推理轨迹与参数CAD片段的对齐，以及通过结构保持参数扰动和排序监督缓解捷径学习。

Result: 在CAD序列生成和重建CAD模型评估上均优于先前最先进方法。

Conclusion: 层次化分解与结构对齐、参数接地策略能有效提升文本到CAD生成的结构一致性和参数精度。

Classification reason: 该论文研究文本到CAD设计生成，属于3D物体/场景生成范畴，与[CANONICAL] subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Focuses on text-to-CAD generation, which is not a priority subtopic and lacks strong overlap with user's configured direction weights and boost keywords. Signals: CAD generation, hierarchical reasoning, structure alignment, parameter grounding, 3D design
...
### [50] [3D Scene Graph Prediction: Generating Hierarchical Models from Partially Observed Environments](https://arxiv.org/abs/2607.10879)
...
TL;DR: 本文提出一种自上而下的框架，用于从部分观测的室内环境生成层次化3D场景图，包括房间层（平面图与可通行性）和物体层（物体布局）。房间层采用混合域图扩散模型联合预测房间类别、楼层边界和房间间可通行性；物体层集成混合离散-连续扩散模型预测物体类别、位置、大小和朝向。该方法在标准基准上优于基于占用的基线方法，并能在真实机器人采集数据上实现超出探索区域的场景预测。


<details>
  <summary>Details</summary>
Motivation: 现有3D室内场景生成方法多聚焦于单房间内的物体布局生成，而高层场景结构（如房间级布局和可通行性）的生成在机器人应用中至关重要但尚未充分探索。本文旨在解决机器人部分探索环境后，预测未探索区域以支持探索或物体搜索等下游任务的问题。

Method: 提出一个两层的层次化3D场景图生成框架：(1) 房间层：提出混合域图扩散模型（mixed-domain graph diffusion model），通过噪声注入和掩码机制联合预测房间类别、楼层边界和房间间可通行性，支持不完整平面图的部分约束；(2) 物体层：集成现有的混合离散-连续扩散模型，在给定平面图条件下联合预测每个房间内物体的类别、位置、大小和朝向。

Result: 在标准基准上，与基于占用的学习基线和LLM方法相比，本文方法在分布外部分平面图上具有显著更好的泛化能力。在真实机器人采集数据上，该方法能够成功预测探索区域之外的场景结构。

Conclusion: 本文提出的层次化3D场景图生成框架能够从部分观测中有效预测未探索区域的房间布局和物体布局，为机器人探索和物体搜索等任务提供了支持。混合域图扩散模型的设计使其能够处理部分约束，无需部分观测训练数据。

Classification reason: 该论文提出从部分观测环境生成层次化3D场景图（包括房间层和物体层），属于3D场景生成范畴，与[3D 生成与重建]（three_d_generation）这一CANONICAL subtopic高度匹配。

Importance reason: Focuses on 3D scene graph prediction for robotics, not aligned with user's priority on continuous multimodal models or video/image generation. Signals: 3D scene graph, hierarchical generation, partial observation, robotics, diffusion model
...
### [51] [FoundationGeo: Learning Spatial Pixel-Wise Fields for Monocular Metric Geometry](https://arxiv.org/abs/2607.11588)
...
TL;DR: FoundationGeo 是一个两阶段单目度量几何框架：第一阶段学习仿射不变的高保真几何模型，第二阶段通过轻量级像素级校准场（尺度场+射线方向校正场）实现度量一致的3D点图重建。同时通过Blender数据引擎合成多样焦距数据，解决相机内参分布不匹配导致的零样本泛化瓶颈。在7个基准上超越现有方法5.2%以上。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度/几何估计方法在跨域泛化时性能剧烈波动，且从相对深度到度量深度的转换通常依赖全局缩放，无法处理空间变化的度量偏差。此外，训练与测试数据间相机内参（尤其是焦距）分布不匹配是零样本度量泛化的关键瓶颈，但此前未被充分认识。

Method: 两阶段框架：Stage 1 用 DINOv3 初始化，在 10.2M 多域数据上训练仿射不变几何模型，辅以局部细节监督；Stage 2 引入轻量级像素级校准场——尺度场（空间变化度量对齐）和射线方向校正场（缓解点图几何方向偏差），输出度量一致3D点图。另用 Blender 数据引擎合成不同焦距的训练数据以覆盖欠采样焦距区域。

Result: 在7个零样本评测基准上，FoundationGeo 展现出强跨域鲁棒性，避免其他方法的剧烈性能波动，综合性能最优，平均超越更重的基线方法5.2%以上。

Conclusion: 像素级校准场比全局缩放更有效地实现度量对齐；相机内参分布匹配对零样本度量泛化至关重要；合成数据可有效修复欠覆盖焦距区域，提升鲁棒性。

Classification reason: 该论文从单张图像恢复度量3D几何（深度/点图），属于3D场景重建范畴，与[3D 生成与重建]（three_d_generation）高度相关。

Importance reason: Strong monocular metric geometry work but outside user's priority subtopics and negative keywords. Signals: monocular metric geometry, spatial calibration fields, Blender data engine, zero-shot generalization, camera intrinsic mismatch
...
### [52] [ABot-3DWorld 0: A Universal World Model to Explore Any 3D Space](https://arxiv.org/abs/2607.11673)
...
TL;DR: ABot-3DWorld 0 提出了一种通用多模态3D世界模型，通过统一的空间生成基元（SGP，全景图+点云）将文本、图像和视频输入转化为高保真、可探索的3DGS世界，支持丰富输入（多视图、视频）的几何精确重建和单图/文本的创意生成，并可将生成世界锚定到地理兴趣点实现地图级空间探索。


<details>
  <summary>Details</summary>
Motivation: 现有3D内容创建方法门槛高，缺乏一个统一的、低门槛的引擎，能够将多种模态输入（文本、单图、多视图、视频）转化为高质量、可自由探索的3D世界，并支持大规模地理空间锚定。

Method: 提出Spatial Generative Primitive (SGP)作为统一中间表示（全景图+点云）。多模态输入首先被提升为SGP：丰富输入（多视图、视频）通过几何精确恢复构建SGP；单图或文本通过生成式补全创建创意世界。然后一个3D一致的全景视频生成器沿规划轨迹探索SGP；最后全景视频重建引擎将生成视频转换为干净的3D高斯泼溅(3DGS)世界。

Result: 在丰富多模态输入下，ABot-3DWorld 0在开源方法中达到SOTA，场景保真度优于Marble。支持从文本、单图到多视图/视频的多种输入模式，并能将生成世界锚定到地理兴趣点。

Conclusion: ABot-3DWorld 0 提供了一个低门槛的通用3D内容创建引擎，统一了多种输入模态到可探索3D世界的转换，在场景保真度上达到开源最优，并首次实现了地图级空间探索能力。

Classification reason: 该论文提出从文本/图像/视频输入生成可探索的3D世界，核心使用3D高斯泼溅(3DGS)重建，属于3D场景生成与重建范畴，因此映射到[multimodal_generation]方向下的[three_d_generation]（3D生成与重建）规范子主题。同时该工作涉及多模态输入到3D世界的转换，也匹配[multimodal_understanding_generation]方向。

Importance reason: Strong 3D world model with multimodal input and map anchoring, but only partially aligns with priority subtopics like world model and unified multimodal. Signals: 3D world model, multimodal generation, spatial generative primitive, 3DGS reconstruction, map-native exploration
...
### [53] [LATO.2: Factorized 3D Mesh Generation with Vertex and Topology Flow](https://arxiv.org/abs/2607.10623)
...
TL;DR: LATO.2 提出一种分解式流匹配框架，将 3D 网格生成解耦为顶点流和条件拓扑流，两个阶段锚定到共享粗体素支架上，支持部件级高分辨率生成和拓扑自适应编辑。


<details>
  <summary>Details</summary>
Motivation: 现有拓扑感知网格生成方法将顶点和连接关系在联合潜空间中建模，导致连续顶点几何与离散组合结构相互纠缠，使得流学习困难，出现顶点漂移和表面断裂问题。

Method: 将网格生成分解为两个阶段：顶点流（vertex flow）生成顶点几何，然后连接性流（connectivity flow）在已生成顶点条件下预测拓扑结构。两个阶段均锚定到共享粗体素支架（voxel scaffold），并分别使用专用 VAE 将顶点恢复至亚体素精度、将离散连接性嵌入连续潜空间。

Result: LATO.2 在几何保真度和连接质量上超越了当前最先进的拓扑感知网格生成方法，并展示了部件级高分辨率生成和拓扑自适应编辑等独特优势。

Conclusion: 分解式流匹配框架有效解耦了网格生成中的几何与拓扑，提升了生成质量和可控性，为高质量 3D 网格生成提供了新范式。

Classification reason: 该论文提出 LATO.2，一种分解式流匹配框架用于拓扑感知的 3D 网格生成，属于 3D 物体/场景生成范畴，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Novel factorized flow matching for 3D mesh generation, but outside user's priority subtopics and negative keywords. Signals: 3D mesh generation, factorized flow matching, vertex and topology flow, part-wise generation, topology-adaptive editing
...
### [54] [Grassmannian Splatting I: Moving rank-2 Spacetime Surfels for Dynamic Scene Rendering](https://arxiv.org/abs/2607.10489)
...
TL;DR: 本文提出 Grassmannian splatting，一种动态场景表示方法，其原语是支持在时空 R^4 中 3-平面上的高斯分布（即 rank-2 时空 surfel）。每个原语携带单位法线 n ∈ S^3/{±1} 和未约束矩阵 L，协方差为 Σ_4D = (P_n L)(P_n L)^T。对时间条件化后每帧得到一个 rank-2 的 surfel（面片），其法线和速度由 n 读出，形状和切向漂移由 L 决定。该方法无需变形场、无需自定义 CUDA，通过预计算协方差接口直接喂入标准 3DGS 光栅化器。在 MonoDyGauBench 的 17 个 HyperNeRF 场景上，训练速度比最强质量基线快 4.9-5.6 倍，PSNR、MS-SSIM 和 LPIPS 排名第二。


<details>
  <summary>Details</summary>
Motivation: 现有原生 4D 高斯泼溅方法（如 Yang et al. 2023, Duan et al. 2024）对全秩时空协方差进行切片，每帧原语是体积椭球体（volumetric ellipsoid），而非更自然的平面 surfel。作者希望设计一种参数化方式，使得条件化后每帧自然得到 rank-2 surfel（面片），从而更高效地表示动态场景中的运动表面。

Method: 提出 Grassmannian splatting，原语为支持在时空 R^4 中 3-平面上的高斯分布。每个原语由单位法线 n ∈ S^3/{±1}（对应 Grassmannian Gr(3,4)）和未约束矩阵 L ∈ R^{4×3} 参数化，协方差 Σ_4D = (P_n L)(P_n L)^T，其中 P_n = I - nn^T 是到 n 正交补的投影。对时间条件化后每帧得到 rank-2 surfel，法线和速度由 n 读出，形状和切向漂移由 L 决定。使用 Schur 补分母中的 soft clamp 正则化静态方向，连续桥接 rank-3 静态和 rank-2 动态行为。无需变形场学习，无需自定义 CUDA，通过预计算协方差接口直接使用标准 3DGS 光栅化器。

Result: 在 MonoDyGauBench 的 17 个 HyperNeRF 场景上，训练速度在所有对比方法中最快（比最强质量基线快 4.9-5.6 倍），PSNR、MS-SSIM 和 LPIPS 指标排名第二。

Conclusion: Grassmannian splatting 通过 Grassmannian 参数化实现了高效的动态场景表示，每帧自然得到 rank-2 surfel 而非体积椭球体，在保持高质量渲染的同时大幅提升训练速度，且无需自定义 CUDA 实现，具有良好的实用性和可扩展性。

Classification reason: 该论文提出 Grassmannian splatting，一种基于 4D 时空高斯原语的动态场景渲染方法，属于动态 3D/4D 场景重建与渲染范畴，与 [CANONICAL] subtopic "4D 与动态场景生成" 高度匹配。

Importance reason: Novel dynamic scene representation with efficient training, but outside user's priority subtopics and lacks autoregressive/diffusion generation focus. Signals: Dynamic scene rendering, Grassmannian splatting, 4D Gaussians, Efficient training, No deformation field
...
### [55] [OmniX: Any-view and Any-time 4D Reconstruction via Feed-forward Trajectory Fields](https://arxiv.org/abs/2607.10840)
...
Importance: 普通相关 (55.13) - 未入选精读

TL;DR: OmniX 是一种前馈式 4D 重建框架，通过解耦动态运动建模与静态几何预测，利用紧凑的动态 token 表示运动，从大相机运动视频中预测稠密 3D 点轨迹，实现任意视角任意时间的动态场景重建。同时构建了基于 UE5 的大规模 4D 数据集（80K 场景、1.28M 多视角视频），在稠密 3D 点轨迹预测和 3D 点跟踪上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式 4D 重建方法要么预测逐帧静态点云而忽略前景运动，要么估计点云轨迹但局限于小相机运动，无法在大视角变化下聚合观测信息并重建完整动态场景。

Method: OmniX 将动态运动建模与静态几何预测解耦，使用一组紧凑的动态 token 表示运动，利用 3D 运动的稀疏低秩结构，为所有图像的所有像素生成轨迹场，同时高效保持全局交互。此外，构建了基于 UE5 的自动 4D 数据引擎，生成包含 80K 场景和 1.28M 多视角视频的大规模数据集。

Result: 在稠密 3D 点轨迹预测和 3D 点跟踪任务上达到 SOTA，在视频深度估计和相机姿态估计上也展现出有竞争力的结果。

Conclusion: OmniX 通过解耦运动与几何、紧凑动态 token 表示以及大规模合成数据，有效解决了大视角变化下动态场景的 4D 重建问题，为前馈式 4D 重建提供了新范式。

Classification reason: 该论文提出 OmniX，一种前馈式 4D 重建框架，从大运动视频中预测稠密 3D 点轨迹，实现任意视角任意时间的动态场景重建。这属于动态 3D/4D 场景生成与重建范畴，与 [CANONICAL] 的 "4D 与动态场景生成" 子主题高度匹配。

Importance reason: Strong 4D reconstruction work but outside user's priority subtopics and lacks direct connection to continuous language/multimodal models or autoregressive generation. Signals: 4D reconstruction, feed-forward trajectory fields, dynamic scene reconstruction, UE5 dataset, SOTA on 3D point tracking
...
### [56] [Latent-Identity Tuning in Text-to-Image Personalization Models](https://arxiv.org/abs/2607.11885)
...
TL;DR: 本文提出一种无需额外训练的潜在身份微调方法，利用预训练冻结编码器的潜在空间发现语义方向，实现细粒度的人脸身份编辑，同时保持跨图像的身份一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于通用文生图模型的个性化与编辑方法在细粒度人脸编辑上精度不足，微小改动可能显著改变身份感知。需要一种更精确的身份编辑方法。

Method: 探索预训练冻结编码器的潜在空间，利用一组潜在 token 捕获身份的不同方面（对应特定空间/语义面部区域），在该空间及选定 token 子空间中识别有意义的语义方向，实现局部、细粒度、语义一致的编辑。

Result: 定性和定量实验表明，该方法能够实现多样化的局部面部编辑，同时保持跨图像的身份一致性。

Conclusion: 提出了一种细粒度身份微调方法，无需额外训练即可在个性化文生图模型中进行精确的面部编辑，为身份保持与编辑提供了新的思路。

Classification reason: 该论文研究文本到图像个性化模型中的身份微调（identity tuning），属于主体驱动/概念定制生成（subject-driven/customization generation），与 [CANONICAL] subtopic "主体与概念驱动生成" 高度匹配。

Importance reason: Focuses on face identity editing, which is not a priority subtopic and lacks strong alignment with user's key directions like continuous models or world models. Signals: No training required, latent space editing, identity consistency, not aligned with priority subtopics, no boost keywords matched
...
### [57] [Exploring Agentic Workflows for Generating High Quality Math Visual Aids](https://arxiv.org/abs/2607.09839)
*Rizwaan Malik, Ashna Khetan, Isabel Sieh, Samin Khan*

Main category: cs.AI, cs.HC, cs.CV
...
TL;DR: 本文提出一种智能体工作流，利用LLM生成质量保证问题，再由VLM评估并迭代改进K-12数学教学图表的生成质量，初步验证了该方法能提升AI生成数学图示的准确性和教育价值。


<details>
  <summary>Details</summary>
Motivation: 当前AI工具（包括LLM）在生成准确且教学合理的数学图示方面表现不佳，即使提供详细描述也难以可靠生成。K-12数学教育中对高质量视觉辅助图的需求与现有AI生成能力之间存在显著差距。

Method: 提出一种智能体工作流：1) LLM根据视觉质量标准自动生成质量保证（QA）问题；2) VLM利用这些QA问题评估生成的图示质量；3) 将评估反馈给生成模型，迭代改进输出结果，形成自我改进循环。

Result: 探索性评估表明该工作流能提升AI生成数学图示的可靠性和教育价值，同时识别出关键改进方向，包括更强的空间推理能力和更全面的图示特征覆盖。

Conclusion: 智能体工作流结合LLM生成的QA问题与VLM的迭代评估反馈，有望提升AI生成数学图示的质量，但当前在空间推理和QA问题覆盖范围上仍有改进空间。

Classification reason: 该论文研究利用LLM智能体工作流迭代生成和改进数学教学图表（视觉辅助图），属于图像生成与可控编辑范畴。虽然论文侧重教育场景，但其核心是AI生成视觉内容的质量评估与迭代优化，与"图像与视频编辑"（visual_editing）中可控编辑、迭代改进的方向高度相关，因此复用该规范子主题。

Importance reason: Focuses on agentic workflows for math diagram generation, which is not aligned with user's priority subtopics on continuous multimodal models, video/image generation, or world models. Signals: Agentic workflow, LLM/VLM evaluation, K-12 math diagrams, iterative improvement, low relevance to continuous language/video generation
...
### [58] [h-Flow: Flexible Flow-based Image Editing via Doob's h-Transform](https://arxiv.org/abs/2607.10800)
...
Importance: 值得关注 (74.6) - 未入选精读

TL;DR: 本文提出h-Flow，一种基于Doob's h-Transform的免训练、理论驱动的流模型图像编辑框架。通过将图像编辑重构为多终端事件下的条件生成，设计源一致性和目标对齐的h函数，并引入速度正交分解实现重建与编辑方向的可控权衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练文生图流模型的图像编辑方法通常需要精心平衡目标对齐（与期望提示一致）和源一致性（与原始图像一致），且依赖反演管线或启发式源到目标轨迹构建，对架构设计和超参数敏感。作者希望提出一种免训练、理论严谨且灵活的编辑框架。

Method: 1) 将Doob's h-Transform从SDE模型扩展到确定性Rectified Flow（RF）框架，通过构造具有相同边际分布的等价SDE。2) 设计专门的h函数用于源一致性和目标对齐，得到闭式重建引导和基于速度的语义编辑信号。3) 引入速度正交分解，将重建方向和编辑方向解耦，实现两个目标之间的可控权衡。

Result: 大量实验表明，h-Flow在多种场景下实现了有效、鲁棒且灵活的编辑效果，优于现有方法。

Conclusion: h-Flow提供了一种免训练、理论扎实的流模型图像编辑新范式，通过Doob's h-Transform统一了源一致性和目标对齐，速度正交分解实现了灵活可控的编辑。

Classification reason: 该论文提出基于Doob's h-Transform的免训练图像编辑框架h-Flow，属于图像编辑方向，与[CANONICAL] subtopic "visual_editing (图像与视频编辑)" 高度匹配。

Importance reason: Strong theoretical contribution to flow-based editing but not aligned with user's priority subtopics. Signals: training-free, flow-based editing, Doob's h-Transform, image editing, theoretical framework
...
### [59] [OmniSCS: Omni Safety-Critical Scenario Synthesis for Autonomous Driving via a Fully Editable Driving World](https://arxiv.org/abs/2607.09764)
...
TL;DR: OmniSCS 提出了一种全可编辑驾驶世界构建与安全关键场景合成系统，通过双策略智能体重建和深度精化背景重建实现高保真场景编辑，支持实时闭环测试，在 nuScenes、Waymo、KITTI 上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有安全关键场景合成方法在场景编辑后难以保持数据保真度，无法高效生成高质量的安全关键场景用于自动驾驶闭环测试。

Method: 提出 OmniSCS 系统，包含两个核心模块：(1) 全可编辑驾驶世界构建模块，通过双策略智能体重建（外观+轨迹）和深度精化背景重建方法保持编辑后的高保真度；(2) 安全关键场景合成模块，支持物体插入和智能体轨迹编辑以合成多样化场景。

Result: 在 nuScenes、Waymo、KITTI 数据集上，OmniSCS 在编辑场景保真度上优于现有方法，并能以 13Hz 实时运行闭环测试，有效增强自动驾驶算法性能。

Conclusion: OmniSCS 为自动驾驶安全关键场景优化和测试提供了一种更安全、更有效且成本更低的解决方案。

Classification reason: 该论文聚焦自动驾驶安全关键场景合成，核心是通过编辑驾驶场景中的智能体外观和轨迹来生成逼真的场景，属于场景编辑与合成范畴，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度相关。

Importance reason: Focuses on autonomous driving safety scenario synthesis, not aligned with user's priority subtopics on continuous language/multimodal models or video generation. Signals: autonomous driving, safety-critical scenario synthesis, scene editing, closed-loop testing, nuScenes/Waymo/KITTI
...
### [60] [Controlling Motion Transfer in Diffusion Transformers via Attention Heads](https://arxiv.org/abs/2607.11081)
...
TL;DR: 本文分析了视频DiT中注意力头级别的运动与结构表征，发现存在专门负责运动和空间结构的注意力头。基于此，提出无需参数更新的头部感知可控运动迁移框架，通过语义对应引导从运动专用头中提炼运动线索，并通过选择性特征注入保留结构，实现了可解释的精确运动迁移。


<details>
  <summary>Details</summary>
Motivation: 扩散Transformer（DiT）在视频生成中取得了高质量、时序连贯的结果，但将其扩展到运动迁移任务（即跟随参考运动同时对齐目标提示）仍面临挑战，主要原因是对DiT中运动与结构表征的理解有限。

Method: 1) 在注意力头级别分析视频DiT，识别出专门负责运动和空间结构的注意力头。2) 提出无需参数更新的头部感知可控运动迁移框架：从运动专用头中通过语义对应引导提炼运动线索，同时通过选择性特征注入保留结构信息。

Result: 该方法实现了精确的运动迁移，同时为基于DiT的可控视频生成提供了可解释的基础。

Conclusion: 头部级别的控制不仅实现了准确的运动迁移，还为DiT的可控视频生成提供了可解释的理论基础。

Classification reason: 该论文研究视频DiT中的运动迁移（motion transfer），属于可控视频编辑范畴，与[CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度匹配。

Importance reason: Analyzes attention head specialization in video DiTs for motion transfer, offering interpretable control without parameter updates, but is not directly aligned with top priority subtopics. Signals: video diffusion transformer, attention head analysis, motion transfer, controllable generation, no parameter update
...
### [61] [GHOST: Geometry-Guided Hallucination of Opaque Surface Textures](https://arxiv.org/abs/2607.11118)
...
TL;DR: GHOST 是一个几何引导的预处理框架，利用视觉基础模型将透明物体区域转换为不透明、结构一致的纹理表示，从而提升下游深度估计和3D重建模型在透明物体上的性能，无需重新训练下游模型。


<details>
  <summary>Details</summary>
Motivation: 透明物体违反朗伯体假设，导致深度估计和3D重建方法在其表面产生严重的几何退化。现有方法需要重新训练或微调下游模型，成本高昂。因此，作者希望设计一个无需重新训练下游模型的预处理框架，将透明区域转换为不透明纹理表示，从而恢复光度线索。

Method: 提出GHOST框架，包含四个模块：(1) TransDINO 用于分离透明物体掩码；(2) TransDecomp 用于解耦透明度物理属性；(3) DAF-Net 恢复表面法线先验以编码几何曲率；(4) GeoSemTransNet 融合多模态线索合成纹理丰富的不透明RGB图像，保留透明物体的3D结构。

Result: 实验表明，GHOST显著提升了多种最先进的深度估计和3D重建模型在透明物体上的精度，通过恢复关键的光度线索改善了几何质量。

Conclusion: GHOST作为一种无需重新训练下游模型的预处理框架，有效解决了透明物体导致的几何退化问题，为透明物体深度估计和3D重建提供了一种实用且高效的解决方案。

Classification reason: 该论文提出一种几何引导的预处理框架，将透明物体区域转换为不透明纹理丰富的RGB图像，本质上是图像编辑/修复任务（将透明区域重渲染为有纹理的不透明表面），属于多模态生成方向下的图像编辑范畴，因此映射到[CANONICAL] subtopic "图像与视频编辑"。

Importance reason: Focuses on transparent object preprocessing for depth estimation, which is not aligned with user's priority subtopics on continuous language models, video/image generation, or world models. Signals: Transparent object depth estimation, Geometry-guided preprocessing, Visual foundation models, No downstream retraining, Not autoregressive or diffusion generation
...
### [62] [The Devil Is in the Leakage: A Disentangled Dual-Purification Framework for High-Fidelity Hairstyle Transfer](https://arxiv.org/abs/2607.11281)
...
TL;DR: 本文提出双净化框架（DPF），通过对抗性发型净化（AHP）和对比几何净化（CGP）两种训练时正则化方法，分别解决发型迁移中参考图像的身份泄露和秃头图像中的瑕疵泄露问题，实现了高保真、保持身份的发型迁移。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在零样本发型迁移任务中难以实现精确的局部编辑，参考发型常与原始身份和姿态纠缠；同时"先秃头再注入发型"的范式存在身份泄露和瑕疵泄露两大问题，导致生成结果保真度不足。

Method: 提出双净化框架（DPF），包含两个训练时正则化器：(1) 对抗性发型净化（AHP），基于互信息启发的对抗目标抑制发型特征中的身份可预测性；(2) 对比几何净化（CGP），通过对比学习目标正则化ControlNet路径，减少模型对秃头条件中几何伪影的依赖。

Result: 在多个基准数据集上达到最先进性能，有效解决了身份泄露和瑕疵泄露问题，实现了高保真、保持身份的发型迁移。

Conclusion: 通过联合净化发型表征和几何路径，DPF框架能够有效解决发型迁移中的泄露问题，实现高质量的身份保持发型迁移。

Classification reason: 该论文研究发型迁移（hairstyle transfer），属于图像编辑中的局部编辑任务，与 [CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度匹配。

Importance reason: Focuses on hairstyle transfer, a niche image editing task, with limited direct relevance to your priority subtopics on continuous models, video generation, or world models. Signals: Hairstyle transfer, identity leakage, dual-purification, diffusion-based pipeline, image editing
...
### [63] [Motion4Motion: Motion Transfer Across Subjects at Inference](https://arxiv.org/abs/2607.11644)
...
TL;DR: Motion4Motion 提出了一种无需训练的跨物种运动迁移框架，通过建模视频中角色的运动流（motion flow）而非骨架，实现了从人类到不同物种（如动物）之间的运动迁移，避免了骨架标注依赖和模型重训练。


<details>
  <summary>Details</summary>
Motivation: 现有视频运动迁移方法严重依赖预定义的人体骨架结构和骨架条件模型训练，难以泛化到不同物种的角色（如动物），且多样骨架的标注数据有限，限制了大规模训练。

Method: 提出 Motion4Motion 框架，跳脱基于骨架的运动迁移范式，转而建模视频中角色的运动流（motion flow），实现无需训练的跨物种运动迁移。

Result: 大量实验和新型应用表明，Motion4Motion 在跨物种运动迁移任务上显著优于基线方法。

Conclusion: 基于运动流的无训练框架可以有效解决跨物种运动迁移问题，为动画制作和数字内容创作提供了更通用的解决方案。

Classification reason: 该论文研究视频间的运动迁移（motion transfer），属于视频编辑/可控编辑范畴，与 [CANONICAL] subtopic "图像与视频编辑"（visual_editing）高度匹配。

Importance reason: Novel training-free motion flow approach, but lacks direct connection to user's priority subtopics like continuous models or autoregressive generation. Signals: training-free motion transfer, cross-species motion flow, no skeleton dependency, video motion transfer, inference-time method
...
### [64] [InterPet4D: A Multimodal 4D Human-Pet Interaction Dataset for Pet Motion Generation](https://arxiv.org/abs/2607.10287)
...
TL;DR: InterPet4D 是首个大规模多模态人-狗交互数据集，包含 680 万帧、13 只狗（11 个品种）与 23 名人类参与者的同步多视角视频、关键点、网格、音频等标注。基于该数据集提出的 InterPetMoGen 运动生成框架在 FID 11.21 上显著优于 Seq2Seq 和 DiT 基线。


<details>
  <summary>Details</summary>
Motivation: 人-宠物交互的估计与生成因缺乏高质量大规模数据集而研究不足，现有数据集多关注单人/多人或单动物运动，缺少同时捕捉人与宠物自然交互的多模态数据。

Method: 构建同步多视角捕捉系统录制人-狗服从任务数据，提供多视角/第一人称视频、分割、2D/3D关键点、网格和音频标注。提出 InterPetMoGen 框架用于人-宠物交互运动生成。

Result: InterPetMoGen 在 FID 指标上达到 11.21，显著优于 Seq2Seq 和 DiT 基线方法，验证了数据集对建模真实人-宠物交互的有效性。

Conclusion: InterPet4D 填补了人-宠物交互数据集的空白，为运动生成、交互理解等任务提供了重要资源，InterPetMoGen 展示了该数据集在生成真实交互运动方面的潜力。

Classification reason: 该论文提出人-宠物交互运动生成数据集与框架，属于动作生成范畴。现有 [CANONICAL] subtopic "数字人与动作生成"（avatar_motion_generation）涵盖人体动作生成，人-宠物交互运动生成可合理归入该大类。

Importance reason: Dataset and motion generation for human-pet interaction is niche and lacks direct alignment with your priority subtopics on continuous multimodal models, video generation, or world models. Signals: New multimodal interaction dataset, Pet motion generation, Outperforms Seq2Seq/DiT baselines, No direct link to continuous language/multimodal models or world models, Low priority subtopic match
...
### [65] [EquiFusion: Kinematics-Agnostic Human Motion Prediction via Equivariant Latent Diffusion](https://arxiv.org/abs/2607.10984)
*Cecilia Curreli, Florian Hofherr, Dominik Muhle, Abhishek Saroha, Riccardo Marin, Daniel Cremers*

Main category: cs.LG, cs.AI, cs.HC, cs.CV
...
TL;DR: EquiFusion 提出首个运动学无关的随机3D人体运动预测模型，通过置换等变潜扩散架构将骨骼连接作为显式输入参数，实现了跨数据集泛化、零样本部分观测预测和定向肢体生成，在多个基准上达到SOTA，且模型体积缩小75%。


<details>
  <summary>Details</summary>
Motivation: 现有随机3D人体运动预测模型将骨骼运动学信息硬编码到模型中，严重限制了泛化能力，无法跨数据集训练，且需要复杂的数据重定向。作者希望打破这一瓶颈，实现运动学无关的人体运动预测。

Method: EquiFusion 采用潜扩散模型结合置换等变架构，将运动学连接信息作为显式输入参数，使内部计算对关节顺序和图结构天然无关。模型在潜空间中进行扩散生成，并通过置换等变设计确保对不同骨骼结构的泛化能力。

Result: 在多个主流基准上达到SOTA性能，模型体积比此前运动学特定方法缩小75%，训练和推理速度更快。实现了跨数据集泛化到未见过的运动学结构，以及从部分/遮挡观测进行运动预测和定向肢体生成等零样本能力。

Conclusion: EquiFusion 建立了灵活、鲁棒的人体运动预测新标准，证明了运动学无关设计在人体运动预测中的可行性和优越性，为未来跨数据集、跨场景的人体运动建模开辟了新方向。

Classification reason: 该论文研究随机3D人体运动预测，属于人体动作生成范畴，与[CANONICAL] subtopic "数字人与动作生成" (avatar_motion_generation) 高度匹配。

Importance reason: Strong technical contribution in motion prediction but lacks direct alignment with user's priority subtopics like continuous language models or video generation. Signals: kinematics-agnostic, permutation equivariant, latent diffusion, cross-dataset generalization, zero-shot partial observation
...
### [66] [HandFlow: Fully Generative 4D Hand Recovery with Flow Matching](https://arxiv.org/abs/2607.11221)
...
TL;DR: HandFlow提出了一种基于flow matching的全生成式框架，用于从单目视频中恢复时间一致的手部3D姿态与形状。它通过单个ODE积分去噪整个时间窗口的MANO参数，使用Flux风格的双流Transformer捕获长程依赖，并引入置信度感知的连续掩码机制处理遮挡和模糊。在DexYCB和HOT3D上达到SOTA，世界空间姿态误差降低30%以上，加速误差最低，且推理速度达47fps，比最快的先前方法快约12倍。


<details>
  <summary>Details</summary>
Motivation: 现有单目4D手部重建方法存在两大问题：逐帧判别式回归器缺乏时序上下文，预测结果抖动严重；时序模型虽能聚合帧间信息，但仍是确定性回归器，在遮挡和运动模糊等模糊观测下表现脆弱。生成式建模通过学习合理手部运动序列的先验，能在视觉证据不完整时恢复连贯的手部状态，因此作者提出全生成式框架来解决这一问题。

Method: HandFlow采用flow matching框架，将整个时间窗口的MANO参数（手部姿态和形状）视为连续潜变量，通过单个ODE积分从噪声去噪得到完整序列。模型使用Flux风格的双流Transformer，在完整序列上进行双向注意力以捕获长程依赖（无需自回归解码）。同时提出置信度感知的连续掩码机制，将观测特征与可学习的掩码token按置信度混合，以处理噪声或缺失的观测。视觉特征来自预训练的手部骨干网络，骨骼观测通过可选的骨骼编码器注入。

Result: 在DexYCB和HOT3D数据集上，HandFlow达到SOTA性能。世界空间姿态误差相比最强基线降低30%以上，加速误差在所有评估方法中最低，同时在逐帧姿态精度上保持竞争力。在单GPU上，HandFlow以47fps的速度重建150帧序列，比最快的先前视频方法快约12倍。

Conclusion: HandFlow验证了全生成式flow matching框架在4D手部重建中的有效性，通过生成式先验和时序建模显著提升了遮挡和模糊场景下的重建质量与时间一致性，同时实现了极高的推理效率。

Classification reason: 该论文关注从单目视频中生成4D手部姿态与形状（MANO参数），属于数字人与动作生成范畴。虽然使用了flow matching生成式框架，但核心任务是手部动作/姿态重建与生成，与[CANONICAL] subtopic "数字人与动作生成"（avatar_motion_generation）高度匹配。

Importance reason: Novel flow-matching generative approach for 4D hand recovery, but outside user's priority subtopics and direction weights. Signals: flow matching, generative hand reconstruction, temporal consistency, confidence-aware masking, 47fps inference
...
### [67] [Toward Inclusive Avatar Design with Limb Differences Through Artificial Intelligence](https://arxiv.org/abs/2607.11512)
*Fernanda Miyuki Yamada, João Paulo Gois, Hiroki Takahashi*

Main category: cs.AI, cs.HC, cs.GR
...
TL;DR: 本文综述了面向肢体差异人群的包容性3D虚拟人生成技术，分析了现有系统在非标准身体形态建模与动画方面的局限性，并指出人工智能是克服数据稀缺和动画限制的关键路径。


<details>
  <summary>Details</summary>
Motivation: 随着扩展现实（XR）在社交和娱乐中的普及，3D虚拟人需要能够代表多样化的身体类型。然而，现有虚拟人系统仅支持标准身体形态，无法准确表现肢体差异、截肢等形态变异，导致包容性不足。

Method: 本文采用综述方法，系统梳理了包容性3D虚拟人定制的新兴技术方法，包括基于AI的生成方法，以及促进尊重和准确表现肢体差异人群的指导方针，同时分析了当前面临的挑战。

Result: 识别出两大核心挑战：1）缺乏多样化的肢体差异数据集；2）非标准解剖结构的动画技术存在局限。提出人工智能（如生成模型）是克服这些限制、推进包容性虚拟人生成的有前景方向。

Conclusion: 包容性虚拟人设计需要AI技术的介入来解决数据稀缺和动画适配问题，未来应加强跨学科合作，推动更公平、更具代表性的虚拟人生成。

Classification reason: 该论文关注3D虚拟人（avatar）的包容性定制，特别是针对肢体差异人群的生成与动画，属于数字人与动作生成（avatar_motion_generation）这一[CANONICAL]子主题的范畴。

Importance reason: Low relevance to priority subtopics and negative keywords not triggered, but offers niche value in inclusive avatar generation. Signals: inclusive avatar design, limb differences, 3D avatar generation, data scarcity, AI-driven animation
...
### [68] [EmoStyle: Affective Conditioning of Style-Specialist Experts for Emotional Image Generation](https://arxiv.org/abs/2607.10165)
...
TL;DR: EmoStyle 提出一种情感条件化的艺术图像生成框架，利用 LLM 预测情感线索（效价-唤醒度、主导情绪、治疗效果标签）并编码为情感条件向量，通过 AdaLN 调制注入扩散模型的去噪块；同时为每种艺术风格训练专用 LoRA 专家，推理时选择对应专家渲染情感线索；最后用轻量 VLM 对生成图像进行候选排序。该方法在 AffectiveArt Challenge 2026 Track 1 中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 情感感知的艺术图像生成需要同时满足三个条件：匹配输入提示、遵循指定艺术风格、传达目标情感。然而训练数据中的视觉和情感属性在测试时不可用，导致生成器无法精确控制色彩、光照、笔触、构图等细粒度条件来表达目标情感。现有方法缺乏将情感线索有效注入生成过程的手段。

Method: 1) LLM 推理器从输入提示预测情感线索（效价-唤醒度、主导情绪、治疗效果标签）和宽高比决策；2) 将情感线索编码为情感条件向量，通过 AdaLN 风格调制注入扩散模型的去噪块；3) 为每种艺术风格桶训练专用 LoRA 适配器，推理时选择对应专家；4) 轻量 VLM 对生成图像进行候选排序，评估提示对齐、风格一致性、情感表达和视觉质量。

Result: 在 AffectiveArt Challenge 2026 Track 1 中获得第一名，验证了情感条件向量注入和风格专用 LoRA 专家的有效性。

Conclusion: EmoStyle 通过结构化生成状态（LLM 预测情感线索 + 风格专用 LoRA）有效弥合了测试提示与情感感知艺术生成所需细粒度条件之间的控制差距，为情感可控的图像生成提供了有效框架。

Classification reason: 该论文提出 EmoStyle，基于扩散模型（Stable Diffusion）进行情感感知的艺术图像生成，属于文本到图像生成范畴，因此映射到 [CANONICAL] 文生图与图像扩散（text_to_image_diffusion）。

Importance reason: Focuses on emotional conditioning for artistic image generation, which is niche and not aligned with user's priority subtopics like continuous models or video generation. Signals: Emotional image generation, AffectiveArt Challenge, LLM predicts affective cues, LoRA style experts, VLM candidate selection
...
### [69] [Spectral Consistent Flow for One-step 3D Medical Image Translation](https://arxiv.org/abs/2607.10627)
...
Importance: 值得关注 (72.71) - 未入选精读

TL;DR: SC-Flow 提出了一种基于谱一致性校正的单步潜空间流匹配框架，用于 3D 医学图像跨模态翻译，通过可学习的频域增益调制动态正则化功率谱密度演化，在保持全局结构一致性的同时恢复细粒度解剖细节。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 医学图像翻译方法通常需要多步采样，效率低下；且潜空间中的平均速度场存在隐式低通调制问题，导致模态纠缠、过平滑和伪影，难以同时保持全局结构一致性和细粒度解剖细节。

Method: 将医学图像翻译建模为随机布朗桥过程，在潜空间中通过预测支持正则化平均速度场直接构建源域到目标域的映射（1-NFE）。引入谱一致性校正器（Spectral Consistency Corrector），通过可学习的频域增益调制动态正则化功率谱密度演化，建立空间纹理与谱能量流之间的显式桥梁。

Result: 在四个数据集上的实验表明，SC-Flow 在各种翻译场景下均实现了显著更准确、一致和鲁棒的性能，优于现有多步方法。

Conclusion: SC-Flow 通过谱一致性校正有效解决了潜空间流匹配中的低通调制问题，实现了单步高效且高质量的 3D 医学图像翻译，为医学图像跨模态生成提供了新范式。

Classification reason: 该论文提出 SC-Flow，在潜空间中使用布朗桥过程与流匹配实现 3D 医学图像翻译（跨模态生成），属于图像到图像的生成方法。虽然具体应用为医学图像，但其核心方法是流匹配（flow matching）与扩散/流模型在潜空间中的图像翻译，与 canonical 子主题"文生图与图像扩散"（text_to_image_diffusion）中的扩散模型、流匹配方法高度相关，因此复用该子主题。

Importance reason: Focuses on 3D medical image translation, which is outside your priority subtopics and flagged by negative keywords. Signals: 3D medical image translation, spectral consistency, one-step flow matching, low personal relevance due to domain mismatch
...
### [70] [Velocity Scheduled Flow Matching](https://arxiv.org/abs/2607.11442)
*Vitalii Bondar*
...
TL;DR: 本文提出 Velocity Scheduled Flow Matching (VSFM)，通过引入非均匀速度调度曲线（如减速/加速曲线）替代 flow matching 中默认的匀速直线插值，使得在相同 NFE 下采样质量显著提升。VSFM 可在推理时直接应用于预训练模型（无需重训），也可从头训练，在 CIFAR-10 上分别降低 FID 达 19.8% 和 17.4%。


<details>
  <summary>Details</summary>
Motivation: 标准 flow matching 使用匀速直线插值，这隐含了采样过程中速度恒定的假设。然而，从数值积分角度看，不同区域对误差的敏感度不同，匀速策略并非最优。作者希望引入可调的速度调度曲线，使模型在误差敏感区域放慢速度、在平坦区域加快速度，从而在相同计算预算下提升生成质量。

Method: VSFM 将条件速度目标从固定的 $x_1 - x_0$ 替换为 $v(t)(x_1 - x_0)$，其中 $v(t)$ 是满足 $\int_0^1 v\,dt = 1$ 的非负速度调度函数。作者研究了六种多项式速度曲线（如匀速、减速、加速、先加速后减速等）。推理时，预训练模型可通过在非均匀 $\tau$-schedule 上积分 ODE 来适配任意速度曲线，无需重训。训练时，模型直接学习带速度调度的条件速度场。

Result: 在 CIFAR-10 上，推理时应用 VSFM（减速曲线）使 FID 降低最多 19.8%（10 NFE 下从 6.82 降至 5.47）。从头训练 VSFM 模型在 4 NFE 下 FID 降低 17.4%（从 11.49 降至 9.49）。分析表明，减速曲线（braking profile）在早期步长较大、后期步长较小，有效降低了 Euler 积分的局部截断误差。

Conclusion: VSFM 是一种简单而有效的 flow matching 改进方法，通过速度调度在不增加计算量的前提下提升采样质量。该方法兼容推理时零开销适配和训练时端到端优化，具有较好的实用价值。未来可探索更复杂的自适应速度调度策略。

Classification reason: 该论文提出 Velocity Scheduled Flow Matching (VSFM)，通过速度调度曲线优化 flow matching 的采样步数效率，在少步（如 4 NFE）下显著降低 FID。这属于生成模型采样加速与高效化的核心范畴，因此映射到 [CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Directly improves flow matching efficiency, a core technique for continuous language/multimodal models and video generation, with immediate inference-time gains. Signals: flow matching acceleration, inference-time plug-in, CIFAR-10 FID reduction, non-uniform velocity scheduling, no retraining required
...
### [71] [Diversify Diffusion with Temperature Sampling and Variance-Corrective Time Shifting](https://arxiv.org/abs/2607.10853)
...
TL;DR: 本文提出方差校正时间偏移（Variance-Corrective Time Shifting），一种无需重新训练的方法，通过对预训练扩散/流匹配模型进行温度采样并校正方差膨胀，在保持样本质量的同时显著提升生成多样性，支持从粗到细的多样性控制。


<details>
  <summary>Details</summary>
Motivation: 扩散模型忠实再现训练分布，但也继承了其中的不平衡性，导致稀有或欠表示的模式难以生成。现有的温度采样方法虽然能重新加权模式，但会膨胀每个模式的方差，破坏逆向扩散过程，降低样本质量。因此需要一种训练无关的修正方法，在保持模式重加权的同时消除方差膨胀。

Method: 提出方差校正时间偏移（Variance-Corrective Time Shifting）：在推理时，对预训练扩散/流匹配模型，查询一个偏移时间步的噪声预测网络，并将得到的分数乘以γ，从而抵消方差膨胀同时保留模式重加权效果。该方法无需重新训练，可应用于DiT、Stable Diffusion和Motion Diffusion等多种骨干网络。此外，通过控制温度干预的时间阶段（高噪声阶段驱动组合多样性，低噪声阶段驱动局部外观变化），实现从粗到细的多样性控制。

Result: 在DiT、Stable Diffusion和Motion Diffusion模型上，该方法在保持样本质量和条件保真度的同时，一致地提升了生成多样性。实验表明，方差校正时间偏移有效解决了温度采样导致的方差膨胀问题，且粗到细的时序控制策略能够灵活调节组合多样性与局部外观多样性。

Conclusion: 本文提出的方差校正时间偏移是一种简单有效的训练无关方法，可将温度采样转化为实用的多样性控制工具，适用于多种预训练扩散和流匹配模型，在几乎不损失样本质量的前提下显著提升生成多样性。

Classification reason: 该论文提出了一种无需重新训练的推理时采样方法（温度采样+方差校正时间偏移），用于提升扩散模型和flow-matching模型的生成多样性。这属于生成模型采样过程的效率与可控性优化，与[CANONICAL] subtopic "生成模型加速与高效化"（efficient_generation）高度相关，该子主题涵盖扩散加速、少步采样、蒸馏等采样优化方法。

Importance reason: Directly addresses diversity control in diffusion/flow models, a key challenge for high-quality generation, with a training-free method that aligns with user's focus on efficient inference and controllable generation. Signals: training-free diversity enhancement, temperature sampling for diffusion/flow, variance correction, coarse-to-fine control, applicable to DiT, Stable Diffusion, Motion Diffusion
...
### [72] [HyperGS: Fast and Generalizable Gaussian Video Representation](https://arxiv.org/abs/2607.11500)
...
TL;DR: HyperGS 是一种前馈式、无需逐视频优化的高斯视频表示方法，通过分解式时空 Transformer 和可学习查询 Transformer 直接从任意视频预测高斯参数，实现零样本泛化到高分辨率视频，编码速度比逐视频优化快 10^4-10^5 倍，同时保持相当的重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于高斯泼溅的视频表示方法需要逐视频优化，编码速度慢且无法跨视频泛化。为了克服这一限制，作者希望设计一种前馈式方法，能够直接从任意视频预测高斯表示，实现快速编码和解码，并泛化到分布外的高分辨率视频。

Method: HyperGS 设计了一个分解式时空 Transformer 从视频中提取 token，然后使用可学习查询 Transformer 为每帧预测 8 参数高斯表示。针对直接预测高斯参数时出现的"针状退化"问题（导致训练崩溃），提出了一种基于秩的几何正则化器，其强度可动态自适应调整以稳定优化。

Result: 在 K400、SSv2 和 UCF101 数据集上，HyperGS 在更小的视频表示尺寸下，PSNR 比之前的视频编码器高出 +2.9-3.1 dB。编码速度比逐视频高斯优化快 10^4-10^5 倍，且能零样本泛化到 720p 视频，无需重新编码即可实现更高分辨率渲染。

Conclusion: HyperGS 将高斯泼溅的快速灵活渲染与前馈预测的速度和泛化能力相结合，推进了高斯表示作为快速、可泛化视频表示的实际方向。

Classification reason: 该论文提出 HyperGS，一种前馈式、无需逐视频优化的高斯视频表示方法，显著加速了视频编码与解码（10^4-10^5倍），属于生成模型加速与高效化方向，因此映射到 [CANONICAL] subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Directly addresses efficient video representation with feedforward Gaussian prediction, aligning with user's high priority on model efficiency and video generation. Signals: feedforward Gaussian video representation, 10^4-10^5x speedup over per-video optimization, zero-shot generalization to 720p video, factorized spatiotemporal Transformer, rank-based geometric regularizer
...
### [73] [Adversarially Guided Diffusion for LiDAR Range Image Synthesis](https://arxiv.org/abs/2607.09787)
...
TL;DR: 本文提出首个基于扩散模型的无限制对抗攻击方法，利用分割损失作为对抗引导，在LiDAR距离图像上生成无限制对抗样本，实现对RangeNet++和CENet分割网络的攻击，并提供了可调节的有效性-真实性权衡。


<details>
  <summary>Details</summary>
Motivation: LiDAR语义分割是自动驾驶中的关键感知任务，错误预测会影响下游规划与安全决策。现有对抗攻击研究多集中于图像分类和3D点云分割，而针对2D距离图像（3D点云的投影）的无限制对抗攻击尚未被探索。

Method: 提出基于扩散模型的无限制对抗攻击方法，在采样过程中直接使用分割损失作为对抗引导（adversarial guidance），生成接近LiDAR数据流形但诱导结构化分割错误的对抗样本。在SemanticKITTI数据集上使用RangeNet++和CENet进行实验。

Result: 该方法在引导强度可调节的情况下提供可控的白盒攻击与迁移攻击效果，相比FGSM和SegPGD基线方法，在有效性与真实性之间实现了更好的权衡，同时保持了具有竞争力的分布真实性和视觉真实性。

Conclusion: 扩散模型结合对抗引导可以生成高质量的无限制对抗样本，为LiDAR距离图像分割的安全性评估提供了新工具，且攻击效果可跨分割架构迁移。

Classification reason: 该论文提出基于对抗引导扩散的LiDAR距离图像合成方法，属于生成模型中的可控生成与引导（guidance）技术，与[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）中的guidance与可控性方向高度匹配。

Importance reason: Focuses on adversarial attacks for LiDAR segmentation, which is outside the user's priority subtopics and negative-keyword domains. Signals: LiDAR range image synthesis, adversarial attack, diffusion model, segmentation, autonomous driving
...
### [74] [Personalized Emotional Intelligence in Generative AI through Symbolic Affective Reasoning](https://arxiv.org/abs/2607.10678)
...
TL;DR: EROS是一个混合AI框架，结合符号推理与深度学习，通过视觉内容实现个性化情感增强。它从大规模图像-情感数据集中学习通用情感规则，识别情感相关区域，预测保持场景语义的同时引导情感响应的视觉修改，并利用可扩展记忆库实现推理时个性化。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型主要局限于生成逼真内容或进行语义推理，缺乏理解、预测和个性化人类情感响应的能力。作者希望构建能够理解、推理并增强人类认知状态的AI系统。

Method: EROS框架包含三个核心组件：(1) 从大规模图像-情感数据集中发现可泛化的情感规则；(2) 识别图像中与情感相关的区域；(3) 预测上下文感知的视觉修改，在保持场景语义的同时将情感响应导向目标状态。此外，EROS包含可扩展的记忆库，支持推理时个性化而无需微调模型。

Result: 在广泛的人类心理物理学实验中，EROS在引发目标情感响应方面优于最先进的大型多模态模型，并能适应个体情感偏好。

Conclusion: EROS为能够理解、推理和增强人类认知状态的AI系统奠定了基础，在心理健康、自适应媒体、教育和人机交互等领域具有潜在应用价值。

Classification reason: 该论文提出EROS框架，通过符号情感推理实现个性化情感增强的图像生成，核心是控制生成内容以引发目标情感响应，属于生成模型的可控性与个性化对齐，因此映射到[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）。同时涉及多模态理解与生成方向。

Importance reason: Focuses on emotional augmentation via symbolic reasoning, which is outside the user's core priority subtopics and lacks direct connection to continuous language models, video/image generation, or world models. Signals: Symbolic reasoning, emotional augmentation, hybrid AI, personalization, no continuous token/video generation/world model keywords
...
### [75] [Read It Back: Pretrained MLLMs Are Zero-Shot Reward Models for Text-to-Image Generation](https://arxiv.org/abs/2607.11886)
...
TL;DR: 本文提出 SpectraReward，一种无需训练的奖励函数，将预训练 MLLM 直接用作文生图强化学习的奖励模型。通过计算生成图像条件下原始提示的平均对数似然作为奖励，无需偏好标签或微调。进一步提出 Self-SpectraReward，让统一多模态模型的理解分支为生成分支提供奖励，形成闭环自改进框架。实验覆盖多种扩散模型、RL 算法和 MLLM 骨干，显著提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有文生图强化学习依赖人工偏好标注或额外训练的奖励模型，成本高且难以扩展。如何利用预训练 MLLM 已有的图文对齐能力，无需额外训练即可作为奖励模型，是本文要解决的核心问题。

Method: SpectraReward 的核心思想是"读回"（Read It Back）：给定生成图像，用 MLLM 计算在该图像条件下原始文本提示的平均对数似然（teacher-forcing），以此作为奖励分数。这直接复用 MLLM 预训练的图文对齐能力，无需偏好数据或奖励模型微调。Self-SpectraReward 则针对统一多模态模型（如 Show-o），让模型自身的理解分支作为生成分支的奖励模型，形成闭环自我改进。

Result: 在两种扩散模型（SD1.5、SDXL）、三种 RL 算法（ReFL、DPO、SPIN-Diffusion）、九种 MLLM 骨干（4B-235B）以及五个 OOD 文生图基准上，SpectraReward 和 Self-SpectraReward 均显著且一致地提升了生成性能，超越了此前基于 MLLM 的奖励训练方法。分析发现更大的奖励 MLLM 并不总是更好，而 Self-SpectraReward 可匹配或超越更大的外部奖励模型。

Conclusion: 本文证明了预训练 MLLM 无需任何训练即可作为有效的文生图奖励模型，且统一多模态模型可实现闭环自我奖励改进。奖励-策略对齐是图像生成强化学习的关键因素，为未来无需外部奖励模型的生成对齐研究提供了新方向。

Classification reason: 该论文提出 SpectraReward，利用预训练 MLLM 作为文生图强化学习的零样本奖励模型，核心贡献在于奖励建模与生成对齐，属于 [CANONICAL] 生成模型对齐与奖励 (generation_alignment) 范畴。

Importance reason: Proposes a training-free reward method for text-to-image generation that aligns with user's focus on unified multimodal models and efficient inference. Signals: training-free reward, MLLM as reward model, text-to-image generation, self-improving framework, unified multimodal model
...
### [76] [ChartSync: A Benchmark for Visuo-Logical Cascading Chart Editing](https://arxiv.org/abs/2607.10301)
...
TL;DR: 本文提出 ChartSync，一个专家验证的图表级联编辑基准，包含870个三元组、9种图表类型和4类任务，用于评估图像编辑模型在统计图表上的视觉-逻辑级联编辑（VLCE）能力。评估14个图像编辑模型和一个代码中介管线后发现，仅两个前沿闭源模型展现出初步的VLCE能力，开源模型在几何同步方面存在严重缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有的图像编辑模型在处理结构化统计图表时，当数据修改需要几何同步时表现不佳。现有方法局限于局部文本替换，无法处理依赖感知的级联更新。因此需要系统性地评估这一能力。

Method: 通过程序化渲染管线构建 ChartSync 基准，包含870个三元组（源图、指令、目标图），覆盖9种图表类型和4类任务（包括235个几何耦合的VLCE实例）。采用双层评估框架：客观视觉指标 + VLM judge 范式，评估低层保真度和多模态理解推理能力。

Result: 评估14个图像编辑模型和1个代码中介管线发现：大多数开源模型在几何同步方面严重下降，仅两个前沿闭源模型（GPT-4o和Gemini 2.0 Flash）展现出初步的VLCE能力，但其残留错误主要涉及语义隔离和背景损坏。

Conclusion: 图表级联编辑是一个具有挑战性的任务，现有模型存在显著能力差距。详细的错误分析揭示了失败范式，为未来多模态架构设计提供了指导方向。

Classification reason: 该论文提出了 ChartSync 基准，专门用于评估图像编辑模型在统计图表上的级联编辑能力，属于生成模型评测与数据集范畴。该论文同时涉及多模态理解（VLM judge）和图像编辑生成，因此匹配 multimodal_generation 和 multimodal_understanding_generation 两个方向。subtopic 直接复用 [CANONICAL] 的 generation_benchmark。

Importance reason: Focuses on chart editing benchmark, not directly aligned with user's priority on continuous multimodal models or video/image generation. Signals: Chart editing benchmark, Visuo-Logical Cascading Editing, image editing evaluation, limited to statistical charts, not in priority subtopics
...
### [77] [3D-DefectBench: A Controlled Factorial Study of Vision-Language Model Evaluation Pipelines for Fine-Grained 3D Generation Defects](https://arxiv.org/abs/2607.10826)
...
Main category: cs.AI, cs.GR, cs.CV
...
TL;DR: 本文提出3D-DefectBench，一个用于系统分析基于VLM的3D缺陷检测流水线的基准和框架。通过平衡因子设计，变化VLM、相机协议、视觉输入和提示模式四个流水线因素，在84种推理设计和约320万次缺陷决策上评估，发现模型选择是影响与人类标签一致性的最大因素，但其他因素也显著影响性能。紧凑的六视图RGB协议在成本效益上表现优异，但最佳VLM评判器仍落后于训练有素的人类标注员。


<details>
  <summary>Details</summary>
Motivation: 自动化评估对于规模化3D生成系统至关重要，但现有研究仅关注VLM模型本身，忽略了渲染方式、视觉证据、任务规范和人类参考标签构建等整个评估流水线对可靠性的影响。缺乏系统性的基准来全面分析这些因素。

Method: 提出3D-DefectBench基准，涵盖几何、纹理和提示遵循性三类共9种细粒度二值缺陷。采用平衡因子设计，变化VLM（12种）、相机协议（单视图/多视图）、视觉输入（RGB/深度/法线）和提示模式四个因素，在84种推理设计上进行约320万次缺陷决策评分，并在更广泛的前沿模型上进行分阶段验证。

Result: 模型选择是与人类标签一致性最大的决定因素，但其他因素也影响性能并与模型选择交互。紧凑的六视图RGB协议与更密集的多视图设置及深度/法线增强输入性能相当。在标准化流水线下，最佳VLM评判器仍落后于训练有素的人类标注员；当专家共识标签被替换为噪声更大的银标签时，纹理一致性急剧下降。

Conclusion: 自动化评判器应作为完整流水线进行评估，并在不同人类参考标注体系下进行校准，而不应仅作为独立模型进行基准测试。研究为3D生成缺陷检测的自动化评估提供了系统性的方法论指导。

Classification reason: 该论文提出了3D-DefectBench基准，系统分析基于VLM的3D生成缺陷检测流水线，属于生成模型评测与数据集方向。该论文同时涉及多模态理解与生成（使用VLM作为评判器），但核心贡献是3D生成质量评测基准，因此主方向为多模态生成，匹配多模态理解与生成。

Importance reason: Focuses on VLM evaluation pipelines for 3D defect detection, which is peripheral to the user's core interests in continuous multimodal models and autoregressive generation. Signals: 3D defect detection, VLM evaluation pipeline, factorial design, not continuous language model, not autoregressive generation
...
### [78] [AU-Guided Synthetic Video Generation for Micro-Expression Recognition](https://arxiv.org/abs/2607.10860)
...
Importance: 值得关注 (66.59) - 未入选精读

TL;DR: 本文提出 EquiME，一个基于 AU（动作单元）引导的图像到视频生成构建的大规模合成微表情数据集，包含 75K 视频，覆盖 5 种目标情绪，并附带人口统计元数据和质量评估。跨数据集实验表明，在 EquiME 上训练的模型在 SAMM 和 CASME II 上取得了有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有微表情识别（MER）数据集规模小、人口覆盖窄、情绪标签受限，限制了模型的泛化能力。本文旨在通过合成数据生成来缓解这一问题。

Method: 提出 AU-guided 图像到视频生成流水线，从 15K 源人脸图像出发，基于动作单元（AU）条件控制生成 75K 微表情视频，覆盖 5 种目标情绪。同时自动推断人口统计元数据并计算视频质量指标。

Result: EquiME 数据集在帧对相似度、空间变化和无参考感知质量指标上表现良好。跨数据集实验（SAMM 和 CASME II）显示，基于 EquiME 训练的模型取得了有竞争力的性能，且在四种不同架构上性能波动较小。

Conclusion: EquiME 作为合成微表情数据集资源，能够有效缓解真实数据稀缺问题，为 MER 研究提供大规模、多样化的训练数据。AU 引导的生成流水线是构建高质量合成微表情视频的有效方法。

Classification reason: 该论文主要贡献是构建了一个名为 EquiME 的合成微表情数据集（75K 视频），属于生成模型的数据集构建与评测工作，因此映射到 [CANONICAL] subtopic "生成模型评测与数据集"（generation_benchmark）。

Importance reason: Focuses on micro-expression dataset generation, which is not a priority subtopic and lacks direct connection to user's core interests in continuous multimodal models or video generation world models. Signals: AU-guided video generation, synthetic dataset for micro-expression, cross-dataset evaluation, limited emotion labels, no direct link to continuous language or world model
...
### [79] [DynEval: Holistic Evaluations of T2I Generative Models in the Wild](https://arxiv.org/abs/2607.11199)
...
TL;DR: 本文提出DynEval，一个面向文生图模型的动态评测框架。通过构建50万对GenDB数据集和25万条DynEvalInstruct指令数据集，采用课程学习策略从大教师VLM蒸馏出轻量级评测器（2B/4B），在11个基准上实现了与人类判断更高的相关性，并对36个T2I模型在42个子类别和9个语义维度上进行了细粒度分析。


<details>
  <summary>Details</summary>
Motivation: 现有T2I生成模型的自动评测器通常依赖静态提示集，难以捕捉部分提示未对齐、组合错误、视觉合理但语义不正确等细粒度失败模式，且缺乏可扩展的标注数据。

Method: 1) 构建GenDB数据集：从DiffusionDB中选取人工编写的提示，采用分层提示-模型生成策略生成50万对提示-图像数据。2) 构建DynEvalInstruct数据集：基于GenDB，通过结构化评测流水线（分解为文本-图像对齐和视觉质量推理两个维度）蒸馏出25万条提示-图像-响应三元组。3) 使用课程学习策略对轻量级VLM进行全参数微调，从大教师模型蒸馏评测能力，得到DynEval-2B和DynEval-4B。

Result: 在11个基准上与人类判断的相关性全面优于现有评测器；对36个T2I模型在42个子类别和9个语义维度上进行了细粒度能力与失败模式分析。

Conclusion: DynEval通过动态评测框架和大规模数据蒸馏，有效提升了T2I生成模型的自动评测质量，能够提供细粒度的多维度分析，为生成模型的可靠评估提供了新范式。

Classification reason: 该论文提出T2I生成模型的动态评测框架DynEval，构建了GenDB和DynEvalInstruct两个大规模数据集，并训练了轻量级评测器，属于生成模型评测与数据集方向。该论文同时涉及多模态理解（使用VLM作为教师模型进行评测蒸馏），因此也匹配multimodal_understanding_generation方向。主方向选择multimodal_generation下的[CANONICAL] subtopic "generation_benchmark"（生成模型评测与数据集），该subtopic在multimodal_understanding_generation下也存在，但主方向更贴合生成评测。

Importance reason: Focuses on T2I evaluation framework, not directly on priority subtopics like autoregressive generation or continuous models. Signals: T2I evaluation, dynamic benchmark, curriculum learning, VLM distillation, fine-grained analysis
...
TL;DR: 本文提出SPARC理论，用谱代数分析自回归生成中的自我纠正盲点。证明盲点出现当且仅当误差传播算子的谱半径≥1，推导出纠正标记的激活阈值，并证明RL验证器-纠正器训练的收敛条件。实验在四个文本骨干和一个视觉自回归模型上验证了理论，谱预测与实测盲点率的RMSE在3.2%以内。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型存在自我纠正盲点：能修复外部来源的相同错误，却无法修复自己输出中的错误。现有工作仅从经验层面记录该现象，缺乏形式化模型解释为何生成token会抑制错误检测能力，也没有纠正标记的定量激活条件和RL自纠正的收敛保证。

Method: 定义误差传播算子为残差流上逐步注意力Jacobian的乘积，证明盲点出现当且仅当该算子谱半径≥1。推导出纠正标记必须超过的谱半径函数阈值（恢复"Wait"标记89.3%盲点降低效果）。证明RL验证器-纠正器训练收敛速率与耦合强度平方/样本数平方根成正比，当且仅当验证器-纠正器耦合矩阵谱范数<1，且该条件跨残差流自回归模态不变。

Result: 在四个文本骨干和一个视觉自回归模型上的实验验证了所有定理，谱预测与实测盲点率之间的RMSE在3.2%以内。

Conclusion: SPARC理论首次从谱代数角度形式化解释了自回归生成中的自我纠正盲点，提供了统一的数学框架，同时覆盖文本LLM和自回归视觉生成，为设计更有效的自纠正机制提供了理论基础。

Classification reason: 论文研究自回归生成中的自我纠正盲点，理论分析覆盖文本LLM和自回归图像/视频生成（visual autoregressive probe）。其核心关注自回归生成范式的理论缺陷，与[多模态生成]方向下的[自回归视觉生成]（CANONICAL）高度相关，因为该子主题涵盖自回归图像与视频生成范式，且论文明确将理论推广到视觉自回归模态。

Importance reason: Directly addresses autoregressive generation blind spot with spectral theory, highly relevant to continuous language/multimodal models and autoregressive image/video generation. Signals: autoregressive generation, self-correction blind spot, spectral radius, continuous language model, visual autoregressive probe
...
### [81] [Improving Sample Diversity in Autoregressive Text-to-Image Generation via Cluster Truncation](https://arxiv.org/abs/2607.10535)
...
TL;DR: 本文首次系统研究自回归文生图模型的样本多样性问题，发现自回归图像生成具有持续高 token 级熵和视觉 token 空间冗余两大特性，限制了现有 token 级解码策略的效果。为此提出 p-less cluster 解码策略，在聚类级别而非 token 级别进行基于熵的截断采样，在四个自回归 T2I 模型和两个数据集上实现了更好的多样性-质量权衡。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在文生图任务中虽然图像质量领先，但存在样本多样性坍塌问题。自回归模型作为扩散模型的替代方案，其序列生成特性天然兼容文本生成中用于提升多样性的解码策略，但自回归图像生成中多样性的系统研究尚属空白。本文旨在探索自回归范式能否在图像质量与样本多样性之间实现更好的帕累托前沿。

Method: 提出 p-less cluster 解码策略：首先对视觉 token 空间进行聚类，然后在每个聚类内进行基于熵的截断采样（保留概率质量 p 的 token），最后从截断后的聚类分布中采样。该方法在聚类级别而非 token 级别进行多样性控制，克服了自回归图像生成中持续高熵和 token 空间冗余带来的挑战。

Result: 在四个自回归 T2I 模型（Parti、LlamaGen、VAR、DALL-E 2）和两个数据集（MS-COCO、PartiPrompts）上，使用 FID、CLIP Score、LPIPS、Vendi Score 等综合指标评估，p-less cluster 在保持图像质量和提示对齐的同时，在大多数模型和数据集上实现了最大的多样性提升。

Conclusion: 自回归图像生成模型可以通过合适的解码策略在多样性-质量权衡上超越扩散模型。p-less cluster 作为一种聚类级别的截断采样方法，有效解决了自回归图像生成中 token 级多样性增强策略失效的问题，为自回归文生图模型的多样性提升提供了新思路。

Classification reason: 该论文研究自回归文生图模型的样本多样性问题，属于自回归视觉生成范式，与 [CANONICAL] 子主题"自回归视觉生成"高度匹配。

Importance reason: Directly addresses diversity-quality tradeoff in autoregressive image generation, a priority subtopic with high weight. Signals: autoregressive image generation, sample diversity, cluster-level decoding, entropy-based truncation, Pareto frontier improvement
...
### [82] [Structure-Detail Decoupled Autoregressive Generation for Fast and High-Fidelity Virtual Try-On](https://arxiv.org/abs/2607.11233)
...
TL;DR: 本文提出STAR-VTON，一种两阶段自回归虚拟试穿框架。第一阶段VAR-VTON在潜空间进行结构合成（服装变形与人物布局），第二阶段在像素空间通过匹配引导的精炼器恢复细粒度服装细节。相比扩散方法，速度提升4倍以上且质量不降，像素空间精炼器可作为即插即用模块提升现有VTON方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在虚拟试穿中因潜空间压缩导致高频细节丢失，且多步去噪推理成本高。VAR模型虽有高质量快速生成的潜力，但缺乏有效的双条件机制，尚未被用于虚拟试穿。作者希望填补这一空白，并进一步解决潜空间生成难以保留细粒度服装细节的问题。

Method: 提出两阶段自回归框架STAR-VTON：第一阶段VAR-VTON，基于VAR架构，引入服装条件与结构引导（DensePose、人体解析），在潜空间进行结构合成（服装变形与人物布局）；第二阶段像素空间精炼器，通过匹配机制建立第一阶段生成结果与源服装之间的密集对应关系，直接将细粒度像素级细节映射到生成结果中。

Result: STAR-VTON在多个虚拟试穿基准上达到SOTA或可比质量，推理速度比扩散方法快至少4倍。像素空间精炼器作为即插即用模块，能有效提升现有VTON方法的细节保真度。

Conclusion: 虚拟试穿的不同子任务应在不同表征空间中处理：结构合成适合潜空间，细粒度细节恢复适合像素空间。这种解耦策略实现了效率与保真度的优异权衡，为自回归模型在虚拟试穿中的应用开辟了新方向。

Classification reason: 该论文提出基于VAR（视觉自回归）的虚拟试穿方法，属于自回归视觉生成范式，与[CANONICAL] subtopic "自回归视觉生成"（autoregressive_visual_generation）高度匹配。

Importance reason: Novel two-stage autoregressive VTON framework with detail decoupling, but outside top priority subtopics. Signals: autoregressive generation, virtual try-on, two-stage decoupling, pixel-space detail refiner, 4x speedup over diffusion
...
### [83] [Next-Dense-Stride Prediction for Multimodal Autoregressive Visual Modeling](https://arxiv.org/abs/2607.09892)
...
TL;DR: DenseAR 提出了一种新的自回归图像生成范式——next-dense-stride 预测，通过在单尺度潜空间网格上以逐步密集的步长遍历，实现从全局结构到细节的粗到细生成。该方法避免了逐光栅顺序的慢推理和多尺度方法的长序列开销，并扩展为支持多模态和多种成像任务的统一模型。在脑 MRI 多对比度翻译、条件生成和分割任务以及 ImageNet 类条件生成上均取得有竞争力的结果。


<details>
  <summary>Details</summary>
Motivation: 现有自回归图像生成模型存在两个主要局限：(1) 逐光栅顺序自回归推理速度慢；(2) 多尺度方法需要长序列的多分辨率 token 来实现粗到细预测，计算成本高。作者希望设计一种既能并行预测多个 token 加速推理，又能以粗到细方式生成高质量图像的范式。

Method: DenseAR 使用一个紧凑的单尺度 tokenizer 将图像编码为潜空间网格。生成时，模型以逐步密集的步长遍历该网格：初始大步长预测捕获全局结构，后续逐步减小步长填充细节。由于同一 stride 下的 token 位置不重叠，它们可以并行预测，从而加速推理。该方法进一步扩展为统一模型，通过任务 token 和模态条件，在单个 backbone 中处理多种模态（如 MRI 多对比度）和多种任务（翻译、生成、分割）。

Result: 在多对比度脑 MRI 上，单个 DenseAR 模型统一了跨模态翻译、模态条件生成和肿瘤分割任务，性能与任务专用方法相当。在 ImageNet 256×256 类条件生成上，DenseAR 在 FID 和 IS 指标上优于单网格无步长排序的基线和多尺度 tokenizer 基线。

Conclusion: DenseAR 通过 next-dense-stride 预测有效解决了自回归图像生成中推理速度慢和多尺度开销大的问题，同时展示了自回归模型在多模态统一建模中的灵活性。该工作为高效自回归视觉生成提供了新思路。

Classification reason: 该论文提出 DenseAR，一种粗到细的 next-dense-stride 预测范式用于自回归图像生成，属于自回归视觉生成方向。该论文也涉及多模态统一建模（医学图像翻译、分割等），因此也匹配多模态理解与生成方向下的统一多模态模型子主题。

Importance reason: Proposes a novel coarse-to-fine autoregressive visual generation paradigm that directly addresses key efficiency and quality bottlenecks in the user's priority subtopics. Signals: new autoregressive paradigm, next-dense-stride prediction, coarse-to-fine generation, efficient inference, unified multimodal model
...
### [84] [Slot-RAE: Streamlining Object-Centric Learning via Direct Representation Auto-Encoders](https://arxiv.org/abs/2607.11196)
...
Importance: 值得关注 (69.89) - 未入选精读

TL;DR: Slot-RAE 提出了一种简化的面向对象中心学习框架，直接在 DINOv3 等视觉基础模型的连续语义特征空间中运行，使用 Slot Attention 和 DiT 解码器进行特征空间扩散，无需外部 VAE 或预训练生成先验，在 COCO 上实现了 SOTA 的无监督物体发现、高保真重建和零样本组合性。


<details>
  <summary>Details</summary>
Motivation: 现有的面向对象中心模型（object-centric models）通常需要复杂的流水线才能同时实现鲁棒的场景分解和高保真生成。基于扩散的方法虽然提升了视觉质量，但几乎都依赖预训练的生成先验（如 Stable Diffusion）和外部 VAE 潜空间，导致系统复杂且计算开销大。作者希望简化这一流程，提出一个完全自包含的框架。

Method: Slot-RAE 的核心思想是在冻结的视觉基础模型（VFM，如 DINOv3）的连续语义特征空间中直接操作。它使用 Slot Attention 进行无监督物体分解，然后采用特征空间扩散过程，利用 Diffusion Transformer (DiT) 解码器从 slot 表示重建 VFM 特征，并引入 Representation Alignment (REPA) 头来对齐重建特征与原始 VFM 特征。整个生成核心（Slot Attention 和 DiT）从头训练，无需 VAE 瓶颈或任务无关的生成预训练。

Result: 在 COCO 数据集上，Slot-RAE 在无监督物体发现、高保真图像重建和零样本组合性方面达到了 SOTA 水平，同时比现有的面向对象中心潜扩散模型显著更快、计算效率更高。

Conclusion: Slot-RAE 证明了直接在视觉基础模型的连续语义特征空间中进行面向对象中心学习是可行且高效的，消除了对复杂外部生成先验的依赖，为更简洁、高效的场景理解与生成统一框架提供了新方向。

Classification reason: 该论文提出 Slot-RAE，直接在 DINOv3 等视觉基础模型的连续语义特征空间中操作，使用 DiT 解码器和 REPA 头进行特征空间扩散，属于连续视觉表征自编码器（RAE）方法，与 canonical subtopic "连续视觉 Tokenizer 与表征自编码器" 高度匹配。

Importance reason: Directly advances continuous visual tokenizer and representation auto-encoder for object-centric learning, aligning with top priority subtopic. Signals: continuous semantic feature space, representation auto-encoder, slot attention, diffusion transformer, object-centric learning
...
### [85] [Dance to Music Generation leveraging Pre-training with Unpaired data and Contrastive Alignment](https://arxiv.org/abs/2607.10537)
...
TL;DR: BackgroundMellow 提出一个多模态协同框架，通过主-专智能体架构将长文本叙事分解为多层音频线索，利用 Tango2 潜扩散模型生成环境音、从专业配乐中检索电影 BGM，并结合 NLP 模块自动预测音频参数（起始时间、时长、响度）进行叠加混合，实现叙事驱动的电影级音景生成。


<details>
  <summary>Details</summary>
Motivation: 现有文本到音频（TTA）框架能合成孤立音效，但在长文本叙事场景中缺乏叙事连贯性、时间对齐和电影情感深度，难以生成沉浸式、同步且富有电影感的音频。

Method: 提出 BackgroundMellow 框架，采用主-专智能体架构：主智能体将文本分解为精确的多层音频线索；专门智能体分别用 Tango2 潜扩散模型生成环境音、用 Cinematic BGM Retriever 从专业配乐中检索背景音乐；NLP 模块根据叙事时间线预测音频参数（起始时间、时长、相对响度）；最后将各层音景叠加混合为统一音频片段。

Result: 在 YouTube 电影预告片数据集上通过最近邻检索评估，验证了框架在时间同步、覆盖度和频谱丰富度方面的有效性。

Conclusion: BackgroundMellow 将故事到音频生成视为精确编排与信号处理问题，通过多智能体协作和自动化混音，无需真实标注即可生成叙事连贯、时间对齐的电影级音景。

Classification reason: 该论文提出从文本叙事生成沉浸式电影级音景（环境音+BGM）的框架，属于文本到音频生成任务，与 [CANONICAL] 音频与多模态联合生成（audio_av_generation）高度匹配。

Importance reason: Focuses on narrative-driven audio generation, which is outside the user's core priorities in continuous multimodal models, video/image generation, and world models. Signals: Multi-modal audio generation, Master-specialist agent architecture, Tango2 latent diffusion, Cinematic BGM retrieval, NLP-based audio parameter prediction
...
### [87] [A Production-Oriented Framework for Evaluation of SFX Generation](https://arxiv.org/abs/2607.09973)
*Mélodie Desbos, Yara Bahram, Eric Granger, Mohammadhadi Shateri*

Main category: cs.SD, eess.AS, cs.AI, eess.SY, eess.SP
...
TL;DR: 本文提出一个面向工业音效（SFX）生成的生产导向评估框架，包含9项生产需求、两阶段评估协议（参考引导的音频到音频变体生成任务 + 能力特定分析），结合客观指标和人工研究，系统比较了多种音频生成与编辑方法在参考对齐、多样性、可控编辑等方面的表现。


<details>
  <summary>Details</summary>
Motivation: 现有音效生成评估通常局限于文本到音频（TTA）、无条件生成或特定任务场景，缺乏针对参考引导的音效变体生成的结构化评估方法，无法满足工业音效设计中对参考感知身份保持、可控变体和效率的实际需求。

Method: 提出一个生产导向评估框架：(1) 定义9项生产需求；(2) 两阶段评估协议——阶段一为参考引导的音频到音频（ATA）变体任务，所有方法在统一ESC-50 SFX适配设置下评估；阶段二为能力特定分析（如SFX变形、时序/能量对齐、修补、目标编辑）；(3) 结合客观指标（FAD、ImageBind参考对齐、多样性）和人工感知身份保持与瞬态诊断研究。

Result: 在统一ATA设置下，AudioX在参考对齐与多样性之间提供了最佳整体权衡，同时支持SFX变形；其他基线在特定编辑操作上表现更优。不同方法在不同生产需求下展现出互补优势与权衡。

Conclusion: 该框架为参考引导的SFX变体生成建立了结构化的评估与决策协议，为未来统一工业音频生成管线的设计提供了实用基础。

Classification reason: 该论文关注音效（SFX）生成与编辑的评估框架，属于音频生成领域，映射到 [CANONICAL] 音频与多模态联合生成（audio_av_generation）子主题。

Importance reason: Focuses on audio SFX evaluation, not aligned with user's priority on visual/video generation and continuous multimodal models. Signals: Audio generation evaluation, Production-oriented framework, Not visual/video, No continuous language model, No autoregressive generation
...
### [88] [MusicMark: A Robust Generative Watermarking Framework for Music Generation](https://arxiv.org/abs/2607.11117)
*Seohwan Yun, Jeeyoung Yun, Yongjin Kim, Juyeon Lee, Sungwoong Kim*

Main category: cs.AI, cs.CR, cs.SD
...
TL;DR: MusicMark 是首个面向音乐生成的生成式水印框架，通过在扩散模型的语义潜空间中嵌入水印消息，使水印成为音乐内容的一部分，从而在神经编解码重合成等攻击下保持鲁棒性，同时不损害生成质量。


<details>
  <summary>Details</summary>
Motivation: AI音乐生成快速发展，但现有音频水印主要针对语音，直接应用于音乐效果不佳；且现有方法多为事后添加扰动，容易被神经编解码重合成等攻击移除，也无法防止水印步骤被绕过。

Method: 在扩散生成模型中引入水印适配器（watermark adapter），在去噪过程中将水印消息嵌入语义潜空间；通过联合训练目标约束水印潜变量接近无水印参考潜变量以保持保真度，并利用攻击增强提升鲁棒性。

Result: MusicMark 在多种攻击（包括神经编解码重合成）下显著优于事后水印基线，同时生成质量与基线相当；在封面歌曲攻击下也表现出更强的鲁棒性。

Conclusion: MusicMark 作为首个生成式音乐水印框架，通过将水印嵌入生成过程本身，有效解决了事后水印的脆弱性和可绕过性问题，为音乐生成的可溯源性和归属提供了更可靠的方案。

Classification reason: 该论文提出音乐生成的水印框架，属于音乐生成领域的子任务，与[CANONICAL] subtopic "音频与多模态联合生成"（audio_av_generation）高度匹配。

Importance reason: Focuses on music watermarking, which is outside the user's priority subtopics and negative keywords. Signals: generative watermarking, music generation, diffusion model, robustness, neural codec re-synthesis
...
### [89] [BeatEdit: Symbolic Music Generation as Explicit Editing](https://arxiv.org/abs/2607.11124)
...
TL;DR: 本文提出DC-GRPO，一种基于分解信用分配的多轮LLM越狱攻击框架。通过将轨迹级GRPO信号分解为逐轮的组相对学习信号，结合即时信用与未来信用，解决了多轮越狱学习中的信用分配问题。在多个受害LLM和基准上，DC-GRPO的静态和动态变体分别达到98.26%和97.88%的ASR5@3，大幅超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现代LLM运行在多轮交互场景中，多轮越狱是现实威胁。现有自动化红队方法在多轮设置中面临信用分配难题：不同轮次的贡献不同，但现有学习信号过于粗糙，无法识别各轮次的独立贡献，导致信用误分配。

Method: 提出DC-GRPO（Decomposed Credit GRPO），将GRPO的轨迹级评分分解为逐轮组相对学习信号。每轮结合即时信用（当前轮次对最终结果的贡献）和未来信用（后续轮次的贡献），通过静态或动态加权规则平衡两种信用来源，避免将单一轨迹级分数广播到整个对话。

Result: 在多个受害LLM和基准上，动态加权和静态加权变体分别达到98.26%和97.88%的平均ASR5@3，显著优于SEMA（86.58%）和TROJail（86.23%）。消融实验表明，逐轮组相对信用分配是核心收益来源，而非特定加权规则。

Conclusion: DC-GRPO通过分解信用分配有效解决了多轮越狱学习中的信用分配问题，实现了极高的攻击成功率。逐轮组相对信用分配框架是核心贡献，不同加权规则均可受益于该框架。

Classification reason: 该论文研究多轮LLM越狱攻击（jailbreaking），属于多模态安全与攻防范畴。虽然论文主要针对文本LLM，但多模态安全与攻防（multimodal_safety_attack）是涵盖越狱攻击、对抗攻击、安全评测的[CANONICAL]子主题，且该子主题在现有分类中已包含大量越狱相关论文，是最合适的映射。

Importance reason: Focuses on LLM jailbreaking, which is not a priority subtopic and lacks alignment with user's key directions. Signals: LLM security, multi-turn attack, credit assignment, GRPO, red teaming
...
### [91] [Safe responses matter: Output-aware safety guardrail mitigate over-refusal in MLLMs](https://arxiv.org/abs/2607.09697)
*Jiayi Li, Kun Zhan*
...
TL;DR: 本文提出一种输出感知的安全护栏方法，通过在MLLM隐藏状态空间训练轻量级分类器，在生成完成前预测输出是否不安全，从而仅在模型实际会产生有害响应时进行干预，大幅减少过度拒绝问题，同时保持与现有方法相当的安全性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM安全机制存在安全性与可用性之间的根本权衡。输入侧安全护栏虽然轻量，但存在严重的过度拒绝问题，会不加区分地阻止良性查询或模型本可以通过拒绝/建议性响应安全回答的请求。作者发现根本原因在于输入感知范式：安全护栏在不考虑模型自身能否生成安全响应的情况下做出安全决策，而MLLM本身已具备内在安全机制可将有害输入转化为无害输出，输入侧护栏反而覆盖了这种能力。

Method: 提出输出感知安全护栏范式，在模型隐藏状态空间中预测即将生成的输出是否不安全。通过多实例对比学习在隐藏状态表示上训练轻量级分类器，区分那些会导致不安全输出的输入和不会导致不安全输出的输入（即使输入本身包含风险元素）。仅在模型实际响应会有害时才进行精确干预。

Result: 大量实验表明，输出感知安全护栏在安全性能上与现有方法相当，同时大幅减少了过度拒绝，保留了模型的可用性和内置安全能力。

Conclusion: 输出感知安全护栏通过关注模型的实际输出而非仅凭输入判断，有效解决了安全性与可用性之间的权衡问题，为MLLM安全防护提供了新的范式方向。

Classification reason: 该论文研究多模态大模型的安全防护机制，提出输出感知的安全护栏以缓解过度拒绝问题，属于多模态安全与攻防方向，因此映射到 [CANONICAL] 多模态安全与攻防 (multimodal_safety_attack)。

Importance reason: Addresses over-refusal in MLLMs but is not aligned with user's priority subtopics on continuous models, generation, or world models. Signals: MLLM safety, over-refusal mitigation, output-aware guardrail, hidden state classifier, utility-preserving safety
...
### [92] [Imperceptible and Reversible Adversarial Examples against Vision-Language Models for Privacy Protection](https://arxiv.org/abs/2607.10329)
...
TL;DR: CloakDiff 是首个面向视觉语言模型的可逆、高保真隐私保护框架，通过扩散对抗编辑与可逆网络生成不可感知的对抗样本，抵御基于文本查询的隐私攻击，同时支持无损恢复原始图像。


<details>
  <summary>Details</summary>
Motivation: VLM 强大的多模态能力使攻击者可通过爬取在线照片并查询 VLM 来提取用户的敏感属性（文本隐私攻击）。现有可逆对抗样本方法仅适用于纯视觉任务，在多模态场景下失效；而当前针对 VLM 的对抗样本依赖高频噪声，严重降低视觉质量。因此需要一种既能有效保护隐私、又保持高视觉质量且可逆的方法。

Method: CloakDiff 结合扩散模型对抗编辑与可逆网络：1）利用可逆网络嵌入原始图像信息，实现无损恢复；2）在像素空间嵌入扰动，并操控潜空间交叉注意力图，确保跨模型和跨提示的强迁移性，同时保持全局视觉结构；3）设计 EDM Heuristic Sampling，一种基于原则的扩散调度策略，用于对抗引导，进一步提升保真度。

Result: 在多个数据集和多种 VLM 上的实验表明，CloakDiff 实现了多模态隐私保护，同时具有高视觉质量和可逆性，优于现有方法。

Conclusion: CloakDiff 是首个面向 VLM 的可逆、高保真隐私保护框架，有效平衡了隐私保护、视觉质量和可逆性，为多模态场景下的隐私安全提供了新思路。

Classification reason: 该论文研究针对VLM的对抗性攻击与防御（可逆对抗样本用于隐私保护），属于多模态安全与攻防范畴，因此映射到[CANONICAL] subtopic "多模态安全与攻防"。

Importance reason: Focuses on adversarial privacy for VLMs, not on the user's priority subtopics like continuous models or generation. Signals: VLM privacy protection, reversible adversarial examples, diffusion-based editing, cross-model transferability, not aligned with user's core interests
...
Importance: 普通相关 (56.43) - 未入选精读

TL;DR: 本文系统综述了AI生成视频（AIGC-V）检测方法，提出将检测任务重新定义为"事实保真度验证"，并构建了一个视觉-语言双视角的四层分类体系，涵盖内在线索分析、时空一致性建模、跨模态一致性推理和语言引导的世界级推理，共综述221篇文献。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成视频的真实感不断提升，传统基于伪影的检测方法已不足以应对。需要从低级特征检测转向高级语义验证，即判断视频中的事件、实体和物理过程是否与真实世界事实一致。

Method: 提出视觉-语言双视角分类法（Vision-Language Dual-View taxonomy），将检测方法组织为四层层次结构：(1) 内在线索分析；(2) 时空一致性建模；(3) 跨模态一致性推理；(4) 语言引导的世界级推理。系统综述了221篇文献，涵盖AIGC-V生成范式、检测方法、评测指标和基准。

Result: 构建了系统的AIGC-V检测分类体系，识别出从伪影匹配到基于证据的语义验证的范式转变，总结了当前评测基准和指标，指出了鲁棒、可解释和可信检测的未来方向。

Conclusion: AI生成视频检测正从传统深度伪造的伪影检测转向基于视觉语言模型和智能体推理的语义验证范式。未来需要更鲁棒的检测方法、更好的可解释性和更全面的评测基准。

Classification reason: 该论文是关于AI生成视频检测的综述，属于多模态安全与攻防（检测伪造/生成内容）范畴。虽然论文也涉及多模态推理，但其核心任务是检测AIGC视频的真伪与事实一致性，这与多模态安全中的深度伪造检测、对抗防御高度相关。因此复用[CANONICAL] subtopic "多模态安全与攻防"。

Importance reason: Survey on AI-generated video detection is tangentially relevant but not aligned with user's priority on generation and world models. Signals: Survey, AI-generated video detection, vision-language dual-view, factual fidelity verification, not generation or world model
...
### [94] [Lesioned Multimodal Language Models Reproduce Aphasic Picture-Naming Patterns](https://arxiv.org/abs/2607.11621)
...
TL;DR: 本文通过对LLaVA 1.6多模态语言模型施加分层、比例和噪声量的受控扰动（模拟脑损伤），成功复现了失语症患者在图片命名任务中的七类错误模式中的六类，并能对97.8%的患者匹配至少六类错误分布，表明通用多模态语言模型可作为失语症患者的数字孪生。


<details>
  <summary>Details</summary>
Motivation: 脑卒中后失语症患者会产生系统性的命名错误，但通用语言模型（非临床专用模型）能否复现这些错误模式尚未被验证。作者希望探究通过对多模态语言模型施加受控扰动，能否模拟失语症患者的图片命名错误特征。

Method: 使用LLaVA 1.6作为基础模型，设计三种扰动参数：层（layer）、比例（proportion）、噪声量（noise amount），对模型单元施加受控损伤。在费城命名测试（Philadelphia Naming Test）上评估278名失语症患者，使用经过验证的神经分类器将模型输出分为七类（正确、语义、混合、无关、新语、无反应、形式性错语）。通过搜索扰动参数空间寻找与个体患者错误分布匹配的配置。

Result: 六类错误（正确、语义、混合、无关、新语、无反应）在特定参数空间区域以临床可比的比例出现，形式性错语除外。对97.8%的患者可匹配至少六类错误分布，79.5%的患者可匹配全部七类。蒙特卡洛基线验证表明匹配反映了类别间联合结构而非边缘重叠。

Conclusion: 该研究建立了复现个体失语症患者图片命名错误模式的定量框架，表明通用语言模型有潜力作为脑卒中后失语症患者的数字孪生，为临床模拟和康复研究提供新工具。

Classification reason: 该论文通过对多模态语言模型（LLaVA）施加扰动/损伤（lesion）来模拟失语症患者的图片命名错误模式，本质上属于对多模态模型的受控攻击与扰动分析。这与"多模态安全与攻防"（multimodal_safety_attack）中对抗攻击、扰动防御的主题高度相关，因此复用该规范子主题。

Importance reason: This paper focuses on clinical aphasia simulation, which is outside the user's priority subtopics and direction weights. Signals: clinical simulation, aphasia, multimodal language model, lesion perturbation, digital twin
...
### [95] [AdvNav: Behavior-Guided Black-Box Adversarial Attacks on Vision-Language Navigation](https://arxiv.org/abs/2607.11063)
...
TL;DR: AdvNav 提出一种行为引导的黑盒对抗攻击框架，通过智能体自身输出的轨迹级和动作级反馈信号，结合自适应扰动调优与遗传进化策略，在无梯度条件下有效攻击视觉-语言导航系统，在 R2R 数据集上对 HAMT 和 MapGPT 达到 49.70%~87.30% 的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有 VLN 对抗攻击方法大多依赖白盒梯度访问，这在真实部署场景中不现实且计算开销大；而现有黑盒方法主要针对单步决策任务，难以处理多步时序依赖的导航任务。因此需要一种无梯度的黑盒攻击方法，仅利用可观测的输入输出就能有效破坏多步感知-行动循环。

Method: AdvNav 框架包含三个核心组件：(1) 双粒度行为反馈（dual-granularity behavior-based feedback），包括轨迹级性能分数、动作级奖励分数和偏离指标，从智能体自身输出行为中提取；(2) 混合优化策略，结合自适应更新（heuristic adaptive update）来动态调整扰动强度，以及遗传进化（genetic evolution）来演化噪声的空间结构；(3) 迭代搜索最有效的扰动配置。

Result: 在 R2R 数据集上，针对基于 Transformer 的 HAMT 和基于 LLM 的 MapGPT 两种骨干网络，AdvNav 分别达到 49.70%、65.96% 和 87.30% 的攻击成功率，验证了方法的有效性和泛化性。

Conclusion: AdvNav 揭示了 VLN 模型在视觉感知层面的关键脆弱性，为未来构建更具鲁棒性的视觉-语言导航系统提供了重要启示。

Classification reason: 该论文研究视觉-语言导航（VLN）中的黑盒对抗攻击，属于多模态安全与攻防范畴，与 [CANONICAL] subtopic "多模态安全与攻防"（multimodal_safety_attack）高度匹配。

Importance reason: Focuses on adversarial attack for VLN, which is not a priority subtopic and lacks alignment with user's key directions like continuous models or generation. Signals: Black-box attack, Vision-Language Navigation, Behavior-guided, Gradient-free, Adversarial robustness
...
### [96] [Technical Report on the CVPR 2026@AdvML Workshop Challenge](https://arxiv.org/abs/2607.11560)
...
TL;DR: 本文是CVPR 2026@AdvML Workshop关于自动驾驶VLA对抗性多模态攻击挑战赛的技术报告。基于DriveLM风格的多视角VQA，参赛者生成对抗性图像和文本后缀扰动，使模型偏离参考答案。报告分析了五项优胜方案，发现图像侧攻击更受青睐、场景级多视角优化优于单视角、QA类型和图结构可用于分配攻击预算、特征空间目标可提升黑盒迁移性、相机图像中的印刷文字是持续漏洞。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶视觉语言模型（VLA）在安全关键场景中日益重要，但其对抗鲁棒性尚未得到充分研究。现有工作缺乏针对多视角、多模态自动驾驶VLA的对抗攻击系统评测，亟需一个标准化的挑战赛来评估和推动该领域的防御研究。

Method: 基于DriveLM风格的多视角VQA构建挑战赛，每个场景包含6个同步相机图像和结构化QA对。参赛者需生成对抗性图像（保持视觉保真度）和仅后缀文本扰动，使模型输出偏离参考答案。比赛分两阶段，Phase II引入隐藏黑盒模型评估迁移性。报告分析了五项优胜方案的技术模式。

Result: 五项优胜方案揭示了若干关键发现：1）由于后缀惩罚，图像侧攻击更受青睐；2）场景级多视角联合优化优于单视角独立攻击；3）QA类型和图结构可作为攻击预算分配的先验；4）特征空间目标可提升黑盒迁移性；5）相机图像中的印刷文字（如路牌、广告牌）是自动驾驶VLA的持续脆弱点。

Conclusion: 该挑战赛系统评估了自动驾驶VLA的对抗鲁棒性，揭示了多模态攻击的有效模式和持续漏洞，为未来自动驾驶多模态系统的鲁棒性评估和防御设计提供了实用参考。

Classification reason: 该论文是关于CVPR 2026@AdvML Workshop挑战赛的技术报告，聚焦于针对自动驾驶视觉语言模型（VLA）的对抗性多模态攻击（图像+文本）。这属于多模态安全与攻防（multimodal_safety_attack）这一[CANONICAL]子主题的范畴，具体涉及对抗攻击、多模态安全评测与鲁棒性评估。

Importance reason: This is a niche challenge report on adversarial attacks for driving VLAs, far from the user's core interests in continuous multimodal models and autoregressive generation. Signals: adversarial attack, autonomous driving, VLA, challenge report, no continuous language model
...
### [97] [Minionese: Comprehensive Benchmark and Mechanistic Study of Multilingual LLM Safety](https://arxiv.org/abs/2607.10112)
...
TL;DR: DeepBias 提出了一种自适应框架，通过"生成-进化-探测"循环，利用 ProposerAgent 和 DiggerAgent 动态生成测试数据并逐步深入探测 LVLM 中的社会偏见，构建了 DeepBiasBench 基准。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM偏见评估方法依赖静态数据集，无法自适应地探测模型偏见的真实深度和极限，缺乏动态演化的评估范式。

Method: 提出DeepBias框架，包含两个核心智能体：(1) ProposerAgent 通过DPO基于目标模型响应迭代优化生成测试数据；(2) DiggerAgent 通过技能库中的深化和改写策略，在多次探测轮次中自适应地改写测试数据，逐步暴露更深层偏见。基于5个SOTA LVLM集成构建了DeepBiasBench基准。

Result: 实验证明DeepBias框架能有效探测LVLM中的深层社会偏见，DeepBiasBench为偏见评估提供了具有挑战性的基准。

Conclusion: DeepBias建立了LVLM安全评估的进化范式，通过自适应动态探测揭示了模型偏见的深层脆弱性。

Classification reason: 该论文研究LVLM中的社会偏见深度探测与安全评估，属于多模态安全与攻防范畴，与[CANONICAL] subtopic "多模态安全与攻防"（multimodal_safety_attack）高度匹配。

Importance reason: Focuses on social bias probing in LVLMs, which is outside the user's priority subtopics and direction weights. Signals: social bias probing, adaptive framework, LVLM safety, static dataset limitation, agent-based generation
...
### [99] [Confidently Wrong: Detecting Hallucinations in Financial Question Answering from LLM Internal States](https://arxiv.org/abs/2607.11414)
*Richard Zhe Wang*
...
TL;DR: 本文研究从LLM内部激活状态检测金融问答中的"自信错误"（confident hallucinations）。通过在残差流上训练线性探针，在FinQA和TAT-QA基准上评估，发现探针在检测自信幻觉方面（AUROC 0.68-0.77）显著优于基于输出概率和模型自我评估的基线方法（AUROC 0.55-0.63），可作为高价值金融场景中LLM答案人工审核的成本效益分流机制。


<details>
  <summary>Details</summary>
Motivation: LLM在金融应用中"自信地犯错"（confidently wrong）比不确定的回答更具危害性，因为后者会引发质疑而前者会无声地破坏下游决策。现有方法主要依赖输出概率或模型自我评估来检测错误，但这些方法在模型高度自信时往往失效。因此，需要更可靠的方法来检测这种"自信幻觉"。

Method: 在Qwen3-8B、Llama-3.1-8B和Gemma-2-9B三个模型的残差流上训练线性探针（linear probes）。行为自信度通过同一问题的8次重采样答案的一致性来衡量（全部一致视为自信）。探针效果与token对数概率、模型True/False自我评估等基线方法进行比较。实验在FinQA和TAT-QA两个真实金融QA基准上进行。

Result: 在FinQA上，所有8次重采样一致的自信答案中有15-23%是错误的。探针在检测幻觉方面AUROC达到0.68-0.77，而最佳基线方法仅0.55-0.63，探针具有显著优势。该结果在三个不同模型上均保持一致。

Conclusion: 基于内部状态的探针可以作为一种成本效益高的分流机制，用于将LLM答案路由到人工审核和质量控制流程中，特别适用于金融等高风险场景。模型内部激活状态携带了超越其可观测输出的额外信息，可用于更可靠地检测自信幻觉。

Classification reason: 该论文研究从LLM内部状态检测幻觉（confident hallucinations），属于多模态幻觉与可靠性范畴。虽然论文聚焦金融QA（FinQA/TAT-QA）而非视觉多模态，但"多模态幻觉与可靠性"（multimodal_hallucination）是[CANONICAL]子主题，涵盖幻觉检测、事实性与可靠性等广义问题，且该子主题描述包含"事实性、可靠性、忠实性提升"，与本论文高度契合。

Importance reason: Focuses on financial domain hallucination detection, which is outside user's priority subtopics and has negative keyword "financial". Signals: Financial QA, confident hallucination detection, internal state probing, AUROC 0.68-0.77, FinQA/TAT-QA
...
### [100] [Direct Image-to-Modern Vietnamese Translation of Han-Nom Manuscripts via Multimodal RLHF Preference Alignment](https://arxiv.org/abs/2607.11434)
...
TL;DR: 本文提出一个多模态RLHF偏好对齐框架，结合CLIP视觉特征、中文BERT和越南语PhoBERT等多模态编码器，通过PPO、DPO和KTO三种偏好优化方法，将汉喃古籍图像直接翻译为现代越南语。实验表明DPO在BLEU-4、ROUGE-L、BERTScore等指标上最优，所有偏好对齐方法均优于纯监督微调基线。


<details>
  <summary>Details</summary>
Motivation: 汉喃古籍翻译面临页面退化、罕见表意字符和有限平行语料的挑战，现有监督学习方法在低资源场景下翻译质量不足，需要利用偏好对齐来提升翻译的词汇和语义质量。

Method: 提出多模态RLHF偏好对齐框架，包含四个编码流：CLIP ViT-L/14@336提取视觉特征、bert-base-chinese编码汉喃文本、vinai/phobert-base编码越南语、T5-small编码器状态。通过模态特定投影和融合模块将2048维拼接向量压缩为512维共享表示。在相同SFT策略基础上，比较PPO、DPO和KTO三种偏好优化方法。

Result: DPO在BLEU-4、ROUGE-L、BERTScore、语义相似度、CER、WER和token准确率上最优；PPO在精确率、召回率和F1上最高；KTO通过期望-不期望效用目标保持竞争力。所有偏好对齐策略均提升了SFT基线的BLEU-4和语义相似度分数。

Conclusion: 多模态偏好优化能够有效补充监督学习，在低资源历史文献翻译任务中提升词汇和语义质量，其中DPO是综合最优的偏好对齐方法。

Classification reason: 该论文提出多模态RLHF偏好对齐框架（PPO/DPO/KTO），用于汉喃古籍图像到现代越南语的翻译，属于多模态强化学习与对齐方向，因此映射到[CANONICAL] subtopic "多模态强化学习与对齐" (multimodal_rl_alignment)。

Importance reason: Focuses on historical document translation, not on the user's priority subtopics like continuous language models or video generation. Signals: Multimodal RLHF, low-resource translation, historical manuscripts, DPO/PPO/KTO comparison, not autoregressive generation
...
### [101] [To Answer or to Abstain: Mitigating Search-Agent Hallucinations via Abstention-Aware Reinforcement Learning](https://arxiv.org/abs/2607.10738)
...
TL;DR: 本文提出AWA-RL（Abstention-Aware Reinforcement Learning），一种基于强化学习的搜索智能体幻觉缓解方法。通过动态塑形"弃权"（abstention）奖励，利用模型查询特定的先验能力和在线策略训练观测，让模型在检索失败时选择不回答而非编造答案。同时提出RA-F1指标衡量能力-可靠性权衡。实验表明，AWA-RL在保持高准确率的同时，将精确率提升最高10.3%，RA-F1提升2.9%。


<details>
  <summary>Details</summary>
Motivation: 现有搜索增强LLM的训练范式主要奖励正确答案，但未对检索失败时编造答案的行为进行惩罚，从而隐含地加剧了幻觉问题。作者希望训练模型在不确定时主动选择"弃权"（不回答），以提升可靠性和可信度。

Method: 提出Abstention-Aware Reinforcement Learning (AWA-RL)，核心包括：(1) 动态塑形弃权奖励，结合模型查询特定的先验能力（基于模型自身对查询的置信度）和在线策略训练中的持续观测；(2) 在RL训练中引入"弃权"动作，使模型学会在检索失败或不确定时选择不回答；(3) 提出RA-F1指标，综合衡量模型的能力（准确率）和可靠性（精确率/弃权率权衡）。

Result: 在开放域QA任务上，相比非弃权基线，AWA-RL将绝对精确率提升最高10.3%，RA-F1提升2.9%，同时原始准确率仅有轻微下降。实验验证了AWA-RL能有效训练出既高能力又高可靠性的搜索智能体。

Conclusion: AWA-RL通过奖励塑形让搜索增强LLM学会在不确定时主动弃权，有效缓解了检索失败导致的幻觉问题，实现了能力与可靠性的良好平衡。该方法为构建可信赖的搜索智能体提供了新思路。

Classification reason: 该论文提出基于强化学习的搜索智能体幻觉缓解方法（AWA-RL），核心是使用RL进行偏好/奖励建模以提升模型可靠性，属于多模态强化学习与对齐方向。虽然论文主要聚焦于搜索增强的LLM而非多模态，但其方法（RL奖励塑形、偏好优化）与多模态RL对齐高度相关，且该[CANONICAL] subtopic涵盖RLHF/DPO/偏好学习等广义对齐方法，可合理映射。

Importance reason: Focuses on search-agent hallucination mitigation, not on the user's priority subtopics like continuous multimodal models or video generation. Signals: RL for hallucination reduction, abstention-aware training, RA-F1 metric, search-agent reliability, not aligned with user's core interests
...
### [102] [Agentic-DPO: From Imitation to Agentic Policy Optimization on Expert Trajectories](https://arxiv.org/abs/2607.10601)
...
TL;DR: Agentic-DPO 是一种轻量级离线 agent 策略优化方法，将专家轨迹转化为状态条件偏好监督，通过一步动作采样和 DPO 风格偏好目标优化 LLM agent，无需在线环境交互或奖励模型，在多个基准上显著超越 SFT。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM agent 通常使用 SFT 从专家轨迹中训练，但 SFT 仅学习模仿专家动作序列，而非训练 agent 在每个状态下从可能的错误动作中选择正确动作。偏好学习和强化学习虽能缓解此问题，但通常需要高成本的环境 rollout 和奖励模型。因此，需要一种低成本、离线的 agent 策略优化方法。

Method: 提出 Agentic-DPO，核心思想是将专家轨迹转化为状态条件偏好监督。在每个专家动作状态，从当前策略采样一步动作，将合理的错误动作作为负样本，与专家动作构成偏好对，使用 DPO 风格偏好目标进行优化。同时提出 Policy-Preserving Augmentation (PPA)，在保持策略不变的前提下对同一潜在轨迹进行多种 schema 渲染，避免偏好学习中策略与 schema 的混淆。整个方法无需在线环境 rollout、奖励模型或完整轨迹的学生探索。

Result: 在 StableToolBench、tau-bench retail 和 Mind2Web 上实验，Agentic-DPO 在不同规模模型上一致优于 SFT。特别地，在 tau-bench 上将 9B 模型的准确率从 21.7%（SFT）提升至 41.4%，在相同骨干网络下匹配在线 GRPO 的性能，且仅需 step-level rollout，无需梯度步骤中的环境交互。

Conclusion: 专家轨迹可以通过从演示转换为状态级动作偏好来支持低成本的 agent 策略优化。Agentic-DPO 提供了一种简单有效的离线偏好学习方法，弥合了 SFT 与在线 RL 之间的差距。

Classification reason: 该论文提出 Agentic-DPO，将专家轨迹转化为状态级偏好监督，使用 DPO 风格偏好目标优化 LLM agent 策略。这属于多模态/语言模型的对齐与偏好学习范畴，与 [CANONICAL] subtopic "多模态强化学习与对齐"（multimodal_rl_alignment）高度匹配。

Importance reason: Strong offline agent policy optimization method, but tangential to user's focus on continuous multimodal models and video generation. Signals: DPO-style preference learning, offline agent optimization, no environment rollouts, significant accuracy gains, LLM agent alignment
...
### [103] [When the Reward Suite Is Leaky: A Preregistered Causal Contrast of Natural Verifier False Positives in RLVR](https://arxiv.org/abs/2607.11022)
*Chuyifei Zhang*
...
TL;DR: 本文通过预注册因果对比实验，系统研究了代码 RLVR 中测试套件自然假阳性（leaky test suite）对奖励信号的影响，发现假阳性奖励确实支付给了真实错误代码而非套件伪影，并提出了一种廉价静态审计方法可在训练前定位暴露风险。


<details>
  <summary>Details</summary>
Motivation: 现有噪声鲁棒性分析假设奖励噪声是对称或可重采样的，但实际代码 RLVR 中的测试套件存在自然的假阳性——即每次出现都接受相同错误程序的持久性、非对称性错误。这种"泄漏"的奖励信号可能误导强化学习过程，需要系统研究其影响。

Method: 采用预注册的双臂因果对比实验：在相同 MBPP 任务、种子和计算资源下，分别使用原始 MBPP 测试（泄漏版）和 MBPP+ 额外测试（加固版）作为 GRPO 的奖励信号。通过静态泄漏审计、人工裁决假阳性、机制分析（假阳性发生率是否增长、基模型是否已产生相同错误输出）等方法进行系统评估。

Result: 平均保留效果有界：在预注册的 1.5 分非劣效性边界下，差距仅 0.20 分（单侧 95% 上限 0.75 分）。奖励假阳性质量与训练前静态泄漏审计高度相关（Spearman 0.80）。泄漏层假阳性份额比干净任务高 43.8 个百分点。经人工裁决，47.57% 的奖励假阳性确实是真正的错误代码。假阳性发生率在训练期内未增长，未训练的基模型已能产生相同错误输出。

Conclusion: 代码 RLVR 中的测试套件泄漏确实会导致奖励为真实错误代码付费，而非仅仅是套件伪影。机制证据支持假阳性源于预存错误模式的选择而非学习到的利用行为。廉价静态审计可在训练前定位暴露风险，加固奖励可消除测量膨胀。该发现对前沿模型自身的评估也提出了警示。

Classification reason: 该论文研究 RLVR（基于强化学习的验证奖励）中测试套件的假阳性问题，属于多模态/代码领域的强化学习奖励建模与对齐问题，与 [CANONICAL] 多模态强化学习与对齐（multimodal_rl_alignment）高度相关。

Importance reason: Focuses on RLVR reward noise in code generation, not on the user's priority subtopics like continuous multimodal models or video generation. Signals: RLVR reward noise, code generation, preregistered causal contrast, false positive analysis, static audit method
...
### [104] [Answer-Conditioned Chain-of-Thought Distillation for Few-Shot Industrial Vision with Small VLMs](https://arxiv.org/abs/2607.10666)
*Shubham Rao*

Main category: cs.LG, cs.AI, cond-mat.mtrl-sci, cs.CV
...
TL;DR: 本文提出答案条件链式思维蒸馏（Answer-Conditioned CoT Distillation），利用前沿VLM为少量标注图像生成带推理过程的解释，然后对3B参数的小VLM进行LoRA微调，在工业视觉分类任务上仅用18-30张标注图像即可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 工业视觉检测中需求频繁变化、新缺陷类型不断出现、标注数据稀缺，现有大模型直接部署成本高，小模型在少样本场景下性能不足。需要一种能快速适应新任务且仅需极少标注数据的方法。

Method: 1) 将训练图像及其正确标签输入前沿VLM（如GPT-4.1），要求其生成合理的视觉解释（CoT）。2) 用这些推理增强的样本通过LoRA微调3B参数的小VLM。关键创新是"答案条件化"——在生成推理时提供正确答案，确保推理指向正确结论，避免前沿模型在困难任务上推理错误带来的负面影响。

Result: 在4个工业分类任务（涵盖3种图像模态）上，仅用18-30张标注图像，方法在所有16个种子-任务组合上均优于直接微调，平均提升+1.7到+4.4个百分点。在焊缝射线照片分类上，微调后的3B模型以24张训练图像超越GPT-4.1达10.0个百分点。

Conclusion: 答案条件化的CoT蒸馏是一种高效、数据高效的少样本工业视觉适配方法，能够将大模型的推理能力迁移到小模型，在极低数据量下取得显著性能提升，且优于直接微调和无条件蒸馏基线。

Classification reason: 该论文使用大VLM生成推理链（CoT）来蒸馏到小VLM，涉及多模态模型的对齐与知识蒸馏，属于多模态强化学习与对齐（multimodal_rl_alignment）范畴下的蒸馏/对齐方法。

Importance reason: Focuses on few-shot industrial vision with small VLMs, which is not aligned with user's priority on continuous multimodal models, video/image generation, or world models. Signals: Few-shot learning, Chain-of-thought distillation, Industrial vision, Small VLMs, LoRA fine-tuning
...
### [105] [Actor as Its Own Critic: Unifying Region Understanding and Localization via CycleGRPO](https://arxiv.org/abs/2607.11581)
...
TL;DR: 本文提出 CycleGRPO，一种统一的强化学习框架，利用区域理解与定位之间的内在对偶性，让 MLLM 先作为 actor 生成区域描述，再作为 critic 将文本定位回空间域，通过 token 级循环一致性奖励实现无需文本标注的联合优化。


<details>
  <summary>Details</summary>
Motivation: 现有的 MLLM 区域理解（区域描述）和定位（指代分割、区域定位）通常采用分离的流水线，需要大量人工标注的文本描述作为监督信号。本文希望利用两个任务之间的内在对偶性，构建一个无需文本真值、仅需区域输入（mask/bbox）的自监督强化学习框架。

Method: 提出 CycleGRPO（Cycle Group Relative Policy Optimization），基于 GRPO 框架。流程为：MLLM 作为 actor 对给定区域生成文本描述 → 同一模型作为 critic 将生成的文本定位回空间域（生成 mask/bbox）→ 通过 token 级循环一致性奖励评估文本的语义判别性（基于定位精度）。采用质量感知的 token 级 cycle-consistency reward，无需任何文本标注。

Result: 基于 SAMTok 构建的模型，在区域描述、区域 VQA、接地对话和指代分割等多个基准上取得一致性能提升，无需任务特定的微调。

Conclusion: CycleGRPO 提供了一种简单且可扩展的方式，通过强化学习自举提升 MLLM 的像素级能力，展示了区域理解与定位联合优化的有效性。

Classification reason: 该论文提出 CycleGRPO，一种基于 GRPO 的强化学习框架，用于联合优化 MLLM 的区域理解与定位能力，属于多模态强化学习与对齐方向。该论文使用强化学习（GRPO）进行训练，与 [CANONICAL] subtopic "多模态强化学习与对齐" 高度匹配。

Importance reason: Novel self-supervised RL loop for region understanding, but lacks direct connection to user's priority subtopics like continuous models or video generation. Signals: CycleGRPO, region understanding, localization, reinforcement learning, MLLM
...
### [106] [PhysMRV: Physical Memory Retrieval and Verification for Physics Plausibility Reasoning](https://arxiv.org/abs/2607.10190)
...
TL;DR: PhysMRV 提出一种无需训练的记忆检索与验证框架，通过将训练视频转化为分层物理记忆库（场景描述、物理事件图、物理规则摘要），在推理时检索相关物理记忆并引导冻结的 VLM 验证物理合理性，在三个物理推理基准上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型在视频理解和 VQA 上表现优异，但在物理合理性推理（如物体交互、因果动力学、物理原理理解）上仍不可靠，存在物理常识推理的持续差距。

Method: PhysMRV 包含两个阶段：(1) 构建阶段：将训练视频转化为分层记忆库，包含场景描述（视觉上下文）、物理事件图（物体交互与因果结构）、物理规则摘要（可复用的物理原理与线索）；(2) 推理阶段：检索与输入视频物理相关的记忆，利用结构化物理证据引导冻结的 VLM 验证物理合理性，无需微调或参数更新。

Result: 在 ImplausiBench、IntPhys2 和 GRASP Level 2 三个物理推理基准上，PhysMRV 在多种 SOTA VLM 上均一致优于直接提示方法，表明结构化物理记忆能有效且可扩展地增强物理合理性推理。

Conclusion: PhysMRV 通过构建和检索结构化物理记忆，无需额外训练即可显著提升 VLM 的物理合理性推理能力，为增强多模态模型的物理常识推理提供了一种可扩展的有效方案。

Classification reason: 该论文研究视频语言模型在物理合理性推理（physical plausibility reasoning）上的能力提升，属于多模态推理范畴。现有 [CANONICAL] subtopic "多模态推理与思维链"（multimodal_reasoning_cot）涵盖视觉推理、科学推理等，物理合理性推理是其子方向，因此映射至此。

Importance reason: Proposes a training-free memory retrieval framework for physical plausibility reasoning, but is not aligned with the user's priority subtopics on continuous models, generation, or world models. Signals: Training-free memory retrieval, Physical plausibility reasoning, Hierarchical memory bank, Frozen VLM guidance, Not aligned with priority subtopics
...
### [107] [SVR-R1: Bootstrapping Multi-modal Reasoning with Self-verification in Reinforcement Learning](https://arxiv.org/abs/2607.10966)
...
TL;DR: SVR-R1 提出一种多轮强化学习框架，让 VLM 在推理过程中对自己的答案进行二元自验证（Yes/No），若验证为 No 则触发重新思考，通过 GRPO 训练将自验证信号转化为学习信号，显著提升多模态推理准确率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLM）在复杂多模态推理任务中仍存在错误累积问题，而推理时的自我修正（self-refinement）与强化学习训练的结合尚未被充分探索。作者希望利用模型自身的验证能力作为学习信号，无需外部监督或辅助评判器，来提升多模态推理的准确性和鲁棒性。

Method: 提出 SVR-R1 框架：对于每个查询，模型使用相同权重先给出答案，然后输出二元自验证（Yes/No）。若验证为 No，则触发第二轮重新思考（rethink）；若验证为 Yes 或达到轮次上限，则输出最终结果并计算基于结果的奖励。该框架基于 GRPO 实现，并设计了异步多轮 rollout 机制，无需外部监督或辅助评判器。

Result: 在视觉语言推理基准上，SVR-R1 相比强基线 GRPO 方法取得了大幅准确率提升。训练动态显示，随着训练进行，模型对验证的依赖逐渐减少（验证轮次减少），但测试准确率反而提高，表明策略内化了自我修正能力，并通过框架选择最自信的答案。

Conclusion: SVR-R1 成功弥合了推理时自我修正与强化学习训练之间的鸿沟，为 VLM 的多模态推理提供了一种简单而有效的自举方法。该工作将开源以促进未来研究。

Classification reason: 该论文提出 SVR-R1，一种通过自验证（self-verification）和强化学习（GRPO）提升视觉语言模型多模态推理能力的方法，核心是多模态推理中的自我修正与推理时扩展，与 [CANONICAL] subtopic "多模态推理与思维链"（multimodal_reasoning_cot）高度匹配。

Importance reason: Directly advances multimodal reasoning with self-verification RL, aligning with user's high-priority continuous multimodal and unified MLLM directions. Signals: multimodal reasoning, self-verification, reinforcement learning, GRPO, VLM
...
### [108] [Mixture of Cognitive Experts in Large Vision-Language Models](https://arxiv.org/abs/2607.10796)
...
TL;DR: 本文提出一种基于 Bloom 认知分类法的证据驱动多模态推理框架，通过多编码器专家集成、两阶段认知言语化和推理轨迹分析模块，提升 LVLM 在视觉推理任务中的可解释性和可靠性，并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有 LVLM 对所有查询采用相同的感知-推理路由，忽略了不同视觉语言问题在认知层级上的差异，且缺乏可解释性和可验证性，容易产生幻觉。

Method: 提出证据驱动多模态推理框架：(1) 集成多个领域专用视觉编码器（目标检测、定位、状态推断、空间布局、文本阅读等）作为专家；(2) 两阶段认知言语化：先产生 Literal Evidence Summary（将专家输出分解为原子化证据陈述），再通过 Bloom Verbalization 将证据转化为分阶段推理轨迹；(3) 轻量级 Reasoning Trace Module 定量分析推理轨迹，使证据使用和推理过程显式化。

Result: 在多个视觉推理基准上观察到感知和推理能力的提升；推理轨迹模块提供了定量证据，表明不同查询会触发不同的认知入口层级和证据使用轨迹，支持细粒度分析。

Conclusion: 将 Bloom 认知分类法引入 LVLM 推理过程，结合多专家证据集成和显式推理轨迹分析，能够提升模型的可解释性、可靠性和推理能力，为构建更可信的多模态推理系统提供了新思路。

Classification reason: 该论文提出了一种基于 Bloom 认知分类法的证据驱动多模态推理框架，通过两阶段认知言语化（Literal Evidence Summary + Bloom Verbalization）和轻量级推理轨迹模块来增强 LVLM 的感知与推理能力，属于多模态推理与思维链（multimodal_reasoning_cot）范畴。

Importance reason: Proposes a cognitive taxonomy for LVLM reasoning but lacks direct connection to user's priority subtopics like continuous models or generation. Signals: LVLM reasoning, Bloom taxonomy, evidence-driven, multi-encoder, hallucination reduction
...
### [109] [The Ebb and Flow of Multimodal Focus: Scheduling Visual Relay Windows for Grounded VLM Reasoning](https://arxiv.org/abs/2607.11436)
...
TL;DR: 本文通过机械论视角分析VLM内部多模态注意力聚焦的动态变化，发现了一个稳定的三阶段注意力再分配模式，并将中间阶段定义为视觉中继窗口（VRW）。基于此，提出TRACE框架，通过轻量级模块在推理时自适应控制视觉中继分配，在多个VLM骨干和基准上显著提升了基于证据的推理能力。


<details>
  <summary>Details</summary>
Motivation: VLM在多模态推理基准上表现越来越好，但视觉证据进入语言栈后往往变得不稳定，削弱了基于证据的推理能力。作者希望理解这种脆弱性的内部机制，并据此设计有效的推理时控制方法。

Method: 首先通过机械论分析揭示VLM深度层中多模态注意力聚焦的三阶段再分配模式：早期问题条件组织、中期视觉主导中继、晚期答案形成。将中期定义为视觉中继窗口（VRW），并证明其几何结构随任务需求变化、与生成质量因果相关。基于此提出TRACE框架，包含两个轻量级训练模块：在预填充阶段重塑中继分配，在解码阶段保持视觉支持。

Result: 在四个开源VLM骨干和七个基准上，TRACE在接地敏感任务上平均提升4.33分，最高提升6.6分，同时在推理密集型任务上也有改进。

Conclusion: 显式控制跨深度层的多模态注意力聚焦为增强基于证据的多模态推理提供了一种统一且有效的机制。

Classification reason: 该论文研究VLM内部多模态注意力聚焦的动态机制，提出推理时控制框架TRACE以增强基于证据的多模态推理。核心贡献在于多模态推理的深度分析与推理时控制，与[多模态推理与思维链]（multimodal_reasoning_cot）这一CANONICAL子主题高度匹配。

Importance reason: Directly advances multimodal reasoning and VLM internal dynamics, aligning with user's priority on continuous multimodal models and unified MLLMs. Signals: VLM internal attention dynamics, Visual Relay Window (VRW), inference-time control, grounded reasoning, multimodal understanding generation
...
### [110] [Interaction Scaling: Grounding the Third Axis of Test-Time Compute](https://arxiv.org/abs/2607.11598)
*Bojie Li, Noah Shi*
...
Importance: 值得关注 (65.21) - 未入选精读

TL;DR: 本文提出测试时计算的第三种扩展方式——交互式扩展（interaction scaling），即模型生成工件后由外部工具观察其实际行为并反馈，模型据此修订。与推理扩展和采样扩展不同，交互扩展通过引入真实观测打破性能天花板。在编程任务上，交互策略达到100%通过率；在视觉任务上，使用真实布局测量工具而非VLM截图评估才能有效修复缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有的两种测试时计算扩展方式（推理扩展和采样扩展）存在根本局限：它们都是"内部"的，所有额外token来自相同的冻结权重和相同的提示，无法告诉模型任何它不知道的信息。作者希望探索第三种方式——交互式扩展，通过引入外部真实观测来突破这一天花板。

Method: 提出"接地性"（grounding）作为控制交互扩展的核心变量，要求反馈和评估指标都必须来自真正观测到缺陷的外部工具。在编程任务上，构建proposer-reviewer框架，让模型生成代码后由编译器/测试工具反馈错误信息。在视觉任务上，对比使用VLM截图评估与使用真实布局测量工具两种反馈方式的效果。

Result: 在固定token预算下，推理扩展和best-of-N采样均出现性能平台期，而交互策略持续提升；proposer-reviewer框架在编程任务上达到100%通过率且零方差。在视觉任务上，VLM作为评判者会将14/15的明显缺陷评为"完美"，而真实布局测量工具能检测并修复40-74%的缺陷。

Conclusion: 交互式扩展是真实存在且区别于推理扩展和采样扩展的第三种测试时计算扩展方式，但其效果只有在反馈和评估指标都真正"接地"（即来自真实观测）时才显现。使用VLM作为视觉评判者会因截图隐藏缺陷而失效，必须使用直接测量真实布局的工具。

Classification reason: 论文研究测试时计算扩展的第三种方式——交互式扩展（interaction scaling），核心是模型通过外部工具获取真实反馈来迭代改进输出。这与多模态推理中的推理时计算扩展（inference scaling）高度相关，属于多模态推理与思维链（multimodal_reasoning_cot）范畴下的推理时计算扩展主题。

Importance reason: Proposes a novel third axis of test-time compute scaling via grounded interaction, breaking internal ceilings of reasoning and sampling. Signals: interaction scaling, test-time compute, grounded feedback, programming tasks, visual artifact repair
...
TL;DR: TreeSoc 提出一种树结构动态推理框架，通过深度优先搜索将复杂足球视频问答分解为有序子任务，并自适应路由领域工具模块，在 SoccerBench 和 NExT-QA 上取得 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 在复杂足球视频场景中面临浅层跨模态对齐、多步推理能力不足以及缺乏协调工具集成的问题，难以处理需要结构化推理的复杂查询。

Method: TreeSoc 将视频问答建模为层次化搜索问题，采用动态深度优先搜索（DFS）机制将复杂查询分解为有序子任务，通过显式中间状态实现迭代推理精炼。树结构分解支持自适应工具路由，在推理节点选择性激活领域特定模块并融合其输出，生成上下文相关的预测。

Result: 在 SoccerBench 上，TextQA、ImageQA、VideoQA 分别达到 85.2%、87.4%、82.2% 的准确率；在 NExT-QA 上达到 74.16%，展示了强跨域泛化能力。

Conclusion: 结构化、工具增强的树推理是鲁棒视频理解的有效范式，为复杂场景多步推理提供了新思路。

Classification reason: 该论文提出 TreeSoc，一种基于树结构动态推理（DFS）的足球视频理解框架，将复杂查询分解为子任务并逐步推理，属于多模态推理与思维链（CoT）范畴，因此映射到 [CANONICAL] subtopic "多模态推理与思维链" (multimodal_reasoning_cot)。

Importance reason: Tree-structured reasoning for video QA is solid but not aligned with your priority on continuous/multimodal models or generation. Signals: tree-structured reasoning, tool synergy, soccer video understanding, DFS decomposition, domain-specific modules
...
### [112] [Beyond the Eye: Efficient Multimodal Reasoning via Self-Regulated Implicit Visual Tools](https://arxiv.org/abs/2607.11106)
...
TL;DR: 本文提出 Beyond the Eye (BEE)，一种自调节隐式视觉工具范式，通过两阶段训练（形式化 CoT 监督微调 + 自调节奖励对齐）使 MLLM 在细粒度视觉感知任务中自适应平衡内部知识与隐式工具调用，减少冗余外部工具依赖和推理延迟，达到 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 现有 "Thinking with Images" (TwI) 范式依赖频繁的外部工具调用和重复图像重编码，导致巨大的计算开销和推理延迟。作者希望让模型自主决定何时调用视觉工具，避免不必要的工具依赖。

Method: BEE 采用两阶段训练：(1) 形式化 CoT 监督微调——构建带有结构化工具槽位和混合调用状态的 CoT 轨迹，激活模型的隐式工具表征和自适应切换能力；(2) 自调节奖励对齐——提出 Net Tool Gain (NTG) 指标量化冗余工具使用，并设计自调节奖励机制，惩罚无效工具依赖，鼓励模型在内部知识不足时才调用隐式工具。

Result: BEE 在细粒度视觉感知任务上达到 SOTA 性能，在通用推理任务上保持竞争力，同时显著提升推理效率，减少了冗余工具调用和推理延迟。

Conclusion: BEE 通过自调节机制有效平衡了内部知识与隐式工具调用，在保持高性能的同时大幅降低推理开销，为多模态推理中高效工具使用提供了新范式。

Classification reason: 该论文提出 BEE 模型，通过形式化 CoT 监督微调和自调节奖励对齐，实现多模态推理中隐式视觉工具的自我调节调用，属于多模态推理与思维链方向，因此映射到 canonical subtopic "multimodal_reasoning_cot"。

Importance reason: Directly advances efficient multimodal reasoning with self-regulated tool use, aligning with user's priority on continuous multimodal models and efficient inference. Signals: multimodal reasoning, efficient inference, self-regulated tool use, chain-of-thought, reward alignment
...
### [113] [When Depth Is Better Told Than Shown: Depth-Ordinal Prompting for Vision-Language Spatial Reasoning](https://arxiv.org/abs/2607.11173)
...
TL;DR: 本文提出 Depth-Ordinal Prompting (DOP)，一种无需训练的方法，通过将单目深度估计结果转换为针对查询目标的序数文本提示（如"物体A比物体B更近"），来提升视觉语言模型的空间推理能力。研究发现直接输入深度图反而会降低性能，而将深度信息以文本序数形式"告诉"模型比"展示"深度图更有效。


<details>
  <summary>Details</summary>
Motivation: VLM 在空间推理（如判断物体远近、前后遮挡关系）上表现不佳。直观的解决方案是输入深度图，但实验发现深度图反而会降低性能。作者希望找到一种无需训练、无需额外模块的方法来有效利用深度信息提升 VLM 的空间推理能力。

Method: DOP 方法：1) 使用单目深度估计器获取场景深度；2) 针对查询目标提取其深度值；3) 将深度信息转换为序数文本提示（如"the chair is closer than the table"），直接注入到语言 prompt 中。该方法不添加深度图像、不训练模块、不注入特征、不使用标签，完全无需训练。

Result: 在多个空间推理基准上，DOP 在伪深度提供可靠物体级排序时显著提升空间推理性能；在原始图像本身已很强的情况下基本保持中性。与最强的无训练深度提示方法相比具有竞争力，且更简单、更具针对性。

Conclusion: 深度信息的形式至关重要：以图像形式展示深度可能有害，而以文本序数形式"告诉"模型则能有效提升空间推理。DOP 是一种简单、无需训练、即插即用的空间推理增强方法。

Classification reason: 该论文研究视觉语言模型的空间推理（spatial reasoning），属于多模态推理范畴。现有 canonical subtopic "多模态推理与思维链"（multimodal_reasoning_cot）涵盖视觉推理与空间推理，与该论文主题高度匹配。

Importance reason: Novel training-free spatial reasoning method but lacks direct connection to user's priority subtopics like continuous models or video generation. Signals: training-free, spatial reasoning, depth ordinal prompting, VLM, form dependence
...
### [114] [Parse, Search, and Confirmation: Training-Free Aerial Vision-and-Dialog Navigation with Chain-of-Thought Reasoning and Structured Spatial Memory](https://arxiv.org/abs/2607.11529)
...
TL;DR: 本文提出PSC-AVDN，一种无需训练的空地视觉对话导航框架，通过三阶段解析-搜索-确认推理流水线与结构化空间记忆（SSM）相结合，在训练-free设置下实现了SOTA性能，甚至超越了一些微调方法。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在高空无人机视觉对话导航任务中因缺乏方向性定位和显式空间记忆而导致导航不可靠，且微调方法资源消耗大，因此需要一种无需训练的高效框架。

Method: 提出PSC-AVDN框架，包含：(1) 解析阶段——用LLM将模糊对话指令转化为稳定的几何方向和目的地线索；(2) 搜索思维链（S-CoT）——在高空观测下逐步探索目标；(3) 确认思维链（C-CoT）——在候选区域进行细粒度验证以解决视觉歧义；(4) 结构化空间记忆（SSM）——融合多尺度视觉观测、空间视觉记忆和结构化几何记忆三种空间线索，提供全局空间上下文和长程记忆。

Result: 在ANDH和ANDH-Full数据集上，PSC-AVDN在训练-free设置下取得了新的SOTA性能，匹配或超越了多个微调方法。

Conclusion: 通过将Chain-of-Thought推理与结构化空间记忆相结合，无需训练即可有效解决高空无人机视觉对话导航中的方向定位和空间记忆缺失问题，为资源受限场景下的AVDN任务提供了可行方案。

Classification reason: 该论文提出PSC-AVDN框架，利用Chain-of-Thought推理（S-CoT和C-CoT）进行空中视觉对话导航，属于多模态推理与思维链方向，因此映射到[CANONICAL] subtopic "多模态推理与思维链"。

Importance reason: Focuses on a niche aerial navigation task with limited cross-application to the user's priority subtopics like continuous language models or video generation. Signals: training-free, aerial navigation, chain-of-thought, structured spatial memory, multimodal reasoning
...
### [115] [Local Multimodal Music Alignment from Global Supervision](https://arxiv.org/abs/2607.10023)
*Irmak Bukey, Zachary Novack, Jongmin Jung, Dasaem Jeong, Chris Donahue*

Main category: cs.MM, cs.LG, cs.SD
...
TL;DR: 本文提出 FuSiLi（Fused Sinkhorn-Localized Similarity），一种基于 Sinkhorn 软对齐的局部多模态相似度评分方法，仅需全局监督即可学习音频与乐谱图像之间的局部对应关系，在跨模态检索和帧级对齐任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 音乐理解需要跨模态的局部对应关系（如音频时间点与乐谱图像位置的映射），但局部监督数据难以获取，通常只有粗粒度的全局配对监督可用。现有对比学习方法（如 CLIP、CLAP）只能学习全局对齐，无法捕捉细粒度的局部对应关系。

Method: 提出 FuSiLi 相似度评分，利用 Sinkhorn 算法对图像 patch 特征和音频帧特征进行软对齐，计算局部相似度矩阵。结合全局对比损失（InfoNCE）和局部 FuSiLi 损失，构建混合对比学习目标。在预训练 CLIP 和 CLAP 编码器基础上，对原始乐谱图像和对应音频对进行微调。

Result: 在跨模态检索和帧级对齐任务上，FuSiLi 在局部对齐性能上显著优于全局和局部基线方法，同时在跨模态检索任务上保持竞争力。

Conclusion: FuSiLi 证明了仅使用全局配对监督即可有效学习多模态局部对应关系，为缺乏细粒度标注的跨模态对齐任务提供了一种实用且有效的解决方案。

Classification reason: 该论文提出 FuSiLi 方法，通过 Sinkhorn 软对齐在局部图像块和音频帧特征上进行多模态对比学习，实现跨模态检索（音频-乐谱图像检索）和帧级对齐。核心任务属于跨模态检索，因此映射到 [CANONICAL] 多模态检索（multimodal_retrieval）子主题。

Importance reason: Focuses on music-specific multimodal alignment, not directly relevant to user's priority subtopics like continuous language models or video generation. Signals: Sinkhorn soft alignment, local multimodal correspondence, global supervision, music audio-score retrieval, CLIP/CLAP fine-tuning
...
### [116] [LightMem-Ego: Your AI Memory for Everyday Life](https://arxiv.org/abs/2607.11487)
...
Main category: cs.HC, cs.AI, cs.MM, cs.CL, cs.CV
...
TL;DR: LightMem-Ego 是一个面向日常生活的轻量级流式多模态记忆系统，通过持续捕获第一人称视觉和音频流，构建层次化记忆（当前/短期/长期），支持对象查找、对话回忆、生活总结等个性化辅助任务，可在智能手机和AI眼镜上部署。


<details>
  <summary>Details</summary>
Motivation: 移动和可穿戴设备上的个人AI助手需要持续感知用户的日常生活，但回答关于过去经历的查询需要轻量级的多模态记忆系统来持续积累、组织和检索长期经验，现有方法难以同时满足轻量化和长时记忆的需求。

Method: LightMem-Ego 持续捕获第一人称视觉和音频流，在共享时间线上对齐，并组织为层次化记忆（当前记忆、短期记忆、长期记忆）。给定用户查询时，系统动态路由到合适的记忆层级，基于多模态证据生成答案。系统可在智能手机和AI眼镜上部署。

Result: 系统支持对象查找、对话回忆、生活总结、日常发现和个性化辅助等多种任务，展示了在真实移动和可穿戴设备上的可行性。

Conclusion: LightMem-Ego 证明了轻量级流式多模态记忆系统在日常生活辅助中的有效性，为个人AI助手的长期记忆能力提供了实用方案。

Classification reason: 该论文构建了一个轻量级流式多模态记忆系统，持续捕获第一人称视觉和音频流，组织为层次化记忆结构，并基于用户查询进行多模态检索与问答。核心是跨模态（视觉+音频）的持续记忆积累与检索，属于多模态检索范畴，因此映射到 canonical subtopic "多模态检索"。

Importance reason: LightMem-Ego focuses on multimodal memory for daily life, which is not a priority subtopic and lacks strong alignment with the user's key directions like continuous language models or video generation. Signals: multimodal memory, egocentric vision, hierarchical memory, lightweight deployment, no continuous token or autoregressive generation
...
### [117] [Prompting-MammAlps: Fine-Grained Text-to-Video Retrieval for Camera-Trap Data](https://arxiv.org/abs/2607.09876)
...
Main category: q-bio.NC, cs.AI, cs.CV, q-bio.QM

Direction: 多模态理解与生成

Subtopic: 多模态检索

Importance: 值得关注 (66.96) - 未入选精读

TL;DR: 本文提出了 Prompting-MammAlps，首个面向相机陷阱生态数据的文本到视频检索基准，并设计了一种细粒度、可解释的检索方法：用视觉Transformer进行时空动作定位并输出结构化文本描述，再通过LLM编码代理解析查询并检索视频，在测试集上F1达34%，远超零样本VLM的18%。


<details>
  <summary>Details</summary>
Motivation: 相机陷阱数据集规模庞大，自动检索特定事件（如动物行为）非常困难。现有文本到视频检索方法缺乏时空理解能力，且难以泛化到生态数据，因此需要专门针对生态场景的细粒度、可解释的检索方法。

Method: 训练视觉Transformer进行时空动作定位，将输出转换为结构化文本描述每个视频；同时，基于动物行为学设计的查询由LLM编码代理处理，该代理使用自定义解析库中的函数解析结构化文本并检索视频，从而降低LLM幻觉风险并提升可解释性。

Result: 在Prompting-MammAlps基准上，该方法在135个生态相关查询和775个候选视频的测试集上达到34%的集合F1分数，而最佳零样本VLM仅达到18%，且缺乏可解释性。

Conclusion: 本文证明了结合结构化视频描述与LLM引导的解析方法在生态数据文本到视频检索中的有效性，显著优于纯VLM方法，并为该领域提供了首个标准化基准。

Classification reason: 该论文研究文本到视频检索（Text-to-Video Retrieval），属于跨模态检索范畴，与 [CANONICAL] 多模态检索（multimodal_retrieval）高度匹配。

Importance reason: Focuses on ecological video retrieval, not aligned with user's priority subtopics like continuous language models or video generation. Signals: Camera-trap retrieval, ecological data, fine-grained retrieval, LLM-based parsing, low F1 score
...
### [118] [SynCLIP: Synonym-Coherent Language-Image Pretraining for Robust Open-Vocabulary Dense Perception](https://arxiv.org/abs/2607.11008)
...
TL;DR: SynCLIP 提出同义词一致的图文预训练框架，通过语义一致空间注意力对齐（SSA）和空间注意力精炼（SAR）模块，解决 CLIP 模型对同义词表达产生不一致空间注意力的问题，并构建同义词增强视觉语料库 SEViC，在开放词汇密集感知任务上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有 CLIP 模型在开放词汇密集感知任务中，对语义等价的同义词（如 "dog" 和 "canine"）会产生不一致的空间注意力图，导致定位不稳定。这种同义词引起的接地不一致性严重影响了模型在真实场景中的鲁棒性和性能。

Method: 1) 语义一致空间注意力对齐（SSA）模块：最小化原始表达与同义表达之间注意力图的差异，增强空间注意力一致性。2) 空间注意力精炼（SAR）模块：选择性增强对齐后注意力图中语义最相关的空间区域，实现更精确稳定的接地。3) 构建同义词增强视觉语料库（SEViC）：为每个类别扩充多个同义词和文本定义，支持同义词一致的预训练。

Result: 在多个开放词汇密集感知基准上，SynCLIP 显著提升了不同语言变体下的接地一致性，在 CLIP 类方法中达到 SOTA 性能。

Conclusion: SynCLIP 通过同义词一致性预训练有效缓解了 CLIP 模型的同义词接地不一致问题，为开放词汇密集感知提供了更鲁棒的解决方案。

Classification reason: 该论文提出 SynCLIP，改进 CLIP 在开放词汇密集感知（OVDP）中的同义词鲁棒性，核心是语言-图像预训练中的语义对齐与空间注意力一致性，属于跨模态表征学习与检索增强方向，因此映射到 [CANONICAL] 多模态检索（multimodal_retrieval）。

Importance reason: Focuses on synonym robustness for CLIP-based dense perception, which is not aligned with user's priority on continuous multimodal models, video generation, or world models. Signals: CLIP-based OVDP, synonym grounding consistency, spatial attention alignment, not in priority subtopics, no boost keywords matched
...
### [119] [CLIR-Bench: Benchmarking Multimodal Question Answering over Irregular Clinical Time Series](https://arxiv.org/abs/2607.09880)
...
TL;DR: CLIR-Bench 是一个面向不规则临床时间序列的多模态问答基准，包含 6,600 个 QA 实例，覆盖 11 个临床变量、4 个能力维度和 11 个任务，用于评估模型在不规则时序数据上的证据检索与推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注规则采样的时间序列问答或静态医疗数据问答，缺乏对模型能否在不规则时序观测中忠实定位证据的评估。

Method: 从去标识化的 ICU 记录出发，通过四阶段流水线构建基准，包含 6,600 个 QA 实例，每个问题关联显式时序证据和任务特定的答案推导规则。

Result: 实验表明现有通用模型在检索和推理稀疏临床证据方面表现不佳，凸显了更强的不规则时间序列推理方法的必要性。

Conclusion: CLIR-Bench 填补了不规则临床时间序列 QA 评测的空白，揭示了当前模型在该任务上的局限性，为未来研究提供了方向。

Classification reason: 该论文提出了一个面向不规则临床时间序列的多模态问答基准 CLIR-Bench，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Focuses on clinical time series QA, which is outside the user's priority subtopics and has negative keywords. Signals: clinical time series, irregular sampling, medical QA, benchmark, multimodal understanding
...
### [120] [ResearchQA: Benchmarking Citation-Grounded Question-Answering on Scientific Papers](https://arxiv.org/abs/2607.11074)
...
TL;DR: ResearchQA 是一个面向科学论文的引用支撑问答基准，包含 6,211 个问答对，覆盖 8 个领域和 4 种问题类型（查找、理解、多跳、对抗），用于评估大模型在论文问答中是否生成可验证引用支撑的答案。


<details>
  <summary>Details</summary>
Motivation: 现有大模型辅助科学阅读的评估方法无法检测答案是否由可验证的引用支撑，缺乏对引用准确性和支撑性的细粒度评测。

Method: 构建了包含 6,211 个问答对的基准数据集，来自 494 篇开放获取论文，覆盖 8 个领域和 4 种问题类型。采用确定性引用匹配器和基于 LLM 的评分器进行引用支撑评估，支持多个有效支撑段落和引用支撑的拒绝回答。

Result: 引用指标（章节覆盖率和引用准确率）比 LLM 评分更能区分模型性能；开源模型在引用准确率上接近最佳闭源模型，且延迟低 3-6 倍。

Conclusion: ResearchQA 为科学论文问答提供了更细粒度的引用支撑评测框架，揭示了现有模型在引用准确性上的显著差异，并表明开源模型在效率与性能间取得了良好平衡。

Classification reason: 该论文提出了一个面向科学论文的引用支撑问答评测基准 ResearchQA，属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）这一规范主题。虽然论文聚焦于文本领域的科学论文问答，但其评测框架（引用支撑、多类型问题、LLM评估）与多模态评测基准的方法论高度一致，且该规范主题涵盖评测协议与自动评估。

Importance reason: Focuses on citation-grounded QA for scientific papers, not on the user's priority subtopics like continuous language models or video generation. Signals: Benchmark for citation verification, Not aligned with priority subtopics, No boost keywords matched, Negative keywords not present, Multimodal understanding but not generation
...
### [121] [RetroHolmes: When Semantic Plausibility Fails Retrospective Physical Process Reasoning](https://arxiv.org/abs/2607.11044)
...
TL;DR: 本文提出回顾性物理过程推理（RPPR）这一新评估范式，并构建了首个真实世界基准 RetroHolmes，包含物体中心图像对及可达性标签和因果步骤序列。通过评估多个 SOTA VLM，发现其在物理推理中存在判断偏差和信念主导等系统性失败模式，并验证了以视觉模拟为中间步骤的分析-合成方法的诊断价值。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 评估协议无法检验模型是否真正具备从稀疏观测中推断隐藏物理过程的能力，缺乏针对物理约束下反向推理的评测基准。

Method: 构建 RetroHolmes 基准，包含物体中心图像对（初始状态→最终状态），标注可达性标签和因果步骤序列。使用该基准评估多个 SOTA VLM，分析其推理失败模式。提出分析-合成（analysis-by-synthesis）方法，以视觉模拟作为中间步骤辅助推理。

Result: SOTA VLM 在 RetroHolmes 上表现不佳，存在可达性评估中的判断偏差和信念主导物理证据的系统性失败（类似 LLM 的谄媚行为）。分析-合成方法通过视觉模拟中间步骤提升了推理表现，验证了基准的诊断价值。

Conclusion: RetroHolmes 揭示了当前 VLM 在物理推理中的根本性缺陷，强调了物理有根基的中间表征对于物理推理的重要性，为未来 VLM 物理推理能力提升提供了评测工具和方向。

Classification reason: 该论文提出了 RetroHolmes，一个用于回顾性物理过程推理（Retrospective Physical Process Reasoning）的真实世界基准，评估 VLM 在物理约束下的反向推理能力。这属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题，因为论文核心贡献是一个新的评测基准和评估范式。

Importance reason: Introduces a novel physical reasoning benchmark but is far from the user's priority on continuous multimodal models and generation. Signals: physical reasoning benchmark, VLM failure modes, analysis-by-synthesis, not generation or continuous model, low priority subtopic match
...
### [122] [What Does Your Short-Answer VQA Score Actually Measure? Evaluator-Dependent Instability in Multimodal Short-Answer Benchmarks](https://arxiv.org/abs/2607.10240)
...
TL;DR: 本文系统研究了短答案VQA评测中语义正确性与表面形式匹配之间的混淆问题。通过人工验证的语义评判器（97.6%精度）审计了超过3.7万个官方错误，发现文本密集型基准中高达一半的错误是语义可接受但仅因表面形式不匹配而被惩罚的答案。这种不稳定性由答案类型决定：抽取式和多跨度答案比标量答案对评估器更敏感。提示和上下文的良性改写也会显著改变项目级正确性。


<details>
  <summary>Details</summary>
Motivation: 短答案VQA基准的分数同时衡量了模型答案的语义正确性和答案是否匹配自动评估器期望的表面形式，但这两者常常被混淆。现有研究缺乏对这种混淆的系统性量化分析，导致VQA分数难以解释。

Method: 使用人工验证的语义评判器（97.6%精度）审计6个视觉语言模型在6个基准上的超过3.7万个官方错误；使用第二个纯文本评判器复现相同模式以排除单一模型伪影；按答案类型（抽取式、多跨度、标量）分析评估器敏感性；通过提示和上下文改写测试稳定性；提出确定性CPU-only的契约修复方法。

Result: 文本密集型基准中高达一半的官方错误是语义可接受但仅因表面形式不匹配而被惩罚的答案；抽取式和多跨度答案比标量答案对评估器更敏感；提示和上下文的良性改写会显著改变项目级正确性；契约修复可以部分恢复被低估的分数。

Conclusion: 官方短答案VQA分数应附带语义审计和答案类型诊断以保持可解释性。评估器依赖的不稳定性是系统性的，需要更鲁棒的评估协议。

Classification reason: 该论文系统性地研究了短答案VQA评测中自动评估器导致的分数不稳定问题，属于多模态评测与基准方向，因此映射到[CANONICAL] subtopic "多模态评测与基准"。

Importance reason: This paper reveals a critical methodological flaw in VQA evaluation, but it is not directly aligned with your priority subtopics on continuous models, generation, or world models. Signals: VQA evaluation instability, semantic vs. surface-form mismatch, evaluator-dependent scoring, text-rich benchmark audit, answer-type sensitivity
...
### [123] [MAG: A Web-Agent Benchmark and Harness for Multimodal Action and Guide Generation](https://arxiv.org/abs/2607.10079)
...
TL;DR: MAG是首个统一网页智能体任务执行与指南生成的多模态基准，基于截图进行Set-of-Mark元素选择或像素坐标定位，构建了完整的评测框架，并设计了GRPO训练方法，将9B智能体的成功率从6.9%提升至13.2%。


<details>
  <summary>Details</summary>
Motivation: 现有数字采用平台（DAP）的网页指南生成与智能体任务执行被当作两个独立问题处理，且大多数方法使用DOM或可访问性树等文本表示而非人类实际操作的渲染截图，缺乏统一的多模态评测基准。

Method: 提出MAG基准，统一任务执行与指南生成为多模态动作与指南生成任务，支持Set-of-Mark元素选择和原始像素坐标两种截图定位方案；构建完整框架包括LLM辅助+人工验证的标注、训练、实时环境评估及联合指标；设计基于专家轨迹增强的GRPO训练方法。

Result: GRPO训练方法将9B智能体成功率从6.9%提升至13.2%（近翻倍），同时提升指南质量；最强模型（GPT-4o等）任务完成率仍低于40%，表明该基准具有挑战性。

Conclusion: MAG为网页智能体研究提供了统一的多模态评测基准，揭示了当前模型在真实网页任务上的显著不足，GRPO训练方法展示了强化学习在提升智能体性能上的潜力，为未来研究留下充足空间。

Classification reason: 该论文提出了MAG，一个统一任务执行与指南生成的网页智能体多模态评测基准，属于多模态评测与基准方向。虽然涉及网页智能体，但核心贡献是新的评测基准和评测协议，因此映射到[多模态评测与基准]（CANONICAL）。

Importance reason: Novel benchmark for web agents but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: New benchmark, Multimodal action and guide, GRPO training, 9B agent improvement, Web agent task
...
### [124] [Benchmarking Dynamic Affective Reasoning: A Viewer-Centric Video Emotion Dataset](https://arxiv.org/abs/2607.10238)
...
TL;DR: 本文提出了 DAR（Dynamic Affective Reasoning），首个面向观众视角的情感转换与因果推理的大规模视频情感基准数据集，包含15,087个视频和36,908个事件对齐的情感片段（27类情感），并定义了情感分割、细粒度分类和情感推理三个任务。同时提出 DAR-R1 两阶段框架（SFT + GRPO），在多个 MLLM 上取得了动态情感推理的最新最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频情感分析通常被建模为静态分类问题，忽略了情感随因果事件累积变化的心理学事实。缺乏一个能够捕捉情感动态转换和因果推理的大规模基准。

Method: 构建 DAR 数据集：从观众视角标注视频中连续事件对应的情感转换，提供密集的、时间对齐的、因果显式的推理链。提出 DAR-R1 两阶段框架：先进行监督微调（SFT），再使用 Group Relative Policy Optimization（GRPO）强化学习优化情感定位与推理能力。

Result: 在10+个多模态大语言模型（MLLM）上进行实验，DAR-R1 在情感定位和情感推理任务上均达到新的最优性能。

Conclusion: 动态情感推理是视频理解中一个重要但被忽视的方向，DAR 基准和 DAR-R1 框架为该领域提供了标准化的评测平台和有效的基线方法。

Classification reason: 该论文提出了 DAR（Dynamic Affective Reasoning）基准，包含15,087个视频和36,908个事件对齐的情感标注，定义了情感分割、细粒度情感分类和情感推理三个任务，属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。

Importance reason: Novel benchmark for dynamic affective reasoning, but outside user's priority subtopics and lacks boost keywords. Signals: new benchmark, dynamic affective reasoning, viewer-centric, emotion transitions, causal reasoning
...
### [125] [Benchmarking the Robustness of Foundation Models for Mammography under Domain Shift](https://arxiv.org/abs/2607.10358)
...
TL;DR: 本文对15个基础模型作为乳腺X光图像特征提取器时的域偏移鲁棒性进行了系统基准测试，使用统一的冻结骨干+线性探测协议，在3个源数据集上训练并在12个OOD数据集上评估。发现乳腺X光专用视觉语言模型（Mammo-FM、MaMA）平均OOD性能最强，但鲁棒性并非仅由乳腺X光预训练解释；DINOv3是强视觉基线，乳腺X光适配预训练并不一致提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 基础模型越来越多地被用作乳腺X光图像的特征提取器，但它们在外部域偏移下的鲁棒性尚不清楚。现有研究缺乏对多种基础模型在统一协议下的系统性域偏移鲁棒性评估。

Method: 采用统一的冻结骨干+线性探测协议，在3个源数据集上训练，在12个任务兼容的OOD数据集上评估。对15个基础模型骨干（包括通用视觉模型、医学视觉模型、乳腺X光专用视觉语言模型等）进行基准测试，涉及乳腺密度、BI-RADS严重程度和癌症状态三个临床任务。进行了标签统一化、数据集级分析以及特征空间分析。

Result: 乳腺X光专用视觉语言模型（Mammo-FM和MaMA）平均OOD性能最强，但鲁棒性不能仅由乳腺X光预训练解释。DINOv3是强竞争力的纯视觉基线。乳腺X光适配预训练并不一致提升泛化能力。即使领先模型在不同数据集上也表现出异质性。特征空间分析显示有用表征可以保留临床信号同时保留数据集和采集结构信息。

Conclusion: 数据集级OOD评估应作为评估乳腺X光表征的核心标准。乳腺X光专用预训练并非鲁棒性的充分条件，通用视觉模型（如DINOv3）仍具竞争力。研究强调了在医学影像基础模型评估中考虑域偏移的重要性。

Classification reason: 该论文系统性地评测了15个基础模型在乳腺X光摄影（mammography）任务中面对域偏移的鲁棒性，属于多模态（视觉-语言）模型在医学影像领域的评测工作，因此映射到[多模态评测与基准]（multimodal_benchmark）这一规范主题。

Importance reason: Focuses on medical domain robustness, not aligned with user's priority on continuous multimodal models and video generation. Signals: Medical domain shift, Mammography foundation models, OOD evaluation, Not aligned with user's priority subtopics, Negative keyword "clinical" present
...
### [126] [SynthDocBench: Controlled Benchmark for Long-Context Visual Document Understanding](https://arxiv.org/abs/2607.10400)
...
TL;DR: SynthDocBench 是一个全合成的长上下文视觉文档理解基准，通过组合设计系统控制文档长度、布局结构、模态组成和问题类型等因素，揭示了现有 VLM 在长文档场景下的三种失败模式：长度增加导致性能急剧下降、文档中间三分之一区域的位置敏感性、以及长文档中图表理解的崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有视觉文档理解基准（如 DocVQA、ChartQA、MMLongBench-Doc）虽然性能表现良好，但真实文档中长度、布局复杂度、模态和问题难度等多因素交织，难以将模型失败归因到具体原因。需要一个能系统控制各因素的基准来揭示模型的真实能力与失败模式。

Method: 采用组合设计方法，独立变化文档长度、布局结构、模态组成和问题类型四个因素，通过 LLM 流水线端到端生成文档。设计了六种布局原型（纯文本、表格、图表、混合等），并引入 40% 的随机覆盖机制防止模型利用虚假相关性。生成了比现有基准更长、结构更多样的长上下文文档。

Result: 在七个前沿 VLM 上评估，发现了三种现有基准无法揭示的失败模式：(1) 文档长度增加导致性能急剧下降；(2) 文档中间三分之一区域对五个模型最难，呈现系统性位置敏感性；(3) 五个模型呈现负向的 Early-to-Late 趋势（最大下降 8.3 个百分点）；(4) 长文档设置下图表理解能力崩溃。

Conclusion: 当前 VLM 可能过度拟合了现有基准的伪影，而非真正实现了鲁棒的长上下文视觉文档理解。SynthDocBench 通过可控因素设计为诊断模型在长文档场景下的真实能力提供了有效工具。

Classification reason: 该论文提出了一个用于长上下文视觉文档理解的全合成基准 SynthDocBench，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Controlled benchmark for long-context document understanding reveals failure modes, but is not aligned with user's priority subtopics. Signals: long-context visual document understanding, controlled benchmark, VLM failure modes, synthetic data, position sensitivity
...
### [127] [Are LLMs Ready for Scientific Discovery? A Capability-Oriented Benchmark for AI Scientists](https://arxiv.org/abs/2607.11079)
...
TL;DR: 本文提出 SDABench，一个面向 LLM 科学数据分析能力的评测基准，包含 527 个真实实例和 6000 个合成实例，覆盖六个核心能力（描述性、探索性、推断性、预测性、因果性、机制性）和五个科学领域。评估 15 个 LLM 发现模型在描述性分析上表现良好，但在假设选择、潜过程建模和机制推理上严重退化。


<details>
  <summary>Details</summary>
Motivation: 现有科学数据分析基准主要评估 LLM 的代码执行或工作流完成能力，忽略了科学分析服务于不同类型科学主张（假设探索、统计推断、机制解释）这一事实，每种类型有不同的假设和有效性标准。需要一个更细粒度的、面向能力的评测框架来揭示 LLM 在科学发现中的真实能力与局限。

Method: 构建 SDABench 基准，将科学数据分析评估重新组织为六个能力维度（描述性、探索性、推断性、预测性、因果性、机制性），覆盖五个领域（生物、化学、环境、地理、物理）。包含 SDA-Real（527 个真实数据实例）和 SDA-Synth（6000 个合成实例），每个实例同时提供多项选择和开放式格式。通过自动化流水线构建，并设计了五阶段错误分析框架来定位 LLM 的失败环节。

Result: 评估 15 个代表性 LLM 发现：模型在描述性分析上表现良好，但在需要假设选择、潜过程建模或机制推理的任务上性能严重退化。更先进的模型能更可靠地识别相关范围和变量，但在选择适当分析程序、建模变量关系和得出有效结论方面仍然困难。

Conclusion: 当前 LLM 在科学数据分析中仍存在显著局限，尤其是在需要统计推断、因果推理和机制建模的高级分析任务上。SDABench 提供了一个细粒度的能力导向评测框架，有助于指导未来 AI 科学家系统的发展方向。

Classification reason: 该论文提出了 SDABench，一个面向 LLM 科学数据分析能力的评测基准，属于多模态/LLM 评测范畴。虽然论文主要关注 LLM 而非多模态，但评测基准类工作应归入 [CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark），该 subtopic 涵盖评测基准、评测协议等广义评测工作。

Importance reason: Focuses on LLM scientific data analysis benchmarks, not on the user's priority subtopics like continuous multimodal models or video generation. Signals: LLM benchmark, scientific discovery, capability evaluation, not aligned with user's priority subtopics, no boost keywords matched
...
TL;DR: Omni-Decision是一个无需训练的证据状态系统，将全模态QA转化为查询范围的证据闭合过程。它维护结构化的证据状态（已确认证据、未解决冲突、事实/计算依赖、开放证据需求），通过确定性状态更新实现目标导向的证据获取、验证、修复和终止。在OmniGAIA和WorldSense上分别达到45.6%和58.3%准确率，比基线提升27.3和30.2个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有多模态agent系统通常将证据留在草稿板、工具轨迹或自由形式历史中，难以追踪哪些证据已被确认、哪些仍然缺失、以及何时证据足够回答问题。这导致在多模态证据稀疏分布的场景下，系统无法有效进行证据获取和决策。

Method: Omni-Decision维护一个结构化的证据状态，包含已确认证据、未解决冲突、事实与计算依赖、以及开放证据需求。共享状态视图条件化规划、证据获取、验证、修复和最终化。来自媒体、网页、计算和验证模块的异构观测通过确定性状态更新进行归一化、判断和提交。系统采用无训练方式，通过状态驱动实现目标导向的证据获取和可检查的修复/停止控制。

Result: 在OmniGAIA上达到45.6%准确率（比基线+27.3），在WorldSense上达到58.3%准确率（比基线+30.2）。无状态消融实验和轨迹审计进一步验证了显式证据状态控制在多步全模态证据搜索中的关键作用。

Conclusion: 显式的证据状态管理能够有效提升全模态QA系统在稀疏证据场景下的表现，提供可检查的证据追踪和修复控制，是一种无需训练的有效范式。

Classification reason: 该论文提出Omni-Decision系统，用于全模态证据搜索QA，并在OmniGAIA和WorldSense基准上进行评测。虽然论文核心是agent系统，但其主要贡献在于多模态QA的评测框架和证据状态管理，与多模态评测与基准（multimodal_benchmark）方向高度相关，因此复用该规范子主题。

Importance reason: Focuses on omni-modal QA agent system design, not directly on the user's priority subtopics like continuous language models or video generation. Signals: Training-free evidence-state system, Omni-modal QA, Strong accuracy gains on OmniGAIA and WorldSense, Structured evidence state tracking, Not aligned with user's priority subtopics
...
### [129] [Do Video-LLMs Actually Watch? Diagnosing Character-Tracking Failures in Long-Form Video](https://arxiv.org/abs/2607.11078)
...
TL;DR: 本文通过九条件诊断协议测试 Video-LLM 在长视频中追踪角色外貌变化的能力，发现模型准确率并非来自真正的角色追踪：当问题中角色名被替换时，模型仅 4-31% 的情况下改变答案，且主要依赖性别线索而非个体身份识别。开源模型在开放式问答中准确率下降 18-25 点，151 个答案无一完全正确。添加字幕、使用关键帧或增加帧数均无法改善，表明瓶颈在于模型无法将视频内容与问题中指定的人物关联起来。


<details>
  <summary>Details</summary>
Motivation: 现有 Video-LLM 基准在长视频角色追踪任务上报告了看似合理的分数（如 InfiniBench 上 37-38%），但这些分数是否真正反映了模型追踪指定角色的能力尚不清楚。作者希望揭示模型在这些任务中究竟学到了什么，以及基准分数是否被表面线索（如性别）所混淆。

Method: 设计九条件诊断协议，包含：(1) 角色名替换测试（换为不同角色、同性别/不同性别角色）；(2) 开放式问答 vs 多项选择对比；(3) 添加字幕、使用信息量最大的帧、加倍帧数等消融实验。测试三个架构不同的开源 Video-LLM（7-8B）及 Gemini 2.5 Flash 作为前沿参考。

Result: (1) 角色名替换后模型仅 4-31% 改变答案，说明模型基本忽略问题中指定的人物；(2) 替换为不同性别角色时模型反应更大（13-28 点差距），说明模型依赖粗粒度性别线索而非个体识别；(3) 开放式问答中开源模型准确率下降 18-25 点，151 个答案无一完全正确；(4) 添加字幕、使用关键帧、加倍帧数均无法改善追踪能力。

Conclusion: 当前 Video-LLM 在长视频角色追踪任务上的表现并非来自真正的角色跟踪能力，而是依赖性别等表面线索。模型无法将视频内容与问题中指定的人物身份关联起来，这是架构层面的根本瓶颈。作者发布了诊断工具包，用于审计基准分数实际衡量的能力。

Classification reason: 该论文系统性地诊断 Video-LLM 在长视频角色追踪任务中的失败模式，设计了一套九条件诊断协议来审计现有基准分数究竟衡量了什么，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准"。

Importance reason: Diagnoses a critical failure mode in Video-LLMs but is a benchmark/analysis paper, not a method advancing the user's priority subtopics. Signals: Video-LLM evaluation, character tracking failure, benchmark diagnosis, not a method paper, low overlap with priority subtopics
...
### [130] [GDP.pdf: Benchmarking Grounded Multimodal Reasoning over Professional PDF Documents](https://arxiv.org/abs/2607.11192)
...
TL;DR: GDP.pdf是一个由专业人士构建的多模态推理基准，包含100个来自十个专业领域的PDF文档问答对，用于评估前沿多模态模型在真实专业文档上的综合推理能力。评测发现最佳模型仅通过15%的任务，主要失败模式包括表格错位、图表误读、脚注忽略等。


<details>
  <summary>Details</summary>
Motivation: 现有文档AI基准通常孤立地评测OCR、布局分析、图表推理、表格问答等单项能力，高分并不代表模型能回答专业人士在实际工作中对特定PDF提出的真实问题。因此需要构建一个直接衡量模型在真实专业文档上综合推理能力的基准。

Method: 由十个专业领域的从业者撰写问题-文档对，每个候选问题需至少两个前沿多模态模型以实质性方式（错误答案、遗漏关键证据、捏造事实）失败才被保留。每个问题附带原子评分标准（rubric），可计算分级评分和严格任务级通过率，并按三级十一类能力标签体系进行标注。

Result: 在100项基准上评测了7个前沿多模态模型，最佳模型仅通过15%的任务，最差仅通过1%。主要错误模式包括：表格对齐错误、图表误读、脚注和排除条款被忽略、平面图符号计数错误、扫描噪声干扰、以及修正条款覆盖原文等。

Conclusion: 现有前沿多模态模型在专业PDF文档的接地多模态推理任务上表现严重不足，揭示了当前模型在细粒度文档理解、跨引用推理和空间理解等方面的系统性缺陷，为未来模型改进提供了明确方向。

Classification reason: 该论文提出了一个针对专业PDF文档的接地多模态推理基准（GDP.pdf），属于多模态评测与基准方向。该基准包含十个专业领域的问题-文档对，评估模型在文本提取、表格/图表理解、空间推理等多种能力上的表现。这与[多模态评测与基准]（multimodal_benchmark）这一CANONICAL subtopic高度匹配。

Importance reason: Benchmark is solid but focuses on document VQA, not on the user's priority of continuous multimodal models or video/image generation. Signals: Professional PDF benchmark, Multimodal reasoning evaluation, Low model pass rate (15%), Detailed error taxonomy, Not aligned with user's priority subtopics
...
### [131] [SLVMBench: Skill Learning from Video Memory](https://arxiv.org/abs/2607.11312)
...
TL;DR: SLVMBench 是首个评估视频大语言模型从长视频记忆中学习技能并实时应用的基准，包含2-3小时视频流（含嵌入的教程视频），测试模型记忆、提取程序知识并迁移到实时任务的能力。评测显示现有视频LLM在此任务上表现困难。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解基准仅关注被动理解，技能学习基准依赖短时即时演示，缺乏评估模型从长视频记忆中学习技能并实时应用的基准。作者希望填补这一空白，揭示视频LLM在真实人类学习场景中的能力局限。

Method: 构建 SLVMBench 基准：将2-3小时教程视频嵌入任意无关视频流中，要求视频LLM从中学习技能并回答关于正在播放视频的实时问题。包含亚秒级时间校准的人工标注、排除常识猜测的精心设计问题以及确保技能覆盖的整理教程。

Result: 对多个最先进的开源和闭源视频LLM的评估显示，模型在从视频中学习和应用技能知识方面表现困难。当技能知识位于长视频记忆中时，性能显著下降。

Conclusion: SLVMBench 揭示了现有视频LLM在实时技能获取与应用方面的关键局限性，为研究长上下文视频记忆中的技能学习提供了首个基准。

Classification reason: 该论文提出了 SLVMBench，一个评估视频大语言模型从长视频记忆中学习技能并应用于实时任务的新基准，属于多模态评测与基准方向。该论文与 [CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark）高度匹配。

Importance reason: Novel benchmark for video-LLM skill learning from long video memory, but lacks direct alignment with user's priority subtopics like continuous models or generation. Signals: First benchmark for skill learning from long video memory, Tests real-time application of procedural knowledge, Reveals major limitations in current video-LLMs, Sub-second temporal calibration, Low relevance to user's focus on continuous models and generation
...
### [132] [Quantum Circuit Vision: Cost-Aware Evaluation of Visual AI Agents for Quantum Code Generation](https://arxiv.org/abs/2607.10057)
...
TL;DR: 本文提出Quantum Circuit Vision，一个面向多模态AI agent的量子电路图视觉理解与代码生成评测框架。构建了132个电路基准（13类，1-10量子比特），评估Claude系列模型，发现中端模型Sonnet在成本-准确率权衡上最优（91%通过率，成本仅为最强模型的18%），电路深度是失败的主要预测因子，链式思维提示无显著效果，级联路由策略可在38%成本下达到84%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有量子代码生成工作主要基于文本输入，缺乏对多模态AI agent在量子电路图视觉理解与代码生成方面的系统性评测，尤其是缺乏成本感知的评估框架。作者希望回答：AI agent能否从电路图中视觉理解并生成可执行代码？以及不同成本层级的模型在此任务上的表现如何？

Method: 构建了包含132个电路图的QCV-Dataset基准，涵盖13个类别（1-10量子比特），配有可执行的Amazon Braket代码和酉保真度验证。评估三个Claude系列模型（Opus、Sonnet、Haiku），进行n=5次重复试验，使用逻辑回归分析失败预测因子，并测试链式思维提示和级联路由策略。

Result: 中端模型Sonnet在核心子集上达到91%通过率，成本仅为最强模型Opus的18%，且准确率差异无统计学显著性（p=0.083）。电路深度是失败的主要预测因子（p<0.001），而非量子比特数。链式思维提示无显著效果（p>0.18）。级联路由策略（廉价→昂贵模型）在38%成本下达到84%准确率。

Conclusion: 多模态AI agent能够从量子电路图中视觉理解并生成可执行代码，但成本-准确率权衡至关重要——中端模型往往是最优选择。模型路由（级联策略）比提示工程更有效地控制成本。电路深度而非量子比特数是主要困难来源，视觉模式识别比显式推理策略对结构耦合的电路图理解更为关键。

Classification reason: 该论文提出了一个多模态AI agent在量子电路图视觉理解上的评测框架（Quantum Circuit Vision），包含132个电路基准数据集和成本感知评估协议。这属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题，因为核心贡献是构建了一个新的多模态视觉理解评测基准，并进行了系统的成本-准确率分析。

Importance reason: Focuses on quantum circuit code generation, which is outside the user's priority subtopics and negative-keyword domains. Signals: quantum circuit, cost-aware evaluation, multimodal agent benchmark, no priority subtopic match, negative keyword overlap with "quantum"
...
### [133] [MM-ToolSandBox: A Unified Framework for Evaluating Visual Tool-Calling Agents](https://arxiv.org/abs/2607.11818)
...
TL;DR: MM-ToolSandBox 是一个面向视觉工具调用智能体的统一评测框架和基准，包含500+工具、16个应用领域、258个人工验证场景和50个UI变体场景。对12个前沿模型的评估显示，最佳模型成功率不足50%，且失败分析揭示视觉精度（而非规划能力）是主要瓶颈——53%的失败源于图像信息提取错误。随着模型规模增大，失败模式从"不知道做什么"转向"看不清什么"，表明不同能力层级的模型需要不同的改进方向。


<details>
  <summary>Details</summary>
Motivation: 现有工具调用智能体评测主要关注纯文本场景，缺乏对视觉信息（多图像、渐进式视觉输入）和真实对话现象（目标修正、错误纠正、状态变化）的评测能力。作者希望构建一个统一的、状态化的、视觉驱动的工具调用评测框架，以系统评估当前模型在视觉工具调用方面的真实能力。

Method: 提出MM-ToolSandBox框架，包含：(1) 状态化执行环境，支持500+工具、16个应用领域、多图像多轮交互；(2) 自动化场景生成流水线，通过信息流引导规划和多阶段质量过滤生成多样化场景；(3) 258个人工验证的标准场景和50个UI交互变体场景。对12个模型（从4B开源到前沿闭源系统）进行系统评估和失败分析。

Result: 最佳模型（GPT-4o）成功率低于50%；53%的失败源于图像信息提取错误（视觉精度问题），而非任务规划错误；发现"规划-精度交叉"现象：小模型主要失败于决策（不知道做什么），大模型主要失败于感知（看不清什么）。

Conclusion: 当前模型在视觉工具调用方面仍缺乏鲁棒性，视觉精度是比规划能力更关键的瓶颈。不同能力层级的模型面临根本不同的挑战，需要针对性的研究方向。MM-ToolSandBox为视觉工具调用智能体的评测提供了统一框架和基准。

Classification reason: 该论文提出了一个面向视觉工具调用智能体的评测框架和基准（MM-ToolSandBox），属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）这一规范主题。论文核心贡献是构建了包含500+工具、多图像多轮交互的评测场景，并对12个模型进行了系统评估，完全符合该主题的定义。

Importance reason: Strong benchmark for visual tool-calling but not aligned with user's priority on continuous multimodal models or generation. Signals: visual tool-calling benchmark, failure analysis reveals visual precision bottleneck, planning-to-precision crossover with scale, not in priority subtopics, no boost keywords matched
...
### [134] [Beyond the Single Camera: Agentic Multi-View Reasoning in Sports Video Understanding](https://arxiv.org/abs/2607.11844)
...
TL;DR: 本文提出 SportMV-Bench，首个多视角体育视频理解基准，包含 787 个多视角视频包和 2592 个问答对，覆盖感知识别、规则理解和裁判决策三类任务。评测发现现有 MLLM 无法有效利用多视角信息，瓶颈在于细粒度视觉感知和视角选择。作者提出 SportMV-Agent 代理框架，通过主动视角选择、感知工具执行和证据推理的迭代循环，相对最强基线提升 14.46%。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 在单视角视频理解上表现良好，但体育视频存在密集遮挡、快速运动和复杂交互，单视角难以解决。实际比赛中多角度摄像头提供互补信息，但缺乏评估 MLLM 多视角体育视频理解的基准。

Method: 构建 SportMV-Bench 基准：从官方比赛录像中通过 LLM 生成、MLLM 验证和人工过滤的流水线构建 787 个多视角视频包和 2592 个 QA 对，分为感知识别(PAR)、规则事件理解(REI)和裁判决策推理(ADR)三类。提出 SportMV-Agent 代理框架，包含主动视角选择、感知工具执行和证据推理的迭代循环。

Result: 现有 MLLM 在多视角体育视频理解上表现不佳，瓶颈在于细粒度视觉感知和视角选择而非逻辑推理或领域知识。SportMV-Agent 相对最强 MLLM 基线提升 14.46%。

Conclusion: 多视角信息对体育视频理解至关重要，现有 MLLM 在此任务上存在明显不足，SportMV-Agent 通过主动视角选择和工具增强推理有效提升了性能，为多视角视频理解提供了新方向。

Classification reason: 该论文提出了 SportMV-Bench，一个多视角体育视频理解基准，并评测了多种 MLLM 的性能，属于多模态评测与基准方向。同时论文提出了 SportMV-Agent 代理框架，但核心贡献是基准构建与评测，因此映射到 [CANONICAL] subtopic "多模态评测与基准"。

Importance reason: Novel multi-view sports benchmark and agent framework, but outside user's priority subtopics and direction weights. Signals: Multi-view video understanding, Sports domain, MLLM benchmark, Agentic framework, Fine-grained perception bottleneck
...
### [135] [TIGER: Text-Conditioned Visual Gated Routing with Acceptance Alignment for Multimodal Speculative Decoding](https://arxiv.org/abs/2607.11131)
...
Importance: 高优先级 (81.3) - 未入选精读

TL;DR: TIGER 提出了一种文本条件视觉门控路由框架，用于多模态推测解码加速。它动态选择与当前文本状态相关的稀疏视觉 token 子集，并采用基于接受前缀长度的奖励进行分组策略训练，使轻量级 drafter 生成更易被大模型验证通过的推测序列，从而提升 VLM 推理速度。


<details>
  <summary>Details</summary>
Motivation: 推测解码在纯文本 LLM 中效果显著，但在 VLM 中增益有限，因为轻量级 drafter 在视觉关键内容上容易产生偏差，而现有多模态加速方法未直接解决无关视觉证据问题，也未优化验证器接受的前缀长度（这是决定加速比的关键因素）。

Method: TIGER 包含两个核心设计：(1) 文本条件视觉门控路由（Text-conditioned Visual Gated Routing），根据 drafter 当前文本状态动态选择稀疏的上下文相关视觉 token 子集，而非使用全部视觉 token 或固定压缩接口；(2) 接受对齐分组策略训练（Acceptance-Aligned Group-based Policy Training），基于验证器返回的接受前缀长度作为奖励信号，结合 KL 锚定的蒸馏热启动，训练 drafter 生成更易被验证通过的推测序列。

Result: 实验表明，TIGER 在精确验证器侧推测解码下，在接受的 token 前缀长度和推测加速比上取得了一致提升，并在视觉路由分析中实现了有利的质量-延迟权衡，下游准确率相当。

Conclusion: TIGER 通过动态视觉 token 选择和接受对齐训练，有效提升了多模态推测解码的加速效果，为 VLM 的高效推理提供了新思路。

Classification reason: 该论文提出 TIGER，一种面向多模态推测解码（speculative decoding）的加速框架，通过视觉 token 门控路由和接受对齐策略优化来加速 VLM 推理。这属于多模态模型的高效推理加速，与 [CANONICAL] 多模态高效化与轻量化（efficient_multimodal）方向高度吻合。

Importance reason: Directly addresses efficient inference for multimodal LLMs, a high-priority subtopic, with a novel routing and alignment method. Signals: multimodal speculative decoding, efficient inference, visual token routing, acceptance alignment, VLM acceleration
...
### [136] [Spectral Heat Flow for Conservative Token Condensation in Vision-Language Models](https://arxiv.org/abs/2607.10640)
...
TL;DR: SpecFlow 是一种无需微调的视觉 token 压缩框架，通过谱热流（spectral heat flow）保持结构一致性、自适应空间分区保证覆盖、以及核心集汇合（coreset sinks）维持统计守恒，在 LLaVA-1.5 上压缩 88.9% 的视觉 token 仍保留 95.6% 的原始性能。


<details>
  <summary>Details</summary>
Motivation: VLM 推理时处理长序列视觉 token 成本高昂，现有 token 剪枝方法在高压缩率下会盲目丢弃信息、破坏空间结构或导致多样性坍塌，亟需一种更稳定的 token 压缩方法。

Method: 将视觉 token 视为 kNN 图中的节点，(i) 通过谱热流计算稳定的重要性场以保持结构连贯性，(ii) 通过自适应空间分区分配预算以保证空间覆盖，(iii) 将丢弃的信息聚合到核心集汇合（coreset sinks）中以维持统计守恒。该方法无需训练，即插即用，兼容 FlashAttention。

Result: 在多个任务、VLM 架构和压缩率下均优于 SOTA 方法。LLaVA-1.5 在压缩 88.9% 视觉 token 时仍保留 95.6% 的原始性能，实现了出色的效率-精度平衡。

Conclusion: SpecFlow 通过从破坏性剪枝转向保守性凝聚（conservative condensation），严格保证空间覆盖和统计守恒，为 VLM 提供了一种高效且稳定的 token 压缩方案。

Classification reason: 该论文提出 SpecFlow，一种无需训练的视觉 token 压缩方法，旨在减少 VLM 推理时的视觉 token 数量以提升效率。这属于多模态高效化与轻量化（efficient_multimodal）中的 token 压缩方向，与 canonical 子主题高度匹配。

Importance reason: Directly addresses efficient inference in multimodal models, a high-priority subtopic, with a novel training-free token condensation method. Signals: training-free token compression, spectral heat flow, conservative condensation, strong performance at high compression, plug-and-play with FlashAttention
...
### [137] [TOLiD: Bridging the Architecture Gap in Vision Foundation Model to LiDAR Pretraining via Token Lifting for Distillation](https://arxiv.org/abs/2607.10762)
...
TL;DR: TOLiD 提出一种自监督预训练方法，通过 token 级蒸馏将视觉基础模型（VFM）的知识迁移到 LiDAR 骨干网络。该方法引入 Frustum Pooling 和 Frustum Attention 将点云特征转换为与 ViT 兼容的 patch token，并通过可见性掩码进行 token 级蒸馏，有效弥合了密集 ViT 与稀疏 3D 编码器之间的架构鸿沟。


<details>
  <summary>Details</summary>
Motivation: 现有的跨模态蒸馏方法通常将 VFM 作为冻结特征源，训练异构的 3D 骨干网络去匹配固定的图像嵌入，这迫使学生网络同时弥合模态鸿沟和跨架构鸿沟（密集 ViT token 表示 vs 稀疏 3D 编码器），导致蒸馏效率低下。

Method: TOLiD 将 LiDAR 骨干与一个从冻结 VFM 教师初始化的学生 ViT 耦合，在兼容的 patch-token 表示上施加监督。具体地，通过 Frustum Pooling 将每个图像 patch 视锥内的点特征聚合为 token，再经 Frustum Attention 进一步处理，并采用可见性掩码进行 token 级蒸馏。对于仅使用 LiDAR 的部署场景，通过掩码双线性采样将 token 特征提升回逐点表示。

Result: 在五个异构 LiDAR 数据集和四个跨传感器适应对上的广泛评估表明，TOLiD 在使用冻结骨干和轻量级头部的情况下实现了改进的迁移性能。

Conclusion: TOLiD 通过 token 级蒸馏有效弥合了 VFM 与 LiDAR 骨干之间的架构鸿沟，为 LiDAR 表征学习提供了一种高效的自监督预训练方法，在跨传感器迁移场景中表现出色。

Classification reason: 该论文提出 TOLiD，通过 token 蒸馏将视觉基础模型（VFM）的知识迁移到 LiDAR 骨干网络，属于跨模态蒸馏与表征学习。这与多模态理解与生成方向下的"多模态高效化与轻量化"（efficient_multimodal）子主题高度相关，因为蒸馏是该子主题的核心方法之一。同时论文涉及跨模态（图像→点云）的知识迁移，属于多模态蒸馏范畴。

Importance reason: Focuses on LiDAR pretraining and cross-modal distillation, which is not a priority subtopic and lacks direct connection to continuous language/multimodal models or video/image generation. Signals: LiDAR pretraining, cross-modal distillation, vision foundation model, token-level distillation, 3D scene understanding
...
### [138] [Opti-Agent-Bench: Benchmarking End-to-End Optimization R&D Agents on Real-World Business Problems](https://arxiv.org/abs/2607.10768)
...
TL;DR: Opti-Agent-Bench是一个评估LLM智能体在真实业务优化问题中端到端能力的基准，涵盖从业务需求理解、数学建模、算法选择、代码实现到报告生成的完整研发流程，揭示了现有模型在约束遗漏、模型-代码不一致等关键缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有优化基准仅评测预结构化数学公式上的求解能力，忽略了从非结构化业务描述到正确数学模型这一最关键的转化环节，无法反映LLM智能体在真实优化研发中的实际能力。

Method: 设计了包含三大支柱的基准：(1) 业务语义真实性，含反模板陷阱防止模式匹配；(2) 模块化评测，含跨模块一致性检查（问题理解、形式建模、实现、报告四个模块）；(3) ORAC双层有效性框架，同时保证任务质量和评分完整性。覆盖整数规划、鲁棒优化、随机规划和非凸优化等工业级任务。

Result: 在多个工业规模任务上暴露了当前模型的关键失败模式：约束遗漏、模型-代码不一致、报告-实现偏差，这些问题在传统单指标评测下不可见。

Conclusion: Opti-Agent-Bench填补了端到端优化研发评测的空白，揭示了现有LLM智能体在完整优化pipeline中的系统性缺陷，为未来优化智能体的发展提供了有价值的评测框架。

Classification reason: 该论文提出了一个面向LLM优化智能体的端到端评测基准（Opti-Agent-Bench），属于生成模型/智能体评测范畴。虽然论文主题是优化问题求解而非传统多模态生成，但"生成模型评测与数据集"（generation_benchmark）是跨方向的通用评测类目，且该基准涉及从业务描述到代码实现的完整pipeline评测，符合评测基准的定义。该subtopic在multimodal_understanding_generation方向下已有非canonical条目，直接复用。

Importance reason: Focuses on business optimization agents, not on the user's priority subtopics like continuous multimodal models or video generation. Signals: Benchmark for optimization agents, real-world business problems, not aligned with user's priority subtopics, lacks boost keywords, no negative keywords triggered
...
### [139] [Model Guides You How to Draw: Adaptive Visual Gating for Unified Multimodal Reasoning](https://arxiv.org/abs/2607.10004)
...
TL;DR: 本文提出 AdaViG，一种无需训练的自适应视觉门控方法，用于统一多模态模型中的交错推理。通过利用模型内部信号（生成意图和视觉保真度）在早期阶段评估视觉生成步骤是否有益，动态决定是否中止无益的视觉生成，从而提升推理准确率并降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 在统一多模态模型的交错推理范式中，生成中间视觉推理步骤并非总是有益的，自生成的视觉步骤可能引入错误视觉证据误导后续推理，同时频繁触发视觉生成会带来巨大的计算和内存开销。因此需要一种方法来自适应地判断何时应该执行视觉生成步骤。

Method: 提出 AdaViG（Adaptive Visual Gating），一种无需训练的自适应视觉门控方法。该方法利用模型内部的两个信号：1）生成意图（Generation Intent）——反映模型是否有具体的文本计划来指导绘制；2）视觉保真度（Visual Fidelity）——衡量视觉生成是否仍然基于原始输入图像。在视觉生成的早期阶段评估这两个信号，当两者都较弱时中止该视觉步骤，从而防止误导性视觉证据进入推理轨迹并避免不必要的计算。

Result: AdaViG 在视觉数学推理任务上准确率提升最高达 5.7%，同时减少视觉生成 FLOPs 25.0%-91.0%，端到端延迟降低 15.4%-45.6%。

Conclusion: 通过自适应地控制视觉生成步骤的触发，可以在提升推理准确性的同时显著降低计算开销，验证了模型内部信号对于判断视觉步骤有效性的价值。

Classification reason: 该论文研究统一多模态模型（UMM）中交错推理时的自适应视觉门控机制，属于理解与生成一体化的统一多模态模型范畴，因此映射到该现有子主题。

Importance reason: Directly addresses unified multimodal reasoning efficiency and accuracy via training-free adaptive gating, aligning with user's high-weight priority on unified multimodal models. Signals: unified multimodal model, adaptive visual gating, training-free, internal signal, generation intent
...
### [140] [Memory-Conditioned Tool Calling for Camera-First Visual Agents](https://arxiv.org/abs/2607.09822)
*Xiaofan Wu, Xi Zeng, Miaoxia Chen, Peishan Chen, Shuyan Li, Jiyun Yao, Hanyong Zhong, Jiahao Zhu*

Main category: cs.IR, cs.HC, cs.AI, cs.CL, cs.CV
...
TL;DR: 本文提出一种记忆条件化的工具调用框架，用于相机优先的视觉智能体。系统通过三层个人视觉记忆（画像、短期焦点、观察记录）在每轮交互中加载，条件化 LLM 工具调用循环，使智能体在仅接收图像输入时能做出更符合用户偏好的多工具查询。实验表明，完整的三层记忆模块能显著提升工具查询相关性和端到端效用。


<details>
  <summary>Details</summary>
Motivation: 在相机优先（camera-first）的视觉智能体场景中，用户只能发送图像，智能体需要自主决定查询哪些工具。现有方法仅依赖图像识别，缺乏对用户个人偏好和记忆的建模，导致工具选择与用户真实需求脱节。本文旨在探索个人视觉记忆是否能改善智能体的工具选择策略。

Method: 设计了一个三层个人视觉记忆结构：profile（用户画像）、short-term focus（短期焦点）、observations（观察记录）。每轮交互时加载记忆以条件化 LLM 工具调用循环，并包含冲突感知的回写机制以更新用户模型。在 800 张图像上使用合成记忆块进行受控消融实验。

Result: 移除完整三层记忆块后，工具查询相关性下降 0.47 分（5 分制，4.21→3.74，相对下降 11.2%），端到端效用下降 0.082（0.842→0.760，相对下降 9.7%）。

Conclusion: 个人视觉记忆能有效改善相机优先智能体的工具选择策略，使其更符合用户偏好。三层记忆结构的设计是有效的，但当前实验基于固定合成记忆块，尚未验证多轮交互中从真实用户历史进行回写的效果。

Classification reason: 该论文研究基于个人视觉记忆的相机优先智能体工具调用，核心是视觉语言模型（VLM/LLM）在图像输入下进行工具选择与参数生成，属于通用视觉语言模型与多模态智能体方向，映射到 canonical 子主题 vlm_mllm_general。

Importance reason: Focuses on tool-calling for camera-first agents, not on the user's priority subtopics like continuous language models or video generation. Signals: Tool calling, Visual memory, Camera-first agents, MLLM application, Not core to user's priority subtopics
...
### [141] [Generalize LMMs to Versatile Visual Modalities via Fabricated Modality Synthesis](https://arxiv.org/abs/2607.10308)
...
TL;DR: 本文提出 VVM-Tuning 训练框架，通过合成多样外观变化的图像训练 LMM 解耦模态不变语义与模态特定外观，并引入模态上下文提示实现零样本泛化到未见视觉模态；同时构建 VVM-Bench 基准评估该能力。


<details>
  <summary>Details</summary>
Motivation: 现有 LMM 主要针对 RGB 图像设计，难以泛化到深度图、热成像、红外等未见过的视觉模态。作者认为不同视觉模态是同一物理世界的不同采样，模型需要同时具备模态无关的语义感知能力和对模态特定特征的适应性。

Method: 提出 VVM-Tuning 框架：(1) 从 RGB 场景合成多种外观变化的图像（如风格迁移、颜色变换），训练模型解耦不变语义与变化外观；(2) 将视觉外观与语言对齐，学习与模态解耦的视觉概念；(3) 在提示中引入模态上下文（modality contexts），通过指令微调帮助模型将外观变化映射回模态相关属性，实现推理时零样本适应未见模态。同时构建 VVM-Bench 基准，包含 6 种真实与合成模态。

Result: 在 5 种不同 LMM 上的实验表明，仅通过合成模态训练，模型在真实世界和新型合成模态上均表现出一致的性能提升，无需在目标模态上进行训练。

Conclusion: 通过合成模态训练和模态上下文提示，可以有效提升 LMM 对未见视觉模态的零样本泛化能力，为构建更通用的多模态系统提供了可行路径。

Classification reason: 该论文研究如何让大型多模态模型（LMM）泛化到未见过的视觉模态（如深度图、热成像等），属于通用视觉语言模型的能力扩展，与 [CANONICAL] subtopic "视觉语言模型与 MLLM" 高度相关。

Importance reason: Addresses LMM generalization to unseen modalities via synthetic training, aligning with multimodal understanding and unified model priorities. Signals: multimodal generalization, zero-shot adaptation, modality synthesis, VVM-Bench, LMM robustness
...
### [142] [Laguerre Geometry for Interpreting Large Language Models](https://arxiv.org/abs/2607.10578)
...
TL;DR: 本文提出用Laguerre几何精确刻画LLM中的概念几何，将概念定义为Laguerre-Voronoi单元或单元并集，从而严格定义、度量和分离概念。进一步将几何分析推入Transformer内部，提出Geometric Lens——一种无需训练、无需超参数的方法，可读出任意层隐藏向量编码的确切概念。同时开发了Laguerre Autoencoder用于2D可视化。实验表明该方法能在上下文干扰下恢复正确事实token。


<details>
  <summary>Details</summary>
Motivation: 现有假设将LLM中的概念表示为单点、线性方向或高斯簇，但这些结构如何以及为何涌现尚不清楚。缺乏一种精确的几何框架来定义、度量和分离概念，也缺乏能够解释Transformer内部隐藏状态语义含义的方法。

Method: 基于Laguerre几何，将概念定义为Laguerre-Voronoi单元或单元并集，通过Laguerre权重揭示概念的包含与层次结构。将Transformer每层分解为分段线性算子，揭示token隐藏轨迹由两种耦合机制控制：静态的分段线性流树和动态的跨token注意力跳跃。由此提出Geometric Lens方法，无需训练即可读出隐藏向量编码的概念。同时开发Laguerre Autoencoder进行2D可视化。

Result: Geometric Lens能够在上下文干扰（in-context interference）场景下准确恢复正确的事实token。Laguerre Autoencoder能够在一张视图中同时呈现决策几何和模型的完整推理轨迹。实验验证了Laguerre几何框架对概念结构的精确刻画能力。

Conclusion: Laguerre几何为LLM概念表征提供了精确的数学框架，Geometric Lens实现了无需训练的可解释性分析，能够揭示概念层次结构并恢复被干扰的正确信息。该方法从解释性几何走向了可操作的可解释性，为理解Transformer内部机制提供了新工具。

Classification reason: 该论文研究LLM中概念几何的数学表征（Laguerre几何），并提出Geometric Lens方法用于解释隐藏层表征。虽然论文主要聚焦LLM而非多模态，但其提出的可解释性方法（Geometric Lens、Laguerre Autoencoder）可直接应用于多模态/视觉语言模型的可解释性分析，属于MLLM可解释性范畴。最接近的现有子主题是"视觉语言模型与MLLM"（vlm_mllm_general），因为该子主题涵盖MLLM架构与通用方法，可解释性是其重要组成部分。

Importance reason: Novel geometric framework for LLM interpretability, but tangential to user's priority on continuous multimodal models and generation. Signals: Novel Laguerre geometry for concept representation, Training-free interpretability method, No direct connection to continuous tokens or video/image generation, Focus on LLM internal structure, not multimodal generation
...
### [143] [The Path to Self-Evolving Clinical Systems: Scaling Medical Agents from Assistance to Autonomy](https://arxiv.org/abs/2607.11175)
...
TL;DR: 本文是一篇关于医学智能体的综述，从临床部署视角出发，将医学智能体形式化为部分可观测下的序列决策系统，提出三级自主性分类（辅助、协作、完全自主），并构建了包含框架扩展、能力扩展和环境扩展的统一扩展主轴。论文指出临床环境扩展（工具、数据、临床gym的整合）是最可行但尚未充分探索的方向，并将临床自我进化（智能体通过与环境的交互而非仅参数扩展来改进）定位为关键研究前沿。


<details>
  <summary>Details</summary>
Motivation: 现有医学智能体文献多从能力优先视角出发，缺乏从临床部署角度系统审视医学智能体所需的任务、抗污染基准和交互训练环境。本文旨在填补这一空白，为构建可信赖、可自我进化的临床医学影像系统提供路线图。

Method: 本文采用综述方法，整合300余篇参考文献（重点覆盖2025-2026年进展），从临床部署视角出发，将医学智能体形式化为部分可观测马尔可夫决策过程（POMDP），提出三级自主性分类体系，并构建统一扩展主轴（框架扩展、能力扩展、环境扩展），系统梳理了放射学、病理学、眼科和医院工作流中的应用及挑战。

Result: 论文识别出临床环境扩展（PACS、EHR、FHIR生态系统中的工具、数据和临床gym整合）是最可行但尚未充分探索的方向；将临床自我进化定位为关键研究前沿；系统梳理了幻觉、级联故障、公平性等部署挑战。

Conclusion: 医学智能体正从任务特定预测器向自主临床系统演进，临床环境扩展和自我进化是未来关键方向。实现可信赖的自主医学智能体需要解决幻觉、级联故障和公平性等挑战，并建立抗污染基准和交互训练环境。

Classification reason: 该论文是一篇关于医学智能体（medical agents）的综述，核心围绕大语言模型和视觉语言模型在临床环境中的自主决策能力。论文讨论了VLM在医学影像（放射、病理、眼科）中的应用、多模态推理、自主规划等，属于多模态理解与生成方向下的通用视觉语言模型与MLLM范畴，因此映射到[CANONICAL] subtopic "vlm_mllm_general"。

Importance reason: Focuses on medical agents, which is a negative keyword, and lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: Medical agents, clinical deployment, self-evolving systems, negative keyword match
...
### [144] [Compositional Context Fine-Tuning Vision-Language Model for Complex Assembly Action Understanding from Videos](https://arxiv.org/abs/2607.10797)
...
TL;DR: 本文提出 Compositional Context Fine-Tuning (CCFT) 方法，将装配动作分解为 Verb、Object、Tool 三个语义元素，通过模板化问答对微调视觉语言模型，并采用 Layer-Partitioned Alternating Training (LP-AT) 实现多任务高效学习，在装配视频理解任务上优于强基线方法。


<details>
  <summary>Details</summary>
Motivation: 装配动作理解是人机协作装配的关键，但由于动作细微、手-物交互精细，现有方法难以有效建模。作者希望将 VLM 适配到这一挑战性领域，实现细粒度、可解释的动作元素级理解。

Method: 提出 CCFT 方法，将装配动作分解为 Verb、Object、Tool 三个语义元素，为每个元素设计模板化问答对进行微调。同时提出 LP-AT 方法，将模型层分区并分配不同的低秩适配器（LoRA）识别不同动作元素，交替更新权重以减少跨任务干扰。基于 HA-ViD 和 IKEA-ASM 数据集构建了 VQA 格式的装配视频理解数据集。

Result: 在 HA-ViD-VQA 和 IKEA-ASM-VQA 数据集上，CCFT 方法在 Verb、Object、Tool 三个元素的识别准确率上均显著优于强动作识别基线方法，同时提供了可解释的元素级预测结果。

Conclusion: CCFT 结合 LP-AT 能有效将 VLM 适配到复杂装配动作理解任务，在有限数据下实现高效多任务学习，并输出可解释的语义元素级预测，支持下游应用。

Classification reason: 该论文对视觉语言模型（VLM）进行微调以适应复杂装配动作理解任务，属于通用视觉语言模型与多模态指令微调范畴，与 [CANONICAL] subtopic "vlm_mllm_general（视觉语言模型与 MLLM）" 高度匹配。

Importance reason: Focuses on fine-tuning VLMs for niche assembly action understanding, not aligned with user's priority subtopics like continuous models or video generation. Signals: VLM fine-tuning, assembly action, compositional decomposition, LP-AT, HA-ViD-VQA dataset
...
### [145] [LaGuadia: Language-Guided Adaptive Distillation from Pathology Foundation Models](https://arxiv.org/abs/2607.11257)
...
TL;DR: LaGuadia 提出一种语言引导的自适应知识蒸馏框架，利用临床病理报告中的关键词作为语义锚点，动态整合多个病理基础模型（PFM）的知识，训练出仅 87M 参数的紧凑病理图像编码器，在 WSI 描述、VQA 和分类任务上达到或超越 GigaPath、UNI 等大规模基础模型。


<details>
  <summary>Details</summary>
Motivation: 病理基础模型（PFM）虽然能提供强大的全切片图像（WSI）表征，但计算成本极高。现有知识蒸馏方法在多教师场景下采用均匀加权，忽略了组织异质性，导致蒸馏效率低下。作者希望利用临床语言作为语义锚点，实现自适应、高效的病理模型蒸馏。

Method: LaGuadia 采用三阶段流水线：(1) 从病理报告中提取临床关键词；(2) 通过视觉-语言元教师 MedSigLIP 将视觉特征与这些关键词对齐，提供密集语义指导；(3) 基于各教师模型与临床语义的对齐程度动态加权，进行自适应知识蒸馏，训练紧凑的学生模型。

Result: 在 WSI 描述、视觉问答和切片级分类任务上，87M 参数的 LaGuadia 学生模型匹配或超越了 GigaPath、UNI 等大规模基础模型，展现出强事实一致性和鲁棒泛化能力。

Conclusion: 临床语言可以作为有效的语义锚点，指导构建高效可靠的数字病理系统。语言引导的自适应蒸馏框架在保持性能的同时大幅降低计算成本，为病理基础模型的实际部署提供了可行方案。

Classification reason: 该论文提出 LaGuadia 框架，利用视觉-语言模型（MedSigLIP）进行临床语言引导的自适应知识蒸馏，属于视觉语言模型在病理领域的应用，与 canonical 子主题 "视觉语言模型与 MLLM" 高度匹配。

Importance reason: Focuses on pathology foundation model distillation, which is outside your priority subtopics and has negative keyword overlap. Signals: pathology foundation model, knowledge distillation, clinical language guidance, WSI captioning, VQA
...
### [146] [Q-BridgeNet: A Quantization Network for Cross-Lingual Sign Language Translation](https://arxiv.org/abs/2607.11215)
...
TL;DR: Q-BridgeNet 提出一种基于量化网络的统一跨语言手语翻译框架，通过残差向量量化学习语言无关的语义基元（共享码本）和语言特定的残差码本，并在量化空间中微调多语言LLM，实现多语言手语到多语言口语的统一翻译。


<details>
  <summary>Details</summary>
Motivation: 现有手语翻译方法大多针对单一语言对（如美国手语-英语），难以扩展到多语言场景。多语言手语翻译面临跨语言冲突问题：不同手语和口语之间存在语义差异，需要统一模型既能捕捉跨语言共享语义，又能保留语言特定变化。

Method: Q-BridgeNet 包含两个核心设计：1）在手语侧，通过自适应分割和残差向量量化学习离散Q-units，其中共享基码本提供语言无关的语义基元，语言特定残差码本细化异构手语语义；2）在口语侧，微调多语言LLM使其在Q-unit空间中运作，利用跨语言先验实现统一翻译模型。

Result: 在PHOENIX14T（德手语-德语）、How2Sign（美手语-英语）和CSL-Daily（中手语-中文）三个数据集上，Q-BridgeNet 在原生语言对上达到SOTA，并在非原生语言对上展现出强泛化能力，有效缓解了跨语言冲突。

Conclusion: Q-BridgeNet 通过量化网络有效解决了多语言手语翻译中的跨语言冲突问题，为统一多语言手语翻译提供了可行方案，在多个基准上取得了最优性能。

Classification reason: 该论文研究跨语言手语翻译（Sign Language Translation），属于多模态理解任务（视觉+语言）。手语翻译可归入视觉问答与文档理解（multimodal_vqa_doc）这一[CANONICAL]子主题，因为该子主题涵盖VQA及视觉语言理解任务，手语翻译本质上是视觉手势序列到文本的跨模态理解与翻译。

Importance reason: Focuses on sign language translation, which is outside the user's priority subtopics and negative keywords. Signals: Sign language translation, quantization network, cross-lingual, not in priority subtopics, no boost keywords matched
...
### [147] [WasteAssistant: Regulation-Guided Visual Question Answering Framework for Intelligent Waste Segregation and Sustainable Managemen](https://arxiv.org/abs/2607.10610)
...
TL;DR: 本文提出WasteAssistant，一个基于VLM/MLLM的视觉问答框架，用于智能垃圾分类。构建了包含13500个QA对的WasteVQA数据集，遵循印度固体废物管理规则。BLIP模型在BLEU和BERTScore上分别达到0.8291和0.9273，优于传统CNN方法。


<details>
  <summary>Details</summary>
Motivation: 现有垃圾分类系统局限于单模态视觉处理，缺乏上下文理解和法规对齐能力，无法满足可持续城市管理需求。

Method: 提出语言引导的视觉AI框架，集成VLM和MLLM进行联合视觉-语言推理，实现基于VQA的垃圾分类范式，并构建WasteVQA数据集（21类、13500个QA对），对齐印度固体废物管理规则2016。

Result: BLIP模型在WasteVQA上BLEU得分0.8291，BERTScore 0.9273，优于传统CNN方法。

Conclusion: 该工作提升了源头垃圾分类准确率，确保法规合规性，支持面向市政和公民的可扩展部署，推动多模态AI在可持续城市基础设施中的应用。

Classification reason: 该论文提出面向垃圾分类的VQA框架，属于视觉问答（VQA）任务，与[CANONICAL] subtopic "视觉问答与文档理解"（multimodal_vqa_doc）高度匹配。

Importance reason: This paper focuses on a specific waste management VQA application with no alignment to the user's priority subtopics like continuous language models, video generation, or world models. Signals: Niche application, No continuous/autoregressive/diffusion focus, No world model or efficient inference, Negative keywords absent but no boost keywords, Low priority subtopic match
...
### [148] [Learning Anatomy-Grounded CT Vision-Language Representations with Organ-Hierarchical Report Knowledge](https://arxiv.org/abs/2607.10953)
...
TL;DR: 提出OKA-CT框架，利用器官层级知识增强CT图像与放射学报告的视觉语言预训练，通过报告解析和LLM辅助语义结构化提取器官条件知识，分两阶段学习：第一阶段注入解剖学证据到CT视觉表征，第二阶段利用器官特定报告证据指导结构化对比学习。在CT-RATE和RAD-ChestCT数据集上取得零样本异常诊断AUROC 84.9和72.2，优于先前CT VLP基线。


<details>
  <summary>Details</summary>
Motivation: 现有医学VLP方法要么将整个扫描与完整报告对齐，要么将局部图像区域与文本片段对齐，未能充分利用放射学报告的关键特性——报告发现按解剖结构组织，异常描述围绕器官、疾病概念、位置和严重程度属性展开。因此需要一种利用器官层级知识进行CT-报告VLP的方法。

Method: OKA-CT框架包含两个阶段：(1) 首先通过放射学报告解析和LLM辅助语义结构化，将自由文本报告转换为器官条件知识，构建器官层级结构；(2) Stage 1：通过细粒度器官条件监督将解剖学证据注入CT视觉表征；(3) Stage 2：利用器官特定报告证据指导结构化报告-CT对比学习，其中层级导出的语义软目标将共享器官级发现但非配对的样本视为弱语义正样本而非统一负样本；(4) 轻量级基于查询的全局分支聚合疾病相关体积证据用于全扫描表征。

Result: 在CT-RATE和RAD-ChestCT数据集上，零样本异常诊断AUROC分别达到84.9和72.2，优于先前CT VLP基线。检索和patch-occlusion分析进一步表明改进了报告-图像对齐，并对疾病相关解剖区域具有更强的敏感性。

Conclusion: OKA-CT通过利用器官层级知识有效提升了CT-报告视觉语言预训练的性能，验证了解剖学结构化知识在医学VLP中的重要性，为零样本医学异常诊断提供了更优的表征学习方法。

Classification reason: 该论文属于医学视觉语言预训练（CT图像与放射学报告对齐），核心任务是医学视觉问答与异常诊断，与多模态理解与生成方向下的"视觉问答与文档理解"（multimodal_vqa_doc）子主题高度相关，该子主题涵盖VQA及文档/图表理解，医学报告理解是其重要子领域。

Importance reason: Focuses on medical CT VLP, which is outside the user's priority subtopics and has negative keywords. Signals: Medical domain, CT vision-language pretraining, organ-hierarchical knowledge, zero-shot abnormality diagnosis, not aligned with user's continuous multimodal or video generation focus
...
### [149] [MonkeyOCRv2: A Visual-Text Foundation Model for Document AI](https://arxiv.org/abs/2607.11562)
...
Importance: 值得关注 (75.28) - 未入选精读

TL;DR: MonkeyOCRv2是一个面向文档AI的视觉-文本预训练模型，构建了1.13亿张覆盖17种语言的最大文档图像预训练语料库MonkeyDoc v2，并提出联合学习图像到文本生成与像素级文档重建的预训练策略。在文本识别、公式识别、文本检测、文档篡改检测、重叠文本分割五个任务上一致提升性能，作为冻结的视觉编码器配合0.7B轻量语言模型在MDPBench上超越此前3B最佳模型2.8%绝对值。


<details>
  <summary>Details</summary>
Motivation: 主流视觉编码器在自然图像上预训练，缺乏对文档图像中密集文本和精细字符笔画的感知能力，需要面向文档的适配才能有效应用于文档AI任务。

Method: 构建大规模文档图像预训练语料库MonkeyDoc v2（1.13亿张图像，17种语言）；提出联合预训练策略：图像到文本生成（对齐视觉表征与文本内容）+ 像素级文档重建（保留字符笔画和布局细节）。

Result: 在文本识别、公式识别、文本检测、文档篡改检测、重叠文本分割五个任务上一致提升；作为冻结视觉编码器配合0.7B语言模型在MDPBench上达到开源SOTA，超越此前3B最佳模型2.8%绝对值，视觉编码器小约11倍；在8个文档理解基准上优于基于CLIP/DINO/SAM的模型。

Conclusion: 面向文档的视觉预训练可以独立成为文档智能的基础，MonkeyOCRv2作为视觉-文本基础模型在文档AI任务上展现出显著优势。

Classification reason: 该论文聚焦文档AI，提出面向文档图像的视觉预训练模型MonkeyOCRv2，在文档解析、文档理解等任务上取得SOTA，属于多模态理解与生成方向下的视觉问答与文档理解（multimodal_vqa_doc）子主题。

Importance reason: Strong document AI contribution but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: Largest document pretraining corpus, joint generation-reconstruction pretraining, strong OCR/document parsing results, no autoregressive or continuous token focus, no video or world model signals
...
### [150] [Evidence-Backed Video Question Answering](https://arxiv.org/abs/2607.11862)
...
TL;DR: 本文提出 E-VQA（证据支持的视频问答）新任务，要求 Video LLM 同时输出语义答案和精确的时空证据（时间片段+密集跟踪分割掩码）。构建了首个像素级时空证据基准 ST-Evidence，并开发自动化流水线生成 16 万规模的 ST-Evidence-Instruct 数据集。微调后的模型在 7B 规模上相比基线提升 +27.2 t-mean 和 +13.8 J&F。


<details>
  <summary>Details</summary>
Motivation: 当前 Video LLM 在视频问答中表现优异，但本质上是黑盒系统，仅输出文本答案而缺乏可验证的视觉依据。现有的可解释性工作依赖文本理由或稀疏的边界框，难以捕捉遮挡、非刚性变形等复杂视频动态。因此需要一种新的任务范式，要求模型同时提供语义答案和精确的时空视觉证据。

Method: 1) 定义 E-VQA 任务：模型需输出语义答案 + 时间片段 + 密集跟踪分割掩码（masklets）。2) 构建 ST-Evidence 基准：首个经人工验证的像素级时空证据基准，支持判别式和生成式两种评估。3) 开发自动化数据生成流水线，构建 ST-Evidence-Instruct（16 万规模）数据集，将高层推理与细粒度视觉定位桥接。4) 在 grounded Video LLM 上进行微调训练。

Result: 在 ST-Evidence 基准上评估现有 SOTA 模型，发现 QA 准确率与真实视觉感知之间存在显著解耦，仅靠规模扩展无法弥合。使用 ST-Evidence-Instruct 微调后的 grounded Video LLM（7B 模型）相比同尺寸 UniPixel 基线，t-mean 提升 +27.2，J&F 提升 +13.8。

Conclusion: E-VQA 任务为视频理解提供了更可解释、可验证的范式。实验表明当前模型在视觉感知方面存在根本性不足，而所提出的自动化数据生成和微调方法能有效提升模型提供精确时空证据的能力，为可解释的视频理解建立了强基线。

Classification reason: 论文提出 Evidence-Backed Video Question Answering (E-VQA) 任务，要求模型同时输出语义答案和精确的时空证据（时间片段+密集跟踪分割掩码），属于视频问答（Video QA）范畴，与 canonical 子主题 "视觉问答与文档理解"（multimodal_vqa_doc）高度匹配。

Importance reason: Novel task for evidence-backed video QA with dense spatio-temporal grounding, but outside user's core priority subtopics. Signals: Video question answering, spatio-temporal grounding, dense tracking masklets, benchmark creation, Video LLM evaluation
...
### [151] [ReflectWorld-MM: An Entity-Oriented Multi-Media Memory System for Open-Ended Video Streams](https://arxiv.org/abs/2607.09759)
...
TL;DR: ReflectWorld-MM 是一个面向开放视频流的实体导向多模态记忆系统，包含感知前端、层次化长期记忆（多尺度情景记忆、实体语义记忆、程序记忆）和完整实现，在6个长视频与终身记忆基准上取得最佳准确率。


<details>
  <summary>Details</summary>
Motivation: 现有多模态智能体在视频流上的记忆系统要么将记忆保留在模型上下文中，要么存储在扁平特征库中，且以帧而非实体为中心组织记忆，这限制了它们处理开放视频流的能力，难以追踪跨时间重复出现的实体。

Method: 提出 ReflectWorld-MM，包含三部分：(1) 感知前端，将流式视频转化为实体解析的观测，并维护有界短期记忆；(2) 层次化长期记忆，基于人类记忆理论，耦合多尺度情景记忆、演化的实体中心语义记忆和程序记忆；(3) 完整实现，可接入任意视频流并插入现成智能体。

Result: 在6个长视频和终身记忆基准上，ReflectWorld-MM 在所有6个基准上均取得最佳准确率，超越了强记忆智能体和前沿模型。

Conclusion: 实体导向的层次化记忆系统能有效提升多模态智能体在开放视频流中的持续理解与推理能力，为构建长期视频记忆系统提供了新范式。

Classification reason: 该论文提出面向开放视频流的实体导向多模态记忆系统，核心是长视频流中的持续理解、记忆与推理，属于视频理解与时序推理（video_understanding_temporal）这一[CANONICAL]子主题。

Importance reason: Focuses on video stream memory and entity tracking, not on the user's priority subtopics like continuous language models or autoregressive generation. Signals: entity-oriented memory, hierarchical long-term memory, video stream understanding, multimodal agent, long-video benchmarks
...
### [152] [Vinci2: Providing Proactive Assistance in Continuous Egocentric Videos](https://arxiv.org/abs/2607.11523)
...
TL;DR: Vinci2 提出面向连续第一视角视频的主动辅助系统，包含大规模基准 EgoServe（3000+服务实例，4种时间记忆跨度，10个服务类别）和训练无关的记忆增强智能体 EgoMemo（多尺度时序摘要、语义知识图谱、视觉嵌入存档），通过检索增强推理判断何时提供主动辅助。


<details>
  <summary>Details</summary>
Motivation: 现有智能辅助系统要么被动等待用户查询，要么对每个检测到的事件都做出响应，没有考虑用户的历史、当前活动或辅助是否受欢迎。连续第一视角视频提供了丰富的动态上下文，使得主动辅助成为可能，但缺乏相应的基准和方法。

Method: 提出 EgoMemo，一种无需训练的记忆增强智能体，维护三种互补记忆表示：多尺度时序摘要、语义知识图谱和视觉嵌入存档。在每个时间步，通过检索增强推理判断是否需要辅助，并生成上下文相关的响应。同时构建 EgoServe 基准，包含3000+服务实例，覆盖4种时间记忆跨度（从即时安全警报到长期习惯指导）和10个服务类别。

Result: EgoMemo 在 EgoServe 上建立了强基线，同时在现有第一视角基准上保持竞争力。实验验证了三种记忆表示的有效性以及检索增强推理在主动辅助决策中的作用。

Conclusion: Vinci2 将主动辅助重新定义为上下文相关的决策问题，通过记忆增强推理实现了从被动响应到主动辅助的转变。EgoServe 基准和 EgoMemo 方法为第一视角视频中的主动辅助研究提供了基础。

Classification reason: 论文关注连续第一视角（egocentric）视频中的主动辅助系统，涉及长视频时序理解、记忆增强推理和视频问答，属于视频理解与时序推理（video_understanding_temporal）这一规范主题。

Importance reason: Proactive egocentric assistance is a niche application with limited direct relevance to the user's priority subtopics on continuous multimodal models and generative world models. Signals: Proactive assistance, Egocentric video, Memory-augmented agent, EgoServe benchmark, Training-free retrieval
...
### [153] [StoryTeller: Training-Free Narrative Grounding for Long-Form Audio Description](https://arxiv.org/abs/2607.11798)
...
TL;DR: StoryTeller 是一个无需训练的框架，通过维护经过验证的叙事记忆，为长视频生成故事感知的音频描述，并引入 StoryAD-QA 基准来评估描述中的叙事信息保留能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型（VLM）在短视频片段上表现良好，但处理长视频时往往独立描述每个时刻，忽略了角色、事件、关系及故事上下文，导致生成的音频描述缺乏叙事连贯性，无法帮助盲人和低视力观众理解电影情节。

Method: StoryTeller 维护一个经过验证的叙事记忆模块，跨场景传递故事相关信息。它仅需原始视频和电影标题，可选检索公开电影元数据来解析名称和故事上下文，并通过语义过滤和 VLM 验证只接受视频支持的事实。无需字幕、脚本、对齐字幕、角色库、预计算人脸身份或任务特定微调。

Result: 在标准 AD 基准和多种长视频上的实验表明，StoryTeller 在自动评估、QA 评估和人工评估中，在叙事连贯性、事实基础性和故事理解方面持续优于强基线方法。

Conclusion: StoryTeller 证明了无需训练即可通过叙事记忆机制显著提升长视频音频描述的故事连贯性和事实准确性，为长视频叙事理解提供了一种高效、实用的解决方案。

Classification reason: 该论文关注长视频音频描述（long-form audio description），涉及跨场景的叙事连贯性、角色与事件跟踪，属于长视频理解与叙事理解范畴，与现有 canonical subtopic "视频理解与时序推理"（video_understanding_temporal）高度匹配。

Importance reason: Focuses on narrative grounding for audio description, which is not aligned with user's priority on continuous multimodal models or video generation. Signals: training-free framework, long-form video understanding, narrative memory, audio description, story-aware QA benchmark
