...
- [世界模型](#direction-world_model) [Total: 36]
  - [具身世界模型与规划控制](#direction-world_model-embodied_world_model) [Total: 26]
...
- [多模态生成](#direction-multimodal_generation) [Total: 36]
  - [3D 生成与重建](#direction-multimodal_generation-three_d_generation) [Total: 7]
...
- [多模态理解与生成](#direction-multimodal_understanding_generation) [Total: 67]
  ...
  - [多模态强化学习与对齐](#direction-multimodal_understanding_generation-multimodal_rl_alignment) [Total: 8]
  ...
  - [多模态评测与基准](#direction-multimodal_understanding_generation-multimodal_benchmark) [Total: 26]
  - [多模态高效化与轻量化](#direction-multimodal_understanding_generation-efficient_multimodal) [Total: 4]
  - [未分类](#direction-multimodal_understanding_generation-uncategorized) [Total: 1]
...
### [1] [Spectral-Progressive Thought Flow for Lightweight Multimodal Reasoning](https://arxiv.org/abs/2606.02842)
...
TL;DR: SpecFlow提出了一种频谱渐进思维流框架，将多模态空间推理中的中间视觉思维表示为固定大小的离散余弦空间（DCT）表示，通过flow matching更新视觉工作区，结合无分类器引导实现文本思维引导视觉状态演化，在保持推理性能的同时将计算和KV缓存开销降低2.1倍。


<details>
  <summary>Details</summary>
Motivation: 多模态空间推理通常依赖长链的中间文本和视觉思维，但累积的视觉token和密集的跨模态注意力导致巨大的计算和内存开销。现有方法随着推理深度增加，视觉工作区不断膨胀，限制了长程推理的可扩展性。

Method: SpecFlow将中间视觉思维表示为固定大小的离散余弦变换（DCT）空间表示，利用DCT的强能量压缩特性保留全局布局和关系结构，仅在需要更高空间精度时引入高频细节。通过无分类器引导（classifier-free guidance）使自回归文本思维引导基于flow的视觉工作区更新，视觉工作区大小固定，更新仅依赖当前视觉状态和累积的文本轨迹。

Result: SpecFlow在多个多模态空间推理基准上取得了有竞争力或更优的推理性能，同时将计算和KV缓存成本降低高达2.1倍，且推理延迟和内存使用与推理深度无关，实现了稳定的长程推理。

Conclusion: SpecFlow通过频谱渐进思维流和固定大小的连续视觉表示，有效解决了多模态空间推理中视觉token膨胀和计算开销大的问题，为轻量级、可扩展的多模态推理提供了新范式。

Classification reason: 该论文提出在离散余弦空间中表示中间视觉思维（visual thought），通过flow-based更新视觉工作区，实现轻量级多模态空间推理。这属于在连续潜空间中进行多模态推理，与[CANONICAL] subtopic "连续多模态推理与潜空间思维"高度吻合。

Importance reason: Directly advances continuous multimodal reasoning with a novel spectral-progressive thought flow, aligning perfectly with your top-weighted priority subtopic. Signals: continuous multimodal reasoning, flow matching, DCT representation, lightweight inference, KV cache reduction
...
### [2] [Wavelet as Tokenizer: Preliminary Results on a Shared Wavelet Token Schema for Natural Signals](https://arxiv.org/abs/2606.02631)
*Shenghao Ding*

Main category: eess.AS, cs.CV, cs.LG, cs.SD, cs.AI

Direction: 连续语言模型与连续多模态模型

Subtopic: 连续视觉 Tokenizer 与表征自编码器

Importance: 值得关注 (74.43) - 未入选精读

TL;DR: 本文探索音频、图像、视频能否共享统一的小波 token 模式，提出基于 Haar DWT/IDWT 的连续 token 模型，在多个数据集上取得有竞争力的重建质量，并验证了稀疏 token 训练的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有不同模态（音频、图像、视频）通常使用各自独立的潜空间网格（如 VAE、ViT），缺乏统一的 token 表示模式。本文希望探索是否可以通过小波变换构建一个跨模态共享的连续 token 模式，为多模态统一建模提供基础。

Method: 提出基于一级 Haar 离散小波变换/逆变换（DWT/IDWT）的前端，将信号转换为共享的系数 token 布局；引入可选的元数据嵌入和轻量级模态值适配器；使用共享的 token 级编码器-解码器主干进行重建。在 Speech Commands（音频）、EuroSAT RGB（图像）和 DAVIS 2017（视频）上评估。

Result: 密集共享模型在音频、图像、视频上分别达到 39.92 dB、29.37 dB、23.93 dB PSNR。固定率能量选择（energy_global）相比均匀选择在压缩比下平均 PSNR 提升 16.73 dB（音频）、16.90 dB（图像）、15.86 dB（视频）。掩码稀疏训练在 50% token 下达到 34.45 dB 视频 PSNR。

Conclusion: 实验结果支持统一小波 token 模式和稀疏 token 接口的可行性，但尚未建立通用的离散词汇表。该工作为跨模态统一 token 表示提供了初步但有希望的方向。

Classification reason: 该论文提出基于小波变换（Haar DWT/IDWT）的连续 token 方案，统一处理音频、图像、视频信号，属于连续视觉/多模态 tokenizer 范畴，与 [CANONICAL] 连续视觉 Tokenizer 与表征自编码器 高度匹配。

Importance reason: Directly advances the user's top-priority continuous multimodal tokenizer direction with a novel wavelet-based unified schema for audio, image, and video. Signals: Unified wavelet token schema for audio/image/video, Continuous token model with Haar DWT/IDWT, Sparse token training achieves 34.45 dB video PSNR at 50% tokens, Strong non-parametric baseline via energy selection, Matches user's priority on continuous visual token and representation autoencoder
...
### [3] [Knowledge Editing in Masked Diffusion Language Models](https://arxiv.org/abs/2606.03924)
...
TL;DR: 本文将 locate-then-edit 知识编辑方法从自回归模型迁移到掩码扩散语言模型（MDM），发现编辑位置跨范式一致（早期到中间层 MLP），但多 token 编辑在 MDM 中性能显著下降，原因是编辑未针对部分掩码的中间状态进行优化，并提出简单修正方法恢复多 token 编辑性能。


<details>
  <summary>Details</summary>
Motivation: 知识编辑方法（如 locate-then-edit）已在自回归语言模型上广泛研究，但掩码扩散语言模型（MDM）采用双向建模和迭代去噪生成，与自回归的 next-token 预测范式完全不同。现有方法能否迁移到 MDM、其底层假设是否成立，是尚未探索的问题。

Method: 将 locate-then-edit 方法迁移到 MDM（LLaDA、Dream），与 ARM（LLaMA、Qwen）进行对比。首先通过因果追踪定位事实存储位置，然后在定位位置进行权重编辑。针对多 token 编辑失败问题，提出在编辑过程中对部分掩码的中间去噪状态进行优化（校正），使编辑适应 MDM 的迭代生成过程。

Result: 因果追踪显示 MDM 和 ARM 的事实存储位置一致（最后 subject token 的早期到中间层 MLP）。单 token 编辑在两者中均成功，但多 token 编辑在 MDM 中系统性退化。提出的中间状态优化校正方法显著恢复了 MDM 的多 token 编辑性能。

Conclusion: Locate-then-edit 方法可以迁移到 MDM，但多 token 编辑面临独特挑战——编辑未针对部分掩码的中间状态优化。通过针对这些状态进行校正，可以有效恢复性能。这为扩散语言模型的知识编辑提供了基础理解和方法。

Classification reason: 该论文研究掩码扩散语言模型（MDM）中的知识编辑，MDM 属于连续语言模型范畴（LLaDA、Dream 等），因此映射到 canonical 子主题"连续语言模型与语言流"。

Importance reason: Directly extends locate-then-edit knowledge editing to masked diffusion language models, a core continuous language model technique, with clear diagnosis and fix for multi-token editing failure. Signals: First transfer of locate-then-edit to MDMs, reveals editing location consistency across paradigms, identifies and fixes multi-token editing degradation, uses LLaDA and Dream models, aligns with continuous language model priority
...
### [4] [Too Much of a Good Thing: When sim2real Efforts Impede Policy Learning (And What to Do About It)](https://arxiv.org/abs/2606.02636)
...
TL;DR: 本文指出 sim2real 迁移中的过度约束会损害策略学习，导致模拟器锁定和探索不足，并提出基于机器人运动学约束的 sim2sim2real 范式作为解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前 sim2real 方法在迁移策略到真实硬件时施加了过多真实世界约束，反而限制了策略在模拟器中的探索空间，导致策略学习效果不佳。作者希望诊断这一问题并提出改进方案。

Method: 作者首先分析了 sim2real 过度约束导致模拟器锁定和策略探索不足的机制，然后提出 sim2sim2real 范式：仅以机器人运动学作为唯一设计约束，在模拟器之间先进行迁移（sim2sim），再迁移到真实世界（sim2real），从而保留策略学习的灵活性。

Result: 论文提供了对当前 sim2real 问题的诊断和解释，并提出了 sim2sim2real 作为潜在解决方案，但未提供实验验证结果。

Conclusion: 过度 sim2real 努力可能适得其反，sim2sim2real 范式通过放松约束、保留策略探索空间，有望改善策略学习效果。

Classification reason: 该论文讨论 sim2real 迁移对机器人策略学习的影响，并提出 sim2sim2real 范式，属于机器人具身智能与策略学习范畴，与 [CANONICAL] 具身世界模型与规划控制 高度相关。

Importance reason: Addresses sim2real over-constraint but lacks direct connection to user's priority subtopics like continuous multimodal models or video generation. Signals: sim2real policy learning, simulator lock-in, robotics kinematics, world model critique, sim2sim2real paradigm
...
### [5] [See, Infer, Intervene: Proactive World Modeling for Goal-Oriented Social Intelligence](https://arxiv.org/abs/2606.03371)
...
TL;DR: 本文提出 See-Infer-Intervene (SII) 框架和 Proactive Intent World Model (PIWM)，用于智能零售场景中主动推断客户意图并选择服务动作。PIWM 使用 AIDA 购买阶段和 BDI 心理场表示客户状态，预测动作条件下的意图转移，并选择五种响应类别。在 GuidanceSalesBench 基准上，基于真实状态的条件选择达到 0.641 macro F1，但端到端视频输入下仅 0.295，表明视频到状态的对齐是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 多模态零售智能体不仅需要识别客户行为，还需要在客户明确请求之前主动判断是否以及如何提供帮助。现有工作缺乏对客户潜在意图的建模和主动服务决策的能力。

Method: 提出 SII 框架，包含 See（观察交互前行为）、Infer（推断潜在客户意图）、Intervene（选择服务动作）三个阶段。PIWM 使用 AIDA（注意、兴趣、欲望、行动）购买阶段和 BDI（信念、欲望、意图）心理场表示客户状态，建模动作条件下的意图转移，从五种响应类（Greet, Elicit, Inform, Recommend, Hold）中选择。构建了 GuidanceSalesBench 零售基准数据集。

Result: 在真实状态条件下，PIWM 在 30 个目标视频上达到 0.641 macro F1，优于零样本 Qwen2.5-VL-7B 基线；端到端视频输入下降至 0.295，低于随机基线 0.414；真实商店试点实验在 20 个视频上达到 0.579 macro F1。

Conclusion: PIWM 在意图推断和服务动作选择上展现了潜力，但视频到状态的对齐是当前部署的主要瓶颈，未来需要改进视觉状态推断能力。

Classification reason: 该论文提出 Proactive Intent World Model (PIWM)，通过动作条件意图状态转移建模来预测客户行为并选择服务干预动作，属于具身/交互场景中的世界模型用于规划与控制，因此映射到 [CANONICAL] 具身世界模型与规划控制。

Importance reason: Proactive world modeling for social intent is novel but the retail domain and low video-to-state performance limit direct relevance to your priority subtopics. Signals: world model, proactive intent inference, retail domain, low video grounding performance, no continuous representation or autoregressive generation
...
### [6] [See Less, Specify More: Visual Evidence Budgets for Generalizable VLAs](https://arxiv.org/abs/2606.02735)
...
TL;DR: 本文提出 S2 (See Less, Specify More) 框架，通过为 VLA 模型提供细粒度的子任务级语言标注和显式的视觉证据预算约束，使执行器从任务充分的视觉证据中学习而非依赖无约束的视觉上下文，在 8 个真实机器人任务上将平均子任务成功率从 54.2% 提升至 79.0%。


<details>
  <summary>Details</summary>
Motivation: VLA 模型在干扰物、外观变化和语义相似任务下的泛化能力不足，主要原因是粗粒度指令导致监督混淆，且模型需要自行从无约束的视觉上下文中推断执行细节。

Method: S2 框架包含两个核心设计：(1) Specify More——保留原始指令作为稳定高层目标，同时为每条轨迹重新标注细粒度的轨迹级和子任务级语言描述以消除执行歧义；(2) See Less——引入显式的视觉证据预算，训练执行器仅从任务充分的视觉证据中行动，无需区域或掩码标注，兼容现成的 VLM 规划器。

Result: 在 TX-G2 和 HSR 两个机器人平台上的 8 个真实任务中，S2 将平均子任务成功率从 pi0.5 基线的 54.2% 提升至 79.0%，消融实验验证了目标保持的局部指导优于指令替换，显式证据预算减少了模型对广泛视觉上下文的依赖。

Conclusion: VLA 模型的泛化能力可以通过训练执行器从信息丰富的局部指导和任务充分的视觉证据中学习来显著提升，而非从弱监督中同时恢复两者。

Classification reason: 该论文提出 S2 框架，通过视觉证据预算和细粒度语言标注改进 VLA（视觉-语言-动作）模型的泛化能力，核心是机器人具身策略学习与执行控制，属于具身世界模型与规划控制方向。

Importance reason: Proposes a novel visual evidence budget constraint for VLA generalization, but focuses on robotics rather than the user's priority subtopics. Signals: VLA generalization, visual evidence budget, subtask-level language, real-robot tasks, pi0 baseline improvement
...
### [7] [SeeTraceAct: Visibility-Aware Latent Planning from Cross-Embodiment Demonstration Videos](https://arxiv.org/abs/2606.02745)
...
TL;DR: SeeTraceAct 提出一种基于单次演示视频的视觉-语言-动作模型（VLA）框架，通过可见性感知的未来末端执行器轨迹预测来增强空间定位能力，在跨具身演示（人→机器人）场景下显著提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的 VLA 模型通常需要大量任务特定的遥操作数据进行适配，而单次演示条件 VLA 在需要精确定位小目标区域的任务中表现不佳。作者希望利用人类演示视频（跨具身）来引导机器人执行未见任务，同时克服现有端到端方法在空间精确定位上的不足。

Method: SeeTraceAct 框架包含三个关键组件：(1) 可见性感知的未来末端执行器轨迹预测——模型预测机器人末端执行器在图像空间中的未来轨迹，并利用可见性掩码（visibility mask）区分可见与不可见的轨迹点；(2) 演示条件 VLA 架构，以单段人类演示视频为条件，结合视觉-语言-动作建模；(3) 引入 RoboCasa-DC 数据集，扩展 RoboCasa 以提供跨具身（人形 vs 机器人）的成对演示视频，支持可复现的评测。

Result: 在 RoboCasa-DC 的四个设置中，SeeTraceAct 均取得最佳成功率；在真实世界 Franka Panda 机械臂以人类演示为条件的实验中，平均成功率提升 12.5 个百分点，显著优于基线方法。

Conclusion: SeeTraceAct 通过可见性感知的轨迹预测有效提升了演示条件 VLA 在需要精确定位任务中的表现，跨具身演示（人类→机器人）的范式具有实用价值，为减少任务特定遥操作数据需求提供了可行方案。

Classification reason: 该论文提出 SeeTraceAct，一种基于演示视频的机器人策略框架，通过可见性感知的未来末端执行器轨迹预测实现精确空间定位，属于具身智能中的机器人策略学习与规划控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on robot planning and cross-embodiment, not on the user's priority subtopics like continuous language models or video generation. Signals: world model, robot policy, cross-embodiment, demonstration-conditioned, spatial grounding
...
### [8] [TTT-VLA: Test-Time Latent Prompt Optimization for Vision-Language-Action Models](https://arxiv.org/abs/2606.03127)
...
TL;DR: TTT-VLA提出了一种基于潜空间提示优化的测试时训练框架，通过在训练时学习潜空间提示作为额外条件信号，在测试时仅优化该提示（不修改策略网络）来适应环境分布偏移，在SimplerEnv上显著提升了单/多具身设置下的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型在大规模数据上训练后，部署时仍面临分布偏移问题。虽然提示（prompt）可以作为高效的策略行为调控接口，但现有方法依赖外部引导。作者希望实现一种无需外部引导、仅通过交互数据自监督信号即可在测试时自适应优化提示的框架。

Method: 提出Latent Prompt Optimization (LPO)框架：训练阶段，在策略学习之外引入一个代理任务（proxy task）来学习潜空间提示，提供额外的条件信号；测试阶段，收集当前环境的交互数据，仅优化潜空间提示（不修改策略参数），利用代理任务的自监督信号完成测试时适应。

Result: 在SimplerEnv上，TTT-VLA在单具身和多具身设置下均一致提升了任务成功率。进一步分析表明，性能提升主要来源于纠正少量关键决策，而非全局改变策略行为。

Conclusion: LPO为部署时改进基础操作策略提供了一种有效且实用的途径，通过测试时优化潜空间提示而非修改策略本身，实现了轻量级的分布偏移适应。

Classification reason: 该论文提出TTT-VLA框架，通过测试时潜空间提示优化来适应分布偏移，提升VLA模型在机器人操作任务中的表现。这属于具身智能中策略学习与规划控制的范畴，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Proposes a novel test-time prompt optimization method for VLA models, but is in robotics (cs.RO) and not directly aligned with your priority subtopics. Signals: Test-time training, latent prompt optimization, vision-language-action models, distribution shift adaptation, robotics manipulation
...
### [9] [ConTrack: Constrained Hand Motion Tracking with Adaptive Trade-off Control](https://arxiv.org/abs/2606.03177)
...
TL;DR: ConTrack 是一个基于强化学习的灵巧手运动跟踪框架，通过将物体跟踪视为约束条件并自适应分配剩余控制权给运动保真度，结合自适应中途轨迹重置库，实现了对长时序、高接触密度演示序列的高效跟踪，在仿真和真实机器人上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 人类演示为机器人操作提供了强先验，但由于运动学差异，将演示迁移到真实机器人上非常困难。现有方法依赖手工设计的奖励函数，需要针对每个序列单独调参，且在有限的交互预算下容易失效。因此需要一种可扩展的、无需繁琐调参的跟踪框架。

Method: ConTrack 将物体跟踪建模为约束条件，将剩余控制权分配给运动保真度（joint motion 和 contact timing），通过双变量更新（dual-variable update）在线自适应调整任务-风格权衡。此外，引入自适应中途轨迹重置库（adaptive mid-trajectory reset library），重用策略可达的仿真状态来稳定长时序学习。

Result: 在仿真跟踪和真实机器人实验中，ConTrack 在成功率和物体位姿精度上显著优于先前方法，同时保持了关节运动和接触时序的保真度。

Conclusion: ConTrack 提供了一种可扩展的灵巧手运动跟踪框架，通过约束优化和自适应重置机制有效解决了长时序、高接触密度跟踪的难题，为人类演示到机器人操作的迁移提供了更可靠的方案。

Classification reason: 该论文提出ConTrack框架，通过强化学习实现灵巧手操作中的运动跟踪，涉及机器人动作规划与控制，属于具身世界模型与规划控制方向。

Importance reason: Focuses on robotic hand tracking, not aligned with user's priority on continuous multimodal models or video generation. Signals: robotics, reinforcement learning, dexterous manipulation, not in priority subtopics, no boost keywords matched
...
### [10] [BotDirector: Robot Storytelling Across the Symmetrical Reality with Multi-modal Interactions](https://arxiv.org/abs/2606.03223)
...
TL;DR: 提出BotDirector系统，通过LLM代理将儿童的日常物品和自然语言交互转化为机器人戏剧叙事，实现自主导航的群体机器人执行故事动作序列。


<details>
  <summary>Details</summary>
Motivation: 机器人讲故事对儿童具有吸引力，但技术复杂性过高，儿童难以参与。现有系统缺乏灵活的场景定制和直观的交互方式。

Method: 构建一个交互式系统，儿童通过日常物品布置场景并使用自然语言与LLM代理交互创建叙事；叙事被转化为基于地图和角色的动作序列，由自导航群体机器人执行。

Result: 系统支持灵活的场景定制，使幼儿能够使用日常物品创作机器人戏剧，实现了具身叙事与多模态交互的融合。

Conclusion: BotDirector通过结合LLM叙事生成、机器人自主导航和实物交互，降低了儿童参与机器人讲故事的门槛，为具身智能在教育场景中的应用提供了新思路。

Classification reason: 该论文提出一个交互式机器人讲故事系统，涉及具身智能、机器人自主导航、动作规划与多模态交互，与[具身世界模型与规划控制]这一规范子主题高度匹配。

Importance reason: Focuses on robot storytelling for children, not on core priority subtopics like continuous language models or video generation. Signals: Robotics, LLM agent, swarm robots, tangible interaction, children
...
### [11] [GeoAlign: Beyond Semantics with State-Guided Spatial Alignment in VLA Models](https://arxiv.org/abs/2606.03240)
...
TL;DR: GeoAlign 提出一种状态引导的空间对齐架构，通过 RGB-D 监督后训练的几何增强特征（GEP）和机器人本体状态查询机制，使 VLA 模型具备几何感知的空间对齐与动态 affordance 选择能力，在 LIBERO、SimplerEnv 和真实 ALOHA 任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有 VLA 模型主要优化语义理解，但在可执行操作中缺乏几何感知的空间对齐和动态 affordance 选择能力，导致在需要精确空间推理的操作任务中表现不佳。

Method: GeoAlign 包含三个关键组件：(1) RGB 几何分支，通过机器人域 RGB-D 监督进行后训练，生成 RGB 导出的几何增强后训练（GEP）特征；(2) 本体状态引导的查询机制，用机器人 proprioceptive state 查询 GEP 特征网格，生成紧凑的、阶段相关的几何 token；(3) 基于这些几何 token 进行动作预测。

Result: 在 LIBERO 上达到 99.0%，在 SimplerEnv-Fractal 三个任务上平均 85.3%，在 8 个几何关键的真实世界 ALOHA 任务上达到 78.8%。消融实验验证了几何后训练和本体状态引导查询的有效性。

Conclusion: GeoAlign 通过几何后训练和状态引导的空间对齐，有效提升了 VLA 模型在几何关键操作任务中的表现，为具身智能中的空间感知策略学习提供了有效方案。

Classification reason: 该论文提出 GeoAlign，一种面向机器人操作策略学习的视觉-语言-动作（VLA）模型，通过状态引导的空间对齐实现几何感知的抓取与操作。这属于具身智能中的策略学习与规划控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong robotics VLA paper but outside user's priority subtopics and negative keywords apply. Signals: VLA model, spatial alignment, RGB-D supervision, LIBERO benchmark, real-world ALOHA
...
### [12] [AirDreamer: Generalist Drone Navigation with World Models](https://arxiv.org/abs/2606.03252)
...
TL;DR: AirDreamer 提出一种基于世界模型的无人机通用导航框架，结合强化学习策略与世界模型环境理解，在未见过的杂乱环境中实现鲁棒导航，无需人工设计的感知管道和规则，并成功实现 sim-to-real 迁移。


<details>
  <summary>Details</summary>
Motivation: 现有无人机导航方法严重依赖人工设计的感知管道和预定义规则，对环境配置敏感，泛化能力差。受动物导航行为启发，作者希望设计一种不依赖手工规则、能泛化到未见环境的通用导航框架。

Method: 设计了一个基于世界模型的导航框架，上层使用强化学习策略，底层利用世界模型进行环境理解。设计了无手工 shaping 项的稀疏奖励函数，避免局部最优陷阱并鼓励偏航控制行为。在仿真和真实无人机上进行验证。

Result: 在挑战性地图中，导航成功率比最佳基线高 5.3%；在未见过的复杂环境中展现出涌现能力，能够逃离其他方法失败的局部最优；实现了无需调参的有效 sim-to-real 迁移。

Conclusion: AirDreamer 证明了世界模型结合强化学习策略的无人机导航框架在泛化性和鲁棒性上优于传统方法，且能够有效从仿真迁移到真实环境，为通用机器人导航提供了新思路。

Classification reason: 该论文提出基于世界模型的无人机导航框架，使用强化学习策略结合世界模型进行环境理解与规划控制，属于机器人/具身智能领域的规划控制方法，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: While solid robotics work, it lacks direct connection to user's priority subtopics like continuous language models or autoregressive generation. Signals: world model for drone navigation, reinforcement learning, sim-to-real transfer, not aligned with user's core interests, no continuous token or video generation focus
...
### [13] [Bridging Predictive Uncertainty and Safe Action: Sample-Conditioned Differentiable Planning for Autonomous Driving](https://arxiv.org/abs/2606.03296)
...
TL;DR: 本文提出一种样本条件可微规划框架，利用条件扩散模型生成多样化的未来交通场景样本，并通过经验条件风险价值（CVaR）尾部风险约束将这些样本直接融入可微规划器，从而弥合预测不确定性与安全运动规划之间的鸿沟。在Waymo Open Motion和Argoverse 2数据集上的开环和闭环评估表明，该方法在安全性、效率和乘坐舒适性方面显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统中，高度表达性的不确定性建模与可解释的安全运动规划之间存在脱节。现有方法要么将预测压缩为单一确定性未来，要么依赖黑盒端到端架构，难以处理复杂动态交互环境中的不确定性。

Method: 提出样本条件可微规划框架：(1) 使用条件扩散模型生成一组多样化的未来场景样本；(2) 将这些样本直接输入可微规划器；(3) 通过经验条件风险价值（CVaR）尾部风险约束显式缓解预测不确定性；(4) 引入有向图表示场景上下文，提升预测效果和计算效率。

Result: 在Waymo Open Motion和Argoverse 2数据集上的开环和闭环评估中，该方法在安全性、效率和乘坐舒适性方面显著优于现有最先进基线。

Conclusion: 通过将扩散生成的多样化未来轨迹显式融入可微优化过程，并引入CVaR尾部风险约束，该框架有效弥合了预测不确定性与安全运动规划之间的鸿沟，为自动驾驶中的鲁棒规划提供了新思路。

Classification reason: 该论文提出基于条件扩散模型生成多样化未来轨迹，并通过可微规划器结合CVaR尾部风险约束进行鲁棒运动规划，属于面向自动驾驶的具身世界模型与规划控制方向，与[具身世界模型与规划控制]规范子主题高度匹配。

Importance reason: Strong in world model and planning but lacks direct alignment with priority subtopics like continuous language models or autoregressive generation. Signals: Conditional diffusion for trajectory generation, Differentiable planning with CVaR risk constraint, Waymo/Argoverse 2 validation, Directed graph scene representation, Safety-efficiency-ride comfort improvements
...
### [14] [SPADE: Sketch-guided Path Planning Augmented with Diffusion Experts](https://arxiv.org/abs/2606.03512)
...
TL;DR: 本文提出SPADE框架，通过扩散模型增强的模仿学习实现自主移动机器人的路径规划，在保持实时性和轻量级的同时，显著提升了泛化能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统路径规划方法依赖复杂的奖励工程或硬件方案，而基于模仿学习的方法在未见环境中的泛化能力有限且演示收集的鲁棒性不足。本文旨在解决这些局限性。

Method: 提出一个增强框架，包含两个主要贡献：(1) 基于ROS 2重构的标注工具；(2) 将扩散模型增强集成到基线行为克隆模型中的新型训练策略。提供专家演示数据集，并通过消融实验评估鲁棒性。

Result: 相比SOTA方法，APE降低39.1%，FID降低33.5%，同时可训练参数减少93.8%。在保持扩散级泛化能力的同时，保留了实时性和边缘部署特性。

Conclusion: SPADE框架成功将扩散模型的泛化能力与轻量级行为克隆模型的实时性相结合，为机器人路径规划中融入人类偏好提供了一种高效且鲁棒的解决方案。

Classification reason: 该论文研究自主移动机器人的路径规划，属于机器人动作规划与控制范畴，与[具身世界模型与规划控制]这一规范子主题高度匹配。

Importance reason: Focuses on robotics path planning with diffusion augmentation, not aligned with user's priority on continuous multimodal models or video generation. Signals: robotics path planning, diffusion augmentation, imitation learning, ROS 2, low parameter count
...
### [15] [Bionic Human-Motion Style Transfer for Physically Executable Whole-Body Control of Humanoid Robots](https://arxiv.org/abs/2606.03536)
...
TL;DR: 本文提出一种仿生的人体运动风格迁移框架，将短时人类风格示例迁移到人形机器人全身运动控制中。采用物理感知的多条件潜扩散模型融合风格、内容和轨迹条件，结合接触一致性和时间平滑正则化生成可执行参考运动，并通过预览式全身跟踪策略实现真实机器人上的稳定执行。在Unitree G1上实现96%的成功率。


<details>
  <summary>Details</summary>
Motivation: 人形机器人需要表达丰富的全身运动以在人类环境中自然交互，但现有方法依赖固定演示或手动脚本，难以将演示风格复用到不同运动内容上。受人类通过步态节奏、姿态、摆臂和身体摆动传达情感与意图的启发，本文旨在实现从短时人类风格示例到机器人运动内容的可复用风格迁移。

Method: 提出一个"生成到控制"两阶段框架：1) 物理感知多条件潜扩散模型，融合风格、内容和轨迹条件，使用无分类器引导调整风格强度，并引入接触一致性和时间平滑正则化；2) 将生成参考转换为G1兼容格式，通过基于预览的全身跟踪策略（采用聚类-蒸馏训练策略）执行。

Result: 在仿真和Unitree G1真实机器人上验证，该方法能成功将短时人类风格示例迁移到多种机器人运动内容上，相比动画导向的风格迁移基线减少了接触和抖动伪影，在125次真实机器人试验中达到96.0%的成功率。

Conclusion: 本文证明了使用短时人类运动示例作为可复用的仿生源，生成物理可执行的人形机器人表达性运动是可行的，为人形机器人的自然交互与行为表达提供了有效途径。

Classification reason: 该论文研究人形机器人的全身运动控制与风格迁移，涉及物理可执行的动作生成与跟踪策略，属于具身智能与机器人控制范畴，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: While technically solid, this humanoid motion style transfer paper is peripheral to your core focus on continuous multimodal models and autoregressive video/image generation. Signals: world model application, humanoid control, diffusion model for motion, not continuous token/flow matching, no video generation
...
### [16] [Partially Observable Adversarial Patch Attacks on Vision-Language-Action Models in Robotics](https://arxiv.org/abs/2606.03556)
...
TL;DR: 本文提出了一种部分可观测的对抗性补丁攻击方法，针对VLA机器人模型。攻击者仅需观察轨迹的前缀即可生成固定补丁，通过注意力图定位关键区域并优化补丁以破坏语义接地和动作轨迹曲率，在仿真和真实环境中显著降低任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有VLA对抗攻击假设攻击者可访问完整执行轨迹，这在现实中不切实际。本文旨在研究更真实的部分可观测场景下，如何利用有限信息（轨迹前缀）生成有效的对抗补丁，揭示VLA模型在感知和控制层面的安全漏洞。

Method: 提出两阶段框架：(1) 补丁定位阶段，利用模型注意力图识别与完整指令对应的视觉关键区域；(2) 补丁优化阶段，同时破坏目标对象的语义接地并增加动作轨迹曲率，使感知和控制错误累积，造成长时程任务失败。

Result: 在仿真和真实机器人环境中，该方法在部分可观测条件下持续产生对抗效果，诱导长时程任务中断，显著降低任务成功率。

Conclusion: VLA模型在部分可观测的对抗攻击下存在严重安全漏洞，现有防御机制不足，亟需开发更鲁棒的VLA系统。

Classification reason: 该论文研究VLA机器人模型在部分可观测条件下的对抗性补丁攻击，属于机器人具身智能系统的鲁棒性与安全性问题，与[具身世界模型与规划控制]方向紧密相关。

Importance reason: Focuses on adversarial patch attacks for VLA robots, which is not a priority subtopic and lacks direct connection to user's core interests in continuous multimodal models or video generation. Signals: Partially observable attack, VLA model robustness, attention-guided patch localization, simulation and real-world validation, perception and control disruption
...
### [17] [PHASER: Phase-Aware and Semantic Experience Replay for Vision-Language-Action Models](https://arxiv.org/abs/2606.03598)
...
TL;DR: PHASER提出了一种架构无关的持续学习框架，通过阶段感知的容量分配和多模态干扰路由策略，解决VLA模型在持续学习新技能时的灾难性遗忘问题，在LIBERO基准上ASR提升高达31%。


<details>
  <summary>Details</summary>
Motivation: VLA模型在开放环境中需要持续学习新技能，但会遭遇严重的灾难性遗忘。传统经验回放（ER）采用均匀采样，忽略了操作轨迹的时间特性，导致对短暂但因果关键的子技能采样不足（阶段饥饿），且完全忽视了历史任务不同程度的遗忘。

Method: PHASER包含三个核心组件：(1) 阶段感知容量分配，为所有子技能提供等量记忆支持；(2) 多模态干扰路由策略，动态优先回放遗忘风险高的历史阶段；(3) Auto-PC轻量级流水线，结合无监督动作信号变化点检测与VLM语义验证，自动提取时间边界，无需密集人工标注。

Result: 在LIBERO持续学习套件上，使用三种VLA骨干网络评估，PHASER相比同等预算的ER方法，平均成功率（ASR）提升高达31%，在LIBERO-Goal CL设置上达到87.8%的最终ASR。

Conclusion: PHASER通过阶段感知的持续学习策略有效缓解了VLA模型的灾难性遗忘问题，且Auto-PC实现了无需人工标注的自主生命周期适应，为VLA模型在开放环境中的持续部署提供了可行方案。

Classification reason: 该论文研究VLA模型在机器人操作中的持续学习问题，属于具身智能/机器人领域，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on robotic continual learning, not aligned with user's priority on generative models and multimodal understanding. Signals: Robotics, continual learning, experience replay, VLA models, catastrophic forgetting
...
### [18] [Ask When It Pays: Cost-Aware Open-Ended Interaction for Instance Goal Navigation](https://arxiv.org/abs/2606.03175)
...
TL;DR: 本文提出一种成本敏感的交互式实例目标导航方法，通过信息增益分析确定最优提问时机与问题类型，并构建了交互效率评测基准与加权成功率指标，同时提出零样本MLLM导航器，仅在预期不确定性降低足以抵消交互成本时才进行查询。


<details>
  <summary>Details</summary>
Motivation: 现有交互式导航方法将轻量级澄清与路线级指导同等对待，导致智能体通过频繁提问提升成功率而非高效消解歧义，缺乏对交互成本的建模与评估。

Method: 对现有导航语料进行信息增益分析，识别出能降低导航不确定性的关键线索，形成紧凑的问题类型集合与数据驱动的成本权重；构建交互行为与效率诊断基准；提出零样本MLLM导航器，在每个决策步选择性查询，仅当预期不确定性降低证明交互成本合理时才提问。

Result: 在多个导航基准上验证了所提方法在交互效率与导航成功率之间的平衡优于现有方法。

Conclusion: 成本敏感的交互策略能有效提升实例目标导航的效率，为具身智能体在真实场景中合理利用交互资源提供了新范式。

Classification reason: 该论文研究具身智能体在实例目标导航（IGN）中通过交互（向oracle提问）来消解歧义，属于具身智能环境中的规划与决策问题，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on cost-aware interaction for embodied navigation, which is peripheral to the user's core interests in continuous multimodal models and generative video/image generation. Signals: Instance goal navigation, cost-sensitive interaction, information-gain analysis, zero-shot MLLM navigator, weighted success rate
...
### [19] [GN0: Toward a Unified Paradigm for Generation, Evaluation, and Policy Learning in Visual-Language Navigation](https://arxiv.org/abs/2606.03682)
...
TL;DR: 本文提出GN0，一个统一的视觉语言导航（VLN）框架，包含大规模数据集GN-Matrix、基于3DGS的高保真仿真平台、首个BEV基准GN-Bench以及RL驱动的导航基础模型BAE，在多个VLN任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有VLN系统受限于导航数据的数量和质量，泛化能力和长程任务能力不足。缺乏统一的数据-仿真-学习框架来支撑大规模、高保真的具身导航研究。

Method: (1) 构建GN-Matrix数据集：从多样化3D场景中通过自动化pipeline生成大规模导航数据；(2) 基于3DGS引擎搭建高保真仿真平台，支持交互式漫游和碰撞感知导航；(3) 提出GN-Bench，首个基于BEV的基准，引入动态3DGS虚拟人进行人机交互评估；(4) 开发RL驱动的导航基础模型BAE，采用DAgger策略在监督学习后暴露模型于rollout状态，打破专家分布限制，结合下游RL探索；(5) 将3DGS渲染的BEV表示作为紧凑记忆，解锁VLM中的潜在空间推理。

Result: 在GN-Bench和VLN-CE基准上，GN0在指令跟随、人类跟随、目标导航等任务上均优于现有SOTA方法。

Conclusion: GN0提供了一个统一的数据-仿真-学习框架，有效推动了具身导航在研究和工业应用中的发展，展示了高保真3DGS仿真与RL结合在VLN中的巨大潜力。

Classification reason: 该论文聚焦视觉语言导航（VLN）中的具身导航任务，包含3D场景构建、仿真平台、RL驱动的导航基础模型以及BEV空间推理，属于具身智能中的世界模型与规划控制方向，因此映射到 canonical subtopic "具身世界模型与规划控制"。

Importance reason: Strong in embodied navigation and world models but lacks direct connection to the user's priority subtopics like continuous language models or autoregressive generation. Signals: world model, embodied navigation, RL-driven foundation model, 3DGS simulation, BEV benchmark
...
### [20] [Neural Navigation Functions for Zero-Shot Generalizable Motion Planning](https://arxiv.org/abs/2606.03756)
...
TL;DR: 本文提出 Neural Navigation Functions (Neural-NF)，一种将数据驱动适应能力嵌入结构化椭圆规划器的学习型导航函数。通过将拉普拉斯特征映射到局部 PDE 系数并求解边值问题，Neural-NF 在目标域上生成全局一致的值函数，保证策略无碰撞、单调下降且全局最小点在目标处。该方法在多种未见几何结构上实现强零样本迁移，性能优于直接预测值函数的学习规划器达 5 倍。


<details>
  <summary>Details</summary>
Motivation: 传统导航函数（如势场法）需要针对每个新环境重新设计或调参，而纯数据驱动的学习规划器往往缺乏结构保证（如无碰撞、收敛性）。本文旨在结合两者的优势：保留椭圆规划器的结构化保证，同时利用数据驱动方法实现跨几何结构的零样本泛化。

Method: Neural-NF 将导航目标函数的学习置于结构化椭圆规划器框架内。具体地，使用拉普拉斯本征特征作为输入，通过神经网络映射到局部 PDE 系数，然后求解对应的边值问题（BVP），在目标域上生成全局一致的值函数。该值函数保证策略无碰撞、单调下降且全局最小点位于目标位置，且任何参数设置下都具有线性可解最优控制解释。

Result: 在多种未见过的环境几何结构上进行零样本迁移实验，Neural-NF 表现出强泛化能力，在成功率、路径质量等指标上显著优于直接预测值函数的学习规划器，性能提升最高达 5 倍。

Conclusion: Neural-NF 成功将数据驱动适应能力与结构化规划器的理论保证相结合，实现了跨环境几何结构的零样本泛化。该方法为机器人运动规划提供了一种兼具理论保证和泛化能力的新范式。

Classification reason: 该论文提出 Neural Navigation Functions (Neural-NF)，一种可零样本迁移到未见环境几何结构的反应式导航函数，属于机器人运动规划与控制领域。这与 [CANONICAL] 具身世界模型与规划控制 高度匹配，该方向涵盖面向机器人、具身智能、动作规划和控制的世界模型。

Importance reason: Strong in structured world models for robotics but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: Zero-shot generalization, Neural navigation functions, PDE-based planning, Robotics motion planning, World model for embodied agents
...
### [21] [Let the Dynamics Flow: Stable Flow Matching Dynamical Systems](https://arxiv.org/abs/2606.03834)
...
TL;DR: 本文提出 Stable Flow Matching Dynamical Systems (SFMDS)，将流匹配生成模型与 Lyapunov 稳定性理论结合，为机器人运动生成提供兼具高容量表达与形式化稳定性保证的框架，并在仿真和真实人形机器人上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 流匹配在模仿学习中表现出色，但缺乏形式化的稳定性保证，而这对于安全、可泛化的机器人行为至关重要。现有将机器人运动建模为动力系统的方法虽能引入稳定性归纳偏置，但难以捕捉复杂任务中丰富的动作分布。

Method: SFMDS 通过流匹配参数化动力系统，同时将模型约束到稳定解族中。提出两种变体：基于惩罚项的软约束和嵌入模型架构的硬结构约束，并将两种形式扩展到 Lie 群上以处理刚体运动。

Result: 在基准数据集、仿真环境以及人形机器人上的实验表明，SFMDS 能够在低维和高维状态空间中学习稳定、可扩展且多模态的动力系统，实现安全且富有表现力的机器人运动生成。

Conclusion: SFMDS 成功弥合了高容量生成模型与形式化 Lyapunov 稳定性保证之间的鸿沟，为机器人安全运动生成提供了新范式，且 Lie 群扩展使其适用于更广泛的机器人应用场景。

Classification reason: 该论文提出基于流匹配的稳定动力系统用于机器人模仿学习，属于机器人动作建模与规划控制，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Focuses on robot motion generation with stability guarantees, not directly on user's priority subtopics like continuous language models or video generation. Signals: flow matching, Lyapunov stability, robot motion generation, imitation learning, humanoid robot
...
### [22] [Denoising Tells When to Replan: Denoising-Variance Adaptive Chunking for Flow-Based Robot Policies](https://arxiv.org/abs/2606.03847)
...
TL;DR: 提出DVAC（去噪方差自适应分块）方法，利用流式策略去噪过程中clean-action估计的方差信号，自适应决定每次预测的动作块中实际执行的动作数量，在接触密集或精度敏感阶段提前重规划，在自由空间运动阶段执行更长的稳定动作序列。在LIBERO、RoboTwin、CALVIN及真实机器人操作任务上提升了成功率并降低了重规划频率。


<details>
  <summary>Details</summary>
Motivation: 现有基于流的机器人策略通常使用固定长度的动作分块（action chunking），但自由空间运动与接触密集/精度敏感的操作阶段需要不同的重规划频率。固定分块无法自适应地适应任务阶段变化，导致效率或成功率受限。

Method: 提出DVAC（Denoising-Variance Adaptive Chunking），一种测试时自适应方法。核心思想：流式策略去噪过程中，clean-action估计的方差在不同任务阶段呈现不同模式——稳定运动阶段方差低，接触/精度敏感阶段方差高。DVAC测量最后几步去噪中clean-action估计的方差，执行低方差稳定前缀动作，在高方差动作被提交前触发重规划。同时引入滚动局部方差估计来跨任务和跨轨迹校准阈值。

Result: 在LIBERO上，基于π₀.₅策略，DVAC将成功率从94.75%提升至98.00%，重规划频率降低43.0%；在RoboTwin和CALVIN上也取得综合提升；真实世界操作实验验证了执行效率的改进。

Conclusion: 去噪过程中的方差信号蕴含了任务阶段的语义信息，可被用于自适应地决定动作执行长度。DVAC方法简单有效，无需额外训练即可提升基于流的机器人策略的性能与效率。

Classification reason: 该论文研究基于流的机器人策略中的动作分块与重规划方法，属于机器人操作策略与规划控制范畴，与[具身世界模型与规划控制]（embodied_world_model）这一规范子主题高度匹配。

Importance reason: Focuses on robot policy replanning, not directly on user's priority subtopics like continuous language models or video generation. Signals: flow-based robot policies, action chunking, denoising variance, adaptive replanning, LIBERO benchmark
...
### [23] [Self-Refining Agentic Reinforcement Learning for Vision-Conditioned UAV Navigation](https://arxiv.org/abs/2606.03963)
...
TL;DR: 提出AgenticRL框架，利用多模态GPT智能体自动设计奖励函数、训练PPO策略并通过闭环反馈自改进，用于无人机视觉导航任务，在仿真和真实场景中分别达到91%成功率和94% sim-to-real准确率。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习在自主导航中依赖人工设计的奖励函数和反复手动调参，耗时且难以保证任务成功率。本文旨在提高奖励设计、策略优化和真实部署的自动化程度。

Method: 提出AgenticRL框架，使用多模态GPT智能体：(1) 根据任务信息和视觉场景生成任务特定奖励函数；(2) 用PPO训练策略；(3) 作为critic评估策略并生成诊断反馈，识别失败模式并闭环改进奖励函数。推理时利用真实图像和自然语言任务信息自动选择合适策略。

Result: 闭环自改进使策略行为相比初始奖励提升71%；在多种导航任务（门穿越、避障、越障着陆、轨迹跟踪、行为学习）上验证；sim-to-real迁移达到91%真实世界成功率和94% sim-to-real准确率。

Conclusion: AgenticRL通过多模态大模型智能体实现了无人机导航中奖励函数自动设计与闭环自改进，显著提升了策略性能和sim-to-real迁移能力，为自主机器人导航提供了更高效的范式。

Classification reason: 该论文研究基于强化学习的无人机导航，使用多模态GPT智能体进行奖励函数设计、策略优化与闭环自改进，属于具身智能中的机器人导航与控制，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Focuses on UAV navigation with agentic RL, not directly aligned with user's priority subtopics on continuous language models, video generation, or world models. Signals: AgenticRL, multimodal GPT for reward design, sim-to-real transfer, UAV navigation, closed-loop self-improvement
...
### [24] [EvoMemNav: Efficient Self-Evolving Fine-Grained Memory for Zero-Shot Embodied Navigation](https://arxiv.org/abs/2606.03509)
...
TL;DR: EvoMemNav 提出了一种高效的自演进细粒度记忆框架用于零样本具身导航。它构建视觉-语义记忆图（VSMGraph），将原始视图作为一等记忆，结合轻量语义线索和拓扑关系组织成房间-视图-对象层次结构。采用预算约束的粗到细策略，粗阶段压缩搜索空间，细阶段调用VLM进行验证决策。通过子任务后的反思驱动写回机制更新图附着的先验知识，无需重训练即可持续改进决策。在GOAT-Bench和HM3D上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有零样本具身导航方法中，基于检测器的场景图将观测压缩为稀疏节点，丢失了细粒度视觉证据并累积噪声；而基于3D重建的方法计算开销过大。因此需要一种高效且能保留细粒度信息的记忆框架来支持长时程规划。

Method: 提出EvoMemNav框架，核心包括：(1) 视觉-语义记忆图(VSMGraph)，将原始视图作为一等记忆，用轻量语义线索和拓扑关系组织成房间-视图-对象层次结构；(2) 预算约束的粗到细策略，粗阶段压缩搜索空间到候选区域，细阶段调用VLM进行精确验证和决策；(3) 反思驱动写回机制，在每个子任务完成后更新图附着的先验知识，编码累积的环境信息以优化后续决策。

Result: 在GOAT-Bench和HM3D数据集上，在物体、文本描述和图像目标三种模态下均取得了SR/SPL指标的持续提升，在多实例区分、减少过早停止和零样本泛化方面表现更优。

Conclusion: EvoMemNav通过保留细粒度视觉信息、粗到细的预算约束策略以及自演进的反思写回机制，有效解决了零样本具身导航中的记忆与规划问题，为长时程具身任务提供了一种高效且可扩展的解决方案。

Classification reason: 该论文研究零样本具身导航中的记忆与规划问题，属于具身智能中的世界模型与规划控制方向，与 [CANONICAL] 具身世界模型与规划控制 高度匹配。

Importance reason: Strong embodied navigation work but outside user's core priority subtopics and negative keywords apply. Signals: zero-shot embodied navigation, visual-semantic memory graph, coarse-to-fine VLM policy, self-evolving memory, GOAT-Bench/HM3D
...
### [25] [Making Embodied AI Reliable: A Community Agenda from Testing to Formal Verification](https://arxiv.org/abs/2606.03593)
...
TL;DR: 本文基于AAAI'26 Bridge Program讨论，提出具身AI可靠性是一个生命周期保障问题，识别了三个互补方向：可信场景测试、组合验证和运行时保障，并倡导通过共享神经符号表示和持续反馈将三者整合为统一保障流程。


<details>
  <summary>Details</summary>
Motivation: 具身AI系统在开放世界中的部署日益增多，但其可靠性保障面临根本性挑战，现有方法（测试、验证、运行时保障）各自独立，缺乏系统性整合。

Method: 基于社区讨论，提出具身AI可靠性保障的三方向框架：(1) 基于验证规范和有意义的覆盖度量的可信场景测试；(2) 利用结构化符号表示系统行为和环境上下文的组合验证；(3) 适应不确定性和分布偏移的运行时保障机制。倡导通过共享神经符号表示和持续反馈实现三者的集成。

Result: 识别了具身AI可靠性的核心挑战（不确定性、人机交互、涌现行为），提出了三方向互补的技术路线，并论证了集成保障工作流的必要性。

Conclusion: 具身AI可靠性需要将测试、验证和运行时保障通过神经符号表示和持续反馈整合为生命周期保障流程，为构建在复杂真实环境中安全可靠运行的具身AI系统奠定基础。

Classification reason: 该论文讨论具身AI系统的可靠性保障，涵盖基于场景的测试、组合验证和运行时保障，属于具身智能系统的可靠性工程，与[具身世界模型与规划控制]方向高度相关。

Importance reason: Focuses on reliability assurance for embodied AI, not directly on the user's priority subtopics of continuous multimodal models or generative world models. Signals: Embodied AI reliability, lifecycle assurance, neuro-symbolic representation, scenario-based testing, compositional verification
...
### [26] [DyaPlex: Full-Duplex Speech-Motion Model for Dyadic Interaction](https://arxiv.org/abs/2606.03874)
...
Importance: 值得关注 (66.48) - 未入选精读

TL;DR: DyaPlex 是一个流式全双工语音-动作模型，用于双人交互场景。它基于预训练的全双工语音模型，通过双塔 Transformer 架构和统一的双人 token 交织机制，实现了同步的语音感知与身体动作生成，在 4000 小时数据集上训练，达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 人类交互是连续且双向的（全双工），现有方法多为单向（半双工）或仅处理单一模态，无法同时感知和生成语音与动作。作者希望构建一个能实时流式处理双向语音-动作交互的模型。

Method: 采用双塔 Transformer 架构：一个冻结的全双工语音基础模型保持零样本对话推理能力，另一个深度耦合的流式动作通路生成身体运动。通过统一的双人 token 交织机制和时间对齐的语音-动作 RoPE 引导交叉注意力，将自回归动作与丰富的语音潜特征对齐。

Result: 在 4000 小时 Seamless Interaction 数据集上训练，在单人和双人交互基准上均取得 SOTA 性能，有效捕捉了跨说话人的依赖关系。

Conclusion: DyaPlex 证明了全双工语音-动作联合建模的可行性，为具身智能体实现自然、同步的多模态交互提供了有效框架。

Classification reason: 该论文提出面向双人交互的全双工语音-动作模型，涉及具身智能体在交互中的动作生成与感知，属于具身世界模型与规划控制范畴，因此映射到 canonical subtopic "具身世界模型与规划控制"。

Importance reason: Strong world model and multimodal interaction work, but lacks direct connection to priority subtopics like continuous language models or video generation. Signals: Full-duplex speech-motion model, Dyadic interaction, Dual-tower Transformer, Streaming multimodal generation, 4000-hour dataset
...
### [27] [VLESA: Vision-Language Embodied Safety Agent for Human Activity Monitoring](https://arxiv.org/abs/2606.03954)
...
TL;DR: VLESA是一个视觉语言具身安全代理框架，通过第一视角视频监控人类活动，利用GRPO训练的目标条件安全Q-filter和意图-动作预测代理，在ASIMOV-2.0基准上实现了更准确的安全干预。


<details>
  <summary>Details</summary>
Motivation: AI系统在物理任务中辅助人类时，安全至关重要——物理动作具有即时且不可逆的后果。现有方法难以处理"意图依赖的安全"问题，即相同动作在不同上下文中可能安全或危险。需要一种能从第一视角视频中实时监控并预测危险动作的框架。

Method: VLESA包含两个核心组件：(1) 目标条件安全Q-filter，通过GRPO（Group Relative Policy Optimization）训练，根据推断的意图评估动作安全性，无需重新训练；(2) 意图-动作预测代理，从视频中联合推断目标并预测未来动作。同时构建了配对第一视角帧与目标条件安全标注的数据集。

Result: 在ASIMOV-2.0基准上，VLESA在精确帧级别实现了比基线更高的干预准确率；GRPO训练的Q-filter通过目标条件约束解码将动作安全性提升了超过41个百分点。

Conclusion: VLESA通过结合视觉语言模型与GRPO优化的安全评估，有效解决了意图依赖的安全监控问题，为具身AI系统的安全部署提供了可行方案。

Classification reason: 该论文提出VLESA框架，利用第一视角视频监控人类活动并预测危险动作，进行实时安全干预。这属于具身智能场景下的安全监控与动作预测，与[具身世界模型与规划控制]（embodied_world_model）高度相关。同时，论文涉及视觉语言模型用于意图推断和动作预测，也与多模态理解与生成方向相关。

Importance reason: Focuses on embodied safety monitoring, not core continuous multimodal or video generation priorities. Signals: world model direction, GRPO training, safety Q-filter, ASIMOV-2.0 benchmark, no continuous token or autoregressive generation
...
### [28] [A Close Look At World Model Recovery In Supervised Fine-Tuned LLM Planners](https://arxiv.org/abs/2606.03685)
...
TL;DR: 本文通过一系列可解释性实验，系统探究了经过监督微调（SFT）的LLM规划器是否恢复了底层世界模型。研究发现：SFT使LLM能够线性编码动作有效性和部分状态谓词；即使模型无法通过输出概率正确分类动作有效性，其内部表示仍可能区分有效与无效动作；更广泛的状态空间覆盖（如随机游走数据）有助于更准确地恢复世界模型。


<details>
  <summary>Details</summary>
Motivation: 虽然SFT能提升LLM在经典规划任务上的端到端规划能力，但尚不清楚模型是否真正学习到了规划问题的内部表示和推理能力。由于经典规划问题的相对复杂性和端到端规划生成的挑战，这一问题难以探索。本文旨在通过可解释性方法深入理解微调LLM规划器中的世界模型恢复情况。

Method: 设计了一系列可解释性实验，从内部表示和生成能力两个维度全面检验世界模型恢复情况。具体包括：使用线性探针（linear probes）检测模型隐藏状态中是否编码了动作有效性和状态谓词；分析模型输出概率与动作有效性的关系；比较不同训练数据（如随机游走数据 vs. 规划轨迹数据）对世界模型恢复的影响。

Result: a) SFT使LLM能够线性编码动作有效性和部分状态谓词；b) 无法通过输出概率正确分类动作有效性的模型，其内部表示仍可能区分有效与无效动作；c) 更广泛的状态空间覆盖（如随机游走数据）能带来更准确的世界模型恢复。

Conclusion: 本文为规划LLM的可解释性研究提供了一套方法论，揭示了关于LLM中知识表示方式的开放性问题的新见解。研究表明SFT不仅提升了规划性能，还使模型在一定程度上恢复了底层世界模型，且训练数据的多样性对世界模型恢复至关重要。

Classification reason: 该论文研究SFT微调后的LLM规划器是否恢复了世界模型（world model），通过可解释性实验分析模型对动作有效性和状态谓词的编码能力。这与世界模型方向下的"具身世界模型与规划控制"（embodied_world_model）高度相关，因为该子主题涵盖面向规划和控制的世界模型，且论文明确使用"world model recovery"术语。

Importance reason: Explores world model recovery in SFT LLM planners, but focuses on classical planning rather than video or multimodal world models. Signals: world model recovery, LLM planning interpretability, supervised fine-tuning, action validity encoding, state space coverage
...
### [29] [EvoDrive: Pareto Evolution for Safety-Critical Autonomous Driving via Self-Improving LLM Agents](https://arxiv.org/abs/2606.03678)
...
TL;DR: EvoDrive 是首个基于 LLM agent 的自动化 Pareto 演化框架，用于安全关键自动驾驶场景生成。它采用模拟器接地 actor-critic 架构，通过记忆驱动 actor 迭代改进生成器、critic 过滤不可行候选、自演化世界评估器优化仿真预算，并维护 Pareto 存档以保持多样化的攻击-真实性权衡。在 MetaDrive 和 CARLA 上的实验表明，EvoDrive 显著扩展了 Pareto 前沿并生成有价值的策略训练场景。


<details>
  <summary>Details</summary>
Motivation: 现有安全关键场景生成方法通常依赖手工启发式规则来管理对抗性与真实性之间的权衡，这限制了生成模式局限于已知先验，难以探索未发现的危险模式。虽然开放式的 agentic 演化可以突破这一限制，但无约束的通用 agent 缺乏严格的模拟器接地，容易将多目标张力坍缩为单标量最大化。

Method: EvoDrive 提出一种模拟器接地的 actor-critic 架构：(1) 记忆驱动 actor 迭代提出对生成器的改进方案；(2) critic 过滤不可行的候选；(3) 自演化世界评估器（self-evolving world evaluator）路由有前景的提议以优化仿真预算；(4) 维护一个 Pareto 存档，保存已评估候选的多样攻击-真实性权衡，并利用仿真反馈指导未来演化。

Result: 在 MetaDrive 和 CARLA 两个自动驾驶模拟器上的基准测试表明，EvoDrive 不仅显著扩展了多种生成器下的 Pareto 前沿，还生成了对策略训练有价值的场景。

Conclusion: EvoDrive 证明了基于 LLM agent 的 Pareto 演化框架能够有效解决安全关键场景生成中的多目标优化问题，在保持真实性的同时最大化对抗性，为自动驾驶系统的验证和训练提供了有价值的工具。

Classification reason: 该论文提出EvoDrive，一个基于LLM agent的安全关键自动驾驶场景生成框架，通过actor-critic架构和Pareto存档在多目标（对抗性与真实性）之间进行演化优化。这属于具身智能/机器人领域中的世界模型用于规划、控制和场景生成，与[具身世界模型与规划控制]方向高度匹配。

Importance reason: Strong in world model and autonomous driving safety, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: LLM agent evolution, Pareto multi-objective scenario generation, safety-critical autonomous driving, simulator-grounded actor-critic, self-improving world evaluator
...
### [30] [Exact equivariance, kept through training, buys zero-shot generalisation across the symmetry group](https://arxiv.org/abs/2606.03003)
*Hongbo Wang*
...
Importance: 高优先级 (89.59) - 未入选精读

TL;DR: 本文证明，由等变编码器和等变预测器构建的潜空间世界模型，其训练损失在整个对称群上精确不变，从而在仅看到部分方向数据的情况下实现跨整个群组的零样本泛化。实验在2D/3D/SE(3)上验证了单步预测误差的群不变性（精度达10^-6），且闭环控制轨迹同样保持等变，而同等假设类的非等变基线在分布外误差放大13.8~157倍。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的物理动力学往往具有对称性（如旋转、平移），但现有世界模型通常需要大量数据增强或暴力缩放来泛化到未见过的方向。作者希望利用等变归纳偏置，使世界模型在训练时自动获得跨对称群的泛化能力，从而减少对数据量和计算量的依赖。

Method: 构建一个由等变编码器 E 和等变预测器 f 组成的潜空间世界模型。当世界动力学携带群 G 的正交表示 ρ(g) 时，单步预测的 relMSE 损失在整个群上精确不变。训练使用 Muon/AdamW + EMA + VICReg，在2D/3D/SE(3)设置下验证。闭环控制中，等变规划器确保控制轨迹在群作用下精确对应。与非等变基线（VN）对比，测试跨群组的泛化误差。

Result: 等变模型在2D上误差因子为×1.00，而非等变基线为×13.8；3D上等变×1.00 vs 基线×17.2；完整SE(3)阶梯上等变×1.00 vs 基线×157。等变模型参数量仅为基线的4.5-7.4倍小。闭环控制中，2D/SO(2)上误差浮点精确，3D/SE(3)上统计平坦（95%置信区间不重叠）。H步 rollout 误差在等变模型中保持平坦（×1.00，≤2×10^-7），而基线残差随 H 累积。

Conclusion: 精确等变性在训练后仍然保持，能够带来跨对称群的零样本泛化，且比数据增强、暴力缩放和软等变方法更优。等变世界模型在单步预测和闭环控制中均表现出群不变性，且 rollout 误差不累积，为构建高效、可泛化的世界模型提供了理论保证和实验验证。

Classification reason: 该论文构建了一个等变潜空间世界模型，利用群等变编码器和预测器保证动力学建模的对称性，属于物理世界建模与动力学建模范畴，与 canonical subtopic "可交互与物理世界建模" 高度匹配。

Importance reason: Proves exact equivariance in world models enables zero-shot generalization across symmetry groups, directly relevant to world model and continuous representation priorities. Signals: world model, equivariant latent dynamics, zero-shot generalization, continuous representation, SE(3) invariance
...
### [31] [PersistGS: Differentiable Physics for Object Permanence in 4D Gaussian Splatting](https://arxiv.org/abs/2606.03479)
...
TL;DR: PersistGS 提出将可微刚体物理仿真与4D高斯泼溅结合，在物体被完全遮挡时通过物理轨迹预测保持物体永久性，相比运动学外推方法在PSNR上提升2.46dB。


<details>
  <summary>Details</summary>
Motivation: 动态3D高斯泼溅方法在物体被完全遮挡时，由于光度监督信号消失，表示物体的高斯会退化。现有方法依赖学习先验追求视觉合理性而非物理正确性。作者希望引入物理约束来保持遮挡期间物体的永久性。

Method: 将场景分解为每个物体的高斯和碰撞网格，通过可微仿真从遮挡前轨迹估计摩擦系数和速度，利用SE(3)轨迹在遮挡期间定位物体高斯。引入质心轮廓损失（centroid silhouette loss）将位置梯度与外观噪声分离。

Result: 在合成场景实验中，PersistGS 比恒速外推方法PSNR提升2.46dB，与真实轨迹上界仅差0.19dB，轨迹误差降低40%。

Conclusion: 可微物理仿真能够有效恢复动态场景重建中遮挡期间的物体永久性，提供比运动学外推更准确的物理一致轨迹预测。

Classification reason: 该论文将可微刚体物理仿真与4D高斯泼溅相结合，在遮挡期间保持物体永久性，属于物理一致的世界建模。匹配世界模型方向下的[可交互与物理世界建模]（interactive_physical_world）规范子主题。同时，该方法涉及动态3D场景生成，也匹配多模态生成方向下的[4D与动态场景生成]（dynamic_4d_generation）。

Importance reason: Directly advances world model and physics-grounded 4D reconstruction, a high-priority direction for the user. Signals: differentiable physics simulation, 4D Gaussian splatting, object permanence under occlusion, rigid body dynamics, centroid silhouette loss
...
### [32] [A 3D Isovist World Model -- Revealing a City's Unseen Geometry and Its Emergent Cross-City Signature](https://arxiv.org/abs/2606.03609)
...
TL;DR: 本文提出一种3D Isovist世界模型，将城市中可导航的"负空间"（建筑之间的开放体积）编码为球面可见性深度图（3D isovist），通过历史isovist序列和动作预测下一时刻的isovist。模型采用深度残差预测、自滚动调度采样和持久潜空间鸟瞰图记忆。令人惊讶的是，仅在曼哈顿和巴黎训练的城市盲模型，其时间潜变量中出现了跨城市空间特征，城市身份可线性解码，表明该特征存在于学习到的动力学中而非外观中。


<details>
  <summary>Details</summary>
Motivation: 现有具身导航世界模型大多预测场景外观（RGB图像），而非智能体实际可穿越的几何空间；而预测鸟瞰图占用网格的方法则丢弃了地面以上的三维结构。缺乏一种能够捕捉可导航几何结构、不依赖光度信息且不丢失三维信息的预测目标。

Method: 将城市可导航空间编码为3D isovist（球面可见性深度图），记录每个方向到最近表面的距离。模型以过去isovist序列和移动动作为输入，预测下一isovist。预测被公式化为深度残差（使解码器保留建筑锐利边缘），训练采用自滚动调度采样（self-rollout scheduled sampling）以保持几何流形上的上下文，并配备持久潜空间鸟瞰图记忆以实现跨路径一致性。

Result: 在曼哈顿和巴黎数据集上，模型能准确预测3D isovist。更重要的发现是：一个从未见过城市标签的模型，其时间潜变量中城市身份可被线性解码器高精度识别，远超单帧基线，表明模型学到了跨城市的空间动力学特征而非外观特征。

Conclusion: 3D isovist世界模型为具身导航提供了一种轻量级、可解释、可复现的几何空间预测框架，其跨城市空间特征的涌现表明该方法有望成为空间推理的几何基础，适用于具身AI、机器人和城市分析。

Classification reason: 该论文提出一种基于3D isovist（可见性深度图）的具身世界模型，用于城市导航中的几何空间预测，属于世界模型方向下的空间3D/4D世界模型（spatial_3d_4d_world_model）这一[CANONICAL]子主题，因为其核心是建模三维可导航空间几何而非外观。

Importance reason: Novel 3D isovist world model for urban geometry, but tangential to user's focus on continuous multimodal models and video generation. Signals: 3D isovist world model, negative space geometry, cross-city spatial signature, robotics and urban analysis, not aligned with priority subtopics
...
### [33] [MetaWorld: Scaling Multi-Agent Video World Model from Single-view Video Data](https://arxiv.org/abs/2606.02753)
...
TL;DR: MetaWorld提出一个从单视角视频数据扩展多智能体视频世界模型的框架，通过单目世界状态展开（MWSU）分解相机运动与主体轨迹，结合主体感知世界生成器和世界状态对齐（WSA）机制，实现跨视角一致的多智能体视频世界建模。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型局限于单智能体单视角，扩展到多智能体场景面临两个关键挑战：多视角协调数据难以获取（数据稀缺），以及独立生成的视频流无法保证共享物理环境在不同视角下的一致性（世界状态对齐）。

Method: 1) 单目世界状态展开（MWSU）：将单目视频分解为相机操作者的自我运动与可见主体的空间轨迹，在共享3D空间中提取同步的多智能体运动数据；2) 主体感知世界生成器（Subject-Aware World Generator）：基于每个智能体的身份图像进行外观驱动的条件生成；3) 世界状态对齐（WSA）：在视频DiT的每个Transformer层插入帧间跨分支交叉注意力机制，联合同步去噪过程，保证静态几何一致性与动态运动一致性。

Result: 大量实验表明MetaWorld在跨视角一致性和身份保真度上达到优越性能，建立了可扩展的、物理驱动的多智能体视频世界建模范式。

Conclusion: MetaWorld成功从单视角视频扩展到多智能体视频世界模型，解决了数据稀缺和世界状态对齐两大挑战，为具身AI和元宇宙中的多智能体场景建模提供了可扩展的解决方案。

Classification reason: 该论文提出MetaWorld，从单视角视频扩展多智能体视频世界模型，核心是视频生成式世界建模与未来预测，属于[world_model]方向下的[CANONICAL] subtopic "视频世界模型"（video_world_model）。同时其视频生成部分也匹配[multimodal_generation]方向。

Importance reason: Directly advances video world models for multi-agent settings, aligning with top priority world model and video generation subtopics. Signals: world model, video generation, multi-agent, single-view to multi-view, cross-view consistency
...
### [34] [Cosmos 3: Omnimodal World Models for Physical AI](https://arxiv.org/abs/2606.02800)
...
Main category: cs.RO, cs.CV, cs.LG, cs.MM, cs.AI
...
TL;DR: Cosmos 3 是一个全模态世界模型家族，采用统一的混合 Transformer 架构，联合处理与生成语言、图像、视频、音频和动作序列，在理解与生成任务上达到 SOTA，并作为具身智能体的通用骨干网络。


<details>
  <summary>Details</summary>
Motivation: 现有模型通常针对单一或少数模态设计（如 VLM、视频生成器、世界模拟器），缺乏一个统一的框架来同时处理 Physical AI 所需的多模态感知、生成与动作规划。作者旨在构建一个全模态（omnimodal）世界模型，将视觉-语言模型、视频生成器、世界模拟器和世界-动作模型统一到一个框架中，为具身智能体提供可扩展的通用骨干。

Method: Cosmos 3 采用混合 Transformer（mixture-of-transformers）架构，支持高度灵活的输入-输出配置，能够联合处理语言、图像、视频、音频和动作序列。模型通过统一的 token 空间和模态条件机制，实现任意模态到任意模态的转换。训练使用大规模多模态数据，包括自建的合成数据集，并采用分阶段训练策略。

Result: 在多种理解与生成任务上达到 SOTA；在 Artificial Analysis 评测中，后训练的 Cosmos 3 模型被评为最佳开源文生图和图生视频模型；在 RoboArena 评测中被评为最佳策略模型。代码、模型权重、合成数据集和评测基准均已开源。

Conclusion: Cosmos 3 证明了全模态世界模型作为具身智能体可扩展通用骨干的可行性，为 Physical AI 提供了一个统一的基础框架。开源发布将加速相关领域的研究与部署。

Classification reason: 该论文提出 Cosmos 3，一个统一处理语言、图像、视频、音频和动作序列的全模态世界模型，核心是面向 Physical AI 的世界建模。它同时涉及视频生成、多模态理解与生成、连续多模态建模，但最核心的贡献是作为世界模型（world simulator + world-action model），因此主方向为 world_model，匹配 [CANONICAL] subtopic "视频世界模型"（video_world_model），该 subtopic 涵盖视频生成式世界模型与动作条件视频生成。

Importance reason: This paper directly advances the user's top-priority continuous multimodal and world model directions with a unified architecture achieving SOTA across understanding and generation. Signals: world model, unified multimodal, video generation, autoregressive video generation, continuous multimodal model
...
### [35] [NVIDIA OmniDreams: Real-Time Generative World Model for Closed-Loop Autonomous Vehicle Simulation](https://arxiv.org/abs/2606.03159)
...
TL;DR: NVIDIA 提出 OmniDreams，一个基于 Cosmos 扩散模型实时自回归生成动作条件视频的生成式世界模型，用于自动驾驶闭环仿真，能够合成极端天气和不可预测动态行为等复杂场景，并展示了作为策略模型骨干的潜力。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶策略在长尾场景中的安全评估是瓶颈。基于重建的神经模拟器受限于初始捕获数据，难以泛化到高度动态或新颖场景。需要一种能生成未观测现象、实时响应驾驶动作的生成式世界模型。

Method: 从 Cosmos 扩散模型出发，经过 mid-training 和 post-training（21k 小时驾驶数据），构建 OmniDreams 生成式世界模型。该模型以自回归方式生成动作条件视频，条件包括过去帧、当前模拟器状态和即时驾驶动作。部署在闭环系统中，与 Alpamayo 1 策略模型和 AlpaSim 编排器协同工作。此外，从 OmniDreams 后训练得到世界-动作模型（WAM）。

Result: OmniDreams 能够实时合成极端天气和不可预测动态行为等复杂场景。WAM 在 Physical AI AV NuRec 数据集上超越 VLA 基线的 Alpamayo 1.5 策略模型，同时仅使用其 1/5 的参数。

Conclusion: OmniDreams 为自动驾驶闭环仿真提供了可扩展的生成式世界模型解决方案，能够实时响应驾驶动作并合成复杂场景。初步结果表明，该世界模型还可作为策略架构的骨干网络，展示了世界模型在策略学习中的潜力。

Classification reason: 该论文提出 OmniDreams，一个基于 Cosmos 扩散模型构建的生成式世界模型，用于自动驾驶闭环仿真中的动作条件视频生成。这属于视频世界模型（video_world_model）的典型工作，因此复用该 canonical subtopic。

Importance reason: Directly advances world model and video generation for autonomous driving, aligning with top priority subtopics and boost keywords. Signals: world model, video generation, autoregressive, diffusion model, autonomous driving
...
### [36] [Reinforcement Learning from Cross-domain Videos with Video Prediction Model](https://arxiv.org/abs/2606.03201)
...
TL;DR: XIPER 提出一种跨域视频预测奖励模型，通过将智能体观测映射到专家域并利用预测似然作为奖励信号，实现在视觉外观差异显著的域间（如颜色、形态、sim-to-real）从专家视频中进行强化学习。


<details>
  <summary>Details</summary>
Motivation: 在强化学习中，从视觉外观不同的专家视频中学习非常困难，因为缺乏奖励信号且存在域间差异（如颜色、形态、sim-to-real 差距）。现有方法难以处理这种跨域模仿学习问题。

Method: XIPER 训练一个跨域视频预测模型，将智能体观测映射到专家域，并使用预测似然作为奖励信号。该模型通过视频预测的方式弥合域间差距，为智能体提供有意义的奖励。

Result: 在 DMC Color Suite（8 个任务）和 DMC Body Suite（3 个任务）上，XIPER 在智能体颜色和形态差异等域间差距下持续优于基线方法。在 sim-to-real 迁移数据集上，仅使用模拟专家视频即可为真实机器人观测产生有意义的奖励信号。

Conclusion: XIPER 通过跨域视频预测模型有效解决了跨域模仿学习中的奖励信号缺失问题，在多种域间差距场景下均表现出色，并展示了从模拟到真实迁移的潜力。

Classification reason: 该论文使用视频预测模型作为奖励信号，从跨域专家视频中学习，属于动作条件视频生成与未来预测范畴，与 [CANONICAL] 视频世界模型 (video_world_model) 高度匹配。

Importance reason: Strongly aligns with world model and video generation priorities, using cross-domain video prediction for RL reward. Signals: world model, video prediction, cross-domain, sim-to-real, reinforcement learning
...
### [37] [PointAction: 3D Points as Universal Action Representations for Robot Control](https://arxiv.org/abs/2606.03943)
...
TL;DR: PointAction 提出通过微调视频生成模型联合预测未来RGB帧和动态3D点图，将点动力学作为通用动作接口，再由扩散动作解码器映射为机器人可执行动作，实现了跨任务、跨本体的机器人操控泛化。


<details>
  <summary>Details</summary>
Motivation: 现有的视频-动作模型（VAM）利用预训练视频扩散模型进行视觉动力学预测，但纯RGB视频预测缺乏度量3D运动、接触几何和细粒度空间约束，导致动作接地模糊；同时，跨任务和跨本体的动作监督数据收集成本高昂。

Method: PointAction 微调一个基础视频生成模型，使其联合预测未来RGB帧和动态3D点图（pointmaps），生成时序一致的3D场景几何运动。这些点动力学作为结构化、与本体无关的动作接口，再由一个扩散动作解码器将其映射为可执行的机器人动作。

Result: 在机器人场景的4D生成质量上达到SOTA；在仿真环境中优于现有基线；能够泛化到预训练中未见过的两种真实机器人手臂。

Conclusion: 通过将度量3D点动力学作为视频预测与动作控制之间的接口，PointAction 有效降低了纯RGB动作接地的歧义性，并在有限动作监督下实现了跨任务和跨本体的迁移，验证了显式4D建模在机器人操控中的价值。

Classification reason: 该论文利用预训练视频扩散模型预测未来RGB帧和动态3D点图，将视频预测桥接到机器人动作控制，属于动作条件视频生成与未来预测，与[世界模型]方向下的[视频世界模型]（video_world_model）规范主题高度匹配。

Importance reason: Bridges video generation world models to robot control via 3D point dynamics, aligning with world model and video generation priorities. Signals: world model, video diffusion, 3D point dynamics, robot control, embodiment-agnostic
...
### [38] [World Models Meet Language Models: On the Complementarity of Concrete and Abstract Reasoning](https://arxiv.org/abs/2606.03603)
...
TL;DR: 本文提出受控具体推理（controlled concrete reasoning）框架，将世界模型的视觉未来模拟与MLLM的抽象推理相结合。构建了两个基准VRQABench和OpenWorldQA，并提出Privileged-Future On-Policy Self-Distillation（PF-OPSD）方法，在训练时利用真实未来视频作为教师信号，测试时无需真实未来信息，在两项基准上分别提升10.6%和10.9%。


<details>
  <summary>Details</summary>
Motivation: 世界模型可以生成具体的视觉未来推演（visual rollout），MLLM可以进行抽象推理，但生成的rollout具有随机性且可能视觉合理但任务错误。因此需要确定何时视觉模拟有用、rollout是否可信以及如何影响最终答案，现有方法缺乏这种受控的具体推理机制。

Method: 提出受控具体推理（controlled concrete reasoning）框架，模型学习调用、验证和整合视觉未来模拟与抽象推理。构建VRQABench（可控空间前瞻）和OpenWorldQA（开放域物理预测）两个人工验证基准。提出Privileged-Future On-Policy Self-Distillation（PF-OPSD），训练时利用真实未来视频和答案作为教师端特权上下文来评估on-policy具体推理轨迹，部署的学生模型在测试时无需真实未来信息。

Result: PF-OPSD在VRQABench和OpenWorldQA上分别比基线提升10.6%和10.9%，同时对噪声或冲突的rollout具有更强的鲁棒性。

Conclusion: 世界模型的具体视觉模拟与MLLM的抽象推理具有互补性，通过受控具体推理框架可以有效结合两者优势，提升未来预测任务的准确性和鲁棒性。

Classification reason: 论文研究世界模型与MLLM结合进行视觉未来预测与抽象推理，核心是视频世界模型（visual rollout/future simulation），匹配[world_model]方向下的[CANONICAL] subtopic "视频世界模型"（video_world_model）。同时涉及MLLM抽象推理，因此也匹配multimodal_understanding_generation方向。

Importance reason: Directly addresses world model and MLLM integration, a core priority, with strong benchmarks and a novel distillation method. Signals: world model + MLLM integration, controlled concrete reasoning, PF-OPSD distillation, VRQABench and OpenWorldQA benchmarks, strong performance gains
...
### [39] [Unified Video-Action Joint Denoising for Dexterous Action and Data Generation](https://arxiv.org/abs/2606.03868)
...
TL;DR: 提出 Donk，一个统一的视频-动作联合去噪模型，在多种条件（语言、初始图像、初始手部状态）下联合建模交互视频与灵巧手轨迹，既可作为动作策略，也可作为数据生成引擎。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型通常将视频先验收窄为观测条件下的动作策略分布，丢失了联合视频-动作分布的丰富信息。作者希望保持更宽的分布，通过联合建模交互视频与可执行手部轨迹来更好地利用视频基础模型的视觉-动力学先验。

Method: 提出 Donk 统一视频-动作去噪模型，基于扩散/去噪框架，在多种条件（语言、初始图像、初始手部状态）下联合去噪生成未来视频帧和双手 MANO 轨迹。无图像条件时，同一架构可从文本条件分布中采样配对的视频-动作 rollout，作为数据引擎。模型统一了动作策略生成和数据生成两种模式。

Result: 在动作生成、视频生成和纯文本条件生成三种评测中，Donk 提升了灵巧手轨迹精度，保持了强视频保真度，并在统一训练框架下生成了平滑的文本条件动作 rollout。

Conclusion: 联合视频-动作去噪是一种有效的范式，能够在保持视频生成质量的同时提升灵巧手动作轨迹的准确性，并且同一架构可同时服务于策略执行和数据生成，展示了视频基础模型在机器人操作中的潜力。

Classification reason: 该论文提出统一视频-动作联合去噪模型 Donk，用于灵巧手操作，同时支持动作策略生成（动作条件视频生成）和数据引擎（文本条件视频-动作 rollout 生成），属于视频世界模型的核心范畴，因此映射到 world_model 方向下的 [CANONICAL] subtopic "视频世界模型 (video_world_model)"。同时，论文也涉及文本条件生成视频-动作数据，可归入多模态生成方向。

Importance reason: Directly advances world model and video generation for dexterous action, aligning with top-priority continuous multimodal and world model subtopics. Signals: world model, video-action joint denoising, dexterous hand trajectory, data generation engine, unified multimodal conditioning
...
### [40] [KC-3DGS: Kurtosis-Constrained Gaussian Splatting for High-Fidelity View Synthesis](https://arxiv.org/abs/2606.03120)
...
TL;DR: KC-3DGS 通过在 3DGS 训练中引入小波域监督（多尺度小波系数对齐损失、峰度集中损失、跨频带协方差惩罚），显著提升了新视角合成的感知质量，尤其在稀疏视图场景下效果明显。


<details>
  <summary>Details</summary>
Motivation: 标准 3DGS 使用像素空间损失（L1、SSIM）仅约束整体重建误差，允许误差在不同频率尺度间重新分布，导致过平滑和结构伪影，尤其在稀疏视图设置中更为严重。作者希望利用自然图像的统计特性（重尾频率分布）来约束渲染图像的频域特征。

Method: 提出 KC-3DGS，在 3DGS 训练中增加三个小波域损失组件：(1) 多尺度小波系数对齐损失，显式惩罚缺失的高频细节；(2) 有监督的峰度集中损失，鼓励渲染图像匹配真实图像的重尾频率统计；(3) 跨频带协方差惩罚，促进频率特化。理论分析表明像素空间损失在小波重分布下存在不可区分的扰动族，而联合目标函数排除了退化解。

Result: 在 MipNeRF360、Tanks&Temples、MVImgNet、DeepBlending 和 WRIVA-ULTRRA 数据集上一致提升感知质量。在 WRIVA-ULTRRA 户外数据集上 DreamSim 提升 9.48%，同时改善 PSNR、SSIM 和 LPIPS。稀疏视图（仅 12 张训练图像）下在 MipNeRF360 上 PSNR 提升高达 0.5 dB。

Conclusion: KC-3DGS 作为一种即插即用的正则化策略，能无缝集成到现有 3DGS 流程中，有效提升渲染质量，尤其适用于稀疏视图和户外场景。

Classification reason: 该论文提出 KC-3DGS，改进 3D Gaussian Splatting（3DGS）的渲染质量，属于 3D 场景重建与神经渲染范畴，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Novel wavelet-domain supervision for 3DGS improves perceptual quality but is not aligned with user's priority subtopics. Signals: 3D Gaussian Splatting, wavelet-domain supervision, novel view synthesis, sparse-view improvement, perceptual quality
...
### [41] [FreeStreamGS: Online Feed-forward 3D Gaussian Splatting from Unposed Streaming Inputs](https://arxiv.org/abs/2606.03254)
...
TL;DR: FreeStreamGS 是一个在线前馈式 3D 高斯泼溅框架，能够从无位姿的流式输入图像序列中实时进行高质量新视角合成，通过解耦内参恢复头和动态点偏移修正策略，解决了长期流式处理中的尺度抖动和位姿-深度漂移问题，渲染质量可与离线方法媲美。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈式 3DGS 方法依赖离线记录的图像序列，无法处理流式、无位姿的在线输入。虽然已有在线几何估计方法，但它们无法直接用于新视角合成，因为 NVS 对高斯尺度的多视角一致性和位姿-几何对齐要求更严格，微小偏差会随时间累积导致渲染质量严重下降。

Method: 提出 FreeStreamGS 框架，包含两个关键机制：(1) 解耦内参恢复头（Decoupled Intrinsic Recovery Head），消除累积的相机内参偏差，防止长期流式处理中的场景尺度抖动；(2) 动态点偏移修正策略（Dynamic Point Refinement Offset），放松刚性的反投影约束，修正耦合的位姿-深度漂移。整体采用在线前馈方式，无需访问未来帧。

Result: 大量实验表明，FreeStreamGS 在渲染质量上与最先进的离线前馈式 3DGS 方法相当，同时支持在线流式处理，无需未来帧信息。

Conclusion: FreeStreamGS 首次实现了从无位姿流式输入进行在线高质量新视角合成，解决了长期流式处理中的尺度抖动和位姿-深度漂移问题，为实时 3D 场景重建与渲染提供了可行的在线方案。

Classification reason: 该论文提出在线前馈式3D高斯泼溅（3DGS）框架，用于从无位姿的流式输入进行新视角合成，属于3D场景重建与渲染范畴，因此映射到 [CANONICAL] subtopic "3D 生成与重建" (three_d_generation)。

Importance reason: Novel online feed-forward 3DGS for unposed streaming inputs, but outside user's priority subtopics. Signals: 3D Gaussian Splatting, online feed-forward, novel view synthesis, unposed streaming, decoupled intrinsic recovery
...
### [42] [SimuScene: Simulation-Ready Compositional 3D Scene Reconstruction from a Single Image](https://arxiv.org/abs/2606.03994)
...
TL;DR: SimuScene 提出一种将物理仿真融入组合式3D场景重建流程的方法，利用物理引擎作为诊断工具，在生成过程中通过重力轴拉伸和模态形状重采样来修正穿透、悬浮等物理不稳定问题，从而从单张图像重建出可直接用于机器人操控的仿真就绪3D场景。


<details>
  <summary>Details</summary>
Motivation: 从单张图像重建可用于物理仿真的3D场景是机器人操控的关键瓶颈。现有单图3D提升方法虽能恢复合理的物体形状，但组合后常出现物体相互穿透、悬浮或下沉等物理不稳定问题。已有的物理感知方法仅将物理作为后处理布局修正，无法解决底层的几何误差。

Method: SimuScene 将物理引擎作为诊断测量工具融入生成过程：在重建过程中对物体在重力下进行诊断性仿真，将穿透和支撑失败转化为定量修正信号，驱动重力轴拉伸和模态形状重采样，形成物理信息反馈循环，逐步修正累积的重建误差。

Result: 在物理稳定性和几何对齐基准上达到最先进性能，并在人形机器人控制和机械臂操控任务中验证了重建场景的实用性。

Conclusion: 将物理仿真融入生成式3D重建流程而非仅作为后处理，能有效提升组合式3D场景的物理稳定性和几何精度，为机器人操控提供可直接部署的仿真就绪场景。

Classification reason: 该论文从单张图像重建可交互、可物理仿真的组合式3D场景，属于3D场景生成与重建范畴，与[3D 生成与重建]（three_d_generation）这一规范子主题高度匹配。

Importance reason: Novel physics-in-the-loop 3D reconstruction, but outside user's priority subtopics and lacks direct connection to continuous/multimodal language models or video generation. Signals: 3D scene reconstruction, physics simulation, single image, robotic manipulation, compositional 3D
...
### [43] [MLP Splatting: Object-Centric Neural Fields](https://arxiv.org/abs/2606.03877)
...
TL;DR: MLP-Splatting 提出用多个紧凑 MLP 作为光场基元替代传统 3DGS 的高斯基元，实现场景分解与高质量新视角合成，支持物体级编辑和开放词汇交互，内存仅为语义 3DGS 方法的 1/15，渲染速度快 3 倍。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 表示方法（如 3DGS 和 NeRF）在新视角合成上表现出色，但难以将场景自然地分解为可操作的物体级基元，需要额外的分割或分组步骤才能实现物体级编辑。作者希望设计一种既能提供高质量渲染，又能天然支持场景分解和物体级交互的 3D 表示方法。

Method: 将场景建模为少量紧凑 MLP 基元，每个 MLP 具有局部空间支持，独立预测辐射度和不透明度。渲染时通过稀疏体素合成对射线-基元交互进行高效组合。仅使用 RGB 监督训练，基元自动学习表示局部场景区域（通常对应物体或物体部件）。可选地结合语义特征蒸馏，支持开放词汇场景交互和开放集实例分割。

Result: 在多个场景上实现了与 SOTA 方法相当的新视角合成质量，同时内存占用仅为语义 3DGS 方法的 1/15，渲染速度快 3 倍。通过选择少量基元即可实现交互式物体级编辑，无需分割掩码。结合语义蒸馏后支持开放词汇交互和开放集实例分割。

Conclusion: MLP-Splatting 通过紧凑 MLP 基元在保持高质量渲染的同时实现了场景的天然分解，为 3D 场景表示提供了一种兼顾渲染质量、内存效率和可编辑性的新范式，在物体级交互和开放词汇场景理解方面具有优势。

Classification reason: 该论文提出 MLP-Splatting，用紧凑 MLP 作为神经基元进行场景分解和新视角合成，属于 3D 场景表示与神经渲染方法，与 canonical 子主题 "3D 生成与重建" 高度匹配。

Importance reason: Novel object-centric neural field representation with strong efficiency gains, but only tangentially touches user's priority on continuous language/multimodal models and video generation. Signals: 3D scene decomposition, compact MLP primitives, memory efficiency (1/15x), faster rendering (3x), open-vocabulary interaction
...
### [44] [PatchScene: Patch-based Voxel Diffusion for Large-Scale Scene Completion](https://arxiv.org/abs/2606.03915)
...
TL;DR: PatchScene 提出了一种基于 patch 的体素扩散框架，用于大规模 LiDAR 场景补全。它通过局部 patch 扩散生成细粒度几何结构，引入置信度引导的时空融合机制整合重叠 patch 与相邻帧，并设计 Annular-Flow 扩散策略利用 LiDAR 径向密度模式从近到远传播高保真信息，实现空间无界场景补全。在 SemanticKITTI 上达到 SOTA，且 20m 训练模型可泛化到 50m 场景。


<details>
  <summary>Details</summary>
Motivation: 现有 LiDAR 场景补全方法依赖全局潜表征或密集体素网格，难以处理大规模场景的细粒度几何细节和远距离区域，且缺乏时空一致性。

Method: 提出 patch-based 体素扩散范式，将场景划分为局部 3D patch 分别进行扩散生成；设计置信度引导的时空融合机制，在统一生成过程中整合重叠 patch 和相邻帧；提出 Annular-Flow 扩散策略，利用 LiDAR 径向密度模式从近到远逐步传播高保真信息。

Result: 在 SemanticKITTI 基准上所有标准指标达到 SOTA，几何精度和时序一致性均超越先前方法。20m 范围训练的模型无需重新训练即可有效泛化到 50m 场景。

Conclusion: PatchScene 通过 patch 级扩散和 Annular-Flow 策略有效解决了大规模 LiDAR 场景补全中的细粒度几何生成和远距离泛化问题，展示了强可扩展性和泛化能力，适用于自动驾驶等真实场景。

Classification reason: 该论文提出基于扩散的大规模 LiDAR 场景补全方法，属于 3D 场景生成与重建任务，与 [CANONICAL] subtopic "3D 生成与重建" 高度匹配。

Importance reason: Focuses on 3D scene completion for autonomous driving, which is outside the user's priority subtopics and negative keywords. Signals: 3D scene completion, LiDAR, diffusion, patch-based, autonomous driving
...
### [45] [Sparse-View Lung Nodule Volumetry from Digitally Reconstructed Radiographs via AReT: Anatomy-Regularized TensoRF](https://arxiv.org/abs/2606.02639)
...
TL;DR: 本文发现 TensoRF 在 X 射线衰减场重建中的一个关键失败模式（默认密度偏移 -10 抑制梯度），提出将密度偏移设为 0 以恢复梯度流，并在此基础上提出 AReT——一种解剖学正则化的张量辐射场框架，仅需三个正交 X 光投影即可实现肺结节的三维体积重建，在 LIDC-IDRI 数据集上对 ≥10mm 结节达到 Pearson r=0.983 的高精度。


<details>
  <summary>Details</summary>
Motivation: 现有 NeRF 方法需要密集多视角采集才能实现医学三维重建，而临床实践中通常只有少量正交 X 光投影可用。此外，TensoRF 默认的密度偏移参数（为 RGB 场景设计）在 X 射线衰减场中会抑制密度梯度，导致稀疏视角下重建失败。本文旨在解决这两个问题，实现仅用三个正交投影的肺结节三维体积重建。

Method: 1) 将 TensoRF 的密度偏移从默认的 -10 改为 0，恢复梯度流动，使稀疏视角重建成为可能。2) 提出 AReT（Anatomy-Regularized TensoRF），结合胸部解剖先验的正则化：L1 稀疏性正则化 + 总变差（TV）平滑正则化。3) 使用 LIDC-IDRI 数据集中 19 名患者的冠状面、矢状面和轴状面投影进行训练，以放射科医生共识分割为金标准进行评估。

Result: 在 ≥10mm 的临床可操作结节（n=14）上，AReT 达到 Pearson r=0.983（p<0.0001），中位绝对体积误差 11.4%，近零系统偏差 -77.3 mm³，比球体体积近似法提升 8.4 倍。解剖感知正则化始终优于生成先验引导的方法。

Conclusion: 通过修正 TensoRF 的密度偏移并引入解剖学正则化，AReT 实现了仅用三个正交 X 光投影的肺结节高精度三维体积重建，为稀疏视角医学影像三维重建提供了有效方案。

Classification reason: 该论文提出 AReT，一种基于 TensoRF（张量辐射场）的肺结节三维重建方法，属于 3D 场景/物体重建范畴，与 canonical subtopic "3D 生成与重建"（three_d_generation）高度匹配。

Importance reason: Focuses on medical 3D reconstruction from sparse X-rays, which is outside the user's priority subtopics and negative-keyword-filtered. Signals: Medical imaging, TensoRF failure mode fix, Sparse-view 3D reconstruction, Lung nodule volumetry, Not in priority subtopics
...
### [46] [BEAST3D: Animal behavioral analysis and neural encoding from multi-view video via Gaussian splatting](https://arxiv.org/abs/2606.02937)
...
TL;DR: BEAST3D 是一个自监督预训练框架，利用多视角视频通过 Vision Transformer 预测 3D 高斯泼溅来重建动物 3D 结构，支持新视角合成、姿态估计和神经编码等下游任务。


<details>
  <summary>Details</summary>
Motivation: 从多视角视频中提取动物 3D 行为表征对于神经科学和行为分析至关重要，但现有方法要么需要大量人工标注（监督姿态估计），要么依赖密集重叠视角的通用 3D 重建模型，不适用于实验室稀疏视角场景。

Method: BEAST3D 使用 Vision Transformer 从多视角图像中预测 3D 高斯泼溅（3D Gaussian Splats），通过可微渲染重建未见视角，同时分割动物与背景。模型直接利用已知相机参数进行条件化，仅需 4 个视角即可重建 3D 结构。

Result: 在四个物种（小鼠、狨猴、斑马鱼、果蝇）的多视角视频上评估，BEAST3D 在新视角合成、多视角姿态估计和神经编码三个下游任务中均取得优异表现，验证了学习到的 3D 表征的丰富性和视角不变性。

Conclusion: BEAST3D 提供了一个无需标注的自监督框架，能够从实验室常见的稀疏多视角视频中学习丰富的 3D 行为表征，为动物行为分析和神经科学中的 3D 结构利用建立了通用框架。

Classification reason: 该论文使用 3D 高斯泼溅（3DGS）从多视角视频中重建动物 3D 结构，属于 3D 场景重建与神经渲染范畴，与 canonical 子主题 "3D 生成与重建" 高度匹配。

Importance reason: Focuses on animal behavior and neural encoding, not aligned with user's priority on continuous multimodal models or video generation. Signals: 3D Gaussian splatting, multi-view video, self-supervised pretraining, animal behavior, neural encoding
...
### [47] [Conditional Latent Diffusion Model with Fourier-based Motion Modelling for Virtual Population Synthesis](https://arxiv.org/abs/2606.03827)
...
TL;DR: 本文提出4D F-MeshLDM，一种条件潜扩散框架，用于虚拟人群合成中的3D+t心脏网格序列生成。该方法结合卷积网格VAE、基于傅里叶级数的结构潜空间运动参数化，以及以临床协变量为条件的扩散先验，实现了周期一致、高保真的动态心脏网格生成。


<details>
  <summary>Details</summary>
Motivation: 医疗器械的计算机模拟试验需要生成虚拟人群的解剖结构，尤其是心血管应用中需要3D+t（动态）网格序列。现有网格生成器大多关注静态解剖，而序列模型缺乏显式的周期性建模，难以生成生理上合理的动态心脏网格。

Method: 提出4D F-MeshLDM框架，包含三个核心组件：(1) 卷积网格VAE，将3D网格编码为结构潜空间；(2) 利用截断傅里叶级数参数化运动轨迹，将时序运动表示为傅里叶系数token；(3) 扩散先验模型学习傅里叶系数token的潜分布，并通过仿射调制以临床协变量（如年龄、性别、BMI等）条件化生成。采样token后经逆傅里叶合成得到周期一致的潜轨迹，解码为3D+t心脏网格序列。

Result: 在5,000名UK Biobank受试者数据上，4D F-MeshLDM在解剖保真度上优于现有基线方法，且实现了接近零的周期闭合误差。生成的虚拟人群能够准确保留临床功能指标（如射血分数等），展示了在可靠的心脏计算机模拟试验中的应用潜力。

Conclusion: 4D F-MeshLDM通过傅里叶运动建模与条件潜扩散相结合，有效解决了动态心脏网格生成中的周期性和可控性问题，为虚拟人群合成和医疗器械计算机模拟试验提供了可靠的技术方案。

Classification reason: 该论文提出4D F-MeshLDM，用于生成3D+t心脏网格序列（动态4D网格），属于动态场景/4D生成范畴，与[CANONICAL] subtopic "4D 与动态场景生成"（dynamic_4d_generation）高度匹配。

Importance reason: Focuses on medical cardiac mesh generation, which is outside the user's priority subtopics and has negative keyword overlap. Signals: Medical imaging, 4D mesh generation, Fourier motion model, Conditional diffusion, Virtual population synthesis
...
### [48] [SparseStreet: Sparse Gaussian Splatting for Real-Time Street Scene Simulation](https://arxiv.org/abs/2606.03909)
...
TL;DR: SparseStreet 提出了一种面向街景场景的稀疏高斯泼溅压缩框架，通过节点级可学习剪枝和背景压缩策略，在保持动态物体高保真度的同时实现高达80%的压缩率，显著降低存储开销并提升渲染速度。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法在街景重建中需要大量高斯基元来捕捉细节，导致存储成本高昂且渲染速度慢。动态物体需要高保真表示以保持时序一致性，而静态背景区域存在大量冗余，因此需要一种针对街景场景的高效压缩方法。

Method: 提出 SparseStreet 通用压缩框架：(1) 节点级可学习剪枝策略，系统性地移除低贡献高斯基元，同时保留视觉关键区域；(2) 场景表示稳定后应用背景压缩，进一步减少静态区域冗余。该方法在保持动态物体几何与外观的同时显著减少高斯基元总数。

Result: 在 Waymo 和 nuScenes 数据集上的实验表明，SparseStreet 实现了高达80%的压缩率，且质量损失极小，实现了资源高效的高保真动态场景重建。

Conclusion: SparseStreet 有效解决了街景场景中3D高斯泼溅的存储和渲染效率问题，通过区分动态与静态区域进行差异化压缩，在压缩比与重建质量之间取得了良好平衡，为实时街景模拟提供了可行方案。

Classification reason: 该论文关注街景动态场景的3D高斯泼溅重建与压缩，属于动态3D场景生成与重建范畴，与 canonical subtopic "4D 与动态场景生成"（dynamic_4d_generation）高度匹配。

Importance reason: Focuses on street scene compression, not directly aligned with user's priority subtopics like continuous language models or autoregressive generation. Signals: 3D Gaussian Splatting, street scene compression, dynamic object preservation, background redundancy reduction, real-time rendering
...
### [49] [Training-Free Multi-Concept LoRA Composition with Prompt-Aware Weighting](https://arxiv.org/abs/2606.03792)
...
TL;DR: 本文提出一种无需训练的提示感知加权方法（W-Switch 和 W-Composite），通过根据触发词在目标提示中的语义重要性动态加权多个 LoRA 模块的输出，实现多概念个性化图像生成，并在 ComposLoRA 基准上取得一致改进。


<details>
  <summary>Details</summary>
Motivation: 现有 LoRA 方法能实现单概念个性化，但多概念组合时不同 LoRA 权重或输出之间的干扰会导致视觉质量下降和概念保真度降低。需要一种简单有效的方法来组合多个 LoRA 模块，实现高质量的多概念定制生成。

Method: 提出两种提示感知加权策略：W-Switch 和 W-Composite。利用每个概念对应提示 token 的相对重要性，在生成过程中动态调整各 LoRA 模块的权重。同时提出基于图像的相似性评估框架，通过自动分割生成图像中的概念区域并与真实参考图像比较，评估图像保真度和身份保持。

Result: 在 ComposLoRA 测试集上，该方法在视觉质量、身份保持和组合性方面一致优于现有 SOTA 方法。LLM 评估和用户研究进一步验证了方法的有效性，并与新提出的定量图像指标一致。

Conclusion: 提示感知的加权策略能有效缓解多 LoRA 组合中的概念干扰问题，无需额外训练即可实现高质量的多概念个性化图像生成。新提出的图像级评估框架也为多概念定制提供了更可靠的定量评价手段。

Classification reason: 该论文研究多概念 LoRA 组合的个性化图像生成，属于主体与概念驱动生成（customization_generation）这一 [CANONICAL] 子主题。

Importance reason: Addresses multi-concept LoRA composition for controllable generation, but lacks direct connection to user's priority subtopics like continuous models or world models. Signals: Training-free multi-concept LoRA, prompt-aware weighting, controllable generation, subject-driven generation, diffusion model personalization
...
### [50] [Inverting the Generation Process of Denoising Diffusion Implicit Models: Empirical Evaluation and a Novel Method](https://arxiv.org/abs/2606.03111)
...
TL;DR: 本文提出一种混合DDIM反演方法，结合梯度下降直接反演（第一步）和不动点方法（后续步骤），显著提高了从生成图像恢复初始噪声图的准确性，并引入自插值测试作为新的评估指标。


<details>
  <summary>Details</summary>
Motivation: DDIM图像生成过程的逆问题（从生成图像恢复初始噪声图）在图像编辑等应用中至关重要，但现有方法在恢复初始潜变量方面精度不足，且缺乏有效的评估指标来衡量反演质量。

Method: 提出混合反演方法：第一步使用梯度下降直接优化反演（direct inversion via gradient descent），后续步骤采用不动点方法（fixed-point method）迭代求解。同时提出自插值测试（self-interpolation test），通过在真实潜变量与预测潜变量之间插值并评估生成图像质量来衡量反演性能。

Result: 在三个数据集上的实验表明，所提方法在初始潜变量预测精度和重建准确率上均显著优于现有方法。自插值测试进一步揭示现有方法虽重建尚可，但初始潜变量预测失败，而本方法在所有指标上均表现最佳。

Conclusion: 所提出的混合DDIM反演方法能更准确地恢复初始噪声图，为扩散模型在图像生成和编辑中的应用提供了更好的基础，自插值测试也为反演质量评估提供了新视角。

Classification reason: 该论文研究DDIM图像生成过程的逆过程（inversion），用于恢复初始潜变量（噪声图），这是图像编辑（如基于噪声图编辑）中的关键技术。因此映射到[CANONICAL] subtopic "图像与视频编辑" (visual_editing)。

Importance reason: Focuses on DDIM inversion for editing, not directly on user's priority subtopics like continuous models or video generation. Signals: DDIM inversion, image editing, reconstruction accuracy, self-interpolation test, not autoregressive or video generation
...
### [51] [JAVEDIT: Joint Audio-Visual Instruction-Guided Video Editing with Agentic Data Curation](https://arxiv.org/abs/2606.03168)
...
TL;DR: 本文提出 JAVEdit，首个指令引导的音视频联合编辑方法。构建了大规模高质量数据集 JAVEdit-100k（约10万条编辑三元组，涵盖5类编辑），设计了智能体在环质量控制机制，并提出了评测基准 JAVEditBench。实验表明 JAVEdit 在6项指标中的5项上优于所有基线。


<details>
  <summary>Details</summary>
Motivation: 现有指令引导视频编辑方法仅关注视觉模态，忽略了音频的联合编辑。缺乏面向音视频联合编辑的数据集和标准化评测基准，限制了该方向的发展。

Method: (1) 构建 JAVEdit-100k 数据集：设计4条生成流水线（subject editing, speech editing, background audio editing, style editing, combined editing），配合智能体在环质量控制机制自动筛选高质量数据。(2) 提出 JAVEdit 基线模型：基于扩散模型架构，支持同时接收文本指令、视频和音频输入，实现音视频的联合编辑。

Result: 在 JAVEditBench 评测基准上，JAVEdit 在6项评估指标中的5项上超越所有基线方法，验证了音视频联合编辑的有效性。

Conclusion: 本文填补了指令引导音视频联合编辑领域的数据集和基准空白，提出的 JAVEdit 模型展示了联合编辑的可行性，为后续研究提供了基础资源和方法参考。

Classification reason: 该论文研究指令引导的音视频联合编辑（joint audio-visual editing），属于视频编辑的扩展方向。现有 [CANONICAL] subtopic "visual_editing（图像与视频编辑）" 涵盖视频编辑，且论文核心是编辑任务，因此映射至此。

Importance reason: Novel joint audio-visual editing dataset and method, but outside user's core priority subtopics. Signals: First joint audio-visual editing dataset, Agent-in-the-loop quality control, Outperforms baselines on 5/6 metrics, Focus on human-centric videos, Lacks direct connection to continuous language/multimodal models or world models
...
### [52] [MariData: One-Step Unpaired Image Translation for Maritime Environments](https://arxiv.org/abs/2606.03246)
...
TL;DR: 本文提出MariData框架，利用CycleGAN-turbo一步式无配对翻译架构，通过零卷积跳跃连接绕过VAE瓶颈，为海洋环境生成合成数据（Day→Foggy/Sunset/Night），在保持小目标结构细节的同时合成逼真天气条件，解决自主船舶感知数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 海洋自主船舶（MASS）的感知系统因缺乏多样化训练数据（尤其是恶劣天气和低光照条件）而受限。由于动态海洋环境中无法采集配对图像，无配对图像翻译成为关键方案，但现有生成模型因潜空间压缩瓶颈难以保留小导航目标（如远处船只、航标）的精细结构细节。

Method: 采用CycleGAN-turbo一步式无配对翻译架构，在生成器中引入零卷积跳跃连接（zero-convolution skip connections）以绕过VAE瓶颈，显式保留小目标细节。编译了7000张海洋图像数据集，用于Day→Foggy、Day→Sunset、Day→Night三种域翻译的训练与评估。

Result: 定性评估和变强度推理实验表明，该方法能有效合成逼真的大气条件，同时保持场景的底层语义结构。Day→Foggy和Day→Sunset模型表现出良好的结构保留能力；Day→Night模型则因不平衡训练分布导致语义幻觉（如生成人工海岸灯光）的挑战。

Conclusion: 本文建立了一种高效、结构感知的数据合成流水线，直接解决了自主海洋导航中的数据稀缺瓶颈。零卷积跳跃连接有效改善了小目标细节保留，但夜间翻译中的语义幻觉问题仍需进一步研究。

Classification reason: 该论文研究基于CycleGAN-turbo的海洋环境无配对图像翻译（Day→Foggy/Sunset/Night），属于图像到图像翻译（unpaired image translation），可归入[图像与视频编辑]（visual_editing）这一规范子主题，因为图像翻译是图像编辑/风格化的一种形式。

Importance reason: Focuses on maritime domain-specific unpaired translation, not aligned with user's priority on continuous multimodal models or autoregressive generation. Signals: Unpaired image translation, CycleGAN-turbo, Maritime data scarcity, Small object preservation, Domain-specific application
...
### [53] [TASE: Truncation-Aware Semantic Embeddings for 3D Scene Understanding and Editing](https://arxiv.org/abs/2606.03314)
...
TL;DR: TASE 提出了一种截断感知语义嵌入方法，将 2D 语义特征投影到可截断的嵌入空间，实现灵活可控的文本驱动 3D 场景编辑，支持从保留细节到大幅几何修改的渐进式编辑控制。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 场景表示方法在可控编辑方面支持有限，难以在保留原始场景细节与进行大幅几何修改之间灵活权衡。需要一种能够显式控制编辑强度的方法。

Method: TASE 将预训练的 2D 语义特征（如 DINOv2）投影到一个截断感知嵌入空间中，通过逐步减少特征通道获得越来越抽象的语义表示，保留更多通道则保留细粒度细节。同时使用尺度和平移等变损失改进多视角一致性。此外，对编辑扩散模型进行微调以缓解几何变化引起的伪影。

Result: 实验表明 TASE 在 3D 场景编辑任务上具有竞争性表现，在涉及大幅几何修改的编辑任务上显著优于先前方法。

Conclusion: TASE 通过截断感知嵌入空间实现了对 3D 场景编辑强度的显式控制，为灵活、可控的 3D 场景编辑提供了有效方案。

Classification reason: 该论文提出 TASE 方法，将预训练的 2D 语义特征投影到截断感知嵌入空间，实现文本驱动的 3D 场景编辑。这属于多模态生成方向下的图像与视频编辑（visual_editing）子主题，因为其核心是 3D 场景的可控编辑，与现有 canonical 子主题高度匹配。

Importance reason: Focuses on 3D scene editing, which is not a priority subtopic, and lacks strong alignment with user's key boost keywords. Signals: 3D scene editing, truncation-aware embeddings, text-driven editing, multi-view consistency, not autoregressive or video generation
...
### [54] [Bootstrap Your Generator: Unpaired Visual Editing with Flow Matching](https://arxiv.org/abs/2606.03911)
...
TL;DR: 本文提出Bootstrap Your Generator (ByG)，一种无需配对数据的flow matching编辑模型训练框架。通过从冻结的基础模型中提取指令跟随线索并结合循环一致性保持结构，配合梯度路由策略弥合训练-推理差距，在图像和视频编辑任务上达到SOTA，甚至超越百万级监督基线。


<details>
  <summary>Details</summary>
Motivation: 现代生成模型具备强大的视觉理解能力，但训练图像/视频编辑模型通常需要大量配对数据，这在视频编辑中成本极高。现有无监督方法依赖外部奖励模型或额外信号，限制了可扩展性。本文旨在利用基础模型自身知识实现无配对训练。

Method: 提出ByG框架：(1) 从冻结的基础flow matching模型中提取指令跟随语义线索（如注意力图、特征）作为训练信号；(2) 结合循环一致性损失保持结构信息；(3) 提出梯度路由策略，将下游损失对干净预测的梯度路由到噪声训练状态，弥合训练-推理差距。整体无需外部奖励模型或配对数据。

Result: 在多个数据稀缺的图像和视频编辑场景中达到SOTA，用户研究表明方法能有效泛化到未见领域，并超越在百万级样本上训练的监督基线。分析验证了梯度路由有效弥合了训练-推理差距，语义线索提取提供了鲁棒训练信号。

Conclusion: ByG证明了无需配对数据即可训练高质量的flow matching编辑模型，通过利用基础模型自身知识替代外部监督信号，为数据稀缺场景下的视觉编辑提供了可扩展的通用框架。

Classification reason: 该论文提出了一种基于flow matching的无配对图像/视频编辑框架ByG，属于图像与视频编辑方向，因此映射到[CANONICAL] subtopic "visual_editing"（图像与视频编辑）。

Importance reason: Strong flow matching editing framework with unpaired training, but not directly aligned with top priority continuous language/multimodal or world model subtopics. Signals: flow matching, unpaired training, image and video editing, gradient routing, cycle-consistency
...
### [55] [Pixel Cube: Diffusion-based Portrait Video Relighting Through Realistic Lighting Reproduction](https://arxiv.org/abs/2606.02919)
...
TL;DR: 本文提出 Pixel Cube，一种基于扩散模型的动态人像视频重光照方法，利用混合训练数据集（真实采集+渲染数据）和 LED 光照系统，以逐帧 HDR 环境图为光照控制，结合预训练视频扩散模型的先验，生成光照一致、身份保持、时间连贯的重光照人像视频。


<details>
  <summary>Details</summary>
Motivation: 现有视频重光照方法在真实感、光照和谐性和时间一致性方面存在不足，且缺乏高质量的真实动态人像重光照数据集。本文旨在解决动态人像视频在任意新光照条件下的真实感重光照问题。

Method: 构建 LED 光照系统用于真实光照模拟和高速视频重光照数据采集，收集真实拍摄和渲染的混合训练数据。利用预训练视频扩散模型的图像先验，以逐帧 HDR 环境图作为光照控制条件，并引入合成背景图像来控制相机曝光水平和色调，训练生成模型实现真实感、身份保持的动态人像视频重光照。

Result: 在野外视频上使用多种环境图进行重光照实验，结果表明该方法在真实感、光照和谐性和时间一致性方面达到最先进水平，能忠实保持表情、肤色、皱纹和面部毛发等细节，且对未见过的数据具有良好的泛化能力。

Conclusion: 本文提出的扩散式人像视频重光照方法能够生成光照真实、时间一致的重光照结果，在肖像摄影等实际应用中具有潜力。

Classification reason: 该论文研究基于扩散模型的动态人像视频重光照（portrait video relighting），属于数字人/肖像动画方向，与 [CANONICAL] 数字人与动作生成（avatar_motion_generation）高度匹配。

Importance reason: Focuses on portrait video relighting, which is not a priority subtopic and lacks strong alignment with user's configured direction weights and boost keywords. Signals: diffusion-based video relighting, portrait video, HDR environment map, hybrid real-rendered data, temporal consistency
...
TL;DR: 本文提出一种基于隐式运动学习的音频驱动肖像动画框架，采用两阶段流水线：第一阶段通过区域感知注意力机制融合外观先验和层次深度线索建模隐式运动特征；第二阶段使用Mamba增强的扩散模型从音频和源图像直接预测这些特征，实现无监督的细粒度运动学习。在380小时新数据集上训练，在多个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统基于关键点的方法难以捕捉细微的运动动态，且现有方法在运动预测与渲染的耦合上限制了灵活性和效率。本文旨在提出一种解耦的隐式运动框架，以生成更真实、时序更连贯的音频驱动人像动画。

Method: 提出两阶段流水线：1）第一阶段使用区域感知注意力机制，结合外观先验和层次深度线索，在潜空间中建模隐式运动特征；2）第二阶段采用Mamba增强的扩散模型，从音频和源图像直接预测这些隐式运动特征，实现无监督的细粒度运动模式学习。两阶段将运动预测与渲染解耦，提升灵活性和效率。

Result: 在多个公开基准和自建数据集上，该方法在准确性、自然度和时序连贯性方面均优于先前工作，达到新的SOTA水平。

Conclusion: 本文验证了解耦运动预测与渲染的两阶段隐式运动框架在音频驱动肖像动画中的有效性，Mamba增强的扩散模型能够从音频中无监督地学习细粒度运动模式，为说话头生成等任务提供了新的技术路径。

Classification reason: 该论文研究音频驱动的肖像动画（talking-head synthesis），属于数字人与动作生成（avatar_motion_generation）这一[CANONICAL]子主题的典型范畴。

Importance reason: Uses Mamba for motion diffusion, but focuses on portrait animation, not the user's priority subtopics. Signals: Mamba-enhanced diffusion, implicit motion learning, audio-driven animation, 380-hour dataset, SOTA benchmarks
...
TL;DR: PHAF 提出一种从仅两张手部图像（背侧和掌侧）快速生成个性化照片级真实感手部虚拟人的方法，通过语义引导网格对齐和密集纹理提取，结合视图修补网络，将纹理生成时间相比现有方法缩短30倍，同时保持相当的视觉质量，适用于AR/VR实时部署。


<details>
  <summary>Details</summary>
Motivation: 现有手部虚拟人方法依赖慢速优化过程，无法在边缘设备上实时部署，限制了AR/VR等实际应用。作者希望从极少量输入图像（仅两张）快速生成高质量、可动画化的手部虚拟人。

Method: PHAF 结合语义引导的网格对齐和密集纹理提取，从两张手部图像高效迁移高频细节；使用基于视图的修补网络对纹理进行细化，确保平滑连续的外观；利用参数化手部模型实现精确的关节运动，兼容标准图形引擎。

Result: 实验表明，PHAF 在视觉保真度上与现有方法相当，但纹理生成时间降低了30倍，支持实时部署在边缘设备上，并能泛化到新视角。

Conclusion: PHAF 证明了从极少量输入快速生成个性化手部虚拟人的可行性，大幅提升了生成效率，为AR/VR等实时交互应用提供了实用方案。

Classification reason: 该论文提出个性化照片级真实感手部虚拟人（hand avatar），从两张图像快速生成纹理并支持实时部署，属于数字人与动作生成（avatar_motion_generation）范畴，该 canonical 子主题涵盖虚拟人、人体动作等，手部虚拟人是其合理子集。

Importance reason: Focuses on hand avatar generation, which is not a priority subtopic and lacks alignment with user's key directions like continuous language models or video generation. Signals: Hand avatar generation, fast texture generation, AR/VR application, limited to two-view input, no connection to continuous multimodal or world model topics
...
TL;DR: AvatarMix 提出一种组合式3D虚拟人换装方法，通过直接组合两个高保真高斯头像的头部和身体，并采用 SeamFix 和 FullbodyFix 两级精修策略，实现身份保持的跨头像服装个性化合成，避免了传统方法中的质量退化和穿模问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D虚拟人换装方法存在两大问题：将2D编辑提升到3D的方法会导致服装或身份质量退化；分别建模身体和服装层的方法容易出现穿模伪影。作者希望提出一种绕过这些问题的组合式范式。

Method: AvatarMix 采用组合式范式，直接从一个高斯头像中提取头部、从另一个中提取身体进行组合。提出两级精修策略：(1) SeamFix：局部扩散模块，精修头发和颈部区域以确保无缝连接；(2) FullbodyFix（可选）：全身精修，在重定向导致服装退化时恢复服装外观。采用基于网格的高斯表示，支持鲁棒的网格重定向技术，精确重塑着装身体以适应目标用户体型。

Result: 实验表明，该方法在服装保真度和身份保持方面达到了最先进水平，能够鲁棒地处理多样化的体型，且由于基于已具有3D一致性的高斯头像进行渲染，相比2D到3D提升方法减少了多视角伪影。

Conclusion: AvatarMix 提供了一种新的3D虚拟人换装个性化视角，通过组合式范式和两级精修策略有效解决了现有方法的质量退化和穿模问题，在服装保真度和身份保持上取得了SOTA结果。

Classification reason: 该论文研究3D虚拟人换装合成，涉及头部与身体的组合、高斯头像渲染、服装个性化，属于数字人与虚拟人相关生成任务，映射到[CANONICAL] subtopic "数字人与动作生成" (avatar_motion_generation)。

Importance reason: Focuses on 3D avatar outfit transfer, which is not a priority subtopic and lacks strong alignment with user's configured direction weights. Signals: 3D avatar composition, outfit personalization, identity preservation, Gaussian avatar, SeamFix refinement
...
### [59] [Text-to-Image Models Need Less from Text Encoders Than You Think](https://arxiv.org/abs/2606.03715)
...
TL;DR: 本文发现文生图扩散Transformer模型实际上只依赖文本编码器中的两个简单方面：相邻token合并为词表示，以及词序（位置编码）。作者构造了一种仅编码单个词义和词序的"带位置标签的词袋"表示，发现其足以生成与完整文本嵌入质量相当的图像，表明图像模型自身承担了解码复杂语言结构的任务。


<details>
  <summary>Details</summary>
Motivation: 文本编码器为文生图模型提供包含丰富上下文信息（如组合性、属性绑定）的文本嵌入，但图像模型是否真正利用了这些信息尚不清楚。作者希望探究文本表示的哪些方面对图像生成是必要的。

Method: 作者构造了一种"带位置标签的词袋"（bag of position-tagged words）文本嵌入，仅编码每个单词的独立含义及其在句子中的位置顺序，完全去除上下文信息。将该嵌入替换到DiT-based文生图模型中，与完整文本嵌入进行对比实验。

Result: 实验表明，这种简化的嵌入在视觉质量和文本忠实度上与完整文本嵌入引导的生成效果相当，说明文生图模型并未充分利用文本编码器中的丰富上下文信息。

Conclusion: 文生图模型实际上主要依赖文本编码器的两个简单方面（词合并与词序），复杂语言结构的解码由图像模型自身完成。这一发现挑战了关于文本编码器作用的普遍认知，为简化文生图模型设计提供了新思路。

Classification reason: 该论文研究文生图模型中文本编码器的作用，属于文本到图像生成的基础方法分析，与 [CANONICAL] 文生图与图像扩散 子主题高度匹配。

Importance reason: Challenges assumptions about text encoder usage in diffusion models, relevant to multimodal generation efficiency. Signals: Text encoder analysis, Diffusion transformer, Bag-of-words representation, Image generation efficiency, Model simplification insight
...
### [60] [Video-Mirai: Autoregressive Video Diffusion Models Need Foresight](https://arxiv.org/abs/2606.03971)
...
Importance: 高优先级 (88.74) - 未入选精读

TL;DR: Video-Mirai 提出一种仅训练阶段的未来感知方法，通过冻结的未来编码器从完整 rollout 中蒸馏目标到因果状态，在不改变推理架构和计算量的情况下，显著提升长视频生成的一致性。


<details>
  <summary>Details</summary>
Motivation: 标准自回归视频扩散训练中，每个因果状态只负责解释当前片段，忽略了未来帧所需的身份、布局和运动信息，导致表示层面的规划缺口（planning gap），使得长视频生成中主体和背景一致性下降。

Method: 在训练时，生成器因果地 rollout 出完整视频序列；一个冻结的未来编码器（foresight encoder）以非因果方式读取完整 rollout 并提取未来感知特征；一个轻量级预测器将这些停止梯度的目标蒸馏回因果状态。推理时丢弃编码器和预测器，保持原始架构和计算量不变。

Result: 在 5 秒 VBench 上总分从 83.8 提升至 84.6；在超出训练时长的 30 秒 rollout 上，主体一致性从 84.9 提升至 88.5，背景一致性从 90.2 提升至 91.9。

Conclusion: 因果性应约束推理而非表示监督，视觉自回归模型需要未来感知能力。Video-Mirai 通过训练阶段的未来目标蒸馏，在不增加推理开销的前提下有效弥合了规划缺口。

Classification reason: 该论文提出 Video-Mirai，一种自回归视频扩散模型的训练方法，属于视频生成方向。虽然方法涉及自回归推理，但核心是视频扩散模型中的因果预测与未来规划，与 [CANONICAL] 文生图与视频扩散 中的 text_to_video_diffusion 子主题高度匹配。

Importance reason: Directly addresses a core limitation in autoregressive video generation with a training-only fix that preserves inference efficiency, highly relevant to your priority on continuous models and world models. Signals: autoregressive video generation, training-only method, future-conditioned distillation, long video consistency, no inference overhead
...
### [61] [AAD-1: Asymmetric Adversarial Distillation for One-Step Autoregressive Video Generation](https://arxiv.org/abs/2606.03972)
...
Importance: 高优先级 (92.46) - 未入选精读

TL;DR: AAD-1 提出了一种非对称对抗蒸馏框架，用于一步自回归图像到视频生成。通过打破生成器与判别器的对称性（生成器保持因果性，判别器双向关注全时空上下文），并结合分阶段训练策略（先用分布匹配预热，再进行对抗蒸馏），有效解决了运动坍塌和训练不稳定问题，在 VBench 上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有一步自回归视频生成方法采用对抗蒸馏，但面临运动坍塌（生成静态视频）和训练不稳定的问题。作者希望设计一种既能保持自回归采样能力，又能有效检测全局时序失败和长程漂移的框架。

Method: 提出非对称对抗蒸馏（AAD-1）框架：(1) 架构上，生成器保持因果性以保留自回归采样能力，判别器采用双向注意力覆盖全时空上下文，输出单个整体真实性分数；(2) 训练策略上，采用分阶段方法：先用分布匹配（distribution matching）引导一步生成器预热，使学生分布接近教师分布，再进行对抗蒸馏。

Result: 在 VBench 基准上，AAD-1 在一来自回归视频生成任务中取得了最先进的性能，有效缓解了运动坍塌问题，生成了动态且高质量的视频。

Conclusion: 非对称对抗蒸馏框架能够有效解决一步自回归视频生成中的运动坍塌和训练不稳定问题，为高效视频生成提供了新思路。

Classification reason: 该论文提出一步自回归图像到视频生成方法，属于视频生成范畴。虽然使用了自回归范式，但核心任务是视频生成（image-to-video），与"文生视频与视频扩散"（text_to_video_diffusion）这一[CANONICAL]主题最为匹配，该主题涵盖视频生成方法（包括非扩散方法）。

Importance reason: Directly advances one-step autoregressive video generation with a novel asymmetric adversarial distillation, highly relevant to your priority on autoregressive video generation and model efficiency. Signals: autoregressive video generation, one-step generation, adversarial distillation, motion collapse solution, VBench SOTA
...
### [62] [Diffusing in the Right Space: A Systematic Study of Latent Diffusability](https://arxiv.org/abs/2606.03578)
...
TL;DR: 本文系统研究了潜扩散模型中视觉tokenizer的潜空间性质对下游生成质量的影响，发现重建质量并不等价于扩散友好性。通过大规模实验对比多种tokenizer，识别出与生成质量稳定相关的潜空间属性，并提出Velocity Irreducible Variance (VIV) 作为衡量轨迹交叉导致的歧义性的新指标，实验表明VIV是生成质量最稳定的预测因子之一。


<details>
  <summary>Details</summary>
Motivation: 现有潜扩散模型依赖视觉tokenizer将图像压缩到潜空间，但tokenizer的重建质量与生成质量之间存在不一致性。已有研究提出了多种扩散友好潜空间的性质解释，但缺乏系统性验证，不清楚哪些因素对生成质量最具预测力，以及结论是否具有泛化性。

Method: 训练了大量具有不同正则化策略、架构和潜配置的tokenizer，使用多个下游扩散骨干网络进行评估。系统分析了多种潜空间属性（语义可分性、仿射等变性、分布均匀性、空间结构、频谱平滑性、流形连续性）与生成质量的相关性。提出了Velocity Irreducible Variance (VIV)，通过测量潜空间中轨迹交叉导致的velocity歧义性来量化扩散友好性。

Result: 识别出多个与生成质量稳定相关的潜空间属性，这些属性在不同实验设置下表现出强泛化性。VIV指标被证明是生成质量最稳定的预测因子之一，优于传统的重建质量指标。

Conclusion: 潜空间的扩散友好性需要超越重建质量进行专门评估，VIV为设计更优的视觉tokenizer和潜扩散模型提供了新的指导原则。

Classification reason: 论文系统研究潜空间扩散友好性（latent diffusability），提出新指标VIV，属于生成模型效率与质量分析，与efficient_generation（生成模型加速与高效化）相关；同时涉及连续视觉tokenizer与潜空间性质，也与continuous_visual_tokenizer相关。但核心贡献是分析潜空间性质对生成质量的影响，更贴合生成模型方向。

Importance reason: Directly addresses latent space properties for diffusion models, aligning with user's priority on continuous language/multimodal models and efficient generation. Signals: latent diffusability, velocity irreducible variance, tokenizer analysis, generation quality prediction, systematic study
...
### [63] [Qwen-Image-Flash: Beyond Objective Design](https://arxiv.org/abs/2606.03746)
...
Main category: cs.CV, cs.GR, cs.LG, cs.AI
...
TL;DR: 本文从训练配方（数据组成、教师指导、任务混合）的角度重新审视少步蒸馏，以 Qwen-Image-2.0 为案例系统分析三个关键因素，提出 Qwen-Image-Flash，表明有效的少步蒸馏不仅需要精心设计的损失函数，还需要原则性的训练流程组织。


<details>
  <summary>Details</summary>
Motivation: 现有少步蒸馏工作大多聚焦于蒸馏目标（loss objective）的设计，而忽略了训练配方（training recipe）对蒸馏效果的关键影响。本文旨在从数据组成、教师指导、任务混合等互补视角系统研究这些因素，以进一步提升少步蒸馏的性能。

Method: 以 Qwen-Image-2.0 为基座模型，系统研究三个训练配方因素：(1) 数据组成——分析不同数据分布对蒸馏效果的影响；(2) 教师指导——探索教师模型输出（如 soft label、特征匹配）的使用策略；(3) 任务混合——研究文生图与指令引导图像编辑任务的联合蒸馏策略。基于实证分析结果，设计并训练 Qwen-Image-Flash 模型。

Result: 揭示了若干非直观的行为规律，表明训练配方的合理组织对少步蒸馏效果有显著影响。Qwen-Image-Flash 在统一文生图和指令引导图像编辑任务上取得了优于基线方法的性能。

Conclusion: 有效的少步蒸馏不仅需要精心设计的蒸馏目标，还需要对训练流程（数据、教师、任务混合）进行原则性的组织。本文为加速视觉生成模型的蒸馏训练提供了新的视角和实用指导。

Classification reason: 该论文研究少步蒸馏（few-step distillation）加速视觉生成模型，属于生成模型加速与高效化的核心范畴，因此映射到 canonical subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Directly addresses efficient inference for text-to-image generation, a high-priority subtopic, with a practical training recipe analysis. Signals: efficient inference, text-to-image generation, few-step distillation, training recipe, Qwen-Image-Flash
...
### [64] [A Quantitative Approximation Framework for Flow Distillation in Diffusion Models](https://arxiv.org/abs/2606.03820)
...
TL;DR: 本文提出了一个扩散模型蒸馏的定量近似框架，将少步采样视为学习流映射组合下的误差传播。通过分析概率流ODE的局部近似误差在低噪声多模态区域中的放大效应，证明了ReLU-ReQU网络对高斯混合分数场的逼近保证，并推导了流映射稳定性估计。基于此，提出了一种稳定性平衡的非均匀时间网格，实验表明相比均匀网格可降低最多51.9%的相对MSE。


<details>
  <summary>Details</summary>
Motivation: 扩散模型蒸馏（如一致性模型、少步扩散）在实践中取得了成功，但其理论基础尚不充分，特别是少步采样中误差如何传播和放大的机制缺乏定量理解。本文旨在建立一个严格的数学框架来理解扩散蒸馏中的近似误差与稳定性问题。

Method: 将少步采样建模为学习流映射的复合函数，分析概率流ODE的局部近似误差传播。在高斯混合Ornstein-Uhlenbeck设定下，分离出两个核心困难：时间相关分数场的逼近和由时间积分Jacobian界控制的动力学放大。使用ReLU-ReQU网络证明分数场的L^p(p_t)逼近保证，推导概率流速度的空间Lipschitz常数L(t)的显式界，并转化为流映射稳定性估计。基于稳定性分析提出非均匀时间网格划分策略。

Result: 理论方面：证明了ReLU-ReQU网络对高斯混合分数场的均匀时间逼近保证，深度和宽度随目标精度呈多对数增长；推导了概率流速度的Lipschitz界和流映射稳定性估计；识别了"Lipschitz不匹配"机制，说明一步蒸馏在结构上不利。实验方面：稳定性平衡的非均匀时间网格相比均匀网格，在8个分段下将端到端相对MSE降低最多51.9%。

Conclusion: 本文为扩散模型蒸馏提供了首个定量近似框架，揭示了低噪声多模态区域中误差放大的机制，并提出了稳定性驱动的非均匀时间网格策略。理论分析与实验验证一致，为理解和改进扩散蒸馏方法提供了坚实的理论基础。

Classification reason: 该论文研究扩散模型的蒸馏（distillation）与少步采样（few-step sampling），属于生成模型加速与高效化的核心主题，因此映射到 canonical subtopic "生成模型加速与高效化" (efficient_generation)。

Importance reason: Directly addresses model efficiency via diffusion distillation, a priority subtopic, with strong theoretical grounding and practical gains. Signals: diffusion distillation, flow map stability, non-uniform time grid, 51.9% MSE reduction, ReLU-ReQU approximation
...
### [65] [Follow-Your-Preference++: Rethinking Preference Alignment for Image Inpainting](https://arxiv.org/abs/2606.03216)
...
TL;DR: 本文系统性地研究了图像修复中的偏好对齐问题，基于DPO框架，使用9种公开奖励模型构建偏好数据，发现奖励模型存在亮度、构图、色彩等偏差，提出简单有效的奖励模型集成方法缓解奖励黑客问题，在不修改架构或引入额外数据的情况下显著超越先前SOTA。


<details>
  <summary>Details</summary>
Motivation: 图像修复任务中，现有方法在生成质量上仍有不足，偏好对齐（preference alignment）作为一种提升生成质量的手段尚未在图像修复领域被系统研究。作者希望从第一性原理出发，重新审视该问题的核心挑战，而非简单提出新方法。

Method: 采用DPO（Direct Preference Optimization）框架，利用9种公开奖励模型构建偏好训练数据（选择/拒绝对）。在两种不同架构和生成机制的基线修复模型上进行实验。提出奖励模型集成策略（ensemble of reward models）来缓解单一奖励模型的偏差（亮度、构图、色彩偏好等），并进一步提出校准集成方法提升鲁棒性。

Result: 在标准指标、大视觉语言模型评估和人类评估上均显著超越先前SOTA。实验表明：(1)大多数奖励模型可为偏好数据构建提供有效信号；(2)偏好数据在候选和样本缩放下呈现一致趋势；(3)奖励模型存在明显偏差，易导致奖励黑客；(4)简单集成可缓解偏差并提升泛化性；(5)偏好对齐可迁移至物体移除任务。

Conclusion: 偏好对齐是提升图像修复质量的有效途径，奖励模型集成是缓解偏差和奖励黑客的简单有效策略。该方法无需修改模型架构或引入额外数据集，具有良好的泛化性和可迁移性。

Classification reason: 该论文研究图像修复中的偏好对齐（preference alignment），使用DPO框架和奖励模型构建偏好数据，属于生成模型对齐与奖励的范畴，与[CANONICAL] subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Focuses on image inpainting preference alignment, which is not a top priority subtopic and lacks direct connection to continuous multimodal models or video generation. Signals: Image inpainting, preference alignment, DPO, reward model ensemble, not top priority subtopic
...
### [66] [MemoGen: Can Past Experience Improve Future Text-to-Image Generation?](https://arxiv.org/abs/2606.03243)
...
TL;DR: MemoGen 是一个无需训练的文生图框架，通过智能体演化层显式推理视觉需求、检索外部知识、评估生成结果，并将成功策略与失败教训存储为可复用的经验记忆，使生成模型在测试时能够自我进化，经过两轮演化后超越 GPT-Image-1 等强商业模型。


<details>
  <summary>Details</summary>
Motivation: 现有文生图模型在需要隐式视觉约束、关系推理或外部知识的提示下仍不可靠。检索增强和智能体方法虽能获取外部知识，但将每次生成视为孤立事件，未能系统性地保留过去的成功或失败经验以供未来使用。作者希望探索文生图系统能否在不更新底层生成器的情况下，从自身的生成经验中持续改进。

Method: MemoGen 是一个训练无关的框架，为现有图像生成器添加智能体演化层。对每个任务，它显式推断视觉需求，必要时检索外部证据和参考，将其转化为可执行的生成约束，评估生成结果，并将任务理解、参考选择、视觉反馈、成功策略和失败教训存储为可复用的经验记忆。在后续演化轮次中，智能体检索相关经验以改进相似生成任务，选择性修复之前失败案例同时保留成功案例，实现无需参数更新的测试时自我进化。

Result: 在知识密集和推理导向的基准（WISE、Mind-Bench）上，仅经过两轮演化，基于开源 Qwen-Image 骨干的 MemoGen 就超越了 Nano Banana Pro 和 GPT-Image-1 等强商业系统。

Conclusion: 显式的经验记忆可以作为可靠的持续学习信号，显著提升文生图模型在复杂推理和知识密集任务上的表现，且无需更新底层生成模型参数。

Classification reason: 该论文提出 MemoGen，通过经验记忆和智能体演化来提升文生图模型在知识密集和推理导向任务上的可靠性，属于生成模型的可控性与对齐优化范畴，与 canonical subtopic "生成模型对齐与奖励"（generation_alignment）高度匹配。

Importance reason: Proposes a novel training-free agentic memory framework for text-to-image generation, aligning with user's interest in controllable generation and efficient inference. Signals: training-free, agentic evolution, experience memory, text-to-image generation, test-time self-evolution
...
### [67] [Are we really tilting? The mechanics of reward guidance in flow and diffusion models](https://arxiv.org/abs/2606.02884)
...
Importance: 高优先级 (80.81) - 未入选精读

TL;DR: 本文从理论上揭示了扩散/流模型中奖励引导（reward guidance）导致reward hacking的根本原因——有限粒子Doob h函数的插件估计近似，并在高斯和高斯混合目标上解析了两种失效模式（模态内偏差和模态选择失败），提出了无需额外计算成本的闭式奖励阻尼调度方案，在FLUX.1文生图上验证了理论的有效性。


<details>
  <summary>Details</summary>
Motivation: 奖励引导算法在推理时引导生成过程朝向奖励倾斜分布，但容易产生reward hacking（过度优化奖励而牺牲保真度）。先前工作将其归因于神经奖励函数的复杂性或扩散训练的隐式偏差，但其根本原因尚不明确。本文旨在从理论上揭示reward hacking的起源。

Method: 论文在高斯和高斯混合目标+二次奖励的最简单非平凡设定下，对Doob h函数的有限粒子插件估计进行闭式分析，分离出两种失效模式：模态内偏差（每个高斯模态内过度优化）和模态选择失败（无法选择高奖励模态）。提出闭式奖励阻尼调度（reward damping schedule）来修正模态内偏差，并阐明best-of-n采样在补偿模态选择失败中的作用。

Result: 在高斯混合目标、2D棋盘分布和FLUX.1文生图实验中，理论分析得到验证：奖励阻尼调度有效修正了模态内偏差，best-of-n采样弥补了模态选择失败，整体提升了奖励引导生成的质量与保真度。

Conclusion: Reward hacking的根本原因在于有限粒子插件估计Doob h函数这一近似，而非奖励函数本身的复杂性。提出的闭式奖励阻尼调度无需额外计算即可修正模态内偏差，为实际部署奖励引导的扩散/流模型提供了理论指导。

Classification reason: 该论文研究扩散/流模型中的奖励引导（reward guidance）机制，分析reward hacking的根源并提出奖励阻尼调度（reward damping schedule）来修正偏差。这属于生成模型对齐与奖励（generation_alignment）范畴，该[CANONICAL] subtopic涵盖奖励建模、guidance与可控性，与论文主题高度匹配。

Importance reason: Provides fundamental theoretical insight into reward hacking in flow/diffusion models, directly relevant to improving reward guidance for multimodal generation alignment. Signals: theoretical analysis of reward hacking, Doob h-function approximation, closed-form reward damping, validated on FLUX.1, addresses multimodal generation alignment
...
### [68] [Any2Poster: Any-Source Poster Generation Across Modalities and Domains](https://arxiv.org/abs/2606.02915)
...
TL;DR: 本文提出 Any2Poster Bench，一个覆盖8种输入模态和5个内容领域的任意源海报生成评测基准，并设计了 Any2Poster Agent 作为基线系统，在事实保留和视觉质量上显著优于先前方法。


<details>
  <summary>Details</summary>
Motivation: 现有海报生成评测局限于论文输入、窄领域或表面视觉相似度，缺乏跨模态、跨领域的标准化评估方法，难以衡量自动海报生成系统的真实进展。

Method: 构建 Any2Poster Bench 基准，包含8种输入模态（PDF、URL、PPTX、DOCX、Markdown、LaTeX、notebooks、视频）和5个内容领域，每对源-海报配以基于问答的事实保留测试和 VLM 视觉质量评估。同时提出 Any2Poster Agent，一个端到端智能体系统，包含源解析、内容组织、布局规划、海报渲染和基于视觉反馈的迭代优化。

Result: Any2Poster Agent 在 Any2Poster Bench 上平均准确率87.25%（跨模态）和87.28%（跨领域）。在 PaperQuiz 风格评估中，整体准确率从 PosterAgent-4o 的51.06-51.33%提升至72.58%，密度增强分数从116-121提升至145.16。

Conclusion: Any2Poster Bench 为多模态、跨领域的海报生成提供了可复用的标准化评测资源，Any2Poster Agent 作为强基线展示了从异构源到高质量海报的可行路径。

Classification reason: 该论文提出了 Any2Poster Bench，一个面向任意源（PDF、URL、PPTX 等）的海报生成评测基准，并设计了基于问答的事实保留测试和 VLM 视觉质量评估。这属于生成模型的评测基准与数据集，与 canonical subtopic "generation_benchmark" 高度匹配。同时论文也涉及多模态理解（VLM 评估），因此也匹配 multimodal_understanding_generation 方向。

Importance reason: The paper focuses on a narrow poster generation benchmark and agent, which is not aligned with the user's priority subtopics on continuous language models, video/image generation, or world models. Signals: Poster generation benchmark, multimodal input parsing, VLM-based evaluation, not autoregressive or diffusion generation, no continuous token or world model focus
...
### [69] [The Road Ahead in Autonomous Driving: The KITScenes Multimodal Dataset](https://arxiv.org/abs/2606.02956)
...
TL;DR: 本文提出了 KITScenes Multimodal，一个面向自动驾驶的高保真多模态数据集，包含全局快门相机、超400m激光雷达、4D成像雷达和冗余GNSS/INS定位，以及最完整的HD地图（含3D交通元素和拓扑连接），并引入四个空间学习基准任务。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集在传感器保真度、地图完整性和地理多样性方面存在不足，缺乏同时具备高精度传感器、完整HD地图和多样化场景的公开数据集。

Method: 构建了一套完全同步的高保真传感器套件（高分辨率全局快门相机、超400m激光雷达、4D成像雷达、冗余GNSS/INS），制作了最完整的HD地图（所有驾驶相关交通元素以3D形式映射并具有拓扑连接），在具有不规则街道布局和混合交通模式的欧洲城市采集数据，并设计了四个基准任务。

Result: 提供了首个在公开数据集中对所有驾驶相关交通元素进行3D重投影精确映射并具备完整拓扑连接的数据集，通过开源软件自动驾驶测试验证了地图质量，四个基准任务为空间学习提供了评测平台。

Conclusion: KITScenes Multimodal 通过高保真传感器、最完整的HD地图和地理多样性，弥补了现有自动驾驶数据集的不足，为空间学习与具身AI研究提供了新的基准平台。

Classification reason: 该论文提出了一个新的自动驾驶多模态数据集 KITScenes Multimodal，包含高保真传感器、HD地图和四个评测基准（在线HD地图构建、长距离深度估计、新视角合成、端到端驾驶），属于多模态生成方向下的"生成模型评测与数据集"（generation_benchmark）规范主题。

Importance reason: Dataset paper with strong sensor and map contributions but low alignment with user's priority on continuous multimodal models and generation. Signals: High-fidelity multimodal dataset, HD map with 3D traffic elements, Four spatial learning benchmarks, European geographic diversity, Limited direct relevance to continuous language/video generation
...
### [70] [AnyAudio-Judge: A Dynamic Rubric-Based Benchmark and Evaluator for Audio Instruction Following](https://arxiv.org/abs/2606.03116)
*Haitao Li, Tian Tan, Yuguang Yang, Shan Yang, Xie Chen*

Main category: cs.SD, eess.AS, cs.AI
...
TL;DR: 本文提出 AnyAudio-Judge，一种基于动态评分规则的音频指令跟随评测基准与评估器。包含 7,920 个样本的基准和 105K 样本的训练语料，通过 SFT+GRPO 训练评估模型，在零样本对齐检测和提供可解释奖励信号方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有音频生成评估方法依赖通用大语言模型的整体评分，难以解耦复杂指令、缺乏可解释性，且无法捕捉细粒度属性不匹配问题。

Method: 提出动态评分规则（Dynamic Rubric）评估范式，将复杂音频描述自适应分解为多个独立可验证的二元评分项。构建 AnyAudio-Judge Bench 基准（7,920 样本，覆盖语音、声音、音乐和混合四种音频域，含精心构造的难负例）。构建 105K 样本的大规模语料并附带 Chain-of-Thought 推理过程，采用 SFT + GRPO 训练评估模型 AnyAudio-Judge。

Result: AnyAudio-Judge 在零样本对齐检测上显著优于现有基线方法，并能提供精确可解释的奖励信号，在下游强化学习音频生成中有效提升指令对齐度。

Conclusion: 基于动态评分规则的评估范式能有效提升音频生成对齐评估的准确性和可解释性，且评估器可作为奖励模型用于强化学习训练。

Classification reason: 该论文提出了音频生成的对齐评测基准 AnyAudio-Judge Bench 和自动化评估器，属于生成模型评测与数据集（generation_benchmark）这一规范主题。

Importance reason: Focuses on audio generation evaluation, not directly on user's priority subtopics like continuous multimodal models or video generation. Signals: Audio instruction following, dynamic rubric evaluation, SFT+GRPO training, interpretable reward signals, not in user's priority subtopics
...
### [71] [Inference-Time Scaling for Joint Audio-Video Generation](https://arxiv.org/abs/2606.03183)
*Jaemin Jung, Kyeongha Rho, Inkyu Shin, Joon Son Chung*

Main category: cs.MM, cs.SD, cs.CV, eess.AS
...
TL;DR: 本文首次系统研究了推理时扩展（Inference-Time Scaling）在联合音视频生成中的应用，提出多验证器框架和自适应奖励加权（ARW）算法，在无需额外训练的情况下显著提升语义对齐、感知质量和音视频同步。


<details>
  <summary>Details</summary>
Motivation: 现有联合音视频生成模型通常需要大量训练资源来提升保真度，而推理时扩展（ITS）作为无需训练的方法在单模态领域已展现出潜力，但将其扩展到多模态领域面临平衡多个异构目标的挑战。本文旨在探索ITS在联合音视频生成中的有效性。

Method: 提出多验证器框架（multi-verifier framework）以克服单目标引导的局限性（如非对称性能权衡和验证器攻击问题）；通过系统分析确定最优多验证器组合；提出自适应奖励加权（ARW）算法，将奖励聚合视为在线优化问题，利用可学习参数校准奖励方差，无需先验知识即可实现鲁棒的多目标选择。

Result: 在VGGSound和JavisBench-mini基准上的实验表明，所提框架显著提升了生成结果的语义对齐、感知质量和音视频同步性能。

Conclusion: 推理时扩展可以有效应用于联合音视频生成，多验证器框架和自适应奖励加权是实现多模态ITS的关键技术，为训练-free的多模态生成质量提升提供了新范式。

Classification reason: 该论文研究联合音视频生成（joint audio-video generation），属于多模态生成方向。现有 canonical subtopic "音频与多模态联合生成"（audio_av_generation）明确涵盖音视频联合生成，因此直接复用该 subtopic。

Importance reason: First systematic study of inference-time scaling for joint audio-video generation, with a novel multi-verifier and adaptive reward weighting method. Signals: inference-time scaling, joint audio-video generation, multi-verifier, adaptive reward weighting, training-free improvement
...
### [72] [SketchSong: Hierarchical Song Generation with Sketch Planning and Fine-Grained Multi-Track Modeling](https://arxiv.org/abs/2606.03169)
*Xiaoyue Duan, Nanxing Hu, Yutang Feng, Xudong Yan, Jiatao Chen, Jinchao Zhang, Jie Zhou*

Main category: cs.SD, cs.MM, cs.LG
...
TL;DR: SketchSong 是一个分层歌曲生成框架，通过歌曲级草图规划（粗到细的时间建模）和细粒度多轨建模（人声、贝斯、鼓、其他乐器），解决了现有歌曲生成中编曲不连贯和音轨建模粗糙的问题，在客观指标和人工听感上均优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有歌曲生成系统虽然能合成逼真的音频，但存在两个问题：1）缺乏显式的歌曲级编曲规划，导致编曲不连贯（如段落过渡弱、动态变化有限）；2）对不同音乐部分的建模过于粗糙，忽略了它们各自的角色和交互，限制了编曲丰富度。

Method: SketchSong 采用分层生成框架：1）时间维度上，先预测从压缩音频表示中提取的高层草图 token 序列（歌曲级编曲规划），再基于这些草图生成细粒度音频 token（粗到细）；2）音轨维度上，显式建模四个音轨（人声、贝斯、鼓、其他乐器），捕捉不同音乐部分的角色和交互。

Result: 在歌曲生成基准上，SketchSong 在客观指标和人工听感测试中均一致优于基线。尽管没有使用额外的偏好优化后训练（如歌词和文本提示对齐），SketchSong 仍取得了与经过后训练的强开源系统相竞争的结果。

Conclusion: SketchSong 的分层草图规划与细粒度多轨建模设计有效提升了歌曲生成的编曲连贯性和音轨丰富度，验证了显式编曲规划和多轨建模在歌曲生成中的有效性。

Classification reason: 该论文提出分层歌曲生成框架SketchSong，涉及多轨音乐（人声、贝斯、鼓、其他乐器）的生成，属于音频生成范畴，与[CANONICAL] subtopic "音频与多模态联合生成"（audio_av_generation）高度匹配。

Importance reason: The paper focuses on song generation, which is not a priority subtopic and lacks strong alignment with the user's configured direction weights and boost keywords. Signals: song generation, hierarchical generation, sketch planning, multi-track modeling, audio generation
...
### [73] [Foley-Omni: A Unified Multimodal Generation Model from Task-Level Audio Synthesis to Complete Video Soundtrack Generation](https://arxiv.org/abs/2606.03672)
...
TL;DR: Foley-Omni 是一个统一多模态音频生成模型，将孤立的任务级音频合成（语音、音效、音乐）扩展到完整的视频音轨联合生成，通过共享潜空间生成过程实现多组件音频的一致生成。同时提出了 V2ST-Bench 基准用于评估完整视频音轨生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有统一音频生成模型大多只支持孤立的任务级合成（如单独生成语音、音效或音乐），但真实视频制作需要为同一视频联合生成多个音频组件（语音、音效、音乐）并保持一致性。现有方法缺乏对这种完整视频音轨生成任务的支持。

Method: Foley-Omni 采用共享潜空间生成架构，在统一的潜空间中对语音、音效和音乐进行联合建模与生成。同时开发了一套音视频数据清洗流水线，并构建了 V2ST-Bench 基准数据集，用于评估完整视频音轨生成任务。

Result: 实验表明，Foley-Omni 在单个合成任务上达到了与专家系统相当的性能，同时在混合音轨生成任务中显著提升了语音可懂度、音视频一致性和感知质量。

Conclusion: Foley-Omni 证明了从孤立任务级音频合成扩展到完整视频音轨联合生成的可行性，为统一多模态音频生成提供了新范式，且 V2ST-Bench 为后续研究提供了标准化评测平台。

Classification reason: 该论文提出统一多模态音频生成模型Foley-Omni，联合建模语音、音效和音乐以完成完整视频音轨生成，属于音频与多模态联合生成方向。匹配到[CANONICAL] subtopic "audio_av_generation"（音频与多模态联合生成），该子主题涵盖音视频联合生成与跨模态条件音频生成，与论文核心任务高度一致。

Importance reason: Focuses on audio generation, not the user's priority subtopics like continuous language models or video generation. Signals: unified multimodal generation, audio soundtrack generation, task-level to complete video, V2ST-Bench benchmark, shared latent generation

Abstract: Recent unified audio generation models can support diverse tasks across speech, sound effects, and music, but most of them still focus on isolated task-level synthesis. However, real video production often requires multiple components of a complete audio track to be generated jointly and consistently for the same video. We present Foley-Omni, a unified multimodal audio generation model that extends isolated task-level synthesis to complete video soundtrack generation by jointly modeling speech, sound effects, and music within a shared latent generation process. To support training and reproducible evaluation, we develop an audiovisual data curation pipeline and introduce V2ST-Bench, a benchmark for holistic video soundtrack generation evaluation. Experiments show that Foley-Omni achieves competitive performance with expert systems on individual synthesis tasks, while improving speech intelligibility, audiovisual consistency and perceptual quality for mixed soundtrack generation.

</details>


### [74] [SegTune: Structured and Fine-Grained Control for Song Generation](https://arxiv.org/abs/2606.02638)
...
TL;DR: SegTune 是一个基于 Diffusion Transformer 的歌曲生成框架，通过分段提示词（segment prompts）实现结构化、细粒度的歌曲控制，结合 LLM 驱动的时长预测器实现歌词-音乐精确对齐，在音乐性和可控性上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有歌曲生成系统虽然能从歌词和全局文本提示生成高质量音频，但无法建模歌曲中随时间变化的属性（如节奏、乐器、情绪等），缺乏对音乐结构和动态的细粒度控制能力。

Method: 提出 SegTune 框架，基于 Diffusion Transformer，允许用户或 LLM 为歌曲的不同段落指定局部音乐描述（segment prompts），这些提示按时间窗口广播到对应区域，同时全局提示保持风格一致性。引入 LLM 驱动的时长预测器，以 LyRiCs 格式自回归生成句子级时间戳，实现精确的歌词-音乐对齐。构建大规模高质量歌曲数据管道，并提出新的评估指标用于段落对齐和人声一致性评估。

Result: 实验表明 SegTune 在音乐性和可控性方面均优于现有基线方法，能够有效实现结构化、细粒度的歌曲生成控制。

Conclusion: SegTune 通过分段提示机制和 LLM 辅助的时长预测，显著提升了歌曲生成的可控性和音乐结构建模能力，为细粒度可控歌曲生成提供了有效方案。

Classification reason: 该论文研究歌曲生成（song generation），属于音频生成范畴，与 [CANONICAL] 音频与多模态联合生成（audio_av_generation）高度匹配。

Importance reason: The paper focuses on song generation, which is not a priority subtopic, and lacks strong alignment with the user's key boost keywords. Signals: Diffusion Transformer, song generation, fine-grained control, LLM-based duration predictor, not in priority subtopics
...
### [75] [EntangleCodec: A Unified Discrete Audio Tokenizer via Semantic-Acoustic Entanglement](https://arxiv.org/abs/2606.02739)
...
TL;DR: EntangleCodec 提出一种统一离散音频分词器，通过语义-声学纠缠学习和对齐丰富字幕，在单一紧凑 token 流中同时捕获语言内容、说话人身份、情感、韵律和声学场景，并利用流匹配扩散解码器实现高质量重建。基于该分词器的音频语言模型在 0.6B 参数下超越 13B+ 的连续表示 LLM，8B 模型在 MMAR 上达到 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有音频分词器难以同时支持理解与生成：面向重建的编解码器保真度高但缺乏丰富语义，而语义感知分词器通常依赖分离的语义和声学流，引入冗余或不对齐。需要一种统一的离散音频分词器，在单一 token 流中同时捕获语义和声学信息。

Method: EntangleCodec 在量化前学习与字幕对齐的语义-声学联合表征。通过将音频与丰富字幕（而非 ASR 转录）对齐，捕获语言内容、说话人身份、情感、韵律和声学场景。使用流匹配扩散解码器实现高质量重建，支持语音、音乐和通用音频。

Result: 重建质量与专用编解码器相当；在音频理解任务上超越所有基于编解码器的基线，MMAR 上提升高达 +7.4%；支持统一的 TTS 和 TTA 生成；基于 EntangleCodec 的 ALM 在 0.6B 参数下以 22 倍少的参数超越 13B+ 的连续表示 LLM，8B 模型在 MMAR 上达到 SOTA。

Conclusion: EntangleCodec 证明了表征质量与模型规模在音频语言建模中同等重要，统一离散音频分词器可以同时支持高质量的理解与生成，具有良好的扩展性。

Classification reason: 该论文提出统一离散音频分词器，支持音频理解与生成（TTS/TTA），属于音频生成与多模态联合生成方向，映射到 [CANONICAL] 的 audio_av_generation（音频与多模态联合生成）。同时涉及理解与生成统一，也匹配 multimodal_understanding_generation 方向。

Importance reason: Strong audio tokenizer with semantic-acoustic entanglement and flow-matching decoder, but user's priority is continuous multimodal models and video generation, not audio. Signals: unified discrete audio tokenizer, semantic-acoustic entanglement, flow-matching diffusion decoder, 0.6B model surpasses 13B+ continuous LLMs, MMAR SOTA at 8B
...
TL;DR: 本文系统研究了多语言多模态大语言模型（MLLM）的对抗鲁棒性与安全对齐问题，覆盖12种语言。发现对抗图像具有跨语言迁移性；多语言安全性差异源于语言理解能力而非真实对齐，低资源语言因视觉理解失败而呈现"安全假象"（safety-by-failure）；而训练阶段即融入多语言能力的模型（如Qwen3-VL）能实现真正的跨语言安全对齐。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM鲁棒性研究主要聚焦英语任务，多语言场景下的对抗鲁棒性和安全性尚未被探索。本文旨在填补这一空白，系统研究多语言MLLM在面对对抗攻击时的鲁棒性表现以及多语言安全对齐的真实状况。

Method: 选取12种不同语言，对开源MLLM（包括通过指令微调获得多语言能力的模型以及训练阶段即融入多语言能力的模型如Qwen3-VL）进行梯度攻击实验，评估对抗图像的跨语言迁移性；同时分析文本和图像（typographic content）两种模态下有害指令的多语言安全对齐表现。

Result: 对抗图像具有强跨语言迁移性；低资源语言因视觉编码器无法解析非英语文字而呈现"安全假象"（safety-by-failure）；Qwen3-VL等训练阶段即融入多语言能力的模型展现出真正的跨语言安全对齐，而仅通过指令微调获得多语言能力的模型存在表面理解导致的虚假安全。

Conclusion: 多语言MLLM的安全性差异很大程度上是语言理解与视觉接地能力差异的副产品，而非真正的安全对齐差异。浅层多语言适应（如翻译指令数据微调）会产生虚假安全，而训练阶段深度融入多语言能力才能实现真正的跨语言安全对齐。

Classification reason: 该论文系统研究了多模态大语言模型在多语言场景下的对抗鲁棒性与安全对齐问题，涉及对抗攻击（梯度攻击）、跨语言迁移攻击、安全对齐失效等现象，属于多模态安全与攻防范畴，因此映射到 canonical subtopic "多模态安全与攻防"。

Importance reason: Focuses on adversarial robustness and safety alignment in multilingual MLLMs, which is a security subtopic not aligned with user's priority on continuous multimodal models and generation. Signals: Multilingual MLLM robustness, cross-lingual adversarial transfer, safety-by-failure phenomenon, Qwen3-VL genuine alignment, not matching priority subtopics
...
### [77] [BYORn: Bootstrap Your Own Responses to Defend Large Vision-Language Models Against Backdoor Attacks](https://arxiv.org/abs/2606.02947)
...
TL;DR: BYORn 是一种针对大型视觉语言模型后门攻击的鲁棒微调框架，通过检测并替换被投毒的目标响应来打破触发器与目标输出之间的关联，在保持干净任务性能的同时显著提升对后门攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 监督微调是适配视觉语言模型到下游任务的主要方式，但该范式极易受到后门攻击，且现有防御方法在开放生成场景下效果不佳。作者观察到被投毒的目标响应通常与图像-文本输入在语义上不合理，因此希望利用这一特性设计防御方法。

Method: BYORn 框架在微调过程中检测语义不合理的（被投毒）目标响应，并动态地用模型自身生成的替代响应替换它们，从而打破触发器与目标输出之间的相关性。该方法的优化目标梯度等价于干净数据分布上经验估计的总体风险上界的梯度。

Result: 实验表明，BYORn 在保持干净任务性能的同时，持续提升了模型对后门攻击的鲁棒性，建立了泛化性能与攻击成功率之间新的权衡前沿。此外，BYORn 对专门设计来绕过该防御的自适应攻击仍然有效。

Conclusion: BYORn 是一种有效的后门攻击防御框架，通过自生成响应替换策略在开放生成场景下实现了鲁棒的微调，为视觉语言模型的安全性提供了新的保障。

Classification reason: 该论文研究针对大型视觉语言模型的后门攻击防御方法，属于多模态安全与攻防范畴，与 canonical subtopic "多模态安全与攻防" 高度匹配。

Importance reason: Focuses on backdoor defense for vision-language models, which is not a priority subtopic and lacks alignment with user's key interests in generation, world models, or continuous representations. Signals: Backdoor defense, vision-language model safety, robust fine-tuning, open-ended generation, adaptive attacks
...
### [78] [When Helping Hurts and How to Fix It: Multi-Agent Debate for Data Cleaning](https://arxiv.org/abs/2606.02866)
...
TL;DR: 本文系统研究了多智能体辩论在数据清洗中的双面效应：辩论会降低生成质量（-1.6到-15.5pp），但能提升错误检测（+27.4pp F1）。作者提出辩论收益条件公式，并通过实验证明对抗性分离（独立Critic+代码执行+证据门控生成）是辩论生效的关键，首次在生成任务上显著超越单智能体（+5.3pp）。


<details>
  <summary>Details</summary>
Motivation: 多智能体辩论在LLM应用中日益流行，但其在数据清洗等生成任务中的效果存在矛盾——有时帮助、有时伤害。作者希望系统性地理解辩论何时有益、何时有害，并找到使辩论可靠生效的配置条件。

Method: 在3个基准、4个模型家族、6000+任务-条件对上系统评估多智能体辩论效果。提出辩论收益条件公式：当挽救错误输出的概率（Critic验证几率×可修复性）超过破坏正确输出的概率时，辩论有益。通过析因实验验证对抗性分离的必要性：使用独立Critic、代码执行接地（code-execution grounding）和证据门控生成（evidence-gated generation）。

Result: 辩论在所有4个模型上降低生成质量（-1.6到-15.5pp），但提升错误检测F1达+27.4pp（d=1.0）。自验证（self-verification）无效，而独立Critic+代码执行+证据门控生成的配置首次在生成任务上显著超越单智能体（+5.3pp, p<0.05）。该条件正确预测所有9种任务类型，在19个已发表比较中零假阳性泛化。

Conclusion: 多智能体辩论在数据清洗中是一把双刃剑：提升检测但降低生成。关键在于对抗性分离——Critic必须独立于Generator，并具备代码执行等接地能力，同时Generator应仅采纳有证据支持的Critic反馈。这为设计有效的多智能体协作系统提供了理论指导和实用配置。

Classification reason: 该论文研究多智能体辩论在数据清洗中的效果，发现批评者（Critic）的幻觉反馈会导致生成者（Generator）接受错误信息（Critique-Induced Confusion, CIC），这本质上是一种多模态/语言模型的安全与可靠性问题，与"多模态安全与攻防"（multimodal_safety_attack）中对抗攻击、防御、安全评测的主题高度相关。论文提出的证据门控生成（evidence-gated generation）也是一种防御机制。

Importance reason: Focuses on multi-agent debate for data cleaning, not on the user's priority subtopics like continuous language models or video generation. Signals: Multi-agent debate, Data cleaning, Critique-induced confusion, Adversarial separation, Error detection improvement
...
### [79] [TRAP: Hijacking VLA CoT-Reasoning via Adversarial Patches](https://arxiv.org/abs/2603.23117)
*Zhengxian Huang, Wenjun Zhu, Haoxuan Qiu, Xiaoyu Ji, Wenyuan Xu*

Main category: cs.CR, cs.RO, cs.AI
...
TL;DR: 本文提出TRAP，首个针对VLA（视觉-语言-动作）模型中CoT推理的定向行为劫持对抗攻击方法。通过放置对抗性补丁（如桌布），在不修改用户指令的情况下，操纵模型的中间CoT推理和最终动作，使机器人执行攻击者定义的目标行为（如将刀递给人类而非苹果）。实验在三种代表性推理VLA模型上验证了有效性，并在真实场景中通过打印补丁实现了攻击。


<details>
  <summary>Details</summary>
Motivation: VLA模型通过集成CoT推理在机器人操作中展现出强大的泛化能力和可解释性，但CoT推理机制的安全性尚未被探索。作者发现CoT对动作生成有强控制力，即使CoT与输入指令语义不一致时仍能主导动作输出，这为攻击提供了新的向量。因此需要研究如何利用对抗性补丁劫持CoT推理以实现定向行为操控。

Method: TRAP攻击方法：1）针对VLA模型的推理到动作通路（reasoning-to-action pathway），设计可打印的对抗性补丁（如桌布图案）；2）通过优化补丁图案，使模型在推理阶段生成攻击者指定的CoT推理内容，进而驱动下游动作执行目标行为；3）攻击不修改用户指令，仅通过环境中的物理补丁实现。在三种不同CoT推理机制的VLA模型上进行评估。

Result: 在三种代表性推理VLA模型上的广泛实验表明，TRAP能够有效劫持模型的CoT推理和下游动作，实现攻击者定义的定向行为。在真实场景中，将补丁打印在纸上并放置在桌面上即可成功实施攻击，证明了该攻击的物理可实现性。

Conclusion: 本文揭示了VLA模型中CoT推理机制存在严重的安全漏洞，对抗性补丁可以劫持CoT推理并操控机器人行为。研究结果强调了在VLA系统中保护CoT推理的紧迫性，为多模态安全领域提出了新的挑战和方向。

Classification reason: 该论文研究针对VLA（视觉-语言-动作）模型中CoT推理的对抗性补丁攻击，属于多模态系统的安全与攻防范畴，与[CANONICAL] subtopic "多模态安全与攻防"（multimodal_safety_attack）高度匹配。同时涉及具身智能/机器人领域，因此也匹配world_model方向下的具身世界模型。

Importance reason: Novel security analysis of VLA CoT reasoning, but outside user's core priority subtopics. Signals: VLA security, adversarial patch, CoT reasoning hijacking, robotics manipulation, real-world attack
...
### [80] [HARVE: Hacking-Aware Reward-Head Vector Editing for Robust Reward Models](https://arxiv.org/abs/2606.03131)
...
TL;DR: 本文提出RewardHackBench（包含13种奖励黑客模式的评测基准）和HARVE（一种免训练的奖励模型头部向量编辑方法），通过识别并移除奖励头向量中与黑客子空间对齐的分量来提升奖励模型的鲁棒性，无需梯度更新或微调。


<details>
  <summary>Details</summary>
Motivation: 奖励模型是LLM对齐的核心，但容易受到奖励黑客攻击（reward hacking）。现有研究缺乏系统性的奖励黑客模式分类和鲁棒性评测，也缺少轻量级、免训练的防御方法。

Method: 1) 构建RewardHackBench，包含13种奖励黑客模式，覆盖高风险领域和通用场景。2) 提出HARVE方法：从残差流方向中识别与特定黑客子类别相关的多维黑客子空间，然后从奖励头向量中移除与该子空间对齐的分量，从而降低奖励头对黑客相关特征的敏感性。该方法仅需少量对比性gold-hacked样本，无需梯度更新或微调。

Result: 在8个奖励模型上的实验表明，HARVE提升了黑客鲁棒性，优于微调基线，且保持了奖励模型的通用能力。进一步分析表明，奖励黑客更适合被建模为多维残差空间结构而非孤立的表面线索。

Conclusion: HARVE是一种有效的免训练奖励模型鲁棒性提升方法，揭示了奖励黑客的多维残差空间结构特性，为奖励模型安全对齐提供了新思路。

Classification reason: 该论文研究奖励模型的鲁棒性与奖励黑客攻击/防御，属于多模态安全与攻防（multimodal_safety_attack）范畴。虽然论文主要针对LLM奖励模型，但奖励模型是多模态对齐（RLHF/DPO）的核心组件，且论文提出的RewardHackBench包含高风险领域和通用场景的奖励黑客模式，与安全攻防主题高度吻合。

Importance reason: Focuses on reward model robustness for LLM alignment, which is outside the user's priority subtopics of continuous multimodal models, video/image generation, and world models. Signals: reward hacking, reward model robustness, training-free editing, LLM alignment, not in priority subtopics
...
### [81] [Investigating Adversarial Robustness of Multi-modal Large Language Models](https://arxiv.org/abs/2606.03713)
*Hashmat Shadab Malik, Muzammal Naseer, Salman Khan*
...
TL;DR: 本文系统研究了多模态大语言模型（MLLM）的对抗鲁棒性，提出诊断性CLIP对齐协议预测鲁棒视觉编码器的迁移效果，发现大规模多模态对抗预训练是关键因素。通过端到端多模态对抗训练，在强攻击下分别提升CIDEr 28分和VQA准确率11.7%。同时验证了测试时随机变换作为黑盒防御的有效性，以及鲁棒模型能显著减少白盒视觉越狱攻击下的有毒生成。


<details>
  <summary>Details</summary>
Motivation: MLLM通过视觉编码器（如CLIP）引入视觉输入，大幅扩大了攻击面，使其易受视觉对抗扰动攻击。现有防御方法通常通过强制对齐CLIP原始嵌入空间来保持与预训练MLLM的兼容性，但这种约束从根本上限制了可实现的鲁棒性。本文旨在系统研究MLLM的对抗鲁棒性，探索更有效的防御策略。

Method: 1) 提出诊断性CLIP对齐协议，在完整MLLM训练前预测哪些鲁棒视觉编码器能有效迁移到多模态场景；2) 将筛选出的鲁棒编码器通过端到端多模态对抗训练集成到MLLM中；3) 探索测试时轻量级视觉随机变换作为黑盒防御；4) 评估鲁棒模型在白盒视觉越狱攻击下的有毒生成缓解效果。

Result: 端到端多模态对抗训练相比受约束的即插即用基线，在强攻击下平均提升CIDEr 28分（图像描述）和VQA准确率11.7%。从鲁棒骨干网络进行端到端对抗训练额外提升CIDEr 1.9分和VQA准确率4.3%。测试时随机变换可将非鲁棒MLLM的对抗性能从接近零提升到与鲁棒模型相当的水平。鲁棒模型显著减少了白盒视觉越狱攻击下的有毒生成。

Conclusion: 大规模多模态对抗预训练（而非仅单模态规模）是鲁棒性有效迁移到多模态场景的关键因素。鲁棒的视觉表征是MLLM对抗训练的必要前提，直接对标准非鲁棒MLLM进行对抗训练会同时损害干净和对抗性能。测试时随机变换是一种有效的轻量级黑盒防御方案。

Classification reason: 该论文系统研究多模态大语言模型（MLLM）的对抗鲁棒性，包括视觉对抗攻击、对抗训练防御、测试时防御以及越狱攻击缓解，属于多模态安全与攻防范畴，因此复用 [CANONICAL] subtopic "多模态安全与攻防"。

Importance reason: Strong systematic study of MLLM adversarial robustness, but outside user's priority subtopics and negative keywords do not apply. Signals: MLLM adversarial robustness, diagnostic CLIP-alignment protocol, end-to-end multimodal adversarial training, robust visual encoders, test-time stochastic defenses
...
TL;DR: 本文发现CLIP视觉表征中存在一个被忽视的噪声区域转换现象：弱噪声下对抗样本呈现"假稳定性"，而强噪声下对抗表征比干净表征更不稳定。基于此提出一种免训练、即插即用的漂移门控机制，仅在检测到对抗性不稳定时触发测试时防御，在13个数据集上显著改善了干净-鲁棒准确率权衡。


<details>
  <summary>Details</summary>
Motivation: 现有测试时防御方法（如噪声视图聚合、高斯噪声锚点插值等）虽然提升了CLIP的对抗鲁棒性，但往往以牺牲干净样本准确率为代价，导致不利的干净-鲁棒权衡。作者希望找到一种方法，在不影响干净准确率的前提下提升对抗鲁棒性。

Method: 首先系统分析CLIP表征空间中不同噪声强度下对抗样本与干净样本的行为差异，发现弱噪声下对抗样本异常稳定（假稳定性），而强噪声下对抗样本变得比干净样本更不稳定。基于这一发现，提出一种训练无关的漂移门控机制（Drift-Gated Mechanism），利用高噪声下的特征漂移作为轻量级门控信号，仅在检测到类似对抗的不稳定性时才触发现有测试时防御（如counterattack或noise-anchoring），否则保持原始预测。

Result: 在13个数据集上，该方法一致改善了干净-鲁棒权衡。在8个细粒度数据集上，平均干净+对抗准确率从65.7%提升至71.4%（counterattack防御）和从68.4%提升至73.2%（noise-anchoring防御）；在ImageNet及4个分布偏移变体上，从56.1%提升至66.2%和从62.1%提升至67.6%。

Conclusion: 本文揭示了CLIP对抗表征在强噪声下的不稳定性现象，并利用这一现象设计了轻量级门控机制，有效改善了测试时防御的干净-鲁棒权衡。该方法无需训练、即插即用，可广泛应用于多种现有防御策略。

Classification reason: 该论文研究视觉语言模型（CLIP）的测试时对抗防御方法，属于多模态安全与攻防范畴，与现有 canonical subtopic "多模态安全与攻防" 高度匹配。

Importance reason: Focuses on adversarial defense for VLMs, not aligned with user's priority on generative models and continuous representations. Signals: Test-time defense, CLIP robustness, clean-robust trade-off, noise-regime transition, drift gating
...
### [83] [CORE: Conflict-Oriented Reasoning for General Multimodal Manipulation Detection](https://arxiv.org/abs/2606.03066)
...
TL;DR: CORE 是一个面向多模态虚假新闻检测的冲突导向推理框架，通过构建冲突归因语料库（CAC）并训练 MLLM 显式捕捉跨模态或与常识的语义/物理冲突，实现鲁棒且泛化的检测，在零样本和少样本场景下均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态虚假新闻检测方法严重依赖针对特定操纵类型的模型和大规模标注数据，泛化能力差，难以应对新兴操纵类型。作者观察到被操纵信息的本质在于其内在冲突（跨模态或与常识的不一致），因此希望赋予 MLLM 显式的冲突捕捉能力，实现通用且可泛化的检测。

Method: CORE 框架包含两个核心组件：1) 冲突归因语料库（CAC），包含细粒度的冲突因素和冲突来源标注；2) 基于 CAC 的冲突导向表示增强与推理训练，使 MLLM 学会显式识别和定位多模态信息中的冲突。

Result: 大量实验表明 CORE 在多种操纵类型上超越现有最先进方法，在零样本和少样本设置下均能有效泛化到未见过的操纵类型。

Conclusion: CORE 通过冲突导向推理范式有效提升了多模态虚假新闻检测的泛化能力，验证了"冲突"作为操纵信息本质特征的合理性，为通用多模态操纵检测提供了新思路。

Classification reason: 该论文研究多模态虚假新闻检测，属于多模态安全与防御方向，与 [CANONICAL] 多模态安全与攻防 子主题高度匹配。

Importance reason: The paper focuses on multimodal fake news detection, which is not a priority subtopic and lacks alignment with the user's key interests in generation, world models, or continuous representations. Signals: multimodal fake news detection, conflict-oriented reasoning, zero-shot generalization, MLLM fine-tuning, not in priority subtopics
...
### [84] [MultiTurnPSB: Evaluating Multi-Turn Jailbreak Attacks an dClassifier-Based Defenses for Medical AI Safety](https://arxiv.org/abs/2606.02630)
...
TL;DR: 本文提出 MultiTurnPSB，一个四轮对抗性越狱攻击数据集，扩展了 PatientSafetyBench。实验发现，在实时对抗攻击下，不安全响应率从第1轮的35%上升到第4轮的近80%；GPT-4.1-mini 和 Claude Sonnet 4.5 在单轮评估中无差异，但多轮下差距达19倍。轻量级输入侧分类器可将第4轮不安全响应降低52个百分点，但45%的误报率是部署的主要障碍。


<details>
  <summary>Details</summary>
Motivation: 现有医疗聊天机器人安全评估仅关注单轮提示，但真实用户会在被拒绝后继续追问、增加紧迫感或援引权威，导致多轮对话中安全防线崩溃。单轮评估无法捕捉这种退化轨迹，亟需多轮越狱攻击评测方法。

Method: 构建 MultiTurnPSB 四轮对抗性扩展数据集，设计固定模板、模板自适应和实时对抗三种攻击策略，评估 GPT-4.1-mini 和 Claude Sonnet 4.5 的安全表现，并测试轻量级输入侧分类器（基于 BERT 和 DistilBERT）的防御效果。

Result: 实时攻击下不安全响应率从第1轮35%升至第4轮近80%；GPT-4.1-mini 和 Claude Sonnet 4.5 在单轮评估中无显著差异，但第4轮差距达19倍。分类器可将第4轮不安全响应降低52个百分点，但误报率达45%。Claude Sonnet 在超过一半的后期轮次中拒绝生成对抗性消息。

Conclusion: 多轮越狱攻击揭示了单轮评估无法发现的重大安全差距；输入侧分类器虽能有效降低不安全响应，但高误报率限制了实际部署；安全训练可能泛化到攻击者角色，值得进一步研究。

Classification reason: 该论文研究多轮越狱攻击（jailbreak attacks）及基于分类器的防御方法，属于多模态安全与攻防范畴，因此映射到 [CANONICAL] subtopic "多模态安全与攻防" (multimodal_safety_attack)。

Importance reason: Focuses on medical AI safety jailbreaks, which is outside your configured priority subtopics and negative-keyword domains. Signals: Multi-turn jailbreak dataset, medical chatbot safety, classifier defense with high false alarm, safety training generalizing to attacker role
...
### [85] [Hallucinations as Orthogonal Noise: Inference-Time Manifold Alignment via Dynamic Contextual Orthogonalization](https://arxiv.org/abs/2606.03022)
...
TL;DR: 本文提出幻觉是残差流中与语义流形正交的噪声，基于此几何框架设计了动态上下文正交化（DCO）方法，在推理时通过正交分解和Z-score抑制来选择性衰减注意力头输出中的正交噪声成分，在Llama-3-8B/70B上实现了优于现有方法的上下文忠实性，同时保持了知识密集型任务上的性能。


<details>
  <summary>Details</summary>
Motivation: LLM幻觉问题严重阻碍了模型的可靠部署，现有方法往往在抑制幻觉和保留参数化知识之间存在权衡。作者希望从几何角度理解幻觉的本质，并提出一种计算高效的推理时干预方法，在不牺牲模型知识能力的前提下有效缓解幻觉。

Method: 基于线性表示假设，提出幻觉是注意力头输出中与上下文语义子空间正交的噪声成分。DCO方法利用输入残差流作为动态上下文锚点，对注意力头输出进行正交分解，将输出分解为与上下文对齐的成分和正交噪声成分；然后通过逐层Z-score抑制机制，基于统计分布选择性衰减异常的正交成分，从而在推理时动态调整模型表示。

Result: 在XSum、NQ-Swap、IFEval等基准上，DCO在上下文忠实性方面优于现有最先进的干预基线；在TriviaQA和TruthfulQA等知识密集型任务上，DCO有效缓解了幻觉抑制与参数知识保留之间的权衡，保持了高水平的性能。

Conclusion: 验证了幻觉作为正交噪声的几何解释，DCO作为一种计算高效的推理时干预方法，能够在不牺牲模型知识能力的前提下有效抑制幻觉，为LLM的可靠部署提供了新思路。

Classification reason: 该论文研究LLM幻觉问题，提出基于几何框架的推理时干预方法DCO来抑制幻觉。虽然论文主要针对文本LLM，但幻觉缓解是多模态理解与生成方向中多模态幻觉与可靠性（multimodal_hallucination）子主题的核心内容，该子主题涵盖幻觉缓解、事实性与可靠性提升，与本文高度相关。

Importance reason: Novel geometric framework for hallucination mitigation, but not aligned with user's priority subtopics on continuous models or generation. Signals: inference-time intervention, orthogonal noise hypothesis, attention head decomposition, Z-score suppression, contextual faithfulness
...
### [86] [Hallucination Is Linearly Decodable from Mid-Layer Hidden States in Quantized LLMs](https://arxiv.org/abs/2606.02628)
...
TL;DR: 本文研究量化LLM（4-bit NF4）隐藏状态中是否编码了线性可分的真实性信号。实验发现，在中间层（Llama/Mistral第13-18层，Qwen第19-25层）使用线性探针即可达到0.904-1.000 AUROC的幻觉检测性能，远超基于采样的检测方法（≤0.541 AUROC）。MLP探针相比线性探针提升不超过0.01 AUROC，表明真实性信号近似线性。首块注意力熵在知识驱动场景中提供互补信号（0.866-0.941 AUROC）。


<details>
  <summary>Details</summary>
Motivation: LLM幻觉是影响大模型可靠性的关键问题，现有检测方法（如基于采样的自一致性）计算成本高且效果有限。作者希望探究LLM内部隐藏状态是否编码了可检测的真实性信号，以及这种信号在什么网络深度最强、是否线性可分，从而为低成本、高效的幻觉检测提供理论基础。

Method: 在三个7B-8B指令微调模型（Llama-3.1-8B、Mistral-7B、Qwen2.5-7B）上加载4-bit NF4量化，在四个幻觉基准（TruthfulQA、HaluEval-QA、FEVER和合成数据集）上提取每层隐藏状态。比较四种检测方法：线性探针、MLP探针、INSIDE EigenScore、自一致性以及注意力熵。线性探针在单层中间隐藏状态上训练二分类器区分真实与幻觉输出。

Result: 线性探针在单层中间层上达到0.904-1.000 AUROC，而基于采样的检测方法在同一协议下不超过0.541 AUROC。MLP探针相比线性探针提升不超过0.01 AUROC。最佳探针层集中在模型特定区间：Llama/Mistral第13-18层（共32层），Qwen第19-25层（共28层）。首块注意力熵在HaluEval-QA上达到0.866-0.941 AUROC。

Conclusion: LLM隐藏状态中编码了近似线性的真实性信号，在中间层最强。线性探针是一种高效、低成本的幻觉检测方法，在量化模型上也能取得优异效果。采样方法表现差的原因是评估协议的结构性不匹配，而非方法本身的固有局限。注意力熵可作为零成本的补充信号。

Classification reason: 该论文研究LLM幻觉检测，属于多模态幻觉与可靠性（multimodal_hallucination）这一[CANONICAL]子主题的范畴。虽然论文主要针对纯文本LLM，但幻觉检测与可靠性是多模态理解与生成方向的核心子主题之一，且该子主题涵盖幻觉缓解与事实性提升，与本论文高度相关。

Importance reason: Focuses on LLM hallucination detection, not on the user's priority subtopics like continuous multimodal models or video generation. Signals: hallucination detection, linear probe, quantized LLM, mid-layer hidden states, low personal relevance
...
### [87] [P\textsuperscript{2}-DPO: Grounding Hallucination in Perceptual Processing via Calibration Direct Preference Optimization](https://arxiv.org/abs/2606.03376)
...
TL;DR: P²-DPO提出了一种新的训练范式，通过让模型自行生成并学习on-policy偏好对，针对感知瓶颈和视觉鲁棒性进行校准，从而缓解LVLM中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有DPO方法在缓解LVLM幻觉时存在三个问题：(1) 未针对注意力区域的感知瓶颈和图像退化的视觉鲁棒性不足；(2) 偏好对往往是视觉无关的（vision-agnostic）；(3) 偏好对本质上是off-policy的，限制了模型学习效果。

Method: 提出P²-DPO框架：(1) 构建on-policy偏好对，分别针对Focus-and-Enhance感知（增强注意力区域）和Visual Robustness（图像退化场景）；(2) 设计校准损失（Calibration Loss），精确对齐视觉信号与文本的因果生成过程。

Result: 在多个基准上，使用相当数量的训练数据和成本，P²-DPO优于依赖昂贵人工反馈的强基线。在注意力区域保真度（ARF）和图像退化场景评估中，验证了该方法在缓解感知瓶颈和提升视觉鲁棒性方面的有效性。

Conclusion: P²-DPO通过on-policy偏好对构建和校准损失，有效缓解了LVLM的幻觉问题，特别是在注意力区域感知和图像退化场景下表现优异，且无需昂贵的人工反馈。

Classification reason: 该论文聚焦于LVLM中的幻觉问题，提出基于DPO的校准方法缓解幻觉，属于多模态幻觉与可靠性这一[CANONICAL]子主题。

Importance reason: Addresses LVLM hallucination via on-policy DPO, but is not aligned with user's priority subtopics on continuous models, generation, or world models. Signals: LVLM hallucination, on-policy DPO, perceptual bottleneck, visual robustness, calibration loss
...
### [88] [OmniHalluc-L: Counterfactual Benchmarking and Modality-Perturbation Reliability Calibration for Long-Form Omni Hallucination](https://arxiv.org/abs/2606.03614)
...
TL;DR: 本文提出OmniHalluc-L基准和Modality-Perturbation Reliability Calibration (MPRC)方法，针对长视频全模态助手在跨模态事件绑定（如将正确音频/视觉证据绑定到错误说话人/时刻）上的"几乎正确"幻觉问题，构建了3,600个反事实配对QA样本，并提出无需更新骨干网络的校准框架，显著提升模型在严格配对准确性上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有长视频全模态助手常出现"几乎正确"的错误——模型听到了正确的话语、看到了正确的事件，却将其绑定到错误的说话人、时刻或模态上。这类错误在标准视频QA评测中难以被检测，因为局部证据仍然有效，单项评分可能同时奖励正确主张和其反事实变体。因此需要更严格的评测协议和校准方法来检测和缓解这类跨模态绑定幻觉。

Method: 1) 提出反事实事件绑定协议：从同一音视频事件证据构建配对的支持性/反事实主张，通过严格配对准确性评估。2) 构建OmniHalluc-L基准：包含638个长视频（平均24.16分钟，共256.87小时）的3,600个单主张QA项。3) 提出MPRC（模态扰动可靠性校准）框架：在视频级折叠内选择音频负样本探针，融合其响应偏移与原生音视频置信度，生成每个主张的支持度估计，无需更新骨干网络。

Result: 在OmniHalluc-L上，Qwen2.5-Omni-7B从32.06%提升至36.22%，Qwen3-Omni-Instruct从41.55%提升至51.09%（闭源参考为76.54%）。在OmniVideoBench和WorldSense上分别提升+2.20和+1.51（Qwen3）。

Conclusion: 长视频全模态助手的跨模态事件绑定幻觉是一个重要但被忽视的问题，反事实评测协议能有效揭示这类"几乎正确"的错误。MPRC作为一种无需更新骨干网络的轻量级校准方法，能有效缓解此类幻觉，且具有良好的跨基准泛化能力。

Classification reason: 该论文研究长视频全模态助手（Omni模型）中的幻觉问题，提出反事实事件绑定协议和模态扰动可靠性校准方法，属于多模态幻觉缓解与可靠性提升方向，与[CANONICAL] subtopic "多模态幻觉与可靠性" 高度匹配。

Importance reason: Focuses on hallucination calibration for long-video omni models, which is peripheral to the user's core interests in continuous multimodal models and autoregressive generation. Signals: Counterfactual benchmarking for omni hallucination, Modality-Perturbation Reliability Calibration, Long-video event-binding errors, No backbone update required, Improves Qwen3-Omni on pair accuracy
...
### [89] [Before Fusion, Ask What to Keep: Contextual Calibration of Multimodal Signals](https://arxiv.org/abs/2606.02679)
*Jiyuan Liu, Liangwei Nathan Zheng, Wei Emma Zhang, Xinpei Wang, Weitong Chen*

Main category: cs.MM, cs.SD, cs.LG, eess.AS
...
TL;DR: 本文提出一种即插即用的多模态校准模块，在融合前对各模态特征进行实例级和维度级的调制，通过跨模态摘要比较提取支持/冲突线索，抑制误导性成分、保留弱证据、增强一致信号，在情感理解、动作识别、音视频事件检测等五个基准上提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态融合并非总是有益的——某些模态对特定输入可能带来干扰而非帮助，且同一模态内的局部特征可能与其他模态的证据不一致。现有方法通常在融合后才处理这些问题，而本文希望在融合前就对各模态特征进行校准，避免不可靠信息污染后续融合过程。

Method: 设计一个轻量级校准模块，在融合前对每个模态的特征进行调制。该模块首先通过跨模态摘要比较（summary-level comparison）提取各模态之间的支持与冲突线索，然后将这些线索转换为实例级（instance-wise）和维度级（dimension-wise）的调制信号，作用于原始模态特征上，而非已融合的表征。该模块可作为即插即用组件附加到不同融合骨干网络上，无需修改预测头。

Result: 在情感理解、动作识别、音视频事件检测、音视频情感分类五个基准上，该方法在序列式和卷积式融合设置下均提升了性能。消融实验表明，在模态移除、合成损坏、训练动态分析和特征可视化等条件下，融合前校准能有效减少不可靠模态的干扰，产生更稳定的多模态优化过程。

Conclusion: 融合前的多模态信号校准是一种有效且通用的策略，能够提升多模态系统的鲁棒性和性能。该模块轻量、即插即用，适用于多种融合架构，为多模态融合提供了一种新的"先校准再融合"范式。

Classification reason: 该论文提出了一种多模态融合前的校准模块，通过跨模态比较来抑制干扰、增强有用信号，本质上属于多模态对齐与信号调控问题。虽然不直接涉及RLHF/DPO，但其核心是跨模态信号的对齐与优化，与"多模态强化学习与对齐"（multimodal_rl_alignment）中关于多模态对齐的主题高度相关，且该方向涵盖多模态信号间的对齐与调控。

Importance reason: Proposes a pre-fusion calibration module for multimodal signals, but the subtopic (multimodal reinforcement learning and alignment) and application domains (sentiment, action, audio-visual) are not aligned with user's priority subtopics. Signals: Multimodal calibration, Pre-fusion modulation, Cross-modal conflict detection, Plug-in module, Sentiment/action/audio-visual benchmarks
...
### [90] [Filter, Then Reweight: Rethinking Optimization Granularity in On-Policy Distillation](https://arxiv.org/abs/2606.02684)
...
TL;DR: 本文提出 FiRe-OPD（Filter, then Reweight），一种在大语言模型 On-Policy Distillation 中联合优化轨迹级和 token 级监督信号的方法。先过滤低质量轨迹，再对保留轨迹内的 token 进行软重加权，相比硬 token 选择能减少信息损失并提升优化稳定性。在强到弱、单教师、多教师等多种设置下均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有 OPD 方法逐渐从全轨迹 KL 监督转向更精细的选择性训练范式，但大多仅关注轨迹选择或 token 选择中的单一粒度。本文旨在重新思考 OPD 的优化粒度，提出同时从轨迹和 token 两个层面联合调整监督信号的方法，以更精细地控制训练信号质量。

Method: FiRe-OPD 包含两个阶段：(1) 轨迹过滤（Filter）：基于奖励信号或质量指标过滤掉低质量的 rollout 轨迹；(2) Token 软重加权（Reweight）：在保留的轨迹内，对每个 token 赋予软权重，强调信息量大的 token 同时避免硬选择带来的信息丢失。该方法可应用于强到弱蒸馏、单教师和多教师等多种 OPD 场景。

Result: 在多个基准上验证了有效性：强到弱设置下 AIME 2024 提升 +6.25，多教师设置下 Miner 提升 +18.81，优于近期 token 级 OPD 方法。实验覆盖了强到弱、单教师、多教师三种设置，展示了方法的通用性和优越性。

Conclusion: FiRe-OPD 通过轨迹过滤与 token 软重加权两阶段联合优化，实现了更细粒度的 OPD 训练，有效缓解了信息损失并提升了优化稳定性。该方法在多种蒸馏设置下均表现优异，为 OPD 的优化粒度研究提供了新思路。

Classification reason: 该论文研究大语言模型中的 On-Policy Distillation（OPD），提出 FiRe-OPD 方法在轨迹和 token 级别联合优化监督信号。这属于多模态强化学习与对齐（multimodal_rl_alignment）范畴，因为 OPD 是 RLHF/对齐训练中的关键技术，涉及偏好学习、奖励信号优化等对齐方法。该 canonical 主题涵盖 RLHF、DPO、偏好学习等，与本文的 OPD 优化高度相关。

Importance reason: Focuses on LLM distillation optimization, not directly on the user's priority subtopics like continuous multimodal models or video generation. Signals: On-policy distillation, trajectory and token-level optimization, strong-to-weak and multi-teacher settings, soft reweighting over hard selection, no direct connection to continuous language/multimodal or video generation
...
### [91] [ASymPO: Asymmetric-Scale Policy Optimization for Asynchronous LLM Post-Training Without Behavior Information](https://arxiv.org/abs/2606.03070)
...
TL;DR: 提出 ASymPO（非对称尺度策略优化），一种无需行为策略概率的异步强化学习方法，通过按当前平均负对数概率归一化每个响应的 token 损失，解决异步 RL 中陈旧响应导致的尺度不平衡问题，在数学推理后训练中取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 异步强化学习可以提升 LLM 后训练的吞吐量，但陈旧响应会导致分布漂移。现有方法依赖行为策略概率进行校正，需要 rollout 和 learner 系统间保持 token 对齐、版本一致且数值一致的行为 log-probability，实现复杂。作者希望探索仅使用当前策略概率即可稳定异步 group-relative RL 的方法。

Method: 提出 ASymPO（Asymmetric-Scale Policy Optimization），核心思想是将每个陈旧响应的 token 损失除以其当前平均 token 负对数概率，从而恢复响应级别的零和平衡。同时提出 SPO（Scaled Policy Optimization）作为固定负缩放基线。两种方法均无需行为策略概率，仅依赖当前策略概率。

Result: 在异步数学推理后训练场景中，ASymPO 和 SPO 均能有效稳定训练，无需行为策略概率即可达到或超越传统行为校正方法的性能。

Conclusion: ASymPO 证明了仅使用当前策略概率即可实现稳定的异步 RL 后训练，简化了系统实现，为大规模 LLM 后训练提供了更高效的替代方案。

Classification reason: 该论文提出异步强化学习方法 ASymPO 用于 LLM 后训练对齐，属于多模态强化学习与对齐（multimodal_rl_alignment）范畴。虽然论文主要聚焦数学推理场景，但其核心方法（异步策略优化、偏好学习、对齐）与该 canonical subtopic 高度匹配。

Importance reason: Novel RL method for LLM post-training but not aligned with user's priority subtopics on continuous multimodal models or video generation. Signals: Asynchronous RL without behavior policy, scale-imbalance failure mode, mathematical reasoning focus, no multimodal or video application, no continuous token or flow matching
...
### [92] [Constitutional On-Policy Safe Distillation](https://arxiv.org/abs/2606.03089)
...
TL;DR: 本文提出 COPSD（Constitutional On-Policy Safe Distillation），一种用于安全对齐的蒸馏方法。作者发现传统 on-policy self-distillation 在安全对齐场景下会出现坍缩问题——宪法条件约束使教师分布偏向短且过度保守的回复，Reverse KL 进一步放大这种收缩。COPSD 通过 Cross-SFT 冷启动校准教师，再进行宪法条件 on-policy 蒸馏，在 12 个基准上实现了更好的安全-有用性权衡，并显著降低了对通用推理能力的安全税。


<details>
  <summary>Details</summary>
Motivation: On-policy self-distillation (OPSD) 在可验证推理任务中已被发现会坍缩，但安全对齐场景下是否也存在类似问题尚不清楚。安全对齐由高层宪法（constitutions）而非显式目标答案引导，看似更适合密集蒸馏，但初步实验表明安全 OPSD 同样存在严重坍缩。作者希望分析坍缩机制并提出缓解方法。

Method: 1) 形式化分析：将坍缩归因于非正交语义空间中的"几何泄漏"（geometric leakage），即安全边界压力会转移到表达性维度。2) 提出 COPSD：首先通过 Cross-SFT（跨模型监督微调）冷启动校准教师模型，使其在安全约束下保持表达性；然后进行宪法条件 on-policy 蒸馏，利用校准后的教师提供密集 token 级监督。

Result: 在 12 个基准上的实验表明，COPSD 在安全-有用性权衡上持续优于基线方法，同时显著降低了对通用推理能力的安全税（safety tax）。

Conclusion: COPSD 有效缓解了安全 OPSD 中的坍缩问题，通过教师校准和宪法条件蒸馏实现了更好的安全对齐效果，为安全蒸馏提供了理论分析和实用方案。

Classification reason: 该论文研究安全对齐中的蒸馏方法（Constitutional On-Policy Safe Distillation），属于多模态/语言模型的对齐与偏好优化范畴，与 canonical subtopic "多模态强化学习与对齐"（multimodal_rl_alignment）高度匹配。

Importance reason: Focuses on safe distillation for LLM alignment, not on the user's priority subtopics like continuous models or video generation. Signals: safety alignment, on-policy distillation, constitutional conditioning, collapse analysis, safety-usefulness trade-off
...
### [93] [When RLHF Fails: A Mechanistic Taxonomy of Reward Hacking, Collapse, and Evaluator Gaming](https://arxiv.org/abs/2606.03238)
*Zelalem Abahana*
...
TL;DR: 本文对 RLHF 的失败模式进行了系统性分类与实证研究，提出了一种基于奖励方向、评判分数和平均评判分数的机制分类法，在 PPO、DPO、UP-PPO 等设置下分析了奖励黑客、崩溃与评估者博弈的动态过程，并证明这些失败可以通过训练动态进行定位和部分预测。


<details>
  <summary>Details</summary>
Motivation: RLHF 通过可学习的代理奖励替代未完全指定的人类目标，但这种替代引入了结构化的失败面——优化可能提高学习奖励的同时降低外部质量。现有研究通常将奖励黑客视为单一终点事件，缺乏对训练过程中失败模式的细粒度分类与动态分析。

Method: 构建了一个紧凑的 RLHF 流水线，包含 PPO、DPO、不确定性惩罚 PPO（UP-PPO）、奖励模型不确定性、近似策略漂移、多样性与重复性诊断以及两个外部 LLM 评判器。通过检查点之间的匹配转换，利用学习奖励、评判分数和平均评判分数的方向对失败模式进行分类，并采用逻辑回归模型预测奖励黑客。

Result: 在 61 个检查点行和 1920 个行级转换中，激进 PPO 的局部奖励黑客率最高（14.45%，bootstrap 95% CI: 10.16-18.75），而 UP-PPO 在相同激进设置下率更低（11.33-10.94%）。逻辑回归模型预测未来行级奖励黑客的 ROC-AUC 达到 0.821，且在 12 个设置中有 3 个发现了检查点平均遗漏的局部奖励黑客。

Conclusion: RLHF 失败不仅是最终模型的病理现象，更是可以在训练动态中被分类、定位和部分预测的过程。该工作为理解和缓解 RLHF 中的奖励黑客问题提供了方法论基础。

Classification reason: 该论文研究 RLHF 中的奖励黑客、崩溃与评估者博弈问题，属于多模态强化学习与对齐（multimodal_rl_alignment）范畴下的 RLHF 失败模式分析，与现有 canonical subtopic 高度匹配。

Importance reason: Focuses on RLHF failure taxonomy, not on the user's priority subtopics like continuous language models or video generation. Signals: RLHF failure taxonomy, reward hacking analysis, PPO/DPO/UP-PPO comparison, training dynamics classification, not aligned with user's priority subtopics
...
### [94] [Mitigating False Credit Propagation: Probabilistic Graphical Reward Aggregation for Rubric-Based Reinforcement Learning](https://arxiv.org/abs/2606.03361)
...
TL;DR: 本文提出GEAR框架，通过概率图模型对基于评分标准的奖励进行依赖感知聚合，解决了传统扁平标量化聚合中忽略评分项间前提与激活关系导致的虚假信用传播（FCP）问题，在多个基准上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 基于评分标准（rubric）的奖励在开放语言模型后训练中越来越常见，但现有方法将各评分项得分作为独立效用进行扁平聚合，忽略了评分项之间的前提依赖和激活关系，导致奖励或惩罚在条件不满足时仍被计入（即虚假信用传播FCP）。作者希望设计一种能感知评分项依赖关系的奖励聚合方法。

Method: 提出GEAR（Graphical Event Aggregation for Rubric rewards）框架：将每个评分项结果建模为类型化评分图上的潜在伯努利事件，通过软抑制机制从不受支持的父事件向其子事件传播，并将事件概率聚合为归一化的期望符号效用。该方法具有线性时间复杂度，可直接嵌入标准基于评分标准的RL流程中，无需改变外部优化算法。

Result: 在HealthBench、WritingBench和PLawBench三个基准上，使用两种策略骨干网络，GEAR一致优于扁平聚合和确定性门控方法，相对提升最高达15.5%。FCP诊断显示，GEAR相比扁平聚合减少了96.5%的信用泄漏，同时比确定性门控保留了更多合法的下游效用。

Conclusion: GEAR通过概率图模型有效解决了基于评分标准的奖励聚合中的虚假信用传播问题，在多个基准上取得显著且一致的改进，为开放语言模型后训练中的奖励设计提供了更可靠的方案。

Classification reason: 该论文提出基于概率图模型的评分聚合方法（GEAR），用于基于评分标准的强化学习奖励聚合，解决虚假信用传播问题。虽然论文主要聚焦于语言模型后训练中的奖励聚合，但其方法可广泛应用于多模态RL对齐场景（如多模态RLHF/DPO中的奖励建模），属于多模态强化学习与对齐方向下的奖励建模与偏好优化范畴，因此映射到[多模态强化学习与对齐]子主题。

Importance reason: Focuses on RL reward aggregation for language models, not directly on the user's priority subtopics like continuous multimodal models or video generation. Signals: RL for language model alignment, rubric-based reward aggregation, probabilistic graphical model, addresses false credit propagation, improves multi-benchmark performance

Abstract: Rubric-based rewards are increasingly used for open-ended language model post-training, but criterion-level scores are often aggregated as independent utilities. This flat scalarization ignores rubric-specified prerequisite and activation relations among criteria, allowing reward or penalty to be counted even when the condition that licenses it is absent. We call this structural reward-aggregation failure \textbf{False Credit Propagation} (FCP). To address this limitation, we propose \ourname (\textbf{G}raphical \textbf{E}vent \textbf{A}ggregation for \textbf{R}ubric rewards), a probabilistic graphical framework for dependency-aware rubric aggregation. \ourname models each criterion outcome as a latent Bernoulli event in a typed rubric graph, propagates soft suppression from unsupported parent events to their children, and aggregates the resulting event probabilities into a normalized expected signed utility. This yields a linear-time reward computation that can be plugged into standard rubric-based RL pipelines without changing the outer optimization algorithm. Experiments on HealthBench, WritingBench, and PLawBench with two policy backbones show that \ourname consistently improves over flat aggregation and deterministic gating, achieving relative gains of up to 15.5\% over flat aggregation. FCP diagnostics further show that \ourname reduces leakage by 96.5\% relative to flat aggregation while preserving more licensed downstream utility than deterministic gating. Our code is publicly available at .

</details>


### [95] [Exploiting Verification-Generation Gap: Test-Time Reinforcement Learning with Confidence-Conditioned Verification](https://arxiv.org/abs/2606.03608)
...
TL;DR: 本文提出TTRL-CoCoV，一种置信度条件自适应的测试时强化学习框架，通过分析验证-生成差距，对高置信样本使用探索增强奖励防止多样性崩溃，对低置信样本使用验证器过滤错误伪标签，在6个推理基准上平均提升Pass@1 +9.8%、Pass@16 +18.7%。


<details>
  <summary>Details</summary>
Motivation: 现有测试时强化学习（TTRL）方法主要关注Pass@1性能，而Pass@k（生成覆盖度）在无标签设置下的优化尚未被充分探索。直接应用RLVR中的Pass@k优势设计效果不佳，需要深入分析其根本原因并提出有效解决方案。

Method: 提出TTRL-CoCoV框架，核心洞察是验证能力通常领先于生成能力。方法包括：(1) 对高置信样本，使用验证器自举（bootstrapping）并施加探索增强奖励以防止多样性崩溃；(2) 对低置信样本，将伪标签选择委托给验证器以过滤错误标签；(3) 对中等置信样本，跳过验证直接使用原始伪标签。通过置信度条件机制自适应地处理不同置信度水平的样本。

Result: 在6个广泛认可的推理基准上，TTRL-CoCoV优于所有竞争方法，相比TTRL平均绝对提升Pass@1 +9.8%、Pass@16 +18.7%。在多个推理基准上，甚至比全监督RL方法在Pass@1上绝对提升高达+5.0%。

Conclusion: TTRL-CoCoV通过置信度条件验证机制有效弥合了验证-生成差距，在无标签设置下同时提升了Pass@1和Pass@k性能，为测试时强化学习提供了新的有效范式。

Classification reason: 该论文提出测试时强化学习方法（TTRL-CoCoV），通过置信度条件验证机制优化LLM的推理能力，属于强化学习与对齐方向。虽然论文主要关注语言模型推理，但其方法（RLVR、Pass@k优化、verifier bootstrapping）与多模态RL对齐方向高度相关，且该方向涵盖多模态RL方法，故映射到现有CANONICAL subtopic "多模态强化学习与对齐"。

Importance reason: Focuses on test-time RL for reasoning, not directly on user's priority subtopics like continuous multimodal models or video generation. Signals: Test-time reinforcement learning, verification-generation gap, confidence-conditioned adaptation, Pass@k optimization, label-free reasoning enhancement
...
TL;DR: 本文发现纯token熵在视觉推理RL中失效，因为视觉敏感token天然低熵而被忽略。提出VEPO框架，通过视觉敏感度与token熵的乘法耦合，将梯度信用分配给既视觉相关又高信息量的token，在7B和3B规模上分别超越熵基线2.28和3.15个点。


<details>
  <summary>Details</summary>
Motivation: 现有基于token熵的信用分配机制在文本RLVR中有效，但在视觉推理中失效——视觉敏感token天然具有低熵，导致被错误忽略。现有多模态RL方法虽重视视觉感知，但缺乏系统性的视觉度量，且未意识到token熵主要驱动语义探索，无法满足精确感知与语义推理交织的需求。

Method: 提出VEPO（Vision-Entropy token-selection for Policy Optimization）框架，核心是视觉敏感度与token熵的乘法耦合机制：通过视觉注意力分析计算每个token的视觉敏感度，将其与token熵相乘得到联合重要性分数，据此选择关键token进行梯度更新，从而将梯度信用分配给既视觉相关又高信息量的token。

Result: 在视觉推理任务上，VEPO在7B规模超越熵基线2.28分，在3B规模超越3.15分。消融实验验证了视觉-熵耦合设计的有效性。

Conclusion: 视觉推理RL中纯熵机制失效，VEPO通过显式引入视觉敏感度与熵的耦合，有效解决了视觉token被忽略的问题，为多模态RL中的信用分配提供了新思路。

Classification reason: 该论文研究视觉推理中的强化学习（RLVR），提出基于视觉敏感性与token熵的耦合策略优化方法VEPO，属于多模态强化学习与对齐方向。现有[CANONICAL] subtopic "多模态强化学习与对齐"（multimodal_rl_alignment）完全覆盖该主题。

Importance reason: Directly addresses a critical failure mode in visual reasoning RL, proposing a principled solution that aligns with user's priority on continuous multimodal models and efficient credit assignment. Signals: visual reasoning RL, token entropy failure, vision-sensitive token selection, VEPO framework, multimodal RL credit assignment
...
### [97] [\textsc{CR-Seg}: Attention-Guided and CoT-Enhanced Coarse-to-Refined Reasoning Segmentation](https://arxiv.org/abs/2606.03564)
...
TL;DR: CR-Seg 提出了一种两阶段粗到细推理分割框架，通过注意力图提取（EAP模块）进行粗定位，并引入全局到局部思维链（GLCoT）引导模型逐步推理，最终利用 SAM 进行掩码细化，在推理分割基准上取得有效结果。


<details>
  <summary>Details</summary>
Motivation: 现有推理分割方法要么依赖语义 token 桥接 MLLM 与分割模型（跨模态对齐困难），要么依赖显式空间提示如边界框（丢失整体响应语义），缺乏有效的粗到细推理机制。

Method: 提出两阶段框架 CR-Seg：(1) EAP 模块从 MLLM 注意力图中提取注意力图和关键点，实现粗目标定位；(2) 引入 GLCoT（全局到局部思维链），引导模型从全局场景逐步推理到局部目标细节；(3) 将粗定位信息输入 SAM 进行掩码细化。

Result: 在推理分割基准上的实验表明 CR-Seg 有效提升了分割性能。

Conclusion: CR-Seg 通过注意力引导的粗定位和 CoT 增强的逐步推理，有效解决了推理分割中的跨模态对齐和语义丢失问题，验证了粗到细策略在复杂语言指令分割任务中的有效性。

Classification reason: 该论文提出 CR-Seg，一个结合 Chain-of-Thought（GLCoT）的推理分割框架，属于多模态推理与思维链方向，与 canonical subtopic "multimodal_reasoning_cot" 高度匹配。

Importance reason: Focuses on reasoning segmentation with CoT, but lacks direct connection to user's priority subtopics like continuous language models or video generation. Signals: reasoning segmentation, coarse-to-refined, attention maps, global-to-local chain-of-thought, multimodal large language model
...
### [98] [GTBench: A Curriculum-Grounded Benchmark for Evaluating LLMs as Mathematical Research Assistants in Graph Theory](https://arxiv.org/abs/2606.03144)
...
TL;DR: GTBench是一个基于课程设计的图论推理基准，包含63个从本科定义到研究生证明的三级难度问题，评估了5个前沿LLM。结果显示GPT-5接近天花板，而其他模型随难度增加性能显著下降，并揭示了算法执行错误、不完整推理失败以及人类与自动评估者之间的系统性分歧。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地被用作技术学科的自学助手，但它们在数学推理方面的可靠性尚不清楚，尤其是在图论这一数学分支中缺乏系统性的课程化评估框架。

Method: 构建了包含63个问题的GTBench基准，按课程难度分为三组（本科定义、算法追踪与结构推理、研究生级证明构造）。在零样本和思维链提示下评估5个模型，使用精确匹配和LLM-as-judge评估前两组，使用混合人类专家与LLM-as-judge评估第三组。

Result: GPT-5在Group 1零样本下达95.8%，在研究生证明上达82%；其他模型随难度增加性能显著下降，Llama在Group 3零样本人类评估中为0%。错误分析显示算法执行错误主导前两组，第三组出现不完整推理失败，人类与自动评估者之间存在系统性分歧（kappa=0.48-0.83）。

Conclusion: GTBench提供了首个课程化图论推理评估框架，揭示了当前LLM在图论推理中的能力差异和失败模式，对AI工具在数学教育和科学研究中的治理具有直接意义。

Classification reason: 该论文评估LLM在图论中的数学推理能力，属于数学/科学推理范畴，与[多模态推理与思维链]（multimodal_reasoning_cot）这一CANONICAL subtopic高度匹配，该subtopic涵盖数学推理。

Importance reason: Focuses on graph theory reasoning benchmarks, not aligned with user's priority on continuous multimodal models or video/image generation. Signals: Graph theory benchmark, LLM reasoning evaluation, curriculum-grounded, not multimodal generation, no continuous token or video focus
...
### [99] [ThoughtFold: Folding Reasoning Chains via Introspective Preference Learning](https://arxiv.org/abs/2606.03503)
...
TL;DR: ThoughtFold 提出了一种通过内省偏好学习来折叠推理链的框架，利用细粒度偏好优化显式惩罚冗余探索，将 DeepSeek-R1-Distill-Qwen-7B 的 token 使用量减少约 56%，同时保持最先进的准确率。


<details>
  <summary>Details</summary>
Motivation: 大推理模型（LRM）在强化学习与可验证奖励（RLVR）的驱动下取得了显著进展，但由于长思维链（CoT）天然包含试错过程，而主流 RLVR 方法倾向于记忆结果正确的 CoT 轨迹，导致冗余探索被强化，产生"过度思考"问题。现有方法主要简单偏好更短的轨迹，但学习信号仍基于结果，无法减少长 CoT 中冗余探索的记忆。

Method: ThoughtFold 框架包含两个核心组件：(1) 内省策略——在每个正确轨迹内部识别冗余，生成一系列候选子轨迹谱；(2) 掩码偏好优化目标——显式惩罚冗余探索，鼓励模型直接桥接关键推理片段，从而将推理链折叠为更简洁的路径。

Result: 在多个推理基准上，ThoughtFold 将 DeepSeek-R1-Distill-Qwen-7B 的 token 使用量减少约 56%，同时保持最先进的准确率，显著提升了推理效率。

Conclusion: ThoughtFold 通过细粒度的内省偏好学习有效缓解了 LRM 的过度思考问题，在不牺牲准确率的前提下大幅提升推理效率，为高效推理模型的发展提供了新思路。

Classification reason: 该论文研究大推理模型（LRM）中思维链（CoT）的冗余探索问题，提出通过细粒度偏好学习折叠推理链以提高效率。虽然论文主要聚焦于纯文本推理模型（如DeepSeek-R1-Distill-Qwen-7B），但其核心问题——推理链效率优化与偏好学习——与多模态推理与思维链（multimodal_reasoning_cot）这一规范子主题高度相关，该子主题涵盖推理时计算扩展、CoT优化等方向。

Importance reason: Focuses on reasoning efficiency via preference learning, not directly on the user's priority subtopics like continuous multimodal models or video generation. Signals: reasoning chain folding, introspective preference learning, token reduction, over-thinking mitigation, RLVR optimization
...
### [100] [PhotoCraft: Agentic Reasoning with Hierarchical Self-Evolving Memory for Deep Image Search](https://arxiv.org/abs/2606.03099)
...
TL;DR: PhotoCraft 提出了一种无需训练的分层记忆系统（工作记忆、情景记忆、语义记忆），用于增强 MLLM 智能体在深度图像搜索中的多步推理能力，在 DISBench 上取得最高 18.5% 的提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于 LLM 的智能体在深度图像搜索中缺乏持久记忆，无法在长时间跨度的多步推理中保持上下文一致性，也无法跨任务迁移经验，导致执行漂移和经验隔离。

Method: 受人类认知启发，PhotoCraft 构建了分层记忆系统，包含工作记忆（维护当前推理上下文）、情景记忆（存储历史搜索经验）和语义记忆（存储通用知识），在推理过程中动态调用这些记忆以保持逻辑一致性和知识可迁移性。

Result: 在 DISBench 基准上，PhotoCraft 在不同 MLLM 骨干网络上均一致提升了上下文感知检索能力，最高提升达 18.5%，有效缓解了无记忆深度图像搜索的关键瓶颈。

Conclusion: PhotoCraft 提供了一种实用且通用的多模态搜索智能体方案，通过分层记忆机制实现了可靠且可泛化的深度图像搜索。

Classification reason: 该论文研究基于多模态大模型的深度图像检索，属于多模态检索范畴，与现有 canonical subtopic "多模态检索"（multimodal_retrieval）高度匹配。

Importance reason: Focuses on memory-augmented MLLM agents for image search, which is not a top priority subtopic and lacks direct connection to continuous models or generation. Signals: MLLM agent, hierarchical memory, image retrieval, training-free, DISBench benchmark
...
### [101] [Mixed-Modality Dual Face-Hair Retrieval](https://arxiv.org/abs/2606.03470)
...
TL;DR: 本文提出混合模态双参考人脸-发型检索任务（DFHR），构建了首个基准 DFHR-Bench（18万+三元组），并提出统一框架 MFHC，通过 token 注入和多视角监督融合解耦的身份与发型嵌入，实现跨模态属性可控检索。


<details>
  <summary>Details</summary>
Motivation: 现有图像检索任务通常只处理单一模态或单一属性的查询，缺乏同时处理跨模态（图像+文本）双参考（身份+发型）的检索范式。本文旨在填补这一空白，实现身份保持下的发型可控检索。

Method: 提出 MFHC（Multimodal Face-Hair Combiner）框架：1）使用解耦的身份和发型嵌入；2）通过 token 注入机制融合两种属性；3）采用多视角监督（多正例、难负例）进行训练。同时构建 DFHR-Bench 基准，包含双图像和图像-文本两种设置下的 18 万+标注三元组。

Result: 在 DFHR-Bench 上进行了全面的实验验证，MFHC 在双图像和图像-文本两种设置下均取得了优于基线方法的检索性能，验证了跨模态属性组合检索的有效性。

Conclusion: 本文定义了混合模态双参考检索这一新任务，提供了基准数据集和有效方法，为身份感知、属性可控的跨模态视觉检索建立了新范式。

Classification reason: 该论文提出混合模态双参考图像检索任务（DFHR），涉及人脸身份与发型属性的跨模态组合检索，属于多模态检索范畴，因此映射到 canonical 多模态检索（multimodal_retrieval）。

Importance reason: Focuses on a niche face-hair retrieval task with limited overlap with user's priority subtopics like continuous language models or video generation. Signals: new retrieval task, benchmark construction, multimodal fusion, no autoregressive or diffusion focus, no continuous token or world model connection
...
### [102] [IdiomX A Multilingual Benchmark for Idiom Understanding, Retrieval, and Interpretation](https://arxiv.org/abs/2606.02584)
*Ayman Ali Sharara*
...
TL;DR: 本文提出 IdiomX，一个大规模多语言习语理解、检索与解释基准，包含 19 万+ 上下文示例、1.2 万+ 习语，覆盖英语、阿拉伯语和法语，定义了四项任务（习语检测、上下文到习语检索、跨语言检索、习语解释），实验表明上下文 Transformer 模型在检测上表现优异，混合检索重排序架构在检索任务上效果显著。


<details>
  <summary>Details</summary>
Motivation: 习语表达因其非组合性、上下文依赖性和跨语言对齐困难，一直是 NLP 的持久挑战。现有习语资源规模有限、上下文多样性不足、多语言覆盖不全，限制了现代语言模型在习语理解上的能力。

Method: 通过可复现的多阶段流水线构建数据集：词汇资源提取、大规模标准化、受控大语言模型增强和结构化验证。定义了四项任务：习语检测、上下文到习语检索、阿拉伯语到英语习语检索、习语解释（建模为语义检索任务）。

Result: 上下文 Transformer 模型显著提升了习语检测性能；混合检索与重排序架构在单语和跨语言习语检索上均有显著增强；习语解释可有效建模为语义检索任务。

Conclusion: IdiomX 提供了一个可扩展的习语语言研究基准，覆盖从检测到检索再到语义解释的渐进式任务，其模块化框架可扩展至更多语言和比喻推理任务。

Classification reason: 该论文构建了 IdiomX 多语言习语理解、检索与解释基准数据集，属于多模态/跨语言语义理解评测，与 [CANONICAL] 多模态评测与基准 (multimodal_benchmark) 高度匹配。

Importance reason: Focuses on multilingual idiom benchmarks, which is outside the user's priority subtopics and negative keywords. Signals: Multilingual benchmark, Idiom detection/retrieval, Not in priority subtopics, No boost keywords matched, cs.CL/IR/AI not core direction
...
### [103] [WebRISE: Requirement-Induced State Evaluation for MLLM-Generated Web Artifacts](https://arxiv.org/abs/2606.03220)
...
TL;DR: WebRISE 是一个评估 MLLM 生成 Web 工件的基准，通过交互合约图（ICG）将任务需求转化为可观测状态、用户意图转换和 DOM/视觉断言，在 442 个任务上对 14 个 MLLM 进行评测，发现最强模型仅达到 65.6% 的转换有效性和 66.3% 的需求覆盖率，且视觉质量不能代表行为正确性。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 生成 Web 工件的评测基准仅通过局部证据评估交互，忽略了决定页面是否正常工作的需求诱导状态和状态转换。需要一种更全面的评测方法来捕捉用户意图和需求约束。

Method: 将任务需求编译为交互合约图（ICG），包含可观测状态、用户意图转换以及 DOM/视觉断言，实现与具体实现无关的浏览器执行。覆盖 5 种输入模态（文本、Markdown、草图、图像、视频），包含 442 个任务、5,495 个转换和 5,271 个需求检查，区分用户陈述功能与隐式产品级约束。

Result: 14 个 MLLM 中，最强模型仅达到 65.6% 转换有效性和 66.3% 需求覆盖率；视觉质量不能代理行为正确性（如 Qwen3.6-35B-A3B 在 Markdown 上视觉得分 80.8 但转换有效性仅 15.5）；视频输入提供最强交互信号（隐式覆盖比文本高 10.6 个百分点）；缺陷注入显示 ICG 评分检测状态错误的速率是检查点式评估的 2-16 倍。

Conclusion: WebRISE 提供了一种更全面的 MLLM 生成 Web 工件评测方法，揭示了当前模型在交互正确性和需求覆盖方面的显著不足，并表明视觉质量不能替代行为正确性评估。

Classification reason: 该论文提出了 WebRISE，一个评估 MLLM 生成的 Web 工件（网页）的基准，包含 442 个任务、交互合约图（ICG）和需求检查。这属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题，因为其核心贡献是构建了一个新的评测基准来评估多模态大模型生成的 Web 工件的交互正确性和需求覆盖度。

Importance reason: Evaluates MLLM web artifact generation but lacks direct connection to your priority subtopics like continuous models or video generation. Signals: MLLM evaluation benchmark, web artifact generation, requirement coverage, interaction contract graph, multimodal understanding generation
...
### [104] [SagaQA: A Multi-hop Reasoning Benchmark for Long-form Narrative Understanding in TV Series](https://arxiv.org/abs/2606.03301)
...
TL;DR: SagaQA 是一个面向完整电视剧集的长视频多跳推理基准，要求模型跨不同剧集进行长距离推理，并系统评估了并行、序列和混合三种规划策略在复杂叙事理解中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理基准多关注相邻帧或短片段的理解，缺乏对完整电视剧集这类长叙事多模态内容的高层理解与跨集推理能力的评测。

Method: 构建 SagaQA 基准数据集，包含需要跨剧集长距离推理的多跳问答对；将现有 agentic 方法分为并行、序列和混合三类规划器，评估其在复杂叙事理解中的推理规划能力。

Result: 混合规划器在 SagaQA 上持续生成更高质量的推理计划，展现出更强的复杂高层叙事理解能力。

Conclusion: SagaQA 填补了长视频多跳推理评测的空白，混合规划策略在复杂叙事理解任务中优于并行和序列策略。

Classification reason: 该论文提出了 SagaQA，一个面向电视剧集长视频的多跳推理评测基准，属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）这一规范主题。

Importance reason: Focuses on narrative reasoning benchmarks, not on the user's priority subtopics like continuous models or generation. Signals: long-form video benchmark, multi-hop reasoning, TV series, planning strategies, multimodal understanding
...
### [105] [SCOPE: Real-Time Natural Language Camera Agent at the Edge](https://arxiv.org/abs/2606.02951)
*Nikolaj Hindsbo, Sina Ehsani, Pragyana Mishra*

Main category: cs.CL, cs.RO, cs.CV, cs.HC, cs.AI
...
TL;DR: SCOPE 是一个面向边缘部署的自然语言 PTZ 相机控制智能体系统，包含 Blender 仿真环境和实体相机部署，发布了 536 个任务的评测基准，评估了 19 种规划器-感知器模型组合，发现更强的 SLM 可减少幻觉并改善工具路由，MoE 模型在延迟和内存方面优于密集模型。


<details>
  <summary>Details</summary>
Motivation: 现有语言驱动的机器人智能体缺乏反映真实世界任务需求的评估体系，尤其是在边缘部署场景下，需要同时考虑自然语言指令、可复现结果、延迟、准确率和错误模式等关键指标。

Method: SCOPE 采用模块化架构，将语言模型（SLM）作为规划器，视觉语言模型（VLM）作为感知器，通过工具调用控制 PTZ 相机。系统在 Blender 仿真环境和实体 PTZ 相机上运行，所有感知、规划和控制均在边缘本地执行。作者构建了 536 个任务的基准（SCOPE-Bench），涵盖 QA、单步/多步指令、计数、空间推理、描述和 OCR 等任务，并使用 LM-as-Judge 评估延迟、准确率和错误模式。

Result: 评估了 19 种规划器-感知器模型组合。更强的 SLM 显著减少幻觉并改善工具路由；一旦 SLM 足够强大，感知成为性能瓶颈。MoE 模型在规划和感知侧均匹配或超越密集模型，同时保持与更小网络相当的延迟和内存占用。量化在精度损失极小的情况下提供了额外效率提升。

Conclusion: SCOPE 验证了在边缘设备上实现实时语言驱动 PTZ 相机控制的可行性，识别了 SLM 能力、感知瓶颈和 MoE 架构的实际设计点，为语言驱动的机器人系统在真实世界部署提供了可参考的评估框架和工程实践。

Classification reason: 论文提出了一个面向自然语言PTZ相机控制的536任务评测基准（SCOPE-Bench），属于多模态评测与基准方向，因此映射到CANONICAL subtopic "多模态评测与基准"。

Importance reason: Focuses on edge deployment of camera agents, not on core continuous multimodal models or video generation priorities. Signals: Edge deployment, PTZ camera control, SLM/VLM evaluation, simulation-to-real, not continuous language or video generation
...
### [106] [Consistent Yet Wrong: Evidence Insensitivity in Spatial Vision-Language Models](https://arxiv.org/abs/2606.02742)
...
TL;DR: 本文发现主流 VLM 在空间距离查询中虽然跨视角预测一致，但往往一致地错误，表明模型依赖先验而非视觉证据。作者提出 ViewDiag 多视角评测协议，从准确性、分布集中度和潜在特征三个维度评估，揭示出"高一致性但低准确性"的决策崩溃现象。


<details>
  <summary>Details</summary>
Motivation: 现有空间 VLM 评测常假设跨视角一致性反映了几何理解能力，但这一假设未经严格检验。作者希望验证：一致性是否真的意味着模型对视觉证据敏感，还是仅仅反映了先验驱动的决策崩溃。

Method: 基于 Hypersim、ScanNet 和 KITTI360 构建 ViewDiag 基准，包含 80 个场景中 176 个物体对的 2-10 个视角。从三个维度评估：(1) 度量准确性；(2) 分布集中度（预测一致性）；(3) 潜在特征探针，区分决策崩溃与表征崩溃。

Result: 多种 VLM 均表现出高预测稳定性伴随显著误差的模式，即"强一致性但低准确性"的聚类。跨视角一致性并不能作为几何理解的可靠代理指标。

Conclusion: 稳定的预测可能反映先验驱动的决策崩溃而非证据敏感推理。ViewDiag 为超越准确率评估空间 VLM 提供了受控基准和诊断框架。

Classification reason: 该论文提出了 ViewDiag，一个用于评估空间 VLM 的多视角评测协议和基准，属于多模态评测与基准方向，因此映射到 [CANONICAL] subtopic "多模态评测与基准"。

Importance reason: Challenges a common assumption in VLM evaluation but is a diagnostic benchmark, not a method advancing core user priorities. Signals: VLM spatial reasoning failure, cross-view consistency misleading, decision collapse vs representation collapse, multi-view evaluation protocol
...
### [107] [Plan2Map: A Multimodal Benchmark for Document-Grounded Geospatial Boundary Reconstruction from Planning Records](https://arxiv.org/abs/2606.02747)
...
TL;DR: Plan2Map 是一个面向英国规划记录文档的多模态地理空间边界重建基准，包含208个案例。作者提出 GeoPlanAgent 系统，将任务分解为证据提取、定位、地图配准、边界分割、投影和验证六个步骤，在基准上达到0.736平均IoU和0.904中位数IoU，远超直接VLM基线。


<details>
  <summary>Details</summary>
Motivation: 规划记录定义了地理区域的限制，但其源文档通常只提供间接的空间证据而非机器可读的边界。现有方法缺乏一个标准化的基准来评估从非结构化规划文档中重建地理空间边界的能力。

Method: 提出 GeoPlanAgent，一个文档引导的地理空间工具循环系统，将任务分解为六个步骤：证据提取（从文档中提取文本和地图信息）、定位（确定大致地理区域）、地图配准（将地图图像与地理坐标对齐）、边界分割（使用监督分割模型生成像素级掩码）、投影（将掩码投影到地理坐标）和验证（检查结果有效性）。

Result: 在 Plan2Map 基准上，GeoPlanAgent 达到0.736平均IoU和0.904中位数IoU，67.8%的预测达到0.8 IoU以上，显著优于直接VLM生成GeoJSON的基线方法。错误主要集中在定位和地图配准阶段，监督边界分割显著提升了像素级掩码质量。

Conclusion: Plan2Map 为从公共规划记录中进行多模态地理空间重建提供了一个具体的测试平台。直接VLM预测仍不可靠，而GeoPlanAgent的模块化分解方法有效提升了重建质量，但定位和地图配准仍是主要瓶颈。

Classification reason: 该论文提出了 Plan2Map，一个面向地理空间边界重建的多模态评测基准，属于多模态理解与生成方向下的多模态评测与基准（multimodal_benchmark）子主题。该基准涉及文档理解、地图图像理解与空间推理，是一个典型的多模态评测工作。

Importance reason: Specialized geospatial benchmark with limited direct relevance to user's priority subtopics on continuous multimodal models and video generation. Signals: Multimodal benchmark, geospatial boundary reconstruction, planning documents, GeoPlanAgent system, VLM baseline comparison
...
### [108] [GeoDrive-Bench: Benchmarking Region-Specific Multimodal Reasoning in Autonomous Driving](https://arxiv.org/abs/2606.02774)
...
TL;DR: GeoDrive-Bench 是一个面向自动驾驶中区域特定交通规则的多模态推理基准，包含 5053 个跨六国的人工验证选择题，覆盖感知、预测、规划与区域推理四个任务。实验表明现有 VLM 在区域驾驶推理上表现差异显著，论文还提出了一种蒸馏算法将区域交通规则知识注入 VLM 内部表征以提升推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 在自动驾驶中表现良好，但其对区域特定交通规则（如不同国家的驾驶文化、交通标志、路权规则等）的处理能力尚未被系统评估，这阻碍了 VLM 在全球不同地区的安全部署。

Method: 构建 GeoDrive-Bench 基准：从六个国家（涵盖不同驾驶文化）收集并人工验证 5053 个多项选择题 QA 对，涉及感知、预测、规划与区域推理四个任务。每个问题要求模型从视觉证据和当地交通惯例中推断正确驾驶行为，且不提供显式国家标签。此外，设计了一种蒸馏算法，将区域特定交通规则知识注入 VLM 的内部表征，使模型更好地对齐视觉场景理解与本地驾驶策略。

Result: 在九个最先进 VLM 上的实验显示，各模型在不同区域驾驶文化下的表现存在显著差异；提出的蒸馏基线模型在跨区域的地理文化推理上取得了改进。

Conclusion: 当前 VLM 仍缺乏鲁棒的区域感知驾驶智能，GeoDrive-Bench 可作为可部署自动驾驶基础模型的诊断与训练测试平台。

Classification reason: 该论文提出了 GeoDrive-Bench，一个面向自动驾驶中区域特定交通规则的多模态推理评测基准，属于多模态评测与基准方向，因此映射到 canonical subtopic "多模态评测与基准"。

Importance reason: Focuses on autonomous driving benchmark, not directly on user's priority subtopics like continuous language models or video generation. Signals: Multimodal reasoning benchmark, Autonomous driving, Region-specific traffic rules, VLM evaluation, Distillation algorithm
...
### [109] [Automated Report-Derived Oncology VQA Benchmark for Evaluating Vision-Language Models on 3D Medical Imaging](https://arxiv.org/abs/2606.02809)
...
TL;DR: 本文提出一个自动化智能体驱动的流水线，从私有放射学报告和3D肿瘤影像中生成多选VQA基准数据集，用于评估VLM在医学影像上的视觉能力。该流水线无需人工标注，可控制实例污染，并在四个癌症队列上构建了基准。零样本评估六个VLM发现无主导模型，且盲消融实验表明视觉依赖高度数据集特异。


<details>
  <summary>Details</summary>
Motivation: 现有医学影像VLM评测基准规模有限、依赖人工标注，且可能泄露到VLM预训练语料中，缺乏临床可扩展且污染可控的评测方案。

Method: 提出自动化智能体流水线，从配对的私有放射学报告和3D肿瘤影像中生成两类多选VQA问题：(1) RADS风格问题，从临床定义的报告模式中确定性推导；(2) 放射学报告衍生问题，由LLM从放射科医生发现中生成并验证。流水线应用于四个内部癌症队列，构建无实例污染的基准。

Result: 零样本评估六个VLM显示无主导模型，所有维度均有显著提升空间。盲消融实验发现视觉依赖高度数据集特异：肝脏报告衍生问题确实需要图像，而肺CT在无图像时几乎可解——领先的闭源模型在盲测中甚至超过有图像时的准确率。

Conclusion: 该自动化流水线可生成临床相关、可扩展且污染可控的医学VQA基准。实验表明即使使用私有临床数据也不能保证对视觉能力的无污染评估，揭示了当前VLM在医学影像理解上的不足。

Classification reason: 该论文提出了一个自动化流水线，从放射学报告和3D肿瘤影像生成多选VQA数据集，用于评估视觉语言模型在医学影像上的表现。这属于多模态评测与基准（multimodal_benchmark）范畴，是[CANONICAL]子主题。

Importance reason: Focuses on medical VLM evaluation, which is outside your priority subtopics and has negative keyword overlap. Signals: Medical imaging benchmark, VLM evaluation, 3D oncology, low personal direction weight, negative keyword medical diagnosis
...
### [110] [CoEval: Ranking Language Models for Custom Tasks Without Labeled Data or Trustworthy Benchmarks](https://arxiv.org/abs/2606.03650)
...
TL;DR: CoEval 是一个开源框架，仅需任务描述即可自动生成无污染评测基准，并通过跨族评委集成对语言模型进行无标签排序，无需人工标注或可信基准，以低成本实现可复现的模型评估。


<details>
  <summary>Details</summary>
Motivation: 当特定任务缺乏标注数据且公开基准因数据泄露而不可信时，选择和排序语言模型变得极为困难。现有方法依赖人工标注或已有基准，成本高且存在污染风险。

Method: CoEval 框架包含两个核心组件：(1) 教师模型根据任务描述合成属性可控的新评测基准，每次运行生成全新数据，避免污染；(2) 跨族评委集成（cross-family judge ensemble）对候选模型进行无标签排序，通过供应商多样性而非评委数量保证可靠性。整个流程无需人工标注。

Result: 在有真实标签的验证场景中，CoEval 恢复了真实的模型排序，与真实正确率的相关性达到 ho=0.86。单个评委可能与真实结果负相关（评委选择遗憾 0.35），而集成评委从未出现负相关。生成的数据与五个主要公开基准的 13-gram 重叠率为零。四项任务共产生 7,978 次评估，成本仅 5.89 美元。

Conclusion: CoEval 提供了一种廉价、无污染、无标签的模型评估方案，可针对每个模型发布重新运行，适用于任何领域，为团队构建自定义排行榜提供了可行路径。

Classification reason: 该论文提出 CoEval 框架，从任务描述出发自动生成无污染评测基准，并使用跨族评委集成对语言模型进行无标签排序。虽然论文主要关注语言模型（LLM）而非多模态，但其核心贡献——无标签、无污染的自动评测框架——与多模态评测与基准（multimodal_benchmark）这一规范主题高度契合，因为该框架可直接应用于多模态模型的评测场景。论文的 arXiv 分类包含 cs.CL 和 cs.AI，且方法本身是领域无关的，属于评测方法论。

Importance reason: Focuses on model evaluation without labeled data, which is useful but not aligned with your priority subtopics on continuous multimodal models or video generation. Signals: No labeled data needed, cross-family judge ensemble, contamination-free benchmark generation, low-cost evaluation pipeline, not aligned with priority subtopics
...
### [111] [Does Language Shift Break Medical Vision-Language Models? Indonesian Radiology Visual Question Answering Case Study](https://arxiv.org/abs/2606.03693)
...
TL;DR: 本文提出IndoRad-VQA，一个印尼语版本的放射学VQA基准，用于评估医学视觉语言模型在非英语临床语言环境下的鲁棒性。实验发现，模型在英语VQA上的强性能并不能保证在印尼语场景中的稳健表现，存在8%-25%的性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有医学VLM评测主要基于英语基准，非英语临床语言环境下的模型鲁棒性尚未被充分探索。本文旨在填补这一空白，评估医学VLM在印尼语放射学VQA任务中的表现。

Method: 将VQA-RAD数据集翻译为印尼语，采用自评估质量控制系统保持临床含义、术语一致性和答案等价性。评估了通用型、东南亚多语言型和医学专用型VLM在英语和印尼语两种提示设置下的表现，并进行了错误分析。

Result: 英语与印尼语设置之间存在8%-25%的性能差距（取决于评估指标）。错误分析揭示了yes/no翻转、左右侧错误和输出语言不匹配等失败模式。

Conclusion: 医学多模态基础模型需要更包容的多语言评测。英语基准上的强性能不能保证非英语临床场景中的稳健表现，亟需更广泛的多语言评估。

Classification reason: 该论文提出了印尼语医学放射学VQA基准IndoRad-VQA，评估医学VLM在非英语语言下的鲁棒性，属于多模态评测与基准方向，因此映射到[CANONICAL] subtopic "多模态评测与基准" (multimodal_benchmark)。

Importance reason: Focuses on medical VLM language robustness, which is outside your priority subtopics and negative-keyword domain. Signals: medical VLM, Indonesian radiology VQA, language robustness gap, non-English evaluation, low priority direction weight
...
### [112] [RobotValues: Evaluating Household Robots When Human Values Conflict](https://arxiv.org/abs/2606.03312)
...
Importance: 值得关注 (75.84) - 未入选精读

TL;DR: RobotValues 是一个包含 10K 个价值冲突场景的家用机器人规划器评测基准，用于评估机器人在任务效率、人类自主性、社会适当性等不同人类价值冲突时的行为偏好。实验发现 VLM 模型存在默认价值偏好（如安全优先），且当被要求优先考虑与自身偏好冲突的价值时，80% 的情况下会选错动作。


<details>
  <summary>Details</summary>
Motivation: 现有家用机器人评测主要关注任务完成度或安全性，但日常家庭环境中常出现多种人类价值（如自主性、效率、隐私、社会适当性）相互冲突的情况，机器人需要在这些价值之间做出权衡。目前缺乏系统性的基准来评估机器人在此类价值冲突场景中的价值偏好。

Method: 通过 LLM 辅助场景生成、利益相关者驱动的价值提取、图像生成和自动质量控制，构建了包含 10K 个价值冲突场景的 RobotValues 基准。每个场景包含一张真实感家庭图像和多个合理的机器人动作选项，每个动作优先考虑不同的人类价值。使用该基准评估了多种 VLM 模型在价值冲突场景中的表现。

Result: VLM 模型表现出默认的价值偏好（如安全和包容性优先），而隐私优先的动作被严重低估。当模型被指示优先考虑与自身默认偏好冲突的特定价值时，80% 的情况下无法覆盖默认动作，选择了错误的动作。

Conclusion: 家用机器人评测不仅应关注任务完成和安全合规，还应评估机器人在人类价值冲突时能否在多个合理动作中做出恰当选择。现有 VLM 模型在价值冲突场景中的表现存在显著不足，需要进一步改进。

Classification reason: 该论文提出了 RobotValues，一个用于评估家用机器人规划器在价值冲突场景中表现的基准。它使用 VLM 进行评测，属于多模态评测与基准方向，因此映射到 canonical 的 multimodal_benchmark 子主题。

Importance reason: Novel benchmark for value alignment in household robots, but tangential to user's core focus on continuous multimodal models and generation. Signals: Value conflict benchmark, VLM evaluation, household robotics, safety vs autonomy, 80% failure to override default actions
...
### [113] [Disentangling Visual and Factual Correctness in LVLMs' Visualization Literacy](https://arxiv.org/abs/2606.03142)
...
TL;DR: 本文提出一个框架来区分LVLM在图表理解中的视觉正确性与事实正确性，通过反事实可视化素养评估测试（CVLAT）和视觉-事实依赖指数（VFRI）揭示：模型的高可视化准确率可能源于事实记忆而非真正的视觉推理，且提示干预的效果高度依赖模型。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM可视化理解评测无法区分模型的回答是基于视觉证据的真实推理还是训练中学到的事实先验知识，导致评测结果混淆了两种来源，无法反映模型真正的可视化素养。

Method: 提出CVLAT（反事实可视化素养评估测试），通过修改图表数据使其与常见事实知识冲突，从而分离视觉正确性与事实正确性；提出VFRI（视觉-事实依赖指数）量化模型在冲突时的偏向；在15个SOTA LVLM上进行三组实验，并与30名人类基线对比。

Result: (1) 多个模型在标准VLAT测试中达到人类水平，但可能反映事实回忆而非视觉理解；(2) 基于CVLAT和VFRI将模型分为可视化导向多数和事实知识导向少数；(3) 提示干预可改变模型偏向，但效果高度依赖模型且方向不对称，高图表理解能力不预示提示可控性。

Conclusion: 高可视化准确率不足以证明忠实的视觉推理；可靠的视觉分析集成不仅需要评估可视化素养，还需要评估模型在视觉证据与事实先验冲突时的仲裁能力。

Classification reason: 该论文研究LVLM的可视化理解能力评测，提出了一个区分视觉正确性与事实正确性的框架（CVLAT），属于多模态评测与基准方向。论文核心贡献是新的评测方法和指标（VFRI），揭示现有可视化素养评测的局限性，因此映射到[多模态评测与基准]（multimodal_benchmark）这一规范子主题。

Importance reason: Focuses on LVLM evaluation methodology for chart understanding, not on the user's priority subtopics like continuous models or video generation. Signals: LVLM evaluation, visualization literacy, factual vs visual reasoning, counterfactual test, prompt intervention analysis
...
### [114] [VistaHop: Benchmarking Multi-hop Visual Reasoning for Visual DeepSearch](https://arxiv.org/abs/2606.03273)
...
TL;DR: VistaHop 是一个用于评估多模态大推理模型在多跳视觉搜索（Visual DeepSearch）中能力的基准，包含 300 张高分辨率图像、25 个视觉搜索场景和 350 个多跳 QA 任务，要求模型通过反复检查图像区域、锚定中间推理步骤并连接细粒度线索来完成复杂视觉查询。实验表明当前最强模型仅达到 24.31% Pass@1，揭示了视觉锚定、证据回溯、长链推理和多锚点信息融合方面的持续局限。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单步视觉理解或静态图像问答，缺乏对迭代图像检查、视觉锚点定位和多跳证据整合能力的评估，无法有效衡量模型在 Visual DeepSearch 场景中的表现。

Method: 构建 VistaHop 基准，包含 300 张高分辨率图像、25 个视觉搜索场景和 350 个多跳 QA 任务；同时开发 VistaArena 统一评估环境，支持文本搜索、图像搜索、图像裁剪和基于证据的答案验证等工具增强推理。

Result: 在 7 个代表性多模态大推理模型上评估，最佳模型 SenseNova-MARS-32B 仅达到 24.31% Pass@1，所有模型在视觉锚定、证据回溯、长链推理和多锚点信息融合方面表现不佳。

Conclusion: 当前多模态大推理模型在 Visual DeepSearch 任务上远未达到实用水平，需要更强的基准和训练方法来推动该领域发展。

Classification reason: 该论文提出了 VistaHop，一个用于评估多跳视觉推理（Visual DeepSearch）的新基准，属于多模态评测与基准方向，与 [CANONICAL] subtopic "多模态评测与基准" 高度匹配。

Importance reason: Benchmark for multi-hop visual reasoning, but not directly aligned with user's priority on continuous multimodal models or generation. Signals: multi-hop visual reasoning, visual deepsearch benchmark, low model performance, not generation-focused, not continuous token or flow matching
...
### [115] [SynCred-Bench: Benchmarking Synthetic Credibility in AI-Generated Visual Misinformation](https://arxiv.org/abs/2606.03348)
...
TL;DR: 本文提出SYNCRED-Bench，一个包含600张AI生成虚假信息图像和450张真实图像负集的评测基准，专门针对"合成可信度"这一新型视觉虚假信息威胁。评估发现现有MLLM在5%假阳性率下仅达到10.5%真阳性率，开源检测器低于5%，商业API达57.6%，人类标注者也仅63%，表明该问题严峻且未被充分探索。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型能够生成带有逼真嵌入文本和布局的视觉内容，创造了"合成可信度"这一新型虚假信息威胁——即AI生成的图像看起来像真实的新闻截图、社交媒体帖子或官方文档，具有表面可信度。现有虚假信息检测系统主要关注视觉伪造痕迹，缺乏对这种合成可信度的检测能力，因此需要专门的基准来评估和推动该方向的发展。

Method: 构建SYNCRED-Bench基准，包含600张AI生成的虚假信息图像，平衡覆盖6种可信形式类别（如新闻截图、社交媒体帖子、官方文档等）和7种细粒度传播风格。同时构建FP450真实图像负集用于测量假阳性率。对15个MLLM、多个开源AIGC检测器和商业API进行系统评估，并在5%假阳性率约束下比较真阳性率。

Result: 在5%假阳性率约束下，15个MLLM平均仅达到10.5%真阳性率，开源AIGC检测器低于5%，商业API达到57.6%。人类标注者也仅达到63%真阳性率。这表明现有系统在检测合成可信度方面严重不足，即使是人类也难以可靠识别。

Conclusion: 合成可信度是一种严重且未被充分探索的视觉虚假信息挑战。现有检测系统（包括MLLM、专用检测器和人类）在识别合成可信度方面表现不佳，亟需开发能够超越表面可信度线索进行推理的检测方法。SYNCRED-Bench为该方向提供了标准化评测基准。

Classification reason: 该论文提出了一个针对AI生成视觉虚假信息中"合成可信度"的评测基准SYNCRED-Bench，包含600张AI生成的虚假信息图像和真实图像负集，并对多个MLLM和检测器进行了系统评估。这属于多模态评测与基准（multimodal_benchmark）范畴，是[CANONICAL]子主题。

Importance reason: Addresses a novel visual misinformation threat but is a benchmark paper with low direct relevance to the user's priority on generative models and efficiency. Signals: Novel synthetic credibility concept, MLLM evaluation benchmark, low detection rates, human performance baseline, not aligned with user's generative or efficiency focus
...
### [116] [Enginuity: A Dataset and Benchmark for Vision-Language Understanding of Engineering Diagrams](https://arxiv.org/abs/2606.03410)
...
TL;DR: 本文提出了 Enginuity，首个面向工程图纸的 VLM 评测基准和数据集，包含结构化零件表提取和自由形式 VQA 两个任务，评估了四个前沿 VLM，发现模型在零件识别与描述忠实度之间存在系统性差距，并指出 token 重叠指标在技术描述上低估模型能力 2-6 倍。


<details>
  <summary>Details</summary>
Motivation: 工程图纸与自然图像或通用文档不同，通过密集的空间布局、领域特定符号和视觉标注与结构化零件表之间的交叉引用编码信息，但现有基准缺乏对此类复杂工程图表的 VLM 能力评测。

Method: 基于美国军用维修手册构建 Enginuity 数据集，定义两个任务：(1) 结构化零件表提取，(2) 自由形式视觉问答。在零样本和思维链提示下评估 GPT-5.2 Chat、Claude Opus 4.7、Gemma 4、Qwen3-VL-32B-Instruct 四个模型。

Result: Task 1 上 Recall@all 达 0.61-0.87，但 Token F1_pen 仅 0.03-0.18；Task 2 显示所有模型存在一致的事实推理差距。token 重叠指标相比语义相似度低估技术描述能力 2-6 倍。

Conclusion: 工程图纸领域对 VLM 仍具挑战性，现有模型在零件识别与描述忠实度之间存在系统性差距，需要领域特定的评测校准方法。

Classification reason: 该论文提出了 Enginuity，首个面向工程图纸的 VLM 评测基准和数据集，属于多模态评测与基准（multimodal_benchmark）这一规范主题。

Importance reason: Focuses on a narrow domain benchmark (engineering diagrams) with limited overlap with user's priority subtopics. Signals: New benchmark for engineering diagrams, VLM evaluation, domain-specific gap in description fidelity, token-overlap metric underestimation, no direct link to continuous models, video generation, or world models
...
### [117] [Eliciting Complex Spatial Reasoning in MLLMs through Wide-Baseline Matching](https://arxiv.org/abs/2606.03577)
...
TL;DR: 本文提出 ReasonMatch-Bench，一个分层评估多模态大模型宽基线匹配能力的基准，并设计 DCRL（动态对应强化学习）方法，通过可验证奖励信号显著提升 MLLM 在空间对应任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 宽基线匹配需要整合几何理解、视角变化、细粒度感知和遮挡推理，是评估 MLLM 空间推理能力的理想测试场景。然而现有 MLLM 缺乏对此能力的系统评估和训练框架。

Method: 构建 ReasonMatch-Bench 基准，按视角位移和匹配粒度分层；设计可扩展的数据生成管线，从大规模视频-3D 语料中自动提取宽基线视图对；提出 DCRL（动态对应强化学习），包含图像级视角递进和点级对应课程，通过可验证奖励信号训练 MLLM。

Result: 在困难子集上，人类 F1 为 84.0，最佳现有基线仅 37.2，而 DCRL 显著提升 ReasonMatch-Bench 性能，并泛化到相关空间基准，同时保持通用视觉理解能力。

Conclusion: DCRL 能有效提升 MLLM 的宽基线匹配与空间推理能力，验证了通过可验证奖励进行强化学习训练的有效性，为 MLLM 在物理环境中的空间推理提供了新范式。

Classification reason: 该论文提出了 ReasonMatch-Bench，一个针对宽基线匹配（Wide-Baseline Matching）的空间推理评测基准，属于多模态大模型的空间推理能力评估，因此映射到 [CANONICAL] 多模态评测与基准（multimodal_benchmark）。同时论文也涉及多模态推理（DCRL训练方法），但核心贡献是评测基准，故归入该方向。

Importance reason: Strong benchmark and RL method for spatial reasoning in MLLMs, but subtopic is not a priority. Signals: MLLM spatial reasoning benchmark, reinforcement learning with verifiable reward, wide-baseline matching, scalable data pipeline, transfer to spatial tasks
...
### [118] [TurtleAI: Benchmarking Multimodal Models for Visual Programming in Turtle Graphics](https://arxiv.org/abs/2606.03626)
*Chao Wen, Jacqueline Staub, Adish Singla*

Main category: cs.CV, cs.CY, cs.AI
...
TL;DR: TurtleAI 是一个面向 Turtle Graphics 视觉编程的多模态模型评测基准，包含823个任务。评测20+个VLM发现成功率普遍低于30%，通过少量种子样本合成数据微调Qwen2-VL-72B可提升约20%性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉编程研究主要关注生产力场景，缺乏对教育导向的视觉编程任务的系统评测，不清楚当前VLM在此类任务上的表现及限制因素。

Method: 构建包含823个任务的TurtleAI基准，涵盖几何模式感知、空间推理和Python代码生成。评测20+个VLM，并提出一种仅需少量种子样本的数据合成技术，用于微调模型。

Result: 大多数VLM成功率低于30%；微调Qwen2-VL-72B后在真实任务上提升约20%；GPT-4o在空间推理和精确视觉复现方面存在困难，微调主要改善了视觉推理与代码实现之间的对齐。

Conclusion: 当前VLM在教育导向的视觉编程任务上表现有限，通过合成数据微调可显著提升性能，但空间推理仍是主要瓶颈。

Classification reason: 该论文提出了 TurtleAI 基准，包含823个基于 Turtle Graphics 的视觉编程任务，用于评测多模态模型在几何模式感知、空间推理和代码生成方面的能力，属于多模态评测与基准（multimodal_benchmark）这一 [CANONICAL] 子主题。

Importance reason: Focuses on education-oriented visual programming benchmark, not directly aligned with user's priority subtopics like continuous language models or video generation. Signals: Multimodal benchmark, Turtle Graphics, visual programming, low VLM success rates, synthetic data fine-tuning
...
### [119] [VidMsg: A Benchmark for Implicit Message Inference in Short Videos](https://arxiv.org/abs/2606.03635)
...
TL;DR: VidMsg 是一个面向短视频隐含信息理解的评测基准，包含 400 个 YouTube 短视频片段，覆盖 9 个主题领域和 52 条细粒度目标信息。该基准通过"信息优先"的流水线构建，支持双向信息-视频检索和多项选择 QA 两种评测任务。实验表明现有视频语言模型在此任务上表现不佳，因为需要语用推理和上下文线索整合能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准主要关注显式的物体识别和动作识别，但短视频中往往包含创作者隐含的意图或信息（如劝说、教育、警示等），现有模型缺乏对这种隐含信息的理解能力。作者希望构建一个专门评测模型对短视频隐含信息理解能力的基准。

Method: 采用"信息优先"（message-first）流水线构建基准：先用 LLM 将目标信息翻译成间接搜索场景，再用这些场景检索候选视频片段，最后由人工标注者筛选出能传达目标信息但不过于直白的片段。基准包含 400 个 YouTube 短视频，9 个主题领域，52 条细粒度信息。设计了双向信息-视频检索和多项选择 QA 两种评测任务。同时提出 VidVec-Msg 基线方法，通过改进视频表示来提升信息导向的检索性能。

Result: 现有视频语言模型和检索模型在 VidMsg 上表现不佳，因为任务需要语用推理、上下文线索整合以及语义相近信息之间的区分能力。VidVec-Msg 基线方法在信息导向检索上有所提升，但仍留有较大改进空间。

Conclusion: VidMsg 揭示了现有视频理解模型在隐含信息推理方面的显著不足，为未来研究提供了一个具有挑战性的评测基准。该基准可用于视频搜索、推荐等需要整体视频理解的应用场景。

Classification reason: 该论文提出了 VidMsg，一个用于评估短视频中隐含信息（implicit message）理解的新基准，属于多模态理解评测范畴。该基准包含视频-消息双向检索和多项选择 QA 两种评测任务，与 [CANONICAL] subtopic "多模态评测与基准"（multimodal_benchmark）高度匹配。

Importance reason: Focuses on a niche video understanding benchmark with limited direct relevance to your core priorities in continuous multimodal models and generation. Signals: Implicit message inference, Video-language benchmark, Pragmatic reasoning, Low overlap with priority subtopics, No autoregressive or diffusion generation
...
### [120] [SLU-2K: A Question-Based Benchmark for Semantic Evaluation of Sign Language Translation](https://arxiv.org/abs/2606.03788)
...
TL;DR: 本文提出SLU-2K，一个包含2350个视频问答对的手语语义理解评测基准，涵盖动作、位置、数字、物体、人物、时间、天气7类问题。评估发现MLLM接近随机水平，SOTA翻译系统准确率仅56.7%-75.2%，表明当前BLEU/ROUGE等表面指标高估了真实语义理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有手语翻译（SLT）评估使用BLEU、ROUGE等基于词汇重叠的指标，无法衡量翻译是否真正保留了源手语序列的语义含义。这与将SLT集成到辅助技术中的最终目标相悖，因此需要一种直接评估语义理解能力的评测方法。

Method: 提出SLU-2K数据集，基于PHOENIX-2014T和CSL-Daily两个流行手语数据集，通过自动化数据生成流水线产生2350个封闭式视频问答对，涵盖动作、位置、数字、物体、人物、时间、天气7个语义类别。使用该基准评估了多种MLLM和两个SOTA翻译系统（MMSTL、SpaMo）。

Result: MLLM在手语语义理解上接近随机水平；SOTA翻译系统（经过域内数据微调）准确率在56.7%-75.2%之间，存在显著的语义差距。这表明当前SLT评估协议高估了真实理解能力。

Conclusion: 未来手语翻译的进展不仅应通过流畅度和n-gram重叠来衡量，还应通过语义正确性来评估。SLU-2K为手语语义理解提供了系统化的评测工具，揭示了现有系统在语义层面的不足。

Classification reason: 该论文提出了SLU-2K，一个基于问答的手语翻译语义理解评测基准，属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。论文聚焦于评估MLLM和手语翻译系统的语义理解能力，而非提出新的生成或理解模型。

Importance reason: Focuses on sign language translation evaluation, which is outside the user's priority subtopics and negative keywords. Signals: Sign language translation, semantic understanding benchmark, MLLM evaluation, low relevance to continuous models or video generation
...
### [121] [OVO-S-Bench: A Hierarchical Benchmark for Streaming Spatial Intelligence in Multimodal LLMs](https://arxiv.org/abs/2606.03890)
...
TL;DR: OVO-S-Bench是一个人工标注的流式空间智能评测基准，包含1,680个问题、348个视频源，覆盖从即时自我中心感知到异中心地图构建四个抽象层次。评测38个MLLM发现，Gemini-3.1-Pro落后人类专家27分（59.2 vs 86.6），异中心地图构建是主要瓶颈，流式空间微调模型反而表现下降，思维链在缺乏流式依据时会放大空间错误。


<details>
  <summary>Details</summary>
Motivation: 现有基准要么在完整视频上进行离线评估，要么针对事件而非空间结构进行评测，缺乏对多模态大模型在连续自我中心流式输入下进行空间智能推理能力的系统性评测。

Method: 构建了包含1,680个问题的基准，每个问题带有查询时间戳和证据区间，评估时模型仅能看到查询前的视频前缀。问题分为四个抽象层次：即时自我中心感知、时空上下文追踪、空间模拟与推理、异中心地图构建。采用12名训练有素的标注者进行多轮质量保证（约804人时）。

Result: 在38个MLLM上评测，Gemini-3.1-Pro得分59.2，人类专家86.6，差距27分。异中心地图构建是主要瓶颈。流式空间微调模型表现不如其基础骨干模型。思维链在缺乏流式依据时会放大空间错误。

Conclusion: OVO-S-Bench揭示了当前MLLM在流式空间智能方面的显著局限性，为下一代流式空间MLLM提供了具有挑战性的测试平台。

Classification reason: 该论文提出了OVO-S-Bench，一个用于流式空间智能的多模态评测基准，属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。

Importance reason: Strong benchmark for streaming spatial MLLMs but lacks direct alignment with user's priority subtopics like continuous language models or video generation. Signals: streaming spatial intelligence benchmark, MLLM evaluation, allocentric mapping bottleneck, chain-of-thought amplifies errors, human-annotated 1680 questions
...
TL;DR: NewtPhys是一个基于真实世界场景多视图图像和物理仿真构建的4D物理标注数据集，提供密集的细粒度标注（包括3D力、逐像素物理量等），并系统评估了56个VLM和10个VFM，揭示了它们在低层次牛顿物理推理上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型物理推理评测工作多使用合成或半合成场景，强调高层事件而缺乏评估真正低层次牛顿物理理解所需的视觉保真度。作者希望构建一个兼具真实视觉复杂性和物理标注的数据集，以填补这一空白。

Method: 从真实世界场景的多视图图像出发，结合物理仿真生成4D物理标注数据，提供跨时间步的密集细粒度标注，包括3D力、amodal逐像素物理量（涵盖物理、跟踪、语义和几何）。基于该数据集，系统评估了56个VLM（含54个开源和2个闭源前沿模型）和10个VFM。

Result: 揭示了现有VLM和VFM在低层次牛顿物理推理上的显著局限性，表明当前基础模型对真实世界物理规律的理解仍然不足。

Conclusion: NewtPhys数据集为物理推理评测提供了更真实的基准，揭示了当前模型的不足，并为未来物理感知视觉研究和下一代物理感知评测的发展奠定了基础。

Classification reason: 该论文提出了NewtPhys，一个基于真实世界多视图图像和物理仿真构建的4D物理标注数据集，并系统评估了56个VLM和10个VFM在牛顿物理理解上的表现。这属于多模态评测与基准（multimodal_benchmark）范畴，因为核心贡献是构建了一个新的物理推理评测基准。同时，该工作也涉及世界模型方向（物理世界建模），但主要贡献是评测基准。

Importance reason: Provides a rigorous physics benchmark but lacks direct connection to user's priority subtopics like continuous models or generation. Signals: Physics reasoning benchmark, VLM/VFM evaluation, Real-world 4D dataset, Low-level Newtonian physics, No generation or continuous model focus
...
### [123] [SVHalluc: Benchmarking Speech-Vision Hallucination in Audio-Visual Large Language Models](https://arxiv.org/abs/2606.02642)
*Chenshuang Zhang, Kyeong Seon Kim, Chengxin Liu, Tae-Hyun Oh*

Main category: cs.SD, eess.AS, cs.CV, cs.LG, cs.MM, cs.AI
...
TL;DR: 本文提出SVHalluc，首个系统评估音视频大语言模型中语音-视觉幻觉的基准，从语义和时间两个维度诊断模型在语音内容与视觉信号对齐上的失败。实验表明开源模型在该任务上近乎随机，而Gemini 2.5 Pro显著领先，揭示了当前模型跨模态理解能力的根本局限。


<details>
  <summary>Details</summary>
Motivation: 现有音视频大语言模型幻觉评测仅关注环境音（如狗叫声）指示事件发生，但人类语音携带更丰富的语义和时序结构，模型能否准确对齐语音内容与对应视觉信号尚未被探索。本文旨在填补这一空白。

Method: 构建SVHalluc基准，从语义对齐（如语音描述与视频内容是否匹配）和时间对齐（如语音事件与视频中对应时间点是否一致）两个互补维度设计评测任务，系统评估多个开源音视频LLM及Gemini 2.5 Pro。

Result: 开源音视频LLM在语音-视觉对齐任务上表现接近随机，而Gemini 2.5 Pro显著优于所有开源模型。分析表明模型失败源于跨模态理解能力不足，而非单模态感知能力缺陷。

Conclusion: 当前音视频大语言模型在语音内容与视觉信号的对齐上存在根本性局限，亟需发展语音锚定的视频理解能力。SVHalluc为这一方向提供了系统评测工具。

Classification reason: 该论文提出了SVHalluc，首个评估音视频大语言模型中语音-视觉幻觉的基准，属于多模态评测与基准（multimodal_benchmark）这一[CANONICAL]子主题。

Importance reason: Focuses on speech-vision hallucination in audio-visual LLMs, which is outside the user's core priorities on continuous language models, video/image generation, and world models. Signals: speech-vision hallucination benchmark, audio-visual LLM evaluation, cross-modality alignment failure, Gemini 2.5 Pro outperforms open-source, semantic and temporal diagnosis
...
### [124] [DeskCraft: Benchmarking Desktop Agents on Professional Workflows and Human-in-the-Loop Collaboration](https://arxiv.org/abs/2606.03103)
...
TL;DR: DeskCraft 是一个面向专业创意/工程软件的长时程桌面 GUI 基准，包含多级难度分类、超过 50 步的长时程任务，并形式化了人机协作交互协议（回合中/回合后交互）。在 538 个任务上评测了 18 个智能体，GPT-5.4 在标准任务上仅达 31.6%，揭示了长时程工作流交付和主动澄清方面的持续失败。


<details>
  <summary>Details</summary>
Motivation: 现有桌面 GUI 基准大多将任务简化为短时程、用户指令一次性提供的场景，无法反映真实专业工作流中长时程、需要人机主动协作的特点。作者旨在填补这一空白，构建更贴近实际的专业桌面智能体评测基准。

Method: 构建 DeskCraft 基准，包含：(1) 多级难度分类（从简单到专家级），长时程任务超过 50 步；(2) 覆盖设计、视频、音频、3D 创作等专业创意软件；(3) 形式化人机协作交互协议，包括回合中交互（智能体主动澄清、用户中断）和回合后交互（用户反馈）。在 538 个任务上评测 18 个闭源和开源智能体。

Result: GPT-5.4 在标准任务上达到 31.6%，在交互式任务上达到 27.6%。分析显示智能体在长时程工作流交付和主动澄清方面存在持续失败。

Conclusion: DeskCraft 揭示了当前桌面智能体在专业长时程工作流和人机协作方面的显著不足，为未来研究提供了有价值的基准和方向。

Classification reason: 该论文提出了 DeskCraft，一个面向专业创意/工程软件的长时程桌面 GUI 基准，评测智能体在复杂工作流中的表现及人机协作能力。这属于多模态智能体评测，与 [CANONICAL] 多模态评测与基准 (multimodal_benchmark) 高度匹配。

Importance reason: Focuses on desktop GUI agents for creative software, which is outside the user's core interests in continuous multimodal models and video/image generation. Signals: Desktop GUI benchmark, Long-horizon tasks, Human-in-the-loop collaboration, Creative software workflows, Low agent performance
...
TL;DR: MedCUA-Bench 是一个面向临床计算机使用智能体的截图交互式基准测试，涵盖10个医学领域的18个临床场景，通过意图级和步骤级目标分离临床推理与UI执行，并在任务完成度和5个临床安全维度上进行评估。23个智能体的评测显示，最佳闭源模型严格成功率为54.2%，开源模型平均仅2.5%，揭示了当前智能体在可靠临床软件使用上的巨大差距。


<details>
  <summary>Details</summary>
Motivation: 现有计算机使用智能体基准主要关注通用网页或桌面任务，缺乏对医学软件的评测。医学GUI具有领域知识要求高、UI设计独特、缺乏公开测试环境、需要安全验证等特点，现有基准无法充分评估智能体在临床场景中的可靠性。

Method: 从真实产品手册和开源医疗系统中重建18个临床场景，覆盖10个医学领域。每个任务配备意图级和步骤级目标以分离临床推理与UI执行，使用确定性检查器评估任务完成度和5个临床安全维度（如患者隐私、数据完整性等）。

Result: 最佳闭源模型严格成功率为54.2%，所有模型在真实OpenEMR上低于9%；开源智能体平均仅2.5%，最佳为16.2%。

Conclusion: MedCUA-Bench 暴露了当前计算机使用智能体在可靠临床软件使用上的显著差距，为未来研究提供了一个可复现的测试平台。

Classification reason: 该论文提出了一个面向临床计算机使用智能体的交互式基准测试（MedCUA-Bench），属于多模态智能体评测范畴，与 canonical 子主题"多模态评测与基准"（multimodal_benchmark）高度匹配。

Importance reason: Focuses on medical UI agents, which is outside your core interests in continuous multimodal models and video generation. Signals: Medical domain benchmark, Clinical computer-use agents, Safety evaluation, Low open-source performance, Not aligned with priority subtopics
...
### [126] [From Answers to States: Verifiable Process-Level Evaluation of Chemical Reasoning in Large Language Models](https://arxiv.org/abs/2606.03660)
...
TL;DR: 本文提出 ChemCoTBench-V2，一个基于规则可验证的化学推理过程级评测基准，包含 5,620 个样本，覆盖分子理解、分子编辑、分子优化和反应预测四大任务。该基准通过确定性化学规则和参考轨迹对模型中间推理步骤进行验证，而非依赖 LLM 评判。实验发现前沿模型在最终答案正确性与结构化推理状态一致性之间存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有化学基准仅评分最终答案，无法检测模型推理过程中违反化学逻辑的情况。而基于 LLM 或人工的过程级评估成本高、不一致且易产生幻觉。因此需要一种低成本、可审计、可验证的化学推理过程级评测方法。

Method: 构建 ChemCoTBench-V2 基准，包含 5,620 个样本，覆盖分子理解、分子编辑、分子优化和反应预测四大类共 18 个报告任务。模型需在专家设计的模板中暴露关键中间步骤，这些步骤通过确定性化学规则进行验证；对于封闭答案任务，使用参考轨迹而非 LLM 评判；对于开放式的分子优化任务，使用 oracle 可验证的状态约束而非严格轨迹匹配。基准报告三个独立信号：最终答案正确性、模板遵循度和步骤级验证器正确性。

Result: 实验发现前沿模型在最终答案成功与结构化推理状态一致性之间存在持续差距：模型经常遵循要求的格式但未能通过化学步骤检查，或者答案正确但推理支持薄弱。该基准能够实现细粒度模型比较，并识别出轨迹首次违反验证器的具体步骤。

Conclusion: ChemCoTBench-V2 提供了一种低成本、可审计的化学推理过程级评估方法，揭示了现有模型在结构化化学推理方面的不足，为未来改进提供了诊断工具。

Classification reason: 该论文提出了 ChemCoTBench-V2，一个用于化学推理过程级评测的基准，属于多模态/科学推理评测范畴。虽然化学推理不直接涉及视觉模态，但该基准关注 LLM 在化学领域的结构化推理过程验证，与多模态评测与基准（multimodal_benchmark）这一 CANONICAL 主题高度契合——它提供了新的评测协议、可验证的步骤级评估信号，属于评测基准类工作。

Importance reason: Focuses on chemical reasoning evaluation, which is outside the user's priority subtopics and includes negative keywords like molecule. Signals: Chemical reasoning benchmark, rule-verifiable process evaluation, molecular understanding, not aligned with user's continuous multimodal or video generation focus
...
### [127] [BigFinanceBench: A Workflow-Grounded Benchmark for Financial-Research Agents](https://arxiv.org/abs/2606.03829)
...
TL;DR: BigFinanceBench 是一个包含 928 个专家编写的金融研究任务的评测基准，每个任务配有参考答案和分步评分标准，用于评估 AI agent 在完整金融分析工作流中的推导质量而非仅最终答案。在 10 个前沿模型上的评测显示最佳系统仅得 58.8% 的分数，表明该领域仍有很大提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有金融评测基准大多评估孤立子技能或最终答案，缺乏对可审计、可复现的完整推导过程的评估。然而金融研究的答案只有在其他分析师能够审计其产生过程（包括数据源选择、时间周期、会计定义、假设和计算步骤）时才具有决策相关性。因此需要一个能够评估完整推导过程的基准。

Method: 构建了 928 个专家编写的开放式金融研究任务，每个任务包含：(1) 一个 ground-truth 参考答案；(2) 一个按点加权的评分标准，将推导过程分解为独立可检查的步骤。总共包含 36,241 个评分点，支持部分评分和失败定位。在 10 个前沿和开源 AI agent 上进行了系统评测。

Result: 最佳系统（前沿闭源模型）仅达到 58.8% 的评分标准分数；最终答案准确率是推导质量的有用但有损的代理指标；模型能力在不同金融工作流上表现不均匀。

Conclusion: BigFinanceBench 揭示了当前 AI agent 在金融研究工作流中的显著不足，强调了评估完整推导过程而非仅最终答案的重要性，为金融 AI 研究提供了更具决策相关性的评测框架。

Classification reason: 该论文提出了一个金融研究领域的专家级评测基准 BigFinanceBench，属于评测基准类工作。虽然领域是金融而非传统多模态，但基准设计（专家标注、分步评分、部分评分）与多模态评测基准在方法论上高度一致，且 arXiv 分类为 cs.AI，属于目标方向覆盖范围。因此映射到 [CANONICAL] subtopic "多模态评测与基准"。

Importance reason: Focuses on financial domain benchmark, not aligned with user's priority on continuous multimodal models, video generation, or world models. Signals: Finance benchmark, workflow evaluation, low model scores, not in priority subtopics, negative keywords present
...
### [128] [TGV-KV: Text-Grounded KV Eviction for Vision-Language Models](https://arxiv.org/abs/2606.03075)
...
TL;DR: TGV-KV 提出了一种文本引导的 KV 驱逐方法，通过文本-视觉预算分配、文本加权排序和文本优先保留策略，在 VLM 推理中大幅压缩 KV 缓存，保持 99.2% 的准确率并提升 52.6% 的吞吐量。


<details>
  <summary>Details</summary>
Motivation: VLM 继承自回归生成范式，缓存所有历史 token 的 KV 导致内存随上下文长度线性增长。视觉模态存在大量冗余，现有 KV 驱逐方法多针对纯语言模型设计，忽视了文本与视觉之间的模态差异，导致 VLM 上性能显著下降。

Method: TGV-KV 包含三个子模块：(1) Text-Vision Budgeting (TVB) 基于互信息交互为每层分配预算；(2) Text-Weighted Ranking (TWR) 评估文本优先级，并基于加权文本-图像注意力对视觉 token 排序；(3) Text-Prioritised Retention (TPR) 策略优先保留文本 KV 以避免信息严重丢失。

Result: 在 LLaVA-NeXT 等五种不同规模和架构的模型上评估，TGV-KV 在 VizWiz-VQA 任务上保留了 99.2% 的全 KV 准确率，在极端保留预算 5% 下将端到端吞吐量提升了 52.6%。

Conclusion: TGV-KV 通过文本引导的视觉信息重要性评估，有效解决了 VLM 中 KV 缓存压缩的性能退化问题，在极低预算下仍能保持高精度并显著提升推理效率。

Classification reason: 该论文研究 VLM 的 KV 缓存压缩/驱逐以加速推理并减少内存消耗，属于多模态模型的高效化与轻量化方向，与 [CANONICAL] subtopic "多模态高效化与轻量化"（efficient_multimodal）高度匹配。

Importance reason: Directly addresses efficient inference for multimodal LLMs, a priority subtopic, with strong practical gains. Signals: KV cache eviction, VLM inference acceleration, text-guided visual importance, 99.2% accuracy retention, 52.6% throughput boost
...
### [129] [When Attention Collapses: Stage-Aware Visual Token Pruning from Structure to Semantics](https://arxiv.org/abs/2606.03569)
...
TL;DR: 本文提出 STS（Structure-to-Semantics），一种两阶段视觉 token 剪枝框架，先通过基于排斥的采样最大化空间结构多样性，再通过指令感知的交叉注意力过滤与提示无关的 token，从而缓解注意力坍塌导致的特征多样性损失，提升 VLM 推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉 token 剪枝方法大多依赖初始注意力分数，但高注意力分数会自然坍塌到语义相似区域，导致特征多样性严重下降，丢失重要的上下文细节。作者希望解决这一"注意力坍塌"问题。

Method: 提出两阶段剪枝框架 STS：第一阶段使用基于排斥的采样机制（repulsion-based sampling）最大化 token 的空间和结构多样性；第二阶段利用指令感知的交叉注意力精确过滤与提示无关的 token。两阶段协同实现先保证几何覆盖，再按语义相关性精炼保留的 token。

Result: 大量实验表明，STS 能够缓解基于注意力选择导致的冗余，提升保留视觉 token 的结构多样性和细粒度任务对齐能力，在降低计算开销的同时保持或提升模型性能。

Conclusion: STS 通过解耦剪枝过程为结构多样性和语义相关性两个阶段，有效克服了注意力坍塌问题，为 VLM 的高效推理提供了一种新的 token 剪枝范式。

Classification reason: 该论文提出视觉 token 剪枝框架 STS，旨在减少 VLM 推理时的计算开销，属于多模态模型高效化/轻量化方向。现有 [CANONICAL] subtopic "多模态高效化与轻量化"（efficient_multimodal）明确包含 token 压缩，因此直接复用。

Importance reason: Proposes a novel two-stage visual token pruning method that addresses attention collapse, directly improving VLM inference efficiency. Signals: visual token pruning, attention collapse, efficient inference, VLM acceleration, structure-to-semantics
...
### [130] [Where Do We (Not) Need Temporal Context in Low-Resource Video Task Adaptation?](https://arxiv.org/abs/2606.03837)
...
TL;DR: 本文系统研究了低资源视频任务适应中时间上下文的分配策略，比较了图像预训练模型与视频表示模型在 PEFT 和 probing 下的表现，揭示了时间上下文在 backbone、PEFT 和 probe 之间的最优分布方式。


<details>
  <summary>Details</summary>
Motivation: 视频理解中标注和计算成本高昂，参数高效微调（PEFT）和 probing 是理想的适应方法。然而，现有视频 PEFT 主要聚焦于适应图像预训练模型，而标准 PEFT 方法也可应用于视频表示，这两种设置很少被系统比较。此外，时间推理通常被限制在模型的单一组件中，时间上下文应如何在 backbone、PEFT 和 probe 之间分布尚不明确。

Method: 作者对视频理解中的模型适应策略进行了系统研究，在关注外观、运动以及空间密集三种设置下，评估了多种 PEFT 和 probing 方法，特别关注数据有限（低资源）场景。通过控制 backbone、PEFT 模块和 probe 中时间上下文的分配，分析了不同组件对时间信息的利用效率。

Result: 实验揭示了时间上下文分配对视频适应效果的重要性，不同设置下 backbone、PEFT 和 probe 对时间信息的依赖程度不同，为低资源视频任务中的参数高效适应提供了新见解。

Conclusion: 时间上下文在 backbone、PEFT 和 probe 之间的合理分配对视频任务适应至关重要，不同任务类型（外观/运动/空间密集）需要不同的时间上下文分布策略，这为未来视频 PEFT 方法设计提供了指导。

Classification reason: 该论文系统研究视频理解中参数高效微调（PEFT）和 probing 策略，属于多模态模型高效化与轻量化方向，与 [CANONICAL] subtopic "多模态高效化与轻量化"（efficient_multimodal）高度匹配。

Importance reason: Systematic study of temporal context allocation in low-resource video adaptation, but lacks direct connection to user's priority subtopics like continuous models or generation. Signals: Video understanding, PEFT, temporal context, low-resource adaptation, probing
...
### [131] [ToolGate: Token-Efficient Pre-Call Control for Tool-Augmented Vision-Language Agents](https://arxiv.org/abs/2606.03054)
...
TL;DR: 本文提出 ToolGate，一种轻量级外部控制器，用于在 ReAct 风格的 VLM 智能体执行工具调用前预测是否跳过该调用，从而在保持准确率的同时将 token 成本降低至 64-69%。


<details>
  <summary>Details</summary>
Motivation: 工具增强的视觉语言智能体在执行每个感知工具调用时成本高昂且有时不必要，基线智能体在局部选择性上表现不佳——有益和有害的工具调用比例相近（11.8% vs 9.9%），大多数调用并不改变即时预测结果。因此需要一种机制在工具输出进入上下文之前决定是否执行调用。

Method: ToolGate 是一个轻量级外部控制器，从轨迹文本和简单结构特征中预测执行/跳过决策。它不修改 VLM 主干，仅作为前置过滤模块，在 ReAct 循环中拦截工具调用提议并做出二元决策。

Result: 在 Qwen3-VL 两个骨干模型上，ToolGate 将 token 成本降低至无限制 ReAct 基线的 64-69%，同时保持跨域设置下的平均准确率。在同域轨迹训练下（Qwen3-VL-30B），平均准确率进一步提升 1.65 个点。

Conclusion: 工具增强的 VLM 智能体不仅受益于更好的感知工具，也受益于对工具输出何时值得付出的显式控制。ToolGate 提供了一种轻量级、有效的预调用控制方案。

Classification reason: 该论文研究工具增强型视觉语言智能体的预调用控制，通过轻量级控制器决定是否执行工具调用以减少 token 消耗，属于多模态高效化与轻量化（token 压缩/高效推理）方向，因此映射到 canonical subtopic "多模态高效化与轻量化"。

Importance reason: Directly addresses token efficiency in multimodal agents, aligning with your model efficiency priority and boosting efficient inference. Signals: token cost reduction, pre-call control, ReAct-style VLM agent, lightweight external controller, cross-domain accuracy preservation
...
### [132] [Benchmarking Visual State Tracking in Multimodal Video Understanding](https://arxiv.org/abs/2606.03920)
...
Importance reason: Directly addresses a critical gap in MLLM video understanding (visual state tracking) with a new benchmark, but lacks direct connection to user's priority subtopics like continuous models or generation. Signals: video understanding benchmark, MLLM evaluation, visual state tracking, temporal reasoning, multimodal understanding
...
### [133] [Zero-Shot 3D Question Answering via Hierarchical View-to-Token Transportation](https://arxiv.org/abs/2606.03100)
...
TL;DR: 提出 KeyVT，一种层次化方法，通过视图级和 token 级的两阶段选择，在有限输入预算下保留最多任务相关3D细节，实现零样本3D问答。视图级结合像素特征与相机参数评估视图重要性，token级通过最优传输框架选取代表性 token 覆盖所有视图特征。


<details>
  <summary>Details</summary>
Motivation: 现有零样本3D场景理解方法从点云采样多个2D视图输入预训练VLM，但受限于输入预算，如何保留尽可能多的任务相关3D细节是一个关键挑战。

Method: 提出 KeyVT 层次化方法：(1) 视图级：结合像素特征与相机参数，基于语义内容和几何位置评估视图重要性，选择空间一致且任务相关的视图；(2) token级：在最优传输框架下，将视图 token 和关键 token 视为嵌入空间中的两个离散分布，通过最小化 OT 距离选取代表性 token 覆盖所有视图特征。

Result: 在三个广泛使用的3D问答基准上，KeyVT 显著优于现有无微调方法，性能可与基于训练的方法相媲美。

Conclusion: 层次化的视图与 token 选择策略能有效在有限输入预算下保留3D细节，提升零样本3D问答性能，验证了最优传输在 token 选择中的有效性。

Classification reason: 该论文研究零样本3D问答，利用2D VLM进行3D场景理解与空间推理，属于多模态理解与生成方向下的"空间与3D多模态推理"（spatial_3d_reasoning）规范子主题。

Importance reason: Novel hierarchical view-to-token selection for zero-shot 3D QA, but tangential to user's priority on continuous/multimodal models and video generation. Signals: zero-shot 3D QA, hierarchical view-token selection, optimal transport for token coverage, 2D VLM for 3D understanding, limited input budget optimization
...
### [134] [MUSE: A Unified Agentic Harness for MLLMs](https://arxiv.org/abs/2606.03005)
...
TL;DR: MUSE 是一个面向多模态大语言模型（MLLM）的统一智能体执行框架，通过可组合的任务表示、视觉处理、感知工具、结构化解析、确定性验证和验证引导修复等模块，在不重新训练模型的情况下显著提升冻结 MLLM 在视觉空间规划、感知、推理和细粒度判别等任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 尽管 MLLM 取得了快速进展，但在人类轻松完成的任务（如从截图导航网格迷宫、选择正确拼图块）上仍然失败。现有工作主要关注模型本身的改进（训练、微调），而本文提出一个互补问题：仅通过改进执行脚手架（execution scaffold），能从冻结的 MLLM 中激发多少能力？

Method: MUSE 是一个多模态统一结构化执行框架，包含可组合模块：任务表示（task representation）、视觉处理（visual processing）、感知工具使用（perception tool use）、结构化解析（structured parsing）、确定性验证（deterministic verification）和验证引导修复（verifier-guided repair）。该框架包装任意现成 MLLM，无需重新训练模型。

Result: 在视觉空间规划、视觉感知、多模态推理和细粒度视觉判别等多个基准上，使用多种 SOTA MLLM 进行评估，MUSE 在所有设置中均优于裸模型，在困难实例上提升最大。进一步分析表明，许多 MLLM 失败源于框架层面的缺陷而非模型本身的能力不足，可通过验证引导修复来解决。

Conclusion: 智能体多模态框架（agentic multimodal harness）是一个关键但尚未充分探索的设计维度，为超越模型中心优化的 MLLM 改进提供了正交途径。

Classification reason: 该论文提出 MUSE，一个为 MLLM 设计的统一结构化执行框架（agentic harness），通过可组合模块增强冻结 MLLM 的能力，属于通用 MLLM 能力增强与系统设计，与 [CANONICAL] subtopic "视觉语言模型与 MLLM" 高度匹配。

Importance reason: Proposes a modular agentic harness to improve frozen MLLMs, aligning with user's interest in efficient inference and unified multimodal models. Signals: Agentic harness for MLLMs, No retraining required, Composable modules, Verifier-guided repair, Consistent gains across benchmarks
...
### [135] [GLINT: Sparsely Gated Vision-Language Alignment for Fine-Grained Radiology Representations](https://arxiv.org/abs/2606.03180)
...
TL;DR: GLINT 提出稀疏门控对齐（Sparsely Gated Alignment）和密集特征正则化（Dense Feature Regularization），解决放射学图像-报告对中全局监督与局部病灶不匹配的问题，在零样本分类、定位和分割任务上取得显著提升，并首次实现 3D CT 零样本分割。


<details>
  <summary>Details</summary>
Motivation: 放射学 VLM 面临图像-报告对的尺度不匹配问题：每个病灶只占据图像的一小部分区域，但监督信号仅来自全局图像-报告级别。现有方法对所有图像块密集分配权重，而非聚焦于与查询相关的稀疏子集，导致细粒度定位能力不足。

Method: GLINT 包含两个核心组件：(1) 稀疏门控对齐（Sparsely Gated Alignment），通过一个独立的门控嵌入空间上的 sigmoid 门控机制，仅激活与每个文本查询相关的图像块，强制实现稀疏对应；(2) 密集特征正则化（Dense Feature Regularization），将可训练编码器的中间特征锚定到冻结的自监督学习（SSL）教师模型（2D 用 DINOv3，3D 用 V-JEPA 2.1），保留细粒度图像块特征。该方法同时适用于 2D 胸部 X 光和 3D 胸部 CT。

Result: GLINT 在零样本分类、定位和分割任务上均优于现有 SSL 编码器和医学 VLM，尤其是在需要稀疏查询特定定位的零样本定位和分割任务上提升最为显著。据作者称，这是首个无需掩码监督即可在 3D CT 体积上实现零样本分割的方法。在下游任务评估中，GLINT 在分类、报告生成和分割上均超越基线。

Conclusion: GLINT 通过显式建模稀疏对应关系，有效解决了放射学 VLM 中全局监督与局部病灶不匹配的核心挑战，为医学影像的细粒度理解提供了新范式，并展示了在 2D 和 3D 场景下的通用性。

Classification reason: 该论文提出 GLINT，一种面向放射学图像的视觉语言模型对齐框架，属于通用视觉语言模型（VLM）架构设计，与 [CANONICAL] subtopic "视觉语言模型与 MLLM" 高度匹配。

Importance reason: Focuses on medical radiology, which is outside your priority subtopics and has negative keyword overlap. Signals: Sparse gating for fine-grained alignment, Zero-shot 3D CT segmentation, Dense feature regularization with SSL teacher, Domain-specific medical VLM, Not in priority subtopics
...
### [136] [PRISM: Synergizing Vision Foundation Models via Self-organized Expert Specialization](https://arxiv.org/abs/2606.03444)
...
TL;DR: PRISM 提出一种双流 MoE 框架，通过"专家解构-动态重组"两阶段范式，将多种视觉基础模型（如 DINO、CLIP、MAE 等）的互补特征融合到单一高效模型中，缓解了传统蒸馏方法中的负迁移问题，在 PASCAL-Context 和 NYUD-v2 上取得了 SOTA。


<details>
  <summary>Details</summary>
Motivation: 不同视觉基础模型（VFMs）具有互补的视觉表征能力，但直接蒸馏融合会导致负迁移（feature conflicts）。现有方法缺乏有效的机制来协调不同 VFM 之间的特征冲突，因此需要一种能够实现模块化专家分工的融合框架。

Method: PRISM 采用双流 MoE 框架，包含两阶段范式：(1) 专家解构阶段——使用教师条件路由器（teacher-conditional router）引导每个 MoE 专家专注于不同的表征子空间，从而缓解特征干扰；(2) 动态重组阶段——路由器学习为下游任务组装定制化的计算路径，实现灵活的特征组合。

Result: 在 PASCAL-Context 和 NYUD-v2 数据集上，PRISM 取得了新的 SOTA 结果，验证了稀疏、涌现的专家专业化是一种可扩展的视觉知识集成方法。

Conclusion: PRISM 通过 MoE 的模块化专家分工有效解决了多 VFM 融合中的负迁移问题，证明了稀疏专家专业化是整合多样化视觉知识的高效可扩展范式。

Classification reason: 该论文提出 PRISM 框架，通过 MoE 架构融合多种视觉基础模型（VFMs）的特征，属于视觉语言/多模态模型架构设计，与 canonical 子主题 "视觉语言模型与 MLLM" 高度匹配。

Importance reason: Focuses on VFM integration via MoE, not directly on user's priority subtopics like continuous language models or video generation. Signals: MoE framework, VFM synergy, negative transfer mitigation, PASCAL-Context SOTA, not aligned with priority subtopics
...
### [137] [Visual Instruction Tuning Aligns Modalities through Abstraction](https://arxiv.org/abs/2606.03871)
...
TL;DR: 本文通过探针分析和因果干预，系统研究了视觉指令微调如何将视觉特征嵌入LLM的层次化抽象中。发现指令微调主要将视觉特征直接注入LLM的中间语义层，绕过早期单模态处理层；这些中间层是多模态处理的语义核心。仅微调中间层即可在视觉中心基准上保持全微调性能，同时减少训练时间。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉指令微调已被广泛用于使LLM处理图像信息，但视觉特征如何嵌入LLM的层次化抽象结构中仍不清楚。理解这一机制有助于揭示多模态融合的本质，并指导更高效的微调策略。

Method: 在多种视觉语言架构上，使用探针分析（probing）和因果干预（causal interventions）研究视觉特征在LLM各层的嵌入位置；通过比较语义等价视觉与文本表征的几何结构，分析微调对抽象阶段的影响；并验证仅微调中间层的策略效果。

Result: 视觉指令微调将视觉特征直接注入LLM的中间语义层，绕过早期单模态处理层；中间层是多模态处理的语义核心；微调扩展并强化了LLM已有的抽象阶段，使视觉特征与已有文本特征对齐；仅微调中间层可在视觉中心基准上保持全微调性能并减少训练时间。

Conclusion: 多模态融合是一种局部化现象，通过重新利用LLM内部的抽象引擎实现。这为理解多模态模型的工作原理提供了新视角，并为高效微调提供了实用指导。

Classification reason: 该论文研究视觉指令微调如何将视觉特征嵌入LLM的层次化抽象中，属于视觉语言模型（VLM/MLLM）的架构分析与理解，与[CANONICAL] subtopic "vlm_mllm_general（视觉语言模型与MLLM）"高度匹配。

Importance reason: Directly analyzes how visual instruction tuning aligns modalities in MLLMs, a core priority subtopic, with actionable insights for efficient fine-tuning. Signals: MLLM alignment, visual instruction tuning, intermediate layer abstraction, efficient fine-tuning, multimodal integration mechanism
...
### [138] [Beyond Encoder Accumulation: Measuring Encoder Roles in Multi-Encoder VLMs](https://arxiv.org/abs/2606.03879)
...
TL;DR: 本文系统研究了多编码器视觉语言模型中不同视觉编码器在联合训练下的角色。通过在 Cambrian-1 基准上重新训练所有 31 个非空编码器子集（约 2 万 GPU 小时），发现：(1) 从头训练得到的编码器排名与在固定检查点上掩码得到的排名不同；(2) 提出 Capacity（单独能力）和 Necessity（移除后性能下降）两个正交维度，发现高 Capacity 编码器配对并非最优，而高 Capacity 锚点+自适应互补编码器可匹配全五编码器性能；(3) 预投影器有效秩（pre-projector effective rank）解释了残差性能变化，最优配对是联合训练后秩保持的锚点与秩扩张的互补编码器组合。


<details>
  <summary>Details</summary>
Motivation: 随着多模态基础模型融合越来越多的异构视觉流（如不同视觉编码器），理解这些编码器在联合训练下的交互方式成为设计原则的前提。然而，当前缺乏分析工具，且参数高效的编码器配置在训练前难以识别。本文旨在填补这一方法论空白。

Method: 在 Cambrian-1 基准套件（16 个基准）上，采用统一流水线重新训练并评估五种常见视觉编码器的所有 31 个非空子集（约 2 万 GPU 小时）。提出 Capacity-Necessity 分解框架：Capacity 衡量编码器单独达到的分数，Necessity 衡量从完整池中移除时的性能下降。进一步分析预投影器有效秩（pre-projector effective rank）与性能的关系。

Result: (1) 从头训练得到的编码器排名与固定检查点掩码排名不同，包括哪个编码器排名第一。(2) Capacity 和 Necessity 两个维度不可互换。两个最高 Capacity 编码器配对是次优的，而高 Capacity 锚点+自适应互补编码器可匹配全五编码器性能，额外添加编码器仅带来边际收益。(3) 在固定参数量下，预投影器有效秩解释了残差性能变化。最优配对是联合训练后秩保持的锚点与秩扩张的互补编码器组合。

Conclusion: Capacity-Necessity 分解和预投影器秩分析揭示了多编码器 LVLM 设计中的方法论差距，并提供了具体的解决原语。这些发现有助于在训练前识别参数高效的编码器配置，为多编码器视觉语言模型的设计提供指导。

Classification reason: 该论文研究多编码器视觉语言模型（LVLM）中不同视觉编码器的角色与贡献，属于通用视觉语言模型架构分析，映射到 [CANONICAL] 的 vlm_mllm_general（视觉语言模型与 MLLM）。

Importance reason: Provides systematic analysis of multi-encoder VLM design, but focuses on encoder roles rather than the user's priority of continuous multimodal models or generation. Signals: Multi-encoder VLM analysis, Capacity-Necessity decomposition, Pre-projector effective rank, Cambrian-1 benchmark, Joint training insights
...
### [139] [CP-Agent: Context-Aware Multimodal Reasoning for Cellular Morphological Profiling under Chemical Perturbations](https://arxiv.org/abs/2606.03435)
...
TL;DR: CP-Agent 是一个基于 MLLM 的智能体系统，通过上下文感知对齐模块 CP-CLIP 联合编码高内涵细胞图像与实验元数据，实现药物扰动下细胞形态变化的可解释推理，用于加速药物发现中的表型筛选。


<details>
  <summary>Details</summary>
Motivation: 现有细胞表型筛选工作流速度慢、成本高且难以解释，药物筛选建模主要关注分子表征学习而忽略了实验上下文（如细胞系、给药方案等），限制了泛化能力和作用机制（MoA）分辨率。

Method: 提出 CP-Agent，核心是 CP-CLIP 上下文感知对齐模块，联合嵌入高内涵图像与实验元数据；结合智能体工具使用与多模态推理，生成结构化报告以指导实验设计与假设优化。

Result: CP-CLIP 在治疗和 MoA 判别任务上达到最高 0.896 的 F1 分数，验证了上下文感知对齐的有效性。

Conclusion: CP-Agent 展示了可解释、可扩展且上下文感知的表型筛选能力，有望加速药物发现中的迭代假设生成流程。

Classification reason: 该论文提出了 CP-Agent，一个面向细胞形态学分析的智能体多模态大语言模型（MLLM），属于通用视觉语言模型与 MLLM 架构范畴，因此映射到 canonical 子主题 vlm_mllm_general。

Importance reason: Focuses on drug discovery cell morphology, which is outside your priority subtopics and has negative keyword overlap. Signals: MLLM for drug screening, context-aware alignment, interpretable phenotypic reasoning, not autoregressive generation, not world model
...
### [140] [Demo2Tutorial: From Human Experience to Multimodal Software Tutorials](https://arxiv.org/abs/2606.03951)
...
TL;DR: Demo2Tutorial 是一个将人类在数字环境中的屏幕录制和交互日志自动转化为结构化多模态软件教程的框架，包含动作解析、步骤规划和教程合成三个模块，生成的教程在人类学习和 GUI 智能体规划方面均优于人类编写的教程和基线方法。


<details>
  <summary>Details</summary>
Motivation: 人类在数字环境中的交互经验（屏幕录制+交互日志）蕴含丰富的程序性知识，但这类非结构化数据难以直接用于教学人类或训练智能体。现有方法缺乏从原始人类经验中自动提取并生成结构化、可复用多模态教程的能力。

Method: Demo2Tutorial 包含三个模块：(1) 多模态动作解析器（Action Parser），利用 MLLM 从屏幕录制和交互日志中重建感知、动作和意图；(2) 步骤规划器（Step Planner），将解析后的步骤抽象为层次化任务图，表示目标和子步骤；(3) 教程合成器（Tutorial Composer），将解析后的经验转化为结构化的图文指令教程。

Result: 在官方软件文档构建的新基准上，Demo2Tutorial 生成的教程质量超越人类编写的教程和基线方法。人类学习实验中，使用该教程的用户任务完成速度更快；GUI 智能体实验中，教程提升了智能体的规划和泛化能力。

Conclusion: 从人类经验中蒸馏出的结构化多模态教程是一种有效的知识表示形式，既能促进人类学习，也能提升 GUI 智能体的能力，展示了从原始交互数据到可复用教学资源的自动化路径。

Classification reason: 该论文提出 Demo2Tutorial 框架，将屏幕录制和交互日志转化为结构化的多模态软件教程，涉及 GUI 理解、文档生成与指令构建，属于多模态理解与生成方向下的视觉问答与文档理解（multimodal_vqa_doc）范畴，因为其核心是解析 GUI 屏幕内容并生成结构化图文指令，与文档/图表/界面理解高度相关。

Importance reason: Focuses on GUI tutorial generation from screen recordings, which is outside the user's priority subtopics and lacks core technical novelty in continuous or generative models. Signals: Multimodal tutorial generation, GUI agent planning, Human learning from video, Action parsing, Step planning
...
### [141] [Distilling Answer-Set Programming Rules from LLMs for Neurosymbolic Visual Question Answering](https://arxiv.org/abs/2606.03269)
...
TL;DR: 本文提出一种从大语言模型中蒸馏 Answer-Set Programming 规则的方法，用于神经符号视觉问答。通过提示 LLM 扩展初始 ASP 推理理论，并利用 VQA 示例和 ASP 求解器反馈验证与修正规则，仅需少量示例即可生成正确规则，提升了 VQA 系统的可解释性和适应性。


<details>
  <summary>Details</summary>
Motivation: 模块化的 VQA 方法使用逻辑表示进行推理，具有可解释性优势，但任务需求变化时手动扩展这些表示对开发者负担很大。本文旨在自动化这一过程，利用 LLM 的知识蒸馏出逻辑规则。

Method: 提出一种规则蒸馏方法：用 LLM 提示来扩展初始的 ASP 推理理论以适应新任务需求；利用 VQA 数据集中的示例指导 LLM 生成规则；通过 ASP 求解器验证结果并利用反馈纠正错误规则。

Result: 在多个不同的 VQA 数据集上验证了方法的有效性，仅需少量示例即可从 LLM 中引出正确的 ASP 规则，表明规则蒸馏是传统数据驱动规则学习的有效替代方案。

Conclusion: 从 LLM 中蒸馏 ASP 规则是一种有前景的神经符号 VQA 方法，兼具可解释性和自动化扩展能力，未来可进一步探索更复杂的推理场景。

Classification reason: 该论文研究视觉问答（VQA），属于多模态理解与生成方向。论文使用逻辑规则（Answer-Set Programming）增强VQA推理的可解释性，与现有规范子主题"视觉问答与文档理解"（multimodal_vqa_doc）高度匹配，因此复用该子主题。

Importance reason: Focuses on neurosymbolic VQA rule distillation, which is not aligned with user's priority on continuous multimodal models and generative world models. Signals: neurosymbolic VQA, rule distillation from LLMs, answer-set programming, interpretability, limited to VQA domain
...
### [142] [Imaginative Perception Tokens Enhance Spatial Reasoning in Multimodal Language Models](https://arxiv.org/abs/2606.03988)
...
Importance: 值得关注 (70.83) - 未入选精读

TL;DR: 本文提出 Imaginative Perception Tokens (IPT)，一种在连续潜空间中生成中间感知表征的方法，用于增强多模态语言模型的空间推理能力。IPT 通过让模型在不可见视角、遮挡路径等场景下推理感知内容，显著提升了空间推理性能，且无需在推理时生成图像。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在空间推理任务中表现不佳，尤其是当关键信息不可直接观察时（如推断不可见视角、追踪遮挡路径等）。传统的文本思维链（CoT）在空间计算中可能存在模态不匹配问题，因此需要一种更自然的中间表征来桥接视觉输入与推理输出。

Method: 提出 Imaginative Perception Tokens (IPT)，即在连续潜空间中生成中间感知表征，外部化模型在替代空间配置下会感知到的内容。设计了三个任务：Perspective Taking (PET)、Path Tracing (PT) 和 Multiview Counting (MVC)，并构建了约 20K 样本的数据集。以统一 VLM BAGEL 为骨干，在训练中引入 IPT 监督信号。

Result: IPT 监督一致性地提升了空间推理性能，通常优于文本思维链训练。在 MVC 任务上准确率提升 3.4%，在 PT 任务上达到与强闭源模型竞争的性能。结合 IPT 和标签监督可获得额外增益，而文本 CoT 在某些情况下会显著降低性能。

Conclusion: IPT 为推理不可观察空间结构提供了一种原则性的监督信号，在提升泛化能力的同时产生可解释的中间表征。文本 CoT 在空间计算中可能存在模态不匹配问题，而连续潜空间中的感知 token 是更自然的推理媒介。

Classification reason: 该论文提出 Imaginative Perception Tokens (IPT)，在连续潜空间中生成中间感知表征以增强空间推理，属于多模态模型在连续 latent 中进行推理的范畴，与 [CANONICAL] 连续多模态推理与潜空间思维 (continuous_multimodal_reasoning) 高度匹配。同时论文也涉及多模态理解与生成方向。

Importance reason: Directly advances continuous multimodal latent reasoning with IPT tokens, a top priority subtopic, and shows strong spatial reasoning gains. Signals: continuous latent representation, spatial reasoning, multimodal language model, intermediate perceptual tokens, chain of continuous vision-language thought
